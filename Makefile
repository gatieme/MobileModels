...
export GBB_CUST_NAME := cs_user
export MODEM_LOGCAT_INUSER := true
export GBB_PRODUCT_NAME := charlotte
export TARGET_ARM_TYPE = arm64
export extra_modem := 
export product_type := 
export chip_type := 
export CFG_PLATFORM := charlotte
export OBB_PRODUCT_NAME := charlotte
export TARGET_BOARD_PLATFORM := charlotte
VERSION = 5
PATCHLEVEL = 10
SUBLEVEL = 43
EXTRAVERSION =
...
DEPMOD=depmod
KMI_GENERATION=9
ifeq ($(strip $(USE_BISHENG_CLANG)), true)
CLANG_PREBUILTS_PATH ?= $(srctree)/../../tools/huaweiplatform/bisheng/bisheng_aarch64/mobile_cpu_llvm/linux-x86/bisheng_clang/
else
...
export PATH := $(srctree)/../../tools/open_source/lz4/lz4-1.9.4/:$(PATH)
...
LINUXINCLUDE += -I$(srctree)/mm \
		...
		-I$(srctree)/platform_source \
		-I$(srctree)/include/platform_include \
		-I$(srctree)/include/platform_include/basicplatform

ifeq ($(chip_type),)
LINUXINCLUDE += -I$(srctree)/drivers/platform_drivers/ap/platform/$(TARGET_BOARD_PLATFORM)
else
ifeq ($(strip $(TARGET_BOARD_PLATFORM)_$(chip_type)),charlotte_csv2)
LINUXINCLUDE += -I$(srctree)/drivers/platform_drivers/ap/platform/$(TARGET_BOARD_PLATFORM)
else
LINUXINCLUDE += -I$(srctree)/drivers/platform_drivers/ap/platform/$(TARGET_BOARD_PLATFORM)_$(chip_type)
endif
endif
...
ifeq ($(USE_BISHENG_CLANG),true)
    CLANG_FLAGS += -Wno-error=strict-prototypes
    CLANG_FLAGS += -Wno-error=int-conversion
    CLANG_FLAGS += -Wno-error=implicit-int
endif
...
drivers-y	+= net/ virt/ platform_source/
libs-y		:= lib/
endif # KBUILD_EXTMOD
...
ifeq ($(strip $(USE_BISHENG_CLANG)), true)
KBUILD_CFLAGS += -fprofile-use=$(srctree)/../../vendor/hisi/bin/pgo/BiSheng-Mobile/kernel_5_10/kernel_20230703.profdata
...
KBUILD_CFLAGS += -mllvm
KBUILD_CFLAGS += -hot-cold-split=true
KBUILD_CFLAGS += -mno-outline
KBUILD_CFLAGS += -mllvm
KBUILD_CFLAGS += -inlinecold-threshold=300
else
KBUILD_CFLAGS += -fprofile-use=$(srctree)/../../vendor/hisi/bin/pgo/clang-r416183b1/kernel_5_10/kernel_20230706.profdata
KBUILD_CFLAGS += -Wno-profile-instr-out-of-date
KBUILD_CFLAGS += -Wno-profile-instr-unprofiled
KBUILD_CFLAGS += -Wno-error=backend-plugin
KBUILD_CFLAGS += -ffunction-sections
...
KBUILD_LDFLAGS += --symbol-ordering-file=$(srctree)/../../vendor/hisi/bin/pgo/clang-r416183b1/kernel_5_10/kernel_symbol_order.txt
KBUILD_LDFLAGS += --no-warn-symbol-ordering
endif
endif
endif
...
ifeq ($(legacy), false)
KBUILD_CFLAGS += -DCONFIG_PARTITION_PLAT_UNLEGACY
endif
ifeq ($(partition_type), recovery_ab)
KBUILD_CFLAGS += -DCONFIG_RECOVERY_AB_PARTITION
endif

ifeq ($(chip_type),cs2)
KBUILD_CFLAGS += -DCONFIG_PLAT_PARTITION_CS2
endif

ifeq ($(strip $(TARGET_BOARD_PLATFORM)_$(chip_type)),charlotte_csv2)
KBUILD_CFLAGS += -DCONFIG_PLAT_PARTITION_CSV2
endif

ifneq ($(extra_modem),)
KBUILD_CFLAGS += -DCONFIG_PARTITION_A_PLUS_B_EXTERNAL_MODEM
endif
...
ifneq ($(KERNEL_BUILD_TOP),)
KBUILD_CFLAGS   += $(call cc-option,-fdebug-prefix-map=$(KERNEL_BUILD_TOP)/=)
endif
