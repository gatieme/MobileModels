...
static irqreturn_t armv8pmu_handle_irq(struct arm_pmu *cpu_pmu)
{
	...
	CALL_HCK_VH(lpcpu_pmu_arm_pmu_per_cpu_num_events, cpu_pmu, &num_events, &idx);
	/*
...
#ifdef CONFIG_PHASE_TASK_PERF
		if (event->hw.target && event->attr.config == sysctl_phase_task_event)
			phase_task_event(event->hw.target, PHASE_ARM_IRQ);
#endif
				  ...
				  struct perf_event *event)
{
	...
	if (armv8pmu_event_is_64bit(event) &&
	    !armv8pmu_has_long_event(cpu_pmu)) {
		CALL_HCK_VH(lpcpu_pmu_armv8pmu_get_chain_idx, cpuc, cpu_pmu, event,
			    &idx, &vendor_flag);
		if (vendor_flag == 0)
			return	armv8pmu_get_chain_idx(cpuc, cpu_pmu);
	} else {
		CALL_HCK_VH(lpcpu_pmu_armv8pmu_get_single_idx, cpuc, cpu_pmu, event,
			    &idx, &vendor_flag);
		if (vendor_flag == 0)
...
static void __armv8pmu_probe_pmu(void *info)
{
	...
	CALL_HCK_VH(lpcpu_pmu_armv8pmu_pre_probe, &probe->supported_cpus, cpu_pmu, &vendor_flag);
	if (vendor_flag == 0) {
...
static int armv8pmu_probe_pmu(struct arm_pmu *cpu_pmu)
{
	...
	CALL_HCK_VH(lpcpu_pmu_armv8pmu_probe, &probe.supported_cpus, cpu_pmu,
		    __armv8pmu_probe_pmu, &probe, &ret, &vendor_flag);
	if (vendor_flag != 0)
		return ret;
