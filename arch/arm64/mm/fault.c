...
spf_retry:
	...
	while ((seq & 1) && --read_seq_retries > 0) {
		usleep_range(50, 50);
		seq = mmap_seq_read_start(mm);
	}
	if (seq & 1) {
...
no_spf:
	...
	if (!mmap_read_trylock(mm)) {
		if (!user_mode(regs) && !search_exception_tables(regs->pc))
			goto no_context;
#ifdef CONFIG_SPECULATIVE_PAGE_FAULT
		if ((fault & VM_FAULT_SPF_RETRY) && --spf_retries > 0) {
			goto spf_retry;
		}
#endif
retry:
					  ...
					  struct pt_regs *regs)
{
	...
	if (!user_mode(regs) &&
		...
		pr_debug("kernel fault address 0x%lx esr 0x%x stack %p\n",
