...
#ifdef CONFIG_HKIP_MAP_KERNEL_PROTECT
static phys_addr_t __init hkip_early_pgtable_alloc(int shift)
{
	phys_addr_t phys;
	void *ptr;

	phys = hkip_pgtable_phys_alloc(PAGE_SIZE, PAGE_SIZE);
	if (!phys)
		panic("Failed to allocate page table page\n");
...
static void __init hkip_map_memblock(pgd_t *pgdp, phys_addr_t start,
				  phys_addr_t end, pgprot_t prot, int flags)
{
	__create_pgd_mapping(pgdp, start, __phys_to_virt(start), end - start,
			     prot, hkip_early_pgtable_alloc, flags);
}
#endif

#ifdef CONFIG_NEED_CHANGE_MAPPING
static void sec_memblk_map_clear(pgd_t *pgd, int flags)
{
	int i;

	for(i = 0; i < MAX_SEC_MEMBLK_SP; i++) {
		struct sec_memory_block *sec_mem = &sec_memblk_sp[i];
		if (sec_mem->size) {
			/*
			 ...
			 */
			__map_memblock(pgd, sec_mem->base,
					sec_mem->base + sec_mem->size,
					PAGE_KERNEL, flags);
			memblock_clear_nomap(sec_mem->base, sec_mem->size);
		}
	}
}

static void sec_memblk_mark_nomap(void)
{
	int i;
	...
	for (i = 0; i < MAX_SEC_MEMBLK_SP; i++) {
		struct sec_memory_block *sec_mem = &sec_memblk_sp[i];
		if (sec_mem->size)
			memblock_mark_nomap(sec_mem->base, sec_mem->size);
	}
}
#endif
...
static void __init map_mem(pgd_t *pgdp)
{
...
#ifdef CONFIG_HKIP_MAP_KERNEL_PROTECT
	phys_addr_t pgtbl_protect_start = 0;
	phys_addr_t pgtbl_protect_end = 0;
	int ret = 0;
#endif
	...
	if (rodata_full || debug_pagealloc_enabled() ||
	    ...
	    IS_ENABLED(CONFIG_MM_LB_L3_EXTENSION))
		flags = NO_BLOCK_MAPPINGS | NO_CONT_MAPPINGS;
	 ...
	 */
#ifdef CONFIG_HKIP_MAP_KERNEL_PROTECT
	ret = hkip_get_pgtable_protect_addr(&pgtbl_protect_start, &pgtbl_protect_end);
	BUG_ON(ret != 0);
	BUG_ON(pgtbl_protect_start == 0);
	BUG_ON(pgtbl_protect_end == 0);
	hkip_map_memblock(pgdp, kernel_start, kernel_end,
		       PAGE_KERNEL, NO_CONT_MAPPINGS);
	hkip_map_memblock(pgdp, pgtbl_protect_start, pgtbl_protect_end,
		       PAGE_KERNEL, NO_CONT_MAPPINGS);
#else
...
void mark_constdata_ro(void)
{
	...
	/* runtime pgds are already protected by hkip, no need to register ro again */
	unsigned long hkip_ro_size = (unsigned long)__rodata_protect_end - (unsigned long)start;

	/*
	 * mark .rodata as read only. Use __init_begin rather than __end_rodata
	 * to cover NOTES ,EXCEPTION_TABLE and runtime pgds.
	 */
	update_mapping_prot(__pa_symbol(__start_rodata), start,
			    section_size, PAGE_KERNEL_RO);
	hkip_register_ro((void *)start, ALIGN(hkip_ro_size, PAGE_SIZE));
#endif
	debug_checkwx();
}
...
#ifdef CONFIG_HKIP_MAP_KERNEL_PROTECT
static void __init hkip_map_kernel_segment(pgd_t *pgdp, void *va_start, void *va_end,
				      pgprot_t prot, struct vm_struct *vma,
				      int flags, unsigned long vm_flags)
{
	phys_addr_t pa_start = __pa_symbol(va_start);
	...
	__create_pgd_mapping(pgdp, pa_start, (unsigned long)va_start, size, prot,
			     hkip_early_pgtable_alloc, flags);
...
static void __init map_kernel(pgd_t *pgdp)
{
	 ...
	 */
#ifdef CONFIG_HKIP_MAP_KERNEL_PROTECT
	hkip_set_pgtable_protect_addr();
	hkip_map_kernel_segment(pgdp, _text, _etext, text_prot, &vmlinux_text, 0,
			   VM_NO_GUARD);
	hkip_map_kernel_segment(pgdp, __start_data_ro_after_init, __end_data_ro_after_init, PAGE_KERNEL,
			   &vmlinux_ro_after_init_data, NO_CONT_MAPPINGS, VM_NO_GUARD);
	hkip_map_kernel_segment(pgdp, __start_rodata, __inittext_begin, PAGE_KERNEL,
			   &vmlinux_rodata, NO_CONT_MAPPINGS, VM_NO_GUARD);
#else
	map_kernel_segment(pgdp, _text, _etext, text_prot, &vmlinux_text, 0,
			   VM_NO_GUARD);
	map_kernel_segment(pgdp, __start_data_ro_after_init, __end_data_ro_after_init, PAGE_KERNEL,
			   &vmlinux_ro_after_init_data, NO_CONT_MAPPINGS, VM_NO_GUARD);
...
#ifdef CONFIG_HKIP_PRMEM
	map_kernel_segment(pgdp, __start_data_wr, __end_data_wr, PAGE_KERNEL,
			   &vmlinux_wr_data, NO_CONT_MAPPINGS, VM_NO_GUARD);
	map_kernel_segment(pgdp, __start_data_wr_after_init, __end_data_wr_after_init, PAGE_KERNEL,
			   &vmlinux_wr_after_init_data, NO_CONT_MAPPINGS, VM_NO_GUARD);
	map_kernel_segment(pgdp, __start_data_rw, __end_data_rw, PAGE_KERNEL,
...
#ifndef CONFIG_MM_LB_L3_EXCLUSIVE
...
{
	pgprot_t sect_prot = mk_pud_sect_prot(prot);
...
{
	pgprot_t sect_prot = mk_pmd_sect_prot(prot);
