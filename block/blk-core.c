...
#ifdef CONFIG_BLK_CGROUP_IOSMART
	blkcg = bio->bi_blkg ? bio->bi_blkg->blkcg : NULL;
...
#ifdef CONFIG_MAS_BLK
		mas_blk_latency_bio_check(bio, BIO_PROC_STAGE_ENTER);
#endif
...
#ifdef CONFIG_HW_STAT_IOSTAT_BLK
		__hw_iostat_acct_size_blk(req, bytes);
#endif
		part_stat_unlock();
	}
}
...
#ifdef CONFIG_HW_STAT_IOSTAT_BLK
		__hw_iostat_acct_lat_blk(req, now - req->start_time_ns);
#endif
		part_stat_unlock();
