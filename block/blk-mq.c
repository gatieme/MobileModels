		...
		unsigned int tag, u64 alloc_time_ns)
{
	...
	rq->rq_flags = 0;
#ifdef CONFIG_MAS_BLK
	rq->mas_cmd_flags = data->mas_io_flag;
#endif
	...
	mas_blk_inline_crypto_req_init(rq);
#endif
		...
		blk_mq_req_flags_t flags)
{
	...
	if (q->mas_queue_ops && q->mas_queue_ops->mq_req_alloc_prep_fn)
		q->mas_queue_ops->mq_req_alloc_prep_fn(&data, data.cmd_flags, false);
	data.cmd_flags |= (REQ_SYNC | REQ_FG);
#endif
...
static __latent_entropy void blk_done_softirq(struct softirq_action *h)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_softirq_hook, (unsigned int)9, (u64)(uintptr_t)blk_done_softirq, 0);
	...
	CALL_HCK_VH(hck_dfx_bbox_softirq_hook, (unsigned int)9, (u64)(uintptr_t)blk_done_softirq, 1);
}
...
{
#ifdef CONFIG_MAS_BLK
	if (rq->q->mas_queue_ops && rq->q->mas_queue_ops->mq_req_complete_fn)
		rq->q->mas_queue_ops->mq_req_complete_fn(rq, rq->q, true);
	else
#endif
...
{
	if (is_flush_rq(rq, rq->mq_hctx) && rq->end_io)
		rq->end_io(rq, 0);
...
#ifdef CONFIG_MAS_BLK
	if (rq->q->mas_queue_ops && rq->q->mas_queue_ops->mq_tag_get_fn)
		rdr_syserr_process_for_ap(
				(u32)MODID_AP_S_PANIC_STORAGE, 0ull,
				0ull);
	else
#endif
			     ...
			     bool at_head)
{
	...
	if (hctx->queue->mas_queue_ops &&
		...
		ctx->queue->mas_queue_ops->mq_req_insert_fn(rq, hctx->queue);
	} else
#endif
	{
