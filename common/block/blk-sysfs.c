...
static
#endif
ssize_t queue_var_store(unsigned long *var, const char *page, size_t count)
{
	int err;
	unsigned long v;
		...
		size_t count)
{
	...
	spin_lock_irq(&q->queue_lock);
	if (qos == 0)
		blk_queue_flag_clear(QUEUE_FLAG_QOS, q);
	else
		blk_queue_flag_set(QUEUE_FLAG_QOS, q);
	spin_unlock_irq(&q->queue_lock);
...
#ifdef CONFIG_MAS_BLK
#define MAS_QUEUE_RO_ENTRY(_prefix, _name)                  \
static struct queue_sysfs_entry _prefix##_entry = {     \
	.attr   = { .name = _name, .mode = 0444 },      \
	.show   = NULL,                       \
	.store  = _prefix##_store,                      \
...
static struct queue_sysfs_entry queue_io_flush_time_entry = {
	.attr = {.name = "io_flush_time_debug", .mode = S_IRUGO },
	.show = __cfi_mas_queue_io_flush_time_debug_show,
};
...
#endif
	&queue_io_flush_time_entry.attr,
#ifdef CONFIG_MAS_MQ_USING_CP
