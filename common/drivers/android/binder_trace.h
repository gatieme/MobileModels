...
TRACE_EVENT(binder_set_latency,
	TP_PROTO(int proc, int thread, int old_latency, unsigned int old_latency_prio,
		 int desired_latency, bool verify),
	TP_ARGS(proc, thread, old_latency, old_latency_prio, desired_latency, verify),
	TP_STRUCT__entry(
		__field(int, proc)
		__field(int, thread)
		__field(int, old_latency)
		__field(int, old_latency_prio)
		__field(int, desired_latency)
		__field(bool, verify)
	),
	TP_fast_assign(
		__entry->proc = proc;
		__entry->thread = thread;
		__entry->old_latency = old_latency;
		__entry->old_latency_prio = old_latency_prio;
		__entry->desired_latency = desired_latency;
		__entry->verify = verify;
	),
	TP_printk("proc=%d thread=%d verify=%d old_latency=%d old_latency_prio=%d => desired_latency=%d",
		  __entry->proc, __entry->thread, __entry->verify,
		  __entry->old_latency, __entry->old_latency_prio, __entry->desired_latency)
);
#endif
