...
static int get_gpio_status(void)
{
	int i;
	int temp_value = 0;
	int gpio_value = 0;
	...
	pr_err("gpio_value = %d g_gpio_count = %d\n", gpio_value, g_gpio_count);
         ...
         struct device_attribute *attr, char *buf)
{
    ...
    switch(info->name) {
        case ANTENNA_BOARD_STATUS_DETECT:
			...
			} else if (g_board_version == STATUS_EXTENDED_GPIO) {
				ret = get_gpio_status();
			} else {
...
static int extended_gpio_dt_parse(struct antenna_device_info *di, struct device_node* np)
{
	int ret;
	int gpio_check;
	...
	pctrl = devm_pinctrl_get(di->dev);
	if (IS_ERR(pctrl)) {
		pr_err("failed to devm pinctrl get\n");
		return -EINVAL;
	}
	pctrl_state = pinctrl_lookup_state(pctrl, "ant_default");
...
static void ant_board_gpio_dt_parse(struct antenna_device_info *di)
{
	...
	g_gpio[0] = extended_gpio_dt_parse(di, np);
	if (g_gpio[0] >= 0) {
		pr_err("set extended_gpio OK, %d\n", g_gpio[0]);
		return;
	}
    for (i = 0; i < g_gpio_count; i++) {
