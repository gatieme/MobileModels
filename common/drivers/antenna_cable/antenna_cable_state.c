...
#define ANT_STATE_GET_SIZE sizeof(int)
#define MAX_ANT_DET_GPIO_NUM 7
...
static int ant_state_get(const struct ant_device* antdev)
{
	...
	for (i = 0; i < g_gpio_count; i++) {
		temp_val = gpio_get_value_cansleep(g_gpio[i]);
		pr_info(TAG "gpio %d value is  %d\n", g_gpio[i], temp_val);
		gpio_val += (temp_val << i);
	}
	pr_info(TAG "STATE = %d\n", gpio_val);
	return gpio_val;
...
static void ant_event_report(struct ant_device *antdev)
{
	...
	if (antdev->prev_state != antdev->curr_state) {
		...
		if (sprintf_s(buf, sizeof(buf), "ANTENNA_CABLE_STATE=%d", antdev->curr_state) == -1)
			pr_err(TAG "sprintf_s error,curr_state = %d \n", antdev->curr_state);
		uevent_envp[0] = buf;
...
static int extended_gpio_dt_parse(struct device *dev, struct device_node *np)
{
	int ret;
	int gpio_check;
	...
	if (IS_ERR(pctrl)) {
		pr_err("failed to devm pinctrl get\n");
		return -EINVAL;
	}

	pctrl_state = pinctrl_lookup_state(pctrl, "antenna_cable_detect");
	if (IS_ERR(pctrl_state)) {
		pr_err("failed to pinctrl lookup state default\n");
	...
	for (index = 0; index < g_gpio_count; index++) {
		gpio_check = of_property_read_u32_index (np, "extended_gpio",
			(u32)index, (u32 *)(&g_gpio[index]));
		if (gpio_check != 0) {
			pr_err("%s : extended_gpio read failed!\n", __func__);
			return -EINVAL;
		}
	}
...
static void ant_dt_parse(struct device *dev)
{
	...
	if (g_gpio_count > MAX_ANT_DET_GPIO_NUM) {
		pr_info("gpio_count erro!\n");
		return;
	}

	if (extended_gpio_dt_parse(dev, np) == 0) {
		pr_info("set extended gpio OK\n");
		return;
	}
...
static void ant_gpio_request_process(struct ant_device *antdev)
{
	int err;
	unsigned int i;
				...
				struct device_attribute *attr, char *buf)
{
	...
	return sprintf_s(buf, ANT_STATE_GET_SIZE, "%d\n", ant_state_get(antdev));
}
...
static int ant_dev_probe(struct platform_device *pdev)
{
	...
	if (g_extended_gpio != 0)
		ant_gpio_request_process(antdev);
