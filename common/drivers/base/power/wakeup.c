...
int wakeup_source_set(const char *name, u8 lock_timeout)
{
	struct wakeup_source *ws = NULL;
...
int wake_unlockByName(const char *name)
{
	struct wakeup_source *ws = NULL;
	int flag = 0;
...
void pm_system_irq_wakeup(unsigned int irq_number)
{
	...
	if (irq_number) {
		...
		if (desc && desc->action && desc->action->name)
			dubai_log_irq_wakeup(desc->action->name);
#endif
...
static int print_active_ws(struct seq_file *m, struct wakeup_source *ws)
{
	ktime_t active_t;
	ktime_t max_t;
	ktime_t total_t;
	ktime_t prevent_sleep_t;
	unsigned long flags;
	unsigned long active_cnt;

	spin_lock_irqsave(&ws->lock, flags);

	prevent_sleep_t = ws->prevent_sleep_time;
	total_t = ws->total_time;
	active_cnt = ws->active_count;
	max_t = ws->max_time;
	...
	ktime_t now = ktime_get();
	active_t = ktime_sub(now, ws->last_time);
	total_t = ktime_add(total_t, active_t);

	if (active_t > max_t)
		max_t = active_t;

	if (ws->autosleep_enabled)
		prevent_sleep_t = ktime_add(prevent_sleep_t, ktime_sub(now, ws->start_prevent_time));

	seq_printf(m, "Active resource: %-12s\t%lu\t\t%lu\t\t%lu\t\t"
			"%lu\t\t%lld\t\t%lld\t\t%lld\t\t%lld\t\t%lld\n",
			ws->name, active_cnt, ws->event_count,
			ws->wakeup_count, ws->expire_count,
			ktime_to_ms(active_t),
			ktime_to_ms(total_t),
			ktime_to_ms(max_t),
			ktime_to_ms(ws->last_time),
			ktime_to_ms(prevent_sleep_t));
...
static int ws_stats_proc_show(struct seq_file *sf, void *unused)
{
	int srcu_idx;
	struct wakeup_source *ws;
	seq_puts(sf, "name\t\t\t\t\tactive_count\tevent_count\twakeup_count\t"
		"expire_count\tactive_since\ttotal_time\tmax_time\t"
		"last_change\tprevent_suspend_time\n");

	srcu_idx = srcu_read_lock(&wakeup_srcu);
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
		print_wakeup_source_stats(sf, ws);
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
		print_active_ws(sf, ws);
	srcu_read_unlock(&wakeup_srcu, srcu_idx);
	print_wakeup_source_stats(sf, &deleted_ws);

	return 0;
}

static int wakeup_sources_stats_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ws_stats_proc_show, NULL);
}

static const struct proc_ops wakeup_sources_stats_proc_fops = {
	.proc_open = wakeup_sources_stats_proc_open,
	.proc_read = seq_read,
...
static int __init wakeup_sources_proc_init(void)
{
	proc_create("wakeup_sources", S_IRUGO,
		(struct proc_dir_entry *)NULL, &wakeup_sources_stats_proc_fops);
	return 0;
}
