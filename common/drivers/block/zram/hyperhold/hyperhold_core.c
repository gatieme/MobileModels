...
static void hyperhold_delay(int *delay_count)
{
 ...
 *
 */
void hyperhold_track(struct zram *zram, u32 index, struct page *page)
{
	...
	hyperhold_zram_lru_add(zram, index, memcg, page);
...
static unsigned long memcg_reclaim_size_vma_sign(struct mem_cgroup *memcg)
{
	/*
	 * zram_hot + eswap_hot + (zram_warm + eswap_warm) * warm_ratio <= (zram + eswap) * zram2ufs_ratio
	 */
	unsigned long zram_size = atomic64_read(&memcg->zram_stored_size);
	unsigned long zram_hot_size = atomic64_read(&memcg->zram_stored_hot_size);
	unsigned long zram_warm_size = zram_size - zram_hot_size;
	unsigned long hyperhold_size = atomic64_read(&memcg->hyperhold_stored_size);
	unsigned long hyperhold_hot_size = atomic64_read(&memcg->hyperhold_stored_hot_size);
	unsigned long hyperhold_warm_size = hyperhold_size - hyperhold_hot_size;
	unsigned long warm_ratio = atomic64_read(&memcg->vma_part_swap_ratio);
	unsigned long zram_hot_warm_size = zram_hot_size + zram_warm_size * warm_ratio / 100;
	unsigned long hyperhold_hot_warm_size = hyperhold_hot_size +
				hyperhold_warm_size * warm_ratio / 100;
	unsigned long size_thre = (zram_size + hyperhold_size) *
				atomic_read(&memcg->memcg_reclaimed.ub_zram2ufs_ratio) / 100;
	unsigned long reclaim_size = (size_thre > hyperhold_hot_warm_size) ?
				(size_thre - hyperhold_hot_warm_size + zram_size - zram_hot_warm_size) : 0;
					...
					unsigned long *mcg_reclaim_sz)
{
	...
	if (unlikely(memcg->in_swapin))
		return -EAGAIN;
#ifdef CONFIG_RAMTURBO
	if (!memcg->active_swapout && *require_size < MIN_RECLAIM_ZRAM_SZ)
#else
	...
	*mcg_reclaim_sz = memcg_reclaim_size(memcg);
	if (acc_swapin_enable()) {
		if (atomic_read(&memcg->active_swapout_type) == BG_SWAPOUT_TYPE_RATIO &&
			atomic64_read(&memcg->active_swapout_cnt) > get_tuning_swapout_cnt_value()) {
			if (atomic64_read(&memcg->memcg_reclaimed.app_score) > ACC_SWAPIN_SKIP_SCORE)
				*mcg_reclaim_sz = memcg_reclaim_size_vma_sign(memcg);
		} else if (atomic_read(&memcg->active_swapout_type) == FG_SWAPOUT_TYPE_RATIO) {
			if (memcg != root_mem_cgroup && strcmp(memcg->name, "activityApp"))
				*mcg_reclaim_sz = atomic64_read(&memcg->zram_stored_reout_size) -
					atomic64_read(&memcg->zram_stored_reout_hot_size);
...
#ifdef CONFIG_RAMTURBO
	if (!memcg->active_swapout && *mcg_reclaim_sz < MIN_RECLAIM_ZRAM_SZ)
#else
	if (*mcg_reclaim_sz < MIN_RECLAIM_ZRAM_SZ)
#endif
		return -EAGAIN;
...
static int hyperhold_permcg_active_reclaim(
			...
			u64 *actual_reclaimed_sz, enum hyperhold_reclaim_source source)
#else
...
#endif
{
	...
	while (*require_size) {
		mcg = get_mem_cgroup(memcg->id.id);
		if (mcg != memcg) {
			if (mcg)
				hh_print(HHLOG_ERR, "current memcg id %d, name %s, ref %d",
					mcg->id.id, mcg->name, refcount_read(&mcg->id.ref));
			break;
		}
#ifdef CONFIG_RAMTURBO
		if (hyperhold_reclaim_extent(memcg, sched,
			require_size, &mcg_reclaim_sz, actual_reclaimed_sz, &io_err))
#else
		if (hyperhold_reclaim_extent(memcg, sched,
...
static int hyperhold_permcg_reclaim(struct mem_cgroup *memcg,
			void *data, enum hyperhold_reclaim_source source)
{
	u64 actual_reclaimed_size;
	return hyperhold_permcg_active_reclaim(memcg, data, &actual_reclaimed_size, source);
}

static unsigned long calc_target_size(struct mem_cgroup *mcg)
{
	unsigned long target_size;
	unsigned long zram_stored_size;
	unsigned long zram_stored_hot_size;
	unsigned long zram_hot_warm_size;
	unsigned long size_with_ratio;
	unsigned int ratio = atomic_read(&mcg->memcg_reclaimed.ub_zram2ufs_ratio);

	zram_stored_size = atomic64_read(&mcg->zram_stored_size);
	zram_stored_hot_size = atomic64_read(&mcg->zram_stored_hot_size);
	zram_hot_warm_size = zram_stored_hot_size + (zram_stored_size - zram_stored_hot_size) *
		atomic64_read(&mcg->vma_part_swap_ratio) / 100;
	size_with_ratio = zram_stored_size * ratio / 100;

	if (size_with_ratio >= zram_hot_warm_size)
		target_size = zram_stored_size;
	else
		target_size = size_with_ratio + zram_stored_size - zram_hot_warm_size;

	return target_size;
}

static void hyperhold_memcg_active_swapout_by_ratio(struct mem_cgroup *mcg, int type)
{
	enum hyperhold_reclaim_source source = RECLAIM_SOURCE_APP_ACTIVE;
	unsigned long target_size;
	unsigned int ratio;
	u64 actual_swapout_size;
	...
	if (!mcg->zram) {
		hh_print(HHLOG_INFO, "%s mcg %s not support zram", __func__,
			(mcg->name == NULL) ? "NULL" : mcg->name);
		return;
	}
	...
	target_size = atomic64_read(&mcg->zram_stored_size) * ratio / 100;
	if (acc_swapin_enable()) {
		if (type == BG_SWAPOUT_TYPE_RATIO) {
			atomic64_inc(&mcg->active_swapout_cnt);
			if (atomic64_read(&mcg->active_swapout_cnt) > get_tuning_swapout_cnt_value())
				target_size = calc_target_size(mcg);
			atomic_set(&mcg->active_swapout_type, BG_SWAPOUT_TYPE_RATIO);
		} else if (type == FG_SWAPOUT_TYPE_RATIO) {
			target_size = atomic64_read(&mcg->zram_stored_reout_size) -
							atomic64_read(&mcg->zram_stored_reout_hot_size);
			atomic_set(&mcg->active_swapout_type, FG_SWAPOUT_TYPE_RATIO);
		}
	}

	hyperhold_permcg_active_reclaim(mcg, &target_size, &actual_swapout_size, source);
	hh_print(HHLOG_DEBUG, "active_swapout_type %lu, active_swapout_cnt %lu, actual_swapout_size %lu\n",
			atomic_read(&mcg->active_swapout_type), atomic64_read(&mcg->active_swapout_cnt), actual_swapout_size);
	atomic64_set(&mcg->last_active_swapout_size, actual_swapout_size);
	atomic_set(&mcg->active_swapout_type, 0);
	hh_print(HHLOG_INFO, "RAMTURBO: %s swapout mcg=%s, ratio=%u, size=%lu", __func__,
		mcg->name, ratio, actual_swapout_size);
}

static void hyperhold_memcg_active_swapout_by_size(struct mem_cgroup *mcg, u64 target_sz,
					u64 *actual_reclaimed_sz, unsigned long *cost_time)
{
	enum hyperhold_reclaim_source source = RECLAIM_SOURCE_APP_ACTIVE; /* app active reclaim */
	u64 reclaim_sz;
	u64 mcg_score;
	...
	hyperhold_permcg_active_reclaim(mcg, &reclaim_sz, actual_reclaimed_sz, source);
...
static void __hyperhold_active_swapout_bg_tasks(u64 target_reclaim_sz_mb)
{
	...
	hyperhold_memcg_active_swapout_by_size(root_mem_cgroup, cur_target_reclaim_sz,
		&single_reclaim_sz, &single_reclaim_cost);
	hh_print(HHLOG_INFO, "RAMTURBO: %s finish swapout ROOT, size=%luBytes, time=%lumsec\n",
		__func__, single_reclaim_sz, jiffies_to_msecs(single_reclaim_cost));
	...
	while (cur_target_reclaim_sz > SZ_1M && (mcg = get_next_memcg(mcg))) {
		...
		if (cur_mcg_score < BG_SWAPOUT_MIN_SCORE) {
			hh_print(HHLOG_INFO, "RAMTURBO: %s iter_mcg_score <%d, stop!", __func__, BG_SWAPOUT_MIN_SCORE);
			break;
		}
		...
		hh_print(HHLOG_INFO, "RAMTURBO: %s start swapout memcg=%s(score=%lu), target=%luBytes\n",
			__func__, mcg->name, cur_mcg_score, cur_target_reclaim_sz);
		hyperhold_memcg_active_swapout_by_size(mcg, cur_target_reclaim_sz,
			&single_reclaim_sz, &single_reclaim_cost);
	...
	hh_print(HHLOG_INFO, "RAMTURBO: %s finish swapout! total_cost=%lu, total_reclaim_cost=%lu, total_reclaimed_size=%lu\n",
		__func__, jiffies_to_msecs(end - start), jiffies_to_msecs(total_reclaim_cost),
		total_reclaim_sz);
}
	...
	struct mem_cgroup *mcg, unsigned int type, unsigned int param)
{
	...
	if (!hyperhold_crypto_enable()) {
		hh_print(HHLOG_ERR, "hyperhold crypto error");
		return;
	}

	if (type == BG_SWAPOUT_TYPE_RATIO) {
		hyperhold_memcg_active_swapout_by_ratio(mcg, type);
	} else if (type == FG_SWAPOUT_TYPE_RATIO) {
		if (acc_swapin_enable())
			hyperhold_memcg_active_swapout_by_ratio(mcg, type);
...
{
	hyperhold_memcg_active_swapout_by_ratio(mcg, 0);
}
#endif
...
unsigned long hyperhold_reclaim_in(unsigned long size)
{
	...
	if (used_exts_num  * 100  > max_exts_num * MAX_USED_EXT_RATIO)
		return 0;
...
void hyperhold_cache_drop_exceed(void)
{
	if (!hyperhold_enable())
		return;
				...
				unsigned long size, bool preload)
{
	...
	if (unlikely(!mcg || !mcg->zram)) {
		if (mcg)
			...
			hh_print(HHLOG_ERR, "no zram in mcg! no memcg\n");
		ret = -EINVAL;
		goto out;
	}
	...
	ret = hyperhold_plug_finish(sched->io_handler, RECLAIM_SOURCE_DEFAULT);
	if (unlikely(ret)) {
		hh_print(HHLOG_ERR,
				...
				unsigned long size, bool preload)
{
	...
	hh_print(HHLOG_DEBUG, "active_swapout_cnt %lu, swapin size %lu, \
		hyperhold_stored_size %lu\n",
		atomic64_read(&mcg->active_swapout_cnt), size,
		atomic64_read(&mcg->hyperhold_stored_size));
...
static void hyperhold_fault_stat(struct zram *zram, u32 index)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT))
		atomic64_inc(&stat->fault_hot_cnt);

	mcg = hyperhold_zram_get_memcg(zram, index);
	if (mcg) {
		atomic64_inc(&mcg->hyperhold_allfaultcnt);
		if (zram_test_flag(zram, index, ZRAM_OBJ_HOT))
			atomic64_inc(&mcg->hyperhold_allfaultcnt_hot);

		score = atomic64_read(&mcg->memcg_reclaimed.app_score);
		if (score >= SHRINK_SYS_ANON_MIN_SCORE && score <= SHRINK_SYS_ANON_MAX_SCORE)
			atomic64_inc(&stat->sys_fault_cnt);
	}
}
...
static void hyperhold_fault2_stat(struct zram *zram, u32 index)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT))
		atomic64_inc(&stat->hyperhold_hot_fault_cnt);

	mcg = hyperhold_zram_get_memcg(zram, index);
	if (mcg) {
		if (atomic64_read(&mcg->memcg_reclaimed.app_score) == FG_APP_SCORE) {
			if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
				atomic64_inc(&stat->hyperhold_hot_fault_cnt_fg);
				atomic64_inc(&mcg->hyperhold_faultcnt_hot_fg);
			}
		}

		atomic64_inc(&mcg->hyperhold_faultcnt);
		if (zram_test_flag(zram, index, ZRAM_OBJ_HOT))
			atomic64_inc(&mcg->hyperhold_faultcnt_hot);

		score = atomic64_read(&mcg->memcg_reclaimed.app_score);
		if (score >= SHRINK_SYS_ANON_MIN_SCORE && score <= SHRINK_SYS_ANON_MAX_SCORE)
			atomic64_inc(&stat->sys_hyperhold_fault_cnt);
	}
}
...
void hyperhold_idle_reclaim(void)
{
	...
	for (memcg = get_next_memcg(NULL); memcg; memcg = get_next_memcg(memcg))
		if (memcg == root_mem_cgroup ||
			...
			hyperhold_permcg_reclaim(memcg, &idle_size, RECLAIM_SOURCE_DEFAULT);
		}
...
static int hyperhold_in_proc_ft(struct file *file)
{
	...
	ret = hyperhold_permcg_reclaim(mem_cgroup_from_task(task), &size, RECLAIM_SOURCE_DEFAULT);
