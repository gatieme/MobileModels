...
#define ACC_SWAPIN_SKIP_SCORE 900
#define FG_SWAPOUT_TYPE_RATIO 3
#define FG_APP_SCORE 0
#define BG_APP_SCORE 999
...
enum hyperhold_fail_point {
	...
	HYPERHOLD_FAULT_OUT_DECOMP_FAIL,
...
struct hyperhold_stat {
	...
	atomic64_t fault_out_lat_dist[FAULT_OUT_LAT_DIST];
	atomic64_t fault_out_lat_dist_fg[FAULT_OUT_LAT_DIST];

	atomic64_t fault_hot_cnt;
	atomic64_t hyperhold_hot_fault_cnt;
	atomic64_t hyperhold_hot_fault_cnt_fg;
	atomic64_t zram_stored_hot_pages;
	atomic64_t zram_stored_hot_size;
	atomic64_t stored_hot_pages;
	atomic64_t stored_hot_size;
	atomic64_t zram_stored_reout_pages;
	atomic64_t zram_stored_reout_size;
	atomic64_t zram_stored_reout_hot_pages;
	atomic64_t zram_stored_reout_hot_size;
	atomic64_t sys_fault_cnt;
	atomic64_t sys_hyperhold_fault_cnt;
};
...
struct space_info_para {
	...
	unsigned long space_rate;
	unsigned long file_magic;
	unsigned long allocated_file_num;
	unsigned long nr_pages;
	unsigned long nr_exts;
	unsigned long nr_exts_per_file;
	atomic64_t used_exts_num;
	atomic64_t max_ext_num;
...
struct hyperhold_file_ops {
	...
	int (*check)(const char *file_path, unsigned long long size);
	void (*free)(const char *file_path, unsigned long long size);
	int (*get_sector)(const char *file_path, sector_t *start_sector);
};

/* hyperhold_file */
enum cust_blk_hp_cmd {
	CUST_BLK_HP_SET_SPACE = 0,
	CUST_BLK_HP_FREE_SPACE,
	CUST_BLK_HP_MAX,
};
...
struct blk_hp_set_space {
	unsigned long space_type;
	unsigned long space_size;
};

#define BLK_HP_CMD	_IO(0x12, 110) /* unistore - Custom Command */
/* end of hyperhold file */
...
void hyperhold_zram_lru_add(struct zram *zram, u32 index,
					struct mem_cgroup *memcg, struct page *page);
...
int hyperhold_plug_finish(void *io_handler, enum hyperhold_reclaim_source source);
...
static inline void hyperhold_show_stack(struct task_struct *task, unsigned long *fp)
{
