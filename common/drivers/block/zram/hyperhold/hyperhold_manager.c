	...
	unsigned long eswpentry, struct mem_cgroup *mcg, int ext_id)
{
	...
	if (preempt_cnt[0] != preempt_cnt[4]) {
		...
		for (i = 0; i < MOVE_TO_HYPERHOLD_POINTS; i++)
			pr_info("[%d]:%d\n", i, preempt_cnt[i]);
		hyperhold_show_stack(current, NULL);
	}

	if (ext_to_par_file(ext_id)) {
		atomic64_add(size, &stat->parfile_stored_size);
		atomic64_add(size, &mcg->hyperhold_parfile_stored_size);
		atomic64_inc(&stat->parfile_stored_pages);
		atomic64_inc(&mcg->hyperhold_parfile_stored_pages);
	}

	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
		atomic64_add(size, &stat->stored_hot_size);
		atomic64_add(size, &mcg->hyperhold_stored_hot_size);
		atomic64_inc(&stat->stored_hot_pages);
		atomic64_inc(&mcg->hyperhold_stored_hot_pages);
	}
			...
			struct io_extent *io_ext)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
		atomic64_sub(size, &stat->stored_hot_size);
		atomic64_dec(&stat->stored_hot_pages);
	}
	...
	if (mcg) {
		...
		if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
			atomic64_sub(size, &mcg->hyperhold_stored_hot_size);
			atomic64_dec(&mcg->hyperhold_stored_hot_pages);
		}
...
#ifdef CONFIG_CRYPTO_DELTA
	if (zram_test_flag(zram, index, ZRAM_SDDC_DELTA))
		zram_sddc_ref_obj_cache_pop(zram->ref_obj_cache, index, &ref_index);
#endif
...
#ifdef CONFIG_CRYPTO_DELTA
	if (zram_test_flag(zram, index, ZRAM_SDDC_DELTA)) {
		if (zram_sddc_ref_obj_cache_push(zram->ref_obj_cache,
					index,
					*(uint32_t*)src)) {
			zram_slot_unlock(zram, index);
			return -ENOMEM;
...
static int shrink_entry_list(struct io_extent *io_ext)
{
	...
	if (!mcg->active_swapout && stored_size > 0 && stored_size < EXTENT_SIZE)
#else
	...
	swap_cnt = zram_get_memcg_coldest_index(zram->area, mcg, swap_index,
						EXTENT_MAX_OBJ_CNT);
	hh_print(HHLOG_DEBUG, "swap_cnt = %d\n", swap_cnt);
	io_ext->cnt = 0;
...
int hyperhold_cache_decomp(struct zram *zram, u32 index, struct page *page)
{
	...
	ret = sddc_decode(zstrm, page, index, src);
	zcomp_ext_stream_put(zram->comp);
...
void hyperhold_manager_memcg_init(struct mem_cgroup *mcg, struct zram *zram)
{
	...
	hh_list_init(mcg_hot_idx(zram->area, mcg->id.id), zram->area->obj_table);
	...
	atomic64_set(&mcg->zram_stored_hot_size, 0);
	atomic64_set(&mcg->zram_stored_hot_pages, 0);
	atomic64_set(&mcg->hyperhold_stored_pages, 0);
	atomic64_set(&mcg->hyperhold_stored_size, 0);
	atomic64_set(&mcg->hyperhold_stored_hot_pages, 0);
	atomic64_set(&mcg->hyperhold_stored_hot_size, 0);
	atomic64_set(&mcg->last_active_swapout_size, 0);
	...
	atomic64_set(&mcg->hyperhold_allfaultcnt_hot, 0);
	atomic64_set(&mcg->hyperhold_faultcnt_hot, 0);
	atomic64_set(&mcg->hyperhold_faultcnt_hot_fg, 0);
	atomic64_set(&mcg->zram_stored_reout_size, 0);
	atomic64_set(&mcg->zram_stored_reout_pages, 0);
	atomic64_set(&mcg->zram_stored_reout_hot_size, 0);
	atomic64_set(&mcg->zram_stored_reout_hot_pages, 0);
	atomic_set(&mcg->active_swapout_type, 0);
	atomic64_set(&mcg->active_swapout_cnt, 0);

	for (i = 0; i < FAULT_OUT_LAT_DIST; ++i)
		atomic64_set(&mcg->fault_out_lat[i], 0);
	for (i = 0; i < FAULT_OUT_LAT_DIST; ++i)
		atomic64_set(&mcg->fault_out_lat_fg[i], 0);
	smp_wmb();
	mcg->zram = zram;
		...
		if (index == last_index || mcg == zram_get_memcg(zram, index)) {
			if (zram_test_flag(zram, index, ZRAM_OBJ_HOT))
				hh_list_del(obj_idx(zram->area, index),
					mcg_hot_idx(zram->area, mcg->id.id),
					zram->area->obj_table);
			else
				hh_list_del(obj_idx(zram->area, index),
...
#ifdef CONFIG_CRYPTO_DELTA
void hyperhold_change_size_of_page(struct zram *zram, u32 index, u32 size_diff)
{
	if (!zram) {
		hh_print(HHLOG_ERR, "NULL zram\n");
		return;
	...
	zram_lru_change_size_of_page(zram, index, size_diff);
}
#endif
				...
				struct page *page)
{
	...
	memcg = get_mem_cgroup(mcg->id.id);
	if (memcg != mcg) {
		hh_print(HHLOG_ERR, "invalid memcg id %d, name %s, ref %d",
			mcg->id.id, mcg->name, refcount_read(&mcg->id.ref));
		if (memcg)
			hh_print(HHLOG_ERR, "current memcg id %d, name %s, ref %d",
				memcg->id.id, memcg->name, refcount_read(&memcg->id.ref));
		return;
	}
	zram_lru_add(zram, index, mcg, page);
}
...
void hyperhold_extent_objs_del(struct zram *zram, u32 index)
{
	...
	if (!atomic_dec_and_test(
		&zram->area->ext_stored_pages[ext_id]))
		return;
	ext_id = get_extent(zram->area, ext_id);
	if (ext_id < 0)
		return;
				 ...
				 void **private)
{
	...
	io_ext->zram = zram;
	io_ext->mcg = get_memcg_with_css_get(
...
void hyperhold_get_page_stat(u64 *orig, u64 *compr)
{
	struct hyperhold_stat *stat = NULL;

	if (unlikely(orig == NULL || compr == NULL)) {
		return;
	}
	...
	*orig = atomic64_read(&stat->stored_pages) << PAGE_SHIFT;
	*compr = atomic64_read(&stat->stored_size);
}
