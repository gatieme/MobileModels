...
static s64 timeout_value[HYPERHOLD_SCENARIO_BUTT] = {
	2000000, 100000, 500000, 2000000
};

/* fault_out: 5ms, 10ms, 20ms, 50ms, 100ms */
static s64 fault_out_lat_dist_value[] = {
...
static void hyperhold_perf_warning(struct timer_list *timer)
{
	struct hyperhold_key_point_record *record =
		from_timer(record, timer, lat_monitor);
#else
static void hyperhold_perf_warning(unsigned long data)
{
	...
	if (likely(record->task))
		hyperhold_show_stack(record->task, NULL);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	timer_setup(&record->lat_monitor, hyperhold_perf_warning, 0);
	mod_timer(&record->lat_monitor,
			jiffies +
	...
	struct hyperhold_key_point_record *record)
{
	...
	struct mem_cgroup *memcg = mem_cgroup_from_task(record->task);
	s64 curr_lat;
	int i;
	unsigned long app_score;
	...
	if (record->scenario == HYPERHOLD_FAULT_OUT) {
		if (record->task) {
			app_score = atomic64_read(&memcg->memcg_reclaimed.app_score);
		} else {
			hh_print(HHLOG_ERR, "record->task == NULL\n");
			app_score = BG_APP_SCORE;
		}
		if (app_score == FG_APP_SCORE) {
			for (i = 0; i < FAULT_OUT_LAT_DIST - 1; i++) {
				if (curr_lat < fault_out_lat_dist_value[i]) {
					atomic64_inc(&stat->fault_out_lat_dist_fg[i]);
					if (memcg)
						atomic64_inc(&memcg->fault_out_lat_fg[i]);
					break;
				}
			}
			if (curr_lat >= fault_out_lat_dist_value[FAULT_OUT_LAT_DIST - 2]) {
				atomic64_inc(&stat->fault_out_lat_dist_fg[FAULT_OUT_LAT_DIST - 1]);
				if (memcg)
					atomic64_inc(&memcg->fault_out_lat_fg[FAULT_OUT_LAT_DIST - 1]);
			}
		}

		for (i = 0; i < FAULT_OUT_LAT_DIST - 1; i++) {
			if (curr_lat < fault_out_lat_dist_value[i]) {
				atomic64_inc(&stat->fault_out_lat_dist[i]);
				if (memcg)
					atomic64_inc(&memcg->fault_out_lat[i]);
				break;
			}
		}
		if (curr_lat >= fault_out_lat_dist_value[FAULT_OUT_LAT_DIST - 2]) {
			atomic64_inc(&stat->fault_out_lat_dist[FAULT_OUT_LAT_DIST - 1]);
			if (memcg)
				atomic64_inc(&memcg->fault_out_lat[FAULT_OUT_LAT_DIST - 1]);
		}
	}
}
...
void hyperhold_perf_end(struct hyperhold_key_point_record *record)
{
	...
	loglevel = record->timeout_flag ? HHLOG_ERR : HHLOG_INFO;
