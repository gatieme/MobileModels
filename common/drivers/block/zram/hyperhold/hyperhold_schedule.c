...
#define HYPERHOLD_KEY_DERIVATION_NONCE_SIZE 16
...
#define HYPERHOLD_IO_END_WORK_POINTS 11

union hyperhold_crypt_iv {
	struct {
					...
					enum hyperhold_reclaim_source source)
{
	...
	if (req->io_para.scenario == HYPERHOLD_RECLAIM_IN) {
		...
		if (source == RECLAIM_SOURCE_ZSWAPD)
			atomic64_add(req->page_cnt * PAGE_SIZE,
				&stat->zswapd_reclaimin_bytes);
#endif
#ifdef CONFIG_RAMTURBO
		if (source == RECLAIM_SOURCE_APP_ACTIVE)
			atomic64_add(req->page_cnt * PAGE_SIZE,
				&stat->app_active_reclaimin_bytes);
#endif
...
static void hyperhold_generate_dun(u64 lblk_num,
	u64 dun[BLK_CRYPTO_DUN_ARRAY_SIZE])
{
	union hyperhold_crypt_iv iv;
	int i;

	memset(&iv, 0, sizeof(union hyperhold_crypt_iv));
...
{
	u64 dun[BLK_CRYPTO_DUN_ARRAY_SIZE];
	u64 lblk_num = bio->bi_iter.bi_sector >> HYPERHOLD_KEY_INDEX_SHIFT;

	hyperhold_generate_dun(lblk_num, dun);
#ifdef CONFIG_BLK_INLINE_ENCRYPTION
	bio_crypt_set_ctx(bio, hyperhold_get_crypto_key_base(), dun,
...
{
#ifdef CONFIG_SCSI_UFS_INLINE_CRYPTO
	bio->ci_key_index = HYPERHOLD_KEY_INDEX;
	bio->ci_key_len = HYPERHOLD_KEY_SIZE;
	bio->ci_key = (void *)hyperhold_io_key;
	bio->index =
		bio->bi_iter.bi_sector >> HYPERHOLD_KEY_INDEX_SHIFT;
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	bio->ci_metadata = hyperhold_io_metadata;
#endif
#endif
}
#endif
...
void hyperhold_crypto_debug(struct hyperhold_entry *io_entry, int i)
{
	...
	v_addr = kmap(io_entry->dest_pages[i]);
	printk("hyperhold_crypto_debug, page: %d\n", i);
	memset(v_addr, 0x5A, PAGE_SIZE);
	for (offset = 0; offset < sizeof(struct page); offset++) {
		symbol = *(v_addr + offset);
		printk("hyperhold_crypto_debug, %d, %02X\n", i, symbol);
	}
}
#endif
	...
	struct hyperhold_segment *segment)
{
	...
	if (segment->req->io_para.scenario == HYPERHOLD_FAULT_OUT)
		bio->mas_bi_opf |= REQ_FAULT_OUT;
	else
		bio->mas_bi_opf |= REQ_BATCH_OUT;
#else
	...
	bool to_par_file)
{
	...
	if (!bdev->bd_disk) {
		hh_print(HHLOG_ERR, "null bd_disk\n");
		return -EIO;
	}
...
{
	hyperhold_crypto_switch = val;
}

int get_hyperhold_crypto_switch(void)
{
...
int hyperhold_plug_finish(void *io_handler, enum hyperhold_reclaim_source source)
{
	...
	hyperhold_stat_io_bytes(req, source);
