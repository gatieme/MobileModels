	...
	struct hyperhold_stat *stat)
{
	...
	for (i = 0; i < FAULT_OUT_LAT_DIST; i++)
		seq_printf(m, "hyperhold_fault_out_lat_dist_level%d:%lld\n",
			i, atomic64_read(&stat->fault_out_lat_dist[i]));
	for (i = 0; i < FAULT_OUT_LAT_DIST; i++)
		seq_printf(m, "hyperhold_fault_out_lat_fg_dist_level%d:%lld\n",
			i, atomic64_read(&stat->fault_out_lat_dist_fg[i]));
}

static void hyperhold_stats_show(struct seq_file *m,
	struct hyperhold_stat *stat)
{
	seq_printf(m, "hyperhold_enable:%lld\n",
		hyperhold_enable());
	seq_printf(m, "hyperhold_reclaim_in_enable:%lld\n",
		hyperhold_reclaim_in_enable());
	seq_printf(m, "hyperhold_out_times:%lld\n",
		atomic64_read(&stat->reclaimin_cnt));
	seq_printf(m, "hyperhold_out_comp_size:%lld\n",
		atomic64_read(&stat->reclaimin_bytes) >> MBYTE_SHIFT);
	if (PAGE_SHIFT < MBYTE_SHIFT)
		seq_printf(m, "hyperhold_out_ori_size:%lld\n",
			atomic64_read(&stat->reclaimin_pages) >>
				(MBYTE_SHIFT - PAGE_SHIFT));
#ifdef CONFIG_HYPERHOLD_ZSWAPD
	seq_printf(m, "hyperhold_zswapd_out_comp_size:%lld\n",
		atomic64_read(&stat->zswapd_reclaimin_bytes) >> MBYTE_SHIFT);
#endif
#ifdef CONFIG_RAMTURBO
	seq_printf(m, "hyperhold_app_active_out_comp_size:%lld\n",
		atomic64_read(&stat->app_active_reclaimin_bytes) >> MBYTE_SHIFT);
#endif
	seq_printf(m, "hyperhold_in_times:%lld\n",
		atomic64_read(&stat->batchout_cnt));
	seq_printf(m, "hyperhold_in_comp_size:%lld\n",
		atomic64_read(&stat->batchout_bytes) >> MBYTE_SHIFT);
	if (PAGE_SHIFT < MBYTE_SHIFT)
		seq_printf(m, "hyperhold_in_ori_size:%lld\n",
	...
	seq_printf(m, "fault_hot_cnt:%lld\n",
		atomic64_read(&stat->fault_hot_cnt));
	seq_printf(m, "hyperhold_hot_fault_cnt:%lld\n",
		atomic64_read(&stat->hyperhold_hot_fault_cnt));
	seq_printf(m, "hyperhold_hot_fault_cnt_fg:%lld\n",
		atomic64_read(&stat->hyperhold_hot_fault_cnt_fg));
	seq_printf(m, "hyperhold_sys_all_fault:%lld\n",
		atomic64_read(&stat->sys_fault_cnt));
	seq_printf(m, "hyperhold_sys_fault:%lld\n",
		atomic64_read(&stat->sys_hyperhold_fault_cnt));
	seq_printf(m, "total_shrink_sys_cnt:%llu\n",
		get_shrink_sys_cnt_value());
	seq_printf(m, "total_shrink_sys_page:%llu\n",
		get_skrink_sys_page_value());
}

static void hyperhold_area_info_show(struct seq_file *m,
	struct hyperhold_stat *stat)
{
	seq_printf(m, "hyperhold_reout_ori_size:%lld\n",
		atomic64_read(&stat->reout_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "hyperhold_reout_comp_size:%lld\n",
		atomic64_read(&stat->reout_bytes) >> MBYTE_SHIFT);
	seq_printf(m, "hyperhold_zram_comp_size:%lld\n",
		atomic64_read(&stat->zram_stored_size) >> MBYTE_SHIFT);
	seq_printf(m, "hyperhold_zram_ori_size:%lld\n",
		atomic64_read(&stat->zram_stored_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "hyperhold_zram_comp_hot_size:%lld\n",
		atomic64_read(&stat->zram_stored_hot_size) >> MBYTE_SHIFT);
	seq_printf(m, "hyperhold_zram_ori_hot_size:%lld\n",
		atomic64_read(&stat->zram_stored_hot_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "hyperhold_store_comp_size:%lld\n",
	...
	seq_printf(m, "hyperhold_parfile_store_ori_size:%lld\n",
		atomic64_read(&stat->parfile_stored_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "zram_stored_reout_pages:%lld\n",
		atomic64_read(&stat->zram_stored_reout_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "zram_stored_reout_size:%lld\n",
		atomic64_read(&stat->zram_stored_reout_size) >> MBYTE_SHIFT);
	seq_printf(m, "zram_stored_reout_hot_size:%lld\n",
		atomic64_read(&stat->zram_stored_reout_hot_size) >> MBYTE_SHIFT);
	seq_printf(m, "zram_stored_reout_hot_pages:%lld\n",
		atomic64_read(&stat->zram_stored_reout_hot_pages) >>
			(MBYTE_SHIFT - PAGE_SHIFT));
	seq_printf(m, "hyperhold_notify_free_size:%lld\n",
...
void hyperhold_psi_show(struct seq_file *m)
{
	...
	hp_cache_show(m, hyperhold_get_global_zram());
}
...
unsigned long hyperhold_get_eswap_used_pages(void)
{
	struct hyperhold_stat *stat = NULL;
	...
	return atomic64_read(&stat->stored_pages);
}
				...
				enum hyperhold_mcg_member mcg_member)
{
	...
	case MCG_DISK_FILE_STORED_SZ:
		...
		break;
	case MCG_DISK_FILE_STORED_PG_SZ:
		val = atomic64_read(&mcg->hyperhold_parfile_stored_pages);
		break;
...
unsigned long get_hyperhold_cache_pages(void)
{
	...
	return hyperhold_cache_pages(zram);
}
...
void hyperhold_decomp_fail_record(u32 index, int ext_id,
	unsigned char *task_comm)
{
	hyperhold_fail_record(HYPERHOLD_FAULT_OUT_DECOMP_FAIL,
		index, ext_id, task_comm);
}
