...
{
	if (IS_ERR_OR_NULL(clk) || !clk->core)
		return;

	if (WARN(clk->core->enable_count == 0, "%s already disabled\n",
		clk->core->name))
		return;

	if (clk->core->ops->disable)
		clk->core->ops->disable(clk->core->hw);
}
EXPORT_SYMBOL_GPL(clk_disable_nolock);
...
int clk_enable_nolock(struct clk *clk)
{
	...
	if (WARN(clk->core->prepare_count == 0,
		"Enabling unprepared %s\n", clk->core->name))
		return -ESHUTDOWN;

	if (clk->core->ops->enable)
		ret = clk->core->ops->enable(clk->core->hw);

	return ret;
}
EXPORT_SYMBOL_GPL(clk_enable_nolock);
