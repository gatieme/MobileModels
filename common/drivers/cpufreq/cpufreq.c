...
struct cpufreq_frequency_table *cpufreq_frequency_get_table(unsigned int cpu)
{
	...
	return policy && (!policy_is_inactive(policy)) ? policy->freq_table : NULL;
}
...
int cpufreq_register_notifier(struct notifier_block *nb, unsigned int list)
{
	...
	default:
		ret = -EINVAL;
		trace_android_rvh_corectl_notifier_chain(1, nb, list, &ret);
	}
...
int cpufreq_unregister_notifier(struct notifier_block *nb, unsigned int list)
{
	...
	default:
		ret = -EINVAL;
		trace_android_rvh_corectl_notifier_chain(0, nb, list, &ret);
	}
					...
					unsigned int target_freq)
{
	...
	trace_android_rvh_drg_cpufreq_check_limit(policy, &target_freq);
