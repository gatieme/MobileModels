...
static u64 get_proc_each_cpu_load(struct cpu_freqs *freqs, struct task_struct *p,
	struct task_struct *child, int dmips_value_buffer[], unsigned int cpu)
{
	unsigned int i;
	u64 cputime;
	u64 cputime_total;
	...
	for (i = 0; i < freqs->max_state; i++) {
		...
		rcu_read_unlock();
		cputime_total += (freqs->freq_table[i] * cputime * dmips_value_buffer[cpu]);
	}
