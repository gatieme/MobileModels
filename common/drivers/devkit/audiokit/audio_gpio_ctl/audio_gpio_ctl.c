...
static int g_tdd_gpio_register = 0;

static int parse_dts_config(struct audio_gpio_ctrl_priv *gpio_priv);
	...
	void __user *arg)
{
	...
	ret = parse_dts_config(gpio_priv);
	if (ret < 0)
		return ret;
...
static int parse_dts_config(struct audio_gpio_ctrl_priv *gpio_priv)
{
	...
	if (g_tdd_gpio_register)
		return 0;

	gpio_priv->tdd_gpio = of_get_named_gpio(gpio_priv->dev->of_node, "tdd_gpio", 0);
	...
	g_tdd_gpio_register = 1;
	gpio_direction_output(gpio_priv->tdd_gpio, 0);
	pr_info("%s: gpio tdd register sucess\n", __func__);
...
err_unregister:
	(void)power_event_bnc_unregister(POWER_BNT_HW_USB, &gpio_priv->event_nb);
err_out:
	kfree(gpio_priv);
