...
my @platform_chip_map = (
            ...
            [ "sdm660_64",              "qcom",          "sdm660"     ],
            [ "hi6250",                 "hisi",          "hi6250"     ],
            [ "hi3660",                 "hisi",          "hi3660"     ],
            [ "kirin970",               "hisi",          "kirin970"   ],
			[ "kirin710",               "hisi",          "kirin710"   ],
			[ "miamicw",               "hisi",          "miamicw"   ],
...
#collect lcd xmls
my $panel_xml_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/panel";
my $udp_panel_xml_path = "$root_path/device/hisi/customize/udp_lcdkit/lcdkit1.0/panel";
my @panel_xmls = (glob("$panel_xml_path/*.xml"), glob("$udp_panel_xml_path/*.xml"));
...
my $vender_list_path;
my $devkit_tool_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/lcdkit_tool";

#clean lcd generate files
if (($target_file_type eq "dtsi") || ($target_file_type eq "trebledto") || ($target_file_type eq "all"))
{
    $out_dtsi_file_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/panel/dtsi";
    ...
    if ($target_dtsi_file_path eq "")
    {
        ...
        {
			if ($chip_name eq "sdm660") {
				$target_dtsi_file_path = "$root_path/kernel/msm-4.4/arch/arm64/boot/dts/qcom/lcdkit";
			} else {
				if ($chip_name eq "msm8953") {
					$target_dtsi_file_path = "$root_path/kernel/msm-4.9/arch/arm64/boot/dts/qcom/lcdkit";
				} else {
...
if (($target_file_type eq "head") || ($target_file_type eq "dto-head") || ($target_file_type eq "all"))
{
    ...
    {
	 if ($chip_name eq "msm8953") {
	     $out_head_file_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/lk/qcom/panel";
	 } else {
             $out_head_file_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/panel/head";
	 }
    }
...
if ((($target_file_type eq "effect") || ($target_file_type eq "all")) && ($chip_plat eq "hisi"))
{
    ...
    {
        $out_effect_file_path = "$root_path/vendor/huawei/chipset_common/devkit/lcdkit/lcdkit1.0/panel/effect";
    }
...
my $product_pad_name = "";
error_print("product_pad_name param num: $shell_parm_count \n");
if ($shell_parm_count > 3) {
	$target_build_variant = shift;
	if ($shell_parm_count == 5) {
		$product_pad_name = shift;
	}
	error_print("product_pad_name: $product_pad_name target_build_variant:$target_build_variant \n");
}
...
foreach my $list_entry (@lcd_map_list) {
    ...
    if ($target_build_variant eq "eng") {
        if ($product_pad_name eq "") {
            #skip new LCD modules to avoid fastboot image too larger issue for other producs compile.
            if ($vender_fd eq "BAH4") {
                error_print("skip product: $lcd_name for fastboot larger size\n");
                next;
            }
        }
        else {
            error_print("defined product_pad_name: $product_pad_name\n");
            #only compile BACH2 lcd modules if set product_pad_name to BACH2 from compile cmd.
            if (($product_pad_name ne $vender_fd) && ($vender_fd ne "DEFAULT")) {
                error_print("skip product_pad_name: $product_pad_name vender_fd:$vender_fd\n");
                next;
            }
        }
    }
...
if (($target_file_type eq "head") || ($target_file_type eq "dto-head") || ($target_file_type eq "all"))
{
    ...
    {
        print $lcd_head_file "\n extern int get_lcdkit_id(void); \n";
        print $lcd_head_file create_hisi_panel_init();
        print $lcd_head_file create_hisi_panel_update();
    }
...
if ($target_file_type eq "trebledts")
{
	...
	foreach my $list_entry (@lcd_map_list) {
		...
		print $lcd_dtsi_file "\t\tdsi_" . lc($product) . "_" . lc($lcd_name) . ": lcdkit_" . lc($product) . "_" . lc($lcd_name) . " {\n";
