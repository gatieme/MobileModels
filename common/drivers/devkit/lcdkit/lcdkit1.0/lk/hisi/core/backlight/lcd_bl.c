...
static void lcdkit_backlight_value_swap(unsigned char *value1, unsigned char *value2)
{
	unsigned char tmp = 0;
	tmp = *value1;
	*value1 = *value2;
...
static int lcdkit_backlight_ic_check(struct backlight_ic_info *pinfo)
{
	...
	if (pinfo->msb_before_lsb) {
		lcdkit_backlight_value_swap(&pinfo->bl_lsb_reg_cmd.cmd_reg,
			&pinfo->bl_msb_reg_cmd.cmd_reg);
		LCDKIT_DEBUG_INFO("backlight ic swap bl_lsb_reg and bl_msb_reg\n");
	}
...
void lcdkit_backlight_get_dts_value(struct backlight_ic_info *pinfo)
{
	int ret = 0;

	ret = get_dts_value(DTS_COMP_SYH62758, "vboost_enable",
		&pinfo->vboost_enable);
	if (ret < 0)
		LCDKIT_DEBUG_ERROR("get vboost_enable false\n");

	if (pinfo->vboost_enable) {
		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_delay1",
			&pinfo->vboost_delay1);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_delay1 false\n");

		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_delay2",
			&pinfo->vboost_delay2);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_delay2 false\n");

		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_cmd1_reg",
			&pinfo->vboost_cmd1.cmd_reg);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_cmd1_reg false\n");
		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_cmd1_val",

			&pinfo->vboost_cmd1.cmd_val);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_cmd1_val false\n");

		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_cmd2_reg",
			&pinfo->vboost_cmd2.cmd_reg);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_cmd2_reg false\n");

		ret = get_dts_value(DTS_COMP_SYH62758, "vboost_cmd2_val",
			&pinfo->vboost_cmd2.cmd_val);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("get vboost_cmd2_val false\n");
	}
}

void lcdkit_backlight_vboost_config(struct backlight_ic_info *pinfo)
{
	int ret = 0;
	if (pinfo->vboost_enable) {
		mdelay(pinfo->vboost_delay1);

		ret = lcdkit_backlight_ic_i2c_write_u8(pinfo->i2c_addr,
			pinfo->vboost_cmd1.cmd_reg, pinfo->vboost_cmd1.cmd_val);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("operation  reg 0x%x failed!\n",
				pinfo->vboost_cmd1.cmd_reg);

		mdelay(pinfo->vboost_delay2);
		ret = lcdkit_backlight_ic_i2c_write_u8(pinfo->i2c_addr,
			pinfo->vboost_cmd2.cmd_reg, pinfo->vboost_cmd2.cmd_val);
		if (ret < 0)
			LCDKIT_DEBUG_ERROR("operation  reg 0x%x failed!\n",
				pinfo->vboost_cmd2.cmd_reg);
	}
...
int lcdkit_backlight_ic_inital(struct backlight_ic_info *pinfo)
{
	...
	if (!strcmp(pinfo->name, "hw_syh62758"))
		lcdkit_backlight_get_dts_value(pinfo);
	...
	lcdkit_backlight_vboost_config(pinfo);
...
int lcdkit_backlight_ic_set_brightness(struct backlight_ic_info *pinfo, unsigned int level)
{
	...
	if (pinfo->msb_before_lsb) {
		/* swap level_lsb level_msb */
		lcdkit_backlight_value_swap(&level_lsb, &level_msb);
		LCDKIT_DEBUG_INFO("backlight ic set brightness msb before lsb\n");
	}
...
int lcdkit_backlight_common_set(int bl_level)
{
	...
	if (lcdkit_infos.lcd_misc->bl_poweron_after_dsi_cmd) {
		mdelay(lcdkit_infos.lcd_delay->delay_af_dsi_cmd);
