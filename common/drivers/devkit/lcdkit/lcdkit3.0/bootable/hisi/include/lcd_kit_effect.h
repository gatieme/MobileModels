...
/* OEMINFO ID used for gmp lut table */
#define OEMINFO_GMP 311
#define OEMINFO_GMP_LEN 64000
#define SN_LENGTH_MAX 54
#define GMP_LUT_LEN (17 * 17 * 17)
#define GMP_MD5_LEN 16
#define ROBOT_BRIGHTNESS_LEVEL_LEN 512
#define ROBOT_EYE_MAX 2
...
struct lcd_kit_gmp_info {
	uint32_t gmp_mode;
	uint32_t gmp_lut_low32bit[GMP_LUT_LEN];
	uint32_t gmp_lut_high4bit[GMP_LUT_LEN];
};

#pragma pack(1)
struct lcd_kit_oeminfo_gmp_info {
	unsigned int sn_len;
	unsigned char sn_data[SN_LENGTH_MAX];
	unsigned int gmp_lut_low32[GMP_LUT_LEN];
	unsigned char gmp_lut_high4[GMP_LUT_LEN];
};

struct lcd_kit_oeminfo_gmp_robot {
	union {
		struct lcd_kit_oeminfo_gmp_info gmp_info[ROBOT_EYE_MAX];
		uint8_t blob[OEMINFO_GMP_LEN - ROBOT_BRIGHTNESS_LEVEL_LEN];
	};
	uint8_t brightness_lvl[ROBOT_BRIGHTNESS_LEVEL_LEN];
};
#pragma pack()

struct lcd_kit_oeminfo_gmp_blob {
	union {
		struct lcd_kit_oeminfo_gmp_info gmp_info;
		uint8_t blob[OEMINFO_GMP_LEN];
	};
};
