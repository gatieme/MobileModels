...
static struct lcd_kit_vci_type lcd_kit_ldo_map[] = {
	...
	{ LDO_24, DEVICE_LCDDIC },
	{ LDO_29, DEVICE_LCDDIG },
	{ LDO_12, DEVICE_SIM_JTAG2 },
	{ LDO_32, DEVICE_CAM_CORE },
...
void lcd_kit_gpio_tx(uint32_t type, uint32_t op)
{
	...
	case LCD_KIT_MIPI_SWITCH:
		...
		break;
	case LCD_KIT_EINK_READY:
		g_lcd_kit_gpio = power_hdl->lcd_eink_ready.buf[POWER_NUMBER];
		break;
	case LCD_KIT_EINK_VSYS:
		g_lcd_kit_gpio = power_hdl->lcd_eink_power_vsys.buf[POWER_NUMBER];
		break;
	case LCD_KIT_EINK_MCU_POWER:
		g_lcd_kit_gpio = power_hdl->lcd_eink_mcu_power.buf[POWER_NUMBER];
		break;
	case LCD_KIT_EINK_RESET:
		g_lcd_kit_gpio = power_hdl->lcd_eink_reset.buf[POWER_NUMBER];
		break;
	case LCD_KIT_EINK_FASTBOOT:
		g_lcd_kit_gpio = power_hdl->lcd_eink_fastboot.buf[POWER_NUMBER];
		break;
	case LCD_KIT_EINK_CFG:
		g_lcd_kit_gpio = power_hdl->lcd_eink_cfg.buf[POWER_NUMBER];
		break;
	default:
