...
static int lcd_check_checksum(int panel_id)
{
	...
	int err_cnt;
	int check_cnt;
	unsigned char read_value[LCD_KIT_CHECKSUM_SIZE + 1] = {0};
	...
	ret = lcd_kit_cmds_rx(panel_id, NULL, read_value, LCD_KIT_CHECKSUM_SIZE,
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	send_fps_lock_cmd(panel_id, val);
	return count;
}
...
static int lcd_mipi_detect(uint32_t panel_id)
{
	...
	count++;
	if (count >= MIPI_DETECT_NUM) {
		loop_num++;
		/* set initial value */
		count = 0;
	}
	LCD_KIT_INFO("count = %d, loop_num = %d\n", count, loop_num);
	if (loop_num >= MIPI_DETECT_LOOP) { // MIPI_DETECT_LOOP -> 15
		...
		if (err_record) // any picture has error, will report
	...
	unsigned char *read_value, int *count, int loop_num)
{
	int i;
	unsigned int *detect_value = NULL;
	unsigned char expect_value[VAL_NUM] = { 0 };
	struct lcd_kit_array_data config_data;
	...
	int loop_num, int *count)
{
	...
	ret = lcd_kit_dual_cmds_rx(panel_id, NULL, read_value0, read_value1,
		MIPI_READ_COUNT - 1, &FACT_INFO->check_sum2_detect.rd_cmds[loop_num]);
	if (ret) {
	...
	unsigned int loop_num, int *count)
{
	...
	LCD_KIT_INFO("enter\n");
	timeout = jiffies + (CHECK_SUM2_LOOP_TIME * FACT_INFO->check_sum2_detect.time * HZ);
	if (!FACT_INFO->check_sum2_detect.rd_cmds[loop_num].cmds) {
	...
	if (loop_num >= MBIST_LOOPS) {
		...
		if (err_record)
			lcd_kit_mbist_detect_dmd_report(
				FACT_INFO->mbist_detect.reg_val);
		/* set initial value */
		loop_num = 0;
	}
	return ret;
