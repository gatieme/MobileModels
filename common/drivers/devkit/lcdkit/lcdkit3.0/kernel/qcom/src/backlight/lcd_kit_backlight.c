...
#define DUAL_BL_UEFI_CHECK_MASK 0xFE
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	if (pbl_config->bl_dimming_support) {
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	if (pbl_config->hw_en) {
		...
		lcd_kit_parse_backlight_param(np, "ic_disable_mdelay",
			&pbl_config->ic_disable_mdelay);
		if (pbl_config->dual_ic) {
...
static int lcd_kit_backlight_dual_ic_config(void)
{
	...
	for (i = 0; i < pbacklight_dev->bl_config.num_of_init_cmds; i++) {
		...
		if (ret != 1)
			LCD_KIT_ERR("reg: 0x%02x, val: 0x%02x", msg.buf[0], msg.buf[1]);
		else
			LCD_KIT_INFO("reg = 0x%x, val = 0x%x\n", buf[0], buf[1]);
...
static void lcd_kit_setbacklight_dual_ic(int bl_lsb, int bl_msb)
{
	struct i2c_adapter *adap = g_backlight_dual_ic_adap;
	struct i2c_msg msg = {0};
	...
	buf[1] = bl_lsb;
	buf2[0] = pbacklight_dev->bl_config.bl_msb_reg_cmd.cmd_reg;
	buf2[1] = bl_msb;
	...
	ret = i2c_transfer(adap, &msg, 1);
	if (ret != 1)
		LCD_KIT_ERR("Dual I2C write reg failed, reg: 0x%02x, val: 0x%02x", msg.buf[0], msg.buf[1]);
	else
		LCD_KIT_INFO("reg = 0x%x, val = 0x%x\n", msg.buf[0], msg.buf[1]);
	ret = i2c_transfer(adap, &msg2, 1);
	if (ret != 1)
		LCD_KIT_ERR("Dual I2C write reg failed, reg: 0x%02x, val: 0x%02x", msg2.buf[0], msg2.buf[1]);
	else
		LCD_KIT_INFO("reg = 0x%x, val = 0x%x\n", msg2.buf[0], msg2.buf[1]);
}
...
static void lcd_kit_backlight_i2c_dmd(void)
{
	int ret;
	...
	dsm_client_record(lcd_dclient,
		"lcd_kit_backlight_ic_config fail backligt ic=%s : ret=0x%x\n",
		backlight_ic_name, ret);
...
static void lcd_kit_backlight_disable(void)
{
	if (pbacklight_dev->bl_config.hw_en != 0 &&
		...
		if (pbacklight_dev->bl_config.dual_ic &&
			pbacklight_dev->bl_config.dual_hw_en_gpio) {
			LCD_KIT_INFO("backlight dual ic disable enter\n");
			gpio_set_value(pbacklight_dev->bl_config.dual_hw_en_gpio,
...
static void lcd_kit_pbacklight_set_gpio_disable(void)
{
	if (pbacklight_dev->bl_config.hw_en != 0 &&
		pbacklight_dev->bl_config.hw_en_pull_low != 0) {
		if (pbacklight_dev->bl_config.dual_ic &&
...
static int lcd_kit_backlight_set_ic_disable(void)
{
	...
	if (pbacklight_dev->bl_config.ic_disable_mdelay)
		mdelay(pbacklight_dev->bl_config.ic_disable_mdelay);
	lcd_kit_pbacklight_set_gpio_disable();
	backlight_init_status = false;
	return ret;
}
...
static void dync_dimming_config(unsigned int bl_level)
{
	...
	if (bl_level == 0) {
		if (pbacklight_dev->bl_config.bl_dimming_resume != 0) {
			...
			if (pbacklight_dev->bl_config.dual_ic) {
				ret = dync_dimming_config_dual_ic(pbacklight_dev->bl_config.dimming_config_reg,
					pbacklight_dev->bl_config.bl_dimming_resume);
				if (ret < 0)
					LCD_KIT_ERR("dync dual dimming config error\n");
				LCD_KIT_INFO("backlight icdync dual dimming config val is reg = 0x%x, avl = 0x%x\n",
					pbacklight_dev->bl_config.dimming_config_reg, pbacklight_dev->bl_config.bl_dimming_resume);
			}
		}
		...
		if (pbacklight_dev->bl_config.bl_dimming_config != 0) {
			...
			if (pbacklight_dev->bl_config.dual_ic) {
				...
				if (ret < 0)
					LCD_KIT_ERR("dync dual dimming config error\n");
				LCD_KIT_INFO("backlight icdync dual dimming config val is reg = 0x%x, avl = 0x%x\n",
					pbacklight_dev->bl_config.dimming_config_reg, pbacklight_dev->bl_config.bl_dimming_config);
			}
		}
...
int lcd_kit_set_backlight(unsigned int bl_level)
{
	...
	if ((backlight_init_status == false) && (bl_level == 0)) {
		/* if set backlight level 0, and ic not enable */
		LCD_KIT_ERR("lcd kit set backlight 0 and ic not enable exit fail\n");
		goto i2c_error;
	}
	...
	*/
	if (!strcmp(backlight_ic_name, "hw_rt8555")) {
		...
		if ((level & 0xFF) == 0xFF)
			level -= 1;
	} else if (!strcmp(backlight_ic_name, "hw_mp3314")) {
		/*
	...
	LCD_KIT_INFO("level = %d, bl_msb = 0x%x, bl_lsb = 0x%x\n", level, bl_msb, bl_lsb);
...
i2c_error:
	/* if set backlight i2c error, disable ic */
	if (pbacklight_dev->bl_config.bl_only != 0) {
		if (backlight_init_status == true && bl_level == 0)
			lcd_kit_backlight_disable();
	}
...
static bool lcd_kit_backlight_dual_ic_check(void)
{
	...
	msgs[0].addr = pbacklight_dev->client->addr;
	msgs[0].flags = 0;
	msgs[0].buf = &pbacklight_dev->bl_config.init_cmds[0].cmd_reg;
	msgs[0].len = 1;
	msgs[1].addr = pbacklight_dev->client->addr;
	msgs[1].flags = I2C_M_RD;
	msgs[1].buf = &val;
	msgs[1].len = 1;

	ret = i2c_transfer(adap, msgs, trans_data_num);
	if (ret != trans_data_num)
		LCD_KIT_ERR("ret:%02x, reg: %02x, val: %02x",ret, msgs[0].buf[0], msgs[1].buf[0]);
	if ((val & DUAL_BL_UEFI_CHECK_MASK) !=
		(pbacklight_dev->bl_config.init_cmds[0].cmd_val & DUAL_BL_UEFI_CHECK_MASK)) {
		LCD_KIT_ERR("reg = 0x%x, cmd_val = 0x%x, val = 0x%x\n", pbacklight_dev->bl_config.init_cmds[0].cmd_reg,
			pbacklight_dev->bl_config.init_cmds[0].cmd_val, val);
		return false;
	}
	return true;
}

static void lcd_kit_backlight_dual_ic_try(void)
{
	int ret = 0;

	if (!lcd_kit_backlight_dual_ic_check()) {
		/* Config dual ic again */
		ret = lcd_kit_gpio_dual_request();
		if (ret)
			LCD_KIT_ERR("can not request gpio_dual\n");
		ret = lcd_kit_backlight_dual_ic_config();
	...
	const struct i2c_device_id *id)
{
	...
	if (pbacklight_dev->bl_config.dual_ic) {
		...
		if (ret) {
			LCD_KIT_ERR("dual_ic_i2c_adapter fail");
		} else {
			/* Check dual ic, if not config in UEFI, config it again */
			lcd_kit_backlight_dual_ic_try();
		}
	}
...
static void parse_backlight_ic_cmdline(void)
{
	char *pstr = NULL;
	char *dstr = NULL;
	uint32_t hw_backlight_len;

	pstr = strstr(saved_command_line, "hw_backlight=");
	if (!pstr) {
		LCD_KIT_ERR("No fastboot hw_backlight info\n");
		return;
	}

	pstr += strlen("hw_backlight=");
	dstr = strstr(pstr, " ");
	if (!dstr) {
		LCD_KIT_ERR("No find the hw_backlight end\n");
		return;
	}

	memset(backlight_ic_name, 0, LCD_BACKLIGHT_IC_NAME_LEN);

	hw_backlight_len = (unsigned long)(dstr - pstr);
	if (hw_backlight_len > LCD_BACKLIGHT_IC_NAME_LEN) {
		LCD_KIT_ERR("hw_backlight_len is %d\n", hw_backlight_len);
		hw_backlight_len = LCD_BACKLIGHT_IC_NAME_LEN;
	}

	memcpy(backlight_ic_name, pstr, hw_backlight_len);
	backlight_ic_name[LCD_BACKLIGHT_IC_NAME_LEN - 1] = 0;
...
int __init lcd_kit_backlight_init(void)
{
	char compatible_name[LCD_BACKLIGHT_IC_COMP_LEN] = {0};
	int len;

	parse_backlight_ic_cmdline();
...
void __exit lcd_kit_backlight_exit(void)
{
	if (backlight_class != NULL) {
		class_destroy(backlight_class);
		backlight_class = NULL;
