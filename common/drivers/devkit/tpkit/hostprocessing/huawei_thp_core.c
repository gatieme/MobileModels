...
void app_send_stylus3_status_to_fw(struct thp_core_data *cd)
{
	int ret;
...
int thp_send_event_to_drv(const char *data)
{
	...
	case THP_SUB_EVENT_STYLUS_SINGLE_CLICK:
		...
		break;
	case THP_SUB_EVENT_SHB_POWER_OFF:
		cd->shb_waitq_flag = WAITQ_WAKEUP;
		wake_up_interruptible(&(cd->shb_waitq));
		thp_log_info("%s: shb power off done\n", __func__);
		break;
	default:
...
static int multi_screen_status_switch_for_qcom(u32 current_status)
{
	struct thp_core_data *cd = thp_get_core_data(MAIN_TOUCH_PANEL);
...
static int multi_screen_status_switch_for_multi_thp(u32 current_status)
{
	...
	case SCREEN_OFF_FOLD:
	case SCREEN_FOLDED:
		if (cd->multi_screen_fold || cd_extra->multi_screen_fold)
			cd_extra->quickly_screen_on_off = true;
		cd->multi_screen_fold = true;
		thp_log_info("%s:screen fold, status = %u\n",
			__func__, current_status);
...
#if defined(CONFIG_HUAWEI_THP_QCOM)
	return multi_screen_status_switch_for_qcom(current_status);
#endif
	if (cd->dual_thp)
...
static void thp_wait_shb_power_off(struct thp_core_data *cd)
{
	if (!cd) {
		thp_log_err("%s:cd is NULL\n", __func__);
		return;
...
static int thp_open_subscreen(struct inode *inode, struct file *filp)
{
	int ret;
...
{
	struct thp_core_data *cd = filp->private_data;
...
static long thp_ioctl_reset(unsigned long reset, u32 panel_id)
{
	...
	thp_log_debug("%s: get rst value : %d\n", __func__, value);
#else
...
static const struct file_operations g_thp_fops[2] = {
	{
		.owner = THIS_MODULE,
		.open = thp_open,
		.release = thp_release,
...
static char *misc_device_name[2] = {
	"thp0",
	"thp1"
};
...
static int thp_misc_dev_register(struct thp_core_data *cd)
{
	...
	cd->thp_misc_device = kzalloc(sizeof(struct miscdevice), GFP_KERNEL);
	if (!cd->thp_misc_device) {
		thp_log_err("%s: thp_misc_device out of memory\n", __func__);
		return -ENOMEM;
	}

	cd->thp_misc_device->minor = MISC_DYNAMIC_MINOR;
	cd->thp_misc_device->fops = &g_thp_fops[cd->panel_id];
	if (cd->dual_thp == 0)
...
static bool thp_check_multi_panel_suspend(struct thp_core_data *cd, u32 *panel_id)
{
...
static int thp_proc_command(struct thp_cmd_node *cmd)
{
	...
	while (true) {
		...
		case THP_MUTIL_SUSPEND_THREAD:
			cd = thp_get_core_data(panel_id);
			if (thp_check_multi_panel_suspend(cd, &panel_id))
				break;
