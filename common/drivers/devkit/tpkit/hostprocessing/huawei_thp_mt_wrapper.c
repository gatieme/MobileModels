...
static int thp_mt_wrapper_open_subscreen(struct inode *inode, struct file *filp)
{
...
{
	unsigned int mask = 0;
	struct thp_core_data *cd = file->private_data;
...
static const struct file_operations g_thp_mt_wrapper_fops[2] = {
	{
		.owner = THIS_MODULE,
	...
	{
		.owner = THIS_MODULE,
		.open = thp_mt_wrapper_open_subscreen,
		.release = thp_mt_wrapper_release,
		.unlocked_ioctl = thp_mt_wrapper_ioctl,
		.compat_ioctl = thp_mt_wrapper_ioctl,
		.poll = thp_mt_wrapper_poll,
...
int thp_mt_wrapper_init(u32 panel_id)
{
	...
	cd->thp_mt_wrapper_misc_device = kzalloc(sizeof(struct miscdevice), GFP_KERNEL);
	if (!cd->thp_mt_wrapper_misc_device) {
		thp_log_err("%s: thp_mt_wrapper_misc_device out of memory\n", __func__);
		return -ENOMEM;
	}
	...
	cd->thp_mt_wrapper_misc_device->fops = &g_thp_mt_wrapper_fops[cd->panel_id];
	if (cd->multi_panel_index != SINGLE_TOUCH_PANEL) {
		...
		cd->thp_mt_wrapper_misc_device->name = (const char *)node;
