...
static bool emcom_mproute_need_reset(struct sock *sk, const uint32_t rst_to_mark, const uint32_t rst_from_mark)
{
	if ((sk->sk_mark & MPROUTE_FWMARK_NEW_MASK) == rst_to_mark)
		return false;

	if (rst_from_mark != INVALID_MARK && (sk->sk_mark & MPROUTE_FWMARK_NEW_MASK) != rst_from_mark)
		return false;
...
static void emcom_mproute_path_handover(struct sock *sk, uint32_t mark)
{
	...
	const struct emcom_xengine_mpflow_ai_init_bind_policy *burst_policy)
{
	...
	if (burst_policy->port_num > EMCOM_MPFLOW_BIND_PORT_CFG_SIZE) {
		emcom_loge("port_num is illegal, port_num: %u", burst_policy->port_num);
		return;
	}
...
int emcom_mproute_getmark(int8_t index, uid_t uid, struct sockaddr *uaddr, uint8_t proto)
{
	...
	mark_count = g_mproute_policy.mark_count;
	best_mark = g_mproute_policy.best_mark;
	...
	if (time_after(jiffies, app->rst_jiffies + app->rst_duration)) {
		...
		emcom_logi("rst_to_mark %u", app->rst_to_mark);
		return app->rst_to_mark;
	}
	...
	if (burst_info->burst_mode != EMCOM_MPFLOW_IP_BURST_FIX) {
		...
		return best_mark;  // not download app, new flow bind to best mark
	}
	...
	struct emcom_xengine_mpflow_ai_burst_port *burst_info)
{
	uint32_t qos_path = 0;
	uint8_t mark_count = 0;
	uint8_t launch_cnt = 0;
	uint32_t burst_path = 0;

	spin_lock_bh(&g_mproute_lock);
	qos_path = g_mproute_policy.best_mark;
	mark_count = g_mproute_policy.mark_count;
	spin_unlock_bh(&g_mproute_lock);

	if (mark_count < LEAST_BURST_PATH_NUM)
		return qos_path;
		...
		if (sk->sk_protocol == IPPROTO_UDP) {
			if (reset->policy.act == SK_ERROR) {
				...
				*need_reset_device = true;
				...
				emcom_mproute_path_handover(sk, reset->policy.rst_to_mark);
			} else {
	...
	const struct reset_mproute_policy_info *reset_policy)
{
	...
	app->rst_to_mark = reset_policy->policy.rst_to_mark;
...
static void emcom_mproute_reset_single_flow(const struct mproute_xengine_head *cfg, uint16_t len)
{
	...
	reset_policy->uid, reset_policy->policy.act, reset_policy->policy.const_perid, reset_policy->policy.rst_to_mark);
	...
	for (; bucket <= tcp_hashinfo.ehash_mask; bucket++) {
		...
		sk_nulls_for_each(sk, node, &head->chain) {
			if (sk->sk_state == TCP_ESTABLISHED || sk->sk_state == TCP_SYN_SENT) {
				...
				if (!emcom_mproute_need_reset(sk, rst_to_mark, rst_from_mark)) {
					continue;
				}
				sk_arr[accum] = sk;
				accum++;
	...
	const reset_act act, const uint32_t rst_from_mark)
{
	...
	for (; idx < accum; idx++) {
		if (act == SK_ERROR) {
			...
			emcom_mproute_path_handover(sk_arr[idx], rst_to_mark);
		} else {
...
static bool emcom_mproute_store_all_flow_info(int uid, struct emcom_xengine_mpflow_ai_info* info,
	struct transfer_mproute_flow_info *reset_all_flow_policy)
{
	...
	if (app_index == INDEX_INVALID) {
		emcom_loge("can't find app_index");
		return false;
	}
	...
	app->all_flow_mark = reset_all_flow_policy->policy.rst_to_mark;
...
void emcom_mproute_close_all_flow(const struct mproute_xengine_head *cfg, uint16_t len)
{
	...
	emcom_logi("reset all flow:uid[%u] rst_to_mark[%u] rst_from_mark[%u] act[%d] time_ms[%u]",
		uid, reset_all_flow_policy->policy.rst_to_mark, reset_all_flow_policy->rst_from_mark,
		act, reset_all_flow_policy->policy.const_perid);

	spin_lock_bh(lock);
	if (!emcom_mproute_store_all_flow_info(uid, info, reset_all_flow_policy)) {
		spin_unlock_bh(lock);
		return;
	}
	spin_unlock_bh(lock);

	emcom_mproute_close_tcp_flow(uid, reset_all_flow_policy->policy.rst_to_mark,
		reset_all_flow_policy->rst_from_mark);
	emcom_mproute_close_udp_flow(uid, reset_all_flow_policy->policy.rst_to_mark, act,
		reset_all_flow_policy->rst_from_mark);
}
...
void emcom_mproute_clear_state(const struct mproute_xengine_head *cfg, uint16_t len)
{
	if (len != (sizeof(int) + sizeof(struct mproute_xengine_head))) {
		emcom_loge("input length error expect: %zu, real: %u", sizeof(int), len);
		return;
	...
	emcom_logd("mproute stop all for reinit");
	emcom_xengine_mpflow_ai_mproute_reinit();

	spin_lock_bh(&g_mproute_lock);
	(void)memset_s(&g_mproute_policy, sizeof(g_mproute_policy), 0, sizeof(g_mproute_policy));
	emcom_mproute_set_default_mark(0);
	spin_unlock_bh(&g_mproute_lock);

	emcom_xengine_network_info_clear();
}
...
uint32_t emcom_mproute_get_best_mark(void)
{
	...
	spin_lock_bh(&g_mproute_lock);
	mark = g_mproute_policy.best_mark;
	spin_unlock_bh(&g_mproute_lock);
...
static void emcom_mproute_update_policy(const struct mproute_xengine_head *cfg, uint16_t len)
{
	...
	emcom_logd("g_mproute_policy markCount:%u, bestMark:%u, mark0:%u, mark1:%u, mark2:%u, mark3:%u",
...
void mproute_evt_process(const struct mproute_xengine_head *cfg, uint16_t len)
{
	...
	case MPROUTE_STOP:
		...
		break;
	case MPROUTE_NOTIFY_CLEAR_STATE:
		emcom_mproute_clear_state(cfg, len);
		break;
	default:
...
void emcom_mproute_evt_proc(int32_t event, const uint8_t *data, uint16_t len)
{
	...
	case NETLINK_EMCOM_DK_MPROUTE_XENGINE:
		...
		emcom_logi("emcom mproute receive event:%d, msg type:%d", event, cfg->type);
