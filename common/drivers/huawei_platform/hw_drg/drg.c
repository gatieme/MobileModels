...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
#define SILVER_IDX 0
#define SILVER_CPUIDLE_C4_EXIT_LATENCY 750
#define GOLD_CPUIDLE_C4_EXIT_LATENCY 1550
#endif
...
struct drg_freq_device {
	...
	/* work used to implement  drg freq */
	struct kthread_work drg_freq_work;
	/* irq work used to queue drg freq work */
	struct irq_work irq_work;
	/* policy used to get freq */
	...
	/* mininum freq request */
	struct freq_qos_request *min_req;
	/* maxinum freq request */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	unsigned int cpuidle_c4_exit_latency;
#endif
};
				     ...
				     unsigned long *max_freq)
{
	...
	*max_freq = ULONG_MAX;
	...
	list_for_each_entry(slave, &freq_dev->slave_list, node) {
		...
		trace_drg_update_freq_range(ruler->name, 0, state,
			rule_min, rule_max);
...
static void drg_rule_update_work(struct kthread_work *work)
{
	...
	for (i = 0; i < ruler->slave_num; i++) {
		...
		if (cpu < (unsigned int)nr_cpu_ids) {
			...
			freq_qos_update_request(freq_dev->min_req, min_freq);
			freq_qos_update_request(freq_dev->max_req, max_freq);

			trace_drg_freq_qos_update(cpu, min_freq, max_freq);
...
static unsigned long drg_freq_check_limit(struct drg_freq_device *freq_dev,
					  unsigned int *freq_cpu, unsigned long *freq_gpu)
#else
static unsigned long drg_freq_check_limit(struct drg_freq_device *freq_dev,
					  unsigned long freq)
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	target_freq = freq_cpu ? *freq_cpu : *freq_gpu;
#else
	target_freq = freq;
#endif
	if (!g_drg_initialized) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	if (freq_cpu)
		*freq_cpu = ret * KHZ / freq_dev->amp;
	else
		*freq_gpu = ret * KHZ / freq_dev->amp;
#endif
	trace_drg_check_limit_final(freq_dev->np->name, cpu, target_freq,
		ret * KHZ / freq_dev->amp);
			     ...
			     unsigned int *target_freq)
{
	...
	if (policy->fast_switch_enabled) {
		irq_work_queue(&freq_dev->irq_work);
		return;
	} else {
		drg_freq_check_limit(freq_dev, target_freq, NULL);
	}
}
#else
 ...
 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
void drg_devfreq_check_limit(void *unused,
			     struct devfreq *df,
			     unsigned long *target_freq)
{
	struct drg_freq_device *freq_dev = NULL;
	...
	drg_freq_check_limit(freq_dev, NULL, target_freq);
}
#else
...
static int drg_isolate_policy_notifier(struct notifier_block *nb,
				       unsigned long event, void *data)
{
	struct drg_rule *ruler = NULL;
	int i;
	...
	list_for_each_entry(ruler, &g_drg_rule_list, node) {
		for (i = 0; i < ruler->master_num; i++) {
			if (!ruler->master_dev[i].freq_dev) {
				pr_err("%s, drg freq dev is NULL", __func__);

				return NOTIFY_DONE;
			}

			if (cpumask_first(&ruler->master_dev[i].freq_dev->cpus) == BIG_CPU) {
				big_cpu = true;
				break;
			}
		}

		if (big_cpu || ruler->user_mode_enable) {
			if (event)
				ruler->enable = ruler->user_mode_enable;
			else
				ruler->enable = !ruler->user_mode_enable;
...
static struct notifier_block drg_isolate_policy_nb = {
	.notifier_call = drg_isolate_policy_notifier,
};
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static bool cluster_cpu_all_pwrdn(struct drg_freq_device *freq_dev)
{
	int cpu;
	...
	for_each_cpu(cpu, &freq_dev->cpus) {
		if (available_idle_cpu(cpu)) {
			...
			if (cpu_idle_latency < freq_dev->cpuidle_c4_exit_latency)
				return false;
		} else {
			return false;
		}
			       ...
			       void *data)
{
	...
	case CPU_PM_ENTER:
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		if (!cluster_cpu_all_pwrdn(freq_dev))
			break;
#endif
...
static void drg_freq_set_freq_work(struct kthread_work *work)
{
	struct drg_freq_device *freq_dev = NULL;
	unsigned int target_freq;
	int index;
	...
	freq_dev = container_of(work, struct drg_freq_device, drg_freq_work);

	if (IS_ERR_OR_NULL(freq_dev))
		return;

	if (freq_dev->policy == NULL)
		return;

	index = freq_dev->policy->cached_resolved_idx;
	if (index < 0)
		return;

	if (freq_dev->policy->freq_table == NULL)
		return;

	target_freq = freq_dev->policy->freq_table[index].frequency;
	trace_drg_set_freq_work(0, freq_dev->policy->cpu, target_freq);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	drg_freq_check_limit(freq_dev, &target_freq, NULL);
#else
	target_freq = drg_freq_check_limit(freq_dev, target_freq);
#endif

	rcu_read_lock_sched();
	data = rcu_dereference_sched(*per_cpu_ptr(&cpufreq_update_util_data,
				     freq_dev->policy->cpu));
	if (data && cpufreq_this_cpu_can_update(freq_dev->policy)) {
		if (target_freq)
			cpufreq_driver_resolve_freq(freq_dev->policy, target_freq);
	}
	rcu_read_unlock_sched();

	trace_drg_set_freq_work(1, freq_dev->policy->cpu, target_freq);

	return;
}

static void drg_freq_irq_work(struct irq_work *irq_work)
{
	struct drg_freq_device *freq_dev = NULL;

	freq_dev = container_of(irq_work, struct drg_freq_device, irq_work);

	if (IS_ERR_OR_NULL(freq_dev))
		return;

	kthread_queue_work(&g_drg_worker, &freq_dev->drg_freq_work);
...
void drg_cpufreq_register(struct cpufreq_policy *policy)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	freq_dev->min_req = (struct freq_qos_request *)
			kzalloc(sizeof(struct freq_qos_request), GFP_ATOMIC);

	if (IS_ERR_OR_NULL(freq_dev->min_req)) {
		pr_err("alloc cpu%d freq min_req err\n", policy->cpu);
		goto unlock;
	}

	freq_dev->max_req = (struct freq_qos_request *)
			kzalloc(sizeof(struct freq_qos_request), GFP_ATOMIC);

	if (IS_ERR_OR_NULL(freq_dev->max_req)) {
		pr_err("alloc cpu%d freq max_req err\n", policy->cpu);
		goto alloc_max_fail;
	}
#endif
	...
	ret = freq_qos_add_request(&policy->constraints,
				   freq_dev->min_req, FREQ_QOS_MIN,
				   policy->min);
	if (ret < 0) {
		pr_err("add cpu%d min qos request err\n", policy->cpu);
		goto add_qos_min_fail;
	}

	ret = freq_qos_add_request(&policy->constraints,
				   freq_dev->max_req, FREQ_QOS_MAX,
				   policy->max);

	if (ret < 0) {
		pr_err("add cpu%d max qos request err\n", policy->cpu);
		goto add_qos_max_fail;
	}
#endif

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	dev = per_cpu(cpuidle_devices, policy->cpu);
	drv = cpuidle_get_cpu_driver(dev);
	if (drv) {
		freq_dev->cpuidle_c4_exit_latency =
			drv->states[drv->state_count - 1].exit_latency;
	} else {
		if (cpumask_first(policy->related_cpus) == SILVER_IDX)
			freq_dev->cpuidle_c4_exit_latency = SILVER_CPUIDLE_C4_EXIT_LATENCY;
		else
			freq_dev->cpuidle_c4_exit_latency = GOLD_CPUIDLE_C4_EXIT_LATENCY;
	}

	pr_info("%s, cpu=%d, exit_latency=%u", __func__, policy->cpu,
		freq_dev->cpuidle_c4_exit_latency);
#endif
	...
	init_irq_work(&freq_dev->irq_work, drg_freq_irq_work);
	kthread_init_work(&freq_dev->drg_freq_work, drg_freq_set_freq_work);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
add_qos_max_fail:
	freq_qos_remove_request(freq_dev->min_req);
add_qos_min_fail:
	kfree(freq_dev->max_req);
	freq_dev->max_req = NULL;
alloc_max_fail:
	kfree(freq_dev->min_req);
	freq_dev->min_req = NULL;
#endif
unlock:
			       ...
			       ssize_t count)
{
	...
	count += scnprintf(buf + count, PAGE_SIZE - count,
			   "\n\t\tmin: %lu\n\t\tmax: %lu\n",
			   drg_dev->min_freq, drg_dev->max_freq);

	count += scnprintf(buf + count, PAGE_SIZE - count,
			   "\t\tstate: %d\n\t\tidle: %d\n",
			   drg_dev->state, drg_dev->idle);
...
static ssize_t show_rule_state(const struct drg_rule *ruler, char *buf)
{
	...
	for (i = 0; i < ruler->master_num; i++)
		count = show_drg_device(&ruler->master_dev[i], buf, count);

	count += scnprintf(buf + count, PAGE_SIZE - count, "slave:\n");
	for (i = 0; i < ruler->slave_num; i++)
		count = show_drg_device(&ruler->slave_dev[i], buf, count);
...
static ssize_t store_enable(struct drg_rule *ruler,
			   ...
			   struct device_node *np, const char *prop)
{
	...
	*drg_dev = (struct drg_device *)
	...
	for (i = 0; i < *dev_num; i++) {
		...
		memcpy(t_drg_dev->divider, drg_dev_spec.args, divider_size);
	}
...
static int drg_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, child) {
		...
		if (of_property_read_u32(child, "huawei,divider-cells",
					 &ruler->divider_num)) {
	...
	isolate_unisolate_notifier_register(&drg_isolate_policy_nb);

	g_drg_work_thread = kthread_run(kthread_worker_fn,
					(void *)&g_drg_worker, "hw-drg");
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	register_trace_android_rvh_drg_cpufreq_check_limit(
					drg_cpufreq_check_limit,
					NULL);
#endif
	return 0;
...
err_probe:
	...
	isolate_unisolate_notifier_unregister(&drg_isolate_policy_nb);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static int __drg_cpufreq_register(void)
{
	struct cpufreq_policy *policy;
	int cpu;
	...
	for_each_possible_cpu(cpu) {
		policy = cpufreq_cpu_get(cpu);
		if (!policy) {
			pr_err("%s, cpufreq policy is NULL", __func__);
			return -ENODEV;
		}
...
void drg_init(void)
{
	...
	ret = __drg_cpufreq_register();
	if (ret) {
		pr_err("%s, drg cpufreq register fail", __func__);
		return;
	}

	ret = platform_driver_register(&drg_driver);
	if (ret) {
		pr_err("drg platform_driver_register fail!");
		return;
	}

	pr_info("%s, drg platform driver register success", __func__);
}
#else
module_platform_driver(drg_driver);
#endif

MODULE_DESCRIPTION("dual rail governor");
