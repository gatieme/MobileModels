...
static int charger_loginfo_dump_content(char *buffer, int size, void *dev_data)
{
	...
	if (power_supply_check_psy_available("usb", &di->usb_psy)) {
		...
		power_supply_get_int_prop(POWER_PLATFORM_USB_PSY_NAME,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &d_data.usb_vol, 0, POWER_UV_PER_MV);
	}
	if (power_supply_check_psy_available("battery", &di->batt_psy)) {
		d_data.ch_en = charge_get_charge_enable_status();
	...
	(void)snprintf(buffer, size, "%-6d %-10s  %-7d"
		...
		charge_sysfs_get_sysfs_value(CHARGE_SYSFS_IIN_THERMAL), d_data.ch_en, d_data.status, d_data.health,
