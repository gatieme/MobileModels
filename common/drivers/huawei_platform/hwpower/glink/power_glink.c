...
#define ADSP_JEITA_TABLE_LEN              16
...
/* rsp message data for notify operation */
struct power_glink_notify_rsp_msg {
	struct pmic_glink_hdr hdr;
	u32 seq_no;
};
...
struct power_glink_init_data {
	...
	u32 iterm;
	u32 jeita_hot_th;
	u32 fcp_pd_support_icl;
	u32 battery_adc_channel;
	u32 charge_flag; /* bit0: AICL_RERUN flag */
	u32 bat_cathode_type;
	u32 fcp_pd_max_cur;
};
...
struct power_glink_dev {
	...
	u32 adsp_jeita_table[ADSP_JEITA_TABLE_LEN];
	u32 reinit_jeita_flag;
	u32 last_seq_no;
	struct work_struct notify_rsp_work;
	int gpio_no;
	bool bat_cathode_type_det;
};
...
EXPORT_SYMBOL_GPL(power_glink_get_property_value);
...
EXPORT_SYMBOL_GPL(power_glink_set_property_value);
...
static void power_glink_notify_rsp_work(struct work_struct *work)
{
	struct power_glink_dev *l_dev = container_of(work,
		struct power_glink_dev, notify_rsp_work);
	struct power_glink_notify_rsp_msg msg = { { 0 } };
	...
	/* prepare header */
	msg.hdr.owner = l_dev->cust_data.msg_owner_id;
	msg.hdr.type = POWER_GLINK_MSG_TYPE_NOTIFY;
	msg.hdr.opcode = POWER_GLINK_MSG_OPCODE_NOTIFY;
	/* prepare data */
	msg.seq_no = l_dev->last_seq_no;

	if (pmic_glink_write(l_dev->client, &msg, sizeof(msg)))
		hwlog_err("%s: send message error\n", __func__);
}
	...
	void *data, size_t len)
{
	...
	if ((l_dev->last_seq_no + 1) != msg->seq_no)
		hwlog_err("lost notify msg: %u:%u\n", l_dev->last_seq_no, msg->seq_no);

	l_dev->last_seq_no = msg->seq_no;
	schedule_work(&l_dev->notify_rsp_work);
	hwlog_info("notify_rx_msg: seq_no=%u notify_id=%u value=%u\n", msg->seq_no,
...
static void power_glink_parse_dts(struct power_glink_dev *l_dev)
{
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		...
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"iterm", &l_dev->init_data.iterm,
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"jeita_hot_th", &l_dev->init_data.jeita_hot_th,
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"fcp_pd_support_icl", &l_dev->init_data.fcp_pd_support_icl,
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"fcp_pd_max_cur", &l_dev->init_data.fcp_pd_max_cur,
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"battery_adc_channel", &l_dev->init_data.battery_adc_channel,
		POWER_GLINK_INIT_INVALID_DATA);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"charge_flag", &l_dev->init_data.charge_flag,
		POWER_GLINK_INIT_INVALID_DATA);
	l_dev->bat_cathode_type_det = of_property_read_bool(l_dev->dev->of_node,
		"huawei,bat_cathode_type_det");
	hwlog_info("%s bat_cathode_type_det=%u\n", __func__, l_dev->bat_cathode_type_det);

	if (!power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), l_dev->dev->of_node,
		"adsp_jeita_table", l_dev->adsp_jeita_table, ADSP_JEITA_TABLE_LEN))
		l_dev->reinit_jeita_flag = 1;
}
...
int power_glink_support_fcp_pd_icl(void)
{
	...
	return l_dev->init_data.fcp_pd_support_icl;
}

extern int haptics_enable_hboost_5v(int value);
static struct power_glink_interface_ops interface_ops = {
	.power_glink_get_property_value = power_glink_get_property_value,
	.power_glink_set_property_value = power_glink_set_property_value,
	.power_glink_support_fcp_pd_icl = power_glink_support_fcp_pd_icl,
	.is_dcp_ever_detected = is_dcp_ever_detected,
	.power_glink_handle_dc_connect_message = power_glink_handle_dc_connect_message,
	.power_glink_handle_charge_type_message = power_glink_handle_charge_type_message,
	.haptics_enable_hboost_5v = haptics_enable_hboost_5v,
};

static void power_glink_init_work(struct work_struct *work)
{
	struct power_glink_dev *l_dev = container_of(work,
		struct power_glink_dev, init_work);
	...
	do {
		...
		if (l_dev->bat_cathode_type_det)
			l_dev->init_data.bat_cathode_type = bat_model_get_bat_cathode_type();
		else
			l_dev->init_data.bat_cathode_type = POWER_GLINK_INIT_INVALID_DATA;
		hwlog_info("bat_cathode_type=%u\n", l_dev->init_data.bat_cathode_type);
		if (status == ADSP_INIT_DONE) {
			...
			power_event_bnc_notify(POWER_BNT_HW_PD, POWER_NE_HW_PD_SET_GLINK_STATUS, NULL);
		}
		i++;
...
static int power_glink_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&l_dev->notify_rsp_work, power_glink_notify_rsp_work);
	...
	power_glink_interface_ops_register(&interface_ops);
