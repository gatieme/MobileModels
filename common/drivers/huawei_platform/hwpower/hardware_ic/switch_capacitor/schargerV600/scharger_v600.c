...
int hi6526_set_term_enable(struct charger_device *dev, bool enable)
{
	...
	if (enable) {
		...
		power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbatt_mv,
...
int hi6526_set_watchdog_timer_execute(struct hi6526_device_info *di, int value)
{
	...
	scharger_err("watch dog timer is %u,the register value is set %u\n",
		     dog_time, val);
...
static int hi6526_soft_vbatt_ovp_protect(struct charger_device *dev)
{
	...
	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		...
		POWER_SUPPLY_DEFAULT_VOLTAGE_NOW, POWER_UV_PER_MV);
	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_VOLTAGE_MAX, &vbatt_max,
		POWER_SUPPLY_DEFAULT_VOLTAGE_MAX,
		POWER_PLATFORM_BAT_VOLT_MAX_UNIT);
...
static struct huawei_usb_misc_ops hi6526_usb_misc_ops = {
	.sc_regn_enable = hi6526_set_sc_regn_enable,
	.adc_enable = hi6526_enable_adc_by_dfg,
};
...
static void hi6526_fcp_scp_ops_register(void)
{
	...
	for (i = 0; i < RET_SIZE_8; i++) {
		if (ret[i])
			scharger_err("register ops failed i %d!\n", i);
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (di->param_dts.ic_role == IC_ONE) {
		...
		hi6526_buck_opt_param(di, VBUS_VSET_5V);
		di->adc_wakelock = power_wakeup_source_register(&client->dev, "adc_wakelock");
		...
		di->adc_wakelock = power_wakeup_source_register(&client->dev, "adc_wakelock_aux");
	...
	if (di->param_dts.dfg_support) {
		hi6526_usb_misc_ops.dev_data = di;
		huawei_usb_misc_ops_register(&hi6526_usb_misc_ops);
	}
...
static void hi6526_i2c_complete(struct device *dev)
{
	...
	di = i2c_get_clientdata(client);
	if (!di || !di->param_dts.dfg_support)
		return;

	if (!di->disable_regn_by_dfg) {
		ret = hi6526_write_mask(di, VDM_CFG_REG_2_ADDR, SC_REGN_EN_MASK, SC_REGN_EN_SHIFT, !di->disable_regn_by_dfg);
		scharger_inf("%s need enable regn ret=%d\n", __func__, ret);
	}
}

static const struct dev_pm_ops hi6526_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(hi6526_i2c_suspend, hi6526_i2c_resume)
	.complete = hi6526_i2c_complete,
};
#define HI6526_PM_OPS (&hi6526_pm_ops)
#else
#define HI6526_PM_OPS (NULL)
#endif /* CONFIG_PM */
