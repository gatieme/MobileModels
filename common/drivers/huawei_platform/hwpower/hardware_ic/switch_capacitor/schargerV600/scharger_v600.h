...
#define scharger_err(fmt, args...) do { pr_err("[scharger]" fmt, ## args); } while (0)
#define scharger_evt(fmt, args...) do { pr_warn("[scharger]" fmt, ## args); } while (0)
#define scharger_inf(fmt, args...) do { pr_info("[scharger]" fmt, ## args); } while (0)
#define scharger_dbg(fmt, args...) do { pr_debug("[scharger]" fmt, ## args); } while (0)
...
struct hi6526_device_info {
	...
	u8 i2c_reg_page;
	u32 scp_error_flag;
	bool get_adc_value_enable;
	bool disable_adc_by_dfg;
	bool disable_regn_by_dfg;
	int iin_set;
	int i_bias_all;
	int last_ichg;
	int force_set_term_flag;
	int i2c_err_cnt;
	int loop_enabled;
	int ucp_ibus_abn_cnt;
	int wdg_ibus_abn_cnt;
	int charger_is_fcp;
	int is_weaksource;
	int chg_last_enable;
	int rst_gpio;
...
#define CHG_PD_ALERT_L_ADDR                 (SOC_SCHARGER_PD_ALERT_L_ADDR(0))
#define CHG_PD_ALERT_H_ADDR                 (SOC_SCHARGER_PD_ALERT_H_ADDR(0))
...
#define EFUSE_PDOB_PRE_ADDR0                0
#define EFUSE_PDOB_PRE_ADDR1                1
#define EFUSE_PDOB_PRE_ADDR2                2
#define EFUSE_PDOB_PRE_ADDR3                3
#define EFUSE_PDOB_PRE_ADDR4                4
#define EFUSE_PDOB_PRE_ADDR5                5
#define EFUSE_PDOB_PRE_ADDR6                6
#define EFUSE_PDOB_PRE_ADDR7                7
...
#define R_MOHM_DEFAULT                      R_MOHM_5 /* resisitance mohm */
...
void hi6526_direct_charge_fault_handle(struct hi6526_device_info *di, u32 lvc_sc_irq_state);
int hi6526_lvc_init(void *dev_data);
int hi6526_lvc_enable(int enable, void *dev_data);
int hi6526_sc_init(void *dev_data);
...
void hi6526_mask_pd_irq(struct hi6526_device_info *di);
void hi6526_unmask_all_irq(struct hi6526_device_info *di);
...
int hi6526_set_sc_regn_enable(bool enable, void *dev_data);
int hi6526_enable_adc_by_dfg(bool enable, void *dev_data);
