...
int hi6526_get_adc_value(struct hi6526_device_info *di, u32 chan, u32 *data)
{
	...
	if (!di->get_adc_value_enable && ((charge_get_charger_type() == CHARGER_REMOVED) || di->disable_adc_by_dfg)) {
		mutex_unlock(&di->adc_conv_lock);
		return 0;
	}
...
void hi6526_parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	(void)of_property_read_u32(np, "dfg_support", (u32 *)&(di->param_dts.dfg_support));
...
{
	int ret, ret0, ret1, ret2, ret3, ret4, ret5, ret6;
	struct hi6526_device_info *di = get_hi6526_device();
	...
	ret4 = hi6526_set_batfet_ctrl(di, CHG_BATFET_EN);
	ret5 = hi6526_set_otg_current(NULL, BOOST_LIM_1000);
	ret6 = hi6526_set_otg_enable(NULL, CHARGE_OTG_DISABLE);
...
int hi6526_set_sc_regn_enable(bool enable, void *dev_data)
{
	...
	di->disable_regn_by_dfg = !enable;
	scharger_inf("%s disable_regn_by_dfg=%u\n", __func__, di->disable_regn_by_dfg);
	return hi6526_write_mask(di, VDM_CFG_REG_2_ADDR, SC_REGN_EN_MASK, SC_REGN_EN_SHIFT, enable);
}
...
int hi6526_enable_adc_by_dfg(bool enable, void *dev_data)
{
	...
	di->disable_adc_by_dfg = !enable;
	scharger_inf("%s disable_adc_by_dfg=%u\n", __func__, di->disable_adc_by_dfg);
	return 0;
}
