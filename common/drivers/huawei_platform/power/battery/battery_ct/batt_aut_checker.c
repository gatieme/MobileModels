	...
	const char *key_id, char *buf)
{
	...
	if (drv_data->bco.prepare(drv_data, RES_CT, &res) ||
		(!res.data) || (res.len == 0)) {
		hwlog_err("[%s] prepare ecc signature failed\n", __func__);
		return -EINVAL;
	}

	hwlog_info("[%s] read act signature length is %d\n", __func__, res.len);
	...
	const char *buf, size_t count)
{
	...
	if (drv_data->bco.set_act_signature(drv_data, RES_ACT, &res)) {
		hwlog_err("[%s] set act signature to ic failed\n", __func__);
		return -EINVAL;
	}
...
static int set_cert_ready(struct batt_chk_data *drv_data)
{
	...
	if (drv_data->bco.set_batt_safe_info(drv_data, BATT_CERT_READY,
		&temp)) {
		hwlog_err("[%s] write cert ready state to ic failed\n", __func__);
		return -1;
	}
	return 0;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* return earlier to avoid setting cert ready */
	if (drv_data->skip_checking_act) {
		hwlog_info("[%s] checker no:%d group:%d skip checking act\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		return count;
	}

	hwlog_info("[%s] set act %lu signature\n", __func__, count);
	if (verrify_act_signature(drv_data, buf, count) ||
		(drv_data->act_state != ACT_PASS)) {
		hwlog_err("[%s] verify act signature failed %d\n",
			__func__, drv_data->act_state);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hwlog_info("[%s] set retrofit %lu signature\n", __func__, count);
	if (verrify_act_signature(drv_data, buf, count) ||
		(drv_data->act_state != RETROFIT_PASS)) {
		hwlog_err("[%s] verify retrofit signature fail %d\n",
			__func__, drv_data->act_state);
		return -EINVAL;
	}
	...
	if (set_cert_ready(drv_data))
		return -EINVAL;
#endif
	hwlog_info("[%s] set retrofit succ\n", __func__);
	return count;
}
				 ...
				 const char *buf, size_t count)
{
	...
	if (!drv_data)
		return -1;
	if (buf[count - 1] == '\n' &&
	    !kstrtouint(buf, POWER_BASE_DEC, &drv_data->free_cycles))
		return -1;
...
static ssize_t skip_checking_act_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct batt_chk_data *drv_data = NULL;
	...
	return snprintf(buf, PAGE_SIZE, "%u", drv_data->skip_checking_act);
}

static ssize_t skip_checking_act_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct batt_chk_data *drv_data = NULL;
	int flag = 0;
	...
	if (!buf || kstrtoint(buf, 0, &flag))
		return -1;

	drv_data->skip_checking_act = (unsigned int)flag;
	return count;
}
...
static const DEVICE_ATTR_RW(skip_checking_act);
#endif /* BATTERY_LIMIT_DEBUG */
...
static const struct attribute *batt_checker_attrs[] = {
	...
	&dev_attr_skip_checking_act.attr,
#endif /* BATTERY_LIMIT_DEBUG */
...
static int check_batt_ct_ops(struct batt_chk_data *drv_data)
{
	...
	if (!drv_data->ic_ops.get_batt_type) {
		...
		hwlog_err("[%s] get_batt_type is not valid\n", __func__);
	}
	if (!drv_data->ic_ops.certification) {
		ret = BATTERY_DRIVER_FAIL;
		hwlog_err("[%s] certification is not valid\n", __func__);
	}
	if (!drv_data->ic_ops.get_batt_sn) {
		ret = BATTERY_DRIVER_FAIL;
	...
	if (!drv_data->ic_ops.set_batt_safe_info) {
		ret = BATTERY_DRIVER_FAIL;
		hwlog_err("[%s] set_batt_safe_info is not valid\n", __func__);
	}
	if (!drv_data->ic_ops.get_batt_safe_info) {
		ret = BATTERY_DRIVER_FAIL;
		hwlog_err("[%s] get_batt_safe_info is not valid\n", __func__);
	}
...
static int get_ic_ops(struct device_node *node, struct batt_chk_data *drv_data)
{
	...
	if (num <= 0) {
		hwlog_err("[%s] No ic matchable for checker(%s)\n", __func__, node->name);
		return ic_unready;
	}
	...
	if (ret) {
		hwlog_err("[%s] %s read dev_ic_phandles failed %d\n", __func__, node->name, ret);
	...
	for (i = 0; i < RETYR_TIME; i++) {
		list_for_each_entry(pos, &batt_aut_ic_head, node) {
			...
			if (!ic_unready) {
				hwlog_info("[%s] %s find valid checker ic\n",
					   __func__, node->name);
	...
	if (ic_unready) {
		...
		hwlog_info("[%s] %s battery can't find ic\n", __func__, node->name);
		goto free_dev_ic_phandles;
	}
	...
	if (check_batt_ct_ops(drv_data)) {
		drv_data->result.ic_status = IC_FAIL_UNKOWN;
		hwlog_err("[%s] %s did not have enough operations!/n", __func__, node->name);
		goto free_dev_ic_phandles;
	}
...
static int battery_cycles_limit_init(struct platform_device *pdev)
{
	...
	if (!drv_data) {
		hwlog_err("[%s] %s can't find valid platform device drvdata\n",
	...
	int num)
{
	...
	hwlog_err("[%s] ic type %d for battery work  %d error\n", __func__, type, num);
	return -1;
}
...
static struct batt_chk_data *batt_chk_data_init(struct platform_device *pdev)
{
	...
	ret = of_property_read_u8(pdev->dev.of_node, "id-of-grp",
				  &drv_data->id_of_grp);
	if (ret) {
		hwlog_err("[%s] %s read grp_id failed\n", __func__, pdev->dev.of_node->name);
		return NULL;
	}

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), pdev->dev.of_node,
		"max_defer_second", &drv_data->max_defer_second, 0);
	hwlog_info("[%s] max_defer_second %u\n", __func__, drv_data->max_defer_second);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), pdev->dev.of_node,
		"skip_checking_act", &drv_data->skip_checking_act, 0);
	hwlog_info("[%s] skip_checking_act %u\n", __func__, drv_data->skip_checking_act);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), pdev->dev.of_node,
		"check_sn_type", &drv_data->check_sn_type, 0);
	hwlog_info("[%s] check_sn_type %u\n", __func__, drv_data->check_sn_type);
...
static int battery_aut_checker_probe(struct platform_device *pdev)
{
	int local_ic;
	struct batt_chk_data *drv_data = NULL;
	unsigned long long cur_time_to_boot;

	if (!pdev)
		return BATTERY_DRIVER_FAIL;
	hwlog_info("[%s] %s is going to probing\n", __func__, pdev->name);
	drv_data = batt_chk_data_init(pdev);
	if (!drv_data) {
		hwlog_err("[%s] battery checker(%s) probe failed\n", __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
	...
	if (local_ic) {
		cur_time_to_boot = ktime_to_timespec64(ktime_get_boottime()).tv_sec;
		if (cur_time_to_boot < (unsigned long long)drv_data->max_defer_second) {
			devm_kfree(&pdev->dev, drv_data);
			return -EPROBE_DEFER;
		}
		hwlog_err("[%s] %s get ic ops failed, cur_time_to_boot %llu, max_defer_second %u\n",
			__func__, pdev->name, cur_time_to_boot, drv_data->max_defer_second);
		return BATTERY_DRIVER_FAIL;
	}
	...
	if (battery_cycles_limit_init(pdev)) {
		hwlog_err("[%s] %s set cycles limit failed\n", __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
	/* battery node initialization */
	if (batt_checker_node_create(pdev)) {
		hwlog_err("[%s] %s create nodes failed\n", __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
	add_to_batt_checkers_lists(&drv_data->entry);
	hwlog_info("[%s] %s probed successfully\n", __func__, pdev->name);
