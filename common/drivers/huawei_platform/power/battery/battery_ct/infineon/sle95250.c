...
#ifdef BATTBD_FORCE_MATCH
static inline void sle95250_pm_qos_add_request(struct pm_qos_request *req, s32 value)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	cpu_latency_qos_add_request(req, value);
#else
	pm_qos_add_request(req, PM_QOS_CPU_DMA_LATENCY, value);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0) */
}

static inline void sle95250_pm_qos_remove_request(struct pm_qos_request *req)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
...
static void sle95250_dev_on(struct sle95250_dev *di)
{
	...
	sle95250_pm_qos_add_request(&di->pm_qos, 0);
	kick_all_cpus_sync();

	if (sched_setscheduler_nocheck(current, SCHED_FIFO, &param))
		hwlog_err("[%s] set current fifo fail\n", __func__);
	else
		hwlog_info("[%s] set current fifo sucess\n", __func__);

	if (di->sysfs_mode)
		bat_type_apply_mode(BAT_ID_SN);
	optiga_swi_slk_irqsave(di);
	hwlog_info("[%s] sle95250_dev_on, ic_index:%u\n", __func__, di->ic_index);
...
static void sle95250_dev_off(struct sle95250_dev *di)
{
	...
	hwlog_info("[%s] sle95250_dev_off, ic_index:%u\n", __func__, di->ic_index);
	...
	if (sched_setscheduler_nocheck(current, SCHED_NORMAL, &param))
		hwlog_err("[%s] set current normal fail\n", __func__);
	sle95250_pm_qos_remove_request(&di->pm_qos);
}
...
static int sle95250_do_act_in_kernel(struct sle95250_dev *di, uint8_t *act)
{
	int ret;
	uint8_t pglked = 0;
...
static int sle95250_do_act_in_bootloader(struct sle95250_dev *di, uint8_t *act)
{
	return sle95250_write_act_sign_nv(di, act);
}

static int sle95250_do_act(struct sle95250_dev *di, const struct power_genl_attr *res)
{
	uint16_t crc_act = 0xffff;
	...
	if (sle95250_is_act_sign_via_nv(di))
		return sle95250_do_act_in_bootloader(di, act);
	return sle95250_do_act_in_kernel(di, act);
}
...
static void sle95250_group_sn_info_parse(struct sle95250_dev *di)
{
	...
	if (!info) {
		hwlog_err("[%s] invalid ic index or group sn info all zero\n", __func__);
		return;
	}
...
static int sle95250_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		hwlog_err("[%s] dts parse fail, ic_index:%u\n",
			__func__, di->ic_index);
		goto sle95250_probe_fail;
	}

	ret = sle95250_ic_ck(di);
	if (ret) {
		hwlog_err("%s ic check fail\n", __func__);
		goto sle95250_probe_fail;
	}
