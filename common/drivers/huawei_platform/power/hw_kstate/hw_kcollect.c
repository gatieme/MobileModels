...
int report_link_info(int type, int uid)
{
	return kcollect(KCOLLECT_KEYLINK_MASK, "[TYPE %d MESSAGE][UID %d]", type, uid);
}

int report_igmp_info(int flag, int uid)
{
	return kcollect(KCOLLECT_IGMP_RPOXY_MASK, "[FLAG %d IGMP][UID %d]", flag, uid);
}
