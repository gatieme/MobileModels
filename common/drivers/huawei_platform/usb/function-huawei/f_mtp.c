/*
 * Copyright (c) Google, Inc. 2010-2020. All rights reserved.
...
#define HW_MTP_BUF_POOL

#ifdef HW_MTP_BUF_POOL
#define MTP_REQ_CNT 100
#endif
...
struct mtp_ext_config_desc_function {
	__u8 b_first_interface_number;
	__u8 b_interface_count;
	__u8 compatible_id[8]; /* just follow spec */
	__u8 sub_compatible_id[8]; /* just follow spec */
...
struct {
	...
	.header = {
		.dw_length = __cpu_to_le32(sizeof(mtp_ext_config_desc)),
		.bcd_version = __cpu_to_le16(0x0100), /* bcd version, follow spec */
		.w_index =  __cpu_to_le16(MTP_EXT_CONFIG_INDEX),
		.b_count = MTP_EXT_CONFIG_COUNT,
	},
	.function = {
		.b_first_interface_number = MTP_EXT_CONFIG_FIRST_INTERFACE_NUM,
		.b_interface_count = MTP_EXT_CONFIG_INTERFACE_COUNT,
		.compatible_id = {'M', 'T', 'P'},
	},
};
...
struct mtp_buf_pool g_mtp_buf_pool[MTP_REQ_CNT] = {{ 0 }};

static char *mtp_get_buf(unsigned int length)
{
	int idx;
	for (idx = 0; idx < MTP_REQ_CNT; idx++) {
		if (!g_mtp_buf_pool[idx].used && (g_mtp_buf_pool[idx].length == length) && (g_mtp_buf_pool[idx].buf)) {
			g_mtp_buf_pool[idx].used = 1;
			return g_mtp_buf_pool[idx].buf;
		}
	}

	for (idx = 0; idx < MTP_REQ_CNT; idx++) {
		if ((!g_mtp_buf_pool[idx].used) && (g_mtp_buf_pool[idx].buf == NULL)) {
			g_mtp_buf_pool[idx].buf = kmalloc(length, GFP_KERNEL);
			pr_info("%s:buffer idx is %d", __func__, idx);
			if (g_mtp_buf_pool[idx].buf) {
				(void)memset_s(g_mtp_buf_pool[idx].buf, length, 0, length);
				g_mtp_buf_pool[idx].used = 1;
				g_mtp_buf_pool[idx].length = length;
				return g_mtp_buf_pool[idx].buf;
			} else {
				pr_err("%s Error to kmalloc MTP buf pool\n", __func__);
				return NULL;
			}
		}
	}
...
static void mtp_free_buf(char *buf)
{
	int idx;
	for (idx = 0; idx < MTP_REQ_CNT; idx++) {
		if (buf == g_mtp_buf_pool[idx].buf) {
			g_mtp_buf_pool[idx].used = 0;
			return;
		}
	}
}
...
#ifdef HW_MTP_BUF_POOL
	req->buf = mtp_get_buf(buffer_size);
#else
...
struct usb_function *function_alloc_mtp_ptp(struct usb_function_instance *fi, bool mtp_config)
{
	...
	pr_err("%s++class=%d,subclass=%d,prot=%d\n", __func__);
