...
static const struct proc_ops subchiptype_proc_fops = {
	.proc_open		= subchiptype_proc_open,
	.proc_read		= seq_read,
...
static const struct proc_ops chiptype_proc_fops = {
	.proc_open		= chiptype_proc_open,
	.proc_read		= seq_read,
	.proc_lseek 	= seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops bt_fm_ver_proc_fops = {
	.proc_open	= bt_fm_ver_proc_open,
	.proc_read	= seq_read,
	.proc_lseek	= seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops bt_power_level_proc_fops = {
	.proc_open	    = bt_power_level_proc_open,
	.proc_read	    = seq_read,
	.proc_lseek	    = seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops bt_nv_path_proc_fops = {
	.proc_open	    = bt_nv_path_proc_open,
	.proc_read	    = seq_read,
	.proc_lseek	    = seq_lseek,
	.proc_release	= single_release,
};
...
/* 是否国内、海外共用一个boardID */
const void *get_bt_is_cn_and_ovs_share_boardid(void)
{
	int bt_property_len;
	struct device_node *dp = NULL;
	...
	return of_get_property(dp,"bt,is_cn_and_ovs_share_boardid", &bt_property_len);
}
EXPORT_SYMBOL(get_bt_is_cn_and_ovs_share_boardid);

static int bt_is_cn_and_ovs_share_boardid_show(struct seq_file *m, void *v)
{
	const char *is_cn_ovs_co_bid = NULL;

	is_cn_ovs_co_bid = get_bt_is_cn_and_ovs_share_boardid();
	if (is_cn_ovs_co_bid != NULL)
	{
		FMDBG("%s enter is_cn_and_ovs_share_boardid:%s;\n", __func__, is_cn_ovs_co_bid);
		seq_printf(m, "%s", is_cn_ovs_co_bid);
	} else {
		FMDERR("%s get is_cn_and_ovs_share_boardid failed;\n", __func__);
	}

	return 0;
}

static int bt_is_cn_and_ovs_share_boardid_open(struct inode *inode, struct file *file)
{
	return single_open(file, bt_is_cn_and_ovs_share_boardid_show, NULL);
}

static const struct proc_ops bt_is_cn_and_ovs_share_boardid_proc_fops = {
	.proc_open    = bt_is_cn_and_ovs_share_boardid_open,
	.proc_read    = seq_read,
	.proc_lseek    = seq_lseek,
	.proc_release    = single_release,
};
...
static const struct proc_ops fm_sinr_threshold_proc_fops = {
	.proc_open	    = fm_sinr_threshold_proc_open,
	.proc_read	    = seq_read,
	.proc_write	    = fm_sinr_threshold_write,
	.proc_lseek	    = seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops fm_sinr_samples_proc_fops = {
	.proc_open	    = fm_sinr_samples_proc_open,
	.proc_read	    = seq_read,
	.proc_write	    = fm_sinr_samples_write,
	.proc_lseek	    = seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops fm_buildin_proc_fops = {
	.proc_open	 = fm_buildin_proc_open,
	.proc_read	 = seq_read,
	.proc_write	 = fm_buildin_write,
	.proc_lseek  = seq_lseek,
	.proc_release = single_release,
};

static void bt_remove_proc(void)
{
	...
	remove_proc_entry("is_cn_and_ovs_share_boardid", device_dir);
	remove_proc_entry("connectivity", 0);
}

static int bt_chiptype_dir_init(void)
{
	int retval = 0;
...
static int bt_nv_dir_init(void)
{
	int retval = 0;
	struct proc_dir_entry *ent = NULL;
	...
	/* Creating read/write "is_cn_and_ovs_share_boardid" entry */
	ent = proc_create("is_cn_and_ovs_share_boardid", 0, device_dir, &bt_is_cn_and_ovs_share_boardid_proc_fops);
	if (ent == NULL) {
		FMDERR("Unable to create /proc/%s/is_cn_and_ovs_share_boardid entry", PROC_DIR);
		retval = -ENOMEM;
		goto fail;
	}
...
static int bt_param_dir_init(void)
{
	int retval = 0;
	struct proc_dir_entry *ent = NULL;
...
static int __init featuretransfer_init(void)
{
	...
	retval = bt_chiptype_dir_init();
	if (retval != 0) {
		return retval;
	}
	
	retval = bt_nv_dir_init();
	if (retval != 0) {
		return retval;
	}
	
	retval = bt_param_dir_init();
	if (retval != 0) {
		return retval;
	}
