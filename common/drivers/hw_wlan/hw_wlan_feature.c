 ...
 * Description: wlan platform feature
 * Author: y00265214
 * Create: 2017-03-03
 */
...
static const struct wifi_chip_cfg wifi_chip_cfgs[] = {
	...
	{ "WIFI_QUALCOMM_WCN6750", "4.4", "wlan" },
	{ "WIFI_QUALCOMM_WCN6856", "4.5", "wlan"},
...
static int devtype_proc_open(struct inode *inode, struct file *file)
{
 ...
 static const struct proc_ops devtype_proc_fops = {
	.proc_open		= devtype_proc_open,
	.proc_read		= seq_read,
...
static int pubfd_proc_open(struct inode *inode, struct file *file)
{
 ...
 static const struct proc_ops pubfd_proc_fops = {
	.proc_open		= pubfd_proc_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static int wlan_fw_ver_proc_open(struct inode *inode, struct file *file)
{
 ...
 static const struct proc_ops wlan_fw_ver_proc_fops = {
	.proc_open		= wlan_fw_ver_proc_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static int fem_check_flag_open(struct inode *inode, struct file *file)
{
 ...
 static const struct proc_ops fem_check_flag_proc_fops = {
	.proc_open		= fem_check_flag_open,
	.proc_read		= seq_read,
	.proc_lseek	    = seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops fem_check_5g_flag_proc_fops = {
	.proc_open		= fem_check_5g_flag_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static const struct proc_ops ini_type_proc_fops = {
	.proc_open		= ini_type_proc_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static int hw_wlan_cust_is_oversea(void)
{
	int ret = WIFI_OVS_CUST;
	int i;
	loff_t pos;
	mm_segment_t old_fs;
	...
	static char cmd_line_content[CMD_LINE_LENGTH] = {0};

	const char cmd_file_path[] = "/proc/cmdline";
	const char vendorCounty[] = "vendorcountry";
	const char cust[WIFI_CN_CUST_NUM][20] = {
		"all/cn", "cmcc/cn", "telecom/cn", "porsche/cn",
		"alliot/cn", "cmcciot/cn", "telecomiot/cn",
		"allcta/cn", "cmcccta/cn", "telecomcta/cn"
	};

	old_fs = get_fs();
	/* 打开cmdline文件 */
	file = filp_open(cmd_file_path, O_RDONLY, 0);
	if (!IS_ERR(file)) {
		pos = 0;
		set_fs(KERNEL_DS);
		/* 读取文件 */
		vfs_read(file, cmd_line_content, sizeof(cmd_line_content), &pos);
		pr_err("hw_wlan_cust_is_oversea read pos0: %d.\n", pos);
		pr_err("hw_wlan_cust_is_oversea read first cmd_line_content: %s.\n", cmd_line_content);
		set_fs(old_fs);
		/* 打印文件 */
		filp_close(file, NULL);
	} else {
		pr_err("Can not find cmdline or open failed.\n");
		filp_close(file, NULL);
		return WIFI_CN_CUST;
	}

	/* 查找是否有vendorcountry 字样，如果没有的话，则是烧片版本， 默认返回国内 */
	result = strstr(cmd_line_content, vendorCounty);
	if (result == NULL) {
		pr_err("hw_wlan_cust_is_oversea read vendorCounty from cmdline is null!\n");
		return WIFI_CN_CUST;
	}
	/* find the string of "cn cust in the cmdLine" */
	for (i = 0; i < WIFI_CN_CUST_NUM; i++) {
		result = strstr(cmd_line_content, cust[i]);
		if (result != NULL) {
			pr_err("hw_wlan_cust_is_oversea read CHN cust from cmdline, cust is : %s!\n", cust[i]);
			return WIFI_CN_CUST;
		}
	}
	pr_err("hw_wlan_cust_is_oversea not read CHN cust from cmdline,so it is oversea product!\n");
...
int hw_wlan_get_cust_ini_path(char *ini_path, size_t len)
{
	...
	int ret;
	int country_type;
	const char comm[] = "common";

	country_type = hw_wlan_cust_is_oversea();
	ini_type = get_hw_wifi_ini_type();
	if (ini_type == NULL) {
		pr_err("hw_wlan: get_hw_wifi_ini_type fail");
		return -1;
	}
	/* ini 配置了common字段 表示国内海外共板 */
	result = strstr(ini_type, comm);

	/* 如果是海外, 并且配置了国内海外共板的标注 */
	if (country_type == WIFI_OVS_CUST && result != NULL) {
		ret = snprintf_s(ini_path, len, len, "WCNSS_qcom_cfg_%s_ovs.ini", ini_type);
		pr_err("hw_wlan: hw_wlan_get_cust_ini_path is ovs!");
	} else {
