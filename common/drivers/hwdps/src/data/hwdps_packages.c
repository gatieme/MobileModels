...
s32 get_package_hashtable_len(void)
{
	s32 count = 0;
	struct package_hashnode_t *package_hashnode = NULL;
	s32 bkt = 0;
	hash_for_each(g_packages_hashtable, bkt, package_hashnode, hash_list) {
		struct list_head *pos = NULL;
		list_for_each(pos, &(package_hashnode->pinfo_list))
			count++;
	}
	return count;
}
	...
	bool free_self)
{
	...
	kfree_sensitive(pinfo->pinfo);
	pinfo->pinfo = NULL;
	hwdps_utils_free_policy_ruleset(pinfo->ruleset_cache);
	kfree_sensitive(pinfo->ruleset_cache);
	...
	struct package_hashnode_t *package_hashnode)
{
	...
	kfree_sensitive(package_hashnode);
}
