	...
	union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		pval->intval = coul_interface_get_battery_voltage(type) * POWER_UV_PER_MV;
		break;
...
static int bat_core_get_charge_counter(void)
{
	int val = POWER_SUPPLY_DEFAULT_CHARGE_COUNTER;
	struct bat_core_device *di = g_bat_core_dev;

	if (di)
		val = coul_interface_get_battery_charge_counter(di->config.coul_type);
...
static struct power_supply_ops bat_core_raw_power_supply_ops = {
	...
	.get_charge_counter_prop = bat_core_get_charge_counter,
...
static void bat_core_update_work_interval(struct bat_core_device *di)
{
	...
	if (di->data.health == POWER_SUPPLY_HEALTH_UNKNOWN) {
		di->work_interval = BAT_CORE_WORK_INTERVAL_ABNORMAL;
		return;
	}
...
static int bat_core_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(system_freezable_power_efficient_wq, &di->monitor_work, 0);
