...
struct multi_bat_ck_dev {
	...
	struct bat_connect_para conn_para[BAT_CONN_CNT_MAX];
};
...
{
	int i, id0, id1, diff_vol, diff_soc, diff_soc_th;
...
int multi_bat_ck_get_support_mode(int cur_mode, bool cur_flag)
{
	...
	for (i = 0; i < di->bat_conn_len; i++) {
		if (!multi_bat_check_thre(di->conn_para[i].thre_para, di->conn_para[i].thre_para_len, di->rt_info,
			cur_flag)) {
			hwlog_err("[%d]thre check fail\n", i);
			continue;
		}
		is_same_mode = (unsigned int)cur_mode & (unsigned int)di->conn_para[i].chg_mode;
		if (!multi_bat_check_diff(di->conn_para[i].diff_para, di->conn_para[i].diff_para_len, di->rt_info,
			is_same_mode)) {
			hwlog_err("[%d]diff check fail\n", i);
			continue;
		}

		hwlog_info("[%d]connect_mode=%d ,chg_mode=%d is support\n", i, di->conn_para[i].bat_conn_mode,
			di->conn_para[i].chg_mode);
	...
	struct bat_diff_para *info, int *info_len, const char *name)
{
	...
	*info_len = len / BAT_DIFF_MAX;
	for (row = 0; row < *info_len; row++) {
		...
		col = row * BAT_DIFF_MAX + BAT_DIFF_VOL;
		info[row].diff_vol = data[col];
		...
		hwlog_info("%s[%d]=%d %d %d %d %d\n", name, row,
			...
			info[row].diff_vol, info[row].exit_diff_soc,
			info[row].enter_diff_soc);
	}
}
...
static int multi_bat_ck_parse_conn_para(struct device_node *np, struct multi_bat_ck_dev *di)
{
	...
	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"bat_connect_para", BAT_CONN_CNT_MAX, BAT_CONN_MAX);
	if (len < 0)
		return -EPERM;

	for (i = 0; i < len; i++) {
		if (power_dts_read_string_index(power_dts_tag(HWLOG_TAG),
			np, "bat_connect_para", i, &tmp_string))
			return -EPERM;
		...
		case BAT_CONN_MODE:
			if (kstrtoint(tmp_string, POWER_BASE_DEC, &idata))
				return -EPERM;
			if ((idata >= BAT_CONN_CNT_MAX) || (idata < 0))
				return -EPERM;
			di->conn_para[row].bat_conn_mode = idata;
			break;
		case BAT_CHG_MODE:
			if (kstrtoint(tmp_string, POWER_BASE_DEC, &idata))
				return -EPERM;
			if (idata < 0)
				return -EPERM;
			di->conn_para[row].chg_mode = idata;
			break;
		case BAT_THRE_NAME:
			multi_bat_ck_parse_thre_para(np, di->conn_para[row].thre_para,
				&di->conn_para[row].thre_para_len, tmp_string);
			break;
		case BAT_DIFF_NAME:
			multi_bat_ck_parse_diff_para(np, di->conn_para[row].diff_para,
				&di->conn_para[row].diff_para_len, tmp_string);
			break;
		default:
			break;
		}
	...
	di->bat_conn_len = len / BAT_CONN_MAX;
	for (i = 0; i < di->bat_conn_len; i++)
		hwlog_info("bat_para[%d]=%d %d\n", i, di->conn_para[i].bat_conn_mode, di->conn_para[i].chg_mode);
...
static void multi_bat_ck_clear_info(struct multi_bat_ck_dev *di)
{
	int i, j;
	...
	for (i = 0; i < di->bat_conn_len; i++) {
		for (j = 0; j < di->conn_para[i].thre_para_len; j++) {
			di->conn_para[i].thre_para[j].en_cur_hys = false;
			di->conn_para[i].thre_para[j].cur_cnt = 0;
		}
	}
}
