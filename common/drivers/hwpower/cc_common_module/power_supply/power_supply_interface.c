...
EXPORT_SYMBOL_GPL(power_supply_sync_changed);
...
void power_supply_get_int_prop(const char *name,
	enum power_supply_property psp, int *val, int dflt, int unit)
{
	int ret;
	union power_supply_propval union_val = { 0 };

	if (!name || !val) {
		hwlog_err("name or val is null, use default value\n");
		*val = dflt;
		return;
	}
	...
	if (ret) {
		hwlog_err("get prop fail, use default value\n");
		*val = dflt;
		return;
	}

	*val = union_val.intval / unit;
}
EXPORT_SYMBOL_GPL(power_supply_get_int_prop);
...
EXPORT_SYMBOL_GPL(power_supply_get_int_property_value);
...
EXPORT_SYMBOL_GPL(power_supply_set_int_property_value);
