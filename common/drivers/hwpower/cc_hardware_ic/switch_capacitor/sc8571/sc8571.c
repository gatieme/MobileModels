...
static int bq25980_otg_enable(struct sc8571_device_info *di, bool enable)
{
	int ret;

	u8 otg_en = enable ? SC8571_OTG_ENABLE : SC8571_OTG_DISABLE;
	u8 acdrv1_on = enable ? SC8571_ACDRV1_ON : SC8571_ACDRV1_OFF;
	...
	ret += sc8571_write_mask(di, SC8571_CTRL2_REG,
		SC8571_ACDRV1_STAT_MASK, SC8571_ACDRV1_STAT_SHIFT, acdrv1_on);
	if (ret)
...
static int sc8571_lvc_charge_enable(int enable, void *dev_data)
{
	...
	if (di->device_id == SWITCHCAP_BQ25980) {
		ret += bq25980_otg_enable(di, !chg_en);
...
static int sc8571_reg_init(struct sc8571_device_info *di)
{
	...
	if (di->device_id == SWITCHCAP_BQ25980)
		ret += bq25980_otg_enable(di, true);
