...
static int sm5450_get_device_id(void *dev_data)
{
	...
	case SY69636_DEVICE_ID_SY69636:
		...
		break;
	case SGM41600_DEVICE_ID_SGM41600:
		di->device_id = SWITCHCAP_SGM41600;
		break;
	default:
...
static int sm5450_get_sc_max_ibat(void *dev_data, int *ibat)
{
	struct sm5450_device_info *di = dev_data;
	...
	if (di->sm5450_sc_para.max_ibat <= 0) {
		hwlog_err("sc_max_ibat read fail\n");
		return -EPERM;
	}

	*ibat = di->sm5450_sc_para.max_ibat;
	hwlog_info("sc_max_ibat : %d\n", di->sm5450_sc_para.max_ibat);

	return 0;
}

static int sm5450_get_lvc_max_ibat(void *dev_data, int *ibat)
{
	struct sm5450_device_info *di = dev_data;
	...
	if (di->sm5450_lvc_para.max_ibat <= 0) {
		hwlog_err("lvc_max_ibat read fail\n");
		return -EPERM;
	}

	*ibat = di->sm5450_lvc_para.max_ibat;
	hwlog_info("lvc_max_ibat : %d\n", di->sm5450_lvc_para.max_ibat);
...
static int sm5450_get_vbat_mv(void *dev_data)
{
	...
	if ((di->device_id == SWITCHCAP_SM5450) && di->vbat_complete_flag) {
		hwlog_info("vbat_average=%d", di->vbat_average);
		return di->vbat_average;
	}

	ret = sm5450_write_mask(di, SM5450_ADCCTRL_REG,
		SM5450_ADCCTRL_ENADC_MASK, SM5450_ADCCTRL_ENADC_SHIFT, 0);
	ret += sm5450_write_mask(di, SM5450_ADCCTRL_REG, SM5450_ADCCTRL_ADCMODE_MASK,
		SM5450_ADCCTRL_ADCMODE_SHIFT, 0);
	ret += sm5450_write_mask(di, SM5450_ADCCTRL_REG, SM5450_ADCCTRL_ENADC_MASK,
		SM5450_ADCCTRL_ENADC_SHIFT, 1);
	if (ret)
		return -EPERM;
...
{
	struct sm5450_device_info *di = dev_data;
	int ocp_th = (di->sm5450_lvc_para.ibus_ocp > 0) ? di->sm5450_lvc_para.ibus_ocp : 5500; /* 5.5A */

	/* needed setting value */
	sm5450_config_rlt_ovp_ref(SM5450_RLTVOVP_REF_1P25, di);
	sm5450_config_rlt_uvp_ref(SM5450_RLTVUVP_REF_1P01, di);
	sm5450_config_vbuscon_ovp_ref_mv(SM5450_BP_VBUSCON_OVP_REF_INIT, di);
	sm5450_config_vbus_ovp_ref_mv(SM5450_BP_VBUS_OVP_REF_INIT, di);
	sm5450_config_ibus_ocp_ref_ma(ocp_th, LVC_MODE, di);
...
{
	struct sm5450_device_info *di = dev_data;
	int ocp_th = (di->sm5450_sc_para.ibus_ocp > 0) ? di->sm5450_sc_para.ibus_ocp : (int)di->sc_ibus_ocp; /* default value 3A if dts no config */
	...
	sm5450_config_vbuscon_ovp_ref_mv(SM5450_VBUSCON_OVP_REF_INIT, di);
	sm5450_config_vbus_ovp_ref_mv(SM5450_VBUS_OVP_REF_INIT, di);
	sm5450_config_ibus_ocp_ref_ma(ocp_th, SC_MODE, di);
	sm5450_config_ibus_ucp_ref_ma(SM5450_BUS_UCP_BASE_300MA_150MA, di); /* 300/150mA */
	sm5450_config_vbat_ovp_ref_mv(SM5450_VBAT_OVP_REF_INIT, di); /* VBAT_OVP 4.8V */
	sm5450_config_ibat_ocp_ref_ma(di->ibat_ocp_sc, di); /* default value 6A if dts no config */
	sm5450_config_ibat_reg_ref_ma(SM5450_IBATREG_REF_BELOW_500MA, di); /* ibat_ocp_ref - 0.5 = 5.5A */
	sm5450_config_vbat_reg_ref_mv(SM5450_VBATREG_REF_BELOW_50MV, di); /* vbat_ovp_ref - 0.05 = 4.45V */
}
...
static int sm5450_reg_init(void *dev_data)
{
	...
	if (di->device_id == SWITCHCAP_SGM41600) {
		ret += sm5450_write_byte(di, SM5450_FLAG_MASK3_REG, SGM41600_FLAG_MASK3_INIT_REG);
	} else {
	...
	if ((di->device_id == SWITCHCAP_SYH69636) || (di->device_id == SWITCHCAP_SGM41600))
...
static int sm5450_enable_adc(int enable, void *dev_data)
{
	int ret;
	u8 reg = 0;
	...
	ret = sm5450_write_mask(di, SM5450_ADCCTRL_REG, SM5450_ADCCTRL_ENADC_MASK, SM5450_ADCCTRL_ENADC_SHIFT, value);
	if (ret)
		return -EPERM;

	ret = sm5450_read_byte(di, SM5450_ADCCTRL_REG, &reg);
	if (ret)
		return -EPERM;

	hwlog_info("adc_enable [%x]=0x%x\n", SM5450_ADCCTRL_REG, reg);
	return 0;
}
...
static struct dc_ic_ops sm5450_lvc_ops = {
	...
	.get_max_ibat = sm5450_get_lvc_max_ibat,
};
...
static struct dc_ic_ops sm5450_sc_ops = {
	...
	.kick_ic_watchdog = sm5450_kick_watchdog_ms,
	.ic_reg_reset_and_init = sm5450_reg_reset_and_init,
	.get_max_ibat = sm5450_get_sc_max_ibat,
};
	...
	struct nty_data *data)
{
	...
	if (flag0 & SGM41600_FLAG1_VDRP_OVP_FLAG_MASK)
		hwlog_info("SGM41600 VDRP OVP happened\n");
}
...
static void sm5450_vbat_work(struct work_struct *work)
{
	u16 data = 0;
	int voltage = 0;
	int i = 0;
	...
	di = container_of(work, struct sm5450_device_info, vbat_work.work);
	if (!di) {
	...
	ret = sm5450_write_mask(di, SM5450_ADCCTRL_REG,
		SM5450_ADCCTRL_ENADC_MASK, SM5450_ADCCTRL_ENADC_SHIFT, 0);
	ret += sm5450_write_mask(di, SM5450_ADCCTRL_REG, SM5450_ADCCTRL_ADCMODE_MASK,
		SM5450_ADCCTRL_ADCMODE_SHIFT, 1);
	if (ret) {
		di->vbat_complete_flag = false;
		hwlog_err("Disable ADC err\n");
		return;
	}

	for (i = 0; i < SM5450_VBATADC_SAMPLING_CNT; i++) {
		ret = sm5450_write_mask(di, SM5450_ADCCTRL_REG, SM5450_ADCCTRL_ENADC_MASK,
			SM5450_ADCCTRL_ENADC_SHIFT, 1);
		if (ret) {
			di->vbat_complete_flag = false;
		...
		usleep_range(12000, 13000); /* sleep 12ms */
	...
	voltage /= SM5450_VBATADC_SAMPLING_CNT;

	di->vbat_average = voltage;
	di->vbat_complete_flag = true;
	...
	schedule_delayed_work(&di->vbat_work, msecs_to_jiffies(100)); /* delay 100ms */
}

static int sm5450_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct sm5450_device_info *di = NULL;

	di = container_of(nb, struct sm5450_device_info, event_nb);
	if (!di)
		return NOTIFY_OK;
...
static void sm5450_check_reset(struct sm5450_device_info *di)
{
	u16 flag = 0;

	if (sm5450_read_word(di, SM5450_VBATADC_H_REG, &flag))
		return;

	/*
	* it means IC reset when VBATADC or other ADC is 0
	* when reset happened, reinit IC register (30ms delay)
	*/
	if (!flag) {
		msleep(30);
		hwlog_info("sm5450 reset happened");
		(void)sm5450_reg_init(di);
	}
}
...
static int sm5450_irq_init(struct sm5450_device_info *di, struct device_node *np)
{
	int ret;
	...
	ret = power_gpio_config_interrupt(np,
		"intr_gpio", "sm5450_gpio_int", &di->gpio_int, &di->irq_int);
	if (ret)
		return ret;
...
static void sm5450_parse_dts(struct device_node *np, struct sm5450_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ibat_ocp_sc", &di->ibat_ocp_sc, SM5450_IBAT_OCP_SC_DEFAULT_VAL);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_ibus_ocp", &di->sc_ibus_ocp, SM5450_IBUS_OCP_REF_INIT);
}

static int sm5450_parse_mode_para_dts(struct device_node *np,
	struct sm5450_mode_para *data, const char *name, int id)
{
	...
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		name, SM5450_COMP_MAX_NUM, SM5450_INFO_TOTAL);
	if (array_len < 0)
		return -EPERM;

	for (row = 0; row < array_len / SM5450_INFO_TOTAL; row++) {
		col = row * SM5450_INFO_TOTAL + SM5450_INFO_IC_NAME;
	...
	if (index >= 0) {
		col = index * SM5450_INFO_TOTAL + SM5450_INFO_MAX_IBAT;
		power_dts_read_string_index(power_dts_tag(HWLOG_TAG),
			np, name, col, &tmp_string);
		kstrtoint(tmp_string, POWER_BASE_DEC, &idata);
		data->max_ibat = idata;
		col = index * SM5450_INFO_TOTAL + SM5450_INFO_IBUS_OCP;
		power_dts_read_string_index(power_dts_tag(HWLOG_TAG),
			np, name, col, &tmp_string);
		kstrtoint(tmp_string, POWER_BASE_DEC, &idata);
		data->ibus_ocp = idata;
...
static int sm5450_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sm5450_parse_mode_para_dts(np, &di->sm5450_lvc_para, "lvc_para", di->device_id);
	sm5450_parse_mode_para_dts(np, &di->sm5450_sc_para, "sc_para", di->device_id);
	sm5450_init_lock_mutex(di);
	sm5450_interrupt_clear(di);

	if ((di->device_id == SWITCHCAP_SM5450) && (di->rev_id != 0)) {
		INIT_DELAYED_WORK(&di->vbat_work, sm5450_vbat_work);
		di->event_nb.notifier_call = sm5450_notifier_call;
...
#ifdef CONFIG_PM
static int sm5450_i2c_suspend(struct device *dev)
{
...
static int sm5450_i2c_resume(struct device *dev)
{
	return 0;
}

static void sm5450_i2c_complete(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct sm5450_device_info *di = NULL;
	...
	sm5450_enable_adc(1, (void *)di);
}

static const struct dev_pm_ops sm5450_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(sm5450_i2c_suspend, sm5450_i2c_resume)
	.complete = sm5450_i2c_complete,
};
#define SM5450_PM_OPS (&sm5450_pm_ops)
#else
#define SM5450_PM_OPS (NULL)
#endif /* CONFIG_PM */
