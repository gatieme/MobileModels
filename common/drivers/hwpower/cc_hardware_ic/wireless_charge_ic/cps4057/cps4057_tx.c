	...
	unsigned int client)
{
	...
	case WLTX_CLIENT_COOLINGCASE:
		di->tx_init_para.pa_mode_en = 1; /* en pa mode */
		break;
	default:
		di->tx_init_para.ping_freq = di->tx_ping_freq;
		di->tx_init_para.ping_interval = CPS4057_TX_PING_INTERVAL;
...
static int cps4057_tx_set_init_para(struct cps4057_dev_info *di)
{
	...
	/*
	 * in pa mode, rx cannot adjust the tx parameter. fop,duty etc.
	 * tx not stop when rx send cet/rpp timeout.
	 */
	ret += cps4057_write_byte_mask(CPS4057_TX_FUNC_EN_ADDR, CPS4057_TX_PA_MODE_MASK,
		CPS4057_TX_PA_MODE_SHIFT, di->tx_init_para.pa_mode_en);
	if (ret) {
