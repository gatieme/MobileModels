...
{
	int ret;
	u16 addr = MT5806_BOOT_QF_BUFFER_ADDR; /* start from adrr 0x7F00 */
	u16 chksum = 0;
	u32 q_data = 0;
...
static int mt5806_fw_check_q_data(struct mt5806_dev_info *di)
{
	...
	ret = mt5806_read_word(di, MT5806_TX_CALI_CNT_ADDR, &q_cnt);
	ret += mt5806_read_word(di, MT5806_TX_CALI_WIDTH_ADDR, &q_width);
...
static int mt5806_fw_program_mtp(struct mt5806_dev_info *di)
{
	...
	if (ret) {
		hwlog_err("program_mtp: get_cali_q_val failed\n");
		goto exit;
	}
