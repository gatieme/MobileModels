...
static void stwlc88_set_mode_cfg(struct stwlc88_dev_info *di, int vset)
{
	...
	if (vset <= RX_HIGH_VOUT) {
		mod_cfg = di->mod_cfg.low_volt;
	} else {
		if (!power_cmdline_is_factory_mode())
			mod_cfg = di->mod_cfg.high_volt;
		else
			mod_cfg = di->mod_cfg.fac_high_volt;
	}

	stwlc88_ask_mode_cfg(mod_cfg);
}
...
static void stwlc88_rx_set_dummy_iload(struct stwlc88_dev_info *di)
{
	u8 iload;

	iload = di->dummy_iload / ST88_RX_DUMMY_ILOAD_STEP;
	(void)stwlc88_write_byte(ST88_RX_DUMMY_ILOAD_ADDR, iload);
}
