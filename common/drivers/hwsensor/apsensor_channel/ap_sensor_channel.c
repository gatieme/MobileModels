...
{
	if (open_count > 0) {
		hwlog_err("%s ap_sensor_channel already opened, open_count :%d\n", __func__, open_count);
		return -1;
	}
	open_count++;
	hwlog_info("%s ok, open_count :%d!\n", __func__, open_count);
	return 0;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	memcpy(acc_sensor_name, buf, size);
...
static ssize_t sensor_show_als1_info(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int index;
	char als_sensor_name[APP_INFO_VALUE_LENTH] = {0};
	int ret = app_info_proc_get("L1-Sensor", als_sensor_name);
	if (ret) {
		hwlog_err("%s, read L1-Sensor type failed : %d!\n", __func__, ret);
	} else {
		hwlog_info("%s, read L1-Sensor type : %s\n", __func__, als_sensor_name);
		index = 0;
		while (als_sensor_name[index] != ' ' && als_sensor_name[index] != '\0')
			index++;
		als_sensor_name[index] = '\0';
...
//	errno_t rt;
	...
	memcpy((char *)&finger_data[0], buf, FINGER_SENSE_MAX_SIZE);
//	if (rt != EOK)
...
//	errno_t rt;
	...
	if (!buf)
		return -EINVAL;
	mutex_lock(&fing_data_lock);
	memcpy(buf, (char *)&finger_data[0], FINGER_SENSE_MAX_SIZE);
//	if (rt != EOK)
//		hwlog_err("memcpy fg data error\n");
	mutex_unlock(&fing_data_lock);
...
static DEVICE_ATTR(als_info, 0664, sensor_show_als_info, NULL);
static DEVICE_ATTR(als1_info, 0664, sensor_show_als1_info, NULL);
...
static int __init ap_sensorhub_init(void)
{
	...
	// defalt lsm6ds3
	memcpy(acc_sensor_name, "acc_s001_002",
