...
EXPORT_SYMBOL(ap_sensor_route_init);
...
EXPORT_SYMBOL(ap_sensor_route_destroy);
...
EXPORT_SYMBOL(ap_sensor_route_read);
/*
...
*/
static inline void ap_sensor_write_to_fifo(struct inputhub_buffer_pos *pwriter,
...
ssize_t ap_sensor_route_write(char *buf, size_t count)
{
	...
	route_item = &package_route_tbl;
	header.timestamp = ktime_get_boottime_ns();
	for (i=0; i < count; i++)
	...
	if (writer.buffer_size < count + HEAD_SIZE) {
		spin_unlock_irqrestore(&route_item->buffer_spin_lock, flags);
		hwlog_info("writer.buffer_size too short %d %d.\n", writer.buffer_size, count);
		return 0;
	}
	...
	ap_sensor_write_to_fifo(&writer, buffer_begin, buffer_end, header.effect_addr, HEAD_SIZE);
	ap_sensor_write_to_fifo(&writer, buffer_begin, buffer_end, buf, count);
	...
	hwlog_info("ap_sensor_route_write route_item->pRead.buffer_size=%d.\n",route_item->pRead.buffer_size);
	return (count + HEAD_SIZE);
}
EXPORT_SYMBOL_GPL(ap_sensor_route_write);
