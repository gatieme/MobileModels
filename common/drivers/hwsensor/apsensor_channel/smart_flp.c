...
{
	pr_info("[%s] get context is ok?", __func__);
	return 0;
}
    ...
    unsigned int subtype, const char *buf, size_t count)
{
    ...
    pr_err("[%s] send cmd to slpi start", __func__);
    ...
    const char *buf, int buf_size, size_t count)
{
...
#endif
    memset((void *)flp_data, 0, sizeof(struct pkt_header));
    ...
    pr_info("[%s], %d, %d, %d, %zu\n", __func__, app_tag, cmd, buf_size, count);
...
EXPORT_SYMBOL_GPL(flp_port_resume);
/*
...
#endif
    memset((void *)&g_rd_info, 0, sizeof(g_rd_info));
...
static int flp_rd_info_resp(const char *head, size_t len)
{
	...
	if (len < sizeof(struct pkt_common_resp)) {
		pr_err("%s data too short:%d\n", __func__,len);
		return -ENOMEM;
	}
...
static int flp_shmem_resp(const char *head, size_t len)
{
	...
	if (len < sizeof(struct flp_shmem_resp_pkt)) {
		pr_err("%s data too short:%d\n", __func__,len);
		return -ENOMEM;
	}
...
#endif
    memset(flp_port, 0, sizeof(struct flp_port_t));
