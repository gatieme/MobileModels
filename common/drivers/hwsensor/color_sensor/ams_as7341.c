...
static int as7341_report_data(int value[])
{
	...
	memcpy(&rgb_data.sensor_data[0], value, AMS_REPORT_DATA_LEN * sizeof(int));
	...
	uint8_t *p_smux)
{
	...
	msg_num = SMUX_WITHOUT_RAM_MSG_SIZE;

	ret = i2c_transfer(ctx->handle->adapter, msgs, msg_num);
	if (ret != msg_num) {
		if (ret < 0)
			hwlog_err("%s: 1st Multi I2C Xfer failed with %d", __func__, -ret);
		else
			hwlog_err("%s: 1st Multi I2C Xfer only %d of %u mesgs",
				__func__, ret, msg_num);
		return;
	}

	ret = i2c_transfer(ctx->handle->adapter, (msgs + SMUX_WITHOUT_RAM_MSG_SIZE), msg_num);
	if (ret != msg_num) {
		if (ret < 0)
			hwlog_err("%s: 2nd Multi I2C Xfer failed with %d", __func__, -ret);
		else
			hwlog_err("%s: 2nd Multi I2C Xfer only %d of %u mesgs",
				__func__, ret, msg_num);
		return;
	}
	...
	bool cal_mode)
{
	...
	color_sensor_get_byte(ctx->handle, AS7341_ENABLE_REG, &en_status);
	if ((en_status & PON) != PON) {
		hwlog_err("%s enable reg modified to %u unanticipatedly\n",
			__func__, en_status);
...
void as7341_als_timer_wrk(struct timer_list * data)
{
...
void as7341_flc_timer_wrk(struct timer_list * data)
{
		...
		for (i = 0; i < AMS_REPORT_DATA_LEN; i++)
			if (sensor_type == AMS_AS7351_REV0)
				color_report_val[i + 1] = raw_data[i] *
					AMS_AS7351_GAIN_OF_GOLDEN / curr_gain;
			else
				color_report_val[i + 1] = raw_data[i] *
...
static void read_fd_data(struct color_chip *chip)
{
	...
	if (fifo_level > 0 && !error_detected) {
		tmp_level = fifo_level;
		start_level = 0;
		do {
			if (tmp_level > 0 && tmp_level <= MAX_READ_LEN_ONE_TIME)
				color_sensor_read_fifo(ctx->handle, AS7341_FDADAL,
					&buf_16_bit[start_level],
					tmp_level * FIFO_LEVEL_RATIO);
			else
				color_sensor_read_fifo(ctx->handle, AS7341_FDADAL,
					&buf_16_bit[start_level],
					MAX_READ_LEN_ONE_TIME * FIFO_LEVEL_RATIO);
			tmp_level = tmp_level - MAX_READ_LEN_ONE_TIME;
			start_level = start_level + MAX_READ_LEN_ONE_TIME;
		} while(tmp_level > 0);
...
struct i2c_driver ams_as7341_driver = {
	.driver = {
		.name = "ams_as7341",
		.owner = THIS_MODULE,
...
EXPORT_SYMBOL(ams_as7341_driver);
