...
static unsigned int vote_val = 0;
#ifdef CONFIG_HUAWEI_DSM
...
EXPORT_SYMBOL(color_sensor_get_byte);
...
EXPORT_SYMBOL(color_sensor_set_byte);
...
EXPORT_SYMBOL(color_sensor_read_fifo);
...
EXPORT_SYMBOL(color_sensor_write_fifo);
...
static ssize_t fwk_color_enable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct color_chip *chip = NULL;
	long value = 0;
	...
	if (kstrtol(buf, ONE_SHOW_LEN, &value))
		return -EINVAL;
	// even nmb is false, odd nmb is ture
	tmp_cmd = (unsigned int)value;
	hwlog_info("recieve ctrl cmd = %u\n", tmp_cmd);
	remainder = tmp_cmd % EVEN_OR_ODD;
	quotient = tmp_cmd / EVEN_OR_ODD;
	if (remainder != 0) {
		vote_val |= 1 << quotient;
		hwlog_info("recieve enable cmd, vote_val = %u\n", vote_val);
		color_sensor_enable(true);
	} else {
		vote_val &= ~(1 << quotient);
		hwlog_info("recieve disable cmd, vote_val = %u\n", vote_val);
		if (vote_val == 0) {
			hwlog_info("vote to close rgb\n");
			color_sensor_enable(false);
		}
	}
	return size;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtol(buf, ONE_SHOW_LEN, &state)) {
		hwlog_err("[%s] Failed to strtobool enable state.\n", __func__);
		return -EINVAL;
	}
...
DEVICE_ATTR(fwk_color_enable, 0660, NULL, fwk_color_enable_store);
...
static struct attribute *color_sensor_attributes[] = {
	...
	&dev_attr_fwk_color_enable.attr,
	NULL,
};
...
int color_vdd_supply(struct device *dev, struct regulator *verg)
{
	int rc;
	int rgltr_max_volt = 0;
	int rgltr_min_volt = 0;
	int rgltr_op_mode = 0;

	if (!verg || !dev) {
		hwlog_err("%s: para is null", __func__);
		return -1;
	}

	rc = of_property_read_u32_array(dev->of_node, "rgltr-min-voltage",
		&rgltr_min_volt, 1);
	if (!rc)
		rc = of_property_read_u32_array(dev->of_node, "rgltr-max-voltage",
			&rgltr_max_volt, 1);
	if (!rc) {
		rc = regulator_set_voltage(verg, rgltr_min_volt, rgltr_max_volt);
		if (rc)
			hwlog_err("%s: regulator_set_voltage failed!", __func__);
		hwlog_info("%s: min-voltage = %d, max-voltage = %d", __func__,
			rgltr_min_volt, rgltr_max_volt);
	}

	rc = of_property_read_u32_array(dev->of_node, "rgltr-load-current",
		&rgltr_op_mode, 1);
	if (!rc) {
		rc = regulator_set_load(verg, rgltr_op_mode);
		if (rc)
			hwlog_err("%s: regulator_set_load failed\n", __func__);
		hwlog_info("%s: load-current = %d", __func__, rgltr_op_mode);
	}

	rc = regulator_enable(verg);
	if (rc) {
		hwlog_err("%s: regulator_enable failed\n", __func__);
		return -1;
	}
	return 0;
}

void color_get_regulator(struct device *dev)
{
	struct regulator *verg = NULL;

	verg = devm_regulator_get(dev, "vdd_ans");
	if (!IS_ERR(verg)) {
		hwlog_info("%s: devm_regulator_get succ!\n", __func__);
		if (color_vdd_supply(dev, verg) < 0)
			hwlog_err("%s : color_vdd_supply failed", __func__);
		usleep_range(20000, 40000);
	}
}
EXPORT_SYMBOL_GPL(color_get_regulator);
...
EXPORT_SYMBOL(color_notify_absent);
...
EXPORT_SYMBOL(color_notify_support);
...
EXPORT_SYMBOL_GPL(color_sensor_init);

void color_sensor_exit(void)
{
	class_destroy(color_sensor_class);
}
EXPORT_SYMBOL_GPL(color_sensor_exit);
