 ...
 * Description: huawei multile hall sensor's module driver
 * Author: swx831057
 * Create: 2020-12-22
 */
...
static bool ak8789_hinge_get_ext_hall_status(void)
{
	unsigned int value;

	value = query_hall_event();
	ext_hall_notify_event(hw_hall_dev.ext_hall_type, (int)value);
	return !!get_ext_hall_status(EXT_HALL_FOLD_TYPE);
}

static struct hall_interface_ops ak8789_ops = {
	.get_hall_status = ak8789_lightstrap_get_hall_status,
	.get_ext_fold_status = ak8789_hinge_get_ext_hall_status,
};
#endif
...
irqreturn_t hall_event_isr(int irq, void *dev)
{
	...
	if (trigger & IRQF_TRIGGER_FALLING) {
...
static int get_gpio_config(struct device *dev,
	...
	char *gpio_name)
{
	...
	ret = devm_gpio_request_one(dev, gpio, GPIOF_DIR_IN, gpio_name);
	if (ret) {
...
static int hall_parse_dts(struct device *dev)
{
	...
	for_each_child_of_node(dev->of_node, temp) {
		...
		if (of_property_read_u32(temp, HALL_IRQ_TYPE, &irq_type))
			AK8789_ERRMSG("read huawei_irq_flags fail\n");

		if (hall_type & SINGLE_N_POLE) {
			gpio_ptr = devm_kzalloc(dev, sizeof(*gpio_ptr), GFP_KERNEL);
			ret = get_gpio_config(dev, temp, gpio_ptr, HALL_SINGLE_N_POLE);
			if (ret)
				return ret;
			...
			snprintf(gpio_ptr->name, MAX_NAME_LEN, "hall%u_north", hall_id);
			list_add_tail(&gpio_ptr->list, &hw_hall_dev.head);
		}

		if (hall_type & SINGLE_S_POLE) {
			gpio_ptr = devm_kzalloc(dev, sizeof(*gpio_ptr), GFP_KERNEL);
			ret = get_gpio_config(dev, temp, gpio_ptr, HALL_SINGLE_S_POLE);
			if (ret)
				return ret;
			...
			gpio_ptr->irq_type = irq_type;
			snprintf(gpio_ptr->name, MAX_NAME_LEN, "hall%u_south", hall_id);
			list_add_tail(&gpio_ptr->list, &hw_hall_dev.head);
		}
	}
