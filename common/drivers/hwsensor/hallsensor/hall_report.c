 ...
 * Description: huawei hall sensor's report interface
 * Author: swx831057
 * Create: 2020-12-30
 */
...
EXPORT_SYMBOL(get_hall_fature_config);
...
EXPORT_SYMBOL_GPL(get_hall_lightstrap_value);
...
EXPORT_SYMBOL(hall_report_value);
...
EXPORT_SYMBOL(get_ext_hall_status);

void check_ext_hall_notify_state(int ext_hall_type,
	unsigned int value)
{
...
EXPORT_SYMBOL(ext_hall_notify_event);
...
void ext_hall_report_value(int ext_hall_type, int value)
{
	...
	if (((unsigned int)ext_hall_type & BIT1_FOR_FOLD_TYPE) > 0) {
		...
		log_info("report ext hall bit1 state = %d\n",
			hall_data.sensor_data[1]);
#ifdef CONFIG_HUAWEI_DUBAI
		HWDUBAI_LOGE("DUBAI_TAG_FOLD_STATE", "state=%d", hall_data.sensor_data[1]);
#endif
	}
	if (((unsigned int)ext_hall_type & BIT2_FOR_NFC_THEME_SHELL) > 0) {
...
EXPORT_SYMBOL(ext_hall_report_value);
