...
#define ALS_DBV_SIZE 39
#define ALS_FLAGS_SIZE 3
...
struct als_under_tp_calidata {
	...
	uint32_t width;
	uint32_t length;
	struct als_under_tp_nv_type als_nv_data;
};

struct als_buf_to_hal_payload {
	uint32_t als;
...
typedef struct als_buf_to_hal {
	...
	uint8_t reserved;
	union {
		struct als_buf_to_hal_payload payload;
		int32_t data[3];
	};
} als_buf_to_hal_t;
#pragma pack()
...
int als_underscreen_calidata_save(als_sensor_t als_sensor_id);
