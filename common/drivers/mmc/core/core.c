...
static void (*p_sdsim_detect)(struct mmc_host *host);
int sdsim_detect_register(void(*p)(struct mmc_host *host))
{
	p_sdsim_detect = p;
	return 0;
}
EXPORT_SYMBOL(sdsim_detect_register);
#endif
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
	pr_debug("sdsim:hostname=%s: %s: sd_sim_mux = %d\n",
		mmc_hostname(host), __func__, host->sd_sim_mux);
	if (mmc_card_is_removable(host) && (host->sd_sim_mux == 1)) {
		if (p_sdsim_detect) {
			p_sdsim_detect(host);
		} else {
			pr_err("sdsim detect point failed\n");
			goto out;
		}
	}
	if (host->sd_cmd_det_result == -1) {
		pr_debug("sdsim this is not a sdcard no need to try freq\n");
		goto out;
	}
#endif
	mmc_claim_host(host);
