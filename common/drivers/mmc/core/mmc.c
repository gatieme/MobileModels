...
static int(*p_sim_gpio_setup)(struct mmc_host *host);
int sim_gpio_setup_register(int(*p)(struct mmc_host *host))
{
	p_sim_gpio_setup = p;
	return 0;
}
EXPORT_SYMBOL(sim_gpio_setup_register);
#endif
...
static int _mmc_resume(struct mmc_host *host)
{
	...
	if (mmc_card_is_removable(host) && (host->sd_sim_mux == 1))
		p_sim_gpio_setup(host);
	if (host->card == NULL) {
		err = -ENOMEM;
		pr_err("sdsim host card is null, can not resume\n");
		goto out;
	}
#endif
	if (!mmc_card_suspended(host->card))
