/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2021-2028. All Rights Reserved.
 * Description: mmc for shared tray with nm & sim card.
...
#define SD_LVST_SWITCH_LOW 0x0
#define SD_LVST_SWITCH_HIGH 0x1
...
int sd_sim_set_pin(struct mmc_host *host, char *pinctrl_name)
{
	...
	pr_err("sdsim pinctrl select state: %s\n", pinctrl_name);
	return 0;
err_pinctrl_put:
...
int regulator_fast_mos_vreg(u8 type_vreg, u16 addr, u8 value)
{
	...
	if (type_vreg == REGULATOR_VREG_TYPE_WRITE) {
		buf_vreg[0] = value;
		err = huawei_pmic_reg_write(REGULATOR_PM8350C, addr, buf_vreg, 1);
		if (err) {
			pr_err("sdsim mos type:%d,addr:0x%x,write:0x%x,err:%d\n", type_vreg, addr, value, err);
		}
		return 0;
	} else if (type_vreg == REGULATOR_VREG_TYPE_READ) {
		err = huawei_pmic_reg_read(REGULATOR_PM8350C, addr, buf_vreg, 1);
		if (err) {
			pr_err("sdsim mos type:%d,addr:0x%x,read:0x%x,err:%d\n", type_vreg, addr, buf_vreg[0], err);
		}
		return buf_vreg[0];
	}
	return 0;
...
static int sdhci_sd_lvst_singal_switch(struct mmc_host *mmc, int switch_value)
{
    int ret;
    int gpio_num = mmc->sd_lvst_gpio_switch;
...
void sdsim_boost_5v_enable(struct mmc_host *host, int enable)
{
	/* boost 5v enable:0x1, disable:0x0 */
	if (host->sd_sim_boost_5v == 1) {
		if (boost_5v_enable(enable, BOOST_CTRL_SD))
			pr_err("sdsim boost_5v enable:%d failed\n", enable);
		else
			pr_err("sdsim boost_5v enable:%d success\n", enable);
	}
}
...
int sim_nmc_cmd_handler(int cmd, int param)
{
	...
	case 1:
		if (param == 0) {
			...
			sdsim_boost_5v_enable(mmc_host_det, 0x0); /* boost 5v disable */
			/* L6c power off */
...
int sdsim_send_cmd_detect(struct mmc_host *host, int cd_state)
{
	...
	msleep(10); /* delay 10ms */
	/* sd level_shifter gpio switch ,0x0:3V io,0x1:1.8V io	*/
    if (g_sd_level_shifter) {
        ret = sdhci_sd_lvst_singal_switch(mmc_detect, SD_LVST_SWITCH_HIGH);
        if (ret) {
            pr_err("sdsim switch to high level-(1.8V) fail,ret:%d\n", ret);
            return ret;
        }
        msleep(10);
    }
    ...
    pr_err("sdsim L5c 1.8V power on\n");
	msleep(10); /* delay 10ms */
...
int sdsim_detect_result_handle(struct mmc_host *host)
{
	...
	if (host->sd_card_inserted) {
		...
		/* L9c fast mos off */
		regulator_fast_mos_vreg(REGULATOR_VREG_TYPE_WRITE, REGULATOR_VREG_ADDR_L9C, 0x0);
		ret = regulator_fast_mos_vreg(REGULATOR_VREG_TYPE_READ, REGULATOR_VREG_ADDR_L9C, 0x0);
		if (ret != 0x0) {
			pr_err("sdsim vdd(L9c) sd mos off fail,ret:0x%x\n", ret);
		} else {
			pr_err("sdsim vdd(L9c) sd mos off success,ret:0x%x\n", ret);
		}
		/* L5c fast mos on */
		regulator_fast_mos_vreg(REGULATOR_VREG_TYPE_WRITE, REGULATOR_VREG_ADDR_L5C, 0x80);
		msleep(5); /* delay 5ms */
		...
		/* sd io config to sd_off - input & no pull/lvshift：output-low pull down */
...
void sdsim_detect(struct mmc_host *host)
{
	...
	pr_err("sdsim rescan init g_sd_sim_detect_status_current %d",
		g_sd_sim_detect_status_current);
    if (g_sd_level_shifter) {
        /* adjust vddio high level to 3v */
        sdhci_msm_adjust_vddio_level(host, VOL_HIGH_LEVEL);
        pr_err("sdsim L6C adjust vddio high level to 3v\n");
    } else {
        /* adjust vddio high level to 1.8v */
        sdhci_msm_adjust_vddio_level(host, VOL_LOW_LEVEL);
        pr_err("sdsim L6C adjust vddio high level to 1.8v\n");
    }
	...
	if (cd_detect == 1)
		sdsim_boost_5v_enable(host, 0x1); /* boost 5v enable */
	else
		sdsim_boost_5v_enable(host, 0x0); /* boost 5v disable */
