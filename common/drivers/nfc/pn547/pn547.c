...
#define I2C_ELAPSE_TIMEOUT (1300 * 1000 * 1000L)
...
static ktime_t g_pre_nxpwrite_time;
#ifdef CONFIG_HUAWEI_DSM
		...
		size_t count, loff_t *offset)
{
	...
	for (retry = 0; retry < NFC_TRY_NUM; retry++) {
		...
		if ((count > MIN_NCI_CMD_LEN_WITH_DOOR_NUM) &&
			(tmp[NCI_CMD_HEAD_OFFSET] == 0x10) && get_is_uid()) {
	...
	g_pre_nxpwrite_time = ktime_get_boottime();
	/* copy data to user */
		...
		size_t count, loff_t *offset)
{
	...
	elapse_time = write_time - g_pre_nxpwrite_time;
	// make sure elapse_time is not overflow
	if (elapse_time < 0) {
		elapse_time = I2C_ELAPSE_TIMEOUT;
	}
	g_pre_nxpwrite_time = write_time;
	pr_info("%s : elapse_time %d\n", __func__, elapse_time);
	...
	if (!gpio_get_value(pdev->irq_gpio)) {
		...
		for (retry = 0; retry < NFC_TRY_NUM; retry++) {
			...
			ret = wait_event_interruptible_timeout(pdev->read_wq,
				!pdev->irq_enabled, (unsigned long)msecs_to_jiffies(1000));
			pn547_disable_irq(pdev);
 ...
 *		<0	:error
 */
static ssize_t pn547_nfc_sim_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int status=-1;
	int retry = 0;
 ...
 *  ssize_t:result
 */
static ssize_t pn547_nfc_sim_switch_store(struct device *dev, struct device_attribute *attr,
 ...
 *  ssize_t:  result
 */
static ssize_t pn547_nfc_sim_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
	struct pn547_dev *pn547_dev;
	pn547_dev = i2c_get_clientdata(i2c_client_dev);
	...
	return (ssize_t)(snprintf(buf,  MAX_ATTRIBUTE_BUFFER_SIZE-1, "%u\n", pn547_dev->sim_switch));
}
 ...
 *		<0	:error
 */
static ssize_t pn547_rd_nfc_sim_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int status=-1;
	struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
	struct pn547_dev *pn547_dev;
 ...
 *  ssize_t:  result
 */
static ssize_t pn547_nfc_enable_status_store(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
	struct pn547_dev *pn547_dev;
 ...
 *  ssize_t:  result
 */
static ssize_t pn547_nfc_enable_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
	struct pn547_dev *pn547_dev;
	pn547_dev = i2c_get_clientdata(i2c_client_dev);
...
void nfc_write_conf_to_tee(void)
{
	...
	if (get_chip_value() == NFCTYPE_SN110) {
...
static ssize_t pn547_nfc_recovery_close_nfc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int status = -1;
	struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
...
static ssize_t pn547_nfc_recovery_close_nfc_store(struct device *dev, struct device_attribute *attr,
            const char *buf, size_t count)
{
	return (ssize_t)count;
}

static int pn547_get_common_pointer()
{
	set_nfc_sim_status_show_pointer(pn547_nfc_sim_status_show);
	set_nfc_sim_switch_show_pointer(pn547_nfc_sim_switch_show);
	set_nfc_sim_switch_store_pointer(pn547_nfc_sim_switch_store);
	set_rd_nfc_sim_status_show_pointer(pn547_rd_nfc_sim_status_show);
	set_nfc_enable_status_show_pointer(pn547_nfc_enable_status_show);
	set_nfc_enable_status_store_pointer(pn547_nfc_enable_status_store);
	set_nfc_recovery_close_nfc_show_pointer(pn547_nfc_recovery_close_nfc_show);
	set_nfc_recovery_close_nfc_store_pointer(pn547_nfc_recovery_close_nfc_store);
...
static int pn547_remove_common_pointer(void)
{
...
{
    if (pn547_get_common_pointer() != 0) {
        pr_err("%s:pn547_get_common_pointer fail\n", __func__);
        return -1;
    }
    if (create_common_sysfs_interfaces(dev) != 0) {
        pr_err("%s:pn547_create_common_sysfs_interfaces fail\n", __func__);
        return -1;
    }
    return 0;
}
...
static int remove_sysfs_interfaces(struct device *dev)
{
    ...
    if (pn547_remove_common_pointer() != 0) {
        pr_err("%s:pn547_remove_common_pointer fail\n", __func__);
        return -1;
    }
    return 0;
}
...
static int pn80t_ese_pwr(struct pn547_dev *pn547_dev, unsigned long arg)
{
	...
	if (!gpio_is_valid(get_ese_svdd_pwr_req())) {
	...
	if (arg == 0) {
		 ...
		 */
		if (gpio_get_value(get_ese_svdd_pwr_req())) {
			...
			gpio_set_value(get_ese_svdd_pwr_req(), 1);
			usleep_range(1000, 1100);
			if (gpio_get_value(get_ese_svdd_pwr_req())) {
		...
		*/
		gpio_set_value(get_ese_svdd_pwr_req(), 0);
		usleep_range(1000, 1100);
		...
		r = gpio_get_value(get_ese_svdd_pwr_req());
	}
	return r;
}

static void pn547_dealwith_setpwr_case(struct pn547_dev *pn547_dev, unsigned long arg)
{
	switch (arg) {
	case MODE_POWER_OFF:
		/* power off */
	...
	case MODE_POWER_ON:
		...
		pr_err("%s firm_gpio %d, ven_gpio %d, power on\n", __func__,
			pn547_dev->firm_gpio, pn547_dev->ven_gpio);
		gpio_set_value(pn547_dev->firm_gpio, 0);
		gpio_set_value(pn547_dev->ven_gpio, 1);
	...
	case MODE_ISO_RST:
		...
		wake_up(&pn547_dev->read_wq);
		break;
	case MODE_FW_DWND_HIGH: // 4 means sn110 download start
		pr_err("%s FW dwld gpio %d ioctl called from NFC \n", __func__, pn547_dev->firm_gpio);
		gpio_set_value(pn547_dev->firm_gpio, 1);
		msleep(10);
		break;
	case MODE_POWER_RESET:
		...
		break;
	case MODE_FW_GPIO_LOW: // 6 means sn110 download complete
		pr_err("%s FW GPIO %d set to 0x00 >>>>>>>\n", __func__, pn547_dev->firm_gpio);
		gpio_set_value(pn547_dev->firm_gpio, 0);
		msleep(5);
		break;
	case MODE_RF_GPIO_HIGH:
		pr_err("%s RF GPIO set rf_gpio %d to 01 >>>>>>>\n", __func__, pn547_dev->rf_gpio);
		if (pn547_dev->rf_gpio > 0) {
			gpio_set_value(pn547_dev->rf_gpio, 1);
		}
		msleep(5);
		break;
	case MODE_RF_GPIO_LOW:
		...
		if (pn547_dev->rf_gpio > 0) {
			gpio_set_value(pn547_dev->rf_gpio, 0);
		}
		msleep(5);
		break;
	default:
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case PN544_SET_PWR:
		pn547_dealwith_setpwr_case(pn547_dev, arg);
		break;
	case ESE_SET_PWR:
		if (get_chip_value() == NFCTYPE_SN110) {
	...
	case ESE_GET_PWR:
		if (get_chip_value() == NFCTYPE_SN110) {
			r = sn110_ese_pwr(pn547_dev, ESE_POWER_STATE);
		} else {
			r = pn80t_ese_pwr(pn547_dev, ESE_POWER_STATE);
		}
		break;
	case PN544_GET_CHIP_TYPE:
		r = get_chip_value();
		break;
	default:
			 ...
			 struct pn547_i2c_platform_data *pdata)
{
	...
	/* nfc_rf gpio */
	pdata->rf_gpio = of_get_named_gpio_flags(np, "nxp,nfc_palau_rf", 0, NULL);
				...
				struct pn547_i2c_platform_data *pdata)
{
	...
	if (pdata->rf_gpio > 0) {
		...
		pr_info("[%s,%d]:pn547 probe rf_gpio %d request ret %d\n", __func__, __LINE__, pdata->rf_gpio, ret);
		if (ret == 0) {
			ret = gpio_direction_output(pdata->rf_gpio, 1);
			pr_info("[%s,%d]:pn547 probe rf_gpio gpio_direction_output ret %d\n", __func__, __LINE__, ret);
		}
	}
	return 0;
...
int nfc_i2c_dev_suspend(struct device *device)
{
	...
	pr_info("%s: irq_enabled = %d, gpio value = %d, irq = %d\n", __func__, pn547_dev->irq_enabled,
		gpio_get_value(pn547_dev->irq_gpio), pn547_dev->client->irq);
...
int nfc_i2c_dev_resume(struct device *device)
{
	...
	pr_info("%s: irq_wake_up = %d, gpio value = %d, irq = %d\n", __func__, pn547_dev->irq_wake_enabled,
		gpio_get_value(pn547_dev->irq_gpio), pn547_dev->client->irq);
		...
		const struct i2c_device_id *id)
{
	...
	pn547_dev->rf_gpio = platform_data->rf_gpio;
