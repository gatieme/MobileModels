...
static int ras_readline(struct file *filp, char *buf, int len)
{
	...
	ras_retn_if(filp == 0, -ENOENT);
	ras_retn_if(filp->f_op->read == 0, -ENOSYS);
...
int rasmap_create(struct RasMap **rm, rasmap_less_t *less, rasmap_free_t *destroy, void *data)
{
	struct RasMap *m = 0;

	ras_retn_if(rm == 0, -EINVAL);
...
int rasmap_add(struct RasMap *rm, void *key, void *val)
{
	...
	} else if (ras_malloc((void **)&rme, sizeof(struct RasMapElement)) != 0) {
		ras_retn(-ENOMEM);
	}
	rme->tm = 0;
	rme->key = key;
...
int rasmap_each(struct RasMap *rm, rasmap_each_t *each, void *data)
{
	...
	ras_retn_if((rm == 0 || each == 0), -EINVAL);
	for (rme = rm->nxt; rme != 0; rme = rme->nxt)
...
int rasmap_size(struct RasMap *rm)
{
	...
	ras_retn_if(rm == 0, cnt);
	for (rme = rm->nxt; rme != 0; rme = rme->nxt)
		cnt++;
	return cnt;
}

static int rasmap_eachrme(struct RasMap *rm, int (*each)(void *data, void *key, void *val), void *data)
{
	struct RasMapElement *rme = rm->rme;

	for (; rme != 0; rme = rme->rme)
		ras_retn_iferr(each(data, rme->key, rme->val));
	return 0;
}
...
void *rasmap_get(struct RasMap *rm, void *key)
{
	...
	return (rme != 0 && rme->tm == 0) ? rme->val : 0;
}
...
void rasmap_clear(struct RasMap *rm)
{
	...
	/* clear firstly */
	for (rm->nxt = 0; rme != 0; rme = rme->nxt)
...
static inline int rasvector_grow(struct RasVector *vec, struct RasBff *trb, int length)
{
...
static inline int rasvector_copy(struct RasVector *vec, struct RasBff *rb, const char *str)
{
	int i = 0;
	int len = strlen(str);
...
static int rasargs_ctor(struct RasArgs *rargs, const char *args, int len)
{
	...
	ras_retn_iferr_then(ras_malloc((void **)&rargs->args,
			    ...
			    ras_free(rargs->arg));
	for (i = 0, j = 0; j < rargs->argc && i < rargs->len; j++)
		rargs->args[j] = rasargs_nextword(rargs, &i);
	if (rargs->args[rargs->argc - 1] == 0)
		rargs->argc--;
	return 0;
}
	...
	for (; pos < len; pos++) {
		...
		if (ras_isxdigit(c)) {
			...
			if (cv != 0) {
				...
				c = a[pos + 1];
				if (c == 0 || ras_isspace(c))
					break;
				ras_retn(ras_invalidparam(a, pos + 1));
			} else if (c == 0 || ras_isspace(c)) {
				break;
			} else {
...
const char *ras_skipword(const char *bff, unsigned long len)
{
	...
	for (idx = 0; idx < len; idx++) {
		if (bff[idx] == 0 || ras_isspace(bff[idx]))
			break;
	}
	if (idx < len)
		return bff + idx;
...
int ras_check(void)
{
	...
	int result  = call_usermodehelper(path, NULL, NULL, UMH_WAIT_PROC);
	if (result)
		return 0;
	return 0;
}
