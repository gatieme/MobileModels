...
static inline int rasmemele_touch(struct RasMemEle *ele, unsigned long block,
				  void (*mmupdate)(struct mm_struct *,
						    struct RasMemEle *, long))
{
	struct mm_struct *mm;
	struct task_struct *tsk;
...
static int rasmemele_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ele->len > len
		    && 0 == rasmemele_touch(ele, ele->len - len,
					    rasmemele_addmm))
			ele->tch += (ele->len - len);

		if (ele->len >= ele->args.total) {
			if (ele->args.fragment)
				fragment_create(ele);
			break;
		}
		if (ele->args.gap)
			...
			ras_sleep(RASTICK); /* malloc 0, delay and retry */
	}
	write_lock(&ele->rwk);
	ele->tsk = 0;
	write_unlock(&ele->rwk);
...
static int rasmem_main(void *mem, int argc, char *args[])
{
	...
	ras_retn_if(rma.total == 0, -EINVAL);
	if (rma.block == 0 || rma.block > rma.total)
		rma.block = rma.total;
...
static ssize_t proc_ops_write(rmem) (struct file *filp,
	...
	ras_retn_iferr(ras_args
			(buf_cmd, count, rasmem_main,
