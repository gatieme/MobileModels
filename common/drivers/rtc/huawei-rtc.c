...
static long write_data_to_file(struct file *file, char *buf, size_t buf_size, loff_t *pos)
{
	mm_segment_t old_fs;
	...
	if (NULL == buf)
	{
		pr_err("func [%s] invalid parameter!\n", __func__);
		return -1;
	}
	...
	{
		bytes_this_time = kernel_write(file, ptemp, bytes_total_to_rw, pos);
	...
	uint64_t buf_size)
{
	...
	file = filp_open(dev_path, O_WRONLY, 0);
 	if (IS_ERR(file))
 	{
		pr_err("func [%s] Open file [%s] failed!\n", __func__, dev_path);
		goto __out;
	}
	/* 2. write data to file in offset pos */
        bytes_written = write_data_to_file(file, buf, buf_size, &pos);
...
write_offset_to_raw_part(struct work_struct *work){
	int ret = -1;
        ...
        while (retry_times) {
            file = filp_open(BSPINFO_RAW_PART_PATH, O_RDONLY, 0); 
                if (IS_ERR(file)) {
                        pr_err("func [%s] Open [%s] failed! retry_times: %d, errno = %ld\n",
                                __func__, BSPINFO_RAW_PART_PATH, retry_times, PTR_ERR(file));
                } else {
                        filp_close(file, NULL);
                        break;
                }   
...
huawei_rtc_read_time(struct device *dev, struct rtc_time *tm){
	int ret = -1;
	time64_t qpnp_rtc_secs, huawei_rtc_secs = 0;
	...
	qpnp_rtc_secs = rtc_tm_to_time64(&qpnp_rtc_time);

	huawei_rtc_secs = qpnp_rtc_secs + offset_info.offset;
	rtc_time64_to_tm(huawei_rtc_secs, tm);
...
int huawei_rtc_init(struct device *dev){
	...
	memcpy_s(&offset_info, sizeof(Offset_Info), smem->rtc_offset_info, sizeof(Offset_Info));
	...
	if(strcmp(OFFSET_TAG, offset_info.off_tag)){
		...
		memset_s(&offset_info, sizeof(Offset_Info), 0, sizeof(Offset_Info));
		strncpy_s(offset_info.off_tag, sizeof(offset_info.off_tag), OFFSET_TAG, strlen(OFFSET_TAG));
