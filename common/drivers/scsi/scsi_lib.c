...
static void scsi_mq_requeue_cmd(struct scsi_cmnd *cmd)
{
	...
	mas_blk_latency_req_check_ufs(cmd->request, REQ_PROC_STAGE_SCSI_FINISH_REQUEUE);
#endif
...
#ifdef CONFIG_MAS_BLK
	if (cmd->request)
		mas_blk_latency_req_check_ufs(cmd->request, REQ_PROC_STAGE_SCSI_INSERT);
#endif
...
static unsigned int scsi_mq_inline_sgl_size(struct Scsi_Host *shost)
{
	...
	if (shost->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_MAS_UFS_MQ))
			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	if (shost->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_MAS_UFS_MQ)) {
		if (!scsi_mq_get_budget(q)) {
			mas_blk_latency_req_check_ufs(req, REQ_PROC_STAGE_SCSI_QUEUE_OUT);
			goto out;
		}
	}
#endif
	if (unlikely(sdev->sdev_state != SDEV_RUNNING)) {
		...
		if (ret != BLK_STS_OK) {
#ifdef CONFIG_MAS_BLK
			mas_blk_latency_req_check_ufs(req, REQ_PROC_STAGE_SCSI_DEVICE_STATE);
#endif
			goto out_put_budget;
		}
	}
...
#ifdef CONFIG_MAS_BLK
		mas_blk_latency_req_check_ufs(req, REQ_PROC_STAGE_SCSI_QUEUE_TARGET);
#endif
		goto out_put_budget;
	}
	if (!scsi_host_queue_ready(q, shost, sdev, cmd)) {
#ifdef CONFIG_MAS_BLK
		mas_blk_latency_req_check_ufs(req, REQ_PROC_STAGE_SCSI_QUEUE_HOST);
#endif
	...
	if (!(req->rq_flags & RQF_DONTPREP)) {
		...
		if (ret != BLK_STS_OK) {
#ifdef CONFIG_MAS_BLK
			mas_blk_latency_req_check_ufs(req, REQ_PROC_STAGE_SCSI_PREPARE_CMD);
#endif
...
int scsi_mq_setup_tags(struct Scsi_Host *shost)
{
...
#ifdef CONFIG_MAS_BLK
	if (shost->queue_quirk_flag & SHOST_QUIRK_MAS_UFS_MQ) {
...
#ifdef CONFIG_HYPERHOLD_CORE
	blk_mq_tagset_health_query_register(&shost->tag_set,
	...
	blk_mq_tagset_ufs_mq_iosched_enable(&shost->tag_set,
			shost->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_MAS_UFS_MQ));
#endif /* CONFIG_MAS_BLK */
