...
static int dsm_ufs_update_fastboot_info(struct ufs_hba *hba)
{
	...
	if (sscanf(pstr, "fastbootdmd=%d", &err_code) != 1) {
...
static void dsm_ufs_fastboot_pwmode_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_FASTBOOT_PWMODE_ERR))) {
		pr_err("UFS DSM Error! Err Num:%lu err_type:%lu\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
	ret = snprintf(dsm_log_buff, *buff_size, "Fastboot Power Mode Change Err\n");
	dsm_log_buff += ret;
	*buff_size -= ret;
}

static void dsm_ufs_fastboot_rw_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_FASTBOOT_RW_ERR))) {
		pr_err("UFS DSM Error! Err Num:%lu err_type:%lu\n", code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
static void dsm_ufs_uic_trans_err(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & ((NUM_1 << UFS_UIC_TRANS_ERR) |
		(NUM_1 << UFS_LINE_RESET_ERR) | (NUM_1 << UFS_PA_INIT_ERR)))) {
		pr_err("UFS DSM Error! Err Num: %lu, %lu\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
static void dsm_ufs_uic_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_UIC_CMD_ERR))) {
		pr_err("UFS DSM Error! Err Num: %lu, %ld\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
	ret = snprintf(dsm_log_buff, *buff_size, "UIC_CMD:%08x,ARG1:%08x,ARG2:%08x,ARG3:%08x\n",
			g_ufs_dsm_adaptor.uic_info[NUM_0], g_ufs_dsm_adaptor.uic_info[NUM_1],
			g_ufs_dsm_adaptor.uic_info[NUM_2], g_ufs_dsm_adaptor.uic_info[NUM_3]);
	dsm_log_buff += ret;
	*buff_size -= ret;
}

static void dsm_ufs_sys_err_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	int i;
	if (!(g_ufs_dsm_adaptor.err_type & ((NUM_1 << UFS_SYSBUS_ERR) |
		(NUM_1 << UFS_CONTROLLER_ERR) | (NUM_1 << UFS_INLINE_CRYPTO_ERR) |
		(NUM_1 << UFS_DEV_ERR) | (NUM_1 << UFS_ENTER_OR_EXIT_H8_ERR)))) {
		pr_err("UFS DSM Error! Err Num: %lu, %ld\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
	if (g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_INLINE_CRYPTO_ERR)) {
	...
	if ((g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_CONTROLLER_ERR))) {
...
static void dsm_ufs_utp_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_UTP_TRANS_ERR))) {
		pr_err("UFS DSM Error! Err Num: %lu, %lu\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
static void dsm_ufs_timeout_cmd(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	int i;
	if (!(g_ufs_dsm_adaptor.err_type & ((NUM_1 << UFS_TIMEOUT_ERR) |
			(NUM_1 << UFS_TIMEOUT_SERIOUS)))) {
		pr_err("UFS DSM Error! Err Num: %lu, %ld\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
	...
	dsm_log_buff += ret;
	*buff_size -= ret;
	for (i = 0; i < COMMAND_SIZE(g_ufs_dsm_adaptor.timeout_scmd[NUM_0]); i++) {
		ret = snprintf(dsm_log_buff, *buff_size, "%02x", g_ufs_dsm_adaptor.timeout_scmd[i]);
		dsm_log_buff += ret;
		*buff_size -= ret;
	}
}

static void dsm_ufs_hardware_and_dev_err(unsigned long code, char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	int i;
	if (!(g_ufs_dsm_adaptor.err_type & ((NUM_1 << UFS_HARDWARE_ERR) |
			(NUM_1 << UFS_DEV_INTERNEL_ERR)))) {
		pr_err("UFS DSM Error! Err Num: %lu, %ld\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
	...
	*buff_size -= ret;
	...
	ret = snprintf(dsm_log_buff, *buff_size,
			...
			g_ufs_dsm_adaptor.sense_buffer[NUM_0], g_ufs_dsm_adaptor.sense_buffer[NUM_2],
			g_ufs_dsm_adaptor.sense_buffer[NUM_12], g_ufs_dsm_adaptor.sense_buffer[NUM_13]);
	dsm_log_buff += ret;
	*buff_size -= ret;
}

static void dsm_ufs_linkup_cmd(struct ufs_hba *hba, unsigned long code,
			char *dsm_log_buff, unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_LINKUP_ERR))) {
		pr_err("UFS DSM Error! Err Num: %lu, %lu\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
static void dsm_ufs_lifetime_excced_cmd(unsigned long code, char *dsm_log_buff,
			unsigned int *buff_size)
{
	int ret;
	if (!(g_ufs_dsm_adaptor.err_type & (NUM_1 << UFS_LIFETIME_EXCCED_ERR))) {
		pr_err("UFS DSM Error! Err Num: %lu, %lu\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
static int set_dsm_ufs_log_buf(struct ufs_hba *hba, unsigned long code,
			char *dsm_log_buff, unsigned int *buff_size)
{
	switch (code) {
	case DSM_UFS_FASTBOOT_PWMODE_ERR:
		dsm_ufs_fastboot_pwmode_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_FASTBOOT_RW_ERR:
		dsm_ufs_fastboot_rw_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_UIC_TRANS_ERR:
	case DSM_UFS_LINE_RESET_ERR:
	case DSM_UFS_PA_INIT_ERR:
		dsm_ufs_uic_trans_err(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_UIC_CMD_ERR:
		dsm_ufs_uic_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_SYSBUS_ERR:
	case DSM_UFS_CONTROLLER_ERR:
	case DSM_UFS_DEV_ERR:
	...
	case DSM_UFS_INLINE_CRYPTO_ERR:
		dsm_ufs_sys_err_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_UTP_ERR:
		dsm_ufs_utp_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_TIMEOUT_ERR:
		dsm_ufs_timeout_cmd(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_HARDWARE_ERR:
	case DSM_UFS_DEV_INTERNEL_ERR:
		dsm_ufs_hardware_and_dev_err(code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_LINKUP_ERR:
		dsm_ufs_linkup_cmd(hba, code, dsm_log_buff, buff_size);
		break;
	case DSM_UFS_LIFETIME_EXCCED_ERR:
		dsm_ufs_lifetime_excced_cmd(code, dsm_log_buff, buff_size);
		break;
...
static void set_dsm_log_common_info(struct ufs_hba *hba, char *dsm_log_buff,
			unsigned int *buff_size, char *product_name)
{
	int ret;
	int i;
	char rev[NUM_5] = {0};
	...
	*buff_size -= ret;
	...
	if ((hba->sdev_ufs_device != NULL)
		...
		snprintf(rev, NUM_5, "%.4s", hba->sdev_ufs_device->rev);
	...
	*buff_size -= ret;
	...
	for (i = 0; i < strlen(g_ufs_dsm_log.dsm_log); i++)
		g_ufs_dsm_log.dsm_log[i] -= NUM_1;
#endif
...
int dsm_ufs_get_log(struct ufs_hba *hba, unsigned long code, char *err_msg)
{
	...
	set_dsm_log_common_info(hba, dsm_log_buff, &buff_size, product_name);
	ret = snprintf(dsm_log_buff, buff_size, "\nError Num:%lu, %s\n", code, err_msg);
	dsm_log_buff += ret;
	buff_size -= ret;

	if (set_dsm_ufs_log_buf(hba, code, dsm_log_buff, &buff_size) == 0)
		return 0;

	ufs_dsm_real_upload_size = sizeof(g_ufs_dsm_log.dsm_log) - buff_size;
	if (buff_size <= 0) {
		ufs_dsm_real_upload_size = sizeof(g_ufs_dsm_log.dsm_log) - NUM_1;
...
/*lint +e648 +e845*/
static void dsm_ufs_handle_work_frist_stage(struct ufs_hba *hba)
{
...
static void dsm_ufs_handle_work_sec_stage(struct ufs_hba *hba)
{
	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_TIMEOUT_SERIOUS)) {
...
static void dsm_ufs_handle_work(struct work_struct *work)
{
	...
	dsm_ufs_handle_work_frist_stage(hba);
	...
	dsm_ufs_handle_work_sec_stage(hba);
