...
static u32 shift_byte(int num, int byte)
{
	return ((num) << (byte));
}

static u32 get_byte(int num, int byte)
{
			...
			u8 *cdb, int rgn_id, int srgn_id)
{
	...
	cdb[HPB_RB_REGION_ID_HIGH_OFFSET] = (unsigned char)get_byte(rgn_id, ONE_BYTE_SHIFT);
	cdb[HPB_RB_REGION_ID_LOW_OFFSET] = (unsigned char)get_byte(rgn_id, ZERO_BYTE_SHIFT);
	cdb[HPB_RB_SUBREGION_ID_HIGH_OFFSET] = (unsigned char)get_byte(srgn_id, ONE_BYTE_SHIFT);
	cdb[HPB_RB_SUBREGION_ID_LOW_OFFSET] = (unsigned char)get_byte(srgn_id, ZERO_BYTE_SHIFT);
	cdb[HPB_RB_AL_LEN3_OFFSET] =
		(unsigned char)get_byte(hba->ufs_hpb->host_table_size_per_srgn, TWO_BYTE_SHIFT);
	cdb[HPB_RB_AL_LEN2_OFFSET] =
		(unsigned char)get_byte(hba->ufs_hpb->host_table_size_per_srgn, ONE_BYTE_SHIFT);
	cdb[HPB_RB_AL_LEN1_OFFSET] =
		(unsigned char)get_byte(hba->ufs_hpb->host_table_size_per_srgn, ZERO_BYTE_SHIFT);
			...
			int srgn_id, char *srgn_hpb_buffer, int lun)
{
	...
	if (memset_s(srgn_hpb_buffer, hba->ufs_hpb->host_table_size_per_srgn, '\0',
		hba->ufs_hpb->host_table_size_per_srgn) !=  EOK)
		dev_err("[%s:%d] memset_s failed!\n", __func__, __line__);
			    ...
			    u64 ppn)
{
	...
	transfer_len = shift_byte(lrbp->cmd->cmnd[READ10_TRANS_LEN_HIGH_OFFSET], ONE_BYTE_SHIFT) |
	...
	cmd[HPB_READ16_PPN1_OFFSET] = get_byte(ppn, SEVEN_BYTE_SHIFT);
	cmd[HPB_READ16_PPN2_OFFSET] = get_byte(ppn, SIX_BYTE_SHIFT);
	cmd[HPB_READ16_PPN3_OFFSET] = get_byte(ppn, FIVE_BYTE_SHIFT);
	cmd[HPB_READ16_PPN4_OFFSET] = get_byte(ppn, FOUR_BYTE_SHIFT);
	cmd[HPB_READ16_PPN5_OFFSET] = get_byte(ppn, THREE_BYTE_SHIFT);
	cmd[HPB_READ16_PPN6_OFFSET] = get_byte(ppn, TWO_BYTE_SHIFT);
	cmd[HPB_READ16_PPN7_OFFSET] = get_byte(ppn, ONE_BYTE_SHIFT);
	cmd[HPB_READ16_PPN8_OFFSET] = get_byte(ppn, ZERO_BYTE_SHIFT);
	cmd[HPB_READ16_TRANS_LEN_OFFSET] = get_byte(transfer_len, ZERO_BYTE_SHIFT);
	...
	if (hba->manufacturer_id == UFS_VENDOR_SAMSUNG) {
		...
		cmd[HPB_READ16_CONTROL_OFFSET] = get_byte(transfer_len, ZERO_BYTE_SHIFT);
		lrbp->cmd->cmd_len = MAX_CDB_SIZE;
	}

	if (memcpy_s(lrbp->cmd->cmnd, MAX_CDB_SIZE, cmd, MAX_CDB_SIZE) != EOK)
		pr_info("[%s:%d] memcpy_s failed!\n", __func__, __line__);
	if (memcpy_s(lrbp->ucd_req_ptr->sc.cdb, MAX_CDB_SIZE, cmd, MAX_CDB_SIZE) != EOK)
		pr_info("[%s:%d] memcpy_s failed!\n", __func__, __line__);
}
					...
					char *buf)
{
	...
	if (hba->ufs_hpb->state == UFSHPB_STATE_OPERATIONAL) {
		...
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE,
			"%s\nhit/total=%lld/%lld(%d%%)\nread buffer count %d\n\
		...
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE, "%s\n", "ufshpb_turn_off");
	}
}
