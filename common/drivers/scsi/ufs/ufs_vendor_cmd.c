...
int ufshcd_compose_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	case UTP_CMD_TYPE_SCSI:
		if (likely(lrbp->cmd)) {
			ufshcd_prepare_req_desc_hdr(lrbp, (u8 *)&upiu_flags,
					lrbp->cmd->sc_data_direction);
			ufshcd_prepare_utp_scsi_cmd_upiu(lrbp, (u8)upiu_flags);
		} else {
			ret = -EINVAL;
		}
		break;
	case UTP_CMD_TYPE_DEV_MANAGE:
		ufshcd_prepare_req_desc_hdr(lrbp, (u8 *) &upiu_flags, DMA_NONE);
		if (hba->dev_cmd.type == DEV_CMD_TYPE_QUERY)
			ufshcd_prepare_utp_query_req_upiu(
					hba, lrbp, (u8)upiu_flags);
		...
		struct scsi_device *sdp, unsigned char* cdb, void* buf)
{
	...
	if (memset_s(&cmnd, sizeof(cmnd), 0, sizeof(cmnd)) != 0)
	...
	if (buf) {
		if (memcpy_s(buf, PAGE_SIZE, dma_buf, PAGE_SIZE) != 0)
...
void ufshcd_set_query_req_length(u8 opcode,
		u8 idn, u32 *data_seg_length, u16 len)
{
	if (opcode != UPIU_QUERY_OPCODE_VENDOR_WRITE)
		return;
	if (idn == VENDOR_IDN_TT)
		*data_seg_length = UPIU_HEADER_DWORD(0, 0, (len >> 8), (u8)len);
...
void ufshcd_set_query_req_desc(u8 opcode, u8 idn, struct utp_upiu_req *ptr, u8 *descriptor)
{
	if (opcode != UPIU_QUERY_OPCODE_VENDOR_WRITE)
		return;
	if (idn == VENDOR_IDN_TT) {
		if (memcpy_s(ptr, VENDOR_TT_MAX_LENGTH, descriptor, VENDOR_TT_MAX_LENGTH) != 0)
			pr_err("[%s] memcpy_s failed!\n", __func__);
	}
	...
	struct ufs_query_vcmd *cmd)
{
	...
	if (cmd->response) {
		if (memcpy_s(cmd->response, sizeof(struct ufs_query_res), response,
			sizeof(struct ufs_query_res)) != 0)
			pr_err("[%s] memcpy_s failed!\n", __func__);
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
EXPORT_SYMBOL(ufshcd_ioctl_query_vcmd);
#endif
				...
				u32 param_size)
{
	...
	if (!cmd.desc_buf)
		return -ENOMEM;
	if (memset_s(cmd.desc_buf, cmd.buf_len, 0, cmd.buf_len) != 0)
		pr_err("[%s] memset_s failed!\n", __func__);
	...
	if (memcpy_s(param_read_buf, cmd.buf_len, cmd.desc_buf, cmd.buf_len) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
out:
	kfree(cmd.desc_buf);
	return ret;
...
static void set_dieid_after_hi1861_ver(u8 *fbuf)
{
	if (memcpy_s(ufs_hixxxx_dieid, UFS_DIEID_NUM_SIZE_THOR920, fbuf + 16, UFS_DIEID_NUM_SIZE_THOR920) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_CHIP_VER_OFFSET, UFS_NAND_CHIP_VER_SIZE,
		fbuf + 36, UFS_NAND_CHIP_VER_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_CONTROLLER_OFFSET, UFS_CONTROLLER_DIEID_SIZE,
		fbuf + 256, UFS_CONTROLLER_DIEID_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_FLASH_OFFSET, UFS_FLASH_DIE_ID_SIZE,
		fbuf + 448, UFS_FLASH_DIE_ID_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DCIP_CRACK_NOW_OFFSET, UFS_DCIP_CRACK_NOW_SIZE,
		fbuf + 440, UFS_DCIP_CRACK_NOW_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DCIP_CRACK_EVER_OFFSET, UFS_DCIP_CRACK_EVER_SIZE,
		fbuf + 441, UFS_DCIP_CRACK_EVER_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
}

static void set_dieid_before_hi1861_ver(u8 *fbuf)
{
	if (memcpy_s(ufs_hixxxx_dieid, UFS_DIEID_NUM_SIZE, fbuf + 12, UFS_DIEID_NUM_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_CHIP_VER_OFFSET, UFS_NAND_CHIP_VER_SIZE,
		fbuf + 28, UFS_NAND_CHIP_VER_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_CONTROLLER_OFFSET, UFS_CONTROLLER_DIEID_SIZE,
		fbuf + 1692, UFS_CONTROLLER_DIEID_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
	if (memcpy_s(ufs_hixxxx_dieid + UFS_DIEID_FLASH_OFFSET, UFS_FLASH_DIE_ID_SIZE,
		fbuf + 1900, UFS_FLASH_DIE_ID_SIZE) != 0)
		pr_err("[%s] memcpy_s failed!\n", __func__);
}
...
static void ufshcd_ufs_set_dieid(struct ufs_hba *hba)
{
	...
	if (memset_s(ufs_hixxxx_dieid, UFS_DIEID_TOTAL_SIZE, 0, UFS_DIEID_TOTAL_SIZE) != 0)
		pr_err("[%s] memset_s failed!\n", __func__);
	...
	if (memset_s(fbuf, HI1861_FSR_INFO_SIZE, 0, HI1861_FSR_INFO_SIZE) != 0)
		pr_err("[%s] memset_s failed!\n", __func__);
	...
	ret = snprintf_s(ufs_product_name, UFS_PRODUCT_NAME_LEN + 1, UFS_PRODUCT_NAME_LEN + 1, hba->dev_info.model_name);
	if (ret <= 0) {
	...
	if (ret != 0) {
		/* after hi1861 ver. */
		set_dieid_after_hi1861_ver(fbuf);
	} else {
		/* hi1861 ver. */
		set_dieid_before_hi1861_ver(fbuf);
	}
out:
	kfree(fbuf);
}
	...
	if (((strncmp(ufs_product_name, UFS_PRODUCT_NAME_THOR920, UFS_PRODUCT_NAME_LEN) == 0) ||
		...
		(vendor_id == UFS_FLASH_VENDOR_M)) {
			for (i = 0; i < dieid_num; i++) {
				...
				ret = snprintf_s(dieid + len, UFS_DIEID_BUFFER_SIZE - len - offset,
						UFS_DIEID_BUFFER_SIZE - len - offset,
						"\r\nDIEID_UFS_FLASH_%c:0x%08X%08X%08X%08X00000000000000000000000000000000\r\n",
						'B' + j++, *(flash_id + i * 4), *(flash_id + i * 4 + 1),
						*(flash_id + i * 4 + 2), *(flash_id + i * 4 + 3));
				if (ret <= 0)
					return -2;
				len += ret;
		...
		for (i = 0; i < dieid_num; i++) {
			ret = snprintf_s(dieid + len, UFS_DIEID_BUFFER_SIZE - len - offset, UFS_DIEID_BUFFER_SIZE - len - offset,
					"\r\nDIEID_UFS_FLASH_%c:0x%08X%08X%08X%08X\r\n",
					'B' + i, *(flash_id + i * 4), *(flash_id + i * 4 + 1),
					*(flash_id + i * 4 + 2), *(flash_id + i * 4 + 3));
			if (ret <= 0)
				return -2;
			len += ret;
...
static int hufs_get_dieid(char *dieid, unsigned int len)
{
	...
	ret = snprintf_s(buf, UFS_DIEID_BUFFER_SIZE, UFS_DIEID_BUFFER_SIZE,
			"\r\nDIEID_UFS_CONTROLLER_A:0x%08X%08X%08X%08X%08X%08X%08X%08X\r\n",
			*controller_id, *(controller_id + 1), *(controller_id + 2), *(controller_id + 3),
			*(controller_id + 4), *(controller_id + 5), *(controller_id + 6), *(controller_id + 7));
	if (ret <= 0)
		return -2;
	length += ret;
	...
	ret = snprintf_s(buf + length, UFS_DIEID_BUFFER_SIZE - length, UFS_DIEID_BUFFER_SIZE - length,
		"\r\nCRACK_NOW:0x%08X\r\n\r\nCRACK_EVER:0x%08X\r\n", die_crack_now, die_crack_ever);
	if (ret <= 0)
		return -2;

	if (len >= strlen(buf))
		if (strncat_s(dieid, UFS_DIEID_BUFFER_SIZE, buf, strlen(buf)) != 0)
			pr_err("[%s] strncat_s failed!\n", __func__);
	else
		return strlen(buf);
...
void hufs_bootdevice_get_dieid(struct ufs_hba *hba)
{
	...
	int ret;
	if (strncmp(hba->dev_info.model_name, "THR93", UFS_PNM_CMP_LEN) == 0) {
		dev_err(hba->dev, "[%s]930 fw !!!\n", __func__);
		return;
	}
