...
#if !defined (CONFIG_MAS_BLK) && !defined (CONFIG_HUAWEI_UFS_VENDOR_MODE)
/* UFSHCD states */
enum {
	UFSHCD_STATE_RESET,
	UFSHCD_STATE_ERROR,
...
static struct ufs_dev_fix ufs_fixups[] = {
	...
	UFS_FIX(UFS_VENDOR_SKHYNIX, UFS_ANY_MODEL,
		UFS_DEVICE_QUIRK_DELAY_BEFORE_LPM),
	UFS_FIX(UFS_VENDOR_XHFZ, UFS_ANY_MODEL,
		UFS_DEVICE_QUIRK_DELAY_BEFORE_LPM),
...
inline bool ufshcd_wb_sup(struct ufs_hba *hba)
{
	...
	return (hba->caps & UFSHCD_CAP_WB_EN) &&
		...
		(hba->dev_info.wspecversion >= UFSHCI_VERSION_31 ||
		hba->dev_info.wspecversion == UFSHCI_VERSION_22);
}
 ...
 */
 #ifndef CONFIG_HUAWEI_UFS_VENDOR_MODE
 static
 #endif
...
#ifndef CONFIG_SCSI_MAS_UFS_MQ_DEFAULT
...
static void ufshcd_gate_work(struct work_struct *work)
{
	...
	if (hba->clk_gating.active_reqs
		...
		|| hba->mas_lrb_in_use || hba->outstanding_tasks
#else
...
#ifdef CONFIG_SCSI_MAS_UFS_MQ_DEFAULT
	if (ufshcd_lba_tag_alloc(hba, cmd))
...
#ifdef CONFIG_SCSI_MAS_UFS_MQ_DEFAULT
		ufshcd_lba_tag_release(hba, tag);
#endif
		return SCSI_MLQUEUE_HOST_BUSY;
	}
					...
					unsigned long completed_reqs)
{
	...
	for_each_set_bit(index, &completed_reqs, hba->nutrs) {
		...
		if (cmd) {
			...
			ufshcd_crypto_clear_prdt(hba, lrbp);
			lrbp->cmd = NULL;	/* Mark the command as completed. */
			ufshcd_lba_tag_release(hba, index);
			ufshcd_release(hba);
			ufshcd_clk_scaling_update_busy(hba);
#else
...
#if defined CONFIG_SCSI_MAS_UFS_MQ_DEFAULT
...
static int ufs_get_device_desc(struct ufs_hba *hba)
{
	...
	if (ufs_proc_set_device(hba, model_index, desc_buf, QUERY_DESC_MAX_SIZE) < 0)
		goto out;
...
static int ufshcd_variant_hba_init(struct ufs_hba *hba)
{
	...
	set_bootdevice_name(BOOT_DEVICE_UFS, hba->dev);
...
static void ufshcd_vreg_set_lpm(struct ufs_hba *hba)
{
	...
	if (!ufshcd_is_link_active(hba) &&
		hba->manufacturer_id == UFS_VENDOR_HI1861)
