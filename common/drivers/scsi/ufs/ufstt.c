...
static int __ufstt_ppn_load(uint32_t lba, void *buf, uint32_t len)
{
	...
	const char cmd[READ_BUFFER_CDB_LEN] = { UFSTT_READ_BUFFER,
			       ...
			       (unsigned char)get_byte(lba, THREE_BYTE_SHIFT),
			       (unsigned char)get_byte(lba, TWO_BYTE_SHIFT),
			       (unsigned char)get_byte(lba, ONE_BYTE_SHIFT),
			       (unsigned char)get_byte(lba, ZERO_BYTE_SHIFT),
			       (unsigned char)get_byte(len, TWO_BYTE_SHIFT),
			       (unsigned char)get_byte(len, ONE_BYTE_SHIFT),
			       (unsigned char)get_byte(len, ZERO_BYTE_SHIFT),
...
static bool check_ufstt_is_legal(struct ufs_hba *hba)
{
	if (!ufstt_sdev) {
...
static void ufstt_print_info(struct ufs_hba *hba)
{
