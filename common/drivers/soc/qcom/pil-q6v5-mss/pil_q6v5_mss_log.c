...
#define WPSS_EXCEPTION "wpss_exception"
...
struct work_data{
    ...
    struct work_struct wpss_work;
    char wpss_reset_reason[SSR_REASON_LEN];
};
...
static void wpss_reset_work_func(struct work_struct *work);
...
static void log_modem_work_func(struct work_struct *work)
{
    ...
    if (g_work_data == NULL) {
        pr_err("[log_modem_reset]g_work_data is NULL!\n");
        return;
    }
...
EXPORT_SYMBOL(save_modem_err_log);
...
void save_modem_reset_log(char reason[], int reasonLength)
{
    ...
    if (g_work_data == NULL) {
        pr_err("[log_modem_reset]save_modem_reset_log null");
        return;
    }
...
EXPORT_SYMBOL(save_modem_reset_log);
...
int create_modem_log_queue(void)
{
    ...
    if(g_work_data != NULL) {
        pr_err("[log_modem_reset]g_work_data is not NULL, Don't create queue!\n");
        return error;
    }
    ...
    INIT_WORK(&(g_work_data->wpss_work), wpss_reset_work_func);
...
EXPORT_SYMBOL(create_modem_log_queue);
...
EXPORT_SYMBOL(destroy_modem_log_queue);

/*the queue work handle, store the modem reason to the exception file*/
static void wpss_reset_work_func(struct work_struct *work)
{
    struct work_data *work_data_self =
        container_of(work, struct work_data, wpss_work);
    if(work_data_self == NULL) {
        pr_err("[wpss_reset]wpss_reset_work_func null");
        return;
    }
    ...
    g_work_data->wpss_reset_reason[0] = '\0';
}

void wpss_reset_save_log(char reason[], int reasonLength)
{
    int is_work_pending;
    if (g_work_data == NULL || g_work_data->log_modem_work_queue == NULL) {
        pr_err("[wpss_reset]save_wpss_reset_log null");
        return;
    }

    is_work_pending = work_pending(&g_work_data->wpss_work);
    if (is_work_pending) {
        pr_err("[wpss_reset]save_wpss_reset_log is_work_pending");
        return;
    }

    strncpy(g_work_data->wpss_reset_reason, reason, SSR_REASON_LEN - 1);
    queue_work(g_work_data->log_modem_work_queue, &(g_work_data->wpss_work));
    pr_info("[wpss_reset] done");
    return;
}
