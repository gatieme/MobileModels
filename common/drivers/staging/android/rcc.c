 ...
 * Dynamically compress & clean used RAM module
 * Author:l00367303
 *
 *  main module implenment.
 *
 */
...
static void cal_time(u64 *total_time, u64 *busy_time)
{
	int i;
	u64 user, nice, system, total_idle;
	u64 cpu_i_idle_time, cpu_i_idle_time_us;

	cpu_i_idle_time_us = -1ULL;
	user = nice = system = total_idle = cpu_i_idle_time = 0;

	for_each_possible_cpu(i) {
		user += kcpustat_cpu(i).cpustat[CPUTIME_USER];
		nice += kcpustat_cpu(i).cpustat[CPUTIME_NICE];
		system += kcpustat_cpu(i).cpustat[CPUTIME_SYSTEM];
		cpu_i_idle_time = kcpustat_cpu(i).cpustat[CPUTIME_IDLE];
		if (cpu_online(i)) {
#ifdef arch_idle_time
			if (!nr_iowait_cpu(i))
				cpu_i_idle_time += arch_idle_time(i);
			total_idle += cpu_i_idle_time;
#else
			cpu_i_idle_time_us = get_cpu_idle_time_us(i, NULL);
			if (cpu_i_idle_time_us != -1ULL)
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0))
				total_idle += cpu_i_idle_time_us * NSEC_PER_USEC;
#else
				total_idle += usecs_to_cputime64(cpu_i_idle_time_us);
#endif
			else
				total_idle += cpu_i_idle_time;
#endif
	...
	*total_time = user + nice + system + total_idle;
...
{
	int cpuload;
	clock_t delta[INDEX_CPU_MAX];
	clock_t tmp = 0;

	u64 total_time, busy_time;

	cpuload = 0;
	cal_time(&total_time, &busy_time);
 ...
 *    thread exit code.
 */
static unsigned int get_system_stat(struct rcc_module *rcc, bool end)
{
	unsigned int ret = 0;
	int value;
	if (!is_cpu_idle(rcc))
...
/**
 * this mode for none full clean
 */
static inline int get_rcc_mode()
{
	return hyperhold_enable() ? RCC_MODE_FILE : RCC_MODE_ANON;
}
	...
	while (true) {
		...
		if (rcc->full_clean_flag) {
			...
			do {
				...
				_UPDATE_STATE();
				rcc->nr_full_clean_pages += nr_pages;
...
static void rcc_setup(struct rcc_module *rcc)
{
	...
	total_ram_pages = totalram_pages;
#endif
	if (total_ram_pages > 3 * TOTAL_RAM_PAGES_1G) { /* >3g */
		rcc->anon_pages_min = RCC_ANON_PAGE_MIN + 2 * RCC_ANON_PAGE_RAM_GAP;
	} else if (total_ram_pages > 2 * TOTAL_RAM_PAGES_1G) { /* 3g */
