...
unsigned int g_dyn_cap_item_num = 10;
unsigned int g_static_cap_item_num = 10;

unsigned int g_control_optimization = 0;
unsigned int g_req_power_num = 0;
unsigned int g_req_power_beta = 0;
unsigned int g_power_range_bucket = 0;
unsigned int g_power_range_token = 0;
unsigned int g_power_range_proportion = 0;
u32 *g_req_power_avg = NULL;
bool g_req_power_avg_init = true;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static s64* get_cpu_static_power_coeff(int cluster)
#else
...
static int hw_cluster_get_static_power(cpumask_t *cpumask, int interval,
				       unsigned long u_volt, unsigned long freq_mhz,
				       u32 *static_power)
#else
static int hw_cluster_get_static_power(cpumask_t *cpumask, int interval,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	for (i = 0; i < g_static_cap_item_num / 2 - 1; i++)
		tmp *= freq_mhz;

	for (i = 0; i < g_static_cap_item_num / 2; i++) {
		cap += cpu_coeff[i] * tmp;
		cap /= cpu_coeff[i + g_static_cap_item_num / 2];
		tmp /= freq_mhz;
	}
	*static_power = t_scale * v_scale / 10000;
	*static_power = *static_power * cap / 1000000;

	trace_cpu_static_power(nr_cpus, temperature, t_scale, v_scale, cap);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static int get_cluster_dyn_cap(struct device_node *np)
{
	int i, count;
	char prop_name[50];
	int ret = 0;
	...
	for (i = 0; i < g_cluster_num; i++) {
		ret = snprintf_s(prop_name, sizeof(prop_name), (sizeof(prop_name) - 1),
				 "huawei,cluster%d_dyn_capacitance", i);
		if (ret < 0) {
		...
		if (count <= 0) {
			pr_err("%s: ipa get invalid dyn_cap%d count\n", __func__, i);
			return -EINVAL;
		}

		ret = of_property_read_u32_array(np, prop_name,
						 g_caps.cluster_dyn_capacitance[i], count);
		if (ret != 0) {
			pr_err("%s: ipa get dyn cap fail\n", __func__);
			return ret;
		}
	}
...
static int get_cluster_static_cap(struct device_node *np)
{
	int ret, i, j;
	const char *cap_tmp;

	for (i = 0; i < g_cluster_num; i++) {
		for (j = 0; j < g_static_cap_item_num; j++) {
			ret = of_property_read_string_index(np, "huawei,cluster_static_capacitance",
							   (g_static_cap_item_num * i + j), &cap_tmp);
			if (ret != 0)
				return ret;

			ret = kstrtoll(cap_tmp, DECIMAL, &g_caps.cluster_static_capacitance[i][j]);
			if (ret != 0)
				return ret;
		}
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ret = of_property_read_u32(np, "huawei,cluster_static_cap_item_num",
				   &g_static_cap_item_num);
	if (ret != 0) {
		pr_err("%s g_dyn_cap_item_num read err\n", __func__);
		goto node_put;
	}

	ret = get_cluster_static_cap(np);
#else
...
int of_parse_ipa_control_optimization(void)
{
	int ret;
	struct device_node *np = NULL;

	np = of_find_node_by_name(NULL, "ipa_control_optimization");
	if (np == NULL) {
		pr_err("ipa_control_optimization node not found\n");
		return -ENODEV;
	}

	ret = of_property_read_u32(np, "huawei,control_optimization", (u32 *)&g_control_optimization);
	if (ret != 0) {
		pr_err("%s control optimization read err\n", __func__);
		goto node_put;
	} else if (g_control_optimization == 0) {
		pr_err("%s control_optimization read 0, disable ipa control optimization\n", __func__);
		goto node_put;
	}

	ret = of_property_read_u32(np, "huawei,req_power_num", (u32 *)&g_req_power_num);
	if (ret != 0) {
		pr_err("%s req_power_num read err\n", __func__);
		goto node_put;
	} else if (g_req_power_num == 0) {
		pr_err("%s req_power_num read 0, disable moving average\n", __func__);
	} else {
		g_req_power_avg = kcalloc(g_req_power_num, sizeof(*g_req_power_avg), GFP_KERNEL);
		if (!g_req_power_avg) {
			g_req_power_num = 0;
			pr_err("%s req_power_avg init fail, disable moving average\n", __func__);
			goto node_put;
		}
	}

	ret = of_property_read_u32(np, "huawei,moving_average_beta", (u32 *)&g_req_power_beta);
	if (ret != 0) {
		pr_err("%s req_power moving_average_beta read err\n", __func__);
		goto free_array;
	} else if (g_req_power_beta == 0) {
		g_req_power_num = 0;
		pr_err("%s moving_average_beta read 0, disable moving average\n", __func__);
	}

	ret = of_property_read_u32(np, "huawei,power_range_bucket", (u32 *)&g_power_range_bucket);
	if (ret != 0) {
		pr_err("%s power range bucket_token_max read err\n", __func__);
		goto free_array;
	} else if (g_power_range_bucket == 0) {
		g_control_optimization = 0;
		pr_err("%s bucket_token_max read 0, disable ipa control optimization\n", __func__);
		goto free_array;
	}
	g_power_range_token = 0;

	ret = of_property_read_u32(np, "huawei,token_proportion", (u32 *)&g_power_range_proportion);
	if (ret != 0) {
		pr_err("%s token_power_proportion read err\n", __func__);
		goto free_array;
	} else if (g_power_range_proportion == 0) {
		g_control_optimization = 0;
		pr_err("%s token_power_proportion read 0, disable ipa control optimization\n", __func__);
	}

	if (np)
		of_node_put(np);

	pr_info("parse ipa_control_optimization finished\n");
	return ret;

free_array:
	if (g_req_power_avg) {
		kfree(g_req_power_avg);
		g_req_power_avg = NULL;
	}
node_put:
	if (np)
		of_node_put(np);
...
EXPORT_SYMBOL(of_parse_ipa_control_optimization);
...
static bool g_soc_cdev_register_ok = false;
...
static int ipa_thermal_probe(struct platform_device *pdev)
{
	...
	if (strncmp(pdev->name, IPA_SENSOR0_NAME, sizeof(IPA_SENSOR0_NAME) - 1) == 0) {
		...
		if (!g_soc_cdev_register_ok) {
			pr_err("%s, soc cdev not registered, Deferring probe...\n", __func__);
			return -EPROBE_DEFER;
		}
