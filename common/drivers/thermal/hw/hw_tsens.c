...
const char *g_hw_tsensor_name[CAPACITY_OF_TSENSOR_ARRAY];
u32 g_hw_tsensor_mode[CAPACITY_OF_TSENSOR_ARRAY];
...
int hw_sec_readtemp_read(u64 reg_address)
{
			     ...
			     int *temp)
{
	...
	if (tm_sensor->reg_no >= 0 && tm_sensor->reg_no < g_tmdev->tsens_num_sensor) {
		adc_value = hw_sec_readtemp_read(tm_sensor->reg_no);
...
int ipa_get_tsensor_id(const char *name)
{
	...
	for (id = 0; id < g_tmdev->tsens_num_sensor; id++) {
		pr_debug("IPA: sensor_name = %s, g_hw_tsensor_name %d = %s\n",
			 name, id, g_hw_tsensor_name[id]);

		if (strncmp(name, g_hw_tsensor_name[id],
			    strlen(g_hw_tsensor_name[id])) == 0) {
			ret = id;
...
{
	int ret = -EINVAL;
	u32 sensor_num = sizeof(g_hw_tsensor_name) / sizeof(char *);
...
static int get_device_tree_data(struct platform_device *pdev)
{
	...
	/* parse .hw tsensor number */
	rc = of_property_read_s32(of_node, "hw,sensors", &tsens_num_sensors);
	if (rc != 0) {
	...
	for (i = 0; i < tsens_num_sensors; i++) {
		rc = of_property_read_string_index(of_node, "hw,tsensor_name",
						   i, &g_hw_tsensor_name[i]); /*lint !e605*/
		if (rc != 0) {
			pr_err("%s g_hw_tsensor_name %d read err\n", __func__, i);
			return -EINVAL;
		}
	}

	rc = of_property_read_u32_array(of_node, "hw,tsensor_mode",
					g_hw_tsensor_mode, tsens_num_sensors);
	if (rc != 0) {
		pr_err("%s g_hw_tsensor_mode read err\n", __func__);
		return -EINVAL;
	}

	rc = of_property_read_u32(of_node, "hw,tsensor_adc_start_value",
				       &register_info);
	if (rc != 0) {
		dev_err(dev, "no hw,tsensor_adc_start_value!\n");
		return -EINVAL;
	}
	g_tmdev->adc_start_value = register_info;

	rc = of_property_read_u32(of_node, "hw,tsensor_adc_end_value",
				       &register_info);
	if (rc != 0) {
		dev_err(dev, "no hw,tsensor_adc_end_value!\n");
		return -EINVAL;
	}
	g_tmdev->adc_end_value = register_info;

	rc = of_property_read_u32(of_node, "hw,pvtsensor_adc_start_value",
				       &register_info);
	if (rc != 0) {
		dev_err(dev, "no hw,pvtsensor_adc_start_value!\n");
		g_tmdev->pvtsensor_adc_start_value = 0;
	} else {
		g_tmdev->pvtsensor_adc_start_value = register_info;
	}

	rc = of_property_read_u32(of_node, "hw,pvtsensor_adc_end_value",
				       &register_info);
	if (rc != 0) {
		dev_err(dev, "no hw,pvtsensor_adc_end_value!\n");
...
static void get_detect_tsensor(struct platform_device *pdev, int i)
{
	...
	rc = snprintf_s(reg_no_name, sizeof(reg_no_name),
			...
			"hw,detect_%s_regno", g_hw_tsensor_name[i]);
	if (rc < 0)
...
static int _tsens_register_thermal(void)
{
	...
	for (i = 0, j = 0; i < g_tmdev->tsens_num_sensor; i++, j++) {
		...
		g_tmdev->sensor[i].mode = g_hw_tsensor_mode[i];
		...
		rc = snprintf_s(node_name, sizeof(node_name),
				(sizeof(node_name) - 1), "hw_tsens_%s",
				g_hw_tsensor_name[i]);
		if (rc < 0)
		...
		g_tmdev->sensor[i].tz_dev =
			thermal_zone_device_register(g_hw_tsensor_name[i],
						     trips_num, (int)mask,
						     &g_tmdev->sensor[i],
