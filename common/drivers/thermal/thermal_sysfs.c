		      ...
		      char *buf)
{
	...
	if (stats == NULL || stats->time_in_state == NULL)
		return len;
	...
	for (i = 0; i < stats->max_states; i++) {
		if (len >= PAGE_SIZE)
			break;
		len += snprintf(buf + len, PAGE_SIZE - len, "state%u\t%llu\n",
				i, ktime_to_ms(stats->time_in_state[i]));
	}
	spin_unlock(&stats->lock);

	if (len >= PAGE_SIZE) {
		pr_warn_once("Thermal time_in_state_ms_show exceeds PAGE_SIZE. Disabling\n");
		return -EFBIG;
	}
		     ...
		     const char *buf, size_t count)
{
	...
	instance = container_of(attr, struct thermal_instance, weight_attr);
#ifdef CONFIG_HW_IPA_THERMAL
	ret = ipa_thermal_cooling_device_weight_store(dev, instance, weight);
	if (ret)
		return ret;
#endif
