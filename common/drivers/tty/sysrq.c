...
static void sysrq_handle_crash(int key)
{
	...
	trace_android_rb_sreason_set("sysrq_crash");
...
{
#ifdef CONFIG_HANDSET_SYSRQ_RESET
	bool sysrq_switch = false;
	trace_android_cmd_himntn_item_switch(HIMNTN_ID_SYSRQ_SWITCH, &sysrq_switch);
	sysrq_enabled = sysrq_switch ? 1 : 0;
	pr_err("sysrq_enabled value is %d , %d\n", sysrq_enabled, sysrq_switch);
#endif
	sysrq_init_procfs();
