...
{
	/* wait 8 seconds for usb device enumerate */
	unsigned long jiffies_expire = jiffies + 8 * HZ;

	if (!udev)
		return -EFAULT;

	dev_info(&udev->dev, "usb trylock_device");
	while (!usb_trylock_device(udev)) {
		...
		*/
		if (time_after(jiffies, jiffies_expire)) {
			dev_err(&udev->dev, "trylock process fails");
			return -EFAULT;
		}
		msleep(100); /* retry usb_trylock_devic every 100ms */
	}
	dev_info(&udev->dev, "trylock process success");
	return 0;
}
...
/* The CONFIG_USB_DEVICE_READ_USE_TRYLOCK is used in the HiSi solution. */
