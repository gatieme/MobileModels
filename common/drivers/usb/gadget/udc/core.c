...
#define UDC_SEARCH_MAX_COUNT 10
#define UDC_SEARCH_WAIT_TIME 500
...
int usb_gadget_probe_driver(struct usb_gadget_driver *driver)
{
	...
	if (first_in == 1)
		...
		count = UDC_SEARCH_MAX_COUNT; /* set count exceed max retry time */
...
re_search:
		list_for_each_entry(udc, &udc_list, list) {
			pr_info("%s, %d dev_name = %s\n", __func__, __LINE__, dev_name(&udc->dev));
			ret = strcmp(driver->udc_name, dev_name(&udc->dev));
			if (!ret)
				break;
		}
		if (ret) {
			/* delay 10 * 500 ms to wait udc ready */
			if (count++ < UDC_SEARCH_MAX_COUNT) {
				mdelay(UDC_SEARCH_WAIT_TIME);
				goto re_search;
			} else {
				ret = -ENODEV;
			}
