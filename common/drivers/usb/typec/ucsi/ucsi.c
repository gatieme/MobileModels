...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void ucsi_set_usb_role_work(struct work_struct *work)
{
	struct ucsi_connector *con = container_of(work, struct ucsi_connector, usb_role_work.work);
	int ret;
	static int cnt = 60; /* Try max to 60 times to reget usb role */

	if (!con->usb_role_sw) {
		if (cnt-- <= 0) {
			dev_err(con->ucsi->dev, "con%d: reget usb role too much times\n", con->num);
			return;
		}

		con->usb_role_sw = fwnode_usb_role_switch_get(con->typec_cap.fwnode);
		if (IS_ERR(con->usb_role_sw) || !con->usb_role_sw) {
			dev_err(con->ucsi->dev, "con%d: failed to reget usb role switch\n", con->num);
			con->usb_role_sw = NULL;
                	/* Wait 1000ms to get usb role again */
			schedule_delayed_work(&con->usb_role_work, msecs_to_jiffies(1000));
			return;
		}
	}

	ret = usb_role_switch_set_role(con->usb_role_sw, con->u_role);
	dev_err(con->ucsi->dev, "con%d: send usb role %d in work ret %d\n",
		con->num, (int)con->u_role, ret);
}

static int ucsi_set_usb_role(struct ucsi_connector *con, enum usb_role u_role)
{
	if (con->usb_role_sw)
		return usb_role_switch_set_role(con->usb_role_sw, u_role);

	dev_err(con->ucsi->dev, "con%d: send usb role %d later\n", con->num, (int)u_role);
	con->u_role = u_role;
	if (!delayed_work_pending(&con->usb_role_work))
        	/* Wait 1000ms to get usb role again */
		schedule_delayed_work(&con->usb_role_work, msecs_to_jiffies(1000));
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	ret = ucsi_set_usb_role(con, u_role);
#else
...
static int ucsi_oem_set_usb_con(int role)
{
	...
	if (!con || !con->ucsi || !con->usb_role_sw)
		return -EIO;

	cur_role = usb_role_switch_get_role(con->usb_role_sw);
	...
	return usb_role_switch_set_role(con->usb_role_sw, cur_role);
}
...
static int ucsi_oem_force_usb_disc(void)
{
	struct ucsi_connector *con = g_oem_con;
	enum usb_role cur_role;
	...
	cur_role = usb_role_switch_get_role(con->usb_role_sw);
	dev_info(con->ucsi->dev, "force usb role %d\n", cur_role);
	if (cur_role == USB_ROLE_DEVICE)
		return usb_role_switch_set_role(con->usb_role_sw, USB_ROLE_NONE);
	return 0;
}
...
static struct pd_ucsi_ops hwpd_ops = {
	...
	.force_usb_disc = ucsi_oem_force_usb_disc,
};
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
static int ucsi_register_port(struct ucsi *ucsi, int index)
{
	...
	INIT_DELAYED_WORK(&con->usb_role_work, ucsi_set_usb_role_work);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (delayed_work_pending(&ucsi->connector[i].usb_role_work))
		cancel_delayed_work_sync(&ucsi->connector[i].usb_role_work);
#endif
