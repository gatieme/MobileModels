...
#ifdef CONFIG_SDP_ENCRYPTION
int fscrypt_setup_sdp_info(struct inode *inode,
	struct fscrypt_info **crypt_info, bool need_dirhash_key)
{
...
fscrypt_setup_encryption_info(struct inode *inode,
			      ...
			      bool need_dirhash_key, struct inode *parent)
{
	...
	if (inode->i_sb->s_cop && inode->i_sb->s_cop->get_keyinfo) {
		res = inode->i_sb->s_cop->get_keyinfo(inode, (void *)parent, &flag);
		if (res != 0) {
...
int fscrypt_get_encryption_info(struct inode *inode, bool allow_unsupported)
{
	...
	if (fscrypt_has_encryption_key(inode) &&
		(!inode->i_crypt_info->ci_hw_enc_flag || !S_ISREG(inode->i_mode)))
		return 0;
