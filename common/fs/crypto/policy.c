...
static bool fscrypt_policies_equal_for_sdp(const union fscrypt_policy *policy1,
	const union fscrypt_policy *policy2)
{
	if (policy1->version != policy2->version)
		return false;
	return (policy1->v2.contents_encryption_mode ==
		policy2->v2.contents_encryption_mode) &&
		(policy1->v2.filenames_encryption_mode ==
		policy2->v2.filenames_encryption_mode) &&
		(policy1->v2.flags == policy2->v2.flags);
}
...
sdp_perm:
	...
	return fscrypt_policies_equal_for_sdp(&parent_policy, &child_policy);
#endif
}
