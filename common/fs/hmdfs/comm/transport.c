...
static void jiffies_to_timeval(const unsigned long jiffies, struct __kernel_sock_timeval *value)
{
	/*
...
static int tcp_set_recvtimeo(struct socket *sock, int timeout)
{
	...
	rc = sock_setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO_NEW, KERNEL_SOCKPTR(&tv), sizeof(tv));
	if (rc)
				 ...
				 struct hmdfs_head_cmd *head)
{
	...
	size_t outlen = 0;
	__u8 *outdata = NULL;
	__u32 recv_len = le32_to_cpu(head->data_len);
	struct connection *connect = tcp->connect;

	if (recv_len == sizeof(*head))
		goto out_recv_head;

	if (recv_len < sizeof(*head) || recv_len > ADAPTER_MESSAGE_LENGTH)
		return -ENOMSG;

	cipherbuffer_len = recv_len - sizeof(*head) + HMDFS_IV_SIZE +
			   HMDFS_TAG_SIZE;
	cipherbuffer = kvzalloc(cipherbuffer_len, GFP_KERNEL);
	if (!cipherbuffer)
		return -ESHUTDOWN;
	outlen = cipherbuffer_len - HMDFS_IV_SIZE - HMDFS_TAG_SIZE;
	outdata = kvzalloc(outlen, GFP_KERNEL);
	if (!outdata) {
		kvfree(cipherbuffer);
		return -ESHUTDOWN;
	}
...
out_recv_head:
	if (connect_recv_callback[connect->status]) {
		connect_recv_callback[connect->status](connect, head, outdata,
						       outlen);
	} else {
		kvfree(outdata);
				      ...
				      struct hmdfs_send_data *msg)
{
	...
	if (ret != 0) {
		hmdfs_err("return error %d", ret);
		mutex_unlock(&connect->send_mutex);
		return ret;
	}
...
{
	int err = 0;
	int bufsize = CONFIG_HMDFS_D2DP_UDP_SND_BUF;
	if (bufsize) {
		hmdfs_debug("overriding UDP buffer size to %d", bufsize);

		err = sock_setsockopt(sock, SOL_SOCKET, SO_SNDBUF,
				      KERNEL_SOCKPTR((char *)&bufsize), sizeof(bufsize));
		if (err)
			hmdfs_err("Can't set UDP send buffer, error %d", err);
	}
	return err;
}
					     ...
					     struct connection_init_info *info)
{
	...
	*/
	...
	d2dp->proto = d2d_protocol_create(d2dp->sock, &d2dp->security,
					  info->d2dp_version);
	if (!d2dp->proto) {
...
static void connection_notify_to_reget(struct connection *conn)
{
#ifdef CONFIG_HMDFS_D2DP_TRANSPORT
	/* D2DP connection depends on TCP, so don't re-get it */
	if (conn->type == CONNECT_TYPE_D2DP) {
		notify_d2dp_failed(conn);
		return;
	}
#endif

	if (conn->link_type == LINK_TYPE_P2P) {
		notify_disconnect(conn);
		return;
	}

	if (conn->link_type == LINK_TYPE_LOOPBACK) {
		/* it is not currently supported to relink */
		hmdfs_err("lookback connection already broken");
		return;
	}

	notify_reget_session(conn);
}
...
void hmdfs_reget_connection(struct connection *conn)
{
	...
	if (conn->fd != INVALID_SOCKET_FD)
		connection_notify_to_reget(conn);
...
#ifdef CONFIG_HMDFS_D2DP_TRANSPORT
	case CONNECT_TYPE_D2DP: {
		struct d2dp_handle *d2dp = d2dp_alloc_handle(conn, info);
		if (!d2dp)
			notify_d2dp_failed(conn);
...
static unsigned int conn_priority(struct connection *conn)
{
				  ...
				  struct connection_init_info *info)
{
	...
	if (on_peer_conn == conn) {
		...
		if (info->link_type == LINK_TYPE_LOOPBACK) {
			/* if LINK_TYPE_LOOPBACK, skip handshake and crypto */
			on_peer_conn->status = CONNECT_STAT_WORKING;
			peer_online(node);
			node->version = DFS_2_0;
			node->conn_operations = hmdfs_get_peer_operation(DFS_2_0);
			hmdfs_info("Succeed to create the loopback network");
			return conn;
		}
