...
unsigned int f2fs_find_continuous_secs(struct f2fs_sb_info *sbi, int secnum)
{
...
void f2fs_hp_set_secs(struct f2fs_sb_info *sbi, int type,
			unsigned int secno, int secnum)
{
	unsigned int i;

	down_write(&SIT_I(sbi)->sentry_lock);
	for (i = secno; i < secno + secnum; ++i)
		__set_sit_entry_type(sbi, type, i, 1);
...
static int f2fs_hp_file_expand(struct inode *inode, unsigned long long len)
{
	...
	secnum = pg_end / BLKS_PER_SEC(sbi);
	if (has_not_enough_free_secs(sbi, 0, secnum)) {
	...
	target_secno = f2fs_find_continuous_secs(sbi, secnum);
	...
	err = f2fs_curseg_set(sbi, CURSEG_COLD_DATA_PINNED, target_segno, &backupseg);
	if (err) {
		unsigned int i;
		for (i = target_segno;
	...
	if (!err)
		f2fs_hp_set_secs(sbi, CURSEG_COLD_DATA, target_segno, secnum);
	f2fs_curseg_restore(sbi, CURSEG_COLD_DATA_PINNED, &backupseg);
	up_write(&sbi->cp_global_sem);
	up_write(&sbi->pin_sem);
				...
				struct hp_file_cfg *cfg)
{
	...
	if (cfg->addr == 0 || offset > MAX_BLKADDR(sbi) || offset < cfg->addr) {
