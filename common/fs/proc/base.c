 ...
 */
 static int proc_pid_ravg_sum(struct seq_file *m, struct pid_namespace *ns,
                              struct pid *pid, struct task_struct *task)
{
	trace_android_vh_ravg_sum(m, task);
	return 0;
}
...
#include "wakeup_new_task_fast_procfs.c"
#endif
...
int proc_rtg_open(struct inode *inode, struct file *filp)
{
	...
	trace_android_rvh_proc_rtg_open(inode, filp, &ret);
	return ret;
}
			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	trace_android_rvh_proc_rtg_compat_ioctl(file, cmd, arg, &ret);
	return ret;
}
#endif
...
long proc_rtg_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	trace_android_rvh_proc_rtg_ioctl(file, cmd, arg, &ret);
	return ret;
}
...
int sched_group_id_open(struct inode *inode, struct file *filp)
{
	...
	trace_android_rvh_sched_group_id_open(inode, filp, &ret);
	return ret;
}
		     ...
		     size_t count, loff_t *offset)
{
	...
	trace_android_rvh_sched_group_id_write(file, buf, count, offset, &ret);
	return ret;
}
...
#if IS_ENABLED(CONFIG_COMMON_HW_RTG_FRAME) || defined(CONFIG_HW_VIP_THREAD) || defined(CONFIG_HUAWEI_SWAP_ZDATA)
 ...
 */
#if IS_ENABLED(CONFIG_COMMON_HW_RTG_FRAME) || defined(CONFIG_HUAWEI_SWAP_ZDATA) || defined(CONFIG_HW_VIP_THREAD)
...
static int pid_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	if (task) {
#if IS_ENABLED(CONFIG_COMMON_HW_RTG_FRAME) || defined(CONFIG_HW_VIP_THREAD)
...
#endif
#ifdef CONFIG_HW_GRADED_SCHED_WAKEUP_NEW_TASK_FAST
	REG("wakeup_new_task_fast", S_IRUSR|S_IWUSR, proc_wakeup_new_task_fast_operations),
#endif
};
