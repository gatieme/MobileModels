...
#endif
#if defined CONFIG_PROCESS_RECLAIM
...
u64 smaps_get_pss(struct vm_area_struct *vma, u64 *swap_pss)
{
	...
	if (swap_pss)
		*swap_pss = mss.swap_pss >> PSS_SHIFT;
	return (mss.pss + mss.swap_pss) >> PSS_SHIFT;
}
#endif
...
#ifdef CONFIG_PROCESS_RECLAIM
#ifdef CONFIG_HUAWEI_SWAP_ZDATA
static void do_lru_gettimeofday(struct __kernel_old_timeval *tv)
{
...
static bool test_page_evictable(struct page *page)
{
	bool ret;
				...
				struct page *page)
{
	 ...
	 */
	if ((rp->type == RECLAIM_INACTIVE) && PageActive(page))
		return true;
	if ((rp->type == RECLAIM_ANON) && !PageAnon(page))
		return true;
	if ((rp->type == RECLAIM_FILE) && PageAnon(page))
		return true;
	if (!test_page_evictable(page) && PageUnevictable(page))
		return true;
...
#ifdef CONFIG_HUAWEI_SWAP_ZDATA
	reclaimed = reclaim_pages_from_list(&page_list, rp->hiber, &rp->nr_writedblock);
#else
...
#ifdef CONFIG_HUAWEI_SWAP_ZDATA
	int walk_ret;
	struct __kernel_old_timeval start_time = {0,0};
	struct __kernel_old_timeval stop_time;
