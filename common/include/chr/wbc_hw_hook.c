 ...
 * Description: this file is used by the netfilter hook
 *              for app download monitor and ad filter.
 * Author: l00371106
 * Create: 2016-10-28
 */
	...
	const struct nf_hook_state *state)
{
	...
	if (g_http_para_out.is_valid == IS_UNUSE) {
		...
		if (tcph->syn == 1 && tcph->ack == 0) {
			...
			p_http_str = (char *)hw_get_payload_addr(skb, IPPROTO_TCP, &vaddr, &dlen); // need unmap_vaddr
	...
	const struct nf_hook_state *state)
{
	...
	is_response = is_http_response(p_http_str, dlen, tcph);
	if (is_response == 1) {
