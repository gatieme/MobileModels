...
void tcp_recover_hiview_report(struct sock *sk, u32 estimate_time)
{
	...
	u32 advmss = tp->advmss;
	if (advmss == 0)
		advmss = 1420;
	u32 unreceived_pkg = (TCP_SKB_CB(tp->ooo_last_skb)->end_seq - tp->rcv_nxt)/advmss;
...
void update_should_enter_recover(struct sock *sk, u32 seq)
{
	...
	if (tp->should_enter_recover)
		goto recover_check;
	rtt_ms = get_current_srtt(tp);
	if (rtt_ms == 0 || rtt_ms > 500)
		rtt_ms = 500; /* average bad RTT from big data */
	advmss = tp->advmss;
	/* Threshold（10s） < (seq – ACK) * RTT / mss */
	if (before(tp->rcv_nxt, seq)
		&& (seq - tp->rcv_nxt) * rtt_ms * 2 > TIME_TEN_SEC * advmss) {
		goto recover_check;
	}
	tp->should_enter_recover = 0;
	tp->should_recover = 0;
