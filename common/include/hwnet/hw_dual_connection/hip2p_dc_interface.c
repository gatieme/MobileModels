...
static void netlink_msg_proc(const char *data, u32 length)
{
	struct hdc_msg *hdc = NULL;
	int ret;

	if (length >= sizeof(struct hdc_msg)) {
		hdc = (struct hdc_msg *)data;
	} else {
		dc_mloge("netlink_msg_proc has an out-of-bounds read");
		return;
	}
...
static void kernel_netlink_receive(struct sk_buff *skb)
{
	...
	if ((skb != NULL) && (skb->len >= NLMSG_HDRLEN)) {
		...
		if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr)) &&
			...
			netlink_msg_proc((char *)(msg->data), (u32)(nlh->nlmsg_len - sizeof(struct nlmsghdr)));
		}
	}
}
