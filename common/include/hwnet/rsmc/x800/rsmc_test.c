...
static void spi_loop_read(void)
{
	...
	if (tf == NULL)
		return;
	memset(tf, 0, DATA_MAX_LEN * sizeof(struct spi_transfer));
...
static int send_data_for_phone_8k(void)
{
	...
	memcpy(frame, info_dat_phone_8k, sizeof(info_dat_phone_8k));
...
static int send_data_for_phone_32k(void)
{
	...
	memcpy(frame, info_dat_phone_32k, sizeof(info_dat_phone_32k));
...
static int send_data_for_spec_32k(void)
{
	...
	memcpy(frame, info_dat_spec_32k, sizeof(info_dat_spec_32k));
