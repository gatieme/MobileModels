...
void print_buffer(u8 *buff, int len, char *tag)
{
	...
	for (index = 0; index < len; index++) {
		ret = sprintf(out + offset, "%02X", buff[index] & 0xFF);
		if (ret < 0) {
...
static void tx_config(struct tx_data_msg *msg)
{
	...
	memset(dat_for_send, 0x00, sizeof(dat_for_send));
	memcpy(dat_for_send, trans_frame, frame_len);
...
void init_tx_data(struct tx_data_msg *msg, bool only_tx)
{
	...
	if (g_tx_ctx.tx_proc)
		return;
	frame_len = msg->frame_len;
	memcpy((void *)g_tx_ctx.buf, (void *)(msg + 1), frame_len);
	memcpy((void *)&g_tx_ctx.msg, (void *)msg, sizeof(struct tx_data_msg));
