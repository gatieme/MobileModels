...
#define HWEVEXT_ADC_NAME_MAX 64
#define HWEVEXT_ADC_DELAY_US_TO_MS   1000
...
enum hwevext_dac_reg_ctl_type {
	...
	HWEVEXT_DAC_PARAM_NODE_TYPE_GPIO,
	HWEVEXT_DAC_PARAM_REGULATOR_TYPE,
...
struct hwevext_adc_gpio_ctl_node {
	int gpio;
	int state;
	int request_status;
	char name[HWEVEXT_ADC_NAME_MAX];
};

struct hwevext_adc_regulator {
	int mic_use_count;
	unsigned int mic_voltage;
	unsigned int mic_current;
...
struct hwevext_adc_i2c_priv {
	...
	int probe_completed;
	unsigned int adc_num_by_same_i2c_addr;
	unsigned int adc_iter;
	/* init regs */
	...
	struct hwevext_adc_regulator *regulator;
};
...
struct hwevext_adc_priv {
	...
	unsigned int gpio_ctl_num;
	struct hwevext_adc_gpio_ctl_node *gpio_ctl;
};
...
static int inline hwevext_adc_gpio_node_ctl(void *priv,
	struct hwevext_adc_reg_ctl *reg)
{
	return 0;
}
#endif /* CONFIG_HWEVEXT_EXTERN_ADC */
