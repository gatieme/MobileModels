...
/*
 * Currently, we only handle for root domain DL accounting
 * and bypass it when there is no deadline task in the
 * system when CONFIG_HW_SCHED_DL_TASK_OPT is set.
 *
 * NOTE: If we add new functions in the future, we should
 * reeassess the impact.
 */
...
static void rebuild_root_domains(void)
{
	...
	/*
	 * If there is no deadline task in the system,
	 * we bypass it.
	 */
	if (!dl_should_rebuild_root_domains())
		return;
#endif
	...
	cpuset_for_each_descendant_pre(cs, pos_css, &top_cpuset) {
		...
		/*
		 * NOTE: If there is no deadline task in the system when
		 * CONFIG_HW_SCHED_DL_TASK_OPT is set, we will
		 * bypass it.
		 */
