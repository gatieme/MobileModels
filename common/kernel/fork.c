					...
					struct kernel_clone_args *args)
{
...
#if defined(CONFIG_HW_VIP_THREAD) || IS_ENABLED(CONFIG_HW_RTG)
	...
	perf_event_fork(p);
#if defined(CONFIG_HW_VIP_THREAD) && defined(CONFIG_HW_GRADED_SCHED_WAKEUP_NEW_TASK_FAST)
	if (unlikely(p->group_leader->wakeup_new_task_fast)) {
		p->static_vip = 1;
		trace_sched_wakeup_new_task_fast(p, 1);
	}
#endif
