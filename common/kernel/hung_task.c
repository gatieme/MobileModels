...
static bool mod_installed = false;
...
{
	did_panic = 1;
	trace_android_set_did_panic(did_panic);
	return NOTIFY_DONE;
}
				  ...
				  void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	trace_android_set_timeout_secs(sysctl_hung_task_timeout_secs);
	...
	for ( ; ; ) {
		...
		if (t <= 0) {
			if (!atomic_xchg(&reset_hung_task, 0) &&
				...
				trace_android_check_tasks(timeout);
				if (!mod_installed) {
