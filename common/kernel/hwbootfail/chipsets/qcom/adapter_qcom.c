...
#define KERNEL_SAVE_MAGIC 0x5a5a
#define KERNEL_DUMP_MAGIC 0xa5a5
	...
	unsigned long src_len)
{
	...
	if (!src || dst == 0 || dst_max == 0 || src_len == 0) {
...
static void capture_kernel_log(char *pbuf, unsigned int buf_size)
{
	...
	if (!kbuf_addr || buf_size != log_buf_len_get()) {
...
{
	if (!get_bf_mem_addr() || get_bf_mem_size() <= BF_SIZE_1K)
...
static void capture_app_log(char *pbuf, unsigned int buf_size, const char *path)
{
	...
	ret = bf_readlink(path, pbuf, buf_size);
