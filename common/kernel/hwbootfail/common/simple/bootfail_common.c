...
#define NATIVE_PART_MOUNT_FAIL_MAGIC 0x553CB31A
...
static struct task_struct *send_bbox_notify_task = NULL;
...
__weak int ksys_fsync(unsigned int fd)
#else
__weak long ksys_fsync(unsigned int fd)
#endif
{
	print_err("please implement ksys_fsync\n");
	return 0;
}

__weak long ksys_statfs(const char __user * path, struct statfs __user *buf)
{
	print_err("please implement ksys_statfs\n");
	return 0;
}

__weak long ksys_newlstat(const char __user *filename, struct stat __user *statbuf)
{
	print_err("please implement ksys_newlstat\n");
	return 0;
}

__weak long ksys_lstat64(const char __user *filename, struct stat64 __user *statbuf)
{
	print_err("please implement ksys_lstat64\n");
	return 0;
}
...
static void process_boot_success(enum boot_stage stage)
{
	...
	if (get_elapsed_time() > BOOT_SLOWLY_TIME) {
		...
		memset_s(&pparam, sizeof(pparam), 0, sizeof(pparam));
...
static int ro_check()
{
	...
	struct bootfail_proc_param pparam;
	enum bootfail_errorcode error_code = get_boot_stage(&stage);
	if (error_code != BF_OK) {
		print_err("get_boot_stage ret: %d!\n", error_code);
		return -1;
	}
	int ret = memset_s(&pparam, sizeof(pparam), 0, sizeof(pparam));
	if (ret != EOK)
		print_err("memset failed ret: %d\n", ret);
	pparam.binfo.stage = stage;
	pparam.binfo.bootfail_errno = NATIVE_DATA_PART_MOUNT_RO;
	pparam.binfo.suggest_recovery_method = METHOD_NO_SUGGESTION;
	ret = sprintf_s(pparam.detail_info, sizeof(pparam.detail_info) - 1,
		"Data partition read only");
	if (ret != EOK) {
		print_err("memset failed ret: %d\n", ret);
		return -1;
	}
	return process_bootfail(&pparam);
}

static long ioctl_stage(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	enum boot_stage stage;
...
static int user_get_boot_timer_status(int *args)
{
	int state;
	int res;

	res = get_boot_timer_state(&state);
	if (res < 0) {
		print_err("get_boot_timer_state failed, ret is %d \n", res);
		return -EFAULT;
	}
	if (copy_to_user(args, &state, sizeof(state)) != 0) {
		print_err("copy_to_user failed!\n");
		return -EFAULT;
	}
...
static int user_get_boot_timer_timeout_value(int * args)
{
	unsigned int timeout_value = 0;
	int res;

	res = get_boot_timer_timeout_value(&timeout_value);
	if (res < 0) {
		print_err("get_boot_timer_timeout_value failed, ret is %d \n", res);
		return -EFAULT;
	}
	if (copy_to_user(args, &timeout_value,
...
static int user_set_timer_timeout_value(int * args)
{
	unsigned int timeout_value = 0;
	if (copy_from_user(&timeout_value, args,
		sizeof(timeout_value)) != 0) {
		print_err("copy_from_user failed!\n");
		return-EFAULT;
	}
	...
	unsigned long arg)
{
	...
	case GET_TIMER_STATUS: {
		res = user_get_boot_timer_status((int *)args);
		if (res)
			pr_err("userspace get boot timer status failed!\n");
		break;
	}
	case RESUME_TIMER:
	...
	case GET_TIMER_TIMEOUT_VALUE:
		res = user_get_boot_timer_timeout_value((int *)args);
		if (res)
			pr_err("userspace get boot timer timeout value failed!\n");
		break;
	case SET_TIMER_TIMEOUT_VALUE:
		res = user_set_timer_timeout_value((int *)args);
		if (res)
			pr_err("userspace set boot timer timeout value failed!\n");
		break;
	default:
		break;
	}
...
static void process_native_part_mount_fail(struct bootfail_proc_param *local_pparam,
						struct bootfail_param *bootfail)
{
	bootfail->magic = NATIVE_PART_MOUNT_FAIL_MAGIC; // magic data
	bootfail->stage = local_pparam->binfo.stage;
	bootfail->bootfail_errno = local_pparam->binfo.bootfail_errno;
	bootfail->reserved = 0;
	bootfail->error_time = local_pparam->binfo.rtc_time;
	int ret = memcpy_s((void *)&bootfail->desc, sizeof(bootfail->desc),
		local_pparam->detail_info, sizeof(bootfail->desc));
	if (ret != 0)
		print_err("memcpy_s failed ret = %d!\n", ret);
	ret = write_to_phys_mem(SUB_RESERVED_UNUSED_PHYMEM_BASE + BF_SIZE_1K,
					sizeof(struct bootfail_param), bootfail, BF_SIZE_256);
	if (ret != 0)
		print_err("write_to_phys_mem failed ret = %d!\n", ret);
	bf_sys_sync();
	msleep(2000);
	bf_sys_sync();
	panic(bootfail->desc);
...
static int __process_upper_bootfail(void *pparam)
{
	...
	memcpy_s((void *)local_pparam, sizeof(*local_pparam), &(tparam->pparam), sizeof(*local_pparam));
	up(&(tparam->sem));
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	if (local_pparam->binfo.bootfail_errno == NATIVE_PART_MOUNT_FAIL) {
		print_err("local_pparam->binfo.bootfail_errno == NATIVE_PART_MOUNT_FAIL !\n");
		struct bootfail_param bootfail;
		process_native_part_mount_fail(local_pparam, &bootfail);
	}
#endif
...
static int get_fs_state(const char *pmount_point, struct statfs *pstat)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS); //lint !e501
	memset_s((void *)pstat, sizeof(*pstat), 0, sizeof(*pstat));
...
static int process_upper_bootfail(unsigned long arg)
{
	...
	memset_s((void *)&pparam, sizeof(pparam), 0, sizeof(pparam));
...
static int process_bootfail(struct bootfail_proc_param *pparam)
{
	...
	memset_s((void *)param, sizeof(*param), 0, sizeof(*param));
	memcpy_s(&param->pparam, sizeof(*pparam), pparam, sizeof(*pparam));
...
static bool is_cmd_of_ioctl_others(unsigned int cmd)
{
	switch (cmd) {
	...
	unsigned long arg)
{
	...
	case GET_BFI_DEV_PATH:
		...
		break;
	case KERNEL_BBOX_NOTIFY:
		if (send_bbox_notify_task != NULL)
			wake_up_process(send_bbox_notify_task);
		return 0;
	default:
		print_err("Invalid CMD: 0x%x\n", cmd);
		ret = -EFAULT;
...
long boot_detector_ioctl(struct file *file,
	unsigned int cmd,
	unsigned long arg, unsigned int *is_used)
{
	*is_used = 1;
	...
	if (is_cmd_of_dynamic_brd(cmd))
		return ioctl_dynamic_brd(file, cmd, arg);
	else if (is_cmd_of_storage_rofa(cmd))
		return ioctl_storage_rofa(file, cmd, arg);
	else if (is_cmd_of_stage(cmd))
		return ioctl_stage(file, cmd, arg);
	else if (is_cmd_of_boot_timer(cmd))
		return ioctl_boot_timer(file, cmd, arg);
	else if (is_cmd_of_action_timer(cmd))
		return ioctl_action_timer(file, cmd, arg);
	else if (is_cmd_of_ioctl_others(cmd))
		return ioctl_others(file, cmd, arg);
	...
	unsigned long data_len)
{
	...
	/*lint -e446*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	paddr = (char *)ioremap((phys_addr_t)(uintptr_t)phys_mem_addr,
		bytes_to_read);
#else
	...
	unsigned long src_len)
{
	...
	/*lint -e446*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	paddr = (char *)ioremap((phys_addr_t)dst,
		bytes_to_write);
#else
...
static bool is_file_existed(const char *pfile_path)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS); //lint !e501
	memset_s((void *)&st, sizeof(st), 0, sizeof(st));
...
bool wait_file_node_ready(const char* file_node, int timeouts)
{
	if (unlikely(!file_node)) {
		print_err("invalid parameter: file_node\n");
		return false;
	}

	while (timeouts > 0) {
		if (is_file_existed(file_node)) {
			print_info("%s is ready!\n", file_node);
			return true;
		}
	...
	print_err("%s is not ready until timeout!\n", file_node);
	return false;
}

static void bbox_dump(const char *log_dir, struct error_info *info)
{
	const char* top_category = "System Reset";
	memset_s(info->module, MODULE_MAX_LEN, 0, MODULE_MAX_LEN);
	strncpy_s(info->module, MODULE_MAX_LEN, info->top_category, CATEGORY_MAX_LEN - 1);
	memset_s(info->top_category, CATEGORY_MAX_LEN, 0, CATEGORY_MAX_LEN);
	strncpy_s(info->top_category, CATEGORY_MAX_LEN - 1, top_category, strlen(top_category));
}

struct module_ops ops = {
	.module = MODULE_BOOTFAIL,
	.dump = bbox_dump,
	.reset = NULL,
...
static int __ref send_bbox_notify_kthread(void *arg)
{
	(void *)arg;
	return send_bbox_notify();
}
...
int bootfail_common_init(struct common_init_params *pparam)
{
	...
	print_info("BootDetector bootfail_common_init!\n");
	if (bbox_register_module_ops(&ops) != 0) {
		print_err("bootfail bbox_register_module_ops failed!\n");
		return -EINVAL;
	}
	send_bbox_notify_task =
		kthread_create(send_bbox_notify_kthread,
			NULL, "send_bbox_notify");
	print_info("BootDetector bootfail_common_init 2!\n");
	if (!is_boot_detector_enabled()) {
		print_info("BootDetector is disabled!\n");
		return -1;
	}
