		     ...
		     const char *text, u16 text_len)
{
	...
	/* fill message */
#ifdef CONFIG_PRINTK_PREFIX
	memcpy(&r.text_buf[0], tmp_buf, tmp_len + tlen);
	memcpy(&r.text_buf[0] + tmp_len + tlen, text, text_len - tmp_len - tlen);
#else
	...
	r.info->pid = current->pid;
	memset(r.info->comm, 0, TASK_COMM_LEN);
	memcpy(r.info->comm, current->comm, TASK_COMM_LEN - 1);
	if (ts_nsec > 0)
