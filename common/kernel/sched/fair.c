...
EXPORT_SYMBOL_GPL(task_util_est);
...
enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	...
	if (should_iowait_boost)
		cpufreq_update_util(rq, SCHED_CPUFREQ_HW | SCHED_CPUFREQ_IOWAIT);
...
static int detach_tasks(struct lb_env *env)
{
	...
	while (!list_empty(tasks)) {
		...
		case migrate_vip:
			...
			if  (p->vip_prio && rtg_target) {
				trace_android_rvh_walt_ktime_clock(&now);
				trace_android_rvh_get_wts_last_enqueued_ts(p, &last);
				if((now - last < vip_task_load_balance_delay) &&
...
#ifdef CONFIG_HUAWEI_SCHED_VIP
	case group_vip_preempt:
		if (sgs->vip_preempt_type <= busiest->vip_preempt_type)
			return false;
		break;
#endif
...
static inline void calculate_imbalance(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	if (busiest->group_type == group_vip_preempt) {
		env->migration_type = migrate_vip;
	}
#endif
...
#ifdef CONFIG_HUAWEI_SCHED_VIP
		case migrate_vip:
			if (!busiest || rq->vip_preempt_type > busiest->vip_preempt_type)
				busiest = rq;
				continue;
			break;
#endif
...
EXPORT_SYMBOL_GPL(capacity_curr_of);
#endif
