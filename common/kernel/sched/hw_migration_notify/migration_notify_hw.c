 ...
 */
static unsigned int util_to_freq(unsigned int cpu, unsigned int util)
{
	unsigned int freq = 0U;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	unsigned int cluster_max_freq = 0;
	trace_android_rvh_get_cluster_max_freq(cpu, &cluster_max_freq);
	freq = cluster_max_freq * (unsigned long)util / arch_scale_cpu_capacity(cpu);
#else
...
void mark_util_change_for_migration(struct task_struct *p,
				    struct rq *src_rq, struct rq *dest_rq)
{
	int src_cpu = cpu_of(src_rq);
	int dest_cpu = cpu_of(dest_rq);
	 ...
	 * must be less than min_freq.
	 */
	if (!nearly_same_freq(src_rq, src_freq_before, src_freq_after))
		sugov_mark_util_change(src_cpu, INTER_CLUSTER_MIGRATION_SRC);
	 ...
	 * save energy by checking the 3rd condition.
	 */
	if (!nearly_same_freq(dest_rq, dest_freq_before, dest_freq_after))
		sugov_mark_util_change(dest_cpu, INTER_CLUSTER_MIGRATION_DST);
}
EXPORT_SYMBOL_GPL(mark_util_change_for_migration);
