...
#if IS_ENABLED(CONFIG_HW_RTG_FRAME_RME)
...
int parse_aux_thread(const struct rtg_str_data *rs_data)
{
	...
	for (p = strsep(&data, ";"); p != NULL; p = strsep(&data, ";")) {
		...
		if (!strcmp(tmp, "auxTid")) {
			...
			info.prio = id;
		} else if (!strcmp(tmp, "avoidAuxNum")) {
			info.avoid_aux_num = id;
		} else {
			pr_err("[AWARE_RTG] parse config failed!\n");
			return -INVALID_ARG;
		}
...
static DEFINE_MUTEX(s_aux_comms_lock);
				 ...
				 struct rtg_proc_data *proc_info)
{
	...
	pr_info("[AWARE_RTG] %s uid:%d, pid:%d, tid:%d, type:%d, rtcnt=%d, rtgid:%d\n",
