...
#if IS_ENABLED(CONFIG_HW_ED_TASK)
...
int perf_ctrl_set_task_rtg_min_freq(void __user *uarg)
{
...
#else
	struct task_min_freq_config cfg;
#endif
	...
	if (copy_from_user(&cfg, uarg, sizeof(struct task_config)))
		return -EFAULT;
#else
	if (copy_from_user(&cfg, uarg, sizeof(struct task_min_freq_config)))
		return -EFAULT;
#endif
...
#else
	ret = set_task_rtg_min_freq(p, cfg.value, cfg.cur_ff_frame);
#endif
