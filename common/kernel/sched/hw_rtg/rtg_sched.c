...
#if IS_ENABLED(CONFIG_HW_RTG_NORMALIZED_UTIL) && IS_ENABLED(CONFIG_HW_SCHED_CLUSTER)
...
extern void __setscheduler_uclamp(struct task_struct *p,
				  const struct sched_attr *attr);
extern void sugov_check_freq_update(int cpu);
static inline unsigned int hw_freq_to_util(unsigned int cluster_cpu, unsigned int freq)
{
	return arch_scale_cpu_capacity(cluster_cpu) *
		(unsigned long)freq / cpu_cluster(cluster_cpu)->max_possible_freq;
}
...
int sched_set_group_window_rollover(unsigned int grp_id)
{
	...
	unsigned long flag;
#if IS_ENABLED(CONFIG_HW_RTG_BOOST)
	int boost;
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
		p = wts_to_ts(wts);
		wts->prev_window_load = wts->curr_window_load;
		wts->curr_window_load = 0;
		wts->prev_window_exec = wts->curr_window_exec;
		wts->curr_window_exec = 0;
#else
...
#if IS_ENABLED(CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
	struct related_thread_group *grp = wts->grp;
#else
...
#elif !IS_ENABLED(CONFIG_HW_CGROUP_RTG)
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	if ((current != p && (p->flags & PF_EXITING) && group_id) ||
#else
...
#endif
{
	...
	for_each_related_thread_group(grp) {
		...
		if (!list_empty(&grp->tasks) &&
			...
			if (grp->ff_info.shift_phase) {
				ff_info->shift_phase = grp->ff_info.shift_phase;
				ff_info->ais_driven = grp->ff_info.ais_driven;
				ff_info->cur_ff_frame =
					grp->ff_info.cur_ff_frame;
				ff_info->prev_ff_frame =
					grp->ff_info.prev_ff_frame;
				grp->ff_info.ais_driven = false;
			}
#endif
		}
		raw_spin_unlock_irqrestore(&grp->lock, flag);
...
#if IS_ENABLED(CONFIG_HW_RTG_BOOST) && IS_ENABLED(CONFIG_SCHED_TUNE)
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	cpumask_andnot(&search_cpus, &search_cpus, cpus_paused_mask);
#else
...
#if IS_ENABLED(CONFIG_HW_RTG_UCLAMP_UTIL)
static int set_task_min_util_tick(struct related_thread_group *grp,
...
#endif
{
	...
	unsigned long flags;
	int ret = 0;
#if IS_ENABLED(CONFIG_UCLAMP_TASK) && !IS_ENABLED(CONFIG_SCHED_TASK_UTIL_CLAMP)
	...
	grp->us_set_min_freq = freq;
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
	grp->ff_info.prev_ff_frame = (cur_ff_frame ? grp->ff_info.cur_ff_frame :
		cur_ff_frame);
	grp->ff_info.cur_ff_frame = cur_ff_frame;
	grp->ff_info.shift_phase = (grp->ff_info.prev_ff_frame !=
		grp->ff_info.cur_ff_frame);
	grp->ff_info.ais_driven = true;
#endif
