...
#ifdef CONFIG_HW_TOP_TASK_SYSFS
#define DEFAULT_TOP_TASK_HIST_SIZE             5
#define DEFAULT_TOP_TASK_STATS_POLICY          0
...
EXPORT_SYMBOL(sched_io_is_busy);
...
static inline bool use_pelt(void)
{
	...
	trace_android_vh_use_pelt_freq(&flag);
	return flag;
#else
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
		if (ff_info->shift_phase) {
			if (ff_info->ais_driven) {
				sg_policy_hw->stored_freq =
					sg_policy_hw->prev_freq;
				sg_policy_hw->prev_freq = new_freq;
			}
			if (sg_policy_hw->stored_freq)
				new_freq = sg_policy_hw->stored_freq;
		} else {
			sg_policy_hw->stored_freq = 0;
			sg_policy_hw->prev_freq = 0;
		}
#endif
...
static unsigned int sugov_next_freq_shared_policy(struct sugov_policy
						  *sg_policy, u64 time,
						  ...
						  unsigned int early_detection_len);

static void hw_sugov_fast_switch(struct sugov_policy *sg_policy);
				   ...
				   unsigned int next_freq)
{
	...
	if (policy->fast_switch_enabled) {
		hw_sugov_fast_switch(sg_policy);
	} else {
...
#if ( LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0) )
static unsigned long get_cpu_util(int cpu)
{
	unsigned long util = 0;
	trace_android_vh_get_cpu_util(cpu, &util);
	return util;
}

static unsigned long top_task_util(struct rq *rq)
{
	unsigned long util = 0;
	trace_android_vh_top_task_util(rq, &util);
	return util;
}

static unsigned long predict_util(struct rq *rq)
{
	unsigned long util = 0;
	trace_android_vh_predict_util(rq, &util);
	return util;
}

static unsigned long max_pred_ls(struct rq *rq)
{
	unsigned long util = 0;
	trace_android_vh_max_pred_ls(rq, &util);
	return util;
}

static unsigned long cpu_util_pred_min(struct rq *rq)
{
	unsigned long util = 0;
	trace_android_vh_cpu_util_pred_min(rq, &util);
	return util;
}
#endif
...
#ifdef CONFIG_HW_RTG_NORMALIZED_UTIL
#ifndef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
void sched_get_max_group_util(const struct cpumask *query_cpus, unsigned long *util,
					      unsigned int *freq)
{
	trace_android_vh_get_max_group_util(query_cpus, util, freq);
}
#else
void sched_get_max_group_util(const struct cpumask *query_cpus,
			      ...
			      struct ff_frame_info *ff_info)
{
	trace_android_vh_get_max_group_util(query_cpus, util, freq, ff_info);
}
#endif
#endif
			   ...
			   struct cpufreq_policy *policy)
{
	...
	raw_spin_lock_irqsave(&sg_policy_hw->timer_lock, irq_flag);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	...
	raw_spin_unlock_irqrestore(&sg_policy_hw->timer_lock, irq_flag);
			    ...
			    struct cpufreq_policy *policy)
{
	...
	if (sg_policy_hw->min_freq > policy->min) {
		sugov_mark_util_change(policy->cpu, POLICY_LIMITS_RESTORE);
		sugov_check_freq_update(policy->cpu);
	}

	sg_policy_hw->min_freq = policy->min;

	if (policy->fast_switch_enabled && (policy->max < policy->cur || policy->min > policy->cur)) {
		sugov_mark_util_change(policy->cpu, POLICY_LIMITS_RESTORE);
		sugov_check_freq_update(policy->cpu);
	}
}
...
#if defined(CONFIG_UCLAMP_TASK) && !defined(CONFIG_SCHED_TUNE)
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
	struct ff_frame_info ff_info;
	memset(&ff_info, 0, sizeof(struct ff_frame_info));
#endif

	if (unlikely(early_detection_len != CONFIG_NR_CPUS))
		return policy->cur;
...
#else
	sched_get_max_group_util(policy->cpus, &sg_policy->rtg_util,
		&sg_policy->rtg_freq, &ff_info);
#endif
...
#else
	return eval_target_freq(sg_policy, util, max, &ff_info);
#endif
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	u64 wallclock = 0;
	trace_android_rvh_walt_ktime_clock(&wallclock);
	now = use_pelt() ? ktime_get_ns() : wallclock;
#else
...
static void hw_sugov_fast_switch(struct sugov_policy *sg_policy)
{
	...
	int early_detection[CONFIG_NR_CPUS] = {0};
	int next_freq = 0;
	int skip_min_sample_time;
	int skip_hispeed_logic;

	next_freq = sugov_next_freq_shared_policy(sg_policy, sg_policy_hw->time,
						  early_detection, CONFIG_NR_CPUS);

	skip_min_sample_time = atomic_xchg(&sg_policy_hw->skip_min_sample_time, 0);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
			trace_android_rvh_walt_update_task_ravg(rq->curr, rq, TASK_UPDATE,
								 wallclock, 0);
#else
				   ...
				   struct sugov_tunables *backup)
{
	...
	struct sugov_tunables_hw *backup_hw = NULL;
	/* initialize the tunable parameters */
	if (backup) {
		backup_hw = &backup->tunables_hw;
...
#endif
#if defined(CONFIG_HW_TOP_TASK) || defined(CONFIG_HW_TOP_TASK_SYSFS)
...
static void sugov_tunables_restore(struct cpufreq_policy *policy)
{
	...
	struct sugov_tunables_hw *cached_hw = &cached->tunables_hw;

	tunables->rate_limit_us = cached->rate_limit_us;
	tunables_hw->hispeed_freq = cached_hw->hispeed_freq;
	tunables_hw->go_hispeed_load = cached_hw->go_hispeed_load;
	tunables_hw->target_loads = cached_hw->target_loads;
	tunables_hw->ntarget_loads = cached_hw->ntarget_loads;
	tunables_hw->overload_duration = cached_hw->overload_duration;
	tunables_hw->above_hispeed_delay = cached_hw->above_hispeed_delay;
	tunables_hw->nabove_hispeed_delay = cached_hw->nabove_hispeed_delay;
	tunables_hw->min_sample_time = cached_hw->min_sample_time;
	tunables_hw->nmin_sample_time = cached_hw->nmin_sample_time;
	tunables_hw->boostpulse_duration = cached_hw->boostpulse_duration;
	tunables_hw->boostpulse_min_interval = cached_hw->boostpulse_min_interval;
	tunables_hw->timer_slack_val = cached_hw->timer_slack_val;
	tunables_hw->fast_ramp_up = cached_hw->fast_ramp_up;
	tunables_hw->fast_ramp_down = cached_hw->fast_ramp_down;
	tunables_hw->freq_reporting_policy = cached_hw->freq_reporting_policy;
	tunables_hw->iowait_boost_step = cached_hw->iowait_boost_step;
				...
				struct sugov_tunables *tunables)
{
	...
	cached_hw->hispeed_freq = tunables_hw->hispeed_freq;
	cached_hw->go_hispeed_load = tunables_hw->go_hispeed_load;
	cached_hw->target_loads = tunables_hw->target_loads;
	cached_hw->ntarget_loads = tunables_hw->ntarget_loads;
	cached_hw->overload_duration = tunables_hw->overload_duration;
	cached_hw->above_hispeed_delay = tunables_hw->above_hispeed_delay;
	cached_hw->nabove_hispeed_delay = tunables_hw->nabove_hispeed_delay;
	cached_hw->min_sample_time = tunables_hw->min_sample_time;
	cached_hw->nmin_sample_time = tunables_hw->nmin_sample_time;
	cached_hw->boostpulse_duration = tunables_hw->boostpulse_duration;
	cached_hw->boostpulse_min_interval = tunables_hw->boostpulse_min_interval;
	cached_hw->timer_slack_val = tunables_hw->timer_slack_val;
	cached_hw->fast_ramp_up = tunables_hw->fast_ramp_up;
	cached_hw->fast_ramp_down = tunables_hw->fast_ramp_down;
	cached_hw->freq_reporting_policy = tunables_hw->freq_reporting_policy;
	cached_hw->iowait_boost_step = tunables_hw->iowait_boost_step;
