			...
			struct cpumask *search_cpus)
{
	...
	for_each_cpu(i, search_cpus) {
		...
		unsigned long cpu_is_reserved = 0;
		trace_android_rvh_is_reserved(i, &cpu_is_reserved);
...
static inline bool check_vip_contend(int cpu, struct task_struct *p)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	unsigned long capacity_curr_of_cpu;
#endif
	 ...
	 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		trace_android_rvh_capacity_curr_of(cpu, &capacity_curr_of_cpu);
		return cpu_util_without(cpu, p) * 100 > capacity_curr_of_cpu * sysctl_vip_separate_threshold;
#else
		return cpu_util_without(cpu, p) * 100 > capacity_curr_of(cpu) * sysctl_vip_separate_threshold;
#endif
}
...
int find_vip_cpu(struct task_struct *p)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	trace_android_rvh_find_rtg_target(p, &rtg_target);
#else
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	trace_android_rvh_cpumask_andnot(&search_cpus, &search_cpus);
#else
	...
	for_each_cpu(i, &search_cpus) {
		...
		int idle_idx = idle_cpu(i) ? idle_get_state_idx(rq)
					: INT_MAX;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
		unsigned long high_irqload = 0;
		unsigned long cpu_is_reserved = 0;
		trace_android_rvh_is_reserved(i, &cpu_is_reserved);
		trace_android_rvh_walt_cpu_high_irqload(i, &high_irqload);
#endif
		trace_sched_find_vip_cpu_each(
			p, i, !list_empty(&rq->vip_prio_thread_list),
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
			cpu_vip_prio, cpu_is_reserved, high_irqload,
#else
		 	cpu_vip_prio, cpu_is_reserved, walt_cpu_high_irqload(i),
#endif
...
#if defined(CONFIG_HW_EAS_SCHED) || defined(CONFIG_HAVE_QCOM_SCHED_WALT)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
		trace_android_rvh_schedtune_prefer_idle(prev, &perfer_idle);
		if (perfer_idle)
#else
...
{
	int kick_cpu = 0;
	trace_android_rvh_unisolated_cpu(rq, &kick_cpu);
...
static int nohz_kick_type(int call_cpu, struct sched_domain *sd)
{
	int type = NOHZ_KICK_ANY;
	int i;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	unsigned long is_min_cap;
	trace_android_rvh_is_min_capacity_cpu(call_cpu, &is_min_cap);
	if (!is_min_cap)
#else
	if (!is_min_capacity_cpu(call_cpu))
#endif
	...
	if (cpu_rq(call_cpu)->misfit_task_load) {
...
int find_new_ilb_opt(void)
{
	...
	for_each_domain(call_cpu, sd) {
		for_each_cpu_and(i, sched_domain_span(sd), cpu_online_mask) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
			trace_android_rvh_cpu_isolated(i, &isolated_cpu);
			if (isolated_cpu)
#else
			if (cpu_isolated(i))
#endif
			...
			if (idle_cpu(i)) {
				bool is_bigger_cpu =
					capacity_orig_of(i) > capacity_orig_of(call_cpu);
	...
	trace_sched_find_new_ilb_opt(type, ilb);
	reset_balance_interval(ilb);
	sched_vip_balance_set_overutilized(call_cpu);
...
EXPORT_SYMBOL_GPL(check_for_migration_vip_thread);
#endif
