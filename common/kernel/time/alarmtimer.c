...
int set_power_on_alarm(time64_t time_sec, bool enable_irq)
{
	...
	// remove rtc alarm
		...
		/* set current as alarm time, and turn off the irq */
                rtc_time64_to_tm(rtc_current_time, &poweroff_rtc_alarm.time);
                poweroff_rtc_alarm.enabled = 0;
                rtc_set_alarm(rtc, &poweroff_rtc_alarm);
                return 0;
	}
        memset_s(&tmp_time, sizeof(tmp_time), 0, sizeof(tmp_time));
        getnstimeofday(&tmp_time);

        memset_s(&setting_time, sizeof(setting_time), 0, sizeof(setting_time));
        rtc_time64_to_tm(time_sec, &setting_time);
        pr_info("%s() setting_time is : [%d-%d-%d] [%d:%d:%d] %lu\n",
                __func__,
                setting_time.tm_year + ALARM_BASE_YEAR,
        ...
        memset_s(&rtc_current_rtc_time, sizeof(rtc_current_rtc_time), 0, sizeof(rtc_current_rtc_time));
        rtc_read_time(rtc, &rtc_current_rtc_time);
        rtc_current_time = rtc_tm_to_time64(&rtc_current_rtc_time);
        pr_info("%s() rtc_current_rtc_time is : [%d-%d-%d] [%d:%d:%d] %lu\n",
                __func__,
                rtc_current_rtc_time.tm_year + ALARM_BASE_YEAR,
	...
	// get offset if system utc time does not equal rtc time
        ...
        memset_s(&poweroff_rtc_alarm, sizeof(poweroff_rtc_alarm), 0, sizeof(poweroff_rtc_alarm));
        ...
        rtc_time64_to_tm(alarm_value, &poweroff_rtc_alarm.time);
        poweroff_rtc_alarm.enabled = (enable_irq == true ? 1 : 0);
        pr_info("%s() set_rtc_time is : [%d-%d-%d] [%d:%d:%d] %lu\n",
                __func__,
                poweroff_rtc_alarm.time.tm_year + ALARM_BASE_YEAR,
...
static int __init alarmtimer_init(void)
{
	...
	alarm_bases[ALARM_REALTIME].get_ktime = &ktime_get_real;
        ...
        alarm_bases[ALARM_POWEROFF_REALTIME].get_ktime = &ktime_get_real;
