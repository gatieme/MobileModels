...
static void tasks_dump(bool verbose)
{
	...
	pr_info("[ pid ]   uid  tgid total_vm    rss rss_anon rss_file nptes swapents " \
		"oom_score_adj s name\n");
	...
	for_each_process(p) {
		...
		pr_info("[%5d] %5d %5d %8lu %6lu %8lu %8lu %5lu %5lu %5hd %c %s%c\n",
		...
		if (points > LMK_REPORT_POINTS)
			trace_android_lowmem_report(task, points);
	}
	rcu_read_unlock();
}
...
static void lowmem_dump(struct work_struct *work)
{
	...
	if (verbose)
		trace_android_mm_mem_stats_show(0);
