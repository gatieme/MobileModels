...
#endif /* CONFIG_HUAWEI_DIRECT_SWAPPINESS */
...
#ifdef CONFIG_HW_KSWAPD_ACCT
	if (current_is_kswapd())
		kswapdacct_shrinker_end(shrinker);
#endif
		if (ret == SHRINK_EMPTY)
			ret = 0;
		freed += ret;
			    ...
			    bool reclaimed, struct mem_cgroup *target_memcg)
{
	...
	if (!page_ref_freeze(page, refcount)) {
		if (current_is_kswapd() && page_is_file_lru(page))
...
#ifdef CONFIG_HUAWEI_SWAP_ZDATA
int reclaim_pages_from_list(struct list_head *page_list, bool hiber, unsigned int *nr_writedblock)
#else
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	...
	if (total_swap_pages && inactive_is_low(lruvec, LRU_INACTIVE_ANON))
		shrink_active_list(SWAP_CLUSTER_MAX, lruvec,
				   sc, LRU_ACTIVE_ANON);
#ifdef CONFIG_TASK_PROTECT_LRU
	shrink_protect_lru_by_overratio(pgdat);
#endif /* CONFIG_TASK_PROTECT_LRU */
}
...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
		shrink_nodes(zone->zone_pgdat, sc);
#else
					...
					nodemask_t *nodemask)
{
	...
	u64 start_throttle_time, end_throttle_time, delta_ms;
	...
	start_throttle_time = ktime_get_ns();
	/*
...
check_pending:
	end_throttle_time = ktime_get_ns();
	delta_ms = (end_throttle_time > start_throttle_time) ?
		(end_throttle_time - start_throttle_time)  / NSEC_PER_MSEC : 0;
	__count_vm_events(PGSCAN_DIRECT_THROTTLE_TIME, delta_ms);
...
#else
	if (cgroup_reclaim(&sc) && this_cpu_read(vm_event_states.event[ALLOCSTALL]))
	    this_cpu_dec(vm_event_states.event[ALLOCSTALL]);
#endif
	psi_memstall_leave(&pflags);
	return reclaimed;
}
#endif /* CONFIG_HUAWEI_RCC */
