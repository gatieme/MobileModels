...
static int __sock_diag_cmd(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	else if (nlh->nlmsg_type == IGMP_PROXY && hndl->igmp_proxy)
		err = hndl->igmp_proxy(skb, nlh);
	else if (nlh->nlmsg_type == IGMP_UNPROXY && hndl->igmp_unProxy)
		err = hndl->igmp_unProxy(skb, nlh);
#endif
	else
		err = -EOPNOTSUPP;
	mutex_unlock(&sock_diag_table_mutex);
