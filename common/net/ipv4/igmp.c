...
#ifdef CONFIG_HUAWEI_KSTATE
			pg_hook_mc_del(sk, iml->multi.imr_address.s_addr);
#endif
		}
...
#ifdef CONFIG_HUAWEI_KSTATE
			pg_hook_mc_del(sk, iml->multi.imr_multiaddr.s_addr);
#endif
		}
		/* decrease mem now to avoid the memleak warning */
		atomic_sub(sizeof(*iml), &sk->sk_omem_alloc);
...
#ifdef CONFIG_HUAWEI_KSTATE
int pg_igmp_proxy(struct net *net)
{
	struct in_device *in_dev;
	struct ip_mreqn mreq;
	char *pg_mc_proxy_addr;

	pr_info("pg cmd: mc proxy");
	pg_mc_proxy_addr = pg_get_proxy_addr();
	if (!pg_mc_proxy_addr) {
		pr_err("pg mc proxy addr is NUll!");
		return -EINVAL;
	}

	mreq.imr_multiaddr.s_addr = in_aton(pg_mc_proxy_addr);
	in_dev = ip_mc_find_dev(net, &mreq);
	if (!in_dev) {
		pr_err("pg mc proxy, the dev lost!");
		return -EINVAL;
	}
	ip_mc_filter_del(in_dev, in_aton(pg_mc_proxy_addr));
	return 0;
}
EXPORT_SYMBOL(pg_igmp_proxy);

int pg_igmp_unproxy(struct net *net)
{
	struct in_device *in_dev;
	struct ip_mreqn mreq;
	char *pg_mc_proxy_addr;

	pr_info("pg cmd: mc unproxy");
	pg_mc_proxy_addr = pg_get_proxy_addr();
	if (!pg_mc_proxy_addr) {
		pr_err("pg mc proxy addr is NUll!");
		return -EINVAL;
	...
	in_dev = ip_mc_find_dev(net, &mreq);
	if (!in_dev) {
		pr_err("pg mc unproxy, the dev lost!");
		return -EINVAL;
	}
	ip_mc_filter_add(in_dev, in_aton(pg_mc_proxy_addr));
	return 0;
}
EXPORT_SYMBOL(pg_igmp_unproxy);
#endif
