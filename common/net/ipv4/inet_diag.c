			     ...
			     bool net_admin)
{
	...
	if (sk->sk_socket != NULL) {
		if (nla_put_u32(skb, INET_DIAG_SK_PID, sk->sk_socket->pid)) {
			goto errout;
		}
	}
#endif
...
#ifdef CONFIG_HUAWEI_KSTATE
	} else if (cmd == IGMP_PROXY && handler->igmp_proxy) {
		err = handler->igmp_proxy(in_skb, req);
	} else if (cmd == IGMP_UNPROXY && handler->igmp_unProxy) {
		err = handler->igmp_unProxy(in_skb, req);
#endif
	} else {
		err = -EOPNOTSUPP;
	}
...
#ifdef CONFIG_HUAWEI_KSTATE
	.igmp_proxy = inet_diag_handler_cmd,
	.igmp_unProxy = inet_diag_handler_cmd,
#endif
};
