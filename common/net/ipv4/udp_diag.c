...
#ifdef CONFIG_HUAWEI_KSTATE
static int igmp_mc_proxy(struct sk_buff *in_skb,
			 const struct inet_diag_req_v2 *req)
{
	struct net *net;

	if (!in_skb) {
		pr_err("sock buff is NULL!");
		return -EINVAL;
	}
...
static int igmp_mc_unproxy(struct sk_buff *in_skb,
			   const struct inet_diag_req_v2 *req)
{
	struct net *net;
...
#endif
#ifdef CONFIG_HUAWEI_KSTATE
	.igmp_proxy = igmp_mc_proxy,
	.igmp_unProxy = igmp_mc_unproxy,
#endif
};
