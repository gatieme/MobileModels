...
DECLARE_TASKLET_OLD(netlink_event_report_tasklet,
	netlink_event_send_tasket);
...
static void netlink_send_unicast_data(struct netlink_ctrl *ctrl, struct list_node *node)
{
	struct sk_buff *skb = NULL;
	struct nlmsghdr *nlh = NULL;
	...
	if (nlh == NULL) {
		wifi_audio_log_err("nlh is NULL Pointer");
		free_list_node(node);
		node = NULL;
		kfree_skb(skb);
		return;
