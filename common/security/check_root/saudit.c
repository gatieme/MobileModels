...
void saudit_log_stack(struct saudit_buffer *sab)
{
	...
	if (sab->offset >= SAUDIT_BUFSIZ - 1)
		return;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	unsigned int nr_entries = stack_trace_save(entries, ARRAY_SIZE(entries), SAUDIT_SELF_DEPTH);
	sab->offset += stack_trace_snprint(sab->info + sab->offset,
		SAUDIT_BUFSIZ - sab->offset - 1, entries, nr_entries, 0);
#else
	save_stack_trace(&trace);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,0)
	sab->offset += stack_trace_snprint(sab->info + sab->offset,
		SAUDIT_BUFSIZ - sab->offset - 1, trace.entries, trace.nr_entries, 0);
#else
