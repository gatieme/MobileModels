...
static void set_item_info(struct stp_item *item, unsigned int id,
			int item_status, int item_credible)
{
	item->id = id;
			...
			const char *rootproc, unsigned int mask, int flag)
{
	...
	for (i = 0; i < MAX_NUM_OF_ITEM; i++) {
		...
		set_item_info(&item, stp_item_info->id, item_status, item_credible);
