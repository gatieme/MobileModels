...
#if defined(CONFIG_HHEE) || defined(CONFIG_HUAWEI_HKIP)
...
static inline void sop_down_read(struct mm_struct *mm)
{
...
static inline void sop_up_read(struct mm_struct *mm)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
	up_read(&mm->mmap_sem);
#else
...
static inline void sop_down_write(struct mm_struct *mm)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
...
static inline void sop_up_write(struct mm_struct *mm)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
...
static int sop_set_xom(struct file *file)
{
	...
	do {
		...
		get_page(p);
		SetPageReserved(p);
		sop_debug("%s: page No %u : refcount: %d\n",
			__func__, count, page_count(p));
		kaddr += PAGE_SIZE;
	} while (--count > 0);
...
static int sop_mmap_privileged(struct file *file, struct vm_area_struct *vma)
{
	...
	if (((node->base != NULL) && (node->len != 0)) || (size > SZ_1M)) {
		pr_err("%s: base or length or size error (%ul, %ul)\n",
			__func__, node->len, size);
...
static int sop_set_tag_privileged(struct sop_user *user, struct sop_mem *mem)
{
	...
	sop_debug("%s: sop add track list success, tag: %s, tagsize: %u,"
		"len: %lx\n", __func__, turbo->tag, turbo->tag_size,
		(unsigned long)turbo->len);
	return 0;
}
