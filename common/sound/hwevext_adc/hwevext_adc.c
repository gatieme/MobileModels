...
enum hwevext_adc_gpio_request_state {
	GPIO_NOT_REQUEST = 0,
	GPIO_REQUEST_SUCCESS,
};
...
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
	...
	.get = snd_hwevext_adc_get_volume, \
	.put = snd_hwevext_adc_put_volume, \
...
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
	...
	.get = snd_hwevext_adc_get_kctl, \
	.put = snd_hwevext_adc_put_kctl, \
...
#if IS_ENABLED (CONFIG_NEED_WRITE_SINGLE_ADC)
...
static unsigned int hwevext_adc_get_fist_bit_from_mask(unsigned int mask)
{
...
static int snd_hwevext_adc_get_kctl_read_reg(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	unsigned int chip_id;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	return snd_hwevext_adc_get_kctl_read_reg(kcontrol, ucontrol);
}
...
static int hwevext_adc_get_prop_num(struct device_node *node, const char *str)
{
	int i = 0;
...
static int hwevext_adc_save_gpio_ctl_node(struct device_node *node,
	struct hwevext_adc_gpio_ctl_node **gpio_ctl, unsigned int num)
{
	struct hwevext_adc_gpio_ctl_node *ctl = NULL;
	struct property *pp = NULL;
	const char *gpio_rst_str = "gpio_ctl";
	int i = 0;
	int ret;
	unsigned int switch_node_size;

	switch_node_size = sizeof(struct hwevext_adc_gpio_ctl_node) * num;
	...
	for_each_property_of_node(node, pp) {
		...
		ret = snprintf_s(ctl[i].name,
			(unsigned long)HWEVEXT_ADC_NAME_MAX,
			(unsigned long)HWEVEXT_ADC_NAME_MAX - 1,
			"hwevext_adc_gpio_ctl_%d", i);
		if (ret < 0) {
			hwlog_err("%s: set gpio name failed\n", __func__);
			ret = -EFAULT;
			goto gpio_ctl_err_out;
		}
...
static int hwevext_adc_save_gpio_ctl_default_value(struct device_node *node,
	struct hwevext_adc_gpio_ctl_node *ctl, unsigned int num)
{
	...
	ret = hwevext_adc_get_prop_of_u32_array(node, gpio_state_default_str,
	...
	if (count != (int)num) {
		...
		hwevext_adc_kfree_ops(gpio_state);
		return -EFAULT;
	}
...
static void hwevext_adc_free_switch_ctl_gpio(unsigned int ctl_num,
	struct hwevext_adc_gpio_ctl_node **ctl)
{
	int i;

	if ((ctl_num == 0) || (ctl == NULL) || (*ctl == NULL))
		return;

	for (i = 0; i < (int)ctl_num; i++) {
		if (gpio_is_valid((*ctl)[i].gpio) &&
			(*ctl)[i].request_status == GPIO_REQUEST_SUCCESS) {
			gpio_free((unsigned int)((*ctl)[i].gpio));
			(*ctl)[i].gpio = -EINVAL;
			(*ctl)[i].request_status = GPIO_NOT_REQUEST;
		}
	}
...
static int hwevext_adc_parse_gpio_ctl(struct device_node *node,
	struct hwevext_adc_priv *adc_priv)
{
	const char *gpio_ctl_str = "gpio_ctl";
	int ret;
	unsigned int gpio_num;

	gpio_num = (unsigned int)hwevext_adc_get_prop_num(node, gpio_ctl_str);
	if (gpio_num == 0) {
		hwlog_err("%s: gpio_ctl gpio num is 0\n", __func__);
		return -EINVAL;
	}
	ret = hwevext_adc_save_gpio_ctl_node(node, &adc_priv->gpio_ctl,
		gpio_num);
	if (ret < 0) {
		hwlog_err("%s: save gpio ctl node err\n", __func__);
		goto parse_gpio_ctl_err;
	}

	ret = hwevext_adc_save_gpio_ctl_default_value(node,
		adc_priv->gpio_ctl, gpio_num);
	if (ret < 0) {
		hwlog_err("%s: save default value err\n", __func__);
		goto parse_gpio_ctl_err;
	}
	adc_priv->gpio_ctl_num = gpio_num;
	return 0;
parse_gpio_ctl_err:
	hwevext_adc_kfree_ops(adc_priv->gpio_ctl);
	adc_priv->gpio_ctl_num = 0;
	return ret;
}

static int hwevext_adc_pinctrl_select_state(struct pinctrl *p, const char *name)
{
...
static int hwevext_adc_parse_dt_switch_ctl_cfg(struct device *dev,
	struct hwevext_adc_priv *adc_priv)
{
	...
	if (of_property_read_bool(node, "need_gpio_ctl_pinctrl")) {
		...
		ret = hwevext_adc_pinctrl_select_state(pinctrl,
			"gpio_ctl_default");
		if (ret)
			return -ENOENT;
	}

	ret = hwevext_adc_parse_gpio_ctl(node, adc_priv);
	if (ret < 0)
	...
	struct hwevext_adc_priv *adc_priv)
{
	...
	ret += hwevext_adc_parse_dt_switch_ctl_cfg(&pdev->dev, adc_priv);

	return ret;
}

static int hwevext_adc_init_switch_ctl_gpio(struct hwevext_adc_priv *adc_priv)
{
	struct hwevext_adc_gpio_ctl_node *ctl = NULL;
	int i;
	int ret;

	ctl = adc_priv->gpio_ctl;
	if ((adc_priv->gpio_ctl_num == 0) || (ctl == NULL))
		return 0;

	for (i = 0; i < (int)adc_priv->gpio_ctl_num; i++) {
		hwlog_info("%s: %d node gpio %d, state %d\n", __func__, i,
			ctl[i].gpio, ctl[i].state);
		if (!gpio_is_valid(ctl[i].gpio)) {
			hwlog_info("%s: gpio:%d invaild, return\n",
				__func__, ctl[i].gpio);
			ret = 0;
			goto switch_ctl_gpio_err_out;
		}
	...
	return 0;
switch_ctl_gpio_err_out:
	hwevext_adc_free_switch_ctl_gpio(adc_priv->gpio_ctl_num,
		&adc_priv->gpio_ctl);
	adc_priv->gpio_ctl_num = 0;
	return ret;
}

int hwevext_adc_gpio_node_ctl(void *priv, struct hwevext_adc_reg_ctl *reg)
{
	struct hwevext_adc_priv *adc_priv = NULL;
	int gpio;

	adc_priv = priv;
	if ((adc_priv == NULL) || (adc_priv->gpio_ctl == NULL)) {
	...
	if (reg->addr >= adc_priv->gpio_ctl_num) {
		hwlog_err("%s: Invalid arg, gpio index %u>= switch num %u\n",
			__func__, reg->addr, adc_priv->gpio_ctl_num);
		return -EINVAL;
	}

	gpio = adc_priv->gpio_ctl[reg->addr].gpio;
	gpio_direction_output((unsigned int)gpio, (int)reg->value);
	if (reg->delay == 0)
		goto hwevext_adc_gpio_node_ctl_exit;

	if (reg->delay > HWEVEXT_ADC_DELAY_US_TO_MS) {
		msleep(reg->delay / HWEVEXT_ADC_DELAY_US_TO_MS);
...
hwevext_adc_gpio_node_ctl_exit:
	return 0;
}
EXPORT_SYMBOL_GPL(hwevext_adc_gpio_node_ctl);
...
static int hwevext_adc_probe(struct platform_device *pdev)
{
	...
	adc_priv = kzalloc(sizeof(struct hwevext_adc_priv), GFP_KERNEL);
	if (adc_priv == NULL)
		return -ENOMEM;
	...
	ret = hwevext_adc_init_switch_ctl_gpio(adc_priv);
	if (ret < 0) {
