...
#ifdef CONFIG_BINDER_HONOR_TRANSACTION_BRIEF
...
#ifdef CONFIG_HN_CGROUP_WORKINGSET
#include <linux/workingset_cgroup.h>
#endif

#if (defined CONFIG_HN_QOS_THREAD) || (defined CONFIG_HN_BINDER_SCHED)
#include <linux/sched/hwqos/hwqos_common.h>
#endif
#ifdef CONFIG_HN_BINDER_SCHED
#include <linux/sched/clock.h>
#include <linux/cgroup.h>
#endif

#ifdef CONFIG_HN_BINDER_FG_REQ_FIRST
...
#ifdef  CONFIG_BINDER_FD_LEAK_DEBUG
...
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
static inline bool is_binder_work_completion_type(struct binder_work *work) {
	return work->type == BINDER_WORK_TRANSACTION_COMPLETE
		|| work->type == BINDER_WORK_TRANSACTION_ONEWAY_SPAM_SUSPECT
...
#endif
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
	if (is_binder_work_completion_type(work))
		work->should_filter = true;
#endif
...
#ifdef CONFIG_HN_BINDER_SCHED
static inline bool binder_proc_has_bgworklist_ilocked(struct binder_proc *proc)
{
	return atomic_read(&binder_sched_switch) && proc->is_system_server &&
	       (!binder_worklist_empty_ilocked(&proc->bg_todo));
}

static inline bool
binder_thread_has_bgworklist_ilocked(struct binder_thread *thread,
...
binder_select_thread_by_type_ilocked(struct binder_proc *proc, int type)
{
...
#endif // CONFIG_HN_BINDER_SCHED

#ifdef CONFIG_HN_BINDER_FG_REQ_FIRST
static int binder_count_show(struct seq_file *m, void *unused)
{
	seq_printf(m, "Total foreground request: %llu\n",
		   (unsigned long long)atomic64_read(&binder_fg_req_num));
	return 0;
}
...
#endif // CONFIG_HN_BINDER_FG_REQ_FIRST

#ifdef CONFIG_HN_QOS_THREAD
static inline bool
binder_thread_check_and_set_dynamic_qos(
...
static inline void
binder_thread_check_and_remove_dynamic_qos(
			struct binder_thread *thread,
			unsigned int oneway)
{
	if (!oneway)
...
#ifdef CONFIG_KSTATE_COMMON
bool spear_binder_report(struct binder_proc *proc, struct binder_proc *target_proc)
{
	if (target_proc == NULL || proc == NULL || target_proc->tsk == NULL || proc->tsk == NULL) {
		return false;
	}
	if (task_is_stopped(target_proc->tsk)) {
		if (target_proc->tsk->cred == NULL || proc->tsk->cred == NULL) {
			return true;
		}

		if ((proc->tsk->cred->euid.val < MIN_APPLICATION_UID) ||
			(proc->tsk->cred->euid.val != target_proc->tsk->cred->euid.val)) {
			return true;
		}
	}
	return false;
...
bool binder_info_prt(int called_pid)
{
	pr_info("check_binder_calling_work proc,  pid:%d\n", called_pid);
	if (kernel_binder_cb_hook != NULL) {
		kernel_binder_cb_hook(0, called_pid);
	}
	return true;
}

bool check_binder_calling_work(int called_pid)
{
	bool ret = false;
	int uid = 0;
	struct binder_proc *proc = NULL;
	struct binder_proc *cache_called_proc = NULL;
	struct binder_thread *called_binder_thread = NULL;
	struct binder_transaction *t = NULL;
	struct rb_node *n = NULL;
	mutex_lock(&binder_procs_lock);
	hlist_for_each_entry(proc, &binder_procs, proc_node) {
		if (proc->pid == called_pid) {
			pr_info("check_binder_calling_work get binder_proc by pid:%d\n", called_pid);
			cache_called_proc = proc;
			if (cache_called_proc == NULL) continue;
			binder_inner_proc_lock(cache_called_proc);
			if (!binder_worklist_empty_ilocked(&cache_called_proc->todo)) {
				binder_inner_proc_unlock(cache_called_proc);
				mutex_unlock(&binder_procs_lock);
				return binder_info_prt(called_pid);
			}
#ifdef CONFIG_HN_BINDER_FG_REQ_FIRST
			if (!binder_worklist_empty_ilocked(&cache_called_proc->fg_todo)) {
				binder_inner_proc_unlock(cache_called_proc);
				mutex_unlock(&binder_procs_lock);
				return binder_info_prt(called_pid);
			}
#endif
#ifdef CONFIG_HN_BINDER_SCHED
			if (!binder_worklist_empty_ilocked(&cache_called_proc->bg_todo)) {
				binder_inner_proc_unlock(cache_called_proc);
				mutex_unlock(&binder_procs_lock);
				return binder_info_prt(called_pid);
			}
#endif
			for (n = rb_first(&cache_called_proc->threads); n != NULL; n = rb_next(n)) {
				called_binder_thread = rb_entry(n, struct binder_thread, rb_node);
				if (!binder_worklist_empty_ilocked(&called_binder_thread->todo)) {
					binder_inner_proc_unlock(cache_called_proc);
					mutex_unlock(&binder_procs_lock);
					return binder_info_prt(called_pid);
				}
				t = called_binder_thread->transaction_stack;
				if (t == NULL || t->from == NULL || t->from->proc == NULL || t->to_proc == NULL) {
					continue;
				}
				uid = t->to_proc->tsk->cred->euid.val;
				if (uid > 10000 && t->from->proc->tsk->cred->euid.val == uid) {
					pr_info("check_binder_calling_work a binder in uid: %d self, ignore!", uid);
					continue;
				}
				pr_info("check_binder_calling_work proc has %s transaction stack, %d:%d call %d:%d\n",
					(t->flags & TF_ONE_WAY) ? "async" : "sync", t->from->proc->pid, t->from->pid,
					t->to_proc->pid, t->to_thread ? t->to_thread->pid : 0);
				binder_inner_proc_unlock(cache_called_proc);
				mutex_unlock(&binder_procs_lock);
				return binder_info_prt(called_pid);
			}
...
{
    if (likely(!tsk || !task_wscg_preread(tsk)))
        return false;
...
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
static bool check_descriptor(struct binder_transaction_data *tr,
			     const char *descriptor, unsigned int size) {
	char *p = NULL;
	char tmp[BINDER_DESCRIPTOR_BYTE_SIZE]; /* size has been controlled, so no need to init */

	if (tr == NULL || descriptor == NULL || size > BINDER_DESCRIPTOR_BYTE_SIZE ||
	    (BINDER_DESCRIPTOR_OFFSET + size) > tr->data_size)
		return false;
	p = (char *)(uintptr_t)(tr->data.ptr.buffer) + BINDER_DESCRIPTOR_OFFSET;
	if (copy_from_user(tmp, p, size))
		return false;
	if (memcmp(tmp, descriptor, size) == 0)
		return true;
	else
		return false;
}

static void check_async_binder_rt(struct binder_transaction *t,
	...
	if (t != NULL) {
		t->work.should_set_async_binder_thread_rt = false;
		t->work.already_set_async_binder_thread_rt = false;
	}

	if ((g_sysctl_async_binder_rt_switch == 1) && oneway &&
	    (tr != NULL && tr->code == BINDER_CODE_FIRST)) {
		if (t != NULL && from_thread->task != NULL &&
		    (strcmp(from_thread->task->comm, ASYNC_BINDER_RT_FROM_THREAD) == 0)) {
			count = get_descriptor_count();
			for (index = 0; index < count; index++) {
				if (check_descriptor(tr, get_descriptor_with_index(index),
										get_descriptor_size_with_index(index))) {
					t->work.should_set_async_binder_thread_rt = true;
					return;
				}
			}
		}
...
static void set_async_binder_rt(struct binder_transaction *t, struct binder_thread *to_thread) {
	struct sched_param params;
	if (t && to_thread && to_thread->task && t->work.should_set_async_binder_thread_rt) {
		if (!to_thread->task->need_next_recovery) {
			to_thread->task->need_next_recovery = true;
			params.sched_priority = 2;
			sched_setscheduler_nocheck(to_thread->task, SCHED_FIFO | SCHED_RESET_ON_FORK, &params);
		}
		t->work.already_set_async_binder_thread_rt = true;
	}
}
#endif
...
static void update_binder_work_params(struct binder_proc *proc,
				      struct binder_work *work, int qos,
				      kuid_t sender_euid)
{
...
static struct binder_thread *
binder_select_thread_by_qos_ilocked(struct binder_proc *proc, int task_qos,
				    bool oneway, struct binder_transaction *t,
				    int attr)
{
	if ((!atomic_read(&binder_sched_switch)) || (!proc->is_system_server) ||
	...
	if (task_qos == VALUE_QOS_LOW)
		return binder_select_thread_by_type_ilocked(
			proc, BINDER_LOOPER_STATE_BG);
	else
		return binder_select_thread_ilocked(proc);
}
...
static int get_qos_and_update_binder_work_params(struct binder_proc *proc,
						 struct binder_transaction *t)
{
	...
	if (atomic_read(&binder_sched_switch) && (qos == VALUE_QOS_LOW))
		update_binder_work_params(proc, &t->work, qos, t->sender_euid);
	check_timeout(proc);
	return qos;
}

static struct list_head *
binder_sched_select_worklist_ilocked(struct binder_proc *proc)
{
#ifdef CONFIG_HN_BINDER_FG_REQ_FIRST
	if (!binder_worklist_empty_ilocked(&proc->fg_todo))
		return &proc->fg_todo;
...
static void trace_bg_enqueue(struct binder_transaction *t, int bg_count)
{
	int is_main_thread = 0;

	if (t->from && t->from->proc && (t->from->proc->pid == t->from->pid))
		is_main_thread = 1;
	pr_info("binder_sched bg enq uid:%d,count:%d,mainThread:%d\n",
		t->work.sender_euid, bg_count, is_main_thread);
}

static bool binder_from_bg_thread(struct binder_transaction *trans, int task_qos)
{
	char task_cpuset[TASK_COMM_LEN] = {0};
	char task_cpuctl[TASK_COMM_LEN] = {0};
	struct cgroup_subsys_state *css_set;
	struct cgroup_subsys_state *css_ctl;
	struct task_struct * task = NULL;

	task = trans->from->task;

	if (task) {
		css_set = task_get_css(task, cpuset_cgrp_id);
		css_ctl = task_get_css(task, cpu_cgrp_id);
		kernfs_name(css_set->cgroup->kn, task_cpuset, TASK_COMM_LEN);
		kernfs_name(css_ctl->cgroup->kn, task_cpuctl, TASK_COMM_LEN);
		css_put(css_set);
		css_put(css_ctl);

		if (!strncmp(task_cpuctl, "background", TASK_COMM_LEN) || !strncmp(task_cpuctl, "system-background", TASK_COMM_LEN) ||
			!strncmp(task_cpuset, "background", TASK_COMM_LEN) || !strncmp(task_cpuset, "system-background", TASK_COMM_LEN)) {
			return true;
		} else if (task_qos == VALUE_QOS_LOW) {
			pr_err("-%s- QQOS_DEBUG entend to set task:%s pid:%d binder to bg_todo with cpuset:%s cpuctl:%s.\n",
					__func__, task->comm, task->pid, task_cpuset, task_cpuctl);
			return false;
		}
	}
...
static void binder_sched_enqueue_work_ilocked(struct binder_proc *proc,
					      struct binder_transaction *t,
					      int task_qos, bool oneway,
					      int attr)
{
	if (atomic_read(&binder_sched_switch) && proc->is_system_server &&
	    (task_qos == VALUE_QOS_LOW) &&
	    (!is_uncontrolled_list(t, t->work.sender_euid, attr)) &&
	    (binder_from_bg_thread(t, task_qos))) {
...
#ifdef CONFIG_BINDER_HONOR_TRANSACTION_BRIEF
			"undelivered transaction %d, to proc %d, %s\n",
			t->debug_id, (t->to_proc ? t->to_proc->pid : 0), reason);
#else
...
#ifdef CONFIG_BINDER_FD_LEAK_DEBUG
	if (file->f_flags & O_NODUP) {
		binder_debug(BINDER_DEBUG_OPEN_CLOSE,
					"%s:: BINDER_FD_TRANS file:0x%p, proc-pid:%d thread-pid:%d fd:%d tgt_proc_pid:%d.\n",
					__func__, file, proc->pid, thread->pid, fd, target_proc->pid);
	}
#endif
				    ...
				    struct binder_thread *thread)
{
	...
	if (thread) {
		...
		do_enqueue = do_enqueue || (is_vip && (!oneway) && binder_enable_fg_switch);
#endif


		if (do_enqueue) {
			/*
 			 * Use foreground todo list,
 			 * while current thread is a vip thread
 			 */
...
#else // CONFIG_HN_BINDER_FG_REQ_FIRST
#ifdef CONFIG_HN_BINDER_SCHED
 		binder_sched_enqueue_work_ilocked(proc, t, task_qos,
 			oneway, proc_attr);
#else
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		*/
		if ((proc->pid != target_proc->pid) &&
			...
			spear_binder_report(proc, target_proc))) {
			if (kernel_binder_cb_hook != NULL) {
				if (!(tr->flags & TF_ONE_WAY)) {
					kernel_binder_cb_hook(proc->pid, target_proc->pid);
				} else {
					if(kstate_is_async_call_exempted != NULL
				&& kstate_is_async_call_exempted(target_proc->pid))
						kernel_binder_cb_hook(-(proc->pid), target_proc->pid);
				}
			}
		}
	...
	if (!reply && !(tr->flags & TF_ONE_WAY)) {
		...
		t->async_from_pid = -1;
		t->async_from_tid = -1;
		t->alloc_buff_size = (u64)tr->data_size;
#endif
	}
	else {
		t->from = NULL;
...
#endif
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
		check_async_binder_rt(t, thread, tr, tr->flags & TF_ONE_WAY);
#endif
	}
...
#ifdef CONFIG_HONOR_DUBAI_COMMON
	c_uid = from_kuid(current_user_ns(), t->sender_euid);
	c_pid = proc->pid;
	s_uid = from_kuid(current_user_ns(), task_euid(target_proc->tsk));
	s_pid = target_proc->pid;
#endif
...
#ifdef CONFIG_BINDER_HONOR_TRANSACTION_BRIEF
		t->timestamp_binder = in_reply_to->timestamp_binder;
#endif
		binder_pop_transaction_ilocked(target_thread, in_reply_to);
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
		t->work.should_filter = true;
#endif
...
#ifdef CONFIG_BINDER_HONOR_TRANSACTION_BRIEF
		t->timestamp_binder = binder_clock();
#endif
		binder_inner_proc_unlock(proc);
...
#ifdef CONFIG_HONOR_DUBAI_COMMON
		if (kernelBinderHook != NULL) {
			kernelBinderHook(reply, c_uid, c_pid, s_uid, s_pid);
		}
#endif
	} else {
...
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
static void binder_wakeup_proc_async_waiting_binder_ilocked(
		struct binder_proc *proc, struct binder_work *work)
{
	struct sched_param params;
	struct binder_thread *thread = binder_select_thread_ilocked(proc);

	if (thread && work->should_set_async_binder_thread_rt &&
		!work->already_set_async_binder_thread_rt) {
		params.sched_priority = 2;
		sched_setscheduler_nocheck(thread->task, SCHED_FIFO | SCHED_RESET_ON_FORK, &params);
		work->already_set_async_binder_thread_rt = true;
		thread->task->need_next_recovery = true;
	}
	...
	if (buffer->async_transaction && buffer->target_node) {
		...
		if (!w) {
			...
			binder_enqueue_work_ilocked(
					w, &proc->todo);
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
			binder_wakeup_proc_async_waiting_binder_ilocked(proc, w);
#else
			binder_wakeup_proc_ilocked(proc);
#endif
		}
...
#ifdef CONFIG_BINDER_FD_LEAK_DEBUG
		if (fixup->file->f_flags & O_NODUP) {
			binder_debug(BINDER_DEBUG_OPEN_CLOSE, "%s:: BINDER_FD_TRANS file::0x%p, proc-pid::%d fd::%d current::%s current-pid::%d.\n",
					__func__, fixup->file, proc->pid, fd, current->comm, current->pid);
		}
#endif
		fixup->file = NULL;
...
#ifdef CONFIG_HN_SET_ASYNC_BINDER_RT
		if (thread && thread->task && !thread->task->need_next_recovery) {
#endif
	...
	while (1) {
		...
		if (!binder_worklist_empty_ilocked(&thread->todo))
			list = &thread->todo;
			 ...
			 wait_for_proc_work)
			list = binder_sched_select_worklist_ilocked(proc);
#endif
#ifdef CONFIG_HN_BINDER_FG_REQ_FIRST
...
skip:
		...
		if (thread && thread->task) {
			// binder thread already RT
			if (thread->task->need_next_recovery) {
				// need to resume
				if (!w->should_set_async_binder_thread_rt && !w->should_filter) {
					thread->task->need_next_recovery = false;
					binder_restore_priority(thread, &proc->default_priority);
				}
			} else {
			// binder thread is not RT and we want it become RT
				if (w->should_set_async_binder_thread_rt &&
					!w->already_set_async_binder_thread_rt) {
					params.sched_priority = 2;
					sched_setscheduler_nocheck(thread->task, SCHED_FIFO | SCHED_RESET_ON_FORK, &params);
					w->already_set_async_binder_thread_rt = true;
					thread->task->need_next_recovery = true;
				}
			}
		}
...
#ifndef CONFIG_BINDER_FD_LEAK_DEBUG
	binder_debug(BINDER_DEBUG_OPEN_CLOSE, "%s: %d:%d\n", __func__,
		     current->group_leader->pid, current->pid);
#else
	binder_debug(BINDER_DEBUG_OPEN_CLOSE, "%s:: file::0x%p, proc::%s proc-pid::%d current::%s current-pid::%d inode::%s\n",
				__func__, filp, current->group_leader->comm, current->group_leader->pid,
				current->comm, current->pid, filp->f_path.dentry->d_iname);
#endif
...
#ifdef CONFIG_BINDER_FD_LEAK_DEBUG
	binder_debug(BINDER_DEBUG_OPEN_CLOSE, "%s:: BINDER_FD file::0x%p, proc-pid::%d private_data::0x%p, f_count::%d.\n",
			__func__,  filp, proc->pid, filp->private_data, filp->f_count);
	if (unlikely(proc->tsk->mm && (proc->tsk->mm != current->mm))) {
		pr_err("%s : binder_flush in wrong context, file:0x%p.\n", __func__, filp);
		dump_stack();
	}
#endif
...
#ifdef CONFIG_BINDER_FD_LEAK_DEBUG
	binder_debug(BINDER_DEBUG_OPEN_CLOSE,
			"%s: file:0x%p, flags:0%o, O_NODUP:0%o, proc-pid:%d private_data:0x%p, f_count:%d.\n",
			__func__, filp, filp->f_flags, filp->f_flags&O_NODUP, proc->pid, proc, filp->f_count);

	pr_err("%s : grp_ldr pid:%d grp_ldr comm:%s rl_parent pid:%d rl_parnet comm:%s parent pid:%d parent comm:%s file:0x%p.\n",
			__func__, current->group_leader->pid, current->group_leader->comm,
			current->real_parent->pid, current->real_parent->comm,
			current->parent->pid, current->parent->comm, filp);

	if ((proc->tsk->mm == NULL) || (proc->tsk->mm && (proc->tsk->mm != current->mm))) {
		pr_err("%s : binder_release in wrong context, file:0x%p.\n", __func__, filp);
		dump_stack();
	}
#endif
			      ...
			      struct binder_proc *proc, int print_all)
{
	...
	list_for_each_entry (w, &proc->fg_todo, entry)
		print_binder_work_ilocked(
			m, proc, "  ", "  pending foreground transaction", w);
#endif
				...
				u64 timestamp_binder)
{
	...
	seq_printf(m,
		   "%s%d:%d to %d:%d code %x wait:%lld.%lld s size:%lld\n",
		   prefix,
		   from_pid, from_tid,
		   to_pid, t->to_thread ? t->to_thread->pid : 0,
		   t->code,
		   timestamp_binder > t->timestamp_binder ? (timestamp_binder - t->timestamp_binder) / 1000000000 : 0,
		   timestamp_binder > t->timestamp_binder ? (timestamp_binder - t->timestamp_binder) % 1000000000 : 0,
		   t->alloc_buff_size);
	spin_unlock(&t->lock);
}
				...
				u64 timestamp_binder)
{
	...
	switch (w->type) {
		case BINDER_WORK_TRANSACTION:
			t = container_of(w, struct binder_transaction, work);
			print_binder_transaction_brief_ilocked(m, prefix, t, timestamp_binder);
			break;
		default:
			break;
	}
				...
				u64 timestamp_binder)
{
	...
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n)) {
		...
		while (t) {
			if (t->from == thread) {
				print_binder_transaction_brief_ilocked(m, "\t", t, timestamp_binder);
				t = t->from_parent;
			} else if (t->to_thread == thread) {
				t = t->to_parent;
			} else {
	...
	for (n = rb_first(&proc->nodes); n != NULL; n = rb_next(n)) {
		...
		binder_node_inner_lock(node);
		list_for_each_entry(w, &node->async_todo, entry)
			print_binder_work_transaction_nilocked(m, "async\t", w, timestamp_binder);
		binder_node_inner_unlock(node);
		last_node = node;
		binder_inner_proc_lock(proc);
	}
				...
				struct binder_proc *proc)
{
	...
	size_t free_buff_space =  binder_alloc_get_free_space(&proc->alloc);
...
int binder_transaction_proc_show(struct seq_file *m, void *unused)
{
	struct binder_proc *proc = NULL;
	u64 now = 0;
	mutex_lock(&binder_procs_lock);
	...
	seq_printf(m, "\npid\tcontext\t\trequest\tstarted\tmax\tready\tfree_async_space\tfree_all_sapce\n");
	hlist_for_each_entry(proc, &binder_procs, proc_node)
...
static int __init binder_init(void)
{
	...
	if (binder_debugfs_dir_entry_root) {
		...
		debugfs_create_file("count", S_IRUGO,
				binder_debugfs_dir_entry_root, NULL,
				&binder_count_fops);
		debugfs_create_file("switch", S_IRUGO,
				binder_debugfs_dir_entry_root, NULL,
				&binder_switch_fops);
#endif
	}
