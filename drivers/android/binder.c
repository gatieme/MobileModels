...
#ifdef CONFIG_FRAME_RTG
static const char *SURFACEFLINGER_NAME = "surfaceflinger";
#endif
...
#define BINDER_SCHED_QOS_GT                    0x01
#define BINDER_SCHED_QOS_EQ                    0x02
...
static uint32_t binder_dfx_mask;
module_param_named(dfx_mask, binder_dfx_mask, uint, 0644);

#define binder_debug(mask, x...) \
	do { \
		if (binder_debug_mask & mask || \
			unlikely(binder_dfx_mask & mask && current->tgid == get_noticed_tgid())) \
...
static bool sched_latency_satisfy(struct task_struct *task,
				  struct binder_priority desired,
				  int flag)
{
	int ori_offset = task->se.latency_offset;
	int des_offset = desired.latency_offset;

	if (flag & BINDER_SCHED_QOS_GT)
		return ori_offset > des_offset;
	if (flag & BINDER_SCHED_QOS_EQ)
		return ori_offset == des_offset;
	return false;
}
#else
static inline bool sched_latency_satisfy(struct task_struct *task,
				  struct binder_priority desired,
				  int flag)
{
	return true;
...
static int binder_count_open(struct inode *inode, struct file *file)
{
	return single_open(file, binder_count_show, inode->i_private);
}

static const struct file_operations binder_count_fops = {
	.owner = THIS_MODULE,
	.open = binder_count_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
{
	if (from && test_task_vip(from->task) &&
		test_task_vip_depth(from->task->vip_depth) &&
		!test_task_vip(thread->task) && binder_enable_fg_switch)
		dynamic_vip_enqueue(thread->task, DYNAMIC_VIP_BINDER,
			from->task->vip_depth);
}

static inline void binder_thread_check_and_remove_dynamic_vip(
	struct binder_thread *thread)
{
	if (test_dynamic_vip(thread->task, DYNAMIC_VIP_BINDER))
		dynamic_vip_dequeue(thread->task, DYNAMIC_VIP_BINDER);
}
#endif
#endif
	...
	unsigned int flags)
{
	...
	if (!(flags & TF_ONE_WAY))
		return dynamic_qos_enqueue(thread->task, from->task,
			DYNAMIC_QOS_BINDER);
	else if (flags & TF_FORCE_TRANS) /* async binder trans */
		return dynamic_qos_async_binder(thread->task, from->task);
	else
		return false;
}
...
static void binder_inherit_vip_prio(struct binder_thread *thread,
				    struct binder_thread *from,
				    unsigned int oneway)
{
	if (!oneway && from)
		request_vip_boost(thread->task,
				  task_vip_pi_prio(from->task),
				  VIP_BOOST_BINDER);
}

static void binder_clear_vip_prio(struct binder_thread *thread,
				  unsigned int oneway)
{
	if (!oneway)
		clear_vip_boost(thread->task, VIP_BOOST_BINDER);
}
#endif
...
static void check_qos_low_to_critical(struct binder_thread *thread,
	struct binder_thread *from, unsigned int flags,
	struct binder_proc *proc)
{
	...
	if (atomic_read(&binder_sched_switch) && cred &&
		(!proc->is_system_server) && thread->task) {
		cur_uid = from_kuid(current_user_ns(), cred->euid);
	...
	is_binder_trans_vertical =
		binder_thread_check_and_set_dynamic_qos(thread, from, flags);
#endif
	...
	uid_t sender_euid, int attr)
{
	if ((!t->from) || (!t->from->proc))
		return true;
	return ((t->from->proc->pid == t->from->pid) || (sender_euid == fg_euid) ||
...
#ifdef CONFIG_HW_QOS_THREAD
static void binder_wakeup_thread_trans_ilocked(struct binder_proc *proc,
					struct binder_thread *thread,
					struct binder_thread *from,
					int flags)
{
	bool sync = !(flags & TF_ONE_WAY);
	struct binder_thread *to_thread = thread;
	struct rb_node *n;
	...
	if (to_thread) {
		if (sync) {
			wake_up_interruptible_sync(&to_thread->wait);
		} else {
			binder_thread_check_and_set_dynamic_qos(to_thread, from, flags);
	...
	for (n = rb_first(&proc->threads); n != NULL; n = rb_next(n)) {
		to_thread = rb_entry(n, struct binder_thread, rb_node);
		if ((to_thread->looper & BINDER_LOOPER_STATE_POLL) &&
			binder_available_for_proc_work_ilocked(to_thread)) {
			if (sync) {
				wake_up_interruptible_sync(&to_thread->wait);
			} else {
				binder_thread_check_and_set_dynamic_qos(to_thread, from, flags);
...
static int to_userspace_prio(int policy, int kernel_priority)
{
	if (is_fair_policy(policy))
		...
		return vip_spare_unmap_rt_priority(MAX_USER_RT_PRIO - 1 - kernel_priority);
#else
...
#ifdef CONFIG_RT_PRIO_EXTEND_VIP
		return MAX_USER_RT_PRIO - 1 - vip_spare_map_rt_priority(user_priority);
#else
				   ...
				   bool verify)
{
	...
	if (task->policy == policy && task->normal_prio == desired.prio)
		goto check_latency_nice;

#ifdef CONFIG_SCHED_VIP_PI
	if (is_vip_prio(desired.prio) || is_vip_prio(task->normal_prio))
		return;
#endif
#ifdef CONFIG_FRAME_RTG
	if (is_special_rtg(task))
		return;
	if ((task->normal_prio < desired.prio) && (!strcmp(task->comm, SURFACEFLINGER_NAME)))
		return;
#endif

	/* Do not set lower priority to task in binder_set_priority(). */
	if (verify && task->normal_prio <= desired.prio)
		goto check_latency_nice;
...
check_latency_nice: __maybe_unused;
#ifdef CONFIG_SCHED_LATENCY_NICE
	if (is_fair_policy(task->policy) && !sched_latency_satisfy(task, desired, BINDER_SCHED_QOS_EQ)) {
		int desired_latency_nice = (int)sched_latency_offset_to_nice(desired.latency_offset);
		if (sched_latency_satisfy(task, desired, BINDER_SCHED_QOS_GT) || !verify) {
			trace_binder_set_latency(task->tgid, task->pid,
						 task->latency_prio,
						 LATENCY_TO_NICE(task->latency_prio),
						 desired_latency_nice, verify);

			struct sched_attr attr = {
				.sched_flags = SCHED_FLAG_LATENCY_NICE | SCHED_FLAG_KEEP_PARAMS,
				.sched_latency_nice = desired_latency_nice,
...
#ifdef CONFIG_SCHED_LATENCY_NICE
	t->saved_priority.latency_offset = task->se.latency_offset;
#endif
				    ...
				    struct binder_thread *thread)
{
...
#ifdef CONFIG_HW_QOS_THREAD
#ifdef CONFIG_HW_BINDER_SCHED
		check_qos_low_to_critical(thread, t->from, t->flags, proc);
#else
...
#endif
#ifdef CONFIG_SCHED_VIP_PI
		binder_inherit_vip_prio(thread, t->from, oneway);
#endif
	...
	if (!pending_async) {
#ifdef CONFIG_HW_QOS_THREAD
		binder_wakeup_thread_trans_ilocked(proc, thread, t->async_from, t->flags);
#else
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		target_proc->tmp_ref++;
#ifdef CONFIG_HUAWEI_KSTATE
		const struct cred *cred = binder_get_cred(target_proc);
		if (((cred != NULL) && (cred->euid.val <= 2000)) &&
		...
		/*
		* 1.target process is eternal-frozen
		* 2.called uid > 2000
		* 3.uid not same
		* 4.called process not foreground
		* 5.All above conditions satisfied, return dead binder
		*/
		if (target_proc->tsk->eternal_frozen == 1) {
			if ((proc->tsk->cred->euid.val > 2000) &&
				(proc->tsk->cred->euid.val != target_proc->tsk->cred->euid.val) &&
				(proc->tsk->signal->oom_score_adj > 0)) {
				pr_info("pid %d is eternally frozen\n", target_proc->pid);
				return_error = BR_DEAD_REPLY;
				return_error_line = __LINE__;
					goto err_dead_binder;
			}
...
#ifdef CONFIG_SCHED_LATENCY_NICE
		t->priority.latency_offset = tg_latency_offset(current);
#endif
	} else {
 ...
 /*lint +e613*/
	...
	if (reply) {
#if defined(CONFIG_HW_QOS_THREAD) || defined(CONFIG_SCHED_VIP_PI)
...
#endif
#ifdef CONFIG_SCHED_VIP_PI
			binder_clear_vip_prio(thread, oneway_flag);
#endif
...
#ifdef CONFIG_POWER_DUBAI
		dubai_log_binder_stats(reply, c_uid, c_pid, s_uid, s_pid);
#endif
	} else {
				...
				struct binder_thread *thread)
{
	...
	if (!ret) {
		binder_inner_proc_lock(target_proc);
		is_dead = target_proc->is_dead;
		binder_inner_proc_unlock(target_proc);
		if (is_dead)
			binder_update_ref_for_handle(target_proc, dest_rdata.desc, false, true, &dest_rdata);
	}
	return ret;
}
				...
				bool do_proc_work)
{
	...
	for (;;) {
		...
		if ((bt != NULL) && (bt->to_proc != NULL)) {
			const struct cred *cred = binder_get_cred(bt->to_proc);
...
#ifdef CONFIG_HW_QOS_THREAD
		dynamic_qos_deque_async_binder(current);
#endif
		if (!(thread->looper & (BINDER_LOOPER_STATE_REGISTERED |
					BINDER_LOOPER_STATE_ENTERED))) {
...
#endif
#ifdef CONFIG_SCHED_VIP_PI
			binder_inherit_vip_prio(thread, t_from, t->flags & TF_ONE_WAY);
#endif
...
static int binder_open(struct inode *nodp, struct file *filp)
{
	...
	if (binder_supported_policy(current->policy)) {
		...
		get_raw_kernel_priority(current,
		...
		/*
		 * The current thread may have joined RTG before this,
		 * and the binder thread is not expected to inherit
		 * the real-time priority here
		 */
		if (current->rtg_depth != 0) {
			pr_info("%s: reset default_priority for binder threads, current=%d(%s)-%d\n",
				__func__, current->pid, current->comm, current->rtg_depth);
			proc->default_priority.sched_policy = SCHED_NORMAL;
			proc->default_priority.prio = current->static_prio;
		}
#endif
#ifdef CONFIG_SCHED_LATENCY_NICE
		proc->default_priority.latency_offset = current->se.latency_offset;
#endif
	} else {
		proc->default_priority.sched_policy = SCHED_NORMAL;
		proc->default_priority.prio = NICE_TO_PRIO(0);
#ifdef CONFIG_SCHED_LATENCY_NICE
		proc->default_priority.latency_offset = 0;
#endif
	}
...
#ifdef CONFIG_DFX_BINDER
	if (task_need_noticed(current->group_leader))
		pr_err_ratelimited("binder_proc_check: proc = %d %p\n", proc->pid, proc);
	current->group_leader->binder_pid = proc->pid;
#endif
...
#ifdef CONFIG_DFX_BINDER
static bool binder_find_proc_by_pid(pid_t pid)
{
	bool found = false;
	struct binder_proc *proc = NULL;
	...
	hlist_for_each_entry(proc, &binder_procs, proc_node) {
		if (proc->pid != pid)
			continue;
		found = true;
		pr_err("binder_proc_check: %d's binder_proc %p still exist\n", pid, proc);
	}
	mutex_unlock(&binder_procs_lock);
...
#ifdef CONFIG_DFX_BINDER
	if (task_need_noticed(current->group_leader)) {
		pr_err("binder_proc_check: start check pid %d\n", proc->pid);
		if (binder_find_proc_by_pid(proc->pid))
			binder_print_related_task(proc->pid);
	}

	pr_err_ratelimited("%d defer release binder_proc\n", proc->pid);
#endif
...
#ifdef CONFIG_DFX_BINDER
	pr_err_ratelimited("%d start release binder_proc\n", proc->pid);
#endif
	binder_debug(BINDER_DEBUG_OPEN_CLOSE,
...
#ifdef CONFIG_BINDER_FILE_DEBUG_LOG
static bool is_binder_file(struct file *filp)
{
	if (IS_ERR(filp))
		return false;
	...
	if (filp->f_op != &binder_fops)
		return false;

	return true;
}

void binder_file_dup_debug(int fd, struct file *filp, const char *str)
{
	if (!is_binder_file(filp))
		return;

	printk("### : binder file error : fd %d : %s!\n", fd, str);
	return;
}
#endif
