				...
				int pid)
{
	...
	if (is_async && (alloc->free_async_space < 3*(size +
		sizeof(struct binder_buffer)) ||
			alloc->free_async_space < 100*1024)) { //100k
		if (kernel_binder_cb_hook != NULL) {
			binder_alloc_debug(BINDER_DEBUG_USER_ERROR,
					"%d async binder %d \n", pid, alloc->pid);
			kernel_binder_cb_hook(-1, alloc->pid); // -1 standby async binder call
		}
	}
#endif
	if (is_async &&
...
size_t binder_alloc_get_free_space(struct binder_alloc *alloc)
{
	...
	mutex_lock(&alloc->mutex);
	node = alloc->free_buffers.rb_node;
	for (node = rb_first(&alloc->free_buffers); node != NULL;
		node = rb_next(node)) {
		buffer = rb_entry(node, struct binder_buffer, rb_node);
		buffer_size = binder_alloc_buffer_size(alloc, buffer);
		total_free_size += buffer_size;
		if (buffer_size > largest_free_size)
			largest_free_size = buffer_size;
