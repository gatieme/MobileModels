...
struct io_extent {
	...
	struct page *pages[PKG_EXTENT_PG_CNT];
				  ...
				  bool fast, bool nofail)
{
	...
	io_ext->pool = pool;
	for (i = 0; i < (int)PKG_EXTENT_PG_CNT; i++) {
...
static void discard_io_extent(struct io_extent *io_ext, unsigned int op)
{
	...
	hyperhold_free_pkg_extent(zram->area, io_ext->ext_id, true);
out:
	hyperhold_free(io_ext);
}
		   ...
		   unsigned long eswpentry, int size)
{
	...
	if (pg_id == PKG_EXTENT_PG_CNT - 1) {
		hh_print(HHLOG_ERR, "ext overflow, addr = %llx, size = %d pg_id = %d offset = %d\n",
			 eswpentry, size, pg_id, offset);
		return;
	}
...
#endif
	if (ext_id != esentry_pkg_extid(handle))
		return true;
		       ...
		       unsigned long eswpentry, struct mem_cgroup *mcg)
{
	...
	atomic_inc(&zram->area->ext_stored_pages[esentry_pkg_extid(eswpentry)]);
	atomic64_add(size, &zram->area->ext_stored_size[esentry_pkg_extid(eswpentry)]);
...
static void extent_unlock(struct io_extent *io_ext)
{
	...
	eswpentry = (((unsigned long)ext_id & PKG_EXTENT_OFFSET_MASK) << PKG_EXTENT_OFFSET_SHIFT) |
		(((unsigned long)ext_id) << EXTENT_SHIFT);
#else
		 ...
		 unsigned long swap_size)
{
	...
	u8 *src = NULL;
	struct hyperhold_stat *stat = hyperhold_get_stat_obj();
	unsigned long ext_off = swap_size;

#ifdef CONFIG_HP_TURBO
	ext_off += (((unsigned long)io_ext->ext_id & PKG_EXTENT_OFFSET_MASK) << PKG_EXTENT_OFFSET_SHIFT) |
		(((unsigned long)io_ext->ext_id) << EXTENT_SHIFT);
#endif
	...
	size = zram_get_obj_size(zram, index);
	if (swap_size + size > PKG_EXTENT_SIZE) {
		zram_slot_unlock(zram, index);
		return -ENOSPC;
	}
...
static int shrink_entry_list(struct io_extent *io_ext)
{
	...
	if (!mcg->active_swapout &&
		!mcg->force_swapout && stored_size < PKG_EXTENT_SIZE)
#else
	if (!mcg->force_swapout && stored_size < PKG_EXTENT_SIZE)
#endif
		return -ENOENT;
	if (!stored_size)
		return -ENOENT;
	...
	if (!swap_index)
		return -ENOMEM;
	io_ext->ext_id = hyperhold_alloc_pkg_extent(zram->area, mcg);
	if (io_ext->ext_id < 0) {
		kfree(swap_index);
	...
	swap_cnt = zram_get_memcg_coldest_index(zram->area, mcg, swap_index,
						EXTENT_MAX_OBJ_CNT);
	io_ext->cnt = 0;
	for (k = 0; k < swap_cnt && swap_size < (int)PKG_EXTENT_SIZE; k++) {
	...
	hh_print(HHLOG_DEBUG, "fill extent = %d, cnt = %d, overhead = %ld.\n",
		 io_ext->ext_id, io_ext->cnt, PKG_EXTENT_SIZE - swap_size);
	if (swap_size == 0) {
...
bool hyperhold_cache_objs_del(struct zram *zram, u32 index,
	unsigned long handle, int ext_id)
{
	struct mem_cgroup *mcg = NULL;
	int ret = 0;
	...
	if (zram_test_overwrite(zram, index, ext_id)) {
		zs_free(zram->mem_pool, handle);
		return false;
	}
		...
		unsigned long eswpentry, bool unpin)
{
	...
	unsigned long addr = zram_get_handle(zram, index);
	size_t size = zram_get_obj_size(zram, index);
	int ext_id = esentry_pkg_extid(addr);

	if (!zram_test_flag(zram, index, ZRAM_WB)) {
		if (unpin)
			hyperhold_cache_unpin(zram->area->cache,
					esentry_pkg_extid(eswpentry));
		return -ENOENT;
	}
	...
	zram_slot_lock(zram, index);
	if (!hyperhold_cache_objs_del(zram, index, handle, ext_id))
		return 0;
	...
	hyperhold_free_pkg_extent(zram->area, ext_id, true);
...
#else
	if (atomic_read(&area->ext_stored_size[ext_id]) > CACHE_SIZE_HIGH)
#endif
		return false;
...
{
	unsigned long nr_cached = hyperhold_cache_pages(zram);
...
unsigned long hyperhold_cache_pages(struct zram *zram)
{
	...
	return zram->area->cache->nr_cached_exts * PKG_EXTENT_PG_CNT;
}
	...
	while (1) {
		...
		if (last_index == index) {
			hh_print(HHLOG_ERR, "dup index %d, mcg[%s] head %lx\n",
					index, (strlen(mcg->name) != 0) ? mcg->name : "null",
					mcg->zram_lru);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
...
void hyperhold_extent_objs_del(struct zram *zram, u32 index)
{
	...
	atomic64_sub(size,
		&zram->area->ext_stored_size[esentry_pkg_extid(eswpentry)]);
	if (!atomic_dec_and_test(
		&zram->area->ext_stored_pages[esentry_pkg_extid(eswpentry)]))
		return;
	ext_id = get_extent(zram->area, esentry_pkg_extid(eswpentry));
	if (ext_id < 0)
		return;
