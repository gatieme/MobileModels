...
static void hyperhold_get_file_index(unsigned int *exts_sum, unsigned int ext_id,
	unsigned int file_num, unsigned int *file_index, unsigned int *file_offset)
{
	unsigned int start = 0;
	unsigned int end = file_num - 1;
	unsigned int middle;
...
static sector_t hyperhold_get_file_sector(int ext_id)
{
	...
	if (unlikely(!space_info->start_sector ||
		...
		hh_print(HHLOG_ERR, "start_sector is err, file_index %u\n",
			file_index);
		return 0;
	}
...
bool hyperhold_space_is_full(int add_num)
{
	...
	is_full = ((space_info->used_exts + add_num) > space_info->alloced_exts);
...
static void hyperhold_save_space_setting(struct blk_hp_set_space *space_setting)
{
	...
	if (space_para->space_type == HP_FILE_SPACE) {
		...
		space_info->exts_sum = vzalloc(space_info->max_file_num * sizeof(unsigned int));
	...
	hh_print(HHLOG_ERR, "max_file_num %u min_file_num %u alloced_file_num %u\n",
		space_info->max_file_num, space_info->min_file_num, space_info->alloced_file_num);
	hh_print(HHLOG_ERR, "max_pages %u max_exts %u\n",
