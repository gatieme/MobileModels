...
#endif
{
	...
	if(mcg && mcg->mcg){
		hh_print(HHLOG_DEBUG, "HHGKI: mcgid:%d,extid:%d,size:%d,stored_size:%d", mcg->mcg->id.id,
				mcg->mcg_ext_id, size, atomic64_read(&mcg->hyperhold_stored_size));
	}
#endif
}
...
#ifdef CONFIG_HYPERHOLD_GKI_DEBUG
	if(mcg && mcg->mcg){
		hh_print(HHLOG_DEBUG, "HHGKI: mcgid:%d,extid:%d,size:%d,stored_size:%d\n", mcg->mcg->id.id,
				mcg->mcg_ext_id, size, atomic64_read(&mcg->hyperhold_stored_size));
	} else {
		hh_print(HHLOG_DEBUG,"mcg = null\n");
	}
#endif
}
				 ...
				 void **private)
{
	...
	io_ext->mcg = get_mem_cgroup(
				hh_list_get_mcgid(ext_idx(zram->area, ext_id),
						  zram->area->ext_table));
#ifdef CONFIG_HYPERHOLD_GKI
	if (io_ext->mcg && !css_tryget(&io_ext->mcg->mcg->css))
#else
