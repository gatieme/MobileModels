	...
	bool perf_end_flag)
{
	...
	count += snprintf(log + count,
		(size_t)(DUMP_BUF_LEN - count),
		"totaltime(us) %lld scenario %u task %s nice %d tgid %d pid %d",
		total_time, record->scenario, record->task_comm, record->nice,
		record->tgid, record->pid);
...
static void hyperhold_perf_warning(unsigned long data)
{
	...
	if (likely(record->task))
		show_stack(record->task, NULL, KERN_ERR);
#endif
...
#else
	setup_timer(&record->lat_monitor, hyperhold_perf_warning,
			(uintptr_t)record);
#endif
	mod_timer(&record->lat_monitor,
			jiffies +
	...
	struct hyperhold_key_point_record *record)
{
	...
	/* Donot record max_latency of page faults for low-priority tasks */
	if (record->scenario == HYPERHOLD_FAULT_OUT && record->nice > 0)
		return;
