...
static struct blk_crypto_key blk_key;
#endif
#endif
...
#ifndef CONFIG_MAS_BLK
	err = blk_crypto_init_key(&blk_key, hyperhold_io_key, HYPERHOLD_KEY_SIZE,
				  false, BLK_ENCRYPTION_MODE_AES_256_XTS,
				  DIV_ROUND_UP(lblk_bits, 8), UFS_SECTOR_SIZE);
	if (err) {
		hh_print(HHLOG_ERR, "Error initializing blk-crypto key");
		return;
	}
#endif
}
...
static void hyperhold_bio_encryption_proc(struct bio *bio)
{
	...
	dun[0] = bio->bi_iter.bi_sector >> HYPERHOLD_KEY_INDEX_SHIFT;
	bio_crypt_set_ctx(bio, &blk_key, dun, GFP_NOIO);
#endif
#else
	bio->mas_bio.ci_key_index = HYPERHOLD_KEY_INDEX;
	bio->mas_bio.ci_key_len = HYPERHOLD_KEY_SIZE;
	bio->mas_bio.ci_key = (void *)hyperhold_io_key;
	bio->mas_bio.index =
		bio->bi_iter.bi_sector >> HYPERHOLD_KEY_INDEX_SHIFT;
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	bio->mas_bio.metadata = hyperhold_io_metadata;
#endif
#endif
}
#endif
...
static void hyperhold_wait_io_finish(struct hyperhold_io_req *req)
{
	...
	if (req->io_para.scenario == HYPERHOLD_FAULT_OUT) {
		...
		wait_for_completion_timeout(&req->io_end_flag, MAX_SCHEDULE_TIMEOUT);
#else
...
static void hyperhold_io_end_work(struct work_struct *work)
{
	...
	/* Do not lower priority */
	if (old_nice > req_nice)
