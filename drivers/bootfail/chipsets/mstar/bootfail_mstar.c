 ...
 * Create: 2020-10-26
 */
#include <bootfail/chipsets/bootfail_mstar.h>
...
struct timer_list g_mstar_pwk_timer;

#define BOOTFAIL_LONGPRESS_COUNT_STEP 6000
#define BOOTFAIL_PWK_RECORD_ENABLE  0x454E4142
#define BOOTFAIL_PWK_RECORD_DISABLE 0x44495342

static DEFINE_MUTEX(g_bootfail_record_mutex);
...
void bootfail_pwk_press(void)
{
	...
	if (init_flag == 0) {
		timer_setup(&g_mstar_pwk_timer, bootfail_pwk_record, 0);
		init_flag++;
	}
	g_mstar_pwk_timer.expires = jiffies + delay_time;
	if (!timer_pending(&g_mstar_pwk_timer))
		add_timer(&g_mstar_pwk_timer);
#endif
}
EXPORT_SYMBOL(bootfail_pwk_press);

void bootfail_pwk_release(void)
{
#ifndef CONFIG_BOOT_DETECTOR_GKI
	del_timer(&g_mstar_pwk_timer);
#endif
}
EXPORT_SYMBOL(bootfail_pwk_release);
