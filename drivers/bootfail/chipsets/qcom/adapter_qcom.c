...
#ifdef CONFIG_ARM
	g_bf_rmem_addr = (struct rb_header *)ioremap_uc(addr, g_bf_rmem_size);
#else
...
{
#if IS_ENABLED(CONFIG_RAINBOW_REASON)
	rb_sreason_set("bootfail");
#endif
	// 8450 & 8550 not support warm reset, so use panic alternate
	panic("bootfail");
}
...
static void save_long_press_meta_log(struct every_number_info *pinfo)
{
	...
	pinfo->last_kmsg_size = get_kmsg_buf_size();
