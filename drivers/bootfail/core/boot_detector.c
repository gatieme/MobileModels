...
#include <bootfail/core/boot_recovery.h>
...
#define BOOTFAIL_INFO_OFFSET_IN_PHYMEM  sizeof(struct bfi_misc_msg)
	...
	int rw_storage_succ)
{
	...
	if (rw_storage_succ == 0 ||
		...
		run_param.adp.pyhs_mem_info.ops.write(
			run_param.adp.pyhs_mem_info.base + BOOTFAIL_INFO_OFFSET_IN_PHYMEM,
			run_param.adp.pyhs_mem_info.size,
			(char *)pparam, sizeof(*pparam));
		run_param.adp.print("bootfail: write to phys mem errno=%d\n", pparam->binfo.bootfail_errno);
	}
	msleep(200);
#if !defined(FACTORY_MODE_ENABLE) || defined(CONFIG_HLTHERM_RUNTEST)
...
static void read_bootfail_from_pyhs_mem(void)
{
	...
	run_param.adp.sec_funcs.memset_s(pparam,
		(unsigned long)sizeof(*pparam), 0,
		(unsigned long)sizeof(*pparam));
	run_param.adp.pyhs_mem_info.ops.read((unsigned long)(pparam),
		sizeof(*pparam), (void *)run_param.adp.pyhs_mem_info.base,
		run_param.adp.pyhs_mem_info.size);
...
int chipset_common_adapter_init(struct adapter *padp)
{
	if (padp == NULL) {
		padp->print("padp: %p\n", padp);
		return -1;
