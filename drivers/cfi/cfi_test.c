...
{
	if (*((uint32_t *)callee - 1) != CONFIG_HONOR_CFI_TAG) {
...
static const struct proc_ops testcall_fops = {
	.proc_open = nop_open,
	.proc_read = testcall_read,
	.proc_release = nop_release,
};

static const struct proc_ops testattack_fops = {
	.proc_open = nop_open,
	.proc_read = testattack_read,
	.proc_release = nop_release,
};
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
static const struct proc_ops proccfi_fops = {
	.proc_open  = proccfi_open,
	.proc_read  = proccfi_read,
	.proc_release = proccfi_release,
};
#else
