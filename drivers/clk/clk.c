...
static void clk_core_disable_lock(struct clk_core *core)
{
	...
	CALL_HCK_VH(clk_enable, core->hw->clk);
...
int clk_set_rate(struct clk *clk, unsigned long rate)
{
	...
	CALL_HCK_VH(clk_set_freq, clk, rate);
	if (clk->exclusive_count)
		clk_core_rate_protect(clk->core);
...
#ifdef CONFIG_ARCH_CLK_EXTREME
static int of_parse_clkspec(const struct device_node *np, int index,
			    const char *name, struct of_phandle_args *out_args)
{
	int ret = -ENOENT;
	struct of_phandle_args hisiclkspec;
	...
	while (np) {
		...
		ret = of_parse_phandle_with_args(hisiclkspec.np, "clocks", "#clock-cells",
							0, out_args);
		if (ret)
			return ERR_PTR(ret);
...
unsigned int clk_get_prepare_count(struct clk *clk)
{
	return !clk ? 0 : clk->core->prepare_count;
}
EXPORT_SYMBOL_GPL(clk_get_prepare_count);

int plat_core_prepare(struct clk *clk)
{
	if (!clk)
		return -ENAVAIL;
...
EXPORT_SYMBOL_GPL(plat_core_prepare);

void plat_core_unprepare(struct clk *clk)
{
	if (!clk)
		return;
...
EXPORT_SYMBOL_GPL(plat_core_unprepare);
