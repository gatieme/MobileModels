    ...
    const unsigned char *buffer, int count)
{
    ...
    while (count) {
        ...
        temp=skb_put(skb, len);
        memcpy_s(temp,skb->len,buffer,len);
        count -= len;
        buffer += len;
...
STATIC int hci_bt_bdaddr(struct hci_dev *hdev)
{
    ...
    for (i = 0; i < MAC_BYTE_LEN; i++) {
        bt_bdaddr.b[i] = device_mac_addr[MAC_BYTE_LEN - i - 1];
    }

    ini_warning("hci_bt_bdaddr get MAC from PLAT: mac= %02x:%s:%s:%s:%02x:%02x \n",
                    bt_bdaddr.b[0], "**", "**", "**", bt_bdaddr.b[0x4], bt_bdaddr.b[0x5]);
    /* send macaddr to device */
    for (i = 0; i < CHECK_MAX_CNT; i++) {
        skb = __hci_cmd_sync(hdev, HCI_SET_BD_ADDR, sizeof(bdaddr_t), &bt_bdaddr, HCI_INIT_TIMEOUT);
        if (!IS_ERR(skb)) {
            OAL_IO_PRINT("retry %d times, MACBT from plat send success to device\n", i);
            break;
        }
...
STATIC int hci_bt_send(struct hci_dev *hdev, struct sk_buff *skb, struct bt_data_combination *data_comb)
{
    ...
    mod_timer(&ps_core_d->ps_pm->pm_priv_data->bfg_timer, jiffies + msecs_to_jiffies(BT_SLEEP_TIME));
...
STATIC int hci_bt_recv_thread(void *data)
{
    ...
    for (;;) {
        ...
        ret = wait_event_interruptible(ps_core_d->bfgx_info[BFGX_BT].rx_wait,
