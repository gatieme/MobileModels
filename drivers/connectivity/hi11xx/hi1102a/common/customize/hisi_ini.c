...
int32 ini_find_var_value_by_path(int8 *path, int32 tag_index, int8 *puc_var, int8 *puc_value, uint32 size)
{
...
#else
        ini_debug("check if puc_var and puc_value is NULL or blank");
#endif
        return INI_FAILED;
    }
    ...
    value = oal_get_usec(&tv[1]) - oal_get_usec(&tv[0]);
    do_div(value, USEC_PER_MSEC);

    ini_debug("time take = %llu",
        (tv[1].tv_sec - tv[0].tv_sec) * MSEC_PER_SEC + value);
#endif
    ...
    if (ini_check_value(puc_value, strlen(puc_value)) == INI_SUCC) {
        /*    Hi110x bug fix liuwenlong/00244150 2013/11/20 begin */
        ini_debug("::return %s:%s::", puc_var, puc_value);
        /*    Hi110x bug fix liuwenlong/00244150 2013/11/20 end */
        return INI_SUCC;
    }
...
static uint32 ini_file_check_timespec(ini_file *fp)
{
    ...
    if (g_ini_file_time_sec != inf_file_get_ctime(fp->f_path.dentry)) {
        ini_info("ini_file time_secs changed from [%ld]to[%ld]\n",
                 g_ini_file_time_sec, (long)inf_file_get_ctime(fp->f_path.dentry));
        g_ini_file_time_sec = (long)inf_file_get_ctime(fp->f_path.dentry);
