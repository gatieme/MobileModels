...
uint8 device_mac_addr[DEVICE_MAC_ADDR_SIZE] = { 0x00, 0x22, 0x33, 0x00, 0x00, 0x00};
EXPORT_SYMBOL(device_mac_addr);
#endif
...
{
    g_board_info.flowctrl_irq = (uint32)gpio_to_irq(g_board_info.flowctrl_gpio);
}
...
int32 board_fm_lan_init(void)
{
    ...
    if (ret == INI_FAILED) {
#ifdef _PRE_WLAN_EXPORT
        ps_print_warning("[fm_lan]: fail to get support ini, default disabled\n");
#else
...
int32 board_irq_init(void)
{
    ...
    if (irq < 0) {
        ps_print_info("get wlan_gpio_irq numb fail!\n");
        return BOARD_FAIL;
    }
    g_board_info.wlan_irq = (uint32)irq;
    ...
    irq = gpio_to_irq(gpio);
    if (irq < 0) {
        ps_print_info("get bfgn_gpio_irq numb fail!\n");
        return BOARD_FAIL;
    }
    g_board_info.bfgx_irq = (uint32)irq;
...
#else
    const char *device_version = "hi1102a";
#endif
...
int32 board_func_init(void)
{
    ...
    if (ret != BOARD_SUCC) {
        /* 兼容1102A */
        g_board_info.chip_nr = BOARD_VERSION_HI1102A;
...
int32 gnss_timesync_event_init(void)
{
    ...
    irq = gpio_to_irq(gpio);
    if (irq < 0) {
        ps_print_info("get timesync_gpio numb fail!\n");
        return BOARD_FAIL;
    }
    bd_info->timesync_irq = (uint32)irq;
...
#define SSI_READ_RETTY 1000
    ...
    for (i = 0; i < SSI_DATA_LEN; i++) {
        ...
        data = data | (uint16)((uint16)rx << (SSI_DATA_LEN - i - 1));
    }
...
int32 ssi_read_value16(uint32 addr, uint16 *value, uint16 last_high_addr)
{
    int32 ret;
    uint16 addr_half_word_high;
    uint16 addr_half_word_low;
 ...
 */
int32 ssi_read_value32(uint32 addr, uint32 *value, uint16 last_high_addr)
{
    int32 ret;
    uint16 reg;
...
static int ssi_read_single_seg_reg(ssi_reg_info *pst_reg_info, void *buf, uint32_t buf_len,
                                   uint32_t seg_index, uint32_t seg_size)
{
    int32_t ret;
...
{
    uint32_t i, seg_size, seg_nums, left_size;
...
retry:
    ...
    seg_nums = (left_size - 1 / buf_len) + 1;
    ...
    for (i = 0; i < seg_nums; i++) {
        ...
        if (is_logfile) {
            write_size = (uint32)vfs_write(fp, buf, seg_size, &fp->f_pos);
                          ...
                          oal_int32 is_logfile)
{
    ...
    uint32_t realloc = 0;
    char filename[SSI_REGINFO_FILE_NAME_LEN];
