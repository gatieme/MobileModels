...
oal_int32 hcc_bus_bindcpu(hcc_bus *hi_bus, oal_uint32 chan, oal_int32 is_bind)
{
    ...
    if (oal_unlikely(hcc != NULL)) {
        if (hcc->hcc_transer_info.hcc_transfer_thread) {
            ...
            oal_print_hi11xx_log(HI11XX_LOG_INFO, "hisi_hcc bind to cpu 0x%lx",
                hi_bus->is_bind ? cpumask_bits(&fast_cpus)[0] : cpumask_bits(&all_cpus)[0]);
#endif
        }
    }
    ...
    if (hi_bus->pst_rx_tsk != NULL) {
        ...
        if (is_bind) {
            ...
            cpumask_clear_cpu(cpumask_first(&all_cpus), &all_cpus);
            set_cpus_allowed_ptr(hi_bus->pst_rx_tsk, &all_cpus);
        }
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "bus rx task thread to 0x%lx",
                             (is_bind ? cpumask_bits(&fast_cpus)[0] : cpumask_bits(&all_cpus)[0]));
#endif
    }
