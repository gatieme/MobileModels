...
void wlan_pm_deepsleep_delay_timeout(struct timer_list *t);
#endif
...
#else
    timer_setup(&pst_wlan_pm->st_watchdog_timer, (void*)wlan_pm_wdg_timeout, 0);
#endif
...
#else
    timer_setup(&pst_wlan_pm->st_deepsleep_delay_timer, (void*)wlan_pm_deepsleep_delay_timeout, 0);
#endif
...
oal_uint32 g_wlan_mem_check_mdelay = 1000;
oal_uint32 g_bfgx_mem_check_mdelay = 1500;

wlan_memdump_t g_st_wlan_memdump_cfg = { 0x60000000, 0x1000 };

oal_uint32 set_wlan_mem_check_mdelay(uint32 mdelay)
{
    g_wlan_mem_check_mdelay = mdelay;
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "set_wlan_mem_check_mdelay::set delay:%ums!!", g_wlan_mem_check_mdelay);
    return 0;
}
oal_uint32 set_bfgx_mem_check_mdelay(uint32 mdelay)
{
    g_bfgx_mem_check_mdelay = mdelay;
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "bfgx_mem_check_mdelay::set delay:%ums!!", g_bfgx_mem_check_mdelay);
    return 0;
}
 ...
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
int32_t wlan_pm_close(oal_void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    struct pm_drv_data *pm_data = pm_get_drvdata();
    int32_t pm_close_ret;
    oal_print_hi11xx_log(HI11XX_LOG_DBG, "wlan_pm_close enter");
    mutex_lock(&pm_data->host_mutex);
    pm_close_ret = wlan_pm_close_pre(pst_wlan_pm);
    if (pm_close_ret != OAL_SUCC) {
        mutex_unlock(&pm_data->host_mutex);
        return pm_close_ret;
    }
...
int32_t hitalk_pm_close(oal_void)
{
    struct wlan_pm_s *pst_hitalk_pm = wlan_pm_get_drv();
    ...
    mutex_lock(&pm_data->host_mutex);
    pm_close_ret = wlan_pm_close_pre(pst_hitalk_pm);
    if (pm_close_ret != OAL_SUCC) {
        OAM_ERROR_LOG0(0, OAM_SF_PWR, "wlan_pm_close_pre FAIL!");
        mutex_unlock(&pm_data->host_mutex);
        declare_dft_trace_key_info("wlan_pm_close_pre", OAL_DFT_TRACE_FAIL);
        return pm_close_ret;
    }
...
OAL_STATIC int32_t wlan_pm_sleep_request_host(struct wlan_pm_s *pst_wlan_pm)
{
 ...
 * 功能: 检查是否可以睡眠
 */
OAL_STATIC int32_t  wlan_pm_sleep_cmd_proc(struct wlan_pm_s *pst_wlan_pm)
{
    int32_t ret;
...
#endif
{
    ...
    struct wlan_pm_s *pm_data = (struct wlan_pm_s *)(uintptr_t)data;
#else
    struct wlan_pm_s *pm_data = from_timer(pm_data, t, st_deepsleep_delay_timer);
#endif
    if (pm_data == NULL) {
        return;
    }
...
#define RETRY_TIMES 3
    ...
    for (i = 0; i < RETRY_TIMES; i++) {
        ret = (oal_int32)wlan_pm_wakeup_dev();
        if (ret == OAL_SUCC) {
            break;
        }
    }
