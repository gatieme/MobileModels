...
OAL_STATIC oal_void hmac_mgmt_update_auth_mib(hmac_vap_stru *pst_hmac_vap,
                                              oal_netbuf_stru *pst_auth_rsp, oal_uint16 auth_rsp_len)
{
    oal_uint16 us_status;
    oal_uint8 auc_addr1[WLAN_MAC_ADDR_LEN] = { 0 };
    oal_uint8 *puc_mac_header = oal_netbuf_header(pst_auth_rsp);
    uint8_t *auth_rsp_payload = NULL;
    uint16_t auth_rsp_payload_len;

    if (auth_rsp_len < MAC_80211_FRAME_LEN) {
        return;
    }
    auth_rsp_payload = puc_mac_header + MAC_80211_FRAME_LEN;
    auth_rsp_payload_len = auth_rsp_len - MAC_80211_FRAME_LEN;
    us_status = mac_get_auth_status(auth_rsp_payload, auth_rsp_payload_len);
    ...
    hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req, dmac_wlan_crx_event_stru *pst_mgmt_rx_event)
{
    ...
    auth_req_payload = mac_get_rx_payload(auth_req, &auth_req_payload_len);
    ...
    if (mac_get_auth_alg(auth_req_payload, auth_req_payload_len) == WLAN_MIB_AUTH_ALG_SAE) {
    ...
    hmac_mgmt_update_auth_mib(hmac_vap, auth_rsp, auth_rsp_len);
...
oal_bool_enum hmac_go_is_auth(mac_vap_stru *pst_mac_vap)
{
    ...
    oal_dlist_search_for_each_safe(pst_entry, pst_dlist_tmp, &(pst_mac_vap->st_mac_user_list_head))
    {
        ...
        /*
         * :
         * 枚举值从1开始，直接用MAC_USER_STATE_ASSOC > pst_user_tmp->en_user_asoc_state判断可能出错
 */
...
OAL_STATIC oal_void hmac_ap_up_category_public(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf,
...
OAL_STATIC oal_void hmac_ap_up_category_vendor(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf,
    uint8_t *payload, uint16_t payload_len)
{
    /* 查找OUI-OUI type值为 50 6F 9A - 09 (WFA P2P v1.0) */
    /* 并用hmac_rx_mgmt_send_to_host接口上报 */
    ...
    hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf, oal_bool_enum_uint8 en_is_protected)
{
    ...
    oal_uint32 payload_len = hmac_get_frame_body_len(pst_netbuf);
    ...
    if (payload_len < MAC_ACTION_CATEGORY_AND_CODE_LEN) {
        OAM_WARNING_LOG1(0, OAM_SF_RX, "{ap_up::payload_len too short[%d].}", pst_rx_ctrl->st_rx_info.us_frame_len);
        return;
    }
    ...
    switch (payload[MAC_ACTION_OFFSET_CATEGORY]) {
        case MAC_ACTION_CATEGORY_BA: {
            hmac_ap_up_category_ba(pst_hmac_vap, payload, pst_hmac_user, payload_len);
        }
        break;

        case MAC_ACTION_CATEGORY_PUBLIC: {
            hmac_ap_up_category_public(pst_hmac_vap, pst_netbuf, payload, payload_len);
        }
        break;

        case MAC_ACTION_CATEGORY_HT: {
            hmac_ap_up_category_ht(pst_hmac_vap, payload, pst_netbuf);
        }
        break;
#if (_PRE_WLAN_FEATURE_PMF != _PRE_PMF_NOT_SUPPORT)
        case MAC_ACTION_CATEGORY_SA_QUERY: {
            hmac_ap_up_category_sa_query(pst_hmac_vap, payload, pst_netbuf, en_is_protected);
        }
        break;
#endif
        case MAC_ACTION_CATEGORY_VHT: {
            hmac_ap_up_category_vht(pst_hmac_vap, payload, pst_netbuf);
        }
        break;
        ...
        case MAC_ACTION_CATEGORY_VENDOR: {
            ...
            hmac_ap_up_category_vendor(pst_hmac_vap, pst_netbuf, payload, payload_len);
        }
        break;

#ifdef _PRE_WLAN_FEATURE_WMMAC
        case MAC_ACTION_CATEGORY_WMMAC_QOS: {
            hmac_ap_up_category_wmmac_qos(pst_hmac_vap, payload, pst_netbuf);
        }
        break;
