...
oal_uint32 hmac_mgmt_rx_delba_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 获取发送端的用户指针 */
    pst_hmac_user = mac_vap_get_hmac_user_by_addr(&pst_vap->st_vap_base_info, puc_da, WLAN_MAC_ADDR_LEN);
...
uint32_t hmac_rx_nb_mgmt(hmac_vap_stru *hmac_vap, dmac_wlan_crx_event_stru *rx_mgmt_event,
    oal_netbuf_stru *netbuf)
{
    ...
    if (oal_any_null_ptr3(hmac_vap, rx_mgmt_event, netbuf)) {
        OAM_ERROR_LOG0(0, OAM_SF_CHBA, "{hmac_rx_nb_mgmt::null pointer.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 判断是否是Beacon帧 */
    if (mac_hdr->st_frame_control.bit_sub_type != WLAN_ACTION || payload[0] != MAC_ACTION_CATEGORY_VENDOR ||
        payload[1] != NB_APP_TRX_ACTION) {
        return OAL_FAIL;
    }
    if (MAX_NB_ACTION_LEN + 2 < payload_len + sizeof(struct nb_msg_hdr_stru)) {
        return OAL_FAIL;
    }
    nb_hdr.cmd = NB_SEND_CTRL_DATA;
    nb_hdr.len = payload_len - 2;
    memset_s(buf, sizeof(buf), 0, sizeof(buf));
    memcpy_s(buf, sizeof(buf), &nb_hdr, sizeof(nb_hdr));
    memcpy_s(buf + sizeof(nb_hdr), sizeof(buf) - sizeof(nb_hdr), payload + 2, payload_len - 2);
    nb_netlink_send(buf, sizeof(nb_hdr) + payload_len - 2);
    return OAL_SUCC;
}
#endif
...
#ifdef _PRE_WLAN_NARROW_BAND
    /* 对于窄带的管理帧不走状态机接口 */
    if (hmac_rx_nb_mgmt(pst_vap, pst_crx_event, pst_netbuf) == OAL_SUCC) {
...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* 对于CHBA相关的管理帧不走状态机接口 */
    if (hmac_chba_vap_start_check(pst_vap)) {
        hmac_rx_chba_mgmt(pst_vap, pst_crx_event, pst_netbuf);
        oal_netbuf_free(pst_netbuf);
        return OAL_SUCC;
    }
#endif
...
oal_uint32 hmac_mgmt_send_disasoc_deauth_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 获取发送端的用户指针 */
    ul_rslt = mac_vap_find_user_by_macaddr(pst_mac_vap, puc_da, WLAN_MAC_ADDR_LEN, &us_user_idx);
    if (ul_rslt != OAL_SUCC) {
    ...
    /* 同步03问题单，用户删除需要上报到上层，防止两边不同步，造成上层存在假关联设备 */
    if (pst_hmac_user != OAL_PTR_NULL) {
