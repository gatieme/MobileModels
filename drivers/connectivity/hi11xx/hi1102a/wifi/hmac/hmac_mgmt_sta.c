...
oal_bool_enum_uint8 hmac_is_ht_mcs_set_valid(oal_uint8 *puc_ht_cap_ie, wlan_channel_band_enum_uint8 en_band)
{
    ...
    ***************************************************************************/
    ...
    /* :对端支持ht能力,但空间速率集为0,要置对端user为不支持ht能力,从而以11a或者11g速率关联 */
    ...
    oal_uint8 *puc_payload, oal_uint16 us_offset, oal_uint16 us_rx_len, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_hmac_user->st_op_rates.uc_rs_nrates == 0) {
        ...
        /*  start 基本速率集和扩展速率集都没有的时候，手动增加最低的两个速率集 */
...
static oal_void hmac_fill_assoc_resp(hmac_vap_stru *pst_hmac_sta, hmac_asoc_rsp_stru *pst_asoc_rsp,
    oal_uint8 *puc_mac_hdr, oal_uint16 us_msg_len)
{
    ...
    /* 获取AP的mac地址 */
    mac_get_bssid(puc_mac_hdr, pst_asoc_rsp->auc_addr_ap, WLAN_MAC_ADDR_LEN);
...
oal_uint32 hmac_sta_wait_asoc_rx(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    us_msg_len = hmac_get_frame_body_len(pst_mgmt_rx_event->pst_netbuf);
    ...
    /* 根据SA 找到对应AP USER结构 */
    ul_rslt = mac_vap_find_user_by_macaddr(&(pst_hmac_sta->st_vap_base_info),
        auc_addr_sa, WLAN_MAC_ADDR_LEN, &us_user_idx);
    if (ul_rslt != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
...
oal_uint32 hmac_sta_update_join_req_params(hmac_vap_stru *pst_hmac_vap, hmac_join_req_stru *pst_join_req)
{
    ...
    /* : 检查扫描结果中信道和带宽是否匹配，发现信道和带宽不匹配，则修改带宽为20MHz */
                                             ...
                                             oal_bool_enum_uint8 en_is_protected)
{
    ...
    ul_ret = mac_vap_find_user_by_macaddr(&pst_hmac_vap->st_vap_base_info, auc_bssid, WLAN_MAC_ADDR_LEN, &us_user_idx);
    if (ul_ret != OAL_SUCC) {
    ...
    if (pst_hmac_user_vap == OAL_PTR_NULL) {
        ...
        /* 没有查到对应的USER,发送去认证消息 */
        hmac_mgmt_send_deauth_frame(&(pst_hmac_vap->st_vap_base_info), auc_bssid, WLAN_MAC_ADDR_LEN,
...
OAL_STATIC oal_uint32 hmac_sta_up_rx_beacon(hmac_vap_stru *pst_hmac_vap_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    oal_uint16 us_frame_offset = MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;
    ...
    ul_ret = mac_vap_find_user_by_macaddr(&(pst_hmac_vap_sta->st_vap_base_info),
        auc_addr_sa, WLAN_MAC_ADDR_LEN, &us_user_idx);
    if (ul_ret != OAL_SUCC) {
...
OAL_STATIC oal_void hmac_sta_up_category_ba(hmac_vap_stru *pst_hmac_vap, mac_ieee80211_frame_stru *pst_frame_hdr,
    hmac_user_stru *pst_hmac_user, oal_uint8 *puc_data, oal_uint32 frame_body_len)
{
...
OAL_STATIC oal_void hmac_sta_up_category_public(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf,
    oal_uint8 *payload, oal_uint16 payload_len)
{
    /* Action */
    switch (payload[MAC_ACTION_OFFSET_ACTION]) {
        case MAC_PUB_VENDOR_SPECIFIC:
            ...
            if (mac_ie_check_p2p_action(payload + MAC_ACTION_OFFSET_ACTION,
                payload_len - MAC_ACTION_OFFSET_ACTION) == OAL_TRUE) {
                hmac_rx_mgmt_send_to_host(pst_hmac_vap, pst_netbuf);
            }
#endif
            break;
...
OAL_STATIC oal_void hmac_sta_up_category_vendor(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf,
    oal_uint8 *payload, oal_uint16 payload_len)
{
    if (payload_len < MAC_ACTION_OFFSET_CATEGORY) {
        return;
    }
#ifdef _PRE_WLAN_FEATURE_P2P
    /* 查找OUI-OUI type值为 50 6F 9A - 09 (WFA P2P v1.0) */
    /* 并用hmac_rx_mgmt_send_to_host接口上报 */
    if (mac_ie_check_p2p_action(payload + MAC_ACTION_OFFSET_CATEGORY,
        payload_len - MAC_ACTION_OFFSET_CATEGORY) == OAL_TRUE) {
        hmac_rx_mgmt_send_to_host(pst_hmac_vap, pst_netbuf);
    }
#endif
}
...
oal_uint32 hmac_get_frame_body_len(oal_netbuf_stru *net_buf)
{
    ...
    if (rx_ctrl->st_rx_info.us_frame_len < rx_ctrl->st_rx_info.uc_mac_header_len) {
        oam_error_log2(0, OAM_SF_ASSOC, "{hmac_get_frame_body_len::us_frame_len %d, mac_header_len %d}",
            rx_ctrl->st_rx_info.us_frame_len, rx_ctrl->st_rx_info.uc_mac_header_len);
        return 0;
    }
                                          ...
                                          oal_bool_enum_uint8 en_is_protected)
{
    ...
    oal_uint8 *payload = OAL_PTR_NULL;
    ...
    pst_rx_ctrl = (dmac_rx_ctl_stru *)oal_netbuf_cb(pst_netbuf);
    payload_len = hmac_get_frame_body_len(pst_netbuf);
    ...
    /* 获取帧体指针 */
    payload = (oal_uint8 *)pst_rx_ctrl->st_rx_info.pul_mac_hdr_start_addr + pst_rx_ctrl->st_rx_info.uc_mac_header_len;

    /* Category */
    switch (payload[MAC_ACTION_OFFSET_CATEGORY]) {
        case MAC_ACTION_CATEGORY_BA: {
            hmac_sta_up_category_ba(pst_hmac_vap, pst_frame_hdr, pst_hmac_user, payload, payload_len);
        }
        break;

        case MAC_ACTION_CATEGORY_WNM: {
            hmac_sta_up_category_wnm(pst_hmac_vap, pst_hmac_user, pst_netbuf, payload);
        }
        break;

        case MAC_ACTION_CATEGORY_PUBLIC: {
            hmac_sta_up_category_public(pst_hmac_vap, pst_netbuf, payload, payload_len);
        }
        break;
#ifdef _PRE_WLAN_FEATURE_WMMAC
        case MAC_ACTION_CATEGORY_WMMAC_QOS: {
            hmac_sta_up_category_wmmac_qos(pst_hmac_vap, pst_hmac_user, payload, payload_len);
        }
        break;
#endif  // _PRE_WLAN_FEATURE_WMMAC
        ...
        case MAC_ACTION_CATEGORY_SA_QUERY: {
            hmac_sta_up_category_sa_query(pst_hmac_vap, pst_netbuf, en_is_protected, payload);
        }
        break;
#endif
        case MAC_ACTION_CATEGORY_VENDOR: {
            if (payload_len <= MAC_ACTION_CATEGORY_AND_CODE_LEN + MAC_OUI_LEN) {
                OAM_WARNING_LOG1(0, OAM_SF_RX, "{hmac_action_category_vendor::payload_len %d.}", payload_len);
                return;
            }
            hmac_sta_up_category_vendor(pst_hmac_vap, pst_netbuf, payload, payload_len);
        }
        break;
#ifdef _PRE_WLAN_FEATURE_ROAM
