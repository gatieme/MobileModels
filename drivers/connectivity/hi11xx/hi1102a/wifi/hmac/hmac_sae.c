...
static uint32_t hmac_chba_check_external_auth_ssid(mac_vap_stru *mac_vap, hmac_external_auth_req_stru *external_auth)
{
    ...
    ret = mac_vap_find_user_by_macaddr(mac_vap, external_auth->auc_bssid, WLAN_MAC_ADDR_LEN, &user_id);
    if (ret != OAL_SUCC) {
    ...
    hmac_external_auth_req_stru *pst_ext_auth)
{
    ...
    if (mac_is_chba_mode(pst_mac_vap)) {
        ret = hmac_chba_check_external_auth_ssid(pst_mac_vap, pst_ext_auth);
    } else {
#endif
        ret = hmac_sta_check_external_auth_ssid(pst_mac_vap, pst_ext_auth);
#ifdef _PRE_WLAN_CHBA_MGMT
    }
#endif
    return ret;
...
#ifdef _PRE_WLAN_CHBA_MGMT
    if (mac_is_chba_mode(&(hmac_vap->st_vap_base_info))) {
        ...
        /* 上报SME认证成功，执行关联 */
        hmac_send_rsp_to_sme_sta(hmac_vap, HMAC_SME_AUTH_RSP, (oal_uint8 *)&st_auth_rsp);
#ifdef _PRE_WLAN_CHBA_MGMT
    }
#endif
}
...
oal_uint32 hmac_config_external_auth(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_SAE, "{hmac_config_external_auth::hmac_vap null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
            "hmac_config_external_auth::pst_hmac_user[%d] NULL.", pst_hmac_vap->st_vap_base_info.uc_assoc_vap_id);
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_CHBA_MGMT
    ...
    if ((mac_is_chba_mode(pst_mac_vap) == OAL_TRUE) &&
        ...
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SAE, "hmac_config_external_auth::chba role[%d] return.",
...
#endif
    hmac_sae_process_external_auth_succ(pst_hmac_vap, pst_hmac_user);
...
oal_void hmac_ap_up_rx_auth_req_to_host(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_auth_req)
{
    ...
    if ((pst_hmac_vap == NULL) || (pst_auth_req == NULL)) {
        return;
    }
    payload = mac_get_rx_payload(pst_auth_req, &payload_len);
    if (payload == NULL) {
        return;
    }
    ...
    /* 获取STA的地址 */
    mac_get_address2(oal_netbuf_header(pst_auth_req), auc_addr2, WLAN_MAC_ADDR_LEN);
    ...
    us_auth_seq = mac_get_auth_seq_num(payload, payload_len);
    ...
    hmac_ap_set_user_auth_alg_num(us_user_index, auth_alg);
}
    ...
    hmac_vap_stru *pst_sta, oal_netbuf_stru *pst_netbuf, oal_uint16 us_status, oal_uint16 us_seq_num)
{
    ...
    if (us_status == MAC_AP_FULL) {
        ...
        /* : AP以具体的理由拒绝接入时(如status code 17接入已满), 需要将状态码上报上层而非透传 */
...
oal_uint32 hmac_sta_process_sae_commit(hmac_vap_stru *pst_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    us_status   = mac_get_auth_status(payload, payload_len);
    us_seq_num  = mac_get_auth_seq_num(payload, payload_len);
...
oal_uint32 hmac_sta_process_sae_confirm(hmac_vap_stru *pst_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (pst_sta->duplicate_auth_seq4_flag == OAL_TRUE) { // wpa3 auth seq4重复帧过滤，禁止上报waps
        oam_warning_log2(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
            "{hmac_sta_wait_auth_seq4_rx::drop sae auth frame, status_code %d, seq_num %d.}", auth_status, auth_seq);
        return OAL_SUCC;
    }
    ...
    oam_warning_log2(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
        "{hmac_sta_process_sae_confirm::rx sae auth frame, status_code %x, seq_num %d.}", auth_status, auth_seq);
...
oal_bool_enum_uint8 hmac_check_illegal_sae_roam(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_bss_rsn_akm)
{
    ...
    mac_mib_get_rsn_akm_suites(pst_mac_vap, auc_sta_rsn_akm, sizeof(auc_sta_rsn_akm));
...
oal_uint32 hmac_report_external_auth_req(hmac_vap_stru *pst_hmac_vap, oal_nl80211_external_auth_action en_action)
{
    ...
    uc_akm_suites_num = mac_mib_get_rsn_akm_suites(&pst_hmac_vap->st_vap_base_info, auc_akm, sizeof(auc_akm));
    ...
    if (mac_is_chba_mode(&(pst_hmac_vap->st_vap_base_info))) {
        hmac_user_stru *hmac_user = mac_res_get_hmac_user(pst_hmac_vap->st_vap_base_info.uc_assoc_vap_id);
        if (hmac_user == NULL) {
            OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
                "{hmac_report_external_auth_req::invalid user [%d]}",
        ...
        l_ret += memcpy_s(st_ext_auth_req.st_ssid.auc_ssid, OAL_IEEE80211_MAX_SSID_LEN,
            hmac_user->chba_user.ssid, st_ext_auth_req.st_ssid.uc_ssid_len);
    } else {
#endif
        l_ret = memcpy_s(st_ext_auth_req.auc_bssid, WLAN_MAC_ADDR_LEN,
...
oal_void hmac_report_ext_auth_worker(oal_work_stru  *pst_sae_report_ext_auth_worker)
{
    ...
    pst_hmac_vap = oal_container_of((oal_delayed_work *)pst_sae_report_ext_auth_worker, hmac_vap_stru,\
        st_sae_report_ext_auth_worker);
    if (pst_hmac_vap == OAL_PTR_NULL) {
