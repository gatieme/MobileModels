...
oal_bool_enum_uint8 mac_ie_check_p2p_action(oal_uint8 *payload, oal_uint16 payload_len)
{
    if (payload_len < MAC_OUI_LEN) {
        return OAL_FALSE;
    }
    /* 找到WFA OUI */
    if ((oal_memcmp(payload, g_auc_p2p_oui, MAC_OUI_LEN) == 0) &&
        (payload[MAC_OUI_LEN] == MAC_OUITYPE_P2P)) {
