...
#endif
{
    ...
    if (pst_mac_vap == NULL) {
        /* ，STA切换到AP模式时，VAP已经删除，此时找不到vap属正常 */
                               ...
                               oal_cfg80211_conn_stru *pst_sme)
{
    ...
    if (l_ret != OAL_SUCC) {
        ...
        /* :同步1103修改，关联命令下发成功，在HMAC释放内存；否则在WAL释放内存 */
                                  ...
                                  oal_uint16 us_reason_code)
{
    ...
    /* 填写和sta关联的ap mac 地址 */
    pst_mac_vap = oal_net_dev_priv(pst_net_device);
    /* Begin： 防止删除p2p组时清空了netdev下的vap，这时disconnect，出现netdev存在，vap被清空，从而使用空指针问题 */
    if (pst_mac_vap == OAL_PTR_NULL) {
                                ...
                                oal_ap_settings_stru *ap_settings)
{
    ...
    *****************************************************************************/
    ...
    /* android主干版本是5.10.43,需要该成员，需要编译 */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 44))  // 1102 需要，暂时用内核版本号区分
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 44))  // 确认正确的 Linux 版本号
#if (_PRE_CONFIG_TARGET_PRODUCT != _PRE_TARGET_PRODUCT_TYPE_E5)
...
#endif
{
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++; /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    //  请留意，如果是Go端(通过p2p0)在一次会话中第二次(第一次发送已成功)给第三方发送Device Discoverability Response等帧时，可能会由于对方不再侦听而超时失败。
