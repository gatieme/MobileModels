 ...
 *
 * Description: Create a BLUEZ system file node and register the driver.
...
struct bt_data_combination g_bt_data_combination = {0};
...
STATIC uint32_t char2byte_bt(const char *strori, char *outbuf)
{
    ...
    for (i = 0; i < l_loop_times; i++) {
        ...
        if (i % 2 == 0) { /* 通过对2取余，将mac地址按位成对存储，将高4位和低4位拼接 */
            ptr_out[i / 2] |= (temp << 4); /* 通过除2，将8位字节中的高4位存储成对MAC地址第1位 */
        } else {
            ptr_out[i / 2] |= temp; /* 通过除2，将8位字节中的低4位存储成对MAC地址第2位 */
        }
    }
...
STATIC int hci_bt_open(struct hci_dev *hdev)
{
    ...
    ps_print_info("hci_bt_open %p, subsys: %d\n", pm_data, BFGX_BT);

    /* Undo clearing this from hci_uart_close() */
    mutex_lock(&(pm_top_data->host_mutex));

    ret = hw_bfgx_open(pm_data->ps_core_data, BFGX_BT);
...
STATIC int hci_bt_close(struct hci_dev *hdev)
{
    ...
    ret = hw_bfgx_close(pm_data->ps_core_data, BFGX_BT);
...
STATIC int hci_bt_data_send(struct hci_dev *hdev, struct sk_buff *skb,
                            struct ps_core_s *ps_core_d, struct bt_data_combination *data_comb)
{
    ...
    uint32_t count = skb->len;

    headroom = data_comb->len + sizeof(struct ps_packet_head) + 1;
    ...
    if (data_comb->len == BT_TYPE_DATA_LEN) {
        skb->data[sizeof(struct ps_packet_head) + 1] = data_comb->type;
    }
...
STATIC int hci_bt_send(struct hci_dev *hdev, struct sk_buff *skb, struct bt_data_combination *data_comb)
{
    int32_t ret;
    struct pm_drv_data *pm_data = (struct pm_drv_data *)hci_get_drvdata(hdev);
    struct ps_core_s *ps_core_d = (struct ps_core_s *)pm_data->ps_core_data;
    ...
    ret = hci_bt_data_send(hdev, skb, ps_core_d, data_comb);
...
STATIC int hci_bt_send_frame(struct hci_dev *hdev, struct sk_buff *skb)
{
    ...
    /* 组合发送 */
    ret = hci_bt_send(hdev, skb, &g_bt_data_combination);
...
STATIC int hci_bt_recv_thread(void *data)
{
    ...
    for (;;) {
        ps_core_d = (struct ps_core_s *)pm_data->ps_core_data;
        if (unlikely(ps_core_d == NULL)) {
            ps_print_err("ps_core_d is NULL\n");
            break;
        }
...
STATIC int bt_register_hci_dev(struct pm_drv_data *pm_data)
{
    ...
    if (IS_ERR(pm_data->st_bt_dev.bt_recv_task)) {
        ...
        mutex_destroy(&g_bt_data_combination.comb_mutex);
...
STATIC void bt_unregister_hci_dev(struct pm_drv_data *pm_data)
{
    ...
    if (!IS_ERR(pm_data->st_bt_dev.bt_recv_task)) {
        oal_kthread_stop(pm_data->st_bt_dev.bt_recv_task);
                             ...
                             struct bt_data_combination *data_comb)
{
    ...
    ps_core_d = (struct ps_core_s *)get_ps_core_from_miscdev(filp->private_data);
    if (ps_core_d == NULL || buf == NULL || ps_core_d->pm_data == NULL) {
        ps_print_err("ps core & buf is null\n");
        return -EINVAL;
    }
...
long hw_bt_ioctl(struct file *file, uint32_t cmd, unsigned long arg)
{
    ...
    ps_core_d = (struct ps_core_s *)get_ps_core_from_miscdev(file->private_data);
    if (ps_core_d == NULL) {
        ps_print_err("ps core is null\n");
        return -EINVAL;
    }

    pm_data = (struct pm_drv_data *)ps_core_d->pm_data;
    if (pm_data == NULL) {
        ps_print_err("pm_data is NULL!\n");
        return -EINVAL;
    }
