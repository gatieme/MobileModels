...
{
    int32_t ret = prepare_to_visit_node(ps_core_d);
    if (ret < 0) {
        ps_print_err("prepare work FAIL\n");
        return -FAILURE;
    }
...
STATIC int32_t bfgx_timeout_dbg(uint32_t bus_id)
{
    ...
    ps_core_d->heartbeat_dbg_flag = 0;
    /* 等待dfr完成，等待进入dfr流程，防止睡眠 */
    while (in_plat_exception_reset() == OAL_FALSE) {
        ;
    };
    post_to_visit_node(ps_core_d);
    ps_print_info("[dfr_test]bfgx device timeout cause --\n");
...
STATIC int32_t bfgx_wkup_fail_test(uint32_t bus_id)
{
    ...
    plat_exception_handler(SUBSYS_BFGX, THREAD_NFC, get_wakeup_excp_by_uart_idx(ps_core_d->pm_data->index));
    return OAL_SUCC;
}
