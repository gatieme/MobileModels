...
static int32_t chr_inner_report_proc(uint32_t errno, uint8_t *ptr_data, uint16_t len)
{
    chr_inner_report_info* report = (chr_inner_report_info*)ptr_data;

    if (len < sizeof(chr_inner_report_info)) {
        return -EINVAL;
    }
...
static int32_t chr_error_log_report_proc(uint32_t errno, uint8_t *ptr_data, uint16_t len)
{
...
static int32_t chr_write_errno_to_queue(uint32_t errno, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    ...
    if (errno == CHR_CHIP_INNER_REPORT_EVENTID) {
        chr_inner_report_proc(errno, ptr_data, len);
        return CHR_SUCC;
    }

    if (errno == CHR_ERROR_LOG_EVENTID) {
        chr_error_log_report_proc(errno, ptr_data, len);
        return CHR_SUCC;
    }
...
static int32_t chr_bfg_dev_tx_handler(uint32_t errno)
{
    ...
    /* alloc skb buf */
    sk_len = sizeof(uint32_t) + sizeof(struct ps_packet_head) + sizeof(struct ps_packet_end);
    skb = alloc_skb(sk_len, (oal_in_interrupt() || oal_in_atomic()) ? GFP_ATOMIC : GFP_KERNEL);
    if (skb == NULL) {
