 ...
 *
 * Description: Implementation of NV Space Read/Write Interfaces
 ...
 */
static const uint32_t g_crc_table[256] = {
...
/* ========================================================================= */
#define do_crc_1(buf) crc = g_crc_table[(crc ^ (*(buf)++)) & 0xff] ^ (crc >> 8)
...
int32_t hisi_conn_nve_set_particion_size(uint32_t particion_id, uint32_t len)
{
    ...
    if (len > CONN_NVE_SIZE ||
        g_memory_used - g_hisi_conn_ptable[particion_id].len + len > CONN_NVE_SIZE) {
        return -CONN_NVE_ERROR_SET_SIZE;
    }
    // 更新分区信息
...
static int32_t hisi_conn_nve_check(uint32_t particion_id, size_t offset, size_t len)
{
    if (particion_id >= HISI_CONN_PTABLE_BUT) {
    ...
    // 每一块的内存, 都不能超时自身的界限
    if (offset + len > g_hisi_conn_ptable[particion_id].len) {
        printk(KERN_ERR "[CONN_NVE][%s]nvalid len, offset = %u, len = %u!\n",
            __func__, (uint32_t)offset, (uint32_t)len);
        return -CONN_NVE_ERROR_OVERSIZE;
    }
    return CONN_NVE_RET_OK;
}
...
int32_t hisi_conn_nve_read(uint32_t particion_id, size_t offset, size_t len, uint8_t *buf)
{
    ...
    oldfs = get_fs();
    set_fs(KERNEL_DS);
    fd = (int)oal_sys_open(CONN_NVE_DEVICE_NAME, O_RDONLY, (int)S_IRWXU | S_IRWXG | S_IRWXO);
    if (fd < 0) {
        printk(KERN_ERR "[CONN_NVE][%s]open nv block device failed, and fd = %x!\n", __func__, fd);
        ret = -ENODEV;
        goto out;
    }
    ret = hisi_conn_nve_check(particion_id, offset, len);
    ...
    ret = (int)oal_sys_read((unsigned int)fd, (char *)buf, len);
    if (ret == -1) {
...
int32_t hisi_conn_nve_write(uint32_t particion_id, size_t offset, size_t len, uint8_t *buf)
{
    ...
    oldfs = get_fs();
    set_fs(KERNEL_DS);
    fd = (int)oal_sys_open(CONN_NVE_DEVICE_NAME, O_RDWR, (int)S_IRWXU | S_IRWXG | S_IRWXO);
    if (fd < 0) {
        printk(KERN_ERR "[CONN_NVE][%s]open nv block device failed, and fd = %x!\n", __func__, fd);
        ret = -ENODEV;
        goto out;
...
EXPORT_SYMBOL(hisi_conn_set_cali_nv);
#endif
