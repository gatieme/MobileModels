 ...
 *
 * Description: Read the changid from the readme.txt file.
...
#define MPXX_CUST_FIRMWARE_PATH "/vendor/firmware/"
#define MPXX_CUST_README_NAME "/readme.txt"
...
STATIC uint8_t* get_readme_path(void)
{
    ...
    path_len = OAL_STRLEN(MPXX_CUST_FIRMWARE_PATH) + OAL_STRLEN(chip_name) +
               OAL_STRLEN(MPXX_CUST_README_NAME) + 1;
    ...
    if (strcat_s(path, path_len, MPXX_CUST_FIRMWARE_PATH) != EOK) {
        ps_print_err("strcat [%s] fail, len %d\n", MPXX_CUST_FIRMWARE_PATH, path_len);
        os_mem_kfree(path);
        return NULL;
    }
    ...
    if (strcat_s(path, path_len, MPXX_CUST_README_NAME) != EOK) {
        ps_print_err("strcat [%s] fail, len %d\n", MPXX_CUST_README_NAME, path_len);
        os_mem_kfree(path);
        return NULL;
    }
...
void plat_changid_print(void)
{
    if (g_changid_buffer != NULL) {
        ps_print_info("changid: %s, chip nr %d\n",
                      g_changid_buffer, get_mpxx_subchip_type());
    }
}
