 ...
 *
 * Description: oneimage functional module
...
#ifdef CONFIG_HWCONNECTIVITY
/* global variables */
STATIC char g_mpxx_chip_type[HW_PROC_BUTT][BUFF_LEN];
...
int is_hisi_chiptype(int32_t chip)
{
    return true;
}
#else
...
STATIC ssize_t hwconn_read_proc_chiptype(struct file *filp, char __user *buffer, size_t len, loff_t *off)
{
    ...
    size = strlen(g_mpxx_chip_type[HW_PROC_CHIPTYPE]);
    if (size <= 0) {
        printk(KERN_ERR "[HW_CONN] g_mpxx_chip_type is blank\n");
        return 0;
    }
    ...
    printk(KERN_INFO "[HW_CONN] chip type[%s] \n", g_mpxx_chip_type[HW_PROC_CHIPTYPE]);

    if (copy_to_user(buffer, g_mpxx_chip_type[HW_PROC_CHIPTYPE], size)) {
...
STATIC ssize_t hwconn_read_proc_supp(struct file *filp, char __user *buffer, size_t len, loff_t *off)
{
    ...
    size = strlen(g_mpxx_chip_type[HW_PROC_SUPP]);
    if (size <= 0) {
    ...
    printk(KERN_INFO "[HW_CONN] supp[%s] \n", g_mpxx_chip_type[HW_PROC_SUPP]);

    if (copy_to_user(buffer, g_mpxx_chip_type[HW_PROC_SUPP], size)) {
...
STATIC ssize_t hwconn_read_proc_p2p(struct file *filp, char __user *buffer, size_t len, loff_t *off)
{
    ...
    size = strlen(g_mpxx_chip_type[HW_PROC_P2P]);
    if (size <= 0) {
    ...
    printk(KERN_INFO "[HW_CONN] p2p[%s] \n", g_mpxx_chip_type[HW_PROC_P2P]);

    if (copy_to_user(buffer, g_mpxx_chip_type[HW_PROC_P2P], size)) {
...
STATIC ssize_t hwconn_read_proc_apd(struct file *filp, char __user *buffer, size_t len, loff_t *off)
{
    ...
    size = strlen(g_mpxx_chip_type[HW_PROC_HOSTAPD]);
    if (size <= 0) {
    ...
    printk(KERN_INFO "[HW_CONN] hostapd[%s]\n", g_mpxx_chip_type[HW_PROC_HOSTAPD]);

    if (copy_to_user(buffer, g_mpxx_chip_type[HW_PROC_HOSTAPD], size)) {
...
STATIC ssize_t hwconn_read_proc_firmware(struct file *filp, char __user *buffer, size_t len, loff_t *off)
{
    ...
    size = strlen(g_mpxx_chip_type[HW_PROC_FIRMWARE]);
    if (size <= 0) {
    ...
    printk(KERN_INFO "[HW_CONN] firmware[%s]\n", g_mpxx_chip_type[HW_PROC_FIRMWARE]);

    if (copy_to_user(buffer, g_mpxx_chip_type[HW_PROC_FIRMWARE], size)) {
...
STATIC int create_hwconn_proc_file(void)
{
    ...
    for (; index < HW_PROC_BUTT; index++) {
        memset_s((void *)g_mpxx_chip_type[index], sizeof(g_mpxx_chip_type[index]),
            0, sizeof(g_mpxx_chip_type[index]));
        ret = read_from_dts(g_mpxx_chip_type[index], sizeof(g_mpxx_chip_type[index]) - 1,
