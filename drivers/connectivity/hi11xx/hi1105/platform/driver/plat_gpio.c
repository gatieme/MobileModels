...
STATIC int32_t _gpio_dts_prop(struct device_node *np, const char *dts_prop, int32_t *prop_val)
{
    int32_t ret;
...
STATIC int32_t get_gpio_nr_from_node(const char *gpio_node, const char *gpio_prop, int32_t *physical_gpio)
{
    ...
    ret = _gpio_dts_prop(np, gpio_prop, physical_gpio);
    if (ret != OAL_SUCC) {
        return BOARD_FAIL;
    }
...
STATIC int32_t _gpio_init(struct conn_gpio_node *gpio_node)
{
    int32_t ret;
...
int32_t conn_gpio_init(struct conn_gpio_chip *chip)
{
    ...
    for (i = 0; i < g_conn_gpio.node_num; i++) {
        ret = _gpio_init(&g_conn_gpio.node[i]);
        if (ret < 0) {
...
OAL_STATIC void _gpio_intr_enable(struct conn_gpio_irq_attr *irq_attr, uint32_t en)
{
    oal_uint flags;

    if (irq_attr->number == 0 || irq_attr->inited == OAL_FALSE) {
        ps_print_err("irq [%s] not initialize", irq_attr->name);
        return;
    }
...
void conn_gpio_intr_enable(enum gpio_type type, uint32_t en)
{
    ...
    if (gpio_node == NULL) {
        ps_print_err("gpio type %d not find", type);
        return;
    }
    ...
    if (g_conn_gpio.ops != NULL && g_conn_gpio.ops->irq_enable != NULL) {
        ...
        _gpio_intr_enable(irq_attr, en);
    }
}

OAL_STATIC int32_t _irq_register(struct conn_gpio_node *gpio_node, oal_irq_handler_t handler, void *dev)
{
    int32_t ret;
    struct conn_gpio_irq_attr *irq_attr = NULL;
    ...
    if (irq_attr->number == 0) {
        ps_print_err("gpio type [%d] irq not initialize", gpio_node->pin.type);
        return -OAL_FAIL;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "success to register %s intr", irq_attr->name);
    return OAL_SUCC;
}
...
int32_t conn_irq_register(enum gpio_type type, oal_irq_handler_t handler, void *dev)
{
    ...
    if (g_conn_gpio.ops != NULL && g_conn_gpio.ops->irq_register != NULL) {
        ...
        return _irq_register(gpio_node, handler, dev);
    }
}

STATIC void _irq_unregister(struct conn_gpio_node *gpio_node, void *dev)
{
    struct conn_gpio_irq_attr *irq_attr = NULL;

    irq_attr = &gpio_node->irq;
    if (irq_attr->number == 0) {
        ps_print_err("gpio index [%d] irq not initialize",
                     gpio_node->pin.number);
        return;
    }

    _gpio_intr_enable(irq_attr, OAL_FALSE);
...
int32_t conn_get_gpio_type_by_number(int32_t gpio_nr)
{
    ...
    for (i = 0; i < g_conn_gpio.node_num; i++) {
        ...
        if (get_spec_from_type(pin_info->type) == NULL) {
            ps_print_err("gpio type %d, number %d not register", pin_info->type, pin_info->number);
            return -OAL_FAIL;
        }
