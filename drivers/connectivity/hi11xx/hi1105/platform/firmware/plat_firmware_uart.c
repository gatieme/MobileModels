...
uint8_t g_data_end_buf[XMODE_DATA_LEN];
...
{
    uint32_t ulbuflen, ulheadtoendlen;
    uint8_t *ptail = NULL;
    uint8_t uart = ps_core_d->pm_data->index;

    if (unlikely((data == NULL)) || (check_uart_bufflen(uart) != SUCC)) {
...
static int32_t xmodem_single_send(struct ps_core_s *ps_core_d, uint8_t *buf, int32_t len)
{
    ...
    if (len != XMODE_DATA_LEN) {
        return -EFAIL;
    }

    st_patch_pkt.head = SOH;
    st_patch_pkt.packet_num = g_xmodem_index;
    crc_value = do_crc_table_1(buf, XMODE_DATA_LEN);
    ...
    for (i = 0; i < HOST_DEV_TIMEOUT; i++) {
        ...
        l_ret = pm_uart_send(ps_core_d, buf, XMODE_DATA_LEN);
 ...
 */
static int32_t patch_xmodem_send(struct ps_core_s *ps_core_d, uint8_t *data, int32_t len)
{
    int32_t l_ret;
    int32_t l_sendlen;
    int32_t l_datalen;
    ...
    while (l_datalen > 0) {
        /* 数据长度不够128个 */
        if (l_datalen < XMODE_DATA_LEN) {
            ps_print_dbg("data_len  %d\n", l_datalen);
            l_ret = memset_s(g_data_end_buf, sizeof(g_data_end_buf), 0x0, XMODE_DATA_LEN);
            if (l_ret != EOK) {
                ps_print_err("data buff init failed\n");
                return -EFAIL;
            }
            l_ret = memcpy_s(g_data_end_buf, sizeof(g_data_end_buf), flag, l_datalen);
            if (l_ret != EOK) {
            ...
            l_sendlen = XMODE_DATA_LEN;
            l_datalen = l_datalen - l_sendlen;
        }

        if (xmodem_single_send(ps_core_d, flag, XMODE_DATA_LEN) != SUCC) {
            return -EFAIL;
        }
...
int32_t patch_down_file(struct ps_core_s *ps_core_d, const uint8_t *puc_file)
{
    ...
    uint8_t *auc_buf;
    int32_t l_len, l_ret, l_count;
    mm_segment_t fs;
    uint8_t uart = ps_core_d->pm_data->index;

    if (puc_file == NULL || g_data_buf[uart] == NULL) {
        return -EFAIL;
    }
    ...
    while (1) {
        ...
        if ((l_len > 0) && (l_len <= READ_PATCH_BUF_LEN)) {
            l_ret = patch_xmodem_send(ps_core_d, g_data_buf[uart], l_len);
            if (l_ret == -EFAIL) {
...
STATIC int32_t get_and_set_baud_cmd_mpxx(struct ps_core_s *ps_core_d, uint8_t *key, uint8_t *value)
{
    int32_t ret;
    int32_t num;
...
STATIC int32_t get_and_set_baud_cmd_mp16c(struct ps_core_s *ps_core_d, uint8_t *key, uint8_t *value)
{
    int32_t ret;
    int32_t num;
...
STATIC int32_t patch_number_baud_cmd(struct ps_core_s *ps_core_d, uint8_t *key, uint8_t *value)
{
    ...
    if (subchip_type == BOARD_VERSION_MP16C) {
        ret = get_and_set_baud_cmd_mp16c(ps_core_d, key, value);
    } else {
        ret = get_and_set_baud_cmd_mpxx(ps_core_d, key, value);
    }
    return ret;
}

STATIC int32_t patch_down_cali_data(struct ps_core_s *ps_core_d, uint8_t *cmd_name, uint32_t fw_addr)
{
    int32_t l_len;
    int32_t l_ret;
    uint8_t *cali_data_buffer_addr = NULL;
    uint8_t uart = ps_core_d->pm_data->index;
    ...
    if (!os_str_cmp(cmd_name, CALI_BFGX_DATA_CMD_KEYWORD)) {
        l_ret = get_bfgx_cali_data(g_data_buf[uart], READ_PATCH_BUF_LEN, fw_addr);
        if (l_ret < 0) {
            ps_print_err("get bfgx cali data failed, len=%d\n", l_len);
            return -EFAIL;
        }
        cali_data_buffer_addr = g_data_buf[uart];
        l_len = sizeof(bfgx_cali_data_stru);
    } else if (!os_str_cmp(cmd_name, CALI_DCXO_DATA_CMD_KEYWORD)) {
        cali_data_buffer_addr = (uint8_t *)get_dcxo_data_buf_addr();
        if (cali_data_buffer_addr == NULL) {
            ps_print_err("[dcxo] buffer alloc error");
            return -EFAIL;
        }
        l_len = DCXO_DEVICE_MAX_BUF_LEN;
    } else {
        return -EFAIL;
    }
    ...
    g_xmodem_index = 1;
    l_ret = patch_xmodem_send(ps_core_d, cali_data_buffer_addr, l_len);
    if (l_ret == -EFAIL) {
...
STATIC int32_t cali_data_download(struct ps_core_s *ps_core_d, uint8_t *key, uint8_t *value, uint8_t *cmd_name)
{
    int32_t l_ret;
    uint32_t len;
    uint32_t value_len;
    ...
    /* 获取bfgx校准数据 */
    l_ret = patch_down_cali_data(ps_core_d, cmd_name, fw_addr);
    if (l_ret < 0) {
...
STATIC int32_t patch_number_cali_data(struct ps_core_s *ps_core_d, uint8_t *key, uint8_t *value)
{
    int32_t ret;

    /* 加载BFGX或者DCXO的校准数据 */
    ret = cali_data_download(ps_core_d, FILES_CMD_KEYWORD, value, key);
    if (ret < 0) {
        ps_print_err("download %s fail\n", key);
    }
...
STATIC uart_firmware_cmd g_number_cmd_type[] = {
    ...
    {CALI_BFGX_DATA_CMD_KEYWORD, patch_number_cali_data},
    {CALI_DCXO_DATA_CMD_KEYWORD, patch_number_cali_data},
...
static int32_t patch_file_addr_send(struct ps_core_s *ps_core_d, uint8_t *data, int32_t data_len, const char *file_path)
{
    int32_t l_ret, l_len;
    int32_t i;
    ...
    /* 删除头部的空格 */
    l_len = os_str_len((char *)file_path);
    for (i = 0; i < l_len; i++) {
        /* 兼容绝对路径和相对路径 */
...
{
    int32_t i, n, l_ret, data_len;
    uint8_t addr[INT32_STR_LEN], data[DATA_BUF_LEN];
    const char *tmp = NULL;
    ...
    l_ret = patch_file_addr_send(ps_core_d, data, data_len, &tmp[i + 1]);
    return l_ret;
}

static void *patch_malloc_cmd_buf(struct ps_core_s *ps_core_d, uint8_t *buf)
{
    int32_t l_len;
    uint8_t *flag = NULL;
    uint8_t *p_buf = NULL;
                                 ...
                                 const uint8_t* cmd_name, const uint8_t* cmd_para)
{
    ...
    } else if (!os_mem_cmp(g_patch_ctrl[uart].cmd[index].cmd_name,
 ...
 */
static int32_t patch_parse_cfg(struct ps_core_s *ps_core_d, uint8_t *buf, int32_t buf_len)
{
    int32_t i;
    uint8_t *cur = NULL;
    uint8_t *next = NULL;
 ...
 */
static int32_t patch_get_cfg(struct ps_core_s *ps_core_d, const char *cfg)
{
    uint8_t *buf = NULL;
    int32_t l_readlen;
    int32_t l_ret;
 ...
 */
static int32_t patch_execute_cmd(struct ps_core_s *ps_core_d, int32_t cmd_type, uint8_t *cmd_name, uint8_t *cmd_para)
{
    int32_t l_ret;
...
/*
 * Prototype    : patch_download_cmd
 * Description  : download cmd process
 */
static int32_t patch_download_cmd(struct ps_core_s *ps_core_d)
{
    int32_t l_ret;
    int32_t i;
...
STATIC void uart_cfg_file_init(struct ps_core_s *ps_core_d)
{
    ...
    if (l_subchip_type == BOARD_VERSION_MP15) {
        if (!mpxx_is_asic()) {
            memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                     UART_CFG_FILE_FPGA_MP15, os_str_len(UART_CFG_FILE_FPGA_MP15));
        } else {
            memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                     UART_CFG_FILE_PILOT_MP15, os_str_len(UART_CFG_FILE_PILOT_MP15));
        }
    } else if (l_subchip_type == BOARD_VERSION_MP16) {
        if (!mpxx_is_asic()) {
            memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                     UART_CFG_FILE_FPGA_MP16, os_str_len(UART_CFG_FILE_FPGA_MP16));
        } else {
            memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                     UART_CFG_FILE_PILOT_MP16, os_str_len(UART_CFG_FILE_PILOT_MP16));
        }
    } else if (l_subchip_type == BOARD_VERSION_MP16C) {
        if (!mpxx_is_asic()) {
            if (uart == BUART) {
                memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                         BUART_CFG_FILE_FPGA_MP16C, os_str_len(BUART_CFG_FILE_FPGA_MP16C));
            } else {
                memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                         GUART_CFG_FILE_FPGA_MP16C, os_str_len(GUART_CFG_FILE_FPGA_MP16C));
            }
        } else {
            if (uart == BUART) {
                memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                         BUART_CFG_FILE_PILOT_MP16C, os_str_len(BUART_CFG_FILE_PILOT_MP16C));
            } else {
                memcpy_s(g_patch_ctrl[uart].cfg_path, sizeof(g_patch_ctrl[uart].cfg_path),
                         GUART_CFG_FILE_PILOT_MP16C, os_str_len(GUART_CFG_FILE_PILOT_MP16C));
            }
        }
    } else {
 ...
 */
static int32_t patch_init(struct ps_core_s *ps_core_d)
{
    int32_t l_ret;
    uint8_t uart = ps_core_d->pm_data->index;
...
#define RETRY_CNT 3
    ...
    for (counter = 0; counter < RETRY_CNT; counter++) {
        ret = patch_download_cmd(ps_core_d);
        if (ret) {
