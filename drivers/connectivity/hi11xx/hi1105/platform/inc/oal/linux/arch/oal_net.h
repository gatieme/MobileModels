 ...
 *
 * Description: oal_net interface file
 * Author: @CompanyNameTag
 */

#ifndef OAL_LINUX_NET_H
#define OAL_LINUX_NET_H
...
#define NAPI_POLL_WEIGHT_LEV1          8 // 一次napi软中断调度skb数
#define NAPI_POLL_WEIGHT_LEV2          16
#define NAPI_POLL_WEIGHT_LEV3          24
#define NAPI_STAT_PERIOD               1000
#define NAPI_WATER_LINE_LEV1           5000 /* 按接收60M大包吞吐pps计算 */
...
/* 每个时间占用4个字节 */
#define skb_time_offset(_trace_id) (_trace_id << 2)

/* Device Ring Rx Delay */
/* BUS RX: data指向hcc hdr: [24(time) = data + 5+1+3+31] */
#define ete_rx_skb_delay(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) + 40 + skb_time_offset(_trace_id)))
#define set_rx_skb_ete_delay_trace_time(_rx_skb, _trace_id, _new_time) \
    ete_rx_skb_delay(_rx_skb,  _trace_id) = _new_time

/* HCC RX: data指向payload: [24(time) = data - 24] */
#define rx_skb_delay(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) - 24 + skb_time_offset(_trace_id)))
#define set_rx_skb_hcc_delay_trace_time(_rx_skb, _trace_id, _new_time) \
    rx_skb_delay(_rx_skb,  _trace_id) = _new_time

/* napi_poll:data指向payload: [24(time) = data - (24+12+2(ether+type))] */
/* net_if:   data指向payload: [24(time) = data - (24+12+2(ether+type))] */
#define host_rx_skb_delay(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) - 38 + skb_time_offset(_trace_id)))
#define set_rx_skb_host_delay_trace_time(_rx_skb, _trace_id, _new_time) \
    host_rx_skb_delay(_rx_skb,  _trace_id) = _new_time
/* data指向了802.3/802.11头 */
#define get_rx_skb_host_delay_trace_time(_rx_skb,  _trace_id)  host_rx_skb_delay(_rx_skb,  _trace_id)

/* Host Ring Rx Delay */


/* Device Ring Tx Delay */
/* data指向ETH头 */
#define xmit_tx_skb_delay(_tx_skb)       (*(uint32_t*)((uint8_t*)oal_netbuf_cb(_tx_skb) + 44))
#define set_tx_skb_xmit_delay_trace_time(_tx_skb, _new_time)  (xmit_tx_skb_delay(_tx_skb) = _new_time)
#define get_tx_skb_xmit_delay_trace_time(_tx_skb)              xmit_tx_skb_delay(_tx_skb)

/* data指向了CB头 19字节 */
#define hcc_tx_skb_delay(_tx_skb)        (*(uint32_t*)((uint8_t*)oal_netbuf_cb(_tx_skb) + 40))
#define set_tx_skb_hcc_delay_trace_time(_tx_skb, _new_time)   (hcc_tx_skb_delay(_tx_skb) = _new_time)
#define get_tx_skb_hcc_delay_trace_time(_tx_skb)               hcc_tx_skb_delay(_tx_skb)

/* data指向了hcc hdr, TIME起始地址: 5+1+3+19字节 */
#define ete_tx_skb_delay(_tx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_tx_skb) + 28 + skb_time_offset(_trace_id)))
/* 发送所有时间保存在CB后 */
#define set_tx_skb_ete_delay_trace_time(_tx_skb, _trace_id, _new_time) \
    (ete_tx_skb_delay(_tx_skb,  _trace_id) = _new_time)
...
#define oal_nla_for_each_nested(pos, nla, rem) nla_for_each_nested(pos, nla, rem)
#define oal_nla_ok(_nla, _rem_len) nla_ok(_nla, _rem_len)
...
typedef struct {
    ...
    uint32_t queue_len_max;
    uint32_t period_pkts;  /* 统计周期pps,以便调整napi参数 */
    uint32_t period_start; /* 统计周期起始时间戳 */
...
#define HISI_IEEE80211_BAND_6GHZ NL80211_BAND_6GHZ
#endif
#else
...
#endif
{
    ...
    switch (band) {
        ...
        case NL80211_BAND_6GHZ:
            if (l_channel == 2) { // 6G频段下信道号为2对应中心频率为5935
                return 5935;
            } else {
                return 5950 + l_channel * 5; // 6G频段下信道（除2信道外）对应的频率 = 基数5950 + 信道号 * 5
            }
            break;
#endif
        default:
...
OAL_STATIC OAL_INLINE int32_t oal_ieee80211_frequency_to_channel(int32_t center_freq)
{
    int32_t channel;

    /* see 802.11 17.3.8.3.2 and Annex J */
    if (center_freq == 0) { // 中心频率为0，对应的信道号为0
        channel = 0;
    } else if (center_freq == 2484) { // 2G频段下中心频率为2484对应信道号为14
        channel = 14;
    } else if (center_freq < 2484) { // 2G频段下中心频率小于2484对应的信道号 = （频率 - 2407）/ 5;
        channel = (center_freq - 2407) / 5;
    } else if (center_freq >= 4910 && center_freq <= 4980) {  // 5G频段下中心频率在[4910, 4980]对应的信道号 =(频率 - 4000)/ 5;
        channel = (center_freq - 4000) / 5;
    } else if (center_freq >= 5180 && center_freq <= 5825) {  // 5G频段下中心频率在[5180, 5825]对应的信道号 =(频率 - 5000)/ 5;
        channel = (center_freq - 5000) / 5;
    } else if (center_freq == 5935) { // 6G频段下20M带宽中心频率为5935对应信道号为2
        channel = 2;
    } else if (center_freq >= 5955 && center_freq <= 7115) {  // 6G频段下中心频率在[5955, 7115]对应的信道号 =（频率 - 5950) / 5;
        channel = (center_freq - 5950) / 5;
    } else if (center_freq >= 58320 && center_freq <= 64800) { // 60G频段下中心频率在[58320, 64800]对应的信道号 =(频率 - 56160)/ 2160;
        channel = (center_freq - 56160) / 2160;
    } else { // 中心频率大于64800对应信道号为0
        channel = 0;
    }
    return channel;
}
...
OAL_STATIC OAL_INLINE int32_t oal_net_register_netdev(oal_net_device_stru *p_net_device)
{
    ...
    const uint16_t us_needed_headroom_size = 76; /* 76字节保持与历史项目相同; hcc hdr+asmdu扩展ether头 */
#else
    const uint16_t us_needed_headroom_size = 64; /* 64字节hcc hdr */
#endif
    ...
    oal_netdevice_tailroom(p_net_device) = 32; /* 32字节tail */
 ...
 *             l_align: 需要几字节对齐
 */
OAL_STATIC OAL_INLINE oal_netbuf_stru *_oal_netbuf_alloc(uint32_t ul_size, int32_t l_reserve, uint32_t ul_align)
{
    ...
    if (ul_align) {
        /* 计算为了能使4字节对齐的偏移量 */
        ul_offset = ((uintptr_t)pst_netbuf->data) % ul_align;
        if (ul_offset) {
            skb_reserve (pst_netbuf, (int)(ul_align - ul_offset));
        }
    }
...
OAL_STATIC OAL_INLINE uint32_t _oal_netbuf_free(oal_netbuf_stru *pst_netbuf)
{
    ...
    // 此问题单日志分析看，在关中断的上下文中调用了skb free接口，所以干脆全部修改为any版本
    // 适配PCIE中断上下文释放内存
...
#define oal_netbuf_realloc_tailroom(pst_netbuf, ul_tailroom)                         \
    ({                                                                               \
        oal_netbuf_stru *__no_tmp_skb = NULL;                                \
...
/*
 * 函 数 名  : oal_spin_lock_bh_head
 * 功能描述  : 利用内核skb head原生锁
 * 输入参数  : pst_lock:自旋锁地址
 */
OAL_STATIC OAL_INLINE void oal_spin_lock_head_bh(oal_netbuf_head_stru *netbuf_head)
{
    spin_lock_bh(&netbuf_head->lock);
}

/*
 * 函 数 名  : oal_spin_unlock_bh_head
 * 功能描述  : Spinlock在软中断以及内核线程等核心态上下文环境下的解锁操作。利用内核skb head原生锁
 */
OAL_STATIC OAL_INLINE void oal_spin_unlock_head_bh(oal_netbuf_head_stru *netbuf_head)
{
    spin_unlock_bh(&netbuf_head->lock);
}


OAL_STATIC OAL_INLINE void netbuf_unlink(struct sk_buff *skb, struct sk_buff_head *list)
{
 ...
 */
OAL_STATIC OAL_INLINE int32_t oal_nla_put_u16(oal_netbuf_stru *skb, int32_t attrtype, uint16_t value)
{
 ...
 */
OAL_STATIC OAL_INLINE void *oal_nla_data(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE uint32_t oal_nla_get_u8(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE uint32_t oal_nla_get_u16(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE uint32_t oal_nla_get_u32(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE uint32_t oal_nla_total_size(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE int oal_nla_len(const oal_nlattr_stru *pst_nla)
{
 ...
 */
OAL_STATIC OAL_INLINE int oal_nla_type(const oal_nlattr_stru *pst_nla)
{
