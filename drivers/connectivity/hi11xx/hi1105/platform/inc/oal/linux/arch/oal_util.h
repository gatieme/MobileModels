 ...
 *
 * Description: oal_util_linux.c header file
 * Author: @CompanyNameTag
 */

#ifndef OAL_LINUX_UTIL_H
#define OAL_LINUX_UTIL_H
...
#include <linux/kobject.h> /* add sys for 51 */
...
#define DECLARE_PACKED __attribute__((__packed__))
...
typedef ssize_t oal_ssize_t; /* for sys interface 51 */
...
#define OAL_STRTOK_MAP_SIZE 32

static OAL_INLINE void oal_random_ether_addr(uint8_t *addr, uint8_t len)
{
 ...
 *             ul_count:指定要读取的长度
 */
OAL_STATIC OAL_INLINE int32_t oal_file_read(oal_file_stru *file, char *pc_buf, uint32_t ul_count)
{
...
OAL_STATIC OAL_INLINE int32_t oal_file_read_ext(oal_file_stru *file, oal_file_pos pos, char *pc_buf, uint32_t count)
{
    if (oal_unlikely((file == NULL) || (pc_buf == NULL))) {
        oal_warn_on(1);
        return -OAL_ERR_CODE_READ_FILE_FAIL;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
    return kernel_read(file, pc_buf, count, &pos);
#else
    return kernel_read(file, pos, pc_buf, count);
#endif
}
...
OAL_STATIC OAL_INLINE int8_t *oal_strtok(int8_t *pc_token, const int8_t *pc_delemit, int8_t **ppc_context)
{
    ...
    uint8_t *pc_str = NULL;
    const uint8_t *pc_ctrl = (uint8_t *)pc_delemit;
    uint8_t uc_map[OAL_STRTOK_MAP_SIZE];
    int32_t l_count;
    ...
    /* Clear control map */
    for (l_count = 0; l_count < OAL_STRTOK_MAP_SIZE; l_count++) {
        uc_map[l_count] = 0;
    }
...
OAL_STATIC OAL_INLINE int oal_strtol(const int8_t *pc_nptr, int8_t **ppc_endptr, int32_t l_base)
{
...
OAL_STATIC OAL_INLINE void oal_print_hex_dump(uint8_t *addr, int32_t len, int32_t groupsize, char *pre_str)
{
#ifdef CONFIG_PRINTK
    ...
    print_hex_dump(KERN_INFO, pre_str, DUMP_PREFIX_OFFSET, 16, 1, addr, len, true); /* rowsize is 16内核函数固定传参 */
    printk(KERN_INFO "\n");
#endif
}
 ...
 * 功能描述  : linux int64 除法
 */
OAL_STATIC OAL_INLINE int64_t oal_div_s64(int64_t dividend, int32_t divisor)
{
...
OAL_STATIC OAL_INLINE uint64_t oal_get_monotonic_boottime(void)
{
    ...
    return ((uint64_t)(ts.tv_sec) * OAL_S_TO_US) + ts.tv_nsec / OAL_US_TO_NS;
#else
    return ((uint64_t)tv.tv_sec * OAL_S_TO_US) + tv.tv_usec;
#endif
}
