 ...
 *
 * Description: oal_hcc_bus.c header file
 * Author: @CompanyNameTag
 */

#ifndef OAL_HCC_BUS_H
#define OAL_HCC_BUS_H
...
#define OAL_BUS_MAXCPU_NUM   nr_cpu_ids
...
#define HCC_BUS_PCIE  1 /* mpxx PCIE */
...
#else
#define HCC_BUS_BUTT  (HCC_BUS_PCIE2 + 1)
#endif
...
#else
#define HOST_WAIT_BOTTOM_INIT_TIMEOUT 200000
#endif
...
typedef struct _hcc_bus_opt_ops {
    ...
    int32_t (*poweroff_complete)(hcc_bus *pst_bus);   /* check wifi close complete */
    ...
    int32_t (*change_linkspeed)(hcc_bus *pst_bus, uint32_t request_speed);
    void (*default_pm_ctrl)(hcc_bus *pst_bus, int32_t power_option);
...
int32_t hcc_add_bus(hcc_bus *pst_bus, const char *name);
...
void hcc_bus_sched_flowctrl_gpio_task(hcc_bus* pst_bus, int32_t l_irq);
...
{
    if (oal_warn_on(hi_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_get_state: hibus is null");
        return 0;
    }

    if (oal_warn_on(hi_bus->opt_ops == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_get_state: hi_bus->opt_ops is null");
        return 0;
    }

    if (hi_bus->opt_ops->get_bus_state == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_get_state: hi_bus->opt_ops->get_bus_state is null");
        return 0;
    }
...
OAL_STATIC OAL_INLINE void hcc_bus_and_wake_lock(hcc_bus *hi_bus)
{
    hcc_bus_wake_lock(hi_bus);
    hcc_bus_lock(hi_bus);
}

OAL_STATIC OAL_INLINE void hcc_bus_and_wake_unlock(hcc_bus *hi_bus)
{
    hcc_bus_unlock(hi_bus);
...
OAL_STATIC OAL_INLINE int32_t hcc_bus_wakeup_request(hcc_bus *hi_bus)
{
    ...
    if (oal_warn_on(hi_bus->opt_ops == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_wakeup_request: opt_ops NULL");
        return -OAL_ENODEV;
    }

    if (hi_bus->opt_ops->wakeup_request == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_wakeup_request: opt_ops->wakeup_request NULL");
        return -OAL_EIO;
    }
                                                     ...
                                                     hcc_netbuf_queue_type qtype)
{
    ...
    if (oal_unlikely(!hi_bus)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_tx_netbuf_list: bus is null");
        return -OAL_EINVAL;
    }

    if (oal_likely(hi_bus->opt_ops->tx_netbuf_list)) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "tx netbuf list:%s, skb head:%p, len:%d, qtype:%d",
        ...
        if (oal_unlikely(ret < 0)) {
            ...
            if (g_hcc_tx_err_cnt >= ul_long_delay_tx_err_times) {
                oal_msleep(1000); /* 等待1000ms */
            } else {
                oal_msleep(100); /* 等待100ms */
            }
            return 0;
        } else {
...
{
    if (oal_unlikely(!hi_bus)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc_bus_rx_netbuf_list: bus is null");
        return -OAL_EINVAL;
    }
...
OAL_STATIC OAL_INLINE void hcc_bus_default_pm_ctrl(hcc_bus *hi_bus, int32_t power_option)
{
    if (oal_unlikely(hi_bus == NULL)) {
        return;
    }

    if (oal_unlikely(hi_bus->opt_ops->default_pm_ctrl == NULL)) {
        return;
    }

    hi_bus->opt_ops->default_pm_ctrl(hi_bus, power_option);
}
