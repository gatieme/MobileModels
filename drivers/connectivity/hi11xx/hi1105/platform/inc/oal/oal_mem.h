...
#define GT_OAL_MEM_CONTEXT      (&g_gt_mem_pool_ctx)
#define WIFI_OAL_MEM_CONTEXT    (&g_wifi_mem_pool_ctx)
#define SDT_OAL_MEM_CONTEXT     (&g_sdt_mem_pool_ctx)

#if defined(_PRE_MULTI_INSTANCE_GT)
#define CUR_OAL_MEM_CONTEXT     GT_OAL_MEM_CONTEXT
#elif defined(_PRE_MULTI_INSTANCE_WIFI)
#define CUR_OAL_MEM_CONTEXT     WIFI_OAL_MEM_CONTEXT
#else
#define CUR_OAL_MEM_CONTEXT     NULL
#endif

typedef struct oal_mem_pool_context_stru oal_mem_pool_context_stru;
extern oal_mem_pool_context_stru g_gt_mem_pool_ctx;
extern oal_mem_pool_context_stru g_wifi_mem_pool_ctx;
extern oal_mem_pool_context_stru g_sdt_mem_pool_ctx;
...
#define oal_mem_alloc_m(_en_pool_id, _us_len, _uc_lock) \
    oal_mem_alloc(THIS_FILE_ID, __LINE__, _en_pool_id, _us_len, _uc_lock, CUR_OAL_MEM_CONTEXT)
#define oal_mem_free_m(_p_data, _uc_lock) \
    oal_mem_free(THIS_FILE_ID, __LINE__, _p_data, _uc_lock, CUR_OAL_MEM_CONTEXT)

#define oal_get_mem_flag(_p_data, _puc_state)

#define oal_mem_free_array_m(_p_data, _num) \
    oal_mem_free_array(THIS_FILE_ID, __LINE__, _p_data, _num, CUR_OAL_MEM_CONTEXT)

#define oal_mem_incr_user_m(_pst_mem, _uc_lock) \
    oal_mem_incr_user(THIS_FILE_ID, __LINE__, _pst_mem, _uc_lock, CUR_OAL_MEM_CONTEXT)

#define oal_mem_alloc_enhanced_m(_en_pool_id, _us_len, _uc_lock) \
    oal_mem_alloc_enhanced(THIS_FILE_ID, __LINE__, _en_pool_id, _us_len, _uc_lock, CUR_OAL_MEM_CONTEXT)

#define oal_mem_free_enhanced_m(_pst_mem, _uc_lock) \
    oal_mem_free_enhanced(THIS_FILE_ID, __LINE__, _pst_mem, _uc_lock, CUR_OAL_MEM_CONTEXT)

#define oal_mem_print_normal_pool_info_ctx(en_pool_id, ctx) \
    oal_mem_print_normal_pool_info(en_pool_id, ctx)

#define oal_mem_alloc_enhanced_ctx(THIS_FILE_ID, __LINE__, _en_pool_id, _us_len, _uc_lock, ctx) \
    oal_mem_alloc_enhanced(THIS_FILE_ID, __LINE__, _en_pool_id, _us_len, _uc_lock, ctx)

#define oal_mem_free_enhanced_ctx(THIS_FILE_ID, __LINE__, _pst_mem, _uc_lock, ctx) \
    oal_mem_free_enhanced(THIS_FILE_ID, __LINE__, _pst_mem, _uc_lock, ctx)

#define oal_mem_alloc_ctx(_en_pool_id, _us_len, _uc_lock, ctx) \
    oal_mem_alloc(THIS_FILE_ID, __LINE__, _en_pool_id, _us_len, _uc_lock, ctx)

#define oal_mem_free_ctx(_p_data, _uc_lock, ctx) \
    oal_mem_free(THIS_FILE_ID, __LINE__, _p_data, _uc_lock, ctx)

/* host输出内存信息接口 */
#define oal_mem_info_print(_pool_id)

#define oal_mem_trace_m(_p_data, _uc_lock) \
    oal_mem_trace(THIS_FILE_ID, __LINE__, _p_data, _uc_lock, CUR_OAL_MEM_CONTEXT)

#define oal_mem_trace_enhanced_m(_pst_mem, _uc_lock) \
    oal_mem_trace_enhanced(THIS_FILE_ID, __LINE__, _pst_mem, _uc_lock, CUR_OAL_MEM_CONTEXT)
...
#if defined(CONFIG_ARM64) || defined(_PRE_WINDOWS_SUPPORT)
...
typedef struct {
    ...
    uint32_t (*p_memblock_info_func)(uint8_t *puc_origin_data,
                                       ...
                                       uint8_t uc_subpool_id,
                                       uint16_t us_len,
                                       uint32_t alloc_file_id,
                                       uint32_t alloc_line_num);
 ...
 */
typedef struct {
    uint32_t dog_tag;          /* 狗牌，用于内存写越界检查 */
    uint32_t alloc_file_id;    /* 申请netbuf内存时的文件ID */
    uint32_t alloc_line_num;   /* 申请netbuf内存时的行号 */
    uint32_t alloc_time_stamp; /* 申请netbuf内存时的时间戳 */
    uint32_t trace_file_id;    /* netbuf内存在关键路径上的文件ID */
    uint32_t trace_line_num;   /* netbuf内存在关键路径上的行号 */
    uint32_t trace_time_stamp; /* netbuf内存在关键路径上的时间戳 */
...
struct oal_mem_stru_tag {
    ...
    uintptr_t return_addr; /* alloc内存的函数地址 新增 */

    /* 内存调试信息 */
    uint32_t alloc_core_id[WLAN_MEM_MAX_USERS_NUM];    /* 申请内存时的内核ID */
    uint32_t alloc_file_id[WLAN_MEM_MAX_USERS_NUM];    /* 申请内存时的文件ID */
    uint32_t alloc_line_num[WLAN_MEM_MAX_USERS_NUM];   /* 申请内存时的行号 */
    uint32_t alloc_time_stamp[WLAN_MEM_MAX_USERS_NUM]; /* 申请内存时的时间戳 */
    uint32_t trace_file_id;                            /* 内存在关键路径上的文件ID */
    uint32_t trace_line_num;                           /* 内存在关键路径上的行号 */
    uint32_t trace_time_stamp;                         /* 内存在关键路径上的时间戳 */
...
/* 函数声明 */
extern void *oal_mem_alloc(uint32_t file_id,
                           uint32_t line_num,
                           oal_mem_pool_id_enum_uint8 en_pool_id,
                           uint16_t us_len,
                           uint8_t uc_lock,
                           oal_mem_pool_context_stru *context);
extern uint32_t oal_mem_free(uint32_t file_id, uint32_t line_num, void *pst_mem,
                             uint8_t uc_lock, oal_mem_pool_context_stru *context);

extern uint32_t oal_mem_free_array(uint32_t file_id,
                                   uint32_t line_num,
                                   void *p_data,
                                   uint8_t uc_mem_num,
                                   oal_mem_pool_context_stru *context);

extern oal_mem_stru *oal_mem_alloc_enhanced(uint32_t file_id,
                                            uint32_t line_num,
                                            oal_mem_pool_id_enum_uint8 en_pool_id,
                                            uint16_t us_len,
                                            uint8_t uc_lock,
                                            oal_mem_pool_context_stru *context);
extern uint32_t oal_mem_free_enhanced(uint32_t file_id, uint32_t line_num,
                                      oal_mem_stru *p_data, uint8_t uc_lock, oal_mem_pool_context_stru *context);
extern uint32_t oal_mem_init_pool(void);
extern void oal_mem_exit(void);
extern uint32_t oal_mem_incr_user(uint32_t file_id,
                                  uint32_t line_num,
...
extern void oal_mem_start_stat_ctx(oal_mem_pool_context_stru *context);
extern uint32_t oal_mem_end_stat_ctx(oal_mem_pool_context_stru *context);

extern uint32_t oal_mem_trace_enhanced(uint32_t file_id,
                                       uint32_t line_num,
                                       oal_mem_stru *pst_mem,
                                       uint8_t uc_lock,
                                       oal_mem_pool_context_stru *context);
extern uint32_t oal_mem_trace(uint32_t file_id,
                              uint32_t line_num,
                              const void *p_data,
                              uint8_t uc_lock,
                              oal_mem_pool_context_stru *context);
...
extern oal_mem_pool_stru *oal_mem_get_pool(oal_mem_pool_id_enum_uint8 en_pool_id, oal_mem_pool_context_stru *context);
...
extern void oal_mem_print_normal_pool_info(oal_mem_pool_id_enum_uint8 en_pool_id, oal_mem_pool_context_stru *context);
extern void oal_mem_print_pool_info(void);
extern void oal_mem_print_funcname(uintptr_t func_addr);

#define oal_netbuf_duplicate_normal(pst_src_netbuf, add_head_room, add_tail_room) \
    oal_netbuf_duplicate(pst_src_netbuf, OAL_NORMAL_NETBUF, add_head_room, add_tail_room)
