...
int32_t oal_trigger_bus_exception(hcc_bus *hi_bus, int32_t is_sync)
{
    ...
    timeout_jiffies = jiffies + oal_msecs_to_jiffies(2000); /* 2000ms for wait */
    forever_loop() {
        if (hcc_bus_exception_is_busy(hi_bus) == OAL_TRUE) {
            break;
        }
...
OAL_STATIC uint32_t hcc_bus_get_exception_mem_info_gf61(hcc_bus *hi_bus)
{
    ...
    if (is_gt) {
        ...
        size = sizeof(g_gf61_pilot_gt_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    } else {
        mem_info = g_gf61_pilot_wifi_meminfo;
        size = sizeof(g_gf61_pilot_wifi_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    }
...
int32_t hcc_bus_get_exception_mem_info(hcc_bus *hi_bus)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        if (mpxx_is_asic()) {
            ...
            size = sizeof(g_mp15_pilot_asic_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        } else {
            /* mp15 Fpga */
            mem_info = g_mp15_pilot_fpga_meminfo;
            size = sizeof(g_mp15_pilot_fpga_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        }
        ...
        if (mpxx_is_asic()) {
            ...
            size = sizeof(g_mp16_pilot_asic_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        } else {
            /* mp16 Fpga */
            mem_info = g_mp16_pilot_fpga_meminfo;
            size = sizeof(g_mp16_pilot_fpga_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        }
        ...
        size = sizeof(g_mp16c_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        ...
        size = sizeof(g_mp13_pilot_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    } else {
...
OAL_STATIC int32_t hcc_bus_resource_alloc(hcc_bus *pst_bus)
{
    ...
    if (wakelock_name == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc wakelock name space failed, len=%u, [%s]", len, name);
        return -OAL_ENOMEM;
    }

    ret = memcpy_s((void *)wakelock_name, len, (const void *)name, len);
    if (unlikely(ret != EOK)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "memcpy_s failed ret=%d", ret);
