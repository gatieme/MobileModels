...
OAL_STATIC uint32_t g_ft_pcie_loopback_bias_cldo3 = 900;        /* 0.9v */
OAL_STATIC uint32_t g_ft_pcie_loopback_bias_vptx = 900;         /* 0.9v */
OAL_STATIC uint32_t g_ft_pcie_loopback_bias_vph = 1800;         /* 1.8v */
oal_debug_module_param(g_ft_pcie_loopback_rw_test_count, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_loopback_linkup_timeout, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_loopback_bias_cldo3, uint, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_loopback_bias_vptx, uint, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_loopback_bias_vph, uint, S_IRUGO | S_IWUSR);
#endif
...
OAL_STATIC int32_t hcc_flowctrl_info_print_ext(char *buf, int32_t buf_len)
{
    ...
    if ((hi_sdio != NULL) && (hi_sdio->func->card->host->claimer != NULL)) {
        ...
        ret = snprintf_s(buf, buf_len, buf_len - 1, "claim host name:%s\n",
...
static int32_t hcc_queues_info_print_total_pkts(struct hcc_handler *hcc, char *buf, int32_t buf_len)
{
    int i, j;
    int32_t ret;
    uint64_t total;
...
static int32_t hcc_queues_info_print_flowctrl(struct hcc_handler *hcc, char *buf, int32_t buf_len)
{
    int32_t count = 0;
    int j;
    int32_t ret;
...
static int32_t hcc_queues_info_print_thread_info(struct hcc_handler *hcc, char *buf, int32_t buf_len)
{
    int32_t count = 0;
...
OAL_STATIC int32_t hcc_queues_pkts_info_print(void *data, char *buf, int32_t buf_len)
{
    ...
    ret = hcc_queues_info_print_total_pkts(hcc, buf + count, buf_len - count);
    count += ret;
    if (buf_len <= count) {
        return count;
    }

    ret = hcc_queues_info_print_flowctrl(hcc, buf + count, buf_len - count);
    count += ret;
    if (buf_len <= count) {
        return count;
    }

    ret = hcc_queues_info_print_thread_info(hcc, buf + count, buf_len - count);
    count += ret;
    if (buf_len <= count) {
        return count;
    }
                                            ...
                                            const char *buf, size_t count)
{
    ...
    if ((count == 0) || (count >= sizeof(lockname)) || (buf[count] != '\0')) {
        oal_io_print("input illegal!%s\n", __FUNCTION__);
        return -OAL_EINVAL;
    }

    memset_s(lockname, sizeof(lockname), 0, sizeof(lockname));
...
OAL_STATIC void hcc_test_data_rx_print(void)
{
    int32_t ret;

    if (!g_hcc_test_event->rx_stop) {
        oal_io_print("RxTestTimeIsUp\n");
        ret = hcc_send_message(hcc_get_handler(HCC_EP_WIFI_DEV), H2D_MSG_STOP_SDIO_TEST);
        if (ret) {
...
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
    if ((cmd.cmd_type == HCC_TEST_CMD_PCIE_MAC_LOOPBACK_TST) ||
        ...
        ret = memcpy_s(&g_pcie_test_request_ack, sizeof(g_pcie_test_request_ack),
                       ...
                       sizeof(g_pcie_test_request_ack));
        if (ret != EOK) {
            oal_io_print("memcpy_s err!\n");
        }
        oal_complete(&g_pcie_test_trans_done);
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie test done type=%s",
...
static int32_t hcc_test_pcie_loopback_resp(hcc_pcie_test_request_stru *pcie_test_rq,
                                           int32_t is_phy_loopback)
{
    int32_t ret = OAL_SUCC;
    if (pcie_test_rq->request_flag != g_pcie_test_request_ack.request_flag) {
        ret = -OAL_EFAIL;
        oal_print_mpxx_log(MPXX_LOG_WARN, "pcie test request mismatch request 0x%llx , return 0x%llx",
                           pcie_test_rq->request_flag,
                           g_pcie_test_request_ack.request_flag);
    } else {
        if (g_pcie_test_request_ack.response_ret == OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_test_ok linkup cost %u us , rw_test cost %u us , total %u us",
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "pcie %s test %s", is_phy_loopback ? "phy" : "mac",
                       (ret == OAL_SUCC) ? "succ" : "failed");
    return ret;
}

static int32_t hcc_test_pcie_loopback_vol_init(hcc_pcie_test_request_stru *pcie_test_rq)
{
    int32_t ret;
    uint32_t vol = 0;
...
OAL_STATIC int32_t hcc_test_pcie_loopback(int32_t is_phy_loopback)
{
    ...
    memset_s(&g_pcie_test_request_ack, sizeof(g_pcie_test_request_ack), 0, sizeof(g_pcie_test_request_ack));
    g_pcie_test_request_ack.response_ret = HCC_PCIE_RESPONSE_INITIAL_VALUE;

    cmd.cmd_type = map_cmd_type(is_phy_loopback);
    cmd.cmd_len = sizeof(hcc_test_cmd_stru);

    pst_netbuf = hcc_netbuf_alloc(cmd.cmd_len + sizeof(pcie_test_rq));
    if (pst_netbuf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc %u mem failed!", cmd.cmd_len);
        return -OAL_ENOMEM;
    }
    ...
    ret = hcc_test_pcie_loopback_vol_init(&pcie_test_rq);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(pst_netbuf);
        return ret;
    }
    ret = memcpy_s((void *)hcc_get_test_cmd_data(oal_netbuf_data(pst_netbuf)),
                   sizeof(pcie_test_rq), (void *)&pcie_test_rq, sizeof(pcie_test_rq));
    if (ret != EOK) {
        oal_netbuf_free(pst_netbuf);
    ...
    ret = hcc_tx(hcc, pst_netbuf, &st_hcc_transfer_param);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie test send failed ret=%d", ret);
        oal_netbuf_free(pst_netbuf);
        return ret;
    }
    ret = wait_for_completion_interruptible_timeout(&g_pcie_test_trans_done, oal_msecs_to_jiffies(15000)); // 15000 ms
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie test Event  terminated ret=%d\n", ret);
        oal_dft_print_all_key_info();
        return ret;
    } else if (ret == 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pcie test timeout");
        oal_dft_print_all_key_info();
        return -OAL_ETIMEDOUT;
    }
    ret = hcc_test_pcie_loopback_resp(&pcie_test_rq, is_phy_loopback);
    return ret;
}
OAL_STATIC ssize_t hcc_test_pcie_loopback_get_para(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    if (attr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "attr is null");
        return 0;
    }
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost linkup %u us  %u ms\n",
                     g_pcie_test_request_ack.link_up_cost, g_pcie_test_request_ack.link_up_cost / USEC_PER_MSEC);
    if (ret < 0) {
        return count;
    }
    count += ret;
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost rw_test_cost %u us  %u ms\n",
                     g_pcie_test_request_ack.rw_test_cost, g_pcie_test_request_ack.rw_test_cost / USEC_PER_MSEC);
    if (ret < 0) {
        return count;
    }
    count += ret;
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "TimeCost total_test_cost %u us  %u ms\n",
                     g_pcie_test_request_ack.total_test_cost, g_pcie_test_request_ack.total_test_cost / USEC_PER_MSEC);
    if (ret < 0) {
        return count;
    }
    count += ret;
...
static int32_t hcc_test_pcie_loopback_set_para_input_check(struct kobject *dev, struct kobj_attribute *attr,
                                                           const char *buf, size_t count,
                                                           size_t max_count)
{
    if (buf == NULL) {
    ...
    if ((count == 0) || (count >= max_count) || (buf[count] != '\0')) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "input illegal!\n");
        return -OAL_EFAUL;
    }
...
static int32_t hcc_test_pcie_loopback_set_param_input_check(uint32_t vph, uint32_t cldo3,
                                                            uint32_t vptx, uint32_t *vol)
{
    int32_t ret = oal_pcie_get_vol_reg_1v8_value(vph, vol);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
static int32_t hcc_test_pcie_loopback_get_phy_loop_type(const char *test_case, int32_t *is_phy_loopback)
{
                                                   ...
                                                   const char *buf, size_t count)
{
    ...
    char test_case[128]; /* sys文件允许的最大输入字符长度128 */

    ret = hcc_test_pcie_loopback_set_para_input_check(dev, attr, buf, count, sizeof(test_case));
    if (ret != OAL_SUCC) {
        return 0;
    }
    ...
    if (!oal_strncmp("setparam", buf, OAL_STRLEN("setparam"))) {
        ...
        if ((sscanf_s(buf, "%u %u %u", &cldo3, &vptx, &vph) != 3)) { // 3 args
            oal_print_mpxx_log(MPXX_LOG_ERR, "error input:%s", buf);
            return count;
        }
        ret = hcc_test_pcie_loopback_set_param_input_check(vph, cldo3, vptx, &vol);
        if (ret != OAL_SUCC) {
            return count;
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "setbias vph:%umv cldo3:%umv vptx:%umv", vph, cldo3, vptx);
        ...
        ret = hcc_test_pcie_loopback_get_phy_loop_type(test_case, &is_phy_loopback);
        if (ret != OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "error input:%s", buf);
            return count;
        }
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "unkown:%s\n", buf);
    }
    return count;
}
...
OAL_STATIC int32_t hcc_test_fix_wcpu_freq(void)
{
    ...
    if (ret != EOK) {
        oal_netbuf_free(pst_netbuf);
        oal_print_mpxx_log(MPXX_LOG_ERR, "copy cmd to netbuf failed!");
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t hcc_test_normal_start(uint16_t start_cmd)
{
    ...
    hcc = hcc_get_handler(HCC_EP_WIFI_DEV);
    if (hcc == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "wifi is closed!");
        return -OAL_EFAIL;
    }
    ...
    /* 50ms wait the device recv the cmd,change the test mode! */
    oal_msleep(50);
...
OAL_STATIC ssize_t hcc_test_print_thoughput(char *buf, uint32_t buf_len)
{
    ...
    if (buf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "buf is null");
        return 0;
    }
                                          ...
                                          const char *buf, size_t count)
{
    ...
    wlan_pm_disable();
    hcc_bus_send_message(hcc_get_bus(HCC_EP_WIFI_DEV), H2D_MSG_TEST);
    wlan_pm_enable();
    return count;
}
...
OAL_STATIC ssize_t hcc_test_set_switch(struct kobject *dev, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    if ((count == 0) || (count >= sizeof(input)) || (buf[count] != '\0')) {
        oal_io_print("input illegal!%s\n", __FUNCTION__);
        return -OAL_EINVAL;
    }

    memset_s((void *)input, sizeof(input), 0, sizeof(input));
...
#if defined(_PRE_PLAT_FEATURE_HI110X_PCIE) || defined(CONFIG_MMC)
OAL_STATIC int32_t hcc_test_current_bus_chan(int32_t pkt_len, int32_t mode_idx,
                                             unsigned long trans_timeout, uint32_t min_throught,
                                             int32_t retry_cnt)
{
    int32_t ret;
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "set  wcpu fix freq failed! ret=%d", ret);
        wlan_pm_close();
        return ret;
    }

    /* 电压拉偏 */
    hcc_bus_voltage_bias_init(hcc_get_bus(HCC_EP_WIFI_DEV));

retry:

    ret = hcc_test_set_case(&data);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "conn_test_wifi_chan_loop test failed!");
        wlan_pm_close();
        return ret;
    }
    ...
    if (pst_buff != NULL) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc mem failed!");
    }
    ...
    if (min_throught > 0) {
        ...
        if (g_hcc_test_event->test_data.throughput < (uint64_t)min_throught) {
            ...
            oal_print_mpxx_log(MPXX_LOG_INFO, "throught %llu Mbps over limit %u Mbps, retry times %d, test pass!",
                               g_hcc_test_event->test_data.throughput,
                               min_throught,
                               retry_times);
        }
    ...
    hcc_bus_chip_info(hcc_get_bus(HCC_EP_WIFI_DEV), OAL_TRUE, OAL_TRUE);

    if (wlan_pm_close() != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "conn_test_wifi_chan_loop test failed! ret=%d", ret);
    }
...
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
static int32_t conn_test_wifi_chan_pcie(int32_t pkt_len)
{
    int32_t ret;
    ...
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "test PCIe failed, ret=%d", ret);
        return ret;
    }
    ...
    if (get_cust_conf_int32(INI_MODU_PLAT, "pcie_min_throught", &pcie_min_throught) == INI_SUCC) {
        ...
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie min throught %d Mbps from ini", pcie_min_throught);
    }
...
#ifdef CONFIG_MMC
static int32_t conn_test_wifi_chan_sdio(int32_t pkt_len)
{
    int32_t ret;
    hcc_bus *pst_bus = hcc_get_bus(HCC_EP_WIFI_DEV);
    if (pst_bus == NULL) {
    ...
    if (pst_bus->bus_type != HCC_BUS_SDIO) {
        /* 尝试切换到SDIO */
        ret = hcc_switch_bus(HCC_EP_WIFI_DEV, HCC_BUS_SDIO);
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "switch to PCIe failed, ret=%d", ret);
            return -OAL_ENODEV;
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "switch to SDIO ok.");
    }
...
static int32_t conn_test_wifi_chan_restore(hcc_bus *old_bus)
{
    int32_t ret;
    hcc_bus *pst_bus = hcc_get_bus(HCC_EP_WIFI_DEV);
    if ((pst_bus == NULL) || (old_bus == NULL)) {
    ...
    if (old_bus->bus_type != pst_bus->bus_type) {
        ...
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "restore to %u failed, ret=%d", old_bus->bus_type, ret);
            return -OAL_ENODEV;
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "restore to %d ok.", old_bus->bus_type);
    }
...
int32_t conn_test_wifi_chan_loop(const char *param)
{
    ...
    if (oal_pcie_mpxx_working_check() == OAL_TRUE) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "PCIe Chan Test!");
        oal_get_time_cost_start(cost);
        ret = conn_test_wifi_chan_pcie(pkt_len);
        if (ret != OAL_SUCC) {
            (void)conn_test_wifi_chan_restore(old_bus);
            return ret;
        }
        oal_get_time_cost_end(cost);
        oal_calc_time_cost_sub(cost);
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie loop cost %llu us", time_cost_var_sub(cost));
        run_flag++;
    }
#endif
    ...
    if (oal_sdio_mpxx_working_check() == OAL_TRUE) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "SDIO Chan Test!");
        oal_get_time_cost_start(cost);
        ret = conn_test_wifi_chan_sdio(pkt_len);
        if (ret != OAL_SUCC) {
            (void)conn_test_wifi_chan_restore(old_bus);
            return ret;
        }
        oal_get_time_cost_end(cost);
        oal_calc_time_cost_sub(cost);
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio loop cost %llu us", time_cost_var_sub(cost));
        run_flag++;
    }
#endif

    ret = conn_test_wifi_chan_restore(old_bus);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    if (run_flag) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "no wifi chan is runing...");
        return -1;
    }
}
...
{
#if defined(_PRE_PLAT_FEATURE_HI110X_PCIE) && defined(_PRE_CONFIG_GPIO_TO_SSI_DEBUG)
    hcc_bus *pst_bus = NULL;

    g_mpxx_kernel_crash = 1;
...
#elif (defined(CONFIG_KIRIN_PCIE_NOC_DBG) || defined(CONFIG_PCIE_KPORT_NOC_DBG))
...
int32_t hcc_test_init_module(struct hcc_handler *hcc)
{
    ...
    if ((is_gt_support()) && (hcc->bus_dev->dev_id == HCC_EP_GT_DEV)) {
        return hcc_test_init_module_gt(hcc);
    }
#endif
#if (defined(CONFIG_KIRIN_PCIE_NOC_NOTIFY))
...
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_GF61_HOST)
    if ((is_gt_support()) && (hcc->bus_dev->dev_id == HCC_EP_GT_DEV)) {
        return hcc_test_exit_module_gt(hcc);
    }
#endif
#ifdef _PRE_CONFIG_HISI_PANIC_DUMP_SUPPORT
