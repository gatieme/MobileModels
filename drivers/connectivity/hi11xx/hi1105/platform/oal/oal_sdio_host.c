...
#define MPXX_LOG_MODULE_NAME     "[SDIO]"
#define MPXX_LOG_MODULE_NAME_VAR sdio_loglevel
#include "oal_util.h"
#include "oal_sdio.h"
...
#define MEM_DMA_BLOCLALLOC_TIMEOUT 5000
...
#if defined(CONFIG_MMC_SDHCI_ZODIAC) || defined(CONFIG_MMC_DW_HI3XXX)
void dw_mci_sdio_card_detect_change(void);
#elif defined(_PRE_HI375X_SDIO)
int sdhci_nebula_sdio_rescan(int index);
#endif

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
void mmc_power_up(struct mmc_host *host, uint32_t ocr);
...
{
    oal_print_mpxx_log(MPXX_LOG_DBG, "mpxx mmc detect, vendor id:0x%x  product id:0x%x",
                       HISDIO_VENDOR_ID_MP13, HISDIO_PRODUCT_ID_HISI);
}
#else
OAL_STATIC void oal_sdio_detectcard_to_core(int32_t val)
{
...
int32_t oal_sdio_send_msg(hcc_bus *pst_bus, uint32_t val)
{
    ...
    if (oal_unlikely(hi_sdio == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: hi_sdio is null", __FUNCTION__);
        return -OAL_EINVAL;
    }

    if (oal_warn_on(hi_sdio->func == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: !hi_sdio->func is null", __FUNCTION__);
        return -OAL_EINVAL;
    }

    func = hi_sdio->func;
    oal_print_mpxx_log(MPXX_LOG_DBG, "send msg to notice device [0x%8x]", (uint32_t)val);
    if (val >= H2D_MSG_COUNT) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[Error]invaild param[%u]!", val);
        return -OAL_EINVAL;
    }
...
#else
    mmc_power_off(hi_sdio->func->card->host);
#endif
#endif
...
OAL_STATIC int32_t oal_sdio_hcc_bus_power_patch_lauch(hcc_bus *pst_bus)
{
    ...
    if (oal_wait_for_completion_timeout(&pst_bus->st_device_ready,
        ...
        oal_print_mpxx_log(MPXX_LOG_WARN, "wait first device ready timeout... %d ms ",
                           HOST_WAIT_BOTTOM_INIT_TIMEOUT);
        up(&pst_bus->rx_sema);

        if (oal_wait_for_completion_timeout(&pst_bus->st_device_ready,
            (uint32_t)oal_msecs_to_jiffies(15000)) == 0) { // 15000, 15S for device boot
            oal_print_mpxx_log(MPXX_LOG_WARN, "retry 5 second hold, still timeout");
            return -OAL_ETIMEDOUT;
        } else {
            /* 强制调度成功，说明有可能是GPIO中断未响应 */
            oal_print_mpxx_log(MPXX_LOG_WARN, KERN_WARNING "[E]retry succ, maybe gpio interrupt issue");
...
OAL_STATIC int32_t sdio_dev_init(struct oal_sdio *hi_sdio)
{
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to enable sdio function! ret=%d", ret);
    }

    ret = sdio_set_block_size(func, HISDIO_BLOCK_SIZE);
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to set sdio blk size! ret=%d", ret);
    }
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to enable sdio interrupt! ret=%d", ret);
    }

    oal_sdio_release_host(hi_sdio);

    oal_print_mpxx_log(MPXX_LOG_INFO, "sdio function %d enabled.", func->num);

    return ret;
}

void oal_sdio_try_bindcpu(uint32_t irq_id, uint32_t cpu_id)
{
#if defined(CONFIG_NR_CPUS) &&  defined(_PRE_HISI_BINDCPU)
    irq_set_affinity_hint(irq_id, cpumask_of(cpu_id));
#endif
}
...
OAL_STATIC int32_t oal_sdio_bindcpu(hcc_bus *pst_bus, uint32_t chan, int32_t is_bind)
{
    ...
    if (pst_bus->pst_rx_tsk != NULL) {
        ...
        if (is_bind) {
            set_cpus_allowed_ptr(pst_bus->pst_rx_tsk, &fast_cpus);
        } else {
            set_cpus_allowed_ptr(pst_bus->pst_rx_tsk, &slow_cpus);
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "wlan bus rx thread to cpu 0x%lx",
            is_bind ? cpumask_bits(&fast_cpus)[0] : cpumask_bits(&slow_cpus)[0]);
    }
#endif
#endif
#endif
...
OAL_STATIC int32_t oal_sdio_mmc_power_reinit(struct oal_sdio *hi_sdio)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_ERR, "no mmc_power_save_host in linux5.4");
    return -OAL_EFAIL;
#else
    mmc_power_off(hi_sdio->func->card->host);
    hi_sdio->func->card->host->pm_flags &= ~MMC_PM_KEEP_POWER;
    mmc_power_up(hi_sdio->func->card->host, hi_sdio->func->card->ocr);
    ret = mmc_sdio_reinit_card(hi_sdio->func->card->host);
    hi_sdio->func->card->host->pm_flags |= MMC_PM_KEEP_POWER;
#endif

    if (ret < OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to mmc_power_restore_host ret=%d", ret);
        ...
        if (g_wifi_sdio_fail_count >= g_wifi_sdio_fail_painc_limit - 1) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "sdio host ip reset flag=1");
            g_sdio_reset_ip = 1;
        } else if (g_wifi_sdio_fail_count >= g_wifi_sdio_fail_painc_limit) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "sdio cause kernel panic");
        } else {
            oal_print_mpxx_log(MPXX_LOG_ERR, "sdio dev init failed, fail count %u , limit %u",
...
{
    int32_t ret = sdio_dev_init(hi_sdio);
    if (ret) {
        (void)ssi_dump_err_regs(SSI_ERR_SDIO_REINIT_FAIL);
        oal_sdio_release_host(hi_sdio);
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio dev reinit failed ret =%d", ret);
...
OAL_STATIC int32_t oal_sdio_reinit(hcc_bus *pst_bus)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "start to power restore sdio");
    time_start = ktime_get();

    oal_print_mpxx_log(MPXX_LOG_INFO, "wake_sema_count=%d", pst_bus->sr_wake_sema.count);
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "sdio_dev_init ok cost %llu us", trans_us);
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_xfercount_get(struct oal_sdio *hi_sdio, uint32_t *xfercount)
{
    ...
    *xfercount = oal_sdio_readl(hi_sdio->func, HISDIO_REG_FUNC1_XFER_COUNT, &ret);
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[ERROR]sdio read single package len failed ret=%d", ret);
        return ret;
    }
    ...
    *xfercount = oal_sdio_readl(hi_sdio->func, HISDIO_REG_FUNC1_XFER_COUNT, &ret);
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[E]sdio read xercount failed ret=%d", ret);
...
OAL_STATIC int32_t oal_sdio_data_sg_irq(struct oal_sdio *hi_sdio)
{
    ...
    if (oal_unlikely(oal_sdio_check_rx_len(hi_sdio, xfer_count) != OAL_SUCC)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[Error]Sdio Rx Single Transfer len[%u] invalid", xfer_count);
    }

    /* beacuse get buf may cost lot of time, so release bus first */
    if (oal_unlikely(hi_sdio->pst_bus->bus_ops.rx == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: !hi_sdio->pst_bus->bus_ops.rx is null", __FUNCTION__);
        return -OAL_EFAIL;
    }
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_msg_stat(struct oal_sdio *hi_sdio, uint32_t *msg)
{
    ...
    *msg = oal_sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_MSG_FROM_DEV, &ret);
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio readb error![ret=%d]", ret);
        return ret;
    }
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[E]sdio readb error![ret=%d]", ret);
...
OAL_STATIC int32_t oal_sdio_msg_irq(struct oal_sdio *hi_sdio)
{
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_sdio_msg_stat error![ret=%d]", ret);
        return ret;
    }
    msg64 = (unsigned long)msg;

    oal_print_mpxx_log(MPXX_LOG_DBG, "sdio message:0x%8x ", msg);
    ...
    if (!!((1 << D2H_MSG_FLOWCTRL_OFF) & msg) & !!((1 << D2H_MSG_FLOWCTRL_ON) & msg)) {
        oal_print_mpxx_log(MPXX_LOG_WARN, "[ERROR]on/off should come at the same time!");
        oal_warn_on(1);
    }
#endif
    ...
    if (test_and_clear_bit(D2H_MSG_DEVICE_PANIC, &msg64)) {
        ...
        if (hi_sdio->pst_bus->msg[bit].msg_rx) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "device panic msg come, 0x%8x", msg);
            hi_sdio->pst_bus->msg[bit].msg_rx(hi_sdio->pst_bus->msg[bit].data);
        }
    }

    bit = 0;
    /* 表示以第一个参数为迭代位下标，遍历第二个参数地址中bit0~bit15中所有置1的位 */
    for_each_set_bit(bit, (const unsigned long *)&msg64, 16)
    {
        if (oal_unlikely(bit > 15)) { /* 参数地址的大小为bit0~bit15 */
            oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: bit:%d >  5", __FUNCTION__, bit);
            continue;
        };
        hi_sdio->pst_bus->msg[bit].count++;
        hi_sdio->pst_bus->last_msg = bit;
...
OAL_STATIC uint32_t oal_sdio_credit_info_update(struct oal_sdio *hi_sdio)
{
    ...
    if (hi_sdio->sdio_credit_info.short_free_cnt != short_free_cnt) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "short free cnt:%d ==> %d\r",
    ...
    if (hi_sdio->sdio_credit_info.large_free_cnt != large_free_cnt) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "large free cnt:%d ==> %d\r",
...
#ifdef CONFIG_SDIO_DEBUG
OAL_STATIC inline void oal_sdio_extend_buf_get_debug(struct oal_sdio *hi_sdio)
{
...
#ifdef CONFIG_SDIO_DEBUG
        oal_sdio_extend_buf_get_debug(hi_sdio);
#endif
        } else {
...
OAL_STATIC void oal_sdio_transfer_rx_reserved_buff(struct oal_sdio *hi_sdio)
{
    int32_t i, ret;
    uint32_t left_size, seg_nums, seg_size;
    ...
    if (hi_sdio->scatt_info[SDIO_READ].max_scatt_num < seg_nums) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio seg nums :%u large than rx scatt num %u", seg_nums,
                           hi_sdio->scatt_info[SDIO_READ].max_scatt_num);
        return;
    }

    oal_print_mpxx_log(MPXX_LOG_WARN, "drop the rx buff length:%u", ul_extend_len);
    ...
    for_each_sg(sg, sg_t, seg_nums, i)
    {
        if (oal_unlikely(sg_t == NULL)) {
            oal_print_mpxx_log(MPXX_LOG_WARN, "sg_t==null");
            return;
        }
    ...
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio trans revered mem failed! ret=%d", ret);
    }
}
...
OAL_STATIC int32_t oal_sdio_build_rx_netbuf_list(struct oal_sdio *hi_sdio, oal_netbuf_head_stru *head)
{
    ...
    if (oal_unlikely(!oal_netbuf_list_empty(head))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: head is not null", __FUNCTION__);
        return -OAL_EINVAL;
    }
    ...
    for (i = 0; i < HISDIO_EXTEND_REG_COUNT; i++) {
        ...
        if (netbuf == NULL) {
            ...
            oal_print_mpxx_log(MPXX_LOG_ERR, "rx no mem:%u, index:%d", buff_len, i);
        ...
        if (oal_unlikely(!oal_netbuf_head_next(head))) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: head next is null", __FUNCTION__);
            return -OAL_EINVAL;
        }
        if (oal_unlikely(!oal_netbuf_head_prev(head))) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: head prev is null", __FUNCTION__);
            return -OAL_EINVAL;
        }
    ...
    if (oal_warn_on(hisdio_align_4_or_blk(sum_len) != hi_sdio->sdio_extend->xfer_count)) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "scatt total len[%u] should = xfercount[%u],after pad len:%u",
    ...
    if (netbuf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "rx no mem:%u", hi_sdio->sdio_extend->xfer_count);
        declare_dft_trace_key_info("sdio_rx_no_mem", OAL_DFT_TRACE_OTHER);
        goto failed_netbuf_alloc;
    }
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_get_func1_int_status(struct oal_sdio *hi_sdio, uint8_t *int_stat)
{
    ...
    *int_stat = oal_sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_INT_STATUS, &ret);
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to read sdio func1 interrupt status!ret=%d\n", ret);
        return ret;
    }
    ...
    if (g_sdio_extend_func) {
        ...
        *int_stat = oal_sdio_readb(hi_sdio->func, HISDIO_REG_FUNC1_INT_STATUS, &ret);
        if (oal_unlikely(ret)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "failed to read sdio func1 interrupt status!ret=%d", ret);
            return ret;
        }
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_clear_int_status(struct oal_sdio *hi_sdio, uint8_t int_stat)
{
    ...
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "faild to clear sdio func1 interrupt!ret=%d", ret);
        return ret;
    }
#else
...
OAL_STATIC int32_t oal_sdio_do_isr(struct oal_sdio *hi_sdio)
{
    ...
    if (oal_unlikely(OAL_TRUE != oal_sdio_get_state(hi_sdio->pst_bus, OAL_BUS_STATE_RX))) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio closed,state:%u",
    ...
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to read sdio extend area ret=%d", ret);
        return -OAL_EFAIL;
    }
#endif
    ...
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "handle sdio interrupt mask:%d", int_mask);

    if (oal_unlikely((int_mask & HISDIO_FUNC1_INT_MASK) == 0)) {
        oal_print_mpxx_log(MPXX_LOG_VERBOSE, "no sdio interrupt occur[%u], unavailable %s interrupt",
                           int_mask, g_hisdio_intr_mode ? "gpio" : "sdio");
    ...
    if (int_mask & HISDIO_FUNC1_INT_RERROR) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio func1 interrupt Error, try to read the data anyway");
    ...
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "succeed to handle sdio irq");
    return OAL_SUCC;
}
...
OAL_STATIC void oal_sdio_isr(struct sdio_func *func)
{
    ...
    if (oal_unlikely(!func)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: func is null", __FUNCTION__);
        return;
    }

    hi_sdio = sdio_get_drvdata(func);
    if (hi_sdio == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_sdio is NULL");
        return;
    }
    ...
    do {
        ...
        if (oal_unlikely(ret)) {
            hcc_bus_exception_submit(hi_sdio->pst_bus, HCC_BUS_TRANS_FAIL);
...
OAL_STATIC int32_t oal_sdio_interrupt_register(struct oal_sdio *hi_sdio)
{
    ...
    if (!g_hisdio_intr_mode) {
        ...
        if (ret < 0) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "failed to register sdio interrupt");
            oal_sdio_release_host(hi_sdio);
            return -OAL_EFAIL;
        }
        oal_sdio_release_host(hi_sdio);
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio interrupt register!");
...
OAL_STATIC int32_t oal_sdio_get_sleep_reg(struct oal_sdio *hi_sdio)
{
    ...
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "read func0 reg 0x%x failed, ret=%d", HISDIO_WAKEUP_DEV_REG, ret);
    }
...
OAL_STATIC int32_t oal_sdio_sleep_request(hcc_bus *pst_bus)
{
    struct oal_sdio *hi_sdio = (struct oal_sdio *)pst_bus->data;
    if (oal_warn_on(hi_sdio == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hi_sdio is null");
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_sdio_writesb_by_heap(struct oal_sdio *hi_sdio, uint8_t *buf, uint32_t len)
{
    ...
    if (virt_addr_valid(buf) && !object_is_on_stack(buf)) {
        ...
        if (ret < 0) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "write error:%d\n", ret);
        }
        ...
        if ((len < HISDIO_SEND_SIZE) && (hi_sdio->sdio_send_buff != NULL)) {
            ...
            if (memcpy_s(send_buf, len, buf, len) != EOK) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "memcpy_s failed\n");
                return -OAL_EINVAL;
            }
            ...
            if (send_buf == NULL) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "malloc len %d failed\n", len);
                return -OAL_ENOMEM;
            }
...
OAL_STATIC int32_t oal_sdio_patch_writesb(struct oal_sdio *pst_sdio, uint8_t *buf, uint32_t len)
{
    ...
    if (func == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Sdio device is NOT initialized");
        return -OAL_EIO;
    }

    if (buf == NULL || len == 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Write buf is NULL");
        return -OAL_EINVAL;
    }
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_sdio_writesb error:%d", ret);
    }
                                         ...
                                         uint32_t len, uint32_t timeout)
{
    ...
    if (buf == NULL || len == 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Invalid NULL read buf");
    ...
    forever_loop() {
        ...
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "read int mask fail, ret=%d", ret);
            oal_sdio_release_host(pst_sdio);
            return -OAL_EFAIL;
        }
        ...
        if (time_after(jiffies, timeout_jiffies)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "read int mask timeout, int_mask=%x", int_mask);
            oal_sdio_release_host(pst_sdio);
            return -OAL_ETIMEDOUT;
        }
        cpu_relax();
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "clear int mask error:%d", ret);
        oal_sdio_release_host(pst_sdio);
        return -OAL_EFAIL;
    }
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "read xfer_count err:%d", ret);
        oal_sdio_release_host(pst_sdio);
        return -OAL_EFAIL;
    }
    ...
    if (ver_info == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_sdio_patch_readsb alloc %d failed", xfer_count + 1);
        oal_sdio_release_host(pst_sdio);
        return -OAL_ENOMEM;
    }
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hsdio_readsb error:ret=%d", ret);
    } else {
        for (i = 0; i < len; i++) {
            buf[i] = ver_info[i];
        }
    ...
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "=====sdio read:[ret=%d]", ret);
...
OAL_STATIC int32_t oal_sdio_dev_init(struct oal_sdio *hi_sdio)
{
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "failed to clear sdio interrupt! ret=%d", ret);
    ...
    if (!hi_sdio->func->card->cccr.multi_block) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "[Error]did't support muti block transfer! ");
    }
#endif
...
OAL_STATIC void oal_sdio_dev_deinit(struct oal_sdio *hi_sdio)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_sdio_dev_deinit! ");
}

OAL_STATIC OAL_INLINE void oal_sdio_print_state(uint32_t old_state, uint32_t new_state)
{
    if (old_state != new_state) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio state changed, tx[%s=>%s],rx[%s=>%s]",
...
OAL_STATIC void oal_enable_sdio_state(hcc_bus *pst_bus, uint32_t mask)
{
    ...
    if (oal_warn_on(pst_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: pst_bus is null", __FUNCTION__);
        return;
    }
...
OAL_STATIC int32_t oal_sdio_rx_netbuf(hcc_bus *pst_bus, oal_netbuf_head_stru *pst_head)
{
    ...
    if (oal_warn_on(pst_sdio == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: pst_sdio is null", __FUNCTION__);
        return -OAL_EFAIL;
    }
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Failed to read scatt sdio![scatt len:%d]", oal_netbuf_list_len(pst_head));
        oal_sdio_release_host(pst_sdio);
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t oal_sdio_tx_netbuf(hcc_bus *pst_bus, oal_netbuf_head_stru *pst_head, hcc_netbuf_queue_type qtype)
{
    ...
    if (pst_hcc == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio dev's hcc handler is null!");
        return 0;
    }
    if (oal_unlikely(oal_atomic_read(&pst_hcc->state) != HCC_ON)) {
        /* drop netbuf list, wlan close or exception */
        oal_print_mpxx_log(MPXX_LOG_INFO, "drop sdio netbuflist %u", oal_netbuf_list_len(pst_head));
        return 0;
    }
...
OAL_STATIC struct oal_sdio *oal_sdio_alloc(struct sdio_func *func)
{
    ...
    if (hi_sdio == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Failed to alloc hi_sdio!");
        return NULL;
    }
...
OAL_STATIC int32_t oal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *ids)
{
    ...
    if (oal_warn_on(!ids)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: ids is null", __FUNCTION__);
        return -OAL_EFAIL;
    };
    ...
    if (hi_sdio == NULL) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s", g_sdio_enum_err_str);
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "transer limit size:%u", oal_sdio_func_max_req_size(hi_sdio));

    oal_print_mpxx_log(MPXX_LOG_INFO, "+++++++++++++func->enable_timeout= [%d]++++++++++++++++",
                       func->enable_timeout);
    ...
    if (pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio bus init failed");
                                            ...
                                            void *buf, uint32_t size)
{
    ...
    if (oal_warn_on(buf == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: buf is null", __FUNCTION__);
        return -OAL_EFAIL;
    }
    if (oal_warn_on((uintptr_t)buf & 0x3)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: buf :%ld", __FUNCTION__, (uintptr_t)buf);
        return -OAL_EFAIL;
    }
...
int32_t oal_sdio_transfer_tx(struct oal_sdio *hi_sdio, oal_netbuf_stru *netbuf)
{
    ...
    if (tailroom > oal_netbuf_tailroom(netbuf)) {
        ...
        if (oal_unlikely(ret != OAL_SUCC)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "alloc tail room failed");
            return -OAL_EFAIL;
        }
    }
...
OAL_STATIC void check_sg_format(struct scatterlist *sg, uint32_t sg_len)
{
    ...
    for_each_sg(sg, sg_t, sg_len, i)
    {
        ...
        if (oal_warn_on(((uintptr_t)sg_virt(sg_t) & 0x03) || (sg_t->length & 0x03))) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "check_sg_format:[i:%d][addr:%p][len:%u]",
                               i, sg_virt(sg_t), sg_t->length);
        }
    }
}

static int32_t oal_mmc_io_rw_scat_extended_check(struct oal_sdio *hi_sdio,
                                                 ...
                                                 uint32_t addr)
{
    ...
    if (oal_unlikely(sg == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: sg is null", __FUNCTION__);
        return -EINVAL;
    }
    if (oal_unlikely(sg_len == 0)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: sg_len is %d", __FUNCTION__, sg_len);
        return -EINVAL;
    }
    if (oal_unlikely(fn > 7)) { /* 7 max funs, sdio协议固定的内容 */
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: fn is %d", __FUNCTION__, fn);
        return -EINVAL;
    }
...
static inline void oal_mmc_io_rw_dump_mmc_data_info(struct mmc_data *data)
{
#ifdef CONFIG_SDIO_DEBUG
    int i;
    struct scatterlist *sg_t = NULL;
...
static inline int32_t oal_sdio_get_mmc_errno_by_resp(struct mmc_command *cmd)
{
...
static inline void oal_mmc_io_rw_scat_extended_dump(struct mmc_card *card,
                                                    ...
                                                    uint32_t blksz)
{
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "[blksz:%u][blocks:%u][sg_len:%u][mode:%s]",
                       blksz, blocks, sg_len, write ? "write" : "read");
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "%s : [cmd opcode:%d][cmd arg:0x%8x][cmd flags: 0x%8x]",
                       mmc_hostname(card->host), cmd->opcode, cmd->arg, cmd->flags);
    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "sdio %s data transfer start", write ? "write" : "read");
}
                                               ...
                                               uint32_t blocks, uint32_t blksz)
{
    ...
    ret = oal_mmc_io_rw_scat_extended_check(hi_sdio, sg, sg_len, fn, blksz, addr);
    if (oal_unlikely(ret != OAL_SUCC)) {
        return ret;
    }
    ...
    cmd.arg |= fn << 28; // bit28 func num
    cmd.arg |= incr_addr ? 0x04000000 : 0x00000000;
    cmd.arg |= addr << 9; // bit9 func address
    ...
    oal_mmc_io_rw_scat_extended_dump(card, &cmd, write, sg_len, blocks, blksz);

    mmc_set_data_timeout(&data, card);

    mmc_wait_for_req(card->host, &mrq);

    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "wait for %s tranfer over", write ? "write" : "read");
    ...
    if (oal_warn_on(mmc_host_is_spi(card->host))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "HiSi WiFi  driver do not support spi sg transfer!");
        return -EIO;
    }

    ret = oal_sdio_get_mmc_errno_by_resp(&cmd);
    if (oal_unlikely(ret != OAL_SUCC)) {
        return ret;
    }

    oal_mmc_io_rw_dump_mmc_data_info(&data);

    oal_print_mpxx_log(MPXX_LOG_VERBOSE, "Transfer done. %s sucuess!", write ? "write" : "read");
    return 0;
}
                                            ...
                                            uint32_t rw_sz)
{
    ...
    if (oal_unlikely(oal_sdio_get_state(hi_sdio->pst_bus, OAL_BUS_STATE_ALL) != OAL_TRUE)) {
        if (printk_ratelimit()) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "sdio closed,state:%u, %s ignored",
                                           ...
                                           uint32_t rw_sz)
{
    ...
    if (oal_warn_on(!rw_sz)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: rw_sz is null", __FUNCTION__);
        return -OAL_EINVAL;
    }
    if (oal_warn_on(sg_max_len < sg_len)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: sg_max_len:%d <  sg_len:%d", __FUNCTION__, sg_max_len, sg_len);
        return -OAL_EINVAL;
    }

    if (oal_warn_on(!sg_len)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "Sdio %s Scatter list num should never be zero, total request len: %u",
    ...
    if (oal_likely(align_len)) {
        if (oal_unlikely(sg_len + 1 > sg_max_len)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "sg list over,sg_len:%u, sg_max_len:%u", sg_len, sg_max_len);
            return -OAL_ENOMEM;
        }
    ...
    if (oal_warn_on(align_len & 0x3)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "not 4 bytes align:%u", align_len);
    }
...
OAL_STATIC int32_t oal_sdio_transfer_merge_write_sglist(struct oal_sdio *hi_sdio,
                                                        ...
                                                        int32_t rw)
{
    ...
    if (oal_unlikely(nents > sg_max_len)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "tx merged scatt list num %d > sg_len:%u, max seg size:%u\n",
                           nents,
                           sg_max_len,
                           seg_size);
        return -OAL_ENOMEM;
...
OAL_STATIC int32_t oal_sdio_transfer_merge_read_sglist(struct oal_sdio *hi_sdio,
                                                       oal_netbuf_head_stru *head,
                                                       struct scatterlist *sg,
                                                       uint32_t sg_max_len,
                                                       uint32_t *total_len,
                                                       ...
                                                       int32_t rw)
{
    ...
    nents = ((left_size - 1) / seg_size) + 1;
    if (oal_unlikely(nents > sg_max_len)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "rx merged scatt list num %d > sg_len:%u, max seg size:%u\n",
                           nents,
                           sg_max_len,
                           seg_size);
        return -OAL_ENOMEM;
                                                    ...
                                                    int32_t rw)
{
    ...
    /* sdio write/read, sdio use signle transfer when sg list item not align to block size(512B),
     * sdio used burst transfer if align to block size. burst fast than signle over then in sdio.
     * If master cpu more fast than sdio, we use master cpu memcpy sg to 4KB align mems to send when we write,
     * we recv sdio mem to sg 4KB align mems and memcpy to sg list when we read, refer to
     * feature macro CONFIG_HISDIO_H2D_SCATT_LIST_ASSEMBLE & CONFIG_HISDIO_D2H_SCATT_LIST_ASSEMBLE */
#if defined(CONFIG_HISDIO_H2D_SCATT_LIST_ASSEMBLE)
    if (rw == SDIO_WRITE) {
        /* 发送内存拷贝，合并成一块内存 */
        return oal_sdio_transfer_merge_write_sglist(hi_sdio, head, sg, sg_max_len, total_len, sg_len, rw);
    }
#endif

#if defined(CONFIG_HISDIO_D2H_SCATT_LIST_ASSEMBLE)
    if (rw == SDIO_READ) {
        /* 接收到连续内存，接收完成后靠背成离散内存 */
        return oal_sdio_transfer_merge_read_sglist(hi_sdio, head, sg, sg_max_len, total_len, sg_len, rw);
    }
#endif
                                      ...
                                      int32_t rw)
{
    ...
    if (oal_warn_on(rw >= SDIO_OPT_BUTT)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "invaild rw:%d", rw);
        return 0;
    }
    ...
    if (rw == SDIO_WRITE) {
        if (oal_unlikely(hcc_bus_pm_wakeup_device(hi_sdio->pst_bus) != OAL_SUCC)) {
            oal_msleep(100); /* 失败之后等待100ms让出系统调度 */
            return -OAL_EBUSY;
        }
    }
    ...
    if (oal_unlikely(request_sg_len > hi_sdio->scatt_info[rw].max_scatt_num)) {
        oal_print_mpxx_log(MPXX_LOG_WARN, "transfer_netbuf_list realloc sg!, request:%d,max scatt num:%d",
        ...
        if (sg_alloc_table(&sgtable, request_sg_len, GFP_KERNEL)) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "transfer_netbuf_list alloc sg failed!");
            return -OAL_ENOMEM;
        }
        sg_realloc = 1;
        sg = sgtable.sgl;
...
OAL_STATIC void oal_sdio_remove(struct sdio_func *func)
{
    ...
    if (hi_sdio == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[Error]Invalid NULL hi_sdio!");
        return;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "hisilicon connectivity sdio driver has been removed.");
}
...
OAL_STATIC int32_t oal_sdio_suspend(struct device *dev)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "+++++++sdio suspend+++++++++++++");
    if (dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[WARN]dev is null");
        return OAL_SUCC;
    }
    ...
    if (oal_warn_on(hi_sdio->pst_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "[E]sdio's bus is null, fatal error");
        return -OAL_ENODEV;
    }

    if (hbus_to_dev(hi_sdio->pst_bus) == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio is not work...");
        return OAL_SUCC;
    }

    if (hi_sdio->pst_bus != hdev_to_hbus(hbus_to_dev(hi_sdio->pst_bus))) {
        /* sdio非当前接口 */
        oal_print_mpxx_log(MPXX_LOG_INFO, "sdio is not current bus, return");
        return OAL_SUCC;
    }
    ...
    if (down_interruptible(&pst_bus->sr_wake_sema)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, KERN_ERR "sdio_wake_sema down failed.");
        return -OAL_EFAIL;
    }
    ...
    if (hcc_bus_wakelock_active(pst_bus)) {
         ...
         */
        oal_print_mpxx_log(MPXX_LOG_ERR, KERN_ERR "Already wake up");
...
OAL_STATIC int32_t oal_sdio_resume(struct device *dev)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "+++++++sdio resume+++++++++++++");
    if (dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dev is null");
        return OAL_SUCC;
    }
    func = dev_to_sdio_func(dev);
    hi_sdio = sdio_get_drvdata(func);
    ...
    if (oal_warn_on(hi_sdio->pst_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio's bus is null, fatal error");
        return OAL_SUCC;
    }
...
OAL_STATIC struct sdio_device_id const g_oal_sdio_ids[] = {
    { SDIO_DEVICE(HISDIO_VENDOR_ID_MP13,  HISDIO_PRODUCT_ID_HISI) },
    { SDIO_DEVICE(HISDIO_VENDOR_ID_MP15, HISDIO_PRODUCT_ID_MP15_HISI) },
    { SDIO_DEVICE(HISDIO_VENDOR_ID_HISI,    HISDIO_PRODUCT_ID_MP16_HISI) },
    { SDIO_DEVICE(HISDIO_VENDOR_ID_HISI,    HISDIO_PRODUCT_ID_MP16C) },
    {},
};
...
OAL_STATIC void oal_sdio_dev_shutdown(struct device *dev)
{
    ...
    if (g_hisdio_intr_mode) {
        ...
        oal_sdio_release_host(hi_sdio);
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "wlan shutdown faile,ret=%d!", ret);
            return;
        }
    }

    oal_print_mpxx_log(MPXX_LOG_INFO, "wlan shutdown sucuess!");
}
...
OAL_STATIC int32_t oal_sdio_trigger_probe(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "start to register sdio module");

    ret = sdio_register_driver(&g_oal_sdio_driver);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "register sdio driver Failed ret=%d", ret);
    ...
    if (wait_for_completion_timeout(&g_sdio_driver_complete, 10 * HZ)) { /* 10 * HZ -> 10 sec */
        oal_print_mpxx_log(MPXX_LOG_INFO, "hisi sdio load sucuess, sdio enum done.");
    } else {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio enum timeout, reason[%s]", g_sdio_enum_err_str);
        (void)ssi_dump_err_regs(SSI_ERR_SDIO_PROBE_FAIL);
        goto failed_sdio_enum;
    }

    oal_print_mpxx_log(MPXX_LOG_INFO, "shutdown wifi after init sdio");
    oam_warning_log0(0, OAM_SF_ANY, "{oal_sdio_func_probe::shutdown wifi after init sdio.}");

    if (g_mpxx_sdio != NULL) {
        oal_sdio_claim_host(g_mpxx_sdio);
        hcc_bus_disable_state(g_mpxx_sdio->pst_bus, OAL_BUS_STATE_ALL);
        hi_wlan_power_set(0);
        oal_sdio_release_host(g_mpxx_sdio);
    } else {
        oal_print_mpxx_log(MPXX_LOG_ERR, "g_mpxx_sdio is null");
        return -OAL_ENODEV;
    }
...
int32_t oal_sdio_malloc_buffer(struct oal_sdio *hi_sdio)
{
    ...
    if (hi_sdio->sdio_align_buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc sdio_align_buff size %d failed\n", HISDIO_BLOCK_SIZE);
        return -OAL_ENOMEM;
    }
    ...
    if (hi_sdio->sdio_send_buff == NULL) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc sdio_send_buff size %d failed\n", HISDIO_SEND_SIZE);
        return -OAL_ENOMEM;
    }
...
struct oal_sdio *oal_sdio_init_module(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "mpxx sdio driver installing...");
    hi_sdio = (struct oal_sdio *)oal_memalloc(sizeof(struct oal_sdio));
    if (hi_sdio == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc oal_sdio failed [%d]", (int32_t)sizeof(struct oal_sdio));
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_SDIO, CHR_PLAT_DRV_ERROR_ALLOC_MEM);
        return NULL;
    }
    memset_s((void *)hi_sdio, sizeof(struct oal_sdio), 0, sizeof(struct oal_sdio));
    ...
    if (hi_sdio->rx_reserved_buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc rx_reserved_buff failed [%u]", ul_rx_seg_size);
        goto failed_rx_reserved_buff_alloc;
    }
    hi_sdio->rx_reserved_buff_len = ul_rx_seg_size;
    oal_print_mpxx_log(MPXX_LOG_INFO, "alloc %u bytes rx_reserved_buff ", ul_rx_seg_size);
    ...
    if (hi_sdio->sdio_extend == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc sdio_extend failed [%d]",
    ...
    hi_sdio->scatt_info[SDIO_READ].sglist = kzalloc(sizeof(struct scatterlist) * (HISDIO_DEV2HOST_SCATT_MAX + 1),
                                                    GFP_KERNEL);
    if (hi_sdio->scatt_info[SDIO_READ].sglist == NULL) {
        goto failed_sdio_read_sg_alloc;
    }

    /* 2 for algin buff and scatt info buff */
    hi_sdio->scatt_info[SDIO_WRITE].max_scatt_num = HISDIO_HOST2DEV_SCATT_MAX + 2;
    hi_sdio->scatt_info[SDIO_WRITE].sglist = kzalloc(sizeof(struct scatterlist) *
    ...
    hi_sdio->tx_scatt_buff.buff = oal_mem_dma_blockalloc(tx_scatt_buff_len, MEM_DMA_BLOCLALLOC_TIMEOUT);
    if (hi_sdio->tx_scatt_buff.buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc tx_scatt_buff failed,request %u bytes", tx_scatt_buff_len);
        goto failed_sdio_tx_scatt_buff_alloc;
    }
    hi_sdio->tx_scatt_buff.len = tx_scatt_buff_len;

    oal_print_mpxx_log(MPXX_LOG_INFO, "alloc tx_scatt_buff ok,request %u bytes", tx_scatt_buff_len);
#endif
    ...
    hi_sdio->rx_scatt_buff.buff = oal_mem_dma_blockalloc(rx_scatt_buff_len, MEM_DMA_BLOCLALLOC_TIMEOUT);
    if (hi_sdio->rx_scatt_buff.buff == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc rx_scatt_buff failed,request %u bytes", rx_scatt_buff_len);
        goto failed_sdio_rx_scatt_buff_alloc;
    }
    hi_sdio->rx_scatt_buff.len = rx_scatt_buff_len;

    oal_print_mpxx_log(MPXX_LOG_INFO, "alloc rx_scatt_buff ok,request %u bytes", rx_scatt_buff_len);
#endif
...
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "sdio module unregistered");
#ifdef CONFIG_HISDIO_D2H_SCATT_LIST_ASSEMBLE
...
void oal_sdio_tc_msg_001(int msg)
{
    ...
    wlan_pm_disable();
    ret = oal_sdio_send_msg(g_hi_sdio_debug->pst_bus, msg);
    oal_print_mpxx_log(MPXX_LOG_INFO, "send msg 0x%8X, ret = %d", msg, ret);
    wlan_pm_enable();
}
...
void oal_sdio_tc_buf_tx_001(void)
{
    ...
    if (buf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_sdio_tc_buf_tx_001 memmalloc buf error");
        return;
    }
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "[WIFI] oal_sdio_tc_buf_tx_001 failed=%d", ret);
    }
}
...
void oal_sdio_tc_extend_001(void)
{
    ...
    if (netbuf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc netbuf fail");
        return;
    }
...
void oal_sdio_tc_mem_cp_from(unsigned int offset, unsigned int len)
{
    ...
    if (netbuf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "alloc fail");
        return;
    }
    ...
    oal_sdio_release_host(g_hi_sdio_debug);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "read failed ret=%d", ret);
    }
...
void oal_sdio_read_func0(int offset, int len)
{
    ...
    if (g_hi_sdio_debug == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "g_hi_sdio_debug is null");
        oal_netbuf_free(netbuf);
        return;
    }
...
void oal_sdio_read_func1(int offset, int len)
{
    ...
    oal_netbuf_stru *netbuf = oal_netbuf_alloc(len, 0, 0);
    if (netbuf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc fail");
        return;
    }
    oal_netbuf_put(netbuf, len);
    buf = oal_netbuf_data(netbuf);
...
uint32_t oal_sdio_func_max_req_size(struct oal_sdio *pst_hi_sdio)
{
    ...
    if (oal_warn_on(pst_hi_sdio == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: pst_hi_sdio is null", __FUNCTION__);
        return 0;
    };
    /*
     * host transer limit
    ...
    /* device transer limit,per adma descr limit 32*1024 bytes in bootloader,and total we have 20 descs */
...
OAL_STATIC int32_t oal_sdio_gpio_flowctrl_irq(hcc_bus *pst_hi_bus, int32_t l_irq)
{
    ...
    if (oal_unlikely(pst_hi_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, KERN_ERR"sdio bus is null, irq:%d", l_irq);
        return -OAL_EINVAL;
    }

    pst_hcc = pst_hi_bus->bus_dev->hcc;

    if (oal_unlikely(pst_hcc == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hcc handler is null!");
        return -OAL_EFAIL;
    }
    ...
    if (l_gpio_val == 1) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "invalid gpio value = %d!", l_gpio_val);
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}
...
OAL_STATIC int32_t oal_sdio_gpio_irq(hcc_bus *hi_bus, int32_t irq)
{
    ...
    if (oal_unlikely(hi_bus == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, KERN_ERR "sdio bus is null, irq:%d", irq);
        return -OAL_EINVAL;
    }

    if (!hi_bus->pst_pm_callback || !hi_bus->pst_pm_callback->pm_state_get) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "GPIO interrupt function param is NULL");
        return -OAL_EINVAL;
    }
    ...
    if (ul_state == 0) {
        ...
        oal_print_mpxx_log(MPXX_LOG_DBG, "Gpio Rx Data Interrupt.");

        up(&hi_bus->rx_sema);
    } else {
        /* 1==HOST_ALLOW_TO_SLEEP表示当前是休眠，唤醒host */
        if (oal_warn_on(!hi_bus->pst_pm_callback->pm_wakeup_host)) {
            oal_print_mpxx_log(MPXX_LOG_DBG, "%s error:hi_bus->pst_pm_callback->pm_wakeup_host is null",
                               __FUNCTION__);
            return -OAL_FAIL;
        }
        hi_bus->wakeup_int_count++;
        oal_print_mpxx_log(MPXX_LOG_DBG, "Gpio Wakeup Interrrupt %llu,data intr %llu \r",
...
OAL_STATIC int32_t oal_sdio_poweroff_complete(hcc_bus *pst_bus)
{
    return wlan_power_off_complete();
}
...
OAL_STATIC hcc_bus_opt_ops g_sdio_opt_ops = {
    ...
    .poweroff_complete =  oal_sdio_poweroff_complete,
...
OAL_STATIC hcc_bus *oal_sdio_bus_init(struct oal_sdio *hi_sdio)
{
    ...
    if (pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc sdio hcc bus failed, size:%u", (uint32_t)sizeof(hcc_bus));
        return NULL;
    }

    pst_bus->bus_type = HCC_BUS_SDIO;
    pst_bus->bus_id = 0x0;
    pst_bus->dev_id = HCC_EP_WIFI_DEV;
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "add sdio bus failed, ret=%d", ret);
...
{
    if (oal_sdio_mpxx_working_check() != OAL_TRUE) {
        /* sdio driver don't support */
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio driver don't support");
        return OAL_SUCC;
    }
#ifdef CONFIG_MMC
    if (oal_sdio_trigger_probe() != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "sdio first probe failed!");
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
...
#ifdef CONFIG_MMC
    if (g_mpxx_sdio != NULL) {
        oal_sdio_func_remove(g_mpxx_sdio);
    }
#endif
}
...
#else
struct oal_sdio g_mpxx_sdio_ut;
int32_t oal_wifi_platform_load_sdio(void)
{
    return OAL_SUCC;
}
