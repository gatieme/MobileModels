...
#include "pcie_chip.h"
#include "chip/mp13/pcie_chip_mp13.h"
#include "chip/mp16/pcie_chip_mp16.h"
#include "chip/mp16c/pcie_chip_mp16c.h"
#include "pcie_linux.h"
#include "pcie_pm.h"
#include "board.h"

static pcie_chip_id_stru g_pcie_chipinfo_id[] = {
    {PCIE_HISI_DEVICE_ID_MP13,     oal_pcie_chip_info_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP15,     oal_pcie_chip_info_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP16,     oal_pcie_chip_info_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16FPGA, oal_pcie_chip_info_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16C,    oal_pcie_chip_info_init_mp16c}
};
...
static board_chip_id_stru g_board_chipinfo_id[] = {
    {BOARD_VERSION_MP16, oal_pcie_board_init_mp16},
    {BOARD_VERSION_MP16C, oal_pcie_board_init_mp16c},
};
    ...
    for (i = 0; i < chip_num; i++) {
        if (g_board_chipinfo_id[i].chip_id == chip_id) {
            if (g_board_chipinfo_id[i].func != NULL) {
                oal_print_mpxx_log(MPXX_LOG_INFO, "pcie board init 0x%x init succ", chip_id);
                return g_board_chipinfo_id[i].func(&g_pcie_linkup_fixup_ops);
            } else {
                oal_print_mpxx_log(MPXX_LOG_INFO, "pcie board init 0x%x init null func", chip_id);
                return OAL_SUCC;
            }
        }
    }
...
{
    oal_pcie_res *pst_pci_res = pcie_lres->pst_pci_res;

    if (pst_pci_res->chip_info.cb.pcie_config_slave != NULL) {
        return pst_pci_res->chip_info.cb.pcie_config_slave(pst_pci_res, type);
    }
    return -OAL_ENODEV;
}
