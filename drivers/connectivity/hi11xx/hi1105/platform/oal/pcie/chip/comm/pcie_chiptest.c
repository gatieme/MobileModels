...
static pcie_chip_id_stru g_pcie_chiptest_id[] = {
    {PCIE_HISI_DEVICE_ID_MP13,     oal_pcie_chiptest_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP15,     oal_pcie_chiptest_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP16,     oal_pcie_chiptest_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16FPGA, oal_pcie_chiptest_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16C,     oal_pcie_chiptest_init_mp16c}
};
    ...
    for (i = 0; i < chip_num; i++) {
        if (g_pcie_chiptest_id[i].device_id == device_id) {
            if (g_pcie_chiptest_id[i].func != NULL) {
                ...
                oal_print_mpxx_log(MPXX_LOG_INFO, "chiptest init 0x%x init null func", device_id);
                return OAL_SUCC;
            }
        }
    }
    oal_print_mpxx_log(MPXX_LOG_ERR, "not implement chiptest device_id=0x%x", device_id);
    return -OAL_ENODEV;
}
