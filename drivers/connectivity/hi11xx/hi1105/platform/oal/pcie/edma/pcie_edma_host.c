...
OAL_STATIC int32_t oal_pcie_rx_normal_thread(void *data)
{
    ...
    forever_loop() {
        ...
        mutex_unlock(&pst_pcie_res->st_rx_mem_lock);
        g_hisi_softwdt_check.rxtask_cnt++;
        if (resched) {
            /*
             * 补充内存失败，成功则忽略，有可能当前不需要补充内存也视为成功,
                                                                ...
                                                                uint32_t chan_cnt)
{
    ...
    do {
        ...
        if (pst_netbuf == NULL) {
            /* 不应该为空，count有可能有问题 */
            pci_print_log(PCI_LOG_ERR, "tx netbuf queue underflow[left_cnt:%u]", cnt);
...
OAL_STATIC void oal_pcie_h2d_transfer_done(oal_pcie_res *pst_pci_res)
{
    int32_t ret;
    int32_t i, j, flag;
    uint32_t cnt, total_cnt, netbuf_cnt, curr_cnt;
    oal_netbuf_stru *pst_netbuf = NULL;
    ...
    g_hisi_softwdt_check.pcie_done_time = oal_read_once(OAL_TIME_JIFFY);
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        ...
        for (j = 0; j < PCIE_EDMA_MAX_CHANNELS; j++) {
            ...
            pci_print_log(PCI_LOG_DBG, "[q:%d]tx chan:%d pa 0x%llx, cnt:%u", i, j, addr[j].addr, count[j]);
            ...
            if (oal_unlikely(cnt > curr_cnt)) {
                /* count 出错? */
                pci_print_log(PCI_LOG_ERR, "[q:%d]tx chan:%d tx done invalid count cnt %u ,list len %u", i, j,
                              cnt, curr_cnt);
            ...
            if (oal_likely(ret == OAL_SUCC)) {
                ...
                flag = 0; // failed
                pci_print_log(PCI_LOG_ERR, "[q:%d]tx chan:%d tx netbuf queue underflow[cnt:%u:%u, qlen:%u]", i, j,
...
OAL_STATIC void oal_pcie_dump_rx_dma_addr_info(oal_pcie_res *pst_pci_res)
{
    int32_t index = 0;
    unsigned long flags;
    oal_netbuf_stru *pst_netbuf = NULL;
    pcie_cb_dma_res *pst_cb_res = NULL;
    oal_netbuf_head_stru *pst_rxq = &pst_pci_res->st_rx_res.rxq;
    if (oal_netbuf_list_empty(pst_rxq)) {
        return;
    }
    pci_print_log(PCI_LOG_INFO, "rxq dump, len=%d", oal_netbuf_list_len(pst_rxq));
    oal_spin_lock_irq_save(&pst_pci_res->st_rx_res.lock, &flags);
    pst_netbuf = (oal_netbuf_stru *)oal_netbuf_next(pst_rxq);
    // list rxq cb info
    forever_loop() {
        if ((pst_netbuf == (oal_netbuf_stru *)pst_rxq) ||
             (pst_netbuf == NULL)) {
            break;
        }
        pst_cb_res = (pcie_cb_dma_res *)oal_netbuf_cb(pst_netbuf);
        pci_print_log(PCI_LOG_INFO, "[%d]dma addr: 0x%llx len=%u", index++, pst_cb_res->paddr.addr, pst_cb_res->len);
        pst_netbuf = (oal_netbuf_stru *)oal_netbuf_next(pst_netbuf);
    }
    oal_spin_unlock_irq_restore(&pst_pci_res->st_rx_res.lock, &flags);
}

static inline int32_t oal_pcie_rx_dma_addr_match(oal_netbuf_stru *pst_netbuf, edma_paddr_t dma_addr)
{
    pcie_cb_dma_res *pst_cb_res;
    /* DMA地址填到CB中, CB首地址8字节对齐可以直接强转 */
    pst_cb_res = (pcie_cb_dma_res *)oal_netbuf_cb(pst_netbuf);
    if (oal_likely(pst_cb_res->paddr.addr == dma_addr.addr)) {
        return OAL_TRUE;
    }
    pci_print_log(PCI_LOG_ERR, "rx dma addr mismatch, cb's addr 0x%llx , dma_addr 0x%llx",
                  pst_cb_res->paddr.addr, dma_addr.addr);
    return OAL_FALSE;
}

static edma_paddr_t g_addr[PCIE_EDMA_MAX_CHANNELS];
static uint32_t     g_count[PCIE_EDMA_MAX_CHANNELS];

static inline void oal_pcie_d2h_print_addr_info(edma_paddr_t *addr, uint32_t *count, int32_t len)
{
    int32_t i;
    for (i = 0; i < len; i++) {
        pci_print_log(PCI_LOG_INFO, "[%d] last addr info addr=0x%llx count=%u, curr addr=0x%llx count=%u",
                      i, g_addr[i].addr, g_count[i], addr[i].addr, count[i]);
    }
}

static inline void oal_pcie_d2h_reset_addr_info(void)
{
    int32_t i;
...
static inline void oal_pcie_d2h_addr_info_save(edma_paddr_t *addr, uint32_t *count, int32_t len)
{
    int32_t i;
    for (i = 0; i < len; i++) {
        g_addr[i].addr = addr[i].addr;
        g_count[i] = count[i];
    }
}
...
OAL_STATIC void oal_pcie_d2h_transfer_done(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < PCIE_EDMA_MAX_CHANNELS; i++) {
        ...
        pci_print_log(PCI_LOG_DBG, "rx chan:%d pa 0x%llx, cnt:%u", i, addr[i].addr, count[i]);
        ...
        if (oal_unlikely(pst_netbuf == NULL)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "%s error: pst_netbuf is null", __FUNCTION__);
            continue;
        };
        ...
        if (oal_pcie_rx_dma_addr_match(pst_netbuf, addr[i]) != OAL_TRUE) {
            ...
            pci_print_log(PCI_LOG_ERR, "rx chan:%d match failed, rx error, count[i]:%u, errcnt:%u",
                          i, count[i], g_rx_addr_count_err_cnt);
            pci_print_log(PCI_LOG_ERR, "count0:%u, count1:%u", count[0], count[1]);
            oal_pcie_dump_rx_dma_addr_info(pst_pci_res);
            oal_pcie_d2h_print_addr_info(addr, count, PCIE_EDMA_MAX_CHANNELS);
        ...
        if (oal_unlikely(cnt > oal_netbuf_list_len(pst_rxq))) {
            pci_print_log(PCI_LOG_ERR, "rx chan:%d rx done invalid count cnt %u ,list len %u", i,
                          cnt, oal_netbuf_list_len(pst_rxq));
        ...
        if (oal_likely(ret == OAL_SUCC)) {
            ...
            flag = 0;
            pci_print_log(PCI_LOG_ERR, "rx chan:%d cnt=%u netbuf queue failed, ret=%d", i, cnt, ret);
            goto done;
        }
    }
    oal_pcie_d2h_addr_info_save(addr, count, PCIE_EDMA_MAX_CHANNELS);
                                  ...
                                  pcie_h2d_ringbuf_qtype qtype)
{
    ...
    pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (oal_unlikely(OAL_SUCC != hcc_bus_pm_wakeup_device(pst_pci_lres->pst_bus))) {
        oal_msleep(100); /* wait for 100ms retry */
        return -OAL_EIO;
    }
    ...
    forever_loop() {
        ...
        if (dma_mapping_error(&pst_pci_dev->dev, pci_dma_addr)) {
            ...
            oal_print_mpxx_log(MPXX_LOG_ERR, "edma tx map failed, va=%p, len=%d", oal_netbuf_data(pst_netbuf),
                               oal_netbuf_len(pst_netbuf));
            hcc_tx_netbuf_free(pst_netbuf);
            continue;
        }
...
OAL_STATIC oal_irq_handler_t g_msi_mpxx_callback[] = {
...
int32_t oal_pcie_edma_task_init(oal_pcie_res *pst_pcie_res, oal_pcie_msi_stru *msi)
{
    ...
    msi->func = g_msi_mpxx_callback;
    msi->msi_num = (int32_t)(sizeof(g_msi_mpxx_callback) / sizeof(oal_irq_handler_t));
