...
OAL_STATIC int32_t oal_pcie_edma_poweroff_complete(hcc_bus *pst_bus)
{
...
retry:
    ...
    if (pst_wlan_pm != NULL) {
        ul_ret = oal_wait_for_completion_timeout(&pst_wlan_pm->st_wifi_powerup_done,
                                                 (uint32_t)oal_msecs_to_jiffies(2000)); /* 2000ms for wait */
        ...
        oal_msleep(100); /* wait 100ms 这里要用GPIO 做ACK 延迟不可靠, S/R 唤醒 时间较长 */
    }
...
OAL_STATIC hcc_bus_opt_ops g_pcie_edma_opt_ops = {
    ...
    .poweroff_complete =  oal_pcie_edma_poweroff_complete,
