...
#define MPXX_LOG_MODULE_NAME "[PCIE_ETE_H]"
#define HISI_LOG_TAG           "[PCIE_ETE]"
...
#define WIFI_WAKEUP_COMPLETE_TIMEOUT 2000
...
OAL_STATIC int32_t oal_pcie_ete_wake_powerup_handler(void *data)
{
    ...
    if (conn_get_gpio_level(HOST_WKUP_W) == GPIO_LEVEL_LOW) {
retry:
        ...
        if (oal_unlikely(ret == 0)) { // timeout
            ...
            if (retry_cnt++ < max_retry_cnt) {
                ...
                oal_msleep(5); /* 等待5ms，待唤醒io拉低稳定后再次尝试唤醒 */
                goto retry;
            } else {
...
OAL_STATIC int32_t oal_pcie_ete_wakeup_complete(hcc_bus *pst_bus)
{
    ...
    oal_pcie_change_link_state(pst_pci_res, PCI_WLAN_LINK_WORK_UP);
    oal_pcie_device_sr_debug(pst_pci_res, OAL_FALSE);
    mutex_unlock(&pst_pci_res->st_rx_mem_lock);

    if (g_pcie_auto_disable_aspm != 0) {
        /* disable L1 after wakeup */
        oal_pcie_set_device_aspm_dync_disable(pst_pci_res, OAL_TRUE);
    ...
    if (total_time > WIFI_WAKEUP_COMPLETE_TIMEOUT) {
        pci_print_log(PCI_LOG_INFO, "oal_pcie_ete_wakeup_complete long_time");
        declare_dft_trace_key_info("oal_pcie_ete_wakeup_complete long_time", OAL_DFT_TRACE_OTHER);
    }
    oal_ete_sched_rx_threads(pst_pci_res);
...
OAL_STATIC int32_t oal_pcie_ete_poweroff_complete(hcc_bus *pst_bus)
{
    ...
    pst_pci_res = pst_pci_lres->pst_pci_res;
    if (pst_pci_res->chip_info.cb.pcie_poweroff_complete == NULL) {
        return -OAL_ENODEV;
    }
    ...
    /* :20dbm可能存在较长时间占用信号量，wifi等待时间增加 */
    timeout_cnt = hi110x_get_emu_timeout(ACK_CHECK_MAX_CNT);
    ret = -OAL_EFAIL;
    for (i = 0; i < timeout_cnt; i++) {
        msleep(ACK_CHECK_WAIT_TIME);
        ret = pst_pci_res->chip_info.cb.pcie_poweroff_complete(pst_pci_res);
                                                    ...
                                                    oal_pci_dev_stru *pst_pci_dev)
{
    ...
    if (ret) {
        ...
        oal_msleep(1000); /* 整机低功耗流程，可能此时唤醒流程还未走完等1000ms */
...
OAL_STATIC OAL_INLINE int32_t oal_ete_dual_pci_power_up(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev_dual, g_kirin_rc_idx_dual, PCIE_POWER_ON);
    if (ret == OAL_SUCC) {
        pcie_linkup_succ_fixup(1);
    } else {
...
OAL_STATIC int32_t oal_pcie_change_linkspeed(hcc_bus *pst_bus, uint32_t request_speed)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if ((pst_pci_lres != NULL) && (pst_pci_lres->pst_pci_res != NULL)) {
        return (pst_pci_lres->pst_pci_res->chip_info.cb.
                pcie_dyn_change_linkspeed(pst_pci_lres->pst_pci_res, request_speed));
    }
    return -OAL_EINVAL;
}
...
OAL_STATIC hcc_bus_opt_ops g_pcie_ete_opt_ops = {
    ...
    .change_linkspeed = oal_pcie_change_linkspeed,
