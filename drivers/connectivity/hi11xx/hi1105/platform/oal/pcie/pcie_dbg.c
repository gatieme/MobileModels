                                            ...
                                            uint32_t outbound_size, dma_addr_t outbound_dma_addr)
{
    if (pst_pcie_res->chip_info.edma_support == OAL_TRUE) {
        ...
        if (pst_pcie_res->revision == PCIE_REVISION_4_70A) {
            ...
            oal_writel((uint32_t)(outbound_dma_addr >> 32), /* 64位地址取高32位 */
                       inbound_addr +
    ...
    if (pst_pcie_res->chip_info.ete_support == OAL_TRUE) {
        ...
        if (ret == OAL_SUCC) {
            ...
            if (ret == OAL_SUCC) {
                oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_if_devva_to_hostca devva=0x%llx -> iova=0x%llx",
                                   devva, host_iova);
            } else {
...
OAL_STATIC int32_t oal_pcie_addr_switch_test(oal_pcie_res *pst_pcie_res, const char *buf)
{
    ...
    if (pst_pcie_res->chip_info.addr_info.glb_ctrl != 0) {
        ...
        if (ret == OAL_SUCC) {
            ...
            if (ret == OAL_SUCC) {
                oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_get_dev_ca 0x%llx -> 0x%x",
                                   host_va, (uint32_t)dev_cpuaddr);
            } else {
                oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_get_dev_ca 0x%llx  convert failed",
                                   host_va);
            }
        } else {
...
OAL_STATIC int32_t oal_pcie_resume_handler(void *data)
{
    ...
    oal_msleep(25); /* 这里要用GPIO 做ACK 延迟不可靠, MPW2 硬件唤醒15ms,软件6ms 25ms是留余量 */
...
static uintptr_t oal_pcie_get_phy_paddr(oal_pci_dev_stru *pst_pci_dev, oal_pcie_res *pst_pci_res,
...
OAL_STATIC int32_t oal_pcie_testcase_dpattern_test(oal_pcie_res *pst_pcie_res,
                                                   oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
    int32_t ret;
    ...
    for (; *buf == ' '; buf++);
    if ((sscanf_s(buf, "0x%x %u %u", &phy_paddr, &lane, &dump_type) != 3)) { /* 输入3个参数 */
        pci_print_log(PCI_LOG_ERR, "invalid agrument");
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_testcase_ete_memcpy_test(oal_pcie_res *pst_pcie_res,
                                                     oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    ...
    if (sysfs_streq(mode, "tx")) {
        mode_int = 1; // mode 1->tx
    } else if (sysfs_streq(mode, "rx")) {
        mode_int = 2; // mode 2->rx
    } else if (sysfs_streq(mode, "loop")) {
        mode_int = 3; // mode 3->loop
    } else {
    ...
    /* 打开WIFI子系统，但是不启动WIFI初始化 */
    ret = wlan_pm_open_with_cali(OAL_FALSE);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "[ETE_TEST] open wlan failed=%d", ret);
        return OAL_SUCC;
    }
...
OAL_STATIC int32_t oal_pcie_performance_netbuf_queue(oal_pcie_res *pst_pcie_res,
                                                     uint32_t test_time, int32_t alloc_count)
{
    uint64_t count;
    int32_t i;
    int32_t ret = OAL_SUCC;
...
OAL_STATIC pcie_sysfs_debug_info g_pci_debug[] = {
    ...
    { "performance_cpu",          "length(decimal) times(decimal) burst_size(decimal)",
                                  oal_pcie_debug_performance_cpu },
