...
OAL_STATIC oal_pcie_bar_info g_en_bar_tab[] = {
    /*
     * mp13 4.7a 一个BAR [8MB]， 5.0a 为两个BAR[Bar0 8M  BAR1 16KB]
     * (因为mp13 是64bit bar,所以对应bar index寄存器, 是对应bar index=2,
...
void oal_pcie_io_trans64(void *dst, void *src, int32_t size)
{
    ...
    forever_loop() {
        ...
        if ((!((uintptr_t)src & 0x7)) && (remain >= 8) && (!((uintptr_t)dst & 0x7))) { /* 8bytes */
            ...
            *((volatile uint32_t *)dst) = *((volatile uint32_t *)src);
            ...
            oal_print_mpxx_log(MPXX_LOG_ERR, "invalid argument, dst:%pK , src:%pK, remain:%d", dst, src, remain);
        }
    }
}
...
int32_t oal_pcie_memport_copy_test(void)
{
    ...
    if (buff_src == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc %lu buff failed", burst_size);
        return -OAL_EFAIL;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "memcopy: %llu Mbps, trans_time:%llu us, tran_size:%lu",
...
void oal_pcie_regions_info_dump(oal_pcie_res *pci_res)
{
    ...
    for (index = 0; index < region_num; index++, region_base++) {
        oal_io_print("[%15s]va:0x%p, pa:0x%llx, [pci start:0x%8llx end:0x%8llx],[cpu start:0x%8llx end:0x%8llx],\
                     size:%u, flag:0x%x\n",
                     region_base->name, region_base->vaddr, region_base->paddr, region_base->pci_start,
                     region_base->pci_end, region_base->cpu_start, region_base->cpu_end,
                     region_base->size, region_base->flag);
    }
}
...
hcc_bus *oal_pcie_get_current_bus(void)
{
    ...
    if (hcc_bus_isvalid(pst_bus) != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "bus %d is not ready", HCC_BUS_PCIE);
        return NULL;
    }
...
{
    hcc_bus *hi_bus = hcc_get_bus(dev_id);
...
{
    return hcc_bus_master_address_switch(hcc_get_bus(ul_chip_id), dev_cpuaddr, host_va, MASTER_WCPUADDR_TO_VA);
}
...
{
    return hcc_bus_master_address_switch(hcc_get_bus(ul_chip_id),
...
{
    return hcc_bus_slave_address_switch(hcc_get_bus(chip_id), host_iova, addr, SLAVE_IOVA_TO_PCI_SLV);
}
...
{
    return hcc_bus_slave_address_switch(hcc_get_bus(chip_id), devva, host_iova, SLAVE_PCI_SLV_TO_IOVA);
}
...
OAL_STATIC int32_t oal_pcie_ctrl_base_address_init(oal_pcie_res *pst_pci_res)
{
    ...
    if (pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        ...
        if (ret != OAL_SUCC) {
            pci_print_log(PCI_LOG_ERR, "get dev address 0x%x failed", pst_pci_res->chip_info.addr_info.ete_ctrl);
            oal_pcie_regions_info_dump(pst_pci_res);
            oal_pcie_ctrl_base_address_exit(pst_pci_res);
            return -OAL_EFAIL;
        }
        pst_pci_res->pst_ete_base = (void *)st_map.va;
        pst_pci_res->chip_info.cb.ete_address_init(pst_pci_res);
    }
...
int32_t oal_pcie_bar1_init(oal_pcie_iatu_bar* pst_iatu_bar)
{
    ...
    pci_print_log(PCI_LOG_INFO, "iatu bar1 config done");
...
OAL_STATIC int32_t oal_pcie_regions_init(oal_pcie_res *pst_pci_res)
{
    ...
    for (index = 0; index < bar_num; index++, bar_base++) {
        for (; region_idx < region_num; region_idx++, region_base++) {
            ...
            region_base->bar_info = bar_base;
            pci_print_log(PCI_LOG_DBG, "bar idx:%d, region idx:%d, region paddr:0x%llx, region_size:%u\n",
                                  ...
                                  pci_addr_map *addr_map)
{
    ...
    pci_print_log(PCI_LOG_WARN, "addr request 0x%llx failed", dev_cpuaddr);
    return -OAL_ENOMEM;
}
...
OAL_STATIC int32_t oal_pcie_mem_bar_init(oal_pci_dev_stru *pst_pci_dev, oal_pcie_bar_info *bar_curr)
{
    ...
    if (bar_curr->size == 0) {
        pci_print_log(PCI_LOG_ERR, "bar 1 size is zero");
        return -OAL_EIO;
    }
...
OAL_STATIC int32_t oal_pcie_bar_init(oal_pcie_res *pst_pci_res)
{
    ...
    for (index = 0; index < bar_num; index++) {
        /*
         * 获取Host分配的硬件地址资源,mp13为8M大小,
         * mp13 4.7a 对应一个BAR, 5.0a 对应2个bar,
         * 其中第二个bar用于配置iatu表
         */
        ...
        pci_print_log(PCI_LOG_INFO, "preapre for bar idx:%u\n", bar_idx);
    }
    /* 是否支持BAR1 */
    if (pst_pci_res->chip_info.membar_support == OAL_TRUE) {
        /* Get Bar Address */
...
int32_t oal_pcie_device_check_alive(oal_pcie_res *pst_pci_res)
{
    ...
    if (value == 0x101) {
        ...
        oal_print_mpxx_log(MPXX_LOG_WARN, "pcie maybe linkdown, glbctrl=0x%x", value);
        return -OAL_EFAIL;
    }
}
...
#define PCIE_RX_BUF_PENDING_WAIT_CNT 2000
...
int32_t oal_pcie_check_link_state_err_adapter(pci_addr_map addr_map)
{
    pcie_dev_ptr share_mem_address;
    /* record exception result and read again */
    chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                         CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_CHECK_LINK_STATE_EXCP);
    oam_error_log0(0, OAM_SF_ANY, "pcie_maybe_excpeption, check again");
    declare_dft_trace_key_info("pcie_check_linkstate_excpeption", OAL_DFT_TRACE_EXCEP);
...
int32_t oal_pcie_check_link_state(oal_pcie_res *pst_pci_res)
{
    ...
    if (share_mem_address == 0xFFFFFFFF) {
        uint32_t version = 0;
        if (oal_pcie_check_link_state_err_adapter(addr_map) == OAL_SUCC) {
            return OAL_TRUE;
        }
        ...
        pci_data = (oal_pcie_linux_res *)oal_pci_get_drvdata(pcie_res_to_dev(pst_pci_res));
        sleep_st = hcc_bus_get_sleep_state(pci_data->pst_bus);
        ...
        pci_print_log(PCI_LOG_INFO, "read pci version 0x%8x ret=%d, host wakeup dev gpio:%d", version, ret, sleep_st);
        do {
...
int32_t oal_pcie_share_mem_res_map(oal_pcie_res *pst_pci_res)
{
    ...
    forever_loop() {
        ...
        if (ret == OAL_SUCC) {
            ...
            pci_print_log(PCI_LOG_ERR, "get share mem address invalid dev add 0x%x, va 0x%lx",
                          pst_pci_res->chip_info.addr_info.sharemem_addr, addr_map.va);
            return ret;
        }
                                         ...
                                         uint32_t data_size)
{
    ...
    if (oal_unlikely(((uintptr_t)ddr_address & 0x3) || (data_size & 0x3))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ddr address %lu, length 0x%x is invalid",
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "cpu address start:0x%lx + length:0x%x  invalid", start, length);
        return ret;
    }
...
OAL_STATIC int32_t oal_pcie_h2d_message_buf_rd_update(oal_pcie_res *pst_pci_res)
{
    ...
    if (oal_warn_on(st_map.va == 0)) { // after init, va should never be 0
        pci_print_log(PCI_LOG_ERR, "ringbuf_ctrl_dma_addr va is null");
        return -OAL_ENOENT;
    }
...
int32_t oal_pcie_read_d2h_message(oal_pcie_res *pst_pci_res, uint32_t *message)
{
    ...
    if (oal_pcie_d2h_message_buf_read(pst_pci_res, pst_ringbuf,
                                      &pst_pci_res->st_message_res.d2h_res.ringbuf_data_dma_addr,
                                      message) != 0) {
        pci_print_log(PCI_LOG_DBG, "oal_pcie_read_d2h_message --");
        return OAL_SUCC;
    } else {
...
void oal_pcie_trigger_message(oal_pcie_res *pst_pci_res)
{
    if (pst_pci_res->chip_info.edma_support == OAL_TRUE) {
        ...
        oal_print_mpxx_log(MPXX_LOG_ERR, "unkown dma");
    }
}
...
int32_t oal_pcie_send_message_to_dev(oal_pcie_res *pst_pci_res, uint32_t message)
{
    ...
    if (freecount == 0) {
        ...
        ret = oal_pcie_h2d_message_buf_rd_update(pst_pci_res);
        if (oal_unlikely(ret != OAL_SUCC)) {
    ...
    if (ret <= 0) {
        ...
        if (oal_pcie_check_link_state(pst_pci_res) == OAL_FALSE) {
            ...
            chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                                 CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_SEND_MESSAGE_FAIL);
        }
        return -OAL_EIO;
    }
...
OAL_STATIC int32_t oal_pcie_print_pcie_regs(oal_pcie_res *pst_pci_res, uint32_t base, uint32_t size)
{
    ...
    if ((size == 0) || (size > 0xffff)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "size invalid %u\n", size);
        return -OAL_EFAIL;
    }
...
#ifdef CONFIG_PRINTK
            /* print to kenrel msg, 32B per row, 4B per group */
...
int32_t oal_pcie_device_phy_config_reg(oal_pcie_res *pst_pci_res, uint32_t pcie_phy_pa,
                                       uint32_t pos, uint32_t bits, uint32_t val)
{
    ...
    /* phy addr */
    ret = oal_pcie_inbound_ca_to_va(pst_pci_res, pcie_phy_pa, &addr_map);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "get dev address 0x%x failed", pcie_phy_pa);
        return -OAL_EFAIL;
    }

    pci_print_log(PCI_LOG_DBG, "get dev address 0x%x success", pcie_phy_pa);

    reg = oal_readl(addr_map.va);
    oal_setl_bits((void*)addr_map.va, pos, bits, val);
    pci_print_log(PCI_LOG_DBG, "PCIe phy reg config: change from 0x%x to 0x%x", reg, oal_readl(addr_map.va));

    return OAL_SUCC;
}

int32_t oal_pcie_device_phy_read_reg(oal_pcie_res *pst_pci_res, uint32_t pcie_phy_pa,
                                     uint32_t pos, uint32_t bits, uint32_t *val)
{
    int32_t ret;
    uint32_t reg;
    ...
    if (val == NULL) {
        pci_print_log(PCI_LOG_ERR, "val ptr is null");
        return -OAL_EINVAL;
    }
    ...
    reg = oal_readl(addr_map.va);
    *val = (reg >> (uint32_t)(pos)) & (((uint32_t)1 << (bits)) - 1);
