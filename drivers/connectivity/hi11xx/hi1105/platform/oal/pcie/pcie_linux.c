...
#define WIFI_SLEEP_REQUEST_TIMEOUT 2000
...
int32_t g_pcie_forbid_dev_access_host = 0;
oal_debug_module_param(g_pcie_forbid_dev_access_host, int, S_IRUGO | S_IWUSR);
...
OAL_STATIC const oal_pci_device_id_stru g_pcie_id_tab[] = {
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP13, PCI_ANY_ID, PCI_ANY_ID }, /* mp13 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP15, PCI_ANY_ID, PCI_ANY_ID }, /* mp15 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP16FPGA, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 FPGA PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP16, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP16, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP16C, PCI_ANY_ID, PCI_ANY_ID }, /* mp16c tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_GF61, PCI_ANY_ID, PCI_ANY_ID }, /* gf61 tmp PCIE */
    { 0, }
};

int32_t oal_get_pcie_forbid_dev_access_host_flag(void)
{
    return g_pcie_forbid_dev_access_host;
}

void oal_set_pcie_forbid_dev_access_host_flag(int32_t value)
{
    g_pcie_forbid_dev_access_host = value;
}
...
OAL_STATIC int32_t oal_pcie_enable_device(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    return oal_pcie_enable_single_device(pst_pci_lres->pst_pcie_dev, &pst_pci_lres->default_state);
}
...
int32_t oal_pcie_enable_dual_device(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    return oal_pcie_enable_single_device(pst_pci_lres->pst_pcie_dev_dual, &pst_pci_lres->default_state_dual);
}
...
OAL_STATIC int32_t oal_pcie_dual_pci_init(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_lres)
{
    ...
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_res is null");
        return -OAL_ENODEV;
    }

    ret = oal_pci_enable_device(pst_pci_dev);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "enable dual pcie failed, ret=%d", ret);
        return ret;
    }
    ...
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "save dual resource failed, ret=%d", ret);
...
OAL_STATIC int32_t oal_pcie_dual_pci_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    ...
    if (g_dual_pci_support == OAL_FALSE) {
        if (g_pcie_linux_res) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "pcie device init failed, already init!\n");
            return -OAL_EBUSY;
        }
    } else {
        ...
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "read dual flag failed, ret=%d", ret);
            return ret;
        }

        oal_print_mpxx_log(MPXX_LOG_INFO, "dual pcie flag: 0x%x", dword);

        if ((uint16_t)dword == (uint16_t)PCIE_DUAL_PCI_MASTER_FLAG) {
            /* pcie0 */
            if (g_pcie_linux_res != NULL) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "dual pcie must probe master ip first!");
                return -OAL_EBUSY;
            }
        }
...
OAL_STATIC void oal_pcie_init_dma_ops(oal_pci_dev_stru *pst_pci_dev)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "pci dma sync flush cache is %s, inv cache is %s",
                       g_hipci_sync_flush_cache_enable == 0 ? "disable" : "enable",
                       g_hipci_sync_inv_cache_enable == 0 ? "disable" : "enable");
}

OAL_STATIC int32_t oal_pcie_dma_cap_init(oal_pci_dev_stru *pst_pci_dev)
{
    int32_t ret;
    oal_print_mpxx_log(MPXX_LOG_INFO, "before set, dma mask:0x%llx coherent mask:0x%llx",
                       dma_get_mask(&pst_pci_dev->dev), pst_pci_dev->dev.coherent_dma_mask);
    // extern pcie dma 64 bits address limit set.
    ret = dma_set_mask_and_coherent(&pst_pci_dev->dev, DMA_BIT_MASK(64));
    if (ret != 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dma set mask failed=%d", ret);
        return ret;
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "after set, dma mask:0x%llx coherent mask:0x%llx",
                       dma_get_mask(&pst_pci_dev->dev), pst_pci_dev->dev.coherent_dma_mask);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_init_dma_attr(oal_pci_dev_stru *pst_pci_dev)
{
    /* cache ops init */
    oal_pcie_init_dma_ops(pst_pci_dev);
    /* dma nocache cap init */
    return oal_pcie_dma_cap_init(pst_pci_dev);
}
...
OAL_STATIC int32_t oal_pcie_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    ...
    ret = oal_pcie_init_dma_attr(pst_pci_dev);
    if (ret != OAL_SUCC) {
        g_probe_ret = -OAL_EINVAL;
        oal_complete(&g_probe_complete);
        return -OAL_EINVAL;
    }
    ...
    pst_pci_lres->version = (pst_pci_dev->vendor) | (pst_pci_dev->device << 16); /* device为版本号的高16位 */
    ...
    ret = oal_pcie_chiptest_init(pst_pci_lres->pst_pci_res, device_id);
    if (ret != OAL_SUCC) {
        oal_io_print("pci: oal_pcie_chiptest_init failed, device_id = %u\n", (uint32_t)device_id);
    }
                                         ...
                                         int32_t is_bind)
{
    ...
    uint32_t first_big_cpu;
    struct cpumask all_cpus, fast_cpus;
    cpumask_setall(&all_cpus);
    external_get_fast_cpus(&fast_cpus);
    first_big_cpu = cpumask_first(&fast_cpus);
    /* 中断允许绑核, 判定PCIE传输中断绑核 */
#ifdef CONFIG_NR_CPUS
#if (KERNEL_VERSION(2, 6, 35) <= LINUX_VERSION_CODE)
    if (bindcpu_cfg->irq_cpu && pci_dev->irq != 0) {
        if (bindcpu_cfg->is_bind) {
            irq_set_affinity_hint(pci_dev->irq, cpumask_of(first_big_cpu));
        } else {
            /* 2.6.35-rc1 */
            irq_set_affinity_hint(pci_dev->irq, cpumask_of(0));
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie bind irq[%d] to cpu %d",
                           pci_dev->irq, (bindcpu_cfg->is_bind ? first_big_cpu : 0));
    }
#endif
#endif

    /* 线程允许绑核, 判定PCIE补内存线程绑核 */
    if (bindcpu_cfg->thread_cmd && pci_lres->pst_pci_res->pst_rx_hi_task != NULL) {
        if (bindcpu_cfg->is_bind) {
            /* 中断所在核会影响线程迁移 */
            oal_cpumask_clear_cpu(first_big_cpu, &fast_cpus);
            ...
            set_cpus_allowed_ptr(pci_lres->pst_pci_res->pst_rx_hi_task, &all_cpus);
        }
...
OAL_STATIC int32_t oal_pcie_dual_reinit(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_dual_reinit");
...
OAL_STATIC int32_t oal_pcie_sleep_wait_rx_trans_done(oal_pcie_res *pst_pci_res)
{
    if (pst_pci_res->chip_info.ete_support != OAL_TRUE) {
        return OAL_SUCC;
    }
    return oal_ete_sleep_wait_rx_trans_done(pst_pci_res);
}
...
int32_t oal_pcie_sleep_request(hcc_bus *pst_bus)
{
    ...
    if (oal_pcie_sleep_wait_rx_trans_done(pst_pci_res) != OAL_SUCC) {
        declare_dft_trace_key_info("wlan_sleep_wait_rx_done_timeout", OAL_DFT_TRACE_FAIL);
    }
    ...
    if (g_pcie_auto_disable_aspm != 0) {
        ...
        oal_pcie_set_device_aspm_dync_disable(pst_pci_res, OAL_FALSE);
    ...
    oal_pcie_device_sr_debug(pst_pci_res, OAL_TRUE);
    oal_pcie_device_auxclk_switch(pst_pci_res, AUXCLK_RTC);
    oal_pcie_change_link_state(pst_pci_res, PCI_WLAN_LINK_UP);
    mutex_unlock(&pst_pci_res->st_rx_mem_lock);

    // pci_clear_master
    oal_pcie_enable_master(pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_FALSE);

    end_time = ktime_get();
    trans_time = ktime_sub(end_time, start_time);
    total_time = (unsigned long long)ktime_to_us(trans_time);
    pci_print_log(PCI_LOG_WARN, "consume %llu us", total_time);
    if (total_time > WIFI_SLEEP_REQUEST_TIMEOUT) {
        pci_print_log(PCI_LOG_INFO, "oal_pcie_sleep_request long_time");
        declare_dft_trace_key_info("oal_pcie_sleep_request long_time", OAL_DFT_TRACE_OTHER);
    }
...
int32_t oal_pcie_wakeup_request(hcc_bus *pst_bus)
{
    ...
    // pci_set_master
    oal_pcie_enable_master(pst_pci_lres->pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_TRUE);
...
int32_t oal_pcie_get_sleep_state(hcc_bus *pst_bus)
{
    ...
    return (conn_get_gpio_level(HOST_WKUP_W) == GPIO_LEVEL_HIGH) ?
...
int32_t oal_pcie_gpio_rx_data(hcc_bus *hi_bus)
{
    ...
    forever_loop() {
        ...
        if (message < D2H_MSG_COUNT) {
            ...
            } else if (message == PCIE_D2H_MESSAGE_ETE_INTR_ERR) {
                pci_print_log(PCI_LOG_ERR, "dev maybe occur err ete int");
                hcc_bus_print_trans_info(hi_bus, HCC_PRINT_TRANS_FLAG_DEVICE_REGS |
                                                 HCC_PRINT_TRANS_FLAG_DEVICE_STAT);
                hcc_bus_exception_submit(pst_pci_lres->pst_bus, HCC_BUS_TRANS_FAIL);
            }
        }
    }
...
OAL_STATIC hcc_bus *oal_pcie_bus_init(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    pst_bus->bus_id = 0x0;
    pst_bus->dev_id = HCC_EP_WIFI_DEV; /* 这里可以根据 vendor id 区分110X 和118X */
...
int32_t wlan_first_power_on_callback(void *data)
{
    ...
    hi_wlan_power_set(1);
    /* do something before pcie probe */
...
OAL_STATIC int32_t wlan_first_power_off_fail_callback(void *data)
{
    ...
    hw_mpxx_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_PCIE_ENMU_FAIL, "pcie enmu fail\n");
#endif
    oal_io_print("wlan_first_power_off_fail_callback\n");
    pcie_linkup_fail_fixup(0);
...
int32_t oal_pcie_dual_power_on_callback(void *data)
{
    ...
    mdelay(10); /* 等待10ms，待拉gpio后状态稳定后使能phy tracer */
    pcie_linkup_prepare_fixup(1);
    return 0;
}
...
OAL_STATIC void oal_pcie_speed_change_init(void)
{
...
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
int32_t oal_pcie_mpxx_dual_init(void)
{
#ifdef CONFIG_ARCH_KIRIN_PCIE
    int32_t ret;
#endif
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
OAL_STATIC int32_t oal_kirin_pcie_enum(void)
{
    int32_t ret;
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
    ret = oal_kirin_pcie_enum();
    if (ret != OAL_SUCC) {
        oal_io_print("kirin pcie enum fail\n");
        oal_pci_unregister_driver(&g_pcie_drv);
        return ret;
    }
#endif
