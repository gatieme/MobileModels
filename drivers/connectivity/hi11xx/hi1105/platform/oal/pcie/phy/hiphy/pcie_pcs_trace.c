 ...
 *
 * Description:pcie pcs trace driver
...
int32_t pcie_pcs_write_regtable(uintptr_t base_addr, pcs_trace_conf *trace_conf, pcs_set_bits32 set_bits32)
{
    ...
    for (i = 0;; i++) {
        ...
        oal_print_mpxx_log(MPXX_LOG_INFO, "0x%x set pos=%u bits=%u value=0x%x",
                              ...
                              pcs_set_bits32 set_bits32, uint32_t lane)
{
    if (oal_warn_on(set_bits32 == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "set_bits32 is null");
        return -OAL_ENODEV;
    }
                               ...
                               uint8_t *buff, int32_t size)
{
    ...
    if (oal_warn_on(buff == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "buff is null");
        return -OAL_ENODEV;
    }

    if ((set_bits32 == NULL) || (get_bits32 == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "bits32 ops is null");
        return -OAL_ENODEV;
    }
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, " mem buff overrun, size=%d", size);
        return -OAL_ENOMEM;
    }
    offset += ret;
