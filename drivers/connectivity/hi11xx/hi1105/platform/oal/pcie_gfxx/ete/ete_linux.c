                                             ...
                                             hcc_bus *pst_bus)
{
    ...
    ret = oal_ete_transfer_deepwkup_restore(pst_pci_lres);
    if (ret != OAL_SUCC) {
...
OAL_STATIC int32_t oal_pcie_ete_wake_powerup_handler(void *data)
{
    ...
    enum gpio_type  type = (pst_pci_lres->dev_id == HCC_EP_WIFI_DEV ?
                            HOST_WKUP_W : HOST_WKUP_GT);
    ...
    if (type != HOST_WKUP_W) {
        pci_print_log(PCI_LOG_ERR, "dev only support wifi");
        return 0;
    }
    /* for dual pcie */
    mutex_lock(&pst_pci_lres->comm_res->sr_lock);
    if (conn_get_gpio_level(type) == GPIO_LEVEL_LOW) {
retry:
    ...
    mutex_unlock(&pst_pci_lres->comm_res->sr_lock);
...
OAL_STATIC int32_t oal_pcie_ete_suspend_handler(void *data)
{
    ...
    pci_print_log(PCI_LOG_INFO, "ep_res.power_status = %d ep_res.link_status = %d",
                  pst_pci_lres->ep_res.power_status, pst_pci_lres->ep_res.link_state);

    pst_pci_lres->ep_res.power_status = PCIE_EP_IP_POWER_DOWN;
    oal_pcie_change_link_state(pst_pci_lres, PCI_WLAN_LINK_DEEPSLEEP);

    pci_print_log(PCI_LOG_INFO, "pcie_suspend refcnt=%d pci_cnt=%d",
                  oal_atomic_read(&pst_pci_lres->comm_res->refcnt), pst_pci_lres->comm_res->pci_cnt);

    declare_dft_trace_key_info("pcie_suspend_powerdown", OAL_DFT_TRACE_SUCC);
    oal_ete_transfer_deepsleep_clean(pst_pci_lres);

    oal_pcie_power_notifiy_register(oal_pcie_get_pcie_rc_idx(pst_pci_dev),
        oal_pcie_ete_wake_powerup_handler, NULL, (void*)pst_pci_lres);
    return 0;
}
...
OAL_STATIC int32_t oal_pcie_ete_poweroff_complete(hcc_bus *pst_bus)
{
    ...
    if (oal_unlikely(pst_pci_lres->ep_res.link_state < PCI_WLAN_LINK_MEM_UP)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ete int link invaild, link_state:%s",
                           oal_pcie_get_link_state_str(pst_pci_lres->ep_res.link_state));
        return -OAL_ENODEV;
    }

    /* :device下电关闭pll, 异常上报的mac中断会导致pcie异常;因此, 首先屏蔽中断 */
    ...
    for (i = 0; i < ACK_CHECK_MAX_CNT; i++) {
        ...
        ret = pst_pci_lres->ep_res.chip_info.cb.pcie_poweroff_complete(pst_pci_lres);
        if (ret == OAL_SUCC || ret == -OAL_ENODEV) {
            break;
        }
    }
    return ret;
...
int32_t oal_pcie_ete_suspend(oal_pci_dev_stru *pst_pci_dev, oal_pm_message_t state)
{
    ...
    if (atomic_inc_return(&pst_pci_lres->comm_res->refcnt) == 1) {
        ...
        if (down_interruptible(&pst_bus->sr_wake_sema)) {
            oal_io_print("pcie_wake_sema down failed.");
    ...
    oal_pcie_power_notifiy_register(oal_pcie_get_pcie_rc_idx(pst_pci_dev),
        NULL, oal_pcie_ete_suspend_handler, (void *)pst_pci_lres);
    return 0;
}
...
OAL_STATIC int32_t oal_pcie_ete_resume_linkup_check(oal_pcie_linux_res *pst_pci_lres, oal_pci_dev_stru *pst_pci_dev)
{
    ...
    if (ret) {
        ...
        oal_msleep(1000); // 等待1000ms，重新读取pcie配置空间
...
OAL_STATIC void oal_pcie_host_ip_ete_exit(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_host_ip_ete_exit");
...
{
    /* 部分ete power down场景并未调用power lauch, 未走ETE初始化,不需要释放 */
    if (pst_pci_lres->ep_res.link_state > PCI_WLAN_LINK_MEM_UP) {
...
OAL_STATIC OAL_INLINE int32_t oal_ete_bus_power_up(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    ...
    ret = oal_pcie_pm_control(pst_pci_lres, g_rc_idx, PCIE_POWER_ON);
    ...
    if (ret) {
        oal_io_print("pcie power on and link failed, ret=%d\n", ret);
        (void)ssi_dump_err_regs(SSI_ERR_PCIE_POWER_UP_FAIL);
        return ret;
    }
    oal_atomic_inc(&pst_pci_lres->comm_res->refcnt);
    pst_pci_lres->comm_res->pci_cnt = 1; // 1 pcie ip
    oal_pcie_register_event(pst_pci_lres->comm_res->pcie_dev, pst_pci_lres);
...
OAL_STATIC OAL_INLINE int32_t oal_ete_bus_power_patch_lauch(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    ...
    if (ret) {
        oal_io_print("pcie_transfer_res_init failed, ret=%d\n", ret);
        return ret;
    }
