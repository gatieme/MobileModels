 ...
 *
 * Description:pcie driver,Debug code
...
/* Debug functions */
OAL_STATIC int32_t oal_pcie_dump_all_regions_mem(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                 oal_pcie_linux_res *pst_pcie_res)
{
    int32_t index;
    int32_t region_num;
    ...
    pst_pci_dev = pst_pcie_res->comm_res->pcie_dev;
    if (pst_pci_dev == NULL) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(!pst_pcie_res->comm_res->regions.inited)) {
        return -OAL_EBUSY;
    }

    region_num = pst_pcie_res->comm_res->regions.region_nums;
    region_base = pst_pcie_res->comm_res->regions.pst_regions;
...
OAL_STATIC int32_t oal_pcie_dump_all_regions(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                             oal_pcie_linux_res *pst_pcie_res)
{
    if (pst_pcie_res == NULL) {
    ...
    oal_pcie_regions_info_dump(pst_pcie_res->comm_res);
    return OAL_SUCC;
}

OAL_STATIC void oal_pcie_trans_timedebug(ktime_t start_time)
{
    unsigned long long total_time;
    ktime_t  end_time, trans_time;
    static unsigned long long max_trans_time = 0;
    ...
    if (total_time > max_trans_time) {
        max_trans_time = total_time;
    }

    num++;
    pci_print_log(PCI_LOG_INFO, "[PCIE]pcie_trans_timedebug, count [%llu], current time [%llu]us, max time [%llu]us\n",
                  num, total_time, max_trans_time);
}

OAL_STATIC int32_t oal_pcie_debug_read32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                         oal_pcie_linux_res *pst_pcie_res)
{
    ...
    oal_pcie_trans_timedebug(start_time);
    oal_io_print("pcie_read32 0x%8x read success!\n", cpu_address);
    oal_pcie_print_bits(&value, sizeof(uint32_t));
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_debug_write32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                          oal_pcie_linux_res *pst_pcie_res)
{
    ...
    oal_pci_dev_stru *pst_pci_dev = NULL;
    ktime_t start_time;
    if ((sscanf_s(buf, "0x%x 0x%x", &cpu_address, &value) != 2)) { /* 输入2个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_read_dsm32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                             oal_pcie_linux_res *pst_pcie_res)
{
    int32_t ret;
    uint32_t val, type;
...
OAL_STATIC int32_t oal_pcie_debug_write_dsm32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                              oal_pcie_linux_res *pst_pcie_res)
{
    int32_t ret;
    uint32_t val, type;
    if ((sscanf_s(buf, "%u %u", &type, &val) != 2)) { /* 输入2个参数 */
...
#define ACCESS_MEM_MAX 16
struct access_mem {
    int32_t subsys;
    char name[MEM_NAME_LEN]; // 通路访问IP
    int32_t addr; // 访问地址
    int32_t wr_val; // 写入数据的值
    int32_t rd_val; // 读取数据的值
};

struct pm_test_trace_info {
    volatile int32_t cmd; // host下发命令
    volatile int32_t sleep_cnt; // 进入睡眠次数
    volatile int32_t wakeup_cnt; // 被唤醒次数
    volatile int32_t wakeup_int_cnt; // 唤醒后执行中断的次数
    volatile int32_t subsys_sleep_sts; // 各子系统睡眠状态
    volatile int32_t time_runner; // 不断累加器, 观测子系统是否还正常运行
    struct access_mem mems[ACCESS_MEM_MAX];
};

OAL_STATIC struct pm_test_trace_info* oal_pcie_pm_get_trace_info(oal_pcie_linux_res *pst_pcie_res, uint32_t subsys_reg)
{
    int32_t ret;
    uint32_t trace_addr;
    pci_addr_map addr_map;
    struct pm_test_trace_info *trace_info = NULL;

    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, subsys_reg, &addr_map);
    if (ret != OAL_SUCC) {
        oal_io_print("pcie_read32 0x%8x unmap, read failed!\n", subsys_reg);
        return NULL;
    }
    /* 加4移到下一地址, 左移16bit为uint32_t高16位 */
    trace_addr = *(uint16_t *)addr_map.va + ((*(uint16_t *)(addr_map.va + 4)) << 16);
    printk("%-20s:0x%-x\r\n", "trace_addr", trace_addr);
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, trace_addr, &addr_map);
    if (ret != OAL_SUCC) {
        oal_io_print("pcie_read32 0x%8x unmap, read failed!\n", trace_addr);
        return NULL;
    }
    trace_info = (struct pm_test_trace_info *)addr_map.va;

    return trace_info;
}

OAL_STATIC int32_t oal_pcie_subsys_sleep_cmd_proc(struct pm_test_trace_info *trace_info)
{
    oal_writel(0x2, (void *)&trace_info->cmd);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_subsys_access_cmd_proc(struct pm_test_trace_info *trace_info)
{
    oal_writel(0x1, (void *)&trace_info->cmd);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_pm_trace_subsys(struct pm_test_trace_info *trace_info)
{
    int32_t i;

    printk("%-20s:%-d\r\n", "sleep_cnt", trace_info->sleep_cnt);
    printk("%-20s:%-d\r\n", "wakeup_cnt", trace_info->wakeup_cnt);
    printk("%-20s:%-d\r\n", "wakeup_int_cnt", trace_info->wakeup_int_cnt);
    printk("%-20s:0x%-x\r\n", "subsys_sleep_sts", trace_info->subsys_sleep_sts);
    printk("%-20s:%-d\r\n", "time_runner", trace_info->time_runner);
    printk("%-16s %-10s %-10s %-10s\r\n", "mem", "addr", "wr_val", "rd_val");
    for (i = 0; i < ACCESS_MEM_MAX; i++) {
        if (trace_info->mems[i].name[0] == '\0') {
            continue;
        }
        printk("%-14s 0x%08x 0x%08x 0x%08x\r\n", trace_info->mems[i].name,
            trace_info->mems[i].addr, trace_info->mems[i].wr_val, trace_info->mems[i].rd_val);
    }
    return OAL_SUCC;
}

#define GT_TRACE_INFO_REG 0x40001c30
#define WIFI_TRACE_INFO_REG 0x40001c38
#define BT_TRACE_INFO_REG 0x40001c40
#define SLE_TRACE_INFO_REG 0x40001c48
struct pcie_pm_test_info {
    char subsys[8];
    uint32_t trace_reg;
};

struct pcie_pm_test_info g_pm_test_info[] = {
    {"gt", 0x40001c30},
    {"wifi", 0x40001c38},
    {"bt", 0x40001c40},
    {"sle", 0x40001c48},
...
OAL_STATIC int32_t oal_pcie_pm_sleep_test(struct kobject *dev, struct kobj_attribute *attr,
                                          const char *buf, oal_pcie_linux_res *pst_pcie_res)
{
    uint32_t i;
    char subsys[16] = {0};
    struct pm_test_trace_info *trace_info = NULL;

    if ((sscanf_s(buf, "%s", subsys, sizeof(subsys)) != 1)) {
        pci_print_log(PCI_LOG_ERR, "invalid agrument");
        return -OAL_EINVAL;
    }
    for (i = 0; i < oal_array_size(g_pm_test_info); i++) {
        if (oal_strcmp(g_pm_test_info[i].subsys, subsys) == 0) {
            trace_info = oal_pcie_pm_get_trace_info(pst_pcie_res, g_pm_test_info[i].trace_reg);
            oal_pcie_subsys_sleep_cmd_proc(trace_info);
        }
    }
...
OAL_STATIC int32_t oal_pcie_pm_access_test(struct kobject *dev, struct kobj_attribute *attr,
                                           const char *buf, oal_pcie_linux_res *pst_pcie_res)
{
    uint32_t i;
    char subsys[16] = {0};
    ...
    for (i = 0; i < oal_array_size(g_pm_test_info); i++) {
        if (oal_strcmp(g_pm_test_info[i].subsys, subsys) == 0) {
            trace_info = oal_pcie_pm_get_trace_info(pst_pcie_res, g_pm_test_info[i].trace_reg);
            oal_pcie_subsys_access_cmd_proc(trace_info);
            oal_msleep(100); // 写后等待100ms
            oal_pcie_pm_trace_subsys(trace_info);
        }
    }
...
OAL_STATIC int32_t oal_pcie_debug_host_read32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                              oal_pcie_linux_res *pst_pcie_res)
{
...
OAL_STATIC int32_t oal_pcie_debug_host_write32(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                               oal_pcie_linux_res *pst_pcie_res)
{
    ...
    if ((sscanf_s(buf, "0x%lx 0x%x", &cpu_address, &value) != 2)) { /* 输入2个参数 */
        return -OAL_EINVAL;
    }
    ...
    oal_io_print("host_write32  change 0x%lx from 0x%8x to 0x%8x callback-read= 0x%8x\n", cpu_address, old, value,
                 oal_readl((void *)pst_align_addr));
    oal_iounmap(pst_vaddr);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_debug_read16(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                         oal_pcie_linux_res *pst_pcie_res)
{
    /* echo read32 address > debug */
    int32_t ret;
...
OAL_STATIC int32_t oal_pcie_debug_write16(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                          oal_pcie_linux_res *pst_pcie_res)
{
    /* echo write32 address value > debug */
    int32_t ret;
    ...
    oal_io_print("pcie_write16  change 0x%8x from 0x%4x to 0x%4x callback-read= 0x%4x\n", cpu_address, old,
                 (uint16_t)value, readw((void *)addr_map.va));
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_saveconfigmem(oal_pcie_linux_res *pst_pcie_res, const char *file_name, uint32_t cpu_address,
                                          uint32_t length)
{
    struct file *fp = NULL;
    uint32_t index;
    ...
    fs = get_fs();
    set_fs(KERNEL_DS);
    fp = filp_open(file_name, O_RDWR | O_CREAT, 0664); /* 0664->rw-rw-r */
...
OAL_STATIC int32_t oal_pcie_savemem(oal_pcie_linux_res *pst_pcie_res, const char *file_name, uint32_t cpu_address,
                                    uint32_t length)
{
    struct file *fp = NULL;
...
OAL_STATIC int32_t oal_pcie_save_hostmem(oal_pcie_linux_res *pst_pcie_res, const char *file_name,
                                         uintptr_t host_address, uint32_t length)
{
    ...
    if (index) {
        ...
        pci_print_log(PCI_LOG_WARN, "save_hostmem cpu:0x%lx len:%u save_path:%s failed!", host_address, length,
                      file_name);
        ret = -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_savemem(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                          oal_pcie_linux_res *pst_pcie_res)
{
    ...
    if ((sscanf_s(buf, "0x%x %u %s", &cpu_address, &length,
        file_name, sizeof(file_name)) != 3)) { /* 输入3个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_saveconfigmem(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                oal_pcie_linux_res *pst_pcie_res)
{
    char file_name[PCIE_DEBUG_MSG_LEN];
    uint32_t cpu_address, length;
    ...
    if ((sscanf_s(buf, "0x%x %u %s", &cpu_address,
        &length, file_name, sizeof(file_name)) != 3)) { /* 输入3个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_save_hostmem(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                               oal_pcie_linux_res *pst_pcie_res)
{
    char file_name[PCIE_DEBUG_MSG_LEN];
    ...
    if ((sscanf_s(buf, "0x%lx %u %s", &host_address,
        &length, file_name, sizeof(file_name)) != 3)) { /* 输入3个参数 */
        return -OAL_EINVAL;
    }
...
int32_t oal_pcie_print_device_mem(oal_pcie_linux_res *pst_pcie_res, uint32_t cpu_address, uint32_t length)
{
    ...
    if (index) {
        pci_print_log(PCI_LOG_INFO, "readmem cpu:0x%8x len:%u, va:0x%lx, pa:0x%lx  done", cpu_address, index,
                      addr_map.va, addr_map.pa);
        /* print the read buf before errors, rowsize = 16, groupsize = 4 */
        print_hex_dump(KERN_INFO, "readmem: ", DUMP_PREFIX_OFFSET, 16, 4, print_buf, length,
                       true); /* 内核函数固定的传参 */
    } else {
...
OAL_STATIC int32_t oal_pcie_debug_readmem(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                          oal_pcie_linux_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    uint32_t cpu_address, length;

    if ((sscanf_s(buf, "0x%x %u", &cpu_address, &length) != 2)) { /* 输入2个参数 */
        return -OAL_EINVAL;
    }

    return oal_pcie_print_device_mem(pst_pcie_res, cpu_address, length);
}

OAL_STATIC int32_t oal_pcie_debug_readmem_config(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                 oal_pcie_linux_res *pst_pcie_res)
{
    /* echo readconfigmem address length(hex) > debug */
...
OAL_STATIC int32_t oal_pcie_send_test_pkt(int32_t num)
{
    ...
    hcc_bus_dev *bus_dev = hcc_get_get_one_alive_bus_dev();
    struct hcc_handler *hcc = (bus_dev != NULL) ? bus_dev->hcc : NULL;
    ...
    do {
        ...
        hcc_hdr_param_init(&st_hcc_transfer_param, HCC_ACTION_TYPE_TEST, HCC_TEST_SUBTYPE_CMD, 0, HCC_FC_WAIT, 0);
...
OAL_STATIC void oal_pcie_outbound_test_init(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                            uint32_t outbound_size, dma_addr_t outbound_dma_addr)
{
    if (pst_pcie_res->ep_res.chip_info.edma_support == OAL_TRUE) {
        void *inbound_addr = pst_pcie_res->comm_res->st_iatu_bar.st_region.vaddr;
        if (pst_pcie_res->comm_res->revision == PCIE_REVISION_4_70A) {
            ...
            oal_pci_write_config_dword(pst_pci_dev, 0x700 + 0x214, 0x70000000 + outbound_size - 1); /* limit */
            oal_pci_write_config_dword(pst_pci_dev, 0x700 + 0x218, (uint32_t)outbound_dma_addr);    /* target lower */
            oal_pci_write_config_dword(pst_pci_dev, 0x700 + 0x21c, 0);                              /* target upper */
        } else if (pst_pcie_res->ep_res.chip_info.membar_support == OAL_TRUE) {
            ...
            oal_writel(0x0,
                       inbound_addr + hi_pci_iatu_region_ctrl_1_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
            ctr2.as_dword = 0;
            ctr2.bits.region_en = 1;
            ctr2.bits.bar_num = 0x0;
            oal_writel(ctr2.as_dword,
                       inbound_addr + hi_pci_iatu_region_ctrl_2_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));

            oal_writel(0x80000000,
                       inbound_addr + hi_pci_iatu_lwr_base_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
            oal_writel(0x0,
                       inbound_addr + hi_pci_iatu_upper_base_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
            oal_writel(0x80000000 + outbound_size - 1,
                       inbound_addr + hi_pci_iatu_limit_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));

            oal_writel((uint32_t)outbound_dma_addr,
                       inbound_addr + hi_pci_iatu_lwr_target_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
#ifdef CONFIG_ARCH_DMA_ADDR_T_64BIT
            /* 64位地址取高32bit写入寄存器 */
            oal_writel((uint32_t)(outbound_dma_addr >> 32), inbound_addr +
                hi_pci_iatu_upper_target_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
#else
            oal_writel((uint32_t)0, inbound_addr +
                hi_pci_iatu_upper_target_addr_off_outbound_i(hi_pci_iatu_outbound_base_off(index)));
#endif
        } else {
            pci_print_log(PCI_LOG_ERR, "unkown pcie ip revision :0x%x", pst_pcie_res->comm_res->revision);
        }
    }

    if (pst_pcie_res->ep_res.chip_info.ete_support == OAL_TRUE) {
        int32_t ret;
        uint64_t host_iova = 0;
        uint64_t devva = 0;
        ret = pcie_if_hostca_to_devva(pst_pcie_res->pst_bus->dev_id, (uint64_t)outbound_dma_addr, &devva);
        if (ret == OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_if_hostca_to_devva iova=0x%llx -> devva=0x%llx",
                               (uint64_t)outbound_dma_addr, devva);
            ret = pcie_if_devva_to_hostca(pst_pcie_res->pst_bus->dev_id, devva, &host_iova);
            if (ret == OAL_SUCC) {
                oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_if_devva_to_hostca devva=0x%llx -> iova=0x%llx", devva,
                                   host_iova);
            } else {
                oal_print_mpxx_log(MPXX_LOG_ERR, "pcie_if_devva_to_hostca failed devva=0x%llx", devva);
            }
        }
    }
}
 ...
 * mp13 Slave 空间为256M
 */
OAL_STATIC int32_t oal_pcie_outbound_test(oal_pcie_linux_res *pst_pcie_res, const char *buf)
{
    ...
    if (!oal_strcmp(casename, "rebound")) {
        ...
        outbound_vaddr = dma_alloc_coherent(&pst_pci_dev->dev, outbound_size, &outbound_dma_addr, GFP_KERNEL);
        if (outbound_vaddr) {
...
OAL_STATIC int32_t oal_pcie_addr_switch_test(oal_pcie_linux_res *pst_pcie_res, const char *buf)
{
    int32_t ret;
    if (pst_pcie_res->ep_res.chip_info.addr_info.glb_ctrl != 0) {
        uint64_t host_va = 0;
        uint64_t dev_cpuaddr = 0;
        ret = oal_pcie_devca_to_hostva(pst_pcie_res->pst_bus->dev_id,
            pst_pcie_res->ep_res.chip_info.addr_info.glb_ctrl, &host_va);
        if (ret == OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_devca_to_hostva 0x%x -> 0x%llx, value=0x%x",
                               pst_pcie_res->ep_res.chip_info.addr_info.glb_ctrl, host_va,
                               oal_readl((void *)(uintptr_t)host_va));
            /* master hostva->devca */
            ret = oal_pcie_get_dev_ca(0, (void *)(uintptr_t)host_va, &dev_cpuaddr);
            if (ret == OAL_SUCC) {
                oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_get_dev_ca 0x%llx -> 0x%x", host_va,
                                   (uint32_t)dev_cpuaddr);
            } else {
                oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_get_dev_ca 0x%llx  convert failed", host_va);
            }
        } else {
            oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_devca_to_hostva 0x%x  convert failed",
                               pst_pcie_res->ep_res.chip_info.addr_info.glb_ctrl);
        }
    } else {
        oal_print_mpxx_log(MPXX_LOG_ERR, "glb_ctrl is 0");
    }

    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_device_memcheck(oal_pcie_linux_res *pst_pcie_res, uint32_t cpuaddr, uint32_t size,
                                            uint32_t data)
{
    ...
    for (i = 0, mode = 0; i < (size); i += sizeof(uint32_t), mode++) { /* 每次偏移4字节，直到超过size */
        if (mode % 4 < 2) { /* 2块4字节地址写data，2块4字节地址写~data，测试mem用 */
            data_wt = data;
        } else {
            data_wt = ~data;
        }
...
OAL_STATIC int32_t oal_pcie_device_memcheck_auto(oal_pcie_linux_res *pst_pcie_res)
{
    int32_t ret, i;
    ...
    for (i = 0; i < sizeof(test_address) / sizeof(test_address[0]); i++) {
        pci_print_log(PCI_LOG_INFO, "Test address: 0x%8x ,size: 0x%8x start.", test_address[i].address,
                      test_address[i].size);
        ...
        pci_print_log(PCI_LOG_INFO, "Test address: 0x%8x ,size: 0x%8x OK.", test_address[i].address,
                      test_address[i].size);
    }
...
OAL_STATIC int32_t oal_pcie_debug_wlan_power_on(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                oal_pcie_linux_res *pst_pcie_res)
{
    oal_pci_dev_stru *pst_pci_dev = NULL;
...
OAL_STATIC int32_t oal_pcie_testcase_transfer_res_init(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                       const char *buf)
{
    int32_t ret;
...
OAL_STATIC int32_t oal_pcie_testcase_send_test_pkt(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
    return oal_pcie_send_test_pkt(1); /* 表示测试循环次数为1 */
}

OAL_STATIC int32_t oal_pcie_testcase_send_test_pkt2(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    return oal_pcie_send_test_pkt(2); /* 表示测试循环次数为2 */
}

OAL_STATIC int32_t oal_pcie_testcase_send_test_pkt3(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                    const char *buf)
{
    return oal_pcie_send_test_pkt(3); /* 表示测试循环次数为3 */
}

OAL_STATIC int32_t oal_pcie_testcase_outbound_test(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
    buf = buf + OAL_STRLEN("outbound_test");
    for (; *buf == ' '; buf++) {
    }
    return oal_pcie_outbound_test(pst_pcie_res, buf);
}

static uintptr_t oal_pcie_get_phy_paddr(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_res,
                                        uint32_t phy_paddr, uint32_t dump_type)
{
    int32_t ret;
    pci_addr_map st_map;
...
#define PCIE_EYE_TEST_PARAM_NUM 3
/* command: deye_pattern_test phy_address(hex) lanenum dump_type  */
OAL_STATIC int32_t oal_pcie_testcase_dpattern_test(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
    int32_t ret;
    uintptr_t base_addr = 0;
    uint32_t phy_paddr = 0xffffffff;
    uint32_t dump_type, lane;
    buf = buf + OAL_STRLEN("deye_pattern_test");
    for (; *buf == ' '; buf++)
        ;
    if ((sscanf_s(buf, "0x%x %u %u", &phy_paddr, &lane, &dump_type) != PCIE_EYE_TEST_PARAM_NUM)) {
        pci_print_log(PCI_LOG_ERR, "invalid agrument");
        return -OAL_EINVAL;
    }

    pci_print_log(PCI_LOG_INFO, "phy_paddr:0x%x lane=%u dump_type=%u", phy_paddr, lane, dump_type);

    base_addr = oal_pcie_get_phy_paddr(pst_pci_dev, pst_pcie_res, phy_paddr, dump_type);
    if (base_addr == 0) {
        return -OAL_EINVAL;
    }

    /* 手动测试，通过memport测试 */
    ret = pcie_pcs_deye_pattern_host_test(base_addr, lane, dump_type);
    return ret;
}

OAL_STATIC int32_t oal_pcie_testcase_addr_switch_test(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
    buf = buf + OAL_STRLEN("addr_switch_test");
    for (; *buf == ' '; buf++) {
    }
    return oal_pcie_addr_switch_test(pst_pcie_res, buf);
}

OAL_STATIC int32_t oal_pcie_testcase_ram_memcheck(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    /* 遍历itcm,dtcm,pktmem,扫内存 */
    return oal_pcie_device_memcheck_auto(pst_pcie_res);
}

OAL_STATIC int32_t oal_pcie_testcase_pme_enable(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                const char *buf)
{
    int32_t ret;
    int32_t pm;
    ...
    pci_print_log(PCI_LOG_INFO, "PME OFF : %d", pm);
    ret = oal_pci_read_config_dword(pst_pci_dev, pm + PCI_PM_CTRL, &reg);
    if (ret) {
        pci_print_log(PCI_LOG_ERR, "read %d failed", pm + PCI_PM_CTRL);
        return OAL_SUCC;
    }

    oal_io_print("read %d value:\n", pm + PCI_PM_CTRL);
    ...
    if (ret) {
        pci_print_log(PCI_LOG_ERR, "write %d failed", pm + PCI_PM_CTRL);
    } else {
        pci_print_log(PCI_LOG_INFO, "write %d ok", pm + PCI_PM_CTRL);
    }
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_clear_trans_info(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                      const char *buf)
{
    size_t src_size;
    void *src = NULL;

    src = (void *)pst_pcie_res->ep_res.st_tx_res[PCIE_H2D_QTYPE_NORMAL].stat.tx_burst_cnt;
    src_size = sizeof(pst_pcie_res->ep_res.st_tx_res[PCIE_H2D_QTYPE_NORMAL].stat.tx_burst_cnt);
    memset_s(src, src_size, 0, src_size);

    src = (void *)pst_pcie_res->ep_res.st_rx_res.stat.rx_burst_cnt;
    src_size = sizeof(pst_pcie_res->ep_res.st_rx_res.stat.rx_burst_cnt);
    memset_s(src, src_size, 0, src_size);
    oal_io_print("clear_trans_info done\n");
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_show_trans_info(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    uint32_t i, sz;
    oal_io_print("tx h2d trans info:\n");
    sz = oal_array_size(pst_pcie_res->ep_res.st_tx_res[PCIE_H2D_QTYPE_NORMAL].stat.tx_burst_cnt);
    for (i = 0; i < sz; i++) {
        oal_io_print("[%3d] burst cnt:%u\n", i,
                     pst_pcie_res->ep_res.st_tx_res[PCIE_H2D_QTYPE_NORMAL].stat.tx_burst_cnt[i]);
    }

    oal_io_print("\nrx d2h trans info:\n");
    sz = oal_array_size(pst_pcie_res->ep_res.st_rx_res.stat.rx_burst_cnt);
    for (i = 0; i < sz; i++) {
        oal_io_print("[%3d] burst cnt:%u\n", i, pst_pcie_res->ep_res.st_rx_res.stat.rx_burst_cnt[i]);
    }
...
OAL_STATIC int32_t oal_pcie_testcase_send_message(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    int32_t ret;
    uint32_t message;
    ...
    if ((sscanf_s(buf, "%u", &message) != 1)) {
        ...
        pci_print_log(PCI_LOG_INFO, "send pcie message %u to dev %s", message, (ret == OAL_SUCC) ? "succ" : "failed");
    }
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_sched_rx_task(oal_pcie_linux_res *pst_pcie_lres, oal_pci_dev_stru *pst_pci_dev,
                                                   const char *buf)
{
    if (pst_pcie_lres != NULL && pst_pcie_lres->pst_bus != NULL) {
        up(&pst_pcie_lres->pst_bus->rx_sema);
    } else {
...
OAL_STATIC int32_t oal_pcie_testcase_turnoff_message(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_suspend_test(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                  const char *buf)
{
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_resume_test(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                 const char *buf)
{
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_rc_send_polling(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_testcase_ep_send_polling(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev,
                                                     const char *buf)
{
    uint16_t val = 0;
    ...
    if (ret == 0) {
        ...
        val |= (1 << 4); /* bit[4] 写1， PCIE_CAP_ENTER_COMPLIANCE */
...
typedef struct _pcie_debug_testcase_ {
    char *name;
    int32_t (*testcase)(oal_pcie_linux_res *pst_pcie_res, oal_pci_dev_stru *pst_pci_dev, const char *buf);
} pcie_debug_testcase;
...
OAL_STATIC pcie_debug_testcase g_pci_debug_testcases[] = {
    ...
    { "deye_pattern_test", oal_pcie_testcase_dpattern_test },
    ...
    { ETE_MEMCPY_TEST_STR, oal_pcie_testcase_ete_memcpy_test },
#endif
};

OAL_STATIC int32_t oal_pcie_debug_testcase(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                           oal_pcie_linux_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    int32_t i;
...
OAL_STATIC int32_t oal_pcie_performance_read(oal_pcie_linux_res *pst_pcie_res, uint32_t cpu_address, uint32_t length,
                                             uint32_t times, uint32_t burst_size)
{
    int32_t i;
    int32_t ret;
...
OAL_STATIC int32_t oal_pcie_performance_netbuf_alloc(oal_pcie_linux_res *pst_pcie_res, uint32_t test_time)
{
    uint64_t count;
    int32_t ret = OAL_SUCC;
    oal_netbuf_stru *pst_netbuf = NULL;
    ...
    forever_loop()
    {
        ...
        pci_dma_addr = dma_map_single(&pst_pci_dev->dev, oal_netbuf_data(pst_netbuf), oal_netbuf_len(pst_netbuf),
                                      PCI_DMA_FROMDEVICE);
        if (dma_mapping_error(&pst_pci_dev->dev, pci_dma_addr)) {
            pci_print_log(PCI_LOG_INFO, "rx dma map netbuf failed, len=%u", oal_netbuf_len(pst_netbuf));
            oal_netbuf_free(pst_netbuf);
            break;
        }
...
OAL_STATIC int32_t oal_pcie_performance_netbuf_queue(oal_pcie_linux_res *pst_pcie_res, uint32_t test_time,
                                                     int32_t alloc_count)
{
    uint64_t count;
    int32_t i;
    ...
    forever_loop()
    {
        ...
        forever_loop()
        {
            ...
            pst_cb_res = (pcie_cb_dma_res *)oal_netbuf_cb(pst_netbuf);
            /* 释放内存 */
            dma_unmap_single(&pst_pci_dev->dev, (dma_addr_t)pst_cb_res->paddr.addr, pst_cb_res->len,
                             PCI_DMA_FROMDEVICE);
            oal_netbuf_free(pst_netbuf);
            count++;
        }
    ...
    oal_io_print("alloc_count:%d, thoughtput:%llu Mbps, trans_time:%llu us,  %llu s\n", alloc_count, trans_size,
                 trans_us, us_to_s);

    return ret;
}

OAL_STATIC int32_t oal_pcie_performance_cpu(oal_pcie_linux_res *pst_pcie_res, uint32_t length, uint32_t times,
                                            uint32_t burst_size)
{
    int32_t i;
    int32_t ret = OAL_SUCC;
...
OAL_STATIC int32_t oal_pcie_performance_write(oal_pcie_linux_res *pst_pcie_res, uint32_t cpu_address, uint32_t length,
                                              uint32_t times, uint32_t burst_size, uint32_t value)
{
    ...
    oal_io_print(
        " write length:%u, total_size:%llu,burst_size:%u,value:0x%x thoughtput:%llu Mbps, \
...
OAL_STATIC int32_t oal_pcie_debug_performance_read(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                   oal_pcie_linux_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    int32_t ret;
    ...
    /* Just for debug */
    if ((sscanf_s(buf, "0x%x %u %u %u", &cpu_address, &length, &times, &burst_size) != 4)) { /* 输入4个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_performance_cpu(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                  oal_pcie_linux_res *pst_pcie_res)
{
    int32_t ret;
    uint32_t length, times, burst_size;
    ...
    /* Just for debug */
    if ((sscanf_s(buf, "%u %u %u", &length, &times, &burst_size) != 3)) { /* 输入3个参数 */
        return -OAL_EINVAL;
    }
                                                           ...
                                                           const char *buf, oal_pcie_linux_res *pst_pcie_res)
{
    ...
    /* Just for debug */
    if ((sscanf_s(buf, "%u %d", &test_time, &alloc_count) != 2)) { /* 输入2个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_debug_performance_write(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                                    oal_pcie_linux_res *pst_pcie_res)
{
    /* echo readmem address length(hex) > debug */
    int32_t ret;
    ...
    /* Just for debug */
    if ((sscanf_s(buf, "0x%x %u %u %u 0x%x", &cpu_address,
        &length, &times, &burst_size, &value) != 5)) { /* 输入5个参数 */
        return -OAL_EINVAL;
    }
...
OAL_STATIC int32_t oal_pcie_loadfile(oal_pcie_linux_res *pst_pcie_res, char *file_name, uint32_t cpu_address,
                                     int32_t performance)
{
    ...
    if (total_len) {
        if (performance)
            ...
            pci_print_log(PCI_LOG_INFO, "loadfile cpu:0x%8x len:%u loadpath:%s done", cpu_address, total_len,
                          file_name);
    } else {
...
OAL_STATIC int32_t oal_pcie_debug_loadfile(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                           oal_pcie_linux_res *pst_pcie_res)
{
    int32_t ret;
    char file_name[PCIE_DEBUG_MSG_LEN];
    ...
    if ((sscanf_s(buf, "0x%x %s", &cpu_address, file_name, sizeof(file_name)) != 2)) { /* 输入2个参数 */
        pci_print_log(PCI_LOG_ERR,
                      "loadfile argument invalid,[%s], should be [echo writemem address  filename > debug]", buf);
        return -OAL_EINVAL;
    }
...
typedef struct _pcie_sysfs_debug_info_ {
    char *name;
    char *usage;
    int32_t (*debug)(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                     oal_pcie_linux_res *pst_pcie_res);
} pcie_sysfs_debug_info;

OAL_STATIC int32_t oal_pcie_print_debug_info(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                             oal_pcie_linux_res *pst_pcie_res)
{
...
OAL_STATIC pcie_sysfs_debug_info g_pci_debug[] = {
    ...
    { "performance_read", "address(hex) length(decimal) times(decimal) burst_size(decimal)",
      oal_pcie_debug_performance_read },
    { "performance_cpu", "length(decimal) times(decimal) burst_size(decimal)", oal_pcie_debug_performance_cpu },
    { "performance_write", "address(hex) length(decimal) times(decimal) burst_size(decimal) value(hex) ",
      oal_pcie_debug_performance_write },
    { "performance_netbuf_alloc", "test_time(decimal msec)", oal_pcie_debug_performance_netbuf_alloc },
    { "performance_netbuf_queue", "test_time(decimal msec) alloc_count(decimal)",
      oal_pcie_debug_performance_netbuf_queue },
    ...
    { "sleep_test", "subsys(string)", oal_pcie_pm_sleep_test},
    { "access_test", "subsys(string)", oal_pcie_pm_access_test},
};
...
OAL_STATIC ssize_t oal_pcie_set_debug_info(struct kobject *dev, struct kobj_attribute *attr, const char *buf,
                                           size_t count)
{
    int32_t i;
    oal_pcie_linux_res *pst_pcie_res = oal_get_default_pcie_handler();
    ...
    for (i = 0; i < oal_array_size(g_pci_debug); i++) {
        if ((g_pci_debug[i].name != NULL) && (count >= OAL_STRLEN(g_pci_debug[i].name)) &&
...
OAL_STATIC struct attribute *g_hpci_sysfs_entries[] = { &g_dev_attr_debug.attr, NULL };
