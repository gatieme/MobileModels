...
int32_t oal_pcie_memport_copy_test(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "memcopy: %llu Mbps, trans_time:%llu us, tran_size:%lu", trans_size, us_to_s,
                       total_size);
...
hcc_bus_dev *oal_pcie_get_one_alive_bus_dev(void)
{
    oal_pcie_linux_res *wifi_pcie_res = NULL;
    oal_pcie_linux_res *gt_pcie_res = oal_get_pcie_handler(HCC_EP_GT_DEV);

    if (gt_pcie_res != NULL) {
        if (gt_pcie_res->ep_res.link_state >= PCI_WLAN_LINK_MEM_UP) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "return slb hcc for default handle");
            return hcc_get_bus_dev(HCC_EP_GT_DEV);
        }
    }
    wifi_pcie_res = oal_get_pcie_handler(HCC_EP_WIFI_DEV);
    if (wifi_pcie_res != NULL) {
        if (wifi_pcie_res->ep_res.link_state >= PCI_WLAN_LINK_MEM_UP) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "return wifi hcc for default handle");
            return hcc_get_bus_dev(HCC_EP_WIFI_DEV);
        }
    }
    oal_print_mpxx_log(MPXX_LOG_ERR, "none hcc alive for default handle");
    return NULL;
}
...
oal_bool_enum_uint8 oal_pcie_link_state_up(uint8_t dev_id)
{
    ...
    return pci_lres->ep_res.link_state == PCI_WLAN_LINK_WORK_UP;
}
...
int32_t oal_pcie_device_check_alive(oal_pcie_linux_res *pst_pci_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pci_res, pst_pci_res->ep_res.chip_info.addr_info.glb_ctrl, &addr_map);
...
int32_t oal_pcie_check_link_state(oal_pcie_linux_res *pst_pci_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pci_res, pst_pci_res->ep_res.chip_info.addr_info.sharemem_addr,
                                    &addr_map);
    if (oal_unlikely(ret != OAL_SUCC)) {
        /* share mem 地址未映射! */
                                         ...
                                         uint32_t data_size)
{
    ...
    if (oal_unlikely(((uintptr_t)ddr_address & 0x3) || (data_size & 0x3))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ddr address 0x%llx, length %u is invalid", (uint64_t)(uintptr_t)ddr_address,
                           length);
        return -OAL_EINVAL;
    }
                                           ...
                                           uint32_t data_size)
{
    ...
    if (oal_unlikely(((uintptr_t)ddr_address & 0x3) || (data_size & 0x3))) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "ddr address 0x%llx, length %lu is invalid", (uint64_t)(uintptr_t)ddr_address,
                           length);
        return -OAL_EINVAL;
    }
...
int32_t oal_pcie_device_phy_read_reg(oal_pcie_linux_res *pst_pci_res, uint32_t pcie_phy_pa, uint32_t pos, uint32_t bits,
                                     uint32_t *val)
{
    int32_t ret;
    uint32_t reg;
...
OAL_STATIC int32_t oal_pcie_ctrl_base_address_init(oal_pcie_linux_res *pcie_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->chip_info.addr_info.pcie_ctrl, &st_map);
    if (ret != OAL_SUCC) {
    ...
    if (ep_res->chip_info.edma_support == OAL_TRUE) {
        ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->chip_info.addr_info.edma_ctrl, &st_map);
        if (ret != OAL_SUCC) {
    ...
    /* dbi base */
    ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->chip_info.addr_info.dbi, &st_map);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "get dev address 0x%x failed", ep_res->chip_info.addr_info.dbi);
        oal_pcie_regions_info_dump(pcie_res->comm_res);
        oal_pcie_ctrl_base_address_exit(ep_res);
        return -OAL_EFAIL;
    }
    ...
    if (ep_res->chip_info.ete_support == OAL_TRUE) {
        /* ete base */
        ret = oal_pcie_inbound_ca_to_va(pcie_res, ep_res->chip_info.addr_info.ete_ctrl, &st_map);
        if (ret != OAL_SUCC) {
...
int32_t oal_pcie_h2d_doorbell(oal_pcie_linux_res *pst_pci_res)
{
    ...
    if (oal_unlikely(pst_pci_res->ep_res.link_state <= PCI_WLAN_LINK_DOWN)) {
...
int32_t oal_pcie_ringbuf_h2d_refresh(oal_pcie_linux_res *pst_pci_res)
{
    ...
    for (i = 0; i < PCIE_H2D_QTYPE_BUTT; i++) {
        ...
        ret = oal_pcie_inbound_ca_to_va(pst_pci_res, pst_pci_res->ep_res.st_ringbuf.st_h2d_buf[i].base_addr,
                                        &st_map);
        if (ret != OAL_SUCC) {
...
int32_t oal_pcie_host_init(oal_pcie_linux_res *pcie_res)
{
    ...
    mutex_init(&pcie_res->ep_res.st_rx_cnt_res_lock);
...
void oal_pcie_host_exit(oal_pcie_linux_res *pcie_res)
{
    ...
    mutex_destroy(&pcie_res->ep_res.st_rx_cnt_res_lock);
    oal_pcie_sysfs_exit(pcie_res);
}
#endif
