...
#define WAKEUP_IN_PROBE_COMPLETION_TIME 10
...
static int32_t oal_usb_power_ctrl(hcc_bus *hi_bus, hcc_bus_power_ctrl_type ctrl, int (*func)(void *data), void *data);
...
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "type[%u] chan[%u] dev_addr[0x%p]", type, chan, usb_dev);
...
static int32_t oal_usb_power_action(hcc_bus *bus, hcc_bus_power_action_type action)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "action type [%d]", action);
...
static int32_t oal_usb_power_ctrl(hcc_bus *hi_bus, hcc_bus_power_ctrl_type ctrl, int (*func)(void *data), void *data)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "usb not define power ctrl[%d]", ctrl);
    return OAL_SUCC;
}
...
static hcc_bus *oal_usb_bus_init(struct oal_usb_dev_container *usb_dev_container)
{
    ...
    bus = hcc_alloc_bus();
    if (bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "alloc usb hcc bus failed, size:%u", (uint32_t)sizeof(hcc_bus));
        return NULL;
    }
    ...
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "add usb bus failed, ret=%d", ret);
        hcc_free_bus(bus);
        return NULL;
    }
...
static void oal_usb_probe_complete(void)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "probe complete");
...
static int32_t oal_usb_trigger_probe(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "start to register usb module");

    /* wakeup in probe */
    if (wait_for_completion_timeout(&g_usb_probe_complete, WAKEUP_IN_PROBE_COMPLETION_TIME * HZ) == 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "usb enum timeout");
        return -OAL_EFAIL;
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "hisi usb load sucuess, usb enum done.");
    return OAL_SUCC;
}
