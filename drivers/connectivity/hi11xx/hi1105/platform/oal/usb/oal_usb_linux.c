                                       ...
                                       oal_completion *wait)
{
    ...
    if (ret < 0) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "submit tx urb failed [%d]", ret);
        goto error_unanchor;
    }
    if (timeout == 0) {
        return OAL_SUCC;
    ...
    if (ret == 0) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "wait for urb complete failed");
        return -OAL_EIO;
    }
    return OAL_SUCC;
error_unanchor:
...
static struct urb *oal_usb_parpare_bulk_urb(struct oal_usb_xfer_param *param, uint32_t is_in)
{
    ...
    if (urb == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "alloc urb failed");
        return NULL;
    }
    if (is_in) {
...
int32_t oal_usb_bulk_tx(struct oal_usb_xfer_param *param)
{
    ...
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "usb_dev is null");
        return -OAL_ENODEV;
    }
    ...
    /* usb_kill_urb 操作需要保证urb不能被释放 */
    usb_get_urb(urb);
    ret = oal_usb_tx_common(usb_dev, urb, param->timeout);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_WARN, "tx fail");
        usb_kill_urb(urb);
    }
    usb_put_urb(urb);
    return ret;
...
int32_t oal_usb_bulk_rx(struct oal_usb_xfer_param *param)
{
    ...
    if (usb_dev == NULL) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "usb_dev is null");
        return -OAL_ENODEV;
    }

    urb = oal_usb_parpare_urb(param, OAL_USB_INTERFACE_BULK, OAL_USB_IN_DIR);
    if (urb == NULL) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "alloc urb failed");
        return -OAL_EFAIL;
    }
    ...
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_WARN, "rx fail");
        usb_kill_urb(urb);
    }
    usb_put_urb(urb);
    return ret;
...
int32_t oal_usb_intr_tx(struct oal_usb_xfer_param *param)
{
    ...
    ret = oal_usb_tx_common(usb_dev, urb, param->timeout);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "tx fail");
        usb_kill_urb(urb);
    }
    usb_put_urb(urb);
    return ret;
...
int32_t oal_usb_intr_rx(struct oal_usb_xfer_param *param)
{
    ...
    ret = oal_usb_rx_common(usb_dev, urb, param->timeout);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "rx fail");
        usb_kill_urb(urb);
    }
    usb_put_urb(urb);
    return ret;
...
void oal_usb_rx_complete(struct urb *urb, struct oal_usb_xfer_param *param)
{
    ...
    if (urb->status) {
        oal_print_mpxx_log(MPXX_LOG_DBG, "urb->status[%d]", urb->status);
        goto exit;
    }
...
void oal_usb_tx_complete(struct urb *urb, struct oal_usb_xfer_param *param)
{
    ...
    if (urb->status) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "urb->status[%d]", urb->status);
        goto exit;
    }
