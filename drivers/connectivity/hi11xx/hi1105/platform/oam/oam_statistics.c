 ...
 *
 * Description:oam_statistics functional module
...
void oam_stats_report_irq_info_to_sdt(uint8_t *puc_irq_info_addr, uint16_t us_irq_info_len)
{
    ...
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        ...
        us_irq_info_len = SDT_NETBUF_MAX_PAYLOAD - sizeof(oam_ota_hdr_stru);
    }
    ...
    pst_ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_MP13_HOST;
                                                  ...
                                                  uint16_t us_event_queue_info_len)
{
    ...
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        us_event_queue_info_len = SDT_NETBUF_MAX_PAYLOAD - sizeof(oam_ota_hdr_stru);
    }
