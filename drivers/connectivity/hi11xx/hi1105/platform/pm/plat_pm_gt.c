 ...
 *
 * Description:pm gt functional module
 * Author: @CompanyNameTag
 */

#define MPXX_LOG_MODULE_NAME     "[GT_PM]"
#define MPXX_LOG_MODULE_NAME_VAR gt_pm_loglevel
...
OAL_STATIC uint32_t gt_pm_sleep_request(struct gt_pm_s *gt_pm)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "sleep request send!");
...
int32_t gt_pm_allow_sleep_callback(void *data)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "gt_pm_allow_sleep_callback");
...
static int gt_pm_submit_sleep_work(struct gt_pm_s *pm_data)
{
    if (pm_data->packet_cnt == 0) {
        ...
        oal_print_mpxx_log(MPXX_LOG_DBG, "plat:gt_pm_wdg_timeout %d have packet %d....",
...
#endif
{
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "gt_pm_wdg_timeout....%d", pm_data->wdg_timeout_curr_cnt);
    ...
    if (time_after(jiffies, pm_data->packet_check_time)) {
        ...
        oal_print_mpxx_log(MPXX_LOG_DBG, "pkt_num:GT[%d]", pm_data->packet_total_cnt);
#else
        oal_print_mpxx_log(MPXX_LOG_WARN, "pkt_num:GT[%d]", pm_data->packet_total_cnt);
#endif
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "gt_pm_feed_wdg");
    gt_pm_feed_wdg();

    return;
}

static void sleep_request_host_forbid_print(struct gt_pm_s *gt_pm,
...
static int32_t gt_pm_sleep_cmd_send(struct gt_pm_s *gt_pm)
{
    int32_t ret;
    uint8_t retry;
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt sleep cmd send, pkt_num:[%d]", gt_pm->packet_total_cnt);
...
static void trigger_bus_execp_check(struct gt_pm_s *gt_pm)
{
...
OAL_STATIC uint32_t  gt_pm_sleep_cmd_proc(struct gt_pm_s *gt_pm)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "hold deepsleep_wakelock %d ms", GT_WAKELOCK_HOLD_TIME);
...
OAL_STATIC int32_t gt_pm_init_ext1(struct gt_pm_s *gt_pm)
{
    ...
    if (gt_pm->pst_pm_wq == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "Failed to create gt_pm_wq!");
        kfree(gt_pm);
        return OAL_FAIL;
    }
...
struct gt_pm_s *gt_pm_init(void)
{
    ...
    if (gt_pm == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "no mem to allocate gt_pm_data");
        return NULL;
    }
    ...
    if (gt_pm->pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "hcc bus is NULL, failed to create gt_pm_wq!");
        kfree(gt_pm);
        return NULL;
    }
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_init ok!");
    return gt_pm;
}
...
oal_uint gt_pm_exit(void)
{
    ...
    g_gt_pm_info = NULL;
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_exit ok!");
...
OAL_STATIC int32_t gt_pm_custom_cali(struct gt_pm_s *gt_pm)
{
    ...
    if (g_custom_process_func.p_custom_cali_func_gt == NULL) {
    ...
    if (g_custom_process_func.p_custom_cali_func_gt() == OAL_SUCC) {
...
int32_t gt_pm_open(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_open enter");
...
OAL_STATIC uint32_t gt_pm_close_pre(struct gt_pm_s *gt_pm)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_close start!!");
    gt_pm->close_cnt++;
...
uint32_t gt_pm_close(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "gt_pm_close enter");
    mutex_lock(&(pm_top_data->host_mutex));
...
uint32_t gt_pm_close_by_shutdown(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_close_by_shutdown start!!");
...
OAL_STATIC oal_uint gt_pm_pcie_wakeup_cmd(struct gt_pm_s *gt_pm)
{
    ...
    oal_wlan_gpio_intr_enable(hbus_to_dev(gt_pm->pst_bus), OAL_TRUE);
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt wakeup cmd send");
...
oal_uint gt_pm_wakeup_dev(void)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "wakeup dev succ, cost %llu us", trans_us);
    return OAL_SUCC;
}
...
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "gt_pm_stop_wdg");
...
void gt_pm_dump_host_info(void)
{
    ...
    oal_io_print("gt_wkup_host_intr %llu\n", gt_pm_info->pst_bus->gpio_int_count);
...
void gt_device_mem_check_work(oal_work_stru *pst_worker)
{
    ...
    g_ram_reg_test_result = gt_device_mem_check(&g_ram_reg_test_time);
