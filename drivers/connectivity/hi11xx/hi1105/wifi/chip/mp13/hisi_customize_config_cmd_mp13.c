...
OAL_STATIC wlan_cfg_cmd g_wifi_config_cmds[] = {
    ...
    { "tcp_buf_switch",        WLAN_CFG_DEVICE_TX_TCP_BUF },
    ...
    /* 低功耗代理 */
    { "lowpower_agent_switch", WLAN_CFG_INIT_LOWPOWER_AGENT_SWITCH_MP13 },
    /* 可维可测 */
    ...
    { "fem_polar_policy_2g",         WLAN_CFG_INIT_FEM_POLAR_POLICY_2G},
    { "fem_polar_policy_5g",         WLAN_CFG_INIT_FEM_POLAR_POLICY_5G},
    { "dyn_bypass_2g_extlna_rssi_th_h", WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_HIGH_TH},
    { "dyn_bypass_2g_extlna_rssi_th_l", WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_LOW_TH},
    { "dyn_bypass_5g_extlna_rssi_th_h", WLAN_CFG_INIT_DYN_BYPASS_5G_EXTLNA_RSSI_HIGH_TH},
    { "dyn_bypass_5g_extlna_rssi_th_l", WLAN_CFG_INIT_DYN_BYPASS_5G_EXTLNA_RSSI_LOW_TH},
    { NULL,         0 }
};
...
OAL_STATIC void host_params_dyn_bypass_extlna_init(void)
{
    ...
    g_host_init_params[WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_HIGH_TH] = 0xD3D3;
    g_host_init_params[WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_LOW_TH] = 0xD0D0;
    g_host_init_params[WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_HIGH_TH] = 0xDDDDD3D3;
    g_host_init_params[WLAN_CFG_INIT_DYN_BYPASS_2G_EXTLNA_RSSI_LOW_TH] = 0xDADAD0D0;
}
/*
...
OAL_STATIC void host_params_performance_init(void)
{
    ...
    g_host_init_params[WLAN_CFG_DEVICE_TX_TCP_BUF] = OAL_FALSE;
    ...
    host_params_dyn_bypass_extlna_init();
}
...
void host_params_init_first(void)
{
    ...
    /* 低功耗代理 */
    g_host_init_params[WLAN_CFG_INIT_LOWPOWER_AGENT_SWITCH_MP13] = OAL_FALSE;
 ...
 */
OAL_STATIC int32_t hwifi_custom_adapt_device_ini_freq_param(uint8_t *data, uint32_t *data_len)
{
    config_device_freq_h2d_stru device_data = {0};
    uint8_t uc_index;
    int32_t l_val, l_ret;
    uint32_t cfg_id;
    hmac_to_dmac_cfg_custom_data_stru syn_msg = {0};
    const uint32_t dev_data_buff_size = DEV_WORK_FREQ_LVL_NUM;

    if (data == NULL) {
        oam_error_log1(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_ini_freq_param data is NULL len[%d].}", *data_len);
        return INI_FAILED;
    }

    syn_msg.en_syn_id = CUSTOM_CFGID_INI_FREQ_ID;
    ...
    for (uc_index = 0, cfg_id = WLAN_CFG_INIT_PSS_THRESHOLD_LEVEL_0; uc_index < dev_data_buff_size; uc_index++) {
        ...
        if (PPS_VALUE_0 <= l_val && l_val <= PPS_VALUE_3) {
            device_data.st_device_data[uc_index].speed_level = (uint32_t)l_val;
            cfg_id += WLAN_CFG_ID_OFFSET;
        } else {
            oam_error_log1(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_ini_freq_param get wrong PSS TH[%d]!}", l_val);
            return OAL_FALSE;
        }
    }
    ...
    for (uc_index = 0, cfg_id = WLAN_CFG_INIT_DEVICE_TYPE_LEVEL_0; uc_index < dev_data_buff_size; uc_index++) {
        ...
        if (l_val >= FREQ_IDLE && l_val <= FREQ_HIGHEST) {
            device_data.st_device_data[uc_index].cpu_freq_level = (uint32_t)l_val;
            cfg_id++;
        } else {
            oam_error_log1(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_ini_freq_param get wrong DEV TYPE[%d]!}", l_val);
            return OAL_FALSE;
        }
    }
    device_data.uc_set_type = FREQ_SYNC_DATA;

    syn_msg.len = sizeof(device_data);

    l_ret = memcpy_s(data, (WLAN_LARGE_NETBUF_SIZE - *data_len), &syn_msg, CUSTOM_MSG_DATA_HDR_LEN);
    l_ret += memcpy_s(data + CUSTOM_MSG_DATA_HDR_LEN, (WLAN_LARGE_NETBUF_SIZE - *data_len - CUSTOM_MSG_DATA_HDR_LEN),
    ...
    if (l_ret != EOK) {
        oam_error_log2(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_ini_freq_param::memcpy_s fail[%d]. data_len[%d]}",
                       l_ret, *data_len);
        *data_len += (syn_msg.len + CUSTOM_MSG_DATA_HDR_LEN);
        return OAL_FAIL;
    }

    *data_len += (syn_msg.len + CUSTOM_MSG_DATA_HDR_LEN);
    oam_warning_log1(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_ini_freq_param da_len[%d].}", *data_len);
 ...
 */
OAL_STATIC void hwifi_custom_adapt_device_ini_perf_param(uint8_t *puc_data, uint32_t *data_len)
{
    ...
    if (puc_data == NULL) {
        oam_error_log1(0, OAM_SF_CFG, "{adapt_device_ini_perf_param puc_data is NULL last data_len[%d].}", *data_len);
        return;
    }
    ...
    l_ret = memcpy_s(puc_data, (WLAN_LARGE_NETBUF_SIZE - *data_len), &st_syn_msg, CUSTOM_MSG_DATA_HDR_LEN);
    l_ret += memcpy_s(puc_data + CUSTOM_MSG_DATA_HDR_LEN,
        (WLAN_LARGE_NETBUF_SIZE - *data_len - CUSTOM_MSG_DATA_HDR_LEN), &st_device_perf, sizeof(st_device_perf));
    if (l_ret != EOK) {
        *data_len += (sizeof(st_device_perf) + CUSTOM_MSG_DATA_HDR_LEN);
        return;
    }
...
OAL_STATIC void hwifi_custom_adapt_device_ini_pm_switch_param(uint8_t *puc_data, uint32_t *pul_data_len)
{
    ...
    *pul_data_len += sizeof(st_syn_msg);
    ...
    oam_warning_log2(0, OAM_SF_CFG,
                     "{hwifi_custom_adapt_device_ini_pm_switch_param::da_len[%d].device[%d]}",
                     *pul_data_len, g_wlan_device_pm_switch);
}

/*
 * 函 数 名  : hwifi_custom_adapt_device_ini_lp_agent_switch_param
 * 功能描述  : 低功耗代理定制化
 */
OAL_STATIC void hwifi_custom_adapt_device_ini_lp_agent_switch_param(uint8_t *data, uint32_t *data_len)
{
    hmac_to_dmac_cfg_custom_data_stru syn_msg = {0};
    int32_t ret;
    uint8_t lp_agent_switch;

    if (data == NULL) {
        oam_error_log1(0, OAM_SF_CFG,
            "{hwifi_custom_adapt_device_ini_lp_agent_switch_param::data is NULL data_len[%d].}", *data_len);
        return;
    }

    syn_msg.en_syn_id = CUSTOM_CFGID_INI_LP_AGENT_SWITCH_ID;

    lp_agent_switch = (uint8_t)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_LOWPOWER_AGENT_SWITCH_MP13);

    syn_msg.auc_msg_body[0] = lp_agent_switch;
    ...
    if (ret != EOK) {
        oam_error_log2(0, OAM_SF_CFG,
            "{hwifi_custom_adapt_device_ini_lp_agent_switch_param::memcpy_s fail[%d]. data_len[%d]}",
            ret, *data_len);
    ...
    *data_len += sizeof(syn_msg);
    ...
    oam_warning_log2(0, OAM_SF_CFG,
                     "{hwifi_custom_adapt_device_ini_lp_agent_switch_param::da_len[%d].device[%d]lp agent switch}",
                     *data_len, lp_agent_switch);
}
...
int32_t hwifi_custom_adapt_device_ini_param(uint8_t *puc_data)
{
    ...
    /* 低功耗代理 */
    hwifi_custom_adapt_device_ini_lp_agent_switch_param(puc_data + data_len, &data_len);
