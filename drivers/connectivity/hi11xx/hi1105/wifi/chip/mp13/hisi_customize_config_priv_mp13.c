...
OAL_STATIC wlan_cfg_cmd g_wifi_config_priv[] = {
    ...
    { "chba_dbac_chan_follow_switch", WLAN_CFG_PRIV_CHBA_DBAC_CHAN_FOLLOW_SWITCH }, /* chba信道跟随开关 */
    { "fem_mode_5g",                  WLAN_CFG_PRIV_5G_FEM_MODE },
    { NULL,                 0 }
};
...
OAL_STATIC void hwifi_custom_adapt_priv_ini_param_extend_etc(
    hmac_to_dmac_cfg_custom_data_stru *pst_syn_msg, uint8_t uc_cfg_id)
{
    switch (uc_cfg_id) {
        ...
        case WLAN_CFG_PRIV_CLOSE_FILTER_SWITCH:
            ...
            break;
        case WLAN_CFG_PRIV_CHBA_DBAC_CHAN_FOLLOW_SWITCH:
            pst_syn_msg->en_syn_id = CUSTOM_CFGID_PRIV_CHBA_DBAC_CHAN_FOLLOW_SWITCH_ID;
            break;
        default:
            break;
    }
...
OAL_STATIC void hwifi_custom_adapt_priv_feature_ini_param(hmac_to_dmac_cfg_custom_data_stru *pst_syn_msg,
                                                          uint8_t uc_cfg_id, uint8_t *puc_priv_cfg_value)
{
    switch (uc_cfg_id) {
...
/*
 * 函 数 名  : hwifi_custom_adapt_device_priv_ini_hal_ps_pps_param
...
OAL_STATIC void hwifi_custom_adapt_device_priv_ini_param_extend(uint8_t *puc_data, uint32_t data_len, uint32_t *pul_len)
{
    ...
    hwifi_custom_adapt_priv_ini_param(WLAN_CFG_PRIV_CHBA_DBAC_CHAN_FOLLOW_SWITCH, puc_data + data_len, &data_len);
    ...
    uint32_t *pul_data_len)
{
    ...
    /* 为了私有定制化值 */
    hwifi_get_init_priv_value(WLAN_CFG_PRIV_FEM_DELT_POW, &l_priv_fem_ctl_value);
    hwifi_get_init_priv_value(WLAN_CFG_PRIV_FEM_ADJ_TPC_TBL_START_IDX, &l_priv_fem_adj_value);
    hwifi_get_init_priv_value(WLAN_CFG_PRIV_5G_FEM_MODE, &fem_mode_5g);
    ...
    st_fem_pow_ctl.fem_mode_5g = fem_mode_5g;
    st_fem_pow_ctl.uc_ext_pa_isexist[WLAN_RF_CHANNEL_ZERO] =
        (uint8_t) !!cus_get_low_16bits(hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EXT_PA_ISEXIST_2G));
    st_fem_pow_ctl.uc_ext_pa_isexist[WLAN_RF_CHANNEL_ONE] =
