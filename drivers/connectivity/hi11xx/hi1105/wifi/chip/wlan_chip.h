...
struct wlan_chip_ops {
    ...
    /* ASPM开关 */
    void (*trx_aspm_switch_on)(uint32_t aspm);

#ifdef _PRE_WLAN_FEATURE_TCP_ACK_BUFFER
    /* tcp ack缓存命令，06直接赋值，mp13 mp15参数传递到device */
    /* 返回值：OAL_SUCC:传参成功;OAL_FAIL:传参失败 */
    uint32_t (*tcp_ack_buff_config)(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
#endif

#ifdef _PRE_WLAN_FEATURE_CSI
    /* set_csi设置命令，06直接host设置，mp13 mp15参数传递到device */
    /* 返回值：OAL_SUCC:传参成功;OAL_FAIL:传参失败 */
    ...
    /* 获取定制化文件特殊dbdc cap */
    uint8_t (*get_special_dbdc_cap)(void);
    void (*get_rf_band_from_center_freq)(uint16_t center_freq, uint8_t *rf_band_sel);
    uint16_t (*get_center_freq_from_chn)(const mac_channel_stru *channel_info);
    oal_bool_enum_uint8 (*is_support_dual_5g_dbdc_by_radio_cap)(void);
    ...
    /* zero wait dfs流程，目前，1106支持，mp13/mp15不支持 */
    ...
    oal_bool_enum (*is_single_dbdc_ini_en)(void);
...
#endif
    uint8_t (*soft_irq_sch_support)(void);
    uint8_t (*get_pcie_probe_state)(mac_user_stru *mac_user);
#ifdef _PRE_WLAN_CHBA_MGMT
    /* 获取硬件是否支持chba dbdc+dbac共存 */
    oal_bool_enum_uint8 (*is_support_chba_dbdc_dbac)(void);
#endif
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
    void (*lp_miracast_stat_func)(uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps);
    uint32_t (*set_lp_miracast_func)(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
#endif
     void (*need_decrease_sts)(mac_vap_stru *pst_mac_vap, uint8_t *puc_buffer, hmac_scanned_bss_info *scaned_bss);
};

extern const struct wlan_chip_ops *g_wlan_chip_ops;
extern const struct wlan_chip_ops g_wlan_chip_ops_mp13;
extern const struct wlan_chip_ops g_wlan_chip_ops_mp15;

extern const struct wlan_chip_ops g_wlan_chip_ops_mp16;
extern const struct wlan_chip_ops g_wlan_chip_ops_mp16c;
extern const struct wlan_chip_ops g_wlan_chip_ops_gf61;

extern const oal_wlan_cfg_stru g_wlan_spec_cfg_mp13;
extern const oal_wlan_cfg_stru g_wlan_spec_cfg_mp15;

extern const oal_wlan_cfg_stru g_wlan_spec_cfg_mp16;
extern const oal_wlan_cfg_stru g_wlan_spec_cfg_mp16c;
extern const oal_wlan_cfg_stru g_wlan_spec_cfg_gf61;
...
void wlan_chip_get_rf_band_from_center_freq_mp16(uint16_t center_freq,
    uint8_t *rf_band_sel);
...
OAL_STATIC OAL_INLINE void wlan_chip_need_decrease_sts(mac_vap_stru *pst_mac_vap, uint8_t *puc_buffer,
    hmac_scanned_bss_info *scaned_bss)
{
    if (g_wlan_chip_ops->need_decrease_sts != NULL) {
        g_wlan_chip_ops->need_decrease_sts(pst_mac_vap, puc_buffer, scaned_bss);
    }
    return;
}
