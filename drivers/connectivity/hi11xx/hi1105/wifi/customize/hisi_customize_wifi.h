...
#ifndef HISI_CUSTOMIZE_WIFI_H
#define HISI_CUSTOMIZE_WIFI_H
...
#define CUST_READ_NVRAM_POW_MASK BIT(3)
...
#define CUS_MAX_TXPOWER_MAX       238 /* 最大发送功率的最大有效值:238 23.8dBm */

#define CUS_PARAMS_LEN_MAX    150  /* 定制项最大长度 */
...
#define WLAN_SDIO_D2H_ASSEMBLE_COUNT_VAL  32
...
#define WLAN_SMALL_AMSDU_PPS_HIGH         23000
...
#define WLAN_MIN_FAST_PS_IDLE             3
...
#define WLAN_HIEX_DEV_CAP 0x51EF1

#define RF_LINE_TXRX_GAIN_DB_2G_MIN (-100)
#define RF_LINE_TXRX_GAIN_DB_MAX     40
#define RF_LINE_TXRX_GAIN_DB_5G_MIN (-48)
#define FB_GAIN_DELT_POW_MAX 160
#define FB_GAIN_DELT_POW_MIN (-160)

#define RF_LP_LOSS_GAIN_DB_MAX     256
#define RF_LP_LOSS_GAIN_DB_MIN   (-256)
/* 定制化HOST全局变量结构体 */
typedef struct {
    /* ba tx 聚合数 */
...
/* mp13 对外定制化接口 */
uint8_t *hwifi_get_nvram_param(uint32_t nvram_param_idx); /* 当前只是mp13 使用，需要优化 */
