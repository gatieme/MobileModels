...
typedef struct {
    ...
    uint8_t ext_lna_failure; /* 外置lna检测异常状态标记 */
    uint8_t rsv[2]; // 2代表2个保留字节，需要4字节对齐
} mp15_bt20dbm_txcal_param_stru;
// end added of 20dbm cali

extern uint32_t g_wifi_fac_cali_chain;
...
uint32_t hmac_config_d2h_up_vmin(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param);
