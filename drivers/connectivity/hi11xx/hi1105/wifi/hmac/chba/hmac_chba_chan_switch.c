 ...
 * Description: chba mgmt implementation
 * Author:
 * Create: 2021-02-06
 */
...
static uint32_t hmac_chba_get_valid_target_channel_cfg(hmac_vap_stru *hmac_vap, mac_channel_stru *target_channel,
    uint8_t chan_number, wlan_channel_bandwidth_enum_uint8 en_bandwidth);

/*
 * 功能描述  : 设置Channel Switch属性
 * 日    期  : 2021年03月23日
 */
    ...
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf)
{
    ...
    hmac_chba_mgmt_cb_partial_init(&(hmac_vap->st_vap_base_info), peer_addr, frame_buf, frame_len, OAL_FALSE);
    ...
    uint8_t bandwidth, uint8_t switch_slot)
{
    ...
    /* 配置信道切换 */
    hmac_config_chba_channel(hmac_vap, &set_work_chan.channel, set_work_chan.switch_slot);
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_info_stru *chan_switch_info)
{
    ...
    for (idx = 0; idx < device_cnt; idx++) {
        ...
        oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_lost_device_check_and_save::lost_device_mac[XX:XX:XX:%2X:%2X:%2X]",
            chan_switch_info->csa_lost_device[chan_switch_info->csa_lost_device_num].mac_addr[MAC_ADDR_3],
            chan_switch_info->csa_lost_device[chan_switch_info->csa_lost_device_num].mac_addr[MAC_ADDR_4],
            chan_switch_info->csa_lost_device[chan_switch_info->csa_lost_device_num].mac_addr[MAC_ADDR_5]);
...
/*
 * 功能描述  : 岛主发送信道切换指示后的收集ack的定时器超时处理
 * 日    期  : 2021年03月23日
 */
    ...
    mac_channel_stru *target_channel)
{
    ...
    if (mac_regdomain_channel_is_support_bw(target_channel->en_bandwidth,
        target_channel->uc_chan_number, hmac_vap->st_vap_base_info.st_channel.ext6g_band) == OAL_FALSE) {
...
/*
 * 功能描述  : 因CHBA切信道导致DBAC上层下发切信道开始信道切换流程，只需要切换自身信道
 * 日    期  : 2021年03月23日
 */
...
/*
 * 功能描述  : 非岛主节点给岛主发送信道切换请求后等待指示/拒绝帧的超时处理
 * 日    期  : 2021年03月23日
 */
...
/*
 * 功能描述  : 非岛主节点检查工作信道与岛主信道是否一致并切换信道
 * 日    期  : 2021年03月23日
 */
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    ...
    /* 信道号和带宽参数组合不支持 */
    if (mac_regdomain_channel_is_support_bw(chan_switch_ntf->bandwidth, chan_switch_ntf->chan_number,
        OAL_FALSE) == OAL_FALSE) {
        oam_error_log2(0, OAM_SF_CHBA, "hmac_chba_rx_chan_switch_indication::channel number=[%d], bw=[%d] invalid",
            chan_switch_ntf->chan_number, chan_switch_ntf->bandwidth);
        return;
    }
    if (hmac_chba_get_valid_target_channel_cfg(hmac_vap, &target_channel, chan_switch_ntf->chan_number,
        chan_switch_ntf->bandwidth) != OAL_SUCC) {
        return;
    }
    /* 已经在新信道上了，不回confirm */
    ...
    uint8_t chan_number, wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    uint32_t up_vap_num;
    mac_device_stru *mac_device = NULL;
    mac_vap_stru *up_mac_vap[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {};

    /* 获取目标信道配置 */
    target_channel->uc_chan_number = chan_number;
    target_channel->en_bandwidth = en_bandwidth;
    target_channel->en_band = mac_get_band_by_channel_num(target_channel->uc_chan_number);
    ret = mac_get_channel_idx_from_num(target_channel->en_band, target_channel->uc_chan_number,
        OAL_FALSE, &(target_channel->uc_chan_idx));
    if (ret != OAL_SUCC) {
        oam_error_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_chba_get_valid_target_channel_cfg:invalid chan_cfg, chan_num[%d], bw[%d]}",
            target_channel->uc_chan_number, target_channel->en_bandwidth);
        return OAL_FAIL;
    }
    ...
    if (mac_device == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_chba_get_valid_target_channel_cfg::fail to get mac_device}");
        return OAL_FAIL;
    }
    ...
    if (up_vap_num == 0) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_chba_get_valid_target_channel_cfg:mac_device_find_up_vaps fail.}");
        return OAL_FAIL;
    }

    /* 识别原状态和目标状态，避免去dbac和非法状态 */
    if (hmac_chan_switch_get_state_trans(up_mac_vap, &hmac_vap->st_vap_base_info,
        target_channel, up_vap_num, MAC_MVAP_CHAN_SWITCH_CHBA_CSA) == MAC_MVAP_STATE_TRANS_UNSUPPORTED) {
        oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_chba_get_valid_target_channel_cfg:target_channel chann_num[%d] bw[%d] is unspport!}",
            target_channel->uc_chan_number, target_channel->en_bandwidth);
        return OAL_FAIL;
    }
    return OAL_SUCC;
...
/*
 * 功能描述  : 收到CHBA信道调整命令(Service --> Hmac --> CHBA)
 * 日    期  : 2021年05月13日
 * 作    者  : wifi
 */
void hmac_chba_adjust_channel_proc(hmac_vap_stru *hmac_vap, mac_chba_adjust_chan_info *chan_info)
{
    ...
    /* 不支持的信道配置, 不处理 */
    if (hmac_chba_get_valid_target_channel_cfg(hmac_vap, &target_channel, chan_info->chan_number,
        chan_info->bandwidth) != OAL_SUCC) {
        return;
    }
    /* 进信道切换流程前停掉扫描 */
    ...
    mac_channel_stru *social_channel)
{
    ...
    if (band == WLAN_BAND_2G) {
        oam_error_log1(0, 0, "hmac_config_chba_social_channel_params::social chan[%d]", channel_number);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

    ret = mac_get_channel_idx_from_num(band, channel_number, OAL_FALSE, &chan_idx);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, 0, "hmac_config_chba_social_channel_params::config social channel err[%u]", ret);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    ...
    social_channel->uc_chan_idx = chan_idx;
    ...
    mac_channel_stru *social_channel)
{
    ...
    /* 根据定制化使用固定的social channel配置 */
    if (g_chba_social_chan_cap) {
        if (g_chba_init_social_channel == 0) {
            ret = hmac_config_chba_social_channel_params(CHBA_INIT_SOCIAL_CHANNEL,
                WLAN_BAND_WIDTH_80PLUSPLUS, social_channel);
        } else {
            ret = hmac_config_chba_social_channel_params(g_chba_init_social_channel, bandwidth, social_channel);
            if (ret != OAL_SUCC) { /* 定制化指定social channel是非法配置 */
                return hmac_config_chba_social_channel_params(CHBA_INIT_SOCIAL_CHANNEL,
                    WLAN_BAND_WIDTH_80PLUSPLUS, social_channel);
            }
        }
        return ret;
    ...
    /* social channel跟随work channel, 与work channel保持一致 */
    return hmac_config_chba_social_channel_params(channel_number, bandwidth, social_channel);
}
 ...
 * 作    者  : wifi7
 */
static void hmac_config_chba_work_channel(hmac_vap_stru *hmac_vap, hmac_chba_vap_stru *chba_vap_info,
    mac_channel_stru *mac_channel, uint8_t switch_slot)
{
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_config_chba_work_channel::config vap work channel err[%d]}", ret);
        return;
    }
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
        "{hmac_config_chba_work_channel:cur chan_num %d, bw %d; switch to chan_num %d, bw %d}",
        mac_vap->st_channel.uc_chan_number, mac_vap->st_channel.en_bandwidth, mac_channel->uc_chan_number,
        mac_channel->en_bandwidth);
}

/*
 * 功能描述  : 配置CHBA social信道
 * 日    期  : 2022年08月31日
 * 作    者  : wifi7
 */
    ...
    mac_channel_stru *mac_channel, uint8_t switch_slot)
{
    ...
    /* 根据定制化获取合适的social channel配置 */
    ret = hmac_chba_generate_social_channel_cfg(mac_channel->uc_chan_number, mac_channel->en_bandwidth,
        &social_channel);
    if (ret != OAL_SUCC) {
        oam_error_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_config_chba_social_channel:fail to init social chan, chan_num[%d], bw[%d]}",
    ...
    /* chba social信道配置更新同步至dmac */
    channel_cfg.chan_cfg_type_bitmap = CHBA_CHANNEL_CFG_BITMAP_DISCOVER;
    channel_cfg.channel = social_channel;
    channel_cfg.chan_switch_slot_idx = switch_slot;
    ret = hmac_config_send_event(&hmac_vap->st_vap_base_info, WLAN_CFGID_CHBA_SET_CHANNEL,
        sizeof(chba_h2d_channel_cfg_stru), (uint8_t *)&channel_cfg);
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_config_chba_social_channel:config vap work channel err[%d]}", ret);
        return;
    }
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, 0,
        "{hmac_config_chba_social_channel:cur chan_num %d, bw %d; switch to chan_num %d, bw %d}",
        hmac_vap->st_vap_base_info.st_channel.uc_chan_number, hmac_vap->st_vap_base_info.st_channel.en_bandwidth,
        social_channel.uc_chan_number, social_channel.en_bandwidth);
}
...
uint32_t hmac_config_chba_channel(hmac_vap_stru *hmac_vap, mac_channel_stru *mac_channel, uint8_t switch_slot)
{
    ...
    if (oal_any_null_ptr2(hmac_vap, mac_channel)) {
        oam_error_log0(0, OAM_SF_CHBA, "{hmac_config_chba_channel:ptr is NULL!}");
        return OAL_FAIL;
    }
    ...
    if (hmac_chba_chan_check_is_valid(hmac_vap, mac_channel) == OAL_FALSE) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_config_chba_channel:invalid mac_channel!}");
        return OAL_FAIL;
    }
    ...
    if (chba_vap_info == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_config_chba_channel:failt ot get chba vap!}");
        return OAL_FAIL;
    }

    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, 0,
        "{hmac_config_chba_channel:chan_number %d, bw %d, switch_slot %d}",
        mac_channel->uc_chan_number, mac_channel->en_bandwidth, switch_slot);

    /* chba 更新信道后，需要及时下发校准数据，避免device切信道时无校准数据 */
    hmac_cali_send_work_channel_cali_data(&(hmac_vap->st_vap_base_info), mac_channel, CALI_DATA_CSA_TYPE);

    /* 更新social channel */
    hmac_config_chba_social_channel(hmac_vap, chba_vap_info, mac_channel, switch_slot);

    /* 更新work channel */
    hmac_config_chba_work_channel(hmac_vap, chba_vap_info, mac_channel, switch_slot);
    return OAL_SUCC;
}
#endif
