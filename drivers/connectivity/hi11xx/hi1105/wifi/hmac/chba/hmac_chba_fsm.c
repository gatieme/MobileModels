 ...
 */
static uint32_t hmac_chba_fsm_null_fn(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    /* 什么都不做 */
...
uint32_t hmac_fsm_call_func_chba_conn_initiator(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    hmac_fsm_input_type_enum input_event, void *param)
{
...
static void hmac_chba_set_tx_ctl(oal_netbuf_stru *netbuf, uint16_t mpdu_len, uint16_t tx_user_idx)
{
    ...
    tx_ctl->us_mpdu_payload_len = mpdu_len; /* dmac发送需要的mpdu长度 */
    tx_ctl->bit_tx_user_idx = tx_user_idx; /* 发送完成需要获取user结构体 */
}
...
static uint32_t  hmac_chba_initiate_assoc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    assoc_req.us_assoc_timeout = (uint16_t)
        hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_sta_config.dot11AssociationResponseTimeOut >> 1;
    ...
    hmac_asoc_rsp_stru *assoc_rsp)
{
    ...
    uint8_t *msg = (uint8_t *)assoc_rsp;

    if (assoc_rsp->en_result_code == HMAC_MGMT_SUCCESS) {
        hmac_handle_asoc_rsp_succ_sta(hmac_vap, msg, assoc_rsp);
    } else {
        ...
        if (hmac_user->chba_user.connect_info.assoc_cnt >= MAX_ASOC_CNT) {
            ...
            hmac_mgmt_send_deauth_frame(&hmac_vap->st_vap_base_info,
                hmac_user->st_user_base_info.auc_user_mac_addr, MAC_AUTH_NOT_VALID, OAL_FALSE);
 ...
 */
static uint32_t hmac_chba_mgmt_timeout(void *arg)
{
...
static void hmac_chba_start_mgmt_timer(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint32_t timeout)
{
    ...
    frw_timer_create_timer_m(&(hmac_vap->st_mgmt_timer), hmac_chba_mgmt_timeout,
...
static uint32_t hmac_chba_wait_auth(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    ...
    if (mac_mib_get_AuthenticationMode(&(hmac_vap->st_vap_base_info)) != WLAN_WITP_AUTH_SAE) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "{hmac_chba_wait_auth:: auth mode [%d] not SAE.}",
            mac_mib_get_AuthenticationMode(&(hmac_vap->st_vap_base_info)));
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
...
static uint32_t hmac_chba_is_auth_rsp_ok(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf,
...
#define MIN_AUTH_BODY_LEN 6
    ...
    mac_hdr = (mac_ieee80211_frame_stru *)mac_get_rx_cb_mac_header_addr(rx_ctrl);
    payload = mac_get_rx_payload(netbuf, &payload_len);
    if ((payload == NULL) || (mac_hdr == NULL) || (rx_ctrl->uc_mac_header_len < WLAN_MGMT_FRAME_HEADER_LEN) ||
        (payload_len < MIN_AUTH_BODY_LEN)) {
        return OAL_FAIL;
    }
    if (mac_get_frame_sub_type((uint8_t *)mac_hdr) != WLAN_FC0_SUBTYPE_AUTH) {
        return OAL_FAIL;
    }

    if (hmac_chba_whitelist_check(hmac_vap, mac_hdr->auc_address2, WLAN_MAC_ADDR_LEN) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    /* AUTH alg CHECK */
    auth_alg = mac_get_auth_alg(payload, payload_len);
...
static uint32_t hmac_chba_wait_auth_seq2_rx(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    ...
    if (hmac_chba_is_auth_rsp_ok(hmac_vap, netbuf, WLAN_AUTH_TRASACTION_NUM_ONE, WLAN_MIB_AUTH_ALG_SAE) != OAL_SUCC) {
        return OAL_FAIL;
    }

    status_code = mac_get_auth_status(payload, payload_len);
...
static void hmac_chba_print_auth_info(oal_netbuf_stru *netbuf)
{
    uint8_t *payload = NULL;
    uint16_t payload_len;
    ...
    oam_warning_log2(0, OAM_SF_CHBA, "hmac_chba_print_auth_info:status=%d, auth_seq=%d",
        mac_get_auth_status(payload, payload_len), mac_get_auth_seq_num(payload, payload_len));
}
...
static uint32_t hmac_chba_wait_auth_seq4_rx(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    ...
    if (hmac_chba_is_auth_rsp_ok(hmac_vap, netbuf, WLAN_AUTH_TRASACTION_NUM_TWO, WLAN_MIB_AUTH_ALG_SAE) != OAL_SUCC) {
        return OAL_FAIL;
    }

#ifdef _PRE_WLAN_FEATURE_SAE
    if (hmac_vap->duplicate_auth_seq4_flag == OAL_TRUE) { // wpa3 auth seq4重复帧过滤，禁止上报waps
        hmac_chba_print_auth_info(netbuf);
        return OAL_SUCC;
    }
#endif
    oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
...
static void hmac_chba_prepare_assoc_rsp(hmac_user_stru *hmac_user, hmac_asoc_rsp_stru *assoc_rsp,
    uint8_t *mac_hdr, uint8_t *payload, uint32_t payload_len)
{
    ...
    assoc_rsp->asoc_rsp_ie_len = payload_len - OAL_ASSOC_RSP_FIXED_OFFSET; /* 除去assoc rsp 固定部分6字节 */
    ...
    /* 获取关联请求帧信息 */
    assoc_rsp->puc_asoc_req_ie_buff = hmac_user->puc_assoc_req_ie_buff;
    assoc_rsp->asoc_req_ie_len = hmac_user->assoc_req_ie_len;
}
...
static uint32_t hmac_chba_wait_assoc_rx(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    ...
    rx_info = (mac_rx_ctl_stru *)(&(rx_ctrl->st_rx_info));
    mac_hdr = (mac_ieee80211_frame_stru *)mac_get_rx_cb_mac_header_addr(rx_info);
    payload = (uint8_t *)mac_hdr + rx_ctrl->st_rx_info.uc_mac_header_len;
    mac_hdr_len = rx_info->uc_mac_header_len;
    payload_len = hmac_get_frame_body_len(netbuf);
    if (hmac_chba_is_assoc_rsp_ok(hmac_vap, (uint8_t*)mac_hdr, mac_hdr_len, payload, payload_len) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    ret = hmac_process_assoc_rsp(hmac_vap, hmac_user, (uint8_t *)mac_hdr, mac_hdr_len, payload, payload_len);
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
    ...
    /* 准备消息，上报给APP */
    hmac_chba_prepare_assoc_rsp(hmac_user, &assoc_rsp, (uint8_t *)mac_hdr, payload, payload_len);
    hmac_chba_handle_assoc_rsp(hmac_vap, hmac_user, &assoc_rsp);

    /* CHBA关联成功，输出维测信息 */
    hmac_config_vap_info(&(hmac_vap->st_vap_base_info), sizeof(uint32_t), (uint8_t *)&ret);

    return OAL_SUCC;
}

static void hmac_chba_free_assoc_req_by_vap(hmac_user_stru *hmac_user)
{
...
static uint32_t hmac_chba_save_assoc_req_frame_by_vap(hmac_user_stru *hmac_user, uint8_t *assoc_req_frame, uint32_t len)
{
    hmac_user->assoc_req_ie_len = 0;
    hmac_user->puc_assoc_req_ie_buff = oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL, (uint16_t)len, OAL_TRUE);
    if (hmac_user->puc_assoc_req_ie_buff == NULL) {
    ...
    if (memcpy_s(hmac_user->puc_assoc_req_ie_buff, len, assoc_req_frame, len) != EOK) {
        oam_error_log0(0, OAM_SF_ASSOC, "{hmac_chba_wait_assoc::memcpy failed}");
    }
    hmac_user->assoc_req_ie_len = len;
    return OAL_SUCC;
}
...
static void hmac_chba_init_assoc_req_info(hmac_vap_stru *hmac_vap)
{
    ...
    /* 配置支持速率集 */
    support_rate_ie_len = mac_set_supported_rates_ie(&hmac_vap->st_vap_base_info, support_rate_ie,
                                                     WLAN_MAX_SUPP_RATES + MAC_IE_HDR_LEN);
    uc_rs_nrates = support_rate_ie[1];
    mac_mib_set_SupportRateSetNums(&hmac_vap->st_vap_base_info, uc_rs_nrates);
    if (memcpy_s(hmac_vap->auc_supp_rates, WLAN_MAX_SUPP_RATES,
        support_rate_ie + MAC_IE_HDR_LEN, uc_rs_nrates) != EOK) {
        oam_error_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
            "hmac_chba_wait_assoc:: support rate ie len %d, vap rate num %d",
            support_rate_ie_len, uc_rs_nrates);
    }

    /* 使能WMM */
    hmac_vap->st_vap_base_info.st_cap_flag.bit_wmm_cap = OAL_TRUE;
    /* 使能聚合 */
...
static uint32_t hmac_chba_wait_assoc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, void *param)
{
    ...
    hmac_chba_free_assoc_req_by_vap(hmac_user);
    ...
    /* 此处申请的内存，只在上报给内核后释放 */
    if (hmac_chba_save_assoc_req_frame_by_vap(hmac_user,
