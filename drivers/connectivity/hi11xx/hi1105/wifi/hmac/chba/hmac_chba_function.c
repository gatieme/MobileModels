...
void hmac_chba_h2d_update_bssid(mac_vap_stru *mac_vap, uint8_t *domain_bssid, uint8_t domain_bssid_len)
{
    ...
    oam_warning_log4(0, 0, "hmac_chba_h2d_update_bssid::send bssid(XX:XX:XX:%02X:%02X:%02X) to dmac, bitmap 0x%X.",
        chba_params_sync.domain_bssid[MAC_ADDR_3], chba_params_sync.domain_bssid[MAC_ADDR_4],
    ...
    mac_chba_set_sync_request *sync_req_params)
{
    ...
    oam_warning_log3(0, 0, "hmac_chba_request_sync::send sync request action, peer addr is XX:XX:XXX:%02X:%02X:%02X.",
        peer_info->peer_addr[MAC_ADDR_3], peer_info->peer_addr[MAC_ADDR_4], peer_info->peer_addr[MAC_ADDR_5]);
...
uint32_t hmac_chba_sync_waiting_timeout(void *arg)
{
    ...
    oam_warning_log3(0, 0, "hmac_chba_sync_waiting_timeout::send sync req action, peer addr XX:XX:XX:%02X:%02X:%02X.",
        request_sync_info->peer_addr[MAC_ADDR_3], request_sync_info->peer_addr[MAC_ADDR_4],
    ...
    master_info *new_max_rp_device)
{
    ...
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
        "{hmac_chba_update_master_election_info:mac_addr[XX:XX:XX:%02x:%02x:%02x]}",
        new_max_rp_device->master_addr[MAC_ADDR_3], new_max_rp_device->master_addr[MAC_ADDR_4],
...
uint32_t hmac_chba_response_connect(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    hmac_user->chba_user.connect_info.assoc_waiting_time = chba_conn_params.connect_timeout * HMAC_S_TO_MS;
    ...
    chba_req_sync_after_assoc_stru *req_sync)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA,
        ...
        peer_rp_info.master_addr[MAC_ADDR_3], peer_rp_info.master_addr[MAC_ADDR_4],
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    if (chba_ie[1] < (sizeof(mac_ieee80211_vendor_ie_stru) - MAC_IE_HDR_LEN)) {
        oam_warning_log1(0, OAM_SF_CHBA, "hmac_chba_connect_succ_handler: chba ie [%u].", chba_ie[1]);
        return;
    }
    ...
    master_attr = hmac_find_chba_attr(MAC_CHBA_ATTR_MASTER_ELECTION, chba_ie, chba_ie_len);
    if (master_attr == NULL) {
