...
void hmac_chba_print_self_conn_info(void)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_print_self_conn_info::SelfConnInfo. ownerAddr XX:XX:XX:%2X:%2X:%2X",
        self_conn_info->island_owner_addr[MAC_ADDR_3], self_conn_info->island_owner_addr[MAC_ADDR_4],
        self_conn_info->island_owner_addr[MAC_ADDR_5]);
    for (j = 0; j < self_conn_info->island_device_cnt; j++) {
        oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_print_self_conn_info::device addr XX:XX:XX:%2X:%2X:%2X",
            self_conn_info->island_device_list[j].mac_addr[MAC_ADDR_3],
            self_conn_info->island_device_list[j].mac_addr[MAC_ADDR_4],
            self_conn_info->island_device_list[j].mac_addr[MAC_ADDR_5]);
    }
}
...
/* 成功返回 0；失败返回 -1 */
static int hi_hash_init(struct hi_hash_table *ht,
        ...
        if (hmac_chba_get_network_topo_ele(self_conn_info->self_device_id, device_id) == FALSE) {
            ...
            if (self_conn_info->self_device_id != device_id) {
                ...
                oam_warning_log3(0, OAM_SF_CHBA, "hmac_chba_fill_self_island_device_list: mac addr %2X:%2X:%2X",
                    device_id_info[device_id].mac_addr[MAC_ADDR_3], device_id_info[device_id].mac_addr[MAC_ADDR_4],
                    device_id_info[device_id].mac_addr[MAC_ADDR_5]);
            }
    ...
    uint8_t master_device_id)
{
    ...
    /*  CHBA 1.0为一域单岛方案，不应走入根据rssi选岛主的流程。后续开发一域多岛方案则删除 */
        ...
        if (hmac_chba_island_device_check(&attr_pos[index]) == TRUE) {
            ...
            if (oal_compare_mac_addr(self_conn_info->island_owner_addr, &attr_pos[index]) != 0) {
                ...
                oam_warning_log3(0, 0, "rx_island_owner_attr::new owner XX:XX:XX:%2X:%2X:%2X",
                    attr_pos[index + MAC_ADDR_3], attr_pos[index + MAC_ADDR_4], attr_pos[index + MAC_ADDR_5]);
            }
...
/* 功能描述: chba活跃设备哈希表初始化 */
void hmac_chba_alive_dev_table_init(void)
{
    hmac_chba_alive_device_table_stru *alive_dev_table = g_chba_mgmt_info.alive_device_table;

    if (alive_dev_table == NULL) {
        alive_dev_table = hmac_chba_hash_creat();
        /* 创建失败返回 */
        if (alive_dev_table == NULL) {
            oam_error_log0(0, OAM_SF_CHBA, "hmac_chba_module_init::creat hash fail");
            return;
        }
        g_chba_mgmt_info.alive_device_table = alive_dev_table; /* 将新申请到的哈希表存入chba mgmt结构体 */
    } else { /* 如果非空，清空哈希表 */
        hmac_chba_hash_clear(alive_dev_table);
    }
    oam_warning_log0(0, OAM_SF_CHBA, "{hmac_chba_alive_dev_table_init}");
}

/* 功能描述: chba活跃设备哈希表去初始化 */
void hmac_chba_alive_dev_table_deinit(void)
{
    hmac_chba_alive_device_table_stru *alive_dev_table = g_chba_mgmt_info.alive_device_table;
    if (alive_dev_table == NULL) {
        return;
    ...
    /* 释放chba hash表 */
    if (alive_dev_table->node_cnt != 0) { /* 若hash表非空, 则先清空哈希表下各节点 */
        oam_warning_log1(0, OAM_SF_CHBA, "{hmac_chba_module_deinit: chba node cnt[%d]!}", alive_dev_table->node_cnt);
        hmac_chba_hash_clear(alive_dev_table);
    }
    if (alive_dev_table->raw.bkts != NULL) {
        oal_free(alive_dev_table->raw.bkts);
    }
    oal_free(alive_dev_table);
    g_chba_mgmt_info.alive_device_table = NULL;
    oam_warning_log0(0, OAM_SF_CHBA, "{hmac_chba_alive_dev_table_deinit}");
}
/*
 * 功能描述  : CHBA模块初始化
 * 日    期  : 2021年02月24日
...
static void hmac_chba_init_device_mgmt_info(mac_chba_vap_start_rpt *info_report)
{
    ...
    if (device_id >= MAC_CHBA_MAX_DEVICE_NUM) {
        oam_error_log4(0, OAM_SF_CHBA, "hmac_chba_init_device_mgmt_info: addr XX:XX:XX:%2X:%2X:%2X, wrong device_id %d",
            info_report->mac_addr[MAC_ADDR_3], info_report->mac_addr[MAC_ADDR_4], info_report->mac_addr[MAC_ADDR_5],
            device_id);
        return;
    }
    /* 更新本设备连接信息 */
...
void hmac_chba_vap_stop_proc(uint8_t *notify_msg)
{
    ...
    /* chba活跃设备哈希表去初始化 */
    hmac_chba_alive_dev_table_deinit();
}
