    ...
    uint8_t island_peer_flag)
{
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        ...
        peer_addr[MAC_ADDR_3], peer_addr[MAC_ADDR_4], peer_addr[MAC_ADDR_5], chan_number);
}
/*
 * 功能描述  : 启动CHBA同步请求流程
 * 日    期  : 2021年10月23日
    ...
    sync_request *sync_req_ctrl)
{
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        ...
        sync_info_param.sync_type, sync_info_param.request_peer.master_addr[MAC_ADDR_3],
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA,
        ...
        peer_domain_bssid[MAC_ADDR_3], peer_domain_bssid[MAC_ADDR_4], peer_domain_bssid[MAC_ADDR_5]);
}
...
void hmac_chba_domain_merge_start_handler(master_info *another_master, uint8_t *peer_addr, uint8_t mgmt_type)
{
    ...
    if (hmac_chba_get_domain_merge_flag(domain_merge) == OAL_TRUE) {
        oam_warning_log3(0, OAM_SF_CHBA,
            "{hmac_chba_domain_merge_start_handler:domain_merge is running, find_another_dev[XX:XX:XX:%02x:%02x:%02x]}",
            peer_addr[MAC_ADDR_3], peer_addr[MAC_ADDR_4], peer_addr[MAC_ADDR_5]);
        return;
    }
    ...
    hmac_chba_sync_info *sync_info, hmac_chba_domain_merge_info *domain_merge)
{
    if (domain_merge->domain_merge_notify_cnt > MAC_MERGE_REQ_MAX_CNT) { /* 达到req发送上限依然失败, 放弃通知对端 */
        ...
        oam_error_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "{hmac_chba_domain_merge_send_domain_requset_immediately:fail to notify hist_master[%02X:%02X]}",
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        "{hmac_chba_domain_merge_send_domain_requset_immediately:cnt[%d], peer_addr XX:XX:XX:%02X:%02X:%02X}",
        domain_merge->domain_merge_notify_cnt, sync_info->his_master_info.master_addr[MAC_ADDR_3],
        sync_info->his_master_info.master_addr[MAC_ADDR_4], sync_info->his_master_info.master_addr[MAC_ADDR_5]);
}
    ...
    hmac_chba_domain_merge_info *domain_merge)
{
    ...
    oam_warning_log4(0, OAM_SF_CHBA,
        "{hmac_chba_domain_merge_send_notify_beacon_immediately:cnt[%d], bssid XX:XX:XX:%02X:%02X:%02X}",
        domain_merge->domain_merge_notify_cnt, domain_merge->his_bssid[MAC_ADDR_3],
