 ...
 * 功能说明 : hmac_config.c 的头文件
 * 作    者 :
 * 创建日期 : 2013年1月8日
 */

#ifndef HMAC_CONFIG_H
#define HMAC_CONFIG_H
...
typedef struct {
    ...
    uint8_t fips;                     /* Windows FIPS安全机制使能标记 */
    uint8_t resv[3];
#endif
...
extern int gf61_ssi_test(unsigned int addr, unsigned int value);
...
uint32_t hmac_config_d2h_vap_mib_update(mac_vap_stru *mac_vap, uint8_t uc_len, uint8_t *puc_param);
...
uint32_t hmac_config_dpd_cali_factory(mac_vap_stru *pst_mac_vap, uint16_t uc_len, uint8_t *puc_param);
