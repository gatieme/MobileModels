...
#ifndef HMAC_HT_SELF_CURE_C
#define HMAC_HT_SELF_CURE_C
...
OAL_STATIC void hmac_ht_self_cure_add_blacklist(uint8_t *user_mac)
{
    ...
    ret = memcpy_s(ht_self_cure->ht_black_list[ht_self_cure->black_list_num].user_mac_addr,
        sizeof(ht_self_cure->ht_black_list[ht_self_cure->black_list_num].user_mac_addr), user_mac, WLAN_MAC_ADDR_LEN);
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_SCAN, "hmac_ht_self_cure_add_blacklist::memcpy failed!");
    }
    ...
    uint8_t *user_mac, uint8_t event_id)
{
    ...
    if (mac_vap->en_protocol == WLAN_HT_MODE) {
        ret = memcpy_s(ht_self_cure->ht_check_info.ht_assoc_user, sizeof(ht_self_cure->ht_check_info.ht_assoc_user),
            user_mac, WLAN_MAC_ADDR_LEN);
        if (ret != EOK) {
            oam_error_log0(0, OAM_SF_SCAN, "memcpy ht_assoc_user failed!");
        }
        ...
        if (mac_vap->st_channel.en_band == WLAN_BAND_2G && mac_is_smc_ap(user_mac)) {
            ret = memcpy_s(ht_self_cure->ht_check_info.check_ht_user, sizeof(ht_self_cure->ht_check_info.check_ht_user),
                user_mac, WLAN_MAC_ADDR_LEN);
            if (ret != EOK) {
                oam_error_log0(0, OAM_SF_SCAN, "memcpy check_ht_user failed!");
            }
            oam_warning_log2(0, OAM_SF_SCAN, "hmac_ht_self_cure_state_init_assoc_event_handle::add ht user[%02X:%02X]!",
                user_mac[MAC_ADDR_4], user_mac[MAC_ADDR_5]);
        }
    } else {
    ...
    switch (event_id) {
        case HMAC_HT_SELF_CURE_EVENT_TX_DHCP_FRAME:
            ...
            if (ht_self_cure->ht_check_info.tx_dhcp_times > HT_SELF_CURE_CLOSE_TX_DHCP_MAX_TIMES) {
                ...
                ret = memcpy_s(ht_self_cure->ht_check_info.check_ht_user,
                    sizeof(ht_self_cure->ht_check_info.check_ht_user), user_mac, WLAN_MAC_ADDR_LEN);
                if (ret != EOK) {
                    oam_error_log0(0, OAM_SF_SCAN, "hmac_ht_self_cure_state_ht_txdhcp_handle::memcpy failed!");
                }
                oam_warning_log2(0, OAM_SF_SCAN, "hmac_ht_self_cure_state_ht_txdhcp_handle::add ht user[%02X:%02X]!",
                    user_mac[MAC_ADDR_4], user_mac[MAC_ADDR_5]);
            }
            break;
