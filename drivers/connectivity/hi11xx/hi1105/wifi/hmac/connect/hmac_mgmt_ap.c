 ...
 * 1.日    期  : 2017年2月21日
  *   修改内容  : 新生成函数
 */
void hmac_ap_up_rx_auth_req_to_host(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req, uint8_t us_auth_alg_num)
{
    ...
    if ((hmac_vap == NULL) || (auth_req == NULL)) {
        return;
    }
...
oal_bool_enum hmac_user_is_connecting(mac_vap_stru *pst_mac_vap)
{
    ...
    /* CHBA启动但是未同步上，不允许扫描;已同步有其他用户入网的场景，避免入网失败也不允许扫描 */
    if (hmac_chba_vap_start_check(hmac_vap) == OAL_TRUE && (hmac_chba_get_sync_state() != CHBA_WORK)) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_user_is_connecting::chba non sync.}");
        return OAL_TRUE;
    }
#endif
...
void hmac_ap_up_rx_deauth_req(hmac_vap_stru *hmac_vap, uint8_t *mac_hdr, uint32_t mac_hdr_len, uint32_t msg_len)
{
    ...
    hmac_ap_rx_deauth_req(hmac_vap, mac_hdr, protected_status);
}
    ...
    dmac_wlan_crx_event_stru *mgmt_rx_event)
{
    ...
    switch (mgmt_frm_type) {
        ...
        case WLAN_FC0_SUBTYPE_DEAUTH:
            hmac_ap_up_rx_deauth_req(hmac_vap, mac_hdr, mac_hdr_len, msg_len);
            break;
