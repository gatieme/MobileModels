 ...
 * 功能说明 : STA模式组帧，HMAC层 STA特有帧的组帧文件
 * 作    者 : wifi
 * 创建日期 : 2013年6月28日
 */
#include "wlan_spec.h"
...
/*
 * 函 数 名  : encap_cowork_ie_adapter_11r
 * 功能描述  : 填充端管协同IE(adaptive 11r)
 * 1.日    期  : 2019年9月30日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : encap_cowork_ie_narrow_band
 * 功能描述  : 填充端管协同IE(窄带)
 * 1.日    期  : 2019年9月30日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
...
void hmac_add_cowork_ie(hmac_vap_stru *hmac_vap, uint8_t *puc_buffer, uint16_t *pus_ie_len)
{
    ...
    *pus_ie_len = 0;
    ...
    if (curr_bss_dscr->mgmt_len < MAC_80211_FRAME_LEN) {
        return;
    }
    us_frame_body_len = (int32_t)(curr_bss_dscr->mgmt_len - MAC_80211_FRAME_LEN);
...
hmac_scanned_bss_info *hmac_vap_get_scan_bss_info(mac_vap_stru *pst_mac_vap)
{
    ...
    pst_scaned_bss = hmac_scan_find_scanned_bss_by_bssid(pst_bss_mgmt, pst_mac_vap->auc_bssid, WLAN_MAC_ADDR_LEN);
...
/*
 * 函 数 名  : hmac_assoc_set_siso_mode
 * 功能描述  : 如果是满足只支持siso入网的黑名单条件，则以SISO方式入网
 * 1.日    期  : 2018年12月19日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : hmac_update_user_sounding_dim_num
 * 功能描述  : 组关联请求帧时 提前更新user的sounding_dim_num，即txbf天线数，供组帧参考
 * 1.日    期  : 2019年3月11日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : hmac_sta_roam_and_assoc_update_txbf
 * 功能描述  : 漫游或者入网时候刷新txbf黑名单能力
 * 1.日    期  : 2019.4.9
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
{
    if (g_wlan_spec_cfg->feature_txbf_mib_study == OAL_FALSE) { /* 不学习对端能力，2G不规避 */
        return;
    }
...
/*
 * 函 数 名  : mac_set_ht_cap_ie_rom_cb
 * 功能描述  : 填充mac_set_ht_cap_ie_的补充信息
 * 1.日    期  : 2019年10月13日
  *   修改内容  : 新生成函数
 */
...
{
    uint16_t fc;
    mac_ieee80211_frame_stru *mac_head = (mac_ieee80211_frame_stru *)req_frame;
    /*************************************************************************/
...
void hmac_set_assoc_req_ssid_ie(hmac_vap_stru *hmac_vap, uint8_t *req_frame, uint8_t *ie_len, uint8_t *dest_addr)
{
#ifdef _PRE_WLAN_CHBA_MGMT
        if (hmac_chba_vap_start_check(hmac_vap) == OAL_TRUE) {
            hmac_chba_assoc_set_ssid(hmac_vap, req_frame, ie_len, dest_addr);
        } else {
#endif
            *ie_len = mac_set_ssid_ie(&hmac_vap->st_vap_base_info, req_frame, WLAN_FC0_SUBTYPE_ASSOC_REQ);
#ifdef _PRE_WLAN_CHBA_MGMT
        }
#endif
}
static uint16_t hmac_set_assoc_req_frame_base_ie(hmac_vap_stru *hmac_sta, mac_vap_stru *mac_vap,
                                                 uint8_t *req_frame, uint8_t *curr_bssid, uint8_t *dest_addr)
{
    uint8_t ie_len = 0;
    uint16_t out_ie_len;
    /*************************************************************************/
    ...
    /* 设置 SSID IE */
    hmac_set_assoc_req_ssid_ie(hmac_sta, req_frame + out_ie_len, &ie_len, dest_addr);
    out_ie_len += ie_len;
                                                   ...
                                                   hmac_scanned_bss_info *scaned_bss)
{
    ...
    if (scaned_bss->st_bss_dscr_info.en_vendor_vht_capable == OAL_TRUE) {
        mac_set_vendor_vht_ie_b(mac_vap, req_frame + out_ie_len, &ie_len);
        out_ie_len += ie_len;
    }
    ...
    if (scaned_bss->st_bss_dscr_info.en_vendor_novht_capable == OAL_TRUE) {
        mac_set_vendor_novht_ie_b(mac_vap, req_frame + out_ie_len, &ie_len, support_1024qam);
        out_ie_len += ie_len;
    }
    /* 填充QLM Vendor VHT IE,解决无法到MCS11问题 */
    if (g_mac_vendor_vht_switch == OAL_TRUE && scaned_bss->st_bss_dscr_info.chip_oui == WLAN_AP_CHIP_OUI_QLM &&
        scaned_bss->st_bss_dscr_info.en_he_capable == OAL_FALSE &&
        scaned_bss->st_bss_dscr_info.en_vht_capable == OAL_TRUE) {
        mac_set_vendor_vht_ie_q(mac_vap, req_frame + out_ie_len, &ie_len);
        out_ie_len += ie_len;
    }
    return out_ie_len;
}
                                               ...
                                               hmac_vap_stru *hmac_sta)
{
    ...
    /* it_rrm_include_enable标志位是为了VOE5.2认证使用 */
                                               ...
                                               hmac_scanned_bss_info *scaned_bss)
{
    ...
    /* 设置 HT Capability IE  */
...
#ifdef _PRE_WLAN_FEATURE_M2S
        /*  兼容性问题，如果是满足只支持siso入网的黑名单条件，则以SISO方式入网 */
        ...
        if ((scaned_bss->st_bss_dscr_info.en_vht_capable == OAL_TRUE) &&
            (scaned_bss->st_bss_dscr_info.en_vendor_vht_capable == OAL_FALSE)) {
...
static uint16_t hmac_set_assoc_req_opmode_ie(mac_vap_stru *mac_vap, uint8_t *req_frame)
{
    ...
    if ((hmac_user != NULL) && (mac_vap->st_cap_flag.bit_opmode == OAL_TRUE) &&
        (mac_vap->en_protocol != WLAN_HE_MODE)) { /* vht需携带opmode */
...
static void hmac_mgmt_encap_asoc_req_sta_features_proc(mac_vap_stru *mac_vap, hmac_scanned_bss_info *pst_scaned_bss)
{
    ...
    uint8_t *curr_bssid, uint8_t *dest_addr)
{
    ...
    if (oal_any_null_ptr3(hmac_sta, req_frame, dest_addr)) {
    ...
    if (hmac_chba_vap_start_check(hmac_sta) == OAL_TRUE) {
        ...
        pst_scaned_bss = &st_scaned_bss;
        hmac_chba_set_assoc_scan_bss(hmac_sta, pst_scaned_bss);
    }
#endif
    hmac_mgmt_encap_asoc_req_sta_features_proc(mac_vap, pst_scaned_bss);
    hmac_set_assoc_req_frame_head_element(req_frame, mac_vap, curr_bssid, dest_addr);
 ...
 *    *************************************************************************
*/
    req_frame += hmac_set_assoc_req_frame_base_ie(hmac_sta, mac_vap, req_frame, curr_bssid, dest_addr);
    if (pst_scaned_bss != NULL) {
#if defined(_PRE_WLAN_FEATURE_11K) ||  defined(_PRE_WLAN_FEATURE_FTM)
...
#ifdef _PRE_WLAN_CHBA_MGMT
    if (hmac_chba_vap_start_check(hmac_sta) == OAL_TRUE) {
        uc_ie_len = hmac_chba_encape_assoc_attribute(req_frame, 0);
        req_frame += uc_ie_len;
    }
#endif
...
static void hmac_mgmt_auth_req_fill_alg(hmac_vap_stru *hmac_sta, uint8_t *mgmt_frame)
{
    uint16_t auth_type;
...
#ifdef _PRE_WLAN_FEATURE_11R
    if (hmac_sta->bit_11r_enable == OAL_TRUE && (mac_mib_get_ft_trainsistion(&hmac_sta->st_vap_base_info) == OAL_TRUE)
        && (hmac_sta->st_vap_base_info.en_vap_state == MAC_VAP_STATE_ROAMING)) {
        /* FT System */
        mgmt_frame[MAC_80211_FRAME_LEN] = 0x02;
        mgmt_frame[MAC_80211_FRAME_LEN + 1] = 0x00;
#ifdef _PRE_WLAN_FEATURE_PWL
        /* pwl协议下，认证算法是0xFFFF，用PWL_AuthAlg表示 */
        if (hmac_sta->st_vap_base_info.pwl_enable == OAL_TRUE) {
            mgmt_frame[MAC_80211_FRAME_LEN] = 0xFF;
            mgmt_frame[MAC_80211_FRAME_LEN + 1] = 0xFF;
        }
#endif
    }
#endif  // _PRE_WLAN_FEATURE_11R
...
uint16_t hmac_mgmt_encap_auth_req(hmac_vap_stru *hmac_sta, uint8_t *mgmt_frame, uint8_t *dest_addr)
{
    ...
    mac_ieee80211_frame_stru *mac_head = (mac_ieee80211_frame_stru *)mgmt_frame;
#ifdef _PRE_WLAN_FEATURE_11R
    uint16_t app_ie_len;
#endif  // _PRE_WLAN_FEATURE_11R
    ...
    oal_set_mac_addr(((mac_ieee80211_frame_stru *)mgmt_frame)->auc_address1, dest_addr);
    ...
    /*                                                                       */
    /*************************************************************************/
    hmac_mgmt_auth_req_fill_alg(hmac_sta, mgmt_frame);
    ...
    if (user_ap == NULL) {
        if (hmac_user_add(&hmac_sta->st_vap_base_info, dest_addr, &user_idx) != OAL_SUCC) {
            auth_req_len = 0;
        }
    }
    return auth_req_len;
    ...
    mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (mac_is_protectedframe(puc_mac_hrd) == 1) {
        ...
        if (rx_ctl->us_frame_len <= MAC_80211_FRAME_LEN + BYTE_OFFSET_7) {
            oam_error_log0(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                "{hmac_wait_auth_sta::uc_mac_header_len length short.}");
            return 0;
        }
        uc_ch_text_len = puc_mac_hrd[MAC_80211_FRAME_LEN + BYTE_OFFSET_7];
        if ((rx_ctl->us_frame_len < (MAC_80211_FRAME_LEN + BYTE_OFFSET_6 + MAC_IE_HDR_LEN)) ||
            (uc_ch_text_len > (rx_ctl->us_frame_len - MAC_80211_FRAME_LEN - BYTE_OFFSET_6 - MAC_IE_HDR_LEN))) {
            oam_error_log1(pst_sta->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                "{hmac_wait_auth_sta::uc_ch_text_len over length.uc_ch_text_len=[%d]}", uc_ch_text_len);
            return 0;
        }
