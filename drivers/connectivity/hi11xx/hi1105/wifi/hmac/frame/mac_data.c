...
uint8_t mac_get_data_type_from_80211(oal_netbuf_stru *pst_netbuff, uint16_t us_mac_hdr_len, uint32_t payload_len)
{
    ...
    uc_datatype = mac_get_data_type_from_8023(pst_netbuff, (uint8_t *)pst_snap, MAC_NETBUFF_PAYLOAD_SNAP, payload_len,
...
/*
 * 函 数 名  : mac_get_eth_data_subtype
 * 功能描述  : 根据以太网帧类型，判断子类型
 * 1.日    期  : 2021年2月8日
 *   作    者  : wifi
 *   修改内容  : 判断是eth2还是8023
...
uint8_t mac_get_data_type_by_snap(oal_netbuf_stru *netbuff)
{
    ...
    /* 对没有snap头的数据帧进行保护 */
    rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(netbuff);
    ...
    datatype = mac_get_data_type_from_8023(netbuff, (uint8_t *)snap, MAC_NETBUFF_PAYLOAD_SNAP,
        rx_info->us_frame_len - rx_info->uc_mac_header_len, OAM_OTA_FRAME_DIRECTION_TYPE_TX);
...
/*
 * 函 数 名  : mac_get_eapol_key_type
 * 功能描述  : 判断该帧是否为4 次握手的EAPOL KEY ,并识别帧类型
 * 1.日    期  : 2018年9月6日
  *   修改内容  : 新生成函数
 */
 ...
 *   修改内容  : 增加解析长度保护
 */
uint8_t mac_get_data_type_from_8023(oal_netbuf_stru *netbuf, uint8_t *frame_hdr, mac_netbuff_payload_type hdr_type,
    uint32_t payload_len, uint8_t direction)
{
    ...
    if (payload_len < shift_len) {
        oam_warning_log3(0, OAM_SF_ANY, "mac_get_data_type_from_8023:payload_len=%d, shift_len=%d direction=%d",
            payload_len, shift_len, direction);
        oam_report_netbuf_cb(BROADCAST_MACADDR, (uint8_t *)oal_netbuf_cb(netbuf), sizeof(mac_rx_ctl_stru), direction);
        return datatype;
    }
    if (hdr_type == MAC_NETBUFF_PAYLOAD_ETH) {
