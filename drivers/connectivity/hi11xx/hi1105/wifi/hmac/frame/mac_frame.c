...
/* 2 函数原型声明 */
/* 3 全局变量定义 */
uint8_t g_mac_vendor_vht_switch = 0;
/* 4 函数实现 */
/*
 * 函 数 名  : mac_find_ie_ext_ie
 * 功能描述  : 查找指定的IE
 * 1.日    期  : 2017年5月25日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
...
uint8_t *mac_find_ie_ext_ie(uint8_t uc_eid, uint8_t uc_ext_eid, uint8_t *puc_ies, int32_t l_len)
{
    ...
    while ((l_len > MAC_IE_EXT_HDR_LEN) &&
        ...
        l_len = (int32_t)((uint32_t)l_len - (puc_ies[BYTE_OFFSET_1] + MAC_IE_HDR_LEN));
...
/*
 * 函 数 名  : mac_set_11ax_dcm_vendor_ie
 * 功能描述  : 填充厂商自定义ie。由于历史原因，私有ie的bit0-bit4不从定制化获取其值，需要单独赋值
 * 1.日    期  : 2019年10月14日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mac_set_hisi_cap_vendor_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    if (mac_vap_is_work_he_protocol(mac_vap)) {
        ...
        memcpy_s(pst_vendor_ie_param, sizeof(mac_hisi_cap_vendor_ie_stru),
                 &mac_device->st_device_cap.hisi_priv_cap, sizeof(mac_hisi_cap_vendor_ie_stru));
        pst_vendor_ie_param->bit_11ax_support = OAL_TRUE; /* 重新单独赋值 */
    ...
    if (oal_value_eq_any2(mac_vap->en_protocol, WLAN_VHT_MODE, WLAN_VHT_ONLY_MODE)) {
        pst_vendor_ie_param->bit_1024qam_cap = mac_device->st_device_cap.hisi_priv_cap.bit_1024qam_cap;
        pst_vendor_ie_param->bit_vht_3nss_80m_mcs6 = mac_device->st_device_cap.hisi_priv_cap.bit_vht_3nss_80m_mcs6;
    }
...
{
    mac_hisi_cap_vendor_ie_stru *vendor_ie_param = NULL;
    uint8_t *vendor_ie = NULL;
    uint8_t *ie_tmp = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_CAP_IE, payload, (int32_t)msg_len);

    if (ie_tmp != NULL) {
        /* 兼容mp15 C10/C20版本，解决私有IE结构体定义长度不一致问题 */
        /* mp15商用版本私有IE结构体定义如下：
        ...
        if (ie_tmp[1] == (MAC_HISI_CAP_VENDOR_IE_LEN - MAC_IE_HDR_LEN)) {
            vendor_ie = (uint8_t *)&(((mac_hisi_cap_ie_stru *)ie_tmp)->vendor_ie_cap);
            hisi_cap_ie->bit_11ax_support = !!(vendor_ie[0] & BIT0);
            hisi_cap_ie->bit_dcm_support = !!(vendor_ie[0] & BIT1);
            hisi_cap_ie->bit_p2p_csa_support = !!(vendor_ie[0] & BIT2);
            return OAL_SUCC;
        } else if (ie_tmp[1] >= (sizeof(mac_hisi_cap_ie_stru) - MAC_IE_HDR_LEN)) {
            vendor_ie_param = &(((mac_hisi_cap_ie_stru *)ie_tmp)->vendor_ie_cap);
            memcpy_s(hisi_cap_ie, sizeof(mac_hisi_cap_vendor_ie_stru),
                vendor_ie_param, sizeof(mac_hisi_cap_vendor_ie_stru));
            return OAL_SUCC;
        }
    }
    return OAL_FAIL;
...
/*
 * 函 数 名  : mac_set_1024qam_vendor_ie
 * 功能描述  : 填充厂商自定义ie
 * 1.日    期  : 2016年7月10日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : mac_set_adjust_pow_vendor_ie
 * 功能描述  : 填充厂商自定义ie
 * 1.日    期  : 2019年8月20日
 *   修改内容  : 新生成函数
 */
...
void mac_set_vendor_vht_ie_b(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    mac_set_vht_capabilities_ie(mac_vap, puc_buffer + puc_buffer[1] + MAC_IE_HDR_LEN, &ie_len);
    if (ie_len) {
        puc_buffer[1] += ie_len;
...
void mac_set_vendor_vht_ie_q(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    puc_buffer[BYTE_OFFSET_0] = MAC_EID_VENDOR;
    /* length */
    puc_buffer[BYTE_OFFSET_1] = 15; /* 15: Vendor length */
    /* OUI */
    puc_buffer[BYTE_OFFSET_2] = (uint8_t)((MAC_WLAN_CHIP_OUI_QUALCOMM >> BIT_OFFSET_16) & 0xff);
    puc_buffer[BYTE_OFFSET_3] = (uint8_t)((MAC_WLAN_CHIP_OUI_QUALCOMM >> BIT_OFFSET_8) & 0xff);
    puc_buffer[BYTE_OFFSET_4] = (uint8_t)((MAC_WLAN_CHIP_OUI_QUALCOMM) & 0xff);
    /* value */
    puc_buffer[BYTE_OFFSET_5] = 0x01;
    puc_buffer[BYTE_OFFSET_6] = 0x01;
    puc_buffer[BYTE_OFFSET_7] = 0x02;
    puc_buffer[BYTE_OFFSET_8] = 0x01;
    puc_buffer[BYTE_OFFSET_9] = 0x00;
    puc_buffer[BYTE_OFFSET_10] = 0x02;
    puc_buffer[BYTE_OFFSET_11] = 0x01;
    puc_buffer[BYTE_OFFSET_12] = 0x01;
    puc_buffer[BYTE_OFFSET_13] = 0x09;
    puc_buffer[BYTE_OFFSET_14] = 0x02;
    puc_buffer[BYTE_OFFSET_15] = 0x03;
    puc_buffer[BYTE_OFFSET_16] = 0x03;
    *puc_ie_len = puc_buffer[BYTE_OFFSET_1] + MAC_IE_HDR_LEN;
}

/*
 * 函 数 名  : mac_set_vendor_novht_ie_b
    ...
    uint8_t *puc_ie_len, uint8_t en_1024qam_capable)
{
    puc_buffer[BYTE_OFFSET_0] = MAC_EID_VENDOR;
    puc_buffer[BYTE_OFFSET_1] = MAC_WLAN_OUI_VENDOR_VHT_HEADER; /* The Vendor OUI, type and subtype */
    puc_buffer[BYTE_OFFSET_2] = 0x00; /* (MAC_WLAN_OUI_BROADCOM_EPIGRAM >> BIT_OFFSET_16) & 0xff */
    puc_buffer[BYTE_OFFSET_3] = (uint8_t)((MAC_WLAN_OUI_BROADCOM_EPIGRAM >> BIT_OFFSET_8) & 0xff);
    puc_buffer[BYTE_OFFSET_4] = (uint8_t)((MAC_WLAN_OUI_BROADCOM_EPIGRAM)&0xff);
    puc_buffer[BYTE_OFFSET_5] = MAC_WLAN_OUI_VENDOR_VHT_TYPE;
...
/*
 * 函 数 名  : mac_set_ext_capabilities_ftm_twt
 * 1.日    期  : 2017年09月12日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : mac_set_vht_capinfo_field_extend
...
#ifdef _PRE_WLAN_FEATURE_160M
/*
 * 功能描述  : 设置160M center freq segment
 * 1.日    期  : 2020年8月5日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : mac_set_vht_opern_ie_rom_cb
 * 功能描述  : rom cb填充mac_set_vht_opern_ie_rom_cb信息
 * 1.日    期  : 2018年5月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
uint8_t *mac_find_ie(uint8_t uc_eid, uint8_t *puc_ies, int32_t l_len)
{
    ...
    while (l_len > MAC_IE_HDR_LEN && puc_ies[0] != uc_eid) {
        l_len -= (int32_t)puc_ies[1] + MAC_IE_HDR_LEN;
...
{
    mac_ieee80211_vendor_ie_stru  *pst_ie = NULL;
    ...
    while (puc_pos < puc_end) {
        ...
        pst_ie = (mac_ieee80211_vendor_ie_stru *)puc_pos;
...
void mac_set_cap_info_ap(mac_vap_stru *mac_vap, uint8_t *puc_cap_info)
{
    ...
    ***************************************************************************/
    ...
    /* QoS subfield */
    /*  ccb决策，修改assoc rsp能力信息中的qos为0; mib里面不修改 */
...
uint8_t mac_set_wpa_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    for (uc_loop = 0; uc_loop < uc_pair_suites_num; uc_loop++) {
        if (EOK != memcpy_s(&puc_buffer[uc_index], sizeof(uint32_t), (uint8_t *)(&pcip[uc_loop]), sizeof(uint32_t))) {
            oam_error_log0(0, OAM_SF_ANY, "mac_set_wpa_ie::memcpy fail!");
        }
        uc_index += INDEX_OFFSET_4;
    }
    ...
    for (uc_loop = 0; uc_loop < uc_akm_suites_num; uc_loop++) {
        if (EOK != memcpy_s(&puc_buffer[uc_index], sizeof(uint32_t), (uint8_t *)(&akm[uc_loop]), sizeof(uint32_t))) {
            oam_error_log0(0, OAM_SF_ANY, "mac_set_wpa_ie::memcpy fail!");
        }
        uc_index += INDEX_OFFSET_4;
    }
...
uint8_t mac_set_exsup_rates_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***************************************************************************/
    ...
    if (mac_vap->sae_pwe == SAE_PWE_HASH_TO_ELEMENT) {
        uc_rs_nrates++;
    }
...
void mac_set_ht_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    pst_ht_capinfo->bit_rx_stbc = mac_mib_get_RxSTBCOptionImplemented(mac_vap); /* 接收只支持收单流 */
...
void mac_set_vht_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***************************************************************************/
    ...
    pst_vht_capinfo->bit_rx_stbc = (mac_mib_get_VHTRxSTBCOptionImplemented(mac_vap) == OAL_TRUE) ? 1 : 0;
    ...
    pst_vht_capinfo->bit_num_bf_ant_supported = (pst_vht_capinfo->bit_su_beamformee_cap == OAL_FALSE) ? 0 :
        mac_mib_get_VHTBeamformeeNTxSupport(mac_vap) - 1;

#ifdef _PRE_WLAN_FEATURE_M2S
    /*  满足认证 当user的sounding_dim为0时,使用vap的能力位 */
    pst_mac_user = mac_res_get_mac_user(mac_vap->us_assoc_vap_id);
    if (g_wlan_spec_cfg->feature_txbf_mib_study == OAL_TRUE && mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA &&
        pst_mac_user != NULL && (pst_mac_user->st_vht_hdl.bit_num_sounding_dim >= 1)) {
    ...
    pst_vht_capinfo->bit_num_sounding_dim = (pst_vht_capinfo->bit_su_beamformer_cap == OAL_FALSE) ? 0 :
        mac_mib_get_VHTNumberSoundingDimensions(mac_vap);
 ...
 * 功能描述  : 设置vht ie前置条件判断
 */
static oal_bool_enum_uint8 mac_check_if_not_support_vht(mac_vap_stru *mac_vap)
{
    return ((OAL_TRUE != mac_mib_get_VHTOptionImplemented(mac_vap)) ||
            (((OAL_TRUE == mac_is_wep_enabled(mac_vap)) ||
              (OAL_TRUE == mac_is_tkip_only(mac_vap))) &&
             (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP))
...
{
    if (mac_check_if_not_support_vht(mac_vap)) {
        *puc_ie_len = 0;
        return;
    }
...
/*
 * 功能描述  : 设置非160M center freq segment
 * 1.日    期  : 2020年8月5日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
 ...
 *   修改内容  : 新生成函数。
 */
uint16_t mac_get_rsn_capability(const uint8_t *rsn_ie)
{
    uint16_t pairwise_count, akm_count, rsn_capability, suite_idx, index;
    uint8_t  ie_len;
    ...
    /*                                                                            */
    /******************************************************************************/
    ie_len = rsn_ie[BYTE_OFFSET_1];
    if (ie_len < MAC_MIN_RSN_LEN) {
        oam_warning_log1(0, OAM_SF_WPA, "{hmac_get_rsn_capability::invalid rsn ie len[%d]}", rsn_ie[BYTE_OFFSET_1]);
        return 0;
    }

    index = 8; /* 8表示Element ID、Length、Version和Group Cipher Suite长度 */
    /* 获取Pairwise Suite Count */
    pairwise_count = oal_make_word16(rsn_ie[index], rsn_ie[index + 1]);
    if (pairwise_count > WLAN_PAIRWISE_CIPHER_SUITES) {
        oam_warning_log1(0, OAM_SF_WPA, "{hmac_get_rsn_capability::invalid pairwise_count[%d].}", pairwise_count);
        return 0;
    }
    index += INDEX_OFFSET_2;

    /* Pairwise Cipher Suite List */
    for (suite_idx = 0; suite_idx < pairwise_count; suite_idx++) {
        if (mac_ie_reamin_len_is_enough(&rsn_ie[BYTE_OFFSET_2], &rsn_ie[index], ie_len, MAC_IE_REMAIN_LEN_4) ==
            OAL_FALSE) {
            return 0;
        }
        index += INDEX_OFFSET_4;
    ...
    /* AKM Suite Count */
    if (mac_ie_reamin_len_is_enough(&rsn_ie[BYTE_OFFSET_2], &rsn_ie[index], ie_len, MAC_IE_REMAIN_LEN_2) == OAL_FALSE) {
        return 0;
    }
    ...
    /* AKM Suite List */
    for (suite_idx = 0; suite_idx < akm_count; suite_idx++) {
        if (mac_ie_reamin_len_is_enough(&rsn_ie[BYTE_OFFSET_2], &rsn_ie[index], ie_len, MAC_IE_REMAIN_LEN_4) ==
            OAL_FALSE) {
            return 0;
        }
...
void mac_set_power_cap_ie(uint8_t *pst_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    *(puc_buffer + 1) = MAC_PWR_CAP_LEN;
    ...
    pst_regclass_info = mac_get_channel_num_rc_info(pst_mac_vap->st_channel.en_band,
                                                    ...
                                                    pst_mac_vap->st_channel.ext6g_band);
...
/*
 * 函 数 名  : mac_support_channel_cal
 * 功能描述  : 设置支持信道信息元素时，填写信道信息
 */
OAL_STATIC uint8_t mac_support_channel_cal(mac_vap_stru *mac_vap, uint8_t *buffer, uint8_t channel_max_num)
{
    uint8_t channel_idx;
    uint8_t channel_idx_cnt = 0;
    uint8_t channel_ie_len = 0;

    /* 填写信道信息 */
    for (channel_idx = 0; channel_idx < channel_max_num; channel_idx++) {
        /* 修改管制域结构体后，需要增加该是否支持信号的判断 */
        if (OAL_SUCC ==
            mac_is_channel_idx_valid(mac_vap->st_channel.en_band, channel_idx, mac_vap->st_channel.ext6g_band)) {
            channel_idx_cnt++;
            /* channel_idx_cnt为1的时候表示是第一个可用信道，需要写到Fisrt Channel Number */
            if (channel_idx_cnt == 1) {
                buffer++;

                mac_get_channel_num_from_idx(mac_vap->st_channel.en_band, channel_idx,
                    mac_vap->st_channel.ext6g_band, buffer);
            } else if ((channel_max_num - 1) == channel_idx) {
                /* 将Number of Channels写入帧体中 */
                buffer++;
                *buffer = channel_idx_cnt;

                channel_ie_len += INDEX_OFFSET_2;
            }
        } else {
...
void mac_set_supported_channel_ie(uint8_t *pst_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    *********************************************************************************************/
    /* 根据支持的频段获取最大信道个数 */
    switch (pst_mac_vap->st_channel.en_band) {
        case WLAN_BAND_2G:
            channel_max_num = (uint8_t)MAC_CHANNEL_FREQ_2_BUTT;
            break;
        case WLAN_BAND_5G:
            if (pst_mac_vap->st_channel.ext6g_band == OAL_FALSE) {
                channel_max_num = (uint8_t)MAC_CHANNEL_FREQ_5_BUTT;
            } else {
                channel_max_num = (uint8_t)MAC_CHANNEL_FREQ_6_BUTT;
            }
            break;
        default:
            *puc_ie_len = 0;
    ...
    *puc_buffer = MAC_EID_SUPPCHAN;
    ...
    /* 填写信道信息 */
    channel_ie_len = mac_support_channel_cal(pst_mac_vap, puc_buffer, channel_max_num);

    *puc_ie_len_buffer = channel_ie_len;
    *puc_ie_len = channel_ie_len + MAC_IE_HDR_LEN;
}
...
/*
 * 函 数 名  : mac_set_tspec_info_field
...
/*
 * 函 数 名  : mac_encap_2040_coext_mgmt
 * 功能描述  : 封装 20/40 共存管理帧
 * 1.日    期  : 2014年3月12日
  *   修改内容  : 新生成函数
 */
    ...
    uint32_t chan_report)
{
    ...
    /*                                                                                   */
    /*************************************************************************************/
    puc_payload_addr[index++] = MAC_ACTION_CATEGORY_PUBLIC; /* Category */
    puc_payload_addr[index++] = MAC_PUB_COEXT_MGMT;         /* Public Action */

    /* 封装20/40 BSS Coexistence element */
    puc_payload_addr[index++] = MAC_EID_2040_COEXT; /* Element ID */
    puc_payload_addr[index++] = MAC_2040_COEX_LEN;  /* Length */
    puc_payload_addr[index++] = uc_coext_info;      /* 20/40 BSS Coexistence Information field */

    /* 封装20/40 BSS Intolerant Channel Report element */
    /* 只有当STA检测到Trigger Event A时，才包含Operating Class，参见802.11n 10.15.12 */
    puc_payload_addr[index++] = MAC_EID_2040_INTOLCHREPORT; /* Element ID */
    us_ie_len_idx = index;
    puc_payload_addr[index++] = MAC_2040_INTOLCHREPORT_LEN_MIN; /* Length */
    puc_payload_addr[index++] = 0;                              /* Operating Class */
    if (chan_report > 0) {
        ...
        for (uc_chan_idx = 0; uc_chan_idx < uc_max_num_chan; uc_chan_idx++) {
            if (((chan_report >> uc_chan_idx) & BIT0) != 0) {
                ...
                puc_payload_addr[index++] = uc_channel_num;
...
uint8_t *mac_get_rx_payload(oal_netbuf_stru *netbuf, uint16_t *payload_len)
{
    ...
    if ((netbuf == NULL) || (payload_len == NULL)) {
        return NULL;
    }
    ...
    if (payload == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "mac_get_rx_payload:payload is null");
        return NULL;
    }
    *payload_len = mac_get_rx_payload_len(rx_ctl);
    return payload;
}