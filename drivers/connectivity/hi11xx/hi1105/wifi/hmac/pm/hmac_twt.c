...
/*
 * 函数说明 ：函数成功,出参是 twt_setup_req, hmac_vap, frame_len
 * 日     期  : 2020年7月6日
 * 作     者  : wifi
 * 修改内容 : 新生成函数
 */
...
/* 设置wlan ctx action相关参数 */
static uint32_t hmac_add_wlan_ctx_action_param(hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf, uint16_t frame_len)
{
    dmac_ctx_action_event_stru wlan_ctx_action;
    memset_s((uint8_t *)&wlan_ctx_action, sizeof(wlan_ctx_action), 0, sizeof(wlan_ctx_action));
    /* 赋值要传入Dmac的信息 */
    ...
    if (EOK != memcpy_s((uint8_t *)(oal_netbuf_data(netbuf) + frame_len),
        (WLAN_MEM_NETBUF_SIZE2 - frame_len), (uint8_t *)&wlan_ctx_action, sizeof(dmac_ctx_action_event_stru))) {
...
/* 设置CB参数 */
static void hmac_set_twt_teardown_cb(oal_netbuf_stru *twt_setup_req, uint16_t frame_len)
{
    mac_tx_ctl_stru *tx_ctl = NULL;
    memset_s(oal_netbuf_cb(twt_setup_req), oal_netbuf_cb_size(), 0, oal_netbuf_cb_size());
    tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(twt_setup_req);
...
uint32_t hmac_mgmt_tx_twt_teardown(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, mac_twt_action_mgmt_args_stru *twt_action)
{
    mac_vap_stru *mac_vap = NULL;
    ...
    if (oal_any_null_ptr3(hmac_vap, hmac_user, twt_action)) {
        oam_error_log0(0, OAM_SF_11AX, "{hmac_mgmt_tx_twt_teardown::hmac_vap or hmac_user or twt_actions is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (twt_setup_req == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11AX, "{hmac_mgmt_tx_twt_teardown::twt_teardown_req null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    oal_netbuf_prev(twt_setup_req) = NULL;
    oal_netbuf_next(twt_setup_req) = NULL;
    ...
    /* 调用封装管理帧接口 */
    us_frame_len = hmac_mgmt_encap_twt_teardown_req(hmac_vap, hmac_user, oal_netbuf_data(twt_setup_req), twt_action);
    if (hmac_add_wlan_ctx_action_param(hmac_user, twt_setup_req, us_frame_len) != OAL_SUCC) {
        oal_netbuf_free(twt_setup_req);
        return OAL_FAIL;
    }
    ...
    /* 初始化CB */
    hmac_set_twt_teardown_cb(twt_setup_req, us_frame_len);
    event_mem = frw_event_alloc_m(sizeof(dmac_tx_event_stru));
    if (event_mem == NULL) {
    ...
    if (frw_event_dispatch_event(event_mem) != OAL_SUCC) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11AX, "{hmac_mgmt_tx_twt_teardown:: failed.}");
        oal_netbuf_free(twt_setup_req);
        frw_event_free_m(event_mem);
        return OAL_FAIL;
    }
...
static uint8_t hmac_sta_rx_twt_req_rsp_support_check(mac_twt_command_enum_uint8 twt_command, uint8_t negotiation)
{
    /* 收到请求帧时，若responder能力没开，则不处理 */
    ...
    /* 收到单播响应帧时，若单播requester能力没开，则不处理 */
    if ((negotiation < MAC_TWT_NEGO_BROADCAST) &&
        (g_st_mac_device_custom_cfg.st_11ax_custom_cfg.bit_twt_requester_support == OAL_FALSE)) {
        return OAL_FALSE;
    }

    /* 收到广播响应帧时，若广播requester能力没开，则不处理 */
    if ((negotiation >= MAC_TWT_NEGO_BROADCAST) &&
        (g_st_mac_device_custom_cfg.st_11ax_custom_cfg.bit_btwt_requester_support == OAL_FALSE)) {
        return OAL_FALSE;
    }
    return OAL_TRUE;
}
    ...
    uint8_t *puc_payload)
{
    ...
    if (hmac_sta_rx_twt_req_rsp_support_check(twt_command, itwt_element->st_control.bit_negotiation) != OAL_TRUE) {
        oam_warning_log1(0, OAM_SF_11AX, "hmac_sta_rx_twt_setup_frame:: rx setup[%d] twt req/rsp not support",
            twt_command);
        return OAL_FAIL;
    }
...
OAL_STATIC void hmac_update_rx_twt_cfg_event_param(dmac_ctx_update_twt_stru *event, hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, uint8_t idx)
{
    event->en_action_category = MAC_ACTION_CATEGORY_S1G;
    event->uc_action = MAC_S1G_ACTION_TWT_TEARDOWN;
    event->us_user_idx = hmac_user->st_user_base_info.us_assoc_id;
    event->st_twt_cfg.ull_twt_start_time = hmac_vap->st_twt_cfg[idx].ull_twt_start_time;
    event->st_twt_cfg.ull_twt_interval = hmac_vap->st_twt_cfg[idx].ull_twt_interval;
    event->st_twt_cfg.twt_duration = hmac_vap->st_twt_cfg[idx].twt_duration;
    event->st_twt_cfg.uc_twt_flow_id = hmac_vap->st_twt_cfg[idx].uc_twt_flow_id;
    event->st_twt_cfg.uc_twt_announce_bit = hmac_vap->st_twt_cfg[idx].uc_twt_announce_bit;
    event->st_twt_cfg.uc_twt_is_trigger_enabled = hmac_vap->st_twt_cfg[idx].uc_twt_is_trigger_enabled;
    event->st_twt_cfg.uc_twt_session_enable = hmac_vap->st_twt_cfg[idx].uc_twt_session_enable;
    event->st_twt_cfg.uc_next_twt_size = 0; /* 只有twt information帧才有 */
    event->st_twt_cfg.twt_idx = idx;
}

OAL_STATIC void hmac_vap_twt_cfg_param_init(hmac_vap_stru *hmac_vap, mac_twt_teardown_stru *twt_teardown, uint8_t idx)
{
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_sta_rx_twt_teardown_frame(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *puc_payload)
{
    ...
    if (oal_any_null_ptr3(hmac_vap, hmac_user, puc_payload)) {
        oam_error_log0(0, OAM_SF_11AX, "{hmac_sta_rx_twt_teardown_frame::hmac_vap or hmac_user or payload is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (device == NULL) {
        oam_error_log0(0, OAM_SF_11AX, "{hmac_sta_rx_twt_teardown_frame::twt teardown receive failed, device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_11AX, "{hmac_sta_rx_twt_teardown_frame:: flow_id:%d, bit_nego_type:%d}",
        twt_teardown->bit_twt_flow_id, twt_teardown->bit_nego_type);

    /* 只支持删除单播TWT会话 */
    if (twt_teardown->bit_nego_type != 0) {
        oam_warning_log1(0, OAM_SF_11AX, "{not support twt negotiation type:%d}", twt_teardown->bit_nego_type);
        return OAL_FAIL;
    }

    /* 检查TWT是否在运行 */
    if (hmac_vap->st_twt_cfg[idx].uc_twt_session_enable == 0) {
        oam_warning_log0(0, OAM_SF_11AX, "{hmac_sta_rx_twt_teardown_frame:: twt session is already disabled!}");
        return OAL_FAIL;
    }

    /* 检查flow id */
    if (hmac_vap->st_twt_cfg[idx].uc_twt_flow_id != twt_teardown->bit_twt_flow_id) {
        oam_warning_log2(0, OAM_SF_11AX, "{the flow id [%d] is invalid,our flow id is [%d]}",
            twt_teardown->bit_twt_flow_id, hmac_vap->st_twt_cfg[idx].uc_twt_flow_id);
        return OAL_FAIL;
    }

    /*  To-Do, bit number不一样 */
    hmac_vap_twt_cfg_param_init(hmac_vap, twt_teardown, idx);

    /* 抛事件到DMAC处理 */
    event_mem = frw_event_alloc_m(sizeof(dmac_ctx_update_twt_stru));
    if (event_mem == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11AX, "{hmac_sta_rx_twt_teardown_frame::event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 填写事件头 */
    frw_event_hdr_init(&(hmac_to_dmac_crx_sync->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
        DMAC_WLAN_CTX_EVENT_SUB_TYPE_UPDATE_TWT, sizeof(dmac_ctx_update_twt_stru), FRW_EVENT_PIPELINE_STAGE_1,
        mac_vap->uc_chip_id, mac_vap->uc_device_id, mac_vap->uc_vap_id);

    /* 获取帧体信息，由于DMAC的同步，填写事件payload */
    update_twt_cfg_event = (dmac_ctx_update_twt_stru *)(hmac_to_dmac_crx_sync->auc_event_data);
    hmac_update_rx_twt_cfg_event_param(update_twt_cfg_event, hmac_vap, hmac_user, idx);
...
OAL_STATIC void hmac_sta_update_twt_cfg_event(hmac_vap_stru *hmac_vap, frw_event_stru *h2d_crx_sync,
    hmac_user_stru *hmac_user)
{
    dmac_ctx_update_twt_stru *event = NULL;
    event = (dmac_ctx_update_twt_stru *)(h2d_crx_sync->auc_event_data);
    event->en_action_category = MAC_ACTION_CATEGORY_S1G;
    event->uc_action = MAC_S1G_ACTION_TWT_INFORMATION;
    event->us_user_idx = hmac_user->st_user_base_info.us_assoc_id;
    event->st_twt_cfg.ull_twt_start_time = hmac_vap->st_twt_cfg[0].ull_twt_start_time;
    event->st_twt_cfg.ull_twt_interval = hmac_vap->st_twt_cfg[0].ull_twt_interval;
    event->st_twt_cfg.twt_duration = hmac_vap->st_twt_cfg[0].twt_duration;
    event->st_twt_cfg.uc_twt_flow_id = hmac_vap->st_twt_cfg[0].uc_twt_flow_id;
    event->st_twt_cfg.uc_twt_announce_bit = hmac_vap->st_twt_cfg[0].uc_twt_announce_bit;
    event->st_twt_cfg.uc_twt_is_trigger_enabled = hmac_vap->st_twt_cfg[0].uc_twt_is_trigger_enabled;
    event->st_twt_cfg.uc_twt_session_enable = hmac_vap->st_twt_cfg[0].uc_twt_session_enable;
    event->st_twt_cfg.uc_next_twt_size = hmac_vap->st_twt_cfg[0].uc_next_twt_size;
}
/*
 * 1.日    期  : 2019年06月27日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_sta_rx_twt_information_frame(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *payload)
{
    mac_vap_stru *mac_vap = NULL;
    mac_twt_information_frame_stru *frame_body = NULL;
    frw_event_mem_stru *event_mem = NULL; /* 申请事件返回的内存指针 */
    ...
    if (oal_any_null_ptr3(hmac_vap, hmac_user, payload)) {
        oam_error_log0(0, OAM_SF_11AX, "{hmac_sta_rx_twt_information_frame::hmac_vap or hmac_user or payload is null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /*                                                                */
    /******************************************************************/
    frame_body = (mac_twt_information_frame_stru *)(payload);

    oam_warning_log2(0, OAM_SF_11AX, "{hmac_sta_rx_twt_information_frame:: flow id:%d, next twt subfield size:%d}",
        frame_body->st_twt_information_filed.bit_twt_flow_id,
        frame_body->st_twt_information_filed.bit_next_twt_subfield_size);

    /* 检查TWT是否在运行 */
    if (hmac_vap->st_twt_cfg[0].uc_twt_session_enable == 0) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_11AX, "{twt session is disabled!}");
        return OAL_FAIL;
    }
    ...
    if (hmac_vap->st_twt_cfg[0].uc_twt_flow_id !=
        frame_body->st_twt_information_filed.bit_twt_flow_id) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_11AX, "{the flow id [%d] is invalid,our flow id is [%d]}",
            frame_body->st_twt_information_filed.bit_twt_flow_id, hmac_vap->st_twt_cfg[0].uc_twt_flow_id);
        return OAL_FAIL;
    }

    next_size = frame_body->st_twt_information_filed.bit_next_twt_subfield_size;
    payload += sizeof(mac_twt_information_frame_stru);

    next_twt = hmac_get_next_twt(hmac_vap, payload, next_size);
    /* 不支持flex twt且all_twt=0不处理        */
    if (next_twt != 0 && frame_body->st_twt_information_filed.bit_all_twt == 0 &&
        g_st_mac_device_custom_cfg.st_11ax_custom_cfg.bit_twt_requester_support == OAL_TRUE) {
        return OAL_SUCC;
    }

    /*  这里只更新twt start time和next twt size */
    hmac_vap->st_twt_cfg[0].ull_twt_start_time = next_twt;
    hmac_vap->st_twt_cfg[0].uc_next_twt_size = next_size;
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_11AX, "{next_twt=[%ld], subfield_size=[%d]}", next_twt, next_size);
    ...
    /* 填写事件头 */
    frw_event_hdr_init(&(h2d_crx_sync->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
        DMAC_WLAN_CTX_EVENT_SUB_TYPE_UPDATE_TWT, sizeof(dmac_ctx_update_twt_stru), FRW_EVENT_PIPELINE_STAGE_1,
        mac_vap->uc_chip_id, mac_vap->uc_device_id, mac_vap->uc_vap_id);

    /* 获取帧体信息，由于DMAC的同步，填写事件payload */
    hmac_sta_update_twt_cfg_event(hmac_vap, h2d_crx_sync, hmac_user);
...
OAL_STATIC uint32_t hmac_check_twt_setup_req_conditions(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    mac_cfg_twt_setup_req_param_stru *twt_setup_req)
{
    mac_he_hdl_stru he_hdl;
...
uint32_t hmac_config_twt_setup_req(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    hmac_user = mac_vap_get_hmac_user_by_addr(mac_vap, twt_setup_req->auc_mac_addr, WLAN_MAC_ADDR_LEN);
    if (hmac_user == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    /* 检查发送twt 请求需要满足的条件 */
    ret = hmac_check_twt_setup_req_conditions(mac_vap, hmac_user, twt_setup_req);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
uint32_t hmac_config_twt_teardown_req(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    hmac_user = mac_vap_get_hmac_user_by_addr(mac_vap, twt_teardown_req->auc_mac_addr, WLAN_MAC_ADDR_LEN);
    if (hmac_user == NULL) {
...
/*
 * 功能描述  : process_twt_sync_event发送适配
 * 1.日    期  : 2018年12月11日
  *   修改内容  : 新生成函数
 */
...
void hmac_sta_rx_btwt(uint8_t *payload, uint16_t msg_len, hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    if ((btwt_element == NULL) || (sizeof(mac_btwt_ie_stru) != (btwt_element[1] + MAC_IE_HDR_LEN))) {
        return;
    }
