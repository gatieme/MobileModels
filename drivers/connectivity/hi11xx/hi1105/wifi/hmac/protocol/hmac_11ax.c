...
#define MAC_HE_MAX_STS_ABILITY 3
...
static const mac_chan_band_offset_stru g_chan_center_freq_offset[] = {
    /* 为了提高兼容性，40M及其以下都填0 */
    { WLAN_BAND_WIDTH_20M,                 0,   0,  0 },
    { WLAN_BAND_WIDTH_40PLUS,              0,   0,  0 },
    { WLAN_BAND_WIDTH_40MINUS,             0,   0,  0 },
    /*
     * | 主20 | 从20 | 从40       |
     *               |
     *               |中心频率相对于主20偏6个信道
     */
    { WLAN_BAND_WIDTH_80PLUSPLUS,          6,   0,  0 },
     ...
     *               |中心频率相对于主20偏-2个信道
     */
    { WLAN_BAND_WIDTH_80PLUSMINUS,        -2,   0,  0 },
     ...
     *               |中心频率相对于主20偏2个信道
     */
    { WLAN_BAND_WIDTH_80MINUSPLUS,         2,   0,  0 },
     ...
     *               |中心频率相对于主20偏-6个信道
     */
    { WLAN_BAND_WIDTH_80MINUSMINUS,       -6,   0,  0 },
#ifdef _PRE_WLAN_FEATURE_160M
    /* 从20信道+1, 从40信道+1, 从80信道+1 */
    { WLAN_BAND_WIDTH_160PLUSPLUSPLUS,     6,  14,  0 },

    /* 从20信道+1, 从40信道+1, 从80信道-1 */
    { WLAN_BAND_WIDTH_160PLUSPLUSMINUS,    6,  -2,  0 },

    /* 从20信道+1, 从40信道-1, 从80信道+1 */
    { WLAN_BAND_WIDTH_160PLUSMINUSPLUS,   -2,   6,  0 },

    /* 从20信道+1, 从40信道-1, 从80信道-1 */
    { WLAN_BAND_WIDTH_160PLUSMINUSMINUS,  -2, -10,  0 },

    /* 从20信道-1, 从40信道+1, 从80信道+1 */
    { WLAN_BAND_WIDTH_160MINUSPLUSPLUS,    2,  10,  0 },

    /* 从20信道-1, 从40信道+1, 从80信道-1 */
    { WLAN_BAND_WIDTH_160MINUSPLUSMINUS,   2,   6,  0 },

    /* 从20信道-1, 从40信道-1, 从80信道+1 */
    { WLAN_BAND_WIDTH_160MINUSMINUSPLUS,  -6,   2,  0 },

    /* 从20信道-1, 从40信道-1, 从80信道-1 */
    { WLAN_BAND_WIDTH_160MINUSMINUSMINUS, -6, -14,  0 },
#endif
};
#endif
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_update_he_opern_ie_sta(mac_vap_stru *pst_mac_vap,
...
/*
 * 函 数 名  : hmac_proc_he_6ghz_band_cap_ie
 * 功能描述  : STA更新接收asoc_rsp帧中的he 6ghz band cap ie信息
 * 1.日    期  : 2022年11月2日
 *   修改内容  : 新生成函数
 */
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
uint32_t hmac_proc_he_6ghz_band_cap_ie(mac_vap_stru *pst_mac_vap,
    hmac_user_stru *hmac_user, uint8_t *payload)
{
    mac_he_6ghz_band_cap_info_stru he_6ghz_band_cap_ie_value;
    uint32_t ret;

    if (oal_any_null_ptr3(pst_mac_vap, hmac_user, payload)) {
        oam_error_log0(0, OAM_SF_ASSOC,
            "{hmac_proc_he_6ghz_band_cap_ie::ptr is null!}");
        return MAC_NO_CHANGE;
    }

    /* 支持11ax，才进行后续的处理 */
    if (mac_mib_get_HEOptionImplemented(pst_mac_vap) == OAL_FALSE) {
        return MAC_NO_CHANGE;
    }

    memset_s(&he_6ghz_band_cap_ie_value, sizeof(he_6ghz_band_cap_ie_value), 0, sizeof(he_6ghz_band_cap_ie_value));
    ret = mac_ie_parse_he_6ghz_band_cap(payload, &he_6ghz_band_cap_ie_value);
    /* 若解析HE 6ghz band cap IE失败, 也返回OAL_SUCC, 不影响后续IE解析 */
    if (ret != OAL_SUCC) {
        return OAL_SUCC;
    }

    if (memcpy_s(&(hmac_user->st_user_base_info.he_6ghz_band_cap_ie), sizeof(mac_he_6ghz_band_cap_info_stru),
        (uint8_t*)(&he_6ghz_band_cap_ie_value), sizeof(mac_he_6ghz_band_cap_info_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_proc_he_6ghz_band_cap_ie::memcpy fail!");
        return OAL_FAIL;
    }
...
/*
 * 函 数 名  : hmac_sta_up_update_he_ndp_feedback_report_params
 * 功能描述  : STA接收到beacon帧或者关联响应帧更新自身的ndp feedback report参数
 * 1.日    期  : 2018年1月31日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
                                                 ...
                                                 mac_wmm_set_param_type_enum_uint8 en_type)
{
    ...
    if (memcpy_s(pst_edca_para, sizeof(dmac_ctx_sta_asoc_set_edca_reg_stru),
        (uint8_t *)&st_asoc_set_edca_reg_param, sizeof(dmac_ctx_sta_asoc_set_edca_reg_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ASSOC, "hmac_sta_up_update_mu_edca_params_machw::memcpy fail!");
...
/*
 * 函 数 名  : hmac_sta_up_update_assoc_rsp_sr_params
 * 功能描述  : STA接收到assoc帧或者关联响应帧更新自身的sr参数
 * 1.日    期  : 2017年5月25日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
uint32_t hmac_ie_check_he_sta(hmac_vap_stru *pst_hmac_sta,
    hmac_user_stru *pst_hmac_user, uint8_t *puc_payload, uint16_t us_msg_len)
{
    ...
    if (pst_mac_vap->st_channel.ext6g_band == OAL_TRUE) {
        puc_tmp_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_6GHZ_BAND_CAP, puc_payload, us_msg_len);
        if (puc_tmp_ie != NULL) {
            hmac_proc_he_6ghz_band_cap_ie(pst_mac_vap, pst_hmac_user, puc_tmp_ie);
        }
    }
#endif
...
void hmac_mgmt_proc_he_uora(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr_info)
{
    if ((g_wlan_spec_cfg->feature_11ax_uora_is_supported == OAL_TRUE) && is_legacy_sta(mac_vap)) {
        if (bss_dscr_info->en_he_uora == OAL_TRUE) {
...
/*
 * 函 数 名  : hmac_set_he_capabilities_punctured_preamble
 * 功能描述  : 填充HE PHY Capabilities Information域信息中的punctured_preamble字段
 * 1.日    期  : 2020年6月16日
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : hmac_set_he_capabilities_all_ie
 * 功能描述  : 填充全部he capabilities信息
 * 1.日    期  : 2020年6月16日
 *   修改内容  : 新生成函数
 */
    ...
    uint8_t *buffer)
{
    ...
    wlan_chip_need_decrease_sts(mac_vap, buffer, scaned_bss);
...
/*
 * 函 数 名  : hmac_config_set_send_tb_ppdu_flag
 * 功能描述  : 发送过tb 帧之后关闭htc 的arp_probe
 * 1.日    期  : 2019年12月14日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名   : mac_hiex_ie_get_bit_chip_type
 * 功能描述   : 获取vender ie的bit_chip_type参数
 * 1.日    期 : 2020年06月16日
 *   修改内容 : 新生成函数
 */
...
oal_bool_enum_uint8 mac_hiex_ie_get_bit_chip_type(uint8_t *frame_body, uint16_t us_frame_len)
{
    ...
    ie = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_HISI_HIEX_IE, frame_body, (uint32_t)us_frame_len);
    if ((ie == NULL) || (ie[1] + MAC_IE_HDR_LEN < sizeof(mac_hiex_vendor_ie_stru))) {
        return OAL_FALSE;
    }
...
/*
 * 函 数 名  : hmac_scan_update_bss_list_11ax_vht_bw
 * 功能描述  : 计算he opern IE中vht opern info字段的带宽能力
 * 1.日    期  : 2022年12月2日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint8_t hmac_scan_update_bss_list_11ax_vht_bw(mac_bss_dscr_stru *bss_dscr,
                                                         mac_frame_he_oper_ie_stru *he_oper_ie_value)
{
    int8_t chan_off;
    uint8_t vht_bandwidth = WLAN_BAND_WIDTH_BUTT;

    chan_off = (int8_t)(he_oper_ie_value->st_vht_operation_info.uc_center_freq_seg0 -
    ...
    if (he_oper_ie_value->st_vht_operation_info.uc_channel_width == WLAN_MIB_VHT_OP_WIDTH_80) { /* 80MHz */
        vht_bandwidth = hmac_scan_get_bandwidth(g_channel_bandwidth_map_80mhz,
    ...
    if (he_oper_ie_value->st_vht_operation_info.uc_channel_width == WLAN_MIB_VHT_OP_WIDTH_160) { /* 160MHz */
        vht_bandwidth = hmac_scan_get_bandwidth(g_channel_bandwidth_map_160mhz,
...
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
/*
 * 函 数 名  : hmac_scan_update_bss_list_11ax_6ghz_bw
 * 功能描述  : 计算he opern IE中6ghz opern info字段的带宽能力
 * 1.日    期  : 2022年12月2日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint8_t hmac_scan_update_bss_list_11ax_6ghz_bw(mac_bss_dscr_stru *bss_dscr,
    mac_frame_he_6ghz_oper_info_stru *he_6ghz_oper_info)
{
    int8_t chan_off;
    uint8_t he_6ghz_bandwidth = WLAN_BAND_WIDTH_BUTT;

    chan_off = (int8_t)(he_6ghz_oper_info->center_freq_seg0 - bss_dscr->st_channel.uc_chan_number);
    if (he_6ghz_oper_info->bit_channel_width == WLAN_MIB_HE_6GHZ_OP_WIDTH_80) { /* 80MHz */
        he_6ghz_bandwidth = hmac_scan_get_bandwidth(g_channel_bandwidth_map_80mhz,
            sizeof(g_channel_bandwidth_map_80mhz) / sizeof(hmac_channel_bandwidth_map), chan_off);
    }
#ifdef _PRE_WLAN_FEATURE_160M
    if (he_6ghz_oper_info->bit_channel_width == WLAN_MIB_HE_6GHZ_OP_WIDTH_80PLUS80_160) { /* 160MHz */
        he_6ghz_bandwidth = hmac_scan_get_bandwidth(g_channel_bandwidth_map_160mhz,
            sizeof(g_channel_bandwidth_map_160mhz) / sizeof(hmac_channel_bandwidth_map), chan_off);
    }
#endif

    return he_6ghz_bandwidth;
}
#endif

/*
 * 函 数 名  : hmac_scan_update_bss_list_11ax
 * 功能描述  : 更新11ax相关信息
 * 1.日    期  : 2017年5月25日
 *   作    者  : 修改内容
 *   修改内容  : 新生成函数
...
OAL_STATIC void hmac_scan_update_bss_list_11ax(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (he_oper_ie_value.st_he_oper_param.bit_vht_operation_info_present == OAL_TRUE) {
        bandwidth = hmac_scan_update_bss_list_11ax_vht_bw(bss_dscr, &he_oper_ie_value);
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    } else if (he_oper_ie_value.st_he_oper_param.bit_6g_oper_info_present == OAL_TRUE) {
        /* 6G时从he opern IE的6ghz opern info字段解析带宽能力 */
        memset_s(&he_6ghz_oper_info, sizeof(mac_frame_he_6ghz_oper_info_stru),
            0, sizeof(mac_frame_he_6ghz_oper_info_stru));
        if (mac_ie_proc_he_6ghz_opern_info(ie, &he_6ghz_oper_info, &he_oper_ie_value) == OAL_SUCC) {
            bandwidth = hmac_scan_update_bss_list_11ax_6ghz_bw(bss_dscr, &he_6ghz_oper_info);
        }
#endif
    } else {
        return;
...
/*
 * 函 数 名  : hmac_scan_update_11ax_ie
 * 功能描述  : 更新11ax相关ie信息
 * 1.日    期  : 2019年10月12日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_scan_get_non_transmitted_bssid
 * 功能描述  : 计算non-transmitted bssid
 * 1.日    期  : 2019年4月1日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
{
    int32_t ret;
    hal_rx_ctl_stru *transmitted_rx_ctrl = NULL;
    hal_rx_ctl_stru *new_rx_ctrl = NULL;

    transmitted_rx_ctrl = (hal_rx_ctl_stru *)oal_netbuf_cb(netbuf);
    /* copy cb 字段 */
    new_rx_ctrl = (hal_rx_ctl_stru *)oal_netbuf_cb(new_netbuf);
    ret = memcpy_s(new_rx_ctrl, sizeof(hal_rx_ctl_stru), transmitted_rx_ctrl, sizeof(hal_rx_ctl_stru));
    if (ret != EOK) {
...
OAL_STATIC oal_bool_enum_uint8 hmac_scan_proc_buf_len_is_vaild(uint16_t netbuf_len, uint16_t before_ssid_ie_len,
    mac_multi_bssid_frame_info_stru *frame_info)
{
    uint16_t use_buf_len = before_ssid_ie_len;

    /* 检查长度是否足够 */
    if (netbuf_len < use_buf_len) {
        oam_warning_log2(0, OAM_SF_SCAN, "{hmac_scan_proc_check_buf_len::use_buf_len=[%d] over netbuf_len=[%d]}",
            use_buf_len, netbuf_len);
        return OAL_FALSE;
    }
...
OAL_STATIC oal_netbuf_stru *hmac_scan_proc_copy_netbuf(oal_netbuf_stru *netbuf, oal_netbuf_stru *new_netbuf,
    uint16_t *new_buf_len, uint16_t before_ssid_ie_len, uint8_t *after_ssid_ie,
    uint16_t after_ssid_ie_len, mac_multi_bssid_frame_info_stru *frame_info)
{
    ...
    if (memcpy_s(netbuf_data, frame_info->uc_non_transmitted_ssid_len,
        frame_info->auc_non_transmitted_ssid, frame_info->uc_non_transmitted_ssid_len) != EOK) {
    ...
    ret = hmac_scan_proc_copy_ssid_ie_after(netbuf_data, *new_buf_len, after_ssid_ie, after_ssid_ie_len, use_buf_len);
    if (ret != OAL_SUCC) {
        return NULL;
    }
    return new_netbuf;
...
/*
 * 功能描述  : 对于non-transmitted bssid 重新申请netbuf，替换bssid 和ssid 参数
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
{
    uint8_t *mgmt_frame = (uint8_t *)oal_netbuf_data(netbuf);
    uint8_t *mgmt_frame_body = mgmt_frame + MAC_80211_FRAME_LEN;
    uint8_t *transmitted_ssid = NULL;
    uint8_t *after_ssid_ie = NULL;
    oal_netbuf_stru *new_netbuf = NULL;
    uint8_t transmitted_ssid_len = 0;
    uint16_t mgmt_frame_body_len = buf_len - wlan_chip_get_scaned_payload_extend_len() - MAC_80211_FRAME_LEN;
    uint16_t before_ssid_ie_len, after_ssid_ie_len;

    /* 获取transimitted 帧中ssid 长度 */
    transmitted_ssid = mac_get_ssid(mgmt_frame_body, (int32_t)mgmt_frame_body_len, &transmitted_ssid_len);
    if ((transmitted_ssid == NULL) || (transmitted_ssid != NULL && transmitted_ssid_len == 0)) {
        return NULL;
    }

    /* 计算ssid ie 之前的帧长 */
    before_ssid_ie_len = transmitted_ssid - MAC_IE_HDR_LEN - mgmt_frame;

    /* 计算ssid ie 之后帧长 */
    after_ssid_ie = transmitted_ssid + transmitted_ssid_len;
    after_ssid_ie_len = buf_len - (after_ssid_ie - mgmt_frame);

    /* 需要重新申请netbuf 内存 */
    *new_buf_len = buf_len - transmitted_ssid_len + frame_info->uc_non_transmitted_ssid_len;
    if (hmac_scan_proc_buf_len_is_vaild(*new_buf_len, before_ssid_ie_len, frame_info) != OAL_TRUE) {
        return NULL;
    }
    ...
    if (hmac_scan_proc_copy_netbuf(netbuf, new_netbuf, new_buf_len,
        before_ssid_ie_len, after_ssid_ie, after_ssid_ie_len, frame_info) == NULL) {
    ...
    uint16_t left_len, mac_ieee80211_frame_stru *frame_header)
{
    ...
    if (mac_ie_parse_multi_bssid_opt_subie(ie_data, left_len,
...
/*
 * 函 数 名  : hmac_scan_proc_mbssid_scanned_bss
 * 功能描述  : 处理含有multi_bssid Ie 的 扫描结果
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_scan_proc_multi_bssid_scanned_bss(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, uint16_t buf_len)
{
    ...
    uint16_t frame_body_len, left_len;
    mac_multi_bssid_frame_info_stru frame_info;
    uint8_t *mbssid_ie = NULL;
    oal_netbuf_stru *new_netbuf = NULL;
    uint8_t mbssid_ie_len, mgmt_filed_len;
    ...
    if (buf_len < (MAC_80211_FRAME_LEN + wlan_chip_get_scaned_payload_extend_len())) {
        oam_warning_log1(0, OAM_SF_SCAN, "hmac_scan_proc_multi_bssid_scanned_bss::buf_len[%u]", buf_len);
        return OAL_FAIL;
    }
    ...
    if (mbssid_ie == NULL || (mbssid_ie[1] < 1)) {
        return OAL_FAIL;
    }
...
/*****************************************************************************
 函 数 名  : mac_device_trans_bandwith_to_he_capinfo
 功能描述  : 最大支持带宽换到he capability info中的带宽
 输入参数  : device id
 输出参数  : 无
 返 回 值  : TRUE或FALSE
  1.日    期   : 2016年11月30日
...
OAL_STATIC void mac_set_he_mac_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***************************************************************************/
    ...
    if ((uint8_t)wlan_chip_get_11ax_switch_mask() & BIT5) {
        pst_he_mac_capinfo->bit_twt_requester_support = mac_mib_get_he_TWTOptionActivated(mac_vap);
    } else {
        pst_he_mac_capinfo->bit_twt_requester_support = 0;
    }
 ...
 * 功能描述  : 填写he phy capabilities info域,解决行数问题
 * 1.日    期  : 2017年5月26日
 *   修改内容  : 新生成函数
 */
    ...
    mac_frame_he_phy_cap_stru *pst_he_phy_capinfo)
{
    ...
    /* B63:STBC Rx>80MHz 确认HE160M模式rx stbc有收益,mp16及以后项目打开  */
    pst_he_phy_capinfo->bit_stbc_rx_over_80mhz = g_wlan_spec_cfg->rx_stbc_160;
...
/*
 * 功能描述  : 填写he phy sounding信息,解决行数问题
 */
OAL_STATIC void mac_set_he_sounding_bfmer_sts_field(mac_vap_stru *pst_mac_vap,
    mac_frame_he_phy_cap_stru *pst_he_phy_capinfo)
{
...
/*
 * 功能描述  : 降低sts流数,解决对于多天线场景下txbf负收益
 */
void hmac_decrease_below_80mhz_max_sts_ability(mac_vap_stru *pst_mac_vap, uint8_t *puc_buffer,
    hmac_scanned_bss_info *scaned_bss)
{
    ...
    puc_buffer += MAC_IE_HDR_LEN + 1 + MAC_HE_MAC_CAP_LEN;
    pst_he_phy_capinfo = (mac_frame_he_phy_cap_stru *)puc_buffer;
    sts_num = pst_he_phy_capinfo->bit_beamformee_sts_below_80mhz;
    bandwidth = oal_min(scaned_bss->st_bss_dscr_info.en_bw_cap, mac_mib_get_dot11VapMaxBandWidth(pst_mac_vap));
    if (bandwidth == WLAN_BW_CAP_20M) {
        sts_num = oal_min(sts_num, MAC_HE_MAX_STS_ABILITY);
    }
    pst_he_phy_capinfo->bit_beamformee_sts_below_80mhz = sts_num;
    return;
}
/*
 * 功能描述  : 填写he phy sounding信息,解决行数问题
 */
OAL_STATIC void mac_set_he_sounding_bfmee_sts_field(mac_vap_stru *pst_mac_vap,
    mac_frame_he_phy_cap_stru *pst_he_phy_capinfo)
{
    mac_user_stru *pst_mac_user = NULL;
    mac_he_hdl_stru *pst_he_hdl = NULL;
    uint8_t uc_below80mhz, uc_over80mhz;
    ...
    if ((uc_below80mhz >= 1) && (uc_over80mhz >= 1)) {
        /* B34-B36:Beamformee STS ≤ 80MHz,最小值为3， mp13支持4*2 mp15支持8*2 */
        pst_he_phy_capinfo->bit_beamformee_sts_below_80mhz = oal_max(uc_below80mhz - 1, 3);
        /* B37-B39:Beamformee STS > 80MHz,最小值为3， mp13支持4*2 mp15支持8*2 */
        pst_he_phy_capinfo->bit_beamformee_sts_over_80mhz = oal_max(uc_over80mhz - 1, 3);
    }
    if (g_wlan_spec_cfg->feature_txbf_mib_study == OAL_FALSE) { /* 不学习对端 */
        return;
    }
    ...
    if (pst_mac_vap->st_channel.en_band == WLAN_BAND_2G) {
        pst_he_phy_capinfo->bit_beamformee_sts_over_80mhz = 0;
    }
}

/*
 * 函 数 名  : mac_set_he_sounding_capinfo_field
 * 功能描述  : 填写he phy sounding信息,解决行数问题
 * 1.日    期  : 2017年5月26日
 *   修改内容  : 新生成函数
 */
    ...
    mac_frame_he_phy_cap_stru *pst_he_phy_capinfo)
{
    ...
    mac_set_he_sounding_bfmer_sts_field(pst_mac_vap, pst_he_phy_capinfo); // 封装函数降低行数
    ...
    /*  set disable */
    pst_he_phy_capinfo->bit_trigger_su_beamforming_feedback = g_wlan_spec_cfg->trigger_su_beamforming_feedback;

    /* B51:trigger_mu_beamforming_partialBW_feedback */
    pst_he_phy_capinfo->bit_trigger_mu_beamforming_partialbw_feedback = g_wlan_spec_cfg->trigger_mu_partialbw_feedback;
...
/*
 * 函 数 名  : mac_set_he_channel_width
 * 功能描述  : 填写he phy channel width字段
 * 1.日    期  : 2019年11月14日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
{
    /* ，aput 2g时，对5g的带宽不进行置位，只根据是否支持2.4g 40M对Bit0进行置位 */
...
OAL_STATIC void mac_set_he_phy_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ************************************************************************************************/
    ...
    /* B19:STBC Rx<=80MHz */
    if (is_sta(mac_vap)) {
        /* 对端设备不支持stbc时以对方的能力进行配置, 避免出现兼容性问题 */
    ...
    if (is_sta(mac_vap)) {
        pst_he_phy_capinfo->bit_full_bandwidth_ul_mu_mimo = g_wlan_spec_cfg->full_bandwidth_ul_mu_mimo;
    }
...
/*
 * 函 数 名  : mac_set_tx_rx_he_mcs
 * 功能描述  : 在各种带宽模式下，填写tx或者rx的he phy capabilities info域
 * 1.日    期  : 2017年5月26日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
    ...
    uint8_t channel_width_b3, uint8_t *buffer)
{
    ...
    **************************************************************************************************/
    ...
    if (channel_width_b3 != 0) {
        /* TRx HE-MCS Map 80+80 MHz: mp15暂不支持 */
    }
...
/*
 * 函 数 名  : mac_set_he_capabilities_ie
...
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
/*
 * 函 数 名  : mac_set_he_operation_ie_6ghz
 * 功能描述  : 6G时填充he opern IE中的6ghz opern info字段
 * 1.日    期  : 2022年12月2日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC void mac_set_he_operation_ie_6ghz(mac_vap_stru *mac_vap,
    mac_frame_he_6ghz_oper_info_stru *he_6ghz_oper_info)
{
    uint8_t idx;

    he_6ghz_oper_info->primary_channel = mac_vap->st_channel.uc_chan_number;
    he_6ghz_oper_info->bit_duplicate_bcn = 0;
    he_6ghz_oper_info->minimum_rate = 0;

    if (mac_vap->st_channel.en_bandwidth >= WLAN_BAND_WIDTH_40PLUS &&
        mac_vap->st_channel.en_bandwidth <= WLAN_BAND_WIDTH_40MINUS) {
        he_6ghz_oper_info->bit_channel_width = WLAN_MIB_HE_6GHZ_OP_WIDTH_40;
    } else if (mac_vap->st_channel.en_bandwidth >= WLAN_BAND_WIDTH_80PLUSPLUS &&
        mac_vap->st_channel.en_bandwidth <= WLAN_BAND_WIDTH_80MINUSMINUS) {
        he_6ghz_oper_info->bit_channel_width = WLAN_MIB_HE_6GHZ_OP_WIDTH_80;
#ifdef _PRE_WLAN_FEATURE_160M
        ...
        he_6ghz_oper_info->bit_channel_width = WLAN_MIB_HE_6GHZ_OP_WIDTH_80PLUS80_160;
#endif
    } else {
        he_6ghz_oper_info->bit_channel_width = WLAN_MIB_HE_6GHZ_OP_WIDTH_20;
    }

    /* 查表 */
    for (idx = 0; idx < oal_array_size(g_chan_center_freq_offset); idx++) {
        if (g_chan_center_freq_offset[idx].bandwidth == mac_vap->st_channel.en_bandwidth) {
            he_6ghz_oper_info->center_freq_seg0 = (uint8_t)(mac_vap->st_channel.uc_chan_number +
                g_chan_center_freq_offset[idx].freq_seg0_offset);
            he_6ghz_oper_info->center_freq_seg1 = (uint8_t)(mac_vap->st_channel.uc_chan_number +
                g_chan_center_freq_offset[idx].freq_seg1_offset);

            /* 如果查到，则做出相应处理，并退出循环 */
            break;
        }
    }
    /* 如果没查到则说明表格需要更新，与带宽枚举保持一致 */
    if (idx == oal_array_size(g_chan_center_freq_offset)) {
        he_6ghz_oper_info->center_freq_seg0 = 0;
        he_6ghz_oper_info->center_freq_seg1 = 0;
    }
    /* 11ax协议规定，6G带宽<=80M时，center_freq_seg1固定填0 */
    if (mac_vap->st_channel.en_bandwidth <= WLAN_BAND_WIDTH_80MINUSMINUS) {
        he_6ghz_oper_info->center_freq_seg1 = 0;
    }
}
#endif
...
/*
 * 函 数 名  : mac_set_he_opern_param
 * 功能描述  : 填充he operation param子字段信息
 */
OAL_STATIC void mac_set_he_opern_param(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    mac_frame_he_operation_param_stru *he_opern_param = (mac_frame_he_operation_param_stru *)puc_buffer;
    ...
    ***************************************************************************/
    ...
    he_opern_param->bit_default_pe_duration = MAC_HE_DEFAULT_PE_DURATION;

    /* B3:TWT Required */
    he_opern_param->bit_twt_required = 0;

    /* B4-B13:TXOP Duration RTS Threshold, 1023表示该字段不可用 */
    he_opern_param->bit_he_duration_rts_threshold = MAC_HE_DURATION_RTS_THRESHOLD;

    /* B14:VHT Operation Info. Present */
    /* 由于beacon和probe rsp中一定携带VHT信息，故此位置0 */
    he_opern_param->bit_vht_operation_info_present = 0;

    /* B15:Co-Hosted BSS */
    he_opern_param->bit_co_located_bss = 0;

    /* B16:ER SU Disable的取值，0表示支持接收242-tone HE ER SU PPDU */
    he_opern_param->bit_er_su_disable = 0;

    /* B17:6 GHz Operation Info. Present的取值, 不支持6GHz band */
    /* 此位置0,表示HE Operation中6GHz Operation Info字段不存在 */
    he_opern_param->bit_6g_oper_info_present = (mac_vap->st_channel.ext6g_band == OAL_TRUE) ? 1 : 0;

    /* B18-B23保留 */
    he_opern_param->bit_reserved = 0;
}

/*
 * 函 数 名  : mac_set_he_operation_ie
 * 功能描述  : 填充he operation信息
 * 1.日    期  : 2019年07月04日
 *   修改内容  : 新生成函数
 */
void mac_set_he_operation_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
...
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    mac_frame_he_6ghz_oper_info_stru     *he_6ghz_opern_info = NULL;
#endif
    *puc_ie_len = 0;
    ...
    *puc_ie_len   += 1;
    ...
    /* 填充he operation param子字段 */
    mac_set_he_opern_param(mac_vap, puc_buffer);
    ...
    *puc_ie_len   += MAC_HE_OPE_BASIC_MCS_NSS_LEN;
    ...
    if (mac_vap->st_channel.ext6g_band == OAL_TRUE) {
        he_6ghz_opern_info = (mac_frame_he_6ghz_oper_info_stru *)puc_buffer;
        memset_s(he_6ghz_opern_info, sizeof(mac_frame_he_6ghz_oper_info_stru),
            0, sizeof(mac_frame_he_6ghz_oper_info_stru));

        mac_set_he_operation_ie_6ghz(mac_vap, he_6ghz_opern_info);
        /* 填充6ghz operation Information字段 */
        *puc_ie_len += MAC_HE_6GHZ_OPERATION_INFO_LEN;
    }
#endif
...
/*
 * 函 数 名  : mac_get_htc_uph_om_value
 * 功能描述  : 获取uph+om 的htc value
 * 1.日    期  : 2019年5月26日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : mac_get_htc_om_value
 * 功能描述  : 获取 om 的htc value;对于ap 和p2p 仅含有om字段
 * 1.日    期  : 2019年5月26日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 函 数 名  : mac_set_he_ie_in_assoc_rsp
 * 功能描述  : 在assoc rsp中设置HE Capabilities和HE Operationg IE
 * 1.日    期  : 2019年07月20日
 *   修改内容  : 新生成函数
 */
...
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
        if (mac_vap->st_channel.ext6g_band == OAL_TRUE) {
            *puc_ie_len += mac_set_he_6ghz_band_cap_ie(mac_vap, puc_asoc_rsp);
        }
#endif
    }
}
...
/*
 * 函 数 名  : hmac_sta_update_join_multi_bssid_info
 * 功能描述  : 入网时根据ap信息更新mib 能力
 * 1.日    期  : 2019年4月9日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 功能描述  : sta/gc 入网时，是关联在he协议模式下
               sta 时:ap 在wifi6 黑名单，禁止关联在he协议
               gc: gc 入网导致dbac，禁止启动在dbac
 * 1.日    期  : 2020年4月9日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
#ifdef _PRE_WLAN_CHBA_MGMT
    mac_dev = mac_res_get_dev(p_mac_vap->uc_device_id);
    if (p_bss_dscr->en_p2p_scenes ==  MAC_P2P_SCENES_LOW_LATENCY || mac_is_secondary_sta(p_mac_vap) ||
        mac_vap_is_find_chba_vap(mac_dev) == OAL_TRUE) {
#else
    if (p_bss_dscr->en_p2p_scenes ==  MAC_P2P_SCENES_LOW_LATENCY || mac_is_secondary_sta(p_mac_vap)) {
#endif
...
/*
 * 函 数 名  : hmac_sta_join_update_heoption_mib
 * 功能描述  : 如果user 在wif6 blacklist 里面关闭HE mib
 * 1.日    期  : 2020年1月5日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
uint32_t hmac_proc_he_cap_ie(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user, uint8_t *puc_he_cap_ie)
{
    ...
    if (puc_he_cap_ie[1] < MAC_HE_CAP_MIN_LEN) {
        oam_error_log1(0, OAM_SF_11AX, "{hmac_proc_he_cap_ie::invalid he cap ie len[%d].}", puc_he_cap_ie[1]);
        return OAL_FAIL;
    }
    ...
    hmac_calc_tx_ampdu_maxsize(pst_mac_user, WLAN_HE_MODE);
...
uint8_t mac_set_he_6ghz_band_cap_ie(mac_vap_stru *mac_vap, uint8_t *buffer)
{
    ...
    ***************************************************************/
    ...
    return MAC_IE_EXT_HDR_LEN + MAC_HE_6GHZ_BAND_CAP_LEN;
}
#endif
