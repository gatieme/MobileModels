...
/*
 * 功能描述  : STA 11v roam scan work queue实现
 * 1.日    期  : 2021年3月19日
 *   作    者  : wifi
 *   修改内容  : 11v roam scan函数调用改成work queue
 */
...
/*
 * 功能描述  : 解析邻居AP的channel list和bssid list
 *             并分别保存到pst_scan_params->ast_channel_list[]和pst_roam_info->auc_neighbor_rpt_bssid[][]
 * 1.日    期  : 2020年05月09日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 功能描述  : 解析邻居AP的channel list和bssid list
 *             并分别保存到scan_params->ast_channel_list[]和roam_info->auc_neighbor_rpt_bssid[][]
 * 1.日    期  : 2019年10月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
    ...
    hmac_neighbor_bss_info_stru *neighbor_bss, mac_user_stru *mac_user, mac_vap_stru *p2p_vap)
{
    ...
    if (mac_user == NULL || neighbor_bss == NULL) {
        return OAL_FALSE;
    }
...
/*
 * 功能描述  : 获取Candidate Preference的最大值并设置到neighbor_bss_list
 * 1.日    期  : 2020年5月9日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
  * 2.日    期  : 2020年6月30日
 *   作    者  : wifi
 *   修改内容: Neighbor Report重构，先check bssid和channel信息，然后比较candidate preference
 */
 ...
 * 3.日    期   : 2019年6月20日
 *   作    者   : wifi
 *   修改内容   : 原来代码逻辑仅支持处理一个Neighbor report元素中的BSS并选择仅有的一个BSS作为target bss，
 *                MBO特性认证有带有两个或以上Neighbor Report元素的情况，
 *                修改代码逻辑为选择Preference值最大的作为target bss
 */
    ...
    while (sub_ie_len > 1) {
        switch (ie_data[0]) {
            ...
            case HMAC_NEIGH_SUB_ID_TERM_DURATION: /* 占用12个字节 */
                ...
                sub_ie_len -= (int16_t)(HMAC_11V_TERMINATION_ELEMENT_LEN + MAC_IE_HDR_LEN);
            ...
            case HMAC_NEIGH_SUB_ID_VENDOR_SPECIFIC:
                ...
                sub_ie_len -= (int16_t)(ie_data[1] + MAC_IE_HDR_LEN);
    ...
    uint8_t *ie_data_find, uint8_t bss_number, uint16_t len_find)
{
    ...
    for (bss_list_index = 0; bss_list_index < bss_number; bss_list_index++) {
        ...
        if (len_find < (neighbor_ie_len + MAC_IE_HDR_LEN)) {
            return OAL_FAIL;
        }
