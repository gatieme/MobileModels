...
static uint32_t hmac_post_ctx_netbuf_event(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *netbuf,
    uint16_t us_frame_len)
{
    ...
    if (pst_event_mem == NULL) {
        oam_error_log0(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_post_ctx_netbuf_event::pst_event_mem null.}");
    ...
    pst_tx_event->pst_netbuf = netbuf;
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(pst_mac_vap->uc_vap_id, OAM_SF_WMMAC,
            "{hmac_post_ctx_netbuf_event::frw_event_dispatch_event failed[%d].}", ret);
                            ...
                            mac_wmm_tspec_stru *pst_delts_args)
{
    ...
    ret = hmac_post_ctx_netbuf_event(pst_mac_vap, pst_delts, us_frame_len);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(pst_delts);
        return ret;
    }
                                ...
                                mac_wmm_tspec_stru *pst_addts_args)
{
    ...
    ret = hmac_post_ctx_netbuf_event(pst_mac_vap, pst_addts_req, us_frame_len);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(pst_addts_req);
        return ret;
    }
...
uint32_t hmac_mgmt_rx_addts_req_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_any_null_ptr2(pst_hmac_vap, pst_netbuf)) {
        oam_error_log0(0, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame::pst_hmac_vap or pst_netbuf is null!}");
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_WMMAC, "{hmac_mgmt_rx_addts_req_frame::find_user_by_macaddr failed[%d].}", ret);
        return OAL_ERR_CODE_PTR_NULL;
    }
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t hmac_tx_mgmt_reassoc_req(hmac_vap_stru *hmac_vap_sta)
{
    ...
    if (assoc_req_frame == NULL) {
        oam_error_log0(hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_tx_mgmt_reassoc_req::assoc_req_frame null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    hmac_vap_sta->bit_reassoc_flag = OAL_TRUE;
    asoc_frame_len = hmac_mgmt_encap_asoc_req_sta(hmac_vap_sta,
        (uint8_t *)(oal_netbuf_header(assoc_req_frame)), hmac_vap_sta->st_vap_base_info.auc_bssid,
        hmac_vap_sta->st_vap_base_info.auc_bssid);
    if (asoc_frame_len == 0) {
        oam_warning_log0(hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_tx_mgmt_reassoc_req::hmac_mgmt_encap_asoc_req_sta_etc null.}");
        oal_netbuf_free(assoc_req_frame);

        return OAL_FAIL;
    }

    oal_netbuf_put(assoc_req_frame, asoc_frame_len);
    hmac_user_ap = mac_res_get_hmac_user(hmac_vap_sta->st_vap_base_info.us_assoc_vap_id);
    if (hmac_user_ap == NULL) {
        oam_error_log1(hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_WMMAC,
            "{hmac_tx_mgmt_reassoc_req::hmac_user_ap [%d] null.}", hmac_vap_sta->st_vap_base_info.us_assoc_vap_id);
        oal_netbuf_free(assoc_req_frame);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 抛事件让DMAC将该帧发送 */
    ret = hmac_tx_mgmt_send_event(&(hmac_vap_sta->st_vap_base_info), assoc_req_frame, (uint16_t)asoc_frame_len);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(assoc_req_frame);
        oam_warning_log1(hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{mac_tx_mgmt_reassoc_req::hmac_tx_mgmt_send_event_etc failed[%d].}", ret);
        return ret;
    }
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_reassoc_req(mac_vap_stru *mac_vap)
{
    hmac_vap_stru *hmac_vap = NULL;

    if (g_en_wmmac_switch == OAL_FALSE) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_WMMAC,
            "{hmac_config_reassoc_req::wmmac switch is false[%d].}",
            g_en_wmmac_switch);
        return OAL_SUCC;
    }
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_WMMAC, "{hmac_config_reassoc_req::hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_tx_mgmt_reassoc_req(hmac_vap);
    return OAL_SUCC;
}

/*
 * 功能描述  : host是否降级来自device控制
 * 1.日    期  : 2020年8月21日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 功能描述  : 06 在ts建立成功，并且wmmac_switch的认证开关没开的情况下，判断是否需要在业务场景下做降级操作
 * 1.日    期  : 2020年7月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_need_degrade_for_ts_mp16(uint8_t tid, uint8_t need_degrade, uint8_t wmmac_auth_flag)
{
    return (need_degrade && (tid > WLAN_TIDNO_BEST_EFFORT) && (wmmac_auth_flag == OAL_FALSE));
}
/*
 * 功能描述  : mp13 mp15 在ts建立成功，默认关闭
 * 1.日    期  : 2020年7月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_need_degrade_for_ts_mp13(uint8_t tid, uint8_t need_degrade, uint8_t wmmac_auth_flag)
{
    return OAL_FALSE;
}
