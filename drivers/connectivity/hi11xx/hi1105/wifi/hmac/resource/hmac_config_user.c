...
uint32_t hmac_config_kick_process(hmac_vap_stru *hmac_vap, mac_vap_stru *mac_vap, uint16_t user_idx,
    uint16_t us_reason_code)
{
    ...
    /* 发去认证帧 */
    hmac_mgmt_send_disassoc_frame(mac_vap, mac_user->auc_user_mac_addr, us_reason_code, en_pmf);

    /* 设置错误码信息，EAP首次鉴权失败 */
    hmac_user->assoc_err_code = us_reason_code;
#ifdef _PRE_WLAN_CHBA_MGMT
    if (mac_is_chba_mode(mac_vap) == OAL_TRUE) {
        hmac_chba_kick_user(hmac_vap, hmac_user);
        return OAL_SUCC;
    }
#endif
    /* 修改 state & 删除 user */
...
uint32_t hmac_config_kick_user(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *param)
{
    ...
    return hmac_config_kick_process(hmac_vap, mac_vap, user_idx, ku_param->us_reason_code);
}
