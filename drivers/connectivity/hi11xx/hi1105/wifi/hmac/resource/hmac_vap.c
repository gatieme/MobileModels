...
/*
 * 函 数 名  : hmac_restart_all_work_vap
 * 功能描述  : 重启chip下所有工作的vap
 * 1.日    期  : 2017年2月8日
 *   修改内容  : 新生成函数
 */
uint32_t hmac_restart_all_work_vap(uint8_t chip_id)
{
    ...
    for (dev_idx = 0; dev_idx < mac_chip->uc_device_nums; dev_idx++) {
        ...
        for (vap_idx = 0; vap_idx < mac_dev->uc_vap_num; vap_idx++) {
            ...
            if (oal_unlikely(hmac_vap == NULL)) {
                oam_warning_log0(mac_dev->auc_vap_id[vap_idx], OAM_SF_CFG, "{hmac_restart_all_work_vap:vap null.}");
                return OAL_ERR_CODE_PTR_NULL;
            }
            mac_vap = &hmac_vap->st_vap_base_info;
            ...
            if (ret != OAL_SUCC) {
                oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_down_vap failed[%d].}", ret);
                return ret;
            }
            /* down netdev */
            ...
            if (ret != OAL_SUCC) {
                oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_start_vap failed[%d].}", ret);
                return ret;
            }
            /* up netdev */
