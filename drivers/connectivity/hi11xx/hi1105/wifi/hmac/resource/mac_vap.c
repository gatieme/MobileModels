...
/* 找到另一bnad且必须在主路的vap */
static mac_vap_stru *mac_vap_find_another_band_must_on_master_up_vap(mac_vap_stru *mac_vap, uint8_t chan_number)
{
    uint8_t vap_idx;
    wlan_channel_band_enum_uint8 band = mac_get_band_by_channel_num(chan_number);
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        mac_vap_temp = mac_res_get_mac_vap(mac_device->auc_vap_id[vap_idx]);
        if (mac_vap_temp == NULL ||
            mac_vap_temp->uc_vap_id == mac_vap->uc_vap_id ||
            mac_vap_temp->st_channel.en_band == band) {
            continue;
        }

        if (mac_vap_is_up(mac_vap_temp) &&
            mac_vap_is_must_on_master(mac_vap_temp, &mac_vap_temp->st_channel)) {
            return mac_vap_temp;
        }
    }
...
/*
 功能描述  : 设置对当前user 发送数据时需要设置的htc字段
  1.日    期   : 2019年7月8日
    作    者   : wifi
    修改内容   : 新生成函数
...
void mac_vap_tx_data_set_user_htc_cap(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        ...
        oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_11AX, "{mac_vap_tx_data_set_user_htc_cap:: \
            tx_data_include_htc=[%d], tx_data_include_om=[%d], vap_is_he_protocol_=[%d], he_cap=[%d].}",
...
#ifdef _PRE_WLAN_FEATURE_11AX
/*
 * 函 数 名  : mac_vap_get_ap_bandwidth_in_he_opern
 * 功能描述  : 获取HE模式下vht present或6ghz opern info存在时 ap的工作带宽
 */
OAL_STATIC wlan_channel_bandwidth_enum_uint8 mac_vap_get_ap_bandwidth_in_he_opern(
    mac_vap_stru *mac_sta, mac_user_stru *mac_user, wlan_channel_bandwidth_enum_uint8 pre_bandwidth_ap)
{
    mac_he_hdl_stru *mac_he_hdl = NULL;
    wlan_channel_bandwidth_enum_uint8 bandwidth_ap = pre_bandwidth_ap;

    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        mac_he_hdl = MAC_USER_HE_HDL_STRU(mac_user);
        if ((OAL_TRUE == mac_user_is_he_user(mac_user)) && (mac_he_hdl != NULL) &&
            (mac_he_hdl->st_he_oper_ie.st_he_oper_param.bit_vht_operation_info_present == OAL_TRUE)) {
            bandwidth_ap = mac_get_bandwith_from_center_freq_seg0_seg1(
                mac_he_hdl->st_he_oper_ie.st_vht_operation_info.uc_channel_width,
                mac_sta->st_channel.uc_chan_number,
                mac_he_hdl->st_he_oper_ie.st_vht_operation_info.uc_center_freq_seg0,
                mac_he_hdl->st_he_oper_ie.st_vht_operation_info.uc_center_freq_seg1);
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
        } else if ((mac_sta->st_channel.ext6g_band == OAL_TRUE) &&
            (OAL_TRUE == mac_user_is_he_user(mac_user)) && (mac_he_hdl != NULL) &&
            (mac_he_hdl->st_he_oper_ie.st_he_oper_param.bit_6g_oper_info_present == OAL_TRUE)) {
            bandwidth_ap = mac_get_6ghz_bandwith_from_center_freq_seg0_seg1(
                (uint8_t)mac_user->he_6ghz_operation_info.bit_channel_width,
                mac_sta->st_channel.uc_chan_number,
                mac_user->he_6ghz_operation_info.center_freq_seg0,
                mac_user->he_6ghz_operation_info.center_freq_seg1);
#endif
        }
    }
...
/*
 * 函 数 名  : mac_vap_get_ap_usr_opern_bandwidth
 * 功能描述  : 根据AP的operation ie获取ap的工作带宽
 * 1.日    期  : 2017年1月22日
 *   修改内容  : sta根据ap携带的ht和vth能力更新user和vap的带宽信息
 */
uint8_t mac_vap_get_ap_usr_opern_bandwidth(mac_vap_stru *pst_mac_sta, mac_user_stru *pst_mac_user)
{
    ...
    **********************************************************************/
...
#ifdef _PRE_WLAN_FEATURE_11AX
    en_bandwidth_ap = mac_vap_get_ap_bandwidth_in_he_opern(pst_mac_sta, pst_mac_user, en_bandwidth_ap);
#endif
    /* ht 20/40M带宽的处理 */
    ...
    if (mac_regdomain_channel_is_support_bw(en_sta_new_bandwidth,
        pst_mac_sta->st_channel.uc_chan_number, pst_mac_sta->st_channel.ext6g_band) == OAL_FALSE) {
...
/*****************************************************************************
 ...
 输入参数  : 无
 输出参数  : 无
 返 回 值  : MAC_NO_CHANGE or MAC_BW_CHANGE
  1.日    期   : 2017年1月22日
*****************************************************************************/
...
/*
 * 功能描述  : 2g ax wlan0 先入网, p2p/wlan1 5g 后入网时协议不启动在vht
            避免投屏等场景出现dbac
 * 1.日    期  : 2019.3.31
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_vap_avoid_dbac_close_vht_protocol(mac_vap_stru *mac_vap)
{
#ifdef _PRE_WLAN_FEATURE_11AX
    ...
    another_up_vap = mac_vap_find_another_band_must_on_master_up_vap(mac_vap, mac_vap->st_channel.uc_chan_number);
    ...
    if (mac_vap->st_channel.en_band == WLAN_BAND_5G) {
        ...
        oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_ANY,
            ...
            start vap:band=[%d], p2p mode[%d] close vht", another_up_vap->st_channel.en_band,
            mac_vap_is_work_he_protocol(another_up_vap), mac_vap->st_channel.en_band, mac_vap->en_p2p_mode);
        return OAL_TRUE;
    }
#endif
    return OAL_FALSE;
...
/*
 * 功能描述  : p2p 不允许启动在he协议,目的减少dbac 场景
                <1>、已经有vap 工作在5G.且当前p2p 是 2G
                <2>、已经有vap 工作在2G ax.且当前p2p 是 5G
 * 1.日    期  : 2019.3.31
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_vap_can_not_start_he_protocol(mac_vap_stru *mac_vap)
{
    ...
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_ANY,
        ...
        now start vap:band=[%d] can not star he", another_up_vap->st_channel.en_band,
        mac_vap_is_work_he_protocol(another_up_vap), mac_vap->st_channel.en_band);
#endif
    return close_he_flag;
}

/*
 * 功能描述  : p2p 启动带宽大于等于80M，为了减少dbac场景回退到40M
              满足条件:
                <1>、已经有vap 工作在2G ax.且 当前p2p 是 5G
 * 1.日    期  : 2019.3.31
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_vap_p2p_bw_back_to_40m(mac_vap_stru *mac_vap,
    uint8_t channel,  wlan_channel_bandwidth_enum_uint8 *channel_bw)
{
#ifdef _PRE_WLAN_FEATURE_11AX
    mac_vap_stru *another_up_vap = NULL;
    wlan_channel_bandwidth_enum_uint8  channl_band_width;

    /* 辅路支持ax 直接返回 OAL_FALSE */
    if (mac_vap_need_proto_downgrade(mac_vap) == OAL_FALSE) {
        return OAL_FALSE;
    }

    another_up_vap = mac_vap_find_another_band_must_on_master_up_vap(mac_vap, channel);
    if (another_up_vap == NULL) {
        return OAL_FALSE;
    }
    ...
    if (*channel_bw >= WLAN_BAND_WIDTH_80PLUSPLUS &&
        another_up_vap->st_channel.en_band == WLAN_BAND_2G && mac_vap_is_work_he_protocol(another_up_vap)) {
...
uint8_t g_uc_uapsd_cap = WLAN_FEATURE_UAPSD_ENABLE_CAP;
...
/*
 * 功能描述  : 根据user 在board下查找user 对象
 *             用于多芯片之间的Isolation 功能
 * 1.日    期  : 2015年10月31日
 *   修改内容  : 新生成函数
 */
...
{
    uint8_t           *puc_ie = NULL;
    uint32_t           ie_len = pst_app_ie->ie_len;
    ...
    /* 检查该类型的IE是否需要申请内存 */
    if ((mac_vap->ast_app_ie[en_type].ie_max_len < ie_len) || (mac_vap->ast_app_ie[en_type].puc_ie == NULL)) {
        /* 这种情况不应该出现，维测需要 */
    ...
    if (memcpy_s((void *)mac_vap->ast_app_ie[en_type].puc_ie, mac_vap->ast_app_ie[en_type].ie_max_len,
        (void *)pst_app_ie->auc_ie, ie_len) != EOK) {
...
uint32_t mac_vap_exit(mac_vap_stru *vap)
{
    ...
    /* begin: 删除vap 时，如果vap_num 非0 才--， 否则输出error 日志 */
...
{
    if (pst_mac_vap->pst_mib_info == NULL) { /*  规避死机 */
...
uint32_t mac_vap_set_security(mac_vap_stru *pst_mac_vap, mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (pst_beacon_param->en_privacy == OAL_FALSE) {
        /* , 清除组播用户加密信息位置由vap down移动到此处。解决vap down/up后STA关联APUT失败问题。 */
...
/*
 * 函 数 名  : mac_is_chba_mode
 * 功能描述  : 判断当前VAP是否是CHBA模式
  * 1.日    期  : 2020年4月12日
 *   作    者  : wifi7
 *   修改内容  : 新生成函数
 */
