...
void mac_vap_ap_init(mac_vap_stru *vap)
{
    ...
    /* 初始化特性标识 */
    vap->st_cap_flag.bit_uapsd = WLAN_FEATURE_UAPSD_ENABLE_CAP;
    ...
    mac_cfg_add_vap_param_stru *param)
{
    ...
    vap->dbac_enable_ap_follow_channel = OAL_TRUE; /* 默认支持信道跟随 */
...
/*
 * 函 数 名  : mac_vap_init_11n_40m_rates
 * 功能描述  : 初始化11n速率支持40M mcs32
 * 1.日    期  : 2020年2月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
    ...
    mac_data_rate_stru *pst_rates)
{
    ...
    /* begin: mp12 在listen channel的probe respons 携带速率集有重复IE.  */
    ...
    /* end: mp12 在listen channel的probe respons 携带速率集有重复IE.  */
...
void mac_sta_init_bss_rates(mac_vap_stru *pst_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    for (i = 0; i < bss_dscr->uc_num_supp_rates; i++) {
        for (j = 0; j < MAC_DATARATES_PHY_80211G_NUM; j++) {
            if (hmac_is_equal_rates(pst_rates[j].uc_mac_rate, bss_dscr->auc_supp_rates[i])) {
