...
uint32_t hmac_roam_set_sae_mode(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (is_only_support_sae(rsn_akm_suites, WLAN_PAIRWISE_CIPHER_SUITES) ||
        is_only_support_sae(crypto.aul_akm_suite, WLAN_AUTHENTICATION_SUITES) ||
        is_only_support_tbpeke(rsn_akm_suites, WLAN_PAIRWISE_CIPHER_SUITES) ||
...
#ifdef _PRE_WLAN_FEATURE_SAE
    ret = hmac_roam_set_sae_mode(mac_vap, bss_dscr);
    if (ret != OAL_SUCC) {
        return ret;
    }
#endif
