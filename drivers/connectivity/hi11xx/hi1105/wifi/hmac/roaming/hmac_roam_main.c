...
#ifdef _PRE_WLAN_FEATURE_PWL
    g_hmac_roam_main_fsm_func[ROAM_MAIN_STATE_WAIT_PRK_KEY][ROAM_MAIN_FSM_EVENT_TIMEOUT] =
        hmac_roam_pwl_wait_pre_key_timeout;
    g_hmac_roam_main_fsm_func[ROAM_MAIN_STATE_WAIT_PRK_KEY][ROAM_MAIN_FSM_EVENT_START_CONNECT] =
...
void hmac_roam_main_change_state(hmac_roam_info_stru *roam_info, roam_main_state_enum state)
{
    ...
    oam_warning_log2(0, OAM_SF_ROAM, "{hmac_roam_main_change_state::[%d]->[%d]}", roam_info->en_main_state, state);
    roam_info->en_main_state = state;
    ret = hmac_config_send_event(&(roam_info->pst_hmac_vap->st_vap_base_info), WLAN_CFGID_VAP_ROAM_MAIN_STATE_SYN,
        sizeof(state), (uint8_t *)(&state));
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_CFG, "{hmac_roam_main_change_state::hmac_config_send_event failed[%d].}", ret);
    }
}
 ...
 * 2.日    期   : 2019年8月7日
 *   作    者   : wifi
 *   修改内容   : 从aul_akm_suite中提取uc_akm_type
 */
...
OAL_STATIC void hmac_roam_init_roam_config_info(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
    ...
    roam_info->st_config.c_trigger_rssi_2g = (int8_t)g_wlan_cust.c_roam_trigger_b;
    roam_info->st_config.c_trigger_rssi_5g = (int8_t)g_wlan_cust.c_roam_trigger_a;
    roam_info->st_config.c_dense_env_trigger_rssi_2g = (int8_t)g_wlan_cust.c_dense_env_roam_trigger_b;
    roam_info->st_config.c_dense_env_trigger_rssi_5g = (int8_t)g_wlan_cust.c_dense_env_roam_trigger_a;
    roam_info->st_config.uc_delta_rssi_2g = (uint8_t)g_wlan_cust.c_roam_delta_b;
    roam_info->st_config.uc_delta_rssi_5g = (uint8_t)g_wlan_cust.c_roam_delta_a;
    roam_info->st_config.uc_scenario_enable = g_wlan_cust.uc_scenario_enable;
    roam_info->st_config.c_candidate_good_rssi = (int8_t)g_wlan_cust.c_candidate_good_rssi;
    roam_info->st_config.uc_candidate_good_num = (uint8_t)g_wlan_cust.uc_candidate_good_num;
    roam_info->st_config.uc_candidate_weak_num = (uint8_t)g_wlan_cust.uc_candidate_weak_num;
...
uint32_t hmac_roam_show_info(hmac_vap_stru *hmac_vap)
{
    ...
    if (snprintf_s(print_buff, OAM_REPORT_MAX_STRING_LEN, OAM_REPORT_MAX_STRING_LEN - 1,
        "ROAM_EN[%u] MAIN_STATE[%d] ROAM_SCAN_ORTH[%d]\n",
        roam_info->uc_enable, roam_info->en_main_state, roam_info->st_config.uc_scan_orthogonal) < 0) {
                         ...
                         roam_trigger_enum roam_trigger)
{
    ...
    /*  漫游到自己做重关联业务不做检查，即使在信号桥下也支持，比如btcoex m2s 和上层下发的重关联到自己 */
    if (current_bss_ignore == OAL_TRUE) {
        ...
        if (hmac_vap->en_roam_prohibit_on == OAL_TRUE) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_start::blacklist ap not support roam!}");
            return OAL_FAIL;
        }
    }
    ...
    /* 出现其他漫游类型，不属于重复漫游场景，清除保存的重复漫游统计值 */
    if (roam_trigger != ROAM_TRIGGER_BSSID && roam_trigger != ROAM_TRIGGER_11V) {
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_start::check_state fail[%d]!}", ret);
        return ret;
    }
    ...
    if (hmac_vap->st_vap_base_info.st_mbo_para_info.uc_mbo_enable != OAL_TRUE) {
        ...
        if (hmac_vap->ipaddr_obtained == OAL_FALSE) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_start::ip addr not obtained!}");
            return OAL_FAIL;
        }
#ifdef _PRE_WLAN_FEATURE_MBO
    }
...
/*
 * 功能描述  : 通过wal_cfg80211_get_station在识别家庭网络漫游场景下触发漫游
 * 1.日    期  : 2019年7月8日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_handle_home_network(hmac_vap_stru *hmac_vap)
{
    ...
    if ((roam_info->st_config.uc_scenario_enable == OAL_TRUE) &&
        ...
        (roam_info->st_alg.st_home_network.c_trigger_rssi + (int8_t)roam_info->st_alg.uc_roam_fail_cnt * 5))) {
...
#ifdef _PRE_WLAN_FEATURE_11K
/*
 * 功能描述  : 检查是否允许进行neighbor report交互
 * 日    期  : 2019年4月28日
 * 作    者  : wifi
 * 修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_roam_is_neighbor_report_allowed(hmac_vap_stru *hmac_vap)
{
    ...
    if ((cowork_ie[1] + MAC_IE_HDR_LEN) <
        ...
        oam_warning_log1(0, OAM_SF_ROAM,
            "{hmac_roam_is_neighbor_report_allowed::cowork ie too short[%d]}", cowork_ie[1]);
        return OAL_FALSE;
    }
...
OAL_STATIC uint32_t hmac_roam_connecting_fail(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    /*  auth/assoc reject processing, don't delete user when reassoc reject in roaming,
     * especailly for buffalo and synology WPA3 router */
...
/*
 * 功能描述  : 检查是否满足over ds的三个条件: 1. CoworkIE中over ds字段为1 2. current_AP_RSSI>-80 dbm 3. ft_failed不为ture
 * 1.日    期  : 2019年3月15日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
void hmac_roam_update_roam_conflict_stat(hmac_roam_info_stru *roam_info,
    hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    hmac_roam_confict_stat_stru *roam_conflict_info = &roam_info->roam_conflict_info;

    /* 如果是华为路由器通过11v触发漫游至2G，记录此次漫游的触发类型和时间戳
    不是11v触发或者指定bssid触发的漫游，则不属于问题单所示的漫游冲突场景，清除重复漫游统计值 */
    if (hmac_vap->is_hwrouter == OAL_TRUE &&
        hmac_vap->st_vap_base_info.st_channel.en_band == WLAN_BAND_5G && bss_dscr->st_channel.en_band == WLAN_BAND_2G) {
        memcpy_s(roam_conflict_info->pre_llv_roam_2g_bssid, WLAN_MAC_ADDR_LEN,
            bss_dscr->auc_bssid, WLAN_MAC_ADDR_LEN);
        roam_conflict_info->pre_roam_trigger_type = roam_info->en_roam_trigger;
        roam_conflict_info->pre_11v_roam_timestamp = (uint32_t)oal_time_get_stamp_ms();
        if (roam_info->en_roam_trigger != ROAM_TRIGGER_BSSID && roam_info->en_roam_trigger != ROAM_TRIGGER_11V) {
            roam_conflict_info->repeat_11v_roam_cnt = 0;
        }
    } else {
        hmac_roam_alg_clear_repeat_roam_stat(roam_info);
    }
...
OAL_STATIC void hmac_roam_set_related_info(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    /*  漫游切换时，不要从host获取RSSI，直接从device获取 */
    ...
    /* 漫游关联前保存目标AP的rssi */
    hmac_vap->roam_rssi = bss_dscr->c_rssi;
}
...
OAL_STATIC uint32_t hmac_roam_connect_to_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (hmac_vap->st_vap_base_info.pwl_enable == OAL_TRUE) {
        ret = hmac_roam_main_check_state(roam_info, MAC_VAP_STATE_UP, ROAM_MAIN_STATE_WAIT_PRK_KEY);
    } else {
    ...
    if (need_to_stop_user != OAL_FALSE) {
        ...
        if (ret != OAL_SUCC) {
            oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_connect_to_bss::hmac_fsm_call_func_sta fail[%ld]!}", ret);
            return ret;
        }
    }

    hmac_roam_connect_save_old_bss(&roam_info->st_old_bss, hmac_vap, hmac_user);
    hmac_roam_update_roam_conflict_stat(roam_info, hmac_vap, bss_dscr);

    /* 切换状态至connecting */
    hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_CONNECTING);

    /* 漫游切换时，更新获取user rssi的时间戳、connect次数，设置漫游到的bss能力位 */
    hmac_roam_set_related_info(hmac_vap, hmac_user, roam_info, bss_dscr);
...
/*
 * 函数说明  : 将old_bss的内容copy到h2d_sync，用于同步信息到dmac
 *
 * 1.日    期  : 2021年3月23日
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_roam_copy_old_bss(hmac_user_stru *hmac_user,
    hmac_roam_old_bss_stru *old_bss, mac_h2d_roam_sync_stru *h2d_sync)
{
    // 填充同步信息
    ...
    memcpy_s(&(h2d_sync->st_cap_info), sizeof(mac_user_cap_info_stru),
             &(old_bss->st_cap_info), sizeof(mac_user_cap_info_stru));
    memcpy_s(&(h2d_sync->hisi_priv_cap), sizeof(mac_hisi_cap_vendor_ie_stru),
             &(hmac_user->st_user_base_info.hisi_priv_cap), sizeof(mac_hisi_cap_vendor_ie_stru));
...
OAL_STATIC uint32_t hmac_roam_to_old_bss_h2d_sync_info(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    hmac_roam_old_bss_stru *old_bss)
{
    uint32_t ret;
    mac_h2d_roam_sync_stru *h2d_sync = NULL;
    if ((hmac_vap == NULL) || (hmac_user == NULL) || (old_bss == NULL)) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_to_old_bss_h2d_sync_info::there are some params is null }");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (h2d_sync == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_to_old_bss_h2d_sync_info::alloc fail!}");
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }

    hmac_roam_copy_old_bss(hmac_user, old_bss, h2d_sync);
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_to_old_bss_h2d_sync_info::send sync event failed[%d].}", ret);
    }
...
OAL_STATIC uint32_t hmac_roam_to_old_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_to_old_bss::hmac_config_start_vap_event failed[%d].}", ret);
        return ret;
    }
    /* old channel校准数据下发 */
    hmac_cali_send_work_channel_cali_data(&(hmac_vap->st_vap_base_info), &(hmac_vap->st_vap_base_info.st_channel),
        CALI_DATA_NORMAL_JOIN_TYPE);
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_to_old_bss::hmac_syn_vap_state failed[%d].}", ret);
    }

    /* 在漫游过程中可能又建立了聚合，因此回退时需要删除掉 */
    hmac_tid_clear(&hmac_vap->st_vap_base_info, hmac_user);
    /* 回退 bss 时，hmac 2 dmac 同步的相关信息，以便失败的时候回退 */
    ret = hmac_roam_to_old_bss_h2d_sync_info(hmac_vap, hmac_user, old_bss);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_to_old_bss::now resuming to [%02X:XX:XX:%02X:%02X:%02X]}", old_bss->auc_bssid[MAC_ADDR_0],
        old_bss->auc_bssid[MAC_ADDR_3], old_bss->auc_bssid[MAC_ADDR_4], old_bss->auc_bssid[MAC_ADDR_5]);
...
#ifdef _PRE_WLAN_FEATURE_PWL
    hmac_roam_pwl_success_clear_old_key(hmac_user, &(roam_info->st_old_bss));
#endif

    if (roam_info->uc_rssi_type == ROAM_ENV_SPARSE_AP_TH2 &&
        (hmac_roam_check_dual_threshold(roam_info) == OAL_TRUE)) {
        hmac_roam_rssi_trigger_type(hmac_vap, ROAM_ENV_SPARSE_AP_TH1);
    }
...
uint32_t hmac_roam_rssi_trigger_type(hmac_vap_stru *hmac_vap, roam_scenario_enum en_val)
{
    ...
    if (en_val == ROAM_ENV_LINKLOSS) {
        roam_trigger.trigger_2g = ROAM_RSSI_LINKLOSS_TYPE;
        roam_trigger.trigger_5g = ROAM_RSSI_LINKLOSS_TYPE;
        roam_info->st_alg.uc_invalid_scan_cnt = 0;
    } else if (en_val == ROAM_ENV_DENSE_AP) {
        roam_trigger.trigger_2g = roam_info->st_config.c_dense_env_trigger_rssi_2g;
        roam_trigger.trigger_5g = roam_info->st_config.c_dense_env_trigger_rssi_5g;
    } else if (en_val == ROAM_ENV_SPARSE_AP_TH1) {
        roam_trigger.trigger_2g = roam_info->st_config.c_trigger_th1_2g;
        roam_trigger.trigger_5g = roam_info->st_config.c_trigger_th1_5g;
    } else {
        roam_trigger.trigger_2g = roam_info->st_config.c_trigger_rssi_2g;
        roam_trigger.trigger_5g = roam_info->st_config.c_trigger_rssi_5g;
    }
...
uint32_t hmac_roam_pause_user(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    if ((roam_info == NULL) || (roam_info->uc_enable == 0) || (net_device == NULL)) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::roam disable or net_device null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }

    /* 必须保证vap的状态是UP */
    if (mac_vap->en_vap_state != MAC_VAP_STATE_UP) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::state=[%d]}", mac_vap->en_vap_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    /* 清空 HMAC层TID信息 */
    hmac_tid_clear(mac_vap, roam_info->pst_hmac_user);
#ifdef _PRE_WLAN_FEATURE_HIEX
    memset_s(&roam_info->pst_hmac_user->st_ersru, sizeof(mac_hiex_ersru_ctrl_msg_stru),
        0, sizeof(mac_hiex_ersru_ctrl_msg_stru));
#endif
    ret = hmac_config_set_pm_by_module(mac_vap, MAC_STA_PM_CTRL_TYPE_ROAM, MAC_STA_PM_SWITCH_OFF);
    if (ret != OAL_SUCC) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_roam_pause_user::set pm failed[%d].}", ret);
        oal_net_tx_wake_all_queues(net_device);
        return ret;
    }

    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_ROAMING_MODE, sizeof(uint8_t), (uint8_t *)&roaming_mode);
    if (ret != OAL_SUCC) {
    ...
    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::queues stopped!}");
    return OAL_SUCC;
}
...
uint32_t hmac_roam_resume_user(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    hmac_process_assoc_dyn_pcie_vote(hmac_vap, roam_info->pst_hmac_user);
...
OAL_STATIC void hmac_roam_set_scan_channel(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
    ...
    if ((mac_is_dual_sta_mode() != OAL_TRUE) || (mac_is_primary_legacy_vap(mac_vap) == OAL_TRUE)) {
        mac_vap_stru *other_vap = NULL;
...
#ifdef _PRE_WLAN_FEATURE_11K
/*
 * 功能描述  : 触发neighbor report request发送
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 功能描述  : 等待neighbor report response超时处理
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
#ifdef _PRE_WLAN_FEATURE_11R
uint32_t hmac_roam_state_is_valid(hmac_vap_stru *hmac_vap)
{
    hmac_roam_info_stru *roam_info = NULL;
    uint32_t ret;
    roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    if (roam_info == NULL) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_reassoc::roam info null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    ...
    if (roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_reassoc::main_state=[%d] not CONNECTING}", roam_info->en_main_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    if (roam_info->st_connect.en_state != ROAM_CONNECT_STATE_WAIT_ASSOC_COMP) {
        ...
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_reassoc::connect state is[%d]}", roam_info->st_connect.en_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    return OAL_SUCC;
}
...
uint32_t hmac_roam_reassoc(hmac_vap_stru *hmac_vap)
{
    ...
    ret = hmac_roam_state_is_valid(hmac_vap);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    if ((mac_mib_get_ft_trainsistion(&hmac_vap->st_vap_base_info)) == OAL_TRUE) {
        if (mac_mib_get_ft_over_ds(&hmac_vap->st_vap_base_info) &&
            ...
            if (bss_dscr == NULL) {
                oam_error_log0(vap_id, OAM_SF_ROAM, "{hmac_roam_reassoc::bss_dscr is null.}");
            ...
            ret = hmac_fsm_call_func_sta(hmac_vap, HMAC_FSM_INPUT_ROAMING_START, (void *)bss_dscr);
            if (ret != OAL_SUCC) {
                oam_warning_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_reassoc::hmac_fsm_call_func_sta fail[%ld]!}", ret);
                return ret;
            }
        }
...
/*
 * 功能描述  : 解析Neighbor Report Response Action Frame
 * 1.日    期  : 2019年10月16日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_roam_parse_neighbor_response(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    while (framebody_len >= HMAC_NEIGHBOR_RPT_MIN_LEN + MAC_IE_HDR_LEN) {
        if (puc_ie == NULL) {
            puc_ie = ((mac_action_neighbor_rsp_stru *)mac_get_80211_mgmt_payload(netbuf))->neighbor_rpt_ies;
        }
        ...
        framebody_len -= (int32_t)(neighbor_rpt_ie->uc_len + MAC_IE_HDR_LEN);
...
/*
 * 功能描述  : 接收neighbor report response
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
...
/*
 * 功能描述  : 解析neighbor report response，并且设置扫描参数，切换Roaming FSM 到扫描状态
 * 1.日    期  : 2019年4月3日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
  * 2.日    期  : 2020年7月7日
 *   作    者  : wifi
 *   修改内容  : 重构该函数，提取Neighbor Report Response Action Frame的解析部分
 */
OAL_STATIC uint32_t hmac_roam_neighbor_response_process(hmac_roam_info_stru *roam_info, void *param)
{
    mac_scan_req_stru *scan_params = NULL;
...
/*
 * 功能描述  : wlan0触发漫游时，如果wlan1正在漫游，踢掉wlan1，保证wlan0漫游成功率。
 * 1.日    期  : 2020年8月11日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
