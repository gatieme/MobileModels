 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_judge_rx_netbuf_classify(mac_llc_snap_stru *pst_snap, uint32_t buf_len)
{
    if (pst_snap == NULL) {
        return OAL_FALSE;
    }
    ...
    oal_netbuf_head_stru *head)
{
    ...
    (void)oal_warn_on(!oal_netbuf_list_empty(head));
...
#endif
    /*   mpxx bug fix  /wifi  2013/11/25 end */
    ...
    for (uc_device = 0; uc_device < uc_device_max; uc_device++) {
        ...
        for (uc_vap_idx = 0; uc_vap_idx < pst_hmac_device->pst_device_base_info->uc_vap_num; uc_vap_idx++) {
            ...
            if (pst_hmac_vap == NULL) {
                oam_error_log0(uc_vap_idx, OAM_SF_ANY, "{hmac_tcp_ack_process::pst_hmac_vap null.}");
                continue;
            }
...
oal_bool_enum_uint8 hmac_tcp_ack_need_schedule(void)
{
    ...
    if (pst_mac_chip == NULL) {
        oam_error_log0(0, OAM_SF_TX, "{hmac_tcp_ack_need_schedule::pst_mac_chip null.}");
        return OAL_FALSE;
    }
    ...
    hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    buf_len -= pst_rx_ctrl->uc_mac_header_len + sizeof(mac_llc_snap_stru);
    pst_mac_llc_snap_netbuf = (mac_llc_snap_stru *)(netbuf->data + pst_rx_ctrl->uc_mac_header_len);
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
                             ...
                             oal_netbuf_head_stru *pst_netbuf_header)
{
    ...
    (void)oal_warn_on(!oal_netbuf_list_empty(pst_netbuf_header));
...
#define MULTILINK_CASE_ONE 1
#define MULTILINK_CASE_TWO 2
static oal_bool_enum_uint8 g_amax_multilink = OAL_FALSE;
void hmac_amax_multilink_switch(int8_t en_amax_multilink)
{
    if ((en_amax_multilink == MULTILINK_CASE_ONE) || (en_amax_multilink == MULTILINK_CASE_TWO)) {
        if (g_amax_multilink == OAL_FALSE) {
            g_amax_multilink = OAL_TRUE;
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_amax_multilink_switch::enable multilink!}");
        }
    } else {
        if (g_amax_multilink == OAL_TRUE) {
            g_amax_multilink = OAL_FALSE;
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_amax_multilink_switch::disable multilink!}");
        }
    }
}
static void hmac_tcp_ack_filter_disable_by_amax(mac_device_stru *mac_device)
{
    /* 如果Multilink生效则关闭tcp ack过滤 */
    if (g_st_tcp_ack_filter.uc_cur_filter_status == OAL_FALSE) {
        return;
    }
    g_st_tcp_ack_filter.uc_cur_filter_status = OAL_FALSE;
    hmac_tcp_ack_update(mac_device, OAL_FALSE);

    oam_warning_log0(0, OAM_SF_ANY, "{hmac_tcp_ack_filter_disable_by_amax::disable by multilink!}");
}
#endif
...
#ifdef _PRE_CONFIG_HILINK_AMAX
    if (g_amax_multilink == OAL_TRUE) {
        hmac_tcp_ack_filter_disable_by_amax(pst_mac_device);
        return;
    }
#endif
    /* 每秒吞吐量门限 */
