...
static void hmac_vsp_layer_log(uint8_t *data)
{
    ...
    oam_warning_log4(0, 0, "{hmac_vsp_layer_log::RA[0][0x%x] RA[5][0x%x] SA[0][0x%x] SA[5][0x%x]}",
        layer->mac_ra_address[MAC_ADDR_0], layer->mac_ra_address[MAC_ADDR_5], layer->mac_sa_address[MAC_ADDR_0],
        layer->mac_sa_address[MAC_ADDR_5]);
}
...
static void hmac_vsp_pkt_log(uint8_t *data)
{
    ...
    oam_warning_log4(0, 0, "{hmac_vsp_layer_log::frame[%d] slice[%d] layer[0x%x] pkt[%d]}",
        msdu->frame_id, msdu->slice_num, msdu->layer_num, msdu->number);
    oam_warning_log4(0, 0, "{hmac_vsp_layer_log::last[%d] len[%d] cfg_pkt[0x%x] timestamp[%d]}",
        msdu->last, msdu->len, msdu->cfg_pkt, msdu->timestamp);
    oam_warning_log4(0, 0, "{hmac_vsp_layer_log::RA[0][0x%x] RA[5][0x%x] SA[0][0x%x] SA[5][0x%x]}",
        msdu->ra[MAC_ADDR_0], msdu->ra[MAC_ADDR_5], msdu->sa[MAC_ADDR_0], msdu->sa[MAC_ADDR_5]);
}
...
static void hmac_vsp_feedback_log(uint8_t *data)
{
    ...
    oam_warning_log4(0, 0, "{hmac_vsp_feedback_log::RA[0][0x%x] RA[5][0x%x] SA[0][0x%x] SA[5][0x%x]}",
        feedback->ra[MAC_ADDR_0], feedback->ra[MAC_ADDR_5], feedback->sa[MAC_ADDR_0], feedback->sa[MAC_ADDR_5]);
}
