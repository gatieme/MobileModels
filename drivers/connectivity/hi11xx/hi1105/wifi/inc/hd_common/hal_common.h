...
/* 3.4 中断相关枚举定义 */
/* 因为mac error和dmac misc优先级一致，mp13将high prio做实时事件队列来处理，mac error并入dmac misc */
/* 3.4.1 实时事件中断类型 */
typedef enum {
...
/* 3.6.1  芯片密钥类型定义 */
typedef enum {
    HAL_KEY_TYPE_TX_GTK = 0, /* MP12:HAL_KEY_TYPE_TX_IGTK */
    HAL_KEY_TYPE_PTK = 1,
    HAL_KEY_TYPE_RX_GTK = 2,
    HAL_KEY_TYPE_RX_GTK2 = 3, /* mp12使用，mp13和51不使用 */
    HAL_KEY_TYPE_BUTT
...
/* 同mp15的hal_rx_status_stru，与device要保持一致 */
typedef struct {
    /* byte 0 */
    ...
    uint8_t bit_rx_himit_flag : 1; /* rx himit flag = mp13 bit_preabmle */
