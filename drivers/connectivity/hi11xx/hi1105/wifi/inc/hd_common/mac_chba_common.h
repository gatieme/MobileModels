...
* 功能说明   : Hmac与Dmac公用的CHBA变量
* 作者       : wifi
* 创建日期   : 2021年3月18日
*/

#ifndef MAC_CHBA_COMMON_H
#define MAC_CHBA_COMMON_H
...
#define CHBA_PS_ALL_AWAKE_BITMAP (uint32_t)(0xFFFFFFFF)
#define CHBA_PS_HALF_AWAKE_BITMAP (uint32_t)(0x33333333)
#define CHBA_PS_KEEP_ALIVE_BITMAP (uint32_t)(0x00010001)
...
/* d2h chba同步事件 */
typedef enum {
    CHBA_UPDATE_TOPO_INFO = 0, /* 更新岛信息 */
    CHBA_RP_CHANGE_START_MASTER_ELECTION = 1, /* max RP变化触发主设备选举 */
    CHBA_NON_SYNC_START_MASTER_ELECTION = 2, /* 触发host侧主设备选举 */
    CHBA_NON_SYNC_START_SYNC_REQ  = 3, /* 触发host侧主动向master申请sync beacon */
    CHBA_DOMAIN_SEQARATE_START_MASTER_ELECTION = 4, /* 域拆分触发主设备选举 */
    CHBA_SYNC_BUTT
...
typedef enum {
    CHBA_BITMAP_AUTO = 0,
    CHBA_BITMAP_FIX = 1, /* 维测命令:固定chba bitmap */
...
typedef enum {
    MAC_CHBA_SEND_DOMAIN_MERGE_REQUSET = 0, /* 发送域合并通知帧domain requset */
    MAC_CHBA_SEND_DOMAIN_MERGE_BEACON = 1, /* 发送域合并通知beacon帧 */

    MAC_CHBA_SEND_DOMAIN_MERGE_BUTT
} mac_chba_send_domain_merge_notify_type_enum; /* chba域合并通知帧类型枚举 */

typedef struct {
    uint8_t domain_merge_notify_mgmt_type; /* 域合并通知管理帧类型 */
    uint8_t resv[1];
    uint16_t notify_user_id; /* 通知对端如果是自身user则记录user_id, 否则记为非法值 */
} mac_chba_domain_merge_timer_event_stru; /* chba域合并处理信息结构体 */
...
typedef struct {
    ...
    uint8_t device_id;
    uint8_t mac_addr[WLAN_MAC_ADDR_LEN]; /* 该device id对应的设备 */
...
typedef struct {
    ...
    uint8_t peer_mac_addr[WLAN_MAC_ADDR_LEN];         /* 管理帧目标方的 */
...
/* CHBA vap start通知结构体 */
typedef struct {
    uint8_t mac_addr[WLAN_MAC_ADDR_LEN];            /* 本设备 */
...
/* 建链/删链信息上报结构体 */
typedef struct {
    uint8_t self_mac_addr[WLAN_MAC_ADDR_LEN];       /* 本设备 */
    uint8_t peer_mac_addr[WLAN_MAC_ADDR_LEN];       /* 该连接的对端 */
    mac_channel_stru work_channel;
...
typedef struct {
    ...
    uint8_t supported_channel_list[WLAN_5G_CHANNEL_NUM]; /* 支持的信道号列表 */
} mac_chba_set_coex_chan_info;

#define chba_bitmap_to_level(_bitmap) (((_bitmap) == CHBA_PS_ALL_AWAKE_BITMAP) ? CHBA_BITMAP_ALL_AWAKE_LEVEL : \
...
#define chba_level_to_bitmap(_level) (((_level) == CHBA_BITMAP_ALL_AWAKE_LEVEL) ? CHBA_PS_ALL_AWAKE_BITMAP : \
...
#define CHBA_CHANNEL_SEQ_BITMAP_DEFAULT CHBA_CHANNEL_SEQ_BITMAP_50_PERCENT
...
typedef enum chba_chan_seq_cfg_type {
    CHBA_CHAN_SEQ_DEFAULT_TYPE = 0, /* 默认模式信道序列 */
    CHBA_CHAN_SEQ_CHBA_DBAC_TYPE = 1, /* chba dbac模式下信道序列 */

    CHBA_CHAN_SEQ_CFG_BUTT
} chba_chan_seq_cfg_type_enum;

/* CHBA 信道序列配置参数 */
#define CHBA_CHANNEL_SEQ_PARAMS_TYPE_CMD 0
#define CHBA_CHANNEL_SEQ_PARAMS_TYPE_SYNC 1
struct chba_channel_sequence_params {
    uint32_t params_type;
    uint32_t chba_chan_seq_cfg_type;
    union channel_seq_tag {
        struct test_cmd_tag {
        ...
        struct sync_info_tag {
            uint32_t channel_seq_bitmap;
            mac_channel_stru work_channel;
            mac_channel_stru sec_work_channel;
        } sync_info_params;
    } data;
} DECLARE_PACKED;
...
struct mac_chba_d2h_sync_event_info {
    uint8_t sync_event; /* 同步事件类型 */
    master_info max_rp_dev_info; /* 岛内最大rp值设备信息 */
} DECLARE_PACKED;
typedef struct mac_chba_d2h_sync_event_info mac_chba_d2h_sync_event_info_stru;
