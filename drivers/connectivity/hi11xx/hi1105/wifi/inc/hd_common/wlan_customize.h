...
#ifndef WLAN_CUSTOMIZE_H
#define WLAN_CUSTOMIZE_H
...
#define HAL_CUS_FB_LVL_NUM  2 /* 功率校准区分高低两个档位 */
#define HAL_CUS_DPN_MAX  50
#define HAL_CUS_DPN_MIN  (-50)
...
#define HAL_NUM_6G_20M_SIDE_BAND        59    /* 定制化6g边带数 */
#define HAL_NUM_6G_40M_SIDE_BAND        29
#define HAL_NUM_6G_80M_SIDE_BAND        14
#define HAL_NUM_6G_160M_SIDE_BAND       7
#define HAL_5G_20M_SIDE_BAND_ALL        24
#define HAL_5G_40M_SIDE_BAND_ALL        11
#define HAL_NUM_6G_20M_SIDE_BAND1       30
#define HAL_NUM_6G_20M_SIDE_BAND2       29
#define HAL_CUS_NUM_OF_SAR_PARAMS 8 /* 定制化降SAR参数 5G_BAND1~7 2.4G */
#define HAL_CUS_NUM_OF_6G_SAR_PARAMS  8 /* 6g定制化降SAR参数 */
#define HAL_NUM_OF_ALL_SAR_PARAMS (HAL_CUS_NUM_OF_SAR_PARAMS + HAL_CUS_NUM_OF_6G_SAR_PARAMS)
...
#define cus_restric_val_range(_val, _max, _min)  \
...
typedef enum {
    ...
    WLAN_CFGID_SET_CUS_FCC_CE_POW = 6,             /* 功率fcc ce参数定制化 */
#ifdef _PRE_WLAN_FE_NEW_ARCHITECTURE
    WLAN_CFGID_SET_FE_ALL_CUST,
#endif
...
typedef enum {
    ...
    CUST_DELTA_POW_2G_HE20_MCSAX12,
    /* nvram_params18 */
    CUST_DELTA_POW_2G_HE20_MCSAX13,
    CUST_DELTA_POW_2G_HE40_MCSAX12,
    CUST_DELTA_POW_2G_HE40_MCSAX13,
    CUST_DELTA_POW_5G_HE20_MCSAX12,
    /* nvram_params19 */
    CUST_DELTA_POW_5G_HE20_MCSAX13,
    CUST_DELTA_POW_5G_HE40_MCSAX12,
    CUST_DELTA_POW_5G_HE40_MCSAX13,
    CUST_DELTA_POW_5G_HE80_MCSAX12,
    /* nvram_params20 */
    CUST_DELTA_POW_5G_HE80_MCSAX13,
    CUST_DELTA_POW_5G_HE160_MCSAX12,
    CUST_DELTA_POW_5G_HE160_MCSAX13,
    CUST_DELTA_POW_RSV1,
...
typedef struct {
    ...
    uint32_t dpd_module_mask; /* dpd校准模块开关 */
    uint32_t cali_freq_mask; /*  校准频点开关  */
    uint8_t dpd_cali_lvl_num; /* DPD校准档位 */
    uint8_t nv_dpd_cali_status; /* 产线dpd校准完成标记位 */
...
typedef struct {
    ...
    uint8_t uc_ext_pa_isexist[WLAN_RF_CHANNEL_NUMS];
    uint32_t fem_mode_5g;
} dmac_ax_fem_pow_ctl_stru;

typedef struct {
    uint8_t fem_delt_pow[WLAN_CALI_BAND_BUTT];
    uint8_t tpc_adj_pow_start_idx[WLAN_BAND_BUTT];
    uint8_t resv[NUM_3_BYTES];
} dmac_fem_pow_gain_ctl_stru;

typedef struct {
    uint8_t fem_delt_pow_3a[WLAN_CALI_BAND_BUTT];
    uint8_t tpc_adj_pow_start_idx_3a[WLAN_BAND_BUTT];
    uint8_t resv[NUM_3_BYTES];
} dmac_fem_pow_gain_ctl_3a_stru;

typedef struct {
    /* 2g */
    int16_t gain_db_2g[MAC_NUM_2G_BAND];
    /* 5g */
    int16_t gain_db_5g[MAC_NUM_5G_BAND];
} mac_cust_rf_lp_loss_gain_db_stru;

typedef struct {
    int16_t ru_pwr_2g[WLAN_HE_RU_SIZE_996];
    int8_t  rsv[2]; /* 保留字段:3字节 */
} wlan_cust_2g_tpc_ru_pow_stru;

typedef struct {
    int16_t ru_pwr_5g[WLAN_HE_RU_SIZE_BUTT];
    int8_t  rsv[2];
} wlan_cust_5g_tpc_ru_pow_stru;
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
typedef struct {
    int16_t ru_pwr_6g[WLAN_HE_RU_SIZE_BUTT];
    int8_t  rsv[2];
...
typedef struct {
    ...
    dmac_fem_pow_gain_ctl_stru fem_pow;
    ...
    uint32_t alg_switch_ini; // 算法开关bitmap
    ...
    oal_bool_enum_uint8 phy_sample_switch; /* 数采开关 */
    uint8_t dyn_pcie_switch;
    ...
    uint8_t device_access_ddr;
    uint8_t mcast_ampdu_enable;
    ...
    uint8_t uwbcoex_switch;
    oal_bool_enum_uint8 miracast_sink_switch;
#ifdef _PRE_WLAN_CHBA_MGMT
    uint8_t chba_init_social_channel;
    uint8_t chba_social_follow_work_disable;
#else
    uint8_t resv2[2];
#endif
    /*  rom化预留扩展空间  */
    int16_t fb_gain_delta_pow[WLAN_CALI_BAND_BUTT];
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    int16_t fb_gain_rsv; /* 4字节对齐预留字段 */
#endif

    uint8_t dpd_en_start_mcs[WLAN_CALI_BAND_BUTT];
    uint8_t wlan_device_lp_agent_switch;

    uint32_t dpd_lvl_num[WLAN_CALI_BAND_BUTT];
    mac_cust_rf_lp_loss_gain_db_stru rf_lp_loss;

    uint16_t sar_total_time[CUS_SAR_LVL_NUM][WLAN_CALI_BAND_BUTT];
    int8_t sar_slide_pow_amend[HD_EVENT_RF_NUM][WLAN_CALI_BAND_BUTT];
    oal_bool_enum_uint8 sar_slide_window_en;
    uint8_t device_ini_tcp_ack_buf_en;
    uint8_t close_filter_switch;
    uint16_t fem_polar_policy[WLAN_CALI_BAND_BUTT];
    uint8_t chba_dbac_chan_follow_switch;
    uint8_t gle_ant_use;
    uint8_t lp_rx_cap;
    dmac_fem_pow_gain_ctl_3a_stru fem_pow_3a;
    uint16_t fem_mode_3a[WLAN_CALI_BAND_BUTT];
    wlan_cust_2g_tpc_ru_pow_stru tb_ru_2g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_5g_tpc_ru_pow_stru tb_ru_5g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    wlan_cust_6g_tpc_ru_pow_stru tb_ru_6g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#endif
    wlan_cust_2g_tpc_ru_pow_stru tb_ru_2g_3x3_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_5g_tpc_ru_pow_stru tb_ru_5g_3x3_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    wlan_cust_6g_tpc_ru_pow_stru tb_ru_6g_3x3_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#endif

    wlan_cust_2g_tpc_ru_pow_stru tb_ru_2g_4x4_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_5g_tpc_ru_pow_stru tb_ru_5g_4x4_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    wlan_cust_6g_tpc_ru_pow_stru tb_ru_6g_4x4_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
#endif
    int8_t dyn_bypass_extlna_rssi_th_h[WLAN_BAND_BUTT][WLAN_BANDWIDTH_BUTT];
    int8_t dyn_bypass_extlna_rssi_th_l[WLAN_BAND_BUTT][WLAN_BANDWIDTH_BUTT];
    int8_t offline_pow_cali_amend[HD_EVENT_RF_NUM][WLAN_CALI_BAND_BUTT];
    uint32_t extend[10];
}mac_cust_feature_cap_stru;

typedef struct {
    int8_t dpn_2g[DYN_PWR_CUST_2G_SNGL_MODE_BUTT][MAC_2G_CHANNEL_NUM];
    int8_t rsv;
    int8_t dpn_5g[WLAN_BANDWIDTH_BUTT][MAC_NUM_5G_BAND];
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    int8_t dpn_6g[WLAN_BANDWIDTH_BUTT][MAC_NUM_6G_BAND];
#endif
} cus_dy_cali_dpn_stru;

/* FCC/CE边带功率定制项 */
typedef struct {
    int16_t fcc_txpwr_limit_5g_20m[HAL_5G_20M_SIDE_BAND_ALL];
    int16_t fcc_txpwr_limit_5g_40m[HAL_5G_40M_SIDE_BAND_ALL];
    int16_t fcc_txpwr_limit_5g_80m[HAL_NUM_5G_80M_SIDE_BAND];
    int16_t fcc_txpwr_limit_5g_160m[HAL_NUM_5G_160M_SIDE_BAND];
    uint16_t fcc_ce_5g_high_band_max_pwr;
    int16_t rsv_5g;
    int16_t fcc_txpwr_limit_2g[WLAN_2G_SUB_BAND_NUM][HAL_CUS_NUM_FCC_CE_2G_PRO];
    int16_t rsv_2g;

#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    int16_t fcc_txpwr_limit_6g_20m[HAL_NUM_6G_20M_SIDE_BAND];  /* 边带fcc功率参数 */
    int16_t rsv_6g_20m;
    int16_t fcc_txpwr_limit_6g_40m[HAL_NUM_6G_40M_SIDE_BAND];
    int16_t rsv_6g_40m;
    int16_t fcc_txpwr_limit_6g_80m[HAL_NUM_6G_80M_SIDE_BAND];
    int16_t fcc_txpwr_limit_6g_160m[HAL_NUM_6G_160M_SIDE_BAND];
    int16_t rsv_6g_160m;
#endif
    uint8_t rsv[3];
    uint8_t rf_id;
...
typedef struct {
    uint16_t sar_total_time[WLAN_CALI_BAND_BUTT];
} wlan_cust_sar_slide_cfg_stru;

typedef struct {
    wlan_cust_sar_cfg_stru sar_avg_pow;
    wlan_cust_sar_slide_cfg_stru sar_cust_slide;
} wlan_sar_trigger_cfg_stru;
