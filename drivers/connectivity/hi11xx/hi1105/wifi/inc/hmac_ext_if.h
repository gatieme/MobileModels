...
#ifndef HMAC_EXT_IF_H
#define HMAC_EXT_IF_H
...
typedef enum {
    ...
    HMAC_HOST_CTX_EVENT_SUB_TYPE_FT_CONNECT_FAIL, /* 上报11r漫游入网失败 */
#endif                                         // _PRE_WLAN_FEATURE_11R

    HMAC_HOST_CTX_EVENT_SUB_TYPE_11V_ROAM_SUCC, /* 非华为路由器11v漫游成功 */
    ...
    HMAC_HOST_CTX_EVENT_SUB_TYPE_REQ_PWL_PRE_KEY,
#endif
...
#endif
    uint32_t plat_proc_result;
    oal_completion st_plat_process_comp; /* 用来检测device异常恢复过程中平台工作是否完成的信号量 */
    oal_atomic dfr_custom_cali_flag; /* 记录dfr期间校准处理，解决多线程同时校准的问题 */
} hmac_dfr_info_stru;
#endif  // _PRE_WLAN_FEATURE_DFR
...
#define ipaddr(addr)    (((uint8_t *)&addr)[0], ((uint8_t *)&addr)[3])
#define ipaddr6(addr)   ntohs((addr).s6_addr16[0]), ntohs((addr).s6_addr16[7])
