...
#endif
#define WAL_ATCMDSRV_FEM_DSM_THRESHOLD   2
int32_t g_us_tsensor_val = 0;
/*
...
OAL_STATIC void wal_get_mp16_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    ...
    if (*pl_fem_pa_status != 0) {
        ...
        check_cnt++;
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
        if (check_cnt > WAL_ATCMDSRV_FEM_DSM_THRESHOLD) {
            hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_FEMERROR, "%s: fem error,error_chain=0x%x",
                __FUNCTION__, *pl_fem_pa_status);
        }
#endif
    }
    return;
...
OAL_STATIC void wal_get_mp16_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    wlan_cali_data_para_stru *cali_data;
    uint32_t *check_hw_status = NULL;
    uint8_t rf_id;
    ...
    check_hw_status = cali_data->common_cali_data.cali_ctl_data.check_hw_status;
    /* 没做检测则不获取检测结果 */
    if (cali_data->common_cali_data.cali_ctl_data.cali_update_info.elna_det_flag == OAL_FALSE) {
        return;
    }
    cali_data->common_cali_data.cali_ctl_data.cali_update_info.elna_det_flag = OAL_FALSE;
    for (rf_id = 0; rf_id < HD_EVENT_RF_NUM; rf_id++) {
        *elna_gain_status |=
            (int32_t)(((check_hw_status[rf_id] & WLAN_ELNA_GAIN_STATUS_MASK) >> WLAN_ELNA_GAIN_STATUS_OFFSET) <<
            (BIT_OFFSET_4 * rf_id));
    }
    if (*elna_gain_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp16_elna_gain_status::ELNA gain status[0x%x].Bit:[0-3]c0,[4-7]c1,[8-11]c2,[12-15]c3,6g/5g/2g}",
            *elna_gain_status);
        /* DMD上报复用ilna */
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
        hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_LNAERROR, "%s: elna gain error,error_chain=0x%x",
            __FUNCTION__, *elna_gain_status);
#endif
    }
    return;
}

OAL_STATIC void wal_get_mp15_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    mp15_cali_param_stru *cali_data;
    uint8_t rf_id;
    ...
    /* 没做检测则不获取检测结果 */
    if (cali_data[WLAN_RF_CHANNEL_ZERO].elna_det[WLAN_BAND_2G].elna_det_flag == OAL_FALSE &&
        cali_data[WLAN_RF_CHANNEL_ONE].elna_det[WLAN_BAND_2G].elna_det_flag == OAL_FALSE) {
        return;
    }
    for (rf_id = 0; rf_id < WLAN_RF_CHANNEL_NUMS; rf_id++) {
        if (cali_data[rf_id].elna_det[WLAN_BAND_2G].elna_det_flag) {
            *elna_gain_status |= (cali_data[rf_id].elna_det[WLAN_BAND_2G].check_elna_status << (BIT_OFFSET_4 * rf_id));
            cali_data[rf_id].elna_det[WLAN_BAND_2G].elna_det_flag = OAL_FALSE;
        }
    }
    if (*elna_gain_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp15_elna_gain_status::ELNA gain status[0x%x].Bit:[0-3]c0,[4-7]c1,2g/5g}",
            *elna_gain_status);
        /* DMD上报复用ilna */
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
        hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_LNAERROR, "%s: elna gain error,error_chain=0x%x",
...
void wal_atcmsrv_ioctl_get_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    int32_t l_chip_type;

    l_chip_type = get_mpxx_subchip_type();
    if (l_chip_type >= BOARD_VERSION_MP16) {
        wal_get_mp16_elna_gain_status(net_dev, elna_gain_status);
    } else if (l_chip_type == BOARD_VERSION_MP15) {
        wal_get_mp15_elna_gain_status(net_dev, elna_gain_status);
    }
}
#ifdef PLATFORM_DEBUG_ENABLE
    ...
    struct wal_atcmdsrv_wifi_connect_info *connect_info)
{
    ...
    if (scanned_bss_info == NULL) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG,
            "{wal_atcmsrv_ioctl_get_wifi_connect_info::find the bss failed by bssid:%02X:XX:XX:XX:%02X:%02X}",
                                                           ...
                                                           wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    oam_warning_log2(0, OAM_SF_CFG, "wal_atcmsrv_ioctl_get_wifi_connect_info::state %d, rssi %d",
        pst_connect_info->en_status, pst_connect_info->c_rssi);
    oam_warning_log3(0, OAM_SF_CFG, "wal_atcmsrv_ioctl_get_wifi_connect_info::BSSID[%02X:XX:XX:XX:%02X:%02X]",
    ...
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    return wal_ioctl_get_pdet_val(pst_net_dev, &(pst_priv_cmd->pri_data.l_pow));
#else
    int32_t l_ret;
    wal_msg_write_stru st_write_msg;
    mac_vap_stru *pst_mac_vap = NULL;
...
{
#if ((defined(_PRE_WLAN_EXPORT)) && (defined(_PRE_WLAN_NARROW_BAND)))
    return wal_ioctl_get_tsensor_val(pst_net_dev, &(pst_priv_cmd->pri_data.tsensor_val));
#else
    int32_t l_ret;
    wal_msg_write_stru st_write_msg;
    mac_vap_stru *pst_mac_vap = NULL;
    ...
    pst_mac_vap = oal_net_dev_priv(pst_net_dev);
    if (pst_mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_EQUIP_TEST, "{wal_atcmsrv_ioctl_get_tsensor_val::OAL_NET_DEV_PRIV, return null!}");
        return -OAL_EINVAL;
    }
    ...
    ***************************************************************************/
    ...
    l_ret = wal_send_cfg_event(pst_net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH,
                               (uint8_t *)&st_write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_EQUIP_TEST, "{wal_atcmsrv_ioctl_get_tsensor_val:: return err_code [%d]!}", l_ret);
        return l_ret;
    }
