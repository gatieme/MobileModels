 ...
 * 功能说明 : wal_linux_atcmdsrv.c 的头文件
 * 作    者 :
 * 创建日期 : 2015年10月10日
 */

#ifndef WAL_LINUX_ATCMDSRV_H
#define WAL_LINUX_ATCMDSRV_H
...
#define WAL_ATCMDSRB_DPD_TIME     (30 * OAL_TIME_HZ)
#define WAL_ATCMDSRB_FLASH_TIME   (30 * OAL_TIME_HZ)
#define WAL_ATCMDSRB_WRITEF_TIME  (1 * OAL_TIME_HZ)
#define WAL_ATCMDSRB_SSI_TIME     (5 * OAL_TIME_HZ)
/* atcmdsrv私有命令宏定义 */
...
#define WAL_ATCMDSRV_BOARD_SN_LENGTH    12
...
#define WAL_ATCMDSRV_EFUSE_CHIP_ID_MP13    0x03
#define WAL_ATCMDSRV_EFUSE_CHIP_ID_MP15    0x05
...
extern uint16_t g_us_efuse_buffer[WAL_ATCMDSRV_EFUSE_BUFF_LEN];
extern int32_t g_us_tsensor_val;
extern uint32_t g_us_config_word;
extern uint8_t g_us_bsn[WAL_ATCMDSRV_BOARD_SN_LENGTH];
extern int8_t g_us_flash_check_result;
extern int32_t g_write_pow_flag;
extern int32_t g_write_xo_flag;
extern int32_t g_us_spmi_check_result;
extern int32_t g_us_ssi_check_result;
...
enum WAL_ATCMDSRV_IOCTL_CMD {
    ...
    WAL_ATCMDSRV_IOCTL_CMD_DPD_CALI,
    WAL_ATCMDSRV_IOCTL_CMD_POW_CALI,
    WAL_ATCMDSRV_IOCTL_CMD_GET_PDET_VAL,
    WAL_ATCMDSRV_IOCTL_CMD_GET_TSENSOR_VAL,
    WAL_ATCMDSRV_IOCTL_CMD_GET_CONFIG_WORD,
    WAL_ATCMDSRV_IOCTL_CMD_GET_BOARD_SN,
    WAL_ATCMDSRV_IOCTL_CMD_SET_BOARD_SN,
    WAL_ATCMDSRV_IOCTL_CMD_FLASH_TEST,
    WAL_ATCMDSRV_IOCTL_CMD_GETXOCAL,
    WAL_ATCMDSRV_IOCTL_CMD_SETXOCAL_NV,
    WAL_ATCMDSRV_IOCTL_CMD_SETCAL,
    WAL_ATCMDSRV_IOCTL_CMD_GETCAL,
    WAL_ATCMDSRV_IOCTL_CMD_SETCAL_NV,
    WAL_ATCMDSRV_IOCTL_CMD_SPMI_TEST,
    WAL_ATCMDSRV_IOCTL_CMD_SSI_TEST,
    WAL_ATCMDSRV_IOCTL_CMD_SET_LPMODE,
...
struct wal_atcmdsrv_wifi_dpd_cali_info {
    uint32_t cali_chn;
    uint32_t cali_step;
};
struct wal_atcmdsrv_wifi_pow_cali_info {
    uint8_t stream;
    uint8_t band_type;
    uint8_t band_idx;
    uint8_t gain_lvl;
...
struct wal_atcmdsrv_wifi_ssi_reg_info {
    uint32_t ssi_addr_value;
    uint32_t ssi_addr;
};
/* mp12 使用atcmdsrv 下发命令 */
typedef struct wal_atcmdsrv_wifi_priv_cmd {
    /* 校验位,取值mp12,与其他平台区别开来 */
    int32_t l_verify;
    int32_t cmd; /* 命令号 */
    union {
        int32_t l_freq;
        ...
        struct wal_atcmdsrv_wifi_pow_cali_info pow_cal_info;
        struct wal_atcmdsrv_wifi_dpd_cali_info dpd_cali_info;
        int32_t tsensor_val;
        uint32_t config_word;
        uint8_t bsn[WAL_ATCMDSRV_BOARD_SN_LENGTH];
        int8_t flash_check_result;
        struct wal_atcmdsrv_wifi_ssi_reg_info st_ssi_reg_info;
        uint8_t lp_mode;
    } pri_data;
 ...
 */
/* Note: mp13 has 512bits efuse, need to adapt this stuct */
...
static inline uint8_t wal_atcmdsrv_get_hex_char(uint8_t val)
{
    // 9/10代表数字9、10，16进制如果超过9，转化为大写A-F
