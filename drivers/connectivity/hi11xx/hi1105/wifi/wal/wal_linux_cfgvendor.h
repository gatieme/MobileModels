 ...
 * 功能说明 : Linux cfgvendor 头文件
 * 作    者 :
 * 创建日期 : 2016年05月10日
 */

#ifndef WAL_LINUX_CFGVENDOR_H
#define WAL_LINUX_CFGVENDOR_H
...
#define MKEEP_ALIVE_IP_PKT_MAXLEN 256
...
typedef enum {
    ...
    /* define all HID2D related commands between 0x1900 and 0x19FF */
    VENDOR_NL80211_SUBCMD_HID2D_RANGE_START = 0x1900,
    VENDOR_NL80211_SUBCMD_HID2D_RANGE_END = 0x19FF,
...
enum wal_vendor_subcmd {
    ...
    WIFI_SUBCMD_HID2D_CONN_NOTIFY = VENDOR_NL80211_SUBCMD_HID2D_RANGE_START,
...
enum andr_wifi_attr {
    ...
    /* keep last */
    ANDR_WIFI_ATTRIBUTE_LAST,
    ANDR_WIFI_ATTRIBUTE_MAX = ANDR_WIFI_ATTRIBUTE_LAST - 1,
};
...
typedef enum wal_vendor_event {
    ...
    NAN_EVENT_RX  = 32,
    VENDOR_11V_ROAM_SUCC_EVENT = 98,
    VENDOR_FT_CONNECT_FAIL_EVENT = 99, /* 上层vendor事件号定义到45，避免后续添加冲突 */
#ifdef _PRE_WLAN_FEATURE_PWL
    VENDOR_REQ_PWL_PRE_KEY_EVENT  = 100,
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
extern const struct nla_policy wal_andr_wifi_attr_policy[ANDR_WIFI_ATTRIBUTE_MAX + 1];
extern const struct nla_policy wal_gscan_attr_policy[GSCAN_ATTRIBUTE_MAX];
extern const struct nla_policy wal_mkeep_alive_attr_policy[MKEEP_ALIVE_ATTRIBUTE_MAX + 1];
#ifdef _PRE_WLAN_FEATURE_APF
extern const struct nla_policy wal_apf_attr_policy[APF_ATTRIBUTE_MAX + 1];
#endif
#ifdef _PRE_WLAN_FEATURE_NAN
extern const struct nla_policy wal_nan_attr_policy[NAN_ATTRIBUTE_MAX + 1];
#endif
#ifdef _PRE_WLAN_FEATURE_FTM
extern const struct nla_policy wal_rtt_attr_policy[RTT_ATTRIBUTE_MAX + 1];
#endif
#ifdef _PRE_WLAN_CHBA_MGMT
extern const struct nla_policy wal_chba_conn_ntf_attr_policy[MAC_CHBA_CONN_NTF_ATTR_MAX + 1];
#endif
#endif
...
int32_t wal_ioctl_set_sae_pwe(oal_net_device_stru *pst_net_dev, uint8_t sae_pwe);
...
int32_t wl_cfgvendor_stop_vowifi_nat_keep_alive(oal_wiphy_stru *pst_wiphy, oal_wireless_dev_stru *pst_wdev,
    const void *p_data, int32_t l_len);
int32_t wal_cfgvendor_lstats_get_info(oal_wiphy_stru *pst_wiphy, oal_wireless_dev_stru *pst_wdev,
    const void *p_data, int32_t l_len);
#ifdef _PRE_WLAN_FEATURE_APF
