...
OAL_STATIC void wal_update_scanned_bss_info(wal_scanned_bss_info_stru *scanned_bss_info, mac_bss_dscr_stru *bss_dscr)
{
    uint8_t chan_number  = bss_dscr->st_channel.uc_chan_number;
    uint8_t band;
    mac_ieee80211_frame_stru *frame_hdr = NULL;

    band = hmac_get_80211_band_type(&bss_dscr->st_channel);
    /* 初始化 */
    ...
    /* 填bss所在信道的中心频率 */
    scanned_bss_info->s_freq = (int16_t)oal_ieee80211_channel_to_frequency(chan_number, band);
    /* 填管理帧指针和长度 */
...
void wal_update_bss(oal_wiphy_stru *wiphy, hmac_bss_mgmt_stru *bss_mgmt, uint8_t *bssid)
{
    ...
    if (bss_dscr == NULL) {
        oam_warning_log4(0, OAM_SF_ASSOC, "{wal_update_bss::do not find correspond bss from scan result."
                         ...
                         bssid[BYTE_OFFSET_0], bssid[BYTE_OFFSET_3], bssid[BYTE_OFFSET_4], bssid[BYTE_OFFSET_5]);
        return;
    }
    ...
    if (cfg80211_bss != NULL) {
        ...
        /* 因为调用了get需要先put一下才能保证下面的unlink操作将bss删除 */
        oal_cfg80211_put_bss(wiphy, cfg80211_bss);
        /*
    ...
    /* 需要inform bss到内核 */
    if (en_inform_bss) {
        wal_update_scanned_bss_info(&scanned_bss_info, bss_dscr);
                                                 ...
                                                 uint8_t bssid_num)
{
    ...
    if (bssid_name_exist_flag == OAL_FALSE) {
        if (memcpy_s((bssid_name + bssid_num * WLAN_MAC_ADDR_LEN), OAL_MAC_ADDRESS_LEN,
            mac_bss_dscr_stru->auc_bssid, OAL_MAC_ADDRESS_LEN) != EOK) {
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void wal_countrycode_selstudy_result(wal_country_code_stru* wal_country_code, uint8_t vap_id)
{
    hmac_vap_stru *hmac_vap = NULL;
    /* 获取hmac vap */
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(vap_id, OAM_SF_SCAN, "{wal_countrycode_selstudy_result::hmac_vap is null, vap_id[%d]!}");
        return;
    }
    if (g_cust_country_code_ignore_flag.en_country_code_study_ingore_hipriv_flag == OAL_FALSE) {
    /* 国家码结果确认 */
...
/* voe 11r 认证时不进行时戳过滤 */
OAL_STATIC uint32_t wal_bss_filter_by_timestamps(mac_bss_dscr_stru *bss_dscr, uint8_t vap_id)
{
    ...
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(vap_id, OAM_SF_SCAN, "{wal_inform_all_bss::hmac_vap is null, vap_id[%d]!}");
        return OAL_FAIL;
    }

    uc_voe_11r_auth = hmac_vap->bit_voe_11r_auth;
    if (uc_voe_11r_auth == OAL_TRUE) {
...
/* 排除缓冲扫描结果 */
OAL_STATIC oal_bool_enum_uint8 wal_check_bss_is_include_wfd(mac_bss_dscr_stru *bss_dscr, uint8_t vap_id)
{
    uint8_t *p2p_ie = NULL;
    uint16_t offset = MAC_80211_FRAME_LEN + MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;
    p2p_ie = mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_WFD, (bss_dscr->auc_mgmt_buff + offset),
        (int32_t)(bss_dscr->mgmt_len - offset));
    /*  WFD only report current scan result, don't report buffered scan result in driver */
    if ((p2p_ie != NULL) && (p2p_ie[1] > MAC_P2P_MIN_IE_LEN) && (bss_dscr->en_new_scan_bss == OAL_FALSE)) {
...
/* 过滤扫描结果 */
OAL_STATIC uint32_t wal_bss_filter(mac_bss_dscr_stru *bss_dscr, uint32_t *bss_num_not_in_regdomain, uint8_t vap_id)
{
    /* 判断信道是不是在管制域内，如果不在，则不上报内核 */
    if (wal_channel_num_valid(bss_dscr) != OAL_SUCC) {
        (*bss_num_not_in_regdomain)++;
        return OAL_FAIL;
    }

    /* voe 11r 认证时不进行时戳过滤 */
    if (wal_bss_filter_by_timestamps(bss_dscr, vap_id) != OAL_SUCC) {
        return OAL_FAIL;
    }

    /* 排除缓冲扫描结果 */
    if (wal_check_bss_is_include_wfd(bss_dscr, vap_id) == OAL_TRUE) {
        return OAL_FAIL;
    }
...
void wal_inform_all_bss(oal_wiphy_stru *wiphy, hmac_bss_mgmt_stru *bss_mgmt, uint8_t vap_id)
{
...
#ifdef _PRE_WLAN_COUNTRYCODE_SELFSTUDY
    wal_country_code_stru         *wal_country_code = NULL;
    uint8_t                       bssid_name[WAL_BSSID_MAX_NUM][WLAN_MAC_ADDR_LEN];
    uint8_t                       bssid_num = 0;

    wal_country_code = (wal_country_code_stru *)oal_memalloc(sizeof(wal_country_code_stru));
    if (wal_country_code == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_inform_all_bss:: wal_country_code alloc mem return null ptr.}");
        return;
    }
    ...
    oal_dlist_search_for_each(entry, &(bss_mgmt->st_bss_list_head)) {
...
#endif
        /* 判断信道是不是在管制域内，如果不在，则不上报内核 */
        if (wal_bss_filter(bss_dscr, &bss_num_not_in_regdomain, vap_id) != OAL_SUCC) {
            continue;
        }
        ...
        wal_inform_all_bss_init(&scanned_bss_info, bss_dscr);
...
#ifdef _PRE_WLAN_COUNTRYCODE_SELFSTUDY
    /* 国家码自学习结果确认 */
    wal_countrycode_selstudy_result(wal_country_code, vap_id);
    oal_free(wal_country_code);
#endif
    return;
}
                                         ...
                                         mac_cfg80211_scan_param_stru *scan_param)
{
    ...
    for (loop = 0; loop < request->n_channels; loop++) {
        ...
        } else if ((center_freq >= WAL_MIN_FREQ_6G) && (center_freq <= WAL_MAX_FREQ_6G)) {
            scan_param->channels_6g[num_chan_6g++] = chan;
        } else {
            oam_error_log1(0, 0,
                "{wal_set_scan_channel::wrong channel numbers, center frequency is %d.}",
                center_freq);
        }
    }
    ...
    if ((l_ssid_num > 0) && (request->ssids != NULL)) {
        ...
        for (l_loop = 0; l_loop < l_ssid_num; l_loop++) {
            ...
            if (EOK != memcpy_s(pst_scan_param->st_ssids[l_loop].auc_ssid,
                                ...
                                request->ssids[l_loop].ssid_len)) {
                oam_warning_log3(0, OAM_SF_ANY,
                    "wal_set_scan_ssid::memcpy do nothing. cur_idx[%d], cur_ssid_len[%d]. scan_req_ssid_len[%d]",
                    l_loop, pst_scan_param->st_ssids[l_loop].uc_ssid_len, request->ssids[l_loop].ssid_len);
            }
        }
    }
}
...
OAL_STATIC uint32_t wal_set_scan_ie(mac_cfg80211_scan_param_stru *scan_param,
    oal_cfg80211_scan_request_stru *request)
{
    uint8_t *ie = NULL;
    scan_param->ie_len = request->ie_len;
    if (scan_param->ie_len == 0) {
        return OAL_SUCC;
    }
    ie = (uint8_t *)oal_memalloc(request->ie_len);
    if (ie == NULL) {
        oam_error_log1(0, OAM_SF_CFG, "{wal_set_scan_ie::ie(%d) alloc mem return null ptr!}",
                       request->ie_len);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    if (EOK != memcpy_s(ie, request->ie_len, request->ie, request->ie_len)) {
        oam_error_log0(0, OAM_SF_CFG, "wal_set_scan_ie::memcpy fail!");
        oal_free(ie);
        return OAL_FAIL;
    }
    scan_param->puc_ie = ie;
    return OAL_SUCC;
}
/*
                            ...
                            oal_cfg80211_scan_request_stru *request)
{
    ...
    if (ret != OAL_SUCC) {
        hmac_free_scan_param_resource(&st_scan_param);
        return OAL_FAIL;
    }
    ...
    /* 解析内核下发的ie */
    ret = wal_set_scan_ie(&st_scan_param, request);
    if (ret != OAL_SUCC) {
        hmac_free_scan_param_resource(&st_scan_param);
        return ret;
    }
...
int32_t wal_force_scan_complete(oal_net_device_stru *net_dev, oal_bool_enum en_is_aborted)
{
    ...
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_SCAN, "{wal_force_scan_complete:: mac_vap of net_dev is deleted!}");
        return OAL_SUCC;
    }
...
/*
 * 函 数 名  : wal_force_scan_abort_then_scan_comp
 * 功能描述  : 通知扫描完成
 * 1.日    期  : 2018年6月6日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
  * 2.日    期  :  2020年1月9日
 *   作    者  : wifi
 *   修改内容  : 去除MAC_SCAN_ABORT_SYNC逻辑，复用scan complete 逻辑
 */
int32_t wal_force_scan_abort_then_scan_comp(oal_net_device_stru *net_dev)
{
    ...
    if (oal_unlikely(net_dev == NULL || net_dev->ieee80211_ptr == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_force_scan_abort_then_scan_comp::net_dev or ieee80211_ptr null!}");
        return -OAL_EFAUL;
    }
    ...
    if (hmac_device == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{wal_force_scan_abort_then_scan_comp::pst_hmac_device[%d] is null!}", mac_vap->uc_device_id);
        return -OAL_EINVAL;
    }
    ...
    /*
     * 去关联的时候如果有上层扫描请求，
     * 不在此接口里面上报abort完成状态，等到device侧abort完成上报事件
     */
    if ((scan_mgmt->pst_request != NULL) && oal_wdev_match(net_dev, scan_mgmt->pst_request)) {
        /* 删除等待扫描超时定时器 */
        if (hmac_vap->st_scan_timeout.en_is_registerd == OAL_TRUE) {
...
int32_t wal_stop_sched_scan(oal_net_device_stru *netdev)
{
    ...
    if ((scan_mgmt->pst_sched_scan_req != NULL) && (scan_mgmt->en_sched_scan_complete != OAL_TRUE)) {
        ...
        /*  当暗屏PNO运行时，FWK下发normal scan，上报PNO stop event */
