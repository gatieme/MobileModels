...
static uint32_t wal_vendor_cmd_gather_handler(oal_net_device_stru *pst_net_dev, int8_t *pc_command);
...
OAL_STATIC uint32_t wal_vendor_set_p2p_scenes(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd,
    uint8_t *p_cmd)
{
    uint32_t ret;
...
static uint32_t wal_vendor_11ax_wifi6_blacklist_handle(oal_net_device_stru *p_net_dev, uint8_t *pc_cmd,
    uint32_t total_len)
{
    int32_t  ret;
 ...
 *   修改内容  : 新增函数
 */
OAL_STATIC uint32_t wal_hipriv_set_dfs_mode_scenes(oal_net_device_stru *net_dev, uint8_t *cmd)
{
    uint32_t ret;
    uint8_t *temp = NULL;
    uint8_t *cmd_data = NULL;
    ...
    uint8_t *pc_cmd, wal_wifi_priv_cmd_stru *p_priv_cmd)
{
    ...
    if (oal_strncasecmp(pc_cmd, CMD_PCIE_ASPM_STATE, CMD_PCIE_ASPM_STATE_LEN) == 0) {
        ...
        return (uint32_t)wal_hid2d_napi_mode(pst_net_dev, oal_atoi(pc_cmd + CMD_NAPI_STATE_LEN + 1));
        ...
        return (uint32_t)wal_hid2d_sleep_mode(pst_net_dev, oal_atoi(pc_cmd + CMD_PM_STATE_LEN + 1));
        ...
        ret = wal_vendor_priv_cmd_hid2d_apk(pst_net_dev, p_ifr, pc_cmd);
#endif
    } else {
        ret = wal_ioctl_priv_cmd_ext(pst_net_dev, p_ifr, pc_cmd, p_priv_cmd);
    }
    return ret;
}
    ...
    wal_wifi_priv_cmd_stru *st_priv_cmd, oal_ifreq_stru *pst_ifr, int32_t *l_memcpy_ret)
{
    ...
    if (oal_strncasecmp(pc_command, CMD_VOWIFI_GET_PARAM, CMD_VOWIFI_GET_PARAM_LEN) == 0) {
        ...
        l_ret = (int32_t)wal_vendor_cmd_gather_handler(pst_net_dev, pc_command);
        ...
        l_ret = (int32_t)wal_ioctl_get_psm_stat(pst_net_dev, MAC_PSM_QUERY_FLT_STAT, st_priv_cmd);
        if (l_ret != OAL_SUCC) {
        ...
        l_ret = (int32_t)wal_ioctl_set_fastsleep_switch(pst_net_dev, pc_command);
    } else if (0 == oal_strncasecmp(pc_command, CMD_SET_TCP_ACK_CTL, CMD_SET_TCP_ACK_CTL_LEN)) {
        l_ret = (int32_t)wal_ioctl_tcp_ack_buf_switch(pst_net_dev, pc_command);
    } else {
        /* 圈复杂度限制，新增拓展私有命令接口 */
...
int32_t wal_vendor_cmd_parse_lp_miracast(oal_net_device_stru *net_dev, int8_t *command)
{
    int32_t l_ret = 0;
    uint8_t is_enable;
    if (OAL_STRLEN(command) < (OAL_STRLEN((int8_t *)CMD_SET_LP_MIRACAST) + 2)) { /* 2为空格和设置值的长度 */
        oam_warning_log0(0, OAM_SF_M2S, "{wal_vendor_cmd_gather_handler::CMD_SET_LP_MIRACAST cmd len error.}");
        return -OAL_EFAIL;
    }
    is_enable = (uint8_t)oal_atoi(command + OAL_STRLEN((int8_t *)CMD_SET_LP_MIRACAST) + 1);
    oam_warning_log1(0, OAM_SF_M2S, "{CMD_SET_LP_MIRACAST %d.}", is_enable);
    l_ret = wal_ioctl_set_lp_miracast(net_dev, is_enable);
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_M2S, "{wal_vendor_cmd_gather_handler::CMD_SET_LP_MIRACAST err[%d]!}", l_ret);
        return -OAL_EFAIL;
    }
    return l_ret;
}
...
#endif
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
    } else if (0 == oal_strncasecmp(pc_command, CMD_SET_LP_MIRACAST, OAL_STRLEN(CMD_SET_LP_MIRACAST))) {
...
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
            || (0 == oal_strncasecmp(pc_command, CMD_SET_LP_MIRACAST, OAL_STRLEN(CMD_SET_LP_MIRACAST)))
#endif
#ifdef _PRE_WLAN_FEATURE_M2S
