...
/*****************************************************************************
 函 数 名  : wal_event_fsm_init_2
 功能描述  : wal_event_fsm_init函数拆分
 返 回 值  : OAL_SUCC
  1.日    期   : 2020年4月7日
    作    者   : wifi
    修改内容   : wal_event_fsm_init函数拆分
...
void wal_event_fsm_init_2(void)
{
    ...
    g_ast_wal_host_ctx_table[HMAC_HOST_CTX_EVENT_SUB_TYPE_REQ_PWL_PRE_KEY].p_func = wal_event_req_pwl_pre_key;
#endif
}
/*****************************************************************************
...
void wal_event_fsm_init(void)
{
    ...
    g_ast_wal_host_ctx_table[HMAC_HOST_CTX_EVENT_SUB_TYPE_FT_CONNECT_FAIL].p_func = wal_ft_connect_fail_report;
...
#endif
    g_ast_wal_host_ctx_table[HMAC_HOST_CTX_EVENT_SUB_TYPE_11V_ROAM_SUCC].p_func = wal_roam_11v_succ_report;
...
#define MP13_11AC_MCS9_160M_2STREAMS_RATE "1.73Gbps"
#define MP15_11AX_MCS11_160M_2STREAMS_RATE "2.40Gbps"
#define SHENKUO_11AX_MCS9_160M_4STREAMS_RATE "3.84Gbps"
...
OAL_STATIC ssize_t wal_get_wifi_devices_info_print(char *buf, int32_t buf_len)
{
    ...
    char *netcard_info[][NETCARD_INFO] = {
        { "Hi1102", "NULL" }, { "Hi1103", MP13_11AC_MCS9_160M_2STREAMS_RATE },
        { "Hi1102a", "NULL" }, { "Hi1105", MP15_11AX_MCS11_160M_2STREAMS_RATE },
        { "Shenkuo", SHENKUO_11AX_MCS9_160M_4STREAMS_RATE }
    };
...
OAL_STATIC ssize_t wal_get_wifi_devices_info(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    return wal_get_wifi_devices_info_print(buf, PAGE_SIZE);
}
...
OAL_STATIC ssize_t wal_get_bt_wifi_device_info(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    const char *netcard_info[NETCARD_TYPE] = {
        "Hi1102", "Hi1103", "Hi1102a", "Hi1105", "Shenkuo"
    };
...
void wal_destroy_all_vap(void)
{
    ...
    for (uc_vap_id = oal_board_get_service_vap_start_id(); uc_vap_id < WLAN_VAP_SUPPORT_MAX_NUM_LIMIT; uc_vap_id++) {
        ...
        if (pst_net_dev != NULL) {
            ...
            wal_hipriv_del_vap(pst_net_dev, "invalid");
