...
static unsigned int g_system_max_freq;
#endif
...
#ifdef CONFIG_SCHED_SMT_USER_HIDE
	if (current->getting_max_cap)
		max_freq = g_system_max_freq;
	max_freq = adjust_freq_buf(max_freq, policy);
#endif
...
static struct cpufreq_policy *cpufreq_policy_alloc(unsigned int cpu)
{
	...
	CALL_HCK_VH(lpcpu_cpufreq_policy_kobject_init, &policy->kobj,
		    &ktype_cpufreq, cpufreq_global_kobject, "policy%u",
		    topology_physical_package_id(cpu), &ret, &policy_init);
...
#ifdef CONFIG_SCHED_SMT_USER_HIDE
	if (policy->cpuinfo.max_freq > g_system_max_freq)
		g_system_max_freq = policy->cpuinfo.max_freq;
#endif
...
#ifdef CONFIG_HECA
	index = heca_get_kernel_freq_index(policy, index);
#endif
			      ...
			      unsigned int new_pol)
{
	...
	arch_set_max_freq_scale(policy->related_cpus, policy->max, policy->cpuinfo.max_freq);
