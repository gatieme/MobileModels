...
ATOMIC_NOTIFIER_HEAD(cpuidle_state_notifier);
EXPORT_SYMBOL_GPL(cpuidle_state_notifier);
#endif
...
#ifdef CONFIG_CPUIDLE_STATE_NOTIFY
	atomic_notifier_call_chain(&cpuidle_state_notifier, index, &dev->cpu);
#endif
...
#ifdef CONFIG_CPUIDLE_STATE_NOTIFY
	atomic_notifier_call_chain(&cpuidle_state_notifier, PWR_EVENT_EXIT, &dev->cpu);
#endif
