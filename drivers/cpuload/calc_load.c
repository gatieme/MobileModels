...
#if defined(CONFIG_FREQ_STATS_COUNTING_IDLE)
#include <platform_include/cee/linux/time_in_state.h>
#elif defined(CONFIG_HUAWEI_FREQ_STATS_COUNTING_IDLE)
...
static void send_to_user_high(u32 type, int size, const int *data)
{
	...
	(void)memset_s(&high_data, sizeof(high_data), 0, sizeof(high_data));
...
static int pidstat_init(void)
{
	...
	for (i = 0; i < HIGH_LOAD_MAX_TYPE; i++) {
		...
		(void)memset_s(g_pid_stat_mgt[i].head,
			g_pid_stat_mgt[i].max_count *
			sizeof(*(g_pid_stat_mgt[i].head)), 0,
			g_pid_stat_mgt[i].max_count *
			sizeof(*(g_pid_stat_mgt[i].head)));
		(void)memset_s(&g_pid_stat_mgt[i].data,
			sizeof(g_pid_stat_mgt[i].data), 0,
...
static void pid_stat_reset(u32 type)
{
	...
	(void)memset_s(g_pid_stat_mgt[type].head,
		g_pid_stat_mgt[type].index_curr *
		sizeof(*(g_pid_stat_mgt[type].head)), 0,
		g_pid_stat_mgt[type].index_curr *
		sizeof(*(g_pid_stat_mgt[type].head)));
	(void)memset_s(&g_pid_stat_mgt[type].data,
		sizeof(g_pid_stat_mgt[type].data), 0,
		sizeof(g_pid_stat_mgt[type].data));
...
static void pid_stat_clear(void)
{
	...
	for (i = 0; i < HIGH_LOAD_MAX_TYPE; i++) {
		...
		(void)memset_s(&g_pid_stat_mgt[i], sizeof(g_pid_stat_mgt[i]),
			0, sizeof(g_pid_stat_mgt[i]));
	}
}
...
static u64 get_each_cpu_idle_time(int cpu)
{
	u64 idle;
	u64 idle_time = -1ULL;
...
static void get_cpu_time(void)
{
	...
	(void)memset_s(cpu_online, sizeof(cpu_online), 0, sizeof(cpu_online));
	for_each_online_cpu(i) {
		...
		total_time += get_each_cpu_idle_time(i);
...
static void high_load_tickfn(struct work_struct *work)
{
	...
	if (check_intervals >= CPU_LOAD_TIMER_RATE) {
#if defined(CONFIG_FREQ_STATS_COUNTING_IDLE) || defined(CONFIG_HUAWEI_FREQ_STATS_COUNTING_IDLE) || (KERNEL_VERSION(4, 19, 0) <= LINUX_VERSION_CODE)
...
static void high_freqs_load_tick(void)
{
	...
	if (!policy) {
		policy = cpufreq_cpu_get(nr_cpu_ids - 1);
		if (policy)
	...
	ret_err = lpcpu_time_in_freq_get(CONFIG_NR_CPUS - 1,
