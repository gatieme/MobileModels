...
#if IS_ENABLED(CONFIG_ADSP_HONOR_SHARED_MEM)
#include "adsp_shared_mem.h"
#endif
...
{
	hilog_info("%s: send ipi buf to dsp\n", __func__);
...
static int read_data_from_dsp(void *data, int len)
{
	...
	hilog_info("%s: recv ipi data from dsp\n", __func__);

	ret = mtk_spk_send_ipi_buf_to_dsp(data, len);
	if (ret < 0) {
		hilog_err("%s: mtk_spk_send_ipi_buf_to_dsp fail, ret = %d\n",
								__func__, ret);
		return ret;
	}
	...
	if (ret < 0)
		hilog_err("%s: mtk_spk_recv_ipi_buf_from_dsp fail, ret = %d\n",
								__func__, ret);
				...
				const void *data_ptr, int data_len)
{
	...
	if (ret != 0) {
		hilog_err("%s: cmd-%u failed\n", __func__, cmd);
		return -EFAULT;
	}
	hilog_info("%s: cmd-%u success\n", __func__, cmd);
	return 0;
}
#endif
					...
					void __user *arg, int compat_mode)
{
	...
	if ((_IOC_TYPE(cmd)) != (ADSP_IOCTL_MAGIC)) {
		hilog_err("%s: cmd magic number invalid\n", __func__);
		return -EINVAL;
	}

	data_len = _IOC_SIZE(cmd);
	if (data_len > 0 && data_len < MAX_BUFFER_SIZE) {
		...
		hilog_err("%s: data len is zero or too large\n", __func__);
		return -EINVAL;
	}

	if (copy_from_user((void *)data_ptr, arg, data_len)) {
		hilog_err("%s: copy_from_user fail\n", __func__);
		ret = -EFAULT;
		goto EXIT;
	}
	...
	case IPI_GET_BUF:
		...
		if (ret) {
			hilog_err("ipi_get_buf, ret = %d\n", ret);
			ret = -EFAULT;
			goto EXIT;
		}

		if (copy_to_user((void __user *)arg, data_ptr, data_len)) {
			hilog_err("%s: copy_to_user fail\n", __func__);
			ret = -EFAULT;
			goto EXIT;
		}
		break;
...
#else
		hilog_warn("%s: not support SET_ADSP_SHARED_MEM\n", __func__);
#endif
		break;
	default:
...
{
	int ret;
	struct adsp_priv *adsp_priv_data = NULL;
	...
	adsp_priv_data = file->private_data;

	mutex_lock(&adsp_priv_data->do_ioctl_lock);
	ret = adsp_ctrl_do_ioctl_unlock(file, command,
...
static int adsp_remove(struct platform_device *pdev)
{
	...
	adsp_priv = platform_get_drvdata(pdev);
	if (adsp_priv == NULL) {
		hilog_err("%s: adsp_priv invalid argument\n", __func__);
		return -EINVAL;
	}
