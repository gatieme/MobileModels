...
#define HWLOG_TAG adsp_honor_shared_mem
HILOG_REGIST();
...
{
	void *mem_vaddr =
		adsp_get_reserve_mem_virt(ADSP_HONOR_SHARED_MEM_ID);
	phys_addr_t mem_paddr =
		adsp_get_reserve_mem_phys(ADSP_HONOR_SHARED_MEM_ID);
	size_t mem_size =
		adsp_get_reserve_mem_size(ADSP_HONOR_SHARED_MEM_ID);

	if ((mem_size < buf_size) || (mem_vaddr == NULL)) {
		hilog_err("%s: shared mem error\n", __func__);
		return -ENOMEM;
	}
...
EXPORT_SYMBOL(update_adsp_shared_mem);
...
EXPORT_SYMBOL(notify_adsp_shared_mem);

MODULE_DESCRIPTION("adsp_honor_shared_mem driver");
