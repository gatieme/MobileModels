...
void ana_hs_refresh_headset_type(int headset_type)
{
	if (pdata.ana_hs_init_flag == 0) {
		hilog_info("%s: ana hs is not init\n", __func__);
		return;
	}

	pdata.headset_type = headset_type;
	hilog_info("hs_default: refresh headset_type %d\n", pdata.headset_type);
}
EXPORT_SYMBOL(ana_hs_refresh_headset_type);
...
EXPORT_SYMBOL(ana_hs_dev_register);
...
EXPORT_SYMBOL(ana_hs_dev_deregister);
...
EXPORT_SYMBOL(ana_hs_support_usb_sw);
...
EXPORT_SYMBOL(ana_hs_codec_dev_register);
...
EXPORT_SYMBOL(ana_hs_plug_handle);
...
EXPORT_SYMBOL(ana_hs_mic_gnd_swap);
