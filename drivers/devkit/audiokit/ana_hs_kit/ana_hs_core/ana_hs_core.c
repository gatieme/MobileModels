...
#ifdef CONFIG_HONOR_SENSORS_INPUT_INFO
	...
	const char *property_name, bool default_setting)
{
	...
	if (!of_property_read_u32(dev->of_node, property_name, &value)) {
		...
		hilog_info("%s: missing %s, set default value %s\n",
...
static int ana_hs_load_gpio_data(struct device *dev)
{
	...
	for (i = 0; i < g_data->gpio_num; i++) {
		...
		if (g_data->gpio_list[i] < 0) {
			hilog_err("%s load gpio list failed\n", __func__);
	...
	if (ret < 0) {
		hilog_err("%s load gpio init val failed\n", __func__);
		return ret;
	}
	...
	for (i = 0; i < g_data->gpio_num; i++) {
		...
		if (ret < 0) {
			...
			if (ret < 0) {
				hilog_err("%s set output for %d fail %d\n",
					__func__, g_data->gpio_list[i], ret);
				return ret;
			}
		}
...
static void ana_hs_gpio_sequence_update(struct ana_hs_sequence_data *data)
{
	...
	if (!data || !(data->nodes)) {
		hilog_err("%s: gpio sequence data is null\n", __func__);
		return;
	}
...
static void ana_hs_i2c_sequence_update(struct ana_hs_sequence_data *data)
{
	...
	if (!data || !(data->nodes)) {
		hilog_err("%s: i2c sequence data is null\n", __func__);
		return;
	}
	...
	for (i = 0; i < data->num; i++) {
		if (data->nodes[i].mask != 0)
			...
			ret = ana_hs_bus_write(data->nodes[i].key,
				data->nodes[i].value);
		if (ret < 0) {
			hilog_err("%s: write reg 0x%x error\n",
...
static void get_mic_gnd_gpios(struct device_node *node)
{
	...
	if (ret < 0 || count <= 0) {
		hilog_err("%s:get mic_gnd_indexs fail or count is 0\n",
			__func__);
		return;
	}
...
#ifdef CONFIG_HONOR_SENSORS_INPUT_INFO
static void ana_hs_audio_pmic_ldo_set_state(int state)
{
	...
	audio_pmic_ldo_set.pmic_power_state = state;
	hw_pmic_power_cfg(AUDIO_PMIC_REQ, &audio_pmic_ldo_set);
}
#endif

static bool ana_hs_prepare_regulator(void)
{
	int ret;

	g_data->phy_ldo =
		devm_regulator_get(g_data->dev, "ana_hs_switch");
	if (IS_ERR(g_data->phy_ldo)) {
		hilog_err("phy_ldo regulator dts read failed\n");
		g_data->phy_ldo = NULL;
		return false;
	}

	ret = regulator_set_voltage(g_data->phy_ldo,
		g_data->phy_ldo_vol, g_data->phy_ldo_vol);
	if (ret != 0) {
		hilog_err("phy_ldo regulator set voltage failed\n");
		devm_regulator_put(g_data->phy_ldo);
		g_data->phy_ldo = NULL;
		return false;
	}
...
{
	if (!g_data || !g_data->regulator_used) {
		hilog_info("%s: g_data is null or ldo not used\n", __func__);
		return;
	}

	if ((g_data->phy_ldo == NULL) && !ana_hs_prepare_regulator()) {
		hilog_err("%s: regulator used but get failed\n", __func__);
		return;
	}

	hilog_info("%s: regulator config\n", __func__);
#ifdef CONFIG_HONOR_SENSORS_INPUT_INFO
	ana_hs_audio_pmic_ldo_set_state(ANA_HS_LOW);
#endif
	if (regulator_enable(g_data->phy_ldo)) {
		hilog_err("error: regulator enable failed\n");
#ifdef CONFIG_HONOR_SENSORS_INPUT_INFO
		ana_hs_audio_pmic_ldo_set_state(ANA_HS_HIGH);
#endif
	}
}
...
static void plugout_regulator_config(void)
{
	if (!g_data || !g_data->regulator_used || !g_data->phy_ldo) {
		hilog_info("%s: g_data is null or ldo is null\n",
			__func__);
		return;
	}
...
static void plugout_work(struct work_struct *work)
{
	...
	if (g_data->plugin_delay_wq) {
		hilog_info("%s remove plugin work, insert-remov too fast\n",
			__func__);
		cancel_delayed_work(
	...
	if (g_data->ana_hs_in == ANA_HS_PLUG_IN) {
		hilog_info("%s usb analog hs plug out act\n", __func__);
...
static int ana_hs_create_workqueue(void)
{
	...
	if (!(g_data->plugin_delay_wq)) {
		hilog_err("%s plugin wq create failed\n", __func__);
		return -ENOMEM;
	}
	...
	if (!(g_data->plugout_delay_wq)) {
		hilog_err("%s plugout wq create failed\n", __func__);
		ret = -ENOMEM;
		goto err_create_wq;
	}
	...
	struct ana_hs_codec_dev *dev, void *codec_data)
{
	...
	hilog_info("%s analog hs register sucessful\n", __func__);
...
static void ana_hs_core_plug_handle(int hs_state)
{
	/* analog headset driver not be probed, just return */
	if (!g_data) {
		hilog_err("%s pdata is NULL\n", __func__);
		return;
	}
	if (g_data->registed == ANA_HS_NOT_REGISTER) {
		hilog_err("%s codec_ops_dev is not registed\n", __func__);
		return;
	}
	...
	case DIRECT_CHARGE_OUT:
		...
		break;
	default:
		hilog_err("%s unknown state\n", __func__);
		break;
	}
	hilog_info("%s hs_state is %d\n", __func__, hs_state);
}
...
static void gpio_mic_gnd_swap(void)
{
	...
	struct ana_hs_node *node = NULL;

	if (data == NULL) {
		hilog_err("%s: mic_gnd_sequence NULL, do nothing\n", __func__);
		return;
	}
	for (i = 0; i < data->num; i++) {
		node = &data->nodes[i];
		if (node->key >= (unsigned int)(g_data->gpio_num)) {
			hilog_err("%s: invalid gpio index\n", __func__);
			return;
		}
		value = hs_gpio_get_value(g_data->gpio_list[node->key]);
		hilog_info("%s: gpio %d before value %d\n", __func__,
			g_data->gpio_list[node->key], value);
		value = set_mic_sel_val(value);
		hs_gpio_set_value(g_data->gpio_list[node->key], value);
		if (node->delay > 0)
			msleep(node->delay);
	}
}
...
static void i2c_mic_gnd_swap(void)
{
	...
	for (i = 0; i < data->num; i++) {
		...
		if (ret < 0) {
			hilog_err("%s: read reg 0x%x fail\n",
...
static void parse_sequence_data(struct device_node *node)
{
	...
	if (ret < 0)
		hilog_err("%s: parse dts init_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->plug_in_seq, node, "plug_in_seq");
	if (ret < 0)
		hilog_err("%s: parse dts plug_in_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->plug_out_seq, node, "plug_out_seq");
	if (ret < 0)
		hilog_err("%s: parse dts plug_out_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->mic_gnd_sequence, node, "mic_gnd_sequence");
	if (ret < 0)
		hilog_err("%s: parse dts mic_gnd_sequence fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->dp_charger_in_seq, node, "dp_charger_in_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_charger_in_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->dp_cross_seq, node, "dp_cross_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_cross_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->dp_charger_out_seq, node, "dp_charger_out_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_charger_out_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->fb_connect_seq, node, "fb_connect_seq");
	if (ret < 0)
		hilog_err("%s: parse dts fb_connect_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(
		&g_data->fb_disconnect_seq, node, "fb_disconnect_seq");
	if (ret < 0)
		hilog_err("%s: parse dts fb_disconnect_seq fail\n", __func__);
}
...
static void free_sequence_data(void)
{
...
static int ana_hs_get_regulator(struct device *dev)
{
	...
	if (!g_data) {
		hilog_err("%s error: g_data is null\n", __func__);
		return -1;
	}
	...
	if (!g_data->regulator_used) {
		hilog_info("%s: phy_ldo regulator not used\n", __func__);
		return -1;
	}

	ret = of_property_read_s32(node, "ana_hs_switch_vol",
		&g_data->phy_ldo_vol);
	if (ret != 0) {
		hilog_warn("phy_ldo_vol use default\n");
		g_data->phy_ldo_vol = LR_CHANNEL_VOLTAGE;
	}
...
static int sd_gpio_config(void)
{
	...
	virtual_addr = (unsigned long *)ioremap(SD_GPIO_SCTRL_REG,
		sizeof(unsigned long)); // SD_GPIO_SCTRL_REG is used by FSA4476 only
	if (!virtual_addr) {
		hilog_err("%s sd gpio config fail\n", __func__);
		return -1;
	}
...
static int ana_hs_core_probe(struct platform_device *pdev)
{
	...
	if (!g_data) {
		hilog_err("%s can not allocate ana hs dev data\n", __func__);
		return -ENOMEM;
	}
	...
	g_data->wake_lock =  wakeup_source_register(g_data->dev, "ana_hs_core");
	...
	if (g_data->bus_type == ANA_HS_GPIO) {
		...
		if (ret < 0) {
			hilog_err("%s get gpios failed, ret =%d\n", __func__, ret);
			goto ana_hs_err_out;
		}
	...
	if (ret < 0) {
		hilog_err("%s ana hs chip init failed, ret =%d\n", __func__, ret);
		goto ana_hs_err_out;
	}
	...
	if (ret < 0)
		hilog_info("%s: get regulator fail, maybe not needed\n", __func__);
	...
	if (ret) {
		hilog_err("%s misc device register failed\n", __func__);
...
ana_hs_err_out:
	free_sequence_data();
	ana_hs_free_gpio();
	wakeup_source_unregister(g_data->wake_lock);
	g_data->init_flag = 0;
	kfree(g_data);
	g_data = NULL;
