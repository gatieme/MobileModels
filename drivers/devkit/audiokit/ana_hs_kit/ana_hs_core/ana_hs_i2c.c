...
#include "honor_platform/audio/ana_hs_extern_ops.h"
	...
	unsigned int reg_addr, unsigned int *value)
{
	...
	hilog_info("%s: reg 0x%x  value 0x%x\n", __func__, reg_addr, *value);
	return ret;
}
	...
	unsigned int reg_addr, unsigned int mask, unsigned int value)
{
	...
	hilog_info("%s: reg 0x%x value 0x%x mask 0x%x\n", __func__, reg_addr, value, mask);
	return ret;
}
	...
	struct ana_hs_irq_handler **handler)
{
	...
	if (handler_info->gpio < 0) {
		hilog_info("%s: get named irq gpio failed, %d\n", __func__,
			handler_info->gpio);
		ret = of_property_read_u32(node, gpio_irq_str,
			(u32 *)&handler_info->gpio);
		if (ret < 0) {
			hilog_err("%s: read irq gpio failed, %d\n",
				__func__, ret);
			ret = -EFAULT;
			goto err_out;
		}
	...
	if (ret < 0) {
		hilog_err("%s: get irq flag failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (ret < 0) {
		hilog_err("%s: get irq func id failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	struct ana_hs_i2c_data *i2c_priv)
{
	...
	if (ret < 0) {
		hilog_info("%s: read init id failed, %d\n",
			__func__, ret);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (init_func == NULL) {
		hilog_err("%s ana_hs_init_select_func is null\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	init_func();
	...
	const struct i2c_device_id *id)
{
	...
	if (data == NULL) {
		hilog_err("%s: kzalloc data failed \n", __func__);
		return -ENOMEM;
	}
	...
	struct ana_hs_i2c_data *i2c_priv)
{
	...
	if (ret < 0) {
		hilog_info("%s: read remove id failed, %d\n",
			__func__, ret);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (remove_func == NULL) {
		hilog_err("%s ana_hs_remove_select_func is null\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	remove_func();
