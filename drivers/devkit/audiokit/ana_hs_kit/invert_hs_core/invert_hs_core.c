...
void invert_hs_core_control(int connect)
{
	...
	if (!invert_hs_pdata) {
		hilog_info("invert_hs_pdata is null\n");
		return;
	}
	...
	case INVERT_HS_MIC_GND_DISCONNECT:
		...
		mutex_unlock(&invert_hs_pdata->invert_hs_lock);
		__pm_relax(&invert_hs_pdata->wake_lock);
		hilog_info("%s: disconnect MIC and GND", __func__);
		break;
	case INVERT_HS_MIC_GND_CONNECT:
		...
		hilog_info("%s: queue delay work", __func__);
		break;
	default:
		hilog_info("%s: unknown connect type", __func__);
		break;
	}
...
static const struct of_device_id invert_hs_core_of_match[] = {
	{ .compatible = "honor,invert_hs_core", },
	{},
};
...
static int invert_hs_init_mic_gnd_gpio(struct device_node *node)
{
	...
	if (invert_hs_pdata->gpio_mic_gnd < 0) {
		hilog_err("gpio_mic_gnd is unvalid\n");
		return -ENOENT;
	}

	if (!gpio_is_valid(invert_hs_pdata->gpio_mic_gnd)) {
		hilog_err("gpio is unvalid\n");
		return -ENOENT;
	}
	...
	if (ret) {
		hilog_err("error request GPIO for mic_gnd fail %d\n", ret);
		return -ENOENT;
	}
...
static int invert_hs_create_workqueue(void)
{
	...
	if (!(invert_hs_pdata->anc_hs_invert_ctl_delay_wq)) {
		hilog_err("%s: invert_ctl wq create failed\n", __func__);
		return -ENOMEM;
	}
...
static int invert_hs_core_probe(struct platform_device *pdev)
{
	...
	hilog_info("invert_hs:node name is %s\n", node->name);
	if (IS_ERR(p)) {
		hilog_err("could not get pinctrl dev\n");
		ret = -ENOENT;
		goto err_invert_ctl_delay_wq;
	}
	pinctrl_def = pinctrl_lookup_state(p, "default");
	if (IS_ERR(pinctrl_def))
		hilog_info("could not get defstate\n");

	ret = pinctrl_select_state(p, pinctrl_def);
	if (ret)
		hilog_info("could not set pins to default state\n");
	...
	hilog_info("invert_hs probe success\n");
	return 0;
