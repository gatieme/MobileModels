...
#if IS_ENABLED(CONFIG_AUDIO_BOXID_USE_ADC)
...
#define BOXID_ADC_VOL_UV_TO_MV     1000
#define BOXID_USE_ADC_FLAG         0x80
...
__attribute__((weak))int self_dev_adc_get_value(int adc_channel)
{
	pr_info("adc_get_value:use weak func, channel = %d\n", adc_channel);
	return 0;
}

#if IS_ENABLED(CONFIG_AUDIO_BOXID_USE_ADC)
static int boxid_adc_get_value(void)
{
	int value = 0;
	int ret;

	ret = iio_read_channel_processed(device_boxid.iio_ch, &value);
	if (ret < 0) {
		pr_err("%s: read iio_ch %d failed\n", __func__, ret);
		return -1; // return - 1 on failure
	}

	pr_info("%s: adc value %d\n", __func__, value);
	return value / BOXID_ADC_VOL_UV_TO_MV;
}
#else
static inline int boxid_adc_get_value(void)
{
	return 0;
}
#endif // !CONFIG_AUDIO_BOXID_USE_ADC

static int get_hkadc_voltage(unsigned int hkadc_channel)
{
	if (hkadc_channel == BOXID_USE_ADC_FLAG)
		return boxid_adc_get_value();

	return self_dev_adc_get_value(hkadc_channel);
}
...
static int boxid_probe_use_hkadc(struct platform_device *pdev)
{
...
#if IS_ENABLED(CONFIG_AUDIO_BOXID_USE_ADC)
	if (box_hkadc_info->hkadc_channel == BOXID_USE_ADC_FLAG) {
		device_boxid.iio_ch = iio_channel_get(&pdev->dev, "boxid");
		if (IS_ERR(device_boxid.iio_ch)) {
			pr_err("get iio_ch err %ld\n", PTR_ERR(device_boxid.iio_ch));
			return -EPROBE_DEFER;
		}
	}
#endif
	...
	pr_info("hkadc_spk_channel %u, hkadc_min_voltage %u,"
