		...
		struct dsm_dev *dsm_dev)
{
	...
	if (dsm_client->dsm_client == NULL) {
		...
		if (dsm_client->dsm_client == NULL) {
			pr_err("dsm_audio_client register failed\n");
			return -ENOMEM;
		}
		pr_err("dsm_audio_client find in dsm_server\n");
	}

	dsm_client->dsm_str_info_buffer =
		kzalloc(dsm_dev->buff_size, GFP_KERNEL);
	if (!dsm_client->dsm_str_info_buffer) {
		pr_err("dsm audio %s malloc buffer failed\n",
				dsm_dev->name);
		return -ENOMEM;
	}
...
static int audio_dsm_init(void)
{
	...
	if (dsm_audio_client_table == NULL) {
		...
		if (dsm_audio_client_table == NULL) {
			pr_err("dsm_audio_client table malloc failed\n");
			return -ENOMEM;
		}
	}
		...
		unsigned int mesg_no)
{
	...
	if (dsm_audio_client_table == NULL ||
		...
		pr_err("dsm_audio_client did not register\n");
		return -EINVAL;
	}
...
MODULE_DESCRIPTION("Honor dsm audio driver");
