...
#ifdef CONFIG_HONOR_HEADSET_DEBUG_SWITCH
...
{
	// self_dev: use snd_soc_jack_report(),
		...
		loff_t *ppos)
{
	...
	if ((user_buf == NULL) || (ppos == NULL)) {
		hilog_err("user_buf or ppos is NULL\n");
		return -ENOMEM;
	}
...
static int headset_debug_convert_key_value(int status)
{
	...
	if (g_headset_debug.use_snd_jack_report) { // self_dev or qcom
		switch (status) {
		...
		size_t count, loff_t *ppos)
{
	...
	if (byte_writen != count) {
		hilog_err("simple_write_to_buffer err:%zd\n", byte_writen);
		return -ENOMEM;
	}

	ret = kstrtouint(kn_buf, 0, &status);
	if (ret) {
		hilog_err("kstrtouint error %d\n", ret);
		return -EINVAL;
	}
	switch (status) {
...
void headset_debug_init(struct snd_soc_jack *jack, struct switch_dev *sdev)
{
	if (jack == NULL || sdev == NULL) {
		hilog_err("jack or sdev is NULL\n");
		return;
	}
...
void headset_debug_input_init(struct input_dev *accdet_input_dev)
{
	if (accdet_input_dev == NULL) {
		hilog_err("accdet_input_dev is NULL\n");
		return;
	}
