...
static int smartpakit_self_dev_regulator_reg_read(int i2c_addr,
	int reg_addr, int *value)
{
	...
	if ((kit_priv == NULL) || (kit_priv->i2c_num <= 0)) {
		hilog_err("smartpakit_get_misc_priv: Invalid smartpakit_priv param");
		return -EINVAL;
	}
	...
	if ((i2c_priv == NULL) || (value == NULL)) {
		hilog_err("get_i2c_priv_by_addr: invalid input arg, i2c_addr0x%x\n", i2c_addr);
		ret = -EINVAL;
		goto err_out;
	}
	...
	if ((kit_priv->ioctl_ops == NULL) ||
		(kit_priv->ioctl_ops->read_regs == NULL)) {
		    hilog_err("ioctl_ops or i2c_reg_ctl is NULL\n");
		    ret = -ECHILD;
		    goto err_out;
	}
...
static int smartpakit_self_dev_regulator_reg_write(int i2c_addr, int reg_addr,
	int mask, int value)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
	struct smartpakit_priv *kit_priv = NULL;
	...
	i2c_priv = get_i2c_priv_by_addr(kit_priv, i2c_addr);
	if (i2c_priv == NULL) {
		hilog_err("get_i2c_priv_by_addr: invalid i2c addr 0x%x\n", i2c_addr);
		ret = -EINVAL;
		goto err_out;
	}
...
struct self_dev_regulator_reg_ops self_dev_reg_ops = {
	.self_dev_regulator_reg_read = smartpakit_self_dev_regulator_reg_read,
	.self_dev_regulator_reg_write = smartpakit_self_dev_regulator_reg_write,
};
