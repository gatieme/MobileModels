...
static struct pwm_ops_cmd_desc pwm_get_io_map_cmds[] = {
	{IO_REMAP_OPS, &pwm_ops_ctrl, PWM_SKIP_MODE},
};
...
static int pwm_get_io_reg_map(struct pwm_ops_cmd_desc *cmds)
{
	...
	hwlog_info("%s: get pwm io reg map", __func__);
	/* get pwm reg base */
	pwm_base = of_iomap(np, 0);
...
static bool smartpakit_pwm_need_update(struct smartpakit_priv *pakit_priv)
{
	if (!pakit_priv->high_power_config.pwm_level)
		return true;

	if (pakit_priv->high_power_config.voltage_mode == HIGH_VOLTAGE)
		return true;
	...
	struct smartpakit_param_node *node)
{
	...
	if (!smartpakit_pwm_need_update(pakit_priv))
		return 0;
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	ret = smartpakit_high_power_sequence_ctl(pakit_priv, sequence);
	if (ret < 0)
		return ret;
	...
	const void __user *arg, int compat_mode)
{
	...
	ret = smartpakit_high_power_save_poweron_regs(pakit_priv,
		param, &sequence);
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	hwlog_info("%s: pwm_type:%d", __func__,
...
void smartpakit_high_power_reset(struct smartpakit_priv *pakit_priv)
{
	...
	smartpakit_high_power_sequence_ctl(pakit_priv, sequence);
	sequence = &(pakit_priv->high_power_config.power_on_seq);
	smartpakit_high_power_sequence_ctl(pakit_priv, sequence);
	mutex_unlock(&(pakit_priv->high_power_config.ops_lock));
}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_high_power_parse_dt_info(dev, node, pakit_priv);
	if (ret)
		goto init_err;
...
static void smartpakit_high_power_pwm_resources_free(void)
{
...
void smartpakit_high_power_deinit(struct smartpakit_priv *pakit_priv)
{
	...
	smartpakit_high_power_pwm_resources_free();
	smartpakit_high_power_del_timer(pakit_priv);
	sequence = &(pakit_priv->high_power_config.power_off_seq);
