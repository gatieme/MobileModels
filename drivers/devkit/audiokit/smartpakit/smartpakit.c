...
void io_aggre_vote(void)
{
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	int ret = 0;
	sts_sys_status_enum state;

	if (!smartpakit_priv_data || !smartpakit_priv_data->io_aggre_support)
		return;

	mutex_lock(&smartpakit_priv_data->io_aggre_vote_lock);
	smartpakit_priv_data->io_aggre_vote_cnt++;

	if (smartpakit_priv_data->io_aggre_vote_state) {
		hwlog_info("%s: smartpakit has voted, exit", __func__);
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		return;
	}

	state = sts_get_status();
	if (state != STS_ST_INITED) {
		hwlog_info("%s: smartpakit sensorhub state %d", __func__, state);
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		return;
	}

	ret = sts_normal_request(STS_USER_AUDIO);
	if (ret == 0)
		smartpakit_priv_data->io_aggre_vote_state = true;

	hwlog_info("%s: smartpakit vote ret = %d, vote cnt = %d",
		__func__, ret, smartpakit_priv_data->io_aggre_vote_cnt);
	mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
}

void io_aggre_unvote(void)
{
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	int ret = 0;
	sts_sys_status_enum state;
	...
	if (smartpakit_priv_data->io_aggre_vote_cnt > 0) {
		hwlog_info("%s: smartpakit vote is using, exit", __func__);
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		return;
	}
	smartpakit_priv_data->io_aggre_vote_cnt = 0;

	if (!smartpakit_priv_data->io_aggre_vote_state) {
		hwlog_info("%s: smartpakit is not voted, exit", __func__);
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		return;
	}
	...
	state = sts_get_status();
	if (state != STS_ST_INITED) {
		hwlog_err("%s: can not unvote sensorhub state %d", __func__, state);
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		return;
	}

	ret  = sts_normal_release(STS_USER_AUDIO);
	if (ret == 0)
		smartpakit_priv_data->io_aggre_vote_state = false;

	hwlog_info("%s: smartpakit unvote ret = %d, vote cnt = %d",
		__func__, ret, smartpakit_priv_data->io_aggre_vote_cnt);
	mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
}
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
void sensorhub_state_cb(sts_sys_status_enum status, sts_err_info_t *info)
{
	int ret = 0;
	...
	mutex_lock(&smartpakit_priv_data->io_aggre_vote_lock);
	if (smartpakit_priv_data->io_aggre_vote_cnt > 0 &&
		!smartpakit_priv_data->io_aggre_vote_state &&
		status == STS_ST_INITED) {
		mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
		ret = sts_normal_request(STS_USER_AUDIO);
		mutex_lock(&smartpakit_priv_data->io_aggre_vote_lock);
		if (ret == 0)
		...
		hwlog_info("%s: smartpakit vote ret = %d", __func__, ret);
	}
	mutex_unlock(&smartpakit_priv_data->io_aggre_vote_lock);
}
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
...
EXPORT_SYMBOL_GPL(smartpakit_reg_info_add_list);
...
EXPORT_SYMBOL_GPL(smartpakit_reg_info_del_list_all);
...
void smartpakit_reset_pa_ctl_sequence(
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	if (sequence == NULL || sequence->node == NULL) {
		hwlog_err("%s: sequence node is invalid\n", __func__);
		return -EINVAL;
	}
...
void rst_gpio_request_lock(void)
{
	if (!smartpakit_priv_data) {
		hwlog_err("%s: smartpakit_priv_data is null\n", __func__);
		return;
	}
	mutex_lock(&smartpakit_priv_data->rst_gpio_request_lock);
}
EXPORT_SYMBOL_GPL(rst_gpio_request_lock);

void rst_gpio_request_unlock(void)
{
	if (!smartpakit_priv_data) {
		hwlog_err("%s: smartpakit_priv_data is null\n", __func__);
		return;
	}
	mutex_unlock(&smartpakit_priv_data->rst_gpio_request_lock);
}
EXPORT_SYMBOL_GPL(rst_gpio_request_unlock);
...
int smartpakit_hw_reset_chips(struct smartpakit_priv *pakit_priv)
{
	...
	if (is_high_power_smartpa(pakit_priv))
		smartpakit_high_power_reset(pakit_priv);
...
int smartpakit_resume_chips(struct smartpakit_priv *pakit_priv)
{
	...
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++) {
		...
		if (sequence->pa_poweron_flag != SMARTPA_REGS_FLAG_NEED_RESUME)
			continue;
		i2c_priv_p = pakit_priv->i2c_priv[i];
		if (smartpakit_high_power_is_power_down(pakit_priv, i2c_priv_p)) {
			hwlog_warn("%s:high power pa:%d powerdown skip\n",
				__func__, i);
			continue;
		}
...
int smartpakit_dump_chips(struct smartpakit_priv *pakit_priv)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (ret < 0)
			hwlog_err("%s: pa %d dump_regs error", __func__, i);
	}
...
bool smartpakit_is_param_valid(struct smartpakit_set_param *param)
{
...
int smartpakit_fill_param_sequence(
	struct smartpakit_pa_ctl_sequence *sequence,
	...
	struct smartpakit_param_node *node)
{
	...
	if ((sequence == NULL) || (param == NULL) || (node == NULL)) {
...
void smartpakit_print_node_info(struct smartpakit_param_node *node,
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	if ((pakit_priv == NULL) || (sequence == NULL) || (sequence->node == NULL)) {
		hwlog_err("%s: invalid argument\n", __func__);
		return -EINVAL;
	}
...
int smartpakit_simple_pa_gpio_node_ctl(
	struct smartpakit_priv *pakit_priv, struct smartpakit_param_node *node)
{
	if (pakit_priv == NULL || node == NULL) {
		hwlog_err("%s: params is NULL\n", __func__);
		return -EINVAL;
	}
	...
	void __user *arg, int compat_mode)
{
	...
	case SMARTPAKIT_CHECK:
		...
		break;
	case SMARTPAKIT_HIGH_POWER_CONFIG:
		ret = smartpakit_high_power_config_write(pakit_priv, arg,
			compat_mode);
		break;
	case SMARTPAKIT_HIGH_POWER_R_INFO:
		ret = smartpakit_high_power_get_info(pakit_priv, arg);
		break;
	default:
	...
	unsigned int command, unsigned long arg)
{
	...
	case SMARTPAKIT_W_SECR_COMPAT:
		...
		break;
	case SMARTPAKIT_HIGH_POWER_CONFIG_COMPAT:
		command = SMARTPAKIT_HIGH_POWER_CONFIG;
		break;
	default:
		break;
	}
...
EXPORT_SYMBOL_GPL(smartpakit_get_prop_of_u32_array);
...
int smartpakit_get_prop_num(struct device_node *node, const char *str)
{
	int i = 0;
...
EXPORT_SYMBOL_GPL(smartpakit_get_prop_num);
...
int smartpakit_save_switch_ctl_default_value(struct device_node *node,
	struct smartpakit_switch_node *ctl, unsigned int num)
{
...
EXPORT_SYMBOL_GPL(smartpakit_save_switch_ctl_default_value);
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_pinctrl_select_state(pinctrl, pin_ctrl_state_name);
	if (ret)
		goto exit_parse_dt_irq_pinctrl;
...
int smartpakit_parse_i2c_dev_retry_info(struct device *dev,
	struct smartpakit_priv *pakit_priv)
{
	const char *retry_adapt_enable = "i2c_ops_retry_adapt_enable";
	const char *retry_time = "i2c_ops_retry_time";
	const char *retry_delay = "i2c_ops_retry_delay";
	int ret;

	if (of_property_read_bool(dev->of_node, retry_adapt_enable)) {
		pakit_priv->i2c_retry_info.retry_adapt_enable = true;
		hwlog_info("%s: prop existed\n", __func__);
	} else {
		pakit_priv->i2c_retry_info.retry_adapt_enable = false;
		hwlog_debug("%s: prop not existed\n", __func__);
		return 0;
	}

	ret = smartpakit_get_prop_of_u32_type(dev->of_node,
			retry_time, &(pakit_priv->i2c_retry_info.retry_time), false);

	ret += smartpakit_get_prop_of_u32_type(dev->of_node,
			retry_delay, &(pakit_priv->i2c_retry_info.retry_delay), false);

	hwlog_info("%s: i2c ops retry:%u, retry_delay:%u\n", __func__,
		pakit_priv->i2c_retry_info.retry_time,
		pakit_priv->i2c_retry_info.retry_delay);
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	const char *io_aggre_support_str = "io_aggre_support";
	const char *read_version_reg_before_rst_str = "read_version_reg_before_reset";
	int ret;
	...
	if (of_property_read_bool(pdev->dev.of_node, io_aggre_support_str))
		pakit_priv->io_aggre_support = true;
	else
		pakit_priv->io_aggre_support = false;

	if (of_property_read_bool(pdev->dev.of_node, read_version_reg_before_rst_str))
		pakit_priv->read_version_reg_before_rst = true;
	else
		pakit_priv->read_version_reg_before_rst = false;
	...
	ret += smartpakit_parse_i2c_dev_retry_info(&pdev->dev, pakit_priv);
	if (ret < 0)
		return ret;
...
static int smartpakit_probe(struct platform_device *pdev)
{
	...
	ret = smartpakit_high_power_init(&pdev->dev, pdev->dev.of_node,
		pakit_priv);
	if (ret != 0)
		goto err_out;
	...
	mutex_init(&pakit_priv->rst_gpio_request_lock);
	mutex_init(&pakit_priv->io_aggre_vote_lock);
	...
	pakit_priv->io_aggre_vote_cnt = 0;
	pakit_priv->io_aggre_vote_state = false;
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	ret = sts_notify_register(STS_USER_AUDIO, sensorhub_state_cb);
	if (ret != 0)
		hwlog_err("%s: sts notify regist fail", __func__);
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
...
err_high_power:
	smartpakit_high_power_deinit(pakit_priv);
err_out:
...
static int __init smartpakit_init(void)
{
	...
	if (ret != 0)
		hwlog_err("%s: platform_driver_register failed, %d\n",
			__func__, ret);
	ret += i2c_add_driver(&smartpakit_i2c_driver);
	if (ret != 0)
		hwlog_err("%s: i2c driver register failed %d\n", __func__, ret);
