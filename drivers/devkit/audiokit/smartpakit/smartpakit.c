...
extern struct self_dev_regulator_reg_ops self_dev_reg_ops;

int __attribute__((weak)) self_dev_regulator_reg_ops_register(
	struct self_dev_regulator_reg_ops *reg_ops)
{
	return 0;
}
#endif
...
EXPORT_SYMBOL(smartpakit_get_misc_priv);

int smartpakit_get_misc_init_flag(void)
{
	return smartpakit_init_flag;
}
EXPORT_SYMBOL(smartpakit_get_misc_init_flag);
...
EXPORT_SYMBOL(smartpakit_reg_info_add_list);
...
EXPORT_SYMBOL(smartpakit_reg_info_del_list_all);
...
static void smartpakit_set_hw_dev_flag(void)
{
	if (smartpakit_priv_data->i2c_num < smartpakit_priv_data->pa_num) {
		...
		hilog_info("%s: i2c_num < pa_num\n", __func__);
#endif
		return;
	}

	if (smartpakit_is_chip_model_different()) {
		hilog_info("%s: NOTICE: CHIP_MODEL NOT SAME\n", __func__);
	} else {
#ifdef CONFIG_HONOR_DEBUG
		hilog_info("%s: check chip_model success\n", __func__);
#endif
...
int smartpakit_register_i2c_device(struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (i2c_priv->chip_id >= smartpakit_priv_data->pa_num) {
		hilog_err("%s: error, chip_id %u >= pa_num %u\n", __func__,
	...
	if (smartpakit_priv_data->i2c_priv[i2c_priv->chip_id] != NULL) {
		hilog_err("%s: chip_id reduplicated error\n", __func__);
		smartpakit_priv_data->chip_register_failed = true;
		return -EINVAL;
	}
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: i2c_priv registered, success\n", __func__);
#endif
...
int smartpakit_deregister_i2c_device(struct smartpakit_i2c_priv *i2c_priv)
{
	...
	for (i = 0; i < (int)smartpakit_priv_data->pa_num; i++) {
		...
		hilog_info("%s: i2c_priv deregistered, success\n", __func__);
		break;
	}
...
void smartpakit_register_i2c_ctl_ops(struct smartpakit_i2c_ctl_ops *ops)
{
	if ((smartpakit_priv_data == NULL) || (ops == NULL)) {
		hilog_err("%s: ioctl_ops register failed\n", __func__);
		return;
	}

	if (smartpakit_priv_data->ioctl_ops != NULL) {
#ifdef CONFIG_HONOR_DEBUG
		hilog_debug("%s: ioctl_ops had registered, skip\n", __func__);
#endif
		return;
	}
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_debug("%s: ioctl_ops registered, success\n", __func__);
#endif
}
EXPORT_SYMBOL_GPL(smartpakit_register_i2c_ctl_ops);

void smartpakit_register_dsm_info_ops(struct smartpakit_dsm_info_ops *ops)
{
	if ((smartpakit_priv_data == NULL) || (ops == NULL)) {
		hilog_err("%s: ioctl_ops register failed\n", __func__);
		return;
	...
	if (smartpakit_priv_data->dsm_info_ops != NULL) {
#ifdef CONFIG_HONOR_DEBUG
		hilog_debug("%s: ioctl_ops had registered, skip\n", __func__);
#endif
		return;
	...
	smartpakit_priv_data->dsm_info_ops = ops;
#ifdef CONFIG_HONOR_DEBUG
	hilog_debug("%s: ioctl_ops registered, success\n", __func__);
#endif
}
EXPORT_SYMBOL_GPL(smartpakit_register_dsm_info_ops);

void smartpakit_deregister_i2c_ctl_ops(void)
{
	if (smartpakit_priv_data == NULL) {
		hilog_err("%s: ioctl_ops deregister failed\n", __func__);
		return;
	}

	if (smartpakit_priv_data->i2c_num != 0) {
		hilog_debug("%s: skip deregister ioctl_ops\n", __func__);
		return;
	}

	smartpakit_priv_data->ioctl_ops = NULL;
	hilog_info("%s: ioctl_ops deregistered, success\n", __func__);
}
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	if ((pakit_priv->ioctl_ops == NULL) ||
		...
		hilog_err("%s: write_regs is invalid\n", __func__);
		return -ECHILD;
	}
...
int smartpakit_hw_reset_chips(struct smartpakit_priv *pakit_priv)
{
	...
	if ((pakit_priv->ioctl_ops == NULL) ||
		...
		hilog_err("%s: i2c_hw_reset ops is NULL\n", __func__);
		return -ECHILD;
	}
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (ret < 0) {
			hilog_err("%s: chip_id %d reset error\n", __func__, i);
			break;
		}
	}
...
int smartpakit_resume_chips(struct smartpakit_priv *pakit_priv)
{
	...
	if ((pakit_priv->ioctl_ops == NULL) ||
		(pakit_priv->ioctl_ops->write_regs == NULL)) {
		hilog_err("%s: i2c do_write_regs ops is NULL\n", __func__);
		return -ECHILD;
	}
...
int smartpakit_dump_chips(struct smartpakit_priv *pakit_priv)
{
	...
	if ((pakit_priv->ioctl_ops == NULL) ||
		...
		hilog_err("%s: i2c dump_regs ops is NULL\n", __func__);
		return -ECHILD;
	}
...
static bool smartpakit_is_param_valid(struct smartpakit_set_param *param)
{
	...
	if ((param->param_num == 0) ||
		...
		(param->params == NULL)) {
		hilog_err("%s: argument invalid, param num %u\n", __func__,
			param->param_num);
		return false;
	}

	if ((param->param_num % val_num) != 0) {
		hilog_err("%s: regs_num %u %% %u != 0, please check XML\n",
...
#ifdef CONFIG_HONOR_DEBUG
		hilog_info("%s: copy_from_user b64\n", __func__);
#endif
	...
	struct smartpakit_set_param *param, struct smartpakit_param_node **node)
{
	...
	if (ret < 0) {
		hilog_err("%s: get param head failed\n", __func__);
		return ret;
	}
	...
	if (copy_from_user(par_node, (void __user *)param->params, par_size)) {
		hilog_err("%s: get param data failed\n", __func__);
		kfree(par_node);
		return -EIO;
	}
	*node = par_node;
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: ctl_mask=0x%08x, ctl_num=%d\n",
		__func__, param->pa_ctl_mask, sequence->pa_ctl_num);
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++)
		hilog_info("%s: pa_ctl_index:%d %u\n",
	...
	unsigned int node_num)
{
	...
	for (i = 0; i < print_node_num; i++)
		hilog_info("%s: reg_%d=0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	if ((sequence->param_num == 0) || (sequence->node == NULL)) {
		hilog_err("%s: invalid input sequence\n", __func__);
		return -EINVAL;
	}
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	hilog_info("%s:mask=0x%x,poweron_flag=%u,ctl_num=%u,%u node per pa\n",
...
#ifdef CONFIG_HONOR_DEBUG
		hilog_info("%s: set mask 0x%x, index %u\n", __func__,
...
static int smartpakit_ctrl_open(struct inode *inode, struct file *filp)
{
	...
	if (ret < 0) {
		hilog_err("%s: nonseekable_open failed\n", __func__);
		return ret;
	}
	...
	size_t nbytes, loff_t *pos)
{
	...
	if ((file == NULL) || (buf == NULL)) {
		hilog_err("%s: invalid arg, file or buf is NULL\n", __func__);
		return -EINVAL;
	}
	...
	if ((nbytes == 0) || (nbytes > SMARTPAKIT_RW_PARAMS_NUM_MAX)) {
		hilog_err("%s: nbytes %d is 0 or >%u bytes, error\n", __func__,
...
int smartpakit_set_info(struct smartpa_vendor_info *vendor_info)
{
	...
	if ((vendor_info == NULL) ||
		...
		hilog_err("%s: vendor_info error\n", __func__);
		return -EINVAL;
	}
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s:vendor %u,chip_model %s\n", __func__,
...
static int smartpakit_get_self_dev_smartpa_cfg(struct smartpakit_priv *pakit_priv,
	struct smartpakit_info *info)
{
	struct smartpakit_i2c_priv *i2c_priv_p = NULL;
	unsigned int i;
	int ret = 0;
	if ((pakit_priv->dsm_info_ops == NULL) ||
		(pakit_priv->dsm_info_ops->smartpa_info == NULL))
		return 0;
	...
	for (i = 0; i < pakit_priv->pa_num; i++) {
		...
		if (i2c_priv_p == NULL)
			continue;
		ret += pakit_priv->dsm_info_ops->smartpa_info(i2c_priv_p, &(info->otp[i]));
	}
...
#ifdef CONFIG_HONOR_DSM_AUDIO
	if ((pakit_priv->dsm_info_ops != NULL) &&
		(pakit_priv->dsm_info_ops->dsm_report != NULL))
		pakit_priv->dsm_info_ops->dsm_report(pakit_priv);
#endif
	memset(&info, 0, sizeof(info));
	if (smartpakit_get_pa_info_from_dts_config(pakit_priv, &info) < 0)
		return -EINVAL;

	hilog_info("%s: soc_platform=%u, algo_in=%u, out_device=0x%08x\n",
		__func__, info.soc_platform, info.algo_in, info.out_device);
	hilog_info("%s: pa_num=%u, algo_delay_time=%u, chip_vendor=%u\n",
		__func__, info.pa_num, info.algo_delay_time, info.chip_vendor);
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: chip_model = %s, special_name_config = %s\n",
		__func__, info.chip_model, info.special_name_config);
#endif

	if (smartpakit_get_self_dev_smartpa_cfg(pakit_priv, &info) < 0)
		return -EINVAL;

	if (info.pa_num != pakit_priv->pa_num)
		hilog_info("%s: NOTICE I2C_NUM %u != PA_NUM %u\n", __func__,
			info.pa_num, pakit_priv->pa_num);

	if (copy_to_user((void *)arg, &info, sizeof(info))) {
		hilog_err("%s: copy kit info to user failed\n", __func__);
		return -EIO;
	}
	...
	int chip_id, struct list_head *cfg_list)
{
	...
	for (i = 0; i < node_num; i++) {
		...
		if (node_type == SMARTPAKIT_PARAM_NODE_TYPE_IRQ_FILTER) {
			...
			hilog_info("IRQ_FILTER,pa%d,reg 0x%x,m 0x%x, v 0x%x\n",
...
static void smartpakit_get_irq_filter_cfgs(struct smartpakit_priv *pakit_priv)
{
	...
	if ((pakit_priv == NULL) ||
		...
		hilog_err("%s: irq configs invalid\n", __func__);
		return;
	}
	...
	for (j = 0; j < sequence->pa_ctl_num; j++) {
		...
		if (ret < 0)
			hilog_err("%s: get pa%d irq filter err\n", __func__,
				chip_id);
	}
}
	...
	const void __user *arg, int compat_mode, unsigned int id)
{
	...
	if (!pakit_priv->resume_sequence_permission_enable) {
		hilog_err("%s: resume permission disable, skip\n", __func__);
		return -EINVAL;
	}
	...
	ret = smartpakit_get_reg_node_seq_for_multi_chips(sequence, arg,
		compat_mode);
	if (ret < 0) {
		hilog_err("%s: get legal init_regs failed\n", __func__);
		return -EINVAL;
	}
	...
	void __user *arg, unsigned int id)
{
	...
	if (copy_from_user(&reg, (void *)arg, sizeof(reg))) {
		hilog_err("%s: read reg copy_from_user failed\n", __func__);
		ret = EFAULT;
		goto err_out;
	}
	...
	if ((pakit_priv->ioctl_ops == NULL) ||
		...
		hilog_err("%s: ioctl_ops or read_regs is NULL\n", __func__);
		ret = -ECHILD;
		goto err_out;
	}
	...
	if (copy_to_user((void *)arg, &reg, sizeof(reg))) {
		hilog_err("%s: send reg value to user failed\n", __func__);
		ret = -EFAULT;
	}
err_out:
	mutex_unlock(&pakit_priv->i2c_ops_lock);
...
static void smartpakit_i2c_error_reset_and_resume_chip(struct smartpakit_priv *pakit_priv)
{
	if (pakit_priv->need_reset_resume_chip) {
		...
		hilog_info("pa i2c error need reset chip\n");
		...
		if (pakit_priv->i2c_errno == 0) {
			...
			hilog_info("pa i2c resume success\n");
		}
	}
}
...
int smartpakit_gpio_node_ctl(void *priv, struct smartpakit_param_node *reg)
{
	...
	if ((pakit_priv == NULL) || (pakit_priv->switch_ctl == NULL)) {
		hilog_info("%s: switch_ctl is invalid\n", __func__);
		return -EINVAL;
	}
	if (reg == NULL) {
		hilog_err("%s: param node reg is invalid\n", __func__);
		return -EINVAL;
	}

	if (reg->index >= pakit_priv->switch_num) {
		hilog_err("%s: Invalid arg, gpio index %u>= switch num %u\n",
	...
	const void __user *arg, int compat_mode, unsigned int id)
{
	...
	ret = smartpakit_parse_params(&sequence, arg, compat_mode);
	if (ret < 0) {
		hilog_err("%s: parse gpio_state failed\n", __func__);
		goto err_out;
	}
	...
	void __user *arg, int compat_mode)
{
	...
	if ((cmd != I2C_SLAVE) && (cmd != I2C_SLAVE_FORCE))
		hilog_debug("%s: enter, cmd 0x%x, compat_mode=%d\n", __func__,
			cmd, compat_mode);
	...
	case SMARTPAKIT_W_SECR:
		...
		if (smartpakit_ctrl_write_chips == NULL) {
			hilog_err("%s: write func invalid\n", __func__);
			ret = -EINVAL;
			break;
		}
	...
	case SMARTPAKIT_HW_UNPREPARE:
		...
		break;
	default:
		hilog_err("%s: not support cmd = 0x%x\n", __func__, cmd);
		ret = -EIO;
		break;
	}
	...
	if ((cmd != I2C_SLAVE) && (cmd != I2C_SLAVE_FORCE))
		hilog_info("%s: end, cmd:0x%x ret=%d\n", __func__, cmd, ret);
	...
	const char *propname, u32 **value, int *num)
{
	...
	if (count == 0) {
		hilog_debug("%s: %s not existed, skip\n", __func__, propname);
		return 0;
	}
	...
	ret = of_property_read_u32_array(node, propname, array,
		(size_t)(long)count);
	if (ret < 0) {
		hilog_err("%s: get %s array failed\n", __func__, propname);
		ret = -EFAULT;
		goto err_out;
	}
...
EXPORT_SYMBOL(smartpakit_get_prop_of_u32_array);
	...
	const char *key_str, unsigned int *type, bool is_requisite)
{
	...
	if (!of_property_read_bool(node, key_str)) {
		if (is_requisite)
			ret = -EINVAL;
		hilog_debug("%s: %s is not config in dts\n", __func__, key_str);
		return ret;
	}

	ret = of_property_read_u32(node, key_str, type);
	if (ret < 0)
		hilog_err("%s: get %s error\n", __func__, key_str);

	return ret;
}
EXPORT_SYMBOL(smartpakit_get_prop_of_u32_type);
...
EXPORT_SYMBOL(smartpakit_get_prop_of_str_type);
...
static void smartpakit_simple_pa_power_off(struct platform_device *pdev)
{
	...
	if ((pakit_priv == NULL) || (pakit_priv->switch_ctl == NULL) ||
		...
		hilog_err("%s: pakit_priv invalid arg\n", __func__);
		return;
	}
	...
	struct smartpakit_switch_node *ctl, unsigned int num)
{
	...
	if (!of_property_read_bool(node, gpio_state_default_str)) {
		hilog_info("%s: default prop not existed, skip\n", __func__);
		return 0;
	}
	...
	struct smartpakit_switch_node **switch_ctl, unsigned int num)
{
	...
	for_each_property_of_node(node, pp) {
		...
		if (ret < 0) {
			hilog_err("%s: set gpio name failed\n", __func__);
			ret = -EFAULT;
			goto err_out;
		}
		...
		if (ctl[i].gpio < 0) {
			hilog_info("%s: %s of_get_named_gpio failed, %d\n",
				__func__, pp->name, ctl[i].gpio);
			ret = of_property_read_u32(node, pp->name,
				(u32 *)&ctl[i].gpio);
			if (ret < 0) {
				hilog_err("%s: %s read gpio prop failed, %d\n",
...
static int smartpakit_init_switch_ctl_gpio(struct smartpakit_priv *pakit_priv)
{
	...
	for (i = 0; i < (int)pakit_priv->switch_num; i++) {
		hilog_info("%s: %d node gpio %d, state %d\n", __func__, i,
			ctl[i].gpio, ctl[i].state);
		if (ctl[i].gpio == 0) {
			hilog_info("%s: gpio is 0, return\n", __func__);
			ret = 0;
			goto err_out;
		}
		if (gpio_request((unsigned int)ctl[i].gpio, ctl[i].name) < 0) {
			hilog_err("%s: request gpio %d failed\n",
				__func__, ctl[i].gpio);
			ctl[i].gpio = 0;
			ret = -EFAULT;
			goto err_out;
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if (gpio_num == 0) {
		hilog_err("%s: switch_ctl gpio num is 0\n", __func__);
		return -EINVAL;
	}
	ret = smartpakit_save_switch_ctl_node(node, &pakit_priv->switch_ctl,
		gpio_num);
	if (ret < 0) {
		hilog_err("%s: save switch ctl node err\n", __func__);
		goto err_out;
	}
	...
	if (ret < 0) {
		hilog_err("%s: save default value err\n", __func__);
		goto err_out;
	}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if (node == NULL) {
		hilog_debug("%s: node not existed, skip\n", __func__);
		return 0;
	}
	ret = smartpakit_parse_switch_ctl_gpio(node, pakit_priv);
	if (ret < 0)
		hilog_err("%s: get switch ctl config err\n", __func__);
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if (support_irq_pinctrl == false) {
		hilog_debug("%s: node:%s not existed, skip\n",
	...
	if (IS_ERR_OR_NULL(pinctrl)) {
		hilog_err("%s: get pinctrl fail %d\n",
	...
	if (IS_ERR_OR_NULL(pin_default_state)) {
		ret = -EINVAL;
		hilog_err("%s: pinctrl_lookup_state fail %d\n",
	...
	if (pinctrl_select_state(pinctrl, pin_default_state) != 0) {
		ret = -EINVAL;
		hilog_err("%s: pinctrl_select_state fail\n", __func__);
		goto exit_parse_dt_irq_pinctrl;
	}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if (of_property_read_bool(dev->of_node, misc_rw_permission_str)) {
		...
		hilog_info("%s: prop existed\n", __func__);
	} else {
		hilog_debug("%s: prop not existed\n", __func__);
	}

#ifdef SMARTPAKIT_MISC_RW_PERMISSION_ENABLE
	pakit_priv->misc_rw_permission_enable = true;
	hilog_info("%s: config MISC_RW_PERMISSION_ENABLE in eng build\n",
		__func__);
#endif
	return 0;
}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if (of_property_read_bool(dev->of_node, type_str)) {
		...
		hilog_info("%s: %s prop existed\n", __func__, type_str);
	} else {
		hilog_debug("%s: %s not existed\n", __func__, type_str);
	}
	return 0;
}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_get_prop_of_u32_array(dev->of_node, out_device_str,
		&out_device, &count);
	if (ret < 0) {
		hilog_err("%s: get out device prop failed\n", __func__);
		return -EFAULT;
	}

	if ((count <= 0) || (count > SMARTPAKIT_PA_ID_MAX)) {
		hilog_err("%s: get pa_num failed %d, or pa_num %d>%d\n",
	...
	for (i = 0; i < count; i++) {
		if (out_device[i] >= SMARTPAKIT_OUT_DEVICE_MAX) {
			hilog_err("%s: out_device error %u>%d\n", __func__,
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: pa_num %u, out_device 0x%08x\n", __func__,
...
static int smartpakit_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		hilog_err("%s: parse dt_info failed, %d\n", __func__, ret);
		goto err_out;
	}

	ret = smartpakit_init_switch_ctl_gpio(pakit_priv);
	if (ret < 0) {
		hilog_err("%s: init switch ctl err, %d\n", __func__, ret);
		goto err_out;
	}
	...
	if (ret != 0) {
		hilog_err("%s: register miscdev failed, %d\n", __func__, ret);
		goto err_out;
	}
...
#ifdef CONFIG_SND_SOC_SELF_DEV_PA
	hilog_info("%s: register reg ops\n", __func__);
	self_dev_regulator_reg_ops_register(&self_dev_reg_ops);
#endif
...
static int smartpakit_remove(struct platform_device *pdev)
{
	...
	pakit_priv = platform_get_drvdata(pdev);
	if (pakit_priv == NULL) {
		hilog_err("%s: pakit_priv invalid argument\n", __func__);
		return -EINVAL;
	}
	platform_set_drvdata(pdev, NULL);
...
#else
	hilog_info("%s: platform_driver_register\n", __func__);
#endif
	ret = platform_driver_register(&smartpakit_driver);
	if (ret != 0)
		hilog_err("%s: platform_driver_register failed, %d\n",
			__func__, ret);
