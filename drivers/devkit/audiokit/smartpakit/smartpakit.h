...
struct smartpakit_i2c_priv {
	...
	/* self_dev_martpa otp info regs */
	struct smartpakit_reg_ctl_sequence *self_dev_smartpa_otp_sequence;

	/* reg map config */
	struct smartpakit_regmap_cfg *regmap_cfg;

	/* i2c_addr_state for self_dev_martpa */
...
struct self_dev_regulator_reg_ops {
	int (*self_dev_regulator_reg_read)(int i2c_addr, int reg_addr, int *value);
	int (*self_dev_regulator_reg_write)(int i2c_addr, int reg_addr, int mask,
		int value);
};
...
enum smartpakit_param_node_type {
	...
	SMARTPAKIT_PARAM_NODE_TYPE_AW_CORRECT_V1, /* set aw smartpa correction */
	SMARTPAKIT_PARAM_NODE_TYPE_AW_CORRECT_V2, /* set aw smartpa correction */
...
struct smartpakit_dsm_info_ops {
	void (*dsm_report)(struct smartpakit_priv *priv);
	int (*smartpa_info)(struct smartpakit_i2c_priv *i2c_priv, struct self_dev_smartpa_coeff *cfg);
};
...
int aw_smartpa_set_correction_v2(struct regmap *regmap,
	unsigned int value_factor, unsigned int xml_mask, unsigned int vcal_factor);
