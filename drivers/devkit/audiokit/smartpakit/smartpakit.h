...
struct smartpakit_gpio_reset {
	...
	bool skip_repeate_ops_in_probe;
	int gpio;
...
struct smartpakit_i2c_priv {
	...
	unsigned int pa_num_by_same_i2c_addr;
	unsigned int pa_iter;
...
struct smartpakit_high_power_config {
	unsigned int pwm_level;
	unsigned int voltage_mode;
	unsigned int pwm_type;
	bool pwm_enabled;
	struct smartpakit_pa_ctl_sequence power_on_seq;
	struct smartpakit_pa_ctl_sequence power_off_seq;
...
struct i2c_dev_ops_retry_info {
	bool retry_adapt_enable;
...
enum smartpakit_param_node_type {
	...
	SMARTPAKIT_PARAM_NODE_TYPE_PWM_CTL,
...
struct smartpakit_priv {
	...
	struct mutex rst_gpio_request_lock;
	struct mutex io_aggre_vote_lock;
	int io_aggre_vote_cnt;
	bool io_aggre_vote_state;
	...
	struct timer_list *timer;
	struct i2c_dev_ops_retry_info i2c_retry_info;
	struct simple_pa_id_info simple_pa_id;
	struct smartpakit_high_power_config high_power_config;
	bool io_aggre_support;
	bool read_version_reg_before_rst;
};
...
int smartpakit_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id);
