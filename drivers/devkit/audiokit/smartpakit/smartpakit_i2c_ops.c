...
#ifdef CONFIG_HONOR_DSM_AUDIO_MODULE
#define CONFIG_HONOR_DSM_AUDIO
#endif
#ifdef CONFIG_HONOR_DSM_AUDIO
#include <dsm_audio/dsm_audio.h>
#endif

#ifdef CONFIG_HONOR_DEVICEBOX_AUDIO_MODULE
#define CONFIG_HONOR_DEVICEBOX_AUDIO
#endif

#ifdef CONFIG_HONOR_DEVICEBOX_AUDIO
...
#define SELF_DEV_SMARTPA_OTP_REG_NUM      8
#define RETRY_TIMES                3
#define KIT_I2C_READ               0
#define KIT_I2C_WRITE              1

#ifdef CONFIG_SND_SOC_SELF_DEV_PA
int __attribute__((weak)) self_dev_pa_cfg_iomux(enum self_dev_pa_mode mode)
{
	return 0;
}
#endif
...
static void delete_list_info_by_given_chip_id(unsigned int chip_id)
{
	...
	if (pakit_priv == NULL) {
		hilog_err("%s: smartpakit_priv is NULL\n", __func__);
		return;
	}
	...
	unsigned int reg_addr, unsigned int mask, unsigned int read_addr)
{
	...
	if (ret < 0) {
		hilog_info("%s: read reg 0x%x failed, ret = %d\n",
	...
	value ^= mask;
#ifdef CONFIG_HONOR_DEBUG
	hilog_debug("%s: after xor 0x%x, write reg 0x%x = 0x%x\n", __func__,
		mask, reg_addr, value);
#endif
...
static void smartpakit_append_dsm_report(char *dst, char *fmt, ...)
{
	...
	if ((dst == NULL) || (fmt == NULL)) {
		hilog_err("%s, dst or src is NULL\n", __func__);
		return;
	}
...
void smartpakit_handle_i2c_probe_dsm_report(struct smartpakit_priv *pakit_priv)
{
	...
	hilog_info("%s: %s", __func__, report);

	audio_dsm_report_info(AUDIO_SMARTPA, DSM_SMARTPA_I2C_ERR, "%s",
		report);
	kfree(report);
}
EXPORT_SYMBOL(smartpakit_handle_i2c_probe_dsm_report);
#endif
...
#ifdef CONFIG_SND_SOC_SELF_DEV_PA
static void smartpakit_switch_self_dev_smartpa_gpio(const char *model, int state, unsigned int gpio, int i2c_state)
{
	int ret = 0;
	const char *self_dev_smartpa_str = "self_dev_pa";
	const char *self_dev_pa_type = "04-01"; // self_dev_martpa chip_model type

	if (strcmp(model, self_dev_smartpa_str) != 0 &&
		strcmp(model, self_dev_pa_type) != 0) {
		return;
	}

	if (state == GPIO_LOW) {
		/* config gpio mode */
		ret = self_dev_pa_cfg_iomux(RESET_MODE);
		if (ret < 0)
			hilog_err("RESET_MODE err, ret is %d\n", ret);
		gpio_direction_output(gpio, i2c_state);
		hilog_info("pa_cfg_iomux RESET_MODE\n");
	} else {
		/* recovery fuction mode */
		mdelay(1);
		ret = self_dev_pa_cfg_iomux(NORMAL_MODE);
		if (ret < 0)
			hilog_err("NORMAL_MODE err, ret is %d\n", ret);
		gpio_direction_input(gpio);
		hilog_info("pa_cfg_iomux NORMAL_MODE\n");
	}
}
#endif
	...
	struct smartpakit_gpio_sequence *sequence)
{
	...
	for (j = 0; j < num; j++) {
		hilog_info("%s: gpio %d set %u, delay %u\n", __func__, i2c_priv->hw_reset->gpio,
		...
		if (i2c_priv->irq_handler != NULL)
			smartpakit_switch_self_dev_smartpa_gpio(i2c_priv->chip_model,
					(int)node[j].state,
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	hilog_info("%s: pa%d software reset debounce, trigger delayed_work\n",
...
static int smartpakit_i2c_hw_reset(struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (i2c_priv->reset_debounce_wait_time > 0) {
		...
		hilog_info("%s: pa%d reset debounce, trigger delayed_work\n",
			__func__, i2c_priv->chip_id);
		i2c_priv->irq_debounce_jiffies = jiffies +
			msecs_to_jiffies(i2c_priv->reset_debounce_wait_time);
		schedule_delayed_work(&i2c_priv->irq_debounce_work,
	...
	struct smartpakit_reg_ctl *reg, struct list_head *dump_reg_list)
{
	...
	for (j = 0; j < (int)reg->value; j++) {
		...
		hilog_info("%s: pa %u, r reg 0x%x = 0x%x\n", __func__,
	...
	struct list_head *dump_reg_list)
{
	...
	for (i = 0; i < (int)sequence->num; i++) {
		...
		if (ctl_type == SMARTPAKIT_REG_CTL_TYPE_W) {
			...
			ret += ret_once;
			hilog_info("%s: pa %u, w reg 0x%x 0x%x\n", __func__,
...
{
	if (i2c_priv == NULL) {
		hilog_err("%s: i2c_priv NULL\n", __func__);
		return -EINVAL;
	}

	hilog_info("%s: pa %u, dump regs\n", __func__, i2c_priv->chip_id);
	return smartpakit_i2c_dts_reg_ops(i2c_priv,
		i2c_priv->dump_regs_sequence, NULL);
}

int smartpakit_i2c_get_self_dev_smartpa_info(struct smartpakit_i2c_priv *i2c_priv,
	struct self_dev_smartpa_coeff *cfg)
{
	struct smartpakit_reg_info *pos = NULL;
	unsigned char reg_val[SELF_DEV_SMARTPA_OTP_REG_NUM] = {0};
	struct list_head otp_req_list;
	int ret;
	int i = 0;

	if ((i2c_priv == NULL) || (cfg == NULL)) {
		hilog_err("%s: invalid input argument\n", __func__);
		return -EINVAL;
	}

	if (i2c_priv->self_dev_smartpa_otp_sequence == NULL)
		return 0;
	...
	hilog_debug("%s: pa %u, operate self_dev_smartpa otp regs\n", __func__,
		i2c_priv->chip_id);
	ret = smartpakit_i2c_dts_reg_ops(i2c_priv,
		i2c_priv->self_dev_smartpa_otp_sequence, &otp_req_list);
	if (ret < 0) {
		hilog_err("%s: read self_dev_smartpa_otp_regs err\n", __func__);
		return ret;
	}

	list_for_each_entry(pos, &otp_req_list, list) {
		if (i >= SELF_DEV_SMARTPA_OTP_REG_NUM) {
		    hilog_err("%s: self_dev_smartpa_otp_regs num err\n", __func__);
		    break;
		}
	...
	hilog_info("%s: pa%d, 0x%x, 0x%x, 0x%x, 0x%x\n", __func__,
		i2c_priv->chip_id, cfg->kv1, cfg->kv2, cfg->kr1, cfg->kr2);

	smartpakit_reg_info_del_list_all(&otp_req_list);
	return ret;
}
EXPORT_SYMBOL_GPL(smartpakit_i2c_get_self_dev_smartpa_info);
	...
	struct smartpakit_get_param *reg)
{
	...
	if (ret < 0)
		hilog_debug("%s: regmap_read failed ret = %d\n", __func__, ret);
	...
	struct smartpakit_param_node *reg, int index, unsigned int node_num)
{
	...
	case SMARTPAKIT_PARAM_NODE_TYPE_REG_READ:
		...
		hilog_info("%s: read node %d/%u, reg 0x%x = 0x%x, ret = %d\n",
			__func__, index, node_num, reg->index, value, ret);
		break;
	case SMARTPAKIT_PARAM_NODE_TYPE_AW_CORRECT_V1:
		ret = aw_smartpa_set_correction_v1(cfg->regmap, reg->index,
			reg->mask, reg->value);
		break;
	case SMARTPAKIT_PARAM_NODE_TYPE_AW_CORRECT_V2:
		ret = aw_smartpa_set_correction_v2(cfg->regmap, reg->index,
			reg->mask, reg->value);
		break;
	...
	unsigned int num, struct smartpakit_param_node *regs)
{
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: chip model %s, chip_id %u, node num %u\n", __func__,
		i2c_priv->chip_model, i2c_priv->chip_id, num);
#endif

	if ((num == 0) || (regs == NULL)) {
		hilog_err("%s: reg node is invalid\n", __func__);
		return -EINVAL;
	}

	if (!smartpakit_is_need_write_regs(i2c_priv, num, regs)) {
		hilog_info("%s: pa%d not need re-write the same regs\n",
			__func__, i2c_priv->chip_id);
		return 0;
	}
	pakit_priv = smartpakit_get_misc_priv();
	...
	for (i = 0; i < (int)num; i++) {
		...
		if (regs[i].node_type == SMARTPAKIT_PARAM_NODE_TYPE_GPIO) {
			...
			if (ret == 0)
				continue;
			hilog_err("%s: node %d, set gpio%u %u err\n", __func__,
		...
		if (ret < 0) {
			hilog_err("%s: node %d, reg 0x%x w/r %x err, ret %d\n",
...
struct smartpakit_dsm_info_ops dsm_info_ops = {
	.dsm_report   = smartpakit_handle_i2c_probe_dsm_report,
	.smartpa_info  = smartpakit_i2c_get_self_dev_smartpa_info,
};
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (i2c_priv_p == NULL) {
			hilog_err("%s: i2c_priv %d is NULL\n", __func__, i);
			break;
		}
	...
	hilog_info("%s: pa%u irq trigger, sync debounce time\n", __func__,
		i2c_priv->chip_id);
}
	...
	struct list_head *dump_reg_list, struct smartpakit_reg_info *cfg_info)
{
	...
	if ((!dump_reg_list) || (!cfg_info)) {
		hilog_err("%s: Input argument invalid\n", __func__);
		return false;
	}
	...
	list_for_each_entry(read_reg_info, dump_reg_list, list) {
		if ((read_reg_info->chip_id == cfg_info->chip_id) &&
			...
			if (read_val == cfg_val) {
				hilog_info("Filter trigger reset\n");
				return true;
			}
		}
	}
	...
	struct list_head *dump_reg_list)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		if ((pakit_priv->i2c_priv[i] == NULL) ||
			...
			hilog_err("%s: chip %d, check failed\n", __func__, i);
			break;
		}
	...
	struct list_head *dump_reg_list)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (!smartpakit_is_rw_sequence_exited(i2c_priv_p)) {
			hilog_info("%s: pa %d no rw_sequence\n", __func__, i);
			continue;
		}

		hilog_info("%s: pa %u, rw_sequence ctl\n", __func__,
			i2c_priv_p->chip_id);
		rw_sequence = i2c_priv_p->irq_handler->rw_sequence;
		ret = smartpakit_i2c_dts_reg_ops(i2c_priv_p, rw_sequence,
			dump_reg_list);
		if (ret < 0) {
			hilog_err("%s: pa %u, rw_sequence reg ctl failed\n",
...
void smartpakit_i2c_handler_irq(struct work_struct *work)
{
	...
	if (need_reset) {
		...
		hilog_info("%s: not need reset_chips, skip\n", __func__);
	}
...
irqreturn_t smartpakit_i2c_thread_irq(int irq, void *data)
{
	...
	if (i2c_priv->irq_debounce_jiffies > 0) {
		if (time_is_after_jiffies(i2c_priv->irq_debounce_jiffies)) {
			hilog_info("%s: debounce wait, skip this irq\n",
				__func__);
			return IRQ_HANDLED;
		}
	}
	...
	if (i2c_priv->probe_completed == 0) {
		hilog_err("%s: probe not completed, skip\n", __func__);
		return IRQ_HANDLED;
	}

	if (!work_busy(&i2c_priv->irq_handle_work)) {
		hilog_info("%s: schedule_work\n", __func__);
		schedule_work(&i2c_priv->irq_handle_work);
	} else {
		hilog_info("%s: work busy, skip\n", __func__);
	}
...
static void smartpakit_i2c_irq_debounce_work(struct work_struct *work)
{
	...
	hilog_info("%s: reset debounce, delayed_work enter\n", __func__);
	if (work == NULL) {
		hilog_err("%s: invalid argument\n", __func__);
		return;
	}
	...
	hilog_info("%s: pa%d reset debounce, delayed_work stop\n", __func__,
		i2c_priv->chip_id);
}
	...
	struct device *dev)
{
	...
	if ((i2c_priv == NULL) || (i2c_priv->regmap_cfg == NULL)) {
		hilog_err("%s: regmap_cfg invalid argument\n", __func__);
		return NULL;
	}
	...
	struct smartpakit_gpio_reset **reset)
{
	...
	if (ret < 0) {
		hilog_err("%s: set gpio_name failed\n", __func__);
		goto err_out;
	}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (node == NULL) {
		hilog_debug("%s: hw_reset node not existed, skip\n", __func__);
		return 0;
	}
	ret = smartpakit_i2c_parse_hw_reset_info(i2c_priv, node,
		&i2c_priv->hw_reset);
	if (ret < 0)
		hilog_err("%s: parse reset node err\n", __func__);
	...
	const char *ctl_str)
{
	...
	ret = smartpakit_get_prop_of_u32_array(node, ctl_str,
		(u32 **)&ctl->regs, &count);
	if ((count <= 0) || (ret < 0)) {
		hilog_err("%s: get %s failed or count is 0\n",
			__func__, ctl_str);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if ((count % val_num) != 0) {
		hilog_err("%s: count %d %% val_num %d != 0\n",
			__func__, count, val_num);
		ret = -EFAULT;
		goto err_out;
	}
	...
	struct smartpakit_gpio_irq **irq)
{
	...
	if (irq_info->gpio < 0) {
		hilog_debug("%s: of_get_named_gpio failed, %d\n", __func__,
			irq_info->gpio);
		ret = of_property_read_u32(node, gpio_irq_str,
			(u32 *)&irq_info->gpio);
		if (ret < 0) {
			hilog_err("%s: read gpio_irq failed, %d\n",
				__func__, ret);
			ret = -EFAULT;
			goto err_out;
		}
	...
	if (ret < 0) {
		hilog_err("%s: irq_handler get irq_flags failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	irq_info->need_filter_irq = of_property_read_bool(node,
		need_filter_irq_str);
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: need_reset = %d, need_filter_irq = %d\n", __func__,
	...
	if (ret < 0) {
		hilog_err("%s: set gpio_name/irq_name failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (of_property_read_bool(node, rw_sequence_str)) {
		...
		if (ret < 0) {
			hilog_err("%s: parse rw_sequence failed\n", __func__);
			goto err_out;
		}
	} else {
		hilog_debug("%s: rw_sequence not existed, skip\n", __func__);
	}
	*irq = irq_info;
	return 0;
err_out:
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (irq_handler == NULL) {
		hilog_info("%s: irq_handler not configed\n", __func__);
		return 0;
	}

	gpio = irq_handler->gpio;
	if (!gpio_is_valid(gpio)) {
		hilog_err("%s: irq_handler gpio %d invalid\n", __func__, gpio);
		return -EFAULT;
	}

	if (smartpakit_i2c_is_request_same_irq_gpio(gpio)) {
#ifdef CONFIG_HONOR_DEBUG
		hilog_info("%s: irq gpio %d had been requested, skip\n",
			__func__, gpio);
#endif
		return 0;
	}
	...
	if (ret < 0) {
		hilog_err("%s: gpio set GPIOF_DIR_IN failed\n", __func__);
		return -EFAULT;
	}

	irq_handler->irq = gpio_to_irq((unsigned int)gpio);
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: irq_handler get irq %d, irqflags=%u\n", __func__,
	...
	if (ret < 0) {
		hilog_err("%s: devm_request_threaded_irq failed\n", __func__);
		return -EFAULT;
	}
	return 0;
}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (node == NULL) {
		hilog_debug("%s: irq_handler not existed, skip\n", __func__);
		return 0;
	}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (!of_property_read_bool(i2c->dev.of_node, dump_regs_str)) {
		hilog_debug("%s: dump_regs not existed, skip\n", __func__);
		return 0;
	...
	if (ret < 0)
		hilog_err("%s: parse dump_regs failed\n", __func__);

	return ret;
}

static int smartpakit_i2c_parse_dt_self_dev_smartpa(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	const char *self_dev_smartpa_otp_regs_str = "self_dev_martpa_otp";
	const char *self_dev_smartpa_i2c_state_str = "i2c_addr_state";
	int ret;

	if (!of_property_read_bool(i2c->dev.of_node, self_dev_smartpa_otp_regs_str)) {
		hilog_debug("%s: self_dev_smartpa_otp not existed, skip\n", __func__);
		return 0;
	}
	ret = smartpakit_i2c_parse_reg_ctl(&i2c_priv->self_dev_smartpa_otp_sequence,
		i2c->dev.of_node, self_dev_smartpa_otp_regs_str);
	if (ret < 0) {
		hilog_err("%s: parse self_dev_smartpa_otp_regs failed\n", __func__);
		return -EFAULT;
	}

	ret = smartpakit_get_prop_of_u32_type(i2c->dev.of_node, self_dev_smartpa_i2c_state_str,
		&i2c_priv->i2c_addr_state, false);
	if (ret < 0) {
		hilog_err("%s: parse i2c_addr_state failed\n", __func__);
		return -EFAULT;
	}
	...
	struct smartpakit_regmap_cfg **cfg)
{
	...
	if (ret < 0) {
		hilog_err("%s: get reg_bits failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (ret < 0) {
		hilog_err("%s: get val_bits failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if ((ret < 0) || (cfg_info->cfg.cache_type > REGCACHE_FLAT)) {
		hilog_err("%s: get cache_type failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	if (ret < 0) {
		hilog_err("%s: get max_register failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (node == NULL) {
		hilog_debug("%s: regmap_cfg not existed, skip\n", __func__);
		return 0;
	}
	...
	if (cfg->num_defaults > 0) {
		if ((cfg->num_defaults % val_num) != 0) {
			hilog_err("%s: reg_defaults %d%%%d != 0\n",
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: regmap_cfg get w%d,%d,r%d,%d,v%d,%d,default%d\n",
	...
	if (IS_ERR(cfg->regmap)) {
		hilog_err("%s: regmap_init_i2c regmap failed\n", __func__);
		ret = -EFAULT;
		goto err_out;
	}
	return 0;
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (i2c_priv->chip_id >= SMARTPAKIT_PA_ID_MAX) {
		hilog_err("%s: invalid chip_id %u\n", __func__,
			i2c_priv->chip_id);
		return -EFAULT;
	}

	if (i2c_priv->chip_vendor >= SMARTPAKIT_CHIP_VENDOR_MAX) {
		hilog_err("%s: invalid chip_vendor %u\n", __func__,
...
#ifdef CONFIG_HONOR_DEBUG
		hilog_info("%s: sync irq debounce time enabled\n", __func__);
#endif
	}

#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: chip_vendor:%u,chip_id:%u,chip_model:%s\n", __func__,
...
{
	if (i2c_priv == NULL) {
		hilog_err("%s: i2c_priv invalid argument\n", __func__);
		return;
	}
	...
	smartpakit_i2c_free_reg_ctl(i2c_priv->self_dev_smartpa_otp_sequence);
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (ret < 0) {
		hilog_err("%s: request reset gpio err\n", __func__);
		goto err_out;
	}
...
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: pa %u, read version\n", __func__, i2c_priv->chip_id);
#endif
	ret = smartpakit_i2c_dts_reg_ops(i2c_priv, i2c_priv->version_regs_seq,
		NULL);
	if (ret < 0)
		hilog_err("%s: %s read version regs failed, %d\n", __func__,
	...
	const struct i2c_device_id *id)
{
	...
	if (smartpakit_get_misc_init_flag() == 0) {
		hilog_info("%s: this driver need probe_defer\n", __func__);
		return -EPROBE_DEFER;
	}
	...
	hilog_info("%s: device %s, addr=0x%x, flags=0x%x\n", __func__,
		id->name, i2c->addr, i2c->flags);
	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		hilog_err("%s: i2c check functionality error\n", __func__);
		return -ENODEV;
	}
	...
	if (smartpakit_i2c_probe_skip[i2c_priv->chip_id]) {
		hilog_info("%s: chip_id = %u has been probed success, skip\n",
			__func__, i2c_priv->chip_id);
		ret = -EINVAL;
		goto skip_probe;
	}
...
#ifdef CONFIG_HONOR_ARMPC_PLATFORM
	...
	ret += smartpakit_i2c_parse_dt_self_dev_smartpa(i2c, i2c_priv);
	if (ret < 0) {
		smartpakit_i2c_free_hw_reset(i2c_priv->hw_reset);
		goto err_out;
	}
	...
	smartpakit_register_dsm_info_ops(&dsm_info_ops);
	...
	hilog_info("%s: end success\n", __func__);
	return 0;

err_out:
	hilog_err("%s: end failed\n", __func__);
skip_probe:
...
static void smartpakit_i2c_shutdown(struct i2c_client *i2c)
{
	...
	hilog_info("%s: shutdown for smartpakit_i2c\n", __func__);
	if (i2c == NULL) {
		hilog_err("%s: invalid input argument\n", __func__);
		return;
	}
	...
	if ((i2c_priv->hw_reset == NULL) ||
		...
		hilog_info("%s: hw_reset is invalid\n", __func__);
		return;
	}

	/* for shutdown in music playing */
	if (i2c_priv->hw_reset->not_need_shutdown) {
		hilog_info("%s: not need shutdown, skip\n", __func__);
		return;
	}
	...
	for (i = (int)(sequence->num - 1); i >= 0; i--) {
		...
		hilog_info("%s: set gpio %d %u, delay %u\n", __func__,
...
static int smartpakit_i2c_resume(struct device *dev)
{
	...
	if (irq_handler != NULL) {
		...
		hilog_info("%s: end and enable irq : %d\n",
					__func__, irq_handler->irq);
	}
#endif
	return 0;
...
static const struct of_device_id smartpakit_i2c_match[] = {
	{ .compatible = "honor,smartpakit_i2c", },
	{},
};
...
{
#ifdef CONFIG_HONOR_SMARTPAKIT_AUDIO_MODULE
	hilog_info("%s enter from modprobe\n", __func__);
#endif
