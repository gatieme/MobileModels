...
#include <platform_include/audio/sound/au_pa.h>
#endif
...
#define TIME_COUNT_FOR_DMD 5
#define TIME_DIFF 60 // 60s

#define POLYMERIC_CHIP_STATUS_REG 0x25f0054
#define MAX_CHIP_ID 2

#define MAX_DELAY_PERIOD 1000

#ifdef CONFIG_SND_SOC_AU_PA
int __attribute__((weak)) au_pa_cfg_iomux(enum au_pa_mode mode)
{
	return 0;
}
#endif
...
static bool smartpakit_i2c_probe_skip[SMARTPAKIT_PA_ID_MAX] = { 0, 0, 0, 0, 0, 0, 0, 0 };
static bool smartpakit_i2c_ops_gpio_reset_status = false;

static int get_pa_node(struct i2c_client *i2c,
...
static int smartpakit_i2c_dts_reg_ops(struct smartpakit_i2c_priv *i2c_priv,
	...
	struct list_head *dump_reg_list, bool report_dsm);
	...
	unsigned int reg_addr, unsigned int *value)
{
	...
	unsigned int retry_delay = 1; // DEFULT delay 1ms

	pakit_priv = smartpakit_get_misc_priv();
	if (pakit_priv != NULL) {
		if (pakit_priv->i2c_retry_info.retry_adapt_enable) {
			retry = pakit_priv->i2c_retry_info.retry_time;
			retry_delay = pakit_priv->i2c_retry_info.retry_delay;
		}
	}
...
static void smartpakit_delay(unsigned int delay, int index, int num)
{
	...
	delay = delay > MAX_DELAY_PERIOD ? MAX_DELAY_PERIOD : delay;
...
static void io_die_bug_report(void)
{
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	int ret;
	int chip_id;
	...
	for (chip_id = 0; chip_id < MAX_CHIP_ID; chip_id++) {
		...
		data.val = 0;
		ret = sts_reg_op(&data);
		hwlog_info("%s: chip_id %d, ret = %d, val = %u\n",
			__func__, chip_id, ret, data.val);
	}
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
}
...
#ifdef CONFIG_SND_SOC_AU_PA
static bool smartpakit_is_au_smartpa(const char *model)
{
	const char *hismartpa_str = "hi6565";
	const char *au_pa_type = "04-01"; // hismartpa chip_model type

	if (strcmp(model, hismartpa_str) != 0 &&
		strcmp(model, au_pa_type) != 0) {
		return false;
	}
	return true;
}
...
{
	int ret = 0;
	if (!smartpakit_is_au_smartpa(model))
		return;

	if (state == GPIO_LOW) {
		/* config gpio mode */
		ret = au_pa_cfg_iomux(RESET_MODE);
		if (ret < 0)
			hwlog_err("RESET_MODE err, ret is %d\n", ret);
		gpio_direction_output(gpio, i2c_state);
		hwlog_info("au_pa_cfg_iomux RESET_MODE\n");
	} else {
		/* recovery fuction mode */
		mdelay(1);
		ret = au_pa_cfg_iomux(NORMAL_MODE);
		if (ret < 0)
			hwlog_err("NORMAL_MODE err, ret is %d\n", ret);
		gpio_direction_input(gpio);
		hwlog_info("au_pa_cfg_iomux NORMAL_MODE\n");
	}
}

static void smartpakit_switch_hismartpa_gpio_to_idle(const char *model)
{
	if (!smartpakit_is_au_smartpa(model))
		return;

	if (au_pa_cfg_iomux(LOWPOWER_MODE) < 0)
		hwlog_err("LOWPOWER_MODE err\n");

	hwlog_info("au_pa_cfg_iomux LOWPOWER_MODE\n");
}
#endif

static bool smartpa_i2c_skip_repeat_ops_reset_gpio_in_probe(
	struct smartpakit_i2c_priv *i2c_priv,
	...
	int node_num)
{
	...
	if (!(i2c_priv->hw_reset->skip_repeate_ops_in_probe))
		return false;

	/* it need ops reset gpio first */
	if (!smartpakit_i2c_ops_gpio_reset_status)
		return false;

	/* if probe compelte, it need ops reset gpio in irq sence.etc */
	if (smartpakit_i2c_probe_skip[i2c_priv->chip_id])
		return false;

	gpio_status = gpio_get_value((unsigned int)i2c_priv->hw_reset->gpio);
	if (gpio_status == (int)node[node_num - 1].state) {
		hwlog_info("%s: hw_reset gpio already in normal status, skip it\n",
			__func__);
		return true;
	}
...
static void smartpa_i2c_save_ops_reset_gpio_status_in_probe(
	struct smartpakit_i2c_priv *i2c_priv)
{
	if (!(i2c_priv->hw_reset->skip_repeate_ops_in_probe))
		return;
	...
	if (!smartpakit_i2c_ops_gpio_reset_status)
		smartpakit_i2c_ops_gpio_reset_status = true;
}
	...
	struct smartpakit_gpio_sequence *sequence)
{
	...
	struct smartpakit_priv *pakit_priv = NULL;
	struct list_head version_req_list;
	int j;
	int num;
	...
	if (smartpa_i2c_skip_repeat_ops_reset_gpio_in_probe(i2c_priv,
		node, num)) {
		hwlog_info("%s: skip reset gpio ops\n", __func__);
		return;
	}
	...
	if (i2c_priv->version_regs_seq == NULL)
		return;
	INIT_LIST_HEAD(&version_req_list);

	for (j = 0; j < num; j++) {
		if (pakit_priv->read_version_reg_before_rst) {
			if (i2c_priv->first_read_version_reg)
				smartpakit_i2c_dts_reg_ops(i2c_priv, i2c_priv->version_regs_seq,
					&version_req_list, false);
		}
	...
	i2c_priv->first_read_version_reg = false;

	smartpa_i2c_save_ops_reset_gpio_status_in_probe(i2c_priv);
}
...
static int kit_i2c_dts_regs_read(struct smartpakit_i2c_priv *i2c_priv,
	struct smartpakit_reg_ctl *reg, struct list_head *dump_reg_list, bool report_dsm)
{
	...
	for (j = 0; j < (int)reg->value; j++) {
		...
		if (report_dsm)
			smartpakit_i2c_dsm_report(i2c_priv, KIT_I2C_READ, ret_once, NULL);
	...
	for (i = 0; i < (int)sequence->num; i++) {
		...
		if (ctl_type == SMARTPAKIT_REG_CTL_TYPE_W) {
			ret_once = smartpakit_regmap_write(regmap, reg_addr,
				ctl_value);
			if (report_dsm)
				smartpakit_i2c_dsm_report(i2c_priv, KIT_I2C_WRITE, ret_once, NULL);
			...
			if (ctl_value > 0) { /* delay time units: msecs */
				ctl_value = ctl_value > MAX_DELAY_PERIOD ? MAX_DELAY_PERIOD : ctl_value;
				msleep(ctl_value);
			}
		} else { /* SMARTPAKIT_REG_CTL_TYPE_R */
			ret += kit_i2c_dts_regs_read(i2c_priv,
				&sequence->regs[i], dump_reg_list, report_dsm);
		}
	}
...
static int smartpakit_i2c_dump_regs(struct smartpakit_i2c_priv *i2c_priv)
{
	...
	return smartpakit_i2c_dts_reg_ops(i2c_priv,
		i2c_priv->dump_regs_sequence, NULL, false);
}
	...
	struct hismartpa_coeff *cfg)
{
	...
	ret = smartpakit_i2c_dts_reg_ops(i2c_priv,
		i2c_priv->hismartpa_otp_sequence, &otp_req_list, true);
	if (ret < 0) {
	...
	struct smartpakit_param_node *reg, int index, unsigned int node_num)
{
	int ret = 0;
	int value = 0;
	unsigned int delay = reg->delay > MAX_DELAY_PERIOD ? MAX_DELAY_PERIOD : reg->delay;
	...
	case SMARTPAKIT_PARAM_NODE_TYPE_PWM_CTL:
		break;
	...
	default: /* SMARTPAKIT_PARAM_NODE_TYPE_REG_WRITE 1 */
		ret = smartpakit_regmap_complex_write(cfg, reg->index,
			reg->mask, reg->value);
		smartpakit_delay(delay, index, (int)node_num - 1);
		break;
	}
	return ret;
}
	...
	struct list_head *dump_reg_list, struct smartpakit_reg_info *cfg_info)
{
	...
	if ((!dump_reg_list) || (!cfg_info)) {
		hwlog_err("%s: Input argument invalid, not reset\n", __func__);
		return false;
	}
	...
	list_for_each_entry(read_reg_info, dump_reg_list, list) {
		if ((read_reg_info->chip_id == cfg_info->chip_id) &&
			...
			if (read_val) {
				hwlog_info("%s:pa%d,reg:0x%x,info:0x%x,mask:0x%x\n",
					__func__, read_reg_info->chip_id,
					read_reg_info->reg_addr,
					read_reg_info->info, cfg_info->mask);
				hwlog_info("%s:Filter trigger reset\n",
					__func__);
				return true;
			}
		}
	...
	struct list_head *dump_reg_list)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		if (smartpakit_high_power_is_power_down(pakit_priv,
			i2c_priv_p)) {
			hwlog_warn("%s: high power pa power down, skip\n",
				__func__);
			continue;
		}
		...
		ret = smartpakit_i2c_dts_reg_ops(i2c_priv_p, rw_sequence, dump_reg_list, false);
		if (ret < 0) {
...
#if IS_ENABLED(CONFIG_HUAWEI_DSM_AUDIO)
bool record_i2c_irq_time(void)
{
	struct timespec64 tv;
	static int time_num = 0;
	static long time_buf[TIME_COUNT_FOR_DMD] = {0};
	int i;

	ktime_get_real_ts64(&tv);

	if (time_num >= TIME_COUNT_FOR_DMD)
		time_num = 0;

	hwlog_info("%s tv.tv_sec %ld time_buf %ld time_num %d", __func__, tv.tv_sec,
		time_buf[time_num], time_num);
	if ((time_buf[time_num] != 0) && (tv.tv_sec - time_buf[time_num] < TIME_DIFF)) {
		for (i = 0; i < TIME_COUNT_FOR_DMD; i++)
			time_buf[i] = 0;

		hwlog_info("%s dmd report %d", __func__, time_num);
		return true;
	}
	time_buf[time_num] = tv.tv_sec;
	time_num++;
	return false;
}
#endif
...
void smartpakit_i2c_handler_irq(struct work_struct *work)
{
	...
	pakit_priv = i2c_priv->priv_data;
	if (i2c_priv->irq_handler->need_skip_probe_irq) {
		if (pakit_priv->pa_num != pakit_priv->i2c_num) {
			hwlog_info("%s: I2C_NUM != PA_NUM, invalid irq\n", __func__);
			return;
		}
	}
...
{
	if (hw_reset == NULL || *hw_reset == NULL)
		return;
	...
	struct smartpakit_gpio_reset **reset)
{
	...
	const char *skip_repeate_ops_in_probe = "skip_repeate_ops_in_probe";
	...
	reset_info->skip_repeate_ops_in_probe = of_property_read_bool(node,
		skip_repeate_ops_in_probe);
	hwlog_debug("%s: skip_repeate_ops_in_probe = %d\n", __func__,
		(int)reset_info->skip_repeate_ops_in_probe);
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	struct device_node *pa_node = NULL;
	int ret;

	ret = get_pa_node(i2c, i2c_priv, &pa_node);
	if (ret < 0)
		return ret;

	node = of_get_child_by_name(pa_node, hw_reset_str);
	if (node == NULL) {
	...
	struct smartpakit_gpio_irq **irq)
{
	...
	const char *need_skip_irq_str   = "need_skip_probe_irq";
	...
	irq_info->need_skip_probe_irq = of_property_read_bool(node, need_skip_irq_str);
	*irq = irq_info;
	return 0;
err_out:
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	node = of_get_child_by_name(pa_node, irq_handler_str);
	if (node == NULL) {
...
static int smartpakit_i2c_parse_dt_version_regs(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	const char *version_val_str = "version_val";
	...
	if (of_property_read_bool(pa_node, version_val_str)) {
		ret = smartpakit_i2c_parse_reg_ctl(&i2c_priv->version_val_check_seq,
				pa_node, version_val_str);
		if (ret < 0) {
			hwlog_err("%s: version_val get fail\n", __func__);
			return ret;
		}
		i2c_priv->need_version_check = true;
	} else {
		hwlog_debug("%s: version_val not existed, skip\n", __func__);
		i2c_priv->need_version_check = false;
	}

	if (!of_property_read_bool(pa_node, version_regs_str)) {
		hwlog_debug("%s: version_regs not existed, skip\n", __func__);
		return 0;
	}
	ret = smartpakit_i2c_parse_reg_ctl(&i2c_priv->version_regs_seq,
		pa_node, version_regs_str);
	if (ret < 0)
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (!of_property_read_bool(pa_node, hismartpa_otp_regs_str)) {
		hwlog_debug("%s: hismartpa_otp not existed, skip\n", __func__);
		return 0;
	}
	ret = smartpakit_i2c_parse_reg_ctl(&i2c_priv->hismartpa_otp_sequence,
		pa_node, hismartpa_otp_regs_str);
	if (ret < 0) {
		hwlog_err("%s: parse hismartpa_otp_regs failed\n", __func__);
		return -EFAULT;
	}

	ret = smartpakit_get_prop_of_u32_type(pa_node, hismartpa_i2c_state_str,
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	node = of_get_child_by_name(pa_node, regmap_cfg_str);
	if (node == NULL) {
	...
	cfg->regmap = devm_regmap_init_i2c(i2c, &cfg->cfg);
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	const char *high_power_pa_str = "high_power_pa";
	struct device_node *pa_node = NULL;
	int ret;
	...
	ret = smartpakit_get_prop_of_u32_type(pa_node, chip_vendor_str,
		&i2c_priv->chip_vendor, true);
	ret += smartpakit_get_prop_of_u32_type(pa_node, chip_id_str,
		&i2c_priv->chip_id, true);
	ret += smartpakit_get_prop_of_str_type(pa_node, chip_model_str,
		&i2c_priv->chip_model);

	ret += smartpakit_get_prop_of_u32_type(pa_node, wait_time_str,
		&i2c_priv->reset_debounce_wait_time, false);

	ret += smartpakit_get_prop_of_u32_type(pa_node, i2c_addr_str,
	...
	if (of_property_read_bool(pa_node, irq_debounce_str)) {
	...
	hwlog_info("%s: chip_vendor:%u,chip_id:%u,chip_model:%s\n", __func__,
		...
		i2c_priv->chip_model);
#endif
	i2c_priv->high_power_pa = of_property_read_bool(pa_node,
		high_power_pa_str);
	return 0;
...
static void smartpakit_i2c_reset_hw_reset_data(
	struct smartpakit_i2c_priv *i2c_priv)
{
	int gpio;
	...
	if (smartpakit_i2c_is_request_same_reset_gpio(gpio))
		return;

	gpio_free(gpio);
	i2c_priv->hw_reset->gpio = -EINVAL;

	hwlog_info("%s: free reset_gpio\n", __func__);
	if (i2c_priv->reset_debounce_wait_time > 0)
		cancel_delayed_work_sync(&i2c_priv->irq_debounce_work);
}

static void smartpakit_i2c_reset_irq_handler_data(
	struct smartpakit_i2c_priv *i2c_priv)
{
	int gpio;
	...
	devm_free_irq(i2c_priv->dev,
		(unsigned int)(i2c_priv->irq_handler->irq),
		(void *)i2c_priv);

	devm_gpio_free(i2c_priv->dev, gpio);
	i2c_priv->irq_handler->gpio = -EINVAL;
	hwlog_info("%s: free irq gpio\n", __func__);
}

static void smartpakit_i2c_reset_priv_data(
	struct smartpakit_i2c_priv *i2c_priv)
{
	smartpakit_i2c_reset_hw_reset_data(i2c_priv);
	smartpakit_i2c_free_hw_reset(&i2c_priv->hw_reset);
	smartpakit_i2c_reset_irq_handler_data(i2c_priv);
	smartpakit_i2c_free_irq_handler(&(i2c_priv->irq_handler));

	smartpakit_i2c_free_reg_ctl(&i2c_priv->version_regs_seq);
	smartpakit_i2c_free_reg_ctl(&i2c_priv->version_val_check_seq);
	...
	i2c_priv->first_read_version_reg = true;
}
...
#ifdef CONFIG_SND_SOC_AU_PA
	smartpakit_switch_hismartpa_gpio_to_idle(i2c_priv->chip_model);
#endif
...
static bool smartpakit_check_version_value_sequence(
	struct smartpakit_i2c_priv *i2c_priv,
	struct list_head *version_req_list)
{
	int i;
	...
	unsigned int mask;
	struct smartpakit_reg_info *pos = NULL;
	struct smartpakit_reg_ctl_sequence *version_val_check_seq =
			i2c_priv->version_val_check_seq;
	struct smartpakit_reg_ctl *version_val_regs = NULL;

	if (version_val_check_seq == NULL) {
		hwlog_info("%s: version_val_check_seq is NULL\n", __func__);
		return false;
	}

	version_val_regs = version_val_check_seq->regs;
	if (version_val_regs == NULL) {
		hwlog_info("%s: version_val_regs is NULL\n", __func__);
		return false;
	}

	list_for_each_entry(pos, version_req_list, list) {
		hwlog_info("%s:%s, pa_addr 0x%x, pa %u, "
			"reg_addr 0x%x, reg_value 0x%x\n",
			__func__, i2c_priv->chip_model, i2c_priv->i2c->addr,
			pos->chip_id, pos->reg_addr, pos->info);
		for (i = 0; i < version_val_check_seq->num; i++) {
			/* notice: ctl_type use as mask */
			mask = version_val_regs[i].ctl_type;
			if ((pos->reg_addr == version_val_regs[i].addr) &&
					((pos->info & mask) == version_val_regs[i].value)) {
				match_count++;
				hwlog_info("%s:%s, pa %u, reg_addr 0x%x, "
					"value 0x%x, mask 0x%x, match_count:%d\n",
					__func__, i2c_priv->chip_model,
					i2c_priv->chip_id,
					version_val_regs[i].addr,
					version_val_regs[i].value,
					mask, match_count);
				break;
			}
		}
	...
	if (match_count == version_val_check_seq->num) {
		hwlog_info("%s:%s, pa_addr 0x%x, pa %u, "
			"match_count:%d, the version is matched\n",
			__func__, i2c_priv->chip_model, i2c_priv->i2c->addr,
			i2c_priv->chip_id, match_count);
		return true;
	}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	ret = smartpakit_i2c_dts_reg_ops(i2c_priv, i2c_priv->version_regs_seq,
		&version_req_list, true);
	if (ret < 0) {
		hwlog_err("%s: %s, pa %u read version regs failed, %d\n",
			__func__, i2c_priv->chip_model, i2c_priv->chip_id, ret);
		goto read_chip_version_err;
	}

	if (i2c_priv->need_version_check) {
		version_check = smartpakit_check_version_value_sequence(i2c_priv,
			&version_req_list);
		if (!version_check) {
			hwlog_err("%s: version check error\n", __func__);
			ret = -EINVAL;
		}
	}

	smartpakit_reg_info_del_list_all(&version_req_list);
read_chip_version_err:
	return ret;
}
	...
	struct smartpakit_i2c_priv *i2c_priv, struct device_node **node)
{
	...
	if (i2c_priv->pa_num_by_same_i2c_addr == 1) {
		*node = i2c->dev.of_node;
		return 0;
	}

	snprintf(name, SMARTPAKIT_NAME_MAX, "smartpa%d", i2c_priv->pa_iter);
...
static int smartpa_enumrate(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	int ret;
	unsigned int i;

	ret = of_property_read_u32(i2c->dev.of_node, "pa_num_by_same_i2c_addr",
		&i2c_priv->pa_num_by_same_i2c_addr);
	if (ret < 0)
		i2c_priv->pa_num_by_same_i2c_addr = 1;

	for (i = 0; i < i2c_priv->pa_num_by_same_i2c_addr; i++) {
		i2c_priv->pa_iter = i;
		...
		if (smartpakit_i2c_probe_skip[i2c_priv->chip_id]) {
			hwlog_info("%s: chip_id = %u has been probed success, skip\n",
				__func__, i2c_priv->chip_id);
			return SMARTPA_ALREADY_ENUMRATED_NEED_SKIP;
		}
		...
		return SMARTPA_ENUMRATE_SUCCED;
	}

	return ret;
}

int smartpakit_i2c_probe(struct i2c_client *i2c,
	const struct i2c_device_id *id)
{
	...
	rst_gpio_request_lock();
	io_aggre_vote();
	ret = smartpa_enumrate(i2c, i2c_priv);
	io_aggre_unvote();
	if (ret == SMARTPA_ALREADY_ENUMRATED_NEED_SKIP) {
		ret = -EINVAL;
		goto skip_probe;
	} else if (ret < 0) {
		goto err_out;
	}
#ifndef NEW_7425_BASELINE
	rst_gpio_request_unlock();
#endif
...
int smartpakit_i2c_remove(struct i2c_client *i2c)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
...
void smartpakit_i2c_shutdown(struct i2c_client *i2c)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
	struct smartpakit_gpio_sequence *sequence = NULL;
	unsigned int gpio;
...
#ifdef CONFIG_PM
int smartpakit_i2c_suspend(struct device *dev)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
...
int smartpakit_i2c_resume(struct device *dev)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
