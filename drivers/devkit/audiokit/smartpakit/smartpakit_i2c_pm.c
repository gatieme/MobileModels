...
int smartpakit_i2c_freeze(struct device *dev)
{
	...
	if (irq_handler != NULL) {
		...
		hilog_info("%s: end and disable irq : %d\n",
...
void smartpakit_restore_process(struct work_struct *work)
{
	...
	if (work == NULL) {
		hilog_err("%s: work null invalid argument\n", __func__);
		return;
	}
	...
	if (i2c_priv == NULL || i2c_priv->priv_data == NULL) {
		hilog_err("%s:i2c_priv or priv_data null.\n", __func__);
		return;
	}
	...
	if (ret != 0) {
		hilog_err("%s: smartpakit_hw_reset_chips error[%d]\n",
					__func__, ret);
		goto err_out;
	}
	ret = smartpakit_resume_chips(pakit_priv);
	if (ret != 0) {
		hilog_err("%s: smartpakit_resume_chips error[%d]\n",
					__func__, ret);
		goto err_out;
	}
...
int smartpakit_i2c_restore(struct device *dev)
{
	...
	if (!work_busy(&i2c_priv->pm_s4_work)) {
		hilog_info("%s: pm_s4_work schedule_work enter\n", __func__);
