...
{
	if (bias_bl_ops) {
		FASTBOOT_INFO("bias_bl_ops has already been registered!\n");
		return;
	}
	if (ops == NULL) {
		FASTBOOT_INFO("bias_bl_ops register NULL!\n");
		return;
	}
	bias_bl_ops = ops;
}
...
{
	if (ops == NULL) {
		FASTBOOT_INFO("bias_bl_ops unregister NULL!\n");
		return;
	}
	if (bias_bl_ops == ops) {
		bias_bl_ops = NULL;
		FASTBOOT_INFO("bias_bl_ops unregister success!\n");
		return;
	}
	FASTBOOT_INFO("bias_bl_ops unregister fail!\n");
}
