...
#if ((defined MTK_LCD_KIT_SUPPORT) || (defined CONFIG_LCD_KIT))
...
int lcd_kit_ext_ldo_power_ctrl(int enable)
{
	...
	switch (power_hdl->lcd_ext_ldo.buf[0]) {
	case GPIO_MODE:
		if (enable == LCD_RESET_HIGH) {
			if (adapt_ops->gpio_enable)
				adapt_ops->gpio_enable(LCD_KIT_EXT_LDO);
		} else {
			if (adapt_ops->gpio_disable)
				adapt_ops->gpio_disable(LCD_KIT_EXT_LDO);
		}
		break;
	default:
		LCD_KIT_ERR("not support mode\n");
...
int lcd_kit_power_on_early(void *hld)
{
	int i;
	int ret = LCD_KIT_OK;
	struct lcd_kit_array_data *pevent = NULL;

	pevent = power_seq->lk_power_on_early_seq.arry_data;
	for (i = 0; i < power_seq->lk_power_on_early_seq.cnt; i++) {
		if ((!pevent) || (!pevent->buf)) {
			LCD_KIT_ERR("on pevent is null!\n");
			return LCD_KIT_FAIL;
		}
...
static int lcd_kit_parse_power(const char *np)
{
	...
	if (power_hdl->lcd_ext_ldo.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-ext-ldo",
...
struct lcd_kit_common_ops g_lcd_kit_common_ops = {
	...
	.power_on_early = lcd_kit_power_on_early,
};
