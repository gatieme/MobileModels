...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(i2c_info.id, i2c_info.addr, val, len, len, NULL);
#else
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(i2c_info.id, i2c_info.addr, write_data, len, NULL);
#else
	i2c_info.mode = ST_MODE;
	i2c_info.speed = MTK_I2C_STD_SPEED;
