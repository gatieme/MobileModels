...
static int ktd3133_i2c_read_u8(unsigned char addr, unsigned char *dataBuffer)
{
	...
	*dataBuffer = addr;
	...
	len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(ktd3133_i2c.id, ktd3133_i2c.addr, dataBuffer, len, len, NULL);
#else
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(ktd3133_i2c.id, ktd3133_i2c.addr, write_data, len, NULL);
#else
	ktd3133_i2c.mode = ST_MODE;
	ktd3133_i2c.speed = 100;
...
static void ktd3133_enable(void)
{
	...
	if (ktd3133_bl_info.ktd3133_hw_en) {
		lcdkit_gpio_set_mode(ktd3133_bl_info.ktd3133_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(ktd3133_bl_info.ktd3133_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(ktd3133_bl_info.ktd3133_hw_en_gpio,
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void ktd3133_set_backlight_status(void *fdt)
#else
...
int ktd3133_init(void)
{
	...
	if (ktd3133_bl_info.ktd3133_hw_en) {
		ret = lcd_kit_parse_get_u32_default(DTS_COMP_KTD3133,
			KTD3133_HW_EN_GPIO,
			(u32 *)&ktd3133_bl_info.ktd3133_hw_en_gpio, 0);
		if (ret < 0) {
