/* Copyright (c) 2019-2019, Honor Device Tech. Co., Ltd. All rights reserved.
*
...
static int ktz8864_2_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	int ret = FAIL;
	/* read date default length */
	unsigned char len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	struct mt_i2c ktz8864_i2c = {0};
#else
	...
	*data_buffer = addr;
	ktz8864_i2c.id = bl_info.ktz8864_2_i2c_bus_id;
	ktz8864_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(ktz8864_i2c.id, ktz8864_i2c.addr, data_buffer, len, len, NULL);
#else
...
static int ktz8864_2_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	int ret;
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(ktz8864_i2c.id, ktz8864_i2c.addr, write_data, len, NULL);
#else
	ktz8864_i2c.mode = ST_MODE;
	ktz8864_i2c.speed = KTZ8864_I2C_SPEED;
	...
	if (ret != 0) {
		...
		LCD_KIT_ERR("i2c_write slave reg is  0x%x  value %d\n",
			addr, value);
	}
	return ret;
}
...
static int ktz8864_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	...
	if (bl_info.dual_ic) {
	     if (addr == REG_DISPLAY_BIAS_CONFIG_1)
			ktz8864_2_i2c_write_u8(chip_no, addr, bl_info.ktz8864_2_display_bias_cfg_1);
		 else
		    ktz8864_2_i2c_write_u8(chip_no, addr, value);;
	}
...
static int ktz8864_2_i2c_update_bits(u8 chip_no, int reg, u8 mask, u8 val)
{
	int ret;
	u8 tmp;
	u8 orig = 0;

	ret = ktz8864_2_i2c_read_u8(chip_no, &orig, reg);
	if (ret < 0)
		return ret;
	...
	if (tmp != orig) {
		ret = ktz8864_2_i2c_write_u8(chip_no, reg, val);
		if (ret < 0)
			return ret;
	}
...
static int ktz8864_set_backlight(uint32_t bl_level)
{
	...
	if (ret < 0)
		goto i2c_error;
    if (bl_info.dual_ic){
	   ret = ktz8864_2_i2c_update_bits(KTZ8864_SLAV_ADDR, REG_BL_BRIGHTNESS_LSB,
			MASK_BL_LSB, bl_lsb);
	   if (ret < 0)
		goto i2c_error;

	   ret = ktz8864_2_i2c_write_u8(KTZ8864_SLAV_ADDR, REG_BL_BRIGHTNESS_MSB,
			bl_msb);
	   if (ret < 0)
		goto i2c_error;
    }
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void ktz8864_set_backlight_status(void *fdt)
#else
...
static int ktz8864_device_verify(void)
{
	...
	if (bl_info.ktz8864_hw_en) {
		lcdkit_gpio_set_mode(bl_info.ktz8864_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(bl_info.ktz8864_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(bl_info.ktz8864_hw_en_gpio,
...
int ktz8864_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_get_dts_u32_default(bl_info.pfdt,
		bl_info.nodeoffset, KTZ8864_HW_DUAL_IC,
		&bl_info.dual_ic, 0);
	if (ret < 0)
	...
	if (bl_info.dual_ic) {
		ret = lcd_kit_get_dts_u32_default(bl_info.pfdt,
			bl_info.nodeoffset, KTZ8864_2_I2C_BUS_ID,
			&bl_info.ktz8864_2_i2c_bus_id, 0);
		if (ret < 0) {
			LCD_KIT_ERR("parse dts ktz8864_2_i2c_bus_id fail!\n");
			bl_info.ktz8864_2_i2c_bus_id = 0;
			return SUCC;
		}

		ret = lcd_kit_get_dts_u32_default(bl_info.pfdt,
			bl_info.nodeoffset, KTZ8864_2_DISPLAY_BIAS_CFG_1,
			&bl_info.ktz8864_2_display_bias_cfg_1, 0);
		if (ret < 0) {
			LCD_KIT_ERR("parse dts ktz8864_2_display_bias_cfg_1 fail!\n");
			return SUCC;
		}
	}
	...
	if (bl_info.ktz8864_hw_en) {
		ret = lcd_kit_get_dts_u32_default(bl_info.pfdt, bl_info.nodeoffset,
			KTZ8864_HW_EN_GPIO,
			(u32 *)&bl_info.ktz8864_hw_en_gpio, 0);
		if (ret < 0) {
