/*
 * drivers/display/honor/backlight/lm36274.c
 *
 * lm36274 driver reffer to lcd
 *
 * Copyright (C) 2012-2015 HONOR, Inc.
 * Author: HONOR, Inc.
 *
...
static int lm36274_2_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
        int ret = -1;
        /* read date default length */
        unsigned char len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	struct mt_i2c lm36274_i2c = {0};
#else
	struct mt_i2c_t lm36274_i2c = {0};
        ...
        *data_buffer = addr;
        lm36274_i2c.id = lm36274_bl_info.lm36274_2_i2c_bus_id;
        lm36274_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(lm36274_i2c.id, lm36274_i2c.addr, data_buffer, len, len, NULL);
#else
...
static int lm36274_2_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
        int ret;
        ...
        lm36274_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
        ret = mtk_i2c_write(lm36274_i2c.id, lm36274_i2c.addr, write_data, len, NULL);
#else
        lm36274_i2c.mode = ST_MODE;
        lm36274_i2c.speed = LM36274_I2C_SPEED;
...
static int lm36274_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	...
	if (ret != 0)
		LCD_KIT_ERR("%s: i2c_write failed, reg is 0x%x ret: %d\n",
			__func__, addr, ret);
        if (lm36274_bl_info.dual_ic) {
             if (addr == REG_DISPLAY_BIAS_CONFIG_1)
                 lm36274_2_i2c_write_u8(chip_no, addr, lm36274_bl_info.lm36274_2_display_bias_cfg_1);
             else
                 lm36274_2_i2c_write_u8(chip_no, addr, value);;
        }
...
static int lm36274_2_i2c_update_bits(u8 chip_no, u8 reg, u8 mask, u8 val)
{
        int ret;
        u8 tmp;
        u8 orig = 0;

        ret = lm36274_2_i2c_read_u8(chip_no, &orig, reg);
        if (ret < 0)
            return ret;
        ...
        if (tmp != orig) {
            ret = lm36274_2_i2c_write_u8(chip_no, reg, val);
            if (ret < 0)
                return ret;
        }
...
static int lm36274_parse_dts()
{
	...
	for (i = 0; i < LM36274_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
			...
			&lm36274_bl_info.lm36274_reg[i], 0);
		if (ret < 0) {
...
static int lm36274_device_verify(void)
{
	...
	if (lm36274_bl_info.lm36274_hw_en) {
		lcdkit_gpio_set_mode(lm36274_bl_info.lm36274_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(lm36274_bl_info.lm36274_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(lm36274_bl_info.lm36274_hw_en_gpio,
...
int lm36274_init(struct mtk_panel_info *pinfo)
{
	...
	if (ret < 0) {
        ...
        ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
                lm36274_bl_info.nodeoffset, LM36274_HW_DUAL_IC,
                &lm36274_bl_info.dual_ic, 0);
        if (ret < 0)
                LCD_KIT_ERR("parse dts dual_ic fail!\n");
        if (lm36274_bl_info.dual_ic) {
                ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
                        lm36274_bl_info.nodeoffset, LM36274_2_I2C_BUS_ID,
                        &lm36274_bl_info.lm36274_2_i2c_bus_id, 0);
                if (ret < 0) {
                        LCD_KIT_ERR("parse dts lm36274_2_i2c_bus_id fail!\n");
                        lm36274_bl_info.lm36274_2_i2c_bus_id = 0;
                        return LCD_KIT_FAIL;
                }

                ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
                        lm36274_bl_info.nodeoffset, LM36274_2_DISPLAY_BIAS_CFG_1,
                        &lm36274_bl_info.lm36274_2_display_bias_cfg_1, 0);
                if (ret < 0) {
                        LCD_KIT_ERR("parse dts lm36274_2_display_bias_cfg_1 fail!\n");
                        return LCD_KIT_FAIL;
                }
        }
...
int lm36274_set_backlight(uint32_t bl_level)
{
	...
	if (ret < 0)
		goto i2c_error;
        if (lm36274_bl_info.dual_ic){
            ret = lm36274_2_i2c_update_bits(LM36274_SLAV_ADDR, REG_BL_BRIGHTNESS_LSB,
                      MASK_BL_LSB, bl_lsb);
            if (ret < 0)
                   goto i2c_error;

            ret = lm36274_2_i2c_write_u8(LM36274_SLAV_ADDR, REG_BL_BRIGHTNESS_MSB,
                   bl_msb);
            if (ret < 0)
                   goto i2c_error;
        }
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void lm36274_set_backlight_status(void *fdt)
#else
