...
static int lm3697_i2c_read_u8(unsigned char addr, unsigned char *dataBuffer)
{
	...
	*dataBuffer = addr;
	...
	len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(lm3697_i2c.id, lm3697_i2c.addr, dataBuffer, len, len, NULL);
#else
...
static int lm3697_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	len = 2;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(lm3697_i2c.id, lm3697_i2c.addr, write_data, len, NULL);
#else
	lm3697_i2c.mode = ST_MODE;
	lm3697_i2c.speed = 100;
...
static void lm3697_enable(void)
{
	...
	if (lm3697_bl_info.lm3697_hw_en) {
		lcdkit_gpio_set_mode(lm3697_bl_info.lm3697_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(lm3697_bl_info.lm3697_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(lm3697_bl_info.lm3697_hw_en_gpio,
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void lm3697_set_backlight_status(void *fdt)
#else
...
int lm3697_init(void)
{
	...
	if (lm3697_bl_info.lm3697_hw_en) {
		ret = lcd_kit_parse_get_u32_default(DTS_COMP_LM3697,
			LM3697_HW_EN_GPIO,
			(u32 *)&lm3697_bl_info.lm3697_hw_en_gpio, 0);
		if (ret < 0) {
