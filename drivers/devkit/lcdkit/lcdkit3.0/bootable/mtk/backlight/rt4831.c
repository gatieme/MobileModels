...
static int rt4831_2_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	int ret = -1;
	/* read date default length */
	unsigned char len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	struct mt_i2c rt4831_i2c = {0};
#else
	...
	*data_buffer = addr;
	rt4831_i2c.id = rt4831_bl_info.rt4831_2_i2c_bus_id;
	rt4831_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(rt4831_i2c.id, rt4831_i2c.addr, data_buffer, len, len, NULL);
#else
...
static int rt4831_2_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	int ret;
	...
	rt4831_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(rt4831_i2c.id, rt4831_i2c.addr, write_data, len, NULL);
#else
	rt4831_i2c.mode = ST_MODE;
	rt4831_i2c.speed = RT4831_I2C_SPEED;
...
static int rt4831_i2c_write_u8(unsigned char chip_no, unsigned char addr, unsigned char value)
{
	...
	if (ret != 0)
		LCD_KIT_ERR("%s: i2c_write failed, reg is 0x%x ret: %d\n",
			__func__, addr, ret);
	if (rt4831_bl_info.dual_ic) {
	     if (addr == REG_DISPLAY_BIAS_CONFIG_1)
			rt4831_2_i2c_write_u8(chip_no, addr, rt4831_bl_info.rt4831_2_display_bias_cfg_1);
		 else
		    rt4831_2_i2c_write_u8(chip_no, addr, value);;
	}
...
static int rt4831_2_i2c_update_bits(unsigned char chip_no,
	int reg, unsigned char mask, unsigned char val)
{
	int ret;
	unsigned char tmp;
	...
	ret = rt4831_2_i2c_read_u8(chip_no, &orig, reg);
	if (ret < 0)
		return ret;
	...
	if (tmp != orig) {
		ret = rt4831_2_i2c_write_u8(chip_no, reg, val);
		if (ret < 0)
			return ret;
	}
...
static int rt4831_device_verify(void)
{
	...
	if (rt4831_bl_info.rt4831_hw_en) {
		lcdkit_gpio_set_mode(rt4831_bl_info.rt4831_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(rt4831_bl_info.rt4831_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(rt4831_bl_info.rt4831_hw_en_gpio,
...
int rt4831_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_get_dts_u32_default(rt4831_bl_info.pfdt,
		rt4831_bl_info.nodeoffset, RT4831_HW_DUAL_IC,
		&rt4831_bl_info.dual_ic, 0);
	if (ret < 0)
	...
	if (rt4831_bl_info.dual_ic) {
		ret = lcd_kit_get_dts_u32_default(rt4831_bl_info.pfdt,
			rt4831_bl_info.nodeoffset, RT4831_2_I2C_BUS_ID,
			&rt4831_bl_info.rt4831_2_i2c_bus_id, 0);
		if (ret < 0) {
			LCD_KIT_ERR("parse dts rt4831_2_i2c_bus_id fail!\n");
			rt4831_bl_info.rt4831_2_i2c_bus_id = 0;
			return LCD_KIT_FAIL;
		}

		ret = lcd_kit_get_dts_u32_default(rt4831_bl_info.pfdt,
			rt4831_bl_info.nodeoffset, RT4831_2_DISPLAY_BIAS_CFG_1,
			&rt4831_bl_info.rt4831_2_display_bias_cfg_1, 0);
		if (ret < 0) {
			LCD_KIT_ERR("parse dts rt4831_2_display_bias_cfg_1 fail!\n");
			return LCD_KIT_FAIL;
		}
	}
...
int rt4831_set_backlight(uint32_t bl_level)
{
	...
	if (ret < 0)
		goto i2c_error;
	if (rt4831_bl_info.dual_ic){
	     ret = rt4831_2_i2c_update_bits(RT4831_SLAV_ADDR, REG_BL_BRIGHTNESS_LSB,
			  MASK_BL_LSB, bl_lsb);
	     if (ret < 0)
		   goto i2c_error;

	     ret = rt4831_2_i2c_write_u8(RT4831_SLAV_ADDR, REG_BL_BRIGHTNESS_MSB,
			 bl_msb);
     	     if (ret < 0)
		   goto i2c_error;
        }
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void rt4831_set_backlight_status(void *fdt)
#else
