 ...
 *
 * Copyright (c) 2019 Honor Device Co., Ltd.
 *
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(rt8555_i2c.id, rt8555_i2c.addr, data_buffer, len, len, NULL);
#else
...
static int rt8555_2_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	...
	rt8555_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(rt8555_i2c.id, rt8555_i2c.addr, write_data, len, NULL);
#else
	rt8555_i2c.mode = ST_MODE;
	rt8555_i2c.speed = RT8555_I2C_SPEED;
...
static void rt8555_enable(void)
{
	...
	if (g_bl_info.rt8555_hw_en) {
		lcdkit_gpio_set_mode(g_bl_info.rt8555_hw_en_gpio,
			LCDKIT_GPIO_MODE_00);
		lcdkit_gpio_set_dir(g_bl_info.rt8555_hw_en_gpio,
			LCDKIT_GPIO_DIR_OUT);
		lcdkit_gpio_set_out(g_bl_info.rt8555_hw_en_gpio,
			LCDKIT_GPIO_VAL_HIGH);
		if (g_bl_info.dual_ic) {
			lcdkit_gpio_set_mode(g_bl_info.rt8555_2_hw_en_gpio,
				LCDKIT_GPIO_MODE_00);
			lcdkit_gpio_set_dir(g_bl_info.rt8555_2_hw_en_gpio,
				LCDKIT_GPIO_DIR_OUT);
			lcdkit_gpio_set_out(g_bl_info.rt8555_2_hw_en_gpio,
				LCDKIT_GPIO_VAL_HIGH);
		}
...
static int rt8555_set_backlight(uint32_t bl_level)
{
	...
	bl_level = bl_level * g_bl_info.bl_level / RT8555_BL_DEFAULT_LEVEL;
	...
	bl_lsb = bl_level & 0xFF; /* 8 lsb bits */
		...
		LCD_KIT_ERR("write bl level lsb:0x%x msb:0x%x\n", bl_lsb,bl_msb);
	ret = rt8555_i2c_write_u8(RT8555_SLAV_ADDR,
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
void rt8555_set_backlight_status(void *fdt)
#else
...
int rt8555_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_get_dts_u32_default(g_bl_info.pfdt,
		g_bl_info.nodeoffset, "rt8555_level_lsb",
		&g_bl_info.rt8555_level_lsb, 0);
	if (ret < 0) {
		LCD_KIT_ERR("get rt8555_level_lsb dts failed\n");
		goto exit;
	}
	ret = lcd_kit_get_dts_u32_default(g_bl_info.pfdt,
			g_bl_info.nodeoffset, "rt8555_level_msb",
		&g_bl_info.rt8555_level_msb, 0);
	if (ret < 0) {
		LCD_KIT_ERR("get rt8555_level_msb dts failed\n");
		goto exit;
	}
