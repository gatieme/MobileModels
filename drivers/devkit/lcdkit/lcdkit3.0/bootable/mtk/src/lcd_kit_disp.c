	...
	struct mtk_panel_info *pinfo)
{
	...
	params->dsi.data_rate_khz = pinfo->data_rate_khz;
	params->dsi.IsCphy = pinfo->mipi.phy_mode;
	params->lcm_if = pinfo->lcm_if;
	...
	params->dsi.HS_TRAIL = pinfo->hs_trail;
	params->dsi.HS_ZERO = pinfo->hs_zero;
	params->dsi.HS_PRPR = pinfo->hs_prpr;
...
static int32_t poweric_gpio_detect()
{
	int32_t gpio_value;
	int32_t ret;
	gpio_value = lcdkit_gpio_get_in(disp_info->elvdd_detect.detect_gpio);
...
void lcd_kit_change_display_dts(void *fdt)
{
	...
	if (panel_info->bl_ic_ctrl_mode == MTK_PWM_HIGH_I2C_MODE) {
		...
		sy7758_set_backlight_status(fdt);
		mp3314_set_backlight_status(fdt);
		rt4539_set_backlight_status(fdt);
	...
	if (lcd_type == LCD_KIT) {
		...
		lcdkit_set_lcd_panel_status(fdt, disp_info->lcd_name);
	}
	if (lcd_kit_pinfo.blmaxnit.get_blmaxnit_type ==
		GET_BLMAXNIT_FROM_DDIC)
...
void lcd_kit_change_display_dts(void)
{
	...
	if (panel_info->bl_ic_ctrl_mode == MTK_PWM_HIGH_I2C_MODE) {
		...
		sy7758_set_backlight_status();
		mp3314_set_backlight_status();
		rt4539_set_backlight_status();
	} else if (panel_info->bl_ic_ctrl_mode == MTK_AAL_I2C_MODE) {
		sgm37603a_set_backlight_status();
		lm36923_set_backlight_status();
		ktz8864_set_backlight_status();
...
void lcd_kit_btb_check_init()
{
	...
	for (i = 0; i < common_info->lcd_btb_gpio.cnt; i++) {
		...
		if (common_info->btb_check_type == LCD_KIT_BTB_CHECK_GPIO)
			lcdkit_gpio_set_mode(btb_gpio, LCDKIT_GPIO_MODE_00);
		else if (common_info->btb_check_type == LCD_KIT_BTB_CHECK_IRQ)
			lcdkit_gpio_set_mode(btb_gpio, LCDKIT_GPIO_MODE_02);

		lcdkit_gpio_set_dir(btb_gpio, LCDKIT_GPIO_DIR_IN);
		lcdkit_gpio_set_pull_select(btb_gpio, LCDKIT_GPIO_PULL_UP);
	}
	LCD_KIT_INFO("LCD BTB check init OK\n");
}

void lcd_kit_lk_power_on_early()
{
	if (!common_info->lk_power_on_early)
		return;
	if (common_ops->power_on_early) {
		if (common_ops->power_on_early((void *)NULL) != LCD_KIT_OK)
			LCD_KIT_ERR("fail\n");
		else
			LCD_KIT_INFO("done\n");
	}
...
#endif
{
	...
	case MTK_PWM_HIGH_I2C_MODE:
		...
		sy7758_init(&lcd_kit_pinfo);
		mp3314_init(&lcd_kit_pinfo);
		rt4539_init(&lcd_kit_pinfo);
		break;
