...
static uint32_t g_lcd_kit_gpio;
#define LDO_BEGIN 1
#define LDO_MAX 5
...
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	{ 68, 3000000, MT6368_PMIC_RG_LDO_VFP_EN_ADDR, 0x1, 0x1, MT6368_PMIC_RG_LDO_VFP_EN_SHIFT },
	{ 7331, 3100000, MT6373_PMIC_RG_LDO_VFP_EN_ADDR, 0x1, 0x1, MT6373_PMIC_RG_LDO_VFP_EN_SHIFT },
	{ 7318, 1850000, MT6373_PMIC_RG_LDO_VAUD18_EN_ADDR, 0x1, 0x1, MT6373_PMIC_RG_LDO_VAUD18_EN_SHIFT },
#endif
};
...
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	{ 68, 3000000, MT6368_PMIC_RG_VFP_VOSEL_ADDR, 0xc, MT6368_PMIC_RG_VFP_VOCAL_ADDR, 0x0 },
	{ 7331, 3100000, MT6373_PMIC_RG_VFP_VOSEL_ADDR, 0xc, MT6373_PMIC_RG_VFP_VOCAL_ADDR, 0x0 },
	{ 7318, 1850000, MT6373_PMIC_RG_VAUD18_VOSEL_ADDR, 0xc, MT6373_PMIC_RG_VAUD18_VOCAL_ADDR, 0x1 },
#endif
};
/*
 * power type
...
int gpio_cmds_tx(struct gpio_desc *cmds, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		if ((cm == NULL) || (cm->label == NULL)) {
			...
			lcdkit_gpio_set_mode(*(cm->gpio), LCDKIT_GPIO_MODE_00);
			lcdkit_gpio_set_dir(*(cm->gpio), LCDKIT_GPIO_DIR_OUT);
			lcdkit_gpio_set_out(*(cm->gpio), cm->value);
...
void lcd_kit_gpio_tx(uint32_t type, uint32_t op)
{
	...
	case LCD_KIT_VDD:
		...
		break;
	case LCD_KIT_EXT_LDO:
		g_lcd_kit_gpio = power_hdl->lcd_ext_ldo.buf[POWER_NUMBER];
		break;
	default:
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
static int spmic_config_vfp_volt(u32 addr0, u32 data0, u32 addr1, u32 data1)
{
    int ret = 0;
    struct spmi_device *spmi_sdev = NULL;
    int val = 0;
    u16 reg = (u16)addr0;
    u8 reg_val  = (u8)data0;
    ...
    if (!spmi_sdev) {
        LCD_KIT_ERR("get spmi device fail\n");
        return LCD_KIT_FAIL;
    }

    /* 1.8V */
    ret = spmi_ext_register_writel(spmi_sdev, reg, &reg_val, 1);
    if (ret < 0) {
        LCD_KIT_INFO("SPMI volt write addr: 0x%x, val: 0x%x fail\n", reg, reg_val);
        return LCD_KIT_FAIL;
    }
...
static int spmic_config_vfp_en(u32 addr, u32 data0)
{
    int ret = 0;
    struct spmi_device *spmi_sdev = NULL;
    u8 val = 0;
    ...
    spmi_sdev = get_spmi_device(SPMI_MASTER_1, SPMI_SLAVE_5);
    if (!spmi_sdev) {
        LCD_KIT_ERR("%s: get spmi device fail\n", __func__);
        return LCD_KIT_FAIL;
    }
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	if (power_num >= LDO_BEGIN && power_num <= LDO_MAX) {
		...
		if (mt6360_ldo_config_interface(g_ldo_volt[array_index].ldo_addr,
			...
			LCD_KIT_ERR("mt6360_ldo_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
		...
		if (mt6360_pmic_config_interface(g_ldo_volt[array_index].ldo_addr,
			...
			g_ldo_volt[array_index].ldo_mask,
			g_ldo_volt[array_index].ldo_shift)) {
			LCD_KIT_ERR("mt6360_pmic_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
#ifdef MTK_MT6358_LDO_SUPPORT
		if (pmic_config_interface(PMIC_RG_LDO_VLDO28_EN_0_ADDR,
			...
			LCD_KIT_ERR("pmic_config_interface ldo enable fail\n");
			return LCD_KIT_FAIL;
		}
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
		if (spmic_config_vfp_volt(g_ldo_volt[array_index].ldo_addr,
			g_ldo_volt[array_index].ldo_data,
			g_ldo_volt[array_index].ldo_mask,
			g_ldo_volt[array_index].ldo_shift)){
			LCD_KIT_ERR("mt6368/mt6373 pmic_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
		LCD_KIT_INFO("ldo%d set vol\n", power_num);
...
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
		if (spmic_config_vfp_en(g_ldo_enable[array_index].ldo_addr,
			g_ldo_enable[array_index].ldo_data)){
			LCD_KIT_ERR("mt6368/mt6373 pmic_config_interface ldo enable fail\n");
			return LCD_KIT_FAIL;
		}
#endif
		LCD_KIT_INFO("ldo%d config\n", power_num);
