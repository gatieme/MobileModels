...
int lcd_kit_get_tp_color(void)
{
	...
	if (disp_info->tp_color.support) {
		ret = lcd_kit_dsi_cmds_rx(NULL, &read_value,
...
static int lcd_kit_parse_disp_info(const void *fdt, int nodeoffset)
{
	...
	/* dem */
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,dsi-dem-enable",
		&disp_info->dsi_dem_enable, 0);
	/* mipi tx vol */
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,mipi-tx-vol",
		&disp_info->mipi_tx_vol, 0);
	/* rtcode */
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,rtcode",
		&disp_info->rtcode, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,rtcode-inc",
		&disp_info->rtcode_inc, 0);
	...
	struct mtk_panel_info *pinfo)
{
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-output-mode",
		&pinfo->output_mode, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-interface",
		&pinfo->lcm_if, 0);
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-data-rate",
		&pinfo->data_rate, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-data-rate-khz",
		&pinfo->data_rate_khz, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-frame-lp-enable",
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
		"lcd-kit,panel-dsi-hs-trail",
		&pinfo->hs_trail, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
		"lcd-kit,panel-dsi-hs-zero",
		&pinfo->hs_zero, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
		"lcd-kit,panel-dsi-hs-prpr",
		&pinfo->hs_prpr, 0);
	return ret;
}
...
static void lcd_kit_parse_power(const void *fdt, int nodeoffset)
{
	...
	if (power_hdl->lcd_ext_ldo.buf == NULL)
		lcd_kit_parse_dts_array(fdt, nodeoffset, "lcd-kit,lcd-ext-ldo",
...
static void lcd_kit_parse_power_seq(const void *fdt, int nodeoffset)
{
	...
	if (common_info->lk_power_on_early)
		lcd_kit_parse_dts_arrays(fdt, nodeoffset,"lcd-kit,lk-power-on-early-seq",
			&power_seq->lk_power_on_early_seq, 3);
}
...
static void lcd_kit_parse_common_info(const void *fdt, int nodeoffset)
{
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
		"lcd-kit,lk-power-on-early",
		&common_info->lk_power_on_early, 0);
}
...
void lcd_kit_backlight_on_delay(void)
{
	struct mtk_panel_info *panel_info = NULL;

	panel_info = lcm_get_panel_info();
	if (panel_info == NULL) {
		LCD_KIT_INFO("panel info is NULL\n");
		return;
	}
	/* quickly sleep */
	if (disp_info->quickly_sleep_out.support) {
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	disp_info->quickly_sleep_out.panel_on_record = current_time();
#else
	...
	struct mtk_panel_info *pinfo)
{
	...
	pinfo->round_corner.lt_addr = malloc(array_data.cnt);
