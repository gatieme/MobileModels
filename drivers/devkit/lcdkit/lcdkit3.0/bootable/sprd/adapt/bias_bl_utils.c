 ...
 * bias_bl_utils.c
 *
 * bias_bl_utils driver for honor paltform i2c and dts
 * operation function
 *
...
#define SPRD_I2C_STD_SPEED 100000
	...
	unsigned char reg, unsigned char *val)
{
	...
	*val = reg;
	...
	i2c_set_bus_num(i2c_chip_num);
	i2c_init(SPRD_I2C_STD_SPEED, chip_addr);
	ret = i2c_reg_read(chip_addr, reg);
	if (ret < 0) {
		FASTBOOT_ERR("%s: i2c_read failed, reg is 0x%x ret: %d\n",
			__func__, chip_addr, ret);
		return ret;
	}
	*val = ret;
	return ret;
