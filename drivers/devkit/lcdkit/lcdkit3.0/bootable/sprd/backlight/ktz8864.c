...
static int ktz8864_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(bl_info.ktz8864_i2c_bus_id);
	i2c_init(KTZ8864_I2C_SPEED, chip_no);
...
static int ktz8864_device_verify(void)
{
	...
	if (bl_info.ktz8864_hw_en) {
		sprd_gpio_request(NULL, bl_info.ktz8864_hw_en_gpio);
		sprd_gpio_direction_output(NULL, bl_info.ktz8864_hw_en_gpio,
			GPIO_OUT_ONE);
...
int ktz8864_init(struct sprd_panel_info *pinfo)
{
	int ret;
