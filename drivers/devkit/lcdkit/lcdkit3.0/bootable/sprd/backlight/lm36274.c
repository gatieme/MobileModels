...
static int lm36274_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(lm36274_bl_info.lm36274_i2c_bus_id);
	i2c_init(LM36274_I2C_SPEED, chip_no);
...
static int lm36274_device_verify(void)
{
	...
	if (lm36274_bl_info.lm36274_hw_en) {
		sprd_gpio_request(NULL, lm36274_bl_info.lm36274_hw_en_gpio);
		sprd_gpio_direction_output(NULL, lm36274_bl_info.lm36274_hw_en_gpio,
			GPIO_OUT_ONE);
...
int lm36274_init(struct sprd_panel_info *pinfo)
{
	int ret;
	...
	lm36274_bl_info.pfdt = gd->fdt_blob;
...
int lm36274_set_backlight(uint32_t bl_level)
{
	...
	if (panel_info->bl_ic_ctrl_mode == DSI_BACKLIGHT_DCS_I2C_IC) {
