...
static int lp8556_2_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(lp8556_bl_info.lp8556_2_i2c_bus_id);
	i2c_init(LP8556_I2C_SPEED, chip_no);
...
static int lp8556_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(lp8556_bl_info.lp8556_i2c_bus_id);
	i2c_init(LP8556_I2C_SPEED, chip_no);
	ret = i2c_reg_read(chip_no, addr);
	if (ret < 0) {
		LCD_KIT_ERR("%s: i2c_read failed, reg is 0x%x ret: %d\n",
...
static void lp8556_enable(void)
{
	...
	if (lp8556_bl_info.lp8556_hw_en) {
		sprd_gpio_request(NULL, lp8556_bl_info.lp8556_hw_en_gpio);
		sprd_gpio_direction_output(NULL, lp8556_bl_info.lp8556_hw_en_gpio,
			GPIO_OUT_ONE);
...
int lp8556_init(struct sprd_panel_info *pinfo)
{
	int ret;
	...
	lp8556_bl_info.pfdt = gd->fdt_blob;
