...
#define NT50356_I2C_SPEED (100000)
...
static int nt50356_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(nt50356_bl_info.nt50356_i2c_bus_id);
	i2c_init(NT50356_I2C_SPEED, chip_no);
...
void nt50356_set_backlight_status(void *fdt)
{
	int ret;
	int offset;
...
static int nt50356_device_verify(void)
{
	...
	if (nt50356_bl_info.nt50356_hw_en) {
		sprd_gpio_request(NULL, nt50356_bl_info.nt50356_hw_en_gpio);
		sprd_gpio_direction_output(NULL, nt50356_bl_info.nt50356_hw_en_gpio,
			GPIO_OUT_ONE);
...
int nt50356_init(struct sprd_panel_info *pinfo)
{
	int ret;
	...
	nt50356_bl_info.pfdt = gd->fdt_blob;
