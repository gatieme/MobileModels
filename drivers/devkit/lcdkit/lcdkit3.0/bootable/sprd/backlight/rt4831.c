...
static int rt4831_i2c_read_u8(unsigned char chip_no, unsigned char *data_buffer, int addr)
{
	...
	i2c_set_bus_num(rt4831_bl_info.rt4831_i2c_bus_id);
	i2c_init(RT4831_I2C_SPEED, chip_no);
...
static int rt4831_device_verify(void)
{
	...
	if (rt4831_bl_info.rt4831_hw_en) {
		sprd_gpio_request(NULL, rt4831_bl_info.rt4831_hw_en_gpio);
		sprd_gpio_direction_output(NULL, rt4831_bl_info.rt4831_hw_en_gpio,
			GPIO_OUT_ONE);
...
int rt4831_init(struct sprd_panel_info *pinfo)
{
	int ret;
	...
	rt4831_bl_info.pfdt = gd->fdt_blob;
