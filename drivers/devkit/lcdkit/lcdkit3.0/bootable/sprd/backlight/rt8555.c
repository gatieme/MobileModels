...
static int rt8555_2_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(g_bl_info.rt8555_2_i2c_bus_id);
	i2c_init(RT8555_I2C_SPEED, chip_no);
	ret = i2c_reg_read(chip_no, addr);
	if (ret < 0)
...
static int rt8555_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	i2c_set_bus_num(g_bl_info.rt8555_i2c_bus_id);
	i2c_init(RT8555_I2C_SPEED, chip_no);
	ret = i2c_reg_read(chip_no, addr);
	if (ret < 0)
		LCD_KIT_ERR("%s: i2c_read failed, reg is 0x%x ret: %d\n",
...
static int rt8555_2_i2c_write_u8(u8 chip_no, unsigned char addr, unsigned char value)
{
	i2c_set_bus_num(g_bl_info.rt8555_2_i2c_bus_id);
	i2c_init(RT8555_I2C_SPEED, chip_no);
	i2c_reg_write(chip_no, addr, value);
...
static void rt8555_enable(void)
{
	...
	if (g_bl_info.rt8555_hw_en) {
		sprd_gpio_request(NULL, g_bl_info.rt8555_hw_en_gpio);
		sprd_gpio_direction_output(NULL, g_bl_info.rt8555_hw_en_gpio,
			GPIO_OUT_ONE);
		if (g_bl_info.dual_ic) {
			sprd_gpio_request(NULL, g_bl_info.rt8555_2_hw_en_gpio);
			sprd_gpio_direction_output(NULL, g_bl_info.rt8555_2_hw_en_gpio,
				GPIO_OUT_ONE);
		}
...
int rt8555_init(struct sprd_panel_info *pinfo)
{
	int ret;
