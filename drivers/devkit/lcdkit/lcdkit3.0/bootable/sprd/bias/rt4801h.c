...
#define RT4801H_I2C_SPEED 100000
...
static int rt4801h_i2c_read_u8(unsigned char addr, unsigned char *databuffer)
{
	...
	i2c_init(RT4801H_I2C_SPEED, RT4801H_SLAV_ADDR);
	ret = i2c_reg_read(RT4801H_SLAV_ADDR, addr);
	if (ret < 0) {
...
static int rt4801h_i2c_write_u8(unsigned char addr, unsigned char value)
{
	i2c_set_bus_num(i2c_bus_id);
	i2c_init(RT4801H_I2C_SPEED, RT4801H_SLAV_ADDR);
	i2c_reg_write(RT4801H_SLAV_ADDR, addr, value);
...
void rt4801h_set_bias_status(void *fdt)
{
	int ret;
	int offset;
