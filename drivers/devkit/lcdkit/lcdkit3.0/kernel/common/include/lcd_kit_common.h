...
extern struct lcd_kit_power_desc *lcd_kit_get_power_handle(void);
...
struct lcd_kit_string_arrays {
	const char *buf[MAX_FPS_TEST_SUP];
	int cnt;
};

struct mtk_lcd_test_config {
	struct lcd_kit_string_arrays mmi_test_list;
	struct lcd_kit_string_arrays rt_test_list;
	struct lcd_kit_string_arrays tplcd_test_list;
};
...
struct lcd_kit_local_hbm {
	u32 support;
	u32 panel_match_batch_info_support;
	struct lcd_kit_dsi_panel_cmds disable_cmds;
	struct lcd_kit_dsi_panel_cmds on_enable_cmds;
	struct lcd_kit_dsi_panel_cmds off_enable_cmds;
	struct lcd_kit_dsi_panel_cmds panel_read_batch_info;
	struct lcd_kit_array_data panel_batch_value;
};
...
struct lcd_kit_esd {
	...
	u32 tp_esd_event;
	u32 esd_check_mode;
	u32 esd_check_period;
	u32 gpio_detect_dual_support;
	u32 gpio_detect_num_first;
	u32 gpio_normal_value_first;
	u32 gpio_detect_num_second;
	u32 gpio_normal_value_second;
	u32 panel_gpio_offset;
	...
	struct lcd_kit_dsi_panel_cmds reduce_vol_cmds;
	struct lcd_kit_dsi_panel_cmds recover_vol_cmds;
	u32 mipi_err_cnt;
};
...
struct lcd_kit_adapt_ops {
	...
	void (*set_mipi_power_mode)(int enable);
};
...
struct lcd_kit_common_ops {
	...
	int (*panel_power_on_without_mipi)(void *hld);
	int (*panel_power_on_with_mipi)(void *hld);
...
#define PANEL_DEAFULT 0
#define PANEL_VER1 1
struct lcd_kit_panel_ext_info {
	u32 support;
	u32 ver_val;
	struct lcd_kit_dsi_panel_cmds get_ver_cmds;
};
...
struct lcd_kit_common_info {
	...
	u32 is_ext_panel;
	char *panel_ext_name;
	struct lcd_kit_panel_ext_info panel_ext_info;
};
...
struct lcd_kit_power_desc {
	...
	struct lcd_kit_array_data lcd_ext_ldo;
	u32 ktz8864_bl_boost;
	u32 lm36274_bl_boost;
};
...
extern void lcd_backlight_i2c_dmd(void);
extern void lcd_bias_i2c_dmd(void);
