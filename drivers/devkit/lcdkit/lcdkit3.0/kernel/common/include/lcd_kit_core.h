...
struct lcd_kit_ops {
	...
	int (*get_pt_station_status)(int panel_id);
	int (*avdd_mipi_ctrl)(void *hld, int enable);
	int (*get_lcd_status)(void);
	int (*get_panel_power_status)(int panel_id);
	...
	int (*get_sn_code)(int panel_id);
	bool (*panel_event_skip_delay)(int panel_id, void *hld,
		uint32_t event, uint32_t data);
	int (*get_panel_dieid)(uint32_t panel_id, char *buff);
	int (*event_skip_send)(int panel_id, void *hld, uint32_t event, uint32_t enable);
	void (*notify_panel_switch)(void);
	int (*lcd_dimming_switch)(uint32_t panel_id);
	int (*dvdd_hpm_ctrl)(void);
	int (*dvdd_avs_ctrl)(int panel_id, void *hld);
	bool (*exec_ext_ts)(void);
	int (*eink_power_ctrl)(int enable);
	int (*get_panel_id)(void);
	int (*get_el_ctrl_info)(int panel_id);
	void (*send_de_event)(int val, int panel_id);
};
