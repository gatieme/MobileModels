...
#if defined(CONFIG_LCD_KIT_HONOR)
...
EXPORT_SYMBOL(lcd_kit_msg_level);
/* common info */
...
void lcd_get_timestamp(struct timeval *tv)
{
...
EXPORT_SYMBOL(lcd_kit_get_power_handle);
...
static int lcd_kit_ext_ldo_power_on(void)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (!power_hdl->lcd_ext_ldo.buf) {
		LCD_KIT_ERR("can not get lcd reset!\n");
		return LCD_KIT_FAIL;
	}

	switch (power_hdl->lcd_ext_ldo.buf[POWER_MODE]) {
	case GPIO_MODE:
		if (adapt_ops->gpio_enable)
			ret = adapt_ops->gpio_enable(LCD_KIT_EXT_LDO);
		break;
	default:
		LCD_KIT_ERR("not support type:%d\n", power_hdl->lcd_ext_ldo.buf[POWER_MODE]);
		break;
	}
	lcd_kit_proximity_record_time();
	return ret;
...
static int lcd_kit_ext_ldo_power_off(void)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	case GPIO_MODE:
		if (adapt_ops->gpio_disable)
			ret = adapt_ops->gpio_disable(LCD_KIT_EXT_LDO);
		break;
	default:
		LCD_KIT_ERR("not support type:%d\n",
			power_hdl->lcd_ext_ldo.buf[POWER_MODE]);
...
int lcd_kit_ext_ldo_power_ctrl(int enable)
{
	if (enable == LCD_RESET_HIGH)
		return lcd_kit_ext_ldo_power_on();
	else
		return lcd_kit_ext_ldo_power_off();
}
...
static void lcd_kit_set_mipi_power_mode(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	adapt_ops = lcd_kit_get_adapt_ops();

	if (adapt_ops && (adapt_ops->set_mipi_power_mode))
		adapt_ops->set_mipi_power_mode(enable);
}
...
{
#if defined(CONFIG_LCD_KIT_HONOR)
	struct honor_fb_data_type *honorfd = NULL;
	struct honor_panel_info *pinfo = NULL;

	honorfd = (struct honor_fb_data_type *)hld;
	if (!honorfd) {
		LCD_KIT_ERR("honorfd is null!\n");
		return false;
	}
	pinfo = &(honorfd->panel_info);
	if (!pinfo) {
...
static int lcd_kit_event_should_send(void *hld, uint32_t event, uint32_t data)
{
	...
	if (g_esd_flag) {
		LCD_KIT_INFO("esd recover start and power need off\n");
		return ret;
	}
#endif
	...
	uint32_t delay)
{
	...
	case EVENT_2ND_POWER_OFF_TS:
		...
		break;
	case EVENT_EXT_LDO_EN:
		ret = lcd_kit_ext_ldo_power_ctrl(data);
		break;
	case EVENT_MIPI_POWER_MODE:
		lcd_kit_set_mipi_power_mode(data);
		break;
	default:
...
int lcd_kit_event_handler_part(void *hld, uint32_t event, uint32_t data,
...
static int lcd_kit_panel_power_on_part(void *hld, bool before_mipi)
{
	int ret = LCD_KIT_OK;
	int i;
	struct lcd_kit_array_data *pevent = NULL;
	...
	for (i = 0; i < power_seq->power_on_seq.cnt; i++) {
		...
		ret = lcd_kit_event_handler_part(hld, pevent->buf[EVENT_NUM],
			pevent->buf[EVENT_DATA], pevent->buf[EVENT_DELAY], before_mipi);
		if (ret) {
			LCD_KIT_ERR("send event 0x%x error!\n",
				pevent->buf[EVENT_NUM]);
			break;
...
{
	return lcd_kit_panel_power_on_part(hld, true);
}

static int lcd_kit_panel_power_on_with_mipi(void *hld)
{
	return lcd_kit_panel_power_on_part(hld, false);
}
...
{
	if (common_info->is_ext_panel)
		return snprintf(buf, PAGE_SIZE, "%s\n", common_info->panel_ext_name);
	else
...
int lcd_kit_pwm_gpio_esd_check(void)
{
       int esd_te_check_res = 0;
       u32 gpio_val_pwm;

       if (!common_info->esd.gpio_detect_support) {
               LCD_KIT_ERR("not support pwm gpio esd check\n");
               return 0;
       }
       gpio_val_pwm = (u32)lcd_kit_get_gpio_value(
               common_info->esd.gpio_detect_num + common_info->esd.panel_gpio_offset, "esd_detect_gpio");

       if (gpio_val_pwm != common_info->esd.gpio_normal_value)
               esd_te_check_res = LCD_KIT_FAIL;
       else
               esd_te_check_res = LCD_KIT_OK;

       LCD_KIT_INFO("gpio_val_pwm = %d, esd check result:%d\n",
               gpio_val_pwm, esd_te_check_res);

       return esd_te_check_res;
}


int lcd_kit_dual_gpio_esd_check(void)
{
	int esd_te_check_res = 0;
	u32 gpio_val_te_s;
	u32 gpio_val_te_m;

	if (!common_info->esd.gpio_detect_dual_support) {
		LCD_KIT_ERR("not support dual gpio esd check\n");
		return 0;
	}

	gpio_val_te_m = (u32)lcd_kit_get_gpio_value(
		common_info->esd.gpio_detect_num_first + common_info->esd.panel_gpio_offset, "esd_detect_gpio_first");
	gpio_val_te_s = (u32)lcd_kit_get_gpio_value(
		common_info->esd.gpio_detect_num_second + common_info->esd.panel_gpio_offset, "esd_detect_gpio_second");

	if ((gpio_val_te_m != common_info->esd.gpio_normal_value_first) ||
		(gpio_val_te_s != common_info->esd.gpio_normal_value_second))
		esd_te_check_res = LCD_KIT_FAIL;
	else
		esd_te_check_res = LCD_KIT_OK;

	LCD_KIT_INFO("gpio_val_te_m = %d, gpio_val_te_s = %d, esd check result:%d\n",
		gpio_val_te_m, gpio_val_te_s, esd_te_check_res);
...
static void lcd_kit_panel_parse_effect(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,panel-getblmaxnit-in-kernel",
		&common_info->blmaxnit.get_blmaxnit_in_kernel, 0);
...
static void lcd_kit_panel_parse_util(struct device_node *np)
{
	...
	/* lhbm  support*/
	lcd_kit_parse_u32(np, "lcd-kit,panel-match-batch-info-support",
		&common_info->lhbm.panel_match_batch_info_support, 0);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-read-batch-info",
		"lcd-kit,panel-read-batch-info-state", &common_info->lhbm.panel_read_batch_info);
	lcd_kit_parse_array_data(np, "lcd-kit,panel-batch-value",
		&common_info->lhbm.panel_batch_value);
	lcd_kit_parse_u32(np, "lcd-kit,local-hbm-support",
		&common_info->lhbm.support, 0);
	if (common_info->lhbm.support) {
		/* lhbm  disable*/
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,local-hbm-disable-cmds",
			"lcd-kit,local-hbm-disable-cmds-state", &common_info->lhbm.disable_cmds);
		/* lhbm off enable */
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,local-hbm-on-enable-cmds",
			"lcd-kit,local-hbm-on-enable-cmds-state", &common_info->lhbm.on_enable_cmds);
		/* lhbm on enable*/
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,local-hbm-off-enable-cmds",
			"lcd-kit,local-hbm-off-enable-cmds-state", &common_info->lhbm.off_enable_cmds);
	}
	/* panel type */
	lcd_kit_parse_u32(np, "lcd-kit,panel-ext-name-support",
		&common_info->panel_ext_info.support, 0);
	if (common_info->panel_ext_info.support) {
		/* panel ext name */
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-ver-cmds",
			"lcd-kit,panel-ver-cmds-state",
			&common_info->panel_ext_info.get_ver_cmds);
		lcd_kit_parse_u32(np, "lcd-kit,panel-ver-val",
			&common_info->panel_ext_info.ver_val, 0);
		common_info->is_ext_panel = false;
		common_info->panel_ext_name = (char *)of_get_property(np,
			"lcd-kit,panel-ext-name", NULL);
	}
	...
	if (common_info->esd.gpio_detect_support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,tp-esd-gpio-event",
			&common_info->esd.tp_esd_event, 0);
                lcd_kit_parse_u32(np, "lcd-kit,panel-gpio-offset",
                        &common_info->esd.panel_gpio_offset, 0);
	}
	/*dual esd */
	lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-detect-dual-support",
		&common_info->esd.gpio_detect_dual_support, 0);
	if (common_info->esd.gpio_detect_dual_support) {
		lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-detect-num-first",
			&common_info->esd.gpio_detect_num_first, 0);
		lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-normal-value-first",
			&common_info->esd.gpio_normal_value_first, 0);
		lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-detect-num-second",
			&common_info->esd.gpio_detect_num_second, 0);
		lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-normal-value-second",
			&common_info->esd.gpio_normal_value_second, 0);
		lcd_kit_parse_u32(np, "lcd-kit,panel-gpio-offset",
			&common_info->esd.panel_gpio_offset, 0);
	}
	...
	if (common_info->esd.support) {
		lcd_kit_parse_u32(np, "lcd-kit,esd-check-mode",
			&common_info->esd.esd_check_mode, 0);
		lcd_kit_parse_u32(np, "lcd-kit,esd-check-time-period",
			&common_info->esd.esd_check_period, 5000);
...
static void lcd_kit_parse_power(struct device_node *np)
{
	...
	if (power_hdl->lcd_ext_ldo.buf == NULL)
		lcd_kit_parse_array_data(np, "lcd-kit,lcd-ext-ldo",
			&power_hdl->lcd_ext_ldo);
}
...
static void lcd_kit_btb_init(void)
{
	...
	if (common_info->btb_check_type == LCD_KIT_BTB_CHECK_IRQ) {
		...
		/* only one btb pin is connected */
		if (common_info->lcd_btb_gpio.cnt <= 1)
			return;

		/* btb1 is down btb check */
		btb_gpio = common_info->lcd_btb_gpio.buf[1] + common_info->gpio_offset;
...
EXPORT_SYMBOL(lcd_backlight_i2c_dmd);
...
EXPORT_SYMBOL(lcd_bias_i2c_dmd);
