...
static bool lcd_kit_get_event_skip_send(int panel_id, uint32_t event,
	uint32_t enable)
{
	...
	if (lcd_ops->event_skip_send && lcd_ops->event_skip_send(panel_id) &&
		(enable == 0) && lcd_kit_is_power_event(event)) {
...
static bool lcd_kit_event_skip_send(int panel_id, void *hld,
	uint32_t event, uint32_t enable)
{
#ifdef LCD_KIT_DKMD
	return lcd_kit_get_event_skip_send(panel_id, event, enable);
#else
#if defined(CONFIG_LCD_KIT_HISI)
	...
	if (event == EVENT_MIPI) {
		if (enable)
			(void)lcd_kit_check_reg_report_dsm(LCD_ACTIVE_PANEL, hld,
				&common_info->check_reg_on);
		else
			(void)lcd_kit_check_reg_report_dsm(LCD_ACTIVE_PANEL, hld,
				&common_info->check_reg_off);
...
int lcd_kit_event_handler(int panel_id, void *hld, uint32_t event, uint32_t data,
	uint32_t delay)
{
	int ret = LCD_KIT_OK;
