	...
	unsigned char *item_name, unsigned char *tmp_name)
{
	...
	while (1) {
		if (kernel_read(fd, &ch, 1, &pos) != 1) {
			LCD_KIT_INFO("it's end of file\n");
			break;
		}
		cur_seek++;
		ret = vfs_llseek(fd, (off_t)cur_seek, 0);
		if (ret < 0)
...
int lcd_kit_dbg_parse_config(void)
{
	...
	fd = filp_open(LCD_KIT_PARAM_FILE_PATH, O_RDONLY, FILE_LIMIT);
	if (IS_ERR(fd)) {
...
int file_sys_is_ready(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "honor,lcd_panel_type");
	if (!np) {
		LCD_KIT_ERR("NOT FOUND device node %s!\n",
			"honor,lcd_panel_type");
		return LCD_KIT_FAIL;
	}
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	fd = filp_open(xml_name, O_RDONLY, FILE_LIMIT);
	if (IS_ERR(fd)) {
		LCD_KIT_ERR("can not access file %s!\n", xml_name);
		set_fs(fs);
		return LCD_KIT_FAIL;
	}
	filp_close(fd, NULL);
...
struct file *parse_open(const char *path)
{
	int ret;
	mm_segment_t fs;
	struct file *fd = NULL;
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	fd = filp_open(path, O_RDONLY, FILE_LIMIT);
	if (IS_ERR(fd)) {
		LCD_KIT_ERR("%s file doesn't exsit\n", path);
		set_fs(fs);
		return NULL;
