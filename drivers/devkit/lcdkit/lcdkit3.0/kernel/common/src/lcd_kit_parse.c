	...
	struct lcd_kit_array_data *out)
{
	...
	(defined(CONFIG_LCD_KIT_HONOR) || defined(CONFIG_LCD_KIT_MTK))
	if (is_dpd_mode()) {
...
int lcd_kit_parse_string_arrays(struct device_node *np,
	const char *propertyname, struct lcd_kit_string_arrays *string_array)
{
	int cnt, rc;

	cnt = of_property_count_strings(np, propertyname);
	string_array->cnt = cnt;
	rc = of_property_read_string_array(np, propertyname, string_array->buf, cnt);
	if (rc < 0) {
		string_array->cnt = 0;
		LCD_KIT_ERR("get fps test dts fail\n");
		return LCD_KIT_FAIL;
	}
