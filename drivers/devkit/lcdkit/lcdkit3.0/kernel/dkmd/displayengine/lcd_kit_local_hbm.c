...
static int display_engine_alpha_set_inner(int panel_id, void *hld, uint32_t alpha, bool is_on)
{
	...
	if (is_on)
		force_alpha_enable = FORCE_ENABLE;
	else
		force_alpha_enable = FORCE_DISABLE;

	LCD_KIT_DEBUG("alpha:%d, force_alpha_enable:%d\n", alpha, force_alpha_enable);
	...
	struct display_engine_alpha_map *input_alpha_map)
{
	...
	ret = display_engine_brightness_check_lut_inner(g_de_context.lum_lut,
