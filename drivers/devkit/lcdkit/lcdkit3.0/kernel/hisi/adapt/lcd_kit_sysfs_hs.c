...
#else
	if (common_info->panel_model != NULL)
#endif
		return snprintf(buf, PAGE_SIZE, "%s\n", common_info->panel_model);

	if (common_ops->get_panel_name)
		ret = common_ops->get_panel_name(LCD_ACTIVE_PANEL, buf);
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (common_ops->get_panel_info)
		ret = common_ops->get_panel_info(LCD_ACTIVE_PANEL, buf);
	return ret;
}
...
{
	if (lcd_kit_get_product_type() == LCD_NORMAL_TYPE)
		return SINGLE_PANEL;
	return DUAL_PANEL;
}

static int panel_info_cat(char *str, int len)
{
	int ret;
	int fps_rate;
	int i, j;
	...
	panel_id = PUBLIC_CFG->active_panel_id;
	...
	for (i = 0; i < panel_num; i++) {
		...
		get_oled_type(panel_id, oled_type, MAX_BUF);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (hisifd->panel_info.adaptive_frm_rate.is_support_ltpo) {
		fps_rate = hisifd->panel_info.adaptive_frm_rate.cur_frm_rate;
#ifdef LCD_FACTORY_MODE
		fps_rate = disp_info->fps.current_fps;
#endif
	}
#endif
	if (disp_info->fps.fps_need_high_60 != FPS_CONFIG_EN)
		ret = snprintf(str, sizeof(str), "current_fps:%d;default_fps:%d",
			fps_rate, (disp_info->fps.default_fps > 0) ? disp_info->fps.default_fps :
				LCD_FPS_60);
	else
	...
	ret += panel_info_cat(str, LCD_REG_LENGTH_MAX);
	LCD_KIT_INFO("%s\n", str);
	ret = snprintf(buf, PAGE_SIZE, "%s\n", str);
	return ret;
}

int lcd_fps_get_scence(int fps, int *scence)
{
	switch (fps) {
	case LCD_FPS_30:
...
static int lcd_fps_order_no_scence(char *str, size_t size)
{
	int ret;
	int i;
	char tmp[MAX_BUF] = {0};
	...
	if (!str) {
		LCD_KIT_ERR("lcd_fps_ltps_not_scence str NULL\n");
		return LCD_KIT_FAIL;
	}
	...
	ret = snprintf_truncated_s(str, size, "1,%d,%d", disp_info->fps.default_fps,
		disp_info->fps.fps_switch_order.cnt / FPS_ELEM_NUM);

	for (i = 0; i < disp_info->fps.fps_switch_order.cnt;) {
		fps_rate = disp_info->fps.fps_switch_order.buf[i];
		if (i == 0)
			strncat_s(str, size, ",", strlen(","));
		else
			strncat_s(str, size, ";", strlen(";"));
		ret += snprintf_truncated_s(tmp, sizeof(tmp), "%d:%u", fps_rate,
			disp_info->fps.fps_switch_order.buf[i + 1]);
		ret = strncat_s(str, size, tmp, strlen(tmp));
		if (ret != EOK)
			return LCD_KIT_FAIL;
		i += FPS_ELEM_NUM;
	}
...
static int lcd_fps_order_scence(char *str, size_t size)
{
	int ret;
	int i;
	char tmp[MAX_BUF] = {0};
	...
	if (!str) {
		LCD_KIT_ERR("lcd_fps_ltps_get_scence str NULL\n");
		return LCD_KIT_FAIL;
	}
	...
	if (disp_info->fps.fps_need_high_60 != FPS_CONFIG_EN)
		ret = snprintf_truncated_s(str, size, "1,%d,%d", LCD_FPS_SCENCE_60P,
			disp_info->fps.fps_switch_order.cnt / FPS_ELEM_NUM);
	else
		ret = snprintf_truncated_s(str, size, "1,%d,%d", LCD_FPS_SCENCE_60,
			disp_info->fps.fps_switch_order.cnt / FPS_ELEM_NUM);
	...
	for (i = 0; i < disp_info->fps.fps_switch_order.cnt;) {
		...
		if (i == 0)
			...
			strncat_s(str, size, ";", strlen(";"));
		ret += snprintf_truncated_s(tmp, sizeof(tmp), "%d:%u", fps_index,
			disp_info->fps.fps_switch_order.buf[i + 1]);
		ret = strncat_s(str, size, tmp, strlen(tmp));
		if (ret != EOK)
			return LCD_KIT_FAIL;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!disp_info->fps.support || !disp_info->fps.fps_switch_support) {
		ret = snprintf_truncated_s(buf, PAGE_SIZE, "0\n");
		return ret;
	} else if (disp_info->fps.fps_switch_support == LTPS_NOT_SCENCE) {
		/* LTPS not get scence */
		ret = lcd_fps_order_no_scence(str,sizeof(str));
		if (ret == LCD_KIT_FAIL) {
			ret = snprintf_truncated_s(buf, PAGE_SIZE, "0\n");
			return ret;
		}
		...
		/* LTPS get scence */
		ret = lcd_fps_order_scence(str,sizeof(str));
		if (ret == LCD_KIT_FAIL) {
			ret = snprintf_truncated_s(buf, PAGE_SIZE, "0\n");
			return ret;
		}
	...
	LCD_KIT_INFO("%s\n", str);
	ret = snprintf_truncated_s(buf, PAGE_SIZE, "%s\n", str);
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (disp_info->alpm.support)
		ret = snprintf(buf, PAGE_SIZE, "aod_function = %u\n",
			hisifd->aod_function);
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (common_ops->get_acl_mode)
		ret = common_ops->get_acl_mode(LCD_ACTIVE_PANEL, buf);
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (hisifd->panel_power_on) {
		if (common_ops->set_acl_mode) {
			hisifb_activate_vsync(hisifd);
			ret = common_ops->set_acl_mode(LCD_ACTIVE_PANEL, hisifd, val);
			hisifb_deactivate_vsync(hisifd);
		}
	}
	up(&hisifd->blank_sem);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (common_ops->get_vr_mode)
		ret = common_ops->get_vr_mode(LCD_ACTIVE_PANEL, buf);
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (hisifd->panel_power_on) {
		if (common_ops->set_vr_mode) {
			hisifb_activate_vsync(hisifd);
			ret = common_ops->set_vr_mode(LCD_ACTIVE_PANEL, hisifd, val);
			hisifb_deactivate_vsync(hisifd);
		}
	}
	up(&hisifd->blank_sem);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (common_ops->get_effect_color_mode)
		ret = common_ops->get_effect_color_mode(LCD_ACTIVE_PANEL, buf);
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (common_ops->set_effect_color_mode)
		ret = common_ops->set_effect_color_mode(LCD_ACTIVE_PANEL, val);
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n", (uint32_t)hisifd->vsync_ctrl.vsync_infinite);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE,
			...
			"blpwm_input_ena=%u\n"
			"lane_nums=%u\n"
			"panel_effect_support=%u\n"
			"color_temp_rectify_support=%u\n"
			"ddic_rgbw_support=%u\n"
			"hiace=%u\n"
			"effect_enable=%u\n"
			"effect_debug=%u\n"
			...
			pinfo->rgbw_support, pinfo->hiace_support,
			g_enable_effect, g_debug_effect,
	...
	struct hisi_fb_data_type *hisifd)
{
	...
	if (disp_info->oeminfo.brightness_color_uniform.support) {
		lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, read_value, OEM_INFO_SIZE_MAX,
	...
	unsigned int pic_index)
{
	...
	if ((pic_index >= DET1_INDEX) && (pic_index < DETECT_NUM)) {
		...
		ret = lcd_kit_dsi_cmds_tx(LCD_ACTIVE_PANEL, hisifd, &enter_cmds[pic_index]);
		if (ret) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	hisifd = lcd_kit_get_hisifd(dev);
	if (lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE && fold_panel_id == 1)
		hisifd = dpufd_list[BUILT_IN_PANEL_INDEX];
	if (!hisifd || !buf) {
		LCD_KIT_ERR("hisifd or buf is null\n");
		return LCD_KIT_FAIL;
	}
	...
	for (i = 0; i < sizeof(pinfo->sn_code); i++) {
		memset(str_tmp, 0, sizeof(str_tmp));
		ret = snprintf(str_tmp, sizeof(str_tmp), "%u,", pinfo->sn_code[i]);
		if (ret < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (disp_info->recovery_product_type)
		return snprintf(buf, PAGE_SIZE, "%u\n", disp_info->recovery_product_type);

	return snprintf(buf, PAGE_SIZE, "%u\n", hisifd->panel_info.product_type);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	down(&hisifd->blank_sem);
	hisifb_activate_vsync(hisifd);
	ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, dieid_info_data,
		PANEL_DIEID_DATA_SIZE_MAX, &disp_info->panel_dieid_info.cmds);
	hisifb_deactivate_vsync(hisifd);
	up(&hisifd->blank_sem);
	if (ret) {
