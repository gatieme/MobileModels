...
#define LCD_KIT_BUILTIN_PANEL 4
...
#define DISP_INFO lcd_kit_get_display_info(panel_id)
...
struct lcd_kit_sim_serial_panel_info {
	uint32_t sim_backlight_support;
	uint32_t pwr_ref_count_support;
};

struct lcd_kit_check_mipi {
	unsigned int support;
	unsigned int max_number;
	unsigned int sub_support;
	struct lcd_kit_dsi_panel_cmds cmds;
...
struct lcd_kit_disp_info {
	...
	/* product id */
	u32 product_id;
	/* recovery product type */
	u32 recovery_product_type;
	/* vr support */
	u32 vr_support;
	/* ulps support */
	u8 ramless_aod;
	...
	struct lcd_kit_sim_serial_panel_info sim_serial_panel;
	/* mipi error */
	struct lcd_kit_check_mipi check_mipi;
	/* dieid info */
	struct lcd_kit_panel_dieid_info panel_dieid_info;
	/* wait sen idle support */
	u32 wait_send_idle_support;
	/* end */
};
