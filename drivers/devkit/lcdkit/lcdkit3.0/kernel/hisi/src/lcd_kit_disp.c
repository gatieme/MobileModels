...
static struct hisi_fb_panel_data lcd_kit_data = {
	...
	.fps_scence_get = lcd_fps_get_scence,
};
...
struct lcd_kit_disp_info *lcd_kit_get_display_info(uint32_t panel_id)
{
...
int lcd_kit_get_panel_power_on_status(struct hisi_fb_data_type *hisifd)
{
	...
	/*
	* bit[8] = 1 : primary lcd power on
	* bit[8] = 0 : primary lcd power off
	* bit[9] = 1 : builtin lcd power on
	* bit[9] = 0 : builtin lcd power off
	*/
	if (PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE &&
		hisifd->index == LCD_KIT_BUILTIN_PANEL) {
		ret = (temp & 0x200) >> 9; // it means get bit[9] of temp
		LCD_KIT_INFO("inp32(SOC_SCTRL_SCBAKDATA11_ADDR(SOC_ACPU_SCTRL_BASE_ADDR))= 0x%x bit[9] = %d!\n",
			temp, ret);
	} else {
...
static void lcd_open_elvdd_detect(struct hisi_fb_data_type *hisifd)
{
	...
	if (ops->mipi_tx)
		ret = ops->mipi_tx(LCD_ACTIVE_PANEL, (void *)hisifd,
...
static void lcd_kit_first_screenon_hbm_set(struct hisi_fb_data_type *hisifd)
{
	...
	common_ops->hbm_set_handle(LCD_ACTIVE_PANEL, hisifd,
	...
	struct hisi_panel_info *pinfo, struct lcd_kit_ops *lcd_ops)
{
	...
	if (common_ops->panel_power_on)
		ret = common_ops->panel_power_on(LCD_ACTIVE_PANEL, (void *)hisifd);
...
#endif
#ifdef CONFIG_HISYSEVENT
	lcd_kit_hiview_hisysevent("LCD", "LCD_POWER_ON", 0, panel_name);
#endif
	...
	struct hisi_panel_info *pinfo, struct lcd_kit_ops *lcd_ops)
{
	...
	if (common_ops->panel_on_lp)
		ret = common_ops->panel_on_lp(LCD_ACTIVE_PANEL, (void *)hisifd);
...
static void lcd_init_set_last_bkl_level(struct platform_device *pdev,
	struct hisi_fb_data_type *hisifd)
{
#ifdef CONFIG_DPU_FB_V346
	if (PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE && g_last_bkl_lvl > 0) {
		if (g_last_bkl_lvl > hisifd->panel_info.bl_max)
			g_last_bkl_lvl = hisifd->panel_info.bl_max;

		LCD_KIT_INFO("fb%d, set the last backlight level = %d\n", hisifd->index, g_last_bkl_lvl);
		lcd_kit_set_backlight(pdev, g_last_bkl_lvl);
		g_last_bkl_lvl = 0;
	}
#else
	void_unused(pdev);
	...
	struct hisi_panel_info *pinfo, struct lcd_kit_ops *lcd_ops)
{
	...
	if (common_ops->panel_on_hs)
		ret = common_ops->panel_on_hs(LCD_ACTIVE_PANEL, (void *)hisifd);
	/* record panel on time */
	lcd_kit_disp_on_record_time();
	if (common_ops->btb_check)
		common_ops->btb_check(LCD_ACTIVE_PANEL);
	if (lcd_kit_get_power_status(LCD_ACTIVE_PANEL)) {
	...
	lcd_init_set_last_bkl_level(pdev, hisifd);
...
static int lcd_kit_on(struct platform_device *pdev)
{
	...
	if (PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE) {
		if (hisifd->index == LCD_KIT_BUILTIN_PANEL)
...
static void lcd_kit_resume_hisi_panel_info(struct hisi_panel_info *pinfo)
{
	...
	if (pinfo->adaptive_frm_rate.is_support_ltpo)
		return;
#endif
...
#endif
#ifdef CONFIG_HISYSEVENT
		lcd_kit_hiview_hisysevent("LCD", "LCD_POWER_OFF", 0, panel_name);
#endif
		break;
	case LCD_UNINIT_MIPI_LP_SEND_SEQUENCE:
		if (common_ops->panel_off_lp)
			common_ops->panel_off_lp(LCD_ACTIVE_PANEL, hisifd);
...
#ifdef LCD_FACTORY_MODE
	if (lcd_kit_get_pt_station_status(LCD_ACTIVE_PANEL) == IN_POWER_TEST) {
	...
	struct lcd_kit_adapt_ops *adapt_ops)
{
	...
	if ((disp_info->fps.last_update_fps == LCD_KIT_FPS_HIGH) &&
		...
		ret = adapt_ops->mipi_tx(LCD_ACTIVE_PANEL, hisifd,
...
static ssize_t lcd_kit_fp_hbm_extern(struct hisi_fb_data_type *hisifd, int type)
{
	...
	case LCD_KIT_FP_HBM_ENTER:
		if (common_ops->fp_hbm_enter_extern)
			ret = common_ops->fp_hbm_enter_extern(LCD_ACTIVE_PANEL, hisifd);
		break;
	case LCD_KIT_FP_HBM_EXIT:
		if (common_ops->fp_hbm_exit_extern)
			ret = common_ops->fp_hbm_exit_extern(LCD_ACTIVE_PANEL, hisifd);
		break;
	default:
...
void display_on_before_backlight(struct hisi_fb_data_type *hisifd)
{
	...
	if (adapt_ops && adapt_ops->mipi_tx) {
		ret = adapt_ops->mipi_tx(LCD_ACTIVE_PANEL, (void *)hisifd,
			&common_info->display_on_before_backlight_cmds);
		if (ret)
...
static bool lcd_kit_need_enter_hbm(unsigned int bl_level, bool first_screenon)
{
	int panel_id = LCD_ACTIVE_PANEL;
	return (bl_level > common_info->hbm_backlight.normal_bl_max) &&
		(first_screenon || common_info->hbm_backlight.hbm_status == LCD_KIT_HBM_ON);
}

static int lcd_kit_hbm_backlight_handle(struct hisi_fb_data_type *hisifd, unsigned int bl_level, bool first_screenon)
{
	int ret = LCD_KIT_OK;
	int panel_id = LCD_ACTIVE_PANEL;
...
#ifdef LCD_KIT_ALPHA_LHBM
	bl_level = display_engine_get_mapped_hbm_bl(panel_id, bl_level);
	if (lcd_kit_need_enter_hbm(bl_level, first_screenon)) {
		...
		if (common_ops->enter_hbm && common_info->hbm_backlight.hbm_status == LCD_KIT_HBM_OFF)
			ret = common_ops->enter_hbm(panel_id, hisifd);
		if (common_ops->set_hbm_backlight)
			ret = common_ops->set_hbm_backlight(panel_id, hisifd, hbm_level);
	} else {
		if (common_ops->exit_hbm && common_info->hbm_backlight.hbm_status == LCD_KIT_HBM_ON)
			ret = common_ops->exit_hbm(panel_id, hisifd);
	...
	case BL_SET_BY_MIPI:
		if (hisifd->masklayer_flag == MASK_LAYER_SCREENON
			...
			if (common_info->hbm_backlight.support)
				ret = lcd_kit_hbm_backlight_handle(hisifd, bl_level, first_screenon);
			else
				ret = lcd_kit_mipi_set_backlight(hisifd, bl_level);
		}
		break;
...
static int lcd_kit_esd_check(struct platform_device *pdev)
{
	...
	if (common_ops->dsi_handle && !hisifd->esd_happened) {
		...
		ret_dsi = common_ops->dsi_handle(LCD_ACTIVE_PANEL, hisifd);
		if (ret_dsi)
	...
	if (esd_support && common_ops->esd_handle)
		ret = common_ops->esd_handle(LCD_ACTIVE_PANEL, hisifd);

	if (runmode_is_factory() && disp_info->check_mipi.support)
		lcd_kit_check_mipi_error(LCD_ACTIVE_PANEL, hisifd);
...
static int lcd_kit_set_fastboot(struct platform_device *pdev)
{
	...
	if (common_info->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC) {
		ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, (void *)hisifd,
	...
	struct dss_rect *dirty)
{
	...
	if (common_ops->dirty_region_handle) {
		mutex_lock(&COMMON_LOCK->mipi_lock);
		ret = common_ops->dirty_region_handle(LCD_ACTIVE_PANEL,
			hisifd, (struct region_rect *)dirty);
		mutex_unlock(&COMMON_LOCK->mipi_lock);
	}
	return ret;
}
	...
	const char *buf, size_t count)
{
	...
	if (common_ops->set_ce_mode)
		ret = common_ops->set_ce_mode(LCD_ACTIVE_PANEL, hisifd, mode);
	return count;
}
...
static ssize_t lcd_kit_hbm_set_func(struct hisi_fb_data_type *hisifd)
{
	...
	if (common_ops->hbm_set_handle)
		ret = common_ops->hbm_set_handle(LCD_ACTIVE_PANEL,
			hisifd, hisifd->de_info.last_hbm_level,
			hisifd->de_info.hbm_dimming,
			hisifd->de_info.hbm_level,
			disp_info->fps.last_update_fps);
	if (common_info->dfr_info.fps_lock_command_support) {
...
static ssize_t lcd_kit_set_ic_dim_on(struct hisi_fb_data_type *hisifd)
{
	...
	if (common_ops->set_ic_dim_on)
		ret = common_ops->set_ic_dim_on(LCD_ACTIVE_PANEL,
			hisifd, disp_info->fps.last_update_fps);
	return ret;
}
	...
	size_t count)
{
	...
	if (common_ops->set_cabc_mode) {
		mutex_lock(&COMMON_LOCK->mipi_lock);
		common_ops->set_cabc_mode(LCD_ACTIVE_PANEL, hisifd, mode);
		mutex_unlock(&COMMON_LOCK->mipi_lock);
	}
	return count;
}
...
void factory_init(struct hisi_panel_info *pinfo)
{
	...
	if (runmode_is_factory()) {
		if (common_info->esd.fac_esd_support == 1 || disp_info->check_mipi.support == 1) {
...
static int lcd_kit_probe(struct platform_device *pdev)
{
	...
	if (common_ops->common_init)
		common_ops->common_init(LCD_ACTIVE_PANEL, np);
	/* utils init */
	...
	if (lcd_ops) {
		if (lcd_ops->get_sn_code) {
			ret = lcd_ops->get_sn_code(LCD_ACTIVE_PANEL);
			if (ret < 0)
...
static int __init lcd_kit_init(void)
{
	...
	OF_PROPERTY_READ_U32_RETURN(np, "product_type", &floder_type);
	PUBLIC_CFG->product_type = floder_type;
	if (floder_type != LCD_DUAL_PENEL_TYPE)
