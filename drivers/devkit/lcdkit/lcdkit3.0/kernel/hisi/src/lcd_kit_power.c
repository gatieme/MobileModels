...
static struct regulator *ext_iovcc;
static struct regulator *vci;
static struct regulator *vdd;
static struct regulator *bridge_vdd;
/* global gpio */
...
/* ldo power */
static struct vcc_desc ext_iovcc_init_cmds[] = {
	/* vcc get */
	{ DTYPE_VCC_GET, IOVCC_NAME, &ext_iovcc, 0, 0, WAIT_TYPE_MS, 0 },
	/* io set voltage */
	{ DTYPE_VCC_SET_VOLTAGE, IOVCC_NAME, &ext_iovcc, 1800000, 1800000, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc bridge_vdd_init_cmds[] = {
	/* vcc get */
	{ DTYPE_VCC_GET, BRIDGE_VDD_NAME, &bridge_vdd, 0, 0, WAIT_TYPE_MS, 0 },
	/* vcc set voltage */
	{ DTYPE_VCC_SET_VOLTAGE, BRIDGE_VDD_NAME, &bridge_vdd, 1250000, 1250000, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc ext_iovcc_finit_cmds[] = {
	/* vcc put */
	{ DTYPE_VCC_PUT, IOVCC_NAME, &ext_iovcc, 0, 0, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc bridge_vdd_finit_cmds[] = {
	/* vcc put */
	{ DTYPE_VCC_PUT, BRIDGE_VDD_NAME, &bridge_vdd, 0, 0, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc ext_iovcc_enable_cmds[] = {
	/* vcc enable */
	{ DTYPE_VCC_ENABLE, IOVCC_NAME, &ext_iovcc, 0, 0, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc bridge_vdd_enable_cmds[] = {
	/* vcc enable */
	{ DTYPE_VCC_ENABLE, BRIDGE_VDD_NAME, &bridge_vdd, 0, 0, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc ext_iovcc_disable_cmds[] = {
	/* vcc disable */
	{ DTYPE_VCC_DISABLE, IOVCC_NAME, &ext_iovcc, 0, 0, WAIT_TYPE_MS, 0 },
};
...
static struct vcc_desc bridge_vdd_disable_cmds[] = {
	/* vcc disable */
	{ DTYPE_VCC_DISABLE, BRIDGE_VDD_NAME, &bridge_vdd, 0, 0, WAIT_TYPE_MS, 0 },
};
...
int lcd_kit_gpio_tx(uint32_t type, uint32_t op)
{
	...
	if (lcd_kit_get_power_status(LCD_ACTIVE_PANEL) && (type != LCD_KIT_MIPI_SWITCH)) {
	...
	case LCD_KIT_MIPI_SWITCH:
		...
		break;
	case LCD_KIT_BRIDGE_RST:
		g_lcd_kit_gpio = power_hdl->lcd_bridge_rst.buf[POWER_NUM];
		break;
	case LCD_KIT_BRIDGE_VDD:
		g_lcd_kit_gpio = power_hdl->lcd_bridge_vdd.buf[POWER_NUM];
		break;
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	case LCD_KIT_IOVCC:
		if (power_hdl->lcd_iovcc.buf[POWER_NUM] <= 0)
			break;
		if (enable) {
			if (panel_id == LCD_EXT_PANEL)
				ret = vcc_cmds_tx(NULL, ext_iovcc_enable_cmds,
					ARRAY_SIZE(ext_iovcc_enable_cmds));
			else
				ret = vcc_cmds_tx(NULL, iovcc_enable_cmds,
					ARRAY_SIZE(iovcc_enable_cmds));
		} else {
			if (panel_id == LCD_EXT_PANEL)
				ret = vcc_cmds_tx(NULL, ext_iovcc_disable_cmds,
					ARRAY_SIZE(ext_iovcc_disable_cmds));
			else
	...
	case LCD_KIT_VDD:
		...
		break;
	case LCD_KIT_BRIDGE_VDD:
		if (power_hdl->lcd_bridge_vdd.buf[POWER_NUM] <= 0)
			break;
		if (enable)
			ret = vcc_cmds_tx(NULL, bridge_vdd_enable_cmds,
				ARRAY_SIZE(bridge_vdd_enable_cmds));
		else
			ret = vcc_cmds_tx(NULL, bridge_vdd_disable_cmds,
				ARRAY_SIZE(bridge_vdd_disable_cmds));
		break;
	default:
		ret = LCD_KIT_FAIL;
...
{
	int ret = LCD_KIT_OK;
	int panel_id = g_pwr_off_optimize_cfg.panel_id;
	...
	case LCD_KIT_VCI:
		...
		break;
	case LCD_KIT_IOVCC:
		if (panel_id == LCD_EXT_PANEL && PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE)
			ret = vcc_cmds_tx(NULL, ext_iovcc_disable_cmds,
				ARRAY_SIZE(ext_iovcc_disable_cmds));
		else
			ret = vcc_cmds_tx(NULL, iovcc_disable_cmds,
	...
	for (i = 0; i < cnt; i++) {
		if (cm->dtype == DTYPE_VCC_SET_VOLTAGE) {
			...
			if (!strncmp(cm->id, VSP_NAME, strlen(cm->id))) {
				cm->min_uV = (int)power_hdl->lcd_vsp.buf[POWER_VOL];
				cm->max_uV = (int)power_hdl->lcd_vsp.buf[POWER_VOL];
			}
			if (!strncmp(cm->id, VSN_NAME, strlen(cm->id))) {
				cm->min_uV = (int)power_hdl->lcd_vsn.buf[POWER_VOL];
				cm->max_uV = (int)power_hdl->lcd_vsn.buf[POWER_VOL];
			}
			if (!strncmp(cm->id, BIAS_NAME, strlen(cm->id))) {
				cm->min_uV = (int)power_hdl->lcd_vsp.buf[POWER_VOL];
				cm->max_uV = (int)power_hdl->lcd_vsp.buf[POWER_VOL];
			}
			if (!strncmp(cm->id, BRIDGE_VDD_NAME, strlen(cm->id))) {
				cm->min_uV = (int)power_hdl->lcd_bridge_vdd.buf[POWER_VOL];
				cm->max_uV = (int)power_hdl->lcd_bridge_vdd.buf[POWER_VOL];
			}
		}
		cm++;
	}
...
static int iovcc_process(struct platform_device *pdev)
{
	...
	if (panel_id == LCD_EXT_PANEL && PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE) {
		/* init iovcc */
		lcd_kit_power_set(ext_iovcc_init_cmds,
			ARRAY_SIZE(ext_iovcc_init_cmds));
		ret = vcc_cmds_tx(pdev, ext_iovcc_init_cmds,
			ARRAY_SIZE(ext_iovcc_init_cmds));
		if (ret != 0) {
...
static int bridge_vdd_process(struct platform_device *pdev)
{
	int ret = LCD_KIT_OK;
	int panel_id = LCD_ACTIVE_PANEL;

	if (power_hdl->lcd_bridge_vdd.buf &&
		power_hdl->lcd_bridge_vdd.buf[POWER_MODE] == REGULATOR_MODE) {
		/* judge bridge vdd is valid */
		if (power_hdl->lcd_bridge_vdd.buf[POWER_NUM] > 0) {
			/* init bridge vdd */
			lcd_kit_power_set(bridge_vdd_init_cmds,
				ARRAY_SIZE(bridge_vdd_init_cmds));
			ret = vcc_cmds_tx(pdev, bridge_vdd_init_cmds,
				ARRAY_SIZE(bridge_vdd_init_cmds));
			if (ret != 0) {
				LCD_KIT_ERR("LCD bridge vdd init failed!\n");
				return ret;
			}
			ret = vcc_cmds_tx(pdev, bridge_vdd_enable_cmds,
				ARRAY_SIZE(bridge_vdd_enable_cmds));
			if (ret != 0) {
				LCD_KIT_ERR("LCD bridge vdd enable failed!\n");
				return ret;
			}
		}
	}
...
int lcd_kit_power_finit(struct platform_device *pdev)
{
	...
	if (power_hdl->lcd_iovcc.buf &&
		power_hdl->lcd_iovcc.buf[POWER_MODE] == REGULATOR_MODE) {
		if (panel_id == LCD_EXT_PANEL) {
			ret = vcc_cmds_tx(pdev, ext_iovcc_finit_cmds, ARRAY_SIZE(ext_iovcc_finit_cmds));
		} else {
	...
	if (power_hdl->lcd_bridge_vdd.buf &&
		power_hdl->lcd_bridge_vdd.buf[POWER_MODE] == REGULATOR_MODE)
		ret = vcc_cmds_tx(pdev, bridge_vdd_finit_cmds, ARRAY_SIZE(bridge_vdd_finit_cmds));
	return ret;
}
...
static int lcd_kit_bridge_vdd_init(void)
{
	int ret = LCD_KIT_OK;
	int panel_id = LCD_ACTIVE_PANEL;
	...
	if (power_hdl->lcd_bridge_vdd.buf &&
                ...
                if (power_hdl->lcd_bridge_vdd.buf[POWER_NUM] > 0) {
                        /* init bridge vdd */
                        lcd_kit_power_set(bridge_vdd_init_cmds, ARRAY_SIZE(bridge_vdd_init_cmds));
                        ret = vcc_cmds_tx(NULL, &bridge_vdd_init_cmds[1], 1);
                        if (ret != 0) {
                                LCD_KIT_ERR("LCD bridge vdd init failed!\n");
                                return ret;
                        }
...
static int dpd_bridge_vdd_init(struct platform_device *pdev)
{
	int ret;
	int panel_id = LCD_ACTIVE_PANEL;
...
int dpd_regu_init(struct platform_device *pdev)
{
	...
	ret = dpd_bridge_vdd_init(pdev);
	if (ret) {
		LCD_KIT_ERR("dpd bridge vdd init fail\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
