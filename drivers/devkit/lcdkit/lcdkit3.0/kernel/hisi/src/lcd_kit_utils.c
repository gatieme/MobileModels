...
#if defined (CONFIG_LOG_EXCEPTION) && defined (CONFIG_HUAWEI_DSM)
...
#define MAX_WAIT_SEND_IDLE_TIME 2
...
static const int default_gamma_band_level = 3;
bool g_skip_power_on_off = false;
int tp_first_notity_switch_screen_flag = 0;
static struct completion lcd_send_idle_cmd_done;
int lcd_kit_send_idle_cmd_state = 0;

int lcd_kit_check_panel_power_on_status(struct hisi_fb_data_type *hisifd)
{
	return lcd_kit_get_panel_power_on_status(hisifd);
}

void lcd_kit_notify_send_idle_cmd_begin(void)
{
	if (!disp_info->wait_send_idle_support)
		return;

	lcd_kit_send_idle_cmd_state = 1;
}
void lcd_kit_notify_send_idle_cmd_end(void)
{
	if (!disp_info->wait_send_idle_support)
		return;

	complete_all(&lcd_send_idle_cmd_done);
	lcd_kit_send_idle_cmd_state = 0;
}
void lcd_kit_wait_send_idle_cmd_done(void)
{
	if (!disp_info->wait_send_idle_support)
		return;

	if (lcd_kit_send_idle_cmd_state) {
		reinit_completion(&lcd_send_idle_cmd_done);
		if (!wait_for_completion_timeout(&lcd_send_idle_cmd_done,
			msecs_to_jiffies(MAX_WAIT_SEND_IDLE_TIME)))
			LCD_KIT_ERR("wait send idle cmd done time out!\n");
		mdelay(MAX_WAIT_SEND_IDLE_TIME);
		LCD_KIT_INFO("wait send idle cmd end\n");
	}
}
...
static int lcd_kit_get_esd_support(int panel_id)
{
...
void lcd_esd_enable(struct hisi_fb_data_type *hisifd, int enable)
{
	...
	if (lcd_kit_get_esd_support(panel_id)) {
		pinfo->esd_enable = (unsigned int)enable;
		msleep(500);
	}
	LCD_KIT_INFO("pinfo->esd_enable = %d\n", pinfo->esd_enable);
}

static int lcd_kit_judge_value(unsigned int judge_type, char read_value,
...
static void lcd_kit_print_mipi_error_reg(int panel_id, void *hld)
{
	...
	ret = adapt_ops->mipi_rx(panel_id, hld, read_value,
...
void lcd_kit_check_mipi_error(int panel_id, void *hld)
{
#ifdef CONFIG_HUAWEI_DSM
	int ret = LCD_KIT_OK;
	int i;
	unsigned int max_err_number;
	unsigned int flag_err = 0;
	...
	if (disp_info->check_mipi.sub_support)
		lcd_kit_print_mipi_error_reg(panel_id, hld);
	...
	for (i = 0; i < disp_info->check_mipi.value.cnt; i++) {
		if (lcd_kit_judge_value(judge_type[i], read_value[i], expect_value[i])) {
			LCD_KIT_ERR("[ERROR]read_value[%d] = 0x%x, but expect_value = 0x%x!\n",
				i, read_value[i], expect_value[i]);
			flag_err++;
			if (flag_err >= max_err_number) {
				flag_err = 0;
		...
		LCD_KIT_INFO("judge_type = %d, read_value[%d] = 0x%x, expect_value = 0x%x\n",
			judge_type[i], i, read_value[i], expect_value[i]);
	}
	LCD_KIT_INFO("mipi check result:%d\n", ret);
#else
	(void)panel_id;
	(void)hld;
#endif
...
struct hisi_fb_data_type *lcd_kit_get_hisifd(struct device *dev)
{
	if (PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE &&
		LCD_ACTIVE_PANEL == LCD_EXT_PANEL) {
#ifdef CONFIG_DPU_FB_AOD
		if (dpufd_list[BUILT_IN_PANEL_INDEX]->panel_power_on ||
			dpufd_list[BUILT_IN_PANEL_INDEX]->enable_fast_unblank_for_dual_display)
			return dpufd_list[BUILT_IN_PANEL_INDEX];
#else
		if (dpufd_list[BUILT_IN_PANEL_INDEX]->panel_power_on)
			return dpufd_list[BUILT_IN_PANEL_INDEX];
#endif
	...
	return dpufd_list[PRIMARY_PANEL_IDX];
}
...
int lcd_kit_rgbw_set_handle(struct hisi_fb_data_type *hisifd)
{
	...
	if (disp_info->rgbw.backlight_cmds.cmds &&
		...
		rgbw_bl_level = (int)(rgbw_backlight * disp_info->rgbw.rgbw_bl_max /
	...
	struct lcd_kit_adapt_ops *adapt_ops)
{
	...
	// read otp gamma
	if (adapt_ops->mipi_rx(LCD_ACTIVE_PANEL, hld, gamma_60hz, GAMMA_TOTAL_LEN - 1,
	...
	void *hld, struct lcd_kit_adapt_ops *adapt_ops, uint8_t param[])
{
	...
	for (retry_count = 0; retry_count < FLASH_GAMMA_SINGLE_PARAM_RETRY_MAX;
		retry_count++) {
		if (adapt_ops->mipi_rx(LCD_ACTIVE_PANEL, hld, param, FLASH_GAMMA_RX_BUF_SIZE - 1,
...
int lcd_kit_read_fps_gamma(void *hld)
{
	...
	ret = adapt_ops->mipi_tx(LCD_ACTIVE_PANEL, hld,
...
static int lcd_kit_updt_panel_fps(struct hisi_fb_data_type *hisifd, char fps_scence)
{
	...
	/* 256 indicates the maximum length of sdi_cmd */
	if ((cmds->cmd_cnt > 256) || (cmds->cmd_cnt < 0)) {
		LCD_KIT_ERR("dsi0_cmds->cmds || dsi1_cmds->cmds has access overwriting risks\n");
		return LCD_KIT_FAIL;
	}
	...
	unsigned char type, const demura_set_info_t *info)
{
	...
	if (ret)
		return ret;
	/* 256 indicates the maximum length of sdi_cmd */
	if ((cmds0->cmd_cnt > 256) || (cmds1->cmd_cnt > 256) ||
		(cmds0->cmd_cnt < 0) || (cmds1->cmd_cnt < 0)) {
		LCD_KIT_ERR("dsi0_cmds->cmds || dsi1_cmds->cmds has access overwriting risks\n");
		return LCD_KIT_FAIL;
	}
	...
	unsigned char read_type, unsigned char len)
{
	...
	if (dsi == LCD_DSI0)
		ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, out, out_len, cmds);
...
static bool lcd_kit_fps_check_delay(struct hisi_panel_info *pinfo)
{
	...
	struct timespec64 afe_tv;
	struct timespec64 record_tv;

	if (disp_info->fps.fps_can_update_flag)
		return true;
	record_tv.tv_sec = disp_info->quickly_sleep_out.panel_on_record_tv.tv_sec;
	record_tv.tv_nsec = disp_info->quickly_sleep_out.panel_on_record_tv.tv_nsec;
	/* first high fsp switch */
	if (record_tv.tv_sec == 0 && record_tv.tv_nsec == 0 &&
		pinfo->fps == LCD_FPS_60P) {
		if (!lcd_kit_get_thp_afe_status(&afe_tv))
			return false;
		record_tv.tv_sec = afe_tv.tv_sec;
		record_tv.tv_nsec = afe_tv.tv_nsec;
	}
	memset(&tv, 0, sizeof(struct timespec64));
	ktime_get_real_ts64(&tv);
	/* change s to ns */
	delta_time = (tv.tv_sec - record_tv.tv_sec) * 1000000000 +
		tv.tv_nsec - record_tv.tv_nsec;
	/* change ns to ms */
	delta_time /= 1000000;
...
static int lcd_kit_updt_mipi_fps(struct hisi_fb_data_type *hisifd, int scence)
{
	...
	long timeout = (long)msecs_to_jiffies(10);
	...
	if (common_info->dfr_info.fps_lock_command_support && (int)pinfo->fps != frm_rate) {
		ret = wait_event_interruptible_timeout(common_info->dfr_info.fps_wait,
			common_info->dfr_info.hbm_status, timeout);
		common_info->dfr_info.fps_dfr_status = FPS_DFR_STATUS_DOING;
		LCD_KIT_DEBUG("fps %u->%d, ret = %d", pinfo->fps, frm_rate, ret);
		if (ret == 0)
			LCD_KIT_INFO("wait time out and set fps doing\n");
	}
	...
	pinfo->fps_scence = (uint8_t)scence;
	pinfo->fps_updt = (uint8_t)frm_rate;
	LCD_KIT_INFO("take_effect_delayed_frm_cnt:%d, hsa:%d hbp:%d dpi_hsize:%d hline_time:%d width:%d vsa:%d vbp:%d vfp:%d vactive_line:%d dsi_bit_clk_upt:%d\n",
		pinfo->mipi_updt.take_effect_delayed_frm_cnt, pinfo->mipi_updt.hsa, pinfo->mipi_updt.hbp,
		pinfo->mipi_updt.dpi_hsize, pinfo->mipi_updt.hline_time, pinfo->mipi_updt.width, pinfo->mipi_updt.vsa,
		pinfo->mipi_updt.vbp, pinfo->mipi_updt.vfp, pinfo->mipi_updt.vactive_line, pinfo->mipi_updt.dsi_bit_clk_upt);
	return LCD_KIT_OK;
}
...
static int display_engine_set_color_by_index(struct hisi_fb_data_type *hisifd, uint32_t gamma_node_index)
{
	int ret;
	uint32_t fps;
	...
	fps = pinfo->fps;
	ret = display_engine_local_hbm_set_color_by_index(hisifd, fps, gamma_node_index);
	if (ret != LCD_KIT_OK) {
		LCD_KIT_ERR("display_engine_local_hbm_set_color_by_index error, ret:%d\n", ret);
		return LCD_KIT_FAIL;
	}
	return ret;
}

static int lcd_kit_get_fps(struct hisi_fb_data_type *hisifd, int *fps)
{
	struct dpu_panel_info *pinfo = NULL;

	if (!fps) {
		DPU_FB_ERR("fps is NULL\n");
		return LCD_KIT_FAIL;
	}
	if (!hisifd) {
		DPU_FB_ERR("hisifd is NULL\n");
		return LCD_KIT_FAIL;
	}
	pinfo = &(hisifd->panel_info);
	if (!pinfo) {
	...
	size_t count)
{
	...
	ret = lcd_kit_get_fps(hisifd, &fps);
	if (ret != LCD_KIT_OK) {
		DPU_FB_ERR("lcd_kit_get_fps error\n");
		return LCD_KIT_FAIL;
	}
	memset(&local_hbm_cmd, 0, sizeof(local_hbm_cmd));
	if (!hisifd || !cmd)
		return LCD_KIT_FAIL;
	/* param is a node array parameter. The value 1 indicates whether
	   the local HBM is enabled, the value 2 indicates the alpha value,
	   and the value 3 indicates the dbv. */
	sscanf(cmd,"%u %u %u %u", &param[0], &param[1], &param[2], &param[3]);
	...
	case DDIC_CIRCLE_COLOR:
		ret = display_engine_set_color_by_index(hisifd, (uint32_t)param[1]);
		break;
	case DDIC_LOCAL_HBM:
		...
		/* param is a node array parameter. The value 1 indicates whether
		  the local HBM is enabled, the value 2 indicates the alpha value,
		  and the value 3 indicates the dbv. */
		ret = display_engine_local_hbm_alpha_circle_with_alpha_and_dbv(hisifd,
			fps, (bool)param[1], (uint32_t)param[2],
			(uint32_t)param[3]);
		break;
	default:
		DPU_FB_ERR(
			"Invalid local_hbm_cmd.mode, local_hbm_cmd.mode:%d\n",
			local_hbm_cmd.mode);
		break;
	}
	return ret;
...
{
	uint32_t delta;
	struct timespec64 curr_time, tmp;
	ktime_get_real_ts64(&curr_time);
	tmp = disp_info->dbv_stat.last_time[PRIMARY_REGION];

	/* change s to ms */
	delta = (uint32_t)((curr_time.tv_sec - tmp.tv_sec) * 1000 +
		(curr_time.tv_nsec - tmp.tv_nsec) / 1000000);
	disp_info->dbv_stat.dbv[PRIMARY_REGION] += (level * delta);
	disp_info->dbv_stat.ori_dbv[PRIMARY_REGION] += (ori_level * delta);
	ktime_get_real_ts64(&disp_info->dbv_stat.last_time[PRIMARY_REGION]);
	...
	unsigned int ori_level)
{
	...
	ktime_get_real_ts64(&disp_info->dbv_stat.last_time[SLAVE_REGION]);
	...
	unsigned int ori_level)
{
	...
	ktime_get_real_ts64(&disp_info->dbv_stat.last_time[FOLD_REGION]);
...
static void _lcd_set_hbm_stat(struct hisi_fb_data_type *hisifd, int region, unsigned int level)
{
	...
	case PRIMARY_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbm_last_time[PRIMARY_REGION]);
		break;
	case SLAVE_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbm_last_time[SLAVE_REGION]);
		break;
	case FOLD_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbm_last_time[FOLD_REGION]);
		break;
	case REGION_MAX:
		tmp = disp_info->dbv_stat.hbm_last_time[PRIMARY_REGION];
		delta = (uint32_t)((curr_time.tv_sec - tmp.tv_sec) * 1000 +
...
static void _lcd_set_pwon_stat(int region, unsigned int level)
{
	...
	case PRIMARY_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.pwon_last_time[PRIMARY_REGION]);
		break;
	case SLAVE_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.pwon_last_time[SLAVE_REGION]);
		break;
	case FOLD_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.pwon_last_time[FOLD_REGION]);
		break;
	case REGION_MAX:
		tmp = disp_info->dbv_stat.pwon_last_time[PRIMARY_REGION];
		/* change s to ms */
...
static void _lcd_set_hbmon_stat(struct hisi_fb_data_type *hisifd, int region, unsigned int level)
{
	...
	case PRIMARY_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbmon_last_time[PRIMARY_REGION]);
		break;
	case SLAVE_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbmon_last_time[SLAVE_REGION]);
		break;
	case FOLD_REGION:
		...
		ktime_get_real_ts64(&disp_info->dbv_stat.hbmon_last_time[FOLD_REGION]);
		break;
	case REGION_MAX:
		tmp = disp_info->dbv_stat.hbmon_last_time[PRIMARY_REGION];
		/* change s to ms */
...
int lcd_kit_mipi_set_backlight(struct hisi_fb_data_type *hisifd, uint32_t level)
{
	...
	lcd_kit_wait_send_idle_cmd_done();
	if (pinfo->hbm_entry_delay > 0) {
		mutex_lock(&COMMON_LOCK->hbm_lock);
		ret = common_ops->set_mipi_backlight(LCD_ACTIVE_PANEL, hisifd, bl_level);
		pinfo->hbm_blcode_ts = ktime_get();
		mutex_unlock(&COMMON_LOCK->hbm_lock);
	} else {
		ret = common_ops->set_mipi_backlight(LCD_ACTIVE_PANEL, hisifd, bl_level);
	}
	hisifd->de_info.actual_bl_level = (int)bl_level;
	...
	while (udelay_cnt > 0) {
		...
		if ((disp_info != NULL) && (disp_info->transmission.separate_support)) {
			if ((pkg_status & 0x1) == 0x1 && !(phy_status & 0x2) &&
				(pkg_status & disp_info->transmission.separate_status_flag)) {
				is_timeout = 0;
				break;
			}
		} else {
	...
	struct hisi_panel_info *pinfo)
{
	...
	if (mode == SPR_DSC_MODE_SPR_ONLY || mode == SPR_DSC_MODE_SPR_AND_DSC) {
		...
		pinfo->spr.spr_lut_table_len = (uint32_t)spr_lut_table.cnt;
	}
...
static void lcd_kit_parse_check_mipi(struct device_node *np)
{
	int panel_id = LCD_ACTIVE_PANEL;
	...
	lcd_kit_parse_u32(np, "lcd-kit,check-mipi-sub-support",
		&disp_info->check_mipi.sub_support, 0);
...
static void lcd_kit_esd_need_platform_recover(struct device_node *np,
	struct hisi_panel_info *pinfo)
{
	int panel_id = LCD_ACTIVE_PANEL;

	if (runmode_is_factory()) {
		lcd_kit_parse_u32(np, "lcd-kit,esd-need-platform-recover",
			&pinfo->need_platform_recover, 0);
		if (common_info->esd.fac_esd_support)
			pinfo->need_platform_recover = 1;
	} else {
		pinfo->need_platform_recover = 1;
	}
}
...
void lcd_kit_pinfo_init(struct device_node *np, struct hisi_panel_info *pinfo)
{
	...
	/* reload support */
	lcd_kit_parse_u32(np, "lcd-kit,reload-support",  &pinfo->reload_support, 0);
	...
	lcd_kit_parse_u32(np, "lcd-kit,panel-bl-min", &pinfo->bl_min, 0);
	lcd_kit_parse_u32(np, "lcd-kit,panel-bl-max", &pinfo->bl_max, 0);
	lcd_kit_parse_u32(np, "lcd-kit,panel-dbv-max", &pinfo->dbv_max, 0);
	...
	lcd_kit_parse_u32(np, "lcd-kit,eink-lcd",
		&pinfo->eink_lcd, 0);
	lcd_kit_parse_u32(np, "lcd-kit,need-set-dsi1-te-ctrl",
	...
	lcd_kit_parse_u8(np, "lcd-kit,refresh-direction-ctrl",
		&pinfo->lcd_refresh_direction_ctrl, 0);

	/* effect info */
	lcd_kit_parse_u8(np, "lcd-kit,post-hihdr-support",
		&pinfo->post_hihdr_support, 0);
	...
	lcd_kit_parse_u8(np, "lcd-kit,dither-effect-support",
		&pinfo->de_panel_info.dither_effect_support, 0);
	lcd_kit_parse_u8(np, "lcd-kit,dither-noise-mode",
		&pinfo->de_panel_info.dither_noise_mode, 1);
	...
	lcd_kit_parse_u32(np, "lcd-kit,tiny-porch-ratio-support",
		&pinfo->support_tiny_porch_ratio, 0);
	...
	/* spr and dsc config init */
	OF_PROPERTY_READ_U8_DEFAULT(np, "lcd-kit,spr-dsc-mode",
		&pinfo->spr_dsc_mode, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,dummy-pixel",
	...
	lcd_kit_parse_u32(np, "lcd-kit,need-adjust-dsi-vcm",
		&pinfo->need_adjust_dsi_vcm, 0);
	if (pinfo->need_adjust_dsi_vcm) {
		lcd_kit_parse_u32(np, "lcd-kit,mipi-rg-lptx-sri-adjust",
			&pinfo->mipi.rg_lptx_sri_adjust, 0);
		lcd_kit_parse_u32(np, "lcd-kit,mipi-rg-vrefsel-lptx-adjust",
			&pinfo->mipi.rg_vrefsel_lptx_adjust, 0);
	}
	lcd_kit_parse_u32(np, "lcd-kit,support-de-emphasis",
	...
	lcd_kit_parse_u32(np, "lcd-kit,dual-lcd-support",
		&pinfo->dual_lcd_support, 0);
	lcd_kit_parse_u8(np, "lcd-kit,use-dsi1", &pinfo->use_dsi1, 0);
	...
	lcd_kit_parse_u32(np, "lcd-kit,bypass-device-compose",
		&pinfo->bypass_device_compose, 0);
	// change MHZ to HZ
	...
	lcd_kit_esd_need_platform_recover(np, pinfo);
	...
	/* tp read sn support */
	lcd_kit_parse_u32(np, "lcd-kit,tp-read-sn-support",
		&pinfo->tp_read_sn, 0);

	lcd_kit_parse_u8(np, "lcd-kit,lcd-kit-pc-support",
		&pinfo->lcd_kit_pc_support, 0);
	if (pinfo->lcd_kit_pc_support) {
		pinfo->is_only_use_tbu0 = true;
		pinfo->is_only_use_online_axi = true;
	}
}
...
static int lcd_kit_panel_version_read(struct hisi_fb_data_type *hisifd)
{
	...
	ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd,
...
int lcd_kit_panel_version_init(struct hisi_fb_data_type *hisifd)
{
	...
	for (i = 0; i < disp_info->panel_version.version_number; i++) {
		for (j = 0; j < disp_info->panel_version.value_number; j++) {
			...
			if (j == (disp_info->panel_version.value_number - 1)) {
				...
				strncpy_s(disp_info->panel_version.lcd_panel_version, LCD_PANEL_VERSION_SIZE,
					hisifd->panel_info.lcd_panel_version, LCD_PANEL_VERSION_SIZE - 1);
	...
	if (i == (u32)disp_info->panel_version.version_number) {
...
static void lcd_kit_gamma_calibration(struct device_node *np)
{
...
static void lcd_kit_brightness_and_color_uniform(struct device_node *np)
{
	lcd_kit_parse_u32(np,
...
static void lcd_kit_oem_information(struct device_node *np)
{
...
void lcd_kit_parse_effect(struct device_node *np)
{
	...
	/* gamma calibration */
	lcd_kit_gamma_calibration(np);
	/* brightness and color uniform */
	lcd_kit_brightness_and_color_uniform(np);
	/* oem information */
	lcd_kit_oem_information(np);
	/* rgbw */
...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,fps-ifbc-type", &disp_info->fps.fps_ifbc_type, 0);
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-60-cmds",
		"lcd-kit,fps-to-60-cmds-state", &disp_info->fps.fps_to_cmds[index]);
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-normal-60-cmds",
		"lcd-kit,fps-to-normal-60-cmds-state", &disp_info->fps.fps_to_cmds[index]);
	...
	lcd_kit_parse_u32(np, "lcd-kit,fps-update-to90", &disp_info->fps.fps_update_to_90, 0);
...
static void lcd_kit_parse_sot_err_check(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,check-sot-err-support",
		&disp_info->sot_err_check.support, 0);
	if (!disp_info->sot_err_check.support)
		return;

	lcd_kit_parse_dcs_cmds(np, "lcd-kit,check-sot-err-cmds",
		"lcd-kit,check-sot-err-cmds-state",
		&disp_info->sot_err_check.check_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,sot-err-repair11-cmds",
		"lcd-kit,sot-err-repair-cmds-state",
		&disp_info->sot_err_check.repair11_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,sot-err-repair12-cmds",
		"lcd-kit,sot-err-repair-cmds-state",
		&disp_info->sot_err_check.repair12_cmds);
	lcd_kit_parse_array_data(np, "lcd-kit,sot-err-value",
		&disp_info->sot_err_check.check_value);
	disp_info->sot_err_check.dsi0_repair_flag = SOT_REPAIR_NONE;
	disp_info->sot_err_check.dsi1_repair_flag = SOT_REPAIR_NONE;
	disp_info->sot_err_check.dsi0_sot_err_cnt = 0;
	disp_info->sot_err_check.dsi1_sot_err_cnt = 0;
}

static void lcd_kit_parse_sim_serial_panel(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,sim-serial-power-ref-count",
		&disp_info->sim_serial_panel.pwr_ref_count_support, 0);
	lcd_kit_parse_u32(np, "lcd-kit,sim-serial-backlight",
		&disp_info->sim_serial_panel.sim_backlight_support, 0);
}
...
void lcd_kit_parse_alpm(struct device_node *np)
{
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-low-brightness-cmds",
		...
		&disp_info->alpm.low_light_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-close-te-cmds",
		"lcd-kit,panel-aod-close-te-cmds-state",
		&disp_info->alpm.close_te_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-aod-open-te-cmds",
		"lcd-kit,panel-aod-open-te-cmds-state",
		&disp_info->alpm.open_te_cmds);
}
...
void lcd_kit_parse_util(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,recovery-product-type",
		&disp_info->recovery_product_type, 0);
	/* alpm */
	...
	if (disp_info->panel_version.support) {
		...
		if (disp_info->panel_version.cmds.cmds->dlen == 1) {
			...
			LCD_KIT_INFO("panel version special process\n");
		} else {
	...
	/* separate transmission */
	lcd_kit_parse_u32(np, "lcd-kit,separate-transmission-support",
		&disp_info->transmission.separate_support, 0);
	if (disp_info->transmission.separate_support)
		lcd_kit_parse_u32(np, "lcd-kit,separate-transmission-status-flag",
			&disp_info->transmission.separate_status_flag, 0x8000);
	/* pcd errflag */
	...
	lcd_kit_parse_u32(np, "lcd-kit,mipi-cmd-always-lp-mode",
		&disp_info->mipi_cmds_always_run_in_lpmode, 0);
	/* sot error check */
	lcd_kit_parse_sot_err_check(np);
	/* sim serial panel feature */
	lcd_kit_parse_sim_serial_panel(np);
}
...
int lcd_kit_check_pcd_errflag_check(struct hisi_fb_data_type *hisifd)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		(void)lcd_kit_dsi_cmds_tx(LCD_ACTIVE_PANEL, hisifd,
			&disp_info->pcd_errflag.switch_page_cmds);
		ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, read_pcd,
			LCD_KIT_PCD_SIZE - 1, &disp_info->pcd_errflag.read_pcd_cmds);
		expect_value = disp_info->pcd_errflag.pcd_value.buf;
		expect_value_cnt = (uint32_t)disp_info->pcd_errflag.pcd_value.cnt;
		if (ret == LCD_KIT_OK) {
	...
	if (disp_info->pcd_errflag.errflag_support) {
		(void)lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, read_errflag,
...
{
	int pcd_gpio = (int)disp_info->pcd_errflag.gpio_pcd;
	int errflag_gpio = (int)disp_info->pcd_errflag.gpio_errflag;
	...
	int len)
{
	...
	lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, read_value, len - 1,
		&disp_info->gamma_cal.cmds);
	return LCD_KIT_OK;
}
...
static void parse_xcc(struct hisi_panel_info *pinfo, const char *command)
{
	...
	if (command[strlen("xcc_support:")] == '0') {
		...
		if (pinfo->xcc_table == NULL) {
			...
			pinfo->xcc_table_len = (uint32_t)ARRAY_SIZE(xcc_table_def);
		}
	}
}
...
#ifdef LCD_FACTORY_MODE
		*status = lcd_kit_get_pt_station_status(LCD_ACTIVE_PANEL);
#endif
		ret = LCD_KIT_OK;
		break;
	default:
...
void lcd_kit_read_calicolordata_from_share_mem(struct lcd_kit_brightness_color_oeminfo *oeminfo)
{
	...
	vir_addr = (void *)ioremap_wc(g_reserved_him_base, g_reserved_him_size);
#else
	vir_addr = (void *)ioremap_wc(SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_BASE,
	...
	size_t count)
{
	...
	if (pdata->lcd_xcc_store) {
		ret = (ssize_t)pdata->lcd_xcc_store(hisifd->pdev, buf, count);
		if (ret == count) {
			retval = 0;
		} else {
	...
	int link_state)
{
	...
	if (disp_info->mipi_cmds_always_run_in_lpmode)
		link_state = LCD_KIT_DSI_LP_MODE;
	dsi_base_addr = lcd_kit_get_dsi_base_addr(hisifd);
	if (is_mipi_cmd_panel(hisifd)) {
		if (!disp_info->mipi_cmds_always_run_in_lpmode) {
			/* wait fifo empty */
	...
	uint32_t event, uint32_t data)
{
	...
	return lcd_kit_get_power_status(panel_id);
}
...
static void lcd_kit_set_power_by_thermal_handle(void *hld)
{
	int ret = 0;
	struct hisi_fb_data_type *hisifd = NULL;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	struct thermal_zone_device *thermal_zone = NULL;
	unsigned int cur_thermal = 0;
	int panel_id = LCD_ACTIVE_PANEL;
	...
	if (((common_info->set_power.get_thermal == 0) &&
		(cur_thermal <= common_info->set_power.thermal1)) ||
		((common_info->set_power.oldthermal >
		common_info->set_power.thermal2) &&
		(cur_thermal <= common_info->set_power.thermal2))) {
		ret = adapt_ops->mipi_tx(LCD_ACTIVE_PANEL, (void *)hisifd,
			&common_info->set_power.cmds_fir);
		LCD_KIT_INFO("thermal mode1\n");
	} else if (((common_info->set_power.get_thermal == 0) ||
		(common_info->set_power.oldthermal
		<= common_info->set_power.thermal1)) &&
		(cur_thermal > common_info->set_power.thermal1)) {
		ret = adapt_ops->mipi_tx(LCD_ACTIVE_PANEL, (void *)hisifd,
			&common_info->set_power.cmds_sec);
		LCD_KIT_INFO("thermal mode2\n");
	}
	common_info->set_power.get_thermal = 1;
	common_info->set_power.oldthermal = cur_thermal;
}

static int lcd_kit_set_power_by_thermal(void)
{
	struct hisi_fb_data_type *hisifd = NULL;
	...
	hisifb_vsync_disable_enter_idle(hisifd, true);
	hisifb_activate_vsync(hisifd);
	lcd_kit_set_power_by_thermal_handle(hisifd);
	hisifb_vsync_disable_enter_idle(hisifd, false);
	hisifb_deactivate_vsync(hisifd);
	up(&hisifd->power_sem);
...
static int lcd_kit_proximity_power_off(void)
{
	...
	if (lcd_kit_get_pt_mode(LCD_ACTIVE_PANEL)) {
	...
	if (common_ops->panel_power_off)
		common_ops->panel_power_off(LCD_ACTIVE_PANEL, hisifd);
	up(&DISP_LOCK->thp_second_poweroff_sem);
...
{
#ifdef CONFIG_DPU_FB_V346
	return (g_reserved_him_base + g_reserved_him_size - sizeof(*tplcd));
#else
	return (SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_BASE +
		SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_SIZE - sizeof(*tplcd));
#endif
}
...
static int lcd_kit_get_sn_code(int panel_id)
{
	...
	if (common_info && common_info->sn_code.support) {
		...
		if (pinfo->tp_read_sn != 0) {
			LCD_KIT_INFO("read sn from tp ic\n");
			return LCD_KIT_OK;
		}
		...
		if (disp_info && disp_info->oeminfo.barcode_2d.support) {
			ret = lcd_kit_dsi_cmds_rx(LCD_ACTIVE_PANEL, hisifd, read_value,
				OEM_INFO_SIZE_MAX, &disp_info->oeminfo.barcode_2d.cmds);
			if (ret != 0) {
				LCD_KIT_ERR("get sn_code error!\n");
				return LCD_KIT_FAIL;
			}
...
static bool lcd_kit_exec_ext_ts(void)
{
	if (lcd_kit_get_product_type() == LCD_FOLDER_TYPE ||
		lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE)
		return true;
	return false;
}
	...
	uint32_t enable)
{
	...
	if (pinfo->product_type != LCD_FOLDER_TYPE && pinfo->product_type != LCD_DUAL_PENEL_TYPE)
		return false;
	if (!common_ops->is_power_event)
		return false;
	if ((pinfo->product_type == LCD_DUAL_PENEL_TYPE) && !g_skip_power_on_off)
		return false;
	if ((pinfo->skip_power_on_off == FOLD_POWER_ON_OFF) &&
		(enable == 0)) {
		if (common_ops->is_power_event(event)) {
			LCD_KIT_INFO("skip power off!\n");
			return true;
		}
	}
	if ((pinfo->product_type == LCD_FOLDER_TYPE) && (pinfo->skip_power_on_off != SKIP_POWER_ON_OFF))
		return false;
	...
	if (event == EVENT_MIPI) {
		if (common_ops->check_reg_report_dsm)
			return false;
		if (enable)
			(void)common_ops->check_reg_report_dsm(LCD_ACTIVE_PANEL, hld,
...
static int lcd_kit_get_active_panel_id(void)
{
	return LCD_ACTIVE_PANEL;
}

void lcd_kit_notify_panel_switch(void)
{
	if (PUBLIC_CFG->product_type != LCD_DUAL_PENEL_TYPE) {
		LCD_KIT_INFO("is not dual panel no need notify panel switch\n");
		return;
	}
	if (tp_first_notity_switch_screen_flag == 0) {
		LCD_KIT_INFO("tp first switch screen\n");
		tp_first_notity_switch_screen_flag++;
		return;
	}
	if (hisifd_list[PRIMARY_PANEL_IDX]->panel_power_on ||
		hisifd_list[LCD_KIT_BUILTIN_PANEL]->panel_power_on) {
...
struct lcd_kit_ops g_lcd_ops = {
	...
	.set_power_by_thermal = lcd_kit_set_power_by_thermal,
...
static void lcd_kit_dsc_pred_enable_config(struct device_node *np,
	struct hisi_panel_info *pinfo)
{
	uint32_t block_pred_enable_support = 0;
	if (!np) {
	...
	lcd_kit_parse_u32(np, "lcd-kit,vesa-dsc-blk-pred-en-support",
		&block_pred_enable_support, 0);
	if (block_pred_enable_support)
		lcd_kit_parse_u32(np, "lcd-kit,vesa-dsc-blk-pred-en",
			&pinfo->vesa_dsc.block_pred_enable, 0);
}
	...
	struct hisi_panel_info *pinfo)
{
	...
	pinfo->gamma_lut_table_len = (uint32_t)array_data.cnt;
	...
	pinfo->igm_lut_table_len = (uint32_t)array_data.cnt;
	...
	pinfo->gmp_lut_table_len = (uint32_t)array_data.cnt;
	...
	pinfo->xcc_table_len = (uint32_t)array_data.cnt;
	LCD_KIT_DEBUG("-!\n");
}

static void parse_fps_char(struct device_node *np)
{
...
static void parse_fps_cmd(struct device_node *np)
{
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-1-cmds",
		"lcd-kit,fps-to-1-cmds-state",
		&disp_info->fps.fps_to_1_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-10-cmds",
		"lcd-kit,fps-to-10-cmds-state",
		&disp_info->fps.fps_to_10_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-15-cmds",
		"lcd-kit,fps-to-15-cmds-state",
		&disp_info->fps.fps_to_15_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-18-cmds",
		"lcd-kit,fps-to-18-cmds-state",
		&disp_info->fps.fps_to_18_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-20-cmds",
		"lcd-kit,fps-to-20-cmds-state",
		&disp_info->fps.fps_to_20_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-24-cmds",
		"lcd-kit,fps-to-24-cmds-state",
		&disp_info->fps.fps_to_24_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-30-cmds",
		"lcd-kit,fps-to-30-cmds-state",
		&disp_info->fps.fps_to_30_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-40-cmds",
		"lcd-kit,fps-to-40-cmds-state",
		&disp_info->fps.fps_to_40_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-45-cmds",
		"lcd-kit,fps-to-45-cmds-state",
...
static void parse_fps_ext(struct device_node *np)
{
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-1-commands",
		"lcd-kit,fps-to-1-cmds-state",
		&disp_info->fps.fps_to_1_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-10-commands",
		"lcd-kit,fps-to-10-cmds-state",
		&disp_info->fps.fps_to_10_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-15-commands",
		"lcd-kit,fps-to-15-cmds-state",
		&disp_info->fps.fps_to_15_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-18-commands",
		"lcd-kit,fps-to-18-cmds-state",
		&disp_info->fps.fps_to_18_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-20-commands",
		"lcd-kit,fps-to-20-cmds-state",
		&disp_info->fps.fps_to_20_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-24-commands",
		"lcd-kit,fps-to-24-cmds-state",
		&disp_info->fps.fps_to_24_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-30-commands",
		"lcd-kit,fps-to-30-cmds-state",
		&disp_info->fps.fps_to_30_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-40-commands",
		"lcd-kit,fps-to-40-cmds-state",
		&disp_info->fps.fps_to_40_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-45-commands",
		"lcd-kit,fps-to-45-cmds-state",
		&disp_info->fps.fps_to_45_cmds_90hz);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-60-commands",
		"lcd-kit,fps-to-60-cmds-state",
		&disp_info->fps.fps_to_60_cmds_90hz);
}

static void parse_fps_bl(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,bl-to-1-hz",
		&disp_info->fps.bl_to_1_hz, 0);
}

static void parse_fps_aod_bl(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,aod-bl-to-1-hz",
		&disp_info->fps.aod_bl_to_1_hz, 0);
}

static void parse_fps(struct device_node *np)
{
	parse_fps_char(np);
	parse_fps_cmd(np);
	parse_fps_ext(np);
...
static void lcd_kit_parse_panel_dieid(struct device_node *np)
{
...
static void lcd_kit_parse_disp(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,fps-support",
		&disp_info->fps.support, 0);
	...
	lcd_kit_parse_u32(np, "lcd-kit,wait-send-idle-support",
		&disp_info->wait_send_idle_support, 0);
	lcd_kit_parse_panel_dieid(np);
}

static void lcd_kit_init_display(struct device_node *np)
{
	if (!np) {
		LCD_KIT_ERR("np is null\n");
		return;
...
static void parse_ltpo_base_cmd(struct device_node *np, struct dpu_panel_info *pinfo)
{
	int i;
	int j;
	int table_num;
...
static void safe_fps_cmd_to_dsi_cmds(struct dpu_panel_info *pinfo, unsigned int frame_rate, int num)
{
	int i;
	int j;
	int table_num;
...
static void parse_ltpo_safe_cmd(struct device_node *np, struct dpu_panel_info *pinfo)
{
	int i;
	...
	for (i = 0; i < safe_freq.cnt; i++) {
		...
		safe_fps_cmd_to_dsi_cmds(pinfo, safe_freq.buf[i], i);
	}
	LCD_KIT_INFO("-\n");
}

#define MAX_TE_COUNT 20
static void parse_ltpo1_info(struct device_node *np, struct hisi_panel_info *pinfo)
{
	...
	for (i = 0; i < te_freq.cnt; i++) {
		...
		LCD_KIT_ERR("te_freq.buf[%d] is %d!\n", i, te_freq.buf[i]);
	}
    /* ltpo1 120hz dimming seq */
	if ((lcd_kit_parse_array_data(np, "lcd-kit,dimming-seq", &dimming_seq) == LCD_KIT_OK) &&
		...
		pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_num = dimming_seq.cnt;
		for (i = 0; i < dimming_seq.cnt; i++) {
			pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_list[i].frm_rate = dimming_seq.buf[i];
			pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_list[i].repeat_num = repeat_num.buf[i];
			LCD_KIT_INFO("frm_rate[%d] is %d!\n", i, dimming_seq.buf[i]);
			LCD_KIT_INFO("repeat_num[%d] is %d!\n", i, repeat_num.buf[i]);
		}
	}
    /* ltpo1 90hz dimming seq */
	if ((lcd_kit_parse_array_data(np, "lcd-kit,90hz-dimming-seq", &dimming_seq_90hz) == LCD_KIT_OK) &&
		...
		pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_90hz_num = dimming_seq_90hz.cnt;
		for (i = 0; i < dimming_seq_90hz.cnt; i++) {
			pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_90hz_list[i].frm_rate = dimming_seq_90hz.buf[i];
			pinfo->dfr_info.oled_info.ltpo_info.dimming_seq_90hz_list[i].repeat_num = repeat_num_90hz.buf[i];
			LCD_KIT_INFO("90hz seq frm_rate[%d] is %d!\n", i, dimming_seq_90hz.buf[i]);
			LCD_KIT_INFO("90hz seq repeat_num[%d] is %d!\n", i, repeat_num_90hz.buf[i]);
		}
	}
	lcd_kit_parse_u32(np, "lcd-kit,90hz-dimming-delay",
		&pinfo->dfr_info.oled_info.ltpo_info.dimming_delay_90hz, 3000); // 3000 is default value
	parse_ltpo_base_cmd(np, pinfo);
	parse_ltpo_safe_cmd(np, pinfo);
}

static void parse_ltpo2_info(struct device_node *np, struct hisi_panel_info *pinfo)
{
}

static void parse_ltpo_info(struct device_node *np, struct hisi_panel_info *pinfo)
{
	unsigned int i;
	...
	for (i = 0; i < pinfo->dfr_info.oled_info.fps_sup_num; i++) {
		pinfo->dfr_info.oled_info.fps_sup_seq[i] =
			disp_info->fps.panel_support_fps_list.buf[pinfo->dfr_info.oled_info.fps_sup_num - 1 - i];
	...
	if (pinfo->dfr_info.oled_info.ltpo_info.ver == PANEL_LTPO_V1)
		parse_ltpo1_info(np, pinfo);
	if (pinfo->dfr_info.oled_info.ltpo_info.ver == PANEL_LTPO_V2)
		parse_ltpo2_info(np, pinfo);
}

static void lcd_kit_parse_pinfo_fps(struct device_node *np,
	struct hisi_panel_info *pinfo)
{
	int i;
	...
	if (pinfo->dfr_info.oled_info.oled_type == LTPO)
		parse_ltpo_info(np, pinfo);
}
#endif

#ifdef CONFIG_DPU_FB_V346
static int lcd_kit_get_reserve_addr(void)
{
	struct device_node *him_np = NULL;
	int ret;

	him_np = of_find_node_by_path(DTS_HIM_PATH);
	if (!him_np) {
		LCD_KIT_ERR("get him data failed!\n");
		return LCD_KIT_FAIL;
	}
	ret = of_property_read_u32_index(him_np, "reg", 1, &g_reserved_him_base);
	if (ret) {
		LCD_KIT_ERR("failed to get him data base! ret = %d\n", ret);
		return LCD_KIT_FAIL;
	}
	ret = of_property_read_u32_index(him_np, "reg", 3, &g_reserved_him_size);
	if (ret) {
		LCD_KIT_ERR("failed to get him data size! ret = %d\n", ret);
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
...
int lcd_kit_utils_init(struct device_node *np, struct hisi_panel_info *pinfo)
{
	...
	lcd_kit_parse_dt(np);
	/* init display */
	lcd_kit_init_display(np);
	/* parse check mipi */
	lcd_kit_parse_check_mipi(np);
	/* init hisi pinfo */
	lcd_kit_pinfo_init(np, pinfo);
	/* parse mipi swing seting */
	lcd_kit_parse_u32(np, "lcd-kit,mipi-swing-adjust-support",
		&pinfo->mipi_swing_adjust_support, 0);
	/* parse mipi swing seting */
	lcd_kit_parse_u32(np, "lcd-kit,mipi-swing-adjust",
		&pinfo->mipi_swing_adjust, 0x0F);
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	lcd_kit_parse_pinfo_fps(np, pinfo);
#endif
	...
	lcd_kit_dsc_pred_enable_config(np, pinfo);
...
#ifdef CONFIG_DPU_FB_V346
	if (lcd_kit_get_reserve_addr())
		LCD_KIT_ERR("failed to get lcd reserve data!\n");
#endif
	init_completion(&lcd_send_idle_cmd_done);
	return LCD_KIT_OK;
}
