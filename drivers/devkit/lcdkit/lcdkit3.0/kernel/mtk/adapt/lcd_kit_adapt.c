...
void mipi_dsi_cmds_rx(struct mipi_dsi_device *dsi, struct dsi_cmd_desc *cmds)
{
	...
	case DTYPE_GEN_READ2:
		ret = mipi_dsi_generic_read(dsi, cmds->tx_buf[0],
			(size_t)(cmds->tx_len), cmds->rx_buf, cmds->rx_len);
		break;
	default:
		break;
	}
...
int lcd_kit_dsi_cmds_tx_compact(struct lcd_kit_dsi_panel_cmds *panel_cmds,
	bool lock_state)
{
	int ret;
	int i;
	int link_state;
	struct lcd_kit_dsi_cmd_desc *cmds_descs = NULL;
	struct lcd_kit_dsi_cmd_desc *last_cmd = NULL;
	struct mtk_ddic_dsi_msg cmd_msg;

	if (panel_cmds == NULL || panel_cmds->cmds == NULL) {
		LCD_KIT_ERR("panel_cmds is NULL\n");
		return -EINVAL;
	}
	if (panel_cmds->cmd_cnt <= 0) {
		LCD_KIT_ERR("panel_cmds->cmd_cnt is 0\n");
		return -EINVAL;
	}

	/* 2: mipi LP; 0: mipi HS */
	if (panel_cmds->link_state == 0)
		link_state = MIPI_DSI_MSG_USE_LPM;
	else
		link_state = 0;

	(void)memset(&cmd_msg, 0, sizeof(cmd_msg));
	cmd_msg.channel = 0;
	cmd_msg.flags = link_state;
	cmd_msg.tx_cmd_num = panel_cmds->cmd_cnt;
	cmds_descs = panel_cmds->cmds;
	for (i = 0; i < panel_cmds->cmd_cnt; i++) {
		cmd_msg.type[i] = cmds_descs[i].dtype;
		cmd_msg.tx_buf[i] = cmds_descs[i].payload;
		cmd_msg.tx_len[i] = cmds_descs[i].dlen;
	}
	ret = mtk_ddic_dsi_send_cmd_nolock(&cmd_msg, true);
	if (ret != 0) {
		LCD_KIT_ERR("fail cmds count:%d, dtype:0x%x, len:%d\n",
			panel_cmds->cmd_cnt, cmd_msg.type[0], cmd_msg.tx_len[0]);
		return ret;
	} else {
		LCD_KIT_INFO("succeed cmds count:%d, dtype:0x%x, len:%d\n",
			panel_cmds->cmd_cnt, cmd_msg.type[0], cmd_msg.tx_len[0]);
	}

	last_cmd = &cmds_descs[panel_cmds->cmd_cnt - 1];
	if (last_cmd->waittype == WAIT_TYPE_US) {
		udelay(last_cmd->wait);
	} else if (last_cmd->waittype == WAIT_TYPE_MS) {
		if (last_cmd->wait <= 10)
			mdelay(last_cmd->wait);
		else
			msleep(last_cmd->wait);
	} else {
		/* change s to ms */
		msleep(last_cmd->wait * 1000);
	}
...
extern void lcd_kit_set_dsi_power_mode(int enable);
static void lcd_set_mipi_power_mode(int enable)
{
	lcd_kit_set_dsi_power_mode(enable);
}
...
struct lcd_kit_adapt_ops adapt_ops = {
	...
	.set_mipi_power_mode = lcd_set_mipi_power_mode,
};
