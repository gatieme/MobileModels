...
struct lcd_kit_dsi_cmd_desc ddic_test_cmds[DDIC_READ_MAX_REG] = {0};
struct lcd_kit_dsi_panel_cmds ddic_debug = {
	NULL, 0, ddic_test_cmds, 0, 0, 0};

extern int honor_adc_get_value(int adc_channel);
...
static int lcd_fps_get_index(int real_fps)
{
	int fps_index;

	switch (real_fps) {
	case LCD_KIT_FPS_24:
		fps_index = LCD_FPS_24_INDEX;
		break;
	case LCD_KIT_FPS_25:
		fps_index = LCD_FPS_25_INDEX;
		break;
	case LCD_KIT_FPS_30:
		fps_index = LCD_FPS_30_INDEX;
		break;
	case LCD_KIT_FPS_60:
		fps_index = LCD_FPS_60_INDEX;
		break;
	case LCD_KIT_FPS_90:
		fps_index = LCD_FPS_90_INDEX;
		break;
	case LCD_KIT_FPS_120:
		fps_index = LCD_FPS_120_INDEX;
		break;
	case LCD_KIT_FPS_144:
		fps_index = LCD_FPS_144_INDEX;
		break;
	default:
		fps_index = LCD_FPS_60_INDEX;
		LCD_KIT_ERR("undefined fps index, reset index is = %d\n", LCD_FPS_60_INDEX);
		break;
	}
	return fps_index;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (disp_info->fps.support) {
		...
		if (disp_info->fps.order_mode == CUST_MODE) {
			strncat(str, ";current_index:", strlen(";current_index:"));
			memset(tmp, 0, sizeof(tmp));
			snprintf(tmp, sizeof(tmp), "%d", lcd_fps_get_index(disp_info->fps.current_fps));
			strncat(str, tmp, strlen(tmp));
		}
...
static void lcdkit_set_fps_test_station(unsigned int mode)
{
	if (mode >= FPS_MODE_MAX || mode < FPS_MMI_TEST) {
		LCD_KIT_ERR("fps test station set error, mode = %d\n", mode);
		return;
	}
	switch (mode) {
	case FPS_MMI_TEST:
		disp_info->fps_test_type = MMI_TEST;
		break;
	case FPS_RT_TEST:
		disp_info->fps_test_type = RT_TEST;
		break;
	case FPS_TPLCD_TEST:
		disp_info->fps_test_type = TPLCD_TEST;
		break;
	default:
		disp_info->fps_test_type = TPLCD_TEST;
		break;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* set fps test type */
	if (disp_info->fps.order_mode == CUST_MODE) {
		lcdkit_set_fps_test_station(val);
		return (ssize_t)LCD_KIT_OK;
	}

	switch (val) {
        case LCD_FPS_APP_SET_24:
		disp_info->fps.current_fps = 24;
		ret = lcd_kit_dsi_cmds_extern_tx(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_24]);
		break;
        case LCD_FPS_APP_SET_25:
		disp_info->fps.current_fps = 25;
		ret = lcd_kit_dsi_cmds_extern_tx(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_25]);
		break;
        case LCD_FPS_APP_SET_30:
		disp_info->fps.current_fps = 30;
		ret = lcd_kit_dsi_cmds_extern_tx(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_30]);
		break;
	...
	case LCD_FPS_APP_SET_120:
		...
		break;
        case LCD_FPS_APP_SET_144:
		disp_info->fps.current_fps = 144;
		ret = lcd_kit_dsi_cmds_extern_tx(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_144]);
		break;
	default:
...
static int lcd_fps_get_scence(int fps)
{
	...
	switch (fps) {
        case LCD_FPS_SCENCE_24:
                ret_fps = LCD_FPS_24_INDEX;
                break;
        case LCD_FPS_SCENCE_25:
                ret_fps = LCD_FPS_25_INDEX;
                break;
        case LCD_FPS_SCENCE_30:
                ret_fps = LCD_FPS_30_INDEX;
                break;
	...
	case LCD_FPS_SCENCE_120:
		...
		break;
        case LCD_FPS_SCENCE_144:
		ret_fps = LCD_FPS_144_INDEX;
		break;
	default:
		ret_fps = LCD_FPS_60_INDEX;
		break;
...
static void lcdkit_parse_fps_test(char *buf, struct lcd_kit_string_arrays *string_arrays)
{
	int i;
	int cnt;
	cnt = string_arrays->cnt;
	for (i = 0; i < cnt; i++) {
		strncat(buf, string_arrays->buf[i], strlen(string_arrays->buf[i]));
		LCD_KIT_INFO("%s\n", string_arrays->buf[i]);
		if (i != (cnt - 1))
			strncat(buf, "#", strlen("#"));
	}
...
static void lcd_fps_cust_test(char *buf, char test_type)
{
	LCD_KIT_INFO("fps test type:%d", test_type);

	switch (test_type)
	{
	case MMI_TEST:
		lcdkit_parse_fps_test(buf, &disp_info->lcd_test_config.mmi_test_list);
		break;
	case RT_TEST:
		lcdkit_parse_fps_test(buf, &disp_info->lcd_test_config.rt_test_list);
		break;
	case TPLCD_TEST:
		lcdkit_parse_fps_test(buf, &disp_info->lcd_test_config.tplcd_test_list);
		break;
	default:
		lcdkit_parse_fps_test(buf, &disp_info->lcd_test_config.tplcd_test_list);
		LCD_KIT_ERR("undefined fps test item!");
		break;
	}
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!disp_info->fps.support) {
		...
		if (disp_info->fps.order_mode == CUST_MODE) {
			lcd_fps_cust_test(str, disp_info->fps_test_type);
			LCD_KIT_INFO("%s\n", str);
			ret = snprintf(buf, PAGE_SIZE, "%s\n", str);
			return ret;
		}

		if (disp_info->fps.order_mode == INDEX_MODE)
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
			if (i == 0)
				...
				strncat(str, ";", strlen(";"));
			if (disp_info->fps.order_mode == INDEX_MODE) {
				index = lcd_fps_get_scence(disp_info->fps.panel_support_fps_list.buf[i]);
				index = i;
				ret += snprintf(tmp, sizeof(tmp), "%d:%d", index, ORDER_DELAY);
			} else {
				index = disp_info->fps.panel_support_fps_list.buf[i];
				timing = &disp_info->fps.fps_dsi_timming[index];
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (FACT_INFO->check_reg.support) {
		...
		mask_ptr = (char *)FACT_INFO->check_reg.mask.buf;

		lcd_kit_dsi_cmds_extern_rx(read_value,
			&FACT_INFO->check_reg.cmds, MAX_REG_READ_COUNT);
		for (i = 0; i < FACT_INFO->check_reg.value.cnt; i++) {
			if (i < FACT_INFO->check_reg.mask.cnt) {
				read_value[i] = read_value[i] & mask_ptr[i];
				LCD_KIT_INFO("mask_ptr[i]=0x%x, after map read_value[i]=0x%x!\n",
					mask_ptr[i], read_value[i]);
			}
...
static ssize_t lcd_reg_read_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint8_t read_value[DDIC_READ_MAX_REG] = {0};
	char str[LCD_REG_LENGTH_MAX] = {0};
	char tostr[DDIC_READ_MAX_REG] = {0};
	int i;
	int ret = 0;
	unsigned int panel_state;
	...
	LCD_KIT_INFO("lcd_reg_read_show enter!\n");

	lcd_kit_dsi_cmds_extern_rx(read_value, &ddic_debug, DDIC_READ_MAX_REG);
	for (i = 0; i < DDIC_READ_MAX_REG; i++) {
		LCD_KIT_INFO("read_value[%d]:0x%x\n", i, read_value[i]);
		snprintf(tostr, sizeof(tostr), " 0x%02x", read_value[i]);
		strncat(str, tostr, strlen(tostr));
	}
	LCD_KIT_INFO("\n %s\n", str);
	ret = snprintf(buf, PAGE_SIZE, "%s\n", str);
	for (i = 0; i < DDIC_READ_MAX_REG; i++) {
		kfree(ddic_test_cmds[i].payload);
		ddic_test_cmds[i].payload = NULL;
	}
	memset(&ddic_test_cmds[0], 0, DDIC_READ_MAX_REG * sizeof(struct lcd_kit_dsi_cmd_desc));
	ddic_debug.cmd_cnt = 0;
	return ret;
}
...
static ssize_t lcd_reg_read_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int reg_value[LCD_REG_LENGTH_MAX] = {0};
	char *cur;
	char *token;
	int i = 0;
	unsigned int panel_state;
	...
	if (ddic_debug.cmd_cnt >= DDIC_READ_MAX_REG) {
		LCD_KIT_ERR("cmd num is big max support %d\n", DDIC_READ_MAX_REG);
		return count;
	}
	...
	while (token) {
		...
		token = strsep(&cur, ",");
		if (i >= LCD_REG_LENGTH_MAX) {
			LCD_KIT_INFO("count is too long\n");
			return count;
		}
	}
	LCD_KIT_INFO("input_value_num:%d, dtype:0x%x, dlen:0x%x\n", i, reg_value[0], reg_value[1]);
	if (reg_value[1] <= 0 || i < (reg_value[1] + 2)) {
		LCD_KIT_ERR("input cmd len is error!", DDIC_READ_MAX_REG);
		return count;
	}
	ddic_test_cmds[ddic_debug.cmd_cnt].dtype = (char)reg_value[0];
	ddic_test_cmds[ddic_debug.cmd_cnt].dlen = (char)reg_value[1];
	ddic_test_cmds[ddic_debug.cmd_cnt].wait = 0x0A;
	ddic_test_cmds[ddic_debug.cmd_cnt].waittype = 0x0;
	ddic_test_cmds[ddic_debug.cmd_cnt].payload = kzalloc(ddic_test_cmds[ddic_debug.cmd_cnt].dlen, GFP_KERNEL);
	if (!ddic_test_cmds[ddic_debug.cmd_cnt].payload) {
		LCD_KIT_ERR("kzalloc fail\n");
		return LCD_KIT_FAIL;
	}
	for (i = 0; i < ddic_test_cmds[ddic_debug.cmd_cnt].dlen; i++) {
		ddic_test_cmds[ddic_debug.cmd_cnt].payload[i] = reg_value[2 + i];
		LCD_KIT_INFO("payload[%d]:0x%x ", i, ddic_test_cmds[ddic_debug.cmd_cnt].payload[i]);
	}
	LCD_KIT_INFO("\n ddic_test_cmds[%d]:dtype:0x%x, dlen:0x%x, payload[0]:0x%x \n",
		ddic_debug.cmd_cnt,
		ddic_test_cmds[ddic_debug.cmd_cnt].dtype,
		ddic_test_cmds[ddic_debug.cmd_cnt].dlen,
		ddic_test_cmds[ddic_debug.cmd_cnt].payload[0]);
	ddic_debug.cmd_cnt++;
	LCD_KIT_INFO("exit\n");
	return count;
}
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	mtk_drm_esd_switch_ext(DISABLE);
#endif
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (buf == NULL) {
		LCD_KIT_ERR("honorfd or buf is null\n");
		return LCD_KIT_FAIL;
	}
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
