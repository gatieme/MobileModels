...
extern struct lcd_kit_disp_info *lcd_kit_get_disp_info(void);
...
struct lcd_kit_disp_info {
	...
	struct regulator *lcd_vci_supply;
	char *iovcc_regulator_name;
	struct regulator *lcd_iovcc_supply;
	char *vdd_regulator_name;
	struct regulator *lcd_vdd_supply;
	/* product id */
	u32 product_id;
	/* vr support */
	u32 vr_support;
	...
	u32 hs_zero;
	u32 hs_prpr;
	u32 mtk_debug_wr_lock_support;
	u32 mtk_eof_timing_support;
	u32 dsi_dem_enable;
	u32 mipi_tx_vol;
	u32 rtcode;
	u32 rtcode_inc;
	u8 fps_test_type;
	struct mtk_lcd_test_config lcd_test_config;
};

extern int lcd_kit_power_init(void);
extern int lcd_kit_sysfs_init(void);
extern unsigned int lcm_get_panel_backlight_max_level(void);
#if IS_ENABLED(CONFIG_HONOR_DSM)
