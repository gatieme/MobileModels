...
extern struct mipi_dsi_driver lcm_driver;
int lcm_probe(struct mipi_dsi_device *dsi);
int mtk_leds_get_satus(void);
extern struct mtk_panel_info *lcm_get_panel_info(void);
extern void  lcd_kit_bl_ic_set_backlight(unsigned int bl_level);
int lcd_kit_poweric_detect(void);
int lcd_kit_init(void);
void lcdkit_cwb_get_buffer(void **buffer);
void lcdkit_cwb_copy_done(void *buffer, int type);
extern void lcd_kit_disp_on_check_delay(void);
