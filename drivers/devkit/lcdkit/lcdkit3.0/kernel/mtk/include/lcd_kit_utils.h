...
/* lcd fps value */
#define LCD_KIT_FPS_24 24
#define LCD_KIT_FPS_25 25
...
#define LCD_KIT_FPS_144 144
#define MAX_BUF        60
#define LCD_REG_LENGTH_MAX 600
...
struct mtk_panel_info {
	...
	unsigned int esd_recovery_bl_support;
        u32 esd_gpio_detect_support;
	u32 esd_gpio_detect_dual_support;
	u32 gpio_offset;
	u64 pxl_clk_rate;
...
#endif
	unsigned int hbm_vsync_time;
	unsigned int hbm_deadline;
	unsigned int interval_vspen_hwen_down_ms;
	atomic_t aod_update_light;
};
/* enum */
enum {
	RGBW_SET1_MODE = 1,
...
struct lcd_kit_fps {
	...
	struct lcd_kit_array_data hbm_timming[LCD_FPS_SCENCE_MAX];
