...
static int ktd3133_parse_dts(struct device_node *np)
{
	...
	for (i = 0; i < KTD3133_RW_REG_MAX; i++) {
		ret = of_property_read_u32(np, ktd3133_dts_string[i],
	...
	ret = of_property_read_u32(np, KTD3133_HW_ENABLE,
		&ktd3133_bl_info.ktd3133_hw_en);
	if (ret < 0) {
		LCD_KIT_ERR("get ktd3133_hw_enable dts config failed\n");
		return ret;
	}
	ret = of_property_read_u32(np, KTD3133_HW_EN_GPIO,
		&ktd3133_bl_info.ktd3133_hw_en_gpio);
	if (ret < 0) {
	...
	unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			if (ret < 0) {
				LCD_KIT_ERR("write ktd3133 bl cfg reg 0x%x failed\n",
					reg[i]);
				goto exit;
			}
		}
	...
	unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			LCD_KIT_ERR("read ktd3133 bl cfg reg 0x%x failed",
				reg[i]);
			goto exit;
		} else {
...
static int ktd3133_chip_init(struct ktd3133_chip_data *pchip)
{
	...
	ret = ktd3133_config_write(pchip, ktd3133_reg_addr,
		ktd3133_bl_info.ktd3133_reg, KTD3133_RW_REG_MAX);
	if (ret < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "Device control(0x02)= 0x%x\nEprom Configuration0(0x03) = 0x%x\n \
			...
			ktd3133_bl_info.ktd3133_reg[6], ktd3133_bl_info.ktd3133_reg[0],
			ktd3133_bl_info.ktd3133_reg[1], ktd3133_bl_info.ktd3133_reg[2],
			ktd3133_bl_info.ktd3133_reg[3], ktd3133_bl_info.ktd3133_reg[4],
			ktd3133_bl_info.ktd3133_reg[5]);
	return ret;
...
static DEVICE_ATTR(reg, 0644, ktd3133_reg_show, ktd3133_reg_store);
...
static void ktd3133_enable(void)
{
	...
	if (ktd3133_bl_info.ktd3133_hw_en) {
		...
		if (ret)
			LCD_KIT_ERR("ktd3133 Could not request  hw_en_gpio\n");
		ret = gpio_direction_output(ktd3133_bl_info.ktd3133_hw_en_gpio, GPIO_DIR_OUT);
		if (ret)
			LCD_KIT_ERR("ktd3133 set gpio output not success\n");
...
static int ktd3133_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	memset(&ktd3133_bl_info, 0, sizeof(ktd3133_bl_info));
