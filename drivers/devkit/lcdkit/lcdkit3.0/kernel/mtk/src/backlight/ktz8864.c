...
static int ktz8864_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "ktz8864_dimming_support",
		&bl_info.bl_dimming_support);
	if (ret < 0)
		ktz8864_err("get bl_dimming_support dts config failed\n");
	ret = of_property_read_u32(np, "ktz8864_dimming_config",
		&bl_info.bl_dimming_config);
	if (ret < 0)
		ktz8864_err("get bl_dimming_config dts config failed\n");
	ret = of_property_read_u32(np, "ktz8864_dimming_resume",
		&bl_info.bl_dimming_resume);
	if (ret < 0)
		ktz8864_err("get bl_dimming_resume dts config failed\n");
	ret = of_property_read_u32(np, "bl_max_10000_level",
		&bl_info.bl_max_10000_level);
	if (ret < 0) {
		bl_info.bl_max_10000_level = 0;
		ktz8864_err("get bl_max_10000_level dts config failed\n");
	}

	return LCD_KIT_OK;
}

static int ktz8864_2_config_write(struct ktz8864_chip_data *pchip,
			unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	/* get i2c adapter */
	adap = i2c_get_adapter(bl_info.ktz8864_2_i2c_bus_id);
	if (!adap) {
		ktz8864_err("i2c device %d not found\n", bl_info.ktz8864_2_i2c_bus_id);
		ret = -ENODEV;
		goto out;
	}
	...
	for(i = 0; i < size; i++) {
		buf[0] = reg[i];
		buf[1] = val[i];
		if (buf[0] == REG_DISPLAY_BIAS_CONFIG_1)
			buf[1] = bl_info.ktz8864_2_display_bias_cfg_1;
		if (val[i] != 0xffff) {
			ret = i2c_transfer(adap, &msg, 1);
			ktz8864_info("ktz8864_2_config_write reg=0x%x,val=0x%x\n", buf[0], buf[1]);
		}
	}
out:
	i2c_put_adapter(adap);
	ktz8864_err("ktz8864_2_config_write ret =%d\n", ret);
	return ret;
}
...
static int ktz8864_chip_init(struct ktz8864_chip_data *pchip)
{
	...
	ret = of_property_read_u32(np, "dual_ic", &bl_info.dual_ic);
	if (ret < 0) {
		ktz8864_info("can not get dual_ic dts node\n");
	}
	else {
		ret = of_property_read_u32(np, "ktz8864_2_i2c_bus_id",
			&bl_info.ktz8864_2_i2c_bus_id);
		if (ret < 0)
			ktz8864_info("can not get ktz8864_2_i2c_bus_id dts node\n");

		ret = of_property_read_u32(np, "ktz8864_2_display_bias_config_1",
			&bl_info.ktz8864_2_display_bias_cfg_1);
		if (ret < 0)
			ktz8864_info("can not get ktz8864_2_i2c_bus_id dts node\n");
...
{
	int ret;
	int bl_addr = REG_BL_BRIGHTNESS_LSB;
	int val = BL_DISABLE;
	...
	if (bl_info.dual_ic) {
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &val, 1);
		if (ret < 0)
			ktz8864_err("ktz8864_set_backlight_reg exit fail1\n");

		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &val, 1);
		if (ret < 0)
	    	ktz8864_err("ktz8864_set_backlight_reg exit fail2\n");
	}
...
static int ktz8864_set_bias(int vpos, int vneg)
{
	...
	if (ret < 0) {
        ...
        if (bl_info.dual_ic) {
	    ret = ktz8864_2_config_write(ktz8864_g_chip, ktz8864_reg_addr,
                ktz8864_reg, KTZ8864_RW_REG_MAX);
            if (ret < 0) {
                ktz8864_err("ktz8864 config register failed");
                return ret;
	...
	int last_level, int level)
{
	...
	if (val)
		LCD_KIT_ERR("check FAULT_FLAG error: val=0x%x\n", val);
...
static int ktz8864_chip_enable(struct ktz8864_chip_data *pchip)
{
	...
	if (bl_info.dual_ic) {
		ret = ktz8864_2_config_write(pchip, ktz8864_reg_addr, ktz8864_reg, KTZ8864_RW_REG_MAX);
		if (ret < 0) {
			ktz8864_err("ktz8864 slave config register failed\n");
		    return ret;
		}
	}
...
static void ktz8864_dync_dimming_config(unsigned int bl_level)
{
	int ret = -1;
	static int bl_work_state = BL_POWER_ON;

	if (bl_info.bl_dimming_support != BL_DIMMING_SUPPORT)
		return;
	if (bl_level == 0) {
		if (bl_info.bl_dimming_resume != 0) {
			ret = regmap_write(ktz8864_g_chip->regmap,
				REG_BL_CONFIG_2,
				bl_info.bl_dimming_resume);
			if (ret < 0)
				ktz8864_err("dync dimming config error\n");
		}
		bl_work_state = BL_POWER_OFF;
		return;
	}
	if (bl_work_state == BL_POWER_OFF) {
		bl_work_state = BL_POWER_ON;
	} else if (bl_work_state == BL_POWER_ON) {
		if (bl_info.bl_dimming_config != 0) {
			ret = regmap_write(ktz8864_g_chip->regmap,
				REG_BL_CONFIG_2,
				bl_info.bl_dimming_config);
			if (ret < 0)
				ktz8864_err("dync dimming config error\n");
			ktz8864_info("config bl dimming reg[%d]=0x%x\n",
				REG_BL_CONFIG_2, bl_info.bl_dimming_config);
		}
		bl_work_state = BL_CONFIG_LEVEL_POWER_ON;
	}
}
...
int ktz8864_set_backlight_reg(uint32_t bl_level)
{
	...
	ktz8864_info("ktz8864_set_backlight_reg bl_level= %d\n", bl_level);
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	panel_info = lcm_get_panel_info();
	if (panel_info == NULL) {
		ktz8864_err("panel_info is NULL\n");
	...
	if (bl_info.dual_ic && bl_info.bl_max_10000_level > BL_MAX) {
		bl_level = (((bl_level * bl_info.bl_max_10000_level) +
				(panel_info->bl_max / 2)) / panel_info->bl_max);
                ktz8864_info("after map bl_level= %d\n", bl_level);
	}
#endif
	if (ktz8864_10000_bl) {
	...
	ktz8864_dync_dimming_config(bl_level);
	...
	ret = regmap_update_bits(ktz8864_g_chip->regmap, REG_BL_BRIGHTNESS_LSB,
		MASK_BL_LSB, bl_lsb);
	if (ret < 0) {
		ktz8864_err("regmap_update_bits lsb fail\n");
		goto i2c_error;
	}
	...
	ret = regmap_write(ktz8864_g_chip->regmap, REG_BL_BRIGHTNESS_MSB,
		bl_msb);
	if (ret < 0) {
		ktz8864_err("regmap_write msb fail\n");
		goto i2c_error;
	}
	if (bl_info.dual_ic)
	{
	   int bl_addr = 0;
	   bl_addr = REG_BL_BRIGHTNESS_LSB;
	   ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr,
			&bl_lsb, 1);
	   if (ret < 0) {
			ktz8864_err("ktz8864_2_config_write lsb fail\n");
			goto i2c_error;
		}
		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr,
			&bl_msb, 1);
		if (ret < 0) {
			ktz8864_err("ktz8864_2_config_write msb fail\n");
			goto i2c_error;
		}
	}
	backlight_ic_check_fault(ktz8864_g_chip, level);
...
#if IS_ENABLED(CONFIG_LCD_KIT_DRIVER)
#include "lcd_kit_bl.h"
	...
	const struct i2c_device_id *id)
{
	...
	if (bl_info.bl_only != 0) {
        ...
        ktz8864_err("ktz8864 probe finish\n");
	ktz8864_init_status = true;
