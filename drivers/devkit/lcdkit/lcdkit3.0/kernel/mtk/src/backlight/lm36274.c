...
static int lm36274_2_config_write(struct lm36274_chip_data *pchip,
                      unsigned int reg[], unsigned int val[], unsigned int size)
{
        ...
        /* get i2c adapter */
        adap = i2c_get_adapter(bl_info.lm36274_2_i2c_bus_id);
        if (!adap) {
                lm36274_err("i2c device %d not found\n", bl_info.lm36274_2_i2c_bus_id);
                ret = -ENODEV;
                goto out;
        }
        ...
        for(i = 0; i < size; i++) {
                ...
                if (buf[0] == REG_DISPLAY_BIAS_CONFIG_1)
                        buf[1] = bl_info.lm36274_2_display_bias_cfg_1;
                if (val[i] != 0xffff) {
                        ret = i2c_transfer(adap, &msg, 1);
                        lm36274_info("lm36274_2_config_write reg=0x%x,val=0x%x\n", buf[0], buf[1]);
                }
        }
out:
        i2c_put_adapter(adap);
        lm36274_err("lm36274_2_config_write ret =%d\n", ret);
        return ret;
}
...
static int lm36274_parse_dts(struct device_node *np)
{
	...
	if (ret < 0) {
        ...
        if (ret < 0) {
                lm36274_info("can not get dual_ic dts node\n");
        }
        else {
                ret = of_property_read_u32(np, "lm36274_2_i2c_bus_id",
                        &bl_info.lm36274_2_i2c_bus_id);
                if (ret < 0)
                        lm36274_info("can not get lm36274_2_i2c_bus_id dts node\n");

                ret = of_property_read_u32(np, "lm36274_2_display_bias_config_1",
                        &bl_info.lm36274_2_display_bias_cfg_1);
                if (ret < 0)
                        lm36274_info("can not get lm36274_2_i2c_bus_id dts node\n");
	...
	ret = of_property_read_u32(np, "lm36274_dimming_support",
		&bl_info.bl_dimming_support);
	if (ret < 0)
		lm36274_err("get bl_dimming_support dts config failed\n");
	ret = of_property_read_u32(np, "lm36274_dimming_config",
		&bl_info.bl_dimming_config);
	if (ret < 0)
		lm36274_err("get bl_dimming_config dts config failed\n");
	ret = of_property_read_u32(np, "lm36274_dimming_resume",
		&bl_info.bl_dimming_resume);
	if (ret < 0)
		lm36274_err("get bl_dimming_resume dts config failed\n");
	ret = of_property_read_u32(np, "bl_max_10000_level",
		&bl_info.bl_max_10000_level);
	if (ret < 0) {
		bl_info.bl_max_10000_level = 0;
		lm36274_err("get bl_max_10000_level dts config failed\n");
	}
	return LCD_KIT_OK;
}
...
static int lm36274_chip_enable(struct lm36274_chip_data *pchip)
{
	...
	if (pchip == NULL) {
        ...
        if (bl_info.dual_ic) {
                ret = lm36274_2_config_write(pchip, lm36274_reg_addr, lm36274_reg, LM36274_RW_REG_MAX);
                if (ret < 0) {
                    lm36274_err("lm36274 slave config register failed\n");
                    return ret;
                }
        }
...
static void lm36274_dync_dimming_config(unsigned int bl_level)
{
	int ret = -1;
	...
	if (bl_level == 0) {
		if (bl_info.bl_dimming_resume != 0) {
			ret = regmap_write(lm36274_g_chip->regmap,
				REG_BL_CONFIG_2,
				bl_info.bl_dimming_resume);
			if (ret < 0)
				lm36274_err("dync dimming config error\n");
		}
	...
	if (bl_work_state == BL_POWER_OFF) {
		...
		if (bl_info.bl_dimming_config != 0) {
			...
			if (ret < 0)
				lm36274_err("dync dimming config error\n");
			lm36274_info("config bl dimming reg[%d]=0x%x\n",
...
int lm36274_set_backlight(unsigned int bl_level)
{
	...
	if (bl_info.dual_ic && bl_info.bl_max_10000_level > BL_MAX) {
		bl_level = (((bl_level * bl_info.bl_max_10000_level) +
                ...
                lm36274_info("map to bl_level = %u \n", bl_level);
	}
#endif
	if (lm36274_10000_bl) {
	...
	lm36274_dync_dimming_config(bl_level);
	...
	if (ret < 0) {
        ...
        if (bl_info.dual_ic)
        {
           ...
           ret = lm36274_2_config_write(lm36274_g_chip, &bl_addr,
                        &bl_lsb, 1);
           if (ret < 0)
           {
              lm36274_err("lm36274_set_backlight_reg exit fail3\n");
              goto i2c_error;
           }
           ...
           if (ret < 0)
           {
              lm36274_err("lm36274_set_backlight_reg exit fail4\n");
              goto i2c_error;
           }
        }
	backlight_ic_check_fault(lm36274_g_chip, level);
...
static int lm36274_set_bias_voltage(int vpos, int vneg)
{
	...
	if (ret < 0)
        ...
        if (bl_info.dual_ic) {
            ret = lm36274_2_config_write(lm36274_g_chip, lm36274_reg_addr,
                  lm36274_reg, LM36274_RW_REG_MAX);
            if (ret < 0) {
                  lm36274_err("lm36274 config register failed");
                  return ret;
        ...
        lm36274_info("lm36274_set_bias is successful\n");
	lm36274_init_status = true;
...
static int lm36274_set_ic_disable(void)
{
	...
	/* clean up bl val register */
        if (bl_info.dual_ic) {
                ret = lm36274_2_config_write(lm36274_g_chip, &bl_addr, &val, 1);
                if (ret < 0)
                    lm36274_err("lm36274_set_backlight_reg exit fail1\n");

                bl_addr = REG_BL_BRIGHTNESS_MSB;
                ret = lm36274_2_config_write(lm36274_g_chip, &bl_addr, &val, 1);
                if (ret < 0)
                    lm36274_err("lm36274_set_backlight_reg exit fail2\n");
        }
				...
				const struct i2c_device_id *id)
{
	...
	lm36274_info("lm36274 probe finish\n");
	lm36274_init_status = true;
