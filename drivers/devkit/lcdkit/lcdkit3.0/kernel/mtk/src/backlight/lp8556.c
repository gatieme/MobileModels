...
static int lp8556_parse_reg_config(struct lp8556_chip_data *pchip, struct device_node *np)
{
	int ret;
	int i;
	...
	if (!np || !pchip) {
		lp8556_err("detect null pointer\n");
		return -1;
	}

	ret = regmap_read(pchip->regmap, LP8556_EPROM_CFGF, &ic_sub_ver);
	if (ret < 0) {
		lp8556_err("get ic_sub_ver failed\n");
		return -1;
	}
	lp8556_info("get ic_sub_ver is 0x%x\n", ic_sub_ver);
	...
	case LP8556_E03_E07_CFGF_VAL:
		...
		break;
	default:
		lp8556_err("failed to get reg node name\n");
		return -1;
	}

	reg_node = of_find_compatible_node(np, NULL, reg_node_name);
	if (reg_node == NULL) {
		lp8556_err("reg_node is null pointer\n");
		return -1;
	}
	lp8556_info("[%s]:reg_node name is %s\n", __func__, reg_node->name);

	for (i = 0;i < LP8556_RW_REG_MAX;i++ ) {
		ret = of_property_read_u32(reg_node, lp8556_dts_string[i],
		...
		if (ret < 0) {
			...
			lp8556_err("%s:parse %s val is 0x%x\n", __func__, lp8556_dts_string[i], lp8556_bl_info.lp8556_reg[i]);
	}
...
static int lp8556_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "need_compatible",
		&lp8556_bl_info.is_need_compatible);
	if (ret < 0) {
		lp8556_err("get need_compatible dts config failed\n");
		return 0;
	}
			...
			unsigned int reg[],unsigned int val[],unsigned int size)
{
	...
	for(i = 0;i < size;i++) {
		...
		if (val[i] != 0xffff) {
			...
			lp8556_info("write lp8556 backlight config register 0x%x val 0x%x\n",reg[i], val[i]);
		}
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "Device control(0x01)= 0x%x\nEprom Configuration0(0xA0) = 0x%x\n \
			...
			lp8556_bl_info.lp8556_reg[8], lp8556_bl_info.lp8556_reg[9], lp8556_bl_info.lp8556_reg[10], lp8556_bl_info.lp8556_reg[11], lp8556_bl_info.lp8556_reg[12], lp8556_bl_info.lp8556_reg[13], lp8556_bl_info.lp8556_reg[14], lp8556_bl_info.lp8556_reg[15],
			lp8556_bl_info.lp8556_reg[16], lp8556_bl_info.lp8556_reg[17],lp8556_bl_info.lp8556_reg[18], lp8556_bl_info.lp8556_reg[19], lp8556_bl_info.lp8556_reg[20], lp8556_bl_info.lp8556_reg[21]);
	return ret;
...
static int lp8556_set_backlight(uint32_t bl_level)
{
	...
	if (false == lp8556_init_status) {
		lp8556_info("lp8556 is disabled, can not set backlight\n");
	...
	lp8556_info("lp8556 write backlight level:%d bl_msb:0x%x bl_lsb:0x%x\n", bl_level, bl_msb, bl_lsb);
				...
				const struct i2c_device_id *id)
{
	...
	if (lp8556_bl_info.is_need_compatible) {
		ret = lp8556_parse_reg_config(pchip, np);
		if (ret < 0) {
			lp8556_err("parse lp8556 dts reg config failed");
		}
	}
