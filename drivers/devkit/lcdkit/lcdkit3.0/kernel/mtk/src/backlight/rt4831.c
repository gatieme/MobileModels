...
static int rt4831_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "rt4831_dimming_support",
		&g_bl_info.bl_dimming_support);
	if (ret < 0)
		RT4831_ERR("get bl_dimming_support dts config failed\n");
	ret = of_property_read_u32(np, "rt4831_dimming_config",
		&g_bl_info.bl_dimming_config);
	if (ret < 0)
		RT4831_ERR("get bl_dimming_config dts config failed\n");
	ret = of_property_read_u32(np, "rt4831_dimming_resume",
		&g_bl_info.bl_dimming_resume);
	if (ret < 0)
		RT4831_ERR("get bl_dimming_resume dts config failed\n");
	ret = of_property_read_u32(np, "bl_max_10000_level",
		&g_bl_info.bl_max_10000_level);
	if (ret < 0) {
		g_bl_info.bl_max_10000_level = 0;
		RT4831_ERR("get bl_max_10000_level dts config failed\n");
	}

	return LCD_KIT_OK;
}

static int rt4831_2_config_write(struct rt4831_chip_data *pchip,
			unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	/* get i2c adapter */
	adap = i2c_get_adapter(g_bl_info.rt4831_2_i2c_bus_id);
	if (!adap) {
		RT4831_ERR("i2c device %d not found\n", g_bl_info.rt4831_2_i2c_bus_id);
		ret = -ENODEV;
		goto out;
	}
	...
	for(i = 0; i < size; i++) {
		...
		if (buf[0] == REG_DISPLAY_BIAS_CONFIG_1)
			buf[1] = g_bl_info.rt4831_2_display_bias_cfg_1;
		if (val[i] != 0xffff) {
			ret = i2c_transfer(adap, &msg, 1);
			RT4831_INFO("rt4831_2_config_write reg=0x%x,val=0x%x\n", buf[0], buf[1]);
		}
	}
out:
	i2c_put_adapter(adap);
	RT4831_ERR("rt4831_2_config_write ret =%d\n", ret);
	return ret;
}
...
static int rt4831_chip_init(struct rt4831_chip_data *pchip)
{
	...
	if (ret < 0) {
		RT4831_INFO("can not get dual_ic dts node\n");
	}
	else {
		ret = of_property_read_u32(np, "rt4831_2_i2c_bus_id",
			&g_bl_info.rt4831_2_i2c_bus_id);
		if (ret < 0)
			RT4831_INFO("can not get rt4831_2_i2c_bus_id dts node\n");

		ret = of_property_read_u32(np, "rt4831_2_display_bias_config_1",
			&g_bl_info.rt4831_2_display_bias_cfg_1);
		if (ret < 0)
			RT4831_INFO("can not get rt4831_2_i2c_bus_id dts node\n");
...
static int rt4831_set_ic_disable(void)
{
	...
	if (g_bl_info.dual_ic) {
		ret = rt4831_2_config_write(rt4831_g_chip, &bl_addr, &val, 1);
		if (ret < 0)
			RT4831_ERR("rt4831_set_backlight_reg exit fail1\n");

		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = rt4831_2_config_write(rt4831_g_chip, &bl_addr, &val, 1);
		if (ret < 0)
	    	RT4831_ERR("rt4831_set_backlight_reg exit fail2\n");
	}
...
static int rt4831_set_bias(int vpos, int vneg)
{
	...
	if (ret < 0) {
        ...
        if (g_bl_info.dual_ic) {
	    ret = rt4831_2_config_write(rt4831_g_chip, rt4831_reg_addr,
			rt4831_reg, RT4831_RW_REG_MAX);
	    if (ret < 0) {
		    RT4831_ERR("rt4831 config register failed");
	     	return ret;
...
static int rt4831_chip_enable(struct rt4831_chip_data *pchip)
{
	...
	if (g_bl_info.dual_ic) {
		ret = rt4831_2_config_write(pchip, rt4831_reg_addr, rt4831_reg, RT4831_RW_REG_MAX);
		if (ret < 0) {
			RT4831_ERR("rt4831 slave config register failed\n");
		    return ret;
		}
	}
...
static void rt4831_dync_dimming_config(unsigned int bl_level)
{
	int ret = -1;
	static int bl_work_state = BL_POWER_ON;

	if (g_bl_info.bl_dimming_support != BL_DIMMING_SUPPORT)
		return;
	if (bl_level == 0) {
		if (g_bl_info.bl_dimming_resume != 0) {
			ret = regmap_write(rt4831_g_chip->regmap,
				REG_BL_CONFIG_2,
				g_bl_info.bl_dimming_resume);
			if (ret < 0)
				RT4831_ERR("dync dimming config error\n");
		}
	...
	if (bl_work_state == BL_POWER_OFF) {
		...
		if (g_bl_info.bl_dimming_config != 0) {
			ret = regmap_write(rt4831_g_chip->regmap,
				REG_BL_CONFIG_2,
				g_bl_info.bl_dimming_config);
			if (ret < 0)
				RT4831_ERR("dync dimming config error\n");
			RT4831_INFO("config bl dimming reg[%d]=0x%x\n",
				REG_BL_CONFIG_2, g_bl_info.bl_dimming_config);
		}
...
int rt4831_set_backlight(uint32_t bl_level)
{
	...
	if (g_bl_info.dual_ic && g_bl_info.bl_max_10000_level > BL_MAX) {
		bl_level = (((bl_level * g_bl_info.bl_max_10000_level) +
				(panel_info->bl_max / 2)) / panel_info->bl_max);
                RT4831_INFO("map to bl_level = %u\n", bl_level);
	}
#endif
	if (rt4831_10000_bl) {
	...
	rt4831_dync_dimming_config(bl_level);
	...
	if (g_bl_info.dual_ic)
	{
	   ...
	   ret = rt4831_2_config_write(rt4831_g_chip, &bl_addr,
			&bl_lsb, 1);
	   if (ret < 0)
	   {
	     RT4831_ERR("rt4831_set_backlight_reg exit fail3\n");
	     goto i2c_error;
           }
	   ...
	   if (ret < 0)
	   {
	     RT4831_ERR("rt4831_set_backlight_reg exit fail4\n");
	     goto i2c_error;
           }
	}
	...
	const struct i2c_device_id *id)
{
	...
	if(g_bl_info.bl_only) {
        ...
        RT4831_ERR("rt4831 probe finish\n");
	rt4831_init_status = true;
