...
static int rt8555_set_backlight(uint32_t bl_level)
{
	...
	/* 10bit register can only support 1024 level brightness */
	if (bl_level > 0)
		bl_level = bl_level >> 2;
	...
	/*set backlight level*/
	bl_msb = (bl_level >> 8) & 0x03;
	bl_lsb = bl_level & 0xFF;

	ret = regmap_write(g_rtdev->regmap, g_bl_info.rt8555_level_lsb, bl_lsb);
	if (ret < 0)
		rt8555_err("write bl level lsb:0x%x failed\n", bl_lsb);

	ret = regmap_write(g_rtdev->regmap, g_bl_info.rt8555_level_msb, bl_msb);
	if (ret < 0)
		rt8555_err("write bl level msb:0x%x failed\n", bl_msb);
