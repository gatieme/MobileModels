...
#endif
#ifdef CONFIG_HONOR_DUBAI
#include <log/hwlog_kernel.h>
#endif
#include <log/log_jank.h>
#include "mtk_disp_notify.h"
#include <power_manager/usb/hw_pd_dev.h>
#include <fingerprint_interface/fingerprint_interface.h>

#if IS_ENABLED(CONFIG_HONOR_DSM)
...
static enum pcd_check_status g_pcd_check_status = PCD_CHECK_WAIT;
 ...
 /* lcd notifier usb start*/
static struct notifier_block lcd_usb_notifier = {
	.notifier_call = pd_dpm_handle_fb_event,
};

static void lcd_usb_notifier_register_init(void)
{
    LCD_KIT_INFO("%s \n", __func__);
    (void)mtk_disp_notifier_register("lcd_usb", &lcd_usb_notifier);
}
 /* lcd notifier usb end*/

struct lcd_kit_disp_info *lcd_kit_get_disp_info(void)
{
	return &g_lcd_kit_disp_info;
}
EXPORT_SYMBOL(lcd_kit_get_disp_info);
...
EXPORT_SYMBOL(lcm_get_panel_info);
...
int mtk_leds_get_satus(void)
{
	struct mtk_panel_info *panel_info = lcm_get_panel_info();

	if (!panel_info) {
		pr_notice("%s panel_info is NULL\n", __func__);
		return 0;
	}
	if (panel_info->bl_ic_ctrl_mode == MTK_MIPI_BL_IC_PWM_MODE ||
		panel_info->bl_ic_ctrl_mode == MTK_MIPI_MODE)
		return 1;
	return 0;
}
...
void  lcd_kit_bl_ic_set_backlight(unsigned int bl_level)
{
	...
	if (disp_info->close_support) {
		if ((lcd_kit_info.panel_state == LCD_POWER_STATE_OFF)&&(bl_level > 0)) {
			LCD_KIT_ERR("bl_level: %d, panel is disable!\n", bl_level);
			return;
		}
	} else {
...
EXPORT_SYMBOL(lcd_kit_bl_ic_set_backlight);
...
EXPORT_SYMBOL(lcm_get_panel_backlight_max_level);
	...
	struct drm_file *file_priv)
{
	...
	if (common_info && common_info->sn_code.support) {
		param->sn_code_length = lcd_kit_info.sn_code_length;
		memcpy(param->sn_code, lcd_kit_info.sn_code, lcd_kit_info.sn_code_length);
	}
...
static void lcd_kit_timerout_function(struct timer_list *t)
{
...
static void lcd_kit_enable_hbm_timer(void)
{
	...
	timer_setup(&g_backlight_timer, lcd_kit_timerout_function, 0);
	mod_timer(&g_backlight_timer, jiffies + 12 * HZ / 10);
...
int lcd_kit_poweric_detect()
{
	...
	LCD_KIT_INFO("+ \n");
        msleep(disp_info->elvdd_detect.delay);
...
static bool lcd_kit_first_screenon(uint32_t last_bl_level, uint32_t bl_level)
{
	...
	if (last_bl_level == 0 && bl_level != 0) {
		...
		LCD_KIT_INFO("first_screenoff, last_bl_level = %d", last_bl_level);
		ret = false;
	} else {
		ret = false;
	}
...
static int lcm_disable(struct drm_panel *panel)
{
	...
	if (disp_info->close_support && disp_info->current_level) {
		lcd_kit_bl_ic_set_backlight(0);
		LCD_KIT_INFO("panel off, need turn off backlight\n");
	}
	if (common_ops->panel_off_hs)
		common_ops->panel_off_hs(dsi);
...
static int lcm_early_prepare(struct drm_panel *panel)
{
	...
	if (plcd_kit_info == NULL ||
		plcd_kit_info->mipi.mipi_ulpm_after_vci == 0)
		return result;
	...
	struct drm_connector *connector)
{
	...
	if (disp_info->fps.support) {
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
			...
			drm_mode_probed_add(connector, support_mode);
		}
	} else {
	...
	connector->display_info.width_mm = pinfo->width;
	connector->display_info.height_mm = pinfo->height;
	...
	void *handle, unsigned int level)
{
	...
	if (common_info->backlight.need_prepare) {
		for (i = 0; i < common_info->backlight.prepare.cmd_cnt; i++) {
			cb(dsi, handle, common_info->backlight.prepare.cmds[i].payload,
					common_info->backlight.prepare.cmds[i].dlen);
		}
	}
	...
	if (ret < 0)
		pr_err("%s: fail\n", __func__);
	else if ((strcmp("als_s001_002", sensor_name.name) == 0) || (strcmp("tcs3701_l", sensor_name.name) == 0))
		save_light_to_sensorhub(level, level);
#endif
	return LCD_KIT_OK;
}
	...
	unsigned int mode)
{
	...
	list_for_each_entry(p_mode, &connector->modes, head) {
		if (i == mode)
			return p_mode;
		i++;
	}
	...
	struct drm_connector *connector, unsigned int mode)
{
	...
	struct drm_display_mode *p_mode = get_mode_by_id(connector, mode);
	...
	case LCD_KIT_FPS_24:
		ext->params = &ext_params_list[LCD_FPS_SCENCE_24];
		disp_info->fps.current_fps = LCD_KIT_FPS_24;
		break;
	case LCD_KIT_FPS_25:
		ext->params = &ext_params_list[LCD_FPS_SCENCE_25];
		disp_info->fps.current_fps = LCD_KIT_FPS_25;
		break;
	case LCD_KIT_FPS_30:
		ext->params = &ext_params_list[LCD_FPS_SCENCE_30];
		disp_info->fps.current_fps = LCD_KIT_FPS_30;
		break;
	case LCD_KIT_FPS_60:
	...
	case LCD_KIT_FPS_120:
		...
		break;
	case LCD_KIT_FPS_144:
		ext->params = &ext_params_list[LCD_FPS_SCENCE_144];
		disp_info->fps.current_fps = LCD_KIT_FPS_144;
		break;
	default:
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	struct drm_display_mode *cur_m = get_mode_by_id(connector, cur_mode);
	struct drm_display_mode *dst_m = get_mode_by_id(connector, dst_mode);

	if ((disp_info->alpm_state != ALPM_OUT) || (disp_info->lhbm_enter)) {
		LCD_KIT_INFO("in AOD mode not support fps change\n");
		return LCD_KIT_OK;
	}
	...
	} else if (dst_m->vrefresh == LCD_KIT_FPS_144) {
		lcd_fps_scence = LCD_FPS_SCENCE_144;
	} else if (dst_m->vrefresh == LCD_KIT_FPS_30) {
		lcd_fps_scence = LCD_FPS_SCENCE_30;
	} else if (dst_m->vrefresh == LCD_KIT_FPS_25) {
		lcd_fps_scence = LCD_FPS_SCENCE_25;
	} else if (dst_m->vrefresh == LCD_KIT_FPS_24) {
		lcd_fps_scence = LCD_FPS_SCENCE_24;
	} else {
		LCD_KIT_ERR("lcd_fps_scence is error\n");
		return LCD_KIT_FAIL;
	}
	if (cur_m->vrefresh != dst_m->vrefresh) {
...
#define LCD_SUSPEND 1
#define LHBM_FPS_NOT_120 2
#define HBM_EXIT 0
#define HBM_ENTER 1
#define HBM_FORCE_ENTER 2
static int lcd_kit_local_hbm_identify_off_set(int level)
{
	int ret = LCD_KIT_FAIL;
	if (lcd_kit_info.panel_state == LCD_POWER_STATE_OFF) {
		LCD_KIT_INFO("panel state: %d",lcd_kit_info.panel_state);
		return LCD_SUSPEND;
	}
	if (!common_info->lhbm.support) {
		LCD_KIT_INFO("not support lhbm\n");
		return ret;
	}
	LCD_KIT_INFO("level = %d\n", level);
	if (level == HBM_EXIT) {
		ret = lcd_kit_dsi_cmds_tx_compact(&common_info->lhbm.disable_cmds, true);
		if (ret)
			ret = LCD_SUSPEND;
		else {
			disp_info->lhbm_enter = false;
			switch (disp_info->lhbm_fps) {
			case LCD_KIT_FPS_24:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_24]);
				break;
			case LCD_KIT_FPS_25:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_25]);
				break;
			case LCD_KIT_FPS_30:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_30]);
				break;
			case LCD_KIT_FPS_60:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_H60]);
				break;
			case LCD_KIT_FPS_90:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_90]);
				break;
			case LCD_KIT_FPS_120:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_120]);
				break;
			case LCD_KIT_FPS_144:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(&disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_144]);
				break;
			default:
	...
	LCD_KIT_INFO("%s: fps = %d\n", __func__, disp_info->fps.current_fps);
	if ((level == HBM_ENTER) && (disp_info->fps.current_fps != LCD_KIT_FPS_120)
		&& (disp_info->alpm_state == ALPM_OUT)) {
		LCD_KIT_INFO("%s: fps = %d\n", __func__, disp_info->fps.current_fps);
		return LHBM_FPS_NOT_120;
	}

	ret = lcd_kit_dsi_cmds_tx_compact(&common_info->lhbm.off_enable_cmds, true);
	disp_info->lhbm_enter = true;
	ud_fp_on_hbm_completed();
	return ret;
}

static int lcd_kit_local_hbm_identify_on_set(int level)
{
	int ret = LCD_KIT_FAIL;
	if (!common_info->lhbm.support) {
		LCD_KIT_INFO("not support lhbm\n");
	...
	ret = lcd_kit_dsi_cmds_tx_compact(&common_info->lhbm.on_enable_cmds, true);
	disp_info->lhbm_enter = true;
	ud_fp_on_hbm_completed();
	return ret;
}

static int lcd_kit_local_hbm_mmi_set(int level)
{
	int ret = LCD_KIT_FAIL;
	if (!common_info->lhbm.support) {
		LCD_KIT_INFO("not support lhbm\n");
...
static u8 *g_lcdkit_cwb_buffer;
static int g_lcdkit_cwb_width;
static int g_lcdkit_cwb_height;
void lcdkit_cwb_get_buffer(void **buffer)
{
	if (buffer == NULL) {
		LCD_KIT_ERR("%s: buffer is invalid\n", __func__);
		return;
	}
	if (g_lcdkit_cwb_buffer == 0) {
		g_lcdkit_cwb_buffer = kzalloc(sizeof(u8) * SHARE_MEMORY_SIZE,
			GFP_KERNEL);
		LCD_KIT_INFO("lcdkit_cwb alloc buffer\n");
		if (g_lcdkit_cwb_buffer == 0) {
			LCD_KIT_ERR("lcdkit_cwb alloc buffer fail\n");
			return;
		}
	}
	share_mem_buffer_index = 1 - share_mem_buffer_index;
	*buffer = (void *)(g_lcdkit_cwb_buffer +
		share_mem_buffer_index * SHARE_MEMORY_SIZE / 2);
}

extern void mtk_drm_cwb_update_rgb(u8 *va, int width, int height,
	unsigned int frameIndex, unsigned int *bufferPtr);

void lcdkit_cwb_copy_done(void *buffer, int type)
{
	u8 *data = buffer;
	unsigned int *share_buf;
	if (share_mem_frame_index == 0xffffffff)
		share_mem_frame_index = 0;
	else
		share_mem_frame_index++;

	share_buf = (unsigned int *)(share_mem_virt +
		share_mem_buffer_index * SHARE_MEMORY_SIZE / 2);
	mtk_drm_cwb_update_rgb(data, g_lcdkit_cwb_width, g_lcdkit_cwb_height,
		share_mem_frame_index, share_buf);
	...
	struct drm_file *file_priv)
{
	...
	if ((disp_info->alpm_state != ALPM_OUT) && ((hbm_cfg->type == HBM_FOR_MMI)
		|| (hbm_cfg->type == HBM_FOR_LIGHT))) {
	...
	if (hbm_cfg->type == LOCAL_HBM_FOR_MMI)
		ret = lcd_kit_local_hbm_mmi_set(hbm_cfg->level);

	if (hbm_cfg->type == LOCAL_HBM_FOR_ON)
		ret = lcd_kit_local_hbm_identify_on_set(hbm_cfg->level);

	if (hbm_cfg->type == LOCAL_HBM_FOR_OFF)
		ret = lcd_kit_local_hbm_identify_off_set(hbm_cfg->level);
...
static void lcm_phy_time_ext_config(struct mtk_panel_params *panel_params)
{
	if (panel_params == NULL) {
		LCD_KIT_INFO("input param null\n");
		return;
	}

	LCD_KIT_INFO("set dsi time config hs_trail and hs_zero\n");
	panel_params->phy_timcon.hs_trail = disp_info->hs_trail;
	panel_params->phy_timcon.hs_zero = disp_info->hs_zero;
	panel_params->phy_timcon.hs_prpr = disp_info->hs_prpr;
}

static void lcd_dyn_fps_cmds_config(struct mtk_panel_params *panel_params,
	struct lcd_kit_dsi_panel_cmds *cmds)
{
	int i;
	struct lcd_kit_dsi_cmd_desc *fps_cmd = NULL;
	struct dynamic_fps_params *pdyn_fps = NULL;

	if (panel_params == NULL || cmds == NULL) {
		LCD_KIT_ERR("input param null\n");
		return;
	}

	if (cmds->cmd_cnt == 0) {
		LCD_KIT_ERR("no need config switch cmds\n");
		return;
	}
	if (MAX_DYN_CMD_NUM <= cmds->cmd_cnt) {
		LCD_KIT_ERR("config switch cmds is too long\n");
		return;
	}

	pdyn_fps = &panel_params->dyn_fps;
	for (i = 0; i < cmds->cmd_cnt; i++) {
		fps_cmd = &cmds->cmds[i];
		if (fps_cmd->dlen == 0 ||
			fps_cmd->payload == NULL ||
			fps_cmd->dlen > MAX_CMD_PARA)
			continue;
		pdyn_fps->switch_cmd_flag = TRUE;
		pdyn_fps->dfps_cmd_table[i].cmd_num = fps_cmd->dlen;
		memcpy(pdyn_fps->dfps_cmd_table[i].para_list,
			&fps_cmd->payload[0], fps_cmd->dlen);
		LCD_KIT_INFO("dfps_cmd_table[%d] cmd num id %d\n", i, fps_cmd->dlen);
	}
}
	...
	if (disp_info->fps.support) {
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
			...
			if (timing == NULL || timing->buf == NULL) {
				...
				if (pinfo->data_rate_khz)
					ext_params_list[index].data_rate_khz = timing->buf[FPS_RATE_INDEX];
				else
				...
				ext_params_list[index].output_mode = pinfo->output_mode;
				/* lcm_cmd_if is used to distinguish for Panel Cmd single or dual Port */
				ext_params_list[index].lcm_cmd_if = pinfo->lcm_cmd_if;
				/* 0: dphy 1: cphy */
				...
				if (disp_info->fps.hop_support) {
					...
					if (pinfo->data_rate_khz)
						ext_params_list[index].dyn.data_rate_khz = hop_info->buf[6];
					else
				...
				lcm_phy_time_ext_config(&ext_params_list[index]);
				lcd_dyn_fps_cmds_config(&ext_params_list[index],
					&disp_info->fps.fps_to_cmds[index]);
			}
		}
	} else {
		ext_params.data_rate = pinfo->data_rate;
		ext_params.data_rate_khz = pinfo->data_rate_khz;
...
#endif
		lcm_phy_time_ext_config(&ext_params);
	}
...
int lcd_kit_get_bl_max_nit_from_ddic(void)
{
	int ret;
	unsigned char val[20] = {0};

	LCD_KIT_INFO("+\n");

	LCD_KIT_INFO("get_blmaxnit_type:%d\n", common_info->blmaxnit.get_blmaxnit_type);
	if (common_info->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC) {
		ret = lcd_kit_dsi_cmds_extern_rx(val, &common_info->blmaxnit.bl_maxnit_cmds, 1);
		if (ret) {
			LCD_KIT_ERR("bl maxnit get fail! ret:%d\n", ret);
			return LCD_KIT_FAIL;
		}
		LCD_KIT_INFO("max nit from ddic:0x%x, bl_max_nit_min_value : 0x%x\n", val[0], common_info->bl_max_nit_min_value);

		if ((val[0]> BL_MIN) && (val[0]< BL_MAX)) {
			common_info->actual_bl_max_nit =
				(val[0] < BL_REG_NOUSE_VALUE) ? (val[0] + common_info->bl_max_nit_min_value) : (val[0] + common_info->bl_max_nit_min_value - 1);
		} else {
			common_info->actual_bl_max_nit = 0;
		}
		LCD_KIT_INFO("actual max nit is:0x%x\n", common_info->actual_bl_max_nit);
	}
...
int lcd_kit_init(void)
{
	...
	if (common_info->blmaxnit.get_blmaxnit_in_kernel == 0) {
...
static void lcd_kit_keep_ldo_on(struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node, "lcd_vci-supply")) {
		...
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			disp_info->lcd_vci_supply = ldo_reg;
			ret = regulator_enable(ldo_reg);
			if (ret < 0)
				LCD_KIT_ERR("regulator_enable lcd_vci-supply failed\n");
		}
	}
	if (of_property_read_bool(dev->of_node, "lcd_iovcc-supply")) {
		ldo_reg = devm_regulator_get(dev, "lcd_iovcc");
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			disp_info->lcd_iovcc_supply = ldo_reg;
			ret = regulator_enable(ldo_reg);
			if (ret < 0)
				LCD_KIT_ERR("regulator_enable lcd_iovcc-supply failed\n");
		}
	}
	if (of_property_read_bool(dev->of_node, "lcd_vdd-supply")) {
		ldo_reg = devm_regulator_get(dev, "lcd_vdd");
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			disp_info->lcd_vdd_supply = ldo_reg;
			ret = regulator_enable(ldo_reg);
			if (ret < 0)
				LCD_KIT_ERR("regulator_enable lcd_vdd-supply failed\n");
		}
	}
}
...
int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	lcd_usb_notifier_register_init();
	LCD_KIT_INFO("exit\n");
	return ret;
}
...
struct mipi_dsi_driver lcm_driver = {
	.probe = lcm_probe,
	.remove = lcm_remove,
	.driver = {
