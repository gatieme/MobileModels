...
void lcd_kit_gpio_tx(uint32_t type, uint32_t op)
{
	...
	case LCD_KIT_EXT_LDO:
		g_lcd_kit_gpio = power_hdl->lcd_ext_ldo.buf[1];
		break;
	default:
...
static int lcd_kit_regulator_supply(struct regulator *ldo_reg, uint32_t enable)
{
	int ret;

	if (IS_ERR_OR_NULL(ldo_reg)) {
		LCD_KIT_ERR("ldo_reg is failed\n");
		return LCD_KIT_FAIL;
	}
	if (enable)
		ret = regulator_enable(ldo_reg);
		...
		ret = regulator_disable(ldo_reg);
	if (ret < 0)
		LCD_KIT_ERR("regulator supply failed, ret = %d\n", ret);
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	switch (type) {
	case LCD_KIT_VCI:
		if (disp_info->lcd_vci_supply)
			ret = lcd_kit_regulator_supply(
				disp_info->lcd_vci_supply, enable);
		else if (disp_info->vci_regulator_name)
			ret = lcd_kit_regulator_ctrl(
				disp_info->vci_regulator_name, enable);
		break;
	case LCD_KIT_IOVCC:
		if (disp_info->lcd_iovcc_supply)
			ret = lcd_kit_regulator_supply(
				disp_info->lcd_iovcc_supply, enable);
		else if (disp_info->iovcc_regulator_name)
			ret = lcd_kit_regulator_ctrl(
				disp_info->iovcc_regulator_name, enable);
		break;
	case LCD_KIT_VDD:
		if (disp_info->lcd_vdd_supply)
			ret = lcd_kit_regulator_supply(
				disp_info->lcd_vdd_supply, enable);
		else if (disp_info->vdd_regulator_name)
			ret = lcd_kit_regulator_ctrl(
