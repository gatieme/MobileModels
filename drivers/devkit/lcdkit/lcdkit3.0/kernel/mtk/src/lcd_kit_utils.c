...
int lcd_kit_alpm_light_update(int value)
{
	struct mtk_panel_info *panel_info = NULL;

	panel_info = lcm_get_panel_info();
	if (panel_info) {
		atomic_set(&panel_info->aod_update_light, value);
	}
	return LCD_KIT_OK;
}
#endif
...
int lcd_kit_alpm_setting(uint32_t mode)
{
	...
	case ALPM_DISPLAY_OFF:
		...
		if (ret == LCD_KIT_OK)
			disp_info->alpm_light_state = ALPM_DISPLAY_OFF;
		break;
	case ALPM_ON_HIGH_LIGHT:
		...
		if (disp_info->alpm_state == ALPM_IN) {
			if (disp_info->alpm_light_state == ALPM_ON_HIGH_LIGHT) {
				LCD_KIT_INFO("The same aod light config!\n");
				ret = LCD_KIT_OK;
				break;
			}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
			if (!atomic_read(&panel_info->aod_update_light)) {
				LCD_KIT_INFO("cannot update light for not atomic update!\n");
				break;
			}
#endif
		}
		...
		if (disp_info->alpm_state == ALPM_START && ret == LCD_KIT_OK) {
			disp_info->alpm_state = ALPM_IN;
			disp_info->alpm_light_state = ALPM_ON_HIGH_LIGHT;
		}
		break;
	case ALPM_EXIT:
		ret = lcd_kit_dsi_cmds_extern_tx(&disp_info->alpm.exit_cmds);
		disp_info->alpm_state = ALPM_OUT;
		disp_info->alpm_light_state = ALPM_EXIT;
		break;
	case ALPM_ON_LOW_LIGHT:
		...
		if (disp_info->alpm_state == ALPM_IN) {
			if (disp_info->alpm_light_state == ALPM_ON_LOW_LIGHT) {
				LCD_KIT_INFO("The same aod light config!\n");
				ret = LCD_KIT_OK;
				break;
			}
		...
		if (disp_info->alpm_state == ALPM_START && ret == LCD_KIT_OK) {
			disp_info->alpm_state = ALPM_IN;
			disp_info->alpm_light_state = ALPM_ON_LOW_LIGHT;
		}
		break;
	case ALPM_SINGLE_CLOCK:
		...
		ret = LCD_KIT_OK;
		disp_info->alpm_light_state = ALPM_SINGLE_CLOCK;
		break;
	case ALPM_DOUBLE_CLOCK:
		...
		disp_info->alpm_light_state = ALPM_DOUBLE_CLOCK;
		break;
	case ALPM_ON_MIDDLE_LIGHT:
		...
		if (disp_info->alpm_state == ALPM_IN) {
			if (disp_info->alpm_light_state == ALPM_ON_MIDDLE_LIGHT) {
				LCD_KIT_INFO("The same aod light config!\n");
				ret = LCD_KIT_OK;
				break;
			}
		...
		if (disp_info->alpm_state == ALPM_START && ret == LCD_KIT_OK) {
			disp_info->alpm_state = ALPM_IN;
			disp_info->alpm_light_state = ALPM_ON_MIDDLE_LIGHT;
		}
		break;
...
EXPORT_SYMBOL(lcd_kit_disp_on_check_delay);
...
#if KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE
	lcd_get_timestamp(&disp_info->quickly_sleep_out.panel_on_record_tv);
#else
...
void lcd_kit_pinfo_init(struct device_node *np, struct mtk_panel_info *pinfo)
{
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-data-rate",
		&pinfo->data_rate, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-data-rate-khz",
		&pinfo->data_rate_khz, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-frame-lp-enable",
		&pinfo->frame_lp_enable, 0);
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,esd-gpio-detect-dual-support",
		&pinfo->esd_gpio_detect_dual_support, 0);
        OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,esd-gpio-detect-support",
                &pinfo->esd_gpio_detect_support, 0);
	/* effect info */
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-dsi-hs-trail",
			&disp_info->hs_trail, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-dsi-hs-zero",
			&disp_info->hs_zero, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-dsi-hs-prpr",
			&disp_info->hs_prpr, 0);
}
...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	/* lcd_fps_order */
	lcd_kit_parse_u32(np, "lcd-kit,fps-order-mode", &disp_info->fps.order_mode, 0);
	if (disp_info->fps.order_mode == CUST_MODE) {
		lcd_kit_parse_string_arrays(np, "lcd-kit,dsi-fps-mmi-test-list",
			&disp_info->lcd_test_config.mmi_test_list);
		lcd_kit_parse_string_arrays(np, "lcd-kit,dsi-fps-rt-test-list",
			&disp_info->lcd_test_config.rt_test_list);
		lcd_kit_parse_string_arrays(np, "lcd-kit,dsi-fps-tplcd-test-list",
			&disp_info->lcd_test_config.tplcd_test_list);
	}
	/* hopping */
		...
		if (index == LCD_FPS_SCENCE_60) {
			...
			if (disp_info->fps.hop_support)
				lcd_kit_parse_array_data(np, "lcd-kit,fps-120-mipi-hopping",
					&disp_info->fps.hop_info[index]);
		} else if (index == LCD_FPS_SCENCE_144) {
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-144-cmds",
				"lcd-kit,fps-to-144-cmds-state", &disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-144-dsi-timming", &disp_info->fps.fps_dsi_timming[index]);
		} else if (index == LCD_FPS_SCENCE_30) {
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-30-cmds",
				"lcd-kit,fps-to-30-cmds-state", &disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-30-dsi-timming", &disp_info->fps.fps_dsi_timming[index]);
		} else if (index == LCD_FPS_SCENCE_25) {
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-25-cmds",
				"lcd-kit,fps-to-25-cmds-state", &disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-25-dsi-timming", &disp_info->fps.fps_dsi_timming[index]);
		}else if (index == LCD_FPS_SCENCE_24) {
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-24-cmds",
				"lcd-kit,fps-to-24-cmds-state", &disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-24-dsi-timming", &disp_info->fps.fps_dsi_timming[index]);
		}
	}
}
...
void lcd_kit_parse_util(struct device_node *np)
{
	...
	/* dem */
        lcd_kit_parse_u32(np, "lcd-kit,dsi-dem-enable",
		&disp_info->dsi_dem_enable, 0);

        lcd_kit_parse_u32(np, "lcd-kit,closebacklight-support",
		&disp_info->close_support, 0);

	/* mipi tx vol */
        lcd_kit_parse_u32(np, "lcd-kit,mipi-tx-vol",
		&disp_info->mipi_tx_vol, 0);
...
static int lcd_get_2d_barcode(char *buff)
{
	...
	if (ret) {
		...
		strncat(read_value, buff, strlen(buff));
		if (read_value[6] < '0') {
			for (i = 0; i < 6; i++) {
				if (buff[6 + i] == 0) {
					read_value[6 + 2 * i] = '0';
					read_value[6 + 2 * i + 1] = '0';
				} else {
					snprintf(&read_value[6 + 2 * i], 3,
							"%02X\n", buff[6+i]);
				}
			}
			for(j = 0;j < 20;j++)
				LCD_KIT_INFO("read[%d] = %02x\n", j, read_value[j]);
				strncpy(buff, read_value, 20);
			for(k = 0;k < 20;k++)
				LCD_KIT_INFO("buff[%d] = %02x\n", k, buff[k]);
		}
...
static int lcd_kit_get_panel_ver(void)
{
	int ret;
	struct mtk_panel_info *pinfo = NULL;
	...
	if (common_info && common_info->panel_ext_info.support) {
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
		ret = lcd_kit_dsi_cmds_extern_rx(&read_value,
			&common_info->panel_ext_info.get_ver_cmds, 1);
#else
		ret = lcd_kit_dsi_cmds_extern_rx_v1(&read_value,
			&common_info->panel_ext_info.get_ver_cmds, 1);
#endif
		if (ret != 0) {
			LCD_KIT_ERR("get batch_info error!\n");
			return LCD_KIT_FAIL;
		}
		if (read_value >= common_info->panel_ext_info.ver_val) {
			common_info->is_ext_panel = PANEL_VER1;
		} else {
			common_info->is_ext_panel = PANEL_DEAFULT;
		}
	}
	return LCD_KIT_OK;
}
...
struct lcd_kit_ops g_lcd_ops = {
	...
	.get_panel_ver = lcd_kit_get_panel_ver,
};
