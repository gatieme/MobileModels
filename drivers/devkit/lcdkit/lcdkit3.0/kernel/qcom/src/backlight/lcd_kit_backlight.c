	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	for (i = 0; i < pbl_config->num_of_init_cmds; i++) {
		...
		LCD_KIT_ERR("init i is %d, tpye is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
	...
	int last_level, int level)
{
	...
	if (ret < 0) {
		LCD_KIT_ERR("%s: read backlight ic ovp reg fail\n");
		return;
	}
...
static int __init lcd_kit_backlight_init(void)
{
	...
	snprintf(compatible_name, sizeof(compatible_name), "honor,%s", backlight_ic_name);
