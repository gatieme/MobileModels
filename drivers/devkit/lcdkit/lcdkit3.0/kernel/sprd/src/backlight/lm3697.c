...
static int lm3697_parse_dts(struct device_node *np)
{
	...
	for (i = 0; i < LM3697_RW_REG_MAX; i++) {
		ret = of_property_read_u32(np, lm3697_dts_string[i],
	...
	ret = of_property_read_u32(np, LM3697_HW_ENABLE,
		&lm3697_bl_info.lm3697_hw_en);
	if (ret < 0) {
		LCD_KIT_ERR("get lm3697_hw_enable dts config failed\n");
		return ret;
	}
	ret = of_property_read_u32(np, LM3697_HW_EN_GPIO,
		&lm3697_bl_info.lm3697_hw_en_gpio);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, LM3697_BACKLIGHT_CONFIG_LSB_REG_ADDR,
		&lm3697_bl_info.lm3697_level_lsb_reg);
	if (ret < 0) {
		LCD_KIT_ERR("get lm3697_bl_cfg_lsb_reg_addr dts cfg failed\n");
		lm3697_bl_info.lm3697_level_lsb_reg = LM3697_REG_BRIGHTNESS_LSB_B;
	}
	ret = of_property_read_u32(np, LM3697_BACKLIGHT_CONFIG_MSB_REG_ADDR,
		&lm3697_bl_info.lm3697_level_msb_reg);
	if (ret < 0) {
		LCD_KIT_ERR("get lm3697_bl_cfg_msb_reg_addr dts cfg failed\n");
	...
	unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			if (ret < 0) {
				LCD_KIT_ERR("write lm3697 bl cfg reg 0x%x failed\n",
					reg[i]);
				return ret;
			}
		}
	...
	unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			LCD_KIT_ERR("read lm3697 bl cfg reg 0x%x failed",
				reg[i]);
			return ret;
		}
...
static int lm3697_chip_init(struct lm3697_chip_data *pchip)
{
	...
	ret = lm3697_config_write(pchip, lm3697_reg_addr,
		lm3697_bl_info.lm3697_reg, LM3697_RW_REG_MAX);
	if (ret < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "Device Sink Config(0x10)= 0x%x\nEprom Configuration0(0x11) = 0x%x\n \
			...
			lm3697_bl_info.lm3697_reg[0], lm3697_bl_info.lm3697_reg[1],
			lm3697_bl_info.lm3697_reg[2], lm3697_bl_info.lm3697_reg[3],
			lm3697_bl_info.lm3697_reg[4], lm3697_bl_info.lm3697_reg[5],
			lm3697_bl_info.lm3697_reg[6], lm3697_bl_info.lm3697_reg[7],
			lm3697_bl_info.lm3697_reg[8], lm3697_bl_info.lm3697_reg[9],
			lm3697_bl_info.lm3697_reg[10], lm3697_bl_info.lm3697_reg[11],
			lm3697_bl_info.lm3697_reg[12], lm3697_bl_info.lm3697_reg[13],
			lm3697_bl_info.lm3697_reg[14], lm3697_bl_info.lm3697_reg[15],
			lm3697_bl_info.lm3697_reg[16], lm3697_bl_info.lm3697_reg[17],
			lm3697_bl_info.lm3697_reg[18], lm3697_bl_info.lm3697_reg[19]);
	return ret;
	...
	struct device_attribute *dev_attr, const char *buf, size_t size)
{
	...
	LCD_KIT_INFO("reg=0x%x,mask=0x%x,val=0x%x\n", reg, mask, val);
...
i2c_error:
	...
	ret = snprintf((char *)buf, PAGE_SIZE,
...
static DEVICE_ATTR(reg, 0644, lm3697_reg_show, lm3697_reg_store);
...
static void lm3697_enable(void)
{
	...
	if (lm3697_bl_info.lm3697_hw_en) {
		...
		if (ret)
			LCD_KIT_ERR("lm3697 Could not request hw_en_gpio\n");
		ret = gpio_direction_output(lm3697_bl_info.lm3697_hw_en_gpio,
			GPIO_DIR_OUT);
		if (ret)
			LCD_KIT_ERR("lm3697 set gpio output not success\n");
...
static int lm3697_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(*pchip), GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
