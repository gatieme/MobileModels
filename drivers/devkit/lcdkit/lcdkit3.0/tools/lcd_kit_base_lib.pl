...
sub create_fwdtb_dtsi_tail    {    return "        };\n    };\n};";    }
...
sub create_dtsi_header
{
    ...
    if ($target_file_type eq "trebledto")
    {
		if ($chip_plat eq "qcom")
		{
			$dtsi_header .= "&mdss_mdp {\n";
		}
		elsif ($chip_plat eq "sprd")
		{
			$dtsi_header .= "&honor_lcd_config {\n";
		}
		else
		{
			$dtsi_header .= "&honor_lcd_config {\n";
    ...
    $dtsi_header .= "\tdsi_" . lc($file_name) . ": lcd_kit_" . lc($file_name) . " {\n";

    return $dtsi_header;
}

sub parser_hex_arrays_base
{
    my $para = shift;
    my $parser_string = $para;
    my $platform = shift;

    if ($platform eq 'qcom')
    {
        $parser_string =~ s/^\s+|\s+$//g;
        $parser_string =~ s/\s+/ /g;
        $parser_string = "<" . substr($parser_string, 1, -1) . ">";
        $parser_string =~ s/",/>,/g;
        $parser_string =~ s/"/</g;
    }
...
sub honor_cmd_struct_grp
{
    ...
    my $tmp_string = parse_multi_xml($struct_attr[0]);
    if ($tmp_string eq $parse_error_string)    {
        debug_print("honor_cmd_struct_grp fail to parse $struct_attr[0] !\n\n");
    ...
    foreach my $line (@lines)
    {
        ...
        if(scalar @sep > 6)
        {
            ...
            elsif($cmdtype eq "0x0A")
            {
                $dtype = "DTYPE_DSC_LWRITE, ";
            }
            else
            {
                $dtype = "DTYPE_GEN_LWRITE, ";
...
sub create_effect_lut_
{
    my $property = shift;
    my $parse_string = parse_plat_xml($property);
    if ($parse_string eq $parse_error_string)    {
        error_print("create_effect_lut fail to parse $property!\n\n");
        return $parse_error_string;
    }
...
sub create_effect_lut_grp
{
	...
	$toPrint .= "[$arr_size][$arr_size][$arr_size] = {\n";
	foreach my $line (@lines)
	{
		my @sep = split /,/, $line;
...
sub create_file_header
{
    my $filehead;
    $filehead  = "/*---------------------------------------------------------------------------\n";
    $filehead .= " * This file is autogenerated file using honor LCD parser. Please do not edit it.\n";
    $filehead .= " * Update input XML file to add a new entry or update variable in this file\n";
    $filehead .= " * Parser location: vendor/honor/chipset/base/kernel/devkit/lcdkit/tools \n";
    $filehead .= " *---------------------------------------------------------------------------*/\n\n";
    return $filehead;
}
...
sub create_lcd_map_struct
{
    ...
    $lcd_struct .= "/*---------------------------------------------------------------------------*/\n";
    $lcd_struct .= "struct lcd_kit_board_map {\n";
    $lcd_struct .= "    char* compatible;\n";
    $lcd_struct .= "    char* lcd_name;\n";
    $lcd_struct .= "    uint8_t gpio_id;\n";
    $lcd_struct .= "    uint16_t board_id;\n";
    $lcd_struct .= "};\n\n";

    $lcd_struct .= "static struct lcd_kit_board_map lcd_kit_map[] = {\n";
...
sub create_default_panel_map_struct
{
    my $lcd_struct;
    my $name_list = shift;
    $lcd_struct  = "\n/*---------------------------------------------------------------------------*/\n";
    $lcd_struct .= "/* static default panel mapping variable                                           */\n";
    $lcd_struct .= "/*---------------------------------------------------------------------------*/\n";
    $lcd_struct .= "struct lcd_kit_default_panel_map {\n";
    $lcd_struct .= "    char* compatible;\n";
    $lcd_struct .= "    char* lcd_name;\n";
    $lcd_struct .= "    uint8_t gpio_id;\n";
    $lcd_struct .= "    char* product_name;\n";
    $lcd_struct .= "};\n\n";

    $lcd_struct .= "static struct lcd_kit_default_panel_map default_panel_map[] = {\n";
...
sub create_product_map_struct
{
    my $lcd_struct;
    my $name_list = shift;
    $lcd_struct  = "\n/*---------------------------------------------------------------------------*/\n";
    $lcd_struct .= "/* static product mapping variable                                           */\n";
    $lcd_struct .= "/*---------------------------------------------------------------------------*/\n";
    $lcd_struct .= "struct lcd_kit_product_map {\n";
    $lcd_struct .= "    uint16_t board_id;\n";
    $lcd_struct .= "    char* product_name;\n";
    $lcd_struct .= "};\n\n";

    $lcd_struct .= "static struct lcd_kit_product_map product_map[] = {\n";
...
#struc on honor and qcom platform is different, so two function here!
...
sub create_honor_lcd_config_data
{
    my $lcd_config;
    my $lcd_name = shift;
...
sub create_data_init_func
{
    ...
    for $count ( 0 ..  scalar(@$list)-1 ) {
        ...
        {
          $lcd_init .= create_honor_lcd_config_data($temp);
        }
    }
    ...
    #else
    #{
    #    $lcd_init .= create_honor_lcd_config_data($default_panel);
    #}
...
sub create_lcd_effect_config
{
    ...
    $lcd_effect_cfg .= "            pinfo->gmp_lut_table_len = ARRAY_3_SIZE(" . lc($lcd_name) . "_gmp_lut_table_low32bit);\n";
...
sub create_lcd_effect_data_func
{
    ...
    $lcd_effect_func  = "static void lcd_kit_effect_get_data(uint8_t panel_id, struct honor_panel_info* pinfo)\n";
    $lcd_effect_func .= "{\n";
	$lcd_effect_func .= create_plat_effect_config("plat");
    ...
    for $i ( 0 ..  scalar(@$list)-1 ) {
		...
		$temp =~ s/\t//g;
		$temp =~ s/\n//g;
		my @lcd_element = split /,/, $temp;
		my $lcd_name    = $lcd_element[0];
		my $product     = $lcd_element[3];
		my $vender_fd   = $lcd_element[4];

        $lcd_name =~ s/-/_/g;
		if(is_parse_effect($lcd_name, $vender_fd) == 1)
		{
			$lcd_effect_func .= "    case " . $product . "_" . uc($lcd_name) . "_PANEL:\n";
			$lcd_effect_func .= create_lcd_effect_config($product . "_" . $lcd_name);
		}
    }
    ...
    $lcd_effect_func .= "        ".'LCD_KIT_INFO("Panel ID not detected %d\n", panel_id);'."\n";
...
sub create_lcd_effect_data_func_mock
{
    my $lcd_effect_func;
    $lcd_effect_func  = "static void lcd_kit_effect_get_data(uint8_t panel_id, struct honor_panel_info* pinfo)\n";
    $lcd_effect_func .= "{\n";
...
sub create_honor_panel_effect_init
{
    my $panel_init;
    $panel_init  = "static uint8_t lcd_kit_get_panel_id(uint32_t product_id, char* compatible)\n";
    $panel_init .= "{\n";
    #$panel_init .= "    uint8_t lcd_panel_id = " . $invalid_panel . ";\n";
	$panel_init .= "    uint8_t lcd_panel_id = " . $default_panel . ";\n";
    $panel_init .= "    int i = 0;\n";
    ...
    $panel_init .= "    for (i = 0; i < ARRAY_SIZE(lcd_kit_map); ++i) {\n";
	$panel_init .= "        length = strlen(lcd_kit_map[i].panel_compatible);\n";
    $panel_init .= "        if ((lcd_kit_map[i].product_id == product_id) && !strncmp(lcd_kit_map[i].panel_compatible, compatible, (length > len?length:len))){\n";
    $panel_init .= "            lcd_panel_id = lcd_kit_map[i].lcd_id;\n";
    $panel_init .= "            break;\n";
    $panel_init .= "        }\n";
    $panel_init .= "    }\n\n";
	$panel_init .= "    " . 'HONOR_FB_INFO("lcd_panel_id = %d\n", lcd_panel_id);' . "\n";
    $panel_init .= "    return lcd_panel_id;\n";
    $panel_init .= "}\n\n";
...
sub create_honor_panel_effect_init_mock
{
    my $panel_init;
    $panel_init  = "static uint8_t lcd_kit_get_panel_id(uint32_t product_id, char* compatible)\n";
    $panel_init .= "{\n";
...
{
    my $panel_init;
    $panel_init  = "static uint8_t lcdkit_panel_init(uint16_t hw_id)\n";
    $panel_init .= "{\n";
    $panel_init .= "    uint8_t lcd_panel_id = " . $default_panel . ";\n";
    #$panel_init .= "    uint8_t lcd_panel_id = " . $invalid_panel . ";\n";
    ...
    $panel_init .= "    ".'snprintf(lcd_id,sizeof(lcd_id), " honor,lcd_panel_id = %X", lcd_hw_id);'."\n";
