...
my @platform_chip_map = (
            ...
            [ "hi6250",                 "honor",          "hi6250"     ],
            [ "hi3660",                 "honor",          "hi3660"     ],
            [ "honor970",               "honor",          "honor970"   ],
            [ "honor980",               "honor",          "honor980"   ],
            [ "honor990",               "honor",          "honor990"   ],
            [ "baltimore",              "honor",          "baltimore"  ],
            [ "denver",                 "honor",          "denver"     ],
            [ "laguna",                 "honor",          "laguna"     ],
            [ "burbank",                "honor",          "burbank"    ],
            [ "orlando",                "honor",          "orlando"    ],
            ...
            [ "k6983v1_64",             "mtk",           "k6983v1_64" ],
            [ "tb8768p1_64_a32",        "mtk",           "tb8768p1_64_a32"],
            [ "k6873v1_64",             "mtk",           "k6873v1_64" ],
            [ "k6853v1_64",             "mtk",           "k6853v1_64" ],
            [ "k6889v1_64",             "mtk",           "k6889v1_64" ],
            [ "k6833v1_64",             "mtk",           "k6833v1_64" ],
            [ "k6877v1_64",             "mtk",           "k6877v1_64" ],
            [ "k6893v1_64",             "mtk",           "k6893v1_64" ],
            [ "k6895v1_64",             "mtk",           "k6895v1_64" ],
            [ "krn_mgk_64_k510",        "mtk",           "krn_mgk_64_k510" ],
            [ "ums9230_4h10_Natv",      "sprd",          "ums9230_4h10_Natv" ] );
...
for (my $count = 0; $count < @platform_chip_map; $count++) {
    if (($target_plat_name eq $platform_chip_map[$count][0]) ||
		($target_plat_name =~ /$platform_chip_map[$count][0]/)) {
...
if (($chip_plat ne "qcom") && ($chip_plat ne "honor") && ($chip_plat ne "mtk") && ($chip_plat ne "sprd")){
...
{
    $plat_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/qcom/$chip_name/lcdkit";
}
if ($chip_plat eq "mtk")
{
    $plat_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/config/mtk/$chip_name/lcdkit";
}
if ($chip_plat eq "honor")
{
    $plat_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/honor/$chip_name/lcdkit";
}
if ($chip_plat eq "sprd")
{
    $plat_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/sprd/$chip_name/lcdkit";
}
...
#collect plat xmls
my $plat_file;
my @plat_xmls = get_honor_xml_list("plat_xmls");
foreach $plat_file (@plat_xmls) {
    debug_print("get plat xml files $plat_file\n");
}

#collect lcd xmls
my $panel_xml_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/panel";
my @panel_xmls = get_honor_xml_list("panel_xmls");
...
my $vender_list_path;
my $udp_vender_list_path = "$root_path/device/honor/customize/udp_lcdkit/lcdkit3.0/config/honor/$chip_name";
my $devkit_tool_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/tools";
...
if (($target_file_type eq "head") || ($target_file_type eq "dto-head") || ($target_file_type eq "all"))
{
    ...
    {
        $out_head_file_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/panel/head";
    }
...
if ((($target_file_type eq "effect") || ($target_file_type eq "all")) && ($chip_plat eq "honor"))
{
    if ($out_effect_file_path eq "")
    {
        $out_effect_file_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/panel/effect";
    }
...
if ($chip_plat eq "qcom")
{
    $vender_list_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/qcom/$chip_name";
}

if ($chip_plat eq "mtk")
{
    $vender_list_path = "$root_path/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/config/mtk/$chip_name";
}

if ($chip_plat eq "honor")
{
    $vender_list_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/honor/$chip_name";
}

if ($chip_plat eq "sprd")
{
    $vender_list_path = "$root_path/vendor/honor/chipset_common/devkit/lcdkit/lcdkit3.0/config/sprd/$chip_name";
}
...
my @lcd_map_list = get_honor_xml_list("lcd_map_list");
...
foreach my $list_entry (@lcd_map_list) {
    ...
    if ($vender_fd ne 'def')
    {
        ...
        {
            my @version_xmls = get_honor_xml_list("version_xmls", $vender_fd, $version_fd);
        ...
        #collect vender xmls
        my @vender_xmls = get_honor_xml_list("vender_xmls", $vender_fd);
...
#genarate platform effect file
    ...
    push(@lcd_map_group, $compatible.", "."\""."\/honor,lcd_config/lcd_kit_".$file_name."\"".", ".$lcd_gpio.", ".$lcd_bdid);
    if (($lcd_gpio eq '0x02')||($lcd_gpio eq '0x0A'))
    {
        push(@lcd_default_panel_map_group, $compatible.", "."\""."\/honor,lcd_config/lcd_kit_".$file_name."\"".", ".$lcd_gpio.", \"".$product."\"");
    }
    if ($lcd_bdid ne '0')
    {
...
if (($target_file_type eq "head") || ($target_file_type eq "dto-head") || ($target_file_type eq "all"))
{
	...
	my $lcd_kit_tools_patch = "/vendor/honor/chipset/base/kernel/devkit/lcdkit/lcdkit3.0/tools";
	if($out_head_file_path =~ /$lcd_kit_tools_patch/) {
		my $replace_with="";
		substr($out_head_file_path, index($out_head_file_path,$lcd_kit_tools_patch),length($lcd_kit_tools_patch),$replace_with);
	}
    ...
    {
        #print $lcd_head_file create_honor_panel_init();
    }
...
if ((($target_file_type eq "effect") || ($target_file_type eq "all")) && ($chip_plat eq 'honor'))
{
    ...
    if ($parse_effect_from_dts ne "1") {
        ...
        print $lcd_effect_file create_honor_panel_effect_init();
    } else {
        print $lcd_effect_file create_lcd_effect_data_func_mock();
        print $lcd_effect_file create_honor_panel_effect_init_mock();
    }
...
if (($target_file_type eq "effect") && (($chip_plat eq 'honor') || ($chip_plat eq 'mtk')))
{
...
if ($target_file_type eq "trebledts")
{
	...
	open (my $lcd_dtsi_file, '>'.$out_dtsi_file) or die "open $out_dtsi_file fail!\n";
	print $lcd_dtsi_file "/ {\n";
	print $lcd_dtsi_file "\thonor_lcd_config: honor,lcd_config {\n";
	foreach my $list_entry (@lcd_map_list) {
		$list_entry =~ s/ //g;
		$list_entry =~ s/\t//g;
		$list_entry =~ s/\n//g;
...
sub is_parse_effect
{
	...
	if ($vender_fd ne 'def')
	{
		@vender_xmls = get_honor_xml_list("vender_xmls", $vender_fd);
		$lcd_name =~ s/_/-/g;
		my $vender_xml = get_xml_file(\@vender_xmls, $lcd_name . '.xml');
		$xmldoc = $lcd_xml_parser->parse_file($vender_xml);
	...
	if ($chip_plat eq "honor") {
		if ($tag_xmls eq "plat_xmls") {
			@dst_list = (glob("$plat_path/*.xml"),
				glob("$root_path/device/honor/customize/udp_lcdkit/lcdkit3.0/config/honor/$chip_name/lcdkit/*.xml"));
		}
		elsif ($tag_xmls eq "panel_xmls") {
			@dst_list = (glob("$panel_xml_path/*.xml"),
				glob("$root_path/device/honor/customize/udp_lcdkit/lcdkit3.0/panel/*.xml"));
		}
			...
			error_print("honor get xmls'type not exist!\n");
		}
	}
	else {
		if ($tag_xmls eq "plat_xmls") {
		...
		elsif ($chip_plat eq "sprd" && $tag_xmls eq "vender_xmls") {
			$vender_fd = shift;
			@dst_list = glob("$vender_list_path/$vender_fd/*.xml");
		}
		elsif ($tag_xmls eq "version_xmls") {
