 ...
 *
 * Copyright (c) 2013-2020 Honor Technologies Co., Ltd.
 *
...
#ifndef __HONOR_TS_KIT_H_
#define __HONOR_TS_KIT_H_
...
//#if (defined(CONFIG_HONOR_DEVKIT_MTK_3_0) || IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0) || \
...
//#endif
//#include "../hostprocessing/honor_tp_tui.h"
...
#define SUSPEND_DONE 1
#define RESUME_DONE 0
enum ts_gesture_num {
	/* 0.Double tap:KEY_F1 */
 ...
 * Baseline status.
 */
struct baseline_status {
	unsigned short baseline_thermal_count;
	unsigned short baseline_frozen_count;
	unsigned short baseline_fast_count;
	unsigned short baseline_rezero_count;
	unsigned short baseline_errorType;
};
...
struct ts_fingers {
	...
	struct fingers_debug_info fingers_debug_info;
};
...
enum ts_raw_data_type {
	...
	/*self cap raw data multiplexed 1 */
	RAW_DATA_TYPE_SELFCAPRAWDATA = 1,
...
struct ts_factory_extra_cmd {
	...
	unsigned int factory_mode_current_backlight;
};
...
struct ts_feature_info {
	...
	struct ts_lowpower_info lowpower_info;
};
...
struct ts_device_ops {
	...
	int (*ts_lowpower_ctrl)(unsigned int status);
};
...
struct ts_kit_device_data {
	...
	unsigned int irq_config_no_suspend;
	...
	u8 print_all_trx_diffdata_flag;
	unsigned int hide_fw_name;
...
struct ts_kit_platform_data {
	...
	wait_queue_head_t fingers_waitq;
	...
	unsigned int use_ap_gesture;
	u32 tui_another_irq_gpio_num;
};

void ts_inputkey_report(unsigned int gesture_wakeup_value);
void ts_aod_click_report(unsigned int gesture_wakeup_value);
...
extern int tpkit_ops_register(struct tpkit_thp_ops *ops);
extern int tpkit_ops_unregister(struct tpkit_thp_ops *ops);
#endif
#endif
