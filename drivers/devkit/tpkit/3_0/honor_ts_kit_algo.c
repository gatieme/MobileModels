...
static struct timespec64 curr_time[FILTER_GLOVE_NUMBER] = { { 0, 0 } };
static struct timespec64 pre_finger_time[FILTER_GLOVE_NUMBER] = { { 0, 0 } };
...
static int pre_x = -1;
static int pre_y = -1;
static struct timespec64 pre_pen_down_time = { 0, 0 };
static struct timespec64 pre_pen_up_time = { 0, 0 };
...
static int filter_illegal_glove(u8 n_finger, struct ts_fingers *in_info)
{
	...
	/* the new interrupt is a glove signal */
		...
		case FINGER_STATE:
			ktime_get_ts64(&curr_time[n_finger]);
			...
			if ((interval_time > 0) &&
				...
				ktime_get_ts64(&pre_finger_time[n_finger]);
			else
	...
	struct ts_fingers *out_info)
{
	...
	max_fingers = g_ts_kit_platform_data.max_fingers;
	for (index = 0, id = 0; index < max_fingers; index++, id++) {
		if (in_info->cur_finger_number == 0) {
			if (index < FILTER_GLOVE_NUMBER) {
				...
				if (touch_state == FINGER_STATE)
					ktime_get_ts64(&pre_finger_time[index]);
			}
...
int ts_pen_open_confirm(struct ts_pens *pens)
{
	...
	long interval_thr = 0;
	struct timespec64 cur_time = { 0, 0 };

	if (g_ts_kit_platform_data.feature_info.wakeup_gesture_enable_info.switch_value ==
			STYLUS_WAKEUP_LOW_FREQENCY) {
		ktime_get_ts64(&cur_time);
			/* pen hover leave */
		if (pens->tool.pen_inrange_status == TS_PEN_OUT_RANGE) {
			press_state = PEN_REL_STATE;
			/* pen press release */
		} else if (pens->tool.pressure == 0) {
			if (press_state != PEN_HOVER_STATE) {
				if (press_state == PEN_PRESS_STATE)
					ktime_get_ts64(&pre_pen_up_time);
				press_state = PEN_HOVER_STATE;
				interval_time = (cur_time.tv_sec -
						pre_pen_down_time.tv_sec) *
						SENCOND_TO_MILLISECOND +
						(cur_time.tv_nsec -
						pre_pen_down_time.tv_nsec) /
						SENCOND_TO_NANOSECOND;
			...
			if (press_state != PEN_PRESS_STATE) {
				...
				ktime_get_ts64(&pre_pen_down_time);

				interval_thr = REL_TIME_MIN;
				if (start_check_flag == 0)
			...
			if (start_check_flag) { /* can check */
				x = pens->tool.x;
				y = pens->tool.y;
				if ((pen_pos_x == ABNORMAL_VAL) && (pen_pos_y == ABNORMAL_VAL)) {
					pen_pos_x = x;
					pen_pos_y = y;
				} else {
					/* about 3mm length */
