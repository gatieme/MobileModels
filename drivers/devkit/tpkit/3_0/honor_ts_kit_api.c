...
void ts_inputkey_report(unsigned int gesture_wakeup_value)
{
	...
	if (input_dev == NULL) {
		TS_LOG_ERR("%s: input_dev null\n", __func__);
		return;
	}

	input_report_key(input_dev, gesture_wakeup_value, 1);
	input_sync(input_dev);
	input_report_key(input_dev, gesture_wakeup_value, 0);
	input_sync(input_dev);
	TS_LOG_INFO("%s ->done %u\n", __func__, gesture_wakeup_value);
}
...
/* solve gesture mode spi -13 */
#if IS_ENABLED(CONFIG_TP_QCOM_8450)
		error = enable_irq_wake(irq_id);
		TS_LOG_INFO("%s enable irq wake,flag = %d\n", __func__, error);
#endif
		if (dev->ops->chip_before_suspend)
			error = dev->ops->chip_before_suspend();
		break;
...
#if IS_ENABLED(CONFIG_TP_QCOM_8450)
		error = disable_irq_wake(irq_id);
		TS_LOG_INFO("%s disable irq wake,flag = %d\n", __func__, error);
#endif
	...
	int timeout)
{
	...
	if ((dev->easy_wakeup_info.sleep_mode == TS_POWER_OFF_MODE) &&
		(pm_type == TS_BEFORE_SUSPEND) && (!dev->easy_wakeup_info.aod_mode)) {
		if (!((dev->is_parade_solution == 1) &&
				(dev->sleep_in_mode == 0))) {
...
#if defined(CONFIG_HONOR_DEV_SELFCHECK) || defined(CONFIG_HONOR_HW_DEV_DCT)
