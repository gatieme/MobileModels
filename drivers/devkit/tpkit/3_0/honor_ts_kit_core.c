...
//#include <honor_platform/power/honor_charger.h>
#include <honor_ts_kit.h>
#include <honor_ts_kit_platform.h>
#include <honor_ts_kit_misc.h>
#include <tpkit_platform_adapter.h>
#include <honor_ts_kit_api.h>
#include <honor_ts_kit_algo.h>

#if IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)
...
#if defined(CONFIG_HONOR_BCI_BATTERY)
...
#endif
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)) && (IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)))
#include <linux/timer.h>
#endif

#ifdef CONFIG_HONOR_DEVKIT_HONOR
...
#if (!(IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)) && !(defined CONFIG_HONOR_DEVKIT_MTK_3_0))
...
#if (!(defined CONFIG_LCD_KIT_DRIVER) && (IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)) && \
	!(IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX) || IS_ENABLED(CONFIG_TP_QCOM_8450)))
//#include "dsi_panel.h"
#endif
...
#define HONOR_KIT_DEVICE_NAME "honor_kit"
...
#if IS_ENABLED(CONFIG_HONOR_THP) && (!IS_ENABLED(CONFIG_TP_QCOM_8450))
extern int thp_project_id_provider(char *project_id,
	unsigned int len);
#endif

static int parse_feature_config(void);
static int honor_ts_kit_platform_init(void);
extern int syna_tcm_driver_module_init(void);
...
struct tpkit_thp_ops g_tpkit_thp_ops = {
	.power_notify = ts_kit_multi_power_control_notify,
};

#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
void kit_get_timestamp(struct timeval *tv)
{
...
/*
 * The following is a stub function. For honor platform,
...
static void ts_lowpower_ctrl_cmd(struct ts_cmd_node *in_cmd)
{
	unsigned int status = 0;
	int error;
	struct ts_kit_device_data *dev = g_ts_kit_platform_data.chip_data;
	struct ts_lowpower_info *info =
		(struct ts_lowpower_info *)in_cmd->cmd_param.prv_params;
	...
	TS_LOG_DEBUG("+\n");
	if (dev && dev->ops && dev->ops->ts_lowpower_ctrl) {
		TS_LOG_INFO("%s: update lowpower_status: %u\n", __func__, status);
		error = dev->ops->ts_lowpower_ctrl(status);
		if (error < 0) {
			TS_LOG_ERR("%s: lowpower ctrl error, %d\n", __func__, error);
			return ;
		}
	}
	TS_LOG_DEBUG("-\n");
...
#if !(IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX) || IS_ENABLED(CONFIG_TP_QCOM_8450))
...
static void ts_kit_charger_notifier_register(void)
{
	...
	error = honor_register_notifier(&g_ts_kit_platform_data.charger_detect_notify, 1);
	if (error < 0) {
...
#elif IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)
static int charger_detect_notifier_callback(struct notifier_block *self,
	unsigned long event, void *data)
{
	enum ts_charger_state charger_state = USB_PIUG_OUT;
	...
	if (!g_ts_kit_platform_data.feature_info.charger_info.charger_supported)
		return NO_ERR;
	TS_LOG_INFO("%s, charger event:%ld\n", __func__, event);
...
static void ts_kit_charger_notifier_register(void)
{
	...
	g_ts_kit_platform_data.charger_detect_notify.notifier_call =
		charger_detect_notifier_callback;
	error = power_event_nc_register(POWER_NT_CONNECT, &g_ts_kit_platform_data.charger_detect_notify);
	if (error < 0) {
		TS_LOG_ERR("%s, charger_type_notifier_register failed\n",
			__func__);
		g_ts_kit_platform_data.charger_detect_notify.notifier_call = NULL;
...
static int parse_feature_config(void)
{
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"is_fold_device", &g_ts_kit_platform_data.is_fold_device);
	if (rc)
		TS_LOG_ERR("is_fold_device read error\n");
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"use_ap_gesture", &g_ts_kit_platform_data.use_ap_gesture);
	if (rc)
		TS_LOG_ERR("use_ap_gesture read error\n");

	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"setting_multi_gesture_mode", &node_value);
	if (rc) {
		TS_LOG_ERR("setting_multi_gesture_mode read error\n");
	} else {
		if (tskit_is_factory())
			node_value = 0;
		g_ts_kit_platform_data.setting_multi_gesture_mode = node_value;
		TS_LOG_INFO("setting_multi_gesture_mode configed: %u\n", node_value);
	}
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "hide_plain_id",
		&hide_plain_id);
	if (rc) {
		TS_LOG_ERR("hide_plain_id not exsit\n");
	} else {
		if (tskit_is_factory())
			hide_plain_id = 0;
		g_ts_kit_platform_data.hide_plain_id = hide_plain_id;
		TS_LOG_INFO("hide_plain_id configed: %u\n", hide_plain_id);
	}
...
static int ts_creat_i2c_client(void)
{
	...
	if (!client) {
		TS_LOG_ERR("i2c_new_client_device failed\n");
		return -EIO;
	}
...
static int ts_kit_input_tp_device_register(struct input_dev *dev)
{
	...
	if (g_ts_kit_platform_data.is_fold_device == 1)
		input_dev->name = TS_FOLD_INPUT_DEV_NAME;
	else
...
static int ts_kit_irq_init(void)
{
...
#if IS_ENABLED(CONFIG_TP_QCOM_8450)
	irq_flags |= IRQF_ENABLE_WAKEUP_ON_SUSPEND_IGNORE;
	TS_LOG_INFO("%s: irq_flags = %lx\n", __func__, irq_flags);
#endif
	...
	if(!g_ts_kit_platform_data.chip_data->irq_config_no_suspend)
	...
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	...
	case TS_TOUCH_SWITCH:
		ts_touch_switch_cmd();
		break;
	case TS_LOWPOWER_CTRL:
		ts_lowpower_ctrl_cmd(proc_cmd);
		break;
	case TS_FREEBUFF:
...
static int ts_kit_init(void *data)
{
	...
	ts_send_init_cmd();
#if defined(CONFIG_HONOR_BCI_BATTERY) || defined(CONFIG_MTK_CHARGER) || IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)
	...
	struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32(np, "irq_config_no_suspend", &chip_data->irq_config_no_suspend);
	return 0;
}
	...
	struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32_quiet(np,
		"honor,test_capacitance_via_csvfile",
	...
	ts_of_property_read_u32(np, "honor,support_forcekey_cap_value_test",
	...
	ts_of_property_read_u32_quiet(np, "honor,csvfile_use_product_system",
		&chip_data->csvfile_use_product_system);
	ts_of_property_read_u32_quiet(np, "honor,td43xx_ee_short_test",
		&chip_data->td43xx_ee_short_test_support);
	ts_of_property_read_u32_quiet(np, "honor,trx_delta_test_support",
		&chip_data->trx_delta_test_support);
	ts_of_property_read_u32_quiet(np,
		"honor,threshold_tddi_ee_short_partone",
		&chip_data->tddi_ee_short_test_partone_limit);
	ts_of_property_read_u32_quiet(np,
		"honor,threshold_tddi_ee_short_parttwo",
				...
				struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32_quiet(np, "honor,support_gammadata_in_tp",
		&chip_data->support_gammadata_in_tp);
	ts_of_property_read_u32_quiet(np, "honor,support_2dbarcode_info",
		&chip_data->support_2dbarcode_info);
	ts_of_property_read_u32_quiet(np, "honor,read_2dbarcode_oem_type",
...
int ts_kit_power_supply_put(enum ts_kit_power_id power_id)
{
	...
	if (power->type == TS_KIT_POWER_UNUSED)
		return 0;
	if ((--power->use_count) > 0 || (!power->use_count))
		return 0;
...
int honor_ts_chip_register(struct ts_kit_device_data *chipdata)
{
	int ret;
...
EXPORT_SYMBOL(honor_ts_chip_register);
...
static void get_ts_kit_need_init(void)
{
	int rc;
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "need_ts_kit_init",
			&g_ts_kit_platform_data.need_ts_kit_init);
}
	...
	const struct i2c_device_id *id)
{
	...
	if (!ts_dclient)
		ts_dclient = dsm_register_client(&dsm_tp);
#endif
	error = honor_ts_kit_platform_init();
	if (error)
...
static int honor_ts_kit_platform_init(void)
{
	...
	g_ts_kit_platform_data.ts_dev =
		platform_device_alloc("honor_touch", -1);
	 ...
	 * thp driver.
	 */
#if defined(CONFIG_LCD_KIT_DRIVER) && (!IS_ENABLED(CONFIG_TP_QCOM_8450))
	if (g_ts_kit_platform_data.multi_panel_index == SINGLE_TOUCH_PANEL) {
	...
	error = tpkit_ops_register(&g_tpkit_thp_ops);
	if (error)
		TS_LOG_ERR("tpkit_ops_register failed :%d\n", error);
#endif
	...
	init_waitqueue_head(&g_ts_kit_platform_data.fingers_waitq);
	/*
...
static int kit_remove(struct spi_device *sdev)
{
	TS_LOG_INFO("%s: in\n", __func__);
...
static int kit_probe(struct spi_device *sdev)
{
	...
	get_ts_kit_need_init();
	if (g_ts_kit_platform_data.need_ts_kit_init) {
		TS_LOG_ERR(" %s: do not need init return\n", __func__);
		return 0;
	}
	...
	if (g_ts_kit_platform_data.is_fold_device == 1 &&
		recovery_mode == true) {
		TS_LOG_INFO("fold device in recovery\n");
		return 0;
	}
...
#endif
	g_ts_kit_platform_data.spi = sdev;
	spi_set_drvdata(sdev, &g_ts_kit_platform_data);
#if IS_ENABLED(CONFIG_DSM)
	if (!ts_dclient)
		ts_dclient = dsm_register_client(&dsm_tp);
#endif
	...
	syna_tcm_driver_module_init();

out:
	return error;
}

static const struct of_device_id g_kit_match_table[] = {
	{ .compatible = "honor,ts_kit", },
	{ },
};

static const struct spi_device_id g_kit_device_id[] = {
	{ HONOR_KIT_DEVICE_NAME, 0 },
	{ }
};
MODULE_DEVICE_TABLE(spi, g_kit_device_id);

static struct spi_driver g_kit_spi_driver = {
	.probe = kit_probe,
	.remove = kit_remove,
	.id_table = g_kit_device_id,
	.driver = {
		.name = HONOR_KIT_DEVICE_NAME,
		.owner = THIS_MODULE,
		.bus = &spi_bus_type,
		.of_match_table = g_kit_match_table,
	},
};

static int __init g_kit_spi_init(void)
{
	int ret = 0;
	TS_LOG_INFO("%s called\n", __func__);
	ret = spi_register_driver(&g_kit_spi_driver);
	if (ret != 0)
		TS_LOG_ERR("spi register driver failed!");

	TS_LOG_INFO("%s end\n", __func__);
	return ret;
}

static void __exit g_kit_spi_exit(void)
{
	TS_LOG_INFO("%s called\n", __func__);
	spi_unregister_driver(&g_kit_spi_driver);
}
#if IS_ENABLED(CONFIG_TP_QCOM_8450)
...
late_initcall(g_kit_spi_init);
module_exit(g_kit_spi_exit);
