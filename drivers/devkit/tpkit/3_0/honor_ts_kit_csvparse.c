	...
	char *ptr, int32_t *data, int rows)
{
	...
	if (!row_data) {
		TS_LOG_ERR("%s: kzalloc row_data failed.\n", __func__);
		return;
	}
	...
	int rows, int columns)
{
	...
	size = fp->f_op->llseek(fp, 0, SEEK_END);
	TS_LOG_INFO("%s, SEEK_END file size %lld\n", __func__, size);
	stat.size = size;
	size = fp->f_op->llseek(fp, 0, SEEK_SET);
	...
	read_ret = kernel_read(fp, buf, stat.size, &pos);
	if (read_ret > 0) {
		buf[stat.size] = '\0';
		ptr = buf;
