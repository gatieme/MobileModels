...
#if ((defined CONFIG_FB) && (defined CONFIG_HONOR_DEVKIT_HONOR))
#include "honor_fb.h"
#endif
...
static ssize_t tp_kit_lowpower_ctrl_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct ts_lowpower_info *info =
		&g_ts_kit_platform_data.feature_info.lowpower_info;

	if (!dev || !buf || !dev_data) {
		TS_LOG_ERR("%s: pointer is null\n", __func__);
		return -EINVAL;
	}
	if (!g_ts_kit_platform_data.use_ap_gesture) {
		TS_LOG_ERR("%s not support\n", __func__);
		return -EINVAL;
	}
	ret = sscanf(buf, "%1u", &status);
	if (ret <= 0) {
		TS_LOG_ERR("%s: illegal input, ret = %d\n", __func__, ret);
		return -EINVAL;
	}
	info->status = status;
	TS_LOG_INFO("%s: update lowpower_status: %u\n", __func__, status);
	if (dev_data->work_status != SUSPEND_DONE) {
        	TS_LOG_INFO("%s: resumed, not handle lp\n", __func__);
        	return -EINVAL;
	}
	memset(&cmd, 0, sizeof(struct ts_cmd_node));
	cmd.command = TS_LOWPOWER_CTRL;
	cmd.cmd_param.prv_params = (void *)info;
...
static DEVICE_ATTR(tp_ud_lowpower_ctrl, 0660, NULL, tp_kit_lowpower_ctrl_store);
