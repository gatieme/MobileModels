	...
	const struct ts_fingers *src)
{
	...
	memcpy(&dst->fingers_debug_info, &src->fingers_debug_info,
		sizeof(struct fingers_debug_info));
...
static long ts_ioctl_get_fingers_info(unsigned long arg)
{
	...
	wait_event_interruptible(pdata->fingers_waitq,
		(atomic_read(&pdata->fingers_waitq_flag) == AFT_WAITQ_WAKEUP));
	...
	struct ts_fingers *fingers)
{
	...
	for (id = 0; id < 70; id++) {
		waitq_flag = atomic_read(&pdata->fingers_waitq_flag);
		if (waitq_flag == AFT_WAITQ_WAIT)
			break;
		udelay(20);
	}
#else
	...
	if (waitq_flag == AFT_WAITQ_WAIT) {
		...
		return 0;
	} else if (waitq_flag != AFT_WAITQ_IDLE) {
		TS_LOG_DEBUG(
...
{
	int error;
	char *set_node_name = NULL;
	char *get_node_name = NULL;
	...
	if (pdata->multi_panel_index != SINGLE_TOUCH_PANEL) {
		set_node_name = kzalloc(sizeof(char) * MULTI_PANEL_NODE_BUF_LEN, GFP_KERNEL);
		if (!set_node_name) {
			TS_LOG_ERR("%s:set_node_name is null\n", __func__);
			error = -ENOMEM;
			goto error;
		}
		get_node_name = kzalloc(sizeof(char) * MULTI_PANEL_NODE_BUF_LEN, GFP_KERNEL);
		if (!get_node_name) {
			TS_LOG_ERR("%s:get_node_name is null\n", __func__);
			error = -ENOMEM;
			goto error;
		}
