...
void ts_report_input(struct ts_cmd_node *in_cmd)
{
	...
	if (atomic_read(&g_ts_kit_platform_data.chip_data->quickly_screen_on_off) == true) {
		TS_LOG_ERR("multi screen in quickly screen switch, not report input\n");
		return;
	}
	...
	if (((easy_wakeup_info->sleep_mode == TS_GESTURE_MODE) ||
		(easy_wakeup_info->aod_mode == TS_TOUCH_AOD_OPEN) ||
...
int ts_power_off_control_mode(int irq_id, struct ts_cmd_node *in_cmd)
{
	...
	case TS_SUSPEND_DEVICE: /* device power off or sleep */
		...
		atomic_set(&dev->quickly_screen_on_off, false);
...
static int ts_before_suspend(int irq_id,
...
static void set_aft_flag(struct ts_kit_device_data *dev)
{
...
static int ts_suspend_device(int irq_id,
	struct ts_kit_device_data *dev, struct ts_cmd_node *out_cmd)
{
	int error = NO_ERR;
	...
	if (dev->ops->chip_suspend)
		error = dev->ops->chip_suspend();
	/*
	 * in hisi platform, when set setting_multi_gesture_mode 1, should not
	 * enable irq, because we handle irq in sensorhub;
	 * but in qcom platform, we use ap gesture solution, and should enable
	 * irq when suspend.
	 */
#ifndef CONFIG_HUAWEI_DEVKIT_QCOM_3_0
	if (!(dev->is_parade_solution == true) &&
	...
	if (g_ts_kit_platform_data.need_enable_irq_wake)
		if (enable_irq_wake(g_ts_kit_platform_data.irq_id))
			TS_LOG_ERR("%s:irq wake fail\n", __func__);
...
static void ts_ic_shut_down(int irq_id,
	struct ts_kit_device_data *dev)
{
	if (!(dev->is_parade_solution == true))
		disable_irq(irq_id);
...
static int ts_resume_device(int irq_id,
	struct ts_kit_device_data *dev)
{
	int error = NO_ERR;
...
static int ts_after_resume(int irq_id,
	struct ts_kit_device_data *dev)
{
	int error = NO_ERR;
	...
	struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	switch (pm_type) {
	case TS_BEFORE_SUSPEND: /* do something before suspend */
		error = ts_before_suspend(irq_id, dev);
		break;
	/* switch to easy-wakeup mode, and enable interrupts */
	case TS_SUSPEND_DEVICE:
		error = ts_suspend_device(irq_id, dev, out_cmd);
		break;
	case TS_IC_SHUT_DOWN:
		ts_ic_shut_down(irq_id, dev);
		break;
	case TS_RESUME_DEVICE: /* exit easy-wakeup mode and restore sth */
		error = ts_resume_device(irq_id, dev);
		break;
	case TS_AFTER_RESUME: /* do nothing */
		error = ts_after_resume(irq_id, dev);
		break;
	default:
		TS_LOG_ERR("pm_type = %d\n", pm_type);
		error = -EINVAL;
...
static int ts_kit_screenoff_captest_power_ctrl_notify(void)
{
...
static int ts_kit_multi_screen_switch_power_ctrl_notify(enum lcd_kit_ts_pm_type pm_type,
	struct ts_kit_device_data *dev)
{
...
static int ts_kit_suspend_when_tui_working(enum lcd_kit_ts_pm_type pm_type,
	struct ts_kit_device_data *dev)
{
...
static void ts_kit_setting_multi_gesture_mode(struct ts_kit_device_data *dev)
{
	if (dev->screen_switch_flag || dev->multi_screen_unfold) {
		TS_LOG_INFO("screen_switch_flag:%d,multi_screen_unfold:%d\n",
			dev->screen_switch_flag, dev->multi_screen_unfold);
...
static void ts_kit_suspend_disable_irq(struct ts_kit_device_data *dev)
{
	if (!((dev->is_parade_solution == 1) &&
			(dev->sleep_in_mode == 0))) {
...
static void ts_kit_revert_multi_gesture_mode(struct ts_kit_device_data *dev)
{
...
static void ts_kit_resume_send_roi_cmd(struct ts_kit_device_data *dev,
	struct ts_feature_info *feature_info)
{
	int error = 0;
...
int ts_kit_power_control_notify(enum lcd_kit_ts_pm_type pm_type, int timeout)
{
	...
	if (atomic_read(&g_ts_kit_platform_data.state) == TS_UNINIT) {
		TS_LOG_ERR("ts is not init\n");
		return -EINVAL;
	}

	if (!ts_kit_screenoff_captest_power_ctrl_notify())
		return 0;

	dev = g_ts_kit_platform_data.chip_data;
	feature_info = &dev->ts_platform_data->feature_info;
	if (!ts_kit_multi_screen_switch_power_ctrl_notify(pm_type, dev))
		return 0;

	if (!ts_kit_suspend_when_tui_working(pm_type, dev))
		return 0;

	if (g_ts_kit_platform_data.setting_multi_gesture_mode &&
		pm_type == TS_EARLY_SUSPEND)
		ts_kit_setting_multi_gesture_mode(dev);
	...
	if ((g_ts_kit_platform_data.setting_multi_gesture_mode == 0) &&
		...
		ts_kit_suspend_disable_irq(dev);
	...
	if (g_ts_kit_platform_data.setting_multi_gesture_mode &&
		...
		ts_kit_revert_multi_gesture_mode(dev);

	if (pm_type == TS_AFTER_RESUME)
		ts_kit_resume_send_roi_cmd(dev, feature_info);
...
void ts_kit_screen_status_notify(int status)
{
	...
	if (status == QUICKLY_SCREEN_ON_OFF) {
		atomic_set(&dev->quickly_screen_on_off, true);
		return;
	}
...
static int ts_kit_fb_notifier_for_udp(int blank)
{
	int error = NO_ERR;

	switch (blank) {
	case FB_BLANK_UNBLANK:
		TS_LOG_INFO("%s: resume +\n", __func__);
		error = ts_kit_power_control_notify(TS_RESUME_DEVICE, NO_SYNC_TIMEOUT);
		error = ts_kit_power_control_notify(TS_AFTER_RESUME, NO_SYNC_TIMEOUT);
		TS_LOG_INFO("%s: resume -\n", __func__);
		break;
	case FB_BLANK_POWERDOWN:
		TS_LOG_INFO("%s: suspend +\n", __func__);
		error = ts_kit_power_control_notify(TS_BEFORE_SUSPEND, NO_SYNC_TIMEOUT);
		error = ts_kit_power_control_notify(TS_SUSPEND_DEVICE, NO_SYNC_TIMEOUT);
		TS_LOG_INFO("%s: suspend -\n", __func__);
		break;
	default:
		break;
...
int ts_kit_fb_notifier_call(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	blank = event->data;
	error = ts_kit_fb_notifier_for_udp(*blank);
	return error;
}
#endif
...
int ts_get_chip_info(struct ts_cmd_node *in_cmd)
{
	...
	check_watchdog_state = g_ts_kit_platform_data.esd_watchdog_state_check;
	if (check_watchdog_state) {
		timer_state =
			timer_pending(&g_ts_kit_platform_data.watchdog_timer);
		TS_LOG_INFO("%s, esd_watchdog_timer_state=%d\n",
			__func__, timer_state);
	}
	if ((!g_ts_kit_platform_data.chip_data->is_parade_solution) &&
		((!check_watchdog_state) ||
		(check_watchdog_state && timer_state)))
		ts_stop_wd_timer(&g_ts_kit_platform_data);
