...
#define PARADE_TX_NUM (info->buff[1])
#define PARADE_RX_NUM (info->buff[0])
#define PARADE_SENSOR_NUM (PARADE_TX_NUM*PARADE_TX_NUM)
#define PARADE_MUTUAL_DIFF_OFFSET (PARADE_SENSOR_NUM)
#define PARADE_SELF_DIFF_OFFSET (PARADE_MUTUAL_DIFF_OFFSET+PARADE_SENSOR_NUM)
#define PARADE_GRADIENT_RX_OFFSET (PARADE_SELF_DIFF_OFFSET + PARADE_TX_NUM + PARADE_RX_NUM)
#define PARADE_GRADIENT_TX_OFFSET (PARADE_GRADIENT_RX_OFFSET+PARADE_RX_NUM)
#define PARADE_CP_RX_OFFSET (PARADE_GRADIENT_TX_OFFSET+PARADE_TX_NUM)
#define PARADE_CP_TX_OFFSET (PARADE_CP_RX_OFFSET+PARADE_RX_NUM)
#define PARADE_CP_DELTA_RX (PARADE_CP_TX_OFFSET+PARADE_TX_NUM)
...
__attribute__((weak)) int creat_touchscreen(void)
{
	return 0;
}
...
const int get_tskit_register_status(void)
{
	return atomic_read(&g_ts_kit_platform_data.register_flag);
}
EXPORT_SYMBOL(get_tskit_register_status);
...
#ifdef CONFIG_HUAWEI_THP
extern const char *thp_get_vendor_name(u32 panel_id);
#endif
const char *ts_kit_get_vendor_name(u32 panel_id)
{
	const char *thp_vendor_name =
#ifdef CONFIG_HUAWEI_THP
		thp_get_vendor_name(panel_id);
#else
		"null";
#endif
...
static int rawdata_prade_mutual_noise_data(struct seq_file *m,
	struct ts_rawdata_info *info, int row_size, int range_size)
{
	int rd_index = RAWDATA_SIZE_LIMIT + PARADE_MUTUAL_DIFF_OFFSET;
	int index;
	int index1;
...
static int rawdata_prade_self_noise_data(struct seq_file *m,
	struct ts_rawdata_info *info, int row_size, int range_size)
{
	int rd_index = RAWDATA_SIZE_LIMIT + PARADE_SELF_DIFF_OFFSET;
	int index1;
...
static int rawdata_prade_cm_gradient_data(struct seq_file *m,
	struct ts_rawdata_info *info, int row_size, int range_size)
{
	int rd_index = RAWDATA_SIZE_LIMIT + PARADE_GRADIENT_RX_OFFSET;
	int index1;
...
static int rawdata_prade_cp_data(struct seq_file *m,
	struct ts_rawdata_info *info, int row_size, int range_size)
{
	int rd_index = RAWDATA_SIZE_LIMIT + PARADE_CP_RX_OFFSET;
	int index1;
...
static int rawdata_prade_cp_delta_data(struct seq_file *m,
	struct ts_rawdata_info *info, int row_size, int range_size)
{
	int rd_index = RAWDATA_SIZE_LIMIT + PARADE_CP_DELTA_RX;
	int index1;
	...
	list_for_each_entry(rawdatanode, &info->rawdata_head, node) {
		if (strlen(rawdatanode->tptestfailedreason) > 0) {
			if (rawdatanode->typeindex < RAW_DATA_TYPE_FORCEKEY) {
				...
				resulttemp[RESULT_ID_0] = (rawdatanode->typeindex /
					RAWDATA_TESTTMP_NUM) + '0';
				resulttemp[RESULT_ID_1] = (rawdatanode->typeindex %
					RAWDATA_TESTTMP_NUM) + '0';
				resulttemp[RESULT_ID_2] = '\0';
			}
			seq_printf(m, "%s", resulttemp);
...
static const struct proc_ops calibration_proc_fops = {
	.proc_open = calibration_proc_open,
	.proc_read = seq_read,
...
static void parse_extra_feature_config(void)
{
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"support_input_location",
		&g_ts_kit_platform_data.support_input_location);
	if (rc)
		g_ts_kit_platform_data.support_input_location = 0;
}
...
static int parse_feature_config(void)
{
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"esd_watchdog_state_check",
		&g_ts_kit_platform_data.esd_watchdog_state_check);
	if (rc)
		TS_LOG_ERR("esd_watchdog_state_check read error\n");
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"aod_display_support",
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"support_gesture_mode",
		&g_ts_kit_platform_data.support_gesture_mode);
	if (rc) {
		g_ts_kit_platform_data.support_gesture_mode = 1;
		TS_LOG_ERR("support_gesture_mode read error\n");
	}
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"need_enable_irq_wake",
		&g_ts_kit_platform_data.need_enable_irq_wake);
	if (rc) {
		TS_LOG_ERR("need_enable_irq_wake read error\n");
		g_ts_kit_platform_data.need_enable_irq_wake = 0;
	}
	rc = of_property_read_u32(g_ts_kit_platform_data.node,
		"ts_app_info_flag",
...
static int ts_kit_create_sysfs(void)
{
	...
	if (g_ts_kit_platform_data.multi_panel_index ==
		...
		ret = creat_touchscreen();
	}
	if (error + ret) {
		TS_LOG_ERR("%s: Fail create link error = %d, ret = %d\n",
			__func__, error, ret);
		error += ret;
		goto err_free_sysfs;
	}
...
static int ts_kit_chip_init(void)
{
	...
	mutex_init(&g_ts_kit_platform_data.irq_mutex);
...
static void ts_kit_set_input_dev_location(struct input_dev *input_dev)
{
	struct ts_kit_platform_data *data = &g_ts_kit_platform_data;
	int rc;

	if (!data->support_input_location ||
		(data->multi_panel_index == SINGLE_TOUCH_PANEL))
		return;

	rc = snprintf(data->input_dev_location, MULTI_PANEL_NODE_BUF_LEN,
		"%s%d", INPUT_DEV_LOCATION_NAME, data->multi_panel_index);
	if (rc < 0)
		TS_LOG_ERR("%s: snprintf err\n", __func__);

	input_dev->phys = (const char *)(data->input_dev_location);
	TS_LOG_INFO("%s location name is :%s\n", __func__,
		input_dev->phys);
}
...
static int ts_kit_input_tp_device_register(struct input_dev *dev)
{
	...
	ts_kit_set_input_dev_location(input_dev);
...
static struct notifier_block ts_kit_fb_notifier_block = {
	.notifier_call = ts_kit_fb_notifier_call,
};
#endif
...
{
#if (!(defined CONFIG_LCD_KIT_DRIVER) && (defined CONFIG_HUAWEI_DEVKIT_QCOM_3_0) \
	&& !(defined CONFIG_HUAWEI_TS_KIT_UDP))
	int rc;
...
#elif (defined CONFIG_HUAWEI_TS_KIT_UDP)
	fb_register_client(&ts_kit_fb_notifier_block);
#endif
...
static int ts_kit_irq_init(void)
{
	...
	if (g_ts_kit_platform_data.need_enable_irq_wake)
		error = request_irq(g_ts_kit_platform_data.irq_id, ts_irq_handler,
			irq_flags, "ts", &g_ts_kit_platform_data);
	else
		error = request_irq(g_ts_kit_platform_data.irq_id, ts_irq_handler,
...
struct ts_proc_command_process_group {
	enum ts_cmd cmd;
	void (*func)(struct ts_cmd_node *proc_cmd,
		struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync);
};

struct ts_proc_command_directly_group {
	enum ts_cmd cmd;
	int (*func)(struct ts_cmd_node *in_cmd,
		struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync);
};

static void ts_int_process_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_input_algo_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_palm_key_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_report_key_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_report_input_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_report_fingers_pen_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_algo_finger_pen_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_report_finger_pen_directly_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_report_pen_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_power_control_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_fw_update_boot_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_fw_update_sd_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_debug_data_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	disable_irq(g_ts_kit_platform_data.irq_id);
...
static void ts_read_raw_data_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	disable_irq(g_ts_kit_platform_data.irq_id);
...
static void ts_read_calibration_data_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	disable_irq(g_ts_kit_platform_data.irq_id);
...
static void ts_get_calibration_info_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	ts_get_calibration_info(proc_cmd, out_cmd, sync);
}

static void ts_oem_info_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_gamma_info_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_get_chip_info_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_set_info_flag_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_calibrate_device_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_calibrate_device_lpwg_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_dsm_debug_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_glove_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_test_type_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_palm_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_hand_detect_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_force_reset_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
	disable_irq(g_ts_kit_platform_data.irq_id);
...
static void ts_int_err_occur_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
	ts_stop_wd_timer(&g_ts_kit_platform_data);
...
static void ts_err_occur_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
	disable_irq(g_ts_kit_platform_data.irq_id);
...
static void ts_check_status_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_wakeup_gesture_enable_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_holster_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_roi_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_touch_window_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_chip_detect_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_charger_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_regs_store_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
...
static void ts_test_cmd_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)sync;
...
static void ts_tp_init_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)proc_cmd;
	(void *)out_cmd;
...
static void ts_touch_switch_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)proc_cmd;
	(void *)out_cmd;
...
static void ts_freebuff_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_set_sensibility_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_boot_detection_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)proc_cmd;
	(void *)out_cmd;
...
static void ts_set_tp_ic_cmd_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)out_cmd;
	(void *)sync;
...
static void ts_set_recovery_window_func(struct ts_cmd_node *proc_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)proc_cmd;
	(void *)out_cmd;
...
struct ts_proc_command_process_group ts_proc_command_process_table[] = {
	{ TS_INT_PROCESS, ts_int_process_func },
	{ TS_INPUT_ALGO, ts_input_algo_func },
	{ TS_PALM_KEY, ts_palm_key_func },
	{ TS_REPORT_KEY, ts_report_key_func },
	{ TS_REPORT_INPUT, ts_report_input_func },
	{ TS_REPORT_FINGERS_PEN, ts_report_fingers_pen_func },
	{ TS_ALGO_FINGER_PEN, ts_algo_finger_pen_func },
	{ TS_REPORT_FINGER_PEN_DIRECTLY, ts_report_finger_pen_directly_func },
	{ TS_REPORT_PEN, ts_report_pen_func },
	{ TS_POWER_CONTROL, ts_power_control_func },
	{ TS_FW_UPDATE_BOOT, ts_fw_update_boot_func },
	{ TS_FW_UPDATE_SD, ts_fw_update_sd_func },
	{ TS_DEBUG_DATA, ts_debug_data_func },
	{ TS_READ_RAW_DATA, ts_read_raw_data_func },
	{ TS_READ_CALIBRATION_DATA, ts_read_calibration_data_func },
	{ TS_GET_CALIBRATION_INFO, ts_get_calibration_info_func },
	{ TS_OEM_INFO_SWITCH, ts_oem_info_switch_func },
	{ TS_GAMMA_INFO_SWITCH, ts_gamma_info_switch_func },
	{ TS_GET_CHIP_INFO, ts_get_chip_info_func },
	{ TS_SET_INFO_FLAG, ts_set_info_flag_func },
	{ TS_CALIBRATE_DEVICE, ts_calibrate_device_func },
	{ TS_CALIBRATE_DEVICE_LPWG, ts_calibrate_device_lpwg_func },
	{ TS_DSM_DEBUG, ts_dsm_debug_func },
	{ TS_GLOVE_SWITCH, ts_glove_switch_func },
	{ TS_TEST_TYPE, ts_test_type_func },
	{ TS_PALM_SWITCH, ts_palm_switch_func },
	{ TS_HAND_DETECT, ts_hand_detect_func },
	{ TS_FORCE_RESET, ts_force_reset_func },
	{ TS_INT_ERR_OCCUR, ts_int_err_occur_func },
	{ TS_ERR_OCCUR, ts_err_occur_func },
	{ TS_CHECK_STATUS, ts_check_status_func },
	{ TS_WAKEUP_GESTURE_ENABLE, ts_wakeup_gesture_enable_func },
	{ TS_HOLSTER_SWITCH, ts_holster_switch_func },
	{ TS_ROI_SWITCH, ts_roi_switch_func },
	{ TS_TOUCH_WINDOW, ts_touch_window_func },
	{ TS_CHIP_DETECT, ts_chip_detect_func },
	{ TS_CHARGER_SWITCH, ts_charger_switch_func },
	{ TS_REGS_STORE, ts_regs_store_func },
	{ TS_TEST_CMD, ts_test_cmd_func },
	{ TS_TP_INIT, ts_tp_init_func },
	{ TS_TOUCH_SWITCH, ts_touch_switch_func },
	{ TS_FREEBUFF, ts_freebuff_func },
	{ TS_SET_SENSIBILITY, ts_set_sensibility_func },
	{ TS_BOOT_DETECTION, ts_boot_detection_func },
	{ TS_SET_TP_IC_CMD, ts_set_tp_ic_cmd_func },
	{ TS_SET_RECOVERY_WINDOW, ts_set_recovery_window_func },
};
	...
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	...
	for (i = 0; i < ARRAY_SIZE(ts_proc_command_process_table); i++) {
		if (proc_cmd->command == ts_proc_command_process_table[i].cmd) {
			ts_proc_command_process_table[i].func(proc_cmd, out_cmd, sync);
			break;
		}
	}
}
...
static int ts_fw_update_boot_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_fw_update_boot(in_cmd);
	return ret;
}

static int ts_fw_update_sd_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_fw_update_sd(in_cmd);
	return ret;
}

static int ts_oem_info_switch_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;

	(void *)out_cmd;
	ret = ts_oem_info_switch(in_cmd, sync);
	return ret;
}

static int ts_get_chip_info_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_get_chip_info(in_cmd);
	return ret;
}

static int ts_set_info_flag_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_set_info_flag(in_cmd);
	return ret;
}

static int ts_calibrate_device_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;

	(void *)out_cmd;
	ret = ts_calibrate(in_cmd, sync);
	return ret;
}

static int ts_glove_switch_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_glove_switch(in_cmd);
	return ret;
}

static int ts_check_status_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;

	(void *)sync;
	ret = ts_check_status(in_cmd, out_cmd);
	return ret;
}

static int ts_holster_switch_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_holster_switch(in_cmd);
	return ret;
}

static int ts_roi_switch_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	int ret;
	...
	(void *)out_cmd;
	(void *)sync;
	ret = ts_roi_switch(in_cmd);
	return ret;
}

static int ts_touch_switch_direct_func(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	(void *)in_cmd;
	(void *)out_cmd;
...
struct ts_proc_command_directly_group ts_proc_command_directly_table[] = {
	{ TS_FW_UPDATE_BOOT, ts_fw_update_boot_direct_func },
	{ TS_FW_UPDATE_SD, ts_fw_update_sd_direct_func },
	{ TS_READ_RAW_DATA, ts_read_rawdata },
	{ TS_READ_CALIBRATION_DATA, ts_read_calibration_data },
	{ TS_GET_CALIBRATION_INFO, ts_get_calibration_info },
	{ TS_OEM_INFO_SWITCH, ts_oem_info_switch_direct_func },
	{ TS_GET_CHIP_INFO, ts_get_chip_info_direct_func },
	{ TS_SET_INFO_FLAG, ts_set_info_flag_direct_func },
	{ TS_CALIBRATE_DEVICE, ts_calibrate_device_direct_func },
	{ TS_GLOVE_SWITCH, ts_glove_switch_direct_func },
	{ TS_CHECK_STATUS, ts_check_status_direct_func },
	{ TS_HOLSTER_SWITCH, ts_holster_switch_direct_func },
	{ TS_ROI_SWITCH, ts_roi_switch_direct_func },
	{ TS_TOUCH_SWITCH, ts_touch_switch_direct_func },
};
...
int ts_kit_proc_command_directly(struct ts_cmd_node *cmd)
{
	...
	for (i = 0; i < ARRAY_SIZE(ts_proc_command_directly_table); i++) {
		if (cmd->command == ts_proc_command_directly_table[i].cmd) {
			error = ts_proc_command_directly_table[i].func(cmd, &outcmd, cmd->sync);
			goto out;
		}
	}
...
static int ts_kit_init(void *data)
{
	...
	g_ts_kit_platform_data.ts_wake_lock = wakeup_source_register(&g_ts_kit_platform_data.ts_dev->dev, "ts_wake_lock");
#else
	g_ts_kit_platform_data.ts_wake_lock = wakeup_source_register("ts_wake_lock");
#endif
	if (!g_ts_kit_platform_data.ts_wake_lock) {
		TS_LOG_ERR("%s: failed to init wakelock\n", __func__);
		return	-EINVAL;
	}
	TS_LOG_INFO("%s: success to init wakelock\n", __func__);
...
err_out:
	atomic_set(&g_ts_kit_platform_data.state, TS_UNINIT);
	atomic_set(&g_ts_kit_platform_data.power_state, TS_UNINIT);
	wakeup_source_unregister(g_ts_kit_platform_data.ts_wake_lock);
	g_ts_kit_platform_data.ts_wake_lock = NULL;
out:
...
#if defined CONFIG_HUAWEI_DEVKIT_QCOM_3_0
