...
int atmel_get_rawdata_test(struct mxt_data *data)
{
	...
	if (0 == Data_count) {
		Data_count = 1;		/* 1, make sure the Data_ave is valid */
	}
...
int atmel_T37_fetch(struct mxt_data *data, u8 mode)
{
	...
	{
		tem_buf_size = matrix_add_size * sizeof(int) * 2;	/*total rawdata num: x+y+y, malloc enough size*/
		...
		T37_tem_buf_size = matrix_add_size * sizeof(u8) * 2 * 2;	/* total rawdata num :x+y+y, rawdata is short type, malloc enough size*/
		...
		for (i = 0; i < num_pages; i++) {
			...
			if(1 == i) {	// cp mode, page do not include rawdata
				;
			} else {
				...
				switch(i){
					case 0://page 0
						chunk_len = (size_t)matrix_add_size * 2;	/* rawdata is short type */
						break;
					case 2://page 2
						chunk_len = (size_t)(data->info->matrix_ysize) * 2;		/* rawdata is short type */
						break;
				}
		...
		/*total rawdata is :x+y+y, rawdat is short ,total byte:2x+4y */
		...
		T37_tem_buf_size = matrix_mul_size * sizeof(u8) * 2;	/* rawdata is short */
		T37_tem_buf = kzalloc(T37_tem_buf_size, GFP_KERNEL);
		if (!T37_tem_buf) {
			TS_LOG_ERR("alloc mem for T37_tem_buf failed\n");
			ret = -ENOMEM;
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	obuf = kzalloc(MXT_OBJ_BUF_SIZE, GFP_KERNEL);
	if (!obuf) {
					 ...
					 const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &i) == MXT_GET_ONE_PARA && i < MXT_VALID_DEVICE_CTRL) {
		if (i == 1) {
				      ...
				      const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &i) == MXT_GET_ONE_PARA && i < MXT_VALID_DEVICE_CTRL) {
		data->debug_enabled = (i == 1);	//1, enable
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%02x (GPIO 0x%02x)\n",
			 data->t19_msg[0], (data->t19_msg[0] >> 2) & 0xf);	/* bit2~bit5 is valid */
}
...
static int mxt_t19_command(struct mxt_data *data, bool enable, bool wait)
{
	...
	val[0] = MXT_T19_THIRD_VALUE;

	// IC need first set reg +3 for internal pull-up,then set reg
	ret = __atmel_write_reg(data, reg + 3, sizeof(val), val);
	if (ret)
		return ret;

	val[0] = MXT_T19_BASE_VALUE;
	...
	do {
		...
		if (ret)
			return ret;
	} while ((val[0] & 0x4) && (timeout_counter++ <= 100));//4,check bit2 state, 100 retry times
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%hhd", &cmd) == MXT_GET_ONE_PARA) {
		if (cmd == 0) {	// gpio disable
			data->alt_chip = 0;
			return count;
		} else if (cmd == 1) {	// gpio enable
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	offset += scnprintf(buf + offset, PAGE_SIZE, "%x %x %x %x %x %x\n",
			    ...
			    data->t25_msg[2],
			    data->t25_msg[3],
			    data->t25_msg[4], data->t25_msg[5]);	/*msg[0]:test status ,msg[1]~msg[5]:test result info */
	return offset;
}
...
int atmel_t25_selftest(struct mxt_data *data, int cmd)
{
	...
	data->t25_msg[0] = 0;	/* msg[0],test status*/
	data->t25_msg[5] = 0;   /* init 0 */
				      ...
				      const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x", &cmd) == MXT_GET_ONE_PARA) {
		if (mxt_t25_command(data, (u8) cmd, 1) == 0)
			return count;
				   ...
				   loff_t off, size_t count)
{
	...
	if (!data)
		return -ENODEV;
	if (NULL == filp || NULL == kobj || NULL == bin_attr || NULL == buf) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return -EINVAL;
	}
...
int atmel_mem_access_init(struct mxt_data *data)
{
	...
	data->mem_access_attr.attr.mode = S_IRUSR | S_IWUSR | S_IRGRP; // 0640
...
int atmel_debug_msg_init(struct mxt_data *data)
{
	...
	data->debug_msg_attr.attr.mode = S_IRUSR | S_IWUSR | S_IRGRP;	//0640
