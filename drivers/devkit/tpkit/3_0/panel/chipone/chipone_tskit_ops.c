...
#else /* CONFIG_HONOR_DEVKIT_MTK_3_0 */
...
#endif /* CONFIG_HONOR_DEVKIT_MTK_3_0 */
...
static int chipone_chip_init(void)
{
	...
	if (!cts_data->use_honor_charger_detect) {
	...
	struct ts_kit_device_data *data)
{
	...
	cts_data->use_honor_charger_detect =
		of_property_read_bool(of_node, "use-honor-charger-detect");
	TS_LOG_INFO(" - %-32s: %c\n", "Use honor charger detect",
		cts_data->use_honor_charger_detect ? 'Y' : 'N');
...
static int chipone_palm_switch(struct ts_palm_info *info)
{
	return 0;
}
...
struct ts_device_ops chipone_ts_device_ops = {
	...
	.chip_palm_switch = chipone_palm_switch,
};
...
static int __init chipone_ts_module_init(void)
{
	...
	if (root == NULL) {
		TS_LOG_ERR("Honor ts_kit of node NOT found in DTS\n");
		ret = -ENODEV;
		goto out;
	}
	...
	ret = honor_ts_chip_register(cts_data->tskit_data);
	if(ret) {
