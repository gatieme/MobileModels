		...
		u32 addr, const void *src, size_t len, int retry, int delay)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ret) {
			TS_LOG_ERR("Write rDEBUG_INTF len=5B failed %d",
					ret);
			return ret;
		}
	}
		...
		u32 addr, const void *src, size_t len, u32 crc, int retry)
{
	...
	do {
		...
		if ((ret = cts_calc_sram_crc(cts_dev, 0, len, &crc_sram)) != 0) {
			TS_LOG_ERR("Get CRC for sram writesb failed %d retries %d",
				ret, retries);
			continue;
		}
		...
		TS_LOG_ERR("Check CRC for sram writesb mismatch %x != %x retries %d",
...
{
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Writeb to fw reg 0x%04x under program mode", reg_addr);
		return -ENODEV;
	}
...
{
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Writew to fw reg 0x%04x under program mode", reg_addr);
		return -ENODEV;
	}
...
{
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Writel to fw reg 0x%04x under program mode", reg_addr);
		return -ENODEV;
	}
...
{
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Writesb to fw reg 0x%04x under program mode", reg_addr);
		return -ENODEV;
	}
...
static int icnl9911_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
	...
	TS_LOG_INFO("ICNL9911 %s access ddi reg", enable ? "enable" : "disable");

	ret = cts_hw_reg_readb(cts_dev, CTS_DEV_HW_REG_DDI_REG_CTRL, &access_flag);
	if (ret) {
		TS_LOG_ERR("Read HW_REG_DDI_REG_CTRL failed %d", ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Write HW_REG_DDI_REG_CTRL %02x failed %d", access_flag, ret);
		return ret;
	}

	ret = cts_hw_reg_writew(cts_dev, 0x3DFF0, enable ? 0x4BB4 : 0xB44B);
	if (ret) {
		TS_LOG_ERR("Write password failed %d");
...
static int icnl9911s_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
	...
	TS_LOG_INFO("ICNL9911S %s access ddi reg", enable ? "enable" : "disable");
		...
		u32 hwid, u16 fwid)
{
	...
	TS_LOG_INFO("Init hardware data hwid: %06x fwid: %04x", hwid, fwid);
...
int cts_get_lib_version(const struct cts_device *cts_dev, u16 *lib_version)
{
	...
	if (ret) {
		TS_LOG_ERR("Get fw lib main version failed %d", ret);
		return ret;
	}

	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_FW_LIB_SUB_VERSION, &sub_version);
	if (ret) {
		TS_LOG_ERR("Get fw lib sub version failed %d", ret);
		return ret;
	}
...
{
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Send command %u while chip in program mode", cmd);
		return -ENODEV;
	}
		...
		struct cts_device_touch_info *touch_info)
{
	...
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Get touch info in program mode");
		return -ENODEV;
	}

	if (cts_dev->rtdata.suspended) {
		TS_LOG_ERR("Get touch info while is suspended");
		return -ENODEV;
	}
		...
		void *param, size_t size)
{
	...
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Get panel parameter in program mode");
		return -ENODEV;
	}
		...
		const void *param, size_t size)
{
	...
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Set panel parameter in program mode");
		return -ENODEV;
	}
...
{
	TS_LOG_INFO("%s ESD protection",  enable ? "Enable" : "Disable");
...
int cts_enable_get_rawdata(const struct cts_device *cts_dev)
{
	...
	TS_LOG_INFO("Enable get touch data");
	...
	for (i = 0; i < 100; i++) {
		...
		if (ret) {
			TS_LOG_ERR("Get enable get touch data flag failed %d(%s)",
				ret, cts_strerror(ret));
		...
		if (enabled) {
			...
			TS_LOG_DEBUG("Enable get touch data flag NOT set, try again");
		}
...
int cts_disable_get_rawdata(const struct cts_device *cts_dev)
{
	...
	for (i = 0; i < 100; i++) {
		...
		if (enabled) {
			TS_LOG_DEBUG("Enable get touch data flag STILL set, try again");
		} else {
			return 0;
		}
...
int cts_get_rawdata(const struct cts_device *cts_dev, void *buf)
{
	...
	do {
		...
		if (ret) {
			TS_LOG_ERR("Read rawdata failed %d", ret);
		}
	...
	enum cts_work_mode work_mode, u32 flags, u16 addr, u8 data_width)
{
	...
	switch (work_mode) {
		...
		case CTS_WORK_MODE_SUSPEND:
			TS_LOG_ERR("Get baseline of work mode %d(%s) NOT supported",
				work_mode, cts_work_mode2str(work_mode));
			return -ENOTSUPP;
		default:
			TS_LOG_ERR("Get baseline of work mode %d invalid",
				work_mode);
			return -EINVAL;
	}
	...
	TS_LOG_INFO("Get baseline of work-mode: %d(%s), flags 0x%x, "
	...
	if (flags & CTS_GET_TOUCH_DATA_FLAG_ENABLE_GET_TOUCH_DATA_BEFORE) {
		...
		if (ret) {
			TS_LOG_ERR("Enable get touch data failed %d(%s)",
				ret, cts_strerror(ret));
			return ret;
		}
	}
	...
	TS_LOG_ERR("Wait data ready timeout");
	ret = -ETIMEDOUT;
...
read_baseline:
	if (flags & CTS_GET_TOUCH_DATA_FLAG_REMOVE_TOUCH_DATA_BORDER) {
		...
		if (buf == NULL) {
			TS_LOG_ERR("Alloc mem for baseline failed");
			ret = -ENOMEM;
	...
	if (ret) {
		TS_LOG_ERR("Read baseline from 0x%04x size %u failed %d",
			addr, buf_size, ret);
		goto free_mem;
	}
	...
	if (flags & CTS_GET_TOUCH_DATA_FLAG_REMOVE_TOUCH_DATA_BORDER) {
		...
		TS_LOG_DEBUG(" - Remove border rows & cols");
	...
	if (flags & CTS_GET_TOUCH_DATA_FLAG_FLIP_TOUCH_DATA) {
		if (cts_dev->fwdata.flip_x) {
			TS_LOG_DEBUG(" - Flip baseline on row");
			tsdata_flip_x(baseline, rows, cols);
		}
		if (cts_dev->fwdata.flip_y) {
			TS_LOG_DEBUG(" - Flip baseline on col");
...
clear_data_ready_flag:
	if (flags & CTS_GET_TOUCH_DATA_FLAG_CLEAR_DATA_READY) {
		...
		TS_LOG_DEBUG(" - Clear data ready flag");
		r = cts_clr_data_ready_flag(cts_dev);
		if (r) {
			TS_LOG_ERR("Clear data ready flag failed %d(%s)",
				r, cts_strerror(r));
		}
	}
		...
		u8 *boot_mode)
{
	...
	if (ret) {
		TS_LOG_ERR("Read boot mode failed %d", ret);
		return ret;
	}

	*boot_mode &= CTS_DEV_BOOT_MODE_MASK;

	TS_LOG_INFO("Curr dev boot mode: %u(%s)", *boot_mode,
		...
		u8 boot_mode)
{
	...
	TS_LOG_INFO("Set dev boot mode to %u(%s)", boot_mode,
		cts_dev_boot_mode2str(boot_mode));

	ret = cts_hw_reg_writeb_retry(cts_dev, CTS_DEV_HW_REG_BOOT_MODE,
		boot_mode, 5, 5);
	if (ret) {
		TS_LOG_ERR("Write hw register BOOT_MODE failed %d", ret);
		return ret;
	}
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Read firmware Lib version failed %d", ret);
	}
	TS_LOG_INFO("  %-24s: v%x.%x", "Fimrware lib verion",
		(u8)(fwdata->lib_version >> 8),
		(u8)(fwdata->lib_version));

	ret = cts_get_ddi_version(cts_dev, &fwdata->ddi_version);
	if (ret) {
		TS_LOG_ERR("Read ddi version failed %d", ret);
		return ret;
	}
	TS_LOG_INFO("  %-24s: %02x", "DDI init code verion", fwdata->ddi_version);
	...
	if (ret) {
		...
		if (fwdata->rows > cts_dev->hwdata->num_row) {
			TS_LOG_ERR("Read firmware num TX %u > device supported %u",
	...
	if (ret) {
		...
		if (fwdata->cols > cts_dev->hwdata->num_col) {
			TS_LOG_ERR("Read firmware num RX %u > device supported %u",
	...
	if (ret) {
		TS_LOG_ERR("Read firmware Raw dest value failed %d", ret);
		return ret;
	}
	TS_LOG_INFO("  %-24s: %d", "Raw target value", fwdata->rawdata_target);


	ret = cts_fw_reg_readb(cts_dev,
		CTS_DEVICE_FW_REG_ESD_PROTECTION, &fwdata->esd_method);
	if (ret) {
		TS_LOG_ERR("Read firmware Esd method failed %d", ret);
		return ret;
	}
	TS_LOG_INFO("  %-24s: %d", "Esd method", fwdata->esd_method);

#ifdef CONFIG_CTS_EARJACK_DETECT
	ret = cts_fw_reg_readb(cts_dev,
		CTS_DEVICE_FW_REG_EARJACK_DETECT_SUPP, &val);
	if (ret) {
		TS_LOG_ERR("Read firmware earjack detect support failed %d", ret);
		return ret;
	}
	fwdata->supp_headphone_cable_reject = !!(val & BIT(0));
	TS_LOG_INFO("  %-24s: %s", "Headphone cable reject",
...
void cts_show_fw_log(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Get i2c print buf len error");
		return;
	}
	if (len >= max_len) {
		len = max_len - 1;
	...
	if (ret) {
		TS_LOG_ERR("Get i2c print buf error");
		return;
	}
	data[len] = '\0';
...
int cts_suspend_device(struct cts_device *cts_dev)
{
	...
	if (cts_dev->rtdata.suspended) {
		TS_LOG_ERR("Suspend device while already suspended");
		return 0;
	}
	if (cts_dev->rtdata.program_mode) {
		...
		if (ret) {
			TS_LOG_ERR("Failed to exit program mode before suspend:%d", ret);
			return ret;
		}
	}
...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	if (cts_data->cached_firmware == NULL) {
		...
		if (cts_data->boot_firmware_filename[0] == '\0') {
			TS_LOG_ERR("Firmware filename is nul\n");
			goto err_set_program_mode;
		}
		cts_data->cached_firmware = cts_request_firmware_from_fs(
			&cts_data->cts_dev, cts_data->boot_firmware_filename);
		if(cts_data->cached_firmware == NULL) {
	...
	if (ret) {
		TS_LOG_ERR("Update firmware on boot failed %d(%s)",
			ret, cts_strerror(ret));
		goto err_set_program_mode;
	}
...
config_firmware:
	...
	ret = cts_send_command(cts_dev,
		(!!cts_data->roi_switch) ? CTS_CMD_ENABLE_NOI : CTS_CMD_DISABLE_NOI);
	if (ret) {
		TS_LOG_ERR("Send %s failed %d(%s)\n",
			(!!cts_data->roi_switch) ? "CMD_ENABLE_NOI" : "CMD_DISABLE_NOI",
			ret, cts_strerror(ret));
	}
...
int cts_enter_program_mode(struct cts_device *cts_dev)
{
	...
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Enter program mode while alredy in");
		//return 0;
	}
	...
	if (ret) {
		TS_LOG_ERR("Write magic number to i2c_dev: 0x%02x failed %d",
	...
	if (ret) {
		TS_LOG_ERR("Write i2c deglitch register failed\n");
		// FALL through
	}
#else
	cts_set_program_addr(cts_dev);
	...
	if (ret) {
		TS_LOG_ERR("Read BOOT_MODE failed %d", ret);
		return ret;
	}
...
#endif
	{
		TS_LOG_ERR("BOOT_MODE readback %u != I2C/SPI PROMGRAM mode", boot_mode);
		return -EFAULT;
	}
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
	...
	if (!cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Enter normal mode while already in");
		return 0;
	}
	...
	if (ret) {
		TS_LOG_ERR("Set BOOT_MODE to SRAM_BOOT failed %d(%s)",
			ret, cts_strerror(ret));
		// Fall through
	}
	...
	for(retries = 0; retries < 3; retries++) {
		ret = cts_get_dev_boot_mode(cts_dev, &boot_mode);
		if (ret) {
			TS_LOG_ERR("Get CURR_BOOT_MODE failed %d, try to do reset",
				ret, cts_strerror(ret));
			goto reset_device;
		}
		...
		if (boot_mode != CTS_DEV_BOOT_MODE_SRAM &&
			...
			TS_LOG_ERR("Curr boot mode %u(%s) != SRAM_BOOT/FLASH_BOOT",
...
reset_device:
	...
	TS_LOG_ERR("Enter normal mode failed, stay in program mode");
	goto stay_in_program_mode;

init_fwdata:
	ret = cts_init_fwdata(cts_dev);
	if (ret) {
		TS_LOG_ERR("Init firmware data failed %d(%s)",
			ret, cts_strerror(ret));
		return ret;
	}
...
int cts_start_device(struct cts_device *cts_dev)
{
	...
	if (cts_is_device_enabled(cts_dev)) {
		TS_LOG_ERR("Start device while already started");
		return 0;
	}
...
int cts_stop_device(struct cts_device *cts_dev)
{
	...
	if (!cts_is_device_enabled(cts_dev)) {
		TS_LOG_ERR("Stop device while halted");
		return 0;
	}

	if (cts_is_firmware_updating(cts_dev)) {
		TS_LOG_ERR("Stop device while firmware updating, please try again");
		return -EAGAIN;
	}
	...
	if (ret) {
		TS_LOG_ERR("Release all vkey failed %d", ret);
		return ret;
	}
...
int cts_get_fwid(struct cts_device *cts_dev, u16 *fwid)
{
	...
	*fwid = be16_to_cpu(*fwid);
	...
	if (!cts_is_fwid_valid(*fwid)) {
		TS_LOG_ERR("Get invalid firmware id %04x", *fwid);
		ret = -EINVAL;
		goto err_out;
	}
...
int cts_get_hwid(struct cts_device *cts_dev, u32 *hwid)
{
	...
	TS_LOG_INFO("Device hardware data not initialized, try to read from register");
	...
	*hwid &= 0XFFFFFFF0;
	...
	if (!cts_is_hwid_valid(*hwid)) {
		TS_LOG_ERR("Device hardware id %04x invalid", *hwid);
		ret = -EINVAL;
		goto err_out;
	}
...
#ifdef CONFIG_CTS_I2C_HOST
	if (!cts_plat_is_i2c_online(cts_dev->pdata, CTS_DEV_NORMAL_MODE_I2CADDR)) {
		TS_LOG_ERR("Normal mode i2c addr is offline");
	} else
#else
	if (!cts_plat_is_normal_mode(cts_dev->pdata)) {
		TS_LOG_ERR("Normal mode spi addr is offline");
	} else
#endif
	{
		...
		if (ret) {
			TS_LOG_ERR("Get firmware id failed %d, retries %d", ret, retries);
		} else {
			ret = cts_fw_reg_readw_retry(cts_dev,
			...
			if (ret) {
				...
				TS_LOG_INFO("Device firmware version: %04x", device_fw_ver);
			}
			goto init_hwdata;
		}
	}
...
init_hwdata:
	...
	if (ret) {
		TS_LOG_ERR("Device hwid: %06x fwid: %04x not found", hwid, fwid);
		return -ENODEV;
	}
		...
		void *gesture_info, bool trace_point)
{
	...
	if (cts_dev->rtdata.program_mode) {
		TS_LOG_ERR("Get gesture info in program mode");
		return -ENODEV;
	}

	if (!cts_dev->rtdata.suspended) {
		TS_LOG_ERR("Get gesture info while not suspended");
		return -ENODEV;
	}

	if (!cts_dev->rtdata.gesture_wakeup_enabled) {
		TS_LOG_ERR("Get gesture info while gesture wakeup not enabled");
		return -ENODEV;
	}
	...
	if(ret) {
		TS_LOG_ERR("Read gesture info header failed %d", ret);
		return ret;
	}
	...
	if (trace_point) {
		...
		if(ret) {
			TS_LOG_ERR("Read gesture trace points failed %d", ret);
			return ret;
		}
	}
...
static void cts_esd_protection_work(struct work_struct *work)
{
	...
	TS_LOG_INFO("ESD protection work");
...
#endif
	{
		...
		if ((cts_data->esd_check_fail_cnt % 2) == 0) {
			TS_LOG_ERR("ESD protection read normal mode failed, reset chip!");
			ret = cts_plat_reset_device(cts_data->pdata);
			if (ret) {
				TS_LOG_ERR("ESD protection reset chip failed %d", ret);
			}
		}
	} else {
	...
	if (cts_data->esd_check_fail_cnt >= CFG_CTS_ESD_FAILED_CONFIRM_CNT) {
		TS_LOG_ERR("ESD protection check failed, update firmware!!!");
		cts_stop_device_esdrecover(&cts_data->cts_dev);

		if (cts_data->cached_firmware == NULL) {
			TS_LOG_INFO("Firmware is NULL, re-request from system");
			if (cts_data->boot_firmware_filename[0] == '\0') {
				TS_LOG_ERR("Firmware filename is nul, nothing can be done");
				goto unlock_device;
			}
			cts_data->cached_firmware = cts_request_firmware_from_fs(
				&cts_data->cts_dev, cts_data->boot_firmware_filename);
		...
		ret = cts_update_firmware(&cts_data->cts_dev,
			cts_data->cached_firmware, false);
		if (ret) {
			TS_LOG_ERR("ESD recovery update firmware failed %d", ret);
			goto unlock_device;
		}
	...
	schedule_delayed_work(&cts_data->esd_work,
...
{
	if (!cts_data->esd_enabled) {
		TS_LOG_INFO("ESD protection enable");
...
{
	if (cts_data->esd_enabled) {
		TS_LOG_INFO("ESD protection disable");
...
{
	TS_LOG_INFO("Init ESD protection");
...
{
	TS_LOG_INFO("De-Init ESD protection");
...
{
	TS_LOG_INFO("Enter glove mode");

	ret = cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_GLOVE_MODE, 1);
	if (ret) {
		TS_LOG_ERR("Enable Glove mode err");
	}
	else {
...
bool cts_is_charger_exist(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Get charger state failed %d", ret);
	}
...
int cts_set_dev_charger_attached(struct cts_device *cts_dev, bool attached)
{
	...
	TS_LOG_INFO("Set dev charger %s", attached ? "ATTACHED" : "DETATCHED");
	ret = cts_send_command(cts_dev,
		attached ? CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
	if (ret) {
		TS_LOG_ERR("Send CMD_CHARGER_%s failed %d",
...
bool cts_is_earjack_exist(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Get earjack state failed %d", ret);
	}
...
int cts_set_dev_earjack_attached(struct cts_device *cts_dev, bool attached)
{
	...
	TS_LOG_INFO("Set dev earjack %s", attached ? "ATTACHED" : "DETATCHED");
	ret = cts_send_command(cts_dev,
		attached ? CTS_CMD_EARJACK_ATTACHED : CTS_CMD_EARJACK_DETACHED);
	if (ret) {
		TS_LOG_ERR("Send CMD_EARJACK_%s failed %d",
			attached ? "ATTACHED" : "DETACHED", ret);
	}
...
int cts_enable_fw_log_redirect(struct cts_device *cts_dev)
{
	...
	TS_LOG_INFO("Fw log redirect enable");
	ret = cts_send_command(cts_dev, CTS_CMD_ENABLE_FW_LOG_REDIRECT);
	if (ret) {
		TS_LOG_ERR("Send CTS_CMD_ENABLE_FW_LOG_REDIRECT failed %d", ret);
	} else {
...
int cts_disable_fw_log_redirect(struct cts_device *cts_dev)
{
	...
	TS_LOG_INFO("Fw log redirect disable");
	ret = cts_send_command(cts_dev, CTS_CMD_DISABLE_FW_LOG_REDIRECT);
	if (ret) {
		TS_LOG_ERR("Send CTS_CMD_DISABLE_FW_LOG_REDIRECT failed %d", ret);
	} else {
		cts_dev->rtdata.fw_log_redirect_enabled = false;
	}
	return ret;
...
int cts_fw_log_show_finish(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Send CTS_CMD_FW_LOG_SHOW_FINISH failed %d", ret);
	}
...
int cts_get_compensate_cap(struct cts_device *cts_dev, u8 *cap)
{
	...
	if (cts_dev == NULL || cap == NULL) {
		TS_LOG_ERR("Get compensate cap with cts_dev(%p) or cap(%p) = NULL",
			cts_dev, cap);
		return -EINVAL;
	}
	...
	if (auto_calib_comp_cap_enabled) {
		...
		TS_LOG_INFO(" - Wait auto calib comp cap done...");
		...
		do {
			...
			if (ret) {
				TS_LOG_ERR("Get auto calib comp cap done failed %d", ret);
			} else {
		...
		TS_LOG_ERR("Wait auto calib comp cap done timeout");
		return -EFAULT;
	}

enable_read_compensate_cap:
	TS_LOG_INFO(" - Enable read comp cap");
	ret = cts_send_command(cts_dev, CTS_CMD_ENABLE_READ_CNEG);
	if (ret) {
		TS_LOG_ERR("Enable read comp cap failed %d",ret);
		return ret;
	}

	/* Wait compensate cap ready */
	TS_LOG_INFO(" - Wait comp cap ready...");
	i = 0;
	do {
		u8 ready;
		...
		if (ret) {
			TS_LOG_ERR("Read comp cap ready failed %d", ret);
		} else {
			if (ready) {
	...
	TS_LOG_ERR("Wait comp cap ready timeout");
	ret = -ETIMEDOUT;
	goto disable_read_comp_cap;

read_compensate_cap:
	/* Use hardware row & col here */
	TS_LOG_INFO(" - Read comp cap");
	ret = cts_fw_reg_readsb_delay_idle(cts_dev,
		...
		500);
	if (ret) {
		TS_LOG_ERR("Read comp cap failed %d",ret);
		goto disable_read_comp_cap;
	}

	if (cts_dev->fwdata.flip_x) {
		TS_LOG_INFO(" - Flip comp cap on row");
		flip_comp_cap_on_row(cap,
			cts_dev->fwdata.rows, cts_dev->fwdata.cols);
	}
	if (cts_dev->fwdata.flip_y) {
		TS_LOG_INFO(" - Flip comp cap on col");
		flip_comp_cap_on_col(cap,
			cts_dev->fwdata.rows, cts_dev->fwdata.cols);
	}

disable_read_comp_cap:
	TS_LOG_INFO(" - Disable read comp cap");
	i = 0;
	do {
		int r;
		u8  ready;
		...
		if (r) {
			TS_LOG_ERR("Send cmd DISABLE_READ_CNEG failed %d", r);
			continue;
		}
		...
		r = cts_fw_reg_readb(cts_dev,
				CTS_DEVICE_FW_REG_COMPENSATE_CAP_READY, &ready);
		if (r) {
			TS_LOG_ERR("Re-Check comp cap ready failed %d", r);
			continue;
		}

		if (ready) {
			TS_LOG_ERR("Comp cap ready is still set");
			continue;
		} else {
			return ret;
		}
	...
	TS_LOG_ERR("Disable read comp cap failed, try to do reset!");
	...
	if (cts_is_glove_enabled(cts_dev)) {
		...
		if (r) {
			TS_LOG_ERR("Enter dev glove mode failed %d", r);
		}
	}
#endif /* CONFIG_CTS_GLOVE */
	...
	if (cts_is_fw_log_redirect(cts_dev)) {
		...
		if (r) {
			TS_LOG_ERR("Enable fw log redirect failed %d", r);
		}
	}
#endif /* CONFIG_CTS_GLOVE */
...
#define START_BANNER \
	...
	TS_LOG_INFO("Start driver log redirect");

	cts_log_to_file_level = log_level;

	if (log_buffer && log_buf_size) {
		TS_LOG_INFO(" - Start driver log to buffer: %p size: %d level: %d",
	...
	if (filepath && filepath[0]) {
		TS_LOG_INFO(" - Start driver log to file  : '%s' level: %d",
			filepath, log_level);
		...
		if (IS_ERR(cts_log_filp)) {
			...
			cts_log_filp = NULL;
			TS_LOG_ERR("Open file '%s' for driver log failed %d",
				filepath, ret);
		} else {
...
#define END_BANNER \
	...
	TS_LOG_INFO("Stop driver log redirect");

	if (cts_log_filp) {
		int ret;

		TS_LOG_INFO(" - Stop driver log to file");

		cts_write_file(cts_log_filp, END_BANNER, strlen(END_BANNER));
		ret = filp_close(cts_log_filp, NULL);
		if (ret) {
			TS_LOG_ERR("Close driver log file failed %d", ret);
		}
		cts_log_filp = NULL;
	}

	if (cts_log_buffer) {
		TS_LOG_INFO(" - Stop driver log to buffer");
