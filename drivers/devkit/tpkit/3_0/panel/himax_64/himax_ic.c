...
uint8_t *honor_project_id;
static uint32_t honor_project_id_len;
uint8_t *honor_cg_color;
static uint32_t honor_cg_color_len;
uint8_t *honor_sensor_id;
static uint32_t honor_sensor_id_len;
...
static int himax_knuckle(int hx_touch_info_size, uint8_t *buf)
{
	...
	if (buf[hx_touch_info_size + IDX_PKG_NUM] == 0x02) {
		/* report to the upper layer or assign to honor's struct */
...
static int touch_driver_projectid_read(uint8_t *buffer)
{
	...
	/* project id */
	if (type == DATA_0) {
		honor_project_id = kzalloc((len + DATA_1) *
	...
	if (len <= INFO_PAGE_LENGTH - DATA_8)
		memcpy(honor_project_id, buffer + DATA_8, len);
	honor_project_id_len = len;
	TS_LOG_INFO("%s: project id: ", __func__);
	if (type == DATA_0) {
		honor_project_id[len] = '\0';
		TS_LOG_INFO("%s", honor_project_id);
	} else {
		for (j = 0; j < len; j++)
			TS_LOG_INFO("0x%02X", *(honor_project_id + j));
	}
	TS_LOG_INFO("\n");
...
static int touch_driver_tpcolor_read(uint8_t *buffer)
{
	...
	/* CG color */
	if (type == DATA_0) {
		honor_cg_color = kzalloc((len + DATA_1) *
			 sizeof(uint8_t), GFP_KERNEL);
		if (!honor_cg_color) {
			TS_LOG_ERR("%s: Memory allocate FAIL!\n", __func__);
			return -ENOMEM;
	...
	if (len <= INFO_PAGE_LENGTH - DATA_8)
		memcpy(honor_cg_color, buffer + DATA_8, len);
	honor_cg_color_len = len;
	TS_LOG_INFO("%s: cg_color: ", __func__);
	if (type == DATA_0) {
		honor_cg_color[len] = '\0';
		TS_LOG_INFO("%s", honor_cg_color);
	} else {
		for (j = 0; j < len; j++)
			TS_LOG_INFO("0x%02X",
				*(honor_cg_color + j));
...
{
	memcpy(ts->color_id, honor_cg_color,
		sizeof(ts->color_id));
	if (ts->support_get_tp_color) {
	...
	if (ts->support_read_projectid) {
		memcpy(ts->project_id, honor_project_id,
			HIMAX_ACTUAL_PROJECTID_LEN);
		ts->project_id[HIMAX_ACTUAL_PROJECTID_LEN] = '\0';
		memcpy(himax_nc_project_id, honor_project_id,
			HIMAX_ACTUAL_PROJECTID_LEN);
...
static int __init himax_module_init(void)
{
	...
	if (!root) {
		TS_LOG_ERR("[HXTP]honor_ts, find_compatible_node honor,ts_kit error\n");
		err = -EINVAL;
		goto out;
	}
	/* find the chip node */
	...
	err = honor_ts_chip_register(g_himax_nc_ts_data->tskit_himax_data);
	if (err) {
