...
#define ADDR_MODE_SWITCH_HX83112 0x10007F04
...
// for himax debug
#define RAWDATA_TPYE_NONE 0x00
#define RAWDATA_TPYE_BASELINE 0x08
#define RAWDATA_TPYE_IIR_DIFF 0x09
#define RAWDATA_TPYE_DC 0x0A
#define RAWDATA_TPYE_BASELINE2 0x0B
#define RAWDATA_TPYE_PEN_IIR 0x12
#define RAWDATA_TPYE_PEN_DC 0x13
#define RAWDATA_DIAG_CMD_NONE 0
#define RAWDATA_DIAG_ADC_CALIBRATION 4
#define RAWDATA_DIAG_CMD_BASELINE 8
#define USER_RAWDATA_CMD1 0x01
#define USER_RAWDATA_CMD2 0x02
#define USER_RAWDATA_CMD3 0x03
#define USER_RAWDATA_CMD4 0x04
#define USER_RAWDATA_CMD7 0x07
#define USER_DIAG_CMD1 1
#define USER_DIAG_CMD3 3
#define USER_DIAG_CMD7 7
#define USER_DIAG_CMD8 8
#define DSRAM_TYPE0 0
#define DSRAM_TYPE8 8
#define USER_SEPARATION_CHAR 0x0A
#define USER_RAWDATA_CMD_MIN 0x00
#define USER_RAWDATA_CMD_MAX 0x08
#define USER_STORAGE_CMD_MIN 0x00
#define USER_STORAGE_CMD_MAX 0x08
#define BUFF_NUM_TEN 10
#define SEQ_RETRY_TIMES 128
#define DIAG_NUM_16 16
#define DIAG_NUM_15 15
...
#define RESET_LOW_TIME 30
#define RESET_HIGH_TIME 30
...
struct himax_ts_data {
	...
	uint8_t x_channel;
	uint8_t y_channel;
	uint8_t use_screen_res; // 1:display resulotion;0:touch resulotion
	int vendor_fw_ver;
	int vendor_config_ver;
	uint32_t vendor_cid_maj_ver;
	uint32_t vendor_cid_min_ver;
	int vendor_panel_ver;
	int vendor_sensor_id;
	uint8_t power_support;
	uint8_t rst_support;
	uint8_t gesture_need_lcd_reset;
...
void touch_driver_sense_on(uint8_t flashmode);
	...
	extern int16_t *hx_nc_get_mutual_buffer(void);
	extern int16_t *hx_nc_get_self_buffer(void);
	extern int16_t hx_nc_get_diag_command(void);
	extern int16_t hx_nc_get_x_channel(void);
	extern int16_t hx_nc_get_y_channel(void);
	extern void hx_nc_set_mutual_buffer(void);
	extern void hx_nc_set_mutual_new_buffer(void);
	extern void hx_nc_set_mutual_old_buffer(void);
	extern void hx_nc_free_mutual_buffer(void);
	extern void hx_nc_free_mutual_new_buffer(void);
	extern void hx_nc_free_mutual_old_buffer(void);
	extern void hx_nc_free_self_buffer(void);
	extern void hx_nc_set_x_channel(uint8_t x);
	extern void hx_nc_set_y_channel(uint8_t y);
#endif

#ifdef HX_TP_SYS_FLASH_DUMP
	extern bool  hx_nc_get_flash_dump_going(void);
	extern void hx_nc_set_flash_buffer(void);
	extern void hx_nc_free_flash_buffer(void);
	extern void hx_nc_set_sys_operation(uint8_t operation);
...
extern void himax_nc_get_dsram_data(uint8_t* info_data, size_t count);
...
extern void himax_nc_flash_dump_func(uint8_t local_flash_command,
	int flash_size, uint8_t *flash_buffer);
// need to check in detect
extern int himax_nc_loadsensorconfig(void);
extern int hx_nc_fts_ctpm_fw_upgrade_with_fs(unsigned char *fw,
	int len, bool change_iref);
extern uint8_t himax_nc_calculate_checksum(bool change_iref);
...
extern int himax_rw_reg_reformat(uint32_t reg_data, uint8_t *data_buf);
extern int himax_rw_reg_reformat_com(uint32_t reg_addr, uint32_t reg_data,
