...
int ilitek_parse_dts(struct device_node *np)
{
	...
	/*add new roi with fw start*/
	ret = of_property_read_u32(dts_np, ILITEK_DTS_HEAD_ROI, &ilits->head_roi_supported);
	if (ret) {
		TS_LOG_INFO("get head_roi_supported failed, use default: %d\n",
			ILITEK_DTS_DEF_HEAD_ROI);
		ilits->head_roi_supported = ILITEK_DTS_DEF_HEAD_ROI;
	}
	TS_LOG_INFO("HEAD_ROI = %d\n", ilits->head_roi_supported);
	/*add new roi with fw end*/
