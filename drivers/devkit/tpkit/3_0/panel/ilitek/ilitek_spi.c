...
static int ili_spi_pll_clk_wakeup(void)
{
	int index = 0;
	u8 wdata[32] = {0};
	u8 wakeup[12] = {0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3};
	...
	case SPI_WRITE:
		...
		*/
		if (atomic_read(&ilits->mp_stat) && wdata[index] == P5_X_SET_CDC_INIT) {
			...
			wdata[wlen] = checksum;
			wlen++;
			wdata[1] = ((u32)(wlen - index)) >> 8;
			wdata[2] = ((u32)(wlen - index)) & 0xff;
