/*
 * Copyright (C) 2006-2020 ILITEK TECHNOLOGY CORP.
 *
...
#if defined(CONFIG_HONOR_DEVKIT_QCOM)
...
static struct completion mstar_roi_data_done;
static u8 mstar_roi_data_staled;
...
int mstar_hw_reset(void)
{
	...
	ret = pinctrl_select_state(tskit_mstar_data->pctrl,
		tskit_mstar_data->pinctrl_state_reset_high);
	if (ret) {
	...
	ret = pinctrl_select_state(tskit_mstar_data->pctrl,
		tskit_mstar_data->pinctrl_state_reset_low);
	if (ret) {
		TS_LOG_ERR("%s:gpio direction output to high fail, ret=%d\n",__func__, ret);
		ret = -1;
		goto out;
...
int mstar_get_customer_fw_ver(char *ppVersion)
{
	...
	/* ppVersion is pointer to an array of up to 32 bytes */
	snprintf(ppVersion, MSTAR_CUST_FW_VER_LEN, "%d%d", pMajor, pMinor);
   ...
   adapter = i2c_get_adapter(tskit_mstar_data->mstar_chip_data->ts_platform_data->bops->bus_id);
    ...
    if (tskit_mstar_data->apk_info->debug_node_open) {
    	...
    	if (tskit_mstar_data->apk_info->debug_data_frame > 1023) {
			TS_LOG_DEBUG("%s: debug_data_frame = %d > 1024\n",
				__func__,
				tskit_mstar_data->apk_info->debug_data_frame);
...
static int mstar_get_gesture_code(u8 *pPacket, struct ts_fingers *info)
{
	...
	if (nReportGestureType != 0) {
		__pm_wakeup_event(&tskit_mstar_data->mstar_chip_data->ts_platform_data->ts_wake_lock, jiffies_to_msecs(5 * HZ));
...
static s32 mstar_mutual_parse_packet(u8 * pPacket, u16 nLength, MutualTouchInfo_t * pInfo, struct ts_fingers *info)
{
    ...
    if (tskit_mstar_data->roi_enabled) {
		...
		if ((pre_finger_num != touch_count) && (touch_count <= 2) &&
			(touch_count > 0))
			/*
			 * roi_data should be refreshed. But that takes too
			 * long (about 2ms on i2c bus), we delay this work to
			 * function chip_work_after_input. It will be called
			 * after input report.
			 */
			mstar_roi_data_staled = 1;
    }
...
static struct ts_roi_info *mstar_get_roi_info(
...
static unsigned char *mstar_roi_rawdata(void)
{
	...
	if (tskit_mstar_data->roi_enabled) {
		 ...
		 */
		if (mstar_roi_data_staled) {
			if (!wait_for_completion_interruptible_timeout(
				&mstar_roi_data_done, msecs_to_jiffies(
				ROI_DATA_WAIT_TIMEOUT))) {
				TS_LOG_INFO("%s:wait roi data timeout\n",
					__func__);
				/*
				 * Never wait again if data
				 * refreshing gets timeout.
				 */
				mstar_roi_data_staled = 0;
				memset(tskit_mstar_data->roi_data, 0,
					sizeof(tskit_mstar_data->roi_data));
			}
		}
	}
	return tskit_mstar_data->roi_data;
#else
	return NULL;
#endif
}

static void mstar_ts_work_after_input_kit(void)
{
	struct ts_roi_info *roi_info =
		mstar_get_roi_info(tskit_mstar_data->mstar_chip_data);

#ifdef MSTAR_ROI_ENABLE
	if (tskit_mstar_data->roi_enabled && roi_info->roi_supported) {
		/* roi_data is up to date now */
		if (mstar_roi_data_staled == 0)
			return;
		/*
		 ...
		 */
		reinit_completion(&mstar_roi_data_done);
		mstar_get_roi_data_form_fw();

		mstar_roi_data_staled = 0;
		/* If anyone has been blocked by us, wake it up */
		complete_all(&mstar_roi_data_done);
	}
#endif
	return NULL;
}
...
static int mstar_set_info_flag(struct ts_kit_platform_data *info)
{
    return NO_ERR;
}
...
static int mstar_read_flash_28xx(u32 nAddr, int nBlockType, int nLength, u8 * pFlashData)
{
    ...
    for (addr_star = nAddr; addr_star < addr_end; addr_star += addr_step) {
		...
		TS_LOG_DEBUG("  %s() _28xx_addr:0x%x addr_star:0x%x \
			addr_end:%x nLength:%d pFlashData:%pK \n",
			__func__, _28xx_addr, addr_star, addr_end,
			nLength, pFlashData);
...
static int mstar_status_resume(void)
{
	...
	if (info->roi_info.roi_supported) {
		...
		if (retval < 0)
			TS_LOG_ERR("%s, mstar_set_roi_switch failed\n",
				   __func__);
	}
...
static int mstar_get_lcd_panel_info(void)
{
...
static void mstar_power_on_gpio_set(void)
{
	int rc;
#ifdef CONFIG_HONOR_DEVKIT_MTK_3_0
	rc = pinctrl_select_state(tskit_mstar_data->pctrl,
		tskit_mstar_data->pinctrl_state_as_int);
	if (rc < 0)
		TS_LOG_ERR("%s: gpio_direction_input fail\n", __func__);
	pinctrl_select_state(tskit_mstar_data->pctrl,
		tskit_mstar_data->pinctrl_state_reset_high);
#else
	rc = gpio_direction_input(
		tskit_mstar_data->mstar_chip_data->ts_platform_data->irq_gpio);
	if (rc < 0)
		TS_LOG_ERR("%s: gpio_direction_input fail\n", __func__);
...
static int mstar_power_on(void)
{
	...
	if(tskit_mstar_data->self_ctrl_power == 1){
		...
		if (1 == tskit_mstar_data->mstar_chip_data->vci_regulator_type) {
			...
			gpio_direction_output(tskit_mstar_data->mstar_chip_data->vci_gpio_ctrl, 1);
		}
		...
		if(0 == tskit_mstar_data->mstar_chip_data->vddio_gpio_type) {
			...
			gpio_direction_output(tskit_mstar_data->mstar_chip_data->vddio_gpio_ctrl, 1);
		}
	} else{
		TS_LOG_INFO("%s, power control by LCD, nothing to do\n",__func__);
	}
...
static int mstar_power_off(void)
{
	...
	if(tskit_mstar_data->self_ctrl_power == 1){
		...
		if (1 == tskit_mstar_data->mstar_chip_data->vci_regulator_type) {
			...
			gpio_direction_output(tskit_mstar_data->mstar_chip_data->vci_gpio_ctrl, 0);
		}
		...
		if(0 == tskit_mstar_data->mstar_chip_data->vddio_gpio_type) {
			...
			gpio_direction_output(tskit_mstar_data->mstar_chip_data->vddio_gpio_ctrl, 0);
		}
	}
	else{
		TS_LOG_INFO("%s, power control by LCD, nothing to do\n",__func__);
...
static int mstar_pinctrl_init(void)
{
	...
	tskit_mstar_data->pins_idle =
	    pinctrl_lookup_state(tskit_mstar_data->pctrl, "idle");
...
#else
     tskit_mstar_data->pinctrl_state_reset_high= pinctrl_lookup_state(tskit_mstar_data->pctrl, PINCTRL_STATE_RESET_HIGH);
    if (IS_ERR_OR_NULL(tskit_mstar_data->pinctrl_state_reset_high))
    {
		TS_LOG_ERR("Can not lookup %s pinstate \n",PINCTRL_STATE_RESET_HIGH);
		ret = -EINVAL;
		goto err_pinctrl_put;
    }
    tskit_mstar_data->pinctrl_state_reset_low= pinctrl_lookup_state(tskit_mstar_data->pctrl, PINCTRL_STATE_RESET_LOW);
    if (IS_ERR_OR_NULL(tskit_mstar_data->pinctrl_state_reset_low))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_RESET_LOW);
		ret = -EINVAL;
		goto err_pinctrl_put;
    }
    tskit_mstar_data->pinctrl_state_as_int= pinctrl_lookup_state(tskit_mstar_data->pctrl, PINCTRL_STATE_AS_INT);
    if (IS_ERR_OR_NULL(tskit_mstar_data->pinctrl_state_as_int))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_AS_INT);
		ret = -EINVAL;
		goto err_pinctrl_put;
    }
    tskit_mstar_data->pinctrl_state_int_high= pinctrl_lookup_state(tskit_mstar_data->pctrl,PINCTRL_STATE_INT_HIGH);
    if (IS_ERR_OR_NULL(tskit_mstar_data->pinctrl_state_int_high))
    {
		TS_LOG_ERR("-Can not lookup %s pinstate \n",PINCTRL_STATE_INT_HIGH);
		ret = -EINVAL;
		goto err_pinctrl_put;
    }
    tskit_mstar_data->pinctrl_state_int_low= pinctrl_lookup_state(tskit_mstar_data->pctrl,PINCTRL_STATE_INT_LOW);
    if (IS_ERR_OR_NULL(tskit_mstar_data->pinctrl_state_int_low))
    {
	...
	ret = pinctrl_select_state(tskit_mstar_data->pctrl, tskit_mstar_data->pinctrl_state_as_int);
	if (ret < 0){
...
static void mstar_pinctrl_release(void)
{
	...
	tskit_mstar_data->pctrl = NULL;
	tskit_mstar_data->pins_default = NULL;
	tskit_mstar_data->pins_idle = NULL;

#ifdef CONFIG_HONOR_DEVKIT_MTK_3_0
	tskit_mstar_data->pinctrl_state_reset_high = NULL;
	tskit_mstar_data->pinctrl_state_int_high = NULL;
	tskit_mstar_data->pinctrl_state_int_low = NULL;
	tskit_mstar_data->pinctrl_state_reset_low = NULL;
	tskit_mstar_data->pinctrl_state_as_int = NULL;
#endif
...
static int mstar_chip_detect(struct ts_kit_platform_data *platform_data)
{
   ...
   TS_LOG_ERR("%s:mstar_chip_detect (0)\n", __func__);
   ...
   TS_LOG_ERR("%s:mstar_chip_detect (1)\n", __func__);
   ...
   TS_LOG_ERR("%s:mstar_chip_detect (2)\n", __func__);
    ...
    pinctrl_select_state(tskit_mstar_data->pctrl,
    	tskit_mstar_data->pinctrl_state_reset_low);
    pinctrl_select_state(tskit_mstar_data->pctrl,
    	tskit_mstar_data->pinctrl_state_int_low);
    msleep(50);
	...
	roi_info = mstar_get_roi_info(tskit_mstar_data->mstar_chip_data);
	if (roi_info->roi_supported) {
		TS_LOG_INFO("%s: init roi completion success.\n", __func__);
		init_completion(&mstar_roi_data_done);
	}
...
static int mstar_chip_get_info(struct ts_chip_info_param *info)
{
	struct ts_kit_device_data *chip_data =
		tskit_mstar_data->mstar_chip_data;

	if (!info || !chip_data) {
		TS_LOG_ERR("%s:info or chip_data is Null\n", __func__);
		return -EINVAL;
	}

	if (chip_data->ts_platform_data->hide_plain_id) {
		snprintf(info->ic_vendor, sizeof(info->ic_vendor), "%s-%s",
			tskit_mstar_data->project_id,
			tskit_mstar_data->fw_data.cust_ver);
	} else {
		snprintf(info->fw_vendor, sizeof(info->fw_vendor), "%s",
			tskit_mstar_data->fw_data.cust_ver);
		snprintf(info->mod_vendor, sizeof(info->mod_vendor), "%s",
			chip_data->module_name);
		snprintf(info->ic_vendor, sizeof(info->ic_vendor), "%s-%s",
			MSTAR_CHIP_NAME, tskit_mstar_data->project_id);
	}

	TS_LOG_DEBUG("%s:fw_vendor = %s\n", __func__, info->fw_vendor);
...
struct ts_device_ops ts_mstar_ops = {
	...
	.chip_work_after_input = mstar_ts_work_after_input_kit,
#endif
...
static int __init mstar_ts_module_init(void)
{
    ...
    if (!tskit_mstar_data->mstar_chip_data ) {
    	TS_LOG_ERR("Failed to alloc mem for struct mstar_chip_data\n");
       ret =  -ENOMEM;
       goto error_exit;
    }
    ...
    ret = honor_ts_chip_register(tskit_mstar_data->mstar_chip_data);
    if (ret) {
        TS_LOG_ERR("%s:chip register fail, ret=%d\n", __func__, ret);
        goto error_exit;
    }

    TS_LOG_INFO("%s: mstar register successfully\n", __func__);
    return NO_ERR;
