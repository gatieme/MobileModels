...
#else
	snprintf(file_path, sizeof(file_path)-ONE_SIZE, "/odm/etc/firmware/ts/%s_short_open.bin", novatek_kit_project_id);
#endif
...
static int nvt_get_threshold_from_csvfile(uint64_t columns, uint64_t rows, char* target_name, struct get_csv_data *data)
{
	...
	if (CSV_PRODUCT_SYSTEM == nvt_ts->csvfile_use_system) {
		...
		snprintf(file_path, sizeof(file_path)-ONE_SIZE, "/odm/etc/firmware/ts/%s", file_name);
	}
	else{
	    return FAIL;
	}
...
static void nvt_kit_open_test(struct ts_rawdata_info *info, uint8_t x_channel_size, uint8_t y_channel_size)
{
	...
	if (open_ret != NO_ERR) {
		...
		if (nvt_ts->open_test_by_fw > NO_ERR) {
			...
			TestResult_Open = nvt_rawdata_up_low(RawData_Open, RecordResult_Open, X_Channel, Y_Channel,
...
/*******************************************************
Description:
	Novatek touchscreen selftest function for honor_touchscreen.
...
int32_t nvt_kit_selftest(struct ts_rawdata_info *info)
{
	...
	if(nvt_ts->criteria_threshold_flag){
		...
		info->hybrid_buff[0] = X_Channel;
		info->hybrid_buff[1] = Y_Channel;
		info->used_size = X_Channel * Y_Channel * 5  + CHANNEL_NUM;//4:rawdata+diff+short+open
