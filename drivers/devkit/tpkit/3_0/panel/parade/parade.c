...
/*
 * this function is not completed, need to first understand Honor's
		...
		struct parade_touch *tch, int num_cur_tch, struct ts_fingers *info)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		switch (tch->abs[PT_TCH_E]) { // PATCH: Honor does not use our event, they use x=0 or y=0 as this id lift off event
			case PT_EV_LIFTOFF:
...
static int parade_need_fw_update(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR("%s: Fail request firmware\n", __func__);
#ifdef CONFIG_HONOR_DSM
		tskit_parade_data->parade_chip_data->ts_platform_data->dsm_info.constraints_update_status = FWU_REQUEST_FW_FAIL;
#endif /* CONFIG_HONOR_DSM */
		return NO_UPDATA;
	}
...
static int __init parade_module_init(void)
{
	...
	error = honor_ts_chip_register(tskit_parade_data->parade_chip_data);
	if (error) {
