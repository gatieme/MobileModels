...
#define SENSOR_SLEEP_BIT0 (1 << 0)
#define SENSOR_SLEEP_BIT1 (1 << 1)
...
#define SYNAPTICS_RMI4_BARCODE_INFO_SIZE  (3*16)
#define SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_SIZE (10*16)
#define SYNAPTICS_RMI4_BARCODE_INFO_LEN  39
#define SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_LEN 160
#define GAMMA_DATA_HEAD_0 71
#define GAMMA_DATA_HEAD_1 70


#define SYNAPTICS_RMI4_BARCODE_START_OFFSET  2
#define SYNAPTICS_RMI4_BARCODE_NUM_OF_ROW  16
#define SYNAPTICS_RMI4_BARCODE_OFFSET_4  (4*16)
...
#define NOISE_VALUE_0 0
#define NOISE_VALUE_1 1
#define NOISE_VALUE_2 2
...
struct synaptics_grip_data {
	...
	u8 xer; // Edge Ratio of left and right edges
	u8 yer; // Edge Ratio of upper and lower edges
};
...
struct synaptics_rmi4_data {
	...
	struct ts_kit_device_data *synaptics_chip_data;
	...
	unsigned int need_disable_hover;
	unsigned int hover_switch_addr;
	unsigned int hover_switch_bit;
	...
	unsigned int ext_trex_short_wait_report_delay_flag;
	int *upper;
	int *lower;
	...
	int distinguish_ic_type;
	int support_crc_err_do_reset;
	unsigned int ud_finger_status;
	unsigned int screenoff_status_reg;
	unsigned int screenoff_status_switch;
...
enum barcode_oem_type {
	BARCODE_READ_STANDARD_TYPE,
	BARCODE_READ_OEM_TYPE_1,
};
...
extern int synap_fw_data_init(struct synaptics_rmi4_data *rmi4_data);
extern bool synap_check_fw_version(void);
extern int synap_get_fw_data_boot(char *file_name,
				      struct touch_settings
				      *synaptics_sett_param_regs);
extern int synap_get_fw_data_sd(void);
extern void synap_fw_data_release(void);
extern int synap_fw_update(void);
extern int synap_fw_data_s3718_init(struct synaptics_rmi4_data *rmi4_data);
extern bool synap_check_fw_s3718_version(void);
extern int synap_get_fw_data_s3718_boot(char *file_name,
					    struct touch_settings
					    *synaptics_sett_param_regs);
extern int synap_get_fw_data_s3718_sd(void);
extern void synap_fw_data_s3718_release(void);
extern int synap_fw_s3718_update(void);
extern int synap_rmi4_f54_s3207_init(struct synaptics_rmi4_data *rmi4_data,
					 const char *module_name);
extern int synap_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data,
				   const char *module_name);
extern unsigned short synap_f54_get_calibrate_addr(struct
						       synaptics_rmi4_data
						       *rmi4_data,
						       const char *module_name);
#ifdef RED_REMOTE
extern void synap_fw_debug_dev_init(struct synaptics_rmi4_data *rmi4_data);
extern void synap_rmidev_sysfs_irq(void *data);
#endif
extern int synap_get_cap_data(struct ts_rawdata_info *info);
extern int synap_get_calib_data(struct ts_calibration_data_info *info);
extern int synap_get_calib_info(struct ts_calibration_info_param *info);
extern int synap_fw_configid(struct synaptics_rmi4_data *rmi4_data, u8 *buf,
					size_t buf_size);
