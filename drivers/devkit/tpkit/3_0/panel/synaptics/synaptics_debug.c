...
#define  CSV_TP_FORCEKEY_CAP_VALUE_RANGE "ForcekeyCapValueRange"
...
#define COMMAND_TIMEOUT_100MS 100
#define WAIT_COMMAND_LOOP_MAX 20
...
#define RAWDATA_CSV_VERTICAL_SCREEN 2
...
struct synaptics_rmi4_f54_handle {
	...
	unsigned int forcekey_value[CAP_TEST_FORCEKEY_VALUE_NUM];
};
...
static void set_report_size(void)
{
	...
	case F54_HYBRID_SENSING_RAW_CAP:
		if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test)
			f54->report_size = 4 * (f54->rmi4_data->num_of_rx + \
								f54->rmi4_data->num_of_tx + \
								CAP_TEST_FORCEKEY_VALUE_NUM);
		else
			f54->report_size = 4 * (f54->rmi4_data->num_of_rx + f54->rmi4_data->num_of_tx);
		break;
...
static int test_wait_for_command_completion(void)
{
	...
	do {
		...
		msleep(COMMAND_TIMEOUT_100MS);
		timeout_count++;
	} while (timeout_count < WAIT_COMMAND_LOOP_MAX);

	if (timeout_count == WAIT_COMMAND_LOOP_MAX) {
...
static int f54_deltarawimage_report(void)
{
	...
	short DeltaRawimageMaxNum = 0, DeltaRawimageMinNum = 0;
	short result = 0;
	...
	for (i = 0; i < mmi_buf_size; i += 2) {
		...
		if ((Rawimage >= delt_cap_lowlimit)
			...
			TS_LOG_ERR("[No:%d,current is %d] delt_cap_uplimit = %d, delt_cap_lowlimit = %d\n", i / 2, Rawimage,delt_cap_uplimit, delt_cap_lowlimit);
		}
	}
...
static void put_hybrid_data(int index)
{
	...
	for (i = 0, j = index; i < mmi_hybrid_abs_delta; i += 4, j++) {
		if((j < HYBRID_BUF_SIZE) && (i + 3 < mmi_buf_size)){
	...
	if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test && index != 0){
		if((mmi_hybrid_abs_delta /4) + index -1 < HYBRID_BUF_SIZE){
			if(0 == strcmp(f54->rmi4_data->module_name,"BOE")){
				f54->forcekey_value[0] = f54->hybridbuf[(mmi_hybrid_abs_delta /4) + index -2];
				f54->forcekey_value[1] = f54->hybridbuf[(mmi_hybrid_abs_delta /4) + index -1];
			}
			else if(0 == strcmp(f54->rmi4_data->module_name,"LG")){
				f54->forcekey_value[0] = f54->hybridbuf[(mmi_hybrid_abs_delta /4) + index -2];
				f54->forcekey_value[1] = f54->hybridbuf[(mmi_hybrid_abs_delta /4) + index -1];
			}
			else{
				TS_LOG_ERR("read forcekey error!\n");
			}
		}
	}
...
static void check_hybrid_raw_cap(void)
{
	...
	if(0 != result){
	     TS_LOG_ERR("%s, ts_kit_parse_csvfile  parse tx range fail result=%dis 6F\n", __func__, result);
	     goto test_fail;
	}
	...
	for(i = 0; i < tx_num; i++){
		if( false == is_in_range(f54->hybridbuf[index+i+rx_num],
		    ...
		    TS_LOG_ERR("%s, check_hybrid_raw_cap test is out of range, test result is 6F\n", __func__);
			TS_LOG_ERR("current(%d) :%d |uplimit :%d |downlimit:%d\n",i,f54->hybridbuf[index+i+rx_num],
						hybird_raw_tx_range[i+tx_num], hybird_raw_tx_range[i]);
	...
	if(0 != result){
	     TS_LOG_ERR("%s, ts_kit_parse_csvfile  parse rx fail result=%dis 6F\n", __func__, result);
	     goto test_fail;
	}
	...
	TS_LOG_INFO("%s, hybrid_abs_delta_report_size = %d. test result is 6P \n", __func__, f54->report_size);
...
static void check_hybrid_abs_delta(void)
{
	...
	if(0 != result){
	     TS_LOG_ERR("%s, ts_kit_parse_csvfile  parse tx fail result=%dis 9F\n", __func__, result);
	     goto test_fail;
	}
	...
	for(i = 0; i < tx_num; i++){
		if(false == is_in_range(f54->hybridbuf[index+i+rx_num],
		    ...
		    TS_LOG_ERR("%s, hybrid_abs_delta test is out of range, test result is 9F\n", __func__);
			TS_LOG_ERR("current(%d) :%d |uplimit :%d |downlimit:%d\n",i,f54->hybridbuf[index+i+rx_num],
			hybird_abs_tx_range[i+tx_num], hybird_abs_tx_range[i]);
		    strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
	...
	if(0 != result){
	     TS_LOG_ERR("%s, ts_kit_parse_csvfile  parse rx fail result=%dis 9F\n", __func__, result);
	     goto test_fail;
	}
	...
	for(i = 0; i < rx_num; i++){
		if(false == is_in_range(f54->hybridbuf[index+i],
		    ...
		    TS_LOG_ERR("%s, hybrid_abs_delta test is out of range, test result is 9F\n", __func__);
			TS_LOG_ERR("current(%d) :%d |uplimit :%d |downlimit:%d\n",i,f54->hybridbuf[index+i],\
							hybird_abs_rx_range[i+rx_num],\
							hybird_abs_rx_range[i]);
		    strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
			test_failed_reason[RAW_DATA_TYPE_SELFNOISETEST] = TEST_THRESHOLD_FAIL;
		    goto test_fail;
	...
	TS_LOG_INFO("%s, hybrid_abs_delta_report_size = %d. test result is 9P \n",__func__, f54->report_size);
...
static void check_forcekey_cap_value(void)
{
	...
	char file_name[64] = {0};
	int result = 0;
	int32_t  forcekey_cap_value_range[HYBRID_BUF_SIZE]={0};
	snprintf(file_name, sizeof(file_name), "%s_%s_%s_%s_raw.csv",
			f54->rmi4_data->synaptics_chip_data->ts_platform_data->product_name,
			f54->rmi4_data->synaptics_chip_data->chip_name,
			f54->rmi4_data->rmi4_mod_info.project_id_string,
	...
	result = ts_kit_parse_csvfile(file_path, CSV_TP_FORCEKEY_CAP_VALUE_RANGE,
		forcekey_cap_value_range, CAP_TEST_FORCEKEY_VALUE_NUM, CAP_TEST_FORCEKEY_VALUE_NUM);
	if(0 != result){
	     TS_LOG_ERR("%s, ts_kit_parse_csvfile  parse tx fail result=%dis 10F\n", __func__, result);
	     goto test_fail;
	}

	for(i = 0; i < CAP_TEST_FORCEKEY_VALUE_NUM; i++){
		if(false == is_in_range(f54->forcekey_value[i],
			forcekey_cap_value_range[i+CAP_TEST_FORCEKEY_VALUE_NUM], forcekey_cap_value_range[i]) ){
		    TS_LOG_ERR("%s, hybrid_abs_delta test is out of range, test result is 10F\n", __func__);
			TS_LOG_ERR("current(%d) :%d |uplimit :%d |downlimit:%d\n",i,f54->forcekey_value[i],\
							forcekey_cap_value_range[i + CAP_TEST_FORCEKEY_VALUE_NUM],\
							forcekey_cap_value_range[i]);
		    strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
			test_failed_reason[RAW_DATA_TYPE_FORCEKEY] = TEST_THRESHOLD_FAIL;
		    goto test_fail;
		}
	}

	TS_LOG_INFO("%s, forcekey   test result is 10P \n",__func__);
	strncat(buf_f54test_result, "-10P", MAX_STR_LEN);
       return;

test_fail:
	strncat(buf_f54test_result, "-10F", MAX_STR_LEN);
	return;
}
...
static int mmi_hybrid_raw_cap_test(void)
{
	...
	if (result < 0) {
		if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test){
			TS_LOG_ERR("%s(%d), Failed to get data\n", __func__, __LINE__);
			strncat(buf_f54test_result, "-6F-9F-10F", MAX_STR_LEN);
		}
		else{
			TS_LOG_ERR("%s(%d), Failed to get data\n", __func__, __LINE__);
			strncat(buf_f54test_result, "-6F-9F-10F", MAX_STR_LEN);
...
static void mmi_trex_shorts_test(void)
{
	...
	for(i = 0; i < TP_3320_SHORT_ARRAY_NUM; i++) {
		...
		if(buf_backup[i] != f54->rmi4_data->trx_short_circuit_array[i]) {
...
static int f54_delta_rx2_report(void)
{
	...
	if (!limit_tab.unique_test->rx_delta_abslimit) {
		...
		if (TEST_FAIL == synaptics_get_threshold_from_csvfile(rows_size, columns_size-1, CSV_TP_DELTA_ABS_RX_LIMIT, limit_tab.unique_test->rx_delta_abslimit)) {
			TS_LOG_ERR("get csv_Vertical_screen abs_rxdelt_cap_limit err\n");
			ret = TEST_FAIL;
			memset(limit_tab.unique_test->rx_delta_abslimit, 0, rows_size*columns_size*sizeof(int32_t));
		}
		ts_kit_rotate_rawdata_abcd2cbad(rows_size, columns_size-1, limit_tab.unique_test->rx_delta_abslimit,RAWDATA_CSV_VERTICAL_SCREEN);
	...
	if(f54->rmi4_data->synaptics_chip_data->rawdata_newformatflag == TS_RAWDATA_NEWFORMAT) {
		for ( i = 0; i < mmi_buf_size; i+=2) {
			...
			if (Rawimage_rx <= rxdelt_cap_abslimit){
				...
				TS_LOG_ERR("[NO:%d |current is %d] abslimit rx is  %d\n",i/2 - step,Rawimage_rx, rxdelt_cap_abslimit);
			}
			j++;
			if (j == columns_size - 1) {
				i += 2;
...
static int f54_delta_tx2_report(void)
{
	...
	if (!limit_tab.unique_test->tx_delta_abslimit) {
		...
		if (TEST_FAIL == synaptics_get_threshold_from_csvfile(rows_size-1, columns_size, CSV_TP_DELTA_ABS_TX_LIMIT,\
			limit_tab.unique_test->tx_delta_abslimit)) {
		...
		ts_kit_rotate_rawdata_abcd2cbad(rows_size-1, columns_size, limit_tab.unique_test->tx_delta_abslimit,RAWDATA_CSV_VERTICAL_SCREEN);
...
static int check_enhance_raw_capacitance(void)
{
	...
	for (i = 0; i < count; i++) {
		TS_LOG_INFO("rawdata is upper: %d, lower: %d\n", f54->rmi4_data->upper[i], f54->rmi4_data->lower[i]);
		/* rawdatabuf[0] rawdatabuf[1] are num_of_rx and num_of_tx */
		if ((f54->rawdatabuf[i+2] > f54->rmi4_data->upper[i]) || (f54->rawdatabuf[i+2] < f54->rmi4_data->lower[i])) {
			TS_LOG_ERR("rawdata is out of range, failed at %d: upper: %d, lower: %d, raw: %d\n", i, f54->rmi4_data->upper[i], f54->rmi4_data->lower[i], f54->rawdatabuf[i+2]);
			return 0;
		}
	}
...
static int check_csvfile_raw_capacitance(void)
{
	...
	int columns_size = f54->rmi4_data->num_of_tx;
	int rows_size = f54->rmi4_data->num_of_rx;
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(columns_size, rows_size, CSV_TP_CAP_RAW_MIN, limit_tab.mutual_raw_min)) {
		...
		ts_kit_rotate_rawdata_abcd2cbad(columns_size, rows_size, limit_tab.mutual_raw_max,RAWDATA_CSV_VERTICAL_SCREEN);
		ts_kit_rotate_rawdata_abcd2cbad(columns_size, rows_size, limit_tab.mutual_raw_min,RAWDATA_CSV_VERTICAL_SCREEN);
	for (i = 0; i < count; i++) {
		/* rawdatabuf[0] rawdatabuf[1] are num_of_rx and num_of_tx */
...
static void mmi_csvfile_rawdata_test(void)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->trx_delta_test_support) {
		if (TEST_PASS == (f54_delta_rx2_report() & f54_delta_tx2_report())) {
			TS_LOG_INFO("trx_delta test is success, test result is 2P\n");
...
{
	int retval = 0, result = 0, i = 0, j = 0, k = 0, ii = 0, jj = 0, rxIndex = 0;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	for (i=0; i<f54->report_size; i++) {
		TS_LOG_DEBUG("RT26 data byte %d - data 0x%x\n", i, p_data_rt26[i]);
		if (p_data_rt26[i] != 0) {
	...
	p_data_baseline1 = kzalloc((size_t)2 * numberOfRows * numberOfColums, GFP_KERNEL);
	if (!p_data_baseline1) {
		TS_LOG_ERR("%s: Failed to do alloc baseline1 buf\n",
				__func__);
		goto exit_26100_type;
	}
	p_data_baseline2 = kzalloc((size_t)2 * numberOfRows * numberOfColums, GFP_KERNEL);
	if (!p_data_baseline2) {
		TS_LOG_ERR("%s: Failed to do alloc baseline1 buf\n",
				__func__);
		goto exit_26100_type;
	}
	p_data_delta = kzalloc((size_t)2 * numberOfRows * numberOfColums, GFP_KERNEL);
	if (!p_data_delta) {
	...
	for (i = 0; i< 4; i++) {
		...
		for (j=0; j< f54->rmi4_data->num_of_rx; j++)
                       TS_LOG_DEBUG("%s: temp_data[%d] = 0x%02x\n", __func__, j, temp_data[j]);
		...
		if(f54->rmi4_data->ext_trex_short_wait_report_delay_flag)
			msleep(200);
...
exit_26100_type:
	...
	{
		TS_LOG_INFO("%s: test result is succ     5P\n", __func__);
		strncat(buf_f54test_result, SHORT_TEST_PASS, MAX_STR_LEN);
	}
	else
	{
		TS_LOG_INFO("%s: test result is fail      5F\n", __func__);
		strncat(buf_f54test_result, SHORT_TEST_FAIL, MAX_STR_LEN);
		test_failed_reason[RAW_DATA_TYPE_OPENSHORT] = TEST_THRESHOLD_FAIL;
	}
	if(p_data_baseline1)
...
int synap_put_cap_test_dataNewformat(struct ts_rawdata_info_new *info)
{
	...
	/*rawdata*/
	if(f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile
		|| f54->rmi4_data->test_enhance_raw_data_capacitance) {
		...
		if (f54->rmi4_data->synaptics_chip_data->trx_delta_test_support) {
			...
			ts_kit_rotate_rawdata_abcd2cbad(info->rx-1, info->tx, pts_node->values,RAWDATA_CSV_VERTICAL_SCREEN);
			ts_kit_rotate_rawdata_abcd2cbad(info->rx, info->tx-1, pts_node->values + rawbufsize,RAWDATA_CSV_VERTICAL_SCREEN);
	...
	/*5p5f*/
	if(f54->rmi4_data->synaptics_chip_data->td43xx_ee_short_test_support
			|| f54->rmi4_data->support_s3320_short_test
			|| f54->rmi4_data->support_ext_trex_short_test) {
		pts_node = (struct ts_rawdata_newnodeinfo *)kzalloc(sizeof(struct ts_rawdata_newnodeinfo), GFP_KERNEL);
		if (!pts_node) {
	...
	if(f54->rmi4_data->synaptics_chip_data->self_cap_test) {
		...
		/*10p10f*/
		if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test){
			rc = synaptics_alloc_newnode(&pts_node,CAP_TEST_FORCEKEY_VALUE_NUM);
			if(rc < 0) {
				TS_LOG_ERR("malloc failed 6pts_node\n");
				return rc;
			}
			pts_node->testresult = buf_f54test_result[test_num+1];
			pts_node->typeindex = RAW_DATA_TYPE_FORCEKEY;
			pts_node->size = CAP_TEST_FORCEKEY_VALUE_NUM;
			for (i = 0; i < CAP_TEST_FORCEKEY_VALUE_NUM; i++)
				pts_node->values[i] =f54->forcekey_value[i];
			strncpy(pts_node->test_name,"forcekey",sizeof(pts_node->test_name)-1);
			list_add_tail(&pts_node->node, &info->rawdata_head);
			test_num = test_num + test_tmp_num;
		}
	}
...
int synap_get_cap_data(struct ts_rawdata_info *info)
{
	...
	if (rc < 0) {		/*i2c communication failed, mmi result is all failed */
		...
		if (f54->rmi4_data->test_enhance_raw_data_capacitance) {
			...
			TS_LOG_ERR("[%s] devkit 3.0 not support dts raw threshold \n",__func__);
			rc = -EINVAL;
			goto exit;
		...
		if (f54->rmi4_data->support_s3320_short_test){
			mmi_trex_shorts_test();
		}
		if(f54->rmi4_data->support_ext_trex_short_test)
		{
		...
		if(f54->rmi4_data->synaptics_chip_data->self_cap_test)
		{
			...
			rc = mmi_hybrid_raw_cap_test();
			if(rc >= 0) {
				mmi_hybrid_abs_delta_test();
				TS_LOG_INFO("seltest----end~!\n");
				if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test)
				{
					check_forcekey_cap_value();
				}
			}
		}
		mmi_add_static_data();
	...
	case SYNAPTICS_S3706:
		strncat(buf_f54test_result, "-synaptics-",
					strlen("-synaptics-"));
	...
	len_tmp = strlen(buf_f54test_result);
	if ((len_tmp + strlen(f54->rmi4_data->rmi4_mod_info.project_id_string)
	...
	if(f54->rmi4_data->synaptics_chip_data->support_forcekey_cap_value_test){
		info->hybrid_buff_used_size = mmi_hybrid_abs_delta >> 1;
	}
...
int synap_debug_data_test(struct ts_diff_data_info *info)
{
	...
	case READ_RAW_DATA:
		if (f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile) {
			tx_delta_buf = (int *)kzalloc(mmi_buf_size * sizeof(int), GFP_KERNEL);
			if (!tx_delta_buf) {
				TS_LOG_ERR("%s malloc tx_delta_buf failed\n", __func__);
				goto free_debug_mem;
			}
			rx_delta_buf = (int *)kzalloc(mmi_buf_size * sizeof(int), GFP_KERNEL);
			if (!rx_delta_buf) {
				TS_LOG_ERR("%s malloc rx_delta_buf failed\n", __func__);
				goto free_debug_mem;
			}
...
static int synaptics_read_f34(void)
{
	...
	if((f54->rmi4_data->num_of_tx == 0)||(f54->rmi4_data->num_of_tx > RAWDATA_NUM_OF_TRX_MAX)||\
		(f54->rmi4_data->num_of_rx == 0)||(f54->rmi4_data->num_of_rx > RAWDATA_NUM_OF_TRX_MAX)){
		TS_LOG_ERR("num of tx or rx overflow TRX MAX\n");
		return -EINVAL;
	}
...
static int test_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	memset(&ctrl_43 , 0 ,sizeof(struct f55_control_43));
	...
	if (!f55->tx_assignment) {
		TS_LOG_ERR("%s no memory for tx_assignment", __func__);
		retval = -ENOMEM;
		goto error_free_mem;
	}
	f55->rx_assignment = kzalloc(rx_electrodes, GFP_KERNEL);
	if (!f55->rx_assignment) {
		TS_LOG_ERR("%s no memory for rx_assignment", __func__);
		retval = -ENOMEM;
		goto error_free_mem;
	}
	f55->rx_physical = kzalloc(rx_electrodes, GFP_KERNEL);
	if (!f55->rx_physical) {
		TS_LOG_ERR("%s no memory for rx_physical", __func__);
		retval = -ENOMEM;
		goto error_free_mem;
	}
	...
	if (f55->has_force) {
		...
		if (!f55->force_tx_assignment) {
			TS_LOG_ERR("%s no memory for force_tx_assignment", __func__);
			retval = -ENOMEM;
			goto error_free_mem;
		}
		f55->force_rx_assignment = kzalloc(rx_electrodes, GFP_KERNEL);
		if (!f55->force_rx_assignment) {
			TS_LOG_ERR("%s no memory for force_rx_assignment", __func__);
			retval = -ENOMEM;
			goto error_free_mem;
		}
...
error_free_mem:
	...
	if (f55->rx_physical) {
		kfree(f55->rx_physical);
		f55->rx_physical = NULL;
	}
