...
extern int ts_kit_put_one_cmd(struct ts_cmd_node *cmd, int timeout);
...
void synap_rmidev_sysfs_irq(void *data)
{
					    ...
					    char *buf)
{
	...
	attn_state =
	    gpio_get_value(rmidev->rmi4_data->synaptics_chip_data->ts_platform_data->irq_gpio);
			   ...
			   size_t count, loff_t *f_pos)
{
	...
	if (!rmidev->rmi4_data->synaptics_chip_data)
		return 0;
	...
	if (!list_empty(&rmidev->rmi4_data->rmi4_mod_info.support_fn_list)) {
		list_for_each_entry(fhandler,
				    ...
				    support_fn_list, link) {
			if (fhandler->num_of_data_sources) {
				if (fhandler->intr_mask & intr_status) {
					rmidev->rmi4_data->report_touch(rmidev->
									rmi4_data,
									fhandler,
									info);
				}
	...
	retval = ts_kit_put_one_cmd(in_cmd, SHORT_SYNC_TIMEOUT);
	if (retval) {
		TS_LOG_ERR("ts_kit_put_one_cmd error");
	}
...
static int rmidev_open(struct inode *inp, struct file *filp)
{
	...
	if( !rmi4_data->synaptics3718_Tp_Pressure_flag ){
...
static int rmidev_release(struct inode *inp, struct file *filp)
{
	...
	enable_irq(rmi4_data->synaptics_chip_data->ts_platform_data->irq_id);
...
void synap_fw_debug_dev_init(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = NO_ERR;
	dev_t dev_no = 0;
	int attr_count = 0;
	...
	TS_LOG_INFO("synap_fw_debug_dev_init called\n");
	...
	retval = gpio_export(rmi4_data->synaptics_chip_data->ts_platform_data->irq_gpio, false);
	if (retval < 0) {
		TS_LOG_ERR("Failed to export attention gpio\n");
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
					  "attn",
					  rmi4_data->synaptics_chip_data->ts_platform_data->irq_gpio);
		if (retval < 0) {
