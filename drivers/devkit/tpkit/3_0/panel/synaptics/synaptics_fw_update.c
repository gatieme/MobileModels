...
#define SYNAPTICS_FW_MANUAL_UPDATE_FILE_NAME	"ts/touch_screen_firmware.img"
...
static DECLARE_COMPLETION(syncptics_fwu_remove_complete);
...
int synap_fw_configid(struct synaptics_rmi4_data *rmi4_data, u8 *buf,
			  size_t buf_size)
{
	int rc = 0;
	...
	TS_LOG_INFO("synap_fw_configid called\n");
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	memset(&f01_device_status, 0, sizeof(struct f01_device_status));

	if (NULL == header) {
		TS_LOG_ERR("img head is NULL\n");
		goto exit;
	}
	if (NULL == fwu->config_data) {
		TS_LOG_ERR("fwu->config_data is NULL\n");
		goto exit;
	}
	...
	TS_LOG_INFO
	    ("Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X,addr = 0x%02x\n",
	     config_id[0], config_id[1], config_id[2], config_id[3],
	     fwu->f34_fd.ctrl_base_addr);
	...
	TS_LOG_INFO("Img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		    fwu->config_data[0],
		    fwu->config_data[1],
		    fwu->config_data[2], fwu->config_data[3]);
	...
	snprintf(fwu->rmi4_data->rmi4_mod_info.image_config_id,
		 sizeof(unsigned char) * CHIP_INFO_LENGTH,
		 "%02x %02x %02x %02x ", fwu->config_data[0],
		 fwu->config_data[1], fwu->config_data[2], fwu->config_data[3]);
			    ...
			    unsigned char command)
{
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (retval < 0) {
			TS_LOG_ERR
			    ("Failed to wait for idle status (block %d)\n",
			     block_num);
			return retval;
		}
...
int synap_fw_update(void)
{
	int retval = 0;
	struct f01_device_status f01_device_status;

	TS_LOG_INFO("synap_fw_update called\n");
	...
	if (f01_device_status.flash_prog) {
		TS_LOG_INFO
		    ("Device is in flash prog mode, Flash status 0x%02X\n",
...
static int synaptics_read_project_id(void)
{
	...
	memcpy(project_id, fwu->rmi4_data->rmi4_mod_info.product_id_string,
...
*/
int synap_fw_data_init(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	...
	TS_LOG_INFO("synap_fw_data_init done\n");
	return 0;
 ...
 */
int synap_get_fw_data_boot(char *file_name,
			       struct touch_settings *synaptics_sett_param_regs)
{
	int retval = NO_ERR;
	...
	if (fwu->fw_entry_boot->data == NULL) {
		TS_LOG_ERR("fw_image is null\n");
		return -EINVAL;
	}
...
int synap_get_fw_data_sd(void)
{
	...
	if (fwu->fw_entry_sd->data == NULL) {
		TS_LOG_ERR("fw_image is null\n");
		return -EINVAL;
	}
	fwu->fw_image = fwu->fw_entry_sd->data;
	fwu->image_size = fwu->fw_entry_sd->size;
	TS_LOG_INFO("Firmware image size = %ld\n", fwu->fw_entry_sd->size);
	return NO_ERR;
}

bool synap_check_fw_version(void)
{
	enum flash_area flash_area = NONE;
	...
	fw_image = fwu->fw_image;
	parse_header(&header, fw_image);

	if(header.firmware_size){
		/* FW_IMAGE_OFFSET + image_size was ok as above */
		if (!in_bounds(FW_IMAGE_OFFSET, header.firmware_size, fwu->image_size)) {
			TS_LOG_ERR("image size out of bounds\n");
			return false;
		}
	...
	if(header.config_size){
		/* FW_IMAGE_OFFSET + config_size was ok as above */
		if (!in_bounds(FW_IMAGE_OFFSET, header.config_size, fwu->image_size)) {
			TS_LOG_ERR("config_size size out of bounds\n");
			return false;
		}
		fwu->config_data =
...
void synap_fw_data_release(void)
{
	...
	if (fwu && fwu->fw_entry_sd) {
		release_firmware(fwu->fw_entry_sd);
