...
enum touch_data_flip_type {
	TOUCH_DATA_XY_SWAP = 1,
};
...
static unsigned char *g_temp_buf; /* rawdata */
	...
	unsigned int buf_size, unsigned int payload_length)
{
	...
	if ((total_length + 1) > buf_size) {
		TS_LOG_ERR("The buf_size %d is less then request size %d\n",
			buf_size, (total_length + 1));
		return -ENOMEM;
	}
	...
	unsigned int cols, char *file_path)
{
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_SELF_CAP_RAW_DATA_COL_MIN_ARRAY,
			threshold->self_cap_raw_data_col_min_limits, rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			CSV_SELF_CAP_RAW_DATA_COL_MIN_ARRAY);
	ret = ts_kit_parse_csvfile(file_path, CSV_SELF_CAP_RAW_DATA_COL_MAX_ARRAY,
			threshold->self_cap_raw_data_col_max_limits, rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			CSV_SELF_CAP_RAW_DATA_COL_MAX_ARRAY);
	ret = ts_kit_parse_csvfile(file_path, CSV_SELF_CAP_RAW_DATA_ROW_MIN_ARRAY,
			threshold->self_cap_raw_data_row_min_limits, rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			CSV_SELF_CAP_RAW_DATA_ROW_MIN_ARRAY);
	ret = ts_kit_parse_csvfile(file_path, CSV_SELF_CAP_RAW_DATA_ROW_MAX_ARRAY,
			threshold->self_cap_raw_data_row_max_limits, rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			CSV_SELF_CAP_RAW_DATA_ROW_MAX_ARRAY);
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_RAW_DATA_ROW_DELTA_MAX_ARRAY,
		threshold->raw_data_row_delta_max_limits, rows - 1, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s \n", __func__, CSV_RAW_DATA_ROW_DELTA_MAX_ARRAY);

	ret = ts_kit_parse_csvfile(file_path, CSV_RAW_DATA_COL_DELTA_MAX_ARRAY,
		threshold->raw_data_col_delta_max_limits, rows, cols - 1);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s \n", __func__, CSV_RAW_DATA_COL_DELTA_MAX_ARRAY);
...
static void touch_driver_flip_2D_data(unsigned char *buf,
	unsigned int data_size)
{
	...
	unsigned int flip_type = test_hcd->flip_type;
	struct syna_tcm_app_info *app_info = NULL;
	struct syna_tcm_hcd *tcm_hcd = test_hcd->tcm_hcd;

	if (!buf) {
		TS_LOG_ERR("%s Invalid buf\n", __func__);
		goto exit;
	}
	app_info = &tcm_hcd->app_info;
	rows = le2_to_uint(app_info->num_of_image_rows);
	...
	size = rows * cols * 2;
	if (data_size < size) {
		TS_LOG_ERR("data size incorrect data_size:%d, expected:%d\n",
			data_size, size);
		goto exit;
	}

	temp_buf = kzalloc(size * sizeof(unsigned char), GFP_KERNEL);
	if (!temp_buf) {
	...
	switch (flip_type) {
	case TOUCH_DATA_XY_SWAP: /* clockwise 90D */
		for (i = 0; i < rows; i++) {
			for (j = 0; j < cols; j++) {
				ptr[(i * cols + j) * 2 + 0] = temp_buf[(rows * j + i) * 2 + 0];
				ptr[(i * cols + j) * 2 + 1] = temp_buf[(rows * j + i) * 2 + 1];
			}
		}
		break;
	default:
		TS_LOG_ERR("Flip Type %d, not support\n", flip_type);
		break;
	}
	kfree(temp_buf);
	temp_buf = NULL;
...
static void touch_driver_flip_abs_data(unsigned char *buf,
	unsigned int data_size, unsigned int bytes_size)
{
	int i;
	int j;
	unsigned int rows;
	...
	memcpy(temp_buf, buf, size);
	switch (flip_type) {
	case TOUCH_DATA_XY_SWAP:
		for (i = 0; i < cols; i++) {
			for (j = 0; j < bytes_size; j++)
				ptr[i * bytes_size + j] = temp_buf[(rows + i) * bytes_size + j];
		}
		for (i = 0; i < rows; i++) {
			for (j = 0; j < bytes_size; j++)
				ptr[(cols + i) * bytes_size + j] = temp_buf[i * bytes_size + j];
		}
		break;
	default:
		TS_LOG_ERR("Flip Type %d, not support\n", flip_type);
...
static int tskit_driver_open_short_test(struct ts_rawdata_info_new *info)
{
	...
	/* flip the 2D data */
	touch_driver_flip_2D_data(&temp_buf[MESSAGE_HEADER_SIZE], data_length);
	idx = 0;
...
static int tskit_driver_abs_raw_cap_test(struct ts_rawdata_info_new *info)
{
	...
	/* flip the abs data */
	touch_driver_flip_abs_data(&temp_buf[MESSAGE_HEADER_SIZE], data_length, 4);
...
static int testing_get_static_config(unsigned char *buf, unsigned int buf_len)
{
	...
	retval = tskit_driver_read(tcm_hcd, g_temp_buf, G_TEMP_BUF_LEN);
...
static int touch_driver_self_cap_raw_data_test(
	struct ts_rawdata_info_new *info, unsigned char *self_raw_data)
{
	int retval;
	unsigned int idx;
	unsigned int row;
	...
	unsigned int rows;
	unsigned int cols;
	unsigned char *temp_buf = self_raw_data;
	int *temp_buf_row_int_p = NULL;
	int *temp_buf_col_int_p = NULL;
	char testresult = CAP_TEST_PASS_CHAR;
	char failedreason[TS_RAWDATA_FAILED_REASON_LEN] = {0};
	...
	if (tcm_hcd->id_info.mode != MODE_APPLICATION ||
			tcm_hcd->app_status != APP_STATUS_OK) {
		TS_LOG_ERR("%s Application firmware not running mode = %d, app_status = %d\n",
				__func__,
	...
	for (col = 0; col < cols; col++) {
		temp_buf_col_int_p[col] = (int)((temp_buf[idx] & 0xff) |
			(temp_buf[idx+1] << 8));
			TS_LOG_INFO("%s col=%-2d data = %d , limit min = %d max = %d\n",
				__func__, col, temp_buf_col_int_p[col],
				threshold->self_cap_raw_data_col_min_limits[col],
				threshold->self_cap_raw_data_col_max_limits[col]);
		if (col == 0) {
			cap_raw_data_min = temp_buf_col_int_p[col];
			cap_raw_data_max = temp_buf_col_int_p[col];
			cap_raw_data_avg = temp_buf_col_int_p[col];
		} else {
			if (cap_raw_data_min > temp_buf_col_int_p[col])
				cap_raw_data_min = temp_buf_col_int_p[col];
			if (cap_raw_data_max < temp_buf_col_int_p[col])
				cap_raw_data_max = temp_buf_col_int_p[col];
			cap_raw_data_avg = (temp_buf_col_int_p[col] +
				cap_raw_data_avg);
		}
		if (temp_buf_col_int_p[col] > threshold->self_cap_raw_data_col_max_limits[col] ||
			temp_buf_col_int_p[col] < threshold->self_cap_raw_data_col_min_limits[col]) {
			test_hcd->result = false;
			testresult = CAP_TEST_FAIL_CHAR;
			strncpy(failedreason, "-panel_reason",
				TS_RAWDATA_FAILED_REASON_LEN - 1);
			retval = -EINVAL;
			TS_LOG_ERR("%s fail at col=%-2d data = %d 0x%2x 0x%2x, limit min = [%d] max = [%d]\n",
				__func__,
				col, temp_buf_col_int_p[col],
	...
	for (row = 0; row < rows; row++) {
		temp_buf_row_int_p[row] = (int)((temp_buf[idx] & 0xff) |
			(temp_buf[idx+1] << 8));
		TS_LOG_INFO("%s row=%-2d data = %d, limit min = %d max = %d\n",
			__func__, row, temp_buf_row_int_p[row],
			threshold->self_cap_raw_data_row_min_limits[row],
			threshold->self_cap_raw_data_row_max_limits[row]);
		if (cap_raw_data_min > temp_buf_row_int_p[row])
			cap_raw_data_min = temp_buf_row_int_p[row];
		if (cap_raw_data_max < temp_buf_row_int_p[row])
			cap_raw_data_max = temp_buf_row_int_p[row];
		cap_raw_data_avg = (temp_buf_row_int_p[row]+cap_raw_data_avg);

		if (temp_buf_row_int_p[row] > threshold->self_cap_raw_data_row_max_limits[row] ||
			temp_buf_row_int_p[row] < threshold->self_cap_raw_data_row_min_limits[row]) {
			test_hcd->result = false;
			testresult = CAP_TEST_FAIL_CHAR;
			strncpy(failedreason, "-panel_reason",
				TS_RAWDATA_FAILED_REASON_LEN - 1);
			retval = -EINVAL;
			TS_LOG_ERR("%s fail at row=%-2d data = %d 0x%2x 0x%2x, limit min = [%d] max = [%d]\n",
				__func__,
				row, temp_buf_row_int_p[row],
				temp_buf[idx], temp_buf[idx+1],
				threshold->self_cap_raw_data_row_min_limits[row],
	...
	cap_raw_data_avg = (cap_raw_data_avg / (rows + cols));
	memcpy(pts_node->values, temp_buf_row_int_p, rows*sizeof(int));
	memcpy(&(pts_node->values[rows]), temp_buf_col_int_p, cols*sizeof(int));
	...
	pts_node->testresult = testresult;
	pts_node->typeindex = RAW_DATA_TYPE_SELFCAPRAWDATA;
	strncpy(pts_node->test_name, "Self_Cap_Rawdata",
		TS_RAWDATA_TEST_NAME_LEN - 1);
	snprintf(pts_node->statistics_data, TS_RAWDATA_STATISTICS_DATA_LEN,
		"[%d,%d,%d]", cap_raw_data_min, cap_raw_data_max, cap_raw_data_avg);
	if (testresult == CAP_TEST_FAIL_CHAR)
		strncpy(pts_node->tptestfailedreason, failedreason,
			TS_RAWDATA_FAILED_REASON_LEN - 1);
	list_add_tail(&pts_node->node, &info->rawdata_head);
...
exit:
	...
	if (retval >= 0) {
		retval_selfcaptest = touch_driver_self_cap_raw_data_test(info, &temp_buf[idx * 2 +
				MESSAGE_HEADER_SIZE]);
		if (retval_selfcaptest)
			TS_LOG_INFO("touch_driver_self_cap_raw_data_test failed = %d\n", retval_selfcaptest);
		retval = touch_driver_delta_test(info, pts_node->values);
		if (retval)
			TS_LOG_INFO("delta_test failed = %d\n", retval);
		if (retval >= 0 && retval_selfcaptest < 0)
			retval = retval_selfcaptest;
	}
	test_hcd->report_type = 0;
	kfree(temp_buf);
	temp_buf = NULL;
	...
	struct ts_rawdata_info_new *info, int *rawdata)
{
	...
	for (i = 0; i < raw_data_size - cols; i++) {
		TS_LOG_DEBUG("delta row value is[%d, %d] %d > %d\n", i/cols, i%cols,
			pts_node->values[i],
			threshold->raw_data_row_delta_max_limits[i]);
		if (pts_node->values[i] >
			threshold->raw_data_row_delta_max_limits[i]) {
			testresult = CAP_TEST_FAIL_CHAR;
			strncpy(failedreason, "-panel_reason",
				TS_RAWDATA_FAILED_REASON_LEN - 1);
			TS_LOG_ERR("delta row value is[%d, %d] %d > %d\n",
				i/cols, i%cols, pts_node->values[i],
				threshold->raw_data_row_delta_max_limits[i]);
		}
	}
	for (i = 0; i < raw_data_size - rows; i++) {
		TS_LOG_DEBUG("delta col value is[%d, %d] %d > %d\n", i/(cols - 1), i%(cols - 1),
			pts_node->values[i + raw_data_size],
			threshold->raw_data_col_delta_max_limits[i]);
		if (pts_node->values[i + raw_data_size] >
			threshold->raw_data_col_delta_max_limits[i]) {
			testresult = CAP_TEST_FAIL_CHAR;
			strncpy(failedreason, "-panel_reason",
				TS_RAWDATA_FAILED_REASON_LEN - 1);
			TS_LOG_ERR("delta col value is[%d, %d] %d > %d\n",
				i/(cols - 1), i%(cols - 1), pts_node->values[i + raw_data_size],
				threshold->raw_data_col_delta_max_limits[i]);
		}
	}
...
static int touch_driver_predecode_test(void)
{
	...
	for (i = 0 ; i < 3 ; i++) {
		...
		retval = touch_driver_testing_read(tcm_hcd, g_temp_buf,
			(G_TEMP_BUF_LEN * sizeof(*g_temp_buf)));
		...
		if (packsize == cols * rows) {
			/* flip the 2D data */
			touch_driver_flip_2D_data(&g_temp_buf[4], packsize*2);
	...
	struct ts_rawdata_info_new *info)
{
	...
	/* flip the abs data */
	touch_driver_flip_abs_data(&temp_buf[MESSAGE_HEADER_SIZE], data_length, 2);
	...
	for (row = 0; row < rows; row++) {
		...
		if (noise_data_min > temp_buf_row_int_p[row])
			noise_data_min = temp_buf_row_int_p[row];
		if (noise_data_max < temp_buf_row_int_p[row])
			noise_data_max = temp_buf_row_int_p[row];
		noise_data_avg = (temp_buf_row_int_p[row]+noise_data_avg);
...
int tskit_driver_cap_test_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	g_temp_buf = kzalloc(G_TEMP_BUF_LEN * sizeof(*g_temp_buf), GFP_KERNEL);
	if (!g_temp_buf) {
		TS_LOG_ERR("Failed to allocate memory for g_temp_buf\n");
		return -ENOMEM;
	}
	if (!test_hcd) {
	...
	test_hcd->tcm_hcd = tcm_hcd;
	test_hcd->flip_type = TOUCH_DATA_XY_SWAP;
