...
//#include <honor_platform/inputhub/thphub.h>
#endif
...
#define REFLASH_IMAGE_NAME "ts/H008A31300.img"
#define SYNA_VENDOR_NAME "synaptics_tcm"
#define FW_NAME_MAX_LEN 50
#define TS_GAME_TYPE 2
...
static int tskit_driver_gesture_mode_config(bool en, unsigned short mode);
...
static int tskit_driver_lowpower_ctrl(unsigned int status);
...
struct ts_device_ops ts_kit_syna_tcm_ops = {
	...
	.ts_lowpower_ctrl = tskit_driver_lowpower_ctrl,
};

#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
static struct timeval kit_lowpower_switch_time;
#endif

#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
int tskit_spi_irq_suspend_flag;
EXPORT_SYMBOL(tskit_spi_irq_suspend_flag);
#endif

static int tskit_driver_lowpower_ctrl(unsigned int state)
{
	int retval;
	bool lowpower_switch;
#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
	struct timeval lowpower_receive;
	long delta_time;
#endif

	TS_LOG_INFO("%s: called state = %u\n", __func__, state);
	if (tcm_hcd->syna_lowpower_mode == state) {
		TS_LOG_INFO("%s:don't repeat old status %u\n",
			__func__, state);
		return 0;
	}

	if (state)
		lowpower_switch = true;
	else
		lowpower_switch = false;
#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
	kit_get_timestamp(&lowpower_receive);
	delta_time = ((lowpower_receive.tv_sec - kit_lowpower_switch_time.tv_sec) *
		1000000) + lowpower_receive.tv_usec -
		kit_lowpower_switch_time.tv_usec;
	delta_time /= 1000;
	if (delta_time < 10) {
		TS_LOG_INFO("%s: lowpower switch too fast [%ld ms]\n",
			__func__, delta_time);
		kit_get_timestamp(&kit_lowpower_switch_time);
		mdelay(10);
	}
#endif
	retval = tskit_driver_sleep(tcm_hcd, lowpower_switch);
	if (retval < 0) {
		TS_LOG_ERR("%s set sleep fail\n", __func__);
		goto exit;
	}
	tcm_hcd->syna_lowpower_mode = state;
...
static int tskit_driver_hbp_gesture_mode_config(bool en)
{
	int retval = NO_ERR;
	...
	TS_LOG_INFO("%s + %d\n", __func__, en);
	out_buf[0] = C7_SUB_CMD_HBP_FP_EVENT_SWITCH;
	out_buf[1] = 0x02;
	if (en) {
		/* 0x10-single-tap, 0x20-double-tap */
	...
	resp_buf_size = 0;
	retval = tcm_hcd->write_message(tcm_hcd, CMD_MultiFunction, out_buf,
		sizeof(out_buf), &resp_buf_p, &resp_buf_size, &resp_length, NULL, 0);
	if (retval < 0) {
		TS_LOG_ERR("Failed to write command %s, 0x%02x 0x%02x 0x%02x\n",
			STR(C7_SUB_CMD_HBP_FP_EVENT_SWITCH),
...
static int tskit_driver_gesture_mode_config(bool en, unsigned short mode)
{
	int retval;
	unsigned char out_buf[3]; /* cmd length is 3 */
	unsigned char *resp_buf; /* response length is 10 */
	unsigned int resp_buf_size = 0;
	unsigned int resp_length = 0;
	int read_retry;
	...
	resp_buf = kzalloc(RESP_BUF_LEN * sizeof(*resp_buf), GFP_KERNEL); /* response length is 10 */
	if (!resp_buf) {
		TS_LOG_ERR("Failed to allocate memory for resp_buf\n");
		return -ENOMEM;
	}
...
static int tskit_driver_get_project_id(char *project_id)
{
...
#endif
	len = MIN(CHIP_INFO_LENGTH - 1, strlen(tcm_hcd->tcm_mod_info.project_id_string));
	memcpy(project_id, tcm_hcd->tcm_mod_info.project_id_string, len);
	...
	project_id[PROJECT_ID_FW_LEN + 1] = 0;
...
static void tskit_driver_dispatch_response_for_one_package(
	struct syna_tcm_hcd *tcm_hcd, char *in_buf)
{
	int retval = NO_ERR;
	...
	memcpy(tcm_hcd->resp.buf, &in_buf[MESSAGE_HEADER_SIZE],
		tcm_hcd->payload_length);
	tcm_hcd->resp.data_length = tcm_hcd->payload_length;
	UNLOCK_BUFFER(tcm_hcd->resp);
	atomic_set(&tcm_hcd->command_status, CMD_IDLE);
		...
		unsigned char *response_code, unsigned int polling_delay_ms)
{
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		...
		do {
			...
			if (retval > 0) {
				...
				if (atomic_read(&tcm_hcd->command_status) != CMD_BUSY) {
					/* maybe the other thread handle the response though */
					/* it is time out, so here break the loop */
					break;
				}
				timeout += polling_ms;
				reinit_completion(&response_complete);
	...
	if (tcm_hcd->response_code != STATUS_OK) {
		TS_LOG_ERR("response code 0x%02x command 0x%02x\n",
		...
		enum dynamic_config_id id, unsigned short *value)
{
	...
	resp_buf = kzalloc(RESP_BUF_LEN * sizeof(*resp_buf), GFP_KERNEL);
	if (!resp_buf) {
		TS_LOG_ERR("Failed to allocate memory for resp_buf\n");
		return -ENOMEM;
	}
		...
		enum dynamic_config_id id, unsigned short value)
{
	...
	if (resp_buf[1] != STATUS_OK)
		TS_LOG_ERR("Failed to write dynamic 0x%02x payload 0x%02x 0x%02x\n",
			out_buf[0], out_buf[1], out_buf[2]);
	retval = 0;
exit:
	return retval;
...
static int tskit_driver_comm_check(void)
{
	...
	unsigned char *identify_info;
	unsigned char marker = 0;

	if (TS_BUS_SPI == tcm_hcd->syna_tcm_chip_data->ts_platform_data->bops->btype) {
		uboot = kzalloc(UBOOT_LEN * sizeof(*uboot), GFP_KERNEL);
		if (!uboot) {
			TS_LOG_ERR("Failed to allocate memory for uboot\n");
			return -ENOMEM;
		}
		identify_info = kzalloc(IDENTIFY_INFO_LEN * sizeof(*identify_info), GFP_KERNEL);
		if (!identify_info) {
			TS_LOG_ERR("Failed to allocate memory for identify_info\n");
			return -ENOMEM;
		}
		retval = tskit_driver_rmi_read(tcm_hcd,
			PDT_START_ADDR, uboot, UBOOT_LEN);
...
static int tskit_driver_read_one_package(struct ts_fingers *info)
{
	...
	buffer = kzalloc(FIXED_READ_LENGTH * sizeof(*buffer), GFP_KERNEL);
	if (!buffer) {
		TS_LOG_ERR("Failed to allocate memory for buffer\n");
		return -ENOMEM;
	}
...
retry:
	...
	if (tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		if (tcm_hcd->payload_length > 0) {
			if (tcm_hcd->payload_length >= FIXED_READ_LENGTH - MESSAGE_HEADER_SIZE - 1) {
				retval = -1;
				TS_LOG_ERR("Payload length = %d is too big\n", tcm_hcd->payload_length);
				goto exit;
			}
			retval = tskit_driver_read(tcm_hcd,
	...
	if (tcm_hcd->status_report_code > REPORT_IDENTIFY) {
		...
		tskit_driver_dispatch_response_for_one_package(tcm_hcd, buffer);
	}
	retval = 0;
...
static int tskit_driver_suspend_for_gesture_mode(void)
{
	...
	if (dev->easy_wakeup_info.aod_mode == TS_TOUCH_AOD_OPEN)
		gesture_type |= CMD_SINGLE_TAP;
	if (dev->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE)
		gesture_type |= CMD_DOUBLE_TAP;
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909)
		return tskit_driver_hbp_gesture_mode_config(true);
	else
		retval = tskit_driver_gesture_mode_config(true, gesture_type);
	if (retval < 0) {
...
static int tskit_driver_resume_for_gesture_mode(void)
{
	...
	msleep(50); /* sleep 50ms to communication */
	return retval;
}
...
static int tskit_driver_resume(void)
{
...
#endif
	TS_LOG_INFO("%s pt_station_flag:%d\n", __func__, tskit_pt_station_flag);
	...
	if (!tskit_pt_station_flag) {
		...
		if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type ==
			S3909) {
			TS_LOG_INFO("resume pt mode reset\n");
			gpio_direction_output(tcm_hcd->syna_tcm_chip_data->ts_platform_data->reset_gpio,
				GPIO_OUTPUT_LOW);
			mdelay(5); /* sequential */
			gpio_direction_output(tcm_hcd->syna_tcm_chip_data->ts_platform_data->reset_gpio,
...
static int tskit_driver_after_resume(void *feature_info)
{
	...
	int retry = 3;
	/* size */
	static unsigned char *tmp_buffer; /* 64 :tmp buf size */
	...
	tmp_buffer = kzalloc(TMP_BUFFER_LEN * sizeof(*tmp_buffer), GFP_KERNEL);
	if (!tmp_buffer) {
		TS_LOG_ERR("Failed to allocate memory for tmp_buffer\n");
		return -ENOMEM;
	}
	read_buffer = kzalloc(FIXED_READ_LENGTH * sizeof(*read_buffer), GFP_KERNEL);
	if (!read_buffer) {
		TS_LOG_ERR("Failed to allocate memory for read_buffer\n");
		return -ENOMEM;
	}
	if (g_ts_kit_platform_data.udfp_enable_flag) {
		/* sequential */
...
static int tskit_driver_get_fw_prefix(void)
{
	...
	strncat(fwname, &joint_chr, sizeof(char));
	TS_LOG_DEBUG("%s fw name prefix:%s", __func__, fwname);
...
data_release:
			...
			retval = generate_fw_name(force, fw_name, sizeof(fw_name));
			if (retval < 0)
					...
					struct ts_cmd_node *out_cmd)
{
...
#if (IS_ENABLED(CONFIG_TP_QCOM_8450))
	if (tcm_hcd->in_suspend) {
		tskit_spi_irq_suspend_flag = 1;
		TS_LOG_INFO("tskit_spi_irq_suspend_flag = 1\n");
	}
#endif
	if (!tcm_hcd->in_suspend) {
		...
		TS_LOG_DEBUG("in suspend, don't report daemon\n");
	}
	...
	if (tcm_hcd->status_report_code <= REPORT_IDENTIFY) {
		out_cmd->command = TS_INVAILD_CMD;
		TS_LOG_INFO("response data, don't report\n");
	}
...
static int tskit_driver_input_config(struct input_dev *input_dev)
{
	...
	set_bit(TS_SINGLE_CLICK, input_dev->keybit);
...
regulator_retry:
	...
	if (retval < 0) {
		...
		while (retry < 10) {
			TS_LOG_INFO("tskit_driver_get_regulator retry\n");
			msleep(500);
			retry++;
			goto regulator_retry;
		}
		goto err_alloc_mem;
	}
...
static void tskit_driver_chip_touch_switch(void)
{
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		if (stype == TS_SWITCH_SCENE_5) { /* game mode */
			if (soper == TS_SWITCH_GAME_ENABLE) {
				error = tskit_driver_set_dynamic_config(tcm_hcd,
					DC_GAME_MODE_CTRL, TS_GAME_TYPE);
				if (error < 0)
		...
		} else if (stype == TS_SWITCH_SCENE_8) {
			if (soper == TS_SWITCH_GAME_ENABLE) {
				error = tskit_driver_set_dynamic_config(tcm_hcd,
					DC_GAME_MODE_CTRL, 1);
				if (error < 0)
...
int syna_tcm_driver_module_init(void)
{
	int retval = NO_ERR;
	bool found = false;
	...
	retval = honor_ts_chip_register(tcm_hcd->syna_tcm_chip_data);
	if(retval)
	{
