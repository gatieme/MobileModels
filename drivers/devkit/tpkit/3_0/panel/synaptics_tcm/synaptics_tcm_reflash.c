...
#define TMP_RESPONSE_LEN 100

#define FLASH_READ_DATA_LEN 160

#define BOOT_CONFIG_DATA_LEN 128

#define WRITE_FLASH_RESPONSE_LEN 25

#define ERASE_FLASH_RESPONSE_LEN 10
...
static unsigned char *flash_read_data;
static unsigned char *boot_config_data;
...
static int reflash_set_up_flash_access(void)
{
	...
	msleep(50);
	retval = tskit_driver_read(tcm_hcd, tmp_response, TMP_RESPONSE_LEN);
	...
	count = MIN(sizeof(tcm_hcd->id_info), TMP_RESPONSE_LEN);
	if ((count > TMP_RESPONSE_LEN) ||
	...
	memcpy((unsigned char *)&tcm_hcd->id_info,
		...
		MIN(sizeof(tcm_hcd->id_info), TMP_RESPONSE_LEN));
	...
	if (tmp_response[1] == STATUS_OK) {
		memcpy((unsigned char *)&tcm_hcd->boot_info,
			&tmp_response[MESSAGE_HEADER_SIZE],
			MIN(sizeof(tcm_hcd->boot_info), TMP_RESPONSE_LEN));
	} else {
		...
		unsigned int datalen)
{
	...
	response = kzalloc(WRITE_FLASH_RESPONSE_LEN, GFP_KERNEL);
	if (!response) {
		TS_LOG_ERR(
		...
		if (tcm_hcd->bdata &&
			...
			do {
				...
				retval = tskit_driver_read(tcm_hcd, response,
					WRITE_FLASH_RESPONSE_LEN);
				if (retval < 0 || (response[1] != STATUS_OK &&
					response[1] != STATUS_IDLE && response[1] != 0xFF)) {
					TS_LOG_ERR("Failed to get write response\n");
					TS_LOG_ERR("response[0] %x, %x, %x, %x, %x\n",
...
static int reflash_erase_flash(unsigned int page_start, unsigned int page_count)
{
	...
	response = kzalloc(ERASE_FLASH_RESPONSE_LEN, GFP_KERNEL);
	if (!response) {
		TS_LOG_ERR(
				"Failed to allocate memory for response\n");
		return -ENOMEM;
		...
		unsigned int datalen)
{
	...
	msleep(50);
	retval = tskit_driver_read(tcm_hcd,
		flash_read_data, FLASH_READ_DATA_LEN);
	if (retval < 0) {
	...
	if (datalen > FLASH_READ_DATA_LEN - MESSAGE_HEADER_SIZE) {
...
int reflash_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	tmp_response = kzalloc(TMP_RESPONSE_LEN, GFP_KERNEL);
	if (!tmp_response) {
		TS_LOG_ERR(
				"Failed to allocate memory for tmp_response\n");
		return -ENOMEM;
	}
	flash_read_data = kzalloc(FLASH_READ_DATA_LEN, GFP_KERNEL);
	if (!flash_read_data) {
		TS_LOG_ERR(
				"Failed to allocate memory for flash_read_data\n");
		return -ENOMEM;
	}
	boot_config_data = kzalloc(BOOT_CONFIG_DATA_LEN, GFP_KERNEL);
	if (!boot_config_data) {
		TS_LOG_ERR(
				"Failed to allocate memory for boot_config_data\n");
		return -ENOMEM;
	}
...
//	tcm_hcd->read_flash_data = reflash_read_data;
	...
	if (tcm_hcd->htd == false) {
		...
		reflash_read_boot_config(boot_config_data, BOOT_CONFIG_DATA_LEN);
		reflash_parse_boot_config(boot_config_data,
				BOOT_CONFIG_DATA_LEN,
