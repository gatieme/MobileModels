...
#define ENABLE_TOUCH_RESPONSE_LEN 4
#define ENABLE_TOUCH_RESPONSE_TEMP_LEN 1024
#define APP_INFO_LEN 50
#define ID_REPORT_LEN 30
#define GEST_DATA_LEN 6
#define SYN_BYTE_COUNT 8
...
enum touch_report_code {
	...
	TOUCH_NUM_OF_CPU_CYCLES_USED_SINCE_LAST_FRAME, //25
	TOUCH_GESTURE_DATA = 0x1B, //27
	...
	TOUCH_BASELINE_STATUS = 0xCE,
	TOUCH_SIGNAL_INFO = 0xCF,
	TOUCH_FW_STATUS = 0xD0,
};

enum gesture_id {
	NO_GESTURE_DETECTED = 0,
	GESTURE_SINGLE_TAP = 0x01,
	GESTURE_DOUBLE_TAP = 0x02,
};
...
struct gesture_data {
	unsigned char data[GEST_DATA_LEN];
};
...
struct touch_data {
	...
	struct baseline_status baseline_status;
		...
		unsigned int bits, unsigned int *data)
{
	...
	if (bits == 0 || bits > 32) {
		TS_LOG_ERR("Invalid number of bits：%u\n", bits);
		return -EINVAL;
	}
...
static int touch_get_report_data_morethan_32bits(unsigned int in_offset,
	unsigned int in_bits, unsigned int byte_cnt, unsigned char *temp_data)
{
	int retval = NO_ERR;
	unsigned int left_bits;
	unsigned int i = 0;
	unsigned int data = 0;
	unsigned int bits = in_bits;
	unsigned int offset = in_offset;

	if (bits > (byte_cnt * SYN_BYTE_COUNT)) {
		TS_LOG_ERR("Incorrect Data length:%d\n", bits);
		return -EINVAL;
	}
	if (!temp_data) {
		TS_LOG_ERR("temp_data is NULL\n");
		return -EINVAL;
	}

	left_bits = bits;
	while (left_bits) {
		bits = (left_bits >= SYN_BYTE_COUNT) ? SYN_BYTE_COUNT : left_bits;
...
static int touch_parse_report(void)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_GESTURE_DATA:
			...
			retval = touch_get_report_data_morethan_32bits(offset, bits,
				sizeof(struct gesture_data), temp_data);
			if (retval < 0) {
				TS_LOG_ERR("Failed to get number of gesture data\n");
				return retval;
			}
			offset += bits;
			TS_LOG_DEBUG("Gesture Data: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
		...
		case TOUCH_NUM_OF_ACTIVE_OBJECTS:
			...
			if (touch_data->num_of_active_objects == 0) {
				if (end_of_foreach == 0) {
					TS_LOG_ERR("Invalid report, num_active and end_foreach are 0\n");
					return 0;
				}
				idx = end_of_foreach;
			}
		...
		case TOUCH_ROI_DATA:
			...
			if (offset >= (FIXED_READ_LENGTH - MESSAGE_HEADER_SIZE - 1) * 8) {
				TS_LOG_ERR("offset: %d is too big\n", offset);
				return -EINVAL;
			}
		...
		case TOUCH_ESD_DETECT:
			...
			offset += bits;
			break;
		case TOUCH_BASELINE_STATUS:
			bits = config_data[idx++];
			temp_data = (unsigned char *)&touch_data->baseline_status;
			retval = touch_get_report_data_morethan_32bits(offset, bits,
				sizeof(struct baseline_status), temp_data);
			if (retval < 0) {
				TS_LOG_ERR("Failed to get number of baseline status\n");
				return retval;
			}
			offset += bits;
			break;
		case TOUCH_SIGNAL_INFO:
			bits = config_data[idx++];
			temp_data = (unsigned char *)&touch_data->signal_info;
			retval = touch_get_report_data_morethan_32bits(offset, bits,
				sizeof(struct signal_info), temp_data);
			if (retval < 0) {
				TS_LOG_ERR("Failed to get number of signal info\n");
				return retval;
			}
			offset += bits;
			break;
		case TOUCH_FW_STATUS:
			bits = config_data[idx++];
			retval = touch_get_report_data(offset, bits, &data);
			if (retval < 0) {
				TS_LOG_ERR("Failed to get FW status\n");
				return retval;
			}
			touch_data->fw_status = data;
			offset += bits;
			break;
		default:
			bits = config_data[idx++];
...
void touch_report(struct ts_fingers *info)
{
	...
	if (retval < 0) {
		...
		TS_LOG_INFO("config_data = ");
		for (i = 0; i < config_size; i++)
			TS_LOG_INFO("%d ", config_data[i]);
		goto exit;
	}

	if (tcm_hcd->in_suspend) {
		if (touch_data->gesture_id)
			TS_LOG_INFO("in suspend,touch report in gesture_id = %d\n",
				touch_data->gesture_id);
		if (touch_data->gesture_id == GESTURE_DOUBLE_TAP)
			ts_inputkey_report(TS_DOUBLE_CLICK);
		if (touch_data->gesture_id == GESTURE_SINGLE_TAP)
			ts_aod_click_report(TS_SINGLE_CLICK);
		goto exit;
	}
	...
	if ((NEED_REPORT == touch_report_fw && SYNA_FW_SUPPORT_ESD == tcm_hcd->device_status_check) || (!tcm_hcd->device_status_check)) {
		...
		info->fingers_debug_info.fw_status = touch_data->fw_status;
		info->fingers_debug_info.power_im = touch_data->power_im;
		info->fingers_debug_info.frequency = touch_data->nsm_frequency;
		info->fingers_debug_info.ojbect_count =
			touch_data->num_of_active_objects;
		memcpy(&info->fingers_debug_info.baseline_status,
			&touch_data->baseline_status, sizeof(struct baseline_status));
		memcpy(&info->fingers_debug_info.signal_info,
			&touch_data->signal_info, sizeof(struct signal_info));
	}
...
static int tskit_driver_get_app_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	resp_buf = NULL;
	resp_buf_size = 0;
	app_info = kzalloc(APP_INFO_LEN, GFP_KERNEL);
	if (!app_info) {
		TS_LOG_ERR(
				"Failed to allocate memory for app_info\n");
		return -ENOMEM;
	}
get_app_info:
...
static int tskit_driver_get_id_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	unsigned int resp_buf_size = 0;
	unsigned int resp_length = 0;
	unsigned char *id_report;

	resp_buf = NULL;
	resp_buf_size = 0;
	id_report = kzalloc(ID_REPORT_LEN, GFP_KERNEL);
	if (!id_report) {
		TS_LOG_ERR(
				"Failed to allocate memory for id_report\n");
		return -ENOMEM;
	}
	retval = tskit_driver_write_hdl_message(tcm_hcd,
			CMD_IDENTIFY,
...
int tskit_driver_enable_touch(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	unsigned char *response_temp_buf;
	unsigned char *resp_buf = NULL;
	unsigned int resp_buf_size = 0;
	unsigned int resp_length = 0;
	unsigned char cmd = 0;
	...
	response_buf = kzalloc(ENABLE_TOUCH_RESPONSE_LEN, GFP_KERNEL);
	if (response_buf == NULL) {
		TS_LOG_ERR("Failed to kzalloc response_buf\n");
		goto exit;
	}
	response_temp_buf = kzalloc(ENABLE_TOUCH_RESPONSE_TEMP_LEN, GFP_KERNEL);
	if (response_temp_buf == NULL) {
		TS_LOG_ERR("Failed to kzalloc response_temp_buf\n");
		goto exit;
	}
	if (en)
