...
#define SYNA_SPEC_HDL_DELAY_AFTER_LCD 10
#define STATUS_REPORT_LEN 10
#define TEST_BUF_LEN 30
...
int tskit_driver_write_hdl_message(struct syna_tcm_hcd *tcm_hcd,
...
static int zeroflash_download_app_fw(void)
{
	...
	retval = tskit_driver_rmi_write(tcm_hcd,
...
int zeroflash_download_app_firmware(char *file_name)
{
	...
	testbuf = kzalloc(TEST_BUF_LEN, GFP_KERNEL);
	if (!testbuf) {
		TS_LOG_ERR("Failed to allocate memory for testbuf\n");
		return -ENOMEM;
	}
...
int check_report_status(void)
{
	...
	status_report = kzalloc(STATUS_REPORT_LEN, GFP_KERNEL);
	if (!status_report) {
		TS_LOG_ERR("Failed to allocate memory for status_report\n");
		return -ENOMEM;
	}
	while(retry) {
		retval = tskit_driver_read(zeroflash_hcd->tcm_hcd,
				status_report,
				STATUS_REPORT_LEN);
		if (retval < 0) {
...
int zeroflash_download(char *file_name, struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->syna_spec_delay_after_lcd)
		mdelay(SYNA_SPEC_HDL_DELAY_AFTER_LCD);
	...
	while (retry) {
		...
		if (!zeroflash_hcd->fw_status.need_app_config & !zeroflash_hcd->fw_status.need_disp_config) {
			if (tcm_hcd->support_cold_patch) {
				zeroflash_hcd->tcm_hcd->packrat_number_old =
					zeroflash_hcd->tcm_hcd->packrat_number;
				zeroflash_hcd->tcm_hcd->packrat_number =
					zeroflash_hcd->image_info.packrat_number;
			}
...
exit:
	...
	if (tcm_hcd->support_cold_patch && zeroflash_hcd->fw_entry) {
		release_firmware(zeroflash_hcd->fw_entry);
		zeroflash_hcd->fw_entry = NULL;
	}
	return retval;
