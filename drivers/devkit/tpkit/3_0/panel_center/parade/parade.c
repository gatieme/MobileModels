...
static void parade_check_irq_flag(void);
...
static DEFINE_MUTEX(g_exclusive_lock);
static int g_sleep_resume_flag;
static int g_irq_sensorhub_flag;
...
#define PARADE_CMD_SINGLE_DOUBLE_CLICK   (0x03)
#define PARADE_CMD_SINGLE_CLICK          (0x02)
#define PARADE_CMD_DOUBLE_CLICK          (0x01)
...
static void parade_check_irq_flag(void)
{
	...
	if (g_irq_sensorhub_flag) {
		ts_set_irq_status(&g_ts_kit_platform_data, PARADE_IRQ_ENABLE);
#ifdef CONFIG_HUAWEI_SHB_THP
		rc = send_thp_driver_status_cmd(POWER_ON,
			tskit_parade_data->parade_chip_data->ts_platform_data->multi_panel_index,
			ST_CMD_TYPE_MULTI_TP_UD_STATUS);
		if (rc < 0)
			TS_LOG_ERR("send cmd to sensorhub failed\n");
#endif
		mdelay(PARADE_DELAY_100MS);
		parade_pinctrl_select_normal();
		TS_LOG_INFO("%s: abnormal pinctrl\n", __func__);
	}
}
...
static void parade_enter_easy_wakeup(struct work_struct *work)
{
	...
	TS_LOG_INFO("%s: easy_wakeup_mode in\n", __func__);

	if (cd->gesture_mask <= 0 || cd->gesture_mask > PARADE_CMD_SINGLE_DOUBLE_CLICK) {
		TS_LOG_ERR("%s, Invalid gesture mask: %d, reset to %d\n", __func__,
			write_buf[0], PARADE_CMD_SINGLE_CLICK);
		cd->gesture_mask = PARADE_CMD_SINGLE_CLICK;
	}
	write_buf[0] = cd->gesture_mask;
...
#ifdef CONFIG_HUAWEI_SHB_THP
	rc = send_thp_driver_status_cmd(POWER_OFF,
		tskit_parade_data->parade_chip_data->ts_platform_data->multi_panel_index,
		ST_CMD_TYPE_MULTI_TP_UD_STATUS);
	if (rc < 0)
		TS_LOG_ERR("send cmd to sensorhub failed\n");
#endif
	ts_set_irq_status(&g_ts_kit_platform_data, PARADE_IRQ_DISABLE);
	parade_finish_cmd();
	TS_LOG_INFO("%s: easy_wakeup_mode out\n", __func__);
}
...
static void parade_deep_sleep_work_fn(struct work_struct *work)
{
	...
	if (g_sleep_resume_flag == POWER_DEEP_SLEEP) {
		...
		ts_set_irq_status(&g_ts_kit_platform_data, PARADE_IRQ_DISABLE);
		parade_finish_cmd();
		TS_LOG_INFO("%s: deep_sleep out\n", __func__);
		return;
	} else {
		if (cd->parade_chip_data->easy_wakeup_info.sleep_mode != TS_GESTURE_MODE &&
			cd->parade_chip_data->easy_wakeup_info.aod_mode != TS_TOUCH_AOD_OPEN) {
			TS_LOG_INFO("%s: resume in\n", __func__);
			rc = parade_check_cmd_status();
			if (rc) {
				return;
			}
			...
			parade_finish_cmd();
			TS_LOG_INFO("%s: resume out\n", __func__);
			return;
		}
		TS_LOG_INFO("%s: gesture resume in\n", __func__);
		rc = parade_check_cmd_status();
		ts_set_irq_status(&g_ts_kit_platform_data, PARADE_IRQ_ENABLE);
#ifdef CONFIG_HUAWEI_SHB_THP
		rc = send_thp_driver_status_cmd(POWER_ON,
		...
		parade_finish_cmd();
		TS_LOG_INFO("%s: gesture resume out\n", __func__);
	}
...
static int parade_core_suspend(void)
{
	...
	switch (cd->parade_chip_data->easy_wakeup_info.sleep_mode ||
		cd->parade_chip_data->easy_wakeup_info.aod_mode) {
		...
		case TS_GESTURE_MODE: /* gesture easy_wake_up mode. */
			if (cd->core_pdata->wakeup_gesture_enable == 0) {
				...
				if (cd->parade_chip_data->easy_wakeup_info.aod_mode &&
					cd->parade_chip_data->easy_wakeup_info.sleep_mode)
					cd->gesture_mask = PARADE_CMD_SINGLE_DOUBLE_CLICK;
				else if (!cd->parade_chip_data->easy_wakeup_info.aod_mode &&
					cd->parade_chip_data->easy_wakeup_info.sleep_mode)
					cd->gesture_mask = PARADE_CMD_DOUBLE_CLICK;
				else if (cd->parade_chip_data->easy_wakeup_info.aod_mode &&
					!cd->parade_chip_data->easy_wakeup_info.sleep_mode)
					cd->gesture_mask = PARADE_CMD_SINGLE_CLICK;
				TS_LOG_INFO("%s: enter easy wakeup mode , gesture mask = %d\n", __func__, cd->gesture_mask);
				mutex_lock(&cd->system_lock);
				tskit_parade_data->parade_chip_data->easy_wakeup_info.off_motion_on = true;
				mutex_unlock(&cd->system_lock);
	...
	struct ts_kit_device_data *chip_data)
{
	...
	/* vendor name */
	rc = of_property_read_string(core_node, "vendor_name", &chip_data->vendor_name);
	if (rc) {
		TS_LOG_ERR("%s, vendor_name read failed:%d\n", __func__, rc);
		goto fail_free;
	}
	if (chip_data->vendor_name == NULL) {
		goto fail_free;
	}
	TS_LOG_INFO("%s, vendor_name %s\n", __func__, chip_data->vendor_name);
	...
	struct ts_cmd_node *out_cmd, struct parade_core_data *cd)
{
	...
	if (report_id != cd->sysinfo.desc.tch_report_id &&
			...
			report_id != HID_BASELINE_UPDATE_REPORT_ID) {
		is_command = 1;
	}
#else
	if (report_id != cd->sysinfo.desc.tch_report_id &&
		...
		struct result *result, bool *pass, int test_item)
{
	...
	if (test_item) {
		...
		for (i = 0; i < sensor_num; i++) {
			...
			cm_sensor_min = configuration->cm_min_max_table_sensor[(row * tx_num + col) * 2];
			cm_sensor_max = configuration->cm_min_max_table_sensor[(row * tx_num + col) * 2 + 1];
...
static int parade_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	tskit_parade_data->process_state = PT_STATE_INIT;
	tskit_parade_data->parade_chip_data->is_i2c_one_byte = 1;
