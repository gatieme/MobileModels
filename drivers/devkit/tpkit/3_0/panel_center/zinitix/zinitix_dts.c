		...
		struct ts_kit_device_data *dev_data)
{
	...
	ret = of_property_read_u32(np, ZINITIX_CHANNEL_X_NUM, &dev_data->channel_x_num);
	if (ret) {
		TS_LOG_ERR("%s:get device channel_x_num fail, ret=%d\n",
				__func__, ret);
		dev_data->channel_x_num = ZINITIX_CHANNEL_X_NUMBER_DEFAULT;
	}

	ret = of_property_read_u32(np, ZINITIX_CHANNEL_Y_NUM, &dev_data->channel_y_num);
	if (ret) {
		TS_LOG_ERR("%s:get device channel_y_num fail, ret=%d\n",
				__func__, ret);
		dev_data->channel_y_num = ZINITIX_CHANNEL_Y_NUMBER_DEFAULT;
	}
	...
	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d\n", __func__,
			...
			"channel_x_num", dev_data->channel_x_num,
			"channel_y_num", dev_data->channel_y_num);
		...
		struct ts_kit_device_data *dev_data)
{
	...
	ret = of_property_read_u32(np, "support_pinctrl",
		&zinitix_pdata->support_pinctrl);
	TS_LOG_INFO("%s:support_pinctrl = %d\n", __func__, zinitix_pdata->support_pinctrl);

	ret = of_property_read_string(np, "vendor_name", &dev_data->vendor_name);
	TS_LOG_INFO("%s:vendor_name = %s\n", __func__, dev_data->vendor_name);
		...
		struct zinitix_platform_data *zinitix_pdata)
{
	...
	// need_change_esd_check_times default is 3
	touch_driver_of_property_read_u32_default(np, ZINITIX_ESD_FAIL_CHECK_TIME,
		&zinitix_pdata->need_change_esd_check_times, 3);
	TS_LOG_INFO("%s:need_change_esd_check_times =%d\n", __func__,
		zinitix_pdata->need_change_esd_check_times);

	// support_finger_number default is 2
	touch_driver_of_property_read_u32_default(np, ZINITIX_MAX_SUPPORTED_FINGER_NUM,
		&zinitix_pdata->support_finger_number, 2);
	TS_LOG_INFO("%s:support_finger_number =%d\n", __func__,
		zinitix_pdata->support_finger_number);

	touch_driver_of_property_read_u32_default(np, ZINITIX_CLOSE_COORDINATE_ROTATION,
			&zinitix_pdata->zinitix_device_data->close_coordinate_rotation, 0);
	if (zinitix_pdata->zinitix_device_data->close_coordinate_rotation) {
		zinitix_pdata->zinitix_device_data->config_x_max = ZINITIX_DEFAULT_ZERO;
		zinitix_pdata->zinitix_device_data->config_y_max = ZINITIX_DEFAULT_ZERO;
	} else {
		TS_LOG_INFO("%s get close_coordinate_rotation failed, use default\n", __func__);
		zinitix_pdata->zinitix_device_data->config_x_max =
			zinitix_pdata->zinitix_device_data->x_max - 1;
		zinitix_pdata->zinitix_device_data->config_y_max =
			zinitix_pdata->zinitix_device_data->y_max - 1;
	}

	TS_LOG_INFO("%s:glove_supported=%d,glove_switch_addr = 0x%04x,\
		holster_supported = %d,holster_switch_addr = 0x%04x,\
		touch_switch_flag = %d, close_coordinate_rotation = %d\n",\
			__func__,
			...
			zinitix_pdata->zinitix_device_data->close_coordinate_rotation);
