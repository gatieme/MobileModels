...
static struct platform_device honor_ts_kit_color = {
	.name = "honor_ts_kit_color",
	.id = -1,
};

#if (defined CONFIG_HONOR_THP_MTK) || \
	(defined CONFIG_HONOR_DEVKIT_MTK_3_0)
static struct device_attribute tp_color_file = __ATTR(tp_color_info,
	0660, attr_get_tp_color_info, attr_set_tp_color_info);

static struct platform_device honor_tp_color = {
	.name = "honor_tp_color",
	.id = -1,
};
#endif
...
static int __init ts_color_info_init(void)
{
	...
	ret = platform_device_register(&honor_ts_kit_color);
	if (ret) {
		hwlog_err("%s: platform_device_register failed, ret:%d.\n",
			__func__, ret);
		goto REGISTER_ERR;
	}
	if (device_create_file(&honor_ts_kit_color.dev, &ts_kit_color_file)) {
...
#if (defined CONFIG_HONOR_THP_MTK) || \
	(defined CONFIG_HONOR_DEVKIT_MTK_3_0)
	ret = platform_device_register(&honor_tp_color);
	if (ret) {
		hwlog_err("%s: platform_device_register failed, ret: %d\n",
			__func__, ret);
		goto register_dev_err;
	}
	if (device_create_file(&honor_tp_color.dev, &tp_color_file)) {
		hwlog_err("%s: Unable to create interface\n", __func__);
...
SYSFS_CREATE_FILE_ERR:
	platform_device_unregister(&honor_ts_kit_color);
REGISTER_ERR:
	return ret;
...
sysfs_create_tpcolor_file_err:
	platform_device_unregister(&honor_tp_color);
register_dev_err:
	return ret;
#endif
}
...
MODULE_AUTHOR("honor driver group of k3");
MODULE_LICENSE("GPL");
