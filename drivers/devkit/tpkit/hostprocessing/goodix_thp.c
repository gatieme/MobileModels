...
#define CMD_UDFP_EVENT_SWITCH_GT9896 0xB6
#define CMD_AOD_EVENT_SWITCH_GT9896 0xB7
#define CMD_TOUCH_GESTURE_SWITCH_GT9896 0xAD
#define CMD_FRAME_DATA_GT9896 0x90
#define CMD_TOUCH_DATA_GT9896 0x91
...
#define BT_ADSORPTION_BUF2 0x05
#define BT_ADSORPTION_BUF3 0xBB
#define BT_ADSORPTION_BUF4 0x01
#define BT_ADSORPTION_BUF5 0xC1
#define SEND_BT_CONNECT_STATUS_AFTER_RESET_DELAY 200
#define BT_CONNECT_STATUS_CHECK_ADDR 0x10169
#define BT_CONNECT_STATUS_CHECK_ADDR_GT9966 0x10219
#define BT_CONNECT_CMD_RETRY 3
#define BT_CHECK_DATA_LEN 1
#define BT_CONNECT_VALUE 1
#define BT_DISCONNECT_VALUE 0
#define BT_ADSORPTION_VALUE 2
#define BT_AFTER_WRITE_CMD_DELAY 20
#define BT_STYLUS3_CONNECTED_MASK (1 << 1)
#define STYLUS3_ADSORPTION_MASK (1 << 5)
...
#define GOODIX_FRAME_LEN_MAX_GT9966 5120
#define GOODIX_READ_WRITE_BYTE_OFFSET_GT9966 8
#define GOODIX_GESTURE_ADDR_DEFAULT_GT9966 0x10274
#define AFTER_IOVDD_DELAY_9966 12
#define GOODIX_PID_LEN_GT9966 8
#define GOODIX_MASK_VID_LEN_GT9966 3
#define GOODIX_VID_LEN_GT9966 4
...
#define DISABLE_FRAME_WAIT_DELAY 16
...
#define GOODIX_CMD_RAWDATA 0x90
#define WORK_IN_SUSPEND 1
#define POWER_OFF_IN_SUSPEND 0
#define TAS_EVENT_TO_UDFP_MODE_TWO 2

/* for read sn */
#define GOODIX_CMD_ADDR_FLASH_GT9966 0x13B74
#define GOODIX_CMD_ADDR_FLASH_READ_GT9966 0x13B80
#define SN_READ_WAIT_TIME_FOR_GTX9966 5
#define SN_READY_BIT_FOR_GTX9966 0
#define SN_FIRST_WRITE_READY_VALUE 0x04
#define SN_SECOND_WRITE_READY_VALUE 0x07
#define SN_AREA_MAX_LEN 32
#define SN_READ_MAX_RETRY_TIMES 15
#define SN_READ_REC_DATA_LEN 5
#pragma pack(1)
...
static u8 bt_connect_buf[BT_CONNECT_STATUS_BUF_LEN] = {
	0, 0, BT_CONNECT_BUF2, BT_CONNECT_BUF3,
	BT_CONNECT_BUF4, BT_CONNECT_BUF5, 0
};
static u8 bt_disconnect_buf[BT_CONNECT_STATUS_BUF_LEN] = {
	0, 0, BT_DISCONNECT_BUF2, BT_DISCONNECT_BUF3,
	0, BT_DISCONNECT_BUF5, 0
};
static u8 bt_adsorption_buf[BT_CONNECT_STATUS_BUF_LEN] = {
	0, 0, BT_ADSORPTION_BUF2, BT_ADSORPTION_BUF3,
	BT_ADSORPTION_BUF4, BT_ADSORPTION_BUF5, 0
};
...
struct gesture_info gesture_info_tab_for_9897_dual[] = {
	{ GESTURE_TYPE_DOUBLE_TAP_GT9897_DUAL, TS_DOUBLE_CLICK },
	{ GESTURE_TYPE_PEN_SINGLE_TAP_GT9897_DUAL, TS_STYLUS_WAKEUP_TO_MEMO }
};
...
static char *tx_frame_buff;
static char *rx_frame_buff;
...
static int touch_driver_spi_read_check(struct thp_device *tdev, unsigned int len)
{
	if (tdev->thp_core->support_vendor_ic_type == GT9897) {
		if ((len + GOODIX_READ_WRITE_BYTE_OFFSET_GT9897) >
			THP_MAX_FRAME_SIZE) {
			thp_log_err("%s:invalid len:%u\n", __func__, len);
			return -EINVAL;
		}
...
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
static int touch_driver_spi_read_retry(struct thp_device *tdev,
	struct spi_device *spi, struct spi_message *spi_msg)
{
	int retry = 0;
	int ret = 0;
...
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
	ret = touch_driver_spi_read_retry(tdev, spi, spi_msg);
	if (ret < 0) {
		thp_log_err("%s:get retry failed:%d\n", __func__, ret);
		return ret;
	}
#else
	ret = touch_driver_spi_sync(tdev, spi, spi_msg);
	if (ret < 0) {
		thp_log_err("%s:get spi sync failed:%d\n", __func__, ret);
		return ret;
	}
#endif
	return ret;
...
static void touch_driver_spi_copy_data(struct thp_device *tdev, u8 *rx_buf,
	u8 *data, unsigned int len)
{
	...
	u8 *data, unsigned int len)
{
	...
	check_flag = ((tdev == NULL) || (data == NULL) || (tdev->tx_buff == NULL) ||
	...
	ret = touch_driver_spi_read_check(tdev, len);
	if (ret)
		return ret;
	...
	ret = touch_driver_spi_transfer(tdev, spi, &spi_msg);
	if (ret < 0)
		return ret;
	touch_driver_spi_copy_data(tdev, rx_buf, data, len);
	return ret;
}

static int touch_driver_spi_read_frame(struct thp_device *tdev, unsigned int addr,
	u8 *data, unsigned int len)
{
	struct spi_device *spi = NULL;
	u8 *rx_buf = NULL;
	...
	check_flag = ((tdev == NULL) || (data == NULL) || (tx_frame_buff == NULL) ||
		(rx_frame_buff == NULL) || (tdev->thp_core == NULL) ||
		(tdev->thp_core->sdev == NULL));
	if (check_flag) {
		thp_log_err("%s: have null ptr\n", __func__);
		return -EINVAL;
	...
	spi = tdev->thp_core->sdev;
	rx_buf = rx_frame_buff;
	tx_buf = tx_frame_buff;
	cd = tdev->thp_core;
	spi_message_init(&spi_msg);
	memset(xfers, 0, sizeof(xfers));
	...
	struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(thp_node, "support_fake_panel",
		&cd->support_fake_panel);
	if (!rc)
		thp_log_info("%s:support_fake_panel:%d\n",
			__func__, cd->support_fake_panel);
	/*
	...
	cd->not_reuse_ic_type = 0;
	rc = of_property_read_u32(thp_node, "not_reuse_ic_type",
		&value);
	if (!rc) {
		cd->not_reuse_ic_type = value;
		thp_log_info("%s: not_reuse_ic_type %u\n",
			__func__, value);
	}
	cd->disable_frame_report = 0;
	rc = of_property_read_u32(thp_node, "disable_frame_report",
		&value);
	if (!rc) {
		cd->disable_frame_report = value;
		thp_log_info("%s: disable_frame_report %u\n",
			__func__, value);
	}
	cd->need_extra_system_clk = 0;
	rc = of_property_read_u32(thp_node, "need_extra_system_clk",
		&value);
	if (!rc) {
...
static void touch_driver_power_release(u32 panel_id)
{
	struct thp_core_data *cd = thp_get_core_data(panel_id);

	if (cd->power_cfg.power_supply_mode)
		return;
	thp_power_supply_put(THP_VCC, panel_id);
	thp_power_supply_put(THP_IOVDD, panel_id);
}
...
static int touch_driver_power_on(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, 0, tdev->thp_core->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 1, tdev->thp_core->panel_id);
	if (ret)
		thp_log_err("%s:iovdd fail\n", __func__);
...
static int touch_driver_power_on_for_9896(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, NO_DELAY, tdev->thp_core->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	udelay(AFTER_VCC_POWERON_DELAY);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 3, tdev->thp_core->panel_id); /* 3ms */
	if (ret)
...
static int touch_driver_power_on_reset(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9966)
		return NO_ERR;
...
static int touch_driver_power_on_for_9897(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_IOVDD,
		THP_POWER_ON, AFTER_IOVDD_DELAY_9897, cd->panel_id);
	if (ret)
		thp_log_err("%s:vddio fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON,
		AFTER_DVDD_RST_DELAY_9897, cd->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	thp_log_info("%s pull up tp ic reset\n", __func__);
...
static int touch_driver_power_on_for_9966(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON,
		NO_DELAY, cd->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_IOVDD,
		THP_POWER_ON, AFTER_IOVDD_DELAY_9966, cd->panel_id);
	if (ret)
		thp_log_err("%s:vddio fail\n", __func__);
	thp_log_info("%s pull up tp ic reset\n", __func__);
	gpio_set_value(tdev->gpios->rst_gpio, GPIO_HIGH);
...
static int touch_driver_power_on_for_ictype(struct thp_device *tdev)
{
	int ret;
...
static int power_off_not_for_mtk(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_OFF, NO_DELAY, cd->panel_id);
	if (ret)
		thp_log_err("%s:iovdd fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_OFF,
		tdev->timing_config.later_power_off_delay_ms, cd->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
...
static int power_off_for_mtk(struct thp_device *tdev)
{
	...
	if (ret)
		thp_log_err("%s:iovdd fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_OFF, NO_DELAY, cd->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	return ret;
}
...
int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	thp_log_info("%s: apply_spi_memory_in_dma: %d\n", __func__, tdev->thp_core->apply_spi_memory_in_dma);
	if (tdev->thp_core->apply_spi_memory_in_dma)
		goodix_spi_tx_buf = kzalloc(READ_CMD_BUF_LEN, GFP_KERNEL | GFP_DMA);
	else
	...
	ret = touch_driver_power_on_for_ictype(tdev);
	if (ret)
...
// gt9886 udfp data
#define FRAME_HEAD_LEN_GT9896 10
#define UDFP_DATA_LEN_GT9896 68
#define FRAME_DATA_OTHER_GT9896 14
#define UDFP_DATA_OFFSET_GT9896 (FRAME_HEAD_LEN_GT9896 + \
	CHECKSUM_SIZE_U16)
#define SCREENOFF_FRAME_LEN_GT9896  (FRAME_HEAD_LEN_GT9896 + \
	CHECKSUM_SIZE_U16 + UDFP_DATA_LEN_GT9896 + \
	FRAME_DATA_OTHER_GT9896 + UDFP_TAIL_FRAME_INDEX + CHECKSUM_SIZE_U32)

static int touch_driver_get_next_frame_len(u8 *buf)
{
	struct frame_head cur_head;
	static struct frame_head pre_head = {
		0, 0, 0, FRAME_HEAD_LEN,
	...
	u8 state)
{
	...
	cd = tdev->thp_core;
	if (cd->support_vendor_ic_type == GT9896)
		ret = touch_driver_switch_cmd(tdev, CMD_UDFP_EVENT_SWITCH_GT9896,
			state);
	else
	...
	u8 state, struct thp_aod_window window)
{
	...
	if (cd->support_vendor_ic_type == GT9896) {
		thp_log_info("%s state %u, x0,y0,x1,y1: %u, %u, %u, %u\n",
			__func__, state, window.x0, window.y0,
			window.x1, window.y1);
		...
		ret = touch_driver_switch_cmd(tdev, CMD_AOD_EVENT_SWITCH_GT9896,
			state);
		if (!ret)
			gdix_set_mode.aod_mode = state;
		else
			thp_log_err("%s failed enable aod_mode\n", __func__);
...
/*
 * set touch gesture mode
 * @state = 0
 * Set Gesture Mode To 0
 * @state != 0
 * Depending On The Circumstances
 * Set Gesture Mode To 1 Or 2
 */
static void touch_driver_set_gesture_mode(struct thp_core_data *cd,
	u8 state, u8 touch_click)
{
	if (!state)
 ...
 * send gesture cmd
 * @cmd_val
 * cmd_val = 2 Enter touch_click Gesture Mode
 * cmd_val = 1 Enter Gesture Mode
 */
static int touch_driver_send_enter_gesture_cmd(struct thp_device *tdev,
	u8 cmd_val)
{
	int ret;
	struct thp_core_data *cd = NULL;
	...
	if (cd->support_vendor_ic_type == GT9896)
		ret = touch_driver_switch_cmd(tdev,
			CMD_TOUCH_GESTURE_SWITCH_GT9896, cmd_val);
	else
		ret = touch_driver_switch_cmd(tdev,
			CMD_TOUCH_GESTURE_SWITCH_GT9897S, cmd_val);
	...
	struct thp_device *tdev, u8 state, u8 touch_click)
{
	...
	if (!state) {
		if (cd->support_vendor_ic_type == GT9896)
			ret = touch_driver_switch_cmd(tdev,
				CMD_TOUCH_GESTURE_SWITCH_GT9896, 0);
		else
			ret = touch_driver_switch_cmd(tdev,
				CMD_TOUCH_GESTURE_SWITCH_GT9897S, 0);
	} else {
		ret = touch_driver_send_enter_gesture_cmd(tdev, touch_click);
	}
	if (!ret)
		touch_driver_set_gesture_mode(cd, state, touch_click);
	else
	...
	struct thp_udfp_data *udfp_data)
{
	...
	/* read_buff [4:7] = tp_event */
	udfp_data->tpud_data.tp_event = read_buff[4] +
	...
	udfp_data->tpud_data.tp_source = UDFP_DATA_SOURCE_GOODIX;
	...
	thp_log_info("%s:version:%x, tp_event:%d, aod:%u, key:%u\n",
		__func__, udfp_data->tpud_data.version,
		udfp_data->tpud_data.tp_event, udfp_data->aod_event,
		udfp_data->key_event);
	if (udfp_data->key_event == DOUBLE_TAP_FLAG)
		udfp_data->key_event = TS_DOUBLE_CLICK;
	else if (udfp_data->key_event == STYLUS_EVENT_FLAG)
		udfp_data->key_event = TS_STYLUS_WAKEUP_TO_MEMO;
	return NO_ERR;
}

static int parse_event_info_gt9896(const u8 *read_buff, unsigned int len,
	struct thp_udfp_data *udfp_data)
{
	unsigned int tp_peak = 0;
	...
	/* read_buff [0:3] = version */
	udfp_data->tpud_data.version = (read_buff[0] << MOVE_24BIT) +
		(read_buff[1] << MOVE_16BIT) + (read_buff[2] << MOVE_8BIT) +
		read_buff[3];
	/* read_buff [4:7] = tp_event */
	udfp_data->tpud_data.tp_event = (read_buff[4] << MOVE_24BIT) +
		(read_buff[5] << MOVE_16BIT) + (read_buff[6] << MOVE_8BIT) +
		read_buff[7];
	if ((udfp_data->tpud_data.tp_event > TP_FP_EVENT_MAX) ||
		!g_thp_udfp_status)
		udfp_data->tpud_data.tp_event = TP_FP_EVENT_MAX;
	/* read_buff [32:35]  = tp_x */
	udfp_data->tpud_data.tp_x = (read_buff[32] << MOVE_24BIT) +
		(read_buff[33] << MOVE_16BIT) + (read_buff[34] << MOVE_8BIT) +
		read_buff[35];
	/* read_buff [36:39]  = tp_y */
	udfp_data->tpud_data.tp_y = (read_buff[36] << MOVE_24BIT) +
		(read_buff[37] << MOVE_16BIT) + (read_buff[38] << MOVE_8BIT) +
		read_buff[39];
	...
	/* read_buff [56:59]  = aod_event */
	udfp_data->aod_event = (read_buff[56] << MOVE_24BIT) +
		(read_buff[57] << MOVE_16BIT) + (read_buff[58] << MOVE_8BIT) +
		read_buff[59];
	/* read_buff [60:63]  = key_event */
	udfp_data->key_event = (read_buff[60] << MOVE_24BIT) +
		(read_buff[61] << MOVE_16BIT) + (read_buff[62] << MOVE_8BIT) +
		read_buff[63];
	thp_log_info("%s:version:%x, tp_event:%d, aod:%u, key:%u x:%d, y=%d\n",
		__func__, udfp_data->tpud_data.version,
		udfp_data->tpud_data.tp_event, udfp_data->aod_event,
		udfp_data->key_event, udfp_data->tpud_data.tp_x,
...
static int touch_driver_get_event_info_gt9896(struct thp_device *tdev,
	struct thp_udfp_data *udfp_data)
{
	u8 frame_buf[SCREENOFF_FRAME_LEN_GT9896] = { 0 };
	int ret;
	int i;
	u16 tail_index;
	u8 buf[UDFP_DATA_LEN_GT9896 + 1] = { 0 };
	...
	ret = touch_driver_spi_read(tdev, goodix_frame_addr, frame_buf,
		SCREENOFF_FRAME_LEN_GT9896);

	for (i = 0; i < SCREENOFF_FRAME_LEN_GT9896; i = i + 10)
		thp_log_info("tp_info %d~%d: %x, %x, %x, %x, %x, %x, %x, %x, %x, %x\n",
			i,
			i + 9,
			...
			frame_buf[i + 8], frame_buf[i + 9]);

	if ((ret < 0) || checksum_32(frame_buf, sizeof(frame_buf))) {
		thp_log_err("failed get valied frame data, %d,len %d",
			ret, sizeof(frame_buf));
		return -EINVAL;
	}
	tail_index = (frame_buf[SCREENOFF_FRAME_LEN_GT9896 -
		CHECKSUM_SIZE_U32 - 1] |
	(frame_buf[SCREENOFF_FRAME_LEN_GT9896 - CHECKSUM_SIZE_U32 -
		UDFP_TAIL_FRAME_INDEX] << 8));
	thp_log_info("tail frame index:%u\n", tail_index);

	memcpy(buf, frame_buf + UDFP_DATA_OFFSET_GT9896,
		UDFP_DATA_LEN_GT9896);
	thp_log_info("udfp:%*ph", UDFP_DATA_LEN_MIN, buf);
	thp_log_info("udfp:%*ph", UDFP_DATA_LEN_GT9896 - UDFP_DATA_LEN_MIN,
		buf + UDFP_DATA_LEN_MIN);

	ret = parse_event_info_gt9896(buf, UDFP_DATA_LEN_GT9896 + 1, udfp_data);
	if (ret) {
...
static int touch_driver_read_event_info_gt9897(struct thp_device *tdev, u8 *frame_buf, int length)
{
	int ret;
	u8 i;

	if (length < SCREENOFF_FRAME_LEN_GT9897S) {
		thp_log_err("%s: ivallid length\n", __func__);
		return -EINVAL;
	}
	ret = touch_driver_spi_read(tdev, goodix_frame_addr, frame_buf,
 ...
 * return 0 for success.
 */
static int touch_driver_get_event_info_gt9897(struct thp_device *tdev,
	struct thp_udfp_data *udfp_data)
{
	...
	ret = touch_driver_read_event_info_gt9897(tdev, frame_buf, sizeof(frame_buf));
	if (ret < 0) {
		thp_log_err("read info error\n");
		return -EINVAL;
	}
	...
	struct thp_udfp_data *udfp_data)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == GT9896)
		ret = touch_driver_get_event_info_gt9896(tdev, udfp_data);
	else
		ret = touch_driver_get_event_info_gt9897(tdev, udfp_data);
	...
	char *buf, unsigned int len)
{
	...
	ic_flag = ((tdev->thp_core->support_vendor_ic_type == GT9966) ||
		(tdev->thp_core->support_vendor_ic_type == GT9897)) &&
	...
	return touch_driver_spi_read_frame(tdev, goodix_frame_addr, buf, len);
}
...
static void touch_driver_resend_gesture_cmd_for_gt9897_dual(
	struct thp_device *tdev, u8 sync_flag)
{
	int result;
	int retval;
...
static int touch_driver_parse_gesture_data_for_gt9897_dual(
	struct thp_device *tdev, u16 gesture_type,
	unsigned int *gesture_wakeup_value)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(gesture_info_tab_for_9897_dual); i++) {
		if (gesture_type == gesture_info_tab_for_9897_dual[i].gesture_type) {
			thp_log_info("found valid gesture type: 0x%x\n", gesture_type);
			mutex_lock(&tdev->thp_core->thp_wrong_touch_lock);
			*gesture_wakeup_value = gesture_info_tab_for_9897_dual[i].gesture_value;
			mutex_unlock(&tdev->thp_core->thp_wrong_touch_lock);
			return 0;
		}
	}
...
static int touch_driver_check_gesture_data_for_gt9897_dual(
	u8 *gesture_data, int len)
{
	int ret = 0;

	if (gesture_data[0] != IRQ_EVENT_TYPE_GESTURE_GT9897_DUAL) {
		thp_log_err("not gesture irq event, event_type 0x%x\n",
			gesture_data[0]);
		ret = -EINVAL;
	}
	if (checksum32_cmp(gesture_data, len)) {
		thp_log_err("gesture data checksum error\n");
		ret = -EINVAL;
	}
...
static int touch_driver_get_gesture_data_for_gt9897_dual(
	struct thp_device *tdev, u8 *gesture_data, unsigned int len)
{
	int ret;
	int i;

	if (tdev->thp_core->support_vendor_ic_type == GT9966)
		goodix_touch_addr = GOODIX_GESTURE_ADDR_DEFAULT_GT9966;
	msleep(WAIT_FOR_SPI_BUS_RESUMED_DELAY);
	/* wait spi bus resume */
	for (i = 0; i < GESTURE_EVENT_RETRY_TIME; i++) {
		ret = touch_driver_spi_read(tdev, goodix_touch_addr,
			gesture_data, len);
		if (!ret)
			break;
	...
	struct thp_device *tdev, unsigned int *gesture_wakeup_value)
{
	...
	bool gesture_enable = false;

	if (!tdev) {
		thp_log_err("input dev is null\n");
		return -EINVAL;
	}
	gesture_enable = work_status & IC_STATUS_GESTURE;
	if (!gesture_enable) {
		thp_log_err("please enable gesture mode first\n");
		retval = -EINVAL;
		goto err_out;
	}
	retval = touch_driver_get_gesture_data_for_gt9897_dual(tdev,
		gesture_event_head, sizeof(gesture_event_head));
	if (retval) {
	...
	thp_log_info("gesture_data:%*ph\n", (u32)sizeof(gesture_event_head),
		gesture_event_head);
	retval = touch_driver_check_gesture_data_for_gt9897_dual(
		gesture_event_head, GESTURE_EVENT_HEAD_LEN_GT9897_DUAL);
	if (retval) {
		thp_log_err("check gesture data failed\n");
		retval = -EINVAL;
		goto err_out;
	}
	gesture_type = gesture_event_head[GESTURE_TYPE_VALUE_GT897_DUAL];
	retval = touch_driver_parse_gesture_data_for_gt9897_dual(tdev,
		gesture_type, gesture_wakeup_value);
		/* clean sync flag */
	if (touch_driver_spi_write(tdev, goodix_touch_addr, &sync_flag, 1))
		thp_log_err("clean sync flag fail\n");
	return retval;
err_out:
	touch_driver_resend_gesture_cmd_for_gt9897_dual(tdev, sync_flag);
	return -EINVAL;
}
...
static int touch_driver_resume_ap(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->use_ap_gesture ||
		tdev->thp_core->double_click_use_ap_gesture) {
		/* need disable irq before reset tp ic */
	...
	u8 *bt_status_buf, u8 check_status_data)
{
	...
	int ret;
	int connect_addr;
	int connect_check_addr;
	u8 check_value;

	thp_log_info("check_status_data = %d\n", check_status_data);
	if (tdev->thp_core->support_vendor_ic_type == GT9966) {
		connect_addr = GOODIX_CMD_ADDR_DEFAULT_GT9966;
		connect_check_addr = BT_CONNECT_STATUS_CHECK_ADDR_GT9966;
	} else {
		connect_addr = BT_CONNECT_STATUS_ADDR;
		connect_check_addr = BT_CONNECT_STATUS_CHECK_ADDR;
	}
	for (i = 0; i < BT_CONNECT_CMD_RETRY; i++) {
		ret = touch_driver_spi_write(tdev, connect_addr,
			bt_status_buf, BT_CONNECT_STATUS_BUF_LEN);
		if (ret)
			continue;
		msleep(BT_AFTER_WRITE_CMD_DELAY);

		ret = touch_driver_spi_read(tdev, connect_check_addr,
...
static bool touch_driver_get_stylus3_adsorption_status(struct thp_device *tdev)
{
	int stylus3_bt_status;
	int stylus_adsorption_status;
	bool cur_stylus3_status;

	stylus3_bt_status = atomic_read(&tdev->thp_core->last_stylus3_status);
	stylus_adsorption_status =
		atomic_read(&tdev->thp_core->last_stylus_adsorption_status);
	thp_log_info("%s: stylus3_bt_status = %d, stylus_adsorption_status = %d\n",
		__func__, stylus3_bt_status, stylus_adsorption_status);
	cur_stylus3_status = ((stylus3_bt_status & BT_STYLUS3_CONNECTED_MASK) &&
		(stylus_adsorption_status & STYLUS3_ADSORPTION_MASK));

	return cur_stylus3_status;
}


static bool touch_driver_get_stylus3_status(struct thp_device *tdev)
{
	int stylus3_bt_status;
	int stylus_adsorption_status;
	int current_stylus3_work_mode;
	int current_app_dsi_status;
	bool cur_stylus3_status;
	...
	current_app_dsi_status =
		atomic_read(&tdev->thp_core->app_dsi_status);
	thp_log_info("%s: stylus3_bt_status = %d, stylus_adsorption_status = %d\n",
		__func__, stylus3_bt_status, stylus_adsorption_status);
	thp_log_info("%s: current_stylus3_work_mode = %d, current_app_dsi_status = %d\n",
		__func__, current_stylus3_work_mode, current_app_dsi_status);
	cur_stylus3_status = ((stylus3_bt_status & BT_STYLUS3_CONNECTED_MASK) &&
		(!(stylus_adsorption_status & STYLUS3_ADSORPTION_MASK)) &&
		(!current_stylus3_work_mode) && (!current_app_dsi_status));

	return cur_stylus3_status;
}

static void touch_driver_set_stylus3_link_info(struct thp_device *tdev,
	bool delay_enable, bool link_enable, u8 **connect_cmd, u8 *connect_value)
{
	bool stylus_adsorption_status;

	stylus_adsorption_status = touch_driver_get_stylus3_adsorption_status(tdev);
	if (delay_enable)
		msleep(SEND_BT_CONNECT_STATUS_AFTER_RESET_DELAY);
	if (link_enable) {
		*connect_cmd = bt_connect_buf;
		*connect_value = BT_CONNECT_VALUE;
	} else if (stylus_adsorption_status) {
		*connect_cmd = bt_adsorption_buf;
		*connect_value = BT_ADSORPTION_VALUE;
	} else {
		*connect_cmd = bt_disconnect_buf;
		*connect_value = BT_DISCONNECT_VALUE;
	}
	thp_log_info("%s: stylus3 connect_value = %d\n", __func__, *connect_value);
}

static int touch_driver_ai_bt_handler(struct thp_device *tdev,
	bool delay_enable)
{
	int ret;
	u8 bt_connect_value = 0;
	u8 *bt_status_buf_addr = NULL;
	bool scan_status;

	scan_status = touch_driver_get_stylus3_status(tdev);
	touch_driver_set_stylus3_link_info(tdev, delay_enable, scan_status,
	...
	thp_log_info("%s: send stylus3 status to fw succ\n", __func__);
	...
	bool delay_enable)
{
	...
	u8 bt_connect_value = 0;
	u8 *bt_status_buf_addr = NULL;
	bool tp_is_power_off = false;
	bool scan_status = false;

	tp_is_power_off = tdev->thp_core->suspended &&
		(tdev->thp_core->easy_wakeup_info.sleep_mode == TS_POWER_OFF_MODE);
	if (tp_is_power_off) {
		thp_log_info("tp is power off , do not need send bt status to fw\n");
		return 0;
	}
	if (tdev->thp_core->support_vendor_ic_type == GT9966)
		return touch_driver_ai_bt_handler(tdev, delay_enable);
	stylus3_status = atomic_read(&tdev->thp_core->last_stylus3_status);
	scan_status = (stylus3_status > 0) ? true : false;
	touch_driver_set_stylus3_link_info(tdev, delay_enable, scan_status,
		&bt_status_buf_addr, &bt_connect_value);
	ret = send_stylus3_status_to_fw(tdev,
		bt_status_buf_addr, bt_connect_value);
...
#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
static int touch_driver_set_touch_aod_state(struct thp_device *tdev)
{
	int ret;
	struct thp_core_data *cd = NULL;
	...
	if (cd->support_vendor_ic_type == GT9896)
		ret = touch_driver_switch_cmd(tdev,
			CMD_AOD_EVENT_SWITCH_GT9896, ENABLE);
	else
		ret = touch_driver_switch_cmd(tdev,
...
static int touch_driver_set_ap_state(struct thp_device *tdev)
{
	...
	if (cd->aod_touch_status) {
		ret = touch_driver_set_touch_aod_state(tdev);
		if (ret)
...
#ifdef CONFIG_HUAWEI_SHB_THP
/* use kit in sensorhub, sensorhub push-down */
static int touch_driver_suspend_shb_thp_2(struct thp_device *tdev)
{
	int ret;
...
/* use kit in sensorhub, sensorhub no push-down */
static int touch_driver_suspend_shb_thp_1(struct thp_device *tdev)
{
	int ret = 0;
	...
	thp_log_info("%s: enter\n", __func__);
	if (tdev->thp_core->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) {
		ret = touch_driver_switch_cmd(tdev, CMD_TOUCH_GESTURE_SWITCH_GT9897S,
			CMD_GESTURE_DOUBLE);
		if (ret)
			thp_log_err("failed to set_double_click_gesture\n");
	}
	ret = touch_driver_switch_cmd(tdev, CMD_SWITCH_INT_PIN_GT9897,
		CMD_SWITCH_INT_SH);
	if (ret)
...
#ifdef CONFIG_HUAWEI_SHB_THP
static void touch_driver_suspend_gesture_for_dual_shb_1(struct thp_device *tdev)
{
	int ret = 0;
	struct thp_core_data *cd = NULL;

	cd = tdev->thp_core;
	if (cd->sensorhub_support_gesture == 2) {
		ret = touch_driver_suspend_shb_thp_1(tdev);
...
static int touch_driver_suspend_gesture(struct thp_device *tdev)
{
	...
	if (cd->disable_frame_report) {
		thp_log_info("%s: disable frame report\n", __func__);
		ret = touch_driver_send_cmd(tdev, GOODIX_CMD_RAWDATA, data);
		thp_log_info("%s : ret is %d\n", __func__, ret);
		if (ret)
			thp_log_err("disable frame report failed\n");
		mdelay(DISABLE_FRAME_WAIT_DELAY);
	}
	thp_log_info("%s: enter gesture mode,aod:%d,gesture:%d\n",
		__func__, cd->easy_wakeup_info.aod_mode,
		cd->easy_wakeup_info.sleep_mode);
	thp_log_info("%s:aod_touch_status:%u\n", __func__, g_aod_touch_status);
	if (cd->sensorhub_support_gesture == 1) {
		if ((!cd->easy_wakeup_info.aod_mode) &&
			!(g_aod_touch_status && (cd->multi_panel_index == SINGLE_TOUCH_PANEL)))
...
#ifdef CONFIG_HUAWEI_SHB_THP
	touch_driver_suspend_gesture_for_dual_shb_1(tdev);
#endif
#ifndef CONFIG_HUAWEI_THP_MTK
	if (tdev->thp_core->gesture_from_sensorhub) {
		thp_set_irq_status(tdev->thp_core, THP_IRQ_DISABLE);
...
static int touch_driver_suspend_guesture_mode(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_shb_thp) {
		...
		ret = touch_driver_suspend_shb_thp_2(tdev);
#endif
	} else {
		if (tdev->thp_core->tsa_event_to_udfp == TAS_EVENT_TO_UDFP_MODE_TWO) {
#ifdef CONFIG_HUAWEI_SHB_THP
			ret = touch_driver_suspend_shb_thp_1(tdev);
#endif
		} else {
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	cd = tdev->thp_core;
	g_thp_udfp_status = cd->suspend_udfp_status;
	goodix_stylus_status = thp_get_status(THP_STATUS_STYLUS, cd->panel_id) |
	...
	gesture_status = cd->sensorhub_support_gesture &&
...
static int touch_driver_verify_ic_status(struct thp_device *tdev, char expect_status)
{
	int ret;
	int i;
	uint8_t tmp_buf[SN_READ_REC_DATA_LEN + 1] = { 0 };

	for (i = 0; i < SN_READ_MAX_RETRY_TIMES; i++) {
		ret = touch_driver_spi_read(tdev, GOODIX_CMD_ADDR_DEFAULT_GT9966,
			tmp_buf, sizeof(tmp_buf));
		if (ret) {
			msleep(SN_READ_WAIT_TIME_FOR_GTX9966);
			continue;
		}
		if (tmp_buf[SN_READY_BIT_FOR_GTX9966] == expect_status) {
			thp_log_info("%s get target state 0x%x\n", __func__,
				tmp_buf[SN_READY_BIT_FOR_GTX9966]);
			msleep(SN_READ_WAIT_TIME_FOR_GTX9966);
			return 0;
		}
	}
...
static int touch_driver_write_cmd_for_flash_read(struct thp_device *tdev,
	char *cmd, unsigned int len, char except_value)
{
	int ret;

	ret = touch_driver_spi_write(tdev, GOODIX_CMD_ADDR_DEFAULT_GT9966, cmd, len);
	if (ret) {
		thp_log_err("%s write begin_cmd failed\n", __func__);
		return ret;
	}
	msleep(SN_READ_WAIT_TIME_FOR_GTX9966);

	ret = touch_driver_verify_ic_status(tdev, except_value);
	if (ret) {
		thp_log_err("%s time out for first wait spi ready\n", __func__);
		return ret;
	}
	return 0;
}

static int touch_driver_write_cmd_for_sn(struct thp_device *tdev)
{
	int ret;
	uint8_t write_begin_cmd[] = { 0x00, 0x00, 0x04, 0x09, 0x0D, 0x00 }; /* begin cmd for sn */
	uint8_t write_ready_cmd[] = { 0x00, 0x00, 0x04, 0x0B, 0x0F, 0x00 }; /* ready cmd for sn */
	uint8_t write_flashaddr_cmd[] = { 0x27, 0xE2, 0x00, 0x00, 0x00, 0xE2,
		0x07, 0x00, 0x20, 0x00, 0x00, 0x00 }; /* dst addr for flash */

	ret = touch_driver_write_cmd_for_flash_read(tdev, write_begin_cmd,
		sizeof(write_begin_cmd), SN_FIRST_WRITE_READY_VALUE);
	if (ret) {
		thp_log_err("%s write begin cmd failed\n", __func__);
		return ret;
	}
	msleep(SN_READ_WAIT_TIME_FOR_GTX9966);

	ret = touch_driver_spi_write(tdev, GOODIX_CMD_ADDR_FLASH_GT9966,
		write_flashaddr_cmd, sizeof(write_flashaddr_cmd));
	if (ret) {
		thp_log_err("%s write flashaddr_cmd failed\n", __func__);
		return ret;
	}
	msleep(SN_READ_WAIT_TIME_FOR_GTX9966);

	ret = touch_driver_write_cmd_for_flash_read(tdev, write_ready_cmd,
		sizeof(write_ready_cmd), SN_SECOND_WRITE_READY_VALUE);
	if (ret) {
		thp_log_err("%s write ready cmd failed\n", __func__);
		return ret;
	}
	return 0;
}

static int touch_driver_read_sn(struct thp_device *tdev, char *out, unsigned int len)
{
	int ret;
	uint8_t write_finish_cmd[] = { 0x00, 0x00, 0x04, 0x0C, 0x10, 0x00 }; /* finsh cmd for sn */

	ret = touch_driver_write_cmd_for_sn(tdev);
	if (ret) {
		thp_log_err("%s ic is not ready\n", __func__);
		return ret;
	}

	ret = touch_driver_spi_read(tdev, GOODIX_CMD_ADDR_FLASH_READ_GT9966, out, len);
	if (ret) {
		thp_log_err("%s write read sn failed\n", __func__);
		return ret;
	}
	out[len] = '\0';

	ret = touch_driver_spi_write(tdev, GOODIX_CMD_ADDR_DEFAULT_GT9966,
		write_finish_cmd, sizeof(write_finish_cmd));
	if (ret) {
		thp_log_err("%s write finish_cmd failed\n", __func__);
		return ret;
	}
	return ret;
}

static int touch_driver_get_sn(struct thp_device *tdev, char *out, unsigned int len)
{
	int para_check_flag;

	para_check_flag = (len > SN_AREA_MAX_LEN) || (!tdev) || (!tdev->thp_core) || (!out);
	if (para_check_flag) {
		thp_log_err("%s invalid parameter\n", __func__);
		return -EINVAL;
	}

	if (tdev->thp_core->support_oem_info != THP_OEM_INFO_READ_FROM_TP) {
		thp_log_info("%s not support read sn\n", __func__);
		return -EINVAL;
	}
	return touch_driver_read_sn(tdev, out, len);
}
...
int touch_driver_spi_active(struct thp_device *tdev)
{
	...
	ret = thp_set_spi_max_speed(IDLE_SPI_SPEED, tdev->thp_core->panel_id);
	if (ret) {
	...
	ret = thp_set_spi_max_speed(ACTIVE_SPI_SPEED, tdev->thp_core->panel_id);
	if (ret) {
...
struct thp_device_ops goodix_dev_ops = {
	...
	.get_sn_code = touch_driver_get_sn,
...
static int thp_module_init(u32 panel_id)
{
	...
	tx_frame_buff = kzalloc(THP_MAX_FRAME_SIZE, GFP_KERNEL);
	rx_frame_buff = kzalloc(THP_MAX_FRAME_SIZE, GFP_KERNEL);
	if ((dev->tx_buff == NULL) || (dev->rx_buff == NULL) ||
		(tx_frame_buff == NULL) || (tx_frame_buff == NULL)) {
...
static int __init touch_driver_module_init(void)
{
	...
	for (i = 0; i < TOTAL_PANEL_NUM; i++) {
		dn = get_thp_node(i);
		if (dn && of_get_child_by_name(dn, THP_GOODIX_DEV_NODE_NAME))
