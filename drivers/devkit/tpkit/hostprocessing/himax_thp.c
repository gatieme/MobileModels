...
#define HIMAX_121A_AUTO_PLUS_4_EN 0x13
#define HIMAX_121A_AUTO_PLUS_4_DIS 0x12
...
#define HIMAX_102E_TCON_RST_ADDR 0x80020004
#define HIMAX_102E_TCON_RST_ON_CMD 0x00000000
...
#define HX_83121A_ID_PART_1 0x83
#define HX_83121A_ID_PART_2 0x12
#define HX_83121A_ID_PART_3 0x1A
#define HX_83122A_ID_PART_1 0x83
#define HX_83122A_ID_PART_2 0x12
#define HX_83122A_ID_PART_3 0x2A
...
#define VENDOR_ID_CSOT "270"
#define VENDOR_NAME_BOE "boe"
#define VENDOR_NAME_INX "cmi"
#define VENDOR_NAME_CSOT "csot"
#define HX_BOE_TX_NUM 40
#define HX_BOE_RX_NUM 60
#define HX_INX_TX_NUM 30
#define HX_INX_RX_NUM 48
#define HX_DB_CLICK 0x01
#define HX_WAKEUP_MEMO 0x02
#define BYTE_2 2
#define OFFSET_GES 98
#define XFER_CNT 10
#define WAKE_UP_ONLY_HAND 0xA55AA55A
#define WAKE_UP_PEN_HAND 0xA77AA77A
#define WAKE_UP_ONLY_PEN 0xA33AA33A
#define WAKE_UP_NONE     0x00;
#define PEN_ENABLE 0x01
#define PEN_DISABLE 0x00
#define HIMAX_GESTURE_WAKEUP_ADDR 0x10000110
#define HIMAX_CMD_CLEAR_INTERRUPT_ADDR 0x100006EC
#define HIMAX_CMD_CLEAR_INTERRUPT_DATA 0x000000F8
#define HIMAX_CMD_CLEAR_INTERRUPT_DELAY 50

uint32_t g_lpwg_subtype = WAKE_UP_PEN_HAND;
uint32_t g_lpwg_type = PEN_ENABLE;

static uint8_t *g_honor_project_id;
static uint8_t *g_honor_cg_color;
static uint8_t *g_honor_sensor_id;
	...
	uint8_t *data, unsigned int length, uint8_t to_retry)
{
	...
	if ((!g_dma_rx_buf) || (!g_dma_tx_buf) ||
		...
		thp_log_err(tdev->thp_core, "dma_buf kzalloc failed\n");
		return -ENOMEM;
	}
	memset(g_transfer, 0,
...
#ifdef CONFIG_VMAP_STACK
	if (!data) {
		thp_log_err(tdev->thp_core, "data is NULL\n");
		return -ENOMEM;
	}
...
static int touch_driver_83102_bus_write(struct thp_device *tdev,
	...
	unsigned int length, uint8_t toretry)
{
	uint8_t spi_format_buf[HIMAX_NORMAL_DATA_LEN + HIMAX_NORMAL_ADDR_LEN + DATA_2];
	int i;
	...
	uint8_t *write_addr, uint8_t len, uint8_t *write_data)
{
	...
	if (!write_addr) {
		thp_log_err(tdev->thp_core, "%s:write_addr null!\n", __func__);
		return;
	}
	if (!len) {
		thp_log_err(tdev->thp_core, "%s:length invalid!\n", __func__);
		return;
	}
	if (!write_data) {
		thp_log_err(tdev->thp_core, "%s:write_data null!\n", __func__);
		return;
	}
	...
	const u8 *data, unsigned int *gesture_wakeup_value)
{
	...
	thp_log_info(tdev->thp_core,
			"data[%d] = 0x%X, data[%d] = 0x%X\n",
			length, data[length], length - 1, data[length - 1], __func__);
...
int touch_driver_init(struct thp_device *tdev)
{
	...
	if (rc) {
		...
		thp_log_info(tdev->thp_core,  "%s: 9L_flag %d\n",
			__func__, value);
	}

	rc = of_property_read_u32(hx83112_node,
		"himax_ic_83121_flag", &value);
	if (rc) {
		himax_p->himax_ic_83121_flag = 0;
	} else {
		himax_p->himax_ic_83121_flag = value;
		thp_log_info(tdev->thp_core,  "%s: 121_flag %d\n",
			__func__, value);
	}
	...
	rc = thp_parse_timing_config(tdev->thp_core, hx83112_node, &tdev->timing_config);
	if (rc) {
	...
	if (ret) {
		thp_log_err(tdev->thp_core, "%s: Failed to configure regulator\n",
			__func__);
		return ret;
	}
#else
...
static int touch_driver_sense_off(struct thp_device *tdev)
{
	...
	do {
		...
		if ((touch_driver_bus_write(tdev,
			...
			return -ENOMEM;
		if ((himax_p->himax_ic_83121_flag) || (himax_p->himax_ic_83122_flag))
			himax_assign_data(HIMAX_121A_AUTO_PLUS_4_DIS, tmp_data);
		else
		...
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,  "%s: Check save mode data[0] = %02X\n",
			__func__, tmp_data[0]);

		if (tmp_data[0] == HIMAX_FW_SSOFF) {
			if (himax_p->himax_ic_83102_flag) {
				himax_assign_data(HIMAX_102E_TCON_RST_ADDR, tmp_addr);
				himax_assign_data(HIMAX_102E_TCON_RST_ON_CMD, tmp_data);
			} else {
	...
	int bank_idx, int start_para, int read_size, uint8_t *out)
{
	...
	thp_log_info(tdev->thp_core,  "%s:reformat addr=0x%08X, cmd=0x%02X\n",
		__func__, dd_reg_reformat, cmd);
	thp_log_info(tdev->thp_core,  "%s:bank_idx=%d, start_para=%d, read_size=%d\n",
	...
	for (i = 0; i < read_size; i++) {
		...
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,  "%s:use %d;[3]=0x%X,[2]=0x%X,[1]=0x%X,[0]=0x%X\n",
...
static int touch_driver_9l_read_ddreg_en(struct thp_device *tdev, int en)
{
	...
	if (tmp_data[DATA_0] != ENTER_SAVE_MODE) {
		thp_log_info(tdev->thp_core,  "%s: Doesn't enter save mode, need sense off!\n",
			__func__);
		ret = touch_driver_sense_off(tdev);
		if (ret)
			return ret;
	} else {
		thp_log_info(tdev->thp_core,  "%s: Already enter into save mode!\n", __func__);
	}
	if (en) {
		thp_log_info(tdev->thp_core,  "%s: Ready to enable to read ddreg!\n", __func__);
		...
		thp_log_info(tdev->thp_core,  "%s: Ready to disable to read ddreg!\n", __func__);
		touch_driver_rw_reg_reformat_com(ADDR_DD_PW_HX83102,
	...
	uint8_t *start_addr, int reload_length)
{
	...
	if (!length) {
		thp_log_err(tdev->thp_core, "%s:not invalid length!\n", __func__);
		return -EINVAL;
	}
	if (!start_addr) {
		thp_log_err(tdev->thp_core, "%s:start_addr null!\n", __func__);
		return -EINVAL;
	}
	...
	do {
		...
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,  "%s: >>>>>>1.tmp_data[3]=%X,tmp_data[2]=%X\n",
			__func__, tmp_data[DATA_3], tmp_data[DATA_2]);
		thp_log_info(tdev->thp_core,  "%s: >>>>>>2.tmp_data[1]=%X,tmp_data[0]=%X\n",
			__func__, tmp_data[DATA_1], tmp_data[DATA_0]);
		if ((tmp_data[DATA_0 + mod] & LOW_BIT) == LOW_BIT) {
			thp_log_info(tdev->thp_core,  "wait for HW ready!\n");
			...
			if (ret)
				return ret;
			thp_log_info(tdev->thp_core,  "%s: >>>>>>3.tmp_data[3]=%X, tmp_data[2]=%X\n",
				__func__, tmp_data[DATA_3], tmp_data[DATA_2]);
			thp_log_info(tdev->thp_core,  "%s: >>>>>>4.tmp_data[1]=%X, tmp_data[0]=%X\n",
				__func__, tmp_data[DATA_1], tmp_data[DATA_0]);
...
static int touch_driver_projectid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (!len) {
		thp_log_err(cd, "%s:not invalid length!\n", __func__);
		return -EINVAL;
	}
	if (!buffer) {
		thp_log_err(cd, "%s:buffer null!\n", __func__);
		return -EINVAL;
	}
	if (type == STR_TYPE) {
		g_honor_project_id = kcalloc((len + 1),
			sizeof(*g_honor_project_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_project_id = kcalloc(len,
			sizeof(*g_honor_project_id), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: project id UNKNOWN type %d\n",
			__func__, type);
	}
	if (g_honor_project_id) {
		memcpy(g_honor_project_id,
			buffer + MOVE_8BIT, len);
		thp_log_info(cd,  "%s: project id: ", __func__);
		if (type == STR_TYPE) {
			g_honor_project_id[len] = '\0';
			thp_log_info(cd,  "%s", g_honor_project_id);
		} else {
			for (j = DATA_0; j < len; j++)
				thp_log_info(cd,  "0x%02X",
					*(g_honor_project_id +
					j));
		}
		thp_log_info(cd,  "\n");
...
static int touch_driver_cgcolor(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_honor_cg_color = kcalloc((len + 1),
			sizeof(*g_honor_cg_color), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_cg_color = kcalloc(len,
			sizeof(*g_honor_cg_color), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: CG color UNKNOWN type %d\n",
			__func__, type);
	}
	if (g_honor_cg_color) {
		memcpy(g_honor_cg_color,
			buffer + MOVE_8BIT, len);
		thp_log_info(cd,  "%s: cg_color: ", __func__);
		if (type == STR_TYPE) {
			g_honor_cg_color[len] = '\0';
			thp_log_info(cd,  "%s", g_honor_cg_color);
		} else {
			for (j = 0; j < len; j++)
				thp_log_info(cd,  "0x%02X",
					*(g_honor_cg_color +
...
static int touch_driver_sensorid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (!len) {
		thp_log_err(cd, "%s:length invalid!\n", __func__);
		return -EINVAL;
	}
	if (!buffer) {
		thp_log_err(cd, "%s:buffer null!\n", __func__);
	...
	if (type == STR_TYPE) {
		g_honor_sensor_id = kcalloc((len + 1),
			sizeof(*g_honor_sensor_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_sensor_id = kcalloc(len,
			sizeof(*g_honor_sensor_id), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: sensor id UNKNOWN type %d\n",
			__func__, type);
	}
	if (g_honor_sensor_id) {
		memcpy(g_honor_sensor_id,
			buffer + MOVE_8BIT, len);
		thp_log_info(cd,  "%s: sensor id: ", __func__);
		if (type == STR_TYPE) {
			g_honor_sensor_id[len] = '\0';
			thp_log_info(cd,  "%s", g_honor_sensor_id);
		} else {
			for (j = 0; j < len; j++)
				thp_log_info(cd,  "0x%02X",
					*(g_honor_sensor_id +
...
static int touch_driver_read_panel_info(struct thp_device *tdev)
{
	...
	if (!tdev) {
		thp_log_err(tdev->thp_core, "%s: not invalid device!\n", __func__);
		return -EINVAL;
	}
	buffer = kcalloc(INFO_PAGE_LENGTH, sizeof(*buffer), GFP_KERNEL);
	if (!buffer) {
		thp_log_err(tdev->thp_core, "%s: Memory allocate FAIL!\n", __func__);
		return -ENOMEM;
	}
	ret = touch_driver_sense_off(tdev);
	if (ret)
	...
	if (himax_p->himax_ic_hispeed_support) {
		...
		touch_driver_register_write(tdev, tmp_addr,
			HIMAX_NORMAL_ADDR_LEN, tmp_data);
		thp_log_info(tdev->thp_core,  "%s: enter ic hispeed mode!\n",
			__func__);
	}
	for (i = DATA_0; i < INFO_SECTION_NUM; i++) {
		...
		if (touch_driver_hw_check_crc(tdev,
			...
			thp_log_err(tdev->thp_core, "%s:section %d CRC FAIL tmp_addr = 0x%02X, DATA_0 = 0x%02X\n",
				__func__, i, tmp_addr, DATA_0);
			goto err;
		}
		...
		if ((title[DATA_0] == DATA_0) && (title[DATA_1] == DATA_1) &&
			...
			if (ret) {
				thp_log_err(tdev->thp_core, "%s: projectid read fail\n",
					__func__);
				goto err;
			}
			...
			if (ret) {
				thp_log_err(tdev->thp_core, "%s: projectid cgcolor fail\n",
					__func__);
				goto err;
			}
			...
			if (ret) {
				thp_log_err(tdev->thp_core, "%s: sensorid read fail\n",
					__func__);
				goto err;
			}
		} else {
			thp_log_err(tdev->thp_core, "%s: UNKNOWN title %X%X%X\n",
...
static int touch_driver_9l_ic_check(struct thp_device *tdev)
{
	...
	thp_log_info(tdev->thp_core,  "Now [8]=0x%X,[7]=0x%X,[6]=0x%X,[5]=0x%X\n",
		dd_icid_detail[DATA_7], dd_icid_detail[DATA_6],
		dd_icid_detail[DATA_5], dd_icid_detail[DATA_4]);
	thp_log_info(tdev->thp_core,  "Now [4]=0x%X,[3]=0x%X,[2]=0x%X,[1]=0x%X\n",
	...
	thp_log_info(tdev->thp_core,  "Now [1]=0x%02X\n", dd_pen_func_check[DATA_0]);
	...
	if ((ic_info[DATA_0] == DATA_IC_INFO_SP_HX83102_0) &&
		...
		result = 1;
		thp_log_err(tdev->thp_core, "%s: It's 9L!\n", __func__);
	} else {
		result = 0;
		thp_log_err(tdev->thp_core, "%s: IC can't be recognized\n", __func__);
	}
	return result;
}
...
static int touch_driver_121_communication_check(struct thp_device *tdev)
{
	uint8_t tmp_addr[HIMAX_NORMAL_ADDR_LEN] = {0};
	uint8_t tmp_data[HIMAX_NORMAL_DATA_LEN] = {0};
	uint8_t ic_name[HIMAX_NORMAL_DATA_LEN] = {0};
	...
	for (i = 0; i < COMM_TEST_RW_RETRY_TIME; i++) {
		...
		for (j = DATA_0; j < ARRAY_SIZE(himax_id_match_table); j++) {
			vendor_assign_data(himax_id_match_table[j],
				ic_name, HIMAX_NORMAL_DATA_LEN);
			if ((tmp_data[DATA_3] == HX_83122A_ID_PART_1) &&
				(tmp_data[DATA_2] == HX_83122A_ID_PART_2) &&
				(tmp_data[DATA_1] == HX_83122A_ID_PART_3)) {
				break;
			} else if ((tmp_data[DATA_3] == HX_83121A_ID_PART_1) &&
				(tmp_data[DATA_2] == HX_83121A_ID_PART_2) &&
				(tmp_data[DATA_1] == HX_83121A_ID_PART_3)) {
				break;
			} else if ((tmp_data[DATA_3] == ic_name[DATA_3]) &&
				(tmp_data[DATA_2] == ic_name[DATA_2]) &&
				(tmp_data[DATA_1] == ic_name[DATA_1])) {
...
static int touch_driver_83112_communication_check(struct thp_device *tdev)
{
	...
	if ((himax_p->himax_ic_83121_flag) || (himax_p->himax_ic_83122_flag)) {
		ret = touch_driver_121_communication_check(tdev);
		return ret;
	}
	if (!tmp_data) {
		thp_log_info(tdev->thp_core,  "%s:tmp_data is NULL\n", __func__);
	...
	for (i = 0; i < COMM_TEST_RW_RETRY_TIME; i++) {
		...
		thp_log_info(tdev->thp_core,  "%s:Read driver IC ID = %X,%X,%X\n", __func__,
		...
		if (j < ARRAY_SIZE(himax_id_match_table)) {
			thp_log_info(tdev->thp_core,  "83112 found\n");
			break;
		}
		thp_log_err(tdev->thp_core, "%s:Read driver ID register Fail:\n", __func__);
	}
...
static void touch_driver_timing_work(struct thp_device *tdev)
{
	...
	gpio_direction_output(tdev->gpios->rst_gpio, GPIO_LOW);
	thp_time_delay(tdev->thp_core, tdev->timing_config.boot_reset_low_delay_ms);
	gpio_set_value(tdev->gpios->rst_gpio, THP_RESET_HIGH);
	thp_time_delay(tdev->thp_core, tdev->timing_config.boot_reset_hi_delay_ms);
#endif
...
int touch_driver_83102_chip_detect(struct thp_device *tdev)
{
	...
	if (!g_dma_rx_buf) {
		thp_log_err(tdev->thp_core, "g_dma_rx_buf is null\n");
		return -ENOMEM;
	}
	g_dma_tx_buf = (uint8_t *)(tdev->tx_buff);
	if (!g_dma_tx_buf) {
		thp_log_err(tdev->thp_core, "g_dma_tx_buf is null\n");
		return -ENOMEM;
	}
	size_xfer = XFER_CNT * sizeof(struct spi_transfer);
	g_transfer = (struct spi_transfer *)kzalloc(size_xfer, GFP_KERNEL);
	if (!g_transfer) {
		thp_log_err(tdev->thp_core, "g_transfer kzalloc failed\n");
		return -ENOMEM;
	}
#endif
	...
	if (ret) {
		thp_log_err(tdev->thp_core, "%s: read panel info  fail\n", __func__);
		//return -EINVAL;
		return 0;
	}
	/* Get tp_color */
	if (g_honor_cg_color != NULL)
		cypress_ts_kit_color[0] = *g_honor_cg_color;
...
int touch_driver_83112_chip_detect(struct thp_device *tdev)
{
	...
	if (!g_himax_spi_buf.w_buf) {
		thp_log_err(tdev->thp_core, "%s:w_buf request memory fail\n", __func__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (!g_himax_spi_buf.r_buf_a) {
		thp_log_err(tdev->thp_core, "%s:r_buf1 request memory fail\n", __func__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (!g_himax_spi_buf.r_buf_b) {
		thp_log_err(tdev->thp_core, "%s:r_buf2 request memory fail\n", __func__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	char *buf, unsigned int len)
{
	...
	if (!len) {
		thp_log_err(tdev->thp_core, "%s: len illegal\n", __func__);
		return -EINVAL;
	}
	memcpy(buf, g_honor_project_id,
		HIMAX_ACTUAL_PROJECTID_LEN);
	buf[HIMAX_ACTUAL_PROJECTID_LEN] = '\0';

	if (!g_honor_project_id)
	{
		thp_log_err(tdev->thp_core, "%s: g_honor_project_id null\n", __func__);
		return -EINVAL;
	}
	...
	if (!strncmp(temp_buf,
		...
		VENDOR_ID_CSOT, THP_PROJECTID_VENDOR_NAME_LEN)) {
		cd->vendor_name = VENDOR_NAME_CSOT;
	} else {
		thp_log_err(tdev->thp_core, "%s: not support this vendor_id\n", __func__);
		return -EINVAL;
	}

	thp_log_info(tdev->thp_core,  "projectid:%s\n", buf);
	return NO_ERR;
}

static void touch_driver_clear_all_interrupt(
	struct thp_device *tdev)
{
	...
	thp_log_info(tdev->thp_core, "%s: called\n", __func__);
	thp_bus_lock(tdev->thp_core);
	himax_assign_data(HIMAX_CMD_CLEAR_INTERRUPT_ADDR, tmp_addr);
	himax_assign_data(HIMAX_CMD_CLEAR_INTERRUPT_DATA, tmp_data);
	touch_driver_register_write(tdev, tmp_addr,
	HIMAX_NORMAL_ADDR_LEN, tmp_data);
	thp_do_time_delay(HIMAX_CMD_CLEAR_INTERRUPT_DELAY);

	ret = touch_driver_register_read(tdev,
			tmp_addr, HIMAX_NORMAL_DATA_LEN, tmp_data);
	if (ret) {
		thp_log_err(tdev->thp_core, "%s: read clear all interrupt register fail\n", __func__);
		thp_bus_unlock(tdev->thp_core);
		return;
	}
	thp_bus_unlock(tdev->thp_core);
	...
	thp_log_info(tdev->thp_core,
		"%s: read 0x%X:data[3] = 0x%X, data[2] = 0x%X, data[1] = 0x%X, data[0] = 0x%X\n",
		__func__, HIMAX_CMD_CLEAR_INTERRUPT_ADDR, tmp_data[3], tmp_data[2], tmp_data[1],
		tmp_data[0]);
	...
	struct thp_device *tdev)
{
	...
	thp_log_info(tdev->thp_core, "%s: called\n",  __func__);
	thp_bus_lock(tdev->thp_core);
	himax_assign_data(HIMAX_GESTURE_WAKEUP_ADDR, tmp_addr);
	himax_assign_data(g_lpwg_subtype, tmp_data);
	touch_driver_register_write(tdev, tmp_addr,
	HIMAX_NORMAL_ADDR_LEN, tmp_data);
	...
	ret = touch_driver_register_read(tdev,
			tmp_addr, HIMAX_NORMAL_DATA_LEN, tmp_data);
	if (ret) {
		thp_log_err(tdev->thp_core, "%s: read gesture wakeup register fail\n", __func__);
		thp_bus_unlock(tdev->thp_core);
		return;
	}
	thp_bus_unlock(tdev->thp_core);
	...
	thp_log_info(tdev->thp_core,
		"%s: read 0x%X:data[3] = 0x%X, data[2] = 0x%X, data[1] = 0x%X, data[0] = 0x%X\n",
			__func__, HIMAX_GESTURE_WAKEUP_ADDR, tmp_data[3], tmp_data[2], tmp_data[1],
			tmp_data[0]);
...
int touch_driver_83112_resume(struct thp_device *tdev)
{
	...
	gpio_set_value(tdev->gpios->rst_gpio, THP_RESET_HIGH);
	thp_time_delay(tdev->thp_core, tdev->timing_config.resume_reset_after_delay_ms);
#endif
	himax_p = tdev->private_data;
...
int touch_driver_83112_suspend(struct thp_device *tdev)
{
	...
	if ((cd->easy_wakeup_info.sleep_mode != TS_GESTURE_MODE) ||
		...
		if (himax_p->himax_ic_83122_flag)
...
static int touch_driver_nt_bt_handler(struct thp_device *tdev, bool delay_enable)
{
	struct thp_core_data *cd = thp_get_core_data();
	struct thp_easy_wakeup_info *info = NULL;
	unsigned int stylus3_status;

	if (!cd->support_gesture_mode || !cd->lcd_gesture_mode_support ||
		!cd->support_pen_wakeup_gesture) {
		thp_log_err(tdev->thp_core, "%s: gesture not support\n", __func__);
		return -EINVAL;
	}
	...
	if ((stylus3_status > 0) && (cd->stylus_gesture_status == 1)) {
		g_lpwg_type = PEN_ENABLE;
		cd->sleep_mode = TS_GESTURE_MODE;
		if (info->easy_wakeup_gesture)
			g_lpwg_subtype = WAKE_UP_PEN_HAND; /* Hand & Pen */
		else
			g_lpwg_subtype = WAKE_UP_ONLY_PEN; /* Only Pen */
	} else {
		g_lpwg_type = PEN_DISABLE;
		if (info->easy_wakeup_gesture) {
			g_lpwg_subtype = WAKE_UP_ONLY_HAND; /* Only Hand */
			cd->sleep_mode = TS_GESTURE_MODE;
		} else {
			g_lpwg_subtype = WAKE_UP_NONE;
			cd->sleep_mode = TS_POWER_OFF_MODE;
		}
	}

	thp_log_info(tdev->thp_core,
		"%s: status:%u, stylus_gesture_status:%u ,subtype:0x%X, sleep_mode:%u\n",
		__func__, stylus3_status, cd->stylus_gesture_status,
		g_lpwg_subtype, cd->sleep_mode);
...
static struct thp_device_ops hx83112_dev_ops = {
	...
	.clear_interrupt = touch_driver_clear_all_interrupt,
};

int himax_driver_module_init(struct thp_core_data *cd)
{
	int rc = 0;
