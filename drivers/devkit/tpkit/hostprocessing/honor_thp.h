...
#elif IS_ENABLED(CONFIG_HONOR_THP_QCOM)
#include <hwmanufac/runmode_type/runmode_type.h>
#else
#include <linux/honor/hw_cmdline_parse.h>
#endif
#endif
...
#define EVENT_ANOMALY_MAX_COUNT 10
...
#define THP_IRQ_WAKE_ENABLE 1
#define THP_IRQ_WAKE_DISABLE 0
#define NO_ERR 0
...
#define THP_IOCTL_CMD_SET_RESUME_STATUS _IOW(THP_IO_TYPE, 0x13, u32)
...
#define THP_PEN_LIST_MAX_FRAMES 2
...
enum thp_afe_status_type {
	...
	THP_AFE_STATUS_TUI = 3,
	THP_AFE_STATUS_MAX,
};

enum thp_afe_notify_type {
	THP_AFE_NOTIFY_RESUME_FINISH = 3,
	THP_AFE_NOTIFY_MAX,
};
...
enum STYLUS_WAKEUP_CTRL {
	...
	STYLUS_GESTURE_WAKEUP_ENABLE = 5,
	STYLUS_GESTURE_WAKEUP_DISABLE = 6,
	MAX_STATUS = 255,
};
...
struct thp_device_ops {
	...
	int (*chip_wakeup_gesture_enable_switch)(struct thp_device *tdev,
			u8 switch_value);
	int (*tui_enable_switch)(struct thp_device *tdev,
		u8 switch_value);
	...
	void (*clear_interrupt)(struct thp_device *tdev);
#ifdef CONFIG_HONOR_SHB_THP
	int (*switch_int_sh)(struct thp_device *tdev);
	int (*switch_int_ap)(struct thp_device *tdev);
#endif
	int (*suspend_connect_pen)(struct thp_device *tdev);
};
...
enum ts_stylus_mode {
	TS_STYLUS_GESTURE_CLOSE = 0,
	TS_STYLUS_GESTURE_OPEN,
};
...
struct thp_cmd_param {
	union {
		...
		unsigned int aod_touch_status_switch;
	} pub_params;
	void *prv_params;
};
...
enum thp_cmd {
	...
	TS_SCREEN_FOLD,
	TS_SCREEN_UNFOLD,
	AOD_NOTIFY_SUBPANEL_SUSPEND,
	AOD_NOTIFY_SUBPANEL_RESUME,
	AOD_NOTIFY_SUBPANEL_POWER_CTRL,
	AOD_TOUCH_STATUS_SWITCH,
	TS_INVAILD_CMD = 255,
};
...
enum thp_irq_process_status {
	THP_IRQ_IDLE = 0,
	THP_IRQ_PROCESSING = 1,
};
#endif

enum input_mt_grip_type {
	INPUT_MT_WRAPPER_GRIP_TYPE_UNKNOWN = 0,
	INPUT_MT_WRAPPER_GRIP_TYPE_LEFT_HAND  = 1,
	INPUT_MT_WRAPPER_GRIP_TYPE_RIGHT_HAND = 2,
	INPUT_MT_WRAPPER_GRIP_TYPE_BOTH_HAND  = 3,
};

/* screen orientation, clock-wise: 0-0, 1-90, 2-180, 3-270 */
enum intput_mt_screen_rotate {
	INPUT_MT_WRAPPER_SCREEN_ROTATE_0 = 0,
	INPUT_MT_WRAPPER_SCREEN_ROTATE_90 = 1,
	INPUT_MT_WRAPPER_SCREEN_ROTATE_180 = 2,
	INPUT_MT_WRAPPER_SCREEN_ROTATE_270 = 3,
};

struct input_mt_wrapper_grip_data {
	int centroid_x;
	int centroid_y;
	int edge_width_x;
	int edge_width_y;
...
struct thp_mt_wrapper_grip_data {
	struct input_mt_wrapper_grip_data gripdata[INPUT_MT_WRAPPER_MAX_FINGERS];
	enum intput_mt_screen_rotate screen_rotate;
	enum input_mt_grip_type grip_type;
	int grip_num;
};
...
struct thp_core_data {
	...
	struct mutex aod_status_mutex;
	struct mutex grip_event_mutex;
	struct mutex msgq_mutex;
	struct hwspinlock *hwspin_lock;
	struct thp_power_supply thp_powers[THP_POWER_ID_MAX];
	struct thp_mt_wrapper_grip_data grip_data; /* For THP-grip assistance*/
	int project_in_tp;
	...
	unsigned int irq_wake_flag;
	unsigned int fast_booting_solution;
	unsigned int suspend_resume_control;
	...
	u32 aod_notify_subpanel_power_status;
	...
	struct task_struct *pvm_tui_task;
	...
	u32 platform_type;
	u32 holster_support;
	u32 support_esd_event;
	u32 talk_mode_support;
	u32 support_25D_anti_fake;
	...
	u32 support_ignore_first_irq_after_reset;
	...
	struct completion grip_data_setting_flag;
	...
	struct timeval report_pre_time;
	struct timeval report_cur_time;
	...
	bool suspend_fold_flag;
	bool talk_mode_flag;
	unsigned char sleep_mode_temp;
	u32 aod_mode_temp;
	u32 stylus_mode_temp;
	u32 support_screen_switch;
	u32 iovdd_power_on_delay_ms;
	u32 support_deepsleep_mode;
	u32 support_dynamic_resolution;
	u32 res_scale;
	u32 res_scale_pen;
	u32 support_b_protocol;
	u32 event_anomaly_count;
	u32 support_irq_storm_protect;
	u32 support_suspend_cs_low;
	u32 support_lowpower_no_irq;
	u32 support_no_spi_retry;
	u32 lowpower_support;
	u32 is_fold_device;
	u32 fold_status;
	u32 sub_solution;
	int thp_unregister_ic_num;
	struct thp_cmd_node ping_cmd_buff;
	struct thp_cmd_node pang_cmd_buff;
	u8 *spi_sync_tx_buf;
	u8 *spi_sync_rx_buf;
	u8 *queue_data_buf;
	bool thp_prox_enable;
	u8 is_fw_update;
	struct miscdevice *thp_misc_device;
	struct miscdevice *thp_mt_wrapper_misc_device;
	struct thp_mt_wrapper_data *thp_mt_wrapper;
	bool irq_storm_flag;
	bool onetime_poweroff_done;
	bool locked_by_daemon;
	u32 support_pen_wakeup_gesture;
	u32 stylus_gesture_status;
	u32 use_dual_spi_for_pen;
	int ic_type;
#if IS_ENABLED(CONFIG_HONOR_THP_QCOM)
	atomic_t suspend_irq_processing_status;
	wait_queue_head_t system_active_waitq;
	u8 system_active_waitq_flag;
#endif
	u32 support_grip_recognition;
};
...
struct tpkit_thp_ops {
	int (*power_notify)(enum lcd_kit_ts_pm_type pm_type,
...
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
#define thp_log_err(cd, msg, ...) \
	do { \
		if (cd) {\
			if (cd->multi_panel_index == SUB_TOUCH_PANEL) { \
				printk(KERN_ERR "[E/THP1] " msg, ## __VA_ARGS__); \
			} else { \
				printk(KERN_ERR "[E/THP0] " msg, ## __VA_ARGS__); \
			} \
		} else {\
			printk(KERN_ERR "[E/THP] " msg, ## __VA_ARGS__); \
		} \
	} while (0)
#define thp_log_info(cd, msg, ...) \
	do { \
		if (cd) {\
			if (cd->multi_panel_index == SUB_TOUCH_PANEL) { \
				printk(KERN_ERR "[I/THP1] " msg, ## __VA_ARGS__); \
			} else { \
				printk(KERN_ERR "[I/THP0] " msg, ## __VA_ARGS__); \
			} \
		} else {\
			printk(KERN_ERR "[I/THP] " msg, ## __VA_ARGS__); \
		} \
	} while (0)
#define thp_log_debug(cd, msg, ...) \
	do { \
		if (g_thp_log_cfg) { \
			if (cd) {\
				if (cd->multi_panel_index == SUB_TOUCH_PANEL) { \
					printk(KERN_ERR "[D/THP1] " msg, ## __VA_ARGS__); \
				} else { \
					printk(KERN_ERR "[D/THP0] " msg, ## __VA_ARGS__); \
				} \
			} else {\
				printk(KERN_ERR "[D/THP] " msg, ## __VA_ARGS__); \
			} \
		} \
	} while (0)
#else
...
#ifdef CONFIG_HONOR_TRUSTED_TOUCH
void thp_trusted_touch_init(struct thp_core_data *info);
void thp_tui_report_to_htee(struct thp_core_data *cd, struct thp_udfp_data udfp_data);
void thp_vm_deinit(struct thp_core_data *info);
int thp_handle_trusted_touch_tvm(struct thp_core_data *info, int value);
int thp_handle_trusted_touch_pvm(struct thp_core_data *info, int value);
int trusted_touch_msgq_register(struct thp_core_data *cd);
#endif

void thp_pen_send_detect_cmd(struct thp_core_data *cd, struct thp_device *dev,
	int timeout);
int thp_pen_parse_timing_config(struct thp_core_data *cd, struct device_node *timing_cfg_node,
	struct thp_timing_config *timing);
int thp_pen_parse_spi3_config(struct device_node *spi_cfg_node, struct thp_core_data *cd);
