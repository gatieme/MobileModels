...
#include "honor_thp.h"
	...
	(!defined CONFIG_LCD_KIT_DRIVER) && (!defined CONFIG_ARCH_QTI_VM))
#include "dsi_panel.h"
#endif
...
void thp_dmd_report(struct thp_core_data *cd, int dmd_num, const char *psz_format, ...)
{
	va_list args;
	char *input_buf = NULL;
	int ret;
...
#ifndef CONFIG_ARCH_QTI_VM
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
static void send_event_to_fingerprint_ud(struct thp_core_data *cd, struct thp_udfp_data udfp_data);
#endif
#endif
...
int thp_power_control_notify(struct thp_core_data *cd, enum lcd_kit_ts_pm_type pm_type, int timeout);
int thp_single_power_control_notify(enum lcd_kit_ts_pm_type pm_type, int timeout);
...
int thp_multi_power_control_notify(enum lcd_kit_ts_pm_type pm_type, int timeout,
	int panel_index);
int thp_sub_power_control_notify(enum lcd_kit_ts_pm_type pm_type, int timeout);
...
int thp_power_supply_ctrl(struct thp_core_data *cd,
	enum thp_power_id power_id,
	int status, unsigned int delay_ms)
{
	...
	case THP_POWER_LDO:
		...
		if (!status && power->ldo_transition_value != 0) {
			rc = regulator_set_voltage(power->regulator,
				power->ldo_transition_value, power->ldo_transition_value);
			if (rc)
				thp_log_err(cd, "%s: set %d err = %d", __func__,
	...
	struct thp_core_data *cd, int power_id)
{
	...
	case THP_POWER_LDO:
		...
		if (rc) {
			thp_log_info(cd, "%s:not set %s\n", __func__, config_name);
			power->ldo_transition_value = 0;
		}
		thp_log_info(cd, "%s:set %s %d\n", __func__, config_name,
			power->ldo_transition_value);
		break;
	case THP_POWER_PMIC:
		...
		break;
	default:
		thp_log_err(cd, "%s: invaild power type %d", __func__, power->type);
		break;
	}
...
static struct thp_queue *thp_queue_init(struct thp_core_data *cd)
{
	...
	cd->queue_data_buf = vzalloc(cd->thp_queue_buf_len * THP_LIST_MAX_FRAMES);
...
static bool thp_queue_is_empty(struct thp_core_data *cd, struct thp_queue *queue)
{
	if (queue == NULL) {
...
static void thp_queue_clear_all(struct thp_core_data *cd, struct thp_queue *queue)
{
	int i;
	...
	char *tx_buf, char *rx_buf, unsigned int len, unsigned int lock_status)
{
	...
	if ((cd->pre_suspended || cd->suspended) && (!cd->need_work_in_suspend)) {
...
void thp_set_irq_status(struct thp_core_data *cd, int status)
{
	...
	if (atomic_read(&cd->tui_irq_disabled)) {
		thp_log_err(cd, "%s: tui mode don't allow to set irq status %d now.\n",
			__func__, status);
		return;
	}
#endif
...
#ifdef CONFIG_HONOR_TRUSTED_TOUCH
	if (atomic_read(&cd->tui_irq_disabled)) {
		thp_log_err(cd, "%s: tui mode don't allow to set irq wake status %d now.\n",
			__func__, status);
		return;
	}
#endif
...
bool need_work_in_suspend_switch(struct thp_core_data *cd)
{
	...
	result = (double_tap_status || ap_gesture_status ||
		cd->stylus_gesture_status) && !cd->support_shb_thp;
	return result;
}
...
static int thp_suspend(struct thp_core_data *cd)
{
	...
	if (need_work_in_suspend_switch(cd)) {
		if ((cd->use_dual_spi_for_pen == 1) &&(cd->thp_dev->ops->clear_interrupt))
...
static void thp_early_suspend(struct thp_core_data *cd)
{
	...
	if (cd->delay_work_for_pm) {
		if (cd->suspend_resume_waitq_flag != WAITQ_WAKEUP) {
			...
			if (!rc)
				thp_log_err(cd, "%s:wait resume complete timeout\n",
					__func__);
		}
...
#if (!defined(CONFIG_LCD_KIT_DRIVER) && !defined(CONFIG_ARCH_QTI_VM))
	...
	unsigned long event, void *data)
{
	...
	case LCDKIT_TS_EARLY_SUSPEND:
		...
		if (cd->proximity_support != PROX_SUPPORT) {
			...
			thp_prox_suspend_record_time(cd);
			thp_log_info(cd, "%s:early suspend!thp_prox_enable=%d\n",
				__func__, cd->thp_prox_enable);
			cd->need_work_in_suspend = cd->thp_prox_enable;
			cd->prox_cache_enable = cd->thp_prox_enable;
			cd->early_suspended = true;
...
static void thp_easy_wake_mode_change(struct thp_core_data *cd)
{
	...
	if (cd->aod_touch_status != TS_TOUCH_AOD_CLOSE) {
		mutex_lock(&cd->aod_status_mutex);
		cd->aod_mode_temp = cd->aod_touch_status;
		cd->aod_touch_status = TS_TOUCH_AOD_CLOSE;
	}
	if (cd->stylus_gesture_status != TS_STYLUS_GESTURE_CLOSE) {
		cd->stylus_mode_temp = cd->stylus_gesture_status;
		cd->stylus_gesture_status = TS_STYLUS_GESTURE_CLOSE;
	}
	thp_log_info(cd, "MAIN_TOUCH_PANEL enter TS_SCREEN_SWITCH mode\n");
}
	...
	enum lcd_kit_ts_pm_type pm_type)
{
	...
	if ((pm_type == TS_EARLY_SUSPEND) &&
		(atomic_read(&cd->tp_ic_tui_enabled) || atomic_read(&cd->trusted_touch_transition))) {
		if (!wait_for_completion_timeout(&cd->tp_ic_tui_exited,
				msecs_to_jiffies(5000)))
			thp_log_err(cd, "TP IC don't exit TUI mode in 5000 ms\n");
		else
			thp_log_info(cd, "TP IC status is TUI[%d] now\n", atomic_read(&cd->tp_ic_tui_enabled));
	}
#endif
	...
	case TS_SUSPEND_DEVICE:
		if (cd->multi_panel_index != SINGLE_TOUCH_PANEL) {
			if (cd->sleep_mode_temp != TS_POWER_OFF_MODE) {
				...
				thp_log_info(cd, "MAIN_PANEL sleep_mode revert\n");
			}
			if (cd->aod_mode_temp != TS_TOUCH_AOD_CLOSE) {
				cd->aod_touch_status = cd->aod_mode_temp;
				cd->aod_mode_temp = TS_TOUCH_AOD_CLOSE;
				mutex_unlock(&cd->aod_status_mutex);
				thp_log_info(cd, "MAIN_PANEL aod_mode revert\n");
			}
			if (cd->stylus_mode_temp != TS_STYLUS_GESTURE_CLOSE) {
				cd->stylus_gesture_status = cd->stylus_mode_temp;
				cd->stylus_mode_temp = TS_STYLUS_GESTURE_CLOSE;
				thp_log_info(cd, "MAIN_PANEL stylus_mode revert\n");
			}
		}
		thp_log_info(cd, "%s: suspend\n", __func__);
		thp_clean_fingers(cd);
	...
	case TS_BEFORE_SUSPEND:
		...
		if (cd->support_daemon_init_protect &&
			...
			thp_log_info(cd, "%s: suspend when fw update, return directly\n",
				__func__);
			thp_set_status(cd, THP_STATUS_POWER, THP_RESUME);
		...
		if (cd->support_shb_thp_app_switch) {
			...
			if (ret)
				thp_log_err(cd, "%s: send_thp_cmd_to_shb fail\n",
					__func__);
			break;
		}
	...
	case TS_RESUME_DEVICE:
		...
		if (cd->support_daemon_init_protect &&
			...
			thp_log_info(cd, "%s: resume when fw update, return directly\n",
				__func__);
			return 0;
		}
		thp_resume(cd);
...
int thp_proc_screen_switch_cmd(struct thp_core_data *cd, enum lcd_kit_ts_pm_type pm_type, int timeout,
	int panel_index)
{
	int ret = 0;
	...
	case SUB_TOUCH_PANEL:
		...
		if (cd->sub_solution == THP_SOLUTION) {
			...
			if (ops && ops->power_notify)
				ret = ops->power_notify(pm_type, timeout, SUB_TOUCH_PANEL);
			else
				thp_log_err(cd, "%s: ops or ops->power_notify is null\n", __func__);
		}
#endif
		break;
	default:
...
int multi_screen_status_switch(struct thp_core_data *cd, u32 current_status)
{
	struct thp_core_data *cd_sub = thp_get_sub_core_data();
	int i;
	...
	} else if (current_status == SCREEN_UNFOLD && cd->work_status != POWER_OFF_DONE) {
		thp_log_err(cd,
			"%s: folded state inner_screen of TP not poweroff, dmd code:%d\n",
			__func__, DSM_TP_POWEROFF_ERROR_NO);
		thp_dmd_report(cd, DSM_TP_POWEROFF_ERROR_NO,
			"%s, folded state inner_screen of TP not poweroff\n", __func__);
	}
#endif
	if ((cd->sleep_mode == TS_POWER_OFF_MODE) &&
		(cd->aod_touch_status == 0) && (cd->stylus_gesture_status == 0)) {
	...
	if (current_status == SCREEN_UNFOLD) {
		...
		for (i = 0; i < RESUME_TYPE_LEN; i++)
			thp_proc_screen_switch_cmd(cd, pm_type_list[i],
				SHORT_SYNC_TIMEOUT, MAIN_TOUCH_PANEL);
...
static void thp_set_multi_pm_status(struct thp_core_data *cd, enum lcd_kit_ts_pm_type pm_type,
	int panel_index)
{
...
#if IS_ENABLED(CONFIG_DRIVER_BRIGE)
	if (cd->sub_solution == THP_SOLUTION) {
		err = thp_sub_power_control_notify(TS_RESUME_DEVICE, SHORT_SYNC_TIMEOUT);
		if (err)
			thp_log_err(cd, "%s: TS_RESUME_DEVICE fail\n", __func__);
		err = thp_sub_power_control_notify(TS_AFTER_RESUME, SHORT_SYNC_TIMEOUT);
		if (err)
			thp_log_err(cd, "%s: TS_AFTER_RESUME fail\n", __func__);
	} else {
		if (ops && ops->power_notify) {
...
#if IS_ENABLED(CONFIG_DRIVER_BRIGE)
	if (cd->sub_solution == THP_SOLUTION) {
		err = thp_sub_power_control_notify(TS_EARLY_SUSPEND, SHORT_SYNC_TIMEOUT);
		if (err)
			thp_log_err(cd, "%s: TS_EARLY_SUSPEND fail\n", __func__);
		err = thp_sub_power_control_notify(TS_BEFORE_SUSPEND, SHORT_SYNC_TIMEOUT);
		if (err)
			thp_log_err(cd, "%s: TS_BEFORE_SUSPEND fail\n", __func__);
		err = thp_sub_power_control_notify(TS_SUSPEND_DEVICE, SHORT_SYNC_TIMEOUT);
		if (err)
			thp_log_err(cd, "%s: TS_SUSPEND_DEVICE fail\n", __func__);
	} else {
		if (ops && ops->power_notify) {
...
static void aod_notify_subpanel_power(struct thp_core_data *cd,
	struct thp_cmd_node *in_cmd, struct thp_cmd_node *out_cmd)
{
	static unsigned int last_status = INVALID_VALUE;
...
static void aod_touch_status_switch(struct thp_core_data *cd,
	struct thp_cmd_node *in_cmd)
{
	struct thp_core_data *cd_proc = NULL;

	if (in_cmd == NULL) {
		thp_log_err(cd, "%s: in_cmd is null\n", __func__);
		return;
	}
	cd_proc = (struct thp_core_data*)(in_cmd->cmd_param.prv_params);
	if (cd_proc == NULL) {
		thp_log_err(cd, "%s: cd_proc is null\n", __func__);
		return;
	}

	mutex_lock(&cd_proc->aod_status_mutex);
	cd_proc->aod_touch_status =
		in_cmd->cmd_param.pub_params.aod_touch_status_switch;
	mutex_unlock(&cd_proc->aod_status_mutex);
	thp_log_info(cd_proc, "%s: update aod_touch_enable: %u\n", __func__,
		cd_proc->aod_touch_status);
	return;
}
...
static long thp_ioctl_spi_sync(struct thp_core_data *cd, const void __user *data,
...
#ifdef CONFIG_HONOR_TRUSTED_TOUCH
		if (atomic_read(&cd->tp_ic_tui_enabled) && thp_get_status(cd, THP_STATUS_TUI) == 1) {
			thp_log_err(cd, "%s: tui mode don't allow to send cmd to tp ic again.\n", __func__);
			return 0;
		}
#endif
...
static long thp_ioctl_get_frame_count(struct thp_core_data *cd, unsigned long arg)
{
	...
	struct thp_ioctl_get_frame_data *data)
{
	...
	thp_set_irq_status(cd, THP_IRQ_ENABLE);
	mutex_unlock(&cd->suspend_flag_mutex);
	if (thp_queue_is_empty(cd, cd->thp_queue) &&
...
static long thp_ioctl_get_frame(struct thp_core_data *cd, unsigned long arg, unsigned int f_flag)
{
	struct thp_ioctl_get_frame_data data;
...
static long thp_ioctl_set_timeout(struct thp_core_data *cd, unsigned long arg)
{
	...
	if (thp_get_status(cd, THP_STATUS_TUI) == 1) {
		thp_log_info(cd, "set wait time %d ms.(current %dms), do nothing in tui mode\n",
...
static long thp_ioctl_set_afe_status(struct thp_core_data *cd, const void __user *data)
{
	...
	case THP_AFE_STATUS_TUI:
		thp_log_info(cd, "%s: THP_AFE_STATUS_TUI called %d\n",
			__func__, set_afe_status.status);
		if (set_afe_status.status == 0) {
			complete_all(&cd->tp_ic_tui_exited);
		} else if (set_afe_status.status == 1) {
			complete_all(&cd->tp_ic_tui_entered);
		}
		break;
#endif
	default:
	...
	unsigned long arg)
{
	...
	if (atomic_read(&(cd->trusted_touch_enabled))) {
		thp_log_err(cd, "%s: Cannot service ioctl cmd %x in PVM while trusted touch is enabled\n",
				__func__, cmd);
		return 0;
	}
#endif
	...
	case THP_IOCTL_CMD_SPI_SYNC_NO_LOCK:
		ret = thp_ioctl_spi_sync(cd, (void __user *)arg, DONOT_NEED_LOCK);
		break;
...
#ifndef CONFIG_ARCH_QTI_VM
#define SCREENOFF_FINGER_EVENT 5
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
static void send_event_to_fingerprint_ud(struct thp_core_data *cd, struct thp_udfp_data udfp_data)
{
	...
	/* origin fingerdown:0 finggerup:1, screenoff fingerdown:5 fingerup:6 */
	udfp_data.tpud_data.udfp_event += SCREENOFF_FINGER_EVENT;
	thp_log_debug(cd, "%s send screen off fingerprint event\n", __func__);
	if (fp_ops->fp_irq_notify(&udfp_data.tpud_data))
		thp_log_err(cd, "%s: tpud event notify fp failed\n", __func__);
}
#endif
#endif

static void tp_event_dispatch(struct thp_core_data *cd, struct thp_udfp_data udfp_data)
{
	thp_log_info(cd, "%s: fp_event = %d, aod_event = %u, keyevent = %u\n",
		__func__, udfp_data.tpud_data.udfp_event, udfp_data.aod_event,
		udfp_data.key_event);

#ifdef CONFIG_ARCH_QTI_VM
	// send event to htee directly
	thp_tui_report_to_htee(cd, udfp_data);
#else
...
static void thp_copy_frame_to_thp_queue(struct thp_core_data *cd)
{
	...
	if (cd->frame_count >= THP_LIST_MAX_FRAMES) {
		if (cd->frame_count != pre_frame_count)
			thp_log_err(cd, "frame_lite buf full start,frame_count:%d\n",
				cd->frame_count);
...
static irqreturn_t thp_irq_thread(int irq, void *dev_id)
{
...
#else
	if (atomic_read(&(cd->trusted_touch_enabled))) {
		thp_log_err(cd, "%s: Cannot service interrupts in PVM while trusted touch is enabled\n",
				__func__);
		return IRQ_HANDLED;
	}
#endif
	...
	rc = wait_event_interruptible_timeout(cd->system_active_waitq,
...
#ifdef CONFIG_HONOR_BCI_BATTERY
static int thp_charger_detect_notifier_callback(struct thp_core_data *cd, struct notifier_block *self,
					unsigned long event, void *data)
{
	int charger_stat_before = thp_get_status(cd, THP_STATUS_CHARGER);
	int charger_state_new = charger_stat_before;

	thp_log_debug(cd, "%s called, charger type: %lu\n", __func__, event);
	...
	if (charger_stat_before != charger_state_new) {
		thp_log_info(cd, "%s, set new status: %d\n",
			__func__, charger_state_new);
		thp_set_status(cd, THP_STATUS_CHARGER, charger_state_new);
	}

	return 0;
}
static int thp_charger_detect_notifier_callback_main(struct notifier_block *self,
					unsigned long event, void *data)
{
	struct thp_core_data *cd = thp_get_core_data();

	return thp_charger_detect_notifier_callback(cd, self, event, data);
}

static int thp_charger_detect_notifier_callback_sub(struct notifier_block *self,
					unsigned long event, void *data)
{
	struct thp_core_data *cd_sub = thp_get_sub_core_data();
	...
	return thp_charger_detect_notifier_callback(cd_sub, self, event, data);
}
#endif
...
static int thp_projectid_to_vender_name(struct thp_core_data *cd, const char *project_id,
...
static int thp_projectid_to_ic_name(struct thp_core_data *cd, const char *project_id,
...
static int thp_init_chip_info(struct thp_core_data *cd)
{
	...
	rc = thp_projectid_to_ic_name(cd, cd->project_id,
...
void thp_tui_secos_init(void)
{
	...
	if (cd->support_shb_thp) {
		if (cd->thp_dev && cd->thp_dev->ops &&
			...
			if (ret)
				thp_log_err(cd, "%s: switch to sh fail", __func__);
		}
		thp_set_irq_status(cd, THP_IRQ_DISABLE);
...
void thp_tui_secos_exit(void)
{
	...
	if (cd->support_shb_thp) {
		if (cd->thp_dev && cd->thp_dev->ops &&
			...
			if (ret)
				thp_log_err(cd, "%s: switch to ap fail", __func__);
			}
...
static int multi_tp_tui_switch(void *data, int secure)
{
	...
	case M_ON_S_OFF:
		memcpy(&thp_tui_info, &tp_tui_data[MAIN_TOUCH_PANEL],
			sizeof(thp_tui_info));
		ret = tp_tui_data[MAIN_TOUCH_PANEL].tui_drv_switch(data, secure);
...
static void thp_tui_init(struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL) {
		...
		if (cd->multi_panel_index == MAIN_TOUCH_PANEL) {
			thp_log_err(cd, "%s register_tui_driver thp_tui_switch\n",
				__func__);
...
static int thp_edit_product_in_project_id(struct thp_core_data *cd, char *project_id,
...
static int thp_project_init(struct thp_core_data *cd)
{
	...
	if (cd->edit_product_in_project_id)
		ret = thp_edit_product_in_project_id(cd, cd->project_id,
			THP_PROJECT_ID_LEN, cd->product);
	if (cd->support_project_id_mapping)
		ret = thp_project_id_mapping(cd);
	thp_log_info(cd, "%s:edit_product:%d ret:%d\n", __func__,
		cd->edit_product_in_project_id, ret);
	if (ret != 0)
		return -EINVAL;
...
static int thp_ctrl_event_call(struct thp_core_data *cd, struct notifier_block *nb,
	unsigned long event, void *data)
{
	int charger_state_before = thp_get_status(cd, THP_STATUS_CHARGER);
	int charger_state_new = charger_state_before;

	thp_log_debug(cd, "%s char event %d\n", __func__, event);
	switch (event) {
...
static int thp_ctrl_event_call_main(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct thp_core_data *cd = thp_get_core_data();

	return thp_ctrl_event_call(cd, nb, event, data);
}
static int thp_ctrl_event_call_sub(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct thp_core_data *cd_sub = thp_get_sub_core_data();
	...
	return thp_ctrl_event_call(cd_sub, nb, event, data);
}
...
static void thp_nt_charger_notifier_register(struct thp_core_data *cd)
{
	...
	if (!cd->supported_charger) {
		thp_log_info(cd, "%s No supported charger\n", __func__);
		return;
	}
	if (cd->multi_panel_index == SUB_TOUCH_PANEL)
		cd->charger_detect_notify.notifier_call = thp_ctrl_event_call_sub;
	else
		cd->charger_detect_notify.notifier_call = thp_ctrl_event_call_main;
	ret = power_event_nc_register(POWER_NT_CONNECT,
		&cd->charger_detect_notify);
	if (ret) {
		thp_log_err(cd, "%s power_event_nc_register failed\n", __func__);
...
static int thp_core_init(struct thp_core_data *cd)
{
	...
	mutex_init(&cd->aod_status_mutex);
...
#if IS_ENABLED(CONFIG_DSM)
	memset(&cd->dsm_thp, 0, sizeof(cd->dsm_thp));
	cd->dsm_thp.ic_name = THP_DSM_IC_NAME;
	cd->dsm_thp.module_name = THP_DSM_MODULE_NAME;
	cd->dsm_thp.device_name = THP_DSM_DEVICE_NAME;
	cd->dsm_thp.buff_size = THP_DSM_BUFF_SIZE;
	cd->dsm_thp.name = THP_DSM_DEV_NAME_SINGLE;
	if (cd->ic_name)
	...
	if (cd->multi_panel_index != SINGLE_TOUCH_PANEL) {
		if (cd->multi_panel_index == SUB_TOUCH_PANEL)
			cd->dsm_thp.name = THP_DSM_DEV_NAME_SUB;
		else
			cd->dsm_thp.name = THP_DSM_DEV_NAME_MAIN;
	}

	thp_log_info(cd, "%s: dsm_thp.name is %s\n", __func__, cd->dsm_thp.name);
	cd->dsm_thp_dclient = dsm_register_client(&cd->dsm_thp);
#endif
...
#if defined(CONFIG_LCD_KIT_DRIVER)
	if ((cd->multi_panel_index == MAIN_TOUCH_PANEL) || (cd->multi_panel_index == SINGLE_TOUCH_PANEL)) {
	...
	if (cd->multi_panel_index == SUB_TOUCH_PANEL) {
		cd->charger_detect_notify.notifier_call =
				thp_charger_detect_notifier_callback_sub;
	} else {
		cd->charger_detect_notify.notifier_call =
				thp_charger_detect_notifier_callback_main;
	}
	rc = honor_register_notifier(&cd->charger_detect_notify, 1);
	if (rc < 0) {
		thp_log_err(cd, "%s:charger notifier register failed\n", __func__);
		cd->charger_detect_notify.notifier_call = NULL;
	} else {
		thp_log_info(cd, "%s:charger notifier register succ\n", __func__);
	}
#endif
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
#ifndef CONFIG_ARCH_QTI_VM
...
static int thp_parse_test_config(struct thp_core_data *cd, struct device_node *test_node,
...
static void thp_chip_detect(struct thp_core_data *cd, struct thp_cmd_node *in_cmd)
{
	int ret;
...
void thp_send_detect_cmd(struct thp_core_data *cd, struct thp_device *dev, int timeout)
{
	int error;
	struct thp_cmd_node cmd;
...
int thp_register_dev(struct thp_core_data *cd, struct thp_device *dev)
{
	...
	if (!cd->project_in_tp && cd->ic_name && dev->ic_name &&
		...
		thp_log_err(cd, "%s:driver support ic mismatch connected device\n",
					__func__);
		goto register_err;
	}
...
register_err:
	if (cd && cd->fast_booting_solution) {
		...
		thp_log_info(cd, "%s:thp_unregister_ic_num :%d",
			__func__, cd->thp_unregister_ic_num);
#if IS_ENABLED(CONFIG_DSM)
		if (!cd->thp_unregister_ic_num && !atomic_read(&cd->register_flag)) {
			thp_log_err(cd, "%s:ALL TP IC register fail, DMD code: %d",
				__func__, DSM_TPHOSTPROCESSING_DEV_IC_ALL_FAIL);
	...
	struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index == SUB_TOUCH_PANEL)
		...
		cd->spi_config.pl022_spi_config.cs_control = thp_spi_cs_set_master;
#endif
...
int thp_parse_timing_config(struct thp_core_data *cd, struct device_node *timing_cfg_node,
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	cd->res_scale_pen = 1;
	if (!recovery_mode) {
		rc = of_property_read_u32(thp_node, "support_dynamic_resolution",
			&value);
		if (!rc) {
			cd->support_dynamic_resolution = value;
			thp_log_info(cd, "%s:support_dynamic_resolution configed: %u\n",
				 __func__, value);
		}
		if (cd->support_dynamic_resolution) {
			rc = of_property_read_u32(thp_node, "res_scale", &value);
			if (!rc) {
				if (thp_is_factory())
					cd->res_scale = 1;
				else
			...
			rc = of_property_read_u32(thp_node, "res_scale_pen", &value);
			if (!rc) {
				if (thp_is_factory())
					cd->res_scale_pen = 1;
				else
	...
	rc = of_property_read_u32(thp_node, "support_pen_wakeup_gesture", &value);
	if (!rc) {
	...
	struct thp_core_data *cd)
{
	...
	if (cd->use_aod_power_ctrl_notify) {
		if (!of_property_read_u32(thp_node,
			"suspend_delayms_early_to_before", &value)) {
			cd->suspend_delayms_early_to_before = value;
			thp_log_info(cd, "%s:suspend_delayms_early_to_before %u\n",
				__func__, value);
		}
		if (!of_property_read_u32(thp_node,
			"suspend_delayms_before_to_later", &value)) {
			cd->suspend_delayms_before_to_later = value;
			thp_log_info(cd, "%s:suspend_delayms_before_to_later %u\n",
				__func__, value);
		}
		if (!of_property_read_u32(thp_node,
			"resume_delayms_early_to_later", &value)) {
			cd->resume_delayms_early_to_later = value;
			thp_log_info(cd, "%s:resume_delayms_early_to_later %u\n",
				__func__, value);
		}
	}
#endif
	...
	if (!rc) {
		...
		if (!rc) {
			...
			thp_log_info(cd, "%s:gesture from sensorhub configed %u\n",
				__func__, value);
		}
	}
	...
	if (!rc) {
		...
		thp_log_info(cd, "%s:support_extra_key_event_input configed %u\n",
			__func__, value);
	}
...
int is_pt_test_mode(struct thp_device *tdev)
{
	...
	if ((lcd_ops) && (lcd_ops->get_status_by_type)) {
		ret = lcd_ops->get_status_by_type(PT_STATION_TYPE,
			&thp_pt_station_flag);
		if (ret < 0) {
			thp_log_info(tdev->thp_core, "%s: get thp_pt_station_flag fail\n",
				__func__);
			return ret;
		}
	}
...
bool thp_get_prox_switch_status(void)
{
	...
	thp_log_info(cd, "[Proximity_feature] %s :Not support proximity feature!\n",
		__func__);
	return 0;
}
	...
	struct device_node *thp_node)
{
	...
	if (!rc) {
		...
		thp_log_info(cd, "%s:parse success, support_25D_anti_fake = %u\n",
...
static int thp_cmd_sync_allocate(struct thp_core_data *cd, struct thp_cmd_node *cmd, int timeout)
{
...
int thp_put_one_cmd(struct thp_core_data *cd, struct thp_cmd_node *cmd, int timeout)
{
	int error = -EIO;
	unsigned long flags;
...
static int thp_proc_command(struct thp_core_data *cd, struct thp_cmd_node *cmd)
{
	...
	while (true) {
		...
		case AOD_NOTIFY_SUBPANEL_POWER_CTRL:
			aod_notify_subpanel_power(cd, proc_cmd, out_cmd);
			break;
#endif
		case AOD_TOUCH_STATUS_SWITCH:
			aod_touch_status_switch(cd, proc_cmd);
			break;
		default:
			break;
		}
...
static int thp_thread_init(struct thp_core_data *cd)
{
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL)
		...
		cd->thp_task = kthread_create(thp_thread, cd, "ts_thread:%d", cd->multi_panel_index);
...
static int pvm_touch_driver_thread(void *thp_core)
{
	...
	if (!buf) {
		thp_log_err(cd, "%s: kzalloc buf failed!\n", __func__);
		return -ENOMEM;
	}
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL ||
		cd->multi_panel_index == MAIN_TOUCH_PANEL) {
		ret = gh_msgq_recv(cd->vm_info->tp_main_msgq, buf,
			GH_MSGQ_MAX_MSG_SIZE_BYTES, &buf_size, 0);
		if (ret) {
			kfree(buf);
			thp_log_err(cd, "%s: recv msg from vm fail, ret: %d\n", __func__, ret);
			return -EINVAL;
		}
		value = *(int *)buf;
		if (value) {
			ret = gh_msgq_send(cd->vm_info->tp_main_msgq, &cd->ic_type,
				sizeof(cd->ic_type), GH_MSGQ_TX_PUSH);
			if (ret)
				thp_log_err(cd, "%s: send msg to vm fail, ret: %d\n", __func__, ret);
		} else {
			thp_log_err(cd, "%s: invalid value = %d\n", __func__, value);
		}
	} else {
		ret = gh_msgq_recv(cd->vm_info->tp_sub_msgq, buf,
			GH_MSGQ_MAX_MSG_SIZE_BYTES, &buf_size, 0);
		if (ret) {
			kfree(buf);
			thp_log_err(cd, "%s: recv msg from vm fail, ret: %d\n", __func__, ret);
		...
		value = *(int *)buf;
		if (value) {
			ret = gh_msgq_send(cd->vm_info->tp_sub_msgq, &cd->ic_type,
				sizeof(cd->ic_type), GH_MSGQ_TX_PUSH);
			if (ret)
				thp_log_err(cd, "%s: send msg to vm fail, ret: %d\n", __func__, ret);
		}
...
int trusted_touch_msgq_register(struct thp_core_data *cd)
{
	mutex_lock(&cd->msgq_mutex);

	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL ||
		cd->multi_panel_index == MAIN_TOUCH_PANEL) {
		if (!cd->vm_info->tp_main_msgq) {
			cd->vm_info->tp_main_msgq = gh_msgq_register(TRUSTED_TP_MAIN_MSGQ_LABEL);
			if (!cd->vm_info->tp_main_msgq) {
				thp_log_err(cd, "Failed to register on TRUSTED_TP_MAIN_MSGQ_LABEL\n");
				mutex_unlock(&cd->msgq_mutex);
				return -EINVAL;
			}
		} else {
			thp_log_info(cd, "tp_main_msgq already register\n");
		}
	} else {
		if (!cd->vm_info->tp_sub_msgq) {
			cd->vm_info->tp_sub_msgq = gh_msgq_register(TRUSTED_TP_SUB_MSGQ_LABEL);
			if (!cd->vm_info->tp_sub_msgq) {
				thp_log_err(cd, "Failed to register on TRUSTED_TP_SUB_MSGQ_LABEL\n");
				mutex_unlock(&cd->msgq_mutex);
				return -EINVAL;
			}
		} else {
			thp_log_info(cd, "tp_sub_msgq already register\n");
		}
	}
...
{
	cd->pvm_tui_task = kthread_create(pvm_touch_driver_thread, cd, "pvm_touch_driver_thread:%d", 0);
	wake_up_process(cd->pvm_tui_task);
	return NO_ERR;
}
#endif

#ifdef CONFIG_ARCH_QTI_VM
#define TVM_TP_DRIVER_MODULE_INIT_START 1
static int tvm_touch_driver_module_init(struct thp_core_data *cd)
{
	int ret;
	int value;
	int tvm_tp_init_flag = TVM_TP_DRIVER_MODULE_INIT_START;
	void *buf = NULL;
	size_t buf_size;
	...
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL ||
		cd->multi_panel_index == MAIN_TOUCH_PANEL) {
		ret = gh_msgq_send(cd->vm_info->tp_main_msgq, &tvm_tp_init_flag,
			sizeof(tvm_tp_init_flag), 0);
		if (ret)
			thp_vm_log_err(cd, "%s:Failed to send main message: %d\n", __func__, ret);
		ret = gh_msgq_recv(cd->vm_info->tp_main_msgq, buf,
			GH_MSGQ_MAX_MSG_SIZE_BYTES, &buf_size, 0);
		if (ret) {
			kfree(buf);
			thp_vm_log_err(cd, "%s:Failed to receive main message: %d\n", __func__, ret);
			return -EINVAL;
		}
	} else {
		ret = gh_msgq_send(cd->vm_info->tp_sub_msgq, &tvm_tp_init_flag,
			sizeof(tvm_tp_init_flag), 0);
		if (ret)
			thp_vm_log_err(cd, "%s:Failed to send sub message: %d\n", __func__, ret);
		ret = gh_msgq_recv(cd->vm_info->tp_sub_msgq, buf,
			GH_MSGQ_MAX_MSG_SIZE_BYTES, &buf_size, 0);
		if (ret) {
			kfree(buf);
			thp_vm_log_err(cd, "%s:Failed to receive sub message: %d\n", __func__, ret);
			return -EINVAL;
		}
	}
	value = *(int *)buf;
	switch (value) {
		case IC_TYPE_GOODIX:
			goodix_driver_module_init(cd);
			break;
		case IC_TYPE_SYNA:
			syna_driver_module_init(cd);
			break;
		default:
			thp_vm_log_err(cd, "%s:module_init_value= %d\n", __func__, value);
			break;
	}
...
static int thp_probe(struct spi_device *sdev)
{
	...
	if (!lcd_kit_get_ops()) {
		thp_log_info(thp_core, "%s: lcdkit not init, need wait\n", __func__);
		return -EPROBE_DEFER;
	}
#endif
	...
	if (thp_core->multi_panel_index == SUB_TOUCH_PANEL && recovery_mode == true) {
	...
	if (thp_core->use_hwlock) {
		...
		if (!thp_core->hwspin_lock)
			thp_log_err(thp_core, "%s: hwspin_lock request failed\n",
				__func__);
	}

#ifdef CONFIG_HONOR_TRUSTED_TOUCH
	thp_trusted_touch_init(thp_core);
	mutex_init(&(thp_core->fts_clk_io_ctrl_mutex));
#endif
...
#ifdef CONFIG_HONOR_TRUSTED_TOUCH
		if (atomic_read(&thp_core->trusted_touch_mode) != TRUSTED_TOUCH_MODE_NONE) {
			rc = trusted_touch_msgq_register(thp_core);
			if (rc)
				thp_log_err(thp_core, "%s: Failed to trusted_tp_msgq_register\n",__func__);
#ifdef CONFIG_ARCH_QTI_VM
			tvm_touch_driver_module_init(thp_core);
			return NO_ERR;
#endif
			rc = pvm_touch_driver_thread_init(thp_core);
		}
#endif
		syna_driver_module_init(thp_core);

		himax_driver_module_init(thp_core);
...
int thp_probe_delayed(struct thp_core_data *cd)
{
	/* Avoid setting up hardware for TVM during probe */
#ifdef CONFIG_ARCH_QTI_VM
	thp_log_info(cd, "thp_delay_probe %d\n", cd->delayed_vm_probe_pending);
	if (!atomic_read(&cd->delayed_vm_probe_pending)) {
		thp_log_info(cd, "thp_delay_probe %d, return here\n", cd->delayed_vm_probe_pending);
		atomic_set(&cd->delayed_vm_probe_pending, 1);
		return 0;
	}
#endif

	thp_log_info(cd, "%s: called\n", __func__);
	return thp_setup_irq(cd);
}
