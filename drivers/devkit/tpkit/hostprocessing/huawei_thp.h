...
enum thp_sub_event_type {
	...
	THP_SUB_EVENT_REQUEST_AP_RESET = 6,
};
...
struct thp_device_ops {
	...
	int (*get_color_id)(struct thp_device *tdev);
...
enum ts_fingerprint_mode {
	TS_TOUCH_UDFP_CLOSE = 0,
	TS_TOUCH_UDFP_OPEN,
};
...
struct thp_esd_param {
	u32 support_esd_power_reset;
	u32 esd_delay_time;
	u32 support_gesture_mode_revert;
	atomic_t esd_suspend_status_check;
	u8 esd_waitq_flag;
	wait_queue_head_t esd_waitq;
};
...
struct thp_core_data {
	...
	atomic_t last_stylus_lite_status;
	...
	u32 suspend_udfp_status;
	u32 suspend_udfp_status_temp;
	...
	u32 thp_shb_timeout_hard_reset;
	u32 have_internal_flash;
...
enum case_type {
	...
	THP_SECURITY,
	THP_BEHAVIOR,
	THP_INVALID_TYPE = 255
};

struct hisysevent_info {
	const char *casename;
	enum case_type type;
};
