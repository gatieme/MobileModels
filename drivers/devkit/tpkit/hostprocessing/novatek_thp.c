...
#define NVT_CRC_ERR_FLAG_ADDR 0x1FB535
...
#define NVT_ADDR_LOW_BIT_MASK 0x7F
#define NVT_GET_COLOR_ADDR_NB1 0x121035
#define NVT_GET_COLOR_RETRY 3
...
void touch_driver_sw_reset_idle(struct spi_device *sdev)
{
	...
	if ((!w_buf) || (!r_buf)) {
		thp_log_err("%s: w_buf  or r_buf is null\n", __func__);
		return;
	}
...
static void clear_crc_err_flag(struct spi_device *sdev, uint8_t *w_buf,
	uint8_t *r_buf, uint32_t crcerr_addr)
{
	int ret;
	int rw_len;
	int index = 0;
	...
	w_buf[index++] = spi_write_mask(0x7F);
	w_buf[index++] = nvt_hi_word_bit_mask(crcerr_addr);
	w_buf[index++] = nvt_low_word_bit_mask(crcerr_addr);
	rw_len = index; /* 3 is read or write data len */
	ret = touch_driver_spi_read_write(sdev,
		w_buf, r_buf, rw_len);
	if (ret)
		thp_log_err("%s, spi write failed\n", __func__);

	index = 0;
	w_buf[index++] = (uint8_t)(spi_write_mask((crcerr_addr & 0x0000007F)));
	w_buf[index++] = 0xA5; /* clear crc err flag cmd */
	rw_len = index; /* 2 is read or write data len */
	ret = touch_driver_spi_read_write(sdev,
		w_buf, r_buf, rw_len);
	if (ret)
		thp_log_err("%s, spi write failed\n", __func__);
...
static void check_crc_err_flag(struct spi_device *sdev, uint8_t *w_buf,
	uint8_t *r_buf, uint32_t crcerr_addr)
{
	int ret;
	int rw_len;
	...
	ret = memset_s(w_buf, NVT_CHIP_VER_TRIM_RW_LEN,
		0, NVT_CHIP_VER_TRIM_RW_LEN);
	if (ret)
		thp_log_err("%s, memset w_buf fail\n", __func__);
	w_buf[0] = (uint8_t)(spi_read_mask(crcerr_addr));
	rw_len = 3; /* 3 is read or write data len */
	ret = touch_driver_spi_read_write(sdev,
		w_buf, r_buf, rw_len);
	if (ret)
...
static void stop_crc_reboot(struct spi_device *sdev,
	uint8_t *w_buf, uint8_t *r_buf)
{
	uint32_t addr_crcerr = NVT_CRC_ERR_FLAG_ADDR;
	int32_t retry;
	...
	for (retry = 5; retry > 0; retry--) {
		...
		/* clear CRC_ERR_FLAG */
		clear_crc_err_flag(sdev, w_buf, r_buf, addr_crcerr);

		/* check CRC_ERR_FLAG */
		check_crc_err_flag(sdev, w_buf, r_buf, addr_crcerr);
		if (r_buf[2] == 0xA5)
			break;
	}
	if (retry == 0)
		thp_log_err("%s,stop CRC reboot fail. r_buf[2]=0x%02X\n",
			__func__, r_buf[2]);
}

static bool touch_driver_stop_crc_reboot(struct spi_device *sdev,
	uint8_t *w_buf, uint8_t *r_buf)
{
	bool need_stop_crc_reboot_flag = false;

	thp_log_info("read chip id = 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x.\n",
		r_buf[2], r_buf[3], r_buf[4], r_buf[5], r_buf[6], r_buf[7]);
	need_stop_crc_reboot_flag = ((r_buf[2] == 0xFC) ||
		((r_buf[2] == 0xFF) && (r_buf[3] == 0xFF) && (r_buf[4] == 0xFF)));
	if (need_stop_crc_reboot_flag)
		stop_crc_reboot(sdev, w_buf, r_buf);

	return need_stop_crc_reboot_flag;
}
...
static int8_t touch_driver_check_chip_ver_trim(struct spi_device *sdev)
{
	...
	for (retry = NVT_CHIP_VER_TRIM_RETRY_TIME; retry > 0; retry--) {
		...
		if (cd->have_internal_flash) {
			if (touch_driver_stop_crc_reboot(sdev, w_buf, r_buf))
				continue;
		}
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	rc = of_property_read_u32(nova_node, "have_internal_flash",
		&cd->have_internal_flash);
	if (rc)
		cd->have_internal_flash = 0;
	thp_log_info("%s: have_internal_flash %u\n", __func__,
		cd->have_internal_flash);

	return 0;
}

static int touch_driver_ic_series_config(struct thp_device *tdev, struct device_node *nova_node)
{
	bool flag = false;
	int rc = 0;
...
static int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	// ensure that the correct support_vendor_ic_type is used
	if (of_property_read_u32(nova_node, "support_vendor_ic_type",
		&tdev->thp_core->support_vendor_ic_type))
		thp_log_err("parse support_vendor_ic_type fail\n");
	...
	rc = touch_driver_ic_series_config(tdev, nova_node);
	if (rc)
		goto exit;
...
static void send_status_flag_stylus3(uint8_t *w_buf)
{
	/*
	...
	thp_log_info("cmd: %02x,%02x, %02x, %02x, %02x\n", w_buf[0], w_buf[1],
		w_buf[2], w_buf[3], w_buf[4]);
}

static void send_status_flag_stylus3_lite(uint8_t *w_buf)
{
	/*
	 * 3:need to send stylus status
	 * 4:stylus connect or disconnect
...
static int touch_driver_set_cmd_args(struct thp_device *tdev, uint8_t *w_buf)
{
	...
	if (current_stylus3_status)
		send_status_flag_stylus3_lite(w_buf);
	else
		send_status_flag_stylus3(w_buf);

	return 0;
}

static int touch_driver_send_cmd_check(struct thp_device *tdev, uint8_t cur_idx)
{
	int ret;
	uint8_t loop;
	uint8_t stamp_val;
	...
	thp_dmd_report(DSM_TPHOSTPROCESSING_DEV_GESTURE_EXP1,
		...
		stamp_val, cur_idx);
#endif
	thp_log_info("%s send cmd check failed!\n", __func__);
	return -EINVAL;
}
...
static int touch_driver_send_fw_cmd(struct thp_device *tdev, uint8_t cmd)
{
	...
	ret = touch_driver_send_cmd_check(tdev, w_buf[5]);
	if (!ret) {
		thp_log_info("%s cmd received!\n", __func__);
		return NO_ERR;
	}
	return -EINVAL;
}
...
static int touch_driver_addr_write_high_bit(struct thp_device *tdev, u32 addr, uint8_t *w_buf, uint8_t *r_buf)
{
	int ret;

	ret = memset_s(w_buf, NVT_SPI_RW_BUF_LEN, 0, NVT_SPI_RW_BUF_LEN);
	if (ret) {
		thp_log_err("%s: memset w_buf fail\n", __func__);
		return -EINVAL;
	}

	ret = memset_s(r_buf, NVT_SPI_RW_BUF_LEN, 0, NVT_SPI_RW_BUF_LEN);
	if (ret) {
		thp_log_err("%s: memset r_buf fail\n", __func__);
		return -EINVAL;
	}

	w_buf[0] = (uint8_t)spi_write_mask(NVT_ADDR_LOW_BIT_MASK);
	w_buf[1] = nvt_hi_word_bit_mask(addr);
	w_buf[2] = nvt_low_word_bit_mask(addr);
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, r_buf, SEND_CMD_LEN);
	if (ret) {
		thp_log_err("%s: spi write abnormal\n", __func__);
		return -EINVAL;
	}
...
static int touch_driver_addr_read(struct thp_device *tdev, u32 addr, uint8_t *w_buf, uint8_t *r_buf)
{
	int ret;
	...
	w_buf[0] = (uint8_t)(spi_read_mask(addr));
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, r_buf, 3);
	if (ret) {
		thp_log_err("%s: spi read abnormal\n", __func__);
		return -EINVAL;
	}
...
static int touch_driver_read_color_id(struct thp_device *tdev, u32 addr)
{
	int ret;
	uint8_t *r_buf = g_nvt_ts_spi_buf.r_buf;
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	...
	if (!r_buf || !w_buf) {
		thp_log_err("%s: r_buf or w_buf is null\n", __func__);
		return -EINVAL;
	}

	ret = touch_driver_addr_write_high_bit(tdev, addr, w_buf, r_buf);
	if (ret) {
		thp_log_err("%s: addr high bit write fail\n", __func__);
		return -EINVAL;
	}

	ret = touch_driver_addr_read(tdev, addr, w_buf, r_buf);
	if (ret) {
		thp_log_err("%s: addr read fail\n", __func__);
		return -EINVAL;
	}
	cypress_tp_color[0] = r_buf[2];
...
static int touch_driver_get_color_id(struct thp_device *tdev)
{
	...
	for (i = 0; i < NVT_GET_COLOR_RETRY; i++) {
		ret = touch_driver_read_color_id(tdev, NVT_GET_COLOR_ADDR_NB1);
		if (ret == 0)
			break;
		msleep(WAIT_FOR_SPI_BUS_READ_DELAY);
	}

	if (i == NVT_GET_COLOR_RETRY) {
		thp_log_err("%s: get tp color fail\n", __func__);
		return -EINVAL;
	}
	thp_log_info("%s: read tpcolor is 0x%x\n", __func__, cypress_tp_color[0]);
...
static int touch_driver_get_power_status(struct thp_device *tdev)
{
	...
	if (gesture_status != TS_GESTURE_MODE) {
		power_status = thp_get_status(power_status, cd->panel_id);
...
static int touch_driver_set_bt_status(struct thp_device *tdev,
	unsigned int stylus3_status, unsigned int stylus_adsroption_status)
{
	int i;
	int ret;
	struct thp_core_data *cd = tdev->thp_core;
	...
	if (stylus3_status) {
		if (cd->send_adsorption_status_to_fw &&
			stylus_adsroption_status)
			send_status_flag = STYLUS_CONNECT_NEAR;
		else
	...
	bool delay_enable)
{
	...
	unsigned int stylus_adsroption_status;
	unsigned int current_stylus3_work_mode;

	if (!cd->send_bt_status_to_fw)
		return 0;
	if ((touch_driver_get_power_status(tdev)) < 0)
		return 0;
	if (delay_enable)
	...
	current_stylus3_work_mode =
		atomic_read(&tdev->thp_core->last_stylus3_work_mode);
	current_stylus3_status = (stylus3_status & STYLUS3_CONNECT) &&
		!current_stylus3_work_mode;
	return touch_driver_set_bt_status(tdev, current_stylus3_status,
		stylus_adsroption_status);
}
...
static int __init touch_driver_module_init(void)
{
	...
	rc = thp_register_dev(dev, MAIN_TOUCH_PANEL);
	if (rc) {
