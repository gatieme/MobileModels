...
static int g_set_ap_cmd_retry = 0;
	...
	unsigned int *gesture_wakeup_value)
{
	...
	for (i = 0; i < tdev->thp_core->gesture_retry_times; i++) {
		...
		if (retval == TOUCH_REPORT_CONFIG_SIZE)
			break;
		thp_log_info(tdev->thp_core, "%s: spi not work normal, ret %d retry\n",
			__func__, retval);
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(thp_node, "support_ignore_first_irq_after_reset",
		&value);
	if (!rc) {
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	rc = thp_parse_timing_config(tdev->thp_core, syna_node, &tdev->timing_config);
	if (rc)
...
#endif
	ret = thp_power_supply_ctrl(tdev->thp_core, THP_VCC, THP_POWER_ON, 1); /* delay 1ms */
	if (ret)
...
static int touch_driver_power_off_for_3909(struct thp_device *tdev)
{
	...
	if (cd->support_deepsleep_mode) {
		thp_log_info(tdev->thp_core, "%s: no power_off,  And in deepsleep mode~\n", __func__);
...
static int touch_driver_power_off(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(tdev->thp_core, THP_VCC, THP_POWER_OFF, VCC_DELAY);
	if (ret)
		thp_log_err(tdev->thp_core, "%s:power ctrl vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(tdev->thp_core, THP_IOVDD, THP_POWER_OFF, IOVDD_DELAY);
	if (ret)
		thp_log_err(tdev->thp_core, "%s:power ctrl fail\n", __func__);
	gpio_set_value(tdev->gpios->cs_gpio, GPIO_LOW);
...
static int touch_driver_get_flash_cmd(struct thp_device *tdev, struct syna_tcm_boot_info *boot_info,
	...
	if (id_info->mode == MODE_APPLICATION) {
		...
		if (temp_buf[1] == REPORT_IDENTIFY)
			memcpy(id_info, &temp_buf[MESSAGE_HEADER_SIZE],
				sizeof(*id_info));
		thp_log_info(tdev->thp_core, "%s: value = 0x%x,expect value = 0x10\n",
			__func__, temp_buf[1]);
	}
}
	...
	struct syna_tcm_identification *id_info)
{
	...
	retval = touch_driver_get_flash_cmd(tdev, &boot_info,
	...
	memset(tdev->thp_core->project_id, 0, sizeof(tdev->thp_core->project_id));
	memcpy(tdev->thp_core->project_id,
		&temp_buf[IC_PROJECT_ID_START + BOOT_CONFIG_SLOTS],
		THP_PROJECT_ID_LEN);
...
static int touch_driver_chip_detect_3909(struct thp_device *tdev)
{
...
#ifdef CONFIG_ARCH_QTI_VM
	struct syna_tcm_identification *private_data = (struct syna_tcm_identification *)tdev->private_data;
	...
	get_project_id_flag = 1;
	memcpy(tdev->thp_core->project_id, "0123456789", THP_PROJECT_ID_LEN);
	thp_log_info(tdev->thp_core, "%s: called in QTVM, set project id directly\n", __func__);
	return 0;
#else
	thp_log_info(tdev->thp_core, "%s: called\n", __func__);
#endif
	...
	char *frame_buf, unsigned int len)
{
	...
	if (tdev->thp_core->support_ignore_first_irq_after_reset &&
		...
		thp_log_err(tdev->thp_core, "%s:ignore_irq_after_reset count is %u\n",
			__func__, ignore_irq_after_reset);
		return -EINVAL;
	}
...
static int pt_mode_set(const struct thp_device *tdev)
{
	...
	ret = touch_driver_spi_write(tdev->thp_core->sdev,
		spi_write_buf, SYNA_COMMAMD_LEN);
	if (ret < 0)
		thp_log_err(tdev->thp_core, "Failed to send syna active command\n");
	...
	unsigned char *report_to_ap_cmd, int len)
{
	...
	int i;
	unsigned char *data = spi_read_buf;
	unsigned char flag = SYNA_NO_RETRY;

	if (!report_to_ap_cmd || (len <= 0)) {
		thp_log_err(tdev->thp_core, "%s: have null pointer\n", __func__);
		return;
	}
	ret = thp_bus_lock(tdev->thp_core);
	...
	for (i = 0; i < len; i++)
		thp_log_debug(tdev->thp_core, "event %d 0x%02x\n", i, report_to_ap_cmd[i]);
	memset(spi_write_buf, 0, SPI_READ_WRITE_SIZE);
	...
	for (i = 0; i < SYNA_READ_PT_MODE_FLAG_RETRY_TIMES; i++) {
		...
		if ((data[0] == SYNA_PT_PKG_HEAD) &&
			...
			thp_log_info(tdev->thp_core, "%s: get flag success\n", __func__);
			break;
		} else if ((data[0] == SYNA_PT_PKG_HEAD) &&
			(data[1] == SYNA_TP_BUSY) &&
			(data[2] == SYNA_PT_PKG_LEN) &&
			(data[3] == SYNA_PT_PKG_LEN)) {
			flag = SYNA_RETRY;
			mdelay(5);
			thp_log_info(tdev->thp_core, "%s TP IC BUSY\n", __func__);
			continue;
		} else if ((data[0] == SYNA_PT_PKG_HEAD) &&
			(data[1] == SYNA_PT_PKG_HEAD_OFFSET) &&
			(data[2] == SYNA_PT_PKG_LEN) &&
			...
			if (ret < 0)
				thp_log_err(tdev->thp_core, "Failed to send active command\n");
			thp_log_info(tdev->thp_core, "%s second write ret = %d\n", __func__, ret);
			continue;
		} else if ((data[0] == SYNA_PT_PKG_HEAD) &&
			(data[1] == SYNA_PT_PKG_HEAD_OFFSET) &&
			(data[2] == SYNA_DEBUG_INFO_LEN_L) &&
			(data[3] == SYNA_DEBUG_INFO_LEN_H)) {
			/*
				print debug info, 20 bytes per line
				responce to get_debug_info()
			*/
			for (i = 0; i < SYNA_DEBUG_INFO_LEN / 20; i++)
			...
			thp_log_info(tdev->thp_core, "data[0] is invalid, reset\n");
			break;
		} else if ((data[0] == SYNA_PT_PKG_HEAD) &&
				(data[1] == REPORT_IDENTIFY)) {
				/* read a5 10 means fw reboot, need set cmd again */
				flag = SYNA_NO_RETRY;
				mdelay(5);
				ret = touch_driver_spi_write(tdev->thp_core->sdev, spi_write_buf, len);
				if (ret < 0)
		...
		thp_log_info(tdev->thp_core, "%s get flag retry 0x%02x 0x%02x 0x%02x 0x%02x\n",
...
{
	unsigned char enable_tui_first_cmd[SYNA_TUI_CMD_LEN] = { 0x06, 0x01, 0x00, 0xc0 };
	unsigned char enable_tui_second_cmd[SYNA_TUI_CMD_LEN] = { 0xc7, 0x01, 0x00, 0x14 };

	if (switch_value) {
		touch_driver_set_cmd(tdev, enable_tui_first_cmd, sizeof(enable_tui_first_cmd));
		/* delay 5ms for avoiding ic not entering tui mode */
		mdelay(5);
		touch_driver_set_cmd(tdev, enable_tui_second_cmd, sizeof(enable_tui_second_cmd));
		thp_log_info(tdev->thp_core, "enable tui mode\n");
	} else {
		thp_log_info(tdev->thp_core, "daemon notyfiy ic already\n");
	}
...
{
	unsigned char report_to_ap_cmd[SYNA_TUI_CMD_LEN] = { 0xC7, 0x01, 0x00, 0x14 };

	thp_log_info(tdev->thp_core, "%s enter tui again\n", __func__);
	touch_driver_set_cmd(tdev, report_to_ap_cmd, sizeof(report_to_ap_cmd));
	return 0;
}
#else
static int touch_driver_set_ap_one_cmd(struct thp_device *tdev)
...
static void get_debug_info(struct thp_device *tdev)
{
	unsigned char get_debug_cmd[SYNA_CMD_GET_DEBUG_LEN] = { 0xC7, 0x01, 0x00, 0x06 };
	touch_driver_set_cmd(tdev, get_debug_cmd, sizeof(get_debug_cmd));
}

static int touch_driver_set_ap_state(struct thp_device *tdev)
{
	unsigned char report_to_ap_cmd[SYNA_CMD_LEN] = { 0xC7, 0x02, 0x00,
		0x4C, 0x01 };
	unsigned char report_to_ap_cmd_gesture[SYNA_CMD_GESTURE_LEN] = { 0xC7,
		0x05, 0x00, 0x2A, 0x00, 0x00, 0x00, 0x00 };
	...
	if (private_data->stylus_gesture_status_only &&
		...
		touch_driver_set_cmd(tdev, syna_sleep_cmd,
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	private_data->stylus_gesture_status_only = (cd->stylus_gesture_status &&
		!cd->aod_touch_status && !gesture_status && !finger_status);
	thp_log_info(tdev->thp_core, "%s:gesture_status:%u,finger_status:%u,stylus_status=%u\n",
	...
	if ((stylus_status > 0) && (cd->stylus_gesture_status == 0))
		touch_driver_set_cmd(tdev, pen_disable_cmd, sizeof(pen_disable_cmd));
	...
	if ((gesture_status == TS_GESTURE_MODE) || finger_status ||
		...
		if (tdev->thp_core->tsa_event_to_udfp && finger_status) {
			...
			thp_log_info(tdev->thp_core, "%s:tsa_event_to_udfp, ret = %d\n",
				__func__, ret);
		}
#endif
	...
	if (tdev->thp_core->self_control_power) {
		if (is_pt_test_mode(tdev)) {
			...
			if (ret < 0)
				thp_log_err(tdev->thp_core, "%s: failed to set pt mode\n",
					__func__);
		} else if (need_power_off == NEED_WORK_IN_SUSPEND) {
			if (cd->support_shb_thp)
	...
	u8 state)
{
	...
	delta_time = ((lowpower_receive.tv_sec - lowpower_switch_time.tv_sec) *
			1000000) + lowpower_receive.tv_usec -
			lowpower_switch_time.tv_usec;
	delta_time /= 1000;
	if (delta_time < 10) {
		thp_log_info(tdev->thp_core, "%s: lowpower switch too fast [%ld ms]\n",
			__func__, delta_time);
	...
	const char *read_buff, struct thp_udfp_data *udfp_data, int len)
{
	...
	if (tmp_event == TUI_FINGER_DOWN && private_data->tui_finger_last_status == TUI_FINGER_UP)
		udfp_data->tpud_data.udfp_event = TP_EVENT_FINGER_DOWN;
	else if (tmp_event == TUI_FINGER_UP && private_data->tui_finger_last_status == TUI_FINGER_DOWN)
		udfp_data->tpud_data.udfp_event = TP_EVENT_FINGER_UP;
	private_data->tui_finger_last_status = tmp_event;

	thp_log_info(tdev->thp_core, "%s: x %d, y %d, touch event type %d, converted type %d\n", __func__,
		udfp_data->tpud_data.tp_x, udfp_data->tpud_data.tp_y, tmp_event, udfp_data->tpud_data.udfp_event);
}
	...
	const char *read_buff, struct thp_udfp_data *udfp_data, int len)
{
	...
	int retval;
	unsigned char report_to_ap_cmd[SYNA_CMD_GESTURE_LEN] = { 0xC7, 0x05,
	...
	struct thp_udfp_data *udfp_data)
{
	...
	thp_log_info(tdev->thp_core, "data[0]=0x%02x data[1]=0x%02x data[2]=0x%02x data[3]=0x%02x\n",
		data[0], data[1], data[2], data[3]);

	if (cd->support_esd_event == 1 && data[0] == 0xa5 && data[1] == 0x10) {
	...
	if (data[2] == DEBUG_OPEN_VALUE) {
		 ...
		 */
		for (i = 0; i < DEBUG_LOG_LENGTH / 20; i++)
			thp_log_info(tdev->thp_core, "tp_debug_info[%d~%d] %*ph\n",
				20 * i, 20 * (i + 1) - 1,
				20, debug_data + 20 * i + 2);
	}
	if (length > (THP_MAX_FRAME_SIZE - FIRST_FRAME_USEFUL_LEN)) {
		thp_log_info(tdev->thp_core, "%s: out of length\n", __func__);
		length = THP_MAX_FRAME_SIZE - FIRST_FRAME_USEFUL_LEN;
	}
	thp_log_info(tdev->thp_core, "%s: length %d\n", __func__, length);
	if (data[1] == 0xFF) {
		thp_log_err(tdev->thp_core, "%s: should ignore this irq\n", __func__);
		retval = -ENODATA;
		goto error;
	...
	if ((data[0] != MESSAGE_MARKER) || (length > SYNA_CMD_GESTURE_MAX)) {
		...
		if (length > READ_RETRY_LENGTH_MAX) {
			read_data_retry++;
			if (read_data_retry >= READ_RETRY_TIMES) {
				read_data_retry = 0;
				g_set_ap_cmd_retry++;
				if (g_set_ap_cmd_retry >= SET_AP_CMD_RETRY_TIMES) {
					g_set_ap_cmd_retry = 0;
					thp_log_err(tdev->thp_core,
						"%s: set ap cmd too many times, reset\n", __func__);
					thp_bus_unlock(tdev->thp_core);
					get_debug_info(tdev);
					gpio_set_value(tdev->thp_core->gpios.rst_gpio, GPIO_LOW);
					mdelay(2);
					gpio_set_value(tdev->thp_core->gpios.rst_gpio, GPIO_HIGH);
					return -ENODATA;
				} else {
					thp_bus_unlock(tdev->thp_core);
					retval = touch_driver_set_ap_one_cmd(tdev);
	...
	if (length) {
		...
		if (retval) {
			thp_log_err(tdev->thp_core, "parse_event_info fail %d\n", retval);
			goto error;
		}
	} else {
		thp_log_debug(tdev->thp_core, "%s Invalid event\n", __func__);
		retval = -ENODATA;
	}
error:
	thp_bus_unlock(tdev->thp_core);
...
static int touch_driver_suspend_connect_pen(struct thp_device *tdev)
{
	...
	mdelay(2);
	touch_driver_set_cmd(tdev, cmd_enable, sizeof(cmd_enable));
	mdelay(2);
	touch_driver_set_cmd(tdev, pen_enable_cmd, sizeof(pen_enable_cmd));
	return 0;
}
...
int syna_driver_module_init(struct thp_core_data *cd)
{
	...
	if (device_info == NULL) {
		kfree(dev);
		dev = NULL;
		thp_log_err(cd, "%s: syna device info malloc fail\n", __func__);
		return -ENOMEM;
	}
