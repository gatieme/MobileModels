...
#ifdef CONFIG_HONOR_OPT_MEM_POOL
...
static int dmabuf_page_pool_shrink(gfp_t gfp_mask, int nr_to_scan)
{
	...
	mutex_lock(&pool_list_lock);
#ifdef CONFIG_HONOR_OPT_MEM_POOL
	list_for_each_entry_reverse(pool, &pool_list, list) {
#else
...
{
#ifdef CONFIG_HONOR_OPT_MEM_POOL
	int nr_reserved = CONFIG_HONOR_DMABUF_RESERVED * SZ_1M / PAGE_SIZE;
	int nr_shrink = dmabuf_page_pool_shrink(sc->gfp_mask, 0);
	return nr_shrink > nr_reserved ? nr_shrink - nr_reserved : 0;
#else
