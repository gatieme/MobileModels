...
enum {
	...
	DUBAI_BATTERY_PROP_ENERGY_FULL = 10,
	DUBAI_BATTERY_PROP_ENERGY_FULL_DESIGN = 11,
	DUBAI_BATTERY_PROP_ENERGY_NOW = 12,
};
...
static int dubai_get_charge_full(void)
{
	...
	return dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_CHARGE_FULL, -1) / amp;
}
...
static int dubai_get_psy_value(int type, struct power_supply *psy)
{
	int ret = 0;
	switch (type) {
	case DUBAI_BATTERY_PROP_VOLTAGE_NOW:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, -1) / UV_PER_MV;
		break;
	case DUBAI_BATTERY_PROP_CURRENT_NOW:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW, -1);
		break;
	case DUBAI_BATTERY_PROP_CYCLE_COUNT:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, -1);
		break;
	case DUBAI_BATTERY_PROP_HEALTH:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_HEALTH, -1);
		break;
	case DUBAI_BATTERY_PROP_TEMP:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_TEMP, -1);
		break;
	case DUBAI_BATTERY_PROP_ENERGY_FULL:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_ENERGY_FULL, -1);
		break;
	case DUBAI_BATTERY_PROP_ENERGY_FULL_DESIGN:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN, -1);
		break;
	case DUBAI_BATTERY_PROP_ENERGY_NOW:
		ret = dubai_get_psy_intprop(g_batt_psy, POWER_SUPPLY_PROP_ENERGY_NOW, -1);
		break;
	default:
		dubai_err("Invalid type %d", type);
		ret = -1;
		break;
	}
	return ret;
...
static int dubai_get_battery_prop(void __user *argp) {
	...
	case DUBAI_BATTERY_PROP_ENERGY_FULL_DESIGN:
	case DUBAI_BATTERY_PROP_ENERGY_NOW:
		propval.value = dubai_get_psy_value(propval.prop, g_batt_psy);
		break;
	...
	case DUBAI_BATTERY_PROP_CAPACITY:
		...
		dubai_info("Get battery capacity: %d", propval.value);
		break;
