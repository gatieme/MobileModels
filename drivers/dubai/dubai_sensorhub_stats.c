...
#define IOC_CPLD_GET DUBAI_SENSORHUB_DIR_IOC(R, 8, struct cpld_data)
#define IOC_AOD_GET_INFO DUBAI_SENSORHUB_DIR_IOC(WR, 9, struct aod_working_state_monitor)
...
static int dubai_get_cpld_data(void __user *argp);
static int dubai_get_aod_info(void __user *argp);
...
long dubai_ioctl_sensorhub(unsigned int cmd, void __user *argp)
{
	...
	case IOC_LCD_STATE_DURATION_GET:
		...
		break;
	case IOC_CPLD_GET:
		rc = dubai_get_cpld_data(argp);
		break;
	case IOC_AOD_GET_INFO:
		rc = dubai_get_aod_info(argp);
		break;
	default:
		rc = -EINVAL;
		break;
...
{
	int ret;
	struct aod_info_two info_aod;
	struct aod_light_two light_info;
	struct aod_working_state_monitor stat;

	ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_AOD_INFO, &info_aod, sizeof(struct aod_info_two));
	if (ret < 0)
		return ret;

	ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_AOD_BL_INFO, &light_info, sizeof(struct aod_light_two));
	if (ret < 0)
		return ret;

	stat.info_data = info_aod;
	stat.light_data = light_info;
	if (copy_to_user(argp, &stat, sizeof(struct aod_working_state_monitor)))
		return -EFAULT;
...
static int dubai_get_cpld_data(void __user *argp)
{
	...
	struct cpld_data *temp_data;

	temp_data = send_sts_data_to_dubai();
	if (temp_data == NULL) {
		return -EFAULT;
	}
	data.audio_time = temp_data->audio_time;
	data.tp_time = temp_data->tp_time;
	data.camera_time = temp_data->camera_time;
	data.others_time = temp_data->others_time;
	data.ap_time = temp_data->ap_time;
	data.modem_time = temp_data->modem_time;
	data.m7_time = temp_data->m7_time;
	data.coreothers_time = temp_data->coreothers_time;
	data.coretotal = temp_data->coretotal;

	sts_data_reset();
	if (copy_to_user(argp, &data, sizeof(struct cpld_data))) {
		return -EFAULT;
	}
	return 0;
#else
