...
struct aod_info_two {
	struct aod_info data[AOD_MAX_DISPLAY];
} __packed;
...
struct aod_light_two {
	struct aod_light data[AOD_MAX_DISPLAY];
} __packed;

struct aod_working_state_monitor {
	struct aod_info_two info_data;
	struct aod_light_two light_data;
} __packed;
