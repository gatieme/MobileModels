...
static int fi_flow_ctx_init(struct fi_flow_node *newnode, fi_flow_cb *flow_cb)
{
	...
	newnode->flow_ctx.flow_stat.down_seq_min = FI_INVALID_SEQ_NUM;
	newnode->flow_ctx.flow_stat.down_seq_max = FI_INVALID_SEQ_NUM;
	newnode->flow_ctx.down_seq_expect = FI_INVALID_SEQ_NUM;
	if (flow_cb != NULL && flow_cb->flow_identify_cfg != NULL) {
...
bool fi_payload_str_filter_check(struct fi_pkt_parse *pktinfo, fi_rpt_cfg *rpt_cfg)
{
	...
	for (i = 0; i < FI_FLOW_FILTER_STR_MAX; i++) {
		if (!rpt_cfg->filter_str_len[i])
			break;
		if (i < rpt_cfg->filter_str_offset_len)
			filter_str_offset = rpt_cfg->filter_str_offset[i];
		else
			filter_str_offset = rpt_cfg->filter_str_offset[0];
		if (filter_str_offset + rpt_cfg->filter_str_len[i] > payloadlen)
			continue;
		if (memcmp(rpt_cfg->filter_str[i],
				payload + filter_str_offset,
...
static void fi_flow_get_sk_info(struct fi_flow_node *flow)
{
	...
	if (flow->sk != NULL && flow->sk->sk_protocol == IPPROTO_TCP) {
		...
		if (tp->bytes_received < flow->flow_ctx.last_tcp_down_pkt_size) {
			tcp_down_pkt_byte = 0;
		} else {
			tcp_down_pkt_byte = tp->bytes_received - flow->flow_ctx.last_tcp_down_pkt_size;
		}
		if (tcp_down_pkt_byte > 0xffffffff) {
			tcp_down_pkt_byte = 0xffffffff;
		}
		flow->flow_ctx.flow_stat.tcp_down_pkt_byte = (u32)(tcp_down_pkt_byte);
		flow->flow_ctx.last_tcp_down_pkt_size = tp->bytes_received;
	}
}

static void fi_flow_raw_stat_update(struct fi_flow_node *flow, const struct fi_pkt_parse *pktinfo,
	enum fi_dir dir)
{
	...
	if (!pktinfo->flow_cb.flow_identify_cfg->basic_rpt_en)
		return;
	if (dir == FI_DIR_UP) {
		flow_stat->raw_up_pkt_num += pktinfo->is_gso ? pktinfo->raw_frag_num : 1;
		flow_stat->raw_up_pkt_byte += pktinfo->msg.payloadlen;
	} else {
		flow_stat->raw_down_pkt_num += pktinfo->is_gso ? pktinfo->raw_frag_num : 1;
	...
	enum fi_dir dir)
{
	...
	if (dir == FI_DIR_UP) {
		flow_stat->up_pkt_num += pktinfo->is_gso ? pktinfo->filter_frag_num : 1;
		flow_stat->up_pkt_byte += pktinfo->msg.payloadlen;
	} else {
		flow_stat->down_pkt_num += pktinfo->is_gso ? pktinfo->filter_frag_num : 1;
...
bool fi_pkt_report_is_enable(struct fi_flow_node *flow, struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
	...
	pkt_cfg = &pktinfo->flow_cb.flow_identify_cfg->pkt[FI_DIR_DOWN];
	if(dir == FI_DIR_UP && pkt_cfg->report_type == FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET && pktinfo->payloadlen > 0) {
		if (pktinfo->msg.tcp_seq == flow->flow_ctx.last_uplink_seq) {
			return enable;
		}
		flow->flow_ctx.last_uplink_seq = pktinfo->msg.tcp_seq;
		flow->flow_ctx.pkt_ctrl[FI_DIR_DOWN].pkt_num = 0;
		flow->flow_ctx.stat_stop &= ~FI_FLOW_PKT_REPORT_STOP_DOWN;
		flow->flow_ctx.expect_downlink_ack = pktinfo->msg.tcp_seq + pktinfo->msg.payloadlen;
		flow->flow_ctx.expect_downlink_seq = pktinfo->msg.tcp_ack;
	}
	...
	if (dir == FI_DIR_DOWN && pkt_cfg->report_type == FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET) {
		if (pktinfo->msg.tcp_ack < flow->flow_ctx.expect_downlink_ack) {
			return enable;
		}
		if (pktinfo->msg.tcp_seq > flow->flow_ctx.expect_downlink_seq) {
		    //HTTP1.X  applicable, HTTP2.0 inapplicable
			return enable;
		}
	}
	...
	case FI_RPT_TIME_CTRL:
		...
		break;
	case FI_RPT_NUM_CTRL:
	case FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET:
		if (filter_pass) {
	...
	void (*flow_unlock)(uint32_t))
{
	...
	for (i = 0; i < FI_FLOW_TABLE_SIZE; i++) {
		...
		list_for_each_entry_safe(node, tmp, &(flow->flow_table[i].list), list) {
			if (node->flow_cb.flow_identify_cfg != NULL
				&& ((node->flow_ctx.stat_stop & FI_FLOW_PERIODIC_REPORT_NEED) ||
				...
				node->flow_ctx.flow_stat.down_seq_max = FI_INVALID_SEQ_NUM;
				node->flow_ctx.flow_stat.down_seq_min = FI_INVALID_SEQ_NUM;
				node->flow_ctx.flow_tcp_period_init = false;
	...
	void (*flow_unlock)(uint32_t))
{
	...
	for (i = 0; i < FI_FLOW_TABLE_SIZE; i++) {
		const struct fi_flow_head *head = flow->flow_table + i;
		if (list_empty(&(head->list))) {
			continue;
		}
		(*flow_lock)(i);
