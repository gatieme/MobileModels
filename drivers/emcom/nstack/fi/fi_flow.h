...
#define FI_CMP_RANGE		2
#define FI_DPORT_LOW		0
#define FI_DPORT_HIGH		1
...
struct fi_flow_stat {
	...
	u32	rtt;
	u32	total_retrans;
	int32_t down_seq_min;
	int32_t down_seq_max;
	u32 down_loss_cnt;
	u32 total_tcp_hole;
	u32 max_tcp_seq;
	u32 min_tcp_seq;
	u32 tcp_down_pkt_byte;
...
struct fi_flow_ctx {
	...
	uint32_t expect_downlink_ack;
	uint32_t expect_downlink_seq;
	uint32_t last_uplink_seq;
	int32_t down_seq_expect;
	u32 expect_tcp_seq;
	u64 last_tcp_down_pkt_size;
	bool flow_tcp_init;
	bool flow_tcp_period_init;
};
...
typedef enum {
	...
	FI_RPT_NUM_CTRL,
	FI_RPT_TIME_CTRL,
	FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET
} fi_rpt_type;
...
typedef struct {
	...
	uint32_t filter_str_offset[FI_FLOW_FILTER_STR_MAX];
	uint8_t filter_str_offset_len;

	/* feature policy */
	bool				opt_enable[FI_END_PARSE];
	uint32_t			seg_begin;
	uint32_t			seg_end;
	uint32_t			spec_seq_high;
	uint32_t			spec_seq_low;
}fi_rpt_cfg;

typedef struct tag_fi_flow_identify_cfg {
	int			proto;
	int			dport[FI_DPORT_MAX];
	uint32_t	dport_range[FI_CMP_RANGE];
	bool basic_rpt_en;
...
struct fi_pkt_parse {
	...
	__be16 payloadlen;
	char *filter_payload;
	__be16 filter_payloadlen;
};

struct fi_pkt_parse_f {
	enum fi_parse	opt_num;
	bool			(*opt_val_parse)(struct sk_buff *, struct fi_pkt_parse *, struct fi_flow_node *);
};
