...
int8_t cal_rtt_stability_index(int8_t network_type, int32_t avg, int32_t sqrdev)
{
	...
	if ((avg <= STABILITY_RTT_SEGMENT_ENDS[0])
		|| (avg > STABILITY_RTT_SEGMENT_ENDS[4])) {
							...
							const int32_t* segment_ends, int8_t endpoints)
{
	...
	for (index1 = 0; index1 < length; index1++) {
		for (index2 = 0; index2 < segments; index2++) {
			...
			if (array[index1] > segment_ends[index2]
				&& array[index1] <= segment_ends[index2+1]) {
	...
	if (POINT_DIMENSIONS == segments) {
		...
		EMCOM_LOGE(" : unknown error in get_probability_space_point, \
probably incorrect point dimension given");
	}
...
void cal_statistics(const int32_t* array, const int8_t length, int32_t* avg, int32_t* sqrdev)
{
	...
	for (index = 0; index < length; index++) {
		if ((array[index] <= SIGNAL_ABNORMAL_THRESHOLD)
			...
			EMCOM_LOGD(" : illegal data (index %d) %d in average value calculation",
							index, array[index]);
			actual_length--;
			continue;
		}
