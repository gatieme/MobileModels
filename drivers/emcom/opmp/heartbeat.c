...
* Description: opmp heartbeat function
* Author: jiangwenhao jiangwenhao1.
* Create: 2019-01-04
*/
...
#include "securec.h"
#include "honor_platform/emcom/emcom_netlink.h"
...
#define LOGI(msg, ...) do \
		hilog_info("[%s:%d %s]: "msg"\n", kbasename(__FILE__), \
			   __LINE__, __func__, ## __VA_ARGS__); \
	while (0)

#define LOGD(msg, ...) do \
		hilog_debug("[%s:%d %s]: "msg"\n", kbasename(__FILE__), \
			    __LINE__, __func__, ## __VA_ARGS__); \
	while (0)
...
static bool check_tag(const char *buf, cosnt char *tag)
{
