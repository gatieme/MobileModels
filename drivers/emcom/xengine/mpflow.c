...
#include "honor_platform/net/bastet/bastet_utils.h"
#endif
#include "securec.h"
...
static void emcom_xengine_mpflow_timer(struct timer_list *arg)
#else
	...
	const struct emcom_xengine_mpflow_ptn *sptn, uint32_t *offset)
{
	...
	if ((buf == NULL) || (sptn->skip == NULL) || (sptn->shift == NULL)) {
		return false;
	}
...
void emcom_xengine_mpflow_bind2device(struct sock *sk, struct sockaddr *uaddr)
{
	...
	if (emcom_mpdns_is_allowed(uid, NULL)) {
		...
		int cur_net = emcom_xengine_get_net_type(sk->sk_mark & 0xFFFF);
...
void emcom_mpflow_check_mpdns_addr(struct sock *sk, int reason, int result)
{
	...
	init_net_type = emcom_xengine_get_net_type(sk->sk_mark & 0xFFFF);
