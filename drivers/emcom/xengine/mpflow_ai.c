	...
	if (net_dev) {
		...
		if (flags & IFF_RUNNING) {
			...
			if (inet->inet_saddr)
				inet->inet_saddr = 0;
#if IS_ENABLED(CONFIG_IPV6)
			if (inet->pinet6 && !ipv6_addr_any(&inet->pinet6->saddr))
				inet->pinet6->saddr = in6addr_any;
#endif
		}
		dev_put(net_dev);
	}
...
static bool emcom_xengine_mpflow_ai_rehash_sk(struct sock *sk)
{
	if (sk->sk_protocol == IPPROTO_UDP &&
		...
		if (sk->sk_prot && sk->sk_prot->rehash) {
			...
			if (inet_sk(sk)->pinet6 && !ipv6_addr_any(&inet_sk(sk)->pinet6->saddr)) {
				EMCOM_LOGD("[MPFlow_KERNEL] inet6 saddr: "IPV6_FMT, IPV6_INFO(inet_sk(sk)->pinet6->saddr));
				inet_sk(sk)->pinet6->saddr = in6addr_any;
			}
...
static uint8_t emcom_xengine_transfer_ifname(const char* ifname)
{
	if (strncmp(ifname, EMCOM_WLAN0_IFNAME, strlen(EMCOM_WLAN0_IFNAME)) == 0 ||
		strncmp(ifname, EMCOM_DEFAULT_WLAN0_IFNAME, strlen(EMCOM_DEFAULT_WLAN0_IFNAME)) == 0) {
	...
	if (strncmp(ifname, EMCOM_WLAN1_IFNAME, strlen(EMCOM_WLAN1_IFNAME)) == 0 ||
		strncmp(ifname, EMCOM_DEFAULT_WLAN1_IFNAME, strlen(EMCOM_DEFAULT_WLAN1_IFNAME)) == 0) {
	...
	if (ifname != NULL) {
		EMCOM_LOGE("ifname is wrong! ifname:%s", ifname);
...
static void emcom_xengine_net_type_transfer_ifname(int net_type, char* ifname, int len)
{
	...
	case EMCOM_XENGINE_NET_WLAN0:
		ret = strncpy_s(ifname, len, EMCOM_WLAN0_IFNAME, IFNAMSIZ);
		break;
	case EMCOM_XENGINE_NET_CELL0:
		ret = strncpy_s(ifname, len, EMCOM_CELL0_IFNAME, IFNAMSIZ);
		break;
	case EMCOM_XENGINE_NET_WLAN1:
		ret = strncpy_s(ifname, len, EMCOM_WLAN1_IFNAME, IFNAMSIZ);
		break;
	case EMCOM_XENGINE_NET_CELL1:
		ret = strncpy_s(ifname, len, EMCOM_CELL1_IFNAME, IFNAMSIZ);
		break;
	default:
			...
			if (sk->sk_state == TCP_ESTABLISHED || sk->sk_state == TCP_SYN_SENT) {
				...
				if (netdev_get_name(sock_net(sk), ifname, sk->sk_bound_dev_if) !=0) {
					if (sk->sk_bound_dev_if == EMCOM_MPFLOW_BIND_NONE) {
						int net_type = emcom_xengine_get_net_type(sk->sk_mark & 0xFFFF);
				...
				if (bindmode == EMCOM_MPFLOW_BIND_NONE && ((sk->sk_bound_dev_if == EMCOM_MPFLOW_BIND_NONE) ||
					bind_type == emcom_xengine_get_net_type(0) + EMCOM_MPFLOW_BIND_WIFI0 - EMCOM_XENGINE_NET_WLAN0)) {
...
static void emcom_xengine_mpflow_ai_udp_bind_clear(struct sock *sk)
{
	struct inet_sock *inet = NULL;
	EMCOM_LOGI("clear sk %pK sport is %u, state[%u]", sk, sk->sk_num, sk->sk_state);
	local_bh_disable();
	bh_lock_sock(sk);
	sk->is_mp_flow_bind = 0;
	sk->sk_bound_dev_if = EMCOM_MPFLOW_BIND_NONE;
	sk_dst_reset(sk);
	inet = inet_sk(sk);
	if (inet->inet_saddr)
		inet->inet_saddr = 0;
	...
	for (; bucket <= udp_table.mask; bucket++) {
		...
		sk_for_each(sk, &hslot->head) {
			if (sk->sk_state != TCP_SYN_RECV && sk->sk_state != TCP_TIME_WAIT &&
				...
				if (bindmode == EMCOM_MPFLOW_BIND_NONE && ((sk->sk_bound_dev_if == EMCOM_MPFLOW_BIND_NONE) ||
					bind_type == emcom_xengine_get_net_type(0) + EMCOM_MPFLOW_BIND_WIFI0 - EMCOM_XENGINE_NET_WLAN0)) {
					if (sk->sk_protocol == IPPROTO_UDP  && sk->is_mp_flow_bind)
						emcom_xengine_mpflow_ai_udp_bind_clear(sk);
					EMCOM_LOGI("ignore flow already in defult dev, port: %u, state: %u devif: %d bind_type: %u",
						sk->sk_num, sk->sk_state, sk->sk_bound_dev_if, bind_type);
					continue;
				}
...
{
	cur_burst_ratio[0] = burst_info->burst_ratio[WLAN0_RATIO];
	cur_burst_ratio[1] = burst_info->burst_ratio[CELL0_RATIO];
	cur_burst_ratio[2] = burst_info->burst_ratio[WLAN1_RATIO];
	cur_burst_ratio[3] = burst_info->burst_ratio[CELL1_RATIO];
	...
	struct inet_sock *inet, uid_t uid, uint16_t dport, struct sockaddr *uaddr)
{
	...
	if (bind_device == EMCOM_MPFLOW_BIND_WIFI0) {
		...
		err = memcpy_s(ifname, sizeof(char) * IFNAMSIZ, EMCOM_CELL1_IFNAME, (strlen(EMCOM_CELL1_IFNAME) + 1));
...
void emcom_xengine_mpflow_ai_bind2device(struct sock *sk, struct sockaddr *uaddr)
{
	...
	if (sk->sk_protocol == IPPROTO_TCP && (sk->sk_bound_dev_if || sk->is_mp_flow_bind))
		return;
	else if (sk->sk_protocol == IPPROTO_UDP  && sk->is_mp_flow_bind) {
		/* Fix the bug for weixin app bind wifi ip, then reset to lte, cannot find the sk */
		emcom_xengine_mpflow_ai_rehash_sk(sk);
		/* Fix the bug for weixin and toutiao two src ip in one gquic connect */
		sk_dst_reset(sk);
		inet = inet_sk(sk);
		if (inet->inet_saddr)
			inet->inet_saddr = 0;
