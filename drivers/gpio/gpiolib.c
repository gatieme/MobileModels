			       ...
			       struct lock_class_key *request_key)
{
...
#if defined(CONFIG_GPIO_HIV500) || defined(CONFIG_GPIO_PL061_VM)
	CALL_HCK_VH(get_gpio_id, &gpio_ida, base, gc->ngpio, &gdev->id);
#else
	...
	if (base < 0) {
		...
		CALL_HCK_VH(gpio_find_base, &gpio_devices, gc->ngpio, &base);
#else
