	...
	struct hvgr_ctx *ctx, struct hvgr_ct_setup *setup)
{
	...
	if ((setup->dump_buffer == 0ULL) || (setup->dump_buffer & (u64)0x7FF))
		return -EINVAL;
	...
	config.dump_buffer_base_low = setup->dump_buffer & (u64)0xFFFFFFFF;
	config.dump_buffer_base_high = setup->dump_buffer >> OFFSET_32BIT;
