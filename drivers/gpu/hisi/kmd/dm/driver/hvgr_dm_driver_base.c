...
void hvgr_disable_interrupts(struct hvgr_device *gdev)
{
	...
	hvgr_write_reg(gdev, mmu_control_reg(gdev, MMU_IRQ_CLEAR), 0xFFFFFFFF);
...
void hvgr_enable_interrupts(struct hvgr_device *gdev)
{
	...
	hvgr_write_reg(gdev, mmu_control_reg(gdev, MMU_IRQ_CLEAR), 0xFFFFFFFF);
	hvgr_write_reg(gdev, mmu_control_reg(gdev, MMU_IRQ_MASK), 0xFFFFFFFF);
...
irqreturn_t hvgr_gpu_irq(int irq, void *data)
{
	...
	if ((val & CLEAN_CACHES_COMPLETED) != 0)
		atomic_set(&gdev->pm_dev.pm_pwr.flush_status, GPU_FLUSH_CLEAN_DONE);
