...
#define HVGR_MEM_PROFILE_MAX_SIZE ((64 + ((80 + (56 * 64)) * 15) + 56))
...
static void hvgr_mem_profile_kmsg(struct hvgr_ctx *ctx, u64 used)
{
	...
	(void)memcpy_s(buff, ctx->mem_profile_size, ctx->mem_profile_buf, ctx->mem_profile_size);
	pr_err("ctx_%u %10u %10u used summary:%llu\n",ctx->id, ctx->pid, ctx->tgid, used);
	pr_err("%s\n", buff);
...
#ifdef CONFIG_DFX_DEBUG_FS
static uint32_t hvgr_mem_get_jit_pool_pages(struct hvgr_ctx *ctx)
{
	struct hvgr_mem_area *iter = NULL;
	struct hvgr_mem_area *temp = NULL;
	uint32_t jit_pages = 0;
...
ssize_t hvgr_mem_show_page_statics(void *data, char * const buf)
{
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret, "ctx num: %u pool pages: %u jit pages: %u\n",
