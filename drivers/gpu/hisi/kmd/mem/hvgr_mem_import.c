...
static bool hvgr_mem_import_init(struct hvgr_mem_zone * const zone, void * const para)
{
	struct hvgr_mem_import_cb *mem_import = NULL;
	struct hvgr_ctx *ctx = NULL;
...
uint64_t hvgr_mem_import_reserve_va_alloc(struct hvgr_mem_area * const area)
{
	uint64_t addr;
	uint64_t size;
	struct hvgr_ctx *ctx = NULL;
...
static void hvgr_mem_import_put_page(struct page *page)
{
...
{
	dma_addr_t *dma_addrs = area->import.data.user_buf.dma_addrs;
	uint64_t temp_size = area->import.data.user_buf.size;
	uint64_t i;
	bool gpu_wr = ((area->attribute & (HVGR_MEM_ATTR_CPU_WR | HVGR_MEM_ATTR_GPU_WR)) != 0);

	if (area->import.data.user_buf.mm != NULL)
		mmdrop(area->import.data.user_buf.mm);

	for (i = 0; i < area->max_pages; i++) {
		uint64_t local_size;
		dma_addr_t dma_addr = dma_addrs[i];

		local_size = min(temp_size, PAGE_SIZE - (dma_addr & ~PAGE_MASK));
		dma_unmap_page(ctx->gdev->dev, dma_addr, local_size, DMA_BIDIRECTIONAL);
		if (gpu_wr)
			(void)set_page_dirty_lock(area->page_array[i]);
		hvgr_mem_import_put_page(area->page_array[i]);
...
static long hvgr_mem_import_user_map_pages(struct hvgr_ctx * const ctx, uint64_t address,
	uint64_t size, struct hvgr_mem_area *area)
{
	dma_addr_t dma_addr;
	dma_addr_t *dma_addrs = area->import.data.user_buf.dma_addrs;
	uint64_t offset = address & ~PAGE_MASK;
	uint64_t local_size = size;
	uint64_t i;

	for (i = 0; i < area->max_pages; i++) {
		uint64_t min = min(PAGE_SIZE - offset, local_size);

		dma_addr = dma_map_page(ctx->gdev->dev, area->page_array[i], offset, min,
			DMA_BIDIRECTIONAL);
		if (dma_mapping_error(ctx->gdev->dev, dma_addr)) {
			hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user dma map fail 0x%lx/0x%lx",
				ctx->id, i, area->max_pages);
			while (i != 0ul) {
				i--;
				dma_unmap_page(ctx->gdev->dev, dma_addrs[i], PAGE_SIZE,
					DMA_BIDIRECTIONAL);
			}
	...
	uint64_t size, struct hvgr_mem_area *area)
{
	...
	if (area->max_pages > (U64_MAX / sizeof(struct page *))) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user invalid para", ctx->id);
		return -EINVAL;
	}
	...
	if (area->page_array == NULL) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user alloc page array fail", ctx->id);
		return -1;
	}

	if (area->max_pages <= MEM_PHY_ALLOC_THRESHOLD)
		area->import.data.user_buf.dma_addrs =
			kcalloc(area->max_pages, sizeof(struct page *), GFP_KERNEL);
	else
		area->import.data.user_buf.dma_addrs =
			vzalloc((area->max_pages * sizeof(struct page *)));
	if (area->import.data.user_buf.dma_addrs == NULL) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user alloc dma_addr buf fail",
			ctx->id);
		goto alloc_dma_addr_fail;
	}

	rcu_read_lock();
	area->import.data.user_buf.mm = rcu_dereference(ctx->mem_ctx.process_mm);
	mmgrab(ctx->mem_ctx.process_mm);
	rcu_read_unlock();

	write = (area->attribute & (HVGR_MEM_ATTR_CPU_WR | HVGR_MEM_ATTR_GPU_WR)) != 0;

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
	down_read(&current->mm->mmap_sem);
	real_pages = get_user_pages(address, area->max_pages,
		(write ? FOLL_WRITE : 0), area->page_array, NULL);
	up_read(&current->mm->mmap_sem);
#else
	down_read(&current->mm->mmap_lock);
	real_pages = pin_user_pages(address, area->max_pages,
		(write ? FOLL_WRITE : 0), area->page_array, NULL);
	up_read(&current->mm->mmap_lock);
#endif
	tmp_pages = (uint64_t)real_pages;
	if (real_pages < 0 || tmp_pages != area->max_pages) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user get user pages fail 0x%lx:0x%lx",
			ctx->id, tmp_pages, area->max_pages);
		goto get_pages_fail;
	}

	if (hvgr_mem_import_user_map_pages(ctx, address, size, area) != 0)
		goto get_pages_fail;

	area->import.data.user_buf.size = size;
	return 0;
get_pages_fail:
	mmdrop(area->import.data.user_buf.mm);
	if (real_pages > 0) {
		for (i = 0; i < tmp_pages; i++)
			hvgr_mem_import_put_page(area->page_array[i]);
	}

	kvfree(area->import.data.user_buf.dma_addrs);
alloc_dma_addr_fail:
	kvfree(area->page_array);
	return -1;
}

static bool hvgr_mem_import_user_free(struct hvgr_mem_zone * const zone,
	struct hvgr_mem_area * const area)
{
	struct hvgr_ctx *ctx = NULL;
	...
	uint32_t start_page_nr, struct hvgr_mem_area *area)
{
	...
	dma_buf_rec->page_nr = area->pages - start_page_nr;
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	start_page_nr = area->pages;
#endif
	insert_pages = hvgr_mem_import_insert_pages(ctx, area, sg_table, pages, area->pages);
...
void hvgr_mem_import_free_pages(struct hvgr_mem_area *area)
{
	if (area->import.type == HVGR_MEM_IMPORT_UMM) {
		hvgr_mem_import_umm_free_pages(area);
	} else if (area->import.type == HVGR_MEM_IMPORT_USER_BUF) {
		struct hvgr_mem_ctx *pmem = area->zone->pmem;
