...
{
	hvgr_mem_area_slab = kmem_cache_create("hvgr_mem_area", sizeof(struct hvgr_mem_area),
...
long hvgr_mem_sync(struct hvgr_ctx * const ctx, uint64_t uva, uint64_t size, uint64_t direct)
{
	...
	sync_size = min(((uint64_t)PAGE_SIZE - offset_page_byte), size);
...
static long hvgr_mem_init_process_res_cb(struct hvgr_ctx * const ctx)
{
	...
	node = hvgr_kv_get(&ctx->gdev->mem_dev.mem_process_res_rec, ctx->tgid);
	if (node != NULL) {
	...
	uint64_t pages)
{
	...
	ret = hvgr_mmu_take_down(ctx, (area->gva + va_offset), diff_page);
	if (ret != 0) {
	...
	hvgr_mem_free_page(ctx->gdev->dev, (area->page_array + pages), diff_page, area->order,
		policy_id);
...
void hvgr_mem_free_area_when_gpu_fault(struct hvgr_ctx * const ctx, uint64_t prot)
{
	...
	if (!hvgr_kv_is_empty(&pmem->area_rec)) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u free prot %u when gpu fault.", ctx->id, prot);
...
void hvgr_mem_unbind_all_when_gpu_fault(struct hvgr_ctx * const ctx, uint64_t prot)
{
	...
	mutex_lock(&pmem->area_mutex);
	if (!hvgr_kv_is_empty(&pmem->area_rec)) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u unbind prot %u when gpu fault.", ctx->id, prot);
...
static void hvgr_mem_jit_check_leak(struct hvgr_ctx * const ctx)
{
	...
	if (!list_empty(&pmem->jit_using_head))
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u jit using head not free", ctx->id);

	if (!list_empty(&pmem->jit_pool_head))
	...
	uint64_t pages)
{
	...
	if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
		if (hvgr_mem_zap_ptes(ctx, area, (area->uva + va_offset),
			(diff_page << PAGE_SHIFT), false) != 0) {
			area->pages += diff_page;
			area->phy_pages += diff_page;
			hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u resize 0x%lx pages %lu zap cpu fail",
				ctx->id, area->gva, pages);
			return -ENOMEM;
		}
	...
	uint64_t pages)
{
	...
	uint64_t uva_start = area->uva + (area->pages << PAGE_SHIFT);

	ret = hvgr_mem_pool_ctx_alloc(ctx, diff_page, (void **)page_arr,
		hvgr_mem_get_sc_id(area->attribute));
	if (ret != 0) {
	...
	if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
		if (hvgr_mem_insert_pages(ctx, area, uva_start, page_arr, diff_page) != 0) {
			hvgr_mem_pool_ctx_free(ctx, diff_page, (void **)page_arr, 0,
				hvgr_mem_get_sc_id(area->attribute));
			hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u resize 0x%lx pages %lu map uva fail",
				ctx->id, area->gva, pages);
			return -ENOMEM;
		}
	}
	...
	if (ret != 0) {
		if ((area->attribute & HVGR_MEM_MASK_UMD_ACCESS) != 0) {
			if (hvgr_mem_zap_ptes(ctx, area, uva_start,
				(diff_page << PAGE_SHIFT), false) != 0) {
				area->pages = pages;
				area->phy_pages = pages;
				hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u resize 0x%lx pages %lu unmap uva failed %d",
					ctx->id, area->gva, pages, ret);
				return -ENOMEM;
			}
		}
...
long hvgr_mem_resize(struct hvgr_ctx * const ctx, uint64_t gva, uint64_t pages)
{
	...
	if ((area->property != HVGR_MEM_PROPERTY_GPU_MEM) &&
		...
		(area->property != HVGR_MEM_PROPERTY_4G_VA)) {
...
struct hvgr_mem_area *hvgr_mem_import_user(struct hvgr_ctx * const ctx,
	struct hvgr_mem_import_user_para * const para)
{
	struct hvgr_mem_area *area = NULL;
	struct hvgr_mem_zone *zone = NULL;
	uint64_t import_pages;
	long ret;

	import_pages = (PAGE_ALIGN(para->address + para->size) >> PAGE_SHIFT) -
		PFN_DOWN(para->address);
	if (para->address > (U64_MAX - para->size) ||
		import_pages == 0) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import params invalid", ctx->id);
		return NULL;
	}

	zone = hvgr_mem_zone_get_zone(&ctx->mem_ctx, HVGR_MEM_PROPERTY_IMPORT);
	if (zone == NULL) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import zone is null", ctx->id);
		return NULL;
	}

	area = hvgr_mem_new_area(&ctx->mem_ctx);
	if (area == NULL) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user alloc area fail", ctx->id);
		return NULL;
	}
	...
	area->attribute = para->attribute;
	area->pages = import_pages;
	area->max_pages = import_pages;

	ret = hvgr_mem_import_user_get_pages(ctx, para->address, para->size, area);
	if (ret != 0)
		goto get_pages_fail;

	area->uva = hvgr_mem_alloc_va(area, true);
	if (area->uva == 0) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user alloc va fail 0x%lx",
	...
	if (!hvgr_mem_map_gva(area)) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u import user map gva fail", ctx->id);
		goto map_gva_fail;
	}
	...
	area->import.type = HVGR_MEM_IMPORT_USER_BUF;
	area->import.data.user_buf.size = para->size;
	area->user_buf = true;

	hvgr_info(ctx->gdev, HVGR_MEM,
		"ctx_%u import user addr=0x%lx size=0x%lx to prot=%lu:0x%lx, page:%lu/%lu uva/gva=0x%lx",
		ctx->id, para->address, para->size, area->property, area->attribute,
		area->pages, area->max_pages, area->gva);
...
alloc_va_fail:
	hvgr_mem_import_user_free_pages(ctx, area);
get_pages_fail:
	hvgr_mem_delete_area(area);
	return NULL;
}
...
static int hvgr_mem_vm_fault(struct vm_fault *vmf)
{
	int ret;
#else
static vm_fault_t hvgr_mem_vm_fault(struct vm_fault *vmf)
{
	vm_fault_t ret;
#endif
	struct hvgr_mem_area *area = NULL;
	...
	page_offset = (vmf->address - area->uva) >> PAGE_SHIFT;
...
int hvgr_mem_mmap(struct hvgr_ctx * const ctx, struct vm_area_struct *vma)
{
	...
	if (vma->vm_pgoff >= PFN_DOWN(HVGR_MEM_BITMAP_BASE_OFFSET)) {
		area_index = vma->vm_pgoff - PFN_DOWN(HVGR_MEM_BITMAP_BASE_OFFSET);
		...
		ret = hvgr_mem_setup_cpu_mmu(vma, vma->vm_start, area->page_array, area->pages);
		if (ret != 0) {
			mutex_unlock(&ctx->mem_ctx.mmap_mutex);
	...
	uint32_t info_num)
{
	...
	for (i = 0; i < info_num; i++) {
		...
		area_policy_id = hvgr_sc_id_to_policy_id(hvgr_mem_get_sc_id(sc_area->attribute));
	...
	void **target_addr, uint64_t *offset)
{
	...
	*target_addr = vmap(area->page_array, area->pages, VM_MAP, prot);
