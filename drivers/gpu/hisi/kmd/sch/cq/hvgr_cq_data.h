...
#define CQ_KMD_CTX_START_SUBMIT              (1 << 0)
#define CQ_KMD_CTX_ERROR_FLAG                (1 << 1)
...
#define HVGR_TOKEN_DEP_NUMS            8
#define HVGR_FENCE_TIMEOUT_TIME        2000000000u /* 2s */
#define HVGR_FENCE_TIMEOUT_TIME_FPGA   20000000000u /* 20s */

#define HVGR_TOKEN_CB_WORK_RUNNING          (1 << 0)
#define HVGR_TOKEN_CB_RESOURCE              (1 << 1)
#define HVGR_TOKEN_CB_WAIT_FENCE            (1 << 2)
#define HVGR_TOKEN_CB_TRIGGER_FENCE         (1 << 3)
#define HVGR_TOKEN_HOLD_EN_IN               (1 << 4)
#define HVGR_TOKEN_CB_WQ_WAIT               (1 << 5)
#define HVGR_TOKEN_NORMAL_ENTRY             (1 << 6)
#define HVGR_TOKEN_SC_UPDATE_POLICY         (1 << 7)
#define HVGR_TOKEN_HOLD_EN_OUT              (1 << 8)
#define HVGR_TOKEN_FREQ_HINT                (1 << 9)
#define HVGR_TOKEN_NOTIFY_TOP               (1 << 10)
#define HVGR_TOKEN_CL_PROFILING_EXEC        (1 << 11)
#define HVGR_TOKEN_CL_PROFILING_COMPLETE    (1 << 12)
...
#define CQ_QUEUE_ENTRY_NUMBER             4096
...
struct hvgr_msync_ctx {
	/* Lock for change counter. */
	spinlock_t lock;
	uint32_t counter;
...
struct hvgr_msync_table {
	/* Lock for push/pop token and modif token_pending */
	spinlock_t lock;

	/* Pre-alloc tokens. */
	struct hvgr_msync_ctx *token_memory;
	uint32_t idle_tokens;
	struct hvgr_msync_ctx *tokens[CQ_SYNC_TOKEN_NUMS];

	/* Token pending table */
	struct hvgr_msync_ctx *token_pending[CQ_SYNC_TOKEN_NUMS];
};
...
struct hvgr_cq_ctx_data {
	/* Lock for alloc and free cqctx */
	spinlock_t ctx_lock;
	struct hvgr_cq_ctx cq_ctx[CQ_QUEUE_NUM_PER_CTX];

	struct hvgr_msync_table token_pending;
...
struct cq_protect_mode {
	...
	struct hvgr_cq_ctx *bak_running_queue;
