...
#define OFFSET_16BIT    16
#define ENTRY_NUM_MAX   4096
#define OPCODE2_NOTIFY  3
#define BASE_JCD_SIZE   8
...
bool hvgr_get_prot_enhance_from_of(struct hvgr_device * const gdev)
{
	...
	if (of_property_read_u32(gdev->dev->of_node, "prot_enhance", (uint32_t *)&prot_tmp) != 0) {
		hvgr_err(gdev, HVGR_CQ, "Can not get prot_enhance.");
		return false;
	}
...
bool hvgr_mem_protect_jcd_pool_term(struct hvgr_device * const gdev)
{
	...
	gen_pool_free(gdev->mem_dev.protect_pool, gdev->mem_dev.protect_start,
		gdev->mem_dev.protect_size);
	gen_pool_destroy(gdev->mem_dev.protect_pool);
...
int hvgr_mem_protect_jcd_init(struct hvgr_ctx * const ctx)
{
	...
	for (i = 0; i < BASE_JM_MAX_NR_SLOTS; i++)
		ctx->jcd_offset[i] = i * align(sizeof(struct base_jcd), ALIGN_64);
...
void hvgr_mem_protect_jcd_term(struct hvgr_ctx * const ctx)
{
	...
	(void)hvgr_mmu_take_down(ctx, ctx->jcd_area->gva, ctx->jcd_area->pages);
...
static void backup_timer_callback_protect_mode(unsigned long addr)
{
	...
	if (ret > 0)
		...
		hvgr_dmd_msg_set(gdev, DMD_JOB_HANG, "higpu protect mode job hang");
	...
	struct js_info_s *js_info)
{
	...
	hvgr_write_reg(gdev, job_jc_y_reg(gdev, jc_index, JSX_JCY_AFFINITY),
		affinity & (u64)0xFFFFFFFF);

	hvgr_write_reg(gdev, job_jc_y_reg(gdev, jc_index, JSX_JCY_JCD_LO),
		(ctx->jsx_jcd + ctx->jcd_offset[js]) & (u64)0xFFFFFFFF);
	...
	cfg = hvgr_cq_hw_submit_configure_mmu(gdev, ctx, js, jc_index);
	...
	(void)mod_timer(&cqctx->backup_timer_protect_mode,
		jiffies + gdev->cq_dev.reset_timeout_normal * HZ);
}
...
void hvgr_protect_mode_disable_interrupts(struct hvgr_device *gdev)
{
	int i;
	for (i = 0; i < BASE_JM_MAX_NR_SLOTS; i++) {
		hvgr_write_reg(gdev, job_slot_x_reg(gdev, (uint32_t)i, JSX_IRQ_CLEAR), 0xFFFFFFFF);
...
static void hvgr_protect_mode_enable_interrupts(struct hvgr_device *gdev)
{
	...
	for (i = 0; i < BASE_JM_MAX_NR_SLOTS; i++) {
		hvgr_write_reg(gdev, job_slot_x_reg(gdev, (uint32_t)i, JSX_IRQ_CLEAR), 0xFFFFFFFF);
		hvgr_write_reg(gdev, job_slot_x_reg(gdev, (uint32_t)i, JSX_IRQ_MASK), 0xFFFFFFFF);
	}
	...
	uint32_t channel_id)
{
	...
	case HVGR_CQ_ENTER_PROTECTED_WAIT_IDLE:
		...
		if (gcqctx->protect_mode_info.bak_running_queue != NULL) {
			enqueue_flag = (enqueue_flag &&
				(cq_ctx != gcqctx->protect_mode_info.bak_running_queue));
			hvgr_info(gdev, HVGR_CQ, "%s bakrq_id=%u\n", __func__,
					gcqctx->protect_mode_info.bak_running_queue->queue_id);
		}
...
static uint32_t hvgr_cq_protect_exit_multi_user(struct hvgr_device *gdev, uint32_t channel_id)
{
	...
	if ((gcqctx->protect_mode_info.protected_mode_refcount != 1) ||
		(gcqctx->protect_mode_info.bak_running_queue == NULL)) {
	...
	gcqctx->protect_mode_info.protected_mode_transition = false;
	gcqctx->protect_mode_info.running_queue = gcqctx->protect_mode_info.bak_running_queue;
	gcqctx->protect_mode_info.bak_running_queue = NULL;
	if (prot_enhance)
		hvgr_cq_schedule_js_in(gcqctx->protect_mode_info.running_queue);
	else
	...
	uint32_t channel_id)
{
	...
	case HVGR_CQ_EXIT_PROTECTED_WAIT_IDLE:
		...
		if (gcqctx->protect_mode_info.bak_running_queue != NULL)
			hvgr_info(gdev, HVGR_CQ, "%s bakrq_id=%u\n",
				__func__,  gcqctx->protect_mode_info.bak_running_queue->queue_id);
...
void *hvgr_cq_vmap_user_addr(struct hvgr_ctx * const ctx, uint64_t gpu_addr)
{
	...
	return vmap(area->page_array, area->pages, VM_MAP, prot);
}
	...
	struct hvgr_device *gdev, uint32_t channel_id, enum protect_flag flag)
{
	...
	if (flag == CQ_PROTECT_MODE_IN) {
		...
		if (gcqctx->protect_mode_info.protected_mode_refcount == 1) {
			...
			/* Two queue received hold_en at the same time. We just record the 2th. */
			gcqctx->protect_mode_info.bak_running_queue = cq_ctx;
		} else {
