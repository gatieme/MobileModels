...
static int hvgr_alloc_softq_id(struct hvgr_ctx * const ctx)
{
	...
	unused_id = ffs(ctx->softq_unused & (u16)HVGR_MAX_SOFTQ_BITMAP) - 1;
	if (unused_id >= 0 && unused_id < HVGR_MAX_NR_SOFTQ) {
		ctx->softq_unused &= ~((u32)1 << unused_id);
		return unused_id;
	}
...
{
	ctx->softq_unused |= ((u32)1 << softq_id);
}
	...
	struct hvgr_ctx * const ctx)
{
	...
	para.pages = hvgr_page_nums(sizeof(struct hvgr_softq_ctrl));
	...
	struct hvgr_ctx * const ctx)
{
	...
	para.pages = hvgr_page_nums(sizeof(struct hvgr_softq_user_ctrl));
	para.max_pages = para.pages;
	...
	uint32_t entry_nums)
{
	...
	para.property = HVGR_MEM_PROPERTY_DRIVER;
	para.attribute = HVGR_MEM_ATTR_CPU_RD | HVGR_MEM_ATTR_CPU_WR | HVGR_MEM_ATTR_KMD_ACCESS;
	para.pages = hvgr_page_nums(entry_nums * entry_size);
	para.max_pages = para.pages;
...
bool hvgr_softq_register_rev(struct hvgr_soft_q * const qctx,
	soft_q_rev rev, void *priv)
{
	if ((qctx == NULL) || (rev == NULL))
		return false;
...
void hvgr_softq_do_flush(struct hvgr_soft_q * const qctx)
{
	...
	uint32_t rd;
	unsigned long irq_flag = 0;
	uint64_t base = qctx->queue_area->kva;
	...
	spin_lock_irqsave(&qctx->q_lock, irq_flag);
	rd = ctrl->rd;
	while (rd != qctx->user_ctrl->wr) {
		entry = u64_to_ptr(base + rd * ctrl->entry_size);
		qctx->recv(qctx->priv, entry);
		rd = (rd + 1) & (ctrl->entry_nums - 1);
	}
	ctrl->rd = rd;
	spin_unlock_irqrestore(&qctx->q_lock, irq_flag);
}
