...
static int anc_hs_get_adc_delta(void)
{
	...
	while (loop) {
		loop--;
		mdelay(1);
		ear_pwr_h = lpm_adc_get_value(anc_pdata->channel_pwl_h);
		if (ear_pwr_h < 0) {
		...
		ear_pwr_l = lpm_adc_get_value(anc_pdata->channel_pwl_l);
		if (ear_pwr_l < 0) {
...
static void anc_hs_adc_calibration(void)
{
	...
	while (loop) {
		...
		adc_h = lpm_adc_get_value(anc_pdata->channel_pwl_h);
		if (adc_h < 0) {
			hwlog_err("[anc_hs]get adc fail,adc_h:%d\n", adc_h);
			fail_count++;
			continue;
		}
		adc_l = lpm_adc_get_value(anc_pdata->channel_pwl_l);
		if (adc_l < 0) {
...
#define HS_INFO_SIZE 512
	...
	snprintf(buf, HS_INFO_SIZE, "%scalibration_current: %d\n", buf,
		(lpm_adc_get_value(anc_pdata->channel_pwl_h) -
		lpm_adc_get_value(anc_pdata->channel_pwl_l)));
...
static int anc_hs_probe(struct platform_device *pdev)
{
	...
	anc_pdata->wake_lock = wakeup_source_register(NULL, "anc_hs");
#else
    anc_pdata->wake_lock = wakeup_source_register("anc_hs");
#endif
    if (!anc_pdata->wake_lock) {
        return -ENOMEM;
    }
	value_initial();
...
anc_hs_btn_wq_err:
	gpio_free(anc_pdata->gpio_mic_sw);
gpio_mic_sw_err:
	wakeup_source_unregister(anc_pdata->wake_lock);
#endif
	return ret;
}
