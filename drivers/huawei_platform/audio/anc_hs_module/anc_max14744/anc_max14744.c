...
static int anc_max14744_get_adc_delta(void)
{
	...
	while (loop) {
		loop--;
		mdelay(1);
		ear_pwr_h = lpm_adc_get_value(anc_priv->channel_pwl_h);
		if (ear_pwr_h < 0) {
		...
		ear_pwr_l = lpm_adc_get_value(anc_priv->channel_pwl_l);
		if (ear_pwr_l < 0) {
...
static void anc_max14744_adc_calibration(void)
{
	...
	while (loop) {
		...
		adc_h = lpm_adc_get_value(anc_priv->channel_pwl_h);
		if (adc_h < 0) {
			hwlog_err("[anc_hs]get adc fail,adc_h:%d\n", adc_h);
			fail_count++;
			continue;
		}
		adc_l = lpm_adc_get_value(anc_priv->channel_pwl_l);
		if (adc_l < 0) {
...
static int compute_final_voltage(void)
{
	...
	msleep(70);
	adc_volt_low = lpm_adc_get_value(anc_priv->channel_pwl_l);
	adc_volt_high = lpm_adc_get_value(anc_priv->channel_pwl_h);
	...
	const struct i2c_device_id *id)
{
	...
	di->wake_lock = wakeup_source_register(NULL, "anc_max14744");
#else
    di->wake_lock  = wakeup_source_register("anc_max14744");
#endif
    if (!di->wake_lock) {
