...
static int anc_ncx8293_get_adc_delta(void)
{
	...
	while (loop) {
		loop--;
		mdelay(1);
		ear_pwr_h = lpm_adc_get_value(anc_priv->h_channel_pwl);
		if (ear_pwr_h < 0) {
		...
		ear_pwr_l = lpm_adc_get_value(anc_priv->l_channel_pwl);
		if (ear_pwr_l < 0) {
...
static void anc_ncx8293_adc_calibration(void)
{
	...
	while (loop) {
		...
		adc_h = lpm_adc_get_value(anc_priv->h_channel_pwl);
		if (adc_h < 0) {
			hwlog_err("[anc_hs]get adc fail,adc_h:%d\n", adc_h);
			fail_count++;
			continue;
		}
		adc_l = lpm_adc_get_value(anc_priv->l_channel_pwl);
		if (adc_l < 0) {
...
static int compute_final_voltage(void)
{
	...
	msleep(70);
	adc_volt_low = lpm_adc_get_value(anc_priv->l_channel_pwl);
	adc_volt_high = lpm_adc_get_value(anc_priv->h_channel_pwl);
...
static void lock_init(void)
{
	...
	anc_priv->wake_lock = wakeup_source_register(NULL, "anc_ncx8293");
#else
    anc_priv->wake_lock = wakeup_source_register("anc_ncx8293");
#endif
    if (!anc_priv->wake_lock) {
