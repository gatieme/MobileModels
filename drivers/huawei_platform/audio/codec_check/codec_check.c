...
#include <platform_include/audio/platform_drivers/da_combine/da_combine_utils.h>
	...
	const void *usr_para_addr)
{
	...
	if (!codec_check_priv_data->para[scene]) {
		...
		hwlog_info("%s: param for scene %d is exist", __func__, scene);
		kfree(para_in);
		para_in = NULL;
	}
	...
	unsigned int mask, unsigned int *real_val)
{
	...
	*real_val = da_combine_utils_reg_read(reg);
...
static int codec_check_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_SCENE_NUM; i++) {
		if (codec_check_priv_data->para[i]) {
			kfree(codec_check_priv_data->para[i]);
			codec_check_priv_data->para[i] = NULL;
		}
	}
