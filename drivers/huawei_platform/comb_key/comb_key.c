...
static struct delayed_work power_key_5s_work;
static struct delayed_work comb_key_4s_work;
...
static void power_volume_comb_status_distinguish(void)
{
	...
	if ((power_key_status == PRESS_KEY_DOWN) &&
		...
		queue_delayed_work(system_power_efficient_wq, &comb_key_4s_work, msecs_to_jiffies(4000));
	} else {
	...
	if (comb_key_status == COMB_KEY_PRESS_RELEASE) {
		cancel_delayed_work_sync(&comb_key_4s_work);
		hwlog_info("comb_key_4s timer has released");
	}
	...
	unsigned long foo, void *bar)
{
	...
	if (foo == PRESS_KEY_DOWN) {
		queue_delayed_work(system_power_efficient_wq, &power_key_5s_work, msecs_to_jiffies(5000));
		queue_delayed_work(system_power_efficient_wq, &power_key_work, 0);
    } else if (foo == PRESS_KEY_UP) {
		cancel_delayed_work_sync(&power_key_5s_work);
		queue_delayed_work(system_power_efficient_wq, &power_key_work, 0);
    }
...
static void power_key_5s_delayed_work(struct work_struct *work)
{
    (void)work;
    hwlog_info("press_powerkey has down 5s %s success", __func__);
    zrhung_send_event(ZRHUNG_WP_PRESSPOWERKEY, NULL,"presspowerkey");
}

static void comb_key_4s_delayed_work(struct work_struct *work)
{
	(void)work;
	hwlog_info("comb_key has down 4s %s success", __func__);
    cancel_delayed_work_sync(&power_key_5s_work);
    zrhung_send_event(ZRHUNG_WP_COMBKEY, NULL,"combkey");
}
...
static int __init comb_key_init(void)
{
	...
	INIT_DELAYED_WORK(&power_key_5s_work, power_key_5s_delayed_work);
	INIT_DELAYED_WORK(&comb_key_4s_work, comb_key_4s_delayed_work);
#endif
