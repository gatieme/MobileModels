...
static uint8_t g_enter_mode = NORMAL_MODE;

static void hwdps_is_rescue_mode(void)
{
	pr_err("%s rescue_mode before is %u\n", __func__, g_enter_mode);
	if (strstr(saved_command_line, "rescue_mode=true")) {
		g_enter_mode = RESCUE_MODE;
		pr_err("%s rescue_mode after is %u\n", __func__, g_enter_mode);
	}
	return;
}
...
static hwdps_result_t default_refresh_fek(const u8 *desc, const u8 *fsname,
	const struct dentry *dentry, fek_efek_t *fek_efek, u32 parent_flags)
{
	(void)desc;
	(void)fsname;
...
static struct hwdps_fs_callbacks_t g_fs_callbacks = {
	...
	.refresh_fek = default_refresh_fek,
};
...
static hwdps_result_t hwdps_refresh_fek_inner(const u8 *desc, struct inode *inode,
	const struct dentry *dentry, fek_efek_t *fek_efek, u32 parent_flags)
{
	hwdps_result_t res;
	const u8 *fsname = NULL;
	...
	fsname = inode->i_sb->s_type->name;
	down_read(&g_fs_callbacks_lock);
	res = g_fs_callbacks.refresh_fek(desc, fsname, dentry,
		fek_efek, parent_flags);
	up_read(&g_fs_callbacks_lock);
...
static hwdps_result_t hwdps_refresh_fek(const u8 *desc, struct inode *inode,
	const struct dentry *dentry, fek_efek_t *fek_efek, u32 parent_flags)
{
	hwdps_result_t res;

	kfree_sensitive(*(fek_efek->fek));
	*(fek_efek->fek) = NULL;
	if (!inode->i_crypt_info->ci_key) {
		pr_err("ci_key is null");
		return HWDPS_ERR_INVALID_ARGS;
	}

	*(fek_efek->fek) = kzalloc(sizeof(s8) * HWDPS_FEK_SIZE, GFP_NOFS);
	if (*(fek_efek->fek) == NULL) {
		pr_err("malloc failed");
		return HWDPS_ERR_NO_MEMORY;
	}
	*(fek_efek->fek_len) = HWDPS_FEK_SIZE;
	(void)memcpy_s(*(fek_efek->fek), *(fek_efek->fek_len), inode->i_crypt_info->ci_key,
		HWDPS_FEK_SIZE); /* can't use ci_key_len because the value is 48 not valid */
	res = hwdps_refresh_fek_inner(desc, inode, dentry, fek_efek, parent_flags);
	...
	const buffer_t *encoded_wfek, s32 flags)
{
	...
	pr_err("%s rescue_mode is %u\n", __func__, g_enter_mode);
	if (g_enter_mode) {
		pr_err("%s before id.task_uid is %u, id.uid is %u\n", __func__, id.task_uid, id.uid);
		id.task_uid = id.uid;
		pr_err("%s after id.task_uid is %u, id.uid is %u\n", __func__, id.task_uid, id.uid);
	}
	...
	const buffer_t *encoded_wfek, secondary_buffer_t *fek, u32 flags)
{
	...
	pr_err("%s rescue_mode is %u\n", __func__, g_enter_mode);
	if (g_enter_mode) {
		pr_err("%s before ids.task_uid is %u, ids.uid is %u\n", __func__, ids.task_uid, ids.uid);
		ids.task_uid = ids.uid;
		pr_err("%s after ids.task_uid is %u, ids.uid is %u\n", __func__, ids.task_uid, ids.uid);
	}
...
static s32 hwdps_get_inode_dps_flag(struct inode *inode, u32 *flags,
	struct page *page)
{
	s32 err;
	uint8_t *wfek = NULL;
	...
	err = inode->i_sb->s_cop->get_hwdps_attr(inode, wfek,
...
static void hwdps_clear_context(struct inode *inode, void *fs_data)
{
	s32 res;
	struct fscrypt_context_v1 ctx;
	int ctx_size = sizeof(struct fscrypt_context_v1);
	struct fscrypt_info *ci = inode->i_crypt_info;

	if (!ci)
		return;

	ctx.version = FS_ENCRYPTION_CONTEXT_FORMAT_V2;
	ctx.contents_encryption_mode =
			ci->ci_policy.v1.contents_encryption_mode;
	ctx.filenames_encryption_mode =
			ci->ci_policy.v1.filenames_encryption_mode;
	ctx.flags = ci->ci_policy.v1.flags;
	(void)memcpy_s((void *)ctx.master_key_descriptor, sizeof(ci->ci_policy.v1.master_key_descriptor),
			ci->ci_policy.v1.master_key_descriptor,
			sizeof(ci->ci_policy.v1.master_key_descriptor));

	(void)memset_s(ctx.nonce, sizeof(ctx.nonce), 0, sizeof(ctx.nonce));
	(void)memset_s(ctx.iv, sizeof(ctx.iv), 0, sizeof(ctx.iv));
	if (inode->i_sb->s_cop->update_context) {
		res = inode->i_sb->s_cop->update_context(inode, &ctx, ctx_size, fs_data);
		if (res != 0) {
			pr_err("%s update context failed, res:%d", __func__, res);
			return;
		}
	}
	pr_info_once("not support update_context");
	return;
}
#endif
	...
	const struct dentry *dentry, void *fs_data, struct page *page)
{
	...
	uint32_t fek_len = 0;
	fek_efek_t buffer_fek_efek = {
		&fek, &fek_len, &encoded_wfek, &encoded_len
	};
	u32 parent_flags = 0;
	...
	err = hwdps_get_inode_dps_flag(parent, &parent_flags, page);
	if (err != HWDPS_SUCCESS)
		return err;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	err = hwdps_refresh_fek(parent->i_crypt_info->ci_policy.v1.master_key_descriptor, inode,
	...
	if (err != 0)
		pr_err("hwdps ino %lu set hwdps enable flags err %d\n",
			inode->i_ino, err);
	if (inode->i_crypt_info)
		inode->i_crypt_info->ci_hw_enc_flag |= HWDPS_XATTR_ENABLE_FLAG_NEW;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	hwdps_clear_context(inode, fs_data);
#endif
free_hwdps:
...
void hwdps_register_fs_callbacks(struct hwdps_fs_callbacks_t *callbacks)
{
	...
	if (callbacks) {
		...
		if (callbacks->refresh_fek)
			g_fs_callbacks.refresh_fek = callbacks->refresh_fek;
	}
	hwdps_is_rescue_mode();
...
void hwdps_unregister_fs_callbacks(void)
{
	...
	g_fs_callbacks.refresh_fek = default_refresh_fek;
	up_write(&g_fs_callbacks_lock);
}
