...
static int8_t g_lcd = LCD_OTHER;
static int8_t g_ddic = UNKNOWN_DDIC;
typedef struct {
...
static lcd_ddic_t lcd_ddic_info[] = {
	{"190_h01", LCD_BOE, DDIC_HISI},
	{"310_h01", LCD_VISIONOX, DDIC_HISI},
	{"190_c08", LCD_BOE, DDIC_RUIDING692H5},
	{"310_c08_1", LCD_VISIONOX, DDIC_RUIDING692H5},
	{"310_c08_2", LCD_VISIONOX, DDIC_RUIDING692H5_1},
...
static void get_lcd_ddic(int32_t tag)
{
	uint8_t index;
	...
	for (index = 0; index < ARRAY_SIZE(lcd_ddic_info); index++) {
		if (!strncmp(lcd_model, lcd_ddic_info[index].lcd_model, strlen(lcd_ddic_info[index].lcd_model))) {
			g_lcd = lcd_ddic_info[index].lcd;
			g_ddic = lcd_ddic_info[index].ddic;
			break;
		}
	}
	hwlog_info("%s: g_lcd = %d, g_ddic = %d\n", __func__, g_lcd, g_ddic);
}
...
int send_als_calibrate_data_to_mcu(int32_t tag)
{
	...
	if (pf_data->als_phone_type != PCE && pf_data->als_phone_type != XYAO) {
		get_lcd_ddic(tag);
		dev_info->als_offset[4] = g_lcd;
		dev_info->als_offset[5] = g_ddic;
	}
	if (memcpy_s(dev_info->als_sensor_calibrate_data,
		sizeof(dev_info->als_sensor_calibrate_data),
