...
static void select_stk31862_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, stk31862_als_para_table *stk31862_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	uint32_t i;
	stk31862_als_para_table *stk31862_tab = stk31862_array;
	...
	for (i = 0; i < table_size; i++) {
		if ((stk31862_tab->phone_type == pf_data->als_phone_type) &&
			(stk31862_tab->phone_version == pf_data->als_phone_version) &&
			(stk31862_tab->tp_lcd_manufacture ==
			get_tplcd_manufacture() ||
			stk31862_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			stk31862_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(stk31862_tab->tp_color == phone_color ||
			stk31862_tab->tp_color == OTHER))
			break;

		stk31862_tab++;
	}

	stk31862_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = stk31862_tab->stk31862_para[STK31862_MIN_THRESHOLD_NUM];
	dev_info->max_thres = stk31862_tab->stk31862_para[STK31862_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = get_tplcd_manufacture();

	als_set_extend_data(pf_data, dev_info, stk31862_tab->stk31862_para,
		sizeof(stk31862_tab->stk31862_para), (i < table_size));
}

static void select_sip1221lr_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, sip1221lr_als_para_table *sip1221lr_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	uint32_t i;
	sip1221lr_als_para_table *sip1221lr_tab = sip1221lr_array;
	...
	for (i = 0; i < table_size; i++) {
		if ((sip1221lr_tab->phone_type == pf_data->als_phone_type) &&
			(sip1221lr_tab->phone_version == pf_data->als_phone_version) &&
			(sip1221lr_tab->tp_lcd_manufacture ==
			get_tplcd_manufacture() ||
			sip1221lr_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			sip1221lr_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(sip1221lr_tab->tp_color == phone_color ||
			sip1221lr_tab->tp_color == OTHER))
			break;

		sip1221lr_tab++;
	}

	sip1221lr_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = sip1221lr_tab->sip1221lr_para[SIP1221LR_MIN_THRESHOLD_NUM];
	dev_info->max_thres = sip1221lr_tab->sip1221lr_para[SIP1221LR_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = get_tplcd_manufacture();

	als_set_extend_data(pf_data, dev_info, sip1221lr_tab->sip1221lr_para,
		sizeof(sip1221lr_tab->sip1221lr_para), (i < table_size));
}
...
void select_stk31862_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_stk31862_als_data_ex(pf_data, dev_info,
		als_get_stk31862_first_table(),
		als_get_stk31862_table_count());
}

void select_sip1221lr_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_sip1221lr_als_data_ex(pf_data, dev_info,
		als_get_sip1221lr_first_table(),
		als_get_sip1221lr_table_count());
}
