...
#define ALS_TEST_NAME_MAX_LEN        70
...
static char als_test_with_device_name[ALS_DEV_COUNT_MAX][ALS_CAL_NUM][ALS_TEST_NAME_MAX_LEN];
static bool als_test_name_joined_flag[ALS_DEV_COUNT_MAX];
#endif

static unsigned long g_stop_auto_als;

unsigned long get_stop_auto_als(void)
{
	...
	uint16_t *cali_data_u16)
{
	...
	for (i = 0; i < ALS_CAL_NUM; i++)
		als_test.test_name[i] =
			als_test_with_device_name[dev_info->als_dev_index][i];
...
#ifdef SENSOR_DATA_ACQUISITION
static void als_test_name_join_with_device(struct als_device_info *dev_info)
{
	int i;
	int ret;
	int8_t dev_id = dev_info->als_dev_index;
	struct sensorlist_info *sensorlist_info_als =
		get_sensorlist_info_by_index(dev_info->detect_list_id);

	if  ((dev_id >= ALS_DEV_COUNT_MAX) || (dev_id < 0))
		return;

	for (i = 0; i < ALS_CAL_NUM; i++) {
		ret = strcpy_s(als_test_with_device_name[dev_id][i],
			strlen(als_test_name[dev_id][i]) + 1, als_test_name[dev_id][i]);
		if (ret)
			hwlog_err("%s als test name copy fail %d %d %d\n",
				__func__, dev_id, i, ret);
		ret = strcat_s(als_test_with_device_name[dev_id][i], sizeof(als_test_with_device_name[dev_id][i]), "_");
		if (ret)
			hwlog_err("%s strcat fail dev id %d %d\n", __func__, dev_id, i);
		ret = strncat_s(als_test_with_device_name[dev_id][i],
			sizeof(als_test_with_device_name[dev_id][i]),
			sensorlist_info_als->name,
			strlen(sensorlist_info_als->name));
		if (ret)
			hwlog_err("%s strncat fail dev id %d %d\n", __func__, dev_id, i);
		hwlog_info("%s : %s\n", __func__, als_test_with_device_name[dev_id][i]);
	}
	als_test_name_joined_flag[dev_id] = true;
}
	...
	struct als_device_info *dev_info)
{
	...
	if  ((dev_id >= ALS_DEV_COUNT_MAX) || (dev_id < 0))
		return -1;
#ifdef SENSOR_DATA_ACQUISITION
	if (als_test_name_joined_flag[dev_id] == false)
		als_test_name_join_with_device(dev_info);
#endif /* SENSOR_DATA_ACQUISITION */
	...
	if ((sensor_status.delay[tag] == 0) ||
	    (sensor_status.delay[tag] > ALS_PERIOD_MAX)) {
