...
static void cap_prox_calibrate_data_process(int calibrate_index,
	...
	uint16_t *calibrate_thred, int sar_type, uint16_t *calibrate_thred1)
{
	...
	case NEAR_CALIBRATE:
		...
		if (calibrate_thred1) {
			min_threshold[1] = calibrate_thred1[DIFF_MIN_THREDHOLD];
			max_threshold[1] = calibrate_thred1[DIFF_MAX_THREDHOLD];
		}
		break;
	case FAR_CALIBRATE:
		...
		if (calibrate_thred1) {
			min_threshold[1] = calibrate_thred1[OFFSET_MIN_THREDHOLD];
			max_threshold[1] = calibrate_thred1[OFFSET_MAX_THREDHOLD];
		}
		break;
	default:
...
static int32_t cap_prox1_cali_process(unsigned long val)
{
	...
	cap_prox_calibrate_data_process((int)val, pkg_mcu.data, diff_value,
		offset_value, calibrate_thred, HW_CAP_PROX1_SENSOR, NULL);
...
static int32_t cap_prox_cali_process(unsigned long val)
{
	...
	cap_prox_calibrate_data_process((int)val, pkg_mcu.data, diff_value,
		offset_value, calibrate_thred, HW_CAP_PROX_SENSOR, calibrate_thred1);
