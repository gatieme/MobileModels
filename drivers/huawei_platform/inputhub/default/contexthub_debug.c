...
static int set_sensor_slave_addr(int tag, int argv[], int argc)
{
	...
	if (pkg_mcu.errno != 0)
		...
		hwlog_info("set %s new slave addr success\n",
			obj_tag_str[tag]);
	return 0;
}
...
int iomcu_dbg(int tag, int argv[], int argc)
{
	struct write_info pkg_ap;
	uint32_t *send = NULL;
	uint32_t send_len;
	...
	if ((uint32_t)argc > IOMCU_DBG_MAX_ARGC || argc < IOMCU_DBG_MIN_ARGC) {
		hwlog_err("%s : invalid arg number %x.\n", __func__, argc);
		return -1;
	}
	...
	if (send == NULL) {
		hwlog_err("%s: malloc send buffer failed.\n", __func__);
		return -1;
	}
...
static void register_my_debug_operations(void)
{
	...
	register_sensorhub_debug_op(iomcu_dbg);
}
...
static void unregister_my_debug_operations(void)
{
	...
	ret |= (unsigned int)unregister_sensorhub_debug_operation(iomcu_dbg);
	if (ret != 0)
		hwlog_err("%s err\n", __func__);
}
	...
	struct class_attribute *attr, const char *buf, size_t size)
{
	...
	record_last_screen_state(screen_status);
	return size;
}
