...
static struct mutex logbuff_open_mutex;
...
{
	uint32_t ddr_log_buff_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buff_size = DDR_LOG_BUFF_SIZE;
#endif
	hwlog_debug("%s %d\n", __func__,
...
{
	uint32_t ddr_log_buff_copy_size;
	uint32_t ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4:ddr buffer water mark */
	ddr_log_buff_copy_size = (((ddr_log_buf_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buff_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
	/* update sensorhub_log_r */
	if (flush_cnt && (sensorhub_log_buff_left() >=
		(ddr_log_buf_size - ddr_log_buff_copy_size)))
...
{
	hwlog_info("[%s]\n", __func__);
	mutex_lock(&logbuff_open_mutex);
	if (is_opened) {
		hwlog_err("%s sensorhub logbuff already opened !\n", __func__);
		mutex_unlock(&logbuff_open_mutex);
		return -1;
	}
...
static void get_ddr_buff_size(uint32_t *log_buf_size, uint32_t *log_buff_copy_size)
{
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	*log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4: ddr buffer water mark */
	*log_buff_copy_size = (((*log_buf_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	*log_buf_size = DDR_LOG_BUFF_SIZE;
	*log_buff_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
}

static ssize_t sensorhub_logbuff_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int cnt, remain, error = 0;
	uint32_t ddr_log_buff_copy_size = 0, ddr_log_buf_size = 0;
	get_ddr_buff_size(&ddr_log_buf_size, &ddr_log_buff_copy_size);
	...
	error = wait_event_interruptible(sensorhub_log_waitq, is_new_data_available != 0);
	if (error) {
		error = 0;
		goto err;
	}
copy:
	...
	if (sensorhub_log_r > ddr_log_buf_size) {
		remain = 0;
		hwlog_err("%s sensorhub_log_r is too large\n", __func__);
	} else {
		remain = ddr_log_buf_size - sensorhub_log_r;
	}
	error = cnt;
	/* copy to user */
	if (cnt > remain) {
	...
	if (sensorhub_log_r + cnt > ddr_log_buf_size) {
		error = -EFAULT;
		goto out;
	}
	...
	sensorhub_log_r = (sensorhub_log_r + error) % ddr_log_buf_size;
...
static const struct proc_ops sensorhub_logbuff_operations = {
	.proc_open = sensorhub_logbuff_open,
	.proc_read = sensorhub_logbuff_read,
	.proc_release = sensorhub_logbuff_release,
};
#else
...
static int logbuff_full_callback(const struct pkt_header *head)
{
	...
	uint32_t ddr_log_buff_size;
	int ddr_log_buf_copy_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4:ddr buffer water mark */
	ddr_log_buf_copy_size = (((ddr_log_buff_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	ddr_log_buff_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buf_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
	cnt = ddr_log_buf_copy_size;
	...
	new_rear = (update_index + ddr_log_buf_copy_size) % ddr_log_buff_size;
	hwlog_info("[%s]\n", __func__);
	...
	/* update reader pointer */
	if (remain < ddr_log_buf_copy_size) {
...
static int logbuff_flush_callback(const struct pkt_header *head)
{
	...
	flush_size = (flush_head > sensorhub_log_buf_rear) ?
		...
		(ddr_log_buf_size - (sensorhub_log_buf_rear - flush_head));
	...
	/* get rotate log buff index */
	mutex_lock(&logbuff_mutex);
	remain = ddr_log_buf_size - sensorhub_log_buf_rear;
	...
	memcpy(local_log_buff + sensorhub_log_buf_rear,
		ddr_log_buff + sensorhub_log_buf_rear,
		flush_size);
	print_stat(6); /* stat 6 */
...
void reset_logbuff(void)
{
	...
	if (local_log_buff && ddr_log_buff) {
		memset(local_log_buff, 0, ddr_log_buf_size);
		memset(ddr_log_buff, 0, ddr_log_buf_size);
	}
}
...
{
	unsigned int ddr_log_buf_base_addr;
	unsigned int ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	ddr_log_buf_base_addr = dts_ddr_addr_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buf_base_addr = DDR_LOG_BUFF_ADDR_AP;
#endif
	ddr_log_buff = (uint8_t *)ioremap_wc(ddr_log_buf_base_addr, ddr_log_buf_size);
	if (!ddr_log_buff) {
...
int mloc_local_log_buff(void)
{
	...
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
#endif
	local_log_buff = (uint8_t *) vmalloc(ddr_log_buf_size);
	if (!local_log_buff) {
...
static void logbuff_init_success(void)
{
	...
	mutex_init(&logbuff_open_mutex);
	mutex_init(&logbuff_flush_mutex);
	inited = true;
	INIT_DELAYED_WORK(&sensorhub_logbuff_off_check_work,
		sensorhub_logbuff_off_check);
	queue_delayed_work(system_freezable_wq,
		&sensorhub_logbuff_off_check_work, 5 * 60 * HZ); // delay 300 Hz
	hwlog_info("[%s] done\n", __func__);
}

#ifdef CONFIG_INPUTHUB_30
static int shb_logbuff_recovery_notifier(struct notifier_block *nb,
...
static struct notifier_block shb_logbuff_recovery_notify = {
	.notifier_call = shb_logbuff_recovery_notifier,
	.priority = -1,
};
#endif
...
#ifdef CONFIG_INPUTHUB_30
	register_iom3_recovery_notifier(&shb_logbuff_recovery_notify);
#endif
	return 0;
MALLOC_ERR:
...
#ifdef CONFIG_INPUTHUB_30
	unregister_iom3_recovery_notifier(&shb_logbuff_recovery_notify);
#endif
