...
static int sensorhub_pm_resume(struct device *dev)
{
	...
	*flag = RESUME_INIT;
	...
	struct wakeup_config *wakeup = iomcu_config_get(IOMCU_CONFIG_USR_WAKEUP, sizeof(struct wakeup_config));
	if (wakeup->flag) {
		wakeup->flag = 0;
		...
		hwlog_err("recvfromapmsg [%x] [%x] [%x] [%x]\n",
			  wakeup->recvfromapmsg[0],
			  wakeup->recvfromapmsg[1],
			  wakeup->recvfromapmsg[2],
			  wakeup->recvfromapmsg[3]);
		hwlog_err("recvfromlpmsg [%x] [%x] [%x] [%x]\n",
			  wakeup->recvfromlpmsg[0],
			  wakeup->recvfromlpmsg[1],
			  wakeup->recvfromlpmsg[2],
			  wakeup->recvfromlpmsg[3]);
		hwlog_err("sendtoapmsg [%x] [%x] [%x] [%x]\n",
			  wakeup->sendtoapmsg[0],
			  wakeup->sendtoapmsg[1],
			  wakeup->sendtoapmsg[2],
			  wakeup->sendtoapmsg[3]);
		hwlog_err("sendtolpmsg [%x] [%x] [%x] [%x]\n",
			  wakeup->sendtolpmsg[0],
			  wakeup->sendtolpmsg[1],
			  wakeup->sendtolpmsg[2],
			  wakeup->sendtolpmsg[3]);
		hwlog_err("ap lpm3 tap tlpm3 %x %x %x %x\n",
			  wakeup->recvfromapmsgmode,
			  wakeup->recvfromlpmsgmode,
			  wakeup->sendtoapmsgmode,
			  wakeup->sendtolpmsgmode);
		hwlog_err("************ sensorhub has wrong wakeup mesg end\n");
		if (memset_s(&(wakeup), sizeof(struct wakeup_config),
			0, sizeof(struct wakeup_config)) != EOK)
