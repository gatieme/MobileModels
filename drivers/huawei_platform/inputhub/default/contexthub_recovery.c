 ...
 * Description: contexthub recovery source file
...
static void __get_dump_region_list(struct device_node *dump_node,
	const char *prop_name, enum dump_loc loc)
{
	int i;
	int cnt;
	...
	if (!prop || !prop->value) {
		...
		if (of_property_read_u32_array(dump_node, prop_name,
			...
			for (i = 0; i < cnt; i++) {
				dump->elements[g_dump_region_list[loc - 1][i]].loc = loc;
...
OUT:
	...
	struct dump_config *dump = iomcu_config_get(IOMCU_CONFIG_USR_DUMP, sizeof(struct dump_config));
	dump->enable = g_enable_dump;
}

static int write_ramdump_info_to_sharemem(void)
{
	uint32_t sensorhub_dump_buff_addr;
	uint32_t sensorhub_dump_size;
	uint32_t iomcu_config_start;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	iomcu_config_start = dts_ddr_addr_get(DTSI_DDR_CONFIG_ADDR_AP);
	sensorhub_dump_buff_addr = dts_ddr_addr_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	sensorhub_dump_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	sensorhub_dump_buff_addr = SENSORHUB_DUMP_BUFF_ADDR;
	sensorhub_dump_size = SENSORHUB_DUMP_BUFF_SIZE;
	iomcu_config_start = IOMCU_CONFIG_START;
#endif
	struct dump_config *dump = iomcu_config_get(IOMCU_CONFIG_USR_DUMP, sizeof(struct dump_config));
	if (dump == NULL) {
		hwlog_err("dump config is NULL, maybe ioremap %x failed\n",
			iomcu_config_start);
		return -1;
	}

	dump->finish = SH_DUMP_INIT;
	dump->dump_addr = sensorhub_dump_buff_addr;
	dump->dump_size = sensorhub_dump_size;
	...
	bbox->log_addr = current_sh_info.log_addr;
	bbox->log_size = current_sh_info.log_len;

	hwlog_warn("%s dumpaddr low:%x,dumpaddr high:%x,dumplen:%x,end\n",
		__func__, (u32)(dump->dump_addr),
		(u32)((dump->dump_addr) >> 32), /* 32:addr r_shift */
		dump->dump_size);
...
static int write_bbox_log_info_to_sharemem(struct rdr_register_module_result *rdr_info)
{
	struct bbox_config *bbox = iomcu_config_get(IOMCU_CONFIG_USR_BBOX, sizeof(struct bbox_config));
	if (bbox == NULL) {
		hwlog_err("write_bbox_log_info_to_sharemem: bbox config is NULL\n");
		return -1;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	ret = (int)ksys_chown((const char __user *)path, ROOT_UID, SYSTEM_GID);
#else
...
static int get_dump_reason_idx(void)
{
	...
	if (dump->reason >= ARRAY_SIZE(sh_reset_reasons))
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
		ksys_unlink(local_path); /* delete history.log */
#else
...
static int write_sh_dump_file(void)
{
	...
	if (g_sensorhub_extend_dump_buff) {
		...
		sh_savebuf2fs(g_dump_dir, path, g_sensorhub_extend_dump_buff,
			min(dump->ext_dump_size, dzh->len), 1);
	}
	return 0;
}
...
static void get_sh_dump_regs(void)
{
	...
	dp_regs = (uint32_t *)(g_sensorhub_dump_buff +
		sizeof(struct dump_zone_head) + sizeof(struct dump_zone_element_t) * dzh->cnt);

#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buff_size = SENSORHUB_DUMP_BUFF_SIZE;
#endif
	if ((uintptr_t)(dp_regs + MAGIC) > (uintptr_t)(g_sensorhub_dump_buff + ddr_log_buff_size)) {
...
static int rdr_sensorhub_init_early(void)
{
	int ret;
	uint32_t ddr_log_buff_size;
	uint32_t ddr_log_buff_base_addr;
	/* register module. */
...
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_base_addr = dts_ddr_addr_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buff_size = SENSORHUB_DUMP_BUFF_SIZE;
	ddr_log_buff_base_addr = SENSORHUB_DUMP_BUFF_ADDR;
#endif
	g_sensorhub_dump_buff = (uint8_t *)ioremap_wc(ddr_log_buff_base_addr, ddr_log_buff_size);
	if (!g_sensorhub_dump_buff) {
...
recovery_iom3:
	...
	rc = RPROC_ASYNC_SEND(get_ipc_ap_to_lpm_mbx(), &tx_buffer, 1);
	if (rc) {
	...
	rc = RPROC_SYNC_SEND(get_ipc_ap_to_iom_mbx(), &tx_buffer, 1,
		&ack_buffer, 1);
	if (rc) {
...
void ipc_dbg_record(uint8_t dir, struct pkt_header *head, const char *info)
{
	int len;
	...
	pmsg->dir = dir;
#ifdef CONFIG_DFX_BB
	pmsg->clock = dfx_getcurtime();
#endif
	memcpy(&pmsg->hd, head, sizeof(struct pkt_header));
	...
	if (old_state == IOM3_RECOVERY_IDLE) {
		...
		if (g_enable_dump && g_dump_extend_size &&
			...
			if (g_sensorhub_extend_dump_buff) {
				dump->ext_dump_addr =
					virt_to_phys(g_sensorhub_extend_dump_buff);
				dump->ext_dump_size = g_dump_extend_size;
				barrier();
			}
		}
