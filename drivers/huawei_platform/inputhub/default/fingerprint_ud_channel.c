...
#define FHB_CONFIG_SET_DARKENV_PARAM 22
...
static uint8_t g_darkenv_param[4];
static uint8_t g_darkenv_param_len;
...
static void enable_fingerprint_when_recovery_iom3(void)
{
	...
	if (g_darkenv_param[0] == FHB_CONFIG_SET_DARKENV_PARAM)
		fingerprint_recovery_config_para(g_darkenv_param, g_darkenv_param_len);
	...
	size_t len, loff_t *ppos)
{
	...
	if (fp_pkt.buf[0] == FHB_CONFIG_SET_DARKENV_PARAM) {
		g_darkenv_param_len = minimum(len, sizeof(g_darkenv_param));
		memcpy_s(g_darkenv_param, sizeof(g_darkenv_param), fp_pkt.buf, g_darkenv_param_len);
	}
