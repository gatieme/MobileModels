...
#define I2C_ERROR_RESET_TIME_24H    (24 * 3600 * 1000000000) // 1h = 3600s; 1s = 10^9 ns

static void sensor_i2c_nack_recovery_mechanism(const char *temp)
{
	uint64_t total_time;
	static uint64_t start_time;
	static int reset_sensorhub_times = 0;
#ifdef CONFIG_HUAWEI_DSM
	struct dsm_client *client = inputhub_get_shb_dclient();
	dsm_client_record(client, "dmd_case = 0x%x", CONTINUES_I2C_ERROR);
	dsm_client_notify(client, DSM_SHB_ERR_SENSORSERVICE_DATA_UPDATE);
#endif
	if (reset_sensorhub_times == 0) {
		// Get timestamp when first coming in
		start_time = ktime_get_boottime_ns();
		hwlog_info("sensor i2c nack first start %llu\n", start_time);
	} else {
		// Except for the first time, get a new timestamp.
		total_time = ktime_get_boottime_ns();
		hwlog_info("sensor i2c nack start %llu, end %llu\n", start_time, total_time);
		// The timestamp is reset or the timestamp is updated after 24 hours.
		if ((total_time <= start_time) ||
			(total_time - start_time) >= I2C_ERROR_RESET_TIME_24H) {
			start_time = total_time;
			reset_sensorhub_times = 0;
			hwlog_info("sensor i2c nack time end\n");
		}
	}
	reset_sensorhub_times++;
	hwlog_err("i2c continuous read error times:%d,i2c_error:%d\n",
		reset_sensorhub_times, ((int *)temp)[MCU_RESET_IDX]);
	if (reset_sensorhub_times <= I2C_ERROR_RESET_SENSORHUB_TIMES)
...
int write_gyro1_sensor_offset_to_nv(const char *temp, int length)
{
	...
	if (((int *)temp)[MCU_RESET_IDX] == CONTINUES_I2C_ERROR) {
		sensor_i2c_nack_recovery_mechanism(temp);
		return 0;
	}
