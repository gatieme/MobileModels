...
#define CURRENT_CHARLOTTE_MAX_VALUE 20000
#define CURRENT_CHARLOTTE_MIN_VALUE 2000
...
int parameter_value_assignment(int current_value_now)
{
	static int pre_val = 0;
	if (current_value_now < CURRENT_CHARLOTTE_MIN_VALUE ||
		current_value_now > CURRENT_CHARLOTTE_MAX_VALUE)
		return SEND_ERROR;
	if (current_value_now != pre_val) {
		...
		if (memcpy_s(charge_current_data.str_charge,
			...
			return SEND_SUC;
		struct mag_compensation_val *offset_val = NULL;
		offset_val = mag_get_compensation();
		if (offset_val == NULL)
			return SEND_SUC;
		charge_current_data.current_offset_x =
			(offset_val->charger_x_map == 0) ? (int)offset_val->charger_x_val : -(int)offset_val->charger_x_val;
		charge_current_data.current_offset_y =
			(offset_val->charger_y_map == 0) ? (int)offset_val->charger_y_val : -(int)offset_val->charger_y_val;
		charge_current_data.current_offset_z =
			(offset_val->charger_z_map == 0) ? (int)offset_val->charger_z_val : -(int)offset_val->charger_z_val;
		charge_current_data.current_value = current_value_now;
		send_mag_charger_to_mcu();
		pre_val = current_value_now;
	}
	return SEND_SUC;
}
#endif
int send_current_to_mcu_mag(int current_value_now)
{
...
#ifdef CHA_MAG_DETACH
	parameter_value_assignment(current_value_now);
#else
	...
	if ((dev_info->akm_current_x_fac == 0xFF) &&
		...
		(dev_info->akm_current_z_fac == 0xFF) && get_mag_opened()) {
			...
			unsigned long action, void *data)
{
	...
	if (dev_info == NULL)
		return 0;
#ifdef CHA_MAG_DETACH
	if (action < CHARGER_TYPE_MAG_NONE)
		mag_charge_notify_open();
	else if (action >= CHARGER_TYPE_MAG_NONE)
