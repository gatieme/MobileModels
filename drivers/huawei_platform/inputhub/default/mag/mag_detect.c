...
static const char *str_one_soft_para = "softiron_one_parameter";
...
struct mag_compensation_val mag_com_val;
struct mag_compensation_val* mag_get_compensation()
{
	return &mag_com_val;
}
#endif
...
#ifdef CHA_MAG_DETACH
static void read_mag_config3_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct compass_platform_data *pf_data =
		(struct compass_platform_data *)sm->spara;
	int temp = 0;
	if (of_property_read_u32(dn, "fold_type", &temp))
		hwlog_err("%s:read %s mag fold_phone_type fail\n",
			__func__, sm->sensor_name_str);
	else
		pf_data->fold_type = (uint8_t)temp;

	if (of_property_read_u32(dn, "charger_x_val", &temp))
		hwlog_err("%s:read %s mag charger_x_val fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_x_val = (uint8_t)temp;
	if (of_property_read_u32(dn, "charger_x_map", &temp))
		hwlog_err("%s:read %s mag charger_x_map fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_x_map = (uint8_t)temp;
	if (of_property_read_u32(dn, "charger_y_val", &temp))
		hwlog_err("%s:read %s mag charger_y_val fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_y_val = (uint8_t)temp;
	if (of_property_read_u32(dn, "charger_y_map", &temp))
		hwlog_err("%s:read %s mag charger_y_map fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_y_map = (uint8_t)temp;
	if (of_property_read_u32(dn, "charger_z_val", &temp))
		hwlog_err("%s:read %s mag charger_z_val fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_z_val = (uint8_t)temp;
	if (of_property_read_u32(dn, "charger_z_map", &temp))
		hwlog_err("%s:read %s mag charger_z_map fail\n",
			__func__, sm->sensor_name_str);
	else
		mag_com_val.charger_z_map = (uint8_t)temp;
}
#endif
	...
	struct sensor_detect_manager *sm)
{
...
#ifdef CHA_MAG_DETACH
	read_mag_config3_from_dts(dn, sm);
#endif
	...
	if (ret)
		hwlog_err("%s:fill_extend_data_in_dts fail\n",
			str_soft_para);
#ifdef CHA_MAG_DETACH
	ret = fill_extend_data_in_dts(dn, str_one_soft_para,
		pf_data->pdc_one_data, PDC_SIZE,
		EXTEND_DATA_TYPE_IN_DTS_BYTE);
	if (ret)
		hwlog_err("%s:fill_extend_data_in_dts fail\n",
			str_one_soft_para);
...
static int set_far_softiron(int32_t tag, const unsigned char *input, int len)
{
	...
	ret = memcpy_s(cpkt.para, sizeof(cpkt.para), gyro_args_to_mcu,
		sizeof(int) * SOFTIRON_ARGS_NUM * 2); /* multi 2 to uint8 */
	if (ret != EOK) {
