...
static int get_device_index(enum sensor_detect_list sname)
{
	if (sname == PS)
		return PS_DEVICE_ID_0;
	else if (sname == PS1)
		return PS_DEVICE_ID_1;
...
struct ps_device_info *ps_get_device_info(int32_t tag)
{
	if (tag == TAG_PS)
		...
		return (&(g_ps_dev_info[PS_DEVICE_ID_1]));
...
static void read_ps_chip_info(enum sensor_detect_list sname)
{
	struct ps_device_info *dev_info;
	char *chip_info = get_sensor_chip_info_address(sname);
	int index;

	index = get_device_index(sname);
	dev_info = &(g_ps_dev_info[index]);
...
{
	struct ps_platform_data *pf_data;
	struct ps_device_info *dev_info;
	int temp = 0;
	int index;

	index = get_device_index(sm->sensor_id);
	dev_info = &(g_ps_dev_info[index]);
	pf_data = &(ps_data[index]);
	...
	struct sensor_detect_manager *sm)
{
	...
	if (sm->sensor_id == PS) {
		if (replace_ps_type == TP_REPLACE_PS) {
			...
			info = get_sensorlist_info_by_index(sm->sensor_id);
	...
	read_ps_chip_info(sm->sensor_id);
	read_ps_config1_from_dts(dn, sm);
	read_ps_config2_from_dts(dn, sm);
	read_ps_config3_from_dts(dn, sm);
	read_ps_config4_from_dts(dn, sm);
	read_ps_config5_from_dts(dn, sm);
	read_ps_config6_from_dts(dn, sm);
	read_ps_config7_from_dts(dn, sm);
...
void ps_detect_init(struct sensor_detect_manager *sm, uint32_t len)
{
	...
	g_ps_dev_info[PS_DEVICE_ID_1].obj_tag = TAG_PS1;
	g_ps_dev_info[PS_DEVICE_ID_1].detect_list_id = PS1;
	for (i = 0; i < PS_DEV_COUNT_MAX; i++) {
		g_ps_dev_info[i].ps_dev_index = i;
		p = sm + g_ps_dev_info[i].detect_list_id;
		p->spara = (const void *)&(ps_data[i]);
		p->cfg_data_length = sizeof(ps_data[i]);
		if (i == 0)
			continue;
		if (memcpy_s(&(ps_data[i]), sizeof(ps_data[i]),
			&(ps_data[0]), sizeof(ps_data[0])) != EOK)
