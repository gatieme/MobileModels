...
static void ps_test_name_join_with_device(enum sensor_detect_list index)
{
	...
	/* ps sensorlist name's format should be xxx-xxx */
	ret = sscanf_s(sensorlist_info_ps->name, "%*[^-]-%s", device_name,
		PS_DEVICE_NAME_MAX_LEN - 1);
	if (ret < 0) {
		hwlog_err("%s incorrect device name format\n", __func__);
		return;
	}
	hwlog_info("%s get device name: %s\n", __func__, device_name);
	...
	for (i = 0; i < PS_CAL_JOIN_NUM; i++) {
		...
		ret = strncat_s(ps_test_with_device_name[i],
			sizeof(ps_test_with_device_name), device_name, strlen(device_name));
		if (ret)
			hwlog_err("%s strncat fail %d\n", __func__, i);
