 ...
 * Description: sensor channel source file
...
static const struct sensors_cmd_map sensors_cmd_map_tab[] = {
	...
	{ SENSORHUB_TYPE_CARCRASH, TAG_CARCRASH },
...
static void init_hash_tables(void)
{
	...
	for (i = 0; i < sizeof(sensors_cmd_map_tab) / sizeof(sensors_cmd_map_tab[0]); ++i) {
		tag_type[sensors_cmd_map_tab[i].tag] =
...
static bool ap_sensor_flush(int tag)
{
	...
	bool work_on_ap = record[tag].work_on_ap;
	uint8_t *tag_type = get_tag_to_hal_sensor_type();

	if (work_on_ap) {
		if (record[tag].ops.enable) {
			record[tag].ops.enable(1);
			msleep(60); /* sleep 60 ms */
		}
		event.type = tag_type[TAG_FLUSH_META];
		event.length = 4;  /* 4 : sensor data length */
		event.value[0] = tag_type[tag];
...
{
	if (tag != TAG_ORIENTATION || para->flags != 1)
		return;
	int buf = SUB_CMD_BC_SWITCH_REQ;
	struct write_info wr;
	wr.tag = TAG_ORIENTATION;
	wr.cmd = CMD_CMN_CONFIG_REQ;
	wr.wr_buf = (const void *)(&buf);
	wr.wr_len = sizeof(int);
	write_customize_cmd(&wr, NULL, false);
}
...
static void get_acc1_calibrate_data(void)
{
	...
	if (strlen(get_sensor_chip_info_address(ACC1)) == 0)
		return;
...
static void get_vibrator_calibrate_data(void)
{
	...
	if (strlen(get_sensor_chip_info_address(VIBRATOR)) == 0)
		return;
...
static void get_mag1_calibrate_data(void)
{
	...
	if (strlen(get_sensor_chip_info_address(MAG1)) == 0)
		return;
...
static void get_tof_calibrate_data(void)
{
	struct tof_device_info *tof_dev_info = NULL;
	int ret;

	tof_dev_info = tof_get_device_info(TAG_TOF);
	if (tof_dev_info->tof_first_start_flag == 0 && tof_dev_info->tof_enable_flag == 1) {
		hwlog_info("[%s-%d]:send_tof_calibrate_data_to_mcu\n", __func__, __LINE__);
		ret = send_tofp_calibrate_data_to_mcu();
		if (ret < 0)
			hwlog_err("[%s-%d]:send_tof_calibrate_data_to_mcu fail\n",
				__func__, __LINE__);
		else
			hwlog_info("[%s-%d]:send_tof_calibrate_data_to_mcu succ\n",
				__func__, __LINE__);
	}
}
...
static void get_als1_under_tp_calibrate_data(void)
{
	get_als_under_tp_calibrate_data_by_tag(TAG_ALS1);
}
...
static void get_handpress_calibrate_data(void)
{
	...
	if (strlen(get_sensor_chip_info_address(HANDPRESS)) == 0)
		return;
	if (get_handpress_first_start_flag() == 0) {
...
static void config_sensors_when_recovery_iom3(void)
{
	int tag;
	int screen_state;
	...
	for (tag = TAG_SENSOR_BEGIN; tag < TAG_SENSOR_END; ++tag) {
		if (sensor_status.status[tag]) {
			if (tag == TAG_POSTURE) {
				screen_state = get_last_screen_state();
				tell_screen_status_to_mcu(screen_state);
			}
		}
	}
}
