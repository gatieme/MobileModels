...
static int support_preventive_posture_hall;
...
static uint32_t support_hall_fold_value;
...
static uint32_t ext_hall_func_comp_val[MAX_VAL];
...
struct sensor_detect_manager sensor_manager[SENSOR_MAX] = {
	...
	{ "drop", DROP, DET_INIT, TAG_DROP, &g_drop_data,
		sizeof(g_drop_data) },
	{ "acc2", ACC2, DET_INIT, TAG_ACC2, NULL, 0 },
	{ "gyro2", GYRO2, DET_INIT, TAG_GYRO2, NULL, 0 },
};
...
static const struct app_link_info app_link_info_gyro[] = {
	...
	{ SENSORHUB_TYPE_ACCELEROMETER2, TAG_ACC2, 1, { TAG_ACC2 } },
	{ SENSORHUB_TYPE_GYROSCOPE2, TAG_GYRO2, 0, { 0 } },
	{ SENSORHUB_TYPE_DEBUGS, TAG_DEBUGS, 0, { 0 } },
	{ SENSORHUB_TYPE_AUDIO_FUSION, TAG_AUDIO_FUSION, 0, { 0 } },
};
...
int get_support_preventive_posture_hall(void)
{
	return support_preventive_posture_hall;
}
...
uint32_t get_ext_hall_type(void)
{
	return support_hall_fold_value;
}

uint32_t get_ext_hall_bit_val(int bit_index)
{
	if (bit_index < 0 || bit_index >= MAX_VAL) {
		hwlog_info("bit_index val error, val is %d\n", bit_index);
		return 0;
	}
    return ext_hall_func_comp_val[bit_index];
}
...
static int get_hall_config_from_dts(void)
{
	...
	/* get hall fold status */
	if (!of_property_read_u32(sensorhub_node, "support_ext_hall_value_report", &i)) {
		support_hall_fold_value = i;
		hwlog_info("get support_hall_fold_value %u\n", support_hall_fold_value);
	}

	/* get_ext_hall_bit_config */
	if (!of_property_read_u32(sensorhub_node, "fold_hall_comp_val", &i)) {
		ext_hall_func_comp_val[FOLD_HALL] = i;
		hwlog_info("get fold_hall_comp_val %u\n", ext_hall_func_comp_val[FOLD_HALL]);
	}

	if (!of_property_read_u32(sensorhub_node, "nfc_theme_shell_val", &i)) {
		ext_hall_func_comp_val[NFC_THEME_SHELL] = i;
		hwlog_info("get nfc_theme_shell_val %u\n", ext_hall_func_comp_val[NFC_THEME_SHELL]);
	}

	if (!of_property_read_u32(sensorhub_node, "protect_shell_val", &i)) {
		ext_hall_func_comp_val[PROTECT_CASE] = i;
		hwlog_info("get protect_shell_val %u\n", ext_hall_func_comp_val[PROTECT_CASE]);
	}

	if (!of_property_read_u32(sensorhub_node, "ext_fold_hall_val", &i)) {
		ext_hall_func_comp_val[EXT_FOLD_HALL] = i;
		hwlog_info("get ext_fold_hall_val %u\n", ext_hall_func_comp_val[EXT_FOLD_HALL]);
	}
	...
	if (!of_property_read_u32(sensorhub_node, "is_support_preventive_posture_hall", &i)) {
		support_preventive_posture_hall = i;
		hwlog_info("sensor get support_posture_hall: %d\n", support_preventive_posture_hall);
	}
...
static int get_sensors_id_from_dts(void)
{
	...
	ps_get_sensors_id_from_dts(sensorhub_node);
...
static void set_f03015_config(struct fp_gpio_config *gpio_cfg)
{
	struct f03_reg_info reg_info[] = {
	{0xE500, 0x0000, 1},
	{0x00C2, 0x0B20, 0},
...
static const struct fp_sensor_config g_fp_ud_cfgs[] = {
	...
	{ "F03015", FP_IC_MATCH_LEN, set_f03015_config },
...
__weak const int get_tskit_register_status(void)
{
	return 0;
}

__weak const char *ts_kit_get_vendor_name(u32 panel_id) { return NULL; }
	...
	const char *configed_vendor_name)
{
	...
	attached_vendor_name = ts_kit_get_vendor_name(MAIN_TOUCH_PANEL);
#else
...
static void tp_ud_wait_tp_driver_register(void)
{
	int retry_count = 400;

	while (retry_count--) {
		if (!get_thp_unregister_ic_num()) {
			hwlog_info("%s:tp driver have registered\n", __func__);
			break;
		}
		mdelay(5);
	}
...
#ifdef CONFIG_HUAWEI_TS_KIT_3_0
	attached_vendor_name = ts_kit_get_vendor_name(panel_id);
#else
	...
	struct tp_ud_platform_data *tp_data_config)
{
	...
	if (of_property_read_u32(dn, "support_tp_register_ps", &temp)) {
		hwlog_info("%s:read support_tp_register_ps not config\n", __func__);
		tp_data_config->support_tp_register_ps = 0;
	} else {
		tp_data_config->support_tp_register_ps = (uint16_t)temp;
		hwlog_info("%s:read support_tp_register_ps = %u\n",
			__func__, tp_data_config->support_tp_register_ps);
	}
	if (of_property_read_u32(dn, "syna_enter_exit_lowpower", &temp)) {
		hwlog_info("%s:read syna_enter_exit_lowpower not config\n", __func__);
		tp_data_config->syna_enter_exit_lowpower = 0;
	} else {
		tp_data_config->syna_enter_exit_lowpower = (uint16_t)temp;
		hwlog_info("%s:read syna_enter_exit_lowpower = %u\n",
			__func__, tp_data_config->syna_enter_exit_lowpower);
	}
}
...
static void read_thp_features_config_from_dts(struct device_node *dn, struct thp_platform_data *thp_config)
{
	thp_config->gestures.support_fp_ud = (uint8_t)tp_parse_property_u32(dn, "support_fp_ud");
	thp_config->gestures.support_virtual_key = (uint8_t)tp_parse_property_u32(dn, "support_virtual_key");
	thp_config->gestures.support_double_click = (uint8_t)tp_parse_property_u32(dn, "support_double_click");
	thp_config->gestures.support_hw_m_pen = (uint8_t)tp_parse_property_u32(dn, "support_hw_m_pen");
	thp_config->anti_forgery_support = (uint16_t)tp_parse_property_u32(dn, "anti_forgery_support");
	thp_config->support_tp_register_ps = (uint16_t)tp_parse_property_u32(dn, "support_tp_register_ps");
	thp_config->feature_config = (uint16_t)tp_parse_property_u32(dn, "feature_config");
}

static void read_thp_bus_config_from_dts(struct device_node *dn, struct thp_platform_data *thp_config)
{
	thp_config->spi_max_speed_hz = tp_parse_property_u32(dn, "spi_max_speed_hz");
	thp_config->spi_mode = (uint8_t)tp_parse_property_u32(dn, "spi_mode");
	thp_config->gpio_irq = (uint16_t)tp_parse_property_u32(dn, "gpio_irq");
	thp_config->gpio_irq_sh = (uint16_t)tp_parse_property_u32(dn, "gpio_irq_sh");
	thp_config->gpio_cs = (uint16_t)tp_parse_property_u32(dn, "gpio_cs");
	thp_config->ic_type = tp_parse_property_u32(dn, "ic_type");
	thp_config->i2c_max_speed_hz = tp_parse_property_u32(dn, "i2c_max_speed_hz");
	thp_config->tp_sensorhub_irq_flag = (uint16_t)tp_parse_property_u32(dn, "tp_sensorhub_irq_flag");
	thp_config->hover_enable = tp_parse_property_u32(dn, "hover_enable");
}

static void read_thp_fw_config_from_dts(struct device_node *dn, struct thp_platform_data *thp_config)
{
	thp_config->fw_power_config_reg = (uint16_t)tp_parse_property_u32(dn, "fw_power_config_reg");
	thp_config->fw_touch_data_reg = (uint16_t)tp_parse_property_u32(dn, "fw_touch_data_reg");
	thp_config->fw_touch_command_reg = (uint16_t)tp_parse_property_u32(dn, "fw_touch_command_reg");
	thp_config->fw_addr_3 = (uint16_t)tp_parse_property_u32(dn, "fw_addr_3");
	thp_config->fw_addr_4 = (uint16_t)tp_parse_property_u32(dn, "fw_addr_4");
	thp_config->fw_addr_5 = (uint16_t)tp_parse_property_u32(dn, "fw_addr_5");
	thp_config->fw_addr_6 = (uint16_t)tp_parse_property_u32(dn, "fw_addr_6");
	thp_config->fw_addr_7 = (uint16_t)tp_parse_property_u32(dn, "fw_addr_7");
}
...
static void read_thp_from_dts(struct device_node *dn, enum sensor_detect_list sensor_id)
{
	...
	read_thp_features_config_from_dts(dn, thp_config);
	read_thp_bus_config_from_dts(dn, thp_config);
	read_thp_fw_config_from_dts(dn, thp_config);
...
static int detect_i2c_device_rx_len(struct device_node *dn, char *device_name)
{
	uint32_t rx_len;
	uint8_t i2c_detect_short_type = 0;

	if (of_property_read_u8(dn, "i2c_detect_short_type", &i2c_detect_short_type) == 0)
		hwlog_info("%s: success read i2c_detect_short_type:0x%x\n", device_name, i2c_detect_short_type);

	if (i2c_detect_short_type)
		rx_len = 2; /* rx len 2 */
	else
		rx_len = 4; /* rx len 4 */
	hwlog_info("detect_i2c_device_rx_len:%d\n", rx_len);
	return rx_len;
}
...
static int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	if (tag == TAG_I3C) {
		...
		if ((unsigned int)register_add & 0xFF00) {
			register_add_len = 2; /* address len 2 */
			rx_len = detect_i2c_device_rx_len(dn, device_name);
		} else {
			register_add_len = 1;
			rx_len = 1;
		}
...
static int get_hall_gpio_id_from_dts(void)
{
	...
	if (temp[1] == 4) { // 4: phone_type_info is GRL
		if (of_property_read_u32(sensorhub_node, "hall_gpio_id2", &hall_gpio_id2) != 0) {
			hwlog_err("read hall_gpio_id2 from dts fail\n");
		} else {
			motion->hall_gpio_id2 = hall_gpio_id2;
			hwlog_info("%s: hall_gpio_id2 is %d\n", __func__, motion->hall_gpio_id2);
		}
	}
...
static void init_sensors_cfg_data_each_node(void)
{
	...
	if (get_fix_2_i2c() == FIX_2_I2C_SENSOR_DEFAULT)
		set_fix_2_i2c_to_no_grl_dedicated_board();
}
	...
	if (ret) {
		...
		if (pkg_mcu.errno != 0) {
			...
			snprintf(buf, 50, "set %s cfg to mcu success\n",
				sensor_manager[s_id].sensor_name_str);
			hwlog_info("%s\n", buf);
			if (get_fix_2_i2c() == FIX_2_I2C_SENSOR_NEED && pkg_ap.tag == TAG_ACCEL)
				set_fix_2_i2c_2_finishsed();
...
static void register_priv_notifier(enum sensor_detect_list s_id)
{
	switch (s_id) {
	case ACC:
		register_mcu_event_notifier(TAG_ACCEL, CMD_CMN_CONFIG_REQ, acc_data_from_mcu);
		break;
	case ACC1:
		register_mcu_event_notifier(TAG_ACC1, CMD_CMN_CONFIG_REQ, acc1_data_from_mcu);
		break;
	case ACC2:
		register_mcu_event_notifier(TAG_ACC2, CMD_CMN_CONFIG_REQ, acc2_data_from_mcu);
		break;
	case GYRO:
	...
	case GYRO1:
		...
		break;
	case GYRO2:
		register_mcu_event_notifier(TAG_GYRO2, CMD_CMN_CONFIG_REQ,
			gyro2_data_from_mcu);
		break;
	case MAG1:
...
int sensor_set_cfg_data(void)
{
	...
	for (s_id = ACC; s_id < SENSOR_MAX; s_id++) {
		if (s_id == PS) {
			...
			ps_set_foldtype_to_mcu(TAG_PS);
			ps_send_not_replaced_to_mcu(TAG_PS);
		}
