 ...
 * Description: sensor feima source file
...
#define MAX_SELFTEST_CMD 5

struct class *sensors_class;
unsigned long sleeve_test_enabled;

static bool rpc_motion_request;
static time64_t get_data_last_time;
static unsigned long sar_service_info;
static unsigned long rpc_sar_service_req_ex_info;
...
static int rpc_status_change(void)
{
	...
	sar_service_info = (sar_service_info & ~BIT(9)) | /* clean bit 9 */
		((unsigned long)rpc_motion_request << 9); /* set bit 9 */
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, TO_DECIMALISM, &value))
	...
	if (value == AUDIO_SPEAKER_ON || value == AUDIO_SPEAKER_OFF)
		motion_speaker_status_change(value);

	if (value == MOTION_FINGER_UP || value == MOTION_FINGER_DOWN)
		motion_finger_status_change(value);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, TO_DECIMALISM, &sar_service))
...
static ssize_t store_rpc_sar_service_req_ex(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long rpc_sar_service_req_ex = 0;
	unsigned int cmd;
	unsigned int sub_cmd;
	unsigned int ret;

	if (kstrtoul(buf, TO_DECIMALISM, &rpc_sar_service_req_ex))
		hwlog_err("rpc_sar_service_req_ex strout error\n");

	hwlog_info("%s: store_rpc_sar_service_req_ex val (%lu), buf is %s\n",
		__func__, rpc_sar_service_req_ex, buf);

	rpc_sar_service_req_ex_info = rpc_sar_service_req_ex;
	rpc_sar_service_req_ex_info = (rpc_sar_service_req_ex_info & ~BIT(9)) | /* clean bit 9 */
		(1 << 9); /* set bit 9 */
	hwlog_info("store_rpc_sar_service_req_ex  rpc_get_sleep_mode_info is %lu\n", rpc_sar_service_req_ex_info);
	struct write_info pkg_ap = { 0 };
	rpc_ioctl_t pkg_ioctl;

	pkg_ap.tag = TAG_RPC;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ioctl.sub_cmd = SUB_CMD_RPC_GET_SLEEP_MODE_REQ;
	pkg_ioctl.sar_info = rpc_sar_service_req_ex_info;
	pkg_ap.wr_buf = &pkg_ioctl;
	pkg_ap.wr_len = sizeof(pkg_ioctl);
	...
	hwlog_info("rpc_sar_service_req_ex end\n");
	return size;
}
...
{
	struct sensor_cookie *data = (struct sensor_cookie *)dev_get_drvdata(dev);
	uint8_t *tag_type = get_tag_to_hal_sensor_type();
	unsigned int hal_sensor_tag = tag_type[data->tag];
	...
	if (wait_for_completion_interruptible(&get_data->complete) == 0) {
		...
		for (; i < mem_num; i++) {
			...
			offset += (ssize_t)sprintf(buf + offset, "%d\t",
				get_data->data.value[i]);
		}
		offset += (ssize_t)sprintf(buf + offset, "\n");
	}
	return offset;
}
...
static ssize_t show_orientation_set_data_mode(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data = (struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return snprintf(buf, MAX_STR_SIZE, "%d\n",
		sensor_status.orientation_set_mode);
}

static ssize_t store_orientation_set_data_mode(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int ret;
	uint32_t errno;
	...
	struct read_info rd;
	orientation_set_mode_ioctl_t ori_ioctl_t;

	CHECK_SENSOR_COOKIE(data);
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;

	cpkt.hd.tag = (unsigned char)data->tag;
	cpkt.hd.cmd = CMD_CMN_CONFIG_REQ;
	cpkt.subcmd = SUB_CMD_ORIENTATION_CONFIG_REQ;
	cpkt.hd.resp = RESP;
	cpkt.hd.length = SUBCMD_LEN;

	wr.tag = cpkt.hd.tag;
	wr.cmd = cpkt.hd.cmd;
	ori_ioctl_t.sub_cmd = cpkt.subcmd;
	ori_ioctl_t.config_val = val;
	wr.wr_buf = &ori_ioctl_t;
	wr.wr_len = sizeof(ori_ioctl_t);
	ret = write_customize_cmd(&wr, &rd, false);
	if (ret != 0) {
		hwlog_err("%s orientation_set_mode %d failed, ret = %d\n",
			data->name, val, ret);
		return ret;
	}
	errno = (uint32_t)rd.errno;
	sensor_status.orientation_set_mode = val;
	hwlog_info("%s orientation_set_mode %d, ret %d, errno %d\n",
		data->name, val, ret, errno);
	return size;
}

#ifdef CONFIG_SENSOR_DIRECT_CHANNEL
static ssize_t show_sdc_config(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data = (struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return snprintf(buf, MAX_STR_SIZE, "%s\n",
		sensor_status.sdc_fac_disable_result[data->tag]);
}

static ssize_t store_sdc_config(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int ret;
	uint32_t errno;
	...
	struct write_info wr;
	struct read_info rd;
	sdc_ioctl_t sdc_ioctl;

	CHECK_SENSOR_COOKIE(data);
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;
	if (val != 0 && val != 1) { // 1 is sdc disable = true, 0 is disable=false
		hwlog_err("sdc config fail, val:%d is err\n", val);
		return -EINVAL;
	}
	cpkt.hd.tag = (unsigned char)data->tag;
	cpkt.hd.cmd = CMD_CMN_CONFIG_REQ;
	cpkt.subcmd = SUB_CMD_SDC_FAC_DISABLE_REQ;
	cpkt.hd.resp = RESP;
	cpkt.hd.length = SUBCMD_LEN;

	wr.tag = cpkt.hd.tag;
	wr.cmd = cpkt.hd.cmd;
	sdc_ioctl.sub_cmd = cpkt.subcmd;
	sdc_ioctl.sdc_config = val;
	wr.wr_buf = &sdc_ioctl;
	wr.wr_len = sizeof(sdc_ioctl);
	ret = write_customize_cmd(&wr, &rd, false);
	errno = (uint32_t)rd.errno;
	memcpy_s(sensor_status.sdc_fac_disable_result[data->tag],
		SDC_CONFIG_MAXLEN, buf, 2); // 2 is buf size
	hwlog_info("%s sdc fac disable %d, ret %d, errno %d\n", data->name, val, ret, errno);
	return size;
}
#endif
...
static int store_selftest_combine_cmd_hub30(pkt_subcmd_req_t cpkt,
			struct read_info *rd, unsigned long val)
{
	int ret;
	struct write_info wr;
	selftest_ioctl_t selftest_ioctl;

	wr.tag = cpkt.hd.tag;
	wr.cmd = cpkt.hd.cmd;
	selftest_ioctl.sub_cmd = cpkt.subcmd;
	selftest_ioctl.selftest_code = val;
	wr.wr_buf = &selftest_ioctl;
	wr.wr_len = sizeof(selftest_ioctl);
	ret = write_customize_cmd(&wr, rd, false);
	return ret;
}
#endif
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;
	hwlog_err("%s selftest code %d\n", __func__, val);
	if (val <= MAX_SELFTEST_CMD) {
		cpkt.hd.tag = (unsigned char)data->tag;
		cpkt.hd.cmd = CMD_CMN_CONFIG_REQ;
...
#ifndef CONFIG_INPUTHUB_30
		{
			...
			ret = wait_for_mcu_resp_data_after_send(&cpkt,
				...
				4000, &resp_pkt, sizeof(resp_pkt)); /* 4000 ms */
			errno = resp_pkt.errno;
		}
#else

		ret = store_selftest_combine_cmd_hub30(cpkt, &rd, val);
		ret = !ret;
		errno = (uint32_t)rd.errno;
#endif
		if (ret == 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	size = (MAX_STR_SIZE > (sizeof(*data) * FINGERSENSE_DATA_NSAMPLES)) ?
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((vibrator_shake == 1) ||
		((HALL_COVERD & (unsigned int)hall_value) &&
		(get_hall_sen_type() == 0))) {
...
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL
static DEVICE_ATTR(sdc_config_disable, 0660, show_sdc_config, store_sdc_config);
#endif
...
static DEVICE_ATTR(rpc_sar_service_req_ex, 0660, NULL, store_rpc_sar_service_req_ex);
...
static DEVICE_ATTR(cap_prox_sensorlist_info, 0664, cap_prox_sensorlist_info_show, NULL);
...
static DEVICE_ATTR(orientation_set_data_mode, 0660,
	show_orientation_set_data_mode, store_orientation_set_data_mode);
	...
	struct device_attribute *attr, char *buf)
{
	...
	CHECK_SENSOR_COOKIE(data);
	return als_calibrate_dbv_sec_show(data->tag, dev, attr, buf);
}

static DEVICE_ATTR(als_calibrate_dbv_sec, 0664,
...
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL
	&dev_attr_sdc_config_disable.attr,
#endif
...
static struct attribute *ps1_sensor_attrs[] = {
	&dev_attr_get_data.attr,
	&dev_attr_calibrate.attr,
	&dev_attr_calibrate_timeout.attr,
	&dev_attr_sleeve_test_prepare.attr,
...
static const struct attribute_group ps1_sensor_attrs_grp = {
	.attrs = ps1_sensor_attrs,
};
...
static struct attribute *orientation_sensor_attrs[] = {
	&dev_attr_get_data.attr,
	&dev_attr_orientation_set_data_mode.attr,
	NULL,
};
...
static struct attribute *rpc_sensor_attrs[] = {
	...
	&dev_attr_rpc_sar_service_req_ex.attr,
	NULL,
};
...
static const struct attribute_group g_hinge_sensor_attrs_grp = {
	.attrs = hinge_sensor_attrs,
};

static ssize_t carcrash_signal_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int ret;
	unsigned long val = 0;
	struct sensor_cookie *data = (struct sensor_cookie *)dev_get_drvdata(dev);
	struct write_info wr;
	carcrash_ioctl_t carcrash_ioctl;

	CHECK_SENSOR_COOKIE(data);
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;
	if (val != 1) {
		hwlog_err("carcrash config fail, val:%d is err\n", val);
		return -EINVAL;
	}

	wr.tag = (unsigned char)data->tag;
	wr.cmd = CMD_CMN_CONFIG_REQ;
	carcrash_ioctl.sub_cmd = SUB_CMD_CARCRASH_CONFIG_REQ;
	carcrash_ioctl.carcrash_signal = val;
	wr.wr_buf = &carcrash_ioctl;
	wr.wr_len = sizeof(carcrash_ioctl);
	ret = write_customize_cmd(&wr, NULL, false);

	hwlog_info("%s carcrash val %d ret %d\n", data->name, val, ret);
	return size;
}

static DEVICE_ATTR(carcrash_signal, 0664, NULL, carcrash_signal_store);

static struct attribute *carcrash_sensor_attrs[] = {
	&dev_attr_carcrash_signal.attr,
	NULL,
};

static const struct attribute_group g_carcrash_sensor_attrs_grp = {
	.attrs = carcrash_sensor_attrs,
};

static struct sensor_cookie recovery_sensors[] = {
	{
		.tag = TAG_POSTURE,
		.name = "hinge_sensor",
		.attrs_group = &g_hinge_sensor_attrs_grp,
	},
};
...
static struct sensor_cookie all_sensors[] = {
	...
	},
	{
		.tag = TAG_PS1,
		.name = "ps1_sensor",
		.attrs_group = &ps1_sensor_attrs_grp,
	},
	{
		.tag = TAG_ALS,
		.name = "als_sensor",
...
static int erecovery_sensor_register(void)
{
	int i;
	...
	for (i = 0; i < sizeof(recovery_sensors) / sizeof(recovery_sensors[0]); ++i) {
		recovery_sensors[i].dev = device_create(sensors_class, NULL, 0,
			&recovery_sensors[i], recovery_sensors[i].name);
		if (!recovery_sensors[i].dev) {
			hwlog_err("[%s] Failed", __func__);
			return -1;
		} else {
			if (recovery_sensors[i].attrs_group &&
				sysfs_create_group(&recovery_sensors[i].dev->kobj,
					recovery_sensors[i].attrs_group))
				hwlog_err("create files failed in %s\n",
					__func__);
		}
	}
...
static int sensors_feima_init(void)
{
	if (is_sensorhub_disabled()) {
		get_hall_fold_status_for_sensorhub_disabled();
		erecovery_sensor_register();
		return -1;
	}
