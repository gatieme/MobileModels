...
static unsigned long is_gsensor_gather_enable;
...
static void attr_set_selftest_write_cmd(enum obj_tag tag,
	char *selftest_result, struct write_info *pkg_ap)
{
	int err;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, TO_DECIMALISM, &enable))
		return -EINVAL;
	...
	if (is_gsensor_gather_enable == enable) {
		hwlog_info("gsensor gather already seted to state, is_gsensor_gather_enable %lu\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	case TAG_HANDPRESS:
		return sensors_calibrate_show_handpress(tag, dev, attr, buf);
	case TAG_CAP_PROX:
	case TAG_CAP_PROX1:
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case TAG_PS1:
		return attr_ps1_calibrate_write(dev, attr, buf, count);
	case TAG_ALS:
	case TAG_ALS1:
	case TAG_ALS2:
