...
static enum ret_type handpress_calibration_res = RET_INIT;
...
{\
	return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, \
		"%s\n", get_sensor_chip_info_address(tagup)); \
}

sensor_show_info(PS, ps);
static DEVICE_ATTR(ps_info, 0444, sensor_show_ps_info, NULL);

sensor_show_info(PS1, ps1);
static DEVICE_ATTR(ps1_info, 0444, sensor_show_ps1_info, NULL);
...
sensor_show_value(TAG_PS1, ps1);
static DEVICE_ATTR(ps1_read_data, 0664, sensor_show_ps1_read_data, NULL);
...
static ssize_t attr_set_##taglow##_selftest(struct device *dev, \
	struct device_attribute *attr, const char *buf, size_t size) \
{ \
	return attr_set_selftest(TAG_##tagup, CMD_CMN_CONFIG_REQ, \
		sensor_status.taglow##_selftest_result, buf, size); \
}
#endif
...
int enq_msg_data_in_sensorhub_single(struct event events)
{
	...
	if (!dsm_client_ocuppy(client)) {
		ret = dsm_client_copy_ext(client, msg,
	...
	const char *buf, size_t count)
{
	...
	if (kstrtoull(buf, TO_HEXADECIMAL, &val))
		return -EINVAL;
	...
	if (rw_info.rw)
		...
		ret = mcu_i2c_rw(rw_info.i2c_i3c, rw_info.bus_num,
			rw_info.i2c_address, rw_info.buf_temp, 2, NULL, 0); /* 2 bytes */
...
static DEVICE_ATTR(tof_calibrate, 0664, attr_tof_calibrate_show, attr_tof_calibrate_store);
/*
	...
	unsigned int responsed, bool is_subcmd)
{
	...
	pkt_cmn_interval_req_t cpkt_interval;
	memset(&cpkt_interval, 0, sizeof(cpkt_interval));
	struct pkt_header *hd_interval = (struct pkt_header *)&cpkt_interval;
	...
	if (is_subcmd) {
		...
		pkg_ap.cmd = cmd;
		if (cmd == CMD_CMN_INTERVAL_REQ) {
			cpkt_interval.param.period = pare;
			pkg_ap.wr_buf = &hd_interval[1];
			pkg_ap.wr_len = sizeof(cpkt_interval.param);
		} else {
			pkg_ap.wr_buf = &pare;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	spin_lock_irqsave(get_fsdata_lock(), flags);
	...
	if (skip) {
		spin_unlock_irqrestore(get_fsdata_lock(), flags);
		return size;
	}
...
show_enable_func(ps1, TAG_PS1)
store_enable_func(ps1, TAG_PS1, CMD_CMN_OPEN_REQ, CMD_CMN_CLOSE_REQ)
static DEVICE_ATTR(ps1_enable, 0664, show_ps1_enable_result, attr_set_ps1_enable);
show_delay_func(ps1, TAG_PS1)
store_delay_func(ps1, TAG_PS1, CMD_CMN_INTERVAL_REQ)
static DEVICE_ATTR(ps1_setdelay, 0664, show_ps1_delay_result, attr_set_ps1_delay);
...
show_enable_func(carcrash, TAG_CARCRASH)
store_enable_func(carcrash, TAG_CARCRASH, CMD_CMN_OPEN_REQ, CMD_CMN_CLOSE_REQ)
static DEVICE_ATTR(carcrash_enable, 0664,
	show_carcrash_enable_result, attr_set_carcrash_enable);
show_delay_func(carcrash, TAG_CARCRASH)
store_delay_func(carcrash, TAG_CARCRASH, CMD_CMN_INTERVAL_REQ)
static DEVICE_ATTR(carcrash_setdelay, 0664,
	show_carcrash_delay_result, attr_set_carcrash_delay);
...
static void handpress_calibrate_work_func(struct work_struct *work)
{
	...
	ret = write_handpress_offset_to_nv(offset);
	if (ret < 0)  {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	uint8_t hand_calidata[MAX_HP_OFFSET_DATA_LENGTH] = { 0 };
	uint8_t *offset = get_hp_offset();
	...
	if (memcpy_s(offset, MAX_HP_OFFSET_DATA_LENGTH,
		hand_calidata, MAX_HP_OFFSET_DATA_LENGTH) != EOK)
		hwlog_err("%s memcpy_s error\n", __func__);
	handpress_calibration_res = SUC;
	if (val == 1) {
	...
	unsigned int responsed, bool is_subcmd)
{
	...
	if (responsed == NO_RESP) {
		...
		if (pkg_mcu.errno != 0) {
			...
			sensor_status.gyro_ois_status = (int)pare;
		}
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	} else if (source == 3) { /* 3: open, set interval, enable(RESP) */
		source = 1;
		cmd = CMD_CMN_OPEN_REQ;
		ret = ois_commu(TAG_OIS, cmd, source, NO_RESP, false);
		if (ret) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	source = (unsigned int)simple_strtol(buf, NULL, TO_DECIMALISM);
	value = ((source >> 8) & 0xff);
...
static struct attribute *sensor_attributes[] = {
	...
	&dev_attr_carcrash_enable.attr,
	&dev_attr_carcrash_setdelay.attr,
