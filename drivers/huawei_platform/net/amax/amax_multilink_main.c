...
static uint32_t amax_valid_magic_llc_header(struct sk_buff *skb)
{
	uint8_t *ptr = NULL;
	...
	skb_tail = (uint8_t *)(skb->head + skb->tail);
#else
	skb_tail = skb->tail;
#endif
...
PACKET_TYPE amax_inbound_frame_classify(struct sk_buff *pst_buf)
{
	...
	if (amax_valid_magic_llc_header(pst_buf) != SUCC)
		return PACKET_INVALID;
...
static int32_t amax_rx_frame_reassemble(struct sk_buff *net_buf, recv_frame *info)
{
	...
	add_len = sizeof(llc_header) + MAGIC_AMAX_LEN;
#ifdef NET_SKBUFF_DATA_USES_OFFSET
	skb_tail = (uint8_t *)(net_buf->head + net_buf->tail);
#else
	skb_tail = net_buf->tail;
#endif
...
#ifdef NET_SKBUFF_DATA_USES_OFFSET
	skb_tail = (uint8_t *)(netbuff->head + netbuff->tail);
#else
	skb_tail = (uint8_t *)netbuff->tail;
#endif
	add_len = sizeof(llc_header) + MAGIC_AMAX_LEN;
...
int32_t amax_rx_handle_hook(struct sk_buff *netbuf)
{
	...
	// 截获带有amax封装的报文
	if (amax_inbound_frame_classify(netbuf) == PACKET_INBOUND) {
		// 该节点从skb链表中删除
		netbuf->tstamp = ktime_get();
