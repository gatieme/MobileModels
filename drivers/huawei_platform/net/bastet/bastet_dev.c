...
#include <huawei_platform/net/bastet/adrv_sensorhub.h>
...
static struct bst_ind_type_name ind_type_name[] = {
	...
	{ BST_IND_SENSORHUB_RESET, "BST_IND_SENSORHUB_RESET" },
};
...
static int handle_bst_ioc_check_mac_address_available(void __user *argp);
	...
	const void *info, unsigned int len)
{
	...
	if (type >= BST_IND_INVALID && type <= BST_IND_SENSORHUB_RESET)
...
#ifdef CONFIG_MLPS_OPERATE_CONN
		msleep(1000);
		ret = mlps_operate_gnss_power(MLPS_CMD_CLOSE, BASTET);
		if (ret != 0)
			bastet_loge("failed to power off GNSS"); // The GNSS power-off failure does not affect the wifi bastet, does not need to be returned
#endif
		g_is_wifi = false;
	}
	return rc;
...
static int handle_bst_ioc_set_traffic_flow(void __user *argp)
{
	...
	buf_len = (signed)bst_traffic_len(flow_p.cnt);
...
static int handle_bst_ioc_sock_sync_prepare(void __user *argp)
{
	...
	bastet_logi("handle_bst_ioc_sock_sync_start, init_p.is_wifi = %d", sync_prop.is_wifi);
	g_is_wifi = sync_prop.is_wifi;

#ifdef CONFIG_MLPS_OPERATE_CONN
	if (g_is_wifi) {
		ret = mlps_operate_gnss_power(MLPS_CMD_OPEN, BASTET);
		if (ret != 0) {
			bastet_loge("failed to power on GNSS");
			return ret; // 是否有必要上电G核失败后直接返回
		}
		while (1) {
			ret = get_mlps_operate_gnss_status();
			if (ret != MLPS_WAIT_GNSS_OPERATION) {
				bastet_loge("get_mlps_operate_gnss_status ret=%d", ret);
				break;
			} else {
				msleep(10);
			}
...
static int handle_bst_ioc_check_mac_address_available(void __user *argp)
{
	...
	bool result = check_bastet_mac_address_available(&guide);
	if (result)
		rc = 0;
...
#ifdef CONFIG_BASTET_IN_SENSORHUB
static void wifi_rx_skb_by_bastet(struct sk_buff *skb)
{
	int ret = -EFAULT;
	struct net_device *netdev = NULL;

	if (skb == NULL) {
		bastet_loge("%s sk_buff is null\n", __func__);
		return;
	}

	netdev = dev_get_by_name(&init_net, "wlan0");
	if (netdev == NULL) {
		bastet_loge("%s netdev not found\n", __func__);
		return;
	}

	if (netif_running(netdev)) {
		bastet_logi("%s netif_running is true\n", __func__);
	} else {
		bastet_logi("%s netif_running is false\n", __func__);
	}

	skb->protocol = eth_type_trans(skb, netdev);
	bastet_logi("%s skb len: %d, protocol: %d\n", __func__, skb->len, skb->protocol);

	if (in_interrupt()) {
		ret = netif_rx(skb);
		bastet_logi("%s call netif_rx ret = %d\n", __func__, ret);
	} else {
		ret = netif_rx_ni(skb);
		bastet_logi("%s call netif_rx_ni ret = %d\n", __func__, ret);
	}

	bastet_logi("%s end\n", __func__);
}

static int bastet_rx_data_from_sensorhub(const struct pkt_header *head)
{
	struct sk_buff *skb = NULL;
	struct pkt_subcmd_req_t *p_resp = (struct pkt_subcmd_req_t *)(head);
	struct rx_dl_pkt_from_sensorhub *dlpkt = (struct rx_dl_pkt_from_sensorhub *)(p_resp + 1);
	unsigned pkt_len = dlpkt->length;
	unsigned skb_len = pkt_len + ETH_HLEN;

	bastet_logi("head->length %d tag %d length %d ", head->length, dlpkt->tag, pkt_len);
	skb = dev_alloc_skb(skb_len);
	if (skb == NULL) {
		bastet_loge("Failed to call dev_alloc_skb. %d ", skb_len);
		return -1;
	}

	memset_s(skb->data, skb_len, 0, skb_len);

	skb_reserve(skb, ETH_HLEN);

	// copy data to skb
	if (memcpy_s(skb->data, pkt_len, (void *)(dlpkt->data), pkt_len)) {
		bastet_loge("Failed to call memcpy_s %d\n", pkt_len);
		dev_kfree_skb(skb);
		return -1;
	}
	skb_put(skb, pkt_len);

	bastet_logi("ip pkt data %d\n", pkt_len);
	print_hex_dump(KERN_ERR, "pktmsg:", 0, 16, 1, (void *)(dlpkt->data), pkt_len, 1);
	wifi_rx_skb_by_bastet(skb);

	return 0;
}

static ssize_t bastet_sendmsg_to_sensorHub(const char __user *buf, size_t count)
{
	struct write_info pkg_ap;
	int ret = 0;

	u8 *msg = kmalloc(count + SUBCMD_LEN, GFP_KERNEL);
	if (msg == NULL) {
		bastet_loge("Failed to kmalloc size = %d", count);
		return -1;
	}

	ret = memset_s(msg, count + SUBCMD_LEN, 0, count + SUBCMD_LEN);
	if (ret) {
		bastet_loge("Failed to memset_s ret = %d", ret);
	}

	/* msg include event head 8 byte */
	if (memcpy_s(msg + SUBCMD_LEN, BST_MAX_WRITE_PAYLOAD, buf, count)) {
		bastet_loge("buf memcpy to msg failed\n");
	}

	memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap));
	pkg_ap.tag = TAG_BASTET;
	pkg_ap.cmd = CMD_BST_AP_TO_SENSOR;
	uint32_t *psubcmd = (uint32_t *)msg;
	*psubcmd = SUB_CMD_BST_DSPP_MSG;
	pkg_ap.wr_buf = msg;
	pkg_ap.wr_len = count + SUBCMD_LEN;

	bastet_logi("bastet send msg to sensorHub tag %d count %d", TAG_BASTET, count);
	ret = write_customize_cmd(&pkg_ap, NULL, false);
	kfree(msg);
	if (ret) {
		bastet_loge("Failed to send tag %d count %d to sensorHub,ret=%d\n", pkg_ap.tag, count, ret);
		return -1;
	}
...
static int bastet_rcv_msg_from_sensorhub(const struct pkt_header *head)
{
	struct pkt_subcmd_req_t *p_resp = NULL;
	int ret = 0;
	int len = 0;
	int length;
	u8 *msg = NULL;

	p_resp = (struct pkt_subcmd_req_t *)(head);
	if (p_resp == NULL) {
		bastet_loge("Failed to p_resp is NULL");
		return -EFAULT;
	}

	if (p_resp->hd.tag != TAG_BASTET) {
		bastet_loge("Failed to invalid tag %d ", p_resp->hd.tag);
		return -EFAULT;
	}

	bastet_logd("cmd[%d], length[%d], tag[%d], sub_cmd[%d]",
		p_resp->hd.cmd, p_resp->hd.length, p_resp->hd.tag, p_resp->subcmd);

	switch (p_resp->subcmd) {
	case SUB_CMD_BST_DSPP_MSG:
		len = head->length - SUBCMD_LEN;
		/* call func bastet_comm_recv send msg to ap */
		ind_hisi_com((void *)(p_resp + 1), len);
		break;
	case SUB_CMD_BST_DATA_PKT:
		ret = bastet_rx_data_from_sensorhub(head);
		if (ret != 0)
			bastet_loge("Failed to call bastet_rx_data_from_sensorhub ret %d ", ret);
		break;
	default:
		bastet_loge("Failed to  invalid subcmd %d ", p_resp->subcmd);
	}
...
#ifdef CONFIG_BASTET_IN_SENSORHUB
static int bst_recovery_notifier(struct notifier_block *nb, unsigned long foo, void *bar)
{
	...
	case IOM3_RECOVERY_3RD_DOING:
		if (g_is_wifi)
			post_indicate_packet(BST_IND_SENSORHUB_RESET, NULL, 0);
		break;
	default:
		bastet_loge("%s -unknow state %ld\n", __func__, foo);
		break;
	}
	bastet_logi("%s -\n", __func__);
	return 0;
...
static struct notifier_block bst_sensorhub_recovery_notify = {
	.notifier_call = bst_recovery_notifier,
	.priority = -1, /* SENSORHUB_RECOVERY_PRIORITY */
};
#endif
...
static int bastet_open(struct inode *inode, struct file *filp)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	bastet_loge("register_mcu_event_notifier enter\n");
	if (is_sensorhub_disabled()) {
		bastet_loge("sensorhub disabled....\n");
	} else {
		int ret = register_mcu_event_notifier(TAG_BASTET,
			CMD_BST_SENSOR_TO_AP, bastet_rcv_msg_from_sensorhub);
		if (ret != 0)
			bastet_loge("reg notifier failed. [%d]\n", ret);
		bastet_loge("TAG_BASTET reg is success");
	}

	register_iom3_recovery_notifier(&bst_sensorhub_recovery_notify);
	#endif
...
static int bastet_packet_read(char __user *buf, size_t count)
{
	...
	len = (signed)sizeof(*p_bst_device_ind) - (signed)sizeof(pkt->data.cons) +
		(signed)pkt->data.len;
	data = (uint8_t *)(&pkt->data) + sizeof(pkt->data.cons);

	if ((pkt->data.cons == 0) && (count > (unsigned)len)) {
		list_del(&pkt->list);
		size = len;
		isfree = true;
	} else if (((pkt->data.cons == 0) && (count <= (unsigned)len)) ||
		((pkt->data.cons != 0) && (pkt->data.cons + count <= (unsigned)len))) {
		size = (signed)count;
		isfree = false;
	} else {
		list_del(&pkt->list);
		size = len - (signed)pkt->data.cons;
		isfree = true;
	}
	...
	const char __user *buf, size_t count, loff_t *ppos)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	if (msg[BASTET_DSPP_DEVICEID_IDX] == BASTET_DSPP_DEVICE_MOBILE) {
		bastet_comm_write(msg, count, BST_ACORE_CORE_MSG_TYPE_DSPP);
	} else if (msg[BASTET_DSPP_DEVICEID_IDX] == BASTET_DSPP_DEVICE_WIFI) {
		bastet_sendmsg_to_sensorHub(msg, count);
	} else {
		bastet_loge("unknown device id");
	}
	#else
	bastet_comm_write(msg, count, BST_ACORE_CORE_MSG_TYPE_DSPP);
	#endif
...
static int bastet_release(struct inode *inode, struct file *filp)
{
	...
	#ifdef CONFIG_BASTET_IN_SENSORHUB
	unregister_mcu_event_notifier(TAG_BASTET, CMD_BST_SENSOR_TO_AP, bastet_rcv_msg_from_sensorhub);
	#endif
