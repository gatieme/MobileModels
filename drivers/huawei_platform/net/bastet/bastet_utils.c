...
/* minimum mac address byte */
#define MIN_MAC_BYTE 0
/* maximum mac address byte */
#define MAX_MAC_BYTE 0xFF
...
#else
static void channel_occupied_timeout(struct timer_list *data);
#endif
...
#else
static DEFINE_TIMER(channel_timer, channel_occupied_timeout);
#endif
static const unsigned char empty_mac_address[ETH_ALEN] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
static const unsigned char bcast_mac_address[ETH_ALEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
...
bool is_ipv6_addr(struct sock *sk)
{
	...
	if (sk->sk_family == AF_INET6 &&
		!((unsigned)ipv6_addr_type(&sk->sk_v6_rcv_saddr) & IPV6_ADDR_MAPPED))
		return true;
#endif
	return false;
}
...
/*
 * check_mac_address_valid() - check mac address is valid
 * @mac_address[]: mac address array
 * @size: the length of amc address array
 *
...
{
	if (mac_address == NULL || size != ETH_ALEN)
		return false;

	if (memcmp(mac_address, empty_mac_address, ETH_ALEN) == 0 ||
	    memcmp(mac_address, bcast_mac_address, ETH_ALEN) == 0)
	    return false;
...
/*
 * bastet_get_mac_header() - get the mac hedader.
 * @comm_prop_array: array stored comm prop
 * @length: size of the array
 * @sock_p: set sock sync prop data pointer
 *
 * iterate the array to find the identical comm prop
 * copy the mac address to the set sock sync prop
 *
 * Return: 0 success.
 * Otherwise is fail.
 */
...
{
	if (IS_ERR_OR_NULL(comm_prop_array) || IS_ERR_OR_NULL(sock_p)) {
	    bastet_loge("invalid patameter");
	    return -1;
	}

	for (int i = 0; i < length; i++) {
	    if (memcmp(&(comm_prop_array[i].comm_prop), &sock_p->guide,
			sizeof(struct bst_sock_comm_prop)) == 0) {
			if (check_mac_address_valid(comm_prop_array[i].mac_source, ETH_ALEN) &&
				check_mac_address_valid(comm_prop_array[i].mac_dest, ETH_ALEN)) {
				memcpy(sock_p->mac_source, &comm_prop_array[i].mac_source, ETH_ALEN);
				memcpy(sock_p->mac_dest, &comm_prop_array[i].mac_dest, ETH_ALEN);
				return 0;
			}
	    }
	}
...
int bastet_get_mac_header_ipv6(struct bst_mac_address_comm_prop *comm_prop_array,
		int length, struct bst_set_sock_sync_prop_ipv6 *sock_p_ipv6)
{
	if (IS_ERR_OR_NULL(comm_prop_array) || IS_ERR_OR_NULL(sock_p_ipv6)) {
		bastet_loge("invalid patameter");
		return -1;
	}

	for (int i = 0; i < length; i++) {
		if (memcmp(&(comm_prop_array[i].comm_prop_ipv6), &sock_p_ipv6->guide,
		    sizeof(struct bst_set_sock_sync_prop_ipv6)) == 0) {
		    if (check_mac_address_valid(comm_prop_array[i].mac_source, ETH_ALEN) &&
				check_mac_address_valid(comm_prop_array[i].mac_dest, ETH_ALEN)) {
				memcpy(sock_p_ipv6->mac_source, &comm_prop_array[i].mac_source, ETH_ALEN);
				memcpy(sock_p_ipv6->mac_dest, &comm_prop_array[i].mac_dest, ETH_ALEN);
				return 0;
		    }
		}
	}
