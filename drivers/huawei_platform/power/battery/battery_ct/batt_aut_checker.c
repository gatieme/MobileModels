...
static int rt_check_wrapper(struct batt_chk_data *drv_data)
{
	int ret = -1;
	...
	if (drv_data->ic_ops.rt_check) {
		bat_type_apply_mode(BAT_ID_SN);
		ret = drv_data->ic_ops.rt_check(drv_data->ic, &drv_data->key_res);
		bat_type_release_mode(true);
	}
	return ret;
}
...
static DEVICE_ATTR_RO(ic_uuid);
static DEVICE_ATTR_RO(batt_type);
static DEVICE_ATTR_RO(ic_status);
static DEVICE_ATTR_RO(key_status);
static DEVICE_ATTR_RO(sn_status);
static DEVICE_ATTR_RO(batt_id);
static DEVICE_ATTR_RW(batt_source);
static DEVICE_ATTR_RO(check_mode);
static DEVICE_ATTR_RW(act_signature);
static DEVICE_ATTR_RW(retrofit_signature);
#ifdef BATTERY_LIMIT_DEBUG
static DEVICE_ATTR_RO(rematchable_onboot);
static DEVICE_ATTR_RO(rematchable_current);
static DEVICE_ATTR_RO(batt_src_onboot);
static DEVICE_ATTR_RO(batt_cycles);
static DEVICE_ATTR_RO(batt_act_state);
static DEVICE_ATTR_WO(lock_battery);
static DEVICE_ATTR_RW(free_cycles);
static DEVICE_ATTR_RW(check_result);
static DEVICE_ATTR_RW(check_key_type);
static DEVICE_ATTR_RW(check_sn_type);
static DEVICE_ATTR_WO(check_key);
static DEVICE_ATTR_WO(check_sn);
#endif /* BATTERY_LIMIT_DEBUG */
...
static void init_wrapper_ops(struct batt_chk_data *drv_data)
{
	...
	drv_data->bco.rt_check = rt_check_wrapper;
...
int check_sn_binded(struct batt_chk_data *checker_data)
{
	...
	if (checker_data->group_sn) {
		cur_group_sn_str_len = snprintf(cur_group_sn_str, MAX_SN_LEN - 1,
			"%016llX", checker_data->group_sn);
		if (cur_group_sn_str_len < 0 || cur_group_sn_str_len > MAX_SN_LEN) {
			hwlog_err("snprintf group_sn fail\n");
			return -1;
		}
		if (memcmp(bbinfo.info[0], cur_group_sn_str, cur_group_sn_str_len)) {
...
void init_battery_check_result(struct batt_chk_rslt *result)
{
	...
	result->ic_status = IC_PASS;
	result->key_status = KEY_PASS;
	result->sn_status = SN_PASS;
#ifdef BATTBD_FORCE_MATCH
...
static void batt_set_work_status(struct platform_device *pdev, int status)
{
	struct batt_chk_data *drv_data = NULL;
	...
	if (!drv_data)
		return;
	if (drv_data->ic_ops.set_ic_status)
		drv_data->ic_ops.set_ic_status(drv_data->ic, status);
}
...
static void battery_backup_id_init(struct device_node *np, struct batt_chk_data *data)
{
	/* 1:only one line parameters */
	(void)power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"backup_id_check", (int *)&data->backup_check_item, 1, BACKUP_ID_CHECK_STRUCT_LEN);

	hwlog_info("[backup_id_check] is_need_check=%d sn_backup_id=%d content=%d\n",
		data->backup_check_item.is_need_check,
		data->backup_check_item.sn_backup_id,
		data->backup_check_item.content);
}
...
static int battery_aut_checker_probe(struct platform_device *pdev)
{
	...
	if (local_ic) {
		hwlog_err("[%s] %s get ic ops failed\n", __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
	...
	/* get backup id */
	battery_backup_id_init(np, drv_data);
	...
	drv_data->type_entry.ic_dev = pdev;
	drv_data->type_entry.set_work_status = batt_set_work_status;
	bat_type_ic_register(&drv_data->type_entry);
