...
struct batt_ct_ops {
	...
	int (*get_batt_safe_info)(struct platform_device *pdev,
		enum batt_safe_info_t type, void *value);
	void (*set_ic_status)(struct platform_device *pdev, int status);
...
struct batt_chk_data {
	...
	uint64_t group_sn;
	struct batt_type_entry type_entry;
	enum sn_type check_sn_type;
	struct batt_thread_work key_twork;
	struct batt_thread_work sn_twork;
};
...
int byc_init_twork(struct batt_chk_data *drv_data);
