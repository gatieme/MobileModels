...
#define MAX_SN_BUFF_LEN    50

#define BATT_CELL_INDEX        1
#ifndef MAX_IC_COUNT
#define MAX_IC_COUNT           2
#endif
...
static void battct_search_minor_node(struct list_head **target_node, struct batt_checker_entry *entry)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *list_checker = NULL;
	struct batt_chk_data *new_checker = platform_get_drvdata(entry->pdev);

	list_for_each_entry(temp, &batt_checkers_head, node) {
		list_checker = platform_get_drvdata(temp->pdev);
		if ((list_checker->id_in_grp) <= (new_checker->id_in_grp)) {
			*target_node = &temp->node;
			break;
		}
	}
}
...
{
	struct list_head *target_node = &batt_checkers_head;

	if (!entry)
		return;

	/* Make sure checker_list ordered as bat2 bat1 bat0 */
	battct_search_minor_node(&target_node, entry);
	list_add_tail(&entry->node, target_node);
	valid_checkers++;
}
...
static int compare_battery_sn(struct batt_chk_data *checker_data)
{
	...
	if (memcmp(nv_sn, sn, MAX_RAW_SN_LEN)) {
		hwlog_err("battery sn is not same with nv in %s\n",
			__func__);
		return BATTERY_SN_CMP_DIFF;
	}

	return BATTERY_SN_CMP_SAME;
}
...
static int check_batt_moved_by_group_sn(void)
{
	...
	if (memcmp(group_sn, nv_sn, MAX_RAW_SN_LEN))
		return BATTERY_SN_CMP_DIFF;
	return BATTERY_SN_CMP_SAME;
}
...
static int batt_create_cur_group_sn(uint64_t *group_sn_hash_val)
{
	...
	if (i != g_total_checkers) {
		hwlog_err("read sn error\n");
		return -1;
	}
...
/*
 * for different sns in a group
 * it skipes the original software architecture, because original
 * interface does not have group_sn function.
 * if we want use the original software architecture, we can try use
 * the 'prepare' interface with new param.
 */
...
{
	if (check_battery_sn_changed() == BATTERY_SN_CMP_SAME)
		return;
	info->nv_sn_old = 1;
...
static bool check_moved_recheck(struct batt_info *info)
{
	...
	if (info->moved_recheck_logic) {
		need_recheck = (sn_change_flag != BATTERY_SN_CMP_SAME) && (removed_flag == 1);
	} else {
		need_recheck = (sn_change_flag != BATTERY_SN_CMP_SAME) || (removed_flag == 1);
	}
...
static void record_group_sn(struct batt_info *drv_data)
{
	int ret;
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data = NULL;
...
static int check_ic_func(struct batt_info *drv_data, struct batt_chk_rslt *final_result)
{
	struct batt_chk_data *checker_data = NULL;
	struct batt_checker_entry *temp = NULL;
	int ret;
...
static int check_key_func(struct batt_chk_rslt *final_result)
{
	struct batt_chk_data *checker_data = NULL;
	struct batt_checker_entry *temp = NULL;
	int ret;
...
static int check_sn_func(struct batt_chk_rslt *final_result)
{
	struct batt_chk_data *checker_data = NULL;
	struct batt_checker_entry *temp = NULL;
	int ret;
...
static void batt_check_pre(struct batt_info *drv_data)
{
	...
	if (drv_data->check_strategy_no == CHECK_STRATEGY_BOOTING && g_sn_change_onboot < 0)
		g_sn_change_onboot = check_battery_sn_changed();

	batt_cycle_init(drv_data);
}

static int batt_check_last_result(struct batt_info *drv_data, struct batt_chk_rslt *last_result)
{
	int ret;
	int i;
...
static struct batt_chk_rslt batt_update_rslt(enum check_mode check_mode,
	enum ic_cr ic_status, enum key_cr key_status, enum sn_cr sn_status)
{
	struct batt_chk_rslt dummy_result;

	dummy_result.check_mode = check_mode;
	dummy_result.ic_status = ic_status;
	dummy_result.key_status = key_status;
	dummy_result.sn_status = sn_status;

	return dummy_result;
}

static void batt_check_nv_sn(struct batt_info *drv_data)
{
	check_nv_sn(drv_data);
	if (drv_data->sn_checker == final_sn_checker_1)
		get_sn_from_nv(drv_data);
	else if (drv_data->sn_checker == final_sn_checker_2)
		record_group_sn(drv_data);
}

static int real_batt_check(struct batt_info *drv_data, struct batt_chk_rslt *final_result)
{
	struct batt_chk_data *checker_data = NULL;
	struct batt_checker_entry *temp = NULL;

	hwlog_info("real checking started in %s\n", __func__);
	if (check_ic_func(drv_data, final_result)) {
		hwlog_err("check_ic_func failed\n");
		return -EINVAL;
	}

	hwlog_info("checking key status started in %s\n", __func__);
	/* authenticate checker keys */
	if (check_key_func(final_result)) {
		hwlog_err("check_key_func failed\n");
		return -EINVAL;
	}
	...
	if (check_sn_func(final_result)) {
		hwlog_err("check_sn_func failed\n");
		return -EINVAL;
	}
...
static void batt_skip_result(struct batt_chk_rslt *final_result)
{
	enum result_stat result_status;
...
{
	struct batt_info *drv_data = container_of(work,
		struct batt_info, check_work);
	struct batt_thread_work *twork = &drv_data->check_twork;

	batt_kthread_run("battct_check", twork);
}

static void check_thread_func(struct batt_thread_work *kwork)
{
	int ret;
	struct batt_chk_rslt *last_result = NULL;
	struct batt_chk_rslt *final_result = NULL;
	struct batt_info *drv_data = container_of(kwork, struct batt_info, check_twork);

	hwlog_info("battery checking started in %s\n", __func__);

	batt_check_pre(drv_data);
	/* check last result */
	last_result = &drv_data->last_result;
	ret = batt_check_last_result(drv_data, last_result);
	...
	if (ret != 0) {
		...
		*final_result = batt_update_rslt(0, IC_PASS, KEY_PASS, SN_FAIL_NV);
		schedule_delayed_work(&drv_data->dmd_report_dw, 0);
		goto exit_check_func;
	}

	if (shield_ct_sign) {
		*final_result = batt_update_rslt(0, IC_PASS, KEY_PASS, SN_PASS);
	...
	if (!(ret || !is_legal_result(last_result) ||
		(last_result->check_mode == FACTORY_CHECK_MODE) || check_moved_recheck(drv_data) ||
		(get_batt_match_type() == BATTERY_REMATCHABLE) || drv_data->must_real_check)) {
		*final_result = *last_result;
		batt_check_nv_sn(drv_data);
		hwlog_info("battery ct will skip checking in %s\n", __func__);
		goto skip_result_status;
	}

	if (real_batt_check(drv_data, final_result))
		goto process_result_status;
...
{
	int indx = 0;
	char sn_buf[MAX_SN_BUFF_LEN];
	struct binded_info bbinfo;

	if (count >= MAX_SN_BUFF_LEN) {
		hwlog_err("[%s] buf is too long\n", __func__);
		return -1;
	}
	if (sscanf(buf, "%d,%s", &indx, sn_buf) != 2) {
...
static int battct_generate_result_str(char *buff)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data = NULL;
	struct platform_device *pdev = NULL;
	...
	int *dmd_no, int check_strategy_no)
{
	...
	if (*dmd_no) {
		...
		count += battct_generate_result_str(buff + count);
		count += snprintf(buff + count, DMD_BUF_SIZE - count,
			"last_rslt:ic %d, key %d, sn %d, mode %d, batt_removed %d, sn_changed %d\n",
			last_result->ic_status,
			last_result->key_status,
			last_result->sn_status,
			last_result->check_mode,
			power_platform_is_battery_removed(),
			check_battery_sn_changed());
...
static int batt_info_dts_init(struct platform_device *pdev)
{
	struct batt_info *drv_data = platform_get_drvdata(pdev);
...
static struct batt_info *batt_info_data_init(struct platform_device *pdev)
{
	...
	(void)init_thread_work(&drv_data->check_twork, check_thread_func);
	init_kwork_expeller(&drv_data->check_twork, true);
	init_kwork_sche_param(&drv_data->check_twork, true,
		SCHED_FIFO, PRIORITY_MAX);
