...
static int sle95250_cpu_single_core_set(struct sle95250_dev *di)
{
	int cpu_id;
	int num_cpu_on;
	int ret;
	...
	for (cpu_id = 1; cpu_id < di->core_num; cpu_id++) {
...
static void sle95250_turn_on_all_cpu_cores(struct sle95250_dev *di)
{
	int cpu_id;
	int num_cpu_on;
	int ret;
...
#ifdef BATTBD_FORCE_MATCH
static bool sle95250_is_act_sign_via_nv(struct sle95250_dev *di)
{
	return di->act_sign_via_nv == SLE95250_ACT_VIA_NV_VALID;
}
#endif /* BATTBD_FORCE_MATCH */
...
#ifdef BATTBD_FORCE_MATCH
	struct sle95250_nv_act_sign act_info = { 0 };
#endif /* BATTBD_FORCE_MATCH */

	if (!di)
		return -ENODEV;
#ifdef BATTBD_FORCE_MATCH
	if (sle95250_is_act_sign_via_nv(di)) {
		hwlog_info("[%s] nv_sign valid, set cyclk by nv\n", __func__);

		if (di->ic_index >= ARRAY_SIZE(g_nv_ic_map)) {
			hwlog_err("[%s] ic_index is out of nv_ic_map's range\n", __func__);
			return -EINVAL;
		}

		ret = power_nv_read(g_nv_ic_map[di->ic_index], &act_info, sizeof(act_info));
		if (ret) {
			hwlog_err("[%s] read nv failed\n", __func__);
			return ret;
		}
		act_info.cyclk_set_flag = true;
		ret = power_nv_write(g_nv_ic_map[di->ic_index], &act_info, sizeof(act_info));
		if (ret) {
			hwlog_err("[%s] write nv failed\n", __func__);
			return ret;
		}
...
#ifdef BATTBD_FORCE_MATCH
static int sle95250_write_act_sign_nv(struct sle95250_dev *di, uint8_t *act)
{
	int ret;
	struct sle95250_nv_act_sign act_info = { 0 };

	act_info.sign_status = STATUS_BATT_SIGN_WRTIE;
	memcpy(act_info.act_sign, act, SLE95250_ACT_LEN);
	...
	ret = power_nv_write(g_nv_ic_map[di->ic_index], &act_info, sizeof(act_info));
	hwlog_info("[%s] write sign_nv ret:%d\n", __func__, ret);
	if (!ret)
		di->mem.act_sig_ready = false;
	return ret;
}

static int sle95250_read_act_sign_from_nv(struct sle95250_dev *di,
	struct sle95250_nv_act_sign *act)
{
	int ret;
	...
	ret = power_nv_read(g_nv_ic_map[di->ic_index], act, sizeof(*act));
	hwlog_info("[%s] read sign_nv ret:%d\n", __func__, ret);
	return ret;
}

static int sle95250_act_via_nv_read(struct sle95250_dev *di)
{
	int ret;
	struct sle95250_nv_act_sign act_info = { 0 };

	ret = sle95250_read_act_sign_from_nv(di, &act_info);
	if (ret) {
		hwlog_err("[%s] read sign in nv failed\n", __func__);
		return -EINVAL;
	}
	if (act_info.sign_status > STATUS_BATT_SIGN_INIT) {
		memcpy(di->mem.act_sign, act_info.act_sign, SLE95250_ACT_LEN);
...
static int sle95250_nv_set_cert_ready(struct sle95250_dev *di)
{
	hwlog_info("[%s] act_sign in nv, pglk don't lock in kernel\n", __func__);
...
static int sle95250_pglk_iqr(struct sle95250_dev *di, uint8_t *pg_lked)
{
	...
	if (sle95250_is_act_sign_via_nv(di)) {
		hwlog_info("[%s] nv_sign valid, pglk not iqr\n", __func__);
		ret = sle95250_read_act_sign_from_nv(di, &act_info);
		if (ret) {
			hwlog_err("[%s] read sign in nv failed\n", __func__);
			return -EINVAL;
		}
		*pg_lked = act_info.pglk_status;
...
static int sle95250_get_sn_bin(struct sle95250_dev *di, uint8_t *sn_bin, int len)
{
	int ret;
	uint8_t rty;
	...
	struct power_genl_attr *res, const unsigned char **sn, unsigned int *sn_size)
{
	...
	ret = sle95250_cpu_single_core_set(di);
	if (ret) {
		hwlog_err("%s: single core set fail\n", __func__);
		goto get_sn_fail;
	}

	ret = sle95250_get_sn_bin(di, sn_bin, SLE95250_SN_BIN_LEN);
	if (ret) {
		hwlog_err("%s: fail\n", __func__);
		goto get_sn_fail;
	}
	...
	*sn_size = SLE95250_SN_ASC_LEN;
	...
	sle95250_turn_on_all_cpu_cores(di);
	return 0;
get_sn_fail:
	sle95250_turn_on_all_cpu_cores(di);
	return -EINVAL;
...
#else
	if (sle95250_is_act_sign_via_nv(di))
		return sle95250_act_via_nv_read(di);
...
static int sle95250_act_write(struct sle95250_dev *di, uint8_t *act)
{
	...
	if (sle95250_is_act_sign_via_nv(di))
		return sle95250_write_act_sign_nv(di, act);
	...
	for (page_ind = 0; page_ind < page_max; page_ind++) {
		...
		ret = optiga_nvm_write(SLE95250_USER_AREA, page_ind, BYT_OFFSET0, write_len, page_data_w);
		if (ret) {
			page_ind--;
			retry++;
		...
		ret = optiga_nvm_read(SLE95250_USER_AREA, page_ind, BYT_OFFSET0, write_len, page_data_r);
		if (ret) {
			page_ind--;
			retry++;
			if (retry >= SLE95250_ACT_RTY)
...
#else
	if (sle95250_is_act_sign_via_nv(di))
		return sle95250_nv_set_cert_ready(di);
	ret = sle95250_cpu_single_core_set(di);
	if (ret) {
		hwlog_err("%s: single core set fail\n", __func__);
...
static int sle95250_batt_name_check(void)
{
	...
	if (strncmp(batt_name, SLE95250_IC_NAME, strlen(SLE95250_IC_NAME))) {
...
static int sle95250_cyclk_iqr(uint8_t *lk_status)
{
	...
	if (sle95250_is_act_sign_via_nv(di)) {
		ret = sle95250_read_act_sign_from_nv(di, &act_info);
		if (ret) {
			hwlog_err("[%s] read cyclk nv fail\n", __func__);
			return -EAGAIN;
		}
		*lk_status = act_info.cyclk_status;
		return 0;
	}
...
static struct sle95250_sysfs_fld_info sle95250_sysfs_fld_tbl[] = {
	...
	sle95250_sysfs_fld_rw(act_via_nv, ACT_VIA_NV),
	sle95250_sysfs_fld_ro(nv_act, NV_ACT),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct sle95250_nv_act_sign nv_act = { 0 };
	...
	case SLE95250_SYSFS_PGLK:
		...
		break;
	case SLE95250_SYSFS_ACT_VIA_NV:
		hwlog_info("[%s] SLE95250_SYSFS_ACT_VIA_NV, ic_index:%u\n",
			__func__, di->ic_index);

		str_len = scnprintf(buf, PAGE_SIZE, "act_via_nv: %d\n",
			di->act_sign_via_nv);
		break;
	case SLE95250_SYSFS_NV_ACT:
		hwlog_info("[%s] SLE95250_SYSFS_NV_ACT, ic_index:%u\n",
			__func__, di->ic_index);
		ret = sle95250_read_act_sign_from_nv(di, &nv_act);
		if (ret) {
			hwlog_err("[%s] read sn from nv failed, ic_index:%u\n",
				__func__, di->ic_index);
			str_len = scnprintf(buf, PAGE_SIZE, "read nv act fail\n");
			break;
		}
		str_len = scnprintf(buf, PAGE_SIZE, "nv act_sign status is:%u, act is:", nv_act.sign_status);
		for (index_str = 0; index_str < SLE95250_NV_ACT_LEN; index_str++) {
			str_len += snprintf(buf + str_len, PAGE_SIZE - str_len, "%02x",
				nv_act.act_sign[index_str]);
			/* length of "xx" is 3 */
			if ((str_len + 3) > PAGE_SIZE)
				break;
		}
		break;
	default:
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case SLE95250_SYSFS_ACT_VIA_NV:
		...
		if (kstrtol(buf, 10, &val) < 0 ||
			(val < 0) || (val > SLE95250_ACT_VIA_NV_VALID))
			hwlog_err("[%s] val is not valid!, ic_index:%u\n",
				__func__, di->ic_index);
		di->act_sign_via_nv = (int)val;
		break;
	default:
		break;
	}
...
#ifdef BATTBD_FORCE_MATCH
	ret = of_property_read_u32(np, "act_via_nv_flag", &di->act_sign_via_nv);
	if (ret) {
		hwlog_err("[%s] act_via_nv_flag not given in dts, ic_index:%u\n",
			__func__, di->ic_index);
		di->act_sign_via_nv = SLE95250_ACT_VIA_NV_DEFAULT;
	}
	hwlog_info("[%s] act_via_nv_flag = %u, ic_index:%u\n", __func__,
		di->act_sign_via_nv, di->ic_index);
#endif /* BATTBD_FORCE_MATCH */

	ret = of_property_read_u32(np, "core_num", &di->core_num);
	if (ret) {
		hwlog_err("[%s] core_num not given in dts\n", __func__);
		di->core_num = SLE95250_CORE_NUM;
	}
	hwlog_info("[%s] core_num = %u\n", __func__, di->core_num);
	return 0;
}
