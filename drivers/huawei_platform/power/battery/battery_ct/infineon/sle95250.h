...
#define SLE95250_NV_DATA_LEN           (sizeof(int) + sizeof(bool) + 2 * sizeof(uint8_t))
...
enum sle95250_nv_sign_status {
	...
	STATUS_BATT_SIGN_FINE,
	STATUS_BATT_SIGN_WRTIE,
	STATUS_BATT_SIGN_VERIFY,
	STATUS_BATT_SIGN_CLEAR,
};

struct sle95250_nv_act_sign {
	enum sle95250_nv_sign_status sign_status; /* 4Bytes */
	uint8_t act_sign[SLE95250_NV_ACT_LEN]; /* 80Bytes */
	bool cyclk_set_flag;
	uint8_t cyclk_status;
	uint8_t pglk_status;
	char reserved[SLE95250_NV_LEN - SLE95250_NV_ACT_LEN - SLE95250_NV_DATA_LEN];
};
#endif /* BATTBD_FORCE_MATCH */
