...
struct charge_core_data *charge_core_get_params(void)
{
	...
	int cbatt = power_platform_get_battery_ui_capacity();
...
static int charge_core_parse_dts_temp_para(struct charge_core_info *di)
{
	int i, row, col, len;
	int idata[TEMP_PARA_LEVEL * TEMP_PARA_TOTAL] = { 0 };

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), di->dev->of_node,
		"jeita_table", idata, TEMP_PARA_LEVEL, TEMP_PARA_TOTAL);
	if (len < 0)
		return -EINVAL;

	di->data.temp_level = len / TEMP_PARA_TOTAL;
	for (row = 0; row < len / TEMP_PARA_TOTAL; row++) {
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_TEMP_MIN;
		di->temp_para[row].temp_min = idata[col];
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_TEMP_MAX;
		di->temp_para[row].temp_max = idata[col];
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_IIN;
		di->temp_para[row].iin_temp = idata[col];
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_ICHG;
		di->temp_para[row].ichg_temp = idata[col];
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_VTERM;
		di->temp_para[row].vterm_temp = idata[col];
		col = row * TEMP_PARA_TOTAL + TEMP_PARA_TEMP_BACK;
		di->temp_para[row].temp_back = idata[col];
	}

	for (i = 0; i < di->data.temp_level; i++)
		hwlog_info("dts_temp_para[%d] %d %d %d %d %d %d\n",
			i, di->temp_para[i].temp_min, di->temp_para[i].temp_max,
			di->temp_para[i].iin_temp, di->temp_para[i].ichg_temp,
			di->temp_para[i].vterm_temp, di->temp_para[i].temp_back);
	return 0;
}

static int charge_core_parse_battery_temp_para(struct charge_core_info *di,
	struct chrg_para_lut *p_batt_data)
{
	int i;

	if (!charge_core_parse_dts_temp_para(di))
		return 0;
	...
	for (i = 0; i < (p_batt_data->temp_len) / TEMP_PARA_TOTAL; i++) {
		...
		hwlog_info("temp_para[%d] %d %d %d %d %d %d\n", i,
...
static int charge_core_parse_battery_volt_para(struct charge_core_info *di,
	struct chrg_para_lut *p_batt_data)
{
	int i;
	...
	for (i = 0; i < (p_batt_data->volt_len) / VOLT_PARA_TOTAL; i++) {
		...
		hwlog_info("volt_para[%d] %d %d %d %d %d\n", i, di->volt_para[i].vbat_min,
			di->volt_para[i].vbat_max, di->volt_para[i].iin_volt,
...
static int charge_core_parse_battery_segment_para(struct charge_core_info *di,
	struct chrg_para_lut *p_batt_data)
{
	int i;
	...
	for (i = 0; i < di->data.segment_level; i++) {
		...
		hwlog_info("segment_para[%d] %d %d %d %d %d\n", i, di->segment_para[i].vbat_min,
			di->segment_para[i].vbat_max, di->segment_para[i].ichg_segment,
...
static int charge_core_parse_battery_data(struct charge_core_info *di)
{
	int ret;
	...
	ret = charge_core_parse_battery_temp_para(di, p_batt_data);
	if (ret) {
		hwlog_err("parse battery temp para fail\n");
		return -EINVAL;
	}

	ret = charge_core_parse_battery_volt_para(di, p_batt_data);
	if (ret) {
		hwlog_err("parse battery volt para fail\n");
		return -EINVAL;
	}

	ret = charge_core_parse_battery_segment_para(di, p_batt_data);
	if (ret) {
		hwlog_err("parse battery volt para fail\n");
		return -EINVAL;
	}
...
static int charge_core_probe(struct platform_device *pdev)
{
	...
	ret = charge_core_parse_battery_data(di);
	if (ret < 0)
		goto fail_free_mem;
