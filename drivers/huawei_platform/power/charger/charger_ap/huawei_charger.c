...
static bool charge_check_pd_ufcs_adapter_vbus(void)
{
...
static void charger_type_handler(unsigned long type, void *data)
{
	...
	case PD_DPM_VBUS_TYPE_PD:
		...
		if (last_type == CHARGER_TYPE_STANDARD &&
			...
			charge_check_pd_ufcs_adapter_vbus();
			hwlog_info("%s pd switch to dcp already, ignore this pd charger type event\n", __func__);
			break;
		}
