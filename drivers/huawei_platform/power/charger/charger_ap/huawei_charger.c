...
static int sysfs_charger_cvcal(struct charge_device_info *di, long val)
{
	...
	if (!di || !charge_ops_valid(get_dieid_for_nv) || !charge_ops_valid(set_pretrim_code))
		return -1;

	ret = charge_get_dieid_for_nv(dieid, CHGDIEID_LEN);
	if (ret) {
	...
	ret = charge_set_pretrim_code(di->sysfs_data.charger_cvcal_value);
	if (ret) {
...
static int sysfs_charger_cvcal_clear(struct charge_device_info *di, long val)
{
	...
	if (!di || !charge_ops_valid(set_pretrim_code))
		return -1;
...
static int dcp_set_enable_charger(unsigned int val)
{
	...
	charge_set_charge_enable(di->sysfs_data.charge_enable);
#ifdef CONFIG_DIRECT_CHARGER
...
static int dcp_set_ichg_thermal(unsigned int val)
{
	...
	if (di->charge_current > di->sysfs_data.ichg_thl)
		charge_set_charge_current(di->sysfs_data.ichg_thl);
...
static bool fcp_charge_precheck(unsigned int fcp_charge_enable, unsigned int type)
{
	...
	if (!fcp_charge_enable || !charge_fcp_enable()) {
...
static void fcp_charge_check(struct charge_device_info *di, unsigned int type)
{
	...
	if (!fcp_charge_precheck(di->sysfs_data.fcp_charge_enable, type))
		return;
	...
	if (term_allow && (info->ichg < (int)di->core_data->iterm) &&
		...
		if (di->check_full_count > BATTERY_FULL_CHECK_TIMIES) {
			...
			hwlog_info("capacity = %d, ichg = %d, ichg_avg = %d\n",
				power_supply_app_get_bat_capacity(), info->ichg, info->ichg_avg);
		}
	} else {
...
static void charge_full_dmd_report(struct charge_device_info *di, int is_battery_full)
{
	...
	switch (di->charge_full_check_type) {
	case CHARGE_FULL_CHECK_DEFAULT:
		vbat_main = power_supply_app_get_bat_voltage_now();
...
static void charge_stop_charging(struct charge_device_info *di)
{
	...
	if (charge_ops_valid(set_adc_conv_rate))
		charge_set_adc_conv_rate(di->sysfs_data.adc_conv_rate);
	di->check_full_count = 0;
...
static void charger_software_recharge(struct charge_device_info *di)
{
	...
	ret += charge_set_charge_enable(TRUE & di->sysfs_data.charge_enable);
	if (ret)
		hwlog_err("[%s]set charge enable fail\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	case CHARGE_SYSFS_VBUS:
		charge_get_vbus(&di->sysfs_data.vbus);
		return snprintf(buf, PAGE_SIZE, "%d\n", di->sysfs_data.vbus);
	...
	case CHARGE_SYSFS_VOLTAGE_SYS:
		...
		if (charge_ops_valid(get_vbat_sys))
			di->sysfs_data.voltage_sys = charge_get_vbat_sys();
	...
	case CHARGE_SYSFS_CHARGER_GET_CVSET:
		...
		if (charge_ops_valid(get_terminal_voltage))
			di->sysfs_data.charger_get_cvset = charge_get_terminal_voltage();
		return snprintf(buf, PAGE_SIZE, "%d\n",
			di->sysfs_data.charger_get_cvset);
	case CHARGE_SYSFS_IS_TRIM_CHARGER:
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case CHARGE_SYSFS_ICHG_RUNNINGTEST:
		...
		if (di->charge_current > di->sysfs_data.ichg_rt)
			charge_set_charge_current(di->sysfs_data.ichg_rt);
	...
	case CHARGE_SYSFS_REGULATION_VOLTAGE:
		...
		charge_set_terminal_voltage(di->sysfs_data.vterm_rt);
	...
	case CHARGE_SYSFS_DBC_CHARGE_CONTROL:
		if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > 2))
			return -EINVAL;
		if (!charge_ops_valid(set_force_term_enable)) {
			ret = charge_set_force_term_enable((int)val);
			if (ret)
