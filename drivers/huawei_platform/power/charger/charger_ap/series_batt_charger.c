...
#define SERIES_BATT_CAP_FULL_MIN    97
#define SERIES_BATT_CAP_FULL_MAX    100
#define CHARGE_DONE_SET_DELAY_TIME  120000 /* 2minutes */
...
struct series_batt_info {
	...
	int vbat_min;
	int adc_offset;
	int high_temp_status;
	int high_temp_delta_vterm;
	int high_temp_hysteresis;
	u64 usb_connect_time;
	bool en_chg_done_chk;
};
	...
	int term_vol, int vterm_signal)
{
	...
	if (!di->g_ops->get_terminal_voltage) {
		...
		if (cccv_pre < 0)
			...
			cccv_pre = cccv_pre * bc_ic_get_convert_ratio();
	}
...
static int series_batt_get_ircomp(struct series_batt_info *di)
{
	...
	if ((di->i_avg >= di->ircomp_i_min) && (di->i_avg < IAVG_MAX)) {
		...
		vol_ir = (i_avg * di->ircomp_r) / POWER_UV_PER_MV;
...
static int series_batt_get_valid_vterm(struct series_batt_info *di, int vol, int term_vol)
{
	int vol_ir, term_vol_max;
	...
	if (coul_drv_is_smart_battery())
		term_vol_max = coul_drv_battery_vbat_max() * BATTERY_NUM + vol_ir + di->vterm_comp;
	else
		term_vol_max = vol + di->vbat_max + vol_ir + di->vterm_comp;
	term_vol = min(term_vol, term_vol_max);
	term_vol = max(term_vol, VTERM_MIN);
...
static int series_batt_set_vterm(struct series_batt_info *di)
{
	...
	if (coul_drv_is_smart_battery())
		term_vol = vol * BATTERY_NUM;
	else
		term_vol = min(di->vbat_min + vol,  vol * BATTERY_NUM);
	hwlog_info("term_vol:%d\n", term_vol);

	term_vol = series_batt_get_valid_vterm(di, vol, term_vol);
	di->last_cccv = term_vol;
	term_vol = term_vol / bc_ic_get_convert_ratio();
...
static void series_batt_set_vterm_work(struct work_struct *work)
{
	...
	di->i_avg = charge_get_battery_current_avg();
	di->ichg = -power_platform_get_battery_current();
...
static bool series_batt_is_valid_vol(int vol)
{
	return vol >= (VTERM_MIN / bc_ic_get_convert_ratio());
}
...
static int series_batt_set_terminal_voltage(int vol)
{
	...
	if (coul_drv_is_smart_battery()) {
		desired_vol = coul_drv_get_desired_charging_voltage() / bc_ic_get_convert_ratio();
		hwlog_info("smart battery, desired charge voltage: %d", desired_vol);
		if (!series_batt_is_valid_vol(desired_vol))
			return 0;

		bat_temp_get_rt_temperature(BAT_TEMP_MIXED, &batt_temp);
		if (batt_temp <= WARM_LIMIT - di->high_temp_hysteresis)
			di->high_temp_status = 0;

		if (batt_temp > WARM_LIMIT || (di->high_temp_status == 1)) {
			desired_vol -= di->high_temp_delta_vterm;
			di->high_temp_status = 1;
			hwlog_info("battery in high temp, vterm=%d\n", desired_vol);
		}

		vol = min(vol, desired_vol);
	}
...
static int series_batt_set_charge_current(int value)
{
	...
	if (coul_drv_is_smart_battery()) {
		desired_cur = coul_drv_get_desired_charging_current() * bc_ic_get_convert_ratio();
		hwlog_info("smart battery, desired charge current: %d", desired_cur);
		value = min(value, desired_cur);
	}
...
/* only set charge done state after 2 minutes when usb_connect. */
static bool series_batt_charge_done_timer(struct series_batt_info *di)
{
	u64 chg_done_now_time;

	if (!di)
		return true;

	if (di->en_chg_done_chk)
		return true;

	chg_done_now_time = get_jiffies_64();
	if (!time_after64(chg_done_now_time,
		di->usb_connect_time + msecs_to_jiffies(CHARGE_DONE_SET_DELAY_TIME)))
		return false;
	hwlog_info("delay 2 minutes done!\n");
	di->en_chg_done_chk = true;
	return true;
}
...
static int series_batt_get_charge_state(unsigned int *state)
{
	...
	if (coul_drv_is_smart_battery()) {
		desired_cur = coul_drv_get_desired_charging_current();
		desired_vol = coul_drv_get_desired_charging_voltage();
		cap = coul_drv_battery_unfiltered_capacity();
		if ((desired_cur == 0) && (desired_vol == 0) &&
			((cap > SERIES_BATT_CAP_FULL_MIN) && (cap <= SERIES_BATT_CAP_FULL_MAX)) &&
			(series_batt_charge_done_timer(di))) {
			*state |= CHAGRE_STATE_CHRG_DONE;
			hwlog_info("smart series battery charge done\n");
		}
	}
...
static int series_batt_chip_init(struct charge_init_data *init_crit)
{
	struct series_batt_info *di = g_series_batt_di;
...
static int series_batt_set_term_enable(int enable)
{
	struct series_batt_info *di = g_series_batt_di;

	if (!di || !di->g_ops || !di->g_ops->set_term_enable) {
		hwlog_err("g_ops or set_term_enable is null\n");
		return -1;
	}

	if (coul_drv_is_smart_battery())
		enable = di->high_temp_status;

	return di->g_ops->set_term_enable(enable);
}
...
int series_batt_ops_register(struct charge_device_ops *ops)
{
	...
	di->local_ops.set_term_enable = series_batt_set_term_enable;

	ret = charge_ops_register(&di->local_ops, BUCK_IC_TYPE_THIRDPARTY);
	if (ret) {
...
static int get_series_batt_chargelog(char *chargelog, int size, void *dev_data)
{
	...
	snprintf(chargelog, size,
		"%-9d   %-9d   %-9d   %-8d   %-8d   %-10d   %-4d   %-4d   ",
		bat0_temp, bat1_temp, mixed_temp,
		bat0_vol, bat1_vol, series_vol, ibus, vbus);
	return 0;
}

static int series_batt_recv_usb_connect_status_notifier_call(struct notifier_block *connect_nb,
	unsigned long event, void *data)
{
	...
	di = container_of(connect_nb, struct series_batt_info, connect_nb);
	if (!di)
		return NOTIFY_OK;

	switch (event) {
	case POWER_NE_USB_DISCONNECT:
		/* when usb disconnect, never stop set charge done state */
		di->en_chg_done_chk = true;
		break;
	case POWER_NE_USB_CONNECT:
		di->en_chg_done_chk = false;
		di->usb_connect_time = get_jiffies_64();
		break;
	default:
		return NOTIFY_OK;
	}
	...
	struct series_batt_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"adc_offset", &di->adc_offset, ADC_OFFSET);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"high_temp_delta_vterm", &di->high_temp_delta_vterm, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"high_temp_hysteresis", &di->high_temp_hysteresis, 1);
}
...
static int series_batt_probe(struct platform_device *pdev)
{
	...
	di->connect_nb.notifier_call = series_batt_recv_usb_connect_status_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_CONNECT, &di->connect_nb);
	if (ret)
		hwlog_err("register usb_connect_status notifier failed\n");
