...
#define UC_ERROR_RESISTANCE              (-99999)
#define UC_MAX_RESISTANCE                10000
#define UC_STEP_VOL_START                6000
#define UC_BATTERY_CAPACITY_HIGH_TH      95
#define UC_VBUS_ON_THRESHOLD             3000
#define UC_VBAT_VBUS_DIFFERENCE          150
#define UC_OPEN_RETRY_CNT_MAX            3

/*
 * define multistage (cc)constant current and (cv)constant voltage
 * support up to 5 parameters list on dts
 */
#define UC_VOLT_LEVEL                    8

enum uvdm_charge_volt_info {
	UC_PARA_VOL_TH = 0,
	UC_PARA_CUR_TH_HIGH,
	UC_PARA_CUR_TH_LOW,
	UC_PARA_VOLT_TOTAL,
 ...
 */
#define UC_VOLT_GROUP_MAX                8
#define UC_BAT_BRAND_LEN_MAX             64
#define UC_VOLT_NODE_LEN_MAX             16

enum uvdm_charge_bat_info {
	UC_PARA_BAT_ID = 0,
	UC_PARA_TEMP_LOW,
	UC_PARA_TEMP_HIGH,
	UC_PARA_INDEX,
...
struct uvdm_charge_bat_para {
	int temp_low;
	int temp_high;
	int parse_ok;
	char batid[UC_BAT_BRAND_LEN_MAX];
	char volt_para_index[UC_VOLT_NODE_LEN_MAX];
};

struct uvdm_charge_volt_para_group {
	struct uvdm_charge_volt_para volt_info[UC_VOLT_LEVEL];
	struct uvdm_charge_bat_para bat_info;
	int stage_size;
};
 ...
 * support up to 5 parameters list on dts
 */
#define UC_TEMP_LEVEL                    5

enum uvdm_charge_temp_info {
	UC_TEMP_MIN = 0,
	UC_TEMP_MAX,
	UC_TEMP_CUR_MAX,
	UC_TEMP_TOTAL,
...
struct uvdm_charge_temp_para {
	int temp_min;
	int temp_max;
	int temp_cur_max;
};

static struct uvdm_charge_device {
	struct device *dev;
	...
	struct uvdm_charge_volt_para volt_para[UC_VOLT_LEVEL];
	struct uvdm_charge_volt_para orig_volt_para[UC_VOLT_LEVEL];
	struct uvdm_charge_volt_para_group orig_volt_para_p[UC_VOLT_GROUP_MAX];
	struct uvdm_charge_temp_para temp_para[UC_TEMP_LEVEL];
	int stage_need_to_jump[2 * UC_VOLT_LEVEL];
	int error_cnt;
	int working_mode;
	int sysfs_enable_charger;
	unsigned int sysfs_disable_charger[UVDM_DISABLE_END];
...
static struct uvdm_charge_device *g_uvdm_di;
...
{
	struct uvdm_charge_device *l_di = g_uvdm_di;
...
static int uvdm_set_disable_flags(int val, int type)
{
	...
	struct uvdm_charge_device *di = g_uvdm_di;
...
static int uvdm_set_enable_charger(unsigned int val)
{
	struct uvdm_charge_device *di = g_uvdm_di;
	int ret;
	...
	ret = uvdm_set_disable_flags((val ?
		UVDM_CLEAR_DISABLE_FLAGS : UVDM_SET_DISABLE_FLAGS),
		UVDM_DISABLE_SYS_NODE);
	hwlog_info("set enable_charger=%d\n", di->sysfs_enable_charger);
	return ret;
}

static int uvdm_get_enable_charger(unsigned int *val)
{
	struct uvdm_charge_device *di = g_uvdm_di;
...
static int uvdm_set_iin_limit(unsigned int val)
{
	...
	if ((di->stage_size < 1) || (di->stage_size > UC_VOLT_LEVEL)) {
...
static struct power_if_ops uvdm_if_ops = {
	.set_enable_charger = uvdm_set_enable_charger,
	.get_enable_charger = uvdm_get_enable_charger,
...
int uvdm_charge_cutoff_reset_param(void)
{
	...
	di->full_path_resistance = UC_ERROR_RESISTANCE;
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct uvdm_charge_device *di = dev_get_drvdata(dev);
...
static int uvdm_dc_check_adapter_antifake(void)
{
	...
	return adapter_auth_encrypt_start(ADAPTER_PROTOCOL_UVDM,
...
static int uvdm_charge_check_cover_voltage_accuracy(void)
{
	...
	for (i = 0; i < retry; ++i) {
		...
		hwlog_info("[%d]: Verr=%d, Verr_th=%d, Vset=%d, Vread=%d, Vbus=%d\n",
			...
			vadapt, power_supply_app_get_usb_voltage_now());
...
static int uvdm_adjust_voltage_until_current_rise(
	struct uvdm_charge_device *l_di)
{
	int vbat = 0;
	int ibus = 0;
	int vbat_max;
	int vbat_max_temp = coul_drv_battery_vbat_max();
	int adjust_times = 30; /* max times for set adapter vol */
	int soc;
...
static void uvdm_charge_init_adaptor_vbus(struct uvdm_charge_device *l_di)
{
	int vbat = 0;
...
static void uvdm_charge_regulation_core(struct uvdm_charge_device *di)
{
	/* cc and cv stage */
	if (di->cur_stage % 2) {
...
{
	struct uvdm_charge_device *di = NULL;
	int interval;
	...
	di = container_of(work, struct uvdm_charge_device, control_work);
	if (!di) {
		hwlog_err("di is null\n");
		return;
	}
...
void uvdm_charge_calc_thld_work(struct work_struct *work)
{
	...
	di = container_of(work, struct uvdm_charge_device, calc_thld_work);
	if (!di) {
		hwlog_err("di is null\n");
		return;
	}
...
enum hrtimer_restart uvdm_charge_calc_thld_timer_func(struct hrtimer *timer)
{
	...
	di = container_of(timer, struct uvdm_charge_device, calc_thld_timer);
	if (!di) {
		hwlog_err("di is null\n");
		return HRTIMER_NORESTART;
	}
...
enum hrtimer_restart uvdm_charge_control_timer_func(struct hrtimer *timer)
{
	...
	di = container_of(timer, struct uvdm_charge_device, control_timer);
	if (!di) {
		hwlog_err("di is null\n");
		return HRTIMER_NORESTART;
	}
...
static void uvdm_charge_stop_charging_disconnect_check(void)
{
	...
	uvdm_charge_get_device_vbus(&vbus);
	uvdm_charge_get_bat_voltage(&vbat);
	if ((vbus < UC_VBUS_ON_THRESHOLD) ||
		(vbat - vbus) > UC_VBAT_VBUS_DIFFERENCE) {
...
static int uvdm_sc_mode_precheck(struct uvdm_charge_device *di)
{
	...
	if (di->error_cnt >= UVDM_ERR_CNT_MAX) {
		...
		if ((di->dc_err_report_flag == false) &&
			(di->dc_open_retry_cnt <= UC_OPEN_RETRY_CNT_MAX))
...
static int uvdm_sc_mode_adaptor_check(struct uvdm_charge_device *di)
{
	if (g_bucktosc_flag) {
...
static int uvdm_sc_mode_battery_check(struct uvdm_charge_device *di)
{
...
static int uvdm_sc_mode_init_charging_path(struct uvdm_charge_device *di)
{
	unsigned int stage;
...
static int uvdm_sc_mode_start_secure_charge(struct uvdm_charge_device *di)
{
	int stage;
	...
	power_event_bnc_notify(POWER_BNT_SOC_DECIMAL, POWER_NE_SOC_DECIMAL_DC, &max_pwr);
...
static void uvdm_sc_fault_work(struct work_struct *work)
{
	...
	di = container_of(work, struct uvdm_charge_device, fault_work);
	if (!di)
		return;
	...
	unsigned long event, void *data)
{
	...
	di = container_of(nb, struct uvdm_charge_device, fault_nb);
	if (!di)
		return NOTIFY_OK;

	if (event == POWER_NE_UVDM_FAULT_OTG) {
		g_otg_flag = *((int *)data);
		hwlog_info("otg event:flag = %d\n", g_otg_flag);
	} else if (event == POWER_NE_UVDM_FAULT_COVER_ABNORMAL) {
...
static void uvdm_sc_init_resource(void)
{
	...
	g_uvdm_di->charging_lock = power_wakeup_source_register(g_uvdm_di->dev,
		"uvdm_wakelock");
...
static void uvdm_charge_parse_adapter_antifake_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
...
static int uvdm_charge_parse_volt_para(struct device_node *np,
	struct uvdm_charge_device *di, int group)
{
	int i, row, col, len;
	int idata[UC_VOLT_LEVEL * UC_PARA_VOLT_TOTAL] = { 0 };
	int j = group;
	char *volt_para = NULL;
	...
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		volt_para, idata, UC_VOLT_LEVEL, UC_PARA_VOLT_TOTAL);
	if (len < 0)
		return -EINVAL;

	di->orig_volt_para_p[j].stage_size = len / UC_PARA_VOLT_TOTAL;
	hwlog_info("%s stage_size=%d\n", volt_para,
		di->orig_volt_para_p[j].stage_size);

	for (row = 0; row < len / UC_PARA_VOLT_TOTAL; row++) {
		col = row * UC_PARA_VOLT_TOTAL + UC_PARA_VOL_TH;
		idata[col] -= power_platform_get_cv_limit();
		di->orig_volt_para_p[j].volt_info[row].vol_th = idata[col];
		col = row * UC_PARA_VOLT_TOTAL + UC_PARA_CUR_TH_HIGH;
		di->orig_volt_para_p[j].volt_info[row].cur_th_high = idata[col];
		col = row * UC_PARA_VOLT_TOTAL + UC_PARA_CUR_TH_LOW;
...
static void uvdm_charge_parse_group_volt_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
	int i;
	...
	for (i = 0; i < di->stage_group_size; i++) {
		if (uvdm_charge_parse_volt_para(np, di, i))
			return;
	}
...
static void uvdm_charge_parse_bat_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
	...
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"bat_para", UC_VOLT_GROUP_MAX, UC_PARA_BAT_TOTAL);
	if (array_len < 0) {
		...
		strncpy(di->orig_volt_para_p[0].bat_info.batid,
			"default", UC_BAT_BRAND_LEN_MAX - 1);
		strncpy(di->orig_volt_para_p[0].bat_info.volt_para_index,
			"volt_para", UC_VOLT_NODE_LEN_MAX - 1);
		return;
	}

	di->stage_group_size = array_len / UC_PARA_BAT_TOTAL;
	...
	for (i = 0; i < array_len; i++) {
		...
		row = i / UC_PARA_BAT_TOTAL;
		col = i % UC_PARA_BAT_TOTAL;

		switch (col) {
		case UC_PARA_BAT_ID:
			strncpy(di->orig_volt_para_p[row].bat_info.batid,
				tmp_string, UC_BAT_BRAND_LEN_MAX - 1);
			break;
		case UC_PARA_TEMP_LOW:
		...
		case UC_PARA_INDEX:
			strncpy(di->orig_volt_para_p[row].bat_info.volt_para_index,
				tmp_string, UC_VOLT_NODE_LEN_MAX - 1);
			break;
		default:
			break;
		}
...
static void uvdm_charge_parse_temp_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
	int row, col, len;
	int idata[UC_TEMP_LEVEL * UC_TEMP_TOTAL] = { 0 };

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"temp_para", idata, UC_TEMP_LEVEL, UC_TEMP_TOTAL);
	if (len < 0)
		return;

	for (row = 0; row < len / UC_TEMP_TOTAL; row++) {
		col = row * UC_TEMP_TOTAL + UC_TEMP_MIN;
		di->temp_para[row].temp_min = idata[col];
		col = row * UC_TEMP_TOTAL + UC_TEMP_MAX;
		di->temp_para[row].temp_max = idata[col];
		col = row * UC_TEMP_TOTAL + UC_TEMP_CUR_MAX;
...
static int uvdm_charge_parse_stage_jump_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
	...
	/* 2: cc and cv stage */
	for (i = 0; i < 2 * UC_VOLT_LEVEL; ++i)
		di->stage_need_to_jump[i] = -1;

	/* 2: cc and cv stage */
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"stage_need_to_jump", 2 * UC_VOLT_LEVEL, 1);
	if (array_len < 0)
		return -EINVAL;
...
static void uvdm_charge_parse_basic_para(struct device_node *np,
	struct uvdm_charge_device *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
...
int uvdm_charge_parse_dts(struct device_node *np, void *p)
{
	struct uvdm_charge_device *di = (struct uvdm_charge_device *)p;
	...
	if (uvdm_charge_parse_stage_jump_para(np, di))
		return -EINVAL;

	uvdm_charge_parse_basic_para(np, di);
	uvdm_charge_parse_temp_para(np, di);
	uvdm_charge_parse_bat_para(np, di);
	uvdm_charge_parse_group_volt_para(np, di);
	uvdm_charge_parse_adapter_antifake_para(np, di);
	return 0;
}
...
static int uvdm_sc_probe(struct platform_device *pdev)
{
	...
	di->dev = &pdev->dev;
	np = di->dev->of_node;
	ret = uvdm_charge_parse_dts(np, di);
	if (ret)
		goto fail_free_mem;
	...
	di->fault_nb.notifier_call = uvdm_charge_fault_notifier_call;
	ret = power_event_anc_register(POWER_ANT_UVDM_FAULT, &di->fault_nb);
	if (ret < 0)
		goto fail_register;
...
{
	struct uvdm_charge_device *di = platform_get_drvdata(pdev);

	if (!di)
		return -ENODEV;

	power_event_anc_unregister(POWER_ANT_UVDM_FAULT, &di->fault_nb);
	uvdm_charge_sysfs_remove_group(di->dev);
	power_wakeup_source_unregister(di->charging_lock);
	devm_kfree(&pdev->dev, di);
	g_uvdm_di = NULL;
