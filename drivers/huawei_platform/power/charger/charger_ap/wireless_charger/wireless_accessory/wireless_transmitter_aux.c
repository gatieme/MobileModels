...
static int wltx_aux_set_dev_attr(struct wltx_aux_dev_info *di, u16 attr)
{
	return wireless_set_dev_attr(WLTRX_IC_AUX, WIRELESS_PROTOCOL_QI, attr);
}
...
static int wltx_aux_get_acc_dev_sle(struct wltx_aux_dev_info *di)
{
	if (!di->wireless_tx_acc) {
		hwlog_err("%s: wireless_tx_acc is null\n", __func__);
		return -EINVAL;
	...
	if (di->support_sle && di->acc_support_sle)
		di->wireless_tx_acc->dev_sle = 1;
	else
		di->wireless_tx_acc->dev_sle = 0;

	hwlog_info("%s: wireless_tx_acc dev_sle = 0x%x\n",
		__func__, di->wireless_tx_acc->dev_sle);
	return 0;
}
...
static int wltx_aux_get_acc_info(struct wltx_aux_dev_info *di)
{
	...
	ret |= wltx_aux_get_acc_dev_sle(di);
	if (ret) {
...
{
	u8 info_no = g_wltx_aux_di->support_sle ? WL_TX_ACC_INFO_MAX : WL_TX_ACC_INFO_MAX - 1;
	...
	if (g_wltx_aux_di->support_sle)
		snprintf(g_wl_acc_aux_info_tab[WL_TX_ACC_INFO_SLE].value,
			ACC_VALUE_MAX_LEN, "%x", di->dev_sle);
	wireless_acc_report_uevent(g_wl_acc_aux_info_tab, info_no, di->dev_no);
...
static void wltx_aux_start_check_work(struct work_struct *work)
{
	...
	power_event_bnc_notify(POWER_BNT_CONNECT, POWER_NE_WIRELESS_AUX_TX_START, NULL);
	coul_drv_charger_event_rcv(WIRELESS_TX_STATUS_CHANGED);
...
static void wltx_aux_event_work(struct work_struct *work)
{
	...
	case POWER_NE_WLTX_TX_Q_CALIBRATION:
		...
		break;
	case POWER_NE_WLTX_SUPPORT_SLE:
		di->acc_support_sle = di->tx_event_data;
		break;
	default:
...
static void wltx_aux_get_tx_info(struct wltx_aux_dev_info *di)
{
	u8 bigdata_info[WLTX_AUX_TX_INFO_SIZE] = {0};
	int str_len = 0;
	int i;
	int size;
	...
	size = sizeof(di->bigdata_info);
	memset(di->bigdata_info, 0, size);
	if (g_tx_open_flag) {
		wltx_aux_get_acc_dev_no(di);

		bigdata_info[WLTX_AUX_TX_MANUFACTURER] = DEFAULT_MANUFACTURER;
		if (di->wireless_tx_acc->dev_no == ACC_DEV_NO_PEN) {
			bigdata_info[WLTX_AUX_TX_ID] = HWQI_PEN_CD54ID;
			bigdata_info[WLTX_AUX_TX_TYPE] = HWQI_PEN;
		} else if (di->wireless_tx_acc->dev_no == ACC_DEV_NO_KB) {
			bigdata_info[WLTX_AUX_TX_ID] = HWQI_KB_MATEPADID;
			bigdata_info[WLTX_AUX_TX_TYPE] = HWQI_KB;
		}
		bigdata_info[WLTX_AUX_TX_SOFT_VERSION] = DEFAULT_TX_SOFT_VER;

		for (i = 0; i < sizeof(bigdata_info); i++) {
			snprintf(di->bigdata_info + str_len, size - str_len,
				"%02x", bigdata_info[i]);
...
static void wltx_aux_q_calibration(long val)
{
	...
	wltx_ic_q_calibration(WLTRX_IC_AUX);
}
...
void wltx_aux_open_tx(enum wltx_aux_open_type type, bool enable)
{
	...
	if (g_tx_open_flag) {
		...
		if (support_switch) {
			...
			power_event_bnc_notify(POWER_BNT_CONNECT, POWER_NE_WIRELESS_AUX_TX_STOP,
				NULL);
...
static int wltx_aux_probe(struct platform_device *pdev)
{
	...
	if (!wltx_ic_is_ops_registered(WLTRX_IC_AUX))
		return -EPROBE_DEFER;
	...
	if (di->support_sle)
		wltx_aux_set_dev_attr(di, HWQI_TX_SUPPORT_SLE);
