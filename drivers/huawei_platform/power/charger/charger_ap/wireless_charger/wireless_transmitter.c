...
static bool g_coolingcase_can_work = true;

#ifdef CONFIG_CHARGER_OHOS
static void wireless_tx_send_tx_status_event(void)
{
	int len;
	char tx_status_event[POWER_EVENT_NOTIFY_SIZE] = { 0 };

	len = snprintf_s(tx_status_event, POWER_EVENT_NOTIFY_SIZE, POWER_EVENT_NOTIFY_SIZE - 1,
		"WIRELESS_TX_STATUS=%d", tx_status);
	if (len <= 0)
		return;

	hwlog_info("wireless_tx_status=%d\n", tx_status);
	power_event_report_str_uevent(tx_status_event);
}
#endif

static void wireless_tx_send_event(void)
{
#ifdef CONFIG_CHARGER_OHOS
	wireless_tx_send_tx_status_event();
#else
...
static int wltx_check_abnormal_ibatt(void)
{
	...
	} else if (di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) {
		idischrg_max = WLTX_ABNORMAL_IBAT_2;
	} else {
...
static void wltx_chk_abnormal_tx_vin(void)
{
	...
	if (!strstr(src, "CP") || strstr(src, "RVS_SC_CP"))
		return;
...
int wltx_get_tx_ilimit(int charger_type, int ibuck)
{
	...
	if ((di->cur_pwr_sw_scn == PWR_SW_BY_DC_DONE) &&
		((di->cur_pwr_src == PWR_SRC_VBUS_CP) || (di->cur_pwr_src == PWR_SRC_VBUS2)))
...
static void wltx_check_timeout(struct wltx_dev_info *di)
{
	if (di->is_keyboard_online && (di->tx_timeout_cfg > 0) && time_after(jiffies, di->tx_timeout) &&
		(wlrx_get_wired_channel_state() == WIRED_CHANNEL_OFF)) {
		wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
		di->stop_reverse_charge = true;
		hwlog_info("[check_timeout] tx timeout over %dms, charge_stop\n", di->tx_timeout_cfg);
	}
}
...
static void wireless_tx_dsm_dump(struct wltx_dev_info *di, char *dsm_buff)
{
	...
	charge_get_vbus(&charger_vbus);
	if (ret)
		hwlog_err("%s: get tx vin/iin/vrect... fail", __func__);
	snprintf(buff, sizeof(buff),
		"soc=%d tbatt=%d init_tbatt=%d charger_vbus=%dmV "
		"tx_vrect=%umV tx_vin=%umV tx_iin=%umA tx_iin_avg=%umA chip_temp=%d\n",
		soc, tbatt, g_init_tbatt, charger_vbus, tx_vrect, tx_vin, tx_iin,
		di->tx_iin_avg, chip_temp);
...
static void wireless_tx_enable_tx_mode(bool enable)
{
	if (wlrx_get_wired_channel_state() == WIRED_CHANNEL_OFF &&
		!wlrx_get_screen_stop_charging_state()) {
...
static void wltx_select_vset_para(struct wltx_dev_info *di)
{
	if (di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) {
		di->tx_vset.v_ps = di->sc14_dts_vset.v_ps;
		di->tx_vset.v_ping = di->sc14_dts_vset.v_ping;
		di->tx_vset.v_hs = di->sc14_dts_vset.v_hs;
		di->tx_vset.v_dflt = di->sc14_dts_vset.v_dflt;
	} else {
		di->tx_vset.v_ps = di->dts_vset.v_ps;
		di->tx_vset.v_ping = di->dts_vset.v_ping;
		di->tx_vset.v_hs = di->dts_vset.v_hs;
		di->tx_vset.v_dflt = di->dts_vset.v_dflt;
	}
}
...
static int wireless_tx_power_supply(struct wltx_dev_info *di)
{
	...
	wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
	wireless_tx_set_tx_open_flag(false);
	wireless_tx_dsm_report(POWER_DSM_ERROR_WIRELESS_TX_POWER_SUPPLY_FAIL, dsm_buff);
	return WL_TX_FAIL;
}

static void wltx_get_hwid_ilim(struct wltx_dev_info *di, u16 *ilim, u16 *imax)
{
	int i;
	u16 tx_vin = 0;
	struct wltx_dts *dts = wltx_get_dts();

	if (!dts || (di->cur_pwr_src == PWR_SRC_NULL))
		return;

	(void)wltx_ic_get_vin(WLTRX_IC_MAIN, &tx_vin);
	hwlog_info("[%s] tx_vin = %dmV", __func__, tx_vin);
	for (i = 0; i < dts->dflt_ilim_cfg_level; i++) {
		if ((dts->dflt_ilim_cfg[i].pwr_src > 0) &&
			(dts->dflt_ilim_cfg[i].pwr_src != di->cur_pwr_src))
			continue;
		if ((dts->dflt_ilim_cfg[i].vin_min > 0) &&
			(tx_vin <= dts->dflt_ilim_cfg[i].vin_min))
			continue;
		if ((dts->dflt_ilim_cfg[i].vin_max > 0) &&
			(tx_vin >= dts->dflt_ilim_cfg[i].vin_max))
			continue;
		if (ilim)
			*ilim = (u16)dts->dflt_ilim_cfg[i].ilim;
		if (imax)
			*imax = (u16)dts->dflt_ilim_cfg[i].imax;
		break;
	}
}

static bool wltx_rxid_in_ilim_whitelist(int rx_product_id, struct wltx_dts *dts)
{
	int i;

	for (i = 0; i < dts->whitelist_ilim_level; i++) {
		if (dts->whitelist_ilim[i].rx_id == rx_product_id)
			return true;
	}
...
static int wltx_get_whitelist_ilim(struct wltx_dev_info *di,
	struct wltx_dts *dts, u16 *ilim, u16 *imax)
{
	int i;

	for (i = 0; i < dts->whitelist_ilim_level; i++) {
		if (dts->whitelist_ilim[i].rx_id != di->rx_product_id)
			continue;
		if ((dts->whitelist_ilim[i].pwr_src > 0) &&
			(dts->whitelist_ilim[i].pwr_src != di->cur_pwr_src))
			continue;
		if (ilim)
			*ilim = (u16)dts->whitelist_ilim[i].ilim;
		if (imax)
			*imax = (u16)dts->whitelist_ilim[i].imax;
		return 0;
	}
...
static void wltx_get_ilim(struct wltx_dev_info *di, u16 *ilim, u16 *imax)
{
	u8 tx_cap_type;
	u16 tx_ilim;
	struct wltx_dts *dts = wltx_get_dts();
	...
	if (!dts)
		return;
	if (!wltx_get_whitelist_ilim(di, dts, ilim, imax))
		return;
	if ((dts->support_id_prevchk > 0) && di->prot_id_hwrx)
		wltx_get_hwid_ilim(di, ilim, imax);
}

static int wltx_set_ilim(struct wltx_dev_info *di)
{
	u16 tx_ilim;

	wltx_get_ilim(di, &tx_ilim, NULL);
	hwlog_info("[%s] tx_ilim = %u", __func__, tx_ilim);

	return wltx_ic_set_ilim(WLTRX_IC_MAIN, tx_ilim);
}

static int wltx_set_imax(struct wltx_dev_info *di)
{
	u16 tx_imax;

	wltx_get_ilim(di, NULL, &tx_imax);
	hwlog_info("[%s] tx_imax = %u", __func__, tx_imax);

	return wltx_ic_set_ilim(WLTRX_IC_MAIN, tx_imax);
}
...
static int wireless_tx_chip_init(struct wltx_dev_info *di)
{
	...
	(void)wltx_ic_set_ploss(WLTRX_IC_MAIN, di->rx_data.product_type);
	wltx_set_tx_volt(WL_TX_STAGE_CHIP_INIT, di->tx_vset.v_ping, true);
	ret = wireless_tx_limit_iout(di);
	if (ret)
		hwlog_err("%s: limit TX iout fail\n", __func__);
	(void)wltx_ic_set_rp_dm_to(WLTRX_IC_MAIN, di->tx_rp_demodule_timeout);
...
static void wltx_chip_reset(void)
{
	if (wltx_ic_is_in_rx_mode(WLTRX_IC_MAIN)) {
		hwlog_info("[%s] current is tx mode, ignore\n", __func__);
		return;
	}
...
static int wireless_tx_ping_rx(struct wltx_dev_info *di)
{
	...
	if (di->ping_timeout == WL_TX_PING_TIMEOUT_2) {
		wltx_chip_reset();
		if (di->ss_low_cnt > 0)
			ping_delay = WL_TX_PING_DELATY_2;
		if (wltx_msleep(ping_delay))
			return -EINVAL;
		ret = wireless_tx_chip_init(di);
		if (ret)
	...
	if (di->ping_timeout == WL_TX_PING_TIMEOUT_1) {
		...
		power_dsm_report_dmd(POWER_DSM_BATTERY, POWER_DSM_ERROR_WIRELESS_TX_STATISTICS, dsm_buff);
	}
	return WL_TX_FAIL;
}
...
static int wireless_tx_can_do_reverse_charging(void)
{
	...
	if (batt_temp >= WL_TX_BATT_TEMP_MAX) {
		...
		if (batt_temp - g_init_tbatt > WL_TX_TBATT_DELTA_TH)
			wireless_tx_dsm_report(POWER_DSM_ERROR_WIRELESS_TX_BATTERY_OVERHEAT, dsm_buff);
		return WL_TX_FAIL;
	}
	if (di->ss_low) {
		hwlog_err("%s: ss_low\n", __func__);
		di->ss_low = false;
		if (++di->ss_low_cnt <= WLTX_SS_LOW_MAX_CNT)
			wireless_tx_set_tx_status(WL_TX_STATUS_RX_DISCONNECT);
		else
			wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
		return WL_TX_FAIL;
	...
	if (wltx_ic_is_in_rx_mode(WLTRX_IC_MAIN) || wlrx_get_charge_stage() != WLRX_STAGE_DEFAULT) {
...
static bool wltx_rx_disconnect_need_power_off(struct wltx_dev_info *di)
{
	...
	if (di->ss_low_cnt > 0) {
		hwlog_info("[rx_disc_need_power_off] ss_low\n");
		goto no_need_pwroff;
	}
...
static void wireless_tx_fault_event_handler(struct wltx_dev_info *di)
{
	...
	case WL_TX_STATUS_COOLINGCASE_TIMEOUT:
		wireless_tx_set_tx_open_flag(false);
...
void wltx_cancel_work_handler(void)
{
...
static void wltx_ps_tx_volt_check(struct wltx_dev_info *di)
{
	...
	if ((dts->pwr_type == WL_TX_PWR_VBUS_OTG2) ||
		(dts->pwr_type == WL_TX_PWR_5VBST_VBUS_OTG2))
		return;

	if ((dts->pwr_type != WL_TX_PWR_5VBST_VBUS_OTG_CP) &&
		(dts->pwr_type != WL_TX_PWR_5VBST_OTG_RVSSC_CP) &&
		(dts->pwr_type != WL_TX_PWR_VBUS_RVSSC4_CP) &&
		(dts->pwr_type != WL_TX_PWR_5VBST_RVSSC4_CP))
		return wltx_ps_tx_vset_check(di);

	return wltx_ps_tx_vout_check(di);
}

static void wltx_ps_tx_ilim_check(struct wltx_dev_info *di)
{
	if ((di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) &&
		(di->rvs4_ilim_timeout > 0)) {
		if (time_after(jiffies, di->rvs4_ilim_timeout)) {
			di->rvs4_ilim_timeout = 0;
			wltx_ic_prevfod_check(WLTRX_IC_MAIN, WLTX_PREVFOD_ILIM_STOP);
		}
	}
}

static bool wltx_need_check_expect_power(struct wltx_dev_info *di, struct wltx_dts *dts)
{
	if (wltx_cap_get_cur_id(WLTRX_DRV_MAIN) != WLTX_HIGH_PWR_CAP)
		return false;
...
static bool wltx_need_sw2_default_power(struct wltx_dev_info *di, struct wltx_dts *dts)
{
	int soc;
	int tbatt = 0;
	int tbatt_max = WL_TX_HI_PWR_TBATT_MAX;

	soc = bci_show_capacity();
	bat_temp_get_temperature(BAT_TEMP_MIXED, &tbatt);
...
static void wltx_swith_to_default_power(struct wltx_dev_info *di)
{
	cancel_delayed_work_sync(&di->wltx_abnormal_power_check_work);
	wlps_control(WLTRX_IC_MAIN, WLPS_TX_SW, false);
	wltx_disable_pwr_supply();
...
static void wltx_check_expect_power(struct wltx_dev_info *di)
{
	...
	if ((wltx_cap_get_cur_id(WLTRX_DRV_MAIN) != WLTX_DFLT_CAP) &&
		wltx_rxid_in_ilim_whitelist(di->rx_product_id, dts))
		wltx_swith_to_default_power(di);

	if (wltx_need_check_expect_power(di, dts) &&
		wltx_need_sw2_default_power(di, dts))
		wltx_swith_to_default_power(di);
}

static void wltx_coolingcase_timeout1_work(struct work_struct *work)
{
	g_coolingcase_can_work = true;
	hwlog_info("[coolingcase_timeout1_work] permit work\n");
}

static bool wltx_coolingcase_handler_valid(enum wltx_client type, bool enable)
{
	if ((wltx_get_client() == WLTX_CLIENT_COOLINGCASE)  &&
		!enable && (type == WLTX_CLIENT_UI)) {
		g_coolingcase_can_work = false;
		hwlog_info("[coolingcase_handler_valid] forbid work, permit later\n");
		schedule_delayed_work(&g_wltx_di->coolingcase_timeout1_work,
			msecs_to_jiffies(WLTX_COOLINGCASE_OPEN_DEALY)); /* 30min */
	} else if ((wirelesscase_get_online_state() != WLCASE_ONLINE_COOLINGCASE) ||
		(enable && (type != WLTX_CLIENT_COOLINGCASE))) {
		if (delayed_work_pending(&g_wltx_di->coolingcase_timeout1_work))
			cancel_delayed_work(&g_wltx_di->coolingcase_timeout1_work);
		hwlog_info("[coolingcase_handler_valid] clear flag\n");
		g_coolingcase_can_work = true;
	}

	if ((type == WLTX_CLIENT_COOLINGCASE) && (!g_coolingcase_can_work)) {
		hwlog_err("coolingcase_handler_valid: user close wltx, coolingcase can not work\n");
		return false;
	}
...
void wltx_open_tx(enum wltx_client type, bool enable)
{
	...
	if (!wltx_coolingcase_handler_valid(type, enable))
		return;
	...
	if (tx_open_flag) {
		...
		g_wltx_di->ss_low_cnt = 0;
		if (g_wltx_di->cur_pwr_sw_scn == PWR_SW_BY_PREVFOD)
			g_wltx_di->cur_pwr_sw_scn = PWR_SW_BY_VBUS_OFF;
		wireless_tx_set_stage(WL_TX_STAGE_DEFAULT);
		...
		wltx_disable_pwr_supply();
		power_event_bnc_notify(POWER_BNT_CONNECT, POWER_NE_WIRELESS_TX_STOP, NULL);
		wltx_set_client(WLTX_CLIENT_NA);
	}
	wireless_tx_set_tx_status(WL_TX_STATUS_DEFAULT);
}
...
static void wltx_get_monitor_info(struct wltx_dev_info *di)
{
	int ret;

	memset(&(di->power_info), 0, sizeof(di->power_info));
	ret = wltx_ic_get_iin(WLTRX_IC_MAIN, &(di->power_info.iin));
	ret += wltx_ic_get_fop(WLTRX_IC_MAIN, &(di->power_info.fop));
	(void)wltx_ic_get_vrect(WLTRX_IC_MAIN, &(di->power_info.vrect));
	(void)wltx_ic_get_vin(WLTRX_IC_MAIN, &(di->power_info.vin));
	(void)wltx_ic_get_temp(WLTRX_IC_MAIN, &(di->power_info.temp));
	(void)wltx_ic_get_cep(WLTRX_IC_MAIN, &(di->power_info.cep));
	(void)wltx_ic_get_duty(WLTRX_IC_MAIN, &(di->power_info.duty));
	(void)wltx_ic_get_ptx(WLTRX_IC_MAIN, &(di->power_info.ptx));
	(void)wltx_ic_get_prx(WLTRX_IC_MAIN, &(di->power_info.prx));
	(void)wltx_ic_get_ploss(WLTRX_IC_MAIN, &(di->power_info.ploss));
	(void)wltx_ic_get_ploss_id(WLTRX_IC_MAIN, &(di->power_info.ploss_id));
	(void)bat_temp_get_temperature(BAT_TEMP_MIXED, &(di->power_info.tbatt));
	di->power_info.iin_avg = di->tx_iin_avg;
	di->power_info.fop_avg = di->tx_fop_avg;
	di->power_info.soc = bci_show_capacity();

	if (ret) {
		di->i2c_err_cnt++;
		hwlog_err("%s: get tx charger info fail", __func__);
	}
	if (di->i2c_err_cnt > WL_TX_I2C_ERR_CNT) {
		hwlog_err("[%s] get charger info fail over %dtimes, close tx\n",
			__func__, di->i2c_err_cnt);
		wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
		di->stop_reverse_charge = true;
	}
}

static void wltx_check_abnormal_iout(struct wltx_dev_info *di)
{
	if (di->stop_reverse_charge)
		return;

	if ((di->standard_rx == false) && (di->power_info.iin <= WL_TX_IIN_LOW) &&
		(wlrx_get_wired_channel_state() == WIRED_CHANNEL_OFF)) {
		if (++di->tx_iin_low_cnt >= WL_TX_IIN_LOW_TIME / di->monitor_interval) {
			di->tx_iin_low_cnt = WL_TX_IIN_LOW_TIME;
			wireless_tx_set_tx_status(WL_TX_STATUS_CHARGE_DONE);
			di->stop_reverse_charge = true;
			hwlog_info("[%s] tx_iin below for %ds, set tx_status to charge_done\n",
				__func__, WL_TX_IIN_LOW_TIME / POWER_MS_PER_S);
		}
	} else {
		di->tx_iin_low_cnt = 0;
	}

	wltx_calc_tx_fop_avg(di, di->power_info.fop);
	wireless_tx_calc_tx_iin_avg(di, di->power_info.iin);
}

static void wltx_check_abnormal_ptrx(struct wltx_dev_info *di)
{
	struct wltx_dts *dts = wltx_get_dts();

	if (!dts || di->stop_reverse_charge)
		return;

	if ((dts->pwr_low_th > 0) && (di->power_info.ptx < dts->pwr_low_th) &&
		(di->power_info.prx < dts->pwr_low_th)) {
		if (++di->tx_pwr_low_cnt >= WL_TX_PTRX_LOW_TIME / di->monitor_interval) {
			hwlog_info("[%s] ptx/prx power low over %ds, close tx\n",
				__func__, WL_TX_PTRX_LOW_TIME / POWER_MS_PER_S);
			wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
			di->stop_reverse_charge = true;
		}
	} else {
...
{
	char dbg_info[WLTX_DBG_INFO_LEN] = { 0 };

	if (!wltx_need_show_mon_info(di))
		return;

	(void)wltx_ic_get_dbg_info(WLTRX_IC_MAIN, dbg_info, WLTX_DBG_INFO_LEN);
	hwlog_info("[monitor_info] soc=%d tbatt=%d fop=%uk fop_avg=%uk iin=%u "
		"iin_avg=%u vin=%u vrect=%u cep=%d duty=%u ptx=%u prx=%u "
		"ploss=%d ploss_id=%u temp=%d dbg=%s\n",
		di->power_info.soc, di->power_info.tbatt, di->power_info.fop,
		di->power_info.fop_avg, di->power_info.iin, di->power_info.iin_avg,
		di->power_info.vin, di->power_info.vrect, di->power_info.cep,
		di->power_info.duty, di->power_info.ptx, di->power_info.prx,
		di->power_info.ploss, di->power_info.ploss_id, di->power_info.temp,
		dbg_info);
}

static void wltx_monitor_power(struct wltx_dev_info *di)
{
	wltx_get_monitor_info(di);
	wltx_check_abnormal_iout(di);
	wltx_check_abnormal_ptrx(di);
...
{
	if (!tx_open_flag) {
		hwlog_err("need_stop: client closed\n");
		wireless_tx_set_tx_status(WL_TX_STATUS_TX_CLOSE);
		return true;
	}
...
static void wireless_tx_monitor_work(struct work_struct *work)
{
	...
	wltx_ps_tx_ilim_check(di);
	wltx_check_expect_power(di);
	wltx_check_timeout(di);
	wltx_monitor_power(di);
...
static void wireless_tx_start_check_work(struct work_struct *work)
{
	...
	wltx_wlcase_set_lowpower(di->cur_pwr_src);
	g_init_tbatt = 0;
...
static void wltx_coolingcase_timeout_work(struct work_struct *work)
{
	struct wltx_dev_info *di = NULL;

	di = container_of(work, struct wltx_dev_info, coolingcase_timeout_work.work);
	if (!di) {
		hwlog_err("coolingcase_timeout_work: di null\n");
		return;
	}

	if (!tx_open_flag)
		return;
	wireless_tx_set_tx_status(WL_TX_STATUS_COOLINGCASE_TIMEOUT);
	di->stop_reverse_charge = true;
	hwlog_info("close tx to prevent excessive power consumption\n");
}
...
static void wltx_handle_rx_data(struct wltx_dev_info *di)
{
	...
	case POWER_NE_WLTX_GET_RX_PRODUCT_TYPE:
		...
		(void)wltx_ic_set_ploss(WLTRX_IC_MAIN, di->rx_data.product_type);
		if ((di->rx_data.product_type == HWQI_COOLINGCASE) &&
			(wltx_get_client() == WLTX_CLIENT_UI))
			schedule_delayed_work(&di->coolingcase_timeout_work,
				msecs_to_jiffies(WLTX_COOLINGCASE_CLOSE_DEALY));
		if ((di->rx_data.product_type == HWQI_RX_TYPE_EARPHONE_BOX) &&
			(wltx_get_client() == WLTX_CLIENT_UI))
...
{
	switch ((u8)di->tx_event_data) {
	case RX_STATR_CHARGING:
		di->rx_data.product_type = HWQI_RX_TYPE_CELLPHONE;
		(void)wltx_ic_set_ploss(WLTRX_IC_MAIN, di->rx_data.product_type);
...
static void wltx_handle_ask_set_vtx(struct wltx_dev_info *di)
{
	...
	if (wltx_cap_get_cur_id(WLTRX_DRV_MAIN) == WLTX_HIGH_PWR_CAP) {
		if (di->tx_vout.v_ask < WLTX_RX_HIGH_VOUT)
			return;
		di->hp_time_out = jiffies + msecs_to_jiffies(di->hp_timeout_dflt);
	}

	wltx_set_tx_volt(WL_TX_STAGE_REGULATION, di->tx_vout.v_ask, true);
}

static void wltx_handle_ss_event(struct wltx_dev_info *di)
{
	u8 ss = (u8)di->tx_event_data;
	struct wltx_dts *dts = wltx_get_dts();

	if (!dts || (dts->ss_low_th <= 0) || power_cmdline_is_factory_mode())
		return;
...
static void wltx_handle_qi_id_event(struct wltx_dev_info *di)
{
	u16 id = (u16)di->tx_event_data;
	struct wltx_dts *dts = wltx_get_dts();

	if (id == HWQI_PARA_MANUFATURER_ID_HW)
		di->prot_id_hwrx = true;
	else
		di->prot_id_hwrx = false;

	if (!dts || (dts->support_id_prevchk <= 0))
		return;

	wltx_set_ilim(di);
}

static bool wltx_rxid_in_prevfod1_list(struct wltx_dev_info *di)
{
	int i;
	struct wltx_dts *dts = wltx_get_dts();
	...
	for (i = 0; i < dts->prevfod1_product_list_len; i++) {
		if (dts->prevfod1_product_list[i] == di->rx_product_id)
			return true;
	}
...
static void wltx_handle_ask_product_id_event(struct wltx_dev_info *di)
{
	u8 id = (u8)di->tx_event_data;

	if (di->rx_product_id != WIRELESS_DEFAULT_PROCUCT_ID)
		return;

	di->rx_product_id = (int)id;
	if ((di->rx_product_id == WIRELESS_OLD_PROCUCT_ID) ||
		(di->rx_product_id >= HWQI_WITHOUT_PREVFOD_RXID_BEGIN))
		wltx_set_imax(di);

	if ((di->tx_prevfod_cmd_timeout > 0) &&
		wltx_rxid_in_prevfod1_list(di)) {
		cancel_delayed_work_sync(&di->tx_prevfod_cmd_to_work);
		schedule_delayed_work(&di->tx_prevfod_cmd_to_work,
			msecs_to_jiffies(di->tx_prevfod_cmd_timeout));
	}
}

static struct wltx_prevfod_cfg *wltx_select_prevfod_cfg(struct wltx_dev_info *di)
{
	if ((di->rx_product_id == WIRELESS_DEFAULT_PROCUCT_ID) ||
		wltx_rxid_in_prevfod1_list(di))
		return &di->prevfod_cfg1;

	return &di->prevfod_cfg2;
}

static void wltx_handle_prevfod_chk_event(struct wltx_dev_info *di)
{
	cancel_delayed_work_sync(&di->tx_prevfod_cmd_to_work);
	if ((di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) &&
		(di->rvs4_ilim_timeout > 0)) {
	...
	wltx_prevfod_check(wltx_select_prevfod_cfg(di));
}

static struct wltx_event_handler {
	unsigned long event;
	void (*handler)(struct wltx_dev_info *);
} const g_tx_event[] = {
	{ POWER_NE_WLTX_RX_SS, wltx_handle_ss_event },
	{ POWER_NE_WLTX_RX_QI_ID, wltx_handle_qi_id_event },
	{ POWER_NE_WLTX_ASK_RX_PRODUCT_ID, wltx_handle_ask_product_id_event },
	{ POWER_NE_WLTX_START_PREVFOD_CHK, wltx_handle_prevfod_chk_event },
};

static void wltx_handle_tx_events(unsigned long event, struct wltx_dev_info *di)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_tx_event); i++) {
		if (event == g_tx_event[i].event)
			g_tx_event[i].handler(di);
	}
}

static void wltx_ask_set_vtx_work(struct work_struct *work)
{
	struct wltx_dev_info *di = NULL;

	di = container_of(work, struct wltx_dev_info, ask_set_vtx_work);
	if (!di) {
		hwlog_err("ask_set_vtx_work: di null\n");
		return;
	}
...
static void wltx_cfg_event_handler(struct wltx_dev_info *di)
{
	if (tx_status == WL_TX_STATUS_IN_CHARGING)
		return;
	wireless_tx_set_tx_status(WL_TX_STATUS_PING_SUCC);
	di->hs_time_out = jiffies + msecs_to_jiffies(di->hs_timeout_offset);
	if (di->cur_pwr_src == PWR_SRC_RVS_SC4_CP) { /* for prevfod check */
		wltx_ic_prevfod_check(WLTRX_IC_MAIN, WLTX_PREVFOD_ILIM_START);
		di->rvs4_ilim_timeout = jiffies +
			msecs_to_jiffies(WLTX_RVS4_ILIM_TIMEOUT);
	}
	wltx_set_tx_volt(WL_TX_STAGE_REGULATION, di->tx_vset.v_hs, true);
}

static void wltx_prevfod_cmd_to_work(struct work_struct *work)
{
	if (wltx_pwr_prevfod_limit_handler())
		power_event_bnc_notify(POWER_BNT_WLTX, POWER_NE_WLTX_TX_FOD, NULL);
	hwlog_err("prevfod cmd timeout\n");
}
...
static void wireless_tx_event_work(struct work_struct *work)
{
	...
	wltx_handle_tx_events(di->tx_event_type, di);
}

static void wireless_tx_save_event(struct wltx_dev_info *di, unsigned long event, void *data)
{
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_WLTX_RX_QI_ID:
	case POWER_NE_WLTX_ASK_RX_PRODUCT_ID:
	case POWER_NE_WLTX_START_PREVFOD_CHK:
		break;
	case POWER_NE_WLTX_ASK_SET_VTX:
		wireless_tx_save_event(di, event, data);
		schedule_work(&di->ask_set_vtx_work);
		return NOTIFY_OK;
	default:
		return NOTIFY_OK;
	}
...
static int __wltx_parse_stage_vset_para(const char *propname,
	struct wltx_vset_para *dts_vset, struct device_node *np, struct wltx_dev_info *di)
{
	int ret;
	int array_len;
	u32 temp_arr[WLTX_TX_STAGE_VTOTAL] = { 0 };

	array_len = of_property_count_u32_elems(np, propname);
	if (array_len != WLTX_TX_STAGE_VTOTAL)
		return -EFAULT;

	ret = of_property_read_u32_array(np, propname, temp_arr, array_len);
	if (ret)
		return -EFAULT;

	dts_vset->v_ps = (int)temp_arr[WLTX_TX_STAGE_VPS];
	dts_vset->v_ping = (int)temp_arr[WLTX_TX_STAGE_VPING];
	dts_vset->v_hs = (int)temp_arr[WLTX_TX_STAGE_VHS];
	dts_vset->v_dflt = (int)temp_arr[WLTX_TX_STAGE_VDFLT];
	return 0;
}

static void wltx_parse_stage_vset_para(struct device_node *np, struct wltx_dev_info *di)
{
	int ret;

	ret = __wltx_parse_stage_vset_para("tx_stage_vset", &di->dts_vset, np, di);
	if (ret < 0)
		goto parse_err;
	goto print_para;

parse_err:
	di->dts_vset.v_ps = 5000; /* mV */
	di->dts_vset.v_ping = 5000; /* mV */
	di->dts_vset.v_hs = 5000; /* mV */
	di->dts_vset.v_dflt = 5000; /* mV */

print_para:
	hwlog_info("[%s] pwr_supply:%d ping:%d handshake:%d default:%d\n",
		__func__, di->dts_vset.v_ps, di->dts_vset.v_ping,
		di->dts_vset.v_hs, di->dts_vset.v_dflt);
}

static void wltx_parse_sc14_stage_vset_para(struct device_node *np, struct wltx_dev_info *di)
{
	int ret;

	if (of_property_count_u32_elems(np, "tx_sc4_stage_vset") < 0)
		return;
	ret = __wltx_parse_stage_vset_para("tx_sc4_stage_vset", &di->sc14_dts_vset, np, di);
	if (ret < 0)
		goto parse_err;
	goto print_para;

parse_err:
	di->sc14_dts_vset.v_ps = 16000; /* mV */
	di->sc14_dts_vset.v_ping = 8000; /* mV */
	di->sc14_dts_vset.v_hs = 8000; /* mV */
	di->sc14_dts_vset.v_dflt = 16000; /* mV */

print_para:
	hwlog_info("[%s] sc4_pwr_supply:%d ping:%d handshake:%d default:%d\n",
		__func__, di->sc14_dts_vset.v_ps, di->sc14_dts_vset.v_ping,
		di->sc14_dts_vset.v_hs, di->sc14_dts_vset.v_dflt);
}
...
static void wltx_parse_tbatt_alarm_para(struct device_node *np, struct wltx_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_tbatt_alarm_all", &di->protect_cfg.tbatt_alarm_all, 0);
...
static void wltx_print_prevfod_alarm_para(struct wltx_prevfod_cfg *prevfod_cfg)
{
	int i;

	for (i = 0; i < prevfod_cfg->level; i++)
		hwlog_info("prevfod_alarm%d[%d] pwr_src:%d ploss_th[%d %d]\t"
			"cnt_th:%d ilim:%d alarm[0x%x %u %u %u]\n",
			prevfod_cfg->index, i,
			prevfod_cfg->prevfod_alarm[i].pwr_src,
			prevfod_cfg->prevfod_alarm[i].ploss_lth,
			prevfod_cfg->prevfod_alarm[i].ploss_hth,
			prevfod_cfg->prevfod_alarm[i].cnt_th,
			prevfod_cfg->prevfod_alarm[i].ilim,
			prevfod_cfg->prevfod_alarm[i].alarm.src_state,
			prevfod_cfg->prevfod_alarm[i].alarm.plim,
			prevfod_cfg->prevfod_alarm[i].alarm.vlim,
			prevfod_cfg->prevfod_alarm[i].alarm.reserved);
}

static void wltx_parse_prevfod_alarm_para(struct device_node *np, struct wltx_dev_info *di)
{
	int len;

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"tx_prevfod_alarm1", (int *)di->prevfod_cfg1.prevfod_alarm,
		WLTX_PREVFOD_ALARM_ROW, WLTX_PREVFOD_ALARM_COL);
	if (len <= 0) {
		di->prevfod_cfg1.level = 0;
		return;
	}
	di->prevfod_cfg1.level = len / WLTX_PREVFOD_ALARM_COL;
	di->prevfod_cfg1.index = WLTX_PREVFOD_CFG1;

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"tx_prevfod_alarm2", (int *)di->prevfod_cfg2.prevfod_alarm,
		WLTX_PREVFOD_ALARM_ROW, WLTX_PREVFOD_ALARM_COL);
	if (len <= 0) {
		di->prevfod_cfg1.level = 0;
		di->prevfod_cfg2.level = 0;
		return;
	}
	di->prevfod_cfg2.level = len / WLTX_PREVFOD_ALARM_COL;
	di->prevfod_cfg2.index = WLTX_PREVFOD_CFG2;

	wireless_set_dev_attr(WLTRX_IC_MAIN, WIRELESS_PROTOCOL_QI, HWQI_TX_SUPPORT_PREVFOD_CHK);
	wltx_print_prevfod_alarm_para(&di->prevfod_cfg1);
	wltx_print_prevfod_alarm_para(&di->prevfod_cfg2);
}
...
static void wltx_parse_tx_alarm_para(struct wltx_dev_info *di)
{
	...
	wltx_parse_prevfod_alarm_para(np, di);
}
...
static void wireless_tx_parse_dts(struct device_node *np, struct wltx_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"hp_timeout_dflt", &di->hp_timeout_dflt, WLTX_HI_PWR_TIME);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_timeout_cfg", &di->tx_timeout_cfg, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_rp_demodule_timeout", &di->tx_rp_demodule_timeout, WLTX_RP_DEMODULE_TIMEOUT_VAL);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_prevfod_cmd_timeout", &di->tx_prevfod_cmd_timeout, 0);
	...
	wltx_parse_stage_vset_para(np, di);
	wltx_parse_sc14_stage_vset_para(np, di);
...
static void wireless_tx_shutdown(struct platform_device *pdev)
{
	...
	cancel_delayed_work(&di->tx_prevfod_cmd_to_work);
}
...
static int wireless_tx_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&di->coolingcase_timeout_work, wltx_coolingcase_timeout_work);
	INIT_DELAYED_WORK(&di->coolingcase_timeout1_work, wltx_coolingcase_timeout1_work);
	INIT_DELAYED_WORK(&di->tx_prevfod_cmd_to_work, wltx_prevfod_cmd_to_work);
	INIT_WORK(&di->ask_set_vtx_work, wltx_ask_set_vtx_work);
