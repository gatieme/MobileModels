...
int wldc_get_bat_voltage(void)
{
	...
	if (di->use_higher_vbat)
		btb_vol = dcm_get_ic_max_vbtb_with_comp(di->dc_ratio, di->comp_para.vbat_comp);
	else
		btb_vol = dcm_get_ic_vbtb_with_comp(di->dc_ratio, di->ic_data.cur_type,
...
static int wldc_get_bat_current(void)
{
	...
	ret = dcm_get_total_ibat(di->dc_ratio, di->ic_data.cur_type, &bat_curr);
	if (ret) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		dc_fill_eh_buf(di->wldc_err_dsm_buff,
...
int wldc_get_ls_vbus(void)
{
	...
	ret = dcm_get_ic_vbus(di->dc_ratio, di->ic_data.cur_type, &vbus);
	if (ret < 0) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		dc_fill_eh_buf(di->wldc_err_dsm_buff,
...
int wldc_get_ls_ibus(void)
{
	...
	ret = dcm_get_ic_ibus(di->dc_ratio, di->ic_data.cur_type, &ibus);
	if (ret < 0) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		dc_fill_eh_buf(di->wldc_err_dsm_buff,
...
bool wldc_need_cp_div2(void)
{
	...
	return (di->mode_para[di->cur_dc_mode].init_para.cp_ratio == CP_CP_RATIO);
}
...
static void wldc_select_comp_param(struct wldc_dev_info *di)
{
	...
	ic_name = dcm_get_ic_name(di->dc_ratio, CHARGE_IC_AUX);
...
static void wldc_set_imax_by_tx_ability(struct wldc_dev_info *di)
{
	int cp_ratio;
	int path_ratio;
	int imax;
	...
	path_ratio = cp_ratio * di->dc_ratio;
	imax = wlrx_acc_get_tx_imax(WLTRX_DRV_MAIN, path_ratio);
	if (imax > 0)
		di->tx_imax = imax;
}
...
{
	int vmax = WLRX_VBAT_BASE * di->dc_ratio;
...
void wldc_start_charging(struct wldc_dev_info *di)
{
	...
	wldc_set_imax_by_tx_ability(di);
	dcm_config_ic_watchdog(di->dc_ratio, di->ic_data.cur_type, WLDC_WDT_TIMEOUT);
...
{
	int max_vgap = di->max_vgap / SC_DEFAULT_VOLT_RATIO * di->dc_ratio;

	if ((di->max_vgap > 0) &&
		(di->rx_vout_set > rx_vout + max_vgap * cp_ratio))
		di->rx_vout_set = rx_vout + max_vgap * cp_ratio;
...
static void wldc_charge_regulation(struct wldc_dev_info *di)
{
	...
	int vstep = di->vstep / SC_DEFAULT_VOLT_RATIO * di->dc_ratio;
	int cp_iout_err_hth = di->cp_iout_err_hth * SC_DEFAULT_VOLT_RATIO / di->dc_ratio;

	if (wldc_update_regulation_data(di, &data))
		return;

	if (!wlrx_ic_get_vout_reg(WLTRX_IC_MAIN, &vrx_reg))
		di->rx_vout_set = vrx_reg;
	if ((data.rx_iout > data.rx_imax + WLDC_DFT_IMAX_ERR_TH) ||
		(data.rx_iout > di->tx_imax + WLDC_DFT_IMAX_ERR_TH)) {
		di->rx_vout_set = data.rx_vout - vstep * data.cp_ratio;
		hwlog_info("[%s] rx_iout:%dmA>hth:%dmA or tx_imax:%dmA, decrease rx_vout\n",
			__func__, data.rx_iout, data.rx_imax, di->tx_imax);
		goto set_vout;
	}
	if (data.cp_iout_avg > di->cur_cp_iout_hth + cp_iout_err_hth) {
		di->rx_vout_set -= vstep * data.cp_ratio;
		hwlog_info("[%s] cp_iout_avg:%dmA>%dmA, decrease rx_vout\n",
			__func__, data.cp_iout_avg,
			di->cur_cp_iout_hth + cp_iout_err_hth);
		goto set_vout;
	}
	...
	if ((data.cp_iout_avg > di->cur_cp_iout_hth - cp_iout_err_hth) ||
		(data.cp_iout > di->cur_cp_iout_hth - cp_iout_err_hth))
		return;
...
static bool wldc_abnormal_iout_check(struct wldc_dev_info *di)
{
	...
	if ((cp_iout < WLDC_IOUT_LTH) && (cp_iout_avg < WLDC_IOUT_LTH)) {
		cnt++;
...
static void wldc_check_exit_condition(struct wldc_dev_info *di)
{
	...
	if (wlrx_pmode_judge(WLTRX_DRV_MAIN, exp_pid, WLRX_PMODE_FINAL_JUDGE)) {
		hwlog_err("%s: switch to expected pmode\n", __func__);
		wlrx_pmode_set_curr_pid(WLTRX_DRV_MAIN, exp_pid);
		di->stop_flag_info = true;
		return;
	}
	if (dcm_is_ic_close(di->dc_ratio, di->ic_data.cur_type)) {
		hwlog_err("%s: direct_charge ic is closed\n", __func__);
		di->dc_closed = true;
...
static void wldc_check_imax_by_bat_cccv(struct wldc_dev_info *di)
{
	int vbatt;
	int batt_cccv_cur;

	vbatt = wldc_get_bat_voltage();
	batt_cccv_cur = bat_cccv_get_ichg(vbatt);
	hwlog_info("[wldc_check_imax_by_bat_cccv] total_cur=%dmA", batt_cccv_cur);
	di->cur_cp_iout_hth = power_min_positive(batt_cccv_cur / di->dc_ratio, di->cur_cp_iout_hth);
}
...
static void wldc_check_imax_by_batt_balance(struct wldc_dev_info *di)
{
	...
	info[0].vol = dcm_get_ic_vbtb_with_comp(SC_MODE, CHARGE_IC_MAIN,
		di->comp_para.vbat_comp);
	bat_temp_get_temperature(BTB_TEMP_0, &info[0].temp);

	/* update 2st battery info */
	power_supply_get_int_property_value("battery_gauge_aux",
		POWER_SUPPLY_PROP_CURRENT_NOW, &info[1].cur);
	info[1].vol = dcm_get_ic_vbtb_with_comp(SC_MODE, CHARGE_IC_AUX,
		di->comp_para.vbat_comp);
	if (info[1].vol < 0)
		info[1].vol = info[0].vol;
	bat_temp_get_temperature(BTB_TEMP_1, &info[1].temp);

	if (bat_chg_balance_get_cur_info(info, 2, &result, di->battery_connect_mode)) /* 2: two battery */
		return;

	hwlog_info("[check_imax_by_batt_balance] total_cur=%dmA", result.total_cur);
	di->cur_cp_iout_hth = power_min_positive(di->cur_cp_iout_hth,
		result.total_cur / di->dc_ratio);
}
...
static int wldc_get_cp_iin_thermal(struct wldc_dev_info *di)
{
	unsigned int idx;
	int cp_iin_thermal;
	...
	/*
	 * cp_iin_thermal: 2:1 ibus configured default, when use it, we should convert it
	 * to ibat(ibat=2*ibus) first, then ibus=ibat/dc_ratio, dc_ratio may be 2:1/4:1.
	 */
	return cp_iin_thermal * SC_DEFAULT_VOLT_RATIO / di->dc_ratio;
}
...
static void wldc_select_charge_param(struct wldc_dev_info *di)
{
	...
	if ((stage_id < 0) || (stage_id >= WLDC_VOLT_LEVEL) || (stage_id >= di->stage_size)) {
	...
	cp_iin_thermal = wldc_get_cp_iin_thermal(di);
	...
	pctrl_irx = wlrx_pctrl_get_rx_ilimit(WLTRX_DRV_MAIN);
	di->cur_cp_iout_hth = power_min_positive(pctrl_irx * cp_ratio, di->cur_cp_iout_hth);
	...
	tx_evt_ilim = wlrx_get_alarm_ilim(WLRX_VBAT_BASE * di->dc_ratio);
	...
	wldc_check_imax_by_bat_cccv(di);
	wldc_check_imax_by_multi_ic(di);
	wldc_check_imax_by_multi_cp(di);
	wldc_check_imax_by_batt_balance(di);

	if (original_iout_hth != di->cur_cp_iout_hth) {
...
static void wldc_stop_limit_output_current(struct wldc_dev_info *di)
{
	...
	int rx_vstep = 75 * di->dc_ratio * cp_ratio; /* 75mv: limit current step volt */
...
static void wldc_stop_output_check(struct wldc_dev_info *di)
{
	...
	if (vfc_reg > rx_init_vout)
		vfc_reg = rx_init_vout;
...
void wldc_stop_charging(struct wldc_dev_info *di)
{
	...
	ret = dcm_enable_ic(di->dc_ratio, di->ic_data.cur_type, DC_IC_DISABLE);
	if (ret)
	...
	dcm_set_ic_threshold_enable(di->dc_ratio, CHARGE_MULTI_IC, 0);
	dcm_config_ic_watchdog(di->dc_ratio, di->ic_data.cur_type, 0);
	ret = dcm_exit_ic(di->dc_ratio, di->ic_data.cur_type);
	if (ret)
		hwlog_err("%s: ls exit fail\n", __func__);
	ret = dcm_exit_batinfo(di->dc_ratio, di->ic_data.cur_type);
	if (ret)
...
static int wldc_multi_ic_check(struct wldc_dev_info *di)
{
	...
	cp_ratio = di->mode_para[di->cur_dc_mode].init_para.cp_ratio;
	if (mulit_ic_check(di->dc_ratio, di->ic_data.cur_type, &di->ic_check_info, di->dc_ratio) < 0) {
...
void wldc_control_work(struct work_struct *work)
{
	...
	if (wldc_need_stop_work(di)) {
		if (di->dc_closed)
			wlrx_pctrl_count_dcdiscon(WLTRX_DRV_MAIN, true);
...
static int wldc_pmode_cfg_str2int(const char *str, int *pcfg, int offset, int col)
{
	if (kstrtoint(str, 0, &pcfg[offset % col]))
		return -EINVAL;

	return 0;
}

static void wldc_print_mode_para(struct wldc_dev_info *di)
{
	int i;
	struct wldc_init_para *para = NULL;

	for (i = 0; i < di->total_dc_mode; i++) {
		para = &di->mode_para[i].init_para;
...
static void wldc_revise_mode_para(struct wldc_dev_info *di)
{
	int i;
	struct wldc_init_para *p_init = NULL;

	for (i = 0; i < di->total_dc_mode; i++) {
		di->mode_para[i].dis_flag = WLDC_EN;
		p_init = &di->mode_para[i].init_para;
		if (p_init->tx_vout <= 0)
			p_init->tx_vout = p_init->rx_vout;
		if (p_init->dc_ratio <= 0)
			p_init->dc_ratio = di->dc_ratio;
	}
}

static void wldc_parse_mode_para(struct device_node *np, struct wldc_dev_info *di)
{
	int i, row, col, len;
	const char *tmp_str = NULL;

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"init_para_col", (u32 *)&col, WLDC_PMODE_CFG_COL);
	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"init_para", WLDC_PMODE_CFG_ROW, col);
	if (len <= 0)
		return;

	di->mode_para = kcalloc(len / col, sizeof(*(di->mode_para)), GFP_KERNEL);
	if (!di->mode_para) {
		hwlog_err("%s: alloc wldc_mode_para fail\n", __func__);
		return;
	}
	di->total_dc_mode = len / col;
	for (i = 0; i < len; i++) {
		if (power_dts_read_string_index(power_dts_tag(HWLOG_TAG), np,
			"init_para", i, &tmp_str))
			goto parse_err;
		row = i / col;
		if ((i % col) == 0) { /* 0: pmode type */
			if (kstrtoint(tmp_str, 0, &di->mode_para[row].init_para.dc_type))
				goto parse_err;
			continue;
		}
		if ((i % col) == 1) { /* 1: pmode name */
			di->mode_para[row].init_para.dc_name = tmp_str;
			continue;
		}
		if (wldc_pmode_cfg_str2int(tmp_str, /* length: 1char*=2int */
			(int *)&di->mode_para[row].init_para.ext_pwr_type, i - 2, col))
			goto parse_err;
	}

	wldc_revise_mode_para(di);
	wldc_print_mode_para(di);
	return;
	...
	struct wldc_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"use_higher_vbat", &di->use_higher_vbat, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"battery_connect_mode", &di->battery_connect_mode, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"single_ic_mode", &di->ic_mode_para.single_ic_mode, CHARGE_IC_MAIN);
}
	...
	struct wldc_dev_info *di)
{
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "path_switch_type",
		&di->ic_mode_para.path_switch_type, 0);
