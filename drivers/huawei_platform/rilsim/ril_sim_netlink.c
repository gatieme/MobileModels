...
{
	int slot_id;
	if (nlh->nlmsg_len < NLMSG_HDRLEN || (int)nlh->nlmsg_len - NLMSG_HDRLEN != (int)sizeof(knl_sim_pin_info)) {
		hwlog_err("%s: nlmsg_len:%u, NLMSG_HDRLEN:%d, struct size:%zu\n",
...
static int process_sim_msg(const struct nlmsghdr *nlh)
{
	...
	if (nlh->nlmsg_type == NETLINK_KNL_SIM_UPDATE_IND_MSG) {
		...
		hwlog_err("%s: Invalid message from ril, nlmsg_type: %hu\n",
...
static void kernel_ril_sim_receive(struct sk_buff *__skb)
{
	...
	if ((int)skb->len >= NLMSG_HDRLEN) {
		...
		if (nlh->nlmsg_len >= sizeof(*nlh) && skb->len >= nlh->nlmsg_len) {
			hwlog_info("%s: receive message form user space, nlmsg_type: %hu, nlmsg_pid: %u\n",
				__func__, nlh->nlmsg_type, nlh->nlmsg_pid);
			ret = process_sim_msg(nlh);
			hwlog_info("%s: process_sim_msg ret: %d\n", __func__, ret);
		}
	} else {
		hwlog_err("%s: Invalid len message, skb->len: %u\n", __func__, skb->len);
	}
end:
...
static int send_get_pin_info_cnf(u32 pid, int slot_id)
{
	...
	if (!pid || !sim_nlfd) {
		hwlog_err("%s: Invalid pid or sim_nlfd, pid: %u\n", __func__, pid);
		ret = KNL_FAILED;
		goto end;
	}
	...
	hwlog_info("%s: send to ril, pid: %u, ret: %d\n", __func__, pid, ret);
end:
...
static int send_query_verified_cnf(u32 pid)
{
	...
	/* skb will be freed in netlink_unicast */
	ret = netlink_unicast(sim_nlfd, skb, pid, MSG_DONTWAIT);
	hwlog_info("%s: send to ril, pid: %u, is_verified: %d, ret: %d\n",
