...
#elif defined(DDR_CHARLOTTE_PLATFORM)
...
int alloc_conti_pages(unsigned long start_pfn, unsigned long nr_pages)
{
	...
	if (pfn_range_valid(z, start_pfn, nr_pages)) {
		...
		ret = alloc_contig_range(start_pfn, start_pfn + nr_pages,
			MIGRATE_MOVABLE, GFP_KERNEL | __GFP_NOWARN, &dummy);
#else
		ret = alloc_contig_range(start_pfn, start_pfn + nr_pages,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
		virtual_addr = (unsigned long *)ioremap(
				...
				unsigned long arg)
{
	...
	if (_IOC_DIR(cmd) & (_IOC_READ | _IOC_WRITE))
		ret = !access_ok((void __user *)(uintptr_t)arg,
			_IOC_SIZE(cmd));
#else
