...
static enum as7341_sensor_id_t sensor_type;
...
static enum as7341_sensor_id_t as7341_get_chipid(struct i2c_client *handle)
{
	...
	color_sensor_get_byte(handle, AS7341_AUXID_REG, &aux_id_val);
	for (i = 0; i < ARRAY_SIZE(as7341_ids); i++) {
		if (as7341_ids[i] == chipid) {
			hwlog_debug("get this chip info\n");
			// Switch REG_BANK, using the P2RAM_OTP_1 Bit 6 to separate the AS7341 and AS7351
			color_sensor_set_byte(handle, AS7341_CFG0_REG, REG_BANK1);
			color_sensor_get_byte(handle, P2RAM_OTP_1, &p2ram_opt_1);
			color_sensor_set_byte(handle, AS7341_CFG0_REG, REG_BANK0);
			if ((aux_id_val == AS7351_AUX_ID_VAL1) ||
				(aux_id_val == AS7351_AUX_ID_VAL2)) {
				hwlog_info("%s: Chip is AS7351\n", __func__);
				sensor_type = AMS_AS7351_REV0;
			} else {
				hwlog_info("%s: Chip is AS7341\n", __func__);
				sensor_type = AMS_AS7341_REV0;
			}
		}
		return sensor_type;
	}
...
static INT32 as7341_set_fd_gain(struct as7341_ctx *ctx, int gain)
{
	...
	as7341_set_field(ctx->handle, AS7341_FD_CFG3, cfg1,
		FD_GAIN_MASK);
	as7341_set_field(ctx->handle, AS7341_ENABLE_REG, PON | FDEN, PON | FDEN);
	...
	bool cal_mode)
{
	...
	if (data == 0) {
		...
		if (!cal_mode) {
			...
			// kun added for Production line
			if (sensor_type == AMS_AS7351_REV0) {
				as7341_als_set_itime(ctx, AS7341_CAL_ITIME_DEFAULT);
				as7341_als_set_gain(ctx, DEFAULT_CALIB_AS7351_GAIN_ID);
			}
			else {
				as7341_als_set_itime(ctx, AS7341_ITIME_DEFAULT);
				as7341_als_set_gain(ctx, DEFAULT_CALIB_AS7341_GAIN_ID);
			}
		}
		...
		channel_block = 0;
		as7341_set_field(ctx->handle, AS7341_ENABLE_REG, PON | AEN, PON | AEN);
	}
...
static void as7341_handle_first_integration(struct as7341_ctx *ctx)
{
	...
	if (!saturation) {
		if (cal_normal && (sensor_type == AMS_AS7351_REV0))
			as7341_als_set_itime(ctx, AS7341_CAL_ITIME_DEFAULT);
		else
...
static void adjust_raw_values_on_itime(struct as7341_ctx *ctx)
{
	...
	as7341_log_buffer("INT_ADJUST", ctx->alg_ctx.data, AMS_REPORT_DATA_LEN);
	ctx->alg_ctx.meas_steps = STEP_DEFAULT_MEASURE;
}
...
void as7341_als_timer_wrk(struct timer_list *t)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
void as7341_flc_timer_wrk(struct timer_list *t)
{
...
static int as7341_cal_als(struct color_chip *chip)
{
	...
	if (sensor_type == AMS_AS7341_REV0) {
	...
	} else if (sensor_type == AMS_AS7351_REV0) {
		enable_status_before_cali = 0;
		as7341_als_set_enable(chip, AMSDRIVER_ALS_DISABLE);
		ctx->alg_ctx.enabled = 0;
	}
	hwlog_info("%s done\n", __func__);
...
static void adapt_raw_report_als(UINT32 luma_val, UINT32 adc_model1, UINT32 adc_model2,
	UINT32 curr_gain, UINT32 raw_data[])
{
	UINT8 i;
	bool need_re_adapt_color_val = false;

	if (sensor_type != AMS_AS7351_REV0) {
		for (i = 0; i < AMS_REPORT_DATA_LEN; i++)
			color_report_val[i + 1] = raw_data[i] * AMS_AS7341_GAIN_OF_GOLDEN / curr_gain;
		return;
	}

	// for 7351
	if (luma_val > LUMA_COM_VAL)
		need_re_adapt_color_val = true;

	if ((adc_model1 > 0 && adc_model2 > 0) || (adc_model1 == 0 && adc_model2 == 0))
		need_re_adapt_color_val = true;

	for (i = 0; i < AMS_REPORT_DATA_LEN; i++) {
		if (need_re_adapt_color_val)
			color_report_val[i + 1] = raw_data[i] * AMS_AS7351_GAIN_OF_GOLDEN / curr_gain;
		else
...
static void as7341_report_als(struct color_chip *chip)
{
	...
	// kun remove some code and for AS7351 donot need to mul the cal_ctx.ratio
	...
	// model1 and model2 are two different photosensitive channels with different ADC mappings.
	// Therefore, Values of the same ADC mapping channel are added up for judgment.
	model1 = raw_data[0] + raw_data[1] + raw_data[2] + raw_data[7] + raw_data[8];
	model2 = raw_data[3] + raw_data[4] + raw_data[5] + raw_data[6] + raw_data[9];
	...
	if (skip_first_rgb_data) {
		...
		color_report_val[0] = AMS_REPORT_DATA_LEN;
		adapt_raw_report_als(raw_data[PHOTOSENSITIVE_CHANNEL], model1, model2, curr_gain, raw_data);
...
void as7341_store_enable(struct color_chip *chip, int state)
{
	...
	/* kun added one parameter to describe in the whitebalance calibration */
	if (sensor_type == AMS_AS7351_REV0)
		cal_normal = state;
...
static char *as7341_algo_type(void)
{
	if (sensor_type == AMS_AS7351_REV0 && algo_support == 2) // dts algo_support = <2>
		return "as7351_hal";
...
static void configure_functions(struct color_chip *chip)
{
	...
	chip->color_algo_type = as7341_algo_type;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	timer_setup(&chip->work_timer, as7341_als_timer_wrk, 0);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	timer_setup(&chip->fd_timer, as7341_flc_timer_wrk, 0);
#else
