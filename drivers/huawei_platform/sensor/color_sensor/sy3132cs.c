...
void sy3132cs_hj_deduction_dark_double(bool is_dark_environment_j10_double)
{
	...
	if (cali_flag && !is_dark_environment_j10_double)
		return;

	ch5_value = (color_report_val[CH5_VALUE] == 0) ? 1 : color_report_val[CH5_VALUE];

	if (color_report_val[CH5_VALUE] <= CH5_05LUX_LIMIT && (color_report_val[CH2_VALUE] * DATA_EXPAND_100 /
		ch5_value) >= CH2_TO_CHT_RATION)
		double_deduct_check = true;

	ch2_value = color_report_val[CH2_VALUE];
	for (i = 0; i < ALL_CHANNEL_NUM; i++) {
		if ((i > 8) || (double_deduct_check == false))
			continue;
		if (color_report_val[i + 1] < ch2_value * fix_adc_ration[i])
			color_report_val[i + 1] = 0;
		else
			color_report_val[i + 1] -= ch2_value * fix_adc_ration[i];
	}
