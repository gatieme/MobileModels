...
#define MAX_REHANDSHAKE_RETRY_COUNT 25
...
static struct wakeup_source *handshake_wake_lock;
...
/* insert data */
static void rbtree_insert(struct rb_root *root, struct stat_record *data)
{
	struct rb_node **node = &root->rb_node;
...
static void record_stat(unsigned char sid, unsigned char cid,
				       bool is_read)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static void stat_timer_callback(struct timer_list *arg)
#else
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	__init_timer(&stat_timer, NULL, 0);
#else
			       ...
			       int payload_bufsz, int i)
{
	...
	/* memset to 0x00 */
	memset_s(request->frames[i].frame_body, MAX_FRAME_BUF_LEN, 0, MAX_FRAME_BUF_LEN);
	/* 1. Fill tran id */
				 ...
				 unsigned char *payload_buf, int payload_bufsz)
{
	...
	for (i = 0; i < request->frame_count; i++) {
		offset = 0;
		if (encode_frame_header(tid, request, &offset, payload_bufsz, i) < 0)
			return -EFAULT;
		/* Fill payload */
		if (request->frame_count == 1) {
			if (payload_bufsz > 0)
		...
		if (encode_frame_crc(request, offset, i) < 0)
			return -EFAULT;
	}
...
static int handld_upgrade_set_mode(int offset, unsigned char *payload_buf, int payload_bufsz)
{
	int ret;
...
static int handle_common_command(unsigned char *payload_buf, int payload_bufsz)
{
	...
	if (!g_commu_data[UART_DRIVER] ||
	    payload_bufsz < COMMU_COMMON_COMMAND_LEN || payload_buf[offset++] != 0)
		return -EINVAL;
	...
	case COMMAND_UPGRADE_SET_MODE:
		ret = handld_upgrade_set_mode(offset, payload_buf, payload_bufsz);
		break;
	...
	case COMMAND_UPGRADE_RST_MCU:
		ret = spi_slave_ext_sensorhub_reset();
		break;
	default:
		return -EINVAL;
	}
...
static enum commu_driver get_driver_by_transid(unsigned char tid)
{
	int commu_mode = get_commu_mode();

	switch (commu_mode) {
	case SDIO_COMMU_MODE:
		return SDIO_DRIVER;
	case SPI_COMMU_MODE:
		if (tid == TRANSACTION_ID_AP_TO_MCU)
			return SPI_DRIVER;
		return UART_DRIVER;
	case SPI_SLAVE_COMMU_MODE:
		if (tid == TRANSACTION_ID_AP_TO_MCU)
			return SPI_SLAVE_DRIVER;
		return UART_DRIVER;
	default:
		return SPI_SLAVE_DRIVER;
	}
...
static int send_frames(struct frame_request *request, enum commu_driver driver)
{
	int i;
	int ret = 0;
	...
	for (i = 0; i < request->frame_count; ++i) {
		ret = send_frame_data_retry(driver, request->frames[i].frame_body,
					    request->frames[i].frame_total_length);
		if (ret < 0) {
...
int send_frame_data(unsigned char tid,
		    ...
		    unsigned char *payload_buf, int payload_bufsz)
{
	struct frame_request request = {0};
	int ret;
	unsigned char frame_count;
	enum commu_driver driver = get_driver_by_transid(tid);

	ret = handle_inner_cmd(service_id, command_id, payload_buf, payload_bufsz);
	if (ret != 0) {
		hwlog_info("%s handle inner cmd return %d", __func__, ret);
		return ret;
	}
	/* inner cmd hook */
	if (process_inner_cmd_hook(service_id, command_id, payload_buf, payload_bufsz, &ret))
		return ret;

	if (!g_handshake[driver].handshake_flag &&
	    (service_id != SERVICE_ID_COMMU || command_id != SERVICE_MCU_HANDSHAKE_REQ))
		return -EREMOTEIO;
	...
	if (driver == UART_DRIVER || payload_bufsz <=
	...
	ret = send_frames(&request, driver);
			      ...
			      debug_frame_handler handler)
{
	...
	/* get tid */
	tid = frame_buffer[0];
	driver = get_driver_by_transid(tid);
				  ...
				  unsigned char expected_fsn)
{
	...
	if (ret != EOK || expected_fsn != fsn_value ||
				     ...
				     enum commu_driver driver)
{
	...
	ret = memcpy_s(raw_data->payload + offset,
		       ...
		       ext_sensorhub_get_min(payload_len,
					     MAX_FRAME_BUF_LEN - offset));
	if (ret != EOK)
		return -EFAULT;
...
static void close_bt_uart(enum commu_driver driver)
{
	...
	ret = ext_sensorhub_frame_close_uart();
	if (ret < 0)
		pr_warn("%s close uart ret: %d", __func__, ret);
}
			     ...
			     enum commu_driver driver)
{
	...
	if (raw_data->service_id == SERVICE_ID_COMMU &&
		...
		if (!get_commu_data(driver)->active_handshake) {
			...
			if (driver == SPI_SLAVE_DRIVER) {
				__pm_relax(handshake_wake_lock);
				pr_info("%s relax lock for handshake", __func__);
			}
		       ...
		       enum commu_driver driver)
{
	...
	if (resp_list->read_cnt > 0 && !resp_list->buffer) {
		hwlog_debug("%s resp_list->buffer kmalloc fail", __func__);
		kfree(resp_list);
		resp_list = NULL;
		return -ENOMEM;
	}
	...
	if (memcpy_s(resp_list->buffer, resp_list->read_cnt,
		     raw_data->payload, resp_list->read_cnt) != EOK) {
		kfree(resp_list->buffer);
...
static void handshake_resp_work(struct work_struct *work)
{
	...
	msec = (long long)tv.tv_sec * time_factor +
	tv.tv_nsec / (time_factor * time_factor);
	...
	if (get_commu_mode() == SDIO_COMMU_MODE) {
		if (get_rtc_time() < 0) {
...
bool handshake_reset_mcu(void)
{
#ifdef CONFIG_EXT_INPUTHUB_QCOM
	enum commu_driver mcu_driver = SPI_SLAVE_DRIVER;
#else
	enum commu_driver mcu_driver = SPI_DRIVER;
#endif
	if (!g_handshake[UART_DRIVER].handshake_flag)
		g_handshake[UART_DRIVER].rehandshake_try_count++;
	else
		g_handshake[UART_DRIVER].rehandshake_try_count = 0;

	if (!g_handshake[mcu_driver].handshake_flag)
		g_handshake[mcu_driver].rehandshake_try_count++;
	else
		g_handshake[mcu_driver].rehandshake_try_count = 0;

	if (g_handshake[mcu_driver].handshake_flag &&
	    g_handshake[UART_DRIVER].handshake_flag)
		g_rehandshake_reset_count = 0;

	if (g_handshake[mcu_driver].rehandshake_try_count >
	    MAX_REHANDSHAKE_RETRY_COUNT ||
	    g_handshake[UART_DRIVER].rehandshake_try_count >
		MAX_REHANDSHAKE_RETRY_COUNT) {
		hwlog_info("%s spi handshake try count max", __func__);
		g_handshake[mcu_driver].rehandshake_try_count = 0;
		g_handshake[UART_DRIVER].rehandshake_try_count = 0;
		if (g_rehandshake_reset_count < MAX_REHANDSHAKE_RESET_COUNT) {
			g_rehandshake_reset_count++;
			spi_slave_ext_sensorhub_reset();
		} else {
...
void write_reset_mcu(void)
{
	...
	if (g_handshake[mcu_driver].write_fail_count > 0)
		g_handshake[mcu_driver].write_fail_retry_count++;
	else
		g_handshake[mcu_driver].write_fail_retry_count = 0;

	if (g_handshake[mcu_driver].write_fail_count == 0 &&
	    g_handshake[UART_DRIVER].write_fail_count == 0)
		g_write_reset_count = 0;
	if (g_handshake[mcu_driver].write_fail_retry_count >
	    MAX_REHANDSHAKE_RETRY_COUNT ||
	    g_handshake[UART_DRIVER].write_fail_retry_count >
	    MAX_REHANDSHAKE_RETRY_COUNT) {
		hwlog_info("spi write try count max, spi:%d, uart:%d",
			   g_handshake[mcu_driver].write_fail_retry_count,
			   g_handshake[UART_DRIVER].write_fail_retry_count);
		g_handshake[mcu_driver].write_fail_retry_count = 0;
		g_handshake[UART_DRIVER].write_fail_retry_count = 0;
		if (g_write_reset_count < MAX_REHANDSHAKE_RESET_COUNT) {
			...
			hwlog_info("%s write reset sensorhub continuous 3", __func__);
		}
	}
}
...
{
#ifdef CONFIG_EXT_INPUTHUB_QCOM
	if (!g_handshake[SPI_SLAVE_DRIVER].handshake_flag) {
		hwlog_info("%s spi need to rehandshake", __func__);
		g_handshake[SPI_SLAVE_DRIVER].driver = SPI_SLAVE_DRIVER;
		if (!schedule_work(&g_handshake[SPI_SLAVE_DRIVER].handshake_work)) {
			hwlog_warn("%s schedule spi handshake fail", __func__);
			cancel_work_sync(&g_handshake[SPI_SLAVE_DRIVER].handshake_work);
		}
	}
#else
	if (!g_handshake[SPI_DRIVER].handshake_flag) {
		...
		if (!schedule_work(&g_handshake[SPI_DRIVER].handshake_work)) {
			hwlog_warn("%s schedule spi handshake fail", __func__);
			cancel_work_sync(&g_handshake[SPI_DRIVER].handshake_work);
		}
	}
#endif
	if (!g_handshake[UART_DRIVER].handshake_flag) {
		...
		if (!schedule_work(&g_handshake[UART_DRIVER].handshake_work)) {
			...
			cancel_work_sync(&g_handshake[UART_DRIVER].handshake_work);
		}
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static void rehandshake_timer_callback(struct timer_list *arg)
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	__init_timer(&rehandshake_timer, NULL, 0);
#else
...
static int ext_sensorhub_frame_probe(struct platform_device *pdev)
{
	int i;
	int ret;
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		if (!g_commu_data[i])
			continue;
		ret = commu_init(&pdev->dev, g_commu_data[i]);
		if (ret < 0)
			return -EFAULT;
...
static s32 ext_sensorhub_frame_remove(struct platform_device *pdev)
{
	return 0;
}

static const struct of_device_id ext_sensorhub_frame_match[] = {
	{
		.compatible = "huawei,ext_frame",
		.data = NULL,
	},
};
MODULE_DEVICE_TABLE(of, ext_sensorhub_frame_match);


static int ext_sensorhub_frame_freeze(struct device *dev)
{
	int i;
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		...
		disable_irq(g_commu_data[i]->wake_irq);
		disable_irq(g_commu_data[i]->rx_ready_irq);
	}
	return 0;
}

static int ext_sensorhub_frame_restore(struct device *dev)
{
	int i;
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		...
		enable_irq(g_commu_data[i]->wake_irq);
		enable_irq(g_commu_data[i]->rx_ready_irq);
	}
	return 0;
}

static int ext_sensorhub_frame_suspend(struct device *dev)
{
	int i;
	int error;
	...
	if (mem_sleep_current == PM_SUSPEND_MEM)
		return ext_sensorhub_frame_freeze(dev);
#endif
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		...
		error = enable_irq_wake(g_commu_data[i]->wake_irq);
		if (error)
			pr_err("failed to enable IRQ %d as wake source: %d\n",
			       g_commu_data[i]->wake_irq, error);
	}
	return 0;
}

static int ext_sensorhub_frame_resume(struct device *dev)
{
	int error;
	int i;
...
#ifdef CONFIG_DEEPSLEEP
	if (mem_sleep_current == PM_SUSPEND_MEM)
		return ext_sensorhub_frame_restore(dev);
#endif
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		...
		error = disable_irq_wake(g_commu_data[i]->wake_irq);
		if (error)
...
static const struct dev_pm_ops ext_sensorhub_frame_pm_ops = {
	.freeze = ext_sensorhub_frame_freeze,
	.restore = ext_sensorhub_frame_restore,
	.suspend = ext_sensorhub_frame_suspend,
	.resume = ext_sensorhub_frame_resume,
};

static struct platform_driver ext_sensorhub_frame_driver = {
	.probe = ext_sensorhub_frame_probe,
	.remove = ext_sensorhub_frame_remove,
	.driver = {
			.name = "ext-frame",
			.pm = &ext_sensorhub_frame_pm_ops,
			.owner = THIS_MODULE,
			.of_match_table = of_match_ptr(ext_sensorhub_frame_match),
		},
};
...
int ext_sensorhub_frame_init(void)
{
	...
	handshake_wake_lock = wakeup_source_register(NULL, "commu_handshake_lock");
	...
	pr_info("%s use mode %d commu driver", __func__, commu_mode);
	if (commu_mode == SDIO_COMMU_MODE) {
		g_commu_data[SDIO_DRIVER] = get_commu_data(SDIO_DRIVER);
	} else if (commu_mode == SPI_COMMU_MODE) {
		...
		/* use spi and uart commu driver */
		g_commu_data[SPI_SLAVE_DRIVER] =
			get_commu_data(SPI_SLAVE_DRIVER);
		g_commu_data[UART_DRIVER] = get_commu_data(UART_DRIVER);
	}

	ret = platform_driver_register(&ext_sensorhub_frame_driver);

	pr_info("%s register driver ret:%d", __func__, ret);
	return 0;
}
...
void ext_sensorhub_frame_exit(void)
{
	...
	for (i = 0; i < COMMU_DRIVER_COUNT; ++i) {
		if (g_commu_data[i]->driver == SPI_DRIVER ||
		    g_commu_data[i]->driver == SPI_SLAVE_DRIVER)
...
void notify_mcu_reboot(enum commu_driver driver)
{
	...
	if (g_commu_data[driver] && g_commu_data[driver]->active_handshake) {
		...
		ret = schedule_work(&g_handshake[driver].handshake_work);
	}
	if (driver == SPI_SLAVE_DRIVER && ret) {
		/* 5000 ms stay awake till handshake successful */
		__pm_wakeup_event(handshake_wake_lock, 5000);
		pr_info("%s wakeup for handshake", __func__);
	}
	...
	if (driver == SPI_DRIVER || driver == SPI_SLAVE_DRIVER) {
		g_handshake[UART_DRIVER].handshake_flag = false;
...
bool check_commu_handshake(unsigned char tid)
{
	...
	commu_mode = get_commu_mode();
	/* default mode 0 only check sdio driver flag */
	if (commu_mode == SDIO_COMMU_MODE)
		return g_handshake[SDIO_DRIVER].handshake_flag;

	if (commu_mode == SPI_SLAVE_COMMU_MODE)
		return g_handshake[SPI_SLAVE_DRIVER].handshake_flag;
...
void relax_commu_wake_lock(void)
{
	...
	if (g_commu_data[SPI_SLAVE_DRIVER]) {
		pr_debug("%s relax spi wake lock", __func__);
		__pm_relax(g_commu_data[SPI_SLAVE_DRIVER]->user_wake_lock);
	}
}
...
void hold_commu_wake_lock(unsigned int msec)
{
	...
	if (g_commu_data[SPI_SLAVE_DRIVER]) {
		pr_debug("%s hold spi wake lock", __func__);
		__pm_wakeup_event(g_commu_data[SPI_SLAVE_DRIVER]->user_wake_lock,
				  msec);
	}
}

bool sensorhub_enter_recovery_mode(void)
{
	if (enter_recovery || enter_erecovery) {
		hwlog_info("%s ap not in normal mode", __func__);
		return true;
