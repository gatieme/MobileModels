...
static struct ext_sensorhub_route ext_route_tbl[] = {
	...
	{   ROUTE_DISPLAY_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_DISPLAY, 0
	},
	{   ROUTE_INPUT_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_INPUT, 0
	},
	{   ROUTE_FILE_TRANS_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_FILE_TRANS, 0
	},
	{   ROUTE_MATTINGALG_PORT, NULL,
		NULL, NULL, NULL,
		MAX_READ_LIST_LEN_MATTINGALG, 0
	},
};
	...
	struct ext_sensorhub_buf_list *resp_buf, int i)
{
	...
	if (ext_route_tbl[i].wait_resp) {
		...
		if (ext_route_tbl[i].wait_resp->service_id ==
			...
			if (resp_buf->read_cnt > 0 &&
			    ext_route_tbl[i].wait_resp->data == NULL) {
				hwlog_debug("%s, wait_resp->data kmalloc fail", __func__);
				return false;
			}
	...
	struct ext_sensorhub_buf_list *resp_buf, int i)
{
	...
	if (memcpy_s(buf_node->buffer, buf_node->read_cnt,
		     resp_buf->buffer, buf_node->read_cnt) != EOK) {
					...
					unsigned char *command_id)
{
	...
	*command_id = read_buf_node->command_id;
	...
	if (status > 0) {
		...
		if (memcpy_s(buf, count, read_buf_node->buffer, status) != EOK)
...
struct commu_state *get_commu_state(void)
{
	...
	if (commu_mode == SPI_COMMU_MODE ||
	    commu_mode == SPI_SLAVE_COMMU_MODE) {
		/* 2 commu records to show */
