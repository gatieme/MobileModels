	...
	uint8_t als_chn_noise_count)
{
	...
	uint8_t fram_idx;
	uint8_t chn_idx;
	uint8_t duration[FRAME_MAX] = {0};
	uint32_t duration_scope;

	duration_scope = als_end - als_start;
	if ((frame_num == FRAME_MAX) || (als_end <= als_start)) {
		duration_scope = ALS_INTEG_TIME;
	...
	for (fram_idx = 0;  fram_idx < frame_num; fram_idx++) {
		...
		if (fram_idx == FIRST_BOUNDARY) {
			if ((frame_num == 1) && (frame_time_stamp[fram_idx + FRAME_NEXT] < als_start))
				duration[fram_idx] = duration_scope;
			else
				duration[fram_idx] = frame_time_stamp[fram_idx + FRAME_NEXT] - als_start;
		} else if (frame_num - 1 == fram_idx) {
			// last boundary frame timestamp revise
			duration[fram_idx] = als_end - frame_time_stamp[fram_idx];
		} else {
			duration[fram_idx] = frame_time_stamp[fram_idx + FRAME_NEXT] -
				frame_time_stamp[fram_idx];
		}
	}
	...
	for (fram_idx = 0; fram_idx < frame_num; fram_idx++) {
		...
		for (chn_idx = 0; chn_idx < als_chn_noise_count; chn_idx++)
			als_chn_noise[chn_idx] += (uint32_t)(
				display_data->als_channel_noise[fram_idx][chn_idx] *
				duration[fram_idx] / duration_scope);
	}
}
