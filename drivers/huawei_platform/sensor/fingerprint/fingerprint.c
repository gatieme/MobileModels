...
#include <platform_include/cee/linux/hisi_cpufreq_lowtemp.h>
#endif
#include <platform_include/cee/linux/accurate_delay.h>
...
#ifndef FINGERPRINT_TP_DECOUPLE_ENABLE
#include "hisi_fb.h"
...
#define BACKLIGHT_WAIT_TIMEOUT (30000 * HZ / 1000) // 30s
	...
	struct device_attribute *attribute, char *buffer)
{
	...
	hwlog_info("irq_get : %d, irq_cnt: %u\n", irq, g_irq_print_cnt);
...
/* Update 'ic_name' & 'module_name' of struct dsm_dev
 * by unregist and regist dsm
 */
static void update_client_vendor_info(struct dsm_dev *dev)
{
	if (fingerprint_dclient == NULL || dev == NULL)
		return;
	dsm_unregister_client(fingerprint_dclient, dev);
	fingerprint_dclient = dsm_register_client(dev);
	hwlog_info("%s: ic_name=%s, module_name=%s \n", __func__, dev->ic_name, dev->module_name);
}
static void fingerprint_update_vendor_info(unsigned int sensor,char *screen_name)
{
	char *ic_name = NULL;

	mutex_lock(&dsm_lock);
	if (screen_name != NULL)
		dsm_fingerprint.module_name = screen_name;
	...
	snprintf(ic_name, FP_MAX_SENSOR_ID_LEN, "0x%x", sensor);
	dsm_fingerprint.ic_name = ic_name;
	update_client_vendor_info(&dsm_fingerprint);
	kfree(ic_name);
...
EXPORT_SYMBOL_GPL(ud_fp_on_hbm_completed);

void fp_on_lcd_backlight_change(int panel_id, int value)
{
	static int last_value = -1;
	if (g_fingerprint == NULL)
		return;
	// value > 0 is screen on
	if ((last_value == 0) && (value > 0)) {
		g_fingerprint->backlight_status = BACKLIGHT_ON;
		hwlog_info("%s: backlight on\n", __func__);
		wake_up(&g_fingerprint->backlight_queue);
	} else if ((last_value > 0) && (value == 0)) {
		g_fingerprint->backlight_status = BACKLIGHT_OFF;
		hwlog_info("%s: backlight off\n", __func__);
		wake_up(&g_fingerprint->backlight_queue);
	}
	last_value = value;
}
EXPORT_SYMBOL_GPL(fp_on_lcd_backlight_change);
...
struct ud_fp_display_ops g_fp_display_operations = {NULL, NULL, NULL};

void ud_fp_display_register(struct ud_fp_display_ops *fp_display_ops)
{
	if (g_fp_display_operations.de_open_update_te == NULL) {
		g_fp_display_operations.de_open_update_te = fp_display_ops->de_open_update_te;
		hwlog_err("%s:fp_get_local_hbm_mode register sussess\n", __func__);
	} else {
		hwlog_err("%s:fp_get_local_hbm_mode has been registered\n", __func__);
	}
	...
	if (g_fp_display_operations.fp_set_brightness_level == NULL) {
		...
		hwlog_err("%s:fp_set_brightness_level register sussess\n", __func__);
	} else {
		hwlog_err("%s:fp_set_brightness_level has been registered\n", __func__);
	}
}
...
{
	if (g_fp_display_operations.de_open_update_te != NULL) {
		g_fp_display_operations.de_open_update_te = NULL;
		hwlog_err("%s:de_open_update_te unregister sussess\n", __func__);
	} else {
		hwlog_err("%s:de_open_update_te has been unregistered\n", __func__);
	}

	if (g_fp_display_operations.fp_get_local_hbm_mode != NULL) {
		g_fp_display_operations.fp_get_local_hbm_mode = NULL;
		hwlog_err("%s:fp_get_local_hbm_mode unregister sussess\n", __func__);
	} else {
		hwlog_err("%s:fp_get_local_hbm_mode has been unregister\n", __func__);
	}

	if (g_fp_display_operations.fp_set_brightness_level != NULL) {
		g_fp_display_operations.fp_set_brightness_level = NULL;
		hwlog_err("%s:fp_set_brightness_level unregister sussess\n", __func__);
	} else {
		hwlog_err("%s:fp_set_brightness_level has been unregistered\n", __func__);
	}
}
...
{
	hwlog_info("fingerprint_ioctl FP_IOC_CMD_ENABLE_IRQ, irq_cnt: %u\n", g_irq_print_cnt);
	if (fp->use_tp_irq == USE_TP_IRQ) {
		pr_err("%s: USE_TP_IRQ\n", __func__);
		return;
	}
...
{
	hwlog_info("fingerprint_ioctl FP_IOC_CMD_DISABLE_IRQ, irq_cnt: %u\n", g_irq_print_cnt);
	if (fp->use_tp_irq == USE_TP_IRQ) {
		pr_err("%s: USE_TP_IRQ\n", __func__);
		return;
	}
...
#if defined(CONFIG_HUAWEI_DSM)
	fingerprint_update_vendor_info(fp->sensor_id, NULL);
#endif
...
static int fp_ioc_send_screen_name(struct fp_data *fp, const void __user *argp)
{
	int ret;
	...
	hwlog_info("fp_ioctl_cmd info, info.data_len = %d\n",
		info.data_len);

	ret = copy_from_user(fp->screen_name, info.data_addr, info.data_len);
	fp->screen_name[MAX_SCREEN_NAME_LEN - 1] = '\0';
	if (ret) {
		hwlog_err("%s copy_from_user failed\n", __func__);
		return -EFAULT;
	}
#if defined(CONFIG_HUAWEI_DSM)
	fingerprint_update_vendor_info(fp->sensor_id_ud, fp->screen_name);
#endif
	return FP_RETURN_SUCCESS;
}
...
static int fp_ioc_wait_backlight_status(struct fp_data *fp)
{
	do
		wait_event_timeout(fp->backlight_queue,
			fp->backlight_status != BACKLIGHT_NONE, BACKLIGHT_WAIT_TIMEOUT);
	while (fp->backlight_status == BACKLIGHT_NONE);
	int status = fp->backlight_status;
	fp->backlight_status = BACKLIGHT_NONE;
	return status;
}
...
static int fp_ioc_get_s4_status(void __user *argp)
{
	int ret = copy_to_user(argp, &is_s4_mode, sizeof(is_s4_mode));
	if (ret) {
		hwlog_err("%s copy_to_user failed, ret=%d\n",
			__func__, ret);
		return -EFAULT;
	}
	is_s4_mode = false; /* clear after read once */
	return FP_RETURN_SUCCESS;
}
...
#else
	if (g_fp_display_operations.de_open_update_te != NULL)
		g_fp_display_operations.de_open_update_te();
#endif
	return FP_RETURN_SUCCESS;
}

static int fp_ioc_mmi_test_status(const void __user *argp)
{
	unsigned int mmi_test_status = 0;

	if (copy_from_user(&mmi_test_status, argp, sizeof(mmi_test_status))) {
		hwlog_err("%s copy_from_user failed\n", __func__);
		return -EFAULT;
	}

	hwlog_info("fingerprint_ioctl FP_IOC_CMD_MMI_TEST_STATUS = %u\n",
		mmi_test_status);
	if (mmi_test_status == MMI_TEST_ENTER) {
		lpcpu_idle_sleep_vote(ID_FINGERPRINT, 1);
	} else if (mmi_test_status == MMI_TEST_EXIT) {
		lpcpu_idle_sleep_vote(ID_FINGERPRINT, 0);
	}
	...
	unsigned long arg)
{
	...
	case FP_IOC_CMD_GET_BRIGHTNESS_MODE:
		...
		ret = get_supported_hbm_mode(argp);
		break;
	case FP_IOC_CMD_SET_BRIGHTNESS_LEVEL:
		hwlog_info("%s FP_IOC_CMD_SET_BRIGHTNESS_LEVEL\n", __func__);
		ret = set_brightness_level(argp);
		break;
	case FP_IOC_CMD_SEND_SCREEN_NAME:
		hwlog_info("%s FP_IOC_CMD_SEND_SCREEN_NAME\n", __func__);
		ret = fp_ioc_send_screen_name(fp, argp);
		break;
	case FP_IOC_CMD_WAIT_BACKLIGHT_STATUS:
		hwlog_info("%s FP_IOC_CMD_WAIT_BACKLIGHT_STATUS\n", __func__);
		ret = fp_ioc_wait_backlight_status(fp);
		break;
	case FP_IOC_CMD_MMI_TEST_STATUS:
		hwlog_info("%s FP_IOC_CMD_FP_MMI_TEST_STATUS\n", __func__);
		ret = fp_ioc_mmi_test_status(argp);
		break;
	case FP_IOC_CMD_GET_S4_STATUS:
		hwlog_info("%s FP_IOC_CMD_GET_S4_STATUS\n", __func__);
		ret = fp_ioc_get_s4_status(argp);
		break;
	default:
...
static int fingerprint_s4_freeze(struct device *dev)
{
	pr_info("%s\n", __FUNCTION__);
	is_s4_mode = false;
	return 0;
...
static int fingerprint_s4_restore(struct device *dev)
{
	pr_info("%s\n", __FUNCTION__);
	is_s4_mode = true;
	return 0;
...
#if defined(CINFIG_FINGERPEINT_RESET_GPIO)
...
static int fingerprint_device_init(struct fp_data *fp, struct device *dev)
{
	...
	fp->ttw_wl = wakeup_source_register(dev, "fpc_ttw_wl");
#else
	fp->ttw_wl = wakeup_source_register("fpc_ttw_wl");
#endif
	init_waitqueue_head(&fp->hbm_queue);
	init_waitqueue_head(&fp->backlight_queue);
	mutex_init(&fp->lock);
...
static int fingerprint_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(fp->ttw_wl);
	fp->ttw_wl = NULL;
...
static const struct dev_pm_ops fingerprint_pm = {
	...
	.freeze_noirq = fingerprint_s4_freeze,
	.restore_noirq = fingerprint_s4_restore,
};
