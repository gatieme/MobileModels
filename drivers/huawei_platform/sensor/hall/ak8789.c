...
#define HALL_IRQ_NUM_LENGTH 8
#define INVALID_IRQ_GAP_US_TIME  250000
#define DEFAULT_VAL 1
static atomic_t cur_irq_num = ATOMIC_INIT(0);
static int hall_irq_num[HALL_IRQ_NUM_LENGTH] = {0};
static unsigned int g_support_disable_irq = 0;
...
static unsigned int ext_fold_compare_val = 0x01;
static unsigned int is_support_fold_status = 0;
static unsigned int is_support_boost_5v_status = 0;
static unsigned int is_support_notifier = 0;
int hinge_status = 0;
static bool is_support_invalid_irq_check = false;
// record every irq occur time
static long long irq_occur_time_us;
...
static void process_pen_irq(struct ak8789_data *data, unsigned int value)
{
	...
	if (pen_irq_cnt >= limit_irq_cnt || pen_both_on_cnt >= limit_both_on_cnt) {
		...
		hall_report_dsm_info(data, true);
	}
...
int ak8789_register_enable(bool enable)
{
	/**
	 * hall_irq_num[0] is smart cover，[2] is keyboard [4,5] is pen
	*/
	hwlog_info("%s:enable = %d, g_support_disable_irq = %u\n",
		__func__, enable, g_support_disable_irq);
	if (g_support_disable_irq != 1)
		return 0;
	spin_lock_irqsave(&irq_lock, irq_flag);
	if (enable) {
		enable_irq(hall_irq_num[0]);
	} else {
		disable_irq(hall_irq_num[0]);
	}
	spin_unlock_irqrestore(&irq_lock, irq_flag);
	return 0;
}
EXPORT_SYMBOL_GPL(ak8789_register_enable);
...
static int ext_hall_data_compare(int hall_val, unsigned int comp_val)
{
	...
	bit_tmp = (int)(((unsigned int)hall_val >> comp_val) &
		ext_fold_compare_val);
	return bit_tmp;
}

int ak8789_hinge_get_ext_hall_status(void)
{
	int value;

	if (!data_ak8789) {
		hwlog_err("%s: hall not inited\n", __func__);
		return 0;   // 0 is fold
	}

	value = ak8789_get_status(&data_ak8789->cdev);
	hinge_status = ext_hall_data_compare(value, EXT_BIT1_COMP_VAL);
	hwlog_info("%s: hinge_status = %d\n", __func__, hinge_status);
	return hinge_status;
}
EXPORT_SYMBOL_GPL(ak8789_hinge_get_ext_hall_status);

void report_hall_fold_status_when_poweroff_charging(struct ak8789_data *data)
{
	static int pre_status = -1;
	char *fold_on[] = {"FOLDSTAUS=0", NULL};
	char *fold_off[] = {"FOLDSTAUS=1", NULL};

	if (hinge_status == pre_status)
		return;
	pre_status = hinge_status;
	hwlog_info("%s status = %d\n", __func__, hinge_status);
	if (hinge_status == 0)
		kobject_uevent_env(&data->cdev.dev->kobj, KOBJ_CHANGE, fold_on);
	else
		kobject_uevent_env(&data->cdev.dev->kobj, KOBJ_CHANGE, fold_off);
}

static void record_irq_occur_time(int64_t tv_sec, int64_t tv_nsec)
{
	irq_occur_time_us = tv_sec * (1000000L) + tv_nsec / (1000L);
}

static bool is_irq_occur_invalid(void)
{
	struct timespec64 now;
	long long now_time_us;

	ktime_get_real_ts64(&now);
	now_time_us = now.tv_sec * (1000000L) + now.tv_nsec / (1000L);
	if ((now_time_us - irq_occur_time_us) < INVALID_IRQ_GAP_US_TIME)
		return true;
...
static void ak8789_report_delayed_work(struct work_struct *work)
{
	...
	hwlog_info("get hall value:%u, workaround_is_support:%u\n", pdata, workaround_is_support);
	hall_set_irq_info(data, pdata);
	if (workaround_is_support)
		process_pen_irq(data, pdata);
	hinge_status = ext_hall_data_compare(pdata, EXT_BIT1_COMP_VAL);
	if (is_support_notifier)
		ext_hall_status_distinguish((unsigned long)pdata);
	if (is_support_invalid_irq_check) {
		is_invalid_irq_status = is_irq_occur_invalid();
		hwlog_info("this time check is_irq_occur_invalid = %u\n", is_invalid_irq_status);
		if (is_invalid_irq_status) {
			hwlog_info("restart h_delayed_work check work\n");
			queue_delayed_work(data->hall_wq, &h_dev->h_delayed_work,
				msecs_to_jiffies(h_dev->hall_int_delay));
			return;
		}
	}
	if (ak8789_report_data(data, pdata) == 0)
		hwlog_info("report hall event data\n");
	if (is_support_fold_status)
		report_hall_fold_status_when_poweroff_charging(data);
}
				     ...
				     struct ak8789_data *data)
{
	...
	record_irq_occur_time(now.tv_sec, now.tv_nsec);
...
static irqreturn_t ak8789_event_handler(int irq, void *hall_dev)
{
	...
	if (h_dev == NULL) {
		hwlog_err("%s: h_dev = NULL\n", __func__);
		return IRQ_NONE;
	}

	h_desc = irq_to_desc(irq);
	if (h_desc == NULL) {
		hwlog_err("%s:h_desc = NULL\n", __func__);
		return IRQ_NONE;
	}
	...
	if (hall_gpio_type & 0x01) {
		if (trig_val & IRQF_TRIGGER_FALLING) {
			...
			if (ret < 0) {
				hwlog_err("%s:set Rising failed\n", __func__);
				return IRQ_NONE;
			}
		} else if (trig_val & IRQF_TRIGGER_RISING) {
			ret = irq_set_irq_type(irq, IRQF_TRIGGER_FALLING);
			if (ret < 0) {
				hwlog_err("%s:set Falling failed\n", __func__);
				return IRQ_NONE;
			}
		} else {
			hwlog_err("%s:not set trigger\n", __func__);
			return IRQ_NONE;
		}
	} else {
		if (trig_val & IRQF_TRIGGER_LOW) {
			...
			if (ret < 0) {
				hwlog_err("%s:set High failed\n", __func__);
				return IRQ_NONE;
			}
		} else if (trig_val & IRQF_TRIGGER_HIGH) {
			ret = irq_set_irq_type(irq, IRQF_TRIGGER_LOW);
			if (ret < 0) {
				hwlog_err("%s:set Low failed\n", __func__);
				return IRQ_NONE;
			}
		} else {
			hwlog_err("%s:not set high and low\n", __func__);
			return IRQ_NONE;
		}
	}
	...
	hwlog_info("%s: irq %d occurred, trig_val = %u\n", __func__, irq, trig_val);
				   ...
				   enum hall_type type)
{
	...
	/* judge bit 0 is 1 */
	if (gpio_type & 0x01) {
		ret = request_threaded_irq(irq, ak8789_event_handler, ak8789_event_handler,
					   IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | flag,
...
{
	unsigned int tmp = 0;
	if (!of_property_read_u32(node, "is_support_fold_status_report", &tmp)) {
		is_support_fold_status = tmp;
		hwlog_info("is_support_fold_status = %u.\n", is_support_fold_status);
	}

	if (!of_property_read_u32(node, "is_support_notifier", &tmp)) {
		is_support_notifier = tmp;
		hwlog_info("is_support_notifier = %u.\n", is_support_notifier);
	}

	if (!of_property_read_u32(node, "ext_fold_compare_val", &tmp)) {
		ext_fold_compare_val = tmp;
		hwlog_info("ext_fold_compare_val = %u\n", ext_fold_compare_val);
	}

	if (!of_property_read_u32(node, "is_support_boost_5v_enable", &tmp)) {
		is_support_boost_5v_status = tmp;
		hwlog_info("is_support_boost_5v_status = %u.\n", is_support_boost_5v_status);
	}

	if (is_support_boost_5v_status)
		boost_5v_enable(BOOST_5V_ENABLE, BOOST_CTRL_HALL);
	...
	is_support_invalid_irq_check = of_property_read_bool(node, "is_support_invalid_irq_check");
	hwlog_info("is_support_invalid_irq_check = %u\n", is_support_invalid_irq_check);
...
/* ak8789 init_data */
	...
	// for disable smart cover irq
	ret = of_property_read_u32(node, HALL_SUPPORT_DISABLE_IRQ, &g_support_disable_irq);
	if (ret != 0) {
		hwlog_err("%s: Failed to get g_support_disable_irq, use default flag\n",
			__func__);
		g_support_disable_irq = 0;
	}
