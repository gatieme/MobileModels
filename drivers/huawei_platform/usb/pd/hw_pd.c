...
static bool g_abnormal_adapter = false;
...
#define REPORT_BC12_WAIT_PD_FINISH_CNT  20
#define DP_PULL_UP_DELAY_TIME           1300
...
#ifdef CONFIG_REVERSE_CHARGE
static bool pd_dpm_get_rev_sc_dev_flag(void)
{
	if (g_ops && g_ops->pd_dpm_get_rev_sc_dev_flag)
		return g_ops->pd_dpm_get_rev_sc_dev_flag();

	return false;
}

static void pd_dpm_set_rev_sc_dev_flag(bool flag)
{
	if (g_ops && g_ops->pd_dpm_set_rev_sc_dev_flag)
		g_ops->pd_dpm_set_rev_sc_dev_flag(flag);
}

static void pd_dpm_set_rev_sc_switch_flag(int rever_point)
{
	if (g_ops && g_ops->pd_dpm_set_rev_sc_switch_flag)
		g_ops->pd_dpm_set_rev_sc_switch_flag(rever_point);
}

static int pd_dpm_is_reverse_charge_enable(void)
{
	if (g_ops && g_ops->pd_dpm_is_reverse_charge_enable)
		return g_ops->pd_dpm_is_reverse_charge_enable();

	return 0;
}

int hw_pd_is_reverse_charge_enable(void)
{
	return pd_dpm_is_reverse_charge_enable();
}

static bool pd_dpm_is_rev_sc_plugin(void)
{
	if (g_ops && g_ops->pd_dpm_is_rev_sc_plugin)
		return g_ops->pd_dpm_is_rev_sc_plugin();

	return false;
}

static bool pd_dpm_is_rev_sc_plugout(void)
{
	if (g_ops && g_ops->pd_dpm_is_rev_sc_plugout)
		return g_ops->pd_dpm_is_rev_sc_plugout();
...
#ifdef CONFIG_REVERSE_CHARGE
static ssize_t reverse_sc_support_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{

	if (!g_reverse_sc.support_reverse_sc)
		return -EINVAL;

	if (pd_dpm_is_rev_sc_plugin())
		g_reverse_sc.reverse_sc_detect_result |= REVERSE_SC_PLUG_IN;
	else
		g_reverse_sc.reverse_sc_detect_result &= ~REVERSE_SC_PLUG_IN;
	hwlog_info("notify_detect_result = %d\n", g_reverse_sc.reverse_sc_detect_result);
	return scnprintf(buf, PAGE_SIZE, "%d\n", g_reverse_sc.reverse_sc_detect_result);
}

static ssize_t reverse_sc_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if (!g_reverse_sc.support_reverse_sc)
		return -EINVAL;

	hwlog_info("%s = %d\n", __func__, g_reverse_sc.reverse_sc_mode);
	return scnprintf(buf, PAGE_SIZE, "%d\n", g_reverse_sc.reverse_sc_mode);
}

static ssize_t reverse_sc_mode_store(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	long val = 0;
	...
	g_reverse_sc.reverse_sc_mode = (int)val;
	pd_dpm_set_rev_sc_switch_flag(g_reverse_sc.reverse_sc_mode);
	return count;
}
#endif /* CONFIG_REVERSE_CHARGE */
...
#ifdef CONFIG_REVERSE_CHARGE
static DEVICE_ATTR(reverse_sc_support, S_IRWXU | S_IRWXG | S_IROTH, reverse_sc_support_show, NULL);
static DEVICE_ATTR(reverse_sc_mode, S_IRWXU | S_IRWXG | S_IROTH, reverse_sc_mode_show, reverse_sc_mode_store);
#endif /* CONFIG_REVERSE_CHARGE */
...
#ifdef CONFIG_REVERSE_CHARGE
	&dev_attr_reverse_sc_support.attr,
	&dev_attr_reverse_sc_mode.attr,
#endif /* CONFIG_REVERSE_CHARGE */
	NULL,
};
...
static int pd_dpm_hw_get_dp_state(void)
{
	int ret = -EINVAL;

	if (g_ops && g_ops->pd_dpm_get_dp_state)
		ret = g_ops->pd_dpm_get_dp_state(g_client);
...
int hw_pd_set_source_state(struct otg_cap *allow_ext)
{
	...
	/*
	 * waiting 600ms for DP communication, If the DP mode is entered,
	 * the reverse charging of large power is not supported.
	 */
	msleep(600);

	if (pd_dpm_hw_get_dp_state()) {
		hwlog_err("%s not support\n", __func__);
		return -EACCES;
	}
...
#ifdef CONFIG_REVERSE_CHARGE
			if (g_reverse_sc.support_reverse_sc &&
				pd_dpm_is_reverse_charge_enable()) {
...
int pd_dpm_report_bc12(struct notifier_block *usb_nb, unsigned long event, void *data)
{
	...
	if (g_abnormal_adapter) {
		do {
			count++;
			/* sleep 20ms to wait pd communicte finish */
			msleep(20);
			if (count > REPORT_BC12_WAIT_PD_FINISH_CNT) {
				hwlog_info("%s :delay time = %d\n", __func__, count);
				break;
			}
		} while (!di->pd_finish_flag);
		g_abnormal_adapter = false;
	}
...
static void reverse_sc_attach_work(struct work_struct *work)
{
	...
	hwlog_info("%s start\n", __func__);
	g_reverse_sc.reverse_sc_exit = false;
	for (i = 0; i < REVERSE_SC_DETECTED_TIMES; i++) {
		if (g_reverse_sc.reverse_sc_exit) {
			hwlog_info("%s:detect exit\r\n", __func__);
			g_reverse_sc.reverse_sc_detect_result = 0;
			return;
		}

		if (pd_dpm_get_rev_sc_dev_flag()) {
			hwlog_info("%s:pd dev detect success\n", __func__);
			g_reverse_sc.reverse_sc_detect_result |= REVERSE_SC_PD_DEV_DETECTED;
		}

		/* 1: bc1.2 detect succ */
		if (rprot_get_dev_det_result() == 1) {
			hwlog_info("%s: bc1.2 detect success\n", __func__);
			g_reverse_sc.reverse_sc_detect_result |= REVERSE_SC_BC12_DETECTED;
		}

		if ((g_reverse_sc.reverse_sc_detect_result & REVERSE_SC_PD_DEV_DETECTED) &&
			(g_reverse_sc.reverse_sc_detect_result & REVERSE_SC_BC12_DETECTED))
				break;
...
static int reverse_sc_device_detect(void)
{
	hwlog_info("%s\n", __func__);

	if (pd_dpm_is_reverse_charge_enable())
		// 1: skip usb enumeration
		return 1;

	if (pd_dpm_is_rev_sc_plugin())
		schedule_delayed_work(&g_reverse_sc.reverse_sc_attach_work,
...
#ifdef CONFIG_REVERSE_CHARGE
	if (g_reverse_sc.support_reverse_sc && reverse_sc_device_detect())
		return;
#endif /* CONFIG_REVERSE_CHARGE */
...
static void pd_dpm_report_attach(int new_state)
{
	...
	case PD_DPM_USB_TYPEC_HOST_ATTACHED:
		...
		HWDUBAI_LOGE("DUBAI_TAG_OTG_STATE", "state=%d", 1);
		break;
	default:
		break;
	}
...
static void pd_dpm_report_detach(int last_state)
{
	...
	case PD_DPM_USB_TYPEC_HOST_ATTACHED:
		...
		HWDUBAI_LOGE("DUBAI_TAG_OTG_STATE", "state=%d", 0);
		rchg_pd_otg_enable(false);
		hwlog_info("%s:notify charger disable source vbus\n", __func__);
		break;
	default:
...
static void hw_pd_set_dp_pin(int high)
{
	if (!g_water_det_ops || !g_water_det_ops->set_dp_pin) {
		hwlog_err("g_water_det_ops or set_dp_pin is null\n");
		return;
	}

	g_water_det_ops->set_dp_pin(high);
}
...
int pd_dpm_handle_pe_event(unsigned long event, void *data)
{
			...
			case PD_DPM_TYPEC_UNATTACHED:
				...
				if (pd_dpm_analog_hs_state == 1) {
					...
					if (g_reverse_sc.support_reverse_sc) {
						...
						pd_dpm_set_rev_sc_dev_flag(false);
						if (pd_dpm_is_rev_sc_plugout())
							g_reverse_sc.reverse_sc_detect_result = 0;
						g_reverse_sc.reverse_sc_exit = true;
						cancel_delayed_work_sync(&g_reverse_sc.reverse_sc_attach_work);
					}
#endif /* CONFIG_REVERSE_CHARGE */
...
#ifdef CONFIG_REVERSE_CHARGE
			if (g_reverse_sc.support_reverse_sc && pd_dpm_is_reverse_charge_enable())
				return 0;
#endif /* CONFIG_REVERSE_CHARGE */
	...
	case PD_DPM_PE_CABLE_DATA:
		...
		break;
	case PD_DPM_PE_ADAPTER_DATA:
		hwlog_info("PD_DPM_PE_ADAPTER_DATA enter");
		struct pd_dpm_adapter_data *adapter_data = data;
		if (adapter_data->status) {
			g_abnormal_adapter = true;
			/* Pull up Dp for 1.3s and then release it. The abnormal adapter Vbus is enabled */
			hw_pd_set_dp_pin(1);
			msleep(DP_PULL_UP_DELAY_TIME);
			hw_pd_set_dp_pin(0);
		}
		break;
	default:
...
#ifdef CONFIG_REVERSE_CHARGE
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_reverse_sc", &g_reverse_sc.support_reverse_sc, 0);
#endif /* CONFIG_REVERSE_CHARGE */
}
...
#ifdef CONFIG_REVERSE_CHARGE
	if (g_reverse_sc.support_reverse_sc)
		INIT_DELAYED_WORK(&g_reverse_sc.reverse_sc_attach_work, reverse_sc_attach_work);
#endif /* CONFIG_REVERSE_CHARGE */
