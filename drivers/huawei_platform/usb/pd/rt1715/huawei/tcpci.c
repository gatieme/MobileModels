...
int tcpc_check_notify_time(struct tcpc_device *tcpc,
	...
	struct tcpc_device *tcpc, uint8_t type, int mv, int ma)
{
	...
	if (type >= TCP_VBUS_CTRL_PD &&
			tcpc->pd_port.pe_data.pd_prev_connected) {
		tcp_noti.vbus_state.pd_connect_flag = true;
		type |= TCP_VBUS_CTRL_PD_DETECT;
		dpm_flag = tcpm_inquire_dpm_flags(tcpc);
		tcp_noti.vbus_state.ext_power = dpm_flag & DPM_FLAGS_PARTNER_EXTPOWER;
	}
#else
	if (type >= TCP_VBUS_CTRL_PD &&
			tcpc->pd_port.pe_data.pd_prev_connected)
...
int tcpci_report_hpd_state(struct tcpc_device *tcpc, uint32_t dp_status)
{
	...
	if (PD_DP_CFG_DFP_D(dp_data->local_config)) {
		...
		pr_info("\n%s[v2] + state = %d,irq = %d,dp_status = %d\n",
...
int tcpci_dp_configure(struct tcpc_device *tcpc, uint32_t dp_config)
{
	...
	if (MODE_DP_PIN_C == tcp_noti.ama_dp_state.pin_assignment)
		g_mux_type = TCPC_DP;
	else if (MODE_DP_PIN_D == tcp_noti.ama_dp_state.pin_assignment)
	...
	event.typec_orien = tcpc->typec_polarity;
	pd_dpm_handle_combphy_event(event);
	pr_info("\nhuawei_pd %s pd_event_notify, g_mux_type = %d\n", __func__, g_mux_type);
#endif
