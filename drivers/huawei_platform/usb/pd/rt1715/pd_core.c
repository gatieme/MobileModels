...
static const struct {
	...
	{"attemp_enter_dp_mode", DPM_CAP_ATTEMPT_ENTER_DP_MODE},
	{"attemp_discover_cable", DPM_CAP_ATTEMPT_DISCOVER_CABLE},
	{"attemp_discover_id", DPM_CAP_ATTEMPT_DISCOVER_ID},
	{"attemp_discover_svid", DPM_CAP_ATTEMPT_DISCOVER_SVID},
	...
	{"dp_prefer_mf", DPM_CAP_DP_PREFER_MF},
};
...
int pd_handle_soft_reset(struct pd_port *pd_port)
{
	...
	pd_notify_tcp_event_buf_reset_ex(pd_port, TCP_DPM_RET_DROP_RECV_SRESET);
...
int pd_send_message_ex(struct pd_port *pd_port, uint8_t sop_type,
...
{
	return pd_send_message_ex(pd_port,
		...
		uint8_t chunk_nr, uint8_t size, const uint8_t *data)
{
	...
	*ext_hdr = PD_EXT_HEADER_CK(size, request, chunk_nr, true);
	...
	return pd_send_message_ex(pd_port, sop_type, msg, true, cnt, payload);
}
#endif	/* CONFIG_USB_PD_REV30 */
...
int pd_send_soft_reset(struct pd_port *pd_port)
{
	...
	pd_reset_protocol_layer(pd_port, true);
	pd_notify_tcp_event_buf_reset_ex(pd_port, TCP_DPM_RET_DROP_SENT_SRESET);
	tcpci_notify_pd_state(pd_port->tcpc, PD_CONNECT_SOFT_RESET);
...
int pd_send_bist_mode2(struct pd_port *pd_port)
{
	...
	pd_notify_tcp_event_buf_reset_ex(pd_port, TCP_DPM_RET_DROP_SEND_BIST);
