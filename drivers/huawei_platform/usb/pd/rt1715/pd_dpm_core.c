		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	return dpm_find_match_req_info_ex(req_info,
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		...
		find_cap = dpm_find_match_req_info_ex(req_info,
				&sink, src_cap->nr, src_cap->pdos,
...
int pd_dpm_update_tcp_request_rt1715(struct pd_port *pd_port,
...
int pd_dpm_update_tcp_request_ex_rt1715(struct pd_port *pd_port,
...
int pd_dpm_update_tcp_request_again_ex(struct pd_port *pd_port)
{
...
{
	uint32_t reactions = DPM_REACTION_CAP_ALWAYS | DPM_REACTION_CAP_DISCOVER_CABLE;
