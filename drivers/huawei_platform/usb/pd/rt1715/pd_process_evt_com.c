	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	case PD_CTRL_REJECT:
		pd_notify_tcp_event_2nd_result_ex(pd_port, TCP_DPM_RET_REJECT);
		...
		break;
	case PD_CTRL_WAIT:
		pd_notify_tcp_event_2nd_result_ex(pd_port, TCP_DPM_RET_WAIT);
	...
	case PD_CTRL_NOT_SUPPORTED:
		pd_cancel_dpm_reaction(pd_port);
		pd_notify_tcp_event_2nd_result_ex(
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	case PD_TIMER_SENDER_RESPONSE:
		...
		pd_cancel_dpm_reaction(pd_port);
		pd_notify_pe_cancel_pr_swap(pd_port);
		pd_notify_tcp_event_2nd_result_ex(pd_port, TCP_DPM_RET_TIMEOUT);
		if (pd_port->pe_data.pe_state_flags &
			PE_STATE_FLAG_BACK_READY_IF_SR_TIMER_TOUT) {
	...
	case PD_TIMER_DEFERRED_EVT:
		pd_notify_tcp_event_buf_reset_ex(
