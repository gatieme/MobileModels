...
#ifdef CONFIG_TCPC_RT1715
	/* reg:0x10 bit0:CC_STATUS bit1:PWR_STATUS */
	if (get_bit(alert_status, 0) && !get_bit(alert_status, 1)) {
		et_flag_reg = true;
		TCPC_INFO("%s 0x10 -> bit0 = 1,bit1 = 0\n", __func__);
	}
#endif
...
#ifdef CONFIG_TCPC_RT1715
	if (g_vbus_reg_flag && et_flag_reg) {
		if (et7303_get_vbus_reg_status(tcpc)) {
			tcpci_vbus_level_changed(tcpc);
			TCPC_INFO("%s first enter change\n", __func__);
		}
		g_vbus_reg_flag = false;
	}
#endif
...
{
	TCPC_INFO("usb_port_attached, typec_attach_new=%d\n", tcpc->typec_attach_new);
	...
	if (tcpc->pd_inited_flag)
		pd_put_cc_attached_event_ex(tcpc, tcpc->typec_attach_new);
