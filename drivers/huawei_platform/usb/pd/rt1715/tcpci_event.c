...
bool pd_get_deferred_tcp_event_ex(
	struct tcpc_device *tcpc, struct tcp_dpm_event *tcp_event)
{
	bool ret;
...
bool pd_put_deferred_tcp_event_ex(
	struct tcpc_device *tcpc, const struct tcp_dpm_event *tcp_event)
{
	bool ret = true;
	struct pd_port *pd_port = &tcpc->pd_port;
...
void pd_notify_tcp_event_2nd_result_ex(struct pd_port *pd_port, int ret)
{
#ifdef CONFIG_USB_PD_TCPM_CB_2ND
	struct tcp_dpm_event *tcp_event = &pd_port->tcp_event;
...
void pd_notify_tcp_event_1st_result_ex(struct pd_port *pd_port, int ret)
{
	bool cb = true;
	struct tcp_dpm_event *tcp_event = &pd_port->tcp_event;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
	...
	struct tcpc_device *tcpc, uint8_t reason)
{
	...
	pd_notify_tcp_event_2nd_result_ex(&tcpc->pd_port, reason);
...
void pd_notify_tcp_event_buf_reset_ex(struct pd_port *pd_port, uint8_t reason)
{
	struct tcpc_device *tcpc = pd_port->tcpc;

	pd_notify_tcp_event_1st_result_ex(pd_port, reason);
...
bool pd_put_cc_attached_event_ex(
		struct tcpc_device *tcpc, uint8_t type)
{
	bool ret = false;
