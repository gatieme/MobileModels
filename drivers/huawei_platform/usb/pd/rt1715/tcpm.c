...
/* Check status */
int tcpm_check_typec_attached(struct tcpc_device *tcpc)
{
...
int tcpm_get_remote_power_cap_ex(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_get_remote_power_cap_ex);
...
int tcpm_dpm_pd_get_source_cap_ext_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *cb_data,
...
EXPORT_SYMBOL(tcpm_dpm_pd_get_source_cap_ext_ex);
...
int tcpm_dpm_pd_get_status_ex(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_dpm_pd_get_status_ex);
...
int tcpm_dpm_pd_get_pps_status_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *cb_data,
...
EXPORT_SYMBOL(tcpm_dpm_pd_get_pps_status_ex);
	...
	struct pd_port *pd_port, struct tcp_dpm_event *event)
{
	...
	mutex_lock(&pd_port->pd_lock);
	pd_notify_tcp_event_2nd_result_ex(pd_port, reason);
	...
	struct tcpc_device *tcpc, struct tcp_dpm_event *event)
{
	...
	if (imme) {
		...
		mutex_lock(&pd_port->pd_lock);
		ret = pd_put_deferred_tcp_event_ex(tcpc, event);
		mutex_unlock(&pd_port->pd_lock);
	}
...
int tcpm_reset_pd_charging_policy(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *cb_data)
{
	return tcpm_set_pd_charging_policy_ex(
...
int tcpm_set_pd_charging_policy_ex(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_set_pd_charging_policy_ex);
...
int tcpm_set_direct_charge_enable(void *client, bool en)
{
	...
	pr_info("%s : en->%d\n", __func__, en);
	tcpm_set_direct_charge_en(tcpc, en);

	return  TCPM_SUCCESS;
}
EXPORT_SYMBOL(tcpm_set_direct_charge_enable);
#endif
...
int tcpm_set_apdo_charging_policy_ex(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_set_apdo_charging_policy_ex);

int tcpm_inquire_pd_source_apdo_ex(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_inquire_pd_source_apdo_ex);
