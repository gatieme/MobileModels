...
#define  DEFAULT_RAM_SHORT_D2S_GAIN             0x13
#define  DEFAULT_RAM_SHORT_BRK_GAIN             0x04
#define  DEFAULT_RAM_SHORT_BRK_TIME             0x06
#define  DEFAULT_RAM_LONG_D2S_GAIN              0x15
#define  DEFAULT_RAM_LONG_BRK_GAIN              0x08
...
#define  HAPTIC_RAM_BRK_DISABLE_ID_2            28
...
static struct wakeup_source *lra_lock;

static unsigned int vib_notrigger_flag = 0;
	...
	unsigned char play_mode)
{
	...
	case AW869XX_HAPTIC_RAM_MODE:
		...
		if (aw869xx->index >= HAPTIC_RAM_BRK_DISABLE_ID_1)
			aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
				(AW869XX_BIT_PLAYCFG3_PLAY_MODE_MASK & AW869XX_BIT_PLAYCFG3_BRK_EN_MASK),
				(AW869XX_BIT_PLAYCFG3_PLAY_MODE_RAM | AW869XX_BIT_PLAYCFG3_BRK_DISABLE));
		else
			aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
				(AW869XX_BIT_PLAYCFG3_PLAY_MODE_MASK & AW869XX_BIT_PLAYCFG3_BRK_EN_MASK),
				(AW869XX_BIT_PLAYCFG3_PLAY_MODE_RAM | AW869XX_BIT_PLAYCFG3_BRK_ENABLE));
		break;
	...
	case AW869XX_HAPTIC_RTP_MODE:
		...
		aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
			(AW869XX_BIT_PLAYCFG3_PLAY_MODE_MASK & AW869XX_BIT_PLAYCFG3_BRK_EN_MASK),
			(AW869XX_BIT_PLAYCFG3_PLAY_MODE_RTP | AW869XX_BIT_PLAYCFG3_BRK_DISABLE));
		break;
	...
	case AW869XX_HAPTIC_CONT_MODE:
		...
		aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
			(AW869XX_BIT_PLAYCFG3_PLAY_MODE_MASK & AW869XX_BIT_PLAYCFG3_BRK_EN_MASK),
			(AW869XX_BIT_PLAYCFG3_PLAY_MODE_CONT | AW869XX_BIT_PLAYCFG3_BRK_ENABLE));
		break;
	default:
...
static void aw869xx_set_rtp_data(struct aw869xx *aw869xx, uint8_t *data,
	uint32_t len)
{
	pr_info("try write rtp data");
	aw869xx_i2c_writes(aw869xx, AW869XX_REG_RTPDATA, data, len);
	pr_info("try write rtp data end");
}
...
static int aw869xx_rtp_osc_calibration(struct aw869xx *aw869xx)
{
	...
	while (1) {
		if (!aw869xx_haptic_rtp_get_fifo_afs(aw869xx)) {
			...
			if (aw869xx->rtp_cnt != aw869xx_rtp->len) {
				if (aw869xx->timeval_flags == 1) {
					ktime_get_real_ts64(&aw869xx->start);
		...
		if (osc_int_state & AW869XX_BIT_SYSST2_FF_EMPTY) {
			ktime_get_real_ts64(&aw869xx->end);
...
static void aw869xx_fine_grained_adapt_amp_gain(struct aw869xx *aw869xx)
{
	if (aw869xx->amplitude > HAPTIC_GAIN_MAX_ADAPT_LEVELS)
		aw869xx->amplitude = HAPTIC_GAIN_MAX_ADAPT_LEVELS;

	if (aw869xx->amplitude < 0)
		return;

	aw869xx->gain = HAPTIC_MAX_GAIN_REG * aw869xx->amplitude / HAPTIC_GAIN_MAX_ADAPT_LEVELS;
...
static void aw869xx_adapt_amp_again(struct aw869xx *aw869xx, int haptic_type)
{
	...
	case SHORT_VIB_RAM_MODE:
		...
		pr_info("%s short gain = %u\n", __func__, aw869xx->gain);
		break;
	case RTP_VIB_MODE:
		aw869xx->gain = HAPTIC_MAX_GAIN_REG; // define max amp
		break;
	default:
		break;
	}
...
static void aw869xx_play_effect_id(struct aw869xx *aw869xx, uint64_t type)
{
	int len = 0;

	if (type > FINE_GRAINED_RAM_BASE) { // fine grained, amp is 0-99
		aw869xx->effect_mode = SHORT_VIB_RAM_MODE;
		aw869xx->amplitude = type % FINE_GRAINED_AMP_DIV_COFF;
		aw869xx->index = type / FINE_GRAINED_AMP_DIV_COFF - FINE_GRAINED_RAM_ID_BASE_VAL;
		pr_info("%s fine grained index = %d, amp = %d\n", __func__, aw869xx->index, aw869xx->amplitude);
		aw869xx->state = 1;
		aw869xx_fine_grained_adapt_amp_gain(aw869xx);
		schedule_work(&aw869xx->long_vibrate_work);
	} else if ((type > LONG_HAPTIC_RTP_MAX_ID) && (type < FINE_GRAINED_RAM_BASE)) { // long time
		...
		aw869xx->state = 1;
		__pm_wakeup_event(lra_lock, aw869xx->duration + HAPTIC_WAKE_LOCK_GAP);
	...
	size_t len, loff_t *off)
{
	...
	aw869xx_play_effect_id(aw869xx, type);
	mutex_unlock(&aw869xx->lock);
...
#ifdef AAC_RICHTAP_SUPPORT
static void richtap_clean_buf(struct aw869xx *aw_haptic, int status)
{
...
static void richtap_update_fifo_data(struct aw869xx *aw_haptic, uint32_t fifo_len)
{
	int32_t samples_left = 0;
	int pos = 0;
	int retry = 3;

	pr_info("richtap_update_fifo_data\n");
	do {
	...
	aw869xx_set_rtp_data(aw_haptic, aw_haptic->rtp_ptr, pos);
}

static bool richtap_rtp_start(struct aw869xx *aw869xx)
{
	int cnt = 200; // max retry cnt
	bool rtp_work_flag = false;
	uint8_t reg_val = 0;
	...
	while (cnt) {
		aw869xx_i2c_read(aw869xx, AW869XX_REG_GLBRD5, &reg_val);
		if ((reg_val & 0x0f) == 0x08) { // decide glb state in rtp or not
			cnt = 0;
			rtp_work_flag = true;
			pr_info("%s: RTP_GO! glb_state=0x08\n", __func__);
		} else {
...
static void aw869xx_irq_clear(struct aw869xx *aw869xx)
{
	uint8_t reg_val = 0;
...
{
	struct aw869xx *aw_haptic = container_of(work, struct aw869xx, richtap_rtp_work);
	uint32_t retry = 0;
	uint32_t tmp_len = 0;
	...
	} while (tmp_len < aw_haptic->ram.base_addr && retry++ < 30); // max retry 30 times

// 400K 频率写入
// 20K 频率播放
	pr_info("rtp tm_len = %d, retry = %d, aw_haptic->ram.base_addr = %d\n",
		tmp_len, retry, aw_haptic->ram.base_addr);
	if (richtap_rtp_start(aw_haptic)) {
		aw869xx_set_rtp_data(aw_haptic, aw_haptic->rtp_ptr, tmp_len);
		aw869xx_haptic_set_rtp_aei(aw_haptic, true);
		pr_info("try write clear");
		aw869xx_irq_clear(aw_haptic);
		atomic_set(&aw_haptic->richtap_rtp_mode, true);
		pr_info("set rtp mode true");
	}
}

static void aw869xx_richtap_config_stream_mode(struct aw869xx *aw869xx)
{
	richtap_clean_buf(aw869xx, MMAP_BUF_DATA_INVALID);
	mutex_lock(&aw869xx->lock);
	aw869xx_irq_clear(aw869xx);
	aw869xx_haptic_stop(aw869xx);
	mutex_unlock(&aw869xx->lock);
	aw869xx_haptic_set_rtp_aei(aw869xx, false);
	aw869xx_haptic_set_pwm(aw869xx, AW869XX_PWM_12K); // add by hujianglei
	atomic_set(&aw869xx->richtap_rtp_mode, false);
	aw869xx_haptic_upload_lra(aw869xx, OSC_CALI);
	aw869xx_haptic_set_bst_vol(aw869xx, aw869xx->vmax);
	pr_info("set vmax = %d\n", aw869xx->vmax);
	schedule_work(&aw869xx->richtap_rtp_work);
}

static void aw869xx_richtap_config_stop_mode(struct aw869xx *aw869xx)
{
	richtap_clean_buf(aw869xx, MMAP_BUF_DATA_FINISHED);
	mutex_lock(&aw869xx->lock);
	aw869xx_irq_clear(aw869xx);
	aw869xx_haptic_stop(aw869xx);
	mutex_unlock(&aw869xx->lock);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	if (aw869xx->dts_info.is_enabled_richtap_core != true) {
	...
	case RICHTAP_SETTING_GAIN:
		if (arg > 0x80)
			arg = 0x80;
		aw869xx_haptic_set_gain(aw869xx, (uint8_t)arg);
		break;
	case RICHTAP_STREAM_MODE:
		__pm_wakeup_event(lra_lock, HAPTIC_RTP_WAKE_TIME);
		aw869xx_richtap_config_stream_mode(aw869xx);
		break;
	case RICHTAP_STOP_MODE:
		aw869xx_richtap_config_stop_mode(aw869xx);
		break;
	default:
...
static uint8_t aw869xx_rtp_get_fifo_afi(struct aw869xx *aw869xx)
{
	uint8_t ret = 0;
	uint8_t reg_val = 0;
...
{
	unsigned long phys;
	struct aw869xx *aw_haptic = NULL;
	int ret = 0;

	if (!filp || !vma)
		return -EINVAL;
	aw_haptic = (struct aw869xx *)filp->private_data;
	if (!aw_haptic)
		return -EINVAL;
...
static int aw869xx_richtap_rtp_init(struct aw869xx *aw869xx)
{
	uint32_t i;
	struct mmap_buf_format *temp;

	aw869xx->rtp_ptr = kmalloc(RICHTAP_MMAP_BUF_SIZE * RICHTAP_MMAP_BUF_SUM,
		GFP_KERNEL);
	if (aw869xx->rtp_ptr == NULL) {
		pr_err("malloc rtp memory failed\n");
		return -EINVAL;
	}

	aw869xx->start_buf = (struct mmap_buf_format *)__get_free_pages(GFP_KERNEL,
		RICHTAP_MMAP_PAGE_ORDER);
	if (aw869xx->start_buf == NULL) {
		kfree(aw869xx->rtp_ptr);
		pr_err("Error __get_free_pages failed\n");
		return -EINVAL;
	}
	SetPageReserved(virt_to_page(aw869xx->start_buf));
	temp = aw869xx->start_buf;
	for (i = 1; i < RICHTAP_MMAP_BUF_SUM; i++) { // set 1 as start page
		temp->kernel_next = (aw869xx->start_buf + i);
		temp = temp->kernel_next;
	}
	temp->kernel_next = aw869xx->start_buf;
	INIT_WORK(&aw869xx->richtap_rtp_work, richtap_rtp_work);
	atomic_set(&aw869xx->richtap_rtp_mode, false);
	return 0;
}
#endif
...
static int aw869xx_vibrator_init(struct aw869xx *aw869xx)
{
	...
	if (vib_notrigger_flag) {
		pr_info("no led trigger\n ");
		aw869xx->vib_dev.default_trigger = "hw_vb_trigger";
	}
...
static irqreturn_t aw869xx_richtap_aei_irq_handle(struct aw869xx *aw869xx,
	int aei_empty)
{
	pr_info("go richtap_rtp work irq");
	if (aei_empty == 1) { // if rtp almost empty
		...
		// (aw_haptic->ram.base_addr - (aw_haptic->ram.base_addr >> 2)));
		richtap_update_fifo_data(aw869xx,
			(aw869xx->ram.base_addr >> 1)); // 1024
		while (!aw869xx_rtp_get_fifo_afi(aw869xx) &&
			atomic_read(&aw869xx->richtap_rtp_mode) &&
			(aw869xx->curr_buf->status == MMAP_BUF_DATA_VALID)) {
			pr_info("update rtp fifo data");
			richtap_update_fifo_data(aw869xx,
				(aw869xx->ram.base_addr >> 2)); // 512
		}
	}
	if (aw869xx->curr_buf->status == MMAP_BUF_DATA_INVALID) {
		aw869xx_haptic_set_rtp_aei(aw869xx, false);
		atomic_set(&aw869xx->richtap_rtp_mode, false);
	}
	atomic_set(&aw869xx->is_process_irq_work, false);
	return IRQ_HANDLED;
}
...
static irqreturn_t aw869xx_irq(int irq, void *data)
{
	...
	unsigned char rtp_empty = 0;

	if (!aw869xx)
		return IRQ_HANDLED;

	atomic_set(&aw869xx->is_process_irq_work, true);
	aw869xx_i2c_read(aw869xx, AW869XX_REG_SYSINT, &reg_val);
...
#ifdef AAC_RICHTAP_SUPPORT
	if (atomic_read(&aw869xx->richtap_rtp_mode) == true)
		return aw869xx_richtap_aei_irq_handle(aw869xx, rtp_empty);
#endif
	if (rtp_empty == 1)
	...
	struct device_node *np)
{
	...
	val = of_property_read_u32(np, "vib_no_trigger", &tmp);
	if (val != 0)
		pr_info("%s vib_no_trigger not found\n", __func__);
	else
		vib_notrigger_flag = tmp;
	...
	aw869xx->dts_info.is_enabled_richtap_core = of_property_read_bool(np,
		"vib_is_enabled_richtap_core");
	pr_info("%s is_enabled_richtap_core = %u\n", __func__,
		aw869xx->dts_info.is_enabled_richtap_core);
	aw869xx->dts_info.is_support_dts_brk_para = of_property_read_bool(np,
		"is_support_dts_brk_para");
	pr_info("%s is_support_dts_brk_para = %u\n", __func__,
		aw869xx->dts_info.is_support_dts_brk_para);
...
{
	pr_info("%s enter\n", __func__);
	if (aw869xx->dts_info.is_support_dts_brk_para) {
		aw869xx->rtp_d2s_gain = aw869xx->dts_info.d2s_gain;
		aw869xx->rtp_brk_gain = aw869xx->dts_info.cont_brk_gain;
		aw869xx->rtp_brk_time = aw869xx->dts_info.cont_brk_time_dt;
		aw869xx->ram_short_d2s_gain = aw869xx->dts_info.d2s_gain;
		aw869xx->ram_short_brk_gain = aw869xx->dts_info.cont_brk_gain;
		aw869xx->ram_short_brk_time = aw869xx->dts_info.cont_brk_time_dt;
		aw869xx->ram_long_d2s_gain = aw869xx->dts_info.d2s_gain;
		aw869xx->ram_long_brk_gain = aw869xx->dts_info.cont_brk_gain;
		aw869xx->ram_long_brk_time = aw869xx->dts_info.cont_brk_time_dt;
	} else {
	...
	const struct i2c_device_id *id)
{
	...
	lra_lock = wakeup_source_register(aw869xx->dev, "lra869xx");
#else
	lra_lock = wakeup_source_register("lra869xx");
#endif
	...
	atomic_set(&aw869xx->is_process_irq_work, false);
#ifdef AAC_RICHTAP_SUPPORT
	ret = aw869xx_richtap_rtp_init(aw869xx);
	if (ret < 0)
		goto richtap_err;
#endif
...
static int __maybe_unused aw869xx_suspend(struct device *dev)
{
	...
	pr_info("%s enter\n", __func__);
#ifdef AAC_RICHTAP_SUPPORT
	while (cnt && atomic_read(&aw869xx->is_process_irq_work)) {
		usleep_range(2000, 2500);
		cnt--;
	}
	pr_info("%s wait 2*%u suspend time enter\n", __func__, cnt);

	if (atomic_read(&aw869xx->richtap_rtp_mode)) {
		aw869xx_richtap_config_stop_mode(aw869xx);
