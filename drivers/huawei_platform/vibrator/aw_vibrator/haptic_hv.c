...
#define HAPTIC_HV_DRIVER_VERSION	"v0.5.1.0"

char *aw8692x_ram_name = "aw8692x_ram.bin";
...
// static struct pm_qos_request aw_pm_qos_req_vb;
struct aw_haptic_container *aw_rtp;
static unsigned int boot_power_set;
static struct aw_haptic *aw_haptic_file;
						...
						struct device_node *np)
{
	...
	val = of_property_read_u32(np, "boost_fw", &aw_haptic->info.boost_fw);
	if (val != 0) {
		aw_info("boost_fw not found");
	}
	// hardware enable when boost_fw is ture
	if (aw_haptic->info.boost_fw) {
		aw_info("boost_en gpio software control %d", aw_haptic->info.boost_en);
		aw_haptic->info.boost_en = of_get_named_gpio(np, "boost_en", 0);
		if (aw_haptic->info.boost_en >= 0) {
			aw_info("boost en-%d ok\n", aw_haptic->info.boost_en);
		} else {
			aw_err("get boost en gpio fail\n");
		}
		aw_info("boost_en gpio provide ok %d", aw_haptic->info.boost_en);
	} else {
		aw_info("boost_en gpio hardware control %d", aw_haptic->info.boost_en);
	}
	...
	if (val != 0)
		aw_info("boot_power_set not found");

	// boost enable or unable contrl
	val = of_property_read_u32(np, "bst_enable", &aw_haptic->bst_enable);
	if (val != 0)
		aw_info("bst_enable not found");
	aw_info("aw_haptic->bst_enable = %d", aw_haptic->bst_enable);

	// i2c addr update
	val = of_property_read_u32(np, "i2c_real_addr", &aw_haptic->real_i2c_addr);
	if (val != 0)
		aw_info("i2c_real_addr is not found");
	if (aw_haptic->real_i2c_addr != 0) {
		aw_info("i2c_real_addr is %d", aw_haptic->real_i2c_addr);
		aw_haptic->i2c->addr = (uint16_t) aw_haptic->real_i2c_addr;
	}
	return 0;
}
...
static int read_chipid(struct aw_haptic *aw_haptic, uint32_t *reg_val)
{
	...
	if (ret < 0) {
		aw_info ("new way fail");
		return ret;
	}
	*reg_val = value[0] << 8 | value[1];
	aw_info ("reg_val = 0x%02X", reg_val);
	return ret;
}
...
static void ram_vbat_comp(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		if (aw_haptic->ram_vbat_comp == AW_RAM_VBAT_COMP_ENABLE) {
			...
			aw_info("ram vbat comp close(AW_RAM_VBAT_COMP_ENABLE failed)");
		}
	} else {
		aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
		aw_info("ram vbat comp close(flag false)");
	}
}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	aw_haptic = (struct aw_haptic *)file->private_data;

	if (!aw_haptic)
		return 0;
	aw_info("cmd=0x%x, arg=0x%lx\n", cmd, arg);
...
static void aw_adapt_amp_again(struct aw_haptic *aw_haptic, int haptic_type)
{
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	...
	case LONG_VIB_RAM_MODE:
		if (aw_haptic->amplitude >= AW_LONG_MAX_AMP_CFG || aw_haptic->amplitude < 0)
			return;
		aw_haptic->gain = long_amp_again_val[aw_haptic->amplitude];
		aw_info("long gain = %u\n", aw_haptic->gain);
		break;
	case SHORT_VIB_RAM_MODE:
		if (aw_haptic->amplitude >= AW_SHORT_MAX_AMP_CFG || aw_haptic->amplitude < 0)
			return;
		aw_haptic->gain = short_amp_again_val[aw_haptic->amplitude];
		aw_info("short gain = %u\n", aw_haptic->gain);
		break;
	case RTP_VIB_MODE:
		aw_haptic->gain = 0x80; // define max amp
		break;
	default:
		break;
	}
...
static void aw_special_type_process(struct aw_haptic *aw_haptic,
	uint64_t type)
{
	switch (type) {
	case HAPTIC_CHARGING_CALIB_ID:
		aw_haptic->f0_is_cali = HAPTIC_CHARGING_IS_CALIB;
		schedule_work(&aw_haptic->vibrator_work);
		break;
	case HAPTIC_NULL_WAVEFORM_ID:
		aw_info("null wave id");
		break;
	default:
		break;
	}
	...
	const char *buff, size_t len, loff_t *off)
{
	...
	aw_haptic = (struct aw_haptic *)filp->private_data;
	if (!aw_haptic)
		return len;
	mutex_lock(&aw_haptic->lock);
	if (copy_from_user(write_buf, buff, len)) {
		aw_err("[haptics_write] copy_from_user failed\n");
		return len;
	}
	if (kstrtoull(write_buf, 10, &type)) {
		aw_err("[haptics_write] read value error\n");
		return len;
	}
	aw_info("get haptic id = %llu\n", type);

	aw_haptic->emergency_mode_flag = false;
	aw_haptic->cust_boost_on = AW_SEL_BOOST_CFG_ON;
	if ((type < HAPTIC_MAX_SPEC_CMD_ID) && (type > HAPTIC_MIN_SPEC_CMD_ID)) {
		aw_special_type_process(aw_haptic, type);
		mutex_unlock(&aw_haptic->lock);
		return len;
	}

	if (type > LONG_HAPTIC_RTP_MAX_ID) { // long time
		aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
		aw_haptic->index = LONG_VIB_EFFECT_ID;
		aw_haptic->duration = type / LONG_TIME_AMP_DIV_COFF;
		aw_haptic->amplitude = type % LONG_TIME_AMP_DIV_COFF;
		aw_haptic->state = 1;
		aw_adapt_amp_again(aw_haptic, LONG_VIB_RAM_MODE);
		__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
		aw_info("long index = %d, amp = %d\n", aw_haptic->index, aw_haptic->amplitude);
		schedule_work(&aw_haptic->vibrator_work);
	} else if ((type > 0) && (type <= SHORT_HAPTIC_RAM_MAX_ID)) { // short ram haptic
		aw_haptic->effect_mode = SHORT_VIB_RAM_MODE;
		aw_haptic->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw_adapt_amp_again(aw_haptic, SHORT_VIB_RAM_MODE);
		aw_info("short index = %d, amp = %d\n", aw_haptic->index, aw_haptic->amplitude);
		aw_haptic->state = 1;
		schedule_work(&aw_haptic->vibrator_work);
	} else { // long and short rtp haptic
		aw_haptic->effect_mode = RTP_VIB_MODE;
		aw_haptic->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->func->play_stop(aw_haptic);
		aw_haptic->func->set_rtp_aei(aw_haptic, false);
		aw_haptic->func->irq_clear(aw_haptic);
		aw_adapt_amp_again(aw_haptic, RTP_VIB_MODE);
		aw_haptic->state = 1;
		aw_haptic->rtp_idx = aw_haptic->index - BASE_INDEX;
		rtp_len += snprintf(aw_rtp_idx_name, AW_RTP_NAME_MAX - 1,
							"aw_%d.bin", aw_haptic->rtp_idx);
		aw_info(" get rtp name = %s, index = %d, len = %d\n",
			aw_rtp_idx_name, aw_haptic->rtp_idx, rtp_len);
	...
	do {
		if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if (samples_left < fifo_len) {
				memcpy(&aw_haptic->rtp_ptr[pos],
					&aw_haptic->curr_buf->data[aw_haptic->pos],
					samples_left);
				pos += samples_left;
...
static void richtap_rtp_work(struct work_struct *work)
{
	...
	do {
		if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID) {
			if ((tmp_len + aw_haptic->curr_buf->length) >
				aw_haptic->ram.base_addr) {
				memcpy(&aw_haptic->rtp_ptr[tmp_len],
					aw_haptic->curr_buf->data,
					(aw_haptic->ram.base_addr - tmp_len));
				aw_haptic->pos = aw_haptic->ram.base_addr - tmp_len;
				tmp_len = aw_haptic->ram.base_addr;
			} else {
				memcpy(&aw_haptic->rtp_ptr[tmp_len],
					aw_haptic->curr_buf->data,
					aw_haptic->curr_buf->length);
	...
	} while (tmp_len < aw_haptic->ram.base_addr && retry++ < 30); // try cont
...
static void richtap_config_stream_mode(struct aw_haptic *aw_haptic)
{
...
static void richtap_config_stop_mode(struct aw_haptic *aw_haptic)
{
	...
	unsigned int cmd, unsigned long arg)
{
	...
	if (aw_haptic->info.is_enabled_richtap_core != true) {
		pr_info("do not support richtap core feature");
		return -EFAULT;
	}
	switch (cmd) {
	...
	case RICHTAP_STREAM_MODE:
		__pm_wakeup_event(aw_haptic->ws, HAPTIC_RTP_WAKE_TIME);
		richtap_config_stream_mode(aw_haptic);
		break;
	case RICHTAP_STOP_MODE:
		richtap_config_stop_mode(aw_haptic);
		break;
	default:
		aw_info("%s, unknown cmd\n", __func__);
		break;
	}
	return ret;
}
...
static int richtap_rtp_config_init(struct aw_haptic *aw_haptic)
{
	uint32_t i;
	struct mmap_buf_format *temp;

	aw_haptic->rtp_ptr = kmalloc(RICHTAP_MMAP_BUF_SIZE * RICHTAP_MMAP_BUF_SUM,
		GFP_KERNEL);
	if (aw_haptic->rtp_ptr == NULL) {
		aw_err("malloc rtp memory failed\n");
		return -EINVAL;
	}

	aw_haptic->start_buf = (struct mmap_buf_format *)__get_free_pages(GFP_KERNEL,
...
static enum hrtimer_restart vibrator_timer_func(struct hrtimer *timer)
{
	...
	aw_haptic = container_of(timer, struct aw_haptic, timer);
	aw_haptic->state = 0;
	schedule_work(&aw_haptic->vibrator_work);
...
static void aw_long_ram_mode_boost_config(struct aw_haptic *aw_haptic,
	unsigned char wavseq)
{
	if (aw_haptic->cust_boost_on == AW_SEL_BOOST_CFG_ON) {
		aw_haptic->func->set_wav_seq(aw_haptic, 0, 0x14);
		aw_haptic->func->set_wav_loop(aw_haptic, 0, 0); // loop mode
		aw_haptic->func->set_wav_seq(aw_haptic, 1, wavseq);
		aw_haptic->func->set_wav_loop(aw_haptic, 1, 0x0f);
		aw_haptic->func->set_wav_seq(aw_haptic, 2, 0);
		aw_haptic->func->set_wav_loop(aw_haptic, 2, 0);
	} else {
		aw_haptic->func->set_wav_seq(aw_haptic, 0, wavseq);
...
static int aw_haptic_ram_config(struct aw_haptic *aw_haptic)
{
	...
	aw_info("duration=%d,index = %d\n", aw_haptic->duration, aw_haptic->index);
	for (seq_idx = 0; seq_idx < 8; seq_idx++) {
		aw_haptic->func->set_wav_seq(aw_haptic, seq_idx, 0);
		aw_haptic->func->set_wav_loop(aw_haptic, seq_idx, 0);
	}
	if (aw_haptic->effect_mode == SHORT_VIB_RAM_MODE) {
		wavseq = aw_haptic->index;
		wavloop = 0;
		aw_haptic->func->set_wav_seq(aw_haptic, 0, wavseq);
		aw_haptic->func->set_wav_loop(aw_haptic, 0, wavloop);
		aw_haptic->func->set_wav_seq(aw_haptic, 1, 0);
		...
		wavseq = aw_haptic->index;
		aw_long_ram_mode_boost_config(aw_haptic, wavseq);
	}
	return 0;
}

static int aw_haptic_play_repeat_seq(struct aw_haptic *aw_haptic,
									unsigned char flag)
{
	aw_info("enter\n");
	...
	if (flag) {
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_LOOP_MODE);
		aw_haptic->func->play_go(aw_haptic, true);
	}
	return 0;
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw_haptic_emergency_mode(struct aw_haptic *aw_haptic)
{
	int ret;
	int ele = 0;

	ret = power_supply_get_int_property_value("Battery",
		POWER_SUPPLY_PROP_CAPACITY, &ele);
	if (ret < 0) {
		aw_err("%s: get capacity failed", __func__);
		return;
	}
	if (ele <= 1) {
		aw_info("%s: enter!", __func__);
		aw_haptic->emergency_mode_flag = true;
	}
}
#endif

static void aw_boost_config(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic->info.is_enabled_lowpower_bst_config) {
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
		return;
	}
	if (aw_haptic->emergency_mode_flag)
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BYPASS_MODE);
	else
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
}

static void ram_mode_sel_boost_play_cfg(struct aw_haptic *aw_haptic)
{
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw_haptic->info.is_enabled_lowpower_bst_config)
		aw_haptic_emergency_mode(aw_haptic);
#endif

	if (aw_haptic->cust_boost_on == AW_SEL_BOOST_CFG_ON) {
		ram_vbat_comp(aw_haptic, false);
		aw_boost_config(aw_haptic);
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
		aw_haptic->func->play_go(aw_haptic, true);
		return;
	}
	if (aw_haptic->effect_mode == LONG_VIB_RAM_MODE) {
		ram_vbat_comp(aw_haptic, true);
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BYPASS_MODE);
		aw_haptic_play_repeat_seq(aw_haptic, true);
	} else {
		ram_vbat_comp(aw_haptic, false);
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
...
static void vibrator_work_routine(struct work_struct *work)
{
	...
	aw_haptic = container_of(work, struct aw_haptic, vibrator_work);

	/* charging mode */
	mutex_lock(&aw_haptic->lock);
	if (aw_haptic->f0_is_cali == HAPTIC_CHARGING_IS_CALIB) {
		aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
		f0_cali(aw_haptic);
		aw_haptic->f0_is_cali = 0;
		mutex_unlock(&aw_haptic->lock);
		return;
	}
	...
	if (aw_haptic->state) {
		aw_info("state is true");
#ifdef AW_DURATION_DECIDE_WAVEFORM
		ram_select_waveform(aw_haptic);
#endif
		if (aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_RAM_MODE) {
			aw_info("activate_mode == AW_HAPTIC_ACTIVATE_RAM_MODE");
			aw_haptic_ram_config(aw_haptic);
			if (aw_haptic->duration <= AW_LONG_HAPTIC_RUNNING) { // max long vibra config
				aw_info("duration <= AW_LONG_HAPTIC_RUNNINGe");
				ram_mode_sel_boost_play_cfg(aw_haptic);
			} else {
				ram_vbat_comp(aw_haptic, true);
				aw_haptic_play_repeat_seq(aw_haptic, true);
			}
		} else if (aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE) {
			aw_info("activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE");
			aw_haptic->func->cont_config(aw_haptic);
		} else {
			/* other mode */
			aw_err("set other mode\n");
		}
		/* run ms timer */
		if (aw_haptic->effect_mode == LONG_VIB_RAM_MODE ||
			(aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE)) {
			aw_info("run ms timer");
			hrtimer_start(&aw_haptic->timer, ktime_set(aw_haptic->duration / 1000,
				(aw_haptic->duration % 1000) * 1000000), HRTIMER_MODE_REL);
			__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
			aw_info("%s gain = 0x%02X\n",  __func__, aw_haptic->gain);
#ifdef CONFIG_HUAWEI_DUBAI
			HWDUBAI_LOGE("DUBAI_TAG_HAPTIC_DURATION", "duration=%d index=%d",
				aw_haptic->duration, aw_haptic->index);
#endif
		}
	} else {
		aw_info("state is flase");
	}
	mutex_unlock(&aw_haptic->lock);
}
...
static void rtp_play(struct aw_haptic *aw_haptic)
{
	...
	while ((!aw_haptic->func->rtp_get_fifo_afs(aw_haptic))
		...
		while ((aw_haptic->rtp_cnt < aw_rtp->len)) {
			if (real_len > (aw_rtp->len - aw_haptic->rtp_cnt))
				real_len = aw_rtp->len - aw_haptic->rtp_cnt;
			aw_haptic->func->set_rtp_data(aw_haptic,
					&aw_rtp->data[aw_haptic->rtp_cnt],
					real_len);
			aw_haptic->rtp_cnt += real_len;
		}
...
static void rtp_work_routine(struct work_struct *work)
{
	...
	/* fw loaded */
	ret = request_firmware(&rtp_file, aw_rtp_idx_name, aw_haptic->dev);
	if (ret < 0) {
		aw_err("failed to read %s", aw_rtp_idx_name);
	...
	aw_rtp->len = rtp_file->size;
	aw_info("rtp file:[%s] size = %dbytes", aw_rtp_idx_name, aw_rtp->len);
	memcpy(aw_rtp->data, rtp_file->data, rtp_file->size);
	mutex_unlock(&aw_haptic->rtp_lock);
	release_firmware(rtp_file);
	mutex_lock(&aw_haptic->lock);
...
static irqreturn_t irq_richtap_aei_handle(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic->func->get_irq_state(aw_haptic)) {
		pr_info("%s: aw_haptic rtp fifo almost empty\n", __func__);
		/* modify next line second parameter according to ram.base_addr */
		richtap_update_fifo_data(aw_haptic,
			(aw_haptic->ram.base_addr >> 1));
		while (!aw_haptic->func->rtp_get_fifo_afi(aw_haptic) &&
			atomic_read(&aw_haptic->richtap_rtp_mode) &&
			(aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID))
			richtap_update_fifo_data(aw_haptic,
				(aw_haptic->ram.base_addr >> 2));
	}
	...
	atomic_set(&aw_haptic->is_process_irq_work, false);
	return IRQ_HANDLED;
}
...
static irqreturn_t irq_handle(int irq, void *data)
{
	...
	aw_info("irq process work");
	atomic_set(&aw_haptic->is_process_irq_work, true);
#ifdef AAC_RICHTAP_SUPPORT
	if (atomic_read(&aw_haptic->richtap_rtp_mode) == true)
		return irq_richtap_aei_handle(aw_haptic);
#endif
...
static int irq_config(struct device *dev, struct aw_haptic *aw_haptic)
{
	...
	if (!dev || !aw_haptic) {
		aw_err ("dev or aw_haptic is null");
		return -EINVAL;
	}
...
static void ram_load(const struct firmware *cont, void *context)
{
...
#endif
	if (!context) {
		aw_err ("context is null");
		return;
	}
	aw_haptic = context;
	if (!cont) {
		aw_err("failed to read %s", aw8692x_ram_name);
	...
	aw_info("loaded %s - size: %zu", aw8692x_ram_name, cont ? cont->size : 0);
	/* check sum */
	...
	aw_info("kzalloc success");
	aw_fw->len = cont->size;
	aw_info("ready to memcpy");
	memcpy(aw_fw->data, cont->data, cont->size);
	aw_info("ready to release_firmware");
	release_firmware(cont);
	aw_info("ready to container_update");
...
static int ram_update(struct aw_haptic *aw_haptic)
{
	...
	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
							aw8692x_ram_name, aw_haptic->dev, GFP_KERNEL,
							aw_haptic, ram_load);
	if (ret < 0) {
		aw_info ("request_firmware_nowait failed, ret = %d\n", ret);
		return ret;
	}
	return ret;
}
...
static void ram_work_routine(struct work_struct *work)
{
	...
	aw_haptic = container_of(work, struct aw_haptic, ram_work.work);
							...
							struct aw_haptic_ctr *haptic_ctr)
{
	...
	if (!haptic_audio || !haptic_ctr) {
		aw_err ("haptic_audio or haptic_ctr is null");
		return -EINVAL;
	}
...
static enum hrtimer_restart audio_timer_func(struct hrtimer *timer)
{
	...
	if (!timer) {
		aw_err ("timer is null");
		return 0;
	}
	aw_haptic = container_of(timer, struct aw_haptic, haptic_audio.timer);
...
static void audio_work_routine(struct work_struct *work)
{
	...
	aw_haptic = container_of(work, struct aw_haptic, haptic_audio.work);
...
static int vibrator_get_time(struct timed_output_dev *dev)
{
	...
	aw_haptic = container_of(dev, struct aw_haptic, vib_dev);
							...
							const char *buf, size_t count)
{
	...
	aw_haptic->state = val;
	aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
	aw_haptic->index = LONG_VIB_BOOST_EFFECT_ID;
	mutex_unlock(&aw_haptic->lock);
	schedule_work(&aw_haptic->vibrator_work);
...
#ifdef AW8692X_DRIVER_ENABLE
		if (aw_haptic->func == aw8692x_func() &&
							...
							const char *buf, size_t count)
{
	...
	len += snprintf(aw_rtp_idx_name, AW_RTP_NAME_MAX - 1, "aw_%u.bin", val);
	aw_info(" get rtp name = %s, len = %d\n", aw_rtp_idx_name, len);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (aw_haptic->f0_val_err < 0)
		aw_haptic->f0 = 0;
	pr_info("%s cal f0 val = %u\n", __func__, aw_haptic->f0);

#ifdef F0_CONT_F0
	len += snprintf(buf + len, PAGE_SIZE - len, "aw_haptic f0 = %d\n", aw_haptic->f0);
#endif
	len += snprintf(buf + len, PAGE_SIZE - len, "%d", aw_haptic->f0);
	return len;
}
						...
						char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw_haptic f0 = %d\n", aw_haptic->f0);
#endif
	if (aw_haptic->f0 > AW_F0_CALI_THER_H ||
		aw_haptic->f0 < AW_F0_CALI_THER_L)
		cal_rst = 0;
	else
		cal_rst = 1;
...
static int vibrator_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->vib_dev.default_trigger = "hw_vb_trigger";
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (aw_haptic->info.boost_fw) {
		if (gpio_is_valid(aw_haptic->info.boost_en)) {
			ret = devm_gpio_request_one(&i2c->dev, aw_haptic->info.boost_en,
									GPIOF_OUT_INIT_HIGH, "aw869xx_boost_en");
			if (ret) {
				aw_err("boost en gpio request failed");
				goto err_boost_en_request;
			}
			gpio_set_value_cansleep(aw_haptic->info.boost_en, 1);
	...
	aw_haptic->ws = wakeup_source_register(aw_haptic->dev, "aw_haptic");
	if (!aw_haptic->ws) {
		aw_err("failed to wakeup_source_register: %d\n", ret);
	...
	atomic_set(&aw_haptic->is_process_irq_work, false);
#ifdef AAC_RICHTAP_SUPPORT
	ret = richtap_rtp_config_init(aw_haptic);
	if (ret < 0)
		goto richtap_err;
#endif
	aw_haptic_file = aw_haptic;
...
err_ctrl_init:
	wakeup_source_unregister(aw_haptic->ws);
err_irq_config:
	if (gpio_is_valid(aw_haptic->irq_gpio))
...
err_boost_en_request:
	if (gpio_is_valid(aw_haptic->info.boost_en))
		devm_gpio_free(&i2c->dev, aw_haptic->info.boost_en);
...
static int aw_i2c_suspend(struct device *dev)
{
	...
	aw_info("enter");
#ifdef AAC_RICHTAP_SUPPORT
	while (cnt && atomic_read(&aw_haptic->is_process_irq_work)) {
		usleep_range(2000, 2500);
		cnt--;
	}
	pr_info("%s wait 2*%u suspend time enter\n", __func__, cnt);
	if (atomic_read(&aw_haptic->richtap_rtp_mode)) {
		richtap_config_stop_mode(aw_haptic);
		pr_info("%s stop richtap rtp mode", __func__);
		return 0;
	}
#endif
