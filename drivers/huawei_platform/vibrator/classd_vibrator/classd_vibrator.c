...
#define EFFECTS_VIBRATION_TIME              40
#define DELAY_FOR_ROUTE_APPLY               40000
#define DELTA_USLEEP_RANGE                  1000
#define MS_US_RATION                        1000
#define MAX_INTERVAL_FOR_CONTINUOUS_VIBRATE_MS 2800
...
struct vibrator_time_record {
	bool use_vib_delay;
	bool is_first_vib_on;
	long long last_vib_time;
};

struct vibrator_time_record g_vibrator_time_record = {
	.use_vib_delay = false,
	.is_first_vib_on = false,
	.last_vib_time = 0,
};

static void classd_vibrator_set_cali_data(void);
static void classd_vibrator_duration_notify(struct led_classdev *cdev, enum led_brightness state);
static unsigned long g_vib_duration;
...
void init_trans_buffer(uint32_t addr)
{
	if (g_trans_buffer == NULL) {
		g_trans_buffer = ioremap_wc((phys_addr_t)addr, (size_t)HISI_AP_AUDIO_VIBRATOR_DATA_SIZE);
		hwlog_info("%s: end:buffer:%p addr:0x%x", __func__, g_trans_buffer, addr);
	}
}
...
static void hisi_pmic_modify(uint32_t address, uint8_t mask, uint8_t value)
{
	...
	temp = (uint8_t)pmic_read_reg(address);

	temp &= (~mask);
	temp |= value;

	pmic_write_reg(address, temp);
}
...
static void vibrator_on(struct classd_vibrator_data *priv)
{
	...
	if (priv->vib_type == VIB_CLASS_D_TYPE) {
		pmic_write_reg(priv->classd_reg_address, priv->start_vib_value);
...
static void vibrator_power_off(struct classd_vibrator_data *priv)
{
	if (priv->vib_type == VIB_DSP_PCM_TYPE) {
		...
		pmic_write_reg(priv->classd_reg_address, priv->stop_vib_value);
	}
...
static void vibrator_off(struct classd_vibrator_data *priv)
{
	...
	hwlog_info("%s: enter!!", __func__);
	if (priv->vib_type == VIB_CLASS_D_TYPE)
		pmic_write_reg(priv->classd_reg_address, priv->reverse_vib_value);
...
/* this function should only be used in work */
static void set_first_vib_on_delay()
{
	if (g_vibrator_time_record.is_first_vib_on && g_vibrator_time_record.use_vib_delay) {
		hwlog_info("%s: enter sleep for vib on", __func__);
		usleep_range(DELAY_FOR_ROUTE_APPLY, DELAY_FOR_ROUTE_APPLY + DELTA_USLEEP_RANGE);
	}
}

static void update_first_vib_duration(struct led_classdev *cdev)
{
	struct transient_trig_data *transient_data = led_trigger_get_drvdata(cdev->dev);

	if (transient_data == NULL) {
		hwlog_err("%s: invalid data", __func__);
		return;
	}
	g_vib_duration = transient_data->duration * MS_US_RATION;
	hwlog_info("%s: update duration = %lu us", __func__, g_vib_duration);
}

static void set_first_vib_off_delay()
{
	if (g_vibrator_time_record.is_first_vib_on && g_vibrator_time_record.use_vib_delay) {
		unsigned long sleep_time = g_vib_duration < DELAY_FOR_ROUTE_APPLY ? g_vib_duration : DELAY_FOR_ROUTE_APPLY;
		hwlog_info("%s: enter sleep for vib off time = %lu", __func__, sleep_time);
		usleep_range(sleep_time, sleep_time + DELTA_USLEEP_RANGE);
	}
}

static void update_first_vib_status()
{
	g_vibrator_time_record.is_first_vib_on = ktime_to_ms(ktime_sub(ktime_get(), g_vibrator_time_record.last_vib_time)) >
		MAX_INTERVAL_FOR_CONTINUOUS_VIBRATE_MS;
	g_vibrator_time_record.last_vib_time = ktime_get();
}
...
static void vibrator_enable_work(struct work_struct *work)
{
	...
	if (state) {
		update_first_vib_status();
		update_first_vib_duration(&priv->cdev);
		classd_vibrator_duration_notify(&priv->cdev, state);
		set_first_vib_on_delay();
		vibrator_on(priv);
	} else {
		set_first_vib_off_delay();
		classd_vibrator_duration_notify(&priv->cdev, state);
		vibrator_off(priv);
	}
...
static void classd_vibrator_duration_notify(struct led_classdev *cdev, enum led_brightness state)
{
	...
	if (state)
		vibrator_duration_distinguish(transient_data->duration);
	else
...
static int classd_vibrator_parse_dts_param(struct classd_vibrator_data *priv)
{
	...
	ret += of_property_read_u32(dn, "vib_type", &priv->vib_type);
	ret += of_property_read_u32(dn, "reg_address", &priv->classd_reg_address);
	ret += of_property_read_u32(dn, "start_vib_value", &priv->start_vib_value);
	ret += of_property_read_u32(dn, "stop_vib_value", &priv->stop_vib_value);
	if (ret < 0) {
		hwlog_err("%s read reg fail, ret = %d", __func__, ret);
		return -1;
	}
	hwlog_info("%s type is %#x, reg is %#x, val is %#x, %#x", __func__, priv->vib_type, priv->classd_reg_address,
		priv->start_vib_value, priv->stop_vib_value);

	ret = of_property_read_u32(dn, "vibrator_mode", &priv->vibrator_mode);
	if (ret != 0) {
		priv->vibrator_mode = MODE_LDO;
		hwlog_info("%s read vibrator mode fail, may not use, ret = %d", __func__, ret);
	}

	ret = of_property_read_u32(dn, "reverse_vib_value", &priv->reverse_vib_value);
	if (ret < 0) {
		hwlog_info("%s, read reverse_vib_value fail, not support", __func__);
		priv->support_quick_stop = 0; // not support quick stop
	} else {
		priv->support_quick_stop = 1; // support quick stop
	}
	hwlog_info("%s reverse_vib_value is 0x%x, support_quick_stop is %u", __func__, priv->reverse_vib_value,
		priv->support_quick_stop);

	ret = of_property_read_u32(dn, "stop_delay_ms", &stop_delay_time_ms);
	if (ret < 0) {
		hwlog_info("%s, read stop_delay_ms fail, use default", __func__);
		stop_delay_time_ms = DEFAULT_STOP_DELAY_MS;
	}
	ret = of_property_read_u32(dn, "min_vib_timeout", &min_vib_time_ms);
	if (ret < 0) {
		hwlog_info("%s, read min_vib_timeout fail, use default", __func__);
		min_vib_time_ms = DEFAULT_MIN_VIB_TIMEOUT;
	}
	hwlog_info("%s stop_delay_ms = %u, min_vib_timeout is %u", __func__, stop_delay_time_ms, min_vib_time_ms);
	ret = of_property_read_u32(dn, "audio_addr", &priv->audio_addr);
	if (ret < 0) {
		hwlog_info("%s, read audio_addr fail, use default", __func__);
		priv->audio_addr = HISI_AP_AUDIO_VIBRATOR_ADDR;
	}
	hwlog_info("%s audio_addr = 0x%x", __func__, priv->audio_addr);
...
static void play_effect(struct work_struct *work)
{
	...
	classd_open();
	classd_vibrator_set_cali_data();
	vibrator_duration_distinguish(EFFECTS_VIBRATION_TIME);
	update_first_vib_status();
	set_first_vib_on_delay();
...
static int classd_vibrator_probe(struct platform_device *pdev)
{
	...
	if (priv->vib_type == VIB_DSP_PCM_TYPE) {
		...
		init_trans_buffer(priv->audio_addr);
		if (g_trans_buffer == NULL) {
	...
	vibrator_nb_init();
	g_vibrator_time_record.use_vib_delay =
		(priv->vibrator_mode == MODE_VSYS_FLAT || priv->vibrator_mode == MODE_VSYS_LINE || priv->vibrator_mode == MODE_VSYS_XVIB);
	set_long_effect(priv->vibrator_mode == MODE_VSYS_FLAT);
