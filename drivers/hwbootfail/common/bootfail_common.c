	...
	if (is_dir_existed("/data/lost+found")) {
		...
		if (ret == 0) {
			pparam->space_left = (unsigned int)((
				...
				(unsigned long long)statbuf.f_bsize) / BF_SIZE_1M); /* Unit: MB */
...
{
	return (cmd == SET_SHUT_STAGE || cmd == GET_SHUT_STAGE);
}

static bool is_cmd_of_shut_detector(unsigned int cmd)
{
	return (cmd == PROCESS_SHUTFAIL || cmd == GET_SFI_DEV_PATH ||
		cmd == SET_REBOOT || cmd == GET_SFI_DEV_OFFSET);
}

static void process_shut_start(enum shut_stage stage)
{
	if (stage != SHUT_STAGE_FRAMEWORK_START)
		return;

	print_info("start the shut timer now\n");
	start_boot_timer();
}

static int __process_upper_shutfail(void *pparam)
{
	/* sync logcat to storage */
	bf_sys_sync();
	msleep(2000);
	bf_sys_sync();
	shut_fail_error((struct shutfail_proc_param*)pparam);
	return 0;
}

static int process_upper_shutfail(unsigned long arg)
{
	struct task_struct *tsk = NULL;
	struct shutfail_proc_param param;
	memset_s((void *)&param, sizeof(param), 0, sizeof(param));
	if (copy_from_user(&param, ((struct shutfail_proc_param *)(uintptr_t)arg),
		sizeof(param)) != 0) {
		print_err("copy_from_user failed!\n");
		return -EFAULT;
	}
	param.sinfo.rtc_time = get_sys_rtc_time();
	(void)get_shut_stage((enum shut_stage *)&param.sinfo.stage);
	print_info("shutfail: shut_time: %ds\n", param.sinfo.rtc_time);
	tsk = kthread_run(__process_upper_shutfail,
			(void*)&param, "proc upper shutfail");
	if (IS_ERR(tsk)) {
		print_err("kthread_run failed!\n");
		return -1;
	}
...
static long ioctl_shut_detector(struct file *file,
	unsigned int cmd,
	unsigned long arg)
{
	long ret = 0;
	...
	int offset;
	switch (cmd) {
	case PROCESS_SHUTFAIL:
		ret = process_upper_shutfail(arg);
		break;
	case GET_SFI_DEV_PATH:
		if (get_sfi_dev_path() == NULL)
			return -ENODEV;
		/*lint -e666*/
		if (copy_to_user(path->path, get_sfi_dev_path(),
			min(strlen(get_sfi_dev_path()),
			sizeof(path->path) - 1)) != 0) {
			print_err("Failed to copy bfi dev path to user!\n");
			return -EFAULT;
		}
		/*lint +e666*/
		break;
	case GET_SFI_DEV_OFFSET:
		offset = get_sfi_dev_offset();
		if (copy_to_user((int *)(uintptr_t)arg,
	...
	case SET_REBOOT:
		if (copy_from_user(&is_reboot, (int *)(uintptr_t)arg,
			sizeof(is_reboot)) != 0) {
			print_err("copy_from_user failed!\n");
			ret = -EFAULT;
			break;
		}
		if (is_reboot != 0 && is_reboot != 1)
			print_err("error argument!\n");
		print_info("begin to SET_REBOOT. is_reboot: %d\n", is_reboot);
		set_machine_status(is_reboot);
		break;
	}
	return ret;
}
...
{
	long ret = 0;
	enum shut_stage stage;
	enum bootfail_errorcode error_code;

	switch (cmd) {
	case GET_SHUT_STAGE:
		error_code = get_shut_stage(&stage);
		if (error_code != BF_OK) {
			print_err("get_shut_stage ret: %d!\n", error_code);
			ret = -EFAULT;
			break;
		}
		if (copy_to_user((enum shut_stage *)(uintptr_t)arg,
			&stage, sizeof(stage)) != 0) {
			print_err("copy_to_user failed!\n");
			ret = -EFAULT;
			break;
	...
	case SET_SHUT_STAGE:
		...
		process_shut_start(stage);
		error_code = set_shut_stage(stage);
		if (error_code != BF_OK) {
	...
	unsigned long arg)
{
	...
	else if (is_cmd_of_shut_detector(cmd))
		ret = ioctl_shut_detector(file, cmd, arg);
#endif
