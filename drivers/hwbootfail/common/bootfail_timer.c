...
#define SHUT_TIMER_COUNT_STEP 400
#endif
...
#ifdef CONFIG_SHUT_DETECTOR
	unsigned int shut_count_step_value;
	u32 shut_timer_timeout_value;
#endif
};
...
#else
static void wakeup_boot_timer_count_task(struct timer_list *bf_timer);
#endif
...
#ifdef CONFIG_SHUT_DETECTOR
static void process_shut_timer_timeout(void)
{
	struct shutfail_proc_param *pparam = NULL;
	enum bootfail_errorcode errorcode;
	print_info("shut timeout!");
	...
	(void)get_shut_stage((enum shut_stage *)&pparam->sinfo.stage);
	print_info("shut stage is %d\n", pparam->sinfo.stage);
	pparam->sinfo.rtc_time = get_sys_rtc_time();
	print_info("rtc_time is %x", pparam->sinfo.rtc_time);
	errorcode = shut_fail_error(pparam);
	if (errorcode != BF_OK)
		print_err("shut_fail_error fail:%d!\n", errorcode);
	vfree(pparam);
}

static void check_shut_time(struct boot_timer_info *ptimer_info)
{
	static u32 shut_timer_timing_count;
	u32 shut_value = ptimer_info->shut_timer_timeout_value;

	shut_timer_timing_count += ptimer_info->shut_count_step_value;
	print_info("hwshut_timer:update shut timer. shut_value: %d, shut_count: %d\n",
		shut_value, shut_timer_timing_count);

	if (shut_timer_timing_count >= shut_value) {
		print_info("hwshut_timer: %u ms timer expired! shut fail!\n", shut_value);
		ptimer_info->boot_timer_count_task_should_run = false;
		mutex_unlock(&boot_timer_mutex);
		process_shut_timer_timeout();
	}
}
#endif
...
static void wakeup_boot_timer_count_task(struct timer_list *bf_timer)
{
	...
	if (bf_timer == NULL) {
		print_err("please attention bf_timer err\n");
		return;
	}
	ptimer_info = container_of(bf_timer, struct boot_timer_info, boot_timer);
	complete(&ptimer_info->boot_complete);
}
#endif
...
#ifdef CONFIG_SHUT_DETECTOR
		if (get_machine_status() != BOOTUP) {
			delay_time = msecs_to_jiffies(ptimer_info->shut_count_step_value);
		} else {
			delay_time = msecs_to_jiffies(ptimer_info->count_step_value);
		}
		if (ptimer_info->boot_timer_count_task_should_run) {
			if (get_machine_status() != BOOTUP) {
				check_shut_time(ptimer_info);
			} else {
...
int start_boot_timer(void)
{
	...
	ptimer_info->boot_time_task = kthread_create(boot_timer_count_kthread, ptimer_info, "hwboot_time");
	...
	ptimer_info->long_timer_timeout_value = get_long_timer_timeout_value();
	ptimer_info->short_timer_timeout_value = get_short_timer_timeout_value();
	ptimer_info->boot_timer_count_enable = true;
	ptimer_info->boot_timer_count_task_should_run = true;
	mutex_init(&boot_timer_mutex);
#ifdef CONFIG_SHUT_DETECTOR
	ptimer_info->shut_timer_timeout_value = get_shut_timer_timeout_value();
	ptimer_info->shut_count_step_value = SHUT_TIMER_COUNT_STEP;
	if (get_machine_status() != BOOTUP) {
		delay_time = msecs_to_jiffies(ptimer_info->shut_count_step_value);
	} else {
		delay_time = msecs_to_jiffies(ptimer_info->count_step_value);
	...
	ptimer_info->boot_timer.data = (unsigned long)(uintptr_t)ptimer_info;
	ptimer_info->boot_timer.function = wakeup_boot_timer_count_task;
	ptimer_info->boot_timer.expires = jiffies + delay_time;
	ptimer_info->short_timer_last_start_time = jiffies;
#else
	timer_setup(&ptimer_info->boot_timer, wakeup_boot_timer_count_task, 0);
	ptimer_info->boot_timer.expires = jiffies + delay_time;
	ptimer_info->short_timer_last_start_time = jiffies;
#endif
