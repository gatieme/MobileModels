...
#ifdef CONFIG_SHUT_DETECTOR
struct shutfail_log {
	struct shutfail_log_header hdr;
	char log_data[0];
} __packed;
#endif
...
struct boot_detector_run_param {
	...
	int is_init_succ;
#ifdef CONFIG_SHUT_DETECTOR
	enum machine_status run_status;
	int is_shut_fail;
#endif
};
...
enum bootfail_errorcode set_boot_stage(enum boot_stage stage)
{
	...
	run_param.adp.mutex_ops.unlock();
	run_param.adp.print("func: set_boot_stage, error: %d, stage: %x\n",
		errorcode, (unsigned int)stage);
...
#ifdef CONFIG_SHUT_DETECTOR
void set_machine_status(int is_reboot)
{
	run_param.run_status = is_reboot ? REBOOT : SHUTDOWN;
	run_param.adp.print(
				"func: set_machine_status: %d\n",
				run_param.run_status);
}

enum machine_status get_machine_status(void)
{
	return run_param.run_status;
}

enum bootfail_errorcode get_shut_stage(enum shut_stage *pstage)
{
	enum bootfail_errorcode errorcode;
	...
	run_param.adp.mutex_ops.lock();
	errorcode = run_param.adp.shut_stage_ops.get_stage((int *)pstage);
	run_param.adp.mutex_ops.unlock();
	run_param.adp.print("func: get_shut_stage, error: %d, stage: %x\n",
		errorcode, *pstage);

	return errorcode;
}

enum bootfail_errorcode set_shut_stage(enum shut_stage stage)
{
	enum bootfail_errorcode errorcode;
	enum shut_stage curr_stage = SHUT_STAGE_INVALID;
	...
	// recovery from shut fail, not set stage
	if (run_param.is_shut_fail == 1) {
		run_param.adp.print("func: set_shut_stage, "
			"recovery from shut fail, now stage:%d\n",
			(unsigned int)stage);
		return BF_OK;
	}

	(void)get_shut_stage(&curr_stage);
	if ((curr_stage >= stage) && (curr_stage != SHUT_STAGE_SUCC)) {
		run_param.adp.print("func: set_shut_stage, "
			"curr_stage: %x, stage: %x\n",
			(unsigned int)curr_stage, (unsigned int)stage);
		return BF_OK;
	...
	run_param.adp.mutex_ops.lock();
	errorcode = run_param.adp.shut_stage_ops.set_stage((int)stage);
	run_param.adp.mutex_ops.unlock();
	run_param.adp.print("func: set_shut_stage, error: %d, stage: %x\n",
		errorcode, (unsigned int)stage);

	return errorcode;
}

static enum bootfail_errorcode __shut_fail_error(
	struct shutfail_proc_param *pparam)
{
	run_param.adp.print("func: __shut_fail_error, shut stage: %x, ",
		pparam->sinfo.stage);

	struct shutfail_log *plog = NULL;
	struct shutfail_log_header *phdr = NULL;

	unsigned int log_size = sizeof(*plog) + run_param.adp.kernel_log_ops.log_size;
	run_param.adp.print("func: __shut_fail_error, kernel_log_size: %d, ",
		run_param.adp.kernel_log_ops.log_size);
	plog = run_param.adp.mem_ops.malloc(log_size);
	if (plog == NULL) {
		run_param.adp.print("func: __shut_fail_error,"
			"error: %d(BF_ALLOC_MEM_FAIL), malloc size: 0x%lx\n",
			BF_ALLOC_MEM_FAIL, (unsigned long)log_size);
		return BF_ALLOC_MEM_FAIL;
	}
	run_param.adp.sec_funcs.memset_s(plog,
		(unsigned long)log_size, 0,
		(unsigned long)log_size);
	phdr = (struct shutfail_log_header *)(&plog->hdr);
	run_param.adp.sec_funcs.memcpy_s((char *)&phdr->sinfo,
		(unsigned long)sizeof(phdr->sinfo), (char *)&pparam->sinfo,
		(unsigned long)sizeof(phdr->sinfo));
	run_param.adp.sec_funcs.memcpy_s(phdr->log_path,
		(unsigned long)sizeof(phdr->log_path), pparam->log_path,
		(unsigned long)sizeof(phdr->log_path));
	phdr->kernel_log_size = run_param.adp.kernel_log_ops.log_size;
	get_kernel_log(plog->log_data, run_param.adp.kernel_log_ops.log_size);

	run_param.adp.print("func: __shut_fail_error, run_status: %d, ", run_param.run_status);
	run_param.adp.print("func: __shut_fail_error, shut_stage: %d, ", pparam->sinfo.stage);

	(void)run_param.adp.bfi_part.ops.write(
		run_param.adp.bfi_part.shut_dev_path,
		run_param.adp.bfi_part.shut_dev_offset, (char *)plog, log_size);

	if (pparam->sinfo.stage != SHUT_STAGE_KERNEL) {
		if (run_param.run_status == SHUTDOWN) {
			kernel_power_off();
		} else {
			kernel_restart(NULL);
		}
...
enum bootfail_errorcode shut_fail_error(
	struct shutfail_proc_param *pparam)
{
	enum bootfail_errorcode errcode;
	...
	if (run_param.is_init_succ == 0) {
		if (run_param.adp.print != NULL)
			run_param.adp.print("func: shut_fail_error, "
				"error: %d(BF_NOT_INIT_SUCC)\n",
				BF_NOT_INIT_SUCC);
		return BF_NOT_INIT_SUCC;
	}
	...
	run_param.adp.print("func: shut_fail_error!\n");
	run_param.is_shut_fail = 1;

	errcode =  __shut_fail_error(pparam);
