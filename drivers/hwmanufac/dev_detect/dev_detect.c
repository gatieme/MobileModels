...
#define DEV_DETECT_NAME             "honor,dev_detect"
...
int set_hw_dev_flag(int dev_id)
{
	if (dev_id < 0 || dev_id >= (int)DEV_I2C_MAX) {
		hilog_err("%s: dev_id = %d is invalid\n", __func__, dev_id);
		return false;
	}
	...
	hilog_info("%s: set dev_id = %d successful\n", __func__, dev_id);
	return true;
}
...
{
	hilog_info("%s: detect result is:%s\n", __func__, dev_detect_result);
...
static ssize_t dbc_detect_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	hilog_info("%s: dbc_detect_flag is:%s\n", __func__, dbc_detect_flag);
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1,
		"%s\n", dbc_detect_flag);
}
...
static ssize_t rt_detect_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	hilog_info("%s: rt_detect_flag is:%s\n", __func__, rt_detect_flag);
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1,
		"%s\n", rt_detect_flag);
}
...
static ssize_t mmi1_detect_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	hilog_info("%s: mmi1_detect_flag is:%s\n", __func__, mmi1_detect_flag);
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1,
		"%s\n", mmi1_detect_flag);
}
...
static ssize_t mmi2_detect_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	hilog_info("%s: mmi2_detect_flag is:%s\n", __func__, mmi2_detect_flag);
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1,
		"%s\n", mmi2_detect_flag);
}
...
static int create_dev_detect_sysfs(struct class *myclass)
{
	...
	for (dev_index = 0; dev_index < ARRAY_SIZE(dev_list); dev_index++) {
		...
		if (dev_list[dev_index].dev == NULL) {
			hilog_err("%s: failed to create %s device\n",
		...
		if (ret < 0) {
			hilog_err("%s: failed to create %s file\n",
				__func__, dev_list[dev_index].name);
			return -1;
		}
	...
	hilog_info("%s: dev_index:%zu, node_flag:%zu, file_flag:%zu", __func__,
...
static void remove_dev_detect_sysfs(struct class *myclass)
{
	...
	hilog_info("%s: remove dev detect sysfs end\n", __func__);
}
...
static void get_dev_detect_dts_config(const struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(dev_detect_table); i++) {
		...
		if (!result) {
			...
			hilog_err("get devid:%zu device name:%s value fail\n",
				i, dev_detect_table[i].device_name);
		}
	}

	hilog_info("%s: function end\n", __func__);
}
...
static int dev_detect_probe(struct platform_device *pdev)
{
	...
	if (dev_detect_probe_flag) {
		hilog_err("%s: dev_detect_probe is done\n", __func__);
		return 0;
	}

	hilog_info("%s: function begin\n", __func__);
	if (!pdev) {
		hilog_err("%s: none device\n", __func__);
		return -ENODEV;
	}

	np = pdev->dev.of_node;
	if (!np) {
		hilog_err("%s: unable to find device node\n", __func__);
		return -ENODEV;
	}

	myclass = class_create(THIS_MODULE, "dev_detect");
	if (IS_ERR(myclass)) {
		hilog_err("%s: failed to create dev_detect class\n", __func__);
		return -1;
	}

	if (create_dev_detect_sysfs(myclass) < 0) {
		hilog_err("%s: create dev detect sysfs failed\n", __func__);
		ret = -1;
