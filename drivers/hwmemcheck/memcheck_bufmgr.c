...
#define BUF_WRITE_FAIL -1
#define BUF_FORMATE_OK 0
...
extern char *memcheck_get_top_slub(void);
...
static void update_top_slub_info(struct mm_slub_detail_info *top_slub_detail,
	struct slabinfo *slab_info, struct kmem_cache *slab_cache, unsigned long slub_ram_size)
{
	top_slub_detail->active_objs = slab_info->active_objs;
	top_slub_detail->num_objs = slab_info->num_objs;
	top_slub_detail->objsize = (unsigned int)slab_cache->size;
	top_slub_detail->size = slub_ram_size;
	strncpy_s(top_slub_detail->name, MEMCHECK_MEM_NAME_MAX, slab_cache->name, MEMCHECK_MEM_NAME_MAX - 1);
}

void get_top_slub_info(struct mm_slub_detail_info *top_slab_info)
{
	unsigned long slub_ram_size;
	struct kmem_cache *s = NULL;
	struct slabinfo sinfo;

	if (top_slab_info == NULL)
		return;

	memset_s(top_slab_info, sizeof(*top_slab_info), 0, sizeof(*top_slab_info));
	mutex_lock(&slab_mutex);
	list_for_each_entry(s, &slab_caches, list) {
		memset_s(&sinfo, sizeof(sinfo), 0, sizeof(sinfo));
		get_slabinfo(s, &sinfo);
		slub_ram_size = (unsigned long)(s->size * sinfo.num_objs);
		if (slub_ram_size > top_slab_info->size) {
			update_top_slub_info(top_slab_info, &sinfo, s, slub_ram_size);
		}
	}
...
static inline int check_buf_write_result(int ret, size_t total, size_t *used)
{
	if (ret < 0) {
		return BUF_WRITE_FAIL;
	}
	(*used) += min((size_t) ret, total - *used - 1);
	if (*used >= (total - 1)) {
		return BUF_NOT_ENOUGH;
	}
	return BUF_FORMATE_OK;
}
				    ...
				    char *buf, size_t total, size_t used)
{
	...
	formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
						"leak(top) slab name: %s\n", memcheck_get_top_slub()), total, &used);
	if (formate_ret != BUF_FORMATE_OK) {
		goto buf_done;
	}

	formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
						"call_times [caller_backtrace]\n"), total, &used);
	if (formate_ret != BUF_FORMATE_OK) {
		goto buf_done;
	}
	for (i = 0; i < num; i++) {
		...
		formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
						"%zu ", list[i].ref), total, &used);
		if (formate_ret != BUF_FORMATE_OK)
			goto buf_done;
		for (j = 0; j < list[i].stack_entry.stack_len; j++) {
			formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
						"[%pS]", list[i].stack_entry.addrs[j]), total, &used);
			if (formate_ret != BUF_FORMATE_OK)
				goto buf_done;
		}
		formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
...
#else
		formate_ret = check_buf_write_result(snprintf_s(buf + used, total - used, total - used - 1,
						"%pS\t%zu\n", list[i].stack_entry.caller, list[i].ref), total, &used);
		if (formate_ret != BUF_FORMATE_OK)
			goto buf_done;
#endif
	}
				      ...
				      char *buf, size_t total)
{
	...
	for (i = 0; i < ARRAY_SIZE(slub_type); i++) {
		ret_num = mm_track_read(SLUB_TRACK, list, num,
					       slub_type[i]);
		if (ret_num == 0) {
					 ...
					 size_t num, char *buf, size_t total)
{
	...
	for (i = 0; i < ARRAY_SIZE(vm_type); i++) {
		ret_num = mm_track_read(VMALLOC_TRACK, list, num,
					       vm_type[i]);
		if (ret_num == 0) {
				       ...
				       char *buf, size_t total)
{
	...
	ret_num = mm_track_read(BUDDY_TRACK, list, num, BUDDY_TRACK);
	if (ret_num == 0) {
				       ...
				       char *buf, size_t total)
{
	...
	ret_num = mm_track_read(LSLUB_TRACK, list, num, LSLUB_TRACK);
	if (ret_num == 0) {
		memcheck_info("empty buddy stack record");
		return used;
	}
