...
static int process_get_memstat(void *arg)
{
	...
	result = memcheck_get_memstat(arg, &memstat);
...
static int process_get_ion_mem(void *arg)
{
	struct ion_mem_stat ionmem;

	if (copy_from_user(&ionmem, arg, sizeof(ionmem))) {
		memcheck_err("copy_from_user failed\n");
		return -EFAULT;
	}

	if (ionmem.magic != MEMCHECK_MAGIC) {
		memcheck_err("bad maginc number\n");
		return -EINVAL;
	}
	...
	ionmem.memory = get_ion_by_pid(ionmem.id);

	if (copy_to_user((void *)arg, &ionmem, sizeof(ionmem))) {
		memcheck_err("copy_to_user failed\n");
		return -EFAULT;
	}
...
static int process_do_command(const void *arg)
{
	...
	memcheck_info("IOCTL COMMAND id=%u,type=%u,timestamp=%llu,cmd=%d,full_threshold=%d,rate=%d,mem_name=%s\n",
		cmd.id, cmd.type, cmd.timestamp, cmd.cmd, cmd.sample_info.param.full_threshold, cmd.sample_info.param.rate,
	...
	if (cmd.type & (MTYPE_USER_VSS | MTYPE_KERN_ION |
...
long memcheck_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LOGGER_MEMCHECK_GET_ION_MEM:
		ret = process_get_ion_mem((void *)arg);
		break;
