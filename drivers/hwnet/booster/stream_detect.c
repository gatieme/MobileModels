...
#else
static void report_stat(struct timer_list *t)
#endif
{
	char *msg = NULL;
	struct stream_stat *stream = NULL;
...
#else
static void reset_list(struct timer_list *t)
#endif
{
	struct stream_stat *stream = NULL;
...
#else
	timer_setup(&g_stream_ctx->list_timer, NULL, 0);
	timer_setup(&g_stream_ctx->report_timer, NULL, 0);
#endif
