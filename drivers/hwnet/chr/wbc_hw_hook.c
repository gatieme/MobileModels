...
#include "huawei_platform/net/netbooster/video_acceleration.h"
#else
...
static struct http_return g_rtn_stat_wifi[RNT_STAT_SIZE];
...
static void save_app_syn_cnt(u32 uid, u8 type);
...
void chr_update_buf_time(s64 time, u32 protocal)
{
	...
	case SOL_TCP:
		...
		if (dif_jif > FILTER_TIME_LIMIT) {
			...
			atomic64_set(&g_chr_key_val.tcp_buf_time, cur_buf);
#else
			atomic_set(&g_chr_key_val.tcp_buf_time, cur_buf);
#endif
		} else {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			buff = atomic64_read(&g_chr_key_val.tcp_buf_time);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			atomic64_set(&g_chr_key_val.tcp_buf_time, buff);
#else
	...
	case SOL_UDP:
		...
		if (dif_jif > FILTER_TIME_LIMIT) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			atomic64_set(&g_chr_key_val.udp_buf_time, cur_buf);
#else
			atomic_set(&g_chr_key_val.udp_buf_time, cur_buf);
#endif
		} else {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			buff = atomic64_read(&g_chr_key_val.udp_buf_time);
#else
			...
			buff = buff - buff / ALPHA_FILTER_PARA +
				cur_buf / ALPHA_FILTER_PARA;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			atomic64_set(&g_chr_key_val.udp_buf_time, buff);
#else
...
static u32 report_buf(void)
{
	...
	jif = jiffies;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
	buf64 = atomic64_read(&g_chr_key_val.udp_buf_time);
#else
	...
	buf_rtn = tmp_buf;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
	buf64 = atomic64_read(&g_chr_key_val.tcp_buf_time);
#else
 ...
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
static void web_stat_timer(struct timer_list *data)
#else
	...
	if (g_stream_list[hash_cnt].ack_time_stamp != 0) {
		if (g_http_para_out.time_stamp >=
			...
			http_get_delay = (g_http_para_out.time_stamp -
				g_stream_list[hash_cnt].ack_time_stamp) * MULTIPLE;
		else
...
{
	pr_info("%s:\n", __func__);
	memset(&g_rtn_stat_wifi, 0, sizeof(g_rtn_stat_wifi));
	if (delay_chr == NULL)
		return;

	g_rtn_stat_wifi[WLAN_INTERFACE].report_type = WEB_STAT;
	g_rtn_stat_wifi[WLAN_INTERFACE].exception_cnt = delay_chr->exception_cnt;
	g_rtn_stat_wifi[WLAN_INTERFACE].data_direct = delay_chr->data_direct;
	g_rtn_stat_wifi[WLAN_INTERFACE].transport_delay =
		delay_chr->transport_delay;
	g_rtn_stat_wifi[WLAN_INTERFACE].ip_delay = delay_chr->ip_delay;
	g_rtn_stat_wifi[WLAN_INTERFACE].hmac_delay = delay_chr->hmac_delay;
	g_rtn_stat_wifi[WLAN_INTERFACE].driver_delay = delay_chr->driver_delay;
	g_rtn_stat_wifi[WLAN_INTERFACE].android_uid = delay_chr->android_uid;
	chr_notify_event(CHR_WEB_STAT_EVENT, g_user_space_pid, 0,
		g_rtn_stat_wifi);
}
...
static void save_rtt_num(u8 idx)
{
	if (g_stack_rtt[idx].tcp_rtt > RTT_THRESHOLD_L5)
	...
	else if (g_stack_rtt[idx].tcp_rtt > RTT_THRESHOLD_L1)
...
static void rtt_large_abn_report(int index)
{
	...
	if (time_after(jiffies, g_abn_stamp_rtt_large) &&
		...
		g_rtn_abn[index % IPV6_INTERFACE].data_reg_tech =
			get_data_reg_type_chr_enum(g_data_reg_tech);
		spin_unlock_bh(&g_web_stat_lock);
...
void tcp_window_check(struct sk_buff *skb, struct tcphdr *tcph)
{
	u32 uid;
	if (skb == NULL || skb->sk == NULL || tcph == NULL)
		return;
	if (tcph->window == 0 && tcph->syn == 0 && tcph->rst == 0 && tcph->ack == 1) {
		uid = get_uid_from_sock(skb->sk);
		pr_info("%s: uid: %d, port: %u, tcp window size: 0!\n", __func__, uid, skb->sk->sk_num);
	}
}

void tcp_syn_proc(struct sk_buff *skb, struct tcphdr *tcph)
{
	u8 interface_type;
	u32 uid;
	bool is_ipv6_pkt;
	struct inet_connection_sock *icsk = NULL;
	if (skb == NULL || skb->sk == NULL || tcph == NULL || skb->dev == NULL)
		return;
	if (tcph->syn != 1 || tcph->ack != 0)
		return;
	uid = get_uid_from_sock(skb->sk);
	if (uid != g_app_top_uid)
		return;
	icsk = inet_csk(skb->sk);
	if (icsk == NULL)
		return;
	if (icsk->icsk_retransmits < 1) {
		...
		interface_type += IPV6_INTERFACE * is_ipv6_pkt;
		spin_lock_bh(&g_web_stat_lock);
		g_rtn_stat[interface_type].tcp_total_num++;
		save_app_syn_cnt(uid, interface_type);
		spin_unlock_bh(&g_web_stat_lock);
	}
	return;
}
...
u8 http_response_type(const char *pstr)
{
	...
	type = UN_KNOW;
	if (pstr[HTTP_ACK_FROM_START] == HTTP_RESPONSE_NORMAL ||
		pstr[HTTP_ACK_FROM_START] == HTTP_RESPONSE_REDIRECT)
		type = WEB_SUCC;

	if (pstr[HTTP_ACK_FROM_START] == HTTP_RESPONSE_CLIENT_ERROR ||
		pstr[HTTP_ACK_FROM_START] == HTTP_RESPONSE_SERVER_ERROR)
		type = WEB_FAIL;
...
void tcp_win_monitor(struct sock *sk, struct tcphdr *th)
{
	...
	if (sk == NULL || th == NULL || get_uid_from_sock(sk) == 0)
		return;
...
#ifndef CONFIG_HW_BOOSTER
static u32 get_dlen(const char *p_http_str, bool is_ipv6_pkt, struct sk_buff *skb,
...
static bool is_http_para_out(u32 dlen, const char *p_http_str)
{
	if (p_http_str == NULL)
		return false;
...
static bool check_is_http_out(bool is_ipv6_pkt, struct sk_buff *skb, struct ipv6hdr *ip6h, struct iphdr *iph, struct tcphdr *tcph, bool *is_http_out)
{
	u32 dlen;
#ifdef CONFIG_HW_BOOSTER
	u8 *vaddr = NULL;
	char *p_http_str = (char *)hw_get_payload_addr(skb, IPPROTO_TCP, &vaddr, &dlen); // need unmap_vaddr
#else
	char *p_http_str = (char *)((u32 *)tcph + tcph->doff);
	dlen = get_dlen(p_http_str, is_ipv6_pkt, skb, ip6h, iph);
#endif
	if (dlen == 0 || p_http_str == NULL) {
#ifdef CONFIG_HW_BOOSTER
		hw_unmap_vaddr(vaddr);
#endif
		return false;
	}
	*is_http_out = is_http_para_out(dlen, p_http_str);
#ifdef CONFIG_HW_BOOSTER
	hw_unmap_vaddr(vaddr);
#endif
	return true;
...
static unsigned int hook_local_out(void *ops, struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	/* identify the network protocol of an IP packet */
	if (identify_network_protocol(skb, &ip6h, &iph, &tcph) == -1 || skb->dev == NULL)
		return NF_ACCEPT;
	tcp_window_check(skb, tcph);
	tcp_syn_proc(skb, tcph);
	if (get_http_stream_iface(skb, false) == -1 || htons(tcph->dest) != HTTP_PORT ||
	...
	bool is_ipv6_pkt = is_ipv6_packet(skb->sk);
	if (g_http_para_out.is_valid == IS_UNUSE) {
		...
		if (tcph->syn == 1 && tcph->ack == 0) {
			...
			bool is_http_out = false;
			if (!check_is_http_out(is_ipv6_pkt, skb, ip6h, iph, tcph, &is_http_out)) {
				spin_unlock_bh(&g_web_para_out_lock);
				return NF_ACCEPT;
			}
			if (is_http_out) {
...
int is_http_response(char *p_http_str, u32 dlen, struct tcphdr *tcph)
{
...
static unsigned int hook_local_in(void *ops, struct sk_buff *skb, const struct nf_hook_state *state)
{
	u32 dlen;
	bool up_req = false;
	struct iphdr *iph = NULL;
	...
	if (get_http_stream_iface(skb, true) == -1 || htons(tcph->source) != HTTP_PORT ||
	...
	int is_response = is_http_response(p_http_str, dlen, tcph);
	if (is_response == 1) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
	atomic64_set(&g_chr_key_val.tcp_buf_time, 0);
	atomic64_set(&g_chr_key_val.udp_buf_time, 0);
#else
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
	timer_setup(&g_web_stat_timer, web_stat_timer, 0);
#else
...
void save_app_syn_cnt(u32 uid, u8 type)
{
	int i;

	for (i = 0; i < CHR_MAX_REPORT_APP_COUNT; i++) {
		if (uid == g_s_report_app_uid_lst[i]) {
			g_rtn_stat[type].report_app_stat_list[i].tcp_total_num++;
			break;
		}
	}
}
