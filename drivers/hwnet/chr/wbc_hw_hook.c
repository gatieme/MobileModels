...
#ifdef CONFIG_HN_CHR_TCP_SMALL_WIN_MONITOR
#include <net/tcp.h>
#include <hwnet/ipv4/tcp_small_window_chr_monitor.h>
#endif

#include "chr_netlink.h"
#ifdef CONFIG_HN_NETBOOSTER_MODULE
...
u32 get_http_stream_iface(struct sk_buff *skb, bool is_http_para_in)
{
	...
	if (strncmp(skb->dev->name, WEB_DS_NET, WEB_DS_NET_LEN) == 0 ||
		strncmp(skb->dev->name, WEB_DS_NET_SPRD, WEB_DS_NET_SPRD_LEN) == 0) {
	...
	const struct nf_hook_state *state)
{
	...
	unsigned char *p_http_str = NULL;
	bool up_req = false;
	s64 dlen;
	bool is_ipv6_pkt = false;
	unsigned char http_resp[HTTP_RSP_LEN + 1] = {0};
	...
	if (identify_network_protocol(skb, &ip6h, &iph, &tcph) == -1)
		return NF_ACCEPT;
	p_http_str = (unsigned char *)((u32 *)tcph + tcph->doff);
	...
	if (skb->data_len > HTTP_RSP_LEN && (p_http_str - skb->data > 0)) {
		if(skb_copy_bits(skb, p_http_str - skb->data, (void*)http_resp, HTTP_RSP_LEN) == 0) {
			p_http_str = http_resp;
		}
	}
#endif
