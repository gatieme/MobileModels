...
void add_to_droplist(char *url, int len)
{
	...
	temp[len] = 0;
	if (memcpy_s(temp, len + 1, url, len) != EOK) {
...
void add_url_to_node(struct ad_rule *node, const char *url, int len)
{
	...
	urlstr = kmalloc(urllen + 2, GFP_ATOMIC); /* new 101 */
	if (urlstr == NULL)
		return;
	...
	list_for_each_entry(nd, &node->urllist.list, list) {
		if (urllen == nd->len) {
			if (memcmp(nd->url, urlstr, urllen) == 0) {
				pr_info("hnad: already in list url=%s", url);
...
void add_ad_rule_to_list(struct list_info *listinfo, unsigned int uid, const char *url, int len)
{
	...
	hid = get_hash_id(uid);
	if (hid >= MAX_HASH) {
		pr_info("hnad:url to list>=MAX_HASH id=%u\n", hid);
		return;
	}
	...
	pr_info("hnad:add_ad_rule_to_list newuid=%u url=<%s>\n", uid, url);
...
void add_ad_uid_rule(struct list_info *listinfo, const char *rule, int len)
{
	...
	uid = simple_strtoull(rule, NULL, 10);
	if (uid < 1000) {
...
bool match_ad_uid(struct list_info *listinfo, unsigned int uid)
{
	...
	hid = get_hash_id(uid);
	if (hid >= MAX_HASH) {
		pr_info("hnad: match_%s_uid>=MAX_hid=%u uid=%u\n", listinfo->type == AD ? "ad" : "delta", hid, uid);
		return false;
	}
...
bool match_uid_url(struct ad_rule *node, const char *url)
{
	...
	list_for_each_entry(temp_node, &node->urllist.list, list) {
		if (0 != temp_node->len) {
			if (match_url_string(url, temp_node->url, temp_node->len)) {
				pr_info("hnad: match_url_string true rule=%s\n", temp_node->url);
				bret = true;
				break;
			}
		}
...
bool match_ad_url(struct list_info* listinfo, unsigned int uid, const char *data, int datalen)
{
	...
	if (listinfo == NULL || data == NULL || uid == 0xffffffff || datalen <= 0)
		return false;
	...
	if (tempurl == NULL) {
		...
		if (memcpy_s(tempurl, datalen + 1, data, datalen) != EOK) {
			pr_info("hnad: error EOK %s %d", __func__, __LINE__);
			kfree(tempurl);
			return false;
		}
		...
		*(tempurl + datalen) = 0;
		pr_info("hnad:match_%s_url multi part\n", listinfo->type == AD ? "ad" : "delta");
	}
...
void output_ad_rule(struct list_info* listinfo)
{
	...
	pr_info("hnad:output_%s_rule start\n", listinfo->type == AD ? "ad" : "delta");
	for (i = 0; i < MAX_HASH; i++) {
		spin_lock_bh(&listinfo->lock[i]);
		list_for_each_entry(node, &listinfo->head[i], list) {
			pr_info("hnad:output_%s_rule uid=%d\n", listinfo->type == AD ? "ad" : "delta",node->uid);
			list_for_each_entry(temp, &node->urllist.list, list) {
				pr_info("hnad:output_%s_rule=%s len=%d\n",
						listinfo->type == AD ? "ad" : "delta",temp->url, temp->len);
			}
			pr_info("\n");
		}
		spin_unlock_bh(&listinfo->lock[i]);
	}
	pr_info("hnad: output_%s_rule  end\n",listinfo->type == AD ? "ad" : "delta");
}
