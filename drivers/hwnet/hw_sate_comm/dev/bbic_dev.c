	...
	struct ps_core_s *cd)
{
	...
	ret = of_property_read_u32(dev_node, "bbic,usim_ldo_num",
		(unsigned int *)(&cd->usim0_ldo_num));
	if (ret != 0) {
		hwlog_info("%s: failed to get usim_ldo_num,err:%d\n", __func__, ret);
		return -1;
	}
	ret = of_property_read_u32(dev_node, "bbic,usim_ldo_vol_1v8",
		(unsigned int *)(&cd->usim0_ldo_vol_1v8));
	if (ret != 0) {
		hwlog_info("%s: failed to get usim_ldo_vol_1v8,err:%d\n", __func__, ret);
		return -1;
	}
	ret = of_property_read_u32(dev_node, "bbic,sim0_ldo_vol",
		(unsigned int *)(&cd->usim0_card_ldo_vol));
	if (ret != 0) {
		hwlog_info("%s: failed to get usim0_card_ldo_vol,err:%d\n", __func__, ret);
		return -1;
	}
	ret = of_property_read_u32(dev_node, "bbic,sim1_ldo_vol",
		(unsigned int *)(&cd->usim1_card_ldo_vol));
	if (ret != 0) {
		hwlog_info("%s: failed to get usim1_card_ldo_vol,err:%d\n", __func__, ret);
		return -1;
	}
	ret = of_property_read_u32(dev_node, "bbic,usim_ldo_vol_3v",
		(unsigned int *)(&cd->usim0_ldo_vol_3v));
	if (ret != 0) {
		hwlog_info("%s: failed to get usim_ldo_vol_3v,err:%d\n", __func__, ret);
		return -1;
	}
...
static int bbic_pwr_off_proc(struct ps_core_s *cd)
{
	...
	if (gpio_is_valid(cd->ant_vc1)) // GPIO_332 ant vc1
		gpio_set_value(cd->ant_vc1, GPIO_HIGH);
	if (gpio_is_valid(cd->ant_vc2)) // GPIO_351 ant vc2
...
static void rf_pwr_on_proc(struct ps_core_s *cd)
{
	...
	if (bbic_chip_type == BBIC_CHIP_TYPE_WS) {
		buck_boost_set_vout(PA_BST_VOL_4V5, BBST_USER_BOOST_HW_SATE);
...
static int32_t pwr_config_init_ws(struct ps_core_s *cd)
{
	...
	rc = init_gpio(cd->pwr_en, "bbic_pwr_en", GPIO_LOW, GPIO_DIRECTION_OUTPUT);
	if (rc != 0) {
		hwlog_err("%s: init gpio %d failed %d\n",
