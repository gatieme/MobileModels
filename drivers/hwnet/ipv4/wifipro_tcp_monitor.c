...
void wifipro_update_tcp_statistics_outsegs(int mib_type, const struct sk_buff *skb,
	struct sock *from_sk)
{
	...
	if (mib_type == WIFIPRO_TCP_MIB_OUTSEGS) {
		if (solve_wifipro_tcp_mib_outsegs_no_check(sk, skb))
			return;
	} else {
		unknown_segs_count++;
