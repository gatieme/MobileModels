...
{
	hwlog_info("%s: oldSpiStatus:%d, newSpiStatus:%d", __func__,
...
static void rsmc_pa_power_ctrl(struct smc_core_data *cd, bool status)
{
	int gpio_status = status ? GPIO_HIGH : GPIO_LOW;

	hwlog_info("%s: pa power status is %d\n", __func__, gpio_status);
	if (cd->pa_powers.pa_vcc_type == RSMC_POWER_LDO) {
		if (status) {
			buckboost_set_enable(ENABLE, BBST_USER_BOOST_RSMC, cd->pa_powers.pa_pwr_i2c_index);
			buckboost_set_vout(cd->pa_powers.pa_pwr_i2c_on_volt, BBST_USER_BOOST_RSMC,
				cd->pa_powers.pa_pwr_i2c_index);
		} else {
			buckboost_set_enable(DISABLE, BBST_USER_BOOST_RSMC, cd->pa_powers.pa_pwr_i2c_index);
		}
		return;
	}
	if (gpio_is_valid(cd->gpios.pa_pwr_gpio))
		gpio_set_value(cd->gpios.pa_pwr_gpio, gpio_status);
}
...
static int chip_enable(struct smc_core_data *cd)
{
	...
	if (gpio_is_valid(cd->gpios.bright1_gpio))
		gpio_set_value(cd->gpios.bright1_gpio, GPIO_LOW);
	mdelay(ENABLE_SIGNAL_DELAY_MS);
	if (gpio_is_valid(cd->gpios.pa_pwr_gpio)) {
		rsmc_pa_power_ctrl(cd, true);
	} else {
	...
	if (gpio_is_valid(cd->gpios.bright2_gpio))
		gpio_set_value(cd->gpios.bright2_gpio, GPIO_HIGH);
...
static int chip_disable(struct smc_core_data *cd)
{
	...
	if (gpio_is_valid(cd->gpios.bright2_gpio))
		gpio_set_value(cd->gpios.bright2_gpio, GPIO_LOW);
	if (gpio_is_valid(cd->gpios.pa_pwr_gpio)) {
		rsmc_pa_power_ctrl(cd, false);
	} else {
		boost_status = buck_boost_set_enable(GPIO_LOW, BBST_USER_BOOST_RSMC);
...
static void chip_power_on(bool status)
{
	...
	if (gpio_is_valid(cd->gpios.bright1_gpio)) {
		value = gpio_get_value(cd->gpios.bright1_gpio);
		hwlog_info("%s: bright1_gpio value %d", __func__, value);
	}
	if (gpio_is_valid(cd->gpios.bright2_gpio)) {
		value = gpio_get_value(cd->gpios.bright2_gpio);
		hwlog_info("%s: bright2_gpio value %d", __func__, value);
	}
	if (gpio_is_valid(cd->gpios.pa_pwr_gpio)) {
...
static void enable_default_ant(void)
{
	...
	if (gpio_is_valid(cd->gpios.rx_ant_gpio))
		gpio_set_value(cd->gpios.rx_ant_gpio, cd->gpios.rx_ant_default_value);
	if (gpio_is_valid(cd->gpios.tx_ant_gpio))
		gpio_set_value(cd->gpios.tx_ant_gpio, cd->gpios.tx_ant_default_value);
	if (gpio_is_valid(cd->gpios.vc1_ant_gpio))
...
u32 sta(char *data)
{
	...
	ret = sscanf_s(cmd, "$STA,%[0-9a-zA-Z],%[0-9a-zA-Z],%d,%d,%d,%d,%d,%d,%d*%x\r\n",
...
void up_spi_sema(void)
{
	if (g_rsmc_spi_ctx.task == NULL) {
		hwlog_err("%s: spi task not ready", __func__);
		return;
	}
...
int rsmc_spi_msg_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		u32 spi_irq_num = (u32)atomic_read(&g_rsmc_spi_ctx.spi_irq);
		hwlog_info("%s: spi_status:%d,spi_irq:%d", __func__,
			g_rsmc_spi_ctx.spi_status, spi_irq_num);
		while (spi_irq_num > 0) {
			rsmc_spi_recv();
			spi_irq_num = (u32)atomic_read(&g_rsmc_spi_ctx.spi_irq);
			hwlog_info("%s: start rsmc_spi_recv,spi_irq:%d", __func__, spi_irq_num);
		}
...
static bool recv_fw_data_buffer(struct enable_msg *msg)
{
	...
	if ((g_rsmc_fw_ctx.file_len <= 0) || (msg->data_block_len <= 0)) {
		hwlog_err("%s: err file_len:%d,block_len:%d", __func__, msg->file_len, msg->data_block_len);
		return false;
	}
	...
	if ((strlen(g_rsmc_fw_ctx.fw_buffer + start_idx) > g_rsmc_fw_ctx.file_len) ||
		(g_rsmc_fw_ctx.file_len < msg->data_block_len)) {
		vfree(g_rsmc_fw_ctx.fw_buffer);
		g_rsmc_fw_ctx.fw_buffer = NULL;
		return false;
	}
...
static void rsmc_disable(bool send_ack)
{
	...
	chip_power_on(false);
	g_device_ctx.is_chip_on = false;
	spin_lock_dn_inter_lock();
	rsmc_clear_dn_inter_msg_list();
	spin_unlock_dn_inter_lock();
	spin_lock_dn_spi_lock();
	rsmc_clear_dn_spi_msg_list();
	spin_unlock_dn_spi_lock();
