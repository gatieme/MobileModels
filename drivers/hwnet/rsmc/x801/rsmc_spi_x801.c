...
static int smc_setup_tx_ant_gpio(struct smc_core_data *cd)
{
	...
	if (gpio_is_valid(cd->gpios.tx_ant_gpio)) {
		...
		rc = gpio_direction_output(cd->gpios.tx_ant_gpio, cd->gpios.tx_ant_default_value);
		if (rc) {
...
static int smc_setup_rx_ant_gpio(struct smc_core_data *cd)
{
	...
	if (gpio_is_valid(cd->gpios.rx_ant_gpio)) {
		...
		rc = gpio_direction_output(cd->gpios.rx_ant_gpio, cd->gpios.rx_ant_default_value);
		if (rc) {
...
static int smc_setup_bright_gpio(struct smc_core_data *cd)
{
	int rc;

	if (cd == NULL)
		return -EINVAL;
	if (gpio_is_valid(cd->gpios.bright1_gpio)) {
		rc = gpio_request(cd->gpios.bright1_gpio, "rsmc_bright1_gpio");
		if (rc) {
			hwlog_err("%s: bright1_gpio %d request failed %d\n",
				__func__, cd->gpios.bright1_gpio, rc);
			return rc;
		}

		rc = gpio_get_value(cd->gpios.bright1_gpio);
		hwlog_info("%s: bright1_gpio %d value %d",
			__func__, cd->gpios.bright1_gpio, rc);

		rc = gpio_direction_output(cd->gpios.bright1_gpio, GPIO_LOW);
		if (rc) {
			hwlog_err("%s: bright1_gpio %d output low err %d",
				__func__, cd->gpios.bright1_gpio, rc);
			return rc;
		}
	...
	if (gpio_is_valid(cd->gpios.bright2_gpio)) {
		rc = gpio_request(cd->gpios.bright2_gpio, "rsmc_bright2_gpio");
		if (rc) {
			hwlog_err("%s: bright2_gpio %d request failed %d\n",
				__func__, cd->gpios.bright2_gpio, rc);
			return rc;
		}

		rc = gpio_get_value(cd->gpios.bright2_gpio);
		hwlog_info("%s: bright2_gpio %d value %d",
			__func__, cd->gpios.bright2_gpio, rc);

		rc = gpio_direction_output(cd->gpios.bright2_gpio, GPIO_LOW);
		if (rc) {
			hwlog_err("%s: bright2_gpio %d output low err %d",
				__func__, cd->gpios.bright2_gpio, rc);
			return rc;
		}
...
static int smc_setup_gpio(struct smc_core_data *cd)
{
	...
	// bright1&bright2 gpio
	rc = smc_setup_bright_gpio(cd);
	// PA PWR
...
static void smc_free_gpio(struct smc_core_data *cd)
{
	...
	if (gpio_is_valid(cd->gpios.bright1_gpio))
		gpio_free(cd->gpios.bright1_gpio);
	if (gpio_is_valid(cd->gpios.bright2_gpio))
		gpio_free(cd->gpios.bright2_gpio);
	if (gpio_is_valid(cd->gpios.cs_gpio))
...
static int smc_parse_pa_power_config(struct device_node *smc_node,
	struct smc_core_data *cd)
{
	int rc;
	int value;
	...
	rc = of_property_read_u32(smc_node, "pa-vcc-type", &value);
	if (!rc) {
		cd->pa_powers.pa_vcc_type = (u32)value;
	} else {
		cd->pa_powers.pa_vcc_type = RSMC_POWER_GPIO;
		hwlog_info("%s: pa power is contrlloed by normal gpio\n", __func__);
		return 0;
	}

	rc = of_property_read_u32(smc_node, "pa_pwr_i2c_index", &value);
	if (!rc) {
		cd->pa_powers.pa_pwr_i2c_index = (u32)value;
		hwlog_info("%s: pa_powers is %d\n", __func__, value);
	}

	rc = of_property_read_u32(smc_node, "pa_pwr_i2c_on_volt", &value);
	if (!rc) {
		cd->pa_powers.pa_pwr_i2c_on_volt = (u32)value;
		hwlog_info("%s: pa_pwr_i2c_on_volt is %d\n", __func__, value);
	}

	rc = of_property_read_u32(smc_node, "pa_pwr_i2c_off_volt", &value);
	if (!rc) {
		cd->pa_powers.pa_pwr_i2c_off_volt = (u32)value;
		hwlog_info("%s: pa_pwr_i2c_off_volt is %d\n", __func__, value);
	}
	...
	struct smc_core_data *cd)
{
	...
	value = of_get_named_gpio(smc_node, "bright1_gpio", 0);
	hwlog_info("bright1_gpio = %d", value);
	cd->gpios.bright1_gpio = value;

	value = of_get_named_gpio(smc_node, "bright2_gpio", 0);
	hwlog_info("bright2_gpio = %d", value);
	cd->gpios.bright2_gpio = value;

	value = of_get_named_gpio_flags(smc_node, "rx_ant_gpio", 0,
		&cd->gpios.rx_ant_default_value);
	hwlog_info("rx_ant_gpio = %d, default_value = %d",
		value, cd->gpios.rx_ant_default_value);
	cd->gpios.rx_ant_gpio = (u32)value;

	value = of_get_named_gpio_flags(smc_node, "tx_ant_gpio", 0,
		&cd->gpios.tx_ant_default_value);
	hwlog_info("tx_ant_gpio = %d, default_value = %d",
		value, cd->gpios.tx_ant_default_value);
	cd->gpios.tx_ant_gpio = (u32)value;

	value = of_get_named_gpio(smc_node, "sp4t_pwr_gpio", 0);
	hwlog_info("sp4t_pwr_gpio = %d", value);
	cd->gpios.sp4t_pwr_gpio = value;
...
static void smc_spi_cs_set(u32 control)
{
	struct smc_core_data *cd = smc_get_core_data_x801();
	if (cd == NULL) {
		hwlog_info("%s: cd is null\n", __func__);
		return;
	}
	gpio_set_value(cd->gpios.cs_gpio, control);
}

static void smc_parse_spi_pl022_config(struct device_node *smc_node,
	struct smc_core_data *cd)
{
	...
	struct smc_core_data *cd)
{
	...
	smc_parse_spi_pl022_config(smc_node, cd);
	...
	struct smc_core_data *cd)
{
	...
	if (ret != 0)
		return ret;
	ret = smc_parse_pa_power_config(smc_node, cd);
	if (ret != 0)
		return ret;
