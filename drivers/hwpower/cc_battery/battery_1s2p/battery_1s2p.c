...
#define BAT_1S2P_RETRY_TIMES        100
...
static int bat_1s2p_read_battery_charge_counter(void *dev_data)
{
	int charge_counter0 = coul_interface_get_battery_charge_counter(COUL_TYPE_MAIN);
	int charge_counter1 = coul_interface_get_battery_charge_counter(COUL_TYPE_AUX);
	int sum = 0;

	if (charge_counter0 < 0 && charge_counter1 < 0)
		return -EPERM;
	if (charge_counter0 >= 0)
		sum += charge_counter0;
	if (charge_counter1 >= 0)
		sum += charge_counter1;
...
static struct coul_interface_ops g_1s2p_ops = {
	...
	.get_battery_charge_counter =  bat_1s2p_read_battery_charge_counter,
...
static int bat_1s2p_get_mix_soc(struct bat_1s2p_device *di)
{
	...
	for (i = 0; i < BAT_1S2P_COUNT; i++) {
		...
		for (j = 0; j < VOLT_THRESHOLD_COUNT; j++) {
			if (di->bat_info[i].volt <= (int)p_vol[j])
				break;
		}
...
static int bat_1s2p_info_mixing(struct bat_1s2p_device *di)
{
	...
	return bat_1s2p_get_mix_soc(di);
}
...
static void bat_1s2p_detect_fault_send_dmd(struct bat_1s2p_device *di)
{
	...
	if ((!di->bat_info[BAT_MAIN].is_exist) || (!di->bat_info[BAT_AUX].is_exist)) {
		...
		power_dsm_report_dmd(POWER_DSM_BATTERY_DETECT,
			POWER_DSM_DUAL_BATTERY_OUT_OF_POSITION_DETECTION, tmp_buf);
		return;
	}
	...
	if (abs(di->bat_info[BAT_MAIN].soc - di->bat_info[BAT_AUX].soc) > di->cap_diff_thr) {
		snprintf(tmp_buf, BAT_1S2P_DSM_BUF_SIZE,
			...
			di->cap_diff_thr);
		power_dsm_report_dmd(POWER_DSM_BATTERY_DETECT,
			POWER_DSM_DUAL_BATTERY_CAPACITY_DIFFERENT_DETECT, tmp_buf);
	}
}
