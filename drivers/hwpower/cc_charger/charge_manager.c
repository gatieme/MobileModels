...
static void charge_direct_charge_check(unsigned int type)
{
...
{
	if (g_di)
		power_wakeup_unlock(g_di->charge_lock, false);
}
...
static void charge_monitor_work(struct work_struct *work)
{
	...
	if (direct_charge_in_charging_stage() == DC_NOT_IN_CHARGING_STAGE)
		charge_direct_charge_check(charge_type);
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHG_TYPE: /* bc12 result */
		...
		di->bc12_chg_type = val->intval;
		if (charge_get_charger_online())
			charger_type_handler(di->bc12_chg_type, NULL);
		return 0;
	default:
		return 0;
	}
...
static int charge_manager_resume(struct platform_device *pdev)
{
	...
	if (charge_done_status == CHARGE_DONE) {
		power_wakeup_lock(di->charge_lock, false);
		mod_delayed_work(system_power_efficient_wq, &di->charge_work, 0);
		hwlog_info("%s: charge wake lock when resume during charging\n", __func__);
	}
	return 0;
}
#endif /* CONFIG_PM */
