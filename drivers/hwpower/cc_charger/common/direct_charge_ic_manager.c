...
static int dcm_get_vbtb_from_coul(int mode, unsigned int path)
{
	int vbat = POWER_SUPPLY_DEFAULT_VOLTAGE_NOW;
	struct power_supply *psy = NULL;
	const char *psy_name = NULL;

	psy_name = dc_ic_get_ic_battery_gauge_name(mode, path);
	if (!power_supply_check_psy_available(psy_name, &psy))
		psy_name = (path == CHARGE_IC_AUX) ? "battery_gauge_aux" : POWER_PLATFORM_BAT_PSY_NAME;

	if (power_supply_check_psy_available(psy_name, &psy))
		power_supply_get_int_prop(psy_name,
...
static int dcm_get_larger_vbtb_from_ic(int mode)
{
	int vbat_main = dcm_get_vbtb_from_ic(mode, CHARGE_IC_MAIN);
	int vbat_aux = dcm_get_vbtb_from_ic(mode, CHARGE_IC_AUX);

	if (vbat_aux - vbat_main >= VBAT_MAIN_AUX_SC_DIFF)
		return vbat_aux;
...
int dcm_get_ic_vbtb(int mode, unsigned int path)
{
	if (dc_ic_get_vbat_from_coul())
		return dcm_get_vbtb_from_coul(mode, path);

	if (dc_ic_get_larger_vbat_from_ic())
		return dcm_get_larger_vbtb_from_ic(mode);
...
int dcm_set_ic_threshold_enable(int mode, unsigned int path, int enable)
{
	unsigned int index[CHARGE_IC_MAX_NUM] = { 0 };
	int num = dc_ic_get_ic_index(mode, path, index, CHARGE_IC_MAX_NUM);
	int i;
	...
	for (i = 0; i < num; i++) {
		if (dc_set_ic_threshold_enable(mode, index[i], enable))
			return -EPERM;
	}
