...
static unsigned int dc_info_get_min_ibat_th(struct direct_charge_device *l_di)
{
	int batt_soc = 0;
	int i;
	...
	for (i = 0; i < DC_GROUP_MAX; i++) {
		if (l_di->soc_ibat_th_para[i].soc == 0)
			return l_di->min_ibat_th;
		if (batt_soc < l_di->soc_ibat_th_para[i].soc)
			return l_di->soc_ibat_th_para[i].ibat_th;
	}
...
unsigned int dc_info_get_val_with_mode(int local_mode, int type)
{
	...
	case MIN_IBAT_TH:
		return dc_info_get_min_ibat_th(l_di);
	default:
		break;
	}
