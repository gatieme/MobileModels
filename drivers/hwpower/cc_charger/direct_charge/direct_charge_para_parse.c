...
#define DC_TIME_PARA_GROUP_MAX_SIZE     8
	...
	struct direct_charge_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"regulation_by_ibat", (u32 *)&di->regulation_by_ibat, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"vadapt_diff_th", (u32 *)&di->vadapt_diff_th, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"min_ibat_th", (u32 *)&di->min_ibat_th, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"pmu_mode_set_flag", &di->pmu_mode_set_flag, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"single_ic_mode", &di->multi_ic_mode_para.single_ic_mode, CHARGE_IC_MAIN);
}
	...
	struct direct_charge_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"path_switch_type", &di->multi_ic_mode_para.path_switch_type, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"need_boost_5v", &di->need_boost_5v, 0);

	multi_ic_parse_check_para(np, &di->multi_ic_check_info);
}

static int dc_parse_time_para_single(struct device_node *np,
	struct direct_charge_time_para *para, const char *name)
{
	int row, col, len;
	int idata[DC_TIME_PARA_LEVEL * DC_TIME_INFO_MAX] = { 0 };

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		name, idata, DC_TIME_PARA_LEVEL, DC_TIME_INFO_MAX);
	if (len < 0)
		return -1;
...
static void dc_parse_time_para(struct device_node *np,
	struct direct_charge_device *di)
{
	int len, i, row, col, idata, ret;
	const char *tmp_string = NULL;

	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np, "time_para_group",
		DC_TIME_PARA_GROUP_MAX_SIZE, TIME_PARA_GROUP_END);
	if (len <= 0) {
		ret = dc_parse_time_para_single(np, di->time_para, "time_para");
		if (!ret)
	...
	for (i = 0; i < len; i++) {
		if (power_dts_read_string_index(power_dts_tag(HWLOG_TAG), np, "time_para_group", i, &tmp_string))
			return;
		row = i / TIME_PARA_GROUP_END;
		col = i % TIME_PARA_GROUP_END;
		switch (col) {
		case TIME_PARA_GROUP_MODE:
			if (kstrtou32 (tmp_string, POWER_BASE_HEX,
				&di->time_para_p[row].mode))
				return;
			break;
		case TIME_PARA_GROUP_NAME:
			ret = dc_parse_time_para_single(np, di->time_para_p[row].time_info, tmp_string);
			if (ret)
				return;
		}
	}
	di->time_para_p_parse_ok = 1;
}
...
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"dfg_disable", &di->dfg_disable, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sysfs_disable", &di->sysfs_disable, 0);
}

static void dc_parse_watch_dog_para(struct device_node *np,
	struct direct_charge_device *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"wtd_timeout", &di->wtd_timeout, WATCHDOG_TIMEOUT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"kick_wtd_time", &di->kick_wtd_time, KICK_WATCHDOG_TIME);
}

static void dc_parse_ibat_th_para(struct device_node *np,
	struct direct_charge_device *di)
{
	int row, col, len;
	int idata[DC_GROUP_MAX * DC_SOC_IBAT_TH_MAX] = { 0 };

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"soc_ibat_th", idata, DC_GROUP_MAX, DC_SOC_IBAT_TH_MAX);
	if (len < 0)
		return;

	for (row = 0; row < len / DC_SOC_IBAT_TH_MAX; row++) {
		col = row * DC_SOC_IBAT_TH_MAX + DC_SOC_IBAT_TH_SOC;
		di->soc_ibat_th_para[row].soc = idata[col];
		col = row * DC_SOC_IBAT_TH_MAX + DC_SOC_IBAT_TH_IBAT;
		di->soc_ibat_th_para[row].ibat_th = idata[col];
	}
}
...
int dc_parse_dts(struct device_node *np, void *p)
{
	...
	dc_parse_watch_dog_para(np, di);
	dc_parse_ibat_th_para(np, di);
