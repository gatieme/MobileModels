...
void dc_pmode_disable_mode(unsigned int reason, unsigned int mode)
{
	...
	tbl[idx].dis_flag |= reason;
	if (reason & (DC_DIS_BY_IBAT_TH_LOW | DC_DIS_BY_SYSFS_DISABLE | DC_DIS_BY_PRIORITY))
...
static bool dc_pmode_dis_flag_check_failed(struct pmode_table *tbl)
{
	...
	if (tbl->dis_flag & (DC_DIS_BY_IBAT_TH_LOW | DC_DIS_BY_SYSFS_DISABLE | DC_DIS_BY_PRIORITY)) {
		time = power_get_current_kernel_time().tv_sec - tbl->dis_time_begin;
		time_th = ((tbl->dis_flag & DC_DIS_BY_IBAT_TH_LOW) || (tbl->dis_flag & DC_DIS_BY_PRIORITY)) ?
