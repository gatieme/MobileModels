...
static int sc_set_disable_func(unsigned int val)
{
	...
	g_sc_di->force_disable = val;
	hwlog_info("working_mode = %d, work_mode = %d\n", g_sc_di->working_mode, work_mode);
...
static int sc_get_disable_func(unsigned int *val)
{
	if (!g_sc_di || !val) {
...
{
	return dc_sysfs_set_iin_thermal(SC_MODE, (int)val);
}
...
static int sc_get_iin_limit(unsigned int *val)
{
	...
	dc_sysfs_get_iin_thermal_limit(SC_MODE, di->cur_mode, val);
...
static int sc_set_iin_thermal(unsigned int index, unsigned int iin_thermal_value)
{
	...
	return dc_sysfs_set_iin_thermal_array(SC_MODE, index, iin_thermal_value);
}
...
static int sc_set_iin_thermal_all(unsigned int value)
{
	...
	for (i = DC_CHANNEL_TYPE_BEGIN; i < DC_CHANNEL_TYPE_END; i++) {
		if (dc_sysfs_set_iin_thermal_array(SC_MODE, i, value))
			return -EPERM;
	}
	return 0;
}
...
static int sc_get_rt_test_prot_type(void)
{
	struct direct_charge_device *di = g_sc_di;
...
static int sc_get_ibus(int *ibus)
{
	...
	ret = dcm_get_ic_ibus(SC_MODE, CHARGE_MULTI_IC, ibus);
	if (ret || ibus < 0)
		return -EPERM;
	return 0;
}
...
static struct power_if_ops sc_if_ops = {
	...
	.set_disable_func = sc_set_disable_func,
	.get_disable_func = sc_get_disable_func,
	.type_name = "sc",
};
...
static void sc_fault_work(struct work_struct *work)
{
	...
	/* v700 vdrop min is easy to mistouch, need exception handling */
	if (di->charge_fault == POWER_NE_DC_FAULT_V700_VDROPMIN) {
		dc_pmode_disable_mode(DC_DIS_BY_IBAT_TH_LOW, SC_MODE);
		di->scp_stop_charging_flag_info = 1;
	} else {
		dc_ctrl_fill_eh_buf_and_set_err_flag_true(DC_EH_HAPPEN_SC_FAULT, reg_info);
	}
	...
	case POWER_NE_DC_FAULT_INA231:
		...
		dcm_enable_ic(SC_MODE, di->cur_mode, DC_IC_DISABLE);
...
static void sc_init_parameters(struct direct_charge_device *di)
{
	...
	dc_adpt_set_mode_info(SC_MODE, ADAP_MAX_ISET, di->iin_thermal_default);
...
static int sc_probe(struct platform_device *pdev)
{
	...
	dc_cable_init(di->dev, SC_MODE);
	dc_ctrl_init(di->dev, SC_MODE);
	dc_adpt_init(di->dev, SC_MODE);
	...
	dc_comm_set_mode_ratio(SC_MODE, di->dc_volt_ratio);
	g_sc_di = di;
	direct_charge_set_di(di);
	dc_sysfs_init(di->dev, SC_MODE);

	di->charging_wq = create_singlethread_workqueue("sc_charging_wq");
	di->kick_wtd_wq = create_singlethread_workqueue("sc_wtd_wq");
	di->charging_lock = power_wakeup_source_register(di->dev, "sc_wakelock");
	dc_init_work(SC_MODE);
...
fail_create_link:
	dc_sysfs_remove(di->dev, SC_MODE);
	power_wakeup_source_unregister(di->charging_lock);
fail_free_mem:
	dc_cable_remove(di->dev, SC_MODE);
	dc_ctrl_remove(di->dev, SC_MODE);
	dc_adpt_remove(di->dev, SC_MODE);
