	...
	u8 info_no, u8 dev_no)
{
	...
	ret = memcpy_s(di->dev_info[dev_no].key_info, sizeof(struct wireless_acc_key_info) * info_no,
		key_info, sizeof(struct wireless_acc_key_info) * info_no);
	if (ret != EOK) {
		hwlog_err("memcpy_s err\n");
		return;
	}
