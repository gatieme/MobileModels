...
{
	if (enable && dcm_init_ic(di->dc_ratio, type)) {
		di->ic_data.multi_ic_err_cnt++;
		return -EINVAL;
	}
	if (enable && dcm_init_batinfo(di->dc_ratio, type)) {
		di->ic_data.multi_ic_err_cnt++;
		return -EINVAL;
	}
	if (dcm_enable_ic(di->dc_ratio, type, enable)) {
		di->ic_data.multi_ic_err_cnt++;
		return -EINVAL;
	}
...
static int wldc_ic_check_multi_ic_current(struct wldc_dev_info *di)
{
	...
	while (count) {
		if (wldc_get_charge_stage() == WLDC_STAGE_STOP_CHARGING)
			return -EINVAL;
		if (dcm_get_ic_ibus(di->dc_ratio, CHARGE_IC_MAIN, &main_ibus))
			return -EINVAL;
		if (dcm_get_ic_ibus(di->dc_ratio, CHARGE_IC_AUX, &aux_ibus))
			return -EINVAL;
		hwlog_info("[check_multi_ic_current] main ibus=%d,aux_ibus=%d\n",
			main_ibus, aux_ibus);
		if ((main_ibus > WLDC_OPEN_PATH_IOUT_MIN / di->dc_ratio) &&
			(aux_ibus > WLDC_OPEN_PATH_IOUT_MIN / di->dc_ratio))
			return 0;
...
static bool wldc_ic_is_need_multipath(struct wldc_dev_info *di)
{
	...
	/* reference to definition of struct wldc_ic_ibus_para */
	if (ibus < (di->ic_data.ibus_para.hth * SC_DEFAULT_VOLT_RATIO /
		di->dc_ratio)) {
...
{
	if (dcm_enable_ic(di->dc_ratio, CHARGE_IC_AUX, DC_IC_DISABLE)) {
...
static bool wldc_ic_is_need_singlepath(struct wldc_dev_info *di)
{
	...
	if (dcm_get_ic_ibus(di->dc_ratio, CHARGE_MULTI_IC, &ibus))
		return false;

	/* reference to definition of struct wldc_ic_ibus_para */
	if (ibus > (di->ic_data.ibus_para.lth * SC_DEFAULT_VOLT_RATIO /
		di->dc_ratio)) {
...
void wldc_ic_para_reset_in_exit(struct wldc_dev_info *di)
{
	if (!di)
		return;
