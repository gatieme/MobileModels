...
#define WLRX_ACC_SC_ERR_TX_NUM_DEF     0
...
enum wlrx_acc_tx_vpa_lim_cfg {
	WLRX_ACC_TX_VPA_LIM_BEGIN = 0,
	WLRX_ACC_TX_VPA_LIM_TX_TYPE = WLRX_ACC_TX_VPA_LIM_BEGIN,
	WLRX_ACC_TX_VPA_MAX,
	WLRX_ACC_TX_VPK,
	WLRX_ACC_TX_VPA_LIM_END,
};
...
struct wlrx_acc_tx_vpa_lim {
	u32 tx_type;
	u32 vpa_max; /* default tx vpa max */
	u32 vpk; /* default tx vpk */
};
...
struct wlrx_acc_dts {
	...
	struct wlrx_acc_tx_vpa_lim *tx_vpa_lim;
...
bool wlrx_is_sc_err_tx(unsigned int drv_type)
{
	unsigned int i;
	struct wlrx_acc_dev *di = wlrx_acc_get_di(drv_type);
	...
	for (i = 0; i < di->dts->sc_err_tx_len; i++) {
		if (di->tx_type == di->dts->sc_err_tx[i])
			return true;
	}
...
static struct wlrx_acc_tx_vpa_lim *wlrx_acc_get_tx_vpa_lim(struct wlrx_acc_dev *di)
{
	int i;

	if (!di->dts || !di->dts->tx_vpa_lim)
		return NULL;

	for (i = 0; i < di->dts->tx_vpa_lim_len; i++) {
		if (di->dts->tx_vpa_lim[i].tx_type == di->tx_type)
			return &di->dts->tx_vpa_lim[i];
	}
...
static void wlrx_acc_send_vpa_top(struct wlrx_acc_dev *di, unsigned int ic_type)
{
	struct wlrx_acc_tx_vpa_lim *tx_vpa_lim = wlrx_acc_get_tx_vpa_lim(di);

	if (!tx_vpa_lim)
		return;

	if (tx_vpa_lim->vpa_max > 0)
		(void)wireless_send_tx_vpa_lim(ic_type, WIRELESS_PROTOCOL_QI, tx_vpa_lim->vpa_max, WLRX_ACC_TX_VPA_MAX);

	if (tx_vpa_lim->vpk > 0)
		(void)wireless_send_tx_vpa_lim(ic_type, WIRELESS_PROTOCOL_QI, tx_vpa_lim->vpk, WLRX_ACC_TX_VPK);

	hwlog_info("[send_vpa_top] vpa_max=%d, vpk=%d\n", tx_vpa_lim->vpa_max, tx_vpa_lim->vpk);
}
...
static void wlrx_acc_cap_post_handler(unsigned int drv_type)
{
	...
	wlrx_acc_send_vpa_top(di, ic_type);
}
...
static void wlrx_acc_parse_tx_vpa_lim(const struct device_node *np,
	struct wlrx_acc_dts *dts)
{
	int len, i;
	int ret;
	...
	len = power_dts_read_u32_count(power_dts_tag(HWLOG_TAG), np,
		"tx_vpa_lim", WLACC_TX_ERROR, WLRX_ACC_TX_VPA_LIM_END);
	if (len <= 0)
		return;

	row = len / WLRX_ACC_TX_VPA_LIM_END;
	dts->tx_vpa_lim = kcalloc(row, sizeof(struct wlrx_acc_tx_vpa_lim), GFP_KERNEL);
	if (!dts->tx_vpa_lim)
		goto kcalloc_fail;

	ret = power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"tx_vpa_lim", (u32 *)dts->tx_vpa_lim, len);
	if (ret)
		goto parse_tx_vpa_lim_fail;

	for (i = 0; i < row; i++)
		hwlog_info("[parse_tx_vpa_lim] tx_vpa_lim[%d]: %u %u %u\n",
			i, dts->tx_vpa_lim[i].tx_type, dts->tx_vpa_lim[i].vpa_max,
			dts->tx_vpa_lim[i].vpk);
	dts->tx_vpa_lim_len = row;
	return;

parse_tx_vpa_lim_fail:
	kfree(dts->tx_vpa_lim);
	dts->tx_vpa_lim = NULL;
kcalloc_fail:
	hwlog_err("parse_tx_vpa_lim: failed\n");
}
...
static void wlrx_fod_parse_sc_err_tx_dts(const struct device_node *np,
	struct wlrx_acc_dts *dts)
{
	int i, len;

	len = of_property_count_u8_elems(np, "sc_err_tx");
	if ((len <= 0) || (len >= WLACC_TX_ERROR))
		goto default_value;

	dts->sc_err_tx = kzalloc(len * sizeof(*(dts->sc_err_tx)), GFP_KERNEL);
	if (!dts->sc_err_tx)
		return;

	if (power_dts_read_u8_array(power_dts_tag(HWLOG_TAG), np,
		"sc_err_tx", dts->sc_err_tx, len)) {
		kfree(dts->sc_err_tx);
		goto default_value;
	}

	dts->sc_err_tx_len = len;
	goto print;

default_value:
	dts->sc_err_tx_len = WLRX_ACC_SC_ERR_TX_NUM_DEF;
print:
	for (i = 0; i < dts->sc_err_tx_len; i++)
		hwlog_info("sc_err_tx[%d]=0x%x\n", i, dts->sc_err_tx[i]);
}
...
static int wlrx_acc_parse_dts(const struct device_node *np, struct wlrx_acc_dev *di)
{
	...
	wlrx_fod_parse_sc_err_tx_dts(np, di->dts);
	wlrx_acc_parse_tx_ilim(np, di->dts);
	wlrx_acc_parse_tx_vpa_lim(np, di->dts);
