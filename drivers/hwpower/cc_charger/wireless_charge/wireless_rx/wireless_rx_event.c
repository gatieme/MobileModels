...
static void wlrx_evt_handle_rx_opp_evt(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_opp_evt] drv_type:%u\n", di->drv_type);
	if (wlrx_get_charge_stage() < WLRX_STAGE_REGULATION)
		return;

	wlrx_dsm_report(di->drv_type, POWER_DSM_ERROR_WIRELESS_RX_OPP,
		dsm_buff, sizeof(dsm_buff));
}
...
static void wlrx_evt_rxic_work(struct work_struct *work)
{
	...
	case POWER_NE_WLRX_OTP:
		...
		break;
	case POWER_NE_WLRX_OPP_H:
		wlrx_evt_handle_rx_opp_evt(di);
		break;
	default:
		break;
	}
