...
unsigned int wltx_cap_get_exp_id(unsigned int drv_type)
{
	struct wltx_cap_dev *di = wltx_cap_get_di(drv_type);
...
static bool wltx_cap_need_revise_imax(void)
{
	enum wltx_pwr_type pwr_type;
	enum wltx_pwr_src pwr_src;

	pwr_type = wltx_get_pwr_type();
	pwr_src = wltx_get_cur_pwr_src();
	if ((pwr_type == WL_TX_PWR_5VBST_VBUS_OTG_CP) &&
		!strstr(wltx_get_pwr_src_name(pwr_src), "CP"))
		return true;

	if ((pwr_type == WL_TX_PWR_VBUS_OTG2) &&
		(pwr_src == PWR_SRC_VBUS))
		return true;

	if ((pwr_type == WL_TX_PWR_5VBST_VBUS_OTG2) &&
		((pwr_src == PWR_SRC_VBUS) || (pwr_src == PWR_SRC_5VBST_WLSIN)))
		return true;

	if ((pwr_type == WL_TX_PWR_5VBST_OTG_RVSSC_CP) &&
		(pwr_src == PWR_SRC_5VBST))
		return true;
...
{
	if (wltx_cap_need_revise_imax())
		imax = 500; /* 500mA imax for 5vbst/vbus/otg,otg/vbus2 pwr_src */
...
static void wltx_cap_check_high_pwr_id(unsigned int drv_type)
{
	...
	if (soc < dts->high_pwr_soc)
		goto set_dflt_id;
	if (dts->pwr_type == WL_TX_PWR_VBUS_RVSSC4_CP) {
		wltx_cap_set_exp_id(drv_type, WLTX_HIGH_PWR2_CAP);
		return;
	}
