...
int wltx_ic_get_curfod_iin(unsigned int type, u16 *iin)
{
	...
	if (ops && ops->get_curfod_iin)
		return ops->get_curfod_iin(iin, ops->dev_data);
...
int wltx_ic_set_min_duty(unsigned int type, int min_duty)
{
	...
	if (ops && ops->set_min_duty)
		return ops->set_min_duty(min_duty, ops->dev_data);
...
int wltx_ic_set_max_duty(unsigned int type, int max_duty)
{
	...
	if (ops && ops->set_max_duty)
		return ops->set_max_duty(max_duty, ops->dev_data);
...
int wltx_ic_set_ploss(unsigned int type, u8 rx_type)
{
	...
	if (ops && ops->set_ploss)
		return ops->set_ploss(rx_type, ops->dev_data);
...
int wltx_ic_set_open_loop(unsigned int type, bool enable)
{
	...
	if (ops && ops->set_open_loop)
		return ops->set_open_loop(enable, ops->dev_data);
...
void wltx_ic_prevfod_check(unsigned int type, int flag)
{
	...
	if (ops && ops->prevfod_chk)
		return ops->prevfod_chk(flag, ops->dev_data);
}
...
void wltx_ic_get_dbg_info(unsigned int type, char *info, int len)
{
	...
	if (ops && ops->get_dbg_info)
		return ops->get_dbg_info(info, len, ops->dev_data);
}