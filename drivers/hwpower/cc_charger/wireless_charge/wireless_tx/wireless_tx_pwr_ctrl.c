...
static struct {
	...
	{ PWR_SW_BY_SOC_HIGH,  "PWR_SW_BY_SOC_HIGH" }
};
...
const struct wltx_pwr_attr g_attr6[] = {
	...
	{ PWR_SW_BY_OTG_ON,   false, PWR_SRC_OTG },
	{ PWR_SW_BY_OTG_OFF,  false, PWR_SRC_OTG },
	{ PWR_SW_BY_DC_DONE,  true,  PWR_SRC_VBUS2 }
};

/* pwr_type: 5VBST_VBUS_OTG2 */
const struct wltx_pwr_attr g_attr7[] = {
	{ PWR_SW_BY_VBUS_ON,  true,  PWR_SRC_VBUS },
	{ PWR_SW_BY_VBUS_OFF, true,  PWR_SRC_OTG },
	{ PWR_SW_BY_OTG_ON,   false, PWR_SRC_5VBST_WLSIN },
	{ PWR_SW_BY_OTG_OFF,  false, PWR_SRC_OTG },
	{ PWR_SW_BY_DC_DONE,  true,  PWR_SRC_VBUS2 }
};

/* pwr_type: 5VBST_OTG_RVSSC */
const struct wltx_pwr_attr g_attr8[] = {
	{ PWR_SW_BY_VBUS_ON,  true,  PWR_SRC_5VBST },
	{ PWR_SW_BY_VBUS_OFF, true,  PWR_SRC_OTG },
	{ PWR_SW_BY_OTG_ON,   false, PWR_SRC_5VBST },
	{ PWR_SW_BY_OTG_OFF,  true,  PWR_SRC_OTG },
	{ PWR_SW_BY_SOC_HIGH, true,  PWR_SRC_RVS_SC_CP }
};

/* pwr_type: VBUS_RVSSC4_CP */
const struct wltx_pwr_attr g_attr9[] = {
	{ PWR_SW_BY_VBUS_ON,  true,  PWR_SRC_5VBST },
	{ PWR_SW_BY_VBUS_OFF, true,  PWR_SRC_RVS_SC_CP },
	{ PWR_SW_BY_OTG_ON,   false, PWR_SRC_5VBST },
	{ PWR_SW_BY_OTG_OFF,  true,  PWR_SRC_RVS_SC_CP },
	{ PWR_SW_BY_SOC_HIGH,  true, PWR_SRC_RVS_SC4_CP }
};
...
static struct {
	...
	{ WL_TX_PWR_VBUS_OTG2, "VBUS_OTG2", g_attr6, ARRAY_SIZE(g_attr6) },
	{ WL_TX_PWR_5VBST_VBUS_OTG2, "5VBST_VBUS_OTG2", g_attr7, ARRAY_SIZE(g_attr7) },
	{ WL_TX_PWR_5VBST_OTG_RVSSC_CP, "5VBST_OTG_RVSSC_CP", g_attr8, ARRAY_SIZE(g_attr8) },
	{ WL_TX_PWR_VBUS_RVSSC4_CP, "VBUS_RVSSC4_CP", g_attr9, ARRAY_SIZE(g_attr9) }
};
...
static bool wltx_vbus_5vbst_exclusive(enum wltx_pwr_sw_scene scn, enum wltx_pwr_src pwr_src)
{
	struct wltx_dts *dts = wltx_get_dts();

	if (!dts || !dts->vbus_5vbst_exclusive)
		return false;
	if ((scn != PWR_SW_BY_VBUS_ON) && (scn != PWR_SW_BY_DC_DONE))
		return false;
	if ((pwr_src != PWR_SRC_5VBST) && (pwr_src != PWR_SRC_5VBST_WLSIN))
		return false;

	return true;
}

static bool wltx_pwr_scn_src_exclusive(enum wltx_pwr_sw_scene scn, enum wltx_pwr_src cur_src)
{
	return wltx_vbus_5vbst_exclusive(scn, cur_src);
}
	...
	enum wltx_pwr_type type, enum wltx_pwr_sw_scene scn)
{
	...
	enum wltx_pwr_src specified_pwr_src = wltx_specify_pwr_src();

	if (!attr)
		return PWR_SRC_NA;

	if ((specified_pwr_src != PWR_SRC_NULL) &&
		!wltx_pwr_scn_src_exclusive(scn, specified_pwr_src))
		return specified_pwr_src;
	...
	enum wltx_pwr_sw_scene scn, enum wltx_pwr_src cur_src)
{
	...
	if ((cur_src == PWR_SRC_NULL) || wltx_pwr_scn_src_exclusive(scn, cur_src))
		return true;

	if (wltx_need_specify_pwr_src() && wltx_pwr_scn_src_exclusive(scn,
		wltx_specify_pwr_src()))
		return true;

	if (!dts->vbus_5vbst_exclusive && wltx_need_specify_pwr_src())
		return false;
...
static void wltx_check_dc_done(struct wltx_dts *dts)
{
	int soc;
...
static void wltx_update_dc_done_scn(struct wltx_dev_info *di)
{
	struct wltx_dts *dts = wltx_get_dts();

	if (!dts || ((dts->pwr_type != WL_TX_PWR_5VBST_VBUS_OTG_CP) &&
		(dts->pwr_type != WL_TX_PWR_VBUS_OTG2) &&
...
static void wltx_update_soc_high_scn(struct wltx_dev_info *di)
{
	struct wltx_dts *dts = wltx_get_dts();
	unsigned int id;

	if (!dts || ((dts->pwr_type != WL_TX_PWR_5VBST_OTG_RVSSC_CP) &&
	(dts->pwr_type != WL_TX_PWR_VBUS_RVSSC4_CP)))
		return;

	if ((di->cur_pwr_sw_scn != PWR_SW_BY_VBUS_OFF) &&
		(di->cur_pwr_sw_scn != PWR_SW_BY_OTG_OFF) &&
		(di->cur_pwr_sw_scn != PWR_SW_BY_SOC_HIGH))
		return;

	id = wltx_cap_get_exp_id(WLTRX_DRV_MAIN);
	if ((id == WLTX_HIGH_PWR_CAP) || (id == WLTX_HIGH_PWR2_CAP))
		di->cur_pwr_sw_scn = PWR_SW_BY_SOC_HIGH;
	else
...
{
	wltx_update_dc_done_scn(di);
	wltx_update_soc_high_scn(di);
}
...
int wltx_enable_pwr_supply(void)
{
	...
	if ((wltx_get_client() != WLTX_CLIENT_LIGHTSTRAP) &&
		(wltx_get_client() != WLTX_CLIENT_COOLINGCASE))
...
static void wltx_monitor_dc_work(struct work_struct *work)
{
	...
	wltx_check_dc_done(dts);
	if (g_pwr_ctrl_di.dc_done && direct_charge_get_stop_charging_complete_flag() &&
		(di->cur_pwr_sw_scn != PWR_SW_BY_DC_DONE))
...
static void wltx_set_dc_charger_type(unsigned int drv_type)
{
	...
	if (!dts || !dts->high_pwr_th_cfg)
		goto low_pwr_charger;
	...
	if (adp_vmax >= dts->high_pwr_th_cfg->hi_pwr2_vth) {
		adp_imax = dc_get_adapter_max_current(SC_MODE, dts->high_pwr_th_cfg->hi_pwr2_vth);
		hwlog_info("[set_dc_charger_type] scp charger, imax=%d\n", adp_imax);
		if (adp_imax >= dts->high_pwr_th_cfg->hi_pwr2_ith) {
			wltx_reset_dc_charger_type(drv_type, WLTX_SC_HI_PWR2_CHARGER);
			return;
		}
	}
	if (adp_vmax >= dts->high_pwr_th_cfg->hi_pwr_vth) {
		adp_imax = dc_get_adapter_max_current(SC_MODE, dts->high_pwr_th_cfg->hi_pwr_vth);
		hwlog_info("[set_dc_charger_type] scp charger, imax=%d\n", adp_imax);
		if (adp_imax >= dts->high_pwr_th_cfg->hi_pwr_ith) {
