...
{
	return wltx_is_half_volt(cur_vset, vout) ? WLTX_PING_HALF_BRIDGE :
		WLTX_PING_FULL_BRIDGE;
}

static enum wltx_bridge_type wltx_select_pt_bridge(int cur_vset, int vout)
{
	if (wltx_get_cur_pwr_src() == PWR_SRC_RVS_SC4_CP)
		return vout >= WLTRX_VOLT_9V ? WLTX_PT_FULL_BRIDGE : WLTX_PT_HALF_BRIDGE;

	return wltx_is_half_volt(cur_vset, vout) ?
		WLTX_PT_HALF_BRIDGE : WLTX_PT_FULL_BRIDGE;
}
...
static void wltx_config_adap_vout(struct wltx_dev_info *di, int vout, enum wltx_bridge_type type)
{
	int adap_vout;
	const char *src = NULL;
	...
	if (di->cur_pwr_src == PWR_SRC_VBUS2)
		wltx_set_bridge(adap_vout, type);
	...
	enum wltx_stage stage, int vout, bool force)
{
	...
	if ((wltx_get_cur_pwr_src() == PWR_SRC_VBUS_CP) ||
		(wltx_get_cur_pwr_src() == PWR_SRC_VBUS2))
		wltx_config_adap_vout(di, vout, type);
