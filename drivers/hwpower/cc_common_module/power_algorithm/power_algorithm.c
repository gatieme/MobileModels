...
#define POWER_ALGORITHM_MACTH_MAX_LEN         31
...
int power_get_smooth_compensation_value(const struct smooth_comp_data *data)
{
	...
	if ((delta_comp < 0) && (delta_raw > 0))
		...
		current_comp = data->last_comp;
	else if ((current_comp - current_raw <= 0) && (abs(delta_comp) > abs(delta_raw)))
...
/*
 * return y according to : y = ax +b, a = (y1 - y0) / (x1 - x0)
 * if x1 = x0, return y1.
 * the caller should ensure (y1 - y0) * (x - x0) not overflow.
 */
...
/*
 * Returns the value of ceil(divident/divisor)
 */
...
/*
 * Returns whether the two strings are exactly the same.
 */
bool power_strsame(const char *s1, const char *s2)
{
	int len1, len2;
	...
	len1 = strlen(s1);
	len2 = strlen(s2);
	return (len1 == len2) && !strncmp(s1, s2, len1);
}

bool power_factor_matched(u32 src_factor, u32 target_factor)
{
	if (target_factor == 0)
		return true;
 
 	if (target_factor & BIT(POWER_ALGORITHM_MACTH_MAX_LEN)) {
 		target_factor &= ~BIT(POWER_ALGORITHM_MACTH_MAX_LEN);
 		return (src_factor & target_factor) == target_factor;
 	}
 
 	return (src_factor & target_factor) > 0;
}

/*
 * split string with delimiter to int[]
 * caller have to make sure that max_num <= len (res)
 */
int power_strsep_to_array(const char *str, int *res,
	const char *delimiter, int max_num, int *num)
{
	char *token = NULL;
	char *ss = NULL;
	int i = 0;
	...
	if (!str || !res || !delimiter || !num)
		return -EINVAL;

	len = strlen(str);
	ss = (char *)kzalloc(len + 1, GFP_KERNEL);
	if (!ss)
		return -ENOMEM;

	if (strncpy_s(ss, len + 1, str, len))
		return -EPERM;

	while (((token = strsep(&ss, delimiter)) != NULL) && (i < max_num)) {
		hwlog_info("substring[%d]:%s\n", i, token);
		if (kstrtoint(token, 0, &data))
			return -EINVAL;
		res[i] = data;
		i++;
	}
