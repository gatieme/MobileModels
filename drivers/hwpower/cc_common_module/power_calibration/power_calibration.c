...
static struct power_sysfs_attr_info power_cali_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE, wltrx_curr_ic_type),
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_WLTRX_CURR_INIT, wltrx_curr_init),
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_WLTRX_CURR_Y1, wltrx_curr_y1),
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_WLTRX_CURR_Y2, wltrx_curr_y2),
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_WLTRX_CURR_CAL, wltrx_curr_cal),
	power_sysfs_attr_ro(power_cali, 0440, POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA, wltrx_curr_get_para),
	power_sysfs_attr_ro(power_cali, 0440, POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT, wltrx_curr_support),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	case POWER_CALI_SYSFS_COUL_MODE:
		...
		return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d\n", value);
	case POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT;
		if (power_cali_get_data("wltrx_curr", offset, &value))
			return -EINVAL;
		return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d\n", value);
	case POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT;
		ret = power_cali_get_data("wltrx_curr", offset, &a);
		offset = POWER_CALI_SYSFS_WLTRX_CURR_GET_PARA - POWER_CALI_SYSFS_WLTRX_CURR_SUPPORT + 1;
		ret += power_cali_get_data("wltrx_curr", offset, &b);
		if (ret)
			return -EINVAL;
		return scnprintf(buf, POWER_CALI_WR_BUF_SIZE, "%d %d\n", a, b);
	default:
		return 0;
	}
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf_s(buf, "%s %d", user, POWER_CALI_WR_BUF_SIZE, &value) != 2) {
	...
	hwlog_info("set: user=%s, name=%d, value=%d\n",
	...
	case POWER_CALI_SYSFS_COUL_SAVE:
		if (power_cali_save_data("coul") < 0)
			return -EINVAL;
		break;
	case POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE;
		ret = power_cali_set_data("wltrx_curr", offset, value);
		break;
	case POWER_CALI_SYSFS_WLTRX_CURR_INIT:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_INIT - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE;
		ret = power_cali_set_data("wltrx_curr", offset, value);
		break;
	case POWER_CALI_SYSFS_WLTRX_CURR_Y1:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_Y1 - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE;
		ret = power_cali_set_data("wltrx_curr", offset, value);
		break;
	case POWER_CALI_SYSFS_WLTRX_CURR_Y2:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_Y2 - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE;
		ret = power_cali_set_data("wltrx_curr", offset, value);
		break;
	case POWER_CALI_SYSFS_WLTRX_CURR_CAL:
		offset = POWER_CALI_SYSFS_WLTRX_CURR_CAL - POWER_CALI_SYSFS_WLTRX_CURR_IC_TYPE;
		ret = power_cali_set_data("wltrx_curr", offset, value);
		break;
	default:
		break;
