...
int power_dts_read_u16(const char *tag, const struct device_node *np,
	const char *prop, u16 *data, u16 default_value)
{
	if (!tag || !np || !prop || !data)
		return -EINVAL;

	if (of_property_read_u16(np, prop, data)) {
		*data = default_value;
		return -EINVAL;
	}
...
int power_dts_read_u32_index(const char *tag, const struct device_node *np,
	const char *prop, int index, u32 *data)
{
	...
	power_dts_info("%s: prop %s[%d]=%u\n", tag, prop, index, *data);
	return 0;
}

int power_dts_read_u32_index_dflt(const char *tag, const struct device_node *np,
	const char *prop, int index, u32 *data, u32 default_value)
{
	if (!tag || !np || !prop || !data)
		return -EINVAL;
...
int power_dts_read_dynamic_intstr_array(const char *tag,
	const struct device_node *np, const char *prop, int **p, int in_col, int *out_row)
{
	int i, len;
	const char *tmp_str = NULL;

	if (!tag || !np || !prop || !p || !out_row || (in_col <= 0))
		return -EINVAL;

	len = of_property_count_strings(np, prop);
	if ((len <= 0) || ((u32)len % (u32)in_col != 0))
		return -EINVAL;

	*p = (int *)kzalloc(len * sizeof(int), GFP_KERNEL);
	if (!*p)
		return -ENOMEM;

	for (i = 0; i < len; i++) {
		if (of_property_read_string_index(np, prop, i, &tmp_str) ||
			kstrtoint(tmp_str, 0, &(*p)[i])) {
			kfree(*p);
			return -EINVAL;
		}
	}
...
int power_dts_read_str2int_index(const char *tag, const struct device_node *np,
	const char *prop, int index, int *data)
{
	const char *tmp_string = NULL;
	...
	if (of_property_read_string_index(np, prop, index, &tmp_string))
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(power_dts_read_str2int_index);
...
int power_dts_read_u32_index_compatible(const char *tag, const char *comp,
	const char *prop, int index, u32 *data)
{
	struct device_node *np = power_dts_get_compatible(comp);
	...
	return power_dts_read_u32_index(tag, np, prop, index, data);
}
