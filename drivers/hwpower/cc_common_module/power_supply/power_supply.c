...
EXPORT_SYMBOL_GPL(power_supply_ops_register);
...
static int power_supply_get_default_online_prop(int type)
{
	switch (type) {
	case POWER_SUPPLY_PROP_USB:
		return POWER_SUPPLY_USB_DEFAULT_ONLINE;
	default:
...
static int power_supply_get_online_property(int type)
{
	...
	if (!l_ops || !l_ops->get_online_prop)
		return power_supply_get_default_online_prop(type);
...
static int power_supply_get_default_voltage_now_prop(int type)
{
	switch (type) {
	case POWER_SUPPLY_PROP_USB:
		return 0;
...
static int power_supply_get_voltage_now_property(int type)
{
	...
	if (!l_ops || !l_ops->get_voltage_now_prop)
		return power_supply_get_default_voltage_now_prop(type);

	return l_ops->get_voltage_now_prop();
}

static int power_supply_get_default_voltage_max_prop(int type)
{
	switch (type) {
	case POWER_SUPPLY_PROP_USB:
		return 0;
...
static int power_supply_get_voltage_max_property(int type)
{
	...
	if (!l_ops || !l_ops->get_voltage_max_prop)
		return power_supply_get_default_voltage_max_prop(type);
...
static int power_supply_get_current_max_property(int type)
{
	struct power_supply_ops *l_ops = power_supply_get_ops(type);

	if (!l_ops || !l_ops->get_current_max_prop)
		return POWER_SUPPLY_DEFAULT_CURRENT_MAX;

	return l_ops->get_current_max_prop();
}
...
static int power_supply_get_input_current_limit_property(int type)
{
	struct power_supply_ops *l_ops = power_supply_get_ops(type);

	if (!l_ops || !l_ops->get_input_current_limit_prop)
		return POWER_SUPPLY_DEFAULT_IBUS_LIMIT;

	return l_ops->get_input_current_limit_prop();
}

static int power_supply_set_input_current_limit_property(int type, int value)
{
	struct power_supply_ops *l_ops = power_supply_get_ops(type);

	if (!l_ops || !l_ops->set_input_current_limit_prop)
		return -EPERM;

	return l_ops->set_input_current_limit_prop(value);
}

static int power_supply_get_usb_type_property(int type)
{
	struct power_supply_ops *l_ops = power_supply_get_ops(type);

	if (!l_ops || !l_ops->get_usb_type_prop)
		return 0;

	return l_ops->get_usb_type_prop();
}
...
static enum power_supply_usb_type power_supply_usb_supported_types[] = {
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = power_supply_get_current_max_property(type);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		val->intval = power_supply_get_input_current_limit_property(type);
		break;
	case POWER_SUPPLY_PROP_USB_TYPE:
		val->intval = power_supply_get_usb_type_property(type);
		break;
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = power_supply_get_temp_property(type);
		break;
...
static int power_supply_usb_set_property(struct power_supply *psy,
	enum power_supply_property psp, const union power_supply_propval *val)
{
	int type = POWER_SUPPLY_PROP_USB;
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		return power_supply_set_input_current_limit_property(type, val->intval);
	default:
		return -EINVAL;
	}
}

static int power_supply_usb_is_writeable(struct power_supply *psy, enum power_supply_property prop)
{
	switch (prop) {
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		return 1;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static struct power_supply_desc power_supply_usb_desc = {
	.name                  = "usb",
	...
	.set_property          = power_supply_usb_set_property,
	.usb_types             = power_supply_usb_supported_types,
	.num_usb_types         = ARRAY_SIZE(power_supply_usb_supported_types),
	.property_is_writeable = power_supply_usb_is_writeable,
};
#else
...
void power_supply_update_usb_psy_type(enum power_supply_type type)
{
	power_supply_usb_desc.type = type;
}
EXPORT_SYMBOL_GPL(power_supply_update_usb_psy_type);
