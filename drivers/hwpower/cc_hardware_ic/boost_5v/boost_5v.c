...
static int boost_5v_output(struct boost_5v_dev *l_dev, int value)
{
	...
	if (l_dev->use_ext_pmic) {
		...
		if (value == BOOST_5V_ENABLE) {
			buckboost_set_enable(value, BBST_USER_BOOST_5V, l_dev->buckboost_index);
			buckboost_set_vout(l_dev->buckboost_vol, BBST_USER_BOOST_5V, l_dev->buckboost_index);
		} else {
			buckboost_set_enable(value, BBST_USER_BOOST_5V, l_dev->buckboost_index);
		}
	} else {
		gpio_set_value(l_dev->gpio_no, value);
		hwlog_info("gpio_%d %s now\n", l_dev->gpio_no,
			gpio_get_value(l_dev->gpio_no) ? "high" : "low");
	}
...
static int boost_5v_parse_dts(struct boost_5v_dev *l_dev, struct device_node *np)
{
	...
	if (l_dev->use_buckboost) {
		...
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"buckboost_index", &l_dev->buckboost_index, BBST_INDEX_DEFAULT);
	}

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"boost_5v_always_on", &l_dev->always_on, 0);

	if (!power_gpio_config_output(np,
		"gpio_5v_boost", "gpio_5v_boost", &l_dev->gpio_no, 0))
		l_dev->use_gpio = 1;

	if (l_dev->use_buckboost && l_dev->use_gpio && (!power_gpio_config_input(np,
		"gpio_boost_type", "gpio_boost_type", &l_dev->gpio_boost_type))) {
		gpio_val = gpio_get_value(l_dev->gpio_boost_type);
		l_dev->use_buckboost = gpio_val ? 0 : 1;
		l_dev->use_gpio = gpio_val ? 1 : 0;
	} else {
		if (l_dev->use_buckboost)
			l_dev->use_gpio = 0;
		else if (!l_dev->use_gpio)
			return -EPERM;
	}
	if (!l_dev->use_gpio && gpio_is_valid(l_dev->gpio_no))
...
static int boost_5v_probe(struct platform_device *pdev)
{
	...
	if (l_dev->always_on)
		boost_5v_enable(BOOST_5V_ENABLE, BOOST_CTRL_ALWAYS_ON);
...
static int boost_5v_remove(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(l_dev->gpio_boost_type))
		gpio_free(l_dev->gpio_boost_type);
	if (l_dev->use_gpio) {
...
static void boost_5v_shutdown(struct platform_device *pdev)
{
	struct boost_5v_dev *l_dev = platform_get_drvdata(pdev);
