...
static void tps63810_ti_get_vout_param(struct tps63810_device_info *di, unsigned int *vout_min,
	unsigned int *vout_max, unsigned int *vout_step)
{
	if (!di->vsel_state) {
...
static void tps63810_rt_get_vout_param(struct tps63810_device_info *di, unsigned int *vout_min,
	unsigned int *vout_max, unsigned int *vout_step)
{
	if (!di->vsel_state) {
...
{
	u8 vset;
	unsigned int vout_min = 0;
	unsigned int vout_max = 0;
	unsigned int vout_step = 0;
	...
	case TPS63810_DEVID_SGM:
		tps63810_ti_get_vout_param(di, &vout_min, &vout_max, &vout_step);
		break;
	case TPS63810_DEVID_RT:
		tps63810_rt_get_vout_param(di, &vout_min, &vout_max, &vout_step);
		break;
	default:
		return -EINVAL;
	}
...
static int tps63810_forced_pwm_enable(unsigned int enable, void *dev_data)
{
	int ret;
	u8 reg = 0;
	...
	ret = tps63810_read_byte(di, TPS63810_CONTROL_ADDR, &reg);
	if (ret)
		return ret;

	hwlog_info("before control [%x]=0x%x\n", TPS63810_CONTROL_ADDR, reg);

	ret = tps63810_write_mask(di, TPS63810_CONTROL_ADDR,
		TPS63810_FORCED_PWM_MASK, TPS63810_FORCED_PWM_SHIFT, value);
	if (ret)
		return ret;
	...
	hwlog_info("after control [%x]=0x%x\n", TPS63810_CONTROL_ADDR, reg);
	return 0;
}
...
static struct buck_boost_ops tps63810_ops = {
	...
	.set_pwm_enable = tps63810_forced_pwm_enable,
};
