...
static const struct charge_pump_device_data g_charge_pump_device_data[] = {
	...
	{ CP_DEVICE_ID_SC8562, "sc8562" }, /* only use reverse cp mode */
};
...
int charge_pump_reverse_cp_chip_init(unsigned int type, bool enable)
{
	struct charge_pump_ops *l_ops = charge_pump_get_ops(type);
	...
	return l_ops->rvs_cp_chip_init(l_ops->dev_data, enable);
}

int charge_pump_reverse_cp4_chip_init(unsigned int type, bool enable)
{
	struct charge_pump_ops *l_ops = charge_pump_get_ops(type);

	if (!l_ops || !l_ops->rvs_cp4_chip_init)
		return -ENOTSUPP;

	return l_ops->rvs_cp4_chip_init(l_ops->dev_data, enable);
}
