...
int bq40z50_read_byte(struct bq40z50_device_info *di, u8 reg, u8 *value)
{
	...
	if (di->support_i2c_crc)
		return power_i2c_u8_read_byte_with_crc(di->client, reg, value);
	else
...
int bq40z50_write_byte(struct bq40z50_device_info *di, u8 reg, u8 value)
{
	...
	if (di->support_i2c_crc)
		return power_i2c_u8_write_byte_with_crc(di->client, reg, value);
	else
...
int bq40z50_read_word(struct bq40z50_device_info *di, u8 reg, u16 *data)
{
	...
	if (di->support_i2c_crc)
		ret = power_i2c_u8_read_word_with_crc(di->client, reg, data, false);
	else
...
int bq40z50_write_word(struct bq40z50_device_info *di, u8 reg, u16 data)
{
	...
	mutex_lock(&di->rw_lock);
	if (di->support_i2c_crc)
		ret = power_i2c_u8_write_word_with_crc(di->client, reg, data);
	else
...
int bq40z50_read_block(struct bq40z50_device_info *di, u8 reg, u8 *buf, u8 buf_len)
{
	...
	mutex_lock(&di->rw_lock);
	if (di->support_i2c_crc)
		ret = power_i2c_read_block_with_crc(di->client, cmd, sizeof(cmd), buf, buf_len);
	else
...
int bq40z50_write_block(struct bq40z50_device_info *di, u8 reg, u8 *buf, u8 buf_len)
{
	...
	mutex_lock(&di->rw_lock);
	if (di->support_i2c_crc)
		ret = power_i2c_write_block_with_crc(di->client, cmd, BQ40Z50_CMD_LEN + buf_len);
	else
...
static int bq40z50_get_batt_para(struct bq40z50_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), di->dev->of_node,
		"is_support_crc", (u32 *)&di->support_i2c_crc, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), di->dev->of_node,
		"gauge_upgrade_enable", (u32 *)&di->upgrade_param_dts.gauge_upgrade_enable, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), di->dev->of_node,
		"gauge_version", (u32 *)&di->upgrade_param_dts.gauge_version, 0);
...
static void bq40z50_get_battery_manufac_info(void *dev_data)
{
	struct bq40z50_device_info *di = dev_data;
	u16 data = 0;
	int ret;
	int len;
	char manufac_name[BQ40Z50_BLOCK_LEN_MAX] = {0};

	if (!di)
		return;

	di->manufac_date.year = 0;
	di->manufac_date.month = 0;
	di->manufac_date.day = 0;

	ret = bq40z50_read_word(di, BQ40Z50_REG_MANUFACTURER_DATE, &data);
	if (ret) {
		hwlog_err("read manufac date fail\n");
	} else {
		di->manufac_date.year = (data >> MFDATE_YEAR_OFFSET) + MFDATE_YEAR_BASE;
		di->manufac_date.month = (data >> MFDATE_MONTH_OFFSET) & MFDATE_MONTH_MASK;
		di->manufac_date.day = data & MFDATE_MONTH_MASK;
	}

	ret = memset_s(di->manufac_name, BQ40Z50_BLOCK_LEN_MAX, 0, BQ40Z50_BLOCK_LEN_MAX);
	if (ret) {
		hwlog_err("memset_s manufac_name fail\n");
		return;
	}

	ret = bq40z50_read_block(di, BQ40Z50_REG_MANUFACTURER_NAME,
		manufac_name, BQ40Z50_BLOCK_LEN_MAX);
	if (ret) {
		hwlog_err("read manufac name fail\n");
	} else {
		len = manufac_name[0];
		if (len <= BQ40Z50_BLOCK_LEN_MAX - 1) {
			memcpy_s(di->manufac_name, BQ40Z50_BLOCK_LEN_MAX, &manufac_name[1], len);
			hwlog_info("pack manufac name : %s\n", di->manufac_name);
		}
	}
}
...
static int bq40z50_check_chip(struct bq40z50_device_info *di)
{
	...
	while (retry--) {
		ret = bq40z50_read_block(di, BQ40Z50_REG_DEVICE_NAME, device_name, BQ40Z50_DASTATUS_BLOCK_LEN);
		if (!ret)
			break;
	...
	ret = memset_s(di->device_name, BQ40Z50_BLOCK_LEN_MAX, 0, BQ40Z50_BLOCK_LEN_MAX);
	if (ret) {
		hwlog_err("memset_s device_name fail\n");
		return ret;
	}

	len = device_name[0];
	if (len <= BQ40Z50_DASTATUS_BLOCK_LEN - 1)
		memcpy_s(di->device_name, BQ40Z50_BLOCK_LEN_MAX, &device_name[1], len);

	bq40z50_get_battery_manufac_info(di);
...
static int bq40z50_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if ((di->device_id == BATTERY_FUEL_BQ40Z50) || (di->device_id == BATTERY_FUEL_SH366006))
		bq40z50_creat_param_upgrade_delay_work(di);
...
static int bq40z50_remove(struct i2c_client *client)
{
	...
	if ((di->device_id == BATTERY_FUEL_BQ40Z50) || (di->device_id == BATTERY_FUEL_SH366006))
		bq40z50_cancel_param_upgrade_delay_work(di);
