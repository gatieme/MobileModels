...
static int bq40z50_get_battery_soh(void *dev_data)
{
	...
	soh &= BQ40Z50_BIT_MASK_8;
	if (ret || (soh > BQ40Z50_SOH_MAX))
		soh = di->cache.soh;
	else
		di->cache.soh = soh;
...
static int bq40z50_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, "%-18s%-9s%-4d-%-2.2d-%-4.2d%-7d%-7d%-7d%-7d%-7d%-7d%-7d%-7d%-7d%-7d%-7d"
		"%-7d%-7d%-7d%-7d%-7d%-10x%-10x%-10x",
		di->device_name, di->manufac_name, di->manufac_date.year, di->manufac_date.month,
		di->manufac_date.day, log_data.temp, log_data.vbat, log_data.ibat,
...
static int bq40z50_get_log_head(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size,
		"devName           mfName   mfdate      Temp   Vbat   Ibat   AIbat  Rm     "
		"SocA   SocB   SocC   Fcc    dIbat  dVbat  cycle  Vcell1 Vcell2 batS   "
		"SohFcc safeS     pfS       opS       ");
