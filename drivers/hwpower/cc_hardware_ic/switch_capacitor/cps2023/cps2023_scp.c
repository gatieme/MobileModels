...
static int cps2023_is_fcp_charger_type(void *dev_data)
{
	...
	ret = cps2023_read_byte(di, CPS2023_SCP_STATUS_REG, &reg_val);
	if (ret)
		return 0;

	if (reg_val & CPS2023_SCP_STATUS_ENABLE_HAND_SUCCESS_MASK)
		return 1;
...
static int cps2023_fcp_adapter_detect(struct cps2023_device_info *di)
{
	...
	for (i = 0; i < CPS2023_SCP_DETECT_MAX_COUT; i++) {
		ret = cps2023_read_byte(di, CPS2023_SCP_STATUS_REG, &reg_val);
		hwlog_info("reg[%d]=0x%x\n", CPS2023_SCP_STATUS_REG, reg_val);
		if (ret) {
			hwlog_err("read det attach fail\n");
			continue;
		}
		if (reg_val & CPS2023_SCP_ISR1_SCP_DET_FAIL_FLAG_MASK) {
