...
static int hl7139_is_device_close(void *dev_data)
{
	...
	value = (reg & HL7139_CTRL0_CHG_EN_MASK) >> HL7139_CTRL0_CHG_EN_SHIFT;
	return !value;
}
...
static int hl7139_config_vbat_ovp_mv(int ovp_threshold, void *dev_data)
{
	...
	ret = hl7139_write_mask(di, HL7139_REG_CTRL1_REG, HL7139_REG_CTRL1_VBAT_OVP_TH_MASK,
		HL7139_REG_CTRL1_VBAT_OVP_TH_SHIFT, 3); /* vbat_ovp_offset 110mv */
	if (ret)
		return -EPERM;
	hwlog_info("config_vbat_ovp_offset_mv [%x]=0x%x\n", HL7139_REG_CTRL1_REG, value);

	/* config vbat regulation */
	vbat_regulation = ovp_threshold - HL7139_VBAT_OVP_TH_ABOVE_110MV;
...
static int hl7139_config_switching_frequency(int data, void *dev_data)
{
	...
	if (data < HL7139_SW_FREQ_500KHZ)
		data = HL7139_SW_FREQ_500KHZ;

	if (data > HL7139_SW_FREQ_1600KHZ)
		data = HL7139_SW_FREQ_1600KHZ;

	freq = (u8)((data - HL7139_SW_FREQ_500KHZ) / HL7139_SW_FREQ_STEP);
	if (di->rev_id == 0) {
...
static int hl7139_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hwlog_info("hl7139 probe success");
	return 0;
...
#ifdef CONFIG_PM
static int hl7139_i2c_suspend(struct device *dev)
{
...
static int hl7139_i2c_resume(struct device *dev)
{
	return 0;
}

static void hl7139_i2c_complete(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct hl7139_device_info *di = NULL;
...
static const struct dev_pm_ops hl7139_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(hl7139_i2c_suspend, hl7139_i2c_resume)
	.complete = hl7139_i2c_complete,
};
#define HL7139_PM_OPS (&hl7139_pm_ops)
#else
#define HL7139_PM_OPS (NULL)
#endif /* CONFIG_PM */
