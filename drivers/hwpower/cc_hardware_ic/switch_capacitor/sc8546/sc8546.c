...
int sc8546_get_vbus_uvp_status(void *dev_data)
{
	u8 reg = 0;
	int ret;
	struct sc8546_device_info *di = dev_data;
	...
	ret = sc8546_read_byte(di, SC8546_INT_STAT_REG, &reg);
	if (ret)
		return -EIO;
	hwlog_info("INT_STAT [%x]=0x%x\n", SC8546_INT_STAT_REG, reg);

	return (reg & SC8546_ADAPTER_INSERT_STAT_MASK) ? 0 : SC8546_ADAPTER_INSERT_STAT_UVP_MODE;
}
...
int sc8546_config_vac_ovp_th_mv(struct sc8546_device_info *di, int ovp_th)
{
	...
	else if (ovp_th > SC8546_VAC_OVP_MAX)
...
static int sc8546_reg_reset(struct sc8546_device_info *di)
{
	...
	ret = sc8546_read_byte(di, SC8546_CTRL1_REG, &ctrl1_reg);
	ret += sc8546_config_vac_ovp_th_mv(di, SC8546_VAC_OVP_TH_INIT);
	/* support sc control ovp mos */
	if (di->sc_control_ovp) {
		ret += sc8546_write_mask(di, SC8546_OTG_EN_REG,
			SC8546_OTG_EN_MASK, SC8546_OTG_EN_SHIFT, 1);
	}
	if (ret)
		return -EIO;
...
static int sc8546_enable_adc(int enable, void *dev_data)
{
	int ret;
	u8 reg = 0;
	...
	ret = sc8546_write_mask(di, SC8546_ADCCTRL_REG, SC8546_ADC_EN_MASK, SC8546_ADC_EN_SHIFT, value);
	if (ret)
		return -EPERM;

	ret = sc8546_read_byte(di, SC8546_ADCCTRL_REG, &reg);
	if (ret)
		return -EPERM;

	hwlog_info("adc_enable [%x]=0x%x\n", SC8546_ADCCTRL_REG, reg);
	return 0;
}
	...
	struct sc8546_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_control_ovp", (u32 *)&(di->sc_control_ovp), 0);
}
...
#ifdef CONFIG_PM
static int sc8546_i2c_suspend(struct device *dev)
{
	...
	if (di)
		sc8546_enable_adc(0, (void *)di);

	return 0;
}

static int sc8546_i2c_resume(struct device *dev)
{
	return 0;
}

static void sc8546_i2c_complete(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct sc8546_device_info *di = NULL;
	...
	sc8546_enable_adc(1, (void *)di);
}

static const struct dev_pm_ops sc8546_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(sc8546_i2c_suspend, sc8546_i2c_resume)
	.complete = sc8546_i2c_complete,
};
#define SC8546_PM_OPS (&sc8546_pm_ops)
#else
#define SC8546_PM_OPS (NULL)
#endif /* CONFIG_PM */
