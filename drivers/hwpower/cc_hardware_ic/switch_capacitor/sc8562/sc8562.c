...
static int sc8562_set_dpdm_pre_state(struct sc8562_device_info *di, bool value)
{
	int ret;

	if (value) {
		ret = sc8562_write_mask(di, SC8562_DPDM_CTRL2_REG,
			SC8562_DP_BUFF_EN_MASK, SC8562_DP_BUFF_EN_SHIFT, 0);
		ret += sc8562_write_mask(di, SC8562_DPDM_CTRL2_REG,
			SC8562_DM_BUFF_EN_MASK, SC8562_DM_BUFF_EN_SHIFT, 0);
		ret += sc8562_write_mask(di, SC8562_DPDM_CTRL1_REG, SC8562_DP_20K_PD_EN_MASK,
			SC8562_DP_20K_PD_EN_SHIFT, false);
		ret += sc8562_write_mask(di, SC8562_DPDM_CTRL1_REG,
			SC8562_DPDM_EN_MASK, SC8562_DPDM_EN_SHIFT, 1);
	} else {
		ret = sc8562_write_mask(di, SC8562_DPDM_CTRL1_REG,
			SC8562_DPDM_EN_MASK, SC8562_DPDM_EN_SHIFT, 0);
		ret += sc8562_write_mask(di, SC8562_DPDM_CTRL2_REG,
			SC8562_DP_BUFF_EN_MASK, SC8562_DP_BUFF_EN_SHIFT, 1);
		ret += sc8562_write_mask(di, SC8562_DPDM_CTRL2_REG,
			SC8562_DM_BUFF_EN_MASK, SC8562_DM_BUFF_EN_SHIFT, 0);
	}
...
static const int g_sc8562_dpdm[] = {0, 325, 425, 1000, 1350, 2200, 3000, 0};

static int sc8562_get_dpdm_state(int *dp_mv, int *dm_mv, void *dev_data)
{
	u8 data = 0;
	int dp_stat;
	int dm_stat;
	...
	if (sc8562_set_dpdm_pre_state(di, true)) {
		(void)sc8562_set_dpdm_pre_state(di, false);
		hwlog_err("ic_%u enable dpdm fail\n", di->ic_role);
		return -EIO;
	}

	if (sc8562_read_byte(di, SC8562_DPDM_STAT_REG, &data))
		return -EIO;

	sc8562_set_dpdm_pre_state(di, false);
	dp_stat = (int)(data & SC8562_DPDM_STAT_VDP_RD_MASK) >> SC8562_DPDM_STAT_VDP_RD_SHIFT;
	dm_stat = (int)(data & SC8562_DPDM_STAT_VDM_RD_MASK) >> SC8562_DPDM_STAT_VDM_RD_SHIFT;
	hwlog_info("ic_%u VDP_RD=0x%d, VDM_RD=0x%d\n", di->ic_role, dp_stat, dm_stat);

	*dp_mv = g_sc8562_dpdm[dp_stat];
	*dm_mv = g_sc8562_dpdm[dm_stat];
...
static struct dc_batinfo_ops sc8562_batinfo_ops = {
	...
	.get_dpdm_state = sc8562_get_dpdm_state,
};
...
static void sc8562_interrupt_work(struct work_struct *work)
{
	...
	hwlog_info("ic_%u FLAG_VBAT_OVP[0x%x]=0x%x FLAG_FLT_FLAG[0x%x]=0x%x FLAG_VUSB_OVP[0x%x]=0x%x " \
		"FLAG_IBUS_OCP[0x%x]=0x%x FLAG_IBAT_OCP[0x%x]=0x%x FLAG_SCP_FLAG[0x%x]=0x%x " \
		"FLAG_INT_FLAG[0x%x]=0x%x\n", di->ic_role,
