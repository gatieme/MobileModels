...
static int sc8571_get_vac1_mv(int *vac1, void *dev_data)
{
	int ret;
	s16 data = 0;
	struct sc8571_device_info *di = dev_data;
	...
	ret = sc8571_read_word(di, SC8571_VAC1_ADC1_REG, &data);
	if (ret)
		return -EIO;

	if (di->device_id == SWITCHCAP_BQ25980)
		*vac1 = (int)data * BQ25980_VAC_ADC_STEP / BQ25980_BASE_RATIO_UNIT;
	else
		/* VAC ADC STEP: 6.25mV */
		*vac1 = (int)data * 625 / 100;

	hwlog_info("VAC1_ADC=0x%x, vac1=%d\n", data, *vac1);
	return 0;
}
...
static void sc8571_get_bq25980_frequency(int data, int *freq, int *freq_shift, int shift)
{
	switch (data) {
	...
	if ((shift < BQ25980_FREQ_SHIFT_NORMAL) ||
		(shift > BQ25980_FREQ_SHIFT_M_P10)) {
...
static void sc8571_get_frequency(int data, int *freq, int *freq_shift, int shift)
{
	switch (data) {
	...
	if ((shift < SC8571_FREQ_SHIFT_NORMINAL) ||
		(shift > SC8571_FREQ_SHIFT_SPREAD_SPECTRUM)) {
		hwlog_err("config freq shift invalid, use default value\n");
...
static int sc8571_config_switching_frequency(int data, int shift, struct sc8571_device_info *di)
{
	int freq;
	int freq_shift;
	int ret;
	...
	if (di->device_id == SWITCHCAP_BQ25980)
		sc8571_get_bq25980_frequency(data, &freq, &freq_shift, shift);
	else
		sc8571_get_frequency(data, &freq, &freq_shift, shift);
...
static int sc8571_threshold_reg_init(struct sc8571_device_info *di, u8 mode)
{
	...
	if (mode == SC8571_CHARGE_MODE_1_1) {
		...
		fsw_shift = di->sc8571_lvc_para.switching_frequency_shift;
		...
		fsw_shift = di->sc8571_sc_para.switching_frequency_shift;
	} else {
		...
		fsw_shift = SC8571_FREQ_SHIFT_NORMINAL;
	}
	...
	ret = sc8571_config_switching_frequency(fsw, fsw_shift, di);
	if (ret)
...
static int sc8571_dump_reg(char *buffer, int size, void *dev_data)
{
	...
	(void)sc8571_get_vac1_mv(&data.vac1, di);
	...
	snprintf(buff, sizeof(buff), "%-7d%-7d%-7d%-7d%-7d%-7d%-7d",
		data.ibus, data.vbus, data.vac1, data.vusb, data.ibat, sc8571_get_vbat_mv(di), data.temp);
	strncat(buffer, buff, strlen(buff));
...
static void sc8571_interrupt_work(struct work_struct *work)
{
	...
	(void)sc8571_read_byte(di, SC8571_PMID2OUT_OVP_UVP_REG, &flag[0]);
	hwlog_info("PMID2OUT0x%x=0x%x\n", SC8571_PMID2OUT_OVP_UVP_REG, flag[0]);
	...
	struct sc8571_mode_para *data, const char *name, int id)
{
	...
	if (index >= 0) {
		for (i = SC8571_INFO_MAX_IBAT; i < SC8571_INFO_TOTAL; i++) {
			col = index * SC8571_INFO_TOTAL + i;
			power_dts_read_string_index(power_dts_tag(HWLOG_TAG), np, name, col, &tmp_string);
			kstrtoint(tmp_string, POWER_BASE_DEC, &idata[i]);
		}

		data->max_ibat = idata[SC8571_INFO_MAX_IBAT];
		data->ibus_ocp = idata[SC8571_INFO_IBUS_OCP];
		data->ibat_ocp = idata[SC8571_INFO_IBAT_OCP];
		data->vbat_ovp = idata[SC8571_INFO_VBAT_OVP];
		data->switching_frequency = idata[SC8571_INFO_SW_FREQ];
		data->switching_frequency_shift = idata[SC8571_INFO_SW_FREQ_SHIFT];
	}
