...
static void cps4057_ask_mode_cfg(u8 cm, u8 polarity)
{
	int ret;

	ret = cps4057_write_word_mask(CPS4057_RX_FUNC_EN_ADDR,
		CPS4057_RX_CMALL_EN_MASK, CPS4057_RX_CMALL_EN_SHIFT, cm);
	ret += cps4057_write_word_mask(CPS4057_RX_FUNC_EN_ADDR,
...
static void cps4057_set_mode_cfg(struct cps4057_dev_info *di, int vset)
{
	...
	if (vset <= RX_HIGH_VOUT) {
		cm_cfg = di->mod_cfg.cm.l_volt;
		polar_cfg = di->mod_cfg.polar.l_volt;
	} else {
		if (!power_cmdline_is_factory_mode() && (vset >= RX_HIGH_VOUT2)) {
			cm_cfg = di->mod_cfg.cm.h_volt;
			polar_cfg = di->mod_cfg.polar.h_volt;
		} else {
			cm_cfg = di->mod_cfg.cm.fac_h_volt;
			polar_cfg = di->mod_cfg.polar.fac_h_volt;
		}
	}
	cps4057_ask_mode_cfg(cm_cfg, polar_cfg);
}
...
/*
 * revise ldo_cfg para in buck-boost stage
 * typical sc2 and sc4 switch
 */
static int cps4057_revise_ldo_cfg(struct cps4057_dev_info *di)
{
	int vfc_reg = 0;

	(void)cps4057_rx_get_vfc_reg(&vfc_reg, di);
	if (vfc_reg == ADAPTER_9V * POWER_MV_PER_V)
		return cps4057_write_block(CPS4057_RX_LDO_CFG_ADDR,
...
static int cps4057_rx_set_vfc(int vfc, void *dev_data)
{
	...
	if (cps4057_revise_ldo_cfg(di))
		hwlog_err("set_vfc: revise ldo_cfg failed\n");
...
static int cps4057_rx_enable_ldo_opp_vbst(bool vbst_en, void *dev_data)
{
	int ret;
	struct cps4057_dev_info *di = dev_data;
	...
	if (vbst_en) {
		ret = cps4057_write_word(CPS4057_RX_LDO_OPP_VBST_ADDR,
			di->ldo_opp_vbst.vbst);
		ret += cps4057_write_word(CPS4057_RX_LDO_OPP_ITHL_ADDR,
			di->ldo_opp_vbst.iout_lth);
		ret += cps4057_write_word(CPS4057_RX_LDO_OPP_ITHH_ADDR,
			di->ldo_opp_vbst.iout_hth);
	} else {
		ret = cps4057_write_word(CPS4057_RX_LDO_OPP_VBST_ADDR, 0);
	}
...
static u8 cps4057_rx_iload2reg(int dummy_iload)
{
	return (u8)(dummy_iload * 10 / CPS4057_RX_DUMMY_ILOAD_STEP); /* 10: step need divided by 10 */
}

static void cps4057_rx_set_dflt_dummy_iload(struct cps4057_dev_info *di)
{
	(void)cps4057_write_byte(CPS4057_RX_HV_DUMMY_ILOAD_ADDR,
		cps4057_rx_iload2reg(CPS4057_RX_HV_DUMMY_ILOAD_DEFAULT));
	(void)cps4057_write_byte(CPS4057_RX_HV_DUMMY_ILOAD1_ADDR,
		cps4057_rx_iload2reg(CPS4057_RX_HV_DUMMY_ILOAD1_DEFAULT));
	(void)cps4057_write_byte(CPS4057_RX_LV_DUMMY_ILOAD_ADDR,
		cps4057_rx_iload2reg(CPS4057_RX_LV_DUMMY_ILOAD_DEFAULT));
	(void)cps4057_write_byte(CPS4057_RX_LV_DUMMY_ILOAD1_ADDR,
		cps4057_rx_iload2reg(CPS4057_RX_LV_DUMMY_ILOAD1_DEFAULT));
}

static void cps4057_rx_set_sc_dummy_iload(struct cps4057_dev_info *di)
{
	(void)cps4057_write_byte(CPS4057_RX_HV_DUMMY_ILOAD_ADDR,
		cps4057_rx_iload2reg(di->dummy_load.sc4_outside_ask));
	(void)cps4057_write_byte(CPS4057_RX_HV_DUMMY_ILOAD1_ADDR,
		cps4057_rx_iload2reg(di->dummy_load.sc4_within_ask));
	(void)cps4057_write_byte(CPS4057_RX_LV_DUMMY_ILOAD_ADDR,
		cps4057_rx_iload2reg(di->dummy_load.sc2_outside_ask));
	(void)cps4057_write_byte(CPS4057_RX_LV_DUMMY_ILOAD1_ADDR,
		cps4057_rx_iload2reg(di->dummy_load.sc2_within_ask));
}

static int cps4057_rx_fodchk_chip_init(struct cps4057_dev_info *di)
{
	int ret;

	hwlog_info("fodchk_chip_init\n");
	ret = cps4057_write_word(CPS4057_RX_LV_DUMMY_ILOAD_ADDR,
		cps4057_rx_iload2reg(di->fodchk_dummy_iload));
	ret += cps4057_write_word(CPS4057_RX_DROP_MIN_ADDR, 0);
	ret += cps4057_write_word(CPS4057_RX_DROP_MAX_ADDR, 0);
	ret += cps4057_write_word(CPS4057_RX_FOD_C0_ADDR, 0);
	ret += cps4057_write_word(CPS4057_RX_FOD_C1_ADDR, 0);
	ret += cps4057_write_word_mask(CPS4057_RX_FUNC_EN_ADDR,
		CPS4057_RX_CMALL_EN_MASK, CPS4057_RX_CMALL_EN_SHIFT,
		di->fodchk_ask_cap);
	ret += cps4057_write_word_mask(CPS4057_RX_FUNC_EN_ADDR,
		CPS4057_RX_FASTRPP_EN_MASK, CPS4057_RX_FASTRPP_EN_SHIFT,
		CPS4057_RX_FUNC_EN);
	ret += cps4057_write_word_mask(CPS4057_RX_FUNC_EN_ADDR,
		CPS4057_RX_LIMRPP_DIS_MASK, CPS4057_RX_LIMRPP_DIS_SHIFT,
		CPS4057_RX_FUNC_EN);
	if (ret) {
...
static int cps4057_rx_dflt_chip_init(struct cps4057_dev_info *di, unsigned int tx_type)
{
	int ret;
...
static int cps4057_rx_chip_init(unsigned int init_type, unsigned int tx_type, void *dev_data)
{
	...
	case WIRELESS_CHIP_INIT:
		ret += cps4057_rx_dflt_chip_init(di, tx_type);
		/* fall through */
	case ADAPTER_5V * POWER_MV_PER_V:
		hwlog_info("[chip_init] 5v chip init\n");
		cps4057_rx_set_dflt_dummy_iload(di);
...
static struct wlrx_ic_ops g_cps4057_rx_ic_ops = {
	...
	.enable_ldo_opp_vbst    = cps4057_rx_enable_ldo_opp_vbst,
