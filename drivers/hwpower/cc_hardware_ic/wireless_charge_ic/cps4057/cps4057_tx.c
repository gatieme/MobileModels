...
static int cps4057_tx_set_full_bridge(int v_ask, bool flag)
{
	int ret;
	u16 wr;
...
/* select bridge control method, flag is false, firmware control; flag is true, pt control */
static int cps4057_tx_set_bridge(unsigned int v_ask, unsigned int type, void *dev_data)
{
	bool full_bridge_flag = false;
	struct cps4057_dev_info *di = dev_data;

	if (!di)
		return -EINVAL;

	if (!di->tx_ping_bridge_ctrl && ((type == WLTX_PING_HALF_BRIDGE)
		|| (type == WLTX_PING_FULL_BRIDGE)))
		return 0;

	if ((type == WLTX_PING_FULL_BRIDGE) || type == WLTX_PT_FULL_BRIDGE)
		full_bridge_flag = true;
	return cps4057_tx_set_full_bridge(v_ask, full_bridge_flag);

	return -EINVAL;
}

static int cps4057_tx_get_min_duty(u16 *duty, void *dev_data)
{
	int ret;
	u16 pwm_duty = 0;

	ret = cps4057_read_word(CPS4057_TX_PT_MIN_DUTY_ADDR, &pwm_duty);
	if (ret)
		return ret;

	*duty = pwm_duty / CPS4057_TX_PWM_DUTY_UNIT;
	return 0;
}

static int cps4057_tx_set_min_duty(int duty, void *dev_data)
{
	int dbg;
	...
	if (dbg >= 0)
		duty = dbg;
	if ((duty < CPS4057_TX_PT_DUTY_MIN) || (duty > CPS4057_TX_PT_DUTY_MAX)) {
		hwlog_err("set_min_duty: para out of range\n");
		return -EINVAL;
	}

	return cps4057_write_word(CPS4057_TX_PT_MIN_DUTY_ADDR,
		(u16)duty * CPS4057_TX_PT_DUTY_STEP);
}

static int cps4057_tx_get_max_duty(u16 *duty, void *dev_data)
{
	int ret;
	u16 pwm_duty = 0;

	ret = cps4057_read_word(CPS4057_TX_PT_MAX_DUTY_ADDR, &pwm_duty);
	if (ret)
		return ret;
...
static int cps4057_tx_set_max_duty(int duty, void *dev_data)
{
	int dbg;
	...
	return cps4057_write_word(CPS4057_TX_PT_MAX_DUTY_ADDR,
		(u16)duty * CPS4057_TX_PT_DUTY_STEP);
}
...
static int cps4057_tx_set_ping_duty(int ping_duty, void *dev_data)
{
	if ((ping_duty < CPS4057_TX_PING_DUTY_MIN) ||
		(ping_duty > CPS4057_TX_PING_DUTY_MAX)) {
		hwlog_err("set_ping_duty: para out of range\n");
		return -EINVAL;
	}

	return cps4057_write_byte(CPS4057_TX_PING_DUTY_ADDR, (u8)ping_duty);
}
...
static int cps4057_tx_get_curfod_iin(u16 *tx_iin, void *dev_data)
{
	return cps4057_read_word(CPS4067_TX_CURFOD_IIN_ADDR, tx_iin);
}
...
static int cps4057_tx_set_ploss_old(struct cps4057_dev_info *di)
{
	int ret;
...
static int cps4057_tx_set_ploss_new(struct cps4057_dev_info *di, u8 rx_type)
{
	int ret;
	u8 *tx_fod = NULL;

	tx_fod = wltx_get_fod_ploss_th(di->ic_type,
		rx_type, CPS4057_TX_FOD_LEN);
	if (!tx_fod)
		return -EPERM;
	ret = cps4057_write_block(CPS4057_TX_LP_PLOSS_TH_ADDR,
		tx_fod, CPS4057_TX_LP_PLOSS_TH_LEN);
	ret += cps4057_write_block(CPS4057_TX_PLOSS_TH3_ADDR,
		tx_fod + CPS4057_TX_LP_PLOSS_TH_LEN, CPS4057_TX_PLOSS_TH3_LEN);
	ret += cps4057_write_block(CPS4057_TX_HP_PLOSS_TH_ADDR,
		tx_fod + CPS4057_TX_LP_PLOSS_TH_LEN + CPS4057_TX_PLOSS_TH3_LEN,
		CPS4057_TX_HP_PLOSS_TH_LEN);

	return ret;
}

static int cps4057_tx_set_ploss(u8 rx_type, void *dev_data)
{
	int ret;
	struct cps4057_dev_info *di = dev_data;
	...
	if (di->tx_fod_fmt == WLTX_FOD_STATUS_FMT_OLD) {
		ret = cps4057_tx_set_ploss_old(di);
	} else if (di->tx_fod_fmt == WLTX_FOD_STATUS_FMT_NEW) {
		ret = cps4057_tx_set_ploss_new(di, rx_type);
	} else {
		hwlog_err("tx_fod_fmt: get error\n");
		return -EINVAL;
	}

	ret += cps4057_write_word(CPS4057_TX_HP_FOD_CUR_TH2_ADDR,
		di->tx_fod.hp_cur_th2);
	ret += cps4057_write_word(CPS4057_TX_HP_PLOSS_TH3_ADDR,
		di->tx_fod.hp_ploss_th3);
...
static void cps4057_tx_prevfod_chk(int flag, void *dev_data)
{
	static u16 duty_min = 0;
	static u16 duty_max = 0;
	static u16 fop_min = 0;
	static u16 fop_max = 0;
	struct cps4057_dev_info *di = dev_data;
	...
	switch (flag) {
	case WLTX_PREVFOD_ILIM_START:
		cps4057_tx_get_min_fop(&fop_min, di);
		cps4057_tx_get_max_fop(&fop_max, di);
		cps4057_tx_get_min_duty(&duty_min, di);
		cps4057_tx_get_max_duty(&duty_max, di);
		cps4057_tx_set_min_fop(CPS4057_TX_MIN_FOP_PREVFOD, di);
		cps4057_tx_set_max_fop(CPS4057_TX_MAX_FOP_PREVFOD, di);
		cps4057_tx_set_min_duty(CPS4057_TX_PT_DUTY_MIN_PREVFOD, di);
		cps4057_tx_set_max_duty(CPS4057_TX_PT_DUTY_MAX_PREVFOD, di);
		break;
	case WLTX_PREVFOD_ILIM_STOP:
		cps4057_tx_set_min_fop(fop_min > 0 ? fop_min : CPS4057_TX_MIN_FOP, di);
		cps4057_tx_set_max_fop(fop_max > 0 ? fop_max : CPS4057_TX_MAX_FOP, di);
		cps4057_tx_set_min_duty(duty_min > 0 ? duty_min : CPS4057_TX_PT_DUTY_MIN, di);
		cps4057_tx_set_max_duty(duty_max > 0 ? duty_max : CPS4057_TX_PT_DUTY_MAX, di);
		break;
	default:
		break;
	}
...
static int cps4057_sw2tx(struct cps4057_dev_info *di)
{
	int ret;
	int i;
	u8 mode = 0;
...
static int cps4057_tx_set_open_loop(bool enable, void *dev_data)
{
	 u8 val = enable ? 1 : 0; /* 1:en open loop 0:disable open loop */
	 ...
	 return cps4057_write_byte_mask(CPS4057_TX_FUNC_EN_ADDR, CPS4057_TX_PA_MODE_MASK,
		CPS4057_TX_PA_MODE_SHIFT, val);
}

static int cps4057_tx_set_ping_ocp(int value)
{
	int dbg;
	...
	return cps4057_write_word(CPS4057_TX_PING_OCP_ADDR, (u16)value);
}

static int cps4057_tx_set_pt_ocp(int value)
{
	int dbg;
	...
	return cps4057_write_word(CPS4057_TX_OCP_TH_ADDR, (u16)value);
}

static void cps4057_tx_select_init_para(struct cps4057_dev_info *di,
	unsigned int client)
{
	di->tx_init_para.sr_death_time = 0; /* default 23ns */
	di->tx_init_para.ping_ocp = CPS4057_TX_PING_OCP_TH;

	switch (client) {
	case WLTX_CLIENT_UI:
		di->tx_init_para.ping_freq = di->tx_ping_freq;
		di->tx_init_para.tx_min_duty = di->tx_min_duty;
		di->tx_init_para.ping_interval = CPS4057_TX_PING_INTERVAL;
		di->tx_init_para.ping_ocp = di->tx_ping_ocp;
		break;
	...
	case WLTX_CLIENT_BAT_HEATING:
		di->tx_init_para.ping_freq = CPS4057_BAT_HEATING_PING_FREQ;
		di->tx_init_para.ping_interval = CPS4057_BAT_HEATING_PING_INTERVAL;
		break;
	case WLTX_CLIENT_COOLINGCASE:
		di->tx_init_para.sr_death_time = 1; /* 1:config 47ns */
		di->tx_init_para.ping_freq = di->tx_ping_freq;
		di->tx_init_para.tx_min_duty = di->tx_min_duty;
		di->tx_init_para.ping_interval = CPS4057_TX_PING_INTERVAL;
		break;
...
{
	int ret;
	int debug_ping_duty = wlic_dbg_tx_get_param(WLTX_PARAM_PING_DUTY);

	ret = cps4057_sw2tx(di);
	if (ret) {
		hwlog_err("set_init_para: sw2tx failed\n");
		return ret;
	}

	ret = cps4057_tx_set_pt_ocp(CPS4057_TX_OCP_TH);
	...
	ret += cps4057_tx_set_ping_ocp(di->tx_init_para.ping_ocp);
	ret += cps4057_tx_set_min_duty(di->tx_init_para.tx_min_duty, di);
	ret += cps4057_tx_set_ping_freq(di->tx_init_para.ping_freq, di);
	ret += cps4057_tx_set_min_fop(di->min_fop, di);
	...
	ret += cps4057_write_word_mask(CPS4057_TX_FUNC_EN_ADDR,
		CPS4057_PING_BRIDGE_CTRL_MASK, CPS4057_PING_BRIDGE_CTRL_SHIFT,
		di->tx_ping_bridge_ctrl);
	ret += cps4057_write_word_mask(CPS4057_TX_FUNC_EN_ADDR,
		CPS4057_TX_SR_DEATH_TIME_MASK, CPS4057_TX_SR_DEATH_TIME_SHIFT,
		di->tx_init_para.sr_death_time);
	if (di->timer_cc_ctrl_val != CPS4057_TX_TIMER_CC_CTRL_DFLT) {
		ret += cps4057_write_word_mask(CPS4057_TX_TIMER_CC_EN_ADDR,
			CPS4057_TX_TIMER_EN_MASK, CPS4057_TX_TIMER_EN_SHIFT, 1);
		ret += cps4057_write_byte(CPS4057_TX_TIMER_CC_CTRL_ADDR, (u8)di->timer_cc_ctrl_val);
	}
	if (ret) {
	...
	if (debug_ping_duty >= 0)
		cps4057_tx_set_ping_duty(debug_ping_duty, di);
...
static void cps4057_tx_debug_register(struct cps4057_dev_info *di)
{
...
static struct wltx_ic_ops g_cps4057_tx_ic_ops = {
	...
	.prevfod_chk            = cps4057_tx_prevfod_chk,
	...
	.get_curfod_iin         = cps4057_tx_get_curfod_iin,
	...
	.set_min_duty           = cps4057_tx_set_min_duty,
	.set_max_duty           = cps4057_tx_set_max_duty,
	.set_open_loop          = cps4057_tx_set_open_loop,
	.set_ploss              = cps4057_tx_set_ploss,
