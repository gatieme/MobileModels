...
static const char * const g_cps4061_rx_irq_name[] = {
	/* [n]: n means bit in registers */
	[0]  = "power_on",
	[1]  = "ldo_off",
	[2]  = "ldo_on",
...
#define CPS4067_VFC_MAP_SIZE      4
struct cps4067_vfc_map {
	int vpa;
	int vmldo;
};

static struct cps4067_vfc_map const rx_vfc_nonscx_map[CPS4067_VFC_MAP_SIZE] = {
	{ 5000, 5000 }, { 9000, 9000 }, { 12000, 9000 }, { 15000, 9000 }, /* mV */
};
static struct cps4067_vfc_map const rx_vfc_scx_map[CPS4067_VFC_MAP_SIZE] = {
...
static void cps4067_rx_show_irq(struct cps4067_dev_info *di, u32 intr)
{
	u32 i;
	char **irq = NULL;
	int len;
	...
	if (di->chip_id == CPS4067_CHIP_ID) {
		irq = (char **)g_cps4067_rx_irq_name;
		len = ARRAY_SIZE(g_cps4067_rx_irq_name);
	} else {
		irq = (char **)g_cps4061_rx_irq_name;
		len = ARRAY_SIZE(g_cps4061_rx_irq_name);
	}
...
static void cps4067_rx_set_fod_offset(u8 offset, void *dev_data)
{
	struct cps4067_dev_info *di = dev_data;
	...
	cps4067_write_byte(di, CPS4067_RX_FOD_C0_ADDR, offset);
	cps4067_write_byte(di, CPS4067_RX_FOD_C1_ADDR, offset);
	cps4067_write_byte(di, CPS4067_RX_FOD_C2_ADDR, offset);
	cps4067_write_byte(di, CPS4067_RX_FOD_C3_ADDR, offset);
}

static void cps4067_rx_set_turbo_chg_flag(bool flag, void *dev_data)
{
	struct cps4067_dev_info *di = dev_data;
...
static void cps4067_rx_set_cur_pmode_name(const char *cur_pmode_name, void *dev_data)
{
	struct cps4067_dev_info *di = dev_data;
...
static int cps4067_rx_set_rx_vout(int vout, void *dev_data)
{
	...
	ret = cps4067_write_word(di, CPS4067_RX_VOUT_SET_ADDR, (u16)vout);
	if (ret) {
...
static int cps4067_set_intfr_mode_cfg(struct cps4067_dev_info *di)
{
	int i;
	u8 cm_cfg, polar_cfg;
...
static void cps4067_set_mode_cfg(struct cps4067_dev_info *di, int vfc)
{
	...
	if (!cps4067_set_intfr_mode_cfg(di))
		return;
...
static int cps4067_rx_get_vmldo_by_vfc_map(int vfc, void *dev_data)
{
	int i;
	struct cps4067_vfc_map const *map = NULL;
	struct cps4067_dev_info *di = dev_data;
	...
	if (di->is_scx_mode)
		map = (struct cps4067_vfc_map *)rx_vfc_scx_map;
	else
		map = (struct cps4067_vfc_map *)rx_vfc_nonscx_map;
	for (i = 0; i < CPS4067_VFC_MAP_SIZE; i++) {
		if (vfc == map[i].vpa) {
...
static int cps4067_rx_set_vmldo(struct cps4067_dev_info *di, u16 vfc)
{
	...
	vmldo = cps4067_rx_get_vmldo_by_vfc_map(vfc, di);
	if (vmldo < 0)
		return -EINVAL;

	ret = cps4067_write_word(di, CPS4067_RX_FC_VMLDO_ADDR, vmldo);
	ret += cps4067_write_word(di, CPS4067_RX_VOUT_SET_ADDR, vmldo);
	return ret;
}
...
static int cps4067_rx_send_fc_cmd(struct cps4067_dev_info *di, int vfc)
{
	...
	ret = cps4067_write_byte_mask(di, CPS4067_RX_STATUS_ADDR,
		CPS4067_RX_FC_SUCC_MASK, CPS4067_RX_FC_SUCC_SHIFT,
		CPS4067_RX_FC_SUCC_RESET);
	if (ret) {
...
static bool cps4067_rx_is_fc_succ(struct cps4067_dev_info *di, int vset)
{
	int i;
	int cnt;
	u8 rx_status;
...
static int cps4067_rx_set_cm(int cm_type, void *dev_data)
{
	int vfc_reg = 0;
	struct cps4067_dev_info *di = dev_data;
	...
	cps4067_set_mode_cfg(di, vfc_reg);
	return 0;
}

static int cps4067_rx_set_boost_mode(struct cps4067_dev_info *di, int vfc)
{
	if (power_cmdline_is_factory_mode() && di->is_scx_mode)
		return cps4067_rx_enable_boost_mode(di);

	if (vfc >= WLRX_IC_HIGH_VOUT2)
		return cps4067_rx_disable_boost_mode(di);
...
static int cps4067_rx_set_vfc(int vfc, void *dev_data)
{
	...
	ret = cps4067_rx_set_boost_mode(di, vfc);
	if (ret)
		goto out;
	...
	for (i = 0; i < CPS4067_RX_FC_VPA_RETRY_CNT; i++) {
		...
		if (cps4067_rx_is_fc_succ(di, vfc)) {
...
static void cps4067_rx_set_scx_mode(bool scx_mode, void *dev_data)
{
	struct cps4067_dev_info *di = dev_data;
...
static int cps4067_rx_kick_watchdog(void *dev_data)
{
	...
	return cps4067_write_word(dev_data, CPS4067_RX_WDT_ADDR, di->rx_wdt_timeout);
}
...
static int cps4067_rx_enable_ldo_opp_vbst(bool vbst_en, void *dev_data)
{
	int ret;
	struct cps4067_dev_info *di = dev_data;
	...
	if (vbst_en) {
		ret = cps4067_write_word(di, CPS4067_RX_LDO_OPP_VBST_ADDR,
			di->ldo_opp_vbst.vbst);
		ret += cps4067_write_word(di, CPS4067_RX_LDO_OPP_ITHL_ADDR,
			di->ldo_opp_vbst.iout_lth);
		ret += cps4067_write_word(di, CPS4067_RX_LDO_OPP_ITHH_ADDR,
			di->ldo_opp_vbst.iout_hth);
	} else {
		ret = cps4067_write_word(di, CPS4067_RX_LDO_OPP_VBST_ADDR, 0);
	}

	return ret;
}

static u8 cps4067_rx_iload2reg(int dummy_iload, u16 chip_id)
{
	int step = (chip_id == CPS4067_CHIP_ID) ?
		CPS4067_RX_DUMMY_LOAD_STEP : CPS4061_RX_DUMMY_LOAD_STEP;

	return (u8)(dummy_iload * CPS4067_RX_DUMMY_LOAD_COEF / step);
}
...
static int cps4067_rx_5vbuck_chip_init(struct cps4067_dev_info *di)
{
	...
	ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_NO_MOD_ADDR,
		cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_5V_NO_MOD],
		di->chip_id));
	ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_MOD_ADDR,
		cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_5V_MOD],
		di->chip_id));
	ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_MOD_ADDR,
		CPS4067_RX_DUMMY_LOAD_MOD_VAL);
	ret += cps4067_write_word(di, CPS4067_RX_FC_DELTA_ADDR,
		di->rx_vfc_diff);
	ret += cps4067_write_byte(di, CPS4067_RX_FOD_OFFSET_ADDR,
		di->rx_fod_offset);
...
static int cps4067_rx_init_fod_coef(struct cps4067_dev_info *di, unsigned int tx_type)
{
	...
	vrx = cps4067_rx_get_vmldo_by_vfc_map(vfc_reg, di);
...
static int cps4067_rx_fodchk_set_gain_unit(struct cps4067_dev_info *di)
{
	if (di->prevfod_gain_unit != CPS4067_RX_GAIN_UNIT_DEFAULT)
		return cps4067_write_byte(di, CPS4067_RX_GAIN_UNIT_ADDR,
			(u8)di->prevfod_gain_unit);

	return 0;
}

static int cps4067_rx_fodchk_chip_init(struct cps4067_dev_info *di)
{
	int ret;

	hwlog_info("fodchk_chip_init\n");
	ret = cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_NO_MOD_ADDR,
		cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_FODCHK_NO_MOD], di->chip_id));
	ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_MOD_ADDR,
		cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_FODCHK_MOD], di->chip_id));
	ret += cps4067_write_word(di, CPS4067_RX_DROP_MIN_ADDR, 0);
	ret += cps4067_write_word(di, CPS4067_RX_DROP_MAX_ADDR, 0);
	ret += cps4067_write_byte(di, CPS4067_RX_FOD_C0_ADDR, 0);
	ret += cps4067_write_byte(di, CPS4067_RX_FOD_C1_ADDR, 0);
	ret += cps4067_write_byte(di, CPS4067_RX_FOD_C2_ADDR, 0);
	ret += cps4067_write_byte(di, CPS4067_RX_FOD_C3_ADDR, 0);
	ret += cps4067_write_dword_mask(di, CPS4067_RX_FUNC_EN_ADDR,
		CPS4067_RX_CMALL_EN_MASK, CPS4067_RX_CMALL_EN_SHIFT,
		di->fodchk_ask_cap);
	ret += cps4067_write_dword_mask(di, CPS4067_RX_FUNC_EN_ADDR,
		CPS4067_RX_CM_POLARITY_MASK, CPS4067_RX_CM_POLARITY_SHIFT,
		CPS4067_RX_CM_POSITIVE);
	ret += cps4067_write_word_mask(di, CPS4067_RX_FUNC_EN_ADDR,
		CPS4067_RX_FASTRPP_EN_MASK, CPS4067_RX_FASTRPP_EN_SHIFT,
		CPS4067_RX_FUNC_EN);
	ret += cps4067_write_word_mask(di, CPS4067_RX_FUNC_EN_ADDR,
		CPS4067_RX_LIMRPP_DIS_MASK, CPS4067_RX_LIMRPP_DIS_SHIFT,
		CPS4067_RX_FUNC_EN);
	ret += cps4067_rx_fodchk_set_gain_unit(di);
	if (ret) {
...
static int cps4067_rx_dflt_chip_init(struct cps4067_dev_info *di, unsigned int tx_type)
{
	int ret;
	...
	if (di->rx_ldo_opp_en)
		ret += cps4067_write_word_mask(di, CPS4067_RX_FUNC_EN_ADDR,
			CPS4067_RX_MLDO_OPP_EN_MASK, CPS4067_RX_MLDO_OPP_SHIFT,
			di->rx_ldo_opp_en);
...
static int cps4067_rx_chip_init(unsigned int init_type, unsigned int tx_type, void *dev_data)
{
	...
	case WLRX_IC_9VBUCK_CHIP_INIT:
		...
		ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_NO_MOD_ADDR,
			cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_9V_NO_MOD],
			di->chip_id));
		ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_MOD_ADDR,
			cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_9V_MOD],
			di->chip_id));
		ret += cps4067_rx_init_fod_coef(di, tx_type);
		break;
	case WLRX_IC_SC_CHIP_INIT:
		...
		ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_NO_MOD_ADDR,
			cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_SC_NO_MOD],
			di->chip_id));
		ret += cps4067_write_byte(di, CPS4067_RX_DUMMY_LOAD_MOD_ADDR,
			cps4067_rx_iload2reg(di->dummy_iload_new[CPS4067_DUMMY_ILOAD_SC_MOD],
			di->chip_id));
		ret += cps4067_rx_init_fod_coef(di, tx_type);
		break;
	case WIRELESS_FODCHK_CHIP_INIT:
		ret += cps4067_rx_fodchk_chip_init(di);
		break;
	default:
...
static void cps4067_rx_fault_irq_handler(struct cps4067_dev_info *di)
{
	...
	if (di->irq_val & CPS4067_RX_IRQ_OPP_H) {
		di->irq_val &= ~CPS4067_RX_IRQ_OPP_H;
...
static struct wlrx_ic_ops g_cps4067_rx_ic_ops = {
	...
	.set_cm                 = cps4067_rx_set_cm,
	.set_vfc                = cps4067_rx_set_vfc,
	.set_scx_mode           = cps4067_rx_set_scx_mode,
	.set_vout               = cps4067_rx_set_rx_vout,
	.get_vrx_by_vfc_map     = cps4067_rx_get_vmldo_by_vfc_map,
	...
	.enable_ldo_opp_vbst    = cps4067_rx_enable_ldo_opp_vbst,
	.set_turbo_chg_flag     = cps4067_rx_set_turbo_chg_flag,
	.set_cur_pmode_name     = cps4067_rx_set_cur_pmode_name,
	.set_fod_offset         = cps4067_rx_set_fod_offset,
};
