...
static int cps8601_fw_load_q_data(struct cps8601_dev_info *di)
{
	...
	ret = cps8601_fw_status_check(di, CPS8601_CMD_PGM_Q_FACTOR);
	if (ret) {
...
static int cps8601_fw_check_q_data(struct cps8601_dev_info *di)
{
	...
	ret = cps8601_read_byte(di, CPS8601_TX_CALI_CNT_ADDR, &q_cnt);
	ret += cps8601_read_word(di, CPS8601_TX_CALI_WIDTH_ADDR, &q_width);
