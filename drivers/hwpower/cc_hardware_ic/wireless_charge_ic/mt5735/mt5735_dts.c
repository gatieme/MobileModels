...
static void mt5735_parse_tx_basic_para(const struct device_node *np,
	struct mt5735_dev_info *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	struct mt5735_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rx_cep_ctrl", (u32 *)&di->rx_cep_ctrl,
		MT5735_RX_CEP_CTRL_DFLT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	mt5735_parse_tx_basic_para(np, di);
