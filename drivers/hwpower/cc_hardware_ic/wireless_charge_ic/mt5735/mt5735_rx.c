...
static void mt5735_rx_set_turbo_chg_flag(bool flag, void *dev_data)
{
	struct mt5735_dev_info *di = dev_data;
...
static void mt5735_rx_set_cur_pmode_name(const char *cur_pmode_name, void *dev_data)
{
	struct mt5735_dev_info *di = dev_data;
...
static int mt5735_rx_enable_ldo_opp_vbst(bool vbst_en, void *dev_data)
{
	int ret;
	struct mt5735_dev_info *di = dev_data;
	...
	if (vbst_en) {
		ret = mt5735_write_word(MT5735_RX_LDO_OPP_VBST_ADDR,
			di->ldo_opp_vbst.vbst);
		ret += mt5735_write_word(MT5735_RX_LDO_OPP_ITHL_ADDR,
			di->ldo_opp_vbst.iout_lth);
		ret += mt5735_write_word(MT5735_RX_LDO_OPP_ITHH_ADDR,
			di->ldo_opp_vbst.iout_hth);
	} else {
		ret = mt5735_write_word(MT5735_RX_LDO_OPP_VBST_ADDR, 0);
	}
...
static int mt5735_rx_init_dummy_iload(struct mt5735_dev_info *di)
{
	int ret = 0;

	ret += mt5735_write_word(MT5735_RX_ILOAD_TH_ADDR,
		di->dummy_load.th_outside_ask);
	ret += mt5735_write_word(MT5735_RX_ILOAD_ADDR,
		di->dummy_load.val_outside_ask);
	ret += mt5735_write_word(MT5735_RX_ILOAD_ASK_TH_ADDR,
		di->dummy_load.th_within_ask);
	ret += mt5735_write_word(MT5735_RX_ILOAD_ASK_ADDR,
		di->dummy_load.val_within_ask);
	return ret;
}

static int mt5735_rx_dflt_chip_init(struct mt5735_dev_info *di, unsigned int tx_type)
{
	int ret;

	hwlog_info("dflt_chip_init\n");
	di->g_val.rx_stop_chrg_flag = false;
	ret = mt5735_rx_init_dummy_iload(di);
	...
	ret += mt5735_write_byte(MT5735_RX_RPPDLY_CNT_ADDR,
		MT5735_RX_RPPDLY_CNT_DFLT);
	ret += mt5735_write_byte(MT5735_RX_FOD_OFFSET_FACTOR_ADDR,
		di->rx_fod_offset_factor);
	ret += mt5735_write_byte(MT5735_RX_CEP_CTRL,
		di->rx_cep_ctrl);
	if (ret) {
...
static int mt5735_rx_fodchk_chip_init(struct mt5735_dev_info *di)
{
	int ret;

	hwlog_info("fodchk_chip_init\n");
	ret = mt5735_write_word(MT5735_RX_ILOAD_ADDR, 0);
	ret += mt5735_write_word(MT5735_RX_ILOAD_ASK_ADDR, 0);
	ret += mt5735_write_word(MT5735_RX_VRECTY1_ADDR, 0);
	ret += mt5735_write_word(MT5735_RX_VRECTY2_ADDR, 0);
	ret += mt5735_write_byte(MT5735_RX_ASK_CFG_ADDR, di->fodchk_ask_cap);
	ret += mt5735_write_byte(MT5735_RX_FOD_OFFSET0_ADDR, 0);
	ret += mt5735_write_byte(MT5735_RX_FOD_OFFSET1_ADDR, 0);
	ret += mt5735_write_byte(MT5735_RX_RPPDLY_CNT_ADDR, 1);
	ret += mt5735_write_byte(MT5735_RX_FOD_OFFSET_FACTOR_ADDR,
...
static int mt5735_rx_chip_init(unsigned int init_type, unsigned int tx_type, void *dev_data)
{
	...
	case WIRELESS_CHIP_INIT:
		...
		ret += mt5735_rx_dflt_chip_init(di, tx_type);
		/* fall through */
	...
	case WIRELESS_FODCHK_CHIP_INIT:
		ret += mt5735_rx_fodchk_chip_init(di);
		break;
	default:
...
static struct wlrx_ic_ops g_mt5735_rx_ic_ops = {
	...
	.enable_ldo_opp_vbst    = mt5735_rx_enable_ldo_opp_vbst,
	.set_turbo_chg_flag     = mt5735_rx_set_turbo_chg_flag,
	.set_cur_pmode_name     = mt5735_rx_set_cur_pmode_name,
