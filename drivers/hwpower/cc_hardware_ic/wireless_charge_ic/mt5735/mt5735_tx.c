...
static int mt5735_sw2tx(struct mt5735_dev_info *di)
{
	int ret;
	int i;
	u16 mode = 0;
...
static int mt5735_tx_get_curfod_iin(u16 *tx_iin, void *dev_data)
{
	return mt5735_read_word(MT5735_TX_CURFOD_IIN_ADDR, tx_iin);
}
...
static int mt5735_tx_set_ilimit(u16 tx_ilim, void *dev_data)
{
	...
	if (dbg >= 0)
		tx_ilim = (u16)dbg;
	if ((tx_ilim < MT5735_TX_ILIM_MIN) || (tx_ilim > MT5735_TX_ILIM_MAX)) {
		hwlog_err("set_ilimit: out of range\n");
		return -EINVAL;
	}

	return mt5735_write_word(MT5735_TX_ILIM_ADDR, tx_ilim);
}

static int mt5735_tx_set_ploss(u8 rx_type, void *dev_data)
{
	int ret;
	u8 *tx_fod = NULL;
	struct mt5735_dev_info *di = dev_data;
	...
	if (di->tx_fod_fmt == WLTX_FOD_STATUS_FMT_OLD) {
		...
		tx_fod = wltx_get_fod_ploss_th(di->ic_type,
			rx_type, MT5735_TX_FOD_LEN);
		if (!tx_fod)
			return -EPERM;
		ret = mt5735_write_block(MT5735_TX_FOD_ADDR,
			tx_fod, MT5735_TX_FOD_LEN);
	} else {
		hwlog_err("tx_fod_fmt: get error\n");
		return -EINVAL;
	}

	ret += mt5735_write_word(MT5735_TX_10_18W_ITH_ADDR,
		MT5735_TX_10_18W_ITH);
...
static int mt5735_tx_get_min_duty(u8 *duty, void *dev_data)
{
	return mt5735_read_byte(MT5735_TX_PT_MIN_DUTY_ADDR, duty);
}

static int mt5735_tx_set_min_duty(int duty, void *dev_data)
{
	int ret;
	u8 val = 0;
	int dbg;
	...
	if (dbg >= 0)
		duty = dbg;
	if ((duty < MT5735_TX_PT_DUTY_MIN) || (duty > MT5735_TX_PT_DUTY_MAX)) {
		hwlog_err("set_min_duty: para out of range\n");
		return -EINVAL;
	}

	ret = mt5735_write_byte(MT5735_TX_PT_MIN_DUTY_ADDR, (u8)duty);
	(void)mt5735_tx_get_min_duty(&val, dev_data);
	hwlog_info("set_min_duty: readback:0x%x\n", val);
	return ret;
}

static int mt5735_tx_get_max_duty(u8 *duty, void *dev_data)
{
	return mt5735_read_byte(MT5735_TX_PT_MAX_DUTY_ADDR, duty);
}

static int mt5735_tx_set_max_duty(int duty, void *dev_data)
{
	int ret;
	u8 val = 0;
	int dbg;
	...
	ret = mt5735_write_byte(MT5735_TX_PT_MAX_DUTY_ADDR, (u8)duty);
	(void)mt5735_tx_get_max_duty(&val, dev_data);
	hwlog_info("set_max_duty: readback:0x%x\n", val);
	return ret;
}
...
static void mt5735_tx_prevfod_chk(int flag, void *dev_data)
{
	static u8 duty_min = 0;
	static u8 duty_max = 0;
	...
	case WLTX_PREVFOD_ILIM_START:
		mt5735_tx_get_min_fop(&fop_min, di);
		mt5735_tx_get_max_fop(&fop_max, di);
		mt5735_tx_get_min_duty(&duty_min, di);
		mt5735_tx_get_max_duty(&duty_max, di);
		mt5735_tx_set_min_fop(MT5735_TX_MIN_FOP_PREVFOD, di);
		mt5735_tx_set_max_fop(MT5735_TX_MAX_FOP_PREVFOD, di);
		mt5735_tx_set_min_duty(MT5735_TX_PT_DUTY_MIN_PREVFOD, di);
		mt5735_tx_set_max_duty(MT5735_TX_PT_DUTY_MAX_PREVFOD, di);
		break;
	case WLTX_PREVFOD_ILIM_STOP:
		mt5735_tx_set_min_fop(fop_min > 0 ? fop_min : MT5735_TX_MIN_FOP, di);
		mt5735_tx_set_max_fop(fop_max > 0 ? fop_max : MT5735_TX_MAX_FOP, di);
		mt5735_tx_set_min_duty(duty_min > 0 ? duty_min : MT5735_TX_PT_DUTY_MIN, di);
		mt5735_tx_set_max_duty(duty_max > 0 ? duty_max : MT5735_TX_PT_DUTY_MAX, di);
		break;
	default:
		break;
	}
...
static int mt5735_tx_set_open_loop(bool enable, void *dev_data)
{
	u32 mask;
	u32 shift;

	hwlog_info("[set_open_loop] %s\n", enable ? "enable" : "disable");
	mask = enable ? MT5735_TX_CMD_EN_OPEN_LOOP : MT5735_TX_CMD_DIS_OPEN_LOOP;
	shift = enable ? MT5735_TX_CMD_EN_OPEN_LOOP_SHIFT : MT5735_TX_CMD_DIS_OPEN_LOOP_SHIFT;
	return mt5735_write_dword_mask(MT5735_TX_CMD_ADDR, mask, shift, MT5735_TX_CMD_VAL);
}

static int mt5735_tx_set_ping_ocp(int value)
{
	int dbg;
	...
	return mt5735_write_word(MT5735_TX_PING_OCP_ADDR, (u16)value);
}

static int mt5735_tx_set_pt_ocp(int value)
{
	int dbg;
	...
	return mt5735_write_word(MT5735_TX_OCP_TH_ADDR, (u16)value);
}

static int mt5735_tx_set_ping_duty(int ping_duty)
{
	if ((ping_duty < MT5735_TX_PING_DUTY_MIN) ||
		(ping_duty > MT5735_TX_PING_DUTY_MAX)) {
		hwlog_err("set_ping_duty: para out of range\n");
		return -EINVAL;
	}

	hwlog_info("[set_ping_duty] %d\n", ping_duty);
	return mt5735_write_byte(MT5735_TX_PING_DUTY_ADDR, (u8)ping_duty);
}

static void mt5735_tx_select_init_para(struct mt5735_dev_info *di,
	unsigned int client)
{
	di->tx_init_para.ocp_th = MT5735_TX_OCP_TH;
	di->tx_init_para.ping_ocp = MT5735_TX_PING_OCP_DFLT;
	...
	case WLTX_CLIENT_BAT_HEATING:
		di->tx_init_para.ping_freq = MT5735_BAT_HEATING_PING_FREQ;
		di->tx_init_para.ping_interval = MT5735_BAT_HEATING_PING_INTERVAL;
		di->tx_init_para.ocp_th = MT5735_TX_BAT_HEATING_OCP_TH;
		break;
	default:
		di->tx_init_para.ping_freq = di->tx_ping_freq;
		di->tx_init_para.ping_interval = MT5735_TX_PING_INTERVAL;
...
static int mt5735_tx_set_init_para(struct mt5735_dev_info *di)
{
	...
	ret = mt5735_write_byte(MT5735_TX_OTP_TH_ADDR, di->tx_init_para.ocp_th);
	ret += mt5735_tx_set_ping_ocp(di->tx_init_para.ping_ocp);
	...
	ret += mt5735_tx_set_pt_ocp(MT5735_TX_OCP_TH);
	...
	ret += mt5735_tx_set_min_fop(di->min_fop, di);
	...
	ret += mt5735_tx_set_min_duty(di->tx_min_duty, di);
	if (di->tx_ilim_proc_cycle != MT5735_TX_ILIM_PROC_CYCLE_DFLT) {
		ret += mt5735_write_byte(MT5735_TX_NATIONAL_STD_CER_ADDR, MT5735_TX_NATIONAL_STD_EN);
		ret += mt5735_write_byte(MT5735_TX_ILIM_PROC_CYCLE_ADDR, (u8)di->tx_ilim_proc_cycle);
	}
	if (ret) {
	...
	if (debug_ping_duty >= 0)
		mt5735_tx_set_ping_duty(debug_ping_duty);
...
static void mt5735_tx_debug_register(struct mt5735_dev_info *di)
{
...
static struct wltx_ic_ops g_mt5735_tx_ic_ops = {
	...
	.prevfod_chk            = mt5735_tx_prevfod_chk,
	...
	.get_curfod_iin         = mt5735_tx_get_curfod_iin,
	...
	.set_min_duty           = mt5735_tx_set_min_duty,
	.set_max_duty           = mt5735_tx_set_max_duty,
	.set_open_loop          = mt5735_tx_set_open_loop,
	.set_ploss              = mt5735_tx_set_ploss,
