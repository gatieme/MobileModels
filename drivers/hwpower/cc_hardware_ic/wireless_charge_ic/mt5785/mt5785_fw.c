...
static int mt5785_fw_check_mtp(void *dev_data)
{
	...
	if (power_cmdline_is_factory_mode() && mt5785_rx_is_tx_exist(di) &&
		!mt5785_fw_check_mtp_version(di))
		goto succ;
...
exit:
	mt5785_fw_load_bootloader_disable_map(di);
	mt5785_fw_reset(di);
	mt5785_enable_irq(di);
...
static void mt5785_fw_mtp_check_work(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		ret = mt5785_fw_program_mtp(di, g_mt5785_fw, (u16)ARRAY_SIZE(g_mt5785_fw));
		if (ret)
			continue;
...
int mt5785_fw_ops_register(struct wltrx_ic_ops *ops, struct mt5785_dev_info *di)
{
	...
	di->fw_mtp.mtp_len = (u16)ARRAY_SIZE(g_mt5785_fw);
