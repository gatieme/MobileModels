...
{
	if (v_ask > MT5785_TX_SWITCH_BRI_CTL_VTH)
		return mt5785_write_byte(dev_data, MT5785_TX_FUNC_EN_ADDR, MT5785_TX_FUNC_EN);
	else
		return mt5785_write_byte(dev_data, MT5785_TX_FUNC_EN_ADDR, MT5785_TX_FUNC_DIS);
}
...
static int mt5785_tx_set_ping_duty(u16 ping_duty, void *dev_data)
{
	if ((ping_duty < MT5785_TX_PING_DUTY_MIN) ||
		(ping_duty > MT5785_TX_PING_DUTY_MAX)) {
		hwlog_err("set_ping_duty: para out of range\n");
		return -EINVAL;
	}

	return mt5785_write_word(dev_data, MT5785_TX_PING_DUTY_ADDR, ping_duty);
}
...
static int mt5785_tx_set_min_duty(int duty, void *dev_data)
{
	if ((duty < MT5785_TX_PT_DUTY_MIN_DEFAULT) ||
		(duty > MT5785_TX_PT_DUTY_MAX_DEFAULT)) {
		hwlog_err("set_min_duty: para out of range\n");
		return -EINVAL;
	}

	return mt5785_write_byte(dev_data, MT5785_TX_PT_DUTY_MIN_ADDR, (u8)duty);
}

static int mt5785_tx_set_max_duty(int duty, void *dev_data)
{
	if ((duty < MT5785_TX_PT_DUTY_MIN_DEFAULT) ||
		(duty > MT5785_TX_PT_DUTY_MAX_DEFAULT)) {
	...
	return mt5785_write_byte(dev_data, MT5785_TX_PT_DUTY_MAX_ADDR, (u8)duty);
}
...
static int mt5785_tx_set_ploss(u8 rx_type, void *dev_data)
{
	int ret;
	u8 *tx_fod = NULL;
	struct mt5785_dev_info *di = dev_data;
	...
	if (di->tx_fod_fmt == WLTX_FOD_STATUS_FMT_OLD) {
		ret = mt5785_write_word(di, MT5785_TX_PLOSS_TH0_ADDR,
			(u16)di->tx_fod.ploss_th0);
		ret += mt5785_write_word(di, MT5785_TX_PLOSS_TH1_ADDR,
			(u16)di->tx_fod.ploss_th1);
		ret += mt5785_write_word(di, MT5785_TX_PLOSS_TH2_ADDR,
			(u16)di->tx_fod.ploss_th2);
		ret += mt5785_write_word(di, MT5785_TX_PLOSS_TH3_ADDR,
			(u16)di->tx_fod.ploss_th3);
	} else if (di->tx_fod_fmt == WLTX_FOD_STATUS_FMT_NEW) {
		tx_fod = wltx_get_fod_ploss_th(di->ic_type,
			rx_type, MT5785_TX_FOD_LEN);
		if (!tx_fod)
			return -EPERM;
		ret = mt5785_write_block(di, MT5785_TX_FOD_ADDR,
			tx_fod, MT5785_TX_FOD_LEN);
	} else {
		hwlog_err("tx_fod_fmt: get error\n");
		return -EINVAL;
	}

	ret += mt5785_write_byte(di, MT5785_TX_PLOSS_CNT_ADDR,
		(u8)di->tx_fod.ploss_cnt);
	ret += mt5785_write_dword_mask(di, MT5785_TX_CMD_ADDR,
		MT5785_TX_CMD_FOD_EN_MASK, MT5785_TX_CMD_FOD_EN_SHIFT,
		MT5785_TX_CMD_VALUE);
	if (ret) {
...
static int mt5785_tx_set_open_loop(bool enable, void *dev_data)
{
	u32 mask;
	u32 shift;

	hwlog_info("[set_open_loop] %s\n", enable ? "enable" : "disable");
	mask = enable ? MT5785_TX_CMD_OPEN_LOOP_EN_MASK : MT5785_TX_CMD_OPEN_LOOP_DIS_MASK;
	shift = enable ? MT5785_TX_CMD_OPEN_LOOP_EN_SHIFT: MT5785_TX_CMD_OPEN_LOOP_DIS_SHIFT;
	return mt5785_write_dword_mask(dev_data, MT5785_TX_CMD_ADDR,
		mask, shift, MT5785_TX_CMD_VALUE);
}
	...
	unsigned int client)
{
	...
	di->tx_init_para.ping_ocp_th = MT5785_TX_PING_OCP_TH_DEFAULT;
	di->tx_init_para.ping_duty = MT5785_TX_PING_DUTY;
...
static int mt5785_tx_set_init_para(struct mt5785_dev_info *di)
{
	...
	ret += mt5785_tx_set_ping_duty(di->tx_init_para.ping_duty, di);
	ret += mt5785_write_word(di, MT5785_TX_PING_OCP_TH_ADDR,
		di->tx_init_para.ping_ocp_th);
	ret += mt5785_tx_set_min_fop(MT5785_TX_MIN_FOP, di);
	ret += mt5785_tx_set_max_fop(MT5785_TX_MAX_FOP, di);
	ret += mt5785_tx_set_min_duty(di->duty_min, di);
	ret += mt5785_tx_set_max_duty(di->duty_max, di);
...
void mt5785_tx_mode_irq_handler(struct mt5785_dev_info *di)
{
	...
	if (di->irq_val & MT5785_TX_INT_TX_PING_OCP) {
		di->irq_val &= ~MT5785_TX_INT_TX_PING_OCP;
...
static struct wltx_ic_ops g_mt5785_tx_ic_ops = {
	.get_dev_node           = mt5785_dts_dev_node,
	...
	.set_min_duty           = mt5785_tx_set_min_duty,
	.set_max_duty           = mt5785_tx_set_max_duty,
	.set_ploss              = mt5785_tx_set_ploss,
	.set_open_loop          = mt5785_tx_set_open_loop,
};
