...
static const struct adapter_protocol_device_data g_hwufcs_dev_data[] = {
	...
	{ PROTOCOL_DEVICE_ID_SCHARGER_V600, "hisi_ufcs" },
...
int hwufcs_hard_reset_master_mask(u8 mask)
{
	struct hwufcs_ops *l_ops = hwufcs_get_ops();
	...
	if (!l_ops->hard_reset_mask) {
		hwlog_err("hw_reset_mask is null\n");
		return -EPERM;
	}

	hwlog_info("hard_reset_mask %d\n", mask);
	return l_ops->hard_reset_mask(l_ops->dev_data, mask);
}
