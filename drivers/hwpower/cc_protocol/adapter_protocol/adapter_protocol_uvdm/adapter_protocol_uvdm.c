...
static int hwuvdm_send_data(u32 *data, u8 cnt, bool wait_rsp, int type)
{
	int i;
	...
	l_ops->send_data(data, cnt, wait_rsp, l_ops->dev_data, type);
	return 0;
}

int hwuvdm_handle_vdo_data(u32 *data, int len, bool response,
	unsigned int retrys)
{
...
RETRY:
	/* false: need response */
	ret = hwuvdm_send_data(data, len, false, 0);
	if (ret)
		return -EPERM;
...
static struct adapter_protocol_ops adapter_protocol_hwuvdm_ops = {
	...
	.set_send_data = hwuvdm_send_data,
};
