...
int wireless_send_rx_product_id(unsigned int ic_type, int prot, u8 rx_product_id)
{
	...
	if (!l_ops->send_rx_product_id) {
		hwlog_err("send_rx_product_id is null\n");
		return -ENOTSUPP;
	}

	return l_ops->send_rx_product_id(ic_type, rx_product_id);
}
...
int wireless_send_fod_km(unsigned int ic_type, int prot, int km)
{
	...
	if (!l_ops->send_fod_km) {
		hwlog_err("send_fod_km is null\n");
		return -ENOTSUPP;
	}

	return l_ops->send_fod_km(ic_type, km);
}
...
int wireless_send_tx_vpa_lim(unsigned int ic_type, int prot, int val, int type)
{
	...
	if (!l_ops->send_tx_vpa_lim) {
		hwlog_err("send_tx_vpa_lim is null\n");
		return -ENOTSUPP;
	}

	return l_ops->send_tx_vpa_lim(ic_type, val, type);
}
...
int wireless_send_start_prevfod_check(unsigned int ic_type, int prot)
{
	...
	if (!l_ops->send_start_prevfod_chk)
		return -ENOTSUPP;

	return l_ops->send_start_prevfod_chk(ic_type);
}
...
int wireless_set_dev_attr(unsigned int ic_type, int prot, u16 attr)
{
	...
	if (!l_ops->set_dev_attr) {
		hwlog_err("set_dev_attr is null\n");
		return -ENOTSUPP;
	}

	return l_ops->set_dev_attr(ic_type, attr);
}
