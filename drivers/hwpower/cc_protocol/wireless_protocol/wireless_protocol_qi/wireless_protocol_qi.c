...
static struct wireless_protocol_tx_vpa_max {
	unsigned int qi_vpa_max;
	unsigned int vpa_max; /* unit:mv */
} const g_hwqi_tx_vpa_max[] = {
	{ 0, 18000 },
	{ 1, 36000 },
	{ 2, 54000 },
	{ 3, 72000 },
...
static int hwqi_send_rx_product_id(unsigned int ic_type, u8 rx_product_id)
{
	int retry = WIRELESS_RETRY_ONE;

	if (hwqi_send_msg_ack(ic_type, HWQI_CMD_SEND_RX_PRODUCT_ID,
		&rx_product_id, HWQI_PARA_RX_PRODUCT_ID_LEN, retry))
		return -EINVAL;

	hwlog_info("send_rx_product_id succ\n");
	return 0;
}
...
static int hwqi_send_fod_km(unsigned int ic_type, int km)
{
	int retry = WIRELESS_RETRY_ONE;

	if (hwqi_send_msg_ack(ic_type, HWQI_CMD_SEND_FOD_KM,
		(u8 *)&km, HWQI_PARA_FOD_KM_LEN, retry))
		return -EINVAL;
...
static int hwqi_send_tx_vpa_lim(unsigned int ic_type, int val, int type)
{
	u8 data[HWQI_PARA_SET_TX_VPA_LIM_LEN] = { 0 };
	int retry = WIRELESS_RETRY_ONE;

	data[HWQI_PARA_TX_VPA_LIM_TYPE] = (type >> 1);
	data[HWQI_PARA_TX_VPA_LIM_LTH] = val;

	if (hwqi_send_msg_ack(ic_type, HWQI_CMD_SET_TX_VPA_LIM,
		data, HWQI_PARA_SET_TX_VPA_LIM_LEN, retry))
		return -EINVAL;

	hwlog_info("send tx_vpa_lim succ\n");
...
static int hwqi_send_start_prevfod_check(unsigned int ic_type)
{
	int retry = WIRELESS_RETRY_ONE;

	if (hwqi_send_msg_ack(ic_type, HWQI_CMD_SEND_START_PREVFOD_CHK, NULL, 0, retry))
		return -EINVAL;

	hwlog_info("send_start_prevfod_check succ\n");
	return 0;
}
...
static int hwqi_convert_tx_type(u16 qi_type)
{
	int i;
	static struct {
		u16 qi_type;
		...
		{ HWQI_TX_TYPE_CP85, WLACC_TX_CP85 },
		{ HWQI_TX_TYPE_CP60, WLACC_TX_CP60 },
		{ HWQI_TX_TYPE_CP61, WLACC_TX_CP61 },
		{ HWQI_TX_TYPE_CP62_LX, WLACC_TX_CP62_LX },
		{ HWQI_TX_TYPE_CP62_XW, WLACC_TX_CP62_XW },
		{ HWQI_TX_TYPE_CP62R, WLACC_TX_CP62R },
		{ HWQI_TX_TYPE_CP39S, WLACC_TX_CP39S },
		{ HWQI_TX_TYPE_CP39S_HK, WLACC_TX_CP39S_HK },
		{ HWQI_TX_TYPE_CK30_LX, WLACC_TX_CK30_LX },
		{ HWQI_TX_TYPE_CK30_LP, WLACC_TX_CK30_LP },
		{ HWQI_TX_TYPE_W081, WLACC_TX_W081 },
		{ HWQI_TX_TYPE_K080, WLACC_TX_K080 },
	};

	for (i = 0; i < ARRAY_SIZE(type_map); i++) {
		if (qi_type == type_map[i].qi_type)
			return type_map[i].tx_type;
	}

	return WLACC_TX_ERROR;
}
...
static int hwqi_get_tx_type(unsigned int ic_type, int *type)
{
	...
	*type = hwqi_convert_tx_type(data);
...
static void hwqi_parse_tx_extra_capability(struct wlprot_acc_cap *cap, u8 *data)
{
	unsigned int i;
	unsigned int qi_vpa_max;

	cap->support_fan = data[HWQI_TX_EXTRA_CAP_ATTR1] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_FAN_MASK;
	cap->support_tec = data[HWQI_TX_EXTRA_CAP_ATTR1] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_TEC_MASK;
	cap->support_fod_status = data[HWQI_TX_EXTRA_CAP_ATTR1] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_QVAL_MASK;
	cap->support_get_ept = data[HWQI_TX_EXTRA_CAP_ATTR1] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_GET_EPT_MASK;
	cap->support_fop_range = data[HWQI_TX_EXTRA_CAP_ATTR1] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_FOP_RANGE_MASK;
	cap->support_fod_km = data[HWQI_TX_EXTRA_CAP_ATTR2] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_FOD_KM_MASK;
	cap->dc_pwr = data[HWQI_TX_EXTRA_CAP_ATTR2] &
		HWQI_PARA_TX_EXT_CAP_DC_PWR_MASK;

	qi_vpa_max = (data[HWQI_TX_EXTRA_CAP_ATTR2] & HWQI_PARA_TX_EXT_CAP_VPA_MAX_MASK) >>
		HWQI_PARA_TX_EXT_CAP_VPA_MAX_SHIFT;
	for (i = 0; i < ARRAY_SIZE(g_hwqi_tx_vpa_max); i++) {
		if (g_hwqi_tx_vpa_max[i].qi_vpa_max == qi_vpa_max) {
			cap->vpa_max = g_hwqi_tx_vpa_max[i].vpa_max;
			break;
		}
	}
}
...
static int hwqi_get_tx_capability(unsigned int ic_type, struct wlprot_acc_cap *cap)
{
	...
	hwqi_parse_tx_extra_capability(cap, data2);

	hwlog_info("tx_extra_cap: [support] fan=%d tec=%d fod_status=%d fop_range=%d vpa_max=%u km=%d type=%u\n",
		cap->support_fan, cap->support_tec, cap->support_fod_status,
		cap->support_fop_range, cap->vpa_max, cap->support_fod_km, ic_type);
...
static int hwqi_set_dev_attr(unsigned int ic_type, u16 attr)
{
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
...
static struct wireless_protocol_ops wireless_protocol_hwqi_ops = {
	...
	.send_rx_product_id = hwqi_send_rx_product_id,
	...
	.send_fod_km = hwqi_send_fod_km,
	.send_tx_vpa_lim = hwqi_send_tx_vpa_lim,
	.send_charge_mode = hwqi_send_charge_mode,
	.send_start_prevfod_chk = hwqi_send_start_prevfod_check,
	...
	.set_dev_attr = hwqi_set_dev_attr,
...
static int hwqi_handle_ask_packet_cmd_0x0a(unsigned int ic_type, u8 *data)
{
	...
	if (hwqi_send_fsk_ack_msg(ic_type))
		hwlog_err("ask_packet_cmd_0x0a: send fsk fail\n");
...
static int hwqi_handle_ask_packet_cmd_0x1a(unsigned int ic_type, u8 *data)
{
	...
	if (hwqi_send_fsk_ack_msg(ic_type))
		hwlog_err("ask_packet_cmd_0x1a: send fsk fail\n");

	return 0;
}

/* 0x28 + 0x6f + rx_id + checksum */
static int hwqi_handle_ask_packet_cmd_0x6f(unsigned int ic_type, u8 *data)
{
	power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
		POWER_NE_WLTX_ASK_RX_PRODUCT_ID, &data[HWQI_ASK_PACKET_DAT1]);
	if (hwqi_send_fsk_ack_msg(ic_type))
		hwlog_err("ask_packet_cmd_0x6f: send fsk fail\n");
...
static int hwqi_handle_ask_packet_cmd_0x3b(unsigned int ic_type, u8 *data)
{
	...
	if (tx_id == HWQI_HANDSHAKE_ID_HW) {
		if (((l_dev->dev_attr & HWQI_TX_SUPPORT_SLE) == HWQI_TX_SUPPORT_SLE) &&
			l_dev->acc_info.support_sle)
			data[HWQI_ASK_PACKET_DAT2] = HWQI_HANDSHAKE_ID_HW_SLE;
...
/* 0x18 + 0x6d + checksum */
static int hwqi_handle_ask_packet_cmd_0x6d(unsigned int ic_type)
{
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);

	if (!l_dev || ((l_dev->dev_attr & HWQI_TX_SUPPORT_PREVFOD_CHK) !=
		HWQI_TX_SUPPORT_PREVFOD_CHK))
		return -EINVAL;

	(void)hwqi_send_fsk_ack_msg(ic_type);
	power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
		POWER_NE_WLTX_START_PREVFOD_CHK, NULL);
	return 0;
}

/* 0x18 + 0x6e + checksum */
static int hwqi_handle_ask_packet_cmd_0x6e(unsigned int ic_type)
{
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
...
static int hwqi_handle_ask_packet_hdr_0x18(unsigned int ic_type, u8 *data)
{
	...
	case HWQI_CMD_SEND_START_PREVFOD_CHK:
		return hwqi_handle_ask_packet_cmd_0x6d(ic_type);
	case HWQI_CMD_RX_READY_MISSING:
		return hwqi_handle_ask_packet_cmd_0x6e(ic_type);
	default:
...
static int hwqi_handle_ask_packet_hdr_0x28(unsigned int ic_type, u8 *data)
{
	...
	case HWQI_CMD_SEND_RX_PRODUCT_ID:
		return hwqi_handle_ask_packet_cmd_0x6f(ic_type, data);
	default:
...
static int hwqi_handle_ask_packet_hdr_0x71(unsigned int ic_type, u8 *data)
{
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
	u16 manufacturer_id;
	u16 acc_id;
	...
	manufacturer_id = (data[HWQI_ASK_PACKET_DAT1] << POWER_BITS_PER_BYTE) |
		data[HWQI_ASK_PACKET_DAT2];
	acc_id = (data[HWQI_ASK_PACKET_DAT3] << POWER_BITS_PER_BYTE) |
		data[HWQI_ASK_PACKET_DAT4];

	if ((manufacturer_id == HWQI_PARA_MANUFATURER_ID_HW) && (acc_id == HWQI_PARA_ACC_SUPPORT_SLE))
		l_dev->acc_info.support_sle = 1;
	else
		l_dev->acc_info.support_sle = 0;

	if ((l_dev->dev_attr & HWQI_TX_SUPPORT_SLE) == HWQI_TX_SUPPORT_SLE)
		power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
			POWER_NE_WLTX_SUPPORT_SLE, &l_dev->acc_info.support_sle);
	power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
		POWER_NE_WLTX_RX_QI_ID, &manufacturer_id);
	return 0;
}
...
static int hwqi_handle_ask_packet_data(unsigned int ic_type, u8 *data)
{
	...
	case HWQI_CMD_GET_ID_PKT:
		return hwqi_handle_ask_packet_hdr_0x71(ic_type, data);
	default:
