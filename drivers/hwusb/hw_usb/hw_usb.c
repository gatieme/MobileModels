...
static ssize_t hw_usb_monitor_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	hw_usb_monitor_proc_cmdbuf(buf, size);

	return size;
}

static ssize_t hw_usb_monitor_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	if (!g_hw_usb_di)
		return  -ENODEV;
	...
	return scnprintf(buf, PAGE_SIZE,
		"usb_monitor_flag=%u\ndevspd=%d\nhubspd=%d\n",
		g_hw_usb_di->usb_monitor_flag,
		g_hw_usb_di->max_dev_spd, g_hw_usb_di->max_hub_spd);
}
#endif
...
#ifdef CONFIG_HUAWEI_USB_VENDOR
static DEVICE_ATTR(usb_monitor, 0660, hw_usb_monitor_show, hw_usb_monitor_store);
#endif
static struct attribute *hw_usb_ctrl_attributes[] = {
	&dev_attr_usb_speed.attr,
	&dev_attr_usb_event.attr,
#ifdef CONFIG_HUAWEI_USB_VENDOR
	&dev_attr_usb_monitor.attr,
#endif
	NULL,
};
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_OTG_OCP_CHECK_START:
		hw_usb_monitor_handle_event(HW_USB_EVENT_OTG_ON);
#endif
		break;
	default:
		break;
...
#ifdef CONFIG_HUAWEI_USB_VENDOR
	hw_usb_monitor_init(g_hw_usb_di);
#endif
	return 0;

#ifdef CONFIG_HUAWEI_USB_VENDOR
otg_notifier_regist_fail:
	(void)power_event_bnc_unregister(POWER_BNT_CONNECT, &di->connect_nb);
connect_notifier_regist_fail:
	(void)power_event_bnc_unregister(POWER_BNT_HW_USB, &di->hw_usb_nb);
#endif
