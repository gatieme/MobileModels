...
int hcfgk_set_cfg(struct file *file, const void __user *arg)
{
	...
	if (g_ctx.cfg_feature != HCFG_FEATURE_VERSION) {
		spin_unlock(&lock);
	...
	err = memset_s(user_table, mem_size, 0, mem_size);
	if (err != EOK) {
		pr_err("%s %d: memset_s failed, err = %d\n",
	...
	tmp = g_ctx.user_table;
	g_ctx.user_table = user_table;
	user_table = tmp;
	g_ctx.mem_size = mem_size;
	g_ctx.entry_num = entry_num;

	t = g_ctx.user_table;
	g_ctx.table.entry = t->entry;
	g_ctx.table.data = t->data;

	/* make sure last byte in data is 0 terminated */
	g_ctx.table.len = len;
	g_ctx.table.data[len - 1] = '\0';
...
int zrhung_get_config(enum zrhung_wp_id wp, char *data, uint32_t maxlen)
{
	...
	if (!g_ctx.table.entry || !g_ctx.table.data ||
	    ((zrhung_is_id_valid(wp) == 0) && (g_ctx.flag == 0)))
		goto out;

	if (!data || (entry_id >= g_ctx.entry_num) || (maxlen == 0)) {
		ret = -EINVAL;
		goto out;
	}

	if (!g_ctx.table.entry[entry_id].valid) {
		ret = NO_CONFIG;
		goto out;
	}

	if (g_ctx.table.entry[entry_id].offset >= g_ctx.table.len) {
		ret = -EINVAL;
		goto out;
	}

	len = g_ctx.table.len - g_ctx.table.entry[entry_id].offset;
	if (len <= 0) {
		ret = -EINVAL;
		goto out;
	}

	err = strncpy_s(data, maxlen,
		g_ctx.table.data + g_ctx.table.entry[entry_id].offset,
		len > (maxlen - 1) ? (maxlen - 1) : len);
	if (err != EOK) {
		ret = -EFAULT;
		pr_err("%s %d: strncpy_s failed, err = %d\n",
			__func__, __LINE__, err);
		goto out;
	}
	ret = 0;
...
int hcfgk_ioctl_get_cfg(struct file *file, void __user *arg)
{
	...
	err = memset_s(&val, sizeof(val), 0, sizeof(val));
	if (err != EOK) {
		pr_err("%s %d: memset_s failed, err = %d\n",
			__func__, __LINE__, err);
		return -EFAULT;
...
static int xcollie_save_stack(struct task_struct *task, char *out, uint32_t len)
{
...
#else
	nr_entries = stack_trace_save_tsk(task, entries, MAX_STACK_DEPTH, skip);
#endif
	...
	err = memcpy_s(out, len, buf, MAX_LOG_SIZE);
	if (err != EOK) {
		pr_err("%s %d: memcpy_s failed, err = %d\n",
			__func__, __LINE__, err);
		goto err_mutex;
	}
	ret = 0;
