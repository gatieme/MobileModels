/*
 * Copyright (c) Honor Technologies Co., Ltd. 2019. All rights reserved.
...
/* dev path len */
#define IOSTAT_PATHLEN 64

#define IOSTAT_DEVSTAT_PATH "/sys/block/mmcblk0/stat"
#define IOSTAT_USERDATA_PATH "/dev/block/by-name/userdata"
...
static bool g_get_statpath = true; /*true:path not obtained, false:obtained path*/
static char g_statpath[IOSTAT_PATHLEN] = {0};
...
static struct sighand_struct *__lock_task_sighand_ko(struct task_struct *tsk,
					   unsigned long *flags)
{
...
static inline struct sighand_struct *lock_task_sighand_ko(struct task_struct *task,
						       unsigned long *flags)
{
	struct sighand_struct *ret;

	ret = __lock_task_sighand_ko(task, flags);
	(void)__cond_lock(&task->sighand->siglock, ret);
	return ret;
}

static inline void unlock_task_sighand_ko(struct task_struct *task,
						unsigned long *flags)
{
	...
	int whole)
{
	...
	if ((task == NULL) || (ion == NULL) || (task->signal == NULL))
		return ret;
	...
	if (whole && lock_task_sighand_ko(task, &flags)) {
		struct task_struct *t = task;

		task_io_accounting_add(&acct, &task->signal->ioac);
		while_each_thread(task, t)
		task_io_accounting_add(&acct, &t->ioac);
		unlock_task_sighand_ko(task, &flags);
	}
...
static void io_get_statpath(void)
{
	struct file *filp_userdata = NULL;
	char *devname = NULL;
	char *link_path = NULL;
	char devpath[IOSTAT_PATHLEN] = {0};
	char sd_devname[IOSTAT_PATHLEN] = {0};

	while (g_get_statpath) {
		filp_userdata = filp_open(IOSTAT_USERDATA_PATH, O_RDONLY, 0);
		if (IS_ERR(filp_userdata)) {
			pr_err("io_stat:ERR: open userdata path failed, ret:%lu\n",
				(unsigned long)filp_userdata);
			return;
		}

		link_path = d_path(&filp_userdata->f_path, devpath, IOSTAT_PATHLEN);
		devname = strrchr(link_path, '/');
		strncpy(sd_devname, devname+1, 3);
		sprintf(g_statpath, "/sys/block/%s/stat", sd_devname);
		g_get_statpath = false;
		filp_close(filp_userdata, NULL);
	}
...
static int io_dev_get_devstats(struct io_diskstats *pio_d)
{
	...
	if (IS_ERR(filp)) {
		io_get_statpath();
		if (strlen(g_statpath) == 0) {
			pr_err("io_stat:ERR: get stat path failed\n");
			return ret;
		}

		filp = filp_open(g_statpath, O_RDONLY, 0);
		if (IS_ERR(filp)) {
		pr_err("io_stat:ERR: open %s failed, ret:%lu\n",
			g_statpath, (unsigned long)filp);
			return ret;
		}
	}
#if KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE
...
#ifdef CONFIG_HONOR_IO_INFO_DEBUG
static void io_dev_get_cachestats(struct io_output_info *pio_inf)
{
	static struct pgcache_stats pgcache_old_d = {0};
	struct pgcache_stats pgcache_new_d = {0};

	pgcache_new_d.pgfile = global_node_page_state(NR_ACTIVE_FILE) +
		global_node_page_state(NR_INACTIVE_FILE);
	pgcache_new_d.free = global_zone_page_state(NR_FREE_PAGES);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0)
	pgcache_new_d.slab = global_node_page_state(NR_SLAB_RECLAIMABLE_B) +
		global_node_page_state(NR_SLAB_UNRECLAIMABLE_B);
	pgcache_new_d.refault = global_node_page_state(WORKINGSET_REFAULT_FILE);
#else
 	pgcache_new_d.slab = global_node_page_state(NR_SLAB_RECLAIMABLE) +
 		global_node_page_state(NR_SLAB_UNRECLAIMABLE);
 	pgcache_new_d.refault = global_node_page_state(WORKINGSET_REFAULT);
#endif

	pio_inf->pgstat.pgfile = PAGETOKB(pgcache_new_d.pgfile);
	pio_inf->pgstat.free = PAGETOKB(pgcache_new_d.free);
	pio_inf->pgstat.slab = PAGETOKB(pgcache_new_d.slab);
	pio_inf->pgstat.refault = PAGETOKB(pgcache_new_d.refault - pgcache_old_d.refault);

	pgcache_old_d = pgcache_new_d;
...
static cputime64_t io_get_idle_time(int cpu)
{
	cputime64_t idle;
...
#else
static u64 io_get_idle_time(int cpu)
{
	u64 idle;
	u64 idle_time = -1ULL;
...
#ifdef CONFIG_HONOR_IO_INFO_DEBUG
	trace_io_info_debug(pio_inf->read_kb, pio_inf->write_kb, &pio_inf->pgstat);
#endif
...
#ifdef CONFIG_HONOR_IO_INFO_DEBUG
	io_dev_get_cachestats(&g_io_inf); // pgcache stat
#endif
