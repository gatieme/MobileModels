...
static inline int arm_smmu_rpm_get(struct arm_smmu_device *smmu)
{
	...
	if (pm_runtime_enabled(smmu->dev)) {
		trace_printk("If pm_runtime_enabled: pm_runtime_resume_and_get\n");
		atomic_inc(&smmu->usage_count);
		trace_printk("usage count = %d, name = %s\n",atomic_read(&smmu->usage_count), dev_name(smmu->dev));
...
static inline void arm_smmu_rpm_put(struct arm_smmu_device *smmu)
{
	trace_printk("func = %s\n", __func__);
	if (pm_runtime_enabled(smmu->dev)) {
		trace_printk("put_autosuspend\n");
		pm_runtime_mark_last_busy(smmu->dev);
		pm_runtime_put(smmu->dev);
		atomic_dec(&smmu->usage_count);
		trace_printk("usage count = %d, name = %s\n",atomic_read(&smmu->usage_count), dev_name(smmu->dev));
	}
}
...
static void arm_smmu_destroy_domain_context(struct iommu_domain *domain)
{
	...
	trace_printk("arm_smmu_rpm_get: ret = %d\n", ret);
	if (ret < 0)
		return;
...
static struct iommu_device *arm_smmu_probe_device(struct device *dev)
{
	...
	trace_printk("arm_smmu_rpm_put: ret = %d\n", ret);
	arm_smmu_rpm_put(smmu);
...
static int __maybe_unused arm_smmu_runtime_resume(struct device *dev)
{
	...
	if (ret) {
		trace_printk("arm_smmu_power_on: ret = %d\t name = %s\n", ret, dev_name(smmu->dev));
		return ret;
	}
	arm_smmu_device_reset(smmu);
...
static int __maybe_unused arm_smmu_runtime_suspend(struct device *dev)
{
	...
	trace_printk("func = %s\t name = %s\n", __func__, dev_name(smmu->dev));
	clk_bulk_disable(smmu->num_clks, smmu->clks);
...
static int __maybe_unused arm_smmu_pm_resume(struct device *dev)
{
	...
	if (pm_runtime_suspended(dev)) {
		trace_printk("If pm_runtime_suspended is true\n");
		return 0;
	}

	ret = arm_smmu_runtime_resume(dev);
	if (ret) {
		trace_printk("arm_smmu_runtime_resume:  ret = %d\n", ret);
		clk_bulk_unprepare(smmu->num_clks, smmu->clks);
	}
