...
static void mm_iommu_flush_ttwc(struct device *dev,
	struct iommu_domain *domain);
				...
				unsigned long align)
{
	 ...
	 */
	if (!start || !size) {
		WARN(1, "iova start or size should not be 0!\n");
		return NULL;
	}
				...
				struct iova_dom *iova_dom)
{
	...
	while (*p) {
		...
		if (entry->key == iova_dom->key)
			pr_err("%s: ERROR! dmabuf-iova mapping already exists!\n"
				   "iova=0x%lx, size=0x%lx, key=0x%llx, pid=%d, comm=%s\n",
				   __func__, entry->iova, entry->size, entry->key, entry->pid, entry->comm);
...
void mm_iova_dom_info(struct device *dev)
{
	...
	for (n = rb_first(&cookie->iova_root); n; n = rb_next(n)) {
		...
		pr_err("%s: iova:0x%lx size:0x%lx, pid:%d\n",
			dev_name(iova_dom->dev), iova_dom->iova, iova_dom->size, iova_dom->pid);
	}
	spin_unlock(&cookie->iova_lock);

	pr_err("domain %s total size: %lu", dev_name(dev), total_size);
}
				...
				unsigned long *out_size)
{
	...
	if (prot & NO_SLEEP) {
		prot &= ~NO_SLEEP;
		map_size = iommu_map_sg_atomic(domain, iova, sgl, nents, prot);
	} else {
...
static int do_iommu_unmap_sg(struct device *dev, struct iommu_domain *domain,
	size_t iova_size, unsigned long iova, bool is_free_iova)
{
	size_t unmap_size;
	int ret;
	...
	/*
	 * flush tlb to prevent the problem that the tlb is hit
	 * and unknown addresses are accessed after pgtable free
	 */
	if (dev)
				...
				struct iommu_domain *domain)
{
	...
	if (fwspec->ops->flush_ttwc)
		fwspec->ops->flush_ttwc(domain);
#endif
#endif
}
...
static struct iova_dom *__init_iova_dom(struct dma_buf *dmabuf)
{
	...
	if (strncpy_s(iova_dom->comm, TASK_COMM_LEN, current->comm, TASK_COMM_LEN) != EOK)
					     ...
					     struct sg_table *sgt)
{
	...
	*out_size = iova_dom->size;
	...
	if (is_bg_dmabuf(dmabuf))
		(void)record_bg_buffer_iova_info(dmabuf, dev, iova, iova_dom->size, prot);
#endif
	return iova;
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
static unsigned long __mm_iommu_map_bg_dmabuf(struct device *dev,
	struct dma_buf *dmabuf, int prot, unsigned long *out_size, struct sg_table *sgt)
{
	unsigned long iova;
	bool is_down_read = bg_dmabuf_down_read(dmabuf);

	if (is_bg_dmabuf_release(dmabuf)) {
		if (is_down_read)
			bg_dmabuf_up_read(dmabuf);
		dev_err(dev, "%s buffer has been already release\n", __func__);
		return 0;
	}

	iova = __mm_iommu_map_dmabuf(dev, dmabuf, prot, out_size, NULL);
...
static int __mm_iommu_unmap_bg_dmabuf(struct device *dev,
	struct dma_buf *dmabuf, unsigned long iova,
	struct iommu_domain *domain, struct iova_dom *iova_dom,
	struct mm_dom_cookie *cookie, bool is_delete_all)
{
	int ret;
	bool is_down_read = bg_dmabuf_down_read(dmabuf);

	if (is_bg_dmabuf_release(dmabuf)) {
		mm_iommu_free_iova(cookie->iova_pool, iova, iova_dom->size);
		pr_info("dmabuf has been release by bg_buffer, iova-0x%lx size-0x%lx\n",
			iova, iova_dom->size);
		ret = 0;
	} else {
		ret = do_iommu_unmap_sg(dev, domain, iova_dom->size, iova, true);
	}

	delete_bg_buffer_iova_info(dmabuf, dev, iova, is_delete_all);
	...
	unsigned long iova, int prot, unsigned long *out_size)
{
	...
	size_t iova_size;
	size_t map_size;
	int _prot = (int)((unsigned int)prot | (IOMMU_READ | IOMMU_WRITE));

	if (!dev || !dmabuf || !out_size || !sgt) {
	...
	iova_size = mm_iommu_sg_size_get(sgt->sgl, nents);
	map_size = iommu_map_sg(domain, iova, sgt->sgl, nents, _prot);
	if (map_size != iova_size) {
		pr_err("map Fail! iova 0x%lx, iova_size 0x%zx\n",
			iova, iova_size);
		return 0;
...
EXPORT_SYMBOL(kernel_iommu_map_bg_dmabuf);
	...
	struct dma_buf *dmabuf, unsigned long iova)
{
	...
	ret = do_iommu_unmap_sg(dev, domain, iova_dom->size, iova, false);
	return ret;
}
EXPORT_SYMBOL(kernel_iommu_unmap_bg_dmabuf);
#else
static unsigned long __mm_iommu_map_bg_dmabuf(struct device *dev,
					     struct dma_buf *dmabuf, int prot,
					     unsigned long *out_size,
...
/**
 * kernel_iommu_map_dmabuf() - Map ION buffer's dmabuf to iova
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	if (is_bg_dmabuf(dmabuf))
		return __mm_iommu_map_bg_dmabuf(dev, dmabuf, prot, out_size, NULL);
	else
...
/**
 * kernel_iommu_unmap_dmabuf() - Unmap ION buffer's dmabuf and iova
 * @dev: master's device struct
 * @dmabuf: ION buffer's dmabuf, must be allocated by ION
 * @iova: iova which get by kernel_iommu_map_dmabuf()
 *
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	if (is_bg_dmabuf(dmabuf))
		ret = __mm_iommu_unmap_bg_dmabuf(dev, dmabuf, iova,
			domain, iova_dom, cookie, false);
	else
		ret = do_iommu_unmap_sg(dev, domain, iova_dom->size, iova, true);
#else
	ret = do_iommu_unmap_sg(dev, domain, iova_dom->size, iova, true);
...
static int mm_iommu_unmap_sg_check(struct iommu_domain *domain,
				size_t iova_size, unsigned long iova)
{
	...
	cookie = domain->iova_cookie;
	if (!cookie) {
		pr_err("%s, iommu domain cookie is null!\n", __func__);
		return -EINVAL;
	}
	...
	for (n = rb_first(&cookie->iova_root); n; n = rb_next(n)) {
		...
		/*lint -e548 */
		if (is_overlap) {
			pr_err("%s, iova_dom iova: 0x%lx, size: 0x%lx; iova: 0x%lx, size: 0x%lx\n",
...
unsigned long mm_viommu_map_entry(struct device *dev, struct map_entry *m_entry,
	int entry_nums, size_t size, int prot, unsigned long *out_size)
{
	int i, ret;
	unsigned long iova;
	size_t iova_size = 0;
	...
	if (!dev) {
		pr_err("%s, input Err! Dev is NULL\n", __func__);
		return 0;
	}

	if (!m_entry) {
		pr_err("%s, input Err! map entry is NULL\n", __func__);
		return 0;
	}

	if (!size || !entry_nums) {
		pr_err("%s, map size: 0x%lx, entry_nums: %d\n",
					__func__, size, entry_nums);
		return 0;
	}

	domain = iommu_get_domain_for_dev(dev);
	if (!domain) {
		pr_err("%s, Dev(%s) has no iommu domain!\n",
	...
	cookie = domain->iova_cookie;
	if (!cookie) {
		dev_err(dev, "%s, map entry iova_cookie is null!\n", __func__);
		return 0;
	}

	if (!cookie->vm_iova_pool) {
		dev_err(dev, "%s, map entry vm_iova_pool is null!\n", __func__);
		return 0;
	}

	iova_size = ALIGN(size, PAGE_SIZE);
	iova = mm_iommu_alloc_iova(cookie->vm_iova_pool, iova_size,
		cookie->vm_iova.iova_align);
	if (!iova) {
		dev_err(dev, "%s, alloc iova failed! size 0x%lx\n",
			__func__, size);
		return 0;
	}
	prot |= IOMMU_READ | IOMMU_WRITE;
	...
	for (i = 0; i < entry_nums; i++) {
		ret = iommu_map(domain, iova + mapped_size,
					m_entry[i].pa, m_entry[i].size, prot);
		if (ret) {
			pr_err("%s, map fail! i = %d,  entry_nums = %d\n",
				__func__, i, entry_nums);
			goto err;
		}
		mapped_size += m_entry[i].size;
	}

	if (mapped_size != iova_size)  {
		pr_err("%s, map fail! iova 0x%lx, iova_size 0x%zx, mapped_size 0x%lx\n",
			__func__, iova, iova_size, mapped_size);
		goto err;
	}
...
err:
	...
	mm_iommu_free_iova(cookie->vm_iova_pool, iova, iova_size);
	return 0;
}
EXPORT_SYMBOL(mm_viommu_map_entry);

int mm_viommu_unmap(struct device *dev, unsigned long iova, size_t size)
{
	struct iommu_domain *domain = NULL;
	size_t iova_size;
	size_t unmap_size;
	...
	if (!iova) {
		dev_err(dev, "%s, input err! iova is 0\n", __func__);
		return -EINVAL;
	}

	if (!size) {
		dev_err(dev, "%s, input err! size is 0\n", __func__);
		return -EINVAL;
	}
	...
	if (!cookie->vm_iova_pool) {
		dev_err(dev, "%s, vm_iova_pool is null!\n", __func__);
		return 0;
	}
	...
	mm_iommu_free_iova(cookie->vm_iova_pool, iova, size);

	return 0;
}
EXPORT_SYMBOL(mm_viommu_unmap);
#else
static int mm_iommu_unmap_sg_check(struct iommu_domain *domain,
				size_t iova_size, unsigned long iova)
{
				...
				unsigned long iova)
{
	...
	ret = mm_iommu_unmap_sg_check(domain, iova_size, iova);
	if (ret) {
		dev_err(dev, "%s, unmap_sg check fail!\n", __func__);
		return ret;
	}

	ret = do_iommu_unmap_sg(dev, domain, iova_size, iova, true);
				...
				struct iommu_domain *domain, size_t iova_size)
{
	...
	spin_lock_irqsave(&cookie->lazy_free->lock, flag);
	...
	if (i >= pages_num) {
		spin_unlock_irqrestore(&cookie->lazy_free->lock, flag);
				...
				size_t size, int prot)
{
	...
	if (prot & NO_SLEEP) {
		prot &= ~NO_SLEEP;
		ret = iommu_map_atomic(domain, iova, paddr, size, prot);
	} else {
...
int mm_iommu_unmap(struct device *dev, unsigned long iova, size_t size)
{
	 ...
	 * flush tlb to prevent the problem that the tlb is hit
	 * and unknown addresses are accessed after pgtable free
	 * LAZY_FREE should not flush tlb because of performance
	 */
...
#ifdef CONFIG_BG_DMABUF_RECLAIM
	if (is_bg_dmabuf(dmabuf))
		ret = __mm_iommu_unmap_bg_dmabuf(NULL, dmabuf, iova,
			cookie->domain, iova_dom, cookie, true);
	else
		ret = do_iommu_unmap_sg(NULL, cookie->domain, iova_dom->size, iova, true);
#else
	ret = do_iommu_unmap_sg(NULL, cookie->domain, iova_dom->size, iova, true);
#endif
	if (ret)
...
static inline void __dmabuf_release_iommu_lpae(struct dma_buf *dmabuf)
{
#ifdef CONFIG_MM_IOMMU_LPAE
...
#ifdef CONFIG_MM_VIRTIO_IOMMU
	dmabuf_release_viommu(dmabuf);
#endif
	__dmabuf_release_iommu_lpae(dmabuf);
...
#else
static inline int mm_idle_display_lb_attach(void *buffer,
		u32 plc_id)
{
	return 0;
}

static void mm_idle_display_lb_detach(void *buffer, u32 plc_id)
{
	return;
}
#endif
		...
		size_t size, u32 policy_id, struct dma_buf *dmabuf)
{
	...
	ret = mm_iommu_unmap(dev, iova, size);
...
#ifdef CONFIG_MM_RDA_SMMU
static bool is_dmabuf_check(struct device *dev, struct dma_buf *dmabuf)
{
	return true;
}
#else
static bool is_dmabuf_check(struct device *dev, struct dma_buf *dmabuf)
{
	...
	if (!is_dma_heap_dmabuf(dmabuf)) {
		dev_err(dev, "dmabuf is not dma heap buffer\n");
		return false;
	}
#endif
				...
				size_t l3size, size_t lbsize)
{
	...
	if (mm_idle_display_lb_attach(dmabuf->priv, policy_id))
		return 0;
...
err:
	...
	mm_idle_display_lb_detach(dmabuf->priv, policy_id);
...
/**
 * kernel_iommu_map_padding_dmabuf() - Map ION buffer's dmabuf to iova
 * @dev: master's device struct
 * @dmabuf: ION buffer's dmabuf, must be allocated by ION
...
EXPORT_SYMBOL(kernel_iommu_map_padding_dmabuf);
...
/**
 * kernel_iommu_unmap_padding_dmabuf() - Unmap ION buffer's dmabuf and iova
 * @dev: master's device struct
 * @dmabuf: ION buffer's dmabuf, must be allocated by ION
 * @padding_len: iova padding length, out_size = padding_len + dmabuf length
 * @iova: iova which get by kernel_iommu_map_dmabuf()
...
EXPORT_SYMBOL(kernel_iommu_unmap_padding_dmabuf);
#endif
