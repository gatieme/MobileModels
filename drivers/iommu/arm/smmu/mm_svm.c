...
static int invalid_tcu_cache(struct arm_smmu_device *smmu)
{
	...
	ret = atfd_mm_service_smmu_smc(NPU_SMMU_TCU_CACHE_INIT, 0, 0, 0);
	if (ret) {
		mm_svm_print(SVM_ERROR, "NPU_SMMU_TCU_CACHE_INIT FAIL!%s\n",
			__func__);
		return ret;
	}
...
int mm_svm_flush_cache(struct mm_struct *mm, unsigned long addr, size_t size)
{
	...
	if (!mm || !addr || !size) {
		mm_svm_print(SVM_TRACE, "%s flush cache failed, addr:0x%lx size:0x%zx\n",
		       __func__, addr, size);
		return -EINVAL;
	}
...
void mm_svm_show_pte_noncache(struct mm_svm *svm, unsigned long addr,
				size_t size)
{
	size_t step = SZ_4K;
	struct mm_struct *mm = NULL;
...
static int delete_from_svm_list(struct mm_svm *svm)
{
	struct list_head *pos = NULL;
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	return blocking_notifier_chain_register(&evt_notifier_list, n);
#else
...
{
	switch (oas) {
	case MM_SMMU_ADDR_SIZE_32:
		*reg |= (ARM_LPAE_TCR_PS_32_BIT << ARM_LPAE_TCR_IPS_SHIFT);
		break;
	case MM_SMMU_ADDR_SIZE_36:
		*reg |= (ARM_LPAE_TCR_PS_36_BIT << ARM_LPAE_TCR_IPS_SHIFT);
		break;
	case MM_SMMU_ADDR_SIZE_40:
		*reg |= (ARM_LPAE_TCR_PS_40_BIT << ARM_LPAE_TCR_IPS_SHIFT);
		break;
	case MM_SMMU_ADDR_SIZE_42:
		*reg |= (ARM_LPAE_TCR_PS_42_BIT << ARM_LPAE_TCR_IPS_SHIFT);
		break;
	case MM_SMMU_ADDR_SIZE_44:
		*reg |= (ARM_LPAE_TCR_PS_44_BIT << ARM_LPAE_TCR_IPS_SHIFT);
		break;
	case MM_SMMU_ADDR_SIZE_48:
				...
				void *cookie)
{
	...
	cfg->arm_lpae_s1_cfg.ttbr = smmu_domain->s1_cfg.cd.ttbr;
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static void mm_smmu_group_tlb_inv_walk(unsigned long iova, size_t size,
				size_t granule, void *cookie)
{
	mm_smmu_group_tlb_inv_range_nosync(iova, size, granule, false, cookie);
}

static void mm_smmu_group_tlb_inv_add_page(struct iommu_iotlb_gather *gather,
				unsigned long iova, size_t granule, void *cookie)
{
	mm_smmu_group_tlb_inv_range_nosync(iova, granule, granule, true, cookie);
}

static struct iommu_flush_ops arm_smmu_gather_ops = {
	.tlb_flush_all = mm_smmu_group_tlb_inv_context,
	.tlb_flush_walk = mm_smmu_group_tlb_inv_walk,
	.tlb_add_page = mm_smmu_group_tlb_inv_add_page,
};
#else
				...
				struct arm_smmu_domain *dom)
{
	...
	list_for_each_safe(p, n, &grp->smmu_list) {
		smmu = list_entry(p, struct arm_smmu_device, smmu_node);
		if (!smmu) {
			mm_svm_print(SVM_ERROR, "%s smmu invalid\n", __func__);
			continue;
		}

		if (smmu->status != SMMU_ENABLE) {
			mm_svm_print(SVM_ERROR, "%s smmu status invalid: %d\n",
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	desc->l2ptr = dma_alloc_coherent(smmu->dev, size, &desc->l2ptr_dma,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	strtab = dma_alloc_coherent(smmu->dev, size, &cfg->strtab_dma,
				     GFP_KERNEL | __GFP_ZERO);
#else
...
static struct mm_svm *get_svm_by_ssid(u64 ssid)
{
	struct list_head *pos = NULL;
	struct mm_svm *tmp = NULL;
	struct arm_smmu_domain *smmu_domain = NULL;
...
static void mm_smmu_master_write(struct arm_smmu_device *smmu, u32 value,
				u32 offset)
{
	writel_relaxed(value, smmu->base + MM_MASTER_0_BASE + offset);
	writel_relaxed(value, smmu->base + MM_MASTER_1_BASE + offset);
}

static int mm_smmu_master_init(struct arm_smmu_device *smmu)
{
	mm_smmu_master_write(smmu, 0, SMMU_MSTR_GLB_BYPASS);
	mm_smmu_master_write(smmu, MM_VAL_MASK, SMMU_MSTR_SMRX_START_0);
	mm_smmu_master_write(smmu, MM_VAL_MASK, SMMU_MSTR_SMRX_START_1);
	mm_smmu_master_write(smmu, MM_VAL_MASK, SMMU_MSTR_SMRX_START_2);
	return 0;
}
#endif
static int mm_smmu_intr_init(struct arm_smmu_device *smmu)
{
	...
	writel_relaxed(MM_VAL_MASK, smmu->base + SMMU_IRPT_CLR_NS);
...
static int device_probe_idr0(struct arm_smmu_device *smmu)
{
	...
	smmu->asid_bits =
		(reg & IDR0_ASID16) ?
		MM_SMMU_ID_SIZE_16 : MM_SMMU_ID_SIZE_8;
	smmu->vmid_bits =
		(reg & IDR0_VMID16) ?
		MM_SMMU_ID_SIZE_16 : MM_SMMU_ID_SIZE_8;
...
static int device_probe_idr1(struct arm_smmu_device *smmu)
{
	...
	if (smmu->ssid_bits > MM_SSID_MAX_BITS)
		smmu->ssid_bits = MM_SSID_MAX_BITS;

	smmu->sid_bits = (reg >> IDR1_SID_SHIFT) & IDR1_SID_MASK;
	/* we only need 64 ste entries now */
	if (smmu->sid_bits > MM_SID_MAX_BITS)
		smmu->sid_bits = MM_SID_MAX_BITS;
...
static int device_probe_idr5(struct arm_smmu_device *smmu)
{
	...
	case IDR5_OAS_32_BIT:
		smmu->oas = MM_SMMU_ADDR_SIZE_32;
		break;
	case IDR5_OAS_36_BIT:
		smmu->oas = MM_SMMU_ADDR_SIZE_36;
		break;
	case IDR5_OAS_40_BIT:
		smmu->oas = MM_SMMU_ADDR_SIZE_40;
		break;
	case IDR5_OAS_42_BIT:
		smmu->oas = MM_SMMU_ADDR_SIZE_42;
		break;
	case IDR5_OAS_44_BIT:
		smmu->oas = MM_SMMU_ADDR_SIZE_44;
		break;
	default:
		dev_info(smmu->dev,
 ...
 * when CONFIG_MM_SVM_STE_BYPASS is open,
...
int mm_smmu_tcu_init(struct arm_smmu_device *smmu)
{
	...
	pr_err("In mmi_smmu_tcu_set_ns\n");

	ret = atfd_mm_service_smmu_smc(NPU_SMMU_TCU_INIT_NS, 0, 0, 0);
	if (ret) {
		mm_svm_print(SVM_ERROR, "TCU INIT NS FAIL!%s\n", __func__);
		return ret;
	}

	pr_err("Out mm_smmu_tcu_set_ns\n");
...
{
	if (__smmu_master_end_check(smmu->base, MM_MASTER_0_BASE,
	...
	if (__smmu_master_end_check(smmu->base, MM_MASTER_1_BASE,
...
int mm_smmu_tcu_disable(struct arm_smmu_device *smmu)
{
	...
	ret = atfd_mm_service_smmu_smc(NPU_SMMU_TCU_DISABLE, 0, 0, 0);
	if (ret) {
		mm_svm_print(SVM_ERROR, "NPU_SMMU_TCU_DISABLE FAIL!%s\n",
			__func__);
		return ret;
	}
...
static int mm_smmu_group_dt_probe(struct platform_device *pdev)
{
	...
	grp->oas = MM_SMMU_ADDR_SIZE_48;
	grp->ias = MM_SMMU_ADDR_SIZE_48;

	grp->asid_bits = MM_SID_MAX_BITS;
	grp->ssid_bits = MM_SSID_MAX_BITS;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	grp->cdtab_cfg.cdtab =
		dma_alloc_coherent(grp->sgrp_dev, (size_t)size,
				    &grp->cdtab_cfg.cdtab_dma, GFP_KERNEL | __GFP_ZERO);
#else
	grp->cdtab_cfg.cdtab =
