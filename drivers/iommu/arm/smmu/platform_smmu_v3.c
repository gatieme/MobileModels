...
static inline void mm_tcu_inner_cache_bypass(struct arm_smmu_device *smmu)
{
	...
	sec_smmu_ops(smmu->smmuid, MM_TCU_TTWC_BYPASS_ON, SMC_DEFAULT_CMD);
}
...
static void arm_smmu_tcu_read_cmdq(struct arm_smmu_device *smmu, u32 cons_index)
{
	u32 q_index;
	u64 cmd[CMDQ_ENT_DWORDS];

	q_index = cons_index & CMDQ_MAX_INDEX;

	dev_err(smmu->dev, "CMD_INDEX[0x%x]:\n", q_index);
	queue_read(cmd, Q_ENT(&smmu->cmdq.q, q_index), smmu->cmdq.q.ent_dwords, CMDQ_ENT_DWORDS);
	dev_err(smmu->dev, "1:0x%x\n", cmd[0]);
	dev_err(smmu->dev, "2:0x%x\n", cmd[1]);

	q_index = q_index + CMD_NUMBER_NEED_READ > CMDQ_MAX_INDEX ? 0 : q_index + CMD_NUMBER_NEED_READ;
	dev_err(smmu->dev, "CMD_INDEX[0x%x]:\n", q_index);
	queue_read(cmd, Q_ENT(&smmu->cmdq.q, q_index), smmu->cmdq.q.ent_dwords, CMDQ_ENT_DWORDS);
	dev_err(smmu->dev, "1:0x%x\n", cmd[0]);
	dev_err(smmu->dev, "2:0x%x\n", cmd[1]);
...
static void mm_smmu_print_qic_media1_status(struct arm_smmu_device *smmu)
{
	dev_err(smmu->dev, "MEDIA1_CTRL_MSK2:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_CTRL_MSK2_OFFSET));
	dev_err(smmu->dev, "MEDIA1_CTRL_P1_0:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_CTRL_P1_0_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_0:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_0_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_1:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_1_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_2:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_2_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_6:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_6_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_7:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_7_OFFSET));
	dev_err(smmu->dev, "MEDIA1_STAT_P2_9:0x%x\n",
		readl_relaxed(smmu->media_crg_vbase + MEDIA1_STAT_P2_9_OFFSET));
}
...
static void arm_smmu_tcu_info_dump(struct arm_smmu_device *smmu)
{
	...
	arm_smmu_tcu_read_cmdq(smmu, reg2);
	...
	if(smmu->media_crg_vbase)
		mm_smmu_print_qic_media1_status(smmu);

	mm_smmu_tcu_dbg_diu(smmu->smmuid);

	node_status = mm_smmu_tcu_node_status(smmu->smmuid);
	if (smmu->tbu_info.tbu_base) {
		for (i = 0; i < smmu->tbu_info.tbu_nums; i++) {
			if ((node_status >> i) & 0x01) {
				pr_err("tbu_status:0x%x, tbu_name:%s\n",
				readl_relaxed(smmu->tbu_info.vbase[i] + SMMU_TBU_CRACK_OFFSET), smmu->tbu_info.tbu_name[i]);
				mm_smmu_get_tbu_dbg_status(i);
			}
		}
	}
}
...
void arm_smmu_cmdq_issue_cmd(struct arm_smmu_device *smmu,
				struct arm_smmu_cmdq_ent *ent)
{
	u64 cmd[CMDQ_ENT_DWORDS];
	unsigned long flags;
				...
				struct arm_smmu_s1_cfg *cfg)
{
	...
	for (i = 0; i < cfg->cd.ssid_nums; i++) {
		...
		val = (cfg->cd.ttbr & CTXDESC_CD_1_TTB0_MASK <<
			CTXDESC_CD_1_TTB0_SHIFT) | CTXDESC_CD_1_HAD0 | CTXDESC_CD_1_PBHA;
				...
				u32 sid, u32 ssid)
{
	...
	if (!cd) {
		pr_err("%s, not support stage1, cd entry is null!\n", __func__);
		return NULL;
	}
...
#else
static inline int arm_smmu_rdr_dump(void *dump_addr, unsigned int size)
{
	return 1;
}
#endif
...
static void arm_smmu_tlb_inv_context_dev(void *cookie, u32 ssid)
{
	...
	step = ssid - smmu_domain->s1_cfg.cd.ssid;
	WARN(step >= smmu_domain->s1_cfg.cd.ssid_nums,
		"step:%u >= cd.ssid_nums:%u\n", step, smmu_domain->s1_cfg.cd.ssid_nums);
	WARN(smmu_domain->s1_cfg.cd.asid + step >= 1 << smmu->asid_bits,
		"cd.asid:%u, asid_bits:%u\n", smmu_domain->s1_cfg.cd.asid, smmu->asid_bits);

	if (smmu_domain->stage == ARM_SMMU_DOMAIN_S1) {
		cmd.opcode	= CMDQ_OP_TLBI_NH_ASID;
		cmd.tlbi.asid	= smmu_domain->s1_cfg.cd.asid + step;
				  ...
				  size_t granule, void *cookie)
{
	struct arm_smmu_domain *smmu_domain = cookie;
	struct arm_smmu_device *smmu = smmu_domain->smmu;
#ifdef CONFIG_MM_IOMMU_ERRATUM_NHVA_FLUSH_TLB
	arm_smmu_tlb_inv_range_nosync(iova, size, granule, true, cookie);
	/* use leaf == false to invalid pgd/pmd only one time,
	 * so size and granule can be zero.
	 */
	arm_smmu_tlb_inv_range_nosync(iova, 0, 0, false, cookie);
#else
				...
				unsigned int ssid)
{
	...
	arm_smmu_tlb_inv_context_dev(smmu_domain, ssid);
	return 0;
}
...
static void arm_smmu_flush_tlb(struct device *dev, struct iommu_domain *domain)
{
	...
	/*
	 * Mutex should be held here because we might
	 * modify the domain->s1_cfg.cd.asid.
	 * If not, concurrent calls of this func might
	 * lead to omission of tlb-flush.
	 */
	...
	ssid_base = arm_smmu_get_ssid(dev);
	smmu_domain->s1_cfg.cd.ssid = ssid_base;
	smmu_domain->s1_cfg.cd.asid = master->asid_base;
	smmu_domain->s1_cfg.cd.ssid_nums = master->ssid_nums;

	for (i = 0; i < master->ssid_nums; i++)
		arm_smmu_tlb_inv_context_dev(smmu_domain, ssid_base + i);
...
static u32 mm_smmu_alloc_asid_range(struct arm_smmu_device *smmu, int span, int nr)
{
	u32 size;
	u32 asid;
	int i;
	...
	pr_info("%s: allocated asid_base = %u, nr = %u, asid_bitmap = 0x%lX\n",
		__func__, asid, nr, *((unsigned long *)smmu->asid_map));
	return asid;
}

static void mm_smmu_release_asid_range(struct arm_smmu_device *smmu, int idx, int nr)
{
	int i;

	mutex_lock(&smmu->asid_mutex);
	for (i = 0; i < nr; i++)
		clear_bit(idx + i, smmu->asid_map);
	mutex_unlock(&smmu->asid_mutex);
}
...
static void arm_smmu_domain_free(struct iommu_domain *domain)
{
	...
	if (smmu_domain->stage == ARM_SMMU_DOMAIN_S1) {
		...
		if (cfg->cdptr) {
			...
			mm_smmu_release_asid_range(smmu,
				cfg->cd.asid, cfg->cd.ssid_nums);
		}
	} else {
				...
				struct io_pgtable_cfg *pgtbl_cfg)
{
	...
	asid = mm_smmu_alloc_asid_range(smmu, smmu->asid_bits, cfg->cd.ssid_nums);
	if (asid < 0)
		return asid;
...
out_free_asid:
	mm_smmu_release_asid_range(smmu, asid, cfg->cd.ssid_nums);
	return ret;
}
				...
				struct task_struct *task)
{
	...
	ttbr = virt_to_phys(mm->pgd);

	/*
	 * We can not Alloc asid from smmu->asid_map at the moment,
	 * since smmu_domain is newly created and has not attached
	 * to any smmu device yet.
	 * The real asid will be alloced by __svm_smmu_handle_asid
	 * in arm_smmu_svm_domain_finalise_s1.
	 */
	asid = ASID(mm);
	mmput(mm);
	...
	pr_info("%s, ASID(mm):0x%lx,ttbr:0x%llx\n", __func__, asid, cfg->cd.ttbr);
	return 0;
}
	...
	struct arm_smmu_s1_cfg *cfg)
{
	...
	/* Alloc 1 asid for each svm task */
	asid = mm_smmu_alloc_asid_range(smmu_domain->smmu,
		smmu_domain->smmu->asid_bits, 1);
	if (asid < 0)
		return -EINVAL;
...
static u16 __svm_get_mm_domain_asid(struct mm_struct *mm)
{
	...
	struct arm_smmu_domain *smmu_domain = NULL;
	u16 asid;
	struct mutex *mm_svm_mutex = NULL;

	mm_svm_mutex = get_svm_mutex();
				...
				struct device *dev)
{
	...
	arm_smmu_bitmap_free(dev_smmu_domain->ssid_map, cfg->cd.ssid); /* release ssid */
	mm_smmu_release_asid_range(smmu_domain->smmu, cfg->cd.asid, 1); /* release asid */
...
static void arm_smmu_detach_dev(struct device *dev)
{
	...
	asid_base = master->asid_base;
	mm_smmu_release_asid_range(master->smmu, asid_base, master->ssid_nums);
}
				...
				struct device *dev)
{
	...
	if (smmu_domain->stage == ARM_SMMU_DOMAIN_BYPASS) {
		...
		ste->s1_cfg->cd.ssid = arm_smmu_get_ssid(dev);
		ste->s1_cfg->cd.ssid_nums = master->ssid_nums;
				...
				struct arm_smmu_domain *smmu_domain)
{
	...
	if (master->tlb_flush != ASID_FLUSH) {
		asid = mm_smmu_alloc_asid_range(smmu, smmu->asid_bits, master->ssid_nums);
		if (asid < 0)
			return asid;

		smmu_domain->s1_cfg.cd.asid = asid;
		pr_err("into %s, SSID_FLUSH mode need to allocate new asid:%u for this ssid:%d\n",
			__func__, asid,
			arm_smmu_get_ssid(dev)
			);
	}
	...
	if (!cookie) {
		...
		mm_smmu_release_asid_range(smmu, asid, master->ssid_nums);
		return -ENOMEM;
	}
...
out_unlock:
	master->asid_base = smmu_domain->s1_cfg.cd.asid;
	mutex_unlock(&smmu_domain->init_mutex);
	dev_err(dev, "%s: [sid:%u] attach to SMMU\n", __func__, master->sids[0]);
	return ret;
}
...
static void arm_smmu_get_media_crg(struct platform_device *pdev,
				struct arm_smmu_device *smmu)
{
	int ret;
	u32 media_crg;
	void *media_crg_vbase = NULL;
	struct device *dev = &pdev->dev;

	ret = of_property_read_u32(dev->of_node, "media-crg", &media_crg);
	if(ret)
		return;

	media_crg_vbase = ioremap(media_crg, SZ_4K);/*lint !e446*/
	if (!media_crg_vbase)
		return;
	smmu->media_crg_vbase = media_crg_vbase;

	pr_err("%s, media crg base:0x%x, media crg vbase:0x%pK",
			__func__, media_crg, media_crg_vbase);
}

static void arm_smmu_get_tbu_base(struct platform_device *pdev,
				struct arm_smmu_device *smmu)
{
	int ret, i;
	struct device *dev = &pdev->dev;

	if (of_property_read_bool(dev->of_node, "tbu_name")) {
		smmu->tbu_info.tbu_nums = of_property_count_strings(dev->of_node, "tbu_name");
		if (!smmu->tbu_info.tbu_nums) {
			return;
		} else if (smmu->tbu_info.tbu_nums > MAX_MEDIA_TBU_NUM) {
			return;
		}
		smmu->tbu_info.tbu_base = devm_kzalloc(dev,
			sizeof(u32) * smmu->tbu_info.tbu_nums, GFP_KERNEL);
		if (!smmu->tbu_info.tbu_base)
			return;

		ret = of_property_read_u32_array(dev->of_node, "tbu_base",
			(u32 *)smmu->tbu_info.tbu_base, smmu->tbu_info.tbu_nums);
		if (ret) {
			pr_err("[%s], get tbu_base fail in dts\n", __func__);
			return;
		}

		for (i = 0; i < smmu->tbu_info.tbu_nums; i++) {
			smmu->tbu_info.tbu_name[i] = devm_kzalloc(dev,
				MAX_MEDIA_TBU_NAME_LEN, GFP_KERNEL);
			if (!smmu->tbu_info.tbu_name[i])
				return;
		}

		ret = of_property_read_string_array(dev->of_node, "tbu_name",
			smmu->tbu_info.tbu_name, smmu->tbu_info.tbu_nums);
		if (ret < 0)
			pr_err("[%s], get tbu_name fail in dts\n", __func__);

		smmu->tbu_info.vbase = devm_kzalloc(dev,
			sizeof(void *) * smmu->tbu_info.tbu_nums, GFP_KERNEL);
		if (!smmu->tbu_info.vbase)
			return;

		for (i = 0; i < smmu->tbu_info.tbu_nums; i++) {
			smmu->tbu_info.vbase[i] = ioremap(smmu->tbu_info.tbu_base[i], SZ_4K);/*lint !e446*/
			pr_err("%s, tbu_name: %s base:0x%x",
				__func__, smmu->tbu_info.tbu_name[i], smmu->tbu_info.tbu_base[i]);
		}
	} else {
		pr_err("[%s], no tbu_name node\n", __func__);
	}
}
				...
				struct arm_smmu_device *smmu)
{
	...
	arm_smmu_get_media_crg(pdev, smmu);
	arm_smmu_get_tbu_base(pdev, smmu);
				...
				unsigned int req_bit, unsigned int ack_bit)
{
	...
	ret = readl_relaxed_poll_timeout_atomic(smmu->base + ack_off, reg,
		reg & ack_bit, 1, ARM_SMMU_POLL_TIMEOUT_US);
	if (ret) {
		pr_err("req[0x%x] req_bit[0x%x]: 0x%x, ack[0x%x] ack_bit[0x%x]: 0x%x\n",
				req_off, req_bit, readl_relaxed(smmu->base + req_off),
				ack_off, ack_bit, readl_relaxed(smmu->base + ack_off));
	}
	return ret;
}
...
static inline bool is_hsdt_smmuid(int smmuid)
{
	return (smmuid == SMMU_HSDT0 || smmuid == SMMU_HSDT1);
}

static inline void arm_smmu_tcu_unreset(struct arm_smmu_device *smmu)
{
	if (!is_media_normal_smmuid(smmu->smmuid) &&
		!is_hsdt_smmuid(smmu->smmuid))
		return;

	/* clk-on and unreset */
	sec_smmu_ops(smmu->smmuid, MM_TCU_CLK_ON, SMC_DEFAULT_CMD);

	/* set tcu unsec */
	sec_smmu_ops(smmu->smmuid, MM_TCU_CTL_UNSEC, SMC_DEFAULT_CMD);
}
...
static inline void arm_smmu_tcu_reset(struct arm_smmu_device *smmu)
{
	...
	/* clk-off and reset */
	sec_smmu_ops(smmu->smmuid, MM_TCU_CLK_OFF, SMC_DEFAULT_CMD);
}
...
int arm_smmu_tcu_poweron(struct arm_smmu_device *smmu)
{
	...
	if (smmu->status == ARM_SMMU_ENABLE) {
		smmu->cnt++;
		dev_err(smmu->dev, "%s, status is enable, cnt is %d\n", __func__, smmu->cnt);
		spin_unlock_irqrestore(&smmu->tcu_lock, flags);
		return 0;
	}
...
int arm_smmu_tcu_poweroff(struct arm_smmu_device *smmu)
{
	...
	if (--smmu->cnt) {
		dev_err(smmu->dev, "%s, status is still enable, cnt is %d\n", __func__, smmu->cnt);
		goto out_unlock;
	}
