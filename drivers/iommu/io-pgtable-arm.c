...
void *arm_get_pgd_from_ops(struct io_pgtable_ops *ops)
{
...
#ifdef CONFIG_MM_LB_GID_PBHA
	/*
	 * In the process of iommu_map_sg, gid(not pid) may be acquired by page private,
	 * and directly added to high address of pa.
	 * When PBHA enables, the gid will be added to bit 59~62, and the bitwidth of paddr
	 * will be larger than cfg->oas.
	 * That will lead to the failure of the check "if (WARN_ON(iaext || paddr >> cfg->oas))".
	 */
	pbha = paddr & PTE_LB_MASK;
	paddr &= ~(PTE_LB_MASK);
#endif
...
#ifdef CONFIG_MM_LB_GID_PBHA
	/*
	 * If both paddr and iommu_prot carrys gid info,
	 * check that they are the same
	 */
	if (WARN_ON(pbha && (prot & PTE_LB_MASK) && pbha != (prot & PTE_LB_MASK)))
		return -EINVAL;
	prot |= pbha;
#endif
				       ...
				       arm_lpae_iopte *ptep, size_t pgcount)
{
	...
	tablep = __arm_lpae_alloc_pages(tablesz, GFP_KERNEL, cfg);
	if (!tablep)
