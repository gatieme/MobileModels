 ...
 * Copyright (c) 2012-2020 Honor Device Co., Ltd.
 *
...
#if (defined(CONFIG_HONOR_UFS_DSM) && (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)))
#include "dsm_ufs.h"
#endif
#include "ufshcd.h"

#define IOMT_UFS_GET_CHUNKSIZE_OFFSIZE  10
	...
	struct Scsi_Host *scsi_host)
{
	...
	cmd->iomt_start_time.type = IOMT_LATNECY_HIGH_ACC;
	...
	struct scsi_cmnd *cmd)
{
	...
	if (!(iomt_host_info->latency_scatter.usertype_obtain_flag))
		return;
	...
	i = iomt_host_io_latency(iomt_host_info, &(cmd->iomt_start_time), dir, chunk_size);
...
#define	FLAG_DSM_UFS_IO_TIMEOUT		0x00200000
		pr_err("%s:%s(%d) ufs io timeout, judge slot = %u, "
			"opcode = 0x%x, lba = 0x%x, blocks = %u, ticks = %u, dir = %u",
			__func__, ufs_host_name(host), hba->dev_info.wmanufacturerid,
