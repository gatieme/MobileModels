 ...
 */
int create_log(char *log_name, int size)
{
	int ret;
	struct logger_log *log = NULL;
...
EXPORT_SYMBOL(create_log);

static void logger_delete(struct logger_log *current_log)
{
	if (current_log == NULL) {
		return;
	}
...
void destroy_logger(const char *name)
{
	...
	list_for_each_entry_safe(current_log, next_log, &log_list, logs) {
		if (strcmp(current_log->misc.name, name) == 0)
			logger_delete(current_log);
	}
}
EXPORT_SYMBOL(destroy_logger);
