...
int hwdubai_send_event(const char *domain, const char *event_name, enum hisysevent_type type, const char *message)
{
	...
	event = hisysevent_create(domain, "XPOWER_KERNEL_MESSAGE", type);
	if (!event) {
		pr_err("failed to create event");
		return -EINVAL;
	}
	ret = hisysevent_put_string(event, "TAG", event_name);
	if (ret != 0) {
		pr_err("failed to put tag sting to event, ret=%d", ret);
		hisysevent_destroy(&event);
		return ret;
	}
	ret = hisysevent_put_string(event, "MESSAGE", message);
	if (ret != 0) {
		pr_err("failed to put message sting to event, ret=%d", ret);
		hisysevent_destroy(&event);
		return ret;
	}
	ret = hisysevent_write(event);
	if (ret < 0)
		pr_err("failed to send hisysevent, ret=%d", ret);
...
int hwlog_send_hisysevent(const char *domain, const char *event_name, enum hisysevent_type type, const char *fmt, ...)
{
	int len;
	va_list args;
	char message[HW_DUBAI_MAX_MESSAGE_LEN];
	va_start(args, fmt);
	len = vscnprintf(message, HW_DUBAI_MAX_MESSAGE_LEN, fmt, args);
	va_end(args);
	hwdubai_send_event(domain, event_name, type, message);
	return 0;
}
EXPORT_SYMBOL(hwlog_send_hisysevent);
#endif
#endif
