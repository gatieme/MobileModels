...
do {                                                                           \
	if ((obj)->api_version == 0) {                                         \
		...
		hilog_err("cannot use v1 api %s after v2 api used", __func__); \
...
do {                                                                           \
	if ((obj)->api_version == 0) {                                         \
		...
		hilog_err("cannot use v2 api %s after v1 api used", __func__); \
		return INVALID_VERSION;                                        \
	}                                                                      \
} while (0)
...
int imonitor_set_param(struct imonitor_eventobj *event_obj, unsigned short param_id, long value)
{
	...
	if (!event_obj) {
		hilog_err("Bad param for %s", __func__);
		return -EINVAL;
	}

	if (param_id >= event_obj->params_count) {
		hilog_err("Bad param for %s, param_id: %d", __func__, param_id);
		return -EINVAL;
	}
	...
	case BIT_TYPE:
		if (((char)value) > 1) {
			hilog_err("Invalid value for bit type, value: %d", (int)value);
			return -EINVAL;
		}
		/* go through down */
	case TINYINT_TYPE:
	...
	case DATETIME_TYPE:
		if (value == 0) {
			hilog_err("Invalid value for datetime type");
		} else {
	...
	default:
		if ((type > BASE_TYPE_MAX) && (type <= VARCHAR_TYPE)) {
			hilog_err("type error: %d", type);
			return -EINVAL;
		}
		if (value == 0) {
			hilog_err("Invalid value for varchar type");
		} else {
			imonitor_set_param_string(event_obj, param_id, (char *)value);
		}
		break;
...
static int add_path(char **pool, int pool_len, const char *path)
{
	...
	if ((!path) || (path[0] == 0)) {
		hilog_err("Bad param %s", __func__);
		return -EINVAL;
	}

	if ((pool_len <= 0) || (pool_len > MAX_PATH_NUMBER)) {
		hilog_err("pool len is illegal");
		return -EINVAL;
	}
...
#else
			send_hievent(1, str_ptr, EVENT_INFO_PACK_BUF_LEN); // 1:not tail
#endif
...
#else
			send_hievent(0, str_ptr, left_buf_len); // 0:tail
#endif
			sent_cnt++;
			break;
		}
...
int imonitor_send_event(struct imonitor_eventobj *event_obj)
{
	...
	hilog_info("%s : %u", __func__, event_obj->event_id);
	vfree(str);
