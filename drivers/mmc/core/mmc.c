...
#include <platform_include/basicplatform/linux/mmc/emmc_rpmb.h>
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
	dsm_set_sdcard_device_info(card->cid.manfid, card->cid.serial);
#endif
...
static void mmc_select_card_type(struct mmc_card *card)
{
	...
	pr_debug("%s: eMMC mmc_avail_type=0X%x, hs_max_dtr=%d\n",
		 mmc_hostname(card->host), card->mmc_avail_type,
...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 4) {
		 ...
		 * consequently, same size to be erased should be divided to more
		 * pieces to do it, evently makes erase very slow. 
		 */
		if (card->cid.manfid == CID_MANFID_MICRON)
			ext_csd[EXT_CSD_TRIM_MULT] = 0x2;
	...
	if (card->ext_csd.rev >= 7) {
		...
		if(card->ext_csd.fwrev[0] != '\0') {
			pr_info("%s: %s:  eMMC FW version:%d \n",
...
int mmc_select_hs400(struct mmc_card *card)
{
	...
	pr_debug("%s: mmc_avail_type=%x, bus_width=%d\n", __func__,
		 card->mmc_avail_type, host->ios.bus_width);
	...
	if (card->ext_csd.strobe_enhanced_en &&
	    host->ops->enable_enhanced_strobe)
...
static int mmc_select_hs200(struct mmc_card *card)
{
	...
	if (err > 0) {
		...
		/*need to change voltage after set timing on zodiac plat*/
		if (card->mmc_avail_type & EXT_CSD_CARD_TYPE_HS200_1_2V)
	...
	struct mmc_card *oldcard)
{
	...
	pr_info(KERN_INFO "mmc init begin cmd0,caps is 0x%x \n",host->caps);
	...
	if (err) {
#ifdef CONFIG_HUAWEI_SDCARD_DSM
		if (!oldcard && sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
			dsm_client_record(sdcard_dclient, "mmc_send_op_cond failed, err = %d", err);
			dsm_client_notify(sdcard_dclient, DSM_SDCARD_HARDWARE_TIMEOUT_ERR);
		}
#endif
		goto err;
	}
	...
	pr_info(KERN_INFO "mmc init processing cmd2 MMC_ALL_SEND_CID\n");
	/*
	 * Fetch CID from card.
	 */
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
		if (!oldcard && sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
			dsm_client_record(sdcard_dclient,
				"mmc_send_cid failed, err = %d, resp[0] = %d", err, cid[0]);
...
#ifdef CONFIG_BOOTDEVICE
	if ((get_bootdevice_type() == BOOT_DEVICE_EMMC) &&
	    !strncmp(mmc_hostname(host), "mmc0", strlen("mmc0"))) {
	...
	if (card->ext_csd.strobe_enhanced) {
		if (host->caps3 & MMC_CAP3_ENHANCED_STROBE) {
	...
	if (mmc_card_hs200(card)) {
		...
		if (err) {
			pr_err("%s: mmc_select_hs400 execution"
				" failed\n", mmc_hostname(card->host));
			goto free_card;
		}
	 ...
	 */
	if ((host->caps2 & MMC_CAP2_CACHE_CTRL) && (card->ext_csd.cache_size > 0) &&
	    (mmc_screen_test_cache_enable(card))) {
...
static int _mmc_suspend(struct mmc_host *host, bool is_suspend)
{
	...
	if (err) {
		pr_err("%s: mmc suspend zodiac operation failed.\n", __func__);
		goto out;
	}
...
static int _mmc_resume(struct mmc_host *host)
{
	...
	if (!mmc_card_keep_power(host)) {
		...
		if (mmc_card_removable_mmc(host->card)) {
			/* nano MMC card handle */
			while (sdcard_retry-- > 0) {
				mdelay(10);
				err = mmc_init_card(host, host->card->ocr, host->card);
				pr_err("%s invoke mmc_init_card() sdcard_retry[%d] err=%d\n",
						__func__, sdcard_retry, err);
				if (!err)
					break;
			}
...
int mmc_detect_mmc(struct mmc_host *host)
{
	...
	pr_err("%s:%d err=%d ocr=0x%08X bit[0:27]:0x%08X\n",
		__func__, __LINE__, err, ocr, ocr & OCR_REGISTER_BIT_0_TO_27_MASK);
...
err:
	...
	if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
		dsm_client_record(sdcard_dclient,
			"MMC card init failed, freq = %d, err = %d", host->f_init, err);
		dsm_client_notify(sdcard_dclient, DSM_SDCARD_HARDWARE_TIMEOUT_ERR);
	}
#endif
