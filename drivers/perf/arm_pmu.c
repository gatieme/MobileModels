...
static void armpmu_enable(struct pmu *pmu)
{
	...
	CALL_HCK_VH(lpcpu_pmu_arm_pmu_enable, armpmu, hw_events, &enabled, &vendor_flag);
	if (vendor_flag == 0)
...
int perf_num_counters(void)
{
	...
	CALL_HCK_VH(lpcpu_pmu_arm_pmu_per_cpu_num_events, __oprofile_cpu_pmu,
		    &max_events, &vendor_flag);
	if (vendor_flag == 0 && __oprofile_cpu_pmu != NULL)
...
static void cpu_pm_pmu_setup(struct arm_pmu *armpmu, unsigned long cmd)
{
	...
	CALL_HCK_VH(lpcpu_pmu_arm_pmu_per_cpu_num_events, armpmu, &num_events, &idx);
...
int armpmu_register(struct arm_pmu *pmu)
{
	...
	CALL_HCK_VH(lpcpu_pmu_arm_pmu_per_cpu_register, pmu, &vendor_flag, has_nmi);
	if (vendor_flag == 0)
