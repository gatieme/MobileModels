...
static struct task_struct *aisched_task = NULL;
bool is_aisched_task(void)
{
	return current == aisched_task ? true : false;
}

static int perf_ctrl_open(struct inode *inode, struct file *file)
{
	aisched_task = current;
	return 0;
}
