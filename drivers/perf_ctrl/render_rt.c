 ...
 * render_rt.c
 *
 * Copyright (c) 2019-2020 Honor Technologies Co., Ltd.
 *
...
err:
	...
	if (reason)
		printk_ratelimited(KERN_WARNING "[%s %d] fail to find the task comm=%s pid=%d\n",
				   __func__, __LINE__, task->comm, task->pid);
}
...
static inline void init_child_rthread(int index, struct task_struct *p)
{
    ...
    trace_android_rvh_get_walt_ravg_sum(p, &thread->last_walt_ravg_sum);
...
#if KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE
    trace_android_rvh_get_walt_ravg_sum(task, &curr_walt_ravg_sum);
#elif KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE
