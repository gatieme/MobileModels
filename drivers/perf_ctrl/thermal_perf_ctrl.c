...
static const char thermal_cdev_type_name[THERMAL_CDEV_MAX][THERMAL_NAME_LENGTH] = {
	CDEV_GPU_NAME,
...
static const char thermal_zone_name[THERMAL_ZONE_MAX][THERMAL_NAME_LENGTH] = {
	SOC_THERMAL_NAME,
	BOARD_THERMAL_NAME,
};
...
static int get_ipa_freq_limit(struct ipa_stat *status)
{
	status->cluster0 = ipa_freq_limit(0, UINT_MAX);
	status->cluster1 = ipa_freq_limit(1, UINT_MAX);
	if (ipa_get_actor_id("gpu") == 3) {
		status->cluster2 = ipa_freq_limit(2, UINT_MAX);
		status->gpu = ipa_freq_limit(3, UINT_MAX);
	} else {
		status->cluster2 = 0;
		status->gpu = ipa_freq_limit(2, UINT_MAX);
	}
...
int perf_ctrl_get_ipa_stat(void __user *uarg)
{
	...
	ret = get_ipa_freq_limit(&ipa_stat_val);
...
static int get_thermal_cdev_power(struct thermal_cdev_power *tcp)
{
	...
	memset(tcp->cdev_power, 0, sizeof(tcp->cdev_power));
