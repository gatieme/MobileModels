...
#ifdef CONFIG_PINCRTL_VIRTUAL
		CALL_HCK_VH(pinctrl_vm_set_mux, vals->reg, vals->val);
#else
			    ...
			    struct pinctrl_gpio_range *range, unsigned pin)
{
	...
	list_for_each_safe(pos, tmp, &pcs->gpiofuncs) {
		...
		offset = pcs_pin_reg_offset_get(pcs, pin);
#ifdef CONFIG_PINCRTL_VIRTUAL
		CALL_HCK_VH(pinctrl_vm_set_mux, pcs->base + offset, frange->gpiofunc);
#else
		if (pcs->bits_per_mux) {
...
#ifdef CONFIG_PINCRTL_VIRTUAL
	pcs->read = pcs_virt_read;
	pcs->write = pcs_virt_write;
#else
	switch (pcs->width) {
	case 8:
		pcs->read = pcs_readb;
