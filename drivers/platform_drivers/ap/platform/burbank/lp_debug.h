...
struct system_dvs {
 int bias[SYSTEM_VOLT_LEVEL_MAX];
 int bias_r[SYSTEM_VOLT_LEVEL_MAX];
 unsigned char init_lvl;
 unsigned char init_vol;
};
struct system_avs {
 unsigned short volt[AVS_PAGE_MAX][SYSTEM_VOLT_LEVEL_MAX];
 unsigned short volt_r[AVS_PAGE_MAX][SYSTEM_VOLT_LEVEL_MAX];
 unsigned int avs_status;
};
struct system_volt {
 struct system_dvs dvs;
 struct system_avs avs;
};
struct system_avs_bias {
 int bias[SYSTEM_VOLT_LEVEL_MAX];
};
struct system_avs_para_to_bl2 {
 unsigned int version_type;
 struct system_avs_bias peri_self_healing;
};
struct lp_share_image_type {
 unsigned int type[ULTRA_VOLT_MAX];
};
#define AVS_MAX_PROFILE_NUM 15
typedef struct LP_DBG_SHARED_S {
 ...
 unsigned int cluster_volt[6][AVS_MAX_PROFILE_NUM][3];
 unsigned short hpm_volt[3][6][17][3];
 unsigned int dieid[5];
 unsigned int l2_test_result;
 struct lp_share_image_type share_image_type;
 struct system_volt peri_data;
 struct system_volt ao_data;
 unsigned int health_grade_flag;
