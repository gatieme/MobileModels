...
static struct power_supply_attr power_supply_attrs[] = {
	...
	POWER_SUPPLY_ATTR(ID_VOLTAGE),
	POWER_SUPPLY_ATTR(CHARGE_TIME_REMAINING),
	POWER_SUPPLY_ATTR(CATHODE_TYPE),
	POWER_SUPPLY_ATTR(USB_HC),
	POWER_SUPPLY_ATTR(USB_OTG),
	POWER_SUPPLY_ATTR(CHARGE_ENABLED),
	POWER_SUPPLY_ATTR(CHARGE_COUNTER_EXT),
	POWER_SUPPLY_ATTR(BAT_QMAX),
	POWER_SUPPLY_ATTR(MANUFACTURE_YEAR),
	POWER_SUPPLY_ATTR(MANUFACTURE_MONTH),
	POWER_SUPPLY_ATTR(MANUFACTURE_DAY),
	POWER_SUPPLY_ATTR(SERIAL_COUNT),
	/* Properties of type `const char *' */
	POWER_SUPPLY_ATTR(MODEL_NAME),
	POWER_SUPPLY_ATTR(MANUFACTURER),
	POWER_SUPPLY_ATTR(SERIAL_NUMBER),
	POWER_SUPPLY_ATTR(BRAND),
};
...
static ssize_t power_supply_show_property(struct device *dev,
	...
	case POWER_SUPPLY_PROP_USB_TYPE:
		ret = power_supply_show_usb_type(dev, psy->desc,
						&value, buf);
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME ... POWER_SUPPLY_PROP_BRAND:
		ret = sprintf(buf, "%s\n", value.strval);
		break;
	default:
		ret = sprintf(buf, "%d\n", value.intval);
