...
#ifdef CONFIG_SET_REGULATOR_STATE_FACTORY
static int regulator_enable_set(struct regulator_dev *rdev, u64 val);
#endif
				   ...
				   size_t count)
{
	...
	if (regulator_enable_set(rdev, val)) {
		rdev_err(rdev, "%s: regulator set fail\n", __func__);
		return -EINVAL;
	}
	rdev_info(rdev, "%s: end !\n", __func__);
	return count;
}
...
static int regulator_enable_set(struct regulator_dev *rdev, u64 val)
{
	...
	rdev_info(rdev, "%s: enter !\n", __func__);
	if (!rdev) {
		rdev_err(rdev, "%s: rdev = null !\n", __func__);
		return -1;
	}

	regulator = regulator_get(NULL, rdev_name(rdev));
	if (IS_ERR(regulator)) {
		rdev_err(rdev, "debug regulator get failed, ret=%ld\n",
			PTR_ERR(regulator));
		return -1;
	}
