...
static int rasprobe_handler(honor_battery_capacity) (struct rasprobe_instance
							*ri,
							struct pt_regs *regs) {
	if (capacity == -1)
		return 0;
...
static int rasprobe_handler(honor_battery_current) (struct rasprobe_instance
							*ri,
							struct pt_regs *regs) {
	if (capacity == -1)
		return 0;
...
static int rasprobe_handler(honor_battery_voltage) (struct rasprobe_instance
							*ri,
							struct pt_regs *regs) {
...
rasprobe_define(honor_battery_capacity);
rasprobe_define(honor_battery_current);
rasprobe_define(honor_battery_voltage);
#endif

static struct rasprobe *probes[] = {
#ifdef CONFIG_HONOR_COUL
	&rasprobe_name(honor_battery_capacity),
	&rasprobe_name(honor_battery_current),
	&rasprobe_name(honor_battery_voltage),
#endif
};
