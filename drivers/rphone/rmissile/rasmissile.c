...
static int bind_thread(void *arg)
{
	struct sched_param param = {.sched_priority = MAX_RT_PRIO-1};
	sched_setscheduler(current, SCHED_FIFO, &param);
	while(1);
}
static int rmissile_bind(struct RMissile *prm)
{
	struct task_struct *bind_task;

	bind_task = kthread_create(bind_thread, prm, "rmissile_bind");
	if (IS_ERR(bind_task)) {
		pr_err("rmissile bind create kthread fail\n");
		return -1;
	}
	kthread_bind(bind_task, prm->para);
	wake_up_process(bind_task);
	return 0;
}
...
"disable dev's irqs. cmd format: ./rmissile devirq [vendorid] [deviceid]"},
		...
		{13, "bind", rmissile(bind), "bind task to cpu"},
	}
