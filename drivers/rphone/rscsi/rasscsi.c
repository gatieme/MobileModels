...
static int should_fail_check(struct fault_args *pfa, struct scsi_cmnd *scmd)
{
	...
	if (!pfa->gdisk)
		return 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
	if (!scsi_cmd_to_rq(scmd)) {
		tool_err("Scmd->request is NULL!\n");
		return 0;
	}
	if (pfa->gdisk != scsi_cmd_to_rq(scmd)->rq_disk && scsi_cmd_to_rq(scmd)->rq_disk) {
		tool_err("Gendisk different:%p,%p\n", pfa->gdisk,
			 scsi_cmd_to_rq(scmd)->rq_disk);
		return 0;
	}
	if (!scsi_cmd_to_rq(scmd)->rq_disk)
