...
#ifdef CONFIG_MAS_BLK
	if (scmd->request)
		mas_blk_latency_req_check(scmd->request, REQ_PROC_STAGE_SCSI_EH_INC);
#endif
...
#ifdef CONFIG_MAS_BLK
	if (scmd->request)
		mas_blk_latency_req_check(scmd->request, REQ_PROC_STAGE_SCSI_EH_ADD);
#endif
	...
	shost_for_each_device(sdev, shost) {
		...
		list_for_each_entry(scmd, work_q, eh_entry) {
			...
			if (scmd && scmd->request)
				mas_blk_latency_req_check(scmd->request, REQ_PROC_STAGE_SCSI_EH_STU);
#endif
...
#ifdef CONFIG_MAS_BLK
		if (scmd && scmd->request)
			mas_blk_latency_req_check(scmd->request,
...
#if defined(CONFIG_MAS_ORDER_PRESERVE) || defined(CONFIG_MAS_UNISTORE_PRESERVE)
int scsi_eh_send_request_sense(struct scsi_device *sdev)
{
	int ret;
	struct Scsi_Host *shost = sdev->host;
