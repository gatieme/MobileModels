...
atomic_t g_scsi_start_cnt = ATOMIC_INIT(0);
atomic_t g_scsi_done_cnt = ATOMIC_INIT(0);
#endif
...
static int scsi_dispatch_cmd(struct scsi_cmnd *cmd)
{
	...
	unsigned long long unmap_secd, unmap_nrsecd;
#endif
...
#ifdef CONFIG_HONOR_RWLOG
	if (cmd->cmnd[0] == UNMAP) {
		unmap_secd = blk_rq_pos(rq) >> (ilog2(cmd->device->sector_size) - 9);
		unmap_nrsecd = blk_rq_sectors(rq) >> (ilog2(cmd->device->sector_size) - 9);
		trace_scsi_dispatch_cmd_start_lifetime(cmd, unmap_secd, unmap_nrsecd);
	} else {
		trace_scsi_dispatch_cmd_start_lifetime(cmd, 0, 0);
	}
#else
