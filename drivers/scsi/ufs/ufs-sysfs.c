...
const char *ufschd_uic_link_state_to_string(
...
const char *ufschd_ufs_dev_pwr_mode_to_string(
...
#ifdef CONFIG_CGROUP_IOLIMIT_HIBERN8
static u32 g_hibern8_timer = UFS_ACGROUP_IOLIMIT_HIBERN8_OPT_TIMER;
static ssize_t dynamic_hibern8_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "%d\n", ufshcd_ahit_to_us(g_hibern8_timer));
}

static ssize_t dynamic_hibern8_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int  input;
	...
	if (input > UFSHCI_AHIBERN8_MAX)
		return -EINVAL;

	if (input < UFS_AHIT_AUTOH8_TIMER)
		return -EINVAL;

	g_hibern8_timer = ufshcd_us_to_ahit(input);
	return count;
}
u32 get_dynamic_hibern8_timer(void)
{
	return g_hibern8_timer;
}
#endif
...
#ifdef CONFIG_CGROUP_IOLIMIT_HIBERN8
static DEVICE_ATTR_RW(dynamic_hibern8);
#endif
