...
#include "platform_include/basicplatform/linux/ufs/dsm_ufs.h"
...
static int ufshcd_parse_regulator_info(struct ufs_hba *hba)
{
	...
	err = ufshcd_populate_vreg(dev, "buck", &info->buck);
out:
	return err;
}
...
static int ufshcd_pltfrm_restore(struct device *dev)
{
	...
	ufshcd_writel(hba, upper_32_bits(hba->utrdl_dma_addr), REG_UTP_TRANSFER_REQ_LIST_BASE_H);
	ufshcd_writel(hba, lower_32_bits(hba->utmrdl_dma_addr), REG_UTP_TASK_REQ_LIST_BASE_L);
	...
	*/
	...
	dsm_ufs_flag_disable();
#endif
	err = ufshcd_send_scsi_sync_cache(hba, hba->sdev_ufs_device, UFS_ACTIVE_PWR_MODE);
	if (err)
		dev_err(dev, "%s:%d ufshcd_send_scsi_sync_cache error err = %d\n", __func__, __LINE__, err);
...
void ufshcd_pltfrm_shutdown(struct platform_device *pdev)
{
	...
	while (hba->lrb_in_use) {
		if (timeout == 0) {
			dev_err(&pdev->dev,
					"%s: wait cmdq complete reqs timeout! lrb_in_use:%llx outstanding_reqs: %llx\n",
					__func__, hba->lrb_in_use, hba->outstanding_reqs);
			break;
		}
		timeout--;
		mdelay(1);
...
static int hufs_ufshcd_get_core_irqs(struct platform_device *pdev, struct ufs_hba *hba)
{
	...
	for (i = 0; i < UTR_CORE_NUM; i++) {
		...
		if (hba->core_irq[i] < 0) {
			dev_err(dev, "core %u irq resource not available!\n", i);
			return -ENODEV;
		}
...
#endif
		snprintf(hba->core_irq_name[i], CORE_IRQ_NAME_LEN, "ufshcd_hufs_core%u_irq", i);
	}
...
int ufshcd_pltfrm_probe(struct platform_device *pdev)
{
	...
	dfa_get_intr_core_irq(pdev, hba);
#endif
