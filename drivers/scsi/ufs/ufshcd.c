...
#include "platform_include/basicplatform/linux/ufs/ufs-hpb.h"
#include "platform_include/basicplatform/linux/ufs/ufshcd_extend.h"
#include "platform_include/basicplatform/linux/ufs/ufshcd_perf.h"
#include "platform_include/basicplatform/linux/ufs/ufshcd_debug.h"
...
#include <platform_include/basicplatform/linux/ufs/dfa_mcq.h>
#endif
...
static DEFINE_MUTEX(g_camera_tmp_hba_lock);
static struct ufs_hba *hba_camera_tmp = NULL;
#endif
...
/****************************add by hufs,begin***********************/
/********************************************************************/
struct wakeup_source *ffu_lock = NULL;

/* DSM_UFS_HARDWARE_RESET */
int ufs_timeout_lock = 0;
...
UFS_ATTRIBUTE(wb_current_buffer_size, _CURR_WB_BUFF_SIZE); /*lint !e421*/
...
void ufshcd_print_clk_freqs(struct ufs_hba *hba)
{
...
static void ufshcd_print_host_state(struct ufs_hba *hba)
{
	...
	dev_err(hba->dev, "lrb in use=0x%lx\n", hba->lrb_in_use);
	dev_err(hba->dev, "UFS Host mcq enable = %d\n", hba->use_mcq);
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_lrbp(lrbp))
		return dfa_get_tr_ocs(lrbp);
#endif
	if (lrbp->is_hufs_utrd)
		return le32_to_cpu(
			       lrbp->hufs_utr_descriptor_ptr->header.dword_2) &
		       MASK_OCS;
...
static void __ufshcd_release(struct ufs_hba *hba)
{
	...
	if (hba->clk_gating.active_reqs || hba->clk_gating.is_suspended ||
	    hba->ufshcd_state != UFSHCD_STATE_OPERATIONAL ||
#ifdef CONFIG_DFA_MCQ
	    atomic_read(&hba->dhba.dfa_inflts) ||
#endif
#ifdef CONFIG_SCSI_HUFS
		hba->lrb_in_use || hba->outstanding_tasks ||
#else
 ...
 */
inline void ufshcd_send_command(struct ufs_hba *hba, unsigned int task_tag)
{
	int ret = 0;
	...
	if (is_dfa_enabled(hba))
		return dfa_send_command(hba, task_tag);
#endif
	...
	if (is_mcq_enabled(hba)) {
		...
		ufshcd_mcq_send_command(hba, task_tag, lrbp);
	} else if (ufshcd_has_utrlcnr(hba)) {
		set_bit(task_tag, &hba->outstanding_reqs);
#ifdef CONFIG_HUFS_HC_CORE_UTR
	...
	ufsdbg_error_inject_dispatcher(hba, ERR_INJECT_DEDBUG_CTRL_INTR, ret,
			&ret);
...
static inline int ufshcd_hba_capabilities(struct ufs_hba *hba)
{
	...
	hba->esi_sup = (hba->capabilities & MASK_ESI_SUPPORT) >> 31;
...
int ufshcd_send_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	if (hba->autoh8_disable_depth == 0) {
		...
		if (ret) {
			dev_err(hba->dev, "disable autoh8 fail, skip uic reg access\n");
			return ret;
		}
		auto_h8_disabled = 1;
	}
...
int ufshcd_consisten_map_sg(struct scsi_cmnd *cmd)
{
	...
	nents = cmd->sdb.table.nents;
	for_each_sg(cmd->sdb.table.sgl, sg, nents, i) {
		sg->dma_address = phys_to_dma(dev, page_to_phys(sg_page(sg)) + sg->offset);
...
int ufshcd_map_sg(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba))
		return dfa_map_sg(hba, lrbp);
#endif
	cmd = lrbp->cmd;
	if ((host->caps & HUFS_CONSISTENT_ACCESS) &&
		((cmd->cmnd[0] == READ_10) || (cmd->cmnd[0] == WRITE_10)))
		sg_segments = ufshcd_consisten_map_sg(cmd);
	else
	...
	if (sg_segments) {
		...
		if (hba->quirks & UFSHCD_QUIRK_PRDT_BYTE_GRAN) {
			if (lrbp->is_hufs_utrd)
				lrbp->hufs_utr_descriptor_ptr->prd_table_length =
		...
		/* open ufs io consisten access */
		if ((host->caps & HUFS_CONSISTENT_ACCESS) &&
			((cmd->cmnd[0] == READ_10) || (cmd->cmnd[0] == WRITE_10)))
			prd->upper_addr |= (UFS_CONSISTEN_ACCESS_ENABLE | UFS_CONSISTEN_ACCESS_AXDOMAIN | UFS_CONSISTEN_ACCESS_AXCACHE);
		...
		if (lrbp->is_hufs_utrd)
			lrbp->hufs_utr_descriptor_ptr->prd_table_length = 0;
		else
			...
			u8 *upiu_flags, enum dma_data_direction cmd_dir)
{
	...
	struct hufs_utp_transfer_req_desc *mas_req_desc =
		lrbp->hufs_utr_descriptor_ptr;
	u32 data_direction;
	u32 dword_0;
	u32 dword_1 = 0;
...
#ifdef CONFIG_DFA_MCQ
	if (lrbp->dlrb.is_dfa_utrd)
		return dfa_prepare_req_desc_hdr(lrbp, upiu_flags, cmd_dir);
#endif
	...
	if (unlikely(lrbp->cmd && lrbp->cmd->cmnd[0] == UFSTT_READ_BUFFER &&
		     is_ufstt_batch_mode()))
		*upiu_flags |= UPIU_CMD_PRIO;
#endif
	...
	if (lrbp->is_hufs_utrd) {
		...
		mas_req_desc->header.dword_1 = cpu_to_le32(dword_1);
		/*
		 * assigning invalid value for command status. Controller
		 * updates OCS on command completion, with the command
		 * status
		 */
		mas_req_desc->header.dword_2 |=
			cpu_to_le32(OCS_INVALID_COMMAND_STATUS);
		/* dword_3 is reserved, hence it is set to 0 */
		mas_req_desc->header.dword_3 = cpu_to_le32(dword_3);

		mas_req_desc->prd_table_length = 0;
	} else {
		/* Transfer request descriptor header fields */
...
void ufshcd_prepare_utp_nop_upiu(struct ufshcd_lrb *lrbp)
{
	struct utp_upiu_req *ucd_req_ptr = lrbp->ucd_req_ptr;
...
void ufshcd_validate_tag(struct Scsi_Host *host, struct ufs_hba *hba,
								struct scsi_cmnd *cmd)
{
	int tag;
...
int ufshcd_tag_alloc(struct Scsi_Host *host, struct ufs_hba *hba,
						struct scsi_cmnd *cmd)
{
	int tag;
 ...
 *
 */
static void ufs_ffu_pm_runtime_delay_process(struct ufs_hba *hba)
{
...
void ufshcd_start_delay_work(struct ufs_hba *hba,
		struct scsi_cmnd *cmd)
{
...
void ufshcd_check_disable_dev_tmt_cnt(struct ufs_hba *hba,
					     struct scsi_cmnd *cmd)
{
	if ((unlikely((cmd->cmnd[0] == WRITE_BUFFER) &&
		      ((cmd->cmnd[1] & WB_MODE_MASK) == DOWNLOAD_MODE))) ||
...
void ufshcd_disable_dev_tmt_cnt(struct ufs_hba *hba)
{
	unsigned long flags;
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba))
		return dfa_queuecommand(host, cmd);
#endif
	...
	if (unlikely(cmd->cmnd[0] == WRITE_BUFFER &&
		...
		err = wait_for_ufs_all_complete(hba, 2000);
		dev_err(hba->dev,
...
#if defined(CONFIG_HUFS_HC_CORE_UTR) && defined(CONFIG_MAS_QOS_MQ)
...
int ufshcd_clear_cmd(struct ufs_hba *hba, int tag)
{
	int err = 0;
	unsigned long flags;
#ifdef CONFIG_HUFS_HC_CORE_UTR
		...
		struct ufshcd_lrb *lrbp, int max_timeout)
{
	...
	if (!time_left) {
		...
		if (!ufshcd_clear_cmd(hba, lrbp->task_tag)) {
			...
			/*
			 * Mark the command if clearing doorbell failed,
			 * release outstanding_req and tag until it complete
			 */
			hba->ufs_dm_timeout++;
			spin_lock_irqsave(hba->host->host_lock, flags);
...
#if defined(CONFIG_SCSI_UFS_HI1861_VCMD) && defined(CONFIG_HUFS_HC)
	if (hba->is_hi186x_query_vcmd)
		wait_event(hba->dev_cmd.tag_wq,
			   ufshcd_get_vcmd_tag(hba, &tag));
	else
		wait_event(hba->dev_cmd.tag_wq,
	...
	u32 *attr_val)
{
	...
	for (retries = QUERY_REQ_RETRIES; retries > 0; retries--) {
		ret = ufshcd_query_attr(hba, opcode, idn, index,
						selector, attr_val);
		if (ret)
			dev_err(hba->dev, "%s: idn %d, failed with error %d, retries %d\n",
				  ...
				  int *desc_len)
{
	...
	if (desc_id == QUERY_DESC_IDN_DEVICE_UNISTORE)
		*desc_len = QUERY_DESC_DEVICE_UNISTORE_MAX_SIZE;

	if (desc_id == QUERY_DESC_IDN_UNIT_UNISTORE)
 ...
 * Return 0 in case of success, non-zero otherwise
 */
inline int ufshcd_read_unit_desc_param(struct ufs_hba *hba,
					      int lun,
...
static int ufshcd_hufs_hc_utrdl_alloc(struct ufs_hba *hba)
{
	size_t utrdl_size;

	utrdl_size = (sizeof(struct hufs_utp_transfer_req_desc) *
		      (unsigned int)hba->nutrs);
	hba->hufs_utrdl_base_addr = dmam_alloc_coherent(
		hba->dev, utrdl_size, &hba->utrdl_dma_addr, GFP_KERNEL);
	if (!hba->hufs_utrdl_base_addr ||
...
static int ufshcd_memory_alloc(struct ufs_hba *hba)
{
...
#if defined(CONFIG_SCSI_UFS_HI1861_VCMD) && defined(CONFIG_HUFS_HC)
	ucdl_size = sizeof_utp_transfer_cmd_desc(hba) * (ucd_num - 1) +
	 ...
	 */
	if (ufshcd_is_hufs_hc(hba)) {
		if (ufshcd_hufs_hc_utrdl_alloc(hba))
			goto out;
	} else {
...
static void ufshcd_host_memory_configure(struct ufs_hba *hba)
{
	...
	if (ufshcd_is_hufs_hc(hba) && hba->vops &&
		hba->vops->ufshcd_hufs_host_memory_configure) {
		hba->vops->ufshcd_hufs_host_memory_configure(hba);
		return;
	}
...
static int ufshcd_dme_link_startup(struct ufs_hba *hba)
{
	...
	if (ufshcd_is_hufs_hc(hba) && !ufshcd_hufs_use_standard_uic(hba) && hba->vops &&
		hba->vops->hufs_dme_link_startup)
		return hba->vops->hufs_dme_link_startup(hba);
...
int ufshcd_uic_change_pwr_mode(struct ufs_hba *hba, u8 mode)
{
	...
	if (ufshcd_is_hufs_hc(hba) && !ufshcd_hufs_use_standard_uic(hba) && hba->vops &&
		hba->vops->ufshcd_hufs_uic_change_pwr_mode)
		return hba->vops->ufshcd_hufs_uic_change_pwr_mode(hba, mode);
...
int ufshcd_uic_hibern8_exit(struct ufs_hba *hba)
{
	...
	ret = __ufshcd_uic_hibern8_op_irq_safe(hba, HUFS_H8_OP_EXIT);
	if (!ret)
		hba->is_hibernate = false;
	else
...
/* Only for camera use, close and open autoH8 func */
#ifdef CONFIG_SCSI_UFS_SCORPIO
void ufshcd_auto_hibern8_close(void)
{
	mutex_lock(&g_camera_tmp_hba_lock);

	if (hba_camera_tmp == NULL)
		goto out;

	if (ufshcd_is_auto_hibern8_allowed(hba_camera_tmp)) {
		dev_err(hba_camera_tmp->dev, "%s:tele close autoh8\n", __func__);
		ufshcd_disable_auto_hibern8(hba_camera_tmp);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "TAG=0");
	}

out:
	mutex_unlock(&g_camera_tmp_hba_lock);
	return;
}
EXPORT_SYMBOL_GPL(ufshcd_auto_hibern8_close);

void ufshcd_auto_hibern8_open(void)
{
	mutex_lock(&g_camera_tmp_hba_lock);
	...
	if (ufshcd_is_auto_hibern8_allowed(hba_camera_tmp)) {
		dev_err(hba_camera_tmp->dev, "%s:tele open autoh8\n", __func__);
		ufshcd_enable_auto_hibern8(hba_camera_tmp);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "TAG=1");
	}
...
EXPORT_SYMBOL_GPL(ufshcd_auto_hibern8_open);
#endif

#ifdef CONFIG_CGROUP_IOLIMIT_HIBERN8
static struct ufs_hba *hba_iolimit_tmp = NULL;
static DEFINE_MUTEX(g_hba_iolimit_tmp_lock);

void ufshcd_iolimit_wakeup_and_update_h8(void)
{
	u32 timer = 0;
	mutex_lock(&g_hba_iolimit_tmp_lock);

	if (hba_iolimit_tmp == NULL)
		goto out;

	timer = get_dynamic_hibern8_timer();

	if (ufshcd_is_auto_hibern8_allowed(hba_iolimit_tmp)) {
		ufshcd_uic_hibern8_exit(hba_iolimit_tmp);
		ufshcd_auto_hibern8_update(hba_iolimit_tmp, timer);
		trace_ufshcd_iolimit_h8_switch(timer);
	}

out:
	mutex_unlock(&g_hba_iolimit_tmp_lock);
	return;
}
EXPORT_SYMBOL_GPL(ufshcd_iolimit_wakeup_and_update_h8);

void ufshcd_iolimit_reset_h8(void)
{
    mutex_lock(&g_hba_iolimit_tmp_lock);
	...
	if (ufshcd_is_auto_hibern8_allowed(hba_iolimit_tmp)) {
		ufshcd_uic_hibern8_exit(hba_iolimit_tmp);
		ufshcd_auto_hibern8_update(hba_iolimit_tmp, UFS_AHIT_AUTOH8_TIMER);
		trace_ufshcd_iolimit_h8_switch(UFS_AHIT_AUTOH8_TIMER);
	}
out:
	mutex_unlock(&g_hba_iolimit_tmp_lock);
	return;
}
EXPORT_SYMBOL_GPL(ufshcd_iolimit_reset_h8);
#endif
...
static int ufshcd_get_max_pwr_mode(struct ufs_hba *hba)
{
	...
	CALL_HCK_VH(ufs_gear_ctrl_adjust_capbility, hba);
	return 0;
}
			     ...
			     struct ufs_pa_layer_attr *pwr_mode)
{
	...
	if (pwr_mode->pwr_rx == FASTAUTO_MODE ||
	    ...
	    pwr_mode->pwr_rx == FAST_MODE ||
	    pwr_mode->pwr_tx == FAST_MODE) {
		if ((hba->quirks & UFSHCD_QUIRK_UNIPRO_SCRAMBLING) || (hba->scramble))
	...
	ret = ufshcd_uic_change_pwr_mode(hba, pwr_mode->pwr_rx << 4
			| pwr_mode->pwr_tx);
	if (!ret)
		ret = ufshcd_adapt_workaround(hba, pwr_mode);
	if (ret) {
...
int ufshcd_verify_dev_init(struct ufs_hba *hba)
{
	...
	for (retries = NOP_OUT_RETRIES; retries > 0; retries--) {
		...
		err = ufshcd_exec_dev_cmd(hba, DEV_CMD_TYPE_NOP, timeout);
...
#ifdef CONFIG_MAS_BLK
	if (sdev->host->queue_quirk_flag & SHOST_QUIRK(SHOST_QUIRK_HUFS_MQ))
		lun_qdepth =
 ...
 */
int ufshcd_transfer_rsp_status(struct ufs_hba *hba, struct ufshcd_lrb *lrbp, bool
		in_irq)
{
	int result = 0;
	int scsi_status;
	...
	case OCS_SUCCESS:
		...
		case UPIU_TRANSACTION_RESPONSE:
			...
			if ((scsi_status != SAM_STAT_GOOD) &&
				...
				ufshcd_rsp_sense_data(hba, lrbp, scsi_status);
	...
	case OCS_DEVICE_FATAL_ERROR:
		...
		dev_err(hba->dev,
			...
			ocs, lrbp->task_tag);
		ret = dsm_ufs_update_ocs_info(hba, DSM_UFS_DEVICE_FATAL_ERROR, ocs);
		if (!ret)
			schedule_ufs_dsm_work(hba);
		ufshcd_print_evt_hist(hba);
		ufshcd_print_host_state(hba);
	...
	default:
		...
		if ((ocs == OCS_INVALID_CRYPTO_CONFIG) && lrbp && lrbp->cmd && lrbp->cmd->request)
			dev_err(hba->dev, "OCS 9 error from controller ci_key_index 0x%x\n",
...
void ufshcd_compl_one_task(struct ufs_hba *hba, int task_tag)
{
	...
	if (cmd && lrbp->command_type != UTP_CMD_TYPE_DEV_MANAGE &&
		...
		dsm_ufs_restore_cmd(hba, task_tag);
#endif
		...
		cmd->result = result;
		if (!lrbp->req_send)
			dev_err(hba->dev, "%s: request is not sent but completed, tag %d\n",
				__func__, lrbp->task_tag);
		/* Mark completed command as NULL in LRB */
		lrbp->cmd = NULL;
		clear_bit_unlock(task_tag, &hba->lrb_in_use);
		ufstt_unprep_handler(hba, lrbp, now_time);
		req = cmd->request;
		opcode = cmd->cmnd[0];
	...
	} else if (lrbp->command_type == UTP_CMD_TYPE_DM_TIMEOUT) {
		clear_bit_unlock(task_tag, &hba->lrb_in_use);
	}
	ufshcd_complete_time(hba, lrbp, opcode, now_time);
...
check:
	...
	for_each_set_bit(index, &completed_reqs, hba->nutrs)
		ufshcd_compl_one_task(hba, index);
...
#ifdef CONFIG_HUFS_HC
		return ufshcd_hufs_extend_trc_handler(hba, completed_reqs);
#endif
		u32 tr_doorbell;
 ...
 */
int ufshcd_disable_ee(struct ufs_hba *hba, u16 mask)
{
	int err = 0;
	u32 val;
 ...
 */
int ufshcd_enable_auto_bkops(struct ufs_hba *hba)
{
	int err = 0;
...
static void ufshcd_exception_event_handler(struct work_struct *work)
{
	...
	if (status & MASK_EE_TOO_HIGH_TEMP)
		ufs_dev_too_high_temp_event_handler(hba);
...
{
	if (is_mcq_enabled(hba))
		ufshcd_mcq_transfer_req_compl_all(hba, force_compl);
	else
		ufshcd_trc_handler(hba, false);
...
#endif
{
	...
	if (ufshcd_err_handling_should_stop(hba)) {
		if (hba->ufshcd_state != UFSHCD_STATE_ERROR || hba->ufs_init_completed == false)
			hba->ufshcd_state = UFSHCD_STATE_OPERATIONAL;
		spin_unlock_irqrestore(hba->host->host_lock, flags);
		up(&hba->host_sem);
		pm_runtime_enable(hba->dev);
		hba->ufs_in_err_handle = FALSE;
...
static irqreturn_t ufshcd_sl_intr(struct ufs_hba *hba, u32 intr_status)
{
	...
	ufsdbg_error_inject_dispatcher(hba, ERR_INJECT_INTR,
	...
	if (intr_status & UTP_TRANSFER_REQ_COMPL)
		retval |= ufshcd_trc_handler(hba, false);
...
irqreturn_t ufshcd_intr(int irq, void *__hba)
{
	...
	ufshcd_hufs_is_intr_inject(hba, &intr_status);
...
#ifdef CONFIG_SCSI_HUFS
		if (ufshcd_hufs_vs_intr(hba) == IRQ_HANDLED)
			retval = IRQ_HANDLED;
#endif
#ifndef CONFIG_SCSI_HUFS
		intr_status = ufshcd_readl(hba, REG_INTERRUPT_STATUS);
		...
		u8 tm_function, u8 *tm_response)
{
	...
	wait_event(hba->tm_tag_wq,
	    ufshcd_get_tm_free_slot(hba, &free_slot));
	pm_runtime_get_sync(hba->dev);
	spin_lock_irqsave(host->host_lock, flags);
	while (hba->is_hibernate) {
		spin_unlock_irqrestore(host->host_lock, flags);
...
int ufshcd_host_reset_and_restore(struct ufs_hba *hba)
{
	int err = -EAGAIN;
#ifdef CONFIG_HUAWEI_UFS_DSM
	g_ufs_dsm_log.ufs_reset_dmd->full_reset_flag = false;
#endif
	/* DSM_UFS_HARDWARE_RESET */
	ufs_timeout_lock = 0;
	...
	if (err) {
#ifdef CONFIG_HUAWEI_UFS_DSM
		g_ufs_dsm_log.ufs_reset_dmd->full_reset_flag = true;
#endif
 ...
 * Returns zero on success, non-zero on failure
 */
int ufshcd_reset_and_restore(struct ufs_hba *hba)
{
	u32 saved_err;
	u32 saved_uic_err;
	int err = 0;
...
#endif
	CALL_HCK_VH(mas_blk_gear_ctrl_reinit, NULL);
...
static struct request_queue *g_ufs_rpmb_req_queue = NULL;

void ufs_host_set_rpmb_queue(struct request_queue *queue)
{
	smp_store_release(&g_ufs_rpmb_req_queue, queue);
}

struct request_queue *ufs_host_get_rpmb_queue(void)
{
	return smp_load_acquire(&g_ufs_rpmb_req_queue);
}
...
static int ufshcd_scsi_add_wlus(struct ufs_hba *hba)
{
	...
	if (likely(!hba->host->is_emulator)) {
		...
		ufs_host_set_rpmb_queue(hba->sdev_rpmb->request_queue);
...
static int ufs_get_device_desc(struct ufs_hba *hba)
{
	...
	pr_err("%s dev_info->bqueuedepth = %d\n", __func__, dev_info->bqueuedepth);
	b_ufs_feature_sup = desc_buf[DEVICE_DESC_PARAM_UFS_FEAT];

	model_index = desc_buf[DEVICE_DESC_PARAM_PRDCT_NAME];
	hufs_parse_dev_desc(hba, dev_info, desc_buf);
	...
	err = ufshcd_vops_get_device_info(hba, dev_info, desc_buf);
	if (err)
		pr_err("%s err %d\n", __func__, err);
...
static int ufshcd_add_lus(struct ufs_hba *hba)
{
	...
	ufshcd_rev_store(hba);
	ufshcd_high_temp_config(hba);
	ufs_bsg_probe(hba);
	ufshpb_init(hba);
...
static int ufshcd_probe_hba(struct ufs_hba *hba, bool async)
{
	...
	if (is_mcq_supported(hba)) {
		ret = ufshcd_config_mcq(hba, async);
		if (ret)
			goto out;
	}
	...
	ufshcd_vops_set_buck_voltage(hba);

	ufshcd_vops_mphy_config_update(hba);
...
#endif
	ret = ufs_blk_config_init(hba);
	if (ret)
		goto out;

	/* Gear up to HS gear if supported */
	if (!is_dfa_enabled(hba) && hba->max_pwr_info.is_valid) {
		/*
	...
	ret = ufs_switch_hs_check(hba);
	if (ret)
		goto out;

	ufs_update_linkup_err(hba);
	...
	ret = ufs_extend_config_init(hba);
	if (ret)
		goto out;
...
static int __ufshcd_async_scan(struct ufs_hba *hba)
{
	...
	up(&hba->host_sem);
	if (ret) {
		dev_err(hba->dev, "Host controller probe failed\n");
		return ret;
	}
	...
	if (ret)
		dev_err(hba->dev, "Host controller add UFS logical units failed\n");
				     ...
				     enum ufs_dev_pwr_mode pwr_mode)
{
	...
	int ret = -ENODEV;
	struct hufs_host *host = hba->priv;
	unsigned int timeout = host->caps & USE_HUFS_MPHY_TC ? 12000 : 1200;
	...
	if (hba->caps & UFSHCD_CAP_SSU_BY_SELF &&
		...
		ret = ufshcd_send_scsi_sync_cache(hba, sdp, pwr_mode);
		if (ret)
			goto out;
	}
	/*
	 ...
	 */
	if (hba->caps & UFSHCD_CAP_SSU_BY_SELF)
		ret = ufshcd_send_scsi_ssu(hba, sdp, cmd, timeout, &sshdr);
	else
...
static int ufshcd_suspend(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (ffu_lock->active) {
		dev_err(hba->dev, "ffu wake lock is active.\n");
		ret = -EINVAL;
		goto out;
	}

	if (rpmb_get_wake_lock_status()) {
		dev_err(hba->dev, "rpmb wake lock is active.\n");
		ret = -EINVAL;
		goto out;
	}
#endif
		...
		if ((ufshcd_is_runtime_pm(pm_op) && !hba->auto_bkops_enabled) ||
			...
			if (ufshcd_wb_allowed(hba)) {
				...
				if (hba->is_samsung_v5)
					cancel_delayed_work_sync(&hba->wb_flush_work);
			}
		}
...
static int ufshcd_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	hba->ulsr = false;
	dev_err(hba->dev, "ulsr reset end\n");
	goto ulsr;
#endif
...
void ufshcd_remove(struct ufs_hba *hba)
{
	...
	ufshcd_hba_stop(hba);
	wakeup_source_unregister(ffu_lock);
	ufshcd_hba_exit(hba);
}
...
#define UFS_SYNCCACHE_STATUS_REG	(PMIC_HRST_REG19_ADDR(0))
void ufshcd_record_sc(unsigned int scenario, int err_no)
{
	unsigned int reg_val = 1 << scenario;

	if (err_no)
		reg_val |= 0x01;
	pmic_write_reg(UFS_SYNCCACHE_STATUS_REG, reg_val);
}

static void ufshcd_synccache_record_print(struct work_struct *work)
{
	struct ufs_hba *hba = container_of(work, struct ufs_hba,
			synccache_worker.work);
	unsigned int reg_val = pmic_read_reg(UFS_SYNCCACHE_STATUS_REG);
	unsigned int fail = reg_val & 0x01;

	if (fail) {
		dsm_ufs_update_upiu_info(hba, -1, DSM_UFS_POWER_DOWN_FLUSH_ERR);
		schedule_ufs_dsm_work(hba);
	}
	pr_err("SYNCCACHE: ufs_synccache_status - <0x%x>\r\n", reg_val);
	pmic_write_reg(UFS_SYNCCACHE_STATUS_REG, 1);
}
#endif
...
#endif
{
...
#ifdef CONFIG_SCSI_UFS_SYNCCACHE_RECORD
	INIT_DELAYED_WORK(&hba->synccache_worker, ufshcd_synccache_record_print);
	schedule_delayed_work(&hba->synccache_worker, SYNCCACHE_DMD_DELAY * HZ);
#endif
...
#endif /* CONFIG_SCSI_HUFS_MQ_DEFAULT */

#ifdef CONFIG_HUFS_HC_CORE_UTR
	core_utr_qos_ctrl_init(hba);
#endif
	host->nr_hw_queues = hba->nr_hw_queues ? : host->nr_hw_queues;
...
#ifdef CONFIG_HUFS_MANUAL_BKOPS
	host->hufs_dev_quirk_flag |= SHOST_HUFS_DEV_QUIRK(SHOST_QUIRK_BKOPS_ENABLE);
#endif
...
#ifdef CONFIG_SCSI_UFS_HS_ERROR_RECOVER
	hba->recover_wake_lock = wakeup_source_register(hba->dev, "ufs_recover");
	if (hba->recover_wake_lock == NULL)
	{
		dev_err(hba->dev, "request wakeup source failed\n");
		goto out_disable;
	}
	 ...
	 */
	mb();
	ffu_lock = wakeup_source_register(hba->dev, "ffu_wakelock");
	if (ffu_lock == NULL)
	{
		dev_err(hba->dev, "request wakeup source failed\n");
		goto out_disable;
