...
/*lint -e605*/
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
static int sim_read_iodie_sts_config(struct hw_sim_hardware_info *info, struct device_node *np)
{
	u32 ret;
	u32 reg_addr = 0;
	...
	if (ret != 0) {
		hwsim_loge("fail to read bridge_info addr");
...
static u32 sim_read_bridge_config(struct hw_sim_hardware_info *info, struct device_node *np)
{
	...
	hwsim_logi("transfer_type is %d, ret is 0x%x\n", info->bridge_ctrl.type, ret);
...
static int sim_update_bridge_config(struct hw_sim_hardware_info *info, s32 vol)
{
	...
	for (chip_id = 0; chip_id < 2; chip_id++) {
		bridge_chip_reg.slave_id = chip_id;
...
static void hw_sim_get_bridge_dump_info(struct hw_sim_hardware_info *info)
{
	...
	hwsim_loge("sim_%d, sts status is 0x%x\n", info->sim_id, status);
#endif
}

static int hw_sim_read_regulator(u32 sim_id, sim_regulator_flag_type type, struct regulator **sim_regulator,
	const struct device *spmidev, const char *prop_str)
{
	...
	if (spmidev == NULL) {
		hwsim_loge("sim_%d, type_%d, spmidev invalid\n", sim_id, type);
		return HW_SIM_ERR;
	}

	if (prop_str == NULL) {
		hwsim_loge("sim_%d, type_%d, prop invalid\n", sim_id, type);
		return HW_SIM_ERR;
	}
	...
	*sim_regulator = devm_regulator_get(spmidev, prop_str);
	...
	hwsim_logi("get_regulator, sim_%d, type_%d\n", sim_id, type);
...
static void sim_reg_cfg(struct hw_sim_hardware_info *info, s32 vol)
{
	...
	hwsim_logi("sim_%u, vol is %u, sim_sd_cfg_value is 0x%x\n", info->sim_id, vol, sim_sd_cfg_value);
...
static void hw_sim_clear_pmu_by_regulator_type(u32 sim_id, sim_regulator_flag_type type)
{
	...
	case SIM_PMU_SWITCH_FLAG:
		hwsim_loge("sim_%d, type_%d, not used\n", sim_id, type);
		break;
	case SD_IO_PMU_FLAG:
	...
	case ESIM_VCC_FLAG:
		...
		break;
	default:
		hwsim_loge("sim_%d, type_%d, error\n", sim_id, type);
		break;
	}
	hwsim_logi("sim_%d, type_%d, clear finish\n", sim_id, type);
}

static void put_regulator(u32 sim_id, sim_regulator_flag_type type, const struct regulator *pmu)
{
	if (pmu == NULL || type >= REGULATOR_FLAG_BUTT || sim_id >= MAX_SIMHP_NUM || regulator_is_enabled(pmu))
		return;

	hwsim_logi("sim_%d, type_%d\n", sim_id, type);
...
static int hw_sim_disable_regulator(u32 sim_id, sim_regulator_flag_type type, const struct regulator *sim_regulator)
{
	...
	if (regulator_disable(sim_regulator) == 0)
		hwsim_logi("sim_%d, type_%d, regulator_disable ok\n", sim_id, type);
	else
		hwsim_loge("sim_%d, type_%d, regulator_disable fail\n", sim_id, type);
