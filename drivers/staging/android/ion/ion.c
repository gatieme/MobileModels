...
#include "mm/ion_alloc_debug.h"

enum ion_sync_direction {
	ION_SYNC_BEGIN_CPU_ACCESS = 0,
	ION_SYNC_END_CPU_ACCESS = 1,
};
...
#ifdef CONFIG_MM_LB
static int ion_buffer_attach_lb(struct ion_buffer *buffer, unsigned long flags)
{
	unsigned int plc_id;
					   ...
					   unsigned long flags)
{
	...
	if (buffer) {
#ifdef CONFIG_MM_LB
		if (ion_buffer_attach_lb(buffer, flags)) {
...
#endif
	/* alloc fail shouldn't effect func exec */
	(void)alloc_bg_buffer_info((void*)buffer);
...
void ion_buffer_destroy(struct ion_buffer *buffer)
{
	...
	if (is_bg_buffer((void *)buffer)) {
		bg_buffer_destroy((void *)buffer);
	} else {
		if (buffer->heap->type != ION_HEAP_TYPE_CARVEOUT)
...
static int ion_dma_buf_attach(struct dma_buf *dmabuf, struct dma_buf_attachment *attachment)
{
	...
	if (is_bg_buffer((void *)buffer)) {
		ret = bg_dmabuf_attach((void *)buffer);
		if (ret)
			return -EINVAL;
	}
...
static int _ion_dma_sync_sg(struct dma_buf *dmabuf,
					enum dma_data_direction direction,
					enum ion_sync_direction sync_direction)
{
	struct ion_buffer *buffer = dmabuf->priv;
	...
	if (heap != NULL && heap->type == ION_HEAP_TYPE_CPA) {
		pr_err("%s buffer is protected, no sync allowed\n", __func__);
		return 0;
	}
	...
	if (dmabuf->size >= MM_ION_FLUSH_ALL_CPUS_CACHES) {
		...
		if (is_release_ion_mem(buffer)) {
			/* return succeed because operation has nothing to do */
			pr_info("%s buffer has been released, no need to sync cache\n", __func__);
			mutex_unlock(&buffer->lock);
			return 0;
		}

		if (sync_direction == ION_SYNC_BEGIN_CPU_ACCESS)
			dma_sync_sg_for_cpu(&mm_ion_dev->dev,
						table->sgl, table->nents,
						direction);
		else if (sync_direction == ION_SYNC_END_CPU_ACCESS)
...
static int ion_dma_buf_begin_cpu_access(struct dma_buf *dmabuf,
					enum dma_data_direction direction)
{
	return _ion_dma_sync_sg(dmabuf, direction, ION_SYNC_BEGIN_CPU_ACCESS);
}

static int ion_dma_buf_end_cpu_access(struct dma_buf *dmabuf,
					enum dma_data_direction direction)
{
	return _ion_dma_sync_sg(dmabuf, direction, ION_SYNC_END_CPU_ACCESS);
}
...
/*
 * If the new ops_func is added && bg_reclaim enabled,
 * please check bg_reclaim status in new ops_func
 */
...
static int ion_alloc_para_process(struct ion_debug_info *ion_dbg_info, size_t *len,
				  unsigned int *heap_id_mask, unsigned int *flags)
{
	*len = PAGE_ALIGN(*len);
	...
	if (*len > (SZ_2M * 100))
		_ion_debug_print_info(ion_dbg_info, ION_ALLOC_SIZE_GT_200MB);
...
static void ion_alloc_flush_buffer(size_t len, unsigned int flags, struct ion_buffer *buffer)
{
	struct platform_device *mm_ion_dev = get_mm_ion_platform_device();
...
int ion_alloc(size_t len, unsigned int heap_id_mask, unsigned int flags)
{
	...
	struct ion_debug_info ion_dbg_info;
	...
	ion_debug_get_meminfo(&ion_dbg_info);
	fd = ion_alloc_para_process(&ion_dbg_info, &len, &heap_id_mask, &flags);
	if (fd) {
		pr_err("%s: Incorrect alloc size.\n", __func__);
		return fd;
	}

	ion_debug_get_timeinfo(&ion_dbg_info, ION_ALLOC_START);
	down_read(&dev->lock);
	ion_debug_get_timeinfo(&ion_dbg_info, ION_ALLOC_HALF);
	...
	up_read(&dev->lock);
	ion_debug_get_timeinfo(&ion_dbg_info, ION_ALLOC_END);
	trace_ion_alloc(heap_id_mask, len, ion_dbg_info._timedelta);
	ion_debug_print_info(&ion_dbg_info, len, heap_id_mask, flags);
	...
	ion_alloc_flush_buffer(len, flags, buffer);
...
void add_ion_total_size(size_t size)
{
	atomic_long_add(size, &ion_total_size);
}

void sub_ion_total_size(size_t size)
{
	atomic_long_sub(size, &ion_total_size);
}
					...
					struct file *f, unsigned int fd)
{
	...
	if (is_release_ion_mem(ibuf)) {
		pr_err("[bg_release] %16s %16u %16u %16zu %16llu %16u %16s\n",
...
static const struct proc_ops debug_process_heap_fops2 = {
	.proc_open = ion_debug_process_heap_open,
	.proc_read = seq_read,
...
#endif
	seq_printf(s, "%16s %10s %16s %8s %8s ",
...
#endif
	seq_printf(s, "%16s %10u %16zu %8d %8llu ",
		   buffer->task_comm, buffer->pid, buffer->size,
		   buffer->kmap_cnt, buffer->magic);
#ifdef CONFIG_ION_WIDE_INFO
...
#endif
#ifdef CONFIG_ION_BUFFER_REUSE
	seq_printf(s, " %8zu", buffer->nr_cma_pages);
#endif
	seq_printf(s, "\n");
}
...
static int ion_debug_heap_show(struct seq_file *s, void *d)
{
	...
	for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
		...
		if (is_release_ion_mem(buffer)) {
			pr_err("[bg_release] %16s %10u %16zu %4d %16llu\n",
...
void ion_device_add_heap(struct ion_heap *heap)
{
	...
	if (heap->shrinker.count_objects && heap->shrinker.scan_objects) {
		...
		snprintf_s(debug_name, 64, "%s_shrink", heap->name);
	...
	if (heap->reuse_config_fops) {
		...
		snprintf_s(debug_name, 64, "%s_reuse_config", heap->name);

		debug_file = debugfs_create_file(
				debug_name, 0664, dev->heaps_debug_root, heap,
...
static ssize_t total_heaps_kb_show(struct kobject *kobj,
				struct kobj_attribute *attr,char *buf)
{
	return sprintf(buf, "%llu kb \n",
		       div_u64(get_ion_total_size(), 1024));
}
...
static int ion_device_create(void)
{
	...
	entry = proc_create_data("ion_process_info", 0444,
				 NULL, &debug_process_heap_fops2, idev);
	if (!entry)
