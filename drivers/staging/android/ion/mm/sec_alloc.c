...
int mm_sec_cma_reserve(struct reserved_mem *rmem)
{
	...
	struct media_zone_rsvdmem *mz_rev = NULL;
#endif
	...
	if (of_get_flat_dt_prop(node, "media_zone_rsvd", NULL)) {
		if (num_mz_rsvdmem >= MAX_MEDIA_ZONE_RSVDMEM)
			return -EINVAL;

		mz_rev = &media_zone_rsvdmem_sp[num_mz_rsvdmem];
...
RESERVEDMEM_OF_DECLARE(mm_dynamic_cma,
		       "tiny-cma-pool",
		       mm_sec_cma_reserve);
RESERVEDMEM_OF_DECLARE(mm_smemheap_cma,
		       "smemheap-cma",
		       mm_sec_cma_reserve);
					    ...
					    unsigned long align)
{
	...
	page = cma_alloc(cma, count, (u32)get_order((u32)align), GFP_KERNEL);
	if (!page)
		goto free_table;
