...
static int storage_ksys_open(void)
{
	int fd_wr = -1;
...
static int storage_ksys_lseek(int fd_wr)
{
	int res = -1;
...
static int storage_ksys_fsync(int fd_wr)
{
	int res = -1;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	res = ksys_fsync(fd_wr);
#else
...
static unsigned int storage_ro_write_try(void)
{
	...
	res = storage_ksys_lseek(fd_wr); /* write try */
	if (res < 0) {
		goto out;
	}
...
#else
#define STORAGE_ROCHK_FAULT_INJECT_PHYMEM_ADDR  \
	(SUB_RESERVED_UNUSED_PHYMEM_BASE + 16)
#endif
...
int storage_rofi_switch_mmc_card_pwronwp(const struct mmc_card *card)
{
	...
	wp = readl(la);
	if (wp != STORAGE_ROFI_KERNEL_EMMC_PWRON_WP_SET && wp != STORAGE_ROFI_KERNEL_EMMC_PWRON_WP_CLR) {
		iounmap(la);
		return 0;
	}
	...
	if (wp == STORAGE_ROFI_KERNEL_EMMC_PWRON_WP_SET) /* update bit 12 with new temp_wp value */
		csd_be128[EMMC_CSD_WP_BYTE] |= EMMC_CSD_PWRON_WP_MASK;
	else if (wp == STORAGE_ROFI_KERNEL_EMMC_PWRON_WP_CLR)
		csd_be128[EMMC_CSD_WP_BYTE] &= ~EMMC_CSD_PWRON_WP_MASK;

	crc7_val = crc7_be(0, csd_be128, 15); /* cal CRC7 of 15 bytes, set CRC7 into bits 7:1, and 1 into bit 0 */
