...
#ifdef CONFIG_IPA_THERMAL
	int ret;
	gpu_cooling_get_limit_state(dfc, &state);
#endif
...
#ifdef CONFIG_IPA_THERMAL
	ret = gpu_update_limit_freq(dfc, state);
	if(ret < 0)
		return -EINVAL;
#else
...
#ifdef CONFIG_IPA_THERMAL
		gpu_scale_power_by_time(cdev, state, &static_power, &dyn_power);
#else
...
#ifdef CONFIG_ITS_IPA
		gpu_power_to_state(cdev, power, state);
...
#ifdef CONFIG_IPA_THERMAL
			gpu_print_power_table(dfc, freq, voltage);
#else
