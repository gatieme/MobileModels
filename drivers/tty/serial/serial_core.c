...
#if defined(CONFIG_BUS_AMBA_PL011) || defined(CONFIG_SERIAL_UART_V500)
	else
		dev_info(port->dev,
		         "%s: UART%d start transfer failed! [sfc = %lu],[hfc = %d]\n",
		         __func__, port->line, tty->stopped, port->hw_stopped);
#endif
}
...
#if defined CONFIG_BUS_AMBA_PL011
	if ((port->status & mask) && (port->ops->throttle)) {
#else
	if (port->status & mask) {
#endif
