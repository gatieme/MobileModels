...
static void sysrq_handle_crash(int key)
{
	...
	panic("sysrq triggered crash by '%s'\n",
		current->mm && current->active_mm ?
		(char*)(current->active_mm->exe_file->f_path.dentry->d_name.name) :
		current->comm);
}
...
static void sysrq_handle_spin_bug(int key)
{
	spinlock_t s;
	spin_lock(&s);
}

static const struct sysrq_key_op sysrq_spin_bug_op = {
	.handler	= sysrq_handle_spin_bug,
	.help_msg	= "spin_bug(S)",
	.action_msg	= "Trigger a spin bug",
	.enable_mask	= SYSRQ_ENABLE_DUMP,
};
#endif
