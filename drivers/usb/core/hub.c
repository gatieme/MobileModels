...
#include "../../../platform_source/basicplatform/drivers/usb/core/vendor-usb-core.h"
...
#ifndef CONFIG_USB_AUTOSUSPEND_DELAY_QUIRK
#ifdef CONFIG_PM
...
static int finish_port_resume(struct usb_device *udev)
{
	...
	if (udev->reset_resume) {
		 ...
		 */
 retry_reset_resume:
#ifdef CONFIG_USB_PM_NO_RESET_RESUM
...
{
#ifdef CONFIG_USB_WAIT_FOR_CONNECTED_ACCURATELY
	unsigned long jiffies_expire = jiffies + 2 * HZ;
#endif
	int status = 0, delay_ms = 0;

#ifdef CONFIG_USB_WAIT_FOR_CONNECTED_ACCURATELY
	while (delay_ms < 2000 && time_before(jiffies, jiffies_expire)) {
#else
	while (delay_ms < 2000) {
#endif
...
static int hub_set_address(struct usb_device *udev, int devnum)
{
	...
	usb_stub_control_msg(&retval, USB_REQ_SET_ADDRESS);
#endif
	if (retval == 0) {
			...
			if (r) {
				if (r != -ENODEV) {
					dev_err(&udev->dev, "device descriptor read/64, error %d\n",
							r);
					ueb_set_enum_err_type(USB_GET_DESCRIPTOR_ERR);
				}
				retval = -EMSGSIZE;
				continue;
			}
			...
			if (retval < 0) {
				if (retval != -ENODEV) {
					dev_err(&udev->dev, "device not accepting address %d, error %d\n",
							devnum, retval);
					ueb_set_enum_err_type(USB_SET_ADDRESS_ERR);
				}
				goto fail;
			}
			if (udev->speed >= USB_SPEED_SUPER) {
...
loop:
	...
	if (hub->hdev->parent ||
		...
		if (status != -ENOTCONN && status != -ENODEV) {
			...
			usb_check_enum_dev_state(hdev->bus);
		}
	}
...
static void hub_event(struct work_struct *work)
{
	...
	for (i = 1; i <= hdev->maxchild; i++) {
		...
		if (test_bit(i, hub->event_bits)
			...
			if (check_vendor_dock_quirk(hdev, hub, i))
				port_event(hub, i);
#else
			port_event(hub, i);
#endif
