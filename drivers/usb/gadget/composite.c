...
#ifdef CONFIG_USB_HONOR_PC_DRIVER_COMPATIBILITY
	/* return -EOPNOTSUPP for invalid request send by pcdriver */
	if ((ctrl->bRequestType == 0x21) && (ctrl->bRequest == 0x02) &&
		(w_value == 0x01) && (w_index == 0x00) && (w_length == 2))
		return value;
#endif
...
unknown:
		...
		pr_info("non-core control req%02x.%02x v%04x i%04x l%d\n",
