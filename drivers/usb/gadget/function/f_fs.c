...
#define HW_HEAD1           0x4B
#define HW_NEW_HEAD        0xC8
#define HW_HEAD2           0xC9
#define QCOM_HEAD1         0x7B
#define QCOM_HEAD2         0X0
#define MAX_BUF_SIZE       16
#define MAX_BUF_PART_SIZE  17
#define PRE_FIX_TYPE       8
#define ROW_SIZE           1
#define HN_AT_HEAD1        0x41
#define HN_AT_HEAD2        0x54
#endif
...
static void ffs_user_copy_worker(struct work_struct *work)
{
	...
	u8 req_buffer[MAX_BUF_PART_SIZE] = {0};

	if (ffs && ffs->dev_name) {
		if (strcmp(ffs->dev_name, "diag") == 0) {
			if (ret > 0) {
				pr_err("[USB_LOGS]Data %s usb, length = %d\n", io_data->read ? "from" : "to", ret);
				memcpy(req_buffer, (u8 *)io_data->buf,
					sizeof(req_buffer) - 1);
				/* honor diag log printf */
				if (((HW_HEAD1 == *((u8 *)io_data->buf)) &&
					((HW_HEAD2 == *((u8 *)io_data->buf + 1)) ||
					(HW_NEW_HEAD == *((u8 *)io_data->buf + 1)))) ||
					((HN_AT_HEAD1 == *((u8 *)io_data->buf)) &&
					(HN_AT_HEAD2 == *((u8 *)io_data->buf + 1))))
					print_hex_dump(KERN_INFO, "Data: ",
						PRE_FIX_TYPE, ROW_SIZE, DUMP_PREFIX_ADDRESS,
						req_buffer, MAX_BUF_SIZE, 1);

				/*printf  7B 00 01 02 03 04 05 06 07 08 09 9D 5F 7E */
				if ((QCOM_HEAD1 == *((u8 *)io_data->buf)) &&
					(QCOM_HEAD2 == *((u8 *)io_data->buf + 1)))
					print_hex_dump(KERN_INFO, "Data: ",
						PRE_FIX_TYPE, ROW_SIZE, DUMP_PREFIX_ADDRESS,
						req_buffer, MAX_BUF_SIZE, 1);
			} else {
				pr_err("[USB_LOGS]ret = %d invalid\n", ret);
			}
		}
	}
#endif
...
#ifdef CONFIG_USB_HONOR_LOG_DEBUG
		if (!(io_data->read) && (strcmp(io_data->ffs->dev_name, "diag") == 0))
			pr_err("[USB_LOGS]diag read async queued %zd bytes on %s\n", data_len, epfile->name);
#endif
