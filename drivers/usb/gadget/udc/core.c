...
#include <linux/hisi/usb/chip_usb_interface.h>
#endif
...
#ifdef CONFIG_CHIP_USB_CONFIGFS
	gadget->is_removing_driver = 1;
#endif
	gadget->udc->driver->disconnect(gadget);
#ifdef CONFIG_CHIP_USB_CONFIGFS
	gadget->is_removing_driver = 0;
#endif
...
#ifdef CONFIG_CHIP_USB_CONFIGFS
static struct usb_udc* usb_gadget_get_udc(void)
{
	...
	list_for_each_entry(udc, &udc_list, list) {
		if (udc->driver && udc->gadget) {
			dev_info(&udc->dev, "UDC had already get\n");
			break;
		}
	}
...
int dwc3_gadget_get_reset_cnt(void)
{
	struct usb_udc *udc = usb_gadget_get_udc();

	if (!udc || !udc->gadget) {
		pr_err("%s:udc is NULL\n", __func__);
		return 0;
	}

	return udc->gadget->reset_cnt;
}

void usb_gadget_udc_limit_speed(bool flag)
{
	static enum usb_device_speed speed = USB_SPEED_UNKNOWN;
	struct usb_udc *udc = usb_gadget_get_udc();
	...
	mutex_lock(&udc_lock);
	if (speed == USB_SPEED_UNKNOWN)
		speed = udc->driver->max_speed;

	if (flag)
		udc->driver->max_speed = USB_SPEED_HIGH;
	else if (speed != USB_SPEED_UNKNOWN )
		udc->driver->max_speed = speed;

	pr_err("limit usb speed to %d\n", (int)udc->driver->max_speed);
	usb_gadget_udc_set_speed(udc, udc->driver->max_speed);
	mutex_unlock(&udc_lock);
}
#endif
