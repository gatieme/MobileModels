...
static int xhci_abort_cmd_ring(struct xhci_hcd *xhci, unsigned long flags)
{
	 ...
	 */
	if (xhci->quirks & XHCI_CACMD_TIMEOUT_QUIRK) {
		ret = xhci_handshake(&xhci->op_regs->cmd_ring,
			CMD_RING_RUNNING, 0, 100);
		if (ret < 0) {
			if (ret == -ETIMEDOUT) {
				u32 portsc;
				struct xhci_port *port = &xhci->hw_ports[1];

				portsc = readl(port->addr);
				portsc = xhci_port_state_to_neutral(portsc);
				portsc = (portsc | PORT_WR);
				writel(portsc, port->addr);
				xhci_err(xhci, "CA Timeout, Need do warm reset\n");
			} else {
		...
		if (ret < 0) {
			xhci_err(xhci, "Abort failed to stop command ring: %d in 5s\n", ret);
			xhci_halt(xhci);
			xhci_hc_died(xhci);
			return ret;
		}
		...
		struct xhci_event_cmd *event)
{
	...
	if (!cmd_dequeue_dma || cmd_dma != (u64)cmd_dequeue_dma) {
		...
		if (xhci->quirks & XHCI_ABORT_CMD_QUIRK) {
			if (cmd_comp_code != COMP_COMMAND_RING_STOPPED) {
				xhci_warn(xhci,
				  "ERROR mismatched command completion event, but not stop cmd\n");
				return;
			}
		} else {
			xhci_warn(xhci,
	...
	if (cmd_comp_code == COMP_COMMAND_ABORTED) {
		...
		if (cmd->status == COMP_COMMAND_ABORTED) {
			if (xhci->quirks & XHCI_ABORT_CMD_QUIRK) {
				xhci_err(xhci,
					 "Aborted Command completion event\n");
		...
		struct urb *urb, int slot_id, unsigned int ep_index)
{
	...
	is_vendor_setup = setup->bRequestType & USB_TYPE_VENDOR;
	if ((xhci->quirks & XHCI_DELAY_CTRL_DATA_STAGE) &&
		...
		giveback_first_trb(xhci, slot_id, ep_index, 0,
				start_cycle, start_trb);
		wmb();
		if (is_vendor_setup != USB_TYPE_VENDOR)
			mdelay(2);
	}
