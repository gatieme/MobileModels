...
int usb_stor_pre_reset(struct usb_interface *iface)
{
	...
	dev_info(&us->pusb_dev->dev, "reset storage\n");
#endif
...
static const struct usb_device_id usb_quirk_list[] = {
	...
	{ USB_DEVICE(0x2109, 0x0715), .driver_info = US_UDEV_QUIRK_STOP_TRANS_PRE_RESET },
	{ USB_DEVICE(0x0781, 0x55ab), .driver_info = US_UDEV_QUIRK_STOP_TRANS_PRE_RESET },
...
static void usb_detect_quirks(struct us_data *us)
{
	...
	dev_info(&us->pusb_dev->dev, "detect quirks=%x\n", us->udev_quirks);
}
#else
...
static void quiesce_and_remove_host(struct us_data *us)
{
	 ...
	 */
	dev_err(&us->pusb_dev->dev, "%s + cancel_delayed_work_sync\n", __func__);
	...
	dev_err(&us->pusb_dev->dev, "%s + scsi_lock\n", __func__);
	scsi_lock(host);
	dev_err(&us->pusb_dev->dev, "%s - scsi_lock\n", __func__);
	if (!test_bit(US_FLIDX_RESETTING, &us->dflags)) {
		dev_err(&us->pusb_dev->dev, "do transfer stop\n");
		set_bit(US_FLIDX_ABORTING, &us->dflags);
		usb_stor_stop_transport(us);
		dev_err(&us->pusb_dev->dev, "transfer stopped\n");
	}
	dev_err(&us->pusb_dev->dev, "%s + scsi_unlock\n", __func__);
	scsi_unlock(host);
	dev_err(&us->pusb_dev->dev, "%s - scsi_unlock\n", __func__);
	/*
	 ...
	 */
	dev_err(&us->pusb_dev->dev, "%s + scsi_remove_host\n", __func__);
	 ...
	 * reset delays.
	 */
	dev_err(&us->pusb_dev->dev, "%s + scsi_lock 2\n", __func__);
	scsi_lock(host);
	dev_err(&us->pusb_dev->dev, "%s - scsi_lock 2\n", __func__);
	set_bit(US_FLIDX_DISCONNECTING, &us->dflags);
	dev_err(&us->pusb_dev->dev, "%s + scsi_unlock 2\n", __func__);
	scsi_unlock(host);
	dev_err(&us->pusb_dev->dev, "%s - scsi_unlock 2\n", __func__);
	wake_up(&us->delay_wait);
	dev_err(&us->pusb_dev->dev, "%s --\n", __func__);
}
...
static void usb_stor_scan_dwork(struct work_struct *work)
{
	...
	dev_err(dev, "+ %s starting scan\n", __func__);

	/* For bulk-only devices, determine the max LUN value */
	if (us->protocol == USB_PR_BULK &&
	    !(us->fflags & US_FL_SINGLE_LUN) &&
	    !(us->fflags & US_FL_SCM_MULT_TARG)) {
		dev_err(dev, "%s mutex_lock +\n", __func__);
		mutex_lock(&us->dev_mutex);
		dev_err(dev, "%s mutex_lock -\n", __func__);
		...
		dev_err(dev, "%s mutex_unlock +\n", __func__);
		mutex_unlock(&us->dev_mutex);
		dev_err(dev, "%s mutex_unlock -\n", __func__);
	}
	dev_err(dev, "%s scsi_scan_host\n", __func__);
	scsi_scan_host(us_to_host(us));
	dev_err(dev, "- %s scan complete\n", __func__);
...
void usb_stor_disconnect(struct usb_interface *intf)
{
	...
	dev_err(&intf->dev, "%s release_everything\n", __func__);
	release_everything(us);
	dev_err(&intf->dev, "- %s \n", __func__);
}
