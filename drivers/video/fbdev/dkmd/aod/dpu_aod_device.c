...
static uint32_t set_lcd_always_on;
...
static void dpu_aod_dec_atomic(struct aod_data_t *aod_data)
{
	int temp;

	temp = atomic_read(&(aod_data->atomic_v));
	DPU_AOD_INFO("atomic_v = %d\n", temp);
	if (temp <= 0)
		return;

	atomic_dec(&(aod_data->atomic_v));
}

static int dpu_aod_inc_atomic(struct aod_data_t *aod_data)
{
	int temp;

	temp = atomic_inc_return(&(aod_data->atomic_v));
	DPU_AOD_INFO("atomic_v increased to %d\n", temp);
	if (temp == 1)
		return 1;

	DPU_AOD_INFO("no need reget dss, atomic_v = %d\n", temp);
	dpu_aod_dec_atomic(aod_data);
...
bool dpu_aod_need_fast_unblank(uint32_t panel_id)
#else
bool dpu_aod_need_fast_unblank(void)
#endif
{
	if (dpu_get_panel_product_type() != DUAL_PANEL_TYPE &&
		dpu_check_panel_product_type(dpu_get_panel_product_type()) == false)
		return false;
#if defined(CONFIG_DKMD_DPU_AOD)
	DPU_AOD_INFO("cur panel_id is %u, dpu_aod_fold_panel_id is %u, aod_status is %d, finger_down_status is %d!\n",
			panel_id, dpu_aod_fold_panel_id,
			g_aod_data->aod_status, g_aod_data->finger_down_status);
	if (panel_id != dpu_aod_fold_panel_id)
		return false;
#endif
...
static int dpu_aod_start_recovery_init(struct aod_data_t *aod_data)
{
	...
	if (aod_data->vivobus_level_mcu.vivobus_level != 0) {
...
static void aod_sfr_data(void)
{
	...
	offset = offsetof(struct aod_common_data, cmd_type);
	if (offset > g_aod_sfr_data->size) {
		DPU_AOD_ERR("offset is bigger than len\n");
		return;
	}
	len = g_aod_sfr_data->size - offset;
#if defined(CONFIG_INPUTHUB_30)
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)(&(g_aod_sfr_data->cmd_type));
	pkg_ap.wr_len = len;
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
#else
	if (len > IPC_MAX_SIZE - sizeof(struct pkt_header)) {
		ret = shmem_send(TAG_AOD, &(g_aod_sfr_data->cmd_type), len);
		DPU_AOD_INFO("shmem_send tag is %d, size %ud\n", TAG_AOD, len);
		return;
	}
	pkg_ap.tag = TAG_AOD;
...
static int dpu_aod_start_recovery_req(struct aod_data_t *aod_data)
{
	...
	if (g_aod_common_data != NULL) {
		...
		pkg_ap.tag = TAG_AOD;
		pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
		pkg_ap.wr_buf = (const void *)(g_aod_common_data->data);
...
static unsigned long dpu_aod_fb_alloc(struct aod_data_t *aod_data, unsigned long new_size)
{
	...
	if (unlikely((new_size == 0) || (new_size > aod_data->max_buf_size))) {
...
static int dpu_aod_set_display_space_req(aod_display_spaces_mcu_t *display_spaces)
{
	...
	pkt.display_spaces.dual_clocks = display_spaces->dual_clocks;
	pkt.display_spaces.display_space_count = display_spaces->display_space_count;
	pkt.display_spaces.pd_logo_final_pos_y = display_spaces->pd_logo_final_pos_y;

	for (i = 0; i < pkt.display_spaces.display_space_count; i++) {
		pkt.display_spaces.display_spaces[i].x_size = display_spaces->display_spaces[i].x_size;
		pkt.display_spaces.display_spaces[i].y_size = display_spaces->display_spaces[i].y_size;
		pkt.display_spaces.display_spaces[i].x_start = display_spaces->display_spaces[i].x_start;
		pkt.display_spaces.display_spaces[i].y_start = display_spaces->display_spaces[i].y_start;
	}
	dpu_dump_display_space(&pkt.display_spaces);
...
static int dpu_aod_start_req(aod_start_config_mcu_t *start_config)
{
	...
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)(start_config);
	pkg_ap.wr_len = sizeof(aod_start_config_mcu_t);
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
#else
#ifdef CONFIG_CONTEXTHUB_SHMEM
#if SHMEM_START_CONFIG
...
static int dpu_aod_stop_req(aod_display_pos_t *display_pos)
{
	...
	if (g_aod_data->finger_down_status == 2 && dpu_aod_inc_atomic(g_aod_data)){
...
#if !defined(CONFIG_DPU_FB_V320) && !defined(CONFIG_DKMD_DPU_AOD)
...
static int dpu_aod_pos_data_req(void)
{
	...
	if (pkg_mcu.errno != 0) {
		...
		if (pkg_ap.wr_len < pkg_mcu.data_length) {
			...
			(void)memcpy_s(g_aod_pos_data->data, pkg_mcu.data_length,
...
static void fast_unlock_mode_handle(bool forced)
{
	DPU_AOD_INFO("finger_down_status is %d\n",
		g_aod_data->finger_down_status);
	if ((g_aod_data->finger_down_status == STATUS_FINGER_CHECK_OK || forced) &&
		dpu_aod_inc_atomic(g_aod_data)) {
		DPU_AOD_INFO("in fast unlock mode\n");
		mutex_lock(&(g_aod_data->aod_lock));
		g_aod_data->aod_lock_status = true;
		DPU_AOD_INFO("hold aod_lock\n");
...
static int dpu_aod_pause_new_req(void)
{
	...
	g_aod_data->aod_lock_status = true;
	DPU_AOD_INFO("release aod_lock\n");
	g_aod_data->vivobus_level_mcu.vivobus_level = 0;
	fast_unlock_mode_handle(false);
	DPU_AOD_INFO("-\n");
	return ret;
}
...
static int dpu_aod_set_finger_status(struct aod_data_t *aod_data, const void __user* arg)
{
	...
	old_finger_status = aod_data->finger_down_status;
...
#if !defined(CONFIG_DKMD_DPU_AOD)
	if ((dpu_get_panel_product_type() == DUAL_PANEL_TYPE ||
		dpu_check_panel_product_type(dpu_get_panel_product_type()) == true) &&
		(aod_data->finger_down_status == STATUS_FINGER_CHECK_OK) &&
#else
	if	((aod_data->finger_down_status == STATUS_FINGER_CHECK_OK) &&
#endif
		(old_finger_status == STATUS_FINGER_DOWN) &&
		(aod_data->aod_status == false)) {
		DPU_AOD_INFO("forced trigger dss power up wq\n");
		fast_unlock_mode_handle(true);
	}
	return 0;
}
	...
	uint32_t length)
{
	...
	(void)memcpy_s((char *)g_aod_gen_data[cmd_type], length, (char *)aod_gen_data, length);
	return 0;
}
	...
	const void __user *arg)
{
	...
	if (ret) {
		// save aod_gen_data if ipc failed
		aod_gen_data_process(aod_gen_data, length);
		kfree(aod_gen_data);
		aod_gen_data = NULL;
		return ret;
	}
...
static int dpu_aod_fold_info_req(aod_fold_info_config_mcu_t *fold_info_config)
{
	...
	pkt.fold_info_config_param.fold_state = fold_info_config->fold_state;
	pkt.fold_info_config_param.display_region = fold_info_config->display_region;
	pkg_ap.wr_buf = &hd[1];
	pkg_ap.wr_len = sizeof(pkt) - sizeof(pkt.hd);
	/* 1 for lock */
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
	...
	const void __user *arg)
{
	...
	(void)memset_s(&aod_fold_info, sizeof(aod_fold_info), 0, sizeof(aod_fold_info));
	...
	aod_data->fold_info_config_mcu.fold_state =
		aod_fold_info.fold_state;
	aod_data->fold_info_config_mcu.display_region =
		aod_fold_info.display_region;
	ret = dpu_aod_fold_info_req(&aod_data->fold_info_config_mcu);
	if (ret) {
		DPU_AOD_ERR("dpu_aod_fold_info_req fail\n");
		return ret;
	...
	DPU_AOD_INFO("- fold_panel_id 0x%x fold_state %d display_region %d\n",
				dpu_aod_fold_panel_id, aod_fold_info.fold_state, aod_fold_info.display_region);
	return 0;
}
...
static int aod_bl_data_receive(uint32_t *len, void __user *arg)
{
	...
	if (copy_from_user(g_aod_bl_data, arg, *len)) {
		DPU_AOD_ERR("copy_from_user failed\n");
		kfree(g_aod_bl_data);
		g_aod_bl_data = NULL;
		return -EFAULT;
	}
	if (g_aod_bl_data->size != *len) {
		DPU_AOD_ERR("err size=%u, *len=%u\n", g_aod_bl_data->size, *len);
		kfree(g_aod_bl_data);
		g_aod_bl_data = NULL;
		return -EFAULT;
	}
...
static int dpu_aod_bl_data_req(void)
{
	int ret;
	struct write_info pkg_ap;
	struct read_info pkg_mcu;
	...
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)(g_aod_bl_data->data);
	pkg_ap.wr_len = g_aod_bl_data->size - sizeof(uint32_t);
	/* set is_lock be false to avoid blocking face lock, 0 for nolock */
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, &pkg_mcu, 0);
	if (ret)
		DPU_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
	if (pkg_mcu.errno != 0) {
		...
		if (pkg_ap.wr_len < pkg_mcu.data_length) {
			...
			(void)memcpy_s(g_aod_bl_data->data, pkg_mcu.data_length,
				(const void *)(pkg_mcu.data), pkg_mcu.data_length);
			ret = DPU_AOD_OK;
		}
	}
...
static int dpu_aod_get_last_backlight(struct aod_data_t *aod_data, void __user *arg)
{
	...
	ret = aod_bl_data_receive(&length, arg);
	if (ret) {
		DPU_AOD_ERR("aod_bl_data_receive fail!\n");
		return ret;
	}

	ret = dpu_aod_bl_data_req();
	if (ret) {
		DPU_AOD_ERR("dpu_aod_bl_data_req fail\n");
		kfree(g_aod_bl_data);
		g_aod_bl_data = NULL;
		return ret;
	}
...
static int dpu_aod_get_last_screen_status(struct aod_data_t *aod_data, void __user *arg)
{
	...
	pp = (uint8_t *)(void *)g_aod_screen_status_data;
	pp += sizeof(uint32_t) * SCREEN_STATUS_SHIFT; // skip lenth and subcmd
	p = (uint32_t *)(void *)pp;
...
static int dpu_aod_set_display_param_req(aod_display_param_mcu_t *display_param_mcu)
{
	...
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)display_param_mcu; // addr: start addr of subtype
	pkg_ap.wr_len = sizeof(aod_display_param_mcu_t);
	/* 1 for lock */
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
	if (ret) {
		DPU_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
...
static int dpu_aod_set_display_param(struct aod_data_t *aod_data,
	const void __user *arg)
{
	int ret;
	aod_display_param_t aod_display_param;
	...
	(void)memset_s(&aod_display_param, sizeof(aod_display_param), 0, sizeof(aod_display_param));
	if (copy_from_user(&aod_display_param, arg, sizeof(aod_display_param))) {
		DPU_AOD_INFO("copy from user fail!\n.\n");
		return -EFAULT;
	}
	aod_data->display_param_mcu.sub_cmd = aod_display_param.sub_cmd;
	aod_data->display_param_mcu.litehdm_color_mode =
		aod_display_param.litehdm_color_mode;

	ret = dpu_aod_set_display_param_req(&aod_data->display_param_mcu);
	if (ret) {
		DPU_AOD_ERR("dpu_aod_set_display_param_req fail\n");
		return ret;
	}
...
static int dpu_aod_set_vivobus_level_req(aod_set_vivobus_level_data_mcu_t *vivobus_level_mcu)
{
	...
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)vivobus_level_mcu; // addr: start addr of subtype
	pkg_ap.wr_len = sizeof(aod_set_vivobus_level_data_mcu_t);
	/* 1 for lock */
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
	if (ret) {
		DPU_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
...
static int dpu_aod_set_vivobus_level(struct aod_data_t *aod_data,
	const void __user *arg)
{
	int ret;
	aod_set_vivobus_level_data_t aod_vivobus_level;
	...
	(void)memset_s(&aod_vivobus_level, sizeof(aod_vivobus_level), 0, sizeof(aod_vivobus_level));
	if (copy_from_user(&aod_vivobus_level, arg, sizeof(aod_vivobus_level))) {
		DPU_AOD_INFO("copy from user fail!\n.\n");
		return -EFAULT;
	}
	aod_data->vivobus_level_mcu.sub_cmd = aod_vivobus_level.sub_cmd;
	aod_data->vivobus_level_mcu.vivobus_level = aod_vivobus_level.vivobus_level;
...
static int sfr_info_send_to_sensorhub(uint32_t length, struct aod_common_data *sfr_data)
{
	struct write_info pkg_ap;
	uint32_t len;
	int ret = 0;

	DPU_AOD_INFO("sfr send to sensorhub start\n");
	if (offsetof(struct aod_common_data, data) > length) {
		DPU_AOD_ERR("offset is bigger than len\n");
		return ret;
	}
	len = length - offsetof(struct aod_common_data, cmd_type);
#if defined(CONFIG_INPUTHUB_30)
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)(&(sfr_data->cmd_type));
	pkg_ap.wr_len = len;
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
	if (ret) {
		DPU_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
...
#else
	if (len > IPC_MAX_SIZE - sizeof(struct pkt_header)) {
		ret = shmem_send(TAG_AOD, &(sfr_data->cmd_type), len);
		DPU_AOD_INFO("shmem_send tag is %d, size %ld\n", TAG_AOD, len);
		return ret;
	}
	pkg_ap.tag = TAG_AOD;
...
static int dpu_aod_set_safe_frame_rate_info(struct aod_data_t *aod_data, const void __user *arg)
{
	...
	if (copy_from_user(&data, arg, sizeof(struct aod_common_data))) {
		DPU_AOD_ERR("copy_from_user failed\n");
		return -EFAULT;
	}
	...
	if (copy_from_user(g_aod_sfr_data, arg, data.size)) {
		DPU_AOD_ERR("copy_from_user failed\n");
		kfree(g_aod_sfr_data);
		g_aod_sfr_data = NULL;
		return -EFAULT;
	}
	if (g_aod_sfr_data->size != data.size) {
		DPU_AOD_ERR("err size=%u len=%u\n", g_aod_sfr_data->size, data.size);
		kfree(g_aod_sfr_data);
		g_aod_sfr_data = NULL;
		return -EFAULT;
	}
	ret = sfr_info_send_to_sensorhub(data.size, g_aod_sfr_data);
	return ret;
}

static int dpu_aod_set_lcd_always_on_req(aod_lcd_status_data_mcu_t *lcd_status_mcu)
{
	int ret;
	struct write_info pkg_ap;
	...
	pkg_ap.tag = TAG_AOD;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = (const void *)lcd_status_mcu; // addr: start addr of subtype
	pkg_ap.wr_len = sizeof(aod_lcd_status_data_mcu_t);
	/* 1 for lock */
	ret = dpu_aod_send_cmd_to_sensorhub(&pkg_ap, NULL, 1);
	if (ret) {
		DPU_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
...
static int dpu_aod_set_lcd_always_on(struct aod_data_t *aod_data, const void __user *arg)
{
	aod_lcd_status_data_t aod_lcd_info;
	int ret;
	...
	(void)memset_s(&aod_lcd_info, sizeof(aod_lcd_info), 0, sizeof(aod_lcd_info));
	if (copy_from_user(&aod_lcd_info, arg, sizeof(aod_lcd_info))) {
		DPU_AOD_INFO("copy from user fail!\n");
		return -EFAULT;
	}
	if (aod_lcd_info.size != sizeof(aod_lcd_info)) {
		DPU_AOD_ERR("aod_lcd_info wrong size!\n");
		return -EFAULT;
	}
	aod_data->lcd_status_mcu.sub_cmd = aod_lcd_info.sub_cmd;
	aod_data->lcd_status_mcu.lcd_on = aod_lcd_info.lcd_on;

	set_lcd_always_on = aod_lcd_info.lcd_on;
	DPU_AOD_INFO("set_lcd_always_on %u.\n", set_lcd_always_on);

	ret = dpu_aod_set_lcd_always_on_req(&aod_data->lcd_status_mcu);
	if (ret) {
		DPU_AOD_ERR("dpu_aod_set_lcd_always_on_req fail\n");
		return ret;
	}
	DPU_AOD_INFO("-.\n");
	return 0;
...
static long dpu_aod_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	case AOD_IOCTL_AOD_SET_MULTI_GMP:
		...
		break;
	case AOD_IOCTL_AOD_SET_SAFE_FRAME_RATE:
		ret = dpu_aod_set_safe_frame_rate_info(aod_data, argp);
		break;
	case AOD_IOCTL_AOD_FULL_SCREEN_BACKLIGHT:
		ret = dpu_aod_get_last_backlight(aod_data, argp);
		break;
	case AOD_IOCTL_AOD_SET_LCD_ALWAYS_ON:
		ret = dpu_aod_set_lcd_always_on(aod_data, argp);
		break;
	case AOD_IOCTL_AOD_SET_DISPLAY_PARAM:
		ret = dpu_aod_set_display_param(aod_data, argp);
		break;
...
#define	DPU_AOD_SET_POWER_MODE_TRYLOCK_TIMES		400
...
// wait for Sensorhub for 2 seconds and still can't get the aod_lock, there may be something wrong happens with
// whatever User done. Just in case we got the issue for AP waiting aod_lock over 2 seconds, we need to force stop
...
int dpu_aod_set_blank_mode(int blank_mode)
{
	...
	if ((blank_mode == FB_BLANK_UNBLANK)
		...
		if(dpu_aod_inc_atomic(aod_data)) {
