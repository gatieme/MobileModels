...
static struct cnode *get_valid_cmdlist_header(uint32_t scene_id, uint32_t cmdlist_id)
{
	...
	if (unlikely(!client)) {
		dpu_pr_warn("invalid scene_id:%u, cmdlist_id:%u!", scene_id, cmdlist_id);
		return NULL;
	}

	if (unlikely(client->node_type != SCENE_NOP_TYPE)) {
		dpu_pr_warn("invalid scene header client type=%d!", client->node_type);
		return NULL;
	}

	if (unlikely(!client->node)) {
		dpu_pr_warn("header client node is null!");
		return NULL;
	}
...
static int32_t cmdlist_check_cmd_items_valid(struct cnode *list_header)
{
	...
	clist_for_each_entry_safe(node, _node_, list_header, typeof(*node), cnode) {
		...
		if (!node || !node->list_cmd_header) {
			dpu_pr_err("node%u: node or list_cmd_header is NULL", node_count);
			return -1;
		}
...
static int32_t dkmd_cmdlist_exec_hardware_link(struct cnode *list_header)
{
	struct cmdlist_node *node = NULL, *_node_ = NULL;
...
dma_addr_t dkmd_cmdlist_get_dma_addr(uint32_t scene_id, uint32_t cmdlist_id)
{
	...
	if (dkmd_cmdlist_exec_hardware_link(list_header) != 0) {
		dpu_pr_err("scene_id=%u cmdlist_id=%u, node list has NULL pointer", scene_id, cmdlist_id);
		goto out_return;
	}
...
void dkmd_cmdlist_dump_dm(uint32_t scene_id, uint32_t cmdlist_id)
{
	struct cnode *list_header = NULL;
	struct cmdlist_node *node = NULL, *_node_ = NULL;
	...
	list_header = get_valid_cmdlist_header(scene_id, cmdlist_id);
	if (unlikely(!list_header)) {
		dpu_pr_err("list_header is NULL!");
		return;
	}
