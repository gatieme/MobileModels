...
static int32_t cmdlist_alloc_items(struct cmdlist_node *node, uint32_t size, uint32_t dst_addr)
{
	...
	dpu_pr_debug("alloc node list_cmd_header[%#x].node_size[%zu]",
		node->list_cmd_header, size);
...
struct cmdlist_node *cmdlist_create_node(uint32_t scene_id, uint32_t mode, uint32_t size, uint32_t dst_addr)
{
	...
	if (node->cmd_mode == NOP_MODE) {
		/* scene header */
		clist_init(&node->cnode);
		return node;
	}

	if (node->cmd_mode == TRANSPORT_MODE) {
		if (((size % CMDLIST_ITEMS_ALIGN_SIZE) != 0) || (size > CMDLIST_ITEMS_COUNT_MAX * CMDLIST_ITEMS_ALIGN_SIZE)) {
			dpu_pr_err("size=%#x is not 128bit aligned or out of range!", size);
			kfree(node);
			return NULL;
		}
...
int32_t cmdlist_release_node(struct cmdlist_node *node)
{
	...
	if (node->cnode.next && node->cnode.prev)
		clist_remove(&node->cnode); // remove node from appended scene list
...
void cmdlist_dump_node(struct cmdlist_node *node)
{
	...
	dpu_pr_info("   cmd_flag    |   next_list   |   total_items   ");
	dpu_pr_info(" --------------+---------------+-----------------");
	dpu_pr_info("   0x%8x  |   0x%8x  |   0x%8x ",
		node->list_cmd_header->cmd_flag.ul32, node->list_cmd_header->next_list,
		node->list_cmd_header->total_items.ul32);
