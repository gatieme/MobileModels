...
static inline int chash_init(struct chash_table *ht, size_t bkt_size, compare_func equal)
{
	...
	if (bkt_size == 0 || equal == NULL)
		return -1;

	ht->bkt_size = bkt_size;
	ht->equal = equal;
	ht->bkts = (struct cnode *)kzalloc(sizeof(struct cnode) * bkt_size, GFP_KERNEL);
	if (!ht->bkts)
		return -1;
...
{
	struct cnode *list = &ht->bkts[bkt_idx % ht->bkt_size];
...
static inline bool chash_is_valid(const struct chash_table *ht)
{
	return ht->bkt_size > 0 && ht->equal != NULL && ht->bkts != NULL;
}
