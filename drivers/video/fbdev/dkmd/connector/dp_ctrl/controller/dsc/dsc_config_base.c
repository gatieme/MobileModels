...
static void dptx_program_dsc_bits_perpixel(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] %s PPS  bpp=%u", __func__, dptx->vparams.dp_dsc_info.dsc_info.dsc_bpp);
}
...
void dptx_program_dsc_hrdelay(struct dp_ctrl *dptx)
{
	...
	pps |= (dptx->vparams.dp_dsc_info.dsc_info.scale_decrement_interval & GENMASK(11, 8)) >> 8;
