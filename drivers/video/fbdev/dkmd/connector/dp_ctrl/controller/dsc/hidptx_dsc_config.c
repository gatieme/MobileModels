...
static void ldi_clock_delay_calculation(struct dp_ctrl *dptx)
{
	...
	dpu_pr_debug("[DP] [HiDP] h_active_new is %d\n", h_active_new);
	dpu_pr_debug("[DP] [HiDP] h_blank_new is %d\n", h_blank_new);
	dpu_pr_debug("[DP] [HiDP] h_front_porch_new is %d\n", h_front_porch_new);
	dpu_pr_debug("[DP] [HiDP] h_sync_width_new is %d\n", h_sync_width_new);
	dpu_pr_debug("[DP] [HiDP] h_back_porch_new is %d\n", h_back_porch_new);
...
static void dptx_dsc_calc_timestamp(struct dp_ctrl *dptx)
{
	...
	dpu_pr_debug("[DP] [HIDP dsc] chunk size is %d\n", dptx->vparams.dp_dsc_info.dsc_info.chunk_size);

	reg |= dptx->vparams.dp_dsc_info.dsc_info.chunk_size << 0;
	if (unlikely(dptx->vparams.dp_dsc_info.dsc_info.slice_width == 0)) {
		dpu_pr_err("[HIDP dsc] slice_width cannot be zero");
		return;
	}
	...
	dpu_pr_err("[DP] [HIDP dsc] 0x25c write reg is %x\n", reg);
