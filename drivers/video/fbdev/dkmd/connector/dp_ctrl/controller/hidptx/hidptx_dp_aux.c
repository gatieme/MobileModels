...
again:
	...
	if (tries > 5)
		dpu_pr_info("[DP] device len=%d, try=%d", len, tries);
	...
	aux_req_val = dptx_readl(dptx, DPTX_AUX_REQ);
	aux_req_val |= DPTX_CFG_AUX_REQ;
	dptx_writel(dptx, DPTX_AUX_REQ, aux_req_val);
