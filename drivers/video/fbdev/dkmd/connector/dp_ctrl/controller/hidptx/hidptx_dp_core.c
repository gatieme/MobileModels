...
void dptx_global_intr_en(struct dp_ctrl *dptx)
{
	...
	irq_ien = DPTX_IRQ_STREAM0_FRAME | DPTX_IRQ_HPD_SHORT;
...
int dptx_core_deinit(struct dp_ctrl *dptx)
{
	...
	if (dptx->dptx_hdcp_enable_video_transform)
		dptx->dptx_hdcp_enable_video_transform(dptx, false);
...
static int dp_dis_reset(struct dp_ctrl *dptx, bool benable)
{
	...
	if (benable) {
		...
		set_reg(SOC_HSDT1_CRG_PEREN0_ADDR(dp_pri->hsdt1_crg_base), 1, 1, 27);  /* 0xEFB02000 + 0x0 bit27 */
		set_reg(SOC_HSDT1_CRG_PERRSTDIS0_ADDR(dp_pri->hsdt1_crg_base), 1, 1, 25); /* 0xEFB02000 + 0x24 bit25 */
...
static int dptx_core_on(struct dp_ctrl *dptx)
{
	...
	if (dptx->dptx_hdcp_enable_video_transform)
		dptx->dptx_hdcp_enable_video_transform(dptx, true);
	...
	if (dptx->connector->conn_info->base.mode == EDP_MODE) {
		...
		phyifctrl |= DPTX_CFG_AUX_RXTIMEOUT_SEL_MASK;
	}
	...
	hdcp_notification(&dptx->sdev, HOT_HDCP_ON);
	return 0;
}
...
void dptx_core_off(struct dp_ctrl *dptx)
{
	...
	hdcp_notification(&dptx->sdev, HOT_HDCP_OFF);
	msleep(1000); // wait for hdcp ta exit
...
void dptx_phy_set_rate(struct dp_ctrl *dptx, uint32_t rate)
{
	...
	dpu_pr_info("[DP] DPTX set rate=%u", rate);
...
int dptx_phy_wait_busy(struct dp_ctrl *dptx, uint32_t lanes)
{
	...
	case 1:
		...
		break;
	default:
		dpu_pr_err("[DP] Invalid number of lanes %u", lanes);
		break;
	}
...
static void dptx_default_params_from_core(struct dp_ctrl *dptx)
{
	...
	if (dptx->connector->conn_info->base.fpga_flag == 1)
		...
		dptx->max_rate = DPTX_PHYIF_CTRL_MAX_RATE;
