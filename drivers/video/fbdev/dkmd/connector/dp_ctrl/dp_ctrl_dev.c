...
int dptx_is_power_saving_mode(struct dkmd_connector_info *connector_pinfo)
{
	struct dp_private *dp_priv = NULL;
	struct dp_ctrl *dptx = NULL;
	struct dpu_connector *connector = NULL;
	dpu_check_and_return(!connector_pinfo, -EINVAL, err, "connector_pinfo is nullptr");

	connector = get_primary_connector(connector_pinfo);
	dpu_check_and_return(!connector, -EINVAL, err, "connector is nullptr");

	dp_priv = to_dp_private(connector->conn_info);
	dpu_check_and_return(!dp_priv, -EINVAL, err, "dp_priv is nullptr");

	dptx = &dp_priv->dp[MASTER_DPTX_IDX];
	dpu_check_and_return(!dptx, -EINVAL, err, "dptx is nullptr");

	return dptx->power_saving_mode ? 1 : 0;
}
