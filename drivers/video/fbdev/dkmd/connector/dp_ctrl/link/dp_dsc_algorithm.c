...
int dptx_get_slice_count(int ppr)
{
	...
	for (i = 0; i < (uint32_t)ARRAY_SIZE(g_slice_count_maps); i++) {
...
void dptx_sink_color_depth_capabilities(struct dp_ctrl *dptx)
{
	...
	if (dsc_color_depth & DP_DSC_8_BPC)
		dpu_pr_info("[DP] Sink supports 8 bpc");

	if (dsc_color_depth & DP_DSC_10_BPC)
		dpu_pr_info("[DP] Sink supports 10 bpc");

	if (dsc_color_depth & DP_DSC_12_BPC)
		dpu_pr_info("[DP] Sink supports 12 bpc");
}
...
void dptx_sink_color_format_capabilities(struct dp_ctrl *dptx)
{
	...
	if (dsc_color_format & DP_DSC_RGB)
		dpu_pr_info("[DP] Sink supports RGB color format");

	if (dsc_color_format & DP_DSC_YCBCR_444)
		dpu_pr_info("[DP] Sink supports YCbCr 4:4:4 color format");

	if (dsc_color_format & DP_DSC_YCBCR_422_SIMPLE)
		dpu_pr_info("[DP] Sink supports YCbCr 4:2:2 SIMPLE color format");

	if (dsc_color_format & DP_DSC_YCBCR_422_NATIVE)
		dpu_pr_info("[DP] Sink supports YCbCr 4:2:2 NATIVE color format");

	if (dsc_color_format & DP_DSC_YCBCR_420_NATIVE)
		dpu_pr_info("[DP] Sink supports YCbCr 4:2:0 NATIVE color format");
}
...
int dptx_get_rx_line_buffer_depth(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] %s PPS buf_bit_depth=%u", __func__, buf_bit_depth);
...
int dptx_dsc_initial(struct dp_ctrl *dptx)
{
	...
	if (dptx->dsc && (byte & DP_DSC_DECOMPRESSION_IS_SUPPORTED)) {
		...
		dptx->dsc = true;
		if (dptx->connector->conn_info->base.mode == DP_MODE)
			dptx->fec = true;
	} else {
		dptx->dsc = false;
		dptx->fec = false;
