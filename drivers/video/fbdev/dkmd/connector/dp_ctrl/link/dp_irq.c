	...
	struct timing_info *per_timing_info, uint32_t fps, uint32_t fps_default)
{
	...
	if (g_dp_debug_mode_enable && g_dp_vactive_pixels_debug != 0 &&
...
{
	int retval;
	struct dp_ext_disp_info dp_attr = {};
	...
	if (!dptx->mst) {
		...
		if (dptx->dinfo != NULL && dptx->dinfo->is_dprx_ready && dptx->dinfo->get_ext_disp_info != NULL) {
			dptx->dinfo->get_ext_disp_info(&dp_attr, dptx->port_id);
			retval = dptx_read_edid_from_callback(dptx, dp_attr);
		} else {
...
static void dptx_debug_set_clk_hblank_params(struct dp_ctrl *dptx, struct video_params *vparams, struct dtd *mdtd)
{
	...
	if (g_dp_debug_mode_enable && dp_debug_get_clk_hblank_enable(dptx)) {
