...
static int dptx_link_read_status(struct dp_ctrl *dptx)
{
	...
	for (i = 0; i < DP_LINK_STATUS_SIZE; i++) {
		...
		dpu_pr_info("[DP] [0x202] dptx_read_dpcd 0x202, i=%d, status:%lx\n", i, dptx->link.status[i]);
	}
...
int dptx_link_training_lanes_set(struct dp_ctrl *dptx)
{
	...
	for (i = 0; i < (uint32_t)dptx->link.lanes; i++) {
...
int dptx_link_reduce_rate(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] Reducing rate from %u to %u\n", dptx->link.rate, rate);
	dptx->link.rate = rate;
	return 0;
}
