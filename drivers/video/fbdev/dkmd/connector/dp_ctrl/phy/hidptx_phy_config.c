	...
	uint16_t txdrvctrl, uint16_t txeqcoeff)
{
	...
	if ((index0 >= DPTX_COMBOPHY_PARAM_NUM) || (index1 >= DPTX_COMBOPHY_PARAM_NUM)) {
...
void dptx_combophy_set_rate(struct dp_ctrl *dptx, int rate)
{
	...
	case DPTX_PHYIF_CTRL_RATE_HBR:
		...
		dpu_pr_info("[DP] PLL1CKGCTRLR1 get value: 0x%x\n", inp32(dptx->combo_phy_base + PLL1CKGCTRLR1));
		break;
	case DPTX_PHYIF_CTRL_RATE_HBR2:
		if (g_dp_debug_mode_enable) {
			reg = (uint32_t)g_pll1ckgctrlr2;
			outp32(dptx->combo_phy_base + pll1sscg_ctrl_initial_r(2), g_hbr2_pll1_sscg[0]);
			outp32(dptx->combo_phy_base + pll1sscg_cnt_r(2), g_hbr2_pll1_sscg[1]);
			outp32(dptx->combo_phy_base + pll1sscg_cnt2_r(2), g_hbr2_pll1_sscg[2]);
		} else {
			reg = 0x046500d5; // default value for chip spec
			outp32(dptx->combo_phy_base + pll1sscg_ctrl_initial_r(2), PLL_INITIAL_R2_VAL); // default value for chip spec
			outp32(dptx->combo_phy_base + pll1sscg_cnt_r(2), PLL_CNT_R2_VAL); // default value for chip spec
			outp32(dptx->combo_phy_base + pll1sscg_cnt2_r(2), PLL_CNT2_R2_VAL); // default value for chip spec
		}
		dpu_pr_info("[DP] PLL1CKGCTRLR2 config value: 0x%x\n", reg);
		outp32(dptx->combo_phy_base + PLL1CKGCTRLR2, reg);
		dpu_pr_info("[DP] PLL1CKGCTRLR2 get value: 0x%x\n", inp32(dptx->combo_phy_base + PLL1CKGCTRLR2));
		break;
