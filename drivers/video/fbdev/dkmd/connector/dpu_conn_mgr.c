...
static void calculate_dsc_spr_param(struct dsc_calc_info *dsc, struct spr_info *spr, struct dkmd_connector_info *pinfo)
{
	if (dsc->dsc_en != 0)
	...
	pinfo->spr_ctrl = spr->spr_ctrl.value;
	pinfo->spr_border_r_tb = spr->spr_r_bordertb.value;
	pinfo->spr_border_g_tb = spr->spr_g_bordertb.value;
	pinfo->spr_border_b_tb = spr->spr_b_bordertb.value;
}
...
void connector_device_suspend(struct dkmd_connector_info *pinfo)
{
	...
	dpu_pr_info("connector%u device suspend!\n", connector->connector_id);
}
	...
	struct device_node *parent_node)
{
	...
	conn_mgr->disp_ch1subsys_regulator = devm_regulator_get(&pdev->dev, "regulator_disp_ch1subsys");
	if (IS_ERR_OR_NULL(conn_mgr->disp_ch1subsys_regulator))
		dpu_pr_info("get disp_ch1subsys_regulator error, maybe do not support, pls check!\n");
	...
	struct device_node *parent_node)
{
	...
	for_each_child_of_node(parent_node, node) {
		...
		of_property_read_u32(node, "connector_id", &connector_id);
		if (connector_id >= CONNECTOR_ID_MAX) {
			dpu_pr_warn("connector_id=%u over %u\n", connector_id, CONNECTOR_ID_MAX);
			continue;
		}
		if (conn_mgr->connector[connector_id]) {
			dpu_pr_warn("connector connector_id=%u has been alloced\n", connector_id);
			continue;
		}
