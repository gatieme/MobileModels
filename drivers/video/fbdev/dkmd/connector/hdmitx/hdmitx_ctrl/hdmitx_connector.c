...
static void dump_sysctrl_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x0; i < 0x98; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->sysctrl_base + i),
			inp32(hdmitx->sysctrl_base + i + 0x4),
			inp32(hdmitx->sysctrl_base + i + 0x8),
			inp32(hdmitx->sysctrl_base + i + 0xc));
}

static void dump_aon_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x0; i < 0x74; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->aon_base + i),
			inp32(hdmitx->aon_base + i + 0x4),
			inp32(hdmitx->aon_base + i + 0x8),
			inp32(hdmitx->aon_base + i + 0xc));
}

static void dump_pwd_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x0; i < 0x188; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->base + i),
			inp32(hdmitx->base + i + 0x4),
			inp32(hdmitx->base + i + 0x8),
			inp32(hdmitx->base + i + 0xc));
}

static void dump_video_path_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x800; i < 0x88c; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->base + i),
			inp32(hdmitx->base + i + 0x4),
...
static void dump_hdmi_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x1800; i < 0x1bc4; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->base + i),
			inp32(hdmitx->base + i + 0x4),
...
static void dump_hdcp_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x20B0; i < 0x20f8; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->base + i),
			inp32(hdmitx->base + i + 0x4),
...
static void dump_phy_top_reg(struct hdmitx_ctrl *hdmitx)
{
	u32 i;

	for (i = 0x2800; i < 0x281c; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(hdmitx->base + i),
			inp32(hdmitx->base + i + 0x4),
...
static void connector_set_default_edid(struct hdmitx_connector *connector)
{
	if (!connector->src_cap.hdmi.hdmi_support || !connector->user_dvi_mode) {
		dpu_pr_info("force dvi mode edid.user_dvi_mode=%d", connector->user_dvi_mode);
		connector_set_edid_info(connector, g_edid_default_dvi, sizeof(g_edid_default_dvi), EDID_DEFAULT_DVI);
	} else {
		dpu_pr_info("force hdmi mode edid.user_dvi_mode=%d", connector->user_dvi_mode);
...
bool hdmitx_connector_search_mode(struct hdmitx_connector *connector, struct hdmitx_mode_config *mode)
{
	...
	list_for_each_safe(p, n, &connector->valid_modes) {
		...
		dpu_pr_debug("tmp_mode->band vic %u", tmp_mode->band.vic);
	}
...
s32 hdmitx_ctrl_tmds_set_scramble(struct hdmitx_ctrl *hdmitx)
{
	...
	mode_config.band.color_depth = g_hdmitx_color_depth_debug;
	...
	hdmitx_ddc_scdc_set_high_tmds_clock_ratio(hdmitx->ddc, false);
	...
	if (mode_config.tmds_scdc_en && hdmitx->mode == HDMITX_WORK_MODE_TMDS) {
		hdmitx_ddc_scdc_set_high_tmds_clock_ratio(hdmitx->ddc, true);
...
void hdmitx_subsys_init(struct hdmitx_ctrl *hdmitx)
{
	...
	hdmi_write_bits(hdmitx_priv->hsdt1_sys_ctrl_base, DSS2DPTX_SEL, DSS2DPTX_STREAM0_IN_SEL_MASK, 0xa);
...
static void hdmitx_tmds_phy_set(const struct hdmitx_ctrl *hdmitx)
{
	...
	u32 color_format = HDMITX_COLOR_FORMAT_RGB;
	u32 color_depth = g_hdmitx_color_depth_debug;
	u32 pixel_clk = hdmitx->select_mode.detail.pixel_clock; // to fix: here stub, need modify
...
static int hdmitx_frl_mode_start(struct hdmitx_ctrl *hdmitx)
{
	...
	// dpc pll config
	// udp to fix: need frl and tmds divided
...
void hdmitx_hotunplug()
{
	...
	struct hdmitx_private *hdmitx_priv = to_hdmitx_private(hdmitx->connector->conn_info);
