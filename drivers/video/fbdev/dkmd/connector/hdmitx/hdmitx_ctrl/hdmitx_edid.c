	...
	const struct stereo_mandatory_mode *stereo_mode)
{
	return mode->detail.h_active == stereo_mode->width &&
		   ...
		   hdmitx_modes_get_vrefresh(&mode->detail) == stereo_mode->vrefresh;
}
...
static bool edid_block_is_valid(u8 *raw_edid, u32 block, bool print_bad_edid)
{
	...
	if (csum) {
		if (print_bad_edid)
			dpu_pr_err("EDID checksum is invalid, remainder is %u", csum);
	}
	/* per-block-type checks */
	switch (raw_edid[0]) {
	case 0: /* base block 0 */
		if (edid->version != 1) {
			dpu_pr_err("EDID has major version %u, instead of 1", edid->version);
			goto bad;
		}
														   ...
														   const struct std_timing *t)
{
	...
	list_for_each_entry(m, &connector->probed_modes, head) {
		flag = m->detail.h_active == hsize && m->detail.v_active == vsize &&
			hdmitx_modes_get_vrefresh(&m->detail) == vrefresh_rate;
		if (flag)
			return NULL;
	}

	mode = hdmitx_modes_create_mode_by_std_timing(hsize, vsize, vrefresh_rate);
	return mode;
}
...
static s32 edid_get_detail_info(const struct detailed_timing *timing, struct hdmi_detail *detail)
{
	...
	detail->field_rate = hdmitx_modes_get_vrefresh(detail);
	return 0;
}
	...
	const struct detailed_timing *timing)
{
	...
	mode = hdmitx_modes_create_mode_by_detail_timing(&detail);
	if (mode != NULL)
		return mode;
...
static u32 edid_add_established_modes(struct hdmitx_connector *connector, const struct edid *edid)
{
	...
	for (i = 0; i <= EDID_EST_TIMING_NUM; i++) {
		...
		newmode = hdmitx_modes_create_mode_by_vic(VIC_VESA_800X600P60 + i);
		if (newmode != NULL) {
			newmode->parse_type |= MODE_TYPE_ESTABLISHED_TIMINGE;
			hdmitx_modes_add_probed_mode(connector, newmode);
			modes++;
		}
	}
...
static void edid_do_detailed_mode(const struct detailed_timing *timing, void *c)
{
	...
	hdmitx_modes_add_probed_mode(closure->connector, newmode);
	closure->modes++;
														   ...
														   u8 video_index)
{
	...
	newmode = hdmitx_modes_create_mode_by_vic(vic);
	if (newmode == NULL)
		return NULL;
...
static u32 edid_do_cea_modes(struct hdmitx_connector *connector, const u8 *db, u8 len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (mode != NULL) {
			...
			hdmitx_modes_add_probed_mode(connector, mode);
			modes++;
		}
	}
...
static u32 edid_add_hdmi_mode(struct hdmitx_connector *connector, u8 hdmi_vic)
{
	...
	if (!hdmi_vic_is_valid(hdmi_vic)) {
		dpu_pr_err("Unknown HDMITX VIC: %u", hdmi_vic);
		return 0;
	}
...
static void edid_parse_hdr_dynamic_type(struct hdmitx_connector *connector, u16 type, u8 version)
{
	...
	if (type == 0x0001) {
		...
		dpu_pr_debug("un-known dynamic hdr type:%u,version:%u", type, version);
	}
}
	...
	const u8 *hf_vsdb)
{
	...
	dpu_pr_info("cnm_vrr %d, cinema_vrr %d, vrr_min %d, vrr_max %d",
		vrr->cnm_vrr,
		vrr->cinema_vrr,
		vrr->vrr_min,
		vrr->vrr_max);
	...
	const u8 *hf_vsdb)
{
	...
	if (hf_vsdb[5]) { /* check the hf_vsdb's 5th byte. */
		...
		if (max_tmds_clock > 340000) { /* tmds clock is more than 340000KHz. */
			...
			dpu_pr_debug("HF-VSDB: max TMDS clock %u kHz", display->max_tmds_clock);
		}
	...
	if (db_len < 10) { /* if db_len is less than 10, no vrr description. */
		dpu_pr_info("no vrr description");
		return;
	}
	...
	const u8 *hdmi)
{
	...
	if (hdmi[6] & HDMITX_EDID_HDMITX_DC_30) { /* the bit 4 of hdmi's 6th byte. */
		...
		dpu_pr_debug("%s: HDMITX sink does deep color 30", connector->name);
	}

	if (hdmi[6] & HDMITX_EDID_HDMITX_DC_36) { /* the bit 5 of hdmi's 6th byte. */
		dc_bpc = 12; /* 12bit */
		color->depth.rgb_36 = true;
		dpu_pr_debug("%s: HDMITX sink does deep color 36", connector->name);
	}

	if (hdmi[6] & HDMITX_EDID_HDMITX_DC_48) { /* the bit 6 of hdmi's 6th byte. */
		dc_bpc = 16; /* 16bit */
		color->depth.rgb_48 = true;
		dpu_pr_debug("%s: HDMITX sink does deep color 48", connector->name);
	}

	if (dc_bpc == 0) {
		dpu_pr_debug("%s: No deep color support on this HDMITX sink", connector->name);
		return;
	}

	dpu_pr_debug("%s: Assigning HDMITX sink color depth as %u bpc", connector->name, dc_bpc);
	...
	if (hdmi[6] & HDMITX_EDID_HDMITX_DC_Y444) { /* the bit 3 of hdmi's 6th byte. */
		...
		dpu_pr_debug("%s: HDMITX sink does YCRCB444 in deep color", connector->name);
	}
	 ...
	 */
	if (!(hdmi[6] & HDMITX_EDID_HDMITX_DC_36)) /* the bit 5 of hdmi's 6th byte. */
		dpu_pr_debug("%s: HDMITX sink should do DC_36, but does not", connector->name);
}
static void edid_parse_hdmi_vsdb_video(struct hdmitx_connector *connector, const u8 *db)
{
	...
	if (len >= 7) { /* if length is more than 7, has max_tmds_clock decription. */
		...
		if (!info->max_tmds_clock) {
			...
			dpu_pr_warn("zero max_tmds,fix to %u KHz", EDID_DEFAULT_DVI_MAX_TMDS);
		}
	}

	dpu_pr_debug("HDMITX: DVI dual %d, max TMDS clock %u kHz", info->dvi_dual, info->max_tmds_clock);
...
static u32 edid_add_color_info(struct hdmitx_connector *connector, const struct edid *edid)
{
	...
	if ((color->depth.bpc == 0) && (edid->revision < 4) && /* revision is less than 4. */
		...
		dpu_pr_debug("%s: Assigning DFP sink color depth as %u bpc", connector->name, color->depth.bpc);
	}
	...
	dpu_pr_debug("%s: Assigning EDID-1.4 digital sink color depth as %u bpc", connector->name, color->depth.bpc);
