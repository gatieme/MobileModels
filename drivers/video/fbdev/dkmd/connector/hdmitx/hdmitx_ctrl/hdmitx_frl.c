...
static void dump_training_reg(struct hdmitx_frl *frl)
{
	u32 i;

	for (i = 0x9000; i < 0x90A8; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(frl->hal.frl_training_base + i),
			inp32(frl->hal.frl_training_base + i + 0x4),
			inp32(frl->hal.frl_training_base + i + 0x8),
			inp32(frl->hal.frl_training_base + i + 0xc));
}

static void dump_frl_reg(struct hdmitx_frl *frl)
{
	u32 i;

	for (i = 0x0; i < 0x20; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(frl->hal.frl_base + i),
			inp32(frl->hal.frl_base + i + 0x4),
			inp32(frl->hal.frl_base + i + 0x8),
			inp32(frl->hal.frl_base + i + 0xc));

	for (i = 0x100; i < 0x1A4; i = i + 0x10)
		dpu_pr_info("0x%x: 0x%x 0x%x 0x%x 0x%x",
			i,
			inp32(frl->hal.frl_base + i),
			inp32(frl->hal.frl_base + i + 0x4),
...
static bool frl_edid_check(const struct hdmitx_frl *frl)
{
	...
	if (!frl->config.frl_max_rate) {
		dpu_pr_warn("[hdmitx%u]frl_max_rate=%u", frl->id, frl->config.frl_max_rate);
		return false;
	}
...
static void frl_scdc_init_from_sink(struct hdmitx_frl *frl)
{
	...
	dpu_pr_info("sink_ver %u\n"
		"frl_max %u, dsc_frl_max %u, flt_no_timeout %u\n"
		"ffe_levels %d, frl_rate %d\n"
		"flt_ready %u, flt_start %u, flt_update %u\n",
		frl->scdc.sink_ver,
		frl->scdc.frl_max, frl->scdc.dsc_frl_max, frl->scdc.flt_no_timeout,
		frl->scdc.ffe_levels, frl->scdc.frl_rate,
		frl->scdc.flt_ready, frl->scdc.flt_start, frl->scdc.flt_update);
}
...
static void frl_config_init(struct hdmitx_ctrl *hdmitx, struct hdmitx_frl *frl)
{
	...
	config->frl_min_rate = g_frl_xg_xl_min_debug;
