...
static uint64_t get_default_lane_byte_clk(struct mipi_panel_info *mipi)
{
	...
	rg_pll_fbkdiv = vco_clk * (1U << rg_pll_prediv) / DEFAULT_MIPI_REF_CLK_RATE;
...
void mipi_dsi_convert_pxl2cycle(struct dpu_panel_info *pinfo)
{
	...
	if (!pinfo->mipi.need_convert_pixel2cycle)
		return;
	...
	uint64_t *lane_clock, int32_t fpga_flag)
{
	...
	if (fpga_flag) {
		...
		*lane_clock = m_pll * (DEFAULT_MIPI_CLK_RATE / n_pll) / vco_div;
		...
		phy_ctrl->rg_pll_fbkdiv = vco_clk * (1U << phy_ctrl->rg_pll_prediv) / DEFAULT_MIPI_REF_CLK_RATE;
...
void get_dsi_dphy_ctrl(struct dpu_connector *connector, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	 ...
	 */
	mipi_dsi_pll_dphy_config(phy_ctrl, &lane_clock, connector->conn_info->base.fpga_flag);
...
void get_dsi_cphy_ctrl(struct dpu_connector *connector, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	/************************  PLL parameters config  *********************/
	mipi_dsi_pll_cphy_config(phy_ctrl, &lane_clock, connector->conn_info->base.fpga_flag);
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (mipi->phy_mode == CPHY_MODE) {
		...
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001005B, phy_ctrl->rg_vrefsel_vcm);

		/* config dpy mipi drive voltage EQ */
		if (phy_ctrl->rg_vrefsel_eq != 0)
			mipi_config_phy_test_code(mipi_dsi_base, 0x00010057, phy_ctrl->rg_vrefsel_eq);
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (is_mipi_cmd_panel(&pinfo->base)) {
		...
		/* default enable vsync delay */
		set_reg(DPU_DSI_VSYNC_DELAY_CTRL_ADDR(mipi_dsi_base), 0x2, 2, 0);
		set_reg(DPU_DSI_VSYNC_DELAY_TIME_ADDR(mipi_dsi_base), 0x1, 32, 0);
	...
	if (is_mipi_cmd_panel(&pinfo->base)) {
		...
		if (mipi->hs_wr_to_time == 0)
			...
			set_reg(DPU_DSI_HS_WR_TO_TIME_CTRL_ADDR(mipi_dsi_base),
				(0x1 << 24) | (mipi->hs_wr_to_time * phy_ctrl->lane_byte_clk / 1000000000ULL), 25, 0);
	}
