...
static int32_t mipi_dsi_clk_enable(struct dpu_connector *connector)
{
	...
	if (connector->conn_info->sw_post_chn_idx[PRIMARY_CONNECT_CHN_IDX] != 0 &&
		...
		set_reg(DPU_GLB_DISP_MODULE_CLK_SEL_ADDR(connector->dpu_base + DPU_GLB0_OFFSET),
			0x1, 1, 4 - connector->conn_info->sw_post_chn_idx[PRIMARY_CONNECT_CHN_IDX]);
...
static int32_t mipi_dsi_fake_power_off(struct dpu_connector *connector, const void *value)
{
	 ...
	 * 0x1--Register value
	 */
	while (((uint32_t)inp32(DPU_DSI_VSTATE_ADDR(mipi_dsi_base)) & 0x7FF) != 0x1) {
	...
	dpu_pr_info("wait ldi vstate idle done, delay_count = %u\n", delay_count);
	return mipi_dsi_power_off(pinfo);
}
...
static irqreturn_t dpu_dsi_isr(int32_t irq, void *ptr)
{
	...
	static ktime_t frm_start_timestamp[CONNECTOR_ID_MAX] = { 0 };
	...
	if ((isr_s2 & DSI_INT_VACT0_START) == DSI_INT_VACT0_START) {
		...
		if (frm_start_timestamp[connector->connector_id]) {
			int32_t diff = (int32_t)ktime_us_delta(vactive_start_timestamp[connector->connector_id],
				frm_start_timestamp[connector->connector_id]);
			// if wait te time > 2000 us, skip intra frame dvfs
			if (diff > 2000) {
				dpu_pr_info("wait te cost %d us, skip intra frame dvfs, conn_id = %u",
					diff, connector->connector_id);
				dpu_skip_intra_frame_idle_dvfs();
			}
		}
	...
	if ((isr_s2 & DSI_INT_VACT0_END) == DSI_INT_VACT0_END) {
		...
		int32_t diff = (int32_t)ktime_us_delta(vactive_end_timestamp[connector->connector_id],
	...
	if ((isr_s2 & DSI_INT_FRM_START) == DSI_INT_FRM_START) {
		frm_start_timestamp[connector->connector_id] = ktime_get();
...
static int32_t mipi_dcs_cmd_tx_for_platform(struct dpu_connector *connector, const void *value)
{
	...
	if (connector->bind_connector)
		return mipi_dual_dsi_cmds_tx(ddic_cmds->cmds, ddic_cmds->cmd_num,
			connector->bind_connector->connector_base,
			ddic_cmds->cmds, ddic_cmds->cmd_num, dpu_base, EN_DSI_TX_NORMAL_MODE, true);
	idle_cycle = mipi_dsi_fifo_idle_cycle(dpu_base);
	if (idle_cycle >= 0) {
		if (idle_cycle != 0)
			dpu_pr_info("single mipi fifo idle cycle %d\n", idle_cycle);
		return mipi_dsi_cmds_tx(ddic_cmds->cmds, ddic_cmds->cmd_num, dpu_base);
	}
	return 0;
}
...
void mipi_dsi_default_setup(struct dpu_connector *connector)
{
	if (mipi_dsi_is_bypass_by_pg(connector->connector_id)) {
		dpu_pr_info("mipi dsi bypass by pg config");
		return;
	}
