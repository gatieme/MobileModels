...
struct dpu_connector *get_real_connector(uint32_t raw_dsi_index, uint32_t panel_type)
{
	...
	char __iomem *dsi_base)
{
	...
	if (mipi_dsi_cmd_is_write(cm)) {
		...
		ret = mipi_dsi_get_read_value(cm, tmp, tmp_value, out_len, little_endian_support);
		if (ret) {
