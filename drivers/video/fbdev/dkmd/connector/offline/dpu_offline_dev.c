...
static int32_t dpu_offline_isr_enable(struct dpu_connector *connector, const void *value)
{
	...
	/* 1. interrupt mask */
	outp32(DPU_GLB_NS_OFFLINE0_TO_GIC_MSK_ADDR(connector->connector_base + DPU_GLB0_OFFSET), mask);
	...
	/* 4. interrupt umask */
	outp32(DPU_GLB_NS_OFFLINE0_TO_GIC_MSK_ADDR(connector->connector_base + DPU_GLB0_OFFSET), isr_ctrl->unmask);
...
irqreturn_t dpu_offline_isr(int32_t irq, void *ptr)
{
	 ...
	 *
	 */
	isr1_offline_glb_state = inp32(DPU_GLB_NS_OFFLINE0_TO_GIC_O_ADDR(connector_base + DPU_GLB0_OFFSET));
...
static int32_t dpu_offline_isr_setup(struct dpu_connector *connector, const void *value)
{
	...
	ret = snprintf_s(isr_ctrl->irq_name, sizeof(isr_ctrl->irq_name), sizeof(isr_ctrl->irq_name) - 1, \
						"irq_offline_%u", connector->connector_id);
	if (ret < 0) {
