	...
	struct dkmd_connector_info *pinfo, const void *value)
{
	...
	if (priv->disp_on_cmds && (priv->disp_on_cmds_len > 0)) {
		if (connector->bind_connector) {
			ret = mipi_dual_dsi_cmds_tx(priv->disp_on_cmds, priv->disp_on_cmds_len,
				connector->connector_base, priv->disp_on_cmds, priv->disp_on_cmds_len,
				connector->bind_connector->connector_base, EN_DSI_TX_NORMAL_MODE, true);
			dpu_pr_info("use dual mipi\n");
		} else {
			ret = mipi_dsi_cmds_tx(priv->disp_on_cmds, priv->disp_on_cmds_len, connector->connector_base);
		}
	...
	struct dkmd_connector_info *pinfo, const void *value)
{
	...
	if (priv->disp_off_cmds && (priv->disp_off_cmds_len > 0)) {
		if (connector->bind_connector)
			ret = mipi_dual_dsi_cmds_tx(priv->disp_off_cmds, priv->disp_off_cmds_len,
				connector->connector_base, priv->disp_off_cmds, priv->disp_off_cmds_len,
				connector->bind_connector->connector_base, EN_DSI_TX_NORMAL_MODE, true);
		else
			ret = mipi_dsi_cmds_tx(priv->disp_off_cmds, priv->disp_off_cmds_len, connector->connector_base);
...
static int32_t prepare_panel_data(struct panel_drv_private *priv)
{
	...
	if (data->of_device_setup && data->of_device_setup(priv)) {
