...
int get_dp_ext_disp_info(struct dp_ext_disp_info *info, int dp_id)
{
	...
	info = kzalloc(sizeof(struct dp_ext_disp_info), GFP_KERNEL);
	return_value_if_run_error(!info, -ENOMEM, "dp_ext_disp_info kzalloc failed");
	info->port_id = dp_id;

	uint8_t *dp_edid = NULL;
	dp_edid = kzalloc(sizeof(uint8_t) * EDID_LEN, GFP_KERNEL);
	return_value_if_run_error(!dp_edid, -ENOMEM, "dp_edid kzalloc failed");
	...
	if (dp_id == DP_CONNECTOR_EDP0) {
		...
		ret = memcpy_s(dp_edid, sizeof(uint8_t) * EDID_LEN, edid_for_arhud_maxview, sizeof(uint8_t) * EDID_LEN);
		if (ret != 0) {
			kfree(dp_edid);
			cdc_err("[ti983]dp_edid memcpy failed");
			return ret;
		}

		info->edid = dp_edid;
		info->sinfo = kzalloc(sizeof(struct screen_info) * 2, GFP_KERNEL);
		if (ret != 0) {
			kfree(dp_edid);
			cdc_err("[ti983]screen info memcpy failed");
			return ret;
		}

		info->sinfo[0].h_active_pixels = 1920;
		info->sinfo[0].v_active_pixels = 1080;
		info->sinfo[1].h_active_pixels = 1920;
		info->sinfo[1].v_active_pixels = 1080;
	} else if (dp_id == DP_CONNECTOR_EDP1) {
		// passanger
		info->screen_num = 1;
		info->edid_blocks = 0;

		ret = memcpy_s(dp_edid, sizeof(uint8_t) * EDID_LEN, edid_for_passanger, sizeof(uint8_t) * EDID_LEN);
		if (ret != 0) {
			kfree(dp_edid);
			cdc_err("[ti983]dp_edid memcpy failed");
			return ret;
		...
		info->edid = dp_edid;
		info->sinfo = kzalloc(sizeof(struct screen_info), GFP_KERNEL);
		if (ret != 0) {
			kfree(dp_edid);
			cdc_err("[ti983]screen info memcpy failed");
			return ret;
		...
		info->sinfo->h_active_pixels = 2800;
		info->sinfo->v_active_pixels = 1200;
	}
...
int ti983_config_edid(struct serdes_data *serdes)
{
	...
	struct dp_connect_ctrl_info *dpinfo = NULL;
	dpinfo = kzalloc(sizeof(struct dp_connect_ctrl_info), GFP_KERNEL);
	return_value_if_run_error(!dpinfo, -ENOMEM, "dp_connect_ctrl_info kzalloc failed");

	dpinfo->port_id = serdes->p_data->dp_id;
	dpinfo->is_dprx_ready = true;
	dpinfo->get_ext_disp_info = get_dp_ext_disp_info;

	while (max_retry > 0) {
		ret = dpu_dptx_register_ext_disp_callback(dpinfo);
		if (ret < 0) {
