...
int dsi_vp1_sync(uint32_t idx)
{
	...
	while (1) {
		...
		if (ret < 0) {
			cdc_err("vp1 sync err: %d times", try_cnt);
		} else {
			cdc_info("vp1 sync success");
			return ret;
		}
		if (try_cnt > VP_SYNC_MAX_TRY_TIMES) {
			cdc_err("vp1 sync err over 3 times");
			return ret;
		}
		ssleep(1);
	}
...
static int ti981_pm_resume(struct device *dev)
{
	...
	serdes = i2c_get_clientdata(to_i2c_client(dev));
	serdes->lvds_link_status = MIPI_LVDS_CAB_NOT_LINKED;
	serdes->mipi_resumed = MIPI_STATUS_RESUME;
	cdc_info("finish");
	return 0;
}
