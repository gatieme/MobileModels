...
#define dpu_pr_err(msg, ...)  do { \
		pr_err("E DKMD[%s:%d]"msg, __func__, __LINE__, ## __VA_ARGS__); \
	} while (0)

#define dpu_pr_warn(msg, ...)  do { \
		if (g_dkmd_log_level >= DPU_LOG_LVL_WARNING) \
			pr_warn("W DKMD[%s:%d]"msg, __func__, __LINE__, ## __VA_ARGS__); \
	} while (0)

#define dpu_pr_info(msg, ...)  do { \
		if (g_dkmd_log_level >= DPU_LOG_LVL_INFO) \
			pr_info("I DKMD[%s:%d]"msg, __func__, __LINE__, ## __VA_ARGS__); \
	} while (0)
...
#define dpu_pr_debug(msg, ...)  do { \
		if (g_dkmd_log_level >= DPU_LOG_LVL_DEBUG) \
			pr_info("D DKMD[%s:%d]"msg, __func__, __LINE__, ## __VA_ARGS__); \
	} while (0)
...
#define dpu_print_timestamp(msg) do { \
		ktime_t timestamp; \
		timestamp = ktime_get();; \
		dpu_pr_debug("%s timestamp_ns=%llu ns", msg,  ktime_to_ns(timestamp)); \
	} while (0)
