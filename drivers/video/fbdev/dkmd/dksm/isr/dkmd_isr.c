...
static struct list_head* dkmd_isr_get_listener_list(struct dkmd_isr *isr_ctrl, uint32_t listen_bit)
{
	...
	if (index < 0 || index >= ISR_LISTENER_LIST_COUNT) {
		dpu_pr_err("invalid index:%d", index);
		return NULL;
	}
...
void dkmd_isr_request(struct dkmd_isr *isr_ctrl)
{
	int32_t ret;
	...
	dpu_pr_info("setup irq %s, isr_ctrl->irq_no: %d", isr_ctrl->irq_name, isr_ctrl->irq_no);
	/* default disable irq */
	disable_irq(isr_ctrl->irq_no);
}
...
int32_t dkmd_isr_unregister_listener(struct dkmd_isr *isr_ctrl, struct notifier_block *nb, uint32_t listen_bit)
{
	...
	if (!isr_ctrl || !nb) {
		dpu_pr_err("isr_ctrl or nb is null ptr");
		return -1;
	}
	...
	list_for_each_entry_safe(listener_node, _node_, listener_list, list_node) {
		...
		if (listener_node->irq_nofitier.head == nb) {
			dpu_pr_debug("listen bit 0x%x, del node\n", listen_bit);
