...
static int ktz8864_parse_dts(struct device_node *np)
{
	...
	if (!ret) {
		if (bl_info.dual_ic == DUAL_IC_I2C) {
			ret = of_property_read_u32(np, "ktz8864_2_display_bias_config_1",
				&bl_info.ktz8864_2_dis_bias_cfg_1);
			if (ret < 0)
				KTZ8864_ERR("get ktz8864 2 dts config dis_bias_config_1 failed\n");

			ret = of_property_read_u32(np, "ktz8864_2_i2c_bus_id", &bl_info.ktz8864_2_i2c_bus_id);
			if (ret < 0)
	...
	unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	if ((pchip == NULL) || (reg == NULL) || (val == NULL) || (pchip->client == NULL)) {
		KTZ8864_ERR("pchip or reg or val or clinet is null pointer\n");
		return -1;
	}

	KTZ8864_INFO("ktz8864_2_config_write\n");
	/* get i2c adapter */
	...
	msg.len = 2; // 2: msg length
	msg.buf = buf;
	for (i = 0; i < size; i++) {
		buf[0] = reg[i];
		buf[1] = val[i];
		if (buf[0] == REG_DISPLAY_BIAS_CONFIG_1)
			buf[1] = bl_info.ktz8864_2_dis_bias_cfg_1;
...
static int ktz8864_chip_init(struct ktz8864_chip_data *pchip)
{
	...
	if (bl_info.dual_ic == DUAL_IC_I2C) {
		ret = ktz8864_2_config_write(pchip, ktz8864_reg_addr,
			ktz8864_reg, KTZ8864_RW_REG_MAX);
		if (ret < 0) {
			KTZ8864_ERR("ktz8864_2_config_write register failed\n");
			goto out;
		}
	}
...
static int ktz8864_set_ic_disable(void)
{
	...
	if (bl_info.dual_ic == DUAL_IC_I2C) {
		bl_addr = REG_BL_ENABLE;
		bl_val = BL_RESET;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &bl_val, 1);
		if (ret < 0)
			KTZ8864_ERR("ktz8864_2_config_write REG_BL_ENABLE fail\n");

		bl_addr = REG_BL_BRIGHTNESS_LSB;
		bl_val = BL_DISABLE;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &bl_val, 1);
		if (ret < 0)
			KTZ8864_ERR("ktz8864_2_config_write REG_BL_BRIGHTNESS_LSB fail\n");

		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &bl_val, 1);
		if (ret < 0)
			KTZ8864_ERR("ktz8864_2_config_write REG_BL_BRIGHTNESS_MSB fail!\n");
	}
	...
	struct device_attribute *devAttr, const char *buf, size_t size)
{
	...
	if (bl_info.dual_ic == DUAL_IC_I2C) {
		ret = ktz8864_2_config_write(pchip, &reg, &val, 1);
		if (ret < 0)
			goto i2c_error;
	}
...
static int ktz8864_2_en_backlight()
{
	int ret;
	int bl_addr = REG_BL_BRIGHTNESS_LSB;
	int bl_val = BRTGHTNESS_LSB_MAX;
...
static int ktz8864_en_backlight(uint32_t bl_value)
{
	int ret = 0;
	...
	KTZ8864_INFO("ktz8864_en_backlight bl_level = %d\n", bl_value);

	/* first set backlight, enable ktz8864 */
	if ((ktz8864_init_status == false) && (bl_value > 0)) {
		if (bl_info.dual_ic == DUAL_IC_I2C) {
			ret = ktz8864_2_en_backlight();
			if (ret < 0)
				KTZ8864_ERR("ktz8864_2_en_backlight fail\n");
		}

		ret = regmap_update_bits(ktz8864_g_chip->regmap,
			REG_BL_BRIGHTNESS_LSB, MASK_BL_LSB, BRTGHTNESS_LSB_MAX);
		if (ret < 0)
			KTZ8864_ERR("regmap_update_bits REG_BL_BRIGHTNESS_LSB fail\n");
		...
		ret = ktz8864_config_write(ktz8864_g_chip, ktz8864_reg_addr,
			ktz8864_reg, KTZ8864_RW_REG_MAX);
		if (ret < 0) {
			KTZ8864_ERR("ktz8864_config_wright fail\n");
			up(&(ktz8864_g_chip->test_sem));
			return ret;
		}
	...
	if ((ktz8864_init_status == true) && (bl_value == 0))
...
static struct lcd_kit_bl_ops bl_ops = {
	...
	.en_backlight = ktz8864_en_backlight,
	.name = "8864",
};
