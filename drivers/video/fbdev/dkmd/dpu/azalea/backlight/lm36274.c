...
#endif
#ifdef CONFIG_LCD_KIT_DRIVER
	result = of_property_read_u32(np, LM36274_VTC_LINE_BOOST,
		&bl_info.lm36274_vtc_line_boost);
	if (result)
		LM36274_ERR("lm36274 parse vtc line boost failed!\n");
	result = of_property_read_u32(np, LM36274_VTC_END_BOOST,
		&bl_info.lm36274_vtc_end_boost);
	if (result)
		LM36274_ERR("lm36274 parse vtc end boost failed!\n");
#endif
	return ret;
}
...
static int lm36274_set_vtc_bias(int vpos, int vneg, bool state);
static bool lm36274_is_need_bias_disable(void);
...
static struct lcd_kit_bias_ops bias_ops = {
	...
	.set_vtc_bias_voltage = lm36274_set_vtc_bias,
	.set_bias_is_need_disable = lm36274_is_need_bias_disable,
};
...
static void lm36274_get_bias_config(int vsp, int vsn, int *out_vsp, int *out_vsn)
{
	int i;
	int vol_size = ARRAY_SIZE(lm36274_voltage_table);

	for (i = 0; i < vol_size; i++) {
		if (lm36274_voltage_table[i].voltage == vsp) {
			*out_vsp = lm36274_voltage_table[i].value;
			break;
		}
	}
	if (i >= vol_size) {
		LM36274_ERR("not found vsn voltage, use default voltage\n");
		*out_vsp = LM36274_VOL_600;
	}

	for (i = 0; i < vol_size; i++) {
		if (lm36274_voltage_table[i].voltage == vsn) {
			*out_vsn = lm36274_voltage_table[i].value;
			break;
		}
	}
	if (i >= vol_size) {
		LM36274_ERR("not found vsn voltage, use default voltage\n");
		*out_vsn = LM36274_VOL_600;
	}
	LM36274_INFO("vpos = 0x%x, vneg = 0x%x\n", *out_vsp, *out_vsn);
}
...
static int lm36274_set_vtc_bias(int vpos, int vneg, bool state)
{
	...
	if (vpos == 0 || vneg == 0) {
		LM36274_ERR("vtc line test vsp/vsn config error!\n");
		return -1;
	}
	if (!lm36274_g_chip)
		return -1;

	lm36274_get_bias_config(vpos, vneg, &vsp, &vsn);
	LM36274_INFO("vtc line test vsp:0x%x, vsn:0x%x\n", vsp, vsn);

	if (bl_info.lm36274_vtc_line_boost == 0 ||
		bl_info.lm36274_vtc_end_boost == 0) {
		LM36274_ERR("vtc line test bias boost or config error!\n");
		return -1;
	}

	for (i = 0; i < LM36274_RW_REG_MAX; i++) {
		if (lm36274_reg_addr[i] == REG_VPOS_BIAS) {
			bl_info.lm36274_reg[i] = vsp;
		} else if (lm36274_reg_addr[i] == REG_VNEG_BIAS) {
			bl_info.lm36274_reg[i] = vsn;
		} else if (lm36274_reg_addr[i] == REG_LCM_BOOST_BIAS &&
			state == true) {
			bl_info.lm36274_reg[i] = bl_info.lm36274_vtc_line_boost;
			LM36274_INFO("vtc line test bias_boost[%d] 0x%x \n",
				i, bl_info.lm36274_reg[i]);
		} else if (lm36274_reg_addr[i] == REG_LCM_BOOST_BIAS &&
			state == false) {
			bl_info.lm36274_reg[i] = bl_info.lm36274_vtc_end_boost;
			LM36274_INFO("vtc line test bias_boost[%d] 0x%x \n",
				i, bl_info.lm36274_reg[i]);
		}
	}
	LM36274_INFO("lm36274_set_vtc_bias is successful\n");
	return 0;
}
...
static int lm36274_set_bias_voltage(int vpos, int vneg)
{
	...
	ret = lm36274_config_write(lm36274_g_chip, lm36274_reg_addr,
		bl_info.lm36274_reg, LM36274_RW_REG_MAX);
	if (ret < 0)
		LM36274_ERR("i2c access fail to register\n");
	return ret;
}
