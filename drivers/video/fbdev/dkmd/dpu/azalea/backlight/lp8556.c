...
static struct gpio_desc lp8556_hw_en_on_cmds[] = {
	{ DTYPE_GPIO_REQUEST, WAIT_TYPE_US, 0, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_hw_en_gpio, 0 },
	{ DTYPE_GPIO_OUTPUT, WAIT_TYPE_MS, 5, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_hw_en_gpio, 1 },
};

static struct gpio_desc lp8556_2_hw_en_on_cmds[] = {
	{ DTYPE_GPIO_REQUEST, WAIT_TYPE_US, 0, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_2_hw_en_gpio, 0 },
	{ DTYPE_GPIO_OUTPUT, WAIT_TYPE_MS, 5, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_2_hw_en_gpio, 1 },
};

#ifdef CONFIG_LCD_KIT_DRIVER
static struct gpio_desc lp8556_hw_en_disable_cmds[] = {
	{ DTYPE_GPIO_OUTPUT, WAIT_TYPE_US, 0, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_hw_en_gpio, 0 },
	{ DTYPE_GPIO_INPUT, WAIT_TYPE_US, 10, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_hw_en_gpio, 0 },
};

static struct gpio_desc lp8556_2_hw_en_disable_cmds[] = {
	{ DTYPE_GPIO_OUTPUT, WAIT_TYPE_US, 0, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_2_hw_en_gpio, 0 },
	{ DTYPE_GPIO_INPUT, WAIT_TYPE_US, 10, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_2_hw_en_gpio, 0 },
...
static struct gpio_desc lp8556_hw_en_free_cmds[] = {
	{ DTYPE_GPIO_FREE, WAIT_TYPE_US, 50, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_hw_en_gpio, 0 },
};

static struct gpio_desc lp8556_2_hw_en_free_cmds[] = {
	{ DTYPE_GPIO_FREE, WAIT_TYPE_US, 50, GPIO_LP8556_EN_NAME,
	&lp8556_bl_info.lp8556_2_hw_en_gpio, 0 },
};
#endif
...
static char *lp8556_dts_string[LP8556_RW_REG_MAX] = {
	"lp8556_device_control", "lp8556_eprom_cfg0", "lp8556_eprom_cfg1",
	"lp8556_eprom_cfg2",     "lp8556_eprom_cfg3", "lp8556_eprom_cfg4",
	"lp8556_eprom_cfg5",     "lp8556_eprom_cfg6", "lp8556_eprom_cfg7",
	"lp8556_eprom_cfg9",     "lp8556_eprom_cfgA", "lp8556_eprom_cfgE",
	"lp8556_eprom_cfg9E",    "lp8556_led_enable", "lp8556_eprom_cfg98",
};
 ...
 */
unsigned int lp8556_msg_level = 7;
...
static int lp8556_parse_dts(struct device_node *np)
{
	...
	if (lp8556_bl_info.dual_ic == DUAL_LP8556_I3C) {
		ret = of_property_read_u32(np, "lp8556_i2c_bus_id",
			&lp8556_bl_info.lp8556_i2c_bus_id);
		if (ret < 0)
			LP8556_INFO("can not get lp8556_i2c_bus_id dts node\n");
	} else if (lp8556_bl_info.dual_ic == DUAL_LP8556_I2C) {
	...
	if (lp8556_bl_info.dual_ic == DUAL_LP8556_I2C) {
		...
		unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	/* get i2c adapter */
	if (lp8556_bl_info.dual_ic == DUAL_LP8556_I3C)
		i2c_bus_id = lp8556_bl_info.lp8556_i2c_bus_id;
	else if (lp8556_bl_info.dual_ic == DUAL_LP8556_I2C)
		i2c_bus_id = lp8556_bl_info.lp8556_2_i2c_bus_id;
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			LP8556_INFO("%s reg=0x%x,val=0x%x\n",
		...
		unsigned int reg[], unsigned int val[], unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			LP8556_ERR("read lp8556 backlight config register 0x%x failed",
				reg[i]);
			goto exit;
		} else {
...
static int lp8556_chip_init(struct lp8556_chip_data *pchip)
{
	...
	if (lp8556_bl_info.dual_ic) {
		ret = lp8556_2_config_write(pchip, lp8556_reg_addr,
			lp8556_bl_info.lp8556_reg, LP8556_RW_REG_MAX);
		if (ret < 0) {
			LP8556_ERR("lp8556 slave config register failed\n");
			goto out;
		}
	}
	ret = lp8556_config_write(pchip, lp8556_reg_addr,
		lp8556_bl_info.lp8556_reg, LP8556_RW_REG_MAX);
	if (ret < 0) {
...
ssize_t lp8556_set_backlight_init(uint32_t bl_level)
{
	...
	if (false == lp8556_init_status && bl_level > 0) {
		...
		gpio_cmds_tx(lp8556_hw_en_on_cmds,
			ARRAY_SIZE(lp8556_hw_en_on_cmds));
		if (lp8556_bl_info.dual_ic == DUAL_LP8556_I2C)
			gpio_cmds_tx(lp8556_2_hw_en_on_cmds,
				ARRAY_SIZE(lp8556_2_hw_en_on_cmds));
		/* chip initialize */
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = lp8556_config_read(pchip, lp8556_reg_addr,
		lp8556_bl_info.lp8556_reg, LP8556_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE,
		...
		lp8556_bl_info.lp8556_reg[0], lp8556_bl_info.lp8556_reg[1],
		lp8556_bl_info.lp8556_reg[2], lp8556_bl_info.lp8556_reg[3],
		lp8556_bl_info.lp8556_reg[4], lp8556_bl_info.lp8556_reg[5],
		lp8556_bl_info.lp8556_reg[6], lp8556_bl_info.lp8556_reg[7],
		lp8556_bl_info.lp8556_reg[8], lp8556_bl_info.lp8556_reg[9],
		lp8556_bl_info.lp8556_reg[10], lp8556_bl_info.lp8556_reg[11],
		lp8556_bl_info.lp8556_reg[12], lp8556_bl_info.lp8556_reg[13]);
	return ret;
...
static DEVICE_ATTR(reg, 0644, lp8556_reg_show, lp8556_reg_store);
...
{
	gpio_cmds_tx(lp8556_hw_en_disable_cmds,
		ARRAY_SIZE(lp8556_hw_en_disable_cmds));
	gpio_cmds_tx(lp8556_hw_en_free_cmds,
		ARRAY_SIZE(lp8556_hw_en_free_cmds));
	if (lp8556_bl_info.dual_ic == DUAL_LP8556_I2C) {
		gpio_cmds_tx(lp8556_2_hw_en_disable_cmds,
			ARRAY_SIZE(lp8556_2_hw_en_disable_cmds));
		gpio_cmds_tx(lp8556_2_hw_en_free_cmds,
			ARRAY_SIZE(lp8556_2_hw_en_free_cmds));
	}
...
static int lp8556_set_backlight(uint32_t bl_level)
{
	...
	ret = regmap_write(lp8556_g_chip->regmap,
		lp8556_bl_info.lp8556_level_lsb, bl_lsb);
	if (ret < 0)
		LP8556_DEBUG("write lp8556 backlight level lsb:0x%x failed\n",
			bl_lsb);
	ret = regmap_write(lp8556_g_chip->regmap,
		lp8556_bl_info.lp8556_level_msb, bl_msb);
	if (ret < 0)
	...
	if (last_bl_level == 0 && bl_level > 0) {
		if (common_info->check_thread.check_bl_support)
			...
			schedule_delayed_work(&common_info->check_thread.check_work,
				(HZ / 5));
		LP8556_INFO("level = %d, bl_msb = %d, bl_lsb = %d",
...
static int lp8556_backlight_save_restore(int save_enable)
{
	...
	if (save_enable) {
		ret = regmap_read(lp8556_g_chip->regmap,
			lp8556_bl_info.lp8556_level_lsb, &val);
		if (ret < 0) {
		...
		bl_lsb = val & 0xFF;
		ret = regmap_read(lp8556_g_chip->regmap,
			lp8556_bl_info.lp8556_level_msb, &val);
		if (ret < 0) {
			LP8556_ERR("write lp8556 backlight level msb:0x%x failed\n",
				bl_msb);
			return -1;
...
static unsigned int lp8556_test_led_open(struct lp8556_chip_data *pchip, int led_num)
{
	int ret;
	unsigned int i;
	...
	/* Open LEDx string. */
	ret = regmap_write(pchip->regmap, LP8556_LED_ENABLE,
...
static unsigned int lp8556_test_led_short(struct lp8556_chip_data *pchip, int led_num)
{
	unsigned int val = 0;
	int ret;
	unsigned int result = TEST_OK;
...
static void lp8556_dsm_notify(int val)
{
	...
	if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
		dsm_client_record(lcd_dclient,
			"lp8556 happen short, reg:0x02 value is:0x%x\n", val);
			...
			const struct i2c_device_id *id)
{
	...
	pchip->dev = device_create(lp8556_class,
	...
	ret = regmap_read(pchip->regmap, lp8556_reg_addr[0],
		&lp8556_bl_info.lp8556_reg[0]);
	if (ret < 0) {
	...
	if (lp8556_bl_info.dual_ic) {
		ret = lp8556_2_config_write(pchip, lp8556_reg_addr,
			lp8556_bl_info.lp8556_reg, 1);
		if (ret < 0) {
			LP8556_ERR("lp8556 slave not used\n");
			goto err_sysfs;
		}
	}
	ret = of_property_read_u32(np, "lp8556_level_lsb",
		&lp8556_bl_info.lp8556_level_lsb);
	if (ret < 0) {
		LP8556_ERR("get lp8556_level_lsb failed\n");
		goto err_sysfs;
	}

	ret = of_property_read_u32(np, "lp8556_level_msb",
		&lp8556_bl_info.lp8556_level_msb);
	if (ret < 0) {
...
fs_initcall(lp8556_module_init);
