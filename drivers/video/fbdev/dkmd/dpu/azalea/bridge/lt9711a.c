...
int is_lt9711a_onboard_klvv(struct mipi2edp *pdata)
{
	int i, ret, reg, chipid;

	HISI_FB_INFO("check whether lt9711a chip is onboard\n");
	ret = 0;
	reg = 0;
	chipid = 0;
	for (i = 0; i < MAX_CHIP_PROBE_TIMES; i++) {
		...
		HISI_FB_INFO("lt9711a is onboard,chipid is 0x%x\n", chipid);
		return ret;
	}

	HISI_FB_ERR("lt9711a isn't onboard!\n");
	return ret;
}
...
static int lt9711a_i2c_send_byte(char devaddr, const char regoffset, const char value)
{
	...
	if (!adap) {
		HISI_FB_ERR("lt9711a_i2c_send_byte i2c_get_adapter err");
		return -ENODEV;
	}
	...
	HISI_FB_DEBUG("lt9711a_i2c_send_byte regoffset = %d value=%d ret = %d \n", regoffset, value, ret);
...
static int lt9711a_i2c_recv_byte(char devaddr, const char regoffset, char *value)
{
	...
	adap = i2c_get_adapter(I2C_CONTROLLER);
	if (!adap) {
		HISI_FB_ERR("lt9711a_i2c_recv_byte i2c_get_adapter err");
		return -ENODEV;
	}
...
void lt9711_pre_enable_video_stream(struct mipi2edp *pdata)
{
	HISI_FB_INFO("enter lt9711_pre_enable_video_stream\n");

	/* wait for LCD inital finish! */
	mdelay(50);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xff, 0x80);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xee, 0x01);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xff, 0xa9);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0x1e, 0x82);
	HISI_FB_INFO("disable lt9711 video steam\n");
}

int lt9711_enable_video_stream(struct mipi2edp *pdata)
{
	HISI_FB_INFO("enter lt9711_enable_video_stream\n");
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xff, 0x80);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xee, 0x01);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xff, 0xa9);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0x1e, 0xc2);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xff, 0x80);
	lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xee, 0x00);
	HISI_FB_INFO("enable lt9711 video steam\n");
	return 0;
}
...
int lt9711a_get_edid(char *edid, unsigned int len)
{
	...
	if (!edid) {
		HISI_FB_ERR("edid input err");
		return -1;
	}

	if (len < 128) {
		HISI_FB_ERR("len input err");
		return -1;
	}

	res = lt9711a_i2c_recv_byte(I2C_SLAVE_ADDR, 0xd6, ret);
	if(res || ((ret[0] != 238) && (ret[0] != 17))) {
		HISI_FB_ERR("lt9711a_i2c_transfer err");
		return -1;
	}
	...
	if (top != len) {
		HISI_FB_ERR("edid len err");
		return -1;
	}
	res = lt9711a_i2c_send_byte(I2C_SLAVE_ADDR, 0xe0, 0x40);
	if (res) {
		HISI_FB_ERR("lt9711a_i2c_transfer warn");
	}
...
int check_edid(char *edid, unsigned int len)
{
	int i;
	if (!edid) {
		HISI_FB_ERR("Edid The pointer is NULL.\n");
		return -1;
	}
	...
	for (i = 0; i < EDID_HEADER_LEN; i++) {
		if (edid[i] != edid_header[i]) {
			HISI_FB_ERR("Don't match EDID header\n");
			return -1;
		}
	}
	return 0;
