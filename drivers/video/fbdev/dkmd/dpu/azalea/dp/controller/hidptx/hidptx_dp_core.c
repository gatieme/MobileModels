...
static int dp_dis_reset(struct dpu_fb_data_type *dpufd, bool benable)
{
	...
	if (g_fpga_flag == 1) {
		if (benable)
			outp32(dpufd->mmc0_crg + 0x24, 0x10008);
		else
			outp32(dpufd->mmc0_crg + 0x20, 0x10008);
		return 0;
	}
...
int dptx_core_on(struct dp_ctrl *dptx)
{
	...
	if (!dptx_check_dptx_id(dptx)) {
		DPU_FB_ERR("[DP] DPTX_ID not match !\n");
		return -EINVAL;
	}
...
int dptx_phy_wait_busy(struct dp_ctrl *dptx, uint32_t lanes)
{
	...
	DPU_FB_ERR("[DP] PHY BUSY timed out\n");
...
void dptx_phy_set_lanes_power(struct dp_ctrl *dptx, bool bopen)
{
	...
	if (bopen) { /* set lanes to P0 mode */
		switch (lanes) {
		case 4:
			phyifctrl &= ~(DPTX_LANE2_PHY_STATUS_MASK | DPTX_LANE3_PHY_STATUS_MASK); /* go throuth */
		case 2:
			phyifctrl &= ~DPTX_LANE1_PHY_STATUS_MASK; /* go throuth */
		case 1:
