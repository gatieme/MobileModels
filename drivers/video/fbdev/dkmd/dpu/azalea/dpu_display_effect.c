...
#include "product/dpu_displayengine_utils.h"
#include "product/dpu_displayengine_effect.h"
...
uint8_t *g_share_mem_virt;
phys_addr_t g_share_mem_phy;
...
void dpu_effect_hiace_trigger_wq(struct dpu_fb_data_type *dpufd)
{
	...
	if (!dpu_hiace_single_mode_handle_isr(dpufd))
		dpu_display_effect_hiace_trigger_wq(dpufd, dpufd->video_idle_ctrl.idle_frame_display);
#else
	dpu_display_effect_hiace_trigger_wq(dpufd, dpufd->is_idle_display);
#endif
}
#endif
#endif
...
static int dpufb_effect_info_get_handler(void __user *argp)
{
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_ACM) {
		ret = dpu_effect_acm_info_get(dpufd_primary, &effect_info.acm);
		if (ret) {
...
static int dpufb_effect_info_set_handler(const void __user *argp)
{
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_DPPROI) {
		dpu_effect_dpproi_verify(dpufd_primary, &effect_info);
		ret = dpu_effect_dpproi_info_set(dpufd_primary,
	...
	if ((effect_info.modules & DSS_EFFECT_MODULE_LCP_GMP) ||
		...
		if (dpufd_primary->effect_info[effect_info.disp_panel_id].dpp_cmdlist_type != DPP_CMDLIST_TYPE_ROI) {
			...
			ret = dpu_effect_dpp_info_set(dpufd_primary, &effect_info);
		}
#else
		ret = dpu_effect_dpp_buf_info_set(dpufd_primary, &effect_info, false);
#endif
...
#else
	if (effect_info.modules & DSS_EFFECT_MODULE_LCP_GMP) {
		ret = dpu_effect_gmp_info_set(dpufd_primary, &effect_info.lcp);
		if (ret) {
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_LCP_IGM) {
		ret = dpu_effect_igm_info_set(dpufd_primary, &effect_info.lcp);
		if (ret) {
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_LCP_XCC) {
		ret = dpu_effect_xcc_info_set(dpufd_primary, &effect_info.lcp);
		if (ret) {
	...
	if (effect_info.modules & DSS_EFFECT_MODULE_GAMMA) {
		ret = dpu_effect_gamma_info_set(dpufd_primary, &effect_info.gamma);
		if (ret) {
...
#if defined(CONFIG_DPU_FB_V346) || defined(CONFIG_DPU_FB_V510) || \
...
void dpu_effect_set_reg(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE &&
		dpufd->index == BUILT_IN_PANEL_INDEX) {
		DPU_FB_INFO("[effect] builtin use fb0!\n");
	...
	!defined(CONFIG_DPU_FB_V501) && !defined(CONFIG_DPU_FB_V410)
		...
		dpu_effect_roi_region_config(dpufd);
#endif
...
static inline struct dpu_fb_data_type *display_engine_check_input_and_get_dpufd(
	struct fb_info *info, void __user *argp)
{
	struct dpu_fb_data_type *dpufd = NULL;
	...
	uint16_t *buf_map, uint32_t buf_size)
{
	...
	uint32_t num_of_elements = buf_size / (uint32_t)len;
...
static int display_engine_blc_param_set(struct dpu_fb_data_type *dpufd,
	display_engine_blc_param_t *param)
{
	struct dpu_fb_panel_data *pdata = NULL;
	...
	if (abs((int)(ktime_to_ns(dpufd->backlight.bl_timestamp) -
		ktime_to_ns(current_bl_timestamp))) > g_bl_timeinterval) {
		...
		DPU_FB_DEBUG("[effect] delta:%d bl:%d enable:%d skip\n", dpufd->de_info.blc_delta,
			dpufd->bl_level, dpufd->de_info.blc_enable);
...
static int display_engine_ddic_color_param_set(struct dpu_fb_data_type *dpufd,
	display_engine_ddic_color_param_t *param)
{
	int ret = 0;
	struct dpu_fb_panel_data *pdata = NULL;
...
static int display_engine_ddic_cabc_param_set(struct dpu_fb_data_type *dpufd,
	display_engine_ddic_cabc_param_t *param)
{
	int ret = 0;
	struct dpu_fb_panel_data *pdata = NULL;
...
static int display_engine_ddic_rgbw_param_set(struct dpu_fb_data_type *dpufd,
	display_engine_ddic_rgbw_param_t *param)
{
	int ret = 0;
	struct dpu_fb_panel_data *pdata = NULL;
	...
	if ((param->ddic_panel_id != LG_NT36772A_RGBW_ID) &&
		(param->ddic_panel_id != LG_NT36772A_RGBW_ID_HMA) &&
		(param->ddic_panel_id != BOE_HX83112E_RGBW_ID_HMA) &&
		(param->ddic_panel_id != JDI_TD4336_RT8555_RGBW_ID) &&
		(param->ddic_panel_id != SHARP_TD4336_RT8555_RGBW_ID) &&
		(param->ddic_panel_id != LG_NT36772A_RT8555_RGBW_ID)) {
...
static int display_engine_amoled_algo_param_set(struct dpu_fb_data_type *dpufd,
	display_engine_amoled_param_t *param)
{
	int ret = 0;
	...
	up(&dpufd->blank_sem);
	DPU_FB_DEBUG("[effect] first screen on ! HBM_Max_BackLight:%d Hiac_DBVThres: %d \
		HBMEnable: %d amoled_diming_enable : %d\n",
		param->HBM_Max_BackLight, param->Hiac_DBVThres,
		param->HBMEnable, param->amoled_diming_enable);
	DPU_FB_DEBUG("[effect] first screen on ! Lowac_DBVThres:%d Lowac_DBV_XCCThres: %d \
		Lowac_DBV_XCC_MinThres: %d Lowac_Fixed_DBVThres : %d\n",
		param->Lowac_DBVThres, param->Lowac_DBV_XCCThres, param->Lowac_DBV_XCC_MinThres,
		param->Lowac_Fixed_DBVThres);
	DPU_FB_DEBUG("[effect] dc_backlight_delay_us = %d",
...
static void dpufb_dbv_curve_mapped_init(struct dpu_fb_data_type *dpufd)
{
	if (NULL == dpufd) {
		DPU_FB_ERR("[effect] dpufd is NULL\n");
		return;
...
int dpufb_display_engine_init(struct fb_info *info, void __user *argp)
{
	...
	dpufd->de_info.cb_func.get_dither_ctl0 = display_engine_get_dither_ctl0_value;
...
static int dpufb_param_check(struct fb_info *info, void __user *argp)
{
	struct dpu_fb_data_type *dpufd = display_engine_check_input_and_get_dpufd(info, argp);
	if (NULL == dpufd) {
		return -EINVAL;
...
static int dpufb_runmode_check(display_engine_param_t *de_param)
{
	if(de_param == NULL){
	...
	if ((de_param->modules & DISPLAY_ENGINE_DDIC_RGBW) ||
		...
		return 0;
	} else if (dpu_runmode_is_factory()) {
		return -EINVAL;
	}
...
#define DISPLAY_ENGINE_DEBUG(product_type, msg) \
	do { \
		if (dpu_check_panel_product_type(product_type)) \
			DPU_FB_INFO(msg); \
	} while (0)
#else
#define DISPLAY_ENGINE_DEBUG(product_type, msg)
...
static int display_engine_effect_resolve_bl_buf(const char *buf,
...
static void display_engine_effect_handle_xcc(struct dpu_fb_data_type *dpufd,
	struct effect_bl_buf *resolved_buf, const bool dc_enable_changed)
{
	resolved_buf->ddic_alpha = ALPHA_DEFAULT;
	if ((display_engine_alpha_get_support() == DC_ALPHA_SUPPORT) && resolved_buf->dc_enable)
		display_engine_separate_alpha_from_xcc(resolved_buf);
...
#if defined(CONFIG_DPU_FB_V510) || defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
		int panel_id = dpufd->panel_info.disp_panel_id;

		/* Print R,G,B value of xcc with index 1,6,11 */
		DPU_FB_DEBUG("[effect] enable=%d xcc[1]=%d [6]=%d [11]=%d\n",
			resolved_buf->xcc_param.xcc_enable, resolved_buf->xcc_param.xcc_table[1],
			resolved_buf->xcc_param.xcc_table[6],
			resolved_buf->xcc_param.xcc_table[11]);
		if (dpu_effect_xcc_info_set_kernel_multiple(dpufd,
			&(resolved_buf->xcc_param), panel_id) != 0) {
			dpufd->de_info.amoled_param.dc_brightness_dimming_enable = 0;
			dpufd->de_info.amoled_param.amoled_enable_from_hal = 0;
			DPU_FB_ERR("[effect] dpu_effect_xcc_info_set error\n");
		}
		panel_id = (panel_id == DISPLAY_INNEL_PANEL_ID) ?
			DISPLAY_OUTER_PANEL_ID : DISPLAY_INNEL_PANEL_ID;
		if (dpu_effect_xcc_info_set_kernel_multiple(dpufd,
			&(resolved_buf->xcc_param), panel_id) != 0)
			DPU_FB_ERR("[effect] dpu_effect_xcc_info_set_kernel(panel_id=%d) error",
				panel_id);
#elif defined(CONFIG_DPU_FB_V346)
		if (dpu_effect_xcc_info_set_kernel(dpufd, &(resolved_buf->xcc_param)) != 0) {
			dpufd->de_info.amoled_param.dc_brightness_dimming_enable = 0;
			dpufd->de_info.amoled_param.amoled_enable_from_hal = 0;
			DPU_FB_ERR("[effect] dpu_effect_xcc_info_set error\n");
		}
	...
	DISPLAY_ENGINE_DEBUG(dpufd->panel_info.product_type, "[DE] xcc_info_set_kernel -\n");

	(void)memcpy(&last_xcc_param, &(resolved_buf->xcc_param), sizeof(last_xcc_param));
}

static void display_engine_effect_handle_dc_sync(
	struct dpu_fb_data_type *dpufd,
	struct effect_bl_buf *resolved_buf, const bool dc_enable_changed)
{
	...
	pinfo = display_engine_get_panel_info(dpufd, resolved_buf->panel_id);
	if (pinfo == NULL) {
	...
	ssize_t ret, bool dc_enable_changed, bool delta_changed)
{
	...
	if ((display_engine_force_delta_bl_update_get_support() == 1) ||
		(abs((int)(ktime_to_ns(dpufd->backlight.bl_timestamp) -
	...
	struct effect_bl_buf *resolved_buf, ssize_t ret)
{
	...
	if (ret_count == blc_xcc_buf_count) {
		dc_enable_changed = (((int32_t)(resolved_buf->dc_enable) !=
			dpufd->de_info.amoled_param.
			dc_brightness_dimming_enable_real) ||
			((int32_t)(resolved_buf->dimming_enable) !=
			dpufd->de_info.amoled_param.
			amoled_diming_enable));
		display_engine_effect_handle_xcc(dpufd,
			resolved_buf, dc_enable_changed);
		display_engine_effect_handle_dc_sync(dpufd,
			resolved_buf, dc_enable_changed);
	} else {
		dpufd->de_info.blc_enable = (bool)resolved_buf->blc_enable;
		dpufd->de_info.blc_delta = resolved_buf->delta;
	...
	const char *buf, size_t count)
{
	...
	ret_count = display_engine_effect_resolve_bl_buf(buf, &resolved_buf);
...
int dpufb_get_hiace_enable(struct fb_info *info, void __user *argp)
{
	...
	if ((dpufd->panel_info.disp_panel_id < 0) || (dpufd->panel_info.disp_panel_id >= DISP_PANEL_NUM)) {
		DPU_FB_ERR("[effect] disp panel id (%d) is invalid\n", dpufd->panel_info.disp_panel_id);
		return -EINVAL;
	}
...
static int share_mmap_map(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (g_share_mem_virt == NULL || g_share_mem_phy == 0) {
	...
	if (remap_pfn_range(vma, start, __phys_to_pfn(g_share_mem_phy), size, vma->vm_page_prot)) {
...
static int dpu_share_mem_probe(struct platform_device *pdev)
{
	...
	g_share_mem_virt = (uint8_t *)dma_alloc_coherent(&pdev->dev, SHARE_MEMORY_SIZE,
		&g_share_mem_phy, GFP_KERNEL);
	if (g_share_mem_virt == NULL || g_share_mem_phy == 0) {
