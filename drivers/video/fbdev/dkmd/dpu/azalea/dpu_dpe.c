...
#include "product/attrs/dpu_dpe_attrs.h"

#ifdef CONFIG_HISI_OCBC
#include <linux/platform_drivers/ocbc.h>
...
static int dpe_irq_request(struct dpu_fb_data_type *dpufd);
static int dpe_irq_free(struct dpu_fb_data_type *dpufd);
#endif
...
static int dpe_init(struct dpu_fb_data_type *dpufd, bool fastboot_enable)
{
	...
	if (IS_BUILTIN_PANEL) {
		...
		spr_init(dpufd, fastboot_enable);
		init_ifbc(dpufd);
		init_dbuf(dpufd);
		...
		if (dpufd->dss_pxl1_clk)
			clk_disable(dpufd->dss_pxl1_clk);

		if (!is_dp_panel(dpufd))
			set_reg(dpufd->dss_base + DSS_LDI0_OFFSET + LDI_DSI1_CLK_SEL, 0x1, 1, 0);

		if (dpufd->dss_pxl1_clk)
			(void)clk_enable(dpufd->dss_pxl1_clk);

		if (!is_dp_panel(dpufd))
			set_reg(dpufd->dss_base + DSS_LDI0_OFFSET + LDI_DSI1_RST_SEL, 0x1, 1, 0);
#endif
...
static void dpe_check_itf_status(struct dpu_fb_data_type *dpufd)
{
	...
	if (IS_BUILTIN_PANEL || (dpufd->index == EXTERNAL_PANEL_IDX)) {
		if (dpufd->index == BUILT_IN_PANEL_INDEX) {
			itf_idx = 0;
		} else {
			itf_idx = dpufd->index;
		}
...
int dpe_regulator_enable(struct dpu_fb_data_type *dpufd)
{
	...
	dss_regulator_refcount = dss_regulator_ref_table[PRIMARY_PANEL_IDX] +
			...
			dss_regulator_ref_table[BUILT_IN_PANEL_INDEX];
	if (dpufd->index != AUXILIARY_PANEL_IDX)
		DPU_FB_INFO(
			"fb%d, dss_regulator_refcount=%d!, dss_regulator_ref_table: fb0: %d, fb1: %d, fb2: %d, fb3: %d, fb4:%d\n",
	...
	defined(CONFIG_DPU_FB_V330) || defined(CONFIG_DPU_FB_V501)
		...
		DPU_FB_INFO("dss_regulator_ref_table[PRIMARY_PANEL_IDX] == 0");
	}
...
int dpe_regulator_disable(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->index != AUXILIARY_PANEL_IDX)
		DPU_FB_INFO(
			...
			dss_regulator_ref_table[MEDIACOMMON_PANEL_IDX], dss_regulator_ref_table[BUILT_IN_PANEL_INDEX]);
	...
	if (IS_BUILTIN_PANEL) {
		...
		DPU_FB_INFO("fb%d，dss_regulator_ref_table[PRIMARY_PANEL_IDX] == 0!\n", dpufd->index);
	}
...
int mediacrg_regulator_enable(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("fb%d start mediacrg regulator_enable!\n", dpufd->index);
	down(&dpu_fb_dss_regulator_sem);
	...
	if (ret) {
		...
		DPU_FB_INFO("fb%d mediacrg regulator_enable success!\n", dpufd->index);
	}
...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
	if (exit_idle)
		outp32(dpufd->pmctrl_base + NOC_POWER_IDLEREQ, 0x04000000);
	else
		outp32(dpufd->pmctrl_base + NOC_POWER_IDLEREQ, 0x04000400);
#else
	if (exit_idle)
...
{
	int ret = 0;
	static int refcount_dbg[5] = {0};  /* fb node[0~4] */
	int refcount = 0;

	down(&dpu_fb_dss_bus_idle_ctrl_sem);
	if (exit_idle) {
		dss_bus_idle_ctrl_refcount++;
		refcount = dss_regulator_ref_table[PRIMARY_PANEL_IDX] +
			dss_regulator_ref_table[EXTERNAL_PANEL_IDX] +
			dss_regulator_ref_table[AUXILIARY_PANEL_IDX] +
			dss_regulator_ref_table[MEDIACOMMON_PANEL_IDX] +
			dss_regulator_ref_table[BUILT_IN_PANEL_INDEX];
	...
	DPU_FB_DEBUG("fb%d exit_idle[%d], total refcount[%d], fb0[%d] fb1[%d] fb2[%d] fb3][%d] fb4][%d].\n",
		dpufd->index, exit_idle, dss_bus_idle_ctrl_refcount,
		refcount_dbg[0], refcount_dbg[1], refcount_dbg[2], refcount_dbg[3], refcount_dbg[4]);
...
static int dpe_pxl_clk_enable(struct dpu_fb_data_type *dpufd)
{
	...
	if (IS_BUILTIN_PANEL) {
		if (dpufd->dss_pxl0_clk) {
			ret = clk_prepare_enable(dpufd->dss_pxl0_clk);
			if (ret) {
				DPU_FB_ERR("1.dss_pxl0_clk enable failed, error[%d]!\n", ret);
				ret = clk_set_rate(dpufd->dss_pxl0_clk, dpufd->panel_info.pxl_clk_rate);
				if (ret) {
					DPU_FB_ERR("reset clk_set_rate[%llu] failed, error[%d]!\n",
						dpufd->panel_info.pxl_clk_rate, ret);
					return -EINVAL;
				}
				ret = clk_prepare_enable(dpufd->dss_pxl0_clk);
				if (ret) {
					DPU_FB_ERR("2.dss_pxl0_clk enable failed, error[%d]!\n", ret);
					return -EINVAL;
				}
			}
		}
	} else if (dpufd->index == EXTERNAL_PANEL_IDX) {
		if (dpufd_list[PRIMARY_PANEL_IDX] && dpufd_list[PRIMARY_PANEL_IDX]->dss_pxl0_clk) {
			ret = clk_prepare_enable(dpufd_list[PRIMARY_PANEL_IDX]->dss_pxl0_clk);
			if (ret) {
				DPU_FB_ERR("fb%d dss_pxl0_clk enable failed, error=%d!\n",
					dpufd->index, ret);
				return -EINVAL;
			}
		}
...
#elif defined(SUPPORT_DSI_VER_2_0)
static int dpe_pxl_clk_enable(struct dpu_fb_data_type *dpufd)
{
	int ret = 0;
...
#if defined(CONFIG_DPU_FB_V501)
static void dpe_pxl_clk_disable(struct dpu_fb_data_type *dpufd)
{
	if (dpufd == NULL) {
		DPU_FB_ERR("dpufd is NULL point!\n");
		return;
	...
	if (IS_BUILTIN_PANEL) {
		if (dpufd->dss_pxl0_clk)
			clk_disable_unprepare(dpufd->dss_pxl0_clk);
	} else if (dpufd->index == EXTERNAL_PANEL_IDX) {
		if (dpufd->dss_pxl1_clk)
			clk_disable_unprepare(dpufd->dss_pxl1_clk);
		if (dpufd_list[PRIMARY_PANEL_IDX] && dpufd_list[PRIMARY_PANEL_IDX]->dss_pxl0_clk)
			clk_disable_unprepare(dpufd_list[PRIMARY_PANEL_IDX]->dss_pxl0_clk);
	}
}
#elif defined(SUPPORT_DSI_VER_2_0)
static void dpe_pxl_clk_disable(struct dpu_fb_data_type *dpufd)
...
#if defined(CONFIG_DPU_FB_V501)
	if (IS_BUILTIN_PANEL)
		dpufd->pipe_clk_ctrl.underflow_int = underflow ? 1 : 0;  /* underflow clean up flag */
#endif
}
...
static int dpe_set_fastboot(struct platform_device *pdev)
{
	...
	DPU_FB_DEBUG("fb%d, +.\n", dpufd->index);
#ifdef SUPPORT_DSI_VER_2_0
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE && (dpufd->index == PRIMARY_PANEL_IDX)) {
		ret = dpe_irq_request(dpufd);
		dpu_check_and_return((ret != 0), -EINVAL, ERR, "fb%d irq request fail, error=%d!\n",
			dpufd->index, ret);
	}
#endif
	...
	if (dpufd->panel_info.vsync_ctrl_type == VSYNC_CTRL_NONE) {
		...
		dpu_dpp_cmdlist_interrupt_on(dpufd);
#else
...
static int dpe_clk_get(struct platform_device *pdev)
{
	...
	if (IS_BUILTIN_PANEL) {
		...
		if (is_dp_panel(dpufd))
			...
			dpufd->dss_pxl1_clk = devm_clk_get(&pdev->dev, dpufd->dss_pxl0_clk_name);
#elif defined(CONFIG_DPU_FB_V510) || defined(CONFIG_DPU_FB_V600) || \
		defined(CONFIG_DPU_FB_V360)
		if (is_dp_panel(dpufd))
			dpufd->dss_pxl1_clk = devm_clk_get(&pdev->dev, dpufd->dss_pxl1_clk_name);
		else
...
{
	(void)mediacrg_regulator_enable(dpufd);
	(void)dpe_set_clk_rate(pdev);
	(void)dpe_common_clk_enable(dpufd);
	(void)dpe_inner_clk_enable(dpufd);
	(void)dpe_regulator_enable(dpufd);
...
static int dpe_on(struct platform_device *pdev)
{
	...
	DPU_FB_DEBUG("fb%d, +.\n", dpufd->index);
#ifdef SUPPORT_DSI_VER_2_0
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE && IS_BUILTIN_PANEL) {
		ret = dpe_irq_request(dpufd);
		dpu_check_and_return((ret != 0), -EINVAL, ERR, "fb%d irq request fail, error=%d!\n",
			dpufd->index, ret);
	}
...
#ifdef CONFIG_DPU_DPP_CMDLIST
		dpu_dpp_cmdlist_interrupt_off(dpufd);
#else
		dpe_interrupt_mask(dpufd);
...
#ifdef CONFIG_DPU_FB_V501
	if (dpufd->pipe_clk_ctrl.pipe_clk_rate > pinfo->pxl_clk_rate) {
		if (dpufb_get_ldi_hporch_updt_para(dpufd, true) == 0) {
			hbp_updt = dpufd->pipe_clk_ctrl.fps_updt_hporch[0];
			hfp_updt = dpufd->pipe_clk_ctrl.fps_updt_hporch[1];
			hpw_updt = dpufd->pipe_clk_ctrl.fps_updt_hporch[2];
		}
		DPU_FB_INFO("fps updt, ldi porch updated with dp panel\n");
	}
#endif
...
static int dpe_isr_fnc_get(struct dpu_fb_data_type *dpufd, struct dpe_irq *dpe_irq_info)
{
	if (IS_BUILTIN_PANEL) {
		...
		if ((need_config_dsi1(dpufd) || 
			...
			dpufd->panel_info.product_type != PANEL_DUAL_PANEL_TYPE)
...
static int dpe_irq_free(struct dpu_fb_data_type *dpufd)
{
	...
	if (!IS_BUILTIN_PANEL) {
		DPU_FB_WARNING("%d not support free irq\n", dpufd->index);
		return ret;
	}

	ret = dpe_isr_fnc_get(dpufd, &dpe_irq_info);
	if (ret != 0) {
		DPU_FB_ERR("fb%d, dpe_isr_fnc_get error\n", dpufd->index);
		return ret;
	}

	if (dpe_irq_info.irq_name) {
		DPU_FB_INFO("fb%d, free irq[%s]\n", dpufd->index, dpe_irq_info.irq_name);
		free_irq(dpufd->dpe_irq, (void *)dpufd);
	}

	if (dpe_irq_info.dsi_irq_name) {
		dsi_irq = dpufd->dsi0_irq;
		if (dsi_irq == 0) {
			DPU_FB_ERR("dsi_irq is 0 fail---\n");
			return ret;
		}
		DPU_FB_INFO("free dsi_interrupt irq[%s]\n", dpe_irq_info.dsi_irq_name);
		free_irq(dsi_irq, (void *)dpufd);
	}
...
{	
	...
	if (!IS_BUILTIN_PANEL) {
		DPU_FB_WARNING("%d not support request irq\n", dpufd->index);
		return ret;
	}
...
static int dpe_regulator_clk_irq_setup(struct platform_device *pdev)
{
	...
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE && IS_BUILTIN_PANEL) {
		DPU_FB_WARNING("fb%d not need dpe isr fnc setup\n", dpufd->index);
	} else {
...
static void dpu_pdata_cd_fun_init(struct dpu_fb_panel_data *pdata)
{
