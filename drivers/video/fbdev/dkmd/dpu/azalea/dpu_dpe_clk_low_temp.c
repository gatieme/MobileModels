...
void dpufb_low_temperature_exit_idle_clk_protect(struct dpu_fb_data_type *dpufd)
{
	...
	if (g_vote_cmd.dss_pri_clk_rate != 0) {
		up(&g_dpufb_dss_clk_vote_sem);
		return;
	}

	if (dpufd->dss_vote_cmd.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L4) {
		DPU_FB_DEBUG("DSS_CORE_CLK_RATE_L4 to DSS_CORE_CLK_RATE_L3!\n");
		ret = clk_set_rate(dpufd->dss_pri_clk, DEFAULT_DSS_CORE_CLK_RATE_L3);
		if (ret < 0)
			DPU_FB_ERR("fb%d low_temperature clk_set_rate(%llu) failed, error=%d!\n",
				dpufd->index, clk_get_rate(dpufd->dss_pri_clk), ret);
	}
