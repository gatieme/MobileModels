...
static unsigned int g_display_panel_id = 0;
#endif
	...
	char __iomem *gmp_lut_base, char __iomem *gmp_base, uint32_t buff_sel)
{
...
#ifdef CONFIG_DPU_FB_V346
	if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE && g_display_panel_id == 1)
		disp_panel_id = g_display_panel_id;
#endif

	struct gmp_info *gmp = &(dpufd->effect_info[disp_panel_id].
		dpp_buf.buf_info_list[disp_panel_id][buff_sel].gmp);
	...
	DPU_FB_INFO("fb%d gmp_enable = %d disp_panel_id = %d!\n",
		dpufd->index, gmp_enable, disp_panel_id);
}
...
void init_igm_gmp_xcc_gm(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_DEBUG("buff_sel=%u, latest_buf=%u\n", buff_sel,
		dpp_buff_mngr->latest_buf[disp_panel_id]);
	...
	if (is_init) {
		...
		if (dpp_buff_mngr->buf_work_mode == DPP_BUF_WORK_MODE_BOTH_EFFECT) {
			...
			dpufd->effect_info[disp_panel_id].dpp_buf.roi_new_updated = true;
