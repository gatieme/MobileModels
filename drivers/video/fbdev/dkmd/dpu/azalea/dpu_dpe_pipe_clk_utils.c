...
int dpufb_pipe_clk_updt_handler(struct dpu_fb_data_type *dpufd, bool primary_panel_pwr_on)
{
	struct dpu_panel_info *pinfo = NULL;
	struct dpufb_pipe_clk *pipe_clk_ctrl = NULL;
	int ret = 0;
	...
	if (primary_panel_pwr_on) {
		if (dpufd_list[EXTERNAL_PANEL_IDX] && dpufd_list[EXTERNAL_PANEL_IDX]->panel_power_on &&
			...
			ret = dpufb_pipe_clk_updt_config(dpufd);
	} else {
...
static int dpufb_pipe_clk_input_para_check(struct dpu_fb_data_type *dpufd)
{
	struct dpu_fb_data_type *primary_dpufd = NULL;
	struct dpu_panel_info *primary_pinfo = NULL;
...
static int dpufb_pipe_clk_pre_process(struct dpu_fb_data_type *primary_dpufd)
{
	...
	if (primary_dpufd->panel_power_on) {
		if (pipe_clk_ctrl->pipe_clk_rate > primary_pinfo->pxl_clk_rate) {
			...
			dpu_fb_frame_refresh(primary_dpufd, "pipeclk");
		}
	} else {
		if (pipe_clk_ctrl->pipe_clk_rate > primary_pinfo->pxl_clk_rate) {
			ret = dpufb_change_pipe_clk_rate(primary_dpufd, pipe_clk_ctrl->pipe_clk_rate);
...
static void dpu_pipe_clk_updt_work_handler(struct work_struct *work)
{
	...
	dpu_check_and_no_retval((!IS_BUILTIN_PANEL), ERR,
...
void dpufb_pipe_clk_updt_work_init(struct dpu_fb_data_type *dpufd)
{
	...
	if (!IS_BUILTIN_PANEL) {
		DPU_FB_ERR("dpufb_pipe_clk_updt_work_init fail %d\n", dpufd->index);
		return;
	}
	...
	INIT_WORK(&(pipe_clk_ctrl->pipe_clk_handle_work), dpu_pipe_clk_updt_work_handler);
