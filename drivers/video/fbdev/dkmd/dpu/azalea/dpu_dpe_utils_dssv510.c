...
#ifndef CONFIG_DPU_FB_V600
	dsc_base = dpufd->dss_base + DSS_DSC_OFFSET;
#else
	if (IS_BUILTIN_PANEL)
		dsc_base = dpufd->dss_base + DSS_DSC_OFFSET;
	else
		dsc_base = dpufd->dss_base + DSS_DSC_OFFSET_1;
#endif
...
static void config_panel_dsc_para(struct dpu_fb_data_type *dpufd, struct dsc_info_mipi *out_dsc_info)
{
	struct panel_dsc_info *panel_dsc_info = NULL;

	panel_dsc_info = &(dpufd->panel_info.panel_dsc_info);
	panel_dsc_info->dual_dsc_en = out_dsc_info->dual_dsc_en;
	panel_dsc_info->dsc_info.chunk_size = out_dsc_info->chunk_size;

	panel_dsc_info->dsc_insert_byte_num = 0;
	if (panel_dsc_info->dsc_info.chunk_size % 3)
		panel_dsc_info->dsc_insert_byte_num = (3 - panel_dsc_info->dsc_info.chunk_size % 3) & 0x3;
}
#endif
...
#if defined(CONFIG_DPU_FB_V346)
	config_panel_dsc_para(dpufd, out_dsc_info);
#endif
}
		...
		struct dsc_panel_info *dsc, struct dsc_info_mipi *input_dsc_info)
{
	...
	if (input_dsc_info->chunk_size % 6)
		dsc_insert_byte_num = (6 - input_dsc_info->chunk_size % 6);
...
static void init_dsc(struct dpu_fb_data_type *dpufd)
{
	...
	if (pinfo->spr_dsc_mode != SPR_DSC_MODE_NONE) {
		DPU_FB_INFO("spr enable, dsc1.1 bypass\n");
		return;
	}
#endif
...
{
	DPU_FB_DEBUG("[DSC] +\n");
	dpu_check_and_no_retval((dsc == NULL || dsc_base == NULL), ERR, "NULL POINTER");
...
static void dss_dsc_out_mode_cfg(char __iomem *dsc_base, struct panel_dsc_info *dsc)
{
	...
	if (((dsc->dsc_info.chunk_size + dsc->dsc_insert_byte_num) % 6) == 0) {
		...
		DPU_FB_DEBUG("dsc->dsc_insert_byte_num 6\n");
	} else if (((dsc->dsc_info.chunk_size + dsc->dsc_insert_byte_num) % 4) == 0) {
		set_reg(dsc_base + DSC_OUT_CTRL, 0x1, 1, 0);
		DPU_FB_DEBUG("dsc->dsc_insert_byte_num 4\n");
	} else {
		DPU_FB_ERR("chunk_size should be mode by 3 or 2, but chunk_size = %u\n", dsc->dsc_info.chunk_size);
		return;
	}
}
...
static void dss_dsc_cfg(struct dpu_fb_data_type *dpufd)
{
	char __iomem *dsc_base = NULL;
	struct dpu_panel_info *pinfo = NULL;
	struct panel_dsc_info *panel_dsc_info = NULL;

	dpu_check_and_no_retval((dpufd == NULL), ERR, "NULL POINTER");
	DPU_FB_DEBUG("+\n");
	pinfo = &(dpufd->panel_info);
#ifdef SUPPORT_SPR_DSC1_2_SEPARATE
	if (pinfo->spr_dsc_mode != SPR_DSC_MODE_NONE) {
	...
	if (IS_BUILTIN_PANEL) {
		dsc_base = dpufd->dss_base + DSS_DSC_OFFSET;
		DPU_FB_DEBUG("[dsc] offset_0\n");
	} else {
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
		dsc_base = dpufd->dss_base + DSS_DSC_OFFSET_1;
#endif
		DPU_FB_DEBUG("[dsc] offset_1\n");
	}
...
static void dss_dsc_calculation(struct dpu_panel_info *pinfo)
{
	struct panel_dsc_info *panel_dsc_info = NULL;
...
static void dss_dsc_calc_dsc_ctrl(struct dpu_panel_info *pinfo)
{
	struct panel_dsc_info *panel_dsc_info = NULL;
	uint32_t dsc_en = 0;
	uint32_t dual_dsc_en = 0;
...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
static void dss_dsc_do_calculation(struct dpu_panel_info *pinfo)
{
	DPU_FB_DEBUG("+\n");
...
void init_ifbc(struct dpu_fb_data_type *dpufd)
{
	...
	if (is_ifbc_vesa_panel(dpufd)) {
		...
		if (pinfo->vesa_dsc.bits_per_pixel == DSC_0BPP) {
			dss_dsc_do_calculation(&(dpufd->panel_info));
			dss_dsc_cfg(dpufd);
		} else {
			init_dsc(dpufd);
		}
...
void init_post_scf(struct dpu_fb_data_type *dpufd)
{
	...
	dpufd->effect_info[pinfo->disp_panel_id].arsr1p[0].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr1p_update_normal;
	dpufd->effect_info[pinfo->disp_panel_id].arsr1p[1].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr1p_update_fhd;
	// cppcheck-suppress *
	dpufd->effect_info[pinfo->disp_panel_id].arsr1p[2].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr1p_update_hd;
	dpufd->effect_info[pinfo->disp_panel_id].arsr2p[0].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr2p_update_normal;
	dpufd->effect_info[pinfo->disp_panel_id].arsr2p[1].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr2p_update_scale_up;
	// cppcheck-suppress *
	dpufd->effect_info[pinfo->disp_panel_id].arsr2p[2].update =
		dpufd->effect_init_update[pinfo->disp_panel_id].arsr2p_update_scale_down;
...
static uint32_t init_dbuf_calculate_rect_width(struct dpu_fb_data_type *dpufd)
{
	uint32_t ifbc_type;
	uint32_t mipi_idx;
	uint32_t rect_width;
	...
	if (is_dp_panel(dpufd)) {
...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
		if ((dpufd->dp.dsc == true) && (pinfo->ifbc_type != IFBC_TYPE_NONE))
			rect_width = dpufd->dp.vparams.dp_dsc_info.h_active_new;
#endif
	} else {
		mipi_idx = 0;
		ifbc_type = pinfo->ifbc_type;
		...
		if ((pinfo->mode_switch_to == MODE_10BIT_VIDEO_3X) && (pinfo->ifbc_type == IFBC_TYPE_VESA3X_DUAL)) {
			...
			if ((pinfo->xres % xres_div) > 0)
				...
				rect_width = (((panel_dsc_info->dsc_info.chunk_size + panel_dsc_info->dsc_insert_byte_num) +
					(panel_dsc_info->dsc_info.chunk_size + panel_dsc_info->dsc_insert_byte_num) % 6) *
		...
		rect_width = get_hsize_after_spr_dsc(dpufd, rect_width);
	}
...
#if defined(CONFIG_DPU_FB_V350) || defined(CONFIG_DPU_FB_V345) || defined(CONFIG_DPU_FB_V346)
static void init_dbuf_calculate_thd(struct thd *thd_datas, struct dfs_ram_info ram_data)
{
	...
	thd_datas->rqos_in = DBUF_DEPTH * 80 / 100;
	thd_datas->rqos_out = DBUF_DEPTH * 90 / 100;
	thd_datas->flux_req_befdfs_in = DBUF_DEPTH * 70 / 100;
	thd_datas->flux_req_befdfs_out = DBUF_DEPTH * 80 / 100;
	...
	thd_datas->flux_req_sw_en = 0;
	thd_datas->wqos_in = 0;
	thd_datas->wqos_out = 0;
	thd_datas->wr_wait = 0;
	thd_datas->cg_hold = 0;
	thd_datas->rqos_idle = 0;

	DPU_FB_INFO("depth=%d,\n"
		"ram_data.sram_valid_num=0x%x\n"
...
void init_dbuf(struct dpu_fb_data_type *dpufd)
{
	...
	dpu_check_and_no_retval((!IS_BUILTIN_PANEL),  ERR, "dpufd index not support\n");

	dbuf_base = dpufd->dss_base + DSS_DBUF0_OFFSET;

	rect_width = init_dbuf_calculate_rect_width(dpufd);

	if (unlikely(pinfo->pxl_clk_rate_div == 0))
		pinfo->pxl_clk_rate_div = 1;

	ram_data.sram_valid_num = 1;
	ram_data.dfs_ram = 0;
...
static void init_dbuf_calculate_thd(struct thd *thd_datas, struct dfs_ram_info ram_data)
{
...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
	if (is_single_slice_dsc_panel(dpufd_list[PRIMARY_PANEL_IDX])) {
		thd_datas->flux_req_befdfs_in = depth * DBUF_THRESHOLD_IN_FOR_SINGLE_DSC / 100;
		thd_datas->flux_req_befdfs_out = depth * DBUF_THRESHOLD_OUT_FOR_SINGLE_DSC / 100;
	}
#endif
	thd_datas->flux_req_aftdfs_in = thd_datas->flux_req_befdfs_in;
	thd_datas->flux_req_aftdfs_out = thd_datas->flux_req_befdfs_out;
	...
	DPU_FB_INFO("ram_data.sram_valid_num=%d,\n"
		"thd_datas->rqos_in=0x%x\n"
		"thd_datas->rqos_out=0x%x\n"
		"thd_datas->cg_in=0x%x\n"
		"thd_datas->cg_out=0x%x\n"
...
static void init_dpp_ifbc_sw(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->panel_info.dual_lcd_support) {
		// OV0--DPP0, OV1--DPP1
		set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + DPPSW_SIG_CTRL, 0x201, 24, 0);
		set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + DPPSW_DAT_CTRL, 0x201, 24, 0);
		set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + IFBCSW_SIG_CTRL, 0x201, 16, 0);
		set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + IFBCSW_DAT_CTRL, 0x201, 16, 0);
		return;
	}

	if (IS_BUILTIN_PANEL) {
#ifdef CONFIG_DPU_EFFECT_HIHDR
		if (pinfo->post_hihdr_support) {
			set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + HDRSW_SIG_CTRL, 0x102, 16, 0);
			set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + HDRSW_DAT_CTRL, 0x102, 16, 0);

			set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + DPPSW_SIG_CTRL, 0x10202, 24, 0);
			set_reg(dpufd->dss_base + DSS_DISP_GLB_OFFSET + DPPSW_DAT_CTRL, 0x10202, 24, 0);
		} else {
...
static void set_ldi_dpio_reg(struct dpu_fb_data_type *dpufd, dss_rect_t rect, char __iomem *ldi_base)
{
	struct dpu_panel_info *pinfo = NULL;
...
static void set_ldi_dsi_reg(struct dpu_fb_data_type *dpufd, char __iomem *ldi_base)
{
	uint32_t te_source;
	struct dpu_panel_info *pinfo = NULL;
...
static void set_ldi_colorbar_used_reg(struct dpu_fb_data_type *dpufd, char __iomem *ldi_base)
{
	void_unused(dpufd);
...
void init_ldi(struct dpu_fb_data_type *dpufd, bool fastboot_enable)
{
	...
	dpu_check_and_no_retval((!is_dp_panel(dpufd)), INFO, "fb%d is not dp panel\n", dpufd->index);

	for (i = 0; i < dpufd->dp.streams; i++) {
		ldi_base = (i == 0) ? (dpufd->dss_base + DSS_LDI_DP_OFFSET) : (dpufd->dss_base + DSS_LDI_DP1_OFFSET);
		DPU_FB_INFO("fb%d, fastboot_enable = %d!", dpufd->index, fastboot_enable);
		ldi_init_rect(dpufd, &rect);
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
		if ((dpufd->dp.dsc == true) && (pinfo->ifbc_type != IFBC_TYPE_NONE))
			rect.w = (int32_t)(dpufd->dp.vparams.dp_dsc_info.h_active_new);
		else
			mipi_ifbc_get_rect(dpufd, &rect);
#else
		mipi_ifbc_get_rect(dpufd, &rect);
#endif
		set_ldi_dpio_reg(dpufd, rect, ldi_base);
		...
		set_ldi_dsi_reg(dpufd, ldi_base);
		set_ldi_colorbar_used_reg(dpufd, ldi_base);
...
void single_frame_update(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_ATRACE_BEGIN("single_frame_update");
	...
	if (is_dp_panel(dpufd)) {
		...
		ldi_base = dpufd->dss_base + DSS_LDI_DP1_OFFSET;
		set_reg(ldi_base + LDI_FRM_MSK_UP, 0x1, 1, 0);
	} else if (is_mipi_cmd_panel(dpufd)) {
		...
		if (dpufd->panel_info.adaptive_frm_rate.is_support_ltpo) {
			if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE)
				dpufd->panel_info.frm_rate_dimming.need_reset_dimming = true;
			else
				dpu_reset_idle_para(dpufd, FRM_UPDATE);
		}
#endif
	} else {
		DPU_FB_DEBUG("fb%d video panel ldi_en\n", dpufd->index);
	}

	enable_ldi(dpufd);
	DPU_ATRACE_END("single_frame_update");
}
		...
		struct dpu_panel_info *pinfo, char __iomem *dss_base)
{
	...
	halfway_frm_irq = BIT_QUARTER_FRM;
#endif

	pri_unmask = ~0;
	if (is_mipi_cmd_panel(dpufd))
		pri_unmask &= ~(lcd_te_pin | BIT_VACTIVE0_START | BIT_VACTIVE0_END | BIT_FRM_END | halfway_frm_irq);
	else
...
static void external_panel_dpe_interrupt_unmask(struct dpu_fb_data_type *dpufd,
		char __iomem *dss_base)
{
	...
	unmask = ~0;
	if (is_mipi_cmd_panel(dpufd))
		unmask &= ~(lcd_te_pin | BIT_VACTIVE0_START | BIT_VACTIVE0_END | BIT_FRM_END);
	else
...
void dpe_interrupt_unmask(struct dpu_fb_data_type *dpufd)
{
	...
	if (IS_BUILTIN_PANEL) {
		...
		external_panel_dpe_interrupt_unmask(dpufd, dss_base);
	} else if ((dpufd->index == EXTERNAL_PANEL_IDX) && is_dp_panel(dpufd)) {
		unmask = ~0;
