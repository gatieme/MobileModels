...
struct platform_device *dpu_fb_add_device(struct platform_device *pdev)
{
	...
	DPU_FB_INFO("dpufb_init_regulator: fbi_list_index=%d,id=%d \n",fbi_list_index,id);
#ifdef CONFIG_DPU_FB_V346
	if (((type & PANEL_MIPI_CMD) || (type & PANEL_MIPI_VIDEO)) && id == 2) {
	...
	dpufd_list[dpufd->index] = dpufd;
	fbi_list[dpufd->index] = fbi;
...
static void dpu_fb_displayeffect_update(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("[effect]+ disp_panel_id %d\n", disp_panel_id);
...
void dpu_aod_schedule_wq(void)
{
	...
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE) {
		dpufd = dpufd_list[PRIMARY_PANEL_IDX];
		if (dpufd_list[BUILT_IN_PANEL_INDEX]->enable_fast_unblank_for_dual_display)
			dpufd = dpufd_list[BUILT_IN_PANEL_INDEX];

		if (dpufd->enable_fast_unblank_for_dual_display) {
			DPU_FB_INFO("[fb%d] aod schedule work queue!\n", dpufd->index);
		} else {
			DPU_FB_INFO("[fb%d] power up with fb blank!\n", dpufd->index);
...
#endif
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE)
		dpufd->enable_fast_unblank_for_dual_display = false;
}
...
#endif
		if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE)
			dpufd->enable_fast_unblank_for_dual_display = true;
		return true;
	}
...
static void dpufb_check_aod_power_on_status(struct dpu_fb_data_type *dpufd)
{
	struct dpu_fb_data_type *check_dpufd = NULL;
	int ret = 0;
	...
	if (dpufd->index == PRIMARY_PANEL_IDX)
		check_dpufd = dpufd_list[BUILT_IN_PANEL_INDEX];
	else
		check_dpufd = dpufd_list[PRIMARY_PANEL_IDX];
#ifdef CONFIG_LCD_KIT_DRIVER
	ret = lcd_kit_check_panel_power_on_status(check_dpufd);
#endif
	if (ret) {
		DPU_FB_INFO("other panel is power on\n");
		dpu_fb_blank_device(check_dpufd, FB_BLANK_UNBLANK, check_dpufd->fbi);
		dpu_fb_blank_device(check_dpufd, FB_BLANK_POWERDOWN, check_dpufd->fbi);
	}
}
#endif
...
static int dpu_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE &&
		...
		dpufb_check_aod_power_on_status(dpufd);
#endif
...
static int dpu_fb_open(struct fb_info *info, int user)
{
	...
	if (!dpufd->ref_cnt) {
		DPU_FB_DEBUG("fb%d, +!\n", dpufd->index);
		if (dpufd->open_sub_fnc != NULL && dpufd->index != BUILT_IN_PANEL_INDEX) {
#if defined(CONFIG_HWLOG_KERNEL)
		...
		if (IS_BUILTIN_PANEL) {
			dpufd->panel_info.dpu_task = current;
			DPU_FB_INFO("fb%d, -! task id %u\n", dpufd->index, current? current->tgid: 0);
		}
	}
...
static int dpufb_dss_get_platform_product_info(struct dpu_fb_data_type *dpufd, void __user *argp)
{
	...
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE) {
		DPU_FB_INFO("get product_info %d, %d, %d * %d\n", dpufd->index,
			dpufd->panel_info.product_type, dpufd->panel_info.xres, dpufd->panel_info.yres);

		index = DISPLAY_INNEL_PANEL_ID;
		if (dpufd->index == BUILT_IN_PANEL_INDEX)
			index = DISPLAY_OUTER_PANEL_ID;

		get_platform_product_info.xres[index] = dpufd->panel_info.xres;
		get_platform_product_info.yres[index] = dpufd->panel_info.yres;
		get_platform_product_info.width[index] = dpufd->panel_info.width;
		get_platform_product_info.height[index] = dpufd->panel_info.height;
	}
	get_platform_product_info.product_type = dpufd->panel_info.product_type;

#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	if (dpufd->panel_info.adaptive_frm_rate.is_support_ltpo) {
		get_platform_product_info.is_ltpo_panel = dpufd->panel_info.adaptive_frm_rate.is_support_ltpo;
		fps_sup_num = dpufd->panel_info.dfr_info.oled_info.fps_sup_num < FPS_LEVEL_END ?
			dpufd->panel_info.dfr_info.oled_info.fps_sup_num : FPS_LEVEL_END;
		get_platform_product_info.fps_info_count = fps_sup_num;
		for (i = 0; i < fps_sup_num; i++)
			get_platform_product_info.fps_info[i] = dpufd->panel_info.dfr_info.oled_info.fps_sup_seq[i];
	}
#endif
	...
	DPU_FB_INFO("max_hwc_mmbuf_size=%d, max_mdc_mmbuf_size=%d,"
		...
		"bypass_device_compose=%d, dual_lcd_support = %d, fb%d\n",
...
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	if (get_platform_product_info.is_ltpo_panel) {
		DPU_FB_INFO("get_platform_product_info.fps_info_count is %d.\n", get_platform_product_info.fps_info_count);
		for (int i = 0 ; i < fps_sup_num; i++) {
			DPU_FB_INFO("get_platform_product_info.fps_info[%d] is %d.\n", i, get_platform_product_info.fps_info[i]);
		}
	}
#endif
	return 0;
...
#endif
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	case DPUFB_SAFE_FRM_RATE:
		if (dpufd->panel_info.frm_rate_dimming.set_safe_frm_rate != NULL)
			ret = dpufd->panel_info.frm_rate_dimming.set_safe_frm_rate(dpufd, argp);
		break;
#endif
	default:
		break;
...
void dpu_fb_pdp_fnc_register(struct dpu_fb_data_type *dpufd)
{
	...
	if (g_fastboot_enable_flag == 1 && dpufd->index != BUILT_IN_PANEL_INDEX) {
...
static int dpu_fb_register(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpu_fbi_info_init(dpufd, fbi) < 0) {
		DPU_FB_ERR("fb%d failed to dpu_fbi_info_init!\n", dpufd->index);
		return -EINVAL;
	}
	...
	if (dpu_fb_registe_callback(dpufd, &fbi->var, panel_info) < 0) {
		DPU_FB_ERR("fb%d failed to registe_callback!\n", dpufd->index);
		return -EINVAL;
	}
	...
	if (dpu_overlay_init(dpufd)) {
		DPU_FB_ERR("fb%d failed to overlay_init!\n", dpufd->index);
		return -EPERM;
	}

#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	dpu_adaptive_frame_rate_init(dpufd);
#endif
...
int dpu_fb_get_cur_panel(void)
{
	...
	if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE) {
		...
		if (primary_dpufd && primary_dpufd->panel_power_on) {
			DPU_FB_DEBUG("fb%d is power on\n", primary_dpufd->index);
			return DISPLAY_OUTER_PANEL_ID;
		}
	}
	primary_dpufd = dpufd_list[PRIMARY_PANEL_IDX];
	if (primary_dpufd && primary_dpufd->panel_power_on) {
		DPU_FB_DEBUG("fb%d is power on\n", primary_dpufd->index);
		return DISPLAY_INNEL_PANEL_ID;
	}
	return DISPLAY_INNEL_PANEL_ID;
}
