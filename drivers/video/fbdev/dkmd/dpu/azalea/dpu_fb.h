...
extern int g_fastboot_set_needed;

#ifdef CONFIG_DPU_FB_V346
extern uint32_t g_last_bkl_lvl;
#endif

enum {
	EXT_PANEL_SPLIT_EN_OFFSET = 0,
	EXT_PANEL_SPLIT_RATIO_OFFSET = 8,
	EXT_PANEL_USE_DSI1_OFFSET = 16,
};

enum {
	DIMMING_LIST_TYPE_NORMAL = 1,
	DIMMING_LIST_TYPE_DELAY = 2,
};

void de_open_update_te(void);
void de_close_update_te(struct work_struct *work);
extern struct fb_info *fbi_list[DPU_FB_MAX_FBI_LIST];
...
int dpufb_buf_sync_create_fence(struct dpu_fb_data_type *dpufd, int32_t *release_fence_fd, int32_t *retire_fence_fd);
...
int dpu_lcd_ocp_recover(struct notifier_block *nb,
		unsigned long event, void *data);
#endif

#if defined(CONFIG_DPU_FB_AOD)
/* sensorhub aod */
bool dpu_sensorhub_aod_hw_lock(struct dpu_fb_data_type *dpufd);
bool dpu_sensorhub_aod_hw_unlock(struct dpu_fb_data_type *dpufd);
int dpu_sensorhub_aod_unblank(uint32_t msg_no);
int dpu_sensorhub_aod_blank(uint32_t msg_no);
bool dpu_fb_request_aod_hw_lock(struct dpu_fb_data_type *dpufd);
void dpu_fb_release_aod_hw_lock(struct dpu_fb_data_type *dpufd, bool locked);
int dpu_fb_aod_blank(struct dpu_fb_data_type *dpufd, int blank_mode);
bool dpu_fb_get_aod_lock_status(void);
...
static inline uint32_t dpu_aod_get_panel_id(void) { return 0; };
#endif
#endif
...
void dpu_fb_init_sreeninfo_by_img_type(struct fb_fix_screeninfo *fix,
	struct fb_var_screeninfo *var, uint32_t fb_imgType, int *bpp);
void dpu_fb_init_sreeninfo_by_panel_info(struct fb_var_screeninfo *var,
