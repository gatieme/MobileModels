...
void dpufb_set_backlight(struct dpu_fb_data_type *dpufd, uint32_t bkl_lvl, bool enforce)
{
	...
	if (!dpufd->panel_power_on || !dpufd->backlight.bl_updated) {
		...
		if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE && !dpufd->panel_power_on)
			g_last_bkl_lvl = bkl_lvl;
#endif
	...
	if (pdata->set_backlight) {
		if (dpufd->backlight.bl_level_old == (int32_t)temp && !enforce) {
			dpufd->bl_level = bkl_lvl;
			if ((dpufd->panel_info.eink_lcd) && (bkl_lvl == 0)) {
				pdata->set_backlight(dpufd->pdev, bkl_lvl);
				DPU_FB_INFO("set_bl eink-lcd: bl_level_old = %d, current_bl_level = %u, enforce = %d!\n",
					dpufd->backlight.bl_level_old, temp, enforce);
			} else {
				DPU_FB_INFO("set_bl return: bl_level_old = %d, current_bl_level = %u, enforce = %d!\n",
					dpufd->backlight.bl_level_old, temp, enforce);
			}
			return;
		}
...
#ifdef CONFIG_LCD_KIT_DRIVER
	dpufd = lcd_kit_get_hisifd(led_cdev->dev->parent);
#else
