...
#include "dpu_trace.h"
#define DPU_LAYERBUF_FREE "dpufb%u-layerbuf-free"

/* layerbuffer handle, for online compose */
static bool dpu_check_parameter_valid(struct dpu_fb_data_type *dpufd,
	...
	dss_layer_t *layer)
{
	...
	buf_handle = dpu_get_dmabuf(layer->img.shared_fd);
	if (!buf_handle)
		return -EINVAL;
	...
	dss_overlay_t *pov_req, struct list_head *plock_list)
{
	...
	parameter_valid = dpu_check_parameter_valid(dpufd, pov_req, plock_list);
	if (!parameter_valid)
		return -EINVAL;
	...
	struct list_head *pfree_list)
{
	...
	list_for_each_entry_safe(node, _node_, pfree_list, list_node) {
		...
		if (node->buffer_handle) {
			...
			dpu_mmu_tlb_flush(dpufd, node->frame_no);
#endif
			dpu_put_dmabuf(node->buffer_handle);
	...
	struct list_head *plock_list, dss_wb_layer_t *wb_layer4block)
{
	...
	void_unused(pov_req);
	buf_handle = dpu_get_dmabuf(wb_layer4block->dst.shared_fd);
	if (buf_handle) {
	...
	struct list_head *plock_list, dss_layer_t *layer)
{
	...
	node->fence = (dpufd->index == MEDIACOMMON_PANEL_IDX) ?
		dpu_get_fd_sync_fence(layer->acquire_fence) : NULL;
	node->shared_fd = layer->img.shared_fd;
	node->buffer_handle = buf_handle;
	node->timeline = 2;  /* timeline to create the fence on */
...
/**
 * dpu_fb_sync_get_fence() - get fence from timeline
 ...
 */
static struct dpu_fence *dpu_fb_sync_get_fence(struct dpu_timeline *timeline,
		const char *fence_name, int val)
{
	struct dpu_fence *fence = NULL;

	fence = dpu_get_sync_fence(timeline, fence_name, NULL, val);
...
static struct dpu_fence *dpufb_buf_create_fence(struct dpu_fb_data_type *dpufb,
	struct dpufb_buf_sync *buf_sync_ctrl, u32 fence_type,
	int *fence_fd, int value)
{
	struct dpu_fence *sync_fence = NULL;
	char fence_name[32];  /* fence name length */

	if (fence_type == DPU_RETIRE_FENCE) {
		snprintf(fence_name, sizeof(fence_name), "fb%u_retire_%u", dpufb->index, dpufb->ov_req.frame_no);
		sync_fence = dpu_fb_sync_get_fence(
					buf_sync_ctrl->timeline_retire,
					fence_name, value);
	} else {
		snprintf(fence_name, sizeof(fence_name), "fb%u_release_%u", dpufb->index, dpufb->ov_req.frame_no);
		sync_fence = dpu_fb_sync_get_fence(
					buf_sync_ctrl->timeline,
					fence_name, value);
	}

	DPU_ATRACE_BEGIN_WITH_VALUE("dpufb_buf_create_fence", dpufb->ov_req.frame_no);
	...
	/* get fence fd */
	*fence_fd = dpu_get_sync_fence_fd(sync_fence);
	if (*fence_fd < 0) {
		DPU_FB_ERR("%s: get_unused_fd_flags failed error:0x%x\n", fence_name, *fence_fd);
		dpu_put_sync_fence(sync_fence);
		sync_fence = NULL;
		goto end;
	}
	...
	DPU_ATRACE_END_WITH_VALUE("dpufb_buf_create_fence", dpufb->ov_req.frame_no);
	return sync_fence;
}
...
int dpufb_offline_create_fence(struct dpu_fb_data_type *dpufd)
{
	int value;
	int ret = 0;
	struct dpu_fence *release_fence = NULL;
	...
	release_fence = dpufb_buf_create_fence(dpufd, buf_sync_ctrl,
		DPU_RELEASE_FENCE, &(dpufd->ov_req.release_fence), value);
	...
	int32_t *release_fence_fd, int32_t *retire_fence_fd)
{
	...
	unsigned long flags = 0;
	struct dpu_fence *release_fence = NULL;
	struct dpu_fence *retire_fence = NULL;
	struct dpufb_buf_sync *buf_sync_ctrl = NULL;
	...
	refresh = ++buf_sync_ctrl->refresh;
	timeline_max = buf_sync_ctrl->timeline_max;
	spin_unlock_irqrestore(&buf_sync_ctrl->refresh_lock, flags);

	value = timeline_max + refresh;
	release_fence = dpufb_buf_create_fence(dpufd, buf_sync_ctrl,
...
retire_fence_err:
	...
	dpu_put_sync_fence(release_fence);
release_fence_err:
	*retire_fence_fd = -1;
	*release_fence_fd = -1;

	spin_lock_irqsave(&buf_sync_ctrl->refresh_lock, flags);
	--buf_sync_ctrl->refresh;
	spin_unlock_irqrestore(&buf_sync_ctrl->refresh_lock, flags);

	return ret;
}

static int dpufb_buf_sync_wait_fence(struct dpu_fence *fence)
{
	int ret;

	ret = dpu_wait_sync_fence(fence, BUF_SYNC_TIMEOUT_MSEC);
	if (ret < 0)
		DPU_FB_ERR("Waiting on fence=%pK failed, ret=%d\n", fence, ret);

	dpu_put_sync_fence(fence);
...
int dpufb_buf_sync_wait(int fence_fd)
{
	...
	fence = dpu_get_fd_sync_fence(fence_fd);
	if (!fence) {
...
void dpufb_buf_sync_signal(struct dpu_fb_data_type *dpufd)
{
	...
	if (buf_sync_ctrl->refresh) {
		DPU_ATRACE_BEGIN_WITH_VALUE("dpufb_buf_sync_signal", dpufd->ov_req.frame_no);
		val = buf_sync_ctrl->refresh;
		dpu_inc_timeline(buf_sync_ctrl->timeline, val);
		dpu_inc_timeline(buf_sync_ctrl->timeline_retire, val);
		...
		DPU_ATRACE_END_WITH_VALUE("dpufb_buf_sync_signal", dpufd->ov_req.frame_no);
	}
...
void dpufb_buf_sync_register(struct platform_device *pdev)
{
	...
	if (dpufd->index != AUXILIARY_PANEL_IDX) {
		snprintf(tmp_name, sizeof(tmp_name), "dpu_fb%u", dpufd->index);
		buf_sync_ctrl->timeline = dpu_create_timeline(tmp_name);
		dpu_check_and_no_retval(!buf_sync_ctrl->timeline, ERR, "cannot create release fence time line\n");

		snprintf(tmp_name, sizeof(tmp_name), "dpu_fb%u_retire", dpufd->index);
		buf_sync_ctrl->timeline_retire = dpu_create_timeline(tmp_name);
	...
	snprintf(tmp_name, sizeof(tmp_name), DPU_LAYERBUF_FREE, dpufd->index);
...
void dpufb_buf_sync_unregister(struct platform_device *pdev)
{
	...
	if (buf_sync_ctrl->timeline) {
		dpu_destroy_timeline(buf_sync_ctrl->timeline);
		buf_sync_ctrl->timeline = NULL;
	}

	if (buf_sync_ctrl->timeline_retire) {
		dpu_destroy_timeline(buf_sync_ctrl->timeline_retire);
		buf_sync_ctrl->timeline_retire = NULL;
	}
