...
int g_debug_safe_frame_rate;
#ifdef CONFIG_DPU_FB_ENG_DBG
module_param_named(debug_safe_frame_rate, g_debug_safe_frame_rate, int, 0644);
MODULE_PARM_DESC(debug_safe_frame_rate, "dpu safe frame rate debug");
#endif
...
void dpufb_debug_register(struct platform_device *pdev)
{
	...
	if (!lcd_dclient) {
#if defined(CONFIG_LCDKIT_DRIVER)
		if (get_lcdkit_support() && IS_BUILTIN_PANEL) {
