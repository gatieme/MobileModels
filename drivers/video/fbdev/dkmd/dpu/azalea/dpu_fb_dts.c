...
char __iomem *dpufd_soc_acpu_ao_ioc_base;
static char __iomem *dpufd_usb_dp_ctrl_base;
static char __iomem *dpufd_usb_tca_base;
#endif
...
int dpu_fb_read_property_from_dts(struct device_node *np, struct device *dev)
{
	...
	ret = of_property_read_u32(np, "mipi_dphy_opt", &g_mipi_dphy_opt);
	if (ret)
		g_mipi_dphy_opt = 0;
	dev_info(dev, "g_mipi_dphy_opt=0x%x\n", g_mipi_dphy_opt);
#endif
...
int dpu_fb_get_baseaddr_from_dts(struct device_node *np, struct device *dev)
{
	...
	dpufd_dp_base = of_iomap(np, 8);
	dev_check_and_return(dev, !dpufd_dp_base, -ENXIO, err, "failed to get dpufd_dp_base resource\n");

	dpufd_media_common_base = of_iomap(np, 9);
	dev_check_and_return(dev, !dpufd_media_common_base, -ENXIO, err,
		"failed to get dpufd_media_common_base resource\n");

	dpufd_soc_acpu_ao_ioc_base = of_iomap(np, 10);
	dev_check_and_return(dev, !dpufd_soc_acpu_ao_ioc_base, -ENXIO, err,
		"failed to get dpufd_soc_acpu_ao_ioc_base resource\n");
	...
	dpufd_usb_dp_ctrl_base = of_iomap(np, 12);
	dev_check_and_return(dev, !dpufd_usb_dp_ctrl_base, -ENXIO, err,
		"failed to get dpufd_usb_dp_ctrl_base resource\n");

	dpufd_usb_tca_base = of_iomap(np, 13);
	dev_check_and_return(dev, !dpufd_usb_tca_base, -ENXIO, err, "failed to get dpufd_usb_tca_base resource\n");
...
#if defined(CONFIG_DPU_FB_V410) || defined(CONFIG_DPU_FB_V501) || defined(CONFIG_DPU_FB_V510)
	dpufd_pmctrl_base = of_iomap(np, 6);
	dev_check_and_return(dev, !dpufd_pmctrl_base, -ENXIO, err, "failed to get dpufd_pmctrl_base resource\n");
...
#elif defined(CONFIG_DPU_FB_V320) || defined(CONFIG_DPU_FB_V330) || \
...
int dpu_fb_get_clk_name_from_dts(struct device_node *np, struct device *dev)
{
	...
	!defined(CONFIG_DPU_FB_V350) && !defined(CONFIG_DPU_FB_V345) && \
	!defined(CONFIG_DPU_FB_V346) && !defined(CONFIG_DPU_FB_V360)
...
int dpufb_init_irq(struct dpu_fb_data_type *dpufd, uint32_t type)
{
	...
	} else if (dpufd->index == BUILT_IN_PANEL_INDEX) {
		dpufd->fb_num = DPU_FB4_NUM;
		dpufd->dpe_irq = dpufd_irq_pdp;
	} else {
...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
	dpufd->usb_dp_ctrl_base = dpufd_usb_dp_ctrl_base;
	dpufd->usb_tca_base = dpufd_usb_tca_base;
#endif
