...
#define DPU_ESD_RECOVER_MAX_COUNT 10
#define DPU_ESD_CHECK_MAX_COUNT 3
#define DPU_ESD_POWER_OFF_TIME 100
...
static void dpufb_esd_recover(struct dpu_fb_data_type *dpufd)
{
	...
	msleep(DPU_ESD_POWER_OFF_TIME);
	if (ret != 0)
		DPU_FB_ERR("fb%d, blank_mode[%d] failed!\n", dpufd->index, FB_BLANK_POWERDOWN);

	/* lcd panel on */
	if (dpu_fb_blank_sub(FB_BLANK_UNBLANK, dpufd->fbi) != 0)
	...
	dpu_fb_frame_refresh(dpufd, "esd");
#if defined(CONFIG_DPU_FB_AOD)
	up(&dpufd->blank_aod_sem);
#endif
...
#else
		if (esd_check_count < DPU_ESD_CHECK_MAX_COUNT) {
#endif
			...
			if (ret || ((dpufd->esd_recover_state == ESD_RECOVER_STATE_START) &&
				(dpufd->panel_info.need_platform_recover))) {
#else
			if (ret || (dpufd->esd_recover_state == ESD_RECOVER_STATE_START)) {
#endif
				esd_check_count++;
...
#ifdef CONFIG_LCD_KIT_DRIVER
		if ((esd_check_count >= dpufd->panel_info.esd_check_max_count) ||
			((dpufd->esd_recover_state == ESD_RECOVER_STATE_START) && (dpufd->panel_info.need_platform_recover))) {
#else
		if ((esd_check_count >= DPU_ESD_CHECK_MAX_COUNT) ||
...
static void dpufb_esd_check_wq_handler(struct work_struct *work)
{
	...
	if (dpufd->panel_info.emi_protect_enable && dpufd->enter_idle) {
		...
		if (dpufd->emi_protect_check_count >= DPU_EMI_PROTECT_CHECK_MAX_COUNT)
			dpu_fb_frame_refresh(dpufd, "emi");
	}
