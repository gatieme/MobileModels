	...
	uint32_t isr_s2, struct dpufb_secure *secure_ctrl)
{
	...
	if (dpufd->underflow_flag) {
		...
		dpu_alsc_store_data(dpufd, BIT_VACTIVE0_END);
#endif

		if (!dpufd->panel_info.reload_support && need_config_frm_rate_para(dpufd)) {
...
{
	DPU_ATRACE_BEGIN("dss_te_vsync_isr_handle_base");

#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	if (!dpu_afr_vsync_handle(dpufd)) {
		DPU_ATRACE_END("dss_te_vsync_isr_handle_base");
		return;
	}
#endif
	...
	if (dpu_is_async_play(dpufd)) {
		...
		if (isr_s2 & BIT_VACTIVE0_START) {
			...
			if(dpufd->effect_info[disp_panel_id].dpp_cmdlist_type == DPP_CMDLIST_TYPE_ROI)
				dpu_dpp_cmdlist_region_start(dpufd, CONFIG_REGION_BOTTOM);
#endif
		}
	} else {
		if (isr_s2 & BIT_VACTIVE0_START) {
...
irqreturn_t dss_dsi0_isr(int irq, void *ptr)
{
	...
	DPU_ATRACE_INT_WITH_GID_VALUE("dss_dsi0_isr",dpufd->panel_info.dpu_task, isr_s2);
	DPU_ATRACE_BEGIN_WITH_VALUE("dss_dsi0_isr", isr_s2);
	...
	DPU_ATRACE_END_WITH_VALUE("dss_dsi0_isr", isr_s2);
	return IRQ_HANDLED;
}
...
irqreturn_t dss_dsi1_isr(int irq, void *ptr)
{
	...
	if (IS_BUILTIN_PANEL && is_dual_mipi_panel(dpufd)) {
		if (isr_s2 & BIT_LDI_UNFLOW) {
...
irqreturn_t dss_pdp_isr(int irq, void *ptr)
{
	...
	if (isr_s2_dpp & BIT_HIACE_IND) {
		if (dpufd->panel_info.hiace_support && dpufd->hiace_end_wq)
			dpu_effect_hiace_trigger_wq(dpufd);
	}
#endif
	return IRQ_HANDLED;
}
...
{
	dpu_check_and_no_retval(!dpufd->cmdlist_info, ISR_INFO, "cmdlist_info is NULL\n");
	...
	defined(CONFIG_DPU_FB_V510) || defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V360)
...
#ifdef CONFIG_DPU_DPP_CMDLIST
	dpu_dpp_cmdlist_region_complete();
#endif
