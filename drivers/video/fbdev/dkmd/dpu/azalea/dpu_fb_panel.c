...
#include "product/attrs/dpu_panel_attrs.h"
#endif
...
int dpu_lcd_backlight_off(struct platform_device *pdev)
{
	...
	if (dpufd->panel_info.bl_set_type & BL_SET_BY_PWM)
		ret = dpu_pwm_off(pdev);
	else if (dpufd->panel_info.bl_set_type & BL_SET_BY_BLPWM)
		ret = dpu_blpwm_off(pdev);
	else if (dpufd->panel_info.bl_set_type & BL_SET_BY_SH_BLPWM)
		ret = dpu_sh_blpwm_off(pdev);
	else if (dpufd->panel_info.bl_set_type & BL_SET_BY_MIPI)
		;
	else
		DPU_FB_ERR("No such bl_set_type: %d!\n", dpufd->panel_info.bl_set_type);
...
void panel_mode_switch_isr_handler(struct dpu_fb_data_type *dpufd, uint8_t mode_switch_to)
{
	...
	DPU_FB_DEBUG("+\n");
#ifndef PANEL_MODE_SWITCH_SUPPORT
	...
	struct dpu_panel_info *pinfo)
{
	...
	dpu_fb_aod_blank(dpufd, FB_BLANK_UNBLANK);
#endif
	pinfo = &(dpufd->panel_info);
...
int dpufb_config_panel_esd_status(struct dpu_fb_data_type *dpufd, const void __user *argp)
{
	...
	dpu_check_and_return((!IS_BUILTIN_PANEL), -EINVAL, ERR, "no support\n");
...
#if defined(CONFIG_FOLD_DISPLAY)
static void set_panel_info(uint32_t panel_id, struct platform_product_info *product_info,
	struct dpu_panel_info *pinfo_temp)
{
	dpu_check_and_no_retval(!pinfo_temp, ERR, "pinfo_temp NULL Pointer!\n");
