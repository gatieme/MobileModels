 ...
 *
 */
#ifndef DPU_FB_PANEL_STRUCT_H
#define DPU_FB_PANEL_STRUCT_H

#include "dpu_fb_panel_enum.h"
#include "product/dpu_displayengine_struct.h"

#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
#include "dpu_dfr_info.h"
...
#define MAX_FPS_SEQ_NUM 10
#define MAX_AVERAGE_FPS_LEN 10

struct dpu_dfr_ctrl {
	bool is_support_ltpo;
	uint32_t cur_frm_rate;
	uint32_t target_frm_rate;
	uint32_t cur_te_rate;
	atomic_t skip_te_count;
	ktime_t vsync_realtimestamp;
	ktime_t pre_vsync_timestamp;
	spinlock_t skip_te_lock;
	bool vsync_update_single_frm;
};

#define PERIOD_US_1HZ (1 * 1000 * 1000)
#define PERIOD_US_10HZ (1 * 100 * 1000)
#define PERIOD_US_30HZ ((1 * 100 * 1000) / 3)
#define PERIOD_US_90HZ ((1 * 1000 * 1000) / 90)
#define PERIOD_US_120HZ ((1 * 1000 * 1000) / 120)
#define MAX_RECODE_DURATION_US (24 * 60 * 60 * 1000 * 1000UL) // 1 day
#define PRIMARY_SELF_REFRESH_PERIOD_US (1 * 1000 * 1000)
#define FPS_LIST_SIZE 100
...
struct dpu_frm_rate_static {
	...
	uint32_t cur_te_rate;
	struct panel_refresh_statistic_rslt refresh_stat_info;
	void (*on_static)(struct dpu_fb_data_type *dpufd);
};

struct dpu_frm_dimming {
	int fps_list_len;
	int te_idle_count;
	bool rd_refresh_flag;
	bool setup_dimming_list;
	int debug_dimming_list[FPS_LIST_SIZE];
	int debug_list_len;
	int idle_fps_list[FPS_LIST_SIZE];
	int idle_cur_index;
	int idle_index;
	int idle_list_type;
	int idle_gear_count;
	int safe_frame_rate;
	int idle_target_index;
	bool rd_idle_state;
	bool is_idle_work;
	int base_hw_fps;
	bool dimming_func_switch;
	int (*set_safe_frm_rate)(struct dpu_fb_data_type *dpufd, const void __user *argp);
	bool need_reset_dimming;
};
...
struct dpu_panel_info {
	...
	uint32_t blpwm_output_precision;
	...
	uint32_t need_platform_recover;
	...
	uint32_t mipi_swing_adjust_support;
	uint32_t mipi_swing_adjust;
	...
	timeval_compatible hiace_int_timestamp;
	...
	struct dpu_dfr_ctrl adaptive_frm_rate;
	struct dpu_frm_dimming frm_rate_dimming;
	struct dpu_frm_rate_static frm_rate_static;
#endif
	struct task_struct *dpu_task;
	/* Display engine effects */
	display_engine_dpu_panel_info_t de_panel_info;

#if defined(CONFIG_FOLD_DISPLAY) || \
	defined(CONFIG_LCDKIT_DRIVER) || defined(CONFIG_LCD_KIT_DRIVER)
	uint32_t need_two_panel_display;
	int skip_power_on_off;
	/* disp_panel_id : 0: inner panel; 1: outer panel */
#endif
	int disp_panel_id;
	...
	uint8_t lcd_kit_pc_support;
	bool is_only_use_tbu0;
	bool is_use_pc_backlight;
	bool is_only_use_online_axi;
	/* reload support, set 1 mean use dpi shadow register mechanism feature */
	uint32_t reload_support;
};
...
struct dpu_fb_panel_data {
	...
	int (*fps_scence_get)(int fps, int *scence);
