...
#if defined(CONFIG_DRMDRIVER)
	arm_smccc_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP, master_op_type,
		value, (u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_CTRL,
		0, 0, 0, 0, &res);
#endif
	DPU_FB_DEBUG("-\n");
...
int configure_hdcp_reg_get_security(uint32_t addr)
{
	...
	DPU_FB_DEBUG("+\n");
#if defined(CONFIG_DRMDRIVER)
	arm_smccc_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP, addr, 0x00,
		(u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_INT,
		0, 0, 0, 0, &res);
#endif
	DPU_FB_DEBUG("-\n");
...
static void dpu_fb_secure_blank(struct dpu_fb_data_type *dpufd, struct dpufb_secure *secure_ctrl)
{
	int ret;

#if defined(CONFIG_DPU_FB_AOD)
	bool sensorhub_aod_hwlock_succ = dpu_fb_request_aod_hw_lock(dpufd);
#endif
...
#if defined(CONFIG_DPU_FB_AOD)
	(void)dpu_fb_aod_blank(dpufd, FB_BLANK_POWERDOWN);
...
static int notify_dss_tui_request(void *data, int secure)
{
	...
	if (secure_ctrl->secure_event == DSS_SEC_ENABLE) {
		...
		if (secure_ctrl->secure_blank_flag)
			dpu_fb_secure_blank(dpufd, secure_ctrl);
	}
...
#else
		dpufd->set_reg(dpufd, module_base + MCTL_RCH_OV0_SEL, TUI_SEC_RCH, 4, 24);
#endif
	}
}

static void dpu_invalid_smmu_ptw_cache(struct dpu_fb_data_type *dpufd)
{
	if (dpufd == NULL)
		return;

#if defined(CONFIG_DPU_FB_V501)
	set_reg(dpufd->dss_base + DSS_SMMU_OFFSET + SMMU_SCACHEI_ALL, 0x3, 32, 0);
	set_reg(dpufd->dss_base + DSS_SMMU_OFFSET + SMMU_SCACHEI_ALL, 0x2, 32, 0);
	set_reg(dpufd->dss_base + DSS_SMMU_OFFSET + SMMU_SCACHEI_ALL, 0x5, 32, 0);
	set_reg(dpufd->dss_base + DSS_SMMU_OFFSET + SMMU_SCACHEI_ALL, 0x4, 32, 0);
#endif
}

void dpu_drm_layer_online_config(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req_prev, dss_overlay_t *pov_req)
{
	int i;
	int m;
	int j = 0;
	...
	dpu_check_and_no_retval(!dpufd, ERR, "dpufd is NULL\n");
	DPU_ATRACE_BEGIN("dpu_drm_layer_online_config");

	secure_ctrl = &(dpufd->secure_ctrl);
#if defined(CONFIG_DRMDRIVER)
	compose_mode = (IS_BUILTIN_PANEL) ? ONLINE_COMPOSE_MODE : OVL1_ONLINE_COMPOSE_MODE;
#endif

	if (!pov_req) {
		dpu_drm_layer_online_clear(dpufd, pov_req_prev, NULL, 0);
		DPU_FB_DEBUG("pov_req is NULL!\n");
		DPU_ATRACE_END("dpu_drm_layer_online_config");
		return;
	}
		...
		for (i = 0; i < (int)pov_h_block->layer_nums; i++) {
			...
			if (layer->img.secure_mode == 1) {
				...
				if (layer->img.mmu_enable) {
					...
					dpu_invalid_smmu_ptw_cache(dpufd);
				} else {
	...
	dpu_drm_layer_online_clear(dpufd, pov_req_prev, sec_chn, j);
	...
	const int *seclist, int list_max)
{
	...
	int compose_mode = (IS_BUILTIN_PANEL) ? ONLINE_COMPOSE_MODE : OVL1_ONLINE_COMPOSE_MODE;
#endif
...
static bool check_tui_layer_chn_cfg_ok(struct dpu_fb_data_type *dpufd)
{
	...
	/* use chn V0 for TUI compose except dpuv300 */
...
void dpufb_secure_register(struct platform_device *pdev)
{
	...
	if (dpufd->index == BUILT_IN_PANEL_INDEX) {
		DPU_FB_INFO("It's in builtin panel, don't need register tui");
		return;
	}
	secure_ctrl = &(dpufd->secure_ctrl);
...
#if defined(CONFIG_DP_HDCP)
	secure_ctrl->hdcp13_encrypt_enable = dpu_hdcp13_encrypt_enable;
	secure_ctrl->hdcp13_enable = dpu_hdcp13_enable;
	secure_ctrl->hdcp22_enable = dpu_hdcp22_enable;
	secure_ctrl->hdcp_dpc_sec_en = dpu_hdcp_dpc_sec_en;
	secure_ctrl->hdcp_obs_set = dpu_hdcp_obs_set;
	secure_ctrl->hdcp_int_clr = dpu_hdcp_int_clr;
	secure_ctrl->hdcp_int_mask = dpu_hdcp_int_mask;
	secure_ctrl->hdcp_cp_irq = dpu_hdcp_cp_irq;
	secure_ctrl->hdcp_reg_get = dpu_hdcp_reg_get;
	secure_ctrl->hdcp_enc_mode = dpu_hdcp_enc_mode;
#endif
