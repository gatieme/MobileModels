...
bool g_is_set_dss_vote_failed = false;
dss_vote_cmd_t g_fail_vote_cmd;
#endif
...
void dpufb_get_timestamp(timeval_compatible *tv)
{
...
uint32_t dpufb_timestamp_diff(timeval_compatible *lasttime, timeval_compatible *curtime)
{
	uint32_t ret;
...
int dpufb_ctrl_on(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->index == PRIMARY_PANEL_IDX)
		dpu_afr_power_on(dpufd);
#endif

#ifdef CONFIG_DPU_FB_V501
	dpufb_pipe_clk_set_underflow_flag(dpufd, false);
	ret = dpufb_wait_pipe_clk_updt(dpufd, true);
	if (ret < 0) {
		DPU_FB_ERR("dpufb_wait_pipe_clk_updt fail, ret=%d\n", ret);
		if (pdata->off)
			pdata->off(dpufd->pdev);
	...
	if (IS_BUILTIN_PANEL) {
		dpufb_rgb_reg_enable(dpufd);
		dpufb_rgb_notify_panel_state(true);
	}
#endif
...
#ifdef CONFIG_DPU_FB_V501
	dpufb_wait_pipe_clk_updt(dpufd, false);
#endif
...
#if defined(CONFIG_POWER_DUBAI_RGB_STATS)
	if (dpufd->index == PRIMARY_PANEL_IDX)
		dpufb_rgb_notify_panel_state(false);
#endif
...
int dpufb_ctrl_dss_voltage_get(struct fb_info *info, void __user *argp)
{
	...
	dss_vote_cmd.dss_voltage_level = (uint32_t)dpe_get_voltage_level(voltage_value);
...
bool check_primary_panel_power_status(struct dpu_fb_data_type *dpufd)
{
	bool primary_power_on = (dpu_fb_get_cur_panel() == DISPLAY_OUTER_PANEL_ID) &&
		dpufd_list[BUILT_IN_PANEL_INDEX]->panel_power_on;
	if ((dpufd->index == AUXILIARY_PANEL_IDX) && primary_power_on) {
		DPU_FB_INFO("fb%d, panel is power on\n", dpufd->index);
		return true;
	}
...
int dpufb_ctrl_dss_voltage_set(struct fb_info *info, void __user *argp)
{
	...
	if (IS_BUILTIN_PANEL && (dpufd->core_clk_upt_support == 0)) {
		DPU_FB_DEBUG("no support core_clk_upt\n");
		return ret;
	}
...
int dpufb_ctrl_dss_vote_cmd_set(struct fb_info *info, const void __user *argp)
{
	...
	if (!status) {
		DPU_FB_DEBUG("fb%d, set dss vote cmd failed, panel power off!\n", dpufd->index);
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
		if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE) {
			g_is_set_dss_vote_failed = true;
			g_fail_vote_cmd = vote_cmd;
		}
#endif
		ret = -EPERM;
		goto err_out;
...
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	if (IS_BUILTIN_PANEL) {
		dpufd->power_on_timestamp = ktime_get();
		DPU_FB_DEBUG("power_on_timestamp = %llu\n", dpufd->power_on_timestamp);
	}
#endif
...
// reset dss vote cmd when hwc set dss vote cmd failed
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
	dpu_fb_reset_dss_vote_and_voltage(dpufd);
#endif

// set the last target frame rate
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
	if ((dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE) &&
		(dpufd->index == PRIMARY_PANEL_IDX) &&
		g_last_target_frm_rate > 0) {
		dfr_init_set_last_frm_rate(dpufd, g_last_target_frm_rate);

		DPU_FB_INFO("fb%d panel_power_on = %d set the last frame rate = %d\n",
			dpufd->index, dpufd->panel_power_on, g_last_target_frm_rate);
		g_last_target_frm_rate = 0;
	}
#endif
...
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
void dpu_fb_reset_dss_vote_and_voltage(struct dpu_fb_data_type *dpufd)
{
	int current_peri_voltage = 0;
	int ret = 0;
	...
	if ((dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE) &&
		(dpufd->index == PRIMARY_PANEL_IDX) &&
		!dpufd->panel_info.bypass_dvfs &&
		g_is_set_dss_vote_failed) {
		if (g_fail_vote_cmd.dss_voltage_level > dpufd->dss_vote_cmd.dss_voltage_level) {
			ret = dpufb_set_dss_vote_voltage(dpufd, g_fail_vote_cmd.dss_voltage_level,
				&current_peri_voltage);
			if (ret >= 0)
				set_dss_vote_cmd(dpufd, g_fail_vote_cmd);
		} else {
			set_dss_vote_cmd(dpufd, g_fail_vote_cmd);
			ret = dpufb_set_dss_vote_voltage(dpufd, g_fail_vote_cmd.dss_voltage_level,
				&current_peri_voltage);
		}
		g_is_set_dss_vote_failed = false;
		DPU_FB_INFO("fb%d, reset dss vote cmd and voltage, ret = %d\n", dpufd->index, ret);
	}
}
#endif
...
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	if (IS_BUILTIN_PANEL) {
		dpu_afr_power_off(dpufd);
		dpufd->power_off_timestamp = ktime_get();
		DPU_FB_DEBUG("power_off_timestamp = %llu\n", dpufd->power_off_timestamp);
	}
#endif
	return ret;
}
...
int dpu_fb_blank_update_tui_status(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->index == BUILT_IN_PANEL_INDEX) {
		DPU_FB_INFO("It's in builtin panel, don't update tui status");
		return 0;
	}
...
int dpu_fb_blank_device(struct dpu_fb_data_type *dpufd, int blank_mode, struct fb_info *info)
{
	int ret = 0;
	...
	if (dpufd->dp_device_srs != NULL) {
		...
		ret = dpu_fb_blank_sub(blank_mode, info);
		if (ret != 0) {
...
void dpu_fb_sdp_fnc_register(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_DEBUG("[fb%d]+\n", dpufd->index);
	DPU_FB_DEBUG("dual dsi present flag:%d!\n", use_dsi1);

	dpufd->open_sub_fnc = use_dsi1 ? dpu_fb_open_sub : NULL;
	dpufd->release_sub_fnc = use_dsi1 ? dpu_fb_release_sub : NULL;
	dpufd->hpd_open_sub_fnc = use_dsi1 ? NULL : dpu_fb_open_sub;
	dpufd->hpd_release_sub_fnc = use_dsi1 ? NULL : dpu_fb_release_sub;
...
void dpu_fb_init_screeninfo_base(struct fb_fix_screeninfo *fix, struct fb_var_screeninfo *var)
{
...
void dpu_fb_init_sreeninfo_by_img_type(struct fb_fix_screeninfo *fix, struct fb_var_screeninfo *var,
...
void dpu_fb_init_sreeninfo_by_panel_info(struct fb_var_screeninfo *var, struct dpu_panel_info *panel_info,
...
void dpu_fb_data_init(struct dpu_fb_data_type *dpufd)
{
	...
	dpufd->ap_aod_skip_frame_flag = 0;
#endif
}
...
void dpu_fb_init_sema(struct dpu_fb_data_type *dpufd)
{
	...
	sema_init(&dpufd->hiace_clear_sem, 1);
#endif
...
#ifdef MIPI_RESOURCE_PROTECTION
	spin_lock_init(&dpufd->mipi_resource_lock);
#endif
	spin_lock_init(&dpufd->underflow_lock);
}

int dpu_fb_registe_callback(struct dpu_fb_data_type *dpufd, struct fb_var_screeninfo *var,
	struct dpu_panel_info *panel_info)
{
	...
	if (IS_BUILTIN_PANEL) {
		if (dpufd->index == PRIMARY_PANEL_IDX)
			dpu_fb_offlinecomposer_init(var, panel_info);
