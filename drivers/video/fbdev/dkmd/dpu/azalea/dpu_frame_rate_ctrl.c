...
static void dpu_dfr_notice_handle_func(struct work_struct *work)
{
...
static void dpu_dfr_set_end_state(struct dpu_fb_data_type *dpufd)
{
...
static void dpu_dfr_delay_work(struct work_struct *work)
{
	...
	if (dfr_succ) {
		if (is_mipi_video_panel(dpufd)) {
			...
			if (dpufd->panel_info.dfr_info.oled_info.oled_type == PANEL_OLED_LTPO)
				dfr_switch_cur_te_rate(dpufd, frm_rate_ctrl->target_frame_rate);
#endif
...
struct mipi_panel_info *get_mipi_ctrl(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->panel_info.frm_rate_ctrl.target_frame_rate == FPS_60HZ
#if defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	    || (dpufd->panel_info.adaptive_frm_rate.is_support_ltpo &&
			dpufd->panel_info.frm_rate_ctrl.target_frame_rate == FPS_120HZ)
#endif
	) {
		...
		if (dpufd->panel_info.dsi_bit_clk_upt_support &&
			dpufd->panel_info.mipi_updt.dsi_bit_clk_upt != dpufd->panel_info.mipi.dsi_bit_clk_upt)
				mipi->dsi_bit_clk_upt = dpufd->panel_info.mipi.dsi_bit_clk_upt;
	}
	return mipi;
}
...
void dfr_status_convert_on_isr_vstart(struct dpu_fb_data_type *dpufd)
{
	...
	if (!dpufd || (!IS_BUILTIN_PANEL) || !is_lcd_dfr_support(dpufd))
		return;
...
static bool check_target_fps(struct dpu_fb_data_type *dpufd)
{
	...
	if (((current_frame_rate < target_frame_rate) && (target_frame_rate ==(int32_t)(dpufd->ov_req_prev.frameFps))) ||
	...
	char __iomem *mipi_dsi_base, struct frame_rate_ctrl *frm_rate_ctrl)
{
	...
	if (pinfo->reload_support)
		/* set reload shadow req */
		set_reg(mipi_dsi_base + VID_SHADOW_CTRL, 0x1, 1, 8);
...
static bool wait_ldi_vstate_idle(struct dpu_fb_data_type *dpufd, timeval_compatible *tv0)
{
	bool ret = false;
	uint32_t ldi_vstate;
...
static int mipi_dsi_frm_rate_set_reg(struct dpu_fb_data_type *dpufd,
	struct frame_rate_ctrl *frm_rate_ctrl, timeval_compatible *tv0)
{
	char __iomem *mipi_dsi_base = NULL;
	mipi_dsi_base = get_mipi_dsi_base(dpufd);
...
exit:
	...
	dpu_dfr_end_process(dpufd, dfr_succ);
...
int mipi_dsi_frm_rate_ctrl(struct dpu_fb_data_type *dpufd, int frm_rate)
{
	...
	dpu_check_and_return((!IS_BUILTIN_PANEL), 0,
...
{
	struct frame_rate_ctrl *frm_rate_ctrl = NULL;
	uint32_t tmp_fps = FPS_60HZ;
	struct mipi_panel_info *mipi = NULL;

	dpu_check_and_no_retval(!dpufd, ERR, "dpufd is NULL\n");
	dpu_check_and_no_retval((!IS_BUILTIN_PANEL), DEBUG, "fb%d is not primary panel\n",
		dpufd->index);
	...
	if (dpufd->panel_info.dfr_info.oled_info.oled_type == PANEL_OLED_LTPO) {
		if (dpufd->panel_info.frm_rate_ctrl.registered == 0) {
			tmp_fps = FPS_120HZ;
			dpufd->panel_info.fps_updt = tmp_fps;
		} else {
			tmp_fps = dpufd->panel_info.fps_updt;
			dfr_switch_cur_te_rate(dpufd, tmp_fps);
		}
		DPU_FB_INFO("ltpo need init last frame rate, cur te %d frame rate %d\n",
			tmp_fps, dpufd->panel_info.adaptive_frm_rate.cur_frm_rate);
	}
	dpufd->panel_info.adaptive_frm_rate.cur_frm_rate = tmp_fps;
#endif
	frm_rate_ctrl = &(dpufd->panel_info.frm_rate_ctrl);

	frm_rate_ctrl->target_frame_rate = (int)tmp_fps;
	frm_rate_ctrl->current_frame_rate = (int)tmp_fps;
	frm_rate_ctrl->status = FRM_UPDT_DONE;
	dpufd->panel_info.fps = tmp_fps;
	mipi = get_mipi_ctrl(dpufd);
	frm_rate_ctrl->current_hline_time = mipi->hline_time;
	frm_rate_ctrl->current_vfp = mipi->vfp;
	...
	if (dpufd->dfr_create_singlethread == false) {
		...
		dpu_check_and_no_retval(!dpufd->dfr_notice_wq, ERR, "fb%d, create dfr notice workqueue failed!\n",
			dpufd->index);
		INIT_WORK(&dpufd->dfr_notice_work, dpu_dfr_notice_handle_func);

		dpufd->dfr_delay_wq = alloc_ordered_workqueue("dfr_delay_work", WQ_HIGHPRI | WQ_MEM_RECLAIM);
		dpu_check_and_no_retval(!dpufd->dfr_delay_wq, ERR, "fb%d, create dfr delay workqueue failed!\n",
			dpufd->index);
		INIT_WORK(&dpufd->dfr_delay_work, dpu_dfr_delay_work);
