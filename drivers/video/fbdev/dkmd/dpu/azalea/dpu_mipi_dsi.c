...
#include "product/attrs/dpu_mipi_attrs.h"
#include "dpu_trace.h"
...
static void dpu_wait_dfr_end(struct dpu_fb_data_type *dpufd)
{
...
static int dpu_check_dfr_status(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	int ret = 0;
	uint64_t curr_time;
...
int dpufb_wait_for_mipi_resource_available(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	...
	DPU_ATRACE_BEGIN("dpufb_wait_for_mipi_resource_available");
#if defined(CONFIG_DPU_DISPLAY_DFR)
	if (is_mipi_video_panel(dpufd) && is_lcd_dfr_support(dpufd)) {
		if (dpu_check_dfr_status(dpufd, pov_req) < 0) {
			DPU_ATRACE_END("dpufb_wait_for_mipi_resource_available");
			return -EINVAL;
		}
...
static int mipi_dsi_primary_clk_irq_setup(struct dpu_fb_data_type *dpufd, struct platform_device *pdev)
{
	...
	defined(CONFIG_DPU_FB_330) || defined(CONFIG_DPU_FB_V501)
...
static int mipi_dsi_pclk_pctrl_irq_setup(struct dpu_fb_data_type *dpufd, struct platform_device *pdev)
{
	int ret;

	dpufd->dss_pclk_pctrl_clk = devm_clk_get(&pdev->dev, dpufd->dss_pclk_pctrl_name);
	if (IS_ERR(dpufd->dss_pclk_pctrl_clk)) {
		ret = PTR_ERR(dpufd->dss_pclk_pctrl_clk);
		return ret;
	}
	ret = clk_set_rate(dpufd->dss_pclk_pctrl_clk, DEFAULT_PCLK_PCTRL_RATE);
	dpu_check_and_return((ret < 0), -EINVAL, ERR,
		"fb%d dss_pclk_pctrl clk_set_rate(%lu) failed, error=%d!\n",
		dpufd->index, DEFAULT_PCLK_PCTRL_RATE, ret);

	DPU_FB_INFO("dss_pclk_pctrl_clk:[%lu]->[%lu]\n",
		DEFAULT_PCLK_PCTRL_RATE, clk_get_rate(dpufd->dss_pclk_pctrl_clk));
