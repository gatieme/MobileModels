	...
	struct dpu_panel_info *pinfo, const struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	if(pinfo->mipi_swing_adjust_support) {
		DPU_FB_DEBUG("add tx_swing_adjust = 0x%x", pinfo->mipi_swing_adjust);
		mipi_config_phy_test_code(mipi_dsi_base, MIPIDSI_PHY_TST_AFE_CONFIGURATION_12, pinfo->mipi_swing_adjust);
	}
#endif
}
	...
	struct mipi_dsi_timing *timing)
{
	...
	if (is_lcd_dfr_support(dpufd)) {
		...
		|| (pinfo->adaptive_frm_rate.is_support_ltpo && dpufd->panel_info.fps == FPS_120HZ)
#endif
	)
...
static int mipi_dsi_on_sub2(struct dpu_fb_data_type *dpufd, char __iomem *mipi_dsi_base)
{
	...
	if (pinfo->reload_support)
		/* set reload mode */
		set_reg(mipi_dsi_base + VID_SHADOW_CTRL, 0x101, 32, 0);
	...
	struct dpu_fb_data_type *dpufd, struct platform_device *pdev)
{
	...
	if (!panel_next_bypass_powerdown_ulps_support(pdev) && !dpu_check_power_mode(dpufd)) {
		DPU_FB_INFO("%d not support ulps\n", dpufd->index);
		return;
	}
	...
	struct dpu_fb_data_type *dpufd)
{
	...
	dpu_check_and_return((!IS_BUILTIN_PANEL), 0, ERR, "fb%d, not support!\n", dpufd->index);
