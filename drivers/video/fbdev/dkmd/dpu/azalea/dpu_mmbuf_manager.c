...
DEFINE_SEMAPHORE(dpu_mmbuf_sem);
...
static bool mmbuf_reserved_alloc(struct dpu_fb_data_type *dpufd, dss_mmbuf_t *mmbuf_info)
{
	...
	if (dss_mmbuf_reserved_info[mmbuf_info->owner].addr != 0) {
		...
		mmbuf_info->addr = dpu_mmbuf_alloc(dpufd->mmbuf_gen_pool, mmbuf_info->size);
...
void dpu_mmbuf_mmbuf_free_all(struct dpu_fb_data_type *dpufd)
{
	...
	list_for_each_entry_safe(node, _node_, g_mmbuf_list, list_node) {
		if ((node->addr > 0) && (node->size > 0)) {
			...
			DPU_FB_DEBUG("dpu_mmbuf_free, addr=0x%x, size=%d\n", node->addr, node->size);
		}
...
dss_mmbuf_info_t *dpu_mmbuf_info_get(struct dpu_fb_data_type *dpufd, int idx)
{
	...
	if ((idx < 0) || (idx >= DPU_CMDLIST_DATA_MAX)) {
...
void dpu_mmbuf_info_clear(struct dpu_fb_data_type *dpufd, int idx)
{
	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		if (mmbuf_info->mm_used[i] == 1) {
			dpu_mmbuf_free(g_mmbuf_gen_pool, mmbuf_info->mm_base[i], mmbuf_info->mm_size[i]);
	...
	int *use_comm_mmbuf, dss_mmbuf_t *offline_mmbuf, bool has_rot)
{
	...
	if (display_id == PRIMARY_PANEL_IDX || display_id == BUILT_IN_PANEL_INDEX) {
...
void dpu_mmbuf_info_get_online(struct dpu_fb_data_type *dpufd)
{
	...
	tmp = (dpufd->frame_count + 1) % DPU_CMDLIST_DATA_MAX;
	dpu_mmbuf_info_clear(dpufd, tmp);

	tmp = dpufd->frame_count % DPU_CMDLIST_DATA_MAX;
	dpufd->mmbuf_info = &(dpufd->mmbuf_infos[tmp]);
}

int dpu_mmbuf_reserved_size_query(struct fb_info *info, void __user *argp)
{
	int ret;
...
int dpu_mmbuf_request(struct fb_info *info, void __user *argp)
{
	int ret;
	...
	if (unlikely((mmbuf_info.size == 0) || (mmbuf_info.size > mmbuf_size_max) ||
	...
	if (ret != 0) {
		DPU_FB_ERR("fb%d, copy to user failed!ret=%d\n", dpufd->index, ret);
		dpu_mmbuf_free(dpufd->mmbuf_gen_pool, mmbuf_info.addr, mmbuf_info.size);
		return -EINVAL;
	}
...
int dpu_mmbuf_release(struct fb_info *info, const void __user *argp)
{
	int ret;
	struct dpu_fb_data_type *dpufd = NULL;
	dss_mmbuf_t mmbuf_info;
	...
	if (unlikely((mmbuf_info.addr == 0) || (mmbuf_info.size == 0))) {
...
int dpu_mmbuf_free_all(struct fb_info *info, const void __user *argp)
{
	struct dpu_fb_data_type *dpufd = NULL;
