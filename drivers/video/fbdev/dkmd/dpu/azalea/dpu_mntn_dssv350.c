...
static struct noc_err_para_s dss_noc_para[] = {
	...
	{
		.masterid = (u32)SOC_DSS_RD_0_DSS_ATGM_MID,
		.targetflow = MNTN_DEFAULT_TARGETFLOW,
		.bus = NOC_ERRBUS_VIVO,
	},
};
...
static void dss_media_subsys_power_up(struct dpu_fb_data_type *dpufd)
{
	...
	udelay(100);
	/* step 1: module mem sd off */
	outp32(dpufd->pctrl_base + PERI_CTRL102, 0x10000000);
	udelay(1);
	/* step 1.1: module unrst */
	outp32(dpufd->peri_crg_base + PERRSTDIS5, 0x00040000);
	/* step 2: module clk enable */
	outp32(dpufd->peri_crg_base + PEREN6, 0x5C002028);
	outp32(dpufd->peri_crg_base + PEREN4, 0x00000040);
	outp32(dpufd->peri_crg_base + PEREN7, 0x00000040);
	udelay(1);
	/* step 3: module clk disable */
	outp32(dpufd->peri_crg_base + PERDIS6, 0x5C002028);
	outp32(dpufd->peri_crg_base + PERDIS4, 0x00000040);
	outp32(dpufd->peri_crg_base + PERDIS7, 0x00000040);
	udelay(1);
	/* step 4: module iso disable */
	outp32(dpufd->peri_crg_base + ISODIS, 0x00000040);
	/* step 5: memory rempair */
	ret = inp32(dpufd->sctrl_base + PERSTATUS3);
	udelay(400);
	DSS_MNTN_INFO("read sctrl_base PERSTATUS3 = 0x%x\n", ret);
	/* step 6: module unrst */
...
static void dss_vivobus_power_up(struct dpu_fb_data_type *dpufd)
{
	 ...
	 * step 2: module clk enable
	 */
	outp32(dpufd->media_crg_base + MEDIA_CLKDIV9, 0x00180018);
	outp32(dpufd->media_crg_base + MEDIA_PEREN0, 0x08040040);
	outp32(dpufd->media_crg_base + MEDIA_PEREN1, 0x00000048);
	outp32(dpufd->media_crg_base + MEDIA_PEREN0_GENERAL_SEC, 0x00000003);
	udelay(1);
	/* step 3: module clk disable */
	outp32(dpufd->media_crg_base + MEDIA_PERDIS0, 0x08040040);
	outp32(dpufd->media_crg_base + MEDIA_PERDIS1, 0x00000048);
	outp32(dpufd->media_crg_base + MEDIA_PERDIS0_GENERAL_SEC, 0x00000003);
	udelay(1);
	/* step 4: module iso disable
	 * step 5: memory rempair
	 * step 6: module unrst
	 */
	outp32(dpufd->media_crg_base + MEDIA_PERRSTDIS_ISP_SEC, 0x00000100);
	outp32(dpufd->media_crg_base + PERRSTDIS_GENERAL_SEC, 0x00000006);
	/* step 7: module clk enable */
	outp32(dpufd->media_crg_base + MEDIA_PEREN0, 0x08040040);
	outp32(dpufd->media_crg_base + MEDIA_PEREN1, 0x00000048);
	outp32(dpufd->media_crg_base + MEDIA_PEREN0_GENERAL_SEC, 0x00000003);
	/* step 8: bus idle clear */
	outp32(dpufd->pmctrl_base + NOC_POWER_IDLEREQ, 0x40000000);
...
static void dss_to_power_up(struct dpu_fb_data_type *dpufd)
{
	uint32_t ret;
	/* step 1: module mtcmos on
	 * step 1.1: module unrst
	...
	/* step 2: module clk enable */
	outp32(dpufd->media_crg_base + MEDIA_CLKDIV9, 0xC080C080);
	outp32(dpufd->media_crg_base + MEDIA_PEREN0, 0x0008C000);
	outp32(dpufd->media_crg_base + MEDIA_PEREN1, 0x00660000);
	outp32(dpufd->media_crg_base + MEDIA_PEREN2, 0x0000003F);
	udelay(1);
	/* step 3: module clk disable */
	outp32(dpufd->media_crg_base + MEDIA_PERDIS0, 0x0008C000);
