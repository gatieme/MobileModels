...
#if !defined(CONFIG_DPU_CMDLIST_LAST_USED) || defined(CONFIG_DPU_FB_V320) || defined(CONFIG_DPU_FB_V330)
static uint32_t dpu_get_cmdlist_chnidxs_by_poolidxs(uint32_t poolidxs)
{
	int i;
...
static uint32_t dpu_cmdlist_get_cmdlist_idxs_temp(dss_overlay_t *pov_req)
{
	uint32_t i;
	uint32_t k;
	uint32_t m;
	...
	for (m = 0; m < pov_req->ov_block_nums; m++) {
		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			cmdlist_idx = dpu_get_cmdlist_idx_by_chnidx((uint32_t)(layer->chn_idx));
			if (cmdlist_idx < 0)
				return 0;

			cmdlist_idxs_temp |= (1 << (uint32_t)cmdlist_idx);
		}
	}
	...
	if (pov_req->wb_enable == 1) {
		for (k = 0; k < pov_req->wb_layer_nums; k++) {
			...
			cmdlist_idx = dpu_get_cmdlist_idx_by_chnidx((uint32_t)(wb_layer->chn_idx));
			if (cmdlist_idx < 0)
				return 0;
	...
	if (no_ovl_idx == false) {
		cmdlist_idx = dpu_get_cmdlist_idx_by_ovlidx(pov_req->ovl_idx);
		if (cmdlist_idx < 0)
			return 0;
	...
	uint32_t value, uint8_t bw, uint8_t bs)
{
	...
	if (node->item_index >= (node->item_len / sizeof(struct cmd_item))) {
		DPU_FB_ERR("node item_index=%d is larger than %lu!\n",
			node->item_index,
	...
	int pending, int reserved)
{
	...
	for (i = 0; i < DPU_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1) {
			...
			node->list_header->flag.bits.id = (uint32_t)id;
...
int dpu_cmdlist_del_all_node(struct list_head *cmdlist_heads)
{
	...
	if (cmdlist_heads->next == cmdlist_heads || cmdlist_heads->prev == cmdlist_heads)
		return -EINVAL;
...
static void dpu_cmdlist_config_mif_reset(struct dpu_fb_data_type *dpufd,
...
static char __iomem *get_ldi_base(struct dpu_fb_data_type *dpufd)
{
	char __iomem *ldi_base = NULL;
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_idxs)
{
	...
	dpu_check_and_no_retval(!dpufd, ERR, "dpufd or pov_req is NULL!\n");
	dpu_check_and_no_retval(!pov_req, ERR, "dpufd or pov_req is NULL!\n");
...
#endif
static void dpu_mctl_ctl_clear(struct dpu_fb_data_type *dpufd, int mctl_idx)
{
	char __iomem *tmp_base = NULL;
	...
	if (tmp_base) {
		...
		if (dpufb_mctl_clear_ack_timeout(tmp_base))
...
static void get_dpu_cmdlist_config(char __iomem *cmdlist_base, uint32_t *cmdlist_idxs_temp,
	uint32_t *start_sel_temp)
{
	...
	for (i = 0; i < DPU_CMDLIST_MAX; i++) {
		if ((*cmdlist_idxs_temp & 0x1) == 0x1) {
			cmdlist_chnidx = dpu_get_cmdlist_chnidx(i);
			ints_temp = inp32(cmdlist_base + CMDLIST_CH0_INTS + cmdlist_chnidx * offset);
			start_sel = inp32(cmdlist_base + CMDLIST_CH0_CTRL + cmdlist_chnidx * offset);

			if (((ints_temp & 0x2) == 0x2) && ((start_sel & 0x1c) == 0)) {
				set_reg(cmdlist_base + CMDLIST_CH0_CTRL + cmdlist_chnidx * offset, 0x6, 3, 2);
				*start_sel_temp |= (1 << i);
			}
		}
...
static void get_cmdlist_idxs_temp(char __iomem *cmdlist_base, int mctl_idx, uint32_t *cmdlist_idxs_temp,
		...
		*cmdlist_idxs_temp = cmdlist_idxs;
		for (i = 0; i < DPU_CMDLIST_MAX; i++) {
			if ((*cmdlist_idxs_temp & 0x1) == 0x1) {
				cmdlist_chnidx = dpu_get_cmdlist_chnidx(i);
				set_reg(cmdlist_base + CMDLIST_CH0_CTRL + cmdlist_chnidx * offset, 0x6, 3, 2);
				set_reg(cmdlist_base + CMDLIST_CH0_CTRL + cmdlist_chnidx * offset, 0x0, 1, 0);
			}
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_idxs)
{
	...
	DPU_FB_DEBUG("fb%d, +.\n", dpufd->index);
	dpu_check_and_no_retval(!pov_req, ERR, "pov_req is NULL point!\n");
	dpu_check_and_no_retval((cmdlist_idxs == 0), INFO, "cmdlist_idxs==0\n");

	cmdlist_base = dpufd->dss_base + DSS_CMDLIST_OFFSET;
	mctl_idx = pov_req->ovl_idx;
	dpu_check_and_no_retval(((mctl_idx < 0) || (mctl_idx >= DSS_MCTL_IDX_MAX)),
		ERR, "mctl_idx=%d is invalid\n", mctl_idx);
#if defined(CONFIG_DPU_FB_V320)
	...
	if (g_chip_id == DPUV320K_CHIP_ID) {
		dpu_mctl_ctl_clear(dpufd, mctl_idx);
#if defined(CONFIG_DPU_FB_V320)
		up(&dpufd->hiace_clear_sem);
#endif
		DPU_FB_INFO("mif_reset clear over\n");
		return;
	}

	cmdlist_idxs_temp = DPU_CMDLIST_IDXS_MAX;
	get_dpu_cmdlist_config(cmdlist_base, &cmdlist_idxs_temp, &start_sel_temp);

	/* set cmdlist chn soft reset */
	cmdlist_chnidxs = dpu_get_cmdlist_chnidxs_by_poolidxs(cmdlist_idxs);
	set_reg(cmdlist_base+CMDLIST_SWRST, cmdlist_chnidxs, 32, 0);

	/* MCTL_CTL_CLEAR */
	dpu_mctl_ctl_clear(dpufd, mctl_idx);

	dpu_cmdlist_config_mif_reset(dpufd, pov_req, cmdlist_idxs, mctl_idx);

	cmdlist_idxs_temp = start_sel_temp;
	get_cmdlist_idxs_temp(cmdlist_base, mctl_idx, &cmdlist_idxs_temp, cmdlist_idxs);
#if defined(CONFIG_DPU_FB_V320)
	up(&dpufd->hiace_clear_sem);
#endif
}
			...
			if (pov_req && pov_req->wb_enable) {
				if (dpufd->index == MEDIACOMMON_PANEL_IDX) {
					if (dpufd->media_common_cmdlist_data)
						dpu_dump_cmdlist_one_node(&(dpufd->media_common_cmdlist_data->cmdlist_head_temp[i]), i);
				} else {
					if (dpufd->cmdlist_data_tmp[wb_compose_type])
						dpu_dump_cmdlist_one_node(&(dpufd->cmdlist_data_tmp[wb_compose_type]->cmdlist_head_temp[i]), i);
				}
			} else {
				if (dpufd->cmdlist_data)
...
static void dpu_dump_cmdlist_content_for_mdfx(struct dss_cmdlist_node *node, struct file *fd)
{
	char tmp_str[32] = {0};
	char newline_str[] = "\n";
	int ret;
...
static void dpu_dump_cmdlist_one_node_for_mdfx(struct list_head *cmdlist_head, uint32_t cmdlist_idx, struct file *fd)
{
	struct dss_cmdlist_node *node = NULL;
	struct dss_cmdlist_node *_node_ = NULL;
	uint32_t count;
	...
	list_for_each_entry_safe(node, _node_, cmdlist_head, list_node) {
		...
		dpu_dump_cmdlist_content_for_mdfx(node, fd);
...
int dpu_cmdlist_dump_all_node_for_mdfx(struct dpu_fb_data_type *dpufd, uint32_t cmdlist_idxs, struct file *fd)
{
	int i;
	uint32_t cmdlist_idxs_temp;
	...
	for (i = 0; i < DPU_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1)
			dpu_dump_cmdlist_one_node_for_mdfx(&(dpufd->cmdlist_data->cmdlist_head_temp[i]), i, fd);
		cmdlist_idxs_temp = cmdlist_idxs_temp >> 1;
	}
	...
	for (i = 0; i < DPU_CMDLIST_MAX; i++) {
		if ((cmdlist_idxs_temp & 0x1) == 0x1) {
			if (pov_req && pov_req->wb_enable) {
				if (dpufd->index == MEDIACOMMON_PANEL_IDX) {
					if (dpufd->media_common_cmdlist_data)
						dpu_cmdlist_del_all_node(&(dpufd->media_common_cmdlist_data->cmdlist_head_temp[i]));
				} else {
					if (dpufd->cmdlist_data_tmp[wb_compose_type])
						dpu_cmdlist_del_all_node(&(dpufd->cmdlist_data_tmp[wb_compose_type]->cmdlist_head_temp[i]));
				}
			} else {
				if (dpufd->cmdlist_data)
