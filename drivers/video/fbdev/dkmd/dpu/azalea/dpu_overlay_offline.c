...
#if defined(CONFIG_DPU_FB_V410) || defined(CONFIG_DPU_FB_V320) || \
	defined(CONFIG_DPU_FB_V330) || defined(CONFIG_DPU_FB_V501)
static void dpu_tuning_offline_clk_rate(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->need_tuning_clk) {
		...
		if (offline_clk_rate >= target_clk_rate) {
			...
			(void)dpufb_set_mmbuf_clk_value(dpufd, dpufb_get_mmbuf_clk_by_pri_clk(offline_clk_rate));
#endif
		}
...
static void dpu_recovery_online_clk_rate(struct dpu_fb_data_type *dpufd)
{
	uint64_t online_clk_rate;
	uint64_t external_clk_rate;
	uint64_t target_clk_rate;
...
#ifdef ADJUST_MMBUF_BY_PRI_CLK
		(void)dpufb_set_mmbuf_clk_value(dpufd, dpufb_get_mmbuf_clk_by_pri_clk(target_clk_rate));
#endif
...
static uint32_t dpu_get_timeout_interval(void)
{
...
static int32_t dpu_get_wb_type(dss_overlay_t *pov_req)
{
	int32_t wb_type = -1;
...
static int dpu_wait_interrupt_event(struct dpu_fb_data_type *dpufd, int32_t wb_type)
{
	uint32_t timeout_interval = dpu_get_timeout_interval();
	...
	while (times < 50) {
		/*lint -e666*/
		/* The performance is insufficient. After fb0 is powered off, copybit timeout
		 * fb0 is null or fb0 panel power off, add increment timeout
		 */
		if (fb0 == NULL || !fb0->panel_power_on){
			ret = wait_event_interruptible_timeout(dpufd->cmdlist_info->cmdlist_wb_wq[wb_type], /*lint !e578*/
				...
				msecs_to_jiffies(timeout_interval + DSS_OFFLINE_WB_INCREMENT_TIMEOUT));
		}else {
			ret = wait_event_interruptible_timeout(dpufd->cmdlist_info->cmdlist_wb_wq[wb_type], /*lint !e578*/
				(dpufd->cmdlist_info->cmdlist_wb_done[wb_type] == 1), /* wb type status is true */
...
static int dpu_check_two_wch_wb_finish(
	...
	int mctl_idx)
{
	...
	ret = dpu_wait_interrupt_event(dpufd, WB_TYPE_WCH0);

	ret1 = dpu_wait_interrupt_event(dpufd, WB_TYPE_WCH1);

	dpufb_get_timestamp(&tv1);

	ret_cmdlist_state = dpu_cmdlist_check_cmdlist_state(dpufd, (cmdlist_idxs | last_cmdlist_idxs));
	if ((ret <= 0) || (ret1 <= 0) || (ret_cmdlist_state < 0)) {
		...
		dpu_remove_mctl_mutex(dpufd, last_mctl_idx, last_cmdlist_idxs);
		/* remove mctl ch & ov */
		dpu_remove_mctl_mutex(dpufd, mctl_idx, cmdlist_idxs);
		ret = 0;
	}
...
static int dpu_check_wch_wb_finish(
	struct dpu_fb_data_type *dpufd,
	...
	int mctl_idx)
{
	...
	wb_type = dpu_get_wb_type(pov_req);
	if (wb_type < 0) {
	...
	ret = dpu_wait_interrupt_event(dpufd, wb_type);
...
static uint32_t dpu_get_block_info_layers_channels(
	...
	uint32_t block_nums)
{
	...
	if (block_nums > DPU_OV_BLOCK_NUMS)
		return 0;
...
static bool dpu_offline_chn_include_online_chn(
	...
	uint32_t offline_block_nums)
{
	...
	offline_chn_idx = dpu_get_block_info_layers_channels(offline_block_infos, offline_block_nums);
	online_pre_chn_idx = dpu_get_block_info_layers_channels(
	...
	online_pre_pre_chn_idx = dpu_get_block_info_layers_channels(
...
static void dpu_offline_composer_on(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpu_fb_get_cur_panel()) {
		dpufd_primary = dpufd_list[BUILT_IN_PANEL_INDEX];
	} else {
...
static void dpu_offline_composer_off(struct dpu_fb_data_type *dpufd)
{
	struct fb_info *fbi = NULL;
	struct dpu_fb_data_type *dpufd_primary = NULL;
	int new_refcount;
...
static int dpu_add_clear_module_reg_node(
	struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req,
	int cmdlist_idxs,
	bool enable_cmdlist,
	int *use_comm_mmbuf)
{
	...
	if ((cmdlist_idxs < 0) || (cmdlist_idxs >= DPU_CMDLIST_IDXS_MAX)) {
	...
	if (ret != 0) {
		DPU_FB_ERR("dpu_module_init failed! ret=%d\n", ret);
		return ret;
	}

	dpu_prev_module_set_regs(dpufd, pov_req, cmdlist_idxs, enable_cmdlist, use_comm_mmbuf);

	return 0;
}

static void dpu_offline_play_free_data(dss_overlay_t *pov_req)
{
...
static dss_overlay_t *dpu_offline_play_alloc_data(void)
{
	dss_overlay_t *pov_req = NULL;

	pov_req = (dss_overlay_t *)kmalloc(sizeof(dss_overlay_t), GFP_ATOMIC);
	if (!pov_req) {
		DPU_FB_INFO("pov_req gfp_atomic alloc failed!\n");
		pov_req = (dss_overlay_t *)kmalloc(sizeof(dss_overlay_t), GFP_KERNEL);
		if (!pov_req) {
			DPU_FB_ERR("pov_req gfp_kernel alloc failed!\n");
			return NULL;
		}
	}
	if (pov_req)
	...
	bool reset)
{
	...
	dpu_mmu_lock_clear(dpufd);
#endif
	dpu_drm_layer_offline_clear(dpufd, pov_req);
	...
	if (enable_cmdlist) {
		...
		if (reset) {
			...
			dpu_cmdlist_config_reset(dpufd, pov_req, cmdlist_idxs);
		}
		dpu_cmdlist_del_node(dpufd, pov_req, cmdlist_idxs);
		dpu_mmbuf_info_clear(dpufd, 0);
	}
	dpu_offline_play_free_data(pov_req);
}

static int dpu_get_ov_data_from_user(struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req, const void __user *argp)
{
	int ret;
	dss_overlay_block_t *pov_h_block_infos = NULL;
	uint32_t ov_block_size;
	size_t len = sizeof(dss_overlay_block_t);
	...
	ov_block_size = pov_req->ov_block_nums * (uint32_t)len;
	pov_h_block_infos = (dss_overlay_block_t *)kmalloc(ov_block_size, GFP_ATOMIC);
	if (!pov_h_block_infos) {
		DPU_FB_INFO("fb%u, pov_h_block_infos gfp_atomic alloc failed!\n", dpufd->index);
		pov_h_block_infos = (dss_overlay_block_t *)kmalloc(ov_block_size, GFP_KERNEL);
		if (!pov_h_block_infos) {
			DPU_FB_ERR("pov_h_block_infos gfp_kernel alloc failed!\n");
			pov_req->ov_block_infos_ptr = 0;
			return -EINVAL;
		}
	}
	...
	if (ret != 0) {
		DPU_FB_ERR("fb%u, dpu_check_userdata failed!\n", dpufd->index);
		kfree(pov_h_block_infos);
		pov_h_block_infos = NULL;
		pov_req->ov_block_infos_ptr = 0;
		return -EINVAL;
...
static bool dpu_check_csc_config_needed(dss_overlay_t *pov_req_h_v)
{
...
static int dpu_vactive0_start_wait(
	struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req,
	uint32_t use_comm_mmbuf)
{
	...
	void_unused(dpufd);
	void_unused(pov_req);
	timeout_interval = dpu_get_timeout_interval();
	if (use_comm_mmbuf & (1 << DSS_OVL0)) {
#ifdef CONFIG_DPU_FB_V346
		if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE &&
			dpu_fb_get_cur_panel())
			online_dpufd = dpufd_list[BUILT_IN_PANEL_INDEX];
		else
...
static int dpu_get_mctl_idx(dss_overlay_t *pov_req)
{
	int mctl_idx;

	if (dpu_get_wb_type(pov_req) == WB_TYPE_WCH2)
		mctl_idx = DSS_MCTL5;
	else
...
static bool dpu_get_parallel_compose_flag(
	struct dpu_fb_data_type *dpufd,
	uint32_t wb_compose_type)
{
...
static int dpu_ov_layers_compose(struct dpu_fb_data_type *dpufd, dss_overlay_block_t *pov_h_v_block,
...
static int dpu_wb_ov_layers_compose(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	dss_rect_t *wb_ov_block_rect, bool last_block, struct dpu_ov_compose_flag ov_compose_flag)
{
	uint32_t i;
	int ret;
	dss_wb_layer_t *wb_layer = NULL;
	...
	for (i = 0; i < pov_req_h_v->wb_layer_nums; i++) {
		...
		ret = dpu_wb_compose_handler(dpufd, wb_layer, wb_ov_block_rect, last_block, ov_compose_flag);
		if (ret != 0) {
			DPU_FB_ERR("fb%u, dpu dss write back config failed! ret=%d\n", dpufd->index, ret);
			return ret;
		}

		ret = dpu_get_wb_type(pov_req);
		if (ret < 0) {
			DPU_FB_ERR("dpu_get_wb_type error");
			return ret;
		}
		type = (uint32_t)ret;
		ret = dpu_wb_ch_module_set_regs(dpufd, type, wb_layer, ov_compose_flag.enable_cmdlist);
		if (ret != 0)
			return ret;
	}
...
static int dpu_offline_play_h_block_handle(
	struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req,
	...
	bool last_block)
{
	...
	struct dpu_ov_compose_rect ov_compose_rect = { &pov_req->wb_layer_infos[0].dst_rect, wb_ov_block_rect,
		&clip_rect, &aligned_rect };
	struct dpu_ov_compose_flag ov_compose_flag = { false, false, true, false };
	...
	ov_compose_flag.csc_needed = dpu_check_csc_config_needed(pov_req_h_v);
	...
	ret = dpu_ov_layers_compose(dpufd, pov_h_v_block, &ov_compose_rect, &ov_compose_flag);
	if (ret != 0)
		return ret;

	if (h_block_index == 0) {
		ret = dpu_wb_ov_layers_compose(dpufd, pov_req, wb_ov_block_rect, last_block, ov_compose_flag);
		if (ret != 0)
			return ret;
	}

	ret = dpu_mctl_ov_config(dpufd, pov_req, pov_req_h_v->ovl_idx, ov_compose_flag.has_base, false);
...
static int dpu_ov_block_config(
	struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req,
	dss_overlay_block_t *pov_h_block_infos)
{
	...
	for (m = 0; m < pov_req->ov_block_nums; m++) {
		...
		if ((ret != 0) || (block_num == 0) || (block_num >= DPU_CMDLIST_BLOCK_MAX)) {
		...
		for (k = 0; k < block_num; k++) {
			...
			ret = dpu_offline_play_h_block_handle(dpufd, pov_req, &wb_ov_block_rect, m, last_block);
			if (ret != 0) {
...
static int dpu_check_panel_status_and_composer_type(uint32_t fd_index, dss_overlay_t *pov_req)
{
	int32_t wb_type;
	struct dpu_fb_data_type *primary_dpufd = NULL;
	DPU_FB_INFO("dpu_check_panel_status_and_composer_type fd index %d\n", fd_index);

	if (dpu_fb_get_cur_panel()) {
		primary_dpufd = dpufd_list[BUILT_IN_PANEL_INDEX];
	} else {
	...
	if ((wb_type < WB_TYPE_WCH0) || (wb_type >= WB_TYPE_MAX)) {
		DPU_FB_ERR("dpu_get_wb_type failed!\n");
		return -EINVAL;
	}
...
static int dpu_offline_play_data_prepare(
	struct dpu_fb_data_type *dpufd,
	...
	dss_overlay_t **pov_req_out)
{
	...
	pov_req = dpu_offline_play_alloc_data();
	if (!pov_req) {
		DPU_FB_ERR("fb%u, dss_overlay_t alloc failed!\n", dpufd->index);
		return -1;
	}

	ret = dpu_get_ov_data_from_user(dpufd, pov_req, argp);
	if (ret != 0) {
		DPU_FB_ERR("fb%u, dpu_get_ov_data_from_user failed! ret=%d\n", dpufd->index, ret);
		dpu_offline_play_free_data(pov_req);
		return -1;
	}

	ret = dpu_check_panel_status_and_composer_type(dpufd->index, pov_req);
	if (ret != 0) {
		dpu_offline_play_free_data(pov_req);
...
static void dpu_offline_handle_device_power(
	struct dpu_fb_data_type *dpufd,
	uint32_t wb_compose_type,
	bool poweron)
{
	...
	if (poweron) {
		dpu_offline_composer_on(dpufd);
		if (wb_compose_type == DSS_WB_COMPOSE_COPYBIT)
			(void)dpufb_offline_vote_ctrl(dpufd, true);
	} else {
		if (wb_compose_type == DSS_WB_COMPOSE_COPYBIT)
			(void)dpufb_offline_vote_ctrl(dpufd, false);
		dpu_offline_composer_off(dpufd);
	}
#else
	if (poweron) {
		dpu_offline_composer_on(dpufd);
		dpu_tuning_offline_clk_rate(dpufd);
	} else {
		dpu_recovery_online_clk_rate(dpufd);
		dpu_offline_composer_off(dpufd);
	}
#endif
}

static int dpu_offline_play_device_prepare(
	struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req,
	int *enable_cmdlist,
	uint32_t *cmdlist_idxs)
{
	...
	dpu_offline_handle_device_power(dpufd, pov_req->wb_compose_type, true);
	dpufb_offline_layerbuf_lock(dpufd, pov_req, &lock_list);
	dpufb_layerbuf_flush(dpufd, &lock_list);

	pov_h_block_infos = (dss_overlay_block_t *)(uintptr_t)(pov_req->ov_block_infos_ptr);
	ret = dpu_offline_chn_include_online_chn(pov_h_block_infos, pov_req->ov_block_nums);
	if (ret != 0) {
		DPU_FB_ERR("fb%u, dpu_offline_ch_include_online_ch failed!, wb_compose_type=%d\n",
			dpufd->index, pov_req->wb_compose_type);
		dpufb_buf_sync_signal(dpufd);
		dpu_offline_handle_device_power(dpufd, pov_req->wb_compose_type, false);
		return -1;
	}

	dpufd->mmbuf_info = dpu_mmbuf_info_get(dpufd, 0);
	if (!dpufd->mmbuf_info) {
	...
	*enable_cmdlist = g_enable_ovl_cmdlist_offline;
	if (*enable_cmdlist) {
		...
		dpu_cmdlist_data_get_offline(dpufd, pov_req);
...
static int dpu_offline_play_prepare(
	struct dpu_fb_data_type *dpufd,
	const void __user *argp,
	dss_overlay_t **pov_req_out,
	int *enable_cmdlist,
	uint32_t *cmdlist_idxs)
{
	...
	ret = dpu_offline_play_data_prepare(dpufd, argp, pov_req_out);
	if (ret != 0) {
		DPU_FB_INFO("fb%u, data prepare failed!\n", dpufd->index);
		return ret;
	}

	ret = dpu_offline_play_device_prepare(dpufd, *pov_req_out, enable_cmdlist, cmdlist_idxs);
	if (ret != 0) {
		DPU_FB_INFO("fb%u, device prepare failed!\n", dpufd->index);
		dpu_offline_play_free_data(*pov_req_out);
		return ret;
	}
...
static int dpu_offline_play_config_locked(
	struct dpu_fb_data_type *dpufd,
	const void __user *argp,
	...
	struct dss_cmdlist_idxs *cmdlist_idxs_tmp)
{
	...
	ret = dpu_offline_play_prepare(dpufd, argp, ov_req,
		cmdlist_idxs_tmp->enable_cmdlist, cmdlist_idxs_tmp->cmdlist_idxs);

	if (ret != 0) {
		DPU_FB_INFO("fb%u, dpu offline play prepare failed!\n", dpufd->index);
	...
	dpu_drm_layer_offline_config(dpufd, pov_req);
	ret = dpu_ov_block_config(dpufd, pov_req, pov_h_block_infos);
	if (ret != 0) {
		DPU_FB_ERR("fb%u, dpu_ov_block_config failed!\n", dpufd->index);
		goto err_return;
	}

	ret = dpu_add_clear_module_reg_node(dpufd, pov_req,
		*(cmdlist_idxs_tmp->cmdlist_idxs), *(cmdlist_idxs_tmp->enable_cmdlist), use_comm_mmbuf);
	if (ret != 0) {
		DPU_FB_ERR("fb%u dpu_add_clear_module_reg_node failed! ret=%d\n", dpufd->index, ret);
		goto err_return;
	}

	if (*(cmdlist_idxs_tmp->enable_cmdlist))
		dpu_cmdlist_flush_cache(dpufd, *(cmdlist_idxs_tmp->cmdlist_idxs));
...
err_return:
	dpu_offline_clear(dpufd, pov_req,
		*(cmdlist_idxs_tmp->enable_cmdlist), *(cmdlist_idxs_tmp->cmdlist_idxs), false);

	dpu_offline_handle_device_power(dpufd, wb_compose_type, false);
...
static void dpu_cmdlist_handle(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	uint32_t cmdlist_idxs, int mctl_idx, uint32_t wb_compose_type)
{
	bool parallel_compose_flag = dpu_get_parallel_compose_flag(dpufd, wb_compose_type);
	if (parallel_compose_flag) {
		dpu_cmdlist_exec(dpufd, cmdlist_idxs | dpufd->wb_info.cmdlist_idxs);
		dpu_cmdlist_config_start(dpufd, dpufd->wb_info.mctl_idx,
			dpufd->wb_info.cmdlist_idxs, dpufd->wb_info.wb_compose_type);
		dpu_cmdlist_config_start(dpufd, mctl_idx, cmdlist_idxs, wb_compose_type);
	} else {
...
static int dpu_offline_play_work_locked(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	int enable_cmdlist, uint32_t use_comm_mmbuf, uint32_t cmdlist_idxs)
{
	int ret;
	bool reset = false;
	int wb_type = dpu_get_wb_type(pov_req);
	int mctl_idx = dpu_get_mctl_idx(pov_req);
	...
	if (enable_cmdlist) {
		if (use_comm_mmbuf)
			dpu_vactive0_start_wait(dpufd, pov_req, use_comm_mmbuf);

		dpu_cmdlist_handle(dpufd, pov_req, cmdlist_idxs, mctl_idx, wb_compose_type);
	}
	...
	if (parallel_compose_flag)
		ret = dpu_check_two_wch_wb_finish(dpufd, dpufd->wb_info.cmdlist_idxs,
			dpufd->wb_info.mctl_idx, cmdlist_idxs, mctl_idx);
	else
		ret = dpu_check_wch_wb_finish(dpufd, pov_req, wb_compose_type, cmdlist_idxs, mctl_idx);

	if (ret != 0)
		reset = true;

	if (parallel_compose_flag) {
		dpu_offline_clear(dpufd, pov_req, enable_cmdlist,
			(dpufd->wb_info.cmdlist_idxs | cmdlist_idxs), reset);
		memset(&dpufd->wb_info, 0, sizeof(struct dss_wb_info));
	} else {
		dpu_offline_clear(dpufd, pov_req, enable_cmdlist, cmdlist_idxs, reset);
	}
...
int dpu_ov_offline_play(struct dpu_fb_data_type *dpufd, const void __user *argp)
{
	...
	dpu_check_and_return((!dpufd->cmdlist_info), -EINVAL, ERR, "dpufd->cmdlist_info is null!");

	dpufb_get_timestamp(&tv0);

	ret = dpu_offline_play_config_locked(dpufd, argp, &pov_req, &use_comm_mmbuf, &cmdlist_idxs_tmp);
	if (ret != 0) {
	...
	ret = dpu_offline_play_work_locked(dpufd, pov_req, enable_cmdlist, use_comm_mmbuf, cmdlist_idxs);
	if (ret != 0) {
