	...
	struct fb_info *fbi)
{
	...
	if (unlikely(fbi->fix.smem_len == 0)) {
	...
	void __user *argp)
{
	...
	if (g_debug_ovl_online_composer_return) {
		DPU_FB_INFO("fb%d, g_debug_ovl_online_composer_return!\n", dpufd->index);
		return false;
	}
	...
	int dss_free_buffer_refcount)
{
	...
	DPU_ATRACE_BEGIN("dpu_online_play_prepare");
	...
	if (ret != 0) {
		...
		DPU_ATRACE_END("dpu_online_play_prepare");
		return ret;
	}
	...
	bool *masklayer_maxbacklight_flag)
{
	...
	DPU_ATRACE_BEGIN("dpu_online_play_config_start");
	pov_req = &(dpufd->ov_req);
	pov_req_prev = &(dpufd->ov_req_prev);
	...
	if (vsync_time_checked == false) {
		ret = dpu_check_vsync_timediff(dpufd, pov_req);
		if (ret < 0) {
			DPU_ATRACE_END("dpu_online_play_config_start");
			return ret;
		}
	}
	...
	if (dpu_afr_need_update_single_frame(dpufd))
#endif
	{
	...
	struct dpu_fb_data_type *dpufd)
{
	...
	dss_overlay_t *pov_req = NULL;
	struct dss_cmdlist_idxs idxs_tmp = { &cmdlist_pre_idxs, &cmdlist_idxs, &enable_cmdlist };
	DPU_ATRACE_BEGIN("dpu_online_play_config_locked");
	...
	DPU_ATRACE_END("dpu_online_play_config_locked");
	return ret;
	...
	void __user *argp)
{
	...
	DPU_ATRACE_BEGIN_WITH_VALUE("dpu_ov_online_play", dpufd->ov_req.frame_no);
	...
	DPU_ATRACE_END_WITH_VALUE("dpu_ov_online_play", dpufd->ov_req.frame_no);
	return ret;
}
