...
static int dpu_check_sh_aod_blank_refcount(struct dpu_fb_data_type *dpufd, uint32_t msg_no)
{
...
static int dpu_sensorhub_aod_set_pxl_clk(struct dpu_fb_data_type *dpufd)
{
...
static int dpu_aod_config_fold_panel(struct dpu_fb_data_type *dpufd)
{
...
int dpu_sensorhub_aod_unblank(uint32_t msg_no)
{
	...
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE) {
		panel_id = dpu_aod_get_panel_id();
		if (panel_id == DISPLAY_OUTER_PANEL_ID)
	...
	if (!dpu_sensorhub_aod_hw_lock(dpufd))
		goto up_blank_sem;

	if (dpu_check_sh_aod_blank_refcount(dpufd, msg_no))
		goto hw_unlock;

#if defined(CONFIG_FOLD_DISPLAY)
	if (dpufd->panel_info.product_type & PANEL_SUPPORT_TWO_PANEL_DISPLAY_TYPE) {
		if (dpu_aod_config_fold_panel(dpufd))
			goto hw_unlock;
	}
#endif
	...
	ret = dpu_sensorhub_aod_set_pxl_clk(dpufd);
	if (ret < 0)
		goto hw_unlock;
...
hw_unlock:
	...
	if (!dpu_sensorhub_aod_hw_unlock(dpufd))
...
{
	return dpu_sensorhub_aod_hw_lock(dpufd);
}
...
{
	return dpu_aod_get_aod_lock_status();
}
#endif

#if defined(CONFIG_DPU_FB_V320)
static void dpu_fb_video_aod_wait(struct dpu_fb_data_type *dpufd)
{
	uint32_t try_times = 0;

	if (is_mipi_video_panel(dpufd) &&
		dpufd->index == PRIMARY_PANEL_IDX &&
		dpufd_list[AUXILIARY_PANEL_IDX]->panel_power_on)  {
		/* if video panel aod, wait for offline blank */
		DPU_FB_INFO(" wait for offline blank!\n");
		while (dpufd_list[AUXILIARY_PANEL_IDX]->panel_power_on) {
			mdelay(1);
			if (++try_times > 1000) {  /* wait 1s for offline blank */
...
static int dpufb_unblank_refresh(struct dpu_fb_data_type *dpufd, struct fb_info *info)
{
	...
	dpu_fb_frame_refresh(dpufd, "aod");
...
#if defined(CONFIG_DPU_FB_V320)
	dpu_fb_video_aod_wait(dpufd);
#endif

	down(&dpufd->blank_aod_sem);
	if (dpu_fb_get_aod_lock_status() == false) {
