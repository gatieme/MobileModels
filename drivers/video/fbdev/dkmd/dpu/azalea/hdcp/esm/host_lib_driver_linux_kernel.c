 ...
 * Foundation, Inc., 59 Temple Place, Suite 330, dpuv410, MA 02111-1307 USA
 */
/*
...
#define HPI_ADDRESS_ESM0    0xf8440000
#elif defined(CONFIG_DPU_FB_V510)
#define HPI_ADDRESS_ESM0    0xf8540000
#else
#define HPI_ADDRESS_ESM0    0xff350000
#endif
#define HDCP_FW_ADDRESS     RESERVED_DP_HDCP2_PHYMEM_BASE
#define HDCP_FW_SIZE        0x40000
#define HDCP_DATA_ADDRESS   (RESERVED_DP_HDCP2_PHYMEM_BASE + HDCP_FW_SIZE)
 ...
 * Constant strings
 */
static const char *g_my_tag = "ESM HLD: ";

/*
 * Linux device, class and range
 */
static int g_esm_en = 0;
static int g_esm_opened = 0;

/*
 * ESM devices
 */
static esm_device g_esm_devices[MAX_ESM_DEVICES];

/*
 * static function declaration
 */
static int cmd_esm_open_memory_init(esm_device *esm);
static void cmd_esm_start_on_request(esm_hld_ioctl_esm_start *krequest);

static bool check_power_down(ESM_STATUS *request_returned_status)
{
	if ((!g_esm_en) || (!g_esm_opened)) {
		DPU_FB_ERR("DP power down, no access hdcp register\n");
		*request_returned_status = HL_DRIVER_NO_ACCESS;
		return false;
	}
...
static long cmd_hpi_read(esm_device *esm, esm_hld_ioctl_hpi_read *request)
{
	...
	DPU_FB_INFO("%scmd_hpi_read: Reading register at offset 0x%x\n",  g_my_tag, krequest.offset);

	if (!check_power_down(&krequest.returned_status))
		goto Exit;
...
static long cmd_hpi_write(esm_device *esm, esm_hld_ioctl_hpi_write *request)
{
	...
	DPU_FB_INFO("%scmd_hpi_write: Writing 0x%x to register at offset 0x%x\n",
		g_my_tag, krequest.data, krequest.offset);
...
static long cmd_data_read(esm_device *esm, esm_hld_ioctl_data_read *request)
{
	...
	dpu_check_and_return(((esm == NULL) || (request == NULL)), -1, ERR, "esm or request is null pointer\n");
	...
	DPU_FB_INFO("%scmd_data_read: Reading %u bytes from data memory at offset offset 0x%x\n",
		g_my_tag, krequest.nbytes, krequest.offset);
...
static long cmd_data_set(esm_device *esm, esm_hld_ioctl_data_set *request)
{
	...
	DPU_FB_INFO("%scmd_data_set: Setting %u bytes (data=0x%x) of data memory from offset 0x%x\n",
		g_my_tag, krequest.nbytes, krequest.data, krequest.offset);
...
static long cmd_esm_open(struct file *f, esm_hld_ioctl_esm_open *request)
{
	...
	if (cmd_esm_open_memory_init(esm) != HL_DRIVER_SUCCESS)
		goto ErrorExit;
...
static int cmd_esm_open_memory_init(esm_device *esm)
{
	...
	esm->hpi = ioremap(esm->hpi_base, esm->hpi_size);
	if (esm->hpi == NULL) {
		DPU_FB_ERR("%s HPI virtual addr map fail\n",  g_my_tag);
		return HL_DRIVER_NO_MEMORY;
	}

	return HL_DRIVER_SUCCESS;
}
...
static long cmd_esm_start(esm_device *esm, esm_hld_ioctl_esm_start *request)
{
	...
	cmd_esm_start_on_request(&krequest);
...
static void cmd_esm_start_on_request(esm_hld_ioctl_esm_start *krequest)
{
	...
	case 0: /* set I_px_gpio_in[0] when boot up esm */
		DPU_FB_INFO("%s start esm!\n", g_my_tag);
		arm_smccc_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP, DSS_HDCP22_ENABLE,
			1, (u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_CTRL,
			0, 0, 0, 0, &res);
		krequest->returned_status = (int)res.a0;
		break;
	case 1:
		DPU_FB_INFO("%s set HDCP1.3 %d\n", g_my_tag, krequest->value);
		arm_smccc_smc(ACCESS_REGISTER_FN_MAIN_ID_HDCP, DSS_HDCP13_ENABLE,
			krequest->value, (u64)ACCESS_REGISTER_FN_SUB_ID_HDCP_CTRL,
			0, 0, 0, 0, &res);
		krequest->returned_status = (int)res.a0;
		break;
	case 2:
...
static long cmd_get_te_info(esm_device *esm, esm_hld_ioctl_get_te_info *request)
{
	...
	dpu_check_and_return((request == NULL), -1, ERR, "request is null pointer\n");
...
void esm_end_device(void)
{
	...
	memset(g_esm_devices, 0, sizeof(g_esm_devices));
