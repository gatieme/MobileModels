...
/* KLVV only use online channels, and idx_online = idx_offline - 1 */
static int8_t is_only_use_online_axi(struct dpu_fb_data_type *dpufd, int8_t axi_idx)
{
	if (axi_idx != DSS_OFFLINE_AXI0 && axi_idx != DSS_OFFLINE_AXI1)
		return axi_idx;
	if (dpufd == NULL) {
		DPU_FB_ERR("panel info is NULL\n");
		return axi_idx;
	}
	if (dpufd->panel_info.is_only_use_online_axi) {
		/* convert offline idx to online index */
		return axi_idx - 1;
	} else {
		return axi_idx;
	}
...
static void dpu_aif_cfg_for_wblayer(struct dpu_fb_data_type *dpufd,
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block, dss_aif_bw_t aif_bw_tmp[], uint32_t aif_bw_size)
{
	...
	if (unlikely((pov_req->wb_layer_nums == 0) || (pov_req->wb_layer_nums > MAX_DSS_DST_NUM))) {
	...
	for (k = 0; k < pov_req->wb_layer_nums; k++) {
		...
		if ((chn_idx < 0) || (chn_idx >= DSS_CHN_MAX_DEFINE)) {
			DPU_FB_ERR("wb_layer->chn_idx exceeds array limit\n");
			return;
		}

		if (wb_layer->dst.height == 0 || wb_layer->dst.width == 0) {
			DPU_FB_ERR("wb_layer->dst.height or wb_layer->dst.width division by zero\n");
			return;
		}
		...
		if (pov_req->wb_compose_type == DSS_WB_COMPOSE_MEDIACOMMON)
			aif_bw->axi_sel = is_only_use_online_axi(dpufd, DSS_OFFLINE_AXI0);
		else
			aif_bw->axi_sel = is_only_use_online_axi(dpufd, DSS_OFFLINE_AXI1);
	...
	if ((pov_req->wb_compose_type == DSS_WB_COMPOSE_COPYBIT) ||
		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if ((chn_idx < 0) || (chn_idx >= DSS_CHN_MAX_DEFINE)) {
				DPU_FB_ERR("layer->chn_idx exceeds array limit\n");
				return;
			}
			...
			aif_bw_tmp[i].axi_sel = is_only_use_online_axi(dpufd, DSS_OFFLINE_AXI0);
	...
	dss_overlay_t *pov_req, dss_aif_bw_t aif_bw_tmp[], uint32_t bw_sum)
{
	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		...
		if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1)) {
			...
			if (tmp <= (bw_sum / 2)) {  /* dichotomy */
				aif_bw_tmp[i].axi_sel = is_only_use_online_axi(dpufd, DSS_OFFLINE_AXI1);
				if (axi1_cnt >= AXI1_MAX_DSS_CHN_THRESHOLD) {
					/* lint !e676 */
					aif_bw_tmp[i - AXI0_MAX_DSS_CHN_THRESHOLD].axi_sel = is_only_use_online_axi(dpufd, DSS_OFFLINE_AXI0);
					axi0_cnt++;
					axi1_cnt--;
				}
				axi1_cnt++;
...
static void dpu_set_ch_aif_sel(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	dss_overlay_block_t *pov_h_block, uint32_t aif_bw_size)
{
	...
	for (i = 0; i < pov_h_block->layer_nums; i++) {
		...
		aif_bw->is_used = 1;
		aif_bw->chn_idx = chn_idx;
		aif_bw->axi_sel = is_only_use_online_axi(dpufd, get_aif_axi_sel(pov_req->ovl_idx, dpufd->use_axi_cnt));

		if (g_debug_ovl_online_composer)
			DPU_FB_INFO("fb%d, aif, chn_idx=%d, axi_sel=%d.\n", dpufd->index, chn_idx, aif_bw->axi_sel);
	}
}
	...
	dss_overlay_block_t *pov_h_block, dss_aif_bw_t aif_bw_tmp[], uint32_t aif_bw_size)
{
	...
	if (dpu_check_panel_product_type(dpufd->panel_info.product_type)) {
		dpu_set_ch_aif_sel(dpufd, pov_req, pov_h_block, DSS_CHN_MAX_DEFINE);
		return;
	}
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block)
{
	...
	if (pov_req->wb_enable)
		dpu_aif_cfg_for_wblayer(dpufd, pov_req, pov_h_block, aif_bw_tmp, DSS_CHN_MAX_DEFINE);

#ifdef SUPPORT_BALANCED_AIF_DISTRIBUTION
	DPU_FB_DEBUG("pov_req->ovl_idx:%d\n", pov_req->ovl_idx);
	...
	dss_wb_layer_t *wb_layer)
{
	...
	if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1)) {
		...
		aif->aif_ch_ctl = set_bits32(aif->aif_ch_ctl, layer->axi_idx, 2, 0);
		dpufd->dss_module.aif_ch_used[chn_idx] = 1;
		DPU_FB_DEBUG("aif layer:%d axi:%d", layer->chn_idx, layer->axi_idx);
		return 0;
	}
#endif
	...
	dss_layer_t *layer, dss_wb_layer_t *wb_layer, int ovl_idx)
{
	...
	if ((pov_req->ovl_idx == DSS_OVL0) || (pov_req->ovl_idx == DSS_OVL1)) {
		...
		aif1->aif_ch_ctl = set_bits32(aif1->aif_ch_ctl, layer->mmaxi_idx, 2, 0);
		dpufd->dss_module.aif1_ch_used[chn_idx] = 1;
		DPU_FB_DEBUG("aif1 layer:%d mmaxi:%d", layer->chn_idx, layer->mmaxi_idx);
		return 0;
	}
#endif
