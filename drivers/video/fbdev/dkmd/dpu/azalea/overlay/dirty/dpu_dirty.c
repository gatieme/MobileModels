...
#if defined(CONFIG_DPU_FB_V600) || defined(CONFIG_DPU_FB_V346) || defined(CONFIG_DPU_FB_V360)
	dpufd->set_reg(dpufd, dss_base + DSS_HI_ACE_OFFSET + DPE_IMAGE_INFO,
		s_dirty_region_updt->hiace_img_size, 32, 0);

	if (dpufd->panel_info.ifbc_type != IFBC_TYPE_NONE)
		dpufd->set_reg(dpufd, dss_base + DSS_DSC_OFFSET + DSC_PIC_SIZE,
			s_dirty_region_updt->ifbc_size, 32, 0);

	spr_dsc_partial_updt_config(dpufd, &(s_dirty_region_updt->spr_ditry));
#endif
}
	...
	struct dss_rect *dirty)
{
	...
	return ((dirty->x == 0) && (dirty->y == 0) && (dirty->w == (int32_t)(dpufd->panel_info.xres)) &&
		(dirty->h == ((int32_t)(dpufd->panel_info.yres))));
}
#endif

static void get_dpu_dirty_rect(struct dpu_fb_data_type *dpufd,
	struct dss_rect *dirty,
	...
	dss_overlay_t *pov_req)
{
	...
	if (is_dirty_region_empty(pov_req)) {
		...
		if (dpu_check_panel_feature_support(pinfo) && (pov_req->rog_width > 0 && pov_req->rog_height > 0)) {
...
static void dpu_set_ifbc_size(dirty_region_updt_t *dirty_region_updt,
	...
	dss_overlay_t *pov_req)
{
	...
	if ((!IS_BUILTIN_PANEL) || !pinfo->dirty_region_updt_support)
		return 0;

	get_dpu_dirty_rect(dpufd, &dirty, NULL, pov_req);
	...
	/* spr should overlap dirty region.
	 * The modules before SPR(DPP; OV; HIACE; SPR) need use overlaped dirty size.
	 * The modules behind SPR(DSC; DBUF; mipi) need use real dirty size.
	 */
	if (is_dirty_full_screen(dpufd, &dirty) && (pov_req != NULL))
		pov_req->spr_overlap_type = SPR_OVERLAP_NONE;

	spr_fill_dirty_region_info(dpufd, &dirty, pov_req, &(dirty_region_updt->spr_ditry));
	spr_get_real_dirty_region(dpufd, &dirty, pov_req);

	dpu_set_ifbc_size(dirty_region_updt, pinfo, &dirty);
#endif
...
static void dpu_dirty_region_updt_set_reg(struct dpu_fb_data_type *dpufd,
	char __iomem *dss_base, dirty_region_updt_t *s_dirty_region_updt)
{
...
static void get_dpu_dirty_region(struct dpu_fb_data_type *dpufd, struct dss_rect *dirty,
	dss_rect_t *rect, struct dpu_dirty_region *dirty_region)
{
	struct dpu_panel_info *pinfo = NULL;
	...
	rect->w = (int32_t)get_hsize_after_spr_dsc(dpufd, rect->w);
...
static void dpu_set_hrz_ctr(struct dpu_fb_data_type *dpufd,
...
static void dpu_set_vrt_ctr(struct dpu_fb_data_type *dpufd,
	dirty_region_updt_t *dirty_region_updt, struct dpu_panel_info *pinfo,
	dss_rect_t *rect, struct dpu_dirty_region *dirty_region)
{
#ifdef SUPPORT_DSI_VER_2_0
...
static void dpu_set_display_dirty_region(struct dpu_fb_data_type *dpufd,
	...
	dss_overlay_t *pov_req)
{
	...
	struct dss_rect dirty = {0};
	dss_rect_t rect = {0};
	struct dpu_dirty_region dirty_region = {0};
	...
	get_dpu_dirty_rect(dpufd, &dirty, dirty_region_updt, pov_req);
	if (!is_dss_dirty_region(dpufd, &dirty))
		return;
	...
	get_dpu_dirty_region(dpufd, &dirty, &rect, &dirty_region);

	dpu_set_hrz_ctr(dpufd, dirty_region_updt, pinfo, &rect, &dirty_region);
	dpu_set_vrt_ctr(dpufd, dirty_region_updt, pinfo, &rect, &dirty_region);
	...
	dpu_set_display_dirty_region(dpufd, &dirty, rect);
	DPU_FB_INFO("dirty_updt[%d,%d, %d,%d], h_porch_pading=%d, v_porch_pading=%d.\n",
		dirty.x, dirty.y, dirty.w, dirty.h,
		dirty_region.h_porch_pading, dirty_region.v_porch_pading);

	dpu_dirty_region_updt_set_reg(dpufd, dpufd->dss_base, &(dpufd->dss_module.dirty_region_updt));

#if defined(CONFIG_DPU_FB_V501)
	hiace_size_config(dpufd, dirty.w, dirty.h);
#endif
