	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if (!pov_req) {
		DPU_FB_ERR("fb%d, invalid pov_req!\n", dpufd->index);
		return -EINVAL;
	}

	if (!pov_h_block_infos) {
		DPU_FB_ERR("fb%d, invalid pov_h_block_infos!\n", dpufd->index);
		return -EINVAL;
	}

	if (unlikely((pov_req->ov_block_nums == 0) || (pov_req->ov_block_nums > DPU_OV_BLOCK_NUMS))) {
		DPU_FB_ERR("fb%d, invalid ov_block_nums=%d!\n", dpufd->index, pov_req->ov_block_nums);
		return -EINVAL;
	}

	if (unlikely((pov_h_block_infos->layer_nums == 0) ||
...
{
	if (!wb_layer) {
		DPU_FB_ERR("fb%d, invalid wb_layer!\n", index);
		return -EINVAL;
	}
...
{
	if (unlikely(pov_req->wb_layer_nums == 0) ||
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if (dpu_check_userdata_check(dpufd, pov_req, pov_h_block_infos) == -EINVAL)
		return -EINVAL;
...
static void dpufb_dss_media1_subsys_pu_reset(char __iomem *peri_crg_base, char __iomem *sctrl_base)
{
	/* MEDIA1_SUBSYS_PU
	 * 1: module mtcmos on
	 */
...
static void dpufb_dss_pu_reset(char __iomem *media_crg_base)
{
	/* DSS_PU
	 * 1.1: module unrst
	 */
...
void dpufb_dss_disreset(struct dpu_fb_data_type *dpufd)
{
	...
	sctrl_base = dpufd->sctrl_base;
	media_crg_base = dpufd->media_crg_base;

	dpufb_dss_media1_subsys_pu_reset(peri_crg_base, sctrl_base);
	dpufb_dss_vivobus_pu_reset(media_crg_base, pmctrl_base);
	dpufb_dss_pu_reset(media_crg_base);
}
...
static uint32_t *dpu_get_acm_lut_sata_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr0_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr1_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr2_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr3_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr4_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr5_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr6_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static uint32_t *dpu_get_acm_lut_satr7_table(struct dpu_fb_data_type *dpufd,
	uint8_t last_gamma_type)
{
	struct dpu_panel_info *pinfo = &(dpufd->panel_info);
...
static void acm_lut_param_init(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type,
	struct acm_lut_satr *acm_lut)
{
	acm_lut->acm_lut_hue_table = dpu_get_acm_lut_hue_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_sata_table = dpu_get_acm_lut_sata_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr0_table = dpu_get_acm_lut_satr0_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr1_table = dpu_get_acm_lut_satr1_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr2_table = dpu_get_acm_lut_satr2_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr3_table = dpu_get_acm_lut_satr3_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr4_table = dpu_get_acm_lut_satr4_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr5_table = dpu_get_acm_lut_satr5_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr6_table = dpu_get_acm_lut_satr6_table(dpufd, last_gamma_type);
	acm_lut->acm_lut_satr7_table = dpu_get_acm_lut_satr7_table(dpufd, last_gamma_type);
}

static int gamma_lut_param_init(struct dpu_panel_info *pinfo, uint8_t last_gamma_type,
	struct gamma_lut_rgb *gamma_lut)
{
	if (last_gamma_type == 1) {
		...
		if (pinfo->cinema_gamma_lut_table_len > 0 && pinfo->cinema_gamma_lut_table_R &&
			...
			gamma_lut->gamma_lut_table_r = pinfo->cinema_gamma_lut_table_R;
			gamma_lut->gamma_lut_table_g = pinfo->cinema_gamma_lut_table_G;
			gamma_lut->gamma_lut_table_b = pinfo->cinema_gamma_lut_table_B;
		} else {
		...
		if (pinfo->gamma_lut_table_len > 0 && pinfo->gamma_lut_table_R &&
			...
			gamma_lut->gamma_lut_table_r = pinfo->gamma_lut_table_R;
			gamma_lut->gamma_lut_table_g = pinfo->gamma_lut_table_G;
			gamma_lut->gamma_lut_table_b = pinfo->gamma_lut_table_B;
		} else {
...
static bool is_acm_lut_valid(struct acm_lut_satr acm_lut)
{
	return (!(acm_lut.acm_lut_hue_table && acm_lut.acm_lut_sata_table &&
		acm_lut.acm_lut_satr0_table && acm_lut.acm_lut_satr1_table &&
		acm_lut.acm_lut_satr2_table && acm_lut.acm_lut_satr3_table &&
		acm_lut.acm_lut_satr4_table && acm_lut.acm_lut_satr5_table &&
		acm_lut.acm_lut_satr6_table && acm_lut.acm_lut_satr7_table));
}
...
static int dpu_dpp_gamma_para_set_reg(struct dpu_fb_data_type *dpufd, uint8_t last_gamma_type)
{
	...
	char __iomem *acm_lut_base = dpufd->dss_base + DSS_DPP_ACM_LUT_OFFSET;
	struct gamma_lut_rgb gamma_lut = { NULL, NULL, NULL };
	struct acm_lut_satr acm_lut = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };

	acm_lut_param_init(dpufd, last_gamma_type, &acm_lut);
	if (gamma_lut_param_init(pinfo, last_gamma_type, &gamma_lut) < 0)
		return -1;

	if (is_acm_lut_valid(acm_lut)) {
	...
	for (index = 0; index < pinfo->gamma_lut_table_len / 2; index++) {
		i = index << 1;
		outp32(gamma_lut_base + (U_GAMA_R_COEF + index * 4), (gamma_lut.gamma_lut_table_r[i] |
			(gamma_lut.gamma_lut_table_r[i + 1] << 16)));
		outp32(gamma_lut_base + (U_GAMA_G_COEF + index * 4), (gamma_lut.gamma_lut_table_g[i] |
			(gamma_lut.gamma_lut_table_g[i + 1] << 16)));
		outp32(gamma_lut_base + (U_GAMA_B_COEF + index * 4), (gamma_lut.gamma_lut_table_b[i] |
			(gamma_lut.gamma_lut_table_b[i + 1] << 16)));
	}
	outp32(gamma_lut_base + U_GAMA_R_LAST_COEF, gamma_lut.gamma_lut_table_r[pinfo->gamma_lut_table_len - 1]);
	outp32(gamma_lut_base + U_GAMA_G_LAST_COEF, gamma_lut.gamma_lut_table_g[pinfo->gamma_lut_table_len - 1]);
	outp32(gamma_lut_base + U_GAMA_B_LAST_COEF, gamma_lut.gamma_lut_table_b[pinfo->gamma_lut_table_len - 1]);

	acm_set_lut_hue(acm_lut_base + ACM_U_H_COEF, acm_lut.acm_lut_hue_table, pinfo->acm_lut_hue_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATA_COEF, acm_lut.acm_lut_sata_table, pinfo->acm_lut_sata_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR0_COEF, acm_lut.acm_lut_satr0_table, pinfo->acm_lut_satr0_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR1_COEF, acm_lut.acm_lut_satr1_table, pinfo->acm_lut_satr1_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR2_COEF, acm_lut.acm_lut_satr2_table, pinfo->acm_lut_satr2_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR3_COEF, acm_lut.acm_lut_satr3_table, pinfo->acm_lut_satr3_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR4_COEF, acm_lut.acm_lut_satr4_table, pinfo->acm_lut_satr4_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR5_COEF, acm_lut.acm_lut_satr5_table, pinfo->acm_lut_satr5_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR6_COEF, acm_lut.acm_lut_satr6_table, pinfo->acm_lut_satr6_table_len);
	acm_set_lut(acm_lut_base + ACM_U_SATR7_COEF, acm_lut.acm_lut_satr7_table, pinfo->acm_lut_satr7_table_len);
