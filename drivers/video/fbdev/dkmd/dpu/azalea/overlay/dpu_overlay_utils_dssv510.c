...
static int dpu_check_userdata_layer_info(struct dpu_fb_data_type *dpufd,
	dss_overlay_block_t *pov_h_block)
{
	...
	for (i = 0; i < pov_h_block->layer_nums; i++) {
		...
		if (DSS_WIDTH(h_layer->dst_rect.w) < DSS_OFFLINE_MIN_LEN &&
			DSS_WIDTH(h_layer->dst_rect.h) < DSS_OFFLINE_MIN_LEN) {
			DPU_FB_ERR("fb%d, dst_rect.w = %d, dst_rect.h = %d\n",
				dpufd->index, h_layer->dst_rect.w, h_layer->dst_rect.h);
			return -EINVAL;
		}
	}
	...
	dss_overlay_t *pov_req, dss_overlay_block_t *pov_h_block_infos)
{
	...
	if (unlikely((pov_req->ov_block_nums == 0) ||
	...
	for (i = 0; i < pov_req->ov_block_nums; ++i) {
		if (unlikely((pov_h_block_infos[i].layer_nums == 0) ||
		...
		if (dpu_check_userdata_layer_info(dpufd, pov_h_block) < 0) {
			DPU_FB_ERR("fb%d, invalid layer", dpufd->index);
			return -EINVAL;
		}
	}
...
static int dpu_check_layer_buff(
...
void dpu_dpp_acm_gm_set_reg(struct dpu_fb_data_type *dpufd)
{
	...
	dpu_check_and_no_retval((!IS_BUILTIN_PANEL), ERR, "fb%d, not support!\n", dpufd->index);
...
static void dpu_dump_dma_info(struct dpu_fb_data_type *dpufd)
{
	int i;
	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		...
		if ((i <= DSS_RCHN_G1) && (i != DSS_RCHN_D3)) {
			DPU_FB_INFO("HFBCD{0x0990}: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
				inp32(SOC_MDC_HFBCD_MONTIOR_REG1_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HFBCD_MONTIOR_REG2_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HFBCD_MONTIOR_REG3_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HFBCD_DEBUG_REG0_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HFBCD_MONTIOR_REG4_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HFBCD_MONTIOR_REG5_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])));

			DPU_FB_INFO("AFBCD{0x09A0}:0x%x 0x%x\n",
				inp32(SOC_MDC_AFBCD_MONTIOR_REG4_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_AFBCD_MONTIOR_REG5_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])));

			DPU_FB_INFO("HEBCD{0x09E0}:0x%x 0x%x 0x%x 0x%x\n",
				inp32(SOC_MDC_HEBCD_MONITOR_REG0_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HEBCD_MONITOR_REG1_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HEBCD_MONITOR_REG2_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])),
				inp32(SOC_MDC_HEBCD_MONITOR_REG3_ADDR(dpufd->dss_base +
					g_dss_module_base[i][MODULE_DMA])));
		}
#endif
	}
...
{
	dpu_check_and_no_retval(!dpufd->dss_module.post_scf_base, INFO, "post_scf_base is NULL!\n");
...
static void dpu_dump_mediacrg_info(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("Mediacrg QIC: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
		inp32(SOC_MEDIA1_CRG_INTR_STAT_QIC_BUS_NONIDLE_PEND_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_INTR_BUS_DSS_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_INTR_BUS0_VIVO_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_INTR_BUS1_VIVO_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_BUSY_DSS_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_BUSY0_VIVO_ADDR(dpufd->media_crg_base)),
		inp32(SOC_MEDIA1_CRG_QIC_BUSY1_VIVO_ADDR(dpufd->media_crg_base)));
#endif
}
...
static uint32_t dpu_vactive0_get_cmdlist_idxs(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	struct timediff_info *timediff, int ret)
{
	...
	ret1 = dpu_cmdlist_get_cmdlist_idxs(pov_req_prev, &cmdlist_idxs_prev, NULL);
	if (ret1 != 0)
		DPU_FB_INFO("fb%d, dpu_cmdlist_get_cmdlist_idxs pov_req_prev failed! ret = %d\n",
			dpufd->index, ret1);

	ret1 = dpu_cmdlist_get_cmdlist_idxs(pov_req_prev_prev, &cmdlist_idxs_prev_prev, NULL);
	if (ret1 != 0)
...
static int dpu_vactive0_check_and_reset(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req,
	struct timediff_info *timediff, int ret, bool *panel_check)
{
	...
	cmdlist_idxs = dpu_vactive0_get_cmdlist_idxs(dpufd, pov_req, timediff, ret);
...
static int dpu_vactive0_start_event_check(struct dpu_fb_data_type *dpufd, int timeout)
{
	int ret;
	int times = 0;
	DPU_ATRACE_BEGIN("dpu_vactive0_start_event_check");
	...
	DPU_ATRACE_END("dpu_vactive0_start_event_check");

	return ret;
}

static int dpu_vactive0_start_config_br1(
	struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	...
	if (dpufd->vactive0_start_flag == 0) {
		...
		ret = dpu_vactive0_start_event_check(dpufd, timeout_interval);
		if (ret <= 0) {
			dpufb_get_timestamp(&(time_diff.tv1));

			ret = dpu_vactive0_check_and_reset(dpufd, pov_req, &time_diff, ret, &panel_check);
			if (ret)
				return 0;
...
static int dpu_vactive0_start_config_video(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	...
	if (ret <= 0) {
		...
		dpu_dump_current_info(dpufd);
		if (g_debug_ldi_underflow || g_debug_ovl_online_composer_hold)
			dpu_underflow_dump_cmdlist(dpufd, pov_req_dump, NULL);
...
int dpu_vactive0_start_config(struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_req)
{
	...
	DPU_ATRACE_BEGIN("dpu_vactive0_start_config");
	...
	DPU_ATRACE_END("dpu_vactive0_start_config");
	return ret;
}
