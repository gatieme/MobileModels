...
static void dpu_dump_mediacrg_info(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("Mediacrg CLKDIV:\n");
	for (i = 0; i < 10; i++)
		DPU_FB_INFO("clk div%d: 0x%x\n", i + 1,
			inp32(dpufd->media_crg_base + MEDIA_CLKDIV1 + i * 4));
}

static void dpu_dump_dpp_info(struct dpu_fb_data_type *dpufd)
{
	DPU_FB_INFO("dpp: 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x,0x%x, 0x%x\n",
...
static void dpu_dump_arsr_post_info(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("arsr_post: 0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x, 0x%x\n",
...
#else
	DPU_FB_INFO("scf_post: 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x, 0x%x,0x%x\n",
