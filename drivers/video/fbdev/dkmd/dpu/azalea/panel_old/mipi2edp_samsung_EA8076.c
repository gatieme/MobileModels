...
#endif
#define DTS_COMP_SAMSUNG_EA8076 "hisilicon,mipi2edp_samsung_EA8076"
static int g_lcd_fpga_flag;

#include "../bridge/edp_bridge.h"
extern struct mipi2edp *g_bridge_pdata;
...
static int mipi_samsung_EA8076_panel_set_fastboot(struct platform_device *pdev)
{
	...
	dpu_blpwm_set_backlight(dpufd, dpufd->panel_info.bl_default);
...
static int mipi_samsung_EA8076_panel_on(struct platform_device *pdev)
{
...
#endif
	DPU_FB_INFO("mipi_samsung_EA8076_panel_on ");
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		DPU_FB_ERR("pinfo->lcd_init_step is %d\n", pinfo->lcd_init_step);
...
static int mipi_samsung_EA8076_panel_off(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_uninit_step == LCD_UNINIT_MIPI_HS_SEND_SEQUENCE) {
		...
		DPU_FB_ERR(" pinfo->lcd_uninit_step  is %d\n", pinfo->lcd_uninit_step);
		bridge_power_off();
	} else {
...
static int mipi_samsung_EA8076_panel_set_backlight(struct platform_device *pdev, uint32_t bl_level)
{
    ...
    DPU_FB_DEBUG("mipi_samsung_EA8076_panel_set_backlight");
...
static int mipi_samsung_EA8076_probe(struct platform_device *pdev)
{
	...
	DPU_FB_INFO("gpio_amoled_reset=%d, gpio_amoled_te0=%d, gpio_amoled_vci3v1=%d\n",
	...
	DPU_FB_ERR("lcd_display_type:%d",lcd_display_type);
	if (g_lcd_fpga_flag == 1) {
		...
		DPU_FB_ERR("+ not use FPGA\n");
		pinfo->ldi.h_back_porch = 80;
		pinfo->ldi.h_front_porch = 48;
		pinfo->ldi.h_pulse_width = 32;
		pinfo->ldi.v_back_porch = 27;
		pinfo->ldi.v_front_porch = 3;
		pinfo->ldi.v_pulse_width = 10;
		pinfo->pxl_clk_rate = 206016000;
	...
	pinfo->is_use_pc_backlight = true;
