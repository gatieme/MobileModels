	...
	dss_vote_cmd_t target_vote)
{
	...
	if (ret < 0)
		DPU_FB_ERR("set dss_mmbuf_rate %llu failed, error=%d\n",
...
#if defined(LOW_TEMP_VOLTAGE_LIMIT) && defined(SUPPORT_DCA_WITHOUT_VOLT)
	if ((peri_get_temperature(peri_volt_poll_get(DEV_DSS_VOLTAGE_ID, NULL)) != 0)
		&& (target_vote.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L4)) {
		DPU_FB_WARNING("is_lowtemp, return\n");
		return -EINVAL;
	}
#endif

#ifdef SUPPORT_LOW_FREQ_CHIP
	if (get_module_freq_level(kernel, peri) == FREQ_LEVEL2
		&& target_vote.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L4) {
		DPU_FB_DEBUG("low freq chip not support %u", target_vote.dss_pri_clk_rate);
		return -1;
	}
#endif
...
#ifdef NEED_SWITCH_TEMP_CORE_CLK
static void set_dss_temp_clk(struct dpu_fb_data_type *dpufd,
	dss_vote_cmd_t vote_cmd)
{
	...
	/* if dss pri clk rate set from one of the value 325M/406M/541M to 214M
	...
	if ((vote_cmd.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L1)
		&& ((dpufd->dss_vote_cmd.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L2) ||
			(dpufd->dss_vote_cmd.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L3) ||
		...
		if (ret < 0) {
			DPU_FB_ERR("set dss_pri_clk_rate to temp %llu failed, error = %d\n", vote_cmd.dss_pri_clk_rate, ret);
		} else {
			DPU_FB_DEBUG("set dss_pri_clk_rate to temp %llu succ\n", vote_cmd.dss_pri_clk_rate);
		}
	}
}
#endif
...
#ifdef NEED_SWITCH_TEMP_CORE_CLK
	set_dss_temp_clk(dpufd, vote_cmd);
#endif
...
static void dpu_recovery_external_clk_rate(struct dpu_fb_data_type *dpufd)
{
...
int dpe_set_pixel_clk_rate_on_pll0(struct dpu_fb_data_type *dpufd)
{
	int ret = 0;
	uint64_t clk_rate;
#if defined(CONFIG_PERI_DVFS) && !defined(SUPPORT_DCA_WITHOUT_VOLT)
	...
	if (is_dp_panel(dpufd)) {
		dpu_recovery_external_clk_rate(dpufd);
...
int dpe_set_clk_rate(struct platform_device *pdev)
{
	...
	if (check_low_temperature(dpufd) && tmp.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L4) {
		tmp.dss_pri_clk_rate = DEFAULT_DSS_CORE_CLK_RATE_L3;
		DPU_FB_DEBUG("set low_temperature L3 success\n");
	}
#endif
#endif
...
int dpe_set_common_clk_rate_on_pll0(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->index == BUILT_IN_PANEL_INDEX) {
		DPU_FB_INFO("builtin panel set default pri clk rate\n");
		dpufb_set_default_pri_clk_rate(dpufd_list[BUILT_IN_PANEL_INDEX]);
	} else {
	...
	struct dpu_fb_data_type *dpufd, dss_vote_cmd_t vote_cmd)
{
	...
	if (dpufd == NULL || !IS_BUILTIN_PANEL)
		return 0;
	...
	volt_to_set = dpe_get_voltage_value(dpufd, vote_cmd.dss_voltage_level);
...
#ifdef SUPPORT_LOW_TEMPERATURE_PROTECT
			if (check_low_temperature(dpufd) && copybit_clk_rate.dss_pri_clk_rate == DEFAULT_DSS_CORE_CLK_RATE_L4) {
				copybit_clk_rate.dss_pri_clk_rate = DEFAULT_DSS_CORE_CLK_RATE_L3;
				copybit_clk_rate.dss_mmbuf_rate = DEFAULT_DSS_MMBUF_CLK_RATE_L3;
				DPU_FB_DEBUG("set low_temperature L3 success\n");
			}
#endif
...
#ifdef NEED_SWITCH_TEMP_CORE_CLK
			set_dss_temp_clk(dpufd, other_fb_clk_rate);
#endif
