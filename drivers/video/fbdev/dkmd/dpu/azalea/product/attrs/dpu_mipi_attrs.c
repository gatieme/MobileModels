	...
	const char *buf, size_t count)
{
	...
	if (wait_count >= MIPI_CLK_UPDT_TIMEOUT) {
		DPU_FB_ERR("dsi clk %u store timeout\n", dsi_bit_clk_upt_tmp);
		return -EINVAL;
	}
#endif
	...
	DPU_FB_INFO("switch mipi clk to %d, mipi_updt clk to %d\n",
		pinfo->mipi.dsi_bit_clk_upt, pinfo->mipi_updt.dsi_bit_clk_upt);
...
ssize_t mipi_dsi_bit_clock_upt_show(struct platform_device *pdev, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "%u", pinfo->mipi.dsi_bit_clk_upt);
