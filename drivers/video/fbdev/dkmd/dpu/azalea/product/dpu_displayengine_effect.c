...
#include "dpu_displayengine_struct.h"
...
static struct dpu_fb_data_type *display_engine_check_param(struct fb_info *info, void __user *argp)
{
...
static int display_engine_param_get_begin(struct fb_info *info, void __user *argp,
	struct dpu_fb_data_type **dpufd, display_engine_param_t **de_param, size_t *size)
{
	int ret = 0;
...
static int displaye_engine_param_get_end(void __user *argp, display_engine_param_t *de_param,
	size_t size)
{
	if (argp == NULL || de_param == NULL || size == 0) {
		DPU_FB_ERR("[effect] Invalid input:argp or de_param is NULL,size = %zu!\n", size);
		return -EINVAL;
	}
...
static int display_engine_blc_param_get(struct dpu_fb_data_type *dpufd,
...
static int display_engine_color_rectify_param_get(struct dpu_fb_data_type *dpufd,
	display_engine_color_rectify_param_t *param)
{
	int ret = 0;
	...
	memset(&(dpufd->de_info.lcd_color_oeminfo), 0,
		sizeof(struct disp_lcdbrightnesscoloroeminfo));
...
static int display_engine_share_mem_get(struct dpu_fb_data_type *dpufd,
	display_engine_share_memory_t *param)
{
	if (param == NULL) {
		DPU_FB_ERR("[shmmap] params is null\n");
		return -1;
	}
	param->addr_virt = (uint64_t)(uintptr_t)g_share_mem_virt;
	param->addr_phy = (uint64_t)(uintptr_t)g_share_mem_phy;
	return 0;
}
...
static int display_engine_foldable_info_get(struct dpu_fb_data_type *dpufd,
	display_engine_foldable_info_t *param)
{
	if (dpufd == NULL) {
		DPU_FB_ERR("[effect] dpufd is NULL Pointer\n");
		return -1;
	...
	duration_param.size_original_dbv_acc = sizeof(param->original_dbv_acc);
...
static int display_engine_ddic_cabc_param_get(struct dpu_fb_data_type *dpufd,
...
static int display_engine_ddic_color_param_get(struct dpu_fb_data_type *dpufd,
...
static int display_engine_ddic_rgbw_param_get(struct dpu_fb_data_type *dpufd,
	...
	struct dpu_panel_info *pinfo)
{
	...
	param->oled_type = pinfo->dfr_info.oled_info.oled_type;
#endif
	...
	param->hbm_alpha_gain_support = display_engine_hbm_alpha_gain_get_support();
}
...
static int display_engine_panel_info_get(struct dpu_fb_data_type *dpufd,
	display_engine_panel_info_param_t *param)
{
	struct dpu_panel_info *pinfo = NULL;
	...
	display_engine_common_panel_info_param_t *param)
{
	...
	if (dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE &&
		...
		param->current_panel_id = DISPLAY_OUTER_PANEL_ID;
	else
		param->current_panel_id = DISPLAY_INNEL_PANEL_ID;
#else
...
static int display_engine_param_get_default(struct dpu_fb_data_type *dpufd,
...
static int display_engine_param_get_ddic(struct dpu_fb_data_type *dpufd,
	display_engine_param_t *de_param)
{
	int ret = 0;
...
static int display_engine_param_get_with_id(struct dpu_fb_data_type *dpufd,
	display_engine_param_t *de_param, uint32_t panel_id)
{
	int ret = 0;
	...
	if (de_param->modules & DISPLAY_ENGINE_PANEL_INFO) {
		...
		DPU_FB_DEBUG(
			"effect id=%d name:%s res(x:%d,y:%d)lum(min:%d,max:%d)bl(min:%d,max:%d)\n",
			panel_id, de_param->panel_info.panel_name, de_param->panel_info.width,
			de_param->panel_info.height, de_param->panel_info.minluminance,
			de_param->panel_info.maxluminance, de_param->panel_info.minbacklight,
...
static int display_engine_param_get_without_id(struct dpu_fb_data_type *dpufd,
	display_engine_param_t *de_param)
{
	int ret = 0;
...
int display_engine_param_get(struct fb_info *info, void __user *argp)
{
	...
	if (dpu_check_panel_product_type(dpufd->panel_info.product_type) ||
		dpu_get_panel_product_type() == PANEL_DUAL_PANEL_TYPE) {
#ifdef CONFIG_DPU_FB_V346
		uint32_t innel_panel_id = DISPLAY_INNEL_PANEL_ID;
		uint32_t outer_panel_id = DISPLAY_OUTER_PANEL_ID;
#else
		uint32_t innel_panel_id = DISPLAY_OUTER_PANEL_ID;
		uint32_t outer_panel_id = DISPLAY_INNEL_PANEL_ID;
#endif
		ret = display_engine_param_get_with_id(dpufd, &de_param[DISPLAY_ENGINE_PANEL_MAIN],
			innel_panel_id);
		if (ret)
			goto ERR_OUT;
		ret = display_engine_param_get_with_id(dpufd, &de_param[DISPLAY_ENGINE_PANEL_FULL],
			outer_panel_id);
		if (ret)
			goto ERR_OUT;
	} else {
...
void display_engine_separate_alpha_from_xcc(struct effect_bl_buf *resolved_buf)
{
	...
	DPU_FB_DEBUG("source xcc_table[1]:%d, xcc_table[6]:%d, xcc_table[11]:%d\n",
		xcc_table[xcc_coef_index_01], xcc_table[xcc_coef_index_12],
		xcc_table[xcc_coef_index_23]);
	...
	if (max_coef != 0) {
		...
		xcc_table[xcc_coef_index_01] =
			xcc_table[xcc_coef_index_01] * xcc_default_value / max_coef;
		xcc_table[xcc_coef_index_12] =
			xcc_table[xcc_coef_index_12] * xcc_default_value / max_coef;
		xcc_table[xcc_coef_index_23] =
			xcc_table[xcc_coef_index_23] * xcc_default_value / max_coef;
	} else {
	...
	DPU_FB_DEBUG("dest xcc_table[1]:%d, xcc_table[6]:%d, xcc_table[11]:%d, alpha:%d\n",
		xcc_table[xcc_coef_index_01], xcc_table[xcc_coef_index_12],
		xcc_table[xcc_coef_index_23], alpha);
	if (alpha <= 0) {
...
uint32_t display_engine_get_dither_ctl0_value(void *context, uint32_t dither_ctl0)
{
	struct dpu_panel_info *pinfo = (struct dpu_panel_info*)context;
	uint32_t dither_clt0_value = dither_ctl0;

	if (pinfo == NULL) {
		DPU_FB_ERR("[effect] Invalid input: pinfo is NULL! dither_clt0=%u\n", dither_ctl0);
		return dither_clt0_value;
	}

	if (!pinfo->de_panel_info.dither_effect_support)
		return dither_clt0_value;

	/*
	 * +--------+-----------+--------------+-----------------+-----------------+--------------+
	 * | Content|  reserved | uniform_en   | rgb_shift_mode  |hifreq_noise_mode|  dither_en   |
	 * +--------+-----------+--------------+-----------------+-----------------+--------------+
	 * |  Bits  |   31:5    |     4        |        3        |        2:1      |     0        |
	 * +--------+-----------+--------------+-----------------+-----------------+--------------+
	 *
	 */
	dither_clt0_value =
		((pinfo->de_panel_info.dither_noise_mode & 0x11) << 1) | (dither_ctl0 & 0xFFF9);
	DPU_FB_INFO("[effect] dither_noise_mode=%u dither_clt0=0x%x\n",
		pinfo->de_panel_info.dither_noise_mode, dither_ctl0);
	return dither_clt0_value;
}
