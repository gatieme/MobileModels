...
#include "dpu_displayengine_utils.h"
#include "overlay/dpu_overlay_utils.h"
#ifdef CONFIG_LCD_DBV_MAP_OHOS
#include "dbv_curve_map/dbv_map_ocea_190.h"
#include "dbv_curve_map/dbv_map_noah_090_901.h"
#include "dbv_curve_map/dbv_map_noah_100_704.h"
#include "dbv_curve_map/dbv_map_noah_190.h"
#include "dbv_curve_map/dbv_map_wagr_090_b00.h"
#include "dbv_curve_map/dbv_map_wagr_350_c00.h"
#endif
#include "dpu_fb.h"
...
#define VSYNC_MIPI_EXPIRE_COUNT 100
#endif
...
static bool g_de_fp_open_active;
static bool g_is_mask_layer_on;

static bool dpu_mask_layer_backlight_config_check_parameter_valid(struct dpu_fb_data_type *dpufd,
...
static enum mask_layer_change_status dpu_masklayer_change_status(
	struct dpu_fb_data_type *dpufd, dss_overlay_t *pov_prev, dss_overlay_t *pov)
{
	...
	struct dpu_fb_data_type *dpufd)
{
	...
	if (te_interval == 0) {
		DPU_FB_WARNING("te_interval is 0, used default time!");
		te_interval = te_interval_60_fps_us;
	}
	if (left_thres_us == 0 && right_thres_us == 0) {
		DPU_FB_WARNING("left_thres_us && right_thres_us are 0, delay_time use 0!");
		return 0;
	}
...
static void display_engine_local_hbm_get_dbv_threshold(
	struct dpu_fb_data_type *dpufd, uint32_t *threshold)
{
...
u32 display_engine_local_hbm_get_support(void)
{
	...
	return display_engine_local_hbm_get_support_in_lcd_kit();
#else
	return 0;
#endif
}

u32 display_engine_hbm_alpha_gain_get_support(void)
{
#ifdef CONFIG_LCD_KIT_DRIVER
	return display_engine_hbm_alpha_gain_get_support_in_lcd_kit();
#else
	return 0;
#endif
}
...
static int display_engine_local_hbm_set_dbv(struct dpu_fb_data_type *dpufd, uint32_t dbv)
{
	...
	DPU_FB_INFO("fps:%d\n", fps);
	return display_engine_local_hbm_set_dbv_in_lcd_kit(dpufd, dbv, fps);
#else
	return 0;
#endif
}

static int display_engine_local_hbm_set_dbv_is_on(
	struct dpu_fb_data_type *dpufd, uint32_t dbv, uint32_t dbv_threshold, bool is_on)
{
#if defined(CONFIG_DPU_FB_V346)
	uint32_t fps;
	struct dpu_panel_info *pinfo = NULL;
	...
	DPU_FB_INFO("fps:%d\n", fps);
	return display_engine_local_hbm_set_dbv_is_on_in_lcd_kit(dpufd, dbv, dbv_threshold, fps, is_on);
#else
	return 0;
#endif
}
...
{
#ifdef CONFIG_LCD_KIT_DRIVER
	return display_engine_local_hbm_set_circle_in_lcd_kit(dpufd, is_on);
#else
	return 0;
#endif
}
...
{
#ifdef CONFIG_LCD_KIT_DRIVER
	return display_engine_local_hbm_get_mipi_level();
#else
	return 0;
#endif
}
...
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
static void display_engine_local_hbm_set_dimming_enable(struct dpu_fb_data_type *dpufd, bool enable,
	bool is_power_off)
{
	if (dpu_runmode_is_factory() == 1 || !dpufd->panel_info.adaptive_frm_rate.is_support_ltpo)
		return;
	if (is_power_off && enable) {
		dpufd->panel_info.dfr_info.oled_info.ltpo_info.dimming_enable = true;
		DPU_FB_INFO("lhbm exit during power off and enable ltpo dimming\n");
		return;
	}
	if (enable) {
		dpufd->panel_info.dfr_info.oled_info.ltpo_info.dimming_enable = true;
		dpu_reset_idle_para(dpufd, BL_TYPE);
		DPU_FB_INFO("lhbm exit and enable ltpo dimming\n");
	} else {
		dpu_reset_idle_para(dpufd, BL_TYPE);
		dpufd->panel_info.dfr_info.oled_info.ltpo_info.dimming_enable = false;
		DPU_FB_INFO("lhbm enter and disable ltpo dimming\n");
	}
}

static int display_engine_local_hbm_set_fps(struct dpu_fb_data_type *dpufd)
{
	int fps = dpufd->panel_info.frm_rate_dimming.base_hw_fps;
	DPU_FB_INFO("local hbm fps=%d\n", fps);
	if (fps == FPS_120HZ)
		dpu_send_idle_control_cmd(dpufd, PANEL_LTPO_DSI_CMD_TE_120HZ);
	else if (fps == FPS_90HZ)
		dpu_send_idle_control_cmd(dpufd, PANEL_LTPO_DSI_CMD_TE_90HZ);
	else
		DPU_FB_INFO("local hbm fps=%d not supported\n", fps);
	return 0;
}
#endif
...
#if defined(CONFIG_DPU_FB_V346)
	display_engine_local_hbm_set_dbv_is_on(dpufd, dbv, dbv_threshold, true);
#else
...
static int display_engine_local_hbm_color_set_by_grayscale(struct dpu_fb_data_type *dpufd,
	int grayscale)
{
#if (defined(CONFIG_LCD_KIT_DRIVER) && (defined(CONFIG_EFFECT_TERMINAL_API) || \
	defined(CONFIG_DPU_FB_V346)))
...
#ifdef CONFIG_DPU_FRAME_SKIP_LTPO1
	// enter lhbm and disable dimming
	display_engine_local_hbm_set_dimming_enable(dpufd, false, false);
	display_engine_local_hbm_set_fps(dpufd);
#endif
	fps = pinfo->fps;
	DPU_FB_INFO("grayscale:%d, fps:%d\n", grayscale, fps);
	return display_engine_local_hbm_set_color_by_grayscale(dpufd, fps, grayscale);
#else
	return 0;
#endif
}

static int display_engine_local_hbm_mmie_set_circle_alpha_dbv_color(struct dpu_fb_data_type *dpufd,
	int grayscale)
{
	int ret;
...
static void handle_mask_layer_on(struct dpu_fb_data_type *dpufd,
	const int mask_delay_time_before_fp, struct dpu_fb_panel_data *pdata, const int status)
{
	...
	DPU_FB_INFO("ENTER HBM DelayEndTime = %ld us, LastTe= %ld us, delaytime = %ld us",
		ktime_to_us(ktime_get()), ktime_to_us(dpufd->te_timestamp), fp_delay_time_us);

	if (status == MASK_LAYER_SCREENON_ADDED) {
		pdata->lcd_set_hbm_for_screenon(dpufd->pdev, fp_hbm_enter_flag);
	} else {
		pdata->lcd_set_backlight_by_type_func(dpufd->pdev, fp_hbm_enter_flag);
...
#if defined(CONFIG_DPU_FB_AOD) && !defined(CONFIG_DPU_FB_V410)
	/*
...
static void display_engine_local_hbm_exit_circle_alpha_dbv(struct dpu_fb_data_type *dpufd)
{
...
#if defined(CONFIG_DPU_FB_V346)
	int dbv = display_engine_get_mipi_level();
	display_engine_local_hbm_set_dbv_is_on(dpufd, dbv, 0, false);
#else
	...
	g_is_mask_layer_on = false;
#if defined(CONFIG_DPU_FB_V346) && defined(CONFIG_DPU_FRAME_SKIP_LTPO1)
	// lhbm exit and enable ltpo dimming
	display_engine_local_hbm_set_dimming_enable(dpufd, true, false);
#endif
}

static void handle_mask_layer_off(struct dpu_fb_data_type *dpufd,
	const int mask_delay_time_after_fp, struct dpu_fb_panel_data *pdata, const int status)
{
	uint64_t fp_delay_time_us = 0;
	...
	DPU_FB_INFO("EXIT HBM DelayEndTime = %ld us, LastTe= %ld us, delaytime = %ld us",
		ktime_to_us(ktime_get()), ktime_to_us(dpufd->te_timestamp), fp_delay_time_us);
	...
	struct dpu_fb_data_type *dpufd, int *backlight_out)
{
...
#ifdef CONFIG_LCD_DBV_MAP_OHOS
	display_engine_brightness_convert_dbv_map(dpufd, backlight_out);
#endif
	...
	if (dpufd->panel_info.dbv_max > 0 && (uint32_t)(*backlight_out) > dpufd->panel_info.dbv_max)
		*backlight_out = dpufd->panel_info.dbv_max;
}

void display_engine_brightness_convert_dbv_map(
	struct dpu_fb_data_type *dpufd, int *backlight_out)
{
	if (dpufd->panel_info.panel_name == NULL) {
		DPU_FB_ERR("[effect] no panel name\n");
		return;
	}
#ifdef CONFIG_LCD_DBV_MAP_OHOS
	if (!strcmp(dpufd->panel_info.panel_name, "190 205 1 6.506' 1080 x 2376") ||
		!strcmp(dpufd->panel_info.panel_name, "OCEA_190 c06 1 6'506 CMD TFT 1080 x 2376")) {
		DPU_FB_INFO("[effect] dbv_map_ocea_190 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_ocea_190[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_ocea_190 bl out:%d\n", *backlight_out);
	}
	if (!strcmp(dpufd->panel_info.panel_name, "090 901 0 6.756' 1344 x 2772")) {
		DPU_FB_INFO("[effect] dbv_map_noah_090_901 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_noah_090_901[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_noah_090_901 bl out:%d\n", *backlight_out);
	}
	if (!strcmp(dpufd->panel_info.panel_name, "100 704 0 6.756' 1344 x 2772")) {
		DPU_FB_INFO("[effect] dbv_map_noah_100_704 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_noah_100_704[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_noah_100_704 bl out:%d\n", *backlight_out);
	}
	if (!strcmp(dpufd->panel_info.panel_name, "190 206 0 6.756' 1344 x 2772") ||
		!strcmp(dpufd->panel_info.panel_name, "190 c00 0 6.756' 1344 x 2772") ||
		!strcmp(dpufd->panel_info.panel_name, "190 20e 0 6.756' 1344 x 2772")) {
		DPU_FB_INFO("[effect] dbv_map_noah_190 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_noah_190[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_noah_190 bl out:%d\n", *backlight_out);
	}
	if (!strcmp(dpufd->panel_info.panel_name, "WAGR_090 b00 0 12.6' 2560 x 1600")) {
		DPU_FB_INFO("[effect] dbv_map_wagr_090_b00 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_wagr_090_b00[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_wagr_090_b00 bl out:%d\n", *backlight_out);
	}
	if (!strcmp(dpufd->panel_info.panel_name, "WAGR_350 c00 0 12.6' 2560 x 1600") ||
		!strcmp(dpufd->panel_info.panel_name, "WAGR_350 c01 0 12.6' 2560 x 1600")) {
		DPU_FB_INFO("[effect] dbv_map_wagr_350_c00 bl in:%d\n", *backlight_out);
		*backlight_out = dbv_map_wagr_350_c00[*backlight_out];
		DPU_FB_INFO("[effect] dbv_map_wagr_350_c00 bl out:%d\n", *backlight_out);
	}
#endif
}
	...
	dss_overlay_t *pov_req, bool *masklayer_maxbacklight_flag)
{
	...
	parameter_valid = dpu_mask_layer_backlight_config_check_parameter_valid(dpufd, pov_req_prev,
		pov_req);
	...
	if (!dpufd->hbm_is_opened && (status == MASK_LAYER_COMMON_ADDED
		...
		*masklayer_maxbacklight_flag = false;
		...
		DPU_FB_WARNING("circle visible masklayer_change_status = %d\n", status);
	}
}
...
static void dpufb_dc_skip_even_frame(struct dpu_fb_data_type *dpufd)
{
	...
	if (!lcd_kit_is_current_frame_ok_to_set_backlight(dpufd)) {
		real_te_interval = get_real_te_interval(dpufd);
		DPU_FB_INFO("need to skip one frame, real_te_interval = %ld", real_te_interval);
		usleep_range(real_te_interval, real_te_interval);
	}
#else
	(void)dpufd;
...
static void dpufb_dc_backlight_param_update(struct dpu_fb_data_type *dpufd)
{
	...
	pinfo = display_engine_get_panel_info(dpufd, dpufd->panel_info.disp_panel_id);
	if (pinfo == NULL) {
		DPU_FB_ERR("pinfo is NULL\n");
		return;
	}
	...
	int force_alpha_enable)
{
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (display_engine_alpha_get_support() == ALPHA_NOT_SUPPORT) {
		DPU_FB_DEBUG("alpha not support\n");
		return ret;
	}
	...
	if (force_alpha_enable == NO_FORCING) {
		if (alpha != ALPHA_DEFAULT)
			ret = display_engine_enter_ddic_alpha(dpufd, alpha);
		else
			ret = display_engine_exit_ddic_alpha(dpufd, alpha);
	...
	if (dpu_check_panel_product_type(dpufd->panel_info.product_type) &&
		dpufd->alsc.cb_func.send_ddic_alpha)
		dpufd->alsc.cb_func.send_ddic_alpha(alpha);
#endif
...
#if (defined(CONFIG_LCD_KIT_DRIVER) && (defined(CONFIG_EFFECT_TERMINAL_API) || \
	defined(CONFIG_DPU_FB_V346)))
	return display_engine_alpha_get_support_in_lcd_kit();
#else
	return 0;
#endif
}
...
{
#if (defined(CONFIG_LCD_KIT_DRIVER) && defined(CONFIG_EFFECT_TERMINAL_API))
	return display_engine_get_force_delta_bl_update_support();
#else
	return 0;
#endif
}
...
{
	bool need_sync = (dpufd->de_info.amoled_param.dc_brightness_dimming_enable_real !=
		dpufd->de_info.amoled_param.dc_brightness_dimming_enable) ||
		(dpufd->de_info.amoled_param.amoled_diming_enable !=
		dpufd->de_info.amoled_param.amoled_enable_from_hal);
	uint64_t bl_delay_time;
	uint64_t real_te_interval;
	DPU_ATRACE_BEGIN("dpufb_dc_backlight_config");
	if (need_sync && dpufd->dirty_region_updt_enable == 0 &&
		...
		if (dpufd->bl_level > 0 && dpufd->backlight.bl_updated) {
			...
			if (dpufd->de_info.amoled_param.dc_backlight_delay_us == 0)
				return;
			real_te_interval = get_real_te_interval(dpufd);
			bl_delay_time = correct_time_based_on_fps(
				real_te_interval,
				dpufd->de_info.amoled_param.dc_backlight_delay_us);
			usleep_range(bl_delay_time, bl_delay_time);
		} else {
			dpufb_dc_backlight_param_update(dpufd);
		}
	}
	DPU_ATRACE_END("dpufb_dc_backlight_config");
}

/* Display Add: active te signal when using fingeprint , fix fingeprint blink question */
void de_open_update_te()
{
	...
	if (!g_de_fp_open_active) {
		...
		g_de_fp_open_active = true;

		if (dpufd->de_updata_te_wq != NULL)
			queue_work(dpufd->de_updata_te_wq, &dpufd->de_updata_te_work);
	}
}
...
void de_ctrl_ic_dim(struct dpu_fb_data_type *dpufd)
{
	...
	DPU_FB_INFO("ic dimmming is enable : %d", pdata->panel_info->ic_dim_ctrl_enable);
...
void de_open_ic_dim(struct work_struct *work)
{
	...
	dpufd = container_of(work, struct dpu_fb_data_type, de_open_ic_dim_work);
	if (dpufd == NULL) {
		DPU_FB_ERR("[effect] dpufd is NULL\n");
		return;
	}
...
void de_set_hbm_func(struct work_struct *work)
{
	...
	dpufd = container_of(work, struct dpu_fb_data_type, de_set_hbm_func_work);
	if (dpufd == NULL) {
		DPU_FB_ERR("dpufd is NULL\n");
		return;
	}
	...
	if (param->mmi_level != last_mmi_level) {
		...
		if (pdata->lcd_set_hbm_for_mmi_func != NULL) {
			/* clear ddic alpha if needed */
			display_engine_alpha_set(dpufd, ALPHA_DEFAULT);
			pdata->lcd_set_hbm_for_mmi_func(dpufd->pdev, param->mmi_level);
		} else {
		...
		dpufd->de_info.hbm_dimming = (param->dimming == 1) ? true : false;
...
static void init_hbm_workqueue(struct dpu_fb_data_type *dpufd)
{
	...
	if (pdata->panel_info->ic_dim_ctrl_enable ||
		...
		dpufd->de_updata_te_wq = create_singlethread_workqueue(wq_name);
		if (dpufd->de_updata_te_wq == NULL) {
...
static void set_hbm_by_workquen(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->de_updata_te_wq != NULL)
		queue_work(dpufd->de_updata_te_wq, &dpufd->de_set_hbm_func_work);
}

static void display_engine_set_mmi_fp_bl(struct dpu_fb_data_type *dpufd,
	uint32_t mmi_level)
{
	struct dpu_fb_panel_data *pdata = NULL;
	...
	display_engine_hbm_param_t *param)
{
	...
	if (param->mmi_level != last_mmi_level) {
		DPU_FB_INFO("mmi level %d->%d\n", last_mmi_level, param->mmi_level);
		last_mmi_level = param->mmi_level;
		if (!dpufd->panel_power_on || (pdata->lcd_set_hbm_for_mmi_func == NULL)) {
			up(&dpufd->blank_sem);
			return -1;
		} else {
		...
		DPU_FB_INFO("hbm level %d->%d\n",
			dpufd->de_info.last_hbm_level, dpufd->de_info.hbm_level);
		dpufd->de_info.hbm_dimming = (param->dimming == 1) ? true : false;
		if (!dpufd->panel_power_on || (pdata->lcd_hbm_set_func == NULL)) {
			up(&dpufd->blank_sem);
			return -1;
		} else {
			dpufb_activate_vsync(dpufd);
	...
	struct display_engine_ddic_local_hbm_param *param)
{
	...
	if (!dpufd->panel_power_on) {
		...
		if (param->circle_grayscale == 0)
			display_engine_local_hbm_set_dimming_enable(dpufd, true, true);
#endif
		DPU_FB_ERR("fb%d, panel power off!\n", dpufd->index);
		ret = -1;
		goto err_out;
...
/* Creat masklayer backlight notify workqueue for fingerprint. */
static void creat_masklayer_backlight_notify_wq(struct dpu_fb_data_type *dpufd)
{
	...
	INIT_WORK(&dpufd->masklayer_backlight_notify_work,
		dpufb_masklayer_backlight_notify_handler);
}
...
{
#if defined(CONFIG_DPU_FB_V501) || defined(CONFIG_DPU_FB_V600) || \
	defined(CONFIG_DPU_FB_V510) || defined(CONFIG_DPU_FB_V345) || \
	defined(CONFIG_DPU_FB_V360) || defined(CONFIG_DPU_FB_V346)
...
void display_engine_eink_suspend(struct dpu_fb_data_type *dpufd)
{
	...
	if ((!IS_BUILTIN_PANEL) || !dpufd->panel_power_on)
		return;

	panel_ops = lcd_kit_panel_get_ops();
	if (!panel_ops || !panel_ops->eink_suspend)
		return;
	vsync_ctrl = &(dpufd->vsync_ctrl);

	spin_lock(&vsync_ctrl->spin_lock);
	if (vsync_ctrl->vsync_mipi_expire_count > 0)
		vsync_ctrl->vsync_mipi_expire_count--;
	if (vsync_ctrl->vsync_mipi_expire_count == 0) {
		if(panel_ops->eink_judgement_suspend()) {
			spin_unlock(&vsync_ctrl->spin_lock);
			return;
		}
	}
	if (vsync_ctrl->vsync_mipi_expire_count == 0 && vsync_ctrl->vsync_mipi_enabled)
		panel_ops->eink_suspend();
	spin_unlock(&vsync_ctrl->spin_lock);
	return;
}
...
void display_engine_eink_resume(struct dpu_fb_data_type *dpufd)
{
	...
	vsync_ctrl->vsync_mipi_expire_count = VSYNC_MIPI_EXPIRE_COUNT;
	panel_ops->eink_resume();
	vsync_ctrl->vsync_mipi_enabled = 1;
...
void display_engine_eink_count_init(struct dpu_fb_data_type *dpufd)
{
	...
	panel_ops = lcd_kit_panel_get_ops();
	if (!panel_ops || !panel_ops->eink_resume || !panel_ops->eink_suspend)
		return;
	vsync_ctrl = &(dpufd->vsync_ctrl);
