				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "Power:%llu, Current:%llu, Num:%d\n",
				acc_power,
...
#else
static void ina231_monitor_timer_func(struct timer_list * list)
{
	unsigned long expires = 0;
	struct ina231_data *idata = from_timer(idata, list, ina321_timer);
#endif
...
#else
	timer_setup(&(idata->ina321_timer), ina231_monitor_timer_func, 0);
#endif
