...
int32_t request_dm_client(const struct dkmd_base_frame *frame, struct dkmd_network *network)
{
	...
	if (unlikely(!dm_param))
		return -1;
	set_dm_scene_info(frame->scene_id, frame->scene_mode, dm_param);
	...
	const union dkmd_opr_id *next_opr_ids, uint32_t next_opr_ids_num)
{
	...
	if (cmd_data->get_cfg_addr &&
		...
		dpu_pr_err("get_cfg_addr fail");
		return -1;
	}

	if (unlikely(request_opr_cfg_client(frame, cmd_data))) {
		dpu_pr_err("request_opr_cfg_client fail");
		return -1;
	}
	...
	const struct dkmd_pipeline *pipeline, union dkmd_opr_id *pre_opr_id)
{
	...
	for (i = 0; i < pipeline->opr_num - 1; ++i) {
		...
		if (unlikely(!cmd_data)) {
			dpu_pr_err("gen_pipeline_cmd set_opr_cmd_data fail");
			return -1;
		}

		if (unlikely(gen_opr_cmd(cmd_data, frame, pipeline->block_layer, *pre_opr_id, &pipeline->opr_ids[i + 1], 1))) {
			dpu_pr_err("gen_pipeline_cmd gen_opr_cmd fail");
			return -1;
		}
	...
	const struct dkmd_pipeline *pre_pipeline, const struct dkmd_pipeline *post_pipelines, uint32_t post_pipelines_num)
{
	...
	if (unlikely(post_pipelines_num >= POST_PIPELINE_MAX_NUM)) {
		dpu_pr_err("post_pipelines_num = %u is out of range", post_pipelines_num);
		return -1;
	}
	...
	union dkmd_opr_id pre_pipeline_last_opr_id, const struct dkmd_pipeline *post_pipeline)
{
	...
	if (unlikely(!cmd_data)) {
		dpu_pr_err("generate post pipeline set_opr_cmd_data fail");
		return -1;
	}

	if (unlikely(gen_opr_cmd(cmd_data, frame, post_pipeline->block_layer, pre_pipeline_last_opr_id, NULL, 0))) {
		dpu_pr_err("generate post pipeline gen_opr_cmd fail");
		return -1;
	}
...
void set_frame_cmd_data(const struct dkmd_base_frame *frame, const struct dkmd_network *network)
{
	struct dkmd_pipeline *pipeline = NULL;
	struct dkmd_base_layer *block_layer = NULL;
	uint32_t xres = 0;
	uint32_t yres = 0;
	uint32_t i;
	...
	block_layer = pipeline->block_layer;
	xres = rect_width(&frame->layers[0].src_rect);
	...
	for (i = 0; i < DPU_DBUF_REG_NUM; i++)
		dkmd_set_reg(frame->scene_id, network->reg_cmdlist_id, dbuf_info->addr_offset[i], dbuf_info->value[i]);
}