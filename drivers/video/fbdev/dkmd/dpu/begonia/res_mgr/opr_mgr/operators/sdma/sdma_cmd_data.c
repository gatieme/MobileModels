	...
	const struct opr_cmd_data *pre_cmd_data, const struct opr_cmd_data **next_cmd_datas, uint32_t next_oprs_num)
{
	...
	opr_set_sdma_layer_fmt(base_layer->format, base_layer->compress_type,
	...
	dpu_pr_debug("width=%u height=%u iova=0x%llx stride=%u format=%d compress_type=%d",
		width, height, iova, base_layer->stride, base_layer->format, base_layer->compress_type);
