...
#ifdef CONFIG_FS_TOPIO_TRACE
static void erofs_trace_read_completion(struct bio *bio)
{
	struct page *first_page = NULL;
	if (bio->bi_io_vec != NULL) {
		first_page = bio->bi_io_vec[0].bv_page;
		if (first_page && first_page->mapping)
			trace_android_fs_dataread_end(first_page->mapping->host,page_offset(first_page),bio->bi_iter.bi_size);
	}
}

static void erofs_submit_bio_read(struct bio *bio)
{
	struct page *first_page = NULL;
	if (trace_android_fs_dataread_start_enabled() && blk_is_topio() &&
		bio && (bio->bi_io_vec != NULL)) {
		first_page = bio->bi_io_vec[0].bv_page;
...
#ifdef CONFIG_FS_TOPIO_TRACE
	if (trace_android_fs_dataread_end_enabled() && blk_is_topio())
		erofs_trace_read_completion(bio);
#endif
