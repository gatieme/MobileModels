		...
		int num_entries)
{
	...
	while (clu.dir != EXFAT_EOF_CLUSTER) {
		...
		for (; i < dentries_per_clu; i++, dentry++) {
			...
			/*
			 * one seagate rigid disk was unused atfter Edition promotion,
			 * because of fond bogus detry beyond used empty group
			 */
				if (hint_femp->eidx != EXFAT_HINT_NONE &&
					hint_femp->count == CNT_UNUSED_HIT) {
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
		if (sdcard_dclient && !dsm_client_ocuppy(sdcard_dclient)) {
			dsm_client_record(sdcard_dclient, "non-zero size file starts with zero cluster (size : %llu, p_dir : %u, entry : 0x%08x)",
					i_size_read(dir), ei->dir.dir, ei->entry);
			dsm_client_notify(sdcard_dclient, DSM_SDCARD_FILESYSTEM_ERR);
		}
#endif
