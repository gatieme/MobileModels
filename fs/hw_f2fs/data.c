						...
						int create, int flag)
{
	...
	if (!create && f2fs_lookup_extent_cache(inode, pgofs, &ei)) {
		...
		if (flag == F2FS_GET_BLOCK_DIO) {
			 ...
			 * META_MAPPING. However, due to cached pages in
			 * META_MAPPING, there is an issue where newly written
			 * pages are lost by IPU or DIO writes.
			 */
			invalidate_mapping_pages(META_MAPPING(sbi),
...
#ifdef CONFIG_F2FS_FS_COMPRESSION_EX
	if (f2fs_is_compressed_inode(inode)) {
		ret = f2fs_decompress_file_internal(file, SWAP_FILE, 0);
		if (ret)
			return -EINVAL;
	}
#else
