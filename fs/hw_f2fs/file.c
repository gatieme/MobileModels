...
int f2fs_revoke_deduped_inode(struct inode *dedup, const char *revoke_source)
{
	...
	if (inner_inode) {
		...
		f2fs_err(sbi, "dedup ino[%lu] inner_inode is NULL", dedup->i_ino);
		return -EINVAL;
	}

	if (IS_ENCRYPTED(inner_inode)) {
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0))
		err = fscrypt_prepare_encrypt_info(inner_inode);
#else
		err = fscrypt_require_key(inner_inode);
#endif
		if (err)
			return err;
	}
...
static int f2fs_ioc_set_hp_file_wrapper(struct file *filp, unsigned long arg)
{
	...
	if (is_inode_flag_set(inode, FI_COMPRESS_RELEASED)) {
		ret = f2fs_reserve_compress_blocks_internal(inode, SET_HP_FILE);
		if (ret) {
			inode_unlock(inode);
			mnt_drop_write_file(filp);
			return ret;
...
static int f2fs_ioc_resize_fs(struct file *filp, unsigned long arg)
{
	...
	return f2fs_resize_fs(filp, param.len);
}
...
static int get_saved_blocks(struct dnode_of_data *dn, pgoff_t count)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(dn->inode);
	unsigned int saved_blocks = 0;
	...
	while (count) {
		...
		saved_blocks += (cluster_size - valid_blocks);
next:
		count -= cluster_size;
	}
...
out:
	inode_unlock(inode);
	ret = put_user(saved_blocks, (u64 __user *)arg);
	return ret;
}
