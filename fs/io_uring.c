...
static struct io_ring_ctx *io_ring_ctx_alloc(struct io_uring_params *p)
{
	...
	CALL_HCK_VH(hyperframe_ctx_alloc, ctx, &ret);
	if (ret)
		goto err;
...
static struct io_uring_cqe *io_get_cqring(struct io_ring_ctx *ctx)
{
	...
	if (tail - READ_ONCE(rings->cq.head) == rings->cq_ring_entries) {
		CALL_HCK_VH(hyperframe_err_record, __func__,
			0, __LINE__, tail, READ_ONCE(rings->cq.head));
		return NULL;
	}
				       ...
				       struct files_struct *files)
{
	...
	CALL_HCK_VH(hyperframe_err_record, __func__, 0, __LINE__,
...
static void __io_free_req(struct io_kiocb *req)
{
	...
	CALL_HCK_VH(hyperframe_free_req, req);
...
void hf_io_req_complete(struct io_kiocb *req, long res)
{
	io_req_complete(req, res);
	io_put_req(req);
}
#endif
...
static int io_read_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	CALL_HCK_VH(hyperframe_rw_prep, req, sqe, &ret);
	if (ret)
		return ret;
...
static int __io_openat_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	req->flags |= REQ_F_NEED_CLEANUP;
	CALL_HCK_VH(hyperframe_openat_prep, req, sqe);
	return 0;
}
...
static int io_close_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
	...
	CALL_HCK_VH(hyperframe_close_prep, req, sqe);
	return 0;
}
			...
			struct io_comp_state *cs)
{
	...
	CALL_HCK_VH(hyperframe_issue_sqe, req, &ret);
	if (ret)
		return ret;
...
again:
	...
	if (ret == -EAGAIN && !(req->flags & REQ_F_NOWAIT)) {
		...
		CALL_HCK_VH(hyperframe_complete_sqe, opcode, &ret);
		if (ret)
			return;
		/* drop submission reference */
...
static int io_submit_sqes(struct io_ring_ctx *ctx, unsigned int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		CALL_HCK_VH(hyperframe_init_req, req, sqe, &err);
		if (unlikely(err)) {
fail_req:
			io_put_req(req);
			      ...
			      struct io_uring_params *p)
{
	...
	if (!(p->flags & IORING_SETUP_ATTACH_WQ)) {
		...
		if (IS_ERR(ctx->io_wq)) {
			...
			ctx->io_wq = NULL;
			CALL_HCK_VH(hyperframe_err_record, __func__, ret, __LINE__, 0, 0);
		}
		return ret;
	}
...
{
	gfp_t gfp = GFP_KERNEL | __GFP_ZERO | __GFP_NOWARN | __GFP_COMP;
...
static void io_ring_ctx_free(struct io_ring_ctx *ctx)
{
	...
	kfree(ctx->cancel_hash);
	kmem_cache_free(req_cachep, ctx->fallback_req);
	CALL_HCK_VH(hyperframe_ctx_free, ctx);
	kfree(ctx);
}
...
static int io_uring_install_fd(struct io_ring_ctx *ctx, struct file *file)
{
	...
	if (fd < 0) {
		CALL_HCK_VH(hyperframe_err_record, __func__, fd, __LINE__, 0, 0);
		return fd;
	}
...
static struct file *io_uring_get_file(struct io_ring_ctx *ctx)
{
	...
	if (IS_ERR(file)) {
		sock_release(ctx->ring_sock);
		ctx->ring_sock = NULL;
		CALL_HCK_VH(hyperframe_err_record, __func__, PTR_ERR(file), __LINE__, 0, 0);
	} else {
			   ...
			   struct io_uring_params __user *params)
{
	...
	if (limit_mem) {
		...
		if (ret) {
			CALL_HCK_VH(hyperframe_err_record, __func__,
				ret, __LINE__, (int)p->sq_entries, (int)p->cq_entries);
			free_uid(user);
			return ret;
		}
	}
	...
	if (!ctx) {
		if (limit_mem)
			__io_unaccount_mem(user, ring_pages(p->sq_entries,
								p->cq_entries));
		free_uid(user);
		CALL_HCK_VH(hyperframe_err_record, __func__, -ENOMEM, __LINE__, 0, 0);
		return -ENOMEM;
	}
	...
	if (copy_to_user(params, p, sizeof(*p))) {
		ret = -EFAULT;
		CALL_HCK_VH(hyperframe_err_record, __func__, -EFAULT, __LINE__, 0, 0);
		goto err;
	}
	...
	return ret;
err:
	CALL_HCK_VH(hyperframe_err_record, __func__, ret,
		(int)p->sq_entries, (int)p->cq_entries, (int)p->flags);
	io_disable_sqo_submit(ctx);
	io_ring_ctx_wait_and_kill(ctx);
	return ret;
}
