		...
		struct iomap_dio *dio, struct bio *bio, loff_t pos)
{
	...
	blk_throtl_get_quota(dio->iocb->ki_filp->f_inode->i_sb->s_bdev, bio->bi_iter.bi_size,
...
#ifdef CONFIG_CGROUP_IOLIMIT
		if (dio->flags & IOMAP_DIO_WRITE)
			io_write_bandwidth_control(PAGE_SIZE * nr_pages);
		else
			io_read_bandwidth_control(PAGE_SIZE * nr_pages);
#endif
