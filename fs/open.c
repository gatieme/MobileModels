...
long ksys_access(const char __user *filename, int mode)
{
			   ...
			   struct open_how *how)
{
	...
	if (!smt_privileged_user()) {
		char path[64] = { 0 };

		sprintf(path, "/sys/devices/system/cpu/cpu%d", num_possible_cpus() - num_secondary_cpus() - 1);
		if (strstr(tmp->name, path))
			current->cpuinfo_fd = fd;
	}
#endif
	...
	if (fd >= 0) {
		...
		if (!(how->flags & O_CLOEXEC))
			binder_file_dup_debug(fd, f, "open binder without CLOEXEC!");
#endif
	}
	putname(tmp);
	return fd;
...
#ifdef CONFIG_SCHED_SMT_USER_HIDE
	if (current->cpuinfo_fd == fd)
		current->cpuinfo_fd = -1;
#endif
