 ...
 * file: bfmr_public.h
 *
 * define the basic public
 *         enum/macros/interface for BFMR (Boot Fail Monitor and Recovery)
 *
...
enum bfmr_platform_name {
	COMMON_PLATFORM = 0,
	HONOR_PLATFORM,
	QUALCOMM_PLATFORM,
};

enum bfmr_boot_stage {
	PBL_STAGE = 0,
	/* BL1 -- bootloader stage 1, it's xloader for legacy,sbl1 for qualcomm */
	BL1_STAGE,
	/* BL2 -- bootloader stage 2, it's fastboot for legacy,lk for qualcomm */
	BL2_STAGE,
	KERNEL_STAGE,
	NATIVE_STAGE,
...
enum bfm_errno_code {
	...
	/* PL1 -- Platform 1 -- it's only for legacy */
	BL1_PL1_START = (BL1_STAGE << 24) | (HONOR_PLATFORM << 16),
	BL1_VRL_LOAD_FAIL,
	BL1_VRL_VERIFY_FAIL,
	...
	/* PL1 -- Platform 1 -- it's only for legacy */
	BL2_PL1_START = (BL2_STAGE << 24) | (HONOR_PLATFORM << 16),
	BL2_PL1_OCV_ERROR,
	...
	KERNEL_PL1_START = (KERNEL_STAGE << 24) | (HONOR_PLATFORM << 16),
	...
	/* maple system boot failed cause by dlopen libandroidmpl-rt.so */
	MAPLE_ESCAPE_DLOPEN_MRT,
	/* maple sys'mygote'and'maple sys server'crash 4 times at 4 minutes */
	MAPLE_ESCAPE_CRITICAL_CRASH,
	/* mygote data caused crash */
	MAPLE_CRITICAL_CRASH_MYGOTE_DATA,
	/* sysvr data caused crash */
	MAPLE_CRITICAL_CRASH_SYSTEMSERVER_DATA,
	/* android frameworkl stage's bootFail errors */
	ANDROID_FRAMEWORK_ERRNO_START = (ANDROID_FRAMEWORK_STAGE << 24) |
					(COMMON_PLATFORM << 16),
...
enum bfmr_detail_stage {
	...
	BL1_PL1_STAGE_START = (BL1_STAGE << 24) | (HONOR_PLATFORM << 16),
	BL1_PL1_STAGE_DDR_INIT_FAIL,
	BL1_PL1_STAGE_DDR_INIT_OK,
	BL1_PL1_STAGE_STORAGE_INIT_FAIL,
	BL1_PL1_STAGE_STORAGE_INIT_OK,
	...
	BL2_STAGE_STORAGE_INIT_START,
	BL2_STAGE_STORAGE_INIT_FAIL,
	BL2_STAGE_STORAGE_INIT_OK,
	BL2_STAGE_END,

	BL2_PL1_STAGE_START = (BL2_STAGE << 24) | (HONOR_PLATFORM << 16),
	...
	ANDROID_FRAMEWORK_STAGE_START = (ANDROID_FRAMEWORK_STAGE << 24) |
					(COMMON_PLATFORM << 16),
	STAGE_ZYGOTE_START,
	STAGE_VM_START,
	...
	STAGE_SYSTEM_SERVER_DEX2OAT_START = ANDROID_FRAMEWORK_STAGE_START +
					    0x20 + 0x01,
...
#define bfmr_is_bl1_errno(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)BL1_STAGE) << 24))
#define bfmr_is_bl2_errno(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)BL2_STAGE) << 24))
#define bfmr_is_kernel_errno(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)KERNEL_STAGE) << 24))
#define bfmr_is_native_errno(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)NATIVE_STAGE) << 24))
#define bfmr_is_android_framework_errno(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)ANDROID_FRAMEWORK_STAGE) << 24))

#define bfmr_is_boot_success(x) ((x) == STAGE_BOOT_SUCCESS)
#define bfmr_is_bl1_stage(x) \
	((((x) & (0xFF << 24)) == (((unsigned int)BL1_STAGE << 24))) &&\
	 (!bfmr_is_boot_success(x)))
#define bfmr_is_bl2_stage(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)BL2_STAGE) << 24))
#define bfmr_is_kernel_stage(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)KERNEL_STAGE) << 24))
#define bfmr_is_native_stage(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)NATIVE_STAGE) << 24))
#define bfmr_is_android_framework_stage(x) \
	(((x) & (0xFF << 24)) == (((unsigned int)ANDROID_FRAMEWORK_STAGE) << 24))

#define bfmr_get_boot_stage_from_bootfail_errno(x) \
	((((unsigned int)(x)) >> 24) & (0xFF))
#define bfmr_get_boot_stage_from_detail_stage(x) \
	((((unsigned int)(x)) >> 24) & (0xFF))
