...
#define MEMCHECK_MEM_NAME_MAX		64
...
#define LOGGER_MEMCHECK_GET_ION_MEM	_IO(__MEMCHECKIO, 10)

#define LOGGER_MEMCHECK_MIN		LOGGER_MEMCHECK_GET_MEMSTAT
#define LOGGER_MEMCHECK_MAX		LOGGER_MEMCHECK_GET_ION_MEM
...
#define IDX_PSS_EXTRA_START		(IDX_KERN_END + 10)
#define IDX_USER_PSS_MMAP		(IDX_PSS_EXTRA_START + 1)
#define IDX_USER_PSS_FDTRACK		(IDX_PSS_EXTRA_START + 3)
#define IDX_PSS_EXTRA_END		IDX_USER_PSS_FDTRACK

#define NUM_ALL_MAX			(IDX_PSS_EXTRA_END + 1)
...
#define MTYPE_USER_PSS_MMAP		(1 << IDX_USER_PSS_MMAP)
#define MTYPE_USER_PSS_FDTRACK		(1 << IDX_USER_PSS_FDTRACK)
...
#define MTYPE_USER_PSS	(MTYPE_USER_PSS_JAVA | MTYPE_USER_PSS_NATIVE | MTYPE_USER_PSS_MMAP)
...
#define MTYPE_USER	(MTYPE_USER_PSS | MTYPE_USER_VSS | MTYPE_USER_ION | MTYPE_USER_PSS_FDTRACK)
...
#define ADDR_MMAP_ENABLE		(1 << 8)
#define ADDR_MMAP_DISABLE		(1 << 9)
#define ADDR_MMAP_SAVE			(1 << 10)
#define ADDR_MMAP_CLEAR			(1 << 11)
#define ADDR_FD_TRACK_ENABLE		(1 << 20)
#define SIGNO_VALUE_SHIFT		16
#define MAX_TAG_TYPE_NUM		4
...
struct mem_tag_info {
	u32 type;
	struct mem_tag tag_body[MAX_TAG_NUM];
};
struct memstat_all {
	u32 magic;
	u32 id;
	...
	u64 mmap_pss;
	u16 tag_type_num;
	struct mem_tag_info tag_list[0];
};
...
struct track_cmd {
	...
	struct sample_info {
		...
		char mem_name[MEMCHECK_MEM_NAME_MAX];
	} sample_info;
};
