...
#define ADAPTER_PROTOCOL_POWER_CURVE_LEVEL      10
#define ADAPTER_PROTOCOL_POWER_CURVE_SIZE       2

enum adapter_protocol_type {
	ADAPTER_PROTOCOL_BEGIN = 0,
	ADAPTER_PROTOCOL_SCP = ADAPTER_PROTOCOL_BEGIN, /* honor scp */
	ADAPTER_PROTOCOL_FCP, /* honor fcp */
	ADAPTER_PROTOCOL_PD,
	ADAPTER_PROTOCOL_UVDM,
	ADAPTER_PROTOCOL_PPS,
	ADAPTER_PROTOCOL_ADSP_SCP,
	ADAPTER_PROTOCOL_BC12,
	ADAPTER_PROTOCOL_END,
	ADAPTER_PROTOCOL_MAX_COUNT = ADAPTER_PROTOCOL_END,
};
...
enum adapter_support_mode {
	...
	ADAPTER_SUPPORT_PPS_SC = ADAPTER_SUPPORT_SCP_B_SC,
	ADAPTER_SUPPORT_SCP_B_HSC = 0x4,
	ADAPTER_SUPPORT_FCP = 0x8,
};
...
enum adapter_support_type {
	...
	ADAPTER_TYPE_20V5A,       /* 20V5A */
	ADAPTER_TYPE_10V4A_DL,    /* 10v4a */
	ADAPTER_TYPE_20V3P25A_PC,
	ADAPTER_TYPE_20V5A_DL,
	ADAPTER_TYPE_20V6P75A_PC,
	ADAPTER_TYPE_20V5A_PC,
	ADAPTER_TYPE_11V3P2A,
};
...
enum adapter_protocol_chip_vendor {
	...
	ADAPTER_CHIP_ROCKCHIP,
	ADAPTER_CHIP_CHIPOWN,
	ADAPTER_CHIP_ID0X32,
};
...
enum adapter_protocol_device_id {
	...
	PROTOCOL_DEVICE_ID_MT6375,
	PROTOCOL_DEVICE_ID_SC8551S,
	PROTOCOL_DEVICE_ID_HL7139,
	PROTOCOL_DEVICE_ID_SC8546,
	PROTOCOL_DEVICE_ID_RT9756A,
	PROTOCOL_DEVICE_ID_SGM41600A,
	PROTOCOL_DEVICE_ID_MTK_BUILDIN_PD,
...
struct adapter_protocol_power_curve_info {
	int vol;
	int cur;
};
...
struct adapter_protocol_ops {
	...
	int (*get_output_current_set_step)(int *cur);
	...
	bool (*is_ignore_cable_check)(void);
	bool (*is_adp_rdy)(void);
	int(*get_adp_type_data)(int *data);
};

struct adapter_chg_det_ops
{
	int (*chg_det_enable)(int enable);
	int (*get_chg_type)(int *type);
};
...
struct adapter_protocol_dev {
	...
	struct adapter_chg_det_ops *det_ops;
	// for protocol arbitration
	int err_cnt[ADAPTER_PROTOCOL_END];
};

#if IS_ENABLED(CONFIG_ADAPTER_PROTOCOL)
...
int adapter_get_output_current_set_step(int prot, int *cur);
...
int adapter_get_adp_type_data(int prot, int *data);
...
bool adapter_is_ignore_cable_check(int prot);
bool adapter_is_adp_rdy(int prot);
int adapter_protocol_chg_det_register(struct adapter_chg_det_ops *ops);
int adapter_protocol_chg_det_enable(int enable);
int adapter_protocol_get_chg_type(int *type);
int adapter_protocol_get_err_cnt(int prot);
...
static inline int adapter_get_output_current_set_step(int prot, int *cur)
{
	return -1;
}
...
static inline int adapter_get_adp_type_data(int prot, int *type)
{
	return -1;
}
...
static inline bool adapter_is_ignore_cable_check(int prot)
{
	return false;
}

static inline bool adapter_is_adp_rdy(int prot)
{
	return false;
}

static inline int adapter_protocol_chg_det_register(struct adapter_chg_det_ops *ops)
{
	return -1;
}

static inline int adapter_protocol_chg_det_enable(int enable)
{
	return -1;
}

static inline int adapter_protocol_get_chg_type(int *type)
{
	return -1;
}

static inline int adapter_protocol_get_err_cnt(int prot)
{
	return -1;
}
