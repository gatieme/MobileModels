...
#define POWER_SUPPLY_USB_DEFAULT_ONLINE       0
#define POWER_SUPPLY_DEFAULT_CURRENT_MAX      500000 /* uA */
#define POWER_SUPPLY_DEFAULT_IBUS_LIMIT       500000 /* uA */
...
struct power_supply_ops {
	...
	int (*get_type_prop)(void);
	int (*get_input_current_limit_prop)(void);
	int (*set_input_current_limit_prop)(int value);
	int (*get_usb_type_prop)(void);
};
...
static void power_supply_update_usb_psy_type(enum power_supply_type type)
{
}
