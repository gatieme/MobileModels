...
#define CP_GPIO_CONFIG_RETRY_CNT        3
...
#define CP_SET_BP_RETRY_TIMES           3
...
enum charge_pump_device_id {
	...
	CP_DEVICE_ID_PCA9488_AUX,
	CP_DEVICE_ID_HL1512,
	CP_DEVICE_ID_HL1512_AUX,
	CP_DEVICE_ID_SC8502,
	CP_DEVICE_ID_RT9758,
	CP_DEVICE_ID_RT9758_AUX,
	CP_DEVICE_ID_SY6512,
	CP_DEVICE_ID_SC8562,
	CP_DEVICE_ID_END,
};
...
struct charge_pump_ops {
	...
	int (*dev_check)(void *dev_data);
	int (*post_probe)(void *dev_data);
	int (*chip_init)(void *dev_data); /* forward charge chip init */
	int (*rvs_chip_init)(void *dev_data); /* reverse charge chip init */
	int (*rvs_cp_chip_init)(void *dev_data, bool enable); /* reverse charge cp chip init */
	int (*rvs_cp4_chip_init)(void *dev_data, bool enable); /* reverse charge cp chip init */
	int (*set_bp_mode)(void *dev_data); /* forward bypass mode */
	int (*set_cp_mode)(void *dev_data); /* forward 2:1 cp_mode */
	int (*set_rvs_bp_mode)(void *dev_data); /* reverse bypass mode */
	int (*set_rvs_cp_mode)(void *dev_data); /* reverse 2:1 cp_mode */
	bool (*is_bp_open)(void *dev_data);
	bool (*is_cp_open)(void *dev_data);
	int (*get_cp_ratio)(void *dev_data);
	int (*get_cp_vout)(void *dev_data);
	int (*set_rvs_bp2cp_mode)(void *dev_data);
};
...
struct charge_pump_dev {
	...
	int retry_cnt;
	unsigned int gpio_cfg_flag;
	unsigned int gpio_recheck_flag;
...
extern int charge_pump_reverse_cp_chip_init(unsigned int type, bool enable);
extern int charge_pump_reverse_cp4_chip_init(unsigned int type, bool enable);
...
static inline int charge_pump_reverse_cp_chip_init(unsigned int type, bool enable)
{
	return -EPERM;
}

static inline int charge_pump_reverse_cp4_chip_init(unsigned int type, bool enable)
{
	return -EPERM;
}
