...
enum power_if_sysfs_type {
	...
	POWER_IF_SYSFS_IBUS,
	POWER_IF_SYSFS_VBUS,
	POWER_IF_SYSFS_FG_TERM,
	POWER_IF_SYSFS_EEPROM_RT,
	POWER_IF_SYSFS_EEPROM_PT,
	POWER_IF_SYSFS_LDO_ENABLE_CHARGER,
	POWER_IF_SYSFS_LDO_ENABLE_HIZ,
	POWER_IF_SYSFS_CAPACITY_FCC,
	POWER_IF_SYSFS_CAPACITY_RM,
	POWER_IF_SYSFS_BRAND,
	POWER_IF_SYSFS_CYCLE_INFO,
	POWER_IF_SYSFS_PHONE_STATE,
	POWER_IF_SYSFS_CHARGE_TURBO_ENABLE,
	POWER_IF_SYSFS_FUEL_GAUGE_TYPE,
	POWER_IF_SYSFS_CHG_MODE,
	POWER_IF_SYSFS_RBATT,
	POWER_IF_SYSFS_CHARGE_ENABLE_HZ,
	POWER_IF_SYSFS_BATT_TYPE,
	POWER_IF_SYSFS_END,
};
...
enum power_if_op_user {
	...
	POWER_IF_OP_USER_HALL_SENSOR, /* for hall_sensor_monitor */
...
enum power_if_op_type {
	...
	POWER_IF_OP_TYPE_HSC, /* hsc */
	POWER_IF_OP_TYPE_MAINHSC, /* mainhsc */
	POWER_IF_OP_TYPE_AUXHSC, /* auxhsc */
	POWER_IF_OP_TYPE_WL, /* wireless */
	POWER_IF_OP_TYPE_WL_SC, /* wireless sc */
	POWER_IF_OP_TYPE_WL_HSC, /* wireless hsc */
	POWER_IF_OP_TYPE_UVDM, /* uvdm */
	POWER_IF_OP_TYPE_POWER_GPIO, /* power gpio */
	POWER_IF_OP_TYPE_POWER_FIRMWARE, /* power firmware */
	POWER_IF_OP_TYPE_FG, /* fuel guage term status */
	POWER_IF_OP_TYPE_BATT_INFO, /* for adsp batt info */
	POWER_IF_OP_TYPE_EEPROM, /* eeprom test */
	POWER_IF_OP_TYPE_LDO, /* ldo */
	POWER_IF_OP_TYPE_DCP_AUX, /* aux buck standard */
...
struct power_if_ops {
	...
	int (*get_fg_term_status)(int *value);
	int (*set_eeprom_rt_test)(unsigned int value);
	int (*get_eeprom_rt_result)(void);
	int (*get_eeprom_pt_result)(void);
	int (*set_ldo_enable_charger)(int value);
	int (*set_ldo_enable_hiz)(int value);
	int (*set_phone_state)(unsigned int value);
	int (*set_charge_turbo_enable)(unsigned int value);
	int (*get_charge_turbo_enable)(unsigned int *value);
	int (*get_chg_mode)(int *value);
	int (*get_rbatt)(int *value);
	int (*set_charge_enable_hz)(int value);
	int (*get_batt_type)(int *value);
};
