...
/* definition must coincide with bms_hidl enum PowerHidlProtType */
enum adapter_sysfs_protocol_type {
	SYSFS_PROTOCOL_BEGIN = 0,
	SYSFS_PROTOCOL_DEFAULT = SYSFS_PROTOCOL_BEGIN,
	SYSFS_PROTOCOL_SCP,
	SYSFS_PROTOCOL_UFCS,
	SYSFS_PROTOCOL_END,
};
...
enum adapter_protocol_type {
	...
	ADAPTER_PROTOCOL_SCP = ADAPTER_PROTOCOL_BEGIN,
	ADAPTER_PROTOCOL_FCP,
	ADAPTER_PROTOCOL_PD,
...
enum adapter_support_type {
	...
	ADAPTER_TYPE_JLR_20V6P7A,
	ADAPTER_TYPE_5V4P5A_BANK,  /* 5v4.5a */
	ADAPTER_TYPE_10V2P25A_BANK, /* 10v2.25a */
	ADAPTER_TYPE_11V6A_BANK,
	ADAPTER_TYPE_11V6A_CAR,
	ADAPTER_TYPE_HHR_20V4P5A,
	ADAPTER_TYPE_YLR_20V5A_CAR,
	ADAPTER_TYPE_FCR_C_11V6A, /* 20v2a 11v6a */
	ADAPTER_TYPE_CAR_11V6A,
...
enum adapter_antifake_detect_code {
	ADAPTER_ANTIFAKE_FAIL = -1,
	ADAPTER_ANTIFAKE_INIT = 0,
	ADAPTER_ANTIFAKE_SUCC = 1,
};
...
enum adapter_protocol_device_id {
	...
	PROTOCOL_DEVICE_ID_SY6513,
	PROTOCOL_DEVICE_ID_SC8546,
	PROTOCOL_DEVICE_ID_CPS2021,
	PROTOCOL_DEVICE_ID_CPS2023,
	PROTOCOL_DEVICE_ID_SC200X,
	PROTOCOL_DEVICE_ID_STM32G031,
	PROTOCOL_DEVICE_ID_HC32L110,
...
struct adp_scpb_pwr_curve_data {
	u8 min_curr;
	u16 max_curr;
	u16 min_volt;
	u16 max_volt;
...
struct adp_scpb_pwr_data {
	u8 output_mode;
	u8 pwr_type;
	struct adp_scpb_pwr_gear_data gear_data;
	struct adp_scpb_pwr_curve_data curve_data;
};
...
struct adapter_protocol_ops {
	...
	int (*get_chip_serial_num)(int *id);
	...
	int (*get_source_id)(int *source_id);
	int (*get_scpb_pwr)(struct adp_scpb_pwr_data *scpb_pwr, int *size, int max_size);
	int (*auth_encrypt_start)(int key);
	int (*set_usbpd_enable)(int enable, bool check_cport);
	...
	int (*set_send_data)(u32 *data, u8 cnt, bool wait_rsp, int type);
};
...
int adapter_soft_reset_dpdm(int prot);
...
int adapter_get_chip_serial_num(int prot, int *num);
...
int adapter_get_max_power(int prot, int *max_pwr);
...
int adapter_get_source_id(int prot, int *source_id);
int adapter_get_scpb_pwr(int prot, struct adp_scpb_pwr_data *scpb_pwr, int *size, int max_size);
int adapter_auth_encrypt_start(int prot, int key);
int adapter_set_usbpd_enable(int prot, int enable, bool check_cport);
...
int adapter_get_adp_code(int prot, int *code);
...
int adapter_uvdm_send_data(int prot, u32 *data, u8 cnt, bool wait_rsp, int type);
#else
...
static inline int adapter_soft_reset_dpdm(int prot)
{
	return -EPERM;
}
...
static inline int adapter_get_chip_serial_num(int prot, int *num)
{
	return -EPERM;
}
...
static inline int adapter_get_max_power(int prot, int *max_pwr)
{
	return -EPERM;
}
...
static inline int adapter_get_power_curve(int prot, struct adp_pwr_curve_para *val, int *size, int max_size)
{
	return -EPERM;
}
...
static inline int adapter_get_source_id(int prot, int *source_id)
{
	return -EPERM;
}

static inline int adapter_get_scpb_pwr(int prot, struct adp_scpb_pwr_data *scpb_pwr, int *size, int max_size)
{
	return -EPERM;
}

static inline int adapter_auth_encrypt_start(int prot, int key)
{
	return -EPERM;
}

static inline int adapter_set_usbpd_enable(int prot, int enable, bool check_cport)
{
	return -EPERM;
}
...
static inline int adapter_get_adp_code(int prot, int *code)
{
	return -EPERM;
}
...
static inline int adapter_uvdm_send_data(int prot, u32 *data, u8 cnt, bool wait_rsp, int type)
{
	return -EPERM;
}
