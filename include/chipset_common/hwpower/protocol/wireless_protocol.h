...
enum wireless_protocol_device_id {
	...
	WIRELESS_DEVICE_ID_STWLC33,
	WIRELESS_DEVICE_ID_CPS4067,
	WIRELESS_DEVICE_ID_MT5785,
	WIRELESS_DEVICE_ID_CPS8601,
	WIRELESS_DEVICE_ID_MT5806,
...
enum wltx_prot_alarm_src {
	TX_ALARM_SRC_TSART = 0,
	TX_ALARM_SRC_TEMP = TX_ALARM_SRC_TSART,
	TX_ALARM_SRC_PLIM,
	TX_ALARM_SRC_FAN,
	TX_ALARM_SRC_FOD,
	TX_ALARM_SRC_TIME,
	TX_ALARM_SRC_QVAL,
	TX_ALARM_SRC_PREVFOD,
	TX_ALARM_SRC_END,
};
...
struct wireless_protocol_ops {
	...
	int (*send_rx_product_id)(unsigned int ic_type, u8 rx_product_id);
	...
	int (*send_fod_km)(unsigned int ic_type, int km);
	int (*send_tx_vpa_lim)(unsigned int ic_type, int val, int type);
	int (*send_charge_mode)(unsigned int ic_type, u8 mode);
	int (*send_start_prevfod_chk)(unsigned int ic_type);
	...
	int (*set_dev_attr)(unsigned int ic_type, u16 attr);
...
int wireless_send_rx_product_id(unsigned int ic_type, int prot, u8 rx_product_id);
...
int wireless_send_fod_km(unsigned int ic_type, int prot, int km);
int wireless_send_tx_vpa_lim(unsigned int ic_type, int prot, int val, int type);
int wireless_send_charge_mode(unsigned int ic_type, int prot, u8 mode);
int wireless_send_start_prevfod_check(unsigned int ic_type, int prot);
...
/* for wireless accessory */
int wireless_set_dev_attr(unsigned int ic_type, int prot, u16 attr);
...
static inline int wireless_send_rx_product_id(unsigned int ic_type, int prot, u8 rx_product_id)
{
	return -ENOTSUPP;
}
...
static inline int wireless_send_fod_km(unsigned int ic_type, int prot, int km)
{
	return -ENOTSUPP;
}

static inline int wireless_send_tx_vpa_lim(unsigned int ic_type, int prot, int val, int type)
{
	return -ENOTSUPP;
}
...
static inline int wireless_send_start_prevfod_check(unsigned int ic_type, int prot)
{
	return -ENOTSUPP;
}
...
static inline int wireless_set_dev_attr(unsigned int ic_type, int prot, u16 attr)
{
	return -ENOTSUPP;
}
