...
#define WLTX_PREVFOD_ILIM_START   0
#define WLTX_PREVFOD_ILIM_STOP    1
#define WLTX_PREVFOD_CHK_START    2
#define WLTX_PREVFOD_CHK_STOP     3
...
struct wltx_ic_ops {
	...
	void (*prevfod_chk)(int flag, void *dev_data);
	...
	int (*get_curfod_iin)(u16 *iin, void *dev_data);
	...
	int (*set_min_duty)(int min_duty, void *dev_data);
	int (*set_max_duty)(int max_duty, void *dev_data);
	int (*set_ploss)(u8 rx_type, void *dev_data);
	int (*set_open_loop)(bool enable, void *dev_data);
	void (*get_dbg_info)(char *info, int len, void *dev_data);
	int (*set_low_power)(bool enable, void *dev_data);
};
...
void wltx_ic_prevfod_check(unsigned int type, int flag);
...
int wltx_ic_get_curfod_iin(unsigned int type, u16 *iin);
...
int wltx_ic_set_max_duty(unsigned int type, int max_duty);
int wltx_ic_set_min_duty(unsigned int type, int min_duty);
int wltx_ic_set_ploss(unsigned int type, u8 rx_type);
int wltx_ic_set_open_loop(unsigned int type, bool enable);
void wltx_ic_get_dbg_info(unsigned int type, char *info, int len);
#else
...
static inline void wltx_ic_prevfod_check(unsigned int type, int flag)
{
}
...
static inline int wltx_ic_get_curfod_iin(unsigned int type, u16 *iin)
{
	return -EPERM;
}
...
static inline int wltx_ic_set_max_duty(unsigned int type, int max_duty)
{
	return -EPERM;
}

static inline int wltx_ic_set_min_duty(unsigned int type, int min_duty)
{
	return -EPERM;
}

static inline int wltx_ic_set_ploss(unsigned int type, u8 rx_type)
{
	return -EPERM;
}

static inline int wltx_ic_set_open_loop(unsigned int type, bool enable)
{
	return -EPERM;
}

static inline void wltx_ic_get_dbg_info(unsigned int type, char *info, int len)
{
}
