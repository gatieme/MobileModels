...
#if defined(CONFIG_HISYSEVENT) && defined(CONFIG_POWER_DUBAI)
#include <dfx/hiview_hisysevent.h>
#define HW_DOAMIN_DUBAI	"KERNEL_VENDOR"
#define HW_DUBAI_MAX_MESSAGE_LEN	512
int hwdubai_send_event(const char *domain, const char *event_name, enum hisysevent_type type, const char *message);
int hwlog_send_hisysevent(const char *domain, const char *event_name, enum hisysevent_type type, const char *fmt, ...);

#ifndef HWDUBAI_pr
#define HWDUBAI_pr(tag, fmt, ...) \
	hwlog_send_hisysevent(HW_DOAMIN_DUBAI, tag, STATISTIC, fmt, ##__VA_ARGS__)
#endif
...
#define pr_jank(tag, fmt, ...)	\
	hwlog_to_jank(tag, HW_LOG_PRIO_DEBUG, fmt, ##__VA_ARGS__)
#endif
...
#define LOG_JANK_W(tag, fmt, ...) \
	hwlog_to_jank(tag, HW_LOG_PRIO_WARN, fmt, ##__VA_ARGS__)
#endif

#ifndef LOG_JANK_V
#define LOG_JANK_V(tag, fmt, ...) \
	hwlog_to_jank(tag, HW_LOG_PRIO_VERBOSE, fmt, ##__VA_ARGS__)
#endif

#ifndef LOG_JANK_I
#define LOG_JANK_I(tag, fmt, ...) \
	hwlog_to_jank(tag, HW_LOG_PRIO_INFO, fmt, ##__VA_ARGS__)
#endif

#ifndef LOG_JANK_E
#define LOG_JANK_E(tag, fmt, ...) \
	hwlog_to_jank(tag, HW_LOG_PRIO_ERROR, fmt, ##__VA_ARGS__)
#endif

#ifndef HWDUBAI_pr
#define HWDUBAI_pr(tag, fmt, ...) \
	hwlog_to_write(HW_LOG_PRIO_DEBUG, HW_LOG_ID_DUBAI, \
			tag, fmt, ##__VA_ARGS__)
#endif

#ifndef HWDUBAI_LOGV
#define HWDUBAI_LOGV(tag, fmt, ...) \
	hwlog_to_write(HW_LOG_PRIO_VERBOSE, HW_LOG_ID_DUBAI, \
			tag, fmt, ##__VA_ARGS__)
#endif
...
#ifndef HWDUBAI_LOGI
#define HWDUBAI_LOGI(tag, fmt, ...) \
	hwlog_to_write(HW_LOG_PRIO_INFO, HW_LOG_ID_DUBAI, \
			tag, fmt, ##__VA_ARGS__)
#endif

#ifndef HWDUBAI_LOGW
#define HWDUBAI_LOGW(tag, fmt, ...) \
	hwlog_to_write(HW_LOG_PRIO_WARN, HW_LOG_ID_DUBAI, \
			tag, fmt, ##__VA_ARGS__)
#endif

#ifndef HWDUBAI_LOGE
#define HWDUBAI_LOGE(tag, fmt, ...) \
	hwlog_to_write(HW_LOG_PRIO_ERROR, HW_LOG_ID_DUBAI, \
			tag, fmt, ##__VA_ARGS__)
#endif
