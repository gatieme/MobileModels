...
#define WLTX_BIGDATA_BUFF_SIZE           32
#define DEFAULT_MANUFACTURER             0
#define DEFAULT_TX_SOFT_VER              0
#define WLTX_AUX_TX_INFO_SIZE            4 /* less then WLTX_BIGDATA_BUFF_SIZE/2 */
#define WLTX_AUX_TX_Q_CALI               1

enum wltx_aux_open_type {
	WLTX_AUX_OPEN_TYPE_BEGIN = 0,
	WLTX_AUX_OPEN_BY_CLIENT = WLTX_AUX_OPEN_TYPE_BEGIN, /* client:0, dont move */
	WLTX_AUX_OPEN_BY_KB,
	WLTX_AUX_OPEN_BY_COIL_TEST,
	WLTX_AUX_OPEN_BY_LIGHTSTRAP,
	WLTX_AUX_OPEN_BY_PEN,
	WLTX_AUX_OPEN_TYPE_END,
};
...
struct wltx_aux_logic_ops {
	int (*get_dev_no)(void);
	bool (*support_switch)(void);
	void (*set_pen_gpio)(void);
	void (*set_kb_gpio)(void);
	int (*get_ping_timeout)(void);
	struct wltx_acc_dev *(*get_acc_info)(int);
	void (*set_acc_info)(struct wltx_acc_dev *);
};
...
struct wltx_aux_dev_info {
	...
	bool pen_approaching; /* pen is putting on device */
	...
	unsigned int support_sle;
	unsigned int acc_support_sle;
	enum wltx_aux_open_type tx_open_type;
	char bigdata_info[WLTX_BIGDATA_BUFF_SIZE];
};

enum wltx_aux_bigdata_num {
	WLTX_AUX_TX_MANUFACTURER = 0,
	WLTX_AUX_TX_ID,
	WLTX_AUX_TX_TYPE,
	WLTX_AUX_TX_SOFT_VERSION,
};

int wltx_aux_logic_ops_register(struct wltx_aux_logic_ops *ops);
