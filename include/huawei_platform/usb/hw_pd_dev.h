...
#include <chipset_common/hwusb/hw_pd/hw_pd_stub.h>
#else
...
enum pd_dpm_pe_event {
	...
	PD_DPM_PE_ADAPTER_DATA, /* adapter data from pd  */
};
...
struct pd_dpm_ops {
	...
	int (*pd_dpm_get_src_extcap)(void *client);
	void (*pd_dpm_set_src_cap)(void *client, struct otg_cap *allow_ext, int send);
	void (*pd_dpm_set_src_psrdelay)(void *client, int delay);
...
struct pd_dpm_info {
	...
	struct notifier_block wired_rechrg_nb;
	...
	struct delayed_work hw_pd_get_key_work;
	struct delayed_work hw_pd_get_cable_data_work;
	...
	int cur_usb_event;
	struct pd_dpm_cable_info cable_vdo;
	struct pd_dpm_cable_data cable_data;
...
struct pd_dpm_info *get_pd_dpm_info(void);
...
unsigned int get_abnormal_cc_detection(void);
...
static inline int pd_dpm_support_cable_auth(void)
{
	return 0;
}
...
static inline int pd_dpm_notify_direct_charge_status(bool dc)
{
	return 0;
}
...
static inline void pd_dpm_ignore_vbus_only_event(bool flag)
{
}

static inline int pmic_vbus_irq_is_enabled(void)
{
	return 0;
}

static inline bool pmic_vbus_is_connected(void)
{
	return false;
}

static inline void pmic_vbus_disconnect_process(void)
{
}
...
static inline struct pd_dpm_info *get_pd_dpm_info(void)
{
	return NULL;
}

static inline void pd_dpm_report_pd_source_vbus(struct pd_dpm_info *di, void *data)
{
}

static inline void pd_dpm_report_pd_sink_vbus(struct pd_dpm_info *di, void *data)
{
}

static inline unsigned int get_abnormal_cc_detection(void)
{
	return 0;
}

static inline void pd_dpm_handle_abnomal_change(int event)
{
}

static inline int pd_dpm_vbus_notifier_call(struct pd_dpm_info *di,
	unsigned long event, void *data)
{
	return 0;
}

static inline void set_pd_dpm_info_flag(bool pd_finish_flag, bool pd_source_vbus)
{
}

static inline void pd_dpm_report_pd_source_vconn(const void *data)
{
}

static inline void pd_dpm_set_typec_state(int typec_state)
{
}

static inline void hw_pd_set_hv_en(bool hv_en)
{
}
#ifdef CONFIG_REVERSE_CHARGE
static inline int hw_pd_is_reverse_charge_enable(void)
{
	return 0;
}
#endif /* CONFIG_REVERSE_CHARGE */
