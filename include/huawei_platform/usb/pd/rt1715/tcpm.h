...
struct tcpc_device;
#define VBUS_9000MV 9000
#define IBUS_2000MA 2000
...
#ifdef CONFIG_TCPC_RT1715
	TCP_NOTIFY_ABNORMAL_CC_CHANGE_HANDLER,
#endif
	TCP_NOTIFY_PD_STATE,
...
extern int tcpm_get_remote_power_cap_ex(struct tcpc_device *tcpc,
...
#ifdef CONFIG_USB_PD_REV30
extern int tcpm_dpm_pd_get_source_cap_ext_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data,
...
extern int tcpm_dpm_pd_fast_swap(struct tcpc_device *tcpc,
	uint8_t role, const struct tcp_dpm_event_cb_data *data);
extern int tcpm_dpm_pd_get_status_ex(struct tcpc_device *tcpc,
...
extern int tcpm_dpm_pd_get_pps_status_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data,
...
extern int tcpm_set_pd_charging_policy_ex(struct tcpc_device *tcpc,
...
#ifdef CONFIG_TCPC_RT1715
extern int tcpm_set_direct_charge_enable(void *client, bool en);
...
extern int tcpm_set_apdo_charging_policy_ex(
	struct tcpc_device *tcpc, uint8_t policy, int mv, int ma,
	const struct tcp_dpm_event_cb_data *data);
extern int tcpm_inquire_pd_source_apdo_ex(struct tcpc_device *tcpc,
...
static inline int tcpm_get_remote_power_cap_ex(struct tcpc_device *tcpc,
	struct tcpm_remote_power_cap *cap)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_dpm_pd_get_source_cap_ext_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data,
	struct pd_source_cap_ext *src_cap_ext)
{
	return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_dpm_pd_get_status_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data, struct pd_status *status)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_dpm_pd_get_pps_status_ex(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data,
	struct pd_pps_status *pps_status)
{
	return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_set_pd_charging_policy_ex(struct tcpc_device *tcpc,
	uint8_t policy, const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
#ifdef CONFIG_TCPC_RT1715
static inline int tcpm_set_direct_charge_enable(
	void *client, bool en)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_set_apdo_charging_policy_ex(
	struct tcpc_device *tcpc, uint8_t policy, int mv, int ma,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
...
static inline int tcpm_inquire_pd_source_apdo_ex(struct tcpc_device *tcpc,
	uint8_t apdo_type, uint8_t *cap_i, struct tcpm_power_cap_val *cap)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
