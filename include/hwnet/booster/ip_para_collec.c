...
#if defined(CONFIG_KSTATE) || defined(CONFIG_MPTCP)
	...
	struct tcp_res *stat, u8 protocol, u16 hook_type)
{
	...
	if (stat->uid == 0)
		return BACKGROUND;
	if (stat->uid == DNS_UID && is_dns_sk(sk, protocol))
	...
	struct sk_buff *skb, struct sock *sk, struct tcphdr *th, s32 tcp_len)
{
	...
	if (tp->snd_nxt != 0 &&
		...
		sk->sk_state == TCP_SYN_SENT)) {
		stat->out_rts_pkt++;
		if (sk->sk_state == TCP_SYN_SENT && th->syn == 1)
	...
	const struct nf_hook_state *state, u8 protocol, u16 payload_len)
{
	...
	if (skb == NULL || sk == NULL || g_app_ctx == NULL)
		return;
	if (cpu < 0 || cpu >= CONFIG_NR_CPUS)
		return;
	if (state == NULL)
...
#else
void update_dupack_num(struct sock *sk,
	int num_dupack, unsigned int hook)
{
	if (num_dupack > 0)
#endif
...
#else
static void stat_report(struct timer_list *t)
#endif
{
	...
	if (g_app_ctx == NULL) {
		pr_info("[IP_PARA]%s g_app_ctx == NULL", __func__);
		return;
	}
...
#else
	res->res.tcp.sync = (u16)g_app_ctx->report_cnt;
#endif
...
static void process_link_property_update(struct link_property_msg *msg)
{
	...
	if (modem_id != DS_NET_ID && modem_id != DS_NET_SLAVE_ID)
		return;
	if (interface_id > DS_INTERFACE_NAME_ID && interface_id != INVALID_LINK_PROPERTITY_INTERFACE_ID)
		return;
...
#else
	timer_setup(&g_app_ctx->timer, NULL, 0);
#endif
	g_app_ctx->report_tstamp = tcp_jiffies32;
