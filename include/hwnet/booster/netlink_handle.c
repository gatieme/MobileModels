...
#ifdef CONFIG_HN_PACKET_FILTER_BYPASS
#include "hw_packet_filter_bypass.h"
#endif

#ifdef CONFIG_HN_NETWORK_SLICE
#include "network_slice_management.h"
#endif
#include "sock_destroy_handler.h"

#ifdef CONFIG_HN_ICMP_PING_DETECT
...
#endif
#ifdef CONFIG_HN_PACKET_TRACKER
#include "hw_pt.h"
#endif
...
#ifdef CONFIG_HN_STEADY_SPEED_STATS
...
#if defined(CONFIG_HONOR_BASTET_QCOM) || defined(CONFIG_HONOR_BASTET_MTK)
...
const u16 cmd_map_model[CMD_NUM_MAX][MAP_ENTITY_NUM] = {
    ...
    { FOREGROUND_DATA_ACCELERATE_CMD, BASTET_FASTGRAB }
};
...
static void nl_notify_event(struct res_msg_head *msg)
{
	...
	if (!msg) {
		hilog_err("%s:null data\n", __func__);
		return;
	}

	if (g_ctx.chan_state != NETLINK_HANDLE_INIT) {
		hilog_err("%s:module not inited\n", __func__);
		return;
	}
...
static void netlink_handle_rcv(struct sk_buff *__skb)
{
	...
	if (__skb == NULL) {
		hilog_err("Invalid param: zero pointer reference(__skb)\n");
		return;
	}
	skb = skb_get(__skb);
	if (skb == NULL) {
		hilog_err("netlink_handle_rcv: skb = NULL\n");
		return;
	}
	mutex_lock(&g_recv_mtx);
	if (skb->len < NLMSG_HDRLEN) {
		hilog_err("netlink_handle_rcv:skb len error\n");
		goto skb_free;
	}
	nlh = nlmsg_hdr(skb);
	if (nlh == NULL) {
		hilog_err("netlink_handle_rcv:nlh = NULL\n");
		goto skb_free;
	}
	if ((nlh->nlmsg_len < sizeof(struct nlmsghdr))
		|| (skb->len < nlh->nlmsg_len)) {
		hilog_err("netlink_handle_rcv:nlmsg len error\n");
		goto skb_free;
	}
	...
	case NL_MSG_JNI_REQ:
		...
		break;
	default:
		hilog_err("invalid nlmsg_type %d\n", nlh->nlmsg_type);
		break;
	}
...
static int netlink_handle_init(void)
{
	...
	if (!g_ctx.nlfd) {
		hilog_info("%s: netlink_handle_init failed\n", __func__);
		return -1;
	}
	hilog_info("%s: netlink_handle_init success\n", __func__);

	sema_init(&g_ctx.sema, 0);

	g_ctx.task = kthread_run(netlink_handle_thread, NULL, "netlink_handle");
	if (IS_ERR(g_ctx.task)) {
		hilog_err("%s:failed to create thread\n", __func__);
		g_ctx.task = NULL;
		return -1;
	}
...
static int __init netlink_handle_module_init(void)
{
	...
	if (netlink_handle_init()) {
		hilog_err("%s:init netlink_handle module failed\n", __func__);
	...
	if (fn == NULL) {
		hilog_err("%s:init packet filter bypass failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init tcp para failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init uid process bind failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init sock destroy handler failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init icmp ping detect failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init tcp Ca controller failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init PACKET TRACKER failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init tcp acc controller failed\n", __func__);
		return -1;
	}
	...
	if (fn == NULL) {
		hilog_err("%s:init STEADY SPEED failed\n", __func__);
		return -1;
	}
...
#if defined(CONFIG_HONOR_BASTET_QCOM) || defined(CONFIG_HONOR_BASTET_MTK)
    fn = bastet_fastgrab_init(nl_notify_event);
    if (fn == NULL) {
        return -1;
    }
    g_ctx.model_cb[BASTET_FASTGRAB] = fn;
#endif

	hilog_info("%s:netlink_handle module inited:\n", __func__);
	return 0;
}
