...
#define WLAN_THIRD "wlan2"
#define WLAN_MASTER_ID 2
#define WLAN_SLAVE_ID 3
#define WLAN_THIRD_ID 4

static struct wifi_ctx *g_wifi_ctx;
static u32 g_gateway[MAX_WIFI_NUM];
static u32 g_freq[MAX_WIFI_NUM];

#ifdef CONFIG_HW_BOOSTER_QCOM
static int (*p_cfg80211_get_station)(struct net_device *dev, const u8 *mac_addr,
	struct station_info *sinfo);
static u32 (*p_cfg80211_calculate_bitrate)(struct rate_info *rate);

int cfg80211_get_station_register(int(*p)(struct net_device *dev, const u8 *mac_addr,
	struct station_info *sinfo))
{
	p_cfg80211_get_station = p;
	return 0;
}
EXPORT_SYMBOL(cfg80211_get_station_register);

int cfg80211_calculate_bitrate_register(u32(*p)(struct rate_info *rate))
{
	p_cfg80211_calculate_bitrate = p;
	return 0;
}
EXPORT_SYMBOL(cfg80211_calculate_bitrate_register);
#endif
...
#ifdef CONFIG_HW_BOOSTER_QCOM
	if (p_cfg80211_get_station)
		ret = p_cfg80211_get_station(dev, bssid, station_info);
#else
...
#ifdef CONFIG_HW_BOOSTER_QCOM
static int get_correct_rssi(const char *name, int raw_rssi)
{
	int rssi = raw_rssi;
	u32 freq;
	if(!is_qoe_version_advanced())
		return rssi;

	if (strncmp(name, WLAN_MASTER, WIFI_NET_LEN) == 0)
		freq = g_freq[0];
	else if (strncmp(name, WLAN_SLAVE, WIFI_NET_LEN) == 0)
		freq = g_freq[WLAN_SLAVE_ID - WLAN_MASTER_ID];
	else if (strncmp(name, WLAN_THIRD, WIFI_NET_LEN) == 0)
		freq = g_freq[WLAN_THIRD_ID - WLAN_MASTER_ID];
	else
		return rssi;
	if (freq)
		rssi = rssi + QCOM_24G_DELTA_RSSI;
	else
		rssi = rssi + QCOM_5G_DELTA_RSSI;
	return rssi;
}
#endif
...
{
	struct station_info sinfo;
	if (get_wifi_station_info(name, &sinfo) != 0)
		return -EOPNOTSUPP;
	...
	if (sinfo.filled & (BIT(NL80211_STA_INFO_RX_BYTES) | BIT(NL80211_STA_INFO_RX_BYTES64)))
		wifi_stat->info.rx_bytes = (u32)sinfo.rx_bytes;
	if (sinfo.filled & (BIT(NL80211_STA_INFO_TX_BYTES) | BIT(NL80211_STA_INFO_TX_BYTES64)))
		wifi_stat->info.tx_bytes = (u32)sinfo.tx_bytes;
	if (sinfo.filled & BIT(NL80211_STA_INFO_SIGNAL))
#ifdef CONFIG_HW_BOOSTER_QCOM
		wifi_stat->info.rssi = get_correct_rssi(name, sinfo.signal);
	if (sinfo.filled & BIT(NL80211_STA_INFO_TX_BITRATE))
		if (p_cfg80211_calculate_bitrate)
			wifi_stat->info.phy_tx_rate = p_cfg80211_calculate_bitrate(&sinfo.txrate);
	if (sinfo.filled & BIT(NL80211_STA_INFO_RX_BITRATE))
		if (p_cfg80211_calculate_bitrate)
			wifi_stat->info.phy_rx_rate = p_cfg80211_calculate_bitrate(&sinfo.rxrate);
#else
...
static void get_third_wifi_param(struct wifi_stat *wstat)
{
	if (get_wifi_stat(WLAN_THIRD, wstat) != 0)
		return;

	wstat->flag = 1;
	wstat->dev_id = WLAN_THIRD_ID;
	wstat->info.arp_state =
		get_wifi_arp_state(WLAN_THIRD, IFNAMSIZ, g_gateway[2]);
}
	...
	struct wifi_stat *cur, struct wifi_stat *res, int cnt)
{
	...
	if (past == NULL || res == NULL || cur == NULL)
		return;
	if (cur[WLAN0_IDX].flag == 0 && cur[WLAN1_IDX].flag == 0 && cur[WLAN2_IDX].flag == 0)
		return;
	for (i = 0; i < cnt; i++) {
...
static void start_wifi_collect(struct req_msg_head *msg)
{
	struct wifi_req_msg * wifi_req = NULL;
	u32 expires;
	if (msg->len < sizeof(struct wifi_req_msg))
		return;

	wifi_req = (struct wifi_req_msg *)msg;

	if (g_wifi_ctx == NULL)
		return;
	expires = wifi_req->report_expires;
	g_gateway[0] = wifi_req->wifi_gateway[0];
	g_gateway[1] = wifi_req->wifi_gateway[1];
	g_gateway[2] = wifi_req->wifi_gateway[2];
	g_freq[0] = wifi_req->wifi_freq[0];
	g_freq[1] = wifi_req->wifi_freq[1];
	g_freq[2] = wifi_req->wifi_freq[2];
...
static void update_wifi_collect(struct req_msg_head *msg)
{
	struct wifi_req_msg * wifi_req = NULL;
	if (msg->len < sizeof(struct wifi_req_msg))
		return;
