...
#ifdef CONFIG_MAS_MQ_USING_CPP
#define req_cpp(req) ((req)->mas_req.mas_featrue_flag & CUST_REQ_COMMAND_PRIO_PLUS)
#endif
...
struct request {
	...
	uint32_t gear_ctrl_valid;
	void *gear_ctrl_private;
	void *gear_ctrl_upprivate;
	spinlock_t gear_ctrl_setlock;
#endif
...
#define MAS_MQ_QUEUE_DEPTH 192
#define UNISTORE_MQ_QUEUE_DEPTH 32
#define UNISTORE_QUEUE_RESERVE_DEPTH 8
...
#define DATA_REMAP_MAX_NUM 768

struct stor_dev_data_remap_source_buffer {
	unsigned int data_source_addr;
	unsigned char reserved1;
	unsigned char reserved2;
	unsigned short source_length;
};

struct stor_dev_data_remap_response_info {
	unsigned int reserved;
	unsigned int data_info_addr;
	unsigned char status;
	unsigned char fail_reason;
	unsigned short length;
};

typedef void (stor_dev_data_remap_done_fn)(struct stor_dev_data_remap_response_info, void *);

struct stor_dev_data_remap_response_done_info {
	stor_dev_data_remap_done_fn *done;
	void *private_data;
	sector_t start_addr;
};

struct stor_dev_data_remap_info {
	unsigned char hyperhold_datamove_id;
	unsigned int source_buffer_num;
	struct stor_dev_data_remap_source_buffer *source_buffer;
	unsigned int dest_buffer_num;
	struct stor_dev_data_remap_source_buffer *dest_buffer;
	struct stor_dev_data_remap_response_info data_remap_response_info;
	struct stor_dev_data_remap_response_done_info data_remap_done_info;
};
...
typedef int (*lld_dev_data_move_fn)(struct request_queue *,
		struct stor_dev_data_move_info *);
typedef int (*lld_dev_data_remap_fn)(struct request_queue *,
		struct stor_dev_data_remap_info *);
...
typedef int (*lld_dev_slc_mode_error_inject_fn)(
		struct request_queue *, unsigned char);
#endif
#endif
...
struct mas_unistore_ops {
	...
	lld_dev_data_remap_fn dev_data_remap;
	...
	lld_dev_slc_mode_error_inject_fn dev_slc_mode_err_inject;
#endif
};
#endif

#ifdef CONFIG_SCSI_UFS_GEAR_CTRL
typedef int (*lld_gear_ctrl_fn)(struct request_queue *q, u32 set_gear);
typedef int (*lld_read_use_rate_fn)(struct request_queue *q, unsigned long *lrb_use_rate);
typedef int (*lld_gear_level_get_fn)(struct request_queue *q, u32 *gear_level);
typedef int (*lld_gear_level_cap_get_fn)(struct request_queue *q, u32 *min_gear, u32 *max_gear);

struct lld_gear_ctrl_ops {
	lld_gear_ctrl_fn gear_ctrl;
	lld_read_use_rate_fn read_gear_use_rate;
	lld_gear_level_get_fn gear_level_get;
	lld_gear_level_cap_get_fn gear_level_cap_get;
};
#endif
...
#ifdef CONFIG_SCSI_UFS_GEAR_CTRL
	struct lld_gear_ctrl_ops *gear_ctrl_ops;
#endif
...
int mas_blk_data_remap(struct block_device *bi_bdev,
	struct stor_dev_data_remap_info *data_remap_info);
void mas_blk_mq_tagset_data_remap_register(
	struct blk_mq_tag_set *tag_set, lld_dev_data_remap_fn func);
...
void mas_blk_mq_tagset_slc_mode_error_inject_register(
	struct blk_mq_tag_set *tag_set, lld_dev_slc_mode_error_inject_fn func);
int mas_blk_unistore_debug_en(void);
#ifdef CONFIG_MAS_MQ_USING_CPP
int mas_blk_cpplus_debug_en(void);
void inc_cpp_io_sum_cnt(void);
#endif
...
void mas_blk_clear_section_list(struct block_device *bdev,
	unsigned char stream_type);
void mas_blk_queue_cpplus_enable(struct request_queue *q, bool enable);
bool blk_queue_query_cpplus_enable(struct request_queue *q);
#else
...
int virtblk_cookie_enc(const struct bio *bio, char *buf, int buf_len);
#else
static inline int virtblk_cookie_enc(const struct bio *bio, char *buf, int buf_len)
{
	return 0;
}
#endif
...
void mas_virtio_blk_tag_set_init(struct blk_mq_tag_set *tag_set);
#else
static inline void mas_virtio_blk_tag_set_init(struct blk_mq_tag_set *tag_set)
{
	return;
}
#endif
