...
#define IOMT_LATENCY_GAP_ARRAY_SIZE	15
...
#define IOMT_IO_TIMEOUT_DSM_JUDGE_SLOT	\
	(13 % IOMT_LATENCY_STAT_ARRAY_SIZE)
...
static inline unsigned int iomt_get_array_index(const long *array,
	int array_len, unsigned long number, int offset, int unit)
{
	unsigned int i;

	for (i = 0; i < array_len - offset; i++) {
		if (number < (array[i + offset] * unit))
			break;
	}
	...
	unsigned int chunk_size)
{
	...
	if (begin_time->type == IOMT_LATNECY_LOW_ACC) {
		...
		latency_index = iomt_get_array_index(gap_array,
			IOMT_LATENCY_GAP_ARRAY_SIZE, (unsigned long)diff, 0, 1);
		...
		latency_index = iomt_get_array_index(gap_array,
			IOMT_LATENCY_GAP_ARRAY_SIZE, (unsigned long)diff, 0, IOMT_MS_TO_US);
	}

	chunk_size_index = iomt_get_array_index(iomt_chunksize_array,
		IOMT_LATENCY_CHUNKSIZE_ARRAY_SIZE, chunk_size, 1, 1);
