...
#if (defined CONFIG_MEMCG_PROTECT_LRU && defined CONFIG_HYPERHOLD_FILE_LRU)
...
#define ALLOC_STATS_NAME_LEN 16
#define ALLOC_STAT_TIME_MIN_US 64
#define ALLOC_STAT_TIME_MULTIPLE 2
#define ALLOC_STATS_MAX 15
struct alloc_time_stat {
	char name[ALLOC_STATS_NAME_LEN];
	unsigned long count_total;
	unsigned long total_time;
	unsigned long counts[ALLOC_STATS_MAX];
};

extern struct alloc_time_stat global_alloc_stat;
extern struct alloc_time_stat vip_alloc_stat;
extern struct alloc_time_stat cma_alloc_stat;
extern struct alloc_time_stat dmabuf_alloc_stat;
extern struct alloc_time_stat gpu_alloc_stat;
#ifdef CONFIG_ANON_PAGE_STAT
extern struct alloc_time_stat anonymous_page_stat;
#endif
#ifdef CONFIG_FILE_PAGE_STAT
extern struct alloc_time_stat file_page_stat;
#endif
#ifdef CONFIG_SWAP_PAGE_STAT
extern struct alloc_time_stat swap_page_stat;
#endif
extern u64 alloc_time_settions[ALLOC_STATS_MAX];

static inline void init_alloc_time_stat(struct alloc_time_stat *alloc_stat)
{
...
static inline u32 index_of_alloc_time_stat(u64 delta)
{
	int left = 0;
	int right = ALLOC_STATS_MAX - 2;
	int mid;

	if (delta < alloc_time_settions[left])
		return left;

	if (unlikely(delta > alloc_time_settions[right]))
		return ALLOC_STATS_MAX - 1;

	while (left <= right) {
		mid = left + (right - left) / 2;
		if (delta < alloc_time_settions[mid])
			right = mid - 1;
		else
			left = mid + 1;
	}
...
static inline void count_alloc_time(struct alloc_time_stat *alloc_stat,
		u64 delta)
{
	alloc_stat->count_total++;
	alloc_stat->total_time += delta;
	alloc_stat->counts[index_of_alloc_time_stat(delta)]++;
}
...
static inline int mem_cgroup_charge(struct page *page, struct mm_struct *mm,
				    gfp_t gfp_mask)
{
	struct mem_cgroup **memcgp = NULL;
	if (mem_cgroup_disabled())
		return 0;
	return __mem_cgroup_charge(page, mm, gfp_mask, memcgp);
}
#else
int __mem_cgroup_charge(struct page *page, struct mm_struct *mm,
			gfp_t gfp_mask);
