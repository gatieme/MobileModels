 ...
 *
 * PG_referencing indicates whether we have stopped page referencing in advance.
 * Initially PG_referencing = 0, which means we have not stopped in advance.
 * Then we will try to stop page referencing in advance in
 * check_page_references(), and if success, we set PG_referencing to 1.
 */
...
enum pageflags {
	...
	PG_kernelpage,	/* Added for page table. If used in the fastboot, it is bit #22 */
#endif
#ifdef CONFIG_MMU
...
#ifdef CONFIG_DFX_PAGE_TABLE_DUMP
PAGEFLAG(KernelPage, kernelpage, PF_ANY);
#endif
...
#ifdef CONFIG_MM_VIP
PAGEFLAG(Vip, vip, PF_ANY)
#endif

#ifdef CONFIG_VIP_PAGE_CACHE_BUILDIN
PAGEFLAG(Morechance, morechance, PF_ANY)
#endif
...
#ifdef CONFIG_STOP_PAGE_REF
PAGEFLAG(Referencing, referencing, PF_ANY)
TESTCLEARFLAG(Referencing, referencing, PF_ANY)
#endif
...
#ifdef CONFIG_MM_CMA_DEBUG
/* can be used only when CONFIG_MM_CMA_DEBUG is defined */
