...
#ifdef CONFIG_HISI_ADAPTER_DETECT
struct tcp_adapter_status {
	int adapter_cc_without_vbus;
};
#endif
struct tcp_notify {
	union {
...
#ifdef CONFIG_HISI_ADAPTER_DETECT
		struct tcp_adapter_status adapter_status;
#endif
	};
};
...
#define DPM_FLAGS_SOP_PRIME_UVDM		(1 << 10)
...
#ifdef CONFIG_VENDOR_USB_TYPEC
bool hisi_usb_typec_during_hardreset(void);
#else
static inline bool hisi_usb_typec_during_hardreset(void) {return false;}
#endif

#ifdef CONFIG_HISI_ADAPTER_DETECT
bool hisi_usb_typec_support_adapter_detect(void);
#else
static inline bool hisi_usb_typec_support_adapter_detect(void) { return false; }
#endif
...
int hisi_tcpm_send_uvdm(struct tcpc_device *tcpc_dev, uint8_t cnt,
		uint32_t *data, bool wait_resp, int *sop_type);
#endif
