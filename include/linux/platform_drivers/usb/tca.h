...
#define HIUSB_INVALID_PORT_ID		(-1)
...
#if defined(CONFIG_CONTEXTHUB_PD) || defined(CONFIG_USB_HIUSB)
extern int  pd_event_notify(enum tca_irq_type_e irq_type, enum tcpc_mux_ctrl_type mode_type,
...
static inline int pd_event_notify_by_portid(int portid,
			enum tca_irq_type_e irq_type,
			enum tcpc_mux_ctrl_type mode_type,
			enum tca_device_type_e dev_type,
			enum typec_plug_orien_e typec_orien)
