...
#if defined(CONFIG_SCHED_HISI_UTIL_CLAMP)
...
#define UID_NUM_MAX 5
#define UID_SYSTEM 1000
#define UID_MIN_VALUE 10000
#define UID_MAX_VALUE 20000
struct top_uid_info {
	int pid;
	int uid;
};

extern bool enable_vip_page_cache;
extern struct top_uid_info top_uids[UID_NUM_MAX];
extern bool find_top_uids(int file_gid);
extern void update_top_uids(struct task_struct *task, bool add);
#endif
...
#ifdef CONFIG_SCHED_PRED_LOAD_FINE_GRAN
#define NUM_BUSY_BUCKETS	100
#else
...
struct ravg {
	 ...
	 *
	 * 'ravg_sum' represents the sum of the scaled ravg. It incorporates
	 * all windows.
	 *
...
#ifdef CONFIG_SCHED_PRED_LOAD
	u32 predl_sum, predl_time_unscaled, predl;
	...
	DECLARE_BITMAP(predl_busy_buckets_bitmap, NUM_BUSY_BUCKETS);
#endif
	u16 active_windows;
...
enum vip_boost_type {
	...
	VIP_BOOST_CGROUP_RWSEM,
	VIP_BOOST_HW_DYNAMIC_SCHED,
	VIP_BOOST_TYPE_MAX,
};
#endif
...
struct sched_dynamic_prio {
	...
	int saved_prio;
	unsigned int dyn_switched_flag;
#ifdef CONFIG_SCHED_VIP
	unsigned int normal_vip_prio;
#endif
#ifdef CONFIG_SCHED_VIP_PI
	unsigned char vip_boost[VIP_BOOST_TYPE_MAX];
#endif
};
#endif

#ifdef CONFIG_MIPS_LOAD_TRACKING
#define MIPS_HIST_SIZE_MAX       6
struct task_mips_demand {
	u32 scale_inst_nr_history[MIPS_HIST_SIZE_MAX];
	u32 scale_cycle_nr_history[MIPS_HIST_SIZE_MAX];
	u64 window_start;
	int curr_avg_ipc;
};
#endif
...
struct task_struct {
	...
	unsigned int dynamic_smt_expeller;
#endif
	...
	struct task_struct		*sched_waker;
#endif
	/*
	...
	/*
	 * mark task's index in multi wake queue
	 * see WAKE_QUEUE_IDX_SHIFT for more info
	 *
	 * protected by g_wake_queue_rwlock
	 */
	u32				wake_queue_bit;
#endif
...
#ifdef CONFIG_SCHED_DYNAMIC_PRIO
	struct sched_dynamic_prio dyn_prio;
#endif

#ifdef CONFIG_RT_SWITCH_CFS_IF_TOO_LONG
	u64 sum_exec_runtime_wakeup;
#endif
	...
	struct coloc_group *coloc_grp;
	struct coloc_group *last_coloc_grp;
#endif
...
#ifdef CONFIG_DFX_STATS
	struct task_dfx_info	task_dfx_info;
	u64						preempt_dis_sum;
	u64						preempt_dis_start;
	u32						dfx_no;
#endif /* CONFIG_DFX_STATS */
	...
	u64				*time_in_state_total;
	unsigned int			max_state_total;
#endif
#endif
...
# define MAX_LOCK_DEPTH			48UL
...
#if (defined CONFIG_KCOV) || (defined CONFIG_ATF_FUZZ) \
	|| (defined CONFIG_HISP_FUZZ)
	...
	struct callback_head		throttle_work;
	int				throttle_work_cnt;
	int				throttled;
#endif

#ifdef CONFIG_SCHED_SMT_USER_HIDE
	bool				getting_max_cap;
	/* get open file fd */
	int				cpuinfo_fd;
#endif
	...
	/* eternal frozen status, true if it is eternal-frozen. */
	int eternal_frozen;
#endif
#ifdef CONFIG_PHASE
	struct phase_info		*phase_info;
#endif
#ifdef CONFIG_MIPS_LOAD_TRACKING
	struct task_mips_demand		*tmipsd;
#endif
...
#ifdef CONFIG_HISI_EAS_SCHED
/*
 * The __schedule()->trace_sched_switch()->__trace_sched_switch_state
 * (preempt, prev) path needs the __entry->prev_state to be the
 * first loaded one in __schedule() but not an up-to-date one.
 * Otherwise people may get confused about a task been deactivated
 * but shown R state in trace.
 * We cache the wanted prev_state in a this cpu var since the
 * __schedule() path has disabled preemption and interrupt. And we
 * reset it to TASK_STATE_MAX before exiting the path so that other
 * paths must see cached_prev_state as a reset value.
 */
DECLARE_PER_CPU(unsigned long, cached_prev_state);
static inline unsigned int get_tsk_state(struct task_struct *tsk)
{
	unsigned int cached_state = this_cpu_read(cached_prev_state);

	return cached_state != TASK_STATE_MAX
		? cached_state
		: READ_ONCE(tsk->state);
}
#endif
...
{
#ifdef CONFIG_HISI_EAS_SCHED
	unsigned int tsk_state = get_tsk_state(tsk);
#else
...
/* Be carefull to not intersect with system PFA_ flags */
#ifdef CONFIG_HP_CORE
#define PFA_HYPERHOLD_IDLE_RECLAIM	16
#endif
...
#ifdef CONFIG_SCHED_LATENCY_NICE
extern s64 sched_latency_offset_to_nice(s64 latency_offset);
extern int tg_latency_offset(const struct task_struct *p);
#endif
...
extern int sched_core_share_pid(unsigned int cmd, pid_t pid, enum pid_type type,
				unsigned long uaddr);
extern void sched_core_setuid(struct task_struct *p, uid_t new_uid);
#else
static inline void sched_core_free(struct task_struct *tsk) { }
static inline void sched_core_fork(struct task_struct *p) { }
static inline void sched_core_setuid(struct task_struct *p, uid_t new_uid) { }
#endif

#ifdef CONFIG_SCHED_CORE_PREFER
extern int sched_core_prefer_share_pid(unsigned int cmd, pid_t pid, enum pid_type type,
			   unsigned long uaddr);
#endif
...
#else
static inline unsigned int task_expeller_type(struct task_struct *task) {return 0;}
#endif
...
bool is_cpu_down_allowed(unsigned int cpu);
#endif

#ifdef CONFIG_SCHED_HISI_UTIL_CLAMP
int set_task_min_util(struct task_struct *p, unsigned int min_util);
#else
static inline int set_task_min_util(struct task_struct *p, unsigned int min_util)
{
	return 0;
}
#endif
...
static inline bool cpus_overutilized(struct cpumask *cpus) { return false; }
...
#else
#define vip_spare_map_rt_priority(x) (x)
#define vip_spare_unmap_rt_priority(x) (x)
#endif

#ifdef CONFIG_SCHED_VIP_PI
void request_vip_boost(struct task_struct *p, unsigned int vip_prio,
			unsigned int type);
void clear_vip_boost(struct task_struct *p, unsigned int type);
...
#else
static inline unsigned int get_task_min_util(struct task_struct *p)
{
	return 0;
}
static inline unsigned int get_task_max_util(struct task_struct *p)
{
...
enum HW_USERSPACE_THROTTLE_OPERATION {
	OPERATION_TASK_THROTTLE_WORK = 0,
	OPERATION_THROTTLE_USERSPACE_TASK,
	OPERATION_ADD_USERSPACE_THROTTLE_WORK,
	OPERATION_UNTHROTTLE_USERSPACE_TASK,
	OPERATION_ENABLE_TASK_THROTTLE,
	OPERATION_DISABLE_TASK_THROTTLE,
	OPERATION_USERSPACE_THROTTLE_MAX,
};

void sched_unthrottle_userspace_task(struct task_struct *p);
int sched_trace_cfs_rq_nr_running(struct cfs_rq *cfs_rq);
int sched_trace_cfs_rq_nr_throttled_tasks(struct cfs_rq *cfs_rq);
#endif
...
extern unsigned int sysctl_futex_opt_spin_enable;
#endif
