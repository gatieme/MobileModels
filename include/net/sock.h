...
#ifdef CONFIG_TCP_QUICKACK_TWEAK
static inline void sk_quickack_thold_update(struct sock *sk, int val)
{
	if (!sk || !sk_fullsock(sk) || READ_ONCE(sk->quickack_thold) == val)
		return;
	WRITE_ONCE(sk->quickack_thold, val);
}
#endif
