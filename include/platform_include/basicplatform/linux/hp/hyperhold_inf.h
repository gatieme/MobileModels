...
enum hyperhold_reclaim_source {
	RECLAIM_SOURCE_DEFAULT = 0,
#ifdef CONFIG_HYPERHOLD_ZSWAPD
	RECLAIM_SOURCE_ZSWAPD,
#endif
	RECLAIM_SOURCE_APP_ACTIVE,
};
#endif

struct hyperhold_turbo_ext {
	int ext_id;
	struct page *page;
	void *private;
};

typedef void (hyperhold_io_done_fn)(int err, struct hyperhold_turbo_ext *ext);

struct hyperhold_turbo_para {
	int page_num;
	struct hyperhold_turbo_ext *exts;
...
#ifdef CONFIG_RAMTURBO
extern void hyperhold_memcg_all_active_swapout(struct mem_cgroup *);
extern void hyperhold_memcg_active_swapout_bg_tasks(
		struct mem_cgroup *, unsigned int, unsigned int);
#endif
...
extern unsigned long get_hyperhold_cache_pages(void);

extern void hyperhold_cache_drop_exceed(void);
...
static inline unsigned long get_memcg_zram_swapout_cnt(struct mem_cgroup *memcg)
{
	return atomic64_read(&memcg->zram_swapout_cnt);
}

static inline unsigned long get_memcg_zram_swapin_cnt(struct mem_cgroup *memcg)
{
	return atomic64_read(&memcg->zram_swapin_cnt);
}
...
static inline unsigned long get_hyperhold_cache_pages(void)
{
	return 0;
}
static inline void hyperhold_cache_drop_exceed(void) {}
...
extern int hyperhold_alloc_nopkg_extent(void);
extern void hyperhold_free_nopkg_extent(int ext_id);
extern int hyperhold_turbo_reclaim_in(struct hyperhold_turbo_para *para);
extern int hyperhold_turbo_batch_out(struct hyperhold_turbo_para *para);
extern int hyperhold_turbo_fault_out(struct hyperhold_turbo_para *para);
#else
static inline int hyperhold_alloc_nopkg_extent(void)
{
	return -ENOSPC;
}
static inline void hyperhold_free_nopkg_extent(int ext_id) {}
static inline int hyperhold_turbo_reclaim_in(struct hyperhold_turbo_para *para)
{
	return 0;
}
static inline int hyperhold_turbo_batch_out(struct hyperhold_turbo_para *para)
{
	return 0;
}
static inline int hyperhold_turbo_fault_out(struct hyperhold_turbo_para *para)
{
	return 0;
}
#endif
