...
#define PATH_ROOT CONFIG_BBOX_LOG_DIR
#define TO_STR(s) #s
#define BBOX_CONCAT_TO_PATH_ROOT(s) PATH_ROOT TO_STR(s)
#define RDR_REBOOT_TIMES_FILE BBOX_CONCAT_TO_PATH_ROOT(reboot_times.log)
...
enum RDR_SAVE_LOG_FLAG {
	...
	RDR_SAVE_SCTRL_REGS = (0x1 << 4),
	RDR_SAVE_PCTRL_REGS = (0x1 << 5),
	RDR_SAVE_PMCTRL_REGS = (0x1 << 6),
	RDR_SAVE_PERICRG_REGS = (0x1 << 7),
};
...
/*
 * func args:
 * @addr: virtual address that alloced by dfx_bbox_map
 */
...
static inline void *dfx_bbox_map(phys_addr_t paddr, size_t size)
{
	return NULL;
}
...
static inline void rdr_set_wdt_kick_slice(u64 kickslice)
{
	return;
}

static inline u64 rdr_get_last_wdt_kick_slice(void)
{
	return 0;
}
...
static inline void save_log_to_dfx_tempbuffer(u32 reboot_type)
{
	return;
};

static inline void clear_dfx_tempbuffer(void)
{
	return;
};

static inline void systemerror_save_log2dfx(u32 reboot_type)
{
	return;
}

static inline void dfx_bbox_unmap(const void *vaddr)
{
	return;
}
...
static inline u32 rdr_get_diaginfo_size(void)
{
	return 0;
}
...
static inline void *bbox_vmap(phys_addr_t paddr, size_t size)
{
	return NULL;
}

static inline int rdr_dir_size(char *path, u32 path_len, bool recursion)
{
	return 0;
}
#endif
...
#else
static inline void set_boot_time_keypoint(unsigned short stage, int step, unsigned short time, unsigned short value)
{
	(void)stage;
	(void)time;
	(void)step;
...
static inline void set_boottime_addr_early_map_end(void)
{
}
#endif
