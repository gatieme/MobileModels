...
/* hisp device api */
extern int hisp_set_boot_mode(enum hisp_rproc_case_attr type);
...
extern int hisp_dev_setclkdepend(struct clk *aclk_dss,
			struct clk *pclk_dss);
extern int hisp_device_start(struct rproc *rproc);
extern int hisp_device_stop(struct rproc *rproc);
...
extern void hisp_memecc_probe(struct platform_device *pdev);
extern void hisp_memecc_remove(void);
#else
static inline void hisp_memecc_probe(struct platform_device *pdev) {};
static inline void hisp_memecc_remove(void) {};
#endif

#ifdef CONFIG_ISP_BUF_PRIVATIZE
extern struct sg_table *hisp_alloc_dynmem_sg_table(unsigned long length);
extern void hisp_free_dynmem_sg_table(struct sg_table *table);
extern int hisp_ispfw_device_attach(struct platform_device *pdev);
extern int hisp_ispfw_device_remove(struct platform_device *pdev);
#endif
