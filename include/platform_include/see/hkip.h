...
static inline void hkip_set_bit(u8 *bits, size_t pos, size_t max, bool value)
{
	...
	if (unlikely(hkip_hvc4(HKIP_HVC_ROWM_SET_BIT,
			       (unsigned long)(uintptr_t)bits, pos, value)))
		BUG();
}

static inline bool __hkip_get_task_bit(const u8 *bits, struct task_struct *task)
{
...
#else
static inline void hkip_init_task_pidhash(struct task_struct *task)
{
	(void)task;
}

static inline void hkip_check_pid_state(struct task_struct *task)
{
	(void)task;
}

static inline void hkip_init_cred_xidhash(const struct cred *cred, struct task_struct *task)
{
	(void)cred;
	(void)task;
}
...
#define hkip_get_fs() (hkip_is_kernel_fs() ? KERNEL_DS : USER_DS)
...
#else
static inline void hkip_set_pgtable_protect_addr(void) {}

static inline int hkip_get_pgtable_protect_addr(phys_addr_t *start, phys_addr_t *end)
{
	(void)start;
	(void)end;
	return 0;
...
static inline unsigned long hkip_pgtable_phys_alloc(unsigned long size, unsigned long align)
{
	return 0;
}
#endif /* CONFIG_HKIP_MAP_KERNEL_PROTECT */

#ifdef CONFIG_HHEE_DEBUG
void hkip_get_stage2_pgtab(unsigned long addr);
#else
static inline void hkip_get_stage2_pgtab(unsigned long addr) {}
#endif /* CONFIG_HHEE_DEBUG */
