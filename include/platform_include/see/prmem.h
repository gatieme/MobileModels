 ...
 * @locked : records whether the memory is protected through prmem_protect_range
...
/*
 * validate_cond() - Generate a compile-time error if the condition is not met
 ...
 */
#define validate_cond(val, cond) ((val) / (!!cond(val)))
...
#define validate_pool_alignment(alignment)				\
	validate_cond(alignment, test_pool_alignment)
...
#define validate_object_size(size) validate_cond(size, test_object_size)

void prmem_alternative_alloc(void);
...
#define validate_cache_alignment(alignment)				\
	validate_cond(alignment, test_cache_alignment)
...
static inline void *prmem_cache_zalloc(struct prmem_cache *cache,
				       enum prmem_flags flags)
{
	return prmem_cache_alloc(cache, (enum prmem_flags)(flags | PRMEM_ZERO));
}
...
{
	enum prmem_pool_type type = (enum prmem_pool_type)pool_to_prmem_range_type(pool);
...
static inline int prmem_get_type(const void *start, unsigned long size)
{
	int i;
...
void configure_prmem_secure_region(void);
