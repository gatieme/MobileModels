...
TRACE_EVENT(cpufreq_schedutil_eval_target,
	    TP_PROTO(unsigned int cpu,
		     ...
		     unsigned int target),
#ifdef CONFIG_SCHED_RTG
	    TP_ARGS(cpu, util, max, load, rtg_util, rtg_freq, curr, target),
#else
	    ...
	    TP_printk("cpu=%u util=%lu max=%lu load=%u "
#ifdef CONFIG_SCHED_RTG
		      "rtg_util=%lu rtg_freq=%u "
#endif
		      "cur=%u target=%u",
		      __entry->cpu, __entry->util, __entry->max, __entry->load,
#ifdef CONFIG_SCHED_RTG
		      __entry->rtg_util, __entry->rtg_freq,
#endif
