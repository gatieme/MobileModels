...
extern unsigned long walt_task_util(struct task_struct *task);
extern unsigned long walt_uclamp_task_util(struct task_struct *task);
extern unsigned long long hn_get_task_tags(struct task_struct *task);
extern unsigned long long hn_get_task_rsum(struct task_struct *task);
#endif
...
#endif
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
		__array(	char,	cpuset,		TASK_COMM_LEN	)
		__array(	char,	cpuctl,		TASK_COMM_LEN	)
		__field(	unsigned long,		task_util	)
		__field(	unsigned long,		uclamp_task_util)
		__field(	unsigned int,		cpus)
		__field(	unsigned int,		active_cpus)
		__field(	unsigned long long,		tags)
		__field(	unsigned long long,		rsum)
#endif
	),
...
#endif
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
		kernfs_name(task_get_css(next, cpuset_cgrp_id)->cgroup->kn, __entry->cpuset, TASK_COMM_LEN);
		kernfs_name(task_get_css(next, cpu_cgrp_id)->cgroup->kn, __entry->cpuctl, TASK_COMM_LEN);
		__entry->task_util = walt_task_util(next);
		__entry->uclamp_task_util = walt_uclamp_task_util(next);
		__entry->cpus = cpumask_bits(&next->cpus_mask)[0];
		__entry->active_cpus = cpumask_bits(cpu_active_mask)[0];
		__entry->tags = (hn_get_task_tags(next) << 3);
		__entry->rsum = hn_get_task_rsum(next);
#endif
	),

#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
	TP_printk("prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d \
cpuset=%s cpuctl=%s task_util=%lu uclamp_task_util=%lu cpus=%d active_cpus=%d tags=%llu rsum=%llu",
#else
	TP_printk("prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d",
#endif
		...
		__entry->cpuset, __entry->cpuctl,
		__entry->task_util, __entry->uclamp_task_util,
		__entry->cpus, __entry->active_cpus, __entry->tags, __entry->rsum)
#else
		__entry->next_comm, __entry->next_pid, __entry->next_prio)
#endif
);
...
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
		__array(char, h_blocked_function, KSYM_NAME_LEN)
#endif
	),
...
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
		sprint_symbol_no_offset(__entry->h_blocked_function, (unsigned long)__entry->caller);
#endif
	),

#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
	TP_printk("pid=%d iowait=%d caller=%pS h_blocked_function=%s", __entry->pid, __entry->io_wait, __entry->caller, __entry->h_blocked_function)
#else
