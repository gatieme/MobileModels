...
/*
 * Tracepoint for dynamic sched manager:
 */
TRACE_EVENT(sched_status_monitor_hdl,
	TP_PROTO(struct task_struct *producer_task, unsigned int last_exec_runtime, int is_producer_sched),
 
	TP_ARGS(producer_task, last_exec_runtime, is_producer_sched),
 
	TP_STRUCT__entry(
		__array(	char, producer_comm, TASK_COMM_LEN	)
		__field(	pid_t, producer_pid			)
		__field(	int, producer_prio			)
		__field(	unsigned int, last_exec_runtime	)
		__field(	unsigned int, sum_exec_runtime	)
		__field(	int, is_producer_sched	)
	),
 
	TP_fast_assign(
		memcpy_s(__entry->producer_comm, TASK_COMM_LEN, producer_task->comm, TASK_COMM_LEN);
		__entry->producer_pid = producer_task->pid;
		__entry->producer_prio = producer_task->prio;
		__entry->last_exec_runtime = last_exec_runtime;
		__entry->sum_exec_runtime = producer_task->se.sum_exec_runtime;
		__entry->is_producer_sched = is_producer_sched;
	),
 
	TP_printk("producer_comm=%s pid=%d prio=%d last_exec_runtime=%d sum_exec_runtime=%d is_producer_sched=%d",
		__entry->producer_comm, __entry->producer_pid, __entry->producer_prio,
		__entry->last_exec_runtime, __entry->sum_exec_runtime, __entry->is_producer_sched)
);
 
TRACE_EVENT(sched_parse_producer_waker,
	TP_PROTO(pid_t waker_pid, unsigned int call_count, int rb_tree_node_size),
 
	TP_ARGS(waker_pid, call_count, rb_tree_node_size),
 
	TP_STRUCT__entry(
		__field(	pid_t, waker_pid			)
		__field(	unsigned int, call_count			)
		__field(	int, rb_tree_node_size	)
	),
 
	TP_fast_assign(
		__entry->waker_pid = waker_pid;
		__entry->call_count = call_count;
		__entry->rb_tree_node_size = rb_tree_node_size;
	),
 
	TP_printk("waker_pid=%d call_count=%lu rb_tree_node_size=%d",
		__entry->waker_pid, __entry->call_count, __entry->rb_tree_node_size)
);
 
TRACE_EVENT(sched_record_producer_waker,
	TP_PROTO(struct task_struct *producer_task),
 
	TP_ARGS(producer_task),
	...
	TP_fast_assign(
		memcpy_s(__entry->producer_comm, TASK_COMM_LEN, producer_task->comm, TASK_COMM_LEN);
		__entry->producer_pid = producer_task->pid;
		memcpy_s(__entry->waker_comm, TASK_COMM_LEN, producer_task->sched_waker->comm, TASK_COMM_LEN);
		__entry->waker_pid = producer_task->sched_waker->pid;
	),
 
	TP_printk("producer_comm=%s pid=%d waker_comm=%s pid=%d",
		__entry->producer_comm, __entry->producer_pid,
		__entry->waker_comm, __entry->waker_pid)
);
 
DECLARE_EVENT_CLASS(sched_set_task_prio_template,
	TP_PROTO(struct task_struct *task, int is_boost, int policy, int prio),
 
	TP_ARGS(task, is_boost, policy, prio),
 
	TP_STRUCT__entry(
		__array(	char, task_comm, TASK_COMM_LEN	)
		__field(	pid_t, task_pid			)
		__field(	int, curr_policy		)
		__field(	int, curr_prio			)
		__field(	int, is_boost			)
	),
 
	TP_fast_assign(
		memcpy_s(__entry->task_comm, TASK_COMM_LEN, task->comm, TASK_COMM_LEN);
		__entry->task_pid = task->pid;
		__entry->curr_policy = task->policy;
		__entry->curr_prio = task->prio;
		__entry->is_boost = is_boost;
	),
 
	TP_printk("comm=%s pid=%d curr_policy=%d curr_prio=%d is_boost=%d",
		__entry->task_comm, __entry->task_pid, __entry->curr_policy,
		__entry->curr_prio, __entry->is_boost)
);
 
DEFINE_EVENT(sched_set_task_prio_template, sched_set_producer_task_prio,
	TP_PROTO(struct task_struct *producer_task, int is_boost, int policy, int prio),
	TP_ARGS(producer_task, is_boost, policy, prio)
);
 
DEFINE_EVENT(sched_set_task_prio_template, sched_set_waker_task_prio,
	TP_PROTO(struct task_struct *waker_task, int is_boost, int policy, int prio),
	TP_ARGS(waker_task, is_boost, policy, prio)
);
#endif /* CONFIG_HW_DYNAMIC_SCHED */
...
TRACE_EVENT(sched_switch,
	...
	TP_fast_assign(
		...
		/* XXX SCHED_DEADLINE */
		__entry->cpus_mask = cpumask_bits(&next->cpus_mask)[0];
#ifdef CONFIG_HW_VIP_THREAD
		__entry->static_vip = next->static_vip;
		__entry->dynamic_vip = atomic64_read(&next->dynamic_vip);
#else
	...
	TP_printk("prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d"
		...
		__entry->cpus_mask, __entry->static_vip, __entry->dynamic_vip,
...
TRACE_EVENT(sched_blocked_reason,
	...
	TP_printk("pid=%d iowait=%d caller=%s+0x%lx/0x%lx [%s] delay=%lu",
			__entry->pid, __entry->io_wait, __entry->funname,
...
TRACE_EVENT(sched_setscheduler,
	...
	TP_PROTO(struct task_struct *p, int oldpolicy, int oldprio, int oldlatency,
...
TRACE_EVENT(sched_find_energy_efficient_cpu,
...
#ifdef CONFIG_SCHED_LATENCY_NICE
		__entry->tg_latency_offset = tg_latency_offset(tsk);
		__entry->latency_prio = tsk->latency_prio;
#else
	...
	TP_printk("pid=%d comm=%s util=%lu boosted=%u "
		  ...
		  __entry->latency_prio, __entry->tg_latency_offset)
);
...
TRACE_EVENT(sched_userspace_throttle,

	TP_PROTO(struct task_struct *task, struct rq *rq, struct cfs_rq *cfs_rq, int type),

	TP_ARGS(task, rq, cfs_rq, type),
	...
	TP_STRUCT__entry(
		...
		__field(int,	on_rq			)
		__field(int,	on_cpu			)
		__field(int,	nr_throttled_tasks	)
		__field(int,	nr_running_cfs		)
		__field(int,	nr_running_rq		)
		__array(char,	cgroup_path,	CGROUP_NAME_LEN  )
	),
	...
	TP_fast_assign(
		...
		__entry->task_cpu = task_cpu(task);
		__entry->on_rq = task->on_rq;
		__entry->on_cpu = task->on_cpu;
		__entry->nr_throttled_tasks = sched_trace_cfs_rq_nr_throttled_tasks(cfs_rq);
		__entry->nr_running_cfs = sched_trace_cfs_rq_nr_running(cfs_rq);
		__entry->nr_running_rq = sched_trace_rq_nr_running(rq);
		sched_trace_cfs_rq_path(cfs_rq, __entry->cgroup_path, CGROUP_NAME_LEN);
	),

	TP_printk("comm=%s pid=%d prio=%d allowed=%#lx state=%ld"
		" task_cpu=%d on_rq=%d on_cpu=%d type=%d nr=%d/%d/%d cgroup=%s",
		__entry->comm, __entry->pid, __entry->prio, __entry->affinity,
		__entry->state, __entry->task_cpu, __entry->on_rq,
		__entry->on_cpu, __entry->type,
		__entry->nr_throttled_tasks, __entry->nr_running_cfs, __entry->nr_running_rq,
		__entry->cgroup_path)
);
#endif
