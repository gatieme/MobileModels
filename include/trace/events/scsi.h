...
#ifdef CONFIG_HONOR_RWLOG
extern atomic_t g_scsi_start_cnt;
extern atomic_t g_scsi_done_cnt;

TRACE_EVENT(scsi_dispatch_cmd_start_lifetime,

	TP_PROTO(struct scsi_cmnd *cmd, unsigned long long secd, unsigned long long nrsecd),

	TP_ARGS(cmd, secd, nrsecd),
	...
	TP_STRUCT__entry(
		...
		__field( unsigned int,	data_sglen )
		__field( unsigned int,	prot_sglen )
		__field( unsigned int,  start_cnt )
		__field( unsigned char,	prot_op )
		__field( unsigned long long,	sec )
		__field( unsigned long long,	nrsec )
		__dynamic_array(unsigned char,	cmnd, cmd->cmd_len)
	),
	...
	TP_fast_assign(
		...
		__entry->data_sglen	= scsi_sg_count(cmd);
		__entry->prot_sglen	= scsi_prot_sg_count(cmd);
		__entry->start_cnt  = atomic_add_return(1, &g_scsi_start_cnt);
		__entry->prot_op	= scsi_get_prot_op(cmd);
		__entry->sec		= secd;
		__entry->nrsec		= nrsecd;
		memcpy(__get_dynamic_array(cmnd), cmd->cmnd, cmd->cmd_len);
	),

	TP_printk("start_cnt=%u host_no=%u channel=%u id=%u lun=%u data_sgl=%u prot_sgl=%u" \
		  " prot_op=%s cmnd=(%s %s raw=%s) sec:%08x, nrsec:%llu",
		  __entry->start_cnt, __entry->host_no, __entry->channel, __entry->id,
		  __entry->lun, __entry->data_sglen, __entry->prot_sglen,
		  show_prot_op_name(__entry->prot_op),
		  show_opcode_name(__entry->opcode),
		  __parse_cdb(__get_dynamic_array(cmnd), __entry->cmd_len),
		  __print_hex(__get_dynamic_array(cmnd), __entry->cmd_len),
		  __entry->sec,__entry->nrsec)
...
TRACE_EVENT(scsi_dispatch_cmd_error,
	...
	TP_fast_assign(
		...
		__entry->prot_sglen	= scsi_prot_sg_count(cmd);
#ifdef CONFIG_HONOR_RWLOG
		__entry->start_cnt	= atomic_sub_return(1, &g_scsi_start_cnt) + 1;
#endif
		__entry->prot_op	= scsi_get_prot_op(cmd);
		memcpy(__get_dynamic_array(cmnd), cmd->cmnd, cmd->cmd_len);
	),
#ifdef CONFIG_HONOR_RWLOG
	TP_printk("start_cnt=%u host_no=%u channel=%u id=%u lun=%u data_sgl=%u" \
		  " prot_sgl=%u prot_op=%s cmnd=(%s %s raw=%s) rtn=%d",
		  __entry->start_cnt, __entry->host_no, __entry->channel, __entry->id,
		  __entry->lun, __entry->data_sglen, __entry->prot_sglen,
		  show_prot_op_name(__entry->prot_op),
		  show_opcode_name(__entry->opcode),
...
DECLARE_EVENT_CLASS(scsi_cmd_done_timeout_template,
	...
	TP_fast_assign(
		...
		__entry->prot_sglen	= scsi_prot_sg_count(cmd);
#ifdef CONFIG_HONOR_RWLOG
		__entry->done_cnt	= atomic_add_return(1, &g_scsi_done_cnt);
#endif
		__entry->prot_op	= scsi_get_prot_op(cmd);
		memcpy(__get_dynamic_array(cmnd), cmd->cmnd, cmd->cmd_len);
	),
#ifdef CONFIG_HONOR_RWLOG
	TP_printk("done_cnt=%u host_no=%u channel=%u id=%u lun=%u data_sgl=%u " \
		  "prot_sgl=%u prot_op=%s cmnd=(%s %s raw=%s) result=(driver=" \
		  "%s host=%s message=%s status=%s)",
		  __entry->done_cnt, __entry->host_no, __entry->channel, __entry->id,
		  __entry->lun, __entry->data_sglen, __entry->prot_sglen,
		  show_prot_op_name(__entry->prot_op),
		  show_opcode_name(__entry->opcode),
		  __parse_cdb(__get_dynamic_array(cmnd), __entry->cmd_len),
