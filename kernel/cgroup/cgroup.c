...
/*
 * g_css_inc_0 indicates that ref cnt is increase from 0 which may cause double free
 * g_css_dec_0 indicates that ref cnt is decrease below 0 which means unbalanced
 * g_css_mg_freeed indicates that memcg is freeed during hyperhold_fault_out()
 */
atomic_t g_css_inc_0 = ATOMIC_INIT(0);
atomic_t g_css_dec_0 = ATOMIC_INIT(0);
atomic_t g_css_mg_freeed = ATOMIC_INIT(0);
...
#ifdef CONFIG_CGROUP_THDGROUP_RWSEM_OPT
void cgroup_threadgroup_rwsem_boost_enter(struct percpu_rw_semaphore *sem)
{
	if (sem == &cgroup_threadgroup_rwsem)
		request_vip_boost(current, 1, VIP_BOOST_CGROUP_RWSEM);
}
EXPORT_SYMBOL_GPL(cgroup_threadgroup_rwsem_boost_enter);

void cgroup_threadgroup_rwsem_boost_exit(struct percpu_rw_semaphore *sem)
{
	if (sem == &cgroup_threadgroup_rwsem)
		clear_vip_boost(current, VIP_BOOST_CGROUP_RWSEM);
}
EXPORT_SYMBOL_GPL(cgroup_threadgroup_rwsem_boost_exit);
#endif
...
#if defined(CONFIG_HW_FORK_THREAD_CHANGE_CGROUP)
	record_cgroup_for_cpuctl_top_app(cgrp, name);
	record_cgroup_for_cpuset_top_app(cgrp, name);
#endif
...
#if defined(CONFIG_HW_FORK_THREAD_CHANGE_CGROUP)
	clear_cgroup_for_cpuctl_top_app(cgrp);
	clear_cgroup_for_cpuset_top_app(cgrp);
#endif
	cgroup_kn_unlock(kn);
	return ret;
}
...
static int css_panic_dump(struct notifier_block *self,
			  unsigned long v, void *p)
{
	pr_err("inc_0=%d dec_0=%d mg_freed=%d\n",
	       atomic_read(&g_css_inc_0),
	       atomic_read(&g_css_dec_0),
	       atomic_read(&g_css_mg_freeed));
	return 0;
}

static struct notifier_block css_panic_block = {
	.notifier_call = css_panic_dump
};

static int __init register_css_panic_block(void)
{
	atomic_notifier_chain_register(&panic_notifier_list,
				       &css_panic_block);
	return 0;
}
device_initcall(register_css_panic_block);
#endif /* CONFIG_SYSFS */
