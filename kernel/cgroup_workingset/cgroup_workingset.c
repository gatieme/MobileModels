 ...
 *
 * Copyright (c) 2017-2020 Honor Device Co., Ltd
 *
...
static void workingset_attach(struct cgroup_taskset *tset)
{
	...
	cgroup_taskset_for_each(task, new_css, tset) {
		...
		if ((ws->state & E_CGROUP_STATE_MONITOR_INWORKING) ==
			...
			task_set_wscg_monitor(task);
		} else {
			task_clear_wscg_monitor(task);
		}
	}
...
static void workingset_mark_target_process(int pid, bool clear)
{
	...
	do {
		if (clear)
			task_clear_wscg_preread(task);
		else
			task_set_wscg_preread(task);
...
static int workingset_data_parse_owner(struct s_workingset *ws, char *str)
{
	...
	init_owner(ws, uid, pid, owner_name, record_path);
	return 0;
...
#ifdef CONFIG_HN_CGROUP_WORKINGSET_DEBUG
...
#if defined(CONFIG_HN_VIP_THREAD)
...
static void wscg_trace_newtask(void *ignore, struct task_struct *task,
...
static int wscg_register_tracepoints(void)
{
	int ret;

	ret = register_trace_task_newtask(wscg_trace_newtask, NULL);
	WARN_ON(ret);

	return ret;
}

static void wscg_unregister_tracepoints(void)
{
	unregister_trace_task_newtask(wscg_trace_newtask, NULL);
}
...
static void __exit cgroup_workingset_exit(void)
{
	...
	wscg_unregister_tracepoints();
	workingset_record_list_deinit();
