...
static struct dentry *g_record_list_dentry;

static void wscg_record_debug_show(struct seq_file *m, struct s_ws_record *record)
{
	struct s_path_node *path_nodes = record->data.file_array;
	unsigned int file_idx;
	unsigned int stage;
	unsigned int is_major;
	unsigned int page_begin;
	unsigned int page_end;
	unsigned int i = 0;
	unsigned int *cacheseq = record->data.cacheseq;

	seq_printf(m, "stage major page_begin page_num  path:%s\n", record->owner.name);
	for (i = 0; i < record->data.pageseq_cnt; i++) {
		file_idx = (cacheseq[i] >> FILE_OFFSET_BITS) & MAX_TOUCHED_FILES_COUNT;
		if ((file_idx >= record->data.file_cnt) || !path_nodes[file_idx].path)
			continue;
		stage = (cacheseq[i] >> PAGE_STAGE_NUM_SHIFT) & PAGE_STAGE_NUM_MASK;
		is_major = (cacheseq[i] << PAGE_MAJOR_SHIFT) & PAGE_MAJOR_MASK;
		page_begin = cacheseq[i] & MAX_TOUCHED_FILE_OFFSET;
		if (cacheseq[i] & PAGE_RANGE_HEAD_BIT_MASK) {
			page_end = cacheseq[++i] & MAX_TOUCHED_FILE_OFFSET;
		} else {
			page_end = page_begin + 1;
		}
		seq_printf(m, "%-5u %-5u %-10u %-8u  %-s\n", stage, is_major,
			page_begin, page_end - page_begin,
			path_nodes[file_idx].path);
	}
}

static int wscg_records_debug_show(struct seq_file *m, void *private)
{
	struct s_ws_record *record;

	spin_lock(&g_record_list_lock);
	list_for_each_entry(record, &g_record_list, list) {
		wscg_record_debug_show(m, record);
	}
	spin_unlock(&g_record_list_lock);
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(wscg_records_debug);
#endif

static void __init workingset_init_debugfs(void)
{
#if defined(CONFIG_DEBUG_FS)
	g_record_list_dentry = debugfs_create_dir("workingset_cgroup", NULL);

	debugfs_create_file("records", 0444, g_record_list_dentry,
			    NULL, &wscg_records_debug_fops);
#endif
}

static void __exit workingset_deinit_debugfs(void)
{
#if defined(CONFIG_DEBUG_FS)
	debugfs_remove_recursive(g_record_list_dentry);
	g_record_list_dentry = NULL;
#endif
}
...
void workingset_record_list_init(void)
{
	...
	if (nr_total > TOTAL_RAM_PAGES_3G)
		g_max_records_count = MAX_RECORD_COUNT_ON_4G;
	else if (nr_total > TOTAL_RAM_PAGES_2G)
		g_max_records_count = MAX_RECORD_COUNT_ON_3G;
	else if (nr_total > TOTAL_RAM_PAGES_1G)
		g_max_records_count = MAX_RECORD_COUNT_ON_2G;
	else
		g_max_records_count = MAX_RECORD_COUNT_ON_1G;

	spin_lock_init(&g_record_list_lock);
	workingset_init_debugfs();
}

void workingset_record_list_deinit(void)
{
	struct s_ws_record *record;
	struct s_ws_record *next;

	spin_lock(&g_record_list_lock);
	list_for_each_entry_safe(record, next, &g_record_list, list) {
		list_del(&record->list);
		kfree(record);
	}
	g_record_cnt = 0;
	spin_unlock(&g_record_list_lock);
	workingset_deinit_debugfs();
}
	...
	unsigned int *pplayload_length, unsigned int *pchecksum)
{
	...
	*pchecksum = 0;
	...
	ret = workingset_record_writeback_data(filp, record->data.cacheseq,
		sizeof(unsigned int) * record->data.pageseq_cnt,
		pchecksum, &pos);
	if (ret)
		return ret;
...
int workingset_prepare_record_space_wsrcrdlocked(
	const struct s_ws_owner *owner, struct s_ws_record *record,
	bool is_exist, unsigned int playload_size, void **pplayload)
{
...
static int workingset_get_record_playload(
	...
	struct file *filp, struct s_ws_record *record, void *playload)
{
	int len;
	int ret;
...
static void init_record_with_data(
	const struct s_ws_owner *owner,
	const struct s_ws_backup_record_header *header,
	struct s_ws_record *record, void *playload)
{
	struct s_ws_data *data = &record->data;
	unsigned int idx;
	...
	const struct s_ws_owner *owner)
{
	...
	init_record_with_data(owner, &header, record, playload);
