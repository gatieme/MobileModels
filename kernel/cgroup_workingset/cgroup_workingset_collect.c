	...
	int *seq_delta)
{
	...
	if (pc.major) {
		...
		(pc.major & PAGE_MAJOR_MASK) << PAGE_MAJOR_SHIFT;
	...
	const unsigned int hashcode, const unsigned int path_len)
{
	...
	info->path_node.ino = file->f_inode->i_ino;
	info->path_node.file_size = file->f_inode->i_size;
	...
	struct s_cachepage_info *info, struct s_workingset *ws)
{
	...
	if (d_unlinked(file_dentry(info->filp))) {
		return -EPERM;
	}
