...
#elif (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE && KERNEL_VERSION(5, 15, 0) > LINUX_VERSION_CODE)
...
#else
static void workingset_readahead(struct address_space *mapping,
				struct file *filp, pgoff_t index,
				unsigned int nr_to_read)
{
#if (KERNEL_VERSION(5, 15, 0) <= LINUX_VERSION_CODE)
	DEFINE_READAHEAD(ractl, filp, NULL, mapping, index);
#else
...
static int workingset_page_cache_range_read(struct s_readpages_control *rpc)
{
	...
	loff_t isize = i_size_read(rpc->mapping->host);
	pgoff_t index = rpc->offset;
	pgoff_t page_count = DIV_ROUND_UP(isize, PAGE_SIZE);
	pgoff_t end_index = min(rpc->offset + rpc->nr_to_read - 1, page_count - 1);
	pgoff_t begin_index;
	struct pagevec pvec;
	unsigned int nr_pages;
	unsigned int i;
	unsigned int readpages = 0;
	rpc->nr_adjusted = 0;

	if (unlikely(!rpc->mapping->a_ops->readpage &&
		!rpc->mapping->a_ops->readahead)) {
		pr_err("%s,a_ops no readpage\n", __func__);
		return 0;
	}
	...
	while (index <= end_index) {
		begin_index = index;
		pagevec_init(&pvec);
		nr_pages = pagevec_lookup_range(&pvec, rpc->mapping, &index, end_index);
		if (!nr_pages) {
			workingset_readahead(rpc->mapping, rpc->filp, begin_index, index - begin_index);
			readpages += index - begin_index;
			continue;
		}
	...
	struct s_ws_record *record, struct file **filpp)
{
	...
	while (next_loop < data->file_cnt) {
		...
		if (target_euid != myself_euid) {
			...
			__sys_setresuid((gid_t)-1, target_euid, (gid_t)-1);
#else
		...
		for (idx = next_loop, next_loop = data->file_cnt;
			...
			if (!IS_ERR_OR_NULL(filpp[idx])) {
				if ((data->file_array[idx].ino != filpp[idx]->f_inode->i_ino) &&
					(data->file_array[idx].file_size >= NEED_UPDATE_FILE_SIZE_THRESOLD)) {
					ws_dbg("%s find ino-changed-file path:%s\n",
						__func__, data->file_array[idx].path);
					record->need_update = 1;
				}
				continue;
			}
		...
		if (target_euid != myself_euid)
#ifdef CONFIG_ARCH_HAS_SYSCALL_WRAPPER
			__sys_setresuid((gid_t)-1, myself_euid, (gid_t)-1);
#else
	...
	struct file *filp)
{
	...
	struct inode *real_inode = d_real_inode(filp->f_path.dentry);
	rpc->mapping = real_inode->i_mapping;
	rpc->filp = filp;
#else
	rpc->filp = filp;
