...
#else
#ifdef CONFIG_MM_AMA
	struct page *page = ama_alloc_kernel_large(THREADINFO_GFP,
						THREAD_SIZE_ORDER);
#else
...
#ifdef CONFIG_MM_VIP
	mm->mm_vip_tag = oldmm->mm_vip_tag;
#endif
...
void __mmdrop(struct mm_struct *mm)
{
	...
	rwsem_release_reader_info(&mm->mmap_lock);
#endif
	free_mm(mm);
}
...
static int copy_signal(unsigned long clone_flags, struct task_struct *tsk)
{
	...
	/* all new child don't inherit it */
					...
					struct kernel_clone_args *args)
{
	...
	hkip_init_cred_xidhash(p->cred, p);
	/*
	...
	retval = phase_fork(p);
	if (retval)
		goto bad_fork_cleanup_after_phase;
#endif
#ifdef CONFIG_MIPS_LOAD_TRACKING
	retval = mips_fork(p);
	if (retval)
		goto bad_fork_cleanup_after_mips;
#endif
...
#if defined(CONFIG_HW_VIP_THREAD) && defined(CONFIG_HW_GRADED_SCHED)
	if (p->group_leader != NULL)
		init_graded_nice(p, p->group_leader->static_vip);
#endif
...
#ifdef CONFIG_MIPS_LOAD_TRACKING
	mips_free(p);
bad_fork_cleanup_after_mips:
#endif

#ifdef CONFIG_PHASE
	phase_free(p);
bad_fork_cleanup_after_phase:
#endif
	exit_thread(p);
bad_fork_cleanup_io:
	if (p->io_context)
