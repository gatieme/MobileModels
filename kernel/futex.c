...
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
			trace_android_vh_honor_futex_vip_unset(this->owner);
#endif
...
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
	if (!timeout) {
		trace_android_vh_honor_futex_vip_set(q->owner);
	}
#endif
...
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
	if (bitset == FUTEX_BITSET_MATCH_ANY && owner_tid > 500) {
		struct task_struct *owner_task = NULL;
		rcu_read_lock();
		owner_task = find_task_by_vpid(owner_tid);
		if (owner_task != NULL)
