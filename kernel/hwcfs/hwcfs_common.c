 ...
 *
 * Copyright (c) 2017-2023 Huawei Technologies Co., Ltd.
 *
...
int vip_min_migration_delay = 2; /* vip min migration delay time(ms) */

#define S2NS_T 1000000

#if defined(CONFIG_HISI_EAS_SCHED)
#define VIP_ENQUEUE_TOO_LONG (2000 * NSEC_PER_MSEC)
#endif
...
#if defined(CONFIG_HISI_EAS_SCHED)
				p->static_vip &= (~ENQUEUE_TOO_LONG_VIP_STATE);
#endif
				put_task_struct(p);
...
#if defined(CONFIG_HISI_EAS_SCHED)
	if (left_most_task && ((s64)(rq->clock -
		left_most_task->last_queued) > VIP_ENQUEUE_TOO_LONG)) {
		if ((left_most_task->static_vip & ENQUEUE_TOO_LONG_VIP_STATE) !=
			ENQUEUE_TOO_LONG_VIP_STATE) {
			trace_sched_vip_sched(left_most_task, "skip_vip");
			left_most_task->static_vip |= ENQUEUE_TOO_LONG_VIP_STATE;
			printk(KERN_WARNING "task(pid:%d, comm:%s) go to the cfs",
				left_most_task->pid, left_most_task->comm);
		}
		list_move_tail(&left_most_task->vip_entry, &rq->vip_thread_list);
		/* Go to the cfs pick. */
		return NULL;
	}
#endif
...
static void __dynamic_vip_dequeue(struct task_struct *task, int type)
{
	...
	if (type == DYNAMIC_VIP_FUTEX) {
		lockdep_assert_held(&task->pi_lock);
		rq = __task_rq_lock(task, &flags);
	} else {
...
static void try_to_unthrottle_userspace_task(struct rq *rq, struct task_struct *task)
{
	if (task_userspace_throttled(task)) {
		update_rq_clock(rq);
		unthrottle_userspace_task(task);
	}
}
#endif
...
#ifdef CONFIG_HW_USERSPACE_THROTTLE
	try_to_unthrottle_userspace_task(rq, task);
#endif
...
inline bool test_task_vip(struct task_struct *task)
{
	return task && (task->static_vip || atomic64_read(&task->dynamic_vip)) &&
		(task->nr_cpus_allowed > VIP_USE_CPU_ALLOWED);
}
...
static struct task_struct *check_vip_delayed(struct rq *rq)
{
	...
	list_for_each_safe(pos, n, vip_thread_list) {
		...
		if (tsk && tsk->nr_cpus_allowed == 1 && tsk->static_vip != 0) {
			tsk->static_vip = 0;
			list_del_init(&tsk->vip_entry);
			continue;
		}
#endif
		if (!task_current(rq, tsk) &&
			(rq->clock - tsk->enqueue_time) >= interval)
			return tsk;
	}
	return NULL;
}
...
static struct cpumask slow_cpu_mask;
#endif
	...
	struct rq *dst_rq, struct task_struct *task, int cpu_id)
{
	...
	if (cpu_id < nr_cpu_ids && cpu_id >= 0 && p != NULL && dst_rq != NULL) {
...
static bool is_dst_rq_valid(struct rq* dst_rq)
{
	return dst_rq &&
		!dst_rq->rt.rt_nr_running &&
#ifdef CONFIG_SCHED_SMT_EXPELLING
		!rq_forced_idle(dst_rq) &&
#endif
		list_empty(&dst_rq->vip_thread_list);
}

static int get_sched_dest_rq(int src_cpu, const struct cpumask *cpus_ptr, struct rq **dst_rq)
{
	int i;
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		if (i == src_cpu || !cpu_online(i))
			continue;
		if (cpumask_test_cpu(i, cpus_ptr)) {
			*dst_rq = cpu_rq(i);
			raw_spin_rq_lock(*dst_rq);
			if (is_dst_rq_valid(*dst_rq)) {
				raw_spin_rq_unlock(*dst_rq);
				break;
			}
			raw_spin_rq_unlock(*dst_rq);
...
static struct rq *get_dest_rq(int src_cpu,
	const struct cpumask *cpus_ptr, int *cpu_id)
{
	int i;
	struct rq *dst_rq = NULL;

#if defined(CONFIG_HW_EAS_SCHED) || defined(CONFIG_HISI_EAS_SCHED)
	i = get_sched_dest_rq(src_cpu, cpus_ptr, &dst_rq);
#else
	if (cpumask_weight(cpu_coregroup_mask(src_cpu)) == 1) {
		...
		for_each_cpu_and(i, cpus_ptr, cpu_coregroup_mask(src_cpu)) {
			if (i == src_cpu || !cpu_online(i))
				continue;
			dst_rq = cpu_rq(i);
			raw_spin_rq_lock(dst_rq);
...
static int find_delayed_vip_task(int src_cpu, struct rq *src_rq, struct task_struct **p)
{
...
static int __do_vip_balance(void *data)
{
	...
	raw_spin_rq_lock_irq(src_rq);
	if (find_delayed_vip_task(src_cpu, src_rq, &p) == 0) {
		raw_spin_rq_unlock_irq(src_rq);
		return 0;
	}

	/* find a free-cpu */
#if defined(CONFIG_HW_EAS_SCHED) || defined(CONFIG_HISI_EAS_SCHED)
	get_fast_cpus(&cluster);
	get_slow_cpus(&slow_cpu_mask);
	if (cpumask_test_cpu(src_cpu, &cluster))
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
		cpumask_and(&cpus_allowed, &cluster, p->cpus_ptr);
#else
		cpumask_and(&cpus_allowed, &cluster, &(p->cpus_allowed));
#endif
	else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
		cpumask_and(&cpus_allowed, &slow_cpu_mask, p->cpus_ptr);
#else
		cpumask_and(&cpus_allowed, &slow_cpu_mask, &(p->cpus_allowed));
#endif
	raw_spin_rq_unlock(src_rq);
	...
	raw_spin_rq_unlock(src_rq);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	dst_rq = get_dest_rq(src_cpu, p->cpus_ptr, &i);
#else
...
#ifdef CONFIG_HW_USERSPACE_THROTTLE
		try_to_unthrottle_userspace_task(rq, p);
#endif
