...
#ifdef CONFIG_HW_RTG_SCHED_LOCK_PI
static inline bool is_qos_trans_type_enabled(unsigned int type)
{
	return ((type == DYNAMIC_QOS_BINDER) || (type == DYNAMIC_QOS_RWSEM) ||
		(type == DYNAMIC_QOS_MUTEX));
}
#else
static inline bool is_qos_trans_type_enabled(unsigned int type)
{
	return (type == DYNAMIC_QOS_BINDER);
}
#endif
...
static void register_qos_trans_allows(struct task_struct *task, int qos)
{
	...
	if (unlikely(i == QOS_TRANS_THREADS_NUM)) {
		...
		for (i = 1; i < QOS_TRANS_THREADS_NUM; i++)
			(void)memset_s(&g_qos_trans_allows[i],
...
static void unregister_qos_trans_allows(struct task_struct *task, bool by_pid)
{
	int i;
	unsigned long flags = 0;
	...
	spin_lock_irqsave(&g_trans_qos_lock, flags);
	for (i = 0; i < QOS_TRANS_THREADS_NUM; i++) {
		if ((by_pid && g_qos_trans_allows[i].allow_tgid != task->tgid) ||
		    (!by_pid && g_qos_trans_allows[i].allow_pid != task->pid))
			continue;
		(void)memset_s(&g_qos_trans_allows[i],
			sizeof(g_qos_trans_allows[i]),
			0,
...
void set_task_qos_by_tid(struct task_struct *task, int qos)
{
	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		unregister_qos_trans_allows(task, false);
...
void set_task_qos_by_pid(struct task_struct *task, int qos)
{
	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		unregister_qos_trans_allows(task, true);
...
EXPORT_SYMBOL_GPL(get_task_qos);
#endif

#ifdef CONFIG_HW_FUTEX_PI
int get_task_qos_by_type(struct task_struct *task, unsigned int type)
{
	if (unlikely(!is_qos_trans_type_valid(type)))
		return VALUE_QOS_INVALID;
	if (unlikely(!task))
		return VALUE_QOS_INVALID;
	return get_trans_qos_by_type(task, type);
}
#endif
	...
	struct task_struct *from, unsigned int type)
{
	...
	if (!MIN_UTIL_TRANS_ENABLE || !is_qos_trans_type_enabled(type))
		return;

	if (task->min_util_params.trans_flag)
		return;
	min_util_desired = get_task_min_util(from);
	if (min_util_desired == 0 || min_util_desired <= get_task_min_util(task))
		return;
	task->min_util_params.value = get_task_min_util(task);
...
static void dynamic_smt_expeller_enqueue(struct task_struct *task,
	struct task_struct *from, unsigned int type)
{
	unsigned int target_smt_expeller;
	unsigned int curr_smt_expeller;

	struct task_group *tgrp = NULL;

	tgrp = task_group(from);
	target_smt_expeller = tgrp ? max(from->smt_expeller, tgrp->smt_expeller) : from->smt_expeller;

	tgrp = task_group(task);
	curr_smt_expeller = max(task->smt_expeller, task->dynamic_smt_expeller);
	if (tgrp)
		curr_smt_expeller = max(curr_smt_expeller, tgrp->smt_expeller);

	if (target_smt_expeller == SMT_EXPELLER && target_smt_expeller > curr_smt_expeller) {
		task->dynamic_smt_expeller = target_smt_expeller;
		trace_smt_expel_trans_start(from, task);
	}
}

static void dynamic_smt_expeller_dequeue(struct task_struct *task, unsigned int type)
{
	unsigned int dynamic_smt_expeller = task->dynamic_smt_expeller;

	if (dynamic_smt_expeller > SMT_EXPELLEE) {
		task->dynamic_smt_expeller =  SMT_EXPELLEE;
		trace_smt_expel_trans_end(task);
	}
}
#endif
	...
	struct task_struct *from, unsigned int type)
{
	...
	if (trans_qos) {
		...
		tq->trans_type = type;
#ifdef CONFIG_HW_RTG_SCHED
		if (RTG_TRANS_ENABLE && is_qos_trans_type_enabled(type))
			add_trans_thread(task, from);
#endif
#if IS_ENABLED(CONFIG_COMMON_HW_RTG_FRAME)  || defined(CONFIG_HW_MTK_RTG_SCHED)
		if (RTG_TRANS_ENABLE && is_qos_trans_type_enabled(type))
			trace_android_rvh_trans_rtg_sched_enqueue(task, from, type);
#endif
...
#endif
#ifdef CONFIG_SCHED_SMT_EXPELLING
	dynamic_smt_expeller_enqueue(task, from, type);
#endif
#ifdef CONFIG_SCHED_HISI_TASK_MIN_UTIL
...
static void trans_schedule_policy(struct task_struct *task, bool set_rt)
{
	int err;
	...
	if (err < 0)
		pr_err("Failed to set trans schedule policy for pid:%d, prio:%d\n",
			task->pid, sp.sched_priority);
}

bool dynamic_qos_async_binder(struct task_struct *task, struct task_struct *from)
{
	bool ret = false;
	int flags;

	if (unlikely(ASYNC_BINDER_TRANS_DISABLE))
		return ret;

	if (unlikely(!from) || unlikely(!task))
		return ret;

	if (task_has_rt_policy(from) && !task_has_rt_policy(task)) {
		trans_schedule_policy(task, true);
		flags = set_trans_type(atomic_read(&task->trans_flags),
...
void dynamic_qos_deque_async_binder(struct task_struct *task)
{
	int flags;
	...
	if (!get_trans_type(flags, DYNAMIC_QOS_ASYNC_BINDER))
		return;

	trans_schedule_policy(task, false);
	flags = remove_trans_type(flags, DYNAMIC_QOS_ASYNC_BINDER);
...
#endif
#ifdef CONFIG_SCHED_SMT_EXPELLING
	dynamic_smt_expeller_dequeue(task, type);
#endif
...
#if IS_ENABLED(CONFIG_COMMON_HW_RTG_FRAME) || defined(CONFIG_HW_MTK_RTG_SCHED)
	if (RTG_TRANS_ENABLE && is_qos_trans_type_enabled(type))
		trace_android_rvh_trans_rtg_sched_dequeue(task, type);
#endif
