...
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
	cowner = __mutex_owner(lock);
	if (cowner)
		trace_mutex_lock_contention(cowner->pid, jiffies);
#endif
...
skip_wait:
	...
	if (cowner) {
		...
		if (lock->dep_map.name)
			trace_mutex_lock_contention_length(lock->dep_map.name, cowner->pid, jiffies);
#else
		trace_mutex_lock_contention_length("default", cowner->pid, jiffies);
#endif
	}
#endif
