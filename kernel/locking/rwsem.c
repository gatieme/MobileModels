...
#include <linux/sched/hwqos/hwqos_rwsem.h>
#endif
...
#ifdef CONFIG_HN_QOS_THREAD
/*
 * Return the real task structure pointer of the owner and LSB flag.
 */
static inline struct task_struct *rwsem_owner_with_flag(struct rw_semaphore *sem)
{
	return (struct task_struct *)
		(atomic_long_read(&sem->owner) & ~RWSEM_NONSPINNABLE);
}
...
{
	return (struct task_struct *)(owner & ~RW_SEM_FLAGS);
}
#endif
...
rwsem_down_read_slowpath(struct rw_semaphore *sem, long count, unsigned int state)
{
...
#ifdef CONFIG_HN_RW_SEM_DEBUG
	unsigned long start_temp = jiffies;
	unsigned long owner_addr = atomic_long_read(&sem->owner);
	pid_t owner_pid = 0;
	struct task_struct *p_owner_task = __owner_task(owner_addr);
	if (p_owner_task != NULL)
		owner_pid = p_owner_task->pid;
#endif
	 ...
	 */
#ifdef CONFIG_HN_RW_SEM_DEBUG
	if ((owner_addr & RWSEM_READER_OWNED) &&
...
queue:
...
#ifdef CONFIG_HN_QOS_THREAD
	rwsem_dynamic_qos_enqueue(rwsem_owner_with_flag(sem), waiter.task);
#endif
	...
	for (;;) {
#ifdef CONFIG_HN_RW_SEM_DEBUG
		if (jiffies_to_msecs(jiffies - start_temp) > 100) {
			trace_rw_sem_debug(&waiter, (char *)__func__, CALLER_ADDR0,
			 CALLER_ADDR1, CALLER_ADDR2, CALLER_ADDR3, CALLER_ADDR4, CALLER_ADDR5);
			printk(KERN_ERR
			"D state too long, func=%s, owner_pid=%d, current_pid=%d\n",
			 (char *)__func__, owner_pid, current->pid);
			start_temp = jiffies;
                }
#endif
...
rwsem_down_write_slowpath(struct rw_semaphore *sem, int state)
{
	...
	unsigned long start_temp = jiffies;
	pid_t owner_pid = 0;
	struct task_struct *p_owner_task = __owner_task(atomic_long_read(&sem->owner));
	if (p_owner_task != NULL)
		owner_pid = p_owner_task->pid;
#endif
