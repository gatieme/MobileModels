	...
	enum rb_trace_type mode)
{
	...
	for (i = 0; i < (int)cpu_num; i++) {
		buffer->percpu_addr[i] =
			...
			(unsigned)(i * (buffer->percpu_length)));
...
void rb_trace_init(void *paddr_start, void *vaddr_start)
{
	...
	g_rb_trace_mem[TR_IRQ].paddr = (unsigned long)paddr_start + RB_TRACE_IRQ_OFFSET;
	g_rb_trace_mem[TR_IRQ].size = RB_TRACE_SECTION_SIZE;
	g_rb_trace_mem[TR_IRQ].vaddr = (unsigned long)vaddr_start + RB_TRACE_IRQ_OFFSET;
	...
	g_rb_trace_mem[TR_TASK].paddr = (unsigned long)paddr_start + RB_TRACE_TASK_OFFSET;
	g_rb_trace_mem[TR_TASK].size = RB_TRACE_SECTION_SIZE;
	g_rb_trace_mem[TR_TASK].vaddr = (unsigned long)vaddr_start + RB_TRACE_TASK_OFFSET;
