...
DEFINE_PER_CPU(struct sugov_cpu, sugov_cpu);
...
void sugov_deferred_update(struct sugov_policy *sg_policy, u64 time,
				  unsigned int next_freq)
{
	if (!sugov_update_next_freq(sg_policy, time, next_freq))
		return;
				  ...
				  unsigned long util, unsigned long max)
{
	...
	CALL_HCK_VH(map_util_freq_hook, sg_policy, util, max, &next_freq);
...
#ifdef CONFIG_CPU_FREQ_GOV_MIPS
static void sugov_sys(struct kthread_work *work)
{
	struct sugov_policy *sg_policy = container_of(work, struct sugov_policy, sysfs_work);
	struct cpufreq_policy *policy = sg_policy->policy;
	gov_sysfs_set_attr(policy->cpu, schedutil_gov.name, schedutil_user_attrs);

}

static void create_sys_set(struct sugov_policy *sg_policy)
{
	kthread_init_work(&sg_policy->sysfs_work, sugov_sys);
	kthread_queue_work(&sg_policy->worker, &sg_policy->sysfs_work);
}
#endif
...
{
	struct task_struct *thread;
#if defined CONFIG_CPU_FREQ_GOV_SCHEDUTIL_OPT || !defined CONFIG_SCHED_DEADLINE_ENABLE
