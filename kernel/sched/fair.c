...
EXPORT_SYMBOL_GPL(sysctl_sched_migration_cost);
#endif
...
static bool high_prio_task_in_mmap_lock(struct task_struct *p)
{
	...
	if (tg) {
		...
		if (tg->uclamp[UCLAMP_MIN].value > 0)
			return true;
#endif
#ifdef CONFIG_FAIR_GROUP_SCHED
		if (tg->shares > NICE_0_LOAD)
			return true;
#endif
	}
	return false;
...
#ifdef CONFIG_TASK_PRI_PROMOTE_FOR_MMAPLOCK
	if (entity_is_task(se) && high_prio_task_in_mmap_lock(task_of(se)))
		se->vruntime -= sysctl_sched_latency; /* backwards */
#endif
