...
static int ffrt_rtg_add_thread(struct ffrt_rtg_msg *msg)
{
	...
	ret = ffrt_set_group_id(tid, msg->tgid);
	if (ret < 0) {
...
static int ffrt_rtg_set_group_util(struct ffrt_rtg_msg *msg)
{
	...
	ret = sched_set_group_normalized_util(msg->tgid, util, 1);
	trace_perf(ffrt_rtg_update_tgid_util, msg->tgid, util);
	if (ret < 0) {
...
static int ffrt_rtg_set_group_freq(struct ffrt_rtg_msg *msg)
{
	...
	ret = sched_set_group_freq(msg->tgid, freq);
	if (ret < 0) {
...
{
	struct related_thread_group *grp = lookup_related_thread_group(tgid);
	if (grp == NULL)
		return -RTG_FAILED;
...
static int ffrt_rtg_get_group_load(struct ffrt_rtg_msg *msg)
{
	...
	trace_perf(ffrt_rtg_update_group_load, msg->tgid, info.load, info.runtime);
	if (copy_to_user(msg->out, &info, sizeof(info))) {
...
static int ffrt_rtg_set_group_window_rollover(struct ffrt_rtg_msg *msg)
{
	...
	ret = sched_set_group_window_rollover(msg->tgid);
	if (ret < 0) {
...
{
	int cluster_id = msg->data;
	int ret;

	ffrt_rtg_assert_cmd(msg->cmd, RTG_CMD_SET_PREFERRED_CLUSTER);

	ret = sched_set_preferred_cluster(msg->tgid, cluster_id);
	if (ret < 0) {
...
static int ffrt_rtg_set_invalid_interval(struct ffrt_rtg_msg *msg)
{
	...
	ret = sched_set_group_util_invalid_interval(msg->tgid, msg->data);
	if (ret < 0) {
