...
static int sched_set_task_qos_attr(struct task_struct *p, int level, int status)
{
	...
	if (policy_map->policy_flag & QOS_FLAG_UCLAMP) {
		...
		attr.sched_util_min = item->uclamp_min;
		attr.sched_util_max = item->uclamp_max;
	}
	...
	trace_perf(ffrt_qos_attr, level, p->pid, attr.sched_latency_nice, attr.sched_util_min, attr.sched_util_max,
...
int qos_leave(struct qos_ctrl_data *data)
{
	 ...
	 */
	ret = sched_set_task_qos_attr(p, level, QOS_POLICY_DEFAULT);
	if (ret)
