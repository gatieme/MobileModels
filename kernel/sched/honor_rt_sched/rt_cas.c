...
#ifdef CONFIG_HONOR_GLOBAL_BOOST
...
#endif
#if defined(CONFIG_HONOR_RTG_NORMALIZED_UTIL) || defined(CONFIG_HONOR_GLOBAL_BOOST)
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	bool boosted = schedtune_task_boost(task) > 0;
#else
	bool boosted = task_sched_boost(task);
#endif
#endif
...
#if defined(CONFIG_HONOR_GLOBAL_BOOST)
...
#else
			cpu_isolated(cpu), honor_walt_cpu_high_irqload(cpu),
#endif
...
#else
		if (honor_walt_cpu_high_irqload(cpu))
#endif
			continue;
