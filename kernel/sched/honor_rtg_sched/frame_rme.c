...
long ctrl_set_rme_margin(unsigned long arg)
{
	...
	if (g_fps_data[PONG].frame_rate == fps_data.frame_rate) {
		...
		return SUCC;
	} else if (set_frame_rate(fps_data.frame_rate)) {
		atomic_set(&g_fps_state, INIT_STATE);
		return -INVALID_ARG;
	}

	/* check and set frame min_prev_util */
	if (g_fps_data[PONG].min_prev_util != fps_data.min_prev_util) {
		if (set_frame_min_prev_util(fps_data.min_prev_util)) {
			atomic_set(&g_fps_state, INIT_STATE);
			return -INVALID_ARG;
		}
	}

	/* check and set frame max_prev_util */
	if (g_fps_data[PONG].max_prev_util != fps_data.max_prev_util) {
		if (set_frame_max_prev_util(fps_data.max_prev_util)) {
			atomic_set(&g_fps_state, INIT_STATE);
			return -INVALID_ARG;
		}
	}
