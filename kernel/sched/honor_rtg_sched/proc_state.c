...
DECLARE_BITMAP(state_bitmap, STATE_MAX);
#endif

static atomic_t g_rtg_uid;
atomic_t g_rtg_pid;
atomic_t g_rtg_tid;
atomic_t g_rtg_hwuitask0_tid;
atomic_t g_rtg_hwuitask1_tid;

atomic_t g_act_state;
...
#else
static void set_frame(int frame_type)
{
	atomic_set(&g_frame_state, frame_type);
}
...
static int put_state_margin(const char *state_str, const char *margin_str)
{
	...
	if (test_and_set_bit(state, state_bitmap)) {
		kfree(temp);
	} else {
...
#elif defined CONFIG_HONOR_VIP_THREAD
...
static void update_rtg_frame_thread(const struct rtg_data_head *proc_info)
{
	...
	int hwuitask0_tid = proc_info->hwuitask0_tid;
	int hwuitask1_tid = proc_info->hwuitask1_tid;

	if ((uid == -1) || (pid == -1) || (tid == -1)) {
		atomic_set(&g_rtg_uid, -1);
		atomic_set(&g_rtg_pid, -1);
		atomic_set(&g_rtg_tid, -1);
		atomic_set(&g_rtg_hwuitask0_tid, -1);
		atomic_set(&g_rtg_hwuitask1_tid, -1);
		update_frame_thread(-1, -1, -1, -1);
		return;
	}
	...
	if (hwuitask0_tid != atomic_read(&g_rtg_hwuitask0_tid))
		atomic_set(&g_rtg_hwuitask0_tid, hwuitask0_tid);
	if (hwuitask1_tid != atomic_read(&g_rtg_hwuitask1_tid))
		atomic_set(&g_rtg_hwuitask1_tid, hwuitask1_tid);

	update_frame_thread(pid, tid, hwuitask0_tid, hwuitask1_tid);
	pr_debug("[AWARE_RTG] %s uid=%d, pid=%d, tid=%d\n, hwuitask0_tid=%d, hwuitask1_tid=%d",
			__func__, atomic_read(&g_rtg_uid),
			atomic_read(&g_rtg_pid), atomic_read(&g_rtg_tid),
			atomic_read(&g_rtg_hwuitask0_tid), atomic_read(&g_rtg_hwuitask1_tid));
}
...
static void free_margin_list(void)
{
	...
	bitmap_zero(state_bitmap, STATE_MAX);
	write_unlock(&g_state_margin_lock);
}
#endif /* CONFIG_USE_RTG_FRAME_SCHED */
...
EXPORT_SYMBOL_GPL(is_key_aux_comm);
#endif
...
int parse_frame_thread(const struct rtg_str_data *rs_data)
{
	...
	proc_info.pid = 0;
	proc_info.tid = 0;
	proc_info.hwuitask0_tid = 0;
	proc_info.hwuitask1_tid = 0;
	// eg: fgUid:10000;uiTid:10000;renderTid:10000;hwuiTask0Tid:10000;hwuiTask1Tid:10000
	for (p = strsep(&data, ";"); p != NULL; p = strsep(&data, ";")) {
		tmp = strsep(&p, ":");
		if (tmp == NULL || p == NULL)
			continue;
		...
		if (!strcmp(tmp, "fgUid")) {
			...
			proc_info.tid = id;
		} else if (!strcmp(tmp, "hwuiTask0Tid")) {
			proc_info.hwuitask0_tid = id;
		} else if (!strcmp(tmp, "hwuiTask1Tid")) {
			proc_info.hwuitask1_tid = id;
		} else {
			pr_err("[AWARE_RTG] parse config failed!\n");
			return -INVALID_ARG;
		}
...
#if ((!defined(CONFIG_HN_QOS_THREAD)) && (!defined(CONFIG_HONOR_VIP_THREAD)))
	return -ACT_ERR_QOS;
#endif
...
int is_cur_frame(void)
{
	...
	if ((current->pid == atomic_read(&g_rtg_pid)) ||
		...
		(current->pid == atomic_read(&g_rtg_hwuitask0_tid)) ||
		(current->pid == atomic_read(&g_rtg_hwuitask1_tid)))
		return 1;
	return 0;
}
...
bool is_frame_freq_enable(void)
{
