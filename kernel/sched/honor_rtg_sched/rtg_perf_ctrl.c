...
int perf_ctrl_set_task_rtg(void __user *uarg)
{
	...
	ret = honor_sched_set_group_id_by_pid(task.pid, task.grp_id);
	return ret;
}
...
int perf_ctrl_set_cluster_freq(void __user *uarg)
{
	...
	if (copy_from_user(&cfg, uarg, sizeof(struct cluster_config)))
		return -EFAULT;
	...
	ret = set_task_rtg_cluster_freq(p, cfg.cluster, cfg.min_freq, cfg.max_freq);
