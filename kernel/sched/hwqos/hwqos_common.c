	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		if (!strncmp("ndroid.launcher", task->comm, TASK_COMM_LEN)) {
...
#endif
			pr_err("-%s- QQOS_DEBUG Try-set-Launcher-T-M  pid:%d qos:%d\n.",
					__func__, task->pid, qos);
			return;
		} else {
			...
			if (p && !strncmp("ndroid.launcher", p->comm, TASK_COMM_LEN)) {
				...
				trace_rtg_frame_sched("LAUNCHER_TID", task->pid);
#endif
				pr_err("-%s- QQOS_DEBUG Try-set-Launcher-T-T  task:%s pid:%d qos:%d.\n",
						__func__, task->comm, task->pid, qos);
				return;
			}
		}
...
void set_task_qos_by_pid(struct task_struct *task, int qos)
{
	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		if (!strncmp("ndroid.launcher", task->comm, TASK_COMM_LEN)) {
#ifdef CONFIG_HONOR_RTG_FRAME
			trace_rtg_frame_sched("LAUNCHER_PID", task->pid);
#endif
			pr_err("-%s- QQOS_DEBUG Try-set-Launcher-P-M  pid:%d qos:%d.\n",
					__func__, task->pid, qos);
			return;
		} else {
			p = find_task_by_vpid(task->tgid);
