...
#define PHASE_TK_ID_MULTI_FEATURE_REGRESSION	3
...
extern struct phase_think_class phase_tk_mfregression;
#endif

static inline int phase_tk_create_one_cpu(int cpu)
{
	return phase_perf_create(phase_tk->pevents, phase_tk->fevents, cpumask_of(cpu));
}

static inline void phase_tk_release_one_cpu(int cpu)
{
	phase_perf_release(cpumask_of(cpu));
}

static inline int phase_tk_create(void)
{
	int err;
	struct cpumask online_phase_cpumask;

	cpumask_and(&online_phase_cpumask, &sysctl_phase_cpumask, cpu_online_mask);
	err = phase_perf_create(phase_tk->pevents, phase_tk->fevents, &online_phase_cpumask);
	if (err)
		return err;
	...
	if (phase_tk->create) {
		err = phase_tk->create();
		if (err) {
			phase_perf_release(&online_phase_cpumask);
			return err;
		}
	}
...
bool phase_task_need_check_reverse(struct task_struct *p);
...
static inline bool phase_task_need_check_reverse(struct task_struct *p) { return false; };
