...
static int sched_rt_runtime_exceeded(struct rt_rq *rt_rq)
{
...
#ifdef CONFIG_HONOR_RT_THROTTLE
	u64 task_running;
	struct task_struct *curr = rq_of_rt_rq(rt_rq)->curr;
#endif
	...
	task_running = rq_clock(rq_of_rt_rq(rt_rq)) - curr->sched_info.last_arrival;

	if (rt_rq->rt_time > runtime || task_running > runtime) {
#else
	if (rt_rq->rt_time > runtime) {
		 ...
		 */
#ifdef CONFIG_HONOR_RT_THROTTLE
		if (likely(rt_b->rt_runtime) || task_running > runtime) {
#else
...
#include "./honor_rt_sched/rt_cas.c"
#endif
...
#include "./honor_rt_sched/rt_misfit.c"
#endif
