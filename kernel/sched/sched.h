...
{
#ifdef CONFIG_SCHED_SMT_EXPELLING
	bool core_force_idle = rq->core->core_smt_expeller || rq->core->core_forceidle_count;
#else
	bool core_force_idle = rq->core->core_forceidle_count;
#endif
...
extern struct static_key_false sched_fair_group_share_bypass;
#endif
...
#ifdef CONFIG_HW_USERSPACE_THROTTLE
	if ((int)(rq->nr_running) < 0) {
		pr_err("cpu-%d: rq->nr_running=%u, count=%u\n",
				cpu_of(rq), rq->nr_running, count);
		BUG_ON(1);
	}
#endif
...
static inline int uclamp_task_boost(struct task_struct *p);
...
{
	return (uclamp_eff_value(p, UCLAMP_MIN) > 0 || uclamp_task_boost(p) > 0);
}
...
{
	return capacity_orig_of(cpu) * freq / cpu_rq(cpu)->cluster->max_freq;
}
...
{
	return cpu_rq(cpu)->cluster->max_freq * util / capacity_orig_of(cpu);
}

static inline u64 sched_ktime_clock(void)
{
#ifdef CONFIG_SCHED_WALT
	return walt_ktime_clock();
#else
	return ktime_get_ns();
#endif
}
...
#ifdef CONFIG_HW_USERSPACE_THROTTLE
static inline bool task_userspace_throttled(struct task_struct *p)
{
	lockdep_assert_rq_held(task_rq(p));

	WARN_ON(p->throttled != (!list_empty(&p->se.group_node) && !task_on_rq_queued(p)));

	return p->throttled != 0;
}

void unthrottle_userspace_task(struct task_struct *se);
void init_task_userspace_throttle(struct task_struct *p);
#else
static inline bool task_userspace_throttled(struct task_struct *p) { return false; }
static inline void unthrottle_userspace_task(struct task_struct *se){ }
static inline void init_task_userspace_throttle(struct task_struct *p) { }
#endif
