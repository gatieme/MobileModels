...
int grp_margin(unsigned long util, struct related_thread_group *grp)
{
...
static struct sched_cluster *best_cluster(struct related_thread_group *grp)
{
	...
	unsigned long boosted_grp_util = util + (unsigned long)grp_margin(util, grp);
...
#ifndef CONFIG_HW_RTG_CONSTRAINT
	if (interval == 0)
		return -EINVAL;
#endif
...
static inline bool group_should_invalid_util(struct related_thread_group *grp,
					     u64 now)
{
...
static inline bool valid_normalized_util(struct related_thread_group *grp)
{
	...
	if (!is_ffrt_rtg(grp->id) && grp->nr_running == 0)
		return false;
...
static int alloc_group_cputime(struct related_thread_group *grp)
{
	...
	for_each_possible_cpu(i) {
		...
		(void)memset_s(cpu_time, sizeof(*cpu_time), 0,
			       sizeof(*cpu_time));
...
static int add_task_to_group(struct task_struct *p,
			     struct related_thread_group *grp)
{
	struct rq *rq = NULL;
	struct rq_flags flag;
...
static int __sched_set_group_id(struct task_struct *p, unsigned int group_id)
{
	...
	if (old_grp && group_id) {
		pr_err("switching group from %d to %d failed.\n",
			old_grp->id, group_id);
		rc = -EINVAL;
		goto done;
	}
...
void sched_update_rtg_tick(struct task_struct *p)
{
	...
	if (grp->rtg_class != NULL && grp->rtg_class->sched_update_rtg_tick != NULL)
