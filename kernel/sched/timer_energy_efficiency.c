...
u64 relative_slack_ns(ktime_t req_time, struct task_struct *tsk,
			u64 default_slack)
{
	struct task_group *tsk_grp = NULL;
	unsigned int pct;
	u64 slack, max_slack;
	...
	rcu_read_lock();
	tsk_grp = css_tg(task_css(tsk, cpu_cgrp_id));

	pct = tsk_grp->timer_slack_pct;

	/* Try not impacting user experience when the req_time is short. */
	if (ktime_to_ms_round(req_time) <= tsk_grp->timer_slack_exempt_ms)
		pct = 0;

	if (pct > 0 && test_shell(tsk, tsk_grp))
		pct = 0;
	...
	max_slack = tsk_grp->timer_slack_max_delay_ms * NSEC_PER_MSEC;
	slack = min(slack, max_slack);

	trace_perf(relative_slack_ns, req_time, tsk_grp->css.id, pct, slack);
	rcu_read_unlock();
...
{
	struct task_group *tg = css_tg(css);
	tg->timer_slack_pct = (unsigned int)value;
	return 0;
}
...
{
	struct task_group *tg = css_tg(css);
	return (u64)tg->timer_slack_max_delay_ms;
}
				...
				struct cftype *cft, u64 value)
{
	...
	tg->timer_slack_max_delay_ms = (unsigned int)value;
	return 0;
}
...
{
	struct task_group *tg = css_tg(css);
	return (u64)tg->timer_slack_exempt_ms;
}
...
{
	struct task_group *tg = css_tg(css);
	tg->timer_slack_exempt_ms = (unsigned int)value;
	return 0;
}
