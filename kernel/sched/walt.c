...
static void add_top_task(struct task_struct *p, struct rq *rq)
{
	...
	delta_load = add_top_task_load(rq, rq->curr_table, load, uclamp_top_task(p) > 0);
...
{
	bool is_top_task = uclamp_top_task(p) > 0;
				...
				int event, u64 wallclock, bool account_busy)
{
	...
	p->ravg.load_sum += scale_exec_time(window_start - mark_start, rq,
				max_runtime(p) - p->ravg.load_sum, TASK_DC);

	/* Push new sample(s) into task's load history */
	top_task_load_update_history(rq, p, p->ravg.load_sum, 1, event);
	if (nr_full_windows)
		top_task_load_update_history(rq, p,
			scale_exec_time(window_size, rq, max_runtime(p), TASK_DC),
...
out:
	...
	trace_perf(predl_get_busy, p, runtime, start, ret);
	return ret;
}
			  ...
			  struct task_struct *p, u32 runtime)
{
	...
	trace_perf(predl_adjust_runtime, p, util, cap);
	return runtime;
}
#endif
