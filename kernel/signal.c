	...
	if (sig_fatal(p, sig) &&
		...
		if (!sig_kernel_coredump(sig)) {
			...
			do {
				...
				sched_unthrottle_userspace_task(t);
#endif
			...
			enum pid_type type)
{
	...
	xreclaimer_cond_quick_reclaim(p, sig, (struct siginfo *)info, (ret != 0) || !fatal_signal_pending(p));

#ifdef CONFIG_DFX_DIE_CATCH
		cur_flags = current->signal->unexpected_die_catch_flags;
		if ((catch_flags & EXIT_CATCH_FORAPP_FLAG) && !(cur_flags & KILL_CATCH_OLD_FLAG)) {
			if (current->pid != 1 && (sig == SIGKILL || sig == SIGTERM) &&
				to_pid != current->pid)
		    ...
		    const struct timespec64 *ts)
{
	...
	if (!sig && timeout) {
		...
		ret = freezable_schedule_hrtimeout_range(to,
			relative_slack_ns(timeout, tsk, tsk->timer_slack_ns),
			HRTIMER_MODE_REL);
#else
