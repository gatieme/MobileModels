...
#endif
#if (defined CONFIG_HN_DIE_CATCH) && IS_ENABLED(CONFIG_HN_ZEROHUNG)
	unsigned short cur_flags = 0;
	struct kernel_siginfo new_sigino;
#endif
	trace_android_vh_do_send_sig_info(sig, current, p);
#ifdef CONFIG_KSTATE_COMMON
	if ((sig == SIGKILL) || (sig == SIGTERM) ||
		(sig == SIGABRT) || (sig == SIGQUIT)) {
		if (kernel_kill_cb != NULL) {
			kernel_kill_cb(p->tgid, sig);
		}
	}
#endif
...
#ifdef CONFIG_XRECLAIMER
	xreclaimer_cond_quick_reclaim(p, sig, info, (ret != 0));
#endif
#if (defined CONFIG_HN_DIE_CATCH) && IS_ENABLED(CONFIG_HN_ZEROHUNG)
