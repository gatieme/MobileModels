...
extern int sysctl_ion_heap_trace_buf_reuse;
...
#endif
#ifdef CONFIG_HW_GRADED_SCHED
static int sysctl_graded_sched = 1;
				...
				void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write && !error) {
		if (sysctl_graded_sched)
			static_branch_enable(&g_graded_sched_key);
		else
			static_branch_disable(&g_graded_sched_key);
	}
...
#ifdef CONFIG_HKIP_PROTECT_SYSCTL_TABLE
/**
		...
		void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	proc_first_pos_non_zero_ignore(ppos, table);
	buf = (char *)kzalloc(table->maxlen + 1, GFP_KERNEL);
	if (!buf)
		return -1;
...
static struct ctl_table kern_table[] = {
	...
	{
		.procname	= "sched_fair_group_share_bypass",
		.data		= NULL, /* filled in by handler */
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= sysctl_sched_fair_group_share_bypass_handler,
		.extra1		= SYSCTL_ZERO,
		.extra2		= SYSCTL_ONE,
	},
#endif
	...
	{
		.procname	= "futex_opt_spin_enable",
		.data		= &sysctl_futex_opt_spin_enable,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname	= "futex_opt_spin_timeout",
		.data		= &sysctl_futex_opt_spin_timeout,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
	...
	{
		.procname	= "sched_smt_expeller_util_ratio",
		.data		= &sysctl_sched_smt_expeller_util_ratio,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
	...
	{
		.procname	= "perflogd_ex_report",
		.data		= &sysctl_perflogd_ex_report,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= proc_douintvec,
	},
...
#ifdef CONFIG_MM_AMA
	{
		.procname	= "ama_work_scan_size",
		.data		= &sysctl_ama_migrate_scan_size,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0600,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname	= "ama_work_scan_period_delay",
		.data		= &sysctl_ama_migrate_scan_delay,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0600,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname	= "ama_kshrink_watermark",
		.data		= &sysctl_ama_shrink_watermark,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0600,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname	= "ama_kshrinkd_scan_delay",
		.data		= &sysctl_ama_kshrinkd_scan_delay,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0600,
		.proc_handler	= proc_dointvec,
	},
	...
	{
		.procname	= "sched_rotate_big_tasks",
		.data		= &sysctl_sched_rotate_big_tasks,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0640,
		.proc_handler	= proc_dointvec,
	},
	...
	{
		.procname       = "offline_last_compat_core_enabled",
		.data           = &sysctl_offline_last_compat_core_enabled,
		.maxlen         = sizeof(unsigned int),
		.mode           = 0640,
		.proc_handler   = proc_dointvec_minmax,
	},
...
/* CONFIG_DFX_BB_DEBUG opend only in eng software version */
#ifdef CONFIG_DFX_BB_DEBUG
	{
		.procname       = "watchdog",
...
static struct ctl_table vm_table[] = {
...
#ifdef CONFIG_ION_BUFFER_REUSE
	{
		.procname	= "ion_heap_trace_buf_reuse",
		.data		= &sysctl_ion_heap_trace_buf_reuse,
		.maxlen		= sizeof(sysctl_ion_heap_trace_buf_reuse),
		.mode		= 0644,
		.proc_handler	= proc_dointvec_minmax,
		.extra1		= SYSCTL_ZERO,
		.extra2		= SYSCTL_ONE,
	...
	{
		.procname	= "ion_heap_max_zero_orders_in_reuse",
		.data		= &sysctl_ion_heap_max_zero_orders_in_reuse,
		.maxlen		= sizeof(sysctl_ion_heap_max_zero_orders_in_reuse),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
	{
		.procname       = "ion_heap_buffer_min_order_in_reuse",
		.data           = &sysctl_ion_heap_buffer_min_order_in_reuse,
		.maxlen         = sizeof(sysctl_ion_heap_buffer_min_order_in_reuse),
		.mode           = 0644,
		.proc_handler   = proc_dointvec,
	},
#endif
	{
		.procname	= "ion_heap_trace_heap_drain",
		.data		= &sysctl_ion_heap_trace_heap_drain,
		.maxlen		= sizeof(sysctl_ion_heap_trace_heap_drain),
		.mode		= 0644,
		.proc_handler	= proc_dointvec_minmax,
		.extra1		= SYSCTL_ZERO,
		.extra2		= SYSCTL_ONE,
	...
	{
		.procname	= "ion_heap_trace_heap_shrink",
		.data		= &sysctl_ion_heap_trace_heap_shrink,
		.maxlen		= sizeof(sysctl_ion_heap_trace_heap_shrink),
		.mode		= 0644,
		.proc_handler	= proc_dointvec_minmax,
		.extra1		= SYSCTL_ZERO,
		.extra2		= SYSCTL_ONE,
   ...
   (defined(CONFIG_SUPERH) && defined(CONFIG_VSYSCALL))
...
#ifdef CONFIG_OVERWRITE_FAULT_AROUND_BYTES
	{
		...
		.data		= &sysctl_fault_around_bytes,
		.maxlen		= sizeof(unsigned long),
		.mode		= 0640,
		.proc_handler	= fault_around_bytes_handler,
	},
#endif
	{ }
};
...
#ifdef CONFIG_HW_DSS_BOOST
	{
		.procname		= "dss_boost_enabled",
		.data			= &g_dss_boost_enabled,
		.maxlen			= sizeof(int),
		.mode			= S_IRUSR | S_IWUSR,
		.proc_handler	= proc_dointvec_minmax,
		.extra1			= SYSCTL_ZERO,
