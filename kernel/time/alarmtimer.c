...
#ifdef CONFIG_FACTORY_STOPALARM
bool hw_alarm_stop;
...
#ifdef CONFIG_HONOR_DUBAI_COMMON
	alarm->timer.pid = current->pid;
	(void)memcpy_s(alarm->timer.comm, sizeof(alarm->timer.comm), current->comm, TASK_COMM_LEN);
#endif
...
static int alarmtimer_suspend(struct device *dev)
{
	...
	if (ret < 0)
		...
		if ((kernelAlarmtimerHook != NULL) && (rtc_alarm != NULL)) {
			kernelAlarmtimerHook(rtc_alarm->timer.comm, rtc_alarm->timer.pid, rtc_alarm->node.expires);
		}
	}
#endif
	return ret;
