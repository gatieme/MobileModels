...
int del_timer_sync(struct timer_list *timer)
{
	...
	do {
		...
		if (unlikely(ret < 0)) {
			 ...
			 * fix LiveLock
			 *
			 * try_to_del_timer_sync() will do spin_lock, and operate preempt_count in thread_info
			 * The other cpu runs resched_curr(), and exclusive-set TIF_NEED_RESCHED on flags in
			 * the same thread_info
			 *
			 * preempt count and flags is in the same cacheline, so exclusive-set TIF_NEED_RESCHED
			 * may always be detroyed by preempt_count operation, so livelock caused
			 *
			 * so delay 1us here to loose preempt_count operation, let exclusive-set succeed, solve
			 * livelock, and delay 1us here does very little impact on Performance
			 */
			udelay(1);
#endif
		}
			  ...
			  unsigned long baseclk)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_time_hook, (u64)fn, (u32)0);

	fn(timer);

	CALL_HCK_VH(hck_dfx_bbox_time_hook, (u64)fn, (u32)1);
...
bool check_pending_deferrable_timers(int cpu)
{
	if (cpu == tick_do_timer_cpu ||
		...
		if (time_after_eq(jiffies, timer_base_deferrable.next_expiry)
...
signed long __sched schedule_timeout(signed long timeout)
{
	...
	timer_setup_on_stack(&timer.timer, process_timeout,
		test_tsk_thread_flag(current, TIF_TIMER_DEFERRABLE)
		? TIMER_DEFERRABLE : 0);
#else
...
static inline void init_timer_deferrable_global(void)
{
	...
	timer_base_deferrable.next_expiry = timer_base_deferrable.clk + NEXT_TIMER_MAX_DELTA;
}
