...
static int rb_check_pages(struct ring_buffer_per_cpu *cpu_buffer)
{
	...
	BUG_ON(rb_list_head(rb_list_head(head->next)->prev) != head);
	BUG_ON(rb_list_head(rb_list_head(head->prev)->next) != head);

	for (tmp = rb_list_head(head->next); tmp != head; tmp = rb_list_head(tmp->next)) {
		BUG_ON(rb_list_head(rb_list_head(tmp->next)->prev) != tmp);
		BUG_ON(rb_list_head(rb_list_head(tmp->prev)->next) != tmp);
	}
