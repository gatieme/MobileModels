...
struct static_key global_trace_key = STATIC_KEY_INIT_FALSE;
EXPORT_SYMBOL_GPL(global_trace_key);
#endif
...
#define FAULTED_SIZE (sizeof(FAULTED_STR) - 1) /* '\0' is already accounted for */
	...
	marker_trim = (tr->trace_flags & TRACE_ITER_MARKERS_TRIM);
	...
	real_size = cnt;
	if (marker_trim)
		real_size = sizeof(entry->ip) < cnt ? cnt - sizeof(entry->ip) : 0;
	size = sizeof(*entry) + real_size + 2; /* add '\0' and possible '\n' */

	/* If less than "<faulted>", then make sure we can still add that */
	if (!marker_trim && cnt < FAULTED_SIZE)
	...
	entry = ring_buffer_event_data(event);
	entry->ip = _THIS_IP_;
	update_trace_marker_ip(entry->ip);
	real_buf = (char *)(&entry->ip);
	if (!marker_trim)
		real_buf = (char *)(&entry->buf);
	len = __copy_from_user_inatomic(real_buf, ubuf, cnt);
	if (len) {
		if (!marker_trim)
...
#define FAULT_SIZE_ID (FAULTED_SIZE + sizeof(int))
	...
	real_size = cnt;
	if (marker_trim)
		real_size = sizeof(entry->id) < cnt ? cnt - sizeof(entry->id) : 0;
	size = sizeof(*entry) + real_size;
	if (!marker_trim && cnt < FAULT_SIZE_ID)
