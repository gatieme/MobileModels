...
struct workqueue_struct *system_highpri_unbound_wq __read_mostly;
EXPORT_SYMBOL_GPL(system_highpri_unbound_wq);
...
#if defined CONFIG_SCHED_VIP
		set_vip_prio(worker->task, 1);
		mark_vip_lightweight(worker->task);
...
__acquires(&pool->lock)
{
	...
	uptime = ktime_get_ns();
#endif
	CALL_HCK_VH(hck_dfx_bbox_worker_hook, (u64)(worker->current_func), 0);

	worker->current_func(work);

	CALL_HCK_VH(hck_dfx_bbox_worker_hook, (u64)(worker->current_func), 1);
...
void __init workqueue_init_early(void)
{
	...
	system_highpri_unbound_wq = alloc_workqueue("events_highpri_unbound",
