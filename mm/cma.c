...
#ifdef CONFIG_ZONE_MEDIA
#define MAX_RSVD_CMA	(2)
struct rsvd_cma_name {
	const char *name;
};

static struct rsvd_cma_name rsvd_cma_table[MAX_RSVD_CMA] = {
	{.name = "fastboot-cma-mem"},
	{.name = "tiny_cma"},
};

unsigned long total_rscvd_cma_pages;
unsigned long cma_max_pfn;
unsigned long cma_min_pfn;
#endif

#ifdef CONFIG_DFX_MEMCHECK
static inline void track_cma_alloc(unsigned int count)
{
	atomic_long_add(count, &cma_used_count);
}

static inline void track_cma_release(unsigned int count)
{
...
static bool is_rsvd_cma_name(const char *name)
{
	int i;

	for (i = 0; i < MAX_RSVD_CMA; i++) {
		if (!strncmp(rsvd_cma_table[i].name,
			     name, strlen(name)))
			return true;
	}
...
static void set_cma_pfn(const char *name,
			phys_addr_t size,
			unsigned long base_pfn,
			unsigned long count)
{
	if (!is_rsvd_cma_name(name))
		return;

	if (!cma_min_pfn || cma_min_pfn > base_pfn)
		cma_min_pfn = base_pfn;

	/*
	 * DMA zone pfn is mananged as [start, end),
	 * cma reserved pfn is equivalent to [start, end].
	 * So, we must add 1 page for max_pfn
	 */
	if (cma_max_pfn < base_pfn + count)
		cma_max_pfn = base_pfn + count + 1;

	total_rscvd_cma_pages += (size / PAGE_SIZE);
}
#endif
...
static int __init cma_init_reserved_areas(void)
{
	int i;
#ifdef CONFIG_MM_CMA_DEBUG
	if (check_mntn_switch(MNTN_CMA_TRACE))
				 ...
				 struct cma **res_cma)
{
	...
	*res_cma = cma;
	...
	CALL_HCK_VH(hck_dfx_kdump_reserved_addr_save, base, size);

#ifdef CONFIG_ZONE_MEDIA
	set_cma_pfn(name, size, cma->base_pfn, cma->count);
#endif
...
#ifdef CONFIG_DFX_MEMCHECK
		trace_hck_cma_report((char *)cma->name, cma->count, count);
#endif
...
out:
	...
	CALL_HCK_VH(hck_dfx_kdump_cma_set_page_flag, page, count);
...
#ifdef CONFIG_DFX_MEMCHECK
	track_cma_release(count);
#endif
#ifdef CONFIG_MM_CMA_DEBUG
	record_cma_release_info(cma,  pfn, count);
#endif
