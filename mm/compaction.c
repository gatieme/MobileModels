...
#ifdef CONFIG_PROACTIVITY_COMPACT
 /*in COMPACTION_HPAGE_ORDER score*/
 #define TRIGGER_COMPACT_MIN_SCORE 97
 #endif
...
#ifdef CONFIG_HONOR_ZONE_ISOLATE
		struct free_area *area = &cc->zone->free_area[ZONE_ISOLATE_COUNT - zi_index -1][order];
#else
...
static unsigned long fast_find_migrateblock(struct compact_control *cc)
{
	...
	for (order = cc->order - 1;
	     ...
	     order--) {
#ifdef CONFIG_HONOR_ZONE_ISOLATE
		struct free_area *area = &cc->zone->free_area[zi_index][order];
#else
		struct free_area *area = &cc->zone->free_area[order];
#endif
		struct list_head *freelist;
...
static bool should_proactive_compact_node(pg_data_t *pgdat)
{
	int wmark_high;
#ifdef CONFIG_PROACTIVITY_COMPACT
 	if (!(sysctl_compaction_proactiveness && pgdat->proactive_compact_trigger) || kswapd_is_running(pgdat))
 #else
...
static int kcompactd(void *p)
{
	...
	while (!kthread_should_stop()) {
		 ...
		 */
#ifdef CONFIG_PROACTIVITY_COMPACT
 		timeout = sysctl_compaction_proactiveness ? 0 : MAX_SCHEDULE_TIMEOUT;
#else
		...
		if (should_proactive_compact_node(pgdat)) {
			...
			unsigned int prev_score;
 			pr_info("compaction_proactiveness start\n");
#else
			unsigned int prev_score, score;
#endif
			prev_score = fragmentation_score_node(pgdat);
#ifdef CONFIG_PROACTIVITY_COMPACT
 			if (prev_score >= TRIGGER_COMPACT_MIN_SCORE) {
		...
		if (unlikely(pgdat->proactive_compact_trigger)) {
			pgdat->proactive_compact_trigger = false;
 #ifdef CONFIG_PROACTIVITY_COMPACT
 			sysctl_compaction_proactiveness = 0;
 			pr_info("compaction_proactiveness finished\n");
 #endif
		}
	}
