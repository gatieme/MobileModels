...
#ifdef CONFIG_HW_STAT_TRACE_PAGEFAULT
#include <linux/hw_stat.h>
#endif
					...
					void **shadowp)
{
	...
	do {
		...
		if (!huge) {
			...
			__fault_stat_acct(mapping->host, FILE_ADD_PAGES, 1);
#endif
		}
unlock:
		xas_unlock_irq(&xas);
				...
				pgoff_t offset, gfp_t gfp_mask)
{
	...
	if (unlikely(ret))
		...
		if (!(gfp_mask & __GFP_WRITE) && shadow) {
			...
			ret = rejudge_protect_page(page, mapping, gfp_mask, shadow);
			if (unlikely(ret)) {
				inc_enhanced_lru_charge_fail();
		...
		if (get_page_attr(page, mapping) == PAGE_FILE_FG)
			SetPageActive(page);
#endif
		lru_cache_add(page);
	}
...
#else
	return ama_alloc_page_cache(gfp, 0);
#endif
}
...
static struct file *do_sync_mmap_readahead(struct vm_fault *vmf)
{
	 ...
	 */
	fpin = maybe_unlock_mmap_for_io(vmf, fpin);
#ifndef CONFIG_MM_PAGECACHE_HELPER
...
#else
	pch_read_around(&ractl, ra, file, vmf->pgoff);
#endif
	return fpin;
}
...
vm_fault_t filemap_fault(struct vm_fault *vmf)
{
	...
	bool started = false;
	u64 start_time = 0;
	int fault_type = MINOR_FAULT;
#endif
#ifdef CONFIG_SPECULATIVE_PAGE_FAULT
	unsigned long vm_flags;
#endif
#ifdef CONFIG_HW_STAT_TRACE_PAGEFAULT
	int hw_stat_tracing = 0;
#endif
	...
	if (likely(page) && !(vmf->flags & FAULT_FLAG_TRIED)) {
		 ...
		 */
#ifdef CONFIG_HW_STAT_FAULT
		started = filemap_fault_acct_start(&start_time);
		fault_type = MINOR_FAULT;
#endif
#ifdef CONFIG_CGROUP_IOLIMIT
...
#ifdef CONFIG_HW_STAT_TRACE_PAGEFAULT
		if (hw_stat_trace_enabled()) {
			char *path = NULL;
			char name_buf[HW_STAT_MAX_BUF_LEN];
			struct hw_stat_trace_param param = {
				.ignore_qos = 0,
				.note = "MajorFault(name,offset,flags,ino)",
				.note2 = NULL,
				.value1 = offset,
				.value2 = vmf->flags,
				.value3 = inode->i_ino,
			};
			hw_stat_trace_fill_name(&param, &path, inode, name_buf, HW_STAT_MAX_BUF_LEN);
			hw_stat_tracing = hw_stat_trace(HW_STAT_TRACE_START, &param);
		}
#endif
...
retry_find:
		...
		if (!page) {
#ifdef CONFIG_HW_STAT_TRACE_PAGEFAULT
			if (hw_stat_tracing && hw_stat_trace_enabled()) {
				struct hw_stat_trace_param param = {
					.ignore_qos = 0,
					.note = NULL,
					.note2 = NULL,
				...
				hw_stat_tracing = hw_stat_trace(HW_STAT_TRACE_END, &param);
			}
#endif
			if (fpin)
				goto out_retry;
...
#ifdef CONFIG_HW_STAT_FAULT
	if (started)
		filemap_fault_acct_done(inode, fault_type, start_time);
#endif
