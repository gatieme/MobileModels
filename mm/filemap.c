					...
					void **shadowp)
{
	...
	if (!huge) {
#ifdef CONFIG_MEMCG_PROTECT_LRU
		memcgp = get_protect_file_memcg(page, mapping);
		if (!mem_cgroup_disabled())
			error = __mem_cgroup_charge(page, NULL, gfp, &memcgp);
#else
...
#ifdef CONFIG_HONOR_IO_MEASURE
	io_measure_pgcache_stat_inc(IOM_CACHE_ADD);
#endif
	return 0;
error:
#ifdef CONFIG_MEMCG_PROTECT_LRU
				...
				pgoff_t offset, gfp_t gfp_mask)
{
	...
	if (unlikely(ret))
		...
		if (get_page_attr(page, mapping) == PAGE_FILE_VIP)
			SetPageActive(page);
#endif
		lru_cache_add(page);
	}
...
#ifdef CONFIG_HONOR_FILEMAP_TRACE
#define MAX_TRACE_PATHBUF_LEN 256
static void filemap_trace_pagecache_miss(struct file *file,
				pgoff_t start, pgoff_t len,
				pgoff_t index, bool buffer)
{
	char *filepath = NULL;
	char pathbuf[MAX_TRACE_PATHBUF_LEN] = {'\0'};

	if (trace_filemap_op_page_cache_miss_enabled()) {
		filepath = d_path(&file->f_path, pathbuf, MAX_TRACE_PATHBUF_LEN);
		if (!IS_ERR_OR_NULL(filepath))
			trace_filemap_op_page_cache_miss(file->f_mapping,
					filepath, start, len, index, buffer);
	}
}
#endif
...
#ifdef CONFIG_HONOR_IO_MEASURE
	io_measure_pgcache_stat_inc(IOM_CACHE_ACCESS);
#endif
	if (!pagevec_count(pvec)) {
#ifdef CONFIG_HONOR_IO_MEASURE
		io_measure_pgcache_stat_inc(IOM_CACHE_MISS);
#endif
#ifdef CONFIG_HONOR_FILEMAP_TRACE
		filemap_trace_pagecache_miss(filp, index,
		...
		ssize_t already_read)
{
...
#ifdef CONFIG_HONOR_PGPRESS
	readahead_adaptive(filp, NULL, true);
#endif
	...
	do {
		...
		if (error < 0)
			break;
#ifdef CONFIG_CGROUP_IOLIMIT
		io_read_bandwidth_control(PAGE_SIZE * pagevec_count(&pvec));
#endif
		/*
...
#ifdef CONFIG_HN_CGROUP_WORKINGSET
	workingset_pagecache_on_readfile(filp, &begin_pos, iocb->ki_pos >> PAGE_SHIFT, 0);
#endif
...
#ifdef CONFIG_HONOR_RA
static bool lowmem_check(void)
{
...
/*
 * Now erofs partition ra->ra_pages 32 pages, other is 128 pages
 *                 erofs    non-erofs
 * for exec file: 4 pages   16 pages
 * for data file: 8 pages   32 pages
 * lowmem case half of them.
 */

static unsigned int choose_proper_ra_pages(struct file *file, struct vm_fault *vmf)
{
	struct file_ra_state *ra = &file->f_ra;
	unsigned int ra_pages;

	if (vmf && vmf->vma && vmf->vma->vm_flags & VM_EXEC)
		ra_pages = ra->ra_pages / 8;
	else
		ra_pages = ra->ra_pages / 4;

	if (lowmem_check())
		ra_pages /= 2;

	return ra_pages ? ra_pages : 2;
}
#endif
...
static struct file *do_sync_mmap_readahead(struct vm_fault *vmf)
{
	...
	fpin = maybe_unlock_mmap_for_io(vmf, fpin);
#ifdef CONFIG_HONOR_RA
	ra_pages = choose_proper_ra_pages(file, vmf);
...
#ifdef CONFIG_HONOR_PGPRESS
	readahead_adaptive(file, vmf, false);
#endif
...
#endif
#ifdef CONFIG_HONOR_FILEMAP_TRACE
		filemap_trace_pagecache_miss(file, offset, 1, offset, false);
#endif
