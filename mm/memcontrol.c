...
static bool cgroup_memory_nosocket __ro_after_init = true;

/* Kernel memory accounting disabled? */
bool cgroup_memory_nokmem __ro_after_init = true;
#else
/* Socket memory accounting disabled? */
...
#define MEMCG_DEFAULT_NODE_ID 0
		struct mem_cgroup_per_node *mz = memcg->nodeinfo[MEMCG_DEFAULT_NODE_ID];
...
static struct mem_cgroup *mem_cgroup_alloc(void)
{
	...
	snprintf(memcg->alloc_stat.name, sizeof(memcg->alloc_stat.name),
		"memcg_%u", memcg->id.id);
#endif
...
#endif
#ifdef CONFIG_HYPERHOLD_CORE
	atomic64_set(&memcg->hyperhold_cold_writeback, 0);
#endif
#endif
