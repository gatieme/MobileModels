...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	 ...
	 */
	if (!pte_map_lock(vmf)) {
		ret = VM_FAULT_RETRY | VM_FAULT_SPF_RETRY;
...
vm_fault_t finish_fault(struct vm_fault *vmf)
{
	...
	if (!pte_map_lock(vmf))
		return VM_FAULT_RETRY | VM_FAULT_SPF_RETRY;
	ret = 0;
	/* Re-check under ptl */
...
static vm_fault_t do_read_fault(struct vm_fault *vmf)
{
	...
	if (vma->vm_ops->map_pages && fault_around_bytes >> PAGE_SHIFT > 1 &&
		likely(!task_wscg_monitor(current))) {
#else
	if (vma->vm_ops->map_pages && fault_around_bytes >> PAGE_SHIFT > 1) {
#endif
...
#ifdef CONFIG_PAGE_FAULT_STATS
static vm_fault_t do_page_fault_stat(vm_fault_t (*do_page_fault_func)(struct vm_fault *),
			struct vm_fault *vmf, struct alloc_time_stat *page_stat)
{
	s64 delta;
	vm_fault_t ret;
	ktime_t time_start = ktime_get();

	ret = do_page_fault_func(vmf);
	delta = ktime_to_us(ktime_sub(ktime_get(), time_start));
	count_alloc_time(page_stat, delta);
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (!vmf->pte) {
		if (vma_is_anonymous(vmf->vma))
			...
			return do_page_fault_stat(do_anonymous_page, vmf, &anonymous_page_stat);
#else
			return do_anonymous_page(vmf);
#endif
		else
#ifdef CONFIG_FILE_PAGE_STAT
			return do_page_fault_stat(do_fault, vmf, &file_page_stat);
#else
			return do_fault(vmf);
#endif
	}
	...
	if (!pte_present(vmf->orig_pte))
		...
		return do_page_fault_stat(do_swap_page, vmf, &swap_page_stat);
#else
	...
	if (vmf->flags & FAULT_FLAG_WRITE) {
		if (!pte_write(entry))
#ifdef CONFIG_SWAP_PAGE_STAT
			return do_page_fault_stat(do_wp_page, vmf, &swap_page_stat);
#else
