...
#ifdef CONFIG_HW_STAT_TRACE_MMAP
			...
			unsigned long *populate, struct list_head *uf)
{
	...
	*populate = 0;
	...
	if (hihugepaged_vm_flags_check(vm_flags, mm))
			      ...
			      unsigned long fd, unsigned long pgoff)
{
	...
	if (!(flags & MAP_ANONYMOUS)) {
		if (hw_stat_trace_enabled()) {
			struct hw_stat_trace_param param = {
				.ignore_qos = 0,
				.note = "MMAP(name,flags,len,pgoff)",
#ifdef CONFIG_HW_STAT_TRACE_FULLPATH
				.note2 = file->f_path.dentry->d_name.name,
#else
				.note2 = "",
#endif
				.value1 = flags,
			...
			hw_stat_trace(HW_STAT_TRACE_START, &param);
		}
	}
#endif
	...
	if (!(flags & MAP_ANONYMOUS)) {
		if (hw_stat_trace_enabled()) {
			...
			hw_stat_trace(HW_STAT_TRACE_END, &param);
		}
	}
#endif
out_fput:
