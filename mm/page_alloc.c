...
#if defined(CONFIG_ANDROID_LOW_MEMORY_KILLER_DAEMON) || defined(CONFIG_HISI_LOWMEM_DBG)
...
#ifdef CONFIG_H_MM_SLOW_PATH_COUNT
#include <linux/mm/slowpath_count.h>
#endif
 ...
 * default disable watermark_boost for all
 */
				    ...
				    unsigned int order, int migratetype)
{
	...
	area->nr_free++;
#ifdef CONFIG_MM_VIP
	add_vip_page_num(migratetype, order, page);
#endif
}
...
#ifdef CONFIG_MM_VIP
	change_vip_page_num(migratetype, order, page);
#endif
}
...
#ifdef CONFIG_MM_VIP
	change_migrate_type_vip(page, &migratetype);
#endif
			    ...
			    fpi_t fpi_flags)
{
	...
	migratetype = get_pfnblock_migratetype(page, pfn);
#ifdef CONFIG_MM_VIP
	add_mmvip_page_num(page, &migratetype, order);
#endif
...
inline void post_alloc_hook(struct page *page, unsigned int order,
				gfp_t gfp_flags)
{
	CALL_HCK_VH(hck_dfx_kdump_ion_set_page_flag, (gfp_t)(gfp_flags & __GFP_HIGHMEM), page);
	set_page_private(page, 0);
	set_page_refcounted(page);
...
#endif
#ifdef CONFIG_MM_VIP
	[MIGRATE_VIP]         = { MIGRATE_TYPES }, /* Never used */
#endif
#ifdef CONFIG_MEMORY_ISOLATION
						...
						bool force)
{
	...
	for_each_zone_zonelist_nodemask(zone, z, zonelist, ac->highest_zoneidx,
		...
		for (order = 0; order < MAX_ORDER; order++) {
			...
			ret = move_freepages_block(zone, page, migratetype,
									NULL);
			if (ret) {
				spin_unlock_irqrestore(&zone->lock, flags);
				return ret;
...
retry:
...
#ifdef CONFIG_MM_VIP
		migratetype = gfp_migratetype(gfp_flags & ~___GFP_CMA);
		goto retry;
#else
		migratetype = MIGRATE_MOVABLE;
		page = __rmqueue_smallest(zone, order, migratetype);
	...
	if (migratetype == MIGRATE_VIP) {
		count_vm_event(MM_VIP_BUDDY_ALLOC_NUM);
		if (unlikely(!page)) {
			count_vm_event(MM_VIP_BUDDY_ALLOC_FAIL_NUM);
			migratetype = gfp_migratetype((gfp_flags & ~___GFP_CMA) & ~___GFP_VIP);
			page = __rmqueue_smallest(zone, order, migratetype);
		}
	}
#endif
...
static int rmqueue_bulk(struct zone *zone, unsigned int order,
			...
			int migratetype, unsigned int alloc_flags, gfp_t gfp_flags)
{
	int i, alloced = 0;

	spin_lock(&zone->lock);
	for (i = 0; i < count; ++i) {
		struct page *page = __rmqueue(zone, order, migratetype, alloc_flags, gfp_flags);
			...
			int migratetype, unsigned int alloc_flags, gfp_t gfp_flags)
{
	...
	if (list_empty(list) && (migratetype == MIGRATE_CMA) &&
		(zone_page_state(zone, NR_FREE_CMA_PAGES) < pcp->batch)) {
		migratetype = MIGRATE_MOVABLE;
		list = &pcp->lists[migratetype];
	}

	if (list_empty(list)) {
		count += rmqueue_bulk(zone, order,
				pcp->batch, list,
				migratetype, alloc_flags, gfp_flags);
#ifdef CONFIG_MM_VIP
		add_pcppages_count(pcp, migratetype, count);
#else
		pcp->count += count;
#endif
...
static bool free_unref_page_prepare(struct page *page, unsigned long pfn)
{
	...
	migratetype = get_pfnblock_migratetype(page, pfn);
#ifdef CONFIG_MM_VIP
	set_migrate_vip(page, &migratetype);
#endif
	set_pcppage_migratetype(page, migratetype);
	return true;
}
...
static void free_unref_page_commit(struct page *page, unsigned long pfn)
{
	 ...
	 */
#ifdef CONFIG_MM_VIP
	if (migratetype >= MIGRATE_PCPTYPES && migratetype != MIGRATE_VIP) {
#else
	if (migratetype >= MIGRATE_PCPTYPES) {
#endif
...
#ifdef CONFIG_MM_VIP
	free_pcppages_vip(page, zone, pcp, migratetype);
#else
	...
	do {
		...
		if (migratetype == MIGRATE_MOVABLE &&
			...
			list = get_populated_pcp_list(zone, 0, pcp,
					get_cma_migrate_type(), alloc_flags, gfp_flags);
		}
		...
		list_del(&page->lru);
#ifdef CONFIG_MM_VIP
		sub_pcppages_count(pcp, migratetype);
#else
		pcp->count--;
#endif
...
retry:
	page = __rmqueue_pcplist(zone,  migratetype, alloc_flags, pcp,
				 gfp_flags);
#ifdef CONFIG_MM_VIP
	if (!page && migratetype == MIGRATE_VIP) {
		migratetype = gfp_migratetype(gfp_flags & ~___GFP_VIP);
		goto retry;
	}
#endif
			...
			int migratetype)
{
	...
	do {
		...
		if (!page)
			page = __rmqueue(zone, order, migratetype, alloc_flags, gfp_flags);
...
#ifdef CONFIG_MM_VIP
		if ((alloc_flags & ALLOC_VIP) &&
		    !free_area_empty(area, MIGRATE_VIP))
			return true;
#endif
					...
					unsigned int alloc_flags)
{
	...
	if (!(pflags & PF_MEMALLOC_NOCMA) && gfp_mask & __GFP_CMA)
		alloc_flags |= ALLOC_CMA;
...
#ifdef CONFIG_MM_VIP
	if (gfp_mask & ___GFP_VIP)
		alloc_flags |= ALLOC_VIP;
#endif
...
#define RECLAIM_FILECACHE_RETRY_THRESHOLD 100000
static bool should_reclaim_filecache_retry(int order, int *retry_num)
{
	unsigned long file_pages;

	/*
	 * Retry allocation 5 times at most for page with order 2
	 * The larger retry_num, the smaller oom-kill times,
	 * but it's not a good idea to set retry_num too large
	 */
	if (order != 2 || *retry_num >= 5)
		return false;
	...
	if (file_pages < RECLAIM_FILECACHE_RETRY_THRESHOLD)
		return false;

	pr_info("retry %d, file pages %lu\n", *retry_num, file_pages);
...
retry:
	...
	if (should_reclaim_filecache_retry(order, &retry_num))
		goto retry;
...
#ifdef CONFIG_IMPROPER_ALLOC
	improper_alloc_hook(gfp_mask, order, 0);
#endif
...
out:
	...
	CALL_HCK_VH(hck_dfx_bbox_page_trace_hook,
		gfp_mask, (u8)1, (u64)_RET_IP_, page, order);
...
void __free_pages(struct page *page, unsigned int order)
{
	...
	if (put_page_testzero(page)) {
		CALL_HCK_VH(hck_dfx_bbox_page_trace_hook, __GFP_HIGHMEM,
					(u8)0, (u64)_RET_IP_, page, order);
...
#ifdef CONFIG_ZONE_MEDIA
unsigned long calc_media_zone_cmafree_node(int nid)
{
...
unsigned long calc_media_zone_cmafree(void)
{
	int n;
	unsigned long val = 0;

	for_each_node_state(n, N_MEMORY) {
		val += calc_media_zone_cmafree_node(n);
	}
...
long si_mem_available(void)
{
	...
	/*
	 * Add the ioncache and mali pool pages
	 */
...
#endif
#ifdef CONFIG_ZONE_MEDIA
	available -= (long)calc_media_zone_cmafree();
#endif
	if (available < 0)
		available = 0;
	return available;
...
{
#if defined(CONFIG_ZONE_MEDIA) && defined(CONFIG_MM_AMA)
	unsigned long free_zone_media = calc_media_zone_cmafree();
#endif
...
#if defined(CONFIG_ZONE_MEDIA) && defined(CONFIG_MM_AMA)
	val->freeram = val->freeram >= free_zone_media ?
			(val->freeram - free_zone_media) : 0;
#endif
...
void show_free_areas(unsigned int filter, nodemask_t *nodemask)
{
	...
	printk("active_anon:%lu inactive_anon:%lu isolated_anon:%lu\n"
		...
		" mapped:%lu shmem:%lu pagetables:%lu bounce:%lu zspages:%lu\n"
#else
...
#ifdef CONFIG_MM_VIP
	pcp->vip_count = 0;
	INIT_LIST_HEAD(&pcp->lists[MIGRATE_VIP]);
#endif
}
...
#ifdef CONFIG_ZONE_MEDIA
	if (zone_type == ZONE_MEDIA)
		nr_absent = cma_max_pfn - cma_min_pfn - total_rscvd_cma_pages;
#endif
	return nr_absent;
}
...
#if defined(CONFIG_MM_AMA) && defined(CONFIG_ZONE_MEDIA)
		if (!is_node_part0(pgdat->node_id) && (i == ZONE_MEDIA))
			continue;
#endif
...
#ifdef CONFIG_ZONE_MEDIA
		if (zone == ZONE_MEDIA) {
			arch_zone_lowest_possible_pfn[zone] = cma_min_pfn;
			arch_zone_highest_possible_pfn[zone] = cma_max_pfn;
		}
#endif
		start_pfn = end_pfn;
	}
...
#ifdef CONFIG_MM_VIP
void __free_vip_one_page(struct page *page, unsigned long pfn, struct zone *zone,
			 unsigned int order, int migratetype, fpi_t fpi_flags)
{
	__free_one_page(page, pfn, zone, order, migratetype, fpi_flags);
}

bool vip_bulkfree_pcp_prepare(struct page *page)
{
	return bulkfree_pcp_prepare(page);
}

void vip_prefetch_buddy(struct page *page)
{
	prefetch_buddy(page);
}
#endif
			...
			struct page **page_array)
{
	...
	while (nr_populated < nr_pages) {
		...
		page = __rmqueue_pcplist(zone, ac.migratetype, alloc_flags, pcp, gfp);
		if (unlikely(!page)) {
...
failed:
	page = __alloc_pages_nodemask(gfp, 0, preferred_nid, nodemask);
	if (page) {
		if (page_list)
			list_add(&page->lru, page_list);
		else
