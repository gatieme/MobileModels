...
{
	return (value ^ (uintptr_t)canary) & CANARY_MASK;
}
			...
			enum track_item alloc, unsigned long addr)
{
	...
	if (addr) {
		...
		p->pid = current->pid;
		p->when = jiffies;
#ifdef CONFIG_SLUB_DEBUG_ENHANCED
		p->tgid = current->tgid;
		if (memcpy_s(p->comm, TASK_COMM_LEN, current->comm,
				TASK_COMM_LEN) != EOK)
			return;
#endif
	} else {
...
void *kmem_cache_alloc_trace(struct kmem_cache *s, gfp_t gfpflags, size_t size)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_kmalloc_trace_hook, (u8)1,
		(u64)_RET_IP_, (u64)ret, (u64)virt_to_phys(ret), (u32)size);
...
#ifdef CONFIG_HW_SLUB_SANITIZE
		if (unlikely(get_freepointer(s, tail) != prior))
			return;
#endif
		new.counters = counters;
...
#ifdef CONFIG_HW_SLUB_SANITIZE
	if (unlikely(get_freepointer(s, tail_obj) != freelist))
		return;
#endif
		if (unlikely(!this_cpu_cmpxchg_double(
				s->cpu_slab->freelist, s->cpu_slab->tid,
...
static void *kmalloc_large_node(size_t size, gfp_t flags, int node)
{
	...
	flags |= __GFP_COMP;
#ifdef CONFIG_MM_AMA
	page = ama_alloc_kernel_large(flags, order);
#else
...
void kfree(const void *x)
{
	...
	if (unlikely(!PageSlab(page))) {
		...
		BUG_ON(!PageCompound(page));
		kfree_hook(object);
		CALL_HCK_VH(hck_dfx_bbox_kmalloc_trace_hook, (u8)0,
			(u64)_RET_IP_, (u64)x, (u64)virt_to_phys(x), (u32)(PAGE_SIZE << compound_order(page)));
#ifdef CONFIG_H_MM_PAGE_TRACE
...
void *__kmalloc_track_caller(size_t size, gfp_t gfpflags, unsigned long caller)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_kmalloc_trace_hook, (u8)1,
		(u64)caller, (u64)ret, (u64)virt_to_phys(ret), (u32)(size));
	return ret;
}
...
struct loc_track {
	...
	struct location *loc;
#ifdef CONFIG_SLUB_DEBUG_ENHANCED
	int (*get_loc_map)(struct loc_track *t, struct kmem_cache *s,
...
#ifdef CONFIG_SLUB_DEBUG_ENHANCED
#include "slub_debug_ext/slub_usage.h"
#endif
		...
		struct page *page, enum track_item alloc)
{
	...
	for_each_object(p, s, addr, page->objects)
		if (!test_bit(__obj_to_index(s, addr, p), map))
#ifdef CONFIG_SLUB_DEBUG_ENHANCED
			t->get_loc_map(t, s, get_track(s, p, alloc),
...
#ifdef CONFIG_SLUB_DEBUG_ENHANCED
	debugfs_create_file("usage_traces", 0400,
		slab_cache_dir, s, &slab_debugfs_fops);
#endif
}
...
static void set_harden_double_free_check_flags(bool status)
{
	...
	if (status) {
		...
		list_for_each_entry(s, &slab_caches, list) {
			...
			if (s->hw_random_malloc == 0)
				s->hw_random_malloc = get_random_long();
			if (s->hw_random_free == 0)
				s->hw_random_free = get_random_long();
		}
#ifdef CONFIG_SLUB_DEBUG
		slub_debug = (unsigned int)slub_debug | SLAB_DOUBLEFREE_CHECK;
#endif
	} else {
		list_for_each_entry(s, &slab_caches, list)
			s->flags = s->flags & ~SLAB_DOUBLEFREE_CHECK;
...
{
#ifdef CONFIG_SLUB_DEBUG
	if (!status && !((unsigned int)slub_debug & SLAB_DOUBLEFREE_CHECK)) {
