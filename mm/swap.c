...
enum page_attr get_page_attr(struct page *page, struct address_space *mapping)
{
	...
	if (page && mapping && page_is_file_lru(page) && mapping->host) {
		...
		is_find_top_uids = find_top_uids(gid);
		if (mapping->host->i_uid.val == UID_SYSTEM)
			pattr = PAGE_FILE_VIP;
		else if (gid <= UID_MIN_VALUE)
			pattr = PAGE_FILE_SYSTEM;
		else if (is_find_top_uids)
			pattr = PAGE_FILE_FG;
		else
			pattr = PAGE_FILE_BG;
	}
