...
static void swap_range_free(struct swap_info_struct *si, unsigned long offset,
			    unsigned int nr_entries, bool ufs_valid)
#else
static void swap_range_free(struct swap_info_struct *si, unsigned long offset,
			    unsigned int nr_entries)
#endif
{
	...
	si->inuse_pages -= nr_entries;
#ifdef CONFIG_PARA_SWAPD
	if ((si->flags & SWP_BLKDEV) && !ufs_valid)
#else
...
#ifdef CONFIG_PARA_SWAPD
static void swap_free_cluster(struct swap_info_struct *si, unsigned long idx, bool ufs_valid)
#else
...
#endif
{
	...
	unlock_cluster(ci);
#ifdef CONFIG_PARA_SWAPD
	swap_range_free(si, offset, SWAPFILE_CLUSTER, ufs_valid);
#else
...
#ifdef CONFIG_HYPERHOLD
#ifdef CONFIG_PARA_SWAPD
	swap_range_free(p, offset, 1, swp_ufs_valid(entry));
#else
...
#ifdef CONFIG_HYPERHOLD
#ifdef CONFIG_PARA_SWAPD
			swap_free_cluster(si, idx, swp_ufs_valid(entry));
#else
