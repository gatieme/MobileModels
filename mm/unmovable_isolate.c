/*
 * Copyright (c) Honor Device Co., Ltd. 2017. All rights reserved.
 *
...
{
	/* lint -e115 */
	if (zone != NULL && zone_idx(zone) != ZONE_MOVABLE)
		return 1;
	else
		return 0;
}
...
unsigned long get_total_managed_pages(void)
{
	...
	for_each_zone(zone) {
		if (!is_highmem(zone))
			...
			total_managed_pages += (unsigned long)atomic_long_read(&zone->managed_pages);
#else
...
#if (KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE)
		ui_counts = ui_blocks * (unsigned long)atomic_long_read(&zone->managed_pages) / total_managed_pages;
#else
