				    ...
				    struct shrinker *shrinker, int priority)
{
	...
	while (total_scan >= batch_size ||
		...
		if (ret == SHRINK_STOP)
			break;
		freed += ret;
#ifdef CONFIG_SHRINKER_OPT
		if (freed > freeable) {
			// force stop when freeable is arrived.
			break;
		}
#endif
				 ...
				 int priority)
{
	 ...
	 * oom.
	 */
#if defined(CONFIG_HYPERHOLD_FILE_LRU) || defined(CONFIG_HONOR_SCM_BUILDIN)
	if (!mem_cgroup_disabled() && !mem_cgroup_is_root(memcg) && memcg)
#else
...
retry:
	while (!list_empty(page_list)) {
		...
		if (!dirty && !writeback && !PageMorechance(page) &&
			get_page_attr(page, mapping) == PAGE_FILE_VIP) {
...
#ifdef CONFIG_MEMCG_PROTECT_LRU
		if (!PageHuge(page) && page_is_file_lru(page) && !PageDirty(page) &&
...
#ifdef CONFIG_ISOLATED_DETECT
	isolated_detect(file, inactive, isolated);
#endif
			       ...
			       enum lru_list lru)
{
	...
	while (!list_empty(&l_hold)) {
		...
		if (pattr == PAGE_FILE_VIP && !PageMorechance(page)) {
			SetPageMorechance(page);
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	...
	struct blk_plug plug;
	bool scan_adjusted;
#ifdef CONFIG_VMSCAN_TRACE_OPT
	struct cgroup_subsys_state *css = &lruvec_memcg(lruvec)->css;
	...
	trace_mm_shrink_lruvec(memcg_name, sizeof(memcg_name), nr_reclaimed, sc->priority);
#endif
...
#ifdef CONFIG_HYPERHOLD
		if ((atomic64_read(&memcg->memcg_reclaimed.app_score) == 0) && (sc->priority > 10))
			continue;
#endif
...
#ifdef CONFIG_VMSCAN_TRACE_OPT
	trace_mm_shrink_node(nr_reclaimed, nr_scanned, true);
#endif
...
#ifdef CONFIG_VMSCAN_TRACE_OPT
	trace_mm_shrink_node(sc->nr_reclaimed - nr_reclaimed, sc->nr_scanned - nr_scanned, false);
#endif
...
static void snapshot_refaults(struct mem_cgroup *target_memcg, pg_data_t *pgdat)
{
	...
	lruvec->refaults[0] = lruvec_page_state(lruvec, WORKINGSET_ACTIVATE_ANON);
	lruvec->refaults[1] = lruvec_page_state(lruvec, WORKINGSET_ACTIVATE_FILE);
#else
	struct lruvec *target_lruvec;
	unsigned long refaults;
...
#ifdef CONFIG_HYPERHOLD
#define SCAN_CONTROL_NO_EXPORT 1
#include "memcg_reclaim.c"
#endif
