...
{
	register_split_shrinker(shrinker);
}
#else
void register_shrinker_prepared(struct shrinker *shrinker)
{
...
void unregister_shrinker(struct shrinker *shrinker)
{
	...
	if (shrinker->flags & SHRINKER_MEMCG_AWARE)
		unregister_memcg_shrinker(shrinker);
#ifdef CONFIG_SPLIT_SHRINKER
	unregister_split_shrinker(shrinker);
#else
	down_write(&shrinker_rwsem);
...
#endif
{
...
#ifdef CONFIG_SPLIT_SHRINKER
	if (!down_read_trylock(p_shrinker_rwsem))
#else
	if (!down_read_trylock(&shrinker_rwsem))
#endif
		goto out;

	trace_shrinkslabs_start(freed);
	...
	list_for_each_entry(shrinker, &shrinker_list, list) {
		...
		if (ret != 0)
			atomic_long_add(ret, &shrinker->freed);
#endif
		 ...
		 */
#ifdef CONFIG_SPLIT_SHRINKER
		if (rwsem_is_contended(p_shrinker_rwsem)) {
#else
		if (rwsem_is_contended(&shrinker_rwsem)) {
#endif
			freed = freed ? : 1;
	...
	trace_shrinkslabs_end(freed);
	if (current->reclaim_state)
		trace_shrinkslabs_pages(current->reclaim_state->reclaimed_slab);
...
#ifdef CONFIG_STOP_PAGE_REF
	referenced_ptes = page_referenced_spr(page, 1, sc->target_mem_cgroup,
					      &vm_flags, INACTIVE_REF);
#else
				     ...
				     bool ignore_references)
{
	...
	while (!list_empty(page_list)) {
		...
		if (!dirty && !writeback && get_page_attr(page, mapping) == PAGE_FILE_FG) {
			if (!PageMorechance(page)) {
				SetPageMorechance(page);
				count_vm_event(SKIP_RECLAIM_FILE);
				goto keep_locked;
			} else {
				count_vm_event(BACK_RECLAIM_FILE);
			}
		}
#endif
...
#ifdef CONFIG_VIP_PAGE_CACHE_BUILDIN
			mapping = page_mapping(page);
			if (get_page_attr(page, mapping) == PAGE_FILE_BG)
				goto keep_locked;
#endif
...
#ifdef CONFIG_VIP_PAGE_CACHE_BUILDIN
		mapping = page_mapping(page);
		pattr = get_page_attr(page, mapping);
		if (pattr == PAGE_FILE_FG) {
			if (!PageMorechance(page)) {
				SetPageMorechance(page);
...
bool inactive_is_low(struct lruvec *lruvec, enum lru_list inactive_lru)
{
	...
	unsigned long gb;
	const unsigned long percent_constant = 100;
#ifdef CONFIG_HYPERHOLD
	unsigned long set_ratio = get_file_active_inactive_ratio();
#endif
	...
	if (gb)
		inactive_ratio = int_sqrt(10 * gb) * percent_constant;
	else
		inactive_ratio = percent_constant;

#ifdef CONFIG_HYPERHOLD
	if (is_file_lru(inactive_lru) && set_ratio)
		inactive_ratio = set_ratio;
#endif

	trace_android_vh_tune_inactive_ratio(&inactive_ratio, is_file_lru(inactive_lru));

	return (inactive * inactive_ratio / percent_constant < active);
}
...
#ifdef CONFIG_TASK_PROTECT_LRU
	shrink_protect_lru_by_overratio(lruvec->pgdat);
#endif
}
...
static bool allow_direct_reclaim(pg_data_t *pgdat)
{
...
#ifdef CONFIG_MM_AMA
	if (is_node_part0(pgdat->node_id))
		wmark_ok = free_pages > pfmemalloc_reserve / 2;
	else
		wmark_ok = free_pages > pfmemalloc_reserve / 10;

	/* kswapd must be awake if processes are being throttled */
	if (is_node_part0(pgdat->node_id) &&
		!wmark_ok && waitqueue_active(&pgdat->kswapd_wait)) {
	...
	if (is_node_part1(pgdat->node_id) &&
		!wmark_ok && wq_has_sleeper(&pgdat->kshrinkd_wait)){
...
#ifdef CONFIG_HW_ALLOC_ACCT
	end_time = ktime_get();
	__count_vm_events(PGSCAN_DIRECT_THROTTLE_TIME, ktime_ms_delta(end_time, start_time));
#endif
...
#ifdef CONFIG_HW_VIP_PAGE_CACHE
static bool is_foreground_reclaim_scene(void)
{
#if defined(CONFIG_BLK_CGROUP) && defined(CONFIG_BLK_CGROUP_IOSMART)
...
#ifdef CONFIG_HW_VIP_PAGE_CACHE
	if (!is_foreground_reclaim_scene()) {
		/* for bg direct reclaim enlarge the nr_to_reclaim */
		sc.nr_to_reclaim = sc.nr_to_reclaim * 2;
	}
#endif
...
restart:
	...
	do {
		 ...
		 */
		trace_kswapd_start(sc.nr_reclaimed);

		if (kswapd_shrink_node(pgdat, &sc))
			raise_priority = false;

		trace_kswapd_end(sc.nr_reclaimed);
...
static int kswapd(void *p)
{
	...
	parak_set_kswapd_pid(tsk->pid);
#endif

#ifdef CONFIG_RAMTURBO
	if (kswapd_threads == 1)
		set_kswapd_pid(tsk->pid);
#endif
...
void kswapd_stop(int nid)
{
	...
	if (kswapd) {
		...
		NODE_DATA(nid)->kswapd = NULL;
#ifdef CONFIG_RAMTURBO
		/* Set kswapd_pid to -1 when kswapd stops */
		set_kswapd_pid(-1);
#endif
	}
}
 ...
 *
 */
bool page_evictable(struct page *page)
{
	bool ret;
...
int try_to_free_pages_ex(int nr_pages, int mode)
{
	...
	struct scan_control sc = {
		.gfp_mask = mask,
		.reclaim_idx = MAX_NR_ZONES - 1,
		.may_writepage = (mode == RCC_MODE_ANON) ? 1 : 0,
	...
	if (!cgroup_reclaim(&sc) && raw_cpu_read(vm_event_states.event[item]))
...
#ifdef CONFIG_MM_AMA
void ama_shrink_node(pg_data_t *pgdat, struct scan_control *sc)
{
	shrink_node(pgdat, sc);
}
#endif
