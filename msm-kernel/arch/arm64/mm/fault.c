			     ...
			     unsigned int esr, struct pt_regs *regs)
{
	...
	trace_android_rb_sreason_set("unknown_addr");
			      ...
			      struct pt_regs *regs)
{
	...
	if (is_el1_permission_fault(addr, esr, regs)) {
		if (esr & ESR_ELx_WNR) {
			trace_android_rb_sreason_set("rw_only");
			msg = "write to read-only memory";
		} else if (is_el1_instruction_abort(esr)) {
			trace_android_rb_sreason_set("non-executable");
			msg = "execute from non-executable memory";
		} else {
			trace_android_rb_sreason_set("read_unreadable");
			msg = "read from unreadable memory";
		}
	} else if (addr < PAGE_SIZE) {
		trace_android_rb_sreason_set("null_pointer");
		msg = "NULL pointer dereference";
	} else {
		if (kfence_handle_page_fault(addr, esr & ESR_ELx_WNR, regs))
			return;
		trace_android_rb_sreason_set("page_request");
		msg = "paging request";
	}
...
static int do_sea(unsigned long far, unsigned int esr, struct pt_regs *regs)
{
	...
	inf = esr_to_fault_info(esr);
	trace_android_rb_sreason_set("SEA");
...
void do_mem_abort(unsigned long far, unsigned int esr, struct pt_regs *regs)
{
	...
	trace_android_rb_sreason_set("unhandle_fault");
	err = snprintf(attach_info_buffer, RB_SREASON_STR_MAX, "Unhandled_fault_%s", inf->name);
	if (err >= 0)
		trace_android_rb_attach_info_set(attach_info_buffer);
...
void do_sp_pc_abort(unsigned long addr, unsigned int esr, struct pt_regs *regs)
{
	...
	trace_android_rb_sreason_set("SP_PC_AE");
