...
EXPORT_PER_CPU_SYMBOL_GPL(max_freq_scale);

#ifdef CONFIG_HW_SCALE_FREQ_CAPACITY
void topology_set_max_freq_scale(struct cpumask *cpus,
				...
				unsigned long max_freq)
{
	...
	if (WARN_ON_ONCE(!policy_max_freq || !max_freq))
		return;

	scale = (policy_max_freq << SCHED_CAPACITY_SHIFT) / max_freq;

	for_each_cpu(i, cpus)
		per_cpu(max_freq_scale, i) = scale;
}
#endif
