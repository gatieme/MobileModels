...
static struct regmap_qti_debugfs *spmi_map;

static DEFINE_MUTEX(regmap_qti_debugfs_lock);
static LIST_HEAD(regmap_qti_debugfs_list);

int grab_spmi_map(struct regmap_qti_debugfs *map)
{
	if (map == NULL)
		return -EINVAL;
	spmi_map = map;
...
static int dump_prepare(char index)
{
	...
	if (index == '1') {
		...
		value = 0x42;
		ret = regmap_write(spmi_map->regmap, spmi_map->dump_address, value);
...
static const struct proc_ops dump_file_fops = {
	.proc_open = dump_file_open,
	.proc_write = dump_file_write,
	.proc_read = seq_read,
...
int devm_regmap_qti_debugfs_register(struct device *dev, struct regmap *regmap)
{
	...
	*ptr = debug_map;
	...
	if (name) {
		if (!regmap->debugfs_name)
			name = regmap->debugfs_name;
	} else {
		name = dev_name(regmap->dev);
	}

	if (!strcmp(name, "0-00")) {
		if (grab_spmi_map(debug_map))
			pr_err("dump file map grab err", name);
		else
