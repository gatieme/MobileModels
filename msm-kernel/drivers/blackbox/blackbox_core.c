...
static struct error_info_to_category error_info_categories[] = {
	...
	#include <platform/linux/blackbox_expand_event.h>
#endif
};

struct error_info *temp_error_info;
static struct error_info *info;
...
#ifdef CONFIG_LOG_EXCEPTION
	hiview_send_hisysevent("KERNEL_VENDOR", "PANIC", FAULT, "%s", info->error_desc);
#endif
...
{
	if (unlikely(!event || !module || !error_desc || !info)) {
		bbox_print_err("event: %p, module: %p, error_desc: %p, info: %p\n", event,
				module, error_desc, info);
		return -EINVAL;
	}
