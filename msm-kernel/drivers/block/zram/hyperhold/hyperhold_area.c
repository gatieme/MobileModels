...
static int extent_bit2id(struct hyperhold_area *area, int bit)
{
	...
	if (hyperhold_is_file_space())
		return (int)hyperhold_file_bit2id(bit);
...
int mcg_hot_idx(struct hyperhold_area *area, int idx)
{
	if (!area) {
		hh_print(HHLOG_ERR, "NULL area\n");
		return -EINVAL;
	...
	return idx + area->nr_objs + area->nr_exts + area->nr_mcgs;
}
...
static struct hh_list_head *get_obj_table_node(int idx, void *private)
{
	...
	if (idx < area->nr_mcgs) {
		...
		if (!mcg)
			goto err_out;
		return (struct hh_list_head *)(&mcg->zram_hot_lru);
	}
err_out:
	hh_print(HHLOG_ERR, "idx = %d invalid\n", idx);
...
void first_alloc_bit_init(struct hyperhold_area *area)
{
	struct space_info_para *space_info = hyperhold_space_info();

	if (hyperhold_is_file_space() && space_info->enable_par_file)
		atomic_set(&area->last_alloc_bit,
			space_info->par_file_num * space_info->nr_exts_per_file);
	return;
}
...
int get_memcg_zram_entry(struct hyperhold_area *area, struct mem_cgroup *mcg)
{
	...
	hh_lock_list(mcg_hot_idx(area, mcg_id), area->obj_table);
	if (mcg != get_mem_cgroup(mcg_id)) {
		index = ((struct hh_list_head*)(&mcg->zram_hot_lru))->next;
		hh_print(HHLOG_ERR, "memcg corrupted, get index %d directly.\n", index);
		if (index >= area->nr_objs)
			index = -ENOENT;
		hh_unlock_list(mcg_hot_idx(area, mcg_id), area->obj_table);
		return index;
	}
	hh_list_for_each_entry(idx, mcg_hot_idx(area, mcg_id), area->obj_table) { /*lint !e666*/
		index = idx;
		break;
	}
	hh_unlock_list(mcg_hot_idx(area, mcg_id), area->obj_table);
