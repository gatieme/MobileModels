				 ...
				 int *index, int max_cnt)
{
	...
	hh_list_for_each_entry_reverse_safe(i, tmp,
		mcg_idx(area, mcg->id.id), area->obj_table) { /*lint !e666*/
		if (i >= (u32)area->nr_objs) {
			hh_print(HHLOG_ERR, "index = %d invalid\n", i);
			continue;
		}
		if (acc_swapin_enable() && atomic_read(&mcg->active_swapout_type) == FG_SWAPOUT_TYPE_RATIO &&
			!zram_test_flag(zram, i, ZRAM_FROM_HYPERHOLD))
			break;
		index[cnt++] = i;
		if (cnt >= max_cnt) {
	...
	hh_unlock_list(mcg_idx(area, mcg->id.id), area->obj_table);
	hh_print(HHLOG_DEBUG, "part cnt %d ", cnt);

	if (acc_swapin_enable() && atomic_read(&mcg->active_swapout_type) == FG_SWAPOUT_TYPE_RATIO)
		goto out;

	hh_lock_list(mcg_hot_idx(area, mcg->id.id), area->obj_table);
	hh_list_for_each_entry_reverse_safe(i, tmp,
		mcg_hot_idx(area, mcg->id.id), area->obj_table) { /*lint !e666*/
		if (i >= (u32)area->nr_objs) {
			hh_print(HHLOG_ERR, "index = %d invalid\n", i);
			continue;
		}
	...
	hh_unlock_list(mcg_hot_idx(area, mcg->id.id), area->obj_table);
	hh_print(HHLOG_DEBUG, "hot cnt %d", cnt);
...
void zram_lru_change_size_of_page(struct zram *zram, u32 index, u32 size_diff)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
		atomic64_sub(size_diff, &mcg->zram_stored_hot_size);
		atomic64_sub(size_diff, &stat->zram_stored_hot_size);
	}
	atomic64_sub(size_diff, &mcg->zram_stored_size);
	atomic64_sub(size_diff, &stat->zram_stored_size);
}
#endif

void zram_lru_add(struct zram *zram, u32 index,
					struct mem_cgroup *mcg, struct page *page)
{
	unsigned long size;
	struct hyperhold_stat *stat = hyperhold_get_stat_obj();
	int mcgidx;
	...
	if (PageAnonhot(page)) {
		zram_set_flag(zram, index, ZRAM_OBJ_HOT);
		mcgidx = mcg_hot_idx(zram->area, mcg->id.id);
		hh_print(HHLOG_DEBUG, "index=%u, set flag to ZRAM_OBJ_HOT\n", index);
		atomic64_add(size, &mcg->zram_stored_hot_size);
		atomic64_inc(&mcg->zram_stored_hot_pages);
		atomic64_add(size, &stat->zram_stored_hot_size);
		atomic64_inc(&stat->zram_stored_hot_pages);
	} else {
		mcgidx = mcg_idx(zram->area, mcg->id.id);
	}
	hh_list_add(obj_idx(zram->area, index),
			mcgidx, zram->area->obj_table);
...
void zram_lru_add_tail(struct zram *zram, u32 index, struct mem_cgroup *mcg)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
		...
		if (zram_test_flag(zram, index, ZRAM_FROM_HYPERHOLD)) {
			atomic64_add(size, &mcg->zram_stored_reout_hot_size);
			atomic64_inc(&mcg->zram_stored_reout_hot_pages);
			atomic64_add(size, &stat->zram_stored_reout_hot_size);
			atomic64_inc(&stat->zram_stored_reout_hot_pages);
		}
	} else {
		mcgidx = mcg_idx(zram->area, mcg->id.id);
	}
	...
	atomic64_inc(&stat->zram_stored_pages);
	if (zram_test_flag(zram, index, ZRAM_FROM_HYPERHOLD)) {
		atomic64_add(size, &mcg->zram_stored_reout_size);
		atomic64_inc(&mcg->zram_stored_reout_pages);
		atomic64_add(size, &stat->zram_stored_reout_size);
		atomic64_inc(&stat->zram_stored_reout_pages);
	}
}
...
void zram_lru_del(struct zram *zram, u32 index)
{
	...
	if (zram_test_flag(zram, index, ZRAM_OBJ_HOT)) {
		mcgidx = mcg_hot_idx(zram->area, mcg->id.id);
		atomic64_sub(size, &mcg->zram_stored_hot_size);
		atomic64_dec(&mcg->zram_stored_hot_pages);
		atomic64_sub(size, &stat->zram_stored_hot_size);
		atomic64_dec(&stat->zram_stored_hot_pages);
		if (zram_test_flag(zram, index, ZRAM_FROM_HYPERHOLD)) {
			atomic64_sub(size, &mcg->zram_stored_reout_hot_size);
			atomic64_dec(&mcg->zram_stored_reout_hot_pages);
			atomic64_sub(size, &stat->zram_stored_reout_hot_size);
			atomic64_dec(&stat->zram_stored_reout_hot_pages);
		}
	} else {
		mcgidx = mcg_idx(zram->area, mcg->id.id);
	}
	...
	if (zram_test_flag(zram, index, ZRAM_FROM_HYPERHOLD)) {
		atomic64_sub(size, &mcg->zram_stored_reout_size);
		atomic64_dec(&mcg->zram_stored_reout_pages);
		atomic64_sub(size, &stat->zram_stored_reout_size);
		atomic64_dec(&stat->zram_stored_reout_pages);
	}
}
