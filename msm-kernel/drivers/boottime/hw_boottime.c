...
int __init_or_module do_boottime_initcall(initcall_t fn)
{
    ...
    if(duration > BOOT_50_MS)
    {
        ...
        fn, ret, duration);
        snprintf_s(log_info, BOOT_STR_SIZE, BOOT_STR_SIZE - 1,
						"[WARNING] %pS %llu usecs", fn, duration);
        log_boot(log_info);
    }
    return ret;
}

static u32 Second(unsigned long long nsec, unsigned long divs)
{
    if( (long long)nsec < 0 )
    {
        nsec = -nsec;/*lint !e501 */
...
static u32 mSecond(unsigned long long nsec, unsigned long divs)
{
    if( (long long)nsec < 0 )
        nsec = -nsec;
...
void log_boot(char *str)
{
    ...
    strncpy_s((char*)&hw_boottime[boot_log_count].event, BOOT_STR_SIZE, str, strlen(str));
...
static const struct proc_ops hw_boottime_fops = {     
