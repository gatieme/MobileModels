...
STATIC void ps_exe_heartbeat_func(struct ps_core_s *ps_core_d, uint8_t syschar)
{
    ...
    ps_print_info("pkt_num_rx:BT=%lu,GNSS=%lu,FM=%lu,SYS_MSG=%lu,OML=%lu,timer=%d,mod=%d,sleep_flag=%d,frc_awake=%d\n",
                  ps_core_d->rx_pkt_num[BFGX_BT], ps_core_d->rx_pkt_num[BFGX_GNSS],
                  ps_core_d->rx_pkt_num[BFGX_FM], ps_core_d->rx_pkt_sys,
                  ps_core_d->rx_pkt_oml, timer_pending(&pm_data->bfg_timer),
                  pm_data->bfg_timer_mod_cnt, atomic_read(&pm_data->gnss_sleep_flag), pm_data->gnss_frc_awake);
