...
OAL_STATIC chr_callback_stru g_gst_chr_get_wifi_info_callback;
...
/* 本模块debug控制全局变量 */
OAL_STATIC int32 g_log_enable = CHR_LOG_DISABLE;

OAL_STATIC const struct file_operations g_chr_misc_fops = {
    .owner = THIS_MODULE,
    .open = chr_misc_open,
    .read = chr_misc_read,
...
OAL_STATIC struct miscdevice g_chr_misc_dev = {
...
oal_module_symbol(chr_set_at_test);
...
static int32 chr_misc_open(struct inode *fd, struct file *fp)
{
    if (g_log_enable != CHR_LOG_ENABLE) {
...
static ssize_t chr_misc_read(struct file *fp, int8 __user *buff, size_t count, loff_t *loff)
{
    ...
    if (copy_to_user(puser, skb->data, data_len)) {
 ...
 * 功能描述  : 将异常码写入队列
 */
static int32 chr_write_errno_to_queue(uint32 ul_errno, chr_report_flags_enum_uint16 us_flag,
                                      uint8 *ptr_data, uint16 ul_len)
{
    struct sk_buff *skb = NULL;
    uint16 sk_len;
    int32 ret = 0;
    ...
    if (unlikely(ul_len > (USHRT_MAX- sizeof(chr_dev_exception_stru_para)))) {
...
typedef struct {
    char ap_err_code;
    char arp_tx_err_code;
    char mac_hard_err_code;
} dhcp_fail_chip_info_stru;
...
void chr_test(void)
{
    dhcp_fail_chip_info_stru aa = {0};
    aa.ap_err_code = 'j';
    aa.arp_tx_err_code = 'p';
    aa.mac_hard_err_code = 'p';

    __chr_exception_para(CHR_WIFI_TEST_ERRNO, (oal_uint8 *)&aa, OAL_SIZEOF(dhcp_fail_chip_info_stru));
    ...
    chr_hmac_common_info_stru *pst_hmac_common_info)
{
    ...
    ret = memcpy_s(pst_hmac_common_info->ac_fw_ver, OAL_SIZEOF(pst_hmac_common_info->ac_fw_ver),
    ...
    ret += memcpy_s(pst_hmac_common_info->ac_ko_ver, OAL_SIZEOF(pst_hmac_common_info->ac_ko_ver),
        pst_dmac_common_info->ac_ko_ver, OAL_SIZEOF(pst_hmac_common_info->ac_ko_ver));
    ret += memcpy_s(pst_hmac_common_info->ac_dieid, OAL_SIZEOF(pst_hmac_common_info->ac_dieid),
...
{
    hmac_get_wifi_info_ext_stru     get_wifi_info_ext;
    oal_int32                       memcpy_ret;
    /* 使用钩子函数获取hmac侧参数,还是通过device通道上报 */
    if (g_gst_chr_get_wifi_info_callback.chr_get_wifi_ext_info_from_host == OAL_PTR_NULL) {
        return;
    }
    if (g_gst_chr_get_wifi_info_callback.chr_get_wifi_ext_info_from_host(&get_wifi_info_ext) != CHR_SUCC) {
        return;
    }

    /* 获取干扰参数和芯片类型参数 */
    pst_hmac_radio_link_quality_info->en_alg_distance_stat = get_wifi_info_ext.uc_device_distance;
    pst_hmac_radio_link_quality_info->en_adj_intf_state = get_wifi_info_ext.uc_intf_state_cca;
    pst_hmac_radio_link_quality_info->en_co_intf_state = get_wifi_info_ext.uc_intf_state_co;
    pst_hmac_radio_link_quality_info->c_chip_type = get_wifi_info_ext.uc_chip_type;

    /* 获取共存参数 */
    memcpy_ret = memcpy_s(&pst_hmac_btcoex_status->st_bt_status, OAL_SIZEOF(bt_status_stru),
        &get_wifi_info_ext.st_bt_status, OAL_SIZEOF(bt_status_stru));
    memcpy_ret += memcpy_s(&pst_hmac_btcoex_status->st_ble_status, OAL_SIZEOF(btcoex_ble_status_stru),
        &get_wifi_info_ext.st_ble_status, OAL_SIZEOF(btcoex_ble_status_stru));
    if (memcpy_ret != EOK) {
...
void chr_dmac_transition_to_hmac(chr_dmac_info_stru *buff, uint16 len)
{
    ...
    pst_chr_hmac_info = (chr_hmac_info_stru *)oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL,
    ...
    if (g_gst_chr_get_wifi_info_callback.chr_process_web_fail != OAL_PTR_NULL) {
        g_gst_chr_get_wifi_info_callback.chr_process_web_fail(pst_chr_dmac_info->st_dev_exception_info.errno,
    ...
    oal_mem_free_m(pst_chr_hmac_info, OAL_TRUE);
}

/*
 * 函 数 名  : chr_dev_cali_excp_trigger_dfr
 * 功能描述  : 校准异常触发dfr
 * 修改历史     :
 * 1.日    期   : 2019年10月12日
 *   作    者   : h00368777
...
/*
 * 函 数 名  : chr_dev_cali_excp_process
 * 功能描述  : 校准异常恢复处理
...
/*
 * 函 数 名  : chr_dev_exception_process
 * 功能描述  : dev上报chr异常处理。
...
void chr_host_callback_register(chr_get_wifi_info pfunc)
{
    ...
    g_gst_chr_get_wifi_info_callback.chr_get_wifi_info = pfunc;
...
{
    g_gst_chr_get_wifi_info_callback.chr_get_wifi_info = OAL_PTR_NULL;
...
{
    g_gst_chr_get_wifi_info_callback.chr_process_web_fail = pfunc;
}
void chr_process_web_fail_callback_unregister(void)
{
    g_gst_chr_get_wifi_info_callback.chr_process_web_fail = OAL_PTR_NULL;
}
/*
...
void chr_get_wifi_ext_info_callback_register(chr_get_wifi_info_ext pfunc)
{
    ...
    g_gst_chr_get_wifi_info_callback.chr_get_wifi_ext_info_from_host = pfunc;

    return;
}
void chr_get_wifi_ext_info_callback_unregister(void)
{
    g_gst_chr_get_wifi_info_callback.chr_get_wifi_ext_info_from_host = OAL_PTR_NULL;
...
{
    if (g_gst_chr_get_wifi_info_callback.chr_get_wifi_info == OAL_PTR_NULL) {
        OAL_IO_PRINT("{chr_host_tx_handler:: callback is null!}");
        return CHR_EFAIL;
    }
    if (g_gst_chr_get_wifi_info_callback.chr_get_wifi_info(errno) != CHR_SUCC) {
...
{
    const uint file_id_offset = 16;
    const uint file_id_mask = 0xFFFF;
    const uint line_offset = 0;
    const uint line_mask = 0xFFFF;
    uint32 file_id, line, index;
    ...
    for (index = 0; index < oal_array_size(g_limit_file_line_arr); index++) {
        ...
        if ((g_limit_file_line_arr[index].line_id < 0) ||
            (g_limit_file_line_arr[index].line_id == (int32)line)) {
