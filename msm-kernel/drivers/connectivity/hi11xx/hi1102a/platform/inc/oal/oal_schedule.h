...
typedef struct _oal_dft_trace_item_ {
    ...
    struct timespec64 first_timestamp; /* first keyinfo timestamp */
    struct timespec64 last_timestamp;  /* last keyinfo timestamp */
#else
                                                           ...
                                                           oal_uint16 dtype)
{
    ...
    if ((!pst_dft_item->first_timestamp.tv_sec) && (!oal_get_usec(&pst_dft_item->first_timestamp))) {
        oal_do_gettimeofday(&pst_dft_item->first_timestamp);
    } else {
        oal_do_gettimeofday(&pst_dft_item->last_timestamp);
    }
