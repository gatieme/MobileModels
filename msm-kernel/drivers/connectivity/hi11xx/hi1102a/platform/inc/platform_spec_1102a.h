...
/* PROXY STA模式下最大业务VAP个数:PROXY STA + 1个业务STA */
...
/* 整个BOARD支持的最大的VAP数目 */
/* 18个:1个ap,1个sta,15个proxysta,1个配置vap */
#define WLAN_VAP_SUPPORT_MAX_NUM_LIMIT (WLAN_CHIP_MAX_NUM_PER_BOARD * WLAN_DEVICE_MAX_NUM_PER_CHIP * \
                                        WLAN_VAP_MAX_NUM_PER_DEVICE_LIMIT)

/* 整个BOARD支持的最大业务VAP的数目 */
/* 20个:4个ap,1个sta,15个proxysta, */
#define WLAN_SERVICE_VAP_SUPPOTR_MAX_NUM_LIMIT (WLAN_CHIP_MAX_NUM_PER_BOARD * WLAN_DEVICE_MAX_NUM_PER_CHIP * \
                                                (WLAN_VAP_MAX_NUM_PER_DEVICE_LIMIT - 1))
#define WLAN_ASSOC_USER_MAX_NUM 8 /* 关联用户的最大个数 */

#define WLAN_ACTIVE_USER_MAX_NUM WLAN_ASSOC_USER_MAX_NUM /* 活跃用户的最大个数 = 关联用户数 */

/* board最大用户数 = (单APUT场景)最大关联用户数 + 组播用户个数(8+1=9),其他场景不会到此规格 */
...
/* 接收512(数据帧描述符) + 64(管理帧描述符) */
/* 注意! 新增一个子内存池要更新oal_mem.c里的OAL_MEM_BLK_TOTAL_CNT */
