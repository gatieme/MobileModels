...
/*
 * 函 数 名  : mem_trace_owner_search
 * 功能描述  : 搜索该内存节点是属于哪个模块申请的，如果没有搜到返回一个空的owner表项偏移
 */
...
/*
 * 函 数 名  : __mem_trace_add_node
 * 功能描述  : 登记驱动需要负责释放的内存节点(包括以太过来的skb以及驱动本身申请的内存)
 */
...
/*
 * 函 数 名  : __mem_trace_delete_node
 * 功能描述  : 注销驱动释放的内存节点(包括交给以太的skb以及驱动本身释放的内存)
 */
...
/*
 * 函 数 名  : __mem_trace_probe
 * 功能描述  : 在某个路径放置一个内存跟踪的探针
 */
