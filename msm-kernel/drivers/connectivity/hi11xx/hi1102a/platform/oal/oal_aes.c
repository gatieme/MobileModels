...
#define oal_loop4(i)                               \
    do {                                           \
        ...
        pst_aes_key->ul_key_enc[4 * (i) + 5] = t;  \
        t ^= pst_aes_key->ul_key_enc[4 * (i) + 2]; \
        pst_aes_key->ul_key_enc[4 * (i) + 6] = t;  \
        t ^= pst_aes_key->ul_key_enc[4 * (i) + 3]; \
        pst_aes_key->ul_key_enc[4 * (i) + 7] = t;  \
    } while (0)
...
#define oal_loop6(i)                               \
    do {                                           \
        ...
        pst_aes_key->ul_key_enc[6 * (i) + 7] = t;  \
        t ^= pst_aes_key->ul_key_enc[6 * (i) + 2]; \
        pst_aes_key->ul_key_enc[6 * (i) + 8] = t;  \
        t ^= pst_aes_key->ul_key_enc[6 * (i) + 3]; \
        pst_aes_key->ul_key_enc[6 * (i) + 9] = t;  \
        t ^= pst_aes_key->ul_key_enc[6 * (i) + 4]; \
        pst_aes_key->ul_key_enc[6 * (i) + 10] = t; \
        t ^= pst_aes_key->ul_key_enc[6 * (i) + 5]; \
        pst_aes_key->ul_key_enc[6 * (i) + 11] = t; \
    } while (0)
...
#define loop8tophalf(i)                            \
    do {                                           \
        ...
        pst_aes_key->ul_key_enc[8 * (i) + 9] = t;  \
        t ^= pst_aes_key->ul_key_enc[8 * (i) + 2]; \
        pst_aes_key->ul_key_enc[8 * (i) + 10] = t; \
        t ^= pst_aes_key->ul_key_enc[8 * (i) + 3]; \
        pst_aes_key->ul_key_enc[8 * (i) + 11] = t; \
    } while (0)

#define loop8(i)                                              \
    do {                                                      \
        loop8tophalf(i);                                      \
        t = pst_aes_key->ul_key_enc[8 * (i) + 4] ^ ls_box(t); \
        pst_aes_key->ul_key_enc[8 * (i) + 12] = t;            \
        t ^= pst_aes_key->ul_key_enc[8 * (i) + 5];            \
        pst_aes_key->ul_key_enc[8 * (i) + 13] = t;            \
        t ^= pst_aes_key->ul_key_enc[8 * (i) + 6];            \
        pst_aes_key->ul_key_enc[8 * (i) + 14] = t;            \
        t ^= pst_aes_key->ul_key_enc[8 * (i) + 7];            \
        pst_aes_key->ul_key_enc[8 * (i) + 15] = t;            \
    } while (0)
...
{
    OAL_CONST oal_uint32 *pul_key;
    oal_uint32 i, t, u, v, w, j;

    if ((pst_aes_key == OAL_PTR_NULL) || (puc_key == OAL_PTR_NULL)) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* modify by huchikun at 2017.3.6 for completion */
    if (ul_key_len != OAL_AES_KEYSIZE_128
        && ul_key_len != OAL_AES_KEYSIZE_192
        && ul_key_len != OAL_AES_KEYSIZE_256) {
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

    pul_key = (OAL_CONST oal_uint32 *)puc_key;

    pst_aes_key->ul_key_length = ul_key_len;

    pst_aes_key->ul_key_dec[ul_key_len + 24] = pst_aes_key->ul_key_enc[0] = (pul_key[0]);
    pst_aes_key->ul_key_dec[ul_key_len + 25] = pst_aes_key->ul_key_enc[1] = (pul_key[1]);
    pst_aes_key->ul_key_dec[ul_key_len + 26] = pst_aes_key->ul_key_enc[2] = (pul_key[2]);
    pst_aes_key->ul_key_dec[ul_key_len + 27] = pst_aes_key->ul_key_enc[3] = (pul_key[3]);

    /* modify by huchikun at 2017.3.6 for completion */
    /*lint -e744*/
    switch (ul_key_len) {
        case OAL_AES_KEYSIZE_128:
            t = pst_aes_key->ul_key_enc[3];
            for (i = 0; i < 10; ++i) {
                oal_loop4(i);
            }
            break;

        case OAL_AES_KEYSIZE_192:
            pst_aes_key->ul_key_enc[4] = (pul_key[4]);
            t = pst_aes_key->ul_key_enc[5] = (pul_key[5]);
            for (i = 0; i < 8; ++i) {
                oal_loop6(i);
            }
            break;
        ...
        case OAL_AES_KEYSIZE_256:
            pst_aes_key->ul_key_enc[4] = (pul_key[4]);
            pst_aes_key->ul_key_enc[5] = (pul_key[5]);
            pst_aes_key->ul_key_enc[6] = (pul_key[6]);
            t = pst_aes_key->ul_key_enc[7] = (pul_key[7]);
            for (i = 0; i < 6; ++i) {
                loop8(i);
            }
            loop8tophalf(i);
    ...
    /*lint +e744*/
    pst_aes_key->ul_key_dec[0] = pst_aes_key->ul_key_enc[ul_key_len + 24];
    pst_aes_key->ul_key_dec[1] = pst_aes_key->ul_key_enc[ul_key_len + 25];
    pst_aes_key->ul_key_dec[2] = pst_aes_key->ul_key_enc[ul_key_len + 26];
    pst_aes_key->ul_key_dec[3] = pst_aes_key->ul_key_enc[ul_key_len + 27];

    for (i = 4; i < ul_key_len + 24; ++i) {
        j = ul_key_len + 24 - (i & ~3) + (i & 3);
        imix_col(pst_aes_key->ul_key_dec[j], pst_aes_key->ul_key_enc[i]);
    }
                           ...
                           OAL_CONST oal_uint8 *puc_plaintext)
{
    ...
    pul_dst = (oal_uint32 *)puc_ciphertext;
    pul_src = (OAL_CONST oal_uint32 *)puc_plaintext;
    ul_key_len = pst_aes_key->ul_key_length;
    pul_kp = pst_aes_key->ul_key_enc + 4;

    b0[0] = (pul_src[0]) ^ pst_aes_key->ul_key_enc[0];
    b0[1] = (pul_src[1]) ^ pst_aes_key->ul_key_enc[1];
    b0[2] = (pul_src[2]) ^ pst_aes_key->ul_key_enc[2];
    b0[3] = (pul_src[3]) ^ pst_aes_key->ul_key_enc[3];

    if (ul_key_len > 24) {
        f_nround(b1, b0, pul_kp);
        f_nround(b0, b1, pul_kp);
    }
                           ...
                           OAL_CONST oal_uint8 *puc_ciphertext)
{
    ...
    pul_dst = (oal_uint32 *)puc_plaintext;
    pul_src = (OAL_CONST oal_uint32 *)puc_ciphertext;
    ul_key_len = pst_aes_key->ul_key_length;
    pul_kp = pst_aes_key->ul_key_dec + 4;

    b0[0] = (pul_src[0]) ^ pst_aes_key->ul_key_dec[0];
    b0[1] = (pul_src[1]) ^ pst_aes_key->ul_key_dec[1];
    b0[2] = (pul_src[2]) ^ pst_aes_key->ul_key_dec[2];
    b0[3] = (pul_src[3]) ^ pst_aes_key->ul_key_dec[3];

    if (ul_key_len > 24) {
        i_nround(b1, b0, pul_kp);
        i_nround(b0, b1, pul_kp);
    }
...
OAL_STATIC oal_int32 oal_aes_key_setup_encrypt(oal_uint8 *in_key, oal_uint32 key_len, oal_aes_key_stru *pst_aes_key)
{
    ...
    if ((key_len != OAL_AES_KEYSIZE_128) &&
        (key_len != OAL_AES_KEYSIZE_192) &&
        (key_len != OAL_AES_KEYSIZE_256)) {
        return OAL_FAIL;
    }

    pst_aes_key->ul_key_length = key_len;

    pst_aes_key->ul_key_dec[key_len + OAL_AES_FRAME_HEAD_LEN]     = pst_aes_key->ul_key_enc[0] = oal_le32_to_host(key[0]);
    pst_aes_key->ul_key_dec[key_len + OAL_AES_FRAME_HEAD_LEN + 1] = pst_aes_key->ul_key_enc[1] = oal_le32_to_host(key[1]);
    pst_aes_key->ul_key_dec[key_len + OAL_AES_FRAME_HEAD_LEN + 2] = pst_aes_key->ul_key_enc[2] = oal_le32_to_host(key[2]);
    pst_aes_key->ul_key_dec[key_len + OAL_AES_FRAME_HEAD_LEN + 3] = pst_aes_key->ul_key_enc[3] = oal_le32_to_host(key[3]);
    ...
    switch (key_len) {
        ...
        case OAL_AES_KEYSIZE_192: {
            ...
            t = pst_aes_key->ul_key_enc[5] = oal_le32_to_host(key[5]);
            for (i = 0; i < 8; ++i) {
                oal_loop6(i);
            }
        }
        ...
        default:  // Coverity修改:前面已经限定只有这三种情况了
        {
            ...
            pst_aes_key->ul_key_enc[6] = oal_le32_to_host(key[6]);
            t = pst_aes_key->ul_key_enc[7] = oal_le32_to_host(key[7]);
            for (i = 0; i < 6; ++i) {
                loop8(i);
            }
            loop8tophalf(i);
    ...
    pst_aes_key->ul_key_dec[0] = pst_aes_key->ul_key_enc[key_len + 24];
    pst_aes_key->ul_key_dec[1] = pst_aes_key->ul_key_enc[key_len + 25];
    pst_aes_key->ul_key_dec[2] = pst_aes_key->ul_key_enc[key_len + 26];
    pst_aes_key->ul_key_dec[3] = pst_aes_key->ul_key_enc[key_len + 27];

    for (i = 4; i < key_len + OAL_AES_FRAME_HEAD_LEN; ++i) {
        j = key_len + OAL_AES_FRAME_HEAD_LEN - (i & ~3) + (i & 3);
        imix_col(pst_aes_key->ul_key_dec[j], pst_aes_key->ul_key_enc[i]);
    }
...
OAL_STATIC OAL_INLINE oal_void oal_bip_ipn_swap(
    oal_uint8 *d, oal_uint8 d_len, OAL_CONST oal_uint8 *s, OAL_CONST oal_uint8 s_len)
{
    *d++ = s[5];
    *d++ = s[4];
    *d++ = s[3];
...
OAL_STATIC oal_void oal_gf_mulx(oal_uint8 *pad)
{
    ...
    carry = pad[0] & 0x80;
    for (i = 0; i < OAL_AES_BLOCK_SIZE - 1; i++) {
        pad[i] = (oal_uint8)((pad[i] << 1) | (pad[i + 1] >> 7));
    }
    pad[OAL_AES_BLOCK_SIZE - 1] <<= 1;
    if (carry) {
        pad[OAL_AES_BLOCK_SIZE - 1] ^= 0x87;
    }
}

OAL_STATIC OAL_INLINE oal_uint32 oal_crypto_cipher_encrypt_one(oal_aes_key_stru *aes_key, oal_uint8 *dst,
                                                               OAL_CONST oal_uint8 *src)
{
    oal_uint alignmask = 3;

    if ((((uintptr_t)dst) | (uintptr_t)src) & alignmask) {
                                              ...
                                              oal_uint8 *mac)
{
    ...
    cbc = scratch;
    pad = scratch + OAL_AES_BLOCK_SIZE;

    memset_s(cbc, OAL_AES_BLOCK_SIZE, 0, OAL_AES_BLOCK_SIZE);
    ...
    e = 0;
    pos = addr[0];
    end = (oal_uint8 *)(pos + len[0]);

    while (left >= OAL_AES_BLOCK_SIZE) {
        for (i = 0; i < OAL_AES_BLOCK_SIZE; i++) {
            cbc[i] ^= *pos++;
            if (pos >= end) {
                e++;
                pos = addr[e];
                end = (oal_uint8 *)(pos + len[e]);
            }
        }
        if (left > OAL_AES_BLOCK_SIZE) {
            oal_crypto_cipher_encrypt_one(aes_key, cbc, cbc);
        }
        left -= OAL_AES_BLOCK_SIZE;
    }

    memset_s(pad, OAL_AES_BLOCK_SIZE, 0, OAL_AES_BLOCK_SIZE);
    ...
    memcpy_s(mac, OAL_CMAC_TLEN, pad, OAL_CMAC_TLEN);
...
{
    OAL_CONST oal_uint8 *addr[3];
    oal_uint32 len[3];
    oal_uint8 zero[OAL_CMAC_TLEN];

    memset_s(zero, sizeof(zero), 0, sizeof(zero));
    addr[0] = aad;
    len[0] = aad_len;
    addr[1] = data;
    len[1] = data_len - OAL_CMAC_TLEN;
    addr[2] = zero;
    len[2] = OAL_CMAC_TLEN;
...
oal_uint32 oal_crypto_aes_cmac_encrypt(oal_aes_ctx_stru *aes_ctx, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_netbuf_tailroom(pst_netbuf) < OAL_SIZEOF(*mmie)) {
        return OAL_FAIL;
    }
    oal_netbuf_put(pst_netbuf, OAL_SIZEOF(*mmie));
    mmie = (oal_mmie_stru *)(uintptr_t)(pst_netbuf->tail - OAL_SIZEOF(*mmie));

    memset_s((oal_void *)mmie, OAL_SIZEOF(*mmie), 0, OAL_SIZEOF(*mmie));
    ...
    for (i = (oal_int)OAL_SIZEOF(aes_ctx->pn) - 1; i >= 0; i--) {
        pn[i]++;
        if (pn[i]) {
            break;
        }
    }
    oal_bip_ipn_swap(mmie->sequence_number, OAL_MMIE_IPN_LEN * sizeof(oal_uint8),
        pn, OAL_AES_CTX_PN_LEN * sizeof(oal_uint8));
                                                  ...
                                                  oal_netbuf_stru *pst_netbuf)
{
    oal_mmie_stru *mmie = OAL_PTR_NULL;
    oal_uint8 aad[OAL_AAD_LEN];
    oal_uint8 mic[OAL_MMIE_MIC_LEN]   = {0};
    oal_uint8 ipn[OAL_AES_CTX_PN_LEN] = {0};

    if (pst_netbuf->len < OAL_AES_FRAME_HEAD_LEN + OAL_SIZEOF(*mmie)) {
        return OAL_FAIL;
    }

    mmie = (oal_mmie_stru *)(pst_netbuf->data + pst_netbuf->len - OAL_SIZEOF(*mmie));
    if ((mmie->element_id != OAL_EID_MMIE) || (mmie->length != OAL_SIZEOF(*mmie) - OAL_IE_HDR_LEN)) {
        /* Invalid MMIE */
        return OAL_FAIL;
    }

    oal_bip_ipn_swap(ipn, OAL_AES_CTX_PN_LEN * sizeof(oal_uint8),
        mmie->sequence_number, OAL_MMIE_IPN_LEN * sizeof(oal_uint8));
    ...
    memcpy_s(aes_ctx->pn, OAL_SIZEOF(aes_ctx->pn), ipn, OAL_SIZEOF(ipn));
...
oal_void oal_crypto_bip_enmic(oal_uint8 uc_igtk_keyid,
                              ...
                              oal_netbuf_stru *pst_netbuf,
                              oal_uint16 *pst_frame_len)
{
    oal_aes_ctx_stru st_aes_ctx;
    oal_uint32 ul_ret;
    ...
    st_aes_ctx.pn[3] = pst_igtk_seq[2];
    ...
    *pst_frame_len += OAL_MGMT_FRAME_MMIE_LEN;
}
#endif
