...
OAL_STATIC OAL_INLINE oal_void oal_sdio_detect_change(oal_void)
{
    if (g_sdio_host != NULL) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "hi1102a mmc detect start");
        g_sdio_host->rescan_disable = 0;
        g_sdio_host->rescan_entered = 0;
        mmc_detect_change(g_sdio_host, 0);
    } else {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "g_sdio_host is null");
    }
}

OAL_STATIC OAL_INLINE oal_int32 oal_mmc_power_on(struct oal_sdio *hi_sdio)
{
    oal_int32 ret;

    hi_sdio->func->card->host->pm_flags &= ~MMC_PM_KEEP_POWER;
    ret = mmc_hw_reset(hi_sdio->func->card->host);
    hi_sdio->func->card->host->pm_flags |= MMC_PM_KEEP_POWER;

    return ret;
}

OAL_STATIC OAL_INLINE oal_void oal_mmc_power_off(struct oal_sdio *hi_sdio)
{
...
oal_void oal_sdio_detectcard_to_core(oal_void)
{
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "hi1102a mmc detect, vendor id:0x%x  product id:0x%x",
                             HISDIO_VENDOR_ID_HI1102A, HISDIO_PRODUCT_ID_HISI);

    oal_sdio_detect_change();
}
...
OAL_STATIC void oal_sdio_sw_power_down_proc(hcc_bus *pst_bus, struct oal_sdio *hi_sdio)
{
...
OAL_STATIC oal_int32 oal_sdio_power_action(hcc_bus *pst_bus, hcc_bus_power_action_type action)
{
    ...
    if (action == HCC_BUS_SW_POWER_DOWN) {
        oal_sdio_sw_power_down_proc(pst_bus, hi_sdio);
    }
...
oal_int32 oal_sdio_switch_clean_res(hcc_bus *pst_bus)
{
    ...
    if (oal_wait_for_completion_timeout(&hi_sdio->st_sdio_shutdown_response,
                                        (oal_uint32)oal_msecs_to_jiffies(10000)) == 0) {
...
#ifdef CONFIG_ARCH_HI1103_SDIO_DEBUG
void oal_sdio_reinit_debug_info(void)
{
...
oal_void oal_sdio_build_rx_netbuf_list_work(oal_work_stru *pst_work)
{
    ...
    hcc_bus *pst_hcc_bus = NULL;
    oal_netbuf_head_stru *phead = NULL;
    oal_netbuf_stru *netbuf = NULL;

    if (hcc == NULL) {
        return;
    }
    pst_hcc_bus = hcc->bus_dev->cur_bus;
    ...
    phead = &pst_hcc_bus->rx_netbuf_head;
...
OAL_STATIC oal_int32 oal_sdio_writesb_by_heap(struct oal_sdio *hi_sdio, oal_uint8 *buf, oal_uint32 len)
{
    ...
    if (virt_addr_valid(buf) && !object_is_on_stack(buf)) {
        ...
        if ((len < HISDIO_SEND_SIZE) && (hi_sdio->sdio_send_buff != NULL)) {
            ...
            send_buf = (oal_uint8 *)kmalloc(len, GFP_KERNEL);
            if (send_buf == NULL) {
                                           ...
                                           oal_uint32 len, oal_uint32 timeout)
{
    ...
    return (ret ? -OAL_EFAIL : (oal_int32)xfer_count);
#else
    return -1;
#endif
}
...
{
    oal_int32 ret = OAL_SUCC;
    oal_uint32 tailroom, tailroom_add;
                         ...
                         oal_uint32 sg_len)
{
    ...
    for_each_sg(sg, sg_t, sg_len, i)
    {
        ...
        if (oal_warn_on(((uintptr_t)sg_virt(sg_t) & 0x03) || (sg_t->length & 0x03))) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "check_sg_format:[i:%u][addr:%p][len:%u]",
                                 i, sg_virt(sg_t), sg_t->length);
        }
    }
}

OAL_STATIC int32_t oal_mmc_io_rw_scat_extended_check(struct oal_sdio *hi_sdio,
...
OAL_STATIC void oal_mmc_io_rw_scat_extended_dump(struct mmc_card *card,
...
OAL_STATIC int32_t oal_sdio_get_mmc_errno_by_resp(struct mmc_command *cmd)
{
...
OAL_STATIC void oal_mmc_io_rw_dump_mmc_data_info(struct mmc_data *data)
{
#ifdef CONFIG_SDIO_DEBUG
    int i;
                                                        ...
                                                        int32_t rw)
{
    ...
    for_each_sg(sg, sg_t, nents, i)
    {
        ...
        sg_set_buf(sg_t, hi_sdio->tx_scatt_buff.buff + i * seg_size, oal_min(seg_size, left_size));
        left_size = left_size - seg_size;
        idx++;
    }
...
OAL_STATIC oal_int32 oal_sdio_resume(struct device *dev)
{
    ...
    /* 內核起来后会清MMC_PM_KEEP_POWER，这里重新置位上，否则下次唤醒内核尝试发送cmd5失败 */
    hi_sdio->func->card->host->pm_flags |= MMC_PM_KEEP_POWER;
#endif
    return OAL_SUCC;
}
...
struct oal_sdio *oal_sdio_init_module()
{
    ...
    hi_sdio->tx_scatt_buff.buff = oal_mem_dma_blockalloc(tx_scatt_buff_len, 5 * MSEC_PER_SEC); /* 5 seconds timeout */
    ...
    hi_sdio->rx_scatt_buff.buff = oal_mem_dma_blockalloc(rx_scatt_buff_len, 5 * MSEC_PER_SEC); /* 5 seconds timeout */
