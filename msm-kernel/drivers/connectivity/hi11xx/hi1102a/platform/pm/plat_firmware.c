...
int32 string_to_num(uint8 *string, int32 *number)
{
    ...
    for (i = 0; (string[i] >= '0') && (string[i] <= '9'); i++) {
        l_num = (l_num * 10) + (int32)(string[i] - '0');   /* 字符串转数字的逻辑需要 */
    }
...
int32 number_type_cmd_send(uint8 *key, const char *value)
{
    ...
    for (i = 0, n = 0; (i <= value_len) && (n < INT32_STR_LEN); i++) {
        if ((value[i] == ',') || (value_len == i)) {
            ps_print_dbg("auc_num = %s, i = %u, n = %u\n", auc_num, i, n);
            if (n == 0) {
                continue;
            }
...
int32 write_device_reg16(uint32 address, uint16 value)
{
    ...
    buf_len = snprintf_s(buf_tx, sizeof(buf_tx), sizeof(buf_tx) - 1, "%s%c2%c0x%x%c%hu%c",
                         WMEM_CMD_KEYWORD,
                         COMPART_KEYWORD,
                         COMPART_KEYWORD,
                         address,
...
int32 wifi_device_mem_dump(struct st_wifi_dump_mem_info *pst_mem_dump_info, uint32 count)
{
    ...
    uint32 i;
    uint8 *data_buf = NULL;
    uint8 buff[DEV_MEMDUMP_BUFF_LEN];
...
int32 wifi_device_mem_dump(struct st_wifi_dump_mem_info *pst_mem_dump_info, uint32 count)
{
    ...
    uint32 sdio_transfer_limit;

    sdio_transfer_limit = hcc_get_max_trans_size(hcc_get_110x_handler());
    if (!ps_is_device_log_enable() || (sdio_transfer_limit == 0)) {
        return 0;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (OAL_IS_ERR_OR_NULL(fp)) {
            ps_print_err("create file error, errno %ld, filename is [%s]\n",
                         PTR_ERR(fp), pst_mem_dump_info[i].file_name);
            break;
        }
...
int32 sdio_read_path_mem(uint8 *key, uint8 *value, uint8 *file_name)
{
    ...
    sdio_transfer_limit = oal_min(PAGE_SIZE, sdio_transfer_limit);
    if (sdio_transfer_limit == 0) {
        ps_print_err("buffer len is error\n");
        return -ENOMEM;
    }
    ...
    if (IS_ERR(fp)) {
        ps_print_err("create file error = %ld\n", PTR_ERR(fp));
        os_mem_kfree(tmp_data_buf);
        return SUCC;
    }
...
STATIC int32 file_open_get_len(int8 *path, os_kernel_file_stru **fp, uint32 *file_len)
{
    ...
    *fp = filp_open(path, O_RDONLY, 0);
    if (IS_ERR_OR_NULL(*fp)) {
        ps_print_err("filp_open [%s] fail!!, errno:%ld\n", path, PTR_ERR(fp));
        set_fs(fs);
        *fp = NULL;
        return -EFAIL;
    }
 ...
 * 返 回 值  : 0表示成功，-1表示失败
 */
int32 firmware_read_cfg(uint8 *cfg_patch, uint8 *puc_read_buffer, uint32_t buf_len)
{
    mm_segment_t fs;
    int32 ret;
    os_kernel_file_stru *fp = NULL;
    ...
    if (OAL_IS_ERR_OR_NULL(fp)) {
        set_fs(fs);
        ps_print_err("open file %s fail, errno=%ld\n", cfg_patch, PTR_ERR(fp));
        fp = NULL;
        return -EFAIL;
    }

    memset_s(puc_read_buffer, buf_len, 0, buf_len);

    ret = oal_file_read_ext(fp, fp->f_pos, puc_read_buffer, buf_len);
                         ...
                         uint8 *puc_cmd_para, uint32 cmd_para_len)
{
    ...
    if (!os_mem_cmp(handle, (uint8 *)FILE_TYPE_CMD_KEY, os_str_len((uint8 *)FILE_TYPE_CMD_KEY))) {
        ...
        l_cmdlen = l_cmdlen - (int32)os_str_len(FILE_TYPE_CMD_KEY);
        ...
        l_cmdlen = l_cmdlen - (int32)os_str_len(NUM_TYPE_CMD_KEY);
...
int32 firmware_get_cfg(uint8 *cfg_patch, uint32 ul_index)
{
    ...
    l_readlen = firmware_read_cfg(cfg_patch, puc_read_cfg_buf, READ_CFG_BUF_LEN);
    if (l_readlen < 0) {
