...
void bfgx_uart_baud_change_expire(oal_timeout_func_para_t data)
{
    struct pm_drv_data *pm_data = oal_get_timer_container(pm_data, data, baud_change_timer);
    uint64 flags;
    struct ps_core_s *ps_core_d = NULL;
    if (unlikely(pm_data == NULL)) {
...
static inline int32 ps_change_baud_rate_retry(uint64 baud_rate, uint8 flowctl_status)
{
...
void host_wkup_dev_work(struct work_struct *work)
{
    ...
    ps_get_core_reference(&ps_core_d);
    if ((pm_data == NULL) || (ps_core_d == NULL)) {
...
STATIC int32_t disallow_msg_send_check(struct ps_core_s *ps_core_d)
{
    const uint32_t wait_timeout = 5 * HZ;
    if (ps_core_d == NULL) {
        ps_print_err("ps_core has not inited\n");
        return -FAILURE;
    }
    ...
    if (down_timeout(&ps_core_d->sr_wake_sema, wait_timeout) == -ETIME) {
        ps_print_err("uart not resume!, sr_cnt %d\n", atomic_read(&ps_core_d->sr_cnt));
        return -FAILURE;
    }
    ...
    if (bfg_wait_tty_resume(ps_core_d) == OAL_FALSE) {
        ps_print_err("host send disallow msg wait tty resume fail!\n");
        return -FAILURE;
    }
...
void host_send_disallow_msg(struct work_struct *work)
{
    ...
    if (disallow_msg_send_check(ps_core_d) < 0) {
    ...
    if (oal_atomic_read(&g_ir_only_mode) == 0) {
        ...
        if ((!bfgx_other_subsys_all_shutdown(BFGX_GNSS)) ||
            ...
            mod_timer(&pm_data->bfg_timer, jiffies + oal_msecs_to_jiffies(PLATFORM_SLEEP_TIME));
...
STATIC int32_t bfg_enter_sleep_check(struct pm_drv_data *pm_data, struct ps_core_s *ps_core_d)
{
...
STATIC int32_t agree_allow_bfg_sleep(struct ps_core_s *ps_core_d, struct pm_drv_data *pm_data)
{
    ...
    if ((gnss_get_lowpower_state(pm_data) == GNSS_NOT_AGREE_SLEEP) || (ps_chk_tx_queue_empty(ps_core_d) == false)) {
        ps_print_info("tx queue not empty, or sleep vote[%d], frc awake[%d]\n",
...
STATIC void host_allow_bfg_sleep_cmd(struct pm_drv_data *pm_data)
{
    ...
    if (allow_sleep) {
        ...
        ps_print_info("wait device sleep ack timeout[%d], timeleft[%ld], dev state[%d]\n",
                      WAIT_DEVACK_TIMEOUT_MSEC, timeleft, pm_data->bfgx_dev_state);
    }
}
...
void host_allow_bfg_sleep(struct work_struct *work)
{
    ...
    if (bfg_enter_sleep_check(pm_data, ps_core_d) != SUCCESS) {
    ...
    mod_timer(&pm_data->dev_ack_timer, jiffies + oal_msecs_to_jiffies(WAIT_DEVACK_MSEC));
...
void bfgx_print_subsys_state(void)
{
    ...
    int32 total;
    int32 count = 0;
    char print_str[200]; /* 200:bfgx state format len */
    struct ps_core_s *ps_core_d = NULL;
...
*/
int32_t gnss_set_frc_awake(struct pm_drv_data *pm_data, uint32_t frc)
{
    int32_t ret;
    struct ps_core_s *ps_core_d = NULL;
    ...
    ps_print_info("gnss_frc_awake %d!\n", frc);
...
void gnss_lowpower_vote(uint32_t vote)
{
    ...
    ps_print_info("gnss_lowpower_vote %d!\n", vote);
    if (vote == GNSS_AGREE_SLEEP) {
        ...
        if (!timer_pending(&pm_data->bfg_timer) && (gnss_get_lowpower_state(pm_data) == GNSS_AGREE_SLEEP)) {
            ps_print_info("gnss_lowpower_vote sumit sleep work!\n");
...
int32 wifi_notify_bfgx_status(uint8 status)
{
    struct ps_core_s *ps_core_d = NULL;
    int32 ret;
...
static int32 device_mem_check_prepare(unsigned long long *time)
{
    if (time == NULL) {
...
static int32 device_mem_check_wcpu(void)
{
    int32 ret;
    ...
    if (ret == SUCCESS) {
        ...
        ps_print_err("Memory check failed because the firmware fails to be downloaded. ret=%d\n", ret);
        board_power_off(WLAN_POWER);
        return ret;
    }
...
static int32 device_mem_check_bcpu(void)
{
    int32 ret;
    uint8 *file_name = WIFI_DUMP_PATH "/readm_bfgx";
    ...
    ret = firmware_download_function(RAM_BCPU_REG_TEST_CFG);
...
STATIC int32_t low_power_irq_init(struct pm_drv_data *pm_data)
{
    int32_t ret;
...
STATIC int32_t low_power_workq_init(struct pm_drv_data *pm_data)
{
...
STATIC void low_power_probe_timer_init(struct pm_drv_data *pm_data)
{
    ...
    pm_data->uc_dev_ack_wait_cnt = 0;
#else
    timer_setup(&pm_data->dev_ack_timer, (void*)devack_timer_expire, 0);
#endif
...
#else
    timer_setup(&pm_data->bfg_timer, (void*)bfg_timer_expire, 0);
#endif
...
#else
    timer_setup(&pm_data->baud_change_timer, (void*)bfgx_uart_baud_change_expire, 0);
#endif
}

STATIC int32_t low_power_probe(void)
{
    int32_t ret;
    ...
    if (pm_data->pst_wlan_pm_info == 0) {
        ps_print_err("wlan pm init failed!\n");
        goto WLAN_INIT_FAIL;
    }
    ...
    if (ret < 0) {
        ps_print_warning("%s : register_pm_notifier failed, ret = %d\n", __func__, ret);
    }
