...
/* 全局变量定义 */
oal_uint8 *g_dcxo_data_buf = NULL;  // 存储从NV中读取的参数信息，解析cfg文件时赋值一次，后续GNSS业务需要，也会更新该变量
...
oal_int32 read_coeff_from_nvram(oal_uint8 *pc_out, nv_part_str *nv_para)
{
    ...
    if (nv_para->part_id < 0) {  // 对于小于0的nv id为虚拟配置，目前用来配置dcxo默认值
        ...
        info.valid_size = (oal_uint32)nv_para->total_size;
        info.nv_number = (oal_uint32)nv_para->part_id;
...
report_chr:
    chr_nv_rd.ul_struct_len = OAL_SIZEOF(plat_chr_nv_rd_excep_stru);
   ...
   // 依次遍历平台分区，工厂分区，自学习分区，读取nv内容并进行处理
    for (loop = 0; loop < nv_size; loop++) {
        ...
        if (g_gst_dcxo_nv_part[loop].process_cali != NULL) {
            ...
            g_dcxo_info.use_part_id = (oal_uint32)g_gst_dcxo_nv_part[loop].part_id;
        }
