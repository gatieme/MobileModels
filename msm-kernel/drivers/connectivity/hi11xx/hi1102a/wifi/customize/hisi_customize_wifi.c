...
#ifdef _PRE_WLAN_FEATURE_NRCOEX
/*
 * 函数名:host_params_init_nrcoex_params
 * 功能:初始化NR共存相关的系统默认值
 * 修改历史:
 * 1.日    期:20190929
 *   作    者:r00356856
...
/*
 * 函数名:hwifi_get_nrcoex_rule_value
 * 功能:读取NR共存相关的定制化项的值
 * 修改历史:
 * 1.日    期:20190929
 *   作    者:r00356856
...
/*
 * 函数名:hwifi_set_nrcoex_rule
 * 功能:从定制化中读取的值配置NRCOEX的规则
 * 修改历史:
 * 1.日    期:20190929
 *   作    者:r00356856
 *   修改历史:新生成函数
...
/*
 * 函数名:hwifi_config_init_nrcoex_params
 * 功能:读取NR共存相关的定制化参数
 * 修改历史:
 * 1.日    期:20190929
 *   作    者:r00356856
...
int32 hwifi_get_mac_addr(uint8 *puc_buf)
{
    ...
    wifimac = get_slave_wlan_mac_address();
    if (wifimac == NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hisi_customize_wifi::wifimac is NULL!");
        return INI_FAILED;
    }

    if (!ether_is_all_zero(wifimac)) {
        ret = memcpy_s(puc_buf, MAC_LEN, wifimac, MAC_LEN);
        if (ret != EOK) {
    ...
    if (!ret) {
        ...
        if (sum != 0) {
            ...
            if (set_slave_wlan_mac_address(puc_buf, MAC_LEN) != INI_SUCC) {
                OAL_IO_PRINT("set 1102a mac address from cnss_utils failed.\n");
            } else {
                OAL_IO_PRINT("set 1102a mac complete, 1102a mac_address %02X:XX:XX:XX:XX:%02X. \n",
                puc_buf[0], puc_buf[MAC_LEN-1]);
            }
        } else {
