    ...
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf)
{
    ...
    mac_set_cb_ac(tx_ctl, WLAN_WME_AC_MGMT);
 ...
 *   作    者  : x00579508
 */
static void hmac_chba_work_channel_switch(hmac_vap_stru *hmac_vap, mac_channel_stru *mac_channel,
    uint8_t switch_slot)
{
    mac_vap_stru *mac_vap = NULL;
    hmac_chba_vap_stru *chba_vap_info = hmac_get_chba_vap();
 ...
 * 作    者  : x00579508
 */
static void hmac_chba_chan_work_channel_switch(uint8_t channel_number, uint8_t bandwidth, uint8_t switch_slot)
{
    uint8_t device_id;
    uint32_t ret;
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_info_stru *chan_switch_info)
{
    ...
    for (idx = 0; idx < device_cnt; idx++) {
        ...
        if (memcpy_s(chan_switch_info->csa_lost_device[chan_switch_info->csa_lost_device_num].mac_addr,
                     OAL_MAC_ADDR_LEN, peer_addr, OAL_MAC_ADDR_LEN) != EOK) {
            return 0;
        }
    ...
    if (lost_device_info.csa_lost_device_num != 0) {
        ...
        for (idx = 0; idx < lost_device_info.csa_lost_device_num; idx++) {
            if (memcpy_s(lost_device_info.csa_lost_device[idx].mac_addr, OAL_MAC_ADDR_LEN,
                         chan_switch_info->csa_lost_device[idx].mac_addr, OAL_MAC_ADDR_LEN) != EOK) {
                return;
            }
        }
    }
 ...
 * 作    者  : x00579508
 */
oal_uint32 hmac_chba_retx_chan_switch_ind_timeout(oal_void *p_arg)
{
    ...
    if (chan_switch_info->chan_switch_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&(chan_switch_info->chan_switch_timer));
    }

    /* 保存未收到ack的设备mac地址，已备后续补救, 并通知dmac计算出补救的时间点 */
    if (hmac_chba_lost_device_check_and_save(chba_vap_info, chba_mgmt_info, chan_switch_info)) {
        hmac_chba_csa_lost_device_notify(&(hmac_vap->st_vap_base_info), chan_switch_info);
    }

    /* 抛事件给DMAC，配置自己的信道切换 */
    hmac_chba_chan_work_channel_switch(chan_switch_info->last_target_channel,
 ...
 * 作    者  : x00579508
 */
oal_uint32 hmac_chba_collect_chan_switch_ack_timeout(oal_void *p_arg)
{
    uint32_t ret;
    ...
    if (chan_switch_info->ack_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&(chan_switch_info->ack_timer));
    }

    /* 存在保活设备没回ack，timeout时间加长 */
    if (hmac_chba_check_keepalive_non_confirm(self_conn_info)) {
        timeout = CHBA_KEEPALIVE_RETX_CHAN_SWITCH_IND_TIMEOUT;
    }

    /* 给岛内没有收到ack的节点重传信道切换指示帧 */
    ...
    mac_channel_stru *target_channel, uint8_t remain_slot, uint8_t type)
{
    ...
    if (hmac_chba_is_valid_channel(target_channel->uc_chan_number) != OAL_SUCC) {
        oam_error_log2(0, OAM_SF_CHBA, "hmac_chba_start_switch_chan::invalid number %d, bwmode %d.",
    ...
    if (mac_regdomain_channel_is_support_bw(target_channel->en_bandwidth,
        ...
        oam_error_log2(0, OAM_SF_CHBA, "hmac_chba_start_switch_chan::target channel number=[%d], bw=[%d] invalid",
            target_channel->uc_chan_number, target_channel->en_bandwidth);
        return OAL_FAIL;
    }
    chan_switch_info = &chba_mgmt_info->chan_switch_info;
 ...
 * 作    者  : x00579508
 */
static oal_uint32 hmac_chba_chan_switch_req_timeout(oal_void *p_arg)
{
    uint32_t ret;
    uint32_t curr_time;
    hmac_chba_chan_switch_notify_stru chan_switch_ntf = { 0 };
    ...
    if (chan_switch_info->req_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&(chan_switch_info->req_timer));
    }
    ...
    uint16_t payload_len)
{
    ...
    if ((chba_vap_info->work_channel.uc_chan_number != owner_chan_number) ||
        ...
        hmac_chba_chan_work_channel_switch(owner_chan_number, owner_chan_bandwidth, MAC_CHBA_CHAN_SWITCH_IMMEDIATE);
    }
    return;
}
 ...
 * 作    者  : x00579508
 */
static void hmac_chba_rx_chan_switch_indication(hmac_chba_mgmt_info_stru *chba_mgmt_info,
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    uint32_t ret;
    hmac_chba_chan_switch_info_stru *chan_switch_info = NULL;
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_CHBA, "hmac_chba_rx_chan_switch_indication::send CSN confirm fail");
    }

    /* 抛事件给DMAC，配置自己的信道切换 */
    hmac_chba_chan_work_channel_switch(chan_switch_ntf->chan_number, chan_switch_ntf->bandwidth,
 ...
 * 作    者  : x00579508
 */
static void hmac_chba_rx_chan_switch_confirm(hmac_chba_mgmt_info_stru *chba_mgmt_info,
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    uint32_t idx, ret;
    uint8_t device_cnt;
    ...
    if (oal_any_null_ptr4(chba_mgmt_info, chan_switch_ntf, peer_addr, hmac_vap)) {
        return;
    }
    chan_switch_info = &chba_mgmt_info->chan_switch_info;
    /* 处于补救流程，判断是否收集齐ack */
    ...
    uint16_t payload_len)
{
    ...
    if (chan_switch_ntf.notify_type == MAC_CHBA_CHAN_SWITCH_REQ) {
        ...
        hmac_chba_rx_chan_switch_indication(chba_mgmt_info, &chan_switch_ntf, sa_addr);
    } else if (chan_switch_ntf.notify_type == MAC_CHBA_CHAN_SWITCH_CONFIRM) {
        hmac_chba_rx_chan_switch_confirm(chba_mgmt_info, &chan_switch_ntf, sa_addr);
    } else {
...
void hmac_chba_adjust_channel_proc(hmac_vap_stru *hmac_vap, mac_chba_adjust_chan_info *chan_info)
{
    ...
    /* 进信道切换流程前停掉扫描 */
    hmac_config_scan_abort(&hmac_vap->st_vap_base_info, sizeof(oal_uint32), &pedding_data);

    oam_warning_log2(0, OAM_SF_CHBA, "hmac_chba_adjust_channel_proc::target channel number=[%d], bw=[%d].",
        chan_info->chan_number, chan_info->bandwidth);
    target_channel.uc_chan_number = chan_info->chan_number;
    target_channel.en_bandwidth = chan_info->bandwidth;
    /* 建链触发的切信道处理 */
    if (chan_info->switch_type == MAC_CHBA_CHAN_SWITCH_FOR_COEX) {
        /* 启动信道切换流程 */
        hmac_chba_start_switch_chan(mgmt_info, &target_channel, MAC_CHBA_CHAN_SWITCH_IMMEDIATE,
            MAC_CHBA_CHAN_SWITCH_FOR_COEX);
        return;
    }
    /* 卡顿触发的切信道：岛主直接切信道，非岛主先发CSN req帧通知岛主 */
    if (chan_info->switch_type == MAC_CHBA_CHAN_SWITCH_FOR_PERFORMANCE) {
        if (self_conn_info->role == CHBA_MASTER || self_conn_info->role == CHBA_ISLAND_OWNER) {
            /* 启动信道切换流程 */
            hmac_chba_start_switch_chan(mgmt_info, &target_channel, MAC_CHBA_CHAN_SWITCH_IMMEDIATE,
                MAC_CHBA_CHAN_SWITCH_FOR_PERFORMANCE);
            return;
...
void hmac_chba_stub_chan_switch_report_proc(uint8_t *recv_msg)
{
    ...
    /* 抛事件给DMAC，配置自己的信道切换 */
    hmac_chba_chan_work_channel_switch(info_report->target_channel.uc_chan_number,
...
uint32_t hmac_chba_test_chan_switch_cmd(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    if (test_params->switch_chan_num != 0xFF) {
        ...
        /* 填写channel_idx */
        ret = mac_get_channel_idx_from_num(mac_channel.en_band, mac_channel.uc_chan_number,
            &(mac_channel.uc_idx));
        if (ret != OAL_SUCC) {
            return ret;
        }

        hmac_chba_stub_channel_switch_report(&mac_channel);
    }
