...
void mac_set_domain_bssid(uint8_t *domain_bssid, mac_vap_stru *mac_vap, hmac_chba_vap_stru *chba_vap_info)
{
    ...
    if (hmac_chba_get_sync_state() == CHBA_NON_SYNC) {
        ...
        hmac_generate_chba_domain_bssid(domain_bssid, WLAN_MAC_ADDR_LEN * sizeof(uint8_t),
            mac_mib_get_StationID(mac_vap), WLAN_MAC_ADDR_LEN * sizeof(uint8_t));
    } else {
        /* 获取域BSSID */
...
static void hmac_chba_set_device_capability(uint8_t *attr_buf, uint16_t *attr_len)
{
    ...
    attr_buf[index++] = hwifi_get_chba_social_chan_cap();
    ...
    attr_ctrl_info *attr_ctrl)
{
    ...
    /* DTS2022010800778 beacon帧封装vap_bitmap字段所需, 允许通过低功耗等级获取其bitmap配置, 后续该vap_bitmap字段将被删除 */
...
void hmac_chba_encap_pnf_action(hmac_chba_vap_stru *chba_vap_info, uint8_t *pnf_buf, uint16_t *pnf_len)
{
    ...
    /* DTS2022010800778 PNF帧封装vap_bitmap字段所需, 允许通过低功耗等级获取其bitmap配置, 后续该vap_bitmap字段将被删除 */
    vap_bitmap = chba_level_to_bitmap(chba_vap_info->vap_bitmap_level);
    /* 封装bitmap属性(必选) */
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    switch (payload[MAC_CHBA_ACTION_TYPE_LEN]) {
        case MAC_CHBA_PNF:
            hmac_chba_rx_pnf_proc(hmac_vap, sa_addr, WLAN_MAC_ADDR_LEN, domain_bssid, payload + MAC_CHBA_ACTION_HDR_LEN,
        ...
        case MAC_CHBA_CHANNEL_SWITCH_NOTIFICATION:
            hmac_chba_rx_chan_switch_notify_action(hmac_vap, sa_addr, WLAN_MAC_ADDR_LEN,
                payload + MAC_CHBA_ACTION_HDR_LEN,
                payload_len - MAC_CHBA_ACTION_HDR_LEN);
            break;
        case MAC_CHBA_SYNC_REQUEST:
            hmac_chba_rx_sync_request_proc(hmac_vap, sa_addr, WLAN_MAC_ADDR_LEN, payload + MAC_CHBA_ACTION_HDR_LEN,
                payload_len - MAC_CHBA_ACTION_HDR_LEN);
            break;
        case MAC_CHBA_DOMAIN_MERGE:
            hmac_chba_rx_domain_merge_proc(hmac_vap, sa_addr, WLAN_MAC_ADDR_LEN, payload + MAC_CHBA_ACTION_HDR_LEN,
                payload_len - MAC_CHBA_ACTION_HDR_LEN);
            break;
        default:
            break;
    ...
    oal_netbuf_stru *netbuf)
{
    ...
    mac_hdr = (mac_ieee80211_frame_stru *)rx_ctrl->st_rx_info.pul_mac_hdr_start_addr;
...
void hmac_chba_send_sync_beacon(hmac_vap_stru *hmac_vap, uint8_t *bssid, uint8_t addr_len)
{
    ...
    if (hmac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CHBA, "hmac_chba_send_sync_beacon::hmac_vap null.");
        return;
    }
    ...
    *vendor_ie_len_pos = vendor_ie_len;
    ...
    MAC_GET_CB_TX_USER_IDX(tx_ctl) = MAX_TX_USER_IDX;
 ...
 * 作    者  : m00488873
 */
void hmac_chba_send_sync_action(uint8_t *sa_addr, uint8_t *bssid,
    uint8_t action_type, uint8_t request_type)
{
    ...
    if (hmac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CHBA, "hmac_chba_send_sync_action: hmac_vap is null.");
        return;
    }
    mac_vap = &hmac_vap->st_vap_base_info;
    if (sa_addr == NULL) {
        oam_warning_log0(0, OAM_SF_CHBA, "hmac_chba_send_sync_action: sa_addr is null.");
        return;
    }
...
uint32_t hmac_chba_tx_deauth_disassoc_complete(mac_vap_stru *mac_vap, oal_uint8 len, oal_uint8 *param)
{
    ...
    /* DTS2022031405172 chba_kick_user_timer定时器启动，即这是上层发起的去关联。需在发送完成时再进行del user处理 */
