    ...
    mac_chba_set_sync_request *sync_req_params)
{
    ...
    /* DTS2021090723991 若开启硬件过滤, 其他bssid的广播帧将无法收到, 因此现暂定在请求同步前更新一次bssid
        此处根据后续方案决定是否保留 */
...
/*
 * 功能描述  : 检查是否需要通知其他节点切换信道，如果需要进行通知
 * 日    期  : 2021年02月08日
 * 作    者  : m00488873
 */
...
void hmac_chba_sync_process_after_asoc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    /* DTS2022022805196 将新user添加到topo与岛信息中
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    /* DTS2021111819797 低功耗开关定时器pause计数, 启动定时器的同时需要将计数清0, 否则会一直累加 */
