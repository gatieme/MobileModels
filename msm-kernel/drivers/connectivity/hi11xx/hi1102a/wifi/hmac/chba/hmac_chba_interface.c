 ...
 * 作    者  : x00579508
 */
void hmac_chba_stub_channel_switch_report(mac_channel_stru *target_channel)
{
    mac_chba_stub_chan_switch_rpt info_report;
    info_report.target_channel = *target_channel;

    hmac_chba_stub_chan_switch_report_proc((uint8_t *)(&info_report));
}
...
uint8_t hmac_chba_bw_mode_convert(uint8_t band, uint8_t channel_num, uint8_t bw)
{
    ...
    if (band == WLAN_BAND_2G) {
        ...
        total_num = HMAC_CHANNEL_NUM_2G;
    } else {
        candidate_list = g_aus_channel_candidate_list_5g;
        total_num = HMAC_CHANNEL_NUM_5G;
    }
    ...
    for (idx = 0; idx < total_num; idx++) {
        if (candidate_list[idx].uc_chan_number == channel_num) {
            ...
            if ((oal_nl80211_chan_width)WLAN_BANDWIDTH_TO_IEEE_CHAN_WIDTH(bandwidth, OAL_TRUE) == bw) {
                break;
            }
        }
    }
    ...
    mac_channel_stru *mac_channel)
{
    ...
    /* 判断信道号是否合法 */
    err = (int32_t)mac_is_channel_num_valid(mac_channel->en_band, (uint8_t)primer_channel);
    if (err != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    oam_warning_log4(0, OAM_SF_CHBA, "hmac_chba_chan_convert_center_freq_to_bw::channel at hmac: band[%d], \
        uc_chan_idx[%d], uc_chan_number[%d], bandwidth[%d]", mac_channel->en_band, mac_channel->uc_idx,
    ...
    mac_channel_stru *mac_channel, mac_vap_stru *mac_vap)
{
    ...
    kernel_channel->center_freq_20m = (mac_channel->en_band == WLAN_BAND_2G) ?
        (g_ast_freq_map_2g[mac_channel->uc_idx].us_freq) :
        (g_ast_freq_map_5g[mac_channel->uc_idx].us_freq);
