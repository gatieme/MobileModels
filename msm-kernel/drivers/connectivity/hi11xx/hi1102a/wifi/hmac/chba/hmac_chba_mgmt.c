...
void hmac_chba_print_island_info(hmac_chba_island_list_stru *island_info)
{
    ...
    oam_warning_log2(0, OAM_SF_CHBA, "hmac_chba_print_island_info::island cnt %d, aliveDeviceCnt %d",
...
void hmac_chba_print_self_conn_info(void)
{
    ...
    oam_warning_log3(0, OAM_SF_CHBA,
        ...
        self_conn_info->self_device_id, self_conn_info->role,
        self_conn_info->island_owner_valid);
...
/*
 * 功能描述  : CHBA 哈希节点的key值对比函数，key值相同表示找到该节点
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
...
/*
 * 功能描述  : 根据岛信息填写本设备连接信息，输入岛信息中包含岛主信息的case
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
 ...
 * 作    者  : x00579508
 */
void hmac_chba_fill_self_conn_info_with_owner(hmac_chba_per_island_tmp_info_stru *curr_island,
...
static void hmac_chba_check_island_owner_vaild(hmac_chba_per_island_tmp_info_stru *curr_island,
    mac_chba_self_conn_info_stru *self_conn_info, uint8_t owner_device_id, uint8_t device_cnt)
{
    uint32_t i;

    if (owner_device_id == UINT8_INVALID_VALUE) {
...
/*
 * 功能描述  : 根据岛信息填写本设备连接信息，输入岛信息中不包含岛主信息的case
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
    ...
    mac_chba_self_conn_info_stru *self_conn_info)
{
    ...
    if (self_conn_info->island_owner_valid == TRUE) {
        ...
        hmac_chba_check_island_owner_vaild(curr_island, self_conn_info, owner_device_id, device_cnt);
    }
...
/*
 * 功能描述  : 更新活跃设备表中一个设备的信息，返回device id
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
uint8_t hmac_chba_one_dev_update_alive_dev_table(const uint8_t* mac_addr)
 ...
 * 作    者  : x00579508
 */
uint32_t hmac_chba_del_non_alive_device(mac_vap_stru *mac_vap, oal_uint8 len, oal_uint8 *param)
{
    uint8_t device_id;
    hmac_chba_hash_node_stru cmp;
    struct hi_node *node = NULL;
...
/*
 * 功能描述  : 根据PNF或beacon的payload更新设备管理信息，返回帧发送方的device id
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
uint8_t hmac_chba_update_dev_mgmt_info_by_frame(uint8_t *payload, uint16_t len,
    uint8_t *peer_addr, int8_t *rssi, uint8_t *linkcnt, uint8_t sa_is_my_user)
{
    uint8_t *data = NULL;
    uint8_t i, chan_number, bandwidth, device_id, conn_device_id;
    int16_t offset;
    ...
    if (MAC_CHBA_ATTR_HDR_LEN + data[MAC_CHBA_ATTR_ID_LEN] < MAC_CHBA_LINK_INFO_LINK_CNT_POS + 1 ||
        ...
        OAM_ERROR_LOG0(0, OAM_SF_CHBA, "hmac_chba_update_dev_mgmt_info_by_frame::link info len invalid.");
        return UINT8_INVALID_VALUE;
    }
    ...
    *linkcnt = data[offset];
    ...
    for (i = 0; i < *linkcnt; i++) {
        if (offset < 0 || offset >= MAC_CHBA_ATTR_HDR_LEN + data[MAC_CHBA_ATTR_ID_LEN]) {
            break;
        }
        ...
        if (conn_device_id >= MAC_CHBA_MAX_DEVICE_NUM ||
            (conn_device_id == hmac_chba_get_own_device_id() && sa_is_my_user == OAL_FALSE)) {
            continue;
        }
...
/*
 * 功能描述  : 查找rssi最大的非保活设备，排除指定的设备。两种使用场景:1)不排除原岛主，2)排除原岛主
 * 日    期  : 2021年09月07日
 * 作    者  : x00579508
 */
...
{
    uint8_t max_rssi_device_id = UINT8_INVALID_VALUE;
    int8_t max_pnf_rssi;
    ...
    /* DTS2022052001978 CHBA 1.0为一域单岛方案，不应走入根据rssi选岛主的流程。后续开发一域多岛方案则删除 */
...
/*
 * 功能描述  : 更新某个已存在的岛的岛主，有变更则更新，无变更则维持原岛主
 * 日    期  : 2021年09月07日
 * 作    者  : x00579508
 */
...
{
    uint8_t max_rssi_device_id = UINT8_INVALID_VALUE;
    uint8_t island_owner_device_id, island_owner_change;
...
void hmac_chba_encap_island_owner_attr(hmac_chba_island_list_stru *tmp_island_info)
{
    ...
    hi_list_for_each_safe(chba_node, tmp, &tmp_island_info->island_list) {
        if (index < 0 || index >= MAC_CHBA_MGMT_MID_PAYLOAD_BYTES) {
            break;
        }
    ...
    /* 设置Attr Length字段 */
    attr_buf[MAC_CHBA_ATTR_ID_LEN] = (uint8_t)(index - MAC_CHBA_ATTR_HDR_LEN);
    /* 设置island owner cnt字段 */
    attr_buf[MAC_CHBA_ATTR_HDR_LEN] = island_owner_cnt;

    /* 保存attr_buf的长度 */
    chba_mgmt_info->island_owner_attr_len = (uint8_t)index;
}
    ...
    uint16_t payload_len)
{
    ...
    for (island_idx = 0; island_idx < island_cnt; island_idx++) {
        if (index < 0 || index >= MAC_CHBA_ATTR_HDR_LEN + attr_pos[MAC_CHBA_ATTR_ID_LEN]) {
            break;
        }
...
static void hmac_chba_init_device_mgmt_info(mac_chba_vap_start_rpt *info_report)
{
    ...
    ret = memcpy_s(device_id_info[device_id].mac_addr,  OAL_MAC_ADDR_LEN,
        info_report->mac_addr, OAL_MAC_ADDR_LEN);
    if (ret != 0) {
...
/*
 * 功能描述  : CHBA收到更新网络拓扑信息的处理：一般是device收到beacon、pnf并上报后
 * 日    期  : 2021年02月24日
 * 作    者  : x00579508
 */
