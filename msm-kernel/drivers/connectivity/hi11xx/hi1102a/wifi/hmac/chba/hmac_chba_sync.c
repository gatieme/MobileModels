...
uint32_t hmac_chba_try_once_sync_request(hmac_chba_sync_info *sync_info, sync_info_flags *sync_flags,
    sync_request *sync_req_ctrl);
void hmac_chba_domain_merge_sync_handler(hmac_vap_stru *hmac_vap, hmac_chba_sync_info *sync_info);
void hmac_chba_sync_listen_timeout(void);
oal_uint32 hmac_chba_sync_wait_timeout(oal_void *p_arg);
/* 4 函数实现 */
...
void hmac_chba_sync_module_init(hmac_chba_device_para_stru *device_info)
{
    ...
    if (sync_info->sync_waiting_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&sync_info->sync_waiting_timer);
    }
    if (sync_info->domain_merge_waiting_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&sync_info->domain_merge_waiting_timer);
    }
    if (sync_info->domain_merge_next_timer.en_is_registerd) {
        frw_immediate_destroy_timer(&sync_info->domain_merge_next_timer);
    }
...
static void hmac_chba_sync_own_rp_info(hmac_chba_sync_info *sync_info)
{
    ...
    hmac_config_send_event(mac_vap, WLAN_CFGID_CHBA_OWN_RP_SYNC, OAL_SIZEOF(ranking_priority),
        (oal_uint8 *)&sync_info->own_rp_info);
}
...
/*
 * 功能描述  : 当SyncRequestList为空，且有链路时，填充SyncRequestList
 * 日    期  : 2021年03月05日
 * 作    者  : m00488873
 */
    ...
    sync_request *sync_req_ctrl)
{
    ...
    if (oal_any_null_ptr3(sync_info, sync_req_ctrl, own_addr)) {
        return OAL_FAIL;
    }
 ...
 * 作    者  : m00488873
 */
void hmac_chba_start_sync_request(hmac_chba_sync_info *sync_info)
{
    ...
    /* 尝试一次主动同步请求 */
    if (hmac_chba_try_once_sync_request(sync_info, sync_flags, sync_req_ctrl) != OAL_SUCC) {
    ...
    sync_request *sync_req_ctrl)
{
    ...
    if (sync_info == NULL || sync_flags == NULL || sync_req_ctrl == NULL || hmac_vap == NULL) {
...
oal_uint32 hmac_chba_sync_wait_timeout(oal_void *p_arg)
{
    ...
    if (p_timer->en_is_registerd) {
        frw_immediate_destroy_timer(p_timer);
    }
    ...
    /* 尝试下一次主动同步请求 */
    if (hmac_chba_try_once_sync_request(sync_info, &sync_info->sync_flags, sync_req_ctrl) != OAL_SUCC) {
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    hmac_generate_chba_domain_bssid(peer_domain_bssid, WLAN_MAC_ADDR_LEN * sizeof(uint8_t),
        master_addr, WLAN_MAC_ADDR_LEN * sizeof(uint8_t));
 ...
 * 作    者  : m00488873
 */
oal_uint32 hmac_chba_domain_merge_sync_wait_timeout(oal_void *p_arg)
{
    hmac_chba_sync_info *sync_info = hmac_chba_get_sync_info();
    hmac_chba_domain_merge_info *domain_merge = hmac_chba_get_domain_merge_info();
 ...
 * 作    者  : m00488873
 */
oal_uint32 hmac_chba_domain_merge_resp_wait_timeout(oal_void *p_arg)
{
    hmac_chba_sync_info *sync_info = hmac_chba_get_sync_info();
    hmac_chba_domain_merge_info *domain_merge = hmac_chba_get_domain_merge_info();
    frw_timeout_stru *p_timer = &sync_info->domain_merge_waiting_timer;
...
/*
 * 功能描述  : 下一个slot到期处理函数，继续发beacon帧
 * 日    期  : 2021年03月15日
 * 作    者  : m00488873
 */
oal_uint32 hmac_chba_next_slot_timeout(oal_void *p_arg)
{
    hmac_chba_sync_info *sync_info = hmac_chba_get_sync_info();
    hmac_chba_domain_merge_info *domain_merge = hmac_chba_get_domain_merge_info();
...
void hmac_chba_multiple_domain_detection_after_asoc(uint8_t *sa_addr, chba_req_sync_after_assoc_stru *req_sync)
{
    ...
    if (hmac_chba_sync_dev_rp_compare(cur_master, &another_master)) {
...
uint32_t hmac_chba_sync_multiple_domain_info(mac_vap_stru *mac_vap, oal_uint8 len, oal_uint8 *puc_param)
{
    hmac_chba_sync_info *sync_info = hmac_chba_get_sync_info();
    ...
    if (hmac_chba_sync_dev_rp_compare(cur_master, &multi_domain->another_master)) {
    ...
    uint8_t update_flag)
{
    ...
    if (sync_info == NULL || new_master == NULL || hmac_vap == NULL) {
 ...
 * 作    者  : m00488873
 */
uint32_t hmac_chba_sync_become_master_handler(hmac_chba_sync_info *sync_info, uint8_t rrm_list_flag)
{
    uint8_t *own_addr = NULL;
    int32_t err;
...
int32_t hmac_chba_sync_become_island_owner_handler(hmac_chba_sync_info *sync_info)
{
    ...
    if (sync_info == NULL || hmac_vap == NULL) {
 ...
 * 作    者  : m00488873
 */
uint32_t hmac_chba_update_master_election_info(mac_vap_stru *mac_vap, oal_uint8 len, oal_uint8 *param)
{
    hmac_chba_sync_info *sync_info = hmac_chba_get_sync_info();
