...
/*
 * 函 数 名  : hmac_init_user_11v_ctl_info
 * 功能描述  : 每次收到bsst_req帧时，初始化11v漫游信息
 * 修改历史  :
 * 1.日    期  : 2018年7月1日
 *   作    者  : l00422505
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_11v_bsst_req_filter
 * 功能描述  : 兼容性11v AP识别，连续若干次失败后，开启dmac过滤11v bsstreq功能
 * 修改历史  :
 * 1.日    期  : 2018年7月1日
 *   作    者  : l00422505
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_rx_bsst_req_action_free_res
 * 功能描述  : 释放已申请的内存
 * 修改历史  :
 * 1.日    期  : 2018年4月7日
 *   作    者  : t00422961
...
/*
 * 函 数 名  : hmac_11v_check_invalid_bss_in_neighbor_list
 * 功能描述  : 过滤neighbor list中的无效候选BSS
 * 修改历史  :
 * 1.日    期  : 2019年10月26日
 *   作    者  : y00452465
 *   修改内容:新生成函数
...
/*
 * 函 数 名: hmac_rx_bsst_is_rejected
 * 功能描述:判断是否需要拒绝接收到的bsst帧
 * 修改历史:
 * 1.日 期:2019年7月23日
 * 作    者:y00452465
 * 修改内容:新生成函数
...
/*
 * 函 数 名  : hmac_11v_should_single_channel_scan
 * 功能描述  : 11v应该单信道扫描
 * 修改历史     :
 * 1.日    期   : 2019年12月23日
 *   作    者   : h00368777
...
{
    pst_req_info->st_request_mode.bit_candidate_list_include = puc_data[3] & BIT0; /* puc_data[3]第0bit */
    pst_req_info->st_request_mode.bit_abridged = (puc_data[3] & BIT1) ? OAL_TRUE : OAL_FALSE; /* puc_data[3]第1bit */
    /* puc_data[3]第2bit */
    ...
    oal_uint16 *pus_handle_len, oal_uint16 us_frame_len)
{
    ...
    if ((pst_req_info->st_request_mode.bit_termination_include) &&
        (us_frame_len >= (*pus_handle_len) + MAC_IE_HDR_LEN + HMAC_11V_TERMINATION_TSF_LENGTH + 2)) { /* 2表示duration占的字节数 */
        (*pus_handle_len) += MAC_IE_HDR_LEN; /* 去掉元素头 */
        ...
        (*pus_handle_len) += 2; /* 2表示duration已经处理 */
    }
}
    ...
    hmac_bsst_req_info_stru *pst_req_info, hmac_bsst_rsp_info_stru *pst_bsst_rsp_info, oal_uint32 ul_beacon_period)
{
    ...
    pst_bsst_rsp_info->uc_status_code = 0;       /* 默认设置为同意切换 */
    pst_bsst_rsp_info->uc_termination_delay = 0; /* 仅当状态码为5时有效，此次无意义设为0 */
...
/*
 * 函 数 名  : hmac_get_bss_num_from_neighbor_list
 * 功能描述  : 统计Neighbor report IE数量
 * 修改历史  :
 * 1.日    期  : 2019年10月26日
 *   作    者  : y00452465
 *   修改内容:新生成函数
...
/*
 * 函 数 名  : hmac_11v_handle_bssid_info
 * 功能描述  : 解析解析Neighbor report ie中的BSSID INFO及保留字段
 * 修改历史  :
 * 1.日    期  : 2019年10月26日
 *   作    者  : y00452465
 *   修改内容:新生成函数
...
/*
 * 函 数 名  : hmac_11v_handle_subelement
 * 功能描述  : 解析Neighbor report ie中的Subelement
 * 修改历史  :
 * 1.日    期  : 2019年10月26日
 *   作    者  : y00452465
 *   修改内容:新生成函数
                                               ...
                                               hmac_neighbor_bss_info_stru *pst_neighbor_list_info)
{
    ...
    while (s_sub_ie_len > 0) {
        switch (puc_ie_data[0]) {
            ...
            default:
            {
                s_sub_ie_len -= (oal_int16)(puc_ie_data[1] + MAC_IE_HDR_LEN);
...
/*
 * 函 数 名  : hmac_select_valid_bss_from_neighbor_list
 * 功能描述  : 遍历neighbor list查找目标ap
 * 修改历史  :
 * 1.日    期  : 2019年10月26日
 *   作    者  : y00452465
 *   修改内容:新生成函数
...
/*
 * 函 数 名  : hmac_get_target_bss_from_neighbor_list
 * 功能描述  : 从数据中解析出Neighbor Report List
 * 修改历史  :
 * 1.日    期  : 2017年3月14日
 *   作    者  : shichongfu
 *  修改内容   : 新生成函数
