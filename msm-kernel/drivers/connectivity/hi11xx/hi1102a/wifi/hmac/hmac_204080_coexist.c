                                             ...
                                             hmac_scan_record_stru *pst_scan_record)
{
    ...
    /* 获取主信道、次信道中心频点 */
    ul_pri_freq = (oal_uint32)g_ast_freq_map_2g[pst_mac_vap->st_channel.uc_chan_number - 1].us_freq;
    if (pst_mac_vap->st_channel.en_bandwidth == WLAN_BAND_WIDTH_40PLUS) {
    ...
    oal_dlist_search_for_each(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        ul_pri = (oal_uint32)g_ast_freq_map_2g[pst_bss_dscr->st_channel.uc_chan_number - 1].us_freq;
        ul_sec = ul_pri;
...
/*
 * 函 数 名  : hmac_ap_up_rx_should_ingore_2040
 * 功能描述  : 判断是否忽略20/40共存
 * 修改历史      :
 * 1.日    期   : 2019年10月17日
 *   作    者   : h00368777
...
oal_void hmac_rx_notify_channel_width(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 根据发送端地找到对应AP USER结构 */
    pst_hmac_user = mac_vap_get_hmac_user_by_addr(pst_mac_vap, auc_sta_addr, WLAN_MAC_ADDR_LEN);
...
{
    /* DTS2018062802652:如果STA或者AP不支持HT且不支持VHT，则直接返回 */
    if (((mac_mib_get_HighThroughputOptionImplemented(pst_mac_vap) == OAL_FALSE) &&
