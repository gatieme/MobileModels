...
#define HMAC_TX_BUF_BITMAP_LOG2_WORD_SIZE 5 /* log2(32)等于5 */
...
#define hmac_ba_is_set(_bm, _n) (((_n) < (HMAC_BA_BMP_SIZE)) && \
                                ((_bm)[(_n) >> 5] & (1 << ((_n)&31))))
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_ba_isset(hmac_ba_rx_stru *pst_ba_rx_hdl, oal_uint16 us_seqno)
{
    ...
    if (hmac_baw_within(pst_ba_rx_hdl->us_baw_head, HMAC_BA_BMP_SIZE, us_seqno)) {
        ...
        if (hmac_ba_is_set(pst_ba_rx_hdl->aul_rx_buf_bitmap, us_index)) {
            return OAL_TRUE;
        }
    }
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void hmac_ba_update_rx_bitmap(hmac_user_stru *pst_hmac_user,
                                                        mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    while (hmac_ba_seqno_lt(pst_ba_rx_hdl->us_baw_head,
                            DMAC_BA_SEQNO_SUB(pst_ba_rx_hdl->us_baw_start, (HMAC_BA_BMP_SIZE - 1))) == OAL_TRUE) {
...
/*
 * 函 数 名  : hmac_tcp_ack_bitmap_index_get
 * 功能描述  : 获取seq num对应的bitmap序列号(host侧bitmap为2个64bit的数组)
 * 修改历史  :
 * 1.日    期  : 2019年3月22日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_uint8 hmac_tcp_ack_bitmap_index_get(oal_uint16 us_seq_num)
{
    oal_uint8 uc_rx_bitmap_size = WLAN_AMPDU_RX_BUFFER_SIZE * 2 - 1; /* 获取bitmap序列号,host侧bitmap为2个64bit的数组 */
...
/*
 * 函 数 名  : hmac_tcp_ack_reorder_index_get
 * 功能描述  : 获取seq num对应的reorder队列index
 * 修改历史  :
 * 1.日    期  : 2019年3月22日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_uint8 hmac_tcp_ack_reorder_index_get(oal_uint16 us_seq_num)
{
    return us_seq_num & (WLAN_AMPDU_RX_BUFFER_SIZE - 1);
}
...
OAL_STATIC OAL_INLINE hmac_rx_buf_stru *hmac_remove_frame_from_reorder_q(
    hmac_ba_rx_stru *pst_ba_rx_hdl, oal_uint16 us_seq_num, oal_bool_enum_uint8 *pen_tcp_ack_filtered)
{
    ...
    pst_rx_buff = &(pst_ba_rx_hdl->ast_re_order_list[uc_reorder_index]);
    *pen_tcp_ack_filtered = OAL_FALSE;
    ...
    if (pst_rx_buff->en_tcp_ack_filtered[uc_bitmap_index]) {
        *pen_tcp_ack_filtered = OAL_TRUE;
        ...
        if (pst_rx_buff->in_use && (pst_rx_buff->us_seq_num == us_seq_num)) {
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE hmac_rx_buf_stru *hmac_get_frame_from_reorder_q(
    hmac_ba_rx_stru *pst_ba_rx_hdl, oal_uint16 us_seq_num)
{
    oal_uint16        us_idx;
    hmac_rx_buf_stru *pst_rx_buff;
...
OAL_STATIC OAL_INLINE oal_void hmac_ba_del_lut_index(oal_uint8 *puc_ba_lut_index_table,
...
extern oal_uint8 hmac_mgmt_check_set_rx_ba_ok(hmac_vap_stru *pst_hmac_vap,
                                              hmac_user_stru *pst_hmac_user,
                                              hmac_ba_rx_stru *pst_ba_rx_info,
                                              mac_device_stru *pst_device);

extern oal_void hmac_reorder_ba_rx_buffer_bar(hmac_ba_rx_stru *pst_rx_ba,
                                              oal_uint16 us_start_seq_num,
                                              mac_vap_stru *pst_vap);
extern oal_uint32 hmac_ba_reset_rx_handle(mac_device_stru *pst_mac_device,
                                          hmac_ba_rx_stru **ppst_rx_ba,
                                          oal_uint8 uc_tid,
                                          oal_bool_enum_uint8 en_is_aging);
extern oal_void hmac_up_rx_bar(hmac_vap_stru *pst_hmac_vap,
                               dmac_rx_ctl_stru *pst_rx_ctl,
                               oal_netbuf_stru *pst_netbuf);
extern oal_bool_enum_uint8 hmac_is_device_ba_setup(void);
extern oal_void hmac_ba_update_reorder_queue(hmac_ba_rx_stru *pst_ba_rx_hdl, mac_tcp_ack_record_stru *pst_tcp_ack_info);
