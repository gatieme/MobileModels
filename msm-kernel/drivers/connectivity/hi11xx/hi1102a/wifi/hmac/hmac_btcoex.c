...
OAL_STATIC oal_uint32 hmac_delba_send_timeout(oal_void *p_arg)
{
    ...
    if (oal_atomic_read(&(pst_hmac_btcoex_arp_req_process->ul_rx_unicast_pkt_to_lan)) == 0) {
...
oal_void hmac_btcoex_arp_fail_delba_process(oal_netbuf_stru *pst_netbuf, mac_vap_stru *pst_mac_vap)
{
    ...
    if ((pst_hmac_user_btcoex->uc_ba_size > 0) && (pst_hmac_user_btcoex->uc_ba_size < WLAN_AMPDU_RX_BA_LUT_WSIZE)) {
        ...
        if ((uc_data_type == MAC_DATA_ARP_REQ) &&
            ...
            frw_create_timer(&(pst_hmac_btcoex_arp_req_process->st_delba_opt_timer),
...
/*
 * 函 数 名  : hmac_btcoex_check_by_ba_size
 * 功能描述  : 检查共存场景下BA SIZE
 * 修改历史  :
 * 1.日    期  : 2016年4月12日
 *   作    者  : g00306640
...
/*
 * 函 数 名  : hmac_btcoex_process_btble_status
 * 功能描述  : 处理BT状态发生的变化
 * 修改历史  :
 * 1.日    期  : 2019年3月28日
 *   作    者  : r00356856
...
/*
 * 函 数 名  : hmac_btcoex_ap_tpye_identify
 * 功能描述  : 共存场景下兼容性问题AP识别
 * 修改历史  :
 * 1.日    期  : 2019年4月3日
 *   作    者  : r00356856
...
/*
 * 函 数 名  : hmac_btcoex_process_exception_ap
 * 功能描述  : 共存场景下处理识别出来的兼容性问题AP
 * 修改历史  :
 * 1.日    期  : 2019年4月12日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
