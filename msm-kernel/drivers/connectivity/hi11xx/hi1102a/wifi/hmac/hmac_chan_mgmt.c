...
uint32_t hmac_report_channel_switch(hmac_vap_stru *hmac_vap, mac_channel_stru *channel)
{
    ...
    if (mac_is_channel_num_valid(channel->en_band, channel->uc_chan_number) != OAL_SUCC) {
    ...
    if (mac_is_channel_idx_valid(channel->en_band, channel->uc_idx) != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_CHAN, "hmac_report_channel_switch::invalid channel idx, idx=%d", channel->uc_idx);
        return OAL_FAIL;
    }

    chan_switch.center_freq = (channel->en_band == WLAN_BAND_2G) ?
        g_ast_freq_map_2g[channel->uc_idx].us_freq : g_ast_freq_map_5g[channel->uc_idx].us_freq;
                                                        ...
                                                        oal_uint8 uc_pri_chan_idx)
{
    ...
    (uc_pri_chan_idx + uc_affected_ch_idx_offset) : (uc_num_supp_chan > 0) ? (uc_num_supp_chan - 1) : 0;
                                                             ...
                                                             oal_uint8 *puc_end_ch_idx)
{
    ...
    if (mac_device_is_auto_chan_sel_enabled(pst_mac_device) == OAL_TRUE) {
        ...
        *puc_start_ch_idx = 0;
        *puc_end_ch_idx = (uc_num_supp_chan > 0) ? (uc_num_supp_chan - 1) : 0;
...
oal_uint32 hmac_start_bss_in_available_channel(hmac_vap_stru *pst_hmac_vap)
{
    ...
    mac_vap_set_bssid(&pst_hmac_vap->st_vap_base_info,
        pst_hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_sta_config.auc_dot11StationID, WLAN_MAC_ADDR_LEN);
...
#endif
    // 上报supplicant
    ul_ret = hmac_report_channel_switch(pst_hmac_vap, &pst_hmac_vap->st_vap_base_info.st_channel);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHAN,
            "hmac_chan_switch_to_new_chan_complete::hmac_report_channel_switch return fail. ret[%d]", ul_ret);
    }
