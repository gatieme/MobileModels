...
OAL_STATIC oal_uint32 hmac_crypto_tkip_michael_mic(oal_uint8 *puc_key,
    ...
    oal_uint8 auc_mic[IEEE80211_WEP_MICLEN], uint32_t mic_len)
{
    ...
    ul_msb = get_le32(puc_key, 4);
    ul_lsb = get_le32(puc_key + 4, 4); /* puc_key偏移4字节后取4个字节以小端模式组成32位整数赋值给ul_lsb */

    /* Michael MIC pseudo header: DA, SA, 3 x 0, Priority */
    ul_msb ^= get_le32(auc_hdr, 4);
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[4], 4); /* auc_hdr偏移4字节后取4个字节以小端模式组成32位整数与ul_msb按位异或 */
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[8], 4); /* auc_hdr偏移8字节后取4个字节以小端模式组成32位整数与ul_msb按位异或 */
    michael_block(ul_msb, ul_lsb);
    ul_msb ^= get_le32(&auc_hdr[12], 4); /* auc_hdr偏移12字节后取4个字节以小端模式组成32位整数与ul_msb按位异或 */
    michael_block(ul_msb, ul_lsb);
...
oal_uint32 hmac_crypto_tkip_enmic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    ul_ret = hmac_crypto_tkip_michael_mic(puc_tx_mic_key, pst_netbuf, pst_cb->pst_frame_header, ul_hdrlen,
        ul_pktlen - ul_hdrlen, auc_mic, IEEE80211_WEP_MICLEN);
    if (ul_ret != OAL_SUCC) {
        return ul_ret;
    }

    /* 4.1 拷贝mic到帧尾部 */
    if (memcpy_s(puc_mic_tail, ul_tailroom, auc_mic, IEEE80211_WEP_MICLEN) != EOK) {
...
oal_uint32 hmac_crypto_tkip_demic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    ul_ret = hmac_crypto_tkip_michael_mic(puc_rx_mic_key, pst_netbuf,
        ...
        ul_hdrlen, ul_pktlen - (ul_hdrlen + IEEE80211_WEP_MICLEN), auc_mic, IEEE80211_WEP_MICLEN);
    if (ul_ret != OAL_SUCC) {
        return ul_ret;
    }
