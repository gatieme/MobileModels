...
oal_uint16 g_wlan_assoc_user_max_num = 32; /* 关联用户的最大个数 Root AP模式下为32个,Repeater模式下是15个 */
oal_uint16 hmac_get_wlan_assoc_user_max_num(oal_void)
{
    return g_wlan_assoc_user_max_num;
}
oal_void hmac_set_wlan_assoc_user_max_num(oal_uint16 assoc_user_max_num)
{
    g_wlan_assoc_user_max_num = assoc_user_max_num;
}
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC oal_uint32 oam_cfg_restore_all_item(oal_int32 al_default_cfg_data[], oal_uint32 len)
{
    oal_uint32 ul_loop;
 ...
 */
    ...
    for (ul_loop = 0; ul_loop < OAM_CFG_TYPE_BUTT; ul_loop++) {
        ...
        if (l_ret != OAL_SUCC) {
            ...
            oam_cfg_restore_all_item(al_default_cfg_data, OAM_CFG_TYPE_BUTT);
            oal_free(puc_plaintext);
            oal_free(puc_ciphertext);
...
/*
 函 数 名  : hmac_chr_wifi_ext_info_completion
 功能描述  : 将查询到的数据赋值到chr上报结构体中
 修改历史      :
  1.日    期   : 2019年8月12日
    作    者   : x00456388
    修改内容   : 新生成函数
                                                      ...
                                                      chr_wifi_ext_info_stru *pst_wifi_ext_info)
{
    ...
    pst_wifi_ext_info->en_tas_state = (oal_uint8)board_get_wifi_tas_gpio_state();
...
/*
 函 数 名  : hmac_chr_get_wifi_ext_info_from_device
 功能描述  : chr查询额外wifi信息函数入口
 修改历史      :
  1.日    期   : 2019年8月12日
    作    者   : x00456388
    修改内容   : 新生成函数
...
oal_void hmac_chr_get_vap_info(mac_vap_stru *pst_mac_vap, mac_device_stru *pst_mac_device,
    hmac_chr_vap_info_stru *pst_chr_vap_info)
{
...
oal_uint32 hmac_chr_get_chip_info(oal_uint32 chr_event_id)
{
    ...
    for (uc_vap_index = 0; uc_vap_index < pst_mac_device->uc_vap_num; uc_vap_index++) {
        ...
        if (IS_LEGACY_STA(pst_mac_vap)) {
            ...
            hmac_chr_get_vap_info(pst_mac_vap, pst_mac_device, &st_hmac_chr_info.st_vap_info);
...
/*
 * 函 数 名  : hmac_get_bt_info
 * 功能描述  : 为了不在device侧打patch,从host侧获取额外的wifi信息
 * 修改历史  :
 * 1.日    期  : 2019年6月14日
 *   作    者  : shaoping
 *   修改内容  : 新生成函数
...
oal_void hmac_chr_trigger_roam(mac_vap_stru  *pst_mac_vap)
{
    ...
    if (memcpy_s(pst_hmac_vap->st_20m_ap_info.auc_bssid, OAL_MAC_ADDR_LEN, pst_mac_vap->auc_bssid, OAL_MAC_ADDR_LEN) !=
        EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_chr_trigger_roam:memcpy_s fail!");
        return;
    }
