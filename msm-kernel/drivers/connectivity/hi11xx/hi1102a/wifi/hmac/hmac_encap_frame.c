                                   ...
                                   oal_uint16 us_trans_id)
{
    ...
    /* us_trans_id的低8位赋值(us_trans_id第1字节，puc_data[MAC_80211_FRAME_LEN + 2]) */
    puc_data[MAC_80211_FRAME_LEN + 2] = (us_trans_id & 0x00FF);
    /* us_trans_id的高8位赋值(us_trans_id第2字节，puc_data[MAC_80211_FRAME_LEN + 3]) */
...
oal_uint16 hmac_encap_sa_query_rsp(mac_vap_stru *pst_mac_vap, oal_uint8 *pst_hdr, oal_uint8 *puc_data)
{
    ...
    /* sa query resp复制sa query req的第2字节 */
    puc_data[MAC_80211_FRAME_LEN + 2] = pst_hdr[MAC_80211_FRAME_LEN + 2];
    /* sa query resp复制sa query req的第3字节 */
    puc_data[MAC_80211_FRAME_LEN + 3] = pst_hdr[MAC_80211_FRAME_LEN + 3];

    us_len = MAC_80211_FRAME_LEN + MAC_SA_QUERY_LEN;
    return us_len;
}
#define UPVAP2STAID pst_up_vap2->pst_mib_info->st_wlan_mib_sta_config.auc_dot11StationID
#define MACVAPID    pst_mac_vap->pst_mib_info->st_wlan_mib_sta_config.auc_dot11StationID
/*
 ...
 *   修改内容  : 新生成函数
 */
oal_uint16 hmac_mgmt_encap_deauth(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, oal_uint8 *puc_da,
                                  oal_uint16 us_err_code)
{
    ...
    if (us_err_code & MAC_SEND_TWO_DEAUTH_FLAG) {
        ...
        if (oal_unlikely(pst_mac_device == OAL_PTR_NULL)) {
            us_deauth_len = 0;
            OAM_ERROR_LOG1(0, OAM_SF_ANY, "{mgmt_encap_deauth::pst_mac_device[%d] null!}", pst_mac_vap->uc_device_id);
        ...
        if (ul_ret == OAL_SUCC) {
            ...
            if (pst_up_vap2->pst_mib_info == OAL_PTR_NULL) {
                us_deauth_len = 0;
                OAM_ERROR_LOG0(pst_up_vap2->uc_vap_id, OAM_SF_AUTH, "mgmt_encap_deauth: pst_up_vap2 mib ptr null.");
                return us_deauth_len;
            }
            oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address2, UPVAP2STAID);
            oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address3, pst_up_vap2->auc_bssid);
        }

        OAM_WARNING_LOG1(0, OAM_SF_AUTH, "hmac_mgmt_encap_deauth: send the second deauth frame. error code:%d",
                         us_err_code);
    } else
#endif
    {
        ...
        oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address2, MACVAPID);
        oal_set_mac_addr(((mac_ieee80211_frame_stru *)puc_data)->auc_address3, pst_mac_vap->auc_bssid);
    }
    /*************************************************************************/
    /* Set the contents of the frame body */
    ...
    puc_data[MAC_80211_FRAME_LEN + 1] = (us_err_code & 0xFF00) >> 8; /* 取us_err_code高8位 */
 ...
 *   修改内容  : 新生成函数
 */
oal_uint16 hmac_mgmt_encap_disassoc(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, oal_uint8 *puc_da,
                                    oal_uint16 us_err_code)
{
...
oal_uint16 hmac_encap_notify_chan_width(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_data, oal_uint8 *puc_da)
{
    ...
    /* puc_data的MAC_80211_FRAME_LEN + 2byte代表en_bandwidth */
