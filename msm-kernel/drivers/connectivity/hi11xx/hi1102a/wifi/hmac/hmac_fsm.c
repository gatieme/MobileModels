...
oal_void hmac_fsm_change_state(hmac_vap_stru *pst_hmac_vap, mac_vap_state_enum_uint8 en_vap_state)
{
    ...
    ul_ret = hmac_config_vap_state_syn(&(pst_hmac_vap->st_vap_base_info), sizeof(en_vap_state),
                                       (oal_uint8 *)(&en_vap_state));
    if (ul_ret != OAL_SUCC) {
    ...
    /* STA 模式下只有UP状态才开启低功耗开关定时器,
       对于从漫游状态切换到UP状态，此时不起这个定时器DTS2019072901740 */
    if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        if ((en_vap_state == MAC_VAP_STATE_UP) && (en_old_state != MAC_VAP_STATE_ROAMING)) {
            frw_create_timer(&(pst_hmac_vap->st_ps_sw_timer),
...
/*
 * 函 数 名  : hmac_fsm_init_sta_rx_mgmt
 * 功能描述  : 初始化sta接收管理帧状态机函数表
 * 修改历史  :
 * 1.日    期  : 2019年12月10日
 *   作    者  : y00452465
 *   修改内容  : 封装函数
 */
oal_void hmac_fsm_init_sta_rx_mgmt(oal_void)
{
    oal_uint32 ul_state;
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_STA_JOIN_COMP][HMAC_FSM_INPUT_RX_MGMT] = hmac_sta_wait_beacon_before_auth;
...
OAL_STATIC oal_void hmac_fsm_init_sta(oal_void)
{
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_STA_NBFH_COMP][HMAC_FSM_INPUT_AUTH_REQ] = hmac_sta_wait_auth;
 ...
 *   修改内容  : 新生成函数
 */
oal_uint32 hmac_fsm_call_func_ap(hmac_vap_stru *pst_hmac_vap, hmac_fsm_input_type_enum_uint8 en_input,
                                 oal_void *p_param)
{
 ...
 *   修改内容  : 新生成函数
 */
oal_uint32 hmac_fsm_call_func_sta(hmac_vap_stru *pst_hmac_vap, hmac_fsm_input_type_enum_uint8 en_input,
                                  oal_void *p_param)
{
    if (oal_unlikely(pst_hmac_vap == OAL_PTR_NULL)) {
