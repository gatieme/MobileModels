...
/*
 * 函 数 名  : hmac_netlink_location_send
 * 功能描述  : 定位事件上报平台
 * 修改历史  :
 * 1.日    期  : 2017年11月3日
 *   作    者  : wwx408792
    ...
    switch (pst_location_event->uc_location_type) {
        ...
        case MAC_HISI_LOCATION_CSI_IE:
            ...
            if ((puc_payload[0] == 0) &&
                ...
                /* Type 4Bytes */
                *(oal_uint32 *)&puc_send_csi_buf[l_index] = (oal_uint32)2; /* 2赋值给types */
                l_index += 4; /* 跳过Type 4Bytes */

                /* len 4Bytes */
                pul_len = (oal_uint32 *)&puc_send_csi_buf[l_index];
                l_index += 4;  /* 跳过len 4Bytes */

                /* mac1 6Bytes */
                l_ret += memcpy_s(&puc_send_csi_buf[l_index], HMAC_CSI_SEND_BUF_LEN - l_index,
                                  pst_location_event->auc_mac_server, WLAN_MAC_ADDR_LEN);
                l_index += WLAN_MAC_ADDR_LEN;
                ...
                /* timestamp23Bytes */
                l_index += snprintf_s(puc_send_csi_buf + l_index, HMAC_CSI_SEND_BUF_LEN - l_index,
                    HMAC_FTM_SEND_BUF_LEN - l_index - 1, "%04d-%02d-%02d %02d:%02d:%02d.%03ld",
                    st_local_time.tm_year + 1900, st_local_time.tm_mon + 1, st_local_time.tm_mday, /* 年份从1900开始 */
                    /* 1ms等于1000us */
                    st_local_time.tm_hour, st_local_time.tm_min, st_local_time.tm_sec, oal_get_usec(&st_tv)
                        / USEC_PER_MSEC);
                ...
                *pul_len = l_index + MAC_REPORT_RSSIINFO_SNR_LEN;
            }
        ...
        case MAC_HISI_LOCATION_FTM_IE:
            ...
            *(oal_uint32 *)&puc_send_ftm_buf[l_index] = (oal_uint32)3; /* 3赋值给types */
            l_index += 4; /* 跳过Type 4Bytes */
            *(oal_uint32 *)&puc_send_ftm_buf[l_index] = 99; /* 99赋值给len */
            l_index += 4;  /* 跳过len 4Bytes */
            l_ret += memcpy_s(&puc_send_ftm_buf[l_index], HMAC_FTM_SEND_BUF_LEN - l_index,
                              pst_location_event->auc_mac_server, WLAN_MAC_ADDR_LEN);
            l_index += WLAN_MAC_ADDR_LEN;
            l_ret += memcpy_s(&puc_send_ftm_buf[l_index], HMAC_FTM_SEND_BUF_LEN - l_index,
                              pst_location_event->auc_mac_client, WLAN_MAC_ADDR_LEN);
            l_index += WLAN_MAC_ADDR_LEN;
            l_index += snprintf_s(puc_send_ftm_buf + l_index, HMAC_FTM_SEND_BUF_LEN - l_index,
                HMAC_FTM_SEND_BUF_LEN - l_index - 1, "%04d-%02d-%02d %02d:%02d:%02d.%03ld",
                /* 初始年份从1900开始 */
                st_local_time.tm_year + 1900, st_local_time.tm_mon + 1, st_local_time.tm_mday, st_local_time.tm_hour,
                st_local_time.tm_min, st_local_time.tm_sec, oal_get_usec(&st_tv) / USEC_PER_MSEC); /* 1ms等于1e6 ns */
            puc_payload = (oal_uint8 *)(pst_location_event->auc_payload);
            /* 56表示拷贝数 */
            l_ret += memcpy_s(&puc_send_ftm_buf[l_index], HMAC_FTM_SEND_BUF_LEN - l_index, puc_payload, 56);
            break;
        default:
            return OAL_SUCC;
    }
...
/*
 * 函 数 名  : hmac_proc_location_action
 * 功能描述  : 处理来自芯片的定位事件
 * 修改历史  :
 * 1.日    期  : 2017年11月3日
 *   作    者  : g00260350
...
oal_uint32 hmac_proc_location_action(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (l_str_len == -1) {
        OAM_ERROR_LOG1(0, OAM_SF_FTM, "{hmac_proc_location_action: ****************l_str_len = %d }", l_str_len);
        return OAL_FAIL;
    }
    ...
    if (pst_lca->uc_location_type == MAC_HISI_LOCATION_CSI_IE) {
        if ((pst_lca->auc_payload[0] == 0) && ((pst_lca->auc_payload[1] == 0) || (pst_lca->auc_payload[1] == 1))) {
            ...
            oal_kernel_file_print(f_file, "%02d-%02d-%02d-%08d : ", st_time.tm_hour, st_time.tm_min,
                                  st_time.tm_sec, oal_get_usec(&st_tv));
            /* rssi snr */
...
#ifdef _PRE_WLAN_FEATURE_LOCATION_RAM
/*
 * 函 数 名  : hmac_huawei_action_process
 * 功能描述  : 完成repeater上报的私有测量信息
 * 修改历史  :
 * 1.日    期  : 2017年11月2日
 *   作    者  : g00260350
 *   修改内容  : 新生成函数
