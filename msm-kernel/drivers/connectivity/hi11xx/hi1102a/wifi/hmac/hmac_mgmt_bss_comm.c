...
#ifdef _PRE_WLAN_FEATURE_BTCOEX
/*
 * 函 数 名  : hmac_btcoex_get_ba_size
 * 功能描述  : 获取共存场景下特殊AP发送方向聚合个数
 * 修改历史  :
 * 1.日    期  : 2018年12月20日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
    ...
    hmac_vap_stru *pst_hmac_vap, hmac_ba_rx_stru *pst_addba_rsp, oal_uint16 *pus_param)
{
    ...
    if ((pst_hmac_user != OAL_PTR_NULL) && (pst_hmac_device != OAL_PTR_NULL)) {
        ...
        /* DTS2019010906273 电话场景下对外宣称不支持AMSDU,避免AP的AMSDU聚合包包长太长导致重传带来的性能性能掉低 */
        /* DTS2019120302032 音乐场景下为了提高用户体验也不再支持AMSDU */
    ...
    hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, mac_action_mgmt_args_stru *pst_action_args)
{
    ...
    /* 对tid对应的tx BA会话状态加锁 */
    oal_spin_lock_bh(&(pst_hmac_user->ast_tid_info[uc_tidno].st_ba_tx_info.st_ba_status_lock));
    /* DTS2018041602959 接收方向判断接收方向status，发送方向判断发送方向status */
    /* DTS2018050202260,接收方向delba发送失败，或者对端没有收到，要再次发送delba时需要放宽判断,这里主要保证rx delba始终可以发送即可，无其他影响 */
                                  ...
                                  oal_uint32 frame_body_len)
{
    ...
    /* DTS2018051605170 使用超时机制去删除聚合。降低主动删除聚合后，立马收到匹配的ADDBA RSP，
...
/*
 * 函 数 名 : hmac_wpas_sae_auth_update_cb
 * 功能描述 : SAE认证时更新认证帧cb字段
 *            漫游时dmac调用dmac_tx_force发送管理帧, 要求cb中netbuf_num = 1且user_idx对应的dmac_user存在
 *            原函数未填写上述两个字段, 认证帧发送失败, 导致认证超时
 * 修改历史 :
 * 1.日    期 : 2019年8月28日
 *   作    者 : g00428180
 *   修改内容 : 新生成函数
