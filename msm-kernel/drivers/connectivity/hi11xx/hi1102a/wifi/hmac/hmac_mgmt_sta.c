 ...
 * 功能描述  :P2P_DEVICE 在监听状态接收到管理帧处理，HS2.0查询过程的ACTION上报
 * 修改历史  :
 * 1.日    期  : 2014年11月13日
...
/*
 * 函 数 名  : hmac_is_ht_mcs_set_valid
 * 功能描述  : 判断assoc rsp 帧中HT capability IE MCS set是否异常
 * 修改历史  :
 * 1.日    期  : 2018年2月13日
 *   作    者  : duankaiyong
 *   修改内容  : 新生成函数
...
oal_bool_enum_uint8 hmac_is_ht_mcs_set_valid(oal_uint8 *puc_ht_cap_ie, wlan_channel_band_enum_uint8 en_band)
{
    ...
    ***************************************************************************/
    ...
    /* DTS2018082911365:仅针对惠普打印机M202dw降协议 */
    ...
    /* DTS2017082600989:对端支持ht能力,但空间速率集为0,要置对端user为不支持ht能力,从而以11a或者11g速率关联 */
...
/*
 * 函 数 名  : hmac_sta_update_ht_opern
    ...
    oal_uint8 *puc_payload, oal_uint16 us_offset, oal_uint16 us_rx_len, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_hmac_user->st_op_rates.uc_rs_nrates == 0) {
        ...
        /* DTS2019021200771 start 基本速率集和扩展速率集都没有的时候，手动增加最低的两个速率集 */
                                  ...
                                  oal_uint8 *puc_mac_hdr, oal_uint8 *puc_payload, oal_uint16 us_msg_len)
{
    ...
    /* DTS2018060807930:11n STA不使能rx ampdu+amsdu,增强抗干扰性能 */
...
oal_uint32 hmac_sta_wait_asoc_rx(hmac_vap_stru *pst_hmac_sta, oal_void *pst_msg)
{
    ...
    if (en_asoc_status != MAC_SUCCESSFUL_STATUSCODE) {
        ...
        /* BEGIN:DTS2016062110680 STA 保存AP 在assoc rsp 帧中的status code */
...
/*
 * 函 数 名  : hmac_sta_get_mngpkt_sendstat
 * 功能描述  : 获取请求帧的发送状态
 * 修改历史  :
 * 1.日    期  : 2018年8月5日
 *   作    者  : j00293173
...
/*
 * 函 数 名  : hmac_sta_update_capability
 * 功能描述  :
 * 修改历史  :
 * 1.日    期  : 2013年7月3日
 *   作    者  : z00241943
...
/*
 * 函 数 名  : hmac_update_sta_country_code
 * 功能描述  : 更新STA VAP下记录的国家码
 * 修改历史  :
 * 1.日    期  : 2020年2月26日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_set_device_channel_info
 * 功能描述  : 设置device侧的信道带宽等状态信息
 * 修改历史  :
 * 1.日    期  : 2020年2月26日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_set_FortyMHzOperationImplemented_by_chr
 * 功能描述  : 根据CHR事件的结果来更新该mib项, 不能上网时的弱场场景下需要强制切到20M
 * 修改历史  :
 * 1.日    期  : 2020年2月26日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
                                          ...
                                          oal_bool_enum_uint8 en_is_protected)
{
    ...
    switch (puc_data[MAC_ACTION_OFFSET_CATEGORY]) {
        case MAC_ACTION_CATEGORY_BA: {
            hmac_sta_up_category_ba(pst_hmac_vap, pst_frame_hdr, pst_hmac_user, puc_data, frame_body_len);
        }
        break;
        ...
        case MAC_ACTION_CATEGORY_PUBLIC: {
            hmac_sta_up_category_public(pst_hmac_vap, pst_netbuf, puc_data, frame_body_len);
        }
        break;
#ifdef _PRE_WLAN_FEATURE_WMMAC
        case MAC_ACTION_CATEGORY_WMMAC_QOS: {
            hmac_sta_up_category_wmmac_qos(pst_hmac_vap, pst_hmac_user, puc_data, frame_body_len);
        }
        break;
#endif  // _PRE_WLAN_FEATURE_WMMAC
