...
/*
 函 数 名  : hmac_config_query_chr_ext_info_rsp_event
 功能描述  : chr查询wifi额外信息上报接口
 修改历史      :
  1.日    期   : 2019年8月12日
    作    者   : x00456388
    修改内容   : 新生成函数
...
/*
 * 函 数 名  : hmac_proc_query_response_flag_etc
 * 功能描述  : 处理上报内核的速率标记
 * 修改历史  :
 * 1.日    期  : 2020.2.12
 *   作    者  : l00423930
    ...
    dmac_query_station_info_response_event *pst_rsp_event)
{
    ...
    uc_flag |= ((pst_rsp_event->rate_info.txrx_rate.tx_rate.flags & MAC_RATE_INFO_FLAGS_60G) ? RATE_INFO_FLAGS_DMG : 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
    uc_flag |= ((pst_rsp_event->rate_info.txrx_rate.tx_rate.flags & MAC_RATE_INFO_FLAGS_60G) ? RATE_INFO_FLAGS_60G : 0);
#endif
    /* DTS2016092102848 11b/g/n 20M /40M场景下,单WiFi冲包IW读取的TCP上行与UDP上行TX速率速率与实际物理层速率不一致 */
...
#else
    uc_flag |=
        ((pst_rsp_event->rate_info.txrx_rate.tx_rate.flags & MAC_RATE_INFO_FLAGS_60G) ? RATE_INFO_FLAGS_DMG : 0);
#endif
