...
/*
 * 函 数 名  : hmac_roam_alg_adjust_5g_rssi_weight
 * 功能描述  : 调整5G AP rssi权重
 * 修改历史  :
 * 1.日    期  : 2018年9月22日
 *   作    者  : t00422961
                                                         ...
                                                         mac_bss_dscr_stru *pst_bss_dscr, oal_int8 c_target_weight_rssi)
{
    ...
    if (pst_bss_dscr->st_channel.en_band == WLAN_BAND_5G) {
        c_delta_rssi = (oal_int16)pst_roam_info->st_config.c_delta_rssi_5g;
    } else {
        c_delta_rssi = (oal_int16)pst_roam_info->st_config.c_delta_rssi_2g;
    }
    ...
    if (c_delta_rssi < ROAM_RSSI_DIFF_4_DB) {
        c_delta_rssi = ROAM_RSSI_DIFF_4_DB;
    }

    if (s_current_rssi >= ROAM_RSSI_NE70_DB) {
        return (s_target_rssi - s_current_rssi - c_delta_rssi);
    }

    if (c_delta_rssi >= ROAM_RSSI_DIFF_4_DB + 2) { /* 步进2DB至4DB */
        c_delta_rssi -= 2;
    }
...
/*
 * 函 数 名  : hmac_roam_alg_check_bss_is_valid
 * 功能描述  : 检查背景扫描结果，过滤无效bss
 * 修改历史     :
 * 1.日    期   : 2019年9月24日
 *  作     者   : y00452465
 *  修改内容   : 新生成函数
...
oal_uint32 hmac_roam_alg_bss_in_ess(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    if (oal_compare_mac_addr(pst_mac_vap->auc_bssid, pst_bss_dscr->auc_bssid) != 0) {
        ...
        /* DTS2019091113544检查漫游扫描结果，提前过滤无效bss，防止暗屏下频繁触发漫游扫描 */
...
/*
 * 函 数 名  : hmac_roam_alg_check_bssid_limit
 * 功能描述  : 封装查找黑名单函数
 * 修改历史  :
 * 1.日    期  : 2020年02月13日
 *   作    者  : h00368777
...
oal_bool_enum_uint8 hmac_roam_alg_need_to_stop_roam_trigger(hmac_roam_info_stru *pst_roam_info)
{
    ...
    if (pst_roam_alg->uc_invalid_scan_cnt++ > 4) { /* 连续扫描到当前关联且首次关联的 bss 的次数是否大于4 */
        pst_roam_alg->uc_invalid_scan_cnt = 0xff;
        return OAL_TRUE;
    }
