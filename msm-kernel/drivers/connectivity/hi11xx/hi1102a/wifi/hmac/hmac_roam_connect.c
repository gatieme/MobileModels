                                                    ...
                                                    oal_uint16 us_msg_len)
{
    ...
    /* DTS2016122304229: asoc_rsp 帧拷贝一份上报上层,防止帧内容上报wal侧处理后被hmac侧释放 */
...
OAL_STATIC oal_uint32 hmac_roam_process_auth_seq2(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
...
#ifdef _PRE_WLAN_FEATURE_11R
    if (pst_hmac_vap->bit_11r_enable == OAL_TRUE) {
        if (mac_get_auth_alg(puc_mac_hdr) == WLAN_WITP_AUTH_FT) {
    ...
    if (mac_get_auth_alg(puc_mac_hdr) != WLAN_WITP_AUTH_OPEN_SYSTEM) {
        return OAL_SUCC;
    }
