...
/*
 * 函 数 名  : hmac_roam_is_wpa_cipher
 * 功能描述  : 判断加密套件是否属于wpa/wpa2
 * 修改历史  :
 * 1.日    期  : 2018年11月10日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_roam_get_curr_cipher
 * 功能描述  : 根据us_wpa_type(wpa/rsn)返回bss信息中对应数组存储的某个加密方式
 * 修改历史  :
 * 1.日    期  : 2018年11月10日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_roam_select_best_cipher
 * 功能描述  : 获取扫描到的bss信息中加密能力最高的加密套件
 * 修改历史  :
 * 1.日    期  : 2018年11月10日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
...
OAL_STATIC oal_uint32 hmac_roam_renew_privacy(hmac_vap_stru *pst_hmac_vap, mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    /* DTS2019030704404: 对齐标杆, 支持漫游至非pmf enable ap, pmf能力更新移至join之前 */
...
oal_uint32 hmac_roam_init(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
        ...
        pst_hmac_vap->pul_roam_info =
            (oal_uint32 *)oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL, OAL_SIZEOF(hmac_roam_info_stru), OAL_TRUE);
        if (pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    pst_roam_info->uc_enable = g_wlan_customize.uc_roam_switch;
#else
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    pst_roam_info->st_config.uc_scan_band = g_wlan_customize.uc_roam_scan_band;
    pst_roam_info->st_config.uc_scan_orthogonal = g_wlan_customize.uc_roam_scan_orthogonal;
    pst_roam_info->st_config.c_trigger_rssi_2g = g_wlan_customize.c_roam_trigger_b;
    pst_roam_info->st_config.c_trigger_rssi_5g = g_wlan_customize.c_roam_trigger_a;
    pst_roam_info->st_config.c_delta_rssi_2g = (oal_uint8)g_wlan_customize.c_roam_delta_b;
    pst_roam_info->st_config.c_delta_rssi_5g = (oal_uint8)g_wlan_customize.c_roam_delta_a;
#else
    ...
    pst_roam_info->st_config.c_delta_rssi_2g = ROAM_RSSI_DIFF_10_DB;
    pst_roam_info->st_config.c_delta_rssi_5g = ROAM_RSSI_DIFF_10_DB;
#endif
...
oal_uint32 hmac_roam_exit(hmac_vap_stru *pst_hmac_vap)
{
    ...
    frw_immediate_destroy_timer(&(((hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info)->st_connect.st_timer));
    hmac_roam_main_del_timer((hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info);
...
OAL_STATIC oal_void hmac_roam_main_del_timer(hmac_roam_info_stru *pst_roam_info)
{
    ...
    frw_immediate_destroy_timer(&(pst_roam_info->st_timer));
    return;
}
...
/*
 * 函 数 名  : hmac_roam_ping_pong_clear
 * 功能描述  : 乒乓场景相关参数清零
 * 修改历史  :
 * 1.日    期  : 2019年11月16日
 *   作    者  : y00452465
...
/*
 * 函 数 名  : hmac_roam_ping_pong_check
 * 功能描述  : 识别乒乓场景
 * 修改历史  :
 * 1.日    期  : 2019年11月16日
 *   作    者  : y00452465
                           ...
                           oal_bool_enum_uint8 en_current_bss_ignore, roam_trigger_enum_uint8 en_roam_trigger)
{
    ...
    /* DTS2018080712286 漫游到自己做重关联业务不做检查，即使在信号桥下也支持，比如btcoex m2s 和上层下发的重关联到自己 */
...
#endif
/*
 * 函 数 名  : hmac_get_rssi_from_scan_result
    ...
    hmac_roam_info_stru *pst_roam_info, oal_void *p_param, mac_bss_dscr_stru **ppst_bss_dscr_out)
{
    ...
    /* DTS2018112904470:加密信息更新失败不进行漫游 */
...
OAL_STATIC oal_uint32 hmac_roam_connecting_fail(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* DTS2019070100269: auth/assoc reject processing, don't delete user when reassoc reject in roaming,
