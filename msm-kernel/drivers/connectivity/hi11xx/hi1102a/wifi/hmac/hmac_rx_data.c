...
/*
 * 函 数 名  : hmac_rx_amsdu_check_frame
 * 功能描述  : 检查amsdu帧是否合理。
 * 修改历史  :
 * 1.日    期  : 2019年9月5日
 *   作    者  : r00356856
 ...
 */
    if (pst_rx_ctrl->st_rx_info.bit_amsdu_enable == OAL_FALSE) {
        ...
        if (hmac_11i_ether_type_filter(pst_vap, &pst_hmac_user->st_user_base_info,
            ...
            /* DTS2015070908972,us_ether_type为0导致企业级加密失败,添加维测信息看是否报文解析异常 */
    ...
    dmac_msdu_stru *pst_msdu)
{
    ...
    /* 重新获取该MPDU的控制信息 */
    pst_rx_ctrl = (hmac_rx_ctl_stru *)oal_netbuf_cb(pst_netbuf);
    /* DTS2018042703577 去分片有可能释放netbuf，需要重新获取frame hdr */
    pst_frame_hdr = (mac_ieee80211_frame_stru *)pst_rx_ctrl->st_rx_info.pul_mac_hdr_start_addr;
    ...
    /* DTS2018031601896 当msdu.da在vap下,进行wlan_to_wlan转发,防止to_lan到内核不进行转发 */
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
...
/*
 * 函 数 名 : hmac_rx_check_data
 * 功能描述 : 检查接收到的特定广播帧data
 * 修改历史 :
 * 1.日 期  : 2019年9月17日
 * 作    者   : b00484323
 * 修改内容     : 新生成函数
...
/*
 * 函 数 名 : hmac_rx_check_snap
 * 功能描述 : 检查接收到的特定广播帧snap头
 * 修改历史 :
 * 1.日 期  : 2019年9月17日
 * 作    者   : b00484323
 * 修改内容     : 新生成函数
...
/*
 * 函 数 名 : hmac_packet_check_rsp
 * 功能描述 : 特定广播帧定时器关闭,上报检查结果
 * 修改历史 :
 * 1.日 期  : 2019年9月17日
 * 作    者   : b00484323
 * 修改内容     : 新生成函数
...
/*
 * 函 数 名 : hmac_rx_need_check_payload
 * 功能描述 : 检查接收到的特定广播帧mac头,是否是特定的广播帧
 * 返 回 值 : OAL_TRUE 帧头符合特定广播帧
 * 1.日 期  : 2019年9月17日
 * 作    者   : b00484323
 * 修改内容     : 新生成函数
 */
...
/*
 * 函 数 名 : hmac_rx_verification_filter
 * 功能描述 : 检查接收到的特定广播帧内容是否符合要求,根据返回值判断是否过滤
 * 返 回 值 : OAL_TRUE 符合特定广播帧要求,需要过滤,不再上报kernel
 * 1.日 期  : 2019年9月17日
 * 作    者   : b00484323
 * 修改内容     : 新生成函数
 */
