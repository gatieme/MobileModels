...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* DTS2021120124050 chba建链响应方不需要处理external auth事件。 */
...
oal_void hmac_ap_up_rx_auth_req_to_host(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_auth_req)
{
    ...
    auth_alg = mac_get_auth_algo_num(pst_auth_req);
    hmac_ap_set_user_auth_alg_num(us_user_index, auth_alg);
}

/*
 * 函 数 名  : hmac_sta_sae_commit_handle_exception
 * 功能描述  : 接收SAE Commit消息时对上层无法处理的异常情况进行处理
 * 修改历史  :
 * 1.日    期  : 2019年12月23日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
    ...
    hmac_vap_stru *pst_sta, oal_netbuf_stru *pst_netbuf, oal_uint16 us_status, oal_uint16 us_seq_num)
{
    ...
    if (us_status == MAC_AP_FULL) {
        ...
        /* DTS2019120704065: AP以具体的理由拒绝接入时(如status code 17接入已满), 需要将状态码上报上层而非透传 */
...
/*
 * 函 数 名 : hmac_only_support_sae
 * 功能描述 : AKM数组是否只包含SAE认证方式
 * 修改历史 :
 * 1.日    期 : 2019年9月21日
 *   作    者 : g00428180
 *   修改内容 : 新生成函数
...
/*
 * 函 数 名 : hmac_check_illegal_sae_roam
 * 功能描述 : 检查STA和目标AP的认证方式是否满足SAE漫游要求
 * 修改历史 :
 * 1.日    期 : 2019年9月20日
 *   作    者 : g00428180
 *   修改内容 : 新生成函数
