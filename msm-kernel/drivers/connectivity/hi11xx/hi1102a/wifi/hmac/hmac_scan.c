...
/*
 * 函 数 名  : hmac_scan_update_bss_list_nb
 * 功能描述  : 解析 nb IE
 * 修改历史  :
 * 1.日    期  : 2017年12月4日
 *   作    者  : wangcunlian
...
/*
 * 函 数 名  : hmac_scan_update_vht_cap
 * 功能描述  : 解析VHT capablities info field，判断AP是否支持11ac
 * 修改历史     :
 * 1.日    期   : 2019年9月21日
 *  作     者   : y00452465
 *  修改内容   : 新生成函数
...
OAL_STATIC oal_void hmac_scan_update_vht_cap(mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_ie)
{
    ...
    /* DTS2019092000546: vht cap info非全0或rx mcs非全f，则认为支持11ac */
                                                ...
                                                oal_uint16 us_offset)
{
    ...
    /* DTS2019031307582&DTS2019012107931 识别出是REALTEKS芯片 */
    if (OAL_PTR_NULL !=
...
OAL_STATIC oal_void hmac_scan_chip_oui_process(mac_bss_dscr_stru *pst_bss_dscr,
                                               ...
                                               oal_uint16 us_offset)
{
    /* DTS2019022009780:用于识别TP-LINK 847N，记录芯片厂家OUI，其他芯片厂家记录需要时再增加 */
    ...
    mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_frame_body, oal_uint16 us_frame_len, mac_device_stru *pst_mac_dev)
{
    ...
    if (puc_ie != OAL_PTR_NULL) {
        ...
        /* DTS2015032407334 斐讯FIR304商用AP 11g模式，发送的支持速率集个数为12，不符合协议规定,
           为增加兼容性，修改判断分支为12 */
...
/*
 * 函 数 名  : mac_ie_find_vendor_vht_ie
...
/*
 * 函 数 名  : hmac_scan_get_bss_max_nss
...
/*
 * 函 数 名  : hmac_scan_get_num_sounding_dim
...
oal_uint8 hmac_scan_get_num_sounding_dim(oal_uint8 *puc_frame_body, oal_uint16 us_frame_len)
{
    ...
    /* DTS2018051904000 增加兼容性，还需从私有vendor vht ie里面查找number of sounding dimension */
...
/*
 * 函 数 名  : hmac_scan_get_11v_ability
 * 功能描述  : 获取扫描11v能力
 * 修改历史  :
 * 1.日    期  : 2019年3月19日
 *   作    者  : l00422505
...
/*
 * 函 数 名  : hmac_scan_get_11k_ability
 * 功能描述  : 获取扫描11k能力
 * 修改历史      :
 * 1.日    期   : 2019年7月5日
 *   作    者   : h00368777
...
/*
 * 函 数 名  : hmac_scan_need_update_timestamp
 * 功能描述  : 判断是否可以更新扫描信息的时间戳
 * 修改历史  :
 * 1.日    期  : 2018年5月03日
 *   作    者  : duankaiyong
 *   修改内容  : 新生成函数
...
{
    /* DTS2018011600988 对隐藏ssid ap，之前扫描到probe rsp， 携带ssid， 当前扫描到为beacon ，不携带ssid ，则不更新时间戳 */
    if ((pst_new_bss->st_bss_dscr_info.ac_ssid[0] == '\0') &&
        (pst_old_bss->st_bss_dscr_info.ac_ssid[0] != '\0')) {
        /* DTS2015031705427:隐藏SSID,若保存过此AP信息,且ssid不为空,此次通过BEACON帧扫描到此AP信息,且SSID为空,则不进行更新 */
        return OAL_FALSE;
    }
...
{
    /* DTS2016052803088 该逻辑仅针对一轮扫描过程中出现beacon、probe rsp时的处理，
...
/*
 * 函 数 名  : hmac_scan_11v_bsstreq_filer_switch
 * 功能描述  : 根据扫描结果，如果信号强度高，下发关闭过滤功能
 * 修改历史  :
 * 1.日    期  : 2019年4月21日
 *   作    者  : l00422505
 *   修改内容  : 新生成函数
...
oal_uint32 hmac_scan_proc_scanned_bss(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* begin: DTS2018011600988 对隐藏ssid AP, 接收到beacon 帧，不更新时间戳，避免删除隐藏ssid 网络后，还显示扫描结果 */
    ...
    /* end: DTS2018011600988 对隐藏ssid AP, 接收到beacon 帧，不更新时间戳，避免删除隐藏ssid 网络后，还显示扫描结果 */
                                                     ...
                                                     mac_scan_rsp_stru *pst_d2h_scan_rsp_info)
{
    ...
    /* DTS2018052506313, 解决上层去关联命令时host上报扫描abort状态后dmac又抛上来一次扫描abort状态导致时序乱了的问题 */
    ...
    /* BEGIN:DTS2015072801307 1102 作为ap ，40M 带宽下执行扫描，扫描完成后VAP 状态修改为扫描前的状态 */
    ...
    /* 如果扫描回调函数不为空，则调用回调函数 */
    /* DTS2017082803150为了提升去关联后重关联的速度,所以在scan abort后立即上报扫描结果,需要芯片上报上一次扫描结果后才会触发新一轮扫描,从而影响重关联速度 */
    ...
    mac_scan_rsp_stru *pst_d2h_scan_rsp_info)
{
    ...
    if (pst_hmac_vap->en_wait_roc_end == OAL_TRUE) {
        /* DTS2017072102239有此标记就通知结束, 防止wal层下发扫描的时候在扫描, 下发abort, 但此时刚好扫描正常结束,
           abort未执行,这里就未complete
         */
...
oal_uint32 hmac_scan_proc_scan_comp_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* DTS2015110908011 防止compete事件和正在处理的扫描不一致 */
    ...
    if (pst_hmac_vap->en_wait_chan_meas_end == OAL_TRUE) {
        OAM_WARNING_LOG1(0, OAM_SF_SCAN, "{hmac_scan_proc_scan_comp_event::chan meas end, scan rsp status[%d]}",
            pst_d2h_scan_rsp_info->en_scan_rsp_status);
        oal_complete(&(pst_hmac_vap->st_chan_meas_end_ready));
        pst_hmac_vap->en_wait_chan_meas_end = OAL_FALSE;
    }
    return OAL_SUCC;
}
/*
...
OAL_STATIC oal_uint32 hmac_scan_proc_scan_timeout_fn(void *p_arg)
{
    ...
    /* END:DTS2015072801307 1102 作为ap ，40M 带宽下执行扫描，扫描完成后VAP 状态修改为扫描前的状态 */
    /* 设置扫描响应状态为超时 */
...
oal_uint32 hmac_scan_proc_scan_req_event(hmac_vap_stru *pst_hmac_vap, oal_void *p_params)
{
    ...
    if (ul_ret != OAL_SUCC) {
        /* DTS2017082205164 扫描被拒绝必须恢复到原来的状态 */
        if (pst_scan_params->uc_scan_func == MAC_SCAN_FUNC_P2P_LISTEN) {
...
oal_uint32 hmac_scan_process_chan_result_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    hmac_chan_meas_scan_complete_handler(pst_scan_record, uc_scan_idx);
    return OAL_SUCC;
}
...
/*
 * 函 数 名  : hmac_scan_rrm_proc_save_bss
 * 功能描述  : 向dmac发送已保存bss的chan num, bssid,
 * 修改历史  :
 * 1.日    期  : 2014年11月27日
 *   作    者  : yangguisen
 *   修改内容  : 新生成函数
