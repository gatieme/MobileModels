...
oal_tcp_ack_type_enum_uint8 hmac_tcp_opt_get_tcp_ack_type(
...
oal_tcp_ack_type_enum_uint8  hmac_tcp_opt_rx_get_tcp_ack_type_index(
    oal_netbuf_stru *skb, hmac_vap_stru    *pst_hmac_vap, oal_uint8 *p_uc_index, oal_uint8 dir);
oal_tcp_ack_type_enum_uint8  hmac_tcp_opt_tx_get_tcp_ack_type_index(
    oal_netbuf_stru *skb, hmac_vap_stru    *pst_hmac_vap, oal_uint8 *p_uc_index, oal_uint8 dir);
oal_bool_enum_uint8 hmac_tcp_opt_judge_is_same_tcp_flow_info(
    hmac_vap_stru    *pst_hmac_vap,
    const struct wlan_tcp_flow  st_tcp_flow_info,
    oal_uint8 *p_uc_index,
    const hcc_chan_type dir);
oal_uint8 hmac_tcp_opt_add_new_node(
    hmac_vap_stru    *pst_hmac_vap,
    const struct wlan_tcp_flow *tcp_info,
    const oal_tcp_header_stru *pst_tcp_hdr,
    const hcc_chan_type dir);

oal_uint16 hmac_tcp_opt_tcp_ack_list_filter(
    hmac_vap_stru    *pst_hmac_vap, hmac_tcp_opt_queue type, hcc_chan_type dir, oal_netbuf_head_stru  *head);
oal_uint32 hmac_tcp_opt_tcp_ack_filter(oal_netbuf_stru *skb, hmac_vap_stru    *pst_hmac_vap, hcc_chan_type dir);

void hmac_trans_queue_filter(
    hmac_vap_stru    *pst_hmac_vap, oal_netbuf_head_stru  *head_t,
    hmac_tcp_opt_queue type, hcc_chan_type dir);
void hmac_tcp_ack_process_hcc_queue(
    hmac_vap_stru    *pst_hmac_vap,
