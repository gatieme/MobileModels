                                           ...
                                           oal_uint32 ul_framelen)
{
    ...
    pst_snap_head->auc_org_code[2] = SNAP_RFC1042_ORGCODE_2; /* 将auc_org_code第2byte置零 */
...
oal_uint32 hmac_amsdu_tx_process(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    if (ul_frame_len < ETHER_HDR_LEN) {
        OAM_ERROR_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU,
            "{hmac_amsdu_process::ul_frame_len < ETHER_HDR_LEN.}");
                                       ...
                                       oal_netbuf_stru *pst_buf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    us_80211_frame_len = us_mpdu_len + SNAP_LLC_FRAME_LEN + 2 + MAC_80211_QOS_HTC_4ADDR_FRAME_LEN; /* 2代表duration */
    ...
    if (oal_netbuf_headroom(pst_buf) < (SNAP_LLC_FRAME_LEN + 2)) { /* 2代表duration */
        return;
    }
