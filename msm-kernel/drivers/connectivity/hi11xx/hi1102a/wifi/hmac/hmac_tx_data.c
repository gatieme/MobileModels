...
/*
 * 函 数 名  : hmac_tx_classify_tcp
 * 功能描述  : 识别tcp报文，赋值对应的tid (降圈复杂度)
 * 修改历史  :
 * 1.日    期  : 2019年3月7日
 *   作    者  : duankaiyong
 *   修改内容  : 新生成函数
    ...
    if ((oal_ntoh_16(pst_tcp_hdr->us_dport) == MAC_CHARIOT_NETIF_PORT) ||
        ...
        *puc_tid = WLAN_DATA_VIP_TID;
        ...
        if (oal_netbuf_is_tcp_ack((oal_ip_header_stru *)pst_ip) == OAL_TRUE) {
            ...
            /* DTS2018101804659 识别WifiDisplay RTSP业务为VO，避免WFD断链 */
...
#ifdef _PRE_WLAN_FEATURE_APF
/*
 * 函 数 名  : hmac_auto_set_apf_switch_in_suspend
 * 功能描述  : 在暗屏模式下根据pps动态调整apf filter是否开启
 * 修改历史  :
 * 1.日    期  : 2019年4月16日
 *   作    者  : x00456388
 *   修改内容  : 新生成函数
...
#ifdef _PRE_WLAN_FEATURE_WAPI
        || (pst_mac_device->uc_wapi == OAL_TRUE) /* DTS2019062007004 WAPI加密不做APF过滤,避免误过滤 */
#endif
) {
        return;
    }
...
/*
 * 函 数 名  : hmac_set_psm_activity_timer
 * 功能描述  : 通过pps值来设置dmac侧低功耗定时器周期和重启次数
 * 修改历史  :
 * 1.日    期  : 2018年10月10日
 *   作    者  : x00456388
 *   修改内容  : 新生成函数
                                    ...
                                    oal_netbuf_stru *pst_buf)
{
    ...
    if (g_hitalk_status & NARROW_BAND_ON_MASK) {
        if (!MAC_GET_CB_IS_VIPFRAME(pst_tx_ctl) && (oal_netbuf_len(pst_buf) > 200)) { /* 判断netbuf len是否大于200 */
...
/*
 * 函 数 名  : hmac_bridge_vap_should_drop
 * 功能描述  : bridge vap xmit中是否需要drop掉这些帧
 * 修改历史  :
 * 1.日    期  : 2019年4月9日
 *   作    者  : h00368777
 *   修改内容  : 新生成函数
                             ...
                             oal_netbuf_stru *pst_buf)
{
    ...
    if ((pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
        ...
        /* DTS2017092906461,解决与TPlink7500兼容性问题,AP在VHT协议下,误将非聚合数据帧当做
           聚合帧,回复BA确认导致芯片持续重传 */
