...
/*
 * 函 数 名  : hmac_user_destroy_timer
 * 功能描述  : memzero user之前先destroy user中的所有静态timer
 * 修改历史     :
 * 1.日    期   : 2019年12月5日
 *   作    者   : h00368777
 *   修改内容   : 新生成函数
...
/*
 * 函 数 名  : hmac_user_alloc_multi_user
 * 功能描述  : 申请multiuser
...
/*
 * 函 数 名  : hmac_txbf_ability_sync
 * 功能描述  : 同步txbf能力至device
 * 修改历史  :
 * 1.日    期  : 2019年2月19日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_txbf_ability_enable
 * 功能描述  : add user时开启/关闭txbf能力
 * 修改历史  :
 * 1.日    期  : 2019年2月19日
 *   作    者  : g00428180
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : hmac_amsdu_ampdu_compability_enable
 * 功能描述  : 识别联合聚合兼容性AP
 * 修改历史  :
 * 1.日    期  : 2019年3月11日
 *   作    者  : hmac
...
{
    if (!IS_AP(&hmac_vap->st_vap_base_info) || assoc_id >= MAC_RES_MAX_USER_NUM) {
        return;
    }
...
oal_uint32 hmac_user_del(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        /* DTS2019090211121:每次STA去关联时，都重新调整扫描随机mac地址 */
...
#endif
#ifdef _PRE_WLAN_FEATURE_ROAM
        /* DTS2018102303302:STA删除用户时设置IP地址被删除 */
...
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_chba_del_user_proc(pst_hmac_vap, pst_hmac_user);
#endif
    /* 从vap中删除用户 */
...
#endif
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_update_chba_vap_info(pst_hmac_vap, pst_hmac_user);
    hmac_user_del_clear_vap_user_bitmap(pst_hmac_vap, us_user_index);
#endif
    /* 释放用户内存 */
...
/*
 * 函 数 名  : hmac_set_roam_prohibit_on
 * 功能描述  : 设置此ap是否不许漫游
 * 修改历史  :
 * 1.日    期  : 2019年2月27日
 *   作    者  : h00368777
...
/*
 * 函 数 名  : hmac_sta_user_num_check
 * 功能描述  : 判断STA下user个数是否正常
 * 修改历史  :
 * 1.日    期  : 2019年3月6日
 *   作    者  : r00356856
 *   修改内容  : 新生成函数
...
oal_uint32 hmac_user_add(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac_addr, oal_uint16 *pus_user_index)
{
    ...
    if ((pst_mac_vap == OAL_PTR_NULL) || (puc_mac_addr == OAL_PTR_NULL) || (pus_user_index == OAL_PTR_NULL)) {
    ...
    ***************************************************************************/
    ...
    frw_event_hdr_init(&(pst_event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX, DMAC_WLAN_CTX_EVENT_SUB_TYPE_ADD_USER,
        OAL_SIZEOF(dmac_ctx_add_user_stru), FRW_EVENT_PIPELINE_STAGE_1,
        pst_mac_vap->uc_chip_id, pst_mac_vap->uc_device_id, pst_mac_vap->uc_vap_id);
...
#endif
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_user_add_update_vap_user_bitmap(pst_hmac_vap, us_user_idx);
#endif
    ...
    mac_cfg_add_user_param_stru *add_user)
{
    ...
    st_ht_hdl.uc_min_mpdu_start_spacing =
        mac_vap->pst_mib_info->st_wlan_mib_ht_sta_cfg.ul_dot11MinimumMPDUStartSpacing;
    st_ht_hdl.uc_max_rx_ampdu_factor =
        mac_vap->pst_mib_info->st_wlan_mib_ht_sta_cfg.ul_dot11MinimumMPDUStartSpacing;
    /* 设置ht速率集 */
    st_ht_hdl.bit_short_gi_20mhz =
        mac_vap->pst_mib_info->st_phy_ht.en_dot11ShortGIOptionInTwentyImplemented;
    st_ht_hdl.bit_short_gi_40mhz = mac_mib_get_ShortGIOptionInFortyImplemented(mac_vap);
    st_ht_hdl.bit_tx_stbc =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTTxSTBCOptionImplemented;
    st_ht_hdl.bit_rx_stbc =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTRxSTBCOptionImplemented;
    st_ht_hdl.bit_ht_delayed_block_ack =
        mac_vap->pst_mib_info->st_wlan_mib_sta_config.en_dot11DelayedBlockAckOptionImplemented;
    st_ht_hdl.bit_max_amsdu_length =
        mac_vap->pst_mib_info->st_wlan_mib_ht_sta_cfg.en_dot11MaxAMSDULength;
    ...
    /* 配置user 速率 */
    memcpy_s(st_ht_hdl.uc_rx_mcs_bitmask, WLAN_HT_MCS_BITMASK_LEN,
        mac_vap->pst_mib_info->st_supported_mcsrx.auc_dot11SupportedMCSRxValue,
    ...
    mac_cfg_add_user_param_stru *add_user)
{
    ...
    st_vht_hdl.bit_max_mpdu_length =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.ul_dot11MaxMPDULength;
    ...
    st_vht_hdl.bit_rx_ldpc =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTLDPCCodingOptionImplemented;
    st_vht_hdl.bit_short_gi_80mhz =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTShortGIOptionIn80Implemented;
    st_vht_hdl.bit_short_gi_160mhz =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTShortGIOptionIn160and80p80Implemented;
    st_vht_hdl.bit_tx_stbc =
        mac_vap->pst_mib_info->st_wlan_mib_phy_vht.en_dot11VHTTxSTBCOptionImplemented;
    st_vht_hdl.bit_rx_stbc = 0;
    st_vht_hdl.bit_su_beamformer_cap =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.en_dot11VHTSUBeamformerOptionImplemented;
    st_vht_hdl.bit_su_beamformee_cap =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.en_dot11VHTSUBeamformeeOptionImplemented;
    st_vht_hdl.bit_num_bf_ant_supported =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.ul_dot11VHTBeamformeeNTxSupport - 1;

    /* sounding dim是bfer的能力 */
    st_vht_hdl.bit_num_sounding_dim =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.ul_dot11VHTNumberSoundingDimensions;
    st_vht_hdl.bit_mu_beamformer_cap =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.en_dot11VHTMUBeamformerOptionImplemented;
    st_vht_hdl.bit_mu_beamformee_cap =
        mac_vap->pst_mib_info->st_wlan_mib_vht_txbf_config.en_dot11VHTMUBeamformeeOptionImplemented;

    st_vht_hdl.bit_vht_txop_ps =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.en_dot11VHTTXOPPowerSaveOptionImplemented;
    st_vht_hdl.bit_htc_vht_capable =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.en_dot11VHTControlFieldSupported;
    st_vht_hdl.bit_max_ampdu_len_exp =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.ul_dot11VHTMaxRxAMPDUFactor;
    if (memcpy_s(&st_vht_hdl.st_rx_max_mcs_map, sizeof(mac_rx_max_mcs_map_stru),
        (&(mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.us_dot11VHTRxMCSMap)),
        sizeof(mac_rx_max_mcs_map_stru)) != EOK) {
        OAM_ERROR_LOG0(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_config_user_set_vht_hdl::memcpy_s failed.}");
    }
    st_vht_hdl.bit_rx_highest_rate =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.ul_dot11VHTRxHighestDataRateSupported;
    if (memcpy_s(&st_vht_hdl.st_tx_max_mcs_map, sizeof(mac_tx_max_mcs_map_stru),
        (&(mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.us_dot11VHTTxMCSMap)),
        sizeof(mac_tx_max_mcs_map_stru)) != EOK) {
        OAM_ERROR_LOG0(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_config_user_set_vht_hdl::memcpy_s failed.}");
    }
    st_vht_hdl.bit_tx_highest_rate =
        mac_vap->pst_mib_info->st_wlan_mib_vht_sta_config.ul_dot11VHTTxHighestDataRateSupported;
...
{
    uint32_t rslt;
    uint8_t tmp_rate[WLAN_SUPP_RATES] = {0x8c, 0x12, 0x98, 0x24, 0xb0, 0x48, 0x60, 0x6c};
    /* 更新关联用户的legacy速率集合 */
    hmac_user_init_rates(hmac_user);
    hmac_add_and_clear_repeat_op_rates(tmp_rate, WLAN_SUPP_RATES * sizeof(uint8_t), &(hmac_user->st_op_rates));
    ...
    /* 设置空间流 */
    mac_user_set_num_spatial_stream(&(hmac_user->st_user_base_info), WLAN_SINGLE_NSS);
    ...
    /* 05上面那个函数是从下面函数里面提取出来的，02a直接调用原函数 */
    hmac_user_set_avail_num_space_stream(&(hmac_user->st_user_base_info), hmac_vap->st_vap_base_info.en_vap_rx_nss);
    /* 设置带宽 */
    ...
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
        ...
        hmac_user->st_user_base_info.uc_avail_num_spatial_stream, hmac_user->st_user_base_info.en_cur_bandwidth);
...
oal_uint32 hmac_config_del_user(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 获取用户对应的索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, pst_del_user->auc_mac_addr, WLAN_MAC_ADDR_LEN, &us_user_index);
    if (ul_ret != OAL_SUCC) {
...
/*
 * 函 数 名  : hmac_set_multi_user_allocated
 * 功能描述  : 把申请到的multiuser设置为allocated且memzero
 * 修改历史     :
 * 1.日    期   : 2019年12月5日
 *   作    者   : h00368777
 *   修改内容   : 新生成函数
...
oal_uint32 hmac_user_add_notify_alg(mac_vap_stru *pst_mac_vap, oal_uint16 us_user_idx)
{
    ...
    if (mac_is_chba_mode(pst_mac_vap)) {
        hmac_chba_update_channel_seq(pst_mac_vap);
    }
 ...
 *   修改内容  : 新生成函数
 */
hmac_user_stru *mac_vap_get_hmac_user_by_addr(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac_addr, uint8_t mac_addr_len)
{
    oal_uint32 ul_ret;
    ...
    /* 根据mac addr找sta索引 */
    ul_ret = mac_vap_find_user_by_macaddr(pst_mac_vap, puc_mac_addr, mac_addr_len, &us_user_idx);
    if (ul_ret != OAL_SUCC) {
