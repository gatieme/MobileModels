                                                    ...
                                                    mac_status_code_enum_uint16 *pen_status_code)
{
    ...
    /* DTS2019013108828:秘钥套件个数字段为0时应拒绝对端关联 */
    if (uc_len == 0) {
    ...
    /* DTS2018090700877: 检查pmkid count/pmkid list字段长度合法性 */
 ...
 * 功能描述  : AP 检查接收到的ASOC REQ消息中的SECURITY参数.如出错,则返回对应的错误码
 * 修改历史  :
 * 1.日    期  : 2013年8月31日
 *   作    者  : duankaiyong
 *   修改内容  : 新生成函数
                                           ...
                                           oal_uint8 *puc_ie, oal_int32 l_len)
{
    ...
    /* 获取成对密钥套件个数,最少2个字节长度 */
    if (l_ie_len_left > 2) {
        /* DTS2016060609164，规避套件个数高字节异常为1，目前套件个数不应超过1个字节大小，否则是出现异常 */
