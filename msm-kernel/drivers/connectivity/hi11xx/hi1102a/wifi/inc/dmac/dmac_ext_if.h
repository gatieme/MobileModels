...
typedef enum {
    ...
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_CHBA_PARAMS,
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_CHBA_SELF_CONN_INFO_SYNC,
    DMAC_WLAN_CTX_EVENT_SUB_TYPE_CHBA_TOPO_INFO_SYNC,
#endif
...
typedef struct {
    ...
    oal_uint8 uc_last_is_amsdu;          /* 是否是amsdu大包聚合 */
    oal_uint8 uc_resv[1];
#ifdef _PRE_DEBUG_MODE
...
#define MAC_GET_RX_PAYLOAD_ADDR(_pst_rx_ctl, _pst_buf) (get_netbuf_payload(_pst_buf))
...
OAL_STATIC OAL_INLINE oal_uint8 mac_get_cb_is_bar(mac_tx_ctl_stru *pst_tx_ctrl)
{
    ...
    return ((pst_mac_naked_frame->st_frame_control.bit_type == WLAN_CONTROL) &&
            (pst_mac_naked_frame->st_frame_control.bit_sub_type == WLAN_BLOCKACK_REQ));
#else
...
OAL_STATIC OAL_INLINE oal_uint8 mac_get_cb_is_qosdata(mac_tx_ctl_stru *pst_tx_ctrl)
{
    ...
    if (pst_mac_qos_htc_frame->st_frame_control.bit_type == WLAN_DATA_BASICTYPE) {
        if ((pst_mac_qos_htc_frame->st_frame_control.bit_sub_type == WLAN_QOS_DATA) ||
            (pst_mac_qos_htc_frame->st_frame_control.bit_sub_type == WLAN_QOS_NULL_FRAME)) {
            ul_ret = OAL_TRUE;
        }
    }
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_rx_cb_mac_hdr(mac_rx_ctl_stru *pst_cb_ctrl,
...
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    (((mac_ieee80211_frame_stru *)((oal_uint8 *)pst_tx_ctrl + OAL_MAX_CB_LEN))->st_frame_control.bit_sub_type) =
        uc_subtype;
...
/*
 * 函 数 名  : mac_get_cb_data_frame_type
 * 功能描述  : 获取网络层数据类型
 * 修改历史  :
 * 1.日    期  : 2018年8月24日
 *   作    者  : h00368777
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_is_need_pause_tid(mac_tx_ctl_stru *pst_tx_ctrl,
                                                            oal_uint8 uc_en_need_pause_tid)
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    return;
#else
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_frame_hdr(mac_tx_ctl_stru *pst_tx_ctrl,
                                                    mac_ieee80211_frame_stru *pst_mac_hdr_addr)
{
#if (defined(_PRE_PRODUCT_ID_HI110X_DEV))
    return;
#else
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_is_use_4_addr(mac_tx_ctl_stru *pst_tx_ctrl,
                                                        oal_uint8 uc_en_is_use_4_addr)
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    return;
#else
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_80211_mac_head_type(mac_tx_ctl_stru *pst_tx_ctrl,
                                                              oal_uint8 uc_80211_mac_head_type)
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    return;
#else
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_is_seq_ctrl_bypass(mac_tx_ctl_stru *pst_tx_ctrl,
                                                             oal_uint8 uc_en_seq_ctrl_bypass)
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    return;
#else
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE oal_void mac_set_cb_event_sub_type(mac_tx_ctl_stru *pst_tx_ctrl,
                                                         oal_uint8 uc_en_event_sub_type)
{
#if defined(_PRE_PRODUCT_ID_HI110X_DEV)
    return;
#else
...
#ifdef _PRE_DEBUG_MODE_USER_TRACK
extern oal_uint32 mac_user_check_txrx_protocol_change(mac_user_stru *pst_mac_user,
                                                      oal_uint8 uc_present_mode,
                                                      oam_user_info_change_type_enum_uint8 en_type);
#endif

extern oal_uint32 dmac_rx_free_netbuf_list(oal_netbuf_head_stru *pst_netbuf_hdr,
                                           oal_netbuf_stru **pst_netbuf,
                                           oal_uint16 us_nums);

extern oal_uint32 dmac_tid_pause(dmac_tid_stru *pst_tid, oal_uint8 uc_type);
extern oal_uint32 dmac_tid_resume(hal_to_dmac_device_stru *pst_hal_device, dmac_tid_stru *pst_tid,
                                  oal_uint8 uc_type);
extern oal_void mac_set_cb_qosdata(mac_tx_ctl_stru *pst_tx_ctrl, oal_uint8 uc_qos);
extern oal_uint32 mac_vap_set_cb_tx_user_idx(mac_vap_stru *pst_mac_vap, mac_tx_ctl_stru *pst_tx_ctl,
                                             oal_uint8 *puc_data);
