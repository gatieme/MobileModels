...
oal_void mac_init_regdomain(oal_void)
{
    ...
    l_ret = memcpy_s(g_st_mac_regdomain.ac_country, OAL_SIZEOF(g_st_mac_regdomain.ac_country),
        ac_default_country, OAL_SIZEOF(ac_default_country));
    if (l_ret != EOK) {
    ...
    *************************************************************************/
    ...
    pst_regclass->ul_channel_bmap = MAC_GET_CH_BIT(MAC_CHANNEL1) | MAC_GET_CH_BIT(MAC_CHANNEL2) |
                                 ...
                                 oal_uint8 *puc_len)
{
    ...
    for (uc_chan_idx = 0; uc_chan_idx < MAC_CHANNEL_FREQ_5_BUTT; uc_chan_idx++) {
        ...
        ul_ret = mac_get_channel_num_from_idx(MAC_RC_START_FREQ_5, uc_chan_idx, &uc_chan_num);
        if (ul_ret != OAL_SUCC) {
            oam_warning_log0(0, OAM_SF_ANY, "{mac_set_country_ie_5g::mac_get_channel_num_from_idx fail.}");
        }
 ...
 *     修改内容   : 新生成函数
 */
oal_uint32 mac_get_channel_idx_from_num(oal_uint8 uc_band,
...
mac_regclass_info_stru *mac_get_channel_num_rc_info(oal_uint8 uc_band, oal_uint8 uc_ch_num)
{
    ...
    if (mac_get_channel_idx_from_num(uc_band, uc_ch_num, &uc_channel_idx) != OAL_SUCC) {
        oam_warning_log2(0, OAM_SF_ANY, "{mac_get_channel_num_rc_info::mac_get_channel_idx_from_num failed. \
            band:%d, ch_num:%d", uc_band, uc_ch_num);
        return OAL_PTR_NULL;
    }
...
oal_uint32 mac_regdomain_set_country(oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    /* 更新管制域信息 */
    if (memcpy_s((oal_uint8 *)&g_st_mac_regdomain, sizeof(g_st_mac_regdomain),
        (oal_uint8 *)pst_mac_regdom, ul_size) != EOK) {
