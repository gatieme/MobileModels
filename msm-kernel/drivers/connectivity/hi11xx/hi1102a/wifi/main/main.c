...
oal_int32 g_wifi_init_ret = 0;
oal_int32 g_wifi_init_flag = 0;

static struct completion *s_wifi_sysfs_init = NULL;

oal_int32  wifi_sysfs_init(oal_void)
{
#ifdef CONFIG_ARCH_QCOM
    s_wifi_sysfs_init = hi11xx_get_wifi_plat_completion();
    if (s_wifi_sysfs_init == NULL) {
        OAL_IO_PRINT("s_wifi_sysfs_init is NULL\n");
        return 0;
    }
    if (g_wifi_init_flag == 0) {
        if (oal_wait_for_completion_timeout(s_wifi_sysfs_init,
            (oal_uint32)oal_msecs_to_jiffies(WIFI_INIT_WAIT_TIMEOUT)) == 0) {
            OAL_IO_PRINT("s_wifi_sysfs_init is timeout\n");
        }
        g_wifi_init_ret = hi1102_host_main_init();
        g_wifi_init_flag = 1;
    } else {
