...
#define wal_msg_req_stru_init(name)    \
    do {                                                             \
        memset_s((oal_void *)(&(name)), OAL_SIZEOF(name), 0, OAL_SIZEOF(name)); \
        (name).ul_request_address = (oal_ulong)(uintptr_t)(&(name));       \
    } while (0)
...
#endif
extern oal_uint32 hmac_config_send_check_data(mac_vap_stru *pst_mac_vap,
                                              oal_uint8 uc_type);
...
#endif
extern oal_uint32 hmac_config_set_cus_priv(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param);
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
extern oal_uint32 wal_config_tas_rssi_access(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param);
#endif
extern oal_net_device_stru *wal_config_get_netdev(const int8_t *pc_name, int32_t array_len);
#ifdef __cplusplus
#if __cplusplus
}
#endif
