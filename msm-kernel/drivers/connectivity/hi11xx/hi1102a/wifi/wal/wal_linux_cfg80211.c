                            ...
                            mac_vap_stru *pst_mac_vap, oal_beacon_parameters *pst_beacon_info)
{
    ...
    /* 找到wmm ie，顺便判断下uapsd是否使能 */
        ...
        /* 填写 msg 消息体 */
#ifdef _PRE_WLAN_FEATURE_P2P
        /* DTS2018042406001 解决Go不支持UAPSD的问题 */
...
/*
 * 函 数 名  : hmac_chan_meas_scan_complete_handler
 * 功能描述  : 信道测量扫描打断处理函数
 * 修改历史
 * 1.日    期  : 2021年12月15日
 *   作    者  : lwx654807
    ...
    uint32_t flags, uint8_t *mac_addr, uint8_t *mac_addr_mask)
{
    ...
    uint8_t idx;
    /*
     * DTS2022070501057 flags的bit3为1表示上层下发随机mac扫描。
...
#endif
{
    ...
    pst_mac_vap = oal_net_dev_priv(pst_netdev);
    if (pst_mac_vap == NULL) {
        /* DTS2017051010360，STA切换到AP模式时，VAP已经删除，此时找不到vap属正常 */
        oam_warning_log0(0, OAM_SF_CFG, "{wal_cfg80211_scan::can't get mac vap from netdevice priv data!}");
        goto fail;
    }

    /* DTS2018120608200关联阶段不下发，规避关联成功瞬间WLAN界面空白的不良体验 */
...
/*
 * 函 数 名  : wal_free_connect_param_resource
                               ...
                               oal_cfg80211_conn_stru *pst_sme)
{
    ...
    if (l_ret != OAL_SUCC) {
        ...
        /* DTS2018093000933:同步1103修改，关联命令下发成功，在HMAC释放内存；否则在WAL释放内存 */
                                  ...
                                  oal_uint16 us_reason_code)
{
    ...
    /* 填写和sta关联的ap mac 地址 */
    pst_mac_vap = oal_net_dev_priv(pst_net_device);
    /* Begin：DTS2017052301136 防止删除p2p组时清空了netdev下的vap，这时disconnect，出现netdev存在，vap被清空，从而使用空指针问题 */
    if (pst_mac_vap == OAL_PTR_NULL) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_disconnect::pst_mac_vap is null!}\r\n");
        return -OAL_EFAIL;
    }
    /* End；DTS2017052301136 */
...
/*
 * 函 数 名  : wal_pno_scan_ssid_enabled
                                                   ...
                                                   oal_station_info_stru *pst_stats)
{
    ...
    /* DTS2019011702246:防止窄带模式下信号太弱上层踢用户 */
...
/*
 * 函 数 名  : wal_check_all_vap_state_connecting
 * 功能描述  : check all vap state in case other vap is connecting now
 * 修改历史  :
 * 1.日    期  : 2019年2月27日
 *   作    者  : baokaijie
 *   修改内容  : 新生成函数
...
/*
 * 函 数 名  : wal_drv_init_roc_channel
 * 功能描述  : 保持在指定信道消息参数初始化
 * 修改历史  :
 * 1.日    期  : 2019年2月27日
 *   作    者  : baokaijie
                                    ...
                                    oal_uint32 ul_wait)
{
    ...
    // DTS2018042403899 如果是Go, 仍然继续发送(比如Go Device Discoverability Request)
    if (!IS_P2P_GO(pst_mac_vap)) {
        // 如果不是Go，则做如下判断
        /* DTS2017072601352需要offchanel但驱动已经不在roc状态了,此action帧不需要发送 */
                                          ...
                                          oal_uint64 *pull_cookie)
{
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++; /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    /* DTS2018102404984:若检测发现cookie空间用完，则清除最旧的一个cookie */
    ...
    // DTS2017072601352 不管是否需要切离信道，后面发送管理帧时都需要等待一定时间
    if (ul_wait == 0) {
    ...
    uc_retry = 0;
    /* DTS2017072601352发错信道,软件不能一直重传,加入软件重传次数限制 */
    /* 发送失败，则尝试重传 */
    do {
    ...
    // DTS2018042403899 请留意，如果是Go端(通过p2p0)在一次会话中第二次(第一次发送已成功)给第三方发送Device Discoverability Response等帧时，可能会由于对方不再侦听而超时失败。
...
#ifdef _PRE_WLAN_FEATURE_P2P
        /* DTS2018090408506,失败超时需要恢复原vap状态，并取消device侧remain on channel */
        if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_STA_LISTEN) {
