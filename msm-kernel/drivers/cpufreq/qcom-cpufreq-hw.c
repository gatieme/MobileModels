...
#define MIN_VALID_FREQUENCY		556800
		...
		struct device_attribute *attr, char *buf)
{
	...
	c->hard_freq = readl_relaxed(c->base + offsets[REG_DOMAIN_STATE]) &
				GENMASK(7, 0);
	c->hard_freq = DIV_ROUND_CLOSEST_ULL(c->hard_freq * xo_rate, 1000);
...
static int qcom_cpufreq_hw_cpu_init(struct cpufreq_policy *policy)
{
	...
	if (c->dcvsh_irq > 0 && !c->is_irq_requested) {
		...
		c->freq_limit_attr.attr.mode = 0440;
		...
		c->hard_freq_attr.attr.mode = 0440;
		c->hard_freq = U32_MAX;
