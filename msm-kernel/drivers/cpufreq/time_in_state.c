...
#ifdef CONFIG_HUAWEI_PERF_CTRL
static int time_adj_freq_get(int cpu, u64 *timeadjfreq)
{
	unsigned int i;
...
int perf_ctrl_get_cpu_busy_time(void __user *uarg)
{
	...
	for (i = 0; i < busy_time.cpu_count; i++) {
		ret = time_adj_freq_get(i, &busy_time.time_adj_freq[i]);
		if (ret < 0)
			return ret;
	}
...
EXPORT_SYMBOL(perf_ctrl_get_cpu_busy_time);
#endif
...
int time_in_state_update_idle(struct notifier_block *nb,
			      unsigned long val, void *data)
{
	int cpu = *(int *)data;
	long new_idle_idx = (long)val;
	struct time_in_state *icpu = NULL;
	unsigned long flags;
	...
	new_idle_idx = new_idle_idx + 1;
	if (new_idle_idx < 0 || new_idle_idx >= icpu->max_idle_state)
		return -EINVAL;

	spin_lock_irqsave(&icpu->lock, flags);
	update_cur_state(icpu);
	icpu->cur_idle_idx = new_idle_idx;
	spin_unlock_irqrestore(&icpu->lock, flags);
...
static struct notifier_block idle_notifier = {
	.notifier_call = time_in_state_update_idle,
};

int time_in_state_init(void)
{
	...
	return idle_status_notifier_register(&idle_notifier);
...
EXPORT_SYMBOL(time_in_state_init);
