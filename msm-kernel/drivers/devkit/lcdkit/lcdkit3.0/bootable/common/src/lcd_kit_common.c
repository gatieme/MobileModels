...
int lcd_kit_eink_ready_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_eink_ready.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_ready!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_aod.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_READY, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_READY, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_ready mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_ready mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

int lcd_kit_eink_vsys_power_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_eink_power_vsys.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_vsys!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_aod.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_VSYS, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_VSYS, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_vsys mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_vsys mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

int lcd_kit_eink_mcu_power_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_aod.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_mcu_power!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_eink_mcu_power.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_MCU_POWER, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_MCU_POWER, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_mcu_power mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_mcu_power mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

int lcd_kit_eink_reset_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_aod.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_reset!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_eink_reset.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_RESET, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_RESET, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_reset mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_reset mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
int lcd_kit_eink_fastboot_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_aod.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_fastboot!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_eink_fastboot.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_FASTBOOT, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_FASTBOOT, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_fastboot mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_fastboot mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

int lcd_kit_eink_cfg_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_aod.buf == NULL) {
		LCD_KIT_ERR("can not get lcd lcd_eink_cfg!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_eink_cfg.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_EINK_CFG, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_EINK_CFG, adapt_ops);
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd lcd_eink_cfg mode is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd lcd_eink_cfg mode is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
	...
	uint32_t delay)
{
	...
	case EVENT_MIPI_SWITCH:
		...
		break;
	case EVENT_EINK_READY:
		ret = lcd_kit_eink_ready_ctrl(data);
		break;
	case EVENT_EINK_VSYS:
		ret = lcd_kit_eink_vsys_power_ctrl(data);
		break;
	case EVENT_EINK_MCU_POWER:
		ret = lcd_kit_eink_mcu_power_ctrl(data);
		break;
	case EVENT_EINK_RESET:
		ret = lcd_kit_eink_reset_ctrl(data);
		break;
	case EVENT_EINK_FASTBOOT:
		ret = lcd_kit_eink_fastboot_ctrl(data);
		break;
	case EVENT_EINK_CFG:
		ret = lcd_kit_eink_cfg_ctrl(data);
		break;
	case EVENT_NONE:
...
static void lcd_kit_parse_power_vci(const char *np)
{
	/* vci */
	if (power_hdl->lcd_vci.buf != NULL)
		return;
...
static void lcd_kit_parse_power_reset(const char *np)
{
	/* lcd reset */
	if (power_hdl->lcd_rst.buf != NULL)
		return;
...
static void lcd_kit_parse_power_iovcc(const char *np)
{
	/* iovcc */
	if (power_hdl->lcd_iovcc.buf != NULL)
		return;
...
static void lcd_kit_parse_power_vsp(const char *np)
{
	/* vsp */
	if (power_hdl->lcd_vsp.buf != NULL)
		return;
...
static void lcd_kit_parse_power_vsn(const char *np)
{
	/* vsn */
	if (power_hdl->lcd_vsn.buf != NULL)
		return;
...
static void lcd_kit_parse_power_backlight(const char *np)
{
	/* backlight */
...
static void lcd_kit_parse_power_te0(const char *np)
{
	/* TE0 */
...
static void lcd_kit_parse_power_vdd(const char *np)
{
	/* lcd vdd */
	if (power_hdl->lcd_vdd.buf != NULL)
		return;
...
static void lcd_kit_parse_power_aod(const char *np)
{
	if (power_hdl->lcd_aod.buf != NULL)
		return;
...
static void lcd_kit_parse_power_poweric(const char *np)
{
...
static void lcd_kit_parse_power_mipi_switch(const char *np)
{
	if (power_hdl->lcd_mipi_switch.buf != NULL)
		return;
...
static void lcd_kit_parse_power_eink_vsys(const char *np)
{
	/* eink vsys */
	if (power_hdl->lcd_eink_power_vsys.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-vsys",
		&power_hdl->lcd_eink_power_vsys);
}

static void lcd_kit_parse_power_eink_mcu_power(const char *np)
{
	/* eink mcu power */
	if (power_hdl->lcd_eink_mcu_power.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-mcu_power",
		&power_hdl->lcd_eink_mcu_power);
}

static void lcd_kit_parse_power_eink_fastboot(const char *np)
{
	/* eink fastboot */
	if (power_hdl->lcd_eink_fastboot.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-fastboot",
		&power_hdl->lcd_eink_fastboot);
}

static void lcd_kit_parse_power_eink_ready(const char *np)
{
	/* eink ready */
	if (power_hdl->lcd_eink_ready.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-ready",
		&power_hdl->lcd_eink_ready);
}

static void lcd_kit_parse_power_eink_reset(const char *np)
{
	/* eink reset */
	if (power_hdl->lcd_eink_reset.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-reset",
		&power_hdl->lcd_eink_reset);
}

static void lcd_kit_parse_power_eink_test_cfg(const char *np)
{
	/* eink test cfg */
	if (power_hdl->lcd_eink_cfg.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-eink-cfg",
		&power_hdl->lcd_eink_cfg);
}

static void lcd_kit_parse_power_tp_reset(const char *np)
{
	/* tp reset */
	if (power_hdl->tp_rst.buf != NULL)
		return;
...
static int lcd_kit_parse_power(const char *np)
{
	...
	lcd_kit_parse_power_poweric(np);
	lcd_kit_parse_power_mipi_switch(np);
	lcd_kit_parse_power_eink_vsys(np);
	lcd_kit_parse_power_eink_mcu_power(np);
	lcd_kit_parse_power_eink_ready(np);
	lcd_kit_parse_power_eink_reset(np);
	lcd_kit_parse_power_eink_fastboot(np);
	lcd_kit_parse_power_eink_test_cfg(np);
...
static int lcd_kit_parse_common_info(const char *np)
{
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,backlight-set-bit",
