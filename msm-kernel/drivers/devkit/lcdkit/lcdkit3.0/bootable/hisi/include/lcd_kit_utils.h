...
#define RGB_SIZE  72
#endif
#define FLASH_D1_SIZE 12
#define FLASH_D1_CNT 6
#define MAX_ERR_NUM 2

#define WRITE_START 0
#define WRITE_OTP_OVER 1
#define WRITE_ALL_OVER 2
...
struct lcd_kit_tp_color_desc {
	...
	uint32_t second_read_support;
	struct lcd_kit_dsi_panel_cmds cmds;
	struct lcd_kit_dsi_panel_cmds second_read_cmds;
};
...
struct lcd_kit_hbm {
	...
	struct lcd_kit_dsi_panel_cmds screen_on_effect_cmds;
	...
	uint32_t hbm_gamma_local[RGB_SIZE];
#endif
};
...
struct lcd_kit_check_arr {
	uint8_t *buf;
	int cnt;
};
struct lcd_kit_flash_erase {
	u32 support;
	u32 all_write_value;
	u32 pmic_value;
	u32 read_check_num;
	u32 elvdd_set_num;
	u32 check_err_num;
	u32 write_flash_offset;
	struct lcd_kit_check_arr *read_check_arr;
	struct lcd_kit_dsi_panel_cmds sleep_out_cmds;
	struct lcd_kit_dsi_panel_cmds acess_key_cmds;
	struct lcd_kit_dsi_panel_cmds flash_erase_cmds;
	struct lcd_kit_dsi_panel_cmds auth_read_cmds;
	struct lcd_kit_dsi_panel_cmds write_false_cmds;
	struct lcd_kit_dsi_panel_cmds write_true_cmds;
	struct lcd_kit_dsi_panel_cmds write_true2_cmds;
	struct lcd_kit_dsi_panel_cmds otp_flag_cmds;
	struct lcd_kit_dsi_panel_cmds elvdd_set_cmds;
	struct lcd_kit_dsi_panel_cmds pmic1_set_read_cmds;
	struct lcd_kit_dsi_panel_cmds restore_pmic_cmds;
	struct lcd_kit_dsi_panel_cmds elvss_set_cmds;
	struct lcd_kit_dsi_panel_cmds flash_writting_cmds;
	struct lcd_kit_dsi_panel_cmds *read_check_cmds;
};
