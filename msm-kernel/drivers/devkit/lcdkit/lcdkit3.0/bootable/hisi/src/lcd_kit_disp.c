...
static struct hisi_fb_data_type lcd_kit_hisifd_ext;
...
{
	return &lcd_kit_hisifd_ext;
}

static void lcd_kit_dual_panel_sim_disp_switch(struct hisi_fb_data_type *hisifd)
{
...
static void lcd_kit_sot_para_init(void)
{
	int ret;

	if (LCD_ACTIVE_PANEL == LCD_MAIN_PANEL) {
		ret = memcmp(g_lcdcfg_read_from_nv.panel0_sncode,
			lcd_kit_get_panel_barcode(), LCD_NV_SN_CODE_LEN);
		if (ret != 0) {
			LCD_KIT_ERR("inner panel is not original panel!\n");
			return;
		}
		...
		LCD_KIT_INFO("panel-0: dsi0_flag %u, dsi1_flag %u\n",
			g_lcdcfg_read_from_nv.panel0_dsi0_flag,
			g_lcdcfg_read_from_nv.panel0_dsi1_flag);
	} else if (LCD_ACTIVE_PANEL == LCD_EXT_PANEL) {
		ret = memcmp(g_lcdcfg_read_from_nv.panel1_sncode,
			lcd_kit_get_panel_barcode(), LCD_NV_SN_CODE_LEN);
		if (ret != 0) {
			LCD_KIT_ERR("out panel is not original panel!\n");
			return;
		}
		disp_info->sot_err_check.dsi0_repair_flag =
...
static void lcd_kit_read_nv_sot_err(void)
{
	int ret;
	errno_t err;
	struct hisi_nve_info_user user_info;
	struct nve_operators *nve_ops = NULL;
	static bool nv_read_flag[LCD_ACTIVE_PANEL_BUTT] = { false, false };

	if (nv_read_flag[LCD_ACTIVE_PANEL]) {
		LCD_KIT_INFO("lcdcfg nv alreay read\n");
		return;
	}
	nv_read_flag[LCD_ACTIVE_PANEL] = true;
	nve_ops = get_operators(NVE_MODULE_NAME_STR);
	if (!nve_ops) {
		LCD_KIT_ERR("get nve ops fail\n");
		return;
	}
	(void)memset_s(&user_info, sizeof(struct hisi_nve_info_user),
			0, sizeof(struct hisi_nve_info_user));
	(void)memset_s(&g_lcdcfg_read_from_nv, sizeof(struct lcd_kit_nv_sot_cfg),
			0, sizeof(struct lcd_kit_nv_sot_cfg));
	user_info.nv_operation = LCD_NV_READ;
	user_info.nv_number = LCD_NV_NUMBER;
	user_info.valid_size = sizeof(struct lcd_kit_nv_sot_cfg);
	err = strncpy_s(user_info.nv_name, (unsigned int)sizeof(user_info.nv_name),
		"LCDCFG", (unsigned int)sizeof(user_info.nv_name) - 1);
	if (err != EOK)
		LCD_KIT_ERR("nv_name copy error %d\n", err);
	user_info.nv_name[sizeof(user_info.nv_name) - 1] = '\0';
	ret = nve_ops->hisi_nve_direct_access(&user_info);
	if (ret != 0) {
		LCD_KIT_ERR("hisi_nve_direct_access read error %d\n", ret);
		return;
	}
	memcpy_s((void *)&g_lcdcfg_read_from_nv, sizeof(struct lcd_kit_nv_sot_cfg),
		(void *)user_info.nv_data, sizeof(struct lcd_kit_nv_sot_cfg));
	lcd_kit_sot_para_init();
}
	...
	struct hisi_fb_data_type *hisifd)
{
	...
	lcd_kit_dual_panel_sim_disp_switch(hisifd);
	LCD_KIT_INFO("fb%d, +!\n", hisifd->index);
	...
	case LCD_INIT_MIPI_LP_SEND_SEQUENCE:
		...
		lcd_kit_parse_local_hbm_gamma(pinfo, hisifd);
#endif
		/* read project id */
		...
		lcd_kit_flash_write_check(hisifd);
	...
	struct hisi_fb_data_type *hisifd, uint32_t bl_level)
{
	...
	pinfo = hisifd->panel_info;
	if (pinfo && pinfo->display_on_before_backlight) {
		LCD_KIT_INFO("display_on_before_backlight = %d\n", pinfo->display_on_before_backlight);
...
static struct hisi_fb_panel_data lcd_kit_panel_data_ext = {
	.on               = lcd_kit_panel_on,
	.off              = lcd_kit_panel_off,
	.set_backlight    = lcd_kit_set_backlight,
	.next             = NULL,
...
static void lcd_kit_probe_sub(struct hisi_fb_data_type *hisifd)
{
	if (PUBLIC_CFG->product_type == LCD_DUAL_PANEL_SIM_DISPLAY_TYPE) {
		if (hisifd->index == EXTERNAL_PANEL_IDX) {
			lcd_kit_panel_data_ext.next = hisifd->panel_data;
			hisifd->panel_data = &lcd_kit_panel_data_ext;
		}
		if (hisifd->index == PRIMARY_PANEL_IDX) {
		...
		lcd_kit_enable_dual_gmp(hisifd);
#endif
	} else {
		lcd_kit_panel_data.next = hisifd->panel_data;
		hisifd->panel_data = &lcd_kit_panel_data;
...
static int lcd_kit_init(struct system_table *systable)
{
	...
	if (lcd_type_ops->get_lcd_type() == LCD_KIT) {
		...
		disp_info->elvdd_detect.detect_gpio_dts =
			lcd_type_ops->get_elvdd_detect_gpio();
		LCD_KIT_INFO("DTS elvdd detect gpio = %d\n",
			disp_info->elvdd_detect.detect_gpio_dts);
