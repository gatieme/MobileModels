...
#ifdef CFG_DFX_SINGLE_AP
static struct lcd_kit_oeminfo_gmp_blob g_lcd_gmp_blob;
static int g_lcd_gmp_update_index = 0;
#endif
...
{
	return SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_SIZE;
}
...
{
	int ret;
	unsigned long gm_addr = SUB_RESERVED_LCD_GAMMA_MEM_PHYMEM_BASE;
	unsigned long gm_size = SUB_RESERVED_LCD_GAMMA_MEM_PHYMEM_SIZE;
...
#ifdef CFG_DFX_SINGLE_AP
static int lcd_kit_fetch_gmp_from_oeminfo(void)
{
	struct oeminfo_operators *oeminfo_ops = NULL;
	static bool is_fetehed = false;
	if (is_fetehed) {
		LCD_KIT_INFO("gmp info has been updated.\n");
		return LCD_KIT_OK;
	}
	oeminfo_ops = get_operators(OEMINFO_MODULE_NAME_STR);
	if (!oeminfo_ops) {
		LCD_KIT_ERR("get_operators error\n");
		return LCD_KIT_FAIL;
	}

	if (oeminfo_ops->get_oeminfo(OEMINFO_GMP, OEMINFO_GMP_LEN,
		(char *)&g_lcd_gmp_blob)) {
		LCD_KIT_ERR("get gmp oeminfo error\n");
		return LCD_KIT_FAIL;
	}
	is_fetehed = true;
	return LCD_KIT_OK;
...
static void lcd_kit_set_robot_gmp_dts(void *fdt, struct fdt_operators *fdt_ops)
{
	struct lcd_kit_oeminfo_gmp_robot *robot_gmp = NULL;
	static int i = PRIMARY_PANEL_IDX;
	uint32_t tmp[GMP_LUT_LEN] = { 0 };
	int offset, j;

	robot_gmp = (struct lcd_kit_oeminfo_gmp_robot *)&g_lcd_gmp_blob;

	for (; i < g_lcd_gmp_update_index; i++) {
		lcd_kit_panel_switch(i);
		offset = fdt_ops->fdt_path_offset(fdt, disp_info->lcd_name);
		fdt_ops->fdt_setprop_u32(fdt, offset, "lcd-kit,gmp-support", 1); // 1 means enable
		fdt_ops->fdt_setprop_u32(fdt, offset,
			"lcd-kit,parse-effect-from-dts", 1); // 1 means enable

		// every panel has 256 light level
		fdt_ops->fdt_setprop_operators(fdt, offset, "lcd-kit,light-cali-map",
			(const void *)&robot_gmp->brightness_lvl[i * 256], 256);

		// set low32
		memset_s(tmp, sizeof(tmp), 0, sizeof(tmp));
		for (j = 0; j < GMP_LUT_LEN; j++)
			tmp[j] = cpu_to_fdt32(robot_gmp->gmp_info[i].gmp_lut_low32[j]);
		fdt_ops->fdt_setprop_operators(fdt, offset,
			(const char *)"lcd-kit,gmp-lut-table-low32bit", (const void *)tmp,
			GMP_LUT_LEN * sizeof(uint32_t));

		// set high4
		memset_s(tmp, sizeof(tmp), 0, sizeof(tmp));
		for (j = 0; j < GMP_LUT_LEN; j++) {
			tmp[j] = (uint32_t)robot_gmp->gmp_info[i].gmp_lut_high4[j];
			tmp[j] = cpu_to_fdt32(tmp[j]);
		}
		fdt_ops->fdt_setprop_operators(fdt, offset,
			(const char *)"lcd-kit,gmp-lut-table-high4bit", (const void *)tmp,
			GMP_LUT_LEN * sizeof(uint32_t));
	}
}

static int lcd_kit_update_gmp_lut_from_oeminfo(void)
{
	...
	lcd_kit_set_robot_gmp_dts(fdt, fdt_ops);
	return LCD_KIT_OK;
}

static void lcd_kit_gmp_dts_overlay(void)
{
	...
	fdt_ops->register_update_dts_func(lcd_kit_update_gmp_lut_from_oeminfo);
#else
	(void)lcd_kit_update_gmp_lut_from_oeminfo();
#endif
	LCD_KIT_INFO("-\n");
}

int lcd_kit_enable_dual_gmp(struct hisi_fb_data_type *hisifd)
{
	struct lcd_kit_oeminfo_gmp_robot *robot_gmp = NULL;
	static uint32_t tmp_high4[2][GMP_LUT_LEN]; // 2 means dual panel
	uint8_t *high4_addr = NULL;
	int i;
	if (lcd_kit_fetch_gmp_from_oeminfo() < 0)
		return LCD_KIT_FAIL;
	robot_gmp = (struct lcd_kit_oeminfo_gmp_robot *)&g_lcd_gmp_blob;

	if (!hisifd || !hisifd->panel_info) {
		LCD_KIT_ERR("invalid hisifd.\n");
		return LCD_KIT_FAIL;
	}

	hisifd->panel_info->gmp_support = 1; // 1 means enable
	hisifd->panel_info->gmp_lut_table_low32bit =
		robot_gmp->gmp_info[hisifd->index].gmp_lut_low32;
	high4_addr = robot_gmp->gmp_info[hisifd->index].gmp_lut_high4;
	for (i = 0; i < GMP_LUT_LEN; i++)
		tmp_high4[hisifd->index][i] = high4_addr[i];
	hisifd->panel_info->gmp_lut_table_high4bit = tmp_high4[hisifd->index];

	hisifd->panel_info->gmp_lut_table_len = GMP_LUT_LEN;
	g_lcd_gmp_update_index++;
	lcd_kit_gmp_dts_overlay();
	return LCD_KIT_OK;
}
#endif
