...
#if defined(FASTBOOT_DISPLAY_LOGO_DPUV600)
...
int lcd_kit_get_tp_color(struct hisi_fb_data_type *hisifd)
{
	...
	if (disp_info->tp_color.support) {
		...
		if (!pinfo->tp_color && disp_info->tp_color.second_read_support) {
			ret = lcd_kit_dsi_cmds_rx(hisifd, &read_value, 1,
				&disp_info->tp_color.second_read_cmds);
			if (!ret)
				pinfo->tp_color = read_value;
		}
		LCD_KIT_INFO("tp color = %d\n", pinfo->tp_color);
	} else {
		pinfo->tp_color = disp_info->tp_color.default_value;
	}
	return ret;
}
...
#if defined(FASTBOOT_PLAT_BALTIMORE)
int lcd_kit_parse_local_hbm_gamma(struct hisi_panel_info *pinfo,
	struct hisi_fb_data_type *hisifd)
{
	int ret;
	int i;
	uint8_t hbm_gamma_local[RGB_SIZE];
	ret = lcd_kit_dsi_cmds_rx(hisifd, hbm_gamma_local,
		RGB_SIZE, &disp_info->hbm.hbm_60_hz_gamma_read_cmds);
	if (ret < 0) {
		LCD_KIT_ERR("hbm_60_hz_gamma_read_cmds fail\n");
		return LCD_KIT_FAIL;
	}

	for (i = 0; i < RGB_SIZE; i++) {
		disp_info->hbm.hbm_gamma_local[i] = hbm_gamma_local[i];
		LCD_KIT_DEBUG("Local_hbm_gamma[i] = %d\n", i,
			disp_info->hbm.hbm_gamma_local[i]);
	}
	pinfo->hbm_gamma.buf = disp_info->hbm.hbm_gamma_local;
	pinfo->hbm_gamma.cnt = RGB_SIZE;
	return LCD_KIT_OK;
}
#endif
...
static void lcd_kit_parse_tp_color(const char *np)
{
	...
	lcd_kit_parse_get_u32_default(np,
		"lcd-kit,second-read-tp-color-support", &disp_info->tp_color.second_read_support, 0);
	lcd_kit_parse_get_u32_default(np,
		"lcd-kit,tp-color-default", &disp_info->tp_color.default_value, 0);
	lcd_kit_parse_get_u32_default(np,
		"lcd-kit,little-endian-support", &disp_info->little_endian_support, 0);
	if (disp_info->tp_color.support)
		lcd_kit_parse_dcs_cmds(np,
		"lcd-kit,tp-color-cmds",
		"lcd-kit,tp-color-cmds-state",
		&disp_info->tp_color.cmds);
	if (disp_info->tp_color.second_read_support)
		lcd_kit_parse_dcs_cmds(np,
		"lcd-kit,second-read-tp-color-cmds",
		"lcd-kit,tp-color-cmds-state",
		&disp_info->tp_color.second_read_cmds);
}
...
static void lcd_kit_parse_elvdd_detect(const char *np)
{
	...
	if (disp_info->elvdd_detect.support) {
		...
		if (disp_info->elvdd_detect.detect_gpio_dts) {
			LCD_KIT_INFO("DTS elvdd_detect_gpio_dts = %d, XML detect_gpio = %d\n",
				disp_info->elvdd_detect.detect_gpio_dts,
...
static void lcd_kit_parse_flash_erase_cmd(const char *compatible)
{
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,acess-key-cmds",
		"lcd-kit,acess-key-cmds-state",
		&disp_info->flash_erase.acess_key_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,flash-erase-cmds",
		"lcd-kit,flash-erase-cmds-state",
		&disp_info->flash_erase.flash_erase_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,auth-read-cmds",
		"lcd-kit,auth-read-cmds-state",
		&disp_info->flash_erase.auth_read_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,write-false-cmds",
		"lcd-kit,write-false-cmds-state",
		&disp_info->flash_erase.write_false_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,write-true-cmds",
		"lcd-kit,write-true-cmds-state",
		&disp_info->flash_erase.write_true_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,write-true2-cmds",
		"lcd-kit,write-true2-cmds-state",
		&disp_info->flash_erase.write_true2_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,otp-flag-read-cmds",
		"lcd-kit,otp-flag-read-cmds-state",
		&disp_info->flash_erase.otp_flag_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,elvdd-set-cmds",
		"lcd-kit,elvdd-set-cmds-state",
		&disp_info->flash_erase.elvdd_set_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,pmic-set-read-cmds",
		"lcd-kit,pmic-set-read-cmds-state",
		&disp_info->flash_erase.pmic1_set_read_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,restore-pmic-cmds",
		"lcd-kit,restore-pmic-cmds-state",
		&disp_info->flash_erase.restore_pmic_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,elvss-set-cmds",
		"lcd-kit,elvss-set-cmds-state",
		&disp_info->flash_erase.elvss_set_cmds);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,flash-write-cmds",
		"lcd-kit,flash-write-cmds-state",
		&disp_info->flash_erase.flash_writting_cmds);
}

static void lcd_kit_parse_flash_erase_check(const char *compatible)
{
	u32 value;
	u32 buf_size;
	u32 i;
	char cmd_name[] = "lcd-kit,read-check-cmds1";
	char arr_name[] = "lcd-kit,read-check-arr1";

	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,read-check-num",
		&value, 0);
	if (!disp_info->flash_erase.read_check_cmds) {
		disp_info->flash_erase.read_check_cmds = alloc(
			value * sizeof(struct lcd_kit_dsi_panel_cmds));
		if (!disp_info->flash_erase.read_check_cmds) {
			LCD_KIT_ERR("alloc fail\n");
			return;
		}
		disp_info->flash_erase.read_check_arr = alloc(
			value * sizeof(struct lcd_kit_check_arr));
		if (!disp_info->flash_erase.read_check_arr) {
			LCD_KIT_ERR("alloc fail\n");
			return;
		}
		for (i = 0; i < value; i++) {
			cmd_name[sizeof(cmd_name) - 2] = '1' + i;
			arr_name[sizeof(arr_name) - 2] = '1' + i;
			lcd_kit_parse_dcs_cmds(compatible,
				cmd_name,
				"lcd-kit,read-check-cmds-state",
				&disp_info->flash_erase.read_check_cmds[i]);
			lcd_kit_parse_get_u32_default(compatible,
				arr_name, &buf_size, 0);
			disp_info->flash_erase.read_check_arr[i].buf = alloc(buf_size);
			if (!disp_info->flash_erase.read_check_arr[i].buf) {
				LCD_KIT_ERR("alloc fail\n");
				return;
			}
			disp_info->flash_erase.read_check_arr[i].cnt = buf_size;
		}
	}
	disp_info->flash_erase.check_err_num = 0;
	disp_info->flash_erase.read_check_num = value;
}
static void lcd_kit_parse_flash_erase(const char *compatible)
{
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,flash-erase-support",
		&disp_info->flash_erase.support, 0);
	if (!disp_info->flash_erase.support)
		return;
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,pmic-value",
		&disp_info->flash_erase.pmic_value, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,all-write-value",
		&disp_info->flash_erase.all_write_value, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,elvdd-set-num",
		&disp_info->flash_erase.elvdd_set_num, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,write-flash-offset",
		&disp_info->flash_erase.write_flash_offset, 0);
	lcd_kit_parse_dcs_cmds(compatible,
		"lcd-kit,flash-mem-sleep-out-cmds",
		"lcd-kit,flash-mem-sleep-out-cmds-state",
		&disp_info->flash_erase.sleep_out_cmds);
	lcd_kit_parse_flash_erase_cmd(compatible);
	lcd_kit_parse_flash_erase_check(compatible);
}

static void lcd_kit_parse_sot_err_check(const char *compatible)
{
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,check-sot-err-support",
		&disp_info->sot_err_check.support, 0);
	if (!disp_info->sot_err_check.support)
		return;

	lcd_kit_parse_dcs_cmds(compatible, "lcd-kit,sot-err-repair11-cmds",
		"lcd-kit,sot-err-repair-cmds-state",
		&disp_info->sot_err_check.repair11_cmds);
	lcd_kit_parse_dcs_cmds(compatible, "lcd-kit,sot-err-repair12-cmds",
		"lcd-kit,sot-err-repair-cmds-state",
...
static void lcd_kit_parse_aod_gamma_check(const char *compatible)
{
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,aod-check-gamma-support",
		&disp_info->aod.check_gamma_support, 0);
}
...
static int lcd_kit_parse_disp_info(const char *np)
{
	...
	lcd_kit_parse_flash_erase(np);
	lcd_kit_parse_aod_gamma_check(np);
	return LCD_KIT_OK;
}
...
#if !(defined(FASTBOOT_DISPLAY_LOGO_DPUV330) || defined(FASTBOOT_DISPLAY_LOGO_DPUV501))
...
#if defined(FASTBOOT_DISPLAY_LOGO_DPUV501) || defined(FASTBOOT_DISPLAY_LOGO_DPUV330)
...
static void lcd_kit_aod_init(struct hisi_panel_info *pinfo)
{
	...
	if (pinfo->aod_bl[3].aod_level_bl_cmds_len)
		lcd_kit_aod_cmd_init(&(pinfo->aod_bl[3].aod_level_bl_cmds),
			&(disp_info->aod.aod_level4_bl_cmds));
#if defined(FASTBOOT_PLAT_BALTIMORE)
	pinfo->local_hbm_support = disp_info->hbm.local_hbm_support;
	LCD_KIT_INFO("pinfo local hbm support = %d\n", pinfo->local_hbm_support);
	if (pinfo->local_hbm_support) {
		pinfo->screen_on_effect_cmds_len =
			disp_info->hbm.screen_on_effect_cmds.cmd_cnt;
		lcd_kit_aod_cmd_init(&(pinfo->screen_on_effect_cmds),
			&(disp_info->hbm.screen_on_effect_cmds));
		pinfo->circle_color_cmds_len =
			disp_info->hbm.circle_color_cmds.cmd_cnt;
		lcd_kit_aod_cmd_init(&(pinfo->circle_color_cmds),
			&(disp_info->hbm.circle_color_cmds));
		pinfo->enter_circle_cmds_len =
			disp_info->hbm.enter_circle_cmds.cmd_cnt;
		lcd_kit_aod_cmd_init(&(pinfo->enter_circle_cmds),
			&(disp_info->hbm.enter_circle_cmds));
		pinfo->exit_circle_cmds_len =
			disp_info->hbm.exit_circle_cmds.cmd_cnt;
		lcd_kit_aod_cmd_init(&(pinfo->exit_circle_cmds),
			&(disp_info->hbm.exit_circle_cmds));
		pinfo->node_grayscale.buf = disp_info->hbm.node_grayscale.buf;
		pinfo->node_grayscale.cnt = disp_info->hbm.node_grayscale.cnt;
	}
#endif
	...
	pinfo->aod_check_gamma_support = disp_info->aod.check_gamma_support;
}
	...
	struct hisi_panel_info *pinfo)
{
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-height",
		&pinfo->height, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,align-adjust-support",
		&pinfo->align_adjust_support, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-bl-type",
		&pinfo->bl_set_type, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,display-on-before-backlight",
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-bl-boot",
		&pinfo->bl_boot, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,adjust-unlock-pos-y",
		&pinfo->adjust_unlock_pos_y, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,eink-lcd",
		&pinfo->eink_lcd, 0);
	lcd_kit_parse_get_u32_default(np,
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,dual-lcd-support",
	...
	lcd_kit_parse_get_u8_default(np, "lcd-kit,refresh-direction-ctrl",
...
char *lcd_kit_get_panel_barcode(void)
{
	if (LCD_ACTIVE_PANEL == LCD_EXT_PANEL)
		return g_ext_panel_barcode;
	return g_main_panel_barcode;
}
...
static int lcd_add_status_disabled_hifastboot(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(lcd_kit_map); ++i) {
		...
		if (PUBLIC_CFG->product_type == LCD_DUAL_PANEL_SIM_DISPLAY_TYPE) {
			lcd_kit_panel_switch(EXTERNAL_PANEL_IDX);
			if (!strcmp(disp_info->lcd_name, lcd_kit_map[i].lcd_name)) {
				LCD_KIT_INFO("lcd name %s\n", disp_info->lcd_name);
				continue;
			}
			lcd_kit_panel_switch(PRIMARY_PANEL_IDX);
		}
		ret = fdt_ops->fdt_path_offset(fdt, lcd_kit_map[i].lcd_name);
		offset = ret;
		ret = fdt_ops->fdt_setprop_string(fdt, offset,
...
static int lcd_kit_auth_write(struct hisi_fb_data_type *hisifd, int flag)
{
	int ret;
	...
	case WRITE_ALL_OVER:
		...
		break;
	default:
		LCD_KIT_ERR("read auth_val or flag_val fail\n");
		break;
	}
...
static int lcd_kit_parameter_write(struct hisi_fb_data_type *hisifd)
{
	int ret;
	int i;
	int j;
	int offset;
	uint8_t read_value[FLASH_D1_SIZE] = {0};

	ret = lcd_kit_dsi_cmds_tx(hisifd,
		&disp_info->flash_erase.elvdd_set_cmds);
	ret += lcd_kit_dsi_cmds_rx(hisifd, read_value, FLASH_D1_SIZE,
		&disp_info->flash_erase.pmic1_set_read_cmds);
	for (i = 0; i < FLASH_D1_CNT; i++) {
		for (j = 0; j < FLASH_D1_SIZE; j++) {
			offset = i * FLASH_D1_SIZE + j +
				disp_info->flash_erase.write_flash_offset;
			disp_info->flash_erase.elvss_set_cmds.cmds[1].payload[offset] =
				read_value[j] + disp_info->flash_erase.elvdd_set_num;
		}
	}
	ret += lcd_kit_dsi_cmds_tx(hisifd,
		&disp_info->flash_erase.elvss_set_cmds);
...
static void lcd_kit_get_all_check_flash(struct hisi_fb_data_type *hisifd)
{
	u32 i;
	int ret;
	struct lcd_kit_check_arr *check_arr =
		disp_info->flash_erase.read_check_arr;

	for (i  = 0; i < disp_info->flash_erase.read_check_num; i++) {
		ret = lcd_kit_dsi_cmds_rx(hisifd, check_arr[i].buf, check_arr[i].cnt,
			&(disp_info->flash_erase.read_check_cmds[i]));
		if (ret) {
			LCD_KIT_ERR("read err%d\n", i);
			continue;
		}
	}
}

static int lcd_kit_check_flash(struct hisi_fb_data_type *hisifd)
{
	int ret;
	int error_num = 0;
	uint8_t read_value[BUF_MAX] = {0};
	u32 i;
	struct lcd_kit_check_arr *check_arr = disp_info->flash_erase.read_check_arr;

	if (disp_info->flash_erase.check_err_num > MAX_ERR_NUM) {
		LCD_KIT_INFO("check fail max\n");
		return LCD_KIT_OK;
	}

	for (i  = 0; i < disp_info->flash_erase.read_check_num; i++) {
		if (check_arr[i].cnt > BUF_MAX) {
			LCD_KIT_ERR("%dcnt too long:%d\n", i, check_arr[i].cnt);
			error_num++;
			break;
		}
		ret = lcd_kit_dsi_cmds_rx(hisifd, read_value, check_arr[i].cnt,
			&(disp_info->flash_erase.read_check_cmds[i]));
		if (ret) {
			LCD_KIT_ERR("read err%d\n", i);
			error_num++;
		...
		if (memcmp(read_value, check_arr[i].buf, check_arr[i].cnt)) {
			LCD_KIT_ERR("check err%d\n", i);
			error_num++;
			break;
		}
	}
	...
	if (!error_num) {
		LCD_KIT_INFO("check succ\n");
		disp_info->flash_erase.check_err_num = MAX_ERR_NUM + 1;
		return LCD_KIT_OK;
	}
	disp_info->flash_erase.check_err_num++;

	return LCD_KIT_FAIL;
}
static int lcd_kit_read_flash(struct hisi_fb_data_type *hisifd,
	uint8_t *auth_val, uint8_t *flag_val)
{
	int ret;

	if ((!auth_val) || (!flag_val))
		return LCD_KIT_FAIL;
	ret = lcd_kit_dsi_cmds_tx(hisifd,
		&disp_info->flash_erase.sleep_out_cmds);
	ret += lcd_kit_dsi_cmds_tx(hisifd,
		&disp_info->flash_erase.acess_key_cmds);
	ret += lcd_kit_dsi_cmds_rx(hisifd, auth_val, 1,
		&disp_info->flash_erase.auth_read_cmds);
	ret += lcd_kit_dsi_cmds_rx(hisifd, flag_val, 1,
		&disp_info->flash_erase.otp_flag_cmds);
	if (ret) {
		LCD_KIT_ERR("read auth_val or flag_val fail\n");
		return LCD_KIT_FAIL;
...
static int lcd_kit_write_otp(struct hisi_fb_data_type *hisifd,
	uint8_t auth_val, uint8_t flag_val)
{
	int ret;

	LCD_KIT_INFO("auth_byte is 0x%x\n", auth_val);
	if ((auth_val == disp_info->flash_erase.all_write_value) ||
		(disp_info->flash_erase.check_err_num > MAX_ERR_NUM)) {
		LCD_KIT_INFO("need write whole code\n");
		ret = lcd_kit_dsi_cmds_tx(hisifd,
			&disp_info->flash_erase.elvdd_set_cmds);
		ret += lcd_kit_dsi_cmds_tx(hisifd,
			&disp_info->flash_erase.restore_pmic_cmds);
		ret += lcd_kit_dsi_cmds_tx(hisifd,
			&disp_info->flash_erase.elvss_set_cmds);
		if (ret) {
			LCD_KIT_ERR("write restore_pmic fail\n");
			return LCD_KIT_FAIL;
		}
	} else {
		LCD_KIT_INFO("flag_byte is 0x%x\n", flag_val);
		if ((flag_val == disp_info->flash_erase.pmic_value) &&
			(!disp_info->flash_erase.check_err_num)) {
			disp_info->flash_erase.check_err_num = MAX_ERR_NUM + 1;
			return LCD_KIT_FAIL;
		}

		if (lcd_kit_auth_write(hisifd, WRITE_START)) {
			LCD_KIT_ERR("write auth_val fail\n");
			return LCD_KIT_FAIL;
		}

		if (lcd_kit_parameter_write(hisifd)) {
			LCD_KIT_ERR("write parameter fail\n");
			return LCD_KIT_FAIL;
		}
	}
	if (lcd_kit_dsi_cmds_tx(hisifd,
		&disp_info->flash_erase.flash_writting_cmds)) {
		LCD_KIT_ERR("flash writing fail\n");
		return LCD_KIT_FAIL;
	}
...
static void lcd_kit_flash_write(struct hisi_fb_data_type *hisifd)
{
	int ret;
	uint8_t auth_val = 0;
	uint8_t flag_val = 0;
	...
	if (lcd_kit_read_flash(hisifd, &auth_val, &flag_val))
		return;
	if (lcd_kit_write_otp(hisifd, auth_val, flag_val))
		return;
	...
	if ((auth_val == disp_info->flash_erase.all_write_value) ||
		(disp_info->flash_erase.check_err_num > MAX_ERR_NUM))
		ret += lcd_kit_auth_write(hisifd, WRITE_ALL_OVER);
	else
		ret += lcd_kit_auth_write(hisifd, WRITE_OTP_OVER);
	if (ret) {
		LCD_KIT_ERR("write auth_val fail\n");
		return;
	}
	lcd_kit_get_all_check_flash(hisifd);
	...
	LCD_KIT_INFO("flash erase succ\n");
}

void lcd_kit_flash_write_check(struct hisi_fb_data_type *hisifd)
{
	static int on_num = 0;
	...
	if (!on_num) {
		on_num = 1;
		lcd_kit_flash_write(hisifd);
	} else {
		if (lcd_kit_check_flash(hisifd))
			lcd_kit_flash_write(hisifd);
	}
}
