...
struct mipi_detect_cmd {
	u32 support;
	u32 pic_index;
	u32 threshold;
	u32 dual_check;
	u32 err_flag[MIPI_DETECT_NUM];
	u8 reg_val[MIPI_DETECT_LOOP][MIPI_DETECT_NUM][VAL_NUM];
	struct lcd_kit_array_data value[MIPI_DETECT_NUM];
	struct lcd_kit_dsi_panel_cmds rd_cmds[MIPI_DETECT_NUM];
	struct lcd_kit_dsi_panel_cmds enter_cmds[MIPI_DETECT_NUM];
};

struct check_sum2_detect_cmd {
	u32 support;
	u32 dual_dsi_support;
	u32 loop_index;
	u32 threshold;
	u32 delay;
	u32 time;
...
struct lcd_fact_ops {
	...
	ssize_t (*oneside_mode_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*fps_cmd_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*fps_cmd_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*standby_current_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*mipi_detect_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*mipi_detect_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*check_sum2_detect_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*check_sum2_detect_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
	ssize_t (*mbist_detect_show)(struct device *dev,
		struct device_attribute *attr, char *buf);
	ssize_t (*mbist_detect_store)(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
};
...
struct lcd_standby_curr_detect {
	u32 support;
	int channel_id;
	const char *iovcc_id;
	const char *vci_id;
...
struct lcd_fact_desc {
	...
	/* lcd kit mipi error intercept test cmd */
	struct mipi_detect_cmd mipi_detect;
	/* lcd kit check sum2 intercept test cmd */
	struct check_sum2_detect_cmd check_sum2_detect;
	/* mbist detect test witch picture */
	struct mbist_detect_desc mbist_detect;
	/* ldo check */
	...
	/* standby current detect */
	struct lcd_standby_curr_detect standby_current_det;
};

/* variable declare */
struct lcd_fact_desc *lcd_kit_get_fact_info(int panel_id);
#define FACT_INFO lcd_kit_get_fact_info(panel_id)
