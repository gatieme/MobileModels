...
static int lcd_kit_dbg_ddic_bin(char *par);
...
struct lcd_kit_dbg_func item_func[] = {
	...
	{ "PanelDbgDdicBin", lcd_kit_dbg_ddic_bin },
};
...
static struct lcd_kit_dsi_panel_cmds dsi_cmds;

#define READ_DDIC_BUF_LEN                       200
#define DEBUG_HEAD_MAX                          10
#define READ_WRITE_TYPE                         0
#define HEAD_ADDR_INDEX                         1
#define RW_LEN_INDEX                            2
...
static int lcd_kit_debug_clear_file(char *file_path)
{
	mm_segment_t old_fs;
	struct file *fp;

	fp = filp_open(file_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (fp == NULL) {
		LCD_KIT_ERR("open file fail\n");
		return LCD_KIT_FAIL;
	}
...
static int lcd_ddic_debug_fs_write(char *file_path, uint8_t *read_buf, uint32_t write_len)
{
	mm_segment_t old_fs;
	struct file *fp;
	...
	vfs_write(fp, read_buf, write_len, &(fp->f_pos));
...
static int lcd_kit_dbg_mipi_rx(char *par, uint8_t *read_buf, char *dbg_cmds)
{
	int len;
	int ret;

	struct lcd_kit_dbg_ops *dbg_ops = lcd_kit_get_debug_ops();
	...
	if (dbg_ops->dbg_mipi_rx == NULL) {
		LCD_KIT_ERR("dbg_mipi_rx is NULL!\n");
		return LCD_KIT_FAIL;
	}

	ret = dbg_ops->dbg_mipi_rx(read_buf, READ_DDIC_BUF_LEN, &dsi_cmds);
	if (ret == LCD_KIT_FAIL) {
		LCD_KIT_ERR("dbg_mipi_rx fail!\n");
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_dbg_get_read_bytes(void)
{
	int read_byte = 0;
	int dcs_type;
	int i = 0;
	...
	for (i = 0; i < dsi_cmds.cmd_cnt; i++) {
		dcs_type = dsi_cmds.cmds[i].dtype - '\0';
		if ((dcs_type == DTYPE_DCS_READ) || (dcs_type == DTYPE_GEN_READ)
		|| (dcs_type == DTYPE_GEN_READ1) || (dcs_type == DTYPE_GEN_READ2))
			read_byte += dsi_cmds.cmds[i].dlen;
	}

	if (read_byte > READ_DDIC_BUF_LEN)
		read_byte = READ_DDIC_BUF_LEN;

	return read_byte;
}

static int lcd_kit_dbg_write_read_data(uint8_t *read_buf)
{
	int ret;

	ret = lcd_ddic_debug_fs_write(LCD_KIT_READBIN_FILE_PATH, read_buf, READ_DDIC_BUF_LEN);
	if (ret == LCD_KIT_FAIL)
		return LCD_KIT_FAIL;

	read_buf[0] = DEBUG_OK_FLAG;
	ret = lcd_ddic_debug_fs_write(MIPI_FLAG_PATH, read_buf, sizeof(uint8_t));
	if (ret != LCD_KIT_OK)
		return LCD_KIT_FAIL;

	LCD_KIT_INFO("write read data done\n");
...
static int lcd_kit_dbg_cmd(char *par)
{
	...
	int ret;
	char *dbg_cmds;
	struct lcd_kit_dbg_ops *dbg_ops;

	// Read debug_flag.bin's first byte to check whether the read/write is OK
	lcd_kit_debug_clear_file(MIPI_FLAG_PATH);
	...
	ret = lcd_kit_dbg_mipi_rx(par, read_buf, dbg_cmds);
	if (ret == LCD_KIT_FAIL) {
		kfree(dbg_cmds);
		dbg_cmds = NULL;
		return LCD_KIT_FAIL;
	...
	read_byte = lcd_kit_dbg_get_read_bytes();
	if (read_byte > 0) {
		LCD_KIT_INFO("corresponding hex data:\n");
		for (i = 0; i < read_byte; i++) {
			LCD_KIT_INFO("0x%x", read_buf[i]);
			if ((i + 1) % PRI_LINE_LEN == 0)
				LCD_KIT_INFO("\n");
		}
	}

	return lcd_kit_dbg_write_read_data(read_buf);
}

static int lcd_ddic_debug_parse_head(char *buf, uint32_t *head_val)
{
	char *cur = NULL;
	char *token = NULL;
	int i = 0;
	...
	/* parse buf */
	cur = (char *)buf;
	while (!lcd_kit_is_valid_char(*cur))
		cur++;

	token = strsep(&cur, ",");
	while (token) {
		head_val[i++] = simple_strtol(token, NULL, 16);
		token = strsep(&cur, ",");
		if (i >= DEBUG_HEAD_MAX) {
...
{
	uint8_t read_buf[READ_DDIC_BUF_LEN] = {0};
	uint32_t head_val[DEBUG_HEAD_MAX];
	int ret;
	struct lcd_kit_dbg_ops *dbg_ops;
	...
	ret = lcd_ddic_debug_parse_head(par, head_val);
	if (ret != LCD_KIT_OK)
		return LCD_KIT_FAIL;

	LCD_KIT_INFO("head_type:0x%x, head_addr:0x%x, length:0x%x\n", head_val[READ_WRITE_TYPE],
		head_val[HEAD_ADDR_INDEX], head_val[RW_LEN_INDEX]);
	...
	switch (head_val[READ_WRITE_TYPE]) {
	case DEBUG_READ_BIN:
		if (dbg_ops->read_ddic_bin)
			ret = dbg_ops->read_ddic_bin(head_val[HEAD_ADDR_INDEX], head_val[RW_LEN_INDEX]);
		break;
	case DEBUG_WRITE_BIN:
		if (dbg_ops->write_ddic_bin)
			ret = dbg_ops->write_ddic_bin(head_val[HEAD_ADDR_INDEX], head_val[RW_LEN_INDEX]);
		break;
	default:
		LCD_KIT_ERR("invaild head type\n");
		return LCD_KIT_FAIL;
	}
	if (ret != LCD_KIT_OK)
		return LCD_KIT_FAIL;
...
static int lcd_kit_dbg_cmdstate(char *par)
{
	...
	dsi_cmds.link_state = lcd_kit_hex_char_to_value(*par);
	LCD_KIT_INFO("dsi_cmds link_state = %d\n", dsi_cmds.link_state);
	return LCD_KIT_OK;
}
