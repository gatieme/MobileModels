...
static ssize_t lcd_kit_sec_model_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.model_lock);
	if (sysfs_ops->sec_model_show)
		ret = sysfs_ops->sec_model_show(dev, attr, buf);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.model_lock);
	return ret;
}

static ssize_t lcd_kit_camera_prox_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.camera_lock);
	if (sysfs_ops->camera_prox_show)
		ret = sysfs_ops->camera_prox_show(dev, attr, buf);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.camera_lock);
	return ret;
}
...
static ssize_t lcd_kit_alpm_setting_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = lcd_kit_get_sysfs_ops();
	...
	if (sysfs_ops->alpm_function_show)
		ret = sysfs_ops->alpm_setting_show(dev, attr, buf);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_ops->alpm_setting_store)
		count = (size_t)sysfs_ops->alpm_setting_store(dev, attr, buf, count);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.alpm_setting_lock);
	return count;
}
...
static ssize_t lcd_kit_idle_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = lcd_kit_get_sysfs_ops();
	...
	if (!sysfs_ops->idle_mode_store) {
		LCD_KIT_ERR("idle_mode_store is NULL\n");
		return LCD_KIT_FAIL;
	}
	return sysfs_ops->idle_mode_store(dev, attr, buf, count);
}
...
static ssize_t lcd_kit_product_type_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (sysfs_ops->product_type_show)
		return sysfs_ops->product_type_show(dev, attr, buf);
	return LCD_KIT_OK;
}

static ssize_t lcd_kit_amoled_pcd_check_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (sysfs_ops->pcd_check_show)
		return sysfs_ops->pcd_check_show(dev, attr, buf);
	return LCD_KIT_OK;
}

static ssize_t lcd_kit_panel_dieid_info_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	if (sysfs_ops->panel_dieid_info_show)
		ret = sysfs_ops->panel_dieid_info_show(dev, attr, buf);

	return ret;
}

static ssize_t panel_dimming_switch_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.panel_dimming_switch_lock);
	if (sysfs_ops->panel_dimming_switch_store)
		sysfs_ops->panel_dimming_switch_store(dev, attr, buf, count);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.panel_dimming_switch_lock);
	return count;
}
...
static DEVICE_ATTR(alpm_setting, 0644, lcd_kit_alpm_setting_show,
	lcd_kit_alpm_setting_store);
...
static DEVICE_ATTR(display_idle_mode, 0644,
	NULL, lcd_kit_idle_mode_store);
/*lint -e1564*/
...
static DEVICE_ATTR(lcd_product_type, 0644, lcd_kit_product_type_show, NULL);
static DEVICE_ATTR(pcd_check, 0644, lcd_kit_amoled_pcd_check_show, NULL);
/*lint +e1564*/
static DEVICE_ATTR(panel_dieid_info, 0644, lcd_kit_panel_dieid_info_show, NULL);
static DEVICE_ATTR(lcd_camera_prox, 0644, lcd_kit_camera_prox_show, NULL);
static DEVICE_ATTR(panel_dimming_switch, 0644, NULL, panel_dimming_switch_store);
static DEVICE_ATTR(sec_lcd_model, 0644, lcd_kit_sec_model_show, NULL);
...
struct attribute *lcd_kit_conf[] = {
	...
	&dev_attr_display_idle_mode.attr,
	&dev_attr_pre_camera_position.attr,
	&dev_attr_panel_version.attr,
	&dev_attr_lcd_product_type.attr,
	&dev_attr_pcd_check.attr,
	&dev_attr_panel_dieid_info.attr,
	&dev_attr_lcd_camera_prox.attr,
	&dev_attr_panel_dimming_switch.attr,
	&dev_attr_sec_lcd_model.attr,
};
