...
#define RAD_RX_BUFF_LEN 4
#define RAD_CMD_LEN 4
	...
	unsigned short u16_length)
{
	...
	xfer.tx_buf = u8_buf;
	xfer.rx_buf = u8_data;
	/* spi_transfer length need include tx_buf length and rx_buf length */
	xfer.len = RAD_TX_BUFF_LEN + u16_length;
...
void set_u8_mode_val(unsigned char *u8_mode, unsigned short u16_length)
{
	*u8_mode |= RAD_SPI_PDA_MODE_ENABLE;
	...
	/*
	  if rx_buf length equals 4, setting u8_mode at word mode,
	  so that driver could read at once.
	 */
	if (u16_length == RAD_RX_BUFF_LEN)
	...
	char *u8_r_data, unsigned short u16_length)
{
	...
	unsigned char u8_buf[RAD_TX_BUFF_LEN];
	unsigned char u8_data[MAX_READ_PACKET_SIZE + RAD_TX_BUFF_LEN];
	...
	const char *u8_w_data, unsigned short u16_length)
{
	int ret;
	unsigned char u8_mode = RAYDIUM_DMA_ENABLE;
	unsigned char u8_buf[MAX_WRITE_PACKET_SIZE + RAD_TX_BUFF_LEN];
	struct spi_device *sdev = tdev->thp_core->sdev;
	struct spi_message msg;
	struct spi_transfer xfer;
	unsigned char dma_rx_buf;
	...
	u8_buf[0] = RAYDIUM_SPI_NID;
	u8_buf[1] = u32_addr & MASK_8BIT;
	memcpy(u8_buf + RAD_TX_BUFF_LEN, u8_w_data, u16_length);
	memset(&xfer, 0, sizeof(xfer));
	xfer.tx_buf = u8_buf;
	xfer.rx_buf = &dma_rx_buf;
	xfer.len = RAD_TX_BUFF_LEN + u16_length;
...
{
	int ret;
	unsigned char r_buf[RAD_RX_BUFF_LEN];
	unsigned char u8_loop;

	for (u8_loop = 0; u8_loop < RAD_SEND_CMD_RETEY; u8_loop++) {
		mdelay(5); /* wait fw respone, retry delay time */
		ret = touch_driver_spi_read(tdev, RAYDIUM_PDA_COMMAND_ADDR, r_buf, RAD_RX_BUFF_LEN);
		if (ret) {
...
static int touch_driver_send_fw_cmd(struct thp_device *tdev, unsigned char *u8_cmd)
{
	...
	unsigned char w_buf[RAD_CMD_LEN];
	unsigned char r_buf[RAD_RX_BUFF_LEN];
	unsigned char u8_loop;
	...
	ret = touch_driver_spi_write(tdev, cmd_addr, w_buf, RAD_CMD_LEN);
	if (ret) {
...
{
	unsigned char u8_buf[RAD_RX_BUFF_LEN];
	int ret;

	ret = touch_driver_spi_read(tdev, RAYDIUM_CHK_SPI_CMD, u8_buf, RAD_RX_BUFF_LEN);
	if (ret < 0) {
	...
	char *buf, unsigned int len)
{
	...
	ret = touch_driver_spi_read(tdev, RAYDIUM_CHK_PID_CMD, u8_buf, RAD_RX_BUFF_LEN);
	if (ret < 0) {
...
{
	int ret = 0;
	unsigned char u8_wbuffer[RAD_CMD_LEN];
