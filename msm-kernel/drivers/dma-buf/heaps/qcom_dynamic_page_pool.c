...
int dynamic_page_pool_remove_batch(struct dynamic_page_pool *pool,
				   struct list_head *pages, int max_count)
{
	struct page *page;
	int num_pages = 0;

	unsigned long flags;
	if (unlikely(!(pool->high_count + pool->low_count)))
		return 0;

	spin_lock_irqsave(&pool->lock, flags);
	while (num_pages < max_count) {
		if (pool->high_count) {
			page = list_first_entry(&pool->high_items, struct page, lru);
			pool->high_count--;
	...
	if (num_pages) {
		atomic_sub(num_pages, &pool->count);
		mod_node_page_state(page_pgdat(page), NR_KERNEL_MISC_RECLAIMABLE,
					-(num_pages << pool->order));
	}
	spin_unlock_irqrestore(&pool->lock, flags);
	return num_pages;
}
...
static int dynamic_page_pool_shrink(gfp_t gfp_mask, int nr_to_scan)
{
	...
	list_for_each_entry(pool, &pool_list, list) {
		if (only_scan) {
			...
			if (is_qcom_system_heap(pool->dma_heap)) {
				gt_wm_count = qcom_system_pool_gt_wm_pages();
				if (gt_wm_count <= 0)
					continue; /* less than watermark, no shrink */
				if (nr_to_scan > gt_wm_count)
					nr_to_scan = (int)gt_wm_count;
			}
