...
static void rainbow_cache_set(unsigned int level, unsigned int errtype)
{
	...
	err = snprintf(attach_info_buffer, RB_SREASON_STR_MAX, "%s_%s", level_str[level], bit_str[errtype]);
	if (err >= 0)
		rb_sreason_set(attach_info_buffer);
}
...
{
	int ret;
	struct hiview_hievent *hi_event = hiview_hievent_create(err_code);
