/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2017-2022. All rights reserved.
 * Description: communication for emcom module
 * Author: chenshuo gerry.chen@huawei.com
 * Create: 2017-04-20
 */

#ifndef _EMCOM_NETLINK_H
#define _EMCOM_NETLINK_H

#include <net/sock.h>
#include <linux/netlink.h>

#define EMCOM_SOCKET_BASE_EVENT 6
#define EMCOM_SOCKET_INFO_EVENT 7

#define EMCOM_NETLINK_INIT 1
#define EMCOM_NETLINK_EXIT 0

#define EMCOM_SUB_MOD_COMMON 0
#define EMCOM_SUB_MOD_XENIGE 1
#define EMCOM_SUB_MOD_SMARTCARE 2
#define EMCOM_SUB_MOD_NWEVAL 4
#define EMCOM_SUB_MOD_NSTACK 5
#define EMCOM_SUB_MOD_MPROUTE 6
#define EMCOM_SUB_MOD_MAX 7

#define EMCOM_SUB_MOD_MASK 0xFF00
#define EMCOM_SUB_EVT_MASK 0x00FF
#define EMCOM_SUB_MOD_MASK_LEN 8

#define EMCOM_SUB_MOD_COMMON_BASE (EMCOM_SUB_MOD_COMMON << 8)
#define EMCOM_SUB_MOD_XENIGE_BASE (EMCOM_SUB_MOD_XENIGE << 8)
#define EMCOM_SUB_MOD_SMARTCARE_BASE (EMCOM_SUB_MOD_SMARTCARE << 8)
#define EMCOM_SUB_MOD_NWEVAL_BASE (EMCOM_SUB_MOD_NWEVAL << 8)
#define EMCOM_SUB_MOD_NSTACK_BASE (EMCOM_SUB_MOD_NSTACK << 8)
#define EMCOM_SUB_MOD_MPROUTE_BASE (EMCOM_SUB_MOD_MPROUTE << 8)

typedef enum monk_knl_msg_type {
	NETLINK_EMCOM_DK_BASE = EMCOM_SUB_MOD_COMMON_BASE | NLMSG_MIN_TYPE,
	NETLINK_EMCOM_DK_REG,    /* send from apk to register the PID for netlink kernel socket */
	NETLINK_EMCOM_DK_UNREG,      /* when apk exit send this type message to unregister */
	NETLINK_EMCOM_DK_XENIGE_BASE = EMCOM_SUB_MOD_XENIGE_BASE,
	NETLINK_EMCOM_DK_START_ACC,
	NETLINK_EMCOM_DK_STOP_ACC,
	NETLINK_EMCOM_DK_CLEAR,
	NETLINK_EMCOM_DK_RRC_KEEP,
	NETLINK_EMCOM_DK_KEY_PSINFO,
	NETLINK_EMCOM_DK_SPEED_CTRL,
	NETLINK_EMCOM_DK_START_UDP_RETRAN,
	NETLINK_EMCOM_DK_STOP_UDP_RETRAN,
	/* send from xengine to register the UIDs binding MPIP for netlink kernel socket  */
	NETLINK_EMCOM_DK_CONFIG_MPIP,
	/* send from xengine to start the binding MPIP when apks start to be used  */
	NETLINK_EMCOM_DK_START_MPIP,
	NETLINK_EMCOM_DK_STOP_MPIP,    /* send from xengine to stop the binding MPIP  */
	NETLINK_EMCOM_DK_CLEAR_MPIP,
	NETLINK_EMCOM_DK_START_FAST_SYN,
	NETLINK_EMCOM_DK_STOP_FAST_SYN,
	NETLINK_EMCOM_DK_OPMP_INIT_HEARTBEAT,
	NETLINK_EMCOM_DK_OPMP_DEINIT_HEARTBEAT,
	NETLINK_EMCOM_DK_OPMP_TRIGGER_HEARTBEAT,
	NETLINK_EMCOM_DK_ACTIVE_CCALG,
	NETLINK_EMCOM_DK_DEACTIVE_CCALG,
	NETLINK_EMCOM_DK_START_MPFLOW,
	NETLINK_EMCOM_DK_STOP_MPFLOW,
	NETLINK_EMCOM_DK_MPF_RST_LOC_INTF,
	NETLINK_EMCOM_DK_MPF_INIT_BIND_CONFIG,
	NETLINK_EMCOM_DK_MPF_BIND_IP_POLICY,
	NETLINK_EMCOM_DK_MPF_BIND_PORT_POLICY,
	NETLINK_EMCOM_DK_STOP_MPFLOW_V2,
	NETLINK_EMCOM_DK_NOTIFY_NETWORK_INFO,
	NETLINK_EMCOM_DK_CONFIG_MPDNS,
	NETLINK_EMCOM_DK_NOTIFY_MPDNS_RESULT,
	NETLINK_EMCOM_DK_MPF_RST_ALL_FLOW,
	NETLINK_EMCOM_DK_MPF_CHANGE_BURST_RATIO,
	NETLINK_EMCOM_DK_MPF_VLAN_START,
	NETLINK_EMCOM_DK_MPF_VLAN_STOP,
	NETLINK_EMCOM_DK_MPF_VLAN_UPDATE_APP_INFO,
	NETLINK_EMCOM_DK_MPF_VLAN_NW_STATUS_INFO,
	NETLINK_EMCOM_DK_MPF_VLAN_CLEAR,
	NETLINK_EMCOM_DK_SMARTCARE_BASE = EMCOM_SUB_MOD_SMARTCARE_BASE,
	NETLINK_EMCOM_DK_SMARTCARE_FI_APP_LAUNCH,  /* game launch */
	NETLINK_EMCOM_DK_SMARTCARE_FI_APP_STATUS,  /* game status change */
	NETLINK_EMCOM_DK_NWEVAL_BASE = EMCOM_SUB_MOD_NWEVAL_BASE,
	/* to start the rtt report process for evaluation */
	NETLINK_EMCOM_DK_NWEVAL_START_RTT_REPORT,
	/* to stop the rtt report process for evaluation */
	NETLINK_EMCOM_DK_NWEVAL_STOP_RTT_REPORT,
	/* to trigger the rtt evaluation once */
	NETLINK_EMCOM_DK_NWEVAL_TRIGGER_EVALUATION,
	NETLINK_EMCOM_DK_NSTACK_BASE = EMCOM_SUB_MOD_NSTACK_BASE,
	NETLINK_EMCOM_DK_NSTACK_FI_CFG,  /* FI cfg */

	NETLINK_EMCOM_DK_MPROUTE_BASE = EMCOM_SUB_MOD_MPROUTE_BASE | 0,
	NETLINK_EMCOM_DK_MPROUTE_XENGINE,
} emcom_msg_type_en;

typedef enum monk_unsol_knl_msgtype {
	NETLINK_EMCOM_KD_COMM_BASE = EMCOM_SUB_MOD_COMMON_BASE | NLMSG_MIN_TYPE,
	NETLINK_EMCOM_KD_XENIGE_BASE = EMCOM_SUB_MOD_XENIGE_BASE,
	/* send from kernel to xengine to report the network device of MPIP failed  */
	NETLINK_EMCOM_KD_XENIGE_DEV_FAIL,
	NETLINK_EMCOM_KD_MPTCP_SOCKET_CLOSED,
	NETLINK_EMCOM_KD_MPTCP_SOCKET_SWITCH,
	NETLINK_EMCOM_KD_MPTCP_PROXY_FALLBACK,
	NETLINK_EMCOM_KD_MPTCP_FALLBACK,
	NETLINK_EMCOM_KD_OPMP_HEARTBEAT_REPORT,
	NETLINK_EMCOM_KD_MPFLOW_FALLBACK,
	NETLINK_EMCOM_KD_HOST_IND,
	NETLINK_EMCOM_KD_SMARTCARE_BASE = EMCOM_SUB_MOD_SMARTCARE_BASE,
	NETLINK_EMCOM_KD_SMARTCARE_NM,
	NETLINK_EMCOM_KD_SMARTCARE_FI_REPORT_RTT,
	NETLINK_EMCOM_KD_SMARTCARE_FI_REPORT_APP_STATUS,
	NETLINK_EMCOM_KD_NWEVAL_BASE = EMCOM_SUB_MOD_NWEVAL_BASE,
	NETLINK_EMCOM_KD_NWEVAL_RTT_REPORT,
	NETLINK_EMCOM_KD_NWEVAL_RTT_READY,
} emcom_unsol_msg_type_en;

void emcom_send_msg2daemon(int cmd, const void *data, int len);
#endif /* _EMCOM_NETLINK_H */
