...
static int adreno_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "adreno probe begin\n");
	adreno_add_gmu_components(&pdev->dev, &match);

	if (match) {
		dev_info(&pdev->dev, "adreno probe component_master_add_with_match begin\n");
		return component_master_add_with_match(&pdev->dev,
				&adreno_ops, match);
	} else {
		dev_info(&pdev->dev, "adreno probe adreno_bind\n");
...
static int __init kgsl_3d_init(void)
{
	...
	printk(KERN_INFO "kgsl_3d_init begin\n");
	ret = kgsl_core_init();
	if (ret)
		return ret;
	...
	printk(KERN_INFO "kgsl_3d_init end\n");
	return ret;
}

static void __exit kgsl_3d_exit(void)
{
	printk(KERN_INFO "kgsl_3d_exit begin\n");
	platform_driver_unregister(&adreno_platform_driver);
	gmu_core_unregister();
	kgsl_core_exit();
	printk(KERN_INFO "kgsl_3d_exit end\n");
}
