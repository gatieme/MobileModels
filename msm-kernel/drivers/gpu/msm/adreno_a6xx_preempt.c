	...
	bool atomic)
{
	...
	if (!atomic) {
		...
		if (ret) {
			report_dmd_inirq(GPU_OTHER_FAULT, "_update_wptr");
...
static void _a6xx_preemption_done(struct adreno_device *adreno_dev)
{
	...
	if (status & 0x1) {
		...
		report_dmd_inirq(GPU_OTHER_FAULT, "_a6xx_preemption_done");
...
static void _a6xx_preemption_fault(struct adreno_device *adreno_dev)
{
	...
	report_dmd_inirq(GPU_OTHER_FAULT, "_a6xx_preemption_fault");
...
err:
	...
	if (!atomic) {
		report_dmd_inirq(GPU_OTHER_FAULT, "a6xx_preemption_trigger");

		gmu_core_fault_snapshot(device);
		adreno_dispatcher_fault(adreno_dev,
			ADRENO_GMU_FAULT_SKIP_SNAPSHOT);
	} else {
		report_dmd_inirq(GPU_OTHER_FAULT, "a6xx_preemption_trigger1");
