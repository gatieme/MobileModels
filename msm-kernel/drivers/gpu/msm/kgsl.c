...
static void hook_gpu_get_total_used(void *ignore, unsigned long *gpu_total)
{
	*gpu_total = gpu_get_total_used();
}
...
static void hook_memcheck_gpumem_info_show(void *ignore, int unused)
{
	memcheck_gpumem_info_show();
}

static void hook_memcheck_gpumem_get_usage(void *ignore, pid_t pid, u64 *mapped, u64 *total)
{
	memcheck_gpumem_get_usage(pid, mapped, total);
}
...
int __init kgsl_core_init(void)
{
	...
	pr_info("kgsl_core_init begin\n");
	REGISTER_HCK_VNH(gpu_get_total_used, hook_gpu_get_total_used, NULL);
	REGISTER_HCK_VRH(mm_memcheck_gpumem_info_show, hook_memcheck_gpumem_info_show, NULL);
	REGISTER_HCK_VRH(mm_memcheck_gpumem_get_usage, hook_memcheck_gpumem_get_usage, NULL);
