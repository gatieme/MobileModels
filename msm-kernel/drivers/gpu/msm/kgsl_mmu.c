				...
				struct kgsl_memdesc *memdesc)
{
	...
	struct completion *gdsc_gate = NULL;
	struct regulator *cx_gdsc = NULL;
	if (adreno_is_gen7(adreno_dev)) {
		struct gen7_gmu_device *gmu = to_gen7_gmu(adreno_dev);
		gdsc_gate = &gmu->gdsc_gate;
		cx_gdsc = gmu->cx_gdsc;
	...
	if (PT_OP_VALID(pagetable, mmu_map)) {
		...
		if (gdsc_gate && cx_gdsc) {
			ret = wait_for_completion_timeout(gdsc_gate, msecs_to_jiffies(5000));
			if (!ret) {
				dev_err(device->dev, "GPU CX wait timeout. Dumping CX votes:\n");
				/* Dump the cx regulator consumer list */
				qcom_clk_dump(NULL, cx_gdsc, false);
			}
		}
