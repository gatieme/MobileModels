...
#ifdef CONFIG_HW_IPA_THERMAL
extern void hw_thermal_init(struct work_struct *work);
static DECLARE_WORK(hw_thermal_init_work, hw_thermal_init);
#endif
		...
		const char *governor)
{
	...
	if (!IS_ERR(pwrscale->cooling_dev)) {
		/*
		 * register hw_ipa_thermal driver after gpu_cdev successfully registers,
		 * queue hw_thermal_init work.
		 */
		schedule_work(&hw_thermal_init_work);
		pr_info("ipa thermal, gpu_cdev register ok");
	}
#else
