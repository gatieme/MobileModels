...
TRACE_EVENT(gpu_static_power,
	TP_PROTO(int temperature, unsigned long t_scale,
		unsigned long m_volt, unsigned long load,
		unsigned long static_power),

	TP_ARGS(temperature, t_scale, m_volt, load, static_power),

	TP_STRUCT__entry(
		__field(int, temperature)
		__field(unsigned long, t_scale)
		__field(unsigned long, m_volt)
	...
	TP_fast_assign(
		...
		__entry->m_volt = m_volt;
		__entry->load = load;
		__entry->static_power = static_power;
	),

	TP_printk("temperature = %d, t_scale = %lu, m_volt = %lu, load = %lu, static_power = %lu",
		__entry->temperature, __entry->t_scale, __entry->m_volt, __entry->load, __entry->static_power)
);
#endif
#endif /* _KGSL_TRACE_H */
