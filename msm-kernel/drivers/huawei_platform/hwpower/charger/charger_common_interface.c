...
int charge_get_battery_current_avg(void)
{
	if (charge_support_thirdparty_buck())
		return battery_get_bat_avg_current() / 10; /* 10: unit convert to mA */
	else
...
{
	/* 1:valid buff size */
	return power_glink_interface_set_property_value(
...
void charge_set_usbpd_disable(bool flag)
{
	...
	if ((usb_type == POWER_SUPPLY_USB_TYPE_PD) ||
		...
		power_glink_interface_set_property_value(id, &pd_disable, GLINK_DATA_ONE);
...
#if (defined(CONFIG_HUAWEI_POWER_GLINK_INTERFACE) || defined(CONFIG_HUAWEI_BUCK_CHARGER))
...
#endif /* CONFIG_HUAWEI_POWER_GLINK_INTERFACE || CONFIG_HUAWEI_BUCK_CHARGER */
EXPORT_SYMBOL(charge_get_charger_type);
EXPORT_SYMBOL(charge_set_charger_type);
...
{
	return huawei_charger_get_vterm_dec(value);
}

void charge_update_iterm(int iterm)
{
	if (charge_support_thirdparty_buck())
		buck_charge_update_iterm(iterm);
	else
		chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_ITERM, VOTE_CLIENT_FFC, true, iterm);
}
