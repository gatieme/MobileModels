...
static ssize_t power_glink_dbg_set_vsys_min_store(void *dev_data,
	const char *buf, size_t size)
{
	struct power_glink_dbg_info *info = dev_data;
	u32 value = 0;
	...
	if (power_glink_set_property_value(POWER_GLINK_PROP_ID_SET_SYS_MIN, &value, GLINK_DATA_ONE)) {
		hwlog_err("dbg_set_vsys_min fail\n");
		return -EINVAL;
	}

	hwlog_info("dbg_set_vsys_min: %u\n", value);
	return size;
}
...
static int __init power_glink_dbg_init(void)
{
	...
	power_dbg_ops_register("power_glink", "set_vsys_min", (void *)info,
		NULL, power_glink_dbg_set_vsys_min_store);
	hwlog_err("%s %d end\n", __func__, __LINE__);
	return 0;
}
...
MODULE_SOFTDEP("pre: power_glink_module");
