...
#include <linux/hw/ipc_msg.h>
#include <linux/hw/hw_adc.h>
#include <securec.h>
#include "hw_peripheral_tm.h"

#define PM_TM_DEV_NAME	"hw_peripheral_tm"
#ifdef CONFIG_HW_EXTEND_MAILBOX
#define HW_PERIP_RPROC_SEND_ID	HW_RPROC_LPM3_MBX31
#else
#define HW_PERIP_RPROC_SEND_ID	HW_RPROC_LPM3_MBX28
#endif
#define HW_TM_RPROC_RECV_ID		HW_RPROC_LPM3_MBX0
#define HW_PERIP_TO_LPM3_MSG_NUM	2
#define HW_PERIP_RED_BACK_MSG_NUM	2
...
static struct hw_peripheral_tm_chip *g_tm_dev;
int g_tm_debug = 0;
...
static int hw_peripheral_get_highres_temp(struct thermal_zone_device *thermal,
					    int *temp)
{
	...
	volt = hw_adc_get_value(chip->chanel);
	if (volt <= 0) {
	...
	ret = hw_peripheral_ntc_2_temp(chip, temp, volt);
	if (ret < 0) {
...
#ifndef CONFIG_HW_THERMAL_PERIPHERAL_HIGHRESTEMP
static int hw_peripheral_tm_get_temp(struct thermal_zone_device *thermal,
				       int *temp)
{
	int ret;
	...
	ret = hw_peripheral_get_highres_temp(thermal, temp);
	if (ret < 0) {
		pr_err("%s,get temp failed\n\r", __func__);
		return ret;
	}
...
static int hw_peripheral_tm_get_mode(struct thermal_zone_device *thermal,
...
int hw_peripheral_tm_target(union ipc_data *tm_ipc, int *value)
{
	int ret;
	u32 ack_buffer[2] = {0};
	...
	ret = RPROC_SYNC_SEND(HW_PERIP_RPROC_SEND_ID, (rproc_msg_t *)tm_ipc,
			      (rproc_msg_len_t)HW_PERIP_TO_LPM3_MSG_NUM,
			      (rproc_msg_t *)ack_buffer,
			      (rproc_msg_len_t)HW_PERIP_RED_BACK_MSG_NUM);
	if (ret != 0) {
...
static int hw_peripheral_tm_get_trip_temp(struct thermal_zone_device *thermal,
					    int trip, int *temp)
{
	...
	ret = hw_peripheral_tm_target(&tm_ipc, &volt);
	if (ret < 0) {
		pr_err("%s hw_peripheral_tm_target ret %d\n", __func__, ret);
		return -EINVAL;
	}

	ret = hw_peripheral_ntc_2_temp(tm_sensor, temp, volt);
	if (ret < 0) {
		pr_err("%s hw_peripheral_get_temp ret %d volt %d\n",
		       __func__, ret, volt);
		return -EINVAL;
	}
	*temp = (*temp) / 1000;
...
static int hw_peripheral_tm_set_trip_temp(struct thermal_zone_device *thermal,
					    int trip, int temp)
{
	struct periph_tsens_tm_device_sensor *tm_sensor = thermal->devdata;
	u16 volt = 0;
	...
	ret = hw_peripheral_temp_2_ntc(tm_sensor, temp, &volt);
	if (ret < 0) {
		pr_err("%s: hw_peripheral_temp_2_ntc ret %d\n", __func__, ret);
		return -EINVAL;
	}
	...
	ret = hw_peripheral_tm_target(&tm_ipc, NULL);
	if (ret < 0) {
		pr_err("%s hw_peripheral_tm_target ret %d\n", __func__, ret);
		return -EINVAL;
	}
...
static int hw_peripheral_tm_trip_type(struct thermal_zone_device *thermal,
...
static int
hw_peripheral_tm_activate_trip_type(struct thermal_zone_device *thermal,
				      int trip,
...
static struct thermal_zone_device_ops hw_peripheral_thermal_zone_ops = {
#ifdef CONFIG_HW_THERMAL_PERIPHERAL_HIGHRESTEMP
	.get_temp = hw_peripheral_get_highres_temp,
#else
	.get_temp = hw_peripheral_tm_get_temp,
#endif
	.get_mode = hw_peripheral_tm_get_mode,
	.get_trip_type = hw_peripheral_tm_trip_type,
	.activate_trip_type = hw_peripheral_tm_activate_trip_type,
	.get_trip_temp = hw_peripheral_tm_get_trip_temp,
	.set_trip_temp = hw_peripheral_tm_set_trip_temp,
};
...
static int get_ntc_chanel(struct platform_device *pdev, int i)
{
	...
	rc = snprintf_s(temp_buffer, sizeof(temp_buffer),
			sizeof(temp_buffer) - 1, "hw,detect_%s_tm_chanel",
			hw_peripheral_chanel[i]);
	if (rc < 0) {
...
{
	struct device_node *of_node = pdev->dev.of_node; /*lint !e578*/
	const char *eq_channel_ntc_state = NULL;
	char temp_buffer[BOARD_BUFFER_LENGTH] = {0};
	int rc;

	rc = snprintf_s(temp_buffer, sizeof(temp_buffer),
			sizeof(temp_buffer) - 1,
			"hw,detect_%s_tm_state",
			hw_peripheral_chanel[i]);
	if (rc < 0) {
		dev_err(&pdev->dev, "snprintf_s error in %s :%d line\n",
			__func__, __LINE__);
	...
	rc = of_property_read_string(of_node, temp_buffer,
				     &eq_channel_ntc_state);
	if (rc != 0) {
		dev_err(&pdev->dev, "detect %d tm ntc state failed\n", i);
		return -EINVAL;
	}
	if (strncmp(eq_channel_ntc_state, "enable",
		    strlen(eq_channel_ntc_state)) == 0) {
		g_tm_dev->sensor[i].state = 1;
	} else if (strncmp(eq_channel_ntc_state, "disable",
		   strlen(eq_channel_ntc_state)) == 0) {
...
{
	struct device_node *of_node = pdev->dev.of_node; /*lint !e578*/
	const char *eq_channel_ntc_name = NULL;
	char temp_buffer[BOARD_BUFFER_LENGTH] = {0};
	int rc;
	...
	g_tm_dev->sensor[i].ntc_name =
		(const char *)kmemdup((const void *)eq_channel_ntc_name,
				      strlen(eq_channel_ntc_name) + 1,
				      GFP_KERNEL);
...
static int get_periph_tm_device_tree_data(struct platform_device *pdev)
{
	...
	rc = of_property_read_s32(of_node, "hw,peripheral_sensors",
	...
	g_tm_dev = devm_kzalloc(&pdev->dev, sizeof(struct hw_peripheral_tm_chip) +
				periph_tsens_num_sensors *
...
static int hw_tm_m3_notifier(struct notifier_block *nb, unsigned long len,
			       void *msg)
{
	int i, hw_channel;
...
static int parse_adc_table(void)
{
	...
	for (i = 0; i < g_tm_dev->tsens_num_sensor; i++) {
		...
		(void)hw_peripheral_get_table_info(g_tm_dev->sensor[i].ntc_name,
						     NULL, &t_id);
		g_tm_dev->chub->adc_table[used_tbl_idx].table_id = t_id;
		s_size = hw_peripheral_get_table_info(NULL, &src, &t_id);
...
static int hw_peripheral_tm_probe(struct platform_device *pdev)
{
	int rc, i, flag;
	...
	for (i = DETECT_SYSTEM_H_CHANEL, flag = DETECT_SYSTEM_H_CHANEL;
...
#ifdef CONFIG_HW_THERMAL_TRIP
		if ((enum hw_peripheral_temp_chanel)i == DETECT_SYSTEM_H_CHANEL) {
		...
		g_tm_dev->sensor[i].tz_dev =
			thermal_zone_device_register(name, trips_num, (int)mask,
						     &g_tm_dev->sensor[i],
						     &hw_peripheral_thermal_zone_ops,
						     NULL, 0, 0);
		flag++;
	...
	g_tm_dev->nb.notifier_call = hw_tm_m3_notifier;
	rc = RPROC_MONITOR_REGISTER(HW_TM_RPROC_RECV_ID, &g_tm_dev->nb);
	if (rc != 0) {
...
static int hw_peripheral_tm_remove(struct platform_device *pdev)
{
	struct hw_peripheral_tm_chip *chip = platform_get_drvdata(pdev);
	int i;
...
static struct of_device_id hw_peripheral_tm_match[] = {
	{ .compatible = "hw,hw-peripheral-tm", },
	{}
};

static struct platform_driver hw_peripheral_tm_driver = {
	.probe = hw_peripheral_tm_probe,
	.remove = hw_peripheral_tm_remove,
	.driver = {
		.name = PM_TM_DEV_NAME,
		.owner = THIS_MODULE,
		.of_match_table = hw_peripheral_tm_match,
	},
};

static int __init hw_peripheral_tm_init(void)
{
	return platform_driver_register(&hw_peripheral_tm_driver);
}

static void __exit hw_peripheral_tm_exit(void)
{
	platform_driver_unregister(&hw_peripheral_tm_driver);
}
...
int periph_get_temp(u32 sensor, int *temp)
{
	...
	for (i = 0; i < g_tm_dev->tsens_num_sensor; i++) {
		if ((sensor + TSENSOR_USED_NUM) == g_tm_dev->sensor[i].sensor_num) {
			...
			if (thermal != NULL)
				ret = hw_peripheral_get_highres_temp(thermal, &tmp);
			*temp = tmp;
		}
	}
...
int ipa_get_periph_id(const char *name)
{
	...
	for (id = 0; id < sensor_num; id++) {
		pr_info("IPA: sensor_name = %s, hw_tsensor_name %d = %s\n",
...
{
	int ret = -EINVAL;
	u32 periph_num = sizeof(hw_peripheral_chanel) / sizeof(char *);
...
module_init(hw_peripheral_tm_init);
module_exit(hw_peripheral_tm_exit);
