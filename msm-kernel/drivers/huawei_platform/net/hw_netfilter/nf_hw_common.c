...
const char *strfind(const char *s1, const char *s2)
{
	...
	do {
		...
		if ((s) && (*s) && (*p == *s || tolower(*p) == tolower(*s))) {
			++p;
			++s;
		} else {
			p = s2;
...
const char *strfindpos(const char *s1, int len1, const char *s2, int len2)
{
	if (len1 == 0)
		return NULL;
	if (len1 < len2)
...
const char *substring(const char *src, const char *f, const char *s, int *l)
{
	...
	*l = 0;
	...
	if (strlen(s) == 0)
		return NULL;
	pf = strfindpos(src, datalen, f, flen);
	if (pf == NULL)
		return NULL;
	ps = strfindpos(pf + flen, datalen - (pf + flen - src), s, slen);
	if (ps == NULL)
		return NULL;
	ret = pf + strlen(f);
	*l = ps - ret;
...
char *get_url_form_data(const char *data, int datalen)
{
	...
	if (strfindpos(urlpath, HTTP_HEAD_LEN, "http://", strlen("http://"))) {
