...
struct batt_ct_ops_list {
	...
	int (*ct_ops_register)(struct platform_device *dev,
		struct batt_ct_ops *ops);
	void (*ic_memory_release)(struct platform_device *dev);
};
...
#define dev_get_drv_data(data, dev) \
do { \
	data = dev_get_drvdata(dev); \
	if (!data) \
		hwlog_err("[%s] get drvdata failed\n", __func__); \
} while (0)
...
struct batt_ct_wrapper_ops {
	...
	int (*set_act_signature)(struct batt_chk_data *data, enum res_type type,
		const struct power_genl_attr *res);
...
struct batt_chk_data {
	...
	int ic_cycles;
	int report_cycles;
	unsigned int max_defer_second;
	unsigned int skip_checking_act;
