	...
	unsigned char subcmd, void *data, unsigned int len)
{
	...
	if (send_batt_info_mesg(&info, data, len)) {
		hwlog_err("[%s] checker no:%d group:%d send %d attr fail\n",
...
static int prepare_key_res(struct batt_chk_data *drv_data)
{
	...
	if (!drv_data->key_res.data || (drv_data->key_res.len == 0)) {
		hwlog_err("[%s] battery authentication resource error %d\n",
			__func__, drv_data->key_res.len);
		return -1;
	}
...
static int check_ecc_key(struct batt_chk_data *drv_data)
{
	...
	hwlog_info("[%s] checker no:%d group:%d certification start\n",
		__func__, drv_data->id_in_grp, drv_data->id_of_grp);
	if (bco->prepare(drv_data, RES_CT, &res)) {
		hwlog_err("[%s] checker no:%d group:%d prepear for key failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		return -1;
	}
	...
	if (bco->certification(drv_data, &drv_data->key_res,
		...
		hwlog_err("[%s] checker no:%d group:%d certification failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		return -1;
	}
...
{
	struct power_genl_attr ecc_res;
	struct power_genl_attr act_res;
	...
	if (bco->prepare(drv_data, RES_CT, &ecc_res) || !ecc_res.data ||
		...
		hwlog_err("[%s] checker no:%d group:%d prepear for ecc sign %d failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp, ecc_res.len);
		return -1;
	}
	...
	if (bco->prepare(drv_data, RES_ACT, &act_res) || !act_res.data ||
		...
		hwlog_err("[%s] checker no:%d group:%d prepear for act sign %d failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp, act_res.len);
		return -1;
	}
...
void byc_key_func(struct work_struct *work)
{
	...
	hwlog_info("[%s] checker no:%d group:%d prepare to check\n",
		__func__, drv_data->id_in_grp, drv_data->id_of_grp);
#ifdef BATTBD_FORCE_MATCH
	...
	if (drv_data->result.key_status != KEY_PASS)
		return;

	if (check_act_key(drv_data) ||
		(drv_data->act_state == SIGN_FAIL)) {
		hwlog_err("[%s] checker no:%d group:%d battery act failed %d\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp,
			drv_data->act_state);
...
void byc_act_sign_func(struct work_struct *work)
{
	...
	hwlog_info("[%s] checker no:%d group:%d prepare to act\n",
		__func__, drv_data->id_in_grp, drv_data->id_of_grp);
...
sn_func_entry:
	if (bco->prepare(drv_data, RES_SN, &res)) {
		hwlog_err("[%s] checker no:%d group:%d key prepear failed\n",
	...
	if (bco->get_batt_sn(drv_data, &drv_data->sn_res, &drv_data->sn,
		&drv_data->sn_len)) {
		hwlog_err("[%s] battery no:%d group:%d get batt sn failed %d\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp, sn_fail);
...
static void check_uuid_bind(struct batt_chk_data *drv_data)
{
	...
	if (!bco->get_ic_uuid) {
		hwlog_err("[%s] battery no:%d group:%d get_ic_uuid is null\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		drv_data->result.sn_status = SN_FAIL_IC;
		return;
	}
	...
	if (bco->get_ic_uuid(drv_data, &uuid, &len)) {
		hwlog_err("[%s] battery no:%d group:%d get uuid failed\n",
			__func__, drv_data->id_in_grp, drv_data->id_of_grp);
		drv_data->result.sn_status = SN_FAIL_IC;
		return;
	}
	...
	hwlog_info("[%s] battery no:%d group:%d uuid check status %d\n",
		__func__, drv_data->id_in_grp, drv_data->id_of_grp,
...
static void byc_rematchable_update(struct batt_chk_data *drv_data)
{
	...
	hwlog_info("[%s] set battery unrematchable, ic cycles:%d:%d\n",
		__func__, drv_data->ic_cycles, drv_data->free_cycles);
}

static int update_group_sn(struct batt_chk_data *drv_data)
{
	if (drv_data->check_sn_type != CHECK_GROUP_SN) {
		hwlog_err("[%s] it's single battery\n", __func__);
		return 0;
	}
	if (!drv_data->bco.set_batt_safe_info) {
		hwlog_err("[%s] not support\n", __func__);
		return 0;
	}
	if (!drv_data->group_sn) {
		hwlog_err("[%s] group sn NULL\n", __func__);
		return -1;
	}
	if (drv_data->bco.set_batt_safe_info(drv_data,
		BATT_MATCH_INFO, (void *)&drv_data->group_sn)) {
		hwlog_err("[%s] set group sn fail\n", __func__);
		return -1;
	}
	return 0;
}
	...
	int real_cycles)
{
	...
	if (update_group_sn(drv_data))
		return;

	if (drv_data->bco.set_batt_safe_info(drv_data,
		BATT_SPARE_PART, &temp)) {
		hwlog_err("[%s] clear battery spare sign failed\n", __func__);
	} else {
		drv_data->batt_src_current = BATTERY_ORIGINAL;
		hwlog_info("[%s] clear battery spare sign, ic cycles:%d:%d\n",
			__func__, drv_data->ic_cycles, drv_data->spare_cycles);
	}
}
	...
	int current_cycles)
{
	...
	if (drv_data->bco.set_batt_safe_info(drv_data,
		...
		hwlog_err("[%s] increase ic charge cycles failed\n", __func__);
		return;
	}
	...
	if (drv_data->bco.get_batt_safe_info(drv_data, BATT_CHARGE_CYCLES,
		&drv_data->ic_cycles)) {
		hwlog_err("[%s] get battery charge cycles from ic failed\n",
			__func__);
		return;
	}

	hwlog_info("[%s] after refresh charge cycles coul:%d, ic:%d\n",
		__func__, drv_data->coul_cycles,
		drv_data->ic_cycles);
...
static void cycles_work_func(struct work_struct *work)
{
	...
	int tmp;
	struct batt_chk_data *drv_data = container_of(work,
		struct batt_chk_data, cycles_work);
	...
	if (tmp == drv_data->report_cycles)
		return;

	drv_data->report_cycles = tmp;
	current_coul_cycles = drv_data->report_cycles;
	hwlog_info("[%s] recv charge cycles:%d, coul:%d, ic:%d\n",
		__func__, current_coul_cycles, drv_data->coul_cycles,
		drv_data->ic_cycles);
...
int byc_register_cycles_listener(struct batt_chk_data *drv_data)
{
	...
	if (drv_data->bco.get_batt_safe_info(drv_data, BATT_SPARE_PART,
		...
		hwlog_err("[%s] get battery cell type from ic failed\n", __func__);
		goto ic_driver_fail;
	}
	...
	drv_data->report_cycles = BATT_INVALID_CYCLES;
