...
#define DEFAULT_BATTERY_PERMIT 1
...
int batt_get_sn_nv(unsigned char *sn, int *len)
{
	int ret;
	int nv_sn_len;
	struct binded_info bbinfo = { 0 };

	if (!sn || !len) {
		hwlog_err("[%s] input sn or len is null\n", __func__);
		return -1;
	}

	ret = power_nv_read(POWER_NV_BBINFO, &bbinfo, sizeof(bbinfo));
	if (ret) {
		hwlog_err("[%s] failed\n", __func__);
		return -1;
	}
	nv_sn_len= strlen(bbinfo.info[0]);
	*len = nv_sn_len;
	memcpy(sn, bbinfo.info[0], nv_sn_len);
	return 0;
}
...
int get_battery_identifier(char *buf, int buf_size, int identifier_index, int identifier_len)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if ((len + identifier_len + 2) > buf_size) {
			hwlog_err("[%s] buff is too small\n", __func__);
			return -EOVERFLOW;
		}

		rc = memcpy_s(buf + len, buf_size, checker_data->sn + identifier_index, identifier_len);
		if (rc != EOK)
			return -EINVAL;
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (checker_data->bco.get_batt_type) {
			if (checker_data->bco.get_batt_type(checker_data,
							    &batt_type, &len)) {
				hwlog_err("[%s] Get battery type error\n",
					  __func__);
				return -1;
			}
			if (len >= name_size) {
				hwlog_err("[%s] buffer is too small found\n",
					  __func__);
				return -1;
			}
...
int get_battery_item_code(char *buf, int buf_len)
{
	struct batt_checker_entry *temp = NULL;
	struct platform_device *pdev = NULL;
	struct batt_chk_data *checker_data = NULL;
	...
	if (!buf || !((buf_len > 0) && (buf_len < ITEM_CODE_BUF_MAX)))
		return -ENXIO;
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!checker_data->sn) {
			...
			if (checker_data->bco.get_batt_sn(checker_data, 0,
				&checker_data->sn, &checker_data->sn_len)) {
				hwlog_err("[%s] get battery sn failed\n", __func__);
				return -EINVAL;
			}
		}

		/* cur_len + item code len + '\n' + '\0' */
		if (len + ITEM_CODE_LEN + 2 > buf_len) {
			hwlog_err("[%s] buff is too small\n", __func__);
			return -EOVERFLOW;
		}

		snprintf(buf + len, ITEM_CODE_LEN + 1, "%s", checker_data->sn);
		buf[ITEM_CODE_LEN + len] = '\n';
		len += ITEM_CODE_LEN + 1;
	}
...
static int compare_battery_sn(struct batt_chk_data *checker_data)
{
	...
	unsigned char nv_sn[MAX_RAW_SN_LEN] = { 0 };
	unsigned int sn_len = 0;

	ret = batt_get_sn_nv(nv_sn, &sn_len);
	if (ret || !nv_sn || (sn_len == 0)) {
		hwlog_err("[%s] get battery sn_nv failed\n", __func__);
		return -1;
	}
	...
	if (sn_len < MAX_RAW_SN_LEN) {
		hwlog_err("[%s] battery sn len %d error\n",
			__func__, sn_len);
		return -1;
	}

	if (memcmp(nv_sn, sn, MAX_RAW_SN_LEN)) {
		hwlog_err("[%s] battery sn is not same with nv\n",
			__func__);
		return 1;
	}
...
static int check_batt_moved_by_group_sn(void)
{
	...
	ret = batt_get_sn_nv(nv_sn, &nv_sn_len);
	if (ret) {
		hwlog_err("[%s] batt_get_sn_nv failed\n", __func__);
		return -1;
	}
...
int check_battery_sn_changed(void)
{
	...
	if (same_nums != g_total_checkers) {
		hwlog_err("[%s] battery checkers number is wrong %d:%d\n",
			__func__, same_nums, g_total_checkers);
		return -1;
	}
...
print_work_mode:
	hwlog_info("[%s] work mode is %s\n", __func__, work_mode_str[work_mode]);
}
	...
	unsigned char subcmd, const void *data, int len)
{
	...
	case ACT_SIGN:
		...
		hwlog_info("[%s] mesg %d result %d for battery checker\n",
			__func__, subcmd, act_result);
		complete(&checker_data->act_sign_complete);
		return;
	default:
		hwlog_err("[%s] unkown subcmd for checker(NO:%d Group:%d)\n",
			  __func__, checker_data->id_in_grp, checker_data->id_of_grp);
		return;
	}
	...
	if (len <= MAX_DATA_LEN && len > 0) {
		...
		hwlog_err("[%s] subcmd get too large data length %d\n", __func__, len);
	}
}
...
static int batt_info_cb(struct sk_buff *skb_in, struct genl_info *info)
{
	...
	if (!info) {
		hwlog_err("[%s] info is null found\n", __func__);
		return -1;
	}

	if (!info->attrs) {
		hwlog_err("[%s] info attrs is null found\n", __func__);
		return -1;
	}

	dev_attr = info->attrs[POWER_GENL_BATT_INFO_DEV_ATTR];
	if (!dev_attr) {
		hwlog_err("[%s] dev_attr is null found\n", __func__);
		return -1;
	}
	data_attr = info->attrs[POWER_GENL_BATT_INFO_DAT_ATTR];
	if (!data_attr) {
		hwlog_err("[%s] data_attr is null found\n", __func__);
		return -1;
	}
	...
	if (len != sizeof(*dev_info)) {
		hwlog_err("[%s] dev attr in batt info mesg len uncorrect\n", __func__);
		return -1;
	}

	pdev = get_checker_pdev(dev_info);
	if (!pdev) {
		hwlog_err("[%s] Can't find battery checker(NO.:%d in Group:%d)\n",
			  __func__, dev_info->id_in_grp, dev_info->id_of_grp);
		return -1;
	}

	hwlog_info("[%s] mesg %d for battery checker no.:%d in group:%d\n",
		__func__, dev_info->subcmd, dev_info->id_in_grp, dev_info->id_of_grp);
...
{
	hwlog_info("[%s] board information going to process\n", __func__);
	if (len != strlen(board_info_cb_mesg[NEW_BOARD_MESG_INDEX])) {
		hwlog_err("[%s] board info callback mesg len %d illegal\n", __func__, len);
		new_board = 0;
		return 0;
	}
...
static int batt_dmd_cb(unsigned char version, void *data, int len)
{
	...
	if (len < head) {
		hwlog_err("[%s] dmd info call back mesg length %d illegal\n", __func__, len);
		return 0;
	}
	...
	if (len < (head + record->content_len)) {
		hwlog_err("[%s] dmd info call back mesg length %d:%d illegal\n",
			  __func__, len, head + record->content_len);
		kfree(record);
		return 0;
	}
...
static int batt_mesg_init(void)
{
	...
	if (ret)
		hwlog_err("[%s] power_genl_add_op failed %d\n", __func__, ret);
	...
	const int *dmd_index, int dmd_num)
{
	...
	if (power_genl_easy_send(POWER_GENL_TP_POWERCT, POWER_GENL_CMD_BATT_FINAL_RESULT,
		...
		hwlog_err("[%s] send battery final result status failed\n", __func__);
	kfree(msg_buf);
...
static int send_board_info(void)
{
	...
	if (power_genl_easy_send(POWER_GENL_TP_POWERCT, POWER_GENL_CMD_BOARD_INFO, 0,
		...
		hwlog_err("[%s] send board information message failed\n", __func__);
		return -1;
	}
...
int send_batt_info_mesg(struct nl_dev_info *info, void *data, unsigned int len)
{
	...
	if (power_genl_send(POWER_GENL_TP_POWERCT, POWER_GENL_CMD_BATT_INFO,
		...
		hwlog_err("[%s] Send attributes failed\n", __func__);
		return -1;
	}
...
static int get_last_check_result(struct batt_chk_rslt *result)
{
	if (power_nv_read(POWER_NV_BLIMSW, result,
		sizeof(*result))) {
		hwlog_err("[%s] get last check result failed\n", __func__);
		return -1;
	}
...
static void record_final_check_result(struct batt_chk_rslt *result)
{
	if (power_nv_write(POWER_NV_BLIMSW, result,
		sizeof(*result)))
		hwlog_err("[%s] record final check result failed\n", __func__);
}
...
static void get_sn_from_nv(struct batt_info *drv_data)
{
	...
	if (bbinfo.version != drv_data->sn_version) {
		hwlog_info("[%s] sn binding nv info version different from dts\n", __func__);
	...
	case PAIR_BIND_VERSION:
		for (i = 0; i < MAX_SN_BUFF_LENGTH; i++) {
			if (memcmp(bbinfo.info[i], sn, sn_len))
				continue;
			hwlog_info("[%s] board and battery well matched\n", __func__);
			if ((i > 0) ||
		...
		break;
	default:
		hwlog_err("[%s] Unkown BBINFO NV version(%u) found\n",
			  __func__, bbinfo.version);
		break;
	}
...
static int check_devs_match(struct batt_info *drv_data)
{
	...
	if (valid_checkers > MAX_VALID_CHECKERS) {
		hwlog_err("[%s] too much(%u) valid battery checkers\n",
			  __func__, valid_checkers);
		return -1;
	}
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		checker_data = platform_get_drvdata(pdev);
		if (!checker_data->sn) {
			hwlog_err("[%s] %s sn is null\n", __func__, checker_data->ic->name);
			kfree(buf);
		...
		if (memcmp(sn, checker_data->sn, sn_len - 1) ||
			...
			drv_data->result.sn_status = SN_SNS_UNMATCH;
			kfree(buf);
			hwlog_err("[%s] sn body match fail\n", __func__);
			return 0;
		}
...
static int record_batt_sn(struct batt_info *drv_data)
{
	...
	if (record_sn_to_nv(&bbinfo))
		hwlog_err("[%s] write sn to nv failed\n", __func__);
	else
		hwlog_info("[%s] write sn to nv success\n", __func__);
...
static int final_sn_checker_1(struct batt_info *drv_data)
{
	...
	if (valid_checkers > 1) {
		if (check_devs_match(drv_data)) {
			hwlog_info("[%s] checking battery match failed\n", __func__);
			return -1;
		}
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (valid_checkers > 1) {
			...
			if (drv_data->sn_version != PAIR_BIND_VERSION) {
				hwlog_err("[%s] PAIR_BIND_VERSION unmatch sn len\n", __func__);
				return -1;
			}
		} else {
			drv_data->sn_len = checker_data->sn_len;
			if (drv_data->sn_version != RAW_BIND_VERSION) {
				hwlog_err("[%s] RAW_BIND_VERSION unmatch sn len\n", __func__);
				return -1;
			}
		}
	}
	...
	if (binding_check(drv_data)) {
		hwlog_err("[%s] checking battery binded with board failed\n", __func__);
		return -1;
	}
	return 0;
}
...
static int final_sn_checker_2(struct batt_info *drv_data)
{
	...
	if (cur_group_sn_str_len < 0) {
		hwlog_err("[%s] snprintf group_sn fail\n", __func__);
		return -1;
	}
	...
	/* prepare group sn for setting battery old */
	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		checker_data->group_sn = drv_data->current_group_sn;
	}
...
static int prepare_bind_info(void)
{
	...
	if (send_board_info()) {
		hwlog_err("[%s] checking stopped by send get board info failed\n", __func__);
		return -1;
	}
...
static void cycles_work_func(struct work_struct *work)
{
	...
	if (tmp != drv_data->batt_cycles) {
		hwlog_info("[%s] recv charge cycles:%d\n", __func__, batt_cycle);
...
static void check_func(struct work_struct *work)
{
	...
	for (i = 0; i < RETRY_TIMES; i++) {
		...
		if (!ret) {
		hwlog_info("[%s] last result: ic %d, key %d, sn %d, mode %d\n",
			__func__, last_result.ic_status, last_result.key_status,
		...
		msleep(1000);
		hwlog_err("[%s] get last result failed, retry times: %d\n", __func__, i);
	}
	...
	if (shield_ct_sign) {
		...
		*final_result = dummy_result;
		...
		if (record_batt_sn(drv_data))
			hwlog_err("[%s] record battery sn failed\n", __func__);
		hwlog_info("[%s] battery ct shielding to dummy result\n", __func__);
	...
	if (!(ret || !is_legal_result(&last_result) ||
		...
		*final_result = last_result;
		...
		hwlog_info("[%s] battery ct will skip checking\n", __func__);
		goto skip_result_status;
	}

	hwlog_info("[%s] real checking started\n", __func__);
	...
	if (drv_data->total_checkers != valid_checkers) {
		...
		hwlog_err("[%s] some checkers failed to probe\n", __func__);
		goto process_result_status;
	}
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			...
			hwlog_info("[%s] checker(NO:%d Group:%d) status illegal\n",
				   __func__, checker_data->id_in_grp,
				   checker_data->id_of_grp);
			goto process_result_status;
		}
	...
	hwlog_info("[%s] checking key status started\n", __func__);
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			*final_result = checker_data->result;
			hwlog_info("[%s] batt(NO:%d Group:%d) key status illegal\n",
				   __func__, checker_data->id_in_grp,
				   checker_data->id_of_grp);
			goto process_result_status;
		}
	...
	hwlog_info("[%s] getting board status started\n", __func__);
	prepare_bind_info();

	hwlog_info("[%s] getting battery status started\n", __func__);
	...
	hwlog_info("[%s] checking all battery checkers's sn started\n",
		   __func__);
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			hwlog_info("[%s] batt(NO:%d Group:%d) sn status illegal\n",
				   __func__, checker_data->id_in_grp,
				   checker_data->id_of_grp);
			goto process_result_status;
		}
	...
	if (drv_data->sn_checker(drv_data)) {
		hwlog_err("[%s] final sn check failed\n", __func__);
		return;
	}
	...
	char *buf, int buf_len, char index)
{
	...
	if ((cur_len + sn_len + 3) > buf_len) {
		hwlog_err("[%s] battery sn length error %d %d %d\n",
			__func__, cur_len, sn_len, buf_len);
		return -1;
	}
...
static ssize_t battery_item_code_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	/* buf_len = (ITEM_CODE_LEN + '\n') * 2 + '\0' */
	int buf_len = (ITEM_CODE_LEN + 1) * 2 + 1;
	char temp_buf[(ITEM_CODE_LEN + 1) * 2 + 1] = { 0 };
	int ret;

	ret = get_battery_item_code(temp_buf, buf_len);

	switch (ret) {
	case -ENXIO:
		return snprintf(buf, PAGE_SIZE, "buff null\n");
	case -ENOTSUPP:
		return snprintf(buf, PAGE_SIZE, "ic error\n");
	case -EINVAL:
		return snprintf(buf, PAGE_SIZE, "get battery sn failed\n");
	case -EOVERFLOW:
		return snprintf(buf, PAGE_SIZE, "buff is too small\n");
	case -EUNATCH:
		return snprintf(buf, PAGE_SIZE, "batt_ct is not supported\n");
	default:
		break;
	}
...
/*
 * battery_permit : always true in QCOM, because anti-private repairs are not adapted
 */
...
static ssize_t battery_permit_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d", DEFAULT_BATTERY_PERMIT);
}
...
int prepare_dmd_no(struct batt_chk_rslt *result)
{
	...
	case IC_PASS:
		break;
	default:
		hwlog_err("[%s] illegal IC checking result %d\n",
			  __func__, result->ic_status);
		break;
	}
	...
	case KEY_PASS:
		break;
	default:
		hwlog_err("[%s] illegal KEY checking result %d\n",
			  __func__, result->key_status);
		break;
	}
	...
	case SN_PASS:
		break;
	default:
		hwlog_err("[%s] illegal SN checking result %d\n",
			  __func__, result->sn_status);
		break;
	}
			      ...
			      int *dmd_no)
{
	...
	if (buff)
		return buff;
	hwlog_info("[%s] final result(ic:%02x, key:%02x, sn:%02x).\n",
		   __func__, result->ic_status, result->key_status, result->sn_status);
...
static void dmd_report_func(struct work_struct *work)
{
	...
	if (dmd_buf) {
		...
		hwlog_err("[%s] prepare dmd mesg failed\n", __func__);
	}
}
...
void dmd_record_reporter(struct work_struct *work)
{
	...
	list_for_each_entry_safe(pos, tmp, &g_dmd_list.dmd_head, node) {
		...
		hwlog_info("[%s] report dmd record %d %d\n", __func__, pos->dmd_no,
			   pos->content_len);
...
static void init_shield_ct_sign(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "shield_ct_sign", &sign))
		hwlog_info("[%s] %s read shield_ct_sign failed\n", __func__, pdev->name);
	else
		hwlog_info("[%s] shield_ct_sign is %d\n", __func__, sign);
...
static struct batt_info *batt_info_data_init(struct platform_device *pdev)
{
	...
	if (ret || (sn_check_type >= ARRAY_SIZE(final_sn_checkers))) {
		hwlog_err("[%s] %s read sn_check_type failed\n", __func__, pdev->name);
		return NULL;
	}
	...
	if (ret || (drv_data->sn_version == ILLEGAL_BIND_VERSION) ||
		...
		hwlog_err("[%s] %s read sn_version failed\n", __func__, pdev->name);
		return NULL;
	}
	...
	if (drv_data->total_checkers == 0) {
		hwlog_err("[%s] no valid checker found under huawei_batt_info\n", __func__);
		return NULL;
	}
...
static int battery_info_probe(struct platform_device *pdev)
{
	...
	if ((work_mode == RECOVERY_MODE) || (work_mode == ERECOVERY_MODE)) {
		hwlog_info("[%s] Recovery mode not support now\n", __func__);
	...
	if (batt_mesg_init()) {
		hwlog_err("[%s] %s general netlink initialize failed\n", __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
	/* battery node initialization */
	if (batt_info_node_create(pdev)) {
		hwlog_err("[%s] %s battery information nodes create failed\n",
			  __func__, pdev->name);
		return BATTERY_DRIVER_FAIL;
	}
