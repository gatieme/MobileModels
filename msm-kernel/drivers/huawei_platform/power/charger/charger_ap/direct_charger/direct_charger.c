...
void direct_charge_exit(void)
{
	...
	dc_adpt_clear_inherent_info();
	dc_comm_set_icon_send_flag(false);
}
...
static int direct_charge_get_imax_by_time(struct direct_charge_device *di)
{
	...
	if (direct_charge_turbo_get_time_para(&para, &size)) {
...
void direct_charge_select_charging_param(void)
{
	...
	adp_max_cur = dc_adpt_get_curr(l_di->local_mode, l_di->adaptor_vset) *
		l_di->dc_volt_ratio;
...
void direct_charge_select_charging_stage(void)
{
	...
	if (source_info.output_curr < l_di->ibat_abnormal_th)
...
void direct_charge_stop_charging(void)
{
	...
	if (l_di->stop_charging_sleep_enable && (batt_volt > 4400))
