...
static int fcp_start_charging(struct huawei_battery_info *info)
{
	...
	wired_connect_send_icon_uevent(ICON_TYPE_QUICK);
#endif
	power_event_bnc_notify(POWER_BNT_FCP, POWER_NE_FCP_CHARGING_START, NULL);
...
#if IS_ENABLED(CONFIG_HUAWEI_POWER_GLINK_INTERFACE)
	huawei_charger_set_fcp_current_limit(FCP_CURRET_LIMIT);
#endif
...
void fcp_charge_check(struct huawei_battery_info *info)
{
	...
	hwlog_err("in fcp_charge_check, stage = %u, success = %d\n",
		hvdcp_get_charging_stage(), HVDCP_STAGE_SUCCESS);

	if (charge_get_monitor_work_flag() == CHARGE_MONITOR_WORK_NEED_STOP)
		return;

	power_if_kernel_sysfs_get(POWER_IF_OP_TYPE_HVC, POWER_IF_SYSFS_ENABLE_CHARGER, &fcp_enable);
	if (!charge_fcp_enable() || !fcp_enable) {
		hwlog_info("[%s] fcp charge already disable\n", __func__);
		if (info->chr_type == CHARGER_TYPE_FCP) {
			hvdcp_set_charging_stage(HVDCP_STAGE_DEFAUTL);
			charge_set_charger_type(CHARGER_TYPE_STANDARD);
		}
		return;
