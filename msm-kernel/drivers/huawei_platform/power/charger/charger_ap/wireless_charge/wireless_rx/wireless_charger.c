...
static void wireless_charge_stop_charging(struct wlrx_dev_info *di)
{
	...
	wlrx_buck_set_iin_prop(WLTRX_DRV_MAIN, 0, 0, WLRX_BUCK_DFLT_IMAX);
...
static void wlrx_wls_disconnect_set_wired_channel(void)
{
	...
	if (!wdcm_dev_exist()) {
		...
		if ((mode == VBUS_CH_NOT_IN_OTG_MODE) && dts && dts->wired_sw_dflt_on)
...
void wlrx_switch_to_wired_handler(void)
{
	...
	vbus_ch_get_mode(VBUS_CH_USER_WR_TX, VBUS_CH_TYPE_BOOST_GPIO, &mode);
	if (mode == VBUS_CH_IN_OTG_MODE) {
		hwlog_info("[switch_to_wired_handler] already in sink_otg state\n");
		return;
	}
...
EXPORT_SYMBOL_GPL(wlrx_switch_to_wired_handler);
...
EXPORT_SYMBOL_GPL(wireless_charger_pmic_vbus_handler);
...
static int wireless_charge_probe(struct platform_device *pdev)
{
	...
	ret = wlrx_sysfs_init(WLTRX_DRV_MAIN, &pdev->dev);
	if (ret) {
		hwlog_err("probe: sysfs_init failed\n");
...
alloc_fail:
	wlrx_sysfs_deinit(WLTRX_DRV_MAIN);
sysfs_init_fail:
