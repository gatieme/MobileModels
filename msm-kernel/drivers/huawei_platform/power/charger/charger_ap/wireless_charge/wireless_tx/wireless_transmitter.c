...
static struct delayed_work g_sysfs_check_work;
static struct device *g_wltx_dev;
...
static int wltx_check_abnormal_vbatt(void)
{
	...
	for (i = 0; i < 3; i++) {
		power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbatt,
...
static void wireless_tx_dsm_dump(struct wltx_dev_info *di, char *dsm_buff)
{
	...
	power_supply_get_int_prop(POWER_PLATFORM_USB_PSY_NAME,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &charger_vbus, 0, POWER_UV_PER_MV);
...
static bool wltx_need_check_expect_power(struct wltx_dev_info *di)
{
	struct wltx_dts *dts = wltx_get_dts();
...
static void wltx_show_monitor_info(struct wltx_dev_info *di)
{
	...
	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CAPACITY, &data.soc,
		POWER_SUPPLY_DEFAULT_CAPACITY, 1);
...
static void wireless_tx_monitor_work(struct work_struct *work)
{
	...
	if (wltx_need_check_expect_power(di))
...
static void wltx_sysfs_check_work(struct work_struct *work)
{
	struct wltx_dev_info *di = g_wltx_di;
	...
	hwlog_err("sysfs_check_work: di null, remove sysfs\n");
	wireless_tx_sysfs_remove_group(g_wltx_dev);
}

static void wltx_sysfs_init(struct device *dev)
{
	...
	g_wltx_dev = dev;
	wireless_tx_sysfs_create_group(dev);
	first_in = false;
	INIT_DELAYED_WORK(&g_sysfs_check_work, wltx_sysfs_check_work);
	schedule_delayed_work(&g_sysfs_check_work, msecs_to_jiffies(SYSFS_CHECK_DELAY));
}
...
alloc_fail:
	cancel_delayed_work(&g_sysfs_check_work);
	wireless_tx_sysfs_remove_group(&pdev->dev);
	return ret;
}
