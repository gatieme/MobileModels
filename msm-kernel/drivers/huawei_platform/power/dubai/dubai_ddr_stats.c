...
{
	int32_t freq_cnt = 0;
	trace_android_vh_ddr_stats_get_freq_count(&freq_cnt);
	g_nr_freq = freq_cnt;
...
static int32_t dubai_get_ddr_time_in_state(struct dubai_ddr_time_in_state *time_in_state)
{
	...
	freq_residency = kzalloc(freq_cnt * sizeof(struct ddr_freq_residency), GFP_KERNEL);
	if (!freq_residency) {
		dubai_err("Failed to allocate mem for freq_residency");
		return -1;
	}

	memset(freq_residency, 0, freq_cnt * sizeof(struct ddr_freq_residency));
	trace_android_vh_ddr_stats_get_residency(freq_cnt, freq_residency, &ret);
	if (ret <= 0) {
