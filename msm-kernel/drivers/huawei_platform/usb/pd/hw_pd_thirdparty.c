...
static void drm_unblank_work(struct work_struct *work)
{
...
static int pd_dpm_handle_drm_event(struct notifier_block *self,
	unsigned long event, void *data)
{
	struct drm_panel_notifier *drm_event = data;
	int *blank = NULL;

	if (!g_pd_di || !drm_event)
		return NOTIFY_DONE;
	...
	if ((event == DRM_PANEL_EVENT_BLANK) && (*blank == DRM_PANEL_BLANK_UNBLANK))
		schedule_work(&g_pd_di->drm_work);

	return NOTIFY_DONE;
}

static struct notifier_block pd_dpm_handle_drm_notifier = {
	.notifier_call = pd_dpm_handle_drm_event,
};

static void pd_dpm_drm_notifier_work(struct work_struct *work)
{
	(void)lcd_kit_drm_notifier_register(0, &pd_dpm_handle_drm_notifier);
}
...
int pd_dpm_request_dpdm(bool enable)
{
	...
	if (!g_pd_di->dpdm_reg && of_get_property(g_pd_di->dev->of_node,
				"dpdm-supply", NULL)) {
		hwlog_info("before devm_regulator_get\n");
		g_pd_di->dpdm_reg = devm_regulator_get(g_pd_di->dev, "dpdm");
		if (IS_ERR(g_pd_di->dpdm_reg)) {
			rc = PTR_ERR(g_pd_di->dpdm_reg);
			hwlog_err("Couldn't get dpdm regulator rc=%d\n", rc);
			g_pd_di->dpdm_reg = NULL;
			return rc;
		}
	}
	hwlog_info("%s middle\n", __func__);

	mutex_lock(&g_pd_di->dpdm_lock);
	if (enable) {
		hwlog_info("before enable DPDM regulator\n");
		if (g_pd_di->dpdm_reg && !g_pd_di->dpdm_enabled) {
			hwlog_info("enabling DPDM regulator\n");
			rc = regulator_enable(g_pd_di->dpdm_reg);
			if (rc < 0)
				hwlog_err("Couldn't enable dpdm regulator rc=%d\n", rc);
			else
				g_pd_di->dpdm_enabled = true;
		}
	} else {
		hwlog_info("after disable DPDM regulator\n");
		if (g_pd_di->dpdm_reg && g_pd_di->dpdm_enabled) {
			hwlog_info("disabling DPDM regulator\n");
			rc = regulator_disable(g_pd_di->dpdm_reg);
			if (rc < 0)
				hwlog_err("Couldn't disable dpdm regulator rc=%d\n", rc);
			else
				g_pd_di->dpdm_enabled = false;
		}
	}
	mutex_unlock(&g_pd_di->dpdm_lock);
...
enum cur_cap pd_dpm_get_cvdo_cur_cap(void)
{
	...
	cap = (g_pd_di->cable_vdo_st.cable_vdo & CABLE_CUR_CAP_MASK) >>
		CABLE_CUR_CAP_SHIFT;
	if (cap == PD_DPM_CURR_5A)
		cap += (g_pd_di->cable_vdo_st.cable_vdo_ext &
...
int pd_dpm_get_cable_vdo(void *data, struct pd_dpm_cable_info *cable_vdo)
{
	if (!data)
		return -1;

	memcpy(cable_vdo, data, sizeof(*cable_vdo));
	hwlog_info("%s cable_vdo=%x, add_vdo=%x\n", __func__,
		cable_vdo->cable_vdo, cable_vdo->cable_vdo_ext);
#ifdef SNDJ
...
int pd_dpm_handle_pe_event(unsigned long event, void *data)
{
	...
	case PD_DPM_PE_EVT_TYPEC_STATE:
		{
			...
			case PD_DPM_TYPEC_UNATTACHED:
				...
				g_pd_di->cable_vdo_st.cable_vdo = 0;
				g_pd_di->cable_vdo_st.cable_vdo_ext = 0;
				break;
	...
	case PD_DPM_PE_CABLE_VDO:
		if (pd_dpm_get_cable_vdo(data, &(g_pd_di->cable_vdo_st)))
			return 0;
		break;
	default:
...
static void pd_dpm_init_resource(struct pd_dpm_info *di)
{
	...
	INIT_DELAYED_WORK(&di->drm_notifier_work, pd_dpm_drm_notifier_work);
	INIT_WORK(&g_pd_di->drm_work, drm_unblank_work);

	if (abnormal_cc_detection)
		schedule_delayed_work(&di->drm_notifier_work,
			msecs_to_jiffies(DRM_NOTIFIER_DELAYED_TIME));
}
...
static int extcon_init(struct pd_dpm_info *di)
{
	int ret = 0;
	 ...
	 */
	di->extcon = devm_extcon_dev_allocate(di->dev, rpm_extcon_cable);
	if (IS_ERR(di->extcon)) {
		ret = PTR_ERR(di->extcon);
		hwlog_err("extcon dev alloc fail %d\n", ret);
		goto out;
	}

	ret = devm_extcon_dev_register(di->dev, di->extcon);
	if (ret) {
		hwlog_err("extcon dev reg fail %d\n", ret);
		goto out;
	}

	/* Support reporting polarity and speed via properties */
	extcon_set_property_capability(di->extcon, EXTCON_USB,
	...
	extcon_set_property_capability(di->extcon, EXTCON_USB_HOST,
		EXTCON_PROP_USB_TYPEC_POLARITY);
	extcon_set_property_capability(di->extcon, EXTCON_USB_HOST,
		EXTCON_PROP_USB_SS);
out:
...
static const struct of_device_id pd_dpm_callback_match_table[] = {
	{
		.compatible = "huawei,pd_dpm_thirdparty",
		.data = NULL,
	},
	{},
};
