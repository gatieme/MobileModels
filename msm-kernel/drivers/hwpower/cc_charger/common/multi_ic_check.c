	...
	struct multi_ic_check_para *info)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ocp_process_type", &info->ocp_process_type, 0);
	...
	multi_ic_parse_diff_para(np, info->isys_error, "isys_error");
}
...
static int multi_ic_check_get_dmd_num(int dmd_level)
{
	...
	case MULTI_IC_DMD_LEVEL_INFO:
		return POWER_DSM_MULTI_CHARGE_CURRENT_RATIO_INFO;
	case MULTI_IC_DMD_LEVEL_WARNING:
		return POWER_DSM_MULTI_CHARGE_CURRENT_RATIO_WARNING;
	case MULTI_IC_DMD_LEVEL_ERROR:
		return POWER_DSM_MULTI_CHARGE_CURRENT_RATIO_ERROR;
	default:
		return -EPERM;
	}
}
...
static void multi_ic_handle_ocp_by_limit(struct multi_ic_check_para *info,
	int *ibus, int *max_ibus, int volt_ratio)
{
	int i;
	int idx = 0;
	int tmp = 0;

	for (i = 1; i < CHARGE_PATH_MAX_NUM; i++) {
		if (max_ibus[i] * ibus[idx] < ibus[i] * max_ibus[idx])
			idx = i;
	}
	...
	for (i = 0; i < CHARGE_PATH_MAX_NUM; i++)
		tmp += ibus[i] * max_ibus[idx] / ibus[idx];

	hwlog_info("update limit curr=%d\n", tmp * volt_ratio);
	multi_ic_check_update_limit_curr(info, tmp * volt_ratio);
}

static bool multi_ic_handle_ocp_by_force_single_path(struct multi_ic_check_para *info,
	int *ibus, int *max_ibus)
{
	int i;

	for (i = 0; i < CHARGE_PATH_MAX_NUM; i++) {
		if ((max_ibus[i] == 0) || (ibus[i] <= max_ibus[i]))
			continue;
...
static void multi_ic_check_normal_status(int working_mode, struct multi_ic_check_para *info)
{
	u32 cur_time;

	cur_time = (u32)power_get_current_kernel_time().tv_sec;
	if (cur_time - info->multi_ic_start_time < MULTI_IC_CHECK_TIMEOUT)
		return;
...
static void multi_ic_check_isys(u32 isys, struct multi_ic_check_para *info)
{
	int i;

	for (i = 0; i < MULTI_IC_DIFF_PARA_LEVEL; i++) {
		if (isys <= info->isys_error[i].diff_val)
			continue;

		hwlog_info("check isys=%u, limit_current=%d\n", isys, info->isys_error[i].limit_current);
		multi_ic_check_update_limit_curr(info, info->isys_error[i].limit_current);
		return;
	}
}

static void multi_ic_check_balance(int working_mode, int mode, struct multi_ic_check_para *info, int volt_ratio)
{
	int i;
	...
	isys = total_ibus * volt_ratio - total_ibat;
	hwlog_info("%s total_ibus=%d volt_ratio=%d total_ibat=%d\n", __func__, total_ibus, volt_ratio, total_ibat);
	if (isys > HIGH_POWER_CUR_TH) {
		hwlog_info("high power scenario\n");
		multi_ic_check_isys(isys, info);
		return;
	}

	multi_ic_check_normal_status(working_mode, info);
}

static bool multi_ic_check_ocp(int working_mode, struct multi_ic_check_para *info,
	int volt_ratio)
{
	int i;
	int ibus[CHARGE_PATH_MAX_NUM];
	int tmp_ibat;
	int max_ibus[CHARGE_PATH_MAX_NUM] = { 0 };
	...
	for (i = 0; i < CHARGE_PATH_MAX_NUM; i++) {
		...
		max_ibus[i] = tmp_ibat / volt_ratio;
		hwlog_info("ibus=%d, max_ibus=%d\n", ibus[i], max_ibus[i]);
	}

	if (info->ocp_process_type == 0)
		return multi_ic_handle_ocp_by_force_single_path(info, ibus, max_ibus);

	multi_ic_handle_ocp_by_limit(info, ibus, max_ibus, volt_ratio);
	return false;
}

static bool multi_ic_check_threshold(int working_mode, struct multi_ic_check_para *info,
	int volt_ratio)
{
	int i;
...
{
	if (!info || (volt_ratio == 0))
		return -EPERM;

	if (mode != CHARGE_MULTI_IC)
		return 0;

	hwlog_info("multi_ic_check begin\n");
	if (multi_ic_check_threshold(working_mode, info, volt_ratio))
		return -EPERM;

	if (multi_ic_check_ocp(working_mode, info, volt_ratio))
		return -EPERM;

	multi_ic_check_balance(working_mode, mode, info, volt_ratio);
