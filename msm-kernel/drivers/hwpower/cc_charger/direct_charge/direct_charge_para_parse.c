...
static void dc_parse_mode_switch_info(struct device_node *np,
	struct direct_charge_device *di)
{
	if (!power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"mode_switch", di->mode_switch, MODE_SWITCH_INFO_END))
		hwlog_info("mode_switch %d %d %d %d\n",
			di->mode_switch[MODE_SWITCH_STRATEGY],
...
int dc_parse_dts(struct device_node *np, void *p)
{
	...
	dc_parse_mode_switch_info(np, di);
