...
static bool dc_mode_switch_by_multi_bat_check(unsigned int mode)
{
	int bat_mode = multi_bat_ck_get_support_mode(mode, true);
	unsigned int op_mode = dc_get_optimal_mode();

	hwlog_info("bat_mode=%d mode=%d op_mode=%u\n", bat_mode, mode, op_mode);
	if (bat_mode < 0)
		return false;
	...
	if (!((unsigned int)bat_mode & mode) ||
		((mode != op_mode) && ((unsigned int)bat_mode & op_mode))) {
		hwlog_info("%s need switch mode\n", __func__);
		return true;
	}
	return false;
}

static bool dc_mode_switch_by_thermal(struct direct_charge_device *di)
{
	int cur = dc_sysfs_get_iin_thermal(di->working_mode);
	u32 delta_time = power_get_current_kernel_time().tv_sec - dc_ctrl_get_start_time();

	if (di->mode_switch[MODE_SWITCH_KEEP_TIME] &&
		(delta_time <= di->mode_switch[MODE_SWITCH_KEEP_TIME])) {
		hwlog_info("%s current mode need keep %ds, disable mode switch\n", __func__,
			di->mode_switch[MODE_SWITCH_KEEP_TIME]);
		return false;
	}

	if (di->mode_switch[MODE_SWITCH_THERML_LOW_TH] && (cur > 0) &&
		(cur <= di->mode_switch[MODE_SWITCH_THERML_LOW_TH])) {
		hwlog_info("%s cur=%d thermal_low_th=%u need switch mode\n", __func__, cur,
			di->mode_switch[MODE_SWITCH_THERML_LOW_TH]);
		return true;
	}

	if (di->mode_switch[MODE_SWITCH_THERML_HIGH_TH] && (cur > 0) &&
		(cur > di->mode_switch[MODE_SWITCH_THERML_HIGH_TH])) {
		hwlog_info("%s cur=%d thermal_high_th=%u need switch mode\n", __func__, cur,
			di->mode_switch[MODE_SWITCH_THERML_HIGH_TH]);
		return true;
	}
...
static bool dc_need_mode_switch(struct direct_charge_device *di)
{
	switch (di->mode_switch[MODE_SWITCH_STRATEGY]) {
	case SWITCH_STRATEGY_MULTI_BAT_CHECK:
		return dc_mode_switch_by_multi_bat_check(di->working_mode);
	case SWITCH_STRATEGY_MULTI_BAT_CHECK_OR_THERMAL:
		return dc_mode_switch_by_multi_bat_check(di->working_mode) ||
			dc_mode_switch_by_thermal(di);
	case SWITCH_STRATEGY_MULTI_BAT_CHECK_AND_THERMAL:
		return dc_mode_switch_by_multi_bat_check(di->working_mode) &&
			dc_mode_switch_by_thermal(di);
	default:
		return false;
...
void dc_control_work(struct work_struct *work)
{
	...
	if (dc_need_mode_switch(di)) {
		dc_disable_mode(DC_DIS_BY_PRIORITY, di->working_mode);
