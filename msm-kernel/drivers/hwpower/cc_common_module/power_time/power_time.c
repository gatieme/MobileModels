...
EXPORT_SYMBOL_GPL(power_get_timeofday);
...
EXPORT_SYMBOL_GPL(power_get_uptime);

void power_get_idle_time(struct timespec64 *idle)
{
	if (!idle)
		return;
	power_platform_get_idle_time(idle);
}
EXPORT_SYMBOL_GPL(power_get_idle_time);
#else
...
EXPORT_SYMBOL_GPL(power_get_local_time);
#else
