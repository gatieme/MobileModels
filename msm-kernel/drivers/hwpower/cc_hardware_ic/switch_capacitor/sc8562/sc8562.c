...
static int sc8562_config_pmid2out_uvp_th_mv(struct sc8562_device_info *di, int mode)
{
	...
	if (mode == SC8562_CHG_R12_CONVERTER_MODE)
...
static int sc8562_reverse12_mode_enable(bool enable, struct sc8562_device_info *di)
{
	int ret;
	u8 ctrl1_reg = 0;
	u8 ctrl4_reg = 0;
	...
	if (reverse_en) {
		ret += sc8562_write_mask(di, SC8562_CTRL4_REG, SC8562_CTRL4_MODE_MASK,
			SC8562_CTRL4_MODE_SHIFT, SC8562_CHG_R12_CONVERTER_MODE);
		ret += sc8562_write_multi_mask(di, SC8562_CTRL1_REG, SC8562_CTRL1_ACDRV_MANUAL_EN_MASK |
			SC8562_CTRL1_OVPGATE_EN_MASK | SC8562_CTRL1_WPCGATE_EN_MASK, 0x20); /* ACDRV_MANUAL_EN */
	} else {
		ret += sc8562_write_mask(di, SC8562_CTRL4_REG, SC8562_CTRL4_MODE_MASK,
			SC8562_CTRL4_MODE_SHIFT, SC8562_CHG_F21SC_MODE);
	}
...
{
	u8 mode = enable ? SC8562_CHG_R12_CONVERTER_MODE : SC8562_CHG_F21SC_MODE;
...
static int sc8562_reverse_cp_chip_init(void *dev_data, bool enable)
{
	...
	if (enable) {
		do {
			sc8562_reverse12_mode_enable(true, di);
			if (!sc8562_is_device_close(dev_data)) {
	...
	ret = sc8562_reverse12_mode_enable(false, di);
	return ret;
}
