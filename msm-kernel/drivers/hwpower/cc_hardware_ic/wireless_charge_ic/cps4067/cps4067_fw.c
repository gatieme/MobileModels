...
static void cps4067_fw_print_vrect(struct cps4067_dev_info *di)
{
	u16 vrect = 0;
	int ret;

	ret = cps4067_read_word(di, CPS4067_TX_VRECT_ADDR, &vrect);
	hwlog_info("[print_vrect] vrect=%d ret=%d\n", vrect, ret);
}
...
static int cps4067_fw_status_check(struct cps4067_dev_info *di, u32 cmd)
{
	...
	/* wait for 50ms*20=1000ms for status check */
	for (i = 0; i < 20; i++) {
...
static int cps4067_fw_check_mtp(void *dev_data)
{
	...
	if (power_cmdline_is_factory_mode() && cps4067_rx_is_tx_exist(di) &&
		!cps4067_fw_check_mtp_version(di))
		goto succ;
