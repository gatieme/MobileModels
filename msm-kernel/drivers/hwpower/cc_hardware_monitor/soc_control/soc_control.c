...
static const char * const g_soc_ctrl_op_user_table[] = {
	...
	[SOC_CTRL_OP_USER_BATT_CT] = "batt_ct",
};
...
static void soc_ctrl_recovery_control(struct soc_ctrl_dev *l_dev)
{
	...
	hwlog_info("recovery=%d cur_soc=%d, min_soc=%d, max_soc=%d\n",
		l_dev->work_mode, bat_cap, l_dev->min_soc, l_dev->max_soc);
...
static void soc_ctrl_event_work(struct work_struct *work)
{
	...
	int interval = SOC_CTRL_LOOP_TIME;
	...
	if (l_dev->soc_ctrl_interval > 0)
		interval = l_dev->soc_ctrl_interval;
	...
	void *data, int result, const char *client_str)
{
	...
	if ((l_dev->enable && (l_dev->max_soc == result)) ||
		...
		hwlog_info("ignore same, last enable=%d max_soc=%d\n",
			l_dev->enable, l_dev->max_soc);
		return 0;
	}
	...
	if (result < 0) {
		...
		l_dev->enable = 1; /* enable soc control */
	}
	/* process soc control */
	soc_ctrl_event_control(l_dev->event);
	return 0;
	...
	void *data, int result, const char *client_str)
{
	...
	/* only update min_soc co-work with max soc */
	if (result >= 0)
	...
	const char *buf)
{
	...
	/* SOC_CTRL_L_VOTE_OBJECT must be before SOC_CTRL_H_VOTE_OBJECT */
	ret = power_vote_set(SOC_CTRL_L_VOTE_OBJECT, user, enable, min_soc);
	ret += power_vote_set(SOC_CTRL_H_VOTE_OBJECT, user, enable, max_soc);
...
static int __init soc_ctrl_init(void)
{
	...
	power_vote_create_object(SOC_CTRL_L_VOTE_OBJECT, POWER_VOTE_SET_MIN,
		soc_ctrl_l_vote_callback, l_dev);
	power_vote_create_object(SOC_CTRL_H_VOTE_OBJECT, POWER_VOTE_SET_MIN,
		soc_ctrl_h_vote_callback, l_dev);
...
static void __exit soc_ctrl_exit(void)
{
	...
	power_vote_destroy_object(SOC_CTRL_L_VOTE_OBJECT);
	power_vote_destroy_object(SOC_CTRL_H_VOTE_OBJECT);
	kfree(l_dev);
	g_soc_ctrl_dev = NULL;
}
