...
#define MONITOR_INTERVAL_INIT      30000 /* 30000 ms */
...
static void pinctrl_mos_en(int flag)
{
	int ret;
	struct uscp_device_info *di = g_uscp_di;
	...
	if (flag)
		ret = power_pinctrl_config_state(di->dev, "mos_en_on");
	else
		ret = power_pinctrl_config_state(di->dev, "mos_en_off");

	if (ret)
		hwlog_err("power_pinctrl_config_state fail\n");
}
...
static void uscp_set_gpio_switch(struct uscp_device_info *di, int value)
{
	...
	if (di->use_pinctrl) {
		hwlog_info("use pinctl to pull mos gpio\n");
		pinctrl_mos_en(value);
		return;
	}
...
static int uscp_check_usb_temp(void)
{
	...
	if ((temp > USB_TEMP_UPPER_LIMIT) || (temp < USB_TEMP_LOWER_LIMIT)) {
		...
		(void)uscp_dmd_report(POWER_DSM_ERROR_NO_USB_SHORT_PROTECT_NTC, &temp_info);
	...
	struct uscp_temp_info *temp_info)
{
	...
	if ((tusb >= di->uscp_threshold_tusb) && (tdiff >= di->open_hiz_temp)) {
		if (di->dmd_hiz_enable && di->dmd_notify_enable_hiz) {
			if (!uscp_dmd_report(POWER_DSM_ERROR_NO_USB_SHORT_PROTECT_HIZ, temp_info))
	...
	if ((tusb >= di->uscp_threshold_tusb) && (tdiff >= di->open_mosfet_temp)) {
		...
		if (di->dmd_notify_enable) {
			if (!uscp_dmd_report(POWER_DSM_ERROR_NO_USB_SHORT_PROTECT, temp_info))
...
static void uscp_check_work(struct work_struct *work)
{
	...
	if (((di->check_count == CHECK_COUNT_DEFAULT_VAL) ||
		(di->check_interval == MONITOR_INTERVAL_INIT)) &&
	...
	if (temp_info.bat_temp < USB_TEMP_LOWER_LIMIT) {
		cnt++;
		di->check_interval = MONITOR_INTERVAL_SLOW;
	} else {
		cnt = 0;
	...
	if (cnt > TEMP_ABNORMAL_TIEMS) {
		cnt = 0;
		hwlog_info("battery temp abnormal, exit uscp check\n");
		return;
	}

	schedule_delayed_work(&di->check_work, msecs_to_jiffies(di->check_interval));
}
#else
static void uscp_check_work(struct work_struct *work)
{
...
static int uscp_parse_dts(struct device_node *np, struct uscp_device_info *di)
{
	...
	di->use_pinctrl = of_property_read_bool(np, "use_pinctrl");
	return 0;
}
