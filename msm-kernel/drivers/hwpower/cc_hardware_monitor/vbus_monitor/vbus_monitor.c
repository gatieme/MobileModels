...
static void vbus_send_wireless_aux_tx_open_uevent(void)
{
	struct power_event_notify_data n_data;

	n_data.event = "AUX_TX_OPEN=";
	n_data.event_len = 12; /* length of AUX_TX_OPEN= */
	power_event_report_uevent(&n_data);
}

static void vbus_send_wireless_aux_tx_close_uevent(void)
{
	struct power_event_notify_data n_data;

	n_data.event = "AUX_TX_CLOSE=";
	n_data.event_len = 13; /* length of AUX_TX_CLOSE= */
	power_event_report_uevent(&n_data);
}
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_WIRELESS_TX_STOP:
		...
		break;
	case POWER_NE_WIRELESS_AUX_TX_START:
		vbus_send_wireless_aux_tx_open_uevent();
		power_vote_set(VBUS_MONITOR_VOTE_OBJECT, "wireless_aux_tx", true, true);
		break;
	case POWER_NE_WIRELESS_AUX_TX_STOP:
		vbus_send_wireless_aux_tx_close_uevent();
		power_vote_set(VBUS_MONITOR_VOTE_OBJECT, "wireless_aux_tx", false, false);
		break;
	default:
		break;
	}
