...
int adapter_show_device_info(int prot)
{
	...
	case ADAPTER_PROTOCOL_FCP:
		...
		break;
	case ADAPTER_PROTOCOL_UFCS:
		hwlog_info("min_volt=%d\n", l_dev->info.min_volt);
		hwlog_info("max_volt=%d\n", l_dev->info.max_volt);
		hwlog_info("min_cur=%d\n", l_dev->info.min_cur);
		hwlog_info("volt_step=%d\n", l_dev->info.volt_step);
		hwlog_info("curr_step=%d\n", l_dev->info.curr_step);
		hwlog_info("output_mode=%d\n", l_dev->info.output_mode);
		break;
	default:
		break;
	}
...
int adapter_get_chip_serial_num(int prot, int *num)
{
	...
	if (!l_ops->get_chip_serial_num) {
		hwlog_err("get_chip_serial_num is null\n");
		return -EPERM;
	}

	return l_ops->get_chip_serial_num(num);
}
...
int adapter_get_power_curve(int prot, struct adp_pwr_curve_para *val,
	int *size, int max_size)
{
	...
	return l_ops->get_power_curve(val, size, max_size);
}
...
int adapter_get_adp_code(int prot, int *code)
{
	...
	if (!l_ops->get_adp_code) {
		hwlog_err("get_adp_code is null\n");
		return -EPERM;
	}

	return l_ops->get_adp_code(code);
}
...
int adapter_get_source_id(int prot, int *source_id)
{
	...
	if (!l_ops->get_source_id) {
		hwlog_err("get_source_id is null\n");
		return -EPERM;
	}

	return l_ops->get_source_id(source_id);
}
...
int adapter_get_scpb_pwr(int prot, struct adp_scpb_pwr_data *scpb_pwr, int *size, int max_size)
{
	...
	if (!l_ops || !scpb_pwr || !size)
		return -EPERM;

	if (!l_ops->get_scpb_pwr) {
		hwlog_err("get_scpb_pwr is null\n");
		return -EPERM;
	}

	return l_ops->get_scpb_pwr(scpb_pwr, size, max_size);
}
...
int adapter_auth_encrypt_start(int prot, int key)
{
	...
	return l_ops->auth_encrypt_start(key);
}
...
bool adapter_is_undetach_cable(int prot)
{
	...
	if (!l_ops || !l_ops->is_undetach_cable)
		return false;
