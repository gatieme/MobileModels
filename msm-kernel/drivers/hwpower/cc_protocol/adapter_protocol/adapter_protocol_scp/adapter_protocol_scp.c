...
#define HWSCP_BEFORE_SIGN_M7_REG       0x2f
#define HWSCP_SIGN_M7_REG              0x54
#define M7_ADAP_MAX_VOLTAGE            11000
...
static bool hwscp_detect_adapter_is_m7_by_0x54(void)
{
	int value[BYTE_ONE] = { 0 };
	int value2f[BYTE_ONE] = { 0 };

	/* if want read 0x54,must read 0x2f firstly */
	if (hwscp_reg_read(HWSCP_BEFORE_SIGN_M7_REG, value2f, BYTE_ONE))
		return -EPERM;
	/*  Read 0x54 only for slove m7 adap type */
	if (hwscp_reg_read(HWSCP_SIGN_M7_REG, value, BYTE_ONE))
		return -EPERM;

	hwlog_info("adapter reg [0x%x] = 0x%x\n", HWSCP_SIGN_M7_REG, value[0]);
	return value[0] == HWSCP_ADP_TYPE_M7_CAR;
}
...
static int hwscp_set_usbpd_enable(int enable)
{
	...
	if (hwscp_get_reg7e_rw_error_flag() || !hwscp_is_cport_adp()) {
		hwlog_info("disabled pd is not supported\n");
		return 0;
	}
