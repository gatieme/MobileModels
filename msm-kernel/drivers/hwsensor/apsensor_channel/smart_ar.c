    ...
    unsigned int subtype, const char *buf, size_t count)
{
	...
	(void)memcpy(&ar_buf_hal->ar_cfg_buf, buf, count);
...
static void fifo_in(ar_data_buf_t *pdata, const char *data, unsigned int len, unsigned int align)
{
	...
	if ((pdata->write_index + len) >  pdata->buf_size) {
		retPtr = memcpy(pdata->data_buf + pdata->write_index, data, (size_t)(pdata->buf_size - pdata->write_index));
		if (retPtr == NULL)
			hwlog_err("hismart [%s] memcpy fail[%p]\n", __func__, retPtr);
		retPtr = memcpy(pdata->data_buf, data + pdata->buf_size - pdata->write_index,
			(size_t)(len + pdata->write_index - pdata->buf_size));
		if (retPtr == NULL)
			hwlog_err("hismart [%s] memcpy fail[%p]\n", __func__, retPtr);
	} else {
		retPtr = memcpy(pdata->data_buf + pdata->write_index, data, (size_t)len);
		if (retPtr == NULL)
			hwlog_err("hismart [%s] memcpy fail[%p]\n", __func__, retPtr);
	}
...
static int fifo_out(ar_data_buf_t *pdata, unsigned char *data, unsigned int count)
{
	...
	if ((pdata->read_index + count) >  pdata->buf_size) {
		retPtr = memcpy(data, pdata->data_buf + pdata->read_index, (size_t)(pdata->buf_size - pdata->read_index));
		if (retPtr == NULL)
			hwlog_err("hismart:[%s] memcpy fail\n", __func__);
		retPtr = memcpy(data + pdata->buf_size - pdata->read_index,
			pdata->data_buf, (size_t)(count + pdata->read_index - pdata->buf_size));
	} else {
		retPtr = memcpy(data, pdata->data_buf + pdata->read_index, (size_t)count);
	}
	if (retPtr == NULL)
		hwlog_err("hismart:[%s] memcpy fail\n", __func__);
	...
	context_iomcu_cfg_t *dest, const context_iomcu_cfg_t *new_set)
{
	...
	(void)memset((void *)dest, 0, sizeof(context_iomcu_cfg_t));
...
static int  ar_stop_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	(void)memset((void *)&ar_port->ar_config, 0, sizeof(context_iomcu_cfg_t));
	devinfo->usr_cnt--;
	...
	context_dev_info_t *devinfo, unsigned int context_max)
{
	...
	(void)memset(config, 0, sizeof(context_iomcu_cfg_t));
#ifdef CONFIG_CONTEXTHUB_UNIFORM_INTERVAL
	...
	for (i = 0; i < devinfo->cfg.context_num; i++, plist++) {
		...
		retPtr = memcpy((void *)&config->context_list[plist->head.context], (void *)plist, sizeof(context_config_t));
		if (retPtr == NULL)
			hwlog_err("hismart [%s] memcpy fail[%d]\n", __func__, retPtr);
	}
	return 0;
}
	...
	unsigned long arg, unsigned int context_max)
{
	...
	(void)memset((void *)&devinfo->cfg, 0, sizeof(context_iomcu_cfg_t));
#ifdef CONFIG_CONTEXTHUB_UNIFORM_INTERVAL
...
STATIC int ar_config_to_iomcu(const ar_port_t *ar_port)
{
	...
	if (devinfo->usr_cnt) {
		...
		retPtr = memcpy(pcfg, &ar_port->ar_config, sizeof(context_iomcu_cfg_t));
		if (retPtr == NULL)
			hwlog_err("hismart [%s] memcpy fail[%d]\n", __func__, retPtr);
		...
		struct read_info rd;
		(void)memset((void *)&rd, 0, sizeof(struct read_info));
...
static int get_data_from_mcu(const struct pkt_header *head, work_handler wh)
{
	...
	// if (memcpy(wk->data, head, len) != EOK) {
	if (memcpy(wk->data, head, len) == NULL) {
		kfree(wk->data);
		kfree(wk);
		hwlog_err("%s memcpy error\n", __func__);
		return -EFAULT;
	}
...
int ar_state_shmem(const struct pkt_header *head)
{
	...
	ret = memcpy(g_ar_dev.activity_shemem.context_event, staus->context_event, g_ar_dev.activity_shemem.user_len);
	if (ret == NULL)
		hwlog_err("hismart:[%s] memcpy fail\n", __func__);
...
static int ar_state_v2_cmd(const ar_port_t *ar_port, unsigned long arg)
{
	...
	(void)memset(&g_ar_dev.activity_shemem, 0, sizeof(ar_state_shmen_t));
	...
	size_t len, loff_t *ppos)
{
	int ret = 0;
	char *user_data;
	if (len < sizeof(ar_data_req_t)) {
		hwlog_err("%s len too short\n", __func__);
		return len;
	}
	user_data = (char *)kzalloc(len, GFP_KERNEL);
...
static void get_support_activity_from_dts(void)
{
	...
	for (i = 0; i < dts_support_num; i++) {
		...
		if (memcpy(g_support_activities_buf + offset,
					support_activity, strlen(support_activity) + 1) == NULL) { // 1 is '\0'
...
static int ar_init(void)
{
	...
	if (g_dts_status == DTS_DISABLED)
		return ret;
	(void)memset((void *)&g_ar_dev, 0, sizeof(g_ar_dev));
    ...
    hwlog_err("[qfy] hismart:[%s]:line[%d] cannot register smart ar ret[%d]\n", __func__, __LINE__, ret);
	if (ret) {
