...
int als_under_tp_nv_read(als_sensor_t als_sensor)
{
	uint8_t *buf = NULL;
...
void send_als_save_data_request(als_sensor_t als_sensor)
{
	...
	hwlog_info("%s enter\n", __func__);
	als_buf.payload.als = (uint32_t)als_sensor;
	ret = memcpy_s(&als_buf.payload.calidata, sizeof(struct als_under_tp_calidata), &als_store_calidata, sizeof(struct als_under_tp_calidata));
	if (ret < 0)
		pr_err("memcpy fail\n");
...
ssize_t als_under_tp_calidata_store_sub_nv_gamma(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count, als_sensor_t als_sensor)
{
	int i;
	int ret;
	uint32_t als_undertp_calidata[ALS_UNDER_TP_CALDATA_LEN] = { 0 };
	pr_info("%s count %d\n", __func__, count);
	...
	pr_info("attr_als_under_tp_calidata_store: x = %d, y = %d, width = %d, len = %d\n",
		als_store_calidata.x, als_store_calidata.y,
		als_store_calidata.width, als_store_calidata.length);

	/* start buf 4 size minus 2 */
	if (memcpy_s(&als_store_calidata.als_nv_data,
		sizeof(struct als_under_tp_nv_type),
		&als_undertp_calidata[4],
		(ALS_UNDER_TP_CALDATA_LEN - 4) * sizeof(uint32_t)) != EOK)
	...
	for (i = 0; i < ALS_SUB_BLOCK_SIZE; i++)
		pr_info("attr_als_under_tp_calidata_store: als_under_tp_cal_data.sub_block[%d] = %d\n",
			i, als_store_calidata.als_nv_data.sub_block[i]);
	for (i = 0; i < ALS_DBV_SIZE; i++)
		pr_info("attr_als_under_tp_calidata_store: als_under_tp_cal_data.dbv[%d] = %d\n",
			i, als_store_calidata.als_nv_data.dbv[i]);
	for (i = 0; i < ALS_FLAGS_SIZE; i++)
		pr_info("attr_als_under_tp_calidata_store: als_under_tp_cal_data.flags[%d] = %d\n",
			i, als_store_calidata.als_nv_data.flags[i]);

	ret = als_underscreen_calidata_save(als_sensor);
	if (ret != 0)
		send_als_save_data_request(als_sensor);
	return count;
}
	...
	struct device_attribute *attr, const char *buf, size_t count, als_sensor_t als_sensor)
{
	...
	ret = als_under_tp_calidata_store_sub_nv_gamma(dev, attr, buf, count, als_sensor);
	return ret;
}
EXPORT_SYMBOL(als_under_tp_calidata_store);
...
ssize_t als_under_tp_calidata_show_cct(char *buf)
{
	...
	size = ARRAY_SIZE(als_show_calidata.als_nv_data.sub_block);
	for (i = 0; i < size; i++) {
		ret = snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s,%d",
		buf, als_show_calidata.als_nv_data.sub_block[i]);
		if (ret <= 0) {
			pr_info("%s: write sub_block[%d] to buf fail\n", __func__, i);
			return -1;
		}
	}
	size = ARRAY_SIZE(als_show_calidata.als_nv_data.dbv);
	for (i = 0; i < size; i++) {
		ret = snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s,%d",
		buf, als_show_calidata.als_nv_data.dbv[i]);
		if (ret <= 0) {
			pr_info("%s: write res[%d] to buf fail\n", __func__, i);
			return -1;
		}
	}
	size = ARRAY_SIZE(als_show_calidata.als_nv_data.flags);
	for (i = 0; i < size; i++) {
		ret = snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s,%d",
		buf, als_show_calidata.als_nv_data.flags[i]);
		if (ret <= 0) {
...
ssize_t als_under_tp_calidata_show(struct device *dev,
	struct device_attribute *attr, char *buf, als_sensor_t als_sensor)
{
	int ret;
	pr_info("%s: enter\n", __func__);
	...
	ret = als_under_tp_nv_read(als_sensor);
	if (ret != 0) {
...
EXPORT_SYMBOL(als_under_tp_calidata_show);
...
EXPORT_SYMBOL(attr_als_rgb_enable_store);
...
EXPORT_SYMBOL(attr_als_rgb_debug_enable_store);
