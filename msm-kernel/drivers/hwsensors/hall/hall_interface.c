...
EXPORT_SYMBOL_GPL(hall_interface_get_hall_status);

bool hall_interface_get_ext_hall_status(void)
{
	if (!g_hall_interface_dev || !g_hall_interface_dev->p_ops)
		return false;

	return g_hall_interface_dev->p_ops->get_ext_fold_status();
}
EXPORT_SYMBOL_GPL(hall_interface_get_ext_hall_status);
...
EXPORT_SYMBOL_GPL(hall_interface_ops_register);
