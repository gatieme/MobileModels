...
#define ADC_TIMEOUT_MAX_TIMES           120
static u32 g_adc_timeout_cnt;
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		g_adc_timeout_cnt++;
		dev_err(adc->dev, "ADC channel timeout cunt: %u\n", g_adc_timeout_cnt);
		if (g_adc_timeout_cnt++ >= ADC_TIMEOUT_MAX_TIMES) {
			g_adc_timeout_cnt = ADC_TIMEOUT_MAX_TIMES;
			log_usertype = get_logusertype_flag();
			if ((log_usertype != COMMERCIAL_USER) && (log_usertype != OVERSEA_COMMERCIAL_USER)) {
				dev_err(adc->dev, "beta version trigger crash dump\n");
				BUG_ON(1); /* trigger crash dump for analyze adc timeout issue */
			}
		}
#endif
		ret = -ETIMEDOUT;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	[ADC7_AMUX_THM2_30K_PU] = ADC5_CHAN_TEMP("amux_thm2_pu1", 0,
					SCALE_HW_CALIB_THERM_30K_PU_PM7)
#endif
