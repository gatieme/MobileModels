...
extern void fp_ops_register(struct ud_fp_ops *ops);
extern void fp_ops_unregister(struct ud_fp_ops *ops);
...
static unsigned int g_irq_print_cnt = 0;
static int g_lhbm_interrupted_by_power = 0;
static unsigned long g_finger_down_time = 0;

static int ud_fingerprint_irq_notify(struct tp_to_udfp_data *tp_data, size_t len);
                       ...
                       char* buffer)
{
    ...
    fpc_log_info("[fpc] irq_get : %d, irq_cnt: %u\n", irq, g_irq_print_cnt);
                       ...
                       const char* buffer, size_t count)
{
	...
	data.tp_event = (int)value;
	data.tp_source = UDFP_DATA_SOURCE_SIMULATE;
	fpc_log_info("%s: tp_event: %d\n", __func__, data.tp_event);

	(void)ud_fingerprint_irq_notify(&data, sizeof(struct tp_to_udfp_data));
	...
	struct device_attribute *attribute, const char *buffer, size_t count)
{
	...
	strncpy(fp->module_id_ud, buffer,
		module_id_len - 1);
/*	if (ret != EOK) {
	...
	unsigned int sensor_type, char *chip_info)
{
	...
	return (ssize_t)snprintf(chip_info, FP_MAX_SENSOR_ID_LEN, "%x\n", sensor_id);
}
...
static void fingerup_event_function(int *cur_state, struct fp_data *fp)
{
	...
	*cur_state = FINGER_UP_STATE;
	g_finger_down_time = jiffies - g_finger_down_time;
...
static int ud_fingerprint_handle_tp_msg(int tp_event)
{
...
static void fp_set_udfp_data(struct tp_to_udfp_data *tp_data, size_t len)
{
	struct fp_data *fp = g_fp_data;
	if (tp_data->tp_event != TP_EVENT_FINGER_DOWN)
		return;
	if (fp == NULL) {
		fpc_log_err("%s fp is null\n", __func__);
		return;
	}
	if (len > sizeof(struct tp_to_udfp_data))
		len = sizeof(struct tp_to_udfp_data);
	memcpy(&fp->udfp_data, tp_data, len);
	fpc_log_info("size: %u, version: %d, tp_source: %d, tp_event: %d, "
		"tp_coverage: %d, tp_peak: %d", sizeof(struct tp_to_udfp_data),
		tp_data->version, tp_data->tp_source, tp_data->tp_event,
		tp_data->tp_coverage, tp_data->tp_peak);
}
...
static int ud_fingerprint_irq_notify(struct tp_to_udfp_data *tp_data, size_t len)
{
	...
	fpc_log_info("tp_event = %d, g_tp_state = %d\n", tp_data->tp_event, g_tp_state);
	if (g_close_tp_irq) {
		fpc_log_info("tp irq status is close, not handle tp event\n");
		return FP_RETURN_SUCCESS;
	}
	fp_set_udfp_data(tp_data, len);
	return ud_fingerprint_handle_tp_msg(tp_data->tp_event);
}
...
static irqreturn_t fingerprint_irq_handler(int irq, void* handle)
{
    ...
    {
       __pm_wakeup_event(fingerprint->ttw_wl, msecs_to_jiffies(fingerprint->irq_hold_time));
    }
...
struct ud_fp_display_ops g_fp_display_operations = {NULL, NULL};

void ud_fp_display_register(struct ud_fp_display_ops *fp_display_ops)
{
	if (g_fp_display_operations.fp_get_local_hbm_mode == NULL) {
		g_fp_display_operations.fp_get_local_hbm_mode = fp_display_ops->fp_get_local_hbm_mode;
		fpc_log_err("%s:fp_get_local_hbm_mode register sussess\n", __func__);
	} else {
		fpc_log_err("%s:fp_get_local_hbm_mode has been registered\n", __func__);
	}

	if (g_fp_display_operations.fp_set_brightness_level == NULL) {
		g_fp_display_operations.fp_set_brightness_level = fp_display_ops->fp_set_brightness_level;
		fpc_log_err("%s:fp_set_brightness_level register sussess\n", __func__);
	} else {
		fpc_log_err("%s:fp_set_brightness_level has been registered\n", __func__);
	}
}
EXPORT_SYMBOL_GPL(ud_fp_display_register);

void ud_fp_dispaly_unregister(void)
{
	if (g_fp_display_operations.fp_get_local_hbm_mode != NULL) {
		g_fp_display_operations.fp_get_local_hbm_mode = NULL;
		fpc_log_err("%s:fp_get_local_hbm_mode unregister sussess\n", __func__);
	} else {
		fpc_log_err("%s:fp_get_local_hbm_mode has been unregister\n", __func__);
	}

	if (g_fp_display_operations.fp_set_brightness_level != NULL) {
		g_fp_display_operations.fp_set_brightness_level = NULL;
		fpc_log_err("%s:fp_set_brightness_level unregister sussess\n", __func__);
	} else {
		fpc_log_err("%s:fp_set_brightness_level has been unregistered\n", __func__);
	}
}
EXPORT_SYMBOL_GPL(ud_fp_dispaly_unregister);
...
static int get_supported_hbm_mode(void __user *argp)
{
	...
	if (g_fp_display_operations.fp_get_local_hbm_mode != NULL) {
		hbm_mode = g_fp_display_operations.fp_get_local_hbm_mode();
	}
...
static int get_lhbm_interrupted_by_power(void __user *argp)
{
	int ret = copy_to_user(argp, &g_lhbm_interrupted_by_power, sizeof(g_lhbm_interrupted_by_power));
	if (ret) {
		fpc_log_err("%s copy_to_user failed, ret=%d\n", __func__, ret);
		return -EFAULT;
	}
...
void ud_fp_notify_lhbm_interrupted_by_power(void)
{
	g_lhbm_interrupted_by_power = 1;
}
EXPORT_SYMBOL_GPL(ud_fp_notify_lhbm_interrupted_by_power);
...
static int set_brightness_level(const void __user *argp)
{
	...
	if (g_fp_display_operations.fp_set_brightness_level == NULL)
		return FP_RETURN_SUCCESS;
	if (grayscale != 0)
		g_lhbm_interrupted_by_power = 0;
	return g_fp_display_operations.fp_set_brightness_level(grayscale);
}

//static int fp_irq_is_disabled = 0;
static void fp_ioc_enable_irq(struct fp_data *fp)
{
    fpc_log_info("FP_IOC_CMD_ENABLE_IRQ, irq_cnt: %u\n", g_irq_print_cnt);
...
{
    fpc_log_info("FP_IOC_CMD_DISABLE_IRQ, irq_cnt: %u\n", g_irq_print_cnt);
    if (fp->use_tp_irq == USE_TP_IRQ)
        return;
    mutex_lock(&fp->lock);
    if (fp->irq_num == 1) {
...
static int fp_ioc_set_wakelock_status(struct fp_data *fingerprint, const void __user *argp)
{
    int key;
...
static int fp_ioc_set_ipc_wakelocks(const void __user *argp)
{
    ...
    if (set_ipc_wakelock)
        pm_stay_awake(g_fp_data->dev);
    else
        pm_relax(g_fp_data->dev);

    fpc_log_info("FP_IOC_CMD_SET_IPC_WAKELOCKS =%d\n",
        set_ipc_wakelock);
    return FP_RETURN_SUCCESS;
}

static int fp_ioc_wakelock_timeout(const void __user *argp)
{
    int value;
    ...
    smp_rmb();
    __pm_wakeup_event(g_fp_data->ttw_wl, msecs_to_jiffies(value));
    fpc_log_info("FP_IOC_CMD_WAKELOCK_TIMEOUT value = %d\n", value);
    return FP_RETURN_SUCCESS;
}
...
EXPORT_SYMBOL(ud_fp_on_hbm_completed);
...
static int fp_ioc_get_touch_info(struct fp_data *fp, void __user *argp)
{
	int ret;
	unsigned int length = sizeof(struct tp_to_udfp_data);
	struct fp_ioctl_cmd info = {0};

	ret = copy_from_user(&info, argp, sizeof(struct fp_ioctl_cmd));
	if (ret) {
		fpc_log_err("%s copy_from_user failed, ret=%d\n", __func__, ret);
		return -EFAULT;
	}
	fpc_log_info("%s: called, sizeof(tp_to_udfp_data):%u, data_len:%u\n",
		__func__, sizeof(struct tp_to_udfp_data), info.data_len);

	length = (info.data_len < length) ? info.data_len : length;
	ret = copy_to_user(info.data_addr, &fp->udfp_data, length);
	if (ret) {
		fpc_log_err("%s copy_to_user failed, ret=%d\n", __func__, ret);
		return -EFAULT;
	}
...
static int fp_ioc_get_touch_duration(void __user *argp)
{
	int ret;
	int value;
	long duration = 0;
	if (g_tp_state == FINGER_DOWN_STATE)
		duration = jiffies - g_finger_down_time;
	else if (g_tp_state == FINGER_UP_STATE)
		duration = g_finger_down_time;
	value = (int)(duration * 1000 / HZ);
	ret = copy_to_user(argp, &value, sizeof(value));
	if (ret) {
		fpc_log_err("copy_to_user failed, ret=%d\n", ret);
		return -EFAULT;
	}
...
static long fingerprint_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	case FP_IOC_CMD_SET_WAKELOCK_STATUS:
		error = fp_ioc_set_wakelock_status(fingerprint, argp);
		break;
	case FP_IOC_CMD_SET_IPC_WAKELOCKS:
		error = fp_ioc_set_ipc_wakelocks(argp);
		break;
	case FP_IOC_CMD_WAKELOCK_TIMEOUT:
		error = fp_ioc_wakelock_timeout(argp);
		break;
	...
	case FP_IOC_CMD_WAIT_FINGER_UP:
		...
		break;
	case FP_IOC_CMD_GET_LOCAL_HBM_SUPPORT:
		fpc_log_info("%s FP_IOC_CMD_GET_LOCAL_HBM_SUPPORT\n", __func__);
		error = get_supported_hbm_mode(argp);
		break;
	case FP_IOC_CMD_SET_LOCAL_HBM_LEVEL:
		fpc_log_info("%s FP_IOC_CMD_SET_LOCAL_HBM_LEVEL\n", __func__);
		error = set_brightness_level(argp);
		break;
	case FP_IOC_CMD_GET_LOCAL_HBM_INTERRUPTED_BY_POWER:
		fpc_log_info("%s FP_IOC_CMD_GET_LOCAL_HBM_INTERRUPTED_BY_POWER\n", __func__);
		error = get_lhbm_interrupted_by_power(argp);
		break;
	...
	case FP_IOC_CMD_GET_FINGER_STATUS:
		fpc_log_info("%s: FP_IOC_CMD_GET_FINGER_STATUS\n", __func__);
		error = fp_get_finger_status(argp);
		break;
	case FP_IOC_CMD_GET_TOUCH_INFO:
		fpc_log_info("%s: FP_IOC_CMD_GET_TOUCH_INFO\n", __func__);
		error = fp_ioc_get_touch_info(fingerprint, argp);
		break;
	case FP_IOC_CMD_GET_TOUCH_DURATION:
		fpc_log_info("%s: FP_IOC_CMD_GET_TOUCH_DURATION\n", __func__);
		error = fp_ioc_get_touch_duration(argp);
		break;
	default:
...
static int fingerprint_probe(struct platform_device* pdev)
{
    ...
    fingerprint->ttw_wl = wakeup_source_register(dev, "fpc_ttw_wl");
...
use_tp_irq_tag:
	...
	memset(&fingerprint->udfp_data, 0, sizeof(struct tp_to_udfp_data));
...
static int fingerprint_remove(struct platform_device* pdev)
{
    ...
    wakeup_source_unregister(fingerprint->ttw_wl);
#if defined(CONFIG_FB)
	if (fingerprint->fb_notify.notifier_call != NULL) {
		fingerprint->fb_notify.notifier_call = NULL;
