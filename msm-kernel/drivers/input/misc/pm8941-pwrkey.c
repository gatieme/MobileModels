...
static int pm8010gpio_reboot_notify(struct notifier_block *nb,
				    unsigned long code, void *unused)
{
	gpio_direction_output(pm8010_reset_gpio, 0);
	return NOTIFY_DONE;
}

static struct notifier_block pm8010_reset_notifier = {
	.notifier_call = pm8010gpio_reboot_notify,
};
...
static irqreturn_t pm8941_pwrkey_irq(int irq, void *_data)
{
	...
	if (pwrkey->sw_debounce_time_us) {
		...
		dev_info(pwrkey->dev, "key event received after %llu us, debounce time=%u us\n",
			elapsed_us, pwrkey->sw_debounce_time_us);
		if (elapsed_us < pwrkey->sw_debounce_time_us) {
			dev_err(pwrkey->dev, "ignoring the key event");
			return IRQ_HANDLED;
		}
	}
	 ...
	 */
	trace_android_hung_wp_screen_qcom_pkey_press(pwrkey->code, sts);
	if (pwrkey->code == KEY_POWER)
		trace_android_hung_wp_screen_powerkey_ncb(1 ^ sts);

#ifdef CONFIG_HW_COMB_KEY
	if (pwrkey->code == KEY_POWER)
		power_key_status_distinguish(!!sts);
	if (pwrkey->code == KEY_VOLUMEDOWN)
		volume_key_status_distinguish(!!sts);
#endif

#if IS_ENABLED(CONFIG_BOOT_DETECTOR_QCOM)
        if (sts == BF_PWK_PRESS_FLAG) {
                        pr_err("bootfail_pwk_check:press\n");
                        bootfail_pwk_press();
        } else if (sts == BF_PWK_RELEASE_FLAG) {
                        pr_err("bootfail_pwk_check:release\n");
	...
	dev_info(pwrkey->dev, "PWRKEY status=0x%02x, last_release_time %llu us\n",
		pwrkey->last_status, pwrkey->last_release_time);
...
static int pm8941_pwrkey_sw_debounce_init(struct pm8941_pwrkey *pwrkey)
{
	...
	if (pwrkey->subtype >= PON_SUBTYPE_GEN2_PRIMARY)
		pwrkey->sw_debounce_time_us = 4 * USEC_PER_SEC /
						(1 << (0xf - (val & 0xf)));
	else
		pwrkey->sw_debounce_time_us = 4 * USEC_PER_SEC /
						(1 << (0x7 - (val & 0x7)));
...
static int pm8941_pwrkey_probe(struct platform_device *pdev)
{
	...
	pm8010_reset_gpio = of_get_named_gpio(pdev->dev.of_node, "pm8010-reset-gpios", 0);

	error = gpio_request(pm8010_reset_gpio, "PM8010_RESET_GPIO");
	if (error)
		dev_err(&pdev->dev, "failed to request reset gpio: %d\n", error);

	register_reboot_notifier(&pm8010_reset_notifier);
...
static int pm8941_pwrkey_remove(struct platform_device *pdev)
{
	...
	unregister_reboot_notifier(&pm8010_reset_notifier);
