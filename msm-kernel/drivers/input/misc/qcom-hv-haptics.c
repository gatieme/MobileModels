...
static struct dsm_dev dsm_hboost = {
	.name = "dsm_hboost",
	.device_name = "qcom-hv-haptics",
	.ic_name = NULL,
	.module_name = NULL,
	.fops = NULL,
	.buff_size = 1024,
...
#define DEFAULT_F0_REF				235
#define DEFAULT_F0_RANGE			8
...
#define DEFAULT_RC_CLK_CAL_COUNT        595
#define DEFAULT_T_LRA_US                4255
#define DEFAULT_CL_T_LRA_US             4319
#define CL_T_LRA_US_MAX_LMT             4444
#define CL_T_LRA_US_MIN_LMT             4081


#define is_between(val, min, max)	\
	(((min) <= (max)) && ((min) <= (val)) && ((val) <= (max)))

#define QCOM_HAPTIC_NAME                       "haptics"
#define MAX_WRITE_BUF_LEN                       16
#define QOCM_RTP_NAME_MAX                       64
...
#define LONG_HAPTIC_RANGE                       8
#define SHORT_HAPTIC_RANGE                      5
#define HW_CTRL_NOT_FOLLOW_HW                   0x40
#define HW_CTRL_FOLLOW_HW                       0xE0
#define HW_HBOOST_EN_CFG                        0x80
#define HW_HBOOST_DIS_CFG                       0x00
#define HAPTICS_CFG_EN_CTL                      0x80
#define HAPTICS_CFG_DIS_CTL                     0x00
#define HW_HBST_ADDR_BASE                       2
#define RTP_MAX_ID                              100
#define MAX_RTP_TYPE_MOTION                     11
#define BOOST_STATUS4_CHECK                     0x01
#define BOOST_STATUS1_CHECK                     0x04
#define HAPTICS_CFG_SPMI_PLAY_CHECK             0x80
static unsigned int rtp_haptic_vb_time[MAX_RTP_TYPE_MOTION] = {1, 11, 16, 20, 20, 11, 11, 15, 19, 23};
static struct timespec64 now, last;
...
struct haptics_chip {
	...
	struct hrtimer			ext_hbst_off_timer;
	struct hrtimer			timer;
	spinlock_t			boost_enable_lock;
	...
	bool				haptics_not_onboard;
	bool        is_hv_haptics;
	bool				hboost_enabled;
	bool				rtp_wave_is_running;
};
...
static unsigned int boost_power_alwayson = 0;
static int haptics_stop_fifo_play(struct haptics_chip *chip);
static int haptics_frequency_hz_check(struct haptics_chip *chip);
static int haptics_start_lra_calibrate(struct haptics_chip *chip);
static void haptics_getnstimeofday(struct timespec64 *ts64)
{
	ktime_get_real_ts64(ts64);
}
static int haptics_boost_vreg_enable(struct haptics_chip *chip, bool en, enum boost_source_t source);
static int haptics_wait_hboost_ready(struct haptics_chip *chip, enum boost_source_t source);
		...
		u16 base, u8 offset, u8 *val, u32 length)
{
	...
	if (rc < 0)
			pr_err("haptics read addr %#x failed, rc=%d\n", addr, rc);
		...
		u16 base, u8 offset, u8 *val, u32 length)
{
	...
	if (rc < 0)
		pr_err("haptics  write addr %#x failed, rc=%d\n", addr, rc);
		...
		u16 base, u8 offset, u8 mask, u8 val)
{
	...
	if (rc < 0)
		pr_err("haptics update addr %d failed, rc=%d\n", addr, rc);

	return rc;
}

#ifdef CONFIG_HUAWEI_DSM
static void hboost_status_check_dmd(struct haptics_chip *chip)
{
	u8 boost_status4;
	u8 boost_status1;
	u8 haptics_cfg_spmi_play;
	int rc;

	if (!vib_dclient) {
		pr_err("haptics register dsm vib_dclient failed!\n");
		return;
	}

	rc = haptics_read(chip, chip->hbst_addr_base,
		HAP_BOOST_STATUS4_REG, &boost_status4, 1);
	if (rc < 0) {
		pr_err("haptics Read HAP_BOOST_STATUS4_REG failed\n");
		return;
	}
	rc = haptics_read(chip, chip->hbst_addr_base,
		HAP_PTN_FIFO_READY_STS_REG, &boost_status1, 1);
	if (rc < 0) {
		pr_err("haptics Read HAP_PTN_FIFO_READY_STS_REG failed\n");
		return;
	}
	rc = haptics_read(chip, chip->cfg_addr_base,
		HAP_CFG_SPMI_PLAY_REG, &haptics_cfg_spmi_play, 1);
	if (rc < 0) {
		pr_err("haptics Read HAP_CFG_SPMI_PLAY_REG failed\n");
		return;
	}

	if ((boost_status4 & BOOST_STATUS4_CHECK) && !(boost_status1 & BOOST_STATUS1_CHECK)
		&& !(haptics_cfg_spmi_play & HAPTICS_CFG_SPMI_PLAY_CHECK)) {
		if (dsm_client_ocuppy(vib_dclient)) {
			pr_err("haptics %s buffer is busy!\n", __func__);
			return;
		}
		dsm_client_record(vib_dclient, "haptics hboost 5v stuck\n");
		dsm_client_notify(vib_dclient, DSM_VIBRATOR_HBST_ERROR);
		pr_err("haptics %s hboost already stuck!\n", __func__);
	}
}
#endif

#define BOOST_VREG_OFF_DELAY_SECONDS 2
int haptics_enable_hboost_5v(int value)
{
	struct haptics_chip *chip = g_chip;
	int rc;
	unsigned long flags;
	...
	pr_info("haptics %s enter use vreg enable = %d\n", __func__, value);

	if (value) {
		rc = haptics_wait_hboost_ready(chip, EXT_BOOST);
		if (!rc)
			pr_info("haptics wait ext hboost success\n");

		spin_lock_irqsave(&chip->boost_enable_lock, flags);
		rc = haptics_boost_vreg_enable(chip, true, EXT_BOOST);
		if (rc < 0) {
			pr_err("haptics disable ext_boost vreg failed, rc=%d\n", rc);
			spin_unlock_irqrestore(&chip->boost_enable_lock, flags);
			return rc;
		}
		spin_unlock_irqrestore(&chip->boost_enable_lock, flags);
	} else {
		hrtimer_start(&chip->ext_hbst_off_timer,
			ktime_set(BOOST_VREG_OFF_DELAY_SECONDS, 0),
			HRTIMER_MODE_REL);
	}
	return 0;
}
EXPORT_SYMBOL_GPL(haptics_enable_hboost_5v);
				...
				struct haptics_effect *effects, int effects_count)
{
	...
	for (i = 0; i < effects_count; i++) {
		...
		pr_debug("haptics effect %d\n", effect->id);
		pr_debug("haptics vmax_mv = %d\n", effect->vmax_mv);
		if (effect->pattern) {
			for (j = 0; j < SAMPLES_PER_PATTERN; j++) {
				sample = &effect->pattern->samples[j];
				pr_debug("haptics pattern = %d, period = %s, f_lra_x2 = %d\n",
						sample->amplitude,
						period_str[sample->period],
						sample->f_lra_x2);
			}

			pr_debug("haptics pattern play_rate_us = %d\n",
					effect->pattern->play_rate_us);
			pr_debug("haptics pattern play_length_us = %d\n",
					effect->pattern->play_length_us);
			pr_debug("haptics pattern preload = %d\n",
		...
		if (effect->fifo) {
			...
			pr_debug("haptics %s\n", str);
			kfree(str);
		pr_debug("haptics FIFO data preload: %d, play rate: %s, play length: %uus\n",
					effect->fifo->preload,
		...
		if (effect->brake && !effect->brake->disabled) {
			...
			pr_debug("haptics %s\n", str);
			kfree(str);
			pr_debug("haptics brake mode: %s\n",
					brake_str[effect->brake->mode]);
			pr_debug("haptics brake play length: %dus\n",
					effect->brake->play_length_us);
		}

		pr_debug("haptics pattern src: %s\n", src_str[effect->src]);
		pr_debug("haptics auto resonance %s\n",
			...
			enum hap_status_sel sel, u8 data[])
{
	...
	pr_debug("haptics Get status data[%s] = (%#x, %#x)\n", name, data[0], data[1]);
		...
		struct haptics_chip *chip, bool in_boot)
{
	...
	if (in_boot && ((chip->hw_type == HAP520) || (chip->hw_type == HAP520_MV))) {
		if (chip->hap_cfg_nvmem == NULL) {
			pr_debug("haptics nvmem device for hap_cfg is not defined\n");
			return -EINVAL;
		}
		...
		if (rc < 0) {
			pr_err("haptics read SDAM %#x failed, rc=%d\n",
	...
	pr_debug("haptics rc_clk_cal = %u, auto_res_done = %d\n",
	...
	if (rc_clk_cal == CAL_RC_CLK_DISABLED_VAL && !auto_res_done) {
		...
		pr_debug("haptics tlra_cl_err_sts = %#x\n", tlra_cl_err_sts);
		...
		pr_debug("haptics cal_tlra_cl_sts = %#x\n", cal_tlra_cl_sts);
		config->cl_t_lra_us = div_u64(tmp, 1000000);
		...
		pr_debug("haptics tlra_ol = %#x, tlra_cl_err_sts = %#x, cal_tlra_cl_sts = %#x\n",
		...
		pr_info("haptics config->rc_clk_cal_count == %u, config->t_lra_us = %u",
			config->rc_clk_cal_count, config->t_lra_us);
		config->rc_clk_cal_count = DEFAULT_RC_CLK_CAL_COUNT;
		config->t_lra_us = DEFAULT_T_LRA_US;
		config->cl_t_lra_us = DEFAULT_CL_T_LRA_US;
	} else if (rc_clk_cal == CAL_RC_CLK_AUTO_VAL && auto_res_done) {
		/*
		...
		pr_debug("haptics last_good_tlra_cl_sts = %#x, cal_tlra_cl_sts = %#x\n",
		...
		pr_info("haptics config->rc_clk_cal_count == %u, config->t_lra_us = %u",
			config->rc_clk_cal_count, config->t_lra_us);
		if (config->cl_t_lra_us > CL_T_LRA_US_MAX_LMT || config->cl_t_lra_us < CL_T_LRA_US_MIN_LMT) {
			config->cl_t_lra_us = DEFAULT_T_LRA_US;
			pr_info("haptics reconfig->rc_clk_cal_count == %u, config->t_lra_us = %u",
				config->rc_clk_cal_count, config->t_lra_us);
		}
	} else {
		pr_err("haptics Can't get close-loop LRA period in rc_clk_cal mode %u\n",
				rc_clk_cal);
		return -EINVAL;
	}

	pr_err("haptics OL_TLRA %u us, CL_TLRA %u us, RC_CLK_CAL_COUNT %#x\n",
...
static int haptics_set_vmax_mv(struct haptics_chip *chip, u32 vmax_mv)
{
	...
	if (vmax_mv > chip->max_vmax_mv) {
		pr_err("haptics vmax (%d) exceed the max value: %d\n",
	...
	if (rc < 0)
		pr_err("haptics config VMAX failed, rc=%d\n", rc);
	else
		pr_info("haptics Set Vmax to %u mV\n", vmax_mv);
...
static int haptics_set_vmax_headroom_mv(struct haptics_chip *chip, u32 hdrm_mv)
{
	...
	if (hdrm_mv > VMAX_HDRM_MAX_MV) {
		pr_err("haptics headroom (%d) exceed the max value: %d\n",
	...
	if (rc < 0)
		pr_err("haptics config VMAX_HDRM failed, rc=%d\n", rc);
...
static int haptics_get_vmax_headroom_mv(struct haptics_chip *chip, u32 *hdrm_mv)
{
	...
	rc = haptics_read(chip, chip->cfg_addr_base,
			HAP_CFG_VMAX_HDRM_REG, &val, 1);
	if (rc < 0) {
		pr_err("haptics Get Vmax HDRM failed, rc=%d\n",
				rc);
		return rc;
	}
...
static int haptics_enable_autores(struct haptics_chip *chip, bool en)
{
	...
	rc = haptics_masked_write(chip, chip->cfg_addr_base,
			...
			en ? AUTORES_EN_BIT : 0);
	if (rc < 0)
		pr_err("haptics %s auto resonance failed, rc=%d\n",
...
static int haptics_set_direct_play(struct haptics_chip *chip, u8 amplitude)
{
	...
	if (rc < 0)
		pr_err("haptics config DIRECT_PLAY failed, rc=%d\n", rc);
...
static bool is_boost_vreg_enabled_in_open_loop(struct haptics_chip *chip)
{
	...
	pr_info("haptics HAP_BOOST_VREG_EN_REG val = 0x%x\n", val);
	if (rc < 0)
		return false;

	chip->hboost_enabled = (val & VREG_EN_BIT);
	rc = haptics_read(chip, chip->hbst_addr_base, HAP_BOOST_HW_CTRL_FOLLOW_REG, &val, 1);
	pr_info("haptics HAP_BOOST_HW_CTRL_FOLLOW_REG val = 0x%x\n", val);
	if (rc < 0)
		return false;

	if (!(val & FOLLOW_HW_EN_BIT) && chip->hboost_enabled) {
		pr_info("haptics HBoost is enabled in open loop condition\n");
		return true;
	}
...
static int haptics_boost_vreg_enable(struct haptics_chip *chip, bool en, enum boost_source_t source)
{
	int rc;
	u8 val;
	static u32 vote_status = 0;
	bool final_vote_status;
	u8 cfg_ctl;

	pr_info("haptics haptics_boost_vreg_enable enter en = %u\n", en);
	...
	if (chip->hap_cfg_nvmem == NULL) {
		pr_info("haptics nvmem device for hap_cfg is not defined\n");
		return 0;
	}
	if (en)
		vote_status |= (1 << source);
	else
		vote_status &= ~(1 << source);

	if (vote_status > 0)
		final_vote_status = true;
	else
		final_vote_status = false;

	pr_info("haptics haptics_boost_vreg_enable vote_status = %u, final_vote_status = %u, hboost_enabled = %u\n",
		vote_status, final_vote_status, chip->hboost_enabled);
	if (chip->hboost_enabled == final_vote_status) {
		pr_info("haptics vote status not changed\n");
		return 0;
	}

	if (is_boost_vreg_enabled_in_open_loop(chip)) {
		pr_info("haptics Ignore %s hBoost while it's enabled in open-loop mode\n",
			en ? "enabling" : "disabling");
		return 0;
	}
	pr_info("haptics haptics_boost_vreg_enable not in open loop\n");

	if (chip->haptics_not_onboard) {
		cfg_ctl = final_vote_status ? HAPTICS_CFG_EN_CTL : HAPTICS_CFG_DIS_CTL;
		rc = haptics_write(chip, chip->cfg_addr_base, HAP_BOOST_VREG_EN_REG, &cfg_ctl, 1);
		if (rc < 0)
			pr_err("haptics HAP_BOOST_VREG_EN_REG failed, rc=%d\n", rc);
	}

	val = final_vote_status ? HAP_VREG_ON_VAL : HAP_VREG_OFF_VAL;
	rc = nvmem_device_write(chip->hap_cfg_nvmem,
			PBS_ARG_REG, 1, &val);
	if (rc < 0) {
		pr_info("haptics haptics write SDAM %#x failed, rc=%d\n",
				PBS_ARG_REG, rc);
		return rc;
	}
	...
	if (rc < 0)
		pr_info("haptics haptics Write SDAM %#x failed, rc=%d\n",
				PBS_TRIG_SET_REG, rc);
	pr_info("haptics %s now vote hoost vreg to %d\n", __func__, final_vote_status);
	chip->hboost_enabled = final_vote_status;
	return 0;
}
...
static int haptics_wait_hboost_ready(struct haptics_chip *chip, enum boost_source_t source)
{
	...
	pr_info("haptics haptics_wait_hboost_ready enter\n");

#ifdef CONFIG_HUAWEI_DSM
	hboost_status_check_dmd(chip);
#endif

	if (source == HAPTICS_BOOST) {
		if ((hrtimer_get_remaining(&chip->hbst_off_timer) > 0) ||
			hrtimer_active(&chip->hbst_off_timer)) {
			hrtimer_cancel(&chip->hbst_off_timer);
			pr_info("haptics hboost is still on, ignore\n");
			return 0;
		}
	} else if (source == EXT_BOOST) {
		if ((hrtimer_get_remaining(&chip->ext_hbst_off_timer) > 0) ||
			hrtimer_active(&chip->ext_hbst_off_timer)) {
			hrtimer_cancel(&chip->ext_hbst_off_timer);
			pr_info("haptics ext_hboost is still on, ignore\n");
			return 0;
		}
	}
	/*
	...
	for (i = 0; i < HBOOST_WAIT_READY_COUNT; i++) {
		...
		if (is_swr_play_enabled(chip)) {
			pr_debug("haptics Ignore waiting hBoost when SWR play is in progress\n");
		...
		pr_info("haptics %s HAP_BOOST_VREG_EN_REG = 0x%x\n", __func__, val);
		if (!rc && !(val & VREG_EN_BIT)) {
			pr_info("haptics %s rc = 0x%x\n", __func__, val);
			rc = haptics_read(chip, chip->hbst_addr_base,
					HAP_BOOST_STATUS4_REG, &val, 1);
			if (!rc && !(val & BOOST_DTEST1_STATUS_BIT)) {
				pr_info("haptics %s HAP_BOOST_STATUS4_REG = 0x%x\n", __func__, val);
				return 0;
			}
		}

		pr_info("haptics hBoost is busy, wait %d\n", i);
		usleep_range(HBOOST_WAIT_READY_INTERVAL_US,
				HBOOST_WAIT_READY_INTERVAL_US + 1);
	}

	if (i == HBOOST_WAIT_READY_COUNT) {
		pr_err("haptics hboost is not ready for haptics play\n");
		return -EBUSY;
	}
...
static int haptics_enable_hpwr_vreg(struct haptics_chip *chip, bool en)
{
	...
	pr_info("haptics haptics_enable_hpwr_vreg enter\n");
	if (chip->hpwr_vreg == NULL || chip->hpwr_vreg_enabled == en)
		return 0;

	pr_info("haptics haptics_enable_hpwr_vreg set vout\n");
	if (en) {
		rc = regulator_set_voltage(chip->hpwr_vreg,
				chip->hpwr_voltage_mv * 1000, INT_MAX);
		if (rc < 0) {
			pr_err("haptics Set hpwr voltage failed, rc=%d\n",
					rc);
			return rc;
		}
		...
		if (rc < 0) {
			pr_err("haptics Enable hpwr failed, rc=%d\n",
					rc);
		...
		if (rc < 0) {
			pr_err("haptics Disable hpwr failed, rc=%d\n",
					rc);
			return rc;
		}
...
static int haptics_open_loop_drive_config(struct haptics_chip *chip, bool en)
{
	...
	if ((is_boost_vreg_enabled_in_open_loop(chip) ||
		...
		if ((chip->wa_flags & TOGGLE_CAL_RC_CLK) &&
			...
			pr_debug("haptics Toggle CAL_EN in open-loop-VREG playing\n");
		}
...
static int haptics_enable_play(struct haptics_chip *chip, bool en)
{
	...
	if (rc < 0) {
		pr_err("haptics Write SPMI_PLAY failed, rc=%d\n", rc);
		return rc;
	}
	...
	if (en) {
		spin_lock_irqsave(&chip->boost_enable_lock, flags);
		rc = haptics_boost_vreg_enable(chip, true, HAPTICS_BOOST);
		if (rc < 0) {
			pr_err("haptics Keep boost vreg on failed, rc=%d\n",
				rc);
			spin_unlock_irqrestore(&chip->boost_enable_lock, flags);
			return rc;
		}
...
static int haptics_set_brake(struct haptics_chip *chip, struct brake_cfg *brake)
{
	...
	if (rc < 0) {
		pr_err("haptics set brake CFG failed, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("haptics set brake pattern failed, rc=%d\n", rc);
		return rc;
	}
		...
		enum pattern_src src)
{
	...
	if (src != PATTERN1 && src != PATTERN2) {
		pr_err("haptics no pattern src specified!\n");
		return -EINVAL;
	}
	...
	rc = haptics_write(chip, chip->ptn_addr_base, ptn_tlra_addr,
			values, 2);
	if (rc < 0) {
		pr_err("haptics update pattern TLRA failed, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("haptics write pattern data failed, rc=%d\n", rc);
		return rc;
	}
					...
					u8 *data, u32 length)
{
	...
	if (rc < 0) {
			pr_err("haptics bulk write FIFO_DIN failed, rc=%d\n",
				rc);
		return rc;
	}
	...
	if (left) {
		...
		rc = haptics_write(chip, chip->ptn_addr_base,
					...
					HAP_PTN_FIFO_DIN_NUM);
			if (rc < 0)
				pr_err("haptics write FIFO_DIN failed, rc=%d\n", rc);
		...
		for (i = 0; i < left; i++) {
			rc = haptics_write(chip, chip->ptn_addr_base,
					...
					(data + i), 1);
			if (rc < 0) {
				pr_err("haptics write FIFO_DIN_1B failed, rc=%d\n",
						rc);
				return rc;
			}
		}
...
static int haptics_get_fifo_fill_status(struct haptics_chip *chip, u32 *fill)
{
	...
	pr_debug("haptics filled=%u, available=%u, full=%d, empty=%d\n",
...
static int haptics_get_available_fifo_memory(struct haptics_chip *chip)
{
	...
	if (fill > get_max_fifo_samples(chip)) {
		pr_err("haptics Filled FIFO number %d exceed the max %d\n",
				fill, get_max_fifo_samples(chip));
		return -EINVAL;
	} else if (fill == get_max_fifo_samples(chip)) {
		pr_err("haptics no FIFO space available\n");
		return -EBUSY;
	}
...
static int haptics_set_manual_rc_clk_cal(struct haptics_chip *chip)
{
	...
	if (cal_count == 0) {
		pr_debug("haptics Ignore setting RC_CLK_CAL_COUNT\n");
		return 0;
	}
				...
				enum s_period period_per_s)
{
	...
	if (rc < 0)
		pr_err("haptics Set FIFO play rate failed, rc=%d\n", rc);
							...
							u32 thresh)
{
	...
	if (rc < 0)
		pr_err("haptics Set FIFO empty threshold failed, rc=%d\n",
				rc);
...
static int haptics_set_fifo(struct haptics_chip *chip, struct fifo_cfg *fifo)
{
	...
	if (rc < 0) {
		pr_err("haptics write FIFO samples failed, rc=%d\n", rc);
		return rc;
	}
...
static int haptics_load_constant_effect(struct haptics_chip *chip, u8 amplitude)
{
	...
	if (chip->play.in_calibration) {
		pr_err("haptics calibration in progress, ignore playing constant effect\n");
		rc = -EBUSY;
		goto unlock;
	}
					...
					struct haptics_effect *effect)
{
	...
	if (play->pattern_src == DIRECT_PLAY ||
		...
		pr_err("haptics pattern src %d can't be used for predefined effect\n",
				play->pattern_src);
		return -EINVAL;
	}
	...
	if (play->pattern_src == PATTERN1 || play->pattern_src == PATTERN2) {
		if (play->effect->pattern->preload) {
			pr_debug("haptics Ignore preloaded effect: %d\n",
					play->effect->id);
			return 0;
		}
			...
			s16 __user *data, u32 length, s16 magnitude)
{
	...
	pr_debug("haptics Copy custom FIFO samples successfully\n");
	fifo->num_s = custom_data.length;
	fifo->play_length_us = get_fifo_play_length_us(fifo,
			chip->custom_effect->t_lra_us);

	if (chip->play.in_calibration) {
		pr_err("haptics calibration in progress, ignore playing custom effect\n");
		rc = -EBUSY;
		goto cleanup;
	}
			...
			s16 __user *data, u32 length, s16 magnitude)
{
	...
	if (i == effects_count) {
		pr_err("haptics effect%d is not supported!\n",
	...
	mutex_lock(&chip->play.lock);
	if (chip->play.in_calibration) {
		pr_err("haptics calibration in progress, ignore playing predefined effect\n");
		rc = -EBUSY;
		goto unlock;
	}
	...
	if (rc < 0) {
		pr_err("haptics Play predefined effect%d failed, rc=%d\n",
				effects[i].id, rc);
		goto unlock;
	}
	mutex_unlock(&chip->play.lock);
...
static u8 get_direct_play_max_amplitude(struct haptics_chip *chip)
{
	...
	pr_info("haptics max amplitude for direct play: %#x\n", amplitude);
	return (u8)amplitude;
}
		...
		struct ff_effect *effect, struct ff_effect *old)
{
	...
	if (rc < 0) {
		pr_info("haptics disable hpwr_vreg failed, rc=%d\n");
		return rc;
	}
	...
	case FF_CONSTANT:
		...
		pr_info("haptics upload constant effect, length = %dus, amplitude = %#x\n",
				length_us, amplitude);
		haptics_load_constant_effect(chip, amplitude);
		if (rc < 0) {
			pr_err("haptics set direct play failed, rc=%d\n",
					rc);
			return rc;
		}
		hrtimer_start(&chip->timer,
			ktime_set(effect->replay.length / 1000, (effect->replay.length % 1000) * 1000000),
			HRTIMER_MODE_REL);

		break;
	case FF_PERIODIC:
		if (effect->u.periodic.waveform != FF_CUSTOM) {
			pr_err("haptics Only support custom waveforms\n");
			return -EINVAL;
		}
		...
		if (effect->u.periodic.custom_len ==
			...
			if (rc < 0) {
				pr_err("haptics Upload custom FIFO data failed\n",
						rc);
				return rc;
			}
			...
			rc = haptics_load_periodic_effect(chip,
					...
					effect->u.periodic.magnitude);
			if (rc < 0) {
				pr_err("haptics Upload periodic effect failed\n",
						rc);
				return rc;
			}
		}
		...
		break;
	default:
		pr_err("haptics %d effect is not supported\n",
				effect->type);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		pr_err("haptics enable hpwr_vreg failed, rc=%d\n", rc);
		return rc;
	}

	rc = haptics_wait_hboost_ready(chip, HAPTICS_BOOST);
	if (rc < 0 && chip->play.pattern_src == FIFO) {
		 ...
		 */
		pr_debug("haptics stop FIFO play explicitly to restore SW status\n");
		mutex_lock(&chip->play.lock);
...
static int haptics_stop_fifo_play(struct haptics_chip *chip)
{
	...
	if (atomic_read(&chip->play.fifo_status.is_busy) == 0) {
		pr_debug("haptics FIFO playing is not in progress\n");
		return 0;
	}
	...
	pr_debug("haptics stopped FIFO playing successfully\n");
	return 0;
}
...
static int haptics_playback(struct input_dev *dev, int effect_id, int val)
{
	...
	pr_info("haptics playback val = %d\n", val);
	if (chip->rtp_wave_is_running == true && val == 0) {
		pr_info("haptics_playback rtp_wave_is_running\n");
		return 0;
	}
	if (!!val) {
		...
		if (play->pattern_src == FIFO &&
				atomic_read(&play->fifo_status.is_busy)) {
			pr_err("haptics FIFO playing is not done yet, defer stopping in erase\n");
			return 0;
		}
...
static int haptics_erase(struct input_dev *dev, int effect_id)
{
	...
	pr_info("haptics haptics_erase enter\n");
	if (chip->rtp_wave_is_running == true) {
		pr_info("haptics haptics_erase rtp_wave_is_running, return\n");
		return 0;
	}
	mutex_lock(&play->lock);
	if ((play->pattern_src == FIFO) &&
			atomic_read(&play->fifo_status.is_busy)) {
		if (atomic_read(&play->fifo_status.written_done) == 0) {
			pr_debug("haptics cancelling FIFO playing\n");
			atomic_set(&play->fifo_status.cancelled, 1);
		}

		rc = haptics_stop_fifo_play(chip);
		if (rc < 0) {
			pr_err("haptics stop FIFO playing failed, rc=%d\n",
					rc);
			mutex_unlock(&play->lock);
			return rc;
		}
	...
	rc = haptics_enable_hpwr_vreg(chip, false);
	if (rc < 0)
		pr_err("haptics disable hpwr_vreg failed, rc=%d\n");
...
static void haptics_set_gain(struct input_dev *dev, u16 gain)
{
	...
	if (chip->play.pattern_src == DIRECT_PLAY) {
		...
		pr_debug("haptics Set amplitude: %#x\n", amplitude);
...
static int haptics_store_cl_brake_settings(struct haptics_chip *chip)
{
	...
	if (rc < 0)
		pr_err("haptics store RNAT/RCAL to SDAM failed, rc=%d\n");
...
{
	u32 cl_f_lra;
	u32 f0_cali_min = chip->config.vib_f0_ref - chip->config.vib_f0_range;
	u32 f0_cali_max = chip->config.vib_f0_ref + chip->config.vib_f0_range;

	if (chip->config.cl_t_lra_us == 0) {
		pr_info("haptics f0 = %d\n", chip->config.cl_t_lra_us);
		return -EINVAL;
	}

	cl_f_lra = USEC_PER_SEC / chip->config.cl_t_lra_us;
	if ((cl_f_lra < f0_cali_min) || (cl_f_lra > f0_cali_max)) {
		chip->config.cl_t_lra_us = USEC_PER_SEC / chip->config.vib_f0_ref;
		pr_info("haptics f0 check failed, f0 = %d\n", cl_f_lra);
		return -EINVAL;
	}
...
static int haptics_hw_init(struct haptics_chip *chip)
{
	...
	if (!chip->haptics_not_onboard) {
		...
		/* check frequency_hz */
		rc = haptics_frequency_hz_check(chip);
		if (rc < 0)
			pr_info("haptics f0 check, rc=%d\n", rc);
	}
...
static irqreturn_t fifo_empty_irq_handler(int irq, void *data)
{
	...
	if (atomic_read(&status->written_done) == 1) {
		...
		if (num != get_max_fifo_samples(chip)) {
			pr_debug("haptics %d FIFO samples still in playing\n",
		...
		pr_debug("haptics FIFO playing is done\n");
	} else {
		if (atomic_read(&status->cancelled) == 1) {
			pr_debug("haptics FIFO programming got cancelled\n");
			goto unlock;
		}
		...
		if (!fifo || !fifo->samples) {
			pr_err("haptics no FIFO samples available\n");
			goto unlock;
		}
		...
		if (rc < 0) {
			pr_err("haptics Update FIFO samples failed, rc=%d\n",
					rc);
			goto unlock;
		}
		...
		if (status->samples_written == fifo->num_s) {
			pr_debug("haptics FIFO programming is done\n");
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	kbuf = kzalloc(CHAR_PER_PATTERN_S * SAMPLES_PER_PATTERN + 1, GFP_KERNEL);
	if (!kbuf)
		return -ENOMEM;
...
static int haptics_create_debugfs(struct haptics_chip *chip)
{
	...
	if (IS_ERR(hap_dir)) {
		rc = PTR_ERR(hap_dir);
		pr_err("haptics create haptics debugfs directory failed, rc=%d\n",
				rc);
		return rc;
	}
	...
	if (IS_ERR(file)) {
		rc = PTR_ERR(file);
		pr_err("haptics create preload_effect_idx debugfs failed, rc=%d\n",
				rc);
		goto exit;
	}
		...
		struct device_node *node, struct haptics_effect *effect)
{
	...
	if (tmp > SAMPLES_PER_PATTERN * 3) {
		pr_err("haptics Pattern src can only play 8 samples at max\n");
		return -EINVAL;
	}
		...
		if (rc < 0) {
			pr_err("haptics Read wf-pattern-data failed, rc=%d\n",
					rc);
			return rc;
		}
		...
		for (i = 0; i < tmp / 3; i++) {
			if (data[3 * i] > 0x1ff || data[3 * i + 1] > T_LRA_X_8
					|| data[3 * i + 2] > 1) {
				pr_err("haptics allowed tuples: [amplitude(<= 0x1ff) period(<=6(T_LRA_X_8)) f_lra_x2(0,1)]\n");
				return -EINVAL;
			}
		...
		if (effect->pattern->preload) {
			if (config->preload_effect != -EINVAL) {
				pr_err("haptics effect %d has been defined as preloaded\n",
						config->preload_effect);
	...
	rc = of_property_read_u32(node, "qcom,wf-pattern-period-us", &tmp);
		if (rc < 0)
			pr_err("haptics Read qcom,wf-pattern-period-us failed, rc=%d\n",
					rc);
		else
			effect->pattern->play_rate_us = tmp;

		if (effect->pattern->play_rate_us > TLRA_MAX_US) {
			pr_err("haptics qcom,wf-pattern-period-us (%d) exceed the max value: %d\n",
					effect->pattern->play_rate_us,
					TLRA_MAX_US);
			return -EINVAL;
		}
	...
	effect->pattern->play_length_us = get_pattern_play_length_us(effect->pattern);
		if (effect->pattern->play_length_us == -EINVAL) {
			pr_err("haptics get pattern play length failed\n");
			return -EINVAL;
		}
		...
		struct device_node *node, struct haptics_effect *effect)
{
	...
	if (rc < 0) {
		pr_err("haptics Get qcom,wf-fifo-period failed, rc=%d\n", rc);
		return rc;
	} else if (tmp > F_48KHZ) {
		pr_err("haptics FIFO playing period %d is not supported\n",
					tmp);
			return -EINVAL;
		}
	...
	effect->fifo->period_per_s = tmp;
		...
		if (effect->fifo->play_length_us == -EINVAL) {
			pr_err("haptics get fifo play length failed\n");
			return -EINVAL;
		}
		...
		struct device_node *node, struct haptics_effect *effect)
{
	...
	if (effect->brake->mode > AUTO_BRAKE) {
		pr_err("haptics can't support brake mode: %d\n",
				effect->brake->mode);
		return -EINVAL;
	}
	...
	if (effect->brake->brake_wf == WF_SINE) {
		...
		if (effect->brake->sine_gain > BRAKE_SINE_GAIN_X8) {
			pr_err("haptics can't support brake sine gain: %d\n",
					effect->brake->sine_gain);
			return -EINVAL;
		}
	}
	...
	if (tmp > BRAKE_SAMPLE_COUNT) {
		pr_err("haptics more than %d brake samples\n",
				BRAKE_SAMPLE_COUNT);
		return -EINVAL;
	}
	...
	if (tmp > 0) {
		...
		if (rc < 0) {
			pr_err("haptics Read wf-brake-pattern failed, rc=%d\n",
					rc);
			return rc;
		}
...
static int haptics_parse_effects_dt(struct haptics_chip *chip)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		rc = haptics_parse_per_effect_dt(chip, child,
					&chip->effects[i]);
		if (rc < 0) {
			pr_err("haptics parse effect %d failed, rc=%d\n",
					i);
			of_node_put(child);
			return rc;
		}
...
static int haptics_parse_lra_dt(struct haptics_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,lra-period-us",
					&config->t_lra_us);
	if (rc < 0) {
		pr_err("haptics Read T-LRA failed, rc=%d\n", rc);
		return rc;
	}

	if (config->t_lra_us > TLRA_MAX_US) {
		pr_err("haptics qcom,lra-period-us (%d) exceed the max value: %d\n",
	...
	if (config->drv_wf >= WF_RESERVED) {
		pr_err("haptics Can't support drive shape: %d\n",
				config->drv_wf);
		return -EINVAL;
	}
	...
	if (config->brake.brake_wf >= WF_RESERVED) {
		pr_err("haptics Can't support brake shape: %d\n",
				config->brake.brake_wf);
		return -EINVAL;
	}
...
static int haptics_get_revision(struct haptics_chip *chip)
{
	...
	chip->hbst_revision = (val[1] << 8) | val[0];
		pr_debug("haptics haptics revision: HAP_CFG %#x, HAP_PTN %#x, HAP_HBST %#x\n",
...
static int haptics_parse_hpwr_vreg_dt(struct haptics_chip *chip)
{
	...
	if (IS_ERR(chip->hpwr_vreg)) {
		...
		if (rc != -EPROBE_DEFER)
			pr_err("haptics Failed to get qcom,hpwr-supply, rc=%d\n",
					rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("haptics Failed to read qcom,hpwr-voltage-mv, rc=%d\n",
				rc);
		return rc;
	}
...
static int haptics_parse_dt(struct haptics_chip *chip)
{
	...
	chip->haptics_not_onboard =
		of_property_read_bool(node, "qcom,haptics_not_onboard");
	if (of_find_property(node, "nvmem-cells", NULL)) {
		...
		if (IS_ERR(chip->cl_brake_nvmem)) {
			...
			if (rc != -EPROBE_DEFER)
				pr_err("haptics Failed to get nvmem-cells, rc=%d\n",
							rc);
	...
	if (of_find_property(node, "nvmem", NULL)) {
		...
		if (IS_ERR(chip->hap_cfg_nvmem)) {
			...
			if (rc != -EPROBE_DEFER)
				pr_err("haptics Failed to get hap_cfg nvmem device, rc=%d\n",
						rc);
			return rc;
		}
	}
	...
	if (of_find_property(node, "qcom,pbs-client", NULL)) {
		...
		if (!chip->pbs_node) {
			pr_err("haptics Failed to get PBS client\n");
			return -ENODEV;
		}
	}
	...
	if (!addr) {
		pr_err("haptics Read HAPTICS_CFG address failed\n");
		rc = -EINVAL;
		goto free_pbs;
	}
	...
	if (!addr) {
		pr_err("haptics Read HAPTICS_PATTERN address failed\n");
		rc = -EINVAL;
		goto free_pbs;
	}
	...
	pr_err("haptics Read HAPTICS_HBOOST address failed\n");
		rc = -EINVAL;
		goto free_pbs;
	} else if (addr != NULL) {
	...
	if (config->vmax_mv >= MAX_VMAX_MV) {
		pr_err("haptics qcom,vmax-mv (%d) exceed the max value: %d\n",
	...
	if (config->fifo_empty_thresh >= get_max_fifo_samples(chip)) {
		pr_err("haptics FIFO empty threshold (%d) should be less than %d\n",
	...
	if (tmp > 0) {
		...
		if (rc < 0) {
			pr_err("haptics Read brake-pattern failed, rc=%d\n",
					rc);
			goto free_pbs;
		}
	...
	if (config->is_erm) {
		...
		if (rc < 0) {
			pr_err("haptics Parse device-tree for LRA failed, rc=%d\n",
					rc);
			goto free_pbs;
		}
	}

	config->vib_f0_ref = DEFAULT_F0_REF;
	of_property_read_u32(node, "vib_f0_ref", &config->vib_f0_ref);

	config->vib_f0_range = DEFAULT_F0_RANGE;
	of_property_read_u32(node, "vib_f0_range", &config->vib_f0_range);

	boost_power_alwayson = 0;
	of_property_read_u32(node, "boost_power_alwayson", &boost_power_alwayson);
	pr_info("haptics Parse device-tree for boost, boost = %u\n", boost_power_alwayson);

	config->preload_effect = -EINVAL;
	rc = haptics_parse_effects_dt(chip);
	if (rc < 0) {
		pr_err("haptics Parse device-tree for effects failed, rc=%d\n",
				 rc);
		goto free_pbs;
	}
...
static int swr_slave_reg_enable(struct regulator_dev *rdev)
{
	...
	if (rc < 0) {
		pr_err("haptics Failed to enable SWR_PAT, rc=%d\n",
				rc);
		return rc;
	}
	...
	if (is_swr_play_enabled(chip)) {
		...
		if (rc < 0) {
			pr_err("haptics Failed to enable SWR_IGNORE, rc=%d\n", rc);
			return rc;
		}
		...
		rc = haptics_masked_write(chip, chip->cfg_addr_base,
				...
				SWR_IGNORE_BIT, 0);
		if (rc < 0) {
			pr_err("haptics Failed to disable SWR_IGNORE, rc=%d\n", rc);
			return rc;
		}
	}
done:
...
static int swr_slave_reg_disable(struct regulator_dev *rdev)
{
	...
	if (rc < 0) {
		pr_err("haptics Failed to disable SWR_PAT, rc=%d\n", rc);
		return rc;
	}
...
static int haptics_init_swr_slave_regulator(struct haptics_chip *chip)
{
	...
	if (rc < 0) {
		pr_err("haptics Failed to read SWR_ACCESS, rc=%d\n",
				rc);
		return rc;
	}
	...
	if (IS_ERR(chip->swr_slave_rdev)) {
		...
		if (rc != -EPROBE_DEFER)
			pr_err("haptics register swr-slave-reg regulator failed, rc=%d\n",
					rc);
	}
...
static int haptics_pbs_trigger_isc_config(struct haptics_chip *chip)
{
	...
	if (chip->pbs_node == NULL) {
		pr_err("haptics PBS device is not defined\n");
		return -ENODEV;
	}

	rc = qpnp_pbs_trigger_single_event(chip->pbs_node);
	if (rc < 0)
		pr_err("haptics Trigger PBS to config ISC failed, rc=%d\n",
				rc);
...
static u32 get_lra_impedance_capable_max(struct haptics_chip *chip)
{
	...
	pr_debug("haptics LRA impedance capable max: %u mohms\n", mohms);
	return mohms;
}
...
static int haptics_detect_lra_impedance(struct haptics_chip *chip)
{
	...
	for (i = 0; i < MAX_SWEEP_STEPS; i++) {
		...
		pr_debug("haptics sweeping milli_pct %u, amplitude %#x\n",
	...
	if (lra_min_mohms == 0)
		pr_warn("haptics Short circuit detected!\n");
	else if (lra_max_mohms >= capability_mohms)
		pr_warn("haptics Open circuit detected!\n");
	else
		pr_debug("haptics LRA impedance is between %u - %u mohms\n",
...
static int haptics_detect_lra_frequency(struct haptics_chip *chip)
{
	...
	if (rc < 0) {
		pr_err("haptics Read AUTORES_CFG failed, rc=%d\n", rc);
		return rc;
	}
...
static int haptics_start_lra_calibrate(struct haptics_chip *chip)
{
	...
	if ((chip->play.pattern_src == FIFO) &&
		...
		pr_err("haptics In FIFO playing, ignore calibration\n");
		rc = -EBUSY;
		goto unlock;
	}
	...
	rc = haptics_enable_play(chip, false);
	if (rc < 0) {
		pr_err("haptics Stop playing failed, rc=%d\n", rc);
		goto unlock;
	}
	...
	if (rc < 0) {
		pr_err("haptics Detect LRA frequency failed, rc=%d\n", rc);
		goto unlock;
	}
	...
	if (g_test == 1) {
		...
		if (rc < 0) {
		pr_err("haptics Detect LRA impedance failed, rc=%d\n", rc);
			goto unlock;
		}
	}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("haptics lra_calibration_store enter\n");
...
static int haptics_load_cust_effect(struct haptics_chip *chip,
			const u8 *data, u32 length, u32 max_mv)
{
	struct haptics_play_info *play = &chip->play;
	struct custom_fifo_data custom_data = {};
	struct fifo_cfg *fifo;
	...
	custom_data.idx = 0;
	custom_data.play_rate_hz = 16000;
	...
	fifo = chip->custom_effect->fifo;
	//memcpy(&custom_data, data, sizeof(custom_data));


	pr_info("haptics custom data length %d with play-rate %d Hz\n",
			custom_data.length, custom_data.play_rate_hz);
	rc = haptics_convert_sample_period(chip, custom_data.play_rate_hz);
	if (rc < 0) {
		pr_info("haptics Can't support play rate: %d Hz\n",
				custom_data.play_rate_hz);
		return rc;
	}
	...
	memcpy(fifo->samples, data,	custom_data.length);

	pr_info("haptics Copy custom FIFO samples successfully\n");
	fifo->num_s = custom_data.length;
	fifo->play_length_us = get_fifo_play_length_us(fifo,
			chip->custom_effect->t_lra_us);

	if (chip->play.in_calibration) {
		pr_info("haptics calibration in progress, ignore playing custom effect\n");
		rc = -EBUSY;
		goto cleanup;
	}
...
static char *haptics_ram_name = "haptics_rtp.bin";

static ssize_t lra_fifo_store(struct class *c,
		struct class_attribute *attr, const char *buf, size_t count)
{
	struct haptics_chip *chip = container_of(c,
			struct haptics_chip, hap_class);
	...
	if (val) {
		/* fw loaded */
		rc = request_firmware(&rtp_file, haptics_ram_name, chip->dev);
		if (rc < 0) {
			pr_info("haptics failed to read %s\n", haptics_ram_name);
			return rc;
		}
		rc = haptics_load_cust_effect(chip,
				rtp_file->data,
				rtp_file->size,
				DEFAULT_VMAX_MV);
		...
		rc = haptics_enable_hpwr_vreg(chip, true);
		if (rc < 0) {
			pr_info("haptics enable hpwr_vreg failed, rc=%d\n", rc);
			return rc;
		}

		rc = haptics_wait_hboost_ready(chip, HAPTICS_BOOST);
		if (rc < 0) {
			pr_info("haptics haptics_wait_hboost_ready failed, rc=%d\n", rc);
			return rc;
		}
		rc = haptics_enable_play(chip, true);
		if (rc < 0)
		...
		struct class_attribute *attr, char *buf)
{
	...
	pr_info("haptics lra_frequency_hz_show enter\n");
	if (chip->config.cl_t_lra_us == 0)
		return -EINVAL;
		...
		struct class_attribute *attr, char *buf)
{
	...
	pr_info("haptics lra_impedance_show enter\n");
	if (chip->config.measure_lra_impedance)
...
static struct attribute *hap_class_attrs[] = {
	...
	&class_attr_lra_fifo.attr,
	&class_attr_primitive_duration.attr,
	NULL,
};
ATTRIBUTE_GROUPS(hap_class);

static int qcom_file_open(struct inode *inode, struct file *file)
{
	if (!try_module_get(THIS_MODULE))
		return -ENODEV;

	file->private_data = (void *)g_chip;

	return 0;
}

static int qcom_file_release(struct inode *inode, struct file *file)
{
	if (!file)
		return 0;
...
static long qcom_file_unlocked_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	return 0;
}

static ssize_t qcom_file_read(struct file *filp, char *buff, size_t len,
	loff_t *offset)
{
	return len;
}
static enum hrtimer_restart haptics_vibrator_timer_func(struct hrtimer *timer)
{
	struct haptics_chip *chip = NULL;
	int rc;
	...
	rc = haptics_enable_play(chip, false);
	if (rc < 0) {
		pr_info("haptics disable play failed, rc=%d\n", rc);
		return rc;
	}
...
static int haptics_long_vibrator(struct haptics_chip *chip, int scale,
	int duration)
{
	int rc;
	int amplitude;

	if (scale > LONG_HAPTIC_RANGE) {
		pr_info("haptics scale %d is invalid\n", scale);
		return -1;
	}
	amplitude = (LONG_HAPTIC_RANGE + 1 - scale) * MAX_AMPLITUDE / LONG_HAPTIC_RANGE;
	rc = haptics_load_constant_effect(chip, amplitude);
	if (rc < 0) {
		pr_info("haptics load constant effect failed, rc=%d\n", rc);
		return rc;
	}
	...
	rc = haptics_enable_play(chip, true);
	if (rc < 0) {
		pr_info("haptics enable play failed, rc=%d\n", rc);
		return rc;
	}
...
static int haptics_short_vibrator(struct haptics_chip *chip, int scale,
	char *firmware_name)
{
	const struct firmware *rtp_file = NULL;
	int rc;
	...
	max_mv = (SHORT_HAPTIC_RANGE + 1 - scale) * DEFAULT_VMAX_MV / SHORT_HAPTIC_RANGE;

	pr_info("haptics fifo enter\n");

	/* fw loaded */
	rc = request_firmware(&rtp_file, firmware_name, chip->dev);
	if (rc < 0) {
		pr_info("haptics failed to read %s\n", firmware_name);
		return rc;
	}
...
static ssize_t qcom_file_write(struct file *filp, const char *buff,
	size_t len, loff_t *off)
{
	struct haptics_chip *chip = NULL;
	struct haptics_play_info *play = NULL;
	char write_buf[MAX_WRITE_BUF_LEN] = {0};
	char qcom_rtp_idx_name[QOCM_RTP_NAME_MAX] = {0};
	uint64_t type = 0;
	uint64_t rtp_type = 0;
	static uint64_t rtp_gap_time = 0;
	static unsigned long int gap_rt_time = 0;
	int amplitude;
	int index;
	int duration;
	int rc;
	...
	chip = (struct haptics_chip *)filp->private_data;
	if (!chip)
		return len;
	...
	pr_info("haptics %s: effect_id = %d\n",__func__, type);
	if (type < RTP_MAX_ID) {
		haptics_getnstimeofday(&now);
		gap_rt_time = (now.tv_sec - last.tv_sec) * 1000 +
			(now.tv_nsec - last.tv_nsec) / 1000000;
		pr_info("haptics gap_rt_time = %lu, rtp_gap_time = %lu\n", gap_rt_time, rtp_gap_time);
		if (gap_rt_time < rtp_gap_time) {
			pr_info("haptics hapic not end, wait\n");
			return len;
		}
		rtp_type = type / 10;
		rtp_gap_time = rtp_haptic_vb_time[rtp_type] + 2;
	}

	/* stop last play */
	if ((hrtimer_get_remaining(&chip->timer) > 0) ||
		hrtimer_active(&chip->timer)) {
		hrtimer_cancel(&chip->timer);
		pr_info("haptics last long effect is still on, ignore\n");
		haptics_enable_play(chip, false);
	}
	...
	if (type > LONG_HAPTIC_RTP_MAX_ID) { // long time
		...
		rc = haptics_long_vibrator(chip, amplitude, duration);
		if (rc < 0) {
			pr_info("haptics haptics_long_vibrator failed, rc=%d\n", rc);
			return rc;
		}
	} else { // short rtp haptic
		chip->rtp_wave_is_running = true;
		amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		len += snprintf(qcom_rtp_idx_name, QOCM_RTP_NAME_MAX - 1,
			"haptics_rtp_%d.bin", index);
		pr_info("haptics %s get rtp name = %s, index = %d, len = %d\n", __func__,
			qcom_rtp_idx_name, index, len);
		rc = haptics_short_vibrator(chip, amplitude, qcom_rtp_idx_name);
		haptics_getnstimeofday(&now);
		gap_rt_time = (now.tv_sec - last.tv_sec) * 1000 +
			(now.tv_nsec - last.tv_nsec) / 1000000;
		last.tv_sec = now.tv_sec;
		last.tv_nsec = now.tv_nsec;
		chip->rtp_wave_is_running = false;
		if (rc < 0) {
			pr_info("haptics haptics_short_vibrator failed, rc=%d\n", rc);
			return rc;
		}
	}
...
static const struct file_operations fops = {
	.owner = THIS_MODULE,
	.read = qcom_file_read,
	.write = qcom_file_write,
	.unlocked_ioctl = qcom_file_unlocked_ioctl,
	.open = qcom_file_open,
	.release = qcom_file_release,
};

static struct miscdevice qcom_haptic_misc = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = QCOM_HAPTIC_NAME,
	.fops = &fops,
};
...
static enum hrtimer_restart haptics_disable_hbst_timer(struct hrtimer *timer)
{
	...
	spin_lock_irqsave(&chip->boost_enable_lock, flags);
	rc = haptics_boost_vreg_enable(chip, false, HAPTICS_BOOST);
	if (rc < 0)
		pr_err("haptics disable boost vreg failed, rc=%d\n", rc);
	else
		pr_debug("haptics boost vreg is disabled\n");
	spin_unlock_irqrestore(&chip->boost_enable_lock, flags);

	return HRTIMER_NORESTART;
}

static enum hrtimer_restart haptics_disable_ext_hbst_timer(struct hrtimer *timer)
{
	struct haptics_chip *chip = container_of(timer,
		struct haptics_chip, ext_hbst_off_timer);
	int rc;
	unsigned long flags;

	spin_lock_irqsave(&chip->boost_enable_lock, flags);
	rc = haptics_boost_vreg_enable(chip, false, EXT_BOOST);
	if (rc < 0)
		pr_err("haptics disable ext_boost vreg failed, rc=%d\n", rc);
	else
		pr_debug("haptics ext_boost vreg is disabled\n");
	spin_unlock_irqrestore(&chip->boost_enable_lock, flags);
...
{
	/* HAP520_MV does not support soundwire ,Haptics config version 3 does not support soundwire */
	if (chip->hw_type == HAP520_MV || chip->cfg_revision == HAP_CFG_V3)
		return false;
...
#ifdef CONFIG_HUAWEI_DSM
static void hboost_client_register(void)
{
	if (vib_dclient == NULL)
		vib_dclient = dsm_register_client(&dsm_hboost);
}
#endif
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	if (!chip->regmap) {
		pr_err("haptics Get regmap failed\n");
		return -ENXIO;
	}

	rc = haptics_parse_dt(chip);
	if (rc < 0) {
		pr_err("haptics Parse device-tree failed, rc = %d\n", rc);
		return rc;
	}

	rc = haptics_init_custom_effect(chip);
	if (rc < 0) {
		pr_err("haptics Init custom effect failed, rc=%d\n", rc);
		return rc;
	}
	...
	hrtimer_init(&chip->ext_hbst_off_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	chip->ext_hbst_off_timer.function = haptics_disable_ext_hbst_timer;

	mutex_init(&chip->play.lock);
	spin_lock_init(&chip->boost_enable_lock);

	rc = haptics_hw_init(chip);
	if (rc < 0) {
		pr_err("haptics Initialize HW failed, rc = %d\n", rc);
		return rc;
	}
	...
	if (is_swr_supported(chip)) {
		...
		if (rc < 0) {
			pr_err("haptics Initialize swr slave regulator failed, rc = %d\n",
				rc);
			return rc;
		}
	}
	...
	if (rc < 0) {
		pr_err("haptics request fifo-empty IRQ failed, rc=%d\n",
				rc);
		return rc;
	}
	...
	chip->timer.function = haptics_vibrator_timer_func;

	if (chip->haptics_not_onboard)
		goto haptics_not_onboard;
	...
	if (rc < 0) {
		pr_err("haptics create input FF device failed, rc=%d\n",
				rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("haptics register input device failed, rc=%d\n",
				rc);
		goto destroy_ff;
	}
	...
	if (rc < 0) {
		pr_err("haptics register hap_class failed, rc=%d\n");
		goto destroy_ff;
	}
	...
	/* haptic init */
	if (of_property_read_bool(chip->dev->of_node, "vibrator_effect_support")) {
		rc = misc_register(&qcom_haptic_misc);
		if (rc < 0) {
			pr_info("haptics create haptic_misc failed, rc=%d\n",
					rc);
			return rc;
		}
	}

	pr_info("haptics %s exit\n", __func__);
...
haptics_not_onboard:
	...
	if (rc < 0)
		pr_err("haptics Creating debugfs failed, rc=%d\n", rc);
#endif
#ifdef CONFIG_HUAWEI_DSM
	hboost_client_register();
#endif
	g_chip = chip;
	return 0;
destroy_ff:
...
static int haptics_suspend_config(struct device *dev)
{
	...
	pr_info("haptics haptics_suspend enter\n");

	if (chip->haptics_not_onboard)
		return 0;

	pr_info("haptics haptics_suspend go disable schedule\n");
	mutex_lock(&play->lock);
	if ((play->pattern_src == FIFO) &&
			atomic_read(&play->fifo_status.is_busy)) {
		if (atomic_read(&play->fifo_status.written_done) == 0) {
	...
	if (chip->wa_flags & SW_CTRL_HBST) {
		hrtimer_cancel(&chip->hbst_off_timer);
		spin_lock_irqsave(&chip->boost_enable_lock, flags);
		haptics_boost_vreg_enable(chip, false, HAPTICS_BOOST);
		spin_unlock_irqrestore(&chip->boost_enable_lock, flags);
	}
	rc = haptics_enable_hpwr_vreg(chip, false);
	if (rc < 0)
