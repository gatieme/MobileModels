		...
		char *host_name)
{
	...
	pr_info("[IO_LATENCY_SCATTER]%s/%u: "
		...
		"%lu:[%lu/%lu/%lu],%lu:[%lu/%lu/%lu],"
		"above:[%lu/%lu/%lu]\n",
		host_name, HZ, gap_array[0], ring_node->stat_array[0].read_count,
		ring_node->stat_array[0].write_count, ring_node->stat_array[0].other_count,
		gap_array[1], ring_node->stat_array[1].read_count,
		ring_node->stat_array[1].write_count, ring_node->stat_array[1].other_count,
		gap_array[2], ring_node->stat_array[2].read_count,
		ring_node->stat_array[2].write_count, ring_node->stat_array[2].other_count,
		gap_array[3], ring_node->stat_array[3].read_count,
		ring_node->stat_array[3].write_count, ring_node->stat_array[3].other_count,
		gap_array[4], ring_node->stat_array[4].read_count,
		ring_node->stat_array[4].write_count, ring_node->stat_array[4].other_count,
		gap_array[5], ring_node->stat_array[5].read_count,
		ring_node->stat_array[5].write_count, ring_node->stat_array[5].other_count,
		gap_array[6], ring_node->stat_array[6].read_count,
		ring_node->stat_array[6].write_count, ring_node->stat_array[6].other_count,
		gap_array[7], ring_node->stat_array[7].read_count,
		ring_node->stat_array[7].write_count, ring_node->stat_array[7].other_count,
		gap_array[8], ring_node->stat_array[8].read_count,
		ring_node->stat_array[8].write_count, ring_node->stat_array[8].other_count,
		gap_array[9], ring_node->stat_array[9].read_count,
		ring_node->stat_array[9].write_count, ring_node->stat_array[9].other_count,
		gap_array[10], ring_node->stat_array[10].read_count,
		ring_node->stat_array[10].write_count, ring_node->stat_array[10].other_count,
		gap_array[11], ring_node->stat_array[11].read_count,
		ring_node->stat_array[11].write_count, ring_node->stat_array[11].other_count,
		ring_node->stat_array[12].read_count, ring_node->stat_array[12].write_count,
	...
	char *buf)
{
	...
	unsigned int  ring_buffer_index =
		latency_scatter->ring_buffer_current_index % IOMT_LATENCY_STAT_RING_BUFFER_SIZE;

	/* total */
	bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written, "total:\t");
	for (i = 0; i < IOMT_LATENCY_STAT_ARRAY_SIZE; i++)
		bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written, "%lu/%lu/%lu\t",
	...
	/* gap */
	bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written, "gap:\t");
	...
	for (i = 0; i < IOMT_LATENCY_STAT_RING_BUFFER_SIZE; i++) {
		ring_node = &latency_scatter->stat_ring_buffer[(++ring_buffer_index) % IOMT_LATENCY_STAT_RING_BUFFER_SIZE];
		bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written, "stat:\t");
		for (j = 0; j < IOMT_LATENCY_STAT_ARRAY_SIZE; j++)
			bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written,
				"%lu/%lu/%lu\t", ring_node->stat_array[j].read_count,
				ring_node->stat_array[j].write_count, ring_node->stat_array[j].other_count);
		bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written, "(%lu)\n", ring_node->stat_tick);
	}
	/* setting */
	bytes_written += scnprintf(buf + bytes_written, PAGE_SIZE - bytes_written,
...
static void init_latency_scatter_array(struct iomt_host_latency_stat *latency_scatter)
{
	unsigned int i;
	for (i = 0; i < IOMT_LATENCY_STAT_ARRAY_SIZE; i++) {
	...
	const struct iomt_host_ops *iomt_host_ops)
{
	...
	latency_scatter->queue_run_interval = IOMT_LATENCY_SCATTER_TIMER_INTERVAL;
	latency_scatter->attr_create_func = iomt_host_ops->create_latency_attr_func;
	latency_scatter->attr_delete_func = iomt_host_ops->delete_latency_attr_func;

	if (iomt_host_ops->host_name_func)
		latency_scatter->host_name = iomt_host_ops->host_name_func(host);

	INIT_DELAYED_WORK(&latency_scatter->latency_scatter_work, iomt_host_latency_scatter_work);
	latency_scatter->latency_scatter_workqueue = create_workqueue(latency_scatter->host_name);

	init_latency_scatter_array(latency_scatter);
	for (i = 0; i < IOMT_LATENCY_GAP_ARRAY_SIZE; i++) {
		latency_scatter->gap_array[i] = iomt_latency_gap_array[i];
		latency_scatter->high_acc_gap_array[i] = iomt_high_acc_latency_gap_array[i] * IOMT_MS_TO_US;
	}
	if (iomt_host_ops->reinit_latency_gap_array_func)
		iomt_host_ops->reinit_latency_gap_array_func(latency_scatter->gap_array, IOMT_LATENCY_GAP_ARRAY_SIZE);
	...
	if (latency_scatter->latency_scatter_workqueue != NULL)
		queue_delayed_work(latency_scatter->latency_scatter_workqueue,
			&latency_scatter->latency_scatter_work, latency_scatter->queue_run_interval);
}
