 ...
 *       @Filename:  iotrace.c
 *       @Description:
 *       @Author:  zhabin 00196516,
 *
 *        Version:  1.0
 *        Created:  07/06/2015 03:14:11 PM
 *        Revision:  none
 *
 *        Version:  2.0
 *        Modified: 11/24/2016 12:09:40 AM
 *        by: qiulaibin 00355850
 *
 *        Version:  2.1
 *        Modified: 11/29/2017 11:38:40 AM
 *        by: qiulaibin 00355850
 *        Description: apr for hi1861 fsr upload
 *
 *        Version:  3.0
 *        Modified: 1/26/2018 10:51:40 AM
 *        by: qiulaibin 00355850
 *        Description: 1) io_delay&cnt upload dmd.
 *                     2) add mode 0x5B for systrace
 *                     3) add ufs lrq tag for scsi cmd done
 *                     4) add filename for syscall
 *
 *        Version:  3.1
 *        Modified: 5/23/2018 11:38:40 AM
 *        by: qiulaibin 00355850
 *        Description: iotrace adapt with kenrel 4.9
 *
 * **********************************************************************************
 */
...
static int iotrace_ufs_fsr_get(void)
{
    ...
    if(fsr_scsi_tmp->device->host == NULL)
        return -1;
    ...
    if ( (hba->manufacturer_id == NULL) || (hba->manufacturer_id != 0x8B6) )
    {
        io_trace_print("fsr is only for hi1861!\n");
        return -1;
    }

    struct ufs_kirin_host *host_kirin = hba->priv;
...
static void scsi_dispatch_cmd_done(void *ignore, struct scsi_cmnd *cmd)
{
    ...
    if (io_trace_this->enable){
        io_trace_global_log(SCSI_BLK_CMD_RW_END_TAG, (*addr) * 8, (*size) * 16, SCSI_BLK_CMD_RW_START_TAG, 0, hba->lrb_in_use, rw_flags, NULL, 0);
    }
    return;
}
#endif
...
static int apr_work_thread_fn(void *data)
{
    ...
    while (1) {
        ...
        if((ufs_manufacture != 0x8b6) && apr_work_thread)
        {
        ...
        __set_current_state(TASK_RUNNING);
        if ((!io_trace_this->enable) || (fsr_scsi == NULL))
        {
            continue;
        }
...
static int __init io_trace_init(void)
{
    ...
    //start fsr get thead
    if((ufs_manufacture == 0x8b6) && (apr_work_thread == NULL)){
