...
struct ir_pwm {
	...
	unsigned long *ctrl_use;
	unsigned long *abr_sleep;
};
...
#define IR_PWM_UP_LIMIT_DELTA   50
#define IR_PWM_LOW_LIMIT_DELTA  (-50)
...
static void ir_pwm_enable(struct ir_pwm *ir_pwm, bool lock)
{
	int ret;
	...
	case USE_CLK_IMITATE:
		if (lock)
			...
			ret = clk_enable_nolock(ir_pwm->clk);
...
static void ir_pwm_disable(struct ir_pwm *ir_pwm, bool lock)
{
	switch (ir_pwm->pwm_mode) {
	case USE_PWM_SUBSYS:
...
static void ir_pwm_put_stat_value(struct ir_pwm *ir_pwm,
	unsigned long *buf, int index, unsigned long value)
{
	if (!buf)
		return;

	if ((index < 0) || (index > ir_pwm->tx_size))
		return;

	buf[index] = value;
}

#ifdef CONFIG_UBSAN_ARRAY_BOUNDS
#ifdef __clang__
__attribute__((no_sanitize("array-bounds")))
#endif /* __clang__ */
#endif /* CONFIG_UBSAN_ARRAY_BOUNDS */
	...
	unsigned long us)
{
	...
	while (tus > IR_PWM_SLEEP_THR) {
		...
		if (us <= delta) {
			ir_pwm_put_stat_value(ir_pwm, ir_pwm->abr_sleep,
				ir_pwm->tx_cnt - 1, delta);
...
static void ir_pwm_tx_next(struct ir_pwm *ir_pwm)
{
	...
	ir_pwm_put_stat_value(ir_pwm, ir_pwm->total_use,
		ir_pwm->tx_cnt - 1,
		ktime_us_delta(cur_time, ir_pwm->start_time));

	ir_pwm_put_stat_value(ir_pwm, ir_pwm->ctrl_use,
		ir_pwm->tx_cnt - 1,
		ktime_us_delta(cur_time, ir_pwm->ctrl_time));

next_wave:
	ir_pwm->tx_cnt++;
	ir_pwm->start_time = cur_time;
}

static void ir_pwm_set_cpu_to_current(void)
{
	long retval;
	int current_cpu;
	...
	hwlog_info("set cpu to current cpu %ld in %s\n",
		retval, __func__);
}

static void ir_pwm_set_cpu_to_all(void)
{
	long retval;
	cpumask_t dstp;
	...
	hwlog_info("set cpu to all cpu %ld in %s\n",
		retval, __func__);
}

static void ir_pwm_tx_prepare(struct ir_pwm *ir_pwm)
{
	struct sched_param param = { .sched_priority = MAX_RT_PRIO - 4 };

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	cpu_latency_qos_add_request(&ir_pwm->pm_qos, 0);
#else
	pm_qos_add_request(&ir_pwm->pm_qos, PM_QOS_CPU_DMA_LATENCY, 0);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0) */

	sched_setscheduler_nocheck(current, SCHED_FIFO, &param);

	ir_pwm_set_cpu_to_current();
}

static void ir_pwm_tx_unprepare(struct ir_pwm *ir_pwm)
{
	struct sched_param param = { .sched_priority = 0 };

	ir_pwm_set_cpu_to_all();

	sched_setscheduler_nocheck(current, SCHED_NORMAL, &param);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	cpu_latency_qos_remove_request(&ir_pwm->pm_qos);
#else
	pm_qos_remove_request(&ir_pwm->pm_qos);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0) */
}
...
static int ir_pwm_tx_thread(void *param)
{
	...
	while (true) {
		...
		while (ir_pwm->tx_cnt < ir_pwm->tx_size) {
			...
			if (!ir_pwm->tx_start)
				break;
			ir_pwm->ctrl_time = ktime_get();
		}
		ir_pwm_tx_next(ir_pwm);
		ir_pwm_tx_unlock(ir_pwm);
		ir_pwm->tx_start = false;
		complete(&ir_pwm->tx_done);
		ir_pwm_disable(ir_pwm, true);
		ir_pwm_tx_unprepare(ir_pwm);
	}

	return 0;
}

static void ir_pwm_free_buf(struct ir_pwm *ir_pwm)
{
	kfree(ir_pwm->total_use);
	ir_pwm->total_use = NULL;
	kfree(ir_pwm->ctrl_use);
	ir_pwm->ctrl_use = NULL;
	kfree(ir_pwm->abr_sleep);
	ir_pwm->abr_sleep = NULL;
}

static void ir_pwm_init_buf(struct ir_pwm *ir_pwm,
	const unsigned int *buf, u32 n)
{
	int stat_size = (n + 1) * sizeof(unsigned long);

	ir_pwm->tx_buf = buf;
	ir_pwm->tx_size = n;
	ir_pwm->tx_cnt = 0;

	ir_pwm_free_buf(ir_pwm);
	ir_pwm->total_use = kmalloc(stat_size, GFP_KERNEL);
	if (!ir_pwm->total_use)
		goto malloc_err;

	ir_pwm->ctrl_use = kmalloc(stat_size, GFP_KERNEL);
	if (!ir_pwm->ctrl_use)
		goto malloc_err;

	ir_pwm->abr_sleep = kmalloc(stat_size, GFP_KERNEL);
	if (!ir_pwm->abr_sleep)
		goto malloc_err;
...
static void ir_pwm_reset_buf(struct ir_pwm *ir_pwm)
{
	ir_pwm->tx_buf = NULL;
	ir_pwm->tx_cnt = 0;
	ir_pwm->tx_size = 0;
...
static void ir_pwm_trace_print(struct ir_pwm *ir_pwm, bool check_limit)
{
	int i;
	long delta;
	unsigned int count;

	if (!ir_pwm->total_use || !ir_pwm->ctrl_use || !ir_pwm->abr_sleep)
		return;

	count = (ir_pwm->tx_cnt < ir_pwm->tx_size) ? ir_pwm->tx_cnt :
		ir_pwm->tx_size;
	for (i = 0; i < count; i++) {
		delta = ir_pwm->total_use[i] - ir_pwm->tx_buf[i];
		if (check_limit && ((delta < IR_PWM_UP_LIMIT_DELTA) &&
			(delta > IR_PWM_LOW_LIMIT_DELTA)))
			continue;

		pr_err("%s_abr_delta: index=%d used=%ld need=%ld delta=%ld sleep=%ld ctrl=%ld\n",
			__func__, i, ir_pwm->total_use[i], ir_pwm->tx_buf[i],
			delta, ir_pwm->abr_sleep[i], ir_pwm->ctrl_use[i]);
	}
}

#ifdef IR_CORE_DEBUG
static void ir_pwm_debug_trace_print(struct ir_pwm *ir_pwm)
{
	ir_pwm_trace_print(ir_pwm, !ir_pwm->ir->trace_mode);
}
#else
static void ir_pwm_debug_trace_print(struct ir_pwm *ir_pwm)
{
	ir_pwm_trace_print(ir_pwm, true);
}
#endif /* IR_CORE_DEBUG */
...
static int ir_pwm_transmit(struct device *dev, const unsigned int *buf, u32 n)
{
	...
	pr_err("%s: use %ldus %u-%u\n", __func__, delta, ir_pwm->tx_cnt,
		ir_pwm->tx_size);
	ir_pwm_debug_trace_print(ir_pwm);
