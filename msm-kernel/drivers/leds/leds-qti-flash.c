...
#define HIVIEW_MAX_CONTENT_LEN_LED     150
#define HIVIEW_MAX_MODULE_NAME_LEN_LED 32
#define HIVIEW_MAX_IC_NAME_LEN_LED     32

static DEFINE_MUTEX(g_led_hiview_lock);

struct hwled_hievent_info {
	int error_no;
	char ic_name[HIVIEW_MAX_IC_NAME_LEN_LED];
	char module_name[HIVIEW_MAX_MODULE_NAME_LEN_LED];
	char content[HIVIEW_MAX_CONTENT_LEN_LED];
};

struct driver_led_dmd_extra_info {
	char ic_name[HIVIEW_MAX_IC_NAME_LEN_LED];
	char module_name[HIVIEW_MAX_MODULE_NAME_LEN_LED];
	char error_scene[HIVIEW_MAX_CONTENT_LEN_LED];
};
...
} driver_led_error_type;

struct driver_led_error_type_info {
	driver_led_error_type error_type;
	int error_type_base_no;
	const char* error_type_info;
};

static const struct driver_led_error_type_info
	g_driver_led_error_type_info[DRIVER_ERROR_TYPE_MAX] = {
	{ LED_FLASH_I2C_ERR, 927013080, "LED flash i2c error" },
};

struct led_dmd_info {
	driver_led_error_type error_type;
	struct driver_led_dmd_extra_info extra_info;
};

void vendor_hiview_report(struct hwled_hievent_info* driver_info)
{
	struct hiview_hievent* hi_event = NULL;
	if (!driver_info) {
		log_err("invalid params driver_info");
		return;
	}

	mutex_lock(&g_led_hiview_lock);
	hi_event = hiview_hievent_create(driver_info->error_no);
	if (!hi_event) {
		log_err("create hievent fail");
		mutex_unlock(&g_led_hiview_lock);
		return;
	}
...
void vendor_led_hiview_report(struct led_dmd_info* led_info)
{
	struct hwled_hievent_info driver_info;
	char error_info[HIVIEW_MAX_CONTENT_LEN_LED] = { 0 };
	int error_no = 0;
	int type;
	...
	if (led_info->error_type >= DRIVER_ERROR_TYPE_MAX ||
		led_info->error_type <= DRIVER_ERROR_TYPE_INVALID) {
		log_err("dmd error type is invalid");
		return;
	}

	for (type = 0; type < DRIVER_ERROR_TYPE_MAX; type++)
		if (g_driver_led_error_type_info[type].error_type == led_info->error_type) {
			error_no += g_driver_led_error_type_info[type].error_type_base_no;
			break;
		}

	if (type < DRIVER_ERROR_TYPE_MAX && strncat_s(error_info,
		HIVIEW_MAX_CONTENT_LEN_LED,
		g_driver_led_error_type_info[type].error_type_info,
		HIVIEW_MAX_CONTENT_LEN_LED - strlen(error_info) - 1))
		log_err("led hiview type info strncat_s failed");

	driver_info.error_no = error_no;
	if (strncpy_s(driver_info.content, sizeof(driver_info.content),
		error_info, sizeof(driver_info.content) - 1))
		log_err("led hiview type info strncpy_s failed");
	log_inf("camera led dmd error_no is %d, content is %s",
		driver_info.error_no, driver_info.content);
	vendor_hiview_report(&driver_info);
}

static bool vendor_is_led_dmd_enable()
{
	...
	if (!ret)
		...
		log_inf("led-dmd-enable %d", ret);

	return ret;
}

int vendor_led_hiview_init(struct led_dmd_info* info)
{
	if (memset_s(&(info->extra_info), sizeof(struct driver_led_dmd_extra_info),
		0, sizeof(struct driver_led_dmd_extra_info))) {
		log_err("led-dmd memset_s is fialed");
		return -1;
	}
...
static void vendor_led_hiview_handle(int error_type)
{
	struct led_dmd_info led_info;

	if (!vendor_is_led_dmd_enable())
		return;

	if (vendor_led_hiview_init(&led_info)) {
		log_err("led hiview init failed");
		return;
	}

	led_info.error_type = error_type;
	vendor_led_hiview_report(&led_info);
}
...
static irqreturn_t qti_flash_led_irq_handler(int irq, void *_led)
{
	...
	if (irq == led->led_fault_irq) {
		if (irq_status & FLASH_LED_FAULT_RT_STS) {
			...
			vendor_led_hiview_handle(LED_FLASH_I2C_ERR);
		}
