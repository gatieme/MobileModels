				...
				bool app_crash)
{
	...
	if (!memcmp(data->client.app_name, "securennxta64", strlen("securennxta64")) &&
			did_reboot == true) {
		dump_stack();
		pr_err("Do not unload securennxta64 app from tz when reboot\n");
		goto unload_exit;
	}
...
static ssize_t reboot_stat_write(struct file *file, const char __user *buf, size_t size,
				loff_t *pos)
{
	char tmp;
...
static ssize_t reboot_stat_read(struct file *file, char __user *buf, size_t size,
				loff_t *pos)
{
	return 0;
}

static const struct proc_ops reboot_stat_proc_ops = {
	.proc_write = reboot_stat_write,
	.proc_read  = reboot_stat_read,
};

static int qseecom_init(void)
{
	proc_create("reboot_stat", 0644, NULL, &reboot_stat_proc_ops);
