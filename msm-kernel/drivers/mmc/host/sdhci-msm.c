...
#include "cqhci.h"
#include "../core/core.h"
#if IS_ENABLED(CONFIG_SDSIM_MUX)
...
EXPORT_SYMBOL(mmc_host_det);
extern u32 g_sd_sim_detect_status_current;
extern bool g_sd_level_shifter;
extern int sdsim_detect_register(void(*p)(struct mmc_host *host));
extern int sim_gpio_setup_register(int(*p)(struct mmc_host *host));
#endif
						...
						struct sdhci_msm_host *msm_host)
{
	...
	if (sdhci_msm_dt_parse_vreg_info(dev,
        ...
        if (of_property_read_u32(np, "sd-card", &sd_card_value)) {
                dev_err(dev, "failed parsing sd-card data\n");
                sd_card_value = 0;
	}
        msm_host->mmc->is_sd_card = sd_card_value;
#if IS_ENABLED(CONFIG_SDSIM_MUX)
	if (sdhci_msm_dt_parse_vreg_info(dev,
	...
	if (of_property_read_u32(np, "sd-sim-boost-5v", &boost_5v_enable)) {
		dev_err(dev, "failed parsing sd-sim-boost-5v data\n");
		boost_5v_enable = 0;
	}
	msm_host->mmc->sd_sim_mux = value;
	msm_host->mmc->sd_sim_boost_5v = boost_5v_enable;
	...
	msm_host->mmc->sd_lvst_gpio_switch = of_get_named_gpio_flags(np, "sd-lvst-gpios", 0, NULL);
	pr_err("sdsim sd_lvst_gpio_switch %d\n", msm_host->mmc->sd_lvst_gpio_switch);
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
	g_sd_level_shifter = msm_host->uses_level_shifter;
#endif
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
EXPORT_SYMBOL(sdhci_msm_vreg_set_optimum_mode);
#endif
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
EXPORT_SYMBOL(sdhci_msm_vreg_set_voltage);
#endif
...
#endif
{
	...
	if (!strcmp(vreg->name, "vdd-io") && g_sd_level_shifter &&
		(g_sd_sim_detect_status_current == SD_SIM_SIM_NORMAL ||
		g_sd_sim_detect_status_current == SD_SIM_SIM_PASS)) {
		pr_err("sdsim this is sim card,not allow L6c enable,g_sd_sim_detect_status_current:%d", g_sd_sim_detect_status_current);
		return ret;
	}
#endif
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
EXPORT_SYMBOL(sdhci_msm_vreg_enable);
#endif
...
#if IS_ENABLED(CONFIG_SDSIM_MUX)
EXPORT_SYMBOL(sdhci_msm_vreg_disable);
#endif
			...
			bool enable, bool is_init)
{
	...
	if (mmc_card_is_removable(mmc) && (mmc->sd_sim_mux == 1)) {
		...
		if (!enable && !g_sd_level_shifter) {
			if (g_sd_sim_detect_status_current == SD_SIM_SIM_NORMAL ||
				g_sd_sim_detect_status_current == SD_SIM_SIM_PASS) {
				vreg_table[1]->is_always_on = true;
				pr_err("sdsim vdd-io(L6c) sim is_always_on %d\n",vreg_table[1]->is_always_on);
			} else {
				vreg_table[1]->is_always_on = false;
				pr_err("sdsim vdd-io(L6c) no sim is_always_on %d\n",vreg_table[1]->is_always_on);
			}
		} else {
			vreg_table[1]->is_always_on = false;
		}
...
static void sdhci_msm_handle_pwr_irq(struct sdhci_host *host, int irq)
{
	...
	if (mmc_card_is_removable(host->mmc) &&
		...
		pr_err("sdsim not sure the card result %d detect_nm_card_fail %d\n", host->mmc->sd_cmd_det_result, detect_nm_card_fail);
	} else {
		detect_nm_card_fail = false;
		pr_err("sdsim this is a sdcard result %d detect_nm_card_fail %d\n", host->mmc->sd_cmd_det_result, detect_nm_card_fail);
	}
#endif
	/* Handle BUS ON/OFF*/
	...
	if (pwr_state) {
		...
		if (!ret)
			ret = sdhci_msm_set_vqmmc(msm_host, mmc,
					pwr_state & REQ_BUS_ON);
#if IS_ENABLED(CONFIG_SDSIM_MUX)
		if (host->mmc->sd_cmd_det_result == -1 && g_sd_level_shifter) {
			pr_err("sdsim this is not a sdcard,no pincfg\n");
		} else {
			if (!ret)
...
struct mmc_host *g_sdio_host;
EXPORT_SYMBOL(g_sdio_host);
#endif
// modify end
...
// modify by hw
#ifdef CONFIG_CONNECTIVITY_HI110X
	if (strcmp(dev_name(&pdev->dev), "8844000.sdhci") == 0 || (strcmp(mmc_hostname(host->mmc), "mmc2") == 0)) {
		pr_err("%s: add sdio host\n", __func__);
		g_sdio_host = host->mmc;
	}
#endif
// modify end
...
static __maybe_unused int sdhci_msm_runtime_resume(struct device *dev)
{
	...
	sdhci_msm_vote_pmqos(msm_host->mmc, msm_host->sdhci_qos->active_mask);
...
{
	sdsim_detect_register(sdsim_detect);
	sim_gpio_setup_register(sim_gpio_setup);
	return platform_driver_register(&sdhci_msm_driver);
}
module_init(sdhci_msm_init)
#else
