...
static struct completion s_wifi_sysfs_init;

static bool hi11xx_init_completion_flag = 0;

static int hi11xx_wifi_init_completion(void)
{
	hi11xx_init_completion_flag = 0;
	init_completion(&s_wifi_sysfs_init);

	if (s_wifi_sysfs_init.done == 0)
		hi11xx_init_completion_flag = 1;

	return 0;
}

int hi11xx_wifi_plat_complete(void)
{
	if (hi11xx_init_completion_flag)
		complete(&s_wifi_sysfs_init);

	return 0;
}
EXPORT_SYMBOL(hi11xx_wifi_plat_complete);

struct completion *hi11xx_get_wifi_plat_completion(void)
{
	if (hi11xx_init_completion_flag) {
		return &s_wifi_sysfs_init;
	} else {
		return NULL;
	}
}
EXPORT_SYMBOL(hi11xx_get_wifi_plat_completion);
...
static void cnss_recovery_work_handler(struct work_struct *work)
{
	...
	if (!plat_priv->recovery_enabled) {
		trace_android_rb_sreason_set("wlan_crash");
...
static const struct platform_device_id cnss_platform_id_table[] = {
	...
	{ .name = "qca6750", .driver_data = QCA6750_DEVICE_ID, },
	{ },
};
