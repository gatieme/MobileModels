...
#define GF_BDF_FILE               "_bdwlang.bin"
...
static bool is_diff_chip_vendor(struct cnss_plat_data *plat_priv, char *filename, u32 filename_len)
{
	int length = 0;
	int wifi_parameter_len = 0;
	const char *wifi_ic_vendor = NULL;
	struct device_node *dp = NULL;
	...
	dp = of_find_node_by_path("/huawei_wifi_info");
	if (!dp) {
		cnss_pr_err("%s:huawei_wifi_info is null!\n", __func__);
		return false;
	} else {
		cnss_pr_dbg("%s:dp->name:%s,dp->full_name:%s;\n", __func__, dp->name, dp->full_name);
	}

	/* get wifi,ic_vendor from the device feature configuration (.dtsi file) */
	wifi_ic_vendor = of_get_property(dp, "wifi,ic_vendor", &length);
	if (wifi_ic_vendor == NULL) {
		pr_err("%s:iwifi,ic_vendor is null", __func__);
		return false;
	} else {
		pr_err("%s:get ic_vendor success:wifi_ic_vendor[%s] chipid[0x%x] boardid[0x%x].\n",
			__func__, wifi_ic_vendor, plat_priv->chip_info.chip_id, plat_priv->board_info.board_id);
	}

	if (strncmp(wifi_ic_vendor, "true", length) != 0)
		return false;

#ifdef CONFIG_CNSS2_CALIBRATION
	cnss_pr_err("%s:Enter CONFIG_CNSS2_CALIBRATION success.\n", __func__);
	file = filp_open(tempWifiCalBinPath, O_RDONLY, 0);
	if (!IS_ERR(file)) {
		/*在指定路径下找到了tempWlanCal_bdwlan.bin文件，会尝试加载这个bin，用于产线校准。*/
		cnss_pr_err("Find tempWlanCal_bdwlan.bin success.Try to load tempWlanCal_bdwlan.bin.\n");
		filp_close(file, NULL);
		snprintf(filename, filename_len, "%s"BDF_FILE, "tempWlanCal");
		return true;
	} else {
		cnss_pr_err("Find tempWlanCal_bdwlan.bin fail.Try to load bin file corresponding to boardid.\n");
	}
#endif

	pubfd = of_get_property(dp, "wifi,pubfile_id", &wifi_parameter_len);
	/* retrive bdf file name from capability */
	if (pubfd == NULL) {
		cnss_pr_err("%s:get wlan feature pubfd failed, use default bdf file.\n", __func__);
		snprintf(filename, filename_len, DEFAULT_BDF_FILE);
	} else {
		pr_err("%s: get wifi,pubfile_id[%s].\n", __func__, pubfd);
		if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK) {
			snprintf(filename, filename_len, "%s"GF_BDF_FILE, pubfd);
		} else {
...
//end
	...
	case CNSS_BDF_ELF:
		...
		break;
	case CNSS_BDF_BIN:
		is_support = is_diff_chip_vendor(plat_priv, filename_tmp, filename_len);
		if (!is_support) {
			...
			snprintf(filename, filename_len, filename_tmp);
			cnss_pr_err("%s: get filename[%s],filename_tmp[%s].\n", __func__, filename, filename_tmp);
		}
#if 0
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
