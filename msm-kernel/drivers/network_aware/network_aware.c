 ...
 * Description: this file is used for background speed limit.
 * Author: x00305155
 * Create: 2017-08-03
 */
...
#define AWARE_NET_CTRL_MODE_THIRD_LEVEL 11
...
static void limit_bg(void)
{
	...
	} else if (aware_net_ctrl.mode <= AWARE_NET_CTRL_MODE_THIRD_LEVEL) {
		sleep_long =
...
void tcp_network_aware(bool is_recving)
{
	...
	if (aware_net_ctrl.mode > AWARE_NET_CTRL_MODE_SECOND_LEVEL) {
		limit_bg();
		return;
	}
...
static const struct proc_ops proc_bg_uids_operations = {
	.proc_open = bg_uids_open,
	.proc_read = seq_read,
	.proc_write = bg_uids_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static const struct proc_ops proc_aware_ctrl_operations = {
	.proc_open = aware_ctrl_open,
	.proc_read = seq_read,
	.proc_write = aware_ctrl_write,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#endif
