...
#if IS_ENABLED(CONFIG_HW_RTG_PERF_CTRL)
	cap |= BIT(CAP_RTG_CMD);
#endif
#if IS_ENABLED(CONFIG_RENDER_RT)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
#if IS_ENABLED(CONFIG_RENDER_RT)
	register_trace_android_vh_remove_render_rthread(remove_render_rthread, NULL);
	register_trace_android_rvh_add_waker_to_render_rthread(add_waker_to_render_rthread, NULL);
	register_trace_android_rvh_add_render_rthread(add_render_rthread, NULL);
#endif
#if IS_ENABLED(CONFIG_HUAWEI_SCHED_STAT_YIELD)
	register_trace_android_rvh_sched_stat_yield_init(sched_stat_yield_init, NULL);
	register_trace_android_rvh_sched_stat_yield_update(sched_stat_yield_update, NULL);
#endif
#endif
	return 0;
}
