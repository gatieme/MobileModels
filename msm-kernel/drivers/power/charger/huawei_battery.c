...
#define pr_fmt(fmt)    "HUAWEI_BATTERY: " fmt
...
int huawei_battery_get_chg_usb_current(void)
{
	return chg_usb_current;
}
...
static void battery_detect_ibus_work(struct work_struct *work)
{
	...
	if (iusb_ma >= (IINLIM_1000)) {
		...
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_USB_ICL, VOTE_CLIENT_IBUS_DETECT,
			true, current_adjusted);
		if (rc < 0)
			dev_err(info->dev, "Vote for usb current limit fail, rc %d\n", rc);
		if (info->thermal_limit_current) {
			rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_USB_ICL, VOTE_CLIENT_THERMAL,
...
static inline int batt_imp_curr_limit(struct huawei_battery_info *info, int curr)
{
	...
	if (!info->jeita_hw_curr_limit) {
		...
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_FCC, VOTE_CLIENT_JEITA, true, curr);
	}
	if (rc < 0)
...
static inline int batt_imp_volt_limit(struct huawei_battery_info *info, int volt)
{
	...
	if (!info->jeita_hw_volt_limit)
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_VTERM, VOTE_CLIENT_JEITA, true, volt);
	if (rc < 0)
...
static inline int batt_imp_chg_dis(struct huawei_battery_info *info, bool enable)
{
	...
	if (!info->jeita_hw_chg_dis)
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_DIS_CHG, VOTE_CLIENT_JEITA, enable, 0);
	if (rc < 0)
...
static int battery_jeita_solution(struct huawei_battery_info *info)
{
	...
	case POWER_SUPPLY_HEALTH_GOOD:
		if (last_health == POWER_SUPPLY_HEALTH_WARM)
			chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_DIS_CHG, VOTE_CLIENT_WARM_WR, true, 0);
		...
		if (last_health == POWER_SUPPLY_HEALTH_WARM)
			chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_DIS_CHG, VOTE_CLIENT_WARM_WR, false, 0);
		break;
...
#if IS_ENABLED(CONFIG_HUAWEI_POWER_GLINK_INTERFACE)
	(void)power_glink_interface_get_property_value(POWER_GLINK_PROP_ID_GET_PLUGIN_STATUS, &plugin, 1);
	if (!(plugin & CHG_CABLE_STATUS_USBIN) &&
		(charge_detecion_get_sinksource_type() == POWER_SUPPLY_STOP_SINK)) {
		pr_info("%s plugin is %d, usb is not online\n", __func__, plugin);
		return;
	}
#endif
...
static int sync_max_voltage(struct huawei_battery_info *info)
{
	...
	rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_VTERM, VOTE_CLIENT_BASP, true, val.intval);
...
static int set_prop_input_current_max(struct huawei_battery_info *info, int curr)
{
	...
	return chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_USB_ICL, VOTE_CLIENT_USER,
		true, curr);
}
#endif /* CONFIG_HUAWEI_POWER_GLINK_INTERFACE */
...
static void charge_start_charging(struct huawei_battery_info *info)
{
	pr_info("---->START CHARGING\n");
	power_wakeup_lock(info->chg_ws, false);
	mod_delayed_work(system_power_efficient_wq, &info->monitor_charging_work, msecs_to_jiffies(0));
	/* notify start charging event */
	power_event_bnc_notify(POWER_BNT_CHARGING, POWER_NE_CHARGING_START, NULL);
}

static void charge_stop_charging(struct huawei_battery_info *info)
{
	...
	charge_set_first_insert(0); /* clear first insert flag */
#ifdef CONFIG_DIRECT_CHARGER
	if (!direct_charge_get_cutoff_normal_flag())
		direct_charge_exit();
...
{
	struct huawei_battery_info *info = g_info;
...
power_event_bnc_notify(POWER_BNT_CONNECT, POWER_NE_USB_DISCONNECT, NULL);
	...
	dc_reset_operate(DC_RESET_DPDM);
	charge_stop_charging(info);
}
...
static void charger_action_work(struct work_struct *work)
{
	...
	flag = queue_delayed_work(system_power_efficient_wq, &info->monitor_charging_work,
		msecs_to_jiffies(0));
	if (!flag) {
		cancel_delayed_work_sync(&info->monitor_charging_work);
		flag = queue_delayed_work(system_power_efficient_wq, &info->monitor_charging_work,
...
static void release_charge_lock(void)
{
	if (g_info)
		power_wakeup_unlock(g_info->chg_ws, false);
}

void huawei_battery_handle_charge_done(void)
{
	if (charge_get_charger_type() == CHARGER_TYPE_FCP) {
		...
		(void)hvdcp_set_adapter_voltage(ADAPTER_5V * POWER_MV_PER_V);
	}

	release_charge_lock();
}
EXPORT_SYMBOL_GPL(huawei_battery_handle_charge_done);
	...
	enum power_supply_property prop, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_DIS_CHG, VOTE_CLIENT_USER, !val->intval, 0);
		pr_err("%s: vote for dis charge = %d\n", __func__, !val->intval);
		goto OUT;
	case POWER_SUPPLY_PROP_VOLTAGE_BASP:
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_VTERM, VOTE_CLIENT_BASP, true, val->intval);
		goto OUT;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_VTERM, VOTE_CLIENT_USER, true, val->intval);
		goto OUT;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		rc = chg_vote_set(VOTE_OBJ_TYPE_BUCK, VOTE_OBJ_FCC, VOTE_CLIENT_USER, true, val->intval);
		goto OUT;
...
static struct charger_deteciton_ops huawei_battery_detection_ops = {
...
static int huawei_battery_probe(struct platform_device *pdev)
{
	...
	ret = charger_detecion_ops_register(&huawei_battery_detection_ops);
	if (ret) {
		pr_err("register charge detecion ops failed\n");
		goto PSY_REG_FAIL;
	}
	info->chr_type = charge_get_charger_type();
	if (info->chr_type == CHARGER_TYPE_STANDARD)
