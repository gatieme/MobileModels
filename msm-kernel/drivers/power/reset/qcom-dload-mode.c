			      ...
			      void *ptr)
{
	...
	if ((cmd != NULL && cmd[0] != '\0') && (!strcmp(cmd, "bootfail"))) {
		pr_err("rainbow mreason set RB_M_BOOTFAIL, sreason set upper bootfail, need warm reset!\n");
		trace_android_rb_mreason_set(RB_M_BOOTFAIL);
		trace_android_rb_sreason_set("upper bootfail");
		reboot_mode = REBOOT_WARM;
	}
#endif
