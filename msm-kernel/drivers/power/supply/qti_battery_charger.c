...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define QTI_BETA_FILE_PATH_1            "/hw_product/region_comm/china/log_collect_service_beta.xml"
#define QTI_BETA_FILE_PATH_2            "/system/etc/log_collect_service_beta.xml"
#define QTI_TIMEOUT_MAX_TIMES           3
static bool g_log_version_flag;
static u32 g_timeout_cnt;
#endif
enum psy_type {
	PSY_TYPE_BATTERY,
	PSY_TYPE_USB,
				...
				int len)
{
	...
	if (!rc) {
		...
		if (!rc) {
			pr_err("Error, timed out sending message\n");
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			if (g_log_version_flag && (g_timeout_cnt++ >= QTI_TIMEOUT_MAX_TIMES))
				BUG_ON(1); /* trigger crash dump for analyze timeout issue */
#endif
				...
				enum battery_charger_prop prop_id, int *val)
{
	...
	if (!strcmp(name, "battery"))
		psy = power_supply_get_by_name("bk_battery");
	else if (!strcmp(name, "usb"))
		psy = power_supply_get_by_name("bk_usb");
	else
...
static void power_send_charge_type_message(u32 msg)
{
	...
	case POWER_SUPPLY_USB_TYPE_SDP:
		power_glink_interface_handle_charge_type_message(POWER_GLINK_NOTIFY_VAL_SDP_CHARGER);
		break;
	case POWER_SUPPLY_USB_TYPE_DCP:
		power_glink_interface_handle_charge_type_message(POWER_GLINK_NOTIFY_VAL_DCP_CHARGER);
		break;
	case POWER_SUPPLY_USB_TYPE_CDP:
		power_glink_interface_handle_charge_type_message(POWER_GLINK_NOTIFY_VAL_CDP_CHARGER);
		break;
	default:
		break;
	}
...
static void battery_chg_update_usb_type_work(struct work_struct *work)
{
	...
	if (first_in) {
		...
		case POWER_SUPPLY_USB_TYPE_PD_PPS:
			power_glink_interface_handle_dc_connect_message(POWER_GLINK_NOTIFY_VAL_START_CHARGING);
			mdelay(50);
	...
	switch (pst->prop[USB_ADAP_TYPE]) {
	case POWER_SUPPLY_USB_TYPE_SDP:
		power_supply_update_usb_psy_type(POWER_SUPPLY_TYPE_USB);
		break;
	case POWER_SUPPLY_USB_TYPE_DCP:
	...
	case QTI_POWER_SUPPLY_USB_TYPE_HVDCP_3P5:
		power_supply_update_usb_psy_type(POWER_SUPPLY_TYPE_USB_DCP);
		break;
	case POWER_SUPPLY_USB_TYPE_CDP:
		power_supply_update_usb_psy_type(POWER_SUPPLY_TYPE_USB_CDP);
		break;
	case POWER_SUPPLY_USB_TYPE_ACA:
		power_supply_update_usb_psy_type(POWER_SUPPLY_TYPE_USB_ACA);
		break;
	case POWER_SUPPLY_USB_TYPE_C:
		power_supply_update_usb_psy_type(POWER_SUPPLY_TYPE_USB_TYPE_C);
		break;
	case POWER_SUPPLY_USB_TYPE_PD:
				...
				size_t len)
{
	...
	if (pst && pst->psy) {
		 ...
		 */
#if defined(CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION)
		if (notify_msg->notification == BC_USB_STATUS_GET)
...
static int battery_chg_init_psy(struct battery_chg_dev *bcdev)
{
	...
	usb_psy_desc.name = "bk_usb";
	usb_psy_desc.type = POWER_SUPPLY_TYPE_UNKNOWN;
	pr_info("register bk_battery and bk_usb\n");
	#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static int bk_usb_get_online(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_ONLINE, &value);
	return value;
}

static int bk_usb_get_voltage_now(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_VOLTAGE_NOW, &value);
	return value;
}

static int bk_usb_get_voltage_max(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_VOLTAGE_MAX, &value);
	return value;
}

static int bk_usb_get_current_now(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_CURRENT_NOW, &value);
	return value;
}

static int bk_usb_get_current_max(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_CURRENT_MAX, &value);
	return value;
}

static int bk_usb_get_input_current_limit(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &value);
	return value;
}

static int bk_usb_set_input_current_limit(int value)
{
	return power_supply_set_int_property_value("bk_usb", POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, value);
}

static int bk_usb_get_usb_type(void)
{
	int value = 0;

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_USB_TYPE, &value);
	return value;
}

static int bk_usb_get_temp(void)
{
	int value = -40; /* default usb conn temp */

	(void)power_supply_get_int_property_value("bk_usb", POWER_SUPPLY_PROP_TEMP, &value);
	return value;
}

static struct power_supply_ops usb_power_supply_ops = {
	.type_name = "usb",
	.get_online_prop = bk_usb_get_online,
	.get_voltage_now_prop = bk_usb_get_voltage_now,
	.get_voltage_max_prop = bk_usb_get_voltage_max,
	.get_current_now_prop = bk_usb_get_current_now,
	.get_current_max_prop = bk_usb_get_current_max,
	.get_input_current_limit_prop = bk_usb_get_input_current_limit,
	.set_input_current_limit_prop = bk_usb_set_input_current_limit,
	.get_usb_type_prop = bk_usb_get_usb_type,
	.get_temp_prop = bk_usb_get_temp,
};
#endif
...
static bool battery_chg_check_log_version(void)
{
	int ret;

	ret = ksys_access(QTI_BETA_FILE_PATH_1, 0);
	if (!ret) {
		pr_info("hw_product path access ok\n");
		return true;
	}

	ret = ksys_access(QTI_BETA_FILE_PATH_2, 0);
	if (!ret) {
		pr_info("system path access ok\n");
		return true;
	}
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	pr_err("%s init psy end, rc=%d\n", __func__, rc);
	if (rc < 0)
		goto error;

#if defined(CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION)
	if (power_supply_ops_register(&usb_power_supply_ops))
		pr_err("usb power supply ops register fail\n");
#endif
	...
	pr_err("%s ok\n", __func__);
#if defined(CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION)
	(void)qplat_fg_register();
	g_log_version_flag = battery_chg_check_log_version();
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
