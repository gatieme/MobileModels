...
#endif
/* AR000GATQJ add dump base transplantation */
#if IS_ENABLED(CONFIG_HUAWEI_MODEM_CRASH_LOG)
#include "../soc/qcom/pil-q6v5-mss/pil_q6v5_mss_log.h"
#include "../soc/qcom/pil-q6v5-mss/pil_q6v5_mss_ultils.h"
#define MODEM_SYS_NAME "4080000.remoteproc-mss"
...
#ifdef CONFIG_BLACKBOX
	err = snprintf(attach_info_buffer, RB_SREASON_STR_MAX, "%s_crash", q6v5->rproc->name);
	if (err >= 0)
		trace_android_rb_sreason_set(attach_info_buffer);
#endif
	/*
...
/* AR000GATQJ add dump base transplantation */
static void save_simple_dump(char *msg, struct qcom_q6v5 *q6v5)
{
	size_t trace_size;
	size_t crash_size;
	char *modem_log_trace;
	char *modem_log_crash;

	if (strcmp(q6v5->rproc->name, MODEM_SYS_NAME) == 0) {
		modem_log_trace = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_ERR_F3_TRACE_LOG, &trace_size);
		modem_log_crash = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_ERR_CRASH_LOG, &crash_size);
		save_modem_reset_log(msg, MAX_SSR_REASON_LEN);
		save_modem_err_log(modem_log_trace, trace_size, SMEM_ERR_F3_TRACE_LOG);
		save_modem_err_log(modem_log_crash, crash_size, SMEM_ERR_CRASH_LOG);
	}
}
...
/* AR000GATQJ add dump base transplantation */
#if IS_ENABLED(CONFIG_HUAWEI_MODEM_CRASH_LOG)
	dev_err(q6v5->dev, "CONFIG_HUAWEI_MODEM_CRASH_LOG success\n");
	save_simple_dump(msg, q6v5);
#endif
	return IRQ_HANDLED;
}
