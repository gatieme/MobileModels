...
#define RETRY_LOAD_COUNT 3
#define RETRY_LOAD_INTERNAL 1000
			   ...
			   size_t pd_count)
{
	...
	for (i = 0; i < pd_count; i++) {
		...
		if (ret < 0) {
			dev_err(pds[i], "[qdbg]get sync failed\n");
...
static int adsp_load(struct rproc *rproc, const struct firmware *fw)
{
	...
	dev_err(adsp->dev, "Enter Adsp_load rproc->firmware = %s!\n", rproc->firmware);
	dev_err(adsp->dev, "Enter Adsp_load adsp->pas_id = %d!\n", adsp->pas_id);
	rproc_coredump_cleanup(adsp->rproc);
...
retry:
	...
	if (ret == -EBUSY && retry_count < RETRY_LOAD_COUNT) {
		retry_count++;
		dev_err(adsp->dev, "Load failed with EBUSY, retry count = %d!\n", retry_count);
		msleep(RETRY_LOAD_INTERNAL);
		goto retry;
	}
...
static int adsp_start(struct rproc *rproc)
{
	...
	dev_err(adsp->dev, "[qdbg][%s] enter\n", __func__);

	qcom_q6v5_prepare(&adsp->q6v5);

	dev_err(adsp->dev, "[qdbg][%s] bus scaling\n", __func__);

	ret = do_bus_scaling(adsp, true);
	if (ret < 0)
		goto disable_irqs;

	dev_err(adsp->dev, "[qdbg][%s] pds_enable for active pds\n", __func__);

	ret = adsp_pds_enable(adsp, adsp->active_pds, adsp->active_pd_count);
	if (ret < 0)
		goto unscale_bus;

	dev_err(adsp->dev, "[qdbg][%s] pds_enable for proxy pds\n", __func__);

	ret = adsp_pds_enable(adsp, adsp->proxy_pds, adsp->proxy_pd_count);
	if (ret < 0)
		goto disable_active_pds;

	dev_err(adsp->dev, "[qdbg][%s] toggle load state\n", __func__);

	ret = adsp_toggle_load_state(adsp->qmp, adsp->qmp_name, true);
	if (ret)
		goto disable_proxy_pds;

	dev_err(adsp->dev, "[qdbg][%s] clk prepare for xo\n", __func__);

	ret = clk_prepare_enable(adsp->xo);
	if (ret)
		goto disable_load_state;

	dev_err(adsp->dev, "[qdbg][%s] clk prepare for aggre2\n", __func__);

	ret = clk_prepare_enable(adsp->aggre2_clk);
	if (ret)
		goto disable_xo_clk;

	dev_err(adsp->dev, "[qdbg][%s] enable regulators\n", __func__);

	ret = enable_regulators(adsp);
	if (ret)
		goto disable_aggre2_clk;

	dev_err(adsp->dev, "[qdbg][%s] auth_and_reset\n", __func__);
