...
/* AR000GATQJ add dump base transplantation */
#if IS_ENABLED(CONFIG_HUAWEI_MODEM_CRASH_LOG)
	if (strcmp(rproc->name, MODEM_SYS_NAME) == 0) {
		pr_err("remoteproc: CONFIG_HUAWEI_MODEM_CRASH_LOG create_modem_log_queue\n");
...
static int rproc_stop(struct rproc *rproc, bool crashed)
{
	...
	if (strcmp(rproc->name, MODEM_SYS_NAME) == 0) {
		...
		pr_err("pil_tz_scm_pas_probe: destroy_modem_log_queue\n");
	}
#endif
...
#ifdef CONFIG_BLACKBOX
	bbox_subsystem_crash_notify(rproc->name);
	dev_err(dev, "bbox_subsystem_crash_notify: %s\n", rproc->name);
#endif
...
static void rproc_crash_handler_work(struct work_struct *work)
{
	...
	if (rproc->state == RPROC_CRASHED || rproc->state == RPROC_OFFLINE) {
		...
		printk("rproc_crash_handler_work return due to RPROC_CRASHED or RPROC_OFFLINE\n");
		mutex_unlock(&rproc->lock);
