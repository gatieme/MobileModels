				 ...
				 enum fault_area area_type)
{
	...
	list_for_each_safe(pos, next, area_list_head) {
		...
		if (area_node->fault.start > index
			...
			continue;
		if (bdev && strcmp(disk_name, area_node->fault.disk) != 0)
			continue;
				  ...
				  block_t *relative_index, unsigned int *size)
{
	...
	if (disk_index >= cp_blkaddr && disk_index < cp_blkaddr + (cp_blocks >> 1)) {
		...
		*size = F2FS_BLKSIZE;
	} else if (disk_index >= cp_blkaddr + (cp_blocks >> 1) && disk_index < cp_blkaddr + cp_blocks) {
		/* cp2 */
		relative_block_offset =
		    disk_index - (cp_blocks >> 1) - cp_blkaddr;
		*area_type = FAULT_AREA_CP1;
		*size = F2FS_BLKSIZE;
	} else if (disk_index >= nat_blkaddr && disk_index < nat_blkaddr + nat_blocks) {
		/* nat */
	...
	} else if (disk_index >= sit_blkaddr && disk_index < sit_blkaddr + sit_blocks) {
		/* sit */
	...
	} else if (disk_index >= ssa_blkaddr && disk_index < ssa_blkaddr + ssa_blocks) {
		/* ssa */
...
#else
static int rasprobe_handler(get_meta_page_ex) (struct rasprobe_instance *ri, struct pt_regs *regs) {
#endif
