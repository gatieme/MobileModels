			      ...
			      struct dentry *den, int ct)
{
	...
	if (ci == 0) /* find the position to inject */
		return -ENOMEM;
	ci->ct = ct;
	ci->size = size;
	switch (ct) {
...
static int check_equal(void *dt, struct ChfilImpl *ci)
{
	...
	return (ci->den != 0 && chfilimpl_equal(ci, den)) ? 1 : 0;
}
...
static int rasprobe_handler(rw_verify_area) (struct rasprobe_instance *ri,
	...
	if (rw_verify_ret < 0 || !rd->args[2])
		return 0; /* nothing read return directly */

	pos = *(loff_t *)rd->args[2];
	if (!flp || !FILE_DEN(flp))
		return 0;
...
#endif
{
	...
	if (IS_ERR(fp))
		return 0;
	if (should_fail(fail_deplenish_check,
