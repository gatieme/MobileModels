...
static int qcom_glink_rx_data(struct qcom_glink *glink, size_t avail)
{
	...
	if (!left_size) {
		...
		if (channel->ept.cb) {
			...
			if (ret < 0) {
				if (ret != -ENODEV) {
					CH_ERR(channel, "callback error ret = %d", ret);
				}
				ret = 0;
			}
		} else {
...
static int qcom_glink_native_rx(struct qcom_glink *glink, int iterations, int irq)
{
	struct glink_msg msg;
	unsigned long flags;
	unsigned int param1;
...
static irqreturn_t qcom_glink_native_intr(int irq, void *data)
{
	...
	ret = qcom_glink_native_rx(glink, 10, irq);
...
static irqreturn_t qcom_glink_native_thread_intr(int irq, void *data)
{
	...
	qcom_glink_native_rx(glink, 0, irq);
				     ...
				     size_t size)
{
	...
	if (!ret) {
		...
		if (strcmp(glink->name, "slpi") == 0)
			panic("%s intent request ack timed out", glink->name);
		ret = -ETIMEDOUT;
