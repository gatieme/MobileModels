...
extern int ufshcd_ioctl_query_vcmd(struct ufs_hba *hba,
			    struct ufs_ioctl_query_data *ioctl_data,
			    void __user *buffer);
extern int ufs_ioctl_vendor_package(struct ufs_hba *hba,
	struct ufs_ioctl_vendor_state_t *ufs_vendor_ioctl_state,
	const void __user *buffer);
extern int ufs_ioctl_vendor_package_tick(struct ufs_hba *hba, struct scsi_device *dev,
	struct ufs_ioctl_vendor_state_t *ufs_vendor_ioctl_state,
	void __user *buffer);
#endif
...
extern void ufs_qcom_parse_clkscaling(struct ufs_hba *hba);
...
static void ufs_qcom_parse_irq_affinity(struct ufs_hba *hba)
{
	...
	if (np) {
		...
		host->perf_mask.bits[0] = (unsigned long)mask;
