 ...
 * ufs_qcom_parse_clkscaling - read from DTS whether clkscaling modes should be disabled.
 */
void ufs_qcom_parse_clkscaling(struct ufs_hba *hba)
{
	struct device_node *node = hba->dev->of_node;

	hba->disable_clkscaling = of_property_read_bool(node, "qcom,disable-clkscaling");
	if (hba->disable_clkscaling)
		dev_info(hba->dev, "(%s) All clkscaling is disabled\n",
			__func__);
}
EXPORT_SYMBOL(ufs_qcom_parse_clkscaling);
#endif
