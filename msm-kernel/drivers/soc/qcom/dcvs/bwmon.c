			...
			size_t count)
{
	...
	ret = kstrtouint(buf, 10, &ff_type);
	if (ret < 0 || ff_type > FF_FRAME_MAX_VALUE)
		return ret;
	spin_lock_irqsave(&node->ff_lock, flags);
	node->prev_ff_frame = ff_type ? node->cur_ff_frame : ff_type;
	node->cur_ff_frame = ff_type;
	if (node->cur_ff_frame)
		queue_work(bwmon_wq, &node->hw->work);
	spin_unlock_irqrestore(&node->ff_lock, flags);
	return count;
}
...
static ssize_t show_rw_bytes(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	struct hwmon_node *node = to_hwmon_node(kobj);
	unsigned int delta;
	...
	return scnprintf(buf, PAGE_SIZE, "%u\n", MBPS_TO_KHZ(mbps, node->hw->dcvs_width));
}

static BWMON_ATTR_RO(rw_bytes);
#endif
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
			spin_lock_irqsave(&hw->node->ff_lock, flags2);
			if (!hw->node->cur_ff_frame)
				queue_work(bwmon_wq, &hw->work);
			spin_unlock_irqrestore(&hw->node->ff_lock, flags2);
#else
			queue_work(bwmon_wq, &hw->work);
#endif
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
	spin_lock_init(&node->ff_lock);
	node->cur_ff_frame = 0;
	node->prev_ff_frame = 0;
#endif
