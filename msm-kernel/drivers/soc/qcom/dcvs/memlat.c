...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
#define FF_FRAME_MAX_VALUE 2
static ssize_t store_ff_frame(struct kobject *kobj,
			struct attribute *attr, const char *buf,
			size_t count)
{
	int ret;
	...
	ret = kstrtouint(buf, 10, &val);
	if (ret < 0 || val > FF_FRAME_MAX_VALUE)
		return ret;

	spin_lock_irqsave(&memlat_data->ff_lock, flags);
	memlat_data->prev_ff_frame = val ? memlat_data->cur_ff_frame : val;
	memlat_data->cur_ff_frame = val;
	/* prev == state => ff disabled || 2 consequetive non ff frame */
	memlat_data->shift_phase =
		(memlat_data->prev_ff_frame != memlat_data->cur_ff_frame);
	if (memlat_data->cur_ff_frame)
		queue_work(memlat_data->memlat_wq, &memlat_data->work);
	spin_unlock_irqrestore(&memlat_data->ff_lock, flags);

	return count;
}

static ssize_t show_ff_frame(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	ssize_t res = 0;
	unsigned long flags;
	spin_lock_irqsave(&memlat_data->ff_lock, flags);
	res = scnprintf(buf, PAGE_SIZE, "%u\n", memlat_data->cur_ff_frame);
	spin_unlock_irqrestore(&memlat_data->ff_lock, flags);
	return res;
}
#endif
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
	if (memlat_data->shift_phase) {
		mon->stored_cur_freq = mon->prev_cur_freq;
		mon->prev_cur_freq = mon->cur_freq;
		if (mon->stored_cur_freq)
			mon->cur_freq = mon->stored_cur_freq;
	} else {
		mon->prev_cur_freq = 0;
		mon->stored_cur_freq = 0;
	}
#endif
}
...
#ifdef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
	dev_data->cur_ff_frame = 0;
	dev_data->prev_ff_frame = 0;
	spin_lock_init(&dev_data->ff_lock);
	dev_data->shift_phase = false;
#endif
