...
TRACE_EVENT(icc_set_bw_ori,
	...
	TP_PROTO(struct icc_path *p, u32 icc_ab, u32 icc_ib),

	TP_ARGS(p, icc_ab, icc_ib),
	...
	TP_fast_assign(
		...
		__entry->icc_ab = icc_ab;
		__entry->icc_ib = icc_ib;
	),

	TP_printk("path=%s icc_ab(avg)=%u icc_ib(peak)=%u",
		  __get_str(path_name),
		  __entry->icc_ab,
		  __entry->icc_ib)
);
