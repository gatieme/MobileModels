...
#if IS_ENABLED(CONFIG_HW_POWEROFF_WDT)
...
void qcom_wdt_trigger_bite(void)
{
	...
	trace_android_rb_mreason_set(RB_M_AWDT);
	trace_android_rb_sreason_set("wdog_bite");
...
#if IS_ENABLED(CONFIG_HW_POWEROFF_WDT)
void qcom_wdt_pet_stop(void *unused, void *ignore)
{
...
void show_irq_stat(void) {
	if (wdog_data == NULL) {
		pr_err("%s get ptr failed", __func__);
	}
	pr_err("%s get wdog ptr", __func__);
	print_irq_stat(wdog_data);
}
EXPORT_SYMBOL(show_irq_stat);
