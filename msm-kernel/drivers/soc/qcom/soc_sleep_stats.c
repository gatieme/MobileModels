...
#include <trace/hooks/dubai_hooks.h>
#endif
...
static struct subsystem_data soc_subsystems[] = {
	{ "MPSS", 605, 1 },
	{ "WPSS", 605, 13 },
	{ "ADSP", 606, 2 },
	{ "ADSP_ISLAND", 613, 2 },
...
static struct stats_prv_data *soc_sleep_drv = NULL;
...
static void print_subsystem_sleep_stats_inner(void)
{
	struct sleep_stats *stat;
	int i;
	u64 accumulated;

	for (i = 0; i < ARRAY_SIZE(soc_subsystems); i++) {
		stat = qcom_smem_get(soc_subsystems[i].pid, soc_subsystems[i].smem_item, NULL);
		if (IS_ERR(stat))
			continue;
		...
		if (stat->last_entered_at > stat->last_exited_at)
			accumulated += arch_timer_read_counter()
			       - stat->last_entered_at;
#ifdef CONFIG_HUAWEI_DUBAI
		HWDUBAI_LOGE("DUBAI_TAG_SOC_SLEEP", "name=%s count=%d duration=%ld",
		    soc_subsystems[i].name, stat->count, accumulated);
#endif
		pr_err("\n%s\n"
			   "\tCount                    :%u\n"
...
static void print_soc_sleep_stats_inner(struct stats_prv_data *prv_data)
{
	struct sleep_stats stat;
	void __iomem *reg;
	int i;
	...
	for (i = 0; i < prv_data[0].config->num_records; i++) {
		reg = prv_data[i].reg;
		type = readl_relaxed(reg + STAT_TYPE_ADDR);
		...
		accumulated = readq(reg + ACCUMULATED_ADDR);

		stat.last_entered_at = get_time_in_sec(stat.last_entered_at);
		stat.last_exited_at = get_time_in_sec(stat.last_exited_at);
		accumulated = get_time_in_sec(accumulated);
...
#ifdef CONFIG_HUAWEI_DUBAI
		HWDUBAI_LOGE("DUBAI_TAG_SOC_SLEEP", "name=%s count=%d duration=%ld",
		    stat_type, stat.count, accumulated);
#endif
		pr_err("\n%s\n"
			   "\tCount                    :%u\n"
			   "\tLast Entered At(sec)     :%llu\n"
		...
		if (prv_data[0].config->appended_stats_avail) {
			...
			pr_err("\tClient Votes             :0x%x\n\n", app_stat.client_votes);
			reg += STAT_TYPE_ADDR + (i * sizeof(struct sleep_stats));
			reg += i * sizeof(struct appended_stats);
		} else {
			reg += STAT_TYPE_ADDR + (i * sizeof(struct sleep_stats));
		}
	}
...
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
void ddr_stats_get_freq_count(void *unused, int32_t *freq_cnt)
#else
...
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
void ddr_stats_get_residency(void *unused, int freq_count, struct ddr_freq_residency *data, int *ret)
#else
...
static int soc_sleep_pm_notify(struct notifier_block *nb, unsigned long mode, void *data)
{
	...
	case PM_POST_SUSPEND:
		print_soc_sleep_stats_inner(soc_sleep_drv);
		print_subsystem_sleep_stats_inner();
		break;
	default:
		break;
	}
...
static void soc_sleep_sr_init(void)
{
	int ret;
...
static void soc_sleep_sr_exit(void)
{
...
static int soc_sleep_stats_probe(struct platform_device *pdev)
{
	...
	soc_sleep_drv = devm_kzalloc(&pdev->dev, config->num_records *
				sizeof(struct stats_prv_data), GFP_KERNEL);
	if (!soc_sleep_drv)
		return -ENOMEM;

	for (i = 0; i < config->num_records; i++)
		soc_sleep_drv[i].config = config;
...
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
	register_trace_android_vh_ddr_stats_get_freq_count(ddr_stats_get_freq_count, NULL);
	register_trace_android_vh_ddr_stats_get_residency(ddr_stats_get_residency, NULL);
#endif
...
skip_ddr_stats:
#if IS_ENABLED(CONFIG_DEBUG_FS)
	root = create_debugfs_entries(reg_base, ddr_gdata->ddr_reg, soc_sleep_drv,
				      pdev->dev.of_node);
