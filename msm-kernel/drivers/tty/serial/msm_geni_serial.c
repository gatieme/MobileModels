...
#include <../../blackbox/platform/rainbow_himntn.h>
#include <platform/linux/rainbow.h>
extern void cmd_himntn_item_switch(unsigned int index, bool *rtn);
#endif
						...
						unsigned long arg)
{
	...
	if (port->pm_auto_suspend_disable) {
		UART_LOG_DBG(port->ipc_log_misc, uport->dev,
		"%s.pm_auto_suspend_disable, vote return.\n", __func__);
		return ret;
	}
#endif
	switch (cmd) {
	...
	if (!uart_console(uport)) {
		if (msm_port->ioctl_count) {
			...
			for (i = 0; i < msm_port->ioctl_count; i++) {
				UART_LOG_DBG(msm_port->ipc_log_pwr, uport->dev, "%s IOCTL vote present. Forcing off\n", __func__);
				msm_geni_serial_power_off(uport);
			}
		...
		if (ret) {
			UART_LOG_DBG(msm_port->ipc_log_pwr, uport->dev, "%s: Failed to suspend:%d\n", __func__, ret);
		}
		...
		if (!IS_ERR_OR_NULL(msm_port->serial_rsc.geni_gpio_shutdown)) {
			...
			if (ret)
				IPC_LOG_MSG(msm_port->ipc_log_misc, "%s: Error %d pinctrl_select_state\n", __func__, ret);
		}
...
static int msm_geni_serial_probe(struct platform_device *pdev)
{
	...
	if (is_console) {
		...
		if (dev_port->pm_auto_suspend_disable) {
			dev_info(&pdev->dev, "set pm auto suspend disable\n");
			pm_runtime_set_active(&pdev->dev);
			pm_runtime_forbid(&pdev->dev);
		} else {
			dev_info(&pdev->dev, "set pm auto suspend\n");
...
static struct platform_driver msm_geni_serial_pf_driver = {
...
static int __init msm_geni_serial_init(void)
{
	...
	ret = platform_driver_register(&msm_geni_serial_pf_driver);
	if (ret) {
		if (con_enabled)
...
{
	platform_driver_unregister(&msm_geni_serial_pf_driver);
	uart_unregister_driver(&msm_geni_serial_hs_driver);
	console_unregister(&msm_geni_console_driver);
}
