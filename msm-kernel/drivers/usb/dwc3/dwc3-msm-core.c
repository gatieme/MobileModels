...
#define HINT_INDEX_1 (u32)1
#define HINT_INDEX_2 (u32)2
#define HINT_INDEX_3 (u32)3
#define HINT_INDEX_4 (u32)4
#define HINT_INDEX_5 (u32)5
#define HINT_INDEX_6 (u32)6
#define HINT_INDEX_7 (u32)7
#define HINT_INDEX_8 (u32)8

#define hint_set_resume(hint, idx) ((hint) = (((hint) & 0xFFFFFF00) | (idx)))
#define hint_set_resume_work(hint, idx) ((hint) = (((hint) & 0xFFFF00FF) | ((idx) << 8)))
#define hint_set_suspend(hint, idx) ((hint) = (((hint) & 0xFF00FFFF) | ((idx) << 16)))
#define hint_set_sm_otg(hint, idx) ((hint) = (((hint) & 0x00FFFFFF) | ((idx) << 24)))

static u32 g_hint_index;
static enum usb_role g_last_role = USB_ROLE_NONE;
...
static int dwc3_msm_resume(struct dwc3_msm *mdwc)
{
	...
	struct dwc3 *dwc = NULL;
	struct usb_irq *uirq;
	enum usb_role role = dwc3_msm_get_role(mdwc);

	if (mdwc->dwc3)
		dwc = platform_get_drvdata(mdwc->dwc3);

	dev_info(mdwc->dev, "%s: exit lpm, role=%d, hint=%x\n", __func__, (int)role, g_hint_index);
	...
	mdwc->resume_pending = false;
	hint_set_resume(g_hint_index, 0);
	hint_set_resume_work(g_hint_index, 0);
...
static irqreturn_t msm_dwc3_pwr_irq_thread(int irq, void *_mdwc)
{
	...
	hint_set_resume_work(g_hint_index, HINT_INDEX_3);
	if (atomic_read(&mdwc->in_lpm)) {
		dwc3_resume_work(&mdwc->resume_work);
	} else {
		mdwc->resume_pending = false;
...
static int dwc3_msm_set_role(struct dwc3_msm *mdwc, enum usb_role role)
{
	...
	switch (role) {
	case USB_ROLE_HOST:
		g_last_role = USB_ROLE_HOST;
		mdwc->vbus_active = false;
		mdwc->id_state = DWC3_ID_GROUND;
		break;

	case USB_ROLE_DEVICE:
		g_last_role = USB_ROLE_DEVICE;
...
static void msm_dwc3_perf_vote_work(struct work_struct *w)
{
	...
	if (mdwc->irq_cnt >= PM_QOS_THRESHOLD)
		in_perf_mode = true;

	pr_debug("%s: in_perf_mode:%u, interrupts in last sample:%u\n",
		 __func__, in_perf_mode, mdwc->irq_cnt);
...
static void dwc3_core_complete(struct device *dev)
{
	...
	if (!mdwc->in_host_mode) {
		if (mdwc->resume_pending && (g_last_role == USB_ROLE_HOST))
