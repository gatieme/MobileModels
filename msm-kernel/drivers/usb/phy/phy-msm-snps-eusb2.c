...
static int msm_eusb2_repeater_reset_and_init(struct msm_eusb2_phy *phy)
{
	...
	ret = usb_repeater_reset(phy->ur, false);
	if (ret)
...
static int msm_eusb2_phy_probe(struct platform_device *pdev)
{
	...
	for (r_idx = 0; r_idx < MAX_USB_REPEATER_NUM; r_idx++) {
		ur = devm_usb_get_repeater_by_phandle(dev, "usb-repeater", r_idx);
		if (IS_ERR(ur)) {
			ret = PTR_ERR(ur);
			dev_err(dev, "%s, %d, usb-repeater %d get failed \n", __func__, __LINE__, r_idx);
			if (r_idx == MAX_USB_REPEATER_NUM - 1)
				goto err_ret;
			continue;
		}
		break;
