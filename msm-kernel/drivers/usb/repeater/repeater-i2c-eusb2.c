...
static struct gpio_desc *g_shared_reset_gpiod = NULL;
static struct gpio_desc **g_ti_reset_gpiod = NULL;
static struct gpio_desc **g_ds_reset_gpiod = NULL;
static bool g_repeater_added = false;
static int g_repeater_probe_times;
struct eusb2_repeater *g_eusb_repeater;
...
static int eusb2_repeater_power(struct eusb2_repeater *er, bool on)
{
	...
	dev_info(er->ur.dev, "%s turn %s regulators. power_enabled:%d\n",
	...
	pr_info("%s(): eUSB2 repeater egulators are turned ON.\n", __func__);
	return ret;
...
err_vdd18:
	...
	dev_info(er->ur.dev, "eUSB2 repeater's regulators are turned OFF.\n");
	return ret;
}
...
static int eusb2_repeater_init(struct usb_repeater *ur)
{
	...
	switch (er->chip->repeater_type) {
	case TI_REPEATER:
		eusb2_i2c_write_reg(er, 0x73, 0x83); /* adjust otg disconnect thr */
		eusb2_i2c_write_reg(er, 0x70, 0x7d); /* adjust tx */
		eusb2_i2c_write_reg(er, 0x71, 0x3b); /* adjust pre emphasis */
		break;
	case NXP_REPEATER:
		eusb2_i2c_write_reg(er, 0x08, 0x63); /* swing configure 500mV */
		eusb2_i2c_write_reg(er, 0x0A, 0x01); /* disconnet thr configure 675mV */
		eusb2_i2c_write_reg(er, 0x06, 0x50);
		eusb2_i2c_write_reg(er, 0x07, 0x00);
		break;
	default:
		break;
	}
...
static int eusb2_repeater_reset(struct usb_repeater *ur, bool bring_out_of_reset)
{
	...
	if (er->reset_gpiod)
		...
		dev_err(er->ur.dev, "TI repeater reset_gpiod not initialized yet\n");
...
static bool eusb2_repeater_present(struct eusb2_repeater *er)
{
	int ret;
	u8 reg_val = 0;
	int retry = 0;

	eusb2_repeater_reset(&(er->ur), false);
	ret = eusb2_repeater_powerup(&(er->ur));
	udelay(10); /* add delay between reset low and high */
	eusb2_repeater_reset(&(er->ur), true);
	udelay(3000); /* add delay after reset high */
	do {
		/* NXP and DIODES should respond 0x6:USB2 RX CONTROL */
		ret = eusb2_i2c_read_reg(er, 0x6, &reg_val);
		if (!ret) {
			dev_info(er->ur.dev, "succeed to read 0x6, val 0x%x\n", reg_val);
		} else {
			/* TI should respond 0x70 at least:TX_ADJUST_PORT1 */
			ret = eusb2_i2c_read_reg(er, 0x70, &reg_val);
			if (!ret)
				dev_info(er->ur.dev, "succeed to read 0x70, val 0x%x\n", reg_val);
		}
		retry++;
		dev_info(er->ur.dev, "read repeater ic reg, retry: %d\n", retry);
	} while (retry < 3 && ret);
	eusb2_repeater_powerdown(&(er->ur));
...
static const struct of_device_id eusb2_repeater_id_table[] = {
	...
	},
	{
		...
		.data = &repeater_chip[DS_REPEATER]
	},
	{ },
};
MODULE_DEVICE_TABLE(of, eusb2_repeater_id_table);

void eusb_repeater_reset_powerdown(void)
{
	int ret = 0;

	ret = eusb2_repeater_reset(&(g_eusb_repeater->ur), false);
	dev_info(g_eusb_repeater->ur.dev, "eusb_repeater reset result = %d\n", ret);
	ret = eusb2_repeater_powerdown(&(g_eusb_repeater->ur));
	dev_info(g_eusb_repeater->ur.dev, "eusb_repeater powerdown result = %d\n", ret);
}
...
static int eusb2_repeater_i2c_probe(struct i2c_client *client)
{
	...
	g_repeater_probe_times++;
	if (g_repeater_added) {
		dev_info(dev, "already enabled other repeaters, skip eusb2_repeater_i2c_probe\n");
		return 0;
	}
	...
	/* because single GPIO cannot share between different repeater instance, we need reuse shared_reset_gpiod among repeaters */
	switch (er->chip->repeater_type) {
	case NXP_REPEATER:
		...
		dev_info(dev, "NXP repeater init reset GPIO\n");
		g_shared_reset_gpiod = er->reset_gpiod;
		if (g_ti_reset_gpiod)
			*g_ti_reset_gpiod = g_shared_reset_gpiod;
		if (g_ds_reset_gpiod)
			*g_ds_reset_gpiod = g_shared_reset_gpiod;
		break;
	case TI_REPEATER:
		/* we just reuse the reset_gpiod for TI repeater */
		if (NULL != g_shared_reset_gpiod) {
			dev_info(dev, "g_shared_reset_gpiod inited, assign it to TI repeater\n");
			er->reset_gpiod = g_shared_reset_gpiod;
		} else {
			dev_info(dev, "g_shared_reset_gpiod not init, record the TI reaper gpiod for update later\n");
			g_ti_reset_gpiod = &(er->reset_gpiod);
		}
		break;
	case DS_REPEATER:
		if (NULL != g_shared_reset_gpiod) {
			dev_info(dev, "g_shared_reset_gpiod inited, assign it to DS repeater\n");
			er->reset_gpiod = g_shared_reset_gpiod;
		} else {
			dev_info(dev, "g_shared_reset_gpiod not init, record the DS reaper gpiod for update later\n");
			g_ds_reset_gpiod = &(er->reset_gpiod);
		}
		break;
	default:
		goto err_probe;
	...
	if (!eusb2_repeater_present(er) && (g_repeater_probe_times < MAX_REPEATER)) {
		devm_regulator_put(er->vdd18);
		devm_regulator_put(er->vdd3);
		dev_err(dev, "usb repeater probe done but it's not connected\n");

		return 0;
	}

	g_eusb_repeater = er;
	eusb_repeater_register(&eusb_repeater_reset_powerdown);
	ret = usb_add_repeater_dev(&er->ur);
	if (ret)
		goto err_probe;

	g_repeater_added = true;
	dev_info(dev, "usb repeater probe ok\n");
	return 0;
