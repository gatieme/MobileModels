 ...
 *
 ******************************************************/
const unsigned char aw8622x_reg_list[] = {
	AW8622X_REG_ID,
	AW8622X_REG_SYSST,
	AW8622X_REG_SYSINT,
	AW8622X_REG_SYSINTM,
...
static char aw8622x_rtp_idx_name[AW_RTP_NAME_MAX] = {0};
...
int aw8622x_haptic_stop(struct aw8622x *aw8622x)
{
	...
	while (cnt) {
		...
		if ((reg_val & AW8622X_BIT_GLBRD5_STATE) ==
			...
			cnt--;
			aw_dev_info("%s wait for standby, glb_state=0x%02X\n",
					__func__, reg_val);
...
static int aw8622x_haptic_ram_config(struct aw8622x *aw8622x)
{
	...
	aw_dev_info("%s : duration=%d,index = %d\n", __func__, aw8622x->duration, aw8622x->index);

	for (seq_idx = 0; seq_idx < 8; seq_idx++) {
		aw8622x_haptic_set_wav_seq(aw8622x, seq_idx, 0);
		aw8622x_haptic_set_wav_loop(aw8622x, seq_idx, 0);
	}
	if (aw8622x->effect_mode == SHORT_VIB_RAM_MODE) {
		wavseq = aw8622x->index;
		wavloop = 0;
		...
		aw_dev_info("%s config 1 :wavseq=%x, wavloop=%x\n", __func__, wavseq, wavloop);
	} else {
		/*
		if (aw8622x->duration <= 30) { // just play brk_wav one_time
			aw8622x_haptic_set_wav_seq(aw8622x, 0, AW8622X_BRK_WAV);
			aw8622x_haptic_set_wav_loop(aw8622x, 0, 0);
			aw8622x_haptic_set_wav_seq(aw8622x, 1, 0);
			aw8622x_haptic_set_wav_loop(aw8622x, 1, 0);
			aw_dev_info("%s config 2 :wavseq=%x, wavloop=%x\n", __func__, AW8622X_BRK_WAV,0);
		} else {
			total_wavloop = ((aw8622x->duration - 20) * 100) / 426;
			if (total_wavloop <= SEQ_LONG_EIGHT_CYCLE * AW8622X_LONG_SEQ_MAX_TIME){
				aw8622x_haptic_duraton_ram_config(aw8622x, total_wavloop);
				aw_dev_info("%s config 3 :wavseq=%x, wavloop=%x\n", __func__, wavseq, wavloop);
			}
			else{ // duration > 20+8*5.88*6*15 = 4253.6ms
				aw8622x_haptic_long_duraton_ram_config(aw8622x);
				aw_dev_info("%s config 4 :wavseq=%x, wavloop=%x\n", __func__, wavseq, wavloop);
			}
		 }*/

		aw8622x_haptic_set_wav_seq(aw8622x, 0, 4);
		aw8622x_haptic_set_wav_loop(aw8622x, 0, 0x0F);
		aw8622x_haptic_set_wav_seq(aw8622x, 1, 0);
		aw8622x_haptic_set_wav_loop(aw8622x, 1, 0);
		aw8622x_i2c_write_bits(aw8622x, AW8622X_REG_PLAYCFG3,
				AW8622X_BIT_PLAYCFG3_BRK_EN_MASK,
...
static ssize_t aw8622x_cali_val_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw8622x *aw8622x = container_of(cdev, struct aw8622x, vib_dev);
	...
	aw_dev_info("enter val show");
	if (!buf || !aw8622x)
		return 0;
	aw_dev_info("%s f0 val = %u\n", __func__, aw8622x->f0);

	if (aw8622x->f0_val_err < 0)
		aw8622x->f0 = 0;
	aw_dev_info("%s cal f0 val = %u\n", __func__, aw8622x->f0);

#ifdef F0_CONT_F0
	len += snprintf(buf + len, PAGE_SIZE - len, "aw8622x f0 = %d\n", aw8622x->f0);
#endif

	len += snprintf(buf + len, PAGE_SIZE - len, "%d", aw8622x->f0);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (aw8622x->f0 > AW8622X_F0_CALI_THER_H ||
		aw8622x->f0 < AW8622X_F0_CALI_THER_L)
		cal_rst = 0;
	else
		cal_rst = 1;
...
static DEVICE_ATTR(vibrator_calib, 0644, aw8622x_cali_show, aw8622x_cali_store);
static DEVICE_ATTR(cali_val, 0600, aw8622x_cali_val_show, NULL);
...
static void aw8622x_vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw8622x->state) {
		if (aw8622x->activate_mode ==
			...
			aw8622x_haptic_ram_config(aw8622x);
			if (aw8622x->duration <= AW_LONG_HAPTIC_RUNNING) { // max long vibra config
				aw8622x_haptic_ram_vbat_comp(aw8622x, false);
				aw8622x_haptic_play_mode(aw8622x, AW8622X_HAPTIC_RAM_MODE);
				aw8622x_haptic_play_go(aw8622x, true);
			} else {
				...
				AW_HAPTIC_ACTIVATE_CONT_MODE) {
				aw_dev_info("%s mode:%s\n", __func__,
...
static void aw_adapt_amp_again(struct aw8622x *aw8622x, int haptic_type)
{
	switch (haptic_type) {
	case LONG_VIB_RAM_MODE:
		if (aw8622x->amplitude >= AW_LONG_MAX_AMP_CFG || aw8622x->amplitude < 0)
			return;
		aw8622x->gain = long_amp_again_val[aw8622x->amplitude];
		aw_dev_info("long gain = %u\n", aw8622x->gain);
		break;
	case SHORT_VIB_RAM_MODE:
		if (aw8622x->amplitude >= AW_SHORT_MAX_AMP_CFG || aw8622x->amplitude < 0)
			return;
		aw8622x->gain = short_amp_again_val[aw8622x->amplitude];
		aw_dev_info("short gain = %u\n", aw8622x->gain);
		break;
	case RTP_VIB_MODE:
		aw8622x->gain = 0x80; // define max amp
		break;
	default:
		break;
	}
...
void aw8622x_haptic_file_config(struct aw8622x *aw8622x, uint64_t type)
{
	unsigned char reg_val = 0;
	int ret = 0;
	int rtp_len = 0;
	...
	if (type > LONG_HAPTIC_RTP_MAX_ID) { // long time
		aw8622x->effect_mode = LONG_VIB_RAM_MODE;
		aw8622x->index = LONG_VIB_EFFECT_ID;
		aw8622x->duration = type / LONG_TIME_AMP_DIV_COFF;
		...
		aw8622x->effect_mode = SHORT_VIB_RAM_MODE;
		aw8622x->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw8622x->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw_adapt_amp_again(aw8622x, SHORT_VIB_RAM_MODE);
		aw_dev_info("short index = %d, amp = %d\n", aw8622x->index, aw8622x->amplitude);
		aw8622x->state = 1;
		schedule_work(&aw8622x->vibrator_work);
	} else { // long and short rtp haptic
		aw8622x->effect_mode = RTP_VIB_MODE;
		aw8622x->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw8622x->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw8622x_haptic_stop(aw8622x);
		aw8622x_haptic_set_rtp_aei(aw8622x, false);
		ret = aw8622x_i2c_read(aw8622x, AW8622X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
		if (ret < 0) {
			aw_dev_err("%s: failed to read register 0x02: %d\n", __func__, ret);
		}
		aw_dev_info("%s SYSINT=%x\n", __func__,reg_val);
		aw_adapt_amp_again(aw8622x, RTP_VIB_MODE);
		aw8622x->state = 1;
		aw8622x->rtp_idx = aw8622x->index - BASE_INDEX;
		rtp_len += snprintf(aw8622x_rtp_idx_name, AW_RTP_NAME_MAX - 1,
			"aw_%d.bin", aw8622x->rtp_idx);
		aw_dev_info(" get rtp name = %s, index = %d, len = %d\n",
			aw8622x_rtp_idx_name,aw8622x->rtp_idx, rtp_len);
		schedule_work(&aw8622x->rtp_work);
	}
}