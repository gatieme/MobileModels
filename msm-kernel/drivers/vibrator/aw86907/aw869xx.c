	...
	size_t len, loff_t *off)
{
	...
	if (aw869xx->ram_init == 0) {
		pr_info("%s: container_update is not finished\n", __func__);
		return len;
	}
	...
	aw869xx->emergency_mode_flag = false;
	aw869xx->need_notify_duration = false;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aw869xx->effect_mode = LONG_VIB_RAM_MODE;
	aw869xx->need_notify_duration = false;
	if (aw869xx->duration > DURATION_NOTIFY_TIME_LIMIT)
	    aw869xx->need_notify_duration = true;
...
static void aw869xx_reset_valid_amp(struct aw869xx *aw869xx)
{
	if (aw869xx->set_amplitude_flag != 1)
		return;

	if (aw869xx->dts_info.is_enabled_lower_amp)
		aw869xx->gain = long_amp_again_val[THIRD_LONG_HAPTIC_AMP];

	aw869xx->set_amplitude_flag = 0;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aw869xx_reset_valid_amp(aw869xx);
	aw869xx->state = val;
	mutex_unlock(&aw869xx->lock);
	schedule_work(&aw869xx->long_vibrate_work);
	return count;
...
static ssize_t aw869xx_set_amplitude_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw869xx *aw869xx = NULL;
...
static DEVICE_ATTR(set_amplitude, S_IWUSR | S_IRUGO, NULL,
	aw869xx_set_amplitude_store);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw869xx_emergency_mode(struct aw869xx *aw869xx)
{
	int ret;
	int ele = 0;
	...
	if (ret < 0) {
		pr_err("%s: get capacity failed", __func__);
		return;
	}
	if (ele <= 1) {
		pr_info("%s: enter!", __func__);
		aw869xx->emergency_mode_flag = true;
	}
}
#endif

static void aw869xx_boost_config(struct aw869xx *aw869xx)
{
	if (!aw869xx->dts_info.is_enabled_lowpower_bst_config) {
		aw869xx_haptic_bst_mode_config(aw869xx, AW869XX_HAPTIC_BST_MODE_BOOST);
		return;
	}
	if (aw869xx->emergency_mode_flag)
		aw869xx_haptic_bst_mode_config(aw869xx, AW869XX_HAPTIC_BST_MODE_BYPASS);
	else
		aw869xx_haptic_bst_mode_config(aw869xx, AW869XX_HAPTIC_BST_MODE_BOOST);
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw869xx->dts_info.is_enabled_lowpower_bst_config)
		aw869xx_emergency_mode(aw869xx);
#endif

	if (aw869xx->cust_boost_on == AW869XX_SEL_BOOST_CFG_ON) {
		aw869xx_haptic_ram_vbat_compensate(aw869xx, false);
		aw869xx_boost_config(aw869xx);
		aw869xx_haptic_play_mode(aw869xx, AW869XX_HAPTIC_RAM_MODE);
		aw869xx_haptic_play_go(aw869xx);
		return;
	}
...
static void aw869xx_vib_duration_notify(struct aw869xx *aw869xx)
{
	if (aw869xx->state && aw869xx->need_notify_duration) {
		vibrator_duration_distinguish(aw869xx->duration);
		aw869xx->need_notify_duration = false;
	} else {
...
static void aw869xx_long_vibrate_work_routine(struct work_struct *work)
{
	...
	aw869xx_haptic_stop(aw869xx);
	aw869xx_haptic_upload_lra(aw869xx, F0_CALI);
	aw869xx_vib_duration_notify(aw869xx);
	if (aw869xx->state) {
	...
	struct device_node *np)
{
	...
	aw869xx->dts_info.is_enabled_lowpower_bst_config = of_property_read_bool(np,
		"vib_lowpower_bst_config");
	pr_info("%s aw869xx->dts_info.is_enabled_lowpower_bst_config = %d\n",
		__func__, aw869xx->dts_info.is_enabled_lowpower_bst_config);

	aw869xx->dts_info.is_enabled_lower_amp = of_property_read_bool(np,
		"vib_is_enabled_lower_amp");
	pr_info("%s aw869xx->dts_info.is_enabled_lower_amp = %d\n",
		__func__, aw869xx->dts_info.is_enabled_lower_amp);
...
struct i2c_driver aw869xx_i2c_driver = {
	.driver = {
...
EXPORT_SYMBOL(aw869xx_i2c_driver);
