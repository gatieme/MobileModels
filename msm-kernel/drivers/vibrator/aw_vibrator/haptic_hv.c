	...
	char *buff, size_t len, loff_t *offset)
{
	...
	case AW_HAPTIC_CMD_READ_REG:
		...
		if (pbuff != NULL) {
			...
			if (ret) {
				aw_err("copy to user fail\n");
			}
			kfree(pbuff);
		} else {
			aw_err("alloc memory fail\n");
		}
		break;
	default:
		aw_err("unknown cmd %d\n", aw_haptic->fileops.cmd);
		break;
	}
	...
	const char *buff, size_t len, loff_t *off)
{
	...
	aw_haptic->need_notify_duration = false;
	...
	if (type > LONG_HAPTIC_RTP_MAX_ID) { // long time
		aw_haptic->need_notify_duration = true;
		if (type > (HAPTIC_LM_LONG_VB_BASE_VAL + LONG_HAPTIC_RTP_MAX_ID)) {
			// only for lm 8 special types
			type = type - HAPTIC_LM_LONG_VB_BASE_VAL;
			aw_info("get adapt haptic id = %llu", type);
			aw_haptic->need_notify_duration = false;
		}
...
static void vib_duration_notify(struct aw_haptic *aw_haptic)
{
	if (aw_haptic->state && aw_haptic->need_notify_duration == true) {
		vibrator_duration_distinguish(aw_haptic->duration);
		aw_haptic->need_notify_duration = false;
	} else {
...
static void reset_valid_amp(struct aw_haptic *aw_haptic)
{
	if (aw_haptic->set_amplitude_flag != 1)
		return;

	if (aw_haptic->info.is_enabled_lower_amp)
		aw_haptic->gain = long_amp_again_val[THIRD_LONG_HAPTIC_AMP];

	aw_haptic->set_amplitude_flag = 0;
}
							...
							const char *buf, size_t count)
{
	...
	aw_haptic->state = val;
	aw_haptic->need_notify_duration = false;
	if (aw_haptic->duration > DURATION_NOTIFY_TIME_LIMIT && aw_haptic->state == 1)
		aw_haptic->need_notify_duration = true;
	aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
	aw_haptic->index = LONG_VIB_BOOST_EFFECT_ID;
	aw_haptic->gain = long_amp_again_val[MAX_LONG_HAPTIC_AMP];
	reset_valid_amp(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
	schedule_work(&aw_haptic->vibrator_work);
	...
	if (sscanf(buf, "%x %x %x", &flag, &reg_num, &reg_addr) == 3) {
		...
		if (flag == AW_SEQ_WRITE) {
			...
			if (aw_haptic->func == aw8692x_func() &&
				...
				aw_haptic->i2c_info.reg_data[0] =
						AW8692X_BIT_PLAYCFG1_BST_VOUT_6V;
			else if (aw_haptic->func == aw8692x_func() &&
				reg_addr < AW8692X_REG_PLAYCFG1 &&
...
static ssize_t set_amplitude_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	...
	if (aw_haptic->info.is_enabled_lower_amp) {
		if (val == ADJUST_GAIN_CAM_RESONANCES)
			aw_haptic->set_amplitude_flag = 1;
		else
			aw_haptic->set_amplitude_flag = 0;
	}
	return count;
...
struct i2c_driver aw_i2c_driver = {
	.driver = {
			.name = AW_I2C_NAME,
			.owner = THIS_MODULE,
