				     ...
				     void *data, __u32 len)
{
	...
	if (conn_impl->type == CONNECT_TYPE_TCP) {
		...
		body->d2dp_version = d2d_get_version();
...
static struct connection_msg_head *
hmdfs_init_msg_head(__u8 ops, __le16 request_id, __u32 send_len,
		    __u64 local_iid)
{
...
static inline struct connection_handshake_req *
msg_get_data_from_head(struct connection_msg_head *head)
{
...
void peer_online(struct hmdfs_peer *peer)
{
	...
	wake_up_interruptible_all(&peer->establish_loopback_connection_wq);
...
int hmdfs_disconnect_node(struct hmdfs_peer *node)
{
	int ret = 1;
	...
	ret = wait_event_interruptible_timeout(node->deleting_list_wq,
				hmdfs_conn_deleting_list_empty(node),
				HMDFS_WAIT_CONN_RELEASE);
	if (ret <= 0) {
		hmdfs_dump_deleting_list(node);
		hmdfs_err("timeout or signal interrupt ret = %d", ret);
	}
...
void hmdfs_connections_stop(struct hmdfs_sb_info *sbi)
{
	...
	list_for_each_entry_safe(node, con_tmp, &sbi->connections.node_list,
		...
		/*
		 * Keep peer when wait_event interrupted by signal or timeout
		 * make sure peer is valid in connection_release
		 * avoid use-after-free to trigger panic
		 */
		if (hmdfs_disconnect_node(node) <= 0)
			peer_get(node);
		hmdfs_del_peer(node);
		mutex_lock(&sbi->connections.node_lock);
	}
...
retry:
	...
	if (!candidate && is_loopback_device(node)) {
		time_left = wait_event_interruptible_timeout(
			node->establish_loopback_connection_wq,
			(node->status == NODE_STAT_ONLINE &&
			!list_empty(&node->conn_impl_list)),
			node->loopback_connection_establish_timeout * HZ);

		if (time_left) {
			hmdfs_info("loopbacknetwork is successfully established");
			goto retry;
		}

		if (time_left == -ERESTARTSYS || time_left == 0)
			hmdfs_err("waiting for loopbacknetwork timeout");
	}
...
static void init_peer_for_loopback(struct hmdfs_peer *node)
{
	node->loopback_connection_establish_timeout = TIMEOUT_1S;
	init_waitqueue_head(&node->establish_loopback_connection_wq);
}
