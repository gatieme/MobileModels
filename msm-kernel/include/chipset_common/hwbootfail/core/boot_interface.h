...
enum shut_stage {
	SHUT_STAGE_INVALID = 0,
	SHUT_STAGE_FRAMEWORK_START,
	SHUT_STAGE_FRAMEWORK_FINISH,
	SHUT_STAGE_NATIVE_START,
	SHUT_STAGE_NATIVE_FINISH,
	SHUT_STAGE_KERNEL,
	SHUT_STAGE_SUCC,
};

/*
 * Total size of this struct: 16B.
 * You're strongly recommended to dynamically allocate memory for the struct.
 */
struct shutfail_basic_info {
	unsigned long long rtc_time;
	int stage;  /* stage saved by shutdetector */
};

/*
 * Total size of this struct: 512B.
 * You're strongly recommended to dynamically allocate memory for the struct.
 */
struct shutfail_proc_param {
	struct shutfail_basic_info sinfo;
	char log_path[3][128]; /* max count and size of upper layer logs */
	char reserved[116]; /* reserved count */
} __packed;
 ...
 * You're strongly recommended to dynamically allocate memory for the struct.
 */
struct shutfail_log_header {
	struct shutfail_basic_info sinfo;
	char log_path[3][128]; /* max count and size of upper layer logs */
	int kernel_log_size;
	char reserved[112]; /* reserved count */
} __packed;
#endif
