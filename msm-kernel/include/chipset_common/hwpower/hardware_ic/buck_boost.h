...
enum buck_boost_vout_set_by_gpio {
	BBST_VOUT_SET_BY_GPIO_EN = 0,
	BBST_VOUT_SET_GPIO_NUM,
	BBST_VOUT_SET_VOL_TH,
	BBST_VOUT_SET_GPIO_VAL, /* 0: vout high voltage when gpio output high, 1: vout low voltage when gpio output high */
...
struct buck_boost_dev {
	...
	int bbst_gpio_en;
	int boost_use_gpio;
	unsigned int vout_vote_type;
	unsigned int vout_set_by_gpio[BBST_VOUT_SET_BY_GPIO_END];
};
