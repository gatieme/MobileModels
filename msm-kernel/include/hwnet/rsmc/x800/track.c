...
static void proc_pilot_data(struct track_msg *msg)
{
	...
	memset(&info->eh[w_idx], 0x00, sizeof(struct complex));
	memset(&info->ph[w_idx], 0x00, sizeof(struct complex));
	memset(&info->lh[w_idx], 0x00, sizeof(struct complex));
	...
	info->win_idx = w_idx;
	memset(&info->e, 0x00, sizeof(struct complex));
	memset(&info->p, 0x00, sizeof(struct complex));
	memset(&info->l, 0x00, sizeof(struct complex));
...
void set_magic_code(struct rx_init_msg *msg)
{
	if (msg == NULL)
		return;
	memcpy(magic_code, msg->magic_code, FRM_PERIOD);
}
