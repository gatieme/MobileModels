...
DECLARE_RESTRICTED_HOOK(android_rvh_drg_cpufreq_check_limit,
	TP_PROTO(struct cpufreq_policy *policy, unsigned int *target_freq),
	TP_ARGS(policy, target_freq), 1);

DECLARE_RESTRICTED_HOOK(android_rvh_corectl_notifier_chain,
	TP_PROTO(int is_register, struct notifier_block *nb,
		unsigned int list, int *ret),
	TP_ARGS(is_register, nb, list, ret), 1);
