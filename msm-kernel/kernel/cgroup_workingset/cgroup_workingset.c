...
#define WORKINGSET_PREREAD_TIMEOUT		(3*HZ)

static bool g_module_initialized;
static struct s_ws_collector *g_collector;
static struct delayed_work g_preread_timeout;
...
static inline int workingset_clear_pte_young_of_process(int pid)
{
	/*
	 * Because accesses to pages with PTE_AF cleared will set this bit
	 * by hardware instead of raising an access flag fault.
	 * so we have not a good resolvent for collecting the mapped file page.
	 */
	return 0;
}
#else
	...
	if (target_state == E_CGROUP_STATE_MONITOR_PREREAD ||
		...
		if (record)  {
			if ((target_state == E_CGROUP_STATE_MONITOR_ABORT) ||
				...
				cancel_delayed_work_sync(&g_preread_timeout);
			}
			...
			if ((target_state == E_CGROUP_STATE_MONITOR_PREREAD) &&
				...
				record->state |= E_RECORD_STATE_PREREADING;
				schedule_delayed_work(&g_preread_timeout,
				    WORKINGSET_PREREAD_TIMEOUT);
...
static ssize_t workingset_clear_young_write(
	struct kernfs_open_file *of, char *buf, size_t nbytes, loff_t off)
{
	int ret;
	int pid;
	...
	ret = workingset_clear_pte_young_of_process(pid);
	if (ret) {
		return ret;
	} else {
		ws->clear_young = true;
...
static struct cftype files[] = {
	...
	},
	{
		.name = "clearYoung",
		.flags = CFTYPE_ONLY_ON_ROOT,
		.write = workingset_clear_young_write,
	},
	{ } /* Terminate */
};
	...
	unsigned int count, bool is_pagefault)
{
	...
	if (!is_pagefault && (g_collector->monitor->state ==
...
static void workingset_preread_timeout(struct work_struct *work)
{
	struct s_workingset *ws = NULL;
	...
	mutex_lock(&ws->mutex);
	if (ws->state == E_CGROUP_STATE_MONITOR_INWORKING ||
	    ws->state == E_CGROUP_STATE_MONITOR_PAUSED) {
		workingset_preread_force_stop();
		ws_dbg("%s\n", __func__);
	}
	mutex_unlock(&ws->mutex);
...
static int __init cgroup_workingset_init(void)
{
	...
	INIT_DEFERRABLE_WORK(&g_preread_timeout, workingset_preread_timeout);
