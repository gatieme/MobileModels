...
EXPORT_SYMBOL_GPL(sysctl_sched_rt_runtime);
...
void set_graded_load_weight(struct task_struct *p, bool update_load)
{
	int prio = p->graded_prio - MAX_RT_PRIO;
...
EXPORT_SYMBOL(set_graded_load_weight);
#endif
...
EXPORT_SYMBOL_GPL(__setscheduler_uclamp);
...
#ifdef CONFIG_HW_GRADED_SCHED
static inline int normal_graded_prio(struct task_struct *p)
{
	return __normal_prio(p->policy, p->rt_priority, PRIO_TO_NICE(p->graded_prio));
}

static int effective_graded_prio(struct task_struct *p)
{
	p->normal_prio = normal_graded_prio(p);
	/*
	 * If we are RT tasks or we were boosted to RT priority,
	 * keep the priority unchanged. Otherwise, update priority
	 * to the normal priority:
					 ...
					 struct rq_flags *rf)
{
	...
	if (dest_cpu >= nr_cpu_ids) {
		pr_err("[==affinity_check_invalid==] task=%d(%s) vip=%d, status=%d/%d/%d, src_cpu=%d, dest_cpu=%d, affinity=%*pbl/%*pbl/%*pbl, cpumask=%*pbl/%*pbl\n",
			p->pid, p->comm, p->vip_prio,
			task_running(rq, p), p->state, p->on_rq,
			task_cpu(p), dest_cpu,
			cpumask_pr_args(p->cpus_ptr),
			cpumask_pr_args(new_mask),
			cpumask_pr_args(&allowed_mask),
	...
	if (task_running(rq, p) || p->state == TASK_WAKING) {
		...
		pr_info("[==affinity_check_migrate==] task=%d(%s) vip=%d, status=%d/%d/%d, src_cpu=%d, dest_cpu=%d, affinity=%*pbl/%*pbl/%*pbl, cpumask=%*pbl/%*pbl\n",
			p->pid, p->comm, p->vip_prio,
			task_running(rq, p), p->state, p->on_rq,
			task_cpu(p), dest_cpu,
			cpumask_pr_args(p->cpus_ptr),
...
try_to_wake_up(struct task_struct *p, unsigned int state, int wake_flags)
{
	...
	trace_android_rvh_add_waker_to_render_rthread(p);
...
static void __sched_fork(unsigned long clone_flags, struct task_struct *p)
{
	...
	trace_android_rvh_sched_stat_yield_init(p);
...
int sched_fork(unsigned long clone_flags, struct task_struct *p)
{
	...
	trace_android_rvh_init_new_task_load(p);
	...
	if (unlikely(p->sched_reset_on_fork)) {
		if (task_has_dl_policy(p) || task_has_rt_policy(p)) {
			...
			trace_android_rvh_hwrtg_fork_prio(current, p);
			p->rt_priority = 0;
...
void wake_up_new_task(struct task_struct *p)
{
	...
	trace_android_rvh_add_render_rthread(p);
...
#ifdef CONFIG_DFX_RAINBOW_TRACE
	trace_android_rb_trace_task_write((void *)next);
#endif
...
void scheduler_tick(void)
{
	...
	trace_android_rvh_hwrtg_task_load(curr, &old_load);
	...
	trace_android_vh_sched_update_rtg_tick(rq, &old_load);
	...
	trace_android_vh_check_for_rt_migration(rq, curr);
	trace_android_vh_scheduler_tick(rq);

	trace_android_vh_corectl_check(cpu, rq);
...
static void __sched notrace __schedule(bool preempt)
{
	...
	if (likely(prev != next)) {
...
#ifdef CONFIG_RT_SYNC_OPT
		/*
		 * Wake_sync==true means prev has TIF_WAKE_SYNC, that is:
		 * Prev has called SYNC wakeup from its last scheduling and
		 * the wakee has been queued on this rq (and also the wakee
		 * was woken from sleeping/dequeued state).
		 * And if prev didn't sleep, then it's a sync waiter, who is
		 * runnable but has nothing more to do. Increase the count.
		 ...
		 * have more runnable tasks than rt_sync_waiting count.
		 * We should better keep the sync waiters runnable to save
		 * unnecessary scheduling cost.
		 */
		if (wake_sync && prev->on_rq) {
			rq->rt_sync_waiting++;
		} else if (rq->rt_sync_waiting > 0) {
			/*
			 * We didn't guarantee the scheduling to be what we
			 * want. Break it out in some cases.
			 * e.g. someone else preempt us or no tasks left.
			 */
			if (prev->on_rq || next == rq->idle)
				rq->rt_sync_waiting = 0;
			else
				rq->rt_sync_waiting--;
		}
#endif
...
#ifdef CONFIG_HW_GRADED_SCHED
void set_graded_nice(struct task_struct *p, long nice)
{
	bool queued, running;
	int old_prio;
	struct rq_flags rf;
	...
	p->graded_prio = NICE_TO_PRIO(nice);
	set_graded_load_weight(p, true);
	old_prio = p->prio;
	p->prio = effective_graded_prio(p);
...
#ifdef CONFIG_SCHED_LATENCY_NICE
	trace_sched_setscheduler(p, oldpolicy, oldprio, oldlatency, attr->sched_flags, user, pi);
#endif
...
static void do_sched_yield(void)
{
	...
	trace_android_rvh_sched_stat_yield_update(NULL);
...
void sched_show_task(struct task_struct *p)
{
	...
	rcu_read_unlock();
	pr_cont(" stack:%5lu pid:%5d tgid:%5d ppid:%6d flags:0x%08lx state:%d\n",
