...
#if defined(CONFIG_HW_EAS_SCHED) || defined(CONFIG_HW_MULTI_MARGIN_RT)
...
hw_capacity_margin_general(struct task_struct *p, int cpu)
{
	...
	if (p->sched_class == &rt_sched_class)
		return hw_capacity_margin_rt(cpu);
#endif
...
#if defined(CONFIG_HW_EAS_SCHED) || defined (CONFIG_HW_EAS_SYSFS)
#ifdef CONFIG_HW_SCHED_PRED_LOAD
...
#ifdef CONFIG_HW_MULTI_MARGIN_RT
	eas_attr_add("capacity_margin_rt",
		NULL,
		capacity_margin_rt_show,
		capacity_margin_rt_store,
		0640);
#endif
...
#endif /* CONFIG_HW_EAS_SCHED || CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE */
