...
#define num_clusters num_sched_clusters
#else
#include <linux/topology_hw.h>
#endif
#if defined(CONFIG_HW_MULTI_MARGIN_RT) || defined(CONFIG_HW_MULTI_MARGIN)
#include "multi_margin_hw.h"

#define capacity_margin_show_func(data)						\
ssize_t capacity_##data##_show(struct kobject *kobj,				\
...
#define capacity_margin_store_func(data)						\
ssize_t capacity_##data##_store(struct kobject *kobj, struct kobj_attribute *kattr,	\
...
{											\
	...
	i = 0;										\
	for_each_sched_cluster(cluster) {						\
		cluster->capacity_##data = new_margin[i++];				\
	}										\
											\
	kfree(new_margin);								\
	return count;									\
...
capacity_margin_show_func(margin)
capacity_margin_store_func(margin)
...
capacity_margin_store_func(margin_rt)
capacity_margin_show_func(margin_rt)
#endif
