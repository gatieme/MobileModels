...
static int _init_aux_info(struct aux_info *aux_info)
{
	struct related_thread_group *grp = NULL;
	unsigned long flags;

	(void)memset(aux_info, 0, sizeof(struct aux_info));
...
int __init init_aux_info(void)
{
	int ret = 0;

	ret = _init_aux_info(&g_aux_info);
