	...
	if (is_rt_task && (atomic_read(&frame_info->curr_rt_thread_num) <
		...
		if (!is_rtg_rt_task(task)) {
			if (test_and_read_rtg_rt_thread_num() >= RTG_MAX_RT_THREAD_NUM) {
				pr_err("[AWARE_RTG] change %s, pid=%d from CFS to RT fail, too much rtg_rt_thread, num=%d\n",
					task ? task->comm : NULL, task ? task->pid : -1, test_and_read_rtg_rt_thread_num());
				goto out;
			}
			need_inc_flag = true;
		}
...
#elif IS_ENABLED(CONFIG_HW_RTG_FRAME_USE_NORMALIZED_UTIL)
	...
	if (pid > 0) {
		if (old_task && (pid == old_task->pid) && (old_prio == new_prio)) {
			if (is_rt_task && atomic_read(&frame_info->curr_rt_thread_num) <
				...
				pr_info("[AWARE_RTG] update rt_thread fail, pid = %d, old_task = %d, current_rt_thread_nums = %d\n",
					pid, old_task->pid, atomic_read(&frame_info->curr_rt_thread_num));
			trace_rtg_frame_sched(frame_info->rtg->id, "curr_rt_thread_num",
					      atomic_read(&frame_info->curr_rt_thread_num));
			return old_task;
		}
	...
	if (atomic_read(&frame_info->frame_sched_state) == 1) {
		if (task && is_rt_task) {
			if (atomic_read(&frame_info->curr_rt_thread_num) <
				...
				new_prio = NOT_RT_PRIO;
				pr_info("[AWARE_RTG] update rt_thread fail, pid = %d, old_task = %d, current_rt_thread_nums = %d\n",
					pid, old_task ? old_task->pid : -1, atomic_read(&frame_info->curr_rt_thread_num));
			}
		}
		trace_rtg_frame_sched(frame_info->rtg->id, "curr_rt_thread_num",
				      atomic_read(&frame_info->curr_rt_thread_num));
...
#if IS_ENABLED(CONFIG_HW_RTG_FRAME_USE_NORMALIZED_UTIL)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
int __init init_frame_info(void)
#else
