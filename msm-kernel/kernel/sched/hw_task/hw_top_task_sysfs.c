				      ...
				      const char *buf, size_t count)
{
	...
	list_for_each_entry(sg_policy, &attr_set->policy_list, tunables_hook) {
		for_each_cpu(cpu, sg_policy->policy->cpus) {
			...
			trace_android_vh_set_top_task_hist_size(cpu_rq(cpu), val);
#else
			walt_rq_set(cpu_rq(cpu), top_task_hist_size, val);
#endif
		}
	}
				      ...
				      const char *buf, size_t count)
{
	...
	list_for_each_entry(sg_policy, &attr_set->policy_list, tunables_hook) {
		for_each_cpu(cpu, sg_policy->policy->cpus) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
			trace_android_vh_set_top_task_stats_policy(cpu_rq(cpu), val);
#else
			walt_rq_set(cpu_rq(cpu), top_task_stats_policy, val);
#endif
		}
	}
				      ...
				      const char *buf, size_t count)
{
	...
	list_for_each_entry(sg_policy, &attr_set->policy_list, tunables_hook) {
		for_each_cpu(cpu, sg_policy->policy->cpus) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
			trace_android_vh_set_top_task_stats_empty_window(cpu_rq(cpu), val);
#else
			walt_rq_set(cpu_rq(cpu), top_task_stats_empty_window, val);
#endif
		}
	}
