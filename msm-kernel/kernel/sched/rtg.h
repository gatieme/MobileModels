...
TRACE_EVENT(cgroup_rtg_debug,

	TP_PROTO(int id, const char *s, u64 value),
	...
	TP_printk("%s_%d state=%llu cpu_id=0", __get_str(str), __entry->id, __entry->value)
);
#endif
...
DECLARE_EVENT_CLASS(sched_rtg_template,
	...
	TP_STRUCT__entry(
		...
		__field(int, prio)
#if IS_ENABLED(CONFIG_HUAWEI_VIP_COMMON)
