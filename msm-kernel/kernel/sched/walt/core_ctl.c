...
EXPORT_SYMBOL(cpus_paused_by_us);
#else
				...
				const char *buf, size_t count)
{
	...
	pr_info("ccprinfoSetMin:%*pbl %d  %*pbl\n",
		cpumask_pr_args(&state->cpu_mask), val, cpumask_pr_args(cpu_active_mask));
				...
				const char *buf, size_t count)
{
	...
	pr_info("ccprinfoSetMax:%*pbl %d  %*pbl\n",
		cpumask_pr_args(&state->cpu_mask), val, cpumask_pr_args(cpu_active_mask));
...
#if IS_ENABLED(CONFIG_BIG_CLUSTER_CORE_CTRL)
...
int core_ctl_set_boost(bool boost)
{
	...
	pr_info("ccprinfoSetSt: %d  %*pbl\n",
		boost, cpumask_pr_args(cpu_active_mask));
...
#if IS_ENABLED(CONFIG_DRG)
void isolate_unisolate_call_notifier(unsigned int flag)
{
...
void corectl_check(void *unused, int cpu, struct rq *rq)
{
	...
	trace_core_ctl_check(cpu, wrq->window_start);
}
#endif
...
void corectl_notifier_chain(void *unused, int is_register,
	struct notifier_block *nb, unsigned int list, int *ret)
{
	if (list == CPUFREQ_GOVINFO_NOTIFIER) {
		if (is_register)
			*ret = atomic_notifier_chain_register(&cpufreq_govinfo_notifier_list, nb);
		else
			*ret = atomic_notifier_chain_unregister(&cpufreq_govinfo_notifier_list, nb);
	} else {
		*ret = -EINVAL;
	}
}
...
#if IS_ENABLED(CONFIG_BIG_CLUSTER_CORE_CTRL)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	register_trace_android_rvh_corectl_notifier_chain(
		corectl_notifier_chain, NULL);
	register_trace_android_vh_corectl_check(corectl_check, NULL);
#endif
