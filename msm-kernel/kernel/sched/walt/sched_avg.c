				...
				unsigned long prev_nr_run, u64 curr_time)
{
	...
	if (dequeue && (walt_cpu_util(cpu) * BUSY_LOAD_FACTOR) >
			capacity_orig_of(cpu))
		load_trigger = true;

	if (dequeue && walt_cpu_util(cpu) > per_cpu(coloc_hyst_busy, cpu))
		coloc_load_trigger = true;

	if (dequeue) {
		for_each_possible_cpu(i) {
			total_util += walt_cpu_util(i);
	...
	if (agg_hyst_time) {
		...
		trace_sched_busy_hyst_time(cpu, agg_hyst_time, prev_nr_run,
					walt_cpu_util(cpu), per_cpu(hyst_time, cpu),
