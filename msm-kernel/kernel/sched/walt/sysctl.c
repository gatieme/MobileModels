...
unsigned int sysctl_sched_walt_init_task_load_pct;
unsigned int sysctl_sched_walt_cpu_overload_irqload;

int sysctl_sched_walt_init_task_load_pct_sysctl_handler(
				...
				loff_t *ppos)
{
	...
	case GROUP_ID:
#if IS_ENABLED(CONFIG_HW_RTG)
		ret = _sched_set_group_id(task, val);
#else
...
#ifdef CONFIG_HW_SCHED_WALT
	sysctl_sched_use_walt_cpu_util = 1;

	sysctl_sched_use_walt_task_util = 1;

	sysctl_sched_walt_init_task_load_pct = sysctl_sched_init_task_load_pct;

	sysctl_sched_walt_cpu_overload_irqload = (3 * NSEC_PER_SEC / CONFIG_HZ);
#endif
}
