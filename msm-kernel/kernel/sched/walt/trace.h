...
#ifdef CONFIG_HW_WALT_LB_OPT
		__entry->task_util		= walt_task_util(cpu_rq(cpu)->curr);
#endif
		__entry->cpu_util		= walt_cpu_util(cpu);
		__entry->capacity_orig		= capacity_orig_of(cpu);
	),

#ifdef CONFIG_HW_WALT_LB_OPT
	TP_printk("cpu=%d nr_running=%u cfs_nr_running=%u nr_big=%u nr_rtg_hp=%u task_util=%u cpu_util=%u capacity_orig=%u",
		__entry->cpu, __entry->nr_running, __entry->cfs_nr_running,
		__entry->nr_big, __entry->nr_rtg_high_prio_tasks,
		__entry->task_util, __entry->cpu_util, __entry->capacity_orig)
#else
...
TRACE_EVENT(core_ctl_check,

	TP_PROTO(int cpu, u64 window_start),

	TP_ARGS(cpu, window_start),
	...
	TP_printk("cpu=%d, window_start=%lu",
		__entry->cpu, __entry->window_start)
);

#include <trace/events/pred_load.h>

#ifdef CONFIG_HW_TOP_TASK
#include <trace/events/hw_top_task.h>
#endif

#if IS_ENABLED(CONFIG_HW_RTG)
#if IS_ENABLED(CONFIG_HW_RTG_FRAME)
#include "../hw_rtg/include/frame.h"
#include "../hw_rtg/include/frame_info.h"
#endif
#include "../rtg.h"
#endif
...
TRACE_EVENT(rtg_frame_sched,
	...
	TP_fast_assign(
		__assign_str(str, s);
#if IS_ENABLED(CONFIG_HW_RTG)
		__entry->rtgid = rtgid != -1 ?
			rtgid : (((struct walt_task_struct *) current->android_vendor_data1)->grp ?
				((struct walt_task_struct *) current->android_vendor_data1)->grp->id : 0);
#else
		__entry->rtgid = rtgid != -1 ? rtgid : (current->wts->grp ? current->wts->grp->id : 0);
#endif
