...
#include "walt.h"
#include "trace.h"
#if IS_ENABLED(CONFIG_HUAWEI_PERF_CTRL)
...
#ifdef CONFIG_HW_MULTI_MARGIN_RT
#define DEFAULT_RT_MARGIN 1024
#endif
...
#ifndef CONFIG_HW_SCHED_WALT
static unsigned int walt_cpu_high_irqload;
#else
static unsigned int sched_walt_cpu_high_irqload;
#endif
...
void walt_task_dump(struct task_struct *p)
{
	...
	SCHED_PRINT(predl_window_size);
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_SYNC
	SCHED_PRINT(new_predl_window_size);
#endif
#endif
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
	trace_cgroup_rtg_debug(0, "sched_freq_aggr_en", sched_freq_aggr_en);
#endif
...
migrate_top_tasks(struct task_struct *p, struct rq *src_rq, struct rq *dst_rq)
{
	...
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
#ifdef CONFIG_HW_TOP_TASK
	u32 curr_window = wts->curr_load;
	u32 prev_window = wts->prev_load;
#else
...
static void fixup_busy_time(struct task_struct *p, int new_cpu)
{
	...
	if (!same_freq_domain(new_cpu, task_cpu(p))) {
		...
		mark_util_change_for_migration(p, src_rq, dest_rq);
#endif
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE
		wrq->predl_window_start = sync_wrq->window_start;
#endif
				...
				int start, u32 runtime)
{
	...
	u32 ret = runtime;
#if defined(CONFIG_HW_SCHED_PRED_LOAD) && defined(CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON)
	bool predl_size_tunnable = false;
...
#if defined(CONFIG_HW_SCHED_PRED_LOAD) && defined(CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON)
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE
	if (predl_window_size != sched_ravg_window)
		predl_size_tunnable = true;
#endif
	if (!predl_size_tunnable && ((u64)ret << SCHED_CAPACITY_SHIFT) >
	    ((u64)capacity_curr_of(cpu_of(rq)) * (wrq->prev_window_size)))
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE
	if (predl_window_size != sched_ravg_window &&  wts->predl > new)
		new = wts->predl;
#endif
...
#ifdef CONFIG_HW_TOP_TASK
	wts->curr_load = wts->load_avg;
	curr_window = wts->curr_load;
#endif
...
#if defined(CONFIG_HW_TOP_TASK) && defined(CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL)
	sugov_mark_top_task(rq, max(curr_window, prev_window));
#endif
}
				 ...
				 int event, u64 wallclock, u64 irqtime)
{
	...
	bool new_task;
#if IS_ENABLED(CONFIG_HW_RTG_CPU_TIME)
	struct group_cpu_time *cpu_time;
#endif
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
	struct related_thread_group *grp;
#elif IS_ENABLED(CONFIG_QCOM_WALT_RTG)
	struct walt_related_thread_group *grp;
#endif
	int cpu = rq->cpu;
#ifdef CONFIG_HW_TOP_TASK
	int nr_full_window = 0;
	u32 old_curr_window = wts->curr_load;
#else
...
#ifdef CONFIG_HW_TOP_TASK
		nr_full_window = div64_u64((window_start - mark_start), sched_ravg_window);
#endif
	}
	...
	if (new_window && !is_idle_task(p) && !exiting_task(p))
		rollover_top_task_load(p, nr_full_window);
#endif
	...
	if (!account_busy_for_cpu_time(rq, p, irqtime, event)) {
		...
		if (!new_window)
			/*
			 * account_busy_for_cpu_time() = 0 so busy time no
			 * need to be accounted to the current window. No
			 * rollover since we didn't start a new window.
			 * Therefore, no changes for curr_table or prev_table
			 */
			return;
...
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG) || IS_ENABLED(CONFIG_HW_CGROUP_RTG)
	grp = wts->grp;
	if (grp) {
...
#if IS_ENABLED(CONFIG_HW_TASK_RAVG_SUM)
				...
				u64 delta, int event)
{
	...
	delta = scale_exec_time(delta, rq);
	wts->android_vendor_data1 += delta;
}
#else
			       ...
			       int event, u64 wallclock)
{
	...
	if (!new_window) {
		 ...
		 * window.
		 */
		ret = add_to_task_demand(rq, p, wallclock - mark_start);
						...
						u64 wallclock, u64 irqtime)
{
	...
	if (wts->mark_start < wrq->window_start)
		trace_walt_window_rollover(cpu_of(rq));
#endif
...
static void init_new_task_load(struct task_struct *p)
{
	...
	wts->predl_sum = 0;
	for (i = 0; i < RAVG_HIST_SIZE_MAX; ++i)
		wts->predl_sum_history[i] = init_load_windows;
	for (i = 0; i < NUM_BUSY_BUCKETS; ++i)
		wts->predl_busy_buckets[i] = 0;
	memset(wts->predl_prev_window_cpu, 0, sizeof(wts->predl_prev_window_cpu));
	memset(wts->predl_curr_window_cpu, 0, sizeof(wts->predl_curr_window_cpu));
#endif
}
...
static struct walt_sched_cluster init_cluster = {
	...
	.aggr_grp_load		= 0,
#ifdef CONFIG_HW_MULTI_MARGIN_RT
	.capacity_margin_rt     = DEFAULT_RT_MARGIN,
#endif
};
...
#ifdef CONFIG_HW_MULTI_MARGIN_RT
	cluster->capacity_margin_rt	=	DEFAULT_RT_MARGIN;
#endif
...
EXPORT_SYMBOL(walt_clusters_parsed);
#else
...
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG)
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
bool uclamp_task_colocated(struct task_struct *p)
#else
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
EXPORT_SYMBOL(uclamp_task_colocated);
#endif
...
#else
		grp_id = colocate ? DEFAULT_CGROUP_COLOC_ID : 0;
...
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG) || IS_ENABLED(CONFIG_HW_CGROUP_RTG)
	ret = is_min_capacity_cluster(cluster) ==
		     (!grp->skip_min && (boost_policy != SCHED_BOOST_ON_BIG));
#else
	ret = false;
#endif
...
static void transfer_busy_time(struct rq *rq,
				struct walt_related_thread_group *grp,
					struct task_struct *p, int event)
#else
void transfer_busy_time(struct rq *rq,
			struct related_thread_group *grp,
			struct task_struct *p, int event)
#endif
{
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
EXPORT_SYMBOL(transfer_busy_time);
#endif
#endif
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
				curr_grp_load = wrq->grp_time.prev_runnable_sum;
				max_grp_load = max_grp_load > curr_grp_load ?
					       max_grp_load : curr_grp_load;
				aggr_grp_load += curr_grp_load;
#else
				aggr_grp_load +=
					wrq->grp_time.prev_runnable_sum;
#endif
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
		cluster->aggr_grp_load = max_grp_load;
		trace_cgroup_rtg_debug(id, "aggr_grp_load", max_grp_load);
#else
...
#if IS_ENABLED(CONFIG_HW_CGROUP_RTG)
	trace_cgroup_rtg_debug(0, "aggr_grp_load_total", total_grp_load);
#endif
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_SYNC
		predl_window_size_change_handle(wc);
#endif
	}
...
void walt_fill_ta_data(struct core_ctl_notif_data *data)
{
	...
	struct walt_related_thread_group *grp;
#endif
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG) || IS_ENABLED(CONFIG_HW_RTG)
	unsigned long flags;
...
#else
	sched_walt_cpu_high_irqload = div64_u64((u64)sched_ravg_window * 95, (u64) 100);
#endif
}
...
#ifdef CONFIG_HW_SCHED_PRED_LOAD_WINDOW_SIZE_TUNNABLE
	wrq->sum_pred_load = 0;
	wrq->curr_max_predls = 0;
	wrq->prev_max_predls = 0;
#endif
#ifdef CONFIG_HW_TOP_TASK
	wrq->top_task_hist_size = DEFAULT_TOP_TASK_HIST_SIZE;
	wrq->top_task_stats_policy = DEFAULT_TOP_TASK_STATS_POLICY;
	wrq->top_task_stats_empty_window = DEFAULT_TOP_TASK_STATS_EMPTY_WINDOW;
#endif
...
static void android_rvh_update_cpu_capacity(void *unused, int cpu, unsigned long *capacity)
{
	...
	*capacity = rq->cpu_capacity_orig > rt_pressure ?
			(rq->cpu_capacity_orig - rt_pressure) : 1UL;
}
		...
		struct rq *rq, bool *need_update)
{
	...
	*need_update = false;
	...
	if (rtg_target)
		...
		task_fits = task_fits_max(p, rq->cpu);
...
#ifdef CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON
		sugov_mark_util_change(cpu_of(rq), FORCE_UPDATE);
#else
...
static void android_vh_cpu_util_pred_min(void *unused, struct rq *rq, unsigned long *util)
{
	if (unlikely(walt_disabled))
		return;
#ifdef CONFIG_HW_SCHED_PRED_LOAD
	*util = cpu_util_pred_min(rq);
#else
	*util = 0;
#endif
}

static void android_rvh_get_cluster_max_freq(void *unused, int cpu, unsigned int *freq)
{
	if (unlikely(walt_disabled))
		return;
	*freq = cpu_cluster(cpu)->max_freq;
}

static void android_vh_get_cpu_util(void *unused, int cpu, unsigned long *util)
{
	struct walt_cpu_load walt_load = {0};

	if (unlikely(walt_disabled))
		return;
	*util = cpu_util_freq_walt(cpu, &walt_load);
}

static void android_rvh_get_wrq_window_start(void *unused, struct rq *rq, u64 *window_start)
{
	struct walt_rq *wrq = (struct walt_rq *) rq->android_vendor_data1;

	if (unlikely(walt_disabled))
		return;
	*window_start = wrq->window_start;
}

static void android_rvh_get_wts_mark_start(void *unused, struct rq *rq, u64 *mark_start)
{
	struct walt_task_struct *wts = (struct walt_task_struct *) rq->curr->android_vendor_data1;

	if (unlikely(walt_disabled))
		return;
	*mark_start = wts->mark_start;
}

static void android_rvh_get_wts_last_enqueued_ts(void *unused, struct task_struct *p, u64 *time)
{
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
	if (unlikely(walt_disabled))
		return;
	*time = wts->last_enqueued_ts;
}

static void android_vh_max_pred_ls(void *unused, struct rq *rq, unsigned long *util)
{
	if (unlikely(walt_disabled))
		return;
#ifdef CONFIG_HW_SCHED_PRED_LOAD
...
static void android_vh_top_task_util(void *unused, struct rq *rq, unsigned long *util)
{
	if (unlikely(walt_disabled))
		return;
#ifdef CONFIG_HW_TOP_TASK
	*util = top_task_util(rq);
#else
	*util = 0;
#endif
}

static void android_vh_predict_util(void *unused, struct rq *rq, unsigned long *util)
{
	if (unlikely(walt_disabled))
		return;
#ifdef CONFIG_HW_SCHED_PRED_LOAD
	*util = predict_util(rq);
#else
	*util = 0;
#endif
}

static void android_rvh_walt_ktime_clock(void *unused, u64 *wallclock)
{
	if (unlikely(walt_disabled))
		return;
	*wallclock = walt_ktime_get_ns();
...
static void android_rvh_walt_update_task_ravg(void *unused, struct task_struct *p, struct rq *rq, int event, u64 wallclock, u64 irqtime)
{
	if (unlikely(walt_disabled))
		return;
	walt_update_task_ravg(p, rq, event, wallclock, irqtime);
}

static void android_rvh_get_top_task_skip_hispeed(void *unused, bool *defined)
{
...
static void android_vh_use_pelt_freq(void *unused, bool *flag)
{
	*flag = false;
}

static void android_rvh_find_rtg_target(void *unused, struct task_struct *p, struct cpumask **rtg_target)
{
#if IS_ENABLED(CONFIG_HW_RTG)
        *rtg_target = find_rtg_target(p);
#endif
...
static void android_rvh_walt_cpu_high_irqload(void *unused, int num, unsigned long *util)
{
        *util = walt_cpu_high_irqload(num);
}

static void android_rvh_schedtune_prefer_idle(void *unused, struct task_struct *p, unsigned long *util)
{
#if IS_ENABLED(CONFIG_HW_RTG) && IS_ENABLED(CONFIG_UCLAMP_TASK) && !IS_ENABLED(CONFIG_SCHED_TUNE)
        *util = schedtune_prefer_idle(p);
#endif
}

static void android_rvh_is_reserved(void *unused, int cpu, unsigned long *util)
{
        if (unlikely(walt_disabled))
                return;
        *util = is_reserved(cpu);
}

static void android_rvh_cpu_isolated(void *unused, int cpu, int *util)
{
#if IS_ENABLED(CONFIG_HW_RTG)
        *util = cpu_paused(cpu);
#else
...
static void android_rvh_capacity_curr_of(void *unused, int cpu, unsigned long *util)
{
        if (unlikely(walt_disabled))
                return;
        *util = capacity_curr_of(cpu);
}

static void android_rvh_is_min_capacity_cpu(void *unused, int cpu, unsigned long *util)
{
        if (unlikely(walt_disabled))
                return;
        *util = is_min_capacity_cpu(cpu);
}

static void android_rvh_unisolated_cpu(void *unused, struct rq *rq, int *util)
{
        struct walt_sched_cluster *cluster = NULL;
        struct walt_sched_cluster *kick_cluster = NULL;
        struct walt_rq *wrq = (struct walt_rq *) rq->android_vendor_data1;
        struct walt_sched_cluster *rq_cluster = wrq->cluster;

        if (cpumask_weight(&rq_cluster->cpus) == 1) {
                /* Find the prev cluster. */
                for_each_sched_cluster(cluster) {
                        if (cluster == rq_cluster)
                                break;
                        kick_cluster = cluster;
                }
        } else {
                kick_cluster = rq_cluster;
        }
        ...
        for_each_cpu(*util, &kick_cluster->cpus) {
#if IS_ENABLED(CONFIG_HW_RTG)
                if (!cpu_online(*util) || cpu_paused(*util))
                        continue;
#else
                if (!cpu_online(*util))
                        continue;
...
static void android_rvh_cpumask_andnot(void *unused, struct cpumask *dstp,  const struct cpumask *src1p)
{
#if IS_ENABLED(CONFIG_HW_RTG)
        cpumask_andnot(dstp, src1p, cpus_paused_mask);
#endif
}
...
#ifdef CONFIG_HW_TOP_TASK
static void android_vh_set_top_task_hist_size(void *unused, struct rq *rq, unsigned int val)
{
	walt_rq_set(rq, top_task_hist_size, val);
}

static void android_vh_set_top_task_stats_policy(void *unused, struct rq *rq, unsigned int val)
{
	walt_rq_set(rq, top_task_stats_policy, val);
}

static void android_vh_set_top_task_stats_empty_window(void *unused, struct rq *rq, unsigned int val)
{
	walt_rq_set(rq, top_task_stats_empty_window, val);
}
#endif

#if IS_ENABLED(CONFIG_HW_RTG)
static void android_rvh_init_new_task_load(void *unused, struct task_struct *p)
{
	if (unlikely(walt_disabled))
		return;
	init_new_task_load(p);
...
static void android_vh_sched_update_rtg_tick(void *unused, struct rq *rq, unsigned int *old_load)
{
...
static void android_vh_hwrtg_binder_check(void *unused, struct task_struct *task, int *rtg_depth)
{
	struct walt_task_struct *wts = (struct walt_task_struct *) task->android_vendor_data1;

	*rtg_depth = wts->rtg_depth;
}

static void android_rvh_hwrtg_fork_prio(void *unused, struct task_struct *curr,
					struct task_struct *p)
{
...
static void android_rvh_hwrtg_task_load(void *unused, struct task_struct *curr, unsigned int *old_load)
{
	*old_load = task_load(curr);
}

#include <../hw_rtg/include/proc_state.h>
static void android_rvh_is_key_aux_comm(void *unused, const struct task_struct *task,
					const char *comm, bool *ret)
{
	*ret = is_key_aux_comm(task, comm);
}
#endif

#if IS_ENABLED(CONFIG_HW_RTG_FRAME)
#include <../hw_rtg/include/iaware_rtg.h>
static void android_rvh_proc_rtg_open(void *unused, struct inode *inode,
				     struct file *filp, int *ret)
{
	*ret = proc_rtg_open(inode, filp);
}

static void android_rvh_proc_rtg_compat_ioctl(void *unused, struct file *file,
					    unsigned int cmd, unsigned long arg,
					    long *ret)
{
	*ret = proc_rtg_compat_ioctl(file, cmd, arg);
}

static void android_rvh_proc_rtg_ioctl(void *unused, struct file *file,
				      unsigned int cmd, unsigned long arg,
				      long *ret)
{
	*ret = proc_rtg_ioctl(file, cmd, arg);
}

#include <../hw_rtg/include/trans_rtg.h>
static void android_rvh_trans_rtg_sched_enqueue(void *unused, struct task_struct *task,
						struct task_struct *from, unsigned int type)
{
	trans_rtg_sched_enqueue(task, from, type);
}

static void android_rvh_trans_rtg_sched_dequeue(void *unused, struct task_struct *task,
						unsigned int type)
{
...
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG) || IS_ENABLED(CONFIG_HW_RTG_DEBUG)
...
static void android_rvh_sched_group_id_open(void *unused, struct inode *inode,
					    struct file *filp, int *ret)
{
	*ret = single_open(filp, sched_group_id_show, inode);
}
static void android_rvh_sched_group_id_write(void *unused, struct file *file,
					     const char __user *buf, size_t count,
					     loff_t *offset, ssize_t *ret)
{
	*ret = sched_group_id_write(file, buf, count, offset);
}
#endif
#if IS_ENABLED(CONFIG_HW_RTG_NORMALIZED_UTIL)
#ifndef CONFIG_HW_AI_SCHED_QCOM_DDR_CONTROL
static void android_vh_get_max_group_util(void *unused,
					  const struct cpumask *query_cpus,
					  unsigned long *util, unsigned int *freq)
{
	sched_get_max_group_util(query_cpus, util, freq);
}
#else
static void android_vh_get_max_group_util(void *unused,
					  const struct cpumask *query_cpus,
...
{
	sched_get_max_group_util(query_cpus, util, freq, ff_info);
}
#endif
#endif

#if IS_ENABLED(CONFIG_HW_TASK_RAVG_SUM)
static void android_vh_ravg_sum(void *unused, struct seq_file *file,
                                struct task_struct *task)
{
	struct walt_task_struct *wts = (struct walt_task_struct *)task->android_vendor_data1;
	seq_printf(file, "%llu\n", wts->android_vendor_data1);
}
#endif
...
static void register_walt_hooks(void)
{
	...
	register_trace_android_rvh_find_rtg_target(android_rvh_find_rtg_target, NULL);
	register_trace_android_rvh_walt_cpu_high_irqload(android_rvh_walt_cpu_high_irqload, NULL);
	register_trace_android_rvh_schedtune_prefer_idle(android_rvh_schedtune_prefer_idle, NULL);
	register_trace_android_rvh_is_reserved(android_rvh_is_reserved, NULL);
	register_trace_android_rvh_cpu_isolated(android_rvh_cpu_isolated, NULL);
	register_trace_android_rvh_cpumask_andnot(android_rvh_cpumask_andnot, NULL);
	register_trace_android_rvh_capacity_curr_of(android_rvh_capacity_curr_of, NULL);
	register_trace_android_rvh_is_min_capacity_cpu(android_rvh_is_min_capacity_cpu, NULL);
	register_trace_android_rvh_unisolated_cpu(android_rvh_unisolated_cpu, NULL);
	register_trace_android_vh_dump_throttled_rt_tasks(dump_throttled_rt_tasks, NULL);
#ifdef CONFIG_HW_TOP_TASK
	register_trace_android_vh_set_top_task_hist_size(android_vh_set_top_task_hist_size, NULL);
	register_trace_android_vh_set_top_task_stats_policy(android_vh_set_top_task_stats_policy, NULL);
	register_trace_android_vh_set_top_task_stats_empty_window(android_vh_set_top_task_stats_empty_window, NULL);
#endif
	register_trace_android_vh_cpu_util_pred_min(android_vh_cpu_util_pred_min, NULL);
	register_trace_android_rvh_get_cluster_max_freq(android_rvh_get_cluster_max_freq, NULL);
	register_trace_android_vh_get_cpu_util(android_vh_get_cpu_util, NULL);
	register_trace_android_rvh_get_wrq_window_start(android_rvh_get_wrq_window_start, NULL);
	register_trace_android_rvh_get_wts_mark_start(android_rvh_get_wts_mark_start, NULL);
	register_trace_android_rvh_get_wts_last_enqueued_ts(android_rvh_get_wts_last_enqueued_ts, NULL);
	register_trace_android_vh_max_pred_ls(android_vh_max_pred_ls, NULL);
	register_trace_android_vh_predict_util(android_vh_predict_util, NULL);
	register_trace_android_vh_top_task_util(android_vh_top_task_util, NULL);
	register_trace_android_rvh_walt_ktime_clock(android_rvh_walt_ktime_clock, NULL);
	register_trace_android_rvh_walt_update_task_ravg(android_rvh_walt_update_task_ravg, NULL);
	register_trace_android_rvh_get_top_task_skip_hispeed(android_rvh_get_top_task_skip_hispeed, NULL);
	register_trace_android_vh_use_pelt_freq(android_vh_use_pelt_freq, NULL);
#if IS_ENABLED(CONFIG_HW_RTG)
	register_trace_android_rvh_init_new_task_load(android_rvh_init_new_task_load, NULL);
	register_trace_android_vh_sched_update_rtg_tick(android_vh_sched_update_rtg_tick, NULL);
	register_trace_android_vh_hwrtg_binder_check(android_vh_hwrtg_binder_check, NULL);
	register_trace_android_rvh_hwrtg_fork_prio(android_rvh_hwrtg_fork_prio, NULL);
	register_trace_android_rvh_hwrtg_task_load(android_rvh_hwrtg_task_load, NULL);
	register_trace_android_rvh_is_key_aux_comm(android_rvh_is_key_aux_comm, NULL);
#endif
#if IS_ENABLED(CONFIG_HW_RTG_FRAME)
	register_trace_android_rvh_proc_rtg_open(android_rvh_proc_rtg_open, NULL);
#if IS_ENABLED(CONFIG_COMPAT)
	register_trace_android_rvh_proc_rtg_compat_ioctl(android_rvh_proc_rtg_compat_ioctl, NULL);
#endif
	register_trace_android_rvh_proc_rtg_ioctl(android_rvh_proc_rtg_ioctl, NULL);
	register_trace_android_rvh_trans_rtg_sched_enqueue(android_rvh_trans_rtg_sched_enqueue, NULL);
	register_trace_android_rvh_trans_rtg_sched_dequeue(android_rvh_trans_rtg_sched_dequeue, NULL);
#endif
#if IS_ENABLED(CONFIG_QCOM_WALT_RTG) || IS_ENABLED(CONFIG_HW_RTG_DEBUG)
	register_trace_android_rvh_sched_group_id_open(android_rvh_sched_group_id_open, NULL);
	register_trace_android_rvh_sched_group_id_write(android_rvh_sched_group_id_write, NULL);
#endif
#if IS_ENABLED(CONFIG_HW_RTG_NORMALIZED_UTIL)
	register_trace_android_vh_get_max_group_util(android_vh_get_max_group_util, NULL);
#endif
#if IS_ENABLED(CONFIG_HW_TASK_RAVG_SUM)
	register_trace_android_vh_ravg_sum(android_vh_ravg_sum, NULL);
#endif
}
...
#endif
#if IS_ENABLED(CONFIG_HW_RTG_FRAME)
extern int __init init_frame_info(void);
#endif
#if IS_ENABLED(CONFIG_HW_RTG_AUX)
extern int __init init_aux_info(void);
#endif
#if IS_ENABLED(CONFIG_HW_RTG_DEBUG)
extern int __init init_sched_debug_procfs(void);
extern int __init init_aux_sched_debug_procfs(void);
extern int __init init_sched_rtg_debug_procfs(void);
#endif
...
static int walt_module_init(void)
{
	...
	init_aux_sched_debug_procfs();
	init_sched_rtg_debug_procfs();
#endif
