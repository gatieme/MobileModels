...
extern int _sched_set_group_id(struct task_struct *p, unsigned int group_id);
extern unsigned int sched_get_group_id(struct task_struct *p);
extern struct related_thread_group *lookup_related_thread_group(unsigned int group_id);
...
struct walt_sched_cluster {
	...
	unsigned int            capacity_margin_rt;
#endif
};
...
#ifdef CONFIG_HW_SCALE_FREQ_CAPACITY
	unsigned long scale_freq = (topology_get_freq_scale(cpu) <<		\
				    SCHED_CAPACITY_SHIFT) /			\
...
{
	struct walt_rq *wrq = (struct walt_rq *) cpu_rq(cpu)->android_vendor_data1;
	u64 util = wrq->walt_stats.cumulative_runnable_avg_scaled;
...
static inline bool walt_low_latency_task(struct task_struct *p)
{
	...
	return wts->low_latency &&
		(walt_task_util(p) < sysctl_walt_low_latency_task_threshold);
}
...
static inline enum sched_boost_policy task_boost_policy(struct task_struct *p)
{
	...
	if (policy == SCHED_BOOST_ON_BIG) {
		...
		if (sched_boost_type == CONSERVATIVE_BOOST &&
			walt_task_util(p) <= sysctl_sched_min_task_util_for_boost &&
...
static inline bool walt_uclamp_boosted(struct task_struct *p)
{
	return ((uclamp_eff_value(p, UCLAMP_MIN) > 0) &&
			(walt_task_util(p) > sysctl_sched_min_task_util_for_uclamp));
}
...
static inline bool __cpu_overutilized(int cpu, int delta)
{
	return (capacity_orig_of(cpu) * 1024) <
		((walt_cpu_util(cpu) + delta) * sched_capacity_margin_up[cpu]);
}
...
static inline bool asym_cap_sibling_group_has_capacity(int dst_cpu, int margin)
{
	...
	total_util = walt_cpu_util(sib1) + walt_cpu_util(sib2);
...
static inline unsigned long walt_task_util_est(struct task_struct *p)
{
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
#ifdef CONFIG_HW_SCHED_WALT
	if (unlikely(walt_disabled || !sysctl_sched_use_walt_task_util))
...
{
	return clamp(walt_task_util_est(p),
...
static inline unsigned long uclamp_task_util(struct task_struct *p)
{
	return walt_task_util_est(p);
}
#endif
#if IS_ENABLED(CONFIG_HW_RT_CAS) || IS_ENABLED(CONFIG_HW_RT_ACTIVE_LB)
...
static inline bool walt_should_kick_upmigrate(struct task_struct *p, int cpu)
{
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
#if IS_ENABLED(CONFIG_HW_RTG)
	struct related_thread_group *rtg = wts->grp;
#else
...
#define cpus_paused_mask ((const struct cpumask *)&cpus_paused_by_us)
#define cpu_paused(cpu) cpumask_test_cpu((cpu), cpus_paused_mask)
#endif
