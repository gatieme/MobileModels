...
/*
 * walt_cpu_util_without: compute cpu utilization without any contributions from *p
 ...
 */
static unsigned long walt_cpu_util_without(int cpu, struct task_struct *p)
{
	unsigned int util;
		...
		bool *energy_eval_needed, bool *ignore_cluster)
{
	...
	*order_index = i;
	...
	if (*order_index == 0 &&
			(walt_task_util(p) >= MIN_UTIL_FOR_ENERGY_EVAL) &&
...
retry_ignore_cluster:
	for (cluster = 0; cluster < num_sched_clusters; cluster++) {
		...
		for_each_cpu(i, &visit_cpus) {
			 ...
			 */
			wake_cpu_util = walt_cpu_util_without(i, p);
...
cpu_util_next_walt(int cpu, struct task_struct *p, int dst_cpu)
{
	...
	if (unlikely(queued)) {
		if (task_cpu(p) == cpu) {
			if (dst_cpu != cpu)
				util = max_t(long, util - walt_task_util(p), 0);
		} else if (dst_cpu == cpu) {
...
static inline bool select_cpu_same_energy(int cpu, int best_cpu, int prev_cpu)
{
	...
	if (walt_cpu_util(best_cpu) <= walt_cpu_util(cpu))
		return false;
...
{
	return capacity_orig_of(cpu) - walt_cpu_util(cpu);
}
