...
static int find_first_idle_if_others_are_busy(const cpumask_t *src_mask)
{
	...
	for_each_cpu(i, src_mask) {
		...
		if (walt_cpu_util(i) < SMALL_TASK_THRESHOLD && cpu_rq(i)->nr_running == 1) {
			/*
		...
		int *has_misfit, bool is_newidle)
{
	...
	for_each_cpu(i, src_mask) {
		...
		if (cpu_rq(i)->nr_running < 2)
			continue;

		if (cpu_rq(i)->nr_running == 2 &&
			(walt_task_util(cpu_rq(i)->curr) < SMALL_TASK_THRESHOLD) &&
			walt_fair_task(cpu_rq(i)->curr))
			continue;
#else
