				...
				struct cpumask *lowest_mask, int ret, int *best_cpu)
{
	...
	unsigned long tutil = walt_task_util(task);
...
static void android_rvh_find_cas_cpu(void *unused, struct sched_domain *sd,
				     ...
				     int *cpu)
{
	...
	target_cpu = find_cas_cpu(sd, task, lowest_mask);

	if (target_cpu != -1)
		*cpu = target_cpu;
}

static void android_rvh_sched_enable_rt_cas(void *unused, unsigned int *enable_rt_cas)
{
	*enable_rt_cas = sysctl_sched_enable_rt_cas;
}
...
static void android_vh_check_for_rt_migration(void *unused, struct rq *rq,
					      struct task_struct *curr)
{
...
#if IS_ENABLED(CONFIG_HW_RT_CAS)
static void android_vh_rt_task_fits_capacity(void *unused, struct task_struct *p, unsigned int cpu, int *ret)
{
	unsigned long tutil = boosted_task_util(p);
	unsigned long cpu_capacity = capacity_orig_of(cpu);
	unsigned int rt_misfit_margin;
	...
	rt_misfit_margin = hw_capacity_margin_rt(cpu);

	*ret = tutil * rt_misfit_margin <=
...
#if IS_ENABLED(CONFIG_HW_RT_CAS)
	register_trace_android_rvh_find_cas_cpu(android_rvh_find_cas_cpu, NULL);
	register_trace_android_rvh_sched_enable_rt_cas(android_rvh_sched_enable_rt_cas, NULL);
	register_trace_android_vh_rt_task_fits_capacity(android_vh_rt_task_fits_capacity, NULL);
#else
	register_trace_android_rvh_select_task_rq_rt(walt_select_task_rq_rt, NULL);
#endif
#if IS_ENABLED(CONFIG_HW_RT_ACTIVE_LB)
	register_trace_android_vh_check_for_rt_migration(android_vh_check_for_rt_migration, NULL);
#endif
}
