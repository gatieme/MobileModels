...
#ifdef CONFIG_HW_MM_POLICY
atomic_long_t vip_allocstall_count = ATOMIC_LONG_INIT(0);
#endif
...
void pgalloc_count_inc(bool is_slowpath, unsigned int order)
{
	u64 jiffs = 0;
#ifdef CONFIG_HW_MM_POLICY
	bool vip_stall = rt_task(current) || vip_care_allocstall();
	if (!enable || (!vip_stall && is_background()))
		return;
#else
...
#ifdef CONFIG_HW_MM_POLICY
		if (vip_stall)
			atomic_long_inc(&vip_allocstall_count);
#endif
...
#ifdef CONFIG_HW_MM_POLICY
	seq_printf(s, "Total allocstall of the vip threads:%ld\n",
	    atomic_long_read(&vip_allocstall_count));
#endif
	return 0;
}
