...
void pch_get_ra_para(struct file_ra_state *ra, pgoff_t offset)
{
	ra->size = pch_shrink_mmap_pages(ra->ra_pages);
	ra->start = (pgoff_t)max_t(long, 0, offset - (ra->size >> 1));
				    ...
				    int forward)
{
	...
	do_page_cache_ra(&ractl, req_size, async_size);
#else
				    ...
				    pgoff_t offset)
{
	...
	__pch_mmap_readextend(vma->vm_flags, ra, file, offset, 1);
}
#endif
...
#else /* v5.8-rc1 remove NR_UNSTABLE_NFS */
	nr_dirty = global_node_page_state(NR_FILE_DIRTY) +
				global_node_page_state(NR_WRITEBACK);
#endif
