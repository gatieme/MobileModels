...
void xreclaimer_cond_quick_reclaim(struct task_struct *tsk, int sig)
{
	/* Signal from kernel may hold task lock, so... */
	if (!sysctl_xreclaimer_enable || !tsk || sig != SIGKILL)
#else
void xreclaimer_cond_quick_reclaim(struct task_struct *tsk, int sig,
				   struct siginfo *info, bool skip)
{
	/* Signal from kernel may hold task lock, so... */
...
static void __xreclaimer_exit_mmap(struct mm_struct *mm, bool mmap_locked)
{
...
{
	__xreclaimer_exit_mmap(mm, false);
}

void xreclaimer_exit_mmap_locked(struct mm_struct *mm)
{
	__xreclaimer_exit_mmap(mm, true);
}
