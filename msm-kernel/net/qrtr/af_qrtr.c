...
static void qrtr_log_tx_suspend_debug(struct qrtr_hdr_v1 *cb, u64 pl_buf)
{
	if (s2idle_state_flag || suspend_freeze_processes_flag ||
		...
		pr_info(
			...
			(unsigned int)pl_buf, (unsigned int)(pl_buf >> PL_BUFF_BIAS),
...
void qrtr_print_wakeup_reason(const void *data)
{
	...
	pr_info("%s: src[0x%x:0x%x] dst[0x%x:0x%x] [%08x %08x] service[0x%x]\n",
		...
		service_id);
#ifdef CONFIG_HUAWEI_DUBAI
	HWDUBAI_LOGE("DUBAI_TAG_QRTR_RESUME_PKT", "src=%d dst=%d service=%d", cb.src_node,
		cb.dst_node, service_id);
#endif
}
