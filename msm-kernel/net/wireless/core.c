...
#define MAC_LEN 6
static u8 wifimac[MAC_LEN] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

#ifdef CONFIG_ARCH_QCOM
int set_slave_wlan_mac_address(const u8 *mac_list, const uint32_t len)
{
	int ret;
	ret = memcpy_s(wifimac, MAC_LEN, mac_list, len);
	if (ret != 0) {
		pr_err("hwifi_set_slave_mac_addr::memcpy fail![%d]", ret);
		return -1;
	}
	return 0;
}
EXPORT_SYMBOL(set_slave_wlan_mac_address);
#endif /* CONFIG_ARCH_QCOM */

u8* get_slave_wlan_mac_address(void)
{
	return wifimac;
}
EXPORT_SYMBOL(get_slave_wlan_mac_address);
#endif
