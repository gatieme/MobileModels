	...
	while ((paths + i) && *(paths + i)) {
		if (paths[i] == ':') {
			...
			if (ntoken >= MAX_PROC_NUM) {
				rs_log_error(RSCAN_LOG_TAG, "paths_explode overflow");
				break;
			}
			...
			if ((paths + i + 1) && *(paths + i + 1)) {
				...
				rs_log_error(RSCAN_LOG_TAG, "white list formate abnormal");
				break;
			}
		}
		i++;
...
static void paths_implode(char *paths, uint32_t paths_len, char **const tokens, int ntoken)
{
	int ti;
	paths[0] = '\0';

	for (ti = 0; ti < ntoken; ti++) {
		uint32_t tmp_len = sprintf_s(paths, paths_len, ti == 0 ? "%s" : ":%s", tokens[ti]);
		if (tmp_len <= 0) {
			rs_log_error(RSCAN_LOG_TAG, "paths_implode sprintf_s error");
			return;
		}
		paths_len -= tmp_len;
		paths += tmp_len;
			...
			int next, int token_count)
{
	...
	ret = memmove_s(tokens + cur,
			...
			(ulong)((token_count - next) * sizeof(char *)));
	if (ret != EOK)
		rs_log_error(RSCAN_LOG_TAG, "memmove_s fail\n");
...
bool init_rprocs_trustlist(const char *trustlist)
{
	...
	if (strlen(trustlist) >= PAGE_SIZE) {
		rs_log_error(RSCAN_LOG_TAG, "trustlist over the PAGE_SIZE, %u",
				...
				char *rprocs, ssize_t rprocs_len)
{
	...
	if ((exec_path == NULL) || (final_tokens == NULL) || (rprocs == NULL) ||
		...
		rs_log_error(RSCAN_LOG_TAG, "strip white list failed, input value invalid");
		return;
	}

	tmp = vmalloc((ulong)rprocs_len);
	if (tmp == NULL) {
		rs_log_error(RSCAN_LOG_TAG, "rprocs_strip_trustlist failed, out of memory");
		return;
	}
	...
	if (!format_rprocs(exec_path, &ntoken, tmp)) {
		rs_log_error(RSCAN_LOG_TAG, "strip white list failed, format_rprocs error");
		vfree(tmp);
		return;
	}
	...
	for (i = 0; i < ntoken; i++) {
		if (!find_proc_in_init_list(exec_path[i])) {
			...
			rs_log_error(RSCAN_LOG_TAG, "the root proc is abnormal: %s", exec_path[i]);
		}
	}

	/* reconstruct */
	paths_implode(rprocs, rprocs_len, final_tokens, final_ntoken);
	vfree(tmp);
}
...
void rprocs_strip_trustlist(char *rprocs, ssize_t rprocs_len)
{
	...
	if (tokens == NULL) {
		rs_log_error(RSCAN_LOG_TAG, "no enough memory for allocation");
		return;
	}
...
static int get_task_exe(struct mm_struct *mm, pid_t pid, char *out,
			uint32_t outlen, char *dbuf, int dbuflen)
{
	struct file *exe_file = NULL;
	char *pathname = NULL;
	...
	if ((mm == NULL) || (out == NULL) || (dbuf == NULL)) {
		rs_log_error(RSCAN_LOG_TAG, "input parameter invalid");
		return -EINVAL;
	}
	exe_file = my_get_mm_exe_file(mm);
	if (exe_file == NULL) {
		rs_log_error(RSCAN_LOG_TAG, "get mm exe file %d failed", pid);
		return 0;
	}
	pathname = d_path(&exe_file->f_path, dbuf, dbuflen);
	fput(exe_file);
	if (pathname == NULL || IS_ERR(pathname)) {
		rs_log_error(RSCAN_LOG_TAG, "get task exe d_path %d failed", pid);
		return 0;
	}
	...
	len = strlen(pathname);
	if (len >= PATH_MAX) {
		rs_log_error(RSCAN_LOG_TAG, "value of len larger than PATH_MAX");
		return 0;
	}

	if (outlen < len) {
		rs_log_warning(RSCAN_LOG_TAG, "get task exe path \"%s\", value of len larger than %d",
			pathname, outlen);
		return -1;
	}
...
int get_root_procs(char *out, uint32_t outlen)
{
	char *tmp = NULL;
	...
	if (tmp == NULL) {
		rs_log_error(RSCAN_LOG_TAG, "get free page failed");
		return -EINVAL;
	}
