...
static int smartpa_set_algo_scene_default(struct adsp_misc_priv *pdata, void *data, size_t len)
{
	UNUSED(pdata);
	UNUSED(data);
	UNUSED(len);
	...
	struct adsp_misc_priv *pdata)
{
	...
	memcpy(&pdata->pa_info, &param->pa_info, sizeof(pdata->pa_info));
#ifndef CONFIG_FINAL_RELEASE
	hwlog_info("%s: enter, cmd = %d, chip_vendor = %d, len:%d\n", __func__, cmd, chip_vendor, len);
#endif
	switch (cmd) {
	...
	case SET_CALIBRATION_VALUE:
		...
		if (chip_vendor != 4) {
			if (param->size != pdata->pa_info.pa_num * sizeof(uint32_t)
				|| param->size > sizeof(pdata->calibration_value)) {
			...
			if (param->size == sizeof(struct calib_oeminfo_common))
				memcpy(&pdata->calib_oeminfo, param->data, param->size);
			else
	...
	case SET_ALGO_SECENE:
		...
		ret = adsp_misc_cap_intf[chip_vendor].set_algo_scene(pdata, param->data, param->size);
		break;
	case SMARTPA_SET_BATTERY:
		...
		ret = adsp_misc_cap_intf[chip_vendor].set_algo_battery(pdata, param->data, param->size);
		break;
	default:
		break;
	}
...
static int soc_adsp_send_ultrasonic_param(struct adsp_misc_ultrasonic_info *param,
	unsigned char *data, unsigned int len, struct adsp_misc_priv *pdata)
{
	...
	cmd = param->cmd;
	ret = cust_ultrasonic_set_param(pdata, param->data, param->size, param->cmd);
	if (ret)
		pr_err("%s: send cmd = %d, ret = %d\n", __func__, cmd, ret);
	...
	struct adsp_misc_priv *pdata)
{
	...
	if ((param_out_len > MIN_PARAM_OUT) && (adsp_param.param_out != NULL)) {
		...
		result->size = param_out_len - sizeof(*result);
		ret = soc_adsp_send_ultrasonic_param(param_in, result->data, result->size,
				pdata);
		if (ret < 0)
			goto err;
