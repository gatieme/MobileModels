	...
	if (index >= 0) {
		...
		if (pdata->mi2s_gpio_p[index]) {
			if (atomic_read(&(pdata->mi2s_gpio_ref_cnt[index])) == 0) {
				pr_info("%s: msm_cdc_pinctrl_select_active_state\n", __func__);
				...
				pr_info("%s: mi2s_gpio ref cnt is %d\n",
					__func__, atomic_read(&(pdata->mi2s_gpio_ref_cnt[index])));
			}
	...
	if (index >= 0) {
		...
		if (pdata->mi2s_gpio_p[index]) {
			...
			pr_info("%s: mi2s_gpio ref cnt is %d\n",
				__func__, atomic_read(&(pdata->mi2s_gpio_ref_cnt[index])));
			if (atomic_read(&pdata->mi2s_gpio_ref_cnt[index]) == 0)  {
				pr_info("%s: msm_cdc_pinctrl_select_sleep_state\n", __func__);
