...
enum {
	DMIC_MICBIAS_EN_ACTIVE = 0,
	DMIC_MICBIAS_EN_SLEEP
};

#define VOLTAGE_SET_UA_LOAD 3000
#define VOLTAGE_DEFAULT_VALUE_MIN 3296000
#define VOLTAGE_DEFAULT_VALUE_MAX 3600000
...
struct msm_asoc_mach_data {
	...
	struct device_node *dmic_micbias_en_gpios_p; /* used by pinctrl API */
...
static void msm_dmic_micbias_en_event_custom(struct device *dev,
	struct msm_asoc_mach_data *pdata, int state)
{
	int ret = 0;

	if (!of_property_read_bool(dev->of_node, "qcom,cdc-dmic-micbias-en"))
		return;

	switch (state) {
	case DMIC_MICBIAS_EN_ACTIVE:
		ret = msm_cdc_pinctrl_select_active_state(pdata->dmic_micbias_en_gpios_p);
		if (ret < 0) {
			pr_err("%s: gpio set cannot be activated %sd",
				__func__, "dmic_en_gpio");
			return;
		}
		break;
	case DMIC_MICBIAS_EN_SLEEP:
		ret = msm_cdc_pinctrl_select_sleep_state(pdata->dmic_micbias_en_gpios_p);
		if (ret < 0) {
		...
		break;
	default:
		pr_err("%s: unsurpport state is %d", __func__, state);
		break;
	}
	return;
}
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		(*dmic_gpio_cnt)++;
		if (*dmic_gpio_cnt == 1) {
			msm_dmic_micbias_en_event_custom(component->dev, pdata,
				DMIC_MICBIAS_EN_ACTIVE);
...
{
	struct snd_soc_card *card = &snd_soc_card_waipio_msm;
	struct msm_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	const int *data = NULL;
	...
	if (ghscv) {
		pr_info("headset set regulator!\n");

		data = pdata->voltage_data;
		if (en)
			regulator_set_voltage(ghscv, data[VOLTAGE_MAX_VAL], data[VOLTAGE_MAX_VAL]);
		else
			regulator_set_voltage(ghscv, data[VOLTAGE_MIN_VAL], data[VOLTAGE_MIN_VAL]);

		regulator_set_load(ghscv, VOLTAGE_SET_UA_LOAD);
		regulator_enable(ghscv);
	}
...
static struct usb_headphone_ops waipio_vbus_gpio_ops = {
	.otg_need_gpio_switch = usb_headphone_vbus_gpio_switch,
};

static void do_set_pinctrl(struct platform_device *pdev, struct msm_asoc_mach_data *pdata)
{
	struct pinctrl_state *pinctrl_active = NULL;

	struct pinctrl *pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR_OR_NULL(pinctrl)) {
		dev_err(&pdev->dev, "%s: Cannot get lowpower gpio pinctrl:%ld\n",
			__func__, PTR_ERR(pinctrl));
		return ;
	}

	pinctrl_active = pinctrl_lookup_state(pinctrl, "lowpower_default");
	if (IS_ERR_OR_NULL(pinctrl_active)) {
		dev_err(&pdev->dev, "%s: Cannot get lowpower_default pinctrl state:%ld\n",
			__func__, PTR_ERR(pinctrl_active));
		return ;
	}
...
static void set_headset_lowpwer_pin(struct platform_device *pdev,
	struct msm_asoc_mach_data *pdata)
{
	int ret;

	do_set_pinctrl(pdev, pdata);

	/* pdata != NULL always true*/
	...
	gpio_direction_output(pdata->hs_lp_switch_gpio, 0);
}
...
static void register_headset_lowpower(struct platform_device *pdev)
{
	...
	set_headset_lowpwer_pin(pdev, pdata);

	register_usb_low_power_otg(&waipio_vbus_gpio_ops);

	ghscv = devm_regulator_get(&pdev->dev, "cdc-vdd-mic-bias");
	if (IS_ERR_OR_NULL(ghscv)) {
		dev_info(&pdev->dev, "normal headset vbus\n");
		ghscv = NULL;
		return ;
	}

	ret = of_property_read_u32_array(pdev->dev.of_node,
			"qcom,cdc-vdd-mic-bias-voltage", data, sizeof(data) / sizeof(*data));
	if (ret < 0) {
		dev_info(&pdev->dev, "use default voltage\n");

		data[VOLTAGE_MIN_VAL] = VOLTAGE_DEFAULT_VALUE_MIN;
		data[VOLTAGE_MAX_VAL] = VOLTAGE_DEFAULT_VALUE_MAX;
	}

	regulator_set_voltage(ghscv, data[VOLTAGE_MIN_VAL], data[VOLTAGE_MIN_VAL]);

	pdata->voltage_data[VOLTAGE_MIN_VAL] = data[VOLTAGE_MIN_VAL];
	pdata->voltage_data[VOLTAGE_MAX_VAL] = data[VOLTAGE_MAX_VAL];
}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	pdata->dmic67_gpio_p = of_parse_phandle(pdev->dev.of_node,
					      "qcom,cdc-dmic67-gpios",
					       0);
	pdata->dmic_micbias_en_gpios_p = of_parse_phandle(pdev->dev.of_node,
					      "qcom,cdc-dmic-micbias-en-gpios",
					       0);
	...
	if (pdata->dmic_micbias_en_gpios_p)
		msm_cdc_pinctrl_set_wakeup_capable(pdata->dmic_micbias_en_gpios_p, false);
