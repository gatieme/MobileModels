...
static struct hw_cloud_network_ops cloud_network_ops = {
	.rx_handle = NULL,
	.tx_handle = NULL,
};
#endif
...
#ifdef CONFIG_HW_WIFI_AMAX
	if (amax_hook_tx_data(skb, &dev) != 0)
		return NETDEV_TX_OK;
#endif
#ifdef CONFIG_HW_DC_MODULE
...
#ifdef CONFIG_HW_CLOUD_NETWORK
	if (skb->sk != NULL && cloud_network_ops.tx_handle &&
		(cloud_network_ops.tx_handle(skb, skb->dev) == 0))
...
static __latent_entropy void net_tx_action(struct softirq_action *h)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_softirq_hook, (u32)7,
		(u64)(uintptr_t)net_tx_action, 0);
...
#ifdef CONFIG_HW_CLOUD_NETWORK
	if (cloud_network_ops.rx_handle &&
		(cloud_network_ops.rx_handle(skb, skb->dev) == GRO_DROP))
		return NET_RX_DROP;
#endif
...
static __latent_entropy void net_rx_action(struct softirq_action *h)
{
	...
	CALL_HCK_VH(hck_dfx_bbox_softirq_hook, (u32)8,
		(u64)(uintptr_t)net_rx_action, 0);
	for (;;) {
...
#ifdef CONFIG_HW_CLOUD_NETWORK
int cloud_network_op_register(const struct hw_cloud_network_ops *ops)
{
	if (ops == NULL)
		return -1;
	cloud_network_ops.rx_handle = ops->rx_handle;
	cloud_network_ops.tx_handle = ops->tx_handle;
	return 0;
}

void cloud_network_op_unregister(void)
{
	cloud_network_ops.rx_handle = NULL;
	cloud_network_ops.tx_handle = NULL;
	return;
}
#endif
