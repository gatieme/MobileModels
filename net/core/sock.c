...
#include <hwnet/booster/cloud_network/cloud_network_param.h>
#include <hwnet/booster/cloud_network/cloud_network_device.h>
#include <hwnet/booster/cloud_network/cloud_network_session.h>
#endif
...
static void __sock_set_mark(struct sock *sk, u32 val)
{
	if (val != sk->sk_mark) {
#ifdef CONFIG_HUAWEI_XENGINE
		emcom_xengine_notify_sk_mark_modify(sk, val);
#endif
		sk->sk_mark = val;
		sk_dst_reset(sk);
	}
...
static void __sk_destruct(struct rcu_head *head)
{
	...
	if (is_cloud_network_enable())
		cloud_network_inet6_release_handle(sk);
#endif
...
#ifdef CONFIG_HW_CLOUD_NETWORK
	if (is_cloud_network_enable() && tun_dev_match_by_name(dst->dev))
...
void sock_init_data_uid(struct socket *sock, struct sock *sk, kuid_t uid)
{
	...
	sk->quickack_try_cnt = 0;
	sk->quickack_thold = 0;
#endif

#ifdef CONFIG_HW_NETWORK_DCP
	sk->is_first_packet = 0;
	sk->v6_index = 0;
#endif
