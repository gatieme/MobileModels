...
#ifdef CONFIG_HN_WIFI
#define HN_TCP_TIMESTAMP_ERR_THRESHOLD    1
static unsigned int g_default_ipv4_sysctl_tcp_timestamps;
static void hn_tcp_check_and_disable_timestamps(struct net *net,
	struct sk_buff *skb);
#endif
	...
	struct sk_buff *skb)
{
	...
	if (net->ipv4.sysctl_tcp_timestamps &&
	    (++tcp_ts_err > HN_TCP_TIMESTAMP_ERR_THRESHOLD)) {
		pr_err("TCP timestamp error, check network interface and try to disable ts.\n");
		if (hn_timestamps_get_wifi_connect_status() &&
...
unsigned int hn_get_currect_ipv4_sysctl_tcp_timestamps(void)
{
					 ...
					 const struct tcphdr *th)
{
	...
	if (th->ack) {
		...
		if (tp->rx_opt.saw_tstamp && tp->rx_opt.rcv_tsecr &&
			...
			hn_tcp_check_and_disable_timestamps(sock_net(sk), skb);
#endif
			NET_INC_STATS(sock_net(sk),
		...
		if (icsk->icsk_retransmits > 2) {
			SOCK_DEBUG(sk, "icsk_retransmits=%d\n", icsk->icsk_retransmits);
			notify_chr_thread_to_send_msg(inet->inet_daddr, inet->inet_saddr, sk);
		} else {
