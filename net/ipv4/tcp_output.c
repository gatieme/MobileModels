...
#ifdef CONFIG_HONOR_FEATURE_PRINT_PID_NAME
#include <hwnet/booster/pid_socket.h>
#endif
...
extern unsigned int hn_get_currect_ipv4_sysctl_tcp_timestamps(void);
#endif
...
#ifdef CONFIG_HN_WIFI
static void hn_tcp_timestamps_recovery(struct net *net)
{
	if (net == NULL)
		return;
	...
	if (!hn_timestamps_get_wifi_connect_status() &&
		...
		net->ipv4.sysctl_tcp_timestamps =
			hn_get_currect_ipv4_sysctl_tcp_timestamps();
}
#endif
...
#ifdef CONFIG_HN_WIFI
	hn_tcp_timestamps_recovery(sock_net(sk));
#endif
	tcp_connect_init(sk);
