...
#ifdef CONFIG_HONOR_MSS_AUTO_CHANGE
#define TCP_MSS_REDUCE_SIZE 200
#define TCP_MSS_MIN_SIZE 1200

static void tcp_reduce_mss(struct inet_connection_sock *icsk, struct sock *sk)
{
	...
	if (icsk && sk) {
		...
		if (tp && tp->mss_cache > TCP_MSS_MIN_SIZE &&
			...
			if (tp->mss_cache - TCP_MSS_REDUCE_SIZE <
				...
				tp->rx_opt.mss_clamp = tp->mss_cache -
					TCP_MSS_REDUCE_SIZE;
...
#ifdef CONFIG_HN_SYN_LINEAR_RETRY
