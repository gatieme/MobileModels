...
#ifdef CONFIG_HW_CLOUD_NETWORK
	if (is_cloud_network_enable() && cloud_network_started())
		master_session_delete(&ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple,
			nf_ct_net(ct));
#endif
...
#ifdef CONFIG_HW_CLOUD_NETWORK
		if (is_cloud_network_enable() && cloud_network_started())
			master_session_delete(&tmp->tuplehash[IP_CT_DIR_ORIGINAL].tuple,
				nf_ct_net(tmp));
#endif
