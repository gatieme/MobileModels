	...
	struct npu_stream_msg *stream_msg)
{
	...
	ret = npu_get_ttbr(&(stream_msg->ttbr), &(stream_msg->tcr), current->tgid);
	...
	cond_return_error(cq_info == NULL || sq_info == NULL, -1,
		"calc cq info or sq_info error, cq_index = %d, sq_index = %d\n",
		cq_index, sq_index);
	...
	cq_stream_num = (u32)npu_get_cq_ref_by_communication_stream(dev_id, cq_index);
	if (cq_stream_num > 1) {
	...
	struct npu_stream_msg *stream_msg)
{
	...
	cq_stream_num = (u32)npu_get_cq_ref_by_stream(dev_id, cq_index);
	if (cq_stream_num > 1) {
