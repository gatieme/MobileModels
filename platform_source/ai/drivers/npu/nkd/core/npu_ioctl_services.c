	...
	struct npu_entity_info *hwts_sq_sub_info)
{
	...
	// verify hwts_sqe
	ret = g_npd->npd_verify_sink_tasks(proc_ctx, stream_buf_addr, stream_len);
	if (ret != 0) {
		vfree(stream_buf_addr);
	...
	sqe_num = g_npd->npd_format_hw_sqe(model_desc_info, stream_buf_addr,
	...
	npu_model_desc_t *model_desc)
{
	...
	for (stream_idx = model_desc->stream_cnt - 1; stream_idx >= 0; stream_idx--) {
		...
		ret = g_npd->npd_format_sw_sqe(model_desc, stream_buf_addr, &first_task_id, devid, stream_idx);
		if (ret != 0) {
			vfree(stream_buf_addr);
			npu_drv_err("npu_verify_hwts_sqe fail, length= %d, ret= %d\n",
				stream_len, ret);
	...
	unsigned long arg)
{
	...
	cond_return_error(model_prio_info.priority > MAX_PRIORITY_LEVEL,
		-EINVAL, "priority is invalid\n");

	ret = npu_proc_set_model_priority(proc_ctx, model_prio_info);
	...
	unsigned long arg)
{
	...
	ret = npu_proc_set_model_profiling(proc_ctx, model_profiling_info);
		cond_return_error(ret != 0, ret,
		"npu_proc_set_model_priority fail, ret= %d\n", ret);
	return  ret;
}
...
static int (*npu_ioctl_call[NPU_MAX_CMD])(struct npu_proc_ctx *proc_ctx, unsigned long arg) = {
	...
	[_IOC_NR(NPU_ENTER_WORKMODE)] = npu_ioctl_enter_workwode,
	[_IOC_NR(NPU_EXIT_WORKMODE)] = npu_ioctl_exit_workwode,
