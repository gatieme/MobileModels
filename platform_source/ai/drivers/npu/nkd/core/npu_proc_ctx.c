...
void npu_release_proc_ctx(struct npu_proc_ctx *proc_ctx)
{
	...
	if (proc_ctx->manager_release) {
		...
		npu_drv_err("npu_release_proc_ctx release direct mm,cnt=%u\n", proc_ctx->task_cnt);
...
int npu_proc_check_stream_id(struct npu_proc_ctx *proc_ctx, u32 stream_id)
{
	...
	if (test_bit(stream_id, proc_ctx->stream_bitmap)) {
		return 0;
	} else {
...
int npu_proc_check_event_id(struct npu_proc_ctx *proc_ctx, u32 event_id)
{
	...
	if (test_bit(event_id, proc_ctx->event_bitmap)) {
		return 0;
	} else {
		npu_drv_err("invalidate id %u\n", event_id);
		return -EFAULT;
...
int npu_proc_check_hwts_event_id(struct npu_proc_ctx *proc_ctx, u32 event_id)
{
	...
	if (test_bit(event_id, proc_ctx->hwts_event_bitmap)) {
		return 0;
	} else {
		npu_drv_err("invalidate id %u\n", event_id);
		return -EFAULT;
...
int npu_proc_check_model_id(struct npu_proc_ctx *proc_ctx, u32 model_id)
{
	...
	if (test_bit(model_id, proc_ctx->model_bitmap)) {
		return 0;
	} else {
...
int npu_proc_check_task_id(struct npu_proc_ctx *proc_ctx, u32 task_id)
{
	...
	if (test_bit(task_id, proc_ctx->task_bitmap)) {
		return 0;
	} else {
		npu_drv_err("invalidate id %u\n", task_id);
		return -EFAULT;
...
int npu_proc_set_model_priority(struct npu_proc_ctx *proc_ctx, npu_model_prio_info_t model_prio_info)
{
	...
	id_entity = (struct npu_id_entity *)(
		...
		id_allocator->entity_size * (model_prio_info.model_id - id_allocator->start_id));

	if (id_entity == NULL || id_entity->id != model_prio_info.model_id) {
		npu_drv_err("fatal error, can not find id entity id = %u\n", model_prio_info.model_id);
		mutex_unlock(&id_allocator->lock);
		return -1;
	}
	id_entity->data[0] = (u32)model_prio_info.priority;
	mutex_unlock(&id_allocator->lock);
	return 0;
}

int npu_proc_set_model_profiling(struct npu_proc_ctx *proc_ctx, npu_model_profiling_info_t model_profiling_info)
{
	struct npu_id_entity *id_entity = NULL;
	struct npu_id_allocator *id_allocator = NULL;
	...
	id_entity = (struct npu_id_entity *)(
		(uintptr_t)id_allocator->id_entity_base_addr +
		id_allocator->entity_size * (model_profiling_info.model_id - id_allocator->start_id));

	if (id_entity == NULL || id_entity->id != model_profiling_info.model_id) {
		npu_drv_err("fatal error, can not find id entity id = %u\n", model_profiling_info.model_id);
		mutex_unlock(&id_allocator->lock);
		return -1;
	}
	/* profiling occupy at index 2 */
	id_entity->data[2] = model_profiling_info.profiling;
	mutex_unlock(&id_allocator->lock);
	return 0;
}
...
int npu_save_model_info(struct npu_proc_ctx *proc_ctx, npu_model_desc_t *model_desc)
{
	...
	if (id_entity == NULL || id_entity->id != model_desc->model_id) {
		...
		id_entity->data[0] = model_desc->priority;
		id_entity->data[1] = model_desc->execute_pid;
		id_entity->data[3] = 0;    // 3 is extcute times
		id_entity->data[4] = 0;    // 4 is reboot timees
		id_entity->data[5] = NPU_MODEL_EXECUTE_ENABLE;    // 5 is execute enable
		id_entity->data[6] = model_desc->uid;    // 6 is uid
	}
	mutex_unlock(&id_allocator->lock);
	...
	npu_model_desc_sub_t *model_desc_sub)
{
	...
	if (id_entity == NULL || id_entity->id != model_id || id_entity->is_allocated == 0) {
