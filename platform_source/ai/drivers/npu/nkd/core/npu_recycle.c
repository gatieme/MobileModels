...
bool npu_is_proc_resource_leaks(const struct npu_proc_ctx *proc_ctx)
{
	...
	if (!list_empty_careful(&proc_ctx->sink_stream_list) ||
		...
		!list_empty_careful(&proc_ctx->task_set) ||
		!list_empty_careful(&proc_ctx->excpt_task_set))
		result = true;
...
void npu_recycle_npu_resources(struct npu_proc_ctx *proc_ctx)
{
	...
	/* recycle stream */
	(void)npu_recycle_stream(proc_ctx);

	/* recycle event */
	(void)npu_recycle_event_id(proc_ctx);

	/* recycle hwts event */
	(void)npu_recycle_hwts_event_id(proc_ctx);

	/* recycle model */
	npu_recycle_model_id(proc_ctx);

	(void)npu_task_set_clear(cur_dev_ctx, proc_ctx);
