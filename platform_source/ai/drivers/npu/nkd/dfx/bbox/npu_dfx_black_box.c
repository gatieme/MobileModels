	...
	char log_buf[], unsigned int buf_size)
{
	...
	cond_return_void(ret < 0, "snprintf_s is fail, ret = %d", ret);
}
 ...
 */
#ifndef CONFIG_NPU_SWTS
void npu_bbox_dump_tsch_log(void)
{
	...
	npu_mntn_copy_reg_to_bbox(tsch_log_title, strlen(tsch_log_title));
	...
	struct work_struct *work __attribute__((__unused__)))
{
	...
	if (module_info->exp_message_len > module_info->buffer_len ||
		...
		bbox_tslog_addr_vir = g_bbox_addr_vir + sizeof(exce_module_info_t);
...
static void npu_mntn_reset_work(
	struct work_struct *work __attribute__((__unused__)))
{
	g_npd->npd_notify(NULL, NPD_NOTICE_MNTN_RESET, 0, 0);  // plat_mntn_reset
}
	...
	char *pathname, pfn_cb_dump_done pfn_cb)
{
	...
	g_npu_mntn_private->core_id = coreid;
...
static struct rdr_exception_info g_rdr_exception_info[] = {
		...
		/* AICORE exception code 0xc0000200-0xc000027f */
...
void npu_bbox_record_err_task(struct npu_task_dump_info *error_task)
{
	...
	if (error_task != NULL) {
		ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX,
			"\r\nNPU error task info:\r\n"
			"[Name]: %s, [ModelId]: %u, [StreamId]: %u, [TaskId]: %u, "
			"[RtErrCode]: 0x%x, [TaskType]: %u, [BlockDim]: %u, [Priority]: %u\r\n",
			error_task->func_name, error_task->model_id, error_task->strm_id, error_task->task_id,
			error_task->complete_err, error_task->task_type, error_task->block_dim, error_task->priority);
		cond_return_void(ret < 0, "snprintf_s is fail, ret = %d\n", ret);
		npu_mntn_copy_reg_to_bbox(log_buf, strlen(log_buf));
	}
}
