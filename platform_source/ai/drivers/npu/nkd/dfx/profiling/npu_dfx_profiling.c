...
static struct profiling_device g_prof_char_dev;
static struct profiling_dev_ctx g_profiling_dev_ctx = {PROF_PROC_CTX_PID_UNSET};
static const char *char_driver_name = "prof_drv";
...
static inline u32 npu_prof_get_ring_buff_size(struct profiling_ring_buff_manager *ring_buff,
	u32 circle_len)
{
...
int npu_prof_move_ring_buff_to_user(struct profiling_ring_buff_manager *ring_buff,
	u32 circle_len, uint64_t src_addr, uint64_t dest)
{
	u32 tmp_len = 0;
	unsigned long ret = 0;

	if (src_addr == 0 || ring_buff == NULL || dest == 0 ||
		circle_len == 0) {
		npu_drv_err("addr is null. src_addr:0x%pK, ring_buff:0x%pK, dest:0x%pK, circle_len:%u\n",
	...
	if (ring_buff->read < ring_buff->write) {
		...
		ret = copy_to_user((void __user *)(uintptr_t)dest,
			(const void *)(uintptr_t)(src_addr + ring_buff->read), tmp_len);
		if (ret > 0) {
			npu_drv_err("copy_to_user fail, src_addr:0x%pK, dest:0x%pK, write:0x%x, read:0x%x\n",
		...
		if (ring_buff->write > 0) {
			ret = copy_to_user((void __user *)(uintptr_t)(dest + tmp_len),
				(const void *)(uintptr_t)src_addr, ring_buff->write);
			if (ret > 0) {
				npu_drv_err("copy_to_user fail, src_addr:0x%pK, dest:0x%pK, write:0x%x, read:0x%x\n",
				(u64)(uintptr_t)src_addr, (u64)(uintptr_t)dest, ring_buff->write,
				ring_buff->read);
...
int npu_prof_register_cdev(void)
{
	...
	unsigned int major;
	dev_t devno;
	struct profiling_device *priv = &g_prof_char_dev;
	...
	priv->device = device_create(priv->dev_class, NULL, devno, NULL, "%s",
		char_driver_name);
...
void profiling_clear_mem_ptr(void)
{
	...
	cond_return_void(g_prof_buff == NULL, "prof buf nullptr\n");

	for (i = 0; i < PROF_CHANNEL_MAX; i++) {
		ring_manager = &(g_prof_buff->head.manager.ring_buff[i]);
