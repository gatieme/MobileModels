...
uint64_t get_prof_channel_data_addr(struct prof_buff_desc *prof_buff, u32 channel)
{
	...
	cond_return_error(channel >= PROF_TYPE_CNT, 0, "invalid channel\n");

#ifndef CONFIG_NPU_PHY_MEM
	data_addr = (vir_addr_t)prof_buff->head.manager.ring_buff[channel].base_addr_l +
	((vir_addr_t)prof_buff->head.manager.ring_buff[channel].base_addr_h << 32);
#else
	data_addr = prof_buff->head.manager.ring_buff[channel].vir_addr;
#endif
	return data_addr;
}
...
#else
	addr[PROF_CHANNEL_HWTS_LOG] = NPU_NS_LOG_BASE_ADDR;
	addr[PROF_CHANNEL_HWTS_PROFILING] = NPU_NS_PROF_BASE_ADDR;
	prof_buff->head.manager.ring_buff[PROF_CHANNEL_HWTS_LOG].vir_addr = (vir_addr_t)(uintptr_t)
		ioremap_wc((phys_addr_t)addr[PROF_CHANNEL_HWTS_LOG], PROF_HWTS_LOG_SIZE);
	prof_buff->head.manager.ring_buff[PROF_CHANNEL_HWTS_PROFILING].vir_addr = (vir_addr_t)(uintptr_t)
		ioremap_wc((phys_addr_t)addr[PROF_CHANNEL_HWTS_PROFILING], PROF_HWTS_PROFILING_SIZE);
#endif
