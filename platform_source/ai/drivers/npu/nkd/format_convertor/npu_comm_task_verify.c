...
typedef int (*verify_ts_task_func)(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task);

int verify_nonsink_task_comm_field(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
...
int verify_evt_rec_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	if (npu_proc_check_event_id(proc_ctx, ts_task->u.event_task.event_id) != 0) {
...
int verify_wait_evt_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	if (npu_proc_check_event_id(proc_ctx, ts_task->u.event_task.event_id) != 0) {
		npu_drv_err("invalid event_id:%u, task_id:%u\n",
...
int verify_memcpy_comm_task(struct npu_proc_ctx *proc_ctx,
...
int verify_maintenance_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
int verify_create_stream_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
int verify_model_maintenance_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	if (npu_proc_check_model_id(proc_ctx, ts_task->u.model_maintenance_task.model_id) != 0) {
...
int verify_model_execute_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	if (proc_ctx->dev_type == NPU) {
		if (npu_proc_check_model_id(proc_ctx, ts_task->u.model_execute_task.model_id) != 0) {
	...
	if (ts_task->u.model_execute_task.pid == 0)
		npu_drv_err("invalid model_execute_task.pid:%d, task_id:%u\n",
...
int verify_bypass_comm_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static const verify_ts_task_func verify_ts_sqe_map[] = {
	/* 0  1                  2                       3 */
...
int npu_verify_ts_sqe(struct npu_proc_ctx *proc_ctx, void *ts_task)
{
	...
	ret = verify_nonsink_task_comm_field(proc_ctx, ts_sqe);
	if (ret != 0) {
	...
	ret = verify_func(proc_ctx, ts_sqe);
	if (ret != 0) {
