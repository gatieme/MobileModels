...
static void npu_heart_beat_event_proc(struct npu_dev_ctx *dev_ctx)
{
	...
	cond_goto_error(sq == NULL, out, ret, -ENOMEM,
	...
	ktime_get_coarse_real_ts64(&sq->wall);
	ktime_get_raw_ts64(&now);
	sq->cntpct = __arch_counter_get_cntvct();
#else
	struct timespec wall = current_kernel_time();
	getrawmonotonic64(&now);
	sq->wall = timespec_to_timespec64(wall);
	sq->cntpct = arch_counter_get_cntvct();
#endif
...
out:
	dev_ctx->heart_beat.timer.expires = jiffies + dev_ctx->heart_beat.cycle * HZ;
	add_timer(&dev_ctx->heart_beat.timer);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static void npu_heart_beat_event(struct timer_list *timer)
{
	struct npu_dev_ctx* dev_ctx = get_dev_ctx_by_id(0);
	unused(timer);

	npu_heart_beat_event_proc(dev_ctx);
}
#else
	...
	void *data)
{
	...
	if (cq->u.exception_info.aicore_bitmap) {
		for (i = 0; i < dev_ctx->ai_core_num; i++) {
			if ((cq->u.exception_info.aicore_bitmap & (0x01U << i)) == 0) {
				core_count++;
				continue;
			}
			core_bitmap |= (0x01U << i);
			if ((dev_ctx->inuse.ai_core_error_bitmap & (0x01U << i)) == 0) {
				npu_drv_err("receive TS message ai core: %u exception\n", i);
				npu_rdr_exception_report(EXC_TYPE_TS_AICORE_EXCEPTION);
			}
		}
	}
...
int npu_heart_beat_is_exception(const u8 *cq_slot)
{
	...
	/* Since the TSCH side only reports RDR_TYPE_HWTS_BUS_ERROR, EXC_TYPE_TS_AICORE_EXCEPTION(0xc0000200),
	   EXC_TYPE_TS_AICORE_TIMEOUT(0xc0000201), EXC_TYPE_TS_AICORE_EFF_EXCEPTION(0xc0000240),
	   EXC_TYPE_TS_AICORE_EFF_TIMEOUT(0xc0000241), EXC_TYPE_TS_AIV_EXCEPTION(0xc0000280),
	   EXC_TYPE_TS_AIV_TIMEOUT(0xc0000281), and EXC_TYPE_HWTS_ERROR(0xc0000800~0xc00008ff),
	   it only needs to judge aicore and hwts exceptions, and other exceptions code will not be reported
	   here. */
	if ((u32)cq->exception_code >= EXC_TYPE_TS_AICORE_EXCEPTION &&
...
int npu_heart_beat_exception_process(const u8 *cq_slot, u32 size)
{
	...
	/* Since the TSCH side only reports RDR_TYPE_HWTS_BUS_ERROR, EXC_TYPE_TS_AICORE_EXCEPTION(0xc0000200),
	   ...
	   it only needs to judge aicore and hwts exceptions, and other exceptions code will not be reported
	   here. */
	cond_return_error(!(cq->exception_code >= EXC_TYPE_TS_AICORE_EXCEPTION &&
	...
	npu_rdr_taskerr_exception_report(cq->exception_code, &(cq->u.hwts_exception_info));
...
static void npu_heart_beat_callback(const u8 *cq_slot, const u8 *sq_slot)
{
	...
	if (cq->report_type != 0) {
		...
		timestamp.tv_usec = (u64)cq->exception_time.tv_nsec / 1000;
		...
		if ((unsigned int)cq->exception_code >= DMD_EXC_TYPE_EXCEPTION_START &&
			...
			if (tmp_dsm_client != NULL && (dsm_client_ocuppy(tmp_dsm_client) == 0)) {
				dsm_client_record(tmp_dsm_client, "npu ts exception\n");
				dsm_client_notify(tmp_dsm_client,
		...
		npu_rdr_exception_report(cq->exception_code);
	}

	npu_heart_beat_del_node(dev_ctx, cq);

	if ((cq->u.exception_info.ts_status != 0) ||
		(cq->u.exception_info.syspcie_sysdma_status != 0) ||
		(cq->u.exception_info.aicore_bitmap != 0)) {
		dev_ctx->heart_beat.exception_info = (u8 *)cq_slot;
		npu_heart_beat_ai_down(dev_ctx, dev_ctx->heart_beat.exception_info);
	}
}
	...
	u64 *cq_addr)
{
	...
	if (ret) {
		npu_drv_err("create functional cq failed\n");
		return -ENOMEM;
	}
	*cq_addr = cq_0_addr;
	return 0;
	...
	u64 sq_addr, u64 cq_addr)
{
	...
	if (ret != 0) {
		npu_drv_err("npu mailbox message send for res failed ret = %d\n",
			ret);
		return -ENOMEM;
	}
...
int npu_heart_beat_resource_init(struct npu_dev_ctx *dev_ctx)
{
	...
	dev_ctx->heart_beat.cycle = NPU_HEART_BEAT_CYCLE;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	timer_setup(&dev_ctx->heart_beat.timer, npu_heart_beat_event, 0);
#else
