...
static void npu_heart_beat_work(struct work_struct *work)
{
	...
	cond_goto_error(cq_info->tail >= NPU_MAX_DFX_CQ_DEPTH, fail,
		cq_info->tail, 0, "heart_beat cq_tail invalid, exceed max limit");
	...
	while (cq_slot[0] == cq_info->phase) {
		...
		if (npu_heart_beat_is_exception(cq_data) == 1) {
			...
			if (memcpy_s(cq_data_tmp, NPU_DFX_MAX_CQ_SLOT_LEN, cq_data,
				cq_info->slot_len - NPU_DFX_DETAILED_CQ_OFFSET) != 0) {
				npu_drv_err("memcpy_s fail");
				goto fail;
			}
			exception_flag = 1;
			break;
	...
	writel((u32)cq_info->tail, (void *)cq_info->doorbell);
	...
	u32 size)
{
	...
	cq_info->addr = ioremap(cq_info->phy_addr, buf_size);
#else
	...
	struct npu_dfx_create_cq_para *cq_para)
{
	...
	cond_goto_error(npu_get_dfx_cq_memory(&cq_info[cq_index], len), fail,
		ret, -ENOMEM, "get dfx cq memory failed\n");
...
irqreturn_t npu_dfx_cq_irq_handler(int irq, void *data)
{
	...
	list_for_each_safe(pos, n, &int_context->int_list_header) {
		...
		if (cq_info->tail >= NPU_MAX_DFX_CQ_DEPTH) {
			npu_drv_err("cq_tail:%u invalid, exceed limit\n", cq_info->tail);
			break;
		}
...
static void npu_dfx_work(struct work_struct *work)
{
	...
	struct npu_dfx_cqsq *cqsq = (struct npu_dfx_cqsq *)cq_info->dfx_cqsq_ptr;
	cond_return_void(cqsq == NULL, "dfx_cqsq_ptr is null\n");
	...
	cond_goto_error(cq_info->addr == NULL, fail, sq_index, 0);
	cond_goto_error(cq_info->tail >= NPU_MAX_DFX_CQ_DEPTH, fail,
		cq_info->tail, 0, "dfx cq_tail invalid, exceed max limit");
	...
	const struct npu_dev_ctx *cur_dev_ctx)
{
	...
	for (i = 0; i < NPU_MAX_DFX_SQ_NUM; i++) {
		...
		ret = npu_get_doorbell_vaddr(DOORBELL_RES_DFX_SQ, i,
			&doorbell_vaddr);
		if (ret) {
			npu_drv_err("get doorbell vaddr sq failed %u\n", i);
			break;
		}
...
static void npu_dfx_cq_destroy(struct npu_dfx_cq_info *cq_info, u32 lock_num)
{
	u32 i;
	for (i = 0; i < lock_num; i++)
		mutex_destroy(&cq_info[i].lock);
}
	...
	const struct npu_dev_ctx *cur_dev_ctx, struct npu_dfx_cqsq *cqsq)
{
	...
	for (i = 0; i < NPU_MAX_DFX_CQ_NUM; i++) {
		...
		ret = npu_get_doorbell_vaddr(DOORBELL_RES_DFX_CQ, i,
			&doorbell_vaddr);
		if (ret) {
			npu_dfx_cq_destroy(cq_info, i + 1);
			npu_drv_err("get doorbell vaddr cq failed %u\n", i);
			break;
		}
...
int npu_dfx_cqsq_init(struct npu_dev_ctx *cur_dev_ctx)
{
	...
	cqsq->int_context.wq = create_workqueue("npu-cqsq-work");
	...
	ret = request_irq(cqsq->int_context.irq_num, npu_dfx_cq_irq_handler,
		IRQF_TRIGGER_NONE, "npu-functional_cq", &cqsq->int_context);
...
void npu_destroy_dfx_cqsq(struct npu_dev_ctx *cur_dev_ctx)
{
	...
	cqsq = (struct npu_dfx_cqsq *)cur_dev_ctx->dfx_cqsq_addr;
	if (cqsq == NULL) {
		npu_drv_err("cqsq is null\n");
		return;
	}
	...
	if (cqsq->cq_info != NULL) {
		npu_dfx_cq_destroy(cqsq->cq_info, NPU_MAX_DFX_CQ_NUM);
		kfree(cqsq->cq_info);
	}
