	...
	u32 size, phys_addr_t *phy_addr, u8 **sq_addr, unsigned int *buf_size)
{
	...
	*sq_addr = ioremap(*phy_addr, buf_size_tmp);
#else
...
int npu_dfx_send_sq(u32 devid, u32 sq_index, const u8 *buffer, u32 buf_len)
{
	...
	cond_return_error(cur_dev_ctx->pm.ts_work_status == NPU_TS_DOWN, NPU_TS_DOWN,
	...
	credit = (int)((sq_info->tail >= sq_info->head) ?
	...
	writel(tail, (void *)sq_info[sq_index].doorbell);
