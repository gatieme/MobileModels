	...
	unsigned long *iova)
{
	...
	if (ret != 0) {
		npu_drv_err("iova alloc fail, ret=%d\n", ret);
		return ret;
	}
	ret = npu_iova_map(*virt_addr, iova);
	if (ret != 0) {
		npu_drv_err("iova map fail, ret=%d\n", ret);
		npu_iova_free(*virt_addr);
	}
#else
	*virt_addr = get_shm_virt_addr(0, NPU_SQ_MEM);
	*iova = get_shm_phy_addr(0, NPU_SQ_MEM);
#endif
	return ret;
}
...
int npu_hwts_sq_init(u8 dev_id)
{
	...
	if (ret != 0) {
		npu_drv_err("pool regist error! ret = %d", ret);
		return -1;
	}
#ifndef CONFIG_NPU_SWTS
...
int npu_free_hwts_sq(u8 dev_id, u32 hwts_sq_id)
{
	...
	ret = npu_free_entity(dev_id, hwts_sq_id >= NPU_MAX_LONG_HWTS_SQ_NUM ?
		...
		hwts_sq_id);
	if (ret != 0) {
		npu_drv_err("free entity error! hwts_sq_id = %u, ret = %d",
		hwts_sq_id, ret);
		return ret;
	}
