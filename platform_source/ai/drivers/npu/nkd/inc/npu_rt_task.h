...
typedef enum npu_task_type {
	...
	NPU_TASK_STREAM_CANCEL = 23, /* stream cancel task */
	NPU_TASK_CANCEL_INDICATE = 24, /* cancel indicate task */
	NPU_TASK_TYPE_KERNEL_AIV = 25, /*  */
...
typedef struct npu_rt_stream_cancel_task {
	u16 stream_id;
	u16 indicate_id;
	u8 reserved[44];
} npu_rt_stream_cancel_task_t;

typedef struct npu_rt_cancel_indicate_task {
	u8 reserved[48];
} npu_rt_cancel_indicate_task_t;
/* *
...
typedef struct npu_rt_task {
	...
	// u8 task_prof_en;
	struct {
		uint8_t sync_recv_enable : 1;
		uint8_t task_prof_enable : 1;
	} task_flag;
	u8 reserved[3];
	/* 48 bytes */
	union {
		...
		npu_rt_stream_cancel_task_t stream_cancel_task; // type 23
		npu_rt_cancel_indicate_task_t cancel_indicate_task; // type 24
