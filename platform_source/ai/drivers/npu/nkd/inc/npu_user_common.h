...
struct npu_ts_cq_info {
	u32 head;
	u32 tail;
	atomic_t count_report;
	u32 index;
	u32 phase;
	u32 stream_num;
	u32 communication_stream_num;
	u8 slot_size;
	u8 rsv[35];
};
...
typedef struct npu_model_prio_info {
	u32 model_id;
	u32 priority;
} npu_model_prio_info_t;

typedef struct npu_model_profiling_info {
	u32 model_id;
	u32 profiling;
} npu_model_profiling_info_t;
...
typedef execute_info_t stop_info_t;

struct npu_execute_info {
	u32 uid;
	u32 model_id;
	u32 execute_times;
	u32 reboot_times;
...
struct npu_toughness_info {
	u32 model_cnt;
	u32 size;
	u32 is_all_data;
	struct npu_execute_info info[NPU_MAX_MODEL_ID];
};

struct npu_enable_info {
	u32 model_id;
	u8 flag;
};

#define INPUT_OUTPUT_MAX_NUM (5)
#define DUMP_NAME_MAX_LEN (64)
...
typedef struct mem_region_info {
	struct buffer_info weight_buffer;
	struct buffer_info feature_map_buffer;
	struct buffer_info input_buffers[INPUT_OUTPUT_MAX_NUM];
	struct buffer_info output_buffers[INPUT_OUTPUT_MAX_NUM];
};

struct npu_task_dump_info {
	char func_name[DUMP_NAME_MAX_LEN];
	uint16_t model_id;
	uint16_t strm_id;
	uint16_t task_id;
	uint16_t task_type;
	...
	uint16_t priority;
	uint32_t uid;
	char process_name[DUMP_NAME_MAX_LEN];
	char model_name[DUMP_NAME_MAX_LEN];
