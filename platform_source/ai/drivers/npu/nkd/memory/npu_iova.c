...
static DEFINE_MUTEX(sg_table_va_map_lock);
	...
	unsigned long long va, unsigned long size)
{
	...
	mutex_lock(&sg_table_va_map_lock);
	...
	mutex_unlock(&sg_table_va_map_lock);
	return 0;
}
	...
	unsigned long size)
{
	...
	pages = vmalloc(sizeof(struct page *) * (unsigned int)nents);
	...
	*virt_addr = (unsigned long long)(uintptr_t)vmap(pages, nents, VM_MAP,
	...
	if (*virt_addr) {
		// Ensure that any D-cache lines for the interval are invalidated. For KO feature, omit temp;
...
void npu_iova_free(unsigned long long virt_addr)
{
	...
	if (ret || table == NULL) {
		npu_drv_err(
			"virt_addr should be called by iova alloc\n");
		return;
	}
...
int npu_iova_map(unsigned long long virt_addr, unsigned long *iova)
{
	...
	if (ret != 0 || table == NULL) {
		npu_drv_err(
			"virt_addr should be called by iova_alloc\n");
		return -1;
	}

	sg = table->sgl;

	*iova = mm_iommu_map_sg(platform_info->p_ts_subsys_dev, sg, 0,
		&size);
	npu_drv_debug("iova 0x%lx, va 0x%llx\n", (uintptr_t)*iova,
		(uintptr_t)virt_addr);
	if (*iova == 0) {
		npu_drv_err("mm_iommu_map_sg fail\n");
		return -1;
	}
	return 0;
}
...
int npu_iova_unmap(unsigned long long virt_addr, unsigned long iova)
{
	...
	sg = table->sgl;
	// 1.free iova
	ret = mm_iommu_unmap_sg(platform_info->p_ts_subsys_dev,
		sg, iova);
...
int npu_iova_map_ion(int fd, int prot, unsigned long *iova)
{
	...
	npu_drv_info("iova map ion start\n");
	...
	*iova = kernel_iommu_map_dmabuf(platform_info->p_ts_subsys_dev,
		dmabuf, prot, &size);
	dma_buf_put(dmabuf);
	cond_return_error((*iova) == 0, -EINVAL, "kernel_iommu_map_dmabuf fail\n");
...
int npu_iova_unmap_ion(int fd, unsigned long iova)
{
	...
	npu_drv_debug("iova device ptr = %pK, dmabuf = %pK, fd = %d\n",
		platform_info->p_ts_subsys_dev, dmabuf, fd);
	ret = kernel_iommu_unmap_dmabuf(platform_info->p_ts_subsys_dev,
		dmabuf, iova);
	dma_buf_put(dmabuf);
	cond_return_error(ret != 0, -EINVAL, "kernel_iommu_unmap_dmabuf fail\n");
