	...
	struct npu_pool_manager *pool_manager)
{
	...
	npu_drv_info("pool regist to alloc pools:size=%lu * %u\n",
	...
	alloc_pool_func alloc_pool, free_pool_func free_pool)
{
	...
	cond_return_error(ret != 0, ret, "pool manager init id fail: %d\n",
		ret);

	// 2. init pool info table
	ret = npu_pool_manager_init_pool_info(pool_manager);
	if (ret != 0) {
		npu_drv_err("pool manager init pool info fail: %d\n", ret);
	...
	tag_npu_mem_pool_type type)
{
	...
	cond_goto_error(entity_info == NULL, out, entity_info, NULL,
		"entity_info is NULL\n");
	...
	cond_goto_error(npu_pool_manager_check_extend_pool(pool_manager,
		...
		"pool manager check extend pool fail:poolindex=%u\n",
		pool_index);
