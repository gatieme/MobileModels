...
EXPORT_SYMBOL(npu_calc_hwts_sq_info);
...
EXPORT_SYMBOL(npu_calc_hwts_cq_info);
...
EXPORT_SYMBOL(npu_calc_profiling_info);
...
int npu_shm_v200_init(u8 dev_id)
{
	...
	if (ret != 0) {
		npu_drv_err("init swap buff failed!");
...
static int npu_devmem_flush_cache(struct vm_area_struct *vma, unsigned long size,
	pid_t process_id)
{
	...
	cond_goto_error(pid_struct == NULL, pid_struct_get_fail,
		ret, -ENXIO, "pid_struct is null\n");

	tsk = get_pid_task(pid_struct, PIDTYPE_PID);
	cond_goto_error(tsk == NULL, task_get_fail,
		ret, -ENXIO, "get pid task failed\n");

	mm = get_task_mm(tsk);
	cond_goto_error(mm == NULL, mm_get_fail,
		ret, -ENXIO, "get mm is null\n");

	cond_goto_error(mm_svm_flush_cache(mm, vma->vm_start, size), flush_cache_fail,
		ret,-ENXIO, "flush cache fail\n");
...
int npu_devmem_swapin(struct vm_area_struct *vma, u64 devmem_base,
	unsigned long size, unsigned long align_size, pid_t process_id)
{
	...
	cond_return_error(npu_devmem_flush_cache(vma, size, process_id), -ENXIO);
	return 0;
}
...
int l2_mem_swapin(struct vm_area_struct *vma, pid_t process_id)
{
	u64 l2_base = 0;
	...
	err = npu_devmem_swapin(vma, l2_base, l2_len, l2_len, process_id);
	if (err) {
...
int l2_mem_swapout(struct vm_area_struct *vma, u8 dev_id, pid_t process_id)
{
	u64 l2_base = 0;
	unsigned long l2_len = 0;
	...
	err = npu_devmem_swapout(vma, pad_base, l2_len, l2_len, process_id);
	if (err) {
