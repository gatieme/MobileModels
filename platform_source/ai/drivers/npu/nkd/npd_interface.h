...
struct npd_registry {
	...
	int (*npd_query_sku_value)(enum npu_sku_var_e sv);
	int (*npd_send_mail)(void *mailbox, int mailbox_len, const void *msg, int msg_len);
	int (*npd_verify_sink_tasks)(struct npu_proc_ctx *proc_ctx, void *stream_buf_addr, u64 ts_sq_len);
...
extern struct npd_registry *g_npd; // NPD driver registry data
