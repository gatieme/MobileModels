...
static int npu_pm_check_workmode(u32 curr_work_mode, u32 workmode)
{
	if (workmode >= NPU_ISPNN_SEPARATED)
		return -1;
	if ((npu_bitmap_get(curr_work_mode, NPU_NONSEC) != 0) && (workmode == NPU_SEC))
		return -1;
	if ((npu_bitmap_get(curr_work_mode, NPU_SEC) != 0) && (workmode == NPU_NONSEC))
		return -1;
	...
	struct npu_dev_ctx *dev_ctx, u32 work_mode)
{
	...
	cond_return_error(
		dev_ctx->pm.power_stage == NPU_PM_UP, 0,
	...
	dev_ctx->pm.ts_work_status = (u32)(work_mode != NPU_SEC ? NPU_TS_WORK :
			NPU_TS_SEC_WORK);
	...
	struct npu_dev_ctx *dev_ctx)
{
	...
	if (dev_ctx->pm.power_stage == NPU_PM_DOWN) {
		npu_drv_warn("npu dev %d has already powerdown!\n", dev_ctx->devid);
		dev_ctx->pm.work_mode = 0;
		return 0;
	}

	if (npu_bitmap_get(dev_ctx->pm.work_mode, NPU_SEC) == 0) {
	...
	ret = plat_info->adapter.pm_ops.npu_power_down(dev_ctx,
		work_mode);
	if (ret != 0) {
...
static void npu_task_cnt_update(struct npu_dev_ctx *dev_ctx, u32 cnt, u32 flag)
{
	int ref_cnt;

	if (flag == OPS_ADD) {
		atomic_add((int)cnt, &dev_ctx->pm.task_ref_cnt);
		npu_drv_debug("add , task ref cnt = %d, cnt = %u\n",
			atomic_read(&dev_ctx->pm.task_ref_cnt), cnt);
	} else {
		ref_cnt = atomic_sub_return((int)cnt, &dev_ctx->pm.task_ref_cnt);
		npu_drv_debug("sub , task ref cnt = %d, cnt = %u\n",
			atomic_read(&dev_ctx->pm.task_ref_cnt), cnt);
		if (ref_cnt == 0)
			npu_drv_debug("idle timer add, ref_cnt = %u\n", cnt);
	}
}
	...
	struct npu_dev_ctx *dev_ctx, uint32_t work_mode)
{
	int ret;
	unused(work_mode);
	ret = npu_pm_enter_workmode(proc_ctx, dev_ctx, NPU_NONSEC);
	cond_return_error(ret != 0, ret, "enter workmode failed\n");
...
{
	return npu_pm_enter_workmode(proc_ctx, dev_ctx, wm_info->work_mode);
}
...
int npu_pm_proc_ffrt_enter_wm(struct npu_proc_ctx *proc_ctx, struct npu_dev_ctx *dev_ctx, u32 work_mode)
{
	unused(proc_ctx);
	unused(dev_ctx);
	unused(work_mode);
...
int npu_pm_proc_ffrt_exit_wm(struct npu_proc_ctx *proc_ctx, struct npu_dev_ctx *dev_ctx, u32 work_mode)
{
	unused(proc_ctx);
	unused(dev_ctx);
	unused(work_mode);
