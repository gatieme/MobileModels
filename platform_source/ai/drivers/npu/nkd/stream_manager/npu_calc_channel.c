...
int npu_send_alloc_stream_mailbox(u8 cur_dev_id, int stream_id, int cq_id)
{
	...
	ret = npu_create_alloc_stream_msg(cur_dev_id, stream_id,
		alloc_stream_msg);
	if (ret != 0) {
		npu_drv_err("alloc stream create_alloc_stream_msg failed ret = %d\n", ret);
	...
	u32 sq_id, u32 cq_id)
{
	...
	inform_ts = (cur_dev_ctx->pm.ts_work_status == NPU_TS_WORK) ? NPU_HAVE_TO_INFORM : NPU_NO_NEED_TO_INFORM;
...
int npu_free_non_sink_stream(u8 dev_id, int stream_id)
{
	...
	if (atomic_read(&cur_dev_ctx->pm.power_access) == 1)
		goto recycle;
#endif

#ifndef CONFIG_NPU_SWTS
	mutex_lock(&cur_dev_ctx->pm.npu_power_mutex);
	if (cur_dev_ctx->pm.ts_work_status == NPU_TS_WORK) {
		...
		if (npu_get_sq_ref_by_stream(dev_id, sq_id) <= 1)
			free_stream_msg->flag = 1;
