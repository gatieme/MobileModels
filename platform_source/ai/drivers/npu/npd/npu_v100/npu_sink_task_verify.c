...
static int verify_evt_rec_task(struct npu_proc_ctx *proc_ctx,
...
static int verify_wait_evt_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static int verify_kernel_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *rt_task)
{
	unused(proc_ctx);
...
static int verify_memcpy_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *rt_task)
{
	unused(proc_ctx);
	if (((rt_task->u.memcpy_task.src_addr & npu_bit_mask(4)) != 0) ||
		rt_task->u.memcpy_task.src_addr == 0) {
		npu_drv_err("invalid src_addr not 16B align or is 0, sdma task_id:%u\n",
			rt_task->task_id);
		return -1;
	}
	if (((rt_task->u.memcpy_task.dst_addr & npu_bit_mask(4)) != 0) ||
		rt_task->u.memcpy_task.dst_addr == 0) {
		npu_drv_err("invalid dst_addr not 16B align or is 0, sdma task_id:%u\n",
			rt_task->task_id);
		return -1;
	}
	if (((rt_task->u.memcpy_task.length & npu_bit_mask(4)) != 0) ||
...
static int verify_event_wait_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *rt_task)
{
	unused(proc_ctx);
	unused(rt_task);
...
static int verify_pctrace_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *rt_task)
{
	unused(proc_ctx);
	unused(rt_task);
...
static int verify_cteate_l2_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *rt_task)
{
	unused(proc_ctx);
	unused(rt_task);
...
static int verify_fusion_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
static int verify_maintenance_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
...
static int verify_create_stream_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static int verify_model_maintenance_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static int verify_model_execute_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static int verify_profiling_enable_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
static int verify_profiling_disable_task(struct npu_proc_ctx *proc_ctx,
	npu_rt_task_t *ts_task)
{
	unused(proc_ctx);
	unused(ts_task);
...
{
	u64 rt_task_num = ts_sq_len / NPU_RT_TASK_SIZE;
	...
	npu_drv_debug("stream_buf_addr:0x%pK, ts_sq_len:%llu, rt_task_num:%llu",
		(uintptr_t)stream_buf_addr, ts_sq_len, rt_task_num);
