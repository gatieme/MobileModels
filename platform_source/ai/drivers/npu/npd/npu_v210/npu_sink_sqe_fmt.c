...
void format_aiv_sqe(void *hwts_sqe_addr, npu_rt_task_t *hwts_task,
		npu_model_desc_info_t *model_desc_info)
{
#ifdef CONFIG_NPU_SUPPORT_AIV
	struct hwts_kernel_sqe *kernel_sqe = (struct hwts_kernel_sqe *)hwts_sqe_addr;
	unused(model_desc_info);

	kernel_sqe->type = NPU_HWTS_SQE_VECTOR_CORE;
	...
	kernel_sqe->kernel_credit = AIC_AIV_SQE_KERNEL_CREDIT;
	kernel_sqe->res2 = 0;
	...
	kernel_sqe->res6 = 0;
	kernel_sqe->aicore_stack_base = hwts_task->u.kernel_task.stack_buff_addr;

	npu_drv_debug("kernel_sqe= %pK, struct size= %lu, stream_id= %u, task_id= %u, "
		"pc_start= 0x%llx, param_base= 0x%llx, rd_cond= %u\n",
		kernel_sqe,
	...
	npu_model_desc_info_t *model_desc_info)
{
	...
	npu_drv_debug("kernel_sqe= %pK, struct size= %d, stream_id= %u, task_id= %u, "
		"pc_start= 0x%llx, param_base= 0x%llx, aicore_stack= 0x%llx, rd_cond= %u\n",
		kernel_sqe,
		sizeof(struct hwts_kernel_sqe),
		hwts_task->stream_id,
		hwts_task->task_id,
	...
	npu_model_desc_info_t *model_desc_info)
{
	...
	memcpy_sqe->post_p = 0;
	memcpy_sqe->wr_cqe = 0;
	memcpy_sqe->l1_lock = 0;
	memcpy_sqe->l1_unlock = 0;
...
int npu_hwts_query_sqe_info(u64 sqe)
{
	...
	npu_drv_warn("sqe.l1_lock = %u\n", sqe_head->l1_lock);
	npu_drv_warn("sqe.l1_unlock = %u\n", sqe_head->l1_unlock);
...
EXPORT_SYMBOL(npu_hwts_query_sqe_info);
