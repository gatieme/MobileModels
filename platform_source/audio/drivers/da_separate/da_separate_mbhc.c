...
void da_separate_mbhc_set_micbias(struct da_separate_mbhc *mbhc, bool enable)
{
	...
	if (priv->not_support_mbhc) {
		AUDIO_LOGI("not support mbhc\n");
		return;
	}
...
static void hs_plug_in_detect(struct mbhc_data *priv)
{
	...
	/* check state - plugin */
	if (irq_status_check(priv, IRQ_STAT_PLUG_IN) == 0) {
...
static void mbhc_support_init(const struct device_node *np,
	struct mbhc_data *priv)
{
	priv->not_support_mbhc = false;
	if (of_property_read_bool(np, "not_support_mbhc")) {
		priv->not_support_mbhc = true;
		AUDIO_LOGI("mbhc is not supported\n");
	}
}
	...
	struct mbhc_data *priv)
{
	...
	for (i = VOLTAGE_POLE3; i < VOLTAGE_NUM; i++) {
		if (of_property_read_u32(np, voltage_config[i].min, &val) == 0)
			priv->intervals[i].min = val;
		if (of_property_read_u32(np, voltage_config[i].max, &val) == 0)
						...
						const struct da_separate_mbhc_ops *codec_ops)
{
	...
	if (codec == NULL || mbhc == NULL || codec_ops == NULL) {
	...
	priv->codec = codec;
	mbhc_support_init(codec->dev->of_node, priv);
	if (priv->not_support_mbhc) {
		AUDIO_LOGI("not support mbhc\n");
		*mbhc = &priv->mbhc_pub;
		return 0;
	...
	ret = mbhc_priv_init(codec, priv, codec_ops);
	if (ret != 0)
