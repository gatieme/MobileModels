...
#define DTS_COMP_HIFIDSP_NAME "hisilicon,hifidsp"
...
#else
static atomic_t g_dsp_in_suspend;
static atomic_t g_dsp_in_saving;
#endif
...
{
	return g_misc_data.base_virt;
}

static unsigned int get_syscache_pid(unsigned int session)
{
	if (session == SYSCACHE_SESSION_AUDIO_BUFFER)
		return PID_AUDIO;

	if (session == SYSCACHE_SESSION_AUDIO_CACHE)
		return PID_HIFI;

	return PID_VOICE;
}

static int dsp_request_quota(unsigned int pid)
{
	return lb_request_quota(pid);
}

static int dsp_release_quota(unsigned int pid)
{
...
void dsp_reset_release_syscache(void)
{
	...
	for (i = 0; i < SYSCACHE_SESSION_CNT; i++) {
		if (!g_request_flag[i])
			continue;
		pid = get_syscache_pid(i);

		ret = dsp_release_quota(pid);
		if (ret != 0) {
...
static void send_audio_syscache_quota_ack(unsigned int session, unsigned int msg_type)
{
	uint32_t ret = 0;
	struct syscache_quota_ack_msg msg = { 0 };

	if (session != SYSCACHE_SESSION_AUDIO_CACHE)
		return;

	msg.msg_id = ID_AP_DSP_SYSCACHE_QUOTA_ACK;
	msg.session = session;
	msg.msg_type = msg_type;

	ret = mailbox_send_msg(MAILBOX_MAILCODE_ACPU_TO_HIFI_MISC, &msg, sizeof(msg));
	if (ret != 0)
		loge("send syscache quota ack msg error %u\n", ret);
}
	...
	const unsigned char *data, unsigned int len)
{
	...
	send_audio_syscache_quota_ack(msg->session, msg->msg_type);
...
const static struct msg_proc msg_proc_table[] = {
	...
	{ ID_DSP_AP_REPORT_HIEVENT_CMD, SOCDSP_OM_WORK_A2DP_OFFLOAD, socdsp_om_rev_data_handle },
#endif
	{ ID_HIFI_AP_SYSCACHE_QUOTA_CMD, 0, set_audio_syscache_quota },
};
...
void dsp_misc_msg_process(void *cmd)
{
	...
	size_t size = ARRAY_SIZE(msg_proc_table);
	...
	logd("cmd id is not in proc table: %u\n", cmd_id);
}
	...
	struct mb_queue *mail_handle, unsigned int mail_len)
{
	...
	if (cmd_para->cmd_type == HIFI_CHN_SYNC_CMD) {
		...
		logw("unknown msg come from dsp, type: %d\n", cmd_para->cmd_type);
	}
...
static long dsp_misc_ioctl(struct file *fd, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == HIFI_MISC_IOCTL_SYNCMSG) {
		...
		return g_misc_data.misc_proc.sync_msg_proc((uintptr_t)data32);
	}
	...
	struct dsp_misc_priv *priv)
{
	...
	if (of_property_read_string(pdev->dev.of_node, "platform-type", &platform_type) == 0) {
		if (strncmp(platform_type, "ASIC", strlen("ASIC")) == 0) {
			priv->platform_type = DSP_PLATFORM_ASIC;
			logi("platform type is ASIC\n");
		} else if (strncmp(platform_type, "FPGA", strlen("FPGA")) == 0) {
...
{
	uint32_t phy_data_size;

	phy_data_size = DSP_UNSEC_REGION_SIZE;
	g_misc_data.base_phy = (unsigned char *)DSP_UNSEC_BASE_ADDR;
	g_misc_data.base_virt = (unsigned char *)ioremap_wc(get_phy_addr(DSP_UNSEC_BASE_ADDR, PLT_HIFI_MEM),
		phy_data_size);
	if (!g_misc_data.base_virt) {
...
static int soc_dsp_misc_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		pr_err("dsp lpp init fail ret: %d\n", ret);
	...
	dsp_misc_mmap_probe(pdev, (uintptr_t)g_misc_data.base_phy);
