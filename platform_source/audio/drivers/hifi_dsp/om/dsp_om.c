...
/*lint -e548 */
void set_dsp_img_loaded(void)
{
	if (g_om_priv.dsp_loaded_indicate_addr == NULL) {
		loge("dsp img loaded indicate addr error\n");
		return;
	}
	...
	if (g_om_priv.dsp_loaded_sign > 0) {
		logw("dsp img loaded error: 0x%x\n", g_om_priv.dsp_loaded_sign);
		g_om_priv.dsp_loaded = false;
	} else {
		g_om_priv.dsp_loaded = true;
	}
}
...
bool is_dsp_img_loaded(void)
{
	if (!g_om_priv.dsp_loaded)
		logw("dsp img isn't loaded, errno is 0x%x\n", g_om_priv.dsp_loaded_sign);
...
void dsp_om_init(struct platform_device *pdev, unsigned char *unsec_virt_addr)
{
	...
	g_om_priv.dsp_timestamp_addr = (unsigned int *)ioremap(
		get_phy_addr(SYS_TIME_STAMP_REG, PLT_SCTRL_MEM), 0x4);
	if (!g_om_priv.dsp_timestamp_addr) {
