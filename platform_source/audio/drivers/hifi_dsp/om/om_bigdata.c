...
#define BIGDATA_A2DP_OFFLOAD_EVENTID 916200006
...
typedef struct {
	...
	unsigned short top_mic_rms;
	unsigned short right_mic_rms;
	unsigned short left_mic_rms;
	unsigned short head_mic_rms;
	unsigned short mic_blk_rate;
	unsigned short mic_err_rate;
...
static void socdsp_om_show_audio_detect_info(struct work_struct *work)
{
	...
	case SOCDSP_CPU_OM_DUMP_LOG_INFO:
		...
		snprintf(log_path, sizeof(log_path) - 1, "%s%s", PATH_ROOT, "running_trace/hifi_log/hifi_requset/");
...
#else
	logw("imonitor is not supported\n");
#endif
}

#ifdef CONFIG_LOG_EXCEPTION
static void vqm_fill_event_buffer(char *event_buf, size_t length, uint32_t msg_id, uint32_t size, void *buf)
{
	...
	uint32_t alarm_id = msg_id;
	int index;
	int ret;
	uint8_t *pch = (uint8_t *)buf;

	ret = snprintf_s(event_buf, length, length - 1, "adsp_chr={%04d%04d", (int)alarm_id, (int)data_size);
	if (ret < 0) {
		loge("fill alarm id error:%d", ret);
		return;
	}
	event_buf += ret;
	length -= ret;
	...
	for (index = 0; index < data_size && length > 0; index++) {
		ret = snprintf_s(event_buf, length, length - 1, "%c%c",
			(char)hex_to_ch[(*pch >> 4) & 0xf], (char)hex_to_ch[*pch & 0xf]);
		if (ret < 0) {
			loge("fill data fail:%x, ret=%d", *pch, ret);
			break;
		}
		event_buf += ret;
		length -= ret;
	...
	ret = snprintf_s(event_buf, length, length - 1, "}");
	if (ret < 0)
		loge("final fill fail:%x, ret=%d", *pch, ret);
}
#endif

static void voip_3a_bigdata_update_imonitor(unsigned int eventid, voip_3a_om *data)
{
#ifdef CONFIG_LOG_EXCEPTION
	struct imonitor_eventobj *voip_bigdata_obj = NULL;
	char voip_adsp_chr[1024] = {'0'};
	/* creat imonitor obj */
	voip_bigdata_obj = imonitor_create_eventobj(eventid);
	if (voip_bigdata_obj == NULL) {
		logw("imonitor obj create for handset or handfree fail\n");
	...
	logi("msg_type: %d, msg_id: %d, msg_len: %d", data->recv_msg_type, data->msg_id, data->msg_len);
	vqm_fill_event_buffer(voip_adsp_chr, sizeof(voip_adsp_chr), data->msg_id, data->msg_len, data->data);
	imonitor_set_param_string_v2(voip_bigdata_obj, "voip_adsp_chr", voip_adsp_chr);
	imonitor_send_event(voip_bigdata_obj);
	imonitor_destroy_eventobj(voip_bigdata_obj);
#else
	logw("imonitor is not supported\n");
...
#endif
		break;
	case SOCDSP_VOIP_INFO_MSG:
		if ((sizeof(voip_3a_om)) < data_len) {
			logw("unavailable data from socdsp, type: %u, len: %u\n",
				socdsp_msg_type, data_len);
			return;
		}
		voip_3a_bigdata_update_imonitor(BIGDATA_VOIP_BLKEVENTID, (voip_3a_om *)&data);
		break;
...
static void socdsp_om_auxhear_bigdata_handle(struct work_struct *work)
{
	...
	static struct timespec64 bigdata_time1;
	static struct timespec64 bigdata_time2;
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ktime_get_coarse_ts64(&bigdata_time2);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		ktime_get_coarse_ts64(&bigdata_time1);
#else
...
static int a2dp_offload_report_info(unsigned int event_id, struct a2dp_offload_report_info *info)
{
#ifdef CONFIG_LOG_EXCEPTION
	...
	logi("a2dp_offload_report_hievent ++\n");
	...
	if (event == NULL) {
		loge("a2dp_offload create hievent failed\n");
		return -1;
	}

	hiview_hievent_put_integral(event, "ErrType", 1);
	hiview_hievent_put_integral(event, "SubErrorCode", 0);

	for (i = 0; i < INT_INFO_BUT; i++)
		hiview_hievent_put_integral(event, A2DP_OFFLOAD_REPORT_STR[i], info->int_info[i]);

	for (i = INT_INFO_BUT; i < STR_INFO_BUT; i++)
		hiview_hievent_put_string(event, A2DP_OFFLOAD_REPORT_STR[i], info->str_info[i - INT_INFO_BUT]);

	ret = hiview_hievent_report(event);
	if (ret < 0)
		loge("a2dp_offload report hievent failed\n");
...
static bool a2dp_offload_need_report_info(struct a2dp_offload_event_info *info)
{
	if (info->clear_queue_cnt > A2DP_OFFLOAD_REPORT_POP_CNT)
		return true;

	return false;
}

static void convert_data_to_string(char *str, const uint32_t str_len,
	const uint32_t *data, const uint32_t data_len)
{
	int i;
	uint32_t len = 0;

	if (str == NULL || data == NULL) {
		loge("str or data is NULL\n");
		return;
	}

	for (i = 0; i < data_len; ++i)
		len += snprintf(str + len, str_len - len, "%d,", data[i]);

	return;
}

static int a2dp_offload_record_event_info(char *data, int data_len)
{
	struct a2dp_offload_event_info *info = NULL;
	struct a2dp_offload_report_info report_info = {0};
	int ret = 0;
	int i;

	if (data_len != (sizeof(struct a2dp_offload_event_info))) {
		logw("unavailable data from socdsp, data len: %u\n", data_len);
		return -1;
	}

	info = (struct a2dp_offload_event_info *)data;
	if (info == NULL) {
		logw("unavailable data from socdsp, data is NULL\n");
		return -1;
	}

	report_info.int_info[CODEC_TYPE] = info->codec_type;
	report_info.int_info[START_TIMESTAMP] = info->paly_start_timestamp;
	report_info.int_info[STOP_TIMESTAMP] = info->paly_stop_timestamp;
	report_info.int_info[BITRATE_CNT] = info->bitrate_modify_cnt;
	report_info.int_info[MTU_CNT] = info->mtu_modify_cnt;
	report_info.int_info[CLEAR_QUEUE_CNT] = info->clear_queue_cnt;
	convert_data_to_string((char *)&report_info.str_info[BITRATE_INFO - INT_INFO_BUT][0],
		A2DP_OFFLOAD_MAX_INFO_LEN - 1,
		info->bitrate, sizeof(info->bitrate) /sizeof(info->bitrate[0]));
	convert_data_to_string((char *)&report_info.str_info[MTU_INFO - INT_INFO_BUT][0],
		A2DP_OFFLOAD_MAX_INFO_LEN - 1,
		info->mtu, sizeof(info->mtu)/sizeof(info->mtu[0]));
	convert_data_to_string((char *)&report_info.str_info[CLEAR_QUEUE_INFO - INT_INFO_BUT][0],
		A2DP_OFFLOAD_MAX_INFO_LEN - 1,
		info->clear_queue_timestamp, sizeof(info->clear_queue_timestamp) / sizeof(info->clear_queue_timestamp[0]));

	for (i = 0; i < INT_INFO_BUT; i++)
		logi("%s %d\n", A2DP_OFFLOAD_REPORT_STR[i], report_info.int_info[i]);

	for (i = INT_INFO_BUT; i < STR_INFO_BUT; i++)
		logi("%s %s\n", A2DP_OFFLOAD_REPORT_STR[i], report_info.str_info[i - INT_INFO_BUT]);

	if (a2dp_offload_need_report_info(info))
		ret = a2dp_offload_report_info(BIGDATA_A2DP_OFFLOAD_EVENTID, &report_info);

	return ret;
}

static void socdsp_om_work_a2dp_offload_handle(struct work_struct *work)
{
	unsigned char data[MAIL_LEN_MAX] = {'\0'};
	unsigned int data_len = 0;
	int ret = 0;

	get_workqueue_data(SOCDSP_OM_WORK_A2DP_OFFLOAD, data, &data_len);

	ret = a2dp_offload_record_event_info(data, data_len);
	if (ret)
		logw("record a2dp offload event fail\n");
}
...
static struct socdsp_om_work_info work_info[SOCDSP_OM_WORK_MAX] = {
	...
	{ SOCDSP_OM_WORK_A2DP_OFFLOAD, "hifi_om_work_a2dp_offload",
		socdsp_om_work_a2dp_offload_handle, {0} },
};
