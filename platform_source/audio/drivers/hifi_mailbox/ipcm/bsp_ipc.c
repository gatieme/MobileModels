...
#ifdef CONFIG_HIFI_IPC_EXTEND_MAILBOX
#define SYS_IPC_CORE_LIT 0
#define SYS_IPC_CORE_BIG 0
#define SYS_IPC_CORE_HIFI 3
#define MAILBOX_SEND_NUM 32
#else
#define SYS_IPC_CORE_LIT 0
...
static int audio_hifi_mbox_init(void)
{
	...
	ret = RPROC_MONITOR_REGISTER(g_ipc_config[IPC_CORE_IS_RECEIVE].rproc_id, &g_rx_nb);
	if (ret)
...
{
	RPROC_MONITOR_UNREGISTER(g_ipc_config[IPC_CORE_IS_RECEIVE].rproc_id, &g_rx_nb);
}
...
int ipc_int_send(enum ipc_int_core dst_core, enum ipc_int_lev level)
{
	...
	if (dst_core == IPC_CORE_SOCDSP) {
		ipc_msg[0] = (source << 24) | (level << 8); /* fill first int */

		ret = RPROC_ASYNC_SEND(g_ipc_config[role].rproc_id, (mbox_msg_t *)ipc_msg,
			IPC_MSG_LEN);
		if (ret != 0) {
