...
static int mailbox_init_mem(const struct mb_cfg *cfg)
{
	...
	if ((data_addr == 0) || (head == NULL) || (data_size == 0))
		return mb_logerro_p1(MB_E_GUT_INIT_CORESHARE_MEM, cfg->butt_id);
	...
	unsigned long data_addr)
{
	...
	if (msg_head->partition != MAILBOX_MSGHEAD_NUMBER)
		ret = mb_logerro_p1(MB_F_GUT_MSG_CHECK_FAIL, msg_head->mail_code);
...
static int mailbox_receive_all_mails(struct mb_buff *mbuf)
{
	...
	while (size_left > 0) {
		...
		if (mail_len == 0)
			ret = mb_logerro_p1(MB_F_GUT_READ_MAIL, mbuf->channel_id);
	...
	uint32_t use_sum)
{
	...
	if (use_sum != MAILBOX_USER_NUM)
		ret = (uint32_t)mb_logerro_p1(MB_E_GUT_INIT_USER_POOL_TOO_LARGE, use_sum);
...
{
	if (use_sum > MAILBOX_USER_NUM)
		return (uint32_t)mb_logerro_p1(MB_F_GUT_INIT_USER_POOL_TOO_SMALL, use_sum);
...
static void mailbox_destory_box(void)
{
...
static void mailbox_memory_config_init(uintptr_t *act_head_buttom, uintptr_t *head_buttom,
	uintptr_t *act_data_buttom, uintptr_t *data_buttom)
{
	*act_head_buttom = get_phy_addr(MAILBOX_MEM_BASEADDR, PLT_HIFI_MEM)
		+ MAILBOX_MEM_HEAD_LEN;
	*head_buttom = get_phy_addr(MAILBOX_HEAD_BOTTOM_ADDR, PLT_HIFI_MEM);
	*act_data_buttom = get_phy_addr(MAILBOX_MEM_BASEADDR, PLT_HIFI_MEM)
		+ MAILBOX_MEM_LENGTH;
	*data_buttom = get_phy_addr(MAILBOX_MEMORY_BOTTOM_ADDR, PLT_HIFI_MEM);
}

static int mailbox_check_memory_config(void)
{
#ifdef MAILBOX_OPEN_MNTN
	uintptr_t act_head_buttom, head_buttom, act_data_buttom, data_buttom;

	mailbox_memory_config_init(&act_head_buttom, &head_buttom, &act_data_buttom, &data_buttom);
...
{
	size_t hsize = sizeof(struct mb_mail);
	...
	uint32_t size)
{
	...
	size_t mail_size = sizeof(struct mb_mail);
...
int mailbox_sealup_buff(struct mb_buff *mb_buf, uint32_t usr_size)
{
	...
	if (mb_buf->seq_num == MAILBOX_SEQNUM_START)
		(void)mb_logerro_p1(MB_I_SEND_FIRST_MAIL, mb_buf->mailcode);
...
int mailbox_init(void)
{
	...
	g_slice_reg = (void *)ioremap(get_phy_addr(SYS_TIME_STAMP_REG, PLT_SCTRL_MEM),
		TIMER_REG_SIZE);
	if (g_slice_reg == NULL) {
