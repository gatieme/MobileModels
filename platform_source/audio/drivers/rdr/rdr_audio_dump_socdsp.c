...
#define MAX_NMI_COMPLTET_NUM 50
...
bool is_dsp_power_on(void)
{
	...
	power_status_addr = (unsigned int *)ioremap_wc(
		get_phy_addr(DRV_DSP_POWER_STATUS_ADDR, PLT_HIFI_MEM), 0x4);
...
static bool is_nmi_complete(void)
{
	...
	nmi_flag_addr = (unsigned int *)ioremap_wc(get_phy_addr(DRV_DSP_NMI_FLAG_ADDR, PLT_HIFI_MEM),
		0x4ul);
	if (!nmi_flag_addr) {
...
struct parse_log g_parse_socdsp_log[] = {
	...
	{ SOCDSP_ORIGINAL_CPUVIEW_SIZE + SOCDSP_ORIGINAL_INNERLOG_SIZE,
		SOCDSP_ORIGINAL_DYNMEM_SIZE, PARSE_MEM_DYN_LOG_SIZE, parse_socdsp_dynmem },
};
...
static int dump_socdsp_ddr(const char *filepath)
{
	...
	buf = (char *)ioremap_wc(get_phy_addr(RDR_DSP_DUMP_ADDR, PLT_HIFI_MEM),
		RDR_DSP_DUMP_SIZE);
	if (!buf) {
