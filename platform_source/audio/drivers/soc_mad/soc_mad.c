...
void soc_mad_select_din(unsigned int mode)
{
	...
	if (mode == LOW_FREQ_MODE)
		...
		if (mad_data.mad_mode == MAD_DMIC)
			writel(0x1, mad_data.reg_base_addr[MAD_REG] + MAD_MUX_SEL);
		else
...
void soc_mad_hpf_config(unsigned int mode, unsigned int hpf_cfg)
{
	unsigned int i;
	for (i = MAD_REG; i < REG_TYPE_NUM; i++) {
		if (!mad_data.reg_base_addr[i]) {
	...
	if (hpf_cfg >= HPF_CFG_INVALID || mode >= MODE_CNT) {
		AUDIO_LOGE("invalid para hpf_cfg: %d, mode:%d ", hpf_cfg, mode);
		return;
	}

	if (hpf_cfg == HPF_ENABLE) {
		if (mode == HIGH_FREQ_MODE) {
			writel(0x0, mad_data.reg_base_addr[MAD_REG] + ADC_CTRL);
			return;
		}

		writel(0x0, mad_data.reg_base_addr[MAD_REG] + HPF1_768K_CTRL);
		writel(0x0, mad_data.reg_base_addr[MAD_REG] + HPF2_768K_CTRL);
		writel(0x0, mad_data.reg_base_addr[MAD_REG] + HPF3_768K_CTRL);
		writel(0x0, mad_data.reg_base_addr[MAD_REG] + HPF4_768K_CTRL);
	} else {
		if (mode == HIGH_FREQ_MODE) {
			writel(0xFF, mad_data.reg_base_addr[MAD_REG] + ADC_CTRL);
			return;
		}

		writel(0x1, mad_data.reg_base_addr[MAD_REG] + HPF1_768K_CTRL);
		writel(0x1, mad_data.reg_base_addr[MAD_REG] + HPF2_768K_CTRL);
		writel(0x1, mad_data.reg_base_addr[MAD_REG] + HPF3_768K_CTRL);
		writel(0x1, mad_data.reg_base_addr[MAD_REG] + HPF4_768K_CTRL);
	}
}
...
static int mad_pinctrl_init(struct platform_device *pdev)
{
	...
	AUDIO_LOGI("soc mad pinctrl init ok");
