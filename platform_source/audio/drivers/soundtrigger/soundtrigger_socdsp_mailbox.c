...
static DECLARE_COMPLETION(start_msg_complete);
static DECLARE_COMPLETION(stop_msg_complete);
static DECLARE_COMPLETION(parameter_set_msg_complete);
static DECLARE_COMPLETION(parameter_get_msg_complete);
static DECLARE_COMPLETION(lp_start_msg_complete);
static DECLARE_COMPLETION(lp_stop_msg_complete);
static DECLARE_COMPLETION(lp_parameter_set_msg_complete);
static DECLARE_COMPLETION(lp_parameter_get_msg_complete);
static DECLARE_COMPLETION(lp_status_get_msg_complete);
...
#define ULL_BITS_NUM (sizeof(uint64_t) * BITS_OF_BYTE)

#define ull_bit(y) ((uint64_t)(0x1ULL << (y)))
#define set_ull_bit(x, y) ((x) |= (uint64_t)(0x1ULL << (y)))
#define clr_ull_bit(x, y) ((x) &= ~((uint64_t)(0x1ULL << (y))))

static int g_socdsp_handle;
static int g_socdsp_lp_handle;
static int g_lp_wakeup_status;
static char *g_model_buf;
static uint64_t wakeup_is_start;
static uint64_t wakeup_lp_is_start;

static void soundtrigger_lp_hotword_detected_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_LP_SOUNDTRIGGER_TRIGGER);
}
...
static void soundtrigger_elapsed_timeout_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_SOUNDTRIGGER_TIMEOUT);
}

static void soundtrigger_normal_hotword_detected_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_NORMAL_SOUNDTRIGGER_TRIGGER);
}

static void soundtrigger_wakeup_elapsed_timeout_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_SOUNDTRIGGER_NORMAL_TIMEOUT);
}

static DECLARE_WORK(hotword_lp_detected_queue, soundtrigger_lp_hotword_detected_work);
static DECLARE_WORK(set_mute_queue, soundtrigger_set_mute_work);
static DECLARE_WORK(elapsed_timeout_queue, soundtrigger_elapsed_timeout_work);
static DECLARE_WORK(hotword_normal_detected_queue, soundtrigger_normal_hotword_detected_work);
static DECLARE_WORK(wakeup_elapsed_timeout_queue, soundtrigger_wakeup_elapsed_timeout_work);
...
static void wakeup_hotword_ack_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	...
	schedule_work(&hotword_normal_detected_queue);
}
...
static void wakeup_lp_get_status_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	g_lp_wakeup_status = rcv_msg->socdsp_handle;
	if (msg_complete)
		complete(msg_complete);
}
...
static void wakeup_lp_hotword_ack_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	...
	schedule_work(&hotword_lp_detected_queue);
}
...
{
	schedule_work(&elapsed_timeout_queue);
}

static void wakeup_elapsed_timeout_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	schedule_work(&wakeup_elapsed_timeout_queue);
}
...
static struct msg_node soundtrigger_msg_table[] = {
	...
	{
		&lp_status_get_msg_complete,
		LP_WAKEUP_CHN_MSG_STATUS_GET,
		LP_WAKEUP_CHN_MSG_STATUS_GET_ACK,
		wakeup_lp_get_status_proc,
		"lp wakeup get status"
	},
...

	{
		NULL,
		0,
		WAKEUP_CHN_MSG_ELAPSED_TIMEOUT_ACK,
		wakeup_elapsed_timeout_proc,
		"normal wakeup elapsed timeout"
	}
};
...
static bool is_wakeup_start(uint32_t module_id, bool is_lp)
{
	if (module_id >= ULL_BITS_NUM)
		return false;

	if (is_lp)
		return ull_bit(module_id) & wakeup_lp_is_start;
	else
		return ull_bit(module_id) & wakeup_is_start;
}

static void set_wakeup_start(uint32_t module_id, bool is_lp, bool value)
{
	if (module_id >= ULL_BITS_NUM)
		return;

	if (is_lp)
		value ? set_ull_bit(wakeup_lp_is_start, module_id) : clr_ull_bit(wakeup_lp_is_start, module_id);
	else
		value ? set_ull_bit(wakeup_is_start, module_id) : clr_ull_bit(wakeup_is_start, module_id);

	AUDIO_LOGI("set wakeup start flag, is_lp: %d, start_flag: 0x%llx",
		is_lp, is_lp ? wakeup_lp_is_start : wakeup_is_start);
}
...
int start_recognition_msg(uint16_t msg_type, int module_id, bool is_lp)
{
	...
	AUDIO_LOGI("start_recognition_msg, is lp: %d, module_id: %d", is_lp, module_id);
	...
	set_wakeup_start(module_id, is_lp, true);
...
int stop_recognition_msg(uint16_t msg_type, int module_id, bool is_lp)
{
	...
	if (!is_wakeup_start(module_id, is_lp)) {
		AUDIO_LOGI("recognition not started, is lp: %d, module_id: %d", is_lp, module_id);
		return ret;
	}

	init_completion(stop_msg_node->msg_complete);

	AUDIO_LOGI("stop_recognition_msg, is lp: %d, module_id: %d", is_lp, module_id);
	...
	set_wakeup_start(module_id, is_lp, false);
...
int get_handle_msg(uint16_t msg_type, int *socdsp_handle)
{
	...
	AUDIO_LOGI("get_handle_msg, msg_type:%u", msg_type);
...
int soundtrigger_mailbox_init(void)
{
	...
	soundtrigger_rcv_wake_lock = wakeup_source_register(NULL, "soundtrigger_rcv_msg");
#else
	soundtrigger_rcv_wake_lock = wakeup_source_register("soundtrigger_rcv_msg");
#endif
	if (soundtrigger_rcv_wake_lock == NULL) {
		AUDIO_LOGE("request wakeup source failed");
		mailbox_reg_msg_cb(MAILBOX_MAILCODE_HIFI_TO_ACPU_WAKEUP, NULL, NULL);
		return -ENOMEM;
	}

	g_model_buf = ioremap_wc(get_phy_addr(AP_AUDIO_WAKEUP_MODEL_ADDR, PLT_HIFI_MEM),
		AP_AUDIO_WAKEUP_MODEL_SIZE);
	if (g_model_buf == NULL) {
		AUDIO_LOGE("model buffer ioremap err");
		wakeup_source_unregister(soundtrigger_rcv_wake_lock);
		soundtrigger_rcv_wake_lock = NULL;
		mailbox_reg_msg_cb(MAILBOX_MAILCODE_HIFI_TO_ACPU_WAKEUP, NULL, NULL);
		return -ENOMEM;
	}
