	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (state > 0) {
		da_separate_update_bits(codec, VOICE_R_DN_PGA_CTRL_REG,
...
#define CUSTOMER_KCONTROLS \
	...
	SOC_ENUM_EXT("AU_PA_PGA_CFG", au_pa_pga_cfg_enum[0], au_pa_pga_cfg_get, au_pa_pga_cfg_put), \
...
static const struct snd_kcontrol_new analog_au_en_mixer[] = {
	SOC_DAPM_SINGLE("ANA_AUL_EN", VIR0_ADDR, REG_BIT16, 1, 0),
	SOC_DAPM_SINGLE("ANA_AUR_EN", VIR0_ADDR, REG_BIT17, 1, 0),
	...
	SOC_DAPM_SINGLE("ANA_AU_ULTR_EN", VIR0_ADDR, REG_BIT30, 1, 0),
	SOC_DAPM_SINGLE("ANA_AU_MAD_EN", VIR1_ADDR, REG_BIT6, 1, 0),
};
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (priv->classh_clk_enable)
			da_separate_update_bits(codec, CODEC_CLK_EN3_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_DACL_EN_OFFSET) | BIT(SDM_L_CALT_VLD_OFFSET),
			BIT(AU_MST_DACL_EN_OFFSET) | BIT(SDM_L_CALT_VLD_OFFSET));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* src16 disable */
		...
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_DACL_EN_OFFSET) | BIT(SDM_L_CALT_VLD_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* au smt dacr enable */
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_DACR_EN_OFFSET) | BIT(SDM_R_CALT_VLD_OFFSET),
			BIT(AU_MST_DACR_EN_OFFSET) | BIT(SDM_R_CALT_VLD_OFFSET));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		...
		/* au smt dacr disable */
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_DACR_EN_OFFSET) | BIT(SDM_R_CALT_VLD_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		da_separate_update_bits(codec, CODEC_CLK_EN1_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (val_l == 0)
			da_separate_update_bits(codec, DACL_MIXER4_CTRL0_REG,
				BIT(DACL_MIXER4_IN3_MUTE_OFFSET),
				BIT(DACL_MIXER4_IN3_MUTE_OFFSET));

		if (val_r == 0)
			da_separate_update_bits(codec, DACR_MIXER4_CTRL0_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_ADC_MIC3_EN_OFFSET),
			BIT(AU_MST_ADC_MIC3_EN_OFFSET));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		da_separate_update_bits(codec, DAC_FILTER_CTRL_REG,
			BIT(AU_MST_ADC_MIC3_EN_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* codec ana/au en */
		da_separate_update_bits(codec, CLK_EN_CFG_REG,
			BIT(CODEC_ANA_EN_OFFSET) | BIT(AU_EN_OFFSET),
			BIT(CODEC_ANA_EN_OFFSET) | BIT(AU_EN_OFFSET));
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* codec ana/au dis */
		da_separate_update_bits(codec, CLK_EN_CFG_REG,
			BIT(CODEC_ANA_EN_OFFSET) | BIT(AU_EN_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		da_separate_update_bits(codec, CLASSD_MIN_TN_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		da_separate_update_bits(codec, CODEC_ANA_RW7_REG, MICBISA2_SET_MASK, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		da_separate_update_bits(codec, CODEC_ANA_RW7_REG, MICBISA2_SET_MASK, 0xff);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_write_adapter(codec, ANA_IRQ_REG0_REG, IRQ_MSK_COMP);
		da_separate_update_bits(codec, ANA_IRQM_REG0_REG, IRQ_MSK_BTN_NOR, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
...
#define AUDIO_INPUT_OUTPUT_WIDGET \
	...
	SND_SOC_DAPM_OUTPUT("ANA_AU OUTPUT"), \
...
#define AUDIO_MIXER_WIDGET \
	...
	SND_SOC_DAPM_MIXER_E("ANA_AU_EN MIXER", SND_SOC_NOPM, 0, 0, \
...
#define AUDIO_MUX_WIDGET \
	...
	SND_SOC_DAPM_MUX("ADC1 MUX", DAC_FILTER_CTRL_REG, AU_MST_ADCL_EN_OFFSET, 0, &adc1_mux), \
	SND_SOC_DAPM_MUX("ADC2 MUX", DAC_FILTER_CTRL_REG, AU_MST_ADCR_EN_OFFSET, 0, &adc2_mux), \
...
#define ASP_AUDIO_PLAY \
	...
	{ "AU_EN MIXER",          "AU_DACL_EN",   "AD_DA_LOOP MUX" }, \
	{ "AU_EN MIXER",          "AU_DACR_EN",   "AD_DA_LOOP MUX" }, \
	{ "AUDIO_DLINK OUTPUT",   NULL,           "AU_EN MIXER" }, \

#define PMU_AUDIO_PLAY_HEADPHONE \
	{ "ANA_AU_EN MIXER",     "ANA_AUL_EN",   "SMT_ANA_DAC_SDATA INPUT" }, \
	{ "ANA_AU_EN MIXER",     "ANA_AUR_EN",   "SMT_ANA_DAC_SDATA INPUT" }, \
	{ "ANA_DAC_L MUX",       "AUL",          "ANA_AU_EN MIXER" }, \
	{ "ANA_DAC_R MUX",       "AUR",          "ANA_AU_EN MIXER" }, \
...
#define ASP_AUDIO_CAP \
	{ "AU_EN MIXER",           "AU_ADCL_EN",   "ADC1 INPUT" }, \
	{ "AU_EN MIXER",           "AU_ADCR_EN",   "ADC2 INPUT" }, \
	{ "AU_EN MIXER",           "AU_ADC3_EN",   "ADC3 INPUT" }, \
	{ "DA_AD_LOOP MUX",        "NORMAL",       "AU_EN MIXER" }, \
	{ "ADC1 MUX",              "ADC1",         "DA_AD_LOOP MUX" }, \
	{ "ADC1 MUX",              "ADC3",         "DA_AD_LOOP MUX" }, \
	{ "ADC2 MUX",              "ADC2",         "DA_AD_LOOP MUX" }, \
	{ "ADC2 MUX",              "ADC3",         "AU_EN MIXER" }, \
...
#define PMU_AUDIO_CAP \
	...
	{ "ANA_AU_EN MIXER",   "ANA_MUX3_EN",  "ANA_MUX3 MUX" }, \
	{ "ANA_AU_EN MIXER",   "ANA_MUX4_EN",  "ANA_MUX4 MUX" }, \
	{ "ANA_AU_EN MIXER",   "ANA_MUX5_EN",  "ANA_MUX5 MUX" }, \
	{ "ANA_AU OUTPUT",     NULL,           "ANA_AU_EN MIXER" }, \
...
#define PLL \
	...
	{ "DAC_DL MIXER",         NULL,   "SIF_CLK" }, \
	{ "DAC_DR MIXER",         NULL,   "SIF_CLK" }, \
	{ "AUDIO_UL_FIFO SWITCH", NULL,   "SIF_CLK" }, \
	{ "AUDIO_UR_FIFO SWITCH", NULL,   "SIF_CLK" }, \
	{ "MIC3_UP_FIFO SWITCH",  NULL,   "SIF_CLK" }, \
	{ "ECHO_FIFO SWITCH",     NULL,   "SIF_CLK" }, \
	{ "AUDIO_CARKIT_SWITCH",  NULL,   "SIF_CLK" }, \
	{ "ANA_AU_EN MIXER",     NULL,   "SMT_IBIAS SUPPLY" }, \
	{ "ANA_SPK SWITCH",       NULL,   "CLASSD SUPPLY" }, \

#define ASP_OM \
	{ "AD_DA_LOOP MUX",  "AD_DA_LOOP",             "DACL_FILTER" }, \
	{ "AD_DA_LOOP MUX",  "AD_DA_LOOP",             "DACR_FILTER" }, \
	{ "DA_AD_LOOP MUX",  "DALR_ADLR_LOOP",         "AU_EN MIXER" }, \
	{ "DA_AD_LOOP MUX",  "DA_SOUT_AD_SIN_LOOP",    "AU_EN MIXER" }, \
	{ "DA_AD_LOOP MUX",  "DA_SOUT_MIC3_SIN_LOOP",  "AU_EN MIXER" }, \
...
#define PMU_OM \
	...
	{ "ANA_AU_EN MIXER",   NULL,            "DAC_LOOPBACK MUX" }, \
	{ "ANA_AU_EN MIXER",   NULL,            "DAC_LOOPBACK MUX" }, \
