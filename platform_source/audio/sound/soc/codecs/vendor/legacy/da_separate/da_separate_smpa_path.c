/*
 * da_separate_smpa_path.c codec driver.
 *
...
#define LOG_TAG "DA_separate_smpa"
...
{
	da_separate_update_bits_for_runtime_resume(codec, CLASSH_CTRL1_REG,
		BIT(CLASSH_BYPASS_OFFSET), BIT(CLASSH_BYPASS_OFFSET));
	da_separate_update_bits_for_runtime_resume(codec, CLASSH_CTRL1_REG,
		MASK_ON_BIT(DN_CFG_LEN, DN_CFG_OFFSET), 0x1);

	da_separate_reg_write_for_runtime_resume(codec, LUT_REG0_3_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG4_7_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG8_11_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG12_15_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG16_19_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG20_23_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG24_27_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG28_31_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG32_35_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG36_39_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG40_43_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG44_47_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG48_51_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG52_55_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG56_59_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG60_63_REG, LUT_REG_INIT);
	da_separate_reg_write_for_runtime_resume(codec, LUT_REG64_67_REG, LUT_REG_INIT);
}
...
#ifdef CONFIG_SND_SOC_CODEC_DA_SEPARATE_V5
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* enable i2s1 mode */
		da_separate_update_bits(codec, I2S1_TDM_CTRL1_REG,
			BIT(S1_I2S_TDM_MODE_OFFSET), 0);
		/* enable i2s1 tx */
		da_separate_update_bits(codec, I2S1_CTRL_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* enable tdm tx */
		da_separate_update_bits(codec, I2S1_TDM_CTRL0_REG,
...
static const struct snd_kcontrol_new dac_spa_switch[] = {
	SOC_DAPM_SINGLE("DAC_SPA_EN",
		DAC_FILTER_CTRL_REG, AU_SPA_DAC_EN_OFFSET, 1, 0),
};

#ifdef CONFIG_SND_SOC_CODEC_DA_SEPARATE_V5
static const struct snd_kcontrol_new smpa1_virtual_switch[] = {
	SOC_DAPM_SINGLE("AU_PA1_EN",
...
static const struct snd_soc_dapm_widget smpa_dapm_widgets[] = {
	SND_SOC_DAPM_INPUT("AU_PA_DLINK INPUT"),
	...
	SND_SOC_DAPM_PGA_E("AU_PA1_DACR_FILTER", SND_SOC_NOPM, 0, 0, NULL, 0, \
		smpa1_dacr_filter_event, (SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD)),
	SND_SOC_DAPM_SWITCH("AU_PA1 SWITCH", SND_SOC_NOPM, 0, 0, smpa1_virtual_switch),
	SND_SOC_DAPM_SWITCH("AU_PA2 SWITCH", SND_SOC_NOPM, 0, 0, smpa2_virtual_switch),
#endif
};

#define SMPA_ECHO_UPLINK \
	{ "AU_EN MIXER",         "AU_ADC_SPA_EN",   "ADC_SPA INPUT" }, \
	{ "ADC_SPAI_FILTER",     NULL,              "AU_EN MIXER" }, \
	{ "ADC_SPAV_FILTER",     NULL,              "AU_EN MIXER" }, \
...
#define SMPA_DOWNLINK \
	...
	{ "AU_EN MIXER",     "AU_DAC_SPA_EN",   "DAC_SPA SWITCH" }, \
	{ "DACL_SPA OUTPUT",  NULL,             "AU_EN MIXER" }, \

#define AUDIO_SMPA_DOWNLINK \
	{ "VOICE_DR_SMPA1 PGA",     NULL,             "AU_PA_DLINK INPUT" }, \
	{ "VOICE_DL_SMPA2 PGA",     NULL,             "AU_PA_DLINK INPUT" }, \
	{ "AU_PA2 SWITCH",          "AU_PA2_EN",      "VOICE_DL_SMPA2 PGA" }, \
	{ "DACL_SPA OUTPUT",        NULL,             "AU_PA2 SWITCH" }, \
	{ "DAC_SPA SWITCH",         "DAC_SPA_EN",     "VOICE_DR_SMPA1 PGA" }, \
	{ "AU_EN MIXER",            "AU_DAC_SPA_EN",  "DAC_SPA SWITCH" }, \
	{ "AU_PA1 SWITCH",          "AU_PA1_EN",      "AU_EN MIXER" }, \
	{ "AU_PA1_DACR_FILTER",     NULL,             "AU_PA1 SWITCH" }, \
	{ "DACR_SPA OUTPUT",        NULL,             "AU_PA1_DACR_FILTER" }, \
...
int add_smpa(struct snd_soc_component *codec)
{
	...
	return da_separate_add_codec_resource(codec, &smpa_resource);
}
