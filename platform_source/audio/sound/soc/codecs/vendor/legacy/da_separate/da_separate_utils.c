...
#define DBG_PMU_CLASSD_EN_START (PAGE_PMU_CTRL + CLASSD_EN_REG)
#define DBG_PMU_CLASSD_EN_END (PAGE_PMU_CTRL + CLASSD_EN_REG)
...
static void __iomem *remaped_sctrl_scperclken0 = NULL;
...
int da_separate_reg_addr_map(struct platform_device *pdev)
{
	...
	remaped_sctrl_scperclken0 = devm_ioremap(&pdev->dev,
		get_phy_addr(SOC_SCTRL_SCPERCLKEN0_ADDR(SOC_ACPU_SCTRL_BASE_ADDR), PLT_SCTRL_MEM),
		SOC_SCTRL_SCPERCLKEN0_ADDR_SIZE);
	if (!remaped_sctrl_scperclken0) {
		AUDIO_LOGE("sctrl addr ioremap failed");
		return -ENOMEM;
	}
#endif
	OUT_FUNCTION;
...
unsigned int da_separate_get_sctrl_scperclken0_data(void)
{
	return readl(remaped_sctrl_scperclken0);
}

void da_separate_reg_addr_unmap(struct platform_device *pdev)
{
	unsigned int i;
...
#ifdef SOC_SCTRL_SCPEREN0_gt_clk_mad_acpu_START
	if (remaped_sctrl_scperclken0) {
		devm_iounmap(&pdev->dev, remaped_sctrl_scperclken0);
		remaped_sctrl_scperclken0 = NULL;
	}
#endif
...
{
	bool is_valid = false;
	size_t reg_page_array_size = ARRAY_SIZE(reg_page_array);
	unsigned int i = 0;
...
{
	AUDIO_LOGD("get suspend usage_count: 0x%x child_count: 0x%x status: 0x%x disable_depth: %d"
			" asp_subsys_clk: %d, asp_codec_clk: %d",
			atomic_read(&(priv->codec->dev->power.usage_count)),
			atomic_read(&(priv->codec->dev->power.child_count)),
...
static unsigned int page_reg_read(struct platform_data *priv, unsigned int reg)
{
	...
	case PAGE_VIRCODEC:
		...
		break;
	default:
		AUDIO_LOGD("reg=0x%pK", (void *)(unsigned long)reg);
...
/* can only be used for runtime resume processes, others are prohibited from use. */
static unsigned int reg_read_for_runtime_resume(struct platform_data *priv, unsigned int reg)
{
	volatile unsigned int ret;
	unsigned int reg_type;
	unsigned int reg_value;
	...
	unsigned int value)
{
	...
	case PAGE_VIRCODEC:
		...
		break;
	default:
		AUDIO_LOGD("reg=0x%pK, value=0x%x", (void *)(unsigned long)reg, value);
		break;
	}
}
	...
	unsigned int value)
{
	...
	if (!_reg_value_valid(priv, reg_type, reg_value)) {
		AUDIO_LOGE("invalid reg: 0x%pK, value: %d", (void *)(uintptr_t)reg, value);
		runtime_put_sync(priv, reg_type);
		return;
	}
...
/* can only be used for runtime resume processes, others are prohibited from use. */
static void reg_write_for_runtime_resume(struct platform_data *priv, unsigned int reg,
	unsigned int value)
{
	unsigned int reg_type;
	unsigned int reg_value;
...
/* can only be used for runtime resume processes, others are prohibited from use. */
int da_separate_reg_write_for_runtime_resume(struct snd_soc_component *codec, unsigned int reg,
	unsigned int value)
{
	int ret = 0;
	struct platform_data *priv = get_private_data(codec);
	...
	reg_write_for_runtime_resume(priv, reg, value);
...
int da_separate_update_bits(struct snd_soc_component *component, unsigned int reg,
	unsigned int mask, unsigned int value)
{
	...
	old = snd_soc_component_read32(component, reg);
	new = (old & ~mask) | (value & mask);
	change = (old != new);
	if (change != 0)
		snd_soc_component_write_adapter(component, reg, new);

	return change;
}

/* can only be used for runtime resume processes, others are prohibited from use. */
int da_separate_update_bits_for_runtime_resume(struct snd_soc_component *component, unsigned int reg,
	unsigned int mask, unsigned int value)
{
	int change;
	unsigned int old;
	unsigned int new;
	struct platform_data *priv = get_private_data(component);
	if (priv == NULL) {
		AUDIO_LOGE("private data is null");
		return -EINVAL;
	}

	old = reg_read_for_runtime_resume(priv, reg);
	new = (old & ~mask) | (value & mask);
	change = (old != new);
	if (change != 0)
		reg_write_for_runtime_resume(priv, reg, new);
	...
	const struct reg_seq_config *reg_seq_array, size_t len)
{
	...
	for (i = 0; i < len; i++) {
		if (reg_seq_array[i].cfg.update_bits)
			da_separate_update_bits(codec, reg_seq_array[i].cfg.addr,
				reg_seq_array[i].cfg.mask, reg_seq_array[i].cfg.val);
		else
			 snd_soc_component_write_adapter(codec, reg_seq_array[i].cfg.addr,
