...
#include <linux/platform_drivers/drv_pmic_if.h>
...
#endif
#ifdef CONFIG_SND_SOC_DA_SEPARATE_FPGA
#include "da_separate_fpga.h"
#endif

#define LOG_TAG "DA_separate_v5"
...
#define FS_MIC3_UPLINK_MASK 0xE00000
#define FS_MIC3_UPLINK_VALUE 0x600000
...
struct da_separate_v5_customize {
...
static struct da_separate_v5_customize codec_customize;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((classd_voltage % CLASSD_VOLTAGE_DIV) != 0||
...
static const struct snd_kcontrol_new da_separate_v5_kcontrols[] = {
	CUSTOMER_KCONTROLS
};

static void hplr_pga_power_off(struct snd_soc_component *codec)
{
	IN_FUNCTION;

	snd_soc_component_write_adapter(codec, CODEC_ANA_RW5_REG, 0x40);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW5_REG, 0x4C);
	msleep(30);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW5_REG, 0x7C);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW19_REG, 0x8C);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW5_REG, 0xFC);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW24_REG, 0x36);
	snd_soc_component_write_adapter(codec, CODEC_ANA_RW20_REG, 0x00);

	OUT_FUNCTION;
}

static void ana_dac_power_off(struct snd_soc_component *codec)
{
	IN_FUNCTION;

	/* close dac */
	da_separate_update_bits(codec, DAC_CHAN_CTRL_REG,
		BIT(DACL_DWA_EN_OFFSET), 0);
	da_separate_update_bits(codec, CODEC_ANA_RW4_REG,
	...
	/* close  daclr-d */
	da_separate_update_bits(codec, CLK_EN_CFG_REG, ANA_DACLR_EN_MASK, 0x0);

	OUT_FUNCTION;
}

static void headphone_pop_off(struct snd_soc_component *codec)
{
	IN_FUNCTION;
...
static void classd_pop_off(struct snd_soc_component *codec)
{
	IN_FUNCTION;
	...
	da_separate_update_bits(codec, CTRL_REGA_CLASSD_ONOFF_REG,
		BIT(CTRL_REGA_REG_CLASSD_EN_OFFSET), BIT(CTRL_REGA_REG_CLASSD_EN_OFFSET));

	da_separate_update_bits(codec, CTRL_REGB_CLASSD_CTRL0_REG,
		BIT(CTRL_REGB_CLASSD_DRV_EN_OFFSET), BIT(CTRL_REGB_CLASSD_DRV_EN_OFFSET));

	OUT_FUNCTION;
}

static void set_power_codec(struct snd_soc_component *codec, int on)
{
	IN_FUNCTION;

	if (on) {
		da_separate_update_bits(codec, CODEC_ANA_RW10_REG,
			BIT(DISCHARGE_DISABLE_OFFSET), BIT(DISCHARGE_DISABLE_OFFSET));
		da_separate_update_bits(codec, CODEC_ANA_RW12_REG,
			BIT(IDEL_TONE_CTRL_OFFSET), BIT(IDEL_TONE_CTRL_OFFSET));
		da_separate_update_bits(codec, CODEC_ANA_RW3_REG,
		...
		da_separate_update_bits(codec, CODEC_ANA_RW22_REG,
			BIT(STB_CTRL_SEC_OFFSET) | BIT(STB_CTRL_OFFSET),
			BIT(STB_CTRL_SEC_OFFSET) | BIT(STB_CTRL_OFFSET));
		da_separate_update_bits(codec, CODEC_ANA_RW23_REG,
			BIT(STB_1N_OFFSET) | BIT(STB_N12DB_GAIN_OFFSET),
			BIT(STB_1N_OFFSET) | BIT(STB_N12DB_GAIN_OFFSET));
		snd_soc_component_write_adapter(codec, CODEC_ANA_RW1_REG, 0x07);
		msleep(6);
		snd_soc_component_write_adapter(codec, CODEC_ANA_RW1_REG, 0x03);
		snd_soc_component_write_adapter(codec, CODEC_ANA_RW1_REG, 0x01);
	} else {
		snd_soc_component_write_adapter(codec, CODEC_ANA_RW1_REG, 0x04);
	}

	OUT_FUNCTION;
}

static void set_ibias(struct snd_soc_component *codec, bool enable)
{
...
static void da_separate_v5_enable_hsd(struct snd_soc_component *codec)
{
	da_separate_update_bits(codec, CODEC_ANA_RW33_REG,
		BIT(PLUG_IN_DETECT_MODE_OFFSET), BIT(PLUG_IN_DETECT_MODE_OFFSET));
	da_separate_update_bits(codec, CODEC_ANA_RW8_REG,
		BIT(MBHD_COMP_HSD_VOLTAGE_PD_OFFSET) | BIT(MBHD_COMP_HSD_PD_OFFSET), 0);
}

static void da_separate_v5_set_ibias_hsmicbias(struct snd_soc_component *codec, bool enable)
{
...
static void reset_pmu_codec(struct snd_soc_component *codec)
{
	IN_FUNCTION;

	/* smartstar codec reset */
	snd_soc_component_write_adapter(codec, CTRL_REG_PMU_SOFT_RST_REG, 0x22);
	usleep_range(10000, 11000);
	snd_soc_component_write_adapter(codec, CTRL_REG_PMU_SOFT_RST_REG, 0x2F);
...
static void reset_asp_codec(struct snd_soc_component *codec)
{
	...
	da_separate_reg_write_for_runtime_resume(codec, R_RST_CTRLEN, BIT(RST_EN_CODEC_N));
	da_separate_reg_write_for_runtime_resume(codec, R_RST_CTRLDIS, BIT(RST_DISEN_CODEC_N));
	da_separate_reg_write_for_runtime_resume(codec, R_GATE_EN, BIT(GT_CODEC_CLK));
	da_separate_update_bits_for_runtime_resume(codec, R_CODEC_DMA_SEL,
		BIT(AUDIO_DMA_SEL_OFFSET) | BIT(VOICE_DMA_SEL_OFFSET) |
		BIT(ULTR_DMA_SEL_OFFSET) | BIT(MIC_DMA_SEL_OFFSET),
		BIT(AUDIO_DMA_SEL_OFFSET) | BIT(VOICE_DMA_SEL_OFFSET) |
		BIT(ULTR_DMA_SEL_OFFSET) | BIT(MIC_DMA_SEL_OFFSET));
...
static void asp_pmu_reg_init(void)
{
	...
	/* disable mixer */
	da_separate_update_bits_for_runtime_resume(soc_codec, DACL_MIXER4_CTRL0_REG,
	...
	da_separate_update_bits_for_runtime_resume(soc_codec, DACR_MIXER4_CTRL0_REG,
	...
	da_separate_update_bits_for_runtime_resume(soc_codec, CTRL_REGB_CLASSD_CTRL1_REG,
		BIT(CTRL_REGB_CLASSD_MUTE_SEL_OFFSET),
		BIT(CTRL_REGB_CLASSD_MUTE_SEL_OFFSET));

	OUT_FUNCTION;
}

static void classh_clk_enable_init(struct snd_soc_component *codec)
{
...
static void asp_soc_reg_init(void)
{
...
#if (defined(CONFIG_SND_SOC_CODEC_DA_SEPARATE_V6B)) || (defined(CONFIG_SND_SOC_CODEC_DA_SEPARATE_V6L))
	au_pa_reg_init(soc_codec);
#else
	smpa_asp_soc_reg_init(soc_codec);
#endif

	/* set i2s1 tdm mode fs */
	da_separate_update_bits_for_runtime_resume(soc_codec, FS_CTRL3_REG,
		I2S1_TDM_FS_MASK, I2S1_TDM_FS_MASK);

	/* set voice dl fs */
	da_separate_update_bits_for_runtime_resume(soc_codec, FS_CTRL0_REG,
		FS_VOICE_L_DLINK_MASK, FS_VOICE_L_DLINK_VALUE);

	/* set voice dr fs */
	da_separate_update_bits_for_runtime_resume(soc_codec, FS_CTRL2_REG,
		FS_VOICE_R_DLINK_MASK, FS_VOICE_R_DLINK_VALUE);

	/* set bt l rx pga gain */
	da_separate_update_bits_for_runtime_resume(soc_codec, BT_L_RX_PGA_CTRL_REG,
		BT_L_RX_PGA_GAIN_MASK, BT_L_RX_PGA_GAIN_VALUE);

	/* set bt r rx pga gain */
	da_separate_update_bits_for_runtime_resume(soc_codec, BT_R_RX_PGA_CTRL_REG,
		BT_R_RX_PGA_GAIN_MASK, BT_R_RX_PGA_GAIN_VALUE);

	/* set i2s1 tdm tx sel */
	da_separate_update_bits_for_runtime_resume(soc_codec, I2S1_TDM_CTRL0_REG,
	...
	if (codec_customize.support_3mic)
		da_separate_update_bits_for_runtime_resume(soc_codec, FS_CTRL0_REG,
			FS_MIC3_UPLINK_MASK, FS_MIC3_UPLINK_VALUE);
	OUT_FUNCTION;
}

static void da_separate_v5_asp_reg_init(struct snd_soc_component *codec)
{
	IN_FUNCTION;
...
static void hsd_cmp_force_close(struct snd_soc_component *codec)
{
	da_separate_update_bits(codec, CODEC_ANA_RW8_REG,
		BIT(MBHD_COMP_HSD_VOLTAGE_PD_OFFSET) | BIT(MBHD_COMP_HSD_PD_OFFSET),
		BIT(MBHD_COMP_HSD_VOLTAGE_PD_OFFSET) | BIT(MBHD_COMP_HSD_PD_OFFSET));
}

static void hsmicbias_force_close(struct snd_soc_component *codec)
{
	...
	da_separate_update_bits(codec, ANA_IRQM_REG0_REG,
	...
	/* hs micbias pd */
	da_separate_update_bits(codec, CODEC_ANA_RW7_REG,
...
static int audio_codec_register_au_pa(struct snd_soc_component *codec)
{
	int ret = 0;
	const char *au_pa = NULL;
	struct device_node *np = codec->dev->of_node;

	if (of_property_read_bool(np, "is_au_pa")) {
		ret = of_property_read_string(np, "is_au_pa", &au_pa);
		if (ret < 0) {
			AUDIO_LOGE("get is_au_pa failed");
			return ret;
		}
		if(strcmp(au_pa,"true") == 0)
			ret = au_pa_set_default_func(codec);
	}
	return ret;
}
...
void audio_codec_mute_pga(bool mute)
{
	...
	struct platform_data *priv = NULL;
	struct da_separate_v5_customize *customize = NULL;
	...
	if (mute) {
		...
		val = snd_soc_component_read32(soc_codec, DACL_MIXER4_CTRL0_REG);
		if (val != INVALID_REG_VALUE) {
			customize->mute_dacl_reg_val = val;
			da_separate_update_bits(soc_codec, DACL_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, MUTE_DAC_VALUE);
		} else {
			AUDIO_LOGE("read dacl reg fail");
		}

		val = snd_soc_component_read32(soc_codec, DACR_MIXER4_CTRL0_REG);
		if (val != INVALID_REG_VALUE) {
			customize->mute_dacr_reg_val = val;
			da_separate_update_bits(soc_codec, DACR_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, MUTE_DAC_VALUE);
		} else {
			AUDIO_LOGE("read dacl reg fail");
		}
		...
		if (customize->mute_dacl_reg_val != INVALID_REG_VALUE) {
			da_separate_update_bits(soc_codec, DACL_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, customize->mute_dacl_reg_val);
			customize->mute_dacl_reg_val = INVALID_REG_VALUE;
		}

		if (customize->mute_dacr_reg_val != INVALID_REG_VALUE) {
			da_separate_update_bits(soc_codec, DACR_MIXER4_CTRL0_REG, MUTE_DAC_VALUE_MASK, customize->mute_dacr_reg_val);
			customize->mute_dacr_reg_val = INVALID_REG_VALUE;
		}
		AUDIO_LOGI("unmute audio_dlr_mixer for flash driver");
	}
}
EXPORT_SYMBOL_GPL(audio_codec_mute_pga); //lint !e580

static void da_separate_v5_shutdown(struct platform_device *dev)
{
	AUDIO_LOGI("Begin");
...
static int asp_resource_init(struct snd_soc_component *codec)
{
	...
	if (IS_ERR_OR_NULL(priv->resource.asp_codec_clk)) {
		AUDIO_LOGE("get clk err: %pK", priv->resource.asp_codec_clk);
...
static void schg_set_classd_supply(struct snd_soc_component *codec, bool enable)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct da_separate_v5_customize *customize = priv->customized_data;
	...
	if (enable) {
		...
		if (regulator_is_enabled(regulator_schg) == 0) {
...
static void gpio_set_classd_supply(struct snd_soc_component *codec, bool enable)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static void da_separate_v5_set_classd_supply(struct snd_soc_component *codec, bool enable)
{
...
static int pmu_resource_init(struct snd_soc_component *codec)
{
	int ret;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct device_node *np = codec->dev->of_node;
...
#ifdef CONFIG_SND_SOC_DA_SEPARATE_FPGA
		if(!is_pmu_version_valid(codec)) {
			priv->pmu_invalid = true;
			return 0;
		}
#endif
	}
	...
	priv->resource.set_classd_supply = da_separate_v5_set_classd_supply;
...
static void pmu_resource_deinit(struct snd_soc_component *codec)
{
	int ret;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);

	if (priv->board_type == FPGA_BOARD && priv->pmu_invalid) {
		AUDIO_LOGI("pmu is invalid");
		return;
	}
...
static int pinctrl_init(struct snd_soc_component *codec)
{
	int ret;
	...
	/* I2S1 & I2S2 & AU & claasd bst_5v_en iomux config */
...
pinctrl_operation_err:
	devm_pinctrl_put(priv->resource.pin.pctrl);
	priv->resource.pin.pctrl = NULL;
...
static void pinctrl_deinit(struct snd_soc_component *codec)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static enum classd_supply_type get_classd_supply_type(struct snd_soc_component *codec)
{
...
static bool get_3mic_support(struct snd_soc_component *codec)
{
	const char *support_3mic = NULL;
	struct device_node *np = codec->dev->of_node;

	if (of_property_read_string(np, "is_support_3mic", &support_3mic)) {
		AUDIO_LOGE("get is_support_3mic failed");
		return false;
	}
	if (strcmp(support_3mic, "true") == 0) {
		AUDIO_LOGI("support 3mic");
		return true;
	}
	AUDIO_LOGI("do not support 3mic");
	return false;
}

static void classd_supply_init(struct snd_soc_component *codec)
{
	enum classd_supply_type supply_type = codec_customize.supply_type;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static void classd_supply_deinit(struct snd_soc_component *codec)
{
...
static void hac_gpio_init(struct snd_soc_component *codec)
{
	unsigned int val = 0;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct device_node *np = codec->dev->of_node;

	if (of_property_read_u32(np, "hac_gpio", &val) == 0) {
...
static void hac_gpio_deinit(struct snd_soc_component *codec)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static void pmu_codec_support_init(struct snd_soc_component *codec)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct device_node *np = codec->dev->of_node;

	priv->not_support_pmu_codec = false;
	if (of_property_read_bool(np, "not_support_pmu_codec")) {
		priv->not_support_pmu_codec = true;
		AUDIO_LOGI("pmu codec is not supported\n");
	}
}

static void pm_runtime_init(struct snd_soc_component *codec)
{
	struct device_node *np = codec->dev->of_node;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static void pm_runtime_deinit(struct snd_soc_component *codec)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static void cfg_au_driving_force(struct snd_soc_component *codec)
{
	...
	if (of_property_read_u32(np, "au_driving_force", &dts_cfg) == 0) {
		if (dts_cfg <= MAX_AU_DRIVING_FORCE)
			value = dts_cfg;
	}

	da_separate_update_bits(codec, CTRL_REG_DIG_IO_DS_CODEC_CFG_REG,
...
static int resource_init(struct snd_soc_component *codec)
{
	int ret;
	...
	if (ret) {
		AUDIO_LOGE("pum resource init error, ret = %d", ret);
		goto end;
	}
	...
	ret = da_separate_asp_resource_enable(codec);
	if (ret) {
	...
	hac_gpio_init(codec);
	pmu_codec_support_init(codec);
	pm_runtime_init(codec);
	cfg_au_driving_force(codec);
	reset_pmu_codec(codec);
	da_separate_v5_asp_reg_init(codec);
	AUDIO_LOGI("resource init ok");

	goto end;

pinctrl_init_err:
	da_separate_asp_resource_disable(codec);
...
static void resource_deinit(struct snd_soc_component *codec)
{
...
static void customized_data_init(struct da_separate_v5_customize *data)
{
	int ret;
	...
	codec_customize.support_3mic = get_3mic_support(soc_codec);

	ret = snd_soc_add_component_controls(soc_codec, da_separate_v5_kcontrols, ARRAY_SIZE(da_separate_v5_kcontrols));
	if (ret != 0)
		AUDIO_LOGE("add kcontrols failed, ret: %d", ret);
}

/* lint -e429 */
static int priv_data_init(struct snd_soc_component *codec)
{
	struct platform_data *priv = NULL;
	unsigned int i;
...
/* lint +e429 */
static void priv_data_deinit(struct snd_soc_component *codec)
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
...
static int da_separate_v5_codec_init(struct snd_soc_component *codec)
{
	int ret;
	...
	ret = audio_codec_register_au_pa(codec);
	if (ret) {
		AUDIO_LOGE("register au pa error, ret = %d\n", ret);
		goto register_au_pa_err;
	}
...
static void da_separate_v5_codec_deinit(struct snd_soc_component *codec)
{
	if (codec == NULL) {
		AUDIO_LOGE("parameter is null");
		return;
...
static struct da_separate_mbhc_ops da_separate_v5_mbhc_ops = {
	da_separate_v5_set_ibias_hsmicbias,
	da_separate_v5_enable_hsd,
};

static struct codec_ops da_separate_v5_codec_ops = {
	da_separate_v5_codec_init,
	da_separate_v5_codec_deinit,
	da_separate_v5_shutdown,
	da_separate_v5_asp_reg_init,
	&da_separate_v5_mbhc_ops,
};

static int __init da_separate_v5_codec_data_init(void)
{
	da_separate_codec_driver_init(&da_separate_v5_codec_ops);
...
arch_initcall(da_separate_v5_codec_data_init);
