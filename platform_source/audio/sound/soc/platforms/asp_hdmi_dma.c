	...
	enum asp_hdmi_interface type)
{
	...
	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		if (type == ASP_HDMI_I2S) {
			dmac_reg_set_bit(asp_hdmi_sio_ch_status1_l(io_ch),
				HDMI_SIO_CHANNEL_TYPE_BIT);
			dmac_reg_set_bit(asp_hdmi_sio_ch_status1_r(io_ch),
				HDMI_SIO_CHANNEL_TYPE_BIT);
		} else {
			dmac_reg_set_bit(asp_hdmi_spdif_ch_status1_l(io_ch),
				HDMI_SIO_CHANNEL_TYPE_BIT);
			dmac_reg_set_bit(asp_hdmi_spdif_ch_status1_r(io_ch),
				HDMI_SIO_CHANNEL_TYPE_BIT);
		}
	}
}
	...
	enum asp_hdmi_interface type)
{
	...
	if (sample_rate >= SAMPLE_RATE_NO_SUPPORT) {
		AUDIO_LOGE("sample rate %u is no support, and will use default io config",
			sample_rate);
		return;
	}

	if (type == ASP_HDMI_I2S)
		reg_value = dmac_reg_read(ASP_HDMI_SIO_CH0_STATUS1_L);
	else
		reg_value = dmac_reg_read(ASP_HDMI_SPDIF_CH0_STATUS1_L);

	reg_value &= (unsigned int)(~HDMI_SPDIF_SAMPLE_RATE_MASK);
	reg_value |= (g_asp_hdmi_sio_sample_rate[sample_rate][1]) << HDMI_SPDIF_SAMPLE_RATE_SHIFT;

	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		if (type == ASP_HDMI_I2S) {
			dmac_reg_write(asp_hdmi_sio_ch_status1_l(io_ch), reg_value);
			dmac_reg_write(asp_hdmi_sio_ch_status1_r(io_ch), reg_value);
		} else {
			dmac_reg_write(asp_hdmi_spdif_ch_status1_l(io_ch), reg_value);
			dmac_reg_write(asp_hdmi_spdif_ch_status1_r(io_ch), reg_value);
		}
	}
	...
	enum asp_hdmi_interface type)
{
	...
	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		if (type == ASP_HDMI_I2S) {
			dmac_reg_set_bit(asp_hdmi_sio_ch_status1_l(io_ch), HDMI_SIO_HDCP_BIT);
			dmac_reg_set_bit(asp_hdmi_sio_ch_status1_r(io_ch), HDMI_SIO_HDCP_BIT);
		} else {
			dmac_reg_set_bit(asp_hdmi_spdif_ch_status1_l(io_ch), HDMI_SIO_HDCP_BIT);
			dmac_reg_set_bit(asp_hdmi_spdif_ch_status1_r(io_ch), HDMI_SIO_HDCP_BIT);
		}
	}
	...
	enum asp_hdmi_interface type)
{
	...
	if (type == ASP_HDMI_I2S)
		reg_value = dmac_reg_read(ASP_HDMI_SIO_CH0_STATUS2_L);
	else
		reg_value = dmac_reg_read(ASP_HDMI_SPDIF_CH0_STATUS2_L);

	reg_value &= (unsigned int)(~HDMI_SPDIF_BITWIDTH_MASK);
	...
	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		if (type == ASP_HDMI_I2S) {
			dmac_reg_write(asp_hdmi_sio_ch_status2_l(io_ch), reg_value);
			dmac_reg_write(asp_hdmi_sio_ch_status2_r(io_ch), reg_value);
		} else {
			dmac_reg_write(asp_hdmi_spdif_ch_status2_l(io_ch), reg_value);
			dmac_reg_write(asp_hdmi_spdif_ch_status2_r(io_ch), reg_value);
		}
	}
	AUDIO_LOGI("set io bitwidth %u", bit_width);
}

static void i2s_io_config_dump(unsigned int channel_num)
{
	unsigned int io_ch;
	unsigned int i;
	unsigned int reg_value;
	...
	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		reg_value = dmac_reg_read(asp_hdmi_sio_ch_status1_l(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status1 l vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_sio_ch_status2_l(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status2 l vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_sio_ch_status1_r(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status1 r vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_sio_ch_status2_r(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status2 r vaule: 0x%x",
			io_ch, reg_value);
	}
}

static void spdif_io_config_dump(unsigned int channel_num)
{
	unsigned int io_ch;
	unsigned int i;
	unsigned int reg_value;
	...
	for (i = 0; i < channel_num; i += 2) {
		io_ch = i / 2;
		reg_value = dmac_reg_read(asp_hdmi_spdif_ch_status1_l(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status1 l vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_spdif_ch_status2_l(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status2 l vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_spdif_ch_status1_r(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status1 r vaule: 0x%x",
			io_ch, reg_value);
		reg_value = dmac_reg_read(asp_hdmi_spdif_ch_status2_r(io_ch));
		AUDIO_LOGI("set asp hdmi io ch %u status2 r vaule: 0x%x",
			io_ch, reg_value);
	}
}

static void io_config_dump(unsigned int channel_num, enum asp_hdmi_interface type)
{
	if (type == ASP_HDMI_I2S)
		i2s_io_config_dump(channel_num);
	else
		spdif_io_config_dump(channel_num);
}
	...
	enum asp_hdmi_interface type)
{
	...
	io_config_channel_type(channel_num, type);

	io_config_sample_rate(channel_num, parameters.sample_rate, type);

	io_config_copy_right(channel_num, parameters.is_hdcp, type);

	io_config_bit_width(channel_num, parameters.tx3_conf->bit_width, type);

	io_config_dump(channel_num, type);
}
...
int asp_hdmi_dma_start(enum asp_hdmi_interface type)
{
	/* dma enable */
	dmac_reg_write(ASP_HDMI_DMA_EN, HDMI_DMA_EN_MASK);
	...
	if (type == ASP_HDMI_I2S)
		dmac_reg_write(ASP_HDMI_I2S_SET, HDMI_I2S_SET_MASK); /* set i2s */
	else
		dmac_reg_write(ASP_HDMI_SPDIF_CTRL, HDMI_SPDIF_SET_MASK); /* set spdif */
...
MODULE_DESCRIPTION("ASP HDMI DMA Driver");
