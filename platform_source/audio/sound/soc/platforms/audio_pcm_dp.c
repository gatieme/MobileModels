...
static void playback_hw_available_monitor(struct snd_pcm_runtime *runtime)
{
	...
	if (avail < (int)rt_period_size)
		AUDIO_LOGI("there is no avail data avail[%ld] rt period size[%u]",
...
static void disable_regulator(struct audio_dp_data *pdata)
{
	if (regulator_bulk_disable(1, &pdata->regu))
...
err_set_hw_param:
	disable_hdmi();
	dp_clk_disable(pdata);
err_clk:
	free_irq(pdata->irq, pcm);
err_irq:
	disable_regulator(pdata);
	...
	struct snd_pcm_hw_params *params)
{
	...
	asp_hdmi_io_config(sio_parameters, prtd->pdata->asp_interface_type);
...
static int audio_pcm_dp_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			asp_cfg_dp_module_enable(prtd->pdata->asp_interface_type);
			asp_hdmi_tx3_enable();
			asp_hdmi_dma_start(prtd->pdata->asp_interface_type);
			spin_unlock_irqrestore(&g_substream_spinlock, flags);
		}
		break;
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			asp_hdmi_dma_stop();
			asp_hdmi_tx3_disable();
			asp_cfg_dp_module_disable(prtd->pdata->asp_interface_type);
			spin_unlock_irqrestore(&g_substream_spinlock, flags);
		}
		break;
	...
	struct vm_area_struct *vma)
{
	...
	ret = dma_mmap_wc(substream->pcm->card->dev,
		vma, runtime->dma_area, runtime->dma_addr, runtime->dma_bytes);

	return ret;
}

static int audio_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	buf->addr = get_phy_addr(AUDIO_DP_DATA_LOCATION, PLT_HIFI_MEM);
...
static int audio_component_pcm_dp_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
	return audio_pcm_dp_new(rtd);
}

static void audio_component_pcm_dp_free(struct snd_soc_component *component,
	struct snd_pcm *pcm)
{
	audio_pcm_dp_free(pcm);
}

static int audio_component_pcm_dp_open(struct snd_soc_component *component,
	struct snd_pcm_substream *substream)
{
	return audio_pcm_dp_open(substream);
}

static int audio_component_pcm_dp_close(struct snd_soc_component *component,
	struct snd_pcm_substream *substream)
{
	return audio_pcm_dp_close(substream);
}

static int audio_component_pcm_dp_ioctl(struct snd_soc_component *component,
	struct snd_pcm_substream *substream, unsigned int cmd, void *arg)
{
	return snd_pcm_lib_ioctl(substream, cmd, arg);
}

static int audio_component_pcm_dp_hw_params(struct snd_soc_component *component,
	struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	return audio_pcm_dp_hw_params(substream, params);
}

static int audio_component_pcm_dp_hw_free(struct snd_soc_component *component,
	struct snd_pcm_substream *substream)
{
	return audio_pcm_dp_hw_free(substream);
}

static int audio_component_pcm_dp_prepare(struct snd_soc_component *component,
	struct snd_pcm_substream *substream)
{
	return audio_pcm_dp_prepare(substream);
}

static int audio_component_pcm_dp_trigger(struct snd_soc_component *component,
	struct snd_pcm_substream *substream, int cmd)
{
	return audio_pcm_dp_trigger(substream, cmd);
}

static snd_pcm_uframes_t audio_component_pcm_dp_pointer(struct snd_soc_component *component,
	struct snd_pcm_substream *substream)
{
	return audio_pcm_dp_pointer(substream);
}

static int audio_component_pcm_dp_mmap(struct snd_soc_component *component,
	struct snd_pcm_substream *substream, struct vm_area_struct *vma)
{
	return audio_dp_normal_mmap(substream, vma);
}
#else
...
static struct snd_soc_component_driver g_audio_pcm_dp_platform = {
	...
	.pcm_construct = audio_component_pcm_dp_new,
	.pcm_destruct = audio_component_pcm_dp_free,
	.open = audio_component_pcm_dp_open,
	.close = audio_component_pcm_dp_close,
	.ioctl = audio_component_pcm_dp_ioctl,
	.hw_params = audio_component_pcm_dp_hw_params,
	.hw_free = audio_component_pcm_dp_hw_free,
	.prepare = audio_component_pcm_dp_prepare,
	.trigger = audio_component_pcm_dp_trigger,
	.pointer = audio_component_pcm_dp_pointer,
	.mmap = audio_component_pcm_dp_mmap,
	.idle_bias_on = true,
};
...
static int audio_pcm_dp_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(node, "asp_interface_type", &type) == 0)
		pdata->asp_interface_type = (enum asp_hdmi_interface)type;
	else
		pdata->asp_interface_type = ASP_HDMI_SPDIF;
	AUDIO_LOGI("asp_interface_type is %u", type);
