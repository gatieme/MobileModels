...
static uint32_t pcm_stream_status[PCM_DEVICE_MAX][PCM_STREAM_MAX];  /* pcm stream status 0:close 1:open */
static spinlock_t pcm_lock[PCM_DEVICE_MAX][PCM_STREAM_MAX];
struct snd_pcm_substream *g_substream[PCM_DEVICE_MAX][PCM_STREAM_MAX];
...
static void update_pcm_buff(struct snd_pcm_substream *substream, bool discard)
{
	...
	if (!discard) {
		...
		if (ret < 0)
			AUDIO_LOGW("mode is %d, device is %d: notify set buf fail, ret is %d",
...
static int pcm_thread_start(struct snd_pcm *pcm)
{
	...
	sched_cpus = *cpu_possible_mask;
	cpumask_clear_cpu(0, &sched_cpus);
	cpumask_clear_cpu(1, &sched_cpus);
	set_cpus_allowed_ptr(thread_info->pcm_run_thread, &sched_cpus);
...
static bool is_valid_device_mode(int device, int mode)
{
	if (priv_cust == NULL) {
		AUDIO_LOGE("cust is null");
		return false;
	...
	if ((mode != SNDRV_PCM_STREAM_PLAYBACK) && (mode != SNDRV_PCM_STREAM_CAPTURE)) {
		AUDIO_LOGE("pcm mode: %d is invalid", mode);
		return false;
	}
	...
	struct mb_queue *mail_handle, unsigned int mail_len)
{
	...
	pcm_device = mail_buf.pcm_device;
	pcm_mode = mail_buf.pcm_mode;

	if (!is_valid_device_mode(pcm_device, pcm_mode))
		return;

	spin_lock(&pcm_lock[pcm_device][pcm_mode]);
	substream = g_substream[pcm_device][pcm_mode];
	if (!is_valid_substream(substream)) {
		spin_unlock(&pcm_lock[pcm_device][pcm_mode]);
		AUDIO_LOGE("invalid sub stream");
		return;
	}

	if (pcm_stream_status[pcm_device][pcm_mode] == 0) {
		spin_unlock(&pcm_lock[pcm_device][pcm_mode]);
		AUDIO_LOGI("pcm stream has closed");
		return;
	}
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (!priv_cust->is_valid_pcm_device(device)) {
		AUDIO_LOGW("pcm device: %d is not support", device);
		return ret;
	}
...
static int audio_pcm_open(struct snd_pcm_substream *substream)
{
	...
	AUDIO_LOGI("pcm open, device %u, mode %u", pcm_device, pcm_mode);
	...
	spin_lock(&pcm_lock[pcm_device][pcm_mode]);
	pcm_stream_status[pcm_device][pcm_mode] = 1;
	g_substream[pcm_device][pcm_mode] = substream;
	spin_unlock(&pcm_lock[pcm_device][pcm_mode]);
...
static int audio_pcm_close(struct snd_pcm_substream *substream)
{
	...
	spin_lock(&pcm_lock[device][substream->stream]);
	pcm_stream_status[device][substream->stream] = 0;
	g_substream[device][substream->stream] = NULL;
	spin_unlock(&pcm_lock[device][substream->stream]);
...
static int preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	if (!priv_cust->is_valid_pcm_device(pcm->device) || (stream >= PCM_STREAM_MAX)) {
		AUDIO_LOGW("Invalid argument: device %d stream %d", pcm->device, stream);
		return -EINVAL;
	}
...
static int audio_pcm_platform_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < PCM_DEVICE_MAX; i++) {
		for (j = 0; j < PCM_STREAM_MAX; j++) {
			spin_lock_init(&pcm_lock[i][j]);
			pcm_stream_status[i][j] = 0;
		}
	}
