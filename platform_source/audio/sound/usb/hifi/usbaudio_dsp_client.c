...
static void print_audio_format(const struct audioformat *fp)
{
	...
	AUDIO_LOGW("begin");
	AUDIO_LOGW("formats %llu channels %u fmt_type %u frame_size %u iface %d altsetting %u altset_idx %u attributes %u endpoint %u ep_attr %u datainterval %u protocol %u maxpacksize %u clock %u",
		fp->formats,
		fp->channels,
		fp->fmt_type,
		fp->frame_size,
	...
	for (i = 0; i < fp->nr_rates; i++)
		AUDIO_LOGW("rate[%u]: %u", i, fp->rate_table[i]);
...
static bool is_need_lift(void)
{
	struct device_node *hw_audio_node = NULL;

	hw_audio_node = of_find_node_by_path("/audio_hw_config/hardware_info");
	if (hw_audio_node == NULL) {
		AUDIO_LOGI("%s: Cannot find hw audio node\n", __func__);
		return true;
	}
	if (of_property_read_bool(hw_audio_node, "support_new_ccc"))
		return false;
	AUDIO_LOGI("%s: support_new_ccc is false\n", __func__);
	return true;
}
...
static void set_customsized_headset_volume(struct usb_device *dev, u32 usb_id)
{
	...
	if (!is_need_lift()) {
		AUDIO_LOGI("the device does not require the proprietary volume control");
		return;
	}
	...
	struct usb_host_interface *ctrl_intf, int ctrlif, struct usbaudio_pcms *pcms)
{
	...
	if (is_usbaudio_device(dev) && !is_forced_match_arm_usbid(usb_id) &&
		...
		if (need_reset_vbus(usb_id)) {
			if (!usb_start_hifi_usb_reset_power(dev))
				return true;
		} else if (!usb_start_hifi_usb(dev)) {
			return true;
		} else {
	...
	struct usbaudio_pcms *pcms, unsigned int running, unsigned int rate)
{
	...
	if (ret < 0) {
		...
		if (ret == -ETIMEDOUT)
			usb_stop_hifi_usb_reset_power(chip->dev);
		return ret;
	}
