...
static DECLARE_COMPLETION(g_probe_msg_complete);
static DECLARE_COMPLETION(g_disconnect_msg_complete);
static DECLARE_COMPLETION(g_nv_check);
...
#define HIFI_USBAUDIO_DSP_LOAD_TIMEOUT (5 * HZ)
...
int usbaudio_nv_check(void)
{
	...
	retval = wait_dsp_load_completion_timeout(HIFI_USBAUDIO_DSP_LOAD_TIMEOUT);
	if (retval == 0) {
		AUDIO_LOGE("usbaudio dsp load check timeout");
		return -ETIME;
	} else {
		AUDIO_LOGI("usbaudio dsp load check intime");
	}
