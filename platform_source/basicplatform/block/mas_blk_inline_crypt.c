...
void mas_blk_inline_crypto_bio_split_post(struct bio *bio, unsigned int bi_size)
{
	if (!bio->ci_key)
		return;
	...
	bio->index += bi_size / 4096; /* ufs encrypto size */
}
#else
...
struct blk_inline_crypto_cookie {
	struct virtio_blk_cookie head;
	int	ci_key_len;
	u8	ci_key[MAS_BLK_CI_KEY_LEN];
	int	ci_key_index;
...
{
	struct blk_inline_crypto_cookie *cookie = (struct blk_inline_crypto_cookie *)buf;

	if (!bio->ci_key)
		return BLK_STS_OK;

	if (buf_len < sizeof(struct blk_inline_crypto_cookie)) {
		pr_err("%s: invalid para\n", __func__);
		return BLK_STS_IOERR;
	}

	if (bio->ci_key_len != MAS_BLK_CI_KEY_LEN && bio->ci_key_len != MAS_BLK_CI_KEY_LEN_2) {
		pr_err("%s: init key len invalid!\n", __func__);
		return BLK_STS_NOTSUPP;
	}

	cookie->head.type = VIRTIO_BLK_COOKIE_TYPE_INLINE_CRYPTO;
	cookie->head.len = sizeof(struct blk_inline_crypto_cookie);
	cookie->ci_key_len = bio->ci_key_len;
	cookie->ci_key_index = bio->ci_key_index;
	cookie->index = bio->index;
	(void)memcpy_s(cookie->ci_key, MAS_BLK_CI_KEY_LEN, bio->ci_key, (size_t)(bio->ci_key_len));
	(void)memcpy_s(cookie->ci_metadata, METADATA_BYTE_IN_KDF, bio->ci_metadata, METADATA_BYTE_IN_KDF);
	return BLK_STS_OK;
}
#endif
...
int blk_inline_crypto_cookie_dec(struct bio *bio, const char *buf, int buf_len)
{
	...
	bio->ci_key_len = cookie->ci_key_len;
	bio->ci_key_index = cookie->ci_key_index;
	bio->index = cookie->index;
	bio->ci_key = cookie->ci_key;
	bio->ci_metadata = cookie->ci_metadata;
