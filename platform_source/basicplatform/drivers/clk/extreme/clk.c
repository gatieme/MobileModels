...
static int media_online_check(const char *node)
{
...
{
	static int no_media1 = -1;
	static int no_media2 = -1;

	if ((no_media1 = -1) && (id == MEDIA1))
		no_media1 = media_online_check("no_media1");

	if ((no_media2 = -1) && (id == MEDIA2))
		no_media2 = media_online_check("no_media2");
...
void __iomem *hs_clk_base(u32 ctrl)
{
	...
	case HS_HSDT1CRG:
		...
		break;
	case HS_AONCRG:
		np = of_find_compatible_node(NULL, NULL, "hisilicon,awake_crg");
		break;
	case HS_LITECRG:
...
char *hs_base_addr_transfer(long unsigned int base_addr)
{
	...
	if (base_addr == (uintptr_t)hs_clk.crgctrl)
		...
		return "IOMCUCRG";
	else if (base_addr == (uintptr_t)hs_clk.litecrg)
		return "LITECRG";
	else if (base_addr == (uintptr_t)hs_clk.aoncrg)
		return "AONCRG";
	else
		return "NONE";
}
	...
	int nums, struct clock_data *data)
{
	...
	for (i = 0; i < nums; i++) {
		u32 shift = (u32)(ffs(clks[i].mux_mask) - 1);
		u32 width = (u32)(fls(clks[i].mux_mask) - ffs(clks[i].mux_mask) + 1);
