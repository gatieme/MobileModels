				...
				loff_t *ppos)
{
	...
	while (ret == 0) {
		...
		len = list->buffer[list->tail].len > (int)count ?
			(int)count : list->buffer[list->tail].len;
					...
					unsigned char report_type)
{
	...
	len = (ret < (int)count) ? ret : (int)count;
...
static unsigned int hw_dev_add_hidraw_table(struct hidraw *dev)
{
	unsigned int minor;
...
void hw_disconnect(struct hid_device *hid)
{
	...
	device_destroy(hw_class, MKDEV(hw_major, hidraw->minor));
				...
				unsigned long arg)
{
	...
	case HIDIOCGRDESCSIZE:
		if (put_user(dev->hid->rsize, (unsigned int __user *)(uintptr_t)arg))
			ret = -EFAULT;
		break;
	case HIDIOCGRDESC:
		if (get_user(len, (__u32 __user *)(uintptr_t)arg))
			ret = -EFAULT;
	...
	case HIDIOCGRAWINFO:
		...
		dinfo.vendor = (__s16)(dev->hid->vendor);
		dinfo.product = (__s16)(dev->hid->product);
