...
#define I2C_V300_IC_CON 0x0
#define I2C_V300_IC_TAR 0x4
#define I2C_V300_IC_ENABLE 0x6C
#define I2C_V300_IC_STATUS 0x70
#define I2C_V300_IC_TXFLR 0x74
#define I2C_V300_IC_RXFLR 0x78
...
#define I2C_V300_IC_TX_ABRT_SOURCE 0x80
#define I2C_V300_IC_DATA_CMD 0x10
#define I2C_V300_IC_ERR_TX_ABRT 0x1

#define DW_IC_TXDMAE (1 << 1) /* enable transmit dma */
#define DW_IC_RXDMAE (1 << 0) /* enable receive dma */

#define I2C_V300_IC_RD_CFG (1 << 8)
#define I2C_V300_IC_CON_RECOVERY_CFG 0x43
#define I2C_V300_IC_TAR_RECOVERY_CFG 0x7F

#define I2C_V300_STATUS_WRITE_IN_PROGRESS 0x1
...
#define I2C_V300_SEC_SLAVE_MAX_NUM 2
#define I2C_V300_DEFAULT_SEC_SLAVE 0x28
#define I2C_V300_DEFAULT_SEC_SLAVE_NUM 1
#define I2C_V300_READ_REG               0xc3001102
#define I2C_V300_WRITE_REG              0xc3001103
#define I2C_V300_SLAVE_WRITE_REG        0xc3001104
...
static u32 secure_dw_readl(u32 base_addr, u32 offset)
{
	...
	arm_smccc_1_1_smc(I2C_V300_READ_REG, base_addr, offset, &res);
	return (u32)res.a1;
}
...
static void secure_dw_writel(u32 base_addr, u32 offset, u32 b)
{
	...
	arm_smccc_1_1_smc(I2C_V300_WRITE_REG, base_addr, offset, b, &res);
}
...
{
	struct dw_i2c_controller *ctrl = dev->priv_data;
...
static void i2c_dw_dma_probe_initcall(struct dw_i2c_dev *dev)
{
	...
	tx_conf.dst_addr = controller->mapbase + I2C_V300_IC_DATA_CMD;
	tx_conf.dst_addr_width = DMA_SLAVE_BUSWIDTH_2_BYTES;
	tx_conf.direction = DMA_MEM_TO_DEV;
	tx_conf.dst_maxburst = I2C_DMA_MAX_BURST;

	rx_conf.src_addr = controller->mapbase + I2C_V300_IC_DATA_CMD;
...
static void i2c_dw_dma_tx_callback(void *data)
{
	...
	regmap_write(dev->map, DW_IC_DMA_CR, controller->dmacr & DW_IC_DMA_CR_MASK);
...
{
	regmap_write(dev->map, DW_IC_DMA_TDLR, dev->tx_fifo_depth - I2C_DMA_MAX_BURST);
	regmap_write(dev->map, DW_IC_DMA_RDLR, I2C_DMA_RDLR_BURST);
}
...
static void hck_i2c_dw_secure_read(struct dw_i2c_dev *dev,
	unsigned int reg, unsigned int *val, bool *err)
{
	if (dev->secure_mode) {
		*val = secure_dw_readl(dev->reg_base, reg);
		*err = true;
	} else {
		*err = false;
	}
...
static void hck_i2c_dw_secure_write(struct dw_i2c_dev *dev,
	unsigned int reg, unsigned int val, bool *err)
{
	if (dev->secure_mode) {
		secure_dw_writel(dev->reg_base, reg, val);
		*err = true;
	} else {
		*err = false;
	}
...
static void hck_i2c_dw_read_comp_type(struct dw_i2c_dev *dev,
	unsigned int *reg, bool *err)
{
	if (dev->secure_mode) {
		*reg = secure_dw_readl(dev->reg_base, DW_IC_COMP_TYPE);
		*err = true;
	} else {
		*err = false;
	}
...
static void hck_i2c_dw_dma_fifo_cfg(struct dw_i2c_dev *dev,
	bool *err)
{
	regmap_write(dev->map, DW_IC_TX_TL, 15);
	regmap_write(dev->map, DW_IC_RX_TL, 15);
	i2c_dw_dma_fifo_cfg(dev);
	regmap_write(dev->map, DW_IC_CON, dev->master_cfg);
	*err = true;
}

static void hck_i2c_dw_xfer_init_irq_mask(struct dw_i2c_dev *dev,
	bool *err)
{
	regmap_write(dev->map,
		DW_IC_INTR_MASK, DW_IC_INTR_TX_ABRT | DW_IC_INTR_STOP_DET);
	*err = true;
}

static void hck_i2c_dw_irq_mask_all(struct dw_i2c_dev *dev)
{
	regmap_write(dev->map, DW_IC_INTR_MASK, 0);
}

static void hck_i2c_dw_stop_irq_handler(struct dw_i2c_dev *dev,
	unsigned int stat)
{
	struct dw_i2c_controller *controller = NULL;
...
static int i2c_dw_xfer_msg_dma_check_ctrl(
	struct dw_i2c_dev *dev, struct dw_i2c_controller *controller)
{
	if (!controller) {
		dev_err(dev->dev, "%s:i2c ctrler doesn't be init\n", __func__);
		return -ENODEV;
...
static int i2c_dw_prepare_xfer(struct dw_i2c_dev *dev,
	struct dw_i2c_controller *controller, int tx_len)
{
	int ret;
	int rx_len = 0;
...
clr_err:
	if (ret < 0) {
		...
		dev->msg_write_idx = 0;
		dev->msg_read_idx = 0;
		dev->status &= ~I2C_V300_STATUS_WRITE_IN_PROGRESS;
 ...
 */
static void dw_i2c_prepare_recovery(struct i2c_adapter *adap)
{
	struct dw_i2c_dev *dev = i2c_get_adapdata(adap);
	int ret;

	ret = dw_i2c_pins_ctrl(dev, PINCTRL_STATE_IDLE);
	if (ret < 0)
		dev_warn(dev->dev, "pins are not configured to idle\n");
}

static void dw_i2c_unprepare_recovery(struct i2c_adapter *adap)
{
	struct dw_i2c_dev *dev = i2c_get_adapdata(adap);
	int ret;

	ret = dw_i2c_pins_ctrl(dev, PINCTRL_STATE_DEFAULT);
	if (ret < 0)
		dev_warn(dev->dev, "pins are not configured to default\n");
}

void dw_i2c_scl_recover_bus(struct i2c_adapter *adap)
{
	struct dw_i2c_dev *dev = i2c_get_adapdata(adap);
	...
	if (bri == NULL || bri->sda_gpiod == NULL) {
		dev_info(dev->dev, "not have gpio scl/sda property\n");
		return;
	}
	sda_gpio = desc_to_gpio(bri->sda_gpiod);
	ret = gpio_request_one(sda_gpio, GPIOF_IN, "i2c-sda");
	if (ret)
		dev_warn(dev->dev,
	...
	regmap_write(dev->map, I2C_V300_IC_ENABLE, 0);

	/* speed is 100KHz */
	regmap_write(dev->map, I2C_V300_IC_CON, I2C_V300_IC_CON_RECOVERY_CFG);

	/* config slave address to 0x7F */
	regmap_write(dev->map, I2C_V300_IC_TAR, I2C_V300_IC_TAR_RECOVERY_CFG);
	/* enable IC, 1:Enable */
	regmap_write(dev->map, I2C_V300_IC_ENABLE, 1);
	/* recived data from bus */
	regmap_write(dev->map, I2C_V300_IC_DATA_CMD, I2C_V300_IC_RD_CFG);
...
static void i2c_v300_prepare_xfer(struct dw_i2c_dev *dev,
	struct dw_i2c_controller *controller, struct i2c_msg msgs[], int num)
{
	int r;
...
static void i2c_v300_xfer_end(struct dw_i2c_dev *dev)
{
	int r;
...
static int i2c_v300_xfer_stat_check(struct dw_i2c_dev *dev,
	struct dw_i2c_controller *controller)
{
	unsigned int stat, stat1, stat2;
	int ret = 0;

	regmap_read(dev->map, DW_IC_STATUS, &stat);
	regmap_read(dev->map, DW_IC_TXFLR, &stat1);
	regmap_read(dev->map, DW_IC_RXFLR, &stat2);
	if ((DW_IC_TX_AND_RX_EMPTY != (stat & (DW_IC_INTR_TX_OVER | DW_IC_INTR_RX_FULL)))
			|| (stat1 & DW_IC_TX_FIFO_DATA_MASK)
			|| (stat2 & DW_IC_RX_FIFO_DATA_MASK)) {
...
static void i2c_v300_xfer_timeout_handle(struct i2c_adapter *adap,
	struct dw_i2c_controller *controller)
{
	struct dw_i2c_dev *dev = i2c_get_adapdata(adap);
	int ret = 0;
...
static int i2c_v300_dw_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	struct dw_i2c_dev *dev = i2c_get_adapdata(adap);
	struct dw_i2c_controller *controller = dev->priv_data;
	int totallen = 0;
	...
	i2c_v300_prepare_xfer(dev, controller, msgs, num);
	...
	if (i2c_v300_xfer_stat_check(dev, controller) < 0) {
		ret = -EAGAIN;
		goto done;
	}
	...
	if (likely(!dev->cmd_err)) {
		...
		if ((dev->status & (STATUS_READ_IN_PROGRESS|STATUS_WRITE_IN_PROGRESS))
				|| (DW_IC_INTR_TX_EMPTY == (stat & DW_IC_INTR_TX_EMPTY))) {
			dev_err(dev->dev, "write or read is not complete, status 0x%x, len = %d.\n", dev->status, totallen);
			if (controller->rst_for_status_err) {
				reset_i2c_controller(dev);
				ret = -ETIMEDOUT;
			} else {
				ret = -EAGAIN;
	...
	if (-ETIMEDOUT == ret) {
		i2c_v300_xfer_timeout_handle(adap, controller);
		ret = -EAGAIN;/*lint !e838  */
	}

	i2c_v300_xfer_end(dev);
	if (ret < 0)
	...
	struct dw_i2c_dev *d)
{
	...
	u32 secure_slave_num = 0;
	u32 secure_slave_reg[I2C_V300_SEC_SLAVE_MAX_NUM] = {0};
	struct arm_smccc_res res;
	int ret;
	u32 i;
	...
	ret = of_property_read_u32(dev->of_node, "secure-slave-num",
		&secure_slave_num);
	if (ret) {
		secure_slave_num = I2C_V300_DEFAULT_SEC_SLAVE_NUM;
	...
	if (ret) {
		secure_slave_reg[0] = I2C_V300_DEFAULT_SEC_SLAVE;
		dev_info(dev, "doesn't have slave-reg property, use default\n");
	}

	for (i = 0; i < secure_slave_num; i++)
		arm_smccc_1_1_smc(I2C_V300_SLAVE_WRITE_REG, d->reg_base,
	...
	struct platform_device *pdev)
{
	...
	struct dw_i2c_controller *ctrl = NULL;
	u32 secure_mode = 0;
	int ret;
	...
	ctrl = devm_kzalloc(dev, sizeof(struct dw_i2c_controller), GFP_KERNEL);
	if (!ctrl)
		return -ENOMEM;
	...
	struct device *dev)
{
	...
	gpio_recovery_info->recover_bus = i2c_generic_scl_recovery;

	gpio_recovery_info->prepare_recovery = dw_i2c_prepare_recovery;
	gpio_recovery_info->unprepare_recovery = dw_i2c_unprepare_recovery;

	gpio_recovery_info->scl_gpiod = gpio_to_desc(gpio_scl);
	gpio_recovery_info->sda_gpiod = gpio_to_desc(gpio_sda);

	return gpio_recovery_info;
}

static void hs_dw_i2c_read_ctrl_cfg(struct dw_i2c_controller *ctrl,
	struct device *dev)
{
	int ret;
	...
	ret = of_property_read_u32(dev->of_node, "rst-status", &ctrl->rst_for_status_err);
	if (ret) {
		dev_info(dev, "no reset!\n");
		ctrl->rst_for_status_err = false;
	} else {
		dev_info(dev, "rst_for_status_err : %d\n", ctrl->rst_for_status_err);
	}
	...
	ctrl->recover_bus = dw_i2c_scl_recover_bus;
}

#ifdef CONFIG_FMEA_FAULT_INJECTION
static int stub_hs_dw_i2c_injection(void)
{
	if (pmic_read_reg(FAULT_INJECT_REG) == KERNEL_FAULT_I2C) {
		pr_err("%s start i2c init fail fault injection.\n", __func__);
		return -ENODEV;
	}
...
static int hs_dw_i2c_init_adapter(struct i2c_adapter *adap,
...
static int hs_dw_i2c_fill_i2c_dev(struct dw_i2c_dev *d, struct device *dev)
{
	...
	ctrl = (struct dw_i2c_controller *)d->priv_data;
	...
	regmap_read(d->map, DW_IC_COMP_PARAM_1, &param1);
...
static void hs_dw_i2c_register_hck(void)
{
	static bool initd;

	if (initd)
		return;
	REGISTER_HCK_VH(i2c_dw_secure_read, hck_i2c_dw_secure_read);
	REGISTER_HCK_VH(i2c_dw_secure_write, hck_i2c_dw_secure_write);
	REGISTER_HCK_VH(i2c_dw_read_comp_type, hck_i2c_dw_read_comp_type);
	REGISTER_HCK_VH(i2c_dw_dma_fifo_cfg, hck_i2c_dw_dma_fifo_cfg);
	REGISTER_HCK_VH(i2c_dw_xfer_init_irq_mask, hck_i2c_dw_xfer_init_irq_mask);
	REGISTER_HCK_VH(i2c_dw_irq_mask_all, hck_i2c_dw_irq_mask_all);
	REGISTER_HCK_VH(i2c_dw_stop_irq_handler, hck_i2c_dw_stop_irq_handler);
	initd = 1;
}
...
static int hs_dw_i2c_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_FMEA_FAULT_INJECTION
	if (stub_hs_dw_i2c_injection())
		return -ENODEV;
#endif

	if ((hs_dw_i2c_new_dev(&d, pdev) != 0) || !d) {
	...
	hs_dw_i2c_register_hck();

	if (i2c_dw_init_regmap(d)) {
		dev_err(dev, "regmap init fail!\n");
		return -1;
	}
	...
	if (i2c_dw_set_timings_master(d)) {
		pr_err("%s: i2c_dw_set_timings_master fail\n", __func__);
		return -1;
	}
