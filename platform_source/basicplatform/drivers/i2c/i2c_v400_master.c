...
static u32 secure_i2c_v400_readl(u32 base_addr, u32 offset)
{
	...
	arm_smccc_1_1_smc(I2C_V400_READ_REG, base_addr, offset, &res);
	return (u32)res.a1;
}

static void secure_i2c_v400_writel(u32 base_addr, u32 offset, u32 val)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_smc(I2C_V400_WRITE_REG, val, base_addr, offset, &res);
}

static int i2c_v400_reg_read(void *context, u32 offset, u32 *val)
{
	struct hs_i2c_dev *dev = context;

	if (dev->secure_mode)
		*val = secure_i2c_v400_readl(dev->reg_base, offset);
	else
		*val = readl_relaxed(dev->base + offset);

	return 0;
}

static int i2c_v400_reg_write(void *context, u32 offset, u32 val)
{
	struct hs_i2c_dev *dev = context;

	if (dev->secure_mode)
		secure_i2c_v400_writel(dev->reg_base, offset, val);
	else
...
static int i2c_v400_init_regmap(struct hs_i2c_dev *dev)
{
	struct regmap_config map_cfg = {
		...
		.reg_read = i2c_v400_reg_read,
		.reg_write = i2c_v400_reg_write,
		.max_register = HS_SLV_SDA_SETUP,
	};
...
static void hs_i2c_fifo_clear(struct hs_i2c_dev *dev)
{
	...
	regmap_read(dev->map, FIFO_CTRL, &fifo_ctrl);
	fifo_ctrl = fifo_ctrl | FIFO_CLEAR;
	regmap_write(dev->map, FIFO_CTRL, fifo_ctrl);
	fifo_ctrl = fifo_ctrl & (~FIFO_CLEAR);
	regmap_write(dev->map, FIFO_CTRL, fifo_ctrl);
}
...
static void hs_i2c_speed_config(struct hs_i2c_dev *dev)
{
	u32 hcnt, lcnt;
	u32 speed_mode;
	u32 min_cnt;
	...
	input_clock_khz = hs_i2c_clk_rate(dev);

	/* minimum value of hcnt, commonly used to SS/FS/HS */
	...
	regmap_write(dev->map, SS_SCL_HCNT, hcnt);
	regmap_write(dev->map, SS_SCL_LCNT, lcnt);
	dev_info(dev->dev, "i2c std speed hcnt %u lcnt %u\n", hcnt, lcnt);
	...
	lcnt = (input_clock_khz/ I2C_SPEED_FAST) - hcnt \
		- I2C_SCL_RISE_TIME * input_clock_khz * I2C_MUL_1K;
	hcnt = (hcnt > min_cnt) ? (hcnt - min_cnt) : 1;
	regmap_write(dev->map, FS_SCL_HCNT, hcnt);
	regmap_write(dev->map, FS_SCL_LCNT, lcnt);
	dev_info(dev->dev, "i2c fast speed hcnt %u lcnt %u\n", hcnt, lcnt);
	...
	lcnt = (input_clock_khz / I2C_SPEED_FAST_PLUS) - hcnt \
		- I2C_SCL_RISE_TIME * input_clock_khz * I2C_MUL_1K;
	hcnt = (hcnt > min_cnt) ? (hcnt - min_cnt) : 1;
	regmap_write(dev->map, HS_SCL_HCNT, hcnt);
	regmap_write(dev->map, HS_SCL_LCNT, lcnt);
	dev_info(dev->dev, "i2c fast-plus speed hcnt %u lcnt %u\n", hcnt, lcnt);
};
 ...
 * run time.
 */
static void hs_i2c_init_master(struct hs_i2c_dev *dev)
{
	hs_i2c_speed_config(dev);

	/* Configure SDA Hold Time */
	if (dev->hold_time)
		regmap_write(dev->map, SDA_HOLD, dev->hold_time);
	else
		regmap_write(dev->map, SDA_HOLD, I2C_DEFULT_HOLD_TIME);

	/* configure the i2c master */
	regmap_write(dev->map, FRAME_CTRL, dev->master_cfg);

	if (dev->smbus) {
		/* Configure FIFO */
		regmap_write(dev->map, SMBUS_FIFO_CLR, SMBUS_FIFO_CLR_EN);
		udelay(1);
		regmap_write(dev->map, SMBUS_FIFO_CLR, SMBUS_FIFO_CLR_ALL);
		/* cfg fifo */
		regmap_write(dev->map, FIFO_CTRL, SMBUS_FIFO_TX_THD | SMBUS_FIFO_RX_THD);

		regmap_write(dev->map, SMBUS_HOST_CON, SMBUS_CHOSE_I2C_MASTER);
	} else {
		/* Configure FIFO */
		hs_i2c_fifo_clear(dev);
		regmap_write(dev->map, FIFO_CTRL, FIFO_TX_THD | FIFO_RX_THD);
	}
}

static void hs_i2c_disable_int(struct hs_i2c_dev *dev)
{
	regmap_write(dev->map, INT_MASK, 0);
}
...
static void hs_i2c_dma_clear(struct hs_i2c_dev *dev)
{
	...
	dev->using_dma = false;
	regmap_write(dev->map, DMA_CTRL, I2C_DMA_DISABLE);
}
...
static void i2c_scl_recover_bus(struct i2c_adapter *adap)
{
	...
	ret = i2c_pins_ctrl(dev, PINCTRL_STATE_IDLE);
	if (ret < 0)
		dev_err(dev->dev, "pins are not configured to default\n");

	scl_gpio = desc_to_gpio(bri->scl_gpiod);
	ret = gpio_request_one(scl_gpio, GPIOF_DIR_OUT, "i2c-scl");
	if (ret)
		 dev_err(dev->dev,
			"Can't get SCL gpio: %d. Not using SCL polling\n",
			scl_gpio);

	spin_lock_irqsave(&dev->spinlock, flags);
	gpio_direction_output(scl_gpio, I2C_GPIO_HIGH);
	for (i = 0; i < RECOVERY_CLK_COUNT; i++) {
		gpio_direction_output(scl_gpio, I2C_GPIO_LOW);
		ndelay(CLK_LOW_DELAY);
		gpio_direction_output(scl_gpio, I2C_GPIO_HIGH);
		ndelay(CLK_HIGH_DELAY);
	}
...
static int hs_i2c_wait_bus_not_busy(struct hs_i2c_dev *dev)
{
	...
	while (timeout > 0) {
		regmap_read(dev->map, TRANS_STATE, &stat);
...
static void hs_i2c_xfer_init(struct hs_i2c_dev *dev)
{
	...
	regmap_read(dev->map, FRAME_CTRL, &ic_con);
	if (msgs[dev->msg_write_idx].flags & I2C_M_TEN)
		ic_con |= I2C_CON_10BITADDR_MASTER;
	else
		ic_con &= ~I2C_CON_10BITADDR_MASTER;

	regmap_write(dev->map, FRAME_CTRL, ic_con);
	regmap_write(dev->map, SLV_ADDR, msgs[dev->msg_write_idx].addr);

	/* Enforce disabled interrupts (due to HW issues) */
	hs_i2c_disable_int(dev);

	/* Clear and enable interrupts */
	regmap_write(dev->map, INT_CLR, I2C_INT_CLEAR);
	regmap_write(dev->map, INT_MASK, I2C_INTR_DEFAULT_MASK);
}
 ...
 */
static void hs_i2c_xfer_msg(struct hs_i2c_dev *dev)
{
	struct i2c_msg *msgs = dev->msgs;
	u32 intr_mask;
	...
	if (dev->cmd_err == (unsigned int)I2C_INT_TRERR)
		return;
	...
	for (; dev->msg_write_idx < dev->msgs_num; dev->msg_write_idx++) {
		...
		if (ret)
			break;
		if ((dev->status & STATUS_WRITE_IN_PROGRESS) == 0) {
			/* new i2c_msg */
		...
		while (buf_len > 0 && tx_limit > 0 && rx_limit > 0) {
			...
			regmap_read(dev->map, FIFO_STATE, &stat);
			if (stat & (I2C_V400_TX_FULL | I2C_V400_RX_FULL))
				break;
			/*
			...
			if (msgs[dev->msg_write_idx].flags & I2C_M_RD) {
				regmap_write(dev->map, CMD_TXDATA, cmd |
					I2C_READ_CMD);
				rx_limit--;
				dev->rx_outstanding++;
			} else {
				regmap_write(dev->map, CMD_TXDATA, cmd | *buf++);
			}
			buf_len--;
			tx_limit--;
		}
	...
	regmap_write(dev->map, INT_MASK, intr_mask);
}

static u8 hs_i2c_recv_len(struct hs_i2c_dev *dev, u8 len)
{
	struct i2c_msg *msgs = dev->msgs;
...
static void hs_i2c_read(struct hs_i2c_dev *dev)
{
	struct i2c_msg *msgs = dev->msgs;
	u32 stat;
	...
	for (; dev->msg_read_idx < dev->msgs_num; dev->msg_read_idx++) {
		...
		if ((msgs[dev->msg_read_idx].flags & I2C_M_RD) == 0)
			continue;

		if ((dev->status & STATUS_READ_IN_PROGRESS) == 0) {
			len = msgs[dev->msg_read_idx].len;
			buf = msgs[dev->msg_read_idx].buf;
		} else {
			len = dev->rx_buf_len;
		...
		for (; len > 0; len--) {
			...
			regmap_read(dev->map, FIFO_STATE, &stat);
			if (stat & I2C_V400_RX_EMPTY)
				break;
			regmap_read(dev->map, RXDATA, &tmp);
			*buf = tmp & 0xff;
...
static int hs_i2c_xfer_msg_dma(struct hs_i2c_dev *dev, int *alllen)
{
	...
	regmap_write(dev->map, DMA_CTRL, I2C_DMA_ENABLE);
...
static int hs_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	struct hs_i2c_dev *dev = i2c_get_adapdata(adap);
	int totallen = 0;
	int ret;
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	if (dev->support_iodie && g_iodie_state == STS_ST_ERROR) {
		dev_err(dev->dev, "iodie error, please retry\n");
	...
	regmap_read(dev->map, FIFO_STATE, &stat);
	if ((stat & I2C_FIFO_TX_EMPTY) && !dev->using_dma)
		hs_i2c_xfer_msg(dev);
	...
	/* no error */
	if (likely(dev->cmd_err == 0)) {
		regmap_read(dev->map, INT_MASK, &stat);
		if ((dev->status &
	...
	struct hs_i2c_dev *d)
{
	...
	u32 secure_slave_reg[I2C_V400_SEC_SLAVE_MAX_NUM] = {0};
	struct arm_smccc_res res;
	int ret;
	u32 i;
	...
	ret = of_property_read_u32(dev->of_node, "secure-slave-num",
		&secure_slave_num);
	if (ret) {
		secure_slave_num = I2C_V400_DEFAULT_SEC_SLAVE_NUM;
	...
	if (ret) {
		secure_slave_reg[0] = I2C_V400_DEFAULT_SEC_SLAVE;
		dev_info(dev, "doesn't have slave-reg property, use default\n");
	}

	for (i = 0; i < secure_slave_num; i++)
		arm_smccc_1_1_smc(I2C_V400_SLAVE_WRITE_REG, d->reg_base,
	...
	struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "smbus-flag", &smbus);
	if (ret)
		smbus = 0;
	pr_info("%s smbus=%u\n", __func__, smbus);
	d->smbus = smbus;

	ret = of_property_read_u32(dev->of_node, "iomcu-i2c", &iomcu_i2c);
	if (ret)
		iomcu_i2c = 0;
	pr_info("%s iomcu_i2c=%u\n", __func__, iomcu_i2c);
	d->iomcu_i2c = iomcu_i2c;

	ret = of_property_read_u32(dev->of_node, "hold-time", &hold_time);
	if (ret)
		hold_time = 0;
	pr_info("%s hold_time=%u\n", __func__, hold_time);
	d->hold_time = hold_time;

	ret = of_property_read_u32(dev->of_node, "disable-restart", &disable_restart);
	if (ret)
		disable_restart = 0;
	pr_info("%s disable_restart=%u\n", __func__, disable_restart);
	d->disable_restart = disable_restart;
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	ret = of_property_read_u32(dev->of_node, "support-iodie", &d->support_iodie);
	if (ret)
		d->support_iodie = 0;
	pr_info("%s support_iodie=%u\n", __func__, d->support_iodie);
	if (d->support_iodie) {
		g_iodie_state = STS_ST_PRE_INITED;
		ret = sts_notify_register(STS_USER_I2C, iodie_state_cb);
		if (ret != 0)
			pr_err("i2c sts_notify_register failed\n");
	}
#endif
	d->pinctrl_flag = 0;
	d->pinctrl = NULL;
...
static u32 hs_i2c_read_clear_intrbits(struct hs_i2c_dev *dev)
{
	...
	regmap_read(dev->map, INT_MSTAT, &stat);
	if (stat & I2C_INT_TRCMP)
		regmap_write(dev->map, INT_CLR, I2C_INT_TRCMP);
	if (stat & I2C_INT_FIFOERR)
		regmap_write(dev->map, INT_CLR, I2C_INT_FIFOERR);
	if (stat & I2C_INT_RX_AFULL)
		regmap_write(dev->map, INT_CLR, I2C_INT_RX_AFULL);
	if (stat & I2C_INT_TX_AEMPTY)
		regmap_write(dev->map, INT_CLR, I2C_INT_TX_AEMPTY);
	if (stat & I2C_INT_TRERR) {
		regmap_read(dev->map, TRANS_ERR, &dummy);
		dev->error_source = dummy;
		regmap_write(dev->map, INT_CLR, I2C_INT_TRERR);
	}
...
static int hs_i2c_fill_i2c_dev(struct hs_i2c_dev *d, struct device *dev)
{
	...
	d->functionality = HS_IC_DEFAULT_FUNCTIONALITY | I2C_FUNC_10BIT_ADDR;

	if (d->disable_restart)
		d->master_cfg = speed_mode;
	else
...
static int hs_i2c_master_probe(struct platform_device *pdev)
{
	...
	if (i2c_v400_init_regmap(d)) {
		dev_err(dev, "%s: regmap init fail\n", __func__);
		goto clr_err;
	}
