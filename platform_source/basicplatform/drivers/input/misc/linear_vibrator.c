 ...
 * Description: Linear vibrator driver for pmic vibrator.
 *
...
#include "linear_vibrator_pmic23.h"
#else
...
#define LINEAR_VIBRATOR_FREQ_CALCU_BASE 10000000
...
static void linear_vibrator_set_clock(int pmic_vibrator_clk_onoff)
{
	...
	if (pmic_vibrator_clk_onoff)
		sub_pmic_reg_write(LINEAR_VIBRATOR_LRA_ANA_CFG2,
...
static int linear_vibrator_frep_convert(void)
{
	u32 f0;

	if ((g_vdev->linear_vibrator_freq >> 3) == 0) { // calc f0 val
		dev_err(g_vdev->dev, "freq calc err, pmu not init");
		return 0;
	}
	f0 = LINEAR_VIBRATOR_FREQ_CALCU_BASE / (g_vdev->linear_vibrator_freq >> 3);
	return f0;
}
...
static void linear_vibrator_get_freq(void)
{
	...
	lra_frep_convert_f0 = linear_vibrator_frep_convert();
	dev_info(g_vdev->dev, "frep is 0x%x, convert to %d\n",
		g_vdev->linear_vibrator_freq, lra_frep_convert_f0);
}
...
static void linear_vibrator_freq_dmd(struct linear_vibrator_dev *vdev)
{
	...
	if (vdev->linear_vibrator_freq == 0)
		return;
...
#ifdef CONFIG_PMU_LINEAR_HAPTIC_EVADE
	if (pmu_linear_haptic_check_splice_haptic(vdev->state)) {
		pmu_linear_haptic_play_splice_haptic_effect(vdev->state);
...
static void classd_vibrator_duration_notify(struct led_classdev *cdev, enum led_brightness state)
{
	...
	if (transient_data == NULL) {
		dev_err(g_vdev->dev, "%s: invalid data", __func__);
		return;
	}
	...
	struct led_classdev *led_dev, enum led_brightness state)
{
	...
	vdev->state = state;
#ifdef CONFIG_HUAWEI_VIBRATOR
	classd_vibrator_duration_notify(led_dev, state);
#endif
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((g_vdev->linear_vibrator_freq > LINEAR_VIBRATOR_FREQ_MAX) ||
		(g_vdev->linear_vibrator_freq < LINEAR_VIBRATOR_FREQ_MIN))
		cal_rst = 0;
	else
		cal_rst = 1;

	return snprintf_s(buf, VIBRATOR_TIMEOUT_MIN_SIZE,
		VIBRATOR_TIMEOUT_MIN_SIZE - 1, "%d", cal_rst);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	UNUSED(attr);
	if ((count > MAX_INPUT_SIZE) || (kstrtoll(buf, 10, &value))) {
		dev_err(g_vdev->dev, "cali_store value error\n");
		return count;
	}
	dev_info(g_vdev->dev, "%s value is %lld\n", __func__, value);

	linear_vibrator_enable_ctrl(led_cdev, value);
	mdelay(1000);
	linear_vibrator_enable_ctrl(led_cdev, 0);
	...
	struct device_attribute *attr, char *buf)
{
	...
	f0 = linear_vibrator_frep_convert();
	dev_info(g_vdev->dev, "%s cal_val f0 is %d\n", __func__, f0);

	return snprintf_s(buf, VIBRATOR_TIMEOUT_MIN_SIZE,
		VIBRATOR_TIMEOUT_MIN_SIZE - 1, "%d", f0);
}
...
static DEVICE_ATTR(vibrator_calib, 0644, cali_show, cali_store);
static DEVICE_ATTR(cali_val, 0644, cali_val_show, NULL);
...
#ifdef CONFIG_LINEAR_VIBRATOR_DEBUG
...
static void linear_vibrator_remove(struct spmi_device *pdev)
{
	struct linear_vibrator_dev *vdev = NULL;
	...
	mutex_destroy(&vdev->lock);
	wakeup_source_unregister(&vdev->wakelock);
