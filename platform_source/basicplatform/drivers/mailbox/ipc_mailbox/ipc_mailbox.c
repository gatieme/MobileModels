...
#define TX_FIFO_CELL_SIZE (sizeof(struct ipc_mbox_task *))
#define RX_FIFO_CELL_SIZE (sizeof(struct ipc_mbox_rx_task *))
#define MAILBOX_MAX_TX_FIFO 256
#define MAILBOX_MAX_RX_FIFO 256
...
static struct ipc_mbox_task *g_txtaskbuffer;
static spinlock_t g_rx_task_buffer_lock;
static struct ipc_mbox_rx_task *g_rxtaskbuffer;
...
{
	struct ipc_mbox_task *ptask = NULL;
	int i;

	if (g_txtaskbuffer) {
		mbox_pr_info("g_txtaskbuffer initialized");
		return 0;
	}
	...
	g_txtaskbuffer = kcalloc(TX_TASK_DDR_NODE_NUM, sizeof(*ptask),
		GFP_KERNEL);
	if (!g_txtaskbuffer) {
		mbox_pr_err("g_txtaskbuffer is null");
		return -ENOMEM;
	}
...
struct ipc_mbox_task *ipc_mbox_node_alloc(void)
{
	int index;
	struct ipc_mbox_task *ptask = g_txtaskbuffer;
	unsigned long flags = 0;

	if (!g_txtaskbuffer) {
		mbox_pr_err("%s g_txtaskbuffer is null", __func__);
		return NULL;
	}
	...
	mbox_msg_t *tx_buffer, mbox_msg_len_t tx_buffer_len, int need_auto_ack)
{
	...
	ret = memcpy_s((void *)tx_task->tx_buffer,
			MBOX_CHAN_DATA_SIZE * (sizeof(*(tx_task->tx_buffer))),
			(void *)tx_buffer,
...
static int ipc_mbx_init_rx_task_buf(void)
{
	struct ipc_mbox_rx_task *ptask = NULL;
	int i;

	if (g_rxtaskbuffer) {
		mbox_pr_info("g_rxtaskbuffer initialized");
		return 0;
	}

	g_rxtaskbuffer = kcalloc(RX_TASK_DDR_NODE_NUM, sizeof(*ptask), GFP_KERNEL);
	if (!g_rxtaskbuffer) {
		mbox_pr_err("g_rxtaskbuffer is null");
		return -ENOMEM;
	}

	if (memset_s(g_rxtaskbuffer, sizeof(*ptask) * RX_TASK_DDR_NODE_NUM,
		0, sizeof(*ptask) * RX_TASK_DDR_NODE_NUM) != EOK)
		mbox_pr_err("memset g_rxtaskbuffer failed");

	ptask = g_rxtaskbuffer;
	for (i = 0; i < RX_TASK_DDR_NODE_NUM; i++) {
		/*
		 * Init the rx buffer's node, set the flag to available
		 */
		ptask->used_flag = RX_TASK_DDR_NODE_AVA;
		ptask++;
	}
...
struct ipc_mbox_rx_task *ipc_mbox_rx_node_alloc(void)
{
	int index;
	struct ipc_mbox_rx_task *ptask = g_rxtaskbuffer;
	unsigned long flags = 0;
	...
	spin_lock_irqsave(&g_rx_task_buffer_lock, flags);
	for (index = 0; index < RX_TASK_DDR_NODE_NUM; index++) {
		if (ptask->used_flag == RX_TASK_DDR_NODE_AVA)
			break;
		ptask++;
	}

	if (likely(index != RX_TASK_DDR_NODE_NUM))
		ptask->used_flag =
			RX_TASK_DDR_NODE_OPY; /* set the node occupied  */
	else
		ptask = NULL;

	spin_unlock_irqrestore(&g_rx_task_buffer_lock, flags);

	return ptask;
}

void ipc_mbox_rx_task_free(struct ipc_mbox_rx_task **rx_task)
{
	unsigned long flags = 0;
	...
	spin_lock_irqsave(&g_rx_task_buffer_lock, flags);
	(*rx_task)->used_flag = RX_TASK_DDR_NODE_AVA;
	spin_unlock_irqrestore(&g_rx_task_buffer_lock, flags);
}
...
int ipc_mbox_vm_flag(struct ipc_mbox *mbox)
{
	...
	mdev = mbox->tx;
	return mdev->ops->get_vm_flag(mdev);
}
EXPORT_SYMBOL(ipc_mbox_vm_flag);

static int set_status(struct ipc_mbox_device *mdev, int status)
{
	int ret = 0;
	unsigned long flags = 0;
	unsigned int old_status = mdev->status;
...
static void clr_status(struct ipc_mbox_device *mdev, int status)
{
	unsigned long flags = 0;
	unsigned int old_status = mdev->status;
...
static int ipc_mbox_task_send_async(
	struct ipc_mbox_device *mdev, struct ipc_mbox_task *tx_task)
{
	int ret;
...
static void ipc_mbox_sync_fail_print(
	struct ipc_mbox_device *mdev, int need_irq_enable, int receipted)
{
...
static void ipc_mbox_set_complete(struct ipc_mbox_device *mdev,
...
static int ipc_mbox_sync_process_record(struct ipc_mbox_device *mdev,
	struct ipc_mbox_task *tx_task, unsigned long *flags)
{
...
static unsigned long ipc_mbox_get_tx_timeout(struct ipc_mbox_device *mdev)
{
...
int ipc_mbox_msg_send_sync(struct ipc_mbox *mbox, mbox_msg_t *tx_buffer,
	...
	mbox_msg_len_t ack_buffer_len)
{
	...
	ret = memcpy_s((void *)tx_task.tx_buffer,
			...
			tx_buffer_len * (sizeof(*tx_buffer)));
	if (ret != EOK) {
		mbox_pr_err("%s: memcpy_s failed, mbox->id %d, tx_buffer_len %d",
			__func__,  mbox->rproc_id, tx_buffer_len);
		ret = -EINVAL;
		goto out;
	}
	tx_task.tx_buffer_len = tx_buffer_len;
...
static bool ipc_mbox_msg_async_can_send_directly(struct ipc_mbox_device *mdev)
{
	/* can't send directly in atomic context due to ipc use mutext lock,
	 * so we simply ignore them
	 * */
#ifdef CONFIG_IPC_ASYNC_AND_INTERRUPT_OPTIMIZED
	if (in_atomic() || (kfifo_len(&mdev->fifo) >= TX_FIFO_CELL_SIZE) ||
		mdev->ops->is_stm(mdev, SOURCE_STATUS) ||
		mdev->ops->is_stm(mdev, DESTINATION_STATUS))
		return false;
...
int ipc_mbox_msg_send_async(
	struct ipc_mbox *mbox, struct ipc_mbox_task *tx_task)
{
	struct ipc_mbox_device *mdev = NULL;
	int ret;
	unsigned long flags = 0;
	...
	mdev = mbox->tx;
	if (ipc_mbox_msg_async_can_send_directly(mdev))
	{
		mutex_lock(&mdev->dev_lock);
		ret = ipc_mbox_task_send_async(mdev, tx_task);
		if (ret)
			mbox_pr_err("%s: ipc_mbox_task_send_async send fail\n", __func__);
...
static struct ipc_mbox_task *ipc_mbox_dequeue_task(
	struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_task *tx_task = NULL;
	unsigned long flags = 0;
...
static inline void ipc_do_async_ack_work(struct ipc_mbox_task *tx_task)
{
...
static void tx_time_judge_print(struct ipc_mbox_device *mdev,
	struct ipc_mbox_task *tx_task)
{
	if (tx_task->ts[10] - tx_task->ts[0] > 3000000000) {
		int i;
		for (i = 0; i < 11; i++)
		pr_err("ts[%d]=%llu\n", i, tx_task->ts[i]);

		for (i = 0; i < 2; i++)
		pr_err("mdev ts[%d]=%llu delay[%d]=%llu\n", i, mdev->ts[i].timestamp, i, mdev->ts[i].delay);
#ifndef CONFIG_LIBLINUX
		pr_err("id=%d curr delay=%llu\n", tx_task->rproc_id, current->sched_info.run_delay);
#endif
	}
}
...
{
	struct ipc_mbox_device *mdev = (struct ipc_mbox_device *)context;
	struct ipc_mbox_task *tx_task = NULL;
	int ret;
	u64 wakeup_ts, sleep_ts;

	while (!kthread_should_stop()) {
		sleep_ts = ktime_get_ns();
		mdev->ts[0].timestamp = sleep_ts;
#ifndef CONFIG_LIBLINUX
		mdev->ts[0].delay = current->sched_info.run_delay;
#endif
		ret = wait_event_interruptible(mdev->tx_wait,
			(kfifo_len(&mdev->fifo) >= TX_FIFO_CELL_SIZE));
		if (unlikely(ret)) {
			mbox_pr_info("%s wait event SIGPENDING", __func__);
		...
		wakeup_ts = ktime_get_ns();
		mdev->ts[1].timestamp = wakeup_ts;
#ifndef CONFIG_LIBLINUX
		mdev->ts[1].delay = current->sched_info.run_delay;
#endif
		mutex_lock(&mdev->dev_lock);
		/*
		...
		tx_task->ts[7] = wakeup_ts;
		tx_task->ts[8] = ktime_get_ns();
		while (tx_task) {
			ipc_notify_mailbox_event(
				EVENT_IPC_ASYNC_TASK_SEND, mdev, "");
			if (tx_task->need_auto_ack != MANUAL_ACK) {
				tx_task->ts[9] = ktime_get_ns();
				ret = ipc_mbox_task_send_async(mdev, tx_task);
				tx_task->ts[10] = ktime_get_ns();
				/* over 3s */
				tx_time_judge_print(mdev, tx_task);
			} else {
				ret = ipc_mbox_task_send_sync(mdev, tx_task);
...
{
	struct ipc_mbox_device *mdev =
		(struct ipc_mbox_device *)(uintptr_t)context;
...
#ifdef CONFIG_IPC_ASYNC_AND_INTERRUPT_OPTIMIZED
static void ipc_recv_rx_task(struct ipc_mbox_device *mdev)
{
	int ret;
	mbox_msg_t *rx_buffer = NULL;
	mbox_msg_len_t rx_len;
	struct ipc_mbox_rx_task *rx_task = NULL;
	...
	rx_task = ipc_mbox_rx_node_alloc();
	if (!rx_task) {
		mbox_pr_err("rx task no mem\n");
		goto out;
	}

	ret = memcpy_s((void *)rx_task->rx_buffer,
		MBOX_CHAN_DATA_SIZE * (sizeof(*(rx_task->rx_buffer))),
		(void *)rx_buffer, rx_len * sizeof(*rx_buffer));
	if (ret != EOK) {
		mbox_pr_err("memcpy_s rx fail\n");
		goto rx_task_free;
	}

	rx_task->rx_buffer_len = rx_len < MBOX_CHAN_DATA_SIZE ? rx_len : MBOX_CHAN_DATA_SIZE;
	if (kfifo_avail(&mdev->rx_fifo) < RX_FIFO_CELL_SIZE) {
		mbox_pr_err("rx kfifo is full\n");
		goto rx_task_free;
	}

	if (kfifo_in(&mdev->rx_fifo, &rx_task, RX_FIFO_CELL_SIZE) != RX_FIFO_CELL_SIZE)
		mbox_pr_err("no enough rx kfifo\n");

	goto out;

rx_task_free:
	ipc_mbox_rx_task_free(&rx_task);

out:
	mdev->ops->enable_irq(mdev);
}

static int rx_kfifo_len(struct ipc_mbox_device *mdev)
{
	int len;
	unsigned long flags;

	local_irq_save(flags);
	len = kfifo_len(&mdev->rx_fifo);
...
static void ipc_mbox_rx_bh(unsigned long context)
{
	...
	while (rx_kfifo_len(mdev) >= RX_FIFO_CELL_SIZE) {
		if (kfifo_out(&mdev->rx_fifo, &rx_task, RX_FIFO_CELL_SIZE)) {
			atomic_notifier_call_chain(&mdev->notifier,
				rx_task->rx_buffer_len, (void *)rx_task->rx_buffer);
			ipc_mbox_rx_task_free(&rx_task);
		}
	}
...
static inline void ipc_recv_rx_task(struct ipc_mbox_device *mdev) { }
#endif
static irqreturn_t ipc_mbox_interrupt(int irq, void *p)
{
	struct ipc_mbox_device *mdev = (struct ipc_mbox_device *)p;
	const char *err_msg =
	...
	/* ipc */
	if (mdev->ops->get_vm_flag(mdev) || mdev->ops->is_stm(mdev, DESTINATION_STATUS)) {
		mbox_pr_debug("mdev %s ipc", mdev->name);
		mdev->cur_task = RX_TASK;
		mdev->cur_irq = irq;
		mdev->ops->disable_irq(mdev);
		ipc_recv_rx_task(mdev);
...
int ipc_mdev_get_irq(struct ipc_mbox_device *mdev);
void ipc_mbox_schedule_irq_bh(struct ipc_mbox_device *mdev)
{
	int mdev_irq;
	mdev_irq = ipc_mdev_get_irq(mdev);
	mbox_pr_err("schedule_irq_bh mdev->name: %s, priv->irq:%d\n", mdev->name, mdev_irq);
	mdev->cur_task = TX_TASK;
	mdev->cur_irq = mdev_irq;
	mdev->ops->disable_irq(mdev);

	if (unlikely(mdev->completed == COMPLETED)) {
		mbox_pr_err("mdev %s rx tx_task has completedL\n", mdev->name);
		return;
	}
	mdev->completed = COMPLETED;
	complete(&mdev->complete);
}
EXPORT_SYMBOL(ipc_mbox_schedule_irq_bh);
#endif

static void ipc_mbox_shutdown(
	struct ipc_mbox_device *mdev, enum mbox_mail_type_t mail_type)
{
	mutex_lock(&mdev->dev_lock);
	if (!--mdev->configured) {
...
static void ipc_mbox_free(struct ipc_mbox **mbox)
{
	struct ipc_mbox *_mbox = *mbox;
...
{
	struct ipc_mbox *_mbox = NULL;
	struct ipc_mbox_device *mdev[MAIL_TYPE_MAX] = { NULL };
	int i;
...
static int ipc_tx_mbox_startup(struct ipc_mbox_device *mdev)
{
	unsigned int tx_buff = mdev->ops->get_fifo_size(mdev) * TX_FIFO_CELL_SIZE;
	...
	(void)memset_s(mdev->ts, sizeof(struct record) * 2, 0, sizeof(struct record) * 2);
...
static void ipc_tx_mbox_startup_rollback(struct ipc_mbox_device *mdev)
{
	kfifo_free(&mdev->fifo);
	tasklet_kill(&mdev->tx_bh);
}
	...
	if (!mdev->configured++) {
		...
		case RX_MAIL:
			ret = kfifo_alloc(&mdev->rx_fifo,
				MAILBOX_MAX_RX_FIFO * RX_FIFO_CELL_SIZE, GFP_KERNEL);
			if (ret)
				goto deconfig;
			tasklet_init(
...
static struct ipc_mbox *ipc_mbox_alloc(struct ipc_mbox_device *tx_mdev,
	struct ipc_mbox_device *rx_mdev, int rproc_id)
{
 ...
 */
static struct ipc_mbox_device *ipc_mbox_device_get(
	struct list_head *list, enum mbox_mail_type_t mtype, int rproc_id)
{
	struct ipc_mbox_device *mdev = NULL;
	struct ipc_mbox_device *_mdev = NULL;
...
void ipc_mbox_device_deactivate(struct ipc_mbox_device **list)
{
	struct ipc_mbox_device *mdev = NULL;
	struct ipc_mbox_task *tx_task = NULL;
	int i;
...
void ipc_mbox_device_activate(struct ipc_mbox_device **list)
{
	struct ipc_mbox_device *mdev = NULL;
	int i;
...
static int ipc_mbox_device_is_valid(struct ipc_mbox_device *mdev)
{
...
int ipc_mbox_device_register(
	struct device *parent, struct ipc_mbox_device **list)
{
	struct ipc_mbox_device *mdev = NULL;
	int ret;
	int i;
...
static int __init ipc_mbox_init(void)
{
	...
	if (ret) {
		mbox_pr_err("%s: init fail", __func__);
		return ret;
	}

	spin_lock_init(&g_rx_task_buffer_lock);
	ret = ipc_mbx_init_rx_task_buf();
	if (ret) {
		mbox_pr_err("%s: ipc_mbx_init_rx_task_buf init fail", __func__);
		return ret;
	}
...
static void __exit ipc_mbox_exit(void)
{
	...
	if (g_txtaskbuffer)
		kfree_sensitive(g_txtaskbuffer);

	if (g_rxtaskbuffer)
		kfree_sensitive(g_rxtaskbuffer);
#else
	if (g_txtaskbuffer)
		kzfree(g_txtaskbuffer);

	if (g_rxtaskbuffer)
		kzfree(g_rxtaskbuffer);
#endif
}
