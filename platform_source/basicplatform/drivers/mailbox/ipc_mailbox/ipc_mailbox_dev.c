...
#ifndef CONFIG_IPC_SECURITY_HEADER_FILE_REPLACE
...
#define MAILBOX_ASYNC_UDELAY_CNT 250
#else
...
struct ipc_device {
	...
	struct ipc_mbox_device **mdev_res;
...
static void __ipc_writel(struct ipc_device *idev, unsigned int val, unsigned int offset)
{
	__raw_writel(val, idev->base + offset);
}

static unsigned int __ipc_readl(struct ipc_device *idev, unsigned int offset)
{
...
static const char *rproc_analysis(
	struct ipc_mbox_device *mdev, unsigned int pro_code)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	unsigned char index;

	index = _rproc_find_index(pro_code);
	if (!index)
		return "ERR_RPROC";
	index--;
...
{
	__ipc_writel(idev, idev->unlock, (unsigned int)ipc_lock(idev->ipc_version));
}

static inline unsigned int __ipc_lock_status(struct ipc_device *idev)
{
	return __ipc_readl(idev, (unsigned int)ipc_lock(idev->ipc_version));
}

static inline void __ipc_set_src(struct ipc_mbox_device_priv *priv)
{
	__ipc_writel(priv->idev, \
		(unsigned int)ipc_bitmask((unsigned int)priv->src), \
		(unsigned int)ipc_mbx_source(priv->mbox_channel, priv->idev->reg_spc));
}

static inline unsigned int __ipc_read_src(struct ipc_mbox_device_priv *priv)
{
	return __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_source(priv->mbox_channel, priv->idev->reg_spc));
}

static inline void __ipc_set_des(struct ipc_mbox_device_priv *priv)
{
	__ipc_writel(priv->idev, (unsigned int)ipc_bitmask((unsigned int)priv->des),
		(unsigned int)ipc_mbx_dset(priv->mbox_channel, priv->idev->reg_spc));
}

static inline unsigned int __ipc_des_status(struct ipc_mbox_device_priv *priv)
{
	return __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_dstatus(priv->mbox_channel, priv->idev->reg_spc));
}

static inline void __ipc_send(
	struct ipc_mbox_device_priv *priv, unsigned int tosend)
{
	__ipc_writel(priv->idev, tosend,
		(unsigned int)ipc_mbx_send(priv->mbox_channel, priv->idev->reg_spc));
}
...
{
	return __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_data(priv->mbox_channel,
...
{
	__ipc_writel(priv->idev, data, ipc_mbx_data(priv->mbox_channel,
		(unsigned int)index, priv->idev->reg_spc));
}
...
{
	return __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_imask(priv->mbox_channel, priv->idev->reg_spc));
}
	...
	struct ipc_mbox_device_priv *priv, unsigned int toclr)
{
	...
	reg = __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_imask(priv->mbox_channel, priv->idev->reg_spc));
	reg = reg & (~(toclr));

	__ipc_writel(priv->idev, reg,
		(unsigned int)ipc_mbx_imask(priv->mbox_channel, priv->idev->reg_spc));
}

static inline void __ipc_cpu_imask_all(struct ipc_mbox_device_priv *priv)
{
	__ipc_writel(priv->idev, (~0),
		(unsigned int)ipc_mbx_imask(priv->mbox_channel, priv->idev->reg_spc));
}
...
static inline void __ipc_cpu_iclr(
	struct ipc_mbox_device_priv *priv, unsigned int toclr)
{
	__ipc_writel(priv->idev, toclr,
		(unsigned int)ipc_mbx_iclr(priv->mbox_channel, priv->idev->reg_spc));
}

static inline unsigned int __ipc_status(struct ipc_mbox_device_priv *priv)
{
	return __ipc_readl(priv->idev,
		(unsigned int)ipc_mbx_mode(priv->mbox_channel, priv->idev->reg_spc));
}

#ifdef CONFIG_IPC_MAILBOX_DEBUG
static unsigned int __ipc_mbox_irstatus(
	struct ipc_mbox_device_priv *priv)
{
	if (priv->idev->ipc_version == IPC_VERSION_BOY)
		return __ipc_readl(priv->idev,ipc_cpux_irst_ex(
			(unsigned int)priv->src, priv->mbox_channel));
	else
		return __ipc_readl(priv->idev,
			ipc_cpux_irst((unsigned int)priv->src));
}
#endif
	...
	unsigned int channel_region)
{
	if (priv->idev->ipc_version == IPC_VERSION_BOY)
		return __ipc_readl(priv->idev,
			(unsigned int)ipc_cpux_imst_ex((unsigned int)src, channel_region));
	else
		return __ipc_readl(
			priv->idev, (unsigned int)ipc_cpux_imst((unsigned int)src));
}

static inline void __ipc_mode(
	struct ipc_mbox_device_priv *priv, unsigned int mode)
{
	__ipc_writel(priv->idev, mode,
		(unsigned int)ipc_mbx_mode(priv->mbox_channel, priv->idev->reg_spc));
}

static int ipc_mdev_startup(struct ipc_mbox_device *mdev)
{
	/*
...
static void ipc_mdev_shutdown(struct ipc_mbox_device *mdev)
{
	/*
	 * nothing will be done during suspend & resume flow for HI3xxx IPC.
	 * see dummy like SR function, ipc_mdev_suspend & ipc_mdev_resume.
...
static void ipc_mdev_dump_status(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	/*
...
static void ipc_mdev_dump_regs(struct ipc_mbox_device *mdev)
{
#ifdef CONFIG_IPC_MAILBOX_DEBUG
	struct ipc_mbox_device_priv *priv = mdev->priv;

	mdev_err("%s CPU_IMST: 0x%08x", mdev->name,
		__ipc_mbox_istatus(priv, priv->src,
		priv->mbox_channel / MBX_IRQ_REGION_SIZE));
	mdev_err("%s CPU_IRST: 0x%08x", mdev->name,
		__ipc_mbox_irstatus(priv));
#endif
}

static int ipc_mdev_check(struct ipc_mbox_device *mdev,
...
static void ipc_mdev_clr_ack(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	unsigned int imask;
	unsigned int toclr;
...
static void ipc_mdev_clr_irq_and_ack(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	unsigned int status;
	unsigned int imask;
...
static void ipc_mdev_ack(
	struct ipc_mbox_device *mdev, mbox_msg_t *msg, mbox_msg_len_t len)
{
}

static mbox_msg_len_t ipc_mdev_hw_read(
	struct ipc_mbox_device *mdev, mbox_msg_t *msg)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
 ...
 */
static int ipc_mdev_is_stm(struct ipc_mbox_device *mdev, unsigned int stm)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	int is_stm = 0;
...
static mbox_msg_len_t ipc_mdev_receive_msg_host(
	struct ipc_mbox_device *mdev, mbox_msg_t **buf)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	...
	buf_phy = virt_to_phys(mdev->rx_buffer);
	arm_smccc_hvc(IPC_HW_REC, priv->idev->iores->start, priv->mbox_channel,
		priv->idev->reg_spc, buf_phy, priv->capability,
		priv->idev->rproc_src_id[0], priv->idev->rproc_src_id[1],
		&res);
	*buf = mdev->rx_buffer;
	if (res.a0) {
		mdev_err("%s error res %u", __func__, res.a0);
		return 0;
	}
#endif
	...
	struct ipc_mbox_device *mdev, mbox_msg_t **buf)
{
	...
	/* guest only rx buffer */
	if (priv->idev->vm_flag)
		return ipc_mdev_receive_msg_host(mdev, buf);
...
static int ipc_mdev_unlock(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_occupy(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_hw_send(struct ipc_mbox_device *mdev, mbox_msg_t *msg,
...
static void ipc_mdev_ensure_channel(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	unsigned int loop = priv->timeout /
	...
	if (mdev->ops->is_stm(mdev, IDLE_STATUS)) {
		...
		/* the worst situation is to delay 305ms */
		while (timeout < loop) {
			if (timeout < MAILBOX_ASYNC_UDELAY_CNT) {
#ifdef CONFIG_IPC_ASYNC_AND_INTERRUPT_OPTIMIZED
				usleep_range(15, 20); /* 250*20us+60*5ms = 305ms */
#else
				udelay(5); /* 1000*5us+60*5ms = 305ms */
#endif
			} else {
				/*
...
static int ipc_mdev_send_msg(struct ipc_mbox_device *mdev, mbox_msg_t *msg,
	mbox_msg_len_t len, int ack_mode)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	int err = 0;
...
static void ipc_mdev_release(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_gic_irq_request(struct ipc_mbox_device *mdev,
	irq_handler_t handler, void *dev_id)
{
	...
	else if (!strcmp("mailbox-4", mdev->name))
		ret = request_irq(priv->irq, handler,
			IRQF_NO_SUSPEND, mdev->name, dev_id);
	else
		ret = request_irq(priv->irq, handler, 0, mdev->name, dev_id);

	/* mailbox-2 set intr affinity */
	if (priv->intr_affinity && cpu_online(priv->intr_affinity)) {
		ret = irq_set_affinity(priv->irq, cpumask_of(priv->intr_affinity));
		if (ret)
			mdev_err("%s intr_affinity set affinity fail, irq %d",
					mdev->name, priv->irq);
	}

	/* for guest mailbox irq */
	if (priv->idev->vm_flag && cpu_online(1)) {
		ret = irq_set_affinity(priv->irq, cpumask_of(1));
		if (ret)
			mdev_err("%s irq set affinity fail, irq %d", mdev->name, priv->irq);
	}
...
static void ipc_mdev_irq_free(struct ipc_mbox_device *mdev, void *p)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static void ipc_mdev_irq_enable(struct ipc_mbox_device *mdev)
{
	enable_irq((unsigned int)mdev->cur_irq);
}

static void ipc_mdev_irq_disable(struct ipc_mbox_device *mdev)
{
...
static struct ipc_mbox_device *ipc_mdev_match_irq_dev(
	...
	unsigned int src)
{
	...
	unsigned int ipc_type = _priv->idev->ipc_type;
...
static struct ipc_mbox_device *ipc_mdev_irq_to_mdev(
	struct ipc_mbox_device *mdev, struct list_head *list, int irq)
{
	struct ipc_mbox_device_priv *priv = NULL;
	enum remote_processor_type_t src;
...
static unsigned int ipc_mdev_timeout(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static unsigned int ipc_mdev_fifo_size(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static unsigned int ipc_mdev_sched_priority(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static unsigned int ipc_mdev_sched_policy(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
{
	struct ipc_mbox_device_priv *priv = (struct ipc_mbox_device_priv *)mdev->priv;
	return priv->irq;
}
EXPORT_SYMBOL(ipc_mdev_get_irq);
#endif

static int ipc_mdev_get_rproc_id(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_get_ipc_version(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_get_channel_size(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_get_channel_id(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static int ipc_mdev_get_slice(struct ipc_mbox_device *mdev, u64 *slice)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static void ipc_mdev_show_one_mdev_info(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
...
static void ipc_mdev_show_mdev_info(struct ipc_mbox_device *mdev, int idx)
{
	if (idx == 0)
...
static int ipc_mdev_get_vm_flag(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	struct ipc_device *idev = priv->idev;
...
struct ipc_mbox_dev_ops ipc_mdev_ops = {
	...
	.get_vm_flag = ipc_mdev_get_vm_flag,
};

static void ipc_mdev_put(struct ipc_device *idev)
{
	struct ipc_mbox_device **list = idev->mdev_res;
	struct ipc_mbox_device *mdev = NULL;
	int i = 0;

	iounmap(idev->base);
	if (idev->system_time_reg_addr)
		iounmap(idev->system_time_reg_addr - SCTRL_TIME_ADDR);
...
static int mdev_idev_init(struct ipc_device *idev,
	struct device_node *node, struct ipc_mbox_device **mdevs)
{
	...
	ret = of_property_read_u32(node, "vm_flag", &idev->vm_flag);
	if (ret == 0)
		mdev_info("prop vm_flag set");
...
static struct ipc_mbox_device_priv *mdev_priv_init(struct device_node *son,
	struct ipc_device *idev, struct ipc_mbox_device *mdev,
	unsigned int used)
{
	int ret;
	struct ipc_mbox_device_priv *priv = NULL;
	...
	ret = of_property_read_u32(son, "intr_affinity", &(priv->intr_affinity));
	if (ret)
		priv->intr_affinity = 0;
...
static int ipc_init_one_mdev(struct ipc_mbox_device **mdev_list,
	...
	int *mdev_idx_temp)
{
	...
	mdev = kzalloc(sizeof(struct ipc_mbox_device), GFP_KERNEL);
	if (!mdev)
		return -ENOMEM;
...
static int ipc_mdev_get(struct ipc_device *idev,
	struct ipc_mbox_device **mdevs, int mdev_num, struct device_node *node)
{
...
static int ipc_mdev_probe(struct platform_device *pdev)
{
	...
	struct ipc_mbox_device **mdev_res = NULL;
	...
	idev->iores = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (!idev->iores) {
		mdev_err("can not get resource!\n");
		ret = -ENOMEM;
		goto free_idev;
	}
...
MODULE_AUTHOR("Technologies Co., Ltd.");
