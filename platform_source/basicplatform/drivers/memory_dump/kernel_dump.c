	...
	for (i = 0; i < KDUMP_RESERVED_MAX; i++) {
		...
		if (g_kdump_reserve_mem[i].base != 0) {
			for (_phys = g_kdump_reserve_mem[i].base; _phys < (g_kdump_reserve_mem[i].base + g_kdump_reserve_mem[i].size); _phys += page_size) {
				page = (struct page *)(uintptr_t)phys_to_page((uintptr_t)_phys);
				SetPageMemDump(page);
				cnt++;
			}
		}
...
static void kernel_dump_printcb(struct memblock_type *print_mb_cb, struct kernel_dump_cb *cb)
{
	...
	pr_info("cb->kimage_voffset is 0x%llx\n", cb->kimage_voffset);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
s64 g_phystart_addr __ro_after_init = -1;
void dfx_set_phystart_addr(s64 addr)
{
	g_phystart_addr = addr;
}
...
#endif
/*
 * The size come from "setup_per_cpu_areas->pcpu_embed_first_chunk->pcpu_setup_first_chunk".
 * Each upgrade needs to check whether there is any change
 */
static size_t get_pcpu_base_size(void)
{
	size_t static_size = __per_cpu_end - __per_cpu_start;
	size_t reserved_size = PERCPU_MODULE_RESERVE;
	size_t size_sum = PFN_ALIGN(static_size + reserved_size +
		max_t(size_t, PERCPU_DYNAMIC_RESERVE, PERCPU_DYNAMIC_EARLY_SIZE));
	size_t dyn_size = size_sum - static_size - reserved_size;
	return static_size + reserved_size + dyn_size;
}
...
int kernel_dump_init(void)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	size_t k_pcpu_base_size = get_pcpu_base_size();
#endif
	...
	cb->struct_page_size = (u32)sizeof(struct page);
#ifdef CONFIG_RANDOMIZE_BASE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	cb->phys_offset = (u64)g_phystart_addr;
#else
	...
	cb->kimage_voffset = kimage_voffset;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	cb->extra_mem_size[1] = ALIGN((u64)k_pcpu_base_size, PAGE_SIZE)*num_possible_cpus();
#else
	cb->extra_mem_size[1] = ALIGN((u64)pcpu_base_size, PAGE_SIZE)*num_possible_cpus();
#endif
...
#error "Configurations other than CONFIG_PLATMEM and CONFIG_SPARSEMEM_VMEMMAP are not supported"
	...
	cb->mbr_size = (u64)sizeof(struct memblock_region);
