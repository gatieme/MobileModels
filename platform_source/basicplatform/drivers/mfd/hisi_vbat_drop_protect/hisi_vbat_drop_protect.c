...
static void hisi_vbat_drop_restore_cluster_freq(struct hvdev *hvdev)
{
	...
	ret = hv_set_freq(hvdev, 0xFFFFFFFF);
	if (!ret)
...
#ifdef CONFIG_HUAWEI_DSM
void hisi_vbat_drop_dmd_report(void)
{
	int ret;
	...
	ret = snprintf_s(buf, POWER_DSM_BUF_SIZE_0512, POWER_DSM_BUF_SIZE_0512 - 1,
		"[VBAT_DROP] brand: %s,voltage: %dmV,current: %dmA, capacity: %d,temperature: %d\n",
		brand, vol, curr, cap, temp);
	if (ret > 0)
...
static void hisi_vbat_drop_interrupt_work(struct work_struct *work)
{
...
#ifdef CONFIG_HUAWEI_DSM
	if (battery_check_cnt == 0)
		hisi_vbat_drop_dmd_report();
#endif
	...
	if (vbatt_mv > (int)di->vbat_drop_vol_mv + VBAT_DROP_VOL_INC_MV) {
		...
		bat_vol_normal_cnt++;
		/* Try a few times */
		if (bat_vol_normal_cnt > di->vbat_drop_vol_normal_cnt) {
#ifdef CONFIG_HW_VOTE
			...
			queue_delayed_work(system_power_efficient_wq,
				...
				msecs_to_jiffies(di->delayed_work_time)); /* default 2000ms */
		}
	} else {
		bat_vol_normal_cnt = 0;
		queue_delayed_work(system_power_efficient_wq,
...
static int hisi_vbat_drop_parse_dts(struct hisi_vbat_drop_protect_dev *di)
{
	...
	/* get vbat drop vol normal count */
	if (of_property_read_u32(
		    np, "vbat_drop_vol_normal_cnt", &di->vbat_drop_vol_normal_cnt)) {
		di->vbat_drop_vol_normal_cnt = VBAT_DROP_VOL_NORMAL_CNT;
		dev_err(di->dev, "[%s]: vbat_drop_vol_normal_cnt is not set , use default!\n", __func__);
	}

	/* get vbat drop delayed work time */
	if (of_property_read_u32(
		    np, "delayed_work_time", &di->delayed_work_time)) {
		di->delayed_work_time = DELAYED_WORK_TIME_DEFAULT;
		dev_err(di->dev, "[%s]: delayed_work_time is not set , use default!\n", __func__);
	}

	if (of_property_read_s32(np, "xpu_ctrl_en", &di->xpu_ctrl))
		di->xpu_ctrl = 0;
	if (di->xpu_ctrl) {
		dev_err(di->dev, "[%s]: xpu_ctrl_en %d\n", __func__, di->xpu_ctrl);
		return 0;
	}
...
static u32 vbat_drop_atoi(const char *s)
{
	const char *p = NULL;
	char c;
	u64 ret = 0;
...
static int dbg_control_show(struct seq_file *s, void *data)
{
	char show_info[] = "example:echo V 3900 > vbat_drop_ctr";

	seq_printf(s, "%s\n", show_info);
	return 0;
}

static void dbg_control_set_value_handler(
	struct hisi_vbat_drop_protect_dev *di,
	const char *tmp, size_t count)
{
	unsigned int i;
	const char *vol = NULL;

	if (tmp[0] == 'v' || tmp[0] == 'V') {
		for (i = 2; i < (count - 1); i++) { /* 2:jump v */
			if (tmp[i] != ' ') {
				vol = &tmp[i];
				break;
			}
		}
		di->vbat_drop_vol_mv = vbat_drop_atoi(vol);
		hisi_vbat_drop_vol_set(di->vbat_drop_vol_mv);
	}
}

static ssize_t dbg_control_set_value(struct file *filp,
	const char __user *buffer, size_t count, loff_t *ppos)
{
	struct seq_file *s = filp->private_data;
	struct hisi_vbat_drop_protect_dev *di = s->private;
	...
	dbg_control_set_value_handler(di, tmp, count);
...
static int dbg_control_open(struct inode *inode, struct file *file)
{
	return single_open(file, dbg_control_show, inode->i_private);
}

static const struct file_operations vbat_drop_control_fops = {
	.open       = dbg_control_open,
	.read       = seq_read,
	.write      = dbg_control_set_value,
	.llseek     = seq_lseek,
	.release    = single_release,
};

static void hisi_vbat_drop_debugfs_create(
	struct hisi_vbat_drop_protect_dev *di)
{
	struct dentry *d = NULL;
	struct dentry *file = NULL;

	d = debugfs_create_dir("hisi_vbat_drop_debugfs", NULL);
	if (!d) {
		pr_err("failed to create hisi vbat drop debugfs dir!\n");
		return;
	}

	file = debugfs_create_file("vbat_drop_ctr", S_IRUSR,
		d, di, &vbat_drop_control_fops);
	if (!file)
		goto err;

	return;
err:
	pr_err("vbat_drop debugfs init failed\n");
...
static int hisi_vbat_drop_protect_probe(struct platform_device *pdev)
{
...
#else
	di->vbatt_check_lock = wakeup_source_register("vbatt_drop_check_wake");
#endif
#ifdef CONFIG_HW_VOTE
	...
	HISI_VBAT_DROP_PMIC_REG_WRITE(
		PMIC_VSYS_DROP_IRQ_MASK_REG, ~PMIC_VSYS_DROP_IRQ_MASK);
#ifdef CONFIG_DFX_DEBUG_FS
	hisi_vbat_drop_debugfs_create(di);
#endif
