...
#include <platform_include/basicplatform/linux/mfd/spmi_fer.h>
	...
	unsigned int care_bit, unsigned int index, unsigned int bit)
{
	...
	(void)snprintf(cur_str + strlen(cur_str), PMIC_PRINT_BUF_SIZE,
	...
	} else if (unlikely(strstr(saved_command_line, "androidboot.swtype=factory"))) {
...
static void pmic_mntn_record_events(struct pmic_mntn_desc *pmic_mntn)
{
	...
	(void)snprintf(pmic_mntn->init_log_show, PMIC_PRINT_BUF_SIZE,
		"\nPMIC REGISTER DUMP\n");
	len -= strlen(pmic_mntn->init_log_show);
	(void)snprintf(pmic_mntn->init_log_show + strlen(pmic_mntn->init_log_show),
		len, "  addr     data\n");
	...
	for (index = 0; index < pmic_mntn->record_reg_n; index++) {
		...
		(void)snprintf(pmic_mntn->init_log_show +
	...
	int irq, void *data)
{
	...
	spmi_fer_irq_ocp_mask(pmic_mntn->fer_num);
...
static void pmic_mntn_ocp_wq_handler(struct work_struct *work)
{
	...
	spmi_fer_irq_ocp_unmask(pmic_mntn->fer_num);
...
static void pmic_mntn_otmp_wk_handler(struct work_struct *work)
{
	...
	memset((void *)pmic_mntn->irq_log_show, '\0', PMIC_PRINT_BUF_SIZE);
	(void)snprintf(pmic_mntn->irq_log_show, PMIC_PRINT_BUF_SIZE,
	...
	struct pmic_mntn_desc *pmic_mntn)
{
	...
	for (index = 0; index < pmic_mntn->ocp_reg_n; index++) {
		(void)snprintf(compatible_string, PMIC_DTS_ATTR_LEN,
	...
	struct pmic_mntn_desc *pmic_mntn)
{
	...
	ret = of_property_read_u32(
		np, "pmic-mnt-fer-flag", (u32 *)&pmic_mntn->fer_num);
	if (ret)
		pmic_mntn->fer_num = PMU_INVALID_FER_NUM;
...
int pmic_mntn_drv_init(struct pmic_mntn_desc *pmic_mntn)
{
	...
	if (!strcmp(np->parent->name, "pmic1")) {
