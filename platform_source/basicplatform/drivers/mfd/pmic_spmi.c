...
static struct vendor_pmic *g_comm_pmic;
...
{
	unsigned int i;
	unsigned char val[VERSION_LEN] = {0};
	unsigned short version;

	for (i = 0; i < VERSION_LEN; i++)
		*(val + i) = pmic_read_reg(PMU_VERSION_ADDR + i);

	version = (val[4] << VERSION_HIGH) | val[5];

	pr_err("%s version is 0x%x\n", __func__, version);
...
#define BB_CFG_REG (0x915)
#define BB_PFM_MODE 1
#define BB_CCM_MODE 0
void pmic_set_buckboost_mode(int mode)
{
	unsigned int val = 0;

	val = sub_pmic_reg_read(BB_CFG_REG);
	if (mode == BB_CCM_MODE) {
		val &= (~BIT(1));
	} else if (mode == BB_PFM_MODE) {
		val |= (BIT(1));
	} else {
		pr_err("%s mode is %d error\n", __func__, mode);
		return;
	}

	pr_info("%s mode is %d, val 0x%x\n", __func__, mode, val);
	sub_pmic_reg_write(BB_CFG_REG, val);
}
EXPORT_SYMBOL(pmic_set_buckboost_mode);
...
static int of_irq_get_byfer(struct device_node *np, struct vendor_pmic *pmic)
{
	struct device *dev = pmic->dev;

	pmic->irq = of_irq_get_byname(np, "fer");
	if (pmic->irq < 0) {
		dev_err(dev, "[%s] of_irq_get_byname irq failed\n",
			__func__);
		return -ENODEV;
	}
	dev_err(dev, "[%s] of_irq_get_byname irq %d\n",
		__func__, pmic->irq);
	return 0;
}

static int of_irq_get_bygpio(struct device_node *np, struct vendor_pmic *pmic,
	const char *name)
{
	...
	if (pmic->gpio < 0) {
		pr_err("[%s] of_get_gpio_flags %d\n", __func__, pmic->gpio);
		return pmic->gpio;
	}

	if (!gpio_is_valid(pmic->gpio)) {
		pr_err("[%s] gpio_is_valid\n", __func__);
		return -EINVAL;
	}
	...
	struct spmi_device *pdev, struct vendor_pmic *pmic, const char *name)
{
	...
	ipar = of_get_property(np, "interrupt-parent", &statlen);
	if (ipar != NULL) {
		ret = of_irq_get_byfer(np, pmic);
		if (ret != 0)
			return -EINVAL;
		itype = IRQ_FER;
		flags = IRQF_TRIGGER_HIGH | IRQF_NO_SUSPEND;
	} else {
		ret = of_irq_get_bygpio(np, pmic, name);
		if (ret < 0)
			return ret;
		itype = IRQ_GPIO;
		flags = IRQF_TRIGGER_LOW | IRQF_SHARED | IRQF_NO_SUSPEND;
	}
...
success:
	if (of_device_is_compatible(np, SPMI_MMW_PMIC_COMP))
		...
		g_sub_pmic = pmic;
	else if (of_device_is_compatible(np, SPMI_PMIC_COMP))
		g_pmic = pmic;
	else
		g_comm_pmic = pmic;
...
static const struct of_device_id of_spmi_pmic_match_tbl[] = {
	...
	{ .compatible = SPMI_COMMON_PMIC_COMP, },
	{  }   /* end */
};
