...
#include <platform_include/basicplatform/linux/mmc/hw_write_protect.h>
...
void dsm_set_sdcard_device_info(const unsigned manfid, const unsigned int serial)
{
	int ret = 0;
	ret = snprintf(sdcard_dclient->module_name, DSM_MAX_MODULE_NAME_LEN, "%06x", manfid);
	if (ret < 0) {
		printk(KERN_ERR "manfid error");
		memset(sdcard_dclient->module_name, 0, sizeof(sdcard_dclient->module_name));
	}
	ret = snprintf(sdcard_dclient->ic_name, DSM_MAX_IC_NAME_LEN, "%08x", serial);
	if (ret < 0) {
		printk(KERN_ERR "serial error");
		memset(sdcard_dclient->ic_name, 0, sizeof(sdcard_dclient->ic_name));
	}
	printk(KERN_INFO "sdcard info manfid is %s, psn is %s", sdcard_dclient->module_name, sdcard_dclient->ic_name);
}
