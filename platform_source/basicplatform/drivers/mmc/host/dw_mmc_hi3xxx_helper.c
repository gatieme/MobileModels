	...
	int *use_sam_dly, int *enable_shift)
{
	...
	if (host->use_samdly_flag) {
		/* clk_change tuning use_samdly_range */
...
#ifndef CONFIG_SD_SDIO_CRC_RETUNING
static void dw_mci_shiften(struct dw_mci *host, int sam_phase_val,  int *enable_shift)
{
	int start, end;

	start = host->enable_shift_range[0];
	end = host->enable_shift_range[1];

	/*
	 * phase min: 0,phase max: 9
	 * enable_shift_range = <4 8>  [4,8] = 1, {0-3,9}  = 0
	 */
	if ((start <= end) && (start <= sam_phase_val) &&
		(sam_phase_val <= end)) {
		*enable_shift = 1;
		return;
	}

	/* enable_shift_range = <8 4>  (4,8) = 0, {8,9,0-4}  = 1 */
	if ((start > end) && (start <= sam_phase_val ||
		sam_phase_val <= end))
		*enable_shift = 1;
}
	...
	int *use_sam_dly, int *enable_shift)
{
	...
	*enable_shift = 0;
...
#else
	/* only used in Aries_cs platform */
	if (host->is_cs_timing_config == IS_CS_TIMING_CONFIG) {
		/* Aries_cs tuning use_sam_dly range */
		if (sam_phase_val >= 4 && sam_phase_val <= 8)
			*use_sam_dly = 1;
		/* Aries_cs tuning enable_shift range */
	...
	if (host->enable_shift_flag) {
		dw_mci_shiften(host, sam_phase_val, enable_shift);
	} else {
		/* default tuning enable_shift range */
...
{
	/* no need to dump message, use zodiac dump */
}
...
int dw_mci_sdio_wakelog_switch(struct mmc_host *mmc, bool enable)
{
	...
	slot->sdio_wakelog_switch = slot->sdio_wakelog_switch &&
		(MMC_CAP3_SUPPORT_WIFI & (mmc->caps3));
...
#ifdef CONFIG_SD_SDIO_CRC_RETUNING
/*
 * name:dw_mci_zodiac_change_timing_config_cancer
 * func: the soc platform of cancer when sd or sdio CRC issue,
 * will excute here to set SDR104 timing config for retuning
 */
static void dw_mci_zodiac_change_timing_config_cancer(struct dw_mci *host)
{
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		...
		/* cancer SDR104M bus_clock parameter */
...
/*
 * name: dw_mci_zodiac_change_timing_config_capricorn
 * func: the soc platform of capricorn when sd or sdio CRC issue,
 * will excute here to set SDR104 timing config for retuning
 */
static void dw_mci_zodiac_change_timing_config_capricorn(struct dw_mci *host)
{
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		memcpy(hs_timing_config[DWMMC_SD_ID][MMC_TIMING_UHS_SDR104],
			hs_timing_960_clock_source_capricorn,
			sizeof(hs_timing_960_clock_source_capricorn)); /* unsafe_function_ignore: memcpy */
		/* capricorn bus_clock parameter */
		host->use_samdly_range[RANGE_MIN] = 4;
		host->use_samdly_range[RANGE_MAX] = 8;
		host->enable_shift_range[RANGE_MIN] = 4;
		host->enable_shift_range[RANGE_MAX] = 8;
...
/*
 * name:dw_mci_zodiac_change_timing_config_aries
 * func: the soc platform of aries when sd or sdio CRC issue,
 * will excute here to set SDR104 timing config for retuning
 */
static void dw_mci_zodiac_change_timing_config_aries(struct dw_mci *host)
{
	if (host->hw_mmc_id == DWMMC_SD_ID) {
		memcpy(hs_timing_config[DWMMC_SD_ID][MMC_TIMING_UHS_SDR104],
...
void dw_mci_hs_change_timing_config(struct dw_mci *host)
{
	...
	if (of_device_is_compatible(np, "hisilicon,capricorn-dw-mshc"))
		dw_mci_zodiac_change_timing_config_capricorn(host);
	else if (of_device_is_compatible(np, "hisilicon,cancer-dw-mshc"))
		dw_mci_zodiac_change_timing_config_cancer(host);
	else if (of_device_is_compatible(np, "hisilicon,libra-dw-mshc"))
		dw_mci_zodiac_change_timing_config_libra(host);
	else if (of_device_is_compatible(np, "hisilicon,taurus-dw-mshc"))
		dw_mci_zodiac_change_timing_config_taurus(host);
	else
		dw_mci_zodiac_change_timing_config_aries(host);
}
#endif
...
static int dw_mci_send_tuning(struct dw_mci_slot *slot, u32 opcode, u32 blksz,
...
handle:
	if (host->flags & TIMEOUT_ENABLE_PIO) {
		...
		if (host->timeout_cnt == TIMEOUT_MAX_NUM)
