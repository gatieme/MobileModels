 ...
 * Copyright (c) Zodiac Technologies Co., Ltd. 2018-2019. All rights reserved.
 *
...
void register_gpio_number_group(int gpionum)
{
	if (gpionum <= 0)
		return;
	/* sd function pin-num increases by 1 */
	gpio_current_number_for_sd_clk = gpionum;
	gpio_current_number_for_sd_cmd = gpionum + 1;
	gpio_current_number_for_sd_data0 = gpionum + 2;
	gpio_current_number_for_sd_data1 = gpionum + 3;
	gpio_current_number_for_sd_data2 = gpionum + 4;
	gpio_current_number_for_sd_data3 = gpionum + 5;

	if (gpionum == GPIO_104) {
		iomg_base_gpio_for_sd_clk = IOMG_BASE_GPIO_104;
		iocg_base_gpio_for_sd_clk = IOCG_BASE_GPIO_104;
	} else if (gpionum == GPIO_160) {
		iomg_base_gpio_for_sd_clk = IOMG_BASE_GPIO_160;
		iocg_base_gpio_for_sd_clk = IOCG_BASE_GPIO_160;
	} else if (gpionum == GPIO_120) {
...
static void config_sdsim_levershit_sim_normal(void)
{
	set_sd_sim_group_io_register(gpio_current_number_for_sd_clk, FUNCTION4,
		PULL_TYPE_NP, DRIVER_STRENGTH_4MA_0);
	set_sd_sim_group_io_register(gpio_current_number_for_sd_cmd, FUNCTION0,
		PULL_TYPE_PD, DRIVER_STRENGTH_4MA_0);
	set_sd_sim_group_io_register(gpio_current_number_for_sd_data0,
			...
			int cmd1_result, struct mmc_host *mmc_host_temp, detect_gpio_status_array_s *current_state)
{
	...
	if (!cmd1_result) {
		...
		if (current_module == MODULE_SD) {
			...
			if (host->lever_shift) {
				if (host->flags & WIFI_ONLY_GPIO_NP) {
					switch_gpio_change(SWITCH_GPIO_SD_SIDE);
					config_sdsim_gpio_mode(SDSIM_MODE_SD_NORMAL);
					pr_info("%s  wifi only set gpio state to PU", __func__);
				} else if (host->flags & SIMCARD_CMD_PD) {
					switch_gpio_change(SWITCH_GPIO_SIM_SIDE);
					config_sdsim_levershit_sim_normal();
					pr_info("%s  levershit set cmd gpio state to PD", __func__);
				}
			} else {
#ifndef CONFIG_HISI_ESIM
				switch_gpio_change(SWITCH_GPIO_SIM_SIDE);
...
static int sd_sim_plug_out(void)
{
	...
	if (dw_mci_check_zodiacmntn(MNTN_SD2JTAG) ||
...
{
	return (dw_mci_check_zodiacmntn(MNTN_SD2JTAG) == 1 ||
		dw_mci_check_zodiacmntn(MNTN_SD2DJTAG) == 1 ||
		dw_mci_check_zodiacmntn(MNTN_SD2UART6) == 1);
}
