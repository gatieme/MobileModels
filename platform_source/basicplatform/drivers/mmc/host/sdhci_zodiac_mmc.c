...
#include <platform_include/basicplatform/linux/mmc/cmdq_hci.h>
#include <linux/pm_runtime.h>
#include <linux/mmc/card.h>
#include <linux/regulator/consumer.h>
#include <platform_include/basicplatform/linux/rdr_pub.h>
#include <mntn_subtype_exception.h>
#include <platform_include/basicplatform/linux/mmc/dsm_emmc.h>
#include <linux/mmc/sdio.h>
#include <linux/mmc/sd.h>
#ifdef CONFIG_MMC_HSQ
#include "mmc_hsq.h"
#endif
#include <platform_include/basicplatform/linux/mmc/sdhci_zodiac_mux_sdsim.h>

#include "sdhci-pltfm.h"
#include <platform_include/basicplatform/linux/mmc/sdhci_zodiac_mmc.h>
#include <platform_include/basicplatform/linux/mmc/sdhci_zodiac_mmc_antlia.h>

#define DRIVER_NAME "sdhci-zodiac"
struct sdhci_host *sdhci_host_from_sd_module;
struct sdhci_host *sdhci_sdio_host;

static u32 con_clk = SDHCI_ZODIAC_CONTROLLER_CLK_FREQ;
int g_sdio_reset_ip;
...
static void sdhci_zodiac_clock_enable(struct device *dev, struct sdhci_zodiac_data *sdhci_zodiac)
{
	if (!IS_ERR(sdhci_zodiac->biu_clk)) {
		if (clk_prepare_enable(sdhci_zodiac->biu_clk))
			dev_err(dev, "Unable to enable biu clock.\n");
	}
	dev_info(dev, "%s: sdhci_zodiac->biu_clk=%lu.\n", __func__, clk_get_rate(sdhci_zodiac->biu_clk));

	if (!IS_ERR(sdhci_zodiac->niu_clk)) {
		if (clk_prepare_enable(sdhci_zodiac->niu_clk))
			dev_err(dev, "Unable to enable niu clock.\n");
	}
	dev_info(dev, "%s: sdhci_zodiac->niu_clk=%lu.\n", __func__, clk_get_rate(sdhci_zodiac->niu_clk));

	if (!IS_ERR(sdhci_zodiac->ciu_clk)) {
		if (clk_prepare_enable(sdhci_zodiac->ciu_clk))
			dev_err(dev, "Unable to enable SD clock.\n");
	}
	dev_info(dev, "%s: sdhci_zodiac->ciu_clk=%lu.\n", __func__, clk_get_rate(sdhci_zodiac->ciu_clk));
}

static void sdhci_zodiac_clock_disable(struct sdhci_zodiac_data *sdhci_zodiac)
{
	if (!IS_ERR(sdhci_zodiac->ciu_clk))
	...
	if (!IS_ERR(sdhci_zodiac->niu_clk))
		clk_disable_unprepare(sdhci_zodiac->niu_clk);
}
...
static void sdhci_zodiac_select_clk_src(struct sdhci_host *host, struct mmc_ios *ios)
{
	...
	/* The spread spectrum function selects a dedicated clock source. */
	if (ios->timing != MMC_TIMING_LEGACY)
		sdhci_zodiac_sd_ssc_config(host);
	else
		sdhci_zodiac_sd_ssc_disconfig(host);

	reg = sdhci_readl(host, SDHCI_CRG_CTRL0);
	reg &= ~(SDHCI_ZODIAC_CLK_DLY_DRV_MASK | SDHCI_ZODIAC_CLK_DIV_MASK);
	reg |= (crg->clk_div << SDHCI_ZODIAC_CLK_DIV_OFFSET) | \
			(crg->clk_dly_drv << SDHCI_ZODIAC_CLK_DLY_DRV_OFFSET) | \
			(crg->sample_mode << SDHCI_ZODIAC_CLK_SAMPLE_MODE_OFFSET);
...
#ifdef CONFIG_MMC_HSQ
static void sdhci_zodiac_request_done(struct sdhci_host *host,
...
static int sdhci_zodiac_soft_tuning(struct sdhci_host *host, u32 opcode, bool set)
{
	...
	if (set == false) {
		...
		sdhci_zodiac->hw_tuning_phase_best = sdhci_readl(host,TUNING_RESULT_VALUE);
...
static void sdhci_zodiac_power_off(struct sdhci_host *host)
{
	...
	if ((sdhci_zodiac->sdhci_host_id == SDHCI_SD_ID) && sdhci_zodiac->mux_sdsim) {
		if (sd_sim_detect_status_current == SD_SIM_DETECT_STATUS_SIM) {
			dev_info(&host->mmc->class_dev, "%s %s SIM has detected,but SD "
...
static void sdhci_zodiac_power_up(struct sdhci_host *host)
{
	...
	/*
	 * Some NM cards fail to be identified due to insufficient delay when switching between 1.8V and 3V.
	 * The NM card manufacturer recommends 250ms delay
	 */
	if (sdhci_zodiac->mux_sdsim && sdhci_zodiac->mux_vcc_status == MUX_SDSIM_VCC_STATUS_2_9_5_V)
...
void sdhci_zodiac_put_regulator(struct sdhci_host *host)
{
	...
	if (sdhci_zodiac->sdhci_host_id == SDHCI_SD_ID) {
		if (mmc->bus_suspend) {
			dev_info(&host->mmc->class_dev, "suspend no need put regulator\n");
			return;
		}

		if (mmc->supply.vqmmc) {
			devm_regulator_put(mmc->supply.vqmmc);
			mmc->supply.vqmmc = NULL;
		}

		if (mmc->supply.vmmc) {
			devm_regulator_put(mmc->supply.vmmc);
			mmc->supply.vmmc = NULL;
		}

		sdhci_zodiac->need_get_mmc_regulator = 1;
		dev_info(&host->mmc->class_dev, "regulator put\n");
	}
}
...
void sdhci_zodiac_get_regulator(struct sdhci_host *host)
{
	...
	if ((sdhci_zodiac->sdhci_host_id == SDHCI_SD_ID) &&
			(sdhci_zodiac->need_get_mmc_regulator == 1)) {
		mmc->supply.vmmc = devm_regulator_get(mmc->parent, "vmmc");
		if (IS_ERR(mmc->supply.vmmc)) {
				dev_info(&host->mmc->class_dev, "no vmmc regulator found\n");
				mmc->supply.vmmc = NULL;
		}

		mmc->supply.vqmmc = devm_regulator_get(mmc->parent, "vqmmc");
		if (IS_ERR(mmc->supply.vqmmc)) {
				dev_info(&host->mmc->class_dev, "no vqmmc regulator found\n");
				mmc->supply.vqmmc = NULL;
		}

		sdhci_zodiac->need_get_mmc_regulator = 0;
		dev_info(&host->mmc->class_dev, "regulator get\n");
	}
}
#endif
...
#ifdef CONFIG_MMC_SIM_GPIO_EXCHANGE
		sdhci_zodiac_put_regulator(host);
#endif
		break;
	case MMC_POWER_UP:
#ifdef CONFIG_MMC_SIM_GPIO_EXCHANGE
		sdhci_zodiac_get_regulator(host);
#endif
...
#ifdef CONFIG_MMC_SDHCI_MUX_SDSIM
	if (sdhci_zodiac->sdhci_host_id == SDHCI_SD_ID) {
		if (sdhci_zodiac->first_mux_sdsim_probe_init) {
			status = sd_sim_detect_run(host, status, MODULE_SD,
					SLEEP_MS_TIME_FOR_DETECT_UNSTABLE);
		} else {
			dev_info(&host->mmc->class_dev, " jump sd_sim_detect_run because "
				"first_mux_sdsim_probe_init now\n");
			sdhci_zodiac->first_mux_sdsim_probe_init = CARD_ON;
		}
#ifdef CONFIG_DFX_DEBUG_FS
		if (host->mmc->sim_remove_nano) {
			sd_sim_detect_status_current = SD_SIM_DETECT_STATUS_UNDETECTED;
			dev_info(&host->mmc->class_dev, " nano sd remove test: sd_sim_detect_status_current = 0\n");
			status = 1;
			dev_err(&host->mmc->class_dev, "nano sd test sim_remove_nano = 1\n");
		}
#endif
	}
#endif
...
#ifdef CONFIG_MMC_HSQ
	.request_done = sdhci_zodiac_request_done,
#endif
};
...
static int sdhci_zodiac_suspend(struct device *dev)
{
	...
	/*
	 * during the host suspend, if card is inserted and init first
	 * then resume host the card will fail to be identified.
	 */
	if (sdhci_zodiac->gpio_cd) {
		disable_irq(gpio_to_irq(sdhci_zodiac->gpio_cd));
		cancel_work_sync(&sdhci_zodiac->card_work);
		dev_info(dev, " disable gpio detect\n");
	}
	...
	sdhci_zodiac->old_power_mode = MMC_POWER_OFF;
	sdhci_zodiac_disable_clk(host);
	if (!(host->quirks2 & SDHCI_QUIRK2_ZODIAC_FPGA))
		sdhci_zodiac_clock_disable(sdhci_zodiac);
...
static int sdhci_zodiac_resume(struct device *dev)
{
	...
	if (!(host->quirks2 & SDHCI_QUIRK2_ZODIAC_FPGA))
		sdhci_zodiac_clock_enable(dev, sdhci_zodiac);
	...
	if (host->mmc->ios.timing >= MMC_TIMING_MMC_HS200) {
		dev_info(dev, "%s: tuning_move_sample=%d, tuning_init_sample=%d\n",
			__func__, sdhci_zodiac->tuning_move_phase, sdhci_zodiac->hw_tuning_phase_best);
		 if (!sdhci_zodiac->tuning_move_phase)
			sdhci_zodiac->tuning_move_phase = sdhci_zodiac->hw_tuning_phase_best;
		sdhci_zodiac_set_tuning_phase(host, sdhci_zodiac->tuning_move_phase);
	...
	if (sdhci_zodiac->gpio_cd) {
		enable_irq(gpio_to_irq(sdhci_zodiac->gpio_cd));
		dev_info(dev, " enable gpio detect\n");
	}
...
static int sdhci_zodiac_runtime_resume(struct device *dev)
{
	...
	ret = sdhci_zodiac_runtime_resume_host(host);
#ifdef CONFIG_MMC_HSQ
	if (!ret)
		ret = mmc_hsq_resume(host->mmc);
#endif
...
static void sdhci_zodiac_populate_dt(struct platform_device *pdev)
{
	...
	if (sdhci_zodiac->mux_sdsim) {
		if (of_property_read_u32(np, "gpionumber_sdclk", &value)) {
			dev_info(&pdev->dev, "%s: %s gpionumber_sdclk property not found, using value of 320 as default\n",
				dev_name(sdhci_zodiac->dev), __func__);
...
static int sdhci_zodiac_clock_init(struct sdhci_host *host, struct platform_device *pdev, struct sdhci_zodiac_data *sdhci_zodiac)
{
	...
	if (sdhci_zodiac->sdhci_host_id == SDHCI_EMMC_ID) {
		sdhci_zodiac->niu_clk = devm_clk_get(&pdev->dev, "niu");
		if (IS_ERR(sdhci_zodiac->niu_clk)) {
			dev_err(&pdev->dev, "niu clock not found.\n");
			ret = PTR_ERR(sdhci_zodiac->niu_clk);
			return ret;
		}
	}
	...
	if (of_device_is_available(np)) {
		if (of_property_read_u32(np, "clk-xin", &con_clk))
			dev_err(&pdev->dev, "clk-xin cannot get from dts, use the default value!\n");
	}

	ret = clk_set_rate(sdhci_zodiac->ciu_clk, con_clk);
	if (ret)
		dev_err(&pdev->dev, "Error setting desired con_clk=%u, get clk=%lu\n",
			con_clk, clk_get_rate(sdhci_zodiac->ciu_clk));

	sdhci_zodiac_clock_enable(&pdev->dev, sdhci_zodiac);
...
static irqreturn_t sdhci_zodiac_card_detect(int irq, void *data)
{
	...
	struct sdhci_host *host = sdhci_zodiac->host;
	struct mmc_host *mmc = host->mmc;
	if (mmc->sim_remove_nano)
		mmc->sim_remove_nano = 0;
#endif
...
static int sdhci_zodiac_probe(struct platform_device *pdev)
{
	...
	if (sdhci_zodiac_get_cd_status(host))
		dev_info(&host->mmc->class_dev, "card is present\n");
	else
		dev_info(&host->mmc->class_dev, "card is not present\n");
