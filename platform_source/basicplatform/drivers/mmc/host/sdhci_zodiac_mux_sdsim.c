...
unsigned int iomg_base_gpio_pd_for_sd_clk;
...
int sd_clk_driver_strength = SD_CLK_DRIVER_DEFAULT;
int sd_cmd_driver_strength = SD_CMD_DRIVER_DEFAULT;
int sd_data_driver_strength = SD_DATA_DRIVER_DEFAULT;
...
void register_gpio_number_group(int start_gpio_number_for_sd_clk)
{
	...
	if (start_gpio_number_for_sd_clk == GPIO_320) {
		iomg_base_gpio_for_sd_clk = IOMG_BASE_GPIO_128;
		iomg_base_gpio_pd_for_sd_clk = IOMG_BASE_GPIO_PD_128;
		iocg_base_gpio_for_sd_clk = IOCG_BASE_GPIO_128;
	}
}
	...
	int gpionumber, int function_select, int pulltype, int driverstrenth)
{
	...
	static void *iomg_pd_ctrl_reg;

	pulltype_this = (unsigned int)pulltype;
	driverstrenth_this = (unsigned int)driverstrenth;

	if (!iomg_base_gpio_for_sd_clk || !iocg_base_gpio_for_sd_clk || !iomg_base_gpio_pd_for_sd_clk)
		return;
	...
	if (unlikely(!iomg_pd_ctrl_reg))
		iomg_pd_ctrl_reg = ioremap(iomg_base_gpio_pd_for_sd_clk, 0x1000);
	...
	/* iomg_pd_ctrl_reg: set 0x1 to use gpio func, set 0 to use gpio */
	if (function_select)
		writel(0x1, iomg_pd_ctrl_reg + 0x004 * (gpionumber - gpio_current_number_for_sd_clk));
	else
		writel(0x0, iomg_pd_ctrl_reg + 0x004 * (gpionumber - gpio_current_number_for_sd_clk));
...
static void sd_sim_get_4pin_state(struct sdhci_host *host, detect_gpio_status_array_s *current_state)
{
	...
	if (sdhci_zodiac->need_get_mmc_regulator) {
...
int sd_sim_send_cmd_detect(struct sdhci_host *host, int current_module)
{
	...
	int cmd1_result, result, present;
	...
	sdhci_zodiac->mux_vcc_status = MUX_SDSIM_VCC_STATUS_1_8_0_V;
	present = sdhci_zodiac->card_detect;
	sdhci_zodiac->card_detect = CARD_ON;
	cmd1_result = mmc_detect_sd_or_mmc(mmc_host_temp);

	if (!present)
		sdhci_zodiac->card_detect = CARD_REMOVED;
	else
		sdhci_zodiac->card_detect = present;
