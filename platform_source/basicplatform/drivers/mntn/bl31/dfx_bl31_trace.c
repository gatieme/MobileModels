...
static noinline u64 __kernel_cpuid_notify_bl31(u64 cpu)
{
	...
	arm_smccc_smc(MNTN_BB_CPUID_FID_VALUE, cpu, 0, 0, 0, 0, 0, 0, &res);
...
u64 bl31_disable_dmss_int_smc(void)
{
	register u64 x0 asm("x0") = (u64)MNTN_DISABLE_DMSS_INT_VALUE;
...
static noinline u64 kernel_init_notify_bl31(void)
{
	...
	arm_smccc_smc(MNTN_BB_BL31INIT_FID_VALUE, 0, 0, 0, 0, 0, 0, 0, &res);
...
static int bl31_trace_irq_smc_init(u8 *phy_addr, u8 *virt_addr, u32 log_len)
{
	...
	int ret;
	u32 ratio[DFX_HOOK_CPU_NUMBERS][DFX_HOOK_CPU_NUMBERS] = BL31_RECORD_RATIO; /* HERE:[8][8] */

	if (unlikely(cpu_num > DFX_HOOK_CPU_NUMBERS)) {
		BB_PRINT_ERR("[%s] fail, invalid cpu num %llu\n", __func__, cpu_num);
		return -1;
	}

	head_len = (u64)(sizeof(bl31_trace_irq_smc_head_t) + sizeof(u64) * cpu_num);
	BB_PRINT_DBG("[%s], num_online_cpus [%llu] head_len %llu field_count %u!\n",
		__func__, cpu_num, head_len, (u32) sizeof(bl31_trace_t));

	min_size = (u64)(head_len + cpu_num * (sizeof(struct dfx_ap_ringbuffer_s) +
		PERCPU_TOTAL_RATIO * sizeof(bl31_trace_t)));
	...
	for (i = 0; i < cpu_num; i++) {
		...
		size = ((log_len - head_len) / PERCPU_TOTAL_RATIO) * ratio[cpu_num - 1][i];
		size = align8(size);

		head->offset[i] = offset;

		ret = dfx_ap_ringbuffer_init((struct dfx_ap_ringbuffer_s *)(virt_addr + offset),
...
{
	u32 min_size = (u32)(sizeof(struct dfx_ap_ringbuffer_s) + sizeof(dmss_exception_trace_t));
...
static int bl31_trace_addr_len_set(u64 addr, u64 len)
{
	...
	bl31_ctrl_addr = (u8 *) ioremap(ATF_SUB_RESERVED_BL31_SHARE_MEM_PHYMEM_BASE + data[0], (u64) data[1]);
...
static int bl31_cleartext_exception_print(const char *dir_path, u64 log_addr, u32 log_len)
{
	...
	struct dfx_ap_ringbuffer_s temp;
	rdr_exception_trace_t *trace = NULL;
	struct file *fp = NULL;
	bool error = false;
	u32 start, end, i;

	q = (struct dfx_ap_ringbuffer_s *)(uintptr_t) log_addr;
				...
				u8 *addr, u32 len, u32 cpu)
{
	...
	q = (struct dfx_ap_ringbuffer_s *)addr;
	if (memcpy_s(&temp, sizeof(temp), q, sizeof(*q)) != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return -1;
	}
