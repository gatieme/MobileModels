...
struct rdr_exception_info_s g_einfo[] = {
	...
	{ { 0, 0 }, MODID_AP_S_PANIC_IO_DIE_STS, MODID_AP_S_PANIC_IO_DIE_STS, RDR_ERR,
	 RDR_REBOOT_NOW, RDR_AP, RDR_AP, RDR_AP,
	 (u32)RDR_REENTRANT_DISALLOW, (u32)AP_S_PANIC, HI_APPANIC_IO_DIE_STS, (u32)RDR_UPLOAD_YES,
	 "ap", "ap", 0, 0, 0 },
#endif
};
...
static int vm_reboot_reason = 0;
#define HVC_FN_SET_REBOOT_REASON 0x20230301
#endif
...
static unsigned char *get_rdr_ap_dump_start_addr(void)
{
	...
	unsigned int timers = (unsigned int)(sizeof(*g_rdr_ap_root) / SIZE_1K + 1);
...
int __init hook_buffer_alloc(void)
{
	...
	percpu_arr_size = (unsigned int)ARRAY_SIZE(g_trace_buffer_percpu);
	single_arr_size = (unsigned int)ARRAY_SIZE(g_trace_buffer_single);
...
static int __init ap_dump_buffer_init(void)
{
	...
	/* Task record area */
	if (get_ap_last_task_switch_from_dts()) {
		g_last_task_struct_size = (unsigned int)sizeof(struct task_struct);
...
u64 rdr_get_last_wdt_kick_slice(void)
{
	...
	for (i = 1; i < WDT_KICK_SLICE_TIMES; i++)
		last_kick_slice = (u64)max(last_kick_slice, g_rdr_ap_root->wdt_kick_slice[i]);
...
void save_bl31_exc_memory(void)
{
	...
	len = (unsigned int)strlen(fullpath_arr);
	iret = memcpy_s(dst_str, LOG_PATH_LEN, fullpath_arr, len + 1);
	if (iret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
...
void save_iomcu_logbuff(void)
{
	...
	(void)memset_s(fullpath_arr, LOG_PATH_LEN, 0, LOG_PATH_LEN);
	(void)memset_s(dst_str, LOG_PATH_LEN, 0, LOG_PATH_LEN);
	...
	len = (unsigned int)strlen(fullpath_arr);
	iret = memcpy_s(dst_str, LOG_PATH_LEN - 1, fullpath_arr, len + 1);
	if (iret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
	}
	strncat(&dst_str[len], "/iomcu_logbuff.bin", (LOG_PATH_LEN - 1 - len));
	iret = rdr_copy_file_apend(dst_str, SRC_IOMCU_LOGBUFF_MEMORY);
	if (iret)
		BB_PRINT_ERR("[%s], save iomcu_logbuff.bin error, ret = %d\n", __func__, iret);
}
#endif
...
void save_thee_exc_memory(void)
{
	int iret;
	unsigned long len;
	char fullpath_arr[LOG_PATH_LEN] = "";
	...
	len = (unsigned long)strlen(fullpath_arr);
	iret = memcpy_s(dst_str, LOG_PATH_LEN, fullpath_arr, len + 1);
	if (iret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
	}
	strncat(&dst_str[len], "/thee_mntn_mem.bin",
		((LOG_PATH_LEN - 1) - len));
	iret = rdr_copy_file_apend(dst_str, SRC_THEE_MNTN_MEMORY);
	if (iret)
		BB_PRINT_ERR("[%s], save thee_mntn_memory.bin error, ret = %d\n", __func__, iret);
}
#endif
...
void save_kernel_dump(void *arg)
{
	...
	if (arg && (LOG_PATH_LEN - 1 >= strlen(g_log_path))) {
		len = (u32)strnlen(g_log_path, LOG_PATH_LEN);
...
#ifdef CONFIG_RESET_VM_SOLO
static void vm_set_reboot_reason(unsigned int reboot_reason)
{
	struct arm_smccc_res res = {0};

	vm_reboot_reason = reboot_reason;
	BB_PRINT_PN("%s: arm_smccc_hvc begin reboot_reason[%u]\n", __func__, reboot_reason);
	arm_smccc_hvc(HVC_FN_SET_REBOOT_REASON, reboot_reason, 0, 0, 0, 0, 0, 0, &res);
...
void vm_set_reboot_reason_subtype(unsigned int reboot_reason, unsigned int subtype)
{
	...
	vm_reboot_reason = reboot_reason;
	BB_PRINT_PN("%s: arm_smccc_hvc begin reboot_reason[%u], subtype[%u]\n", __func__, reboot_reason, subtype);
	arm_smccc_hvc(HVC_FN_SET_REBOOT_REASON, reboot_reason, subtype, 0, 0, 0, 0, 0, &res);
	BB_PRINT_PN("%s: arm_smccc_hvc return res.a0[0x%lx]\n", __func__, res.a0);
}
#endif
...
{
#ifdef CONFIG_RESET_VM_SOLO
	vm_set_reboot_reason(reboot_reason);
#else
	pmic_set_reboot_reason(reboot_reason);
#endif
}
...
{
#ifdef CONFIG_RESET_VM_SOLO
	vm_set_reboot_reason_subtype(reboot_reason, subtype);
#else
	pmic_set_reboot_reason_subtype(reboot_reason, subtype);
#endif
}
#endif
...
void rdr_ap_register_logbuf_dump(void)
{
	...
	g_logbuff_dump_info->idx_size = (unsigned int)sizeof(u32);
...
#define combine_32bits_to_64bits(high, low)    ((((high) & 0xFFFFFFFF) << 32) + ((low) & 0xFFFFFFFF))

static void set_logringbuffer_addr_size(unsigned long long *prb_log_buf, unsigned long long *prb_log_info,
			unsigned long long *prb_log_desc, struct log_ring_buffer_dump_info *p)
{
	*prb_log_buf = combine_32bits_to_64bits((unsigned long long)p->logringbuf_size[MNTN_LOGRINGBUFFER],
			 p->logringbuf_addr[MNTN_LOGRINGBUFFER]);
	*prb_log_info = combine_32bits_to_64bits((unsigned long long)p->logringbuf_size[MNTN_LOGRINGBUFFER_INFO],
			p->logringbuf_addr[MNTN_LOGRINGBUFFER_INFO]);
	*prb_log_desc = combine_32bits_to_64bits((unsigned long long)p->logringbuf_size[MNTN_LOGRINGBUFFER_DESC],
			p->logringbuf_addr[MNTN_LOGRINGBUFFER_DESC]);
}

void rdr_log_buf_notify_bl31(void)
{
	struct arm_smccc_res res;
	struct log_ring_buffer_dump_info ringbuf_dumpinfo;
	unsigned long long logbuf;
	unsigned long long prb_log_buf;
	unsigned long long prb_log_info;
	unsigned long long prb_log_desc;

	get_logringbuffer_addr_size(&ringbuf_dumpinfo);
	set_logringbuffer_addr_size(&prb_log_buf, &prb_log_info, &prb_log_desc, &ringbuf_dumpinfo);
	logbuf = combine_32bits_to_64bits((unsigned long long)dfx_ex_log_buf_len,
			(unsigned long long)virt_to_phys(dfx_ex_log_buf));
	printk("dfx_ap_ealry_panic happen, will notify bl31\n");
	flush_logbuff_range();

	if (check_mntn_switch(MNTN_PANIC_INTO_LOOP))
		do {} while (1);
	arm_smccc_1_1_smc(MNTN_LOG_BUF_DUMP, logbuf, prb_log_buf, prb_log_info, prb_log_desc, &res);
}
#endif
