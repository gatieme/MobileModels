...
#if defined(CONFIG_RESET_VM_SOLO) || defined(CONFIG_DFX_PSCI_RESET)
...
void set_exception_info(unsigned long address)
{
	...
	exception_buf_len = (unsigned long)sprint_symbol(exception_buf, address);
}
...
__no_sanitize_address void *memcpy_rdr(void *dest, const void *src, size_t count)
{
	char *tmp = dest;
	const char *s = src;
...
static int read_version_property(char *version, int i, char *buf, struct file *fp, size_t count)
{
	...
	ret = vfs_read(fp, buf, i, &fp->f_pos);
	if (ret < 0) {
		BB_PRINT_ERR("[%s],vfs_read fail\n", __func__);
		return 0;
	}
	buf[BUFFER_SIZE - 1] = '\0';
...
void get_product_version(char *version, size_t count)
{
	...
	BB_PRINT_PN("[%s], NO version on PC!\n", __func__);
	return; /* NO Phoenix version on PC, so just return */
#endif
...
void rdr_regs_dump(void *dest, const void *src, size_t len)
{
	...
	u32 *cur_dst = NULL;
	const u32 *cur_src = NULL;

	remain  = len % sizeof(*cur_dst);
	mult    = len / sizeof(*cur_src);
	cur_dst = dest;
	cur_src = src;

	for (i = 0; i < mult; i++)
		*(cur_dst++) = readl(cur_src++);

	for (i = 0; i < remain; i++)
		*((u8 *)cur_dst + i) = readb((u8 *)cur_src + i);
}
#else
void rdr_regs_dump(void *dest, const void *src, size_t len)
{
...
int dfx_trace_hook_install(void)
{
	int ret;
	enum hook_type hk;
	...
	for (hk = HK_IRQ; hk < HK_MAX; hk++) {
		...
		if ((hk == HK_MEM_ALLOCATOR) || (hk == HK_ION_ALLOCATOR)) {
			if (!check_mntn_switch(MNTN_MEM_TRACE))
				continue;
		}
#endif
		ret = dfx_ap_hook_install(hk);
		if (ret) {
...
void dfx_trace_hook_uninstall(void)
{
	enum hook_type hk;

	for (hk = HK_IRQ; hk < HK_MAX; hk++)
		dfx_ap_hook_uninstall(hk);
}
 ...
 * Input:          arg:if arg is null, the func is called dfx_ap_dump
 * Return:         0:success;
 */
int save_mntndump_log(void *arg)
{
	...
	if (vfs_stat(SRC_ETR_DUMP, &mem_stat) == 0)
		save_etr_dump();
#endif

#ifdef CONFIG_DFX_EAGLE_EYE
	if (vfs_stat(FTRACE_DUMP_FS_NAME, &mem_stat) == 0)
		save_eeye_ftrace_memory();
#endif
	...
	if (vfs_stat(SRC_HKRR_MNTN_MEMORY, &mem_stat) == 0) {
		save_hkrr_symbol_memory();
		BB_PRINT_ERR("\n%s: hkrr info saved\n", __func__);
	}
#endif
	...
	if (vfs_stat(SRC_THEE_MNTN_MEMORY, &mem_stat) == 0)
		save_thee_exc_memory();
#endif

	if (vfs_stat(SRC_LOGBUFFER_MEMORY, &mem_stat) == 0)
		save_logbuff_memory();

#ifdef CONFIG_DFX_CONTEXTHUB_LOG_SAVE
	if (vfs_stat(SRC_IOMCU_LOGBUFF_MEMORY, &mem_stat) == 0)
		save_iomcu_logbuff();
#endif
...
/*
 * Description:   After phone reboots, save dfx_ap_log
 * Input:         log_path:path;modid:excep_id
 */
void save_dfx_ap_log(char *log_path, u32 modid)
{
	struct rdr_exception_info_s temp;
	int ret;
	unsigned int path_root_len;
	...
	path_root_len = (unsigned int)strlen(PATH_ROOT);
 ...
 */
void dfx_ap_set_reset_keypoint(u32 value)
{
	u32 local;
...
int dfx_ap_nmi_notify_init(void)
{
...
void dfx_ap_vm_off(void)
{
	...
	BB_PRINT_PN("%s: arm_smccc_hvc begin\n", __func__);
	arm_smccc_hvc(PSCI_0_2_FN_SYSTEM_OFF, 0, 0, 0, 0, 0, 0, 0, &res);
	BB_PRINT_PN("%s: arm_smccc_hvc return res.a0[0x%lx]\n", __func__, res.a0);
}
#endif

#ifdef CONFIG_DFX_PSCI_RESET
void dfx_psci_reset(void)
{
	struct arm_smccc_res res = {0};
	BB_PRINT_PN("%s: enter psci smc atf\n", __func__);
	arm_smccc_1_1_smc(PSCI_0_2_FN_SYSTEM_RESET, 0, 0, 0, &res);
	BB_PRINT_PN("%s: psci smc atf res.a0 = 0x%lx\n", __func__, res.a0);
}
#endif
/*
 * Description : Initialize the reset reason
 */
void dfx_ap_nmi_notify_lpm3(void)
{
	unsigned int value;
	void __iomem *addr;
...
{
#if defined(CONFIG_DFX_PSCI_RESET)
	dfx_psci_reset();
#else
...
/*
 * Description : print curtime and curslice and assign values before reset.
 */
void get_bbox_curtime_slice(void)
{
	u64 curtime, curslice;

	curtime = dfx_getcurtime();
 ...
 */
void rdr_ap_reset(u32 modid, u32 etype, u64 coreid)
{
	...
	/* MNTN_PANIC_INTO_LOOP will disbale ap reset */
...
#if !defined(CONFIG_GPIO_ENHANCE) && !defined(CONFIG_DFX_MNTN_PC)
...
#ifdef CONFIG_RESET_VM_SOLO
	vm_set_reboot_reason_subtype(e_info->e_exce_type, e_info->e_exce_subtype);
#else
	set_reboot_reason(e_info->e_exce_type);
	set_subtype_exception(e_info->e_exce_subtype, false);
#endif
#else
	set_reboot_reason_subtype(e_info->e_exce_type, e_info->e_exce_subtype);
#endif
}
 ...
 */
void dfx_ap_callback(u32 argc, void *argv)
{
	int ret;
...
static int skp_rdr_copy_file_apend(const char *dst, const char *src)
{
	...
	if (kdump_size < 0 || kdump_size > KDUMP_MAX_SIZE) {
...
#ifdef CONFIG_DFX_MNTN_PC
/**
 * The head len of kerneldump is 32.
 * set the head to zero. next time will not dump again.
 **/
static void set_partlabel_head_zero(const char *dst)
{
	long fd;
	char buf[SZ_32];
	long cnt;
	if (!dst) {
		BB_PRINT_ERR("rdr:%s():dst is NULL\n", __func__);
		return;
	}

	fd = SYS_OPEN(dst, O_WRONLY | O_APPEND, FILE_LIMIT);
	if (fd < 0) {
		BB_PRINT_ERR("rdr:%s():open %s failed, return [%ld]\n", __func__, dst, fd);
		return;
	}

	(void)memset_s(buf, SZ_32, 0, SZ_32);
	cnt = SYS_WRITE((unsigned int)fd, buf, SZ_32);
	if (cnt <= 0)
		BB_PRINT_ERR("rdr:%s():write %s failed, return [%ld]\n", __func__, dst, cnt);
	SYS_CLOSE((unsigned int)fd);
	BB_PRINT_ERR("rdr:%s():set dst %s head to zero successful.\n", __func__, dst);
	return;
}
#endif
...
#ifdef CONFIG_DFX_MNTN_PC
		if (rdr_wait_partition("/dev/disk/by-partlabel/kernel", RDR_WAIT_PARTITION_TIME)) {
			BB_PRINT_ERR("kernel dump flash partition is not ready.\n");
			return -1;
		}
		ret = skp_rdr_copy_file_apend(dst_str, "/dev/disk/by-partlabel/kernel");
		set_partlabel_head_zero("/dev/disk/by-partlabel/kernel");
#else
		ret = skp_rdr_copy_file_apend(dst_str, "/dev/block/by-name/userdata");
#endif
		if (ret == 0) {
			BB_PRINT_ERR("%s:set kdump_gzip flag\n", __func__);
			create_dfx_kdump_gzip_log_file();
		} else {
			BB_PRINT_ERR("%s:copy file fail\n", __func__);
		}
	}
...
void save_pstore_info(const char *dst_dir_str)
{
	...
	dfx_remove_pstore_entry();
	kfree(pbuff);
}
/*
 * Description:    copy file from /sys/fs/pstore/psotre to dst_dir_str
 */

void rdr_get_console_log(const char *path)
{
	int ret;
	char dst_str[PATH_MAXLEN] = {0};
	char fullpath_arr[PATH_MAXLEN] = {0};
	char ap_path[PATH_MAXLEN] = {0};
	struct kstat pstore_stat;

	if (!path) {
		pr_err("%s():%d:path null!\n", __func__, __LINE__);
		return;
	}
	ret = snprintf_s(fullpath_arr, PATH_MAXLEN, PATH_MAXLEN - 1, "%s%s", PSTORE_PATH, "rdr_console");
	if (unlikely(ret < 0)) {
		pr_err("%s():%d:snprintf_s fail!\n", __func__, __LINE__);
		return;
	}
	if (vfs_stat(fullpath_arr, &pstore_stat) != 0) {
		pr_err("%s():%d:pstore is not exist!\n", __func__, __LINE__);
		return;
	}

	ret = snprintf_s(ap_path, PATH_MAXLEN, PATH_MAXLEN - 1, "%sap_log/", path);
	if (unlikely(ret < 0)) {
		pr_err("%s():%d:snprintf_s fail!\n", __func__, __LINE__);
		return;
	}
	...
	ret = mntn_filesys_create_dir(ap_path, DIR_LIMIT);
	if (ret != 0) {
		BB_PRINT_ERR("%s: ret is [%d]\n", __func__, ret);
		return;
	}
	ret = snprintf_s(dst_str, PATH_MAXLEN, PATH_MAXLEN - 1, "%s%s", ap_path, "last_kmsg");
	if (unlikely(ret < 0)) {
		pr_err("%s():%d:snprintf_s fail!\n", __func__, __LINE__);
		return;
	}
...
void rdr_save_console_log(const char *path)
{
	rdr_creat_console_log();
	rdr_get_console_log(path);
	rdr_remove_console_log();
}
/*
...
void save_fastboot_log(const char *dst_dir_str)
{
	int ret;
	u32 len;
	char fblog_path[NEXT_LOG_PATH_LEN];
	char last_fblog_path[NEXT_LOG_PATH_LEN];
	...
	(void)memset_s(last_fblog_path, NEXT_LOG_PATH_LEN, 0, NEXT_LOG_PATH_LEN);

	(void)memset_s(fblog_path, NEXT_LOG_PATH_LEN, 0, NEXT_LOG_PATH_LEN);
	len = (u32)strlen(dst_dir_str);
	ret = memcpy_s(last_fblog_path, NEXT_LOG_PATH_LEN - 1, dst_dir_str, len);
	if (ret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
	}
	ret = memcpy_s(&last_fblog_path[len], NEXT_LOG_PATH_LEN - strlen(last_fblog_path),
			"/last_fastboot_log", strlen("/last_fastboot_log") + 1);
	if (ret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
	}
	ret = memcpy_s(fblog_path, NEXT_LOG_PATH_LEN - 1, dst_dir_str, len);
	if (ret != EOK) {
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
		return;
	}
	ret = memcpy_s(&fblog_path[len], NEXT_LOG_PATH_LEN - strlen(fblog_path), "/fastboot_log",
	...
	ret = rdr_copy_file_apend(last_fblog_path, LAST_FASTBOOT_LOG_FILE);
	if (ret)
		BB_PRINT_ERR("rdr_copy_file_apend [%s] fail, ret = [%d]\n", LAST_FASTBOOT_LOG_FILE, ret);

	/* Generate the curr_fastbootlog file */
	ret = rdr_copy_file_apend(fblog_path, FASTBOOT_LOG_FILE);
	if (ret)
...
static bool is_modem_exception(void)
{
	...
	modem_module = find_dfx_module(MODEM_DRIVER, strlen(MODEM_DRIVER));
	if (!modem_module) {
	...
	BB_PRINT_ERR("[%s]:module_start %pK\n", __func__, (void *)module_start);
	module_end = (uintptr_t)module_start + modem_module->core_layout.size;
	BB_PRINT_ERR("[%s]:module end is %pK\n", __func__, (void *)module_end);
	save_stack_trace_tsk(current, &trace);
	BB_PRINT_ERR("[%s]:save_stack_trace_tsk done\n", __func__);

	for (i = 0; i < trace.nr_entries; i++) {
		BB_PRINT_ERR("[%s]:trace.entries[%u] = %pK\n", __func__, i, (void *)trace.entries[i]);
...
static bool is_modem_exception(void)
{
	...
	BB_PRINT_ERR("%s start\n", __func__);
	modem_module = find_module(MODEM_DRIVER);
	if (!modem_module) {
		BB_PRINT_ERR("modem ko module no found\n");
		return false;
	}
	...
	module_start = (uintptr_t)modem_module->core_layout.base;
	BB_PRINT_ERR("[%s]:module_start %pK\n", __func__, (void *)(uintptr_t)module_start);
	module_end = (uintptr_t)module_start + modem_module->core_layout.size;
	BB_PRINT_ERR("[%s]:module end is %pK\n", __func__, (void *)(uintptr_t)module_end);
	nr_entries = stack_trace_save_tsk(current, entries, MAX_STACK_TRACE_DEPTH, skip);
	BB_PRINT_ERR("[%s]:save_stack_trace_tsk done\n", __func__);

	for (i = 0; i < nr_entries; i++) {
		BB_PRINT_ERR("[%s]:trace.entries[%u] = %pK\n", __func__, i, (void *)(uintptr_t)entries[i]);
		if (entries[i] >= module_start && entries[i] <= module_end) {
			BB_PRINT_ERR("[%s]:modem drv panic happen\n", __func__);
			return true;
		}
	}
...
#endif /* END 5.10 */
#else
static inline bool is_modem_exception(void)
{
	return false;
}
#endif
...
static void save_panic_cpuid(unsigned int reboot_cpuid)
{
	...
	pmic_write_reg(PMU_SAVE_CPU_ID, value);
 ...
 */
int rdr_ap_panic_notify(struct notifier_block *nb,
				unsigned long event, void *buf)
{
#ifdef CONFIG_DFX_HW_DIAG
	union dfx_hw_diag_info hwdiag_apanic_info;
#endif
	...
	dfx_hw_diaginfo_trace(CPU_PANIC_INFO, &hwdiag_apanic_info);
#endif
	...
	} else if (watchdog_hiwdt_hardlockup_happen()) {
 ...
 */
int rdr_ap_die_notify(struct notifier_block *nb,
