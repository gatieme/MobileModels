...
{
	unsigned int min_size = (unsigned int)sizeof(struct dfx_ap_ringbuffer_s) + fieldcnt;
				...
				enum hook_type hk, u32 ratio[][DFX_HOOK_CPU_NUMBERS])
{
	...
	min_size = cpu_num * ((u64)sizeof(struct dfx_ap_ringbuffer_s) + PERCPU_TOTAL_RATIO * (u64)(unsigned int)fieldcnt);
...
void irq_trace_hook(unsigned int dir, unsigned int old_vec, unsigned int new_vec)
{
	...
	if (g_arch_timer_func_ptr) {
		...
		info.jiff = info.clock * SECOND_TICK_CNT;
		do_div(info.jiff, NS_INTERVAL);
#else
...
void dfx_ap_defopen_hook_install(void)
{
	...
	size = (u32)ARRAY_SIZE(g_dfx_defopen_hook_id);
	for (i = 0; i < size; i++) {
