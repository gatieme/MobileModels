...
enum rdr_lpm3_system_error_type {
	M3_WDT_TIMEOUT = DFX_BB_MOD_LPM_START,
	AP_WDT_TIMEOUT,
	SYSTEM_ERROR_TYPE_MAX = DFX_BB_MOD_LPM_END,
};
...
#define IPC_TRACE_LINESIZE		16
#define IRQ_LONG_TIME_TRACE_LINESIZE		12
...
#ifdef M3_RDR_IPC_TRACK_OFFSET
static int ipc_trace_parse(const char *dir_path, struct file *fp,
			   u64 log_addr, u32 log_len)
{
	...
	/* print ipc info & time */
	rdr_cleartext_print(fp, &error, "num  pos mbx cmd obj       time  pos mbx cmd obj       time\n");
	for (i = 0; i < log_len / IPC_TRACE_LINESIZE; i++) {
		print_addr = log_addr + i * IPC_TRACE_LINESIZE;
		rdr_cleartext_print(fp, &error, "%03u: %02u  %02u  %02u  %02u",
				    i + 1,
					*((u8 volatile *)(uintptr_t)print_addr),
				    *((u8 volatile *)(uintptr_t)(print_addr + 1)),
					*((u8 volatile *)(uintptr_t)(print_addr + 2)),
					*((u8 volatile *)(uintptr_t)(print_addr + 3)));
		rdr_cleartext_print(fp, &error, "  0x%08X",
				    *((u32 volatile *)(uintptr_t)(print_addr + 4)));
		rdr_cleartext_print(fp, &error, "  %02u  %02u  %02u  %02u",
					*((u8 volatile *)(uintptr_t)print_addr + 8),
				    *((u8 volatile *)(uintptr_t)(print_addr + 9)),
					*((u8 volatile *)(uintptr_t)(print_addr + 10)),
					*((u8 volatile *)(uintptr_t)(print_addr + 11)));
		rdr_cleartext_print(fp, &error, "  0x%08X\n",
...
#ifdef INT_LONG_TIME_TRACK_OFFSET
static int irq_long_time_trace_parse(const char *dir_path, struct file *fp,
			   u64 log_addr, u32 log_len)
{
	u32 i;
	bool error = false;
	...
	/* print irq_id & time in & time out */
	rdr_cleartext_print(fp, &error, "num  irq      time_in       time_out\n");
	for (i = 0; i < log_len / IRQ_LONG_TIME_TRACE_LINESIZE; i++) {
		print_addr = log_addr + i * IRQ_LONG_TIME_TRACE_LINESIZE;
		...
		rdr_cleartext_print(fp, &error, "     0x%08X",
				    *((u32 volatile *)(uintptr_t)(print_addr + 4)));
		rdr_cleartext_print(fp, &error, "     0x%08X\n",
				    *((u32 volatile *)(uintptr_t)(print_addr + 8)));
	}
	if (error)
		return -EINVAL;
...
#ifdef IRQ_MASK_LONG_TIME_TRACK_OFFSET
static int irq_mask_long_time_trace_parse(const char *dir_path, struct file *fp,
			   u64 log_addr, u32 log_len)
{
	u32 i;
	bool error = false;
	...
	/* print time& during & lr */
	rdr_cleartext_print(fp, &error, "num       time_in       during       handler\n");
	for (i = 0; i < log_len / IRQ_LONG_TIME_TRACE_LINESIZE; i++) {
		print_addr = log_addr + i * IRQ_LONG_TIME_TRACE_LINESIZE;
		rdr_cleartext_print(fp, &error, "%02u:     0x%08X",
				    i + 1,
				    *((u32 volatile *)(uintptr_t)print_addr));
		rdr_cleartext_print(fp, &error, "    0x%08X",
				    *((u32 volatile *)(uintptr_t)(print_addr + 4)));
		rdr_cleartext_print(fp, &error, "    0x%08X\n",
				    *((u32 volatile *)(uintptr_t)(print_addr + 8)));
...
#ifdef STACK_BACKUP_SRAM_OFFSET
static int lpm3_stack_parse(const char *dir_path, struct file *fp,
			    u64 log_addr, u32 log_len)
{
	u32 i, j;
	bool error = false;
	...
	/* register address interval is 16 */
	for (i = 0; i < log_len; i = i + 16) {
		rdr_cleartext_print(fp, &error, "0x%03x  ", i);
		for(j = 0; j < 16; j++)
				rdr_cleartext_print(fp, &error, " %02x",
				    *((u8 volatile *)(uintptr_t)(log_addr + i + j)));
...
static struct log_lpm3_cleartext g_lpm3_cleartext[] = {
	...
	{
		"LPM3_EXC_SPECIAL",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET,
		EXC_SPECIAL_SAVE_SIZE,
	...
	{
		"DDR_REGS",
		M3_RDR_DDR_MNTN_OFFSET,
		M3_RDR_DDR_MNTN_SIZE,
		system_reg_parse
	},
	{
		"NOC_REGS",
		M3_RDR_SYS_CONTEXT_NOC_REG_OFFSET,
		M3_RDR_SYS_CONTEXT_NOC_REG_SIZE,
		system_reg_parse
	},
	{
		"SR_TICKMARK",
		M3_RDR_SYS_CONTEXT_LPMCU_STAT_OFFSET,
		LPMCU_TELE_MNTN_DATA_TICKMARK_SIZE,
		system_reg_parse
	},
#ifdef M3_RDR_IPC_TRACK_OFFSET
	{
		"IPC_TRACE",
		M3_RDR_IPC_TRACK_OFFSET,
		IPC_TRACK_SIZE,
		ipc_trace_parse
	},
#endif
	...
	{
		"INT_LONG_TIME_TRACE",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET + INT_LONG_TIME_TRACK_OFFSET,
		INT_LONG_TIME_TRACK_SIZE,
		irq_long_time_trace_parse
	},
#endif
#ifdef IRQ_MASK_LONG_TIME_TRACK_OFFSET
	{
		"IRQ_MASK_LONG_TRACE",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET + IRQ_MASK_LONG_TIME_TRACK_OFFSET,
		IRQ_MASK_LONG_TIME_TRACK_SIZE,
		irq_mask_long_time_trace_parse
	},
#endif
#ifdef STACK_BACKUP_SRAM_OFFSET
	{
		"LPM3_STACK",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET + STACK_BACKUP_SRAM_OFFSET,
		STACK_BACKUP_SRAM_SIZE,
		lpm3_stack_parse
	},
#endif
#ifdef M3_RDR_CXPU_SEC_DATA_OFFSET
	{
		"CPU_INFO",
		M3_RDR_CXPU_SEC_DATA_OFFSET,
		M3_RDR_CXPU_SEC_DATA_SIZE,
		system_reg_parse
	},
#endif
#ifdef M3_RDR_GXPU_DATA_SIZE
	{
		"GPU_INFO",
		M3_RDR_GXPU_RAM_OFFSET,
		M3_RDR_GXPU_DATA_SIZE,
		system_reg_parse
	},
#endif
};
		    ...
		    pfn_cb_dump_done pfn_cb)
{
	...
	ret = RPROC_ASYNC_SEND(IPC_ACPU_LPM3_MBX_5, (mbox_msg_t *)msg,
			       M3_IPC_MSG_LEN);
	if (ret != 0)
...
static int rdr_lpm3_thread_body(void *arg)
{
	...
	(void)arg;
	base_addr = RESERVED_LPMX_CORE_PHYMEM_BASE;
	lpmcu_ddr_base = (char *)ioremap((phys_addr_t)base_addr, //lint !e446
					 RESERVED_LPMX_CORE_PHYMEM_SIZE);
	...
	while (1) {
		...
		if (g_pfn_cb_dumpdone != NULL) {
			...
			mntn_filesys_write_log(g_lpmcu_ddr_memory_path,
					       lpmcu_ddr_base,
					       RESERVED_LPMX_CORE_PHYMEM_SIZE, 0);
...
int rdr_lpm3_stat_dump(void)
{
	...
	ret = RPROC_ASYNC_SEND(IPC_ACPU_LPM3_MBX_5, (mbox_msg_t *)&msg, 1);
	if (ret != 0) {
...
#ifdef CONFIG_LPMCU_MNTN_RISCV
	rdr_cleartext_print(fp, &error, "slice      0x%x\n", p_exc->slice);
	rdr_cleartext_print(fp, &error, "rtc        0x%x\n", p_exc->rtc);
	rdr_cleartext_print(fp, &error, "lr         0x%x\n", p_exc->lr);
	rdr_cleartext_print(fp, &error, "pc         0x%x\n", p_exc->pc);
	rdr_cleartext_print(fp, &error, "mcause     0x%x\n", p_exc->mcause);
	rdr_cleartext_print(fp, &error, "exc_trace  0x%x\n", p_exc->exc_trace);
	rdr_cleartext_print(fp, &error, "ddr_exc    0x%x\n", p_exc->ddr_exc);
	rdr_cleartext_print(fp, &error, "irq_id     0x%x\n", p_exc->irq_id);
	rdr_cleartext_print(fp, &error, "task_id    0x%x\n", p_exc->task_id);
	rdr_cleartext_print(fp, &error, "pc_async   0x%x\n", p_exc->pc_async);
	rdr_cleartext_print(fp, &error, "lr_async   0x%x\n", p_exc->lr_async);
	rdr_cleartext_print(fp, &error, "mtval      0x%x\n", p_exc->mtval);
#else
	rdr_cleartext_print(fp, &error, "slice      0x%x\n", p_exc->slice);
	rdr_cleartext_print(fp, &error, "rtc        0x%x\n", p_exc->rtc);
				...
				u32 reason_idx)
{
	...
	for (i = 0; i < RISCV_REG_NUM; i++)
		rdr_cleartext_print(fp, &error, "X%u: 0x%x\n", i, p->x[i]);
	rdr_cleartext_print(fp, &error, "pc: 0x%x\n", p->pc);
	rdr_cleartext_print(fp, &error, "sp: 0x%x\n", p->sp);
	rdr_cleartext_print(fp, &error, "epc: 0x%x\n", p->mepc);
	rdr_cleartext_print(fp, &error, "mcause: 0x%x\n", p->mcause);
	rdr_cleartext_print(fp, &error, "mtval: 0x%x\n", p->mtval);
	rdr_cleartext_print(fp, &error, "mstatus: 0x%x\n", p->mstatus);
	rdr_cleartext_print(fp, &error, "cxcptsc: 0x%x\n", p->cxcptsc);
	rdr_cleartext_print(fp, &error, "\n");
#else
	rdr_cleartext_print(fp, &error, "****REGION %u****\n", reason_idx);
			       ...
			       u64 log_addr, u32 log_len)
{
	...
	for (idx = 0; idx < RDR_BACKUP_IDEX_MAX; idx++) {
		core_addr =
			log_addr + idx * (u32)sizeof(struct rdr_lpmcu_cregs);
			       ...
			       u64 log_addr, u32 log_len)
{
	...
	for (i = 0; i < RISCV_LOCIEN_REG_NUM; i++) {
		print_addr += (i * sizeof(u32));
		rdr_cleartext_print(fp, &error, "locien%u    0x%x\n", i,
				    *((u32 volatile *)(uintptr_t)print_addr));
	}
	for (i = 0; i < RISCV_LOCIPD_REG_NUM; i++) {
		print_addr += (i * sizeof(u32));
		rdr_cleartext_print(fp, &error, "locipd%u    0x%x\n", i,
				    *((u32 volatile *)(uintptr_t)print_addr));
	}
	for (i = 0; i < RISCV_LOCIPRI_REG_NUM; i++) {
		print_addr += (i * sizeof(u32));
		rdr_cleartext_print(fp, &error, "locipri%u    0x%x\n", i,
				    *((u32 volatile *)(uintptr_t)print_addr));
	}
	print_addr += sizeof(u32);
	rdr_cleartext_print(fp, &error, "prithd    0x%x\n",
				*((u32 volatile *)(uintptr_t)print_addr));
	rdr_cleartext_print(fp, &error, "\n");
	if (error)
		return -EINVAL;
...
static int rdr_lpm3_cleartext_print(const char *dir_path,
				    u64 log_addr, u32 log_len)
{
	bool error = false;
	u32 idx;
...
static void rdr_addr_send_lpm3(void)
{
	...
	g_lpmcu_rdr_ddr_addr = (char *)dfx_bbox_map((phys_addr_t)g_current_info.log_addr,
...
static int __init rdr_lpm3_init(void)
{
	...
	ret = RPROC_MONITOR_REGISTER(IPC_LPM3_ACPU_MBX_2, &g_rdr_ipc_block);
	if (ret != 0) {
	...
	(void)rdr_lpm3_reset_off(LPMCU_RESET_OFF_MODID_PANIC,
				 check_mntn_switch(MNTN_LPM3_PANIC_INTO_LOOP));
	return ret;
}
