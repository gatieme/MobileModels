...
char *rdr_get_exception_core(u64 coreid)
{
	...
	case RDR_DSS:
		...
		break;
	case RDR_DDR:
		core = exception_core[EXCEPTION_CORE_DDR];
		break;
	case RDR_SILS:
		core = exception_core[EXCEPTION_CORE_SILS];
		break;
	case RDR_RDA_AP:
		core = exception_core[EXCEPTION_CORE_RDA_AP];
		break;
	default:
				...
				unsigned long event, void *ptr)
{
	...
	switch (event) {
	case PM_POST_HIBERNATION: /* call after PC exit S4 */
		rdr_ap_root_restore();
	...
	case PM_HIBERNATION_PREPARE: /* call before PC enter S4 */
		rdr_ap_root_backup();
	...
	case PM_SUSPEND_PREPARE: /* call before suspend */
		...
		if (times == 0)
			BB_PRINT_ERR("%s: wait for saving log timeout(5S)\n", __func__);
		BB_PRINT_DBG("%s: suspend -\n", __func__);
		break;
	default:
		return NOTIFY_DONE;
...
{
	u32 i;
	u32 modid_size = (u32)ARRAY_SIZE(g_modid_list);
...
void *bbox_vmap(phys_addr_t paddr, size_t size)
{
	...
	for (i = 0; i < pages_count; i++)
		*(pages + i) = (struct page *)(uintptr_t)phys_to_page((uintptr_t)(paddr + PAGE_SIZE * i));
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
void get_pt_regs(struct pt_regs *regs)
{
	u64 tmp1, tmp2;
...
static void show_data(unsigned long addr, int nbytes, const char *name)
{
	 ...
	 */
	p = (u32 *)(uintptr_t)(addr & ~(sizeof(u32) - 1));
	nbytes += (int)(addr & (sizeof(u32) - 1));
	nlines = (nbytes + 31) / 32;
	...
	for (i = 0; i < nlines; i++) {
		 ...
		 */
		printk("%04lx ", (uintptr_t)(void *)p & 0xffff);
		for (j = 0; j < 8; j++) {
			u32	data = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
