...
#include <platform_include/see/bl31_log.h>
...
#include "platform_ap/rdr_ap_exception_logsave.h"
#include "platform_ap/rdr_ap_adapter.h"
#include "rdr_inner.h"
#include "rdr_print.h"
#include "rdr_field.h"
#include "../save_soc_register.h"
...
int rdr_save_buf(const char *buf, u32 len)
{
	...
	ret = snprintf_s(local_path, PATH_MAXLEN, PATH_MAXLEN - 1, "%s/%s", PATH_ROOT, "history.log");
	if (ret < 0) {
		BB_PRINT_ERR("%s(), snprintf_s fail, ret = %d\n", __func__, ret);
		return -1;
	}
				...
				bool is_save_done, u32 bootup_keypoint)
{
	...
	dfx_hw_diaginfo_record(date);
#endif
#ifdef CONFIG_BLACKBOX_EXPAND_EVENT
	if (is_save_done) {
		struct error_info *perror_info;
		char tmp_buf[sizeof(struct error_info)];
		(void)memset_s(tmp_buf, sizeof(tmp_buf), 0, sizeof(tmp_buf));
		perror_info = (struct error_info *)tmp_buf;
		(void)snprintf(perror_info->event, sizeof(perror_info->event) - 1, "%s%s%s",
				rdr_get_exception_type(p->e_exce_type), subtype_name ? ":" : "", subtype_name ? subtype_name : "");
		(void)strncpy(perror_info->module, rdr_get_exception_core(p->e_from_core),
				min(strlen(ADAPTER_SYSTEM), sizeof(perror_info->module) - 1));
		(void)strncpy(perror_info->category, rdr_get_category_name(p->e_exce_type, p->e_exce_subtype),
				min(strlen(rdr_get_category_name(p->e_exce_type, p->e_exce_subtype)), sizeof(perror_info->category) - 1));
		(void)strncpy(perror_info->error_time, date, min(strlen(date), sizeof(perror_info->error_time) - 1));
		(void)strncpy(perror_info->log_path, PATH_ROOT, min(strlen(PATH_ROOT), sizeof(perror_info->log_path) - 1));
		(void)strncpy(perror_info->top_category, "system exception",
				min(strlen("system exception"), sizeof(perror_info->category) - 1));
		if (!strcmp(reboot_from_ap, "true"))
			perror_info->sysreboot = 1;
		else
			perror_info->sysreboot = 0;
		(void)snprintf(perror_info->error_desc, ERROR_DESC_MAX_LEN, "bootup_keypoint:%u", bootup_keypoint);
		perror_info->error_desc[ERROR_DESC_MAX_LEN - (sizeof(struct error_info) - ERROR_DESC_MAX_LEN)] = '\0';
		bbox_notify_error(EVENT_ADAPTER, ADAPTER_SYSTEM, (const char *)tmp_buf, false);
	}
#endif
...
static void rdr_save_bl31_log(const char *path)
{
	...
	log_size = res.a2;
	read_buffer = kzalloc(log_size, GFP_KERNEL);
	if (read_buffer == NULL) {
		BB_PRINT_ERR("%s(): no memory\n", __func__);
		return;
	}

	ret = bl31_logbuf_read(read_buffer, log_size);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): bl31_logbuf_read error%d\n", __func__, ret);
		goto save_err;
	}

	ret = rdr_savebuf2fs(path, "bl31_mntn_memory.bin", read_buffer, log_size, 0);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): rdr_savebuf2fs error%d\n", __func__, ret);
		goto save_err;
	}
...
static void rdr_save_logbuf_log(const char *path)
{
	...
	dst = vzalloc(DST_LOG_BUFF_LEN);
	if (dst == NULL) {
		BB_PRINT_ERR("%s():%d:vmalloc buf fail\n", __func__, __LINE__);
		return;
	}

	ret = logbuf_print_all(dst, DST_LOG_BUFF_LEN);
	if (ret < 0) {
		BB_PRINT_ERR("%s():%d:logbuf_print_all fail\n", __func__, __LINE__);
		goto err;
	}

	ret = rdr_savebuf2fs(path, "kernel_logbuf.bin", dst, DST_LOG_BUFF_LEN, 0);
	if (ret < 0) {
...
static void rdr_save_logbuf_log(const char *path)
{
	...
	ret = rdr_savebuf2fs(path, "kernel_logbuf.bin", dst, lenth, 0);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): rdr_savelogbuf2fs error = %d\n", __func__, ret);
		goto err;
	}

	buffer = (char *)dfx_ex_log_first_idx;

	ret = rdr_savebuf2fs(path, "kernel_logbuf.bin", buffer, sizeof(lenth), 1);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): rdr_savefirst_idx2fs error = %d\n", __func__, ret);
		goto err;
	}

	buffer = (char *)dfx_ex_log_next_idx;
...
/*
 * Check if dfx partion path including PART_DFX keyword.
 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0))
static bool check_dfx_path(const char *path)
{
	int len = (int)(strlen(path) - strlen(PART_DFX));
	if (len < 0 || strncmp(path + len, PART_DFX, strlen(PART_DFX)) != 0)
		return false;
	return true;
}
#else
static inline bool check_dfx_path(const char *path)
{
	return true;
}
#endif
...
bool is_need_save_dfx2file(void)
{
	...
	if (!check_mntn_switch(MNTN_DFXPARTITION_TO_FILE)) {
	...
	if (unlikely(!check_dfx_path(p_name))) {
	...
	if (rdr_wait_partition(p_name, RDR_WAIT_PARTITION_TIME) != 0) {
		BB_PRINT_PN("%s():%d: %s is not ready\n", __func__, __LINE__, p_name);
		kfree(buf);
		goto out;
	}
#endif
...
int check_dfx_head_valid(struct dfx_head_info *base, const void *cur_addr, u32 size)
{
	...
	if (cur_addr >= (void *)base && size <= EVERY_NUMBER_SIZE &&
		cur_addr + size <= (void *)base + noreboot_size)
		return 0;

	BB_PRINT_ERR("%s(), base:0x%pK, cur_addr:0x%pK, size:0x%x, noreboot_size:0x%x\n",
	     __func__, base, cur_addr, size, noreboot_size);
...
static int read_part_into_buff(const char *part_name, char *tmp_buf, unsigned int size)
{
	...
	if (unlikely(!check_dfx_path(part_name))) {
...
static int write_info_into_buff(struct every_number_info *every_number_info, const char *part_name, char *tmp_buf)
{
	...
	cfo = (int)dfx_head_info->every_number_addr[dfx_head_info->cur_number];
...
void systemerror_save_log2dfx(u32 reboot_type)
{
	...
	fastbootlog_addr = ioremap_wc(SUB_RESERVED_FASTBOOT_LOG_PYHMEM_BASE,
	...
	every_number_info->boot_time = dfx_getcurtime();
...
static void save_buffer_to_dfx_tempbuffer(struct every_number_info *every_number_info)
{
	...
	if ((unsigned long long)ret != dfx_head_info->temp_number_addr) {
		BB_PRINT_ERR("%s():%d:lseek fail[%d]\n", __func__, __LINE__, ret);
		goto close;
	}
