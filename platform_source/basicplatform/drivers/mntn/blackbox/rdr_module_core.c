...
static int rdr_get_module_info(u64 coreid, struct rdr_register_module_result *retinfo)
{
	...
	case RDR_GENERAL_SEE:
		ret = rdr_get_areainfo(RDR_AREA_GENERAL_SEE, retinfo);
		break;
	case RDR_NPU:
	...
	case RDR_DSS:
		...
		break;
	case RDR_DDR:
		ret = rdr_get_areainfo(RDR_AREA_DDR, retinfo);
		break;
	case RDR_SILS:
		ret = rdr_get_areainfo(RDR_AREA_SILS, retinfo);
		break;
	case RDR_RDA_AP:
		ret = rdr_get_areainfo(RDR_AREA_RDA_AP, retinfo);
		break;
	default:
		ret = -1;
	}
