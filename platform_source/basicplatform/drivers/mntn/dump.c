...
static void *memdump_remap_type(uintptr_t phys_addr, size_t size, pgprot_t pgprot)
{
	...
	pages[0] = (struct page *)(uintptr_t)phys_to_page(phys_addr);
				...
				loff_t *off)
{
	...
	copy = (ssize_t)(uintptr_t) min(bytes, (size_t) (info->size - *off));
				  ...
				  size_t bytes, loff_t *off)
{
	...
	for (addr = g_memdump_addr; addr < (g_memdump_addr + g_memdump_size);
	     addr += PAGE_SIZE) {
		page = (struct page *)(uintptr_t)pfn_to_page(addr >> PAGE_SHIFT);
	...
	pr_info("%s The reserved memory has been released\n", __func__);
