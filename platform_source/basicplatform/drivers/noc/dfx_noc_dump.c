...
#include "dfx_noc_dump.h"

#define ERRLOG_REG_BUFFER_MAX_SIZE 8
#define REG_VAL_BUF_MAX_SIZE 10

/* Gloable Stucture For Recording NoC Dump Info. */
static struct dfx_noc_dump_info g_noc_dump_info;
...
static int noc_dump_err_probe(void)
{
	...
	uint reg_val_buf[REG_VAL_BUF_MAX_SIZE];
	...
	for (j = 0; j < g_noc_dump_info.noc_bus_node_info.noc_node_idx; j++) {
		...
		for (k = 0; k < ERRLOG_REG_BUFFER_MAX_SIZE; k++) {
		...
		noc_err_get_msg(reg_val_buf, node->bus_id, pt_err_msg, REG_VAL_BUF_MAX_SIZE);
...
static int noc_dump_reg_init(struct noc_dump_data *pt_dump)
{
	...
	reg_num = dfx_noc_get_dump_reg_list_num();
...
int noc_dump_init(void)
{
	...
	/* Get NoC Device Stucture Pointer. */
	g_noc_dump_info.noc_device_ptr = platform_get_drvdata(noc_get_pt_pdev());

	/* Get NoC Bus Node Stucture Pointer and Number. */
	...
	for (i = 0; i < dfx_noc_get_dump_reg_list_num(); i++) {
	...
	if (g_noc_dump_info.noc_device_ptr->noc_property->noc_debug) {
		...
		pr_err("noc_device_ptr=%pK\n", g_noc_dump_info.noc_device_ptr);
		pr_err("noc_node_ptr=%pK\n", g_noc_dump_info.noc_bus_node_info.noc_node_pptr);
		pr_err("noc_node_idx=%d\n", g_noc_dump_info.noc_bus_node_info.noc_node_idx);
	}
#ifdef CONFIG_DFX_BB
	/* function "register_module_dump_mem_func" can only be used when CONFIG_BB */
