...
#define pr_fmt(fmt) "partition ap kernel:" fmt
...
#include <platform_include/basicplatform/linux/partition/partition_def.h>
#include <platform_include/basicplatform/linux/partition/partition.h>
...
enum partition_boot_type ufs_boot_partition_type = XLOADER_A;
enum partition_boot_type emmc_boot_partition_type = XLOADER_A;
...
int partition_set_storage_type(unsigned int ptn_type)
{
	...
	dev_type = get_bootdevice_type();
	if (dev_type == BOOT_DEVICE_EMMC)
		return emmc_set_boot_partition_type(ptn_type);
