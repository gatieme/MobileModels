...
/* PCIe SMMUv3 */
#define HSDTSCTRL_AWMMUS			0x320
#define HSDTSCTRL_ARMMUS			0x324
...
#define EFUSE_DIEID_START_BIT		1184
#define EFUSE_CHIP_VERSION_START	(1336 - (EFUSE_DIEID_START_BIT))
#define EFUSE_CHIP_VERSION_SIZE		1
#define EFUSE_CHIP_V120			1
static u32 pcie_get_version_efuse(void)
{
	u32 efuse;
	struct efuse_desc desc;
	u32 start_bit = EFUSE_CHIP_VERSION_START;

	efuse = 0;
	desc.buf = &efuse;
	desc.start_bit = EFUSE_CHIP_VERSION_START;
	desc.buf_size = EFUSE_CHIP_VERSION_SIZE;
	desc.bit_cnt = EFUSE_CHIP_VERSION_SIZE;

	desc.item_vid = EFUSE_KERNEL_DIEID_VALUE;
	if (efuse_read_value_t(&desc) != EFUSE_OK)
		return 0;

	PCIE_PR_I("version efuse val = %u", efuse);
	return efuse;
}

static u32 g_burb_version = 0;
static void pcie_fnpll_cfg_init_set(struct pcie_kport *pcie)
{
	if (g_burb_version == EFUSE_CHIP_V120)
		writel(0x000000CA, pcie->pcie_pll_base + FNPLL_PCIE_ANALOG_CFG0);
	else
	...
	if (g_burb_version == EFUSE_CHIP_V120)
		...
		writel(0x000240FF, pcie->pcie_pll_base + FNPLL_PCIE_ADPLL_FTDC_LOCK_THR);
...
static int pcie_fnpll_ctrl(struct pcie_kport *pcie, u32 enable)
{
	...
	hsdt_crg_reg_write(pcie, 0x00000002, HSDT_CRG_FNPLL_CFG6); /* EN:0 and BYPASS:1 */
	hsdt_crg_reg_write(pcie, 0x2EE00000, HSDT_CRG_FNPLL_FCW);  /* 100M refclk */
	udelay(1);
	hsdt_crg_reg_write(pcie, 0x00000003, HSDT_CRG_FNPLL_CFG6); /* EN:1 and BYPASS:1 */
...
static int pcie_refclk_on(struct pcie_kport *pcie)
{
	...
	if (ret) {
		...
		if (pcie->dtsinfo.ep_flag)
			PCIE_PR_E("ref_clk needless in EP mode");
		else
			return ret;
	}
...
static noinline uint64_t pcie_smmu_handle(uint64_t fid, uint64_t type)
{
	...
	arm_smccc_smc((u64)fid, (u64)type,
		      0, 0, 0, 0, 0, 0, &res);
	return res.a0;
}
...
static int pcie_smmu_cfg(struct pcie_kport *pcie, u32 enable)
{
	...
	if (enable) {
		...
		(void)pcie_smmu_handle(FID_PCIE_SET_MEM_UNSEC, PCIE_SMMU_MEM_CTRL_SET);
		(void)pcie_smmu_handle(FID_PCIE_MSIC_FUNC_SET, PCIE_SMMU_FDUL_ON);
		ret = mm_smmu_poweron(pcie->pci->dev);
		if (ret) {
			(void)pcie_smmu_handle(FID_PCIE_MSIC_FUNC_SET, PCIE_SMMU_FDUL_OFF);
		...
		(void)pcie_tbu_config(pcie, DISABLE);
		(void)mm_smmu_poweroff(pcie->pci->dev);
		(void)pcie_smmu_handle(FID_PCIE_MSIC_FUNC_SET, PCIE_SMMU_FDUL_OFF);
	}
...
static int pcie_smmu_init(struct pcie_kport *pcie, u32 enable)
{
	if (is_smmu_bypass(pcie)) {
		if (enable && pcie_smmu_handle(FID_PCIE_SET_TBU_BYPASS, PCIE_SMMU_BYPASS)) {
			PCIE_PR_E("Bypass TBU failed");
			return -1;
		}
		PCIE_PR_I("Bypass TBU OK");
		return 0;
	}
...
int pcie_plat_init(struct platform_device *pdev, struct pcie_kport *pcie)
{
	...
	g_burb_version = pcie_get_version_efuse();
