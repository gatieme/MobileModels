...
#define PCIE_AUX_CLK_FREQ			19000000
...
#define PCIE_TBU_TOK_MODIFY		(0x400F00)
...
static noinline uint64_t pcie_tbu_bypass(struct pcie_kport *pcie)
{
	...
	args.data0 = FID_PCIE_SET_TBU_BYPASS;
	args.data1 = (u64)pcie->rc_id;
	ret = ffa_platdrv_send_msg(&args);
	if (ret != 0) {
		PCIE_PR_E("Bypass TBU failed, ret = %d", ret);
		return ret;
	}
	return args.data0;
#else
...
static noinline uint64_t pcie_mem_unsec(struct pcie_kport *pcie)
{
	...
	int ret;
	struct ffa_send_direct_data args = {0};
	args.data0 = FID_PCIE_SET_MEM_UNSEC;
	args.data1 = (u64)pcie->rc_id;
	ret = ffa_platdrv_send_msg(&args);
	if (ret != 0) {
		PCIE_PR_E("config mem unsec failed, ret = %d", ret);
		return ret;
	}
	return args.data0;
#else
...
static int pcie_tbu_config(struct pcie_kport *pcie, u32 enable)
{
	...
	if (enable) {
		reg_val &= ~PCIE_TBU_TOK_TRANS_MSK;
		reg_val |= PCIE_TBU_TOK_MODIFY;
...
static noinline uint64_t pcie_phy_get_efuse(struct pcie_kport *pcie)
{
	...
	item.buf = &val;
	item.buf_size = 1;
	item.item_vid = EFUSE_KERNEL_COMBO2PCIEEDPPHY_CALI;
	ret = efuse_read_item(&item);
	if (ret != EFUSE_OK) {
		PCIE_PR_E("Failed to get pcie phy efuse, ret = %x", ret);
		return 0;
	}
	PCIE_PR_E("pcie phy efuse = 0x%x", val);
	return (uint64_t)val;
}
...
static void pcie_ctrl_ffe_cfg(struct pcie_kport *pcie)
{
	...
	pcie_config_mac_ffe_map_test(pcie->rc_id, FIXED, P_0);
}
...
static void pcie_tp_mem_tra(struct pcie_kport *pcie)
{
	u32 val;

	val = pcie_apb_ctrl_readl(pcie, SOC_PCIECTRL_CTRL20_ADDR);
	val &= ~PCICTRL_TP_MEM_TRA_MASK;
	val |= PCICTRL_TP_MEM_TRA_BIT;
	pcie_apb_ctrl_writel(pcie, val, SOC_PCIECTRL_CTRL20_ADDR);
}

static void pcie_sp_mem_tra(struct pcie_kport *pcie)
{
	u32 val;

	val = pcie_apb_ctrl_readl(pcie, SOC_PCIECTRL_CTRL23_ADDR);
	val &= ~PCICTRL_SP_MEM_TRA_MASK;
	val |= PCICTRL_SP_MEM_TRA_BIT;
	pcie_apb_ctrl_writel(pcie, val, SOC_PCIECTRL_CTRL23_ADDR);
}

#ifdef CONFIG_PCIE_PHY_DIGITAL_GATHER
void ltssm_log_kthread_init(struct pcie_kport *pcie, u32 enable);
#endif
static int pcie_turn_on(struct pcie_kport *pcie, enum rc_power_status on_flag)
{
	...
	/* MEM ctrl TRA valued b'01 when Dual rail mem used as single rail mem, in order to improving yield rate */
	pcie_sp_mem_tra(pcie);
	pcie_tp_mem_tra(pcie);
...
#ifdef CONFIG_PCIE_PHY_DIGITAL_GATHER
	ltssm_log_kthread_init(pcie,ENABLE);
#endif
...
#ifdef CONFIG_PCIE_PHY_DIGITAL_GATHER
	ltssm_log_kthread_init(pcie,DISABLE);
#endif
	mutex_lock(&pcie->power_lock);
	if (!atomic_read(&(pcie->is_power_on))) {
