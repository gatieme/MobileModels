...
static void xw_phy3_stat_dump(struct pcie_phy *phy)
{
	uint32_t i;

	I("kband gen3[0x%x], kband gen2[0x%x]\n",
			xw_phy_r(phy, XW_PHY3_PLLBANDCTRLG3, 8, 0),
			xw_phy_r(phy, XW_PHY3_PLLBANDCTRLG2, 8, 0));
	I("txtermkn[0x%x], txtermkp[0x%x], rxtermk[0x%x]\n",
			xw_phy_r(phy, XW_PHY3_TERMKCTRL, 14, 10),
			xw_phy_r(phy, XW_PHY3_TERMKCTRL, 8, 4),
			xw_phy_r(phy, XW_PHY3_TERMKCTRL, 28, 24));

	I("adc offsetk error[0x%x]\n", xw_phy_r(phy, XW_PHY3_PCLK_SHARE_INT_SET, 13, 13));
	I("tx_dccp[0x%x], tx_dccn[0x%x]\n",
			xw_phy_r(phy, XW_PHY3_DCCK_RES, 5, 0),
			xw_phy_r(phy, XW_PHY3_DCCK_RES, 13, 8));
	for (i = 0; i < XW_PHY3_OFFSETK_NUMS; i++)
		I("RX_OFFSET_SA0/SA1/SA2/SA3/GS0/GS1 (G1/G2) rx_gain_sel=%d [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G1_2_SEL0 + 0x4 * i, 29, 0));

	for (i = 0; i <= 5; i++)
		I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=%d [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G1_2_EQ, 4 + 5 * i, 0 + 5 * i));

	I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=6 [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G1_2_EQ + 0x4, 4, 0));
	I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=7 [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G1_2_EQ + 0x4, 9, 5));

	for (i = 0; i < XW_PHY3_OFFSETK_NUMS; i++)
		I("RX_OFFSET_SA0/SA1/SA2/SA3/GS0/GS1 (G3) rx_gain_sel=%d [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G3_SEL0 + 0x4 * i, 29, 0));

	for (i = 0; i <= 5; i++)
		I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=%d [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G3_EQ, 4 + 5 * i, 0 + 5 * i));

	I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=6 [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G3_EQ + 0x4, 4, 0));
	I("RX_OFFSET_EQ result (G1/G2) rx_gain_sel=7 [0x%x]\n", i,
			xw_phy_r(phy, XW_PHY3_OFFSETK_G3_EQ + 0x4, 9, 5));

	I("tx v48 calibration value[0x%x]\n", xw_phy_r(phy, XW_PHY3_TX_V48_CALI_VAL, 6, 6));
}

static void xw_phy2_init(struct pcie_phy *phy)
{
	uint32_t reg_val;
...
static void xw_phy3_rc_init(struct pcie_phy *phy)
{
	xw_phy3_stat_dump(phy);

	phy->rtermk_v48 = xw_phy_r(phy, XW_PHY3_TX_V48_CALI_VAL, 6, 6);
	if (phy->rtermk_v48 == 1) {
		xw_phy_w(phy, XW_PHY3_PIPE_LOCAL, 5, 0, 0x39);
		xw_phy_w(phy, XW_PHY3_PIPE_LOCAL, 13, 8, 0x11);
	} else {
		xw_phy_w(phy, XW_PHY3_PIPE_LOCAL, 5, 0, 0x2A);
		xw_phy_w(phy, XW_PHY3_PIPE_LOCAL, 13, 8, 0xC);
	}
}

static void xw_phy3_ep_init(struct pcie_phy *phy)
{
	xw_phy_w(phy, XW_PHY3_BYPASS_ASYN_PWR_ACK, 0, 0, 0x1);
	xw_phy_w(phy, XW_PHY3_REFCLK_FROM_PAD, 1, 0, 0x2);
}

static void xw_phy3_init(struct pcie_phy *phy)
{
	I("+%s+\n", __func__);

	if (phy->trimming_done)
		xw_phy_write(phy, phy->efuse_val, XW_PHY_RG_BG_VREF_SEL_ADDR);
	...
	/* rg_rx_lfps_vref [2:0] set 0b11 : 90 mVpp */
	xw_phy_w(phy, XW_PHY3_RXLOSCTRL, 2, 0, 0x3);

	/* reg_rx_dfe_en [17] set disable */
	xw_phy_w(phy, XW_PHY3_RXEQACTRL2, 17, 17, 0);

	/* open pcs interrupts */
	xw_phy_write(phy, 0x0, XW_PHY3_PCS_INT);
...
{
	enum phy_rate phy_rate;
	phy_rate = phy->phy_rate;
...
static void down_bit_clear(struct pcie_phy *phy)
{
	...
	} else if (phy->phy_rate == PHY_RATE_GEN3) {
		reg_val = xw_phy_read(phy, XW_PHY3_PLLVCOK_CTRL);
		reg_val |= XW_PHY3_VCOK5G_DONE_CLEAR_BIT;
		reg_val |= XW_PHY3_VCOK8G_DONE_CLEAR_BIT;
		xw_phy_write(phy, reg_val, XW_PHY3_PLLVCOK_CTRL);

		reg_val = xw_phy_read(phy, XW_PHY3_RXOFSTUPDTCTRL_ADDR);
		reg_val |= XW_PHY3_CLE_EQOFSTK_DONE_BIT;
		reg_val |= XW_PHY3_CLE_SAOFSTK_DONE_BIT;
		reg_val |= XW_PHY3_CLE_GSOFSTK_DONE_BIT;
		xw_phy_write(phy, reg_val, XW_PHY3_RXOFSTUPDTCTRL_ADDR);
	} else {
		E("%s   error phy type\n", __func__);
	}
}

static bool is_xw_phy2_ready(struct pcie_phy *phy)
{
	uint32_t reg_val;
...
static bool is_xw_phy3_ready(struct pcie_phy *phy)
{
	uint32_t reg_val;
	int timeout = 0;

	while(1) {
		reg_val = xw_phy_read(phy, XW_PHY3_PCS_POWER_STATUS_ADDR);
		reg_val &= ~PHY_PCS_STATUS_MASK;
		if ((reg_val >> PHY_POWERON_STATUS_BIT) & 1) {
			D("PHY3 power on in %d us\n", timeout * 5);
			D("phy3: check phy init error(SHARE 0x400): 0x%x\n",
					xw_phy_read(phy, XW_PHY3_PCLK_SHARE_INT_SET));
			D("phy3: check phy init error(LANE 0xC00): 0x%x\n",
					xw_phy_read(phy, XW_PHY3_PCLK_LANE_INT_SET));
			down_bit_clear(phy);
		...
		udelay(5);
		timeout++;
		if (timeout > PHY3_POWERON_TIMEOUT) {
			E("PHY3 power on timeout!\n");
			E("phy3: check phy init error(SHARE 0x400): 0x%x\n",
					xw_phy_read(phy, XW_PHY3_PCLK_SHARE_INT_SET));
			E("phy3: check phy init error(LANE 0xC00): 0x%x\n",
					xw_phy_read(phy, XW_PHY3_PCLK_LANE_INT_SET));
			xw_phy3_stat_dump(phy);
			return false;
		}
...
static bool is_xw_phy_ready(struct pcie_phy *phy)
{
	...
	switch (phy_rate) {
	case PHY_RATE_GEN2:
		return is_xw_phy2_ready(phy);
	case PHY_RATE_GEN3:
		return is_xw_phy3_ready(phy);
	default:
		E("%s   error phy type\n", __func__);
		return is_xw_phy2_ready(phy);
	}
...
static void mask_non_fatal_interrupt(struct pcie_phy *phy)
{
	uint32_t val;

	/* mask vco_8g_err/tx_term_over_under/rx_term_over_under */
	val = xw_phy_read(phy, SHARE_IRQ_REG);
	val |= (VCO_8G_ERR_MK | RXTERM_OVUD_MK | TXTERM_OVUD_MK);
	xw_phy_write(phy, val, SHARE_IRQ_REG);

	/* mask rx_synchead/rx_symbol */
	val = xw_phy_read(phy, RX_IRQ_REG);
	val |= (RX_SYMB_ERR_MK | RX_SYNCHEADER_ERR_MK);
	xw_phy_write(phy, val, RX_IRQ_REG);

	/* mask tx_kcode_err/sym_err_cnt/g3_enc_buf_over_f/g3_enc_buf_under_f */
	val = xw_phy_read(phy, TX_IRQ_REG);
	val |= TX_IRQ_MASK;
	xw_phy_write(phy, val, TX_IRQ_REG);

	/* mask no_eios_det */
	val = xw_phy_read(phy, EIOS_DET_IRQ_REG);
	val |= EIOS_DET_IRQ_MASK;
	xw_phy_write(phy, val, EIOS_DET_IRQ_REG);

	/* mask pstproc/preproc/fom_timeout/eqa_timeout/gas_timeout */
	val = xw_phy_read(phy, PMA_IRQ_REG);
	val |= PMA_IRQ_MASK;
	xw_phy_write(phy, val, PMA_IRQ_REG);
}
...
static void init_phy_irq_to_acore(struct pcie_phy *phy, bool enable)
{
	...
	if (enable)
		mask_non_fatal_interrupt(phy);

	val = xw_phy_read(phy, PCS_TOP_MASK_REG);
	if (enable)
		val &= ~PCS_TOP_MASK_BIT;
	else
		val |= PCS_TOP_MASK_BIT;
	xw_phy_write(phy, val, PCS_TOP_MASK_REG);
}
...
struct pcie_phy_ops xw_phy_ops = {
	...
	.phy3_eye_monitor = NULL,
	.phy3_debug_info = NULL,
#endif
};
...
int32_t pcie_phy_register(struct platform_device *pdev)
{
	...
	ret = pcie_phy_bind(phy_info->id, phy_info, &xw_phy_ops);
	if (ret) {
		E("register phy ops\n");
		return ret;
	}
