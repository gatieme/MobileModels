...
static void pcie_get_pinctrl(struct pcie_kport *pcie,
				      struct platform_device *pdev)
{
	int ret;
	...
	if (pcie->gpio_id_reset < 0) {
		PCIE_PR_I("Failed to get perst gpio number");
		return;
	}
	...
	if (pcie->dtsinfo.ep_device_type != EP_DEVICE_WIFI &&
		...
		if (ret) {
			PCIE_PR_I("Failed to request gpio-%d", pcie->gpio_id_reset);
			pcie->gpio_id_reset = -1;
		}
	}
...
static void pcie_get_phy_type(struct pcie_kport *pcie, struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	struct pcie_dtsinfo *dtsinfo = &pcie->dtsinfo;
	u32 val;
	...
	ret = of_property_read_u32(np, "phy_type", &val);
	if (ret) {
		dtsinfo->phy_type.phy_rate = PCI_GEN2;
		dtsinfo->phy_type.vendor_type = XW_PHY;
		PCIE_PR_I("Failed to get phy_type, set default rate[0x%x] vendor[0x%x]",
			dtsinfo->phy_type.phy_rate, dtsinfo->phy_type.vendor_type);
	} else {
		dtsinfo->phy_type.phy_rate = val & PHY_CFG_RATE_MASK;
		dtsinfo->phy_type.vendor_type = (val & PHY_CFG_VENDOR_MASK) >> PHY_CFG_VENDOR_BIT;
		PCIE_PR_I("Current phy_type, rate[0x%x] vendor[0x%x]",
			dtsinfo->phy_type.phy_rate, dtsinfo->phy_type.vendor_type);
	}
}
...
#if !defined CONFIG_PCIE_KPORT_EP_FPGA_VERIFY && !defined CONFIG_PCIE_KPORT_JUN && !defined CONFIG_PCIE_KPORT_NAP
	return;
#endif
...
int pcie_rd_dma_conf(struct pcie_kport  *pcie, u32 where, u32 size)
{
	u32 reg_val = pcie_read_dbi(pcie->pci, pcie->pci->dbi_base,
		where | PCIE_DMA_LOCAL_OFFSET, size);

	return reg_val;
}

void pcie_wr_dma_conf(struct pcie_kport *pcie, u32 where, u32 size, u32 val)
{
	pcie_write_dbi(pcie->pci, pcie->pci->dbi_base,
		where | PCIE_DMA_LOCAL_OFFSET, size, val);
}
...
static void hck_pcie_refclk_host_vote(struct pcie_port *pp, u32 vote)
{
	pcie_refclk_host_vote(pp, vote);
}

static void hck_pcie_release_resource(struct device *dev, struct list_head *resources)
{
...
static void hck_pcie_bypass_pm(struct pci_dev *dev, bool *err)
{
	...
	class = ep_dev->class >> PCI_PROGRAM_INTERFACE;
...
#ifdef CONFIG_PCIE_KPORT_EP_FPGA_VERIFY
static void hck_pcie_stub_class(u32 *class, u32 new_class)
{
	PCIE_PR_I("PCIe: Stub FPGA Class Version\n");
	if (((*class & PCI_CLASS_CODE_MASK) >> PCI_CLASS_CODE_SHIFT) == 0)
		*class |= (new_class << PCI_CLASS_CODE_SHIFT);
}

static void hck_pcie_stub_bar(u32 *number)
{
	*number = 1;
}
#endif
...
static void register_trace_hck_pcie(void)
{
	...
	if (registered == false) {
		REGISTER_HCK_VH(kport_pcie_refclk_host_vote, hck_pcie_refclk_host_vote);
		REGISTER_HCK_VH(kport_pcie_release_resource, hck_pcie_release_resource);
		REGISTER_HCK_VH(kport_pcie_bypass_pm, hck_pcie_bypass_pm);
#ifdef CONFIG_PCIE_KPORT_EP_FPGA_VERIFY
		REGISTER_HCK_VH(kport_pcie_stub_class, hck_pcie_stub_class);
		REGISTER_HCK_VH(kport_pcie_stub_bar, hck_pcie_stub_bar);
#endif
		registered = true;
	}
}
...
#ifdef CONFIG_PCIE_KPORT_NAP
builtin_platform_driver(pcie_ep_driver);
#endif
#endif
