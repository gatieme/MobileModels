...
#define IRQ_AXI_TIMEOUT	5
#define IRQ_PHY			8
#define EXTRA_IRQ_NUM		1

#define IRQ_MSI_NAME	"pcie-msi"
#define IRQ_LINKDOWN_NAME	"pcie-linkdown"
#define IRQ_AXI_TIMEOUT_NAME	"pcie-axi-timeout"
#define IRQ_PHY_NAME		"pcie-phy-irq"
...
#define SOC_PCIECTRL_CTRL23_ADDR	0x05C
...
#define PCICTRL_TP_MEM_TRA_MASK	        (0x3 << 27)
#define PCICTRL_TP_MEM_TRA_BIT	        (0x1 << 27)
#define PCICTRL_SP_MEM_TRA_MASK	        (0x3 << 16)
#define PCICTRL_SP_MEM_TRA_BIT	        (0x1 << 16)
...
#define PCIE_PERST_IN_N_CTRL_BIT2     (0x1 << 2)
#define PCIE_PERST_IN_N_CTRL_BIT3     (0x1 << 3)
...
#define PORT_GEN3_EQ_LOCAL_FS_LF_OFF	0x894
#define PORT_GEN3_EQ_PSET_COEF_MAP		0x898
#define PORT_GEN3_EQ_PSET_INDEX_OFF  	0x89c
#define PORT_GEN3_EQ_COEFF_LEGALITY_STATUS 0x8a4
#define PORT_GEN3_EQ_CONTROL_REG	0x8a8
...
#define PCI_EYEPARAM_MAX_NUMS	0xF
...
#define PHY_CFG_VENDOR_MASK	0xF0
#define PHY_CFG_RATE_MASK	0xF
#define PHY_CFG_VENDOR_BIT	4
...
struct pcie_kport {
	...
	struct work_struct axi_timeout_work;
	...
	u32 ep_link_status;
	enum link_speed max_link_speed;
...
static inline void pcie_phy_ffe_cfg(struct pcie_kport *pcie)
{
	if (pcie->phy && pcie->phy_ops && pcie->phy_ops->phy_ffe_cfg)
		pcie->phy_ops->phy_ffe_cfg(pcie->phy);
}
...
{
	if (pcie->phy && pcie->mcu_ops && pcie->mcu_ops->phy_core_start)
		return pcie->mcu_ops->phy_core_start(pcie->phy);

	return 0;
}

static inline int32_t pcie_phy_core_utest(struct pcie_kport *pcie)
{
	if (pcie->phy && pcie->mcu_ops && pcie->mcu_ops->phy_core_utest)
		return pcie->mcu_ops->phy_core_utest(pcie->phy);
...
{
	if (pcie->phy && pcie->mcu_ops && pcie->mcu_ops->phy_core_clk_config)
		return pcie->mcu_ops->phy_core_clk_config(pcie->phy, !!en);
