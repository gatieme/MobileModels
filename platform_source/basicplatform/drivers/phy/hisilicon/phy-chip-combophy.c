...
#include "phy_chip_combophy_debugfs.h"
...
#define PERLANE_TXVCMCTRL_LANE0(base)	(USB_TCA_PERLANE_BASE(base) + 0x380)
#define PERLANE_TXVCMCTRL_LANE1(base)	(USB_TCA_PERLANE_BASE(base) + 0x780)
...
static const uint32_t combophy_default_pll0sscgcntr0 = 0x01040107u;
static const uint32_t combophy_default_txvcmctrl = 0x8u;
...
static int combophy_set_orien(struct hiusb_usb3phy *combophy,
			     enum hiusb_phy_orien orien)
{
	struct phy_priv *priv = chip_combophy_to_phy_priv(combophy);

	pr_info("set orien %d\n", orien);

	if (orien != HIUSB3_PHY_ORIEN_NORMAL &&
	     orien != HIUSB3_PHY_ORIEN_FLIP)
		return -EINVAL;

	mutex_lock(&combophy->phy_mutex);
	if (orien == HIUSB3_PHY_ORIEN_NORMAL)
		priv->typec_orien = TYPEC_ORIEN_POSITIVE;
	else
		priv->typec_orien = TYPEC_ORIEN_NEGATIVE;

	mutex_unlock(&combophy->phy_mutex);
	pr_info("typec_orien %d\n", priv->typec_orien);

	return 0;
}

static int combophy_set_dpmode(struct hiusb_usb3phy *combophy,
			     enum hiusb_phy_dp_mode dpmode)
{
	struct phy_priv *priv = chip_combophy_to_phy_priv(combophy);

	pr_info("set dpmode %d\n", dpmode);

	mutex_lock(&combophy->phy_mutex);
	if (dpmode == HIUSB3_PHY_DP_2LANE) {
		priv->phy_mode = TCPC_USB31_AND_DP_2LINE;
	} else if (dpmode == HIUSB3_PHY_DP_4LANE) {
		priv->phy_mode = TCPC_DP;
	} else if (dpmode == HIUSB3_PHY_DP_0LANE) {
		priv->phy_mode = TCPC_USB31_CONNECTED;
	} else {
...
#else
static int combophy_set_mode(struct hiusb_usb3phy *combophy,
...
static void combophy_config_phy_para(struct phy_priv *priv)
{
	...
	if (priv->is_combophy_v2) {
		writel(priv->txvcmctrl, PERLANE_TXVCMCTRL_LANE0(priv->usb_tca));
		writel(priv->txvcmctrl, PERLANE_TXVCMCTRL_LANE1(priv->usb_tca));
	}
}
...
{
	struct hiusb_usb3phy *combophy = phy_get_drvdata(phy);
...
test_stub:
	...
	chip_combophy_create_regdump_debugfs(priv);
	mutex_unlock(&combophy->phy_mutex);

	if (priv->debug)
		chip_combophy_regdump(combophy);
	pr_info("-\n");
...
static int combophy_exit(struct phy *phy)
{
	...
	mutex_lock(&combophy->phy_mutex);
	chip_combophy_destroy_regdump_debugfs(priv);
...
static void get_phy_param(struct device_node *np, struct phy_priv *priv)
{
	...
	if (priv->is_combophy_v2)
		of_get_combophy_param(txvcmctrl);
}
...
#ifdef CONFIG_USB_HIUSB
	priv->combophy.set_orien = combophy_set_orien;
	priv->combophy.set_dpmode = combophy_set_dpmode;
#else
	priv->combophy.set_mode = combophy_set_mode;
#endif
	priv->combophy.register_debugfs = chip_combophy_register_debugfs;
	priv->combophy.regdump = chip_combophy_regdump;
	mutex_init(&priv->combophy.phy_mutex);
