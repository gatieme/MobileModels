...
static inline unsigned pch_virt_read(void __iomem *reg)
{
	return pinctrl_hvc(PINCTRL_VM_ID | PINCTRL_READ, (size_t)reg, 0, 0);
}

static inline void pch_virt_write(unsigned val, void __iomem *reg)
{
	pinctrl_hvc(PINCTRL_VM_ID | PINCTRL_WRITE, (size_t)reg, (size_t)val, 0);
}

static inline void pch_mux_virt_set(size_t reg, size_t val)
{
...
#ifdef CONFIG_PINCRTL_VIRTUAL
		pch_mux_virt_set(vals->reg, vals->val);
#else
		if(vals->mux_reg) {
			    ...
			    struct pinctrl_gpio_range *range, unsigned pin)
{
	...
	list_for_each_safe(pos, tmp, &pch->gpiofuncs) {
		...
		mux_bytes = pch->width / BITS_PER_BYTE;
#ifdef CONFIG_PINCRTL_VIRTUAL
		pch_mux_virt_set(pch->base + pin * mux_bytes, frange->gpiofunc);
#else
...
#ifdef CONFIG_PINCRTL_VIRTUAL
	pch->read = pch_virt_read;
	pch->write = pch_virt_write;
#else
	pch->read = pch_readl;
