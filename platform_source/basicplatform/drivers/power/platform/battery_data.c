...
static unsigned int g_fcc_match_flag;
	...
	if ((id_voltage > g_p_data[id_index]->id_identify_min) &&
		...
		(id_voltage > g_p_data[id_index]->id_voltage_max))
			...
			power_dsm_report_format_dmd(POWER_DSM_BATTERY_DETECT,
				POWER_DSM_BATTERY_DETECT_ERROR_NO,
...
static int get_battery_data_by_id_sn(unsigned int id_index, unsigned int indentify_fcc,
		unsigned char *id_sn)
{
#ifdef CONFIG_HUAWEI_BATTERY_INFORMATION
	...
	if (indentify_fcc == 0)
		return 0;
	/* indentify by fcc */
	if (g_p_data[id_index]->fcc == indentify_fcc)
		return 0;

	return -EINVAL;
#else
	if (g_fcc_match_flag && (g_p_data[id_index]->fcc == indentify_fcc))
		return 0;
...
struct coul_battery_data *get_battery_data(unsigned int id_voltage, unsigned int indentify_fcc)
{
	unsigned int i;
	int ret;
	...
	if (ret) {
		bat_data_err("get id_sn from ic fail!\n");
		(void)memset_s(id_sn, ID_SN_SIZE, 0, ID_SN_SIZE);
	}
#endif
	for (i = 0; i < g_bat_data_size; i++) {
		if (!strncmp(g_p_data[i]->identify_type,
			...
			ret = get_battery_data_by_id_sn(i, indentify_fcc, id_sn);
			if (!ret)
				break;
		} else {
	...
	if (i == g_bat_data_size) {
		if ((g_default_batt_id_index >= 0) &&
			((unsigned int)g_default_batt_id_index <
						g_bat_data_size)) {
			i = (unsigned int)g_default_batt_id_index;
...
static int battery_data_probe(struct platform_device *pdev)
{
	...
	(void)of_property_read_u32(np, "fcc_match", &g_fcc_match_flag);
	bat_data_info("g_fcc_match_flag=%u\n", g_fcc_match_flag);
