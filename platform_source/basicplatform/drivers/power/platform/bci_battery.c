...
struct bci_device_info {
	...
	u32 battery_ov_th;
	u32 psy_use_cache;
	u32 bci_work_dc_interval;
	int last_cap_enable;
	int startup_delta_cap;
};
...
static enum power_supply_property bci_battery_props[] = {
...
#ifdef CONFIG_COUL_PMIC6X21V800
	POWER_SUPPLY_PROP_CURRENT_RAW,
#endif
	...
	POWER_SUPPLY_PROP_CATHODE_TYPE,
	POWER_SUPPLY_PROP_SERIAL_COUNT,
};
...
static int power_supply_get_prop_value(const char *name,
...
static int bci_health(void)
{
	...
	if (power_supply_get_prop_value("huawei-battery", POWER_SUPPLY_PROP_HEALTH, &val))
...
static int bci_capacity(void)
{
	...
	if (power_supply_get_prop_value("huawei-battery", POWER_SUPPLY_PROP_CAPACITY, &val))
...
static int bci_temp(void)
{
	int temp = 0;
#ifdef CONFIG_HUAWEI_BATTERY_TEMP
	if (bat_temp_get_temperature(BAT_TEMP_MIXED, &temp))
	...
	struct bci_device_info *di)
{
	...
	return (int)retval_cap;
}

/* exit scp (example : 87.5% to 87%) */
static void reset_capacity_fifo(struct bci_device_info *di,
	int rep_soc, int round_soc, int base)
{
	...
	for (i = 0; i < WINDOW_LEN; i++)
		g_capacity_filter[i] = (unsigned int)round_soc;

	di->capacity_filter_count = 0;
	g_capacity_sum = (unsigned int)(rep_soc * WINDOW_LEN / base);
	remaing = (int)g_capacity_sum - round_soc * WINDOW_LEN;

	for (i = 0; i < remaing; i++) {
		g_capacity_filter[i] += 1;
		di->capacity_filter_count = (unsigned int)i;
	}
}
	...
	struct bci_device_info *di)
{
	...
	if (coul_drv_low_temp_opt() && (bci_temp() < LOW_TEMP_FIVE_DEGREE) &&
		...
		di->monitoring_interval = WORK_INTERVAL_NOARMAL / HALF;
#ifdef CONFIG_DIRECT_CHARGER
	if (direct_charge_in_charging_stage() == DC_IN_CHARGING_STAGE && di->ac_online &&
		(di->monitoring_interval > di->bci_work_dc_interval))
		di->monitoring_interval = di->bci_work_dc_interval;
#endif
#ifdef CONFIG_WIRELESS_CHARGER
	...
	for (i = 0; i < WORK_INTERVAL_PARA_LEVEL; i++) {
		if ((capacity >= di->interval_data[i].cap_min) &&
			...
			di->monitoring_interval =
				(u16)di->interval_data[i].work_interval;
			break;
		}
	}
...
static int bci_get_get_ocv_by_cap(int cap, int *ocv)
{
	int ret = -1;

	ret = battery_ocv_get_ocv_by_cap(cap, ocv);
	if (ret || *ocv < OCV_BY_MAP_MIN || *ocv > OCV_BY_MAP_MAX) {
		bci_err("%s %d, get voltage by cap failed, ocv = %d\n",
			__func__, __LINE__, *ocv);
		ret = -1;
	}
	return ret;
}

static void bci_get_ocv_corresponds_to_zero_cap_by_basp(
	struct bci_device_info *di)
{
	int voltage = 0;
	int ret, i;

	ret = bci_get_get_ocv_by_cap(CAPACITY_ZERO, &voltage);
	if (ret) {
		bci_err("%s %d, get voltage by cap failed\n",
			__func__, __LINE__);
		unused(di);
		return;
	}
	bci_info("%s, voltage = %d\n",  __func__, voltage);

	for (i = 0; i < CAP_LOCK_PARA_LEVEL; i++)
	{
		if (di->vth_correct_data[i].cap == CAPACITY_ONE) {
			di->vth_correct_data[i].level_vol = voltage;
			bci_err("%s %d, di->vth_correct_data[%d].level_vol = %d \n",
				__func__, __LINE__,
				i, di->vth_correct_data[i].level_vol);
			break;
		}
	}
}
	...
	struct bci_device_info *di)
{
	...
	for (i = 0; i < CAP_LOCK_PARA_LEVEL; i++) {
		if ((curr_capacity < di->vth_correct_data[i].cap) &&
			(di->prev_capacity >= (unsigned int)di->vth_correct_data[i].cap) &&
			(di->bat_temperature >= 0)) {
			count = DEBOUNCE_TIMES;
			if (di->vth_correct_data[i].cap == CAPACITY_ONE)
				bci_get_ocv_corresponds_to_zero_cap_by_basp(di);

			while (count > 0) {
#ifdef CONFIG_HUAWEI_BATTERY_VOLTAGE
				/* if multi battery, get MIN voltage */
				bat_vot = hw_battery_voltage(BAT_ID_MIN);
#else
				bat_vot = coul_drv_battery_voltage();
#endif
				battery_volt = max(battery_volt, bat_vot);
				msleep(1000);
				count--;
			}
...
static void bci_get_ocv_corresponds_to_one_cap_by_basp(
	struct bci_device_info *di)
{
	int voltage = 0;
	int ret;

	ret = bci_get_get_ocv_by_cap(CAPACITY_ONE, &voltage);
	if (ret) {
		bci_err("%s %d, get voltage by cap failed\n",
			__func__, __LINE__);
		unused(di);
	...
	bci_info("%s, voltage = %d\n",  __func__, voltage);
	di->cap_one_vth = voltage;
}

static int recla_capacity_one_by_vth(struct bci_device_info *di,
	int cap)
{
	int curr_capacity = cap;
	int bat_volt, count;

	if ((di->bat_temperature >= 0) && (di->bat_temperature < 500) &&
		(cap > CAPACITY_ONE)&&
		(cap <= (CAPACITY_ONE + CAPACITY_GAP))) {
		bci_get_ocv_corresponds_to_one_cap_by_basp(di);
		bat_volt = coul_drv_battery_voltage();
		if(bat_volt <= di->cap_one_vth) {
			count = DEBOUNCE_TIMES;
			while(count > 0) {
				bat_volt = coul_drv_battery_voltage();
					if (bat_volt > di->cap_one_vth) {
					bci_err("%s recla cap one fail: vol = %d, count = %d, cap_one_vth = %d\n",
						__func__, bat_volt, count,
						di->cap_one_vth);
						break;
					}
				count--;
	...
	struct bci_device_info *di)
{
	...
	if (curr_capacity != di->prev_capacity) {
		if (abs((int)di->prev_capacity - curr_capacity) >=
			CHG_ODD_CAPACITY)
	...
	struct bci_device_info *di)
{
	...
	if (di->prev_capacity == (unsigned int)correct_capacity)
		return 0;
	...
	di->prev_capacity = (unsigned int)correct_capacity;
	return 1;
}
...
static int bci_get_capacity_map_by_basp(const int curr_capacity)
{
	int basp_capacity;
	int koffset, boffset;
	int cap;
	...
	cap = coul_drv_get_soc_by_ocv(DEFAULT_TEMP_TENTH, voltage);
	bci_info("%s, voltage = %d, cap = %d\n",  __func__, voltage, cap);

	if (cap <= CAPACITY_ONE * TENTH)
		return curr_capacity;

	min_t(int, cap, CAPACITY_FULL * TENTH);
	max_t(int, cap, CAPACITY_ZERO * TENTH);

	koffset = CAPACITY_FULL * TENTH * 10000/
			(CAPACITY_FULL * TENTH - cap);
	boffset = CAPACITY_FULL * cap * 10000 /
			(cap - CAPACITY_FULL * TENTH);
	basp_capacity = koffset * curr_capacity + boffset;
	if (basp_capacity <= 0)
		basp_capacity = CAPACITY_ZERO; // If the capacity is less than zero, take zero
	else
		basp_capacity = basp_capacity / 10000 + 1; // Rounds a number up
	bci_info("%s, k = %d, b = %d, curr_capacity = %d, basp_capacity = %d\n",
		__func__, koffset, boffset, curr_capacity, basp_capacity);

	min_t(int, basp_capacity, CAPACITY_FULL);
	max_t(int, basp_capacity, CAPACITY_ZERO);

	return basp_capacity;
}

static int capacity_changed(struct bci_device_info *di)
{
	int curr_capacity, cap, ret, basp_capacity;
	...
	if ((strstr(saved_command_line, "androidboot.swtype=factory") &&
			...
			(!di->bat_exist) || (is_fake_battery)) {
		curr_capacity = calc_capacity_from_voltage();
		di->recla_cap_one_en = 0;
		cap = 0;
	} else {
		cap = bci_capacity();
		if (di->capacity == -1)
	...
	basp_capacity = bci_get_capacity_map_by_basp(curr_capacity);

	bci_info("%s %d, cap = %d, basp_capacity = %d, monitoring_interval = %d\n",
		__func__, __LINE__, cap, basp_capacity, di->monitoring_interval);

	ret = check_curr_capacity(basp_capacity, di);
	if (ret != -1)
		return ret;

	if ((di->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
		 (di->charge_status == POWER_SUPPLY_STATUS_NOT_CHARGING)) {
		if (di->recla_cap_one_en)
			basp_capacity = recla_capacity_one_by_vth(di, basp_capacity);
	}

	if (judge_charge_status(di, &basp_capacity) == 0)
		return 0;

	ret = capacity_correct_check(basp_capacity, di);
	return ret;
}
	...
	unsigned long event, void *priv_data)
{
	...
	bci_info("[%s] power supply changed, soc %d\n", __func__, di->capacity);
...
static int check_batt_volt_overhigh(char *buf)
{
	...
	if (volt > (int)di->battery_ov_th) {
		...
		if (avg_volt > (int)di->battery_ov_th) {
			ret = snprintf_s(buf,
...
static int check_soc_vary_err(char *buf)
{
	...
	static unsigned int monitor_cnt;
	int deta_soc = 0;
	struct bci_device_info *di = g_bci_dev;
...
void batt_info_dump(char *pstr)
{
...
#ifdef CONFIG_HUAWEI_CHARGER_AP
	charge_get_vbus(&info_dump[VBUS]);
	info_dump[IBUS] = charge_get_ibus();
#endif
	...
	info_dump[TUSB] = power_temp_get_average_value(POWER_TEMP_USB_PORT);
	...
	char *buf, int i)
{
	if (batt_dsm_array[i].check_error(buf)) {
		di->bat_err = (unsigned int)batt_dsm_array[i].error_no;
#if defined(CONFIG_HUAWEI_DSM)
...
static void get_error_info(struct bci_device_info *di)
{
	...
	buf = (char *)kzalloc(DSM_BATTERY_MAX_SIZE, GFP_KERNEL);
	if (!buf) {
		bci_err("call kzalloc failed\n");
		return;
	}
...
#define SDP_CURRENT_MAX 500
#define CDP_CURRENT_MAX 1400
#define DCP_CURRENT_MAX 2000
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if (charge_get_charger_type() != CHARGER_REMOVED)
			val->intval = VOLTAGE_MAX * MILLI_TO_MICRO;
#endif
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = 0;
#ifdef CONFIG_HUAWEI_CHARGER
		if (chip_get_charger_type() == CHARGER_TYPE_SDP)
			val->intval = SDP_CURRENT_MAX * MILLI_TO_MICRO;
		else if (chip_get_charger_type() == CHARGER_TYPE_CDP ||
				 chip_get_charger_type() == CHARGER_TYPE_UNKNOWN)
			val->intval = CDP_CURRENT_MAX * MILLI_TO_MICRO;
		else if (chip_get_charger_type() == CHARGER_TYPE_DCP ||
				(chip_get_charger_type() == PLEASE_PROVIDE_POWER &&
				 charge_get_charger_type() != USB_EVENT_OTG_ID))
			val->intval = DCP_CURRENT_MAX * MILLI_TO_MICRO;
#endif
		break;
	default:
		return -EINVAL;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		val->intval = vbus * MILLI_TO_MICRO;
#endif
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = 0;
	...
	union power_supply_propval *val, struct bci_device_info *di)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		if (di->psy_use_cache)
			val->intval = coul_drv_battery_cc_cache();
		else
...
#endif
		break;
	case POWER_SUPPLY_PROP_CATHODE_TYPE:
		val->intval = (int)coul_drv_get_cathode_type();
		break;
	case POWER_SUPPLY_PROP_SERIAL_COUNT:
#ifdef CONFIG_HUAWEI_BATTERY_VOLTAGE
		val->intval = hw_battery_get_series_num();
#else
		val->intval = 1;
#endif
		break;
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		di->bat_current = coul_drv_battery_current();
		val->intval = di->bat_current;
		break;
#ifdef CONFIG_COUL_PMIC6X21V800 /* Current without filter */
	case POWER_SUPPLY_PROP_CURRENT_RAW:
		if (di->current_raw_en)
			val->intval = coul_drv_battery_current();
		else
			val->intval = 0;
		break;
#endif
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		...
		break;
	default:
		ret = get_property_extra(psp, val, di);
	}
	return ret;
}
...
{
	return (g_capacity_sum * (unsigned int)base) / WINDOW_LEN;
}
...
void bci_capacity_sync_filter(int rep_soc, int round_soc, int base)
{
	...
	reset_capacity_fifo(di, rep_soc, round_soc, base);

	/* step2: capacity changed (example: 86% to 87%) */
	if (prev_soc != round_soc) {
		bci_info("[%s] power supply changed, capacity %d, round_soc %d\n",
			__func__, di->capacity, round_soc);
		di->capacity = round_soc;
		di->prev_capacity = (unsigned int)round_soc;
		power_supply_changed(di->bat);
	}
	prev_soc = round_soc;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((di == NULL) || (kstrtol(buf, DEC, &val) < 0) || (val < 0) || (val > 3000)) /* mv */
		return -EINVAL;
	di->event = val;
	...
	struct platform_device *pdev, struct bci_device_info *di)
{
	...
	if (g_google_battery_node) {
		...
		if (IS_ERR(di->bat_google)) {
			bci_debug("failed to register battery\n");
...
static int bci_get_last_capacity(void)
{
	unsigned int val;
	int capacity;

	val = reg_spmi_read(COUL_HARDWARE_SAVE_LAST_SOC);
	if ((val & SAVE_LAST_SOC_FLAG) == 0) {
		bci_info("%s invalid last soc!\n", __func__);
		return -1;
	}
	capacity = (int)(val & SAVE_LAST_SOC);
	return capacity;
}

/* smooth first capacit to avoid capacit jump in startup step */
static int bci_get_startup_capacity(struct bci_device_info *di, int capacity)
{
	int last_powerdown_capacity;

	last_powerdown_capacity = bci_get_last_capacity();
	if ((di->last_cap_enable == 0) || (last_powerdown_capacity < 0))
		return capacity;

	if(abs(capacity - last_powerdown_capacity) < di->startup_delta_cap)
		capacity = last_powerdown_capacity;

	bci_info("battery capacity=%d, last powerdown capacity %d\n",
		capacity, last_powerdown_capacity);
	return capacity;
}

static void bci_save_last_capacity(int capacity)
{
	unsigned int val;

	val = (unsigned int)capacity;
	reg_spmi_write(COUL_HARDWARE_SAVE_LAST_SOC,
		SAVE_LAST_SOC_FLAG | (val & SAVE_LAST_SOC));
}

static void bci_save_powerdown_capacity(struct bci_device_info *di)
{
	if (di->capacity >= 0 && di->last_cap_enable != 0)
		bci_save_last_capacity(di->capacity);
}
...
static void capacity_info_init(struct bci_device_info *di)
{
	...
	di->power_supply_status = POWER_SUPPLY_HEALTH_GOOD;
	di->charge_status = POWER_SUPPLY_STATUS_DISCHARGING;
	battery_capacity = bci_capacity();
	di->capacity = bci_get_startup_capacity(di, battery_capacity);
	di->prev_capacity = di->capacity;

	for (i = 0; i < WINDOW_LEN; i++) {
		g_capacity_filter[i] = (unsigned int)di->capacity;
...
{
	int i;
	size_t array_size = sizeof(di->vth_correct_data);
	...
	count = of_property_count_elems_of_size(np, "vth_correct_para",
		sizeof(int));
	if ((count <= 0) || ((count % CAP_PARA_MEM_NUM) != 0) ||
		(count > (array_size / int_size))) {
		di->vth_correct_en = 0;
		bci_err("Uncorrect vth_correct_para size %d\n", count);
		return -EINVAL;
	}

	if (of_property_read_u32_array(np, "vth_correct_para",
			(u32 *)&di->vth_correct_data[0], count)) {
		di->vth_correct_en = 0;
	...
	struct bci_device_info *di)
{
	...
	for (i = 0; i < array_len; i++) {
		...
		ret = kstrtol(bci_data_string, DEC, &idata); /* length */
		if (ret)
			bci_err("kstrtol failed\n");
...
static void get_cap_dts_info(const struct device_node *np,
	struct bci_device_info *di)
{
	int ret;
	unsigned int last_cap_enable = 0;
	unsigned int startup_delta_cap = 0;

	ret = of_property_read_u32(np, "last_cap_enable", &last_cap_enable);
	if (ret)
		bci_err("dts:can not get last_cap_enable, use default : %u\n",
			last_cap_enable);
	di->last_cap_enable = (int)last_cap_enable;
	bci_info("dts:get last_cap_enable = %u\n", last_cap_enable);

	ret = of_property_read_u32(np, "startup_delta_cap", &startup_delta_cap);
	if (ret)
		bci_err("dts:can not get startup_delta_cap,use default: %u!\n",
			startup_delta_cap);
	di->startup_delta_cap = (int)startup_delta_cap;
	bci_info("dts:get startup_delta_cap = %u\n", startup_delta_cap);
}
	...
	struct bci_device_info *di)
{
	...
	if (of_property_read_u32(np, "current_raw_en",
		(u32 *)&di->current_raw_en)) {
		di->current_raw_en = 0;
		bci_err("error:get current_raw_en value failed, set default!\n");
	}
	bci_info("current_raw_en =%d", di->current_raw_en);
	...
	if (of_property_read_u32(np, "battery_ov_th", &di->battery_ov_th))
		di->battery_ov_th = BATT_VOLT_OVERHIGH_TH;

	if (of_property_read_u32(np, "bci_work_dc_interval", &di->bci_work_dc_interval))
		di->bci_work_dc_interval = WORK_INTERVAL_NOARMAL / HALF;

	bci_info("chg_full_rpt_thr=%u, chg_full_wait_times=%u, " \
		"charge_current_overhigh_th=%d, battery_ov_th=%u\n",
		di->chg_full_rpt_thr, di->chg_full_wait_times,
		g_charge_current_overhigh_th, di->battery_ov_th);
	...
	if (of_property_read_u32(np, "psy_use_cache", &di->psy_use_cache)) {
		di->psy_use_cache = 0;
		bci_info("[%s] do not find psy_use_cache\n", __func__);
	} else {
		bci_info("[%s] find psy_use_cache\n", __func__);
	}

	if (of_property_read_u32(np, "recla_cap_one_en",
		(u32 *)&di->recla_cap_one_en))
		di->recla_cap_one_en = 0;

	if (of_property_read_u32(np, "cap_one_vth", (u32 *)&di->cap_one_vth))
		di->cap_one_vth = BAT_VOL_3300;
...
static int bci_battery_probe(struct platform_device *pdev)
{
	...
	g_low_power_lock = wakeup_source_register(&pdev->dev, "low_power_wake_lock");
#else
	g_low_power_lock = wakeup_source_register("low_power_wake_lock");
#endif
	if (!g_low_power_lock) {
		bci_err("%s wakeup source register failed\n");
		return -ENOMEM;
	}
...
supply_regist_failed:
	wakeup_source_unregister(g_low_power_lock);
...
static void bci_battery_shutdown(struct platform_device *pdev)
{
	...
	bci_save_powerdown_capacity(di);
	bci_unregister_notifier(&di->nb, 1);
	cancel_delayed_work(&di->bci_monitor_work);
	wakeup_source_unregister(g_low_power_lock);
	bci_info("%s --\n", __func__);
	...
	pm_message_t state)
{
	...
	bci_info("%s:+\n", __func__);
	g_suspend_capacity = (unsigned int)bci_capacity();
	cancel_delayed_work(&di->bci_monitor_work);
	bci_info("%s:-\n", __func__);
	return 0;
}
...
static int bci_battery_resume(struct platform_device *pdev)
{
	...
	if ((di->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
		(di->charge_status == POWER_SUPPLY_STATUS_NOT_CHARGING)) {
		if ((g_suspend_capacity - (unsigned int)resume_capacity) >= DELTA_CAPACITY) { /* limit cap */
			g_capacity_sum = 0;
			for (i = 0; i < WINDOW_LEN; i++) {
				g_capacity_filter[i] = (unsigned int)resume_capacity;
				g_capacity_sum += g_capacity_filter[i];
			}
		}
	}
