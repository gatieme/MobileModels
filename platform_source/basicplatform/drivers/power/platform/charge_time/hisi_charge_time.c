...
static u64 get_current_time(void)
{
	...
	u64 now = dfx_getcurtime() / NSEC_PRE_SECOND;
#else
...
static int hisi_chg_time_partition_ready(void)
{
	...
	ret = _sys_access(p_name, 0);
	if (ret) {
		ct_err("%s()-line=%d, name:%s\n", __func__, __LINE__, p_name);
		return -1;
	}
	return 0;
}
...
static int hisi_chg_time_flash_open(int flags)
{
	...
	p_name[CHG_DUR_BUF_SHOW_LEN] = '\0';
	fd_dfx = _sys_open(p_name, flags, CHG_DUR_FILE_LIMIT);
	if (fd_dfx < 0)
		ct_err("%s()-line=%d\n", __func__, __LINE__);
...
int show_chg_time_log_head(char *buf, size_t dest_max)
{
	size_t len = strlen(buf);

	return snprintf_s(buf + len, dest_max - len, dest_max - len - 1, "cTime  ");
}

int show_chg_time_log_content(char *buf, size_t dest_max)
{
	int len = (int)strlen(buf);
	int chg_time_remaining = hisi_chg_time_remaining(0);

	return snprintf_s(buf + len, dest_max - len, dest_max - len - 1, "%-5d  ", chg_time_remaining);
}
	...
	if (di->batt_info.soc == (di->pre_capacity + 1)) {
		if (di->pre_capacity == di->start_capacity &&
			...
			di->cur_curve.step_time[di->batt_info.soc] =
				(int)(timestemp - di->pre_timestemp);
