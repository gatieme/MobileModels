 ...
 *
 * Description: HI6523 charger driver
 *
...
static void set_boot_weaksource_flag(void)
{
	...
	pmic_write_reg(WEAKSOURCE_FLAG_REG, reg_val);
#endif
}
...
static int hi6523_set_bat_comp(int value)
{
	 ...
	 * (Less than CHG_IR_COMP_MIN), reg is assigned to 0;
	 ...
	 * [Greater than CHG_IR_COMP_110MOHM, reg is assigned to 7;
	 */
...
static int hi6523_calc_charge_current_bias(int charge_current)
{
	 ...
	 */
		 ...
		 */
		i_bias = (i_delta_ma / 100) * 100; /* more than 100ma */
...
static int hi6523_apply_set_vdpm_cv(struct hi6523_dpm_policy_params *pdi,
		struct hi6523_device_info *di, int *last_vdpm)
{
	...
	tmp_index = pdi->vterm_vdpm_tbl_len - 1;
	vterm_tmp = vterm_vdpm[tmp_index].cv;
	pdi->vdpm_set_val = vterm_vdpm[tmp_index].vdpm_set_val;
	vterm_tmp = (di->term_vol_mv < vterm_tmp) ? di->term_vol_mv : vterm_tmp;
	pdi->vterm_set = (u8)((vterm_tmp - CHG_FAST_VCHG_MIN) / CHG_FAST_VCHG_STEP_50);
	pdi->vterm = vterm_tmp;
	pdi->ret_vdpm = hi6523_write_mask(CHG_OTG_RESERVE2,
			CHG_5V_DPM_SHIFT_MSK, CHG_5V_DPM_SHIFT, pdi->vdpm_set_val);
	if (pdi->ret_vdpm == 0)
		pdi->ret_vterm = hi6523_write_mask(CHG_FAST_VCHG_REG,
				CHG_FAST_VCHG_MSK, CHG_FAST_VCHG_SHIFT,
				pdi->vterm_set);
	*last_vdpm = VINDPM_4700;

	return (pdi->ret_vdpm || pdi->ret_vterm);
}

static void hi6523_apply_judge_vindpm(
	int vindpm, int *last_vdpm, struct hi6523_dpm_policy_params *pdi)
{
	if (vindpm >= *last_vdpm) {
		...
		pdi->ret_vterm = hi6523_write_mask(CHG_FAST_VCHG_REG,
			CHG_FAST_VCHG_MSK, CHG_FAST_VCHG_SHIFT, pdi->vterm_set);
		if (pdi->ret_vterm == 0) {
			pdi->ret_vdpm = hi6523_write_mask(CHG_OTG_RESERVE2,
					CHG_5V_DPM_SHIFT_MSK, CHG_5V_DPM_SHIFT,
					pdi->vdpm_set_val);
...
static int hi6523_apply_dpm_policy(int vindpm)
{
	...
	int i;
	struct hi6523_dpm_policy_params params = {0};
	...
	params.vterm_vdpm_tbl_len = (int)(sizeof(vterm_vdpm) / (sizeof(vterm_vdpm[0])));
	scharger_inf("vterm_vdpm_tbl_len is %d\n", params.vterm_vdpm_tbl_len);
	for (i = 0; i < params.vterm_vdpm_tbl_len; i++) {
		if (vindpm == vterm_vdpm[i].vdpm) {
			params.vterm = vterm_vdpm[i].cv;
			params.vdpm_set_val = vterm_vdpm[i].vdpm_set_val;
			break;
		}
	}

	if (i == params.vterm_vdpm_tbl_len) {
		ret = hi6523_apply_set_vdpm_cv(&params, di, &last_vdpm);
		return ret;
	}

	params.vterm = (di->term_vol_mv < params.vterm) ? di->term_vol_mv : params.vterm;
	params.vterm_set = (u8)((params.vterm - CHG_FAST_VCHG_MIN) / CHG_FAST_VCHG_STEP_50);
	scharger_inf("Do dpm_set, Vdpm: %d, Vterm: %d\n", vindpm, params.vterm);
	hi6523_apply_judge_vindpm(vindpm, &last_vdpm, &params);
	return (params.ret_vdpm || params.ret_vterm);
}
...
static int hi6523_config_otg_opt_param(void)
{
	...
	/* need to config regs according to otg current 3A */
	if (otg_lim_set != 3) {
...
static int hi6523_get_charger_state(void)
{
	...
	switch (data) {
	case 0: /* 0: charger not onsite */
	case 1: /* 1: charger not onsite */
	case 2: /* 2: charger not onsite */
		ret = 0; /* charger not onsite */
		break;
	case 3: /* 3: charger on site */
		ret = 1;
		break;
	default:
...
static int hi6523_get_vbus_gain_cal(int *vol)
{
	...
	/* 1: 1 bit right moving */
	value = (reg_val & CHG_GAIN_CAL_VALUE_MASK) >> 1;
	/* 4: 4 bits right moving */
...
int hi6523_get_vbus_mv(unsigned int *vbus_mv)
{
	 ...
	 */
	if (gain_cal != 800) {
		tmp = result * vref * 7 / 4096 * (100000 - 125 * (unsigned int)gain_cal) / 100000;
	} else {
...
static int hi6523_get_ibus_ma(void)
{
	...
	ibus = ibus_data * (unsigned int)ilimit / ibus_ref_data;
...
static int hi6523_vbat_sys(void)
{
	...
	for (i = 0; i < retry_times; ++i) {
		v_sample = lpm_adc_get_value(adc_channel_vbat_sys);
		if (v_sample < 0)
...
static int hi6523_get_vbat_sys(void)
{
	...
	if (cnt > 0)
		 /* average battery vol value = 3 * 1000 * s / c */
...
static int hi6523_dump_register(char *reg_value, int size, void *dev_data)
{
	...
	snprintf(buff, sizeof(buff), "%-8.2d", hi6523_get_ibus_ma());
	strncat(reg_value, buff, strlen(buff));
	snprintf(buff, sizeof(buff), "%-8.2d", vbus);
	strncat(reg_value, buff, strlen(buff));
	snprintf(buff, sizeof(buff), "%-8.2d", i_bias_all);
	strncat(reg_value, buff, strlen(buff));
	for (i = 0; i < HI6523_REG_TOTAL_NUM; i++) {
		snprintf(buff, sizeof(buff), "0x%-8x", reg[i]);
		strncat(reg_value, buff, strlen(buff));
	}
	return 0;
}
...
static int hi6523_get_register_head(char *reg_head, int size, void *dev_data)
{
	...
	strncat(reg_head, buff, strlen(buff));
	for (i = 0; i < HI6523_REG_TOTAL_NUM; i++) {
		snprintf(buff, sizeof(buff), "Reg[0x%2x] ", i);
		strncat(reg_head, buff, strlen(buff));
	}
	return 0;
}
...
static int hi6523_set_watchdog_timer(int value)
{
	...
	if (dog_time >= WATCHDOG_TIMER_40_S)
		val = 3; /* 3: for 40S timer */
	else if (dog_time >= WATCHDOG_TIMER_20_S)
		val = 2; /* 2: for 20S timer */
 ...
 * initializied
 */
static int hi6523_is_water_intrused(void *dev_data)
{
	u32 dm_data = 0;
	u32 dp_data = 0;
	u32 dm_vol;
...
static int hi6523_chip_init(struct charge_init_data *init_crit)
{
#define RET_SIZE 16
	int ret[RET_SIZE] = {0};
	int i;
	...
	ret[2] = hi6523_set_charge_enable(CHG_DISABLE); /* 2: chg_en */
	ret[3] = hi6523_set_fast_safe_timer(CHG_FASTCHG_TIMER_20H); /* 3: fs_t */
	ret[4] = hi6523_set_term_enable(CHG_TERM_DIS); /* 4: term_en */
	ret[5] = hi6523_set_input_current(CHG_ILIMIT_470); /* 5: iin */
	ret[6] = hi6523_set_charge_current(CHG_FAST_ICHG_500MA); /* 6: ichg */
	ret[7] = hi6523_set_terminal_voltage(CHG_FAST_VCHG_4400); /* 7:vterm */
	ret[8] = hi6523_set_terminal_current(CHG_TERM_ICHG_150MA); /* 8:iterm */
	ret[9] = hi6523_set_watchdog_timer(WATCHDOG_TIMER_40_S); /* 9: whgt */
	ret[10] = hi6523_set_precharge_current(CHG_PRG_ICHG_200MA); /* 10:ipchg */
	ret[11] = hi6523_set_precharge_voltage(CHG_PRG_VCHG_2800); /* 11:vpchg */
	ret[12] = hi6523_set_batfet_ctrl(CHG_BATFET_EN); /* 12:batctrl */
	ret[13] = hi6523_set_bat_comp(di->param_dts.bat_comp); /* 13:batcomp */
	ret[14] = hi6523_set_otg_current(BOOST_LIM_1000); /* 14: iotg */
	ret[15] = hi6523_set_otg_enable(CHARGE_OTG_DISABLE); /* 15: otg_en */
 ...
 */
static void hi6523_plugout_check_process(enum chip_charger_type type)
{
	struct hi6523_device_info *di = g_hi6523_dev;
				    ...
				    unsigned long event, void *data)
{
	...
	di->charger_type = (enum chip_charger_type)event;
			...
			long temp, struct scharger_check_voltage *water)
{
	...
	if (dm_ary_idx % WATER_VOLT_PARA) {
		if (kstrtol(chrg_vol_string, base, &tmp) == 0) {
			water->dm_vol_data[dm_ary_idx / WATER_VOLT_PARA].vol_max = (unsigned int)tmp;
			scharger_inf("dm_vol[%d]max:%d\n",
				     dm_ary_idx / WATER_VOLT_PARA, tmp);
		}
	} else {
		if (kstrtol(chrg_vol_string, base, &tmp) == 0) {
			water->dm_vol_data[dm_ary_idx / WATER_VOLT_PARA].vol_min = (unsigned int)tmp;
			...
			long temp, struct scharger_check_voltage *water)
{
	...
	if (dp_ary_idx % WATER_VOLT_PARA) {
		if (kstrtol(chrg_vol_string, base, &tmp) == 0) {
			water->dp_vol_data[dp_ary_idx / WATER_VOLT_PARA].vol_max = (unsigned int)tmp;
			scharger_inf("dp_vol[%d]max = %d\n",
				     dp_ary_idx / WATER_VOLT_PARA, tmp);
		}
	} else {
		if (kstrtol(chrg_vol_string, base, &tmp) == 0) {
			water->dp_vol_data[dp_ary_idx / WATER_VOLT_PARA].vol_min = (unsigned int)tmp;
		      ...
		      struct hi6523_device_info *di)
{
	...
	batt_node =
		of_find_compatible_node(NULL, NULL, "hisilicon,bci_battery");
	if (batt_node != NULL) {
...
static int hi6523_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	di->charger_type = chip_get_charger_type();
	...
	ret = charge_ops_register(&hi6523_ops, BUCK_IC_TYPE_THIRDPARTY);
	if (ret) {
