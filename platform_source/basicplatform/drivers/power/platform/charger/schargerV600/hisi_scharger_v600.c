...
static void hi6526_buck_opt_param(int vbus_vol)
{
	...
	if (di == NULL) {
		...
		hi6526_opt_regs_set(v510_buck_common_opt_regs,
			    	ARRAY_SIZE(v510_buck_common_opt_regs));
	}
	switch (vbus_vol) {
	case VBUS_VSET_5V:
		if ((di->hi6526_version != CHIP_VERSION_V100) && (di->hi6526_version != CHIP_VERSION_V200))
			hi6526_opt_regs_set(v510_buck_5v_extra_opt_regs,
				    	ARRAY_SIZE(v510_buck_5v_extra_opt_regs));
		break;
	case VBUS_VSET_9V:
		if ((di->hi6526_version != CHIP_VERSION_V100) && (di->hi6526_version != CHIP_VERSION_V200))
			hi6526_opt_regs_set(v510_buck_9v_extra_opt_regs,
				    	ARRAY_SIZE(v510_buck_9v_extra_opt_regs));
		break;
	case VBUS_VSET_12V:
		if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
			...
			hi6526_opt_regs_set(v510_buck_12v_extra_opt_regs,
				    	ARRAY_SIZE(v510_buck_12v_extra_opt_regs));
		}
		break;
	default:
		break;
...
int hi6526_set_charge_current(int charge_current)
{
	...
	/* Chip limit */
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
		max_curr = CHG_FAST_ICHG_2500MA;
	else
		max_curr = CHG_FAST_ICHG_3040MA;
