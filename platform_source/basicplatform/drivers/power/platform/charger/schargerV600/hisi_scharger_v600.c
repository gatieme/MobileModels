...
#endif
#include "hisi_scharger_v600_include.h"
...
#if defined (CONFIG_SYSFS_SCHG) && defined (CONFIG_HUAWEI_CHARGER_AP)
/*
...
{
	size_t i;
	size_t limit = ARRAY_SIZE(hi6526_sysfs_field_tbl);
...
int hi6526_get_vbus_uvp_state(void)
{
	...
	int ret;
	struct hi6526_device_info *di = g_hi6526_dev;
	u32 chg_irq_status0_addr,chg_irq_status6_addr;

	if (di == NULL)
		return -ENOMEM;

	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		chg_irq_status0_addr = CHG_IRQ_STATUS0;
		chg_irq_status6_addr = CHG_IRQ_STATUS6;
	}else {
		chg_irq_status0_addr = V510_CHG_IRQ_STATUS0;
		chg_irq_status6_addr = V510_CHG_IRQ_STATUS6;
	}

	if (di->param_dts.vusb_uv_flag) {
		ret = hi6526_read(chg_irq_status6_addr, &val);
		scharger_dbg("%s:0x%x\n", __func__, val);
		if (ret) {
		...
		ret = hi6526_read(chg_irq_status0_addr, &val);
		scharger_dbg("%s:0x%x\n", __func__, val);
		if (ret) {
			scharger_err("%s read failed:%d\n", __func__, ret);
			return ret;
...
int set_buck_mode_enable(int enable)
{
	...
	if (enable) {
		...
		(void)hi6526_write_mask(CHG_ANTI_REVERBST_REG, CHG_ANTI_REVERBST_RESET_MASK,
			CHG_ANTI_REVERBST_RESET_SHIFT, 1);
		(void)hi6526_write_mask(CHG_ANTI_REVERBST_REG, CHG_ANTI_REVERBST_RESET_MASK,
			CHG_ANTI_REVERBST_RESET_SHIFT, 0);
	}
...
int hi6526_set_bat_comp(int value)
{
	...
	if (di->hi6526_version == CHIP_VERSION_V100 || di->hi6526_version == CHIP_VERSION_V200) {
		max_comp_val = CHG_IR_COMP_MAX;
		comp_step = CHG_IR_COMP_STEP_15;
	} else {
		max_comp_val = V510_CHG_IR_COMP_MAX;
		comp_step = CHG_IR_COMP_STEP_8;
	}

	if (value < CHG_IR_COMP_MIN)
		value = CHG_IR_COMP_MIN;
	else if (value > max_comp_val)
		value = max_comp_val;

	reg = (u8)(value / comp_step);
...
int hi6526_set_fast_safe_timer(u32 safe_timer)
{
	u32 reg_val;
	struct hi6526_device_info *di = g_hi6526_dev;
	...
	switch (safe_timer) {
	case CHG_FASTCHG_TIMER_5H:
		if (di->hi6526_version == CHIP_VERSION_V100 || di->hi6526_version == CHIP_VERSION_V200)
			reg_val = 0;
		else
			reg_val = 1;
		break;
	case CHG_FASTCHG_TIMER_8H:
		reg_val = 1;
		break;
	case CHG_FASTCHG_TIMER_12H:
		reg_val = 2;
		break;
	case CHG_FASTCHG_TIMER_20H:
		reg_val = 3;
		break;
	default:
		scharger_err("%s: safe timer mode err\n", __func__);
		return -EINVAL;
	}
...
static int hi6526_get_charge_enable(void)
{
	u8 charge_state = 0;
...
static void hi6526_buck_opt_param(int vbus_vol)
{
	...
	else if (di->hi6526_version == CHIP_VERSION_V510)
		hi6526_opt_regs_set(v510_buck_common_opt_regs,ARRAY_SIZE(v510_buck_common_opt_regs));
	else
		hi6526_opt_regs_set(v511_buck_common_opt_regs,ARRAY_SIZE(v511_buck_common_opt_regs));

	switch (vbus_vol) {
	case VBUS_VSET_5V:
		if (di->hi6526_version == CHIP_VERSION_V510)
			hi6526_opt_regs_set(v510_buck_5v_extra_opt_regs,ARRAY_SIZE(v510_buck_5v_extra_opt_regs));
		else if (di->hi6526_version == CHIP_VERSION_V511)
			hi6526_opt_regs_set(v511_buck_5v_extra_opt_regs,ARRAY_SIZE(v511_buck_5v_extra_opt_regs));
		break;
	case VBUS_VSET_9V:
		if (di->hi6526_version == CHIP_VERSION_V510)
			hi6526_opt_regs_set(v510_buck_9v_extra_opt_regs,ARRAY_SIZE(v510_buck_9v_extra_opt_regs));
		else if (di->hi6526_version == CHIP_VERSION_V511)
			hi6526_opt_regs_set(v511_buck_9v_extra_opt_regs,ARRAY_SIZE(v511_buck_9v_extra_opt_regs));
		break;
	case VBUS_VSET_12V:
		if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
			hi6526_opt_regs_set(buck_12v_extra_opt_regs,ARRAY_SIZE(buck_12v_extra_opt_regs));
		else if (di->hi6526_version == CHIP_VERSION_V510)
			hi6526_opt_regs_set(v510_buck_12v_extra_opt_regs,ARRAY_SIZE(v510_buck_12v_extra_opt_regs));
		else
			hi6526_opt_regs_set(v511_buck_12v_extra_opt_regs,ARRAY_SIZE(v511_buck_12v_extra_opt_regs));
		break;
	default:
		break;
	}

	if (di->param_dts.r_coul_uohm == R_UOHM_10K) {
...
int hi6526_set_input_current(int cin_limit)
{
	...
	if (cin_limit < CHG_ILIMIT_130)
		...
		i_in_limit = (u8)(cin_limit / CHG_ILIMIT_STEP_100);
	/*
...
void hi6526_set_input_current_limit(int enable)
{
	...
	if (enable) {
		if (di->buck_vbus_set < VBUS_VSET_9V) {
			if (strstr(saved_command_line, "androidboot.swtype=factory")) {
				if (di->input_current > CHG_ILIMIT_1500)
					(void)hi6526_set_input_current(CHG_ILIMIT_1500);
			} else {
			...
			if (strstr(saved_command_line, "androidboot.swtype=factory")) {
				if (di->input_current > CHG_ILIMIT_825) {
					(void)hi6526_set_input_current(CHG_ILIMIT_825);
					scharger_inf("%s set CHG_ILIMIT 825\n", __func__);
				}
			} else {
...
static int hi6526_calc_charge_current_bias(int charge_current)
{
	 ...
	 */
#ifdef CONFIG_HUAWEI_CHARGER_AP
	i_coul_ma = -coul_drv_battery_current() / bc_ic_get_convert_ratio();
#else
	i_coul_ma = -coul_drv_battery_current();
#endif
	/*
...
static int hi6526_calc_gap_mode_bias(int charge_current)
{
	int vbat;
	int charge_current_calc = charge_current;

	vbat = coul_drv_battery_voltage();
	/* low battery voltage GAP mode: vbat <= SYSMIN_3500MV + 150mv - MARGIN_250MV */
	if (vbat <= VBAT_GAP_THRE) {
		charge_current_calc = charge_current * CHG_FAST_ICHG_STEP_160 / CHG_FAST_ICHG_STEP_100;
		scharger_inf("%s low vol gap mode, set charge_current:%d\n", __func__, charge_current_calc);
		return charge_current_calc;
	}

	/*
	 * force GAP mode:
	 * 1.vbat > SYSMIN_3500MV + 150mv - MARGIN_250MV
	 * 2.the charge current is configured as the lowest three gears.
	 */
	if (charge_current <= CHG_FAST_ICHG_300MA) {
		charge_current_calc = charge_current * CHG_FAST_ICHG_STEP_160 / CHG_FAST_ICHG_STEP_100;
	} else if (charge_current < CHG_FAST_ICHG_640MA) {
		charge_current_calc = CHG_FAST_ICHG_300MA;
		charge_current_calc = charge_current_calc * CHG_FAST_ICHG_STEP_160 / CHG_FAST_ICHG_STEP_100;
	}
	scharger_inf("%s change charge_current:%d to %d\n", __func__, charge_current, charge_current_calc);
...
int hi6526_set_charge_current(int charge_current)
{
	...
	/* Chip limit */
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
		min_curr = CHG_FAST_ICHG_100MA;
	else
		min_curr = CHG_FAST_ICHG_160MA;

	charge_current = hi6526_calc_charge_current_bias(charge_current);

	if ((di->hi6526_version == CHIP_VERSION_V510) || (di->hi6526_version == CHIP_VERSION_V511))
		charge_current = hi6526_calc_gap_mode_bias(charge_current);
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
		...
		i_chg_limit = (u8)((charge_current / CHG_FAST_ICHG_STEP_160) - 1);
 ...
 */
static int __set_terminal_voltage1(int charge_voltage)
{
	u8 data;
	struct hi6526_device_info *di = g_hi6526_dev;
	int max_vol = 0;
	...
	if (di->hi6526_version == CHIP_VERSION_V100 || di->hi6526_version == CHIP_VERSION_V200)
		max_vol = CHG_FAST_VCHG_MAX;
	else if (di->hi6526_version == CHIP_VERSION_V510)
		max_vol = V510_CHG_FAST_VCHG_MAX;

	scharger_inf("%s charge_voltage:%d   max_vol: %d\n", __func__, charge_voltage,max_vol);

	if (charge_voltage < CHG_FAST_VCHG_MIN)
		charge_voltage = CHG_FAST_VCHG_MIN;
	else if (charge_voltage > max_vol)
		charge_voltage = max_vol;
...
static void hi6526_overtemp_fault_hander(void)
{
	int tbat = 0;
	int vbat = 0;
	int ret = 0;
	...
	if (tbat >= V511_BUCK_OVER_TEMP && vbat > di->cv_val) {
		scharger_inf("%s disable charger:  tbat = %d, vbat = %d\n", __func__,tbat,vbat);
		di->overtemp_flag = 1;
	}
}

static int __set_terminal_voltage2(int charge_voltage)
{
	u8 data1 = V511_CV_INIT_REG_VAL;
	u8 data2 = 0;
	int ret = 0;
	int flag = 0;
	struct hi6526_device_info *di = g_hi6526_dev;
	...
	if (charge_voltage < V511_CHG_FAST_VCHG_MIN) {
		hi6526_overtemp_fault_hander();
		charge_voltage = V511_CHG_FAST_VCHG_MIN;
	} else if (charge_voltage > V511_CHG_FAST_VCHG_MAX) {
		charge_voltage = V511_CHG_FAST_VCHG_MAX;
	}

	di->term_vol_mv = charge_voltage;

	data1 += (u8)((charge_voltage - V511_CHG_FAST_VCHG_MIN) * 1000 / CHG_FAST_VCHG_STEP_16600UV);
	data2 = (u8)((charge_voltage - V511_CHG_FAST_VCHG_MIN) * 1000 % CHG_FAST_VCHG_STEP_16600UV / 1000);
	scharger_dbg("%s charge_voltage:%d   data1: %d  data2: %d\n", __func__, charge_voltage,data1,data2);

	if (data2 < CHG_FAST_VCHG_8MV) {
		flag = PLUS;
	} else if (data2 == CHG_FAST_VCHG_8MV) {
		flag = PLUS;
		data2 = V511_VCHG_MINOR_REG_MAX_VAL;
	} else {
		flag = SUB;
		data1++;
		data2 = CHG_FAST_VCHG_16MV - data2;
	}

	ret += hi6526_write_mask(CHG_FAST_VCHG_REG, CHG_FAST_VCHG_MSK,CHG_FAST_VCHG_SHIFT, data1);
	ret += hi6526_write_mask(V511_VCHG_MINOR_ADJUST_ADDR, V511_VCHG_MINOR_MASK,
				 V511_VCHG_MINOR_SHIFT, flag);
	ret += hi6526_write_mask(V511_VCHG_MINOR_ADJUST_ADDR, V511_VTERM_MINUTE_ADJUST_MASK,
				 V511_VTERM_MINUTE_ADJUST_SHIFT, data2);

	return ret;
}

int __set_terminal_voltage(int charge_voltage)
{
	struct hi6526_device_info *di = g_hi6526_dev;
	...
	if (di->hi6526_version == CHIP_VERSION_V100 || di->hi6526_version == CHIP_VERSION_V200
			|| di->hi6526_version == CHIP_VERSION_V510)
		return __set_terminal_voltage1(charge_voltage);
	else
		return __set_terminal_voltage2(charge_voltage);
...
int hi6526_set_terminal_voltage(int charge_voltage)
{
	...
	di->cv_val = charge_voltage;

	if (g_hi6526_cv_info->cv_ori_val != charge_voltage) {
		g_hi6526_cv_info->cv_trim_flag = 0;
		scharger_err("%s vterm:%d changed, reset vterm:%d\n", __func__,g_hi6526_cv_info->cv_ori_val, charge_voltage);
	}

	if (g_hi6526_cv_info->cv_trim_flag == 1) {
		charge_voltage = g_hi6526_cv_info->cv_new_set_val;
		scharger_err("%s vterm:%d trimmed, use trimmed value:%d\n",__func__,
						g_hi6526_cv_info->cv_ori_val, g_hi6526_cv_info->cv_new_set_val);
	}
...
static int hi6526_get_charge_state(unsigned int *state)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		chg_status_reg = CHG_STATUS_REG;
	}else {
		chg_status_reg = V510_CHG_IRQ_STATUS8;
	}
	...
	*state = 0;
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		...
		if ((reg2 & HI6526_V510_WATCHDOG_MSK) != HI6526_V510_WATCHDOG_OK)
			*state |= CHAGRE_STATE_WDT_FAULT;
	}
...
int hi6526_set_charge_enable(int enable)
{
	...
	if (di->hi6526_version == CHIP_VERSION_V511 && di->overtemp_flag)
		enable = 0;
...
int hi6526_set_otg_enable(int enable)
{
	...
	if (enable) {
		if (di->hi6526_version == CHIP_VERSION_V100 || di->hi6526_version == CHIP_VERSION_V200)
			...
			hi6526_opt_regs_set(v510_otg_opt_regs, ARRAY_SIZE(v510_otg_opt_regs));
		else
			hi6526_opt_regs_set(v511_otg_opt_regs, ARRAY_SIZE(v511_otg_opt_regs));
...
static int hi6526_get_charger_state(void)
{
	...
	switch (data) {
	case 0: /* 0: not charging state */
	case 1: /* 1: not charging state */
	case 2: /* 2: not charging state */
		ret = 0; /* not charging */
		break;
	case 3: /* 3: charging state */
		ret = 1; /* charging */
		break;
	default:
...
{
	struct hi6526_debug_value value = {0};
	int i;
	int ret;
	int offset;
	...
	ret = hi6526_get_vbus_mv((unsigned int *)&(value.vbus));
	ret += hi6526_get_ibat(&(value.ibat));
	ret += hi6526_get_vusb(&(value.vusb));
	ret += hi6526_get_vout(&(value.vout), di);
	value.vbat = hi6526_get_vbat();
	value.ibus = hi6526_get_ibus_ma();
	ret += hi6526_get_chip_temp(&(value.temp));
	ret += hi6526_get_tsbat(&(value.tbat));
	if (ret)
	...
	val_array[0] = value.ibus; /* ibus site */
	val_array[1] = value.vbus;
	val_array[2] = value.ibat;
	val_array[3] = value.vusb;
	val_array[4] = value.vout;
	val_array[5] = value.vbat;
	val_array[6] = i_bias_all;
	val_array[7] = value.temp;
	val_array[8] = value.tbat;
...
static int hi6526_dump_register(char *buffer, int size, void *dev_data)
{
	...
	int str_len = size - 1;
	errno_t err;
	size_t len = str_len - strlen(buffer) - 1;
	struct hi6526_device_info *di = g_hi6526_dev;
...
static int hi6526_set_ship_mode_enable(bool enable)
{
	return hi6526_set_batfet_disable(enable);
}
...
int hi6526_set_watchdog_timer(int value)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		watch_ctrl_reg = WATCHDOG_CTRL_REG;
	}else {
		watch_ctrl_reg = V510_WATCHDOG_CTRL_REG;
	}
	...
	return hi6526_write_mask(watch_ctrl_reg, WATCHDOG_TIMER_MSK,
				 WATCHDOG_TIMER_SHIFT, val);
}

int hi6526_set_watchdog_timer_ms(int ms)
{
	return hi6526_set_watchdog_timer(ms / 1000); /* 1000: micro second to second */
}
...
int hi6526_ibat_res_sel(int resisitance)
{
	...
	if (resisitance == R_UOHM_2K)
		val = 0;
	else if ((resisitance == R_UOHM_5K) || (resisitance == R_UOHM_10K))
		val = 1;
	else
...
int cv_pre_trim_cali_value_get(int trim_a, u8 src_val, u8 *des_val)
{
	size_t index = 0;
	size_t i, index_tmp;
	size_t limit = ARRAY_SIZE(ate_trim_tbl);
	...
	index_tmp = index + (unsigned int)trim_a;
...
int hi6526_set_pretrim_code(int val)
{
	...
	if (di->hi6526_version == CHIP_VERSION_V200)
		ret = hi6526_efuse_read(EFUSE2, EFUSE_BYTE7, &ate_code, 0);
	else
		ret = hi6526_efuse_read_2(EFUSE_BYTE15, &ate_code, 0);
	...
	if (di->hi6526_version == CHIP_VERSION_V200)
		...
		ret = hi6526_efuse_write_2(EFUSE_BYTE15, pre_trim_code, 1);
...
#ifdef CONFIG_DIRECT_CHARGER
static int hi6526_dc_charge_init(void *dev_data)
{
	unused(dev_data);
	return 0;
}

static int hi6526_dc_discharge(int val, void *dev_data)
{
	unused(dev_data);
	return 0;
}

static int hi6526_dc_charge_exit(void *dev_data)
{
	unused(dev_data);
	return hi6526_switch_to_buck_mode();
}

static int hi6526_sc_charge_enable(int enable, void *dev_data)
{
	unused(dev_data);
	return hi6526_sc_enable(enable);
}

static int hi6526_sc_charge_is_close(void *dev_data)
{
	unused(dev_data);
	return hi6526_sc_is_close();
}

static int hi6526_get_sc_device_id(void *dev_data)
{
	unused(dev_data);
	return hi6526_get_switchcap_id();
}

static int hi6526_dc_config_watchdog_ms(int time, void *dev_data)
{
	unused(dev_data);
	return hi6526_set_watchdog_timer_ms(time);
}

static int hi6526_dc_kick_watchdog_timer(void *dev_data)
{
	unused(dev_data);
	return hi6526_reset_watchdog_timer();
}

static int hi6526_dc_get_vbat_mv(void *dev_data)
{
	unused(dev_data);
	return hi6526_get_vbat();
}

static int hi6526_dc_get_vbus_mv(int *vbus, void *dev_data)
{
	unused(dev_data);
	return hi6526_get_vbus_mv2(vbus);
}

static int hi6526_dc_get_ibat_ma(int *ibat, void *dev_data)
{
	unused(dev_data);
	return hi6526_get_ibat(ibat);
}

static int hi6526_dc_get_ibus_ma(int *ibus, void *dev_data)
{
	unused(dev_data);
	return hi6526_batinfo_get_ibus_ma(ibus);
}

static int hi6526_dc_get_chip_temp(int *temp, void *dev_data)
{
	unused(dev_data);
	return hi6526_get_chip_temp(temp);
}
#endif

static int hi6526_get_chg_mode(void)
{
	struct hi6526_device_info *di = get_hi6526_device();
	...
	scharger_inf("chg_mode %d \n",di->chg_mode);
	return di->chg_mode;
}

static int hi6526_cv_get_vterm(void)
{
	struct hi6526_device_info *di = get_hi6526_device();
...
static void hi6526_reset_cv_value(void)
{
	int ret;
	u8 efuse_ori_val = 0;
	struct hi6526_device_info *di = get_hi6526_device();
	...
	scharger_inf("%s +", __func__);

	if (g_hi6526_cv_info->cv_trim_flag == 1) {
		scharger_inf("cv_ori_val = %d \n",di->cv_val);
		ret = __set_terminal_voltage(di->cv_val);
		if (ret) {
			g_hi6526_cv_info->cv_trim_flag = 0;
		...
		/* need to reset default efuse in v200 and v510 */
		if (di->hi6526_version == CHIP_VERSION_V200) {
			ret = hi6526_efuse_read(EFUSE2, EFUSE_BYTE7, &efuse_ori_val, 0);
			scharger_inf("%s val_ate=0x%x, ret1=%d!\n", __func__, efuse_ori_val, ret);
			ret = hi6526_efuse_write(EFUSE2, EFUSE_BYTE7, efuse_ori_val, 1);
		} else if (di->hi6526_version == CHIP_VERSION_V510) {
			ret = hi6526_efuse_read_2(EFUSE_BYTE15, &efuse_ori_val, 0);
			scharger_inf("%s val_ate=0x%x, ret1=%d!\n", __func__, efuse_ori_val, ret);
			ret = hi6526_efuse_write_2(EFUSE_BYTE15, efuse_ori_val, 1);
		}

		if (ret) {
			g_hi6526_cv_info->cv_trim_flag = 0;
			scharger_err("%s reset efuse fail\n", __func__);
			return;
		}
	}

	g_hi6526_cv_info->cv_trim_flag = 0;
	g_hi6526_cv_info->cv_new_set_val = 0;

	scharger_inf("%s -", __func__);
}

static int _hi6526_cv_set_vterm_by_efuse(int cv_ori, int cv_new_set)
{
	int ret = 0;
	int steps;
	int delta_v = cv_new_set - cv_ori;

	g_hi6526_cv_info->cv_ori_val = cv_ori;
	if (delta_v> CHG_FAST_VCHG_17MV) {
		g_hi6526_cv_info->cv_new_set_val = cv_ori + CHG_FAST_VCHG_17MV;
		ret = __set_terminal_voltage(g_hi6526_cv_info->cv_new_set_val);
		if (ret) {
			g_hi6526_cv_info->cv_trim_flag = 0;
			scharger_err("%s set vterm fail\n", __func__);
			return -1;
		}
		steps = (delta_v - CHG_FAST_VCHG_17MV ) / CV_TRIM_STEP_8MV;
	} else {
		g_hi6526_cv_info->cv_new_set_val = cv_ori;
		steps = delta_v / CV_TRIM_STEP_8MV;
	}

	if (steps > TRIM_A_MAX)
		steps = TRIM_A_MAX;
	scharger_inf("%s steps = %d\n",__func__,steps);

	ret = hi6526_set_pretrim_code(steps);
	if (ret)
		scharger_err("%s ate trim failed \n",__func__);

	g_hi6526_cv_info->cv_trim_flag = 1;
	scharger_inf("%s -", __func__);

	return 0;
}

static int _hi6526_cv_set_vterm_by_reg(int cv_ori, int cv_new_set)
{
	int ret;

	g_hi6526_cv_info->cv_ori_val = cv_ori;
	g_hi6526_cv_info->cv_new_set_val = cv_new_set;
	ret = __set_terminal_voltage(cv_new_set);
	if (ret) {
		g_hi6526_cv_info->cv_trim_flag = 0;
		scharger_err("%s set vterm fail\n", __func__);
		return -1;
...
static int hi6526_cv_set_vterm(int cv_ori, int cv_new_set)
{
	int cv_cur;
	int delta_v = cv_new_set - cv_ori;
	...
	scharger_inf("%s cv_ori = %d  cv_new_set = %d\n",__func__,cv_ori,cv_new_set);
	if (delta_v > CV_TRIM_PROTECT_58MV || delta_v <= 0) {
		g_hi6526_cv_info->cv_trim_flag = 0;
		scharger_err("%s delta_v is error,cv_new_set:%d, cv_ori:%d, stop cv trim\n",
			 __func__, cv_new_set, cv_ori);
		return -1;
	}

	cv_cur = hi6526_cv_get_vterm();
	if (cv_cur != cv_ori) {
		g_hi6526_cv_info->cv_trim_flag = 0;
	...
	if (di->hi6526_version == CHIP_VERSION_V200 || di->hi6526_version == CHIP_VERSION_V510)
		return _hi6526_cv_set_vterm_by_efuse(cv_ori,cv_new_set);
	else
		return _hi6526_cv_set_vterm_by_reg(cv_ori,cv_new_set);

	return 0;
}


static void hi6526_clear_cv_trim_flag(void)
{
	g_hi6526_cv_info->cv_trim_flag = 0;
}

static int hi6526_cv_get_dieid(char *dieid, unsigned int len)
{
	int ret;
	errno_t err;
	...
	ret = hi6526_efuse_get_dieid(dieid, HI6526_DIEID_LENGTH);
	if (ret)
		return -1;
...
struct scharger_cv_ops hi6526_cv_ops = {
	.get_charge_done_state = hi6526_get_charger_state,
	.get_chg_mode = hi6526_get_chg_mode,
	.get_chg_enable = hi6526_get_charge_enable,
	.get_vterm = hi6526_cv_get_vterm,
	.set_vterm = hi6526_cv_set_vterm,
	.clear_cv_trim_flag = hi6526_clear_cv_trim_flag,
	.reset_cv_value = hi6526_reset_cv_value,
	.get_dieid = hi6526_cv_get_dieid,
};
...
struct charge_device_ops hi6526_ops = {
	...
	.set_ship_mode_enable = hi6526_set_ship_mode_enable,
};
...
static void hi6526_set_dp_pin(int high)
{
	struct hi6526_device_info *di = g_hi6526_dev;
	u8 reg_val = 0;
	...
	scharger_inf("%s  set %d\n", __func__, high);
	if (high) {
		(void)hi6526_write_mask(CHG_FCP_CTRL_REG, CHG_FCP_EN_MSK,
					CHG_FCP_EN_SHIFT, 1);
		(void)hi6526_write_mask(CHG_FCP_DET_CTRL_REG, CHG_FCP_DET_EN_MSK | CHG_FCP_MODE_MSK,
					0, CHG_FCP_DET_EN_MSK | CHG_FCP_MODE_MSK);
		(void)hi6526_write_mask(CHG_FCP_MODE2_REG, CHG_FCP_MODE2_SET_MSK, 0, 0x02); /* 0x01: (3.3V,0.6V) */
	} else {
		(void)hi6526_write_mask(CHG_FCP_CTRL_REG, CHG_FCP_EN_MSK,
					CHG_FCP_EN_SHIFT, 0);
		(void)hi6526_write_mask(CHG_FCP_DET_CTRL_REG, CHG_FCP_DET_EN_MSK | CHG_FCP_MODE_MSK,
					0, ~(CHG_FCP_DET_EN_MSK | CHG_FCP_MODE_MSK));
		(void)hi6526_write_mask(CHG_FCP_MODE2_REG, CHG_FCP_MODE2_SET_MSK, 0, 0x00); /* 00：（0.6V,0V) */
	}

	mutex_unlock(&di->fcp_detect_lock);
}

static struct water_det_ops pd_det_ops = {
	.is_water_detected = NULL,
	.set_dp_pin = hi6526_set_dp_pin,
};
#endif
...
#ifdef CONFIG_SCHARGER_SCP_PROTOCOL
...
static struct dc_ic_ops hi6526_lvc_ops = {
	.dev_name = "hi6526",
	.ic_init = hi6526_dc_charge_init,
	.ic_exit = hi6526_dc_charge_exit,
	.ic_enable = hi6526_lvc_enable,
	.ic_discharge = hi6526_dc_discharge,
	.is_ic_close = hi6526_lvc_is_close,
	.get_ic_id = hi6526_get_loadswitch_id,
	.config_ic_watchdog = hi6526_dc_config_watchdog_ms,
	.kick_ic_watchdog = hi6526_dc_kick_watchdog_timer,
};
...
static struct dc_ic_ops hi6526_sc_ops = {
	...
	.ic_init = hi6526_dc_charge_init,
	.ic_exit = hi6526_dc_charge_exit,
	.ic_enable = hi6526_sc_charge_enable,
	.ic_discharge = hi6526_dc_discharge,
	.is_ic_close = hi6526_sc_charge_is_close,
	.get_ic_id = hi6526_get_sc_device_id,
	.config_ic_watchdog = hi6526_dc_config_watchdog_ms,
	.kick_ic_watchdog = hi6526_dc_kick_watchdog_timer,
};

static struct dc_batinfo_ops hi6526_batinfo_ops = {
	.init = hi6526_dc_charge_init,
	.exit = hi6526_dc_charge_init,
	.get_bat_btb_voltage = hi6526_dc_get_vbat_mv,
	.get_bat_package_voltage = hi6526_dc_get_vbat_mv,
	.get_vbus_voltage = hi6526_dc_get_vbus_mv,
	.get_bat_current = hi6526_dc_get_ibat_ma,
	.get_ic_ibus = hi6526_dc_get_ibus_ma,
	.get_ic_temp = hi6526_dc_get_chip_temp,
...
#ifdef CONFIG_DIRECT_CHARGER
		if (hi6526_disable_direct_charge() == 0) {
			hi6526_lvc_ops.dev_data = (void *)g_hi6526_dev;
			hi6526_sc_ops.dev_data = (void *)g_hi6526_dev;
			hi6526_batinfo_ops.dev_data = (void *)g_hi6526_dev;
			...
			ret[3] = dc_batinfo_ops_register(IC_ONE,
				&hi6526_batinfo_ops, SWITCHCAP_SCHARGERV600);
		}
#endif
#ifdef CONFIG_WIRED_CHANNEL_SWITCH
 ...
 *                   REMOVED: cancel work
 */
static void hi6526_plugout_check_process(enum chip_charger_type type)
{
	struct hi6526_device_info *di = g_hi6526_dev;
...
{
	enum chip_charger_type type = (enum chip_charger_type)event;
...
static void hi6526_common_opt_param(void)
{
	int ret =0;
	struct hi6526_device_info *di = get_hi6526_device();
	...
	if ((di->hi6526_version != CHIP_VERSION_V100) && (di->hi6526_version != CHIP_VERSION_V200)){
		ret = hi6526_write(V510_CHG_DCP_CFG_ADDR,V510_DCP_CFG_OK_MSK);
		if (ret) {
			scharger_err("v510 dcp cfg failed %d\n", ret);
		} else {
			scharger_err("v510 dcp cfg succ\n");
		}
	}
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200))
		...
		hi6526_opt_regs_set(v510_common_opt_regs, ARRAY_SIZE(v510_common_opt_regs));
	else
		hi6526_opt_regs_set(v511_common_opt_regs, ARRAY_SIZE(v511_common_opt_regs));
}

static void hi6526_ops_register(struct hi6526_device_info *di)
{
	int ret;

	(void)hisi_tcpc_reg_ops_register(di->client, &hi6526_tcpc_reg_ops);
	(void)hisi_tcpc_irq_gpio_register(di->client, di->gpio_int);
	(void)hisi_tcpc_notify_chip_version(di->hi6526_version);
#ifdef CONFIG_SERIES_BATT
	if (coul_drv_is_smart_battery())
		ret = series_batt_ops_register(&hi6526_ops);
	else
		ret = charge_ops_register(&hi6526_ops, BUCK_IC_TYPE_THIRDPARTY);
#else
	ret = charge_ops_register(&hi6526_ops, BUCK_IC_TYPE_THIRDPARTY);
#endif
	if (ret)
	...
	if (di->hi6526_version != CHIP_VERSION_V100) {
		ret = scharger_cv_ops_register(&hi6526_cv_ops);
		if (ret)
			scharger_err("register hi6526_cv_ops failed!\n");
	}
#endif
...
#ifdef CONFIG_HISI_ADAPTER_DETECT
	ret = water_det_ops_register(&pd_det_ops);
	if (ret)
		scharger_inf("[%s] water_det_ops_register %d\n", __func__, ret);
#endif
}

static int hi6526_regmap_read(void *context, unsigned int addr, unsigned int *val)
{
	unused(context);
	return hi6526_read(addr, (u8 *)val);
}

static int hi6526_regmap_write(void *context, unsigned int addr, unsigned int val)
{
	unused(context);
	return hi6526_write((u16)addr, (u8)val);
}

static const struct regmap_config hi6526_regmap_config = {
	.reg_bits = 16,
	.val_bits = 8,
	.reg_read = hi6526_regmap_read,
	.reg_write = hi6526_regmap_write,
};
			...
			const struct i2c_device_id *id)
{
	...
	g_hi6526_cv_info = devm_kzalloc(&client->dev, sizeof(*g_hi6526_cv_info), GFP_KERNEL);
	if (g_hi6526_cv_info == NULL) {
		scharger_err("[%s] g_hi6526_cv_info is NULL!\n", __func__);
		return -ENOMEM;
	}
	...
	di->regmap = devm_regmap_init(di->dev, NULL, di, &hi6526_regmap_config);
	if (IS_ERR(di->regmap))
	...
	di->hi6526_wake_lock = wakeup_source_register(&client->dev, "upc_vbus_wakelock");
#else
	di->hi6526_wake_lock = wakeup_source_register("upc_vbus_wakelock");
#endif
	...
	if (ret < 0) {
		scharger_err("chip_charger_type_notifier_register failed\n");
	...
	hi6526_common_opt_param();
	hi6526_buck_opt_param(VBUS_VSET_5V);
	hi6526_unmask_all_irq();

	hi6526_ops_register(di);
...
hi6526_create_sysfs_fail:
	...
	ret = chip_charger_type_notifier_unregister(&di->usb_nb);
	if (ret < 0)
		scharger_err("chip_charger_type_notifier_unregister failed\n");
...
static void hi6526_shutdown(struct i2c_client *client)
{
	...
	ret = hi6526_set_vbus_vset(VBUS_VSET_5V);
	if (ret)
		scharger_err("%s hisi_charger_shutdown failed\n", __func__);
}
