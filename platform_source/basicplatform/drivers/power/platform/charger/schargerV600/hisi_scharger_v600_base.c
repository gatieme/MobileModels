 ...
 *
 * Description: HI6526 charger basic common inner api
 *
...
static void scharger_i2c_err_monitor(void)
{
	...
	if (scharger_i2c_err_cnt >= I2C_ERR_MAX_COUNT) {
		...
		power_event_anc_notify(POWER_ANT_CHARGE_FAULT,
			POWER_NE_CHG_FAULT_I2C_ERR, NULL);
#endif
	}
}
...
static u32 vbus_adc_conver(u32 chan, u8 lvc_mode, u32 data)
{
	...
	/* 0.4 4/10 is the coefficient, 1500mV is reference voltage */
...
int hi6526_get_adc_value(u32 chan, u32 *data)
{
	...
	if ((di->param_dts.adc_single_mode_en == 0) && (lvc_mode || sc_mode)) {
		...
		/* 2: the addr increment step is 2 */
		ret = ret + hi6526_read(CHG_ADC_DATA_H_REG + (chan * 2), &adc_data[1]);
	...
	ret = ret + hi6526_adc_loop_enable(CHG_ADC_DIS);
	ret = ret + hi6526_set_adc_channel(chan);
	ret = ret + hi6526_adc_enable(CHG_ADC_EN);

	ret = ret + hi6526_write_mask(CHG_ADC_START_REG, CHG_ADC_START_MSK,
	...
	*data = vbus_adc_conver(chan, lvc_mode, *data);
	...
	ret = ret + hi6526_adc_enable(CHG_ADC_DIS);
 ...
 *                [temp] dquals to [1308.518 or 1437.2779] minus [4.2392 or 3.4070] times [T]
 */
...
int hi6526_get_chip_temp(int *temp)
{
	...
	ret = __hi6526_get_chip_temp(temp);
	if(ret)
		scharger_err("[%s]get chip fail,ret:%d\n", __func__, ret);
	scharger_dbg("[%s]:%d\n", __func__, *temp);
...
int hi6526_get_ibat(int *ibat_ma)
{
	...
	if (di->param_dts.r_coul_uohm == R_UOHM_10K)
		val /= (R_UOHM_10K / R_UOHM_5K);

	if (di->param_dts.r_coul_uohm == R_UOHM_1K)
		val *= (R_UOHM_2K / R_UOHM_1K);
	...
	u32 adc_code)
{
	...
	ibat = (long)-coul_drv_battery_current();
	...
	/* conver adc code to volt mV 2-12 = 4096 */
...
/*
 * For charger 510 read efuse
 * Parameters: offset: offset byte of efuse
 *             value: efuse value
 * return value: 0-success or others-fail
 */
int hi6526_efuse_read_2(u8 offset, u8 *value, u8 pre_cali)
{
	...
	/* 0x62 bit0 write 0 */
	ret += hi6526_write_mask(V510_EFUSE_PDOB_SEL_REG,
					 V510_EFUSE_PDOB_SEL_MASK,
					 V510_EFUSE_PDOB_SEL_SHIFT,
					 V510_EFUSE_PDOB_SEL_CALI_DIS);

	/* 0x64 bit0 write 0 */
	ret += hi6526_write_mask(V510_EFUSE_RD_CTRL,
					 V510_EFUSE_RD_MASK,
					 V510_EFUSE_RD_SHIFT,
					 V510_EFUSE_RD_CTRL_DIS);

	/* 0x64 bit0 write 1 */
	ret += hi6526_write_mask(V510_EFUSE_RD_CTRL,
					 V510_EFUSE_RD_MASK,
					 V510_EFUSE_RD_SHIFT,
					 V510_EFUSE_RD_CTRL_EN);
	mdelay(1);
	/* 0x62 bit0 write 1 */
	if (pre_cali)
		ret += hi6526_write_mask(V510_EFUSE_PDOB_SEL_REG,
					 V510_EFUSE_PDOB_SEL_MASK,
					 V510_EFUSE_PDOB_SEL_SHIFT,
					 V510_EFUSE_PDOB_SEL_CALI_EN);

	/* 0x6D bit7 write 1 */
	ret += hi6526_write_mask(V510_EFUSE_EN_REG, V510_EFUSE_EN_MASK,
			V510_EFUSE_EN_SHIFT, V510_EFUSE_EN);

	/* 0x6D[6:0] wrie offset */
	ret += hi6526_write_mask(V510_EFUSE_SEL_REG, V510_EFUSE_SEL_MASK, V510_EFUSE_SEL_SHIFT, offset);
	mdelay(1);
	ret += hi6526_read(V510_EFUSE_RDATA_REG, value);

	/* 0x6D bit7 write 0 */
	ret += hi6526_write_mask(V510_EFUSE_EN_REG, V510_EFUSE_EN_MASK,
			V510_EFUSE_EN_SHIFT, V510_EFUSE_DIS);
...
int hi6526_efuse_write_2(u8 offset, u8 value, int pre_cali)
{
	...
	/* 0x69[4:0] write offset */
	ret += hi6526_write_mask(V510_EFUSE_PDOB_PRE_ADDR_WE,
				V510_EFUSE_PDOB_PRE_ADDR_MASK,
				V510_EFUSE_PDOB_PRE_ADDR_SHIFT, offset);

	/* 0x6A write pretrim val */
	ret += hi6526_write_mask(V510_EFUSE_PDOB_PRE_WDATA,
				 V510_EFUSE_PDOB_PRE_WDATA_MASK,
				 V510_EFUSE_PDOB_PRE_WDATA_SHIFT, value);

	mdelay(1);
	/* 0x69 bit7 write 1/0 */
	ret += hi6526_write_mask(V510_EFUSE_PDOB_PRE_ADDR_WE,
				V510_EFUSE_PDOB_PRE_WE_MASK,
				V510_EFUSE_PDOB_PRE_WE_SHIFT,
				V510_EFUSE_PDOB_PRE_WE_EN);
	ret += hi6526_write_mask(V510_EFUSE_PDOB_PRE_ADDR_WE,
				V510_EFUSE_PDOB_PRE_WE_MASK,
				V510_EFUSE_PDOB_PRE_WE_SHIFT,
				V510_EFUSE_PDOB_PRE_WE_DIS);
...
void hi6526_reset_n_efuse_dis(int dis)
{
	...
	ret = hi6526_efuse_read(EFUSE3, EFUSE_BYTE0, &efuse_val, 0);
	if (dis)
		efuse_val = (efuse_val & (~CC_RESET_MASK)) | CC_RESET_MASK_DIS;
	else
		efuse_val = (efuse_val & (~CC_RESET_MASK));

	ret = ret + hi6526_efuse_write(EFUSE3, EFUSE_BYTE0, efuse_val, 1);
	ret = ret + hi6526_efuse_read(EFUSE3, EFUSE_BYTE0, &efuse_val, 1);
	if (ret)
		scharger_err("%s reset fail\n", __func__);
...
int hi6526_reset_watchdog_timer(void)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		watchdog_soft_rst_reg = WATCHDOG_SOFT_RST_REG;
	}else {
		watchdog_soft_rst_reg = V510_WATCHDOG_SOFT_RST_REG;
	}
				     ...
				     struct hi6526_device_info *di)
{
	...
	ret = of_property_read_u32(np, "scp_post_exit",
				   (u32 *)&(di->param_dts.scp_post_exit));
	if (ret) {
		scharger_err("get scp_post_exit err,set default value\n");
		di->param_dts.scp_post_exit = 0;
	}

	scharger_inf("%s scp_post_exit=%d\n",
		     __func__, di->param_dts.scp_post_exit);
}
...
static void get_coul_battery_dts(struct hi6526_device_info *di)
{
	...
	coul_np = of_find_compatible_node(NULL, NULL, "hisilicon,coul_core");
	if (!coul_np) {
		scharger_err("%s coul_core unfind\n", __func__);
		return;
	}
	ret = of_property_read_u32(coul_np, "r_coul_mohm", &r_coul_uohm);
	if (ret) {
		ret = of_property_read_u32(coul_np, "r_coul_uohm", &r_coul_uohm);
		if (ret) {
			r_coul_uohm = R_UOHM_DEFAULT;
			scharger_inf("get r_coul_uohm err,use default value 2000 uohm!\n");
		}
	} else {
		r_coul_uohm *= 1000;
	}
	di->param_dts.r_coul_uohm = (int)r_coul_uohm;
	scharger_inf("prase_dts r_coul_uohm=%d\n", di->param_dts.r_coul_uohm);
...
void parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	ret = of_property_read_u32(np, "disable_dc_func",
				   (u32 *)&(di->param_dts.disable_dc_func));
	if (ret) {
		di->param_dts.disable_dc_func = 0;
		scharger_inf("get disable_dc_func err,use default value 0!\n");
	}
	scharger_inf("parse_dts disable_dc_func=%d\n", di->param_dts.disable_dc_func);
	...
	ret = of_property_read_u32(np, "default_vterm",
				   (u32 *)&(di->param_dts.default_vterm));
	if (ret) {
		scharger_err("get default_vterm failed\n");
		di->param_dts.default_vterm = CHG_FAST_VCHG_4400;
	}
	scharger_inf("prase_dts default_vterm=%d\n",
		     di->param_dts.default_vterm);
...
static int hi6526_fcp_cmd_transfer_check(void)
{
	...
	do {
		...
		ret = ret + hi6526_read(CHG_FCP_ISR2_REG, &reg_val2);
		ret = ret + hi6526_read(CHG_FCP_IRQ3_REG, &reg_val3);
		ret = ret + hi6526_read(CHG_FCP_IRQ4_REG, &reg_val4);
		if (ret) {
		...
		if (di->dc_ibus_ucp_happened && (i > FCP_ACK_RETRY_CYCLE / 2))
			break;
...
int hi6526_fcp_adapter_reg_read_block(u8 reg, u8 *val, u8 num, void *dev_data)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret = ret + hi6526_read(CHG_FCP_ISR2_REG, &reg_val2);
		if (reg_val1 != 0)
			ret = ret + hi6526_write(CHG_FCP_ISR1_REG, reg_val1);
		if (reg_val2 != 0)
			ret = ret + hi6526_write(CHG_FCP_ISR2_REG, reg_val2);

		ret = ret + hi6526_write(CHG_FCP_CMD_REG, CHG_FCP_CMD_MBRRD);
		ret = ret + hi6526_write(CHG_FCP_ADDR_REG, reg);
		ret = ret + hi6526_write(CHG_FCP_LEN_REG, data_len);
		ret = ret + hi6526_write_mask(CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
...
int hi6526_fcp_adapter_reg_read(u8 *val, u8 reg)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret = ret + hi6526_write(CHG_FCP_CMD_REG, CHG_FCP_CMD_SBRRD);
		ret = ret + hi6526_write(CHG_FCP_ADDR_REG, reg);
		ret = ret + hi6526_write_mask(CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
					 CHG_FCP_SNDCMD_SHIFT, CHG_FCP_EN);
...
int hi6526_fcp_adapter_reg_write(u8 val, u8 reg)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret = ret + hi6526_write(CHG_FCP_CMD_REG, CHG_FCP_CMD_SBRWR);
		ret = ret + hi6526_write(CHG_FCP_ADDR_REG, reg);
		ret = ret + hi6526_write(CHG_FCP_WDATA_REG, val);
		ret = ret + hi6526_write_mask(CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
					 CHG_FCP_SNDCMD_SHIFT, CHG_FCP_EN);
...
/* 0: success,other: fail */
int hi6526_disable_direct_charge(void)
{
	struct hi6526_device_info *di = get_hi6526_device();
	...
	if (di->param_dts.disable_dc_func != 1) {
		scharger_inf("enable direct charge func\n");
		return 0;
	} else {
		return 1;
	}
...
int hi6526_fcp_master_reset(void *dev_data)
{
	...
	if (hi6526_disable_direct_charge() == 1)
		ret2 += hi6526_config_opt_param(VBUS_VSET_5V);
	if (ret0 || ret1 || ret2)
		return -1;
	return 0;
}
...
/* LEM */
int hi6526_post_exit(void *dev_data)
{
	struct hi6526_device_info *di = get_hi6526_device();
	if (di == NULL) {
		scharger_err("hi6526_device_info is null\n");
...
int hi6526_fcp_stop_charge_config(void *dev_data)
{
	...
	(void)hi6526_fcp_master_reset(dev_data);
...
int hi6526_scp_master_init(void)
{
	...
	ret7 = hi6526_set_otg_enable(CHARGE_OTG_DISABLE);
...
int hi6526_efuse_get_dieid(u8 *dieid, unsigned int len)
{
	...
	if (di->hi6526_version == CHIP_VERSION_V100) {
	...
	} else if(di->hi6526_version == CHIP_VERSION_V200){
		...
		ret += hi6526_efuse_read_2(EFUSE_BYTE31, &dieid[7], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE30, &dieid[6], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE29, &dieid[5], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE28, &dieid[4], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE27, &dieid[3], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE26, &dieid[2], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE25, &dieid[1], 0);
		ret += hi6526_efuse_read_2(EFUSE_BYTE24, &dieid[0], 0);
	}
...
int hi6526_get_dieid_str(char *dieid, unsigned int len)
{
	...
	if (di->hi6526_version == CHIP_VERSION_V100){
		...
		ret = snprintf_s(dieid, len, len - 1,
				 "\r\nHi6526v510:0x%02x%02x%02x%02x%02x%02x%02x%02x\r\n",
				 val[7], val[6], val[5], val[4], val[3],
				 val[2], val[1], val[0]);
	}
	if (ret < 0)
...
int hi6526_chip_init(struct charge_init_data *init_crit)
{
	int i;
	...
	case ADAPTER_5V:
		ret[0] = hi6526_ibat_res_sel(di->param_dts.r_coul_uohm);
	...
	ret[11] = hi6526_set_terminal_voltage(di->param_dts.default_vterm);
	...
	ret[20] = hi6526_set_otg_enable(CHARGE_OTG_DISABLE);
