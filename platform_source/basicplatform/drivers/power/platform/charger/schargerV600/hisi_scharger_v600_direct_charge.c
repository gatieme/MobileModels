 ...
 *
 * Description: HI6526 charger direct charge inner api
 *
...
struct opt_regs g_v510_lvc_opt_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x2d3, 0xff, 0, 0x15, 0, 0),
...
struct opt_regs g_v510_lvc_extra_opt_regs_1moh[] = {
	reg_cfg(0x2b9, 0xff, 0, 0x00, 0, 0),
};
...
struct opt_regs g_v510_sc_opt_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x2d3, 0xff, 0, 0x15, 0, 0),
	reg_cfg(0x2d4, 0xff, 0, 0x95, 0, 0),
	reg_cfg(0x2e1, 0xff, 0, 0x6C, 0, 0),
	...
	reg_cfg(0x2de, 0xff, 0, 0X1F, 0, 0),
	reg_cfg(0x286, 0xff, 0, 0x06, 0, 0),
	reg_cfg(0x2b7, 0xff, 0, 0x70, 0, 0),
	reg_cfg(0x2bd, 0xff, 0, 0x01, 0, 0),
	reg_cfg(0x2be, 0xff, 0, 0x09, 0, 0),
	reg_cfg(0x2bf, 0xff, 0, 0x09, 0, 0),
	reg_cfg(0x2c0, 0xff, 0, 0x09, 0, 0),
	reg_cfg(0x2c1, 0xff, 0, 0x10, 0, 0),
...
struct opt_regs g_v510_sc_opt_regs_after_enabled[] = {
	reg_cfg(0x2b9, 0xff, 0, 0x50, 0, 0),
	reg_cfg(0x2bb, 0xff, 0, 0xF7, 20, 0),
};
...
static int hi6526_record_chip_track(void)
{
	...
	di->dbg_info[index].ts_nsec = dfx_getcurtime();
#else
...
static void hi6526_lvc_opt_regs(void)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		...
		hi6526_opt_regs_set(g_v510_lvc_opt_regs, ARRAY_SIZE(g_v510_lvc_opt_regs));
		hi6526_opt_regs_set(g_v510_lvc_extra_opt_regs_1moh, ARRAY_SIZE(g_v510_lvc_extra_opt_regs_1moh));
	}
}
...
static void hi6526_sc_opt_regs(void)
{
	...
	if ((di->hi6526_version == CHIP_VERSION_V100) || (di->hi6526_version == CHIP_VERSION_V200)){
		...
		hi6526_opt_regs_set(g_v510_sc_opt_regs, ARRAY_SIZE(g_v510_sc_opt_regs));
	}
}
					...
					int enable)
{
	...
	if (enable) {
		if (di->chg_mode == LVC) {
			...
			hi6526_opt_regs_set(g_v510_sc_opt_regs_after_enabled,
				ARRAY_SIZE(g_v510_sc_opt_regs_after_enabled));
			}
		} else {
...
static void hi6526_dc_set_ibat_regulator(int ma)
{
	...
	reg_val = (u8)((ma - DC_REGULATOR_IBAT_MIN) / DC_REGULATOR_IBAT_SETP);
...
static void hi6526_dc_set_vbat_regulator(int mv)
{
	...
	reg_val = (u8)((mv - DC_REGULATOR_VBAT_MIN) / DC_REGULATOR_VBAT_SETP);
...
static void hi6526_dc_set_vout_regulator(int mv)
{
	...
	reg_val = (u8)((mv - DC_REGULATOR_VOUT_MIN) / DC_REGULATOR_VOUT_SETP);
...
static void hi6526_dc_set_ibus_regulator(int ma)
{
	...
	reg_val = (u8)((ma - DC_REGULATOR_IBUS_MIN) / DC_REGULATOR_IBUS_SETP);
