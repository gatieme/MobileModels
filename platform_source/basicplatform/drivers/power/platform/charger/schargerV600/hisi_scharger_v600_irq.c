				 ...
				 char *buf_addr, int *offset, const char *buf)
{
	...
	if (strlen(g_buck_irq_str[index]) <
		((unsigned int)str_len - strlen(buf_addr))) {
...
void hi6526_direct_charge_fault_handle(u32 lvc_sc_irq_state)
{
	...
	if (lvc_sc_irq_state & FAULT_VBUS_OVP)
		fault_type = POWER_NE_DC_FAULT_VBUS_OVP;
	...
	power_event_anc_notify(POWER_ANT_UVDM_FAULT, fault_type, data);
	if (hi6526_lvc_enable(0, di) != 0)
		scharger_err("hi6526_lvc_enable err\n");
	if (hi6526_sc_enable(0) != 0)
...
static void hi6526_chip_overtemp_handle(void)
{
	...
	data = &(di->dc_nty_data);
	hi6526_direct_dmd_report(POWER_NE_DC_FAULT_TDIE_OTP, data);
#endif
}
...
static void hi6526_buck_vbat_ovp_handle(void)
{
	...
	for (i = 0; i < 5; i++) { /* status number is 5 */
		ret = hi6526_read(chg_irq_status0_addr, &irq_st0);
...
static void hi6526_force_clear_all_irq(void)
{
	...
	if (di->hi6526_version != CHIP_VERSION_V100 && di->hi6526_version != CHIP_VERSION_V200) {
		for (i = 0; i < ufcs_irq_reg_num; i++) {
			ret = hi6526_read(CHG_UFCS_IRQ_ADDR + i, &value);
			if (ret)
				scharger_inf("%s i2c read error i %d\n", __func__, i);

			if (value) {
				scharger_inf("%s:ufcs irq flag%d:0x%x\n", __func__, i, value);
				ret = hi6526_write(CHG_UFCS_IRQ_ADDR + i, value);
				if (ret)
					scharger_inf("%s i2c write error i %d\n",
					     	__func__, i);
			}
...
again:
	...
	if (irq_state & CHG_BUCK_IRQ) {
		(void)hi6526_read_block(CHG_BUCK_IRQ_ADDR,
					(u8 *)&buck_irq_state, 3); /* 3 bytes be read */
		scharger_inf(
	...
	if (irq_state & CHG_LVC_SC_IRQ) {
		(void)hi6526_read_block(CHG_LVC_SC_IRQ_ADDR,
					(u8 *)&lvc_sc_irq_state, 3); /* 3 bytes be read */
		scharger_err(
	...
	if (irq_state & CHG_OTHERS_IRQ) {
		(void)hi6526_read_block(CHG_OTHERS_IRQ_ADDR,
					(u8 *)&others_irq_state, 3); /* 3 bytes be read */
		scharger_inf(
	...
	if (irq_state & CHG_FCP_IRQ) {
		ret = ret || hi6526_read_block(CHG_FCP_IRQ_ADDR1,
					       (u8 *)&fcp_irq_state1, 2); /* 2 bytes be read */
		ret = ret || hi6526_read_block(CHG_FCP_IRQ_ADDR2,
					       (u8 *)&fcp_irq_state2, 2); /* 2 bytes be read */
...
int hi6526_irq_init(struct hi6526_device_info *di, struct device_node *np)
{
	...
	scharger_err("%s, gpio_int %d, irq %d\n", __func__, di->gpio_int, di->irq_int);
