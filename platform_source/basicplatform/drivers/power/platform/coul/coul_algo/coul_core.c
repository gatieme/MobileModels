	...
	struct smartstar_coul_device *di, int rbatt_tbl, int fcc_uah)
{
	...
	if (di->delta_rc_charger_enable) {
		if (!coul_could_cc_update_ocv(di))
			goto out;
	}
...
void coul_core_charging_begin(struct smartstar_coul_device *di)
{
	...
	/* record current cycle when no last fcc cycle in NV */
	if (!di->last_fcc_cycle) {
		di->last_fcc_cycle = di->batt_chargecycles / PERCENT;
		di->is_nv_need_save = 1;
	}
	...
	int new_fcc_uah, int delta_fcc_uah, int design_fcc_mah, int fcc_uah)
{
	 ...
	 */
	ret = sprintf_s(buff, (size_t)DSM_BUFF_SIZE_MAX, " [refresh fcc warning] batt %d, old_fcc:%dmAh, "
	...
	if (ret < 0)
		coul_core_err("%s sprintf_s fcc refresh failed\n", __func__);
	else
...
static bool check_nondc_volt_dec_valid(int volt_dec)
{
	return (volt_dec >= NONDC_VOLT_DEC_MIN) && (volt_dec <= NONDC_VOLT_DEC_MAX);
}

static int limited_fcc_debouce(int prev_fcc_mah, int target_fcc_mah, int max_dec_percent)
{
	if (prev_fcc_mah > target_fcc_mah * (PERCENT + max_dec_percent) / PERCENT)
		return prev_fcc_mah - target_fcc_mah * max_dec_percent / PERCENT;

	return target_fcc_mah;
}

static int limit_max_fcc_by_basp(struct smartstar_coul_device *di)
{
	int delta_cv_pc, max_fcc_mah, designed_fcc_mah, fcc_before_adjust;

	if ((di->batt_chargecycles / PERCENT < NO_BASP_POLICY_CYCLE) ||
		!check_nondc_volt_dec_valid(di->nondc_volt_dec)) {
		coul_core_info("%s, batt%d, cycle = %d, delta_cv = %d, not limit fcc by basp\n",
			__func__, di->batt_index, di->batt_chargecycles / PERCENT, di->nondc_volt_dec);
		return 0;
	}
	...
	fcc_before_adjust = coul_calculate_fcc_uah(di) / PERMILLAGE;
	designed_fcc_mah = coul_interpolate_fcc(di, di->batt_temp);
	max_fcc_mah = designed_fcc_mah * delta_cv_pc / PERCENT;

	if (fcc_before_adjust <= max_fcc_mah) {
		coul_core_info("%s batt%d, cur_fcc = %d, max_basp_fcc = %d, not limit\n",
			__func__, di->batt_index, fcc_before_adjust, max_fcc_mah);
		return 0;
	}

	di->fcc_real_mah = limited_fcc_debouce(fcc_before_adjust, max_fcc_mah, FCC_DECRE_MAX_LIMIT);
	readjust_fcc_table(di);
	coul_core_info("%s batt%d, max_fcc = %d, last_fcc = %d, cur_fcc = %d\n",
		__func__, di->batt_index, max_fcc_mah, fcc_before_adjust, di->fcc_real_mah);
	return 1;
}

static bool check_last_fcc_cycle_valid(struct smartstar_coul_device *di)
{
	return (di->last_fcc_cycle > 0) && (di->last_fcc_cycle < di->batt_chargecycles / PERCENT);
}

static void limit_max_fcc_by_sf_tbl(struct smartstar_coul_device *di)
{
	int designed_fcc_mah, sf_fcc_mah, sf_percent, cur_fcc_mah;
	int cur_cycle = di->batt_chargecycles / PERCENT;

	/* invalid fcc cycle or fcc self study within 50 cycles, do not follow fcc_sf_tbl */
	if (!check_last_fcc_cycle_valid(di) || (cur_cycle - di->last_fcc_cycle <= INVALID_FCC_CYCLE)) {
		coul_core_info("%s batt%d last_fcc_cycle = %d, cur_cycle = %d, use study fcc %d\n",
			__func__, di->batt_index, di->last_fcc_cycle, cur_cycle,
			di->batt_fcc / PERMILLAGE);
		return;
	}

	designed_fcc_mah = coul_interpolate_fcc(di, di->batt_temp);
	sf_percent = coul_interpolate_scalingfactor_fcc(di, (int)di->batt_chargecycles / PERCENT);
	sf_fcc_mah = (designed_fcc_mah * sf_percent) / PERCENT;
	cur_fcc_mah = coul_calculate_fcc_uah(di) / PERMILLAGE;
	if (cur_fcc_mah <= sf_fcc_mah) {
		coul_core_info("%s batt%d cur_fcc %d < sf_fcc %d, use prev fcc\n", __func__,
			di->batt_index, cur_fcc_mah, sf_fcc_mah);
		return;
	}

	di->fcc_real_mah = limited_fcc_debouce(cur_fcc_mah, sf_fcc_mah, FCC_DECRE_MAX_LIMIT);
	readjust_fcc_table(di);
	coul_core_info("%s batt%d have not fresh fcc in 50 cycles, last_fcc = %d, adjusted_fcc=%d\n",
		__func__, di->batt_index, cur_fcc_mah, di->fcc_real_mah);
}

static void check_refresh_fcc_valid(struct smartstar_coul_device *di)
{
	/* only decrease fcc once during a full charge */
	if (limit_max_fcc_by_basp(di))
		return;

	limit_max_fcc_by_sf_tbl(di);
}
