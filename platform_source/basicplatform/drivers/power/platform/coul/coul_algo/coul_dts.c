	...
	const struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "delta_rc_charger_enable",
		(unsigned int *)&di->delta_rc_charger_enable);
	if (ret)
		di->delta_rc_charger_enable = 0;
	coul_core_info("delta_rc_charger_enable: flag is %d\n", di->delta_rc_charger_enable);
}
