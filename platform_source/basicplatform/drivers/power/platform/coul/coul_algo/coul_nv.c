...
int coul_get_initial_params(struct smartstar_coul_device *di)
{
	...
	coul_core_info("cycles=%d,limit_fcc=%d\n"
		"report_full_fcc_real =%d,reg_c=%d, reg_v=%d,batt_id=%d\n",
		pinfo->charge_cycles, pinfo->limit_fcc,
		pinfo->report_full_fcc_real, di->boot_info.calc_ocv_reg_c,
		di->boot_info.calc_ocv_reg_v, di->boot_info.hkadc_batt_id_voltage);
...
#define MAX_TMP_BUF_LEN 10
	...
	if(*batt_index >= BATTERY_COUNT) {
		...
		for(int i =0 ;i < MAX_RECORDS_CNT; i++)
