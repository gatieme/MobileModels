...
int coul_core_is_battery_exist(void *data)
{
	...
	coul_core_info("CONFIG_HLTHERM_RUNTEST=y, set batt_exist=0\n");
	di->batt_exist = 0;
	return 0;
#endif
...
int coul_interpolate_scalingfactor_fcc(
...
#ifdef CONFIG_BATT_SOH
static void acr_check(struct smartstar_coul_device *di)
{
...
#ifdef CONFIG_BATT_SOH
static void dcr_check(struct smartstar_coul_device *di)
{
...
static void eis_freq_check(struct smartstar_coul_device *di)
{
	...
	if (di->charged_cnt == 0)
		di->chg_done_eis_freq_time = (int)(dfx_getcurtime() / NSEC_PER_SEC);

	chg_done_now_time = (int)(dfx_getcurtime() / NSEC_PER_SEC);

	/* get eis_freq cal period */
	eis_freq_time_inc = chg_done_now_time - di->chg_done_eis_freq_time;
	...
	if ((eis_freq_time_inc > EIS_FREQ_CYCLE_S) &&
		(di->eis_freq_ntf_cnt <= SAME_CYC_MAX_TRIAL)) {
		di->chg_done_eis_freq_time = (int)(dfx_getcurtime() / NSEC_PER_SEC);
		call_coul_blocking_notifiers(BATT_EIS_FREQ, (void *)&di->batt_index);
		di->eis_freq_ntf_cnt++;
		coul_core_info("eis freq notify success,batt_%d, eis_freq_time_inc = [%d]S, %dth notify\n",
			di->batt_index, eis_freq_time_inc, di->eis_freq_ntf_cnt);
	}
}
#endif
