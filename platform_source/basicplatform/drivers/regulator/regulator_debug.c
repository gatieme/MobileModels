...
struct pc_record_info {
	...
	unsigned char *percpu_addr[CPUS_NUM_MAX];
	unsigned int   percpu_length[CPUS_NUM_MAX];
};
...
static void r_test_put(struct regulator *regulator)
{
	...
	if (regulator->enable_count == 0) {
		...
		list_for_each_entry(node, &regulator_test_list, list) {
			...
			if (strcmp(rdev_node->constraints->name, rdev->constraints->name) == 0) {
				list_del(&node->list);
...
void get_current_state(const char *ldo_name, int length)
{
	struct regulator *r_test = NULL;
	const struct regulator_ops *ops = NULL;
	int state_value;
	...
	if (ops->get_current_limit) {
		state_value = regulator_get_current_limit(r_test);
		pr_info("current_value is %d uA\n", state_value);
	}
	r_test_put(r_test);
	return;
}
...
static void dbg_control_regulator_set_value_handler(const char *tmp, size_t count)
{
	...
	if (tmp[0] == 'R' || tmp[0] == 'r') {
		...
		for (i = 2, j = 0; i < (count - 1); i++, j++)
			ptr[j] = tmp[i];
		ptr[j] = '\0';
		get_current_state(ptr, ARRAY_LINE);
	}
}
...
static int ip_regulator_state_show(struct device *dev, void *data)
{
	...
	if ((strstr(rdev->constraints->name, "ldo") != NULL) ||
		...
		(strstr(rdev->constraints->name, "pmic") != NULL))
		return 0;
...
static void track_regulator_onoff(struct regulator_dev *rdev)
{
...
static void track_regulator_set_vol(struct regulator_dev *rdev,
...
static void track_regulator_set_mode(struct regulator_dev *rdev,
	u8 mode)
{
	if (IS_ERR_OR_NULL(rdev)) {
		pr_err("%s param is null!\n", __func__);
...
static int regulator_percpu_buffer_init(u8 *addr, u32 size, u32 fieldcnt,
	u32 magic_number, enum BUF_TYPE_EN buf_type)
{
	int i, ret;
	errno_t err;
	u32 cpu_num = num_possible_cpus();
	if (cpu_num > CPUS_NUM_MAX) {
		pr_err("[%s] cpu number [%u] error!\n", __func__, cpu_num);
		return -1;
	}
	...
	g_regulator_track_info.buffer_size = (unsigned int)(size - sizeof(struct pc_record_info));
	g_regulator_track_info.dump_magic = magic_number;

	/* set per cpu buffer */
	for (i = 0; i < cpu_num; i++) {
		/* elements for per line, */
		g_regulator_track_info.percpu_length[i] =
			ALIGN8(g_regulator_track_info.buffer_size / cpu_num);
...
static int regulator_buffer_init(u8 *addr, u32 size, enum BUF_TYPE_EN buf_type)
{
	return regulator_percpu_buffer_init(addr, size,
		sizeof(struct regulator_record_info), REGULATOR_MAGIC_NUM, buf_type);
}
...
STATIC int __init track_regulator_record_init(void)
{
	...
	REGISTER_HCK_VH(regulator_on_off, track_regulator_onoff);
	REGISTER_HCK_VH(regulator_set_vol, track_regulator_set_vol);
	REGISTER_HCK_VH(regulator_set_mode, track_regulator_set_mode);
