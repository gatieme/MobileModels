...
#define READ_SIZE		2
#define NORMAL_VERSION		0
#define SPECIAL_VERSION		1
#define ENABLE_MASK_MAX		8
...
struct current_sense_ctrl_reg {
	u32 ctrl_reg;
	u32 enable_mask;
	u32 channel_ctrl_reg;
	u32 channel_ctrl_mask;
};

struct regulator_register_current_sense_info {
	struct current_sense_ctrl_reg current_sense_ctrl;
	u32 hkadc_channel_val;
	u32 current_sense_channel_val;
	u32 current_sense_flag;
	u32 delay_time;
	u32 current_voltage_coefficient;
};

struct regmap_regulator {
	const char *name;
	struct regulator_register_info register_info;
	struct regulator_register_current_sense_info register_current_sense_info;
...
static int regmap_regulator_enable(struct regulator_dev *dev)
{
	...
	/* set enable register */
	if (sreg->register_info.enbable_mask < ENABLE_MASK_MAX) {
		ret0 = regmap_write_bits(regmap, sreg->register_info.ctrl_reg,
					sreg->register_info.enable_mask,
					sreg->register_info.enable_mask);
		ret1 = regmap_write_bits(regmap, sreg->register_info.ctrl_reg,
					sreg->register_info.enbable_mask, 0);
		if(ret0 || ret1)
			ret = -1;
		else
			ret = 0;
...
static int regmap_regulator_disable(struct regulator_dev *dev)
{
	...
	if (sreg->register_info.enbable_mask < ENABLE_MASK_MAX) {
		...
		ret1 = regmap_write_bits(regmap, sreg->register_info.ctrl_reg,
					sreg->register_info.enbable_mask,
					sreg->register_info.enbable_mask);
		if(ret0 || ret1)
			ret = -1;
		else
...
static void regulator_enable_delay(unsigned int delay)
{
...
static int regmap_regulator_get_current(struct regulator_dev *dev)
{
	struct regmap_regulator *sreg = rdev_get_drvdata(dev);
	struct regmap *regmap = dev->regmap;
	int ret;
	...
	if (sreg->register_current_sense_info.current_sense_flag != 1) {
		pr_err("[%s] not support current sense\n",__func__);
		return 0;
	}

	ret = regmap_write_bits(regmap, sreg->register_current_sense_info.current_sense_ctrl.channel_ctrl_reg,
							sreg->register_current_sense_info.current_sense_ctrl.channel_ctrl_mask,
							sreg->register_current_sense_info.current_sense_channel_val);
	if (ret < 0) {
		pr_err("%s regmap write_bits ctrl reg 0x%x failed\n", __func__,
			sreg->register_current_sense_info.current_sense_ctrl.channel_ctrl_reg);
		return ret;
	}

	brand_debug("<[%s]: current sense channel[%d] is selected>\n",__func__,
			sreg->register_current_sense_info.current_sense_channel_val);

	ret = regmap_write_bits(regmap, sreg->register_current_sense_info.current_sense_ctrl.ctrl_reg,
							sreg->register_current_sense_info.current_sense_ctrl.enable_mask,
							sreg->register_current_sense_info.current_sense_ctrl.enable_mask);
	if (ret < 0) {
		pr_err("%s regmap write_bits ctrl reg 0x%x failed\n", __func__,
			sreg->register_current_sense_info.current_sense_ctrl.ctrl_reg);
		return ret;
	}

	regulator_enable_delay(sreg->register_current_sense_info.delay_time);

	voltage = lpm_adc_get_value(sreg->register_current_sense_info.hkadc_channel_val);
	if (voltage < 0) {
		pr_err("%s hkadc channel[%d] failed\n", __func__,
			sreg->register_current_sense_info.hkadc_channel_val);
		return -1;
	}
	brand_debug("hkadc channel[%d] value is %d\n", sreg->register_current_sense_info.hkadc_channel_val, voltage);

	ret = regmap_write_bits(regmap, sreg->register_current_sense_info.current_sense_ctrl.ctrl_reg,
							sreg->register_current_sense_info.current_sense_ctrl.enable_mask, 0);
	if (ret < 0) {
		pr_err("%s regmap write_bits ctrl reg 0x%x failed\n", __func__,
			sreg->register_current_sense_info.current_sense_ctrl.ctrl_reg);
		return ret;
	...
	brand_debug("<[%s]: disable current sense is ok>\n",__func__);

	current_val = voltage*100*1000;

	do_div(current_val,sreg->register_current_sense_info.current_voltage_coefficient);

	brand_debug("<[%s]: current sense value is %d uA>\n",__func__,current_val);
	...
	struct device_node *np)
{
	...
	ret = regmap_dt_select_version(pdev, np);
	if (ret == SPECIAL_VERSION) {
		/* sub pmu is pmic6422v300 */
		if (!of_property_read_u32_array(np, "pmic-ctrl-1",
			register_info, 2)) {
			sreg->register_info.ctrl_reg = register_info[0];
			sreg->register_info.enable_mask = register_info[1];
		}
		if (!of_property_read_u32_array(np, "pmic-vset-1",
			register_info, 2)) {
			sreg->register_info.vset_reg = register_info[0];
			sreg->register_info.vset_mask = register_info[1];
		}
		/*
		 * master pmu is pmic6421v900 ldo8 & ldo25
		 * parse .n_voltages, and .volt_table
		 */
		if (!of_property_read_u32(np, "pmic-n-voltages-1", &sreg->rdesc.n_voltages)) {
		/* alloc space for .volt_table */
			v_table = devm_kzalloc(
				dev, sizeof(unsigned int) * sreg->rdesc.n_voltages, GFP_KERNEL);
			if (!v_table) {
				ret = -ENOMEM;
				dev_err(dev, "no memory for .volt_table\n");
				return ret;
			}
			if (!of_property_read_u32_array(np, "pmic-vset-table-1",
				v_table, sreg->rdesc.n_voltages))
			sreg->rdesc.volt_table = v_table;
		} /* end */
	}
	return 0;
}
					...
					struct platform_device *pdev)
{
	...
	sreg->register_info.enbable_mask = ENABLE_MASK_MAX;

	if (of_property_read_u32_array(np, "pmic-ctrl-enb",
						register_info, READ_SIZE) == 0)
		sreg->register_info.enbable_mask = register_info[1];
...
static int regmap_dt_parse_current_sense_info(struct regmap_regulator *sreg,
					struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	int ret;
	u32 val = 0;
	u32 current_sense_info[3] = {0};

	sreg->register_current_sense_info.current_sense_flag = 0;
	/* parse .register_current_sense_info.current_sense_flag */
	ret = of_property_read_u32(np, "pmic-current-sense-support",
				   &val);

	if (val != 1) {
		dev_err(dev, "not support current sense\n");
		return 0;
	}
	sreg->register_current_sense_info.current_sense_flag = val;

	/* if np no "pmic-current-sense-adc-channel-ctrl" node, just return */
	ret = of_property_read_u32(np, "pmic-current-sense-adc-channel-ctrl",
						&val);
	if (ret != 0) {
		dev_err(dev, "no pmic-current-sense-adc-channel-ctrl property set\n");
		return ret;
	}
	sreg->register_current_sense_info.hkadc_channel_val = val;

	/* if np no "pmic-current-sense-channel-ctrl" node */
	ret = of_property_read_u32_array(np, "pmic-current-sense-channel-ctrl",
						current_sense_info, 3);
	if (ret != 0) {
		dev_err(dev, "no pmic-current-sense-channel-ctrl property set\n");
		return ret;
	}
	sreg->register_current_sense_info.current_sense_ctrl.channel_ctrl_reg = current_sense_info[0];
	sreg->register_current_sense_info.current_sense_ctrl.channel_ctrl_mask = current_sense_info[1];
	sreg->register_current_sense_info.current_sense_channel_val = current_sense_info[2];

	/* if np no "pmic-current-sense-ctrl" node */
	ret = of_property_read_u32_array(np, "pmic-current-sense-ctrl",
						current_sense_info, 2);
	if (ret != 0) {
		dev_err(dev, "no pmic-current-sense-ctrl property set\n");
		return ret;
	}
	sreg->register_current_sense_info.current_sense_ctrl.ctrl_reg = current_sense_info[0];
	sreg->register_current_sense_info.current_sense_ctrl.enable_mask = current_sense_info[1];

	/* parse .register_current_sense_info.delay_time */
	ret = of_property_read_u32(np, "pmic-current-sense-delay-time",
				   &val);
	if (ret != 0) {
		dev_err(dev, "no pmic-current-sense-delay-time property set\n");
		return ret;
	}
	sreg->register_current_sense_info.delay_time = val;

    /* parse .register_current_sense_info.current_voltage_coefficient */
	ret = of_property_read_u32(np, "pmic-current-voltage-coefficient",
				   &val);
	if (ret != 0) {
		dev_err(dev, "no pmic-current-voltage-coefficient set\n");
		return ret;
	}
	sreg->register_current_sense_info.current_voltage_coefficient = val;
				...
				struct platform_device *pdev)
{
	...
	/* parse regmap's dt current_sense_info part */
	ret = regmap_dt_parse_current_sense_info(sreg, pdev);
	if (ret) {
		dev_err(dev, "failure in regmap_dt_parse_current_sense_info\n");
		devm_kfree(dev, v_table);
		return ret;
	}
...
static struct regulator_ops pmic_ldo_rops = {
	...
	.get_current_limit = regmap_regulator_get_current,
...
static void _of_get_regulator_voltage_threshold(struct device_node *np,
	struct regulation_constraints *constraint)
{
	u32 pval;

	if (!of_property_read_u32(np, "regulator-min-microvolt-1", &pval))
		constraint->min_uV = pval;
	if (!of_property_read_u32(np, "regulator-max-microvolt-1", &pval))
		constraint->max_uV = pval;

	/* Voltage change possible? */
	if (constraint->min_uV != constraint->max_uV)
		constraint->valid_ops_mask |= REGULATOR_CHANGE_VOLTAGE;

	/* Do we have a voltage range, if so try to apply it? */
	if (constraint->min_uV && constraint->max_uV)
		constraint->apply_uV = true;
}

static void of_get_regulator_voltage_threshold(struct platform_device *pdev,
	struct regulator_init_data *initdata)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct regulation_constraints *constraint = &initdata->constraints;
	int ret;

	ret = regmap_dt_select_version(pdev, np);
	if (ret == SPECIAL_VERSION)
		_of_get_regulator_voltage_threshold(np, constraint);
}
								...
								struct platform_device *pdev)
{
	...
	of_get_regulator_voltage_threshold(pdev, initdata);
