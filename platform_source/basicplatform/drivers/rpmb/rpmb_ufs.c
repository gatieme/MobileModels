...
struct rpmb_time_record rpmb_time_point[RPMB_FROM_MAX];

static void ufs_rpmb_time_stamp_dump(void)
{
	int i;

	pr_err("hisee_counter_lock [%llu]\n", g_hisee_counter_lock);
	for (i = 0; i < RPMB_FROM_MAX; i++)
		pr_err("source[%d]: state[%d], lock[%llu], release[%llu]\n", i,
		       rpmb_time_point[i].state,
		       rpmb_time_point[i].cmd_lock_done,
		       rpmb_time_point[i].cmd_lock_release);
...
static int ufs_bsg_ioctl_rpmb_cmd(enum func_id id,
				  ...
				  bool is_write, enum rpmb_req_source source)
{
	int32_t fd = -1;
	unsigned int i;
...
#ifndef CONFIG_RPMB_REQ_LOCK_DISABLE
	mutex_lock(&rpmb_ufs_cmd_lock);
	rpmb_time_point[source].cmd_lock_done = dfx_getcurtime();
#endif
	/*lint -e501*/
	oldfs = get_fs();
...
out:
	...
	rpmb_time_point[source].cmd_lock_release = dfx_getcurtime();
#endif
	return ret;
}
...
static void ufs_rpmb_combine_cmd(struct combine_cmd *cmd, uint8_t region)
{
					    ...
					    uint8_t region)
{
	...
	memcpy(cmd->transfer_frame, cmd->frame, data->buf_bytes); /* [false alarm]: enough reserved memory, 32KB */
...
static int ufs_rpmb_work(struct rpmb_request *request)
{
	...
	if (result)
		return result;
	rpmb_time_point[RPMB_FROM_MSPC].resource_alloc = dfx_getcurtime();
	rpmb_time_point[RPMB_FROM_MSPC].state = (int)(request->info.state);
	...
	case RPMB_STATE_WR_CNT:
		...
		g_hisee_counter_lock = dfx_getcurtime();
#endif
		ufs_rpmb_get_counter(request, rpmb_data, sense_buffer,
				     transfer_frame);
		break;
				  ...
				  struct rpmb_request *req)
{
	...
	ret = ufs_bsg_ioctl_rpmb_cmd(RPMB_FUNC_ID_SE, rpmb_data, false, RPMB_FROM_AUTH_KEY);
...
static inline void ufs_read_frame_recombine(
...
static void ufs_rpmb_ioctl_combine_request(enum rpmb_op_type operation,
			       struct ufs_blk_ioc_rpmb_data *rpmb_data,
			       struct storage_blk_ioc_rpmb_data *storage_data,
			       uint8_t *sense_buffer[])
{
			    ...
			    struct storage_blk_ioc_rpmb_data *storage_data)
{
	...
	rpmb_time_point[RPMB_FROM_SECURE_OS].resource_alloc = dfx_getcurtime();
	rpmb_time_point[RPMB_FROM_SECURE_OS].state = (int)operation;
	ret = ufs_bsg_ioctl_rpmb_cmd(id, rpmb_data,
				     (operation == RPMB_OP_WR_DATA),
				     RPMB_FROM_SECURE_OS);
...
static int __init rpmb_ufs_register(void)
{
	return rpmb_ufs_init(0);
}
device_initcall(rpmb_ufs_register);
#endif
