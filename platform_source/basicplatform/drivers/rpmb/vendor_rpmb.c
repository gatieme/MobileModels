...
#include <platform_include/basicplatform/linux/rpmb/rpmb_ffa.h>
#endif
...
{
#ifdef CONFIG_RPMB_FFA_SUPPORT
	return rpmb_ffa_send(_function_id, _arg0, _arg1, _arg2);
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
			  ...
			  struct rpmb_operation *rpmb_ops)
{
	...
	if (temp_cost_time > RPMB_TIMEOUT_TIME_IN_KERNEL || debug_enable) {
		pr_err("[%s]rpmb access cost time is more than 800ms, state[%d]\n",
		       __func__, (int32_t)state);
		pr_err("mspc start[%llu], workqueue start[%llu], ufs start[%llu], end[%llu]\n",
		       g_mspc_start_time, g_work_queue_start,
		       g_rpmb_ufs_start_time, g_mspc_end_time);
		pr_err("tee start[%llu], end[%llu]\n", g_os_start_time,
		       g_os_end_time);
...
#if defined(CONFIG_PRODUCT_ARMPC) || defined(CONFIG_PRODUCT_CDC_ACE)
...
static void rpmb_os_time_stamp(enum rpmb_op_type operation,
			       struct rpmb_operation *rpmb_ops)
{
	u64 temp_cost_time;
	...
	if (temp_cost_time > RPMB_TIMEOUT_TIME_IN_KERNEL) {
		pr_err("[%s]rpmb access cost time is more than 800ms, operation[%d]\n",
...
out:
	...
	rpmb_os_time_stamp(operation, rpmb_ops);
	return ret;
}
...
#ifdef CONFIG_SCSI_UFS_VIRTUALIZ_AGENT
static int rpmb_virtualiz_agent_recv_process(char *buf, unsigned int buf_size)
{
	int i;
	struct rpmb_config_info *rpmb_config = NULL;

	if (!buf || (buf_size < sizeof(struct rpmb_config_info))) {
...
struct ufs_agent_ops rpmb_virtualiz_agent_ops = {
	{ SHMEC_UFS_RPMB },
	0,
	rpmb_virtualiz_agent_recv_process,
	NULL,
	NULL,
};

static int rpmb_virtualiz_agent_init(void)
{
	struct device_node *np = NULL;
	int ret;
	char buf[MAX_SHMEC_BUF_SIZE];

	np = of_find_compatible_node(NULL, NULL, "vendor,rpmb_virtualiz");
	if (!np)
		return 0;

	rpmb_virtualiz_agent_ops.irq = irq_of_parse_and_map(np, 0);
	ret = ufs_virtualiz_agent_register(&rpmb_virtualiz_agent_ops, SHMEC_MODULE_UFS_RPMB);
	if (ret)
		pr_err("%s register agent fail\n", __func__);

	/* send event to host, notify host to send rpmb info */
	ret = ufs_virtualiz_agent_send_buf(buf, MAX_SHMEC_BUF_SIZE, SHMEC_MODULE_UFS_RPMB);
	if (ret)
...
#ifdef CONFIG_RPMB_SHARE_MEM
	if (of_property_read_u64(np, "base", (u64*)&bl31_smem_base)) {
		pr_err("rpmb get share_mem base address failed\n");
		goto err_node;
	}
#else
