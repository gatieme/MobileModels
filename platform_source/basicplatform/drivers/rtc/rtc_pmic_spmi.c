...
#ifdef CONFIG_RTC_V500
#define SOC_RTC_DR 0x20   /* Data read register */
#define SOC_RTC_MR 0x08   /* Match register */
#define SOC_RTC_LR 0x04   /* Data load register */
#define SOC_RTC_CR 0x00   /* Control register */
#define SOC_RTC_IMSC 0x14 /* Interrupt mask and set register  1:mask 0:enable */
#define SOC_RTC_RIS 0x18  /* Raw interrupt status register */
#define SOC_RTC_MIS 0x1c  /* Masked interrupt status register */
#define SOC_RTC_ICR 0x10  /* Interrupt clear register */
#else
...
EXPORT_SYMBOL_GPL(pmu_rtc_readcount);
#endif

static void show_pmu_rtc_reg_time(struct vendor_rtc_dev *ldata, unsigned int reg_offset)
{
	struct rtc_time tm;
	unsigned long time;

	time = pmu_rtc_read_bulk(ldata->pmu_rtc_baseaddr + reg_offset);
	rtc_time64_to_tm(time, &tm);
	SHOW_TIME(&tm);
}
...
void pmu_rtc_setalarmtime(unsigned long time)
{
	...
	show_pmu_rtc_reg_time(ldata, PMU_RTC_MR);
#endif
}
...
static int soc_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	rtc_time64_to_tm(readl(ldata->soc_rtc_baseaddr + SOC_RTC_DR), tm);
...
static void rtc_shutdown(struct spmi_device *pdev)
{
	...
	if (poweroff_rtc_alarm.enabled) {
		time = rtc_tm_to_time64(&poweroff_rtc_alarm.time);
		pmu_rtc_setalarmtime(time);
	} else {
		pmu_rtc_write_bulk(
...
static int request_rtc_irq(struct device *dev, struct vendor_rtc_dev *prtc)
{
	...
	if (devm_request_irq(dev, prtc->soc_rtc_irq, soc_rtc_interrupt,
		IRQF_NO_SUSPEND, "vendor-soc-rtc", prtc)) {
...
static void pmu_rtc_show_alarmtime(void)
{
	struct vendor_rtc_dev *ldata = NULL;
	...
	pr_info("PMU RTC Realtime:");
	show_pmu_rtc_reg_time(ldata, PMU_RTC_DR);

	pr_info("PMU RTC Alarmtime:");
	show_pmu_rtc_reg_time(ldata, PMU_RTC_MR);
}
...
static int rtc_probe(struct spmi_device *pdev)
{
	...
	prtc->rtc_dev = devm_rtc_device_register(
		&pdev->dev, "vendor-rtc", &rtc_ops, THIS_MODULE);
...
unsigned long rtc_test(int cmd, unsigned long wtime)
{
	struct vendor_rtc_dev *data = rtc_ldata_get();
	unsigned long rtime;
	unsigned int reg_val;
