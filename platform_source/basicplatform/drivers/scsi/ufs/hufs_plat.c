...
#else
#ifndef CONFIG_LIBLINUX_UFS_ACE_HOST
...
enum ufs_sk_state gufs_sk_state = UFSHCD_SK_STATE_INVAL;
struct delayed_work fbx_gensk_worker;
#endif
#endif
...
#endif
		for (i = 0; i < strlen(ufs_product_name); i++)
			ufs_product_name[i] += 1;
	} else {
...
static void hufs_hci_host_reg_log(struct ufs_hba *hba)
{
	...
	dev_err(hba->dev, ": REG_UFS_GCR_CFG:   0x%08x\n",
		ufshcd_readl(hba, REG_UFS_GCR_CFG));
	dev_err(hba->dev, ": REG_UFS_MCQ_CFG:   0x%08x\n",
		ufshcd_readl(hba, REG_UFS_MCQ_CFG));
#endif
...
#ifndef CONFIG_UFS_4_0_HC
	/* modify sec read aquila */
...
#ifdef CONFIG_LIBLINUX_UFS_ACE_HOST
int hufs_uie_config_init(struct ufs_hba *hba)
{
	...
	dev_err(hba->dev, "%s: UFS inline crypto ldk.\n", __func__);

#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	/* Send a notification through the event channel.
	 * to enable the guest to derive the key. */
	if (ufshcd_eh_in_progress(hba)) {
		err = ufs_virtualiz_service_send_buf(buf, MAX_SHMEC_BUF_SIZE, SHMEC_MODULE_UFS_CRYPTO);
		/* when guest not start, host occur err_handle, here Do not handle the error
		 * of the channel not exist. */
		if (err == (-ENOENT))
			err = 0;
		else if (err)
			pr_err("%s notify agent derive key fail.\n", __func__);
	}
#endif
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_KM
static void ufshcd_fbex_gen_sk(struct work_struct *work)
{
	int err = 0;
	int stat_rpmb = 0;

	do {
		stat_rpmb = get_rpmb_init_status();
		/* stat_rpmb=1 echo RPMB_DRIVER_IS_READY */
		if (stat_rpmb == RPMB_STATUS_IS_READY)
			break;
		mdelay(50);
	}while(stat_rpmb != RPMB_STATUS_IS_READY);

	err = fbex_gen_sk(FILE_ENCRY_GEN_SK_TYPE);
	if (err)
		pr_err("%s: fbex_gen_sk return failed, err=%d !\n", __func__, err);
	gufs_sk_state = UFSHCD_SK_STATE_AVABLE;
}

int ufshcd_get_sk_state()
{
	unsigned long timeout = (1 * 10 * 1000);
	unsigned int mspc_flag = 0;
	int err = 0;

	err = fbe_ca_msp_available(&mspc_flag);
	if (err) {
		pr_err("%s: mspc_flag get failed,err = %d.\n", __func__, err);
		BUG();
	}
	...
	while (gufs_sk_state != UFSHCD_SK_STATE_AVABLE) {
		if (!timeout) {
			pr_err("%s: wait ufssk_stat avable timeout !\n", __func__);
			return -ETIMEDOUT;
		}
		timeout--;
		mdelay(1);
...
static void hufs_read_scbakdata(void __iomem * sctrl_addr)
{
	unsigned int reg_value;
	unsigned int timeout = 100;

	do {
		reg_value = readl(SCBAKDATA_SOC_SCTRL_SCBAKDATA1_MSK_ADDR(sctrl_addr));
		/* mspc is ready,then restore sk */
		if (reg_value & 0x1)
			return;
		mdelay(2);
		if ((timeout--) == 0) {
			timeout = 100;
			pr_err("%s: wait mspc ready timeout !\n", __func__);
		}
	}while(!(reg_value & 0x1));
}

static int hufs_ursl_restorekey(struct ufs_hba *hba)
{
	int err = 0;
	struct hufs_host *host = hba->priv;

	hufs_read_scbakdata(host->sysctrl);

	err = fbex_gen_sk(FILE_ENCRY_RESTORE_SK_TYPE);
	if(err) {
		pr_err("%s: fbex_gen_sk retrun err=%d !\n", __func__, err);
		return err;
	}
	err = fbex_restore_key();
	if(err) {
		pr_err("%s: fbex_restore_key return err=%d !\n", __func__, err);
		return err;
	}
	return err;
}
...
int hufs_genkey_and_restorekey(struct ufs_hba *hba)
{
	int err = 0;
	unsigned int mspc_flag = 0;
	...
	err = fbe_ca_msp_available(&mspc_flag);
	if (err) {
		dev_err(hba->dev, "%s: mspc_flag get failed,err = %d.\n", __func__, err);
		BUG();
	}

	if (ufshcd_eh_in_progress(hba)) { /* ufs reset */
		if (mspc_flag == FBE_CA_MSP_AVAILABLE) {
			err = fbex_gen_sk(FILE_ENCRY_RESTORE_SK_TYPE);
			if (err) {
				dev_err(hba->dev, "%s: km sk restore return err = %d\n", __func__, err);
				BUG();
			}
		}
		err = fbex_restore_key();
		...
		if(mspc_flag == FBE_CA_MSP_AVAILABLE) { /* mspc is avable */
			if (hba->ulsr) { /* ursl */
				err = hufs_ursl_restorekey(hba);
				if (err)
					pr_err("%s: hufs_ursl_restorekey end, err=%d !\n", __func__, err);
				gufs_sk_state = UFSHCD_SK_STATE_AVABLE;
			} else {
				INIT_DELAYED_WORK(&fbx_gensk_worker, ufshcd_fbex_gen_sk);
				schedule_delayed_work(&fbx_gensk_worker, 0);
			}
		} else { /* mspc is not avable */
			if (hba->ulsr) { /* ursl */
				err = fbex_restore_key();
				if (err)
					pr_err("%s: fbex_restore_key end, err=%d !\n", __func__, err);
				gufs_sk_state = UFSHCD_SK_STATE_AVABLE;
			}
		}
	}
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V2
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	err = hufs_genkey_and_restorekey(hba);
	if (err) {
		dev_err(hba->dev, "%s: UFS gensk_and_restore failed.\n", __func__);
		BUG();
	}
#else
...
void hufs_get_meta_data_factor(struct ufshcd_lrb *lrbp,
...
void hufs_uie_utrd_prepare(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if (is_dfa_enabled(hba))
		return dfa_uie_utrd_prepare(hba, lrbp);
#endif
	...
	struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if (status == PRE_CHANGE) {
		...
		hufs_pwr_change_post_change(hba);
	}
	return ret;
}
	...
	struct device_node *np, struct hufs_host *host)
{
	...
	struct st_caps_map caps_arry[] = {
		...
		{ "ufs-use-hufs-mphy-5", USE_HUFS_MPHY_5 },
		...
		{ "hufs-smsv6-txrt-adapt", USE_SMSV6_TX_RT_ADAPT },
		{ "hufs-consistent-access", HUFS_CONSISTENT_ACCESS },
	};
...
static void hufs_samsungv6_dt_parse(
	struct device_node *parent_np, struct hufs_host *host)
{
	struct device_node *child_np = NULL;
	struct samsung_pnm *samsung_v6 = NULL;
	char *pnm = NULL;
	int ret;

	INIT_LIST_HEAD(&host->hba->samsung_v6_whitelist);
	for_each_child_of_node (parent_np, child_np) {
		ret = of_property_read_string(
			child_np, "smsv6",
			(const char **)(&pnm));
		...
		samsung_v6 = devm_kzalloc(host->hba->dev, sizeof(*samsung_v6),
				GFP_KERNEL);
		if (!samsung_v6) {
			dev_err(host->hba->dev,
				"%s Failed to alloc memory!\n", __func__);
		...
		samsung_v6->ufs_pnm = pnm;
		INIT_LIST_HEAD(&samsung_v6->p);

		list_add(&samsung_v6->p, &host->hba->samsung_v6_whitelist);
	}
}
	...
	struct hufs_host *host, struct device_node *np)
{
	...
	hufs_samsungv6_dt_parse(np, host);
#ifdef CONFIG_HUFS_MANUAL_BKOPS
	...
	if (of_find_property(np, "hufs_standard_uic", NULL)) {
		host->hufs_standard_uic = true;
		pr_err("hufs use standard unipro command\n");
	} else {
		host->hufs_standard_uic = false;
		pr_err("hufs ufs self uic config\n");
	}

	if (of_find_property(np, "ufs-use-host1", NULL))
		host->host_id = 1;

	pr_err("hufs use hostid:%d\n", host->host_id);
}
				   ...
				   struct ufs_pa_layer_attr *pwr_mode)
{
	...
	if ((pwr_mode->pwr_rx == FASTAUTO_MODE ||
		...
		ufshcd_dme_set(hba, UIC_ARG_MIB(PA_ADAPTAFTERLRSTINPA_INIT),
			       PA_HS_APT_NO);
		if (hba->manufacturer_id == UFS_VENDOR_SKHYNIX)
...
const struct ufs_hba_variant_ops ufs_hba_hufs_vops = {
	...
	.set_buck_voltage = hufs_set_buck_voltage,
	.mphy_config_update = hufs_mphy_config_update,
};
EXPORT_SYMBOL(ufs_hba_hufs_vops);

#ifdef CONFIG_SCSI_UFS_VIRTUALIZ_AGENT
static int ufs_crypto_agent_restore_key(int is_ursl)
{
	int err = 0;

	pr_err("%s fbe restore key start, is_ursl is %d\n", __func__, is_ursl);
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	err = fbex_enable_kdf();
	if (err) {
		pr_err("%s fbe enable kdf err 0x%x\n", __func__, err);
		return err;
	}
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_KM
	unsigned int mspc_flag = 0;
	...
	err = fbe_ca_msp_available(&mspc_flag);
	if (err) {
		pr_err("%s mspc_flag get err 0x%x\n", __func__, err);
		BUG();
	}
	if (mspc_flag == FBE_CA_MSP_AVAILABLE) {
		if (is_ursl == true) {
			np = of_find_compatible_node(NULL, NULL, "hisilicon,sysctrl");
			if (!np) {
				pr_err("%s can't find device node hisilicon,sysctrl\n", __func__);
				return -ENXIO;
			}
			sctrl_base = of_iomap(np, 0);
			hufs_read_scbakdata(sctrl_base);
			iounmap(sctrl_base);
		}
		err = fbex_gen_sk(FILE_ENCRY_RESTORE_SK_TYPE);
		if (err) {
			pr_err("%s fbe restore key err 0x%x\n", __func__, err);
			BUG();
		}
	}
	err = fbex_restore_key();
	...
	pr_err("%s fbe restore key ok\n", __func__);
	return err;
}

struct delayed_work ufs_virtualiz_recv_worker;
static void ufs_virtualiz_crypto_agent_restore_key(struct work_struct *work)
{
	ufs_crypto_agent_restore_key(false);
}

static int ufs_crypto_agent_recv_process(char *buf, unsigned int buf_size)
{ /* ufs reset */
	UNUSED(buf);
	UNUSED(buf_size);

	INIT_DELAYED_WORK(&ufs_virtualiz_recv_worker, ufs_virtualiz_crypto_agent_restore_key);
	schedule_delayed_work(&ufs_virtualiz_recv_worker, 0);

	return 0;
}

static int ufs_crypto_agent_resume(void)
{ /* ursl */
	int err;

	err = ufs_crypto_agent_restore_key(true);
	if (err)
		pr_err("%s resume fbe restore key err 0x%x\n", __func__, err);

	return err;
}

struct ufs_agent_ops ufs_crypto_agent_ops = {
	{ SHMEC_UFS_CRYPTO },
	0,
	ufs_crypto_agent_recv_process,
	NULL,
	ufs_crypto_agent_resume,
};

static int ufs_crypto_agent_init(void)
{
	struct device_node *np = NULL;
	int ret;

	np = of_find_compatible_node(NULL, NULL, "vendor,ufs_crypto_virtualiz");
	if (!np)
		return 0;

	ufs_crypto_agent_ops.irq = irq_of_parse_and_map(np, 0);
	ret = ufs_virtualiz_agent_register(&ufs_crypto_agent_ops, SHMEC_MODULE_UFS_CRYPTO);
	if (ret) {
	...
	pr_err("%s UFS inline crypto V3.0.\n", __func__);

#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_KM
	unsigned int mspc_flag = 0;
	ret = fbe_ca_msp_available(&mspc_flag);
	if (ret) {
		pr_err("%s: mspc_flag get err = %d.\n", __func__, ret);
		BUG();
	}
...
#ifdef CONFIG_SCSI_UFS_VIRTUALIZ_SERVICE
struct ufs_service_ops ufs_crypto_service_ops = {
	{ SHMEC_UFS_CRYPTO },
	NULL,
};

static int ufs_crypto_service_init(void)
{
	int ret;

	ret = ufs_virtualiz_service_register(&ufs_crypto_service_ops, SHMEC_MODULE_UFS_CRYPTO);
	if (ret) {
...
#ifdef CONFIG_SCSI_UFS_VIRTUALIZ_AGENT
	err = ufs_crypto_agent_init();
	if (err)
		pr_err("%s ufs crypto agent init fail.\n", __func__);
#endif

#ifdef CONFIG_SCSI_UFS_VIRTUALIZ_SERVICE
	err = ufs_crypto_service_init();
	if (err)
		pr_err("%s ufs crypto service init fail.\n", __func__);
#endif
