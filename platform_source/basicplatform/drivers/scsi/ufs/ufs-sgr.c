...
#define UNIPRO_CLK_AUTO_GATE_CONFIG 0x03001C0E
#define UFSHC_CLOCK_GATING_CONFIG 0xF

#define UFS_PSW_MEM_DS_MASK (1 << 26)
#define UFS_PSW_MEM_SD_MASK (1 << 27)
#define UFS_PSW_MEM_MASK (UFS_PSW_MEM_SD_MASK | UFS_PSW_MEM_DS_MASK)

#define MPHY_TOP_ISO_EN (1 << 9)
#define MPHY_ISO_EN (1 << 10)
#define MPHY_SR_FIX_SIZE 4
#define TACTIVE_COMPEN_300US 300
#define TACTIVE_REG_MAX  0x6

#define UFS_MPHYMCU_SC_REG_UCE_M3_MEMCTRL_OFFSET 0x00C

static const int granul_time[] = {1, 4, 8, 16, 32, 100};
...
void ufs_soc_init(struct ufs_hba *hba)
{
	...
	writel(0x0C000000, SOC_SCTRL_SCPERCTRL6_ADDR(host->sysctrl));
	...
	sys_ctrl_set_bits(host,
		BIT(SOC_SCTRL_SCISODIS_ufs_mphy_top_iso_en_START) |
			BIT(SOC_SCTRL_SCISODIS_ufs_mphy_iso_en_START),
		SOC_SCTRL_SCISODIS);
	...
	/* config TP/SP memory TRA */
	reg = ufs_sys_ctrl_readl(host, UFS_SYS_MEMORY_CTRL_D1W2R);
	reg |= BIT(11);
	reg &= (~BIT(12));
	sys_ctrl_set_bits(host, reg, UFS_SYS_MEMORY_CTRL_D1W2R);

	reg = ufs_sys_ctrl_readl(host, UFS_SYS_MEMORY_CTRL);
	reg |= BIT(16);
	reg &= (~BIT(17));
	sys_ctrl_set_bits(host, reg, UFS_SYS_MEMORY_CTRL);

	reg = ufs_sys_ctrl_readl(host, UFS_SYS_AO_MEMORY_CTRL);
	reg |= BIT(16);
	reg &= (~BIT(17));
	sys_ctrl_set_bits(host, reg, UFS_SYS_AO_MEMORY_CTRL);
...
static void mphy_iso_enable(struct ufs_hba *hba)
{
	...
	if (host->caps & USE_MAIN_RESUME)
		writel(MPHY_ISO_EN,
		       SOC_SCTRL_SCISOEN_ADDR(host->sysctrl));
	else
		writel(MPHY_ISO_EN | MPHY_TOP_ISO_EN,
		       SOC_SCTRL_SCISOEN_ADDR(host->sysctrl));
}
...
static void mphy_iso_disable(struct ufs_hba *hba)
{
	...
	writel(0x00000600, SOC_SCTRL_SCISODIS_ADDR(host->sysctrl));
}
...
int hufs_suspend(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	writel(0x0C000400, SOC_SCTRL_SCPERCTRL6_ADDR(host->sysctrl));
...
int hufs_main_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	writel(UFS_PSW_MEM_MASK, SOC_SCTRL_SCPERCTRL6_ADDR(host->sysctrl));
	...
	/* Step 8. config TP/SP memory TRA */
	value = ufs_sys_ctrl_readl(host, UFS_SYS_MEMORY_CTRL_D1W2R);
	value |= BIT(11);
	value &= (~BIT(12));
	sys_ctrl_set_bits(host, value, UFS_SYS_MEMORY_CTRL_D1W2R);

	value = ufs_sys_ctrl_readl(host, UFS_SYS_MEMORY_CTRL);
	value |= BIT(16);
	value &= (~BIT(17));
	sys_ctrl_set_bits(host, value, UFS_SYS_MEMORY_CTRL);

	value = ufs_sys_ctrl_readl(host, UFS_SYS_AO_MEMORY_CTRL);
	value |= BIT(16);
	value &= (~BIT(17));
	sys_ctrl_set_bits(host, value, UFS_SYS_AO_MEMORY_CTRL);

	value = ufs_mphy_mcu_readl(host, UFS_MPHYMCU_SC_REG_UCE_M3_MEMCTRL_OFFSET);
	value |= BIT(16);
	value &= (~BIT(17));
	ufs_mphy_mcu_writel(host, value, UFS_MPHYMCU_SC_REG_UCE_M3_MEMCTRL_OFFSET);

	/* Step 9. wait for UFS MPHY PLL lock */
...
void hufs_device_hw_reset(struct ufs_hba *hba)
{
	...
	if (likely(!(host->caps & USE_HUFS_MPHY_TC))) {
		/*
		 * Enable device reset, bit[14] is rst_device_rst_n
		 * bit[30] is bitmask
		 */
		writel(0x40000000,
		       SOC_SCTRL_SCPERCTRL17_ADDR((host->sysctrl)));
		/* wait 2 ms */
		mdelay(2);
...
static void mphy_tactive_time_adapt(
	struct ufs_hba *hba, uint32_t tactive_compen_time)
{
	u32 tactivate_val;
	u32 tactivate_compen;
	u32 tactivate_round;
	u32 granul_val;

	hufs_uic_read_reg(hba, PA_TACTIVATE, &tactivate_val);
	hufs_uic_read_reg(hba, PA_GRANULARITY, &granul_val);
	if ((granul_val < 1) || (granul_val > TACTIVE_REG_MAX))
		granul_val = TACTIVE_REG_MAX;

	/* add 300us time compensation for tactivate */
	tactivate_compen = tactive_compen_time / granul_time[granul_val - 1];
	tactivate_round = tactive_compen_time % granul_time[granul_val - 1];
	/* tactivate val need round up */
	if (tactivate_round != 0)
		tactivate_compen += 1;

	hufs_uic_write_reg(hba, PA_TACTIVATE, tactivate_val + tactivate_compen);
}
...
int hufs_link_startup_post_change(struct ufs_hba *hba)
{
	...
	if (!(host->caps & USE_HUFS_MPHY_TC) || hba->host->is_emulator) {
		...
		mphy_tactive_time_adapt(hba, TACTIVE_COMPEN_300US);
	}
...
static void mphy_pmc_samsung_config(struct ufs_hba *hba, u32 gear)
{
	u32 cdr_bw;
...
void v300_gears_config(struct ufs_hba *hba, u32 gear_rx)
{
	...
	mphy_pmc_samsung_config(hba, gear_rx);
}
...
int ufs_get_hufs_hc(struct hufs_host *host)
{
	...
	if (IS_ERR(host->ufs_mphy_mcu_sctrl)) {
		err = PTR_ERR(host->ufs_mphy_mcu_sctrl);
		dev_err(dev, "ufs mphy mcu iomap error, err = 0x%x!\n", err);
		return -ENOMEM;
	}
	return 0;
}
...
void hufs_txpost_enable(struct ufs_pa_layer_attr *dev_req_params,u32 *value)
{
	if (((dev_req_params->pwr_rx == FAST_MODE) ||
		...
		(dev_req_params->gear_rx >= UFS_HS_G2)) {
