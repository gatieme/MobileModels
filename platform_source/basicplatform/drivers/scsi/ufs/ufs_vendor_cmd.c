...
static void ufshcd_cpplus_init(struct ufs_hba *hba,
	uint32_t feature)
{
	hba->host->cpplus_enable = 0;

	if ((hba->manufacturer_id != UFS_VENDOR_HI1861) ||
		strstarts(hba->model, UFS_MODEL_THOR920))
		return;

	hba->host->cpplus_enable = feature & VENDOR_FEATURE_CPPLUS;
	dev_info(hba->dev, "%s %d\n", __func__, hba->host->cpplus_enable);
}
#endif
				       ...
				       struct ufs_dev_desc *dev_desc)
{
	...
	ufshcd_cpplus_init(hba, dev_desc->vendor_feature);
#endif
#endif
}
...
static int ufshcd_ioctl_query_read_fsr(struct ufs_hba *hba,
