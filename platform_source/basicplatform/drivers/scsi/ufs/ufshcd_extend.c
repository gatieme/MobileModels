...
void ufshcd_high_temp_config(struct ufs_hba *hba)
{
	...
	int ret;
	if ((strstarts(hba->model, UFS_MODEL_THOR935) && (hba->dev_info.fw_version % 100 >= HIGH_TEMP_SUPPORT_FW_REV)) ||
		strstarts(hba->model, UFS_MODEL_THOR610) ||
		strstarts(hba->model, UFS_MODEL_THOR850)) {
		if ((hba->d_ext_ufs_feature_sup & UFS_DEV_TOO_HIGH_TEMP_SUPPORT)) {
			...
			dev_err(hba->dev, "%s:not support too high temp feature!\n", __func__);
		}
	} else {
		dev_err(hba->dev, "%s:not THOR935 or THOR850 or THOR610!\n", __func__);
	}
#endif
}
/**
