/*
 * ufshcd_hufs_extend.c
 *
 * The hufs extend implements for ufshcd.c
 *
...
static void ufs_set_ymtc_sec_unique_number(uint8_t *snum_buf, const uint8_t *desc_buf)
{
	int i, idx;

	/* ymtc only need 6Byte, add a 0x00 before every byte */
	for (i = 0; i < YMTC_SNUM_NUM_INDEX_FIRST; i++) {
		snum_buf[i * NUM_2] = 0x00;
		snum_buf[i * NUM_2 + 1] = desc_buf[QUERY_DESC_HDR_SIZE + i];
	}
	for (i = YMTC_SNUM_NUM_INDEX_FIRST; i < YMTC_SNUM_NUM_INDEX_SEC; i++) {
		snum_buf[i * NUM_2] = 0x00;
		idx = QUERY_DESC_SNUM_YMTC + i * NUM_4;
		snum_buf[i * NUM_2 + 1] = ((desc_buf[idx] & 0xF) << NUM_4) | (desc_buf[idx + NUM_2] & 0xF);
	}
}
				      ...
				      const char *product_name)
{
	...
	case UFS_VENDOR_XHFZ:
		for (i = 0; i < XHFZ_SNUM_NUM_INDEX_FIRST; i++) {
			idx = QUERY_DESC_SNUM_XHFZ + i * 2 + 1;
			snum_buf[i] = desc_buf[idx];
		}
		for (i = XHFZ_SNUM_NUM_INDEX_FIRST; i < XHFZ_SNUM_NUM_INDEX_SEC; i++) {
			idx = QUERY_DESC_SNUM_XHFZ_SEC + i * 2 + 1;
			snum_buf[i] = desc_buf[idx];
		}
		break;
	case UFS_VENDOR_YMTC:
	case UFS_VENDOR_CWHC:
		ufs_set_ymtc_sec_unique_number(snum_buf, desc_buf);
		break;
	default:
...
#endif /* CONFIG_BOOTDEVICE */
}

static void ufs_scramble_config(struct ufs_hba *hba)
{
	if (strstarts(hba->model, UFS_MODEL_THOR935) || strstarts(hba->model, UFS_MODEL_THOR850) ||
		strstarts(hba->model, UFS_MODEL_THOR610))
		hba->scramble = true;
}

static void hufs_get_smsv6_info(struct ufs_hba *hba)
{
	char *product_name = hba->model;
	struct list_head *node = NULL;
	struct samsung_pnm *samsung_v6 = NULL;

	list_for_each(node, &hba->samsung_v6_whitelist) {
		samsung_v6 = list_entry(node, struct samsung_pnm, p);
		if (memcmp(product_name, samsung_v6->ufs_pnm, MAX_MODEL_LEN) == 0) {
			hba->is_samsung_v6 = true;
			return;
		}
	}
	hba->is_samsung_v6 = false;
}

#ifdef CONFIG_UFS_H8_IDLE_3MS
static void ufshcd_set_opt_ahit(struct ufs_hba *hba)
{
	unsigned long flags;

	if (strstarts(hba->model, UFS_MODEL_THOR935)) {
		spin_lock_irqsave(hba->host->host_lock, flags);
		hba->ahit = UFS_AHIT_AUTOH8_OPT_TIMER;
		spin_unlock_irqrestore(hba->host->host_lock, flags);
	}
}
#endif

int hufs_get_device_info(struct ufs_hba *hba,
		struct ufs_dev_desc *dev_desc, u8 *desc_buf)
{
	int err;

	strlcpy(hba->model, dev_desc->model, (MAX_MODEL_LEN + 1));
	memset(desc_buf, 0, QUERY_DESC_MAX_SIZE);

	/* spec is unicode but sec use hex data */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	err = ufshcd_read_desc_param(hba, QUERY_DESC_IDN_STRING, dev_desc->serial_num_index, 0,
	...
	ufs_scramble_config(hba);
	hufs_get_smsv6_info(hba);
#ifdef CONFIG_UFS_H8_IDLE_3MS
	ufshcd_set_opt_ahit(hba);
#endif
	return 0;
}
...
#endif
#ifdef CONFIG_SCSI_UFS_GEAR_CTRL
	ufshcd_gear_ctrl_op_register(hba);
#endif
}
