 ...
 * Description: A driver for the SPI bus master.
...
#include "sts.h"
static sts_sys_status_enum g_iodie_state;
#endif
static int hs_spi_runtime_suspend(struct device *dev);
static int hs_spi_runtime_resume(struct device *dev);
/*
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
static void iodie_state_cb(sts_sys_status_enum status, sts_err_info_t *info)
{
	(void)(info);
	g_iodie_state = status;
	pr_err("iodie state change to %d\n", status);
}
#endif
...
static void restore_state(struct hs_spi_controller *spi_control)
{
	...
	if (SPI_V600_CONTROLLER_CHECK(readl(SSP_VER(spi_control->virtbase)))) {
		writel(DEFAULT_SSP_REG_FIFOCR_V600, SSP_FIFOCR(spi_control->virtbase));
		writel(DISABLE_ALL_INTERRUPTS_V600, SSP_IMC(spi_control->virtbase));
		writel(CLEAR_ALL_INTERRUPTS_V600, SSP_ICR(spi_control->virtbase));
	} else {
...
static void spi_iodie_pre_cfg(struct hs_spi_controller *spi_ctrl,
	u32 *irq_msk_st, u32 *comm_cr)
{
	/* disable SPI */
	writel((readl(SSP_ENR(spi_ctrl->virtbase)) &
		(~SSP_ENR_MASK_EN)), SSP_ENR(spi_ctrl->virtbase));

	*irq_msk_st = readl(SSP_IMC(spi_ctrl->virtbase));
	*comm_cr = readl(SSP_COMMONCR(spi_ctrl->virtbase));

	if (SPI_V600_CONTROLLER_CHECK(readl(SSP_VER(spi_ctrl->virtbase))))
		writel(DISABLE_ALL_INTERRUPTS_V600, SSP_IMC(spi_ctrl->virtbase));
	else
		writel(DISABLE_ALL_INTERRUPTS, SSP_IMC(spi_ctrl->virtbase));

	writel((readl(SSP_COMMONCR(spi_ctrl->virtbase)) & (~(BIT(26) | BIT(27)))),
			SSP_COMMONCR(spi_ctrl->virtbase));
...
static void spi_iodie_post_cfg(struct hs_spi_controller *spi_ctrl,
	u32 irq_msk_st, u32 comm_cr)
{
	/* disable SPI */
	writel((readl(SSP_ENR(spi_ctrl->virtbase)) &
	...
	writel(irq_msk_st, SSP_IMC(spi_ctrl->virtbase));
	writel(comm_cr, SSP_COMMONCR(spi_ctrl->virtbase));
...
static void spi_proc_iodie(struct hs_spi_controller *spi_ctrl)
{
	u32 irq_msk_st;
	u32 comm_cr;
	int ret;
	struct spi_transfer *xfer = spi_ctrl->cur_transfer;
	u8 temp_rx[DUMMY_MAX] = {0};
	u8 temp_tx[DUMMY_MAX] = {0};
	unsigned long time, timeout;

	if (!spi_ctrl->support_iodie)
		return;

	if (!xfer->rx_buf)
		return;

	if (readl(SSP_COMMONCR(spi_ctrl->virtbase)) & SSP_COMMONCR_LOOPBACK)
		return;

	spi_ctrl->tx = (void*)temp_tx;
	spi_ctrl->tx_end = spi_ctrl->tx + spi_ctrl->iodie_dummy;
	spi_ctrl->write = WRITING_NULL;

	spi_ctrl->rx = (void*)temp_rx;
	spi_ctrl->rx_end = spi_ctrl->rx + spi_ctrl->iodie_dummy;

	spi_iodie_pre_cfg(spi_ctrl, &irq_msk_st, &comm_cr);

	gpio_direction_output(spi_ctrl->iodie_cs_gpio, 0);
	udelay(10);
	timeout = jiffies + msecs_to_jiffies(SPI_POLLING_TIMEOUT);
	while (spi_ctrl->tx < spi_ctrl->tx_end || spi_ctrl->rx < spi_ctrl->rx_end) {
		time = jiffies;
		readwriter(spi_ctrl);
		if (time_after(time, timeout)) {
			dev_err(&spi_ctrl->pdev->dev,"%s: timeout!\n", __func__);
			break;
		}
	}
	gpio_direction_output(spi_ctrl->iodie_cs_gpio, 1);
	/* buf len > dummy */
	if (xfer->len > spi_ctrl->iodie_dummy) {
		ret = memmove_s(xfer->rx_buf, xfer->len - spi_ctrl->iodie_dummy,
			xfer->rx_buf + spi_ctrl->iodie_dummy, xfer->len - spi_ctrl->iodie_dummy);
		if (ret != EOK)
			dev_err(&spi_ctrl->pdev->dev, "%s: memmove_s failed, ret is %d\n", __func__, ret);
		ret = memcpy_s(xfer->rx_buf + xfer->len - spi_ctrl->iodie_dummy, spi_ctrl->iodie_dummy,
			temp_rx, spi_ctrl->iodie_dummy);
		if (ret != EOK)
			dev_err(&spi_ctrl->pdev->dev, "%s: memcpy_s failed, ret is %d\n", __func__, ret);
	} else {
		ret = memcpy_s(xfer->rx_buf, xfer->len, temp_rx + spi_ctrl->iodie_dummy - xfer->len,
			xfer->len);
		if (ret != EOK)
			dev_err(&spi_ctrl->pdev->dev, "%s: memcpy_s failed, ret is %d\n", __func__, ret);
	}

	spi_iodie_post_cfg(spi_ctrl, irq_msk_st, comm_cr);
}
...
static void hs_proc_exclusive_dma_irq(struct hs_spi_controller *spi_control, u32 irq_status)
{
	struct spi_transfer *xfer = spi_control->cur_transfer;
	struct spi_message *msg = spi_control->cur_msg;

	/* clear irq */
	writel(irq_status, INTR_SPI_DMA_CLR(spi_control->virtbase));
	/* mask  all irq */
	writel(EXCLUSIVE_DMA_ALL_INT, INTR_SPI_DMA_MASK(spi_control->virtbase));
	/* stop dma */
	writel(0, SSP_DMA_CTRL(spi_control->virtbase));

	__dma_unmap_area(xfer->rx_buf, xfer->len, DMA_FROM_DEVICE);
	__dma_unmap_area(xfer->tx_buf, xfer->len, DMA_TO_DEVICE);
	if (irq_status & INT_DMA_COMPLETE) {
		msg->actual_length += spi_control->cur_transfer->len;
		spi_proc_iodie(spi_control);
		if (spi_control->cur_transfer->cs_change)
			hs_spi_cs_control(spi_control, SSP_CHIP_DESELECT);
		...
		msg->state = STATE_ERROR;
		tasklet_schedule(&spi_control->pump_transfers);
		pr_err("spi transfer error irq 0x%x\n", irq_status);
	}
}
...
static irqreturn_t hs_spi_interrupt_handler(int irq, void *dev_id)
{
	...
	irq_status = readl(INTR_SPI_DMA_RAW(spi_control->virtbase));
	if (irq_status) {
		hs_proc_exclusive_dma_irq(spi_control, irq_status);
		return IRQ_HANDLED;
	}
	...
	if (spi_control->tx == spi_control->tx_end)
		...
		writel((readl(SSP_IMC(spi_control->virtbase)) &
		       ~SSP_IMC_MASK_RXIM) | SSP_IMC_MASK_TXIM,
...
static void hs_spi_config_dma_addr(struct hs_spi_controller *spi_control,
		struct spi_transfer *xfer)
{
	writel((u32)(xfer->tx_dma & SPI_32BITS_MASK), SSP_SDMA_ADDR_L_R(spi_control->virtbase));
	writel((u32)(xfer->tx_dma >> 32), SSP_SDMA_ADDR_H_R(spi_control->virtbase));

	writel((u32)(xfer->rx_dma & SPI_32BITS_MASK), SSP_SDMA_ADDR_L_W(spi_control->virtbase));
	writel((u32)(xfer->rx_dma >> 32), SSP_SDMA_ADDR_H_W(spi_control->virtbase));
}

static void config_exclusive_dma(struct spi_master *master)
{
	struct hs_spi_controller *spi_control = spi_master_get_devdata(master);
	struct spi_transfer *xfer = NULL;

	xfer = spi_control->cur_transfer;

	/* mask  all irq */
	writel(EXCLUSIVE_DMA_ALL_INT, INTR_SPI_DMA_MASK(spi_control->virtbase));

	/* enable sdma */
	writel(DMA_ENBALE_APB | spi_control->cur_transfer->len, SSP_DMA_CTRL(spi_control->virtbase));

	xfer->rx_dma = virt_to_phys(xfer->rx_buf);
	__dma_map_area(xfer->rx_buf, xfer->len, DMA_FROM_DEVICE);

	xfer->tx_dma = virt_to_phys(xfer->tx_buf);
	__dma_map_area(xfer->tx_buf, xfer->len, DMA_TO_DEVICE);

	hs_spi_config_dma_addr(spi_control, xfer);

	/* enable all irq */
	writel(0, INTR_SPI_DMA_MASK(spi_control->virtbase));

	writel((readl(SSP_ENR(spi_control->virtbase)) | SSP_ENR_MASK_EN), SSP_ENR(spi_control->virtbase));

	writel(DMA_CMD_START, SSP_DMA_CMD_START(spi_control->virtbase));
}
...
static void pump_transfers(unsigned long data)
{
	...
	flush(spi_control);
	if (spi_control->cur_chip->xfer_type == EXCLUSIVE_DMA_TRANSFER) {
		config_exclusive_dma(spi_control->master);
		return;
	}
...
static void do_interrupt_dma_transfer(struct hs_spi_controller *spi_control)
{
	 ...
	 */
	u32 irqflags = (u32)(ENABLE_ALL_INTERRUPTS | SSP_IMC_MASK_RXIM);
	...
	if (spi_control->cur_chip->enable_dma) {
		...
		if (SPI_V600_CONTROLLER_CHECK(readl(SSP_VER(spi_control->virtbase)))) {
			irqflags = DISABLE_ALL_INTERRUPTS_V600 & ~SSP_IMC_MASK_RORIM;
		} else{
...
static void do_exclusive_dma_transfer(struct spi_master *master)
{
	struct hs_spi_controller *spi_control = spi_master_get_devdata(master);
					...
					struct spi_message *msg)
{
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	if (spi_control->support_iodie && g_iodie_state == STS_ST_ERROR) {
		msg->status = -EIO;
		giveback(spi_control);
		return -EIO;
	}
	...
	else if (spi_control->cur_chip->xfer_type == EXCLUSIVE_DMA_TRANSFER)
		do_exclusive_dma_transfer(master);
	else
...
static int hs_spi_prepare_transfer_hardware(struct spi_master *master)
{
	...
	if (spi_control->spi_hwspin_lock) {
		ret = hwspin_lock_timeout_in_atomic(spi_control->spi_hwspin_lock,
		GET_HARDWARE_TIMEOUT);
		if (ret) {
			dev_err(&spi_control->pdev->dev,
	...
	ret = hs_spi_runtime_resume(master->dev.parent);
	return ret;
}
...
static int hs_spi_unprepare_transfer_hardware(struct spi_master *master)
{
	...
	hs_spi_runtime_suspend(master->dev.parent);
	if (spi_control->spi_hwspin_lock)
		hwspin_unlock_in_atomic(spi_control->spi_hwspin_lock);
	...
	struct hs_spi_config_chip const *chip_info)
{
	if ((chip_info->com_mode != INTERRUPT_TRANSFER)
		...
		&& (chip_info->com_mode != EXCLUSIVE_DMA_TRANSFER)) {
		dev_err(&spi_control->pdev->dev,
	...
	struct chip_data *chip, struct ssp_clock_params clk_freq)
{
	...
	SSP_WRITE_BITS(chip->cr, (u32)clk_freq.cpsdvsr, SSP_COMMONCR_MASK_DRPR,
		BIT_DRPR);
...
static int hs_spi_controller_setup(struct spi_device *spi)
{
	...
	if ((chip_info->clk_freq.cpsdvsr == 0)
		...
		if (ret != EOK) {
			status = -EINVAL;
			dev_err(&spi->dev, "%s memcpy_s failed\n", __func__);
			goto err_config_params;
		}
...
static void hs_spi_parse_iodio_cfg(struct platform_device *pdev,
	struct hs_spi_controller *spi_ctrl)
{
	int ret;
	struct device_node *np = pdev->dev.of_node;

	if (of_property_read_u32(np, "support-iodie", &spi_ctrl->support_iodie))
		spi_ctrl->support_iodie = 0;

	if (spi_ctrl->support_iodie) {
		of_property_read_u32(np, "iodie-dummy-bytes", &spi_ctrl->iodie_dummy);
		of_property_read_u32(np, "iodie-cs", &spi_ctrl->iodie_cs_gpio);
	} else {
		dev_err(&pdev->dev, "not support iodie\n");
		return;
	}

	if ((!spi_ctrl->iodie_dummy) || (!spi_ctrl->iodie_cs_gpio)) {
		spi_ctrl->support_iodie = 0;
		dev_err(&pdev->dev, "support-iodie cfg err\n");
		return;
	}

	ret = gpio_request(spi_ctrl->iodie_cs_gpio, "iodie_cs");
	if (ret) {
		spi_ctrl->support_iodie = 0;
		dev_err(&pdev->dev, "request iodie cs gpio failed\n");
		return;
	}

	gpio_direction_output(spi_ctrl->iodie_cs_gpio, 1);

	dev_err(&pdev->dev, "sprt-iodie %d, dummy_bytes %d, iodie_cs %d\n",
		spi_ctrl->support_iodie, spi_ctrl->iodie_dummy,
		spi_ctrl->iodie_cs_gpio);
}
			...
			u64 *clk_rate)
{
	...
	if (of_property_read_u32(np, "hardware-mutex",
		...
		spi_control->spi_hwspin_lock = NULL;
	...
	platform_data->rt = of_property_read_bool(np, "spi,rt");
	dev_info(&pdev->dev, "spi,rt : %d\n", platform_data->rt);
	...
	hs_spi_parse_iodio_cfg(pdev, spi_control);
	...
	struct hs_spi_controller *spi_control, u64 clk_rate)
{
	...
	if (IS_ERR(spi_control->clk)) {
		...
		dev_err(&pdev->dev, "could not retrieve SSP/SPI bus clock %d\n", status);
		ret = E_SPI_NO_CLK;
		return ret;
	}
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	if (spi_control->support_iodie) {
		g_iodie_state = STS_ST_PRE_INITED;
		status = sts_notify_register(STS_USER_SPI, iodie_state_cb);
		if (status != 0)
			dev_err(dev, "sts_notify_register failed\n");
	}
#endif
...
static int hs_spi_master_remove(struct platform_device *pdev)
{
