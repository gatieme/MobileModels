...
static int atfd_spmi_smc(u64 _function_id, u64 _arg0,
	u64 _arg1, u64 _arg2, u64 _arg3)
{
	struct arm_smccc_res res = {0};

#ifndef CONFIG_LIBLINUX
	arm_smccc_smc(_function_id, _arg0, _arg1, _arg2, _arg3, 0, 0, 0, &res);
#else
	arm_smccc_1_1_smc(_function_id, _arg0, _arg1, _arg2, _arg3, 0, 0, 0, &res);
#endif
	...
	u8 opcode, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	if (ctrl->always_sec) {
		...
		for (i = 0; i < bc; i++) {
			ret = atfd_spmi_smc((u64)SPMI_FN_MAIN_ID, (u64)SPMI_READ,
	...
	u8 opcode, u8 sid, u16 addr, u8 bc, u8 *buf)
{
	...
	if (ctrl->always_sec) {
		spin_lock_irqsave(&ctrl->sec_lock, flags);
		for (i = 0; i < bc; i++) {
			ret = atfd_spmi_smc((u64)SPMI_FN_MAIN_ID, (u64)SPMI_WRITE,
	...
	struct device_driver *drv)
{
	...
	if (drv->name)
		return strncmp(dev_name(&spmi_dev->dev), drv->name, SPMI_NAME_SIZE) == 0;
	return 0;
}
