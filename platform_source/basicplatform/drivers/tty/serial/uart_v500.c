...
static int uart_v500_suspend(struct device *dev)
{
...
static int uart_v500_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct uart_v500_port *uap = platform_get_drvdata(pdev);
	int ret;
...
static void uart_v500_disable_msk(struct uart_port *port)
{
	...
	uap->int_msk |= UART_V500_INT_CTSM | UART_V500_INT_RIM | UART_V500_INT_DCDM | UART_V500_INT_DSRM;
	...
	struct device *dev, struct uart_v500_dma_data *plat)
{
	...
	request_chan = dma_request_chan(dev, "tx");
...
static irqreturn_t uart_v500_int_handler(int irq, void *dev_id)
{
	...
	if (status) {
		do {
			...
			if (status & (UART_V500_INT_CTSS | UART_V500_INT_RIS | UART_V500_INT_DCDS | UART_V500_INT_DSRS))
...
static int uart_v500_hwinit(struct uart_port *port)
{
	...
	uart_v500_write(UART_V500_INT_STAUS, uap, REG_UART_INT_CLR);
	...
	uart_v500_write(~(UART_V500_INT_RXM | UART_V500_INT_RTM) &
...
static int uart_v500_startup(struct uart_port *port)
{
	...
	uart_cfg |= UART_V500_CFG_UART_EN | UART_V500_CFG_RX_EN |
		UART_V500_CFG_TX_EN | UART_V500_CFG_FIFO_EN;
	uart_v500_write(uart_cfg, uap, REG_UART_CONFIG);
...
static void uart_v500_disable_uart(struct uart_v500_port *uap)
{
	...
	uart_cfg = (UART_V500_CFG_TX_EN | UART_V500_CFG_UART_EN | UART_V500_CFG_FIFO_EN);
	uart_v500_write(uart_cfg, uap, REG_UART_CONFIG);
	spin_unlock_irq(&uap->port.lock);
	...
	struct ktermios *termios)
{
	...
	if (uap->fifosize > 1)
		uart_cfg |= (UART_V500_CFG_RX_EN | UART_V500_CFG_TX_EN |
			UART_V500_CFG_UART_EN | UART_V500_CFG_FIFO_EN);
...
{
	if ((unsigned int)flags & UART_CONFIG_TYPE) {
...
static int uart_v500_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "cts-flag",
		&uap->cts_flag))
		uap->cts_flag = 0;
...
static SIMPLE_DEV_PM_OPS(uart_v500_dev_pm_ops,
	uart_v500_suspend, uart_v500_resume);
