...
void chip_charger_type_notify(enum chip_charger_type charger_type, void *data)
{
	int ret;
...
{
	if ((chip_dwc->charger_type == CHARGER_TYPE_SDP) ||
		(chip_dwc->charger_type == CHARGER_TYPE_CDP))
		return true;
...
static void bc_again_work(struct work_struct *work)
{
	...
	if (bc_again_allowed(chip_dwc)) {
		usb_dbg("current charger_type %d, start bc_again\n", chip_dwc->charger_type);
		bc_again(chip_dwc);
	}
...
void schedule_bc_again(struct chip_dwc3_device *chip_dwc)
{
	...
	else if ((chip_dwc->charger_type == CHARGER_TYPE_DCP)
...
int chip_usb_bc_init(struct chip_dwc3_device *chip_dwc)
{
	...
	if (of_property_read_bool(dev->of_node, "disable_bc_detect"))
		chip_dwc->disable_bc_detect = 1;
	else
		chip_dwc->disable_bc_detect = 0;
