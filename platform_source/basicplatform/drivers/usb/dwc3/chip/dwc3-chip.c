...
#include <linux/platform_drivers/usb/chip_usb_log.h>
...
static void chip_usb_state_notify(enum usb_state state,
			struct chip_dwc3_device *chip_dwc)
{
	int ret;

	if (!chip_dwc->notify_state)
		return;
...
void chip_usb_set_hifi_ip_first(int flag)
{
	...
	chip_dwc3->hifi_ip_first = (unsigned int)flag;
}
...
{
	if (chip_dwc3->wake_lock && !chip_dwc3->wake_lock->active) {
...
static int start_device(struct chip_dwc3_device *chip_dwc)
{
	...
	chip_usb_state_notify(USB_STATE_DEVICE, chip_dwc);
	...
	if ((chip_dwc->charger_type != CHARGER_TYPE_DCP) ||
		((chip_dwc->charger_type == CHARGER_TYPE_DCP)
		&& (chip_dwc->bc_dcp_again_flag == 0)))
...
static void stop_device(struct chip_dwc3_device *chip_dwc)
{
	...
	chip_usb_state_notify(USB_STATE_OFF, chip_dwc);
}
...
static int start_host(struct chip_dwc3_device *chip_dwc)
{
	...
	chip_usb_state_notify(USB_STATE_HOST, chip_dwc);
	...
	if(chip_dwc->use_new_frame) {
		ret = chip_usb_controller_probe(chip_dwc);
		if (ret) {
...
#ifdef CONFIG_PRODUCT_ARMPC
	if(chip_dwc->use_new_frame)
		chip_usb_controller_destroy(chip_dwc);
#endif
...
static void dwc3_speed_change_work(struct work_struct *work)
{
	...
	if (chip_dwc->use_new_frame && chip_dwc->notify_speed) {
		...
		if ((chip_dwc->speed != USB_CONNECT_HOST) &&
				(chip_dwc->speed != USB_CONNECT_DCP))
			power_event_bnc_notify(POWER_BNT_HW_USB, POWER_NE_HW_USB_SPEED, &chip_dwc->speed);
#endif
...
int chip_usb20_phy_calibrate(void)
{
	...
	return dwc3_usb20_phy_calibrate(chip_dwc3);
}
EXPORT_SYMBOL_GPL(chip_usb20_phy_calibrate);
...
static int usb_get_hardware(struct chip_dwc3_device *chip_usb)
{
	...
	chip_usb->usb_tzpc_config = of_property_read_bool(node,
			"usb_tzpc_config");
...
static int get_resource(struct chip_dwc3_device *chip_dwc3)
{
	...
	ret = usb_get_hardware(chip_dwc3);
	if (ret) {
	...
	chip_dwc3->notify_state = of_property_read_bool(dev->of_node,
				"notify_state");
	of_property_read_u32(dev->of_node, "portid", &(chip_dwc3->portid));
	...
	chip_dwc3->usb_wakeup.usb_support_s3_wakeup =
		device_property_read_bool(dev, "usb_support_s3_wakeup");

	chip_dwc3->usb_wakeup.irq = of_irq_get_byname(dev->of_node,
						      "wakeup_irq");
	if (chip_dwc3->usb_wakeup.irq < 0) {
		usb_info("usb driver not support irq.\n");
		chip_dwc3->usb_wakeup.irq = 0;
	}

	chip_dwc3->usb_wakeup.reg_addr = of_iomap(dev->of_node, 0);
	if (chip_dwc3->usb_wakeup.reg_addr == NULL)
		usb_info("usb driver not support reg.\n");
...
static int dwc3_initialize(struct chip_dwc3_device *chip_dwc)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	chip_dwc->wake_lock = wakeup_source_register(dev, "usb_wake_lock");
#else
	chip_dwc->wake_lock = wakeup_source_register("usb_wake_lock");
#endif
	if (!chip_dwc->wake_lock)
		usb_err("dw3c wakeup source register failed.\n");
	return 0;
}
...
static int dwc3_extcon_dev_register(struct device *dev,
					   struct chip_dwc3_device *chip_dwc)
{
	int ret;
...
static int dwc3_probe(struct platform_device *pdev)
{
	...
	ret = dwc3_extcon_dev_register(dev, chip_dwc);
	if (ret) {
		usb_err("dwc3 extcon dev register failed, %d\n", ret);
		return ret;
	}
...
err_wakeup_source_unregister:
	wakeup_source_unregister(chip_dwc->wake_lock);
	chip_dwc->wake_lock = NULL;
...
static int dwc3_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(chip_dwc3->wake_lock);
	chip_dwc3->wake_lock = NULL;
	chip_dwc3_dev = NULL;
...
void dwc3_handle_wakeup_event(struct device *dev)
{
	struct chip_dwc3_device *chip_dwc3 = NULL;
	u32 portsc_val;
	int i;

	chip_dwc3 = platform_get_drvdata(to_platform_device(dev));
	if (chip_dwc3->usb_wakeup.reg_addr == NULL ||
	    chip_dwc3->usb_wakeup.usb_do_wakeup_flag == true)
		return;

	for (i = 0; i < PORT_NUM; i++) {
		portsc_val = readl((u32 *)chip_dwc3->usb_wakeup.reg_addr +
				    PORTSC_OFFSET + i * NUM_PORT_REGS);
		if ((portsc_val & PORT_PLS_MASK) == XDEV_RESUME) {
			usb_info("do system wakeup when resume event come.\n");
			pm_system_wakeup();
			chip_dwc3->usb_wakeup.usb_do_wakeup_flag = true;
			return;
		}
...
irqreturn_t wakeup_irq(int irq, void *dev)
{
	dwc3_handle_wakeup_event(dev);
	return IRQ_HANDLED;
}

int dwc3_request_wakeup_irq(struct device *dev, struct chip_dwc3_device *chip_dwc3)
{
	int retval;
	unsigned long irqflags = IRQF_SHARED | IRQF_NO_SUSPEND;

	if (chip_dwc3->usb_wakeup.irq == 0) {
		usb_err("dwc3 irq is 0.\n");
		return 0;
	}

	retval = request_irq(chip_dwc3->usb_wakeup.irq, &wakeup_irq, irqflags,
			    "dwc3 wakeup event irq", dev);
	if (retval != 0) {
		usb_err("request irq %d failed.\n", chip_dwc3->usb_wakeup.irq);
		return retval;
	}

	usb_info("dwc3 request irq %d success.\n", chip_dwc3->usb_wakeup.irq);
	return 0;
}

int dwc3_free_wakeup_irq(struct device *dev, struct chip_dwc3_device *chip_dwc3)
{
	if (chip_dwc3->usb_wakeup.irq == 0)
		return 0;

	free_irq(chip_dwc3->usb_wakeup.irq, dev);
	usb_info("dwc3 free irq %d.\n", chip_dwc3->usb_wakeup.irq);
	return 0;
}
...
static int dwc3_suspend(struct device *dev)
{
	...
	if (chip_dwc3->usb_wakeup.usb_support_s3_wakeup) {
		dwc3_handle_wakeup_event(dev);
		if (dwc3_request_wakeup_irq(dev, chip_dwc3))
			usb_err("dwc3 request failed.\n");
...
static int dwc3_resume(struct device *dev)
{
	...
	if (chip_dwc3->usb_wakeup.usb_support_s3_wakeup) {
		...
		dwc3_free_wakeup_irq(dev, chip_dwc3);
		if (chip_dwc3->usb_wakeup.usb_do_wakeup_flag == true) {
			pm_system_cancel_wakeup();
			chip_dwc3->usb_wakeup.usb_do_wakeup_flag = false;
		}
		return 0;
	}
