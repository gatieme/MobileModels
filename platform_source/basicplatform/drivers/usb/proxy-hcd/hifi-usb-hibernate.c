...
void hifi_usb_hibernation_ctrl(enum hibernation_ctrl_type type, bool set)
{
	...
	if ((proxy->hibernation_support != 0)
		...
		if (proxy->hibernation_ctrl == hibernation_ctrl_mask) {
			dbg("Do hibernate\n");
#ifdef CONFIG_USB_HIUSB
			ret = hiusb_start_hifi_hibernate(proxy->current_port);
#else
...
static void hifi_usb_complete_wakeup_callback(struct chip_usb_event *unused)
{
	struct hifi_usb_proxy *proxy = get_hifi_usb_proxy_handle();
...
#ifdef CONFIG_USB_HIUSB
	ret =hiusb_start_hifi_wakeup(proxy->current_port);
	if (ret)
		return ret;
#else
	event.type = HIFI_USB_WAKEUP;
	event.flags = EVENT_CB_AT_COMPLETE;
	event.callback = hifi_usb_complete_wakeup_callback;
...
err_hw_revive:
	...
	usb_stop_hifi_usb_by_portid(proxy->current_port);
