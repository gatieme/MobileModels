...
static int __start_hifi_usb(int portid, bool reset_power)
{
...
#ifdef CONFIG_USB_HIUSB
	return hiusb_start_hifi_usb(portid, reset_power);
#else
	(void)portid;
	if (reset_power)
...
int usb_start_hifi_usb(struct usb_device *udev)
{
	int portid;
	portid = usb_find_portid_by_udev(udev);
	pr_info("porid id :%d\n", portid);
	if (portid < 0)
		return -ENODEV;
	return __start_hifi_usb(portid, false);
}
EXPORT_SYMBOL(usb_start_hifi_usb);

int usb_start_hifi_usb_reset_power(struct usb_device *udev)
{
	int portid;
	portid = usb_find_portid_by_udev(udev);
	pr_info("porid id :%d\n", portid);
	if (portid < 0)
		return -ENODEV;
	return __start_hifi_usb(portid, true);
}
EXPORT_SYMBOL(usb_start_hifi_usb_reset_power);

void usb_stop_hifi_usb_by_portid(int portid)
{
	info("\n");
...
#ifdef CONFIG_USB_HIUSB
	hiusb_stop_hifi_usb(portid, false);
#else
	(void)portid;
...
EXPORT_SYMBOL(usb_stop_hifi_usb_by_portid);

void usb_stop_hifi_usb(struct usb_device *udev)
{
	int portid;
	portid = usb_find_portid_by_udev(udev);
	pr_info("porid id :%d\n", portid);
	if (portid < 0)
		return;
	usb_stop_hifi_usb_by_portid(portid);
}
EXPORT_SYMBOL(usb_stop_hifi_usb);

void usb_stop_hifi_usb_reset_power_by_portid(int portid)
{
	info("\n");
...
#ifdef CONFIG_USB_HIUSB
	hiusb_stop_hifi_usb(portid, true);
#else
	(void)portid;
...
void usb_stop_hifi_usb_reset_power(struct usb_device *udev)
{
	int portid;
	portid = usb_find_portid_by_udev(udev);
	pr_info("porid id :%d\n", portid);
	...
	usb_stop_hifi_usb_reset_power_by_portid(portid);
}
EXPORT_SYMBOL(usb_stop_hifi_usb_reset_power);

int usb_find_portid_by_udev(struct usb_device *udev)
{
#ifdef CONFIG_USB_HIUSB
	if (udev == NULL)
		return -EINVAL;
	...
	if(usb_using_hifi_usb(udev)) {
		return get_hifi_usb_portid();
	} else {
		return hiusb_find_portid_by_udev(udev);
	}
#else
	return 0;
#endif
}
EXPORT_SYMBOL(usb_find_portid_by_udev);
...
static void fill_dev_ctrl_msg(struct hifi_usb_op_msg *op_msg, __u8 msg_type, int slot_id)
{
	...
	op_msg->data_len = (__u16)sizeof(op_msg->dev_ctrl);
}
		...
		int slot_id, struct usb_endpoint_descriptor *desc)
{
	...
	op_msg->data_len = (__u16)sizeof(op_msg->ep_ctrl);
}
...
int hifi_usb_proxy_update_device(struct proxy_hcd_client *client, int slot_id, struct usb_ext_cap_descriptor *ext_cap)
{
	...
	op_msg->data_len = (__u16)sizeof(op_msg->bos_data);
	ret = hifi_usb_send_op_msg_and_wait(proxy);
	mutex_unlock(&proxy->msg_lock);
					...
					char *buf)
{
	...
	op_msg->data_len = (__u16)sizeof(op_msg->hub_ctrl);
...
static int do_isoc_xfer_test(struct proxy_hcd_client *client, struct urb_msg *urb_msg)
{
	...
	if (op_msg->data_len > (__u16)sizeof(op_msg->data))
		op_msg->data_len = (__u16)sizeof(op_msg->data);
...
static int prepare_urb_op_msg(struct hifi_usb_proxy *proxy, struct urb_msg *urb_msg, struct hifi_usb_op_msg *op_msg)
{
	...
	op_msg->data_len = (__u16)sizeof(op_msg->urb);
...
{
	if (proxy->runstop == 0 && proxy->hibernation_state == 0) {
		info("hifi reset, but hifi usb not running && not in hibernate-state\n");
		return;
	}
	...
	usb_stop_hifi_usb_reset_power_by_portid(proxy->current_port);
	mutex_lock(&proxy->msg_lock);
}
...
int hifi_usb_runstop_and_wait(struct hifi_usb_proxy *proxy, bool run)
{
	...
	proxy->runstop_msg.reserved = (u16)proxy->current_port;
	info("portid %d\n", proxy->current_port);
...
int set_hifi_usb_portid(int portid)
{
	if (hifi_usb == NULL) {
	...
	hifi_usb->current_port = portid;
	return 0;
}

int get_hifi_usb_portid(void)
{
	if (hifi_usb == NULL) {
		err("No proxy, hifi usb exited\n");
		WARN_ON(1);
...
static void confirm_udev_timer_fn(struct timer_list *data)
#else
...
#endif
{
	...
	portid = get_hifi_usb_portid();
	if (portid < 0)
		return;
	if (!isr_enabled())
		return;

	info("No usb device connected, do STOP_HIFI_USB port %d\n", portid);
	usb_stop_hifi_usb_by_portid(portid);
}

static int get_qos_ops_resource(struct device *dev, struct hifi_usb_proxy *hifiusb)
{
	int ret;
	ret = get_chip_usb_reg_cfg_array(dev, "multi-port-qos-set",
			&(hifiusb->multi_port_qos_set), &(hifiusb->multi_port_qos_set_num));
	if (ret)
		goto exit_multi_port_qos_set;
	ret = get_chip_usb_reg_cfg_array(dev, "multi-port-qos-reset",
			&(hifiusb->multi_port_qos_reset), &(hifiusb->multi_port_qos_reset_num));
	if (ret)
		goto exit_multi_port_qos_reset;
	return 0;
exit_multi_port_qos_reset:
	free_chip_usb_reg_cfg_array(hifiusb->multi_port_qos_set, hifiusb->multi_port_qos_set_num);
exit_multi_port_qos_set:
	err("get multi port qos fail!\n");
	return ret;
}

static void put_qos_ops_resource(struct hifi_usb_proxy *hifiusb)
{
	if(!hifiusb->multi_port_qos_enable)
		return;
	free_chip_usb_reg_cfg_array(hifiusb->multi_port_qos_set, hifiusb->multi_port_qos_set_num);
	free_chip_usb_reg_cfg_array(hifiusb->multi_port_qos_reset, hifiusb->multi_port_qos_reset_num);
}
...
static void hifi_usb_dts_config(struct hifi_usb_proxy *hifiusb)
{
	...
	hifiusb->multi_port_qos_enable = of_property_read_bool(dev->of_node, "multi-port-qos-enable");
	dbg("multi_port_qos_enable %d!\n", hifiusb->multi_port_qos_enable);
	if(hifiusb->multi_port_qos_enable) {
		ret = get_qos_ops_resource(dev, hifiusb);
		if (ret < 0)
			err("read elements form multi_port_qos failed due to %d\n", ret);
	}
}
...
static int hifi_usb_init(struct proxy_hcd_client *client)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	proxy->hifi_usb_wake_lock = wakeup_source_register(NULL, "hifi_usb_wake_lock");
	timer_setup(&proxy->confirm_udev_timer, confirm_udev_timer_fn, 0);
#else
	proxy->hifi_usb_wake_lock = wakeup_source_register("hifi_usb_wake_lock");
	...
	if (ret) {
		...
		wakeup_source_unregister(proxy->hifi_usb_wake_lock);
		put_qos_ops_resource(proxy);
		kfree(proxy);
		return ret;
	}
...
void hifi_usb_resume(struct proxy_hcd_client *client)
{
	...
	if (hifi_usb_in_hibernation(proxy)) {
		...
		if (((proxy->port_status & USB_PORT_STAT_ENABLE) == 0) || (udev && udev->reset_resume)) {
			mutex_unlock(&proxy->msg_lock);
#ifdef CONFIG_USB_HIUSB
			hiusb_wakeup_hifi_usb(proxy->current_port);
#else
