...
static int phcd_get_ep_index_from_desc(struct usb_endpoint_descriptor *desc)
{
	...
	if (usb_endpoint_xfer_control(desc)) {
		index = (desc->bEndpointAddress & 0xf) * 2; /* 2 indicates ep directions IN and OUT */
	} else {
		index = ((desc->bEndpointAddress & 0xf) * 2) + /* 2 indicates ep directions IN and OUT */
			...
			struct proxy_hcd_ep *phcd_ep, struct urb_msg *urb_msg)
{
	...
	if (urb_msg->slot_id != (__u32)(phcd->phcd_udev.udev->slot_id)) {
...
int phcd_urb_complete(struct proxy_hcd *phcd, struct urb_msg *urb_msg)
{
	...
	status = (int)le32_to_cpu(urb_msg->status);
...
static void fill_phcd_urb_comm(struct urb_msg *urb_msg, struct urb *urb)
{
	...
	urb_msg->status				= (__s32)__cpu_to_le32(urb->status);
	...
	urb_msg->interval				= (__s32)__cpu_to_le32(urb->interval);
}
...
{
	u32 status;
	unsigned int max_ports = phcd->port_count;
