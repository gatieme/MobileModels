...
{
	(void)memset_s(hifi_usb_base_quirk_devices, sizeof(hifi_usb_base_quirk_devices), 0,
			sizeof(hifi_usb_base_quirk_devices));
	return;
}
...
static bool is_usbaudio_device(struct usb_device *udev, int configuration)
{
	...
	if ((audio_intf_num == 1) && (hid_intf_num <= 1) && (other_intf_num == 0)) {
		info("this is usb audio device\n");
		return true;
	}

#if defined(CONFIG_PRODUCT_CDC) || defined(CONFIG_PRODUCT_CDC_ACE)
	if ((audio_intf_num == 1) && (hid_intf_num == 2) && (other_intf_num == 0)) {
		info("this is usb audio device for Karaoke\n");
		return true;
	}
#endif
	return false;
...
static bool is_hauwei_fs_earphone(struct usb_device *udev)
{
	if (udev->parent == NULL) {
		WARN_ON(1);
		return false;
	...
	info("idVendor 0x%x, idProduct 0x%x, speed %d\n", le16_to_cpu(udev->descriptor.idVendor),
		le16_to_cpu(udev->descriptor.idProduct), udev->speed);
	...
	if (udev->speed == USB_SPEED_FULL) {
		info("this is hauwei_fs_earphone\n");
		return true;
	}
...
bool stop_hifi_usb_when_non_usbaudio(struct usb_device *udev, int configuration)
{
	...
	if (is_non_usbaudio_device(udev, configuration)) {
		...
		if (match_reset_power_devices(udev))
			usb_stop_hifi_usb_reset_power(udev);
		else
			usb_stop_hifi_usb(udev);

		return true;
	}

	if (is_hauwei_fs_earphone(udev))
		chip_usb20_phy_calibrate();
