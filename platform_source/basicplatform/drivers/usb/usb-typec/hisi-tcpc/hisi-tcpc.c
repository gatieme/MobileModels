...
static struct tcpc_desc *desc_role_def;
struct hisi_tcpc_chip {
	...
	struct hisi_tcpc_reg *reg_info;
};

static struct hisi_tcpc_reg hiv600_regs = {
	.cdr_cfg_0 = 0x80 + 0x58,
	.dbg_cfg_1 = 0x80 + 0x5B,
	.sc_buck_en = 0x80 + 0x7E,
	.irq_gbl_msk = 0x180 + 0x48,
	.irq_msk_pd_bit = 0x1 << 2,
	.abnormal_irq_sts = 0x80 + 0x68,
	.abnormal_irq_msk = 0x180 + 0x4E,
	.cc_ovp_irq = 0x1 << 7,
	.cfg_reg_1 = 0x280 + 0x0E,
};

static struct hisi_tcpc_reg hiv700_regs = {
	.cdr_cfg_0 = 0x100 + 0x50,
	.dbg_cfg_1 = 0x100 + 0x53,
	.sc_buck_en = 0x300 + 0x33,
	.irq_gbl_msk = 0x800 + 0xD,
	.irq_msk_pd_bit = 0x1 << 1,
	.abnormal_irq_sts = 0x800 + 0xC,
	.abnormal_irq_msk = 0x800 + 0x19,
	.cc_ovp_irq = 0x1 << 2,
	.cfg_reg_1 = 0x900 + 0xB0,
	.jtag_sbu_en = 0x900 + 0xC,
	.aux_sbu_en = 0x300 + 0x3B,
};

static int hisi_tcpc_set_cc(struct tcpc_device *tcpc, int pull);

/*
 * Only for Reading TCPCI Register
 * reg: u8, [0x, 0xFF]
 */
...
/*
 * Only for Writing TCPCI Register
 * reg: u8, [0x, 0xFF]
 */
...
static void hisi_tcpc_disable_vbus(struct i2c_client *client)
{
	...
	reg |= (TCPC_V10_REG_POWER_CTRL_FORCE_DISCHARGE_DISCONNECT);
	(void)hisi_tcpc_i2c_write8(client, TCPC_V10_REG_POWER_CTRL, reg);
	mdelay(50); /* 50: mdelay time */
}

/* sourcing_vbus, ext otg */
static void hisi_tcpc_ext_otg_valid(struct hisi_tcpc_chip *chip, int valid, unsigned int msecs)
{
	u8 val = valid ? EXT_OTG_EN_BIT : 0;

	if (chip->chip_version != TCPC_CHIP_VERSION_V600 && chip->otg_vbst_ori) {
		msleep(msecs);
		(void)hisi_tcpc_i2c_write8(chip->client, PD_EXT_OTG_EN_REG, val);
	}
}
		...
		int mv, int ma)
{
	...
	if (type == TCP_VBUS_CTRL_REMOVE) {
		...
		hisi_tcpc_ext_otg_valid(chip, 0, 10);
		return 0;
	}
	...
	if (mv == TCPC_VBUS_SOURCE_0V)
		...
		hisi_tcpc_ext_otg_valid(chip, 1, 150);
...
static void hisi_tcpc_fr_swap(struct tcpc_device *tcpc, bool enable)
{
	...
	if (!get_desc_support_reverse_sc() && chip->otg_vbst_ori == 0) { /* 1: 5v vbst out */
		D("Inside Otg\n");
		return;
	}
	...
	/* FR_Swap Detect en */
	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->cfg_reg_1);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	if (enable)
		...
		rdata &= ~PD_FRS_DETECT;
	(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->cfg_reg_1, rdata);
}
...
static void hisi_tcpc_sc_buck_ctrl(struct tcpc_device *tcpc, bool en)
{
	...
	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->sc_buck_en);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	if (en)
		...
		rdata |= BIT_SC_BUCK_EN;
	(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->sc_buck_en, rdata);
}
...
static bool hisi_tcpc_chip_v610(struct tcpc_device *tcpc)
{
	...
	return chip->chip_sub_version == CHIP_VERSION_V610;
}
...
static int hisi_tcpc_get_extended_status(struct tcpc_device *tcpc, uint8_t *status)
{
	int ret;

	ret = hisi_tcpc_read8(tcpc, TCPC_V10_REG_EXTENDED_STATUS);
	if (ret < 0)
		return ret;
...
/*
 * polarity: 0 -normal, 1 -fliped, others - unattached
 * aux_sbu_en: flipped
 * jtag_sbu_en: normal
 */
int hisi_tcpc_set_dp_aux(struct tcpc_device *tcpc, uint8_t polarity)
{
	struct hisi_tcpc_chip *chip = hisi_tcpc_get_dev_data(tcpc);

	if (chip->chip_version == TCPC_CHIP_VERSION_V600) {
		I("Not charger Aux\n");
		return 0;
	}

	if (polarity == 0 || polarity == 1) {
		I("Aux polarity: %u-%s\n", polarity, polarity ? "Flipped" : "Normal");
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->aux_sbu_en, polarity ? 1 : 0);
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->jtag_sbu_en, polarity ? 0 : 1);
	} else  {
		I("disconnect\n");
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->aux_sbu_en, 0);
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->jtag_sbu_en, 0);
	}

	D("aux_sbu, readback 0x%x\n", hisi_tcpc_i2c_read8(chip->client,chip->reg_info->aux_sbu_en));
	D("jtag_sbu, readback 0x%x\n", hisi_tcpc_i2c_read8(chip->client,chip->reg_info->jtag_sbu_en));
						  ...
						  bool en)
{
	...
	int ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->cdr_cfg_0);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return;
	}
	...
	(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->cdr_cfg_0, ret);
}
...
bool get_desc_support_reverse_sc(void)
{
	if (!desc_role_def)
		return false;
	D("cur_support_reverse_sc: %d\n", desc_role_def->support_reverse_sc);
	return desc_role_def->support_reverse_sc;
}
...
static int hisi_tcpc_set_rx_enable(struct tcpc_device *tcpc, uint8_t enable)
{
	...
	if (hisi_usb_is_reverse_charge_enable() == 1 && enable == PD_RX_CAP_PE_READY_DFP) {
		enable &= ~TCPC_RX_CAP_SOP_PRIME;
		enable &= ~TCPC_RX_CAP_SOP_PRIME_PRIME;
	}
#endif
			...
			uint16_t *msg_head, enum tcpm_transmit_type *frame_type)
{
	...
	rv = hisi_tcpc_block_read(TCPC_V10_REG_RX_BYTE_CNT, 8, buf); /* 8: filp */
	if (rv) {
	...
	enum tcpm_transmit_type type, uint16_t header, const uint32_t *data)
{
	...
	if (type < TCPC_TX_HARD_RESET) {
		data_cnt = (int)sizeof(uint32_t) * PD_HEADER_CNT(header);
		packet_cnt = data_cnt + (int)sizeof(uint16_t);
...
static struct tcpc_ops hisi_tcpc_ops = {
	...
	.get_ext_status = hisi_tcpc_get_extended_status,
...
static void hisi_tcpc_wdt_rst_mask_typec(struct hisi_tcpc_chip *chip)
{
	int ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->dbg_cfg_1);
	uint8_t reg_val;
	...
	if (ret < 0) {
		...
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->dbg_cfg_1, reg_val);
	}

	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->dbg_cfg_1);
	if (ret < 0)
		 E("hisi_tcpc_i2c_read8 error again ret %d\n", ret);
}

static void hisi_tcpc_poll_ctrl(struct hisi_tcpc_chip *chip)
{
	...
	schedule_delayed_work(&chip->poll_work, msecs_to_jiffies(40)); /* 40: delayed time */
}
...
static void hisi_tcpc_cc_abnormal_irq_handler(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->abnormal_irq_sts);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return;
	}
	...
	if (reg_val & chip->reg_info->cc_ovp_irq) {
		E("CC OVP\n");
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->abnormal_irq_sts,
					   chip->reg_info->cc_ovp_irq);
...
static int hisi_tcpc_get_irq(struct hisi_tcpc_chip *chip)
{
	if (chip->irq_gpio > 0)
		return gpio_to_irq(chip->irq_gpio);
...
static void __hisi_tcpc_init_abnormal_irq(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->abnormal_irq_msk);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return;
	}
	...
	if (reg_val & chip->reg_info->cc_ovp_irq)
		reg_val &= ~chip->reg_info->cc_ovp_irq;

	(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->abnormal_irq_msk, reg_val);
}
...
static void __hisi_tcpc_init_alert(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_i2c_read8(chip->client, chip->reg_info->irq_gbl_msk);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return;
	}
	...
	if (reg_u8 & chip->reg_info->irq_msk_pd_bit) {
		reg_u8 &= ~chip->reg_info->irq_msk_pd_bit;
		(void)hisi_tcpc_i2c_write8(chip->client, chip->reg_info->irq_gbl_msk, reg_u8);
	}
}
...
static int hisi_tcpc_init_alert(struct tcpc_device *tcpc)
{
	...
	chip->irq = hisi_tcpc_get_irq(chip);

	pr_err("hisi_tcpc_init_alert chip->irq %d, gpio %d\n", chip->irq, chip->irq_gpio);

	if (chip->irq <= 0) {
		E("Fail to get irq num\n");
		return -1;
	}
	...
	chip->irq_wakelock = wakeup_source_register(chip->dev, "hisi_tcpc_irq_wakelock");

	if (chip->chip_version == TCPC_CHIP_VERSION_V700)
		ret = request_threaded_irq(chip->irq, hisi_tcpc_intr_handler, NULL,
			irq_flag | IRQF_ONESHOT,
			chip->tcpc_desc->name, chip);
	else
		ret = request_irq(chip->irq, hisi_tcpc_intr_handler, irq_flag,
			chip->tcpc_desc->name, chip);
	if (ret) {
...
static void hisi_tcpc_get_otg_vbst_type(struct hisi_tcpc_chip *chip)
{
	...
	/* Default: Inner OTG */
	if (chip->chip_version != TCPC_CHIP_VERSION_V600 && chip->otg_vbst_ori)
		(void)hisi_tcpc_i2c_write8(chip->client, PD_EXT_OTG_SEL_REG, EXT_OTG_BIT);
}
		...
		struct device *dev)
{
	...
	if (of_property_read_u32(np, "support_reverse_sc", &val) >= 0) {
		if (val) {
			desc->support_reverse_sc = true;
		} else {
			desc->support_reverse_sc = false;
		}
	}
	D("support_reverse_sc : %d\n", desc->support_reverse_sc);
...
static int hisi_tcpc_match_version(struct hisi_tcpc_chip *chip)
{
	int ret;
	struct i2c_client *client = chip->client;
	struct device_node *device = client->dev.of_node;

	ret = hisi_tcpc_i2c_read16(client, TCPC_V10_REG_VID);
	if (ret < 0) {
	...
	chip->chip_sub_version = hisi_tcpc_get_chip_version();

	if(of_device_is_compatible(device, HISI_TCPC_V600_NODE)) {
		chip->chip_version = TCPC_CHIP_VERSION_V600;
		chip->reg_info = &hiv600_regs;
		I("Chip V600");
		return 0;
	}

	if(of_device_is_compatible(device, HISI_TCPC_V700_NODE)) {
		chip->chip_version = TCPC_CHIP_VERSION_V700;
		I("Chip V700");
		chip->reg_info = &hiv700_regs;
		return 0;
	}

	E("chip node not match\n");
	return -1;
}
...
static int hisi_tcpc_probe(struct platform_device *pdev)
{
	...
	ret = hisi_tcpc_match_version(chip);
	if (ret < 0) {
		E("I2C Communicate %d\n", ret);
		goto fail_free;
	}
	...
	hisi_tcpc_wdt_rst_mask_typec(chip);
...
static int hisi_tcpc_resume(struct device *dev)
{
	...
	hisi_tcpc_init_alert_mask(chip->tcpc);
	hisi_tcpc_init_fault_mask(chip->tcpc);
 
	ret = hisi_tcpc_i2c_read8(client, chip->reg_info->irq_gbl_msk);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return 0;
	}
...
void set_desc_role_def(uint8_t tar_role)
{
	if (!desc_role_def)
		return;
	switch (tar_role) {
	case TYPEC_ROLE_SNK:
		desc_role_def->role_def = TYPEC_ROLE_SNK;
		break;
	case TYPEC_ROLE_SRC:
		desc_role_def->role_def = TYPEC_ROLE_SRC;
		break;
	case TYPEC_ROLE_DRP:
		desc_role_def->role_def = TYPEC_ROLE_DRP;
		break;
	default:
		break;
	}
