...
void hisi_rchg_timeout_alert(struct tcpc_device *tcpc);
...
#define TCPCI_SRC_VBUS_TIMEOUT_CNT 50
#define POWER_ROLE_SOURCE			0
#define POWER_ROLE_SINK				1
#define RCG_TO_RSCP					2
#define RSCP_TO_RCG					3
#define BCG_TO_RSCP					4
#define RSCP_TO_BCG					5
#define BCG_TO_RCG					6
#define RCG_TO_BCG					7
#define PR_SINK_NODE				0
#define PR_SOURCE_NODE_NONE			1
#define PR_SOURCE_NODE_REVER		2
#define CC_UNATTACH_SCOPE			3

static void tcpci_wait_source_vbus(struct tcpc_device *tcpc, bool expect)
{
...
static inline void tcpci_set_cc(struct tcpc_device *tcpc, int pull)
{
	...
	tcpc->typec_local_cc = (uint8_t)pull;
		...
		struct tcpc_device *tcpc, int power_role, int data_role)
{
	...
	uint8_t pd_rev = tcpc->pd_port.pd_revision[0];
#else
...
/*
 * Both CC and Vbus unattached, sinking vbus(mv != 0) should bypassed
 */
static inline int tcpci_need_sink_vbus(struct tcpc_device *tcpc, int mv)
{
	uint16_t pwr_status = 0;

	(void)tcpci_get_power_status(tcpc, &pwr_status);
	(void)tcpci_get_cc(tcpc);

	if ((tcpc->typec_remote_cc[0] == 0) && (tcpc->typec_remote_cc[1] == 0) &&
		((pwr_status & TCPC_REG_POWER_STATUS_VBUS_PRES) == 0) && mv != 0) {
			I("cc&vbus both unattach, bypass sink vbus\n");
		return 0;
	}
		...
		uint8_t type, int mv, int ma)
{
	...
	I("sink vbus: %d--%d\n", mv, ma);
	if (!tcpci_need_sink_vbus(tcpc, mv))
		return 0;
...
static inline int tcpci_notify_adapter_status(struct tcpc_device *tcpc, int state)
{
	struct tcp_adapter_status status;

	status.adapter_cc_without_vbus = state;
	srcu_notifier_call_chain(&tcpc->evt_nh,
			TCP_NOTIFY_ADAPTER_ABNORMAL, &status);
		...
		struct tcp_ny_cable_vdo cable_vdo)
{
	...
	tcp_noti.cable_vdo.vdo = cable_vdo.vdo;
	tcp_noti.cable_vdo.vdo_ext = cable_vdo.vdo_ext;
		...
		uint32_t dp_config)
{
	...
	hisi_tcpc_set_dp_aux(tcpc, tcpc->typec_polarity);
...
static inline int tcpci_notify_uvdm(struct tcpc_device *tcpc, bool ack)
{
	...
	if (ack) {
		if (pd_port->uvdm_cnt > PD_DATA_OBJ_SIZE)
			pd_port->uvdm_cnt  = PD_DATA_OBJ_SIZE;
		tcp_noti.uvdm_msg.uvdm_cnt = pd_port->uvdm_cnt;
		tcp_noti.uvdm_msg.uvdm_svid = pd_port->uvdm_svid;
		for (i = 0; i < tcp_noti.uvdm_msg.uvdm_cnt; i++)
			tcp_noti.uvdm_msg.uvdm_data[i] = pd_port->uvdm_data[i];
	}
...
static inline int tcpci_notify_alert(struct tcpc_device *tcpc, uint32_t ado)
{
	struct tcp_notify tcp_noti;

	tcp_noti.alert_msg.ado = ado;
	srcu_notifier_call_chain(&tcpc->evt_nh, TCP_NOTIFY_ALERT, &tcp_noti);
	return 0;
}
#endif
