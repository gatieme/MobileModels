...
#define SRC_PDO_ONE                   1
#define SRC_PDO_TWO                   2
#define SRC_CAP_HIGH_VOL              9000
#define SRC_CAP_HIGH_CUR              2000
#endif
...
#ifdef CONFIG_DUAL_ROLE_USB_INTF
		if (get_desc_support_reverse_sc() && hisi_usb_is_reverse_charge_enable() == 1) {
...
#ifdef CONFIG_DUAL_ROLE_USB_INTF
	if ((pd_port->ext_src_cap.otg_vol == SRC_CAP_HIGH_VOL &&
		pd_port->ext_src_cap.otg_cur == SRC_CAP_HIGH_CUR) ||
		hisi_usb_is_reverse_charge_enable()) {
			src_cap1->nr = SRC_PDO_TWO;
	} else {
		src_cap1->nr = SRC_PDO_ONE;
	}
#endif
...
void hisi_pd_dpm_src_evaluate_request(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (accept_request) {
		if (get_desc_support_reverse_sc()) {
			I("set pd_negotiate_flag\n");
			pd_port->pd_negotiate_flag = true;
		}
...
{
	D("\n");
	pd_send_uvdm(pd_port, pd_port->uvdm_sop_type);
		...
		pd_event_t *pd_event, bool ack)
{
	...
	pd_port->dpm_flags &= ~(DPM_FLAGS_SOP_PRIME_UVDM);
		...
		pd_event_t *pd_event)
{
	...
	if (pd_port->dpm_flags & DPM_FLAGS_SOP_PRIME_UVDM) {
		D("send sop prime uvdm\n");
		if (vdm_put_dpm_vdm_request_event(pd_port,
