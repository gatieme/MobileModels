...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
static bool tcpci_check_hard_reset_complete(
...
int hisi_tcpci_alert(struct tcpc_device *tcpc_dev)
{
	...
	if (ret == HISI_TCPC_ALERT_STATUS_ZERO ||
		...
		if (tcpc_dev->cnt >= HISI_TCPC_OTHER_IRQ_MAX_LOOPS) {
...
static int __tcpci_set_wake_lock_pd(struct tcpc_device *tcpc,
...
static int tcpci_report_usb_port_attached(struct tcpc_device *tcpc)
{
	I("usb_port_attached\n");

	if (tcpc->typec_attach_new != TYPEC_ATTACHED_AUDIO && !tcpc->pd_port.called_by_timeout)
		tcpci_set_wake_lock_pd(tcpc, true);

#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	pd_put_cc_attached_event(tcpc, tcpc->typec_attach_new);
	if(tcpc->pd_port.called_by_timeout)
		tcpc->pd_port.called_by_timeout = false;
#endif

	return 0;
}

static int tcpci_report_usb_port_detached(struct tcpc_device *tcpc)
{
	I("usb_port_detached\n");
	if (get_desc_support_reverse_sc())
		set_desc_role_def(TYPEC_ROLE_DRP);
...
static void report_usb_port_changed_handle_unattached(struct tcpc_device *tcpc)
{
#ifdef CONFIG_DUAL_ROLE_USB_INTF
	int transfer_path = tcpc->pd_port.state_transfer_path;
	if (!get_desc_support_reverse_sc() || (transfer_path == 0 || tcpc->pd_port.prev_path == transfer_path ||
		transfer_path == RCG_TO_BCG || transfer_path == BCG_TO_RCG)) {
		...
		if (get_desc_support_reverse_sc() &&
			(transfer_path == BCG_TO_RSCP || transfer_path == RCG_TO_RSCP))
			tcpc->dual_role_mode = DUAL_ROLE_PROP_MODE_UFP;
		}
	}
#endif
...
#endif
		report_usb_port_changed_handle_unattached(tcpc);
		tcpc->pd_port.prev_path = transfer_path;
		break;
...
{
	tcpc->pd_port.called_by_timeout = true;
	hisi_tcpci_report_usb_port_changed(tcpc);
}
