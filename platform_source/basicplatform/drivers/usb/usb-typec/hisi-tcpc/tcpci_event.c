...
{
	if (PD_HEADER_TYPE(header) < ARRAY_SIZE(pd_ctrl_msg_name))
...
static inline void print_received_ext_msg(pd_msg_t *pd_msg, uint16_t header)
{
	if(PD_HEADER_TYPE(header) < ARRAY_SIZE(pd_ext_msg_name))
		I("ext msg: %s\n", pd_ext_msg_name[PD_HEADER_TYPE(header)]);
	else
		I("ext msg: 0x%x\n", PD_HEADER_TYPE(header));
}
#else
...
static void print_received_data_msg(pd_msg_t *pd_msg, uint16_t header)
{
	...
	if(PD_HEADER_TYPE(header) < ARRAY_SIZE(pd_data_msg_name))
	...
	if (PD_HEADER_TYPE(header) == PD_DATA_VENDOR_DEF) {
		...
		if (PD_VDO_SVDM(vdm_hdr))
			D("version %u, Object Position %u, Command Type %s, Command %d(%s)\n",
				...
				vdm_cmd,
				(vdm_cmd < 7) ? vdm_cmd_name[(int)vdm_cmd] : /* (vdm_cmd < 7): vdm_cmd */
...
static void print_received_msg(pd_msg_t *pd_msg)
{
	...
	else if (PD_HEADER_EXT(pd_msg->msg_hdr))
		print_received_ext_msg(pd_msg, header);
	else
		...
		pd_event_t *pd_event)
{
	...
	*pd_event =
	...
	tcpc_dev->pd_event_head_index = (uint8_t)index;
...
bool hisi_usb_get_pd_neg_event(struct tcpc_device *tcpc_dev)
{
	...
	return tcpc_dev->pd_port.pd_negotiate_flag;
}
...
void hisi_usb_set_pd_neg_event(struct tcpc_device *tcpc_dev, bool flag)
{
	...
	tcpc_dev->pd_port.pd_negotiate_flag = flag;
}
...
static bool reset_pe_vdm_state(struct tcpc_device *tcpc_dev,
		uint32_t vdm_hdr)
{
...
static bool __pd_get_deferred_tcp_event(struct tcpc_device *tcpc_dev,
...
static bool __pd_put_hw_event(struct tcpc_device *tcpc_dev,
		uint8_t hw_event)
{
	pd_event_t evt = {
		.event_type = PD_EVT_HW_MSG,
...
static bool __pd_put_pe_event(struct tcpc_device *tcpc_dev,
		uint8_t pe_event)
{
	pd_event_t evt = {
		.event_type = PD_EVT_PE_MSG,
...
static int tcpc_event_thread(void *param)
{
	...
	struct sched_param sch_param = {.sched_priority = MAX_RT_PRIO - 2}; /* 2: set Priority */
...
int hisi_tcpci_event_init(struct tcpc_device *tcpc_dev)
{
	...
	tcpc_dev->tcpci_event_wake_lock = wakeup_source_register(&tcpc_dev->dev, "tcpci_event");
