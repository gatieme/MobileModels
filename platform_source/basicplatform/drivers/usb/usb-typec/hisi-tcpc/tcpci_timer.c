...
static const char * const hisi_tcpc_timer_name[] = {
	...
	"TYPEC_RCHG_UNATTACHED_EVENT",
#ifdef CONFIG_HISI_ADAPTER_DETECT
	"TYPEC_TIMER_CC_WAITFOR_VBUS",
#endif
...
static const uint32_t tcpc_timer_timeout[PD_TIMER_NR] = {
	...
	TIMEOUT_VAL(2200), /* TYPEC_RCHG_UNATTACHED_EVENT */
#ifdef CONFIG_HISI_ADAPTER_DETECT
	TIMEOUT_VAL(300), /* TYPEC_TIMER_CC_WAITFOR_VBUS */
#endif
...
static enum hrtimer_restart tcpc_timer_rchg_unattached_event(struct hrtimer *timer)
{
	int index = TYPEC_RCHG_UNATTACHED_EVENT;
	struct tcpc_device *tcpc_dev = tcpc_hrtimer_to_dev(timer, index);
...
#ifdef CONFIG_HISI_ADAPTER_DETECT
static enum hrtimer_restart tcpc_timer_wait_vbus(struct hrtimer *timer)
{
	int index = TYPEC_TIMER_CC_WAITFOR_VBUS;
	struct tcpc_device *tcpc_dev = tcpc_hrtimer_to_dev(timer, index);
...
static tcpc_hrtimer_call tcpc_timer_call[PD_TIMER_NR] = {
	...
	[TYPEC_RCHG_UNATTACHED_EVENT] = tcpc_timer_rchg_unattached_event,

#ifdef CONFIG_HISI_ADAPTER_DETECT
	[TYPEC_TIMER_CC_WAITFOR_VBUS] = tcpc_timer_wait_vbus,
#endif
