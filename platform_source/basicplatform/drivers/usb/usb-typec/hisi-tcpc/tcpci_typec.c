...
static int typec_norp_src_attached_entry(struct tcpc_device *tcpc_dev)
{
	uint16_t power_status = 0;

	if (tcpci_get_power_status(tcpc_dev, &power_status))
		return 0;
...
static void typec_custom_src_attached_entry(struct tcpc_device *tcpc_dev)
{
	...
	if (cc1 == TYPEC_CC_VOLT_SNK_DFT && cc2 == TYPEC_CC_VOLT_SNK_DFT) {
		D("Custom_src attached\n");
		typec_set_plug_orient(tcpc_dev, TYPEC_CC_RD,
			typec_check_cc1(TYPEC_CC_VOLT_OPEN));
...
static void typec_cc_src_detect_vsafe0v_entry(struct tcpc_device *tcpc_dev)
{
	...
	if (get_desc_support_reverse_sc()) {
		if (tcpc_dev->pd_port.state_transfer_path != RSCP_TO_BCG) {
...
static bool typec_cc_change_sink_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpci_get_power_status(tcpc_dev, &pwr_status);
	hisi_tcpci_vbus_level_init(tcpc_dev, pwr_status);
	vbus_valid = tcpci_check_vbus_valid(tcpc_dev);

	switch (tcpc_dev->typec_state) {
	case typec_attached_snk:
		if ((typec_get_cc_res() == TYPEC_CC_VOLT_OPEN) && !vbus_valid)
			snk_remove = true;
		else
...
#ifdef CONFIG_TYPEC_CAP_CUSTOM_SRC_SUPPORT
	case typec_attached_custom_src:
		if (typec_check_cc_any(TYPEC_CC_VOLT_OPEN)  && !vbus_valid)
			snk_remove = true;
		break;
#endif
...
static void typec_attach_wait_entry(struct tcpc_device *tcpc_dev)
{
	...
	if (as_sink && hisi_usb_typec_support_adapter_detect() && (!tcpci_check_vbus_valid(tcpc_dev)))
		hisi_tcpc_enable_timer(tcpc_dev, TYPEC_TIMER_CC_WAITFOR_VBUS);
#endif
}
...
static int typec_attached_snk_cc_detach(struct tcpc_device *tcpc_dev)
{
	...
	tcpci_get_power_status(tcpc_dev, &pw_status);
	D("Power register: 0x%x\n", pw_status);
...
static void typec_detach_wait_entry(struct tcpc_device *tcpc_dev)
{
	...
	case typec_attached_src:
		I("Exit Attached.SRC immediately\n");
		hisi_tcpc_reset_typec_debounce_timer(tcpc_dev);
		...
		uint32_t timer_id)
{
	...
	case TYPEC_TIMER_PDDEBOUNCE:
		...
		break;
	case TYPEC_RCHG_UNATTACHED_EVENT:
		if (tcpc_dev->typec_attach_new == 0) {
			tcpc_dev->pd_port.curr_rever_flag = false;
			tcpc_dev->pd_port.prev_rever_flag = false;
			tcpc_dev->pd_port.pd_curr_unattach = false;
			tcpc_dev->pd_port.state_transfer_path = 0;
			D("Unattach timer timeout clear all flag\n");
			hisi_rchg_timeout_alert(tcpc_dev);
		}
		break;

#ifdef CONFIG_HISI_ADAPTER_DETECT
	case TYPEC_TIMER_CC_WAITFOR_VBUS:
		I("abnormal, adapter pull cc without vbus\n");
		tcpci_notify_adapter_status(tcpc_dev, 1);
		break;
#endif
...
static int typec_attached_snk_vbus_absent(struct tcpc_device *tcpc_dev)
{
	...
	if (tcpc_dev->typec_during_direct_charge &&
		...
		D("Ignore vbus_absent(snk), DirectCharge\n");
		return 0;
	}
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
	if (tcpc_dev->pd_wait_hard_reset_complete ||
		...
		if (typec_get_cc_res() != TYPEC_CC_VOLT_OPEN) {
			D("Ignore vbus_absent(snk), HReset & CC!=0\n");
			return 0;
		}
	}
#endif
...
#ifdef CONFIG_HISI_ADAPTER_DETECT
	if (hisi_tcpc_timer_enabled(tcpc_dev, TYPEC_TIMER_CC_WAITFOR_VBUS))
		hisi_tcpc_disable_timer(tcpc_dev, TYPEC_TIMER_CC_WAITFOR_VBUS);
#endif
		...
		uint8_t typec_role)
{
	...
	if (typec_role == TYPEC_ROLE_UNKNOWN || typec_role >= TYPEC_ROLE_NR) {
		I("Wrong TypeC-Role: %d\n", typec_role);
		return -1;
	}

	if (typec_role == TYPEC_ROLE_TRY_SRC) {
		I("Try.SRC is not supported\n");
		return -1;
	}
	...
	if (get_desc_support_reverse_sc() || tcpc_dev->typec_attach_new == TYPEC_UNATTACHED) {
		switch (tcpc_dev->typec_role) {
		case TYPEC_ROLE_SNK:
			tcpci_set_cc(tcpc_dev, TYPEC_CC_RD);
			break;
...
{
	if (!tcpc_dev || !allow_ext)
		return;

	if (tcpc_dev->pd_port.ext_src_cap.otg_vol == allow_ext->otg_vol) {
		D("aleardy set ext_cap\n");
		return;
	}

	D("set src cap, vol=%d, cur=%d, send=%d\n",
		allow_ext->otg_vol, allow_ext->otg_cur, send);
	tcpc_dev->pd_port.ext_src_cap.otg_vol = allow_ext->otg_vol;
	tcpc_dev->pd_port.ext_src_cap.otg_cur = allow_ext->otg_cur;

	if (send && !tcpc_dev->pd_port.curr_rever_flag) {
		D("update srccap 1\n");
		tcpc_dev->pd_port.pe_pd_state = PE_SRC_SEND_CAPABILITIES; 
		hisi_pd_dpm_send_source_caps(&tcpc_dev->pd_port);
	}
}
...
int hisi_usb_get_src_extcap(struct tcpc_device *tcpc_dev)
{
	...
	return (tcpc_dev->pd_port.local_src_cap_default.nr > 1) ? 1 : 0;
}
...
{
	if (get_desc_support_reverse_sc())
		hisi_tcpc_enable_timer(tcpc, TYPEC_RCHG_UNATTACHED_EVENT);
}
