...
#ifdef CONFIG_USB_POWER_DELIVERY_SUPPORT
bool hisi_tcpm_during_hardreset(struct tcpc_device *tcpc)
{
	return tcpc->pd_wait_hard_reset_complete;
}
...
int hisi_tcpm_put_cc_attach_event(struct tcpc_device *tcpc)
{
...
bool hisi_tcpm_usb_comm(struct tcpc_device *tcpc_dev)
{
	return tcpc_dev->pd_port.dpm_flags & DPM_FLAGS_PARTNER_USB_COMM;
}
...
int hisi_tcpm_send_uvdm(struct tcpc_device *tcpc_dev,
		uint8_t cnt, uint32_t *data, bool wait_resp, int *sop_type)
{
	bool ret = false;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	...
	pd_port->uvdm_sop_type = TCPC_TX_SOP;
	if (sop_type) {
		pd_port->uvdm_sop_type = *sop_type;
