...
static void hisi_usb_typec_state_change(struct tcp_ny_typec_state *typec_state)
{
	...
	int ret;
#ifdef CONFIG_DUAL_ROLE_USB_INTF
	if (get_pd_unattach_state(typec->tcpc_dev) && typec_state->new_state == TYPEC_UNATTACHED) {
...
bool hisi_usb_typec_charger_type_pd(void)
{
	...
	if (typec->adapter_ops && typec->adapter_ops->charger_type_pd)
		return typec->adapter_ops->charger_type_pd();
...
bool hisi_usb_pd_usb_comm(void *client)
{
	struct usb_typec *typec = _typec;
...
static int __tcpc_notifier_work(struct tcpc_notify *noti)
{
	...
	struct tcp_ny_uvdm uvdm_msg = {0};
#ifdef CONFIG_HISI_ADAPTER_DETECT
	struct tcp_adapter_status status = {0};
#endif
	int i;
	...
	case TCP_NOTIFY_UVDM:
		uvdm_msg.ack = noti->notify.uvdm_msg.ack;
		uvdm_msg.uvdm_svid = noti->notify.uvdm_msg.uvdm_svid;
		uvdm_msg.uvdm_cnt = noti->notify.uvdm_msg.uvdm_cnt;
		for (i = 0; i < uvdm_msg.uvdm_cnt; i++)
			uvdm_msg.uvdm_data[i] = noti->notify.uvdm_msg.uvdm_data[i];

		ret = hisi_usb_typec_handle_pe_event(typec, TCP_NOTIFY_UVDM,
				(void *)&uvdm_msg);
		break;

#ifdef CONFIG_HISI_ADAPTER_DETECT
	case TCP_NOTIFY_ADAPTER_ABNORMAL:
		status.adapter_cc_without_vbus = noti->notify.adapter_status.adapter_cc_without_vbus;
		ret = hisi_usb_typec_handle_pe_event(typec, TCP_NOTIFY_ADAPTER_ABNORMAL, (void *)&status);
		break;
#endif
...
void hisi_usb_rpd_update_src_cap(void *client, struct otg_cap *allow_ext, int send)
{
	...
	if (!typec || !typec->tcpc_dev || !allow_ext)
		return;
