...
/*
 * support usb communuication -- true: y, false: n
 */
bool oem_pd_usb_comm(void *client)
{
	return hisi_usb_pd_usb_comm(client);
}
...
static struct pd_dpm_ops hisi_device_pd_dpm_ops = {
	...
	.pd_dpm_inquire_usb_comm = oem_pd_usb_comm,
...
static int oem_typec_handle_pe_event(uint32_t evt_type, void *state)
{
	...
	struct tcp_ny_uvdm *uvdm_msg = NULL;
	struct pd_dpm_cable_data dpm_uvdm_data;
#ifdef CONFIG_HISI_ADAPTER_DETECT
	struct tcp_adapter_status *adapter_status = NULL;
	struct pd_dpm_adapter_data adapter_data;
#endif
	int i;
	...
	case TCP_NOTIFY_UVDM:
		uvdm_msg = (struct tcp_ny_uvdm *)state;
		dpm_uvdm_data.cnt = uvdm_msg->uvdm_cnt;
		for (i = 0; i < uvdm_msg->uvdm_cnt; i++)
			dpm_uvdm_data.msg[i] = uvdm_msg->uvdm_data[i];
	...
	case TCP_NOTIFY_ADAPTER_ABNORMAL:
		adapter_status = (struct tcp_adapter_status *)state;
		adapter_data.status = adapter_status->adapter_cc_without_vbus;
