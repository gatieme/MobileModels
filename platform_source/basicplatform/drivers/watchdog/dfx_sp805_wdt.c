...
#define PR_LOG_TAG SP805_WDT_TAG
...
#define DFX_WDT_AMBA_ID        0x00141805
#define DFX_WDT_AMBA_MASK      0x00ffffff
...
void wdt_fast_reset(void)
{
	...
	writel_relaxed(INT_RESET_DISABLE, wdt_base + WDTCONTROL);
	writel_relaxed(WDT_TIMEOUT_KICK, wdt_base + WDTLOAD);
	writel_relaxed(INT_ENABLE | RESET_ENABLE, wdt_base + WDTCONTROL);
...
static void dfx_wdt_mond(struct work_struct *work)
{
	...
	if (work == NULL)
		return;
	wdt = container_of(work, struct sp805_wdt, dfx_wdt_delayed_work.work);
#ifdef CONFIG_DFX_AGING_WDT
