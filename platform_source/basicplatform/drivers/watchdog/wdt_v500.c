 ...
 /*
 *
 * Watchdog driver for V500 watchdog module
 *
...
/* lpm3 message length */
#define M3_MSG_LEN 2
#undef CONFIG_DFX_AGING_WDT
#ifdef CONFIG_DFX_AGING_WDT
...
void wdt_fast_reset(void)
{
	if (wdt_base == NULL)
		return;

	writel_relaxed(LOCKEN, wdt_base + WDLOCKEN);
	writel_relaxed(INT_MASK, wdt_base + WDINTCLR);
	writel_relaxed(WDCNT_DISABLE, wdt_base + WDCNTEN);
	writel_relaxed(WDRST_DISABLE, wdt_base + WDRSTMASK);
	writel_relaxed(WDINT_DISABLE, wdt_base + WDINTMASK);
	writel_relaxed(WDT_TIMEOUT_KICK, wdt_base + WDINTRTIME);
	writel_relaxed(WDT_TIMEOUT_KICK, wdt_base + WDRSTTIME);
	writel_relaxed(WDINT_ENABLE, wdt_base + WDINTMASK);
	writel_relaxed(WDRST_ENABLE, wdt_base + WDRSTMASK);
	writel_relaxed(WDCNT_ENABLE, wdt_base + WDCNTEN);
	writel_relaxed(LOCKDIS, wdt_base + WDLOCKDIS);
}
...
static BLOCKING_NOTIFIER_HEAD(wdt_notifier_list);
void wdt_register_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_register(&wdt_notifier_list, nb);
}

void wdt_unregister_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_unregister(&wdt_notifier_list, nb);
}
...
static void bind_core_handler(void *info)
{
	struct arm_smccc_res res;
	int *cpu_id = info;

	arm_smccc_smc(MNTN_WDT_FIQ_BIND_CORE, 0, 0, 0, 0, 0, 0, 0, &res);

	if (res.a0 == (u64)(*cpu_id))
		pr_crit("watchdog fiq has been bind to core %lu\n", res.a0);
}

static void watchdog_fiq_bind_core(int cpu_id)
{
	static int bind_coreid = WDT_FIQ_NOT_BINDED;

	if (bind_coreid != cpu_id) {
		smp_call_function_single(cpu_id, bind_core_handler, (void *)&cpu_id, 1);
		bind_coreid = cpu_id;
	}
}
 ...
 * \mntn\blackbox\platform_lpm3\rdr_lpm3.c
...
#define PSCI_MSG_TYPE_M3_WDTTIMEOUT IPC_CMD(OBJ_AP, OBJ_LPM3, CMD_INQUIRY, 1)
	...
	msg[1] = (int)default_timeout;
	ret = RPROC_ASYNC_SEND(IPC_ACPU_LPM3_MBX_5, (mbox_msg_t *)msg, M3_MSG_LEN);
	if (ret != 0)
...
static int hiv500_wdt_get_timeout(struct platform_device *pdev, unsigned int *default_timeout)
{
	...
	np = of_find_compatible_node(NULL, NULL, "hisilicon,wdtv500");
	if (IS_ERR(np)) {
...
static int hiv500_wdt_probe(struct platform_device *pdev)
{
	...
	if (check_mntn_switch(MNTN_AP_WDT) == 0) {
...
static int hiv500_wdt_suspend(void)
{
	...
	if (watchdog_active(&wdt->wdd) || wdt->active) {
		cancel_delayed_work(&wdt->dfx_wdt_delayed_work);
#ifdef CONFIG_DFX_SR_AP_WATCHDOG
/*
