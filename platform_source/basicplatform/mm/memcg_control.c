...
/*
 * shrink half of slabs by set prio to 2
 */
u64 zswapd_shrink_slab_prio = 2;

static int kswapd_track_load_policy;
static int kswapd_high_load_swappiness = 100;
...
#endif
atomic64_t file_active_inactive_ratio = ATOMIC64_INIT(0);
/*
...
static ssize_t shrink_sys_anon_param_write(struct kernfs_open_file *of,
				char *buf, size_t nbytes, loff_t off)
{
	unsigned long set_shrink_sys_thre_sz;
	unsigned long set_shrink_root_thre_sz;
	unsigned long set_shrink_sys_tgt_sz;
	unsigned long set_shrink_sys_skip_sz;
	unsigned int set_emergency_swap_ratio;
	const unsigned int params_num = 5;

	buf = strstrip(buf);
	if (sscanf_s(buf, "%lu %lu %lu %lu %u",
		...
		char *buf, size_t nbytes, loff_t off)
{
	...
	for_each_online_node(nid) {
		pgdat = NODE_DATA(nid);
		if (prelaunch)
			reclaimed += reclaim_all_anon_memcg_prelaunch(pgdat, memcg);
		else
			reclaimed += reclaim_all_anon_memcg(pgdat, memcg);
	}
	count_vm_event(FREEZE_RECLAIM_TIMES);
	count_vm_events(FREEZE_RECLAIMED, reclaimed);
...
static int memcg_total_info_per_app_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		anon_size = get_memcg_anon_pages(memcg);
...
#ifdef CONFIG_RAMTURBO
static ssize_t mem_cgroup_force_shrink_all_by_reclaim(
		...
		char *buf, size_t nbytes, loff_t off)
{
	...
	while ((task = css_task_iter_next(&it))) {
		if (task->pid == task->tgid) {
			...
			single_reclaimed_pages =
				all_active_reclaim_by_task(task);
			put_task_struct(task);
			iter_end = jiffies;

			pr_debug("RAMTURBO: %s task=%s(pid=%d) reclaimed %lupages(%luBytes), takes %lums",
	...
	pr_debug("RAMTURBO: %s memcg=%s, totally reclaimed %lupages(%luBytes), takes %lums",
		__func__, memcg->name, total_reclaimed_pages,
		total_reclaimed_pages * PAGE_SIZE,
		...
		char *buf, size_t nbytes, loff_t off)
{
	...
	if (active_ratio > MAX_RATIO) {
		pr_err("RAMTURBO: invalid parameter %s active_ratio %u!", __func__, active_ratio);
		return -EINVAL;
	}
	...
	for_each_online_node(nid) {
		...
		pgdat = NODE_DATA(nid);
		single_reclaimed_pages =
			all_active_reclaim_by_memcg(pgdat, memcg, active_ratio);
		iter_end = jiffies;

		pr_debug("RAMTURBO: %s nid=%d reclaimed %lupages(%luBytes), takes %lums",
			__func__, nid, single_reclaimed_pages,
			single_reclaimed_pages * PAGE_SIZE,
			jiffies_to_msecs(iter_end - iter_start));
		total_reclaimed_pages += single_reclaimed_pages;
	}
	...
	count_vm_event(FREEZE_RECLAIM_TIMES);
	count_vm_events(FREEZE_RECLAIMED, total_reclaimed_pages);
					...
					char *buf, size_t nbytes, loff_t off)
{
	...
	pr_debug("RAMTURBO: swapout_bg_tasks: input args: type=%u, param=%u",
		swapout_type, swapout_param);
	hyperhold_memcg_active_swapout_bg_tasks(memcg,
		swapout_type, swapout_param);
	return nbytes;
}
...
u64 get_file_active_inactive_ratio(void)
{
	return atomic64_read(&file_active_inactive_ratio);
}

static int file_active_inactive_ratio_write(struct cgroup_subsys_state *css,
				struct cftype *cft, u64 val)
{
	atomic64_set(&file_active_inactive_ratio, val);

	return 0;
}

static u64 file_active_inactive_ratio_read(struct cgroup_subsys_state *css,
				struct cftype *cft)
{
	return atomic64_read(&file_active_inactive_ratio);
}
...
#ifdef CONFIG_RAMTURBO
static s64 kswapd_pid_read(struct cgroup_subsys_state *css, struct cftype *cft)
{
	return (s64)get_kswapd_pid();
}
#endif
...
{
	return zswapd_shrink_slab_prio;
}
				...
				char *buf, size_t nbytes, loff_t off)
{
	...
	avail_buffers_value /= MAX_NUMNODES;
	min_avail_buffers_value /= MAX_NUMNODES;
	high_avail_buffers_value /= MAX_NUMNODES;
...
#define ZSWAPD_SHRINK_SALB_MAX_PRIORITY 100
static int zswapd_shrink_slab_prio_write(struct cgroup_subsys_state *css,
	struct cftype *cft, u64 val)
{
	if (val > ZSWAPD_SHRINK_SALB_MAX_PRIORITY)
		return -EINVAL;

	zswapd_shrink_slab_prio = val;
	return 0;
}
...
static int report_app_info_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		seq_printf(m, "%s,%llu,%lu,%lu,%lu,%lu,%lu,%lu,%lu\n",
			...
			anon_size, zram_size, eswap_size,
			get_memcg_zram_swapout_cnt(memcg),
			get_memcg_zram_swapin_cnt(memcg),
...
#ifdef CONFIG_MM_AMA
	seq_printf(m, "kshrinkd_run:%lu\n", vm_buf[KSHRINKD_RUN]);
	seq_printf(m, "kshrinkd_wake_up:%lu\n", vm_buf[KSHRINKD_WAKEUP]);
	seq_printf(m, "kshrinkd_failed:%lu\n", vm_buf[KSHRINKD_FAIL]);
	seq_printf(m, "kshrinkd_skip:%lu\n", vm_buf[KSHRINKD_SKIP]);
	seq_printf(m, "kshrinkd_low:%lu\n", vm_buf[KSHRINKD_LOW]);
	seq_printf(m, "kshrinkd_retry:%lu\n", vm_buf[KSHRINKD_RETRY]);
	seq_printf(m, "kshrinkd_rec_anon:%lu\n", vm_buf[KSHRINKD_RECLAIMED_ANON]);
	seq_printf(m, "kshrinkd_rec_file:%lu\n", vm_buf[KSHRINKD_RECLAIMED_FILE]);
	seq_printf(m, "kshrinkd_scanned_anon:%lu\n", vm_buf[KSHRINKD_SCAN_ANON]);
	seq_printf(m, "kshrinkd_scanned_file:%lu\n", vm_buf[KSHRINKD_SCAN_FILE]);
#endif
	seq_printf(m, "freeze_wake_up:%lu\n", vm_buf[FREEZE_RECLAIM_TIMES]);
	seq_printf(m, "freeze_reclaimed:%lu\n", vm_buf[FREEZE_RECLAIMED]);
	seq_printf(m, "shrink_sys_anon:%lu\n", vm_buf[SHRINK_SYS_ANON]);
	seq_printf(m, "shrink_sys_anon_pages:%lu\n", vm_buf[SHRINK_SYS_ANON_PAGES]);
...
static u64 zswapd_shrink_slab_prio_read(struct cgroup_subsys_state *css,
				struct cftype *cft)
{
	return zswapd_shrink_slab_prio;
}
...
void memcg_eswap_info_show(struct seq_file *m)
{
	...
	anon = get_memcg_anon_pages(memcg);
	file = get_memcg_file_pages(memcg);
...
static struct cftype memcg_policy_files[] = {
	...
	{
		.name = "zswapd_shrink_slab_prio",
		.flags = CFTYPE_ONLY_ON_ROOT,
		.write_u64 = zswapd_shrink_slab_prio_write,
#ifdef CONFIG_HYPERHOLD_DEBUG
		.read_u64 = zswapd_shrink_slab_prio_read,
#endif
	},
	{
...
#endif
	{
		.name = "file_active_inactive_ratio",
		.flags = CFTYPE_ONLY_ON_ROOT,
		.write_u64 = file_active_inactive_ratio_write,
		.read_u64 = file_active_inactive_ratio_read,
	},
	{ }, /* terminate */
};
