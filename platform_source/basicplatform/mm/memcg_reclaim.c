...
static bool is_kswapd_high_load(void)
{
	...
	if (policy <= 0 || policy > TRACK_ALL_CPU)
		return false;

	if (policy == TRACK_BIG_CPU)
		fit_cpu = is_max_cap_orig_cpu(smp_processor_id());
	else if (policy == TRACK_FAST_CPU)
		fit_cpu = (test_fast_cpu(smp_processor_id()) == 1);

	return fit_cpu && get_task_util(current) > get_kswapd_high_load_threshold();
}
...
static void parak_shrink_memcg(struct mm_parad_entry *entry)
{
	...
	unsigned long nr_scanned, nr_reclaimed, nr_reclaimed_anon,
		isolate_count, nr_scanned_file, nr_scanned_anon,
...
unsigned int calc_sys_cur_avail_buffers(int nid)
{
	const unsigned int percent_constant = 100;
...
{
	u32 curr_buffers = calc_sys_cur_avail_buffers(nid);
...
static inline bool high_buffer_is_suitable(nid)
{
...
int parad_task_create(void)
{
	...
	for (i = 0; i < PARAD_TASK_MAX; i++) {
		...
		parad_task->task = kthread_create(
			parad_thread, parad_task, "para_zswapd%u", i);
		if (IS_ERR(parad_task->task)) {
			pr_err("failed to start para_zswapd%u\n", i);
