...
#define VIP_PAGE_NUM_MAX 12500

static atomic_t mm_vip_scene = ATOMIC_INIT(0);
int mm_vip_pool_pages = 0;
atomic_t mmvip_page_num = ATOMIC_INIT(0);
...
/*
 * vip 2.0 scene include : apps in the foreground
 */
static bool is_mm_vip2_scene(void)
{
...
void set_gfp_vip(gfp_t *gfp_mask)
{
	...
	if (mm_vip_pool_pages == 0)
		return;

	bool vip2_scene = is_mm_vip2_scene();
	if (vip2_scene || (atomic_read(&mm_vip_scene) && current->mm && current->mm->mm_vip_tag))
		*gfp_mask |= ___GFP_VIP;
	else if (!vip2_scene && !atomic_read(&mm_vip_scene) && (*gfp_mask & ___GFP_VIP))
		*gfp_mask &= ~___GFP_VIP;
}
...
bool migrate_vip_check(struct page *page)
{
	...
	if (!current_is_kswapd() && (current->pid != get_zswapd_pid()) && !is_mm_vip2_scene())
		return false;

	if (atomic_read(&mmvip_page_num) >= mm_vip_pool_pages)
		return false;
...
static int mm_vip_pool_pages_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", mm_vip_pool_pages);
	return 0;
}

static int mm_vip_pool_pages_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, mm_vip_pool_pages_proc_show, NULL);
}

static ssize_t mm_vip_pool_pages_proc_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *ppos)
{
	unsigned int val = 0;
	int ret = 0;
...
static const struct proc_ops mm_vip_pool_pages_proc_fops = {
		.proc_open		= mm_vip_pool_pages_proc_open,
		.proc_read		= seq_read,
		.proc_write		= mm_vip_pool_pages_proc_write,
		.proc_lseek		= seq_lseek,
		.proc_release	= single_release,
};
...
static int __init proc_mm_vip_scene_init(void)
{
	...
	proc_create("mm_vip_pool_pages", 0660, NULL, &mm_vip_pool_pages_proc_fops);
	return 0;
}
