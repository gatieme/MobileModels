...
static void ivp_dev_poweroff(struct ivp_device *pdev)
{
	int ret;
#ifdef MULTIPLE_ALGO
	int i;
...
static void ivp_irq_deinit_process(struct ivp_device *pdev)
{
	free_irq(pdev->ivp_comm.wdg_irq, pdev);
	if (pdev->ivp_comm.ivp_secmode == NOSEC_MODE)
		free_irq(pdev->ivp_comm.dwaxi_dlock_irq, pdev);
...
static int ivp_copy_section(struct ivp_device *pdev,
		const struct image_section_header *image_sect,
		unsigned int sect_dtsi_len, const unsigned int *source)
{
	unsigned int i;
	...
	if (ddr_flag == true) {
		...
		if (image_sect->size < pdev->ivp_comm.sects[SECT_START_NUM].len)
			mem = ivp_vmap(ivp_ddr_addr, image_sect->size, &offset);
	} else {
		if (image_sect->size < sect_dtsi_len)
			/*lint -e446 */
			mem = ioremap(image_sect->vaddr, image_sect->size);
			/*lint +e446 */
	}
...
static int ivp_load_section(struct ivp_device *pdev, const struct firmware *fw,
		const struct image_section_header *image_sect,
		unsigned int sect_dtsi_len)
{
	...
	case IMAGE_SECTION_TYPE_DATA: {
		...
		ret = ivp_copy_section(pdev, image_sect, sect_dtsi_len, source);
		if (ret < 0)
			return -EINVAL;
	}
	break;
...
static int ivp_load_section_header(const struct firmware *fw,
		struct file_header mheader, struct image_section_header *psect_header)
{
	size_t offset;
	errno_t ret;
	unsigned int idx;
	...
	for (idx = 0; idx < mheader.sect_count; idx++) {
		offset = sizeof(mheader) + sizeof(struct image_section_header) * idx;
		if ((offset + sizeof(struct image_section_header)) > fw->size) {
			ivp_err("image index is err");
			return -EINVAL;
		}
		ret = memcpy_s(&psect_header[idx], sizeof(struct image_section_header),
...
static int ivp_get_validate_section_info(struct ivp_device *pdev,
		...
		struct image_section_header *psect_header, unsigned int *sect_dtsi_len)
{
	unsigned int idx;

	if (((size_t)psect_header->offset + (size_t)psect_header->size) > fw->size) {
	...
	for (idx = 0; idx < SHARE_SECTION_INDEX; idx++) {
		if ((psect_header->vaddr >= pdev->ivp_comm.sects[idx].ivp_addr) &&
			(psect_header->vaddr < (pdev->ivp_comm.sects[idx].ivp_addr +
				pdev->ivp_comm.sects[idx].len))) {
			*sect_dtsi_len = pdev->ivp_comm.sects[idx].len;
			return EOK;
		}
	}
...
static int ivp_chk_section_offset(struct image_section_header sect_header,
		struct image_section_header sect_header_next)
{
	unsigned type;
	unsigned offset_size_sum;
	unsigned next_offset;

	type = sect_header.type;
	next_offset = sect_header_next.offset;

	/* offset + size = next_offset */
	if (type == IMAGE_SECTION_TYPE_BSS) {
		offset_size_sum = sect_header.offset;
	} else if ((type == IMAGE_SECTION_TYPE_EXEC) ||
			(type == IMAGE_SECTION_TYPE_DATA)) {
		offset_size_sum = sect_header.offset + sect_header.size;
	} else {
		ivp_err("error format type 0x%x", type);
		return -EINVAL;
	}

	if (offset_size_sum == next_offset) {
		ivp_info("right format type[0x%x] offset size sum[0x%x]",
			type, offset_size_sum);
		return EOK;
	}

	ivp_err("error format type[0x%x] offset[0x%x] size[0x%x] next offset[0x%x]",
		type, sect_header.offset, sect_header.size, sect_header_next.offset);
	return -EINVAL;
}

static int ivp_chk_section_header(const struct firmware *fw,
		struct file_header mheader,
		const struct image_section_header *psect_header)
{
	int ret;
	unsigned int i;
	size_t size_sum = (size_t)psect_header[0].offset;

	for (i = 0; i < mheader.sect_count; i++) {
		/* offset0 + sum_size(except type == 2)  = fw.size */
		if (psect_header[i].type != IMAGE_SECTION_TYPE_BSS)
			size_sum += (size_t)psect_header[i].size;

		/* check num is section count num subtract 1 */
		if (i < mheader.sect_count - 1) {
			ret = ivp_chk_section_offset(psect_header[i], psect_header[i + 1]);
			if (ret != EOK) {
				ivp_err("check ivp image section offset fail");
				return -EINVAL;
			}
		}
	}
	...
	if (size_sum != fw->size) {
		ivp_err("check ivp image section sum size[0x%lx] fail fw size[0x%lx]",
			size_sum, fw->size);
		return -EINVAL;
	}
...
static int ivp_chk_firmware_header(const struct firmware *fw,
		const struct file_header mheader,
		struct image_section_header *sect_header)
{
	int ret;
	size_t img_sect_head_len;

	if (mheader.sect_count > IVP_IMAGE_MAX_SECT_NUM) {
		ivp_err("check firmware section count fail");
		return -EINVAL;
	}
	img_sect_head_len = sizeof(struct image_section_header);
	(void)memset_s(sect_header, img_sect_head_len * mheader.sect_count, 0,
		img_sect_head_len * mheader.sect_count);

	ret = ivp_load_section_header(fw,mheader, sect_header);
	if (ret != EOK) {
		ivp_err("load image section header fail");
		return -EINVAL;
	}

	/* check entire section header */
	ret = ivp_chk_section_header(fw,mheader, sect_header);
	if (ret != EOK) {
		ivp_err("check image section header size offset fail");
		return -EINVAL;
	}
		...
		struct file_header mheader)
{
	...
	int ret;
	unsigned int sect_dtsi_len;
	struct image_section_header sect_header[IVP_IMAGE_MAX_SECT_NUM];

	ivp_info("start loading image %s, section counts 0x%x",
		filename, mheader.sect_count);
	ret = ivp_chk_firmware_header(fw, mheader, sect_header);
	if (ret != EOK) {
		ivp_err("check firmware image section head fail");
		return -EINVAL;
	}

	for (idx = 0; idx < mheader.sect_count; idx++) {
		if (ivp_get_validate_section_info(pdev, fw, &sect_header[idx],
			&sect_dtsi_len)) {
			ivp_err("get section %u fails", idx);
			return -EINVAL;
		}
		if (ivp_load_section(pdev, fw, &sect_header[idx], sect_dtsi_len)) {
			ivp_err("load section %u fails", idx);
			return -EINVAL;
		}
	}
		...
		struct file_header *mheader)
{
	...
	if (sizeof(struct file_header) > fw->size) {
		ivp_err("image file mheader is err");
		return -EINVAL;
	}

	(void)memset_s(mheader, sizeof(struct file_header), 0,
		sizeof(struct file_header));
	ret = memcpy_s(mheader, sizeof(struct file_header), fw->data,
		sizeof(struct file_header));
	if (ret != EOK) {
		ivp_err("memcpy_s fail, ret [%d]", ret);
		return -EINVAL;
		...
		int segment_type)
{
	...
	if (segment_type == SEGMENT_IRAM_TEXT) {
		...
		sect_dtsi_len = pdev->ivp_comm.sects[IRAM_SECT_INDEX].len;
	} else if (segment_type == SEGMENT_DRAM0_DATA) {
		map_addr = pdev->algo_mem_info[algo_id].algo_data_addr + pdev->base_offset;
		sect_dtsi_len = pdev->ivp_comm.sects[DRAM0_SECT_INDEX].len;
	...
	ivp_info("ivp ioremap section dtsi len[0x%x]", sect_dtsi_len);
	if (program_header->p_memsz < sect_dtsi_len)
	...
	if (segment_type == SEGMENT_DDR_TEXT) {
		ivp_ddr_addr = pdev->algo_mem_info[algo_id].algo_ddr_addr -
			...
			((pdev->ivp_comm.sects[BASE_SECT_INDEX].acpu_addr << IVP_MMAP_SHIFT) +
				pdev->ivp_comm.sects[BASE_SECT_INDEX].len);
		ivp_info("vaddr[%#x]  memsz[%x] max_algo_mem_len[%x]", program_header->p_vaddr,
			program_header->p_memsz, pdev->ivp_comm.sects[ALGO_SECT_INDEX].len);
		if (program_header->p_memsz < pdev->ivp_comm.sects[ALGO_SECT_INDEX].len)
...
static int ivp_get_algo_node(struct ivp_device *pdev, XT_Elf32_Ehdr *elf_head,
	...
	for (i = 0; i < IVP_ALGO_NODE_MAX; i++) {
		if (pdev->algo_mem_info[i].occupied == FREE) {
			...
			for (j = 0; j < elf_head->e_phnum; j++) {
				if (header[j].p_type != PT_LOAD || header[j].p_memsz == 0) {
					continue; /* support dynamic header */
				}
				ret = ivp_alloc_algo_mem(pdev, header[j], i);
				if (ret != 0) {
...
static int ivp_load_foreach_segment(struct ivp_device *pdev,
		...
		const int algo_id, XT_Elf32_Phdr *program_header)
{
	errno_t ret;
	unsigned int i;

	for (i = 0; i < elf_head->e_phnum; i++) {
		if (((size_t)program_header[i].p_offset +
			(size_t)program_header[i].p_memsz) > fw->size) {
			ivp_err("get invalid offset 0x%x memsz 0x%x",
				program_header[i].p_offset, program_header[i].p_memsz);
			return -EINVAL;
		}
		if ((program_header[i].p_type == PT_LOAD) &&
			(program_header[i].p_memsz > 0)) {
			ret = ivp_load_algo_segment(pdev, fw, &program_header[i], algo_id);
			if (ret) {
				ivp_err("ivp_load_algo_segment failed");
				return -EINVAL;
			}
		} else if (program_header[i].p_type == PT_DYNAMIC) {
			ivp_get_dyn_info(pdev, &program_header[i], fw, algo_id);
		}
	}
	return EOK;
}
		...
		const struct firmware *fw)
{
	...
	XT_Elf32_Phdr program_header[MAX_ALGO_PROG_NUM];

	ivp_info("start load ivp algo");
	ret = ivp_load_elf_hdrs(fw, &elf_head, program_header, FIRMWARE_ALGO);
	if (ret != EOK) {
		ivp_err("load elf head failed");
		return -EINVAL;
	}

	ret= ivp_chk_elf_phdr(fw, elf_head, program_header, FIRMWARE_ALGO);
	if (ret != EOK) {
		ivp_err("check elf programe head failed");
		return -EINVAL;
	}

	algo_id = ivp_get_algo_node(pdev, &elf_head, program_header);
	if (algo_id < 0) {
		...
		const XT_Elf32_Phdr *program_header, unsigned int *offset)
{
	...
	ivp_ddr_addr = (pdev->ivp_comm.sects[BASE_SECT_INDEX].acpu_addr <<
		...
		pdev->ivp_comm.sects[BASE_SECT_INDEX].ivp_addr;
	ivp_info("vaddr[%#x]  memsz[%#x] base section len[%#x]", program_header->p_vaddr,
		program_header->p_memsz, pdev->ivp_comm.sects[BASE_SECT_INDEX].len);

	if (program_header->p_memsz < pdev->ivp_comm.sects[BASE_SECT_INDEX].len)
		...
		unsigned int sect_dtsi_len)
{
	...
	if (ddr_flag) {
		...
		if (program_header->p_memsz < sect_dtsi_len) {
			...
			/*lint +e446 */
			ivp_info("load xtensa memory at offset %#x memsz %#x",
				program_header->p_vaddr + pdev->base_offset,
				program_header->p_memsz);
		}
	}
		...
		unsigned int *sect_dtsi_len)
{
	...
	size_t offset_memsz_sum = (size_t)program_header->p_offset +
		(size_t)program_header->p_memsz;

	if ((offset_memsz_sum > fw->size) ||program_header->p_type != PT_LOAD ||
	...
	if (p_vaddr >= pdev->ivp_comm.sects[DRAM0_SECT_INDEX].ivp_addr &&
		...
		*sect_dtsi_len = pdev->ivp_comm.sects[DRAM0_SECT_INDEX].len;
		return EOK;
	} else if (p_vaddr >= pdev->ivp_comm.sects[DRAM1_SECT_INDEX].ivp_addr &&
			(p_vaddr < (pdev->ivp_comm.sects[DRAM1_SECT_INDEX].ivp_addr +
			pdev->ivp_comm.sects[DRAM1_SECT_INDEX].len))) {
		*sect_dtsi_len = pdev->ivp_comm.sects[DRAM1_SECT_INDEX].len;
		return EOK;
	} else if (p_vaddr >= pdev->ivp_comm.sects[IRAM_SECT_INDEX].ivp_addr &&
			(p_vaddr < (pdev->ivp_comm.sects[IRAM_SECT_INDEX].ivp_addr +
			pdev->ivp_comm.sects[IRAM_SECT_INDEX].len))) {
		*sect_dtsi_len = pdev->ivp_comm.sects[IRAM_SECT_INDEX].len;
		return EOK;
	} else if ((program_header->p_vaddr >= pdev->ivp_comm.sects[BASE_SECT_INDEX].ivp_addr) &&
			(program_header->p_vaddr < (pdev->ivp_comm.sects[BASE_SECT_INDEX].ivp_addr +
			pdev->ivp_comm.sects[BASE_SECT_INDEX].len))) {
		*sect_dtsi_len = pdev->ivp_comm.sects[BASE_SECT_INDEX].len;
		return EOK;
	}
		...
		const struct firmware *fw)
{
	...
	unsigned int i;
	XT_Elf32_Ehdr elf_head;
	XT_Elf32_Phdr program_header[MAX_CORE_PROG_NUM];
	unsigned int sect_dtsi_len;

	ivp_info("start analyze ivp core[%u]", pdev->core_id);
	ret = ivp_load_elf_hdrs(fw, &elf_head, program_header, FIRMWARE_CORE);
	if (ret != EOK) {
		ivp_err("load elf head failed");
		return -EINVAL;
	}

	ret= ivp_chk_elf_phdr(fw, elf_head, program_header, FIRMWARE_CORE);
	if (ret != EOK) {
		ivp_err("check elf programe head failed");
		return -EINVAL;
	}
	...
	for (i = 0; i < elf_head.e_phnum; i++) {
		...
		if (ivp_get_core_segment(pdev, fw, &program_header[i],
			&sect_dtsi_len) != EOK) {
			ivp_err("get segment %u fails", i);
			return -EINVAL;
		}
		ret = ivp_load_core_segment(pdev, fw, &program_header[i],
			sect_dtsi_len);
		if (ret) {
...
static int ivp_load_image(struct ivp_device *pdev, const char *name)
{
	...
	ret = ivp_check_image(firmware, &mheader);
	if (ret != 0) {
		release_firmware(firmware);
	...
	ret = ivp_load_firmware(pdev, name, firmware, mheader);
	release_firmware(firmware);
	return ret;
}
