...
static ssize_t ivp_ipc_read(struct file *fd, char __user *buff,
	size_t size, loff_t *off);
static ssize_t ivp_ipc_write(struct file *fd, const char __user *buff,
	size_t size, loff_t *off);
static int ivp_ipc_release(struct inode *inode, struct file *fd);
...
static struct ivp_ipc_packet *ivp_ipc_alloc_packet(size_t len)
{
...
static void ivp_ipc_free_packet(struct ivp_ipc_packet *packet)
{
	if (!packet) {
...
static void ivp_ipc_init_queue(struct ivp_ipc_queue *queue)
{
	if (!queue) {
...
{
#if defined(SEC_IVP_V300)
	(void)pdev;
	(void)buff;
	(void)size;
#elif defined(SEC_IVP_V200) || defined(SEC_IVP_V250)
	if (pdev->ivp_secmode) {
		/* the third char is ipc cmd,the first char is msg index */
#if defined(SEC_IVP_V200)
		/* 3 is array element */
		if ((((unsigned char)buff[3] & TMP_BUFF_MASK) == CMD_ALGORUN) &&
			(buff[0] == 0))
#elif defined(SEC_IVP_V250)
		/* 3 is array element */
		if ((((unsigned char)buff[3] & TMP_BUFF_MASK) == CMD_ALGORUN) &&
			(buff[0] == 0 || buff[0] == 1))
#endif
			return ivp_trans_sharefd_to_phyaddr(pdev,
...
static int ivp_ipc_probe(struct platform_device *platform_pdev)
{
	...
	if (core_id == IVP_CORE0_ID) {
		...
		pdev->send_ipc = IPC_ACPU_IVP_MBX_1;
		pdev->recv_ipc = IPC_IVP_ACPU_MBX_1;
#ifdef IVP_DUAL_CORE
	} else if (core_id == IVP_CORE1_ID) {
		pdev->device.name = "ivp1-ipc";
		pdev->send_ipc = IPC_ACPU_IVP_MBX_3;
		pdev->recv_ipc = IPC_IVP_ACPU_MBX_2;
#endif
	} else {
		kfree(pdev);
...
static const struct of_device_id ivp1_ipc_of_descriptor[] = {
	{.compatible = "hisilicon,dev-ivp1-ipc",},
	{},
};
MODULE_DEVICE_TABLE(of, ipc_mdev_of_match);
#endif
