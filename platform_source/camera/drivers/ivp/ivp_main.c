...
static int ivp_open(struct inode *inode __attribute__((unused)), struct file *fd)
{
...
#ifdef CONFIG_IVP_SMMU_V3
	if (pdev->ivp_comm.ivp_pdev->dev.iommu->fwspec->num_ids == IVP_SMMU_ID_MAX) {
		pdev->sid =
			pdev->ivp_comm.ivp_pdev->dev.iommu->fwspec->ids[IVP_SMMU_SID];
		pdev->ssid =
			pdev->ivp_comm.ivp_pdev->dev.iommu->fwspec->ids[IVP_SMMU_SSID];
	} else {
...
static int ivp_release(struct inode *inode __attribute__((unused)), struct file *fd)
{
	struct ivp_device *pdev = NULL;
	int ret = 0;
...
static int ivp_remove(struct platform_device *plat_devp)
{
	...
	wakeup_source_unregister(pdev->ivp_comm.ivp_power_wakelock);
...
#ifdef IVP_DUAL_CORE
	pdev->core_id = id;
	pdev->base_offset = (id == IVP_CORE0_ID) ? 0 : IVP_BASE_REMAP_OFFSET;
#endif
}
...
static int ivp_probe(struct platform_device *plat_dev)
{
	...
	pdev->ivp_comm.ivp_power_wakelock = wakeup_source_register(&plat_dev->dev, source_name);
	if(!pdev->ivp_comm.ivp_power_wakelock){
		ivp_err("wakeup source register failed");
		kfree(pdev);
		return -ENOMEM;
	}
