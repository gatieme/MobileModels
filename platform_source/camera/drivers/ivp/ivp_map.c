...
{
	int ret = 0;
	size_t sec_fd_size = 0;
	phys_addr_t sec_fd_phys = 0x0;
...
#ifdef IVP_HIFORCE_ENABLE
	ret = mm_dma_heap_secmem_get_phys(dmabuf, &sec_fd_phys, &sec_fd_size);
#else
	ret = ion_secmem_get_phys(dmabuf, &sec_fd_phys, &sec_fd_size);
#endif
	if (ret) {
		ivp_err("%s is fail, ret = %d", __func__, ret);
		return -EINVAL;
	}
	*sfd = sec_fd_phys;
