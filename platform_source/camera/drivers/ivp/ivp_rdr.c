...
enum RDR_IVP_EXC_TYPE {
	MODID_IVP_START = DFX_BB_MOD_IVP_START,
	MODID_IVP_EXC_NOC_1 = MODID_IVP_START,
	MODID_IVP_EXC_NOC_2,
	MODID_IVP_EXC_NOC_3,
	MODID_IVP_EXC_SMMU,
	MODID_IVP_EXC_END = DFX_BB_MOD_IVP_END,
} rdr_ivp_exc_t;
...
#ifndef CONFIG_DFX_QIC_MODID_REGISTER
...
static int ivp_copy_log(struct rdr_ivp_device *dev)
{
	...
	if (is_ivp_in_secmode(dev->pivpdev)) {
		sect = pivp_com->sec_sects + log_sect;
		log_addr = (char*)ivp_vmap(sect->acpu_addr << IVP_MMAP_SHIFT, sect->len, &offset);
		if (!log_addr) {
			ivp_err("sec log_addr vmap fail\n");
			return -1;
		}
		log_size = (sect->len > dev->log_len) ? dev->log_len : sect->len;
		...
		log_size = (pivp_com->sects[log_sect].len > dev->log_len) ?
			dev->log_len : pivp_com->sects[log_sect].len;
		ret = memcpy_s(dev->rdr_addr, dev->log_len, log_addr, log_size);
	}
	if (ret != EOK) {
...
static void ivp_rdr_reset(u32 modid __attribute__((unused)),
	u32 etype __attribute__((unused)), u64 coreid __attribute__((unused)))
{
#ifdef ENG_BUILDMODE
	int i = 0;

	ivp_err("enter.\n");
	while (i < RDR_RESET_DELAY) {
		i++;
		udelay(MS_DIV_US);
	}
	ivp_err("ivp noc reset timeout.\n");

	rdr_syserr_process_for_ap((u32)MODID_AP_S_PANIC_IVP, 0ULL, 0ULL);
#endif
}

#ifdef CONFIG_DFX_QIC_MODID_REGISTER
static int ivp_qic_register()
{
	dfx_qic_modid_register(QIC_IVP0, (u32)MODID_IVP_EXC_NOC_1);
	ivp_info("register qic err, , type:%x", MODID_IVP_EXC_NOC_1);
#ifdef IVP_DUAL_CORE
	dfx_qic_modid_register(QIC_IVP1, (u32)MODID_IVP_EXC_NOC_2);
	ivp_info("register qic err, , type:%x", MODID_IVP_EXC_NOC_2);
#endif
	return EOK;
}
#elif CONFIG_DFX_NOC_MODID_REGISTER
...
static int ivp_rdr_module_register(void)
{
	...
	dev->rdr_addr = dfx_bbox_map((phys_addr_t)dev->log_addr, dev->log_len);
	if (!dev->rdr_addr) {
		ivp_err("dfx_bbox_map rdr_addr failed.\n");
		return -EINVAL;
	}
...
static void rdr_ivp_err_work(struct work_struct *work __attribute__((unused)))
{
	struct rdr_ivp_device *dev = &g_ivp_rdr_dev;
