...
static irqreturn_t ivp_smmu_isr(int irq __attribute__((unused)), void *dev_id)
{
...
static int ivp_iommu_fault_handler_t(
...
static int ivp_smmu_probe(struct platform_device *pdev)
{
	...
	smmu_dev->pgd_base = (unsigned long)kernel_domain_get_ttbr(smmu_dev->dev);
