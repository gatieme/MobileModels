...
static timestamp_mode g_timestamp_mode;
...
bool __weak modify_ack_timestamp_in_user(void)
{
	return false;
}

static void hisp_subdev_get_system_timestamp(hisp_system_time_t *isp_systime)
{
	if (!isp_systime)
		return;

	g_timestamp_mode = isp_systime->mode;

	isp_systime->s_system_counter = __arch_counter_get_cntvct();
	isp_systime->s_system_us = modify_ack_timestamp_in_user() ?
		(g_timestamp_mode == TIMESTAMP_USE_BOOTTIME ?
		ktime_to_us(ktime_get_boottime()) : ktime_get_real_ns() / 1000) : 0;
	isp_systime->s_system_couter_rate = arch_timer_get_rate();

	cam_debug("syscnt:%llu, sysus:%llu, syscnt_rate:%u, timestamp_mode:%d",
		isp_systime->s_system_counter,
		isp_systime->s_system_us,
		isp_systime->s_system_couter_rate,
		g_timestamp_mode);
}
...
static long hisp_ioctl_select(unsigned int cmd, hisp_t *isp, void *arg)
{
	...
	case HISP_IOCTL_SEND_RPMSG:
		...
		if (isp && isp->hw && isp->hw->vtbl) {
			if (current->tgid != hisp_rpmsg_getpid()) {
				cam_err("invalid SEND_RPMSG usr(cur %d, exp %d)", current->tgid, hisp_rpmsg_getpid());
				return -EINVAL;
			}
			rc = isp->hw->vtbl->send_rpmsg(isp->hw, arg, len);
		}
		break;
	case HISP_IOCTL_RECV_RPMSG:
		...
		if (isp && isp->hw && isp->hw->vtbl) {
			if (current->tgid != hisp_rpmsg_getpid()) {
				cam_err("invalid RECV_RPMSG usr(cur %d, exp %d)", current->tgid, hisp_rpmsg_getpid());
				return -EINVAL;
			}
	...
	unsigned int cmd, void *arg)
{
	...
	return hisp_ioctl_select(cmd, isp, arg);
}
...
#define MAX_PINCTRL_NAME 32
#define PINCTRL_EXTRA_PAIRS "pinctrl-extra-pairs"
static int get_extra_pinctrl(struct device *dev, hisp_dt_data_t *dt)
{
	int idx;
	int ret;
	int extra_pairs;
	struct device_node *np = dev->of_node;
	char fullname[MAX_PINCTRL_NAME] = {0}; // store xx-default/xx-idle
	struct extra_pinctrl ep = {
		.name = NULL,
		.pinctrl_default = NULL,
		.pinctrl_idle = NULL,
	};

	extra_pairs = of_property_count_strings(np, PINCTRL_EXTRA_PAIRS);
	if (extra_pairs < 0) // not support extra pinctrl, it's ok
		return 0;
	if ((size_t)extra_pairs > ARRAY_SIZE(dt->extra_pinctrl)) {
		cam_err("%s count: %d exceeds max: %d", PINCTRL_EXTRA_PAIRS,
			extra_pairs, MAX_EXTRA_PINCTRL_PAIRS);
		return -EFAULT;
	}

	for (idx = 0; idx < extra_pairs; ++idx) {
		ret = of_property_read_string_index(
			np, PINCTRL_EXTRA_PAIRS, idx, &ep.name);
		if (ret < 0) {
			cam_err("fail to get index %d of %s", idx, PINCTRL_EXTRA_PAIRS);
			return ret;
		}

		ret = sprintf_s(fullname, sizeof(fullname), "%s-default", ep.name);
		if (ret < 0) {
			cam_err("extra pinctrl name: %s too long", ep.name);
			return ret;
		}
		ep.pinctrl_default = pinctrl_lookup_state(dt->pinctrl, fullname);
		if (IS_ERR_OR_NULL(ep.pinctrl_default)) {
			cam_err("fail to get pinctrl state: %s", fullname);
			return -ENODATA;
		}

		ret = sprintf_s(fullname, sizeof(fullname), "%s-idle", ep.name);
		if (ret < 0) {
			cam_err("extra pinctrl name: %s too long", ep.name);
			return ret;
		}
		ep.pinctrl_idle = pinctrl_lookup_state(dt->pinctrl, fullname);
		if (IS_ERR_OR_NULL(ep.pinctrl_idle)) {
			cam_err("fail to get pinctrl state: %s", fullname);
			return -ENODATA;
		}

		ret = pinctrl_select_state(dt->pinctrl, ep.pinctrl_idle);
		if (ret != 0) {
			cam_err("could not set pinctrl: %s to idle", ep.name);
			return ret;
		}
		cam_info("parsed extra pinctrl: %s, idx: %d", ep.name, idx);
		dt->extra_pinctrl[idx] = ep;
	}
	return 0;
}

__weak int hisp_extra_pinctrl_index(const char *name)
{
	(void)name;
	return -1;
}

__weak int hisp_extra_pinctrl_select_state(int idx, const char *state)
{
	(void)idx;
	(void)state;
	return -1;
...
int set_isp_dt_parameter(struct device *dev, hisp_dt_data_t *dt)
{
	...
	return get_extra_pinctrl(dev, dt);
}
