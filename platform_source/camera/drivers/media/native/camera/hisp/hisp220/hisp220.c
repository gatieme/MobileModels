...
static char const *hisp220_get_name(hisp_intf_t *i);
...
static void hisp220_init_timestamp(void);
static void hisp220_destroy_timestamp(void);
static void hisp220_set_timestamp(unsigned int *timestamp_h, unsigned int
	*timestamp_l);
static void hisp220_handle_msg(hisp_msg_t *msg);
...
static void hisp220_init_timestamp(void)
{
	...
	if (hisp_get_timestamp_mode() == TIMESTAMP_USE_BOOTTIME) {
		tv_usec = ktime_to_us(ktime_get_boottime());
		s_timeval.tv_usec = tv_usec % CAM_MICROSECOND_PER_SECOND;
		s_timeval.tv_sec = tv_usec / CAM_MICROSECOND_PER_SECOND;
	} else {
	...
	if (hisp_get_timestamp_mode() == TIMESTAMP_USE_BOOTTIME) {
		tv_usec = ktime_to_us(ktime_get_boottime());
		s_timeval.tv_nsec = (tv_usec % CAM_MICROSECOND_PER_SECOND) * CAM_NANOSECOND_PER_MICROSECOND;
		s_timeval.tv_sec = tv_usec / CAM_MICROSECOND_PER_SECOND;
	} else {
 ...
 *********************************************/
static void hisp220_set_timestamp(unsigned int *timestamp_h, unsigned int *timestamp_l)
{
	...
	fw_micro_second =
		...
		(u64)s_timeval.tv_sec) * CAM_MICROSECOND_PER_SECOND +
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
...
static int hisp220_init_r8isp_memory_pool(void *cfg)
{
	...
	if ((pcfg->param.security_isp_mode == 0) &&
		(buffer_is_invalid(pcfg->param.sharedFd,
		pcfg->param.iova, pcfg->param.size) != 0)) {
	...
	if (ipool == MEM_POOL_ATTR_READ_WRITE_CACHE_OFF_LINE)
		...
		en_map_type = (pcfg->param.security_isp_mode != 0) ?
...
static int hisp220_deinit_r8isp_memory_pool(void *cfg)
{
	...
	/*
	 * Store the protocol in case the memory pool is deinitialized,
	 * but the power is not turned off. If this happens,
	 * and the protocol is not stored,
	 * it will cause a hardware abstraction layer (HAL) tombstone.
	 */
	prot = s_hisp220.mem_pool[ipool].prot;
	...
	s_hisp220.mem_pool[ipool].prot = prot;
	mutex_unlock(&kernel_rpmsg_service_mutex);
...
static int hisp220_alloc_r8isp_addr(void *cfg)
{
	...
	pcfg->param.ispCoreIova = s_hisp220.mem_pool[ipool].ion_iova + offset;
...
static int hisp220_power_on(hisp_intf_t *i)
{
	...
	uint32_t timeout = (hw_is_fpga_board() != 0) ?
...
static int hisp220_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	...
	if (skb_queue_empty(&kernel_serv->queue)) {
		...
		if (wait_event_interruptible_timeout(kernel_serv->readq, /*lint -e578*/
			((skb_queue_empty(&kernel_serv->queue) == 0) || /*lint -e666*/
			kernel_serv->state == RPMSG_FAIL),
			(long)msecs_to_jiffies(HISP_WAIT_TIMEOUT))) { /*lint -e666*/
