...
static const char *pg_chip_without_jpegenc[] = {
	"pc",
	"lite_pc",
	"unknown",
};
...
static void hjpeg_irq_clr(void __iomem *jpegenc, void __iomem *subctrl);
...
static int hjpeg_encode_process_config(jpgenc_config_t *pcfg)
{
	...
	if (down_timeout(&s_hjpeg.buff_done, jiff) != 0) {
		...
		ret = -1;
		dump_reg();
		hjpeg_irq_clr(s_hjpeg.hw_ctl.jpegenc_viraddr, s_hjpeg.hw_ctl.subctrl_viraddr);
	}
...
static void hjpeg_irq_merge_clr(void __iomem *subctrl1)
{
	u32 set_val;
...
static void hjpeg_irq_default_clr(void __iomem *jpegenc)
{
	if (!jpegenc ) {
		cam_err("%s, input param is null", __func__);
		return;
...
static void hjpeg_irq_clr(void __iomem *jpegenc, void __iomem *subctrl)
{
	...
	switch (g_is_irq_merge) {
	case JPEG_IRQ_IN_OUT:
		hjpeg_irq_inout_clr(jpegenc, subctrl);
		break;
	case JPEG_IRQ_MERGE:
		hjpeg_irq_merge_clr(subctrl);
		break;
	default:
		hjpeg_irq_default_clr(jpegenc);
		break;
	}
}
...
static void hjpeg_encode_finish(void __iomem *jpegenc, void __iomem *subctl)
{
	...
	/* clr jpeg irq */
	hjpeg_irq_clr(jpegenc, subctl);
}

static bool bypass_jpegenc_by_soc_spec(void)
{
	...
	if (np == NULL) {
		cam_err("%s: of_find_compatible_node fail or normal type chip", __func__);
		return false;
	}

	ret = of_property_read_string(np, "soc_spec_set", &soc_spec);
	if (ret < 0) {
		cam_err("%s: read string fail", __func__);
		return false;
	}

	for (i = 0; i < ARRAY_SIZE(pg_chip_without_jpegenc); ++i) {
		ret = strcmp(soc_spec, pg_chip_without_jpegenc[i]);
		if (ret == 0) {
			cam_info("%s: this is pg chip:%s, need bypass jpegenc", __func__, pg_chip_without_jpegenc[i]);
			return true;
		}
	}

	cam_info("%s: no bypass jpegenc", __func__);
	return false;
}
...
static int32_t hjpeg_platform_probe(struct platform_device *pdev)
{
	...
	if (bypass_jpegenc_by_soc_spec()) {
		cam_info("%s: bypass jpegenc\n", __func__);
		return -1;
	}
