...
static int laser_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!s || !(s->ctrl) || !(s->client) || !(s->client->name)) {
		cam_err("can't register event queue");
		return rc;
	}

	s->ctrl->fh = &fh->vfh;
	cam_info("open laser device success, laser_name is %s", s->client->name);
	...
	const struct i2c_device_id *id)
{
	...
	init_subdev_media_entity(sd, CAM_SUBDEV_LASER);
	ret = cam_cfgdev_register_subdev(sd, CAM_SUBDEV_LASER);
