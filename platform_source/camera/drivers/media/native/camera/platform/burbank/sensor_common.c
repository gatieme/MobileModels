 ...
 *
 * Description: camera driver for sensor
 * Create: 2018-08-20
 *
 *
...
#ifdef ARCH_ATOMIC
static atomic_t s_powered = ATOMIC_INIT(0);
#else
...
{
	int ret = 0;
	bool snclk2 = (id == 2 || id == 4); // 2, 4 for different sensor
	...
	if(on) {
		if(0 == id) {
			...
			if(ret) {
				dev_err(dev, "cloud not prepare_enalbe clk_isp_snclk3. \n");
				return ret;
			}
		}
	} else {
...
int hw_mclk_config(sensor_t *s_ctrl, struct sensor_power_setting *power_setting, int state)
{
	int sensor_index;
	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	cam_info("%s enter, pin_type:%d state:%d delay:%u", __func__, pin_type, state, power_setting->delay);
	...
	if(pin_type == FSIN) {
		...
		rc = gpio_direction_output(sensor_info->gpios[pin_type].gpio,
			state ? (power_setting->config_val + 1) % 2 : power_setting->config_val); // 1, 2 for power
		if(rc < 0) {
			cam_err("failed to control gpio[%d]", sensor_info->gpios[pin_type].gpio);
		} else {
			cam_info("%s config gpio[%d] output[%d]", __func__, sensor_info->gpios[pin_type].gpio,
				(state ? (power_setting->config_val + 1) % 2 : power_setting->config_val)); // 1, 2 for power
		}
	}
...
void hw_sensor_i2c_config(sensor_t *s_ctrl, struct sensor_power_setting *power_setting, int state)
{
...
int kernel_sensor_power_config(sensor_t *s_ctrl, struct sensor_power_setting *power_setting, int on_off)
{
	int rc = 0;
	switch(power_setting->seq_type) {
	case SENSOR_DVDD:
		cam_info("%s, seq_type:%u SENSOR_DVDD", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_DVDD, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_DVDD2:
		cam_info("%s, seq_type:%u SENSOR_DVDD2", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_DVDD2, s_ctrl->board_info, power_setting, on_off);
		if(rc) {
	...
	case SENSOR_OIS_DRV:
		...
		rc = hw_sensor_ldo_config(LDO_OISDRV, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_IOVDD:
		cam_info("%s, seq_type:%u SENSOR_IOVDD", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_IOVDD, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_AVDD:
		cam_info("%s, seq_type:%u SENSOR_AVDD", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_AVDD, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_AVDD2:
		cam_info("%s, seq_type:%u SENSOR_AVDD2", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_AVDD2, s_ctrl->board_info, power_setting, on_off);
		if(rc) {
			cam_err("%s power up procedure error", __func__);
			rc = 0;
		}
	...
	case SENSOR_VCM_AVDD:
		...
		rc = hw_sensor_ldo_config(LDO_VCM, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_VCM_AVDD2:
		cam_info("%s, seq_type:%u SENSOR_VCM_AVDD2", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_VCM2, s_ctrl->board_info, power_setting, on_off);
		if(rc) {
			cam_err("%s power up procedure error", __func__);
			rc = 0;
		}
	...
	case SENSOR_AVDD0:
		...
		rc = hw_sensor_ldo_config(LDO_AVDD0, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_AVDD1:
		cam_info("%s, seq_type:%u SENSOR_AVDD1", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_AVDD1, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_MISP_VDD:
		cam_info("%s, seq_type:%u SENSOR_MISP_VDD", __func__, power_setting->seq_type);
		rc = hw_sensor_ldo_config(LDO_MISP, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_MCLK:
	...
	case SENSOR_I2C:
		if (on_off == POWER_ON) {
			cam_info("%s, seq_type:%u SENSOR_I2C", __func__, power_setting->seq_type);
			hw_sensor_i2c_config(s_ctrl, power_setting, on_off);
		}
		break;
	case SENSOR_LDO_EN:
		cam_info("%s, seq_type:%u SENSOR_LDO_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(LDO_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_AVDD1_EN:
		cam_info("%s, seq_type:%u SENSOR_AVDD1_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(AVDD1_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_DVDD0_EN:
		cam_info("%s, seq_type:%u SENSOR_DVDD0_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(DVDD0_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_DVDD1_EN:
		cam_info("%s, seq_type:%u SENSOR_DVDD1_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(DVDD1_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_IOVDD_EN:
		cam_info("%s, seq_type:%u SENSOR_IOVDD_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(IOVDD_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_MISPDCDC_EN:
		cam_info("%s, seq_type:%u SENSOR_MISPDCDC_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(MISPDCDC_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_CHECK_LEVEL:
		if (on_off == POWER_ON) {
			cam_info("%s, seq_type:%u SENSOR_CHECK_LEVEL", __func__, power_setting->seq_type);
			rc = hw_sensor_gpio_config(FSIN, s_ctrl->board_info, power_setting, on_off);
		}
		break;
	case SENSOR_RST:
		cam_info("%s, seq_type:%u SENSOR_RST", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(RESETB, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_PWDN:
		cam_info("%s, seq_type:%u SENSOR_PWDN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(PWDN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_VCM_PWDN:
		cam_info("%s, seq_type:%u SENSOR_VCM_PWDN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(VCM, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_SUSPEND:
		cam_info("%s, seq_type:%u SENSOR_SUSPEND", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(SUSPEND, s_ctrl->board_info, power_setting, POWER_OFF);
		break;
	case SENSOR_SUSPEND2:
		cam_info("%s, seq_type:%u SENSOR_SUSPEND2", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(SUSPEND2, s_ctrl->board_info, power_setting, POWER_OFF);
		break;
	case SENSOR_RST2:
		cam_info("%s, seq_type:%u SENSOR_RST2", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(RESETB2, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_OIS:
		cam_info("%s, seq_type:%u SENSOR_OIS", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(OIS, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_OIS2:
		cam_info("%s, seq_type:%u SENSOR_OIS2", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(OIS2, s_ctrl->board_info, power_setting, on_off);
		if(rc) {
			cam_err("%s power up procedure error", __func__);
			rc = 0;
		}
	...
	case SENSOR_PMIC:
		...
		rc = hw_sensor_pmic_config(s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_CS:
		break;
	case SENSOR_AVDD2_EN:
		cam_info("%s, seq_type:%u SENSOR_AVDD2_EN", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(AVDD2_EN, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_MIPI_SW:
		cam_info("%s, seq_type:%u SENSOR_MIPI_SW", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(MIPI_SW, s_ctrl->board_info, power_setting, on_off);
		break;
	case SENSOR_MIPI_SW2:
		cam_info("%s, seq_type:%u SENSOR_MIPI_SW2", __func__, power_setting->seq_type);
		rc = hw_sensor_gpio_config(MIPI_SW2, s_ctrl->board_info, power_setting, on_off);
		break;
	default:
...
EXPORT_SYMBOL(kernel_sensor_power_config);
...
int hw_sensor_power_down(sensor_t *s_ctrl)
{
	...
	if ((s_ctrl->power_down_setting_array.power_setting == NULL) || (s_ctrl->power_down_setting_array.size == 0)) {
...
int hwsensor_writefile(int index, const char *sensor_name)
{
	...
	rc = snprintf_s(file_name, FILE_NAME_LEN, FILE_NAME_LEN, "/data/camera/hisi_sensor%d", index);
	if (rc != 0)
		cam_err("%s: snprintf fail", __func__);
	filp = filp_open(file_name, O_CREAT | O_WRONLY, 0666); // 0666 for lenth
	...
	rc = snprintf_s(data, FILE_NAME_LEN, FILE_NAME_LEN, ":%s", sensor_name);
	if (rc != 0)
		cam_err("%s: snprintf fail", __func__);
...
int hw_sensor_get_phyinfo_data(struct device_node *of_node, hwsensor_board_info_t *sensor_info, int info_count)
{
	int ret = 0;
	int count = 0;
	...
	sensor_t *sensor)
{
	...
	rc = of_property_read_u32(of_node, "release_value", &sensor_info->release_value);
	...
	rc = of_property_read_u32(of_node, "hold_value", &sensor_info->hold_value);
	...
	rc = of_property_read_u32(of_node, "vendor,topology_type", &sensor_info->topology_type); /*lint !e64 */
	...
	if (!rc && (sensor_info->phyinfo_valid == 1 || sensor_info->phyinfo_valid == 2))
		rc = hw_sensor_get_phyinfo_data(of_node, sensor_info, sensor_info->phyinfo_valid);

	rc = of_property_read_u32(of_node, "dynamic_mipisw_num", &sensor_info->dynamic_mipisw_num);
	...
	if (sensor_info->dynamic_mipisw_num == 1) {
		rc = of_property_read_u32(of_node, "mipisw_enable_value0", &sensor_info->mipisw_enable_value0);
	...
	} else if (sensor_info->dynamic_mipisw_num == 2) { // 2 for dynamic sensor
		rc = of_property_read_u32(of_node, "mipisw_enable_value0", &sensor_info->mipisw_enable_value0);
		cam_info("%s mipisw_enable_value0 0x%x, rc %d\n", __func__, sensor_info->mipisw_enable_value0, rc);
		if (rc < 0) {
			sensor_info->mipisw_enable_value0 = 0;
		...
		rc = of_property_read_u32(of_node, "mipisw_enable_value1", &sensor_info->mipisw_enable_value1);
		cam_info("%s mipisw_enable_value1 0x%x, rc %d\n", __func__, sensor_info->mipisw_enable_value1, rc);
		if (rc < 0) {
	...
	sensor_info->i2c_use_gpio_group = -1;
	rc = of_property_read_u32(of_node, "huawei,i2c_use_gpio_group", &sensor_info->i2c_use_gpio_group);
	cam_info("%s i2c_use_gpio_group %d, rc %d\n", __func__, sensor_info->i2c_use_gpio_group, rc);
	if (rc < 0) {
		sensor_info->i2c_use_gpio_group = -1;
		cam_warn("%s read i2c_use_gpio_group failed, rc %d, set fail value %d\n",
			  __func__, rc, sensor_info->i2c_use_gpio_group);
		rc = 0;
	}
	...
	struct sensor_power_setting_array *power_setting_array, int is_power_on)
{
	...
	seq_type_name = (is_power_on ? "vendor,cam-power-seq-type" : "vendor,cam-power-down-seq-type");
	...
	seq_val_name = (is_power_on ? "vendor,cam-power-seq-val" : "vendor,cam-power-down-seq-val");
	...
	seq_cfg_name = (is_power_on ? "vendor,cam-power-seq-cfg-val" : "vendor,cam-power-down-seq-cfg-val");
	...
	seq_sensor_index_name = (is_power_on ? "vendor,cam-power-seq-sensor-index" : "vendor,cam-power-down-seq-sensor-index");
	...
	seq_delay_name = (is_power_on ? "vendor,cam-power-seq-delay" : "vendor,cam-power-down-seq-delay");
...
{
	/*lint -save -e446 -e666*/
	ktime_t wait = ns_to_ktime((u64)ms * 1000 * 1000); // 1000 * 1000 for ktime
...
{
	cam_info("%s is_fpga=%d", __func__, is_fpga);
	return is_fpga;
}
EXPORT_SYMBOL(hw_is_fpga_board);

int hw_get_power_seq_type_tab(power_seq_type_tab** tab)
{
	cam_info("%s", __func__);
	*tab = seq_type_tab;
	return (int)sizeof(seq_type_tab) / sizeof(seq_type_tab[0]);
}
EXPORT_SYMBOL(hw_get_power_seq_type_tab);
