...
typedef struct _tag_hwsensor_board_info {
	...
	unsigned int mclk; // deprecated since v350
	unsigned int phy_clk_freq; // deprecated since v350
	unsigned int phy_clk_num;  // deprecated since v350
	struct clk *phy_clk[CSI_INDEX_MAX]; // deprecated since v350
	/* this is used to distinguish different */
	/* dvdd voltage values for main camera sensors */
	unsigned int ldo_diverse;
	int csi_id[2];
	int i2c_id[2];
	int multiplex_gpio_id[MULTIPLEX_GPIO_MAX]; // deprecated since v350
	int ao_i2c_id;
	int igs_cam_clk;
	int i2c_pad_type;
	int module_type;
	...
	unsigned int use_power_down_seq;  // deprecated since v350
...
typedef struct _sensor_t {
	...
	// following deprecated since v350
