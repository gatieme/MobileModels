...
static int wl2686c_suspend(struct device *dev)
{
	struct wl2868c *chip = dev_get_drvdata(dev);
	int rc;

	rc = wl2868c_masked_write(chip->regmap, WL2868C_LDO_EN,
					CHIP_ENABLE_BIT, 0);
	if (rc < 0)
...
static int wl2686c_resume(struct device *dev)
{
	struct wl2868c *chip = dev_get_drvdata(dev);
	int rc;
	...
	rc = wl2868c_masked_write(chip->regmap, WL2868C_LDO_EN,
				CHIP_ENABLE_BIT, 1);
	if (rc < 0)
		pr_err("failed to resume chip rc=%d\n", rc);

	return 0;
}

static struct dev_pm_ops wl2686c_pm = {
	.suspend = wl2686c_suspend,
	.resume  = wl2686c_resume,
};
