...
#ifdef CONFIG_COMPAT
static long hw_flash_subdev_ioctl32(struct v4l2_subdev *sd,
		unsigned int cmd, unsigned long arg)
{
	struct hw_flash_cfg_data data;
	long rc = -EFAULT;

	rc = memset_s(&data, _IOC_SIZE(cmd), 0, _IOC_SIZE(cmd));
	if (rc) {
		cam_err("%s memset_s return fail\n", __func__);
		return rc;
	}

	rc = copy_from_user(&data, (void*)arg, _IOC_SIZE(cmd));
	if (rc) {
		cam_err("%s: copy in arg failed!\n", __func__);
		return rc;
	}

	rc = hw_flash_subdev_ioctl(sd, cmd, &data);
	if (rc){
		cam_err("%s subdev_ioctl failed!\n", __func__);
		return rc;
	}

	rc = copy_to_user((void*)arg, &data, _IOC_SIZE(cmd));
	if (rc) {
	...
	struct v4l2_subdev_fh *fh)
{
	...
	if (!flash_ctrl)
		return 0;
	/* when cameraDaemon crash, need turn off all flash */
	for (pos = 0; pos < HWFLASH_POSITION_MAX; pos++) {
		arg.position = pos;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = hw_flash_subdev_ioctl32
#endif
};
