	...
	struct v4l2_subdev_fh *fh)
{
	...
	cdata.cfgtype = SEN_CONFIG_EXT_VCM_POWER_OFF;
...
static long hwsensor_subdev_get_info(struct hwsensor_t *s, hwsensor_info_t *info)
{
	...
	info->igs_cam_clk = sensor->board_info->igs_cam_clk;
	info->i2c_pad_type = sensor->board_info->i2c_pad_type;
	info->matchid_fail_retry_flag =
		sensor->board_info->matchid_fail_retry_flag;
	cam_info("%s bus_type %s, ao_i2c_id:%d, igs_cam_clk:%d, mount_position:%d, name:%s, i2c_pad_type:%d"
		"matchid_fail_retry_flag:%d\n",
		__func__, info->bustype, info->ao_i2c_index, info->igs_cam_clk, info->mount_position, info->name,
		info->i2c_pad_type, info->matchid_fail_retry_flag);

#pragma GCC visibility push(default)
	cam_cfg_err("%s, info_count = %d\n, for print, not err.\n"
...
static int tof_igs_position_switch(sensor_t *hisensor)
{
	int status = -1;
	int tof_gpio_num = 0;
	...
	if (status == GPIO_LOW_STATE) {
		cam_info("%s.gpio[%d] status is %d, get tof igs camera1",
			__func__, tof_gpio_num, status);
	} else if (status == GPIO_HIGH_STATE) {
		cam_info("%s.gpio[%d] status is %d, get tof igs camera2",
			__func__, tof_gpio_num, status);
	} else {
...
int tof_igs_info_store(struct hwsensor_t *s)
{
	if (!s) {
	...
	g_s = s;
	cam_info("tof_igs_info_store");

	return 0;
}

long tof_igs_sensor_ioctl(struct file *file,
	...
	unsigned int cmd, void *arg)
{
	...
	case HWSENSOR_IOCTL_GET_THERMAL:
		...
		break;
	default:
		cam_cfg_err("invalid IOCTL CMD = %d\n", cmd);
		break;
	}
	return rc;
}
...
int hwsensor_register(struct platform_device *pdev, hwsensor_intf_t *si)
{
	...
	init_subdev_media_entity(subdev, CAM_SUBDEV_SENSOR);
	cam_cfgdev_register_subdev(subdev, CAM_SUBDEV_SENSOR);
	...
	if (hisensor->board_info->sensor_index == TOF_IGS_SENSOR_ID) {
		tof_gpio_state = tof_igs_position_switch(hisensor);
		if ((tof_gpio_state == GPIO_HIGH_STATE &&
			hisensor->board_info->tof_igs_num == TOF_IGS_NUM_2) ||
			(tof_gpio_state == GPIO_LOW_STATE &&
			hisensor->board_info->tof_igs_num == TOF_IGS_NUM_1)) {
			cam_info("%s get igs tof sensor dev, on_position sensor is %s",
				__func__, hisensor->board_info->name);
			rc = tof_igs_info_store(sensor);
		}
	}
