...
static void swing_cam_set_timestamp_source(void *arg)
{
	...
	swing_cam_log_info("timestamp source : %d", req->source);
...
{
	int ret;
	u32 length = (u32)sizeof(struct swingcam_data);
	...
	length = kfifo_in(&g_swing_priv.read_kfifo, (unsigned char *)&read_data,
		(u32)sizeof(struct swingcam_data));
	if (length == 0) {
	...
	struct miniisp_msg_t *p_msg_ack)
{
	...
	if ((p_msg_ack->api_name == MINI_EVENT_SENT) &&
		...
		mini_resp.message_size = sizeof(struct miniisp_msg_t);
	...
	struct miniisp_msg_t *p_msg_ack)
{
	...
	case SUB_CMD_SWING_CAM_CAPTURE:
		swing_cam_log_info("Capture Ack Received, camid %d",
...
static int swing_cam_get_resp(const struct pkt_header *head)
{
	...
	swing_cam_log_info("%s cmd[%d], subcmd[%d] length[%d]", __func__,
	...
	swing_cam_log_info("%s size[%d], name[%d] id[%d]", __func__,
...
{
	swing_cam_log_info("enter %s", __func__);
...
static int swing_v4l2_ioctl_cam_match_id(void *arg)
{
	...
	swing_cam_log_info("%s camid:%d, pos:%d, i2c:%d, csi:%d", __func__, match_req->camid,
		match_req->mount_position, match_req->i2c_index,
		match_req->csi_index);
