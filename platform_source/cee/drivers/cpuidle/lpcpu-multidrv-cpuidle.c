...
/*
 * when reboot or poweroff, watchdog-feed thread will be cancelled by self-develop code,
 * causing ap-watchdog intr.
 *
 * when ap-watchdog intr triggers M3 NMI, M3 will send cxpu NMI, causing cxpu in NMI state.
 * Cpuidle wont be handled by cxpu(M3) any more and acpu cant be powered on to return into
 * kernel(to handle ap-watchdog intr, do kernel mntn, etc).
 * So disable cpuidle when kernel reboot or poweroff
 */
static int lpcpu_cpuidle_reboot_notifier(struct notifier_block *nb,
...
static struct notifier_block lpcpu_cpuidle_reboot_nb = {
	.notifier_call = lpcpu_cpuidle_reboot_notifier,
};
...
static int __init lpcpu_idle_init(void)
{
	...
	register_reboot_notifier(&lpcpu_cpuidle_reboot_nb);
	return 0;
}
