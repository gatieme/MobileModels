...
#ifdef CONFIG_DDR_PISCES
#include <kernel_ddr_map.h>
#else
				...
				enum DDR_PERFDATA_CLIENT client)
{
	...
	arm_smccc_smc(ACCESS_REGISTER_FN_MAIN_ID, g_ddr_perfdata_phy_addr,
		      sizeof(struct ddr_perfdata), ACCESS_REGISTER_FN_SUB_ID_DDR_PERF_CTRL,
		      0, 0, 0, 0, &res);
	if (res.a0 != 0)
		pr_err("%s: arm_smccc_smc fail\n", __func__);
...
static int ddr_perf_ctrl_init(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "hisilicon,ddr-perfctrldata");
	if (np == NULL) {
		pr_err("%s: no compatible node found!\n", __func__);
		return -ENODEV;
	}

	ret = of_property_read_u32_array(np, "ddr-perfctrldata", &data[0], 2UL);
	if (ret != 0) {
		of_node_put(np);
	...
	g_ddr_perfdata_phy_addr =
		(phys_addr_t)(ATF_SUB_RESERVED_BL31_SHARE_MEM_PHYMEM_BASE + data[0]);
