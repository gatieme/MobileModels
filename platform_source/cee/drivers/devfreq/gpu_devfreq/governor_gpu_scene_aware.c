...
#include <platform_include/cee/linux/gpufreq.h>

#ifdef CONFIG_POWER_DUBAI
#include <huawei_platform/power/dubai/dubai_gpu_freq_stats.h>
#endif
#if defined(CONFIG_GPU_VIRTUAL_DEVFREQ) || \
	defined(CONFIG_GPU_THROTTLE_DEVFREQ) || \
	defined(CONFIG_HVGR_GPU_THROTTLE_DEVFREQ)
...
#if defined(CONFIG_GPU_THROTTLE_DEVFREQ) || \
	defined(CONFIG_HVGR_GPU_THROTTLE_DEVFREQ) || \
	defined(CONFIG_GPU_VIRTUAL_DEVFREQ)
static inline unsigned long gpu_min_freq(struct devfreq *df)
{
	s32 ret;

	ret = dev_pm_qos_read_value(df->dev.parent, DEV_PM_QOS_MIN_FREQUENCY);
	return (unsigned long)(unsigned int)ret * HZ_PER_KHZ;
}
#endif

#if defined(CONFIG_GPU_THROTTLE_DEVFREQ) || \
	defined(CONFIG_HVGR_GPU_THROTTLE_DEVFREQ)
static inline unsigned long gpu_max_freq(struct devfreq *df)
{
	s32 ret;

	ret = dev_pm_qos_read_value(df->dev.parent, DEV_PM_QOS_MAX_FREQUENCY);
	return (unsigned long)(unsigned int)ret * HZ_PER_KHZ;
}
#endif
						 ...
						 unsigned long *freq)
{
	...
	gpu_devfreq_set_gpu_throttle(gpu_min_freq(df), gpu_max_freq(df), freq);
#endif

#ifdef CONFIG_HVGR_GPU_THROTTLE_DEVFREQ
	hvgr_gpu_devfreq_set_gpu_throttle(gpu_min_freq(df), gpu_max_freq(df),
					  freq);
#endif

#ifdef CONFIG_GPU_VIRTUAL_DEVFREQ
	if (data->user_set_freq == 0) {
		gpu_virtual_devfreq_get_devfreq_min(&devfreq_min);
		if (*freq < devfreq_min && gpu_min_freq(df) <= devfreq_min) {
			virtual_freq = *freq;
...
#ifdef CONFIG_GPU_VIRTUAL_DEVFREQ
	gpu_virtual_devfreq_set_core_mask(virtual_freq, virtual_freq_flag);
#endif
}
...
{
	if (user_set_freq != 0)
		gpu_virtual_devfreq_set_mode(false);
	else
		gpu_virtual_devfreq_set_mode(true);
}
				     ...
				     unsigned long *freq)
{
	...
	if (user_set_freq == 0 && devfreq_min == *freq)
		gpu_virtual_devfreq_get_freq(freq);
}
#endif
					...
					unsigned long *freq)
{
	...
	unsigned long a;
	int err;
	struct devfreq_dev_status stat = { .busy_time = 0, .total_time = 100 };
	struct devfreq_gpu_scene_aware_data *data = NULL;
	struct devfreq_data *priv_data = NULL;
	...
	*freq = stat.current_frequency;
	...
	if (data->window_counter >= data->cur_policy->para[LOADING_WINDOW]) {
		for (i = 0; i < data->cur_policy->ntarget_load - 1 && *freq >= data->cur_policy->target_load[i + 1]; i += 2)
			;
				 ...
				 char * const buf)
{
	...
	mutex_lock(&devfreq->lock);
	gpu_virtual_devfreq_get_enable(&flag);
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
			 "%d\n", flag);
	mutex_unlock(&devfreq->lock);
				  ...
				  const char *buf, size_t count)
{
	...
	mutex_lock(&devfreq->lock);
	gpu_virtual_devfreq_set_enable(flag);
	mutex_unlock(&devfreq->lock);
	if (ret == 0)
		ret = count;
...
static void gpufreq_scene_aware_policy_init(struct devfreq *devfreq)
{
	...
	np = of_find_compatible_node(NULL, NULL, "gpu-scene-aware");
	if (np == NULL) {
		pr_err("%s: find gpu-scene-aware fail\n", __func__);
		return;
	}
...
#ifdef CONFIG_POWER_DUBAI
	dubai_set_gpu_freq_num(profile->max_state);
#endif
