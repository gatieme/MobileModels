...
#include <platform_include/cee/linux/lpcpu_cpufreq_dt.h>
#endif

#ifdef CONFIG_DRG
#include <platform_include/cee/linux/drg.h>
#endif
			      ...
			      unsigned long *_freq, u32 flags)
{
	...
	trace_perf(l3c_devfreq_target, freq);
	trace_perf(clock_set_rate, "l3cache", freq, raw_smp_processor_id());
...
static int l3c_devfreq_start_hwmon(struct l3c_devfreq *l3c)
{
	int ret = 0;
	struct l3c_hwmon_data *hw_data = &(l3c->hw->l3c_hw_data);
	...
	if (hw_data->active)
		goto exit;
	ret = l3c_devfreq_set_events(hw_data, l3c->pmu_associative_cpu);
	if (ret != 0) {
		pr_err("l3c_devfreq:perf event init failed on CPU%u\n", l3c->pmu_associative_cpu);
		WARN_ON_ONCE(1);
		goto exit;
	}
...
static unsigned long l3c_devfreq_calc_next_freq(struct l3c_devfreq *l3c)
{
	...
	trace_perf(l3c_devfreq_counter_info, l3c->hw->count.l3_count,
				       l3c->hw->count.bus_access_count);

	trace_perf(l3c_devfreq_bw_info, l3c->alg.usec_delta,
...
#ifdef CONFIG_HECA
		heca_register_dev_freq_notifier(HECA_DEVFREQ_L3C, devfreq);
#endif
		break;
...
#ifdef CONFIG_HECA
		heca_unregister_dev_freq_notifier(HECA_DEVFREQ_L3C, devfreq);
#endif
		break;
...
static int get_l3c_pmu_associative_cpu_any(struct platform_device *pdev)
{
	int i, nr_cpus, cpu_tmp;
	struct device_node *cpu_node = NULL;
	struct device_node *pmu_node = NULL;
	struct l3c_devfreq *l3c = platform_get_drvdata(pdev);

	pmu_node = of_find_compatible_node(NULL, NULL, "hisi,namtso_pmu");
	if (pmu_node == NULL) {
		/* before baltimore(include itself), the pmu node is "arm,dsu-pmu" */
		pmu_node = of_find_compatible_node(NULL, NULL, "arm,dsu-pmu");
		if (pmu_node == NULL) {
			pr_err("hisi,pmu No compatible node found\n");
			return -ENODEV;
		}
	}
	nr_cpus = of_count_phandle_with_args(pmu_node, "cpus", NULL);
	if (nr_cpus <= 0)
		return -ENODEV;
	for (i = 0; i < nr_cpus; i++) {
		cpu_node = of_parse_phandle(pmu_node, "cpus", i);
		if (!cpu_node)
			continue;
		cpu_tmp = of_cpu_node_to_id(cpu_node);
		if (cpu_tmp < 0) {
			of_node_put(cpu_node);
			continue;
		}
		l3c->pmu_associative_cpu = cpu_tmp;
		of_node_put(cpu_node);
		break;
	}
	if (i == nr_cpus) {
		pr_err("pmu_associative_cpu not found, not register l3c_devfreq\n");
		return -ENODEV;
	} else {
		pr_info("pmu_associative_cpu = %d\n", l3c->pmu_associative_cpu);
	}
	platform_set_drvdata(pdev, l3c);

	of_node_put(pmu_node);
	return 0;
}
...
static int l3c_devfreq_hwmon_setup(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(hw))
		return -ENOMEM;
	dev_err(&pdev->dev, "Line%d l3c->pmu_associative_cpu = %u\n",
		__LINE__, l3c->pmu_associative_cpu);

	mutex_init(&hw->l3c_hw_data.active_lock);
	l3c->hw = hw;
	ret = l3c_devfreq_start_hwmon(l3c);
	if (ret != 0) {
		dev_err(&pdev->dev, "start hwmon failed in setup.\n");
		return -ENODEV;
	}
...
static int l3c_devfreq_probe(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, l3c);
	l3c->pdev = pdev;
	ret = get_l3c_pmu_associative_cpu_any(pdev);
	if (ret != 0) {
		dev_err(&pdev->dev, "get_l3c_pmu_associative_cpu_any failed %d\n", ret);
		goto failed;
	}
	...
	if (l3c->hv_supported != 0) {
		...
		if (ret != 0)
			dev_err(&pdev->dev, "set init freq fail\n");
	} else {
		l3c->l3c_hvdev = NULL;
	}
#endif
