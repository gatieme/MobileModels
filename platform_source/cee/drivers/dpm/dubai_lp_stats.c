...
int ioctrl_get_ddr_ap_vote_num(void __user *argp)
{
	if (argp == NULL)
		return -EFAULT;
	if (platform_qos_debug_record(PLATFORM_QOS_MEMORY_LATENCY) != 0)
		pr_err("update_ddr_ap_vote_latency failed!\n");
	if (platform_qos_debug_record(PLATFORM_QOS_MEMORY_THROUGHPUT) != 0)
		pr_err("update_ddr_ap_vote_throughput failed!\n");
...
int ioctrl_ddr_ap_vote_data(void __user *argp)
{
	int ret;
