...
#define RAS_NAMTSO_READ                        0x00
#define RAS_NAMTSO_INT_CLEAR                   0x01

noinline u64 atfd_ras_smc(u64 _arg0, u64 _arg1)
{
	struct arm_smccc_res res;

	arm_smccc_1_1_smc(RAS_FID_VALUE, _arg0, _arg1, &res);
	if (res.a0 != 0) {
...
static u64 read_namtso_errstatus(void)
{
	u64 ret;

	ret = atfd_ras_smc((u64)RAS_NAMTSO_READ, (u64)ERRXSTATUS);
	return ret;
}

static u64 read_namtso_errmisc0(void)
{
	u64 ret;

	ret = atfd_ras_smc((u64)RAS_NAMTSO_READ, (u64)ERRXMISC0);
	return ret;
}

static u64 read_namtso_errmisc1(void)
{
	u64 ret;

	ret = atfd_ras_smc((u64)RAS_NAMTSO_READ, (u64)ERRXMISC1);
	return ret;
}

static void namtso_clear(void)
{
	(void)atfd_ras_smc((u64)RAS_NAMTSO_INT_CLEAR, (u64)NULL);
}
#endif
...
static void parse(int irq, struct edac_device_ctl_info *edac_dev)
{
	...
	for (i = 0; i < instance->nr_blocks; i++) {
		if (pdata->access_type == MEM) {
			...
			errstatus = read_namtso_errstatus();
			errmisc0 = read_namtso_errmisc0();
#endif
		} else {
