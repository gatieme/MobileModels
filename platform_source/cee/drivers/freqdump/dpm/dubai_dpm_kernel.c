...
int ioctl_dpm_enable_module(const void __user *argp)
{
	...
	ret = (int)copy_from_user(&dpm_switch_cmd, argp, sizeof(dpm_switch_cmd));
	if (ret != 0)
...
int ioctl_dpm_get_peri_num(void __user *argp)
{
	...
	ret = (int)copy_to_user(argp, &peri_ip_num, sizeof(peri_ip_num));
	if (ret != 0)
		pr_err("%s failed, ret is %d\n", __func__, ret);
...
int ioctl_dpm_get_peri_data(void __user *argp)
{
	...
	unsigned int peri_ip_num;
	size_t peri_data_size;
	struct dubai_transmit_t *peri_data = NULL;
	size_t ip_size = sizeof(struct ip_energy);
	...
	ret = (int)copy_from_user(&peri_ip_size, argp, sizeof(peri_ip_size));
	if (ret != 0) {
	...
	ret = (int)copy_to_user(argp, peri_data, peri_data_size);
	if (ret != 0) {
...
int ioctl_dpm_get_gpu_data(void __user *argp)
{
	...
	ret = (int)copy_to_user(argp, &energy, sizeof(energy));
	if (ret != 0)
		pr_err("%s failed, ret is %d\n", __func__, ret);
...
int ioctl_dpm_get_mdm_num(void __user *argp)
{
	...
	ret = (int)copy_to_user(argp, &mdm_num, sizeof(mdm_num));
	if (ret != 0)
		pr_err("%s failed, ret is %d\n", __func__, ret);
...
int ioctl_dpm_get_mdm_data(void __user *argp)
{
	...
	ret = (int)copy_from_user(&mdm_ip_size, argp, sizeof(mdm_ip_size));
	if (ret != 0) {
		pr_err("%s copy from user failed\n", __func__);
		goto err_handler;
	}
	...
	ret = (int)copy_to_user(argp, mdm_data, mdm_data_size);
	if (ret != 0) {
		pr_err("%s copy to user failed\n", __func__);
		goto err_handler;
	}
