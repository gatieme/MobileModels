...
noinline int atfd_service_freqdump_smc(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
	...
	struct ffa_send_direct_data args = {
		...
		.data1 = _arg0,
		.data2 = _arg2,
		.data4 = SHARED_MEMORY_SIZE,
	};

	if (_arg1 == 0) { /* not use shared memory */
		...
		ret = ffa_platdrv_send_msg_with_shmem(&args, g_loadmonitor_virt_addr, SHMEM_INOUT);
		if (ret !=0) {
			pr_err("%s %d, send ffa msg to tzsp with shared memory fail\n", __func__, __LINE__);
			return -EINVAL;
		}
	}
	return (int)_function_id;
}
#else
noinline int atfd_service_freqdump_smc(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
