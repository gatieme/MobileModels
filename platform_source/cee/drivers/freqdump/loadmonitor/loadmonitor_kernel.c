...
#include <platform_include/cee/linux/dubai_lp_stats.h>
#endif
...
static int ioctrl_get_ip_len(void __user *argp)
{
	...
	dpm_ipnum = (u32)(MAX_NUM_LOGIC_IP_DEVICE + 1);
...
static int ioctrl_get_ddr_freq_info(void __user *argp)
{
	...
	ddr_data_len = (IP_NAME_SIZE + ((size_t)ddr_freq_num * sizeof(u64))) * DDR_INFO_LEN;
...
static struct ioctrl_dpm_ldm_t dpm_ldm_func_table[] = {
	...
	{ DPM_IOCTL_DDR_AP_VOTE_NUM, ioctrl_get_ddr_ap_vote_num },
	{ DPM_IOCTL_DDR_AP_VOTE_DATA, ioctrl_ddr_ap_vote_data },
	{ DPM_IOCTL_DDR_VOTE_DATA_CLR, ioctrl_clr_ddr_vote_data },
#ifdef CONFIG_LOWPM_DDR_STATE
	{ DPM_IOCTL_DDR_STATE_NUM, ioctrl_get_ddr_state_num },
	{ DPM_IOCTL_DDR_STATE_DATA, ioctrl_ddr_state_data },
#endif
#endif
...
#ifdef CONFIG_DUBAI_LOADMONITOR
...
#else
static long dpm_loadmonitor_ioctrl(struct file *file __maybe_unused, u_int cmd __maybe_unused,
				   u_long arg __maybe_unused)
{
	return -EINVAL;
}
#endif
...
static bool ip_need_calc(struct ips_info *info, unsigned int ip_index)
{
	return ((info->flag &
		g_monitor_delay_value.monitor_enable_flags) ==
		info->flag && (info->mask & BIT(ip_index)) != 0);
}
...
static int set_device_sample_status(struct ips_info *info,
				    ...
				    unsigned int dev_index, unsigned int sensor_index)
{
	u64 idle_nor_time;
	u64 busy_nor_time;
	u64 busy_low_time;
	...
	if (is_ao_data(info)) {
		...
		if (idle_nor_time + busy_nor_time + busy_low_time > 1000) {
			...
			if (g_print_err_data == -1 ||
				...
				pr_info("%s freq:%u, idle:%u, norm:%u, low:%u, dev_index:%u, sensor_index:%u\n",
	...
	ret = sprintf_s(g_monitor_info.mdev[dev_index].name,
			IP_NAME_MAX_SIZE, "%c%02u",
			info->name,
			sensor_index);
	if (ret == -1) {
		pr_err("%s sprintf_s err device type:%c, index:%u\n",
		       __func__, info->name, sensor_index);
		return ret;
	}
	g_monitor_info.mdev[dev_index].idle_nor_time += (s64)idle_nor_time;
	g_monitor_info.mdev[dev_index].busy_nor_time += (s64)busy_nor_time;
	g_monitor_info.mdev[dev_index].busy_low_time += (s64)busy_low_time;
	g_monitor_info.mdev[dev_index].off_time += (s64)off_time;
	return ret;
}
...
static void calc_monitor_time(void)
{
	...
	size_t ips_num = sizeof(struct loadmonitor_ips) / sizeof(struct ips_info);
...
static int __init dpmldm_init(void)
{
	...
	INIT_DEFERRABLE_WORK(&g_dpm_delayed_work, update_dpm_data);
