...
#define NPU_PM_TSENSOR_NAME		"gpu"
...
static bool npu_is_low_temperature(void)
{
	...
	if ((g_dvfs_data.test & BIT(NPU_LOW_TEMP_DEBUG)) > 0)
		return ((g_dvfs_data.test & BIT(NPU_LOW_TEMP_STATE)) > 0);
#endif
...
{
#ifndef CONFIG_NPU_PM_SHARE_PERI_BUCK
...
int npu_pm_get_module_voltage(const struct npu_pm_module_profile *module_profile)
{
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
	...
	if (strcmp(module_profile->module_name,
		   g_dvfs_data.main_module->module_name) == 0) {
		...
		if (vote_id < (unsigned int)module_profile->profile_count)
			voltage_uv = (int)module_profile->profile_table[vote_id].voltage;
		else
			pr_err("[npu dvfs]%s Failed to read peri volt, id%u\n",
			       __func__, vote_id);
...
int npu_pm_set_module_voltage(const struct npu_pm_module_profile *module_profile,
...
int npu_pm_find_profile_id(const struct npu_pm_module_profile *module_profile,
			     unsigned long *freq)
{
	int id;
...
static struct clk *npu_pm_get_cur_clk(const struct npu_pm_module_profile *module_profile)
{
...
unsigned long npu_pm_get_cur_clk_rate(const struct npu_pm_module_profile *module_profile)
{
	u32 clk_div;
	...
	cur_clk = npu_pm_get_cur_clk(module_profile);
...
static int npu_pm_find_absolute_profile_id(const struct npu_pm_module_profile *module_profile,
					     unsigned int freq)
{
	int id;
...
static int npu_pm_update_temp_stat(void)
{
	bool is_low_temp;
	int stat = TEMP_ERROR;
...
static void npu_pm_clr_temp_stat(void)
{
	g_dvfs_data.temp_stat = TEMP_NORMAL;
}

#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
#define NPU_PM_VOTE_PERI_VOLT_TIMEOUT		5000 /* us */
static int npu_pm_vote_peri_voltage(int tar_pid)
{
	int ret = 0;
	int vote_timeout = NPU_PM_VOTE_PERI_VOLT_TIMEOUT;
	...
	while (vote_timeout > 0) {
		/*
		 * case 1. vote voltage up: Wait here
		 * until the voltage readout >= the vote voltage
		 * case 2. vote voltage down: Other IP may vote a high voltage,
		 * only vote and no need to wait
		 */
	...
	if (ret != 0)
		pr_err("[npu dvfs]%s wait avs status update fail, ret %d\n",
		       __func__, ret);
...
static unsigned int get_avs_voltage(struct npu_pm_module_profile *module_profile,
				    int cur_pid,
				    int tar_pid)
{
...
int npu_pm_voltage_step_up(int cur_pid, int tar_pid, int temp_stat)
{
	int ret = 0;
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
	(void)temp_stat;
	...
	if (cur_pid < tar_pid)
		ret = npu_pm_vote_peri_voltage(tar_pid);
...
int npu_pm_voltage_step_down(int cur_pid, int tar_pid, int temp_stat)
{
	int ret = 0;
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
	(void)temp_stat;
...
int npu_pm_voltage_update(int cur_pid, int tar_pid, int temp_stat)
{
	struct npu_pm_module_profile *module_profile = NULL;
	struct regulator *buck = NULL;
	unsigned int tar_voltage;
...
static void npu_pm_set_clk_freq(const struct npu_pm_module_profile *module_profile,
				  int tar_pid)
{
 ...
 */
static bool npu_pm_check_clk_freq_conflict(const struct npu_pm_module_profile *module_a,
					     int pid_a,
					     const struct npu_pm_module_profile *module_b,
					     int pid_b)
{
...
static int npu_pm_set_module_profile(struct npu_pm_module_profile *module_profile,
				       int cur_pid, int tar_pid)
{
	...
	if (npu_pm_check_clk_freq_conflict(module_profile, cur_pid,
...
static int npu_pm_same_module_clk_conflict_handle(struct npu_pm_module_profile *module_profile,
						    int tar_pid)
{
	int i;
	int ret;
	 ...
	 */
	ret = npu_pm_set_module_profile(module_profile,
...
static int npu_pm_dvfs_module_clk_cfg(int cur_pid, int tar_pid)
{
	int ret;
	int index_a;
	int index_b;
	struct npu_pm_module_profile *module_a = NULL;
	struct npu_pm_module_profile *module_b = NULL;
	...
	for (index_a = 0; index_a < g_dvfs_data.module_count; index_a++) {
		...
		for (index_b = index_a; index_b < g_dvfs_data.module_count; index_b++) {
			 ...
			 */
			if (!npu_pm_check_clk_freq_conflict(module_a,
							      tar_pid,
							      module_b,
							      cur_pid))
				continue;
			...
			if (index_a == index_b) {
				ret = npu_pm_same_module_clk_conflict_handle(module_a,
									       tar_pid);
				if (ret != 0)
					return ret;
				continue;
			...
			ret = npu_pm_set_module_profile(module_b,
							  module_b->cur_pid,
							  tar_pid);
			if (ret != 0) {
		...
		npu_pm_set_clk_freq(module_a, tar_pid);
		ret = npu_pm_set_module_profile(module_a,
						  module_a->cur_pid,
						  tar_pid);
		if (ret != 0) {
			pr_err("[npu dvfs] Failed set %s [%d] -> [%d]\n",
...
int npu_pm_dvfs_hal(unsigned long target_freq)
{
	...
	temp_stat = npu_pm_update_temp_stat();
	...
	tar_pid = npu_pm_find_profile_id(g_dvfs_data.main_module,
					   &target_freq);
	last_freq = g_dvfs_data.current_freq;
	g_dvfs_data.current_freq = target_freq;

	cur_pid = npu_pm_find_absolute_profile_id(g_dvfs_data.main_module,
						    last_freq);
	if (cur_pid < 0) {
	...
	if (cur_pid == tar_pid) {
		ret = npu_pm_voltage_update(cur_pid, tar_pid, temp_stat);
		if (ret != 0)
			pr_err("[npu dvfs] failed to update voltage\n");
		return ret;
	}
	pr_debug("%s %d: [npu dvfs] cur_pid%d, tar_pid%d, target_freq%lu, temp_stat%d\n",
		__func__, __LINE__, cur_pid, tar_pid, target_freq, temp_stat);

	ret = npu_pm_voltage_step_up(cur_pid, tar_pid, temp_stat);
	if (ret != 0) {
		pr_err("[npu dvfs] %s voltage step-up fail\n", __func__);
		return ret;
	}

	ret = npu_pm_dvfs_module_clk_cfg(cur_pid, tar_pid);
	if (ret != 0) {
		pr_err("[npu dvfs] %s clk cfg fail\n", __func__);
		return ret;
	}

	ret = npu_pm_voltage_step_down(cur_pid, tar_pid, temp_stat);
	if (ret != 0) {
...
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
static int npu_pm_set_peri_voltage(const struct npu_pm_module_profile *module_profile,
				     int tar_pid)
{
	int ret = 0;

	/* Only set the voltage for main module */
	if (strncmp(module_profile->module_name,
		    g_dvfs_data.main_module->module_name,
		    strlen(module_profile->module_name)) == 0) {
...
#else
static int npu_pm_module_buck_on(struct npu_pm_module_profile *module_profile,
				   int tar_pid, int temp_stat)
{
	struct npu_pm_profile *profile = NULL;
	struct regulator *buck = NULL;
	unsigned int tar_voltage;
	int ret;
#ifdef CONFIG_NPU_PM_AVS_VDM
...
static int npu_pm_voltage_init(struct npu_pm_module_profile *module_profile,
				 int tar_pid, int temp_stat)
{
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
	(void)temp_stat;
	return npu_pm_set_peri_voltage(module_profile, tar_pid);
#else
	return npu_pm_module_buck_on(module_profile, tar_pid, temp_stat);
#endif
}

int npu_pm_profile_hal_init(unsigned long target_freq)
{
	struct npu_pm_module_profile *module_profile = NULL;
	...
	for (module = 0; module < g_dvfs_data.module_count; module++) {
		...
		/* find clk by cur_pll */
		cur_pll_clk = npu_pm_get_cur_clk(module_profile);
		...
		npu_pm_set_clk_freq(module_profile, tar_pid);
		...
		ret = npu_pm_voltage_init(module_profile, tar_pid, temp_stat);
		if (ret != 0) {
			pr_err("[npu dvfs] enable buck failed%d\n", ret);
			goto err_hand1;
		}
...
void npu_pm_profile_hal_exit(void)
{
	int module;
	struct regulator *buck = NULL;
	struct clk *clk = NULL;
...
#ifdef CONFIG_NPU_PM_SHARE_PERI_BUCK
	ret = npu_pm_vote_peri_voltage(0);
	if (ret != 0)
		pr_err("[npu dvfs] disable buck failed%d\n", ret);
	...
	npu_pm_clr_temp_stat();
}

int npu_pm_dt_buck_data_parse(struct device *dev,
...
static int npu_pm_dt_low_temp_data_parse(struct device *dev,
					   struct device_node *module_np,
					   struct npu_pm_module_profile *module_profile)
{
...
static int npu_pm_dt_reg_data_parse(struct device *dev,
				      struct device_node *module_np,
				      struct npu_pm_module_profile *module_profile)
{
...
int npu_pm_clock_init(struct device *dev)
{
...
struct clk *npu_pm_find_clock(const char *clock_name)
{
	struct npn_clk_list *list_node = NULL;
...
static int npu_pm_dt_module_parse(struct device *dev,
				    struct device_node *module_np,
				    struct npu_pm_module_profile *module_profile)
{
	struct device_node *child = NULL;
	struct device_node *parent = NULL;
	struct npu_pm_profile *profile_table = NULL;
	struct npu_pm_profile *profile = NULL;
	int profile_count;
	const char *clk_name = NULL;
	int ret;
	...
	for_each_child_of_node(parent, child) {
		...
		profile->pll_clk = npu_pm_find_clock(clk_name);
...
static int npu_pm_dt_profile_parse(struct device *dev,
				     struct device_node *module_np,
				     struct npu_pm_module_profile *module_profile)
{
	...
	for (index = 0; index < clock_count; index++) {
		...
		list_node->clk    = npu_pm_find_clock(clk_name);
		list_node->clk_sw = clk_switch;
		list_add_tail(&list_node->list, &module_profile->clk_list.list);
	}

	ret = npu_pm_dt_module_parse(dev, module_np, module_profile);
	if (ret != 0)
...
void npu_pm_dt_ppll_parse(struct device_node *root)
{
	int ret;
...
int npu_pm_profile_data_init(struct device *dev)
{
	...
	g_dvfs_data.module_profile = devm_kzalloc(dev,
						  sizeof(struct npu_pm_module_profile) *
						  module_count,
						  GFP_KERNEL);
	...
	for_each_node_by_type(module_np, "npu-dvfs-module") {
		...
		ret = npu_pm_dt_buck_data_parse(dev, module_np,
						  module_profile);
		if (ret != 0) {
		...
		ret = npu_pm_dt_low_temp_data_parse(dev, module_np,
						      module_profile);
		if (ret != 0) {
		...
		ret = npu_pm_dt_reg_data_parse(dev, module_np,
						 module_profile);
		if (ret != 0) {
		...
		ret = npu_pm_dt_profile_parse(dev, module_np,
						module_profile);
		if (ret != 0) {
		...
		ret = npu_pm_dt_avs_info_parse(dev, module_np,
						 module_profile,
						 main_module_name);
		if (ret != 0) {
...
static int npu_pm_thermal_init(struct device *dev)
{
#ifdef CONFIG_THERMAL_TSENSOR
	g_dvfs_data.tsensor = ipa_get_tsensor_id(NPU_PM_TSENSOR_NAME);
	if (g_dvfs_data.tsensor < 0) {
...
int npu_pm_dvfs_init(struct npu_pm_device *pmdev)
{
	...
	ret = npu_pm_thermal_init(dev);
	if (ret != 0)
		dev_err(dev, "%s get tsensor id failed\n", __func__);

	ret = npu_pm_clock_init(dev);
	if (ret != 0) {
		dev_err(dev, "%s npu clk node parse failed\n", __func__);
		return ret;
	}

	ret = npu_pm_profile_data_init(dev);
	if (ret != 0) {
