...
#define AVS_STD_SVC_VER		0xc9000000
#define AVS_SVC_CALL_COUNT		0xc9000001
#define AVS_SVC_UID		0xC9000002
#define AVS_SVC_VERSION		0xC9000003
#define AVS_NPU_PA_MODE_CFG		0xC900DD01
#define AVS_NPU_PA_MODE_RECOVERY		0xC900DD02
#define AVS_NPU_GET_PA_VALUE		0xC900DD03
...
struct npu_pm_module_profile {
	const char *module_name;
	struct npu_pm_profile *profile_table;
	int profile_count;
	int cur_pid;
...
struct npu_pm_dvfs_data {
	...
	int module_count;
	struct npu_pm_module_profile *main_module;
	bool dvfs_enable;
...
void npu_update_svfd_code(struct npu_pm_module_profile *module_profile,
			  int target);
int npu_pm_dt_avs_info_parse(struct device *dev,
...
unsigned int npu_cluster_avs(struct npu_pm_module_profile *module_profile,
			     unsigned int open_switch);
#else
static inline int npu_pm_dt_avs_info_parse(struct device *dev,
					     struct device_node *module_np,
					     struct npu_pm_module_profile *module_profile,
					     const char *main_module_name)
{
...
static inline void npu_update_svfd_code(struct npu_pm_module_profile *module_profile,
					int target)
{
	(void)module_profile;
	(void)target;
...
static inline unsigned long npu_pm_get_cur_clk_rate(const struct npu_pm_module_profile *module_profile)
{
	(void)*module_profile;
	return 0;
}

static inline int npu_pm_get_module_voltage(const struct npu_pm_module_profile *module_profile)
{
	(void)*module_profile;
	return 0;
}

static int npu_pm_set_module_voltage(const struct npu_pm_module_profile *module_profile,
				       unsigned int voltage)
{
	(void)*module_profile;
	(void)voltage;
...
static inline int npu_pm_profile_hal_init(unsigned long target_freq)
{
	(void)target_freq;
	return 0;
}

static inline void npu_pm_profile_hal_exit(void)
{
}
