...
#else
	g_thermal_max_freq[policy->cpu] = min((u32)freq_qos_read_value(&policy->constraints, FREQ_QOS_MAX), freq);
#endif
}

void update_cpu_freq_limit_counter(struct cpufreq_policy *policy,
				   unsigned int target_freq)
{
	int cpu = policy->cpu;

	if (test_fast_cpu(cpu)) {
...
void cpufreq_get_limit_record(unsigned int normal_counts[],
			      unsigned int reach_max_counts[],
...
again:
	...
	if (!cpumask_intersects(cpu_active_mask, &mask)) {
		retval = -EINVAL;
		pr_err("cpumask intersects is fail, pid is :%d\n", pid);
		goto out_unlock;
	}
#endif
...
static ssize_t perfhub_show(struct kobject *kobj,
			    struct kobj_attribute *attr, char *buf)
{
	ssize_t count;

	mutex_lock(&g_perfhub_mutex);
	count = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%x|%d\n", g_last_mask, g_last_pid);
	mutex_unlock(&g_perfhub_mutex);

	return count;
}

static ssize_t ipa_freq_limit_status_show(struct kobject *kobj,
					  struct kobj_attribute *attr, char *buf)
{
#ifdef CONFIG_FREQ_LIMIT_COUNTER
	...
	for (i = 0; i < NR_CPUS; ++i)
		write_len += scnprintf(buf + write_len, PAGE_SIZE, ";%u %u %u",
				       cpu_freq_normal_counts[i],
				       cpu_limit_cout[i], cpu_max_freq_record[i]);
	return write_len;
#else
	return 0;
#endif
...
static ssize_t perfhub_store(struct kobject *kobj, struct kobj_attribute *attr,
			     const char *buf, size_t count)
{
	...
	ret = strcpy_s(mask_str, sizeof(mask_str)-1, buf);
	if (ret != EOK) {
		pr_err("%s(): invalid input buf: %s\n", __func__, buf);
		return -EINVAL;
	}

	pstrmask = strtok_s(mask_str, delimiter, &next_p);
	if (pstrmask) {
		pstrchr = strtok_s(NULL, delimiter, &next_p);
	} else {
		pr_err("invalid mask_str\n");
		return -EINVAL;
	}
	...
	ret = kstrtol(pstrmask, 16, &cpumask);  // 16 is local used
	if (ret != 0) {
		pr_err("invalid pstrmask, ret = %d\n", ret);
		return -EINVAL;
	}

	ret = kstrtol(pstrchr, 10, &pid);  // 10 is local used
	if (ret != 0) {
	...
	if (cpumask < 0 || cpumask > ((1 << NR_CPUS) - 1)) {
...
struct kobj_attribute perfhub_attribute = __ATTR(cpuaffinity, 0660, perfhub_show, perfhub_store);  // 0660 is local used
struct kobj_attribute ipa_freq_limit_attribute = __ATTR_RO(ipa_freq_limit_status);

static struct kobject *perfhub_kobj;
...
static int __init perfhub_init(void)
{
	...
	ret = sysfs_create_file(perfhub_kobj, &perfhub_attribute.attr);
	if (ret)
		goto perfhub_file_err;

	ret = sysfs_create_file(perfhub_kobj, &ipa_freq_limit_attribute.attr);
	if (ret)
		goto ipa_file_err;
