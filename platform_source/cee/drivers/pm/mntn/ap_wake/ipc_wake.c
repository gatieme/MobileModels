...
#ifdef SOC_IPCNS_AO_MBX_DATA0_ADDR_PM
#include <soc_ipcns_ao_interface.h>
#define SOC_IPCNSAO_MBX_DATA0_ADDR SOC_IPCNS_AO_MBX_DATA0_ADDR
#ifdef CONFIG_IPC_V5
#define SOC_IPCNSAO_MBX_DATA1_ADDR SOC_IPCNS_AO_MBX_DATA1_ADDR
#endif
#define SOC_IPCNSAO_MBX_DATA2_ADDR SOC_IPCNS_AO_MBX_DATA2_ADDR
#define SOC_IPCNSAO_MBX_SOURCE_ADDR SOC_IPCNS_AO_MBX_SOURCE_ADDR
#define SOC_IPCNSAO_MBX_DSTATUS_ADDR SOC_IPCNS_AO_MBX_DSTATUS_ADDR
#else
#include <soc_ipcnsao_interface.h>
#endif
...
#ifdef CONFIG_IPC_V5
	unsigned int data1_offset;
	data1_offset = (ipc->reg_domain == IPC) ? SOC_IPCNS_MBX_DATA1_ADDR(0, mbx) : SOC_IPCNSAO_MBX_DATA1_ADDR(0, mbx);
	ipc_data = read_reg(ipc->reg_domain, data1_offset);
	mem = get_sharemem_data(ipc_data);
	source = 0;
#else
	unsigned int data0_ofset, data2_ofset;
	data0_ofset = (ipc->reg_domain == IPC) ? SOC_IPCNS_MBX_DATA0_ADDR(0, mbx) : SOC_IPCNSAO_MBX_DATA0_ADDR(0, mbx);
	data2_ofset = (ipc->reg_domain == IPC) ? SOC_IPCNS_MBX_DATA2_ADDR(0, mbx) : SOC_IPCNSAO_MBX_DATA2_ADDR(0, mbx);

	ipc_data = read_reg(ipc->reg_domain, data0_ofset);
	mem = get_sharemem_data(ipc_data);

	ipc_data = read_reg(ipc->reg_domain, data2_ofset);
	source = get_sharemem_source(ipc_data);
#endif
	dubai_log_wakeup_info("DUBAI_TAG_SENSORHUB_WAKEUP", "mem=%u source=%u", mem, source);
...
#define ipcns_mbx_data_offset(mbx, idex) SOC_IPCNS_MBX_DATA0_ADDR(4 * (idex), mbx)
#define ipcnsao_mbx_data_offset(mbx, idex) SOC_IPCNSAO_MBX_DATA0_ADDR(4 * (idex), mbx)

static void ipc_mbx_info_show(struct seq_file *s, unsigned int mbx, const struct wake_ipc_info *ipc)
{
	int i, src_id, dest_id, src_offset, dst_offset, data_offset;
	unsigned int ipc_source, ipc_dest;
	const int cpunum = ipc->cpunum;

	lp_msg_cont(s, "mbx-%d:%s, ", mbx, ipc->mbxs[mbx].name);

	src_offset = (ipc->reg_domain == IPC) ? SOC_IPCNS_MBX_SOURCE_ADDR(0, mbx) : SOC_IPCNSAO_MBX_SOURCE_ADDR(0, mbx);
	dst_offset = (ipc->reg_domain == IPC) ? SOC_IPCNS_MBX_DSTATUS_ADDR(0, mbx) : SOC_IPCNSAO_MBX_DSTATUS_ADDR(0, mbx);
	ipc_source = read_reg(ipc->reg_domain, src_offset);
	src_id = proc_mask_to_id(ipc_source);
	ipc_dest = read_reg(ipc->reg_domain, dst_offset);
	...
	for (i = 0; i < IPC_MBXDATA_MAX; i++) {
		data_offset = (ipc->reg_domain == IPC) ? ipcns_mbx_data_offset(mbx, i) : ipcnsao_mbx_data_offset(mbx, i);
		lp_msg_cont(s, "[0x%x]", read_reg(ipc->reg_domain, data_offset));
	}
...
static int read_ipc_mbx_info(struct device_node *ipc_dn, struct wake_ipc_info *ipc_info)
{
	...
	ipc_info->mbxs = kcalloc(ipc_info->mbxnum, sizeof(struct ipc_mbx_info), GFP_KERNEL);
