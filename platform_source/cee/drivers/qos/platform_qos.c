...
#define PLATFORM_QOS_DEBUG_MAX_NUM    (10)
...
{
	if (platform_qos_class >= PLATFORM_QOS_NUM_CLASSES || platform_qos_class <= PLATFORM_QOS_RESERVED) {
		pr_err("%s: bad qos class!\n", __func__);
		return -EINVAL;
	}

	struct platform_qos_object *qos = qos_array[platform_qos_class];
	...
	if (plist_head_empty(&c->list)) {
		pr_info("platform qos class:%d Empty!\n", platform_qos_class);
		goto out;
	}

	plist_for_each_entry(req, &c->list, node) {
		if (req->node.prio != c->default_value && active_reqs < PLATFORM_QOS_DEBUG_MAX_NUM) {
			active_reqs++;
#ifdef CONFIG_FREQ_PLATFORM_QOS_DEBUG
			pr_info("%d: class:%d prio:%d owner:%s\n",
				active_reqs, req->platform_qos_class, (req->node).prio, req->owner);
#else
			pr_info("%d: %d\n", active_reqs, (req->node).prio);
#endif
		}
	}
				   ...
				   void *unused __maybe_unused)
{
	...
	plist_for_each_entry(req, &c->list, node) {
		...
		tot_reqs++;
#ifdef CONFIG_FREQ_PLATFORM_QOS_DEBUG
		seq_printf(s, "%d: class:%d prio:%d owner:%s state:%s\n",
			tot_reqs, req->platform_qos_class, (req->node).prio, req->owner, state);
		pr_info("%d: class:%d prio:%d owner:%s state:%s\n",
			tot_reqs, req->platform_qos_class, (req->node).prio, req->owner, state);
#else
		seq_printf(s, "%d: %d: %s\n", tot_reqs,
			(req->node).prio, state);
			      ...
			      int platform_qos_class, s32 value)
{
	...
	(void)memset_s(req->owner, sizeof(req->owner), 0, sizeof(req->owner));
	(void)snprintf_s(req->owner, sizeof(req->owner) - 1, sizeof(req->owner) - 1,
			"%s pid:%d tid:%d time:%ld", current->comm, task_tgid_nr(current), task_pid_nr(current), ts.tv_sec);
#endif
}
