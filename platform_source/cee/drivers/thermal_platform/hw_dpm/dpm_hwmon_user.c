...
#if defined(CONFIG_DPM_HWMON_DEBUG) && defined(CONFIG_DFX_DEBUG_FS)
...
static void dpm_fit(struct dpm_hwmon_ops *pos, int timer_span, int total_count, int mode)
{
#ifdef CONFIG_DPM_HWMON_PWRSTAT
	powerstat_config(pos, timer_span, total_count, mode);
#else
	int i;
...
#endif
	pr_err("dpm [%d] enabled is %d\n", pos->dpm_module_id, pos->module_enabled);
}
			       ...
			       size_t count, loff_t *f_pos)
{
	...
	dpm_fit(pos, timer_span_ms, total_count, mode);
