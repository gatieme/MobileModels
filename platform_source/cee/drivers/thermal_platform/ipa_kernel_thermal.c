...
#ifdef CONFIG_IPA_LIMIT_RESET
	cpufreq_update_policies(false);
	gpufreq_update_policies(false);
#endif
...
#ifdef CONFIG_IPA_MNTN_INFO
#define cluster_real_freq(up, low) ((up) < (low) ? (up) : (low))
unsigned int g_min_lt_freq = UINT_MAX;
unsigned int g_min_md_freq = UINT_MAX;
unsigned int g_min_bg_freq = UINT_MAX;
int g_temp = -100;
...
static void reset_ipa_mntn_para_info()
{
	int i;
	...
	g_min_lt_freq = UINT_MAX;
	g_min_md_freq = UINT_MAX;
	g_min_bg_freq = UINT_MAX;
	g_temp = -100;
}

static void print_ipa_mntn_info(void)
{
	pr_err("ipa mntn info [little to big] max_temp:%d, soc_limit_freq:[%d-%d-%d], \
		soc_brd_freq:[%d-%d-%d], min_real_freq[%d-%d-%d]\n", g_temp,
		...
		g_min_lt_freq, g_min_md_freq, g_min_bg_freq);
}

static void ipa_get_mntn_info(unsigned int cur_cluster, unsigned int *min_lt_freq,
				unsigned int *min_md_freq, unsigned int *min_bg_freq)
{
	unsigned int lw_freq, lu_freq, mw_freq, mu_freq, bw_freq, bu_freq, l_freq, m_freq, b_freq;
	int temperature;
	...
	if (g_ipa_mntn_cpu_freq_base != NULL) {
		lu_freq = readl(g_ipa_mntn_cpu_freq_base + LITTLE_FREQ_UP_LIMIT_SHIFT_ADDR);
		lw_freq = readl(g_ipa_mntn_cpu_freq_base + LITTLE_FREQ_LW_LIMIT_SHIFT_ADDR);
		mu_freq = readl(g_ipa_mntn_cpu_freq_base + MIDDLE_FREQ_UP_LIMIT_SHIFT_ADDR);
		mw_freq = readl(g_ipa_mntn_cpu_freq_base + MIDDLE_FREQ_LW_LIMIT_SHIFT_ADDR);
		bu_freq = readl(g_ipa_mntn_cpu_freq_base + BIG_FREQ_UP_LIMIT_SHIFT_ADDR);
		bw_freq = readl(g_ipa_mntn_cpu_freq_base + BIG_FREQ_LW_LIMIT_SHIFT_ADDR);
	}
	l_freq = cluster_real_freq(lu_freq, lw_freq);
	m_freq = cluster_real_freq(mu_freq, mw_freq);
	b_freq = cluster_real_freq(bu_freq, bw_freq);
	if (l_freq < *min_lt_freq)
		*min_lt_freq = l_freq;
	if (m_freq < *min_md_freq)
		*min_md_freq = m_freq;
	if (b_freq < *min_bg_freq)
		*min_bg_freq = b_freq;
}
				 ...
				 unsigned long state, unsigned int cur_cluster)
{
	...
	ipa_get_mntn_info(cur_cluster, &g_min_lt_freq, &g_min_md_freq, &g_min_bg_freq);
	if (time_after(jiffies, pre_jiffies + HZ) && (state > cpufreq_cdev->cpufreq_state)) {
		print_ipa_mntn_info();
