...
#ifdef CONFIG_IPA_MNTN_INFO
void __iomem *g_ipa_mntn_cpu_freq_base;
...
static void ipa_mntn_iounmap(void)
{
	if (g_ipa_mntn_cpu_freq_base != NULL) {
		iounmap(g_ipa_mntn_cpu_freq_base);
		g_ipa_mntn_cpu_freq_base = NULL;
	}
}

static void ipa_mntn_ioremap(void)
{
	g_ipa_mntn_cpu_freq_base = ioremap(IPA_MNTN_CPU_FREQ_BASE, IPA_MNTN_CPU_FREQ_SIZE);
...
static int ipa_thermal_probe(struct platform_device *pdev)
{
	...
	if (policy == NULL || !policy->freq_table) {
