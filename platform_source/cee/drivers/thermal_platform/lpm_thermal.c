...
void update_pid_value(struct thermal_zone_device *tz)
{
	...
	mutex_unlock(&tz->lock);
#ifndef CONFIG_LIBLINUX
	if (strcmp(tz->type, "board_thermal") == 0)
		trace_perf(clock_set_rate, "board_sustainablepower", sustainable_power, raw_smp_processor_id());
	if (strcmp(tz->type, "soc_thermal") == 0)
		trace_perf(clock_set_rate, "soc_sustainablepower", sustainable_power, raw_smp_processor_id());
#endif
}
...
int gpufreq_update_policies(bool updatepolicy)
{
...
int cpufreq_update_policies(bool updatepolicy)
{
