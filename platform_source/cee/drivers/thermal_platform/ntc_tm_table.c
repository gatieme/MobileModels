...
/* DCXO: 51k table {volt(mv), temp(C)} */
int g_thermal_hkadc_volt_temp_51k_table[][2] = {
	{98, 120}, {113, 115}, {130, 110}, {150, 105}, {173, 100}, {200, 95},
	{231, 90}, {267, 85}, {309, 80}, {358, 75}, {413, 70}, {476, 65},
	{546, 60}, {624, 55}, {710, 50}, {801, 45}, {897, 40}, {996, 35},
	{1095, 30}, {1192, 25}, {1284, 20}, {1370, 15}, {1447, 10}, {1516, 5},
	{1574, 0}, {1624, -5}, {1664, -10}, {1697, -15}, {1723, -20}, {1743, -25},
	{1758, -30}, {1770, -35}, {1779, -40}
};
			       ...
			       int table_list)
{
	...
	case THERMAL_SOC_VOLT2TEMP_FLAG:
		...
		break;
	case THERMAL_ADC2TEMP_51K_FLAG:
		resistance_min = g_thermal_hkadc_volt_temp_51k_table[index_min][0];
		resistance_max = g_thermal_hkadc_volt_temp_51k_table[index_max][0];
		temper_bottom = g_thermal_hkadc_volt_temp_51k_table[index_min][1];
		temper_top = g_thermal_hkadc_volt_temp_51k_table[index_max][1];
		break;
	default:
			       ...
			       int **volt_to_temperature, int table_list)
{
	...
	else if (THERMAL_ADC2TEMP_51K_FLAG == table_list)
		upper = sizeof(g_thermal_hkadc_volt_temp_51k_table) /
			sizeof(g_thermal_hkadc_volt_temp_51k_table[0][0]) / 2;
...
int peripheral_ntc_2_temp(struct periph_tsens_tm_device_sensor *chip,
			       int *temp, int ntc)
{
	int tempdata = 0;
	...
	*temp = tempdata;
	...
	if (strncmp(chip->ntc_name, "ithermal_hkadc_adc_temp_table",
	...
	else if (strncmp(chip->ntc_name, "ithermal_hkadc_pa_temp_table",
			 strlen(chip->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, chip->chanel,
					       (int **)g_thermal_pa_volt_temp_table,
					       THERMAL_PA_VOLT2TEMP_FLAG);
	else if (strncmp(chip->ntc_name, "ithermal_hkadc_soc_temp_table",
			 strlen(chip->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, chip->chanel,
					       (int **)g_thermal_soc_volt_temp_table,
					       THERMAL_SOC_VOLT2TEMP_FLAG);
	else if (strncmp(chip->ntc_name, "ithermal_hkadc_adc_temp_51k_table",
		    strlen(chip->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, chip->chanel,
					       (int **)g_thermal_hkadc_volt_temp_51k_table,
					       THERMAL_ADC2TEMP_51K_FLAG);
	else
...
#ifdef CONFIG_HKADC_MODEM
int mdm_ntc_2_temp(struct mdm_adc_t *mdm_sensor, int *temp, int ntc)
{
	int tempdata = 0;
	...
	*temp = tempdata;
	...
	if (strncmp(mdm_sensor->ntc_name, "ithermal_hkadc_adc_temp_table",
		    strlen(mdm_sensor->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, mdm_sensor->channel,
					       (int **)g_thermal_hkadc_volt_temp_table,
					       THERMAL_ADC2TEMP_FLAG);
	else if (strncmp(mdm_sensor->ntc_name, "ithermal_hkadc_pa_temp_table",
			 strlen(mdm_sensor->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, mdm_sensor->channel,
					       (int **)g_thermal_pa_volt_temp_table,
					       THERMAL_PA_VOLT2TEMP_FLAG);
	else if (strncmp(mdm_sensor->ntc_name, "ithermal_hkadc_soc_temp_table",
			 strlen(mdm_sensor->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, mdm_sensor->channel,
					       (int **)g_thermal_soc_volt_temp_table,
					       THERMAL_SOC_VOLT2TEMP_FLAG);
	else if (strncmp(mdm_sensor->ntc_name, "ithermal_hkadc_adc_temp_51k_table",
		    strlen(mdm_sensor->ntc_name)) == 0)
		tempdata = get_ntc_temperature(ntc, mdm_sensor->channel,
					       (int **)g_thermal_hkadc_volt_temp_51k_table,
					       THERMAL_ADC2TEMP_51K_FLAG);
				   ...
				   enum hkadc_table_id *table_id)
{
	if (ntc_name != NULL && table_id != NULL) {
		if (strncmp(ntc_name, "ithermal_hkadc_adc_temp_table",
			    strlen(ntc_name)) == 0) {
			*table_id = HKADC_ADC_TABLEID;
		} else if (strncmp(ntc_name, "ithermal_hkadc_pa_temp_table",
				   strlen(ntc_name)) == 0) {
			*table_id = HKADC_PA_TABLEID;
		} else if (strncmp(ntc_name, "ithermal_hkadc_soc_temp_table",
				   strlen(ntc_name)) == 0) {
	...
	if (dest != NULL && table_id != NULL) {
		...
		case HKADC_ADC_51K_TABLEID:
			*dest = (unsigned long)(uintptr_t)g_thermal_hkadc_volt_temp_51k_table;
			return sizeof(g_thermal_hkadc_volt_temp_51k_table);
...
int peripheral_temp_2_ntc(struct periph_tsens_tm_device_sensor *chip,
			       int temp, u16 *ntc)
{
	if (chip->ntc_name == NULL) {
	...
	if (strncmp(chip->ntc_name, "ithermal_hkadc_adc_temp_table",
		...
		*ntc = (u16)temp_2_ntc_bisearch_table(temp,
		    ...
		    strlen(chip->ntc_name)) == 0) {
		*ntc = (u16)temp_2_ntc_bisearch_table(temp,
						      g_thermal_hkadc_volt_temp_51k_table,
						      sizeof(g_thermal_hkadc_volt_temp_51k_table));
	} else {
		pr_err("input ntc name was not found!\n\r");
		return -EINVAL;
	}
