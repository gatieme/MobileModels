...
int tsens_get_temp(u32 sensor, int *temp)
{
	...
	for (i = 0; i < g_tmdev->tsens_num_sensor; i++) {
		if (sensor == g_tmdev->sensor[i].sensor_num) {
			...
			*temp = tmp * 1000;
#ifndef CONFIG_LIBLINUX
			if (strcmp(thermal->type, "cluster0") == 0)
				trace_perf(clock_set_rate, "cluster0_temp", *temp, raw_smp_processor_id());
			else if (strcmp(thermal->type, "cluster1") == 0)
				trace_perf(clock_set_rate, "cluster1_temp", *temp, raw_smp_processor_id());
			else if (strcmp(thermal->type, "cluster2") == 0)
				trace_perf(clock_set_rate, "cluster2_temp", *temp, raw_smp_processor_id());
			else if (strcmp(thermal->type, "gpu") == 0)
				trace_perf(clock_set_rate, "gpu_temp", *temp, raw_smp_processor_id());
#endif
		}
	}
