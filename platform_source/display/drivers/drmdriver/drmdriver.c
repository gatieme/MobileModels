...
s32 configure_dss_service_security(u32 func_id, u32 channel, u32 mode, u32 dpu_index)
{
	struct arm_smccc_res res = {0};
#ifdef CONFIG_DPU_FFA_SUPPORT
	...
	if (get_group_id(func_id) != FID_DISPLAY_GROUP) {
		pr_err("%s:invalid func_id=0%x",
				__func__, func_id);
		return -1;
	}
#ifdef CONFIG_DPU_FFA_SUPPORT
