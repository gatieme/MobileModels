...
static int jpu_check_full_decode_info(const struct jpu_data_t *jpu_req)
{
	...
	if (jpu_req->decode_mode >= JPEG_DECODE_MODE_MAX ||
		jpu_req->decode_mode < JPEG_DECODE_MODE_FULL) {
...
int jpu_job_exec(struct jpu_data_type *jpu_device, const void __user *argp)
{
	...
	if (copy_from_user(jpu_req, argp, sizeof(struct jpu_data_t)) != 0) {
		jpu_err("copy_from_user failed\n");
		goto err_out;
	}

	if (jpu_get_dmabuf(&buf, jpu_req) != 0) {
		jpu_err("jpu_get_dmabuf failed\n");
		goto err_out;
	}

	/* input userdata and init reg */
	ret = jpu_userdata_and_reg_init(jpu_device, pjpu_dec_reg, jpu_req);
	if (ret != 0) {
