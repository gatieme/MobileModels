...
void cmdlst_set_reg_incr(cmd_buf_t *cmd_buf, unsigned int reg, unsigned int size, unsigned int incr)
{
	...
	*(unsigned int *)(uintptr_t)(cmd_buf->data_addr) = (reg & 0x000ffffc) | ((size - 1) << 24) | (incr << 1);
	cmd_buf->data_addr += 4;
	cmd_buf->data_size += 4;
	d("[%s] -\n", __func__);
}
...
static int cmdlst_set_irq_mode(cpe_cmdlst_irq_mode irq_mode, cmd_buf_t *cmd_buf, unsigned int cpu_enable)
{
	...
	case CMD_IRQ_SLAM_NORMAL_MODE: {
		// each layer will configure this.
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_SLAM_IRQ_REG0_REG, 0x3FFF); // clean slam eof
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_SLAM_IRQ_REG1_REG, irq_mode); // select need irq mask
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_ACPU_IRQ_REG1_REG, 0x1); // mask
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_CHN1_REG, cpu_enable); // 0, last is 3.
		break;
	}

	case CMD_IRQ_SLAM_READ_STAT_MODE: {
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_SLAM_IRQ_REG0_REG, 0x3FFF); // clear  cmdlst eof
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_SLAM_IRQ_REG1_REG, irq_mode); // select need irq mask
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_ACPU_IRQ_REG1_REG, 0x1); // mask
		cmdlst_set_reg(cmd_buf, JPG_TOP_OFFSET + SUB_CTRL_CMDLST_CHN1_REG, cpu_enable);
		break;
	}
...
static int cmdlst_set_vpwr(cmd_buf_t *cmd_buf, cmdlst_rd_cfg_info_t *rd_cfg_info)
{
	...
	cmdlst_w3_1_table.vp_wr_cfg[WR_CMDLST].fmt.pix_fmt = DF_D32; // make sure with zhangheng,should D32. original:DF_D64;
...
static int cmdlst_enqueue(unsigned int channel_id, cmdlst_para_t *cmdlst_para)
{
	...
	for (dump_list = ready_list->next; dump_list != ready_list; dump_list = dump_list->next)
		pr_info("list[0x%llx]", (unsigned long long)(uintptr_t)dump_list);    //lint !e559
...
int df_sched_prepare(cmdlst_para_t *cmdlst_para)
{
	...
	memset_s(new_entry, cmdlst_para->stripe_cnt * sizeof(schedule_cmdlst_link_t), 0,
	...
	for (i = 0; i < cmdlst_para->stripe_cnt; i++) {
		...
		new_entry[i].cmd_buf.start_addr  = new_entry[0].cmd_buf.start_addr + (unsigned int)(
											   CMDLST_BUFFER_SIZE * i);  //lint !e647
