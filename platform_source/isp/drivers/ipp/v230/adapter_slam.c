...
static int slam_set_irq_mode(msg_req_slam_request_t *req, schedule_cmdlst_link_t *cmd_link_entry,
							 unsigned int stripe_cnt)
{
	...
	for (i = 0; i < stripe_cnt; i++) {
		if (g_slam_read_st) {
			...
			if (req->basic_cfg.scoremap_en == 1) {
				cmd_link_entry[i].irq_mode = CMD_IRQ_SLAM_SCOREMAP_MODE;
			} else if (req->pyramid_cfg[i].pyramid_en == 1) {
				cmd_link_entry[i].irq_mode = CMD_IRQ_SLAM_PYRAMID_MODE;
			} else {
				cmd_link_entry[i].irq_mode = CMD_IRQ_SLAM_NORMAL_MODE;
			}
...
static int slam_update_cvdr_cfg_tab(msg_req_slam_request_t *slam_request, cfg_tab_cvdr_t *slam_cfg_tab,
									unsigned int pyramid_index)
{
	...
	if (slam_request->streams[pyramid_index][BI_SLAM_Y].buf) {
		...
		if (0 == pyramid_index) {
			slam_stride = slam_request->streams[pyramid_index][BI_SLAM_Y].stride;
	...
	if (slam_request->streams[pyramid_index][BO_SLAM_STAT].buf) {
		...
		if (slam_request->basic_cfg.brief_en == 1) {
			dataflow_cvdr_cfg_d32(slam_cfg_tab, &cfmt,
								  slam_request->streams[pyramid_index][BO_SLAM_STAT].width * 40 + 4 + CVDR_KEY_POINT_OVERFLOW);
		} else {
			dataflow_cvdr_cfg_d32(slam_cfg_tab, &cfmt,
								  slam_request->streams[pyramid_index][BO_SLAM_STAT].width * 72 + 4 + CVDR_KEY_POINT_OVERFLOW);
		}
	}
...
static void slam_basic_config(msg_req_slam_request_t *slam_request, slam_config_table_t *slam_cfg_tab,
							  unsigned int pyramid_index)
{
...
static void slam_pyramid_param_config(msg_req_slam_request_t *slam_request, slam_config_table_t *slam_cfg_tab,
									  unsigned int pyramid_index)
{
...
static void slam_nms_win_param_config(msg_req_slam_request_t *slam_request, slam_config_table_t *slam_cfg_tab,
									  unsigned int pyramid_index)
{
...
static void slam_pattern_param_config(msg_req_slam_request_t *slam_request, slam_config_table_t *slam_cfg_tab)
{
	...
	if (slam_request->basic_cfg.brief_en == 1 || slam_request->basic_cfg.freak_en == 1) {
		for (i = 0; i < SLAM_PATTERN_NUM; i++) {
			...
			temp_pattern_param.bits.pattern_y1 = (unsigned int)slam_request->pattern_cfg[i].pattern_y1;
			temp_pattern_param.bits.pattern_x1 = (unsigned int)slam_request->pattern_cfg[i].pattern_x1;
			temp_pattern_param.bits.pattern_y0 = (unsigned int)slam_request->pattern_cfg[i].pattern_y0;
			temp_pattern_param.bits.pattern_x0 = (unsigned int)slam_request->pattern_cfg[i].pattern_x0;
...
static void slam_update_request_cfg_tab(msg_req_slam_request_t *slam_request, slam_config_table_t *slam_cfg_tab,
										unsigned int pyramid_index)
{
	d("[%s] +\n", __func__);
...
static int slam_set_cmdlst_stripe(cmdlst_stripe_info_t *stripe, unsigned int stripe_cnt)
{
	...
	for (i = 0; i < stripe_cnt; i++) {
		if (g_slam_read_st && i % 2 != 0) {
			stripe[i].rd_cfg_info.fs           = slam_stat_addr + 4 * SLAM_SCOREHIST_NUM * (i - 1) / 2;
...
static void slam_update_cmdlst_cfg_tab(cfg_tab_cmdlst_t *slam_cmdlst_cfg_tab, cmdlst_para_t *slam_cmdlst_para,
									   cmd_buf_t *slam_cmd_buf)
{
	d("[%s] +\n", __func__);
...
int slam_request_handler(msg_req_slam_request_t *slam_request)
{
	...
	if (cpe_mem_get(MEM_ID_SLAM_CFG_TAB, &va, &da)) {
		pr_err("[%s] Failed : cpe_mem_get. %d, 0x%llx, 0x%x.\n", __func__, MEM_ID_SLAM_CFG_TAB, va, da);
		return CPE_FW_ERR;
	}

	slam_config_table_t *slam_cfg_tab = (slam_config_table_t *)(uintptr_t)va;

	if (cpe_mem_get(MEM_ID_CVDR_CFG_TAB_SLAM, &va, &da)) {
		pr_err("[%s] Failed : cpe_mem_get. %d, 0x%llx, 0x%x.\n", __func__, MEM_ID_CVDR_CFG_TAB_SLAM, va, da);
		return CPE_FW_ERR;
	}

	cfg_tab_cvdr_t *slam_cvdr_cfg_tab = (cfg_tab_cvdr_t *)(uintptr_t)va;

	if (cpe_mem_get(MEM_ID_CMDLST_PARA_SLAM, &va, &da)) {
		pr_err("[%s] Failed : cpe_mem_get. %d, 0x%llx, 0x%x. \n", __func__, MEM_ID_CMDLST_PARA_SLAM, va, da);
		return CPE_FW_ERR;
	}
...
int slam_eof_handler(msg_req_slam_request_t *slam_request)
{
	...
	score_hist_local = (unsigned short *)kmalloc(sizeof(unsigned short) * MAX_PYRAMID_LAYER * SLAM_SCOREHIST_NUM,
					   GFP_KERNEL);
	...
	for (i = 0; i < MAX_PYRAMID_LAYER; i++) {
		...
		for (j = 0; j < SLAM_SCOREHIST_NUM; j++) {
			*score_hist_local_tmp = (unsigned short)readl((volatile void __iomem *)(g_slam_stat_addr
									+ 4 * j + i * 4 * SLAM_SCOREHIST_NUM));
			score_hist_local_tmp++;
		}
	}
	ret = copy_to_user((void __user *)user_score_hist, score_hist_local,
					   sizeof(unsigned short) * MAX_PYRAMID_LAYER * SLAM_SCOREHIST_NUM);
...
static int slam_request_dump(msg_req_slam_request_t *req)
{
	...
	pr_info("scoremap_en=%d,freak_en=%d,brief_en = %d,orient_en = %d,nms_en = %d,fast_en = %d,gsblur_en = %d",
	...
	pr_info("coeff_gauss = 0x%x,0x%x,0x%x,0x%x\n", req->gsblur_coef_cfg.coeff_gauss_0,
			req->gsblur_coef_cfg.coeff_gauss_1,req->gsblur_coef_cfg.coeff_gauss_2, req->gsblur_coef_cfg.coeff_gauss_3);
	...
	for (j = 0; j < req->pyramid_layer_num; j++) {
		...
		for (i = 0; i < SLAM_STREAM_MAX; i++)
			pr_info("streams[%d] width  = %d,height = %d,buf = 0x%x,format = %d\n",
					i, req->streams[j][i].width, req->streams[j][i].height, req->streams[j][i].buf,
					req->streams[j][i].format);

		pr_info("pyramid_en = %d", req->pyramid_cfg[j].pyramid_en);

		if (req->pyramid_cfg[j].pyramid_en)
			pr_info("scl_inc = 0x%x,scl_vbottom = 0x%x,scl_vtop = 0x%x,scl_hright = 0x%x,scl_hleft = 0x%x\n",
			req->pyramid_cfg[j].scl_inc, req->pyramid_cfg[j].scl_vbottom, req->pyramid_cfg[j].scl_vtop,
			req->pyramid_cfg[j].scl_hright, req->pyramid_cfg[j].scl_hleft);
