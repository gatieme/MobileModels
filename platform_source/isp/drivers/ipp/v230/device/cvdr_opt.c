							...
							unsigned int isp_clk, pix_format_e format)
{
	...
	pr_info("width = %d, height = %d, line_stride = %d, format = %d, pix_fmt = %d,addr = 0x%x \n",
			cfmt->width, cfmt->height, line_stride, format, cfmt->pix_fmt, cfmt->addr);
	unsigned int i              = 0;
...
//    unsigned int line_bytes     = 0;
	...
	for (i = id; i < (id + config_num); i++) {
		...
		p_cvdr_cfg->vp_wr_cfg[i].fmt.last_page      = (p_cvdr_cfg->vp_wr_cfg[i].fmt.fs_addr +
				align_up(align_up(cfmt->full_width * pix_size / 8, CVDR_ALIGN_BYTES) *
		...
		p_cvdr_cfg->vp_wr_cfg[i].fmt.line_stride    = align_up(cfmt->full_width * pix_size / 8,
		...
		if (line_stride != 0) {
			p_cvdr_cfg->vp_wr_cfg[i].fmt.line_stride  = align_up(line_stride, CVDR_ALIGN_BYTES) / CVDR_ALIGN_BYTES - 1;
			p_cvdr_cfg->vp_wr_cfg[i].fmt.last_page      = (p_cvdr_cfg->vp_wr_cfg[i].fmt.fs_addr +
					align_up(align_up(line_stride, CVDR_ALIGN_BYTES) * cfmt->height, CVDR_TOTAL_BYTES_ALIGN)) >> 15;
		}

		i("fs_addr = 0x%x, id = %d, line_stride = %d, line_wrap = %d, config_width  = %d,"
							...
							unsigned int isp_clk, pix_format_e format)
{
	...
	p_cvdr_cfg->vp_rd_cfg[id].fmt.allocated_du       = (unsigned short)allocated_du;
	...
	p_cvdr_cfg->vp_rd_cfg[id].fmt.line_stride        = align_up(cfmt->full_width * pix_size / 8,
			CVDR_ALIGN_BYTES) / CVDR_ALIGN_BYTES - 1;
	...
	i("id = %d, addr = 0x%x, width = %d, height = %d, config_width = %d,\n"
