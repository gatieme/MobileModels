...
#define cmdlst_set_reg_m(reg, val) cmdlst_set_reg(dev->cmd_buf, reg, val)
#define cmdlst_set_reg_incr_m(reg, size, incr) cmdlst_set_reg_incr(dev->cmd_buf, reg, size, incr)
#define cmdlst_set_reg_data_m(data) cmdlst_set_reg_data(dev->cmd_buf, data)
...
typedef enum _cpe_cmdlst_irq_mode {
	CMD_IRQ_SLAM_SCOREMAP_MODE              = 0x3FE9,  // 0x1C,  only state, score, slam irq
	CMD_IRQ_SLAM_PYRAMID_MODE               = 0x3FEC,  // 0x3FE5, //0x0E, only state , Y, slam irq
	CMD_IRQ_SLAM_NORMAL_MODE                = 0x3FED,  // 0x0C,  only State, slam_irq
	CMD_IRQ_SLAM_READ_STAT_MODE             =  0x3FDF, // 0x1, only cmdlst
	CMD_IRQ_MAX = 0x0,
} cpe_cmdlst_irq_mode;
