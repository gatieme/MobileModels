...
#define d(fmt, args...) do { \
	if (kmsgcat_mask & DEBUG_BIT) \
		printk("[ispcpe][%s] " fmt, __func__, ##args); } while (0)
...
#define IPP_PCTRL_PERI_STAT_ADDR    (0x000000C4) /* offset */
#define ISP_PCTRL_PERI_FLAG         (1 << 3)        /* BIT 3 : ISP */
