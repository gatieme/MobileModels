...
#define MEM_IPP_ERR_ADDR_OFFSET (0x00001000)
...
static int do_ippdump(struct hipp_comdevice_s *dev)
{
	...
	for (i = 0; i < MAX_IPP_COM; i ++) {
		...
		i("Driver.%d: %s.type.0x%x, initialized.0x%x, mode.0x%x, count.e.0x%x, d.0x%x\n", i,
			drv->name, drv->type, drv->initialized, drv->mode, drv->mntn_smmu.icount, drv->mntn_smmu.xcount);
		i("timestamp f.%5u.%06u, l.%5u.%06u, fault.%5u.%06u\n",
...
static unsigned int ipp_mmu_map(struct scatterlist *sg, unsigned int *size)
{
	...
	struct hipp_mapping_s *map_info = NULL;
	unsigned long iova;
	struct scatterlist *sgbuf = NULL;
	...
	map_info->prot = MEM_ATTR_READ | MEM_ATTR_WRITE | MEM_ATTR_SEC;
	...
	pr_info("[%s] a7va.0x%lx, size.0x%x, map_size.0x%x, map_prot.0x%x\n", __func__,
			map_info->daddr, set_size, map_size, MEM_ATTR_READ | MEM_ATTR_WRITE | MEM_ATTR_SEC);
	map_info->sglnum = ipp_map_set_pa_list(dev->sgpool_vaddr, sg, map_size);

	if (atfhipp_trusted_map()) {
...
static unsigned int hipp_trusted_map(struct hipp_common_s *drv, unsigned int sharefd, unsigned int *size)
{
	...
	device = &dev->pdev->dev;
	dmabuf = dma_buf_get(sharefd);
	if (IS_ERR_OR_NULL(dmabuf)) {
		pr_err("[%s] Failed : dma_buf_get, dmabuf.%pK\n", __func__, dmabuf);
		return 0;
	}
...
static int hippsmmu_get_pteaddr(struct hipp_comdevice_s *dev)
{
	...
	dev->domain = iommu_get_domain_for_dev(device);
	...
	pgd_base = kernel_domain_get_ttbr(device);
	dev->bl31_vaddr->pte_nontrus = pgd_base;
	i("- PTE.0x%lx\n", dev->bl31_vaddr->pte_nontrus);
	return 0;
}

static void hippsmmu_put_pteaddr(struct hipp_comdevice_s *dev)
{
	if (!dev) {
		e("Failed : hipp smmu dev.%pK\n", dev);
		return;
	...
	if (dev->bl31_vaddr->pte_nontrus != 0)
		dev->bl31_vaddr->pte_nontrus = 0;

	if (dev->domain != NULL)
		dev->domain = NULL;
}

static int hipp_ion_phys(int share_fd, dma_addr_t *addr)
{
	int ret = -ENODEV;
...
int hipp_ptable_secmem_init(int share_fd)
{
	...
	ret = hipp_ion_phys(share_fd, &secmem_ion_phys);/*lint !e838 */
	if (ret < 0) {
		pr_err("%s, failed to get phy addr,ret:%d!\n", __func__, ret);
		return -ENODEV;
	}

	dev->bl31_vaddr->pte_trusted = (u64)secmem_ion_phys;
	if ((ret = atfhipp_ptable_init()) != 0) {
		e("Failed : atfhipp_ptable_init.(0x%lx).%d\n", (unsigned long)secmem_ion_phys, ret);
		return -ENODEV;
	}
...
static int hippcom_probe(struct platform_device *pdev)
{
	...
	if ((dev->smem_vaddr = hisp_fstcma_alloc(&dev->smem_dma, dev->smem_size, GFP_KERNEL)) == NULL) {
	...
	if ((ret = atfhipp_trusted_smem((uint64_t)dev->smem_dma,
		(uint64_t)(dev->smem_dma + MEM_IPP_ERR_ADDR_OFFSET))) != 0) {
	...
	if (!(dev->genpool = hipp_genpool_setup(dev->genpool_start, dev->genpool_size, 0x8000))) {
		e("Failed : hipp_genpool_setup.(0x%lx, 0x%x).%pK\n", dev->genpool_start, (unsigned int)dev->genpool_size,
			dev->genpool);
		goto free_fstcma;
	}
...
free_fstcma:
	hisp_fstcma_free((void *)dev->smem_vaddr, dev->smem_dma, dev->smem_size);
free_dev:
	kfree(dev);
	if (dev)
		dev = NULL;
