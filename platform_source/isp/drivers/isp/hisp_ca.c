...
struct hisp_ca_rsv_cfginfo {
	struct hisp_ca_meminfo rsv_info[HISP_SEC_RSV_MAX_TYPE];
};

struct hisp_ca_boot_cfginfo {
	struct hisp_ca_rsv_cfginfo rsv;
	struct hisp_ca_img_cfginfo img;
};
...
static struct hisp_ca_boot_cfginfo *g_boot_buf;
...
static int hisp_ca_rsvmem_cfg(
	struct hisp_ca_rsv_cfginfo *buffer,
	...
	struct hisp_ca_meminfo *info, int info_size)
{
	...
	for (i = 0; i < info_size; i++) {
		is_valid = hisp_sectype_is_valid(i);
		if (is_valid == SEC_TYPE_INVALID) {
			hisp_ca_err("ignore invalid buffer[%d]\n", i);
			continue;
		}
		...
		ret = secmem_get_buffer(buffer->img_info[i].info.sharefd,
			&table, &sfd, &type);
#elif defined CONFIG_DMABUF_HEAPS
		ret = mm_dma_heap_secmem_get_buffer(buffer->img_info[i].info.sharefd,
			&table, &sfd, &type);
#endif
		if (ret != 0) {
...
static int hisp_ca_bootmem_cfg(
	struct hisp_ca_boot_cfginfo *buffer,
	struct hisp_ca_meminfo *img, int imginfo_size,
	struct hisp_ca_meminfo *rsv, int rsvinfo_size)
{
	int ret;

	ret = hisp_ca_imgmem_cfg(&buffer->img, img, imginfo_size);
	if (ret < 0) {
		hisp_ca_err("hisp_ca_imgmem_cfg fail. ret.%d!\n", ret);
		return ret;
	}

	ret = hisp_ca_rsvmem_cfg(&buffer->rsv, rsv, rsvinfo_size);
	if (ret < 0) {
...
int hisp_ca_sfdmem_map(struct hisp_ca_meminfo *buffer)
{
...
#elif defined CONFIG_DMABUF_HEAPS
	ret = mm_dma_heap_secmem_get_buffer(buffer->sharefd,
		&table, &sfd, &type);
#endif
	if (ret != 0) {
		hisp_ca_err("secmem_get_buffer fail. ret.%d!\n", ret);
	...
	operation.params[1].tmpref.size   = (uint32_t)sizeof(struct hisp_ca_meminfo);
...
/*
 * Function name:hisp_ca_bootmem_map
 ...
 *      @ 0-->success, others-->failed.
 */
static int hisp_ca_bootmem_map(void *buffer, size_t size, int type)
{
	TEEC_Result result;
	TEEC_Operation operation = {0};
	u32 origin = 0;
 ...
 *      @ 0-->success, others-->failed.
 */
static int hisp_ca_bootmem_unmap(void *buffer, size_t size, int type)
{
	TEEC_Result result;
	TEEC_Operation operation = {0};
	u32 origin = 0;
...
/*
 * Function name:hisp_ca_bootmem_config
 * Discription: boot mem cfg
 * Parameters:
 *      @ img: boot img mem info
 *      @ imginfo_size: img mem info array num
 *      @ rsv: boot rsv mem info
 *      @ rsvinfo_size: rsv mem info array num
 * return value:
 *      @ 0-->success, others-->failed.
 */
		...
		struct hisp_ca_meminfo *rsv, int rsvinfo_size)
{
	...
	if (img == NULL || rsv == NULL) {
		hisp_ca_err("hisp_ca_meminfo is NULL\n");
		return -EPERM;
	}

	g_boot_buf = kzalloc(sizeof(struct hisp_ca_boot_cfginfo), GFP_KERNEL);
	if (g_boot_buf == NULL)
		return -ENOMEM;

	ret = hisp_ca_bootmem_cfg(g_boot_buf, img, imginfo_size, rsv, rsvinfo_size);
	if (ret < 0) {
		hisp_ca_err("hisp_ca_bootmem_cfg fail, ret.%d\n", ret);
		kfree(g_boot_buf);
		g_boot_buf = NULL;
		return ret;
	}

	ret = hisp_ca_bootmem_map((void *)g_boot_buf,
			sizeof(struct hisp_ca_boot_cfginfo),
			TEE_SECISP_BOOT_MEM_CFG_AND_MAP);
	if (ret < 0) {
		hisp_ca_err("hisp_ca_bootmem_map fail, ret.%d\n", ret);
		kfree(g_boot_buf);
		g_boot_buf = NULL;
		return ret;
	}
...
/*
 * Function name:hisp_ca_bootmem_deconfig
...
int hisp_ca_bootmem_deconfig(void)
{
	...
	ret = hisp_ca_bootmem_unmap((void *)g_boot_buf,
			sizeof(struct hisp_ca_boot_cfginfo),
			TEE_SECISP_BOOT_MEM_CFG_AND_UNMAP);
	if (ret < 0) {
		hisp_ca_err("hisp_ca_bootmem_unmap fail, ret.%d\n", ret);
		kfree(g_boot_buf);
		g_boot_buf = NULL;
		return ret;
	}
