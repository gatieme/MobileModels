...
struct hisp_boot_device {
	...
	unsigned int dyn_bootflag;
	unsigned int dyn_bootaddr;
	int bypass_pwr_updn;
	int probe_finished;
	int last_boot_state;
	int ispcpu_status;
	...
	struct platform_device *ispfw_pdev;
#endif
#ifdef CONFIG_HUAWEI_DSM
...
static pid_t hisp_camera_daemon_pid = 0;

pid_t hisp_rpmsg_getpid(void)
{
	return hisp_camera_daemon_pid;
}
EXPORT_SYMBOL(hisp_rpmsg_getpid); /*lint !e508*/
				  ...
				  unsigned int csi_index)
{
	...
	u64 share_pa = 0;
	struct phy_csi_info_t *share_va     = NULL;
	struct phy_csi_info_t *phy_csi_info = NULL;
#endif
	...
	ret = hisp_ffa_connect_phy_csi(phy_info->phy_id, phy_info->phy_work_mode, csi_index);
#else
	share_va = hisp_get_param_info_va();
	if (share_va == NULL) {
		pr_err("[%s] hisp_phy_csi_info_vaddr is NULL!\n", __func__);
		return -EINVAL;
	}
	phy_csi_info = (struct phy_csi_info_t *)share_va;
	...
	share_pa = hisp_get_param_info_pa();
	if (share_pa == 0) {
		pr_err("[%s] hisp_phy_csi_info_paddr is NULL!\n", __func__);
		return -EINVAL;
	}

	ret = hisp_smc_phy_csi_connect(share_pa);
#ifdef DEBUG_HISP
	pr_info("[%s] : phy_csi_connect_type.%d, csia.0x%08x, b.0x%08x, c.0x%08x, e.0x%08x\n",
		__func__, share_va->phy_info.is_master_sensor,
		share_va->phy_info.phy_id, share_va->phy_info.phy_mode,
		share_va->phy_info.phy_freq_mode, share_va->phy_info.phy_freq);
#endif
#endif
	...
	if (ret != 0) {
		pr_err("[%s] Failed : hisp phy_csi_connect fail ret.%d!\n",
				__func__, ret);
		return ret;
	}
	return ret;
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	ret = hisp_ffa_disconnect_phy_csi();
#else
...
struct device *hisp_get_ispfw_device(void)
{
	...
	if (dev->ispfw_pdev == NULL) {
		pr_err("[%s] Failed: ispfw_pdev is NULL!\n", __func__);
		return NULL;
	}

	return &dev->ispfw_pdev->dev;
}
#endif
...
#define TIMEOUT_ISPLOG_START (10)
	...
	rproc = platform_get_drvdata(dev->isp_pdev);
	if (rproc == NULL) {
		hisp_info("platform_get_drvdata return NULL\n");
		return -EINVAL;
	}
...
int hisp_wait_rpmsg_completion(struct rproc *rproc)
{
	...
	if (ret == 0) {
		hisp_err("Failed : wait_for_completion_timeout\n");
		return -ETIME;
	}
...
static int hisp_nsec_bootimg_init(struct rproc *rproc)
{
	...
	void *imgva = NULL;
	void *isp_bw_va = hisp_nsec_get_remap_va();
	u64 isp_bw_pa = hisp_nsec_get_remap_pa();
	int ret;

	imgva = hisp_rproc_get_imgva(rproc);
	domain = hisp_rproc_get_domain(rproc);
	if (!imgva || !domain) {
		hisp_err("%s: image va or domain is NULL\n", __func__);
		return -ENOMEM;
	}

	ret = memcpy_s(isp_bw_va, ISP_MEM_SIZE, imgva, PAGE_SIZE);
	if (ret != 0) {
		hisp_err("%s: copy text to boot image fail\n", __func__);
		return ret;
	}

	if (hisp_smmuv3_mode()) {
		ret = iommu_map(domain, 0, isp_bw_pa,
			PAGE_SIZE, ISP_BOOT_MEM_ATTR);
		if (ret != 0) {
			hisp_err("%s iommu_map failed: ret %d len\n",
				__func__, ret);
			return ret;
		}
	}
...
static int hisp_nsec_bootimg_release(struct rproc *rproc)
{
	...
	if (phy_len != PAGE_SIZE) {
		hisp_err("%s: iommu_unmap failed: phy_len 0x%lx size 0x%lx\n",
...
int hisp_device_start(struct rproc *rproc)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	int ret;
	if (hisp_nsec_boot_mode()) {
		...
		if (ret != 0) {
			hisp_err("Failed : hisp_nsec_bootimg_init.%d\n", ret);
			return ret;
		}
	}
...
start_exit:
	if (hisp_nsec_boot_mode()) {
		if (hisp_nsec_bootimg_release(rproc))
			hisp_err("Failed : hisp_nsec_bootimg_release\n");
	}
	return ret;
}
EXPORT_SYMBOL(hisp_device_start);

int hisp_device_stop(struct rproc *rproc)
{
	int ret;
	if (hisp_nsec_boot_mode()) {
		ret = hisp_nsec_bootimg_release(rproc);
		if (ret != 0) {
			hisp_err("Failed : boot_image_release\n");
			return ret;
		}
	}
...
int hisp_device_attach(struct rproc *rproc)
{
	...
	return hisp_device_start(rproc);
}
...
int hisp_rproc_disable(void)
{
	...
	rproc = platform_get_drvdata(hisp_dev->isp_pdev);
	if (rproc == NULL) {
		hisp_info("platform_get_drvdata return NULL\n");
		return -EINVAL;
	}
...
#ifdef DEBUG_HISP
	if (hisp_get_perf_flag(rproc) == 1)
		hisp_perf_dump(rproc);
#endif

	mutex_lock(&hisp_dev->hisp_power_mutex);
	hisp_dev->hisp_power_state = HISP_ISP_POWER_OFF;
	hisp_camera_daemon_pid = 0;
	mutex_unlock(&hisp_dev->hisp_power_mutex);
...
unsigned int hisp_get_dyn_bootaddr(void)
{
	...
	return hisp_dev->dyn_bootaddr;
}

static void hisp_dyn_bootaddr_alloc(void)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	unsigned int num;

	if (hisp_dev->dyn_bootflag == ISP_DYNBOOT_ENABLE) {
		get_random_bytes(&num, sizeof(num));
		hisp_dev->dyn_bootaddr = (num & (unsigned int)(GENMASK(15, 6))) | BIT(16);
	} else {
		hisp_dev->dyn_bootaddr = 0;
	}

	hisp_debug("dyn_bootaddr is 0x%x.\n", hisp_dev->dyn_bootaddr);
}
...
int hisp_rproc_enable(void)
{
	...
	if (hisp_dev->isp_pdev == NULL) {
		hisp_err("isp_pdev is NULL\n");
		return -EINVAL;
	}

	rproc = platform_get_drvdata(hisp_dev->isp_pdev);
	if (rproc == NULL) {
		hisp_err("platform_get_drvdata return NULL\n");
		return -EINVAL;
	}
	...
	if (err == ISP_PCIE_NREADY)
		return err;
	hisp_perf_mem_state_init(rproc);
#endif
	...
	hisp_camera_daemon_pid = current->tgid;
	mutex_unlock(&hisp_dev->hisp_power_mutex);
...
static void hisp_get_board_type(struct hisp_boot_device *dev)
{
			...
			struct hisp_boot_device *dev)
{
	...
	ret = of_property_read_u32(np, "isp-dynboot-flag", &dev->dyn_bootflag);
	if (ret < 0) {
		dev->dyn_bootflag = 0;
		pr_info("[%s]: not supported isp dynboot flag\n", __func__);
	}
	pr_info("isp-dynboot-flag.0x%x of_property_read_u32.%d\n",
			dev->dyn_bootflag, ret);
...
int hisp_device_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		hisp_err("Failed : hisp_dev_probe_prev.%d\n", ret);
		return ret;
	}
...
#ifdef ISP_BUF_PRIVATIZE
int hisp_ispfw_device_attach(struct platform_device *pdev)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;

	hisp_dev->ispfw_pdev = pdev;

	return 0;
}

int hisp_ispfw_device_remove(struct platform_device *pdev)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;

	(void)pdev;

	hisp_dev->ispfw_pdev = NULL;
