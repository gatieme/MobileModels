...
#define ISPCPU_DEFAULT_LOG_LEVEL     (ISPCPU_LOG_LEVEL_ERR | ISPCPU_LOG_LEVEL_WARN | ISPCPU_LOG_LEVEL_INFO)
...
int hisplogcat_sync(void)
{
	...
	if (dev->share_mem == NULL) {
		pr_err("[%s] Failed: share_mem NULL\n", __func__);
		return -ENOMEM;
	}
...
static int hisplog_ioctl_check(unsigned int cmd, unsigned long args)
{
	...
	if (dev->share_para == NULL) {
		if (hisplogcat_sync() < 0)
			pr_err("[%s] Failed: hisplogcat_sync\n", __func__);
		pr_err("[%s] Failed : share_para NULL\n", __func__);
		return -EAGAIN;
	}
		...
		unsigned long args)
{
	...
	case LOG_WR_OFFSET:
		...
		ret = (int)copy_to_user((void __user *)(uintptr_t)args, &tmp, sizeof(tmp));
		if (ret != 0) {
	...
	case LOG_VERSION_TYPE:
		...
		ret = (int)copy_from_user(&dev->version_type,
			(void __user *)(uintptr_t)args, sizeof(int));
		if (ret != 0) {
		...
		size_t count)
{
	...
	if ((buf == NULL) || (count == 0)) {
		pr_err("[%s] Failed : buf NULL, or invalid count.0x%lx\n", __func__, count);
		return 0;
	}
...
static DEVICE_ATTR(isplogctrl, 0660, hisplogctrl_show,
		hisplogctrl_store);
#endif
