			...
			size_t addr, size_t size, unsigned int prot)
{
	...
	pr_err("%s : map addr.0x%lx, size.0x%lx, prot.0x%x\n",
...
static void hisp_record_memory_map(size_t addr, size_t size, unsigned int prot)
{
	...
	for (i = 0; i < ISP_MEM_POOL_NUM; i++) {
		pr_err("%s : hisp_mem: status.%d iova.0x%x, size.0x%x, prot.0x%x\n", __func__, 
			share_para->hisp_mem_record[i].state, share_para->hisp_mem_record[i].da,
			share_para->hisp_mem_record[i].size, share_para->hisp_mem_record[i].prot);
			...
			unsigned int align)
{
	...
	if (hisp_sec_boot_mode()) {
		...
		pr_info("%s : iova.0x%x, size.0x%x, prot.0x%x, pool_num.%u\n", __func__, iova, size, prot, pool_num);
					 ...
					 unsigned int type)
{
	...
	if (!hisp_smmuv3_mode()) {
		...
		pr_info("%s : iova.0x%x, size.0x%x, prot.0x%x, pool_num.%x\n", __func__, iova, map_size, prot, pool_num);
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	table = hisp_alloc_dynmem_sg_table((unsigned long)size);
	if (IS_ERR_OR_NULL(table)) {
		pr_err("%s: fail to map attachment", __func__);
		return -EINVAL;
	}
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	if (master == R8_MASTER && list_info->fw_table != NULL) {
		...
		pr_err("[%s]: invalid param master[%u]\n", __func__, master);
	}
}
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (ap_size != 0 && list_info->ap_table != NULL) {
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_memory_map ap failed", __func__, list_info->type);
			return -EINVAL;
		}
	}
	...
	if (r8_size != 0 && list_info->fw_table != NULL) {
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_memory_map r8 failed", __func__, list_info->type);
	...
	if (r8_core_size != 0 && list_info->ispfw_table != NULL) {
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_memory_map r8 core failed", __func__, list_info->type);
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (ret != 0) {
		pr_err("[%s] type[%u] hisp_alloc_from_mempool failed", __func__, type);
		return -ENOMEM;
	}

	iova = hisp_fw_phy_memory_map(map_info->paddr, map_info->fw_mem_prot, list_info);
	if (iova == 0) {
		pr_err("[%s] type[%u] hisp_fw_phy_memory_map failed", __func__, type);
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (ret != 0) {
		pr_err("[%s] type[%u] hisp_alloc_sgt_from_fd failed", __func__, type);
		return ret;
	}
	...
	if (ret != 0) {
		pr_err("[%s] type[%u] hisp_fw_memory_map failed", __func__, type);
		goto err_map_r8_iova;
	}
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (ret != 0) {
		pr_err("[%s] type[%u] hisp_alloc_sgt_from_size failed", __func__, type);
	...
	if (ret != 0) {
		pr_err("[%s] type[%u] hisp_ispfw_memory_map failed", __func__, type);
	...
	struct hisp_mem_pool_info_list_s *list_node)
{
	...
	list_for_each_entry_safe(node, node_temp, &list_node->list, list) {
		if (node->type == type) {
			pr_info("[%s] [ISP Clean Mem] type[%u]\n", __func__, type);
			hisp_fw_memory_unmap(node);
			hisp_free_to_mempool(node);
	...
	struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (list_info->addr == 0x0) {
		pr_err("[%s] alloc addr failed with type[%u]\n", __func__, type);
...
void hisp_kcov_shadow_mem_deinit(void)
{
	...
	list_for_each_entry_safe(node, node_temp, &list_node->list, list) {
		if (node->type == NOSEC_ISP_KCOV_MEM || node->type == NOSEC_ISP_SHADOW_MEM) {
			pr_info("[%s] [Fuzz Mem] type[%u]\n", __func__, node->type);

			hisp_unrecord_memory_map(node->type, node);
			hisp_memory_unmap(node->addr, node->size);
			hisp_free_to_mempool(node);
...
void hisp_mem_list_clean(void)
{
	...
	list_for_each_entry_safe(node, node_temp, &list_node->list, list) {
		if (node->type == NOSEC_ISP_MEM ||
			...
			pr_debug("[ISP Clean Mem] type[%u] just unrecord map\n", node->type);
#ifdef HISP_FUZZ
	...
	struct hisp_map_info_s *info, struct hisp_mem_info_s *mem)
{
	...
	if (type >= ISP_MEM_MAX) {
		pr_err("[%s] invalid type %u\n", __func__, type);
		return -EINVAL;
	}

	state = hisp_get_memory_state(type);
	if (state == HISP_MEM_MAP) {
		pr_err("[%s] type[%u] memory mapped already\n", __func__, type);
		return -EINVAL;
	}
	...
	if (info->size >= ISP_MAP_MAX_SIZE ||
		...
		pr_err("[%s] type[%u] size[0x%x-0x%x-0x%x], map size over 0x%x\n",
...
int hisp_mem_map_addr(unsigned int type, struct hisp_map_info_s *info, struct hisp_mem_info_s *mem)
{
	...
	pr_debug("[%s] type[%u] map addr start\n", __func__, type);
	...
	case NOSEC_OFFLINE_FBD_MEM:
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_mem_map_hybrid failed", __func__, type);
			goto MAP_ADDR_ERR;
		}
	...
	case NOSEC_IMU_MEM:
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_mem_map_r8_paddr failed", __func__, type);
			goto MAP_ADDR_ERR;
		}
	...
	case NOSEC_ISPNN_WORKSPACE_MEM:
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_mem_map_r8_fd failed", __func__, type);
			goto MAP_ADDR_ERR;
		}
	...
	case NOSEC_DYNAMIC_MEM:
		...
		if (ret != 0) {
			pr_err("[%s] type[%u] hisp_mem_map_sec failed", __func__, type);
			goto MAP_ADDR_ERR;
		}

		break;
	default:
		pr_err("[%s] type[%u] unsupported", __func__, type);
		goto MAP_ADDR_ERR;
	}
	mutex_unlock(&mem_pool->hisp_mem_mutex);
	...
	pr_debug("[%s] type[%u] map addr end\n", __func__, type);
...
int hisp_mem_unmap_addr(unsigned int type, struct hisp_mem_info_s mem_info)
{
	...
	pr_info("[%s] type[%u] unmap addr start\n", __func__, type);
	pool_info = &mem_pool->iova_pool_info;
	if (pool_info == NULL) {
		pr_err("isp mem pool didn't creat\n");
		return -EINVAL;
	...
	pr_info("[%s] type[%u] unmap addr end\n", __func__, type);
	return 0;
}
...
int hisp_mempool_init(unsigned int addr, unsigned int size)
{
	...
	if (use_mempool == 0) {
		pr_err("[%s] Invalid args use_mempool.%u\n", __func__, use_mempool);
		return 0;
	}
