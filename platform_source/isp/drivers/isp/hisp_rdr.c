...
void hisp_send_fiq2ispcpu(void)
{
	...
	if (hisp_wait_excflag_timeout(ISP_CPU_POWER_DOWN, 10) == 0x0) {
		...
		if (wdt_flag) {
			...
			ret = hisp_ffa_send_fiq();
#else
			ret = hisp_smc_send_fiq2ispcpu();
#endif
			if (ret < 0) {
				pr_err("[%s] Failed : hisp fiq2ispcpu.%d\n", __func__, ret);
				return;
			}
		}
		mdelay(50);
		...
		int len, struct hisp_shared_para *param)
{
	...
	int ret = 0;
	int flag = 0;
	int size = ISP_RDR_MAX_SIZE;
	struct hisp_rdr_device *dev = &hisp_rdr_dev;

	if (len < ISP_RDR_MAX_SIZE)
		size = len + 1;

	if (!strncmp(buf, "sirq", (int)strlen("sirq"))) {
		p += 1;
		q = kzalloc(size, GFP_KERNEL);
		if (q == NULL)
			return flag;

		ret = memcpy_s(q, size, buf, len);
		if (ret != 0) {
