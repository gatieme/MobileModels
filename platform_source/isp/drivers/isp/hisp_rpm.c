...
typedef struct {
	unsigned int type;
	unsigned int rate;
} intf_test_set_clk_rate_t;

typedef struct {
	struct hisp_phy_info_t *phy_info;
	unsigned int csi_index;
} intf_test_phy_csi_connect_t;

typedef struct {
	unsigned int type;
	unsigned int *size;
} intf_test_secboot_memsize_t;

typedef struct {
	unsigned int pool_num;
	int sharefd;
	unsigned int size;
} intf_test_secmem_ca_map_t;

typedef struct {
	unsigned int pool_num;
} intf_test_secmem_ca_unmap_t;

typedef struct {
	unsigned int message_id;
} intf_test_dump_rpmsg_t;

typedef struct {
	unsigned int type;
	struct hisp_map_info_s *info;
	struct hisp_mem_info_s *mem;
} intf_test_mem_map_addr_t;

typedef struct {
	unsigned int type;
	struct hisp_mem_info_s mem_info;
} intf_test_mem_unmap_addr_t;

typedef struct {
	int type;
	union {
		intf_test_set_clk_rate_t param_clk_rate;
		intf_test_phy_csi_connect_t param_phy_csi;
		intf_test_secboot_memsize_t param_secboot_memsize;
		intf_test_secmem_ca_map_t param_secmem_ca_map;
		intf_test_secmem_ca_unmap_t param_secmem_ca_unmap;
		intf_test_dump_rpmsg_t param_dump_rpmsg;
	} u;
} interface_test_t;

enum intf_test_e {
	INTF_TEST_SET_CLK_RATE = 0,
	INTF_TEST_PHY_CSI_CONNECT,
	INTF_TEST_PHY_CSI_DISCONNECT,
	INTF_TEST_RPROC_ENABLE,
	INTF_TEST_RPROC_DISABLE,
	INTF_TEST_SECBOOT_MEMSIZE,
	INTF_TEST_SECBOOT_PREPARE,
	INTF_TEST_SECMEM_CA_MAP,
	INTF_TEST_SECMEM_CA_UNMAP,
	INTF_TEST_DUMP_RPMSG,
	INTF_TEST_RPMSGREFS_RESET,
	INTF_TEST_RPMSGREFS_DUMP,
	INTF_TEST_SENDIN,
	INTF_TEST_RECVDONE,
	INTF_TEST_RECVIN,
	INTF_TEST_RECVX,
	INTF_TEST_BOOT_STAT_DUMP,
};

#define HISP_INTERFACE_TEST_HAL        _IOWR('i', 0x23, interface_test_t)
#endif
...
#define ISP_DEBUG_IOCTL_MAGIC   0x69
#define ISP_DEBUG_IOCTL_MAX_NR  0x23
...
static int hispdbg_intf_test_device(int type, interface_test_t *info)
{
	int ret = 0;
	switch (type) {
	case INTF_TEST_SET_CLK_RATE:
		ret = hisp_set_clk_rate(info->u.param_clk_rate.type, info->u.param_clk_rate.rate);
		break;
	case INTF_TEST_PHY_CSI_CONNECT:
		ret = hisp_phy_csi_connect(info->u.param_phy_csi.phy_info, info->u.param_phy_csi.csi_index);
		break;
	case INTF_TEST_PHY_CSI_DISCONNECT:
		ret = hisp_phy_csi_disconnect();
		break;
	case INTF_TEST_RPROC_ENABLE:
		ret = hisp_rproc_enable();
		break;
	case INTF_TEST_RPROC_DISABLE:
		ret = hisp_rproc_disable();
		break;
	default:
		rpmsg_err("invalid intf type.%d\n", type);
		ret = -EINVAL;
	}
	return ret;
}

static int hispdbg_intf_test_sec(int type, interface_test_t *info)
{
	int ret = 0;
	switch (type) {
	case INTF_TEST_SECBOOT_MEMSIZE:
		ret = hisp_secboot_memsize_get_from_type(info->u.param_secboot_memsize.type,
												info->u.param_secboot_memsize.size);
		break;
	case INTF_TEST_SECBOOT_PREPARE:
		ret = hisp_secboot_prepare();
		break;
	case INTF_TEST_SECMEM_CA_MAP:
		ret = hisp_secmem_ca_map(info->u.param_secmem_ca_map.pool_num,
								info->u.param_secmem_ca_map.sharefd,
								info->u.param_secmem_ca_map.size);
		break;
	case INTF_TEST_SECMEM_CA_UNMAP:
		ret = hisp_secmem_ca_unmap(info->u.param_secmem_ca_unmap.pool_num);
		break;
	default:
		rpmsg_err("invalid intf type.%d\n", type);
		ret = -EINVAL;
...
static int hispdbg_intf_test_rpm(int type, interface_test_t *info)
{
	int ret = 0;
	switch (type) {
	case INTF_TEST_DUMP_RPMSG:
		hisp_dump_rpmsg_with_id(info->u.param_dump_rpmsg.message_id);
		break;
	case INTF_TEST_RPMSGREFS_RESET:
		hisp_rpmsgrefs_reset();
		break;
	case INTF_TEST_RPMSGREFS_DUMP:
		hisp_rpmsgrefs_dump();
		break;
	case INTF_TEST_SENDIN:
		hisp_sendin((void*)(&info->u));
		break;
	case INTF_TEST_RECVDONE:
		hisp_recvdone((void*)(&info->u));
		break;
	case INTF_TEST_RECVIN:
		hisp_recvin((void*)(&info->u));
		break;
	case INTF_TEST_RECVX:
		hisp_recvx((void*)(&info->u));
		break;
	default:
		rpmsg_err("invalid intf type.%d\n", type);
		ret = -EINVAL;
...
static void hispdbg_intternal_call(void)
{
	if (hisp_nsec_device_disable() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_secmem_pa_release() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_secboot_prepare() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_secboot_unprepare() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_sec_ta_enable() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_sec_ta_disable() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_dev_select_idle() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_jpeg_powerup() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_jpeg_powerdn() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_ca_ispcpu_reset() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_ca_ispcpu_disreset() != 0)
		rpmsg_err("internal call failed\n");
	if (hisp_ca_bootmem_deconfig() != 0)
		rpmsg_err("internal call failed\n");
}

static int hispdbg_hal_intf_test(unsigned long arg)
{
	int ret;
	interface_test_t info;
	if ((void __user *)(uintptr_t)arg == NULL)
		return -EFAULT;
	ret = (int)copy_from_user(&info, (void __user *)(uintptr_t)arg, sizeof(interface_test_t));
	if (ret != 0) {
		rpmsg_err("copy_from_user.%d\n", ret);
		return -EFAULT;
	}
	hispdbg_intternal_call();
	...
	case INTF_TEST_RPROC_ENABLE:
	case INTF_TEST_RPROC_DISABLE:
		ret = hispdbg_intf_test_device(info.type, &info);
		break;
	case INTF_TEST_SECBOOT_MEMSIZE:
	case INTF_TEST_SECBOOT_PREPARE:
	case INTF_TEST_SECMEM_CA_MAP:
	case INTF_TEST_SECMEM_CA_UNMAP:
		ret = hispdbg_intf_test_sec(info.type, &info);
		break;
	case INTF_TEST_DUMP_RPMSG:
	case INTF_TEST_RPMSGREFS_RESET:
	case INTF_TEST_RPMSGREFS_DUMP:
	...
	case INTF_TEST_RECVIN:
	case INTF_TEST_RECVX:
		ret = hispdbg_intf_test_rpm(info.type, &info);
		break;
	case INTF_TEST_BOOT_STAT_DUMP:
		hisp_boot_stat_dump();
		break;
	default:
		rpmsg_err("invalid intf type.%d\n", info.type);
...
void hisp_sendin(void *data)
{
	...
	hisp_rpmsg_rdr_save((unsigned int)atomic_read(&dev->rdr_refs),
			...
			const char *buf, size_t count)
{
	...
	if (strncmp(buf, "enable", len) == 0)
		hisp_serv.enable_rpmsg = 1;
	else if (strncmp(buf, "disable", len) == 0)
		hisp_serv.enable_rpmsg = 0;
	else if (strncmp(buf, "bypasspwrdn", len) == 0)
		hisp_serv.bypass_powerdn = 1;
	else if (strncmp(buf, "usepwrdn", len) == 0)
		hisp_serv.bypass_powerdn = 0;
	else if (strncmp(buf, "dumpregs", len) == 0)
		hisp_boot_stat_dump();
	else
...
static int hispdbg_rpmsg_isp_serv_init(void)
{
	...
	ret = snprintf_s(name, HISP_NAME_LENGTH, HISP_NAME_LENGTH - 1,
			"%d", task_pid_nr(current->group_leader));
	if (ret < 0) {
		...
		map_hisp_cfg_data_t *cfg)
{
	...
	case HISP_GET_MAPBUFFER:
		node = hispdbg_get_buffer_node_by_fd((int)cfg->param.sharefd);
		...
		if (node->buf->size != cfg->param.size) {
			...
			hispdbg_put_buffer_node_by_fd((int)cfg->param.sharefd);
			return -EFAULT;
		}
...
static int hispdbg_pool_setup(map_hisp_cfg_data_t *cfg)
{
#ifdef ISP_BUF_PRIVATIZE
	int ret;
	struct hisp_map_info_s map_info = {0};

	map_info.fd = cfg->param.sharefd;
	map_info.size = cfg->param.size;
#ifdef ISP_LINX_CPU_SUPPORT
	ret = hisp_mem_map_addr(*((struct hisp_mem_type *)&cfg->param.usage),
							&map_info, &cfg->param.mem_info);
#else
	map_info.fw_mem_size = cfg->param.map_info.fw_mem_size;
	map_info.fw_mem_prot = cfg->param.prot;
	map_info.ispfw_mem_size = cfg->param.map_info.ispfw_mem_size;
	map_info.ispfw_mem_prot = cfg->param.prot;
	ret = hisp_mem_map_addr(cfg->param.usage, &map_info, &cfg->param.mem_info);
#endif
	if (ret != 0) {
		rpmsg_err("Failed : hisp_mem_map_addr\n");
		return -EINVAL;
	}
#else
	struct hisp_buffer_node *node = NULL;
...
#ifdef ISP_BUF_PRIVATIZE
	map_info.fd = (int)cfg->param.sharefd;
	map_info.size = cfg->param.size;
#ifdef ISP_LINX_CPU_SUPPORT
	ret = hisp_mem_map_addr(*((struct hisp_mem_type *)&cfg->param.usage),
							&map_info, &cfg->param.mem_info);
...
static int hispdbg_pool_destroy(map_hisp_cfg_data_t *cfg)
{
#ifdef ISP_BUF_PRIVATIZE
	int ret;
#ifdef ISP_LINX_CPU_SUPPORT
	ret = hisp_mem_unmap_addr(*((struct hisp_mem_type *)&cfg->param.usage),
							   cfg->param.mem_info);
#else
	ret = hisp_mem_unmap_addr(cfg->param.usage, cfg->param.mem_info);
#endif
	if (ret != 0) {
		rpmsg_err("Failed : hisp_mem_unmap_addr\n");
		return ret;
	}
#else
		...
		map_hisp_cfg_data_t *cfg)
{
	...
	case HISP_POOL_STEUP:
		ret = hispdbg_pool_setup(cfg);
		if (ret != 0) {
			rpmsg_err("Failed : pool_setup.%d\n", ret);
			return ret;
		}
		break;
	case HISP_POOL_DESTROY:
		ret = hispdbg_pool_destroy(cfg);
		if (ret != 0) {
			rpmsg_err("Failed : pool_destroy.%d\n", ret);
			return ret;
		}
		break;
		...
		unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != ISP_DEBUG_IOCTL_MAGIC) {
		pr_err("[%s] type is wrong.\n", __func__);
		return -EINVAL;
	}

	if (_IOC_NR(cmd) >= ISP_DEBUG_IOCTL_MAX_NR) {
		pr_err("[%s] number is wrong.\n", __func__);
		return -EINVAL;
	}

	ret1 = copy_from_user(&cfg, (void __user *)(uintptr_t)arg, sizeof(cfg));
	if (ret1 != 0) {
		rpmsg_err("Failed : copy_from_user\n");
		return -EFAULT;
	}
	...
	ret1 = copy_to_user((void __user *)(uintptr_t)arg,
				(const void *)&cfg, sizeof(cfg));
	if (ret1 != 0) {
		rpmsg_err("Failed : copy_to_user.(0x%lx, 0x%x, 0x%x)\n",
			arg, cfg.param.moduleaddr, cfg.param.size);
		return -EFAULT;
	}
...
static int hispdbg_map_iommu(struct memory_block_s *buf)
{
	...
	iova = kernel_iommu_map_dmabuf(dev, node->buf, (int)buf->prot, &size);
	if (iova == 0 || iova > UINT_MAX) {
		rpmsg_err("Failed : isp_iommu_map_dmabuf\n");
		goto err_dma_buf_get;
	}

	if ((unsigned int)buf->size != size) {
		rpmsg_err("isp_iommu_map_range failed: %d len 0x%lx\n",
				buf->size, size);
		goto err_dma_buf_get;
	}

	buf->da = (unsigned int)iova;
		...
		unsigned long arg)
{
	...
	if (sizeof(arg) > sizeof(struct memory_block_s)) {
		rpmsg_err("cmd arg size too large\n");
		return -EINVAL;
	}
	...
	ret = (int)copy_from_user(&buf, (void __user *)(uintptr_t)arg, sizeof(struct memory_block_s));
	if (ret != 0) {
		rpmsg_err("copy_from_user.%d\n", ret);
		return -EFAULT;
	}
	...
	ret = (int)copy_to_user((void __user *)(uintptr_t)arg, &buf,
...
#if defined(HISP_KERNEL_FUZZ) || defined(HISP_FUZZ)
	ret = hisp_set_boot_mode(NONSEC_CASE);
	if (ret) {
		rpmsg_err("Failed: boot mode\n");
		return -ENODEV;
	}
	isp_serv->enable_rpmsg = 1;
#endif

	if (isp_serv->enable_rpmsg == 0) {
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case HISP_UNMAP_IOMMU:
		...
		break;
	case HISP_INTERFACE_TEST_HAL:
		ret = hispdbg_hal_intf_test(arg);
		if (ret) {
			rpmsg_err("hispdbg_hal_intf_test fail\n");
			goto fail_ioctrl_cmd;
		}
		break;
	default:
		...
		size_t len, loff_t *offp)
{
	...
	if (skb_queue_empty(&hisp_serv.queue)) {
		...
		if (wait_event_interruptible(hisp_serv.readq, empty == 0))
			return -ERESTARTSYS;
	...
	struct hisp_rpmsg_service *priv, void *data, unsigned int len)
{
	...
	ret = rpmsg_send_offchannel(hisp_serv.ept, isp_serv->ept->addr,
					isp_serv->rpdev->dst, data, (int)len);
	if (ret) {
		rpmsg_err("rpmsg_send failed: %d\n", ret);
		return ret;
	}
		...
		struct file *filp, struct poll_table_struct *wait)
{
	...
	if (skb_queue_empty(&hisp_serv.queue) == 0)
