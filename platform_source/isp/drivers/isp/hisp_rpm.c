		...
		unsigned long arg)
{
	map_hisp_cfg_data_t cfg;
	int ret = 0;
	void __user *addr = (void __user *)(uintptr_t)arg;
	unsigned int size;
	size = (unsigned int)sizeof(cfg);
	...
	if ((size + (uintptr_t)addr) < (uintptr_t)addr)
		return -EINVAL;

	ret = (int)copy_from_user(&cfg, (void __user *)(uintptr_t)arg, size);
	if (ret != 0) {
	...
	ret = (int)copy_to_user((void __user *)(uintptr_t)arg,
