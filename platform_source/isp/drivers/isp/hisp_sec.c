...
#define RPMSG_ISP_C0_FEATURES           1
#define RPMSG_VQ0_SIZE                  256
#define RPMSG_VQ1_SIZE                  256
#define OS_ADDR_MAX                     0x8000
...
struct hisp_sec {
	...
	unsigned int ignore_sec_text_size;
...
struct isp_sec_vdev {
	...
	unsigned int d_feature;
	unsigned int g_feature;
...
struct isp_sec_rsc_table {
	...
	/* rpmsg vring mem kernel 5.10 */
	struct isp_sec_rsv_mem vdev0_vring0_mem;
	struct isp_sec_rsv_mem vdev0_vring1_mem;

	/* rpmsg vdev entry */
	struct isp_sec_vdev       rpmsg_vdev;
	struct isp_sec_vdev_vring  rpmsg_vring0;
	struct isp_sec_vdev_vring  rpmsg_vring1;
};

static struct isp_sec_rsc_table g_isp_sec_rsc_table = {
	.version    = 1,
	.num        = 3,
	.reserved   = {0, 0},
	...
	/* offsets to entries */
	.offset = {
		(unsigned int)(uintptr_t)&(((struct isp_sec_rsc_table *)0)->vdev0_vring0_mem),
		(unsigned int)(uintptr_t)&(((struct isp_sec_rsc_table *)0)->vdev0_vring1_mem),
		(unsigned int)(uintptr_t)&(((struct isp_sec_rsc_table *)0)->rpmsg_vdev),
	},

	.vdev0_vring0_mem = {
		RSC_VRING0MEM, ISP_RPMSG_VR0_BASE, 0,
		ISP_RPMSG_VR_SIZE, (IOMMU_READ | IOMMU_WRITE), 0, "vdev0vring0",
	},

	.vdev0_vring1_mem = {
		RSC_VRING1MEM, ISP_RPMSG_VR1_BASE, 0,
		ISP_RPMSG_VR_SIZE, (IOMMU_READ | IOMMU_WRITE), 0, "vdev0vring1",
	},

	/* rpmsg vdev entry */
	.rpmsg_vdev = {
		RSC_VDEV, VIRTIO_ID_HISP_RPMSG, 0,
		RPMSG_ISP_C0_FEATURES, 0, 0, 0, 2, { 0, 0 },
		/* no config data */
	},

	/* the two vrings */
	.rpmsg_vring0 = { ISP_RPMSG_VR0_BASE, 4096, RPMSG_VQ0_SIZE, 1, 0 },
	.rpmsg_vring1 = { ISP_RPMSG_VR1_BASE, 4096, RPMSG_VQ1_SIZE, 2, 0 },
};
...
unsigned int hisp_sectype_is_valid(unsigned int type)
{
	...
	if (type == HISP_SEC_TEXT && dev->ignore_sec_text_size == 1)
		return SEC_TYPE_INVALID;

	return SEC_TYPE_VALID;
}
					...
					unsigned int vaddr, unsigned int offset)
{
	...
	(void)vaddr;
	rproc = platform_get_drvdata(dev->pdev);
	if (rproc == NULL) {
...
int hisp_secboot_memsize_get_from_type(unsigned int type, unsigned int *size)
{
	...
	is_valid = hisp_sectype_is_valid(type);
	if (is_valid == SEC_TYPE_INVALID) {
		pr_info("[%s] ignore invalid type[%d] at current platform, just return 0\n", __func__, type);
		mutex_unlock(&dev->ca_mem_mutex);
		return 0;
	}
...
int hisp_secboot_prepare(void)
{
	...
	for (index = HISP_SEC_TEXT; index < HISP_SEC_BOOT_MAX_TYPE; index++) {
		is_valid = hisp_sectype_is_valid(index);
		if (is_valid == SEC_TYPE_INVALID) {
			pr_info("[%s] ignore type[%d] sfd check, just continue\n", __func__, index);
			continue;
		}
	...
	ret = hisp_ca_bootmem_config(dev->boot_mem, HISP_SEC_BOOT_MAX_TYPE,
			dev->rsv_mem, HISP_SEC_RSV_MAX_TYPE);
	if (ret < 0) {
		dev->boot_mem_ready_flag = 0;
...
int hisp_sec_pwron_prepare(void)
{
	...
	if (dev->use_sec_encrypt)
		ret = memcpy_s((void *)(dev->rsctable_vaddr_const), sizeof(g_isp_sec_rsc_table),
					(void *)&g_isp_sec_rsc_table, sizeof(g_isp_sec_rsc_table));
	else
		ret = hisp_bsp_read_bin("isp_firmware", dev->rsctable_offset,
			dev->rsctable_size, dev->rsctable_vaddr_const);
	if (ret < 0) {
		pr_err("[%s] hisp_bsp_read_bin read rsctable failed.%d\n", __func__, ret);
		return ret;
	}
	pr_info("[%s] -\n", __func__);
			...
			struct hisp_sec *dev)
{
	...
	if (dev->rsctable_offset >= OS_ADDR_MAX ||
		dev->rsctable_size >= OS_ADDR_MAX ||
		dev->rsctable_offset + dev->rsctable_size >= OS_ADDR_MAX) {
		pr_err("[%s] Failed: rsctable offset or size invalid\n", __func__);
		dev->rsctable_offset = 0;
		dev->rsctable_size = 0;
	}
			...
			struct hisp_sec *dev)
{
	...
	ret = of_property_read_u32(np, "use-sec-encrypt", &dev->use_sec_encrypt);
	if (ret < 0) {
		pr_err("Failed : use_sec_encrypt.%d.ret.%d\n", dev->use_sec_encrypt, ret);
		dev->use_sec_encrypt = 0;
	}
	pr_info("use_sec_encrypt.%d of_property_read_u32.%d\n", dev->use_sec_encrypt, ret);
...
static int hisp_sec_getdts(struct platform_device *pdev, struct hisp_sec *dev)
{
	...
	if (dev->mapping_items * sizeof(unsigned int) > sizeof(struct hisp_ca_meminfo)) {
		pr_info("[%s] Failed: mapping_items invalid.%d\n", __func__, dev->mapping_items);
		dev->mapping_items = 0;
	}
		...
		unsigned int etype)
{
	...
	ret = of_property_read_u32(np, "ignore-sec-text-size",
			(unsigned int *)(&dev->ignore_sec_text_size));
	if (ret < 0) {
		pr_err("[%s] Failed: ignore-sec-text-size.ret.%d\n", __func__, ret);
		dev->ignore_sec_text_size = 0;
	}
	pr_info("ignore-sec-text-size 0x%x\n", dev->ignore_sec_text_size);
