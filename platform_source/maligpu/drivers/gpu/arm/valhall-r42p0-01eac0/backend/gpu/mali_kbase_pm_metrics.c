...
int kbasep_pm_metrics_init(struct kbase_device *kbdev)
{
	...
	kbdev->pm.backend.metrics.thro_hint_prev=
		KBASE_JS_ATOM_SCHED_THRO_DEFAULT;
	atomic_set(&kbdev->gpu_dev_data.thro_enable, 1);
#endif
...
#endif
	mutex_init(&kbdev->pm.backend.metrics.m_lock);
	return 0;
}
...
#endif
	mutex_destroy(&kbdev->pm.backend.metrics.m_lock);
}
...
void kbase_pm_get_dvfs_action(struct kbase_device *kbdev)
{
	...
	int util_cl_share[2];
#endif
	mutex_lock(&kbdev->pm.backend.metrics.m_lock);
...
#endif
	mutex_unlock(&kbdev->pm.backend.metrics.m_lock);
}
...
void kbase_pm_metrics_start(struct kbase_device *kbdev)
{
	...
	if (unlikely(!metrics->initialized))
		return;
	enum dvfs_metric_timer_state e_timer_state;
#ifndef CONFIG_PM_DEVFREQ
	e_timer_state = TIMER_ON;
#else
	e_timer_state = TIMER_OFF;
#endif /* CONFIG_PM_DEVFREQ */
	/* Transition to ON, from a stopped state (transition c) */
	if (atomic_xchg(&metrics->timer_state, e_timer_state) == TIMER_OFF) {
#ifndef CONFIG_PM_DEVFREQ
