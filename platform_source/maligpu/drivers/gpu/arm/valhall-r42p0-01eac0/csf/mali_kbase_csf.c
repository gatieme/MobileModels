...
#else
	(void)kthread_queue_work(kctx->csf.pending_wq, &kctx->csf.pending_submission_work);
#endif
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	kctx->csf.pending_wq = kthread_create_worker(0, "pending_wq");
	if (IS_ERR(kctx->csf.pending_wq)) {
		dev_err(kctx->kbdev->dev, "alloc workqueue pending_wq fail.");
		return err;
	}
	sched_set_fifo(kctx->csf.pending_wq->task);
#endif
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
						INIT_WORK(&kctx->csf.pending_submission_work, pending_submission_worker);
#else
						kthread_init_work(&kctx->csf.pending_submission_work, pending_submission_worker);
#endif
				} else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	if (kctx->csf.pending_wq != NULL) {
		kthread_destroy_worker(kctx->csf.pending_wq);
		kctx->csf.pending_wq = NULL;
	}
#endif
...
#ifdef CONFIG_HW_VOTE_GPU_FREQ
	if (kbdev->gpu_dev_data.error_num.cs_fault == 0) {
		u32 first_freq = 0;
		hv_get_last(kbdev->gpu_dev_data.gpu_hvdev, &first_freq);
		kbdev->gpu_dev_data.error_num.first_fault_freq = first_freq;
	}
#endif

#ifdef CONFIG_HUAWEI_DSM
		if (dsm_report_enable(kbdev->gpu_dev_data.error_num.cs_fault, 1000,
			kbdev->gpu_dev_data.factory_mode)) {
			u32 freq = 0;
		#ifdef CONFIG_HW_VOTE_GPU_FREQ
			hv_get_last(kbdev->gpu_dev_data.gpu_hvdev, &freq);
		#endif
			gpu_dsm_report(kbdev, GPU_BIT_STUCK_DSM_NO,
				"Gpu CS_FAULT cs_fault = %u, current freq is %u, first fault freq is %u!!!!",
				kbdev->gpu_dev_data.error_num.cs_fault,
				(unsigned long)freq * KHZ,
				(unsigned long)kbdev->gpu_dev_data.error_num.first_fault_freq * KHZ);
		}
#endif
