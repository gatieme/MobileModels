					    ...
					    struct kbase_kcpu_command_fence_info *fence_info)
{
	...
	if ((kctx->kbdev->gpu_dev_data.game_pid == kctx->tgid) &&
		(kcpu_queue->ai_freq_flag))
		kctx->kbdev->gpu_dev_data.signaled_seqno++;
#endif
					   ...
					   struct kbase_kcpu_command *current_command)
{
	...
	if (fence.basep.ai_freq_flag)
		kcpu_queue->ai_freq_flag = true;
	else
		kcpu_queue->ai_freq_flag = false;
#endif
	...
	for (i = 0; i != queue->num_pending_cmds; ++i) {
		...
		case BASE_KCPU_COMMAND_TYPE_GROUP_SUSPEND: {
			...
			if (!sus_buf->cpu_alloc) {
				for (int i = 0; i < sus_buf->nr_pages; i++)
