...
static int kbase_csf_reset_gpu_now(struct kbase_device *kbdev, bool firmware_inited, bool silent)
{
	...
	dev_err(kbdev->dev, "skip Disable GPU hardware counters.\n");
	/* kbase_hwcnt_context_disable whose function is counters disabled is not invoked due to stability problems. */
