...
void kbase_csf_tiler_heap_reclaim_mgr_init(struct kbase_device *kbdev)
{
	...
	// register_shrinker(reclaim);  // arm patch workaround for the screen flickers
#else
...
#if !defined(CONFIG_MALI_VECTOR_DUMP)
	// unregister_shrinker(&scheduler->reclaim_mgr.heap_reclaim); // arm patch workaround for the screen flickers
#endif
