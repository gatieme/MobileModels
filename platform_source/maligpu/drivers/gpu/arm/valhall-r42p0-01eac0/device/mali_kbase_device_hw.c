...
/* Maximum number of loops polling the GPU for an AS command to complete before we assume
 * the GPU has hung to avoid watchdog timeout */
#define KBASE_AS_INACTIVE_MAX_LOOPS_MODIFIED     50000
...
static int busy_wait_on_irq(struct kbase_device *kbdev, u32 irq_bit)
{
	 ...
	 */
	unsigned int max_loops = KBASE_AS_INACTIVE_MAX_LOOPS_MODIFIED;
