...
static ssize_t show_gpu_error_info(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	...
	total_error_num = kbdev->gpu_dev_data.error_num.page_fault +
		kbdev->gpu_dev_data.error_num.gpu_fault +
		kbdev->gpu_dev_data.error_num.hard_reset +
		kbdev->gpu_dev_data.error_num.cs_fault;

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"page_fault num:%d\n",
		kbdev->gpu_dev_data.error_num.page_fault);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"gpu_fault num:%d\n",
		kbdev->gpu_dev_data.error_num.gpu_fault);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"last_time:%llu\n",
		kbdev->gpu_dev_data.error_num.ts/1000000000 + 1);

	return ret;
}

static ssize_t set_gpu_error_info(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	...
	kbdev->gpu_dev_data.error_num.gpu_fault = 0;
	kbdev->gpu_dev_data.error_num.page_fault = 0;
	kbdev->gpu_dev_data.error_num.hard_reset = 0;
	kbdev->gpu_dev_data.error_num.cs_fault = 0;
	kbdev->gpu_dev_data.error_num.first_fault_freq = 0;
	kbdev->gpu_dev_data.error_num.ts = 0;
	return count;
}

static DEVICE_ATTR(gpu_error_info, S_IRUGO | S_IWUSR,
	show_gpu_error_info, set_gpu_error_info);
	...
	struct device_attribute *attr, char * const buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%i\n",
		atomic_read(&kbdev->gpu_dev_data.thro_enable));
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cur_thro_enable = atomic_read(&kbdev->gpu_dev_data.thro_enable);
	if (thro_enable != cur_thro_enable) {
		atomic_set(&kbdev->gpu_dev_data.thro_enable, thro_enable);
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	kbdev->gpu_dev_data.clock = clk_get(kbdev->dev, "clk_g3d");
	if (IS_ERR_OR_NULL(kbdev->gpu_dev_data.clock)) {
		err = PTR_ERR(kbdev->gpu_dev_data.clock);
		kbdev->gpu_dev_data.clock = NULL;
		if (err == -EPROBE_DEFER) {
			dev_err(kbdev->dev, "Failed to get clock\n");
			goto fail;
		}
		dev_info(kbdev->dev, "Continuing without Mali clock control\n");
		/* Allow probe to continue without clock. */
	} else {
		err = clk_prepare_enable(kbdev->gpu_dev_data.clock);
		if (err) {
			dev_err(kbdev->dev,
...
fail:
	...
	if (kbdev->gpu_dev_data.clock != NULL) {
		clk_put(kbdev->gpu_dev_data.clock);
		kbdev->gpu_dev_data.clock = NULL;
	}
...
void power_control_term(struct kbase_device *kbdev)
{
	...
	if (kbdev->gpu_dev_data.clock) {
		clk_disable_unprepare(kbdev->gpu_dev_data.clock);
		clk_put(kbdev->gpu_dev_data.clock);
		kbdev->gpu_dev_data.clock = NULL;
	}
}
...
static struct attribute *kbase_attrs[] = {
	...
	&dev_attr_runtime_pm_delay.attr,
#endif /* !MALI_USE_CSF */
