		...
		for (i = 0; i < alloc->imported.alias.nents; i++) {
			if (alloc->imported.alias.aliased[i].alloc) {
				...
				err = kbase_mmu_insert_pages(
					...
					NULL, ignore_page_migration, alloc->imported.alias.aliased[i].length);
				if (err)
					goto bad_insert;
		...
		struct kbase_va_region *reg, unsigned long flags)
{
	...
	if (flags & BASE_MEM_SCRAMBLE_BIT)
		reg->flags |= KBASE_REG_SCRAMBLE_BIT;
		...
		struct kbase_va_region *reg)
{
	...
	err = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn, pa,
				     ...
				     kctx->as_nr, alloc->group_id, mmu_sync_info, NULL, true, kbase_reg_current_backed_size(reg));
	if (err == 0)
		return 0;
