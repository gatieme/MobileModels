					...
					enum kbase_caller_mmu_sync_info mmu_sync_info)
{
	...
	/* If the region has flag KBASE_REG_SCRAMBLE_BIT, we will set the
	 * header_page_number of alloc memory to zero to set the scramble,
	 * if not, we do not set the scramble.
	 */
	if (reg->flags & KBASE_REG_SCRAMBLE_BIT) {
		reg->gpu_alloc->header_page_number = 0;
	} else {
		reg->gpu_alloc->header_page_number = kbase_reg_current_backed_size(reg);
	}
...
#endif
	header_page_number = reg->gpu_alloc->header_page_number ? reg->gpu_alloc->header_page_number:
		kbase_reg_current_backed_size(reg);

	err = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn,
				     kbase_get_gpu_phy_pages(reg),
				     kbase_reg_current_backed_size(reg), reg->flags & gwt_mask,
				     kctx->as_nr, alloc->group_id, mmu_sync_info, NULL, true, header_page_number);
	if (err)
		goto bad_insert;
 ...
 * @header_page_number: Number of pages for store afbc header
 *
...
static struct kbase_va_region *kbase_mem_from_umm(struct kbase_context *kctx,
		int fd, u64 *va_pages, u64 *flags, u32 padding, u64 header_page_number)
{
	struct kbase_va_region *reg;
	struct dma_buf *dma_buf;
	...
	*flags |= BASE_MEM_CACHED_CPU;
	...
	/* If header_page_number of Import memory is zero, we do not set the scramble,
	 * if not, we set the scramble.
	 */
	if (header_page_number == 0) {
		reg->gpu_alloc->header_page_number = kbase_reg_current_backed_size(reg);
	} else {
			       ...
			       enum kbase_caller_mmu_sync_info mmu_sync_info)
{
	...
	/* allocate no scramble memory, set header_page_number to current allocate page number */
	ret = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, reg->start_pfn + old_pages,
				     phy_pages + old_pages, delta, reg->flags, kctx->as_nr,
				     reg->gpu_alloc->group_id, mmu_sync_info, reg, false, delta);
