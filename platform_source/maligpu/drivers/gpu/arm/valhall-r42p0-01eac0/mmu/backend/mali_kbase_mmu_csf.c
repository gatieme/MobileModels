...
#include "platform/hisilicon/mali_kbase_config_platform.h"
#include "platform/hisilicon/mali_kbase_config_hifeatures.h"
#ifdef CONFIG_DFX_BB
 ...
 */
void kbase_mmu_report_fault_and_kill(struct kbase_va_region *region,
		struct kbase_context *kctx,
		...
		struct kbase_fault *fault)
{
...
#if (defined(CONFIG_MALI_GPU_BUG_ON) && defined(CONFIG_DFX_BB)) || defined(CONFIG_HUAWEI_DSM)
	...
	if (!pool->dying) {
		...
		if (dsm_report_enable(kbdev->gpu_dev_data.error_num.page_fault,
			30, kbdev->gpu_dev_data.factory_mode))
			gpu_dsm_report(kbdev, GPU_PAGE_FAULT_DSM_NO,
...
#if defined(CONFIG_MALI_GPU_BUG_ON) && defined(CONFIG_DFX_BB)
	kbase_mem_pool_lock(pool);
	if (!pool->dying) {
			if (kbase_has_hi_feature(kbdev,
					KBASE_FEATURE_MMU_FAULT_FILTERING) &&
					!kbdev->gpu_dev_data.factory_mode) {
					kbdev->gpu_dev_data.unhandled_page_fault_count++;
					if (kbdev->gpu_dev_data.unhandled_page_fault_count >= 3)
