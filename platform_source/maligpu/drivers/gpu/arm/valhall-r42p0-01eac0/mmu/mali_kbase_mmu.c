...
page_fault_retry:
	...
	if (grown) {
		...
		err = kbase_mmu_insert_pages_no_flush(
			...
			region->gpu_alloc->group_id, &dirty_pgds, region, false, new_pages);
		if (err) {
...
u64 kbase_mmu_create_ate_scramble_bit(struct kbase_device *const kbdev,
	struct tagged_addr const phy, unsigned long const flags,
	int const level, int const group_id)
{
	u64 entry;

	kbdev->mmu_mode->entry_set_ate_scramble_bit(&entry, phy, flags, level);
	return kbdev->mgm_dev->ops.mgm_update_gpu_pte(kbdev->mgm_dev,
		group_id, level, entry);
}
	...
	while (remain) {
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			for (i = 0; i < count; i++) {
				...
				{
					if (header_index < header_page_number)
					{
						*target = kbase_mmu_create_ate(kbdev,
							phys[i], flags, cur_level, group_id);
						header_index++;
					...
					{
						*target = kbase_mmu_create_ate_scramble_bit(kbdev,
							phys[i], flags, cur_level, group_id);
					}
				}
...
int kbase_mmu_insert_pages(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 vpfn,
			   ...
			   struct kbase_va_region *reg, bool ignore_page_migration, u64 header_page_number)
{
	int err;
	u64 dirty_pgds = 0;
	LIST_HEAD(free_pgds_list);
	...
	err = kbase_mmu_insert_pages_no_flush(kbdev, mmut, vpfn, phys, nr, flags, group_id,
					      &dirty_pgds, reg, ignore_page_migration, header_page_number);
	if (err)
		return err;
...
void kbase_mmu_disable(struct kbase_context *kctx)
{
	...
	if (lock_err)
		dev_err(kbdev->dev, "Failed to lock AS %d for ctx %d_%d", kctx->as_nr, kctx->tgid, kctx->id);
	...
	if (kbdev->pm.backend.l2_state == KBASE_L2_ON) {
		flush_err = kbase_gpu_cache_flush_and_busy_wait(kbdev, GPU_COMMAND_CACHE_CLN_INV_L2_LSC);
		if (flush_err)
			dev_err(kbdev->dev, "Failed to flush GPU cache when disabling AS %d for ctx %d_%d", kctx->as_nr, kctx->tgid,
				kctx->id);
	}
	...
	if (!lock_err) {
		...
		lock_err = kbase_mmu_hw_do_unlock_no_addr(kbdev, &kbdev->as[kctx->as_nr], &op_param);
		if (lock_err)
			dev_err(kbdev->dev, "Failed to unlock AS %d for ctx %d_%d", kctx->as_nr, kctx->tgid, kctx->id);
	}
#if !MALI_USE_CSF
	/*
	 ...
	 */
	kbase_backend_slot_kctx_purge_locked(kctx, kctx);
#endif
