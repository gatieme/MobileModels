...
/* Maximum number of loops polling the GPU for an AS command to complete before we assume the GPU ha
s hung to avoid watchdog timeout */
#define KBASE_AS_INACTIVE_MAXIMUM_LOOPS     50000
...
static int wait_ready(struct kbase_device *kbdev,
		unsigned int as_nr)
{
	u32 max_loops = KBASE_AS_INACTIVE_MAXIMUM_LOOPS;
...
static int wait_l2_power_trans_complete(struct kbase_device *kbdev)
{
	...
	const u32 pwr_trans_wait_time_ms = kbdev->reset_timeout_ms;
	u64 val = 0;
	s64 diff;
	...
	do {
		...
		for (i = 0; i < 1000; i++) {
			val = (u64) kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_HI)) << 32;
			val |= kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_LO));
			if (!val)
				return 0;
		}
	...
	dev_warn(kbdev->dev, "L2_PWRTRANS %016llx set for too long", val);
	...
	if (mmu_cmd == AS_COMMAND_FLUSH_MEM) {
		...
		if (ret)
			dev_warn(kbdev->dev,
				...
				op_param->vpfn << PAGE_SHIFT, ((op_param->vpfn + op_param->nr) << PAGE_SHIFT) - 1, as->number);
