...
static int __init protected_memory_allocator_driver_init(void)
{
	int ret;

	ret = platform_driver_register(&protected_memory_allocator_driver);
	if (ret)
		pr_err("[mali] protected_memory_allocator_driver init failed!\n");

	return ret;
}
rootfs_initcall(protected_memory_allocator_driver_init);
