...
static int kbase_set_hi_features_mask(struct kbase_device *kbdev)
{
	...
	dev_info(kbdev->dev,
		...
		(gpu_vid & GPU_ID_VERSION_MAJOR) >> KBASE_GPU_ID_VERSION_MAJOR_SHIFT,
		(gpu_vid & GPU_ID_VERSION_MINOR) >> KBASE_GPU_ID_VERSION_MINOR_SHIFT,
		((gpu_vid & GPU_ID_VERSION_STATUS) >>
			KBASE_GPU_ID_VERSION_STATUS_SHIFT));
...
static inline void kbase_platform_on(struct kbase_device *kbdev)
{
	...
	if (kbdev->gpu_dev_data.regulator && !refcount) {
		...
		while (loop < max_loop && unlikely(regulator_enable(kbdev->gpu_dev_data.regulator))) {
			dev_err(kbdev->dev, "Failed to enable regulator, retry...%d \n", ++loop);
			WARN_ON(1);
...
/*
 * The register needs to be read before the gpu_powered flag bit is set to initialize hi_features.
 * This operation is not reasonable. However, due to the invoking sequence of the boot process,
 * this operation must be performed only once when the GPU is started.
 * If this operation is not performed, the register cannot be read. As a result,
 * features such as dvfs cannot be enabled. If this operation is performed every time,
 * the resume process of the system is abnormal.
 */
		...
		gpuflink_resume(kbdev->gpu_dev_data.devfreq);
#elif defined(CONFIG_MALI_MIDGARD_DVFS)
		if (kbase_platform_dvfs_enable(kbdev, TIMER_ON, 0) != MALI_TRUE)
			return -EPERM;
#endif
	} else {
...
static inline void kbase_platform_off(struct kbase_device *kbdev)
{
	...
	if (kbdev->gpu_dev_data.regulator && refcount) {
		...
		const int delay_step = 5;
#ifdef CONFIG_PM_DEVFREQ
		gpuflink_suspend(kbdev->gpu_dev_data.devfreq);
		devfreq_suspend_device(kbdev->gpu_dev_data.devfreq);
#elif defined(CONFIG_MALI_MIDGARD_DVFS)
		kbase_platform_dvfs_enable(kbdev, TIMER_OFF, 0);
#endif
		while (loop < max_loop && unlikely(regulator_disable(kbdev->gpu_dev_data.regulator))) {
			dev_err(kbdev->dev, "Failed to disable regulator, retry...%d \n", ++loop);
			udelay(delay_step * loop);
		}
...
#ifdef CONFIG_MALI_VALE
	dev_info(kbdev->dev, "gpu_fpga_exist:%d\n", kbdev->gpu_dev_data.gpu_fpga_exist);
	if (kbdev->gpu_dev_data.gpu_fpga_exist == 0) {
...
static int kbase_gpu_init_device_tree(struct kbase_device * const kbdev)
{
	...
	if (np) {
		...
		if (ret)
			dev_warn(kbdev->dev,
			"Failed to get shader present configuration!\n");
	} else {
		dev_err(kbdev->dev,
...
static int kbase_platform_init(struct kbase_device *kbdev)
{
	...
	/* TODO: DVFS temporary scheme code */
	kbdev->clocks[0] = devm_clk_get(dev, NULL);
	kbdev->nr_clocks = 1;
...
static inline void pm_callback_resume(struct kbase_device *kbdev)
{
	...
	pm_callback_power_on(kbdev);
#endif
	kbase_gpu_devfreq_resume(kbdev);
}
