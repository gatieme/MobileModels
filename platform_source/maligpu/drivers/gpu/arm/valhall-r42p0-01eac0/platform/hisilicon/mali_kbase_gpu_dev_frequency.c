...
int kbase_platform_dvfs_enable(struct kbase_device *kbdev, enum dvfs_metric_timer_state timer_state,
	int freq)
{
	unsigned long flags;
	...
	if (timer_state != atomic_read(&kbdev->pm.backend.metrics.timer_state)) {
		if (atomic_xchg(&kbdev->pm.backend.metrics.timer_state, TIMER_ON) == TIMER_OFF) {
