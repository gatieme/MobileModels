 ...
 * Description: This file describe GPU related data structs
 * Create: 2014-2-24
 *
 ...
 *
 */
#ifndef MALI_KBASE_PLATFORM_H
#define MALI_KBASE_PLATFORM_H
...
struct kbase_gpu_device_data {
	...
	struct hvdev *gpu_hvdev;
#endif /* CONFIG_HW_VOTE_GPU_FREQ */
...
/**
 * struct kbase_gpu_ctx_data - all gpu platform data in context level.
 *
...
#define GPU_SH_NV_MARK           922002006
...
do { \
	if (kbdev->gpu_dev_data.gpu_dsm_client) { \
		if (!dsm_client_ocuppy(kbdev->gpu_dev_data.gpu_dsm_client)) { \
			dsm_client_record(kbdev->gpu_dev_data.gpu_dsm_client, \
				fmt, ##args); \
			dsm_client_notify(kbdev->gpu_dev_data.gpu_dsm_client, \
				dsm_no); \
...
#define gpu_uint64_mulcheck(a, b) do { \
...
#define gpu_uint64_addcheck(a, b) do { \
...
#else
#define gpu_uint64_mulcheck(a, b) CSTD_NOP()
#define gpu_uint64_addcheck(a, b) CSTD_NOP()
#endif /* CONFIG_MALI_DEBUG */

/**
 * gpu_dmd_msg_check - dmd msg check
 *
