...
static uint32_t efuse_check_param(struct efuse_desc *desc)
{
	...
	if (desc == NULL || desc->buf == NULL) {
		efuse_err("[%s]: error, null pointer\n", __func__);
	...
	if (start_bit >= EFUSE_MAX_SIZE || bit_cnt > EFUSE_MAX_SIZE ||
		...
		efuse_err("[%s]: error, invalid params, start_bit = %u, \
	...
	if (buf_size < tmp_len) {
		efuse_err("[%s]: error, buf_size=%u, min need_size=%u\n", __func__, buf_size, tmp_len);
		return EFUSE_INVALID_PARAM_ERR;
	}
...
static uint32_t read_single_interval(struct efuse_desc *desc)
{
	...
	if (ret != EFUSE_OK || desc->bit_cnt > EFUSE_BITS_PER_GROUP) {
		efuse_err("[%s]: error, invalid params\n", __func__);
		return EFUSE_INVALID_PARAM_ERR;
	}
	...
	ret = efuse_ffa(&args);
	if (ret != EFUSE_OK) {
		efuse_err("[%s]: error, ffa failed, ffa_id=0x%08X, ret=0x%08X, item_vid=0x%08X\n", \
		    __func__, args.arg0, ret, desc->item_vid);
...
static uint32_t read_single_interval(struct efuse_desc *desc)
{
	...
	ret = efuse_smc(&args);
	if (ret != EFUSE_OK) {
		efuse_err("[%s]: error, smc failed, smc_id=0x%08X, ret=0x%08X, item_vid=0x%08X\n", \
		    __func__, args.arg0, ret, desc->item_vid);
...
static uint32_t split_deal_read_intervals(struct efuse_desc *desc)
{
	...
	while (bit_cnt > 0) {
		...
		if (ret != EFUSE_OK) {
			...
			efuse_err("[%s]: error,read efuse failed, start_bit=%u, cur_cnt=%u\n", \
...
uint32_t efuse_read_process(struct efuse_desc *desc)
{
	...
	if (desc == NULL) {
		efuse_err("[%s]:error, input null pointer\n", __func__);
		return EFUSE_INVALID_PARAM_ERR;
	}
	...
	if (ret != EFUSE_OK) {
		efuse_err("[%s]:error, split read ret=0x%08X\n", __func__, ret);
		return ret;
	}
