...
#ifdef CONFIG_EFUSE_SUPPORT_FFA
	if (memcpy_s((void *)tmp_buf, sizeof(tmp_buf), (void *)buf, size) != EOK) {
...
#ifdef CONFIG_EFUSE_SUPPORT_FFA
	ret = (s32)efuse_custom_read(buf, &size, EFUSE_TZSP_KERNEL_RD_AVS);
#else
...
static s32 __ioctl_read_efuse(uint32_t cmd, void __user *argp, uint32_t item_vid)
{
	...
	for (i = 0; i < local.bytes; i++)
		pr_info("%02x ", __func__, local.value[i]);
	pr_info("\n");

	ret = (u32)copy_to_user(argp, &local, sizeof(local));
	if (ret != 0) {
