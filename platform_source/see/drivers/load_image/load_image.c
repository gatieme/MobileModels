...
/* read vrl flag */
#define READ_VRL_SUCCESS      0
#define READ_VRL_UNSUCCESS    -1
/* 512KB */
#define VRL_ALL_BUFLEN        0x80000
/* use for all vrl data */
static u8 *g_vrl_buffer = NULL;
static u8 *g_vrl_backup_buffer = NULL;

static s32 g_read_vrl_issuccess = READ_VRL_UNSUCCESS;
			...
			char *buffer)
{
	...
	if (IS_ERR(fp)) {
		sec_print_err("filp_open(%s) failed 0x%llx\n", pathname, fp);
		ret = SEC_ERROR;
		goto free_pname;
	}
...
static s32 copy_vrl_data(u32 vrl_type, u32 offset, u32 length,
			char *buffer)
{
	s32 ret = SEC_ERROR;

	if (g_read_vrl_issuccess != READ_VRL_SUCCESS) {
		sec_print_err("load vrl is not on init.\n");
		ret = read_vrl_dev();
		if (ret != SEC_OK)
			return ret;
	}
	mutex_lock(&g_load_image_lock);
	if (!buffer) {
		sec_print_err("[%s]:buffer(%pK) is null\n", __func__, buffer);
		goto err_out;
	}
	if ((offset > VRL_ALL_BUFLEN) || ((offset + length) > VRL_ALL_BUFLEN) ||
	   ((offset + length) <= offset)) {
		sec_print_err("[%s]:offset[%u] or length[%u] is overflow.\n", __func__, offset, length);
		goto err_out;
	}
	switch (vrl_type) {
	case PRIMVRL:
		if (g_vrl_buffer != NULL ) {
			ret = memcpy_s(g_secboot_buffer, length, (g_vrl_buffer + offset), length);
			if (ret != SEC_OK)
				sec_print_err("failed to copy vrl data!\n");
		}
		break;
	case BACKVRL:
		if (g_vrl_backup_buffer != NULL ) {
			ret = memcpy_s(g_secboot_buffer, length, (g_vrl_backup_buffer + offset), length);
			if(ret != SEC_OK)
				sec_print_err("failed to copy vrl_backup data!\n");
		}
		break;
	default:
		sec_print_err("[%s]:vrl type is error!\n", __func__);
		ret = SEC_ERROR;
		break;
	}
err_out:
		   ...
		   const char *partion_name, u32 vrl_type)
{
	...
	/* get image vrl info */
	ret  = copy_vrl_data(vrl_type, (vrl_index * VRL_SIZE),
...
s32 bsp_load_and_verify_image(struct load_image_info *img_info)
{
	...
	sec_print_err("[bsp_load_and_verify_image] `%s`\n", img_info->partion_name);
	...
	for (vrl_type = PRIMVRL; vrl_type <= BACKVRL; vrl_type++) {
		...
		sec_print_err("load_vrl_to_os\n");
		ret = load_vrl_to_os(&session, img_type, img_info->partion_name,
				     vrl_type);
		if (ret != SEC_OK) {
			sec_print_err("load_vrl_to_os fail, vrl_type=%u\n", vrl_type);
			continue;
		}

		sec_print_err("load_data_to_os\n");
		ret = load_data_to_os(&session, img_type, img_info->partion_name,
				      img_info->image_addr, img_info->image_size);
		if (ret != SEC_OK) {
		 ...
		 * if success, unreset the SOC
		 */
		sec_print_err("verify_soc_image\n");
		ret = verify_soc_image(&session, img_type, img_info->image_addr,
				       SECBOOT_CMD_ID_VERIFY_DATA_TYPE);
		if (ret == SEC_OK) {
			sec_print_err("verify success!\n");
			break;
		}
		sec_print_err("vrl failed, vrl_type is 0x%x\n", vrl_type);
	}
err_out:
...
/* read vrl from /dev/block/by-name/vrl on init */
s32 read_vrl_dev(void)
{
	s32 ret = SEC_OK;

	mutex_lock(&g_copy_secimg_lock);
	if (g_read_vrl_issuccess == READ_VRL_SUCCESS) {
		sec_print_err("the vrl has been read successfully.\n");
		mutex_unlock(&g_copy_secimg_lock);
		return ret;
	}
	g_vrl_buffer = vmalloc(VRL_ALL_BUFLEN);
	if (!g_vrl_buffer) {
		sec_print_err("vrl vmalloc failed.\n");
		ret = SEC_ERROR;
		goto get_vrl_backup;
	}
	ret = bsp_read_bin(PART_VRL, 0, VRL_ALL_BUFLEN, g_vrl_buffer);
	if (ret != SEC_OK) {
		sec_print_err("failed to read the vrl: ret=0x%x\n", ret);
		vfree(g_vrl_buffer);
		g_vrl_buffer = NULL;
	}
get_vrl_backup:
	g_vrl_backup_buffer = vmalloc(VRL_ALL_BUFLEN);
	if (!g_vrl_backup_buffer) {
		sec_print_err("vrl_backup vmalloc failed.\n");
		ret = SEC_ERROR;
		goto err_out;
	}
	ret = bsp_read_bin(PART_VRL_BACKUP, 0, VRL_ALL_BUFLEN, g_vrl_backup_buffer);
	if (ret != SEC_OK) {
		vfree(g_vrl_backup_buffer);
		g_vrl_backup_buffer = NULL;
		sec_print_err("failed to read the vrl_backup: ret=0x%x\n", ret);
	}
err_out:
	if ((g_vrl_buffer != NULL) || (g_vrl_backup_buffer != NULL))
		g_read_vrl_issuccess = READ_VRL_SUCCESS;
	mutex_unlock(&g_copy_secimg_lock);
	return ret;
}
			  ...
			  u32 timeout_ms, u32 *retval)
{
	 ...
	 */
	retry = (s32)(msecs_to_jiffies(timeout_ms) / MAILBOX_MANUACK_TIMEOUT + 1);
	tx_buffer[1] = cmdtype;
