...
static const char *error_messages[PRMEM_ERR_NUMBERS] = {
	...
	[PRMEM_MEMCPY_ERR] = "Memcpy failed",
	[PRMEM_MEMSET_ERR] = "Memset failed",
};

inline void __prmem_err(enum prmem_errno errno, const char *filename, const char *func,
			       ...
			       pgprot_t set_mask, pgprot_t clear_mask)
{
	...
	unsigned long size = (unsigned long)(PAGE_SIZE * numpages);
...
/*
 * These symbols are exposed exclusively for *prmem* self testing and must
 * not be used elsewhere.
 * Users of prmem must rely solely on the prmem API, even for self-test.
 */
int debug_set_prmem_ro(unsigned long addr, int numpages)
{
	return set_prmem_ro(addr, numpages);
}

int debug_set_prmem_rw(unsigned long addr, int numpages)
{
	return set_prmem_rw(addr, numpages);
}
#endif
			       ...
			       unsigned long vstart, unsigned long vend)
{
	...
	if (unlikely((vend - align) < size)) {
		pr_err("[%s:%s %u] addr space smaller than %lu",
				       ...
				       unsigned long size, enum prmem_type type)
{
	...
	if (memset_s(va, sizeof(struct vmap_area), 0, sizeof(struct vmap_area)) != EOK) {
...
{
	enum prmem_pool_type type = (enum prmem_pool_type)prmem_get_type((void *)(uintptr_t)start, size);
...
/*
 * Ensure that the protect_range has been invoked before unprotect_range is called
 * The key to add this node->locked is that there are two interfaces for memory
 * application(pmalloc) and protection(prmem_protect_addr/pool), and only one interface
 * for memory release and unprotection(pfree).
 */
static void prmem_protect_range(struct prmem_node *node, const unsigned long start,
				const unsigned long size)
{
	...
	ret = protect_range(start, size);
	if (likely(ret == 0))
		wr_assign(node->locked, 1);
}

static void prmem_unprotect_range(struct prmem_node *node, const unsigned long start,
				  const unsigned long size)
{
	if (unlikely(node->locked == 0))
		return;

	unprotect_range(start, size);
	wr_assign(node->locked, 0);
}
				...
				struct prmem_node **list)
{
	...
	wr_twin(node).locked = 0;
	wr_ptr_sync_to_wr(node);
	if (memset_s((void *)(uintptr_t)node->start, node->size, 0, node->size) != EOK)
		prmem_err(PRMEM_MEMSET_ERR);
...
static inline bool is_pool_valid(struct prmem_pool *pool)
{
	...
	unsigned long size = (unsigned long)__roundup(sizeof(struct prmem_pool),
				       sizeof(void *));
...
static void pool_free_node(struct prmem_node **n)
{
	...
	prmem_unprotect_range(node, node->start, node->size);
		      ...
		      int alignment, enum prmem_flags flags)
{
	...
	/*
	 * The master lock must be locked before pool lock.
	      ...
	      enum prmem_flags flags)
{
	...
	if (pmalloc_is_start_wr(buf, 1)) {
		...
		if (strncpy_s(buf, len, s, len) != 0) {
			pr_err("prmem pstrdup strncpy err\n");
			return NULL;
		}
	}
	return buf;
...
unsigned long pmalloc_get_page_table_page(void)
{
	...
	if (memset_s(addr, PAGE_SIZE, 0, PAGE_SIZE) != EOK) {
		prmem_err(PRMEM_MEMSET_ERR);
		return (unsigned long)NULL;
	}
...
int pmalloc_put_page_table_page(unsigned long addr)
{
	...
	p = (struct page *)(uintptr_t)virt_to_page((uintptr_t)addr);
	if (unlikely(p == NULL)) {
		prmem_err(PRMEM_INVALID_ADDRESS);
		return -PRMEM_INVALID_ADDRESS;
	}
...
#define overlap(a, b, c, d) ((a <= b) && (c <= d))
static inline bool __ranges_overlap(const void *ptr, unsigned long n,
				    void *start, void *end)
{
	unsigned long s1 = (uintptr_t)ptr;
	unsigned long e1 = s1 + n;
	unsigned long s2 = (uintptr_t)start;
	unsigned long e2 = (uintptr_t)end;

	return !(overlap(s1, s2, e1, s2) || overlap(e2, s1, e2, e1) ||
		 overlap(e2, s1, e1, s1));
}

/**
 * check_prmem_object() - check if range overlaps prmem memory
 * @ptr: address of the range
 * @n: span of the range
 *
 * Returns: NULL for success, otherwise a string with an error message
 */
char *check_prmem_object(const void *ptr, unsigned long n)
{
	...
	if ((n + (uintptr_t)ptr) < (uintptr_t)ptr) {
		prmem_err(PRMEM_INVALID_RANGE);
		return "Invalid range";
	}

	if (__ranges_overlap(ptr, n, __start_data_prmem, __end_data_prmem) ||
	    __ranges_overlap(ptr, n, (void *)PRMEM_START, (void *)PRMEM_SIZE)) {
		prmem_err(PRMEM_OVERLAP_RANGE);
		return "Overlap with PRMEM data";
	}
	return NULL;
}
#endif /* CONFIG_HKIP_PRMEM_HARDENED_USERCOPY */

/* ----------------------------- init --------------------------------- */

void __init init_status(void)
{
#if defined(CONFIG_HKIP_PRMEM_BYPASS)
	prmem_status = PRMEM_BYPASSED;
	protected_status = PMALLOC_FAKE_BYPASSED;
#else
	if (!hhee_is_present()) {
 ...
 */
int __init prepare_mappings(struct prmem_phys_chunk *chunk,
			    unsigned long pfn)
{
	unsigned long i;
	int ret;
...
#else
	if (ret != (int)chunk->pages_per_chunk) {
		prmem_err(PRMEM_NO_PHYS);
		return -PRMEM_NO_PHYS;
	}
#endif
...
{
	if (memset_s(&aux_va, sizeof(aux_va), 0, sizeof(aux_va)) != EOK)
		prmem_err(PRMEM_MEMSET_ERR);
