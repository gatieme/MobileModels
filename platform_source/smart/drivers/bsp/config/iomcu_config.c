...
int iomcu_config_register(int id, int len, int attr)
{
	...
	if (g_iomcu_config->used + len >= g_iomcu_config_default) {
		pr_err("%s: not enough memory. id %x, used %x, len %x.\n", __func__, id, g_iomcu_config->used, len);
		mutex_unlock(&g_iomcu_config_mutex);
		return -EFAULT;
	}

	g_iomcu_config->node[id].offset = g_iomcu_config->used;
	g_iomcu_config->node[id].len = len;
	g_iomcu_config->used += iomcu_config_align(len);
...
void *iomcu_config_get(int id, int len)
{
	if (g_iomcu_config == NULL) {
		pr_err("%s: init failed id %x.\n", __func__, id);
		return ((void *)((char *)g_iomcu_config + g_iomcu_config_default));
	}
	...
	if (g_iomcu_config->node[id].offset == 0) {
		pr_err("%s: unregistered. %x.\n", __func__, id);
		return ((void *)((char *)g_iomcu_config + g_iomcu_config_default));
	}

	if (g_iomcu_config->node[id].offset + len >= g_iomcu_config_default) {
		pr_err("%s: invalid offset. id %x, offset %x, len %x.\n",
			__func__, id, g_iomcu_config->node[id].offset, len);
		return ((void *)((char *)g_iomcu_config + g_iomcu_config_default));
	}

	if (g_iomcu_config->node[id].len != len) {
		pr_err("%s: len mismatch. id %x, len %x-%x.\n",
...
static int iomcu_config_init(void)
{
	...
	g_iomcu_config->used = sizeof(struct iomcu_config);
