...
#define  BLANK_INVIAL  0x5A5AA5A5
static atomic_t g_blank  = ATOMIC_INIT(BLANK_INVIAL);
#endif
...
void sensorhub_recovey_status_to_mcu(void) {
	int blank = atomic_read(&g_blank);
	ctxhub_info("%s: blank: %u\n", __func__, blank);
	if ( blank != BLANK_INVIAL)
		tell_fold_status_to_mcu(blank);
}
#endif
