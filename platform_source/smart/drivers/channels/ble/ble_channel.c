...
#define BLE_MAX_DATA_REPORT_SIZE 4096
...
#define SENSORHUB_RECOVERY_DOING 1
#define SENSORHUB_RECOVERY_DONE 2
	...
	unsigned int count, unsigned char cmd_type)
{
	...
	if (count > BLE_MAX_DATA_REPORT_SIZE) {
...
static void  ble_service_recovery(int recover_flag, unsigned int response)
{
	pr_info("[%s]: notify HAL sensorhub recovery,recover_flag:%d, response:%d.\n", __func__, recover_flag, response);
	...
	g_ble_dev.sh_recover_flag = recover_flag;
		...
		unsigned long action, void *data)
{
	...
	case IOM3_RECOVERY_3RD_DOING:
		...
		ble_service_recovery(1, SENSORHUB_RECOVERY_DOING);
		break;
	case IOM3_RECOVERY_IDLE:
		pr_info("[%s]: sensorhub recovery is done.\n", __func__);
		ble_service_recovery(0, SENSORHUB_RECOVERY_DONE);
		break;
	default:
