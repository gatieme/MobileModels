...
static struct timespec g_ar_stat_last_timestamp = { 0 };
...
static int ar_state_v2_cmd(const struct ar_port_t *ar_port, unsigned long arg)
{
	...
	struct timespec cur_timestamp = { 0 };
	getnstimeofday(&cur_timestamp);
	long time_delta = (cur_timestamp.tv_sec - g_ar_stat_last_timestamp.tv_sec) * MILLISECONDS_PER_SECOND +
					 (cur_timestamp.tv_nsec - g_ar_stat_last_timestamp.tv_nsec) / NANOSECONDS_PER_MILLISECOND;

	if (time_delta >= 500 || time_delta < 0) {
		...
		ctxhub_info("hismart:[%s]:line[%d] send FLP_AR_GET_STATE_REQ, time_delta %ldms\n", __func__, __LINE__, time_delta);
		...
		if (time_delta > 0)
			memcpy_s(&g_ar_stat_last_timestamp, sizeof(struct timespec), &cur_timestamp, sizeof(struct timespec));
	}
...
static int __init ar_init(void)
{
	...
	getnstimeofday(&g_ar_stat_last_timestamp);
