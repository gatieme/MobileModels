...
#define IOMCU_APP_FLP       (FLP_BATCHING|FLP_GEOFENCE|FLP_CELLFENCE|FLP_CELLTRAJECTORY| \
			     FLP_WIFIFENCE|FLP_CELLBATCHING|FLP_DIAG)
...
struct flp_mmap_addr_t {
	unsigned int ddr_shmemext_addr_ap;
	unsigned int ddr_shmemext_size;
	unsigned int cell_mmap_size;
};

struct flp_device_t  g_flp_dev;
static unsigned int g_flp_shmem_transid;
struct flp_mmap_addr_t g_flp_mmap;

#if (defined CONFIG_SMART_FLP_GEOFENCE) || (defined CONFIG_SMART_FLP_WIFIFENCE) || \
	(defined CONFIG_SMART_FLP_CELLFENCE)
...
static void flp_resort_cellbatching_data(char *data, unsigned int len)
{
	...
	if (is_resort) {
		...
		ret = memcpy_s(buff + len - index * sizeof(struct cell_batching_data_t),
			index * sizeof(struct cell_batching_data_t), data, index * sizeof(struct cell_batching_data_t));
		if (ret != EOK) {
			pr_err("%s memcpy_s error\n", __func__);
			kfree(buff);
			return;
...
static void flp_send_data_to_uplayer(struct flp_port_t *flp_port, const char *msg, unsigned int len,
				unsigned int check_type, unsigned int cmd)
{
		...
		case SUB_CMD_FLP_RESET_RESP:
			if (IOMCU_APP_FLP & flp_port->channel_type) {
				...
				flp_generate_netlink_packet(flp_port, data,
					(unsigned int)sizeof(unsigned int), FLP_GENL_CMD_IOMCU_RESET);
			}
			break;
...
static void  flp_service_recovery(void)
{
	...
	list_for_each_safe(pos, tmp, &g_flp_dev.list) {//lint !e838
		...
		if (flp_port->channel_type & IOMCU_APP_FLP) {
			flp_port->channel_type &= ~IOMCU_APP_FLP;
			g_flp_dev.service_type &= ~IOMCU_APP_FLP;
			flp_port->dump_flag = 1;
			flp_generate_netlink_packet(flp_port, (char *)&response,
				(unsigned int)sizeof(unsigned int), FLP_GENL_CMD_IOMCU_RESET);
		}
	}
...
static int flp_common_inject_location(unsigned long arg)
{
	...
	return inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_INJECT_LOCATION_REQ,
			(char *)&inject_data, sizeof(struct iomcu_location), NULL);
}
				...
				unsigned short int shmem_cmd, unsigned int type)
{
	...
	if (ret == FLP_ERR_TIMEOUT && flp_port->dump_flag == 1)
		return ret;
...
static int flp_geofence_get_location(struct flp_port_t *flp_port, unsigned long arg)
{
	...
	inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
		SUB_CMD_FLP_GEOF_GET_LOCATION_REQ, NULL, (size_t)0, NULL);
	return 0;
}
...
static int __flp_location_ioctl(struct flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_BATCHING_STOP:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_STOP_BATCHING_REQ,
			(char *)&data, sizeof(int), NULL);
		flp_port->channel_type &= ~FLP_BATCHING;
		g_flp_dev.service_type &= ~FLP_BATCHING;
		break;
	case FLP_IOCTL_BATCHING_UPDATE:
		if (copy_from_user(&flp_port->gps_batching_config, (void *)((uintptr_t)arg),
			sizeof(struct batching_config_t))) {
			pr_err("%s copy_from_user error\n", __func__);
			return -EFAULT;
		}
	...
	case FLP_IOCTL_BATCHING_LASTLOCATION:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
			SUB_CMD_FLP_GET_BATCHED_LOCATION_REQ, (char *)&data, sizeof(int), NULL);
		break;
	case FLP_IOCTL_BATCHING_FLUSH:
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
			SUB_CMD_FLP_FLUSH_LOCATION_REQ, NULL, (size_t)0, NULL);
		break;
	...
	case FLP_IOCTL_BATCHING_MULT_LASTLOCATION:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
			SUB_CMD_FLP_BATCHING_MULT_LASTLOCATION_REQ, (char *)&data, sizeof(int), NULL);
		break;
	case FLP_IOCTL_BATCHING_MULT_FLUSH:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCHING_MULT_FLUSH_REQ,
			(char *)&data, sizeof(int), NULL);
		break;
...
static int flp_location_ioctl(struct flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_BATCHING_GET_SIZE:
		ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_GET_BATCH_SIZE_REQ,
			NULL, (size_t)0, &rd);
		if (ret == 0) {
...
static void flp_get_mmap_addr(void)
{
	(void)memset_s(&g_flp_mmap, sizeof(struct flp_mmap_addr_t), 0, sizeof(struct flp_mmap_addr_t));
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	g_flp_mmap.ddr_shmemext_addr_ap = dts_ddr_addr_get(DTSI_DDR_SHMEMEXT_ADDR_AP);
	g_flp_mmap.ddr_shmemext_size = dts_ddr_size_get(DTSI_DDR_SHMEMEXT_ADDR_AP);
	g_flp_mmap.cell_mmap_size = dts_ddr_size_get(DTSI_OFFLINE_CELL_MMAP_ADDR);

	if ((g_flp_mmap.ddr_shmemext_addr_ap && g_flp_mmap.ddr_shmemext_addr_ap && g_flp_mmap.cell_mmap_size) == 0) {
		pr_warn("[%s] flp ddr data not correct\n", __func__);
		(void)memset_s(&g_flp_mmap, sizeof(struct flp_mmap_addr_t), 0, sizeof(struct flp_mmap_addr_t));
	}
#else
#if defined(DDR_SHMEMEXT_ADDR_AP) && defined(DDR_SHMEMEXT_SIZE)
	g_flp_mmap.ddr_shmemext_addr_ap = DDR_SHMEMEXT_ADDR_AP;
	g_flp_mmap.ddr_shmemext_size = DDR_SHMEMEXT_SIZE;
	g_flp_mmap.cell_mmap_size = OFFLINE_CELL_MMAP_SIZE;
#endif
#endif
}
...
static unsigned long flp_mmap_page_set(unsigned long offset, unsigned long size)
{
	unsigned long page = 0;
	if (offset == OFFLINE_TYPE_CELL) {
		if (size > g_flp_mmap.cell_mmap_size || g_flp_mmap.cell_mmap_size > g_flp_mmap.ddr_shmemext_size) {
			pr_err("[%s]:line[%d] size.0x%lx.\n", __func__, __LINE__, size);
			return 0;
		}
		page = g_flp_mmap.ddr_shmemext_addr_ap;
	} else {
...
static int flp_mmap_impl(struct file *filp, struct vm_area_struct *vma)
{
	unsigned long size;
	int ret;
	struct flp_port_t *flp_port = NULL;
	...
	page = flp_mmap_page_set(offset, size);
	if (page == 0) {
		mutex_unlock(&g_flp_dev.lock);
		return -EINVAL;
	}
...
static int flp_mmap_stub(struct file *filp, struct vm_area_struct *vma)
{
...
{
	int ret;
	if (g_flp_mmap.ddr_shmemext_addr_ap == 0 || g_flp_mmap.ddr_shmemext_size == 0 || g_flp_mmap.cell_mmap_size == 0)
		ret = flp_mmap_stub(filp, vma);
	else
		ret = flp_mmap_impl(filp, vma);
	return ret;
}
