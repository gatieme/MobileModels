...
struct sdc_mem_info* sdc_buf_get()
{
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	if (!g_sdc_mem_info.phy_addr || !g_sdc_mem_info.size) {
...
static struct sdc_sensor_info_map *sdc_get_sensor_map(int tag)
{
	...
	if (!once_time) {
		for (i = 0; i < SDC_SENSOR_TYPE_MAX; i++) {
			g_sdc_sensor_map[i].size = dts_ddr_size_get(DTSI_DDR_SHMEM_SDC_ACC_ADDR_AP + i);
			g_sdc_sensor_map[i].address = dts_ddr_addr_get(DTSI_DDR_SHMEM_SDC_ACC_ADDR_AP + i);
			if (!g_sdc_sensor_map[i].size || !g_sdc_sensor_map[i].address)
