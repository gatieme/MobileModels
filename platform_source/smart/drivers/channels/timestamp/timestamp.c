...
static struct delayed_work g_timezone_dwork;
...
static void update_timestamp_by_ipc(void)
{
	struct write_info winfo;
	int ret;
	...
	unsigned long action, void *data)
{
	...
	default:
		pr_err("unknown---> lcd unknown cmd %d in %s\n", blank, __func__);
		break;
	}
...
STATIC int ctxhub_plt_probe(struct platform_device *pdev)
{
...
#else
	int ret = is_sensorhub_disabled(); // only inputhub20
#endif
	if (ret)
		return ret;
	...
	queue_delayed_work(system_freezable_power_efficient_wq, &g_timezone_dwork, TIMEZONE_DWORK_DELAYTIME);
#ifdef CONFIG_DKMD_DKSM
	dkmd_register_client(&ctx_fb_notify);
#else
