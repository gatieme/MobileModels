...
#endif
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
static unsigned int syscounter_l32 = 0;
static unsigned int syscounter_h32 = 0;
#else
static unsigned int syscounter_l32 = SOC_SYSCOUNTER_CNTCV_L32_NS_ADDR(0);
static unsigned int syscounter_h32 = SOC_SYSCOUNTER_CNTCV_H32_NS_ADDR(0);
#endif
...
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
static int get_soc_syscounter_reg_addr(u32 *syscounter_addr_l,u32 *syscounter_addr_h)
{
	int ret;
	struct device_node *ddr_node;
	ddr_node = of_find_node_by_name(NULL, "soc_syscounter_reg_addr");
	if (!ddr_node) {
		pr_err("%s, can not find node ddr_log_buff\n", __func__);
		return -1;
	}
	ret = of_property_read_u32_index(ddr_node, "reg", 0 ,syscounter_addr_l);
	if (ret) {
		pr_err("%s failed to get reg err! ret=%d\n", __func__, ret);
		of_node_put(ddr_node);
		return -1;
	}
	ret = of_property_read_u32_index(ddr_node, "reg", 1 , syscounter_addr_h);
	if (ret) {
		pr_err("%s failed to get reg err! ret=%d\n", __func__, ret);
		of_node_put(ddr_node);
		return -1;
...
u64 bsp_get_syscount(void)
{
	...
	unsigned long flags;
	int ret = 0;
	static unsigned int once_time = 0;
...
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	if (!once_time) {
		ret = get_soc_syscounter_reg_addr(&syscounter_l32, &syscounter_h32);
		if (ret) {
			spin_unlock_irqrestore(&dev->r_lock, flags);
			pr_err("%s:get syscounter addr failed!\n ", __func__);
			return 0;
		}
		once_time = 1;
	}
...
static ssize_t show_current_time(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count = snprintf_s(buf, (size_t)PAGE_SIZE, (size_t)PAGE_SIZE - 1,
			   "syscnt = %llu, arch_timer = %llu, tv.sec = %ld, tv.nsec = %ld, c_tv.sec = %ld, c_tv.nsec = %ld\n",
...
static ssize_t show_current_record(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (d == NULL) {
		...
		count = snprintf_s(buf, (size_t)PAGE_SIZE, (size_t)PAGE_SIZE - 1,
				   "syscnt = %llu, tv.sec = %ld, tv.nsec = %ld\n",
...
static ssize_t show_debug_syscount(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (d == NULL) {
		...
		count = snprintf_s(buf, (size_t)PAGE_SIZE, (size_t)PAGE_SIZE - 1,
				   "%s : syscnt = %llu, tv.sec = %ld, tv.nsec = %ld\n",
