...
{
	UINT32 index;
	int32_t is_mapped = MEM_BUF_UNMAPPED;
...
SINT32 vctrl_vdh_map_message_pool(
	mem_buffer_s *mem_map, int32_t share_fd, vcodec_bool is_vdh_all_buf_remap)
{
	int32_t ret;

	vdec_scene scene = mem_map->scene;

	if ((mem_map->is_mapped == MEM_BUF_MAPPED) && (is_vdh_all_buf_remap == VCODEC_TRUE)) {
		mem_map->is_map_virtual = MEM_BUF_MAPPED;
	...
	if (mem_map->is_mapped == MEM_BUF_UNMAPPED) {
		...
		vctrl_assert_ret((ret == 0), "share fd map failed");
		mem_map->is_mapped = MEM_BUF_MAPPED;
		mem_map->share_fd = (uint32_t)share_fd;
	}
	...
	mem_buffer_s *mem_map, scd_config_reg_s *ctrl_reg)
{
	...
	vctrl_assert_ret((mem_map != VCODEC_NULL), "mem_map parameter error");
	vctrl_assert_ret((ctrl_reg != VCODEC_NULL), "ctrl_reg parameter error");
...
static SINT32 vdec_mem_check_scene(mem_buffer_s *mem_map, int32_t share_fd)
{
	int32_t ret;
	vdec_scene scene;
	...
	mem_buffer_s *mem_map, omxvdh_reg_cfg_s *vdm_reg_cfg)
{
	int32_t ret;
	uint32_t index;
	int32_t *share_fd = &(vdm_reg_cfg->vdh_share_fd[0]);
	vcodec_bool is_vdh_frm_buf_remap = vdm_reg_cfg->is_frm_buf_remap;
	vcodec_bool is_vdh_pmv_buf_remap = vdm_reg_cfg->is_pmv_buf_remap;
	vcodec_bool is_vdh_all_buf_remap = vdm_reg_cfg->is_all_buf_remap;
	...
	for (index = 0; index < (VDH_SHAREFD_FRM_BUF
		...
		if (mem_map[index].is_mapped == MEM_BUF_UNMAPPED) {
			...
			mem_map[index].is_mapped = MEM_BUF_MAPPED;
			mem_map[index].share_fd = (uint32_t)share_fd[index];
			vdec_mem_put_map_info(&mem_map[index]);
		}
	}
	return VCTRL_OK;
...
vcodec_bool vctrl_scen_ident(uint32_t cmd)
{
	uint32_t value;
#ifdef SECURE_VS_NOR_SECURE
	read_vreg((int32_t)SCEN_IDENT, value, 0);
#else
	value = rd_scd_reg(SCEN_IDENT);
#endif
	if (value == (uint32_t)current->tgid)
		return VCODEC_TRUE;

	return VCODEC_FALSE;
}

int32_t vfmw_drv_mod_init(void)
{
	osal_init_interface();
...
#ifdef MODULE
	dprint(PRN_ALWS, "%s : Load vcodec_vfmw.ko (%d) success\n", __func__, VFMW_VERSION_NUM);
#endif
...
{
#ifdef MODULE
	dprint(PRN_ALWS, "%s : Unload vcodec_vfmw.ko (%d) success\n", __func__, VFMW_VERSION_NUM);
#endif
}
