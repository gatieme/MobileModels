...
static int32_t dec_dev_get_cur_isr_reg(dec_dev_info *dev, uint16_t *reg_id, uint32_t *instance_id)
{
	...
	reg->decode_time = (uint32_t)(OS_GET_TIME_US() - reg->reg_start_time);
	*reg_id = next_isr_reg;
...
static void dec_dev_insert_fake_int(uint32_t instance_id)
{
	dec_back_up back_up = {0};
	back_up.int_flag = 1;
	dec_dev_insert_backup_reglist(&back_up, instance_id);
	dprint(PRN_ALWS, "instance %u insert fake interrupt.", instance_id);
}

int32_t dec_dev_reset(dec_dev_info *dev, bool is_timeout)
{
	int32_t ret;
	unsigned long lock_flag;
	uint32_t reg_id;
	...
	for (reg_id = 0; (reg_id < reg_pool->reg_num) && (reg_id < VDH_REG_NUM); reg_id++) {
		reg = &reg_pool->vdh_reg[reg_id];
		if (reg->used && is_timeout)
			dec_dev_insert_fake_int(reg->instance_id);
...
{
	struct vfmw_global_info *glb_info = pdt_get_glb_info();
	uint32_t vdh_time_out = dec_dev_get_vdh_timeout();
...
static int32_t dec_dev_decode_status(dec_dev_info *dev, dec_back_up_list *back_up_list)
{
	...
	return OS_EVENT_WAIT(back_up_list->event, (uint64_t)vdh_time_out);
}
	...
	dec_dev_info *dev, dec_back_up_list *back_up_list)
{
	...
	while (dec_dev_reglist_get_back_up(back_up, back_up_list) != DEC_OK) {
		if (dec_dev_decode_status(dev, back_up_list) != OSAL_OK) {
			if (dev->reg_dump_enable != 0 || glb_info->is_fpga != 0) {
			...
			if (dec_dev_reset(dev, true) != DEC_OK)
				return DEC_ERR;
			/* query fake interrupt */
			if (dec_dev_reglist_get_back_up(back_up, back_up_list) != DEC_OK) {
				dprint(PRN_FATAL, "get fake interrupt faild.\n");
				return DEC_ERR;
			}
			return DEC_OK;
		}
	...
	if (back_up->err_flag != 0 && (dev->reg_dump_enable != 0 || glb_info->is_fpga != 0)) {
...
int32_t dec_dev_get_backup(uint32_t instance_id, void *read_backup)
{
	dec_dev_info *dev = VCODEC_NULL;
