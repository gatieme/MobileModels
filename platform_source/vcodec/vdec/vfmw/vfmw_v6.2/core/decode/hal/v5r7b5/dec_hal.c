...
static int32_t dec_hal_config_vdh(dec_dev_info *dev)
{
	...
	for (i = 0; i < vdh->vdh_reg_pool.reg_num; i++) {
		...
		vdh->vdh_reg_pool.vdh_reg[i].reg_id = (uint16_t)i;
...
static void dec_hal_write_reg(vdh_reg_info *reg, dec_dev_cfg *dec_vdh_cfg)
{
	...
	if (dev->poll_irq_enable)
		d32 |= wr_interrupt_mask_bit(0);
	d32 |= wr_interrupt_mask_bit(2);
	d32 |= wr_interrupt_mask_bit(3);
	wr_vreg(vir, VREG_OFS_PXP_INTR_MASK, d32); // bit 0:vdec over, bit 2:vdec part over, bit 3:dss2vdec int

	d32 = 0;
	if (dev->smmu_bypass == 0)
	...
	((vreg_info_state *)(&d32))->vdh_safe_flag = (reg->is_sec != 0) ? 1 : 0;
...
int32_t dec_hal_read_int_info(void *dec_dev, uint16_t reg_id)
{
	...
	if (dev->type == DEC_DEV_TYPE_MDMA) {
		...
		if (dev->back_up.err_flag != 0 || dev->perf_enable != 0)
			dprint(PRN_ALWS,
...
int32_t dec_hal_cancel_vdh(void *dec_dev, uint16_t reg_id)
{
	...
	for (i = 0; i < DEC_CANCEL_ACK_TIMEOUT; i++) {
		rd_vreg(reg->reg_vir_addr, ofs, d32);
		if (((d32 & BSP_CANCEL_TASK_OK_MASK) == BSP_CANCEL_TASK_OK_MASK) &&
			((d32 & PXP_CANCEL_TASK_OK_MASK) == PXP_CANCEL_TASK_OK_MASK)) {
