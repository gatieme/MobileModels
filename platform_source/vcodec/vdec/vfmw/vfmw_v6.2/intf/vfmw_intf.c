...
{
	/* NO_SEC_MODE kill mediacodec, will close vdec, multi reg need reset vdh. */
	if (VDH_REG_NUM > 1)
		vfmw_reset_vdh();
...
{
	int32_t ret;
	struct vdec_stm_dmabuf buf;

	vfmw_init_stm_dmabuf(&buf);
	ret = vfmw_stm_get_dmabuf(&buf, stm_cfg);
	if (ret)
		return ret;

	ret = vfmw_stm_param_check(fd, stm_cfg);
	if (ret)
		goto put_exit;
...
static int32_t vfmw_dec_process(void *fd, void *dev_cfg, void *vdh_state_reg)
{
	...
	vfmw_check_return(vfmw_insert_dmabuf_list(vdec_file->instance_id, dev_cfg) == VFMW_OK, VFMW_ERR);
	ret = vfmw_dec_param_check(dev_cfg);
	if (ret)
		goto error;

	if (VDH_REG_NUM == 1) {
		if (vfmw_reset_vdh() != VFMW_OK)
			goto error;
	}
...
error:
	vfmw_del_from_dmabuf_list(vdec_file->instance_id);
	return VFMW_ERR;
}
...
int32_t vfmw_query_image(struct file *file_handle, void *dev_cfg, void *read_backup)
{
	...
	ret = dec_dev_get_backup(vdec_file->instance_id, read_backup);
	vfmw_del_from_dmabuf_list(vdec_file->instance_id);
	return ret;
}
