...
static struct venc_file_private g_venc_file_private[MAX_OPEN_COUNT];
...
void venc_shared_fd_list_init(struct venc_fd_list *list)
{
	int i;
	for (i = 0; i < MAX_CHANNEL_NUM; ++i) {
		mutex_init(&list[i].lock);
		INIT_LIST_HEAD(&list[i].fdhead);
	}
}

void venc_shared_fd_list_deinit(struct venc_fd_list *list)
{
	int i;
	...
	for (i = 0; i < MAX_CHANNEL_NUM; ++i) {
		mutex_lock(&list[i].lock);
		list_for_each_entry_safe(pos, n, &list[i].fdhead, head) {
			VCODEC_ERR_VENC("deinit fd:%d,%d", pos->shared_fd, pos->refcnt);
			dma_buf_put(pos->dmabuf);
			list_del(&pos->head);
			vfree(pos);
		}
		mutex_unlock(&list[i].lock);
	}
}

int32_t venc_shared_fd_list_add(struct venc_fd_list *list, int32_t fd)
{
...
void venc_shared_fd_list_remove(struct venc_fd_list *list, int32_t fd)
{
	struct venc_fd_listnode *pos = NULL;
	struct venc_fd_listnode *n = NULL;
