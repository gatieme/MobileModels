...
#ifdef SUPPORT_SECURE_VENC
	if (encode_info->is_protected == 1 && g_secure_venc_initialized == false) {
		ret = init_kernel_ca();
		if (ret != 0) {
...
static int show_hardware_usage(char *buf, uint32_t size)
{
	...
	for (i = 0; i < MAX_SUPPORT_CORE_NUM && i < core_num; i++) {
		cur_len = pm_show_period_account(&venc->ctx[i].pm, buf + total_len, (int32_t)size - total_len);
		if (cur_len <= 0)
			return 0;
		total_len += cur_len;

		cur_len = pm_show_total_account(&venc->ctx[i].pm, buf + total_len, (int32_t)size - total_len);
		if (cur_len <= 0)
			return 0;
		total_len += cur_len;
	}
...
static int32_t venc_drv_device_create_file(venc_entry_t *venc)
{
	int32_t ret = 0;
#ifdef VENC_DEBUG_ENABLE
	ret = device_create_file(venc->venc_device, &dev_attr_venc_misc);
	if (ret) {
...
#ifdef SUPPORT_SWITCH_POWER_OFF_PER_FRAME
	ret = device_create_file(venc->venc_device, &dev_attr_venc_frame_power_off);
	if (ret) {
		VCODEC_ERR_VENC("%s, failed to create device file", __func__);
