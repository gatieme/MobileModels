...
struct venc_file_private {
	struct venc_fifo_buffer *encode_done_info;
	struct venc_fd_list fdlist[MAX_CHANNEL_NUM];
};
...
struct venc_mcu_entry {
	uint32_t *intr_hub_reg_base;
	struct venc_dma_mem mcu_ddr;
	void *image;
	uint32_t image_size;
};
