...
void vedu_hal_request_bus_idle(uint32_t *reg_base)
{
	...
	if (i == REQ_BUS_IDLE_TIME_OUT)
		VCODEC_FATAL_VENC("request bus idle timeout");
}

static void vedu_hal_cfg_reg_intraset(const struct encode_info *channel_cfg,
	uint32_t *reg_base)
{
...
static void vedu_hal_cfg_reg_lambda_set(const struct encode_info *channel_cfg,
	uint32_t *reg_base)
{
	uint32_t i;
...
static void vedu_hal_cfg_reg_qpg_map_set(const struct encode_info *channel_cfg,
	uint32_t *reg_base)
{
	vedu_hal_cfg(reg_base,
...
static void vedu_hal_cfg_reg_addr_set(struct encode_info *channel_cfg,
	uint32_t *reg_base)
{
	VCODEC_DBG_VENC("set Vedu Hal cfg reg addr");
	...
	// VCodecV500 36bit va low-----REC,REF0,REF1,PME0,PME1,NBI Relate ADDR
	// Stide---------REC--------------REC-------------REC------------------
	vedu_hal_cfg(reg_base,
	...
	// VCodecV500 36bit va high-----REC,REF0,REF1,PME0,PME1,NBI Relate ADDR
	// Stide----------REC---------------REC-------------REC----------------
	vedu_hal_cfg(reg_base,
...
static void vedu_hal_cfg_reg_slc_head_set(const struct encode_info *channel_cfg,
	uint32_t *reg_base)
{
	// ----------------Stream head info----------------------------
	vedu_hal_cfg(reg_base,
...
static void vedu_hal_cfgreg_rcset(const struct encode_info *channel_cfg, uint32_t *reg_base)
{
	vedu_hal_cfg(reg_base,
	...
	VCODEC_DBG_VENC("----qpg_qp_madi_switch_thr:%d----",
		channel_cfg->all_reg.VEDU_QPG_MADI_SWITCH_THR.bits.qpg_qp_madi_switch_thr);
	VCODEC_DBG_VENC("veduhal_cfgreg_rcset");
}
...
void vedu_hal_cfg_reg_simple(struct encode_info *channel_cfg, int32_t core_id)
{
	...
	vedu_hal_cfg(reg_base,
		...
		channel_cfg->all_reg.VEDU_VCPI_MULTISLC.u32);
	...
	vedu_hal_cfg_smmu(channel_cfg, (uint32_t)core_id);
	venc_hal_cfg_platform_diff(channel_cfg, reg_base);
}

void vedu_hal_cfg_reg(struct encode_info *channel_cfg, int32_t core_id)
{
	venc_entry_t *venc = platform_get_drvdata(venc_get_device());
	uint32_t vcpi_protocol =
		channel_cfg->all_reg.VEDU_VCPI_MODE.bits.vcpi_protocol;
	uint32_t *reg_base = venc->ctx[core_id].reg_base;
	if (reg_base == NULL) {
		VCODEC_FATAL_VENC("register base addr is null, core is %d",core_id);
		return;
	}
	...
	vedu_hal_cfg(reg_base,
		...
		channel_cfg->all_reg.VEDU_EMAR_SCRAMBLE_TYPE.u32);

#ifndef VENC_DPM_ENABLE
	channel_cfg->all_reg.VEDU_VCPI_LOW_POWER.bits.venc_dpm_apb_clk_en = 0;
#else
	channel_cfg->all_reg.VEDU_VCPI_LOW_POWER.bits.venc_dpm_apb_clk_en = 1;
#endif
	vedu_hal_cfg(reg_base,
		offsetof(S_HEVC_AVC_REGS_TYPE, VEDU_VCPI_LOW_POWER.u32),
		channel_cfg->all_reg.VEDU_VCPI_LOW_POWER.u32);
	...
	vedu_hal_cfg(reg_base,
		...
		channel_cfg->all_reg.VEDU_VCPI_OUTSTD.u32);
	...
	vedu_hal_cfg(reg_base,
		offsetof(S_HEVC_AVC_REGS_TYPE, VEDU_VCPI_SPMEM_CTRL_0), VEDU_VCPI_SPMEM_CTRL_0_VALUE);
	vedu_hal_cfg(reg_base,
		offsetof(S_HEVC_AVC_REGS_TYPE, VEDU_VCPI_SPMEM_CTRL_1.u32), VEDU_VCPI_SPMEM_CTRL_1_VALUE);
	vedu_hal_cfg(reg_base,
	...
	vedu_hal_cfg(reg_base,
		...
		channel_cfg->all_reg.VEDU_CURLD_GCFG.u32);
	...
	vedu_hal_cfg(reg_base,
		...
		channel_cfg->all_reg.VEDU_VCPI_I_SLC_INSERT.u32);
	...
	venc_hal_cfg_plat_diff_all_cfg(channel_cfg, reg_base);
