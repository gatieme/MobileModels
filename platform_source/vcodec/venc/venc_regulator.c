...
#endif
#ifdef VENC_ECC_ENABLE
		venc->ctx[i].irq_num.ecc_detect = irq_of_parse_and_map(np, (int32_t)(5 + number * i));
		if (venc->ctx[i].irq_num.ecc_detect == 0)
			VCODEC_FATAL_VENC("parse and map irq ecc_detect failed");

		venc->ctx[i].irq_num.ecc_multpl = irq_of_parse_and_map(np, (int32_t)(6 + number * i));
		if (venc->ctx[i].irq_num.ecc_multpl == 0)
			VCODEC_FATAL_VENC("parse and map irq ecc_multpl failed");
#endif
	}
	return 0;
}
