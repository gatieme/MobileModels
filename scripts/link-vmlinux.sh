...
kallsyms()
{
	...
	if [ -n "${CONFIG_HONOR_HIDESYMS}" ]; then
		local src_blacklist="$(cd `dirname $0`; pwd)/hn_hidesyms_blacklist.txt"
		local tmp_blacklist=".tmp_hidesyms_blacklist.txt"
		local debug_blacklist="debug_hidesyms_blacklist.log"
		...
		cat ${src_blacklist} | sed s/[[:space:]]//g | grep -v "#" | \
			sed -e '/^$/d'| sed 's/$/&.cfi_jt/g' > ${tmp_blacklist}
		cat ${src_blacklist} | sed s/[[:space:]]//g | grep -v "#" | \
			sed -e '/^$/d' >> ${tmp_blacklist}

		if [ -s "${tmp_blacklist}" ]; then
			# Generate debug log
			# if [ -n "${CONFIG_HONOR_HIDESYMS_DEBUGFS}" ]; then
