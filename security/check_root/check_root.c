 ...
 *
 * Copyright (c) 2001-2019, Honor Tech. Co., Ltd.
 *
...
#ifdef CONFIG_HN_KERNEL_STP_ENG_DEBUG
	pr_info_ratelimited("checkroot_risk_id counter:%d,%d,%d,%d.",\
						 checkroot_ref.setuid,\
						 checkroot_ref.setgid,\
						 checkroot_ref.setresuid,\
						 checkroot_ref.setresgid);
...
static int checkroot_readline(struct file *filp, char *buf, int len)
{
	...
	for (; read_len < len; buf++, read_len++) {
		ret = kernel_read(filp, buf, 1, &filp->f_pos);
		if (ret <= 0)
			break;
		if (*buf == '\n') {
			*buf = 0;
