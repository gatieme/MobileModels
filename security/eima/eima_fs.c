 ...
 * Description: the eima_fs.c for MAGIC Integrity Measurement Architecture(MAGIC)
...
static void eima_add_default_policy(void)
{
	...
	s_ret = memcpy_s(g_eima_pif.policy[0].usecase_name,
		EIMA_NAME_STRING_LEN + 1, _EIMA_POLICY_USECASE_NAME, strlen(_EIMA_POLICY_USECASE_NAME) + 1);
	if (s_ret != EOK) {
	...
	for (i = 0; (i < ARRAY_SIZE(g_measure_targets_list)) &&
		...
		s_ret = memcpy_s(g_eima_pif.policy[0].targets[i].file,
			EIMA_NAME_STRING_LEN + 1,
			g_measure_targets_list[i].file, strlen(g_measure_targets_list[i].file) + 1);
		if (s_ret != EOK) {
