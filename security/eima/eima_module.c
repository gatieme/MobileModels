...
static bool get_eima_hook(void)
{
	if (g_send_eima_func != NULL) {
		eima_error("g_send_eima_func ready");
		return true;
	}
	eima_error("g_send_eima_func = NULL");
	return false;
}
...
#ifdef CONFIG_HONOR_EIMA_GKI
	while (get_eima_hook() != true) {
		ssleep(EIMA_RETRY_TIME);
		i++;
		if (i > EIMA_MAX_WAIT_TIME) {
			eima_error("get_eima_hook timeout\n");
			return;
		}
	}
	ssleep(EIMA_DELAY_TIME);
#endif
