...
struct tpm_chip *TPM_ANY_NUM = NULL;
#endif
extern u16 g_alg_id;
...
static int eima_add_digest_entry(struct eima_template_entry *entry)
{
	...
	ret = memcpy_s(&qe->entry, sizeof(struct eima_template_entry),
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	ret = tpm_pcr_extend(TPM_ANY_NUM, pcr_idx, &d);
#else
	ret = tpm_pcr_extend(TPM_ANY_NUM, pcr_idx, hash);
#endif
	if (ret != 0)
		eima_error("Error Communicating to TPM chip, result: %d\n",
			ret);
...
int eima_add_template_entry(struct eima_template_entry *entry)
{
	...
	s_ret = memcpy_s(digest, EIMA_HASH_DIGEST_SIZE,
			entry->digest, EIMA_HASH_DIGEST_SIZE);
	if (s_ret != EOK) {
		eima_error("memcpy_s fail");
		return s_ret;
	}

	if (eima_lookup_digest_entry(digest, EIMA_HASH_DIGEST_SIZE)) {
		result = -EEXIST;
		goto out;
	}
	...
	tpmresult = eima_pcr_extend(digest, EIMA_HASH_DIGEST_SIZE,
				entry->type);
	/* wait mobile office to decide the next operation */
	if (tpmresult != 0) {
		eima_error("the result of eima pcr extend = %d", tpmresult);
		goto out;
	}
