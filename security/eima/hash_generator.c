 ...
 * Description: the hash_generator.c for MAGCI Integrity Measurement
 *     Architecture(EIMA)
 * Create: 2017-12-20
 */
...
struct hashgen_handle *hash_handle_init(const char *alg)
{
	...
	eima_debug("Hash generator - handle init - using algorithm '%s'\n",
		alg);
	...
	if (ret < 0) {
		eima_error("crypto_shash_init error: %d\n", ret);
		goto fn_error_free;
	}
