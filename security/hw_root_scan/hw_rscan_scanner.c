...
static bool kcode_modify_by_jump_label = false;
static bool g_tee_update_code = false;
#ifdef CONFIG_HN_ROOT_SCAN_ENG_DEBUG
static bool inject_tee_kcode = false;
static bool inject_tee_syscall = false;
#endif
...
static uint rscan_trigger_by_stp(char *upload_rootproc)
{
	...
	root_masks = rscan_dynamic(dynamic_ops, scan_result_buf, &scan_err_code, 0);
		...
		uint *error_code, int for_tee)
{
	...
	*error_code = rscan_dynamic_raw_unlock(op_mask, result);
	...
	RSLogDebug(TAG, "rscan_dynamic for_tee %d", for_tee);
	if ((op_mask & D_RSOPID_KCODE) && (g_rscan_skip_flag.skip_kcode == NOT_SKIP)) {
		if (memcmp(result->kcode, g_rscan_clean_scan_result.kcode,sizeof(result->kcode)) != 0) {
			if (!kcode_modify_by_jump_label || memcmp(result->kcode, g_rscan_orig_result.kcode,
					sizeof(result->kcode)) == 0) {
				bad_mask |= D_RSOPID_KCODE;
				RSLogDebug(TAG, "kernel code is abnormal");
			} else {
			memcpy(g_rscan_clean_scan_result.kcode, result->kcode, sizeof(result->kcode));
			memcpy(g_rscan_orig_result.kcode,
				g_rscan_clean_scan_result.kcode, sizeof(g_rscan_clean_scan_result.kcode));
			kcode_modify_by_jump_label = false;
			g_tee_update_code = true;
			RSLogDebug(TAG, "update the kcode baseline");
			}
		} else {
			RSLogDebug(TAG, "no need to update baseline");
		}
		if (for_tee == 1) {
			if (g_tee_update_code == true) {
				RSLogDebug(TAG, "update tee kcode");
				g_tee_update_code = false;
				result->kcode_status = 2;
			}
		}
	} else if (g_rscan_skip_flag.skip_kcode == SKIP) {
		result->kcode_status = 1;
		RSLogDebug(TAG, "SKIP KCODE");
	}

#ifdef CONFIG_HN_ROOT_SCAN_ENG_DEBUG
	if (inject_tee_kcode == true && result->kcode_status == 0) {
		RSLogError(TAG, "inject kcode");
		memset(result->kcode, 1, sizeof(result->kcode));
	}
#endif
	if ((op_mask & D_RSOPID_SYS_CALL) &&
...
#ifdef CONFIG_HN_ROOT_SCAN_ENG_DEBUG
	if (inject_tee_syscall == true) {
		RSLogError(TAG, "inject syscall");
		memset(result->syscalls, 1, sizeof(result->syscalls));
	}
#endif
	if ((op_mask & D_RSOPID_SE_HOOKS) &&
...
EXPORT_SYMBOL(rscan_dynamic_raw);
				...
				struct rscan_result_dynamic *result)
{
	...
	mutex_lock(&scanner_lock);
	ree_status = rscan_dynamic(op_mask, result, &error_code, 1);
	mutex_unlock(&scanner_lock);
...
EXPORT_SYMBOL(rscan_dynamic_raw_and_upload);
...
EXPORT_SYMBOL(rscan_get_status);
...
EXPORT_SYMBOL(load_rproc_whitelist);
...
void root_scan_kcode_modify(void)
{
	if (kcode_modify_by_jump_label) {
		return;
	}
	kcode_modify_by_jump_label = true;
	g_rscan_skip_flag.skip_kcode = SKIP;
	g_kcode_update++;
	RSLogDebug(TAG, "root_scan_kcode_modify by insn");
	return;
}

void root_scan_kcode_modify_finished(void)
{
	if (g_rscan_skip_flag.skip_kcode == NOT_SKIP) {
		return;
	}
	g_rscan_skip_flag.skip_kcode = NOT_SKIP;
	RSLogDebug(TAG, "root_scan_kcode_modify_finished");
	return;
}
...
int root_scan_pause(unsigned int op_mask, const void *reserved)
{
	...
	root_status = rscan_dynamic(dynamic_ops, scan_result_buf, &scan_err_code, 0);
	if (root_status == 0) {
