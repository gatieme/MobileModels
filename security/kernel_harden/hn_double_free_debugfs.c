...
static int __init hnslub_double_free_debugfs_init(void)
{
	struct dentry *debugfs_file = NULL;

	/* create sys/kernel/debug/hnslub_double_free for debugfs */
	slub_double_free_root = debugfs_create_dir("hnslub_double_free", NULL);
...
static void __exit hnslub_double_free_debugfs_exit(void)
{
	debugfs_remove(slub_double_free_root);
	slub_double_free_root = NULL;
}

module_init(hnslub_double_free_debugfs_init);
module_exit(hnslub_double_free_debugfs_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Honor double free debugfs");
