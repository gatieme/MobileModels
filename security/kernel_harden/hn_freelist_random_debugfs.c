/*
 * hn_freelist_random_debugfs.c
 *
 * Honor freelist random debugfs interface
 *
...
static int __init hn_freelist_random_debugfs_init(void)
{
	struct dentry *debug_file = NULL;

	/* create sys/kernel/debug/hnslub for debugfs */
	slub_freelist_root = debugfs_create_dir("hnslub", NULL);
...
static void __exit hn_freelist_random_debugfs_exit(void)
{
	debugfs_remove(slub_freelist_root);
	slub_freelist_root = NULL;
}

module_init(hn_freelist_random_debugfs_init);
module_exit(hn_freelist_random_debugfs_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Honor Freelist Random debugfs");
