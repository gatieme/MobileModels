/*
 * hn_harden_double_free_debugfs.c
 *
 * Honor double free check debugfs interface
 *
...
/* Test when the data in object equals magic number hn_random_free */
static int slub_user_area_test(struct kmem_cache *my_cachep)
{
	void *object = NULL;
	unsigned long *canary = NULL;
	unsigned long random_free = my_cachep->hn_random_free;
...
static int __init hnslub_debugfs_init(void)
{
	struct dentry *debugfs_file = NULL;

	/* create sys/kernel/debug/slub_harden_double_free for debugfs */
	slub_double_free_root = debugfs_create_dir("slub_harden_double_free",
						    NULL);
...
static void __exit hnslub_debugfs_exit(void)
{
	debugfs_remove(slub_double_free_root);
	slub_double_free_root = NULL;
}

module_init(hnslub_debugfs_init);
module_exit(hnslub_debugfs_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Honor harden double free debugfs");
