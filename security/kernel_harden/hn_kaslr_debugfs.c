...
static int __init hn_kaslr_debugfs_init(void)
{
#ifdef CONFIG_RANDOMIZE_BASE
...
static void __exit hn_kaslr_debugfs_exit(void)
{
	debugfs_remove(kaslr_root);
	kaslr_root = NULL;
}

module_init(hn_kaslr_debugfs_init);
module_exit(hn_kaslr_debugfs_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Honor KASLR debugfs");
