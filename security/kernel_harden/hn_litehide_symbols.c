/*
 * hn_litehide_symbols.c
 *
 * Honor Hidden kernel symbols feature
 *
...
static const char *hn_litehidesymbs_blacklist[MAX_HIDESYMS_NUM] = {
	"commit_creds",
	"prepare_kernel_cred"
};
...
#ifdef CONFIG_HONOR_HIDESYMS_DEBUGFS
				   ...
				   size_t cnt, loff_t *ppos)
{
	...
	while (cnt > write) {
...
#ifdef CONFIG_CFI_CLANG
		/* Add .cfi func support, only used by echo syms to debugfs
		 * /sys/kernel/debug/litehide_syms */
		...
		if (!find_sym_from_blacklist(node->sym))
			...
			litehide_sym_node_free(node);
#endif
#ifdef CONFIG_HIDESYMS_CFI_JT
		/* Add .cfi_jt func support, if CONFIG_CFI_CLANG is open,
		 * .cfi func will be added, node->sym will direct to .cfi func
		 * so we should add _jt symbol for Android R */
		memset(blacklist_cfi, 0, SYM_NAME_LEN);
		(void)strncpy(blacklist_cfi, node->sym, strlen(node->sym));
		(void)strncat(blacklist_cfi, "_jt", CFI_JT_FUNC_LEN - 1);
		node = litehide_sym_node_alloc();
		if (!node) {
			mutex_unlock(&hidesyms_lock);
			return -ENOMEM;
...
static __init int initialize_blacklist(void)
{
	...
	for (i = 0; i < MAX_HIDESYMS_NUM; i++) {
		if (!hn_litehidesymbs_blacklist[i])
			continue;
		node = litehide_sym_node_alloc();
		if (!node)
			return -ENOMEM;
		if (strlen(hn_litehidesymbs_blacklist[i]) >
			   SYM_NAME_LEN - CFI_FUNC_LEN) {
			litehide_sym_node_free(node);
			return -EINVAL;
		}
		(void)strncpy(node->sym, hn_litehidesymbs_blacklist[i],
			      strlen(hn_litehidesymbs_blacklist[i]));
		list_add_tail(&node->list, &hidesyms_blacklist);
#ifdef CONFIG_CFI_CLANG
		node = litehide_sym_node_alloc();
                if (!node)
		...
		memset(blacklist_cfi, 0, SYM_NAME_LEN);
		(void)strncpy(blacklist_cfi, hn_litehidesymbs_blacklist[i],
			      strlen(hn_litehidesymbs_blacklist[i]));
		(void)strncat(blacklist_cfi, ".cfi", CFI_FUNC_LEN - 1);
		(void)strncpy(node->sym, blacklist_cfi, strlen(blacklist_cfi));
		list_add_tail(&node->list, &hidesyms_blacklist);
		...
		node = litehide_sym_node_alloc();
                if (!node)
			return -ENOMEM;
                /* Add .cfi_jt asm func support */
		memset(blacklist_cfi, 0, SYM_NAME_LEN);
		(void)strncpy(blacklist_cfi, hn_litehidesymbs_blacklist[i],
			      strlen(hn_litehidesymbs_blacklist[i]));
		(void)strncat(blacklist_cfi, ".cfi_jt", CFI_JT_FUNC_LEN - 1);
		(void)strncpy(node->sym, blacklist_cfi, strlen(blacklist_cfi));
		list_add_tail(&node->list, &hidesyms_blacklist);
#endif
...
static int __init hn_litehide_symbols_init(void)
{
#ifdef CONFIG_HONOR_HIDESYMS_DEBUGFS
...
static void __exit hn_litehide_symbols_exit(void)
{
#ifdef CONFIG_HONOR_HIDESYMS_DEBUGFS
...
module_init(hn_litehide_symbols_init);
module_exit(hn_litehide_symbols_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Honor hide symbols module");
