 ...
 *
 * Copyright (c) 2012-2019 Honor Technologies Co., Ltd.
 *
...
static long kernel_harden_test_compat_ioctl(struct tty_struct *tty,
...
static const struct tty_operations g_kharden_ops = {
	...
	.compat_ioctl = kernel_harden_test_compat_ioctl,
};
...
static int __init kernel_harden_test_init(void)
{
	...
	g_kharden_tty_drv = tty_alloc_driver(KH_TTY_MINORS, TTY_DRIVER_REAL_RAW);
	if (IS_ERR(g_kharden_tty_drv)) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
		tty_driver_kref_put(g_kharden_tty_drv);
#else
