...
extern struct prmem_pool selinux_prot_pool;
...
struct selinux_state {
	...
	bool checkreqprot;
#ifdef CONFIG_HKIP_SELINUX_PROT
	/* selinux_prot protected by prmem */
	struct selinux_prot *prot;
#else
	bool initialized;
#endif
...
{
	if (WARN(!prmem_in_range(state->prot, sizeof(struct selinux_prot),
		PRMEM_RO_NO_RECL), "Attempt to alter selinux_prot!"))
		return 0;
	/* do a synchronized load to avoid race conditions */
	return smp_load_acquire(&state->prot->initialized);
}
...
static inline void selinux_mark_initialized(struct selinux_state *state)
{
	...
	smp_store_release(&state->prot->initialized, true);
	prmem_protect_pool(&selinux_prot_pool);
}
#else
static inline bool selinux_initialized(const struct selinux_state *state)
{
